...
static void
capi_addlasterror(void);
static void
capi_adderror(DWORD err);

static void
CAPI_trace(CAPI_CTX* ctx, char* format, ...);

static int
capi_list_providers(CAPI_CTX* ctx, BIO* out);
static int
capi_list_containers(CAPI_CTX* ctx, BIO* out);
int
capi_list_certs(CAPI_CTX* ctx, BIO* out, char* storename);
void
capi_free_key(CAPI_KEY* key);

static PCCERT_CONTEXT
capi_find_cert(CAPI_CTX* ctx, const char* id, HCERTSTORE hstore);

CAPI_KEY*
capi_find_key(CAPI_CTX* ctx, const char* id);

static EVP_PKEY*
capi_load_privkey(ENGINE* eng,
                  const char* key_id,
                  UI_METHOD* ui_method,
                  void* callback_data);
static int
capi_rsa_sign(int dtype,
              ...
              const RSA* rsa);
static int
capi_rsa_priv_enc(int flen,
                  const unsigned char* from,
                  unsigned char* to,
                  RSA* rsa,
                  int padding);
static int
capi_rsa_priv_dec(int flen,
                  const unsigned char* from,
                  unsigned char* to,
                  RSA* rsa,
                  int padding);
static int
capi_rsa_free(RSA* rsa);

#ifndef OPENSSL_NO_DSA
static DSA_SIG*
capi_dsa_do_sign(const unsigned char* digest, int dlen, DSA* dsa);
static int
capi_dsa_free(DSA* dsa);
#endif

static int
capi_load_ssl_client_cert(ENGINE* e,
                          SSL* ssl,
...
static int
cert_select_simple(ENGINE* e, SSL* ssl, STACK_OF(X509)* certs);
#ifdef OPENSSL_CAPIENG_DIALOG
static int
cert_select_dialog(ENGINE* e, SSL* ssl, STACK_OF(X509)* certs);
#endif

void
engine_load_capi_int(void);

typedef PCCERT_CONTEXT(
  WINAPI* CERTDLG)(HCERTSTORE, HWND, LPCWSTR, LPCWSTR, DWORD, DWORD, void*);
...
capi_ctx_new(void);
static void
capi_ctx_free(CAPI_CTX* ctx);
static int
capi_ctx_set_provname(CAPI_CTX* ctx, LPSTR pname, DWORD type, int check);
static int
capi_ctx_set_provname_idx(CAPI_CTX* ctx, int idx);
...
static int
capi_ctrl(ENGINE* e, int cmd, long i, void* p, void (*f)(void))
{
  int ret = 1;
  CAPI_CTX* ctx;
  BIO* out;
...
capi_init(ENGINE* e)
{
  ...
  if (CryptAcquireContextW(
        &hprov, NULL, rsa_aes_cspname, PROV_RSA_AES, CRYPT_VERIFYCONTEXT)) {
    use_aes_csp = 1;
...
static int
lend_tobn(BIGNUM* bn, unsigned char* bin, int binlen)
{
  int i;
  /*
...
static EVP_PKEY*
capi_get_pkey(ENGINE* eng, CAPI_KEY* key)
{
...
static void
capi_vtrace(CAPI_CTX* ctx, int level, char* format, va_list argptr)
{
  BIO* out;
...
static int
capi_get_provname(CAPI_CTX* ctx, LPSTR* pname, DWORD* ptype, DWORD idx)
{
  DWORD len, err;
  LPTSTR name;
...
capi_get_prov_info(CAPI_CTX* ctx, PCCERT_CONTEXT cert)
{
  DWORD len;
...
static void
capi_dump_prov_info(CAPI_CTX* ctx, BIO* out, CRYPT_KEY_PROV_INFO* pinfo)
{
  ...
  BIO_printf(out,
             ...
             pinfo->dwProvType);
  BIO_printf(
    out, "    Container Name: %s, Key Type %lu\n", contname, pinfo->dwKeySpec);
err:
  OPENSSL_free(provname);
...
static char*
capi_cert_get_fname(CAPI_CTX* ctx, PCCERT_CONTEXT cert)
{
  LPWSTR wfname;
  DWORD dlen;
...
static void
capi_dump_cert(CAPI_CTX* ctx, BIO* out, PCCERT_CONTEXT cert)
{
  X509* x;
...
static HCERTSTORE
capi_open_store(CAPI_CTX* ctx, char* storename)
{
  HCERTSTORE hstore;
...
int
capi_list_certs(CAPI_CTX* ctx, BIO* out, char* id)
{
  char* storename;
  int idx;
  int ret = 1;
  ...
  switch (ctx->lookup_method) {
    case CAPI_LU_SUBSTR:
      return CertFindCertificateInStore(
        hstore, X509_ASN_ENCODING, 0, CERT_FIND_SUBJECT_STR_A, id, NULL);
    case CAPI_LU_FNAME:
      for (;;) {
        cert = CertEnumCertificatesInStore(hstore, cert);
        if (!cert)
...
static CAPI_KEY*
capi_get_key(CAPI_CTX* ctx,
             const WCHAR* contname,
             const WCHAR* provname,
             DWORD ptype,
             DWORD keyspec)
{
  DWORD dwFlags = 0;
...
static CAPI_KEY*
capi_get_cert_key(CAPI_CTX* ctx, PCCERT_CONTEXT cert)
{
  CAPI_KEY* key = NULL;
...
static int
cert_issuer_match(STACK_OF(X509_NAME)* ca_dn, X509* x)
{
  int i;
  X509_NAME* nm;
...
cert_select_dialog(ENGINE* e, SSL* ssl, STACK_OF(X509)* certs)
{
  ...
  cert = ctx->certselectdlg(
    dstore, hwnd, dlg_title, dlg_prompt, dlg_columns, 0, NULL);
