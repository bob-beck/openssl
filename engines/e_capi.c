...
static EVP_PKEY* capi_load_privkey(ENGINE* eng,
                                   const char* key_id,
                                   UI_METHOD* ui_method,
                                   void* callback_data);
static int capi_rsa_sign(int dtype,
                         ...
                         unsigned int* siglen,
                         const RSA* rsa);
static int capi_rsa_priv_enc(int flen,
                             const unsigned char* from,
                             unsigned char* to,
                             RSA* rsa,
                             int padding);
static int capi_rsa_priv_dec(int flen,
                             const unsigned char* from,
                             unsigned char* to,
                             RSA* rsa,
                             int padding);
...
static DSA_SIG* capi_dsa_do_sign(const unsigned char* digest,
                                 int dlen,
                                 DSA* dsa);
...
static int capi_ctx_set_provname(CAPI_CTX* ctx,
                                 LPSTR pname,
                                 DWORD type,
                                 int check);
...
int capi_rsa_priv_enc(int flen,
                      const unsigned char* from,
                      unsigned char* to,
                      RSA* rsa,
                      int padding) {
...
int capi_rsa_priv_dec(int flen,
                      const unsigned char* from,
                      unsigned char* to,
                      RSA* rsa,
                      int padding) {
...
static void capi_vtrace(CAPI_CTX* ctx,
                        int level,
                        char* format,
                        va_list argptr) {
  BIO* out;
...
static int capi_get_provname(CAPI_CTX* ctx,
                             LPSTR* pname,
                             DWORD* ptype,
                             DWORD idx) {
  DWORD len, err;
...
static void capi_dump_prov_info(CAPI_CTX* ctx,
                                BIO* out,
                                CRYPT_KEY_PROV_INFO* pinfo) {
...
static CAPI_KEY* capi_get_key(CAPI_CTX* ctx,
