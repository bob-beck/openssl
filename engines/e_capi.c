...
static EVP_PKEY *
capi_load_privkey (ENGINE *eng, const char *key_id, UI_METHOD *ui_method,
                   void *callback_data)
{
  CAPI_CTX *ctx;
  CAPI_KEY *key;
...
int
capi_rsa_priv_enc (int flen, const unsigned char *from, unsigned char *to,
                   RSA *rsa, int padding)
{
...
int
capi_rsa_sign (int dtype, const unsigned char *m, unsigned int m_len,
...
int
capi_rsa_priv_dec (int flen, const unsigned char *from, unsigned char *to,
                   RSA *rsa, int padding)
{
  int i;
...
static CAPI_KEY *
capi_get_key (CAPI_CTX *ctx, const WCHAR *contname, const WCHAR *provname,
              DWORD ptype, DWORD keyspec)
{
  DWORD dwFlags = 0;
...
static int
capi_load_ssl_client_cert (ENGINE *e, SSL *ssl, STACK_OF (X509_NAME) *ca_dn,
                           X509 **pcert, EVP_PKEY **pkey,
