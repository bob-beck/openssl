...
static int bind_capi(ENGINE* e) {
  ...
  if (capi_rsa_method == NULL) return 0;
#ifndef OPENSSL_NO_DSA
  capi_dsa_method = DSA_meth_new("CryptoAPI DSA method", 0);
  if (capi_dsa_method == NULL) goto memerr;
#endif
...
static int bind_helper(ENGINE* e, const char* id) {
  if (id && (strcmp(id, engine_capi_id) != 0)) return 0;
...
static int lend_tobn(BIGNUM* bn, unsigned char* bin, int binlen) {
  ...
  if (!BN_bin2bn(bin, binlen, bn)) return 0;
  return 1;
}
...
int capi_rsa_priv_dec(int flen, const unsigned char* from, unsigned char* to,
  ...
  if ((tmpbuf = OPENSSL_malloc(flen)) == NULL) return -1;
...
static void capi_vtrace(CAPI_CTX* ctx, int level, char* format,
  ...
  if (!ctx || (ctx->debug_level < level) || (!ctx->debug_file)) return;
...
static int capi_get_provname(CAPI_CTX* ctx, LPSTR* pname, DWORD* ptype,
  ...
  if (!CryptEnumProviders(idx, NULL, 0, ptype, NULL, &len)) {
    err = GetLastError();
    if (err == ERROR_NO_MORE_ITEMS) return 2;
...
static int capi_list_containers(CAPI_CTX* ctx, BIO* out) {
  ...
  for (idx = 0;; idx++) {
    ...
    if (!CryptGetProvParam(hprov, PP_ENUMCONTAINERS, (BYTE*)cname, &clen,
                           flags)) {
      err = GetLastError();
      if (err == ERROR_NO_MORE_ITEMS) goto done;
      CAPIerr(CAPI_F_CAPI_LIST_CONTAINERS, CAPI_R_ENUMCONTAINERS_ERROR);
      capi_adderror(err);
      goto err;
    }
...
static void capi_dump_prov_info(CAPI_CTX* ctx, BIO* out,
  ...
  if (provname == NULL || contname == NULL) goto err;
...
static void capi_dump_cert(CAPI_CTX* ctx, BIO* out, PCCERT_CONTEXT cert) {
  ...
  if (!x) BIO_printf(out, "  <Can't parse certificate>\n");
  ...
  if (flags & CAPI_DMP_FULL) X509_print_ex(out, x, XN_FLAG_ONELINE, 0);
  ...
  if (flags & CAPI_DMP_PEM) PEM_write_bio_X509(out, x);
  X509_free(x);
}
...
static HCERTSTORE capi_open_store(CAPI_CTX* ctx, char* storename) {
  ...
  if (!storename) storename = ctx->storename;
...
void capi_free_key(CAPI_KEY* key) {
  ...
  CryptReleaseContext(key->hprov, 0);
  if (key->pcert) CertFreeCertificateContext(key->pcert);
  OPENSSL_free(key);
}
...
static int capi_ctx_set_provname_idx(CAPI_CTX* ctx, int idx) {
  ...
  DWORD type;
  int res;
  if (capi_get_provname(ctx, &pname, &type, idx) != 1) return 0;
...
static int cert_issuer_match(STACK_OF(X509_NAME)* ca_dn, X509* x) {
  ...
  if (sk_X509_NAME_num(ca_dn) <= 0) return 1;
  for (i = 0; i < sk_X509_NAME_num(ca_dn); i++) {
    nm = sk_X509_NAME_value(ca_dn, i);
    if (!X509_NAME_cmp(nm, X509_get_issuer_name(x))) return 1;
  }
  return 0;
}
...
static int cert_select_dialog(ENGINE* e, SSL* ssl, STACK_OF(X509)* certs) {
  ...
  HWND hwnd;
  int i, idx = -1;
  if (sk_X509_num(certs) == 1) return 0;
  ctx = ENGINE_get_ex_data(e, capi_idx);
  ...
  if (!hwnd && ctx->getconswindow) hwnd = ctx->getconswindow();
...
err:
  if (dstore) CertCloseStore(dstore, 0);
  return idx;
}
#endif
