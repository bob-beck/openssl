...
static char*
file_get_pass(const UI_METHOD* ui_method,
              char* pass,
              size_t maxsize,
              const char* desc,
              const char* info,
              void* data)
{
  ...
  if ((prompt = UI_construct_prompt(ui, desc, info)) == NULL) {
    ATTICerr(0, ERR_R_UI_LIB);
    pass = NULL;
  } else if (UI_add_input_string(ui,
                                 prompt,
...
static int
file_fill_pem_pass_data(struct pem_pass_data* pass_data,
                        const char* desc,
                        const char* info,
                        const UI_METHOD* ui_method,
                        void* ui_data)
...
static int
file_get_pem_pass(char* buf, int num, int w, void* data)
{
  struct pem_pass_data* pass_data = data;
  char* pass = file_get_pass(pass_data->ui_method,
                             buf,
                             num,
                             pass_data->prompt_desc,
                             pass_data->prompt_info,
                             pass_data->data);
 ...
 */
static int
check_suffix(const char* str, const char* suffix)
{
...
/* Helper functions */
static struct embedded_st*
get0_EMBEDDED(OSSL_STORE_INFO* info)
{
  return OSSL_STORE_INFO_get0_data(STORE_INFO_EMBEDDED, info);
}

static void
store_info_free(OSSL_STORE_INFO* info)
{
...
new_EMBEDDED(const char* new_pem_name, BUF_MEM* embedded)
{
    ...
    if ((p12 = d2i_PKCS12(NULL, &blob, (long)len)) != NULL) {
      ...
      *matchcount = 1;
      ...
      if (!PKCS12_mac_present(p12) || PKCS12_verify_mac(p12, "", 0) ||
        ...
        if ((pass = file_get_pass(
               ui_method, tpass, PEM_BUFSIZE, "PKCS12 import", uri, ui_data)) ==
            NULL) {
                          ...
                          const char* propq)
{
  ...
  *matchcount = 1;
  ...
  if ((pass = file_get_pass(ui_method,
                            kbuf,
                            PEM_BUFSIZE,
                            "PKCS8 decrypt pass phrase",
                            uri,
                            ui_data)) == NULL) {
                      ...
                      const char* propq)
{
  ...
  if (pem_name != NULL) {
    if (strcmp(pem_name, PEM_STRING_PKCS8INF) == 0) {
      ...
      *matchcount = 1;
      ...
      if ((slen = check_suffix(pem_name, "PRIVATE KEY")) > 0 &&
        ...
        *matchcount = 1;
        pkey =
          d2i_PrivateKey_ex(pkey_id, NULL, &blob, (long)len, libctx, propq);
      }
    }
  } else {
    int i;
    ...
    for (i = 0; i < EVP_PKEY_asn1_get_count(); i++) {
      ...
      if (!EVP_PKEY_asn1_get0_info(
            &pkey_id, NULL, &pkey_flags, NULL, NULL, ameth) ||
          (pkey_flags & ASN1_PKEY_ALIAS) != 0)
        continue;

      ERR_set_mark(); /* prevent flooding error queue */
      tmp_pkey =
        d2i_PrivateKey_ex(pkey_id, NULL, &tmp_blob, (long)len, libctx, propq);
      if (tmp_pkey != NULL) {
        if (pkey != NULL)
          EVP_PKEY_free(tmp_pkey);
        else
...
static int
file_find_type(OSSL_STORE_LOADER_CTX* ctx)
{
  BIO* buff = NULL;
  char peekbuf[4096] = {
    0,
...
file_open_ex(const OSSL_STORE_LOADER* loader,
             const char* uri,
             OSSL_LIB_CTX* libctx,
             const char* propq,
             const UI_METHOD* ui_method,
...
static OSSL_STORE_LOADER_CTX*
file_open(const OSSL_STORE_LOADER* loader,
          const char* uri,
          const UI_METHOD* ui_method,
          void* ui_data)
{
...
static OSSL_STORE_LOADER_CTX*
file_attach(const OSSL_STORE_LOADER* loader,
            BIO* bp,
            OSSL_LIB_CTX* libctx,
            const char* propq,
            const UI_METHOD* ui_method,
...
static int
file_ctrl(OSSL_STORE_LOADER_CTX* ctx, int cmd, va_list args)
{
  int ret = 1;
...
static int
file_expect(OSSL_STORE_LOADER_CTX* ctx, int expected)
{
...
static int
file_find(OSSL_STORE_LOADER_CTX* ctx, const OSSL_STORE_SEARCH* search)
{
  /*
  ...
  if (OSSL_STORE_SEARCH_get_type(search) == OSSL_STORE_SEARCH_BY_NAME) {
    ...
    BIO_snprintf(
      ctx->_.dir.search_name, sizeof(ctx->_.dir.search_name), "%08lx", hash);
    return 1;
  }
...
static OSSL_STORE_INFO*
file_load_try_decode(OSSL_STORE_LOADER_CTX* ctx,
                     const char* pem_name,
                     const char* pem_header,
                     unsigned char* data,
                     size_t len,
...
static OSSL_STORE_INFO*
file_load_try_repeat(OSSL_STORE_LOADER_CTX* ctx,
                     const UI_METHOD* ui_method,
                     void* ui_data)
{
  OSSL_STORE_INFO* result = NULL;
...
static void
pem_free_flag(void* pem_data, int secure, size_t num)
{
  if (secure)
...
file_read_pem(BIO* bp,
              ...
              int secure)
{
  int i = secure ? PEM_read_bio_ex(bp,
                                   pem_name,
                                   pem_header,
                                   data,
                                   len,
  ...
  if (strlen(*pem_header) > 10) {
    ...
    if (!PEM_get_EVP_CIPHER_INFO(*pem_header, &cipher) ||
        !file_fill_pem_pass_data(
          &pass_data, "PEM pass phrase", uri, ui_method, ui_data) ||
...
static OSSL_STORE_INFO*
file_try_read_msblob(BIO* bp, int* matchcount)
{
  OSSL_STORE_INFO* result = NULL;
  int ispub = -1;

  {
    ...
    if (ossl_do_blob_header(
          &p, sizeof(peekbuf), &magic, &bitlen, &isdss, &ispub) <= 0)
      return 0;
  }
                  ...
                  int* matchcount)
{
...

  {
    ...
    if (!file_fill_pem_pass_data(
          &pass_data, "PVK pass phrase", uri, ui_method, ui_data) ||
...
static int
file_read_asn1(BIO* bp, unsigned char** data, long* len)
{
  BUF_MEM* mem = NULL;
...
static int
file_name_to_uri(OSSL_STORE_LOADER_CTX* ctx, const char* name, char** data)
{
  assert(name != NULL);
  assert(data != NULL);
  {
    ...
    long calculated_length =
      (long)(strlen(ctx->uri) + strlen(pathsep) + strlen(name) + 1 /* \0 */);
...
static int
file_name_check(OSSL_STORE_LOADER_CTX* ctx, const char* name)
{
  const char* p = NULL;
...
static int
file_eof(OSSL_STORE_LOADER_CTX* ctx);
static int
file_error(OSSL_STORE_LOADER_CTX* ctx);
static OSSL_STORE_INFO*
file_load(OSSL_STORE_LOADER_CTX* ctx, const UI_METHOD* ui_method, void* ui_data)
{
  OSSL_STORE_INFO* result = NULL;
  ...
  again:
    ...
    do {
      ...
      if (ctx->type == is_pem) {
        ...
        if ((result = file_try_read_msblob(ctx->_.file.file, &matchcount)) !=
            ...
            (result = file_try_read_PVK(
               ctx->_.file.file, ui_method, ui_data, ctx->uri, &matchcount)) !=
              NULL)
          goto endloop;
      ...
      result = file_load_try_decode(
        ctx, pem_name, pem_header, data, len, ui_method, ui_data, &matchcount);
...
static int
file_close(OSSL_STORE_LOADER_CTX* ctx)
{
...
static int
loader_attic_init(ENGINE* e)
{
  return 1;
}

static int
loader_attic_finish(ENGINE* e)
{
  return 1;
}

static int
loader_attic_destroy(ENGINE* e)
{
