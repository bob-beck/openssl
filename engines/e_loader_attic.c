...
static char* file_get_pass(const UI_METHOD* ui_method, char* pass,
  ...
  if (ui_method != NULL) UI_set_method(ui, ui_method);
...
static int check_suffix(const char* str, const char* suffix) {
  ...
  if (suffix_len >= str_len) return -1;
  p = str + str_len - suffix_len;
  if (*p != ' ' || strcmp(p + 1, suffix) != 0) return -1;
  return (int)(p - str);
}
...
static OSSL_STORE_INFO* new_EMBEDDED(const char* new_pem_name,
  ...
  if ((data = OPENSSL_zalloc(sizeof(*data))) == NULL) return NULL;
 ...
 */
static OSSL_STORE_INFO* try_decode_PKCS12(
    const char* pem_name, const char* pem_header, const unsigned char* blob,
...
static OSSL_STORE_INFO* try_decode_PKCS8Encrypted(
  ...
  if (pem_name != NULL) {
    if (strcmp(pem_name, PEM_STRING_PKCS8) != 0) return NULL;
    *matchcount = 1;
  }
  ...
  *matchcount = 1;
  ...
  if (!PKCS12_pbe_crypt(dalg, pass, (int)strlen(pass), doct->data, doct->length,
...
static OSSL_STORE_INFO* try_decode_PrivateKey(
  ...
  if (pem_name != NULL) {
    if (strcmp(pem_name, PEM_STRING_PKCS8INF) == 0) {
      ...
      *matchcount = 1;
      if (p8inf != NULL) pkey = EVP_PKCS82PKEY_ex(p8inf, libctx, propq);
  ...
  if (pkey == NULL) /* No match */
    return NULL;

  store_info = OSSL_STORE_INFO_new_PKEY(pkey);
  if (store_info == NULL) EVP_PKEY_free(pkey);
 ...
 */
static OSSL_STORE_INFO* try_decode_PUBKEY(
    const char* pem_name, const char* pem_header, const unsigned char* blob,
    size_t len, void** pctx, int* matchcount, const UI_METHOD* ui_method,
    void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq) {
  OSSL_STORE_INFO* store_info = NULL;
  ...
  if (pem_name != NULL) {
    if (strcmp(pem_name, PEM_STRING_PUBLIC) != 0) /* No match */
      return NULL;
    *matchcount = 1;
  }
 ...
 */
static OSSL_STORE_INFO* try_decode_params(
    const char* pem_name, const char* pem_header, const unsigned char* blob,
    size_t len, void** pctx, int* matchcount, const UI_METHOD* ui_method,
    void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq) {
  OSSL_STORE_INFO* store_info = NULL;
...
static OSSL_STORE_INFO* try_decode_X509Certificate(
  ...
  if (store_info == NULL) X509_free(cert);
 ...
 * X.509 CRL decoder.
 */
static OSSL_STORE_INFO* try_decode_X509CRL(
    const char* pem_name, const char* pem_header, const unsigned char* blob,
    size_t len, void** pctx, int* matchcount, const UI_METHOD* ui_method,
    void* ui_data, const char* uri, OSSL_LIB_CTX* libctx, const char* propq) {
  OSSL_STORE_INFO* store_info = NULL;
  ...
  if (len > LONG_MAX) return NULL;
  if (pem_name != NULL) {
    if (strcmp(pem_name, PEM_STRING_X509_CRL) != 0) /* No match */
      return NULL;
    *matchcount = 1;
  }
  ...
  if (store_info == NULL) X509_CRL_free(crl);
...
static int file_find_type(OSSL_STORE_LOADER_CTX* ctx) {
  ...
  if ((buff = BIO_new(BIO_f_buffer())) == NULL) return 0;

  ctx->_.file.file = BIO_push(buff, ctx->_.file.file);
  if (BIO_buffer_peek(ctx->_.file.file, peekbuf, sizeof(peekbuf) - 1) > 0) {
    peekbuf[sizeof(peekbuf) - 1] = '\0';
    if (strstr(peekbuf, "-----BEGIN ") != NULL) ctx->type = is_pem;
  }
  return 1;
}
...
static int file_find(OSSL_STORE_LOADER_CTX* ctx,
  ...
  if (OSSL_STORE_SEARCH_get_type(search) == OSSL_STORE_SEARCH_BY_NAME) {
    ...
    hash = X509_NAME_hash_ex(OSSL_STORE_SEARCH_get0_name(search), NULL, NULL,
                             NULL);
  ...
  if (ctx != NULL) ATTICerr(0, ATTIC_R_UNSUPPORTED_SEARCH_TYPE);
  return 0;
}
...
again: {
  ...
  if (matching_handlers == NULL) goto err;
  ...
  *matchcount = 0;
  for (i = 0; i < OSSL_NELEM(file_handlers); i++) {
    ...
    if (try_matchcount > 0) {
      ...
      if (handler_ctx) handler->destroy_ctx(&handler_ctx);
      ...
      if ((*matchcount += try_matchcount) > 1) {
        ...
        if (handler->destroy_ctx != NULL) handler->destroy_ctx(&handler_ctx);
        handler_ctx = NULL;
        tmp_result = NULL;
        result = NULL;
      }
      if (result == NULL) result = tmp_result;
...
static int file_read_asn1(BIO* bp, unsigned char** data, long* len) {
  ...
  if (asn1_d2i_read_bio(bp, &mem) < 0) return 0;
...
static int file_name_check(OSSL_STORE_LOADER_CTX* ctx, const char* name) {
  ...
  if (ctx->_.dir.search_name[0] == '\0') return 1;
...
static int file_eof(OSSL_STORE_LOADER_CTX* ctx) {
  if (ctx->type == is_dir) return ctx->_.dir.end_reached;
...
static int bind_helper(ENGINE* e, const char* id) {
  if (id && (strcmp(id, loader_attic_id) != 0)) return 0;
