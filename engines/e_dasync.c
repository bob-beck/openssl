...
static int
dasync_rsa_encrypt(EVP_PKEY_CTX* ctx,
                   unsigned char* out,
                   size_t* outlen,
...
static int
dasync_rsa_decrypt(EVP_PKEY_CTX* ctx,
                   unsigned char* out,
                   size_t* outlen,
                   const unsigned char* in,
                   size_t inlen);
...
static int
dasync_aes128_init_key(EVP_CIPHER_CTX* ctx,
                       const unsigned char* key,
                       const unsigned char* iv,
                       int enc);
static int
dasync_aes128_cbc_cipher(EVP_CIPHER_CTX* ctx,
                         unsigned char* out,
                         const unsigned char* in,
                         size_t inl);
static int
...
static int
dasync_aes256_init_key(EVP_CIPHER_CTX* ctx,
                       const unsigned char* key,
                       const unsigned char* iv,
                       int enc);
static int
dasync_aes256_ctr_cipher(EVP_CIPHER_CTX* ctx,
                         unsigned char* out,
                         const unsigned char* in,
                         size_t inl);
static int
...
static int
dasync_aes128_cbc_hmac_sha1_ctrl(EVP_CIPHER_CTX* ctx,
                                 int type,
                                 int arg,
                                 void* ptr);
static int
dasync_aes128_cbc_hmac_sha1_init_key(EVP_CIPHER_CTX* ctx,
                                     ...
                                     int enc);
static int
dasync_aes128_cbc_hmac_sha1_cipher(EVP_CIPHER_CTX* ctx,
                                   unsigned char* out,
                                   const unsigned char* in,
                                   size_t inl);
static int
...
static int
dasync_ciphers(ENGINE* e, const EVP_CIPHER** cipher, const int** nids, int nid);

static int dasync_cipher_nids[] = { NID_aes_128_cbc,
                                    NID_aes_256_ctr,
...
bind_dasync(ENGINE* e)
{
  ...
  EVP_PKEY_meth_set_paramgen(
    dasync_rsa, dasync_rsa_paramgen_init, dasync_rsa_paramgen);
  EVP_PKEY_meth_set_keygen(
    dasync_rsa, dasync_rsa_keygen_init, dasync_rsa_keygen);
  EVP_PKEY_meth_set_encrypt(
    dasync_rsa, dasync_rsa_encrypt_init, dasync_rsa_encrypt);
  EVP_PKEY_meth_set_decrypt(
    dasync_rsa, dasync_rsa_decrypt_init, dasync_rsa_decrypt);
  ...
  if (_hidden_aes_256_ctr == NULL ||
      ...
      !EVP_CIPHER_meth_set_flags(_hidden_aes_256_ctr,
                                 EVP_CIPH_FLAG_DEFAULT_ASN1 |
                                   EVP_CIPH_CTR_MODE | EVP_CIPH_FLAG_PIPELINE |
                                   EVP_CIPH_CUSTOM_COPY) ||
...
static void
wait_cleanup(ASYNC_WAIT_CTX* ctx,
             const void* key,
...
dummy_pause_job(void)
{
  ...
  if (ASYNC_WAIT_CTX_get_fd(
        waitctx, engine_dasync_id, &pipefds[0], (void**)&writefd)) {
    pipefds[1] = *writefd;
  } else {
    ...
    *writefd = pipefds[1];
    ...
    if (!ASYNC_WAIT_CTX_set_wait_fd(
          waitctx, engine_dasync_id, pipefds[0], writefd, wait_cleanup)) {
...
static int
dasync_cipher_ctrl_helper(EVP_CIPHER_CTX* ctx,
                          int type,
                          int arg,
                          void* ptr,
                          int aeadcapable,
...
static int
dasync_cipher_init_key_helper(EVP_CIPHER_CTX* ctx,
                              const unsigned char* key,
                              const unsigned char* iv,
                              int enc,
...
static int
dasync_cipher_helper(EVP_CIPHER_CTX* ctx,
                     unsigned char* out,
                     const unsigned char* in,
                     size_t inl,
                     const EVP_CIPHER* cipher)
  ...
  if (pipes == 0) {
    if (pipe_ctx->aadctr != 0) {
      if (pipe_ctx->aadctr != 1)
        return -1;
      EVP_CIPHER_meth_get_ctrl(cipher)(ctx,
    ...
    for (i = 0; i < pipes; i++) {
      ...
      ret = ret && EVP_CIPHER_meth_get_do_cipher(cipher)(ctx,
                                                         pipe_ctx->outbufs[i],
                                                         pipe_ctx->inbufs[i],
                                                         pipe_ctx->lens[i]);
    }
...
dasync_aes128_cbc_hmac_sha1_ctrl(EVP_CIPHER_CTX* ctx,
                                 ...
                                 void* ptr)
{
  return dasync_cipher_ctrl_helper(
                                     ...
                                     int enc)
{
   ...
   */
  return dasync_cipher_init_key_helper(
    ctx, key, iv, enc, EVP_aes_128_cbc_hmac_sha1());
}
...
dasync_rsa_encrypt(EVP_PKEY_CTX* ctx,
                   ...
                   size_t inlen)
{
  static int (*pencryptfn)(EVP_PKEY_CTX* ctx,
                           unsigned char* out,
                           size_t* outlen,
                           const unsigned char* in,
                           size_t inlen);
...
dasync_rsa_decrypt(EVP_PKEY_CTX* ctx,
                   ...
                   size_t inlen)
{
  static int (*pdecrypt)(EVP_PKEY_CTX* ctx,
                         unsigned char* out,
                         size_t* outlen,
                         const unsigned char* in,
                         size_t inlen);
