...
static int dasync_digest_nids(const int** nids) {
  ...
  if (!init) {
    const EVP_MD* md;
    if ((md = dasync_sha1()) != NULL) digest_nids[pos++] = EVP_MD_get_type(md);
    digest_nids[pos] = 0;
    init = 1;
  }
  *nids = digest_nids;
...
static int bind_helper(ENGINE* e, const char* id) {
  if (id && (strcmp(id, engine_dasync_id) != 0)) return 0;
...
static void dummy_pause_job(void) {
  ...
  if ((job = ASYNC_get_current_job()) == NULL) return;
...
#elif defined(ASYNC_POSIX)
  if (write(pipefds[1], &buf, 1) < 0) return;
#endif
...
#elif defined(ASYNC_POSIX)
  if (read(pipefds[0], &buf, 1) < 0) return;
#endif
}
...
static int dasync_cipher_ctrl_helper(EVP_CIPHER_CTX* ctx, int type, int arg,
  ...
  switch (type) {
    case EVP_CTRL_COPY: {
      ...
      if (inner_cipher_data == NULL) return -1;
    ...
    case EVP_CTRL_AEAD_TLS1_AAD: {
      ...
      if (!aeadcapable || arg != EVP_AEAD_TLS1_AAD_LEN) return -1;

      if (pipe_ctx->aadctr >= SSL_MAX_PIPELINES) return -1;
...
static int dasync_cipher_init_key_helper(EVP_CIPHER_CTX* ctx,
  ...
  if (pipe_ctx->inner_cipher_data == NULL &&
    ...
    if (pipe_ctx->inner_cipher_data == NULL) return 0;
  }
...
static int dasync_cipher_helper(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (pipes == 0) {
    ...
    if (pipe_ctx->aadctr > 0 && pipe_ctx->aadctr != pipes) return -1;
    for (i = 0; i < pipes; i++) {
      if (pipe_ctx->aadctr > 0) {
        EVP_CIPHER_meth_get_ctrl(cipher)(ctx, EVP_CTRL_AEAD_TLS1_AAD,
                                         EVP_AEAD_TLS1_AAD_LEN,
...
static int dasync_rsa_init(EVP_PKEY_CTX* ctx) {
  ...
  if (pinit == NULL) EVP_PKEY_meth_get_init(dasync_rsa_orig, &pinit);
  return pinit(ctx);
}
...
static void dasync_rsa_cleanup(EVP_PKEY_CTX* ctx) {
  ...
  if (pcleanup == NULL) EVP_PKEY_meth_get_cleanup(dasync_rsa_orig, &pcleanup);
  pcleanup(ctx);
}
...
static int dasync_rsa_ctrl(EVP_PKEY_CTX* ctx, int type, int p1, void* p2) {
  ...
  if (pctrl == NULL) EVP_PKEY_meth_get_ctrl(dasync_rsa_orig, &pctrl, NULL);
