...
static int
dasync_digest_nids (const int **nids)
{
...
static int dasync_aes128_cbc_ctrl (EVP_CIPHER_CTX *ctx, int type, int arg,
                                   void *ptr);
static int dasync_aes128_init_key (EVP_CIPHER_CTX *ctx,
...
static int dasync_aes256_ctr_ctrl (EVP_CIPHER_CTX *ctx, int type, int arg,
                                   void *ptr);
static int dasync_aes256_init_key (EVP_CIPHER_CTX *ctx,
                                   const unsigned char *key,
                                   const unsigned char *iv, int enc);
...
static int dasync_cipher_nids[]
    = { NID_aes_128_cbc, NID_aes_256_ctr, NID_aes_128_cbc_hmac_sha1, 0 };

static int
bind_dasync (ENGINE *e)
{
  /* Setup RSA */
...
static int
dasync_init (ENGINE *e)
{
  return 1;
}

static int
dasync_finish (ENGINE *e)
{
  return 1;
}

static int
dasync_destroy (ENGINE *e)
{
  destroy_digests ();
  destroy_ciphers ();
  destroy_pkey ();
...
static int
dasync_pkey (ENGINE *e, EVP_PKEY_METHOD **pmeth, const int **pnids, int nid)
{
...
static int
dasync_digests (ENGINE *e, const EVP_MD **digest, const int **nids, int nid)
{
  int ok = 1;
  if (!digest)
    {
...
static int
dasync_ciphers (ENGINE *e, const EVP_CIPHER **cipher, const int **nids,
                int nid)
{
  int ok = 1;
  if (cipher == NULL)
    {
      ...
      *nids = dasync_cipher_nids;
      return (sizeof (dasync_cipher_nids) - 1)
             / sizeof (dasync_cipher_nids[0]);
    }
...
static void
wait_cleanup (ASYNC_WAIT_CTX *ctx, const void *key, OSSL_ASYNC_FD readfd,
              void *pvwritefd)
{
...
dummy_pause_job (void)
{
  ...
  if (ASYNC_WAIT_CTX_get_callback (waitctx, &callback, &callback_arg)
      && callback != NULL)
    {
       ...
       * the job is complete before the call to ASYNC_pause_job(). A real async
       * engine would only call the callback when the job was actually complete
       */
 ...
 */
static int
dasync_sha1_init (EVP_MD_CTX *ctx)
{
  dummy_pause_job ();
...
static int
dasync_sha1_update (EVP_MD_CTX *ctx, const void *data, size_t count)
{
  dummy_pause_job ();
...
static int
dasync_sha1_final (EVP_MD_CTX *ctx, unsigned char *md)
{
  dummy_pause_job ();
...
static int
dasync_cipher_ctrl_helper (EVP_CIPHER_CTX *ctx, int type, int arg, void *ptr,
                           int aeadcapable, const EVP_CIPHER *ciph)
{
  int ret;
...
static int
dasync_cipher_init_key_helper (EVP_CIPHER_CTX *ctx, const unsigned char *key,
                               const unsigned char *iv, int enc,
...
static int
dasync_cipher_helper (EVP_CIPHER_CTX *ctx, unsigned char *out,
                      const unsigned char *in, size_t inl,
                      const EVP_CIPHER *cipher)
{
  int ret = 1;
...
static int
dasync_cipher_cleanup_helper (EVP_CIPHER_CTX *ctx, const EVP_CIPHER *cipher)
{
  struct dasync_pipeline_ctx *pipe_ctx
      = (struct dasync_pipeline_ctx *)EVP_CIPHER_CTX_get_cipher_data (ctx);
...
static int
dasync_aes128_cbc_ctrl (EVP_CIPHER_CTX *ctx, int type, int arg, void *ptr)
{
...
static int
dasync_aes128_init_key (EVP_CIPHER_CTX *ctx, const unsigned char *key,
                        const unsigned char *iv, int enc)
{
...
static int
dasync_aes128_cbc_cipher (EVP_CIPHER_CTX *ctx, unsigned char *out,
                          const unsigned char *in, size_t inl)
{
...
static int
dasync_aes128_cbc_cleanup (EVP_CIPHER_CTX *ctx)
{
  return dasync_cipher_cleanup_helper (ctx, EVP_aes_128_cbc ());
}

static int
dasync_aes256_ctr_ctrl (EVP_CIPHER_CTX *ctx, int type, int arg, void *ptr)
{
  return dasync_cipher_ctrl_helper (ctx, type, arg, ptr, 0,
                                    EVP_aes_256_ctr ());
}
...
static int
dasync_aes256_init_key (EVP_CIPHER_CTX *ctx, const unsigned char *key,
                        const unsigned char *iv, int enc)
{
...
static int
dasync_aes256_ctr_cipher (EVP_CIPHER_CTX *ctx, unsigned char *out,
                          const unsigned char *in, size_t inl)
{
...
static int
dasync_aes256_ctr_cleanup (EVP_CIPHER_CTX *ctx)
{
...
static int
dasync_aes128_cbc_hmac_sha1_ctrl (EVP_CIPHER_CTX *ctx, int type, int arg,
                                  void *ptr)
{
...
static int
dasync_aes128_cbc_hmac_sha1_init_key (EVP_CIPHER_CTX *ctx,
                                      const unsigned char *key,
                                      const unsigned char *iv, int enc)
{
  /*
...
static int
dasync_aes128_cbc_hmac_sha1_cipher (EVP_CIPHER_CTX *ctx, unsigned char *out,
                                    const unsigned char *in, size_t inl)
{
  return dasync_cipher_helper (ctx, out, in, inl,
                               EVP_aes_128_cbc_hmac_sha1 ());
}

static int
dasync_aes128_cbc_hmac_sha1_cleanup (EVP_CIPHER_CTX *ctx)
{
  /*
   * We can safely assume that EVP_aes_128_cbc_hmac_sha1() != NULL,
   * see comment before the definition of dasync_aes_128_cbc_hmac_sha1().
 ...
 */
static int
dasync_rsa_init (EVP_PKEY_CTX *ctx)
{
...
static void
dasync_rsa_cleanup (EVP_PKEY_CTX *ctx)
{
...
static int
dasync_rsa_paramgen_init (EVP_PKEY_CTX *ctx)
{
...
static int
dasync_rsa_paramgen (EVP_PKEY_CTX *ctx, EVP_PKEY *pkey)
{
...
static int
dasync_rsa_keygen_init (EVP_PKEY_CTX *ctx)
{
...
static int
dasync_rsa_keygen (EVP_PKEY_CTX *ctx, EVP_PKEY *pkey)
{
...
static int
dasync_rsa_encrypt_init (EVP_PKEY_CTX *ctx)
{
...
static int
dasync_rsa_encrypt (EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen,
...
static int
dasync_rsa_decrypt_init (EVP_PKEY_CTX *ctx)
{
...
static int
dasync_rsa_decrypt (EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen,
                    const unsigned char *in, size_t inlen)
{
...
static int
dasync_rsa_ctrl (EVP_PKEY_CTX *ctx, int type, int p1, void *p2)
{
...
static int
dasync_rsa_ctrl_str (EVP_PKEY_CTX *ctx, const char *type, const char *value)
{
