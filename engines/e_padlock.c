...
static int padlock_cfb_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out_arg,
  ...
  if ((chunk = EVP_CIPHER_CTX_get_num(ctx))) { /* borrow chunk variable */
    ...
    if (chunk >= AES_BLOCK_SIZE) return 0; /* bogus value */
  ...
  if ((chunk = nbytes & ~(AES_BLOCK_SIZE - 1))) {
    if (!padlock_cfb_encrypt(out_arg, in_arg, cdata, chunk)) return 0;
    nbytes -= chunk;
  }
...
static int padlock_ofb_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out_arg,
  ...
  if ((chunk = nbytes & ~(AES_BLOCK_SIZE - 1))) {
    if (!padlock_ofb_encrypt(out_arg, in_arg, cdata, chunk)) return 0;
    nbytes -= chunk;
  }
