...
static int ossltest_digest_nids(const int** nids) {
  ...
  if (!init) {
    const EVP_MD* md;
    if ((md = digest_md5()) != NULL) digest_nids[pos++] = EVP_MD_get_type(md);
    if ((md = digest_sha1()) != NULL) digest_nids[pos++] = EVP_MD_get_type(md);
...
static EVP_PKEY* load_key(ENGINE* eng, const char* key_id, int pub,
  ...
  if (!CHECK_AND_SKIP_CASE_PREFIX(key_id, "ot:")) return NULL;
...
static int bind_helper(ENGINE* e, const char* id) {
  if (id && (strcmp(id, engine_ossltest_id) != 0)) return 0;
...
static int ossltest_aes128_cbc_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (tmpbuf == NULL && inl > 0) return -1;

  /* Remember what we were asked to encrypt */
  if (tmpbuf != NULL) memcpy(tmpbuf, in, inl);

  /* Go through the motions of encrypting it */
  ret = EVP_CIPHER_meth_get_do_cipher(EVP_aes_128_cbc())(ctx, out, in, inl);

  /* Throw it all away and just use the plaintext as the output */
  if (tmpbuf != NULL) memcpy(out, tmpbuf, inl);
  OPENSSL_free(tmpbuf);
...
static int ossltest_aes128_gcm_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  /* Throw it all away and just use the plaintext as the output */
  if (tmpbuf != NULL && out != NULL) memcpy(out, tmpbuf, inl);
  OPENSSL_free(tmpbuf);
  ...
  if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
    ...
    if (plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
      ...
      if (key->tls_ver >= TLS1_1_VERSION) {
        if (len < (AES_BLOCK_SIZE + SHA_DIGEST_LENGTH + 1)) return 0;
      ...
      if (pad > maxpad) return 0;
      for (plen = len - pad - 1; plen < len; plen++)
        if (out[plen] != pad) return 0;
    }
  }
...
static int ossltest_aes128_cbc_hmac_sha1_ctrl(EVP_CIPHER_CTX* ctx, int type,
  ...
  switch (type) {
    ...
    case EVP_CTRL_AEAD_TLS1_AAD: {
      ...
      if (arg != EVP_AEAD_TLS1_AAD_LEN) return -1;
