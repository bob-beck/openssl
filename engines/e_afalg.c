...
static int afalg_fin_cipher_aio(afalg_aio* ptr,
                                int sfd,
                                unsigned char* buf,
                                size_t len);
static int afalg_create_sk(afalg_ctx* actx,
...
static int afalg_ciphers(ENGINE* e,
                         const EVP_CIPHER** cipher,
                         const int** nids,
                         int nid);
static int afalg_cipher_init(EVP_CIPHER_CTX* ctx,
                             const unsigned char* key,
                             const unsigned char* iv,
                             int enc);
static int afalg_do_cipher(EVP_CIPHER_CTX* ctx,
                           unsigned char* out,
...
static ossl_inline int io_getevents(aio_context_t ctx,
                                    long min,
                                    long max,
  ...
  if (sizeof(*timeout) != sizeof(struct __timespec32)) {
    int ret = syscall(__NR_io_pgetevents_time64, ctx, min, max, events, timeout,
                      NULL);
...
static void afalg_waitfd_cleanup(ASYNC_WAIT_CTX* ctx,
                                 const void* key,
...
static int afalg_fin_cipher_aio(afalg_aio* aio,
                                int sfd,
                                unsigned char* buf,
                                size_t len) {
  int r;
...
static void afalg_set_iv_sk(struct cmsghdr* cmsg,
                            const unsigned char* iv,
...
static ossl_inline int afalg_set_key(afalg_ctx* actx,
                                     const unsigned char* key,
                                     const int klen) {
  int ret;
...
static int afalg_start_cipher_sk(afalg_ctx* actx,
                                 const unsigned char* in,
                                 size_t inl,
                                 const unsigned char* iv,
                                 unsigned int enc) {
