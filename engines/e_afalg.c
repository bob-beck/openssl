...
err:
  if (actx->bfd >= 0) close(actx->bfd);
  if (actx->sfd >= 0) close(actx->sfd);
  actx->bfd = actx->sfd = -1;
  return 0;
}
...
static int afalg_cipher_init(EVP_CIPHER_CTX* ctx, const unsigned char* key,
  ...
  if ((len = EVP_CIPHER_CTX_get_key_length(ctx)) <= 0) goto err;
  ret = afalg_set_key(actx, key, len);
  if (ret < 1) goto err;

  /* Setup AIO ctx to allow async AFALG crypto processing */
  if (afalg_init_aio(&actx->aio) == 0) goto err;
...
static int afalg_cipher_cleanup(EVP_CIPHER_CTX* ctx) {
  ...
  actx = (afalg_ctx*)EVP_CIPHER_CTX_get_cipher_data(ctx);
  if (actx == NULL || actx->init_done != MAGIC_INIT_NUM) return 1;
  ...
  if (actx->aio.mode == MODE_SYNC) close(actx->aio.efd);
...
static const EVP_CIPHER* afalg_aes_cbc(int nid) {
  ...
  if (cipher_handle == NULL) return NULL;
...
static int bind_helper(ENGINE* e, const char* id) {
  if (id && (strcmp(id, engine_afalg_id) != 0)) return 0;

  if (!afalg_chk_platform()) return 0;
