...
static int
afalg_init_aio(afalg_aio* aio);
static int
afalg_fin_cipher_aio(afalg_aio* ptr, int sfd, unsigned char* buf, size_t len);
static int
afalg_create_sk(afalg_ctx* actx,
                const char* ciphertype,
                const char* ciphername);
static int
afalg_destroy(ENGINE* e);
static int
afalg_init(ENGINE* e);
static int
afalg_finish(ENGINE* e);
static const EVP_CIPHER*
afalg_aes_cbc(int nid);
static cbc_handles*
get_cipher_handle(int nid);
static int
afalg_ciphers(ENGINE* e, const EVP_CIPHER** cipher, const int** nids, int nid);
static int
afalg_cipher_init(EVP_CIPHER_CTX* ctx,
                  const unsigned char* key,
                  const unsigned char* iv,
                  int enc);
static int
afalg_do_cipher(EVP_CIPHER_CTX* ctx,
                unsigned char* out,
                const unsigned char* in,
                size_t inl);
static int
afalg_cipher_cleanup(EVP_CIPHER_CTX* ctx);
static int
afalg_chk_platform(void);
...
io_setup(unsigned n, aio_context_t* ctx)
{
...
static ossl_inline int
io_read(aio_context_t ctx, long n, struct iocb** iocb)
{
...
static void
afalg_waitfd_cleanup(ASYNC_WAIT_CTX* ctx,
                     const void* key,
...
static int
afalg_setup_async_event_notification(afalg_aio* aio)
{
  ASYNC_JOB* job;
  ...
  if ((job = ASYNC_get_current_job()) != NULL) {
    ...
    if (ret == 0) {
      ...
      ret = ASYNC_WAIT_CTX_set_wait_fd(
        waitctx, engine_afalg_id, aio->efd, custom, afalg_waitfd_cleanup);
      if (ret == 0) {
      ...
      if (fcntl(aio->efd, F_SETFL, O_NONBLOCK) != 0) {
        ALG_WARN(
          "%s(%d): Failed to set event fd as NONBLOCKING", __FILE__, __LINE__);
      }
    }
    aio->mode = MODE_ASYNC;
  } else {
...
static int
afalg_fin_cipher_aio(afalg_aio* aio, int sfd, unsigned char* buf, size_t len)
{
  int r;
  int retry = 0;
  unsigned int done = 0;
...
afalg_set_op_sk(struct cmsghdr* cmsg, const ALG_OP_TYPE op)
{
...
static void
afalg_set_iv_sk(struct cmsghdr* cmsg,
                const unsigned char* iv,
...
static ossl_inline int
afalg_set_key(afalg_ctx* actx, const unsigned char* key, const int klen)
{
  int ret;
...
static int
afalg_create_sk(afalg_ctx* actx, const char* ciphertype, const char* ciphername)
{
...
static int
afalg_start_cipher_sk(afalg_ctx* actx,
                      const unsigned char* in,
                      size_t inl,
                      const unsigned char* iv,
                      unsigned int enc)
{
  ...
  if (sbytes < 0) {
    ALG_PERR(
      "%s(%d): sendmsg failed for cipher operation : ", __FILE__, __LINE__);
    return 0;
  }
