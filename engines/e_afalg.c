...
static ossl_inline int
io_getevents (aio_context_t ctx, long min, long max, struct io_event *events,
...
static void
afalg_waitfd_cleanup (ASYNC_WAIT_CTX *ctx, const void *key,
              ...
              if (events[0].res < 0)
                {
                  ...
                  if (events[0].res == -EBUSY && retry++ < 3)
                  ...
                  else
                    {
                       ...
                       * variable
                       *
                       * because long long int should always be at least 64 bit
                       * this should work
                       */
...
static void
afalg_set_iv_sk (struct cmsghdr *cmsg, const unsigned char *iv,
...
static int
afalg_create_sk (afalg_ctx *actx, const char *ciphertype,
                 const char *ciphername)
{
...
static int
afalg_start_cipher_sk (afalg_ctx *actx, const unsigned char *in, size_t inl,
                       const unsigned char *iv, unsigned int enc)
{
  struct msghdr msg;
  struct cmsghdr *cmsg;
  struct iovec iov;
...
#endif
  char
      cbuf[CMSG_SPACE (ALG_IV_LEN (ALG_AES_IV_LEN)) + CMSG_SPACE (ALG_OP_LEN)];
...
static int
afalg_cipher_init (EVP_CIPHER_CTX *ctx, const unsigned char *key,
                   const unsigned char *iv, int enc)
{
  int ciphertype;
  int ret, len;
...
static int
afalg_do_cipher (EVP_CIPHER_CTX *ctx, unsigned char *out,
                 const unsigned char *in, size_t inl)
{
  afalg_ctx *actx;
  int ret;
