...
static int
clean_devcrypto_session (session_op_t *sess)
{
...
static size_t
find_cipher_data_index (int nid)
{
  size_t i;
...
static int
cipher_init (EVP_CIPHER_CTX *ctx, const unsigned char *key,
...
static int
cipher_do_cipher (EVP_CIPHER_CTX *ctx, unsigned char *out,
...
static int
ctr_do_cipher (EVP_CIPHER_CTX *ctx, unsigned char *out,
               const unsigned char *in, size_t inl)
{
  struct cipher_ctx *cipher_ctx
      = (struct cipher_ctx *)EVP_CIPHER_CTX_get_cipher_data (ctx);
...
static int
cipher_ctrl (EVP_CIPHER_CTX *ctx, int type, int p1, void *p2)
{
  struct cipher_ctx *cipher_ctx
      = (struct cipher_ctx *)EVP_CIPHER_CTX_get_cipher_data (ctx);
...
static int
cipher_cleanup (EVP_CIPHER_CTX *ctx)
{
  struct cipher_ctx *cipher_ctx
      = (struct cipher_ctx *)EVP_CIPHER_CTX_get_cipher_data (ctx);
...
static int known_cipher_nids_amount
...
static int
devcrypto_test_cipher (size_t cipher_data_index)
{
...
static void
rebuild_known_cipher_nids (ENGINE *e)
{
  size_t i;
...
static int
get_cipher_nids (const int **nids)
{
...
static void
destroy_all_cipher_methods (void)
{
  size_t i;
...
static int
devcrypto_ciphers (ENGINE *e, const EVP_CIPHER **cipher, const int **nids,
                   int nid)
{
  if (cipher == NULL)
...
static void
devcrypto_select_all_ciphers (int *cipher_list)
{
  size_t i;
...
static int
cryptodev_select_cipher_cb (const char *str, int len, void *usr)
{
...
static size_t
find_digest_data_index (int nid)
{
  size_t i;
...
static int
digest_op (struct digest_ctx *ctx, const void *src, size_t srclen, void *res,
           unsigned int flags)
{
  struct crypt_op cryp;
...
static int
digest_final (EVP_MD_CTX *ctx, unsigned char *md)
{
  struct digest_ctx *digest_ctx
      = (struct digest_ctx *)EVP_MD_CTX_get0_md_data (ctx);
...
static int
digest_copy (EVP_MD_CTX *to, const EVP_MD_CTX *from)
{
...
static int known_digest_nids_amount
    = -1; /* -1 indicates not yet initialised */
...
static int
devcrypto_test_digest (size_t digest_data_index)
{
...
static void
rebuild_known_digest_nids (ENGINE *e)
{
  size_t i;
...
static int
get_digest_nids (const int **nids)
{
...
static void
destroy_all_digest_methods (void)
{
  size_t i;
...
static int
devcrypto_digests (ENGINE *e, const EVP_MD **digest, const int **nids, int nid)
{
  if (digest == NULL)
...
static void
devcrypto_select_all_digests (int *digest_list)
{
  size_t i;
...
static int
cryptodev_select_digest_cb (const char *str, int len, void *usr)
{
  ...
  { DEVCRYPTO_CMD_USE_SOFTDRIVERS, "USE_SOFTDRIVERS",
    ...
    "(" OPENSSL_MSTR (DEVCRYPTO_REQUIRE_ACCELERATED) "=use only accelerated "
                                                     "drivers, " OPENSSL_MSTR (DEVCRYPTO_USE_SOFTWARE) "=allow all drivers, " OPENSSL_MSTR (
                                                         DEVCRYPTO_REJECT_SOFTWARE) "=use if acceleration can't be determined) [default=" OPENSSL_MSTR (DEVCRYPTO_DEFAULT_USE_SOFTDRIVERS) "]",
...
static int
devcrypto_ctrl (ENGINE *e, int cmd, long i, void *p, void (*f) (void))
{
  int *new_list;
  switch (cmd)
    {
 ...
 */
void
engine_load_devcrypto_int (void)
{
  ENGINE *e = NULL;
