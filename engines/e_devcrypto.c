...
static void prepare_cipher_methods(void)
{
    ...
    for (i = 0, known_cipher_nids_amount = 0; i < OSSL_NELEM(cipher_data); i++)
    {
        ...
        if ((known_cipher_methods[i] = EVP_CIPHER_meth_new(
            ...
            !EVP_CIPHER_meth_set_flags(known_cipher_methods[i], cipher_data[i].flags | EVP_CIPH_CUSTOM_COPY |
                                                                    EVP_CIPH_CTRL_INIT | EVP_CIPH_FLAG_DEFAULT_ASN1) ||
    ...
    devcrypto_cmds[] =
        {
            ...
            {DEVCRYPTO_CMD_USE_SOFTDRIVERS, "USE_SOFTDRIVERS",
             "specifies whether to use software (not accelerated) drivers (" OPENSSL_MSTR(DEVCRYPTO_REQUIRE_ACCELERATED) "=use only accelerated drivers, " OPENSSL_MSTR(
                 DEVCRYPTO_USE_SOFTWARE) "=allow all drivers, " OPENSSL_MSTR(DEVCRYPTO_REJECT_SOFTWARE) "=use if "
                                                                                                        "acceleration "
                                                                                                        "can't be "
                                                                                                        "determined) "
                                                                                                        "[default"
            ...
            {DEVCRYPTO_CMD_CIPHERS, "CIPHERS",
             "either ALL, NONE, or a comma-separated list of ciphers to enable [default=ALL]", ENGINE_CMD_FLAG_STRING},

#ifdef IMPLEMENT_DIGEST
            {DEVCRYPTO_CMD_DIGESTS, "DIGESTS",
             "either ALL, NONE, or a comma-separated list of digests to enable [default=ALL]", ENGINE_CMD_FLAG_STRING},
#endif
