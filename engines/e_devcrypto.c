...
static int
cipher_do_cipher(EVP_CIPHER_CTX* ctx,
                 unsigned char* out,
...
static int
ctr_do_cipher(EVP_CIPHER_CTX* ctx,
              unsigned char* out,
              const unsigned char* in,
              size_t inl)
{
...
static int
digest_op(struct digest_ctx* ctx,
          const void* src,
          size_t srclen,
          void* res,
          unsigned int flags)
  ...
  switch (cmd) {
    ...
    case DEVCRYPTO_CMD_USE_SOFTDRIVERS:
      switch (i) {
        ...
        default:
          fprintf(
            stderr, "devcrypto: invalid value (%ld) for USE_SOFTDRIVERS\n", i);
          return 0;
      }
