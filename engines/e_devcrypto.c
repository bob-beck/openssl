...
static size_t find_cipher_data_index(int nid) {
  ...
  for (i = 0; i < OSSL_NELEM(cipher_data); i++)
    if (nid == cipher_data[i].nid) return i;
  return (size_t)-1;
}
...
static int cipher_do_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (ivlen > 0) switch (cipher_ctx->mode) {
...
static int ctr_do_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (inl > cipher_ctx->blocksize) {
    ...
    if (cipher_do_cipher(ctx, out, in, len) < 1) return 0;
    inl -= len;
    out += len;
    in += len;
  }
...
static int devcrypto_ciphers(ENGINE* e, const EVP_CIPHER** cipher,
  ...
  if (cipher == NULL) return get_cipher_nids(nids);
...
static int cryptodev_select_cipher_cb(const char* str, int len, void* usr) {
  ...
  if (len == 0) return 1;
  if (usr == NULL || (name = OPENSSL_strndup(str, len)) == NULL) return 0;
...
static size_t find_digest_data_index(int nid) {
  ...
  for (i = 0; i < OSSL_NELEM(digest_data); i++)
    if (nid == digest_data[i].nid) return i;
  return (size_t)-1;
}
...
static int digest_final(EVP_MD_CTX* ctx, unsigned char* md) {
  ...
  if (md == NULL || digest_ctx == NULL) return 0;
...
static int digest_copy(EVP_MD_CTX* to, const EVP_MD_CTX* from) {
  ...
  if (digest_from == NULL || digest_from->init_called != 1) return 1;
...
static void prepare_digest_methods(void) {
  ...
  finish:
    ...
    if (sess2.ses != 0) ioctl(cfd, CIOCFSESSION, &sess2.ses);
    if (devcrypto_test_digest(i))
      known_digest_nids[known_digest_nids_amount++] = digest_data[i].nid;
  }
}
...
static int devcrypto_digests(ENGINE* e, const EVP_MD** digest, const int** nids,
                             int nid) {
  if (digest == NULL) return get_digest_nids(nids);
