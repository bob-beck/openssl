...
static int ssl_cipher_info_find(const ssl_cipher_table* table, size_t table_cnt,
  ...
  for (i = 0; i < table_cnt; i++, table++) {
    if (table->mask == mask) return (int)i;
  }
  return -1;
}
...
int ssl_load_ciphers(SSL_CTX* ctx) {
  ...
  for (i = 0, t = ssl_cipher_table_cipher; i < SSL_ENC_NUM_IDX; i++, t++) {
    if (t->nid != NID_undef) {
      ...
      if (cipher == NULL) ctx->disabled_enc_mask |= t->mask;
    }
  }
  ...
  for (i = 0, t = ssl_cipher_table_mac; i < SSL_MD_NUM_IDX; i++, t++) {
    ...
    if (md == NULL) {
      ...
      if (!ossl_assert(tmpsize > 0)) return 0;
    ...
    *enc = NULL;
    ...
    if (i == SSL_ENC_NULL_IDX) {
      ...
      *enc = EVP_CIPHER_fetch(ctx->libctx, "NULL", ctx->propq);
      ...
      if (cipher == NULL || !ssl_evp_cipher_up_ref(cipher)) return 0;
...
int ssl_cipher_get_evp_md_mac(SSL_CTX* ctx, const SSL_CIPHER* sslc,
  ...
  if (i == -1) {
    *md = NULL;
    ...
    if (digest == NULL || !ssl_evp_md_up_ref(digest)) return 0;

    *md = digest;
    if (mac_pkey_type != NULL) *mac_pkey_type = ctx->ssl_mac_pkey_id[i];
    if (mac_secret_size != NULL) *mac_secret_size = ctx->ssl_mac_secret_size[i];
  }
  return 1;
}
  ...
  if (comp != NULL) {
    ...
    *comp = NULL;
    ...
    if (comp_methods != NULL) {
      ...
      if (i >= 0) *comp = sk_SSL_COMP_value(comp_methods, i);
    }
    /* If were only interested in comp then return success */
    if ((enc == NULL) && (md == NULL)) return 1;
  }

  if ((enc == NULL) || (md == NULL)) return 0;

  if (!ssl_cipher_get_evp_cipher(ctx, c, enc)) return 0;
...
const EVP_MD* ssl_md(SSL_CTX* ctx, int idx) {
  ...
  if (idx < 0 || idx >= SSL_MD_NUM_IDX) return NULL;
...
static void ssl_cipher_collect_aliases(
    ...
    uint32_t disabled_mkey, uint32_t disabled_auth, uint32_t disabled_enc,
  ...
  for (i = 0; i < num_of_group_aliases; i++) {
    ...
    if (algorithm_mkey)
      if ((algorithm_mkey & mask_mkey) == 0) continue;

    if (algorithm_auth)
      if ((algorithm_auth & mask_auth) == 0) continue;

    if (algorithm_enc)
      if ((algorithm_enc & mask_enc) == 0) continue;

    if (algorithm_mac)
      if ((algorithm_mac & mask_mac) == 0) continue;
...
static void ssl_cipher_apply_rule(uint32_t cipher_id, uint32_t alg_mkey,
  ...
  for (;;) {
     ...
     */
    if (strength_bits >= 0) {
      if (strength_bits != cp->strength_bits) continue;
    } else {
      if (trc_out != NULL) {
        BIO_printf(trc_out,
                   "\nName: %s:"
      ...
      if (cipher_id != 0 && (cipher_id != cp->id)) continue;
      if (alg_mkey && !(alg_mkey & cp->algorithm_mkey)) continue;
      if (alg_auth && !(alg_auth & cp->algorithm_auth)) continue;
      if (alg_enc && !(alg_enc & cp->algorithm_enc)) continue;
      if (alg_mac && !(alg_mac & cp->algorithm_mac)) continue;
      if (min_tls && (min_tls != cp->min_tls)) continue;
    ...
    if (trc_out != NULL) BIO_printf(trc_out, "Action = %d\n", rule);
    ...
    /* Move the added cipher to this location */
      ...
      if (curr->active) ll_append_head(&head, curr, &tail);
...
static int ssl_cipher_strength_sort(CIPHER_ORDER** head_p,
  ...
  curr = *head_p;
  while (curr != NULL) {
    if (curr->active) number_uses[curr->cipher->strength_bits]++;
    curr = curr->next;
  }
  /*
...
static int ssl_cipher_process_rulestr(const char* rule_str,
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (!found) break; /* ignore this entry */
...
static int update_cipher_list(SSL_CTX* ctx, STACK_OF(SSL_CIPHER)** cipher_list,
  ...
  if (tmp_cipher_list == NULL) return 0;
...
STACK_OF(SSL_CIPHER)* ssl_create_cipher_list(
    SSL_CTX* ctx, STACK_OF(SSL_CIPHER)* tls13_ciphersuites,
    STACK_OF(SSL_CIPHER)** cipher_list,
  ...
  if (!check_suiteb_cipher_list(ssl_method, c, &rule_str)) return NULL;
  ...
  if (num_of_ciphers > 0) {
    ...
    if (co_list == NULL) return NULL; /* Failure */
  }
  ...
  for (curr = head; curr != NULL; curr = curr->next) {
    if (curr->active) {
      ...
      if (trc_out != NULL) BIO_printf(trc_out, "<%s>\n", curr->cipher->name);
    }
  }
...
char* SSL_CIPHER_description(const SSL_CIPHER* cipher, char* buf, int len) {
  ...
  if (buf == NULL) {
    len = 128;
    if ((buf = OPENSSL_malloc(len)) == NULL) return NULL;
  } else if (len < 128) {
    return NULL;
  }
...
const char* SSL_CIPHER_get_version(const SSL_CIPHER* c) {
   ...
   */
  if (c->min_tls == TLS1_VERSION) return "TLSv1.0";
...
int SSL_CIPHER_get_bits(const SSL_CIPHER* c, int* alg_bits) {
  ...
  if (c != NULL) {
    if (alg_bits != NULL) *alg_bits = (int)c->alg_bits;
...
STACK_OF(SSL_COMP)* SSL_COMP_set0_compression_methods(
...
int SSL_COMP_add_compression_method(int id, COMP_METHOD* cm) {
  ...
  if (comp_methods == NULL) return 1;

  if (cm == NULL || COMP_get_type(cm) == NID_undef) return 1;
...
const SSL_CIPHER* ssl_get_cipher_by_char(SSL_CONNECTION* s,
  ...
  if (c == NULL || (!all && c->valid == 0)) return NULL;
  return c;
}
...
int SSL_CIPHER_get_cipher_nid(const SSL_CIPHER* c) {
  int i;
  if (c == NULL) return NID_undef;
  i = ssl_cipher_info_lookup(ssl_cipher_table_cipher, c->algorithm_enc);
  if (i == -1) return NID_undef;
...
int ssl_get_md_idx(int md_nid) {
  ...
  for (i = 0; i < SSL_MD_NUM_IDX; i++) {
    if (md_nid == ssl_cipher_table_mac[i].nid) return i;
  }
  return -1;
}
...
int ssl_cert_is_disabled(SSL_CTX* ctx, size_t idx) {
  ...
  if (idx >= SSL_PKEY_NUM) return 0;

  cl = ssl_cert_lookup_by_idx(idx, ctx);
  if (cl == NULL || (cl->amask & ctx->disabled_auth_mask) != 0) return 1;
  return 0;
}
