...
static int
ssl_cipher_info_find (const ssl_cipher_table *table, size_t table_cnt,
                      uint32_t mask)
{
  size_t i;
...
int
ssl_cipher_get_evp_cipher (SSL_CTX *ctx, const SSL_CIPHER *sslc,
...
int
ssl_cipher_get_evp_md_mac (SSL_CTX *ctx, const SSL_CIPHER *sslc,
...
int
ssl_cipher_get_evp (SSL_CTX *ctx, const SSL_SESSION *s, const EVP_CIPHER **enc,
                    const EVP_MD **md, int *mac_pkey_type,
...
static void
ssl_cipher_apply_rule (uint32_t cipher_id, uint32_t alg_mkey,
                       uint32_t alg_auth, uint32_t alg_enc, uint32_t alg_mac,
                       int min_tls, uint32_t algo_strength, int rule,
...
static int
check_suiteb_cipher_list (const SSL_METHOD *meth, CERT *c,
...
static int
update_cipher_list (SSL_CTX *ctx, STACK_OF (SSL_CIPHER) **cipher_list,
...
SSL_set_ciphersuites (SSL *s, const char *str)
{
  ...
  if (ret && sc->cipher_list != NULL)
    return update_cipher_list (s->ctx, &sc->cipher_list,
                               &sc->cipher_list_by_id, sc->tls13_ciphersuites);
...
int
ssl_cipher_get_overhead (const SSL_CIPHER *c, size_t *mac_overhead,
