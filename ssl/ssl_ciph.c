...
static int
ssl_cipher_info_find(const ssl_cipher_table* table,
                     size_t table_cnt,
                     uint32_t mask)
{
  size_t i;
...
ssl_load_ciphers(SSL_CTX* ctx)
{
   ...
   */
  memcpy(
    ctx->ssl_mac_pkey_id, default_mac_pkey_id, sizeof(ctx->ssl_mac_pkey_id));
...
int
ssl_cipher_get_evp(SSL_CTX* ctx,
                   const SSL_SESSION* s,
                   const EVP_CIPHER** enc,
                   const EVP_MD** md,
                   int* mac_pkey_type,
  ...
  if ((*enc != NULL) &&
    ...
    if (c->algorithm_enc == SSL_RC4 && c->algorithm_mac == SSL_MD5)
      ...
      evp = ssl_evp_cipher_fetch(
        ctx->libctx, NID_aes_128_cbc_hmac_sha1, ctx->propq);
    else if (c->algorithm_enc == SSL_AES256 && c->algorithm_mac == SSL_SHA1)
      evp = ssl_evp_cipher_fetch(
        ctx->libctx, NID_aes_256_cbc_hmac_sha1, ctx->propq);
    else if (c->algorithm_enc == SSL_AES128 && c->algorithm_mac == SSL_SHA256)
      evp = ssl_evp_cipher_fetch(
        ctx->libctx, NID_aes_128_cbc_hmac_sha256, ctx->propq);
    else if (c->algorithm_enc == SSL_AES256 && c->algorithm_mac == SSL_SHA256)
      evp = ssl_evp_cipher_fetch(
        ctx->libctx, NID_aes_256_cbc_hmac_sha256, ctx->propq);
...
static void
ll_append_tail(CIPHER_ORDER** head, CIPHER_ORDER* curr, CIPHER_ORDER** tail)
{
  if (curr == *tail)
    return;
  if (curr == *head)
...
static void
ll_append_head(CIPHER_ORDER** head, CIPHER_ORDER* curr, CIPHER_ORDER** tail)
{
  if (curr == *head)
    return;
  if (curr == *tail)
...
static void
ssl_cipher_apply_rule(uint32_t cipher_id,
                      uint32_t alg_mkey,
                      uint32_t alg_auth,
                      uint32_t alg_enc,
                      uint32_t alg_mac,
...
static int
check_suiteb_cipher_list(const SSL_METHOD* meth,
                         CERT* c,
...
static int
ciphersuite_cb(const char* elem, int len, void* arg)
{
...
static __owur int
set_ciphersuites(STACK_OF(SSL_CIPHER)** currciphers, const char* str)
{
...
static int
update_cipher_list_by_id(STACK_OF(SSL_CIPHER)** cipher_list_by_id,
...
static int
update_cipher_list(SSL_CTX* ctx,
...
SSL_CTX_set_ciphersuites(SSL_CTX* ctx, const char* str)
{
  ...
  if (ret && ctx->cipher_list != NULL)
    return update_cipher_list(
      ctx, &ctx->cipher_list, &ctx->cipher_list_by_id, ctx->tls13_ciphersuites);
...
SSL_set_ciphersuites(SSL* s, const char* str)
{
  ...
  if (ret && sc->cipher_list != NULL)
    return update_cipher_list(
      s->ctx, &sc->cipher_list, &sc->cipher_list_by_id, sc->tls13_ciphersuites);
                       ...
                       CERT* c)
{
  ...
  ssl_cipher_apply_rule(
    0, SSL_kECDHE, SSL_aECDSA, 0, 0, 0, 0, CIPHER_ADD, -1, &head, &tail);
  ssl_cipher_apply_rule(
    0, SSL_kECDHE, 0, 0, 0, 0, 0, CIPHER_ADD, -1, &head, &tail);
  ssl_cipher_apply_rule(
    0, SSL_kECDHE, 0, 0, 0, 0, 0, CIPHER_DEL, -1, &head, &tail);

  /* Within each strength group, we prefer GCM over CHACHA... */
  ssl_cipher_apply_rule(
    0, 0, 0, SSL_AESGCM, 0, 0, 0, CIPHER_ADD, -1, &head, &tail);
  ssl_cipher_apply_rule(
    0, 0, 0, SSL_CHACHA20, 0, 0, 0, CIPHER_ADD, -1, &head, &tail);
   ...
   */
  ssl_cipher_apply_rule(
    0, 0, 0, SSL_AES ^ SSL_AESGCM, 0, 0, 0, CIPHER_ADD, -1, &head, &tail);
  ...
  ssl_cipher_apply_rule(
    0, 0, 0, 0, SSL_MD5, 0, 0, CIPHER_ORD, -1, &head, &tail);
   ...
   */
  ssl_cipher_apply_rule(
    0, 0, SSL_aNULL, 0, 0, 0, 0, CIPHER_ORD, -1, &head, &tail);

  ssl_cipher_apply_rule(
    0, SSL_kRSA, 0, 0, 0, 0, 0, CIPHER_ORD, -1, &head, &tail);
  ssl_cipher_apply_rule(
    0, SSL_kPSK, 0, 0, 0, 0, 0, CIPHER_ORD, -1, &head, &tail);

  /* RC4 is sort-of broken -- move to the end */
  ssl_cipher_apply_rule(
    0, 0, 0, SSL_RC4, 0, 0, 0, CIPHER_ORD, -1, &head, &tail);
   ...
   */
  ssl_cipher_apply_rule(
    0, 0, 0, 0, 0, TLS1_2_VERSION, 0, CIPHER_BUMP, -1, &head, &tail);
   ...
   */
  ssl_cipher_apply_rule(
    0, 0, 0, 0, SSL_AEAD, 0, 0, CIPHER_BUMP, -1, &head, &tail);
  ssl_cipher_apply_rule(
    0, SSL_kDHE | SSL_kECDHE, 0, 0, 0, 0, 0, CIPHER_BUMP, -1, &head, &tail);
  ...
  if (HAS_PREFIX(rule_str, "DEFAULT")) {
    ok = ssl_cipher_process_rulestr(
      OSSL_default_cipher_list(), &head, &tail, ca_list, c);
    rule_p += 7;
    if (*rule_p == ':')
      rule_p++;
  }
...
const char*
OPENSSL_cipher_name(const char* stdname)
{
  const SSL_CIPHER* c;
...
const char*
SSL_COMP_get0_name(const SSL_COMP* comp)
{
#ifndef OPENSSL_NO_COMP
  return comp->name;
#else
...
int
SSL_COMP_get_id(const SSL_COMP* comp)
{
#ifndef OPENSSL_NO_COMP
  return comp->id;
#else
...
const SSL_CIPHER*
ssl_get_cipher_by_char(SSL_CONNECTION* s, const unsigned char* ptr, int all)
{
  const SSL_CIPHER* c =
 ...
 */
const char*
OSSL_default_ciphersuites(void)
{
