...
static int ssl_do_config(SSL* s, SSL_CTX* ctx, const char* name, int system) {
  ...
  if (name == NULL && system) name = "system_default";
  if (!conf_ssl_name_find(name, &idx)) {
    if (!system)
      ERR_raise_data(ERR_LIB_SSL, SSL_R_INVALID_CONFIGURATION_NAME, "name=%s",
                     name);
    goto err;
  }
  ...
  if (conf_diagnostics) flags |= SSL_CONF_FLAG_SHOW_ERRORS;
  if (meth->ssl_accept != ssl_undefined_function) flags |= SSL_CONF_FLAG_SERVER;
  ...
  if (!SSL_CONF_CTX_finish(cctx)) ++err;
err:
