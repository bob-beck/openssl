...
static void
tls_int_free(OSSL_RECORD_LAYER* rl);

void
ossl_tls_buffer_release(TLS_BUFFER* b)
{
  OPENSSL_free(b->buf);
  b->buf = NULL;
}
...
static void
TLS_RL_RECORD_release(TLS_RL_RECORD* r, size_t num_recs)
{
  size_t i;
...
void
ossl_tls_rl_record_set_seq_num(TLS_RL_RECORD* r, const unsigned char* seq_num)
{
  memcpy(r->seq_num, seq_num, SEQ_NUM_SIZE);
}

void
ossl_rlayer_fatal(OSSL_RECORD_LAYER* rl,
                  int al,
                  int reason,
                  const char* fmt,
                  ...)
...
int
ossl_set_tls_provider_parameters(OSSL_RECORD_LAYER* rl,
                                 EVP_CIPHER_CTX* ctx,
...
static int
tls_allow_compression(OSSL_RECORD_LAYER* rl)
{
...
static void
tls_release_write_buffer_int(OSSL_RECORD_LAYER* rl, size_t start)
{
  TLS_BUFFER* wb;
  size_t pipes;
...
int
tls_setup_write_buffer(OSSL_RECORD_LAYER* rl,
                       size_t numwpipes,
                       size_t firstlen,
                       size_t nextlen)
{
...
static void
tls_release_write_buffer(OSSL_RECORD_LAYER* rl)
{
...
int
tls_setup_read_buffer(OSSL_RECORD_LAYER* rl)
{
  unsigned char* p;
...
static int
tls_release_read_buffer(OSSL_RECORD_LAYER* rl)
{
  TLS_BUFFER* b;
 ...
 */
int
tls_default_read_n(OSSL_RECORD_LAYER* rl,
                   size_t n,
                   size_t max,
                   int extend,
                   int clearold,
 ...
 */
static int
tls_record_app_data_waiting(OSSL_RECORD_LAYER* rl)
{
  TLS_BUFFER* rbuf;
  size_t left, len;
  unsigned char* p;
...
static int
rlayer_early_data_count_ok(OSSL_RECORD_LAYER* rl,
                           size_t length,
                           size_t overhead,
                           int send)
{
 ...
 */
int
tls_get_more_records(OSSL_RECORD_LAYER* rl)
{
  int enc_err, rret;
  int i;
  size_t more, n;
  ...
  if (rl->use_etm && rl->md_ctx != NULL) {
    ...
    for (j = 0; j < num_recs; j++) {
      ...
      if (i == 0 || CRYPTO_memcmp(md, mac, mac_size) != 0) {
        RLAYERfatal(
          rl, SSL_AD_BAD_RECORD_MAC, SSL_R_DECRYPTION_FAILED_OR_BAD_RECORD_MAC);
        return OSSL_RECORD_RETURN_FATAL;
      }
    }
    /*
...
int
tls_default_validate_record_header(OSSL_RECORD_LAYER* rl, TLS_RL_RECORD* rec)
{
...
int
tls_do_compress(OSSL_RECORD_LAYER* rl, TLS_RL_RECORD* wr)
{
#ifndef OPENSSL_NO_COMP
  int i;
...
int
tls_do_uncompress(OSSL_RECORD_LAYER* rl, TLS_RL_RECORD* rec)
{
#ifndef OPENSSL_NO_COMP
  int i;
  ...
  i = COMP_expand_block(rl->compctx,
                        rec->comp,
...
int
tls_default_post_process_record(OSSL_RECORD_LAYER* rl, TLS_RL_RECORD* rec)
{
  if (rl->compctx != NULL) {
...
int
tls13_common_post_process_record(OSSL_RECORD_LAYER* rl, TLS_RL_RECORD* rec)
{
  ...
  if (rl->msg_callback != NULL)
    rl->msg_callback(
      0, rl->version, SSL3_RT_INNER_CONTENT_TYPE, &rec->type, 1, rl->cbarg);
...
int
tls_read_record(OSSL_RECORD_LAYER* rl,
                void** rechandle,
                int* rversion,
                uint8_t* type,
...
int
tls_release_record(OSSL_RECORD_LAYER* rl, void* rechandle, size_t length)
{
...
int
tls_set_options(OSSL_RECORD_LAYER* rl, const OSSL_PARAM* options)
{
  const OSSL_PARAM* p;
...
int
tls_int_new_record_layer(OSSL_LIB_CTX* libctx,
                         const char* propq,
                         int vers,
                         int role,
                         int direction,
...
static int
tls_new_record_layer(OSSL_LIB_CTX* libctx,
                     const char* propq,
                     int vers,
                     int role,
                     int direction,
...
int
tls_unprocessed_read_pending(OSSL_RECORD_LAYER* rl)
{
  return TLS_BUFFER_get_left(&rl->rbuf) != 0;
}

int
tls_processed_read_pending(OSSL_RECORD_LAYER* rl)
{
  return rl->curr_rec < rl->num_recs;
}

size_t
tls_app_data_pending(OSSL_RECORD_LAYER* rl)
{
  size_t i;
  size_t num = 0;
...
size_t
tls_get_max_records_default(OSSL_RECORD_LAYER* rl,
                            uint8_t type,
                            size_t len,
                            size_t maxfrag,
                            size_t* preffrag)
...
size_t
tls_get_max_records(OSSL_RECORD_LAYER* rl,
                    uint8_t type,
                    size_t len,
                    size_t maxfrag,
                    size_t* preffrag)
...
int
tls_allocate_write_buffers_default(OSSL_RECORD_LAYER* rl,
...
int
tls_initialise_write_packets_default(OSSL_RECORD_LAYER* rl,
                                     OSSL_RECORD_TEMPLATE* templates,
                                     size_t numtempl,
                                     ...
                                     size_t* wpinited)
{
  ...
  for (j = 0; j < numtempl; j++) {
    ...
    if (!WPACKET_init_static_len(
          thispkt, TLS_BUFFER_get_buf(wb), TLS_BUFFER_get_len(wb), 0)) {
      RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
      return 0;
    }
    (*wpinited)++;
...
int
tls_prepare_record_header_default(OSSL_RECORD_LAYER* rl,
                                  WPACKET* thispkt,
...
int
tls_prepare_for_encryption_default(OSSL_RECORD_LAYER* rl,
                                   size_t mac_size,
                                   WPACKET* thispkt,
...
int
tls_post_encryption_processing_default(OSSL_RECORD_LAYER* rl,
                                       size_t mac_size,
...
int
tls_write_records_default(OSSL_RECORD_LAYER* rl,
                          OSSL_RECORD_TEMPLATE* templates,
                          size_t numtempl)
{
  ...
  for (j = 0; j < numtempl + prefix; j++) {
    ...
    if (!rl->funcs->prepare_record_header(
          rl, thispkt, thistempl, rectype, &compressdata)) {
      /* RLAYERfatal() already called */
      goto err;
    }
  ...
  for (j = 0; j < numtempl + prefix; j++) {
    ...
    if (!rl->funcs->post_encryption_processing(
          rl, mac_size, thistempl, thispkt, thiswr)) {
      /* RLAYERfatal() already called */
      goto err;
    }
...
int
tls_write_records(OSSL_RECORD_LAYER* rl,
                  OSSL_RECORD_TEMPLATE* templates,
                  size_t numtempl)
{
...
int
tls_retry_write_records(OSSL_RECORD_LAYER* rl)
{
  int i, ret;
  TLS_BUFFER* thiswb;
  size_t tmpwrit = 0;
...
int
tls_get_alert_code(OSSL_RECORD_LAYER* rl)
{
  return rl->alert;
}

int
tls_set1_bio(OSSL_RECORD_LAYER* rl, BIO* bio)
{
...
int
tls_default_set_protocol_version(OSSL_RECORD_LAYER* rl, int version)
{
...
int
tls_set_protocol_version(OSSL_RECORD_LAYER* rl, int version)
{
  return rl->funcs->set_protocol_version(rl, version);
}

void
tls_set_plain_alerts(OSSL_RECORD_LAYER* rl, int allow)
{
  rl->allow_plain_alerts = allow;
}

void
tls_set_first_handshake(OSSL_RECORD_LAYER* rl, int first)
{
  rl->is_first_handshake = first;
}

void
tls_set_max_pipelines(OSSL_RECORD_LAYER* rl, size_t max_pipelines)
{
...
void
tls_get_state(OSSL_RECORD_LAYER* rl,
              const char** shortstr,
              const char** longstr)
{
  const char *shrt, *lng;
...
const COMP_METHOD*
tls_get_compression(OSSL_RECORD_LAYER* rl)
{
#ifndef OPENSSL_NO_COMP
...
void
tls_set_max_frag_len(OSSL_RECORD_LAYER* rl, size_t max_frag_len)
{
...
int
tls_increment_sequence_ctr(OSSL_RECORD_LAYER* rl)
{
  int i;
...
int
tls_alloc_buffers(OSSL_RECORD_LAYER* rl)
{
...
int
tls_free_buffers(OSSL_RECORD_LAYER* rl)
{
  if (rl->direction == OSSL_RECORD_DIRECTION_WRITE) {
    if (rl->nextwbuf < rl->numwpipes) {
      /*
