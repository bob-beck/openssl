...
int ossl_set_tls_provider_parameters(OSSL_RECORD_LAYER* rl, EVP_CIPHER_CTX* ctx,
  ...
  if (imacsize > 0) macsize = (size_t)imacsize;
...
static int tls_allow_compression(OSSL_RECORD_LAYER* rl) {
  if (rl->options & SSL_OP_NO_COMPRESSION) return 0;
...
int tls_setup_write_buffer(OSSL_RECORD_LAYER* rl, size_t numwpipes,
  ...
  if (firstlen == 0 || (numwpipes > 1 && nextlen == 0)) {
    ...
    if (rl->version == TLS1_3_VERSION) contenttypelen = 1;
...
#ifndef OPENSSL_NO_COMP
    if (tls_allow_compression(rl)) defltlen += SSL3_RT_MAX_COMPRESSED_OVERHEAD;
#endif
    /*
  ...
  for (currpipe = 0; currpipe < numwpipes; currpipe++) {
    ...
    if (p == NULL) {
      ...
      if (p == NULL) {
        if (rl->numwpipes < currpipe) rl->numwpipes = currpipe;
        /*
...
int tls_setup_read_buffer(OSSL_RECORD_LAYER* rl) {
  ...
  if (b->buf == NULL) {
    len = rl->max_frag_len + SSL3_RT_MAX_ENCRYPTED_OVERHEAD + headerlen +
          maxalign;
#ifndef OPENSSL_NO_COMP
    if (tls_allow_compression(rl)) len += SSL3_RT_MAX_COMPRESSED_OVERHEAD;
#endif

    /* Ensure our buffer is large enough to support all our pipelines */
    if (rl->max_pipelines > 1) len *= rl->max_pipelines;

    if (b->default_len > len) len = b->default_len;
...
int tls_default_read_n(OSSL_RECORD_LAYER* rl, size_t n, size_t max, int extend,
  ...
  if (n == 0) return OSSL_RECORD_RETURN_NON_FATAL_ERR;
  ...
  if (!rl->read_ahead && !rl->isdtls) {
    ...
    if (max < n) max = n;
    if (max > rb->len - rb->offset) max = rb->len - rb->offset;
  }
  ...
  while (left < n) {
    ...
    if (ret <= OSSL_RECORD_RETURN_RETRY) {
      ...
      if ((rl->mode & SSL_MODE_RELEASE_BUFFERS) != 0 && !rl->isdtls)
        if (len + left == 0) tls_release_read_buffer(rl);
      return ret;
    }
    left += bioread;
    /*
     ...
     */
    if (rl->isdtls) {
      if (n > left) n = left; /* makes the while condition false */
    }
  }
...
int tls_get_more_records(OSSL_RECORD_LAYER* rl) {
  ...
  do {
    ...
    if ((rl->rstate != SSL_ST_READ_BODY) ||
      ...
      rret =
          rl->funcs->read_n(rl, SSL3_RT_HEADER_LENGTH, TLS_BUFFER_get_len(rbuf),
                            0, num_recs == 0 ? 1 : 0, &n);
  ...
  if (macbufs != NULL) {
    for (j = 0; j < num_recs; j++) {
      if (macbufs[j].alloced) OPENSSL_free(macbufs[j].mac);
    }
...
int tls_default_validate_record_header(OSSL_RECORD_LAYER* rl,
   ...
   */
  if (rl->compctx == NULL) len -= SSL3_RT_MAX_COMPRESSED_OVERHEAD;
#endif
...
int tls_read_record(OSSL_RECORD_LAYER* rl, void** rechandle, int* rversion,
  ...
  while (rl->curr_rec >= rl->num_recs) {
    ...
    if (ret != OSSL_RECORD_RETURN_SUCCESS) return ret;
  }
...
int tls_release_record(OSSL_RECORD_LAYER* rl, void* rechandle, size_t length) {
  ...
  if (rec->length > 0) return OSSL_RECORD_RETURN_SUCCESS;
...
int tls_int_new_record_layer(
  ...
  *retrl = NULL;
  ...
  if (rl == NULL) return OSSL_RECORD_RETURN_FATAL;
  ...
  if (!tls_set1_bio(rl, transport)) goto err;

  if (prev != NULL && !BIO_up_ref(prev)) goto err;
  rl->prev = prev;

  if (next != NULL && !BIO_up_ref(next)) goto err;
  rl->next = next;
...
size_t tls_app_data_pending(OSSL_RECORD_LAYER* rl) {
  ...
  for (i = rl->curr_rec; i < rl->num_recs; i++) {
    if (rl->rrec[i].type != SSL3_RT_APPLICATION_DATA) return num;
...
int tls_prepare_record_header_default(OSSL_RECORD_LAYER* rl, WPACKET* thispkt,
  ...
  *recdata = NULL;
  ...
  if (rl->compctx != NULL) maxcomplen += SSL3_RT_MAX_COMPRESSED_OVERHEAD;
...
int tls_retry_write_records(OSSL_RECORD_LAYER* rl) {
  ...
  if (rl->nextwbuf >= rl->numwpipes) return OSSL_RECORD_RETURN_SUCCESS;
  ...
  for (;;) {
    ...
    if (i >= 0 && tmpwrit == TLS_BUFFER_get_left(thiswb)) {
      ...
      if (++(rl->nextwbuf) < rl->numwpipes) continue;
...
int tls_set1_bio(OSSL_RECORD_LAYER* rl, BIO* bio) {
  if (bio != NULL && !BIO_up_ref(bio)) return 0;
  BIO_free(rl->bio);
  rl->bio = bio;
...
int tls_default_set_protocol_version(OSSL_RECORD_LAYER* rl, int version) {
  if (rl->version != version) return 0;
...
void tls_set_max_pipelines(OSSL_RECORD_LAYER* rl, size_t max_pipelines) {
  ...
  if (max_pipelines > 1) rl->read_ahead = 1;
}
...
void tls_get_state(OSSL_RECORD_LAYER* rl, const char** shortstr,
  ...
  if (shortstr != NULL) *shortstr = shrt;
  if (longstr != NULL) *longstr = lng;
}
...
int tls_alloc_buffers(OSSL_RECORD_LAYER* rl) {
  if (rl->direction == OSSL_RECORD_DIRECTION_WRITE) {
    ...
    if (rl->nextwbuf < rl->numwpipes) return 1;
    /*
     ...
     * demand
     */
    if (!tls_setup_write_buffer(rl, 1, 0, 0)) return 0;
