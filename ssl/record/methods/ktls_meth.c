...
static int ktls_int_check_supported_cipher(OSSL_RECORD_LAYER* rl,
  ...
  if (EVP_CIPHER_is_a(c, "AES-128-CCM")) {
    if (taglen != EVP_CCM_TLS_TAG_LEN) return 0;
    return 1;
  } else
#endif
      if (0
...
static int ktls_configure_crypto(
  ...
  switch (EVP_CIPHER_get_nid(c)) {
    ...
    case NID_chacha20_poly1305:
      if (!ossl_assert(ivlen == TLS_CIPHER_CHACHA20_POLY1305_IV_SIZE)) return 0;
...
static int ktls_set_crypto_state(OSSL_RECORD_LAYER* rl, int level,
  ...
  if (comp != NULL) return OSSL_RECORD_RETURN_NON_FATAL_ERR;
  ...
  if (rl->direction == OSSL_RECORD_DIRECTION_WRITE) {
    if (BIO_flush(rl->bio) <= 0) return OSSL_RECORD_RETURN_NON_FATAL_ERR;
...
static int ktls_allocate_write_buffers(OSSL_RECORD_LAYER* rl,
  ...
  if (!ossl_assert(numtempl == 1)) return 0;
...
static int ktls_alloc_buffers(OSSL_RECORD_LAYER* rl) {
  ...
  if (rl->direction == OSSL_RECORD_DIRECTION_WRITE) return 1;
