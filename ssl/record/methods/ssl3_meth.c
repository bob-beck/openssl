...
static int
ssl3_set_crypto_state(OSSL_RECORD_LAYER* rl,
                      int level,
                      unsigned char* key,
                      size_t keylen,
                      unsigned char* iv,
 ...
 */
static int
ssl3_cipher(OSSL_RECORD_LAYER* rl,
  ...
  if (provided) {
    ...
    if (!sending && mac != NULL) {
      ...
      *p++ = OSSL_PARAM_construct_octet_ptr(
        OSSL_CIPHER_PARAM_TLS_MAC, (void**)&mac->mac, macsize);
