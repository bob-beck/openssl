...
static int
ssl3_set_crypto_state (OSSL_RECORD_LAYER *rl, int level, unsigned char *key,
...
/*
 * ssl3_cipher encrypts/decrypts |n_recs| records in |inrecs|. Calls
 * RLAYERfatal on internal error, but not otherwise. It is the responsibility
 * of the caller to report a bad_record_mac
 *
 * Returns:
 ...
 */
static int
ssl3_cipher (OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *inrecs, size_t n_recs,
...
static int
ssl3_mac (OSSL_RECORD_LAYER *rl, TLS_RL_RECORD *rec, unsigned char *md,
          int sending)
{
