...
int tls1_cbc_remove_padding_and_mac(size_t* reclen, size_t origreclen,
  ...
  if (block_size != 1) {
    ...
    if (to_check > *reclen) to_check = *reclen;
...
static int ssl3_cbc_copy_mac(size_t* reclen, size_t origreclen,
  ...
  *reclen -= mac_size;
  ...
  if (block_size == 1) {
    ...
    if (mac != NULL) *mac = &recdata[*reclen];
    if (alloced != NULL) *alloced = 0;
    return 1;
  }

  /* Create the random MAC we will emit if padding is bad */
  if (RAND_bytes_ex(libctx, randmac, mac_size, 0) <= 0) return 0;
  ...
  *alloced = 1;
  ...
  for (i = 0; i < mac_size; i++) {
    for (j = 0; j < mac_size; j++)
      out[j] |=
          rotated_mac[i] &
          constant_time_eq_8_s((unsigned int)j, (unsigned int)rotate_offset);
    rotate_offset++;
    rotate_offset &= constant_time_lt_s(rotate_offset, mac_size);
