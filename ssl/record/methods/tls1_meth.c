...
static int
tls1_set_crypto_state(OSSL_RECORD_LAYER* rl,
                      int level,
                      unsigned char* key,
                      size_t keylen,
                      unsigned char* iv,
                      ...
                      COMP_METHOD* comp)
{
  ...
  if ((EVP_CIPHER_get_flags(ciph) & EVP_CIPH_FLAG_AEAD_CIPHER) == 0) {
    ...
    if (mac_key == NULL || EVP_DigestSignInit_ex(rl->md_ctx,
                                                 NULL,
                                                 EVP_MD_get0_name(md),
                                                 rl->libctx,
                                                 rl->propq,
  ...
  if (EVP_CIPHER_get_mode(ciph) == EVP_CIPH_GCM_MODE) {
    if (!EVP_CipherInit_ex(ciph_ctx, ciph, NULL, key, NULL, enc) ||
        EVP_CIPHER_CTX_ctrl(
          ciph_ctx, EVP_CTRL_GCM_SET_IV_FIXED, (int)ivlen, iv) <= 0) {
      ERR_raise(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR);
      return OSSL_RECORD_RETURN_FATAL;
    }
    ...
    if (!EVP_CipherInit_ex(ciph_ctx, ciph, NULL, NULL, NULL, enc) ||
        ...
        EVP_CIPHER_CTX_ctrl(
          ciph_ctx, EVP_CTRL_AEAD_SET_TAG, (int)taglen, NULL) <= 0 ||
        EVP_CIPHER_CTX_ctrl(
          ciph_ctx, EVP_CTRL_CCM_SET_IV_FIXED, (int)ivlen, iv) <= 0 ||
  ...
  if ((EVP_CIPHER_get_flags(ciph) & EVP_CIPH_FLAG_AEAD_CIPHER) != 0 &&
      mackeylen != 0 &&
      EVP_CIPHER_CTX_ctrl(
        ciph_ctx, EVP_CTRL_AEAD_SET_MAC_KEY, (int)mackeylen, mackey) <= 0) {
    ERR_raise(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR);
    return OSSL_RECORD_RETURN_FATAL;
  }
 ...
 */
static int
tls1_cipher(OSSL_RECORD_LAYER* rl,
            TLS_RL_RECORD* recs,
            size_t n_recs,
            int sending,
            SSL_MAC_BUF* macs,
            size_t macsize)
{
  ...
  for (ctr = 0; ctr < n_recs; ctr++) {
    ...
    if ((EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(ds)) &
      ...
      pad = EVP_CIPHER_CTX_ctrl(
        ds, EVP_CTRL_AEAD_TLS1_AAD, EVP_AEAD_TLS1_AAD_LEN, buf[ctr]);
      if (pad <= 0) {
        RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return 0;
      }
  ...
  if (n_recs > 1) {
    ...
    if (EVP_CIPHER_CTX_ctrl(
          ds, EVP_CTRL_SET_PIPELINE_OUTPUT_BUFS, (int)n_recs, data) <= 0) {
      RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR, SSL_R_PIPELINE_FAILURE);
      return 0;
    }
    ...
    if (EVP_CIPHER_CTX_ctrl(
          ds, EVP_CTRL_SET_PIPELINE_INPUT_BUFS, (int)n_recs, data) <= 0 ||
        EVP_CIPHER_CTX_ctrl(
          ds, EVP_CTRL_SET_PIPELINE_INPUT_LENS, (int)n_recs, reclen) <= 0) {
      RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR, SSL_R_PIPELINE_FAILURE);
      return 0;
    }
  }
  ...
  if (!rl->isdtls && rl->tlstree) {
    ...
    if (EVP_CIPHER_CTX_ctrl(
          ds, EVP_CTRL_TLSTREE, decrement_seq, rl->sequence) <= 0) {
  ...
  if (provided) {
    ...
    if (!EVP_CipherUpdate(
          ds, recs[0].data, &outlen, recs[0].input, (unsigned int)reclen[0]))
      return 0;
                            ...
                            size_t* prefix)
{
  ...
  *prefix =
   ...
   */
  if (!tls_setup_write_buffer(
        rl, numtempl + *prefix, *prefix ? MAX_PREFIX_LEN : 0, 0)) {
    /* RLAYERfatal() already called */
    return 0;
  }
                              ...
                              size_t* wpinited)
{
  ...
  if (prefix) {
    ...
    if (!WPACKET_init_static_len(
          &pkt[0], TLS_BUFFER_get_buf(wb), TLS_BUFFER_get_len(wb), 0)) {
      RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
      return 0;
    }
    *wpinited = 1;
