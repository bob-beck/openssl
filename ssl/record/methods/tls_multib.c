...
static int
tls_is_multiblock_capable(OSSL_RECORD_LAYER* rl,
                          uint8_t type,
                          size_t len,
                          size_t fraglen)
{
 ...
 */
static int
tls_write_records_multiblock_int(OSSL_RECORD_LAYER* rl,
                                 ...
                                 size_t numtempl)
{
  ...
  if (!tls_is_multiblock_capable(
        rl, templates[0].type, totlen, templates[0].buflen))
    return 0;
  ...
  packleni = EVP_CIPHER_CTX_ctrl(
    rl->enc_ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_AAD, sizeof(mb_param), &mb_param);
