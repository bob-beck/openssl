...
static int tls13_set_crypto_state(OSSL_RECORD_LAYER* rl, int level,
  ...
  if (rl->iv == NULL) return OSSL_RECORD_RETURN_FATAL;

  rl->nonce = OPENSSL_malloc(ivlen);
  if (rl->nonce == NULL) return OSSL_RECORD_RETURN_FATAL;
...
static int tls13_cipher(OSSL_RECORD_LAYER* rl, TLS_RL_RECORD* recs,
  ...
  if (!sending) {
     ...
     * well as the tag
     */
    if (rec->length < rl->taglen + 1) return 0;
  ...
  if (EVP_CipherInit_ex(enc_ctx, NULL, NULL, NULL, nonce, sending) <= 0 ||
      (!sending &&
       EVP_CIPHER_CTX_ctrl(enc_ctx, EVP_CTRL_AEAD_SET_TAG, (int)rl->taglen,
...
static int tls13_post_process_record(OSSL_RECORD_LAYER* rl,
  ...
  if (rec->type != SSL3_RT_ALERT) {
    ...
    for (end = rec->length - 1; end > 0 && rec->data[end] == 0; end--) continue;
...
static int tls13_add_record_padding(OSSL_RECORD_LAYER* rl,
  ...
  if (rl->allow_plain_alerts && thistempl->type != SSL3_RT_ALERT) return 1;
