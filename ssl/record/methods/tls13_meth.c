...
static int
tls13_set_crypto_state(OSSL_RECORD_LAYER* rl,
                       int level,
                       unsigned char* key,
                       size_t keylen,
                       unsigned char* iv,
                       ...
                       COMP_METHOD* comp)
{
  ...
  if (EVP_CipherInit_ex(ciph_ctx, ciph, NULL, NULL, NULL, enc) <= 0 ||
      EVP_CIPHER_CTX_ctrl(
        ciph_ctx, EVP_CTRL_AEAD_SET_IVLEN, (int)ivlen, NULL) <= 0 ||
...
static int
tls13_cipher(OSSL_RECORD_LAYER* rl,
             TLS_RL_RECORD* recs,
             size_t n_recs,
             int sending,
             SSL_MAC_BUF* mac,
             size_t macsize)
{
  ...
  if (EVP_CipherInit_ex(enc_ctx, NULL, NULL, NULL, nonce, sending) <= 0 ||
      (!sending && EVP_CIPHER_CTX_ctrl(enc_ctx,
  ...
  if ((mode == EVP_CIPH_CCM_MODE &&
       EVP_CipherUpdate(
         enc_ctx, NULL, &lenu, NULL, (unsigned int)rec->length) <= 0) ||
      EVP_CipherUpdate(enc_ctx, NULL, &lenu, recheader, sizeof(recheader)) <=
        0 ||
      EVP_CipherUpdate(
        enc_ctx, rec->data, &lenu, rec->input, (unsigned int)rec->length) <=
        0 ||
  ...
  if (sending) {
    /* Add the tag */
    if (EVP_CIPHER_CTX_ctrl(enc_ctx,
...
static int
tls13_validate_record_header(OSSL_RECORD_LAYER* rl, TLS_RL_RECORD* rec)
{
...
static int
tls13_post_process_record(OSSL_RECORD_LAYER* rl, TLS_RL_RECORD* rec)
{
...
static uint8_t
tls13_get_record_type(OSSL_RECORD_LAYER* rl, OSSL_RECORD_TEMPLATE* template)
{
...
static int
tls13_add_record_padding(OSSL_RECORD_LAYER* rl,
