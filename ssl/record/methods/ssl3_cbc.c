 ...
 *   data_size: the secret, reported length of the data once the MAC and
 * padding has been removed. data_plus_mac_plus_padding_size: the public length
 * of the whole record, including MAC and padding. is_sslv3: non-zero if we are
 * to use SSLv3. Otherwise, TLS.
 *
 * On entry: we know that data is data_plus_mac_plus_padding_size in length
 * Returns 1 on success or 0 on error
 */
int
ssl3_cbc_digest_record (const EVP_MD *md, unsigned char *md_out,
