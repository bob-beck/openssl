...
int ssl3_cbc_digest_record(const EVP_MD* md, unsigned char* md_out,
   ...
   */
  if (!ossl_assert(data_plus_mac_plus_padding_size < 1024 * 1024)) return 0;

  if (EVP_MD_is_a(md, "MD5")) {
#ifdef FIPS_MODULE
    return 0;
#else
    if (MD5_Init((MD5_CTX*)md_state.c) <= 0) return 0;
    ...
    if (SHA224_Init((SHA256_CTX*)md_state.c) <= 0) return 0;
    md_final_raw = tls1_sha256_final_raw;
    md_transform =
        (void (*)(void* ctx, const unsigned char* block))SHA256_Transform;
    md_size = 224 / 8;
  } else if (EVP_MD_is_a(md, "SHA2-256")) {
    if (SHA256_Init((SHA256_CTX*)md_state.c) <= 0) return 0;
    md_final_raw = tls1_sha256_final_raw;
    md_transform =
        (void (*)(void* ctx, const unsigned char* block))SHA256_Transform;
    md_size = 32;
    ...
    if (SHA512_Init((SHA512_CTX*)md_state.c) <= 0) return 0;
    md_final_raw = tls1_sha512_final_raw;
    md_transform =
        (void (*)(void* ctx, const unsigned char* block))SHA512_Transform;
    md_size = 64;
     ...
     */
    if (md_out_size != NULL) *md_out_size = 0;
  ...
  if (!is_sslv3) {
    ...
    if (!ossl_assert(mac_secret_length <= sizeof(hmac_pad))) return 0;
  ...
  if (EVP_DigestInit_ex(md_ctx, md, NULL /* engine */) <= 0) goto err;
  if (is_sslv3) {
  ...
  if (ret && md_out_size) *md_out_size = md_out_size_u;
