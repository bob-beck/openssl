...
static void dtls_record_bitmap_update(OSSL_RECORD_LAYER* rl,
  ...
  if (cmp > 0) {
    ...
    shift = -cmp;
    if (shift < sizeof(bitmap->map) * 8) bitmap->map |= (uint64_t)1 << shift;
  }
}
...
static DTLS_BITMAP* dtls_get_bitmap(OSSL_RECORD_LAYER* rl, TLS_RL_RECORD* rr,
  ...
  *is_next_epoch = 0;
  ...
  if (rr->epoch == rl->epoch) return &rl->bitmap;
...
static int dtls_process_record(OSSL_RECORD_LAYER* rl, DTLS_BITMAP* bitmap) {
  ...
  if (!rl->use_etm && (rl->enc_ctx != NULL) &&
    ...
    if (i == 0 || macbuf.mac == NULL ||
      ...
      enc_err = 0;
    if (rr->length > SSL3_RT_MAX_COMPRESSED_LENGTH + mac_size) enc_err = 0;
  }
  ...
  if (macbuf.alloced) OPENSSL_free(macbuf.mac);
  return ret;
}
