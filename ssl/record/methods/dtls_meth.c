...
static int
dtls_record_replay_check(OSSL_RECORD_LAYER* rl, DTLS_BITMAP* bitmap)
{
  int cmp;
  unsigned int shift;
...
static void
dtls_record_bitmap_update(OSSL_RECORD_LAYER* rl, DTLS_BITMAP* bitmap)
{
  int cmp;
  unsigned int shift;
  const unsigned char* seq = rl->sequence;
...
static DTLS_BITMAP*
dtls_get_bitmap(OSSL_RECORD_LAYER* rl,
                TLS_RL_RECORD* rr,
...
static void
dtls_set_in_init(OSSL_RECORD_LAYER* rl, int in_init)
{
  rl->in_init = in_init;
}

static int
dtls_process_record(OSSL_RECORD_LAYER* rl, DTLS_BITMAP* bitmap)
{
  int i;
  int enc_err;
  TLS_RL_RECORD* rr;
...
static int
dtls_rlayer_buffer_record(OSSL_RECORD_LAYER* rl,
...
static int
dtls_copy_rlayer_record(OSSL_RECORD_LAYER* rl, pitem* item)
{
  DTLS_RLAYER_RECORD_DATA* rdata;
...
static int
dtls_retrieve_rlayer_buffered_record(OSSL_RECORD_LAYER* rl,
                                     struct pqueue_st* queue)
{
  pitem* item;
...
again:
  ...
  if ((rl->rstate != SSL_ST_READ_BODY) ||
    ...
    rret = rl->funcs->read_n(
      rl, DTLS1_RT_HEADER_LENGTH, TLS_BUFFER_get_len(&rl->rbuf), 0, 1, &n);
...
static int
dtls_free(OSSL_RECORD_LAYER* rl)
{
  TLS_BUFFER* rbuf;
  size_t left, written;
  pitem* item;
...
static int
dtls_new_record_layer(OSSL_LIB_CTX* libctx,
                      const char* propq,
                      int vers,
                      int role,
                      int direction,
...
{
  if (!tls_post_encryption_processing_default(
...
static size_t
dtls_get_max_record_overhead(OSSL_RECORD_LAYER* rl)
{
  size_t blocksize = 0;
