...
start:
    ...
    if (sc->rlayer.curr_rec >= sc->rlayer.num_recs)
    {
        ...
        do
        {
            ...
            ret = HANDLE_RLAYER_READ_RETURN(
                sc, sc->rlayer.rrlmethod->read_record(sc->rlayer.rrl, &rr->rechandle, &rr->version, &rr->type,
    ...
    if (type == rr->type || (rr->type == SSL3_RT_CHANGE_CIPHER_SPEC && type == SSL3_RT_HANDSHAKE && recvd_type != NULL))
    {
        /*
