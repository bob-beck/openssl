...
int
DTLS_RECORD_LAYER_new (RECORD_LAYER *rl)
{
  DTLS_RECORD_LAYER *d;
...
void
DTLS_RECORD_LAYER_free (RECORD_LAYER *rl)
{
  if (rl->d == NULL)
    return;
...
void
DTLS_RECORD_LAYER_clear (RECORD_LAYER *rl)
{
  DTLS_RECORD_LAYER *d;
  pitem *item = NULL;
  TLS_RECORD *rec;
...
static int
dtls_buffer_record (SSL_CONNECTION *s, TLS_RECORD *rec)
{
  TLS_RECORD *rdata;
  pitem *item;
...
static void
dtls_unbuffer_record (SSL_CONNECTION *s)
{
  TLS_RECORD *rdata;
  pitem *item;
 ...
 * *if* the |recd_type| argument is non NULL. Also if record payloads contain
 ...
 */
int
dtls1_read_bytes (SSL *s, uint8_t type, uint8_t *recvd_type,
 ...
 */
int
dtls1_write_bytes (SSL_CONNECTION *s, uint8_t type, const void *buf,
...
int
do_dtls1_write (SSL_CONNECTION *sc, uint8_t type, const unsigned char *buf,
                size_t len, size_t *written)
{
  int i;
...
void
dtls1_increment_epoch (SSL_CONNECTION *s, int rw)
{
  if (rw & SSL3_CC_READ)
    {
      s->rlayer.d->r_epoch++;
...
uint16_t
dtls1_get_epoch (SSL_CONNECTION *s, int rw)
{
  uint16_t epoch;
