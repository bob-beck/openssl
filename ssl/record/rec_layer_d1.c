...
static int dtls_buffer_record(SSL_CONNECTION* s, TLS_RECORD* rec) {
  ...
  if (pqueue_size(queue) >= 100) return 0;

  /* We don't buffer partially read records */
  if (!ossl_assert(rec->off == 0)) return -1;
...
static void dtls_unbuffer_record(SSL_CONNECTION* s) {
  ...
  if (s->rlayer.curr_rec < s->rlayer.num_recs) return;
...
start:
   ...
   */
  if (SSL_is_init_finished(s)) dtls_unbuffer_record(sc);
   ...
   */
  if (rr->type != SSL3_RT_ALERT && rr->length != 0) sc->rlayer.alert_count = 0;
  ...
  if (sc->s3.change_cipher_spec /* set when we receive ChangeCipherSpec,
    ...
    if (!ssl_release_record(sc, rr, 0)) return -1;
    goto start;
  }
  ...
  if (type == rr->type || (rr->type == SSL3_RT_CHANGE_CIPHER_SPEC &&
    ...
    if (len == 0) {
       ...
       */
      if (rr->length == 0 && !ssl_release_record(sc, rr, 0)) return -1;
      return 0;
    }
    ...
    if (peek) {
      ...
      if (!ssl_release_record(sc, rr, n)) return -1;
    }
#ifndef OPENSSL_NO_SCTP
    /*
  ...
  if (rr->type == SSL3_RT_HANDSHAKE && !ossl_statem_get_in_handshake(sc)) {
    ...
    if (msg_hdr.type == SSL3_MT_FINISHED) {
      ...
      if (dtls1_retransmit_buffered_messages(sc) <= 0) {
        ...
        if (ossl_statem_in_error(sc)) return -1;
      }
      if (!ssl_release_record(sc, rr, 0)) return -1;
