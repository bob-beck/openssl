...
static int
ossl_early_data_count_ok(SSL_CONNECTION* s,
                         size_t length,
                         size_t overhead,
                         int send)
{
...
static int
tls_write_check_pending(SSL_CONNECTION* s,
                        uint8_t type,
...
int
ossl_tls_handle_rlayer_return(SSL_CONNECTION* s,
                              int writing,
                              int ret,
                              char* file,
                              int line)
  ...
  if (ret == OSSL_RECORD_RETURN_RETRY) {
    ...
    if (ret == OSSL_RECORD_RETURN_EOF) {
      if (writing) {
         ...
         */
        ossl_statem_fatal(
          s, SSL_AD_DECODE_ERROR, SSL_R_UNEXPECTED_EOF_WHILE_READING, NULL);
      }
...
start:
  ...
  if (rr->type == SSL3_RT_ALERT) {
    ...
    if (s->msg_callback)
      s->msg_callback(
        0, s->version, SSL3_RT_ALERT, alert_bytes, 2, ssl, s->msg_callback_arg);
    ...
    case SSL3_RT_APPLICATION_DATA:
      ...
      if (ossl_statem_app_data_allowed(s)) {
        ...
        if (!ossl_early_data_count_ok(
              s, rr->length, EARLY_DATA_CIPHERTEXT_OVERHEAD, 0)) {
          /* SSLfatal() already called */
          return -1;
        }
        if (!ssl_release_record(s, rr, 0))
...
static void
rlayer_msg_callback_wrapper(int write_p,
                            int version,
                            int content_type,
                            const void* buf,
                            size_t len,
                            void* cbarg)
{
  ...
  if (s->msg_callback != NULL)
    s->msg_callback(
      write_p, version, content_type, buf, len, ssl, s->msg_callback_arg);
}
...
rlayer_padding_wrapper(void* cbarg, int type, size_t len)
{
  ...
  return s->rlayer.record_padding_cb(
    ssl, type, len, s->rlayer.record_padding_arg);
}
...
int
ssl_set_new_record_layer(SSL_CONNECTION* s,
                         int version,
                         int direction,
                         int level,
