...
size_t ssl3_pending(const SSL* s) {
  ...
  for (i = 0; i < sc->rlayer.num_recs; i++) {
    if (sc->rlayer.tlsrecs[i].type != SSL3_RT_APPLICATION_DATA) return num;
...
const char* SSL_rstate_string_long(const SSL* s) {
  ...
  if (sc->rlayer.rrlmethod == NULL || sc->rlayer.rrl == NULL) return "unknown";
...
static int tls_write_check_pending(SSL_CONNECTION* s, uint8_t type,
  ...
  if (s->rlayer.wpend_tot == 0) return 0;
...
int ssl3_write_bytes(SSL* ssl, uint8_t type, const void* buf_, size_t len,
  ...
  for (;;) {
    ...
    if (maxpipes > SSL_MAX_PIPELINES) maxpipes = SSL_MAX_PIPELINES;
...
start:
   ...
   */
  if (rr->type != SSL3_RT_ALERT && rr->length != 0) s->rlayer.alert_count = 0;
  ...
  if (type == rr->type ||
    ...
    if (recvd_type != NULL) *recvd_type = rr->type;
    ...
    if (len == 0) {
       ...
       */
      if (rr->length == 0 && !ssl_release_record(s, rr, 0)) return -1;
    ...
    do {
      ...
      if (peek) {
        ...
        if (!ssl_release_record(s, rr, n)) return -1;
      }
  ...
  if (rr->type == SSL3_RT_ALERT) {
    ...
    if ((!is_tls13 && alert_level == SSL3_AL_WARNING) ||
      ...
      if (!ssl_release_record(s, rr, 0)) return -1;
  ...
  if ((s->shutdown & SSL_SENT_SHUTDOWN) != 0) {
    if (rr->type == SSL3_RT_HANDSHAKE) {
      ...
      if (!SSL_CONNECTION_IS_TLS13(s)) {
        ...
        if ((s->mode & SSL_MODE_AUTO_RETRY) != 0) goto start;
  ...
  if (rr->type == SSL3_RT_HANDSHAKE) {
    ...
    if (rr->length < n) n = rr->length; /* available bytes */
     ...
     * is zero length
     */
    if ((n > 0 || rr->length == 0) && !ssl_release_record(s, rr, n)) return -1;

    if (*dest_len < dest_maxlen) goto start; /* fragment was too small */
  }
...
int RECORD_LAYER_is_sslv2_record(RECORD_LAYER* rl) {
  if (SSL_CONNECTION_IS_DTLS(rl->s)) return 0;
...
static const OSSL_RECORD_METHOD* ssl_select_next_record_layer(SSL_CONNECTION* s,
                                                              int direction,
                                                              int level) {
  if (s->rlayer.custom_rlmethod != NULL) return s->rlayer.custom_rlmethod;

  if (level == OSSL_RECORD_PROTECTION_LEVEL_NONE) {
    if (SSL_CONNECTION_IS_DTLS(s)) return &ossl_dtls_record_method;
...
int ssl_set_new_record_layer(SSL_CONNECTION* s, int version, int direction,
  ...
  *opts = OSSL_PARAM_construct_end();
  ...
  if (direction == OSSL_RECORD_DIRECTION_READ) {
    ...
    if ((s->mac_flags & SSL_MAC_FLAG_READ_MAC_STREAM) != 0) stream_mac = 1;

    if ((s->mac_flags & SSL_MAC_FLAG_READ_MAC_TLSTREE) != 0) tlstree = 1;
  } else {
    use_etm = SSL_WRITE_ETM(s) ? 1 : 0;
    if ((s->mac_flags & SSL_MAC_FLAG_WRITE_MAC_STREAM) != 0) stream_mac = 1;

    if ((s->mac_flags & SSL_MAC_FLAG_WRITE_MAC_TLSTREE) != 0) tlstree = 1;
  }
    ...
    for (i = 0, j = 0; i < OSSL_NELEM(rlayer_dispatch); i++) {
      switch (rlayer_dispatch[i].function_id) {
        case OSSL_FUNC_RLAYER_MSG_CALLBACK:
          if (s->msg_callback == NULL) continue;
          break;
        case OSSL_FUNC_RLAYER_PADDING:
          if (s->rlayer.record_padding_cb == NULL) continue;
          break;
        default:
          break;
      }
