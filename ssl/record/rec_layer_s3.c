...
void
RECORD_LAYER_init (RECORD_LAYER *rl, SSL_CONNECTION *s)
{
  rl->s = s;
}

int
RECORD_LAYER_clear (RECORD_LAYER *rl)
{
  int ret = 1;
...
int
RECORD_LAYER_reset (RECORD_LAYER *rl)
{
  int ret;
...
int
RECORD_LAYER_read_pending (const RECORD_LAYER *rl)
{
...
int
RECORD_LAYER_processed_read_pending (const RECORD_LAYER *rl)
{
...
static uint32_t
ossl_get_max_early_data (SSL_CONNECTION *s)
{
...
static int
ossl_early_data_count_ok (SSL_CONNECTION *s, size_t length, size_t overhead,
                          int send)
{
  uint32_t max_early_data;
...
void
SSL_CTX_set_default_read_buffer_len (SSL_CTX *ctx, size_t len)
{
  ctx->default_read_buf_len = len;
}

void
SSL_set_default_read_buffer_len (SSL *s, size_t len)
{
...
static int
tls_write_check_pending (SSL_CONNECTION *s, uint8_t type,
 ...
 */
int
ssl3_write_bytes (SSL *ssl, uint8_t type, const void *buf_, size_t len,
                  size_t *written)
{
  ...
  for (;;)
    {
      ...
      i = HANDLE_RLAYER_WRITE_RETURN (s, s->rlayer.wrlmethod->write_records (
                                             s->rlayer.wrl, tmpls, maxpipes));
      if (i <= 0)
        {
          /* SSLfatal() already called if appropriate */
          s->rlayer.wnum = tot;
...
int
ossl_tls_handle_rlayer_return (SSL_CONNECTION *s, int writing, int ret,
                               char *file, int line)
{
 ...
 */
int
ssl_release_record (SSL_CONNECTION *s, TLS_RECORD *rr, size_t length)
{
  ...
  if (rr->rechandle != NULL)
    {
      ...
      if (HANDLE_RLAYER_READ_RETURN (
              s, s->rlayer.rrlmethod->release_record (s->rlayer.rrl,
                                                      rr->rechandle, length))
          <= 0)
        {
 ...
 * ChangeCipherSpec messages are treated as if they were handshake messages
 * *if* the |recvd_type| argument is non NULL. Also if record payloads contain
 * fragments too small to process, we store them until there is enough for the
 * respective protocol (the record protocol may use arbitrary fragmentation and
 * even interleaving): Change cipher spec protocol just 1 byte needed, no need
 * for keeping anything stored Alert protocol 2 bytes needed (AlertLevel,
 * AlertDescription) Handshake protocol 4 bytes needed (HandshakeType, uint24
 * length) -- we just have to detect unexpected Client Hello and Hello Request
 * messages here, anything else is handled by higher layers Application data
 * protocol none of our business
 */
int
ssl3_read_bytes (SSL *ssl, uint8_t type, uint8_t *recvd_type,
...
start:
  ...
  if (type == rr->type
          ...
          && recvd_type != NULL && !is_tls13))
    {
      ...
      while (type == SSL3_RT_APPLICATION_DATA && curr_rec < s->rlayer.num_recs
             && totalbytes < len);
      if (totalbytes == 0)
        {
 ...
 */
int
RECORD_LAYER_is_sslv2_record (RECORD_LAYER *rl)
{
...
static void
rlayer_msg_callback_wrapper (int write_p, int version, int content_type,
...
static int
rlayer_security_wrapper (void *cbarg, int op, int bits, int nid, void *other)
{
  SSL_CONNECTION *s = cbarg;
...
static size_t
rlayer_padding_wrapper (void *cbarg, int type, size_t len)
{
  SSL_CONNECTION *s = cbarg;
  SSL *ssl = SSL_CONNECTION_GET_USER_SSL (s);
...
ossl_ssl_set_custom_record_layer (SSL_CONNECTION *s,
                                  const OSSL_RECORD_METHOD *meth, void *rlarg)
{
...
static int
ssl_post_record_layer_select (SSL_CONNECTION *s, int direction)
{
...
int
ssl_set_new_record_layer (SSL_CONNECTION *s, int version, int direction,
                          int level, unsigned char *secret, size_t secretlen,
                          unsigned char *key, size_t keylen, unsigned char *iv,
                          size_t ivlen, unsigned char *mackey,
                          size_t mackeylen, const EVP_CIPHER *ciph,
                          size_t taglen, int mactype, const EVP_MD *md,
...
int
ssl_set_record_protocol_version (SSL_CONNECTION *s, int vers)
{
