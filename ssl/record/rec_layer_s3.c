...
start:
    ...
    if (s->rlayer.curr_rec >= s->rlayer.num_recs)
    {
        ...
        do
        {
            ...
            ret = HANDLE_RLAYER_READ_RETURN(s, s->rlayer.rrlmethod->read_record(s->rlayer.rrl, &rr->rechandle,
        ...
        (rr->type == SSL3_RT_CHANGE_CIPHER_SPEC && type == SSL3_RT_HANDSHAKE && recvd_type != NULL && !is_tls13))
    {
        ...
        if (type == SSL3_RT_HANDSHAKE && rr->type == SSL3_RT_CHANGE_CIPHER_SPEC && s->rlayer.handshake_fragment_len > 0)
        {
