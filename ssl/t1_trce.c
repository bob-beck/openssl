...
static const char* do_ssl_trace_str(int val, const ssl_trace_tbl* tbl,
  ...
  for (i = 0; i < ntbl; i++, tbl++) {
    if (tbl->num == val) return tbl->name;
  }
  return "UNKNOWN";
}
...
static int do_ssl_trace_list(BIO* bio, int indent, const unsigned char* msg,
  ...
  if (msglen % vlen) return 0;
  while (msglen) {
    val = msg[0];
    if (vlen == 2) val = (val << 8) | msg[1];
...
static int ssl_print_hexbuf(BIO* bio, int indent, const char* name, size_t nlen,
  ...
  if (*pmsglen < nlen) return 0;
  blen = p[0];
  if (nlen > 1) blen = (blen << 8) | p[1];
  if (*pmsglen < nlen + blen) return 0;
  p += nlen;
...
static int ssl_print_version(BIO* bio, int indent, const char* name,
  ...
  if (*pmsglen < 2) return 0;
  vers = ((*pmsg)[0] << 8) | (*pmsg)[1];
  if (version != NULL) *version = vers;
  BIO_indent(bio, indent, 80);
...
static int ssl_print_extension(BIO* bio, int indent, int server,
  ...
  switch (extype) {
    case TLSEXT_TYPE_compress_certificate:
      if (extlen < 1) return 0;
      xlen = ext[0];
      if (extlen != xlen + 1) return 0;
    ...
    case TLSEXT_TYPE_early_data:
      if (mt != SSL3_MT_NEWSESSION_TICKET) break;
      if (extlen != 4) return 0;
...
static int ssl_print_extensions(BIO* bio, int indent, int server,
  ...
  if (extslen > msglen) return 0;
  BIO_printf(bio, "extensions, length = %d\n", (int)extslen);
  msglen -= extslen;
  while (extslen > 0) {
    int extype;
    size_t extlen;
    if (extslen < 4) return 0;
...
static int ssl_print_server_keyex(BIO* bio, int indent,
  ...
  if (!(id & SSL_PSK)) ssl_print_signature(bio, indent, sc, &msg, &msglen);
  return !msglen;
}
...
static int ssl_print_certificates(BIO* bio, const SSL_CONNECTION* sc,
  ...
  while (clen > 0) {
    if (!ssl_print_certificate(bio, sc, indent + 2, &msg, &clen)) return 0;
    if (SSL_CONNECTION_IS_TLS13(sc) &&
        !ssl_print_extensions(bio, indent + 2, server, SSL3_MT_CERTIFICATE,
                              &msg, &clen))
      return 0;
...
static int ssl_print_cert_request(BIO* bio, int indent,
  ...
  if (SSL_CONNECTION_IS_TLS13(sc)) {
    ...
    if (msglen < 1) return 0;
    xlen = msg[0];
    if (msglen < xlen + 1) return 0;
    msg++;
    BIO_indent(bio, indent, 80);
    BIO_printf(bio, "certificate_types (len=%d)\n", (int)xlen);
    if (!ssl_trace_list(bio, indent + 2, msg, xlen, 1, ssl_ctype_tbl)) return 0;
    msg += xlen;
    msglen -= xlen + 1;
  }
  if (SSL_USE_SIGALGS(sc)) {
    if (msglen < 2) return 0;
    xlen = (msg[0] << 8) | msg[1];
    if (msglen < xlen + 2 || (xlen & 1)) return 0;
    msg += 2;
    msglen -= xlen + 2;
    BIO_indent(bio, indent, 80);
...
static int ssl_print_handshake(BIO* bio, const SSL_CONNECTION* sc, int server,
  ...
  switch (htype) {
    ...
    case DTLS1_MT_HELLO_VERIFY_REQUEST:
      if (!dtls_print_hello_vfyrequest(bio, indent + 2, msg, msglen)) return 0;
      break;
    ...
    case SSL3_MT_SERVER_KEY_EXCHANGE:
      if (!ssl_print_server_keyex(bio, indent + 2, sc, msg, msglen)) return 0;
      break;

    case SSL3_MT_CLIENT_KEY_EXCHANGE:
      if (!ssl_print_client_keyex(bio, indent + 2, sc, msg, msglen)) return 0;
      break;
    ...
    case SSL3_MT_CERTIFICATE_REQUEST:
      if (!ssl_print_cert_request(bio, indent + 2, sc, msg, msglen)) return 0;
      break;
    ...
    case SSL3_MT_NEWSESSION_TICKET:
      if (!ssl_print_ticket(bio, indent + 2, sc, msg, msglen)) return 0;
      break;
