...
static const char*
do_ssl_trace_str(int val, const ssl_trace_tbl* tbl, size_t ntbl)
{
  size_t i;
...
static int
do_ssl_trace_list(BIO* bio,
                  int indent,
                  const unsigned char* msg,
                  size_t msglen,
                  size_t vlen,
                  const ssl_trace_tbl* tbl,
                  size_t ntbl)
{
  int val;
...
static const ssl_trace_tbl ssl_ctype_tbl[] = {
  { 1, "rsa_sign" },         { 2, "dss_sign" },
  { 3, "rsa_fixed_dh" },     { 4, "dss_fixed_dh" },
  { 5, "rsa_ephemeral_dh" }, { 6, "dss_ephemeral_dh" },
  { 20, "fortezza_dms" },    { 64, "ecdsa_sign" },
  { 65, "rsa_fixed_ecdh" },  { 66, "ecdsa_fixed_ecdh" },
  { 67, "gost_sign256" },    { 68, "gost_sign512" },
};
                    ...
                    size_t extlen)
{
  ...
  switch (extype) {
    case TLSEXT_TYPE_compress_certificate:
      ...
      if (extlen != xlen + 1)
        return 0;
      return ssl_trace_list(
        bio, indent + 2, ext + 1, xlen, 2, ssl_comp_cert_tbl);
    ...
    case TLSEXT_TYPE_psk_kex_modes:
      ...
      if (extlen != xlen + 1)
        return 0;
      return ssl_trace_list(
        bio, indent + 2, ext + 1, xlen, 1, ssl_psk_kex_modes_tbl);
    ...
    case TLSEXT_TYPE_client_cert_type:
      ...
      if (extlen != xlen + 1)
        return 0;
      return ssl_trace_list(
        bio, indent + 2, ext + 1, xlen, 1, ssl_cert_type_tbl);
...
static int
ssl_print_client_hello(BIO* bio,
                       const SSL_CONNECTION* sc,
                       int indent,
                       const unsigned char* msg,
                       size_t msglen)
{
  ...
  if (!ssl_print_extensions(
        bio, indent, 0, SSL3_MT_CLIENT_HELLO, &msg, &msglen))
    return 0;
  return 1;
}

static int
dtls_print_hello_vfyrequest(BIO* bio,
                            int indent,
                            const unsigned char* msg,
                            size_t msglen)
{
...
static int
ssl_print_server_hello(BIO* bio,
                       int indent,
                       const unsigned char* msg,
                       size_t msglen)
{
  ...
  if (!ssl_print_extensions(
        bio, indent, 1, SSL3_MT_SERVER_HELLO, &msg, &msglen))
    return 0;
  return 1;
}

static int
ssl_get_keyex(const char** pname, const SSL_CONNECTION* sc)
{
...
static int
ssl_print_client_keyex(BIO* bio,
                       int indent,
                       const SSL_CONNECTION* sc,
                       const unsigned char* msg,
                       size_t msglen)
  ...
  switch (id) {
    ...
    case SSL_kRSAPSK:
      if (TLS1_get_version(SSL_CONNECTION_GET_SSL(sc)) == SSL3_VERSION) {
        ...
        if (!ssl_print_hexbuf(
              bio, indent + 2, "EncryptedPreMasterSecret", 2, &msg, &msglen))
          return 0;
      }
      break;
    ...
    case SSL_kGOST18:
      ssl_print_hex(
        bio, indent + 2, "GOST-wrapped PreMasterSecret", msg, msglen);
      msglen = 0;
      break;
  }
...
static int
ssl_print_server_keyex(BIO* bio,
                       int indent,
                       const SSL_CONNECTION* sc,
                       const unsigned char* msg,
                       size_t msglen)
{
  ...
  if (id & SSL_PSK) {
    if (!ssl_print_hexbuf(
          bio, indent + 2, "psk_identity_hint", 2, &msg, &msglen))
      return 0;
  }
  switch (id) {
    case SSL_kRSA:
    ...
    case SSL_kECDHEPSK:
      ...
      if (msg[0] == EXPLICIT_PRIME_CURVE_TYPE)
        ...
        BIO_printf(bio,
                   "named_curve: %s (%d)\n",
                   ssl_trace_str(curve, ssl_groups_tbl),
                   curve);
        msg += 3;
        msglen -= 3;
...
static int
ssl_print_raw_public_key(BIO* bio,
                         const SSL* ssl,
                         int server,
                         int indent,
                         const unsigned char** pmsg,
                         size_t* pmsglen)
{
  ...
  pkey =
    d2i_PUBKEY_ex(NULL, &msg, (long)clen, ssl->ctx->libctx, ssl->ctx->propq);
  if (pkey == NULL)
    return 0;
...
static int
ssl_print_certificates(BIO* bio,
                       const SSL_CONNECTION* sc,
                       int server,
                       int indent,
                       const unsigned char* msg,
  ...
  if ((server && sc->ext.server_cert_type == TLSEXT_cert_type_rpk) ||
    ...
    if (SSL_CONNECTION_IS_TLS13(sc) &&
        !ssl_print_extensions(
          bio, indent + 2, server, SSL3_MT_CERTIFICATE, &msg, &clen))
      return 0;
    return 1;
  }
  BIO_indent(bio, indent, 80);
...
static int
ssl_print_compressed_certificates(BIO* bio,
                                  const SSL_CONNECTION* sc,
                                  int server,
                                  int indent,
                                  const unsigned char* msg,
                                  size_t msglen)
{
  ...
  BIO_indent(bio, indent, 80);
  BIO_printf(bio,
             "Compression type=%s (0x%04x)\n",
             ssl_trace_str(alg, ssl_comp_cert_tbl),
             alg);
  BIO_indent(bio, indent, 80);
  ...
  if ((comp = COMP_CTX_new(method)) == NULL ||
      COMP_expand_block(
        comp, ucdata, (int)uclen, (unsigned char*)msg, (int)clen) != (int)uclen)
    goto err;
...
static int
ssl_print_cert_request(BIO* bio,
                       int indent,
                       const SSL_CONNECTION* sc,
                       const unsigned char* msg,
                       size_t msglen)
{
  ...
  if (SSL_CONNECTION_IS_TLS13(sc)) {
    if (!ssl_print_hexbuf(bio, indent, "request_context", 1, &msg, &msglen))
      return 0;
    if (!ssl_print_extensions(
          bio, indent, 1, SSL3_MT_CERTIFICATE_REQUEST, &msg, &msglen))
      return 0;
    return 1;
  } else {
    if (msglen < 1)
                 ...
                 size_t msglen)
{
  ...
  if (SSL_CONNECTION_IS_TLS13(sc) &&
      !ssl_print_extensions(
        bio, indent + 2, 0, SSL3_MT_NEWSESSION_TICKET, &msg, &msglen))
    return 0;
  if (msglen)
    return 0;
  return 1;
                    ...
                    int indent)
{
  ...
  switch (htype) {
    ...
    case SSL3_MT_COMPRESSED_CERTIFICATE:
      if (!ssl_print_compressed_certificates(
            bio, sc, server, indent + 2, msg, msglen))
        return 0;
      break;
    ...
    case SSL3_MT_ENCRYPTED_EXTENSIONS:
      if (!ssl_print_extensions(
            bio, indent + 2, 1, SSL3_MT_ENCRYPTED_EXTENSIONS, &msg, &msglen))
        return 0;
      break;
          ...
          void* arg)
{
  ...
  switch (content_type) {
    case SSL3_RT_HEADER: {
      ...
      BIO_printf(bio,
                 " TLS Record\nHeader:\n  Version = %s (0x%x)\n",
                 ssl_trace_str(hvers, ssl_version_tbl),
                 hvers);
      if (SSL_CONNECTION_IS_DTLS(sc)) {
        BIO_printf(bio,
      ...
      BIO_printf(bio,
                 "  Content Type = %s (%d)\n  Length = %d",
                 ssl_trace_str(msg[0], ssl_content_tbl),
                 msg[0],
    ...
    case SSL3_RT_HANDSHAKE:
      if (!ssl_print_handshake(
            bio, sc, sc->server ? write_p : !write_p, msg, msglen, 4))
