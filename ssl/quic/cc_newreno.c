...
newreno_bind_diagnostic(OSSL_CC_DATA* cc, OSSL_PARAM* params)
{
  ...
  if (!bind_diag(params,
      ...
      !bind_diag(params,
                 OSSL_CC_OPTION_CUR_CWND_SIZE,
                 sizeof(uint64_t),
                 (void**)&new_p_cur_cwnd_size) ||
      !bind_diag(params,
                 OSSL_CC_OPTION_MIN_CWND_SIZE,
                 sizeof(uint64_t),
                 (void**)&new_p_min_cwnd_size) ||
      !bind_diag(params,
                 OSSL_CC_OPTION_CUR_BYTES_IN_FLIGHT,
                 sizeof(uint64_t),
                 (void**)&new_p_cur_bytes_in_flight) ||
      !bind_diag(params,
                 OSSL_CC_OPTION_CUR_STATE,
                 sizeof(uint32_t),
...
newreno_unbind_diagnostic(OSSL_CC_DATA* cc, OSSL_PARAM* params)
{
  ...
  unbind_diag(
    params, OSSL_CC_OPTION_CUR_CWND_SIZE, (void**)&nr->p_diag_cur_cwnd_size);
  unbind_diag(
    params, OSSL_CC_OPTION_MIN_CWND_SIZE, (void**)&nr->p_diag_min_cwnd_size);
  unbind_diag(params,
              OSSL_CC_OPTION_CUR_BYTES_IN_FLIGHT,
...
newreno_cong(OSSL_CC_NEWRENO* nr, OSSL_TIME tx_time)
{
  ...
  nr->slow_start_thresh = safe_muldiv_u64(nr->cong_wnd,
