...
port_init(QUIC_PORT* port)
{
  ...
  ossl_quic_demux_set_default_handler(
    port->demux, port_default_packet_handler, port);
...
static QUIC_CHANNEL*
port_make_channel(QUIC_PORT* port,
                  SSL* tls,
                  OSSL_QRX* qrx,
                  int is_server,
                  int is_tserver)
 ...
 */
static void
port_bind_channel(QUIC_PORT* port,
                  const BIO_ADDR* peer,
 ...
 */
static int
marshal_validation_token(QUIC_VALIDATION_TOKEN* token,
      ...
      (token->is_retry &&
       (!WPACKET_sub_memcpy_u8(&wpkt, &token->odcid.id, token->odcid.id_len) ||
        !WPACKET_sub_memcpy_u8(
          &wpkt, &token->rscid.id, token->rscid.id_len))) ||
      !WPACKET_sub_memcpy_u8(
        &wpkt, token->remote_addr, token->remote_addr_len) ||
                         ...
                         size_t* pt_len)
{
  ...
  *pt_len = ct_len - iv_len - tag_len;
  ...
  if (!EVP_DecryptInit_ex(port->token_ctx, NULL, NULL, NULL, iv) ||
      !EVP_DecryptUpdate(port->token_ctx,
                         plaintext,
                         &len,
                         data,
                         (int)(ct_len - iv_len - tag_len)) ||
      !EVP_CIPHER_CTX_ctrl(
        port->token_ctx, EVP_CTRL_GCM_SET_TAG, tag_len, (void*)tag) ||
 ...
 */
static int
parse_validation_token(QUIC_VALIDATION_TOKEN* token,
                       ...
                       size_t buf_len)
{
  ...
  if (!PACKET_buf_init(&pkt, buf, buf_len) ||
      ...
      !(token->is_retry == 0 || token->is_retry == 1) ||
      !PACKET_copy_bytes(
        &pkt, (unsigned char*)&token->timestamp, sizeof(token->timestamp)) ||
      (token->is_retry &&
       (!PACKET_get_length_prefixed_1(&pkt, &subpkt) ||
        (token->odcid.id_len = (unsigned char)PACKET_remaining(&subpkt)) >
          QUIC_MAX_CONN_ID_LEN ||
        !PACKET_copy_bytes(
          &subpkt, (unsigned char*)&token->odcid.id, token->odcid.id_len) ||
        !PACKET_get_length_prefixed_1(&pkt, &subpkt) ||
        (token->rscid.id_len = (unsigned char)PACKET_remaining(&subpkt)) >
          QUIC_MAX_CONN_ID_LEN ||
        !PACKET_copy_bytes(
          &subpkt, (unsigned char*)&token->rscid.id, token->rscid.id_len))) ||
      !PACKET_get_length_prefixed_1(&pkt, &subpkt) ||
...
port_send_retry(QUIC_PORT* port, BIO_ADDR* peer, QUIC_PKT_HDR* client_hdr)
{
  ...
  if (!generate_token(
        peer, client_hdr->dst_conn_id, hdr.src_conn_id, &token, 1) ||
  ...
  ok = ossl_quic_wire_encode_pkt_hdr(
    &wpkt, client_hdr->dst_conn_id.id_len, &hdr, NULL);
  if (ok == 0)
    goto err;
 ...
 */
static void
port_send_version_negotiation(QUIC_PORT* port,
                              BIO_ADDR* peer,
                              QUIC_PKT_HDR* client_hdr)
{
  ...
  if (!ossl_quic_wire_encode_pkt_hdr(
        &wpkt, client_hdr->dst_conn_id.id_len, &hdr, NULL))
    return;
 ...
 */
static int
port_validate_token(QUIC_PKT_HDR* hdr,
                    QUIC_PORT* port,
                    BIO_ADDR* peer,
                    QUIC_CONN_ID* odcid,
                    QUIC_CONN_ID* scid,
                    uint8_t* gen_new_token)
{
  ...
  *gen_new_token = 0;
  ...
  if (!decrypt_validation_token(
        port, hdr->token, hdr->token_len, NULL, &dec_token_len) ||
      dec_token_len > MARSHALLED_TOKEN_MAX_LEN ||
      !decrypt_validation_token(
        port, hdr->token, hdr->token_len, dec_token, &dec_token_len) ||
...
generate_new_token(QUIC_CHANNEL* ch, BIO_ADDR* peer)
{
  ...
  if (!generate_token(peer, ch->init_dcid, rscid, &token, 0) ||
      !marshal_validation_token(&token, buffer, &token_buf_len) ||
      !encrypt_validation_token(
        ch->port, buffer, token_buf_len, NULL, &ct_len) ||
      ct_len > ENCRYPTED_TOKEN_MAX_LEN ||
      !encrypt_validation_token(
        ch->port, buffer, token_buf_len, ct_buf, &ct_len) ||
      !ossl_assert(ct_len >= QUIC_RETRY_INTEGRITY_TAG_LEN)) {
    OPENSSL_free(ct_buf);
    cleanup_validation_token(&token);
    return;
...
port_default_packet_handler(QUIC_URXE* e, void* arg, const QUIC_CONN_ID* dcid)
{
  ...
  if (!ossl_quic_wire_decode_pkt_hdr(
        &pkt, SIZE_MAX, 1, 0, &hdr, NULL, &cause_flags)) {
    /*
  ...
  if (hdr.token != NULL &&
      port_validate_token(
        &hdr, port, &e->peer, &odcid, &scid, &gen_new_token) == 0) {
    /*
     * RFC 9000 s 8.1.3
  ...
  port_bind_channel(
    port, &e->peer, &scid, &hdr.dst_conn_id, &odcid, qrx, &new_ch);
