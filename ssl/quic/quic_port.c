...
QUIC_PORT* ossl_quic_port_new(const QUIC_PORT_ARGS* args) {
  ...
  if ((port = OPENSSL_zalloc(sizeof(QUIC_PORT))) == NULL) return NULL;
...
static int port_init(QUIC_PORT* port) {
  ...
  if (port->engine == NULL || port->channel_ctx == NULL) goto err;

  if ((port->err_state = OSSL_ERR_STATE_new()) == NULL) goto err;
...
static void port_transition_failed(QUIC_PORT* port) {
  if (port->state == QUIC_PORT_STATE_FAILED) return;
...
static int port_update_poll_desc(QUIC_PORT* port, BIO* net_bio, int for_write) {
  ...
  if (!validate_poll_descriptor(&d)) return 0;
...
int ossl_quic_port_update_poll_descriptors(QUIC_PORT* port, int force) {
  ...
  if (!force && !port->bio_changed) return 0;

  if (!port_update_poll_desc(port, port->net_rbio, /*for_write=*/0)) ok = 0;

  if (!port_update_poll_desc(port, port->net_wbio, /*for_write=*/1)) ok = 0;
...
int ossl_quic_port_set_net_rbio(QUIC_PORT* port, BIO* net_rbio) {
  if (port->net_rbio == net_rbio) return 1;

  if (!port_update_poll_desc(port, net_rbio, /*for_write=*/0)) return 0;
...
int ossl_quic_port_set_net_wbio(QUIC_PORT* port, BIO* net_wbio) {
  ...
  if (port->net_wbio == net_wbio) return 1;

  if (!port_update_poll_desc(port, net_wbio, /*for_write=*/1)) return 0;
...
static SSL* port_new_handshake_layer(QUIC_PORT* port, QUIC_CHANNEL* ch) {
   ...
   */
  if (!ossl_assert(port->get_conn_user_ssl != NULL)) return NULL;
  user_ssl = port->get_conn_user_ssl(ch, port->user_ssl_arg);
  if (user_ssl == NULL) return NULL;
...
void ossl_quic_port_subtick(QUIC_PORT* port, QUIC_TICK_RESULT* res,
  ...
  if (!port->engine->inhibit_tick) {
    ...
    if (ossl_quic_port_is_running(port)) port_rx_pre(port);
...
static void port_rx_pre(QUIC_PORT* port) {
   ...
   */
  if (!port->allow_incoming && !port->have_sent_any_pkt) return;
...
static int parse_validation_token(QUIC_VALIDATION_TOKEN* token,
  ...
  if (buf == NULL || token == NULL) return 0;
...
static void port_send_retry(QUIC_PORT* port, BIO_ADDR* peer,
  ...
  if (!ossl_assert(sizeof(buffer) >= MARSHALLED_TOKEN_MAX_LEN)) return;
  /*
...
static void port_send_version_negotiation(QUIC_PORT* port, BIO_ADDR* peer,
  ...
  if (!WPACKET_init_static_len(&wpkt, buffer, sizeof(buffer), 0)) return;
  ...
  for (i = 0; i < OSSL_NELEM(supported_versions); i++) {
    if (!WPACKET_put_bytes_u32(&wpkt, supported_versions[i])) return;
  }

  if (!WPACKET_get_total_written(&wpkt, &msg[0].data_len)) return;

  if (!WPACKET_finish(&wpkt)) return;
...
static int port_validate_token(QUIC_PKT_HDR* hdr, QUIC_PORT* port,
  ...
  *gen_new_token = 0;
   ...
   * timestamp.
   */
  if (ossl_time_compare(now, token.timestamp) < 0) goto err;
  ...
  if (token.is_retry) {
    ...
    *scid = token.rscid;
    ...
    if (!ossl_quic_lcidm_get_unused_cid(port->lcidm, odcid)) goto err;
  ...
  if (token.is_retry) {
    *gen_new_token = 1;
    ...
    if (time_diff > ((NEW_TOKEN_LIFETIME * 9) / 10)) *gen_new_token = 1;
  }
...
static void port_default_packet_handler(QUIC_URXE* e, void* arg,
  ...
  if (!ossl_quic_port_is_running(port)) goto undesirable;

  if (port_try_handle_stateless_reset(port, e)) goto undesirable;
   ...
   */
  if (!port->allow_incoming) goto undesirable;

  /*
   * We have got a packet for an unknown DCID. This might be an attempt to
   * open a new connection.
   */
  if (e->data_len < QUIC_MIN_INITIAL_DGRAM_LEN) goto undesirable;
  ...
  if (!ossl_quic_wire_decode_pkt_hdr(&pkt, SIZE_MAX, 1, 0, &hdr, NULL,
     ...
     * negotiation packet
     */
    if ((cause_flags & QUIC_PKT_HDR_DECODE_BAD_VERSION) == 0) goto undesirable;
  }
  ...
  switch (hdr.version) {
    ...
    default:
       ...
       */
      if (e->data_len < 1200) goto undesirable;
   ...
   * connection.
   */
  if (hdr.type != QUIC_PKT_TYPE_INITIAL) goto undesirable;
   ...
   * to port.
   */
  if (new_ch == NULL) goto undesirable;

  /*
   * Generate a token for sending in a later NEW_TOKEN frame
   */
  if (gen_new_token == 1) generate_new_token(new_ch, &e->peer);
...
void ossl_quic_port_raise_net_error(QUIC_PORT* port,
  ...
  if (!ossl_quic_port_is_running(port)) return;
  ...
  if (triggering_ch != NULL) ossl_quic_channel_raise_net_error(triggering_ch);

  OSSL_LIST_FOREACH(ch, ch, &port->channel_list)
  if (ch != triggering_ch) ossl_quic_channel_raise_net_error(ch);
}
