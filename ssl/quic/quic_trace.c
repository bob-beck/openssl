...
static void
put_str(BIO* bio, char* str, size_t slen)
{
  size_t i;
...
static void
put_data(BIO* bio, const uint8_t* data, size_t datalen)
{
  size_t i;
...
static void
put_conn_id(BIO* bio, QUIC_CONN_ID* id)
{
  if (id->id_len == 0) {
...
static void
put_token(BIO* bio, const uint8_t* token, size_t token_len)
{
  if (token_len == 0)
...
static int
frame_reset_stream(BIO* bio, PACKET* pkt)
{
...
static int
frame_stop_sending(BIO* bio, PACKET* pkt)
{
...
static int
frame_crypto(BIO* bio, PACKET* pkt)
{
...
static int
frame_new_token(BIO* bio, PACKET* pkt)
{
  const uint8_t* token;
  size_t token_len;
...
static int
frame_stream(BIO* bio, PACKET* pkt, uint64_t frame_type)
{
...
static int
frame_max_data(BIO* bio, PACKET* pkt)
{
  uint64_t max_data = 0;
...
static int
frame_max_stream_data(BIO* bio, PACKET* pkt)
{
  ...
  BIO_printf(
    bio, "    Max Stream Data: %llu\n", (unsigned long long)max_stream_data);

  return 1;
}

static int
frame_max_streams(BIO* bio, PACKET* pkt)
{
...
static int
frame_data_blocked(BIO* bio, PACKET* pkt)
{
  uint64_t max_data = 0;
...
static int
frame_stream_data_blocked(BIO* bio, PACKET* pkt)
{
  uint64_t stream_id = 0;
  uint64_t max_data = 0;
...
static int
frame_streams_blocked(BIO* bio, PACKET* pkt)
{
  uint64_t max_data = 0;
...
static int
frame_new_conn_id(BIO* bio, PACKET* pkt)
{
  ...
  put_data(bio,
           frame_data.stateless_reset.token,
...
static int
frame_retire_conn_id(BIO* bio, PACKET* pkt)
{
  uint64_t seq_num;
...
static int
frame_path_challenge(BIO* bio, PACKET* pkt)
{
  uint64_t data = 0;
...
static int
frame_path_response(BIO* bio, PACKET* pkt)
{
  uint64_t data = 0;
...
static int
frame_conn_closed(BIO* bio, PACKET* pkt)
{
...
static int
trace_frame_data(BIO* bio, PACKET* pkt)
{
  uint64_t frame_type;
