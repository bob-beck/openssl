...
static int frame_ack(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_ack(pkt, 0, &ack, NULL)) goto end;
...
static int frame_reset_stream(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_reset_stream(pkt, &frame_data)) return 0;
...
static int frame_stop_sending(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_stop_sending(pkt, &frame_data)) return 0;
...
static int frame_crypto(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_crypto(pkt, 1, &frame_data)) return 0;
...
static int frame_new_token(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_new_token(pkt, &token, &token_len)) return 0;
...
static int frame_stream(BIO* bio, PACKET* pkt, uint64_t frame_type) {
  ...
  if (!ossl_quic_wire_decode_frame_stream(pkt, 1, &frame_data)) return 0;
...
static int frame_max_data(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_max_data(pkt, &max_data)) return 0;
...
static int frame_max_streams(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_max_streams(pkt, &max_streams)) return 0;
...
static int frame_data_blocked(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_data_blocked(pkt, &max_data)) return 0;
...
static int frame_streams_blocked(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_streams_blocked(pkt, &max_data)) return 0;
...
static int frame_new_conn_id(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_new_conn_id(pkt, &frame_data)) return 0;
...
static int frame_retire_conn_id(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_retire_conn_id(pkt, &seq_num)) return 0;
...
static int frame_path_challenge(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_path_challenge(pkt, &data)) return 0;
...
static int frame_path_response(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_path_response(pkt, &data)) return 0;
...
static int frame_conn_closed(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_decode_frame_conn_close(pkt, &frame_data)) return 0;
...
static int trace_frame_data(BIO* bio, PACKET* pkt) {
  ...
  if (!ossl_quic_wire_peek_frame_header(pkt, &frame_type, NULL)) return 0;

  switch (frame_type) {
    case OSSL_QUIC_FRAME_TYPE_PING:
      BIO_puts(bio, "Ping\n");
      if (!ossl_quic_wire_decode_frame_ping(pkt)) return 0;
      break;
    ...
    case OSSL_QUIC_FRAME_TYPE_ACK_WITH_ECN:
      ...
      if (!frame_ack(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_RESET_STREAM:
      BIO_puts(bio, "Reset stream\n");
      if (!frame_reset_stream(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_STOP_SENDING:
      BIO_puts(bio, "Stop sending\n");
      if (!frame_stop_sending(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_CRYPTO:
      BIO_puts(bio, "Crypto\n");
      if (!frame_crypto(bio, pkt)) return 0;
      break;
    ...
    case OSSL_QUIC_FRAME_TYPE_STREAM_OFF_LEN_FIN:
      ...
      if (!frame_stream(bio, pkt, frame_type)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_MAX_DATA:
      BIO_puts(bio, "Max data\n");
      if (!frame_max_data(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA:
      BIO_puts(bio, "Max stream data\n");
      if (!frame_max_stream_data(bio, pkt)) return 0;
      break;
    ...
    case OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_UNI:
      ...
      if (frame_type == OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_BIDI)
        ...
        BIO_puts(bio, " (Uni)\n");
      if (!frame_max_streams(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_DATA_BLOCKED:
      BIO_puts(bio, "Data blocked\n");
      if (!frame_data_blocked(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED:
      BIO_puts(bio, "Stream data blocked\n");
      if (!frame_stream_data_blocked(bio, pkt)) return 0;
      break;
    ...
    case OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_UNI:
      ...
      if (frame_type == OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_BIDI)
        ...
        BIO_puts(bio, " (Uni)\n");
      if (!frame_streams_blocked(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID:
      BIO_puts(bio, "New conn id\n");
      if (!frame_new_conn_id(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_RETIRE_CONN_ID:
      BIO_puts(bio, "Retire conn id\n");
      if (!frame_retire_conn_id(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_PATH_CHALLENGE:
      BIO_puts(bio, "Path challenge\n");
      if (!frame_path_challenge(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_PATH_RESPONSE:
      BIO_puts(bio, "Path response\n");
      if (!frame_path_response(bio, pkt)) return 0;
      break;
    ...
    case OSSL_QUIC_FRAME_TYPE_CONN_CLOSE_TRANSPORT:
      ...
      if (!frame_conn_closed(bio, pkt)) return 0;
      break;

    case OSSL_QUIC_FRAME_TYPE_HANDSHAKE_DONE:
      BIO_puts(bio, "Handshake done\n");
      if (!ossl_quic_wire_decode_frame_handshake_done(pkt)) return 0;
      break;
...
int ossl_quic_trace(int write_p, int version, int content_type, const void* buf,
  ...
  switch (content_type) {
    ...
    case SSL3_RT_QUIC_PACKET: {
      ...
      if (!PACKET_buf_init(&pkt, buf, msglen)) return 0;
