 ...
 */
static int
qeng_init(QUIC_ENGINE* qeng, uint64_t reactor_flags);
static void
qeng_cleanup(QUIC_ENGINE* qeng);
static void
qeng_tick(QUIC_TICK_RESULT* res, void* arg, uint32_t flags);
...
qeng_init(QUIC_ENGINE* qeng, uint64_t reactor_flags)
{
  return ossl_quic_reactor_init(
    &qeng->rtor, qeng_tick, qeng, qeng->mutex, ossl_time_zero(), reactor_flags);
}
