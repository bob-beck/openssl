...
uint64_t
ossl_quic_cfq_item_get_frame_type (const QUIC_CFQ_ITEM *item)
{
...
const unsigned char *
ossl_quic_cfq_item_get_encoded (const QUIC_CFQ_ITEM *item)
{
  QUIC_CFQ_ITEM_EX *ex = (QUIC_CFQ_ITEM_EX *)item;
...
size_t
ossl_quic_cfq_item_get_encoded_len (const QUIC_CFQ_ITEM *item)
{
  QUIC_CFQ_ITEM_EX *ex = (QUIC_CFQ_ITEM_EX *)item;
...
int
ossl_quic_cfq_item_get_state (const QUIC_CFQ_ITEM *item)
{
  QUIC_CFQ_ITEM_EX *ex = (QUIC_CFQ_ITEM_EX *)item;
...
uint32_t
ossl_quic_cfq_item_get_pn_space (const QUIC_CFQ_ITEM *item)
{
  QUIC_CFQ_ITEM_EX *ex = (QUIC_CFQ_ITEM_EX *)item;
...
int
ossl_quic_cfq_item_is_unreliable (const QUIC_CFQ_ITEM *item)
{
  QUIC_CFQ_ITEM_EX *ex = (QUIC_CFQ_ITEM_EX *)item;
...
static int
compare (const QUIC_CFQ_ITEM_EX *a, const QUIC_CFQ_ITEM_EX *b)
{
...
static void
list_remove (QUIC_CFQ_ITEM_LIST *l, QUIC_CFQ_ITEM_EX *n)
{
  if (l->head == n)
    l->head = n->next;
  if (l->tail == n)
...
static void
list_insert_head (QUIC_CFQ_ITEM_LIST *l, QUIC_CFQ_ITEM_EX *n)
{
  n->next = l->head;
  n->prev = NULL;
  l->head = n;
...
static void
list_insert_tail (QUIC_CFQ_ITEM_LIST *l, QUIC_CFQ_ITEM_EX *n)
{
  n->prev = l->tail;
  n->next = NULL;
  l->tail = n;
...
static void
list_insert_after (QUIC_CFQ_ITEM_LIST *l, QUIC_CFQ_ITEM_EX *ref,
                   QUIC_CFQ_ITEM_EX *n)
{
  n->prev = ref;
  n->next = ref->next;
...
static void
list_insert_sorted (QUIC_CFQ_ITEM_LIST *l, QUIC_CFQ_ITEM_EX *n,
...
static void
clear_item (QUIC_CFQ_ITEM_EX *item)
{
...
static void
free_list_items (QUIC_CFQ_ITEM_LIST *l)
{
...
void
ossl_quic_cfq_free (QUIC_CFQ *cfq)
{
  if (cfq == NULL)
    return;
...
static QUIC_CFQ_ITEM_EX *
cfq_get_free (QUIC_CFQ *cfq)
{
...
QUIC_CFQ_ITEM *
ossl_quic_cfq_add_frame (QUIC_CFQ *cfq, uint32_t priority, uint32_t pn_space,
                         uint64_t frame_type, uint32_t flags,
...
void
ossl_quic_cfq_mark_tx (QUIC_CFQ *cfq, QUIC_CFQ_ITEM *item)
{
  QUIC_CFQ_ITEM_EX *ex = (QUIC_CFQ_ITEM_EX *)item;
...
void
ossl_quic_cfq_mark_lost (QUIC_CFQ *cfq, QUIC_CFQ_ITEM *item, uint32_t priority)
{
  QUIC_CFQ_ITEM_EX *ex = (QUIC_CFQ_ITEM_EX *)item;
...
/*
 * Releases a CFQ item. The item may be in either state (NEW or TX) prior to
 * the call. The QUIC_CFQ_ITEM pointer must not be used following this call.
 */
void
ossl_quic_cfq_release (QUIC_CFQ *cfq, QUIC_CFQ_ITEM *item)
{
  QUIC_CFQ_ITEM_EX *ex = (QUIC_CFQ_ITEM_EX *)item;
...
QUIC_CFQ_ITEM *
ossl_quic_cfq_get_priority_head (const QUIC_CFQ *cfq, uint32_t pn_space)
{
...
QUIC_CFQ_ITEM *
ossl_quic_cfq_item_get_priority_next (const QUIC_CFQ_ITEM *item,
                                      uint32_t pn_space)
{
  QUIC_CFQ_ITEM_EX *ex = (QUIC_CFQ_ITEM_EX *)item;
