...
void* ossl_quic_wire_encode_frame_crypto(WPACKET* pkt,
  ...
  if (f->data != NULL) memcpy(p, f->data, (size_t)f->len);
...
int ossl_quic_wire_encode_frame_stream_hdr(WPACKET* pkt,
  ...
  if (f->offset != 0) frame_type |= OSSL_QUIC_FRAME_FLAG_STREAM_OFF;
  if (f->has_explicit_len) frame_type |= OSSL_QUIC_FRAME_FLAG_STREAM_LEN;
  if (f->is_fin) frame_type |= OSSL_QUIC_FRAME_FLAG_STREAM_FIN;
  ...
  if (f->has_explicit_len && !WPACKET_quic_write_vlint(pkt, f->len)) return 0;
...
void* ossl_quic_wire_encode_frame_stream(WPACKET* pkt,
  ...
  if (!WPACKET_allocate_bytes(pkt, (size_t)f->len, &p)) return NULL;
...
unsigned char* ossl_quic_wire_encode_transport_param_bytes(
  ...
  if (value != NULL) memcpy(b, value, value_len);
...
int ossl_quic_wire_encode_transport_param_cid(WPACKET* wpkt, uint64_t id,
  ...
  if (cid->id_len > QUIC_MAX_CONN_ID_LEN) return 0;
...
static int expect_frame_header_mask(PACKET* pkt, uint64_t expected_frame_type,
  ...
  if (actual_frame_type != NULL) *actual_frame_type = actual_frame_type_;
...
int ossl_quic_wire_decode_frame_ack(PACKET* pkt, uint32_t ack_delay_exponent,
  ...
  if (ack != NULL) {
    ...
    ack->delay_time = ossl_time_multiply(
        ...
        safe_mul_uint64_t(ack_delay_raw, (uint64_t)1 << ack_delay_exponent,
                          &err));
    if (err) ack->delay_time = ossl_time_infinite();
  ...
  for (i = 0; i < ack_range_count; ++i) {
    ...
    if (start < gap + 2 || len > end) return 0;
  ...
  if (total_ranges != NULL) *total_ranges = ack_range_count + 1;
...
int ossl_quic_wire_decode_frame_crypto(PACKET* pkt, int nodata,
  ...
  if (f->offset + f->len > (((uint64_t)1) << 62) - 1) /* RFC 9000 s. 19.6 */
    return 0;

  if (nodata) {
    f->data = NULL;
  } else {
    if (PACKET_remaining(pkt) < f->len) return 0;

    f->data = PACKET_data(pkt);

    if (!PACKET_forward(pkt, (size_t)f->len)) return 0;
  }
...
int ossl_quic_wire_decode_frame_new_token(PACKET* pkt,
  ...
  *token_len = (size_t)token_len_;
  ...
  if (!PACKET_forward(pkt, (size_t)token_len_)) return 0;
...
int ossl_quic_wire_decode_frame_stream(PACKET* pkt, int nodata,
  ...
  if ((frame_type & OSSL_QUIC_FRAME_FLAG_STREAM_OFF) != 0) {
    if (!PACKET_get_quic_vlint(pkt, &f->offset)) return 0;
  } else {
    f->offset = 0;
  }
   ...
   */
  if (f->offset + f->len > (((uint64_t)1) << 62) - 1) return 0;
...
int ossl_quic_wire_decode_frame_new_conn_id(PACKET* pkt,
  ...
  if (!PACKET_copy_bytes(pkt, f->conn_id.id, len)) return 0;
...
int ossl_quic_wire_decode_frame_conn_close(PACKET* pkt,
  ...
  if (!f->is_app) {
    if (!PACKET_get_quic_vlint(pkt, &f->frame_type)) return 0;
  } else {
    f->frame_type = 0;
  }
...
size_t ossl_quic_wire_decode_padding(PACKET* pkt) {
  ...
  if (!PACKET_forward(pkt, p - start)) return 0;
...
int ossl_quic_wire_decode_transport_param_int(PACKET* pkt, uint64_t* id,
  ...
  if (sub.curr == NULL) return 0;

  if (!PACKET_get_quic_vlint(&sub, value)) return 0;

  if (PACKET_remaining(&sub) > 0) return 0;
...
int ossl_quic_wire_decode_transport_param_cid(PACKET* pkt, uint64_t* id,
  ...
  if (body == NULL || len > QUIC_MAX_CONN_ID_LEN) return 0;
