...
int
ossl_quic_frame_ack_contains_pn(const OSSL_QUIC_FRAME_ACK* ack, QUIC_PN pn)
{
  size_t i;
...
int
ossl_quic_wire_encode_padding(WPACKET* pkt, size_t num_bytes)
{
  /*
...
static int
encode_frame_hdr(WPACKET* pkt, uint64_t frame_type)
{
  return WPACKET_quic_write_vlint(pkt, frame_type);
}

int
ossl_quic_wire_encode_frame_ping(WPACKET* pkt)
{
  return encode_frame_hdr(pkt, OSSL_QUIC_FRAME_TYPE_PING);
}

int
ossl_quic_wire_encode_frame_ack(WPACKET* pkt,
...
int
ossl_quic_wire_encode_frame_reset_stream(WPACKET* pkt,
...
int
ossl_quic_wire_encode_frame_stop_sending(WPACKET* pkt,
...
int
ossl_quic_wire_encode_frame_crypto_hdr(WPACKET* pkt,
...
void*
ossl_quic_wire_encode_frame_crypto(WPACKET* pkt,
                                   const OSSL_QUIC_FRAME_CRYPTO* f)
{
...
int
ossl_quic_wire_encode_frame_new_token(WPACKET* pkt,
...
int
ossl_quic_wire_encode_frame_stream_hdr(WPACKET* pkt,
...
void*
ossl_quic_wire_encode_frame_stream(WPACKET* pkt,
                                   const OSSL_QUIC_FRAME_STREAM* f)
{
...
int
ossl_quic_wire_encode_frame_max_data(WPACKET* pkt, uint64_t max_data)
{
...
int
ossl_quic_wire_encode_frame_max_stream_data(WPACKET* pkt,
                                            uint64_t stream_id,
                                            uint64_t max_data)
{
...
int
ossl_quic_wire_encode_frame_max_streams(WPACKET* pkt,
                                        char is_uni,
...
int
ossl_quic_wire_encode_frame_data_blocked(WPACKET* pkt, uint64_t max_data)
{
...
int
ossl_quic_wire_encode_frame_stream_data_blocked(WPACKET* pkt,
                                                uint64_t stream_id,
...
int
ossl_quic_wire_encode_frame_streams_blocked(WPACKET* pkt,
                                            char is_uni,
                                            uint64_t max_streams)
{
  if (!encode_frame_hdr(pkt,
...
int
ossl_quic_wire_encode_frame_new_conn_id(WPACKET* pkt,
                                        const OSSL_QUIC_FRAME_NEW_CONN_ID* f)
{
  ...
  if (!encode_frame_hdr(pkt, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID) ||
      ...
      !WPACKET_memcpy(
        pkt, f->stateless_reset.token, sizeof(f->stateless_reset.token)))
    return 0;
...
int
ossl_quic_wire_encode_frame_retire_conn_id(WPACKET* pkt, uint64_t seq_num)
{
...
int
ossl_quic_wire_encode_frame_path_challenge(WPACKET* pkt, uint64_t data)
{
...
int
ossl_quic_wire_encode_frame_path_response(WPACKET* pkt, uint64_t data)
{
...
int
ossl_quic_wire_encode_frame_conn_close(WPACKET* pkt,
...
int
ossl_quic_wire_encode_frame_handshake_done(WPACKET* pkt)
{
  return encode_frame_hdr(pkt, OSSL_QUIC_FRAME_TYPE_HANDSHAKE_DONE);
}

unsigned char*
ossl_quic_wire_encode_transport_param_bytes(WPACKET* pkt,
                                            uint64_t id,
...
int
ossl_quic_wire_encode_transport_param_int(WPACKET* pkt,
                                          uint64_t id,
                                          uint64_t value)
{
  if (!WPACKET_quic_write_vlint(pkt, id) ||
...
int
ossl_quic_wire_encode_transport_param_cid(WPACKET* wpkt,
                                          uint64_t id,
                                          const QUIC_CONN_ID* cid)
{
  ...
  if (ossl_quic_wire_encode_transport_param_bytes(
        wpkt, id, cid->id, cid->id_len) == NULL)
    return 0;
 ...
 */
int
ossl_quic_wire_peek_frame_header(PACKET* pkt, uint64_t* type, int* was_minimal)
{
  return PACKET_peek_quic_vlint_ex(pkt, type, was_minimal);
}

int
ossl_quic_wire_skip_frame_header(PACKET* pkt, uint64_t* type)
{
  return PACKET_get_quic_vlint(pkt, type);
}

static int
expect_frame_header_mask(PACKET* pkt,
...
static int
expect_frame_header(PACKET* pkt, uint64_t expected_frame_type)
{
  uint64_t actual_frame_type;
...
int
ossl_quic_wire_peek_frame_ack_num_ranges(const PACKET* orig_pkt,
...
int
ossl_quic_wire_decode_frame_ack(PACKET* pkt,
                                uint32_t ack_delay_exponent,
                                ...
                                uint64_t* total_ranges)
{
  ...
  if (!expect_frame_header_mask(
        pkt, OSSL_QUIC_FRAME_TYPE_ACK_WITHOUT_ECN, 1, &frame_type) ||
  ...
  if (ack != NULL) {
    ...
    ack->delay_time = ossl_time_multiply(
      ...
      safe_mul_uint64_t(
        ack_delay_raw, (uint64_t)1 << ack_delay_exponent, &err));
    if (err)
...
int
ossl_quic_wire_decode_frame_reset_stream(PACKET* pkt,
...
int
ossl_quic_wire_decode_frame_stop_sending(PACKET* pkt,
...
int
ossl_quic_wire_decode_frame_crypto(PACKET* pkt,
                                   int nodata,
...
int
ossl_quic_wire_decode_frame_new_token(PACKET* pkt,
                                      const unsigned char** token,
                                      size_t* token_len)
{
  uint64_t token_len_;
...
int
ossl_quic_wire_decode_frame_stream(PACKET* pkt,
                                   int nodata,
...
int
ossl_quic_wire_decode_frame_max_data(PACKET* pkt, uint64_t* max_data)
{
...
int
ossl_quic_wire_decode_frame_max_stream_data(PACKET* pkt,
                                            uint64_t* stream_id,
                                            uint64_t* max_stream_data)
{
...
int
ossl_quic_wire_decode_frame_max_streams(PACKET* pkt, uint64_t* max_streams)
{
...
int
ossl_quic_wire_decode_frame_data_blocked(PACKET* pkt, uint64_t* max_data)
{
...
int
ossl_quic_wire_decode_frame_stream_data_blocked(PACKET* pkt,
                                                uint64_t* stream_id,
                                                uint64_t* max_stream_data)
{
...
int
ossl_quic_wire_decode_frame_streams_blocked(PACKET* pkt, uint64_t* max_streams)
{
...
int
ossl_quic_wire_decode_frame_new_conn_id(PACKET* pkt,
...
int
ossl_quic_wire_decode_frame_retire_conn_id(PACKET* pkt, uint64_t* seq_num)
{
...
int
ossl_quic_wire_decode_frame_path_challenge(PACKET* pkt, uint64_t* data)
{
...
int
ossl_quic_wire_decode_frame_path_response(PACKET* pkt, uint64_t* data)
{
...
int
ossl_quic_wire_decode_frame_conn_close(PACKET* pkt,
                                       OSSL_QUIC_FRAME_CONN_CLOSE* f)
{
  ...
  if (!PACKET_get_bytes(
        pkt, (const unsigned char**)&f->reason, (size_t)reason_len))
    return 0;
...
size_t
ossl_quic_wire_decode_padding(PACKET* pkt)
{
...
int
ossl_quic_wire_decode_frame_ping(PACKET* pkt)
{
  return expect_frame_header(pkt, OSSL_QUIC_FRAME_TYPE_PING);
}

int
ossl_quic_wire_decode_frame_handshake_done(PACKET* pkt)
{
  return expect_frame_header(pkt, OSSL_QUIC_FRAME_TYPE_HANDSHAKE_DONE);
}

int
ossl_quic_wire_peek_transport_param(PACKET* pkt, uint64_t* id)
{
  return PACKET_peek_quic_vlint(pkt, id);
}

const unsigned char*
ossl_quic_wire_decode_transport_param_bytes(PACKET* pkt,
                                            uint64_t* id,
                                            size_t* len)
{
  uint64_t len_;
...
int
ossl_quic_wire_decode_transport_param_int(PACKET* pkt,
                                          uint64_t* id,
                                          uint64_t* value)
{
  PACKET sub;
...
int
ossl_quic_wire_decode_transport_param_cid(PACKET* pkt,
                                          uint64_t* id,
                                          QUIC_CONN_ID* cid)
{
...
int
ossl_quic_wire_decode_transport_param_preferred_addr(PACKET* pkt,
                                                     QUIC_PREFERRED_ADDR* p)
{
  ...
  if (!PACKET_copy_bytes(&pkt2, p->ipv4, sizeof(p->ipv4)) ||
      ...
      !PACKET_copy_bytes(
        &pkt2, p->stateless_reset.token, sizeof(p->stateless_reset.token)))
    return 0;
