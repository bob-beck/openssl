...
int ossl_quic_hkdf_extract(OSSL_LIB_CTX* libctx, const char* propq,
  ...
  *p++ = OSSL_PARAM_construct_utf8_string(OSSL_KDF_PARAM_DIGEST, (char*)md_name,
                                          0);
...
int ossl_quic_provide_initial_secret(OSSL_LIB_CTX* libctx, const char* propq,
  ...
  if (qrx == NULL && qtx == NULL) return 1;

  /* Initial encryption always uses SHA-256. */
  if ((sha256 = EVP_MD_fetch(libctx, "SHA256", propq)) == NULL) return 0;
  ...
  if (qrx != NULL && !ossl_qrx_provide_secret(
                         qrx, QUIC_ENC_LEVEL_INITIAL, QRL_SUITE_AES128GCM,
                         sha256, rx_secret, sizeof(server_initial_secret)))
    goto err;
  ...
  if (qtx != NULL && !ossl_qtx_provide_secret(
                         qtx, QUIC_ENC_LEVEL_INITIAL, QRL_SUITE_AES128GCM,
                         sha256, tx_secret, sizeof(server_initial_secret)))
    goto err;
