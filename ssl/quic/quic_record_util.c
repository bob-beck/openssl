 ...
 */
int
ossl_quic_hkdf_extract(OSSL_LIB_CTX* libctx,
                       const char* propq,
                       const EVP_MD* md,
                       ...
                       size_t out_len)
{
  ...
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_KEY, (unsigned char*)ikm, ikm_len);
...
int
ossl_quic_provide_initial_secret(OSSL_LIB_CTX* libctx,
                                 const char* propq,
                                 ...
                                 struct ossl_qtx_st* qtx)
{
  ...
  if (!ossl_quic_hkdf_extract(libctx,
                              propq,
                              sha256,
  ...
  if (qrx != NULL && !ossl_qrx_provide_secret(qrx,
  ...
  /* Setup TX cipher. */
  if (qtx != NULL && !ossl_qtx_provide_secret(qtx,
                                              QUIC_ENC_LEVEL_INITIAL,
                                              QRL_SUITE_AES128GCM,
                                              sha256,
                                              tx_secret,
...
static const struct suite_info*
...
const char*
ossl_qrl_get_suite_cipher_name(uint32_t suite_id)
{
...
const char*
ossl_qrl_get_suite_md_name(uint32_t suite_id)
{
  const struct suite_info* c = get_suite(suite_id);
...
uint32_t
ossl_qrl_get_suite_secret_len(uint32_t suite_id)
{
  const struct suite_info* c = get_suite(suite_id);
...
uint32_t
ossl_qrl_get_suite_cipher_key_len(uint32_t suite_id)
{
  const struct suite_info* c = get_suite(suite_id);
...
uint32_t
ossl_qrl_get_suite_cipher_iv_len(uint32_t suite_id)
{
  const struct suite_info* c = get_suite(suite_id);
...
uint32_t
ossl_qrl_get_suite_hdr_prot_cipher_id(uint32_t suite_id)
{
  const struct suite_info* c = get_suite(suite_id);
...
uint32_t
ossl_qrl_get_suite_hdr_prot_key_len(uint32_t suite_id)
{
  const struct suite_info* c = get_suite(suite_id);
...
uint64_t
ossl_qrl_get_suite_max_pkt(uint32_t suite_id)
{
  const struct suite_info* c = get_suite(suite_id);
...
uint64_t
ossl_qrl_get_suite_max_forged_pkt(uint32_t suite_id)
{
  const struct suite_info* c = get_suite(suite_id);
