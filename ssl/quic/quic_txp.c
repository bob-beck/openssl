...
static int
tx_helper_init(struct tx_helper* h,
               OSSL_QUIC_TX_PACKETISER* txp,
               uint32_t enc_level,
               size_t max_ppl,
               size_t reserve)
 ...
 */
static int
tx_helper_append_iovec(struct tx_helper* h,
...
static void
on_regen_notify(uint64_t frame_type,
                uint64_t stream_id,
                QUIC_TXPIM_PKT* pkt,
                void* arg);
static void
on_confirm_notify(uint64_t frame_type,
                  uint64_t stream_id,
                  QUIC_TXPIM_PKT* pkt,
                  void* arg);
static void
...
static int
txp_should_try_staging(OSSL_QUIC_TX_PACKETISER* txp,
                       uint32_t enc_level,
                       uint32_t archetype,
                       uint64_t cc_limit,
...
static int
txp_determine_ppl_from_pl(OSSL_QUIC_TX_PACKETISER* txp,
                          size_t pl,
                          uint32_t enc_level,
                          size_t hdr_len,
                          size_t* r);
...
static int
txp_generate_for_el(OSSL_QUIC_TX_PACKETISER* txp,
                    struct txp_pkt* pkt,
                    int chosen_for_conn_close);
static int
txp_pkt_init(struct txp_pkt* pkt,
             OSSL_QUIC_TX_PACKETISER* txp,
             uint32_t enc_level,
             uint32_t archetype,
...
txp_pkt_postgen_update_pkt_overhead(struct txp_pkt* pkt,
                                    OSSL_QUIC_TX_PACKETISER* txp);
static int
txp_pkt_append_padding(struct txp_pkt* pkt,
                       OSSL_QUIC_TX_PACKETISER* txp,
                       size_t num_bytes);
static int
txp_pkt_commit(OSSL_QUIC_TX_PACKETISER* txp,
               struct txp_pkt* pkt,
               uint32_t archetype,
 ...
 */
void
ossl_quic_tx_packetiser_consume_unvalidated_credit(OSSL_QUIC_TX_PACKETISER* txp,
                                                   size_t credit)
{
  if (txp->unvalidated_credit != SIZE_MAX) {
  ...
  void* free_cb_arg)
{
  ...
  if (txp->initial_token != NULL && txp->initial_token_free_cb != NULL)
    txp->initial_token_free_cb(txp->initial_token,
...
void
ossl_quic_tx_packetiser_set_ack_tx_cb(OSSL_QUIC_TX_PACKETISER* txp,
...
void
ossl_quic_tx_packetiser_notify_handshake_complete(OSSL_QUIC_TX_PACKETISER* txp)
{
                                 ...
                                 QUIC_TXP_STATUS* status)
{
  ...
  for (enc_level = QUIC_ENC_LEVEL_INITIAL; enc_level < QUIC_ENC_LEVEL_NUM;
    ...
    if (!txp_should_try_staging(
          txp, enc_level, archetype, cc_limit, &conn_close_enc_level))
      continue;

    if (!txp_pkt_init(
          &pkt[enc_level], txp, enc_level, archetype, running_total))
      /*
    ...
    rc = txp_generate_for_el(
      txp, &pkt[enc_level], conn_close_enc_level == enc_level);
...
static int
txp_get_archetype_data(uint32_t enc_level,
                       uint32_t archetype,
...
static int
txp_determine_geometry(OSSL_QUIC_TX_PACKETISER* txp,
                       uint32_t archetype,
                       uint32_t enc_level,
                       size_t running_total,
                       QUIC_PKT_HDR* phdr,
                          ...
                          size_t* r)
{
  ...
  if (!ossl_qtx_calculate_plaintext_payload_len(
        txp->args.qtx, enc_level, pl, &pl))
    return 0;
...
static int
txp_need_ping(OSSL_QUIC_TX_PACKETISER* txp,
              uint32_t pn_space,
             ...
             size_t running_total)
{
  ...
  if (!txp_determine_geometry(
        txp, archetype, enc_level, running_total, &pkt->phdr, &pkt->geom))
    return 0;
...
static int
txp_generate_pre_token(OSSL_QUIC_TX_PACKETISER* txp,
                       struct txp_pkt* pkt,
                       int chosen_for_conn_close,
                       int* can_be_non_inflight)
{
  ...
  if (a->allow_ack && tx_helper_get_space_left(h) >= MIN_FRAME_SIZE_ACK &&
    ...
    if (ossl_quic_wire_encode_frame_ack(
          wpkt, txp->args.ack_delay_exponent, &ack2)) {
...
static int
determine_len(size_t space_left,
              size_t orig_len,
              size_t base_hdr_len,
              uint64_t* hlen,
              uint64_t* len)
 ...
 */
static int
determine_crypto_len(struct tx_helper* h,
...
static int
determine_stream_len(struct tx_helper* h,
...
static int
txp_generate_crypto_frames(OSSL_QUIC_TX_PACKETISER* txp,
                           struct txp_pkt* pkt,
                           int* have_ack_eliciting)
{
  ...
  for (i = 0;; ++i) {
    ...
    if (!ossl_quic_sstream_get_stream_frame(
          txp->args.crypto[pn_space], i, &shdr, iov, &num_stream_iovec))
...
static int
txp_plan_stream_chunk(OSSL_QUIC_TX_PACKETISER* txp,
                      struct tx_helper* h,
 ...
 */
static int
txp_generate_stream_frames(OSSL_QUIC_TX_PACKETISER* txp,
                           struct txp_pkt* pkt,
                           uint64_t id,
                           QUIC_SSTREAM* sstream,
                           QUIC_TXFC* stream_txfc,
                           ...
                           uint64_t conn_consumed)
{
  ...
  for (i = 0; i < 2; ++i) {
    if (!txp_plan_stream_chunk(
          txp, h, sstream, stream_txfc, i, &chunks[i], conn_consumed))
      goto err;
  ...
  for (i = 0;; ++i) {
    ...
    if (!determine_stream_len(
          h, shdr, space_left, &hdr_len_implicit, &payload_len_implicit)) {
      *packet_full = 1;
      rc = 1;
    ...
    if (use_explicit_len) {
      ...
      if (!determine_stream_len(
            h, shdr, space_left, &hdr_len_explicit, &payload_len_explicit)) {
        *packet_full = 1;
        rc = 1;
        goto err; /* can't fit anything */
      }
    ...
    ossl_quic_sstream_adjust_iov(
      (size_t)shdr->len, chunks[i % 2].iov, chunks[i % 2].num_stream_iovec);
    ...
    for (j = 0; j < chunks[i % 2].num_stream_iovec; ++j)
      tx_helper_append_iovec(
        h, chunks[i % 2].iov[j].buf, chunks[i % 2].iov[j].buf_len);
...
static int
txp_generate_stream_related(OSSL_QUIC_TX_PACKETISER* txp,
                            struct txp_pkt* pkt,
                            int* have_ack_eliciting,
                    ...
                    int chosen_for_conn_close)
{
  ...
  for (cfq_item = ossl_quic_cfq_get_priority_head(txp->args.cfq, pn_space);
    ...
    switch (frame_type) {
      ...
      case OSSL_QUIC_FRAME_TYPE_NEW_TOKEN:
         ...
         */
        if (!done_pre_token)
          if (txp_generate_pre_token(
                txp, pkt, chosen_for_conn_close, &can_be_non_inflight))
            done_pre_token = 1;
  ...
  if (a.allow_stream_rel && txp->handshake_complete)
    if (!txp_generate_stream_related(
          txp, pkt, &have_ack_eliciting, &pkt->stream_head))
      goto fatal_err;
...
int
ossl_quic_tx_packetiser_schedule_conn_close(OSSL_QUIC_TX_PACKETISER* txp,
