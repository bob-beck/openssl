...
static void tx_helper_cleanup(struct tx_helper* h) {
  if (h->txn.active) tx_helper_rollback(h);
...
static int tx_helper_append_iovec(struct tx_helper* h, const unsigned char* buf,
  ...
  if (!ossl_assert(!h->done_implicit)) return 0;

  if (!txp_el_ensure_iovec(el, h->num_iovec + 1)) return 0;
...
static WPACKET* tx_helper_begin(struct tx_helper* h) {
  ...
  if (!ossl_assert(!h->txn.active)) return NULL;

  if (!ossl_assert(!h->done_implicit)) return NULL;

  data = (unsigned char*)el->scratch + h->scratch_bytes;
  len = el->scratch_len - h->scratch_bytes;

  space_left = tx_helper_get_space_left(h);
  if (!ossl_assert(space_left <= len)) return NULL;

  if (!WPACKET_init_static_len(&h->txn.wpkt, data, len, 0)) return NULL;
...
int ossl_quic_tx_packetiser_set_initial_token(
  ...
  if (!txp_check_token_len(token_len, txp_get_mdpl(txp))) return 0;
...
int ossl_quic_tx_packetiser_generate(OSSL_QUIC_TX_PACKETISER* txp,
  ...
  for (enc_level = QUIC_ENC_LEVEL_INITIAL; enc_level < QUIC_ENC_LEVEL_NUM;
    ...
    if (rc != TXP_ERR_SUCCESS) goto out;
  ...
  if (need_padding) {
    ...
    if (pad_el != QUIC_ENC_LEVEL_NUM && total_dgram_size < min_dpl) {
      ...
      if (!txp_pkt_append_padding(&pkt[pad_el], txp, deficit)) goto out;
  ...
  for (enc_level = QUIC_ENC_LEVEL_INITIAL; enc_level < QUIC_ENC_LEVEL_NUM;
    ...
    if (txpim_pkt_reffed) pkt[enc_level].tpkt = NULL; /* don't free */
...
static int txp_determine_geometry(OSSL_QUIC_TX_PACKETISER* txp,
  ...
  if (!txp_get_archetype_data(enc_level, archetype, &geom->adata)) return 0;
   ...
   */
  if (running_total > mdpl) /* Should not be possible, but if it happens: */
    cmpl = 0;
  else
...
static uint32_t txp_determine_archetype(OSSL_QUIC_TX_PACKETISER* txp,
  ...
  for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM;
       ++pn_space)
    if (probe_info->pto[pn_space] > 0) return TX_PACKETISER_ARCHETYPE_PROBE;
   ...
   * want to send an ACK).
   */
  if (cc_limit == 0) return TX_PACKETISER_ARCHETYPE_ACK_ONLY;
...
static int txp_should_try_staging(OSSL_QUIC_TX_PACKETISER* txp,
  ...
  if (!ossl_qtx_is_enc_level_provisioned(txp->args.qtx, enc_level)) return 0;

  if (!txp_get_archetype_data(enc_level, archetype, &a)) return 0;

  if (!a.bypass_cc && cc_limit == 0) /* CC not allowing us to send. */
    return 0;
  ...
  if (a.allow_handshake_done && txp->want_handshake_done) return 1;
    ...
    for (cfq_item = ossl_quic_cfq_get_priority_head(txp->args.cfq, pn_space);
      ...
      switch (frame_type) {
        case OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID:
          if (a.allow_new_conn_id) return 1;
          break;
        case OSSL_QUIC_FRAME_TYPE_RETIRE_CONN_ID:
          if (a.allow_retire_conn_id) return 1;
          break;
        case OSSL_QUIC_FRAME_TYPE_NEW_TOKEN:
          if (a.allow_new_token) return 1;
          break;
        case OSSL_QUIC_FRAME_TYPE_PATH_RESPONSE:
          if (a.allow_path_response) return 1;
          break;
        default:
          if (a.allow_cfq_other) return 1;
          break;
      }
    }
...
static QUIC_SSTREAM* get_sstream_by_id(uint64_t stream_id, uint32_t pn_space,
  ...
  if (stream_id == UINT64_MAX) return txp->args.crypto[pn_space];
...
static int txp_pkt_append_padding(struct txp_pkt* pkt,
  ...
  if (!ossl_assert(pkt->h_valid)) return 0;
...
static int try_commit_conn_close(OSSL_QUIC_TX_PACKETISER* txp, size_t n) {
  ...
  if (txp->closing_bytes_recv == 0) return 1;
   ...
   */
  if (res && txp->closing_bytes_recv != 0) txp->closing_bytes_xmit += n;
  return res;
}
...
static int determine_stream_len(struct tx_helper* h,
  ...
  if (shdr->has_explicit_len) --base_hdr_len;
...
static int txp_generate_crypto_frames(OSSL_QUIC_TX_PACKETISER* txp,
  ...
  for (i = 0;; ++i) {
    ...
    if (space_left < MIN_FRAME_SIZE_CRYPTO) return 1; /* no point trying */
    ...
    if (chdr.len == 0) return 1; /* nothing to do */
    ...
    if (wpkt == NULL) return 0; /* alloc error */
    ...
    if (!tx_helper_commit(h)) return 0; /* alloc error */
...
static int txp_generate_stream_frames(
  ...
  for (i = 0;; ++i) {
    ...
    if (i > 0) /* Load next chunk for lookahead. */
    ...
    /* Encode the header. */
    wpkt = tx_helper_begin(h);
    if (wpkt == NULL) goto err; /* alloc error */
    ...
    if (!tx_helper_commit(h)) goto err; /* alloc error */
    ...
    *have_ack_eliciting = 1;
    tx_helper_unrestrict(h); /* no longer need PING */
    if (!shdr->has_explicit_len) h->done_implicit = 1;
...
static int txp_generate_stream_related(OSSL_QUIC_TX_PACKETISER* txp,
  ...
  for (ossl_quic_stream_iter_init(&it, txp->args.qsm, 1); it.stream != NULL;) {
    ...
    if (ossl_quic_stream_has_send_buffer(stream) &&
      ...
      if (!ossl_assert(!stream->want_reset_stream)) return 0;
...
static int txp_generate_for_el(OSSL_QUIC_TX_PACKETISER* txp,
  ...
  /* Maximum PN reached? */
  if (!ossl_quic_pn_valid(txp->next_pn[pn_space])) goto fatal_err;

  if (!ossl_assert(pkt->tpkt == NULL)) goto fatal_err;
  ...
  for (cfq_item = ossl_quic_cfq_get_priority_head(txp->args.cfq, pn_space);
    ...
    switch (frame_type) {
      case OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID:
        if (!a.allow_new_conn_id) continue;
        break;
      case OSSL_QUIC_FRAME_TYPE_RETIRE_CONN_ID:
        if (!a.allow_retire_conn_id) continue;
        break;
      case OSSL_QUIC_FRAME_TYPE_NEW_TOKEN:
        if (!a.allow_new_token) continue;
        ...
        break;
      case OSSL_QUIC_FRAME_TYPE_PATH_RESPONSE:
        if (!a.allow_path_response) continue;
     ...
     */
    if (encoded_len > tx_helper_get_space_left(h)) break;

    if (!tx_helper_append_iovec(h, encoded, encoded_len)) goto fatal_err;
   ...
   * =========
   */
  if (have_ack_eliciting) can_be_non_inflight = 0;
...
static int txp_pkt_commit(OSSL_QUIC_TX_PACKETISER* txp, struct txp_pkt* pkt,
  ...
  *txpim_pkt_reffed = 0;
  ...
  if (pkt->h.bytes_appended == 0) return 0;
  ...
  /* Dispatch to FIFD. */
  if (!ossl_quic_fifd_pkt_commit(&txp->fifd, tpkt)) return 0;
  ...
  *txpim_pkt_reffed = 1;
  ...
  /* Send the packet. */
  if (!ossl_qtx_write_pkt(txp->args.qtx, &txpkt)) return 0;
  ...
  for (stream = pkt->stream_head; stream != NULL; stream = stream->txp_next) {
    ...
    if (stream->txp_sent_stop_sending) stream->want_stop_sending = 0;

    if (stream->txp_sent_reset_stream) stream->want_reset_stream = 0;
  ...
  if (tpkt->had_handshake_done_frame) txp->want_handshake_done = 0;
  ...
  if (tpkt->had_ack_frame) txp->want_ack &= ~(1UL << pn_space);

  if (tpkt->had_conn_close) txp->want_conn_close = 0;
...
int ossl_quic_tx_packetiser_schedule_conn_close(
  ...
  if (txp->want_conn_close) return 0;

  /*
   * Arbitrarily limit the length of the reason length string to half of the
   * MDPL.
   */
  if (reason_len > max_reason_len) reason_len = max_reason_len;
...
QUIC_PN ossl_quic_tx_packetiser_get_next_pn(OSSL_QUIC_TX_PACKETISER* txp,
                                            uint32_t pn_space) {
  if (pn_space >= QUIC_PN_SPACE_NUM) return UINT64_MAX;
