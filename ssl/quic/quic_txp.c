...
static int
tx_helper_init (struct tx_helper *h, OSSL_QUIC_TX_PACKETISER *txp,
...
static void
tx_helper_cleanup (struct tx_helper *h)
{
  if (h->txn.active)
...
static void
tx_helper_unrestrict (struct tx_helper *h)
{
  h->reserve_allowed = 1;
}
 ...
 */
static int
tx_helper_append_iovec (struct tx_helper *h, const unsigned char *buf,
                        size_t buf_len)
{
...
/*
 * How many more bytes of space do we have left in our plaintext packet
 * payload?
 */
static size_t
tx_helper_get_space_left (struct tx_helper *h)
{
 ...
 */
static WPACKET *
tx_helper_begin (struct tx_helper *h)
{
...
static void
tx_helper_end (struct tx_helper *h, int success)
{
  if (success)
...
static void
tx_helper_rollback (struct tx_helper *h)
{
  if (!h->txn.active)
    return;
...
static int
tx_helper_commit (struct tx_helper *h)
{
  size_t l = 0;
...
static void txp_pkt_cleanup (struct txp_pkt *pkt,
                             OSSL_QUIC_TX_PACKETISER *txp);
 ...
 */
void
ossl_quic_tx_packetiser_set_validated (OSSL_QUIC_TX_PACKETISER *txp)
{
 ...
 */
void
ossl_quic_tx_packetiser_add_unvalidated_credit (OSSL_QUIC_TX_PACKETISER *txp,
                                                size_t credit)
{
 ...
 */
int
ossl_quic_tx_packetiser_check_unvalidated_credit (OSSL_QUIC_TX_PACKETISER *txp,
                                                  size_t req_credit)
{
...
void
ossl_quic_tx_packetiser_free (OSSL_QUIC_TX_PACKETISER *txp)
{
  uint32_t enc_level;
 ...
 * encryption/AEAD tag overheads. After that, the minimum room for frame data
 * in order to guarantee forward progress must be guaranteed. For example, a
 * crypto stream needs to always be able to serialize at least one byte in a
 * CRYPTO frame in order to make forward progress. Because the offset field of
 * a CRYPTO frame uses a variable-length integer, the number of bytes needed to
 * ensure this also varies.
 *
 * Rather than trying to get this boundary condition check actually right,
 * require a reasonable amount of slack to avoid pathological behaviours.
 * (After all, transmitting a CRYPTO stream one byte at a time is probably not
 * desirable anyway.)
 *
...
static int
txp_check_token_len (size_t token_len, size_t mdpl)
{
  if (token_len == 0)
    return 1;
...
int
ossl_quic_tx_packetiser_set_protocol_version (OSSL_QUIC_TX_PACKETISER *txp,
...
int
ossl_quic_tx_packetiser_set_cur_dcid (OSSL_QUIC_TX_PACKETISER *txp,
...
int
ossl_quic_tx_packetiser_set_cur_scid (OSSL_QUIC_TX_PACKETISER *txp,
...
int
ossl_quic_tx_packetiser_set_peer (OSSL_QUIC_TX_PACKETISER *txp,
                                  const BIO_ADDR *peer)
{
  if (peer == NULL)
    {
...
void
ossl_quic_tx_packetiser_set_qlog_cb (OSSL_QUIC_TX_PACKETISER *txp,
...
int
ossl_quic_tx_packetiser_discard_enc_level (OSSL_QUIC_TX_PACKETISER *txp,
                                           uint32_t enc_level)
{
...
void
ossl_quic_tx_packetiser_schedule_handshake_done (OSSL_QUIC_TX_PACKETISER *txp)
{
  txp->want_handshake_done = 1;
}

void
ossl_quic_tx_packetiser_schedule_ack_eliciting (OSSL_QUIC_TX_PACKETISER *txp,
                                                uint32_t pn_space)
{
...
void
ossl_quic_tx_packetiser_schedule_ack (OSSL_QUIC_TX_PACKETISER *txp,
                                      uint32_t pn_space)
{
 ...
 */
int
ossl_quic_tx_packetiser_generate (OSSL_QUIC_TX_PACKETISER *txp,
      ...
      if (rc)
        {
          ...
          if (enc_level == QUIC_ENC_LEVEL_HANDSHAKE)
            status->sent_handshake = (pkt[enc_level].h_valid
...
static int
txp_get_archetype_data (uint32_t enc_level, uint32_t archetype,
...
static int
txp_determine_geometry (OSSL_QUIC_TX_PACKETISER *txp, uint32_t archetype,
                        uint32_t enc_level, size_t running_total,
                        QUIC_PKT_HDR *phdr, struct txp_pkt_geom *geom)
{
...
static uint32_t
txp_determine_archetype (OSSL_QUIC_TX_PACKETISER *txp, uint64_t cc_limit)
{
...
static int
txp_should_try_staging (OSSL_QUIC_TX_PACKETISER *txp, uint32_t enc_level,
                        uint32_t archetype, uint64_t cc_limit,
                        uint32_t *conn_close_enc_level)
{
  struct archetype_data a;
...
static int
sstream_is_pending (QUIC_SSTREAM *sstream)
{
...
static size_t
txp_determine_pn_len (OSSL_QUIC_TX_PACKETISER *txp)
{
...
static int
txp_determine_ppl_from_pl (OSSL_QUIC_TX_PACKETISER *txp, size_t pl,
                           uint32_t enc_level, size_t hdr_len, size_t *r)
{
  if (pl < hdr_len)
    return 0;
...
static size_t
txp_get_mdpl (OSSL_QUIC_TX_PACKETISER *txp)
{
  return ossl_qtx_get_mdpl (txp->args.qtx);
}

static QUIC_SSTREAM *
get_sstream_by_id (uint64_t stream_id, uint32_t pn_space, void *arg)
{
...
static void
on_regen_notify (uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt,
                 void *arg)
{
  OSSL_QUIC_TX_PACKETISER *txp = arg;
...
static int
txp_need_ping (OSSL_QUIC_TX_PACKETISER *txp, uint32_t pn_space,
...
static int
txp_pkt_init (struct txp_pkt *pkt, OSSL_QUIC_TX_PACKETISER *txp,
              uint32_t enc_level, uint32_t archetype, size_t running_total)
{
  uint32_t pn_space = ossl_quic_enc_level_to_pn_space (enc_level);
  ...
  if (!tx_helper_init (&pkt->h, txp, enc_level, pkt->geom.cmppl,
                       txp_need_ping (txp, pn_space, &pkt->geom.adata) ? 1
                                                                       : 0))
    return 0;
...
static void
txp_pkt_cleanup (struct txp_pkt *pkt, OSSL_QUIC_TX_PACKETISER *txp)
{
  if (!pkt->h_valid)
    return;
...
static int
txp_pkt_postgen_update_pkt_overhead (struct txp_pkt *pkt,
                                     OSSL_QUIC_TX_PACKETISER *txp)
{
  /*
...
static void
on_confirm_notify (uint64_t frame_type, uint64_t stream_id,
                   QUIC_TXPIM_PKT *pkt, void *arg)
{
  OSSL_QUIC_TX_PACKETISER *txp = arg;
...
static int
txp_pkt_append_padding (struct txp_pkt *pkt, OSSL_QUIC_TX_PACKETISER *txp,
                        size_t num_bytes)
{
  WPACKET *wpkt;
...
static void
on_sstream_updated (uint64_t stream_id, void *arg)
{
  OSSL_QUIC_TX_PACKETISER *txp = arg;
  QUIC_STREAM *s;
 ...
 */
static int
try_commit_conn_close (OSSL_QUIC_TX_PACKETISER *txp, size_t n)
{
  int res;
...
static int
txp_generate_pre_token (OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt,
                        int chosen_for_conn_close, int *can_be_non_inflight)
{
...
static int
try_len (size_t space_left, size_t orig_len, size_t base_hdr_len,
         size_t lenbytes, uint64_t maxn, size_t *hdr_len, size_t *payload_len)
{
  size_t n;
...
static int
determine_len (size_t space_left, size_t orig_len, size_t base_hdr_len,
 ...
 */
static int
determine_crypto_len (struct tx_helper *h, OSSL_QUIC_FRAME_CRYPTO *chdr,
                      size_t space_left, uint64_t *hlen, uint64_t *len)
{
  size_t orig_len;
...
static int
determine_stream_len (struct tx_helper *h, OSSL_QUIC_FRAME_STREAM *shdr,
                      size_t space_left, uint64_t *hlen, uint64_t *len)
{
  size_t orig_len;
...
static int
txp_generate_crypto_frames (OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt,
...
static int
txp_plan_stream_chunk (OSSL_QUIC_TX_PACKETISER *txp, struct tx_helper *h,
                       QUIC_SSTREAM *sstream, QUIC_TXFC *stream_txfc,
                       size_t skip, struct chunk_info *chunk,
                       uint64_t consumed)
{
 ...
 * *packet_full is set to 1 if there is no longer enough room for another
 * STREAM frame.
 */
static int
txp_generate_stream_frames (OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt,
                            uint64_t id, QUIC_SSTREAM *sstream,
                            QUIC_TXFC *stream_txfc, QUIC_STREAM *next_stream,
                            int *have_ack_eliciting, int *packet_full,
...
static void
txp_enlink_tmp (QUIC_STREAM **tmp_head, QUIC_STREAM *stream)
{
...
static int
txp_generate_stream_related (OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt,
                             int *have_ack_eliciting, QUIC_STREAM **tmp_head)
{
  QUIC_STREAM_ITER it;
  WPACKET *wpkt;
  uint64_t cwm;
...
static int
txp_generate_for_el (OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt,
                     int chosen_for_conn_close)
{
 ...
 */
static int
txp_pkt_commit (OSSL_QUIC_TX_PACKETISER *txp, struct txp_pkt *pkt,
                uint32_t archetype, int *txpim_pkt_reffed)
{
  int rc = 1;
...
static int
txp_el_ensure_iovec (struct txp_el *el, size_t num)
{
...
void
ossl_quic_tx_packetiser_set_msg_callback (OSSL_QUIC_TX_PACKETISER *txp,
                                          ossl_msg_cb msg_callback,
                                          SSL *msg_callback_ssl)
{
...
void
ossl_quic_tx_packetiser_set_msg_callback_arg (OSSL_QUIC_TX_PACKETISER *txp,
                                              void *msg_callback_arg)
{
...
QUIC_PN
ossl_quic_tx_packetiser_get_next_pn (OSSL_QUIC_TX_PACKETISER *txp,
                                     uint32_t pn_space)
{
...
OSSL_TIME
ossl_quic_tx_packetiser_get_deadline (OSSL_QUIC_TX_PACKETISER *txp)
{
  /*
