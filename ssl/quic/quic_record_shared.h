...
/*
 * States an EL can be in. The Updating and Cooldown states are used by RX
 * only; a TX EL in the Provisioned state is always in the Normal substate.
 *
 ...
 * we are in the PROVISIONED state), 0 if we do not yet have material (we are
 * in the UNPROVISIONED state) and -1 if the EL is discarded (we are in the
 * DISCARDED state).
 */
...
/*
 * Returns 1 if the given keyslot index is currently valid for a given EL and
 * EL state.
 */
