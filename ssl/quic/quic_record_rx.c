...
OSSL_QRX* ossl_qrx_new(const OSSL_QRX_ARGS* args) {
  ...
  if (args->demux == NULL || args->max_deferred == 0) return NULL;

  qrx = OPENSSL_zalloc(sizeof(OSSL_QRX));
  if (qrx == NULL) return NULL;
...
static int qrx_validate_initial_pkt(OSSL_QRX* qrx, QUIC_URXE* urxe,
  ...
  if (rxe->hdr.type != QUIC_PKT_TYPE_INITIAL) goto malformed;

  if (!qrx_validate_hdr_early(qrx, rxe, NULL)) goto malformed;
  ...
  if (!ossl_quic_hdr_protector_decrypt(&el->hpr, &ptrs)) goto malformed;
  ...
  if (!qrx_validate_hdr(qrx, rxe)) goto malformed;
   ...
   */
  if (!qrx_validate_hdr_late(qrx, rxe)) goto malformed;
  ...
  if (rxe->pn > qrx->largest_pn[pn_space]) qrx->largest_pn[pn_space] = rxe->pn;
...
int ossl_qrx_provide_secret(OSSL_QRX* qrx, uint32_t enc_level,
  ...
  if (enc_level >= QUIC_ENC_LEVEL_NUM) return 0;
...
static RXE* qrx_alloc_rxe(size_t alloc_len) {
  ...
  if (alloc_len >= SIZE_MAX - sizeof(RXE)) return NULL;
...
static RXE* qrx_resize_rxe(RXE_LIST* rxl, RXE* rxe, size_t n) {
  ...
  if (n >= SIZE_MAX - sizeof(RXE)) return NULL;
  ...
  if (!ossl_assert(rxe->refcount == 0)) return NULL;
...
static int qrx_validate_hdr_early(OSSL_QRX* qrx, RXE* rxe,
  ...
  if (rxe->hdr.type == QUIC_PKT_TYPE_0RTT) return 0;
...
static size_t qrx_get_cipher_ctx_idx(OSSL_QRX* qrx, OSSL_QRL_ENC_LEVEL* el,
  ...
  *is_old_key = 0;
  ...
  if (!ossl_assert(key_phase_bit <= 1)) return SIZE_MAX;
...
static int qrx_decrypt_pkt_body(OSSL_QRX* qrx, unsigned char* dst,
  ...
  if (src_len > INT_MAX || aad_len > INT_MAX) return 0;

  /* We should not have been called if we do not have key material. */
  if (!ossl_assert(el != NULL)) return 0;
  ...
  if (!ossl_assert(cctx_idx < OSSL_NELEM(el->cctx))) return 0;
  ...
  if (!ossl_assert(nonce_len >= (int)sizeof(QUIC_PN))) return 0;
  ...
  /* Feed AAD data. */
  if (EVP_CipherUpdate(cctx, NULL, &l, aad, (int)aad_len) != 1) return 0;
...
static int qrx_process_pkt(OSSL_QRX* qrx, QUIC_URXE* urxe, PACKET* pkt,
  ...
  if (pkt_is_marked(&urxe->processed, pkt_idx)) already_processed = 1;
   ...
   */
  if (pkt_idx == 0) *first_dcid = rxe->hdr.dst_conn_id;
...
static int qrx_process_datagram(OSSL_QRX* qrx, QUIC_URXE* e,
  ...
  if (!PACKET_buf_init(&pkt, data, data_len)) return 0;
...
static int qrx_process_one_urxe(OSSL_QRX* qrx, QUIC_URXE* e) {
  ...
  if (!ossl_assert(e == ossl_list_urxe_head(&qrx->urx_pending))) return 0;
...
static int qrx_process_pending_urxl(OSSL_QRX* qrx) {
  ...
  while ((e = ossl_list_urxe_head(&qrx->urx_pending)) != NULL)
    if (!qrx_process_one_urxe(qrx, e)) return 0;
...
int ossl_qrx_read_pkt(OSSL_QRX* qrx, OSSL_QRX_PKT** ppkt) {
  ...
  if (!ossl_qrx_processed_read_pending(qrx)) {
    if (!qrx_process_pending_urxl(qrx)) return 0;

    if (!ossl_qrx_processed_read_pending(qrx)) return 0;
  }

  rxe = qrx_pop_pending_rxe(qrx);
  if (!ossl_assert(rxe != NULL)) return 0;
...
void ossl_qrx_pkt_release(OSSL_QRX_PKT* pkt) {
  ...
  rxe = (RXE*)pkt;
  assert(rxe->refcount > 0);
  if (--rxe->refcount == 0) qrx_recycle_rxe(pkt->qrx, rxe);
}
