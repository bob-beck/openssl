...
static int
qrx_validate_hdr_early(OSSL_QRX* qrx, RXE* rxe, const QUIC_CONN_ID* first_dcid);
static int
qrx_relocate_buffer(OSSL_QRX* qrx,
                    RXE** prxe,
                    size_t* pi,
...
static int
qrx_decrypt_pkt_body(OSSL_QRX* qrx,
                     unsigned char* dst,
 ...
 */
static int
qrx_validate_initial_pkt(OSSL_QRX* qrx,
                         QUIC_URXE* urxe,
...
int
ossl_qrx_validate_initial_packet(OSSL_QRX* qrx,
                                 QUIC_URXE* urxe,
...
int
ossl_qrx_provide_secret(OSSL_QRX* qrx,
                        uint32_t enc_level,
                        uint32_t suite_id,
                        EVP_MD* md,
...
qrx_validate_hdr(OSSL_QRX* qrx, RXE* rxe)
{
  ...
  if (!ossl_quic_wire_decode_pkt_hdr_pn(
        rxe->hdr.pn, rxe->hdr.pn_len, qrx->largest_pn[pn_space], &rxe->pn))
    return 0;
 ...
 */
static size_t
qrx_get_cipher_ctx_idx(OSSL_QRX* qrx,
                     ...
                     uint64_t* rx_key_epoch)
{
  ...
  cctx_idx = qrx_get_cipher_ctx_idx(
    qrx, el, enc_level, key_phase_bit, rx_key_epoch, &is_old_key);
  ...
  *pkt = orig_pkt;
  ...
  if (need_second_decode) {
    ...
    if (ossl_quic_wire_decode_pkt_hdr(
          pkt, qrx->short_conn_id_len, 0, 0, &rxe->hdr, NULL, NULL) != 1)
      goto malformed;
  }
...
static int
qrx_process_datagram(OSSL_QRX* qrx,
                     QUIC_URXE* e,
...
int
ossl_qrx_set_key_update_cb(OSSL_QRX* qrx,
...
void
ossl_qrx_set_msg_callback(OSSL_QRX* qrx,
                          ossl_msg_cb msg_callback,
                          SSL* msg_callback_ssl)
{
