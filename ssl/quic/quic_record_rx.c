...
pkt_mark (uint64_t *bitf, size_t pkt_idx)
{
...
pkt_is_marked (const uint64_t *bitf, size_t pkt_idx)
{
  assert (pkt_idx < QUIC_MAX_PKT_PER_URXE);
...
OSSL_QRX *
ossl_qrx_new (const OSSL_QRX_ARGS *args)
{
  OSSL_QRX *qrx;
  size_t i;
...
static void
qrx_cleanup_urxl (OSSL_QRX *qrx, QUIC_URXE_LIST *l)
{
  QUIC_URXE *e, *enext;
...
void
ossl_qrx_update_pn_space (OSSL_QRX *src, OSSL_QRX *dst)
{
  size_t i;
...
void
ossl_qrx_inject_urxe (OSSL_QRX *qrx, QUIC_URXE *urxe)
{
...
void
ossl_qrx_inject_pkt (OSSL_QRX *qrx, OSSL_QRX_PKT *pkt)
{
  RXE *rxe = (RXE *)pkt;
 ...
 */
static int
qrx_validate_initial_pkt (OSSL_QRX *qrx, QUIC_URXE *urxe,
                          const QUIC_CONN_ID *first_dcid, size_t datagram_len)
{
  PACKET pkt, orig_pkt;
  RXE *rxe;
...
int
ossl_qrx_validate_initial_packet (OSSL_QRX *qrx, QUIC_URXE *urxe,
...
static void
qrx_requeue_deferred (OSSL_QRX *qrx)
{
  QUIC_URXE *e;
...
int
ossl_qrx_provide_secret (OSSL_QRX *qrx, uint32_t enc_level, uint32_t suite_id,
                         EVP_MD *md, const unsigned char *secret,
                         size_t secret_len)
{
...
int
ossl_qrx_discard_enc_level (OSSL_QRX *qrx, uint32_t enc_level)
{
  if (enc_level >= QUIC_ENC_LEVEL_NUM)
    return 0;
...
int
ossl_qrx_processed_read_pending (OSSL_QRX *qrx)
{
...
int
ossl_qrx_unprocessed_read_pending (OSSL_QRX *qrx)
{
...
static RXE *
qrx_pop_pending_rxe (OSSL_QRX *qrx)
{
...
/*
 * Ensures there is at least one RXE in the RX free list, allocating a new
 * entry if necessary. The returned RXE is in the RX free list; it is not
 * popped.
 *
 * alloc_len is a hint which may be used to determine the RXE size if
 * allocation is necessary. Returns NULL on allocation failure.
 */
static RXE *
qrx_ensure_free_rxe (OSSL_QRX *qrx, size_t alloc_len)
{
  RXE *rxe;
 ...
 */
static RXE *
qrx_resize_rxe (RXE_LIST *rxl, RXE *rxe, size_t n)
{
  RXE *rxe2, *p;
 ...
 */
static RXE *
qrx_reserve_rxe (RXE_LIST *rxl, RXE *rxe, size_t n)
{
  if (rxe->alloc_len >= n)
    return rxe;
...
static void
qrx_recycle_rxe (OSSL_QRX *qrx, RXE *rxe)
{
 ...
 */
static int
qrx_relocate_buffer (OSSL_QRX *qrx, RXE **prxe, size_t *pi,
                     const unsigned char **pptr, size_t buf_len)
{
  RXE *rxe;
  unsigned char *dst;
...
static uint32_t
qrx_determine_enc_level (const QUIC_PKT_HDR *hdr)
{
  switch (hdr->type)
    {
...
static uint32_t
rxe_determine_pn_space (RXE *rxe)
{
  uint32_t enc_level;
...
static int
qrx_validate_hdr_early (OSSL_QRX *qrx, RXE *rxe,
                        const QUIC_CONN_ID *first_dcid)
{
...
/* Validate header and decode PN. */
static int
qrx_validate_hdr (OSSL_QRX *qrx, RXE *rxe)
{
...
static int
qrx_validate_hdr_late (OSSL_QRX *qrx, RXE *rxe)
{
  int pn_space = rxe_determine_pn_space (rxe);
 ...
 */
static size_t
qrx_get_cipher_ctx_idx (OSSL_QRX *qrx, OSSL_QRL_ENC_LEVEL *el,
                        uint32_t enc_level, unsigned char key_phase_bit,
 ...
 */
static int
qrx_decrypt_pkt_body (OSSL_QRX *qrx, unsigned char *dst,
                      const unsigned char *src, size_t src_len,
                      size_t *dec_len, const unsigned char *aad,
                      size_t aad_len, QUIC_PN pn, uint32_t enc_level,
                      unsigned char key_phase_bit, uint64_t *rx_key_epoch)
{
...
static void
qrx_key_update_initiated (OSSL_QRX *qrx, QUIC_PN pn)
{
...
static int
qrx_process_pkt (OSSL_QRX *qrx, QUIC_URXE *urxe, PACKET *pkt, size_t pkt_idx,
                 QUIC_CONN_ID *first_dcid, size_t datagram_len)
{
  RXE *rxe;
...
static int
qrx_process_datagram (OSSL_QRX *qrx, QUIC_URXE *e, const unsigned char *data,
                      size_t data_len)
{
  int have_deferred = 0;
  PACKET pkt;
...
static int
qrx_process_one_urxe (OSSL_QRX *qrx, QUIC_URXE *e)
{
  int was_deferred;
...
static int
qrx_process_pending_urxl (OSSL_QRX *qrx)
{
  QUIC_URXE *e;
...
int
ossl_qrx_read_pkt (OSSL_QRX *qrx, OSSL_QRX_PKT **ppkt)
{
  RXE *rxe;
...
void
ossl_qrx_pkt_release (OSSL_QRX_PKT *pkt)
{
  RXE *rxe;
...
void
ossl_qrx_pkt_orphan (OSSL_QRX_PKT *pkt)
{
  RXE *rxe;
...
void
ossl_qrx_pkt_up_ref (OSSL_QRX_PKT *pkt)
{
  RXE *rxe = (RXE *)pkt;
...
uint64_t
ossl_qrx_get_bytes_received (OSSL_QRX *qrx, int clear)
{
...
int
ossl_qrx_set_late_validation_cb (OSSL_QRX *qrx,
                                 ossl_qrx_late_validation_cb *cb, void *cb_arg)
{
...
int
ossl_qrx_set_key_update_cb (OSSL_QRX *qrx, ossl_qrx_key_update_cb *cb,
                            void *cb_arg)
{
...
uint64_t
ossl_qrx_get_key_epoch (OSSL_QRX *qrx)
{
  OSSL_QRL_ENC_LEVEL *el
...
int
ossl_qrx_key_update_timeout (OSSL_QRX *qrx, int normal)
{
  OSSL_QRL_ENC_LEVEL *el
      = ossl_qrl_enc_level_set_get (&qrx->el_set, QUIC_ENC_LEVEL_1RTT, 1);
...
uint64_t
ossl_qrx_get_cur_forged_pkt_count (OSSL_QRX *qrx)
{
  return qrx->forged_pkt_count;
}

uint64_t
ossl_qrx_get_max_forged_pkt_count (OSSL_QRX *qrx, uint32_t enc_level)
{
  OSSL_QRL_ENC_LEVEL *el
      = ossl_qrl_enc_level_set_get (&qrx->el_set, enc_level, 1);
...
void
ossl_qrx_allow_1rtt_processing (OSSL_QRX *qrx)
{
  if (qrx->allow_1rtt)
    return;
...
void
ossl_qrx_set_msg_callback (OSSL_QRX *qrx, ossl_msg_cb msg_callback,
                           SSL *msg_callback_ssl)
{
...
void
ossl_qrx_set_msg_callback_arg (OSSL_QRX *qrx, void *msg_callback_arg)
{
  qrx->msg_callback_arg = msg_callback_arg;
}

size_t
ossl_qrx_get_short_hdr_conn_id_len (OSSL_QRX *qrx)
{
