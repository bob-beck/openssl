...
QLOG* ossl_qlog_new(const QLOG_TRACE_INFO* info) {
  ...
  if (qlog->info.now_cb == NULL) qlog->info.now_cb = default_now;
...
QLOG* ossl_qlog_new_from_env(const QLOG_TRACE_INFO* info) {
  ...
  if (info == NULL || qlogdir == NULL) return NULL;
  ...
  if (qlogdir_sep != '\0') filename[l++] = qlogdir_sep;
  ...
  if (qlog == NULL) goto err;

  if (!ossl_qlog_set_sink_filename(qlog, filename)) goto err;

  if (qfilter == NULL || qfilter[0] == '\0') qfilter = "*";

  if (!ossl_qlog_set_filter(qlog, qfilter)) goto err;
...
int ossl_qlog_set_event_type_enabled(QLOG* qlog, uint32_t event_type,
                                     int enabled) {
  if (qlog == NULL || event_type >= QLOG_EVENT_TYPE_NUM) return 0;
...
void ossl_qlog_group_begin(QLOG* qlog, const char* name) {
  if (name != NULL) ossl_json_key(&qlog->json, name);
...
static int lex_match(struct lexer* lex, const char* s, size_t s_len) {
  if ((size_t)(lex->term_end - lex->p) != s_len) return 0;
...
int ossl_qlog_set_filter(QLOG* qlog, const char* filter) {
  ...
  if (!lex_init(&lex, filter, strlen(filter))) return 0;
