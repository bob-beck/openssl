...
QLOG *ossl_qlog_new(const QLOG_TRACE_INFO *info)
{
	...
	if (info->description != NULL &&
	    (qlog->info.description = OPENSSL_strdup(info->description)) ==
		    NULL)
		goto err;
...
QLOG *ossl_qlog_new_from_env(const QLOG_TRACE_INFO *info)
{
	...
	for (i = 0; i < info->odcid.id_len; ++i)
		l += BIO_snprintf(filename + l, strl - l, "%02x",
				  info->odcid.id[i]);
...
static void qlog_event_seq_header(QLOG *qlog)
{
	...
	ossl_json_object_begin(&qlog->json);
	{
		...
		write_str_once(qlog, "description",
			       (char **)&qlog->info.description);
		...
		ossl_json_object_begin(&qlog->json);
		{
			...
			ossl_json_object_begin(&qlog->json);
			{
				...
				write_str_once(qlog, "group_id",
					       (char **)&qlog->info.group_id);

				ossl_json_key(&qlog->json, "system_info");
				ossl_json_object_begin(&qlog->json);
				{
					if (qlog->info.override_process_id !=
					    0) {
						ossl_json_key(&qlog->json,
							      "process_id");
						ossl_json_u64(
							&qlog->json,
							qlog->info
