...
bit_get (const size_t *p, uint32_t bit_no)
{
  return p[bit_no / BITS_PER_WORD] & (((size_t)1) << (bit_no % BITS_PER_WORD));
}

static ossl_unused ossl_inline void
bit_set (size_t *p, uint32_t bit_no, int enable)
{
...
QLOG *
ossl_qlog_new_from_env (const QLOG_TRACE_INFO *info)
{
  QLOG *qlog = NULL;
 ...
 */
int
ossl_qlog_set_sink_bio (QLOG *qlog, BIO *bio)
{
  if (qlog == NULL)
    return 0;
...
int
ossl_qlog_set_sink_file (QLOG *qlog, FILE *f, int close_flag)
{
  BIO *bio;
...
int
ossl_qlog_set_sink_filename (QLOG *qlog, const char *filename)
{
  BIO *bio;
...
int
ossl_qlog_set_event_type_enabled (QLOG *qlog, uint32_t event_type, int enabled)
{
...
int
ossl_qlog_enabled (QLOG *qlog, uint32_t event_type)
{
  if (qlog == NULL)
    return 0;
 ...
 */
static void
write_str_once (QLOG *qlog, const char *key, char **p)
{
  if (*p == NULL)
    return;
...
static void
qlog_event_seq_header (QLOG *qlog)
{
  if (qlog->header_done)
    return;
...
int
ossl_qlog_event_try_begin (QLOG *qlog, uint32_t event_type,
 ...
 */
void
ossl_qlog_group_begin (QLOG *qlog, const char *name)
{
  if (name != NULL)
    ossl_json_key (&qlog->json, name);
...
void
ossl_qlog_group_end (QLOG *qlog)
{
  ossl_json_object_end (&qlog->json);
}

void
ossl_qlog_array_begin (QLOG *qlog, const char *name)
{
  if (name != NULL)
    ossl_json_key (&qlog->json, name);
...
void
ossl_qlog_array_end (QLOG *qlog)
{
  ossl_json_array_end (&qlog->json);
}

void
ossl_qlog_override_time (QLOG *qlog, OSSL_TIME event_time)
{
  qlog->event_time = event_time;
}

void
ossl_qlog_str (QLOG *qlog, const char *name, const char *value)
{
  if (name != NULL)
    ossl_json_key (&qlog->json, name);
...
void
ossl_qlog_str_len (QLOG *qlog, const char *name, const char *value,
                   size_t value_len)
{
  if (name != NULL)
    ossl_json_key (&qlog->json, name);
...
void
ossl_qlog_u64 (QLOG *qlog, const char *name, uint64_t value)
{
  if (name != NULL)
    ossl_json_key (&qlog->json, name);
...
void
ossl_qlog_i64 (QLOG *qlog, const char *name, int64_t value)
{
  if (name != NULL)
    ossl_json_key (&qlog->json, name);
...
void
ossl_qlog_bool (QLOG *qlog, const char *name, bool value)
{
  if (name != NULL)
    ossl_json_key (&qlog->json, name);
...
void
ossl_qlog_bin (QLOG *qlog, const char *name, const void *value,
               size_t value_len)
{
  if (name != NULL)
    ossl_json_key (&qlog->json, name);
...
static int
lex_init (struct lexer *lex, const char *in, size_t in_len)
{
  if (in == NULL)
    return 0;
...
static int
lex_eot (struct lexer *lex)
{
  return lex->p == lex->term_end;
}

static int
lex_peek_char (struct lexer *lex)
{
  return lex_eot (lex) ? -1 : *lex->p;
}

static int
lex_skip_char (struct lexer *lex)
{
  if (lex_eot (lex))
    return 0;
...
static int
lex_match (struct lexer *lex, const char *s, size_t s_len)
{
...
static void
lex_get_rest (struct lexer *lex, const char **str, size_t *str_l)
{
  *str = lex->p;
...
static int
lex_extract_to (struct lexer *lex, char c, const char **str, size_t *str_l)
{
...
filter_match_event (const char *cat, size_t cat_l, const char *event,
                    size_t event_l, const char *expect_cat,
 ...
 */
static void
filter_apply (size_t *enabled, int add, const char *cat, size_t cat_l,
...
static int
lex_fail (struct lexer *lex, const char *msg)
{
  /*
...
static int
validate_name (const char **p, size_t *l)
{
  const char *p_ = *p;
  size_t i, l_ = *l;
...
int
ossl_qlog_set_filter (QLOG *qlog, const char *filter)
{
  struct lexer lex = { 0 };
  char c;
