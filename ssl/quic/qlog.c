...
ossl_qlog_new(const QLOG_TRACE_INFO* info)
{
  ...
  if (!ossl_json_init(
        &qlog->json, NULL, OSSL_JSON_FLAG_IJSON | OSSL_JSON_FLAG_SEQ))
    goto err;
...
ossl_qlog_new_from_env(const QLOG_TRACE_INFO* info)
{
  ...
  l += BIO_snprintf(
    filename + l, strl - l, "_%s.sqlog", info->is_server ? "server" : "client");
...
int
ossl_qlog_event_try_begin(QLOG* qlog,
                          uint32_t event_type,
...
void
ossl_qlog_bin(QLOG* qlog, const char* name, const void* value, size_t value_len)
{
  if (name != NULL)
    ossl_json_key(&qlog->json, name);
