...
OSSL_QTX *
ossl_qtx_new (const OSSL_QTX_ARGS *args)
{
  OSSL_QTX *qtx;
...
void
ossl_qtx_set_mutator (OSSL_QTX *qtx, ossl_mutate_packet_cb mutatecb,
...
void
ossl_qtx_set_qlog_cb (OSSL_QTX *qtx, QLOG *(*get_qlog_cb) (void *arg),
                      void *get_qlog_cb_arg)
{
...
int
ossl_qtx_provide_secret (OSSL_QTX *qtx, uint32_t enc_level, uint32_t suite_id,
...
int
ossl_qtx_discard_enc_level (OSSL_QTX *qtx, uint32_t enc_level)
{
  if (enc_level >= QUIC_ENC_LEVEL_NUM)
    return 0;
...
int
ossl_qtx_is_enc_level_provisioned (OSSL_QTX *qtx, uint32_t enc_level)
{
...
/* Allocate a new TXE. */
static TXE *
qtx_alloc_txe (size_t alloc_len)
{
  TXE *txe;
 ...
 *
 * alloc_len is a hint which may be used to determine the TXE size if
 * allocation is necessary. Returns NULL on allocation failure.
 */
static TXE *
qtx_ensure_free_txe (OSSL_QTX *qtx, size_t alloc_len)
{
  TXE *txe;
 ...
 */
static TXE *
qtx_resize_txe (OSSL_QTX *qtx, TXE_LIST *txl, TXE *txe, size_t n)
{
  TXE *txe2, *p;
 ...
 */
static TXE *
qtx_reserve_txe (OSSL_QTX *qtx, TXE_LIST *txl, TXE *txe, size_t n)
{
  if (txe->alloc_len >= n)
    return txe;
...
static void
qtx_pending_to_free (OSSL_QTX *qtx)
{
...
static void
qtx_add_to_pending (OSSL_QTX *qtx, TXE *txe)
{
...
static size_t
iovec_total_bytes (const OSSL_QTX_IOVEC *iovec, size_t num_iovec)
{
  size_t i, l = 0;
...
static void
iovec_cur_init (struct iovec_cur *cur, const OSSL_QTX_IOVEC *iovec,
                size_t num_iovec)
{
  cur->iovec = iovec;
 ...
 */
static size_t
iovec_cur_get_buffer (struct iovec_cur *cur, const unsigned char **buf,
                      size_t max_buf_len)
{
  size_t l;
...
int
ossl_qtx_calculate_ciphertext_payload_len (OSSL_QTX *qtx, uint32_t enc_level,
...
int
ossl_qtx_calculate_plaintext_payload_len (OSSL_QTX *qtx, uint32_t enc_level,
                                          size_t ciphertext_len,
                                          size_t *plaintext_len)
{
  OSSL_QRL_ENC_LEVEL *el
...
static int
qtx_write_hdr (OSSL_QTX *qtx, const QUIC_PKT_HDR *hdr, TXE *txe,
...
static int
qtx_encrypt_into_txe (OSSL_QTX *qtx, struct iovec_cur *cur, TXE *txe,
                      uint32_t enc_level, QUIC_PN pn, const unsigned char *hdr,
                      size_t hdr_len, QUIC_PKT_HDR_PTRS *ptrs)
{
 ...
 */
static int
qtx_write (OSSL_QTX *qtx, const OSSL_QTX_PKT *pkt, TXE *txe,
           uint32_t enc_level, QUIC_PKT_HDR *hdr, const OSSL_QTX_IOVEC *iovec,
           size_t num_iovec)
{
...
static int
qtx_mutate_write (OSSL_QTX *qtx, const OSSL_QTX_PKT *pkt, TXE *txe,
                  uint32_t enc_level)
{
  int ret;
  QUIC_PKT_HDR *hdr;
...
static int
addr_eq (const BIO_ADDR *a, const BIO_ADDR *b)
{
...
int
ossl_qtx_write_pkt (OSSL_QTX *qtx, const OSSL_QTX_PKT *pkt)
{
  int ret;
 ...
 */
void
ossl_qtx_finish_dgram (OSSL_QTX *qtx)
{
  TXE *txe = qtx->cons;
...
static void
txe_to_msg (TXE *txe, BIO_MSG *msg)
{
...
int
ossl_qtx_flush_net (OSSL_QTX *qtx)
{
...
int
ossl_qtx_pop_net (OSSL_QTX *qtx, BIO_MSG *msg)
{
  TXE *txe = ossl_list_txe_head (&qtx->pending);
...
void
ossl_qtx_set_bio (OSSL_QTX *qtx, BIO *bio)
{
  qtx->bio = bio;
}

int
ossl_qtx_set_mdpl (OSSL_QTX *qtx, size_t mdpl)
{
...
size_t
ossl_qtx_get_mdpl (OSSL_QTX *qtx)
{
  return qtx->mdpl;
}

size_t
ossl_qtx_get_queue_len_datagrams (OSSL_QTX *qtx)
{
  return qtx->pending_count;
}

size_t
ossl_qtx_get_queue_len_bytes (OSSL_QTX *qtx)
{
  return qtx->pending_bytes;
}

size_t
ossl_qtx_get_cur_dgram_len_bytes (OSSL_QTX *qtx)
{
  return qtx->cons != NULL ? qtx->cons->data_len : 0;
}

size_t
ossl_qtx_get_unflushed_pkt_count (OSSL_QTX *qtx)
{
  return qtx->cons_count;
}

int
ossl_qtx_trigger_key_update (OSSL_QTX *qtx)
{
  return ossl_qrl_enc_level_set_key_update (&qtx->el_set, QUIC_ENC_LEVEL_1RTT);
}

uint64_t
ossl_qtx_get_cur_epoch_pkt_count (OSSL_QTX *qtx, uint32_t enc_level)
{
  OSSL_QRL_ENC_LEVEL *el;
...
uint64_t
ossl_qtx_get_max_epoch_pkt_count (OSSL_QTX *qtx, uint32_t enc_level)
{
  OSSL_QRL_ENC_LEVEL *el;
...
void
ossl_qtx_set_msg_callback (OSSL_QTX *qtx, ossl_msg_cb msg_callback,
                           SSL *msg_callback_ssl)
{
...
void
ossl_qtx_set_msg_callback_arg (OSSL_QTX *qtx, void *msg_callback_arg)
{
  qtx->msg_callback_arg = msg_callback_arg;
}

uint64_t
ossl_qtx_get_key_epoch (OSSL_QTX *qtx)
{
  OSSL_QRL_ENC_LEVEL *el;
