...
OSSL_QTX* ossl_qtx_new(const OSSL_QTX_ARGS* args) {
  ...
  if (args->mdpl < QUIC_MIN_INITIAL_DGRAM_LEN) return 0;
...
static TXE* qtx_alloc_txe(size_t alloc_len) {
  ...
  if (alloc_len >= SIZE_MAX - sizeof(TXE)) return NULL;

  txe = OPENSSL_malloc(sizeof(TXE) + alloc_len);
  if (txe == NULL) return NULL;
...
static TXE* qtx_resize_txe(OSSL_QTX* qtx, TXE_LIST* txl, TXE* txe, size_t n) {
  ...
  if (n >= SIZE_MAX - sizeof(TXE)) return NULL;
...
static size_t iovec_cur_get_buffer(struct iovec_cur* cur,
  ...
  for (;;) {
    if (cur->idx >= cur->num_iovec) return 0;

    l = cur->iovec[cur->idx].buf_len - cur->byte_off;
    if (l > max_buf_len) l = max_buf_len;
...
static int qtx_encrypt_into_txe(OSSL_QTX* qtx, struct iovec_cur* cur, TXE* txe,
  ...
  if (!ossl_quic_hdr_protector_encrypt(&el->hpr, ptrs)) return 0;
...
static QLOG* qtx_get_qlog(OSSL_QTX* qtx) {
  if (qtx->get_qlog_cb == NULL) return NULL;
...
static int qtx_mutate_write(OSSL_QTX* qtx, const OSSL_QTX_PKT* pkt, TXE* txe,
  ...
  if (qtx->finishmutatecb != NULL) qtx->finishmutatecb(qtx->mutatearg);
...
int ossl_qtx_write_pkt(OSSL_QTX* qtx, const OSSL_QTX_PKT* pkt) {
  ...
  for (;;) {
    ...
    if (txe == NULL) return 0; /* allocation failure */
    ...
    if (!was_coalescing) {
      ...
      if (pkt->peer != NULL) {
        if (!BIO_ADDR_copy(&txe->peer, pkt->peer)) return 0;
      } else {
        BIO_ADDR_clear(&txe->peer);
      }

      if (pkt->local != NULL) {
        if (!BIO_ADDR_copy(&txe->local, pkt->local)) return 0;
      } else {
   ...
   */
  if (ossl_quic_pkt_type_must_be_last(pkt->hdr->type)) coalescing = 0;

  if (!coalescing) ossl_qtx_finish_dgram(qtx);
...
int ossl_qtx_flush_net(OSSL_QTX* qtx) {
  ...
  if (qtx->bio == NULL) return QTX_FLUSH_NET_RES_PERMANENT_FAIL;
...
int ossl_qtx_set_mdpl(OSSL_QTX* qtx, size_t mdpl) {
  if (mdpl < QUIC_MIN_INITIAL_DGRAM_LEN) return 0;
