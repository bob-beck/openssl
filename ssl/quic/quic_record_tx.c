...
void
ossl_qtx_set_mutator(OSSL_QTX* qtx,
                     ossl_mutate_packet_cb mutatecb,
                     ossl_finish_mutate_cb finishmutatecb,
                     void* mutatearg)
{
...
void
ossl_qtx_set_qlog_cb(OSSL_QTX* qtx,
                     QLOG* (*get_qlog_cb)(void* arg),
                     void* get_qlog_cb_arg)
{
...
int
ossl_qtx_provide_secret(OSSL_QTX* qtx,
                        uint32_t enc_level,
                        uint32_t suite_id,
                        EVP_MD* md,
...
static void
iovec_cur_init(struct iovec_cur* cur,
               const OSSL_QTX_IOVEC* iovec,
               size_t num_iovec)
{
  cur->iovec = iovec;
 ...
 */
static size_t
iovec_cur_get_buffer(struct iovec_cur* cur,
...
int
ossl_qtx_calculate_ciphertext_payload_len(OSSL_QTX* qtx,
                                          uint32_t enc_level,
...
int
ossl_qtx_calculate_plaintext_payload_len(OSSL_QTX* qtx,
                                         uint32_t enc_level,
                                         size_t ciphertext_len,
                                         size_t* plaintext_len)
{
              ...
              QUIC_PKT_HDR_PTRS* ptrs)
{
  ...
  if (!ossl_quic_wire_encode_pkt_hdr(
        &wpkt, hdr->dst_conn_id.id_len, hdr, ptrs) ||
...
static int
qtx_encrypt_into_txe(OSSL_QTX* qtx,
                     ...
                     QUIC_PKT_HDR_PTRS* ptrs)
{
  ...
  for (;;) {
    ...
    if (EVP_CipherUpdate(
          cctx, txe_data(txe) + txe->data_len, &l, src, (int)src_len) != 1) {
      ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
      return 0;
    }
          ...
          size_t num_iovec)
{
  ...
  if (!needs_encrypt) {
    ...
    /* Encrypt into TXE. */
    if (!qtx_encrypt_into_txe(
          qtx, &cur, txe, enc_level, pkt->pn, hdr_start, hdr_len, &ptrs)) {
      ret = QTX_FAIL_GENERIC;
      goto err;
    }
...
void
ossl_qtx_set_msg_callback(OSSL_QTX* qtx,
                          ossl_msg_cb msg_callback,
                          SSL* msg_callback_ssl)
{
