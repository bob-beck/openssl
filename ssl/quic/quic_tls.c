...
static int
quic_free(OSSL_RECORD_LAYER* r);

static int
quic_new_record_layer(OSSL_LIB_CTX* libctx,
                      const char* propq,
                      int vers,
                      int role,
                      int direction,
                      ...
                      OSSL_RECORD_LAYER** retrl)
{
  ...
  *retrl = rl;
  ...
  if (!rl->qtls->args.yield_secret_cb(level,
                                      qdir,
                                      suite_id,
                                      (EVP_MD*)kdfdigest,
                                      secret,
...
static size_t
quic_get_max_records(OSSL_RECORD_LAYER* rl,
                     uint8_t type,
                     size_t len,
                     size_t maxfrag,
                     size_t* preffrag)
...
static int
quic_write_records(OSSL_RECORD_LAYER* rl,
                   OSSL_RECORD_TEMPLATE* template,
                   size_t numtempl)
{
  size_t consumed;
...
static int
quic_read_record(OSSL_RECORD_LAYER* rl,
                 void** rechandle,
                 int* rversion,
                 uint8_t* type,
  ...
  *type = SSL3_RT_HANDSHAKE;
  ...
  if (rl->msg_callback != NULL) {
    ...
    rl->msg_callback(0,
                     ...
                     rl->cbarg);
    rl->msg_callback(
      0, TLS1_3_VERSION, SSL3_RT_INNER_CONTENT_TYPE, type, 1, rl->cbarg);
  }
...
static void
quic_get_state(OSSL_RECORD_LAYER* rl,
               const char** shortstr,
               const char** longstr)
{
  /*
...
static void
free_transport_params_cb(SSL* s,
                         unsigned int ext_type,
                         unsigned int context,
                         const unsigned char* out,
                         void* add_arg)
...
static int
parse_transport_params_cb(SSL* s,
                          unsigned int ext_type,
                          unsigned int context,
...
ossl_quic_tls_tick(QUIC_TLS* qtls)
{
  ...
  if (ret <= 0) {
    err = ossl_ssl_get_error(qtls->args.s,
                             ret,
...
int
ossl_quic_tls_get_error(QUIC_TLS* qtls,
                        uint64_t* error_code,
                        const char** error_msg,
                        ERR_STATE** error_state)
{
