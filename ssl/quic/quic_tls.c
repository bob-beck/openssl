...
static int quic_new_record_layer(
  ...
  *retrl = rl;
  ...
  if (level == OSSL_RECORD_PROTECTION_LEVEL_NONE) return 1;
...
#else
    if (!ossl_assert("Should not happen" == NULL)) goto err;
#endif
  } else {
    kdfdigest = NULL;
  }

  if (!rl->qtls->args.yield_secret_cb(level, qdir, suite_id, (EVP_MD*)kdfdigest,
                                      secret, secretlen,
...
static void quic_get_state(OSSL_RECORD_LAYER* rl, const char** shortstr,
  ...
  if (rl->qtls->inerror) {
    if (shortstr != NULL) *shortstr = "unknown";
    if (longstr != NULL) *longstr = "unknown";
  } else {
    if (shortstr != NULL) *shortstr = "RH";
    if (longstr != NULL) *longstr = "read header";
  }
}
...
int ossl_quic_tls_configure(QUIC_TLS* qtls) {
  ...
  if (nullbio == NULL) return RAISE_INTERNAL_ERROR(qtls);
...
int ossl_quic_tls_tick(QUIC_TLS* qtls) {
  ...
  if (!qtls->configured) {
    ...
    if (sc == NULL) return RAISE_INTERNAL_ERROR(qtls);
    ...
    if (qtls->args.is_server) {
      if (sctx->ext.alpn_select_cb == NULL) return RAISE_INTERNAL_ERROR(qtls);
    } else {
    ...
    if (!ossl_quic_tls_configure(qtls)) return RAISE_INTERNAL_ERROR(qtls);
...
int ossl_quic_tls_set_transport_params(QUIC_TLS* qtls,
  ...
  if (qtls->local_transport_params_consumed) return 0;
