...
int ossl_quic_sstream_get_stream_frame(QUIC_SSTREAM* qss, size_t skip,
  ...
  if (*num_iov < 2) return 0;

  for (i = 0; i < skip && range != NULL; ++i)
    range = ossl_list_uint_set_next(range);

  if (range == NULL) {
    if (i < skip) /* Don't return FIN for infinitely increasing skip */
      return 0;

    /* No new bytes to send, but we might have a FIN */
    if (!qss->have_final_size || qss->sent_final_size) return 0;
  ...
  for (i = 0;; ++i) {
    ...
    if (total_len + src_len > max_len) src_len = (size_t)(max_len - total_len);
...
int ossl_quic_sstream_mark_transmitted(QUIC_SSTREAM* qss, uint64_t start,
  ...
  if (!ossl_uint_set_remove(&qss->new_set, &r)) return 0;
...
int ossl_quic_sstream_mark_lost(QUIC_SSTREAM* qss, uint64_t start,
   ...
   */
  if (!ossl_uint_set_insert(&qss->new_set, &r)) return 0;
...
int ossl_quic_sstream_mark_acked(QUIC_SSTREAM* qss, uint64_t start,
  ...
  if (!ossl_uint_set_insert(&qss->acked_set, &r)) return 0;
...
int ossl_quic_sstream_is_totally_acked(QUIC_SSTREAM* qss) {
  ...
  if (qss->have_final_size && !qss->acked_final_size) return 0;

  if (ossl_quic_sstream_get_cur_size(qss) == 0) return 1;

  if (ossl_list_uint_set_num(&qss->acked_set) != 1) return 0;
