...
static int
crypto_send_cb(const unsigned char* buf,
               size_t buf_len,
               size_t* consumed,
               void* arg)
{
...
static int
yield_secret_cb(uint32_t prot_level,
                int direction,
                uint32_t suite_id,
                EVP_MD* md,
                ...
                void* arg)
{
  ...
  if (sc == NULL)
    return 0;
  return sc->qtcb.yield_secret_cb(
    s, prot_level, direction, secret, secret_len, sc->qtarg);
}

static int
got_transport_params_cb(const unsigned char* params,
                        size_t params_len,
                        void* arg)
{
  SSL* s = (SSL*)arg;
...
int
SSL_set_quic_tls_transport_params(SSL* s,
