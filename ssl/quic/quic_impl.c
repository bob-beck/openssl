...
static void qc_set_default_xso_keep_ref(QUIC_CONNECTION* qc,
                                        QUIC_XSO* xso,
                                        int touch,
                                        QUIC_XSO** old_xso);
 ...
 */
QUIC_TAKES_LOCK
static uint64_t quic_mask_or_options(SSL* ssl,
                                     uint64_t mask_value,
                                     uint64_t or_value) {
  QCTX ctx;
 ...
 */
QUIC_TAKES_LOCK
int ossl_quic_conn_shutdown(SSL* s,
                            uint64_t flags,
 ...
 */
QUIC_NEEDS_LOCK
static void quic_post_write(QUIC_XSO* xso,
                            int did_append,
                            int did_append_all,
                            uint64_t flags,
                            int do_tick) {
 ...
 */
QUIC_NEEDS_LOCK
static int xso_sstream_append(QUIC_XSO* xso,
                              const unsigned char* buf,
                              size_t len,
...
QUIC_NEEDS_LOCK
static int quic_write_blocking(QCTX* ctx,
                               const void* buf,
                               size_t len,
                               uint64_t flags,
                               size_t* written) {
 ...
 * write semantics.
 */
static void aon_write_begin(QUIC_XSO* xso,
                            const unsigned char* buf,
                            size_t buf_len,
                            size_t already_sent) {
...
QUIC_NEEDS_LOCK
static int quic_write_nonblocking_aon(QCTX* ctx,
                                      const void* buf,
                                      size_t len,
                                      uint64_t flags,
                                      size_t* written) {
...
QUIC_NEEDS_LOCK
static int quic_write_nonblocking_epw(QCTX* ctx,
                                      const void* buf,
                                      size_t len,
                                      uint64_t flags,
                                      size_t* written) {
...
QUIC_TAKES_LOCK
int ossl_quic_write_flags(SSL* s,
                          const void* buf,
                          size_t len,
                          uint64_t flags,
                          size_t* written) {
 ...
 */
QUIC_TAKES_LOCK
int SSL_inject_net_dgram(SSL* s,
                         const unsigned char* buf,
                         size_t buf_len,
                         const BIO_ADDR* peer,
 ...
 */
QUIC_TAKES_LOCK
static int qc_getset_idle_timeout(QCTX* ctx,
                                  uint32_t class_,
...
QUIC_TAKES_LOCK
static int qc_get_stream_avail(QCTX* ctx,
                               uint32_t class_,
                               int is_uni,
                               int is_remote,
                               uint64_t* value) {
...
QUIC_TAKES_LOCK
static int qc_getset_event_handling(QCTX* ctx,
                                    uint32_t class_,
                                    uint64_t* p_value_out,
                                    uint64_t* p_value_in) {
  int ret = 0;
...
QUIC_TAKES_LOCK
int ossl_quic_get_value_uint(SSL* s,
                             uint32_t class_,
                             uint32_t id,
                             uint64_t* value) {
  QCTX ctx;
...
QUIC_TAKES_LOCK
int ossl_quic_set_value_uint(SSL* s,
                             uint32_t class_,
                             uint32_t id,
                             uint64_t value) {
  QCTX ctx;
 ...
 * ------------------------------
 */
static int quic_get_stream_error_code(SSL* ssl,
                                      int is_write,
                                      uint64_t* app_error_code) {
  QCTX ctx;
  int state;
 ...
 */
static QUIC_TOKEN* ossl_quic_build_new_token(BIO_ADDR* peer,
                                             uint8_t* token,
                                             size_t token_len) {
...
int ossl_quic_set_peer_token(SSL_CTX* ctx,
                             BIO_ADDR* peer,
                             const uint8_t* token,
                             size_t token_len) {
...
QUIC_TAKES_LOCK
int ossl_quic_conn_poll_events(SSL* ssl,
                               uint64_t events,
                               int do_tick,
                               uint64_t* p_revents) {
  QCTX ctx;
