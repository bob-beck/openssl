 ...
 *   - whether a QSSO was passed (xso == NULL must not be used to determine
 * this because it may be non-NULL when a QCSO is passed if that QCSO has a
 *     default stream);
 ...
 *     SSL_do_handshake() implicitly) are not allowed to change the return
 * value of SSL_get_error.
 */
struct qctx_st
{
  QUIC_OBJ *obj;
...
QUIC_NEEDS_LOCK
static void
quic_set_last_error (QCTX *ctx, int last_error)
{
  if (!ctx->in_io)
    return;
...
QUIC_NEEDS_LOCK
static int
quic_raise_normal_error (QCTX *ctx, int err)
{
  assert (ctx->in_io);
 ...
 */
static int
quic_raise_non_normal_error (QCTX *ctx, const char *file, int line,
 ...
 *      attached.
 *
 *      (Note this means there is no current way to require an SSL object with
 * a QUIC stream which is not a QCSO; a QCSO with a default stream attached is
 * always considered to satisfy QCTX_S.)
 *
 *   QCTX_AUTO_S
 *      The input SSL object may be a QSSO or a QCSO with a default stream
 ...
 *      This determines whether SSL_get_error() is updated; the value it
 * returns is modified only by an I/O call.
 *
 *   QCTX_NO_ERROR
 ...
 * raise a reasonable error code based on the configured preconditions in
 * flags.
 */
static int
wrong_type (const SSL *s, uint32_t flags)
{
 ...
 * the SSL object, and assuming the preconditions demanded by the flags field
 ...
 */
static int
expect_quic_as (const SSL *s, QCTX *ctx, uint32_t flags)
{
...
static int
is_quic_c (const SSL *s, QCTX *ctx, int raiseerrs)
{
 ...
 */
static int
is_quic_cs (const SSL *s, QCTX *ctx, int raiseerrs)
{
...
static int
expect_quic_cs (const SSL *s, QCTX *ctx)
{
  return expect_quic_as (s, ctx, QCTX_C | QCTX_S);
}

static int
expect_quic_csl (const SSL *s, QCTX *ctx)
{
  return expect_quic_as (s, ctx, QCTX_C | QCTX_S | QCTX_L);
}

static int
expect_quic_csld (const SSL *s, QCTX *ctx)
{
...
static int
expect_quic_listener (const SSL *s, QCTX *ctx)
{
  return expect_quic_as (s, ctx, QCTX_L);
}

static int
expect_quic_domain (const SSL *s, QCTX *ctx)
{
 ...
 * stream.
 *
 * remote_init determines if we expect the default XSO to be remotely created
 * or not. If it is -1, do not instantiate a default XSO if one does not yet
 * exist.
 *
 * Channel mutex is acquired and retained on success.
 */
QUIC_ACQUIRES_LOCK
static int ossl_unused
expect_quic_with_stream_lock (const SSL *s, int remote_init, int in_io,
                              QCTX *ctx)
{
 ...
 */
static int ossl_unused
expect_quic_conn_only (const SSL *s, QCTX *ctx)
{
...
/*
 * This predicate is the criterion which should determine API call rejection
 * for *most* mutating API calls, particularly stream-related operations for
 * send parts.
 *
 ...
 */
static int
quic_mutation_allowed (QUIC_CONNECTION *qc, int req_active)
{
...
static int
qctx_is_top_level (QCTX *ctx)
{
  return ctx->obj->parent_obj == NULL;
}

static int
qctx_blocking (QCTX *ctx)
{
...
QUIC_NEEDS_LOCK
static int
block_until_pred (QCTX *ctx, int (*pred) (void *arg), void *pred_arg,
                  uint32_t flags)
{
  QUIC_ENGINE *qeng;
  QUIC_REACTOR *rtor;
...
QUIC_NEEDS_LOCK
static void
quic_unref_port_bios (QUIC_PORT *port)
{
  BIO *b;
...
QUIC_NEEDS_LOCK
static void
qc_cleanup (QUIC_CONNECTION *qc, int have_lock)
{
  SSL_free (qc->tls);
  qc->tls = NULL;
  ...
  if (qc->port != NULL && qc->listener == NULL && qc->pending == 0)
    { /* TODO */
  ...
  if (ctx.is_stream)
    {
      ...
      ossl_quic_stream_map_update_state (
          ossl_quic_channel_get_qsm (ctx.qc->ch), ctx.xso->stream);
...
int
ossl_quic_set_override_now_cb (SSL *s, OSSL_TIME (*now_cb) (void *arg),
                               void *now_cb_arg)
{
  QCTX ctx;
...
void
ossl_quic_conn_force_assist_thread_wake (SSL *s)
{
  QCTX ctx;
...
QUIC_NEEDS_LOCK
static void
qc_touch_default_xso (QUIC_CONNECTION *qc)
{
...
QUIC_NEEDS_LOCK
static void
qc_set_default_xso_keep_ref (QUIC_CONNECTION *qc, QUIC_XSO *xso, int touch,
                             QUIC_XSO **old_xso)
{
  int refs;
...
QUIC_NEEDS_LOCK
static void
qc_set_default_xso (QUIC_CONNECTION *qc, QUIC_XSO *xso, int touch)
{
...
QUIC_NEEDS_LOCK
static void
xso_update_options (QUIC_XSO *xso)
{
...
QUIC_TAKES_LOCK
static uint64_t
quic_mask_or_options (SSL *ssl, uint64_t mask_value, uint64_t or_value)
{
  QCTX ctx;
...
uint64_t
ossl_quic_set_options (SSL *ssl, uint64_t options)
{
...
/* SSL_clear_options */
uint64_t
ossl_quic_clear_options (SSL *ssl, uint64_t options)
{
...
/* SSL_get_options */
uint64_t
ossl_quic_get_options (const SSL *ssl)
{
 ...
 *     we need to be able to have timeouts on recv() calls, and make best
 * effort (non blocking) send() and recv() calls.
 *
 ...
 *
 *     Some OSes support a MSG_DONTWAIT flag which allows a single I/O option
 * to be made non-blocking. However some OSes (e.g. Windows) do not support
 *     this, so we cannot rely on this.
 *
 *     As such, we need to configure any FD in non-blocking mode. This may
 *     confound users who pass a blocking socket to libssl. However, in
 * practice it would be extremely strange for a user of QUIC to pass an FD to
 * us, then also try and send receive traffic on the same socket(!). Thus the
 ...
 *     changed to a malicious value (e.g. if some random unrelated entity on
 * the network starts sending traffic to us) on every read call. This is not a
 *     direct issue because we use the 'stateless' BIO_sendmmsg and
 * BIO_recvmmsg calls only, which do not use this variable. However, we do need
 * to let the user specify the peer in a 'normal' manner. The compromise here
 * is that we grab the current peer value set at the time the write BIO is set
 *     and do not read the value again.
 *
 *   - We also need to support memory BIOs (e.g. BIO_dgram_pair) or custom
 * BIOs. Currently we do this by only supporting non-blocking mode.
 *
 */
 ...
 */
static int
csm_analyse_init_peer_addr (BIO *net_wbio, BIO_ADDR *peer)
{
...
void
ossl_quic_conn_set0_net_rbio (SSL *s, BIO *net_rbio)
{
  QCTX ctx;
...
void
ossl_quic_conn_set0_net_wbio (SSL *s, BIO *net_wbio)
{
  QCTX ctx;
...
BIO *
ossl_quic_conn_get_net_rbio (const SSL *s)
{
  QCTX ctx;
  QUIC_PORT *port;
...
BIO *
ossl_quic_conn_get_net_wbio (const SSL *s)
{
  QCTX ctx;
  QUIC_PORT *port;
...
int
ossl_quic_conn_get_blocking_mode (const SSL *s)
{
  QCTX ctx;
...
QUIC_TAKES_LOCK
int
ossl_quic_conn_set_blocking_mode (SSL *s, int blocking)
{
  int ret = 0;
  unsigned int mode;
  QCTX ctx;
...
int
ossl_quic_conn_set_initial_peer_addr (SSL *s, const BIO_ADDR *peer_addr)
{
  QCTX ctx;
...
QUIC_TAKES_LOCK
int
ossl_quic_get_event_timeout (SSL *s, struct timeval *tv, int *is_infinite)
{
  QCTX ctx;
...
int
ossl_quic_get_rpoll_descriptor (SSL *s, BIO_POLL_DESCRIPTOR *desc)
{
  QCTX ctx;
  QUIC_PORT *port = NULL;
  BIO *net_rbio;
...
int
ossl_quic_get_wpoll_descriptor (SSL *s, BIO_POLL_DESCRIPTOR *desc)
{
  QCTX ctx;
  QUIC_PORT *port = NULL;
  BIO *net_wbio;
...
QUIC_TAKES_LOCK
int
ossl_quic_get_net_read_desired (SSL *s)
{
  QCTX ctx;
  int ret;
  ...
  qctx_lock (&ctx);
  ret = ossl_quic_reactor_net_read_desired (
...
QUIC_TAKES_LOCK
int
ossl_quic_get_net_write_desired (SSL *s)
{
  int ret;
  QCTX ctx;
...
QUIC_NEEDS_LOCK
static void
qc_shutdown_flush_init (QUIC_CONNECTION *qc)
{
  QUIC_STREAM_MAP *qsm;
...
QUIC_NEEDS_LOCK
static int
qc_shutdown_flush_finished (QUIC_CONNECTION *qc)
{
...
static int
quic_shutdown_flush_wait (void *arg)
{
  QUIC_CONNECTION *qc = arg;
...
static int
quic_shutdown_peer_wait (void *arg)
{
  QUIC_CONNECTION *qc = arg;
  return ossl_quic_channel_is_term_any (qc->ch);
}
 ...
 * nor for peer to close connection (when running with
 * SSL_SHUTDOWN_FLAG_WAIT_PEER) Application is supposed to call SSL_shutdown()
 * repeatedly as long as function returns 0 which indicates the operation is
 * still in progress.
 *
...
QUIC_TAKES_LOCK
int
ossl_quic_conn_shutdown (SSL *s, uint64_t flags,
      ...
      if (!qc_shutdown_flush_finished (ctx.qc))
        {
          if (!no_block && qctx_blocking (&ctx))
            {
              ret = block_until_pred (&ctx, quic_shutdown_flush_wait, ctx.qc,
                                      0);
              if (ret < 1)
                {
                  ret = 0;
...
/* SSL_ctrl */
long
ossl_quic_ctrl (SSL *s, int cmd, long larg, void *parg)
{
  QCTX ctx;
...
int
ossl_quic_set_connect_state (SSL *s, int raiseerrs)
{
  QCTX ctx;
...
int
ossl_quic_set_accept_state (SSL *s, int raiseerrs)
{
  QCTX ctx;
...
static int
tls_wants_non_io_retry (QUIC_CONNECTION *qc)
{
...
static int
configure_channel (QUIC_CONNECTION *qc)
{
  assert (qc->ch != NULL);
...
static int
need_notifier_for_domain_flags (uint64_t domain_flags)
{
...
QUIC_NEEDS_LOCK
static int
create_channel (QUIC_CONNECTION *qc, SSL_CTX *ctx)
{
...
QUIC_NEEDS_LOCK
static int
ensure_channel_started (QCTX *ctx)
{
...
QUIC_NEEDS_LOCK
static int
qc_try_create_default_xso_for_write (QCTX *ctx)
{
  uint64_t flags = 0;
  QUIC_CONNECTION *qc = ctx->qc;
...
QUIC_NEEDS_LOCK
static int
qc_wait_for_default_xso_for_read (QCTX *ctx, int peek)
{
...
QUIC_NEEDS_LOCK
static QUIC_XSO *
create_xso_from_stream (QUIC_CONNECTION *qc, QUIC_STREAM *qs)
{
  QUIC_XSO *xso = NULL;
...
static SSL *
quic_conn_stream_new (QCTX *ctx, uint64_t flags, int need_lock)
{
  int ret;
  QUIC_CONNECTION *qc = ctx->qc;
  QUIC_XSO *xso = NULL;
...
QUIC_TAKES_LOCK
SSL *
ossl_quic_conn_stream_new (SSL *s, uint64_t flags)
{
  QCTX ctx;
...
/* SSL_get_error */
int
ossl_quic_get_error (const SSL *s, int i)
{
  QCTX ctx;
 ...
 * ---------
 *
 * The set of functions below provide the implementation of the public
 * SSL_write function. We must handle:
 *
...
QUIC_NEEDS_LOCK
static void
quic_post_write (QUIC_XSO *xso, int did_append, int did_append_all,
...
QUIC_NEEDS_LOCK
static int
sstream_ensure_spare (QUIC_SSTREAM *sstream, uint64_t spare)
{
...
QUIC_NEEDS_LOCK
static int
xso_sstream_append (QUIC_XSO *xso, const unsigned char *buf, size_t len,
...
QUIC_NEEDS_LOCK
static int
quic_write_blocking (QCTX *ctx, const void *buf, size_t len, uint64_t flags,
                     size_t *written)
{
  int res;
 ...
 */
static void
aon_write_begin (QUIC_XSO *xso, const unsigned char *buf, size_t buf_len,
                 size_t already_sent)
{
...
QUIC_NEEDS_LOCK
static int
quic_write_nonblocking_aon (QCTX *ctx, const void *buf, size_t len,
...
QUIC_NEEDS_LOCK
static int
quic_write_nonblocking_epw (QCTX *ctx, const void *buf, size_t len,
                            uint64_t flags, size_t *written)
{
  QUIC_XSO *xso = ctx->xso;
...
QUIC_NEEDS_LOCK
static int
quic_validate_for_write (QUIC_XSO *xso, int *err)
{
  QUIC_STREAM_MAP *qsm;
...
QUIC_TAKES_LOCK
int
ossl_quic_write_flags (SSL *s, const void *buf, size_t len, uint64_t flags,
                       size_t *written)
{
  int ret;
  QCTX ctx;
  ...
  *written = 0;
  ...
  if (!quic_mutation_allowed (ctx.qc, /*req_active=*/0))
    {
      ret = QUIC_RAISE_NON_NORMAL_ERROR (&ctx, SSL_R_PROTOCOL_IS_SHUTDOWN,
                                         NULL);
      goto out;
    }
...
QUIC_TAKES_LOCK
int
ossl_quic_write (SSL *s, const void *buf, size_t len, size_t *written)
{
...
QUIC_NEEDS_LOCK
static int
quic_validate_for_read (QUIC_XSO *xso, int *err, int *eos)
{
  QUIC_STREAM_MAP *qsm;
...
QUIC_NEEDS_LOCK
static int
quic_read_actual (QCTX *ctx, QUIC_STREAM *stream, void *buf, size_t buf_len,
                  size_t *bytes_read, int peek)
{
...
QUIC_TAKES_LOCK
static int
quic_read (SSL *s, void *buf, size_t len, size_t *bytes_read, int peek)
{
  int ret, res;
  QCTX ctx;
...
int
ossl_quic_read (SSL *s, void *buf, size_t len, size_t *bytes_read)
{
  return quic_read (s, buf, len, bytes_read, 0);
}

int
ossl_quic_peek (SSL *s, void *buf, size_t len, size_t *bytes_read)
{
...
QUIC_TAKES_LOCK
static size_t
ossl_quic_pending_int (const SSL *s, int check_channel)
{
  QCTX ctx;
  size_t avail = 0;
...
size_t
ossl_quic_pending (const SSL *s)
{
  return ossl_quic_pending_int (s, /*check_channel=*/0);
}

int
ossl_quic_has_pending (const SSL *s)
{
...
QUIC_TAKES_LOCK
int
ossl_quic_conn_stream_conclude (SSL *s)
{
  QCTX ctx;
  QUIC_STREAM *qs;
  int err;
...
QUIC_TAKES_LOCK
int
SSL_inject_net_dgram (SSL *s, const unsigned char *buf, size_t buf_len,
 ...
 */
SSL *
ossl_quic_get0_connection (SSL *s)
{
  QCTX ctx;
 ...
 */
int
ossl_quic_get_domain_flags (const SSL *ssl, uint64_t *domain_flags)
{
  QCTX ctx;
 ...
 */
int
ossl_quic_get_stream_type (SSL *s)
{
  QCTX ctx;
...
QUIC_TAKES_LOCK
int
ossl_quic_is_stream_local (SSL *s)
{
  QCTX ctx;
  int is_local;
...
QUIC_TAKES_LOCK
int
ossl_quic_set_default_stream_mode (SSL *s, uint32_t mode)
{
  QCTX ctx;
...
QUIC_TAKES_LOCK
int
ossl_quic_attach_stream (SSL *conn, SSL *stream)
{
  QCTX ctx;
  QUIC_XSO *xso;
  int nref;
...
QUIC_NEEDS_LOCK
static int
qc_get_effective_incoming_stream_policy (QUIC_CONNECTION *qc)
{
...
QUIC_NEEDS_LOCK
static void
qc_update_reject_policy (QUIC_CONNECTION *qc)
{
...
QUIC_TAKES_LOCK
int
ossl_quic_set_incoming_stream_policy (SSL *s, int policy, uint64_t aec)
{
  int ret = 1;
  QCTX ctx;
...
QUIC_TAKES_LOCK
static int
qc_getset_idle_timeout (QCTX *ctx, uint32_t class_, uint64_t *p_value_out,
                        uint64_t *p_value_in)
{
  int ret = 0;
...
QUIC_TAKES_LOCK
static int
qc_get_stream_avail (QCTX *ctx, uint32_t class_, int is_uni, int is_remote,
                     uint64_t *value)
{
  int ret = 0;
  ...
  *value = is_remote
               ? ossl_quic_channel_get_remote_stream_count_avail (ctx->qc->ch,
                                                                  is_uni)
               : ossl_quic_channel_get_local_stream_count_avail (ctx->qc->ch,
                                                                 is_uni);
...
QUIC_TAKES_LOCK
static int
qc_getset_event_handling (QCTX *ctx, uint32_t class_, uint64_t *p_value_out,
                          uint64_t *p_value_in)
{
  int ret = 0;
...
QUIC_TAKES_LOCK
static int
qc_get_stream_write_buf_stat (QCTX *ctx, uint32_t class_,
...
QUIC_NEEDS_LOCK
static int
expect_quic_for_value (SSL *s, QCTX *ctx, uint32_t id)
{
  switch (id)
    {
...
QUIC_TAKES_LOCK
int
ossl_quic_get_value_uint (SSL *s, uint32_t class_, uint32_t id,
                          uint64_t *value)
{
  QCTX ctx;
...
QUIC_TAKES_LOCK
int
ossl_quic_set_value_uint (SSL *s, uint32_t class_, uint32_t id, uint64_t value)
{
  QCTX ctx;
...
QUIC_NEEDS_LOCK
static int
wait_for_incoming_stream (void *arg)
{
...
QUIC_TAKES_LOCK
SSL *
ossl_quic_accept_stream (SSL *s, uint64_t flags)
{
  QCTX ctx;
  int ret;
  SSL *new_s = NULL;
  ...
  ossl_statm_get_rtt_info (ossl_quic_channel_get_statm (ctx.qc->ch),
                           &rtt_info);
...
QUIC_TAKES_LOCK
size_t
ossl_quic_get_accept_stream_queue_len (SSL *s)
{
  QCTX ctx;
  size_t v;
 ...
 */
int
ossl_quic_stream_reset (SSL *ssl, const SSL_STREAM_RESET_ARGS *args,
                        size_t args_len)
{
  QCTX ctx;
  QUIC_STREAM_MAP *qsm;
 ...
 */
static void
quic_classify_stream (QUIC_CONNECTION *qc, QUIC_STREAM *qs, int is_write,
                      int *state, uint64_t *app_error_code)
{
  int local_init;
  uint64_t final_size;
...
static int
quic_get_stream_state (SSL *ssl, int is_write)
{
  QCTX ctx;
  int state;
...
int
ossl_quic_get_stream_read_state (SSL *ssl)
{
 ...
 */
int
ossl_quic_get_stream_write_state (SSL *ssl)
{
 ...
 */
static int
quic_get_stream_error_code (SSL *ssl, int is_write, uint64_t *app_error_code)
{
  QCTX ctx;
  int state;
...
int
ossl_quic_get_stream_read_error_code (SSL *ssl, uint64_t *app_error_code)
{
 ...
 */
int
ossl_quic_get_stream_write_error_code (SSL *ssl, uint64_t *app_error_code)
{
 ...
 */
int
ossl_quic_set_write_buffer_size (SSL *ssl, size_t size)
{
  int ret = 0;
  QCTX ctx;
 ...
 */
int
ossl_quic_get_conn_close_info (SSL *ssl, SSL_CONN_CLOSE_INFO *info,
                               size_t info_len)
{
  QCTX ctx;
 ...
 */
int
ossl_quic_key_update (SSL *ssl, int update_type)
{
  QCTX ctx;
 ...
 */
int
ossl_quic_get_key_update_type (const SSL *s)
{
  /*
 ...
 */
static SSL *
alloc_port_user_ssl (QUIC_CHANNEL *ch, void *arg)
{
 ...
 */
SSL *
ossl_quic_new_listener (SSL_CTX *ctx, uint64_t flags)
{
 ...
 */
SSL *
ossl_quic_new_listener_from (SSL *ssl, uint64_t flags)
{
  QCTX ctx;
  QUIC_LISTENER *ql = NULL;
  QUIC_PORT_ARGS port_args = { 0 };
 ...
 */
SSL *
ossl_quic_new_from_listener (SSL *ssl, uint64_t flags)
{
  QCTX ctx;
  QUIC_CONNECTION *qc = NULL;
  QUIC_LISTENER *ql;
 ...
 */
static int
quic_accept_connection_wait (void *arg)
{
  QUIC_PORT *port = arg;
...
QUIC_TAKES_LOCK
SSL *
ossl_quic_accept_connection (SSL *ssl, uint64_t flags)
{
  int ret;
  QCTX ctx;
  SSL *conn_ssl = NULL;
...
quic_token_hash (const QUIC_TOKEN *item)
{
  return (unsigned long)ossl_fnv1a_hash (item->hashkey, item->hashkey_len);
}

static int
quic_token_cmp (const QUIC_TOKEN *a, const QUIC_TOKEN *b)
{
...
SSL_TOKEN_STORE *
ossl_quic_new_token_store (void)
{
  int ok = 0;
...
static void
free_this_token (QUIC_TOKEN *tok)
{
  ossl_quic_free_peer_token (tok);
}

void
ossl_quic_free_token_store (SSL_TOKEN_STORE *hdl)
{
  int refs;
 ...
 */
static QUIC_TOKEN *
ossl_quic_build_new_token (BIO_ADDR *peer, uint8_t *token, size_t token_len)
{
...
int
ossl_quic_set_peer_token (SSL_CTX *ctx, BIO_ADDR *peer, const uint8_t *token,
                          size_t token_len)
{
...
int
ossl_quic_get_peer_token (SSL_CTX *ctx, BIO_ADDR *peer, QUIC_TOKEN **token)
{
  SSL_TOKEN_STORE *c = ctx->tokencache;
  QUIC_TOKEN *key = NULL;
  QUIC_TOKEN *tok = NULL;
...
void
ossl_quic_free_peer_token (QUIC_TOKEN *token)
{
  int refs = 0;
...
QUIC_TAKES_LOCK
size_t
ossl_quic_get_accept_connection_queue_len (SSL *ssl)
{
  QCTX ctx;
  int ret;
 ...
 */
SSL *
ossl_quic_new_domain (SSL_CTX *ctx, uint64_t flags)
{
  QUIC_DOMAIN *qd = NULL;
  QUIC_ENGINE_ARGS engine_args = { 0 };
...
long
ossl_quic_ctx_ctrl (SSL_CTX *ctx, int cmd, long larg, void *parg)
{
  switch (cmd)
    {
    default:
...
long
ossl_quic_callback_ctrl (SSL *s, int cmd, void (*fp) (void))
{
  QCTX ctx;
...
long
ossl_quic_ctx_callback_ctrl (SSL_CTX *ctx, int cmd, void (*fp) (void))
{
  return ssl3_ctx_callback_ctrl (ctx, cmd, fp);
}

int
ossl_quic_renegotiate_check (SSL *ssl, int initok)
{
 ...
 */
int
ossl_quic_get_shutdown (const SSL *s)
{
  QCTX ctx;
  int shut = 0;
...
QUIC_NEEDS_LOCK
static int
test_poll_event_r (QUIC_XSO *xso)
{
  int fin = 0;
  size_t avail = 0;
...
QUIC_NEEDS_LOCK
static int
test_poll_event_er (QUIC_XSO *xso)
{
...
QUIC_NEEDS_LOCK
static int
test_poll_event_w (QUIC_XSO *xso)
{
...
QUIC_NEEDS_LOCK
static int
test_poll_event_ew (QUIC_XSO *xso)
{
...
QUIC_NEEDS_LOCK
static int
test_poll_event_ec (QUIC_CONNECTION *qc)
{
  return ossl_quic_channel_is_term_any (qc->ch);
}
...
QUIC_NEEDS_LOCK
static int
test_poll_event_ecd (QUIC_CONNECTION *qc)
{
  return ossl_quic_channel_is_terminated (qc->ch);
}
...
QUIC_NEEDS_LOCK
static int
test_poll_event_is (QUIC_CONNECTION *qc, int is_uni)
{
...
QUIC_NEEDS_LOCK
static int
test_poll_event_os (QUIC_CONNECTION *qc, int is_uni)
{
  /* Is it currently possible for us to make an outgoing stream? */
  return quic_mutation_allowed (qc, /*req_active=*/1)
         && ossl_quic_channel_get_local_stream_count_avail (qc->ch, is_uni)
                > 0;
}
...
QUIC_NEEDS_LOCK
static int
test_poll_event_el (QUIC_LISTENER *ql)
{
...
QUIC_NEEDS_LOCK
static int
test_poll_event_ic (QUIC_LISTENER *ql)
{
...
QUIC_TAKES_LOCK
int
ossl_quic_conn_poll_events (SSL *ssl, uint64_t events, int do_tick,
                            uint64_t *p_revents)
{
  QCTX ctx;
  uint64_t revents = 0;
...
QUIC_TAKES_LOCK
int
ossl_quic_get_notifier_fd (SSL *ssl)
{
  QCTX ctx;
  QUIC_REACTOR *rtor;
  RIO_NOTIFIER *nfy;
...
QUIC_TAKES_LOCK
void
ossl_quic_enter_blocking_section (SSL *ssl, QUIC_REACTOR_WAIT_CTX *wctx)
{
  QCTX ctx;
  QUIC_REACTOR *rtor;
...
QUIC_TAKES_LOCK
void
ossl_quic_leave_blocking_section (SSL *ssl, QUIC_REACTOR_WAIT_CTX *wctx)
{
  QCTX ctx;
  QUIC_REACTOR *rtor;
...
QUIC_CHANNEL *
ossl_quic_conn_get_channel (SSL *s)
{
  QCTX ctx;
...
int
ossl_quic_set_diag_title (SSL_CTX *ctx, const char *title)
{
