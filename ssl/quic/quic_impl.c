...
static void
qc_set_default_xso_keep_ref(QUIC_CONNECTION* qc,
                            QUIC_XSO* xso,
                            int touch,
                            QUIC_XSO** old_xso);
static SSL*
 ...
 */
static int
quic_raise_non_normal_error(QCTX* ctx,
                            const char* file,
                            int line,
                            const char* func,
                            int reason,
...
#define QUIC_RAISE_NON_NORMAL_ERROR(ctx, reason, msg)                          \
  quic_raise_non_normal_error(                                                 \
    (ctx), OPENSSL_FILE, OPENSSL_LINE, OPENSSL_FUNC, (reason), (msg))
/*
...
expect_quic_with_stream_lock(const SSL* s,
                             int remote_init,
                             int in_io,
                             QCTX* ctx)
{
...
ossl_quic_new(SSL_CTX* ctx)
{
  ...
  if (!ossl_quic_obj_init(
        &qc->obj, ctx, SSL_TYPE_QUIC_CONNECTION, NULL, qc->engine, qc->port)) {
    QUIC_RAISE_NON_NORMAL_ERROR(NULL, ERR_R_INTERNAL_ERROR, NULL);
    goto err;
  }
...
int
ossl_quic_set_override_now_cb(SSL* s,
...
ossl_quic_get_rpoll_descriptor(SSL* s, BIO_POLL_DESCRIPTOR* desc)
{
  ...
  if (desc == NULL || net_rbio == NULL)
    return QUIC_RAISE_NON_NORMAL_ERROR(
      &ctx, ERR_R_PASSED_INVALID_ARGUMENT, NULL);
                        ...
                        size_t args_len)
{
  ...
  if (!wait_peer) {
     ...
     */
    if (args != NULL) {
      ossl_quic_channel_set_tcause(
        ctx.qc->ch, args->quic_error_code, args->quic_reason);
    }
  }
  ...
  switch (cmd) {
    ...
    default:
      ...
      return ossl_ctrl_internal(
        &ctx.qc->obj.ssl, cmd, larg, parg, /*no_quic=*/1);
  }
}
...
ensure_channel_started(QCTX* ctx)
{
  ...
  if (!qc->started) {
    if (!configure_channel(qc)) {
      QUIC_RAISE_NON_NORMAL_ERROR(
        ctx, ERR_R_INTERNAL_ERROR, "failed to configure channel");
      return 0;
    }
    ...
    if (!ossl_quic_channel_start(qc->ch)) {
      ...
      QUIC_RAISE_NON_NORMAL_ERROR(
        ctx, ERR_R_INTERNAL_ERROR, "failed to start channel");
      return 0;
    }
    ...
    if (qc->is_thread_assisted)
      if (!ossl_quic_thread_assist_init_start(&qc->thread_assist, qc->ch)) {
        QUIC_RAISE_NON_NORMAL_ERROR(
          ctx, ERR_R_INTERNAL_ERROR, "failed to start assist thread");
        return 0;
      }
#endif
  }
...
QUIC_NEEDS_LOCK
static int
xso_sstream_append(QUIC_XSO* xso,
                   const unsigned char* buf,
                   size_t len,
...
quic_write_again(void* arg)
{
  ...
  quic_post_write(
    args->xso, actual_written > 0, args->len == actual_written, args->flags, 0);
...
QUIC_NEEDS_LOCK
static int
quic_write_nonblocking_aon(QCTX* ctx,
                           const void* buf,
                           size_t len,
                           uint64_t flags,
                           size_t* written)
...
QUIC_NEEDS_LOCK
static int
quic_write_nonblocking_epw(QCTX* ctx,
                           const void* buf,
                           size_t len,
                           uint64_t flags,
                           size_t* written)
{
  ...
  quic_post_write(
    xso, *written > 0, *written == len, flags, qctx_should_autotick(ctx));
                 ...
                 int peek)
{
  ...
  if (peek) {
    if (!ossl_quic_rstream_peek(
          stream->rstream, buf, buf_len, bytes_read, &is_fin))
      return QUIC_RAISE_NON_NORMAL_ERROR(ctx, ERR_R_INTERNAL_ERROR, NULL);

  } else {
    if (!ossl_quic_rstream_read(
          stream->rstream, buf, buf_len, bytes_read, &is_fin))
      return QUIC_RAISE_NON_NORMAL_ERROR(ctx, ERR_R_INTERNAL_ERROR, NULL);
  }
  ...
  if (!peek) {
    if (*bytes_read > 0) {
      ...
      if (!ossl_quic_rxfc_on_retire(
            &stream->rxfc, *bytes_read, rtt_info.smoothed_rtt))
        return QUIC_RAISE_NON_NORMAL_ERROR(ctx, ERR_R_INTERNAL_ERROR, NULL);
    }
...
ossl_quic_set_default_stream_mode(SSL* s, uint32_t mode)
{
  ...
  switch (mode) {
    ...
    default:
      qctx_unlock(&ctx);
      return QUIC_RAISE_NON_NORMAL_ERROR(
        &ctx, ERR_R_PASSED_INVALID_ARGUMENT, "bad default stream type");
  }
...
qc_update_reject_policy(QUIC_CONNECTION* qc)
{
  ...
  ossl_quic_channel_set_incoming_stream_auto_reject(
    qc->ch, enable_reject, qc->incoming_stream_aec);
}
...
QUIC_TAKES_LOCK
static int
qc_getset_idle_timeout(QCTX* ctx,
                       uint32_t class_,
                       ...
                       uint64_t* p_value_in)
{
  ...
  switch (class_) {
    case SSL_VALUE_CLASS_FEATURE_REQUEST:
      ...
      if (p_value_in != NULL) {
        ...
        if (ossl_quic_channel_have_generated_transport_params(ctx->qc->ch)) {
          QUIC_RAISE_NON_NORMAL_ERROR(
            ctx, SSL_R_FEATURE_NOT_RENEGOTIABLE, NULL);
          goto err;
        }
    ...
    case SSL_VALUE_CLASS_FEATURE_NEGOTIATED:
      if (p_value_in != NULL) {
        QUIC_RAISE_NON_NORMAL_ERROR(
          ctx, SSL_R_UNSUPPORTED_CONFIG_VALUE_OP, NULL);
        goto err;
      }
    ...
    default:
      QUIC_RAISE_NON_NORMAL_ERROR(
        ctx, SSL_R_UNSUPPORTED_CONFIG_VALUE_CLASS, NULL);
      goto err;
  }
...
QUIC_TAKES_LOCK
static int
qc_getset_event_handling(QCTX* ctx,
                         uint32_t class_,
                         uint64_t* p_value_out,
                         uint64_t* p_value_in)
{
...
QUIC_TAKES_LOCK
static int
qc_get_stream_write_buf_stat(QCTX* ctx,
                             uint32_t class_,
                             uint64_t* p_value_out,
...
QUIC_TAKES_LOCK
int
ossl_quic_get_value_uint(SSL* s, uint32_t class_, uint32_t id, uint64_t* value)
{
  QCTX ctx;
  ...
  switch (id) {
    ...
    case SSL_VALUE_STREAM_WRITE_BUF_SIZE:
      return qc_get_stream_write_buf_stat(
        &ctx, class_, value, ossl_quic_sstream_get_buffer_size);
    case SSL_VALUE_STREAM_WRITE_BUF_USED:
      return qc_get_stream_write_buf_stat(
        &ctx, class_, value, ossl_quic_sstream_get_buffer_used);
    case SSL_VALUE_STREAM_WRITE_BUF_AVAIL:
      return qc_get_stream_write_buf_stat(
        &ctx, class_, value, ossl_quic_sstream_get_buffer_avail);
 ...
 */
int
ossl_quic_stream_reset(SSL* ssl,
 ...
 */
static void
quic_classify_stream(QUIC_CONNECTION* qc,
                     QUIC_STREAM* qs,
                     int is_write,
                     int* state,
...
quic_get_stream_error_code(SSL* ssl, int is_write, uint64_t* app_error_code)
{
  ...
  quic_classify_stream(
    ctx.qc, ctx.xso->stream, /*is_write=*/0, &state, app_error_code);
 ...
 */
int
ossl_quic_get_conn_close_info(SSL* ssl,
...
ossl_quic_new_listener(SSL_CTX* ctx, uint64_t flags)
{
  ...
  if (!ossl_quic_obj_init(
        &ql->obj, ctx, SSL_TYPE_QUIC_LISTENER, NULL, ql->engine, ql->port))
    goto err;
...
ossl_quic_new_listener_from(SSL* ssl, uint64_t flags)
{
  ...
  /* Initialise the QUIC_LISTENER's object header. */
  if (!ossl_quic_obj_init(&ql->obj,
                          ssl->ctx,
                          SSL_TYPE_QUIC_LISTENER,
                          &ctx.qd->obj.ssl,
                          NULL,
                          ql->port))
    goto err;
...
/* create channel */
  ...
  ossl_quic_channel_set_msg_callback(
    qc->ch, ql->obj.ssl.ctx->msg_callback, &qc->obj.ssl);
...
int
ossl_quic_set_peer_token(SSL_CTX* ctx,
                         BIO_ADDR* peer,
                         const uint8_t* token,
                         size_t token_len)
{
...
ossl_quic_new_domain(SSL_CTX* ctx, uint64_t flags)
{
  ...
  if (!ossl_quic_obj_init(
        &qd->obj, ctx, SSL_TYPE_QUIC_DOMAIN, NULL, qd->engine, NULL))
    goto err;
...
ossl_quic_callback_ctrl(SSL* s, int cmd, void (*fp)(void))
{
  ...
  switch (cmd) {
    case SSL_CTRL_SET_MSG_CALLBACK:
      ossl_quic_channel_set_msg_callback(
        ctx.qc->ch, (ossl_msg_cb)fp, &ctx.qc->obj.ssl);
...
test_poll_event_is(QUIC_CONNECTION* qc, int is_uni)
{
  return ossl_quic_stream_map_get_accept_queue_len(
           ossl_quic_channel_get_qsm(qc->ch), is_uni) > 0;
}
...
QUIC_TAKES_LOCK
int
ossl_quic_conn_poll_events(SSL* ssl,
                           uint64_t events,
                           int do_tick,
                           uint64_t* p_revents)
{
