...
static int expect_quic_as(const SSL* s, QCTX* ctx, uint32_t flags) {
  ...
  if ((flags & QCTX_AUTO_S) != 0) flags |= QCTX_S;
    ...
    case SSL_TYPE_QUIC_CONNECTION:
      ...
      if ((flags & QCTX_AUTO_S) != 0 && qc->default_xso == NULL) {
        ...
        if ((flags & QCTX_REMOTE_INIT) != 0) {
          if (!qc_wait_for_default_xso_for_read(ctx, /*peek=*/0)) goto err;
        } else {
          if (!qc_try_create_default_xso_for_write(ctx)) goto err;
        }
      }
  ...
  ok = 1;
err:
  if (locked && (!ok || !lock_requested)) qctx_unlock(ctx);
...
static int is_quic_c(const SSL* s, QCTX* ctx, int raiseerrs) {
  ...
  if (!raiseerrs) flags |= QCTX_NO_ERROR;
...
static int ossl_unused expect_quic_with_stream_lock(const SSL* s,
  ...
  if (remote_init >= 0) flags |= QCTX_AUTO_S;

  if (remote_init > 0) flags |= QCTX_REMOTE_INIT;

  if (in_io) flags |= QCTX_IO;
 ...
 */
static int quic_mutation_allowed(QUIC_CONNECTION* qc, int req_active) {
  if (qc->shutting_down || ossl_quic_channel_is_term_any(qc->ch)) return 0;
...
SSL* ossl_quic_new(SSL_CTX* ctx) {
  ...
  if (!create_channel(qc, ctx)) goto err;
...
#if defined(OPENSSL_THREADS)
  if (have_lock) /* tsan doesn't like freeing locked mutexes */
...
void ossl_quic_free(SSL* s) {
  ...
  if (!expect_quic_any(s, &ctx)) return;
  ...
  if (ctx.is_stream) {
     ...
     */
    if (!is_default) SSL_free(&ctx.qc->obj.ssl);
  ...
  if (ctx.qc->listener != NULL) SSL_free(&ctx.qc->listener->obj.ssl);
  if (ctx.qc->domain != NULL) SSL_free(&ctx.qc->domain->obj.ssl);
}
...
int ossl_quic_reset(SSL* s) {
  ...
  if (!expect_quic_any(s, &ctx)) return 0;
...
void ossl_quic_conn_force_assist_thread_wake(SSL* s) {
  ...
  if (!expect_quic_conn_only(s, &ctx)) return;
...
static void qc_set_default_xso_keep_ref(QUIC_CONNECTION* qc, QUIC_XSO* xso,
  ...
  *old_xso = NULL;
  ...
  if (qc->default_xso != xso) {
    *old_xso = qc->default_xso; /* transfer old XSO ref to caller */
    ...
    if (xso == NULL) {
       ...
       */
      if (!ossl_assert(SSL_up_ref(&qc->obj.ssl))) return;
    } else {
      /*
  ...
  if (touch) qc_touch_default_xso(qc);
}
...
static void qc_set_default_xso(QUIC_CONNECTION* qc, QUIC_XSO* xso, int touch) {
  ...
  if (old_xso != NULL) SSL_free(&old_xso->obj.ssl);
}
...
static uint64_t quic_mask_or_options(SSL* ssl, uint64_t mask_value,
  ...
  if (ctx.xso != NULL) {
    ...
    if (ctx.is_stream) ret = ctx.xso->ssl_options;
  }
...
static int csm_analyse_init_peer_addr(BIO* net_wbio, BIO_ADDR* peer) {
  if (BIO_dgram_detect_peer_addr(net_wbio, peer) <= 0) return 0;
...
static int quic_set0_net_rbio(QUIC_OBJ* obj, BIO* net_rbio) {
  ...
  if (old_rbio == net_rbio) return 0;

  if (!ossl_quic_port_set_net_rbio(port, net_rbio)) return 0;

  BIO_free_all(old_rbio);
  if (net_rbio != NULL) BIO_set_nbio(net_rbio, 1); /* best effort autoconfig */
...
static int quic_set0_net_wbio(QUIC_OBJ* obj, BIO* net_wbio) {
  ...
  if (old_wbio == net_wbio) return 0;

  if (!ossl_quic_port_set_net_wbio(port, net_wbio)) return 0;

  BIO_free_all(old_wbio);
  if (net_wbio != NULL) BIO_set_nbio(net_wbio, 1); /* best effort autoconfig */
...
void ossl_quic_conn_set0_net_rbio(SSL* s, BIO* net_rbio) {
  ...
  if (!expect_quic_csl(s, &ctx)) return;

  /* Returns 0 if no change. */
  if (!quic_set0_net_rbio(ctx.obj, net_rbio)) return;
}
...
void ossl_quic_conn_set0_net_wbio(SSL* s, BIO* net_wbio) {
  ...
  /* Returns 0 if no change. */
  if (!quic_set0_net_wbio(ctx.obj, net_wbio)) return;
}
...
BIO* ossl_quic_conn_get_net_rbio(const SSL* s) {
  ...
  if (!expect_quic_csl(s, &ctx)) return NULL;
...
int ossl_quic_conn_shutdown(SSL* s, uint64_t flags,
  ...
  if (!expect_quic_cs(s, &ctx)) return -1;
...
long ossl_quic_ctrl(SSL* s, int cmd, long larg, void* parg) {
  ...
  switch (cmd) {
    case SSL_CTRL_MODE:
      ...
      if (!ctx.is_stream) ctx.qc->default_ssl_mode |= (uint32_t)larg;
    ...
    case DTLS_CTRL_GET_TIMEOUT: /* DTLSv1_get_timeout */
    {
      ...
      if (!ossl_quic_get_event_timeout(s, parg, &is_infinite)) return 0;
...
int ossl_quic_set_connect_state(SSL* s, int raiseerrs) {
  ...
  if (!is_quic_c(s, &ctx, raiseerrs)) return 0;

  if (ctx.qc->as_server_state == 0) return 1;
...
int ossl_quic_set_accept_state(SSL* s, int raiseerrs) {
  ...
  if (ctx.qc->as_server_state == 1) return 1;
...
static int quic_handshake_wait(void* arg) {
  ...
  if (!quic_mutation_allowed(args->qc, /*req_active=*/1)) return -1;

  if (ossl_quic_channel_is_handshake_complete(args->qc->ch)) return 1;

  if (tls_wants_non_io_retry(args->qc)) return 1;
...
static int configure_channel(QUIC_CONNECTION* qc) {
  ...
  if (!ossl_quic_channel_set_peer_addr(qc->ch, &qc->init_peer_addr)) return 0;
...
int ossl_quic_connect(SSL* s) {
  ...
  if (!ossl_quic_set_connect_state(s, 1)) return -1;
...
int ossl_quic_accept(SSL* s) {
  ...
  if (!ossl_quic_set_accept_state(s, 1)) return -1;
...
static int quic_wait_for_stream(void* arg) {
  ...
  if (args->qs != NULL) return 1; /* stream now exists */
...
static int quic_new_stream_wait(void* arg) {
  ...
  if (!quic_mutation_allowed(qc, /*req_active=*/1)) return -1;
...
static SSL* quic_conn_stream_new(QCTX* ctx, uint64_t flags, int need_lock) {
  ...
  if (xso == NULL) goto err;

  qc_touch_default_xso(qc); /* inhibits default XSO */
  if (need_lock) qctx_unlock(ctx);
...
SSL* ossl_quic_conn_stream_new(SSL* s, uint64_t flags) {
  ...
  if (!expect_quic_conn_only(s, &ctx)) return NULL;
...
int ossl_quic_get_error(const SSL* s, int i) {
  ...
  if (!is_quic_cs(s, &ctx, 0 /* suppress errors */)) return SSL_ERROR_SSL;
  ...
  if (net_error) return SSL_ERROR_SYSCALL;
...
int ossl_quic_want(const SSL* s) {
  ...
  if (!expect_quic_cs(s, &ctx)) return SSL_NOTHING;
...
static int sstream_ensure_spare(QUIC_SSTREAM* sstream, uint64_t spare) {
  ...
  if (spare_ <= avail || cur_sz == MAX_WRITE_BUF_SIZE) return 1;
...
static int xso_sstream_append(QUIC_XSO* xso, const unsigned char* buf,
  ...
  if (len > permitted) len = (size_t)permitted;

  if (!sstream_ensure_spare(sstream, len)) return 0;
...
static int quic_write_again(void* arg) {
  ...
  if (args->len == 0) /* Written everything, done. */
    return 1;
...
static int quic_write_nonblocking_aon(QCTX* ctx, const void* buf, size_t len,
   ...
   */
  if (actual_written > 0) aon_write_begin(xso, buf, len, actual_written);
...
static int quic_write_nonblocking_epw(QCTX* ctx, const void* buf, size_t len,
  ...
  if (*written == 0) /* SSL_write_ex returns 0 if it didn't write anything. */
    return QUIC_RAISE_NORMAL_ERROR(ctx, SSL_ERROR_WANT_WRITE);
...
static size_t ossl_quic_pending_int(const SSL* s, int check_channel) {
  ...
  if (check_channel) /* We care about boolean result here only */
...
SSL* ossl_quic_get0_connection(SSL* s) {
  ...
  if (!expect_quic_cs(s, &ctx)) return NULL;
...
SSL* ossl_quic_get0_domain(SSL* s) {
  ...
  if (!expect_quic_any(s, &ctx)) return NULL;
...
int ossl_quic_get_domain_flags(const SSL* ssl, uint64_t* domain_flags) {
  ...
  if (!expect_quic_any(ssl, &ctx)) return 0;

  if (domain_flags != NULL) *domain_flags = ctx.obj->domain_flags;
...
int ossl_quic_get_stream_type(SSL* s) {
  ...
  if (!expect_quic_cs(s, &ctx)) return SSL_STREAM_TYPE_BIDI;
  ...
  if (ossl_quic_stream_is_bidi(ctx.xso->stream)) return SSL_STREAM_TYPE_BIDI;
...
int ossl_quic_set_default_stream_mode(SSL* s, uint32_t mode) {
  ...
  if (!expect_quic_conn_only(s, &ctx)) return 0;
...
int ossl_quic_attach_stream(SSL* conn, SSL* stream) {
  ...
  if (!expect_quic_conn_only(conn, &ctx)) return 0;
...
err:
  qctx_unlock(ctx);
  if (ret && p_value_out != NULL) *p_value_out = value_out;
...
QUIC_NEEDS_LOCK
static void qctx_maybe_autotick(QCTX* ctx) {
  if (!qctx_should_autotick(ctx)) return;
...
QUIC_TAKES_LOCK
static int qc_get_stream_write_buf_stat(
    QCTX* ctx, uint32_t class_, uint64_t* p_value_out,
...
int ossl_quic_get_value_uint(SSL* s, uint32_t class_, uint32_t id,
  ...
  if (!expect_quic_for_value(s, &ctx, id)) return 0;
...
static int wait_for_incoming_stream(void* arg) {
  ...
  if (args->qs != NULL) return 1; /* got a stream */
...
int ossl_quic_stream_reset(SSL* ssl, const SSL_STREAM_RESET_ARGS* args,
  ...
  if (ok) ctx.xso->requested_reset = 1;
...
int ossl_quic_get_conn_close_info(SSL* ssl, SSL_CONN_CLOSE_INFO* info,
  ...
  if (!expect_quic_conn_only(ssl, &ctx)) return -1;
  ...
  info->flags = 0;
  if (!tc->remote) info->flags |= SSL_CONN_CLOSE_FLAG_LOCAL;
  if (!tc->app) info->flags |= SSL_CONN_CLOSE_FLAG_TRANSPORT;
  return 1;
}
...
int ossl_quic_key_update(SSL* ssl, int update_type) {
  ...
  if (!expect_quic_conn_only(ssl, &ctx)) return 0;
...
err:
  if (ql != NULL) ossl_quic_engine_free(ql->engine);
...
SSL* ossl_quic_new_listener_from(SSL* ssl, uint64_t flags) {
  ...
  if (!expect_quic_domain(ssl, &ctx)) return NULL;

  if (!SSL_up_ref(&ctx.qd->obj.ssl)) return NULL;
...
err:
  if (ql != NULL) ossl_quic_port_free(ql->port);
...
SSL* ossl_quic_new_from_listener(SSL* ssl, uint64_t flags) {
  ...
  if (!expect_quic_listener(ssl, &ctx)) return NULL;

  if (!SSL_up_ref(&ctx.ql->obj.ssl)) return NULL;
  ...
  if (ssl->ctx->tokencache == NULL)
    if ((ssl->ctx->tokencache = ossl_quic_new_token_store()) == NULL) goto err;
...
static int quic_accept_connection_wait(void* arg) {
  ...
  if (!ossl_quic_port_is_running(port)) return -1;

  if (ossl_quic_port_have_incoming(port)) return 1;
...
SSL* ossl_quic_accept_connection(SSL* ssl, uint64_t flags) {
  ...
  if (new_ch == NULL && ossl_quic_port_is_running(ctx.ql->port)) {
    ...
    if (!ossl_quic_port_is_running(ctx.ql->port)) goto out;
...
static int quic_token_cmp(const QUIC_TOKEN* a, const QUIC_TOKEN* b) {
  if (a->hashkey_len != b->hashkey_len) return 1;
...
SSL_TOKEN_STORE* ossl_quic_new_token_store(void) {
  ...
  if (newcache == NULL) goto out;

  newcache->cache = lh_QUIC_TOKEN_new(quic_token_hash, quic_token_cmp);
  if (newcache->cache == NULL) goto out;

#if defined(OPENSSL_THREADS)
  if ((newcache->mutex = ossl_crypto_mutex_new()) == NULL) goto out;
#endif

  if (!CRYPTO_NEW_REF(&newcache->references, 1)) goto out;
...
void ossl_quic_free_token_store(SSL_TOKEN_STORE* hdl) {
  ...
  if (!CRYPTO_DOWN_REF(&hdl->references, &refs)) return;
...
static QUIC_TOKEN* ossl_quic_build_new_token(BIO_ADDR* peer, uint8_t* token,
  ...
  if (!BIO_ADDR_rawaddress(peer, NULL, &addr_len)) return NULL;
  ...
  *portptr = port;
  ...
  if (token != NULL) memcpy(new_token->token, token, token_len);
  return new_token;
}
...
int ossl_quic_set_peer_token(SSL_CTX* ctx, BIO_ADDR* peer, const uint8_t* token,
  ...
  if (ctx->tokencache == NULL) return 0;
...
void ossl_quic_free_peer_token(QUIC_TOKEN* token) {
  ...
  if (!CRYPTO_DOWN_REF(&token->references, &refs)) return;
...
SSL* ossl_quic_new_domain(SSL_CTX* ctx, uint64_t flags) {
  ...
  if (!ossl_adjust_domain_flags(domain_flags, &domain_flags)) return NULL;
...
const SSL_CIPHER* ossl_quic_get_cipher_by_char(const unsigned char* p) {
  ...
  if ((ciph->algorithm2 & SSL_QUIC) == 0) return NULL;
...
static int test_poll_event_r(QUIC_XSO* xso) {
   ...
   */
  if (xso->stream->recv_state == QUIC_RSTREAM_STATE_DATA_READ) return 1;
...
int ossl_quic_conn_poll_events(SSL* ssl, uint64_t events, int do_tick,
  ...
  if (!expect_quic_csl(ssl, &ctx)) return 0;

  qctx_lock(&ctx);

  if (ctx.qc != NULL && !ctx.qc->started) {
    /* We can only try to write on non-started connection. */
    if ((events & SSL_POLL_EVENT_W) != 0) revents |= SSL_POLL_EVENT_W;
    goto end;
  }

  if (do_tick) ossl_quic_reactor_tick(ossl_quic_obj_get0_reactor(ctx.obj), 0);
...
void ossl_quic_enter_blocking_section(SSL* ssl, QUIC_REACTOR_WAIT_CTX* wctx) {
  ...
  if (!expect_quic_any(ssl, &ctx)) return;
...
int ossl_quic_set_diag_title(SSL_CTX* ctx, const char* title) {
  ...
  if ((ctx->qlog_title = OPENSSL_strdup(title)) == NULL) return 0;
#endif
