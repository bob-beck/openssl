...
static int
wbuf_init (struct json_write_buf *wbuf, BIO *bio, size_t alloc)
{
...
static void
wbuf_cleanup (struct json_write_buf *wbuf)
{
...
static void
wbuf_set0_bio (struct json_write_buf *wbuf, BIO *bio)
{
  wbuf->bio = bio;
}
...
wbuf_clean (struct json_write_buf *wbuf)
{
  wbuf->cur = 0;
}
...
wbuf_avail (struct json_write_buf *wbuf)
{
...
wbuf_write_char (struct json_write_buf *wbuf, char c)
{
 ...
 */
static int
wbuf_write_str (struct json_write_buf *wbuf, const char *s)
{
  char c;
...
static int
wbuf_flush (struct json_write_buf *wbuf, int full)
{
...
static int
json_ensure_stack_size (OSSL_JSON_ENC *json, size_t num_bytes)
{
  unsigned char *stack;
...
static int
json_push (OSSL_JSON_ENC *json, unsigned int v)
{
  if (v > 1)
    return 0;
...
/*
 * Pop a bit from the stack. Returns 0 if stack is empty. Use json_peek() to
 * get the value before calling this.
 */
static int
json_pop (OSSL_JSON_ENC *json)
{
...
static ossl_inline int
in_ijson (const OSSL_JSON_ENC *json)
{
  return (json->flags & OSSL_JSON_FLAG_IJSON) != 0;
}

static ossl_inline int
in_seq (const OSSL_JSON_ENC *json)
{
  return (json->flags & OSSL_JSON_FLAG_SEQ) != 0;
}

static ossl_inline int
in_pretty (const OSSL_JSON_ENC *json)
{
  return (json->flags & OSSL_JSON_FLAG_PRETTY) != 0;
}

int
ossl_json_init (OSSL_JSON_ENC *json, BIO *bio, uint32_t flags)
{
...
void
ossl_json_cleanup (OSSL_JSON_ENC *json)
{
...
int
ossl_json_flush_cleanup (OSSL_JSON_ENC *json)
{
...
int
ossl_json_reset (OSSL_JSON_ENC *json)
{
...
int
ossl_json_flush (OSSL_JSON_ENC *json)
{
  return wbuf_flush (&json->wbuf, /*full=*/1);
}

int
ossl_json_set0_sink (OSSL_JSON_ENC *json, BIO *bio)
{
...
int
ossl_json_in_error (OSSL_JSON_ENC *json)
{
  return json->error;
}
...
static void
json_raise_error (OSSL_JSON_ENC *json)
{
  json->error = 1;
}

static void
json_undefer (OSSL_JSON_ENC *json)
{
...
static void
json_write_char (OSSL_JSON_ENC *json, char ch)
{
...
static void
json_write_str (OSSL_JSON_ENC *json, const char *s)
{
  if (ossl_json_in_error (json))
    return;
...
static void
json_indent (OSSL_JSON_ENC *json)
{
  size_t i, depth;
...
static int
json_pre_item (OSSL_JSON_ENC *json)
{
  int s;
...
static void
json_post_item (OSSL_JSON_ENC *json)
{
 ...
 */
static void
composite_begin (OSSL_JSON_ENC *json, int type, char ch)
{
 ...
 */
static void
composite_end (OSSL_JSON_ENC *json, int type, char ch)
{
...
void
ossl_json_object_begin (OSSL_JSON_ENC *json)
{
 ...
 */
void
ossl_json_object_end (OSSL_JSON_ENC *json)
{
...
void
ossl_json_array_begin (OSSL_JSON_ENC *json)
{
...
void
ossl_json_array_end (OSSL_JSON_ENC *json)
{
 ...
 */
void
ossl_json_key (OSSL_JSON_ENC *json, const char *key)
{
  if (ossl_json_in_error (json))
    return;
...
void
ossl_json_null (OSSL_JSON_ENC *json)
{
...
void
ossl_json_bool (OSSL_JSON_ENC *json, bool v)
{
  if (!json_pre_item (json))
    return;
...
static void
json_u64 (OSSL_JSON_ENC *json, uint64_t v, int noquote)
{
...
void
ossl_json_u64 (OSSL_JSON_ENC *json, uint64_t v)
{
  json_u64 (json, v, 0);
}
...
void
ossl_json_i64 (OSSL_JSON_ENC *json, int64_t value)
{
  uint64_t uv;
  int quote;
...
void
ossl_json_str (OSSL_JSON_ENC *json, const char *str)
{
  if (!json_pre_item (json))
    return;
...
void
ossl_json_str_len (OSSL_JSON_ENC *json, const char *str, size_t str_len)
{
  if (!json_pre_item (json))
    return;
 ...
 */
void
ossl_json_str_hex (OSSL_JSON_ENC *json, const void *data, size_t data_len)
{
