...
static int depack_do_frame_crypto(PACKET* pkt, QUIC_CHANNEL* ch,
  ...
  *datalen = 0;
  ...
  if (f.len == 0) return 1; /* nothing to do */
...
static int depack_do_frame_stream(PACKET* pkt, QUIC_CHANNEL* ch,
  ...
  *datalen = 0;
  ...
  if (rs_fin) ossl_quic_stream_map_notify_totally_received(&ch->qsm, stream);
...
static void update_streams_bidi(QUIC_STREAM* s, void* arg) {
  ...
  if (!ossl_quic_stream_is_bidi(s)) return;
...
static int depack_do_frame_path_challenge(PACKET* pkt, QUIC_CHANNEL* ch,
  ...
  if ((encoded = OPENSSL_malloc(encoded_len)) == NULL) goto err;

  if (!WPACKET_init_static_len(&wpkt, encoded, encoded_len, 0)) goto err;
...
static int depack_process_frames(QUIC_CHANNEL* ch, PACKET* pkt,
  ...
  while (PACKET_remaining(pkt) > 0) {
    ...
    if (ch->msg_callback != NULL) sof = PACKET_data(pkt);
    ...
    switch (frame_type) {
      case OSSL_QUIC_FRAME_TYPE_PING:
        ...
        if (!depack_do_frame_ping(pkt, ch, enc_level, ackm_data)) return 0;
        break;
      case OSSL_QUIC_FRAME_TYPE_PADDING:
        /* Allowed in all packet types */
        if (!depack_do_frame_padding(pkt)) return 0;
        break;
      ...
      case OSSL_QUIC_FRAME_TYPE_NEW_TOKEN:
        ...
        if (!depack_do_frame_new_token(pkt, ch, ackm_data)) return 0;
        break;
      ...
      case OSSL_QUIC_FRAME_TYPE_DATA_BLOCKED:
        ...
        if (!depack_do_frame_data_blocked(pkt, ch, ackm_data)) return 0;
        break;
      case OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED:
        ...
        if (!depack_do_frame_stream_data_blocked(pkt, ch, ackm_data)) return 0;
        break;
      ...
      case OSSL_QUIC_FRAME_TYPE_PATH_CHALLENGE:
        ...
        if (!depack_do_frame_path_challenge(pkt, ch, ackm_data)) return 0;
      ...
      case OSSL_QUIC_FRAME_TYPE_CONN_CLOSE_TRANSPORT:
        ...
        if (!depack_do_frame_conn_close(pkt, ch, frame_type)) return 0;
        break;
      ...
      case OSSL_QUIC_FRAME_TYPE_HANDSHAKE_DONE:
        ...
        if (!depack_do_frame_handshake_done(pkt, ch, ackm_data)) return 0;
        break;
