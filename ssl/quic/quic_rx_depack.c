 ...
 */
static int
depack_do_implicit_stream_create(QUIC_CHANNEL* ch,
                                 uint64_t stream_id,
                                 uint64_t frame_type,
                                 QUIC_STREAM** result);
...
static int
depack_do_frame_ping(PACKET* pkt,
                     QUIC_CHANNEL* ch,
                     uint32_t enc_level,
...
static int
depack_do_frame_ack(PACKET* pkt,
                    QUIC_CHANNEL* ch,
                    int packet_space,
                    OSSL_TIME received,
                    uint64_t frame_type,
...
static int
depack_do_frame_reset_stream(PACKET* pkt,
                             QUIC_CHANNEL* ch,
                             OSSL_ACKM_RX_PKT* ackm_data)
{
  ...
  if (!ossl_quic_rxfc_on_rx_stream_frame(
        &stream->rxfc, frame_data.final_size, /*is_fin=*/1)) {
    ossl_quic_channel_raise_protocol_error(ch,
...
static int
depack_do_frame_stop_sending(PACKET* pkt,
                             QUIC_CHANNEL* ch,
                             OSSL_ACKM_RX_PKT* ackm_data)
{
  ...
  ossl_quic_stream_map_reset_stream_send_part(
    &ch->qsm, stream, frame_data.app_error_code);
  return 1;
}

static int
depack_do_frame_crypto(PACKET* pkt,
                       QUIC_CHANNEL* ch,
                       ...
                       uint64_t* datalen)
{
  ...
  *datalen = 0;
  ...
  if (!ossl_quic_rstream_queue_data(
        rstream, parent_pkt, f.offset, f.data, f.len, 0)) {
    ossl_quic_channel_raise_protocol_error(ch,
                                           OSSL_QUIC_ERR_INTERNAL_ERROR,
                                           OSSL_QUIC_FRAME_TYPE_CRYPTO,
...
static int
depack_do_frame_new_token(PACKET* pkt,
                          QUIC_CHANNEL* ch,
                          OSSL_ACKM_RX_PKT* ackm_data)
{
  const uint8_t* token;
...
static int
depack_do_frame_stream(PACKET* pkt,
                       QUIC_CHANNEL* ch,
                       OSSL_QRX_PKT* parent_pkt,
                       OSSL_ACKM_RX_PKT* ackm_data,
                       uint64_t frame_type,
                       uint64_t* datalen)
{
  ...
  *datalen = 0;
  ...
  if (!depack_do_implicit_stream_create(
        ch, frame_data.stream_id, frame_type, &stream))
  ...
  if (fce != OSSL_QUIC_ERR_NO_ERROR) {
    ossl_quic_channel_raise_protocol_error(
      ch, fce, frame_type, "flow control violation");
    return 0;
  }
...
static int
depack_do_frame_max_data(PACKET* pkt,
                         QUIC_CHANNEL* ch,
                         OSSL_ACKM_RX_PKT* ackm_data)
{
  uint64_t max_data = 0;
...
static int
depack_do_frame_max_stream_data(PACKET* pkt,
                                QUIC_CHANNEL* ch,
                                OSSL_ACKM_RX_PKT* ackm_data)
{
...
static int
depack_do_frame_max_streams(PACKET* pkt,
                            QUIC_CHANNEL* ch,
                            OSSL_ACKM_RX_PKT* ackm_data,
                            uint64_t frame_type)
{
...
static int
depack_do_frame_data_blocked(PACKET* pkt,
                             QUIC_CHANNEL* ch,
                             OSSL_ACKM_RX_PKT* ackm_data)
{
  uint64_t max_data = 0;
...
static int
depack_do_frame_stream_data_blocked(PACKET* pkt,
                                    QUIC_CHANNEL* ch,
                                    OSSL_ACKM_RX_PKT* ackm_data)
{
  uint64_t stream_id = 0;
...
static int
depack_do_frame_streams_blocked(PACKET* pkt,
                                QUIC_CHANNEL* ch,
                                OSSL_ACKM_RX_PKT* ackm_data,
                                uint64_t frame_type)
{
...
static int
depack_do_frame_new_conn_id(PACKET* pkt,
                            QUIC_CHANNEL* ch,
                            OSSL_ACKM_RX_PKT* ackm_data)
{
...
static int
depack_do_frame_retire_conn_id(PACKET* pkt,
                               QUIC_CHANNEL* ch,
                               OSSL_ACKM_RX_PKT* ackm_data)
{
  uint64_t seq_num;
...
static int
depack_do_frame_path_challenge(PACKET* pkt,
                               QUIC_CHANNEL* ch,
                               OSSL_ACKM_RX_PKT* ackm_data)
{
...
static int
depack_do_frame_path_response(PACKET* pkt,
                              QUIC_CHANNEL* ch,
                              OSSL_ACKM_RX_PKT* ackm_data)
{
  uint64_t frame_data = 0;
...
static int
depack_do_frame_handshake_done(PACKET* pkt,
                               QUIC_CHANNEL* ch,
                               OSSL_ACKM_RX_PKT* ackm_data)
{
...
static int
depack_process_frames(QUIC_CHANNEL* ch,
                      PACKET* pkt,
                      OSSL_QRX_PKT* parent_pkt,
                      uint32_t enc_level,
                      OSSL_TIME received,
  ...
  while (PACKET_remaining(pkt) > 0) {
    ...
    switch (frame_type) {
      ...
      case OSSL_QUIC_FRAME_TYPE_ACK_WITH_ECN:
        ...
        if (!depack_do_frame_ack(
              pkt, ch, packet_space, received, frame_type, parent_pkt))
          return 0;
        break;
      ...
      case OSSL_QUIC_FRAME_TYPE_STREAM_OFF_LEN_FIN:
        ...
        if (!depack_do_frame_stream(
              pkt, ch, parent_pkt, ackm_data, frame_type, &datalen))
          return 0;
        break;
...
ossl_quic_handle_frames(QUIC_CHANNEL* ch, OSSL_QRX_PKT* qpacket)
{
  ...
  if (!PACKET_buf_init(&pkt, qpacket->hdr->data, qpacket->hdr->len) ||
      !depack_process_frames(
        ch, &pkt, qpacket, enc_level, qpacket->time, &ackm_data))
    return 0;
