...
static STREAM_FRAME* stream_frame_new(UINT_RANGE* range, OSSL_QRX_PKT* pkt,
  ...
  if (pkt != NULL) ossl_qrx_pkt_up_ref(pkt);
...
static int append_frame(SFRAME_LIST* fl, UINT_RANGE* range, OSSL_QRX_PKT* pkt,
  ...
  if ((new_frame = stream_frame_new(range, pkt, data)) == NULL) return 0;
  new_frame->prev = fl->tail;
  if (fl->tail != NULL) fl->tail->next = new_frame;
  fl->tail = new_frame;
  ++fl->num_frames;
  return 1;
}
...
int ossl_sframe_list_insert(SFRAME_LIST* fl, UINT_RANGE* range,
  ...
  if (fl->offset >= range->end) goto end;

  /* nothing there yet */
  if (fl->tail == NULL) {
    fl->tail = fl->head = stream_frame_new(range, pkt, data);
    if (fl->tail == NULL) return 0;
  ...
  if (fl->tail->range.start < range->start) {
    if (fl->tail->range.end >= range->end) goto end;

    if (!append_frame(fl, range, pkt, data)) return 0;
    goto end;
  }
  ...
  if (!ossl_assert(sf != NULL)) /* frame list invariant broken */
    return 0;

  if (prev_frame != NULL && prev_frame->range.end >= range->end) goto end;
  ...
  for (next_frame = sf;
    ...
    if (next_frame != NULL) next_frame->prev = drop_frame->prev;
    if (prev_frame != NULL) prev_frame->next = drop_frame->next;
    if (fl->head == drop_frame) fl->head = next_frame;
    if (fl->tail == drop_frame) fl->tail = prev_frame;
    --fl->num_frames;
...
int ossl_sframe_list_lock_head(SFRAME_LIST* fl, UINT_RANGE* range,
  ...
  if (fl->head_locked) return 0;

  ret = ossl_sframe_list_peek(fl, &iter, range, data, fin);
  if (ret) fl->head_locked = 1;
  return ret;
}
...
int ossl_sframe_list_move_data(SFRAME_LIST* fl,
  ...
  if (fl->head_locked) sf = sf->next;

  for (; sf != NULL; sf = sf->next) {
    size_t len;
    const unsigned char* data = sf->data;

    if (limit < sf->range.start) limit = sf->range.start;

    if (data != NULL) {
      if (limit > sf->range.start) data += (size_t)(limit - sf->range.start);
      len = (size_t)(sf->range.end - limit);

      if (!write_at_cb(limit, data, len, cb_arg)) /* data did not fit */
        return 0;
