...
int
ossl_quic_fifd_init(QUIC_FIFD* fifd,
                    QUIC_CFQ* cfq,
                    OSSL_ACKM* ackm,
                    QUIC_TXPIM* txpim,
...
void
ossl_quic_fifd_cleanup(QUIC_FIFD* fifd)
{
  /* No-op. */
}
...
on_lost(void* arg)
{
  ...
  if (pkt->had_max_data_frame)
    fifd->regen_frame(
      OSSL_QUIC_FRAME_TYPE_MAX_DATA, UINT64_MAX, pkt, fifd->regen_frame_arg);
...
int
ossl_quic_fifd_pkt_commit(QUIC_FIFD* fifd, QUIC_TXPIM_PKT* pkt)
{
  ...
  for (i = 0; i < num_chunks; ++i) {
    ...
    if (chunks[i].end >= chunks[i].start &&
        !ossl_quic_sstream_mark_transmitted(
          sstream, chunks[i].start, chunks[i].end))
      return 0;
...
void
ossl_quic_fifd_set_qlog_cb(QUIC_FIFD* fifd,
                           QLOG* (*get_qlog_cb)(void* arg),
                           void* get_qlog_cb_arg)
{
  fifd->get_qlog_cb = get_qlog_cb;
