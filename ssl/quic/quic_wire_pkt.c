...
int
ossl_quic_hdr_protector_init(QUIC_HDR_PROTECTOR* hpr,
                             OSSL_LIB_CTX* libctx,
                             const char* propq,
                             uint32_t cipher_id,
                             ...
                             size_t quic_hp_key_len)
{
  ...
  if (!EVP_CipherInit_ex(
        hpr->cipher_ctx, hpr->cipher, NULL, quic_hp_key, NULL, 1)) {
    ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
    goto err;
  }
...
void
ossl_quic_hdr_protector_cleanup(QUIC_HDR_PROTECTOR* hpr)
{
...
static int
hdr_generate_mask(QUIC_HDR_PROTECTOR* hpr,
...
int
ossl_quic_hdr_protector_decrypt(QUIC_HDR_PROTECTOR* hpr,
...
int
ossl_quic_hdr_protector_decrypt_fields(QUIC_HDR_PROTECTOR* hpr,
                                       const unsigned char* sample,
                                       size_t sample_len,
...
int
ossl_quic_hdr_protector_encrypt(QUIC_HDR_PROTECTOR* hpr,
                                QUIC_PKT_HDR_PTRS* ptrs)
{
...
int
ossl_quic_hdr_protector_encrypt_fields(QUIC_HDR_PROTECTOR* hpr,
                                       const unsigned char* sample,
                                       size_t sample_len,
                                       unsigned char* first_byte,
                                       unsigned char* pn_bytes)
...
int
ossl_quic_wire_decode_pkt_hdr(PACKET* pkt,
...
int
ossl_quic_wire_encode_pkt_hdr(WPACKET* pkt,
                              size_t short_conn_id_len,
...
size_t
ossl_quic_wire_get_encoded_pkt_hdr_len(size_t short_conn_id_len,
                                       const QUIC_PKT_HDR* hdr)
{
  size_t len = 0, enclen;
...
int
ossl_quic_wire_get_pkt_hdr_dst_conn_id(const unsigned char* buf,
                                       size_t buf_len,
                                       size_t short_conn_id_len,
...
int
ossl_quic_wire_decode_pkt_hdr_pn(const unsigned char* enc_pn,
                                 size_t enc_pn_len,
                                 QUIC_PN largest_pn,
                                 QUIC_PN* res_pn)
{
...
int
ossl_quic_wire_determine_pn_len(QUIC_PN pn, QUIC_PN largest_acked)
{
...
int
ossl_quic_wire_encode_pkt_hdr_pn(QUIC_PN pn,
...
int
ossl_quic_validate_retry_integrity_tag(OSSL_LIB_CTX* libctx,
                                       const char* propq,
                                       const QUIC_PKT_HDR* hdr,
...
int
ossl_quic_calculate_retry_integrity_tag(OSSL_LIB_CTX* libctx,
                                        const char* propq,
                                        const QUIC_PKT_HDR* hdr,
                                        const QUIC_CONN_ID* client_initial_dcid,
                                        unsigned char* tag)
{
  ...
  if (!WPACKET_put_bytes_u8(&wpkt, client_initial_dcid->id_len) ||
      !WPACKET_memcpy(
        &wpkt, client_initial_dcid->id, client_initial_dcid->id_len)) {
    ERR_raise(ERR_LIB_SSL, ERR_R_CRYPTO_LIB);
    goto err;
  }
  ...
  if (!ossl_quic_wire_encode_pkt_hdr(
        &wpkt, hdr2.dst_conn_id.id_len, &hdr2, NULL))
    goto err;
  ...
  if (EVP_CIPHER_CTX_ctrl(
        cctx, EVP_CTRL_AEAD_GET_TAG, QUIC_RETRY_INTEGRITY_TAG_LEN, tag) != 1) {
    ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
    goto err;
  }
