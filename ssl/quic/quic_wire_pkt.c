...
static int
hdr_generate_mask(QUIC_HDR_PROTECTOR *hpr, const unsigned char *sample, size_t sample_len, unsigned char *mask)
{
    int                        l = 0;
...
size_t ossl_quic_wire_get_encoded_pkt_hdr_len(size_t short_conn_id_len, const QUIC_PKT_HDR *hdr)
{
    size_t len = 0, enclen;
                                            ...
                                            unsigned char      *tag)
{
    ...
    if (EVP_CipherUpdate(cctx, NULL, &l, hdr->data, (int)(hdr->len - QUIC_RETRY_INTEGRITY_TAG_LEN)) != 1) {
        ERR_raise(ERR_LIB_SSL, ERR_R_EVP_LIB);
        goto err;
    }
