...
int ossl_quic_hdr_protector_decrypt_fields(QUIC_HDR_PROTECTOR* hpr,
  ...
  if (!hdr_generate_mask(hpr, sample, sample_len, mask)) return 0;
...
int ossl_quic_wire_decode_pkt_hdr(PACKET* pkt, size_t short_conn_id_len,
  ...
  if (fail_cause != NULL) *fail_cause = QUIC_PKT_HDR_DECODE_DECODE_ERR;
  ...
  if (l < QUIC_MIN_VALID_PKT_LEN || !PACKET_get_1(pkt, &b0)) return 0;
  ...
  if ((b0 & 0x80) == 0) {
    /* Short header. */
    if (short_conn_id_len > QUIC_MAX_CONN_ID_LEN) return 0;
    ...
    if (partial) {
      if (!PACKET_forward(pkt, sizeof(hdr->pn))) return 0;
    } else {
      if (!PACKET_copy_bytes(pkt, hdr->pn, hdr->pn_len)) return 0;
    }
    ...
    if (version == 0) {
      ...
      if (fail_cause != NULL) *fail_cause |= QUIC_PKT_HDR_DECODE_BAD_VERSION;
      /* Unknown version, do not decode. */
      return 0;
    } else {
      if (l < QUIC_MIN_VALID_PKT_LEN_CRYPTO) return 0;
      ...
      if (hdr->type == QUIC_PKT_TYPE_RETRY) {
        ...
        if (!nodata && len > PACKET_remaining(pkt)) return 0;
...
int ossl_quic_wire_encode_pkt_hdr(WPACKET* pkt, size_t short_conn_id_len,
  ...
  if (!WPACKET_get_total_written(pkt, &off_start)) return 0;

  if (ptrs != NULL) {
    /* ptrs would not be stable on non-static WPACKET */
    if (!ossl_assert(pkt->staticbuf != NULL)) return 0;
    ptrs->raw_start = NULL;
    ptrs->raw_sample = NULL;
    ptrs->raw_sample_len = 0;
    ptrs->raw_pn = 0;
  ...
  if (hdr->type == QUIC_PKT_TYPE_1RTT) {
    ...
    if (hdr->type == QUIC_PKT_TYPE_RETRY) b0 |= hdr->unused;
    ...
    if (hdr->type == QUIC_PKT_TYPE_VERSION_NEG) {
      if (hdr->len > 0 && !WPACKET_reserve_bytes(pkt, hdr->len, NULL)) return 0;
    ...
    if (hdr->type == QUIC_PKT_TYPE_RETRY) {
      if (!WPACKET_memcpy(pkt, hdr->token, hdr->token_len)) return 0;
      return 1;
    }
...
int ossl_quic_wire_get_pkt_hdr_dst_conn_id(const unsigned char* buf,
  ...
  if ((b0 & 0x80) != 0) {
     ...
     */
    if ((buf[1] || buf[2] || buf[3] || buf[4]) && (b0 & 0x40) == 0) return 0;
...
int ossl_quic_validate_retry_integrity_tag(
  ...
  if (hdr == NULL || hdr->len < QUIC_RETRY_INTEGRITY_TAG_LEN) return 0;
...
err:
  ...
  if (wpkt_valid) WPACKET_finish(&wpkt);
