...
static void
ch_save_err_state(QUIC_CHANNEL* ch);
static int
ch_rx(QUIC_CHANNEL* ch, int channel_only, int* notify_other_threads);
static int
ch_tx(QUIC_CHANNEL* ch, int* notify_other_threads);
static int
ch_tick_tls(QUIC_CHANNEL* ch, int channel_only, int* notify_other_threads);
static void
ch_rx_handle_packet(QUIC_CHANNEL* ch, int channel_only);
static OSSL_TIME
ch_determine_next_tick_deadline(QUIC_CHANNEL* ch);
static int
ch_retry(QUIC_CHANNEL* ch,
         const unsigned char* retry_token,
         size_t retry_token_len,
         const QUIC_CONN_ID* retry_scid,
         int drop_later_pn);
static int
ch_restart(QUIC_CHANNEL* ch);

static void
ch_cleanup(QUIC_CHANNEL* ch);
static int
ch_generate_transport_params(QUIC_CHANNEL* ch);
static int
ch_on_transport_params(const unsigned char* params,
                       size_t params_len,
                       void* arg);
static int
ch_on_handshake_alert(void* arg, unsigned char alert_code);
static int
ch_on_handshake_complete(void* arg);
static int
ch_on_handshake_yield_secret(uint32_t prot_level,
                             int direction,
                             uint32_t suite_id,
                             EVP_MD* md,
                             ...
                             void* arg);
static int
ch_on_crypto_recv_record(const unsigned char** buf,
                         size_t* bytes_read,
                         void* arg);
static int
ch_on_crypto_release_record(size_t bytes_read, void* arg);
static int
crypto_ensure_empty(QUIC_RSTREAM* rstream);
static int
ch_on_crypto_send(const unsigned char* buf,
                  size_t buf_len,
                  size_t* consumed,
                  void* arg);
static OSSL_TIME
get_time(void* arg);
static uint64_t
get_stream_limit(int uni, void* arg);
static int
rx_late_validate(QUIC_PN pn, int pn_space, void* arg);
static void
rxku_detected(QUIC_PN pn, void* arg);
static int
ch_retry(QUIC_CHANNEL* ch,
         const unsigned char* retry_token,
         size_t retry_token_len,
         ...
         int drop_later_pn);
static void
ch_update_idle(QUIC_CHANNEL* ch);
static int
ch_discard_el(QUIC_CHANNEL* ch, uint32_t enc_level);
static void
ch_on_idle_timeout(QUIC_CHANNEL* ch);
static void
ch_update_idle(QUIC_CHANNEL* ch);
static void
ch_update_ping_deadline(QUIC_CHANNEL* ch);
static void
ch_on_terminating_timeout(QUIC_CHANNEL* ch);
static void
ch_start_terminating(QUIC_CHANNEL* ch,
                     const QUIC_TERMINATE_CAUSE* tcause,
                     int force_immediate);
static void
ch_on_txp_ack_tx(const OSSL_QUIC_FRAME_ACK* ack, uint32_t pn_space, void* arg);
static void
ch_rx_handle_version_neg(QUIC_CHANNEL* ch, OSSL_QRX_PKT* pkt);
static void
ch_raise_version_neg_failure(QUIC_CHANNEL* ch);
static void
ch_record_state_transition(QUIC_CHANNEL* ch, uint32_t new_state);

DEFINE_LHASH_OF_EX(QUIC_SRT_ELEM);

QUIC_NEEDS_LOCK
static QLOG*
ch_get_qlog(QUIC_CHANNEL* ch)
{
...
ch_init(QUIC_CHANNEL* ch)
{
  ...
  if (!ch->is_server && !ossl_quic_gen_rand_conn_id(ch->port->engine->libctx,
                                                    tx_init_dcid_len,
                                                    &ch->init_dcid))
    goto err;
  ...
  if ((ch->ackm = ossl_ackm_new(get_time,
                                ch,
                                &ch->statm,
                                ch->cc_method,
                                ch->cc_data,
...
int
ossl_quic_channel_init(QUIC_CHANNEL* ch)
{
  return ch_init(ch);
}

void
ossl_quic_channel_bind_qrx(QUIC_CHANNEL* tserver_ch, OSSL_QRX* qrx)
{
  if (tserver_ch->qrx == NULL && tserver_ch->is_tserver_ch == 1) {
    tserver_ch->qrx = qrx;
    ossl_qrx_set_late_validation_cb(
      tserver_ch->qrx, rx_late_validate, tserver_ch);
    ossl_qrx_set_key_update_cb(tserver_ch->qrx, rxku_detected, tserver_ch);
  }
}

QUIC_CHANNEL*
ossl_quic_channel_alloc(const QUIC_CHANNEL_ARGS* args)
{
...
void
ossl_quic_channel_free(QUIC_CHANNEL* ch)
{
  if (ch == NULL)
    return;
...
int
ossl_quic_channel_set_mutator(QUIC_CHANNEL* ch,
...
int
ossl_quic_channel_get_peer_addr(QUIC_CHANNEL* ch, BIO_ADDR* peer_addr)
{
...
int
ossl_quic_channel_set_peer_addr(QUIC_CHANNEL* ch, const BIO_ADDR* peer_addr)
{
...
QUIC_REACTOR*
ossl_quic_channel_get_reactor(QUIC_CHANNEL* ch)
{
  return ossl_quic_port_get0_reactor(ch->port);
}

QUIC_STREAM_MAP*
ossl_quic_channel_get_qsm(QUIC_CHANNEL* ch)
{
  return &ch->qsm;
}

OSSL_STATM*
ossl_quic_channel_get_statm(QUIC_CHANNEL* ch)
{
  return &ch->statm;
}

SSL*
ossl_quic_channel_get0_tls(QUIC_CHANNEL* ch)
{
  return ch->tls;
}

static void
free_buf_mem(unsigned char* buf, size_t buf_len, void* arg)
{
  BUF_MEM_free((BUF_MEM*)arg);
}

int
ossl_quic_channel_schedule_new_token(QUIC_CHANNEL* ch,
...
size_t
ossl_quic_channel_get_short_header_conn_id_len(QUIC_CHANNEL* ch)
{
  return ossl_quic_port_get_rx_short_dcid_len(ch->port);
}

QUIC_STREAM*
ossl_quic_channel_get_stream_by_id(QUIC_CHANNEL* ch, uint64_t stream_id)
{
  return ossl_quic_stream_map_get_by_id(&ch->qsm, stream_id);
}

int
ossl_quic_channel_is_active(const QUIC_CHANNEL* ch)
{
  return ch != NULL && ch->state == QUIC_CHANNEL_STATE_ACTIVE;
}

int
ossl_quic_channel_is_closing(const QUIC_CHANNEL* ch)
{
  return ch->state == QUIC_CHANNEL_STATE_TERMINATING_CLOSING;
}

static int
ossl_quic_channel_is_draining(const QUIC_CHANNEL* ch)
{
  return ch->state == QUIC_CHANNEL_STATE_TERMINATING_DRAINING;
}

static int
ossl_quic_channel_is_terminating(const QUIC_CHANNEL* ch)
{
  return ossl_quic_channel_is_closing(ch) || ossl_quic_channel_is_draining(ch);
}

int
ossl_quic_channel_is_terminated(const QUIC_CHANNEL* ch)
{
  return ch->state == QUIC_CHANNEL_STATE_TERMINATED;
}

int
ossl_quic_channel_is_term_any(const QUIC_CHANNEL* ch)
{
...
int
ossl_quic_channel_is_server(const QUIC_CHANNEL* ch)
{
  return ch->is_server;
}

void
ossl_quic_channel_notify_flush_done(QUIC_CHANNEL* ch)
{
  ch_record_state_transition(ch,
                             ch->terminate_cause.remote
  ...
  ch->terminate_deadline = ossl_time_add(
    get_time(ch), ossl_time_multiply(ossl_ackm_get_pto_duration(ch->ackm), 3));
...
int
ossl_quic_channel_is_handshake_complete(const QUIC_CHANNEL* ch)
{
  return ch->handshake_complete;
}

int
ossl_quic_channel_is_handshake_confirmed(const QUIC_CHANNEL* ch)
{
  return ch->handshake_confirmed;
}

QUIC_DEMUX*
ossl_quic_channel_get0_demux(QUIC_CHANNEL* ch)
{
  return ch->port->demux;
}

QUIC_PORT*
ossl_quic_channel_get0_port(QUIC_CHANNEL* ch)
{
  return ch->port;
}
...
CRYPTO_MUTEX*
ossl_quic_channel_get_mutex(QUIC_CHANNEL* ch)
{
  return ossl_quic_port_get0_mutex(ch->port);
}

int
ossl_quic_channel_has_pending(const QUIC_CHANNEL* ch)
{
...
QUIC_NEEDS_LOCK
static void
ch_trigger_txku(QUIC_CHANNEL* ch)
{
  uint64_t next_pn =
  ...
  if (!ossl_quic_pn_valid(next_pn) || !ossl_qtx_trigger_key_update(ch->qtx)) {
    ossl_quic_channel_raise_protocol_error(
      ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "key update");
    return;
  }
...
QUIC_NEEDS_LOCK
static int
txku_in_progress(QUIC_CHANNEL* ch)
{
...
QUIC_NEEDS_LOCK
static int
txku_recommendable(QUIC_CHANNEL* ch)
{
  if (!txku_allowed(ch))
    return 0;
...
QUIC_NEEDS_LOCK
static int
txku_desirable(QUIC_CHANNEL* ch)
{
...
QUIC_NEEDS_LOCK
static void
ch_maybe_trigger_spontaneous_txku(QUIC_CHANNEL* ch)
{
...
ch_on_crypto_release_record(size_t bytes_read, void* arg)
{
  ...
  if (!ossl_quic_rxfc_on_retire(
        &ch->crypto_rxfc[rx_pn_space], bytes_read, rtt_info.smoothed_rtt))
    return 0;
                             ...
                             void* arg)
{
  ...
  if (direction) {
    ...
    if (!ossl_qtx_provide_secret(
          ch->qtx, enc_level, suite_id, md, secret, secret_len))
      return 0;
    ...
    if (!ossl_qrx_provide_secret(
          ch->qrx, enc_level, suite_id, md, secret, secret_len))
      return 0;
...
ch_on_handshake_complete(void* arg)
{
  ...
  if (ch->pending_new_token != NULL) {
    ...
    ossl_quic_channel_schedule_new_token(
      ch, ch->pending_new_token, ch->pending_new_token_len);
    OPENSSL_free(ch->pending_new_token);
...
static void
txfc_bump_cwm_bidi(QUIC_STREAM* s, void* arg)
{
...
static void
txfc_bump_cwm_uni(QUIC_STREAM* s, void* arg)
{
  if (ossl_quic_stream_is_bidi(s) || ossl_quic_stream_is_server_init(s))
    return;
...
static void
do_update(QUIC_STREAM* s, void* arg)
{
  QUIC_CHANNEL* ch = arg;
...
static uint64_t
min_u64_ignore_0(uint64_t a, uint64_t b)
{
  if (a == 0)
    return b;
  if (b == 0)
                       ...
                       void* arg)
{
  ...
  if (sc == NULL) {
    ossl_quic_channel_raise_protocol_error(
      ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "could not get ssl connection");
    return 0;
  }
  /*
  ...
  if (!PACKET_buf_init(&pkt, params, params_len)) {
    ossl_quic_channel_raise_protocol_error(
      ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "internal error (packet buf init)");
    return 0;
  }
  ...
  while (PACKET_remaining(&pkt) > 0) {
    ...
    switch (id) {
      ...
      case QUIC_TPARAM_STATELESS_RESET_TOKEN:
        ...
        if (!ossl_quic_srtm_add(ch->srtm,
                                ch,
                                ch->cur_remote_seq_num,
  ...
  if (ch->is_server && !ch_generate_transport_params(ch)) {
    ossl_quic_channel_raise_protocol_error(
      ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "internal error");
    return 0;
  }
 ...
 */
void
ossl_quic_channel_subtick(QUIC_CHANNEL* ch,
                          QUIC_TICK_RESULT* res,
                          uint32_t flags)
{
...
ch_tick_tls(QUIC_CHANNEL* ch, int channel_only, int* notify_other_threads)
{
  ...
  if (ossl_quic_tls_get_error(
        ch->qtls, &error_code, &error_msg, &error_state)) {
    ossl_quic_channel_raise_protocol_error_state(
      ch, error_code, 0, error_msg, error_state);
...
static void
ch_rx_check_forged_pkt_limit(QUIC_CHANNEL* ch)
{
  uint32_t enc_level;
  ...
  ossl_quic_channel_raise_protocol_error(
    ch, OSSL_QUIC_ERR_AEAD_LIMIT_REACHED, 0, "forgery limit");
}
...
static int
bio_addr_eq(const BIO_ADDR* a, const BIO_ADDR* b)
{
...
ch_rx_handle_packet(QUIC_CHANNEL* ch, int channel_only)
{
  ...
  if (ch->qrx_pkt->hdr->type == QUIC_PKT_TYPE_VERSION_NEG) {
     ...
     */
    ossl_quic_channel_raise_protocol_error(
      ch, OSSL_QUIC_ERR_CONNECTION_REFUSED, 0, "unsupported protocol version");
    return;
  }
...
static void
free_peer_token(const unsigned char* token, size_t token_len, void* arg)
{
  ossl_quic_free_peer_token((QUIC_TOKEN*)arg);
}

int
ossl_quic_channel_start(QUIC_CHANNEL* ch)
{
  QUIC_TOKEN* token;
   ...
   */
  if (!ch->is_server &&
      ossl_quic_get_peer_token(
        ch->port->channel_ctx, &ch->cur_peer_addr, &token) &&
...
static void
free_token(const unsigned char* token, size_t token_len, void* arg)
{
...
void
ossl_quic_channel_local_close(QUIC_CHANNEL* ch,
         ...
         int drop_later_pn)
{
  ...
  if (!ossl_quic_tx_packetiser_set_initial_token(
     ...
     */
    ossl_quic_channel_raise_protocol_error(
      ch, OSSL_QUIC_ERR_INVALID_TOKEN, 0, "received oversize token");
    OPENSSL_free(buf);
    return 0;
  }
...
int
ossl_quic_channel_on_handshake_confirmed(QUIC_CHANNEL* ch)
{
 ...
 */
static void
copy_tcause(QUIC_TERMINATE_CAUSE* dst, const QUIC_TERMINATE_CAUSE* src)
{
  /*
...
void
ossl_quic_channel_set_tcause(QUIC_CHANNEL* ch,
                             uint64_t app_error_code,
                             const char* app_reason)
{
  QUIC_TERMINATE_CAUSE tcause = { 0 };
...
/* For RXDP use. */
void
ossl_quic_channel_on_remote_conn_close(QUIC_CHANNEL* ch,
...
static void
free_frame_data(unsigned char* buf, size_t buf_len, void* arg)
{
  OPENSSL_free(buf);
}

static int
ch_enqueue_retire_conn_id(QUIC_CHANNEL* ch, uint64_t seq_num)
{
  BUF_MEM* buf_mem = NULL;
  WPACKET wpkt;
  size_t l;
...
void
ossl_quic_channel_on_new_conn_id(QUIC_CHANNEL* ch,
                                 OSSL_QUIC_FRAME_NEW_CONN_ID* f)
{
  ...
  if (new_remote_seq_num > ch->cur_remote_seq_num) {
    ...
    if (!ossl_quic_srtm_add(
          ch->srtm, ch, new_remote_seq_num, &f->stateless_reset)) {
      ossl_quic_channel_raise_protocol_error(
        ch,
        OSSL_QUIC_ERR_CONNECTION_ID_LIMIT_ERROR,
        OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID,
...
void
ossl_quic_channel_inject(QUIC_CHANNEL* ch, QUIC_URXE* e)
{
  ossl_qrx_inject_urxe(ch->qrx, e);
}

void
ossl_quic_channel_inject_pkt(QUIC_CHANNEL* ch, OSSL_QRX_PKT* qpkt)
{
  ossl_qrx_inject_pkt(ch->qrx, qpkt);
}

void
ossl_quic_channel_on_stateless_reset(QUIC_CHANNEL* ch)
{
  QUIC_TERMINATE_CAUSE tcause = { 0 };
...
void
ossl_quic_channel_raise_net_error(QUIC_CHANNEL* ch)
{
  QUIC_TERMINATE_CAUSE tcause = { 0 };
...
int
ossl_quic_channel_net_error(QUIC_CHANNEL* ch)
{
  return ch->net_error;
}

void
ossl_quic_channel_restore_err_state(QUIC_CHANNEL* ch)
{
  if (ch == NULL)
    return;
...
void
ossl_quic_channel_raise_protocol_error_loc(QUIC_CHANNEL* ch,
                                           uint64_t error_code,
                                           uint64_t frame_type,
                                           const char* reason,
                                           ERR_STATE* err_state,
 ...
 */
static OSSL_TIME
ch_get_effective_idle_timeout_duration(QUIC_CHANNEL* ch)
{
  OSSL_TIME pto;
 ...
 */
static int
ch_on_new_conn_common(QUIC_CHANNEL* ch,
                      const BIO_ADDR* peer,
...
int
ossl_quic_channel_on_new_conn(QUIC_CHANNEL* ch,
                              const BIO_ADDR* peer,
                              const QUIC_CONN_ID* peer_scid,
                              const QUIC_CONN_ID* peer_dcid)
{
 ...
 */
int
ossl_quic_bind_channel(QUIC_CHANNEL* ch,
                       const BIO_ADDR* peer,
                       const QUIC_CONN_ID* peer_scid,
                       const QUIC_CONN_ID* peer_dcid,
                       const QUIC_CONN_ID* peer_odcid)
...
SSL*
ossl_quic_channel_get0_ssl(QUIC_CHANNEL* ch)
{
  return ch->tls;
}

static int
ch_init_new_stream(QUIC_CHANNEL* ch,
                   QUIC_STREAM* qs,
                   int can_send,
                   int can_recv)
{
...
static uint64_t*
ch_get_local_stream_next_ordinal_ptr(QUIC_CHANNEL* ch, int is_uni)
{
...
ch_get_local_stream_max_ptr(const QUIC_CHANNEL* ch, int is_uni)
{
  return is_uni ? &ch->max_local_streams_uni : &ch->max_local_streams_bidi;
}

static const QUIC_RXFC*
ch_get_remote_stream_count_rxfc(const QUIC_CHANNEL* ch, int is_uni)
{
  return is_uni ? &ch->max_streams_uni_rxfc : &ch->max_streams_bidi_rxfc;
}

int
ossl_quic_channel_is_new_local_stream_admissible(QUIC_CHANNEL* ch, int is_uni)
{
...
uint64_t
ossl_quic_channel_get_local_stream_count_avail(const QUIC_CHANNEL* ch,
                                               int is_uni)
{
...
uint64_t
ossl_quic_channel_get_remote_stream_count_avail(const QUIC_CHANNEL* ch,
                                                int is_uni)
{
...
QUIC_STREAM*
ossl_quic_channel_new_stream_local(QUIC_CHANNEL* ch, int is_uni)
{
  QUIC_STREAM* qs;
  int type;
  uint64_t stream_id;
...
void
ossl_quic_channel_set_incoming_stream_auto_reject(QUIC_CHANNEL* ch,
                                                  int enable,
                                                  uint64_t aec)
{
...
void
ossl_quic_channel_reject_stream(QUIC_CHANNEL* ch, QUIC_STREAM* qs)
{
...
int
ossl_quic_channel_replace_local_cid(QUIC_CHANNEL* ch,
...
void
ossl_quic_channel_set_msg_callback(QUIC_CHANNEL* ch,
                                   ...
                                   SSL* msg_callback_ssl)
{
  ...
  ossl_quic_tx_packetiser_set_msg_callback(
    ch->txp, msg_callback, msg_callback_ssl);
  /*
...
void
ossl_quic_channel_set_msg_callback_arg(QUIC_CHANNEL* ch, void* msg_callback_arg)
{
...
void
ossl_quic_channel_set_txku_threshold_override(QUIC_CHANNEL* ch,
...
uint64_t
ossl_quic_channel_get_tx_key_epoch(QUIC_CHANNEL* ch)
{
  return ossl_qtx_get_key_epoch(ch->qtx);
}

uint64_t
ossl_quic_channel_get_rx_key_epoch(QUIC_CHANNEL* ch)
{
  return ossl_qrx_get_key_epoch(ch->qrx);
}

int
ossl_quic_channel_trigger_txku(QUIC_CHANNEL* ch)
{
  if (!txku_allowed(ch))
    return 0;
...
int
ossl_quic_channel_ping(QUIC_CHANNEL* ch)
{
  int pn_space = ossl_quic_enc_level_to_pn_space(ch->tx_enc_level);
...
uint16_t
ossl_quic_channel_get_diag_num_rx_ack(QUIC_CHANNEL* ch)
{
  return ch->diag_num_rx_ack;
}

void
ossl_quic_channel_get_diag_local_cid(QUIC_CHANNEL* ch, QUIC_CONN_ID* cid)
{
  *cid = ch->cur_local_cid;
}

int
ossl_quic_channel_have_generated_transport_params(const QUIC_CHANNEL* ch)
{
  return ch->got_local_transport_params;
}

void
ossl_quic_channel_set_max_idle_timeout_request(QUIC_CHANNEL* ch, uint64_t ms)
{
...
uint64_t
ossl_quic_channel_get_max_idle_timeout_actual(const QUIC_CHANNEL* ch)
{
