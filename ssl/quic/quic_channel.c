...
static int ch_tick_tls(QUIC_CHANNEL* ch,
                       int channel_only,
...
static int ch_on_crypto_send(const unsigned char* buf,
                             size_t buf_len,
                             size_t* consumed,
                             void* arg);
 ...
 * "loudly".
 */
void ossl_quic_channel_subtick(QUIC_CHANNEL* ch,
...
static void free_token(const unsigned char* token,
                       size_t token_len,
                       void* arg) {
...
void ossl_quic_channel_set_tcause(QUIC_CHANNEL* ch,
                                  uint64_t app_error_code,
                                  const char* app_reason) {
  QUIC_TERMINATE_CAUSE tcause = {0};
...
static int ch_init_new_stream(QUIC_CHANNEL* ch,
                              QUIC_STREAM* qs,
                              int can_send,
                              int can_recv) {
  uint64_t rxfc_wnd;
