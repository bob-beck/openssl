...
static int ch_init(QUIC_CHANNEL *ch)
{
    ...
    if ((ch->ackm = ossl_ackm_new(get_time, ch, &ch->statm, ch->cc_method, ch->cc_data, ch->is_server)) == NULL)
        goto err;
...
{
    ch_record_state_transition(ch, ch->terminate_cause.remote ? QUIC_CHANNEL_STATE_TERMINATING_DRAINING
     ...
     */
    ch->terminate_deadline = ossl_time_add(get_time(ch), ossl_time_multiply(ossl_ackm_get_pto_duration(ch->ackm), 3));
...
static int ch_on_transport_params(const unsigned char *params, size_t params_len, void *arg)
{
    ...
    if (sc == NULL)
    {
        ossl_quic_channel_raise_protocol_error(ch, OSSL_QUIC_ERR_INTERNAL_ERROR, 0, "could not get ssl connection");
        return 0;
    }
    /*
     ...
     */
    if (ch->is_server && sc->hello_retry_request != SSL_HRR_NONE && ch->got_remote_transport_params)
    {
...
void ossl_quic_channel_set_tcause(QUIC_CHANNEL *ch, uint64_t app_error_code, const char *app_reason)
{
    QUIC_TERMINATE_CAUSE tcause = {0};
