...
static QLOG* ch_get_qlog(QUIC_CHANNEL* ch) {
  ...
  if (ch->qlog != NULL) return ch->qlog;

  if (!ch->use_qlog) return NULL;

  if (ch->is_server && ch->init_dcid.id_len == 0) return NULL;
...
static int ch_init(QUIC_CHANNEL* ch) {
  ...
  if (ch->port == NULL || ch->lcidm == NULL || ch->srtm == NULL) goto err;
  ...
  if (ch->qtx == NULL) goto err;

  ch->txpim = ossl_quic_txpim_new();
  if (ch->txpim == NULL) goto err;

  ch->cfq = ossl_quic_cfq_new();
  if (ch->cfq == NULL) goto err;

  if (!ossl_quic_txfc_init(&ch->conn_txfc, NULL)) goto err;
  ...
  if (!ossl_quic_rxfc_init(
          ...
          DEFAULT_CONN_RXFC_MAX_WND_MUL * DEFAULT_INIT_CONN_RXFC_WND, get_time,
          ch))
    goto err;
  ...
  if (!ossl_statm_init(&ch->statm)) goto err;

  ch->have_statm = 1;
  ch->cc_method = &ossl_cc_newreno_method;
  if ((ch->cc_data = ch->cc_method->new(get_time, ch)) == NULL) goto err;

  if ((ch->ackm = ossl_ackm_new(get_time, ch, &ch->statm, ch->cc_method,
                                ch->cc_data, ch->is_server)) == NULL)
    goto err;
  ...
  if (ch->qrx == NULL && ch->is_tserver_ch == 0) {
    ...
    if ((ch->qrx = ossl_qrx_new(&qrx_args)) == NULL) goto err;
  }
  ...
  if (ch->qrx != NULL) {
    ...
    if (!ossl_qrx_set_key_update_cb(ch->qrx, rxku_detected, ch)) goto err;
  }

  for (pn_space = QUIC_PN_SPACE_INITIAL; pn_space < QUIC_PN_SPACE_NUM;
       ++pn_space) {
    ch->crypto_recv[pn_space] = ossl_quic_rstream_new(NULL, NULL, 0);
    if (ch->crypto_recv[pn_space] == NULL) goto err;
  }
  ...
  if ((ch->qtls = ossl_quic_tls_new(&tls_args)) == NULL) goto err;
...
static void ch_cleanup(QUIC_CHANNEL* ch) {
  ...
  if (ch->lcidm != NULL) ossl_quic_lcidm_cull(ch->lcidm, ch);

  if (ch->srtm != NULL) ossl_quic_srtm_cull(ch->srtm, ch);

  ossl_quic_tx_packetiser_free(ch->txp);
  ossl_quic_txpim_free(ch->txpim);
  ossl_quic_cfq_free(ch->cfq);
  ossl_qtx_free(ch->qtx);
  if (ch->cc_data != NULL) ch->cc_method->free(ch->cc_data);
  if (ch->have_statm) ossl_statm_destroy(&ch->statm);
  ossl_ackm_free(ch->ackm);

  if (ch->have_qsm) ossl_quic_stream_map_cleanup(&ch->qsm);
...
#ifndef OPENSSL_NO_QLOG
  if (ch->qlog != NULL) ossl_qlog_flush(ch->qlog); /* best effort */
...
QUIC_CHANNEL* ossl_quic_channel_alloc(const QUIC_CHANNEL_ARGS* args) {
  ...
  if ((ch = OPENSSL_zalloc(sizeof(*ch))) == NULL) return NULL;
...
int ossl_quic_channel_set_peer_addr(QUIC_CHANNEL* ch,
  ...
  if (ch->state != QUIC_CHANNEL_STATE_IDLE) return 0;
...
int ossl_quic_channel_schedule_new_token(QUIC_CHANNEL* ch,
  ...
  buf_mem = BUF_MEM_new();
  if (buf_mem == NULL) goto err;

  if (!WPACKET_init(&wpkt, buf_mem)) goto err;
  ...
  if (!WPACKET_get_total_written(&wpkt, &l)) goto err;
  ...
  if (cfq_item == NULL) goto err;

  rc = 1;
err:
  if (!rc) BUF_MEM_free(buf_mem);
  return rc;
}
...
const QUIC_TERMINATE_CAUSE* ossl_quic_channel_get_terminate_cause(
...
static int rx_late_validate(QUIC_PN pn, int pn_space, void* arg) {
  ...
  if (!ossl_ackm_is_rx_pn_processable(ch->ackm, pn, pn_space)) return 0;
...
static void ch_maybe_trigger_spontaneous_txku(QUIC_CHANNEL* ch) {
  if (!txku_recommendable(ch) || !txku_desirable(ch)) return;
...
static void rxku_detected(QUIC_PN pn, void* arg) {
  ...
  if (!rxku_allowed(ch)) /* Is RXKU even allowed at this time? */
...
static int ch_on_crypto_send(const unsigned char* buf, size_t buf_len,
  ...
  if (!ossl_assert(sstream != NULL)) return 0;
...
static int crypto_ensure_empty(QUIC_RSTREAM* rstream) {
  ...
  if (!ossl_quic_rstream_available(rstream, &avail, &is_fin)) return 0;
...
static void txfc_bump_cwm_uni(QUIC_STREAM* s, void* arg) {
  if (ossl_quic_stream_is_bidi(s) || ossl_quic_stream_is_server_init(s)) return;
...
static int ch_on_transport_params(const unsigned char* params,
  ...
  while (PACKET_remaining(&pkt) > 0) {
    if (!ossl_quic_wire_peek_transport_param(&pkt, &id)) goto malformed;
  ...
  if (got_retry_scid) QLOG_CID("retry_source_connection_id", &ch->retry_scid);
  if (got_initial_max_data)
  ...
  if (got_ack_delay_exp) QLOG_U64("ack_delay_exponent", ch->rx_ack_delay_exp);
  if (got_max_ack_delay) QLOG_U64("max_ack_delay", ch->rx_max_ack_delay);
  if (got_max_udp_payload_size)
    QLOG_U64("max_udp_payload_size", ch->rx_max_udp_payload_size);
  if (got_max_idle_timeout) QLOG_U64("max_idle_timeout", rx_max_idle_timeout);
  if (got_active_conn_id_limit)
...
static int ch_generate_transport_params(QUIC_CHANNEL* ch) {
  ...
  if ((buf_mem = BUF_MEM_new()) == NULL) goto err;
  ...
  if (!WPACKET_finish(&wpkt)) goto err;

  wpkt_valid = 0;

  if (!WPACKET_get_total_written(&wpkt, &buf_len)) goto err;
  ...
  ok = 1;
err:
  if (wpkt_valid) WPACKET_cleanup(&wpkt);
...
void ossl_quic_channel_subtick(QUIC_CHANNEL* ch, QUIC_TICK_RESULT* res,
  ...
  if (ossl_time_compare(now, ch->idle_deadline) >= 0) {
     ...
     */
    if (!ch->port->engine->inhibit_tick) ch_on_idle_timeout(ch);
...
static int ch_tick_tls(QUIC_CHANNEL* ch, int channel_only,
  ...
  if (ossl_quic_tls_get_error(ch->qtls, &error_code, &error_msg,
    ...
    ossl_quic_channel_raise_protocol_error_state(ch, error_code, 0, error_msg,
                                                 error_state);
    if (notify_other_threads != NULL) *notify_other_threads = 1;
...
static void ch_rx_check_forged_pkt_limit(QUIC_CHANNEL* ch) {
  ...
  for (enc_level = QUIC_ENC_LEVEL_INITIAL; enc_level < QUIC_ENC_LEVEL_NUM;
     ...
     */
    if ((ch->el_discarded & (1U << enc_level)) != 0) continue;

    if (enc_level > ch->rx_enc_level) break;

    l = ossl_qrx_get_max_forged_pkt_count(ch->qrx, enc_level);
    if (l < limit) limit = l;
  }

  if (ossl_qrx_get_cur_forged_pkt_count(ch->qrx) < limit) return;
...
static int ch_rx(QUIC_CHANNEL* ch, int channel_only,
  ...
  for (;;) {
    ...
    if (!ossl_qrx_read_pkt(ch->qrx, &ch->qrx_pkt)) break;
  ...
  if (handled_any && notify_other_threads != NULL) *notify_other_threads = 1;

  /*
   * When in TERMINATING - CLOSING, generate a CONN_CLOSE frame whenever we
   * process one or more incoming packets.
   */
  if (handled_any && closing) ch->conn_close_queued = 1;
...
static void ch_rx_handle_packet(QUIC_CHANNEL* ch, int channel_only) {
   ...
   *      received frame.
   */
  if (!ossl_quic_channel_is_active(ch)) return;
  ...
  if (ch->qrx_pkt->hdr->type == QUIC_PKT_TYPE_VERSION_NEG) {
     ...
     */
    if (ch->qrx_pkt->hdr->version != 0) return;
    ...
    while (PACKET_remaining(&vpkt) > 0) {
       ...
       */
      if (!PACKET_get_net_4(&vpkt, &supported_ver)) return;
  ...
  switch (ch->qrx_pkt->hdr->type) {
    case QUIC_PKT_TYPE_RETRY:
       ...
       */
      if (ch->have_received_enc_pkt) return;
    ...
    case QUIC_PKT_TYPE_0RTT:
      if (!ch->is_server) /* Clients should never receive 0-RTT packets. */
        return;
    ...
    case QUIC_PKT_TYPE_1RTT:
      ...
      if (ch->did_crypto_frame) ch_tick_tls(ch, channel_only, NULL);
...
static void ch_rx_handle_version_neg(QUIC_CHANNEL* ch, OSSL_QRX_PKT* pkt) {
  ...
  if (!PACKET_buf_init(&vpkt, pkt->hdr->data, pkt->hdr->len)) return;

  while (PACKET_remaining(&vpkt) > 0) {
    if (!PACKET_get_net_4(&vpkt, &v)) break;

    if ((uint32_t)v == QUIC_VERSION_1) return;
  }
...
static int ch_tx(QUIC_CHANNEL* ch, int* notify_other_threads) {
   ...
   */
  if (ossl_quic_channel_is_draining(ch)) return 0;
  ...
  if (ossl_quic_channel_is_closing(ch)) {
     ...
     */
    if (!ch->conn_close_queued) return 0;
  ...
  do {
    ...
    if (status.sent_pkt > 0) {
      ...
      if (ch->rxku_pending_confirm_done) ch->rxku_pending_confirm = 0;
   ...
   * well as POLLIN.
   */
  if (ossl_qtx_get_queue_len_datagrams(ch->qtx) > 0) *notify_other_threads = 1;
...
static OSSL_TIME ch_determine_next_tick_deadline(QUIC_CHANNEL* ch) {
  ...
  if (ossl_quic_channel_is_terminated(ch)) return ossl_time_infinite();

  deadline = ossl_ackm_get_loss_detection_deadline(ch->ackm);
  if (ossl_time_is_zero(deadline)) deadline = ossl_time_infinite();
...
int ossl_quic_channel_start(QUIC_CHANNEL* ch) {
  ...
  if (!ossl_quic_tx_packetiser_set_peer(ch->txp, &ch->cur_peer_addr)) return 0;
  ...
  if (!ch_tick_tls(ch, /*channel_only=*/0, NULL)) return 0;
...
void ossl_quic_channel_local_close(QUIC_CHANNEL* ch, uint64_t app_error_code,
  ...
  if (ossl_quic_channel_is_term_any(ch)) return;
...
static int ch_retry(QUIC_CHANNEL* ch, const unsigned char* retry_token,
   ...
   */
  if (ossl_quic_conn_id_eq(&ch->init_dcid, retry_scid)) return 1;

  /* We change to using the SCID in the Retry packet as the DCID. */
  if (!ossl_quic_tx_packetiser_set_cur_dcid(ch->txp, retry_scid)) return 0;

  /*
   * Now we retry. We will release the Retry packet immediately, so copy
   * the token.
   */
  if ((buf = OPENSSL_memdup(retry_token, retry_token_len)) == NULL) return 0;
...
static int ch_discard_el(QUIC_CHANNEL* ch, uint32_t enc_level) {
  if (!ossl_assert(enc_level < QUIC_ENC_LEVEL_1RTT)) return 0;

  if ((ch->el_discarded & (1U << enc_level)) != 0) /* Already done. */
    return 1;
...
int ossl_quic_channel_on_handshake_confirmed(QUIC_CHANNEL* ch) {
  if (ch->handshake_confirmed) return 1;
...
static void ch_start_terminating(QUIC_CHANNEL* ch,
  ...
  if (!ch->have_sent_any_pkt) force_immediate = 1;
  ...
  switch (ch->state) {
    ...
    case QUIC_CHANNEL_STATE_TERMINATING_DRAINING:
       ...
       * timeout expires.
       */
      if (force_immediate) ch_on_terminating_timeout(ch);
...
void ossl_quic_channel_on_new_conn_id(QUIC_CHANNEL* ch,
  ...
  if (f->seq_num > new_remote_seq_num) new_remote_seq_num = f->seq_num;
  ...
  while (new_retire_prior_to > ch->cur_retire_prior_to) {
    if (!ch_enqueue_retire_conn_id(ch, ch->cur_retire_prior_to)) break;
    ++ch->cur_retire_prior_to;
  }
}

static void ch_save_err_state(QUIC_CHANNEL* ch) {
  if (ch->err_state == NULL) ch->err_state = OSSL_ERR_STATE_new();

  if (ch->err_state == NULL) return;
...
void ossl_quic_channel_raise_protocol_error_loc(
  ...
  if (ch->protocol_error) /* Only the first call to this function matters. */
    return;
   ...
   */
  if (err_state != NULL) OSSL_ERR_STATE_restore(err_state);
  ...
  if (src_file != NULL) ERR_set_debug(src_file, src_line, src_func);
...
static OSSL_TIME ch_get_effective_idle_timeout_duration(QUIC_CHANNEL* ch) {
  ...
  if (ch->max_idle_timeout == 0) return ossl_time_infinite();
...
static int ch_on_new_conn_common(QUIC_CHANNEL* ch, const BIO_ADDR* peer,
  ...
  if (!BIO_ADDR_copy(&ch->cur_peer_addr, peer)) return 0;

  ch->init_dcid = *peer_dcid;
  ch->cur_remote_dcid = *peer_scid;
  ch->odcid.id_len = 0;

  if (peer_odcid != NULL) ch->odcid = *peer_odcid;
...
int ossl_quic_bind_channel(QUIC_CHANNEL* ch, const BIO_ADDR* peer,
                           ...
                           const QUIC_CONN_ID* peer_dcid,
                           const QUIC_CONN_ID* peer_odcid) {
  if (peer_dcid == NULL) return 0;

  if (!ossl_assert(ch->state == QUIC_CHANNEL_STATE_IDLE && ch->is_server))
    return 0;

  ch->cur_local_cid = *peer_dcid;
  if (!ossl_quic_lcidm_bind_channel(ch->lcidm, ch, peer_dcid)) return 0;
...
static int ch_init_new_stream(QUIC_CHANNEL* ch, QUIC_STREAM* qs, int can_send,
  ...
  if (can_recv)
    if ((qs->rstream = ossl_quic_rstream_new(NULL, NULL, 0)) == NULL) goto err;

  /* TXFC */
  if (!ossl_quic_txfc_init(&qs->txfc, &ch->conn_txfc)) goto err;
...
QUIC_STREAM* ossl_quic_channel_new_stream_local(QUIC_CHANNEL* ch, int is_uni) {
  ...
  if (*p_next_ordinal >= ((uint64_t)1) << 62) return NULL;
...
QUIC_STREAM* ossl_quic_channel_new_stream_remote(QUIC_CHANNEL* ch,
  ...
  if ((stream_id & QUIC_STREAM_INITIATOR_MASK) != peer_role) return NULL;
...
int ossl_quic_channel_replace_local_cid(QUIC_CHANNEL* ch,
  ...
  if (!ossl_quic_lcidm_debug_remove(ch->lcidm, &ch->cur_local_cid)) return 0;
...
uint64_t ossl_quic_channel_get_max_idle_timeout_request(
    const QUIC_CHANNEL* ch) {
  return ch->max_idle_timeout_local_req;
}

uint64_t ossl_quic_channel_get_max_idle_timeout_peer_request(
    const QUIC_CHANNEL* ch) {
