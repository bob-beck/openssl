 ...
 * this could differ from QUIC_DEFAULT_MAX_DELAY in future as that is a
 * protocol value which determines the value of the maximum ACK delay if the
...
static void ch_start_terminating (QUIC_CHANNEL *ch,
                                  ...
                                  int force_immediate);
static void ch_on_txp_ack_tx (const OSSL_QUIC_FRAME_ACK *ack,
...
ch_init (QUIC_CHANNEL *ch)
{
  ...
  if ((ch->ackm = ossl_ackm_new (get_time, ch, &ch->statm, ch->cc_method,
                                 ch->cc_data, ch->is_server))
      == NULL)
    goto err;

  if (!ossl_quic_stream_map_init (&ch->qsm, get_stream_limit, ch,
                                  &ch->max_streams_bidi_rxfc,
                                  &ch->max_streams_uni_rxfc, ch))
    goto err;
...
ossl_quic_channel_notify_flush_done (QUIC_CHANNEL *ch)
{
  ch_record_state_transition (
      ch, ch->terminate_cause.remote ? QUIC_CHANNEL_STATE_TERMINATING_DRAINING
...
ch_rxku_tick (QUIC_CHANNEL *ch)
{
  ...
  if (!ossl_qrx_key_update_timeout (ch->qrx, /*normal=*/1))
    ossl_quic_channel_raise_protocol_error (ch, OSSL_QUIC_ERR_INTERNAL_ERROR,
                                            0, "RXKU cooldown internal error");
}
 ...
 */
static int
ch_on_crypto_send (const unsigned char *buf, size_t buf_len, size_t *consumed,
                   void *arg)
{
  int ret;
  QUIC_CHANNEL *ch = arg;
...
static int
ch_on_crypto_recv_record (const unsigned char **buf, size_t *bytes_read,
                          void *arg)
{
  QUIC_CHANNEL *ch = arg;
...
static int
ch_on_handshake_yield_secret (uint32_t prot_level, int direction,
                              uint32_t suite_id, EVP_MD *md,
...
static int
ch_on_transport_params (const unsigned char *params, size_t params_len,
                        void *arg)
{
  QUIC_CHANNEL *ch = arg;
  PACKET pkt;
 ...
 */
void
ossl_quic_channel_subtick (QUIC_CHANNEL *ch, QUIC_TICK_RESULT *res,
                           uint32_t flags)
{
...
ch_rx_handle_packet (QUIC_CHANNEL *ch, int channel_only)
{
  ...
  ossl_qlog_event_transport_packet_received (
      ch_get_qlog (ch), ch->qrx_pkt->hdr, ch->qrx_pkt->pn, &iovec, 1,
...
void
ossl_quic_channel_local_close (QUIC_CHANNEL *ch, uint64_t app_error_code,
...
static int
ch_retry (QUIC_CHANNEL *ch, const unsigned char *retry_token,
          size_t retry_token_len, const QUIC_CONN_ID *retry_scid,
          int drop_later_pn)
{
  void *buf;
 ...
 *     if we caused the termination (e.g. we have sent a CONNECTION_CLOSE).
 * Note that we are considered to have caused a termination if we sent the
 * first CONNECTION_CLOSE frame, even if it is caused by a peer protocol
...
void
ossl_quic_channel_set_tcause (QUIC_CHANNEL *ch, uint64_t app_error_code,
                              const char *app_reason)
{
  QUIC_TERMINATE_CAUSE tcause = { 0 };
...
static void
ch_start_terminating (QUIC_CHANNEL *ch, const QUIC_TERMINATE_CAUSE *tcause,
                      int force_immediate)
{
...
/*
 * Updates our idle deadline. Called when an event happens which should bump
 * the idle timeout.
 */
static void
ch_update_idle (QUIC_CHANNEL *ch)
{
  ch->idle_deadline = ossl_time_add (
      get_time (ch), ch_get_effective_idle_timeout_duration (ch));
}
 ...
 */
static int
ch_on_new_conn_common (QUIC_CHANNEL *ch, const BIO_ADDR *peer,
...
int
ossl_quic_channel_on_new_conn (QUIC_CHANNEL *ch, const BIO_ADDR *peer,
                               const QUIC_CONN_ID *peer_scid,
                               const QUIC_CONN_ID *peer_dcid)
{
 ...
 *                    address.
 * @param peer_scid   Pointer to the peer's source connection ID
 * (QUIC_CONN_ID).
 ...
 */
int
ossl_quic_bind_channel (QUIC_CHANNEL *ch, const BIO_ADDR *peer,
                        const QUIC_CONN_ID *peer_scid,
                        const QUIC_CONN_ID *peer_dcid,
                        const QUIC_CONN_ID *peer_odcid)
{
...
static int
ch_init_new_stream (QUIC_CHANNEL *ch, QUIC_STREAM *qs, int can_send,
                    int can_recv)
{
  uint64_t rxfc_wnd;
...
void
ossl_quic_channel_set_msg_callback (QUIC_CHANNEL *ch, ossl_msg_cb msg_callback,
...
void
ossl_quic_channel_set_msg_callback_arg (QUIC_CHANNEL *ch,
