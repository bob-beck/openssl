...
static void uint_set_merge_adjacent(UINT_SET* s, UINT_SET_ITEM* x) {
  ...
  if (x->range.start - 1 != xprev->range.end) return;
...
int ossl_uint_set_insert(UINT_SET* s, const UINT_RANGE* range) {
  ...
  if (!ossl_assert(start <= end)) return 0;
  ...
  for (; z != NULL; z = zprev) {
    ...
    if (z->range.start <= start && z->range.end >= end) return 1;
...
int ossl_uint_set_query(const UINT_SET* s, uint64_t v) {
  ...
  if (ossl_list_uint_set_is_empty(s)) return 0;
