...
void
ossl_qlog_event_connectivity_connection_started (QLOG *qlog,
...
#ifndef OPENSSL_NO_QLOG
static const char *
map_state_to_qlog (uint32_t state, int handshake_complete,
...
void
ossl_qlog_event_connectivity_connection_state_updated (QLOG *qlog,
                                                       uint32_t old_state,
                                                       uint32_t new_state,
...
#ifndef OPENSSL_NO_QLOG
static const char *
quic_err_to_qlog (uint64_t error_code)
{
  switch (error_code)
    {
...
#ifndef OPENSSL_NO_QLOG
static const char *
quic_pkt_type_to_qlog (uint32_t pkt_type)
{
  switch (pkt_type)
    {
...
void
ossl_qlog_event_recovery_packet_lost (QLOG *qlog, const QUIC_TXPIM_PKT *tpkt)
{
#ifndef OPENSSL_NO_QLOG
 ...
 * to log them. We should do this separately to the RXDP code because we want
 * to log the packet and its contents before we start to actually process it in
 ...
 */
static int
log_frame_actual (QLOG *qlog_instance, PACKET *pkt, size_t *need_skip)
{
  uint64_t frame_type;
    ...
    case OSSL_QUIC_FRAME_TYPE_STREAM_OFF_LEN_FIN:
      {
        ...
        if (f.is_fin)
          QLOG_BOOL ("fin", 1);
        *need_skip
            = f.has_explicit_len ? *need_skip + (size_t)f.len : SIZE_MAX;
      }
      break;
...
static void
log_frame (QLOG *qlog_instance, PACKET *pkt, size_t *need_skip)
{
...
static int
log_frames (QLOG *qlog_instance, const OSSL_QTX_IOVEC *iovec, size_t num_iovec)
{
  size_t i;
  PACKET pkt;
  size_t need_skip = 0;
...
static void
log_packet (QLOG *qlog_instance, const QUIC_PKT_HDR *hdr, QUIC_PN pn,
...
void
ossl_qlog_event_transport_packet_sent (QLOG *qlog, const QUIC_PKT_HDR *hdr,
                                       QUIC_PN pn, const OSSL_QTX_IOVEC *iovec,
...
void
ossl_qlog_event_transport_packet_received (QLOG *qlog, const QUIC_PKT_HDR *hdr,
                                           QUIC_PN pn,
