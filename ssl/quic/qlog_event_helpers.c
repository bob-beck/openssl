...
void ossl_qlog_event_connectivity_connection_state_updated(
  ...
  if (state_s != NULL) QLOG_STR("state", state_s);
  QLOG_EVENT_END()
#endif
}
...
static int log_frame_actual(QLOG* qlog_instance, PACKET* pkt,
  ...
  switch (frame_type) {
    ...
    case OSSL_QUIC_FRAME_TYPE_PING:
      if (!ossl_quic_wire_decode_frame_ping(pkt)) goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_RESET_STREAM: {
      ...
      if (!ossl_quic_wire_decode_frame_reset_stream(pkt, &f)) goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_STOP_SENDING: {
      ...
      if (!ossl_quic_wire_decode_frame_stop_sending(pkt, &f)) goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_CRYPTO: {
      ...
      if (!ossl_quic_wire_decode_frame_crypto(pkt, 1, &f)) goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_STREAM_OFF_LEN_FIN: {
      ...
      if (!ossl_quic_wire_decode_frame_stream(pkt, 1, &f)) goto unknown;
      ...
      if (f.is_fin) QLOG_BOOL("fin", 1);
      *need_skip = f.has_explicit_len ? *need_skip + (size_t)f.len : SIZE_MAX;
    } break;
    case OSSL_QUIC_FRAME_TYPE_MAX_DATA: {
      uint64_t x;

      if (!ossl_quic_wire_decode_frame_max_data(pkt, &x)) goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_MAX_STREAMS_UNI: {
      ...
      if (!ossl_quic_wire_decode_frame_max_streams(pkt, &x)) goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_CONN_CLOSE_TRANSPORT: {
      ...
      if (!ossl_quic_wire_decode_frame_conn_close(pkt, &f)) goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID: {
      ...
      if (!ossl_quic_wire_decode_frame_new_conn_id(pkt, &f)) goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_DATA_BLOCKED: {
      ...
      if (!ossl_quic_wire_decode_frame_data_blocked(pkt, &x)) goto unknown;
    ...
    case OSSL_QUIC_FRAME_TYPE_STREAMS_BLOCKED_UNI: {
      ...
      if (!ossl_quic_wire_decode_frame_streams_blocked(pkt, &x)) goto unknown;
...
static void log_frame(QLOG* qlog_instance, PACKET* pkt, size_t* need_skip) {
  ...
  if (!log_frame_actual(qlog_instance, pkt, need_skip)) return;
...
static int log_frames(QLOG* qlog_instance, const OSSL_QTX_IOVEC* iovec,
  ...
  for (i = 0; i < num_iovec; ++i) {
    if (!PACKET_buf_init(&pkt, iovec[i].buf, iovec[i].buf_len)) return 0;

    while (PACKET_remaining(&pkt) > 0) {
      if (need_skip > 0) {
        size_t adv = need_skip;

        if (adv > PACKET_remaining(&pkt)) adv = PACKET_remaining(&pkt);

        if (!PACKET_forward(&pkt, adv)) return 0;
...
static void log_packet(QLOG* qlog_instance, const QUIC_PKT_HDR* hdr, QUIC_PN pn,
  ...
  if (type_s == NULL) type_s = "unknown";

  QLOG_STR("packet_type", type_s);
  if (ossl_quic_pkt_type_has_pn(hdr->type)) QLOG_U64("packet_number", pn);
