...
int ossl_quic_reactor_init(QUIC_REACTOR* rtor,
  ...
  if ((flags & QUIC_REACTOR_FLAG_USE_NOTIFIER) != 0) {
    if (!ossl_rio_notifier_init(&rtor->notifier)) return 0;
...
int ossl_quic_reactor_tick(QUIC_REACTOR* rtor, uint32_t flags) {
  ...
  if (res.notify_other_threads) rtor_notify_other_threads(rtor);
...
static int poll_two_fds(int rfd, int rfd_want_read, int wfd, int wfd_want_write,
   ...
   */
  if (rfd >= FD_SETSIZE || wfd >= FD_SETSIZE) return 0;
#endif
  ...
  if (rfd != INVALID_SOCKET && rfd_want_read) openssl_fdset(rfd, &rfd_set);
  if (wfd != INVALID_SOCKET && wfd_want_write) openssl_fdset(wfd, &wfd_set);

  /* Always check for error conditions. */
  if (rfd != INVALID_SOCKET) openssl_fdset(rfd, &efd_set);
  if (wfd != INVALID_SOCKET) openssl_fdset(wfd, &efd_set);
  ...
  if (notify_rfd > maxfd) maxfd = notify_rfd;
  ...
  if (mutex != NULL) ossl_crypto_mutex_unlock(mutex);
#endif
...
#if defined(OPENSSL_THREADS)
  if (mutex != NULL) ossl_crypto_mutex_lock(mutex);
#endif
  ...
  if (rfd == wfd) {
    ...
    if (rfd >= 0 && pfds[npfd].events != 0) ++npfd;
  } else {
    pfds[npfd].fd = rfd;
    ...
    if (wfd >= 0 && pfds[npfd].events != 0) ++npfd;
  }
...
static void rtor_notify_other_threads(QUIC_REACTOR* rtor) {
  ...
  if (rtor->cur_blocking_waiters == 0) /* Nothing to do in this case. */
    return;
...
int ossl_quic_reactor_block_until_pred(QUIC_REACTOR* rtor,
  ...
  for (;;) {
    ...
    if ((res = pred(pred_arg)) != 0) return res;
