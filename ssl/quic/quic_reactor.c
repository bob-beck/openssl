...
ossl_quic_reactor_init(QUIC_REACTOR* rtor,
                       void (*tick_cb)(QUIC_TICK_RESULT* res,
                                       void* arg,
                                       uint32_t flags),
                       void* tick_cb_arg,
                       CRYPTO_MUTEX* mutex,
 ...
 */
static int
poll_two_descriptors(const BIO_POLL_DESCRIPTOR* r,
                     int r_want_read,
                     ...
                     CRYPTO_MUTEX* mutex)
{
  ...
  return poll_two_fds(
    rfd, r_want_read, wfd, w_want_write, notify_rfd, deadline, mutex);
}
