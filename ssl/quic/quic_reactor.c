...
int
ossl_quic_reactor_init (QUIC_REACTOR *rtor,
...
void
ossl_quic_reactor_cleanup (QUIC_REACTOR *rtor)
{
  if (rtor == NULL)
    return;
...
void
ossl_quic_reactor_set_poll_r (QUIC_REACTOR *rtor, const BIO_POLL_DESCRIPTOR *r)
{
  if (r == NULL)
    rtor->poll_r.type = BIO_POLL_DESCRIPTOR_TYPE_NONE;
  else
...
void
ossl_quic_reactor_set_poll_w (QUIC_REACTOR *rtor, const BIO_POLL_DESCRIPTOR *w)
{
  if (w == NULL)
    rtor->poll_w.type = BIO_POLL_DESCRIPTOR_TYPE_NONE;
  else
...
int
ossl_quic_reactor_can_support_poll_descriptor (const QUIC_REACTOR *rtor,
...
int
ossl_quic_reactor_can_poll_r (const QUIC_REACTOR *rtor)
{
  return rtor->can_poll_r;
}

int
ossl_quic_reactor_can_poll_w (const QUIC_REACTOR *rtor)
{
  return rtor->can_poll_w;
}

int
ossl_quic_reactor_net_read_desired (QUIC_REACTOR *rtor)
{
  return rtor->net_read_desired;
}

int
ossl_quic_reactor_net_write_desired (QUIC_REACTOR *rtor)
{
  return rtor->net_write_desired;
}

OSSL_TIME
ossl_quic_reactor_get_tick_deadline (QUIC_REACTOR *rtor)
{
  return rtor->tick_deadline;
}

int
ossl_quic_reactor_tick (QUIC_REACTOR *rtor, uint32_t flags)
{
...
RIO_NOTIFIER *
ossl_quic_reactor_get0_notifier (QUIC_REACTOR *rtor)
{
 ...
 * target anyway. However, the traditional issues with select(2) turn out not
 * to be an issue on Windows; whereas traditional *NIX select(2) uses a bitmap
 * of FDs (and thus is limited in the magnitude of the FDs expressible),
 * Windows select(2) is very different. In Windows, socket handles are not
 * allocated contiguously from zero and thus this bitmap approach was
 * infeasible. Thus in adapting the Berkeley sockets API to Windows a different
 * approach was taken whereby the fd_set contains a fixed length array of
 * socket handles and an integer indicating how many entries are valid; thus
 * Windows select() ironically is actually much more like *NIX poll(2) than
 * *NIX select(2). In any case, this means that the relevant limit for Windows
 * select() is the number of FDs being polled, not the magnitude of those FDs.
 * Since we only poll for two FDs here, this limit does not concern us.
 *
 ...
 *
 * deadline is a timestamp to return at. If it is ossl_time_infinite(), the
 ...
 */
static int
poll_two_fds (int rfd, int rfd_want_read, int wfd, int wfd_want_write,
...
static int
poll_descriptor_to_fd (const BIO_POLL_DESCRIPTOR *d, int *fd)
{
 ...
 *
 * If mutex is non-NULL, it is assumed be a lock currently held for write and
 * is unlocked for the duration of any wait.
 *
 * Precondition:   mutex is NULL or is held for write (unchecked)
 * Postcondition:  mutex is NULL or is held for write (unless
 *                   CRYPTO_THREAD_write_lock fails)
 */
static int
poll_two_descriptors (const BIO_POLL_DESCRIPTOR *r, int r_want_read,
 ...
 */
static void
rtor_notify_other_threads (QUIC_REACTOR *rtor)
{
  if (!rtor->have_notifier)
    return;
 ...
 */
int
ossl_quic_reactor_block_until_pred (QUIC_REACTOR *rtor,
...
void
ossl_quic_reactor_enter_blocking_section (QUIC_REACTOR *rtor)
{
  ++rtor->cur_blocking_waiters;
}

void
ossl_quic_reactor_leave_blocking_section (QUIC_REACTOR *rtor)
{
