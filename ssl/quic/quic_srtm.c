...
QUIC_SRTM* ossl_quic_srtm_new(OSSL_LIB_CTX* libctx, const char* propq) {
  ...
  if ((srtm = OPENSSL_zalloc(sizeof(*srtm))) == NULL) return NULL;

  /* Use AES-128-ECB as a permutation over 128-bit SRTs. */
  if ((ecb = EVP_CIPHER_fetch(libctx, "AES-128-ECB", propq)) == NULL) goto err;

  if ((srtm->blind_ctx = EVP_CIPHER_CTX_new()) == NULL) goto err;

  if (!EVP_EncryptInit_ex2(srtm->blind_ctx, ecb, key, NULL, NULL)) goto err;
...
int ossl_quic_srtm_add(QUIC_SRTM* srtm, void* opaque, uint64_t seq_num,
  ...
  if (srtm->alloc_failed) return 0;

  /* (opaque, seq_num) duplicates not allowed */
  if ((item = srtm_find(srtm, opaque, seq_num, &head, NULL)) != NULL) return 0;

  if ((item = OPENSSL_zalloc(sizeof(*item))) == NULL) return 0;
  ...
  if (r_item == NULL) {
    ...
    if (new_head != r_item) { /* head changed, update in lhash */
      ...
      if (!srtm_check_lh(srtm, srtm->items_rev)) /* As above. */
        return 0;
    }
  }
...
static int srtm_remove_from_rev(QUIC_SRTM* srtm, SRTM_ITEM* item) {
  ...
  if (rh_item == item) {
    ...
    if (item->next_by_srt_blinded != NULL) {
      ...
      if (!srtm_check_lh(srtm, srtm->items_rev)) return 0;
    } else {
...
int ossl_quic_srtm_remove(QUIC_SRTM* srtm, void* opaque, uint64_t seq_num) {
  ...
  if (prev == NULL) {
    ...
    if (item->next_by_seq_num != NULL) {
      ...
      if (!srtm_check_lh(srtm, srtm->items_fwd)) return 0;
    } else {
  ...
  if (!srtm_remove_from_rev(srtm, item)) return 0;
...
int ossl_quic_srtm_lookup(QUIC_SRTM* srtm,
  ...
  if (opaque != NULL) *opaque = item->opaque;
  if (seq_num != NULL) *seq_num = item->seq_num;
