 ...
 * pointers to an item structure, and another matching a SRT-derived value to
 * an item structure. Multiple items with different seq_num values under a
 * given opaque, and duplicate SRTs, are handled using sorted singly-linked
 * lists.
 *
 * The O(n) insert and lookup performance is tolerated on the basis that the
 * total number of entries for a given opaque (total number of extant CIDs for
 * a connection) should be quite small, and the QUIC protocol allows us to
 * place a hard limit on this via the active_connection_id_limit TPARAM. Thus
 * there is no risk of a large number of SRTs needing to be registered under a
 * given opaque.
 *
...
items_fwd_hash (const SRTM_ITEM *item)
{
  return (unsigned long)(uintptr_t)item->opaque;
}

static int
items_fwd_cmp (const SRTM_ITEM *a, const SRTM_ITEM *b)
{
  return a->opaque != b->opaque;
}

static unsigned long
items_rev_hash (const SRTM_ITEM *item)
{
  /*
...
static int
items_rev_cmp (const SRTM_ITEM *a, const SRTM_ITEM *b)
{
  /*
...
static int
srtm_check_lh (QUIC_SRTM *srtm, LHASH_OF (SRTM_ITEM) *lh)
{
...
QUIC_SRTM *
ossl_quic_srtm_new (OSSL_LIB_CTX *libctx, const char *propq)
{
  QUIC_SRTM *srtm = NULL;
...
static void
srtm_free_each (SRTM_ITEM *ihead)
{
...
void
ossl_quic_srtm_free (QUIC_SRTM *srtm)
{
  if (srtm == NULL)
    return;
 ...
 */
static SRTM_ITEM *
srtm_find (QUIC_SRTM *srtm, void *opaque, uint64_t seq_num, SRTM_ITEM **head_p,
           SRTM_ITEM **prev_p)
{
 ...
 */
static void
sorted_insert_seq_num (SRTM_ITEM *head, SRTM_ITEM *item, SRTM_ITEM **new_head)
{
 ...
 */
static void
sorted_insert_srt (SRTM_ITEM *head, SRTM_ITEM *item, SRTM_ITEM **new_head)
{
 ...
 * mitigation purposes. We compute this once as a cached value when an
 * SRTM_ITEM is formed.
 */
static int
srtm_compute_blinded (QUIC_SRTM *srtm, SRTM_ITEM *item,
...
int
ossl_quic_srtm_add (QUIC_SRTM *srtm, void *opaque, uint64_t seq_num,
                    const QUIC_STATELESS_RESET_TOKEN *token)
{
...
static int
srtm_remove_from_rev (QUIC_SRTM *srtm, SRTM_ITEM *item)
{
  SRTM_ITEM *rh_item;
...
int
ossl_quic_srtm_remove (QUIC_SRTM *srtm, void *opaque, uint64_t seq_num)
{
...
int
ossl_quic_srtm_cull (QUIC_SRTM *srtm, void *opaque)
{
...
int
ossl_quic_srtm_lookup (QUIC_SRTM *srtm,
...
static void
check_mark (SRTM_ITEM *item, void *arg)
{
...
static void
check_count (SRTM_ITEM *item, void *arg)
{
  struct check_args *arg_ = arg;
  uint32_t token = arg_->token;
...
void
ossl_quic_srtm_check (const QUIC_SRTM *srtm)
{
#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
