...
QUIC_SRT_GEN* ossl_quic_srt_gen_new(OSSL_LIB_CTX* libctx, const char* propq,
  ...
  if ((srt_gen = OPENSSL_zalloc(sizeof(*srt_gen))) == NULL) return NULL;

  if ((srt_gen->mac = EVP_MAC_fetch(libctx, "HMAC", propq)) == NULL) goto err;

  if ((srt_gen->mac_ctx = EVP_MAC_CTX_new(srt_gen->mac)) == NULL) goto err;
  ...
  *p++ = OSSL_PARAM_construct_end();
  ...
  if (!EVP_MAC_init(srt_gen->mac_ctx, key, key_len, params)) goto err;
...
int ossl_quic_srt_gen_calculate_token(QUIC_SRT_GEN* srt_gen,
  ...
  if (!EVP_MAC_init(srt_gen->mac_ctx, NULL, 0, NULL)) return 0;
