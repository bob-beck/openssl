...
QUIC_SRT_GEN*
ossl_quic_srt_gen_new(OSSL_LIB_CTX* libctx,
                      const char* propq,
...
void
ossl_quic_srt_gen_free(QUIC_SRT_GEN* srt_gen)
{
  if (srt_gen == NULL)
    return;
...
int
ossl_quic_srt_gen_calculate_token(QUIC_SRT_GEN* srt_gen,
                                  ...
                                  QUIC_STATELESS_RESET_TOKEN* token)
{
  ...
  if (!EVP_MAC_update(
        srt_gen->mac_ctx, (const unsigned char*)dcid->id, dcid->id_len))
    return 0;
