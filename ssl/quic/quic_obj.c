...
int ossl_quic_obj_init(QUIC_OBJ* obj, SSL_CTX* ctx, int type, SSL* parent_obj,
  ...
  if (!ossl_assert(!is_event_leader || parent_obj == NULL)) return 0;

  if (!ossl_ssl_init(&obj->ssl, ctx, ctx->method, type)) goto err;
  ...
  if (!obj_update_cache(obj)) goto err;
...
static int obj_update_cache(QUIC_OBJ* obj) {
  ...
  for (p = obj; p != NULL && !p->is_event_leader; p = p->parent_obj)
    if (!ossl_assert(p == obj || p->init_done)) return 0;

  if (!ossl_assert(p != NULL)) return 0;
...
SSL_CONNECTION* ossl_quic_obj_get0_handshake_layer(QUIC_OBJ* obj) {
  ...
  if (obj->ssl.type != SSL_TYPE_QUIC_CONNECTION) return NULL;
...
int ossl_quic_obj_blocking(const QUIC_OBJ* obj) {
  ...
  if (!ossl_quic_obj_desires_blocking(obj)) return 0;
