...
int
ossl_quic_conn_on_handshake_confirmed(QUIC_CONNECTION* qc);
 ...
 */
void
ossl_quic_conn_raise_protocol_error(QUIC_CONNECTION* qc,
                                    uint64_t error_code,
                                    uint64_t frame_type,
                                    const char* reason);

void
ossl_quic_conn_on_remote_conn_close(QUIC_CONNECTION* qc,
                                    OSSL_QUIC_FRAME_CONN_CLOSE* f);

#define OSSL_QUIC_ANY_VERSION 0xFFFFF
#endif

#define IMPLEMENT_quic_meth_func(                                              \
  version, func_name, q_accept, q_connect, enc_data)                           \
