               ...
               void* arg)
{
  ...
  if (SSL_select_next_proto(
        (unsigned char**)out, outlen, alpn, (unsigned int)alpnlen, in, inlen) !=
...
QUIC_TSERVER*
ossl_quic_tserver_new(const QUIC_TSERVER_ARGS* args,
                      const char* certfile,
                      const char* keyfile)
{
  ...
  ossl_quic_engine_set_time_cb(
    srv->engine, srv->args.now_cb, srv->args.now_cb_arg);
...
void
ossl_quic_tserver_free(QUIC_TSERVER* srv)
{
  if (srv == NULL)
    return;
...
int
ossl_quic_tserver_set_plain_packet_mutator(QUIC_TSERVER* srv,
                                           ossl_mutate_packet_cb mutatecb,
                                           ossl_finish_mutate_cb finishmutatecb,
                                           void* mutatearg)
{
  return ossl_quic_channel_set_mutator(
    srv->ch, mutatecb, finishmutatecb, mutatearg);
}
...
int
ossl_quic_tserver_tick(QUIC_TSERVER* srv)
{
...
int
ossl_quic_tserver_is_connected(QUIC_TSERVER* srv)
{
...
int
ossl_quic_tserver_is_term_any(const QUIC_TSERVER* srv)
{
...
int
ossl_quic_tserver_is_terminated(const QUIC_TSERVER* srv)
{
  return ossl_quic_channel_is_terminated(srv->ch);
}

size_t
ossl_quic_tserver_get_short_header_conn_id_len(const QUIC_TSERVER* srv)
{
  return ossl_quic_channel_get_short_header_conn_id_len(srv->ch);
}

int
ossl_quic_tserver_is_handshake_confirmed(const QUIC_TSERVER* srv)
{
  return ossl_quic_channel_is_handshake_confirmed(srv->ch);
}

int
ossl_quic_tserver_read(QUIC_TSERVER* srv,
                       uint64_t stream_id,
                       unsigned char* buf,
                       size_t buf_len,
                       size_t* bytes_read)
{
  ...
  if (*bytes_read > 0) {
    ...
    if (!ossl_quic_rxfc_on_retire(
          &qs->rxfc, *bytes_read, rtt_info.smoothed_rtt))
      return 0;
  }
...
int
ossl_quic_tserver_has_read_ended(QUIC_TSERVER* srv, uint64_t stream_id)
{
  QUIC_STREAM* qs;
  unsigned char buf[1];
  size_t bytes_read = 0;
  ...
  if (!ossl_quic_rstream_peek(
        qs->rstream, buf, sizeof(buf), &bytes_read, &is_fin))
    return 0;
...
int
ossl_quic_tserver_write(QUIC_TSERVER* srv,
                        uint64_t stream_id,
...
int
ossl_quic_tserver_conclude(QUIC_TSERVER* srv, uint64_t stream_id)
{
  QUIC_STREAM* qs;
...
int
ossl_quic_tserver_stream_new(QUIC_TSERVER* srv, int is_uni, uint64_t* stream_id)
{
  QUIC_STREAM* qs;
...
BIO*
ossl_quic_tserver_get0_rbio(QUIC_TSERVER* srv)
{
  return srv->args.net_rbio;
}

SSL_CTX*
ossl_quic_tserver_get0_ssl_ctx(QUIC_TSERVER* srv)
{
  return srv->ctx;
}

int
ossl_quic_tserver_stream_has_peer_stop_sending(QUIC_TSERVER* srv,
                                               uint64_t stream_id,
...
int
ossl_quic_tserver_stream_has_peer_reset_stream(QUIC_TSERVER* srv,
                                               uint64_t stream_id,
                                               uint64_t* app_error_code)
{
  QUIC_STREAM* qs;
...
int
ossl_quic_tserver_set_new_local_cid(QUIC_TSERVER* srv,
...
uint64_t
ossl_quic_tserver_pop_incoming_stream(QUIC_TSERVER* srv)
{
...
int
ossl_quic_tserver_is_stream_totally_acked(QUIC_TSERVER* srv, uint64_t stream_id)
{
  QUIC_STREAM* qs;
...
int
ossl_quic_tserver_get_net_read_desired(QUIC_TSERVER* srv)
{
...
int
ossl_quic_tserver_get_net_write_desired(QUIC_TSERVER* srv)
{
...
OSSL_TIME
ossl_quic_tserver_get_deadline(QUIC_TSERVER* srv)
{
...
int
ossl_quic_tserver_shutdown(QUIC_TSERVER* srv, uint64_t app_error_code)
{
...
int
ossl_quic_tserver_ping(QUIC_TSERVER* srv)
{
  if (ossl_quic_channel_is_terminated(srv->ch))
    return 0;
...
QUIC_CHANNEL*
ossl_quic_tserver_get_channel(QUIC_TSERVER* srv)
{
  return srv->ch;
}

void
ossl_quic_tserver_set_msg_callback(QUIC_TSERVER* srv,
                                   void (*f)(int write_p,
                                             int version,
                                             int content_type,
                                             const void* buf,
...
int
ossl_quic_tserver_new_ticket(QUIC_TSERVER* srv)
{
  return SSL_new_session_ticket(srv->tls);
}

int
ossl_quic_tserver_set_max_early_data(QUIC_TSERVER* srv, uint32_t max_early_data)
{
  return SSL_set_max_early_data(srv->tls, max_early_data);
}

void
ossl_quic_tserver_set_psk_find_session_cb(QUIC_TSERVER* srv,
