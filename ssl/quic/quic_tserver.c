...
QUIC_TSERVER* ossl_quic_tserver_new(const QUIC_TSERVER_ARGS* args,
  ...
  if (args->net_rbio == NULL || args->net_wbio == NULL) goto err;

  if ((srv = OPENSSL_zalloc(sizeof(*srv))) == NULL) goto err;

  srv->args = *args;

#if defined(OPENSSL_THREADS)
  if ((srv->mutex = ossl_crypto_mutex_new()) == NULL) goto err;
#endif
...
err:
  if (srv != NULL) {
    if (args->ctx == NULL) SSL_CTX_free(srv->ctx);
    SSL_free(srv->tls);
...
int ossl_quic_tserver_tick(QUIC_TSERVER* srv) {
  ...
  if (ossl_quic_channel_is_active(srv->ch)) srv->connected = 1;
...
const QUIC_TERMINATE_CAUSE* ossl_quic_tserver_get_terminate_cause(
    const QUIC_TSERVER* srv) {
...
int ossl_quic_tserver_read(QUIC_TSERVER* srv, uint64_t stream_id,
  ...
  if (qs == NULL) {
     ...
     * Otherwise, fail.
     */
    if (!is_client_init || !ossl_quic_channel_is_active(srv->ch)) return 0;
...
int ossl_quic_tserver_has_read_ended(QUIC_TSERVER* srv, uint64_t stream_id) {
  ...
  if (qs->recv_state == QUIC_RSTREAM_STATE_DATA_READ) return 1;

  if (!ossl_quic_stream_has_recv_buffer(qs)) return 0;
...
int ossl_quic_tserver_write(QUIC_TSERVER* srv, uint64_t stream_id,
  ...
  if (!ossl_quic_channel_is_active(srv->ch)) return 0;
...
int ossl_quic_tserver_shutdown(QUIC_TSERVER* srv, uint64_t app_error_code) {
  ...
  if (ossl_quic_channel_is_terminated(srv->ch)) return 1;
...
int ossl_quic_tserver_ping(QUIC_TSERVER* srv) {
  if (ossl_quic_channel_is_terminated(srv->ch)) return 0;
