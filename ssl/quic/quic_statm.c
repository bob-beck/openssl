...
void
ossl_statm_update_rtt (OSSL_STATM *statm, OSSL_TIME ack_delay,
                       OSSL_TIME override_latest_rtt)
{
  ...
  statm->smoothed_rtt = ossl_time_divide (
      ossl_time_add (ossl_time_multiply (statm->smoothed_rtt, 7),
                     adjusted_rtt),
      8);
}
...
int
ossl_statm_init (OSSL_STATM *statm)
{
...
void
ossl_statm_destroy (OSSL_STATM *statm)
{
  /* No-op. */
}

void
ossl_statm_get_rtt_info (OSSL_STATM *statm, OSSL_RTT_INFO *rtt_info)
{
