...
lcid_hash (const QUIC_LCID *lcid_obj)
{
  SIPHASH siphash = {
    0,
  };
...
static int
lcid_comp (const QUIC_LCID *a, const QUIC_LCID *b)
{
  return !ossl_quic_conn_id_eq (&a->cid, &b->cid);
}

static unsigned long
lcidm_conn_hash (const QUIC_LCIDM_CONN *conn)
{
  return (unsigned long)(uintptr_t)conn->opaque;
}

static int
lcidm_conn_comp (const QUIC_LCIDM_CONN *a, const QUIC_LCIDM_CONN *b)
{
  return a->opaque != b->opaque;
}

QUIC_LCIDM *
ossl_quic_lcidm_new (OSSL_LIB_CTX *libctx, size_t lcid_len)
{
...
static void
lcidm_delete_conn_ (QUIC_LCIDM_CONN *conn, void *arg)
{
  lcidm_delete_conn ((QUIC_LCIDM *)arg, conn);
}

void
ossl_quic_lcidm_free (QUIC_LCIDM *lcidm)
{
  if (lcidm == NULL)
    return;
...
static QUIC_LCID *
lcidm_get0_lcid (const QUIC_LCIDM *lcidm, const QUIC_CONN_ID *lcid)
{
  QUIC_LCID key;
...
static QUIC_LCIDM_CONN *
lcidm_get0_conn (const QUIC_LCIDM *lcidm, void *opaque)
{
  QUIC_LCIDM_CONN key;
...
static QUIC_LCIDM_CONN *
lcidm_upsert_conn (const QUIC_LCIDM *lcidm, void *opaque)
{
...
static void
lcidm_delete_conn_lcid (QUIC_LCIDM *lcidm, QUIC_LCID *lcid_obj)
{
...
/* doall_arg wrapper */
static void
lcidm_delete_conn_lcid_ (QUIC_LCID *lcid_obj, void *arg)
{
  lcidm_delete_conn_lcid ((QUIC_LCIDM *)arg, lcid_obj);
}

static void
lcidm_delete_conn (QUIC_LCIDM *lcidm, QUIC_LCIDM_CONN *conn)
{
...
static QUIC_LCID *
lcidm_conn_new_lcid (QUIC_LCIDM *lcidm, QUIC_LCIDM_CONN *conn,
...
size_t
ossl_quic_lcidm_get_lcid_len (const QUIC_LCIDM *lcidm)
{
  return lcidm->lcid_len;
}

size_t
ossl_quic_lcidm_get_num_active_lcid (const QUIC_LCIDM *lcidm, void *opaque)
{
  QUIC_LCIDM_CONN *conn;
...
static int
lcidm_generate_cid (QUIC_LCIDM *lcidm, QUIC_CONN_ID *cid)
{
#ifdef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
  int i;
...
static int
lcidm_generate (QUIC_LCIDM *lcidm, void *opaque, unsigned int type,
...
int
ossl_quic_lcidm_enrol_odcid (QUIC_LCIDM *lcidm, void *opaque,
...
int
ossl_quic_lcidm_generate_initial (QUIC_LCIDM *lcidm, void *opaque,
...
int
ossl_quic_lcidm_bind_channel (QUIC_LCIDM *lcidm, void *opaque,
                              const QUIC_CONN_ID *lcid)
{
  QUIC_LCIDM_CONN *conn;
  QUIC_LCID *lcid_obj;
...
int
ossl_quic_lcidm_generate (QUIC_LCIDM *lcidm, void *opaque,
...
int
ossl_quic_lcidm_retire_odcid (QUIC_LCIDM *lcidm, void *opaque)
{
  QUIC_LCIDM_CONN *conn;
...
static void
retire_for_conn (QUIC_LCID *lcid_obj, void *arg)
{
...
int
ossl_quic_lcidm_retire (QUIC_LCIDM *lcidm, void *opaque,
                        uint64_t retire_prior_to,
                        const QUIC_CONN_ID *containing_pkt_dcid,
                        QUIC_CONN_ID *retired_lcid, uint64_t *retired_seq_num,
                        int *did_retire)
{
...
int
ossl_quic_lcidm_cull (QUIC_LCIDM *lcidm, void *opaque)
{
  QUIC_LCIDM_CONN key, *conn;
...
int
ossl_quic_lcidm_lookup (QUIC_LCIDM *lcidm, const QUIC_CONN_ID *lcid,
...
int
ossl_quic_lcidm_debug_remove (QUIC_LCIDM *lcidm, const QUIC_CONN_ID *lcid)
{
  QUIC_LCID key, *lcid_obj;
...
int
ossl_quic_lcidm_debug_add (QUIC_LCIDM *lcidm, void *opaque,
...
int
ossl_quic_lcidm_get_unused_cid (QUIC_LCIDM *lcidm, QUIC_CONN_ID *cid)
{
  int i;
