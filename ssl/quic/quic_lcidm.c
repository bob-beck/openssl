...
ossl_quic_lcidm_new(OSSL_LIB_CTX* libctx, size_t lcid_len)
{
  ...
  if (!RAND_bytes_ex(
        libctx, (unsigned char*)&lcidm->hash_key, sizeof(uint64_t) * 2, 0))
    goto err;
...
static QUIC_LCID*
lcidm_conn_new_lcid(QUIC_LCIDM* lcidm,
                    QUIC_LCIDM_CONN* conn,
...
static int
lcidm_generate(QUIC_LCIDM* lcidm,
               void* opaque,
               unsigned int type,
...
int
ossl_quic_lcidm_enrol_odcid(QUIC_LCIDM* lcidm,
                            void* opaque,
...
int
ossl_quic_lcidm_generate_initial(QUIC_LCIDM* lcidm,
                                 void* opaque,
...
int
ossl_quic_lcidm_bind_channel(QUIC_LCIDM* lcidm,
                             void* opaque,
                             const QUIC_CONN_ID* lcid)
{
  QUIC_LCIDM_CONN* conn;
...
int
ossl_quic_lcidm_generate(QUIC_LCIDM* lcidm,
                         void* opaque,
                         OSSL_QUIC_FRAME_NEW_CONN_ID* ncid_frame)
{
  ...
  return lcidm_generate(
    lcidm, opaque, LCID_TYPE_NCID, &ncid_frame->conn_id, &ncid_frame->seq_num);
}
...
int
ossl_quic_lcidm_retire(QUIC_LCIDM* lcidm,
                       void* opaque,
...
int
ossl_quic_lcidm_lookup(QUIC_LCIDM* lcidm,
                       const QUIC_CONN_ID* lcid,
                       uint64_t* seq_num,
                       void** opaque)
{
...
int
ossl_quic_lcidm_debug_add(QUIC_LCIDM* lcidm,
                          void* opaque,
                          const QUIC_CONN_ID* lcid,
                          uint64_t seq_num)
{
