...
static unsigned long lcid_hash(const QUIC_LCID* lcid_obj) {
  ...
  if (!SipHash_set_hash_size(&siphash, sizeof(unsigned long))) goto out;
  if (!SipHash_Init(&siphash, (uint8_t*)lcid_obj->hash_key, 0, 0)) goto out;
...
QUIC_LCIDM* ossl_quic_lcidm_new(OSSL_LIB_CTX* libctx, size_t lcid_len) {
  ...
  if (lcid_len > QUIC_MAX_CONN_ID_LEN) goto err;

  if ((lcidm = OPENSSL_zalloc(sizeof(*lcidm))) == NULL) goto err;
  ...
  if ((lcidm->lcids = lh_QUIC_LCID_new(lcid_hash, lcid_comp)) == NULL) goto err;
...
static QUIC_LCID* lcidm_get0_lcid(const QUIC_LCIDM* lcidm,
  ...
  if (key.cid.id_len > QUIC_MAX_CONN_ID_LEN) return NULL;
...
static QUIC_LCIDM_CONN* lcidm_upsert_conn(const QUIC_LCIDM* lcidm,
  ...
  if ((conn = OPENSSL_zalloc(sizeof(*conn))) == NULL) goto err;

  if ((conn->lcids = lh_QUIC_LCID_new(lcid_hash, lcid_comp)) == NULL) goto err;

  conn->opaque = opaque;

  lh_QUIC_LCIDM_CONN_insert(lcidm->conns, conn);
  if (lh_QUIC_LCIDM_CONN_error(lcidm->conns)) goto err;
...
static QUIC_LCID* lcidm_conn_new_lcid(QUIC_LCIDM* lcidm, QUIC_LCIDM_CONN* conn,
  ...
  if (lcid->id_len > QUIC_MAX_CONN_ID_LEN) return NULL;

  if ((lcid_obj = OPENSSL_zalloc(sizeof(*lcid_obj))) == NULL) goto err;
  ...
  if (lh_QUIC_LCID_error(conn->lcids)) goto err;
...
static int lcidm_generate_cid(QUIC_LCIDM* lcidm, QUIC_CONN_ID* cid) {
  ...
  *cid = lcidm->next_lcid;
  ...
  for (i = lcidm->lcid_len - 1; i >= 0; --i)
    if (++lcidm->next_lcid.id[i] != 0) break;
...
#define MAX_RETRIES 8
  ...
  if ((conn = lcidm_upsert_conn(lcidm, opaque)) == NULL) return 0;
  ...
  if ((lcid_obj = lcidm_conn_new_lcid(lcidm, conn, lcid_out)) == NULL) return 0;

  lcid_obj->seq_num = conn->next_seq_num;
  lcid_obj->type = type;

  if (seq_num != NULL) *seq_num = lcid_obj->seq_num;
...
int ossl_quic_lcidm_enrol_odcid(QUIC_LCIDM* lcidm, void* opaque,
  ...
  key.hash_key = lcidm->hash_key;
  if (lh_QUIC_LCID_retrieve(lcidm->lcids, &key) != NULL) return 0;
...
int ossl_quic_lcidm_bind_channel(QUIC_LCIDM* lcidm, void* opaque,
   ...
   */
  if (ossl_quic_lcidm_lookup(lcidm, lcid, NULL, NULL) != 0) return 0;
...
int ossl_quic_lcidm_retire(QUIC_LCIDM* lcidm, void* opaque,
  ...
  *did_retire = 0;
  ...
  if (args.earliest_seq_num_lcid_obj == NULL) return 1;
  ...
  *did_retire = 1;
  if (retired_lcid != NULL) *retired_lcid = args.earliest_seq_num_lcid_obj->cid;
...
int ossl_quic_lcidm_lookup(QUIC_LCIDM* lcidm, const QUIC_CONN_ID* lcid,
  ...
  if (lcid == NULL) return 0;

  if ((lcid_obj = lcidm_get0_lcid(lcidm, lcid)) == NULL) return 0;

  if (seq_num != NULL) *seq_num = lcid_obj->seq_num;

  if (opaque != NULL) *opaque = lcid_obj->conn->opaque;
...
int ossl_quic_lcidm_debug_remove(QUIC_LCIDM* lcidm, const QUIC_CONN_ID* lcid) {
  ...
  key.cid = *lcid;
  key.hash_key = lcidm->hash_key;
  if ((lcid_obj = lh_QUIC_LCID_retrieve(lcidm->lcids, &key)) == NULL) return 0;
...
int ossl_quic_lcidm_debug_add(QUIC_LCIDM* lcidm, void* opaque,
  ...
  if (lcid == NULL || lcid->id_len > QUIC_MAX_CONN_ID_LEN) return 0;
