...
QUIC_DEMUX *
ossl_quic_demux_new (BIO *net_bio, size_t short_conn_id_len,
...
static void
demux_free_urxl (QUIC_URXE_LIST *l)
{
  QUIC_URXE *e, *enext;
...
void
ossl_quic_demux_free (QUIC_DEMUX *demux)
{
  if (demux == NULL)
    return;
...
void
ossl_quic_demux_set_bio (QUIC_DEMUX *demux, BIO *net_bio)
{
  unsigned int mtu;
...
int
ossl_quic_demux_set_mtu (QUIC_DEMUX *demux, unsigned int mtu)
{
  if (mtu < QUIC_MIN_INITIAL_DGRAM_LEN)
    return 0;
...
void
ossl_quic_demux_set_default_handler (QUIC_DEMUX *demux,
...
static QUIC_URXE *
demux_alloc_urxe (size_t alloc_len)
{
  QUIC_URXE *e;
...
static QUIC_URXE *
demux_resize_urxe (QUIC_DEMUX *demux, QUIC_URXE *e, size_t new_alloc_len)
{
  QUIC_URXE *e2, *prev;
...
static QUIC_URXE *
demux_reserve_urxe (QUIC_DEMUX *demux, QUIC_URXE *e, size_t alloc_len)
{
...
static int
demux_ensure_free_urxe (QUIC_DEMUX *demux, size_t min_num_free)
{
  QUIC_URXE *e;
...
static int
demux_identify_conn_id (QUIC_DEMUX *demux, QUIC_URXE *e,
 ...
 */
static int
demux_process_pending_urxe (QUIC_DEMUX *demux, QUIC_URXE *e)
{
  QUIC_CONN_ID dst_conn_id;
...
static int
demux_process_pending_urxl (QUIC_DEMUX *demux)
{
  QUIC_URXE *e;
  int ret;
 ...
 */
int
ossl_quic_demux_pump (QUIC_DEMUX *demux)
{
  int ret;
...
int
ossl_quic_demux_inject (QUIC_DEMUX *demux, const unsigned char *buf,
...
void
ossl_quic_demux_release_urxe (QUIC_DEMUX *demux, QUIC_URXE *e)
{
...
void
ossl_quic_demux_reinject_urxe (QUIC_DEMUX *demux, QUIC_URXE *e)
{
  assert (ossl_list_urxe_prev (e) == NULL && ossl_list_urxe_next (e) == NULL);
  assert (e->demux_state == URXE_DEMUX_STATE_ISSUED);
...
int
ossl_quic_demux_has_pending (const QUIC_DEMUX *demux)
{
