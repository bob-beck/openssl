...
QUIC_DEMUX*
ossl_quic_demux_new(BIO* net_bio,
                    size_t short_conn_id_len,
                    OSSL_TIME (*now)(void* arg),
                    void* now_arg)
{
...
static int
demux_identify_conn_id(QUIC_DEMUX* demux,
                       QUIC_URXE* e,
...
demux_process_pending_urxe(QUIC_DEMUX* demux, QUIC_URXE* e)
{
  ...
  if (demux->default_cb != NULL) {
    ...
    demux->default_cb(
      e, demux->default_cb_arg, dst_conn_id_ok ? &dst_conn_id : NULL);
  } else {
    /* Discard. */
    ossl_list_urxe_insert_tail(&demux->urx_free, e);
    e->demux_state = URXE_DEMUX_STATE_FREE;
...
int
ossl_quic_demux_inject(QUIC_DEMUX* demux,
