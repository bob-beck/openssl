...
OSSL_QRL_ENC_LEVEL* ossl_qrl_enc_level_set_get(OSSL_QRL_ENC_LEVEL_SET* els,
  ...
  if (!ossl_assert(enc_level < QUIC_ENC_LEVEL_NUM)) return NULL;

  el = &els->el[enc_level];

  if (require_prov) switch (el->state) {
...
int ossl_qrl_enc_level_set_has_keyslot(OSSL_QRL_ENC_LEVEL_SET* els,
  ...
  if (!ossl_assert(el != NULL && keyslot < 2)) return 0;
...
err:
  ...
  if (have_ks0) el_teardown_keyslot(els, enc_level, init_keyslot);
  if (have_ks1) el_teardown_keyslot(els, enc_level, !init_keyslot);
  if (own_md) EVP_MD_free(md);
  return 0;
}
...
void ossl_qrl_enc_level_set_discard(OSSL_QRL_ENC_LEVEL_SET* els,
  ...
  if (el == NULL || el->state == QRL_EL_STATE_DISCARDED) return;
