...
static void
ackm_on_pkts_lost(OSSL_ACKM* ackm,
                  int pkt_space,
...
OSSL_ACKM*
ossl_ackm_new(OSSL_TIME (*now)(void* arg),
              void* now_arg,
              OSSL_STATM* statm,
              const OSSL_CC_METHOD* cc_method,
              OSSL_CC_DATA* cc_data,
...
static void
ackm_process_ecn(OSSL_ACKM* ackm, const OSSL_QUIC_FRAME_ACK* ack, int pkt_space)
{
  struct tx_pkt_history_st* h;
...
int
ossl_ackm_on_rx_ack_frame(OSSL_ACKM* ackm,
                          const OSSL_QUIC_FRAME_ACK* ack,
                          int pkt_space,
                          OSSL_TIME rx_time)
{
  ...
  if (na_pkts->pkt_num == ack->ack_ranges[0].end &&
    ...
    ossl_statm_update_rtt(
      ackm->statm, ack_delay, ossl_time_subtract(now, na_pkts->time));
  }
...
static void
ackm_on_rx_ack_eliciting(OSSL_ACKM* ackm,
                         OSSL_TIME rx_time,
                         int pkt_space,
                         int was_missing)
{
  ...
  if (ossl_time_is_infinite(ackm->rx_ack_flush_deadline[pkt_space]))
    ackm_set_flush_deadline(
      ackm, pkt_space, ossl_time_add(rx_time, tx_max_ack_delay));
  else
    ackm_set_flush_deadline(
      ackm,
      pkt_space,
...
static void
ackm_fill_rx_ack_ranges(OSSL_ACKM* ackm,
                        int pkt_space,
...
void
ossl_ackm_set_loss_detection_deadline_callback(OSSL_ACKM* ackm,
