 ...
 * *****************
 *
 * The TX Packet History object tracks information about packets which have
 * been sent for which we later expect to receive an ACK. It is essentially a
 * simple database keeping a list of packet information structures in packet
 * number order which can also be looked up directly by packet number.
 *
 ...
 * we should not currently need more general functionality such as a sorted
 * list insert.
 */
...
tx_pkt_info_hash (const OSSL_ACKM_TX_PKT *pkt)
{
...
static int
tx_pkt_info_compare (const OSSL_ACKM_TX_PKT *a, const OSSL_ACKM_TX_PKT *b)
{
...
static int
tx_pkt_history_add_actual (struct tx_pkt_history_st *h, OSSL_ACKM_TX_PKT *pkt)
{
...
static int
tx_pkt_history_add (struct tx_pkt_history_st *h, OSSL_ACKM_TX_PKT *pkt)
{
 ...
 * store a set of packet numbers which we have received but which we do not
 * know for a fact that the transmitter knows we have received.
 *
 ...
 *      immediately know that the ACK was lost and the transmitter does not
 * know that we have received the packet.
 *
 ...
 *      transmitter. The transmitter subsequently sends us an ACK which
 * confirms its receipt of the ACK we sent, and we successfully receive that
 * ACK, so we know that the transmitter knows, that we received the original
 *      packet.
 *
 * Only when we reach case (4) are we relieved of any need to track a given
 * packet number we have received, because only in this case do we know for
 * sure that the peer knows we have received the packet. Having reached case
 * (4) we will never again need to generate an ACK containing the PN in
 * question, but until we reach that point, we must keep track of the PN as not
 * having been provably ACKed, as we may have to keep generating ACKs for the
 * given PN not just until the transmitter receives one, but until we know that
 * it has received one. This will be referred to herein as "provably ACKed".
 *
 * **Duplicate handling.** The above discusses the case where we have received
 * a packet with a given PN but are at best unsure whether the sender knows we
 ...
 *     (if we receive such a PN again, it must be ignored)
 *
 * However, if we were to track this state for every PN ever used in the
 * history of a connection, the amount of state required would increase
 * unboundedly as the connection goes on (for example, we would have to store a
 * set of every PN ever received.)
 *
 ...
 *    minimum packet number that increases as ranges are discarded is one way
 * to achieve this with minimal state."
 *
 * This touches on a subtlety of the original requirement quoted above: the
 * receiver MUST discard a packet unless it is certain that it has not
 * processed another packet with the same PN. However, this does not forbid the
 * receiver from also discarding some PNs even though it has not yet processed
 * them. In other words, implementations must be conservative and err in the
 * direction of assuming a packet is a duplicate, but it is acceptable for this
 * to come at the cost of falsely identifying some packets as duplicates.
 *
 ...
 * Thus the PNs the state for which needs to be explicitly tracked is limited
 * to only a small number of recent PNs, and all older PNs have an assumed
 * state.
 *
 ...
 *
 * Note that state (C) corresponds to both "we have processed this PN and it
 * has been provably ACKed" logical state and a subset of the PNs in the "we
 * have never processed this PN before" logical state (namely all PNs which
 * were lost and never received, but which are not recent enough to be above
 * the watermark). The reason we can merge these states and avoid tracking
 * states for the PNs in this state is because the provably ACKed and
 * never-received states are functionally identical in terms of how we need to
 * handle them: we don't need to do anything for PNs in either of these states,
 * so we don't have to care about PNs in this state nor do we have to care
 * about distinguishing the two states for a given PN.
 *
 * Note that under this scheme provably ACKed PNs are by definition always
 * below the watermark; therefore, it follows that when a PN becomes provably
 * ACKed, the watermark must be immediately increased to exceed it (otherwise
 * we would keep reporting it in future ACK frames).
 *
 ...
 * is acknowledged, the receiver can stop acknowledging packets less than or
 *    equal to the Largest Acknowledged field in the sent ACK frame."
 *
 * This is where our scheme's false positives arise. When a packet containing
 * an ACK frame is itself ACK'd, PNs referenced in that ACK frame become
 * provably acked, and the watermark is bumped accordingly. However, the
 * Largest Acknowledged field does not imply that all lower PNs have been
 * received, because there may be gaps expressed in the ranges of PNs expressed
 * by that and previous ACK frames. Thus, some unreceived PNs may be moved
 * below the watermark, and we may subsequently reject those PNs as possibly
 * being duplicates even though we have not actually received those PNs. Since
 * we bump the watermark when a PN becomes provably ACKed, it follows that an
 * unreceived PN falls below the watermark (and thus becomes a false positive
 * for the purposes of duplicate detection) when a higher-numbered PN becomes
 * provably ACKed.
 *
 ...
 *
 * **Data structure.** Our state for the RX handling side of the ACK manager,
 * as discussed above, mainly comprises:
 *
 ...
 * PN is in state (C) if it is below the watermark; otherwise it is in state
 * (B) if it is in the logical set of PNs, and in state (A) otherwise.
 *
 * Note that PNs are only removed from the PN set (when they become provably
 * ACKed or written off) by virtue of advancement of the watermark. Removing
 * PNs from the PN set any other way would be ambiguous as it would be
 * indistinguishable from a PN we have not yet received and risk us processing
 * a duplicate packet. In other words, for a given PN:
 *
 ...
 *
 * **Possible duplicates.** A PN is considered a possible duplicate when
 * either:
 *
...
static void
rx_pkt_history_init (struct rx_pkt_history_st *h)
{
...
static void
rx_pkt_history_destroy (struct rx_pkt_history_st *h)
{
...
static void
rx_pkt_history_trim_range_count (struct rx_pkt_history_st *h)
{
...
static int
rx_pkt_history_add_pn (struct rx_pkt_history_st *h, QUIC_PN pn)
{
  UINT_RANGE r;
...
static int
rx_pkt_history_bump_watermark (struct rx_pkt_history_st *h, QUIC_PN watermark)
{
  UINT_RANGE r;
 ...
 * discarded.
 */
static struct tx_pkt_history_st *
get_tx_history (OSSL_ACKM *ackm, int pkt_space)
{
 ...
 * discarded.
 */
static struct rx_pkt_history_st *
get_rx_history (OSSL_ACKM *ackm, int pkt_space)
{
  assert (!ackm->discarded[pkt_space]);
...
static int
ack_includes_ack_eliciting (OSSL_ACKM_TX_PKT *pkt)
{
...
static uint64_t
ackm_ack_eliciting_bytes_in_flight (OSSL_ACKM *ackm)
{
  int i;
  uint64_t total = 0;
...
static int
range_contains (const OSSL_QUIC_ACK_RANGE *range, QUIC_PN pn)
{
 ...
 */
static OSSL_ACKM_TX_PKT *
ackm_detect_and_remove_newly_acked_pkts (OSSL_ACKM *ackm,
 ...
 */
static OSSL_ACKM_TX_PKT *
ackm_detect_and_remove_lost_pkts (OSSL_ACKM *ackm, int pkt_space)
{
...
static OSSL_TIME
ackm_get_loss_time_and_space (OSSL_ACKM *ackm, int *pspace)
{
...
static OSSL_TIME
ackm_get_pto_time_and_space (OSSL_ACKM *ackm, int *space)
{
  OSSL_RTT_INFO rtt;
  OSSL_TIME duration;
  ...
  for (i = QUIC_PN_SPACE_INITIAL; i < QUIC_PN_SPACE_NUM; ++i)
    {
       ...
       * If stack has sent no ACK eliciting frame at given encryption level
       * then particular timer is zero and we must not attempt to set it. Timer
       * keeps time since epoch (Jan 1 1970) and we must not set timer to past.
       */
      if (!ossl_time_is_zero (ackm->time_of_last_ack_eliciting_pkt[i]))
        {
          t = ossl_time_add (ackm->time_of_last_ack_eliciting_pkt[i],
                             duration);
...
static void
ackm_set_loss_detection_timer_actual (OSSL_ACKM *ackm, OSSL_TIME deadline)
{
...
static int
ackm_set_loss_detection_timer (OSSL_ACKM *ackm)
{
  int space;
...
static int
ackm_in_persistent_congestion (OSSL_ACKM *ackm, const OSSL_ACKM_TX_PKT *lpkt)
{
...
static void
ackm_on_pkts_lost (OSSL_ACKM *ackm, int pkt_space,
...
static void
ackm_on_pkts_acked (OSSL_ACKM *ackm, const OSSL_ACKM_TX_PKT *apkt)
{
...
OSSL_ACKM *
ossl_ackm_new (OSSL_TIME (*now) (void *arg), void *now_arg, OSSL_STATM *statm,
...
int
ossl_ackm_on_tx_packet (OSSL_ACKM *ackm, OSSL_ACKM_TX_PKT *pkt)
{
...
int
ossl_ackm_on_rx_datagram (OSSL_ACKM *ackm, size_t num_bytes)
{
  /* No-op on the client. */
  return 1;
}
...
static void
ackm_process_ecn (OSSL_ACKM *ackm, const OSSL_QUIC_FRAME_ACK *ack,
                  int pkt_space)
{
  struct tx_pkt_history_st *h;
...
int
ossl_ackm_on_rx_ack_frame (OSSL_ACKM *ackm, const OSSL_QUIC_FRAME_ACK *ack,
...
int
ossl_ackm_on_pkt_space_discarded (OSSL_ACKM *ackm, int pkt_space)
{
...
int
ossl_ackm_on_handshake_confirmed (OSSL_ACKM *ackm)
{
...
static void
ackm_queue_probe_anti_deadlock_handshake (OSSL_ACKM *ackm)
{
  ++ackm->pending_probe.anti_deadlock_handshake;
}

static void
ackm_queue_probe_anti_deadlock_initial (OSSL_ACKM *ackm)
{
  ++ackm->pending_probe.anti_deadlock_initial;
}

static void
ackm_queue_probe (OSSL_ACKM *ackm, int pkt_space)
{
  /*
...
int
ossl_ackm_on_timeout (OSSL_ACKM *ackm)
{
  int pkt_space;
...
OSSL_TIME
ossl_ackm_get_loss_detection_deadline (OSSL_ACKM *ackm)
{
  return ackm->loss_detection_deadline;
}

OSSL_ACKM_PROBE_INFO *
ossl_ackm_get0_probe_request (OSSL_ACKM *ackm)
{
  return &ackm->pending_probe;
}

int
ossl_ackm_get_largest_unacked (OSSL_ACKM *ackm, int pkt_space, QUIC_PN *pn)
{
  struct tx_pkt_history_st *h;
  OSSL_ACKM_TX_PKT *p;
 ...
 */
int
ossl_ackm_is_ack_desired (OSSL_ACKM *ackm, int pkt_space)
{
 ...
 */
static int
ack_contains (const OSSL_QUIC_FRAME_ACK *ack, QUIC_PN pkt_num)
{
  size_t i;
 ...
 */
static int
ackm_is_missing (OSSL_ACKM *ackm, int pkt_space, QUIC_PN pkt_num)
{
  /*
 ...
 */
static int
ackm_has_newly_missing (OSSL_ACKM *ackm, int pkt_space)
{
...
static void
ackm_set_flush_deadline (OSSL_ACKM *ackm, int pkt_space, OSSL_TIME deadline)
{
...
static void
ackm_queue_ack (OSSL_ACKM *ackm, int pkt_space)
{
...
static void
ackm_on_rx_ack_eliciting (OSSL_ACKM *ackm, OSSL_TIME rx_time, int pkt_space,
                          int was_missing)
{
  OSSL_TIME tx_max_ack_delay;
...
int
ossl_ackm_on_rx_packet (OSSL_ACKM *ackm, const OSSL_ACKM_RX_PKT *pkt)
{
...
static void
ackm_fill_rx_ack_ranges (OSSL_ACKM *ackm, int pkt_space,
...
ossl_ackm_get_ack_frame (OSSL_ACKM *ackm, int pkt_space)
{
...
OSSL_TIME
ossl_ackm_get_ack_deadline (OSSL_ACKM *ackm, int pkt_space)
{
  if (ackm->rx_ack_desired[pkt_space])
...
int
ossl_ackm_is_rx_pn_processable (OSSL_ACKM *ackm, QUIC_PN pn, int pkt_space)
{
  struct rx_pkt_history_st *h = get_rx_history (ackm, pkt_space);
...
void
ossl_ackm_set_ack_deadline_callback (OSSL_ACKM *ackm,
...
int
ossl_ackm_mark_packet_pseudo_lost (OSSL_ACKM *ackm, int pkt_space, QUIC_PN pn)
{
...
OSSL_TIME
ossl_ackm_get_pto_duration (OSSL_ACKM *ackm)
{
  OSSL_TIME duration;
  OSSL_RTT_INFO rtt;
...
QUIC_PN
ossl_ackm_get_largest_acked (OSSL_ACKM *ackm, int pkt_space)
{
  return ackm->largest_acked_pkt[pkt_space];
}

void
ossl_ackm_set_rx_max_ack_delay (OSSL_ACKM *ackm, OSSL_TIME rx_max_ack_delay)
{
