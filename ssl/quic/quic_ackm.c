...
static int tx_pkt_info_compare(const OSSL_ACKM_TX_PKT* a,
  ...
  if (a->pkt_num < b->pkt_num) return -1;
  if (a->pkt_num > b->pkt_num) return 1;
  return 0;
}
...
static int tx_pkt_history_add_actual(struct tx_pkt_history_st* h,
  ...
  if (!ossl_assert(existing == NULL)) return 0;
...
static int tx_pkt_history_add(struct tx_pkt_history_st* h,
                              OSSL_ACKM_TX_PKT* pkt) {
  if (!ossl_assert(pkt->pkt_num >= h->watermark)) return 0;

  if (tx_pkt_history_add_actual(h, pkt) < 1) return 0;
...
static void rx_pkt_history_trim_range_count(struct rx_pkt_history_st* h) {
   ...
   */
  if (highest != QUIC_PN_INVALID) rx_pkt_history_bump_watermark(h, highest + 1);
}
...
static int rx_pkt_history_add_pn(struct rx_pkt_history_st* h, QUIC_PN pn) {
  ...
  if (pn < h->watermark) return 1; /* consider this a success case */

  if (ossl_uint_set_insert(&h->set, &r) != 1) return 0;
...
static int rx_pkt_history_bump_watermark(struct rx_pkt_history_st* h,
  ...
  if (watermark <= h->watermark) return 1;

  /* Remove existing PNs below the watermark. */
  r.start = 0;
  r.end = watermark - 1;
  if (ossl_uint_set_remove(&h->set, &r) != 1) return 0;
...
static int ack_includes_ack_eliciting(OSSL_ACKM_TX_PKT* pkt) {
  for (; pkt != NULL; pkt = pkt->anext)
    if (pkt->is_ack_eliciting) return 1;
...
static OSSL_ACKM_TX_PKT* ackm_detect_and_remove_newly_acked_pkts(
  ...
  if (pkt == NULL) pkt = ossl_list_tx_history_tail(&h->packets);
...
static OSSL_ACKM_TX_PKT* ackm_detect_and_remove_lost_pkts(OSSL_ACKM* ackm,
  ...
  for (; pkt != NULL; pkt = pnext) {
    ...
    if (pkt->pkt_num > ackm->largest_acked_pkt[pkt_space]) continue;
...
static OSSL_TIME ackm_get_pto_time_and_space(OSSL_ACKM* ackm, int* space) {
  ...
  for (i = QUIC_PN_SPACE_INITIAL; i < QUIC_PN_SPACE_NUM; ++i) {
    ...
    if (i == QUIC_PN_SPACE_APP) {
      ...
      if (!ackm->handshake_confirmed) break;
...
static void ackm_on_pkts_lost(OSSL_ACKM* ackm, int pkt_space,
  ...
  for (p = lpkt; p != NULL; p = pnext) {
    ...
    if (p->is_inflight) {
      ...
      if (p->pkt_num > largest_pn_lost) largest_pn_lost = p->pkt_num;
...
static void ackm_on_pkts_acked(OSSL_ACKM* ackm, const OSSL_ACKM_TX_PKT* apkt) {
  ...
  for (; apkt != NULL; apkt = anext) {
    if (apkt->is_inflight) {
      ...
      if (apkt->pkt_num > last_pn_acked) last_pn_acked = apkt->pkt_num;
...
OSSL_ACKM* ossl_ackm_new(OSSL_TIME (*now)(void* arg), void* now_arg,
  ...
  for (i = 0; i < (int)OSSL_NELEM(ackm->tx_history); ++i) {
    ...
    if (tx_pkt_history_init(&ackm->tx_history[i]) < 1) goto err;
  }
...
int ossl_ackm_on_tx_packet(OSSL_ACKM* ackm, OSSL_ACKM_TX_PKT* pkt) {
  ...
  if (pkt->num_bytes == 0) return 0;

  /* Does not make any sense for a non-in-flight packet to be ACK-eliciting. */
  if (!pkt->is_inflight && pkt->is_ack_eliciting) return 0;
...
int ossl_ackm_on_rx_ack_frame(OSSL_ACKM* ackm, const OSSL_QUIC_FRAME_ACK* ack,
  ...
  if (na_pkts == NULL) {
    if (must_set_timer) ackm_set_loss_detection_timer(ackm);
  ...
  if (na_pkts->pkt_num == ack->ack_ranges[0].end &&
    ...
    if (ossl_time_is_zero(ackm->first_rtt_sample)) ackm->first_rtt_sample = now;
   ...
   */
  if (ack->ecn_present) ackm_process_ecn(ackm, ack, pkt_space);
   ...
   * client's address.
   */
  if (ackm->peer_completed_addr_validation) ackm->pto_count = 0;
...
int ossl_ackm_on_pkt_space_discarded(OSSL_ACKM* ackm, int pkt_space) {
  ...
  if (ackm->discarded[pkt_space]) return 0;
...
static int ack_contains(const OSSL_QUIC_FRAME_ACK* ack, QUIC_PN pkt_num) {
  ...
  for (i = 0; i < ack->num_ack_ranges; ++i)
    if (range_contains(&ack->ack_ranges[i], pkt_num)) return 1;
...
static int ackm_has_newly_missing(OSSL_ACKM* ackm, int pkt_space) {
  ...
  if (ossl_list_uint_set_is_empty(&h->set)) return 0;
...
int ossl_ackm_on_rx_packet(OSSL_ACKM* ackm, const OSSL_ACKM_RX_PKT* pkt) {
   ...
   * acked.
   */
  if (rx_pkt_history_add_pn(h, pkt->pkt_num) != 1) return 0;
...
OSSL_TIME ossl_ackm_get_ack_deadline(OSSL_ACKM* ackm, int pkt_space) {
  if (ackm->rx_ack_desired[pkt_space]) /* Already desired, deadline is now. */
