...
static void stream_map_mark_active(QUIC_STREAM_MAP* qsm, QUIC_STREAM* s) {
  ...
  if (qsm->rr_cur == NULL) qsm->rr_cur = s;
...
static void stream_map_mark_inactive(QUIC_STREAM_MAP* qsm, QUIC_STREAM* s) {
  ...
  if (qsm->rr_cur == s) qsm->rr_cur = active_next(&qsm->active_list, s);
  if (qsm->rr_cur == s) qsm->rr_cur = NULL;
...
int ossl_quic_stream_map_is_local_allowed_by_stream_limit(
  ...
  if (qsm->get_stream_limit_cb == NULL) return 1;
...
int ossl_quic_stream_map_reset_stream_send_part(QUIC_STREAM_MAP* qsm,
  ...
  switch (qs->send_state) {
    ...
    case QUIC_SSTREAM_STATE_READY:
      if (!ossl_quic_stream_map_ensure_send_part_id(qsm, qs)) return 0;
...
int ossl_quic_stream_map_schedule_stop_sending(QUIC_STREAM_MAP* qsm,
   ...
   */
  if (qs->want_stop_sending) return 1;
...
QUIC_STREAM* ossl_quic_stream_map_find_in_accept_queue(QUIC_STREAM_MAP* qsm,
  ...
  if (ossl_quic_stream_map_get_accept_queue_len(qsm, is_uni) == 0) return NULL;
...
static void begin_shutdown_flush_each(QUIC_STREAM* qs, void* arg) {
  ...
  if (!eligible_for_shutdown_flush(qs) || qs->shutdown_flush) return;
...
void ossl_quic_stream_iter_next(QUIC_STREAM_ITER* it) {
  ...
  if (it->stream == it->first_stream) it->stream = NULL;
}
