...
static void
shutdown_flush_done(QUIC_STREAM_MAP* qsm, QUIC_STREAM* qs);

/* Circular list management. */
static void
list_insert_tail(QUIC_STREAM_LIST_NODE* l, QUIC_STREAM_LIST_NODE* n)
{
  /* Must not be in list. */
...
static void
list_remove(QUIC_STREAM_LIST_NODE* l, QUIC_STREAM_LIST_NODE* n)
{
...
static QUIC_STREAM*
list_next(QUIC_STREAM_LIST_NODE* l, QUIC_STREAM_LIST_NODE* n, size_t off)
{
...
#define ready_for_gc_next(l, s)                                                \
  list_next(                                                                   \
    (l), &(s)->ready_for_gc_node, offsetof(QUIC_STREAM, ready_for_gc_node))
...
hash_stream(const QUIC_STREAM* s)
{
  return (unsigned long)s->id;
}

static int
cmp_stream(const QUIC_STREAM* a, const QUIC_STREAM* b)
{
  if (a->id < b->id)
    return -1;
  if (a->id > b->id)
...
int
ossl_quic_stream_map_init(QUIC_STREAM_MAP* qsm,
...
static void
release_each(QUIC_STREAM* stream, void* arg)
{
...
void
ossl_quic_stream_map_cleanup(QUIC_STREAM_MAP* qsm)
{
...
void
ossl_quic_stream_map_visit(QUIC_STREAM_MAP* qsm,
...
QUIC_STREAM*
ossl_quic_stream_map_alloc(QUIC_STREAM_MAP* qsm, uint64_t stream_id, int type)
{
  QUIC_STREAM* s;
  QUIC_STREAM key;
...
void
ossl_quic_stream_map_release(QUIC_STREAM_MAP* qsm, QUIC_STREAM* stream)
{
  if (stream == NULL)
    return;
...
QUIC_STREAM*
ossl_quic_stream_map_get_by_id(QUIC_STREAM_MAP* qsm, uint64_t stream_id)
{
  QUIC_STREAM key;
...
static void
stream_map_mark_active(QUIC_STREAM_MAP* qsm, QUIC_STREAM* s)
{
  if (s->active)
    return;
...
static void
stream_map_mark_inactive(QUIC_STREAM_MAP* qsm, QUIC_STREAM* s)
{
  if (!s->active)
    return;
...
void
ossl_quic_stream_map_set_rr_stepping(QUIC_STREAM_MAP* qsm, size_t stepping)
{
...
static int
stream_has_data_to_send(QUIC_STREAM* s)
{
...
qsm_send_part_permits_gc(const QUIC_STREAM* qs)
{
...
static int
qsm_ready_for_gc(QUIC_STREAM_MAP* qsm, QUIC_STREAM* qs)
{
...
int
ossl_quic_stream_map_is_local_allowed_by_stream_limit(QUIC_STREAM_MAP* qsm,
...
void
ossl_quic_stream_map_update_state(QUIC_STREAM_MAP* qsm, QUIC_STREAM* s)
{
  int should_be_active, allowed_by_stream_limit = 1;

  if (ossl_quic_stream_is_server_init(s) ==
      ossl_quic_channel_is_server(qsm->ch)) {
...
int
ossl_quic_stream_map_ensure_send_part_id(QUIC_STREAM_MAP* qsm, QUIC_STREAM* qs)
{
  switch (qs->send_state) {
    case QUIC_SSTREAM_STATE_NONE:
...
int
ossl_quic_stream_map_notify_all_data_sent(QUIC_STREAM_MAP* qsm, QUIC_STREAM* qs)
{
  switch (qs->send_state) {
    default:
...
int
ossl_quic_stream_map_notify_totally_acked(QUIC_STREAM_MAP* qsm, QUIC_STREAM* qs)
{
  switch (qs->send_state) {
    default:
      /* Wrong state - caller error. */
...
int
ossl_quic_stream_map_reset_stream_send_part(QUIC_STREAM_MAP* qsm,
                                            QUIC_STREAM* qs,
                                            uint64_t aec)
{
  switch (qs->send_state) {
...
int
ossl_quic_stream_map_notify_reset_stream_acked(QUIC_STREAM_MAP* qsm,
                                               QUIC_STREAM* qs)
{
  switch (qs->send_state) {
    default:
...
int
ossl_quic_stream_map_notify_size_known_recv_part(QUIC_STREAM_MAP* qsm,
                                                 QUIC_STREAM* qs,
                                                 uint64_t final_size)
{
...
int
ossl_quic_stream_map_notify_totally_received(QUIC_STREAM_MAP* qsm,
                                             QUIC_STREAM* qs)
{
  switch (qs->recv_state) {
    default:
...
int
ossl_quic_stream_map_notify_totally_read(QUIC_STREAM_MAP* qsm, QUIC_STREAM* qs)
{
  switch (qs->recv_state) {
    default:
      /* Wrong state - caller error. */
...
int
ossl_quic_stream_map_notify_reset_recv_part(QUIC_STREAM_MAP* qsm,
                                            QUIC_STREAM* qs,
...
int
ossl_quic_stream_map_notify_app_read_reset_recv_part(QUIC_STREAM_MAP* qsm,
                                                     QUIC_STREAM* qs)
{
  switch (qs->recv_state) {
    default:
...
int
ossl_quic_stream_map_stop_sending_recv_part(QUIC_STREAM_MAP* qsm,
                                            QUIC_STREAM* qs,
                                            uint64_t aec)
{
  if (qs->stop_sending)
...
int
ossl_quic_stream_map_schedule_stop_sending(QUIC_STREAM_MAP* qsm,
                                           QUIC_STREAM* qs)
{
  if (!qs->stop_sending)
    return 0;
...
QUIC_STREAM*
ossl_quic_stream_map_peek_accept_queue(QUIC_STREAM_MAP* qsm)
{
  return accept_head(&qsm->accept_list);
}

QUIC_STREAM*
ossl_quic_stream_map_find_in_accept_queue(QUIC_STREAM_MAP* qsm, int is_uni)
{
  QUIC_STREAM* qs;
...
void
ossl_quic_stream_map_push_accept_queue(QUIC_STREAM_MAP* qsm, QUIC_STREAM* s)
{
...
static QUIC_RXFC*
qsm_get_max_streams_rxfc(QUIC_STREAM_MAP* qsm, QUIC_STREAM* s)
{
...
void
ossl_quic_stream_map_remove_from_accept_queue(QUIC_STREAM_MAP* qsm,
                                              QUIC_STREAM* s,
                                              OSSL_TIME rtt)
{
...
size_t
ossl_quic_stream_map_get_accept_queue_len(QUIC_STREAM_MAP* qsm, int is_uni)
{
  return is_uni ? qsm->num_accept_uni : qsm->num_accept_bidi;
}

size_t
ossl_quic_stream_map_get_total_accept_queue_len(QUIC_STREAM_MAP* qsm)
{
...
void
ossl_quic_stream_map_gc(QUIC_STREAM_MAP* qsm)
{
...
static int
eligible_for_shutdown_flush(QUIC_STREAM* qs)
{
  /*
...
static void
begin_shutdown_flush_each(QUIC_STREAM* qs, void* arg)
{
  QUIC_STREAM_MAP* qsm = arg;
...
void
ossl_quic_stream_map_begin_shutdown_flush(QUIC_STREAM_MAP* qsm)
{
  qsm->num_shutdown_flush = 0;
...
int
ossl_quic_stream_map_is_shutdown_flush_finished(QUIC_STREAM_MAP* qsm)
{
 ...
 */
void
ossl_quic_stream_iter_init(QUIC_STREAM_ITER* it,
                           QUIC_STREAM_MAP* qsm,
                           int advance_rr)
{
  it->qsm = qsm;
...
void
ossl_quic_stream_iter_next(QUIC_STREAM_ITER* it)
{
  if (it->stream == NULL)
    return;
