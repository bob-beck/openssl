 ...
 * choose to introduce QDSOs. There are also roles such as Port Leader and
 * Event Leader which can be assumed by these different types under different
 ...
 * created.
 *
 * The QUIC_OBJ is a base type for QUIC APL objects which provides
 * functionality common to all QUIC objects and which supports having different
 * APL objects dynamically assume leader roles. It can therefore be seen as an
 * extension of the SSL base class and extends the SSL object for QUIC APL
 * objects. This avoids duplication of functionality for different types of
 * QUIC object and allows access to common responsibilities of different types
 * of APL object without regard to the kind of APL object we are dealing with.
 *
 ...
 * Legend:
 *
 *   *: Not currently modelled in the APL, though QUIC_ENGINE exists
 * internally.
 *
 ...
 *
 * engine: If non-NULL, this object becomes the Event Leader. parent_obj must
 * be NULL iff this is non-NULL as currently the Event Leader is always the
 * root in an APL object hierarchy. If NULL, the contextually applicable engine
 * is determined by using parent_obj and ancestors to find the Event Leader.
 *
...
/*
 * Returns a pointer to the handshake layer object which should be accessible
 * on obj for purposes of handshake API autoforwarding, if any.
 *
 * This returns NULL if a handshake layer SSL object is available but should
 * not be used for autoforwarding purposes, for example on a QSSO.
 */
...
/*
 * Return 1 if an API call directly to the given object should use blocking
 ...
 * ===================
 *
 * These inlines are expressed in terms of the core functions and inlines
 * above.
 */
