...
int
ossl_quic_txfc_init (QUIC_TXFC *txfc, QUIC_TXFC *conn_txfc)
{
...
QUIC_TXFC *
ossl_quic_txfc_get_parent (QUIC_TXFC *txfc)
{
  return txfc->parent;
}

int
ossl_quic_txfc_bump_cwm (QUIC_TXFC *txfc, uint64_t cwm)
{
  if (cwm <= txfc->cwm)
    return 0;
...
uint64_t
ossl_quic_txfc_get_credit_local (QUIC_TXFC *txfc, uint64_t consumed)
{
...
uint64_t
ossl_quic_txfc_get_credit (QUIC_TXFC *txfc, uint64_t consumed)
{
  uint64_t r, conn_r;
...
int
ossl_quic_txfc_consume_credit_local (QUIC_TXFC *txfc, uint64_t num_bytes)
{
  int ok = 1;
...
int
ossl_quic_txfc_consume_credit (QUIC_TXFC *txfc, uint64_t num_bytes)
{
...
int
ossl_quic_txfc_has_become_blocked (QUIC_TXFC *txfc, int clear)
{
...
uint64_t
ossl_quic_txfc_get_cwm (QUIC_TXFC *txfc)
{
  return txfc->cwm;
}

uint64_t
ossl_quic_txfc_get_swm (QUIC_TXFC *txfc)
{
  return txfc->swm;
}
...
int
ossl_quic_rxfc_init (QUIC_RXFC *rxfc, QUIC_RXFC *conn_rxfc,
...
int
ossl_quic_rxfc_init_standalone (QUIC_RXFC *rxfc, uint64_t initial_window_size,
...
QUIC_RXFC *
ossl_quic_rxfc_get_parent (QUIC_RXFC *rxfc)
{
  return rxfc->parent;
}

void
ossl_quic_rxfc_set_max_window_size (QUIC_RXFC *rxfc, size_t max_window_size)
{
  rxfc->max_window_size = max_window_size;
}

static void
rxfc_start_epoch (QUIC_RXFC *rxfc)
{
...
static int
on_rx_controlled_bytes (QUIC_RXFC *rxfc, uint64_t num_bytes)
{
  int ok = 1;
...
int
ossl_quic_rxfc_on_rx_stream_frame (QUIC_RXFC *rxfc, uint64_t end, int is_fin)
{
  uint64_t delta;
...
static int
rxfc_cwm_bump_desired (QUIC_RXFC *rxfc)
{
  int err = 0;
...
static int
rxfc_should_bump_window_size (QUIC_RXFC *rxfc, OSSL_TIME rtt)
{
  /*
...
static void
rxfc_adjust_window_size (QUIC_RXFC *rxfc, uint64_t min_window_size,
                         OSSL_TIME rtt)
{
...
static void
rxfc_update_cwm (QUIC_RXFC *rxfc, uint64_t min_window_size, OSSL_TIME rtt)
{
  uint64_t new_cwm;
...
static int
rxfc_on_retire (QUIC_RXFC *rxfc, uint64_t num_bytes, uint64_t min_window_size,
                OSSL_TIME rtt)
{
...
int
ossl_quic_rxfc_on_retire (QUIC_RXFC *rxfc, uint64_t num_bytes, OSSL_TIME rtt)
{
...
uint64_t
ossl_quic_rxfc_get_cwm (const QUIC_RXFC *rxfc)
{
  return rxfc->cwm;
}

uint64_t
ossl_quic_rxfc_get_swm (const QUIC_RXFC *rxfc)
{
  return rxfc->swm;
}

uint64_t
ossl_quic_rxfc_get_rwm (const QUIC_RXFC *rxfc)
{
  return rxfc->rwm;
}

uint64_t
ossl_quic_rxfc_get_credit (const QUIC_RXFC *rxfc)
{
  return ossl_quic_rxfc_get_cwm (rxfc) - ossl_quic_rxfc_get_swm (rxfc);
}

int
ossl_quic_rxfc_has_cwm_changed (QUIC_RXFC *rxfc, int clear)
{
...
int
ossl_quic_rxfc_get_error (QUIC_RXFC *rxfc, int clear)
{
...
int
ossl_quic_rxfc_get_final_size (const QUIC_RXFC *rxfc, uint64_t *final_size)
{
  if (!rxfc->is_fin)
    return 0;
