...
int ossl_quic_txfc_init(QUIC_TXFC* txfc, QUIC_TXFC* conn_txfc) {
  if (conn_txfc != NULL && conn_txfc->parent != NULL) return 0;
...
int ossl_quic_txfc_consume_credit_local(QUIC_TXFC* txfc, uint64_t num_bytes) {
  ...
  if (num_bytes > 0 && num_bytes == credit) txfc->has_become_blocked = 1;
...
int ossl_quic_txfc_consume_credit(QUIC_TXFC* txfc, uint64_t num_bytes) {
  ...
  if (txfc->parent != NULL) {
    assert(txfc->parent->parent == NULL);
    if (!ossl_quic_txfc_consume_credit_local(txfc->parent, num_bytes)) return 0;
  }
...
int ossl_quic_txfc_has_become_blocked(QUIC_TXFC* txfc, int clear) {
  ...
  if (clear) txfc->has_become_blocked = 0;
...
int ossl_quic_rxfc_init(QUIC_RXFC* rxfc, QUIC_RXFC* conn_rxfc,
  ...
  if (conn_rxfc != NULL && conn_rxfc->parent != NULL) return 0;
...
int ossl_quic_rxfc_on_rx_stream_frame(QUIC_RXFC* rxfc, uint64_t end,
  ...
  if (!rxfc->standalone && rxfc->parent == NULL) return 0;
...
static void rxfc_adjust_window_size(QUIC_RXFC* rxfc, uint64_t min_window_size,
  ...
  if (rxfc_should_bump_window_size(rxfc, rtt)) new_window_size *= 2;

  if (new_window_size < min_window_size) new_window_size = min_window_size;
  if (new_window_size >
...
static void rxfc_update_cwm(QUIC_RXFC* rxfc, uint64_t min_window_size,
  ...
  if (!rxfc_cwm_bump_desired(rxfc)) return;
...
int ossl_quic_rxfc_on_retire(QUIC_RXFC* rxfc, uint64_t num_bytes,
                             OSSL_TIME rtt) {
  if (rxfc->parent == NULL && !rxfc->standalone) return 0;
...
int ossl_quic_rxfc_has_cwm_changed(QUIC_RXFC* rxfc, int clear) {
  ...
  if (clear) rxfc->has_cwm_changed = 0;
...
int ossl_quic_rxfc_get_error(QUIC_RXFC* rxfc, int clear) {
  ...
  if (clear) rxfc->error_code = 0;

  return r;
}

int ossl_quic_rxfc_get_final_size(const QUIC_RXFC* rxfc, uint64_t* final_size) {
  if (!rxfc->is_fin) return 0;

  if (final_size != NULL) *final_size = rxfc->hwm;
