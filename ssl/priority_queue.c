...
static ossl_inline void pqueue_move_down(OSSL_PQUEUE* pq, size_t n) {
  ...
  while (n > 0) {
    ...
    ASSERT_USED(pq, p);
    if (pq->compare(h[n].data, h[p].data) >= 0) break;
    pqueue_swap_elem(pq, n, p);
    n = p;
  }
}
...
static ossl_inline void pqueue_move_up(OSSL_PQUEUE* pq, size_t n) {
  ...
  if (pq->htop > p + 1) {
    ASSERT_USED(pq, p);
    ASSERT_USED(pq, p + 1);
    if (pq->compare(h[p].data, h[p + 1].data) > 0) p++;
  }
...
int ossl_pqueue_push(OSSL_PQUEUE* pq, void* data, size_t* elem) {
  ...
  if (!ossl_pqueue_reserve(pq, 1)) return 0;
...
void* ossl_pqueue_pop(OSSL_PQUEUE* pq) {
  ...
  if (pq == NULL || pq->htop == 0) return NULL;
...
void* ossl_pqueue_remove(OSSL_PQUEUE* pq, size_t elem) {
  ...
  if (pq == NULL || elem >= pq->hmax || pq->htop == 0) return 0;
  ...
  if (n > 0) pqueue_force_bottom(pq, n);
