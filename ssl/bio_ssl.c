...
static int ssl_free(BIO* a) {
  ...
  if (BIO_get_shutdown(a)) {
    if (bs->ssl != NULL && !SSL_in_init(bs->ssl)) SSL_shutdown(bs->ssl);
    if (BIO_get_init(a)) SSL_free(bs->ssl);
...
static long ssl_ctrl(BIO* b, int cmd, long num, void* ptr) {
  ...
  next = BIO_next(b);
  ssl = bs->ssl;
  if (ssl == NULL && cmd != BIO_C_SET_SSL) return 0;
  switch (cmd) {
    case BIO_CTRL_RESET:
      /* TODO(QUIC FUTURE): Add support when SSL_clear() is supported */
      if ((sc = SSL_CONNECTION_FROM_SSL_ONLY(ssl)) == NULL) return 0;
    ...
    case BIO_C_SET_SSL_RENEGOTIATE_BYTES:
      ...
      if ((long)num >= 512) bs->renegotiate_count = (unsigned long)num;
      break;
    ...
    case BIO_C_SET_SSL:
      ...
      if (bio != NULL) {
        ...
        if (next != NULL) BIO_push(bio, next);
        BIO_set_next(b, bio);
      }
      BIO_set_init(b, 1);
      break;
    ...
    case BIO_CTRL_PENDING:
      ret = SSL_pending(ssl);
      if (ret == 0) ret = BIO_pending(SSL_get_rbio(ssl));
      break;
    case BIO_CTRL_FLUSH:
      BIO_clear_retry_flags(b);
      ret = BIO_ctrl(SSL_get_wbio(ssl), cmd, num, ptr);
    ...
    case BIO_CTRL_GET_RPOLL_DESCRIPTOR:
      if (!SSL_get_rpoll_descriptor(ssl, (BIO_POLL_DESCRIPTOR*)ptr)) ret = 0;
      break;
    case BIO_CTRL_GET_WPOLL_DESCRIPTOR:
      if (!SSL_get_wpoll_descriptor(ssl, (BIO_POLL_DESCRIPTOR*)ptr)) ret = 0;
      break;
    default:
      ret = BIO_ctrl(SSL_get_rbio(ssl), cmd, num, ptr);
      break;
...
BIO* BIO_new_buffer_ssl_connect(SSL_CTX* ctx) {
  ...
  if (ctx != NULL && IS_QUIC_CTX(ctx)) /* Never use buffering for QUIC. */
    return BIO_new_ssl_connect(ctx);
#endif

  if ((buf = BIO_new(BIO_f_buffer())) == NULL) return NULL;
  if ((ssl = BIO_new_ssl_connect(ctx)) == NULL) goto err;
  if ((ret = BIO_push(buf, ssl)) == NULL) goto err;
  return ret;
err:
  BIO_free(buf);
  BIO_free(ssl);
...
BIO* BIO_new_ssl_connect(SSL_CTX* ctx) {
  ...
  if ((con = BIO_new(BIO_s_connect())) == NULL) return NULL;

#ifndef OPENSSL_NO_QUIC
  if (ctx != NULL && IS_QUIC_CTX(ctx))
    if (!BIO_set_sock_type(con, SOCK_DGRAM)) goto err;
#endif

  if ((ssl = BIO_new_ssl(ctx, 1)) == NULL) goto err;
  if ((ret = BIO_push(ssl, con)) == NULL) goto err;
  return ret;
err:
  BIO_free(ssl);
  BIO_free(con);
...
BIO* BIO_new_ssl(SSL_CTX* ctx, int client) {
  ...
  if ((ret = BIO_new(BIO_f_ssl())) == NULL) return NULL;
...
int BIO_ssl_copy_session_id(BIO* t, BIO* f) {
  ...
  if ((tdata->ssl == NULL) || (fdata->ssl == NULL)) return 0;
...
void BIO_ssl_shutdown(BIO* b) {
  ...
  for (; b != NULL; b = BIO_next(b)) {
    if (BIO_method_type(b) != BIO_TYPE_SSL) continue;
    bdata = BIO_get_data(b);
    if (bdata != NULL && bdata->ssl != NULL) SSL_shutdown(bdata->ssl);
  }
}
