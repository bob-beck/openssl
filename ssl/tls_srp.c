...
ssl_srp_server_param_with_username_intern(SSL_CONNECTION* s, int* ad)
{
  ...
  *ad = SSL_AD_INTERNAL_ERROR;
  ...
  return ((s->srp_ctx.B = SRP_Calc_B_ex(s->srp_ctx.b,
                                        s->srp_ctx.N,
                                        s->srp_ctx.g,
                                        s->srp_ctx.v,
                                        sctx->libctx,
                            ...
                            const char* grp)
{
  ...
  sc->srp_ctx.s = NULL;
  if (!SRP_create_verifier_BN_ex(user,
                                 pass,
                                 &sc->srp_ctx.s,
                                 &sc->srp_ctx.v,
                                 sc->srp_ctx.N,
...
int
SSL_set_srp_server_param(SSL* s,
                         const BIGNUM* N,
                         const BIGNUM* g,
                         BIGNUM* sa,
                         BIGNUM* v,
...
srp_generate_server_master_secret(SSL_CONNECTION* s)
{
  ...
  if (!SRP_Verify_A_mod_N(s->srp_ctx.A, s->srp_ctx.N))
    goto err;
  if ((u = SRP_Calc_u_ex(s->srp_ctx.A,
                         s->srp_ctx.B,
                         s->srp_ctx.N,
                         sctx->libctx,
                         sctx->propq)) == NULL)
    goto err;
  if ((K = SRP_Calc_server_key(
         s->srp_ctx.A, s->srp_ctx.v, u, s->srp_ctx.b, s->srp_ctx.N)) == NULL)
    goto err;
...
srp_generate_client_master_secret(SSL_CONNECTION* s)
{
  ...
  if ((x = SRP_Calc_x_ex(
         s->srp_ctx.s, s->srp_ctx.login, passwd, sctx->libctx, sctx->propq)) ==
        NULL ||
      (K = SRP_Calc_client_key_ex(s->srp_ctx.N,
                                  s->srp_ctx.B,
                                  s->srp_ctx.g,
                                  x,
                                  s->srp_ctx.a,
...
ssl_srp_calc_a_param_intern(SSL_CONNECTION* s)
{
  ...
  if (RAND_priv_bytes_ex(
        SSL_CONNECTION_GET_CTX(s)->libctx, rnd, sizeof(rnd), 0) <= 0)
    return 0;
...
{
  return tls1_ctx_callback_ctrl(
    ctx, SSL_CTRL_SET_SRP_VERIFY_PARAM_CB, (void (*)(void))cb);
}
...
{
  return tls1_ctx_callback_ctrl(
    ctx, SSL_CTRL_SET_TLS_EXT_SRP_USERNAME_CB, (void (*)(void))cb);
}
...
{
  return tls1_ctx_callback_ctrl(
    ctx, SSL_CTRL_SET_SRP_GIVE_CLIENT_PWD_CB, (void (*)(void))cb);
}
