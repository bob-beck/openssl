...
int ssl_srp_ctx_init_intern(SSL_CONNECTION* s) {
  ...
  if (s == NULL || (ctx = SSL_CONNECTION_GET_CTX(s)) == NULL) return 0;
...
int SSL_srp_server_param_with_username(SSL* s, int* ad) {
  ...
  if (sc == NULL) return SSL3_AL_FATAL;
...
int SSL_set_srp_server_param(SSL* s, const BIGNUM* N, const BIGNUM* g,
  ...
  if (info != NULL) {
    if (sc->srp_ctx.info) OPENSSL_free(sc->srp_ctx.info);
    if ((sc->srp_ctx.info = OPENSSL_strdup(info)) == NULL) return -1;
  }
...
int srp_generate_server_master_secret(SSL_CONNECTION* s) {
  ...
  if (!SRP_Verify_A_mod_N(s->srp_ctx.A, s->srp_ctx.N)) goto err;
...
err:
  BN_clear_free(K);
  BN_clear_free(x);
  if (passwd != NULL) OPENSSL_clear_free(passwd, strlen(passwd));
  BN_clear_free(u);
  return ret;
}
...
BIGNUM* SSL_get_srp_g(SSL* s) {
  ...
  if (sc->srp_ctx.g != NULL) return sc->srp_ctx.g;
...
BIGNUM* SSL_get_srp_N(SSL* s) {
  ...
  if (sc->srp_ctx.N != NULL) return sc->srp_ctx.N;
...
char* SSL_get_srp_username(SSL* s) {
  ...
  if (sc->srp_ctx.login != NULL) return sc->srp_ctx.login;
...
char* SSL_get_srp_userinfo(SSL* s) {
  ...
  if (sc->srp_ctx.info != NULL) return sc->srp_ctx.info;
