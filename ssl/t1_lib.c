...
static int add_provider_groups(const OSSL_PARAM params[], void* data) {
  ...
  if (ginf->realname == NULL) goto err;
  ...
  if (ginf->algorithm == NULL) goto err;
...
static int add_provider_sigalgs(const OSSL_PARAM params[], void* data) {
  ...
  if (sinf->sigalg_name == NULL) goto err;
  ...
  if (p == NULL) {
    ...
    if (sinf->sigalg_oid == NULL) goto err;
  }
  ...
  if (p == NULL) {
    ...
    if (sinf->sig_name == NULL) goto err;
  }
  ...
  if (p == NULL) {
    ...
    if (sinf->hash_name == NULL) goto err;
  }
  ...
  if (p == NULL) {
    ...
    if (sinf->hash_oid == NULL) goto err;
  }
  ...
  if (p == NULL) {
    ...
    if (sinf->keytype_oid == NULL) goto err;
  }
...
int ssl_load_sigalgs(SSL_CTX* ctx) {
  ...
  if (ctx->sigalg_list_len > 0) {
    ...
    if (ctx->ssl_cert_info == NULL) return 0;
...
const TLS_GROUP_INFO* tls1_group_id_lookup(SSL_CTX* ctx, uint16_t group_id) {
  ...
  for (i = 0; i < ctx->group_list_len; i++) {
    if (ctx->group_list[i].group_id == group_id) return &ctx->group_list[i];
  }
...
const char* tls1_group_id2name(SSL_CTX* ctx, uint16_t group_id) {
  ...
  if (tls_group_info == NULL) return NULL;
...
int tls1_group_id2nid(uint16_t group_id, int include_unknown) {
  ...
  if (group_id == 0) return NID_undef;
  ...
  for (i = 0; i < OSSL_NELEM(nid_to_group); i++) {
    if (nid_to_group[i].group_id == group_id) return nid_to_group[i].nid;
  }
  if (!include_unknown) return NID_undef;
...
uint16_t tls1_nid2group_id(int nid) {
   ...
   */
  for (i = 0; i < OSSL_NELEM(nid_to_group); i++) {
    if (nid_to_group[i].nid == nid) return nid_to_group[i].group_id;
  }
...
int tls_valid_group(SSL_CONNECTION* s, uint16_t group_id, int minversion,
  ...
  if (okfortls13 != NULL) *okfortls13 = 0;
  ...
  if (group_minversion < 0 || group_maxversion < 0) return 0;
...
static int tls_group_ix_cmp(const TLS_GROUP_IX* const* a,
  ...
  if (idcmpab != idcmpba) return (idcmpba - idcmpab);
...
int tls1_get0_implemented_groups(int min_proto_version, int max_proto_version,
  ...
  if (grps == NULL || out == NULL || num > INT_MAX) return 0;
  ...
  for (ix = 0; ix < (int)num; ++ix, ++grps) {
    ...
    if ((gix = OPENSSL_malloc(sizeof(*gix))) == NULL) goto end;
    gix->grp = grps;
    gix->ix = ix;
  ...
  for (ix = 0; ix < (int)num; ++ix) {
    ...
    if (!all && gix->grp->group_id == id) continue;
    id = gix->grp->group_id;
    if (sk_OPENSSL_CSTRING_push(out, gix->grp->tlsname) <= 0) goto end;
  }
  ret = 1;
...
int tls1_set_groups(uint16_t** grpext, size_t* grpextlen, uint16_t** ksext,
  ...
  if ((glist = OPENSSL_malloc_array(ngroups, sizeof(*glist))) == NULL) goto err;
  if ((kslist = OPENSSL_malloc_array(1, sizeof(*kslist))) == NULL) goto err;
  if ((tpllist = OPENSSL_malloc_array(1, sizeof(*tpllist))) == NULL) goto err;
  for (i = 0; i < ngroups; i++) {
    unsigned long idmask;
    uint16_t id;
    id = tls1_nid2group_id(groups[i]);
    if ((id & 0x00FF) >= (sizeof(unsigned long) * 8)) goto err;
    idmask = 1L << (id & 0x00FF);
...
static int gid_cb(const char* elem, int len, void* arg) {
  ...
  while (continue_while_loop && len > 0 &&
         ((current_prefix = strchr(prefixes, elem[0])) != NULL ||
          OPENSSL_strncasecmp(
              current_prefix = (char*)DEFAULT_GROUPNAME_FIRST_CHARACTER, elem,
              1) == 0)) {
      ...
      default:
        ...
        for (i = 0; i < OSSL_NELEM(default_group_strings); i++) {
          if ((size_t)len == (strlen(default_group_strings[i].list_name)) &&
            ...
            if (restored_default_group_string == NULL) return 0;
            if (add_keyshare
            ...
            restored_default_group_string[strlen(default_group_strings[i]
                                                     .group_string) +
            ...
            /* we enforce group mode (= append tuple) for DEFAULT_XYZ group
             * lists */
            garg->tuple_mode = 0;
  ...
  if (garg->gidcnt == garg->gidmax) {
    uint16_t* tmp =
        OPENSSL_realloc_array(garg->gid_arr, garg->gidmax + GROUPLIST_INCREMENT,
                              sizeof(*garg->gid_arr));
  ...
  if (remove_group) {
     ...
     * group */
    if (add_keyshare) garg->ksid_arr[garg->ksidcnt++] = gid;
  }
...
static int tuple_cb(const char* tuple, int len, void* arg) {
  ...
  if (restored_tuple_string == NULL) return 0;
...
int tls1_set_groups_list(SSL_CTX* ctx, uint16_t** grpext, size_t* grpextlen,
  ...
  for (i = j = 0; j < gcb.tplcnt; j++) {
    if (gcb.tuplcnt_arr[j] == 0) continue;
...
int tls1_check_group_id(SSL_CONNECTION* s, uint16_t group_id,
  ...
  if (tls1_suiteb(s) && s->s3.tmp.new_cipher != NULL) {
    ...
    if (cid == TLS1_CK_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256) {
      ...
      if (group_id != OSSL_TLS_GROUP_ID_secp384r1) return 0;
    } else {
      /* Should never happen */
      return 0;
    }
  ...
  if (check_own_groups) {
    ...
    if (!tls1_in_list(group_id, groups, groups_len)) return 0;
  }

  if (!tls_group_allowed(s, group_id, SSL_SECOP_CURVE_CHECK)) return 0;
...
static int tls1_check_pkey_comp(SSL_CONNECTION* s, EVP_PKEY* pkey) {
  ...
  if (!EVP_PKEY_is_a(pkey, "EC")) return 1;

  /* Get required compression id */
  point_conv = EVP_PKEY_get_ec_point_conv_form(pkey);
  if (point_conv == 0) return 0;
   ...
   */
  if (s->ext.peer_ecpointformats == NULL) return 1;

  for (i = 0; i < s->ext.peer_ecpointformats_len; i++) {
    if (s->ext.peer_ecpointformats[i] == comp_id) return 1;
  }
  return 0;
}
...
static uint16_t tls1_get_group_id(EVP_PKEY* pkey) {
  ...
  if (curve_nid == NID_undef) return 0;
...
static int tls1_check_cert_param(SSL_CONNECTION* s, X509* x, int check_ee_md) {
  ...
  if (!EVP_PKEY_is_a(pkey, "EC")) return 1;
  /* Check compression */
  if (!tls1_check_pkey_comp(s, pkey)) return 0;
  group_id = tls1_get_group_id(pkey);
  /*
   * For a server we allow the certificate to not be in our list of supported
   * groups.
   */
  if (!tls1_check_group_id(s, group_id, !s->server)) return 0;
  /*
  ...
  if (check_ee_md && tls1_suiteb(s)) {
    ...
    for (i = 0; i < s->shared_sigalgslen; i++) {
      if (check_md == s->shared_sigalgs[i]->sigandhash) return 1;
    }
    return 0;
  }
  return 1;
...
int tls1_check_ec_tmp_key(SSL_CONNECTION* s, unsigned long cid) {
  ...
  if (!tls1_suiteb(s)) return tls1_shared_group(s, 0) != 0;
  /*
...
int ssl_setup_sigalgs(SSL_CTX* ctx) {
  ...
  if (cache == NULL || tmpkey == NULL) goto err;

  tls12_sigalgs_list = OPENSSL_calloc(sigalgs_len, sizeof(uint16_t));
  if (tls12_sigalgs_list == NULL) goto err;
  ...
  for (i = 0, lu = sigalg_lookup_tbl; i < OSSL_NELEM(sigalg_lookup_tbl);
    ...
    if (pctx == NULL) cache[i].available = 0;
  ...
  for (i = 0; i < OSSL_NELEM(tls12_sigalgs); ++i) {
    ...
    for (j = 0; j < sigalgs_len; ent++, j++) {
      if (ent->sigalg != tls12_sigalgs[i]) continue;
  ...
  for (i = 0, lu = sigalg_lookup_tbl; i < OSSL_NELEM(sigalg_lookup_tbl);
    ...
    if (enabled) {
      ...
      if (sa != NULL) {
        ...
        if (strlen(retval) > 0) OPENSSL_strlcat(retval, ":", maxretlen);
...
static const SIGALG_LOOKUP* tls1_get_legacy_sigalg(const SSL_CONNECTION* s,
  ...
  if (idx < 0 || idx >= (int)OSSL_NELEM(tls_default_sigalg)) return NULL;
  ...
  if (SSL_USE_SIGALGS(s) || idx != SSL_PKEY_RSA) {
    ...
    if (!tls1_lookup_md(SSL_CONNECTION_GET_CTX(s), lu, NULL)) return NULL;
    if (!tls12_sigalg_allowed(s, SSL_SECOP_SIGALG_SUPPORTED, lu)) return NULL;
    return lu;
  }
...
int tls12_check_peer_sigalg(SSL_CONNECTION* s, uint16_t sig, EVP_PKEY* pkey) {
  ...
  if (pkeyid == EVP_PKEY_KEYMGMT) pkeyid = lu->sig;
...
int ssl_cipher_disabled(const SSL_CONNECTION* s, const SSL_CIPHER* c, int op,
  ...
  if (c->algorithm_mkey & s->s3.tmp.mask_k ||
    ...
    return 1;
  if (s->s3.tmp.max_ver == 0) return 1;
...
int tls1_set_server_sigalgs(SSL_CONNECTION* s) {
  ...
  if (s->shared_sigalgs != NULL) return 1;
...
SSL_TICKET_STATUS tls_get_ticket_from_client(SSL_CONNECTION* s,
  ...
  *ret = NULL;
   ...
   * resumption.
   */
  if (s->version <= SSL3_VERSION || !tls_use_ticket(s)) return SSL_TICKET_NONE;

  ticketext = &hello->pre_proc_exts[TLSEXT_IDX_session_ticket];
  if (!ticketext->present) return SSL_TICKET_NONE;
...
SSL_TICKET_STATUS tls_decrypt_ticket(SSL_CONNECTION* s,
...
#endif
  {
    ...
    EVP_CIPHER_free(aes256cbc);
    if (SSL_CONNECTION_IS_TLS13(s)) renew_ticket = 1;
  }
  /*
...
static int tls12_sigalg_allowed(const SSL_CONNECTION* s, int op,
  ...
  if (SSL_CONNECTION_IS_TLS13(s) && lu->sig == EVP_PKEY_DSA) return 0;
  /*
  ...
  if (lu->sig == NID_id_GostR3410_2012_256 ||
    ...
    if (!s->server &&
      ...
      if (s->s3.tmp.min_ver >= TLS1_3_VERSION) return 0;
      ...
      for (i = 0; i < num; i++) {
        ...
        if (ssl_cipher_disabled(s, c, SSL_SECOP_CIPHER_SUPPORTED, 0)) continue;

        if ((c->algorithm_mkey & (SSL_kGOST | SSL_kGOST18)) != 0) break;
      }
      if (i == num) return 0;
    }
  }
...
int tls12_copy_sigalgs(SSL_CONNECTION* s, WPACKET* pkt, const uint16_t* psig,
  ...
  for (i = 0; i < psiglen; i++, psig++) {
    ...
    if (lu == NULL || !tls_sigalg_compat(s, lu)) continue;
    if (!WPACKET_put_bytes_u16(pkt, *psig)) return 0;
    /*
  ...
  if (rv == 0) ERR_raise(ERR_LIB_SSL, SSL_R_NO_SUITABLE_SIGNATURE_ALGORITHM);
  return rv;
}
...
int tls1_save_u16(PACKET* pkt, uint16_t** pdest, size_t* pdestlen) {
  ...
  if ((buf = OPENSSL_malloc_array(size, sizeof(*buf))) == NULL) return 0;
...
int tls1_save_sigalgs(SSL_CONNECTION* s, PACKET* pkt, int cert) {
  ...
  if (!SSL_USE_SIGALGS(s)) return 1;
  /* Should never happen */
  if (s->cert == NULL) return 0;
...
int tls1_process_sigalgs(SSL_CONNECTION* s) {
  ...
  for (i = 0; i < s->shared_sigalgslen; i++) {
    ...
    if (SSL_CONNECTION_IS_TLS13(s) && sigptr->sig == EVP_PKEY_RSA) continue;
...
int SSL_get_sigalgs(SSL* s, int idx, int* psign, int* phash, int* psignhash,
  ...
  if (psig == NULL || numsigalgs > INT_MAX) return 0;
  if (idx >= 0) {
    const SIGALG_LOOKUP* lu;

    if (idx >= (int)numsigalgs) return 0;
    psig += idx;
    if (rhash != NULL) *rhash = (unsigned char)((*psig >> 8) & 0xff);
    if (rsig != NULL) *rsig = (unsigned char)(*psig & 0xff);
    lu = tls1_lookup_sigalg(SSL_CONNECTION_GET_CTX(sc), *psig);
    if (psign != NULL) *psign = lu != NULL ? lu->sig : NID_undef;
    if (phash != NULL) *phash = lu != NULL ? lu->hash : NID_undef;
    if (psignhash != NULL) *psignhash = lu != NULL ? lu->sigandhash : NID_undef;
  }
...
int SSL_get_shared_sigalgs(SSL* s, int idx, int* psign, int* phash,
  ...
  if (phash != NULL) *phash = shsigalgs->hash;
  if (psign != NULL) *psign = shsigalgs->sig;
  if (psignhash != NULL) *psignhash = shsigalgs->sigandhash;
  if (rsig != NULL) *rsig = (unsigned char)(shsigalgs->sigalg & 0xff);
  if (rhash != NULL) *rhash = (unsigned char)((shsigalgs->sigalg >> 8) & 0xff);
...
static void get_sigorhash(int* psig, int* phash, const char* str) {
  if (OPENSSL_strcasecmp(str, "RSA") == 0) {
    ...
    *phash = OBJ_sn2nid(str);
    if (*phash == NID_undef) *phash = OBJ_ln2nid(str);
  }
}
...
static int sig_cb(const char* elem, int len, void* arg) {
  ...
  if (len > (int)(sizeof(etmp) - 1)) return 0;
  memcpy(etmp, elem, len);
  etmp[len] = 0;
  p = strchr(etmp, '+');
  /*
    ...
    if (sarg->ctx != NULL) {
      for (i = 0; i < sarg->ctx->sigalg_lookup_cache_len; i++) {
        ...
        if ((alias != NULL && OPENSSL_strcasecmp(etmp, alias) == 0) ||
          ...
          if (!sarg->ctx->sigalg_lookup_cache[i].available) return 1;
...
int tls1_set_sigalgs_list(SSL_CTX* ctx, CERT* c, const char* str, int client) {
  ...
  if (ctx != NULL) sig.ctx = ctx;
  if (!CONF_parse_list(str, ':', 1, sig_cb, &sig)) return 0;
  if (sig.sigalgcnt == 0) {
    ERR_raise_data(ERR_LIB_SSL, ERR_R_PASSED_INVALID_ARGUMENT,
...
int tls1_set_sigalgs(CERT* c, const int* psig_nids, size_t salglen,
  ...
  for (i = 0, sptr = sigalgs; i < salglen; i += 2) {
    ...
    if (j == OSSL_NELEM(sigalg_lookup_tbl)) goto err;
  }
...
static int tls1_check_sig_alg(SSL_CONNECTION* s, X509* x, int default_nid) {
   ...
   * extension.
   */
  if (default_nid == -1 || X509_self_signed(x, 0)) return 1;
  sig_nid = X509_get_signature_nid(x);
  if (default_nid) return sig_nid == default_nid ? 1 : 0;
  ...
  for (i = 0; i < sigalgslen; i++) {
    ...
    sigalg = use_pc_sigalgs ? tls1_lookup_sigalg(SSL_CONNECTION_GET_CTX(s),
                            ...
                            : s->shared_sigalgs[i];
    if (sigalg == NULL) continue;
    if (sig_nid == sigalg->sigandhash) return 1;
    if (sigalg->sig != EVP_PKEY_RSA_PSS) continue;
    /*
     ...
     */
    if (!OBJ_find_sigid_algs(sig_nid, &mdnid, &pknid)) continue;
    if (pknid == EVP_PKEY_RSA && mdnid == sigalg->hash) return 1;
  }
  return 0;
}
...
static int ssl_check_ca_name(STACK_OF(X509_NAME)* names, X509* x) {
  ...
  for (i = 0; i < sk_X509_NAME_num(names); i++) {
    if (!X509_NAME_cmp(nm, sk_X509_NAME_value(names, i))) return 1;
  }
  return 0;
}
...
int tls1_check_chain(SSL_CONNECTION* s, X509* x, EVP_PKEY* pk,
  ...
  if (idx != -1) {
    ...
    if (tls12_rpk_and_privkey(s, idx)) {
      if (EVP_PKEY_is_a(pk, "EC") && !tls1_check_pkey_comp(s, pk)) return 0;
      *pvalid = rv = CERT_PKEY_RPK;
      return rv;
    }
    /* If no cert or key, forget it */
    if (x == NULL || pk == NULL) goto end;
  } else {
    size_t certidx;

    if (x == NULL || pk == NULL) return 0;
  ...
  if (suiteb_flags) {
    int ok;
    if (check_flags) check_flags |= CERT_PKEY_SUITEB;
  ...
  if (TLS1_get_version(SSL_CONNECTION_GET_SSL(s)) >= TLS1_2_VERSION &&
    ...
    if (default_nid > 0 && c->conf_sigalgs) {
      ...
      for (j = 0; j < c->conf_sigalgslen; j++, p++) {
        ...
        if (lu != NULL && lu->hash == NID_sha1 && lu->sig == rsign) break;
      }
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) {
       ...
       */
      if (find_sig_alg(s, x, pk) != NULL) rv |= CERT_PKEY_EE_SIGNATURE;
...
skip_sigs:
  ...
  if (tls1_check_cert_param(s, x, 1))
    ...
    goto end;
  if (!s->server) rv |= CERT_PKEY_CA_PARAM;
  ...
  if (!s->server && strict_mode) {
    ...
    if (check_type) {
      ...
      if (!(rv & CERT_PKEY_CERT_TYPE) && !check_flags) goto end;
    } else {
      rv |= CERT_PKEY_CERT_TYPE;
    }
    ...
    if (!check_flags && !(rv & CERT_PKEY_ISSUER_NAME)) goto end;
  } else
...
EVP_PKEY* ssl_get_auto_dh(SSL_CONNECTION* s) {
  ...
  if (s->cert->dh_tmp_auto != 2) {
    if (s->s3.tmp.new_cipher->algorithm_auth & (SSL_aNULL | SSL_aPSK)) {
      ...
      if (s->s3.tmp.cert == NULL) return NULL;
  ...
  if (dh_secbits < sec_level_bits) dh_secbits = sec_level_bits;
  ...
  if (pctx == NULL || EVP_PKEY_fromdata_init(pctx) != 1) goto err;
...
static int ssl_security_cert_sig(SSL_CONNECTION* s, SSL_CTX* ctx, X509* x,
  ...
  if ((X509_get_extension_flags(x) & EXFLAG_SS) != 0) return 1;
  if (!X509_get_signature_info(x, &nid, &pknid, &secbits, NULL)) secbits = -1;
  /* If digest NID not defined use signature NID */
  if (nid == NID_undef) nid = pknid;
  if (s != NULL)
...
int ssl_security_cert_chain(SSL_CONNECTION* s, STACK_OF(X509)* sk, X509* x,
  ...
  if (x == NULL) {
    ...
    if (x == NULL) return ERR_R_INTERNAL_ERROR;
    start_idx = 1;
  } else
    start_idx = 0;
...
static int check_cert_usable(SSL_CONNECTION* s, const SIGALG_LOOKUP* sig,
   ...
   */
  if (sig->hash != NID_undef) mdname = OBJ_nid2sn(sig->hash);
  ...
  if (s->s3.tmp.peer_cert_sigalgs != NULL) {
    if (!X509_get_signature_info(x, &mdnid, &pknid, NULL, NULL)) return 0;
    for (i = 0; i < s->s3.tmp.peer_cert_sigalgslen; i++) {
       ...
       * signing certificate.
       */
      if (mdnid == lu->hash && pknid == lu->sig) return 1;
    }
    return 0;
  }
...
static int has_usable_cert(SSL_CONNECTION* s, const SIGALG_LOOKUP* sig,
  ...
  if (idx == -1) idx = sig->sig_idx;
  if (!ssl_has_cert(s, idx)) return 0;
...
static const SIGALG_LOOKUP* find_sig_alg(SSL_CONNECTION* s, X509* x,
  ...
  for (i = 0; i < s->shared_sigalgslen; i++) {
    ...
    if (!tls1_lookup_md(sctx, lu, NULL)) continue;
    ...
    if (lu->sig == EVP_PKEY_EC) {
      if (curve == -1) curve = ssl_get_EC_curve_nid(tmppkey);
      if (lu->curve != NID_undef && curve != lu->curve) continue;
    } else if (lu->sig == EVP_PKEY_RSA_PSS) {
      /* validate that key is large enough for the signature algorithm */
      if (!rsa_pss_check_min_key_size(sctx, tmppkey, lu)) continue;
    }
    break;
  }

  if (i == s->shared_sigalgslen) return NULL;
...
int tls_choose_sigalg(SSL_CONNECTION* s, int fatalerrs) {
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    if (!(s->s3.tmp.new_cipher->algorithm_auth & SSL_aCERT)) return 1;
    ...
    if (SSL_USE_SIGALGS(s)) {
      ...
      if (s->s3.tmp.peer_sigalgs != NULL) {
        ...
        for (i = 0; i < s->shared_sigalgslen; i++) {
          ...
          lu = s->shared_sigalgs[i];
          if (!tls_sigalg_compat(s, lu)) continue;
          if (s->server) {
            if ((sig_idx = tls12_get_cert_sigalg_idx(s, lu)) == -1) continue;
          } else {
            int cc_idx = (int)(s->cert->key - s->cert->pkeys);

            sig_idx = lu->sig_idx;
            if (cc_idx != sig_idx) continue;
          }
          /* Check that we have a cert, and sig_algs_cert */
          if (!has_usable_cert(s, lu, sig_idx)) continue;
          if (lu->sig == EVP_PKEY_RSA_PSS) {
            /* validate that key is large enough for the signature algorithm */
            EVP_PKEY* pkey = s->cert->pkeys[sig_idx].privatekey;

            if (!rsa_pss_check_min_key_size(sctx, pkey, lu)) continue;
          }
          if (curve == -1 || lu->curve == curve) break;
        }
#ifndef OPENSSL_NO_GOST
        /*
  ...
  if (sig_idx == -1) sig_idx = lu->sig_idx;
...
SSL_HMAC* ssl_hmac_new(const SSL_CTX* ctx) {
  ...
  if (ctx->ext.ticket_key_evp_cb == NULL && ctx->ext.ticket_key_cb != NULL) {
    if (!ssl_hmac_old_new(ret)) goto err;
    return ret;
  }
#endif
  mac = EVP_MAC_fetch(ctx->libctx, "HMAC", ctx->propq);
  if (mac == NULL || (ret->ctx = EVP_MAC_CTX_new(mac)) == NULL) goto err;
  EVP_MAC_free(mac);
  return ret;
err:
...
int ssl_hmac_init(SSL_HMAC* ctx, void* key, size_t len, char* md) {
  ...
  if (ctx->ctx != NULL) {
    ...
    *p = OSSL_PARAM_construct_end();
    if (EVP_MAC_init(ctx->ctx, key, len, params)) return 1;
  }
#ifndef OPENSSL_NO_DEPRECATED_3_0
  if (ctx->old_ctx != NULL) return ssl_hmac_old_init(ctx, key, len, md);
#endif
  return 0;
}

int ssl_hmac_update(SSL_HMAC* ctx, const unsigned char* data, size_t len) {
  if (ctx->ctx != NULL) return EVP_MAC_update(ctx->ctx, data, len);
#ifndef OPENSSL_NO_DEPRECATED_3_0
  if (ctx->old_ctx != NULL) return ssl_hmac_old_update(ctx, data, len);
#endif
  return 0;
}
...
int ssl_hmac_final(SSL_HMAC* ctx, unsigned char* md, size_t* len,
                   size_t max_size) {
  if (ctx->ctx != NULL) return EVP_MAC_final(ctx->ctx, md, len, max_size);
#ifndef OPENSSL_NO_DEPRECATED_3_0
  if (ctx->old_ctx != NULL) return ssl_hmac_old_final(ctx, md, len);
#endif
  return 0;
}

size_t ssl_hmac_size(const SSL_HMAC* ctx) {
  if (ctx->ctx != NULL) return EVP_MAC_CTX_get_mac_size(ctx->ctx);
#ifndef OPENSSL_NO_DEPRECATED_3_0
  if (ctx->old_ctx != NULL) return ssl_hmac_old_size(ctx);
#endif
  return 0;
}
