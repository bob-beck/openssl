...
discover_provider_groups(OSSL_PROVIDER* provider, void* vctx)
{
  ...
  return OSSL_PROVIDER_get_capabilities(
    provider, "TLS-GROUP", add_provider_groups, &pgd);
}
...
add_provider_sigalgs(const OSSL_PARAM params[], void* data)
{
  ...
  if (ctx->sigalg_list_max_len == ctx->sigalg_list_len) {
    ...
    if (tmp == NULL)
      return 0;
    ctx->sigalg_list = tmp;
    memset(tmp + ctx->sigalg_list_max_len,
           0,
...
discover_provider_sigalgs(OSSL_PROVIDER* provider, void* vctx)
{
  ...
  OSSL_PROVIDER_get_capabilities(
    provider, "TLS-SIGALG", add_provider_sigalgs, &pgd);
  /*
...
tls_group_allowed(SSL_CONNECTION* s, uint16_t group, int op)
{
  ...
  gtmp[1] = group & 0xff;
  return ssl_security(
    s, op, ginfo->secbits, tls1_group_id2nid(ginfo->group_id, 0), (void*)gtmp);
}
...
gid_cb(const char* elem, int len, void* arg)
{
  ...
  while (
    ...
    ((current_prefix = strchr(prefixes, elem[0])) != NULL ||
     OPENSSL_strncasecmp(
       current_prefix = (char*)DEFAULT_GROUPNAME_FIRST_CHARACTER, elem, 1) ==
       0)) {
      ...
      default:
        ...
        for (i = 0; i < OSSL_NELEM(default_group_strings); i++) {
          if ((size_t)len == (strlen(default_group_strings[i].list_name)) &&
              OPENSSL_strncasecmp(
                default_group_strings[i].list_name, elem, len) == 0) {
            /*
...
tuple_cb(const char* tuple, int len, void* arg)
{
  ...
  retval = CONF_parse_list(
    restored_tuple_string, GROUP_DELIMITER_CHARACTER, 1, gid_cb, arg);
                     ...
                     const char* str)
{
  ...
  if (parse_ret == -1) {
    ERR_raise_data(
      ERR_LIB_SSL, ERR_R_PASSED_INVALID_ARGUMENT, "Syntax error in '%s'", str);
    goto end;
  }
...
int
tls1_check_group_id(SSL_CONNECTION* s, uint16_t group_id, int check_own_groups)
{
...
static int
rsa_pss_check_min_key_size(SSL_CTX* ctx,
                           const EVP_PKEY* pkey,
                           const SIGALG_LOOKUP* lu)
{
  const EVP_MD* md;
...
ssl_set_client_disabled(SSL_CONNECTION* s)
{
  ...
  if (ssl_get_min_max_version(
        s, &s->s3.tmp.min_ver, &s->s3.tmp.max_ver, NULL) != 0)
    return 0;
...
static size_t
tls12_shared_sigalgs(SSL_CONNECTION* s,
...
tls1_save_sigalgs(SSL_CONNECTION* s, PACKET* pkt, int cert)
{
  ...
  if (cert)
    return tls1_save_u16(
      pkt, &s->s3.tmp.peer_cert_sigalgs, &s->s3.tmp.peer_cert_sigalgslen);
  else
    return tls1_save_u16(
      pkt, &s->s3.tmp.peer_sigalgs, &s->s3.tmp.peer_sigalgslen);
}
...
int
tls1_set_raw_sigalgs(CERT* c, const uint16_t* psigs, size_t salglen, int client)
{
  uint16_t* sigalgs;
...
int
ssl_security_cert_chain(SSL_CONNECTION* s, STACK_OF(X509)* sk, X509* x, int vfy)
{
  int rv, start_idx, i;
 ...
 */
static int
check_cert_usable(SSL_CONNECTION* s,
                  const SIGALG_LOOKUP* sig,
                  X509* x,
                  EVP_PKEY* pkey)
{
  const SIGALG_LOOKUP* lu;
  ...
  supported = EVP_PKEY_digestsign_supports_digest(
    pkey, sctx->libctx, mdname, sctx->propq);
  if (supported <= 0)
    return 0;
...
has_usable_cert(SSL_CONNECTION* s, const SIGALG_LOOKUP* sig, int idx)
{
  ...
  return check_cert_usable(
    s, sig, s->cert->pkeys[idx].x509, s->cert->pkeys[idx].privatekey);
}
 ...
 */
static int
is_cert_usable(SSL_CONNECTION* s,
               const SIGALG_LOOKUP* sig,
               X509* x,
               EVP_PKEY* pkey)
{
