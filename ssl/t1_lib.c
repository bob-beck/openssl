...
static const SIGALG_LOOKUP* find_sig_alg(SSL_CONNECTION* s,
                                         X509* x,
                                         EVP_PKEY* pkey);
static int tls12_sigalg_allowed(const SSL_CONNECTION* s,
                                int op,
...
int tls_valid_group(SSL_CONNECTION* s,
                    uint16_t group_id,
                    int minversion,
                    int maxversion,
                    int isec,
...
int tls1_check_group_id(SSL_CONNECTION* s,
                        uint16_t group_id,
...
static int rsa_pss_check_min_key_size(SSL_CTX* ctx,
                                      const EVP_PKEY* pkey,
                                      const SIGALG_LOOKUP* lu) {
  const EVP_MD* md;
 ...
 */
int ssl_cipher_disabled(const SSL_CONNECTION* s,
                        const SSL_CIPHER* c,
                        int op,
                        int ecdhe) {
...
int tls12_copy_sigalgs(SSL_CONNECTION* s,
                       WPACKET* pkt,
                       const uint16_t* psig,
                       size_t psiglen) {
  size_t i;
...
int tls1_set_raw_sigalgs(CERT* c,
...
static int ssl_security_cert_key(SSL_CONNECTION* s,
                                 SSL_CTX* ctx,
                                 X509* x,
                                 int op) {
  int secbits = -1;
...
static int ssl_security_cert_sig(SSL_CONNECTION* s,
                                 SSL_CTX* ctx,
                                 X509* x,
                                 int op) {
...
int ssl_security_cert(SSL_CONNECTION* s,
                      SSL_CTX* ctx,
                      X509* x,
                      int vfy,
                      int is_ee) {
...
int ssl_security_cert_chain(SSL_CONNECTION* s,
                            STACK_OF(X509)* sk,
                            X509* x,
                            int vfy) {
  int rv, start_idx, i;
 ...
 */
static int check_cert_usable(SSL_CONNECTION* s,
 ...
 */
static int has_usable_cert(SSL_CONNECTION* s,
                           const SIGALG_LOOKUP* sig,
                           int idx) {
 ...
 */
static int is_cert_usable(SSL_CONNECTION* s,
                          const SIGALG_LOOKUP* sig,
                          X509* x,
                          EVP_PKEY* pkey) {
  size_t idx;
