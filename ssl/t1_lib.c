...
static int
add_provider_groups (const OSSL_PARAM params[], void *data)
{
...
static int
discover_provider_groups (OSSL_PROVIDER *provider, void *vctx)
{
...
static int
add_provider_sigalgs (const OSSL_PARAM params[], void *data)
{
  struct provider_ctx_data_st *pgd = data;
  SSL_CTX *ctx = pgd->ctx;
...
static int
discover_provider_sigalgs (OSSL_PROVIDER *provider, void *vctx)
{
  struct provider_ctx_data_st pgd;
...
static uint16_t
tls1_group_name2id (SSL_CTX *ctx, const char *name)
{
  size_t i;
...
tls1_group_id_lookup (SSL_CTX *ctx, uint16_t group_id)
{
  size_t i;
 ...
 */
void
tls1_get_supported_groups (SSL_CONNECTION *s, const uint16_t **pgroups,
                           size_t *pgroupslen)
{
 ...
 * s->ext.supportedgroups == NULL means legacy syntax (no [*,/,-]) from
 * built-in group array. In this case, we need to send exactly one key share,
 * which MUST be the first (leftmost) eligible group from the legacy list.
 * Therefore, we provide the entire list of supported groups in this case.
 *
...
void
tls1_get_group_tuples (SSL_CONNECTION *s, const size_t **ptuples,
                       size_t *ptupleslen)
{
  SSL_CTX *sctx = SSL_CONNECTION_GET_CTX (s);
...
int
tls_valid_group (SSL_CONNECTION *s, uint16_t group_id, int minversion,
...
int
tls_group_allowed (SSL_CONNECTION *s, uint16_t group, int op)
{
  const TLS_GROUP_INFO *ginfo
...
static void
free_wrapper (TLS_GROUP_IX *a)
{
  OPENSSL_free (a);
}

static int
tls_group_ix_cmp (const TLS_GROUP_IX *const *a, const TLS_GROUP_IX *const *b)
{
...
int
tls1_get0_implemented_groups (int min_proto_version, int max_proto_version,
...
int
tls1_set_groups (uint16_t **grpext, size_t *grpextlen, uint16_t **ksext,
 ...
 */
static int
gid_cb (const char *elem, int len, void *arg)
{
  gid_cb_st *garg = arg;
  size_t i, j, k;
  uint16_t gid = 0;
          ...
          for (i = 0; i < OSSL_NELEM (default_group_strings); i++)
            {
              if ((size_t)len == (strlen (default_group_strings[i].list_name))
                         ...
                         == 0)
                {
                   ...
                   * DEFAULT group string like a tuple which is appended to the
                   * current tuple rather then starting a new tuple. Variable
                   * tuple_mode is the flag which controls append tuple vs
                   * start new tuple.
                   */

                  if (ignore_unknown || remove_group)
                    return -1; /* removal or ignore not allowed here -> syntax
                                  error */
  ...
  if (remove_group)
          ...
          for (j = 0; j < garg->tplcnt; j++)
            {
              ...
              if (k > i)
                { /* remove from one of the previous tuples */
                  garg->tuplcnt_arr[j]--;
                  break; /* We took care not to have group duplicates, hence we
                            can stop here */
                }
            }
    ...
    { /* Processing addition of a single new group */
...
static int
tuple_cb (const char *tuple, int len, void *arg)
{
  gid_cb_st *garg = arg;
  int retval = 1; /* We assume success */
  ...
  if (garg->tuplcnt_arr[garg->tplcnt] > 0)
    { /* Some valid groups are present in current tuple... */
      if (garg->tuple_mode)
        {
          /* We 'close' the tuple */
          garg->tplcnt++;
 ...
 * extracts the string of a tuple using tuple_cb, while the next lower step
 * uses gid_cb to parse and process the groups inside a tuple
 */
int
tls1_set_groups_list (SSL_CTX *ctx, uint16_t **grpext, size_t *grpextlen,
...
int
tls1_check_group_id (SSL_CONNECTION *s, uint16_t group_id,
...
void
tls1_get_formatlist (SSL_CONNECTION *s, const unsigned char **pformats,
                     size_t *num_formats)
{
  /*
...
static int
tls1_check_pkey_comp (SSL_CONNECTION *s, EVP_PKEY *pkey)
{
...
static uint16_t
tls1_get_group_id (EVP_PKEY *pkey)
{
 ...
 */
static int
tls1_check_cert_param (SSL_CONNECTION *s, X509 *x, int check_ee_md)
{
  uint16_t group_id;
  EVP_PKEY *pkey;
 ...
 */
int
tls1_check_ec_tmp_key (SSL_CONNECTION *s, unsigned long cid)
{
...
char *
SSL_get1_builtin_sigalgs (OSSL_LIB_CTX *libctx)
{
...
tls1_lookup_sigalg (const SSL_CTX *ctx, uint16_t sigalg)
{
  size_t i;
...
static int
rsa_pss_check_min_key_size (SSL_CTX *ctx, const EVP_PKEY *pkey,
                            const SIGALG_LOOKUP *lu)
{
  const EVP_MD *md;
 ...
 */
static const SIGALG_LOOKUP *
tls1_get_legacy_sigalg (const SSL_CONNECTION *s, int idx)
{
  if (idx == -1)
    {
      if (s->server)
        {
          ...
          /*
           * As both SSL_PKEY_GOST12_512 and SSL_PKEY_GOST12_256 indices can be
           * used with new (aGOST12-only) ciphersuites, we should find out
 ...
 */
static int
sigalg_security_bits (SSL_CTX *ctx, const SIGALG_LOOKUP *lu)
{
...
static int
tls_sigalg_compat (SSL_CONNECTION *sc, const SIGALG_LOOKUP *lu)
{
  int minversion, maxversion;
 ...
 */
int
tls12_check_peer_sigalg (SSL_CONNECTION *s, uint16_t sig, EVP_PKEY *pkey)
{
...
int
SSL_get_peer_signature_type_nid (const SSL *s, int *pnid)
{
...
int
SSL_get_signature_type_nid (const SSL *s, int *pnid)
{
  const SSL_CONNECTION *sc = SSL_CONNECTION_FROM_CONST_SSL (s);
 ...
 */
int
ssl_cipher_disabled (const SSL_CONNECTION *s, const SSL_CIPHER *c, int op,
                     int ecdhe)
{
 ...
 */
SSL_TICKET_STATUS
tls_get_ticket_from_client (SSL_CONNECTION *s, CLIENTHELLO_MSG *hello,
                            SSL_SESSION **ret)
{
  size_t size;
 ...
 */
SSL_TICKET_STATUS
tls_decrypt_ticket (SSL_CONNECTION *s, const unsigned char *etick,
...
static int
tls12_sigalg_allowed (const SSL_CONNECTION *s, int op, const SIGALG_LOOKUP *lu)
{
...
int
tls12_copy_sigalgs (SSL_CONNECTION *s, WPACKET *pkt, const uint16_t *psig,
                    size_t psiglen)
{
  size_t i;
  int rv = 0;
...
static size_t
tls12_shared_sigalgs (SSL_CONNECTION *s, const SIGALG_LOOKUP **shsig,
...
static int
tls1_set_shared_sigalgs (SSL_CONNECTION *s)
{
...
int
SSL_get_sigalgs (SSL *s, int idx, int *psign, int *phash, int *psignhash,
...
int
SSL_get_shared_sigalgs (SSL *s, int idx, int *psign, int *phash,
...
static void
get_sigorhash (int *psig, int *phash, const char *str)
{
...
static int
sig_cb (const char *elem, int len, void *arg)
{
  sig_cb_st *sarg = arg;
  size_t i = 0;
...
int
tls1_set_raw_sigalgs (CERT *c, const uint16_t *psigs, size_t salglen,
                      int client)
{
  uint16_t *sigalgs;
...
int
tls1_set_sigalgs (CERT *c, const int *psig_nids, size_t salglen, int client)
{
...
static int
tls1_check_sig_alg (SSL_CONNECTION *s, X509 *x, int default_nid)
{
...
static int
ssl_check_ca_name (STACK_OF (X509_NAME) *names, X509 *x)
{
  const X509_NAME *nm;
  int i;
...
int
tls1_check_chain (SSL_CONNECTION *s, X509 *x, EVP_PKEY *pk,
...
int
SSL_check_chain (SSL *s, X509 *x, EVP_PKEY *pk, STACK_OF (X509) *chain)
{
  SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL (s);
...
static int
ssl_security_cert_key (SSL_CONNECTION *s, SSL_CTX *ctx, X509 *x, int op)
{
  int secbits = -1;
...
static int
ssl_security_cert_sig (SSL_CONNECTION *s, SSL_CTX *ctx, X509 *x, int op)
{
...
int
ssl_security_cert (SSL_CONNECTION *s, SSL_CTX *ctx, X509 *x, int vfy,
                   int is_ee)
{
  if (vfy)
    vfy = SSL_SECOP_PEER;
 ...
 * |x| is NULL. If |vfy| is 1 then we are verifying a peer chain and not
 * sending one to the peer. Return values: 1 if ok otherwise error code to use
 */

int
ssl_security_cert_chain (SSL_CONNECTION *s, STACK_OF (X509) *sk, X509 *x,
                         int vfy)
{
  int rv, start_idx, i;
...
static int
tls12_get_cert_sigalg_idx (const SSL_CONNECTION *s, const SIGALG_LOOKUP *lu)
{
 ...
 */
static int
check_cert_usable (SSL_CONNECTION *s, const SIGALG_LOOKUP *sig, X509 *x,
                   EVP_PKEY *pkey)
{
  const SIGALG_LOOKUP *lu;
 ...
 */
static int
has_usable_cert (SSL_CONNECTION *s, const SIGALG_LOOKUP *sig, int idx)
{
 ...
 */
static int
is_cert_usable (SSL_CONNECTION *s, const SIGALG_LOOKUP *sig, X509 *x,
                EVP_PKEY *pkey)
{
  size_t idx;
 ...
 */
static const SIGALG_LOOKUP *
find_sig_alg (SSL_CONNECTION *s, X509 *x, EVP_PKEY *pkey)
{
 ...
 *
 * For clients fatalerrs is set to 0. If a certificate is not suitable it is
 * not a fatal error: we will either try another certificate or not present one
...
int
SSL_CTX_set_tlsext_max_fragment_length (SSL_CTX *ctx, uint8_t mode)
{
...
int
SSL_set_tlsext_max_fragment_length (SSL *ssl, uint8_t mode)
{
...
uint8_t
SSL_SESSION_get_max_fragment_length (const SSL_SESSION *session)
{
...
__owur int
tls13_set_encoded_pub_key (EVP_PKEY *pkey, const unsigned char *enckey,
                           size_t enckeylen)
{
