...
static void ssl_set_option(SSL_CONF_CTX* cctx, unsigned int name_flags,
  ...
  if (cctx->poptions == NULL) return;
  if (name_flags & SSL_TFLAG_INV) onoff ^= 1;
...
static int ssl_match_option(SSL_CONF_CTX* cctx, const ssl_flag_tbl* tbl,
  ...
  if (!(cctx->flags & tbl->name_flags & SSL_TFLAG_BOTH)) return 0;
  if (namelen == -1) {
    if (strcmp(tbl->name, name)) return 0;
...
static int ssl_set_option_list(const char* elem, int len, void* usr) {
  ...
  for (i = 0, tbl = cctx->tbl; i < cctx->ntbl; i++, tbl++) {
    if (ssl_match_option(cctx, tbl, elem, len, onoff)) return 1;
  }
  return 0;
}
...
static int cmd_SignatureAlgorithms(SSL_CONF_CTX* cctx, const char* value) {
  int rv;
  if (cctx->ssl) rv = SSL_set1_sigalgs_list(cctx->ssl, value);
...
static int cmd_ClientSignatureAlgorithms(SSL_CONF_CTX* cctx,
                                         const char* value) {
  int rv;
  if (cctx->ssl) rv = SSL_set1_client_sigalgs_list(cctx->ssl, value);
  /* NB: ctx == NULL performs syntax checking only */
  else
...
static int cmd_Groups(SSL_CONF_CTX* cctx, const char* value) {
  int rv;
  if (cctx->ssl) rv = SSL_set1_groups_list(cctx->ssl, value);
  /* NB: ctx == NULL performs syntax checking only */
  else
...
static int cmd_CipherString(SSL_CONF_CTX* cctx, const char* value) {
  ...
  if (cctx->ctx) rv = SSL_CTX_set_cipher_list(cctx->ctx, value);
  if (cctx->ssl) rv = SSL_set_cipher_list(cctx->ssl, value);
  return rv > 0;
}
...
static int cmd_Ciphersuites(SSL_CONF_CTX* cctx, const char* value) {
  ...
  if (cctx->ctx) rv = SSL_CTX_set_ciphersuites(cctx->ctx, value);
  if (cctx->ssl) rv = SSL_set_ciphersuites(cctx->ssl, value);
  return rv > 0;
}
...
static int cmd_PrivateKey(SSL_CONF_CTX* cctx, const char* value) {
  int rv = 1;
  if (!(cctx->flags & SSL_CONF_FLAG_CERTIFICATE)) return -2;
  if (cctx->ctx)
...
static int cmd_ServerInfoFile(SSL_CONF_CTX* cctx, const char* value) {
  int rv = 1;
  if (cctx->ctx) rv = SSL_CTX_use_serverinfo_file(cctx->ctx, value);
  return rv > 0;
}
...
static int cmd_RequestCAFile(SSL_CONF_CTX* cctx, const char* value) {
  if (cctx->canames == NULL) cctx->canames = sk_X509_NAME_new_null();
  if (cctx->canames == NULL) return 0;
...
static int cmd_DHParameters(SSL_CONF_CTX* cctx, const char* value) {
  ...
  if (cctx->ctx != NULL || cctx->ssl != NULL) {
    ...
    if (in == NULL) goto end;
    if (BIO_read_filename(in, value) <= 0) goto end;
  ...
  if (cctx->ctx != NULL) {
    if ((rv = SSL_CTX_set0_tmp_dh_pkey(cctx->ctx, dhpkey)) > 0) dhpkey = NULL;
  }
  if (cctx->ssl != NULL) {
    if ((rv = SSL_set0_tmp_dh_pkey(cctx->ssl, dhpkey)) > 0) dhpkey = NULL;
  }
end:
...
static int cmd_RecordPadding(SSL_CONF_CTX* cctx, const char* value) {
  ...
  if (commap != NULL) {
    *commap = '\0';
    if (*(commap + 1) == '\0') goto out;
    if (!OPENSSL_strtoul(commap + 1, &endptr, 0, &hs_padding)) goto out;
  }
  if (!OPENSSL_strtoul(copy, &endptr, 0, &block_padding)) goto out;
  if (commap == NULL) hs_padding = block_padding;
...
static int cmd_NumTickets(SSL_CONF_CTX* cctx, const char* value) {
  ...
  if (num_tickets >= 0) {
    if (cctx->ctx) rv = SSL_CTX_set_num_tickets(cctx->ctx, num_tickets);
    if (cctx->ssl) rv = SSL_set_num_tickets(cctx->ssl, num_tickets);
  }
  return rv;
}
...
static int ssl_conf_cmd_skip_prefix(SSL_CONF_CTX* cctx, const char** pcmd) {
  if (pcmd == NULL || *pcmd == NULL) return 0;
  /* If a prefix is set, check and skip */
  if (cctx->prefix) {
    if (strlen(*pcmd) <= cctx->prefixlen) return 0;
...
static int ssl_conf_cmd_allowed(SSL_CONF_CTX* cctx, const ssl_conf_cmd_tbl* t) {
  ...
  if ((tfl & SSL_CONF_FLAG_SERVER) && !(cfl & SSL_CONF_FLAG_SERVER)) return 0;
  if ((tfl & SSL_CONF_FLAG_CLIENT) && !(cfl & SSL_CONF_FLAG_CLIENT)) return 0;
...
static const ssl_conf_cmd_tbl* ssl_conf_cmd_lookup(SSL_CONF_CTX* cctx,
  ...
  for (i = 0, t = ssl_conf_cmds; i < OSSL_NELEM(ssl_conf_cmds); i++, t++) {
    if (ssl_conf_cmd_allowed(cctx, t)) {
      if (cctx->flags & SSL_CONF_FLAG_CMDLINE) {
        if (t->str_cmdline && strcmp(t->str_cmdline, cmd) == 0) return t;
      }
      if (cctx->flags & SSL_CONF_FLAG_FILE) {
        if (t->str_file && OPENSSL_strcasecmp(t->str_file, cmd) == 0) return t;
      }
    }
  }
  return NULL;
...
int SSL_CONF_cmd(SSL_CONF_CTX* cctx, const char* cmd, const char* value) {
  ...
  if (!ssl_conf_cmd_skip_prefix(cctx, &cmd)) goto unknown_cmd;
...
int SSL_CONF_cmd_argv(SSL_CONF_CTX* cctx, int* pargc, char*** pargv) {
  ...
  if (pargc != NULL && *pargc == 0) return 0;
  if (pargc == NULL || *pargc > 0) arg = **pargv;
...
int SSL_CONF_cmd_value_type(SSL_CONF_CTX* cctx, const char* cmd) {
  if (ssl_conf_cmd_skip_prefix(cctx, &cmd)) {
    const ssl_conf_cmd_tbl* runcmd;
    runcmd = ssl_conf_cmd_lookup(cctx, cmd);
    if (runcmd) return runcmd->value_type;
  }
  ...
  if (c != NULL && cctx->flags & SSL_CONF_FLAG_REQUIRE_PRIVATE) {
    for (i = 0; i < cctx->num_cert_filename; i++) {
      ...
      if (p != NULL && c->pkeys[i].privatekey == NULL) {
        if (!cmd_PrivateKey(cctx, p)) return 0;
      }
    }
  }
  if (cctx->canames) {
...
void SSL_CONF_CTX_set_ssl(SSL_CONF_CTX* cctx, SSL* ssl) {
  ...
  if (ssl != NULL) {
    ...
    if (cctx->cert_filename != NULL) cctx->num_cert_filename = sc->ssl_pkey_num;
  } else {
    cctx->poptions = NULL;
