...
static void
ssl_session_oinit(ASN1_OCTET_STRING** dest,
...
static void
ssl_session_sinit(ASN1_OCTET_STRING** dest,
                  ASN1_OCTET_STRING* os,
                  const char* data)
{
  if (data != NULL)
...
int
i2d_SSL_SESSION(const SSL_SESSION* in, unsigned char** pp)
{
  ...
  ssl_session_oinit(
    &as.master_key, &master_key, in->master_key, in->master_key_length);

  ssl_session_oinit(
    &as.session_id, &session_id, in->session_id, in->session_id_length);

  ssl_session_oinit(
    &as.session_id_context, &sid_ctx, in->sid_ctx, in->sid_ctx_length);
  ...
  if (in->ext.tick) {
    ssl_session_oinit(
      &as.tlsext_tick, &tlsext_tick, in->ext.tick, in->ext.ticklen);
  }
...
#ifndef OPENSSL_NO_PSK
  ssl_session_sinit(
    &as.psk_identity_hint, &psk_identity_hint, in->psk_identity_hint);
...
static int
ssl_session_strndup(char** pdst, ASN1_OCTET_STRING* src)
{
  OPENSSL_free(*pdst);
  *pdst = NULL;
  if (src == NULL)
...
static int
ssl_session_memcpy(unsigned char* dst,
                   size_t* pdstlen,
...
SSL_SESSION*
d2i_SSL_SESSION_ex(SSL_SESSION** a,
                   ...
                   const char* propq)
{
  ...
  if (!ssl_session_memcpy(ret->master_key,
                          &tmpl,
                          as->master_key,
  ...
  if (!ssl_session_memcpy(ret->sid_ctx,
                          &ret->sid_ctx_length,
                          as->session_id_context,
