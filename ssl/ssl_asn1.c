...
int i2d_SSL_SESSION(const SSL_SESSION* in, unsigned char** pp) {
  ...
  if (in->peer_rpk != NULL) {
    ...
    if (peer_rpk.length > 0 && peer_rpk.data != NULL) as.peer_rpk = &peer_rpk;
  }
...
static int ssl_session_memcpy(unsigned char* dst, size_t* pdstlen,
  ...
  if (src->length < 0 || src->length > (int)maxlen) return 0;
...
SSL_SESSION* d2i_SSL_SESSION_ex(SSL_SESSION** a, const unsigned char** pp,
  ...
  if (!ssl_session_strndup(&ret->ext.hostname, as->tlsext_hostname)) goto err;

#ifndef OPENSSL_NO_PSK
  if (!ssl_session_strndup(&ret->psk_identity_hint, as->psk_identity_hint))
    goto err;
  if (!ssl_session_strndup(&ret->psk_identity, as->psk_identity)) goto err;
#endif
...
#ifndef OPENSSL_NO_SRP
  if (!ssl_session_strndup(&ret->srp_username, as->srp_username)) goto err;
#endif /* OPENSSL_NO_SRP */
  ...
  if ((a != NULL) && (*a == NULL)) *a = ret;
  *pp = p;
  return ret;

err:
  M_ASN1_free_of(as, SSL_SESSION_ASN1);
  if ((a == NULL) || (*a != ret)) SSL_SESSION_free(ret);
  return NULL;
}
