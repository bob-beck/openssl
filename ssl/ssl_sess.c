...
SSL_SESSION *
lookup_sess_in_cache (SSL_CONNECTION *s, const unsigned char *sess_id,
                      size_t sess_id_len)
{
...
void
SSL_SESSION_get0_ticket (const SSL_SESSION *s, const unsigned char **tick,
                         size_t *len)
{
  *len = s->ext.ticklen;
  if (tick != NULL)
...
int
SSL_SESSION_set1_alpn_selected (SSL_SESSION *s, const unsigned char *alpn,
                                size_t len)
{
...
SSL_SESSION *(*SSL_CTX_sess_get_get_cb (SSL_CTX *ctx)) (
    SSL *ssl, const unsigned char *data, int len, int *copy)
{
