...
/* variant of SSL_get_session: caller really gets something */
{
   ...
   */
  if (!CRYPTO_THREAD_read_lock(ssl->lock)) return NULL;
  sess = SSL_get_session(ssl);
  if (sess != NULL && !SSL_SESSION_up_ref(sess)) sess = NULL;
...
static SSL_SESSION* ssl_session_dup_intern(const SSL_SESSION* src, int ticket) {
  ...
  if (src->peer_rpk != NULL) {
    if (!EVP_PKEY_up_ref(src->peer_rpk)) goto err;
  ...
  if (src->psk_identity_hint) {
    ...
    if (dest->psk_identity_hint == NULL) goto err;
  }
  if (src->psk_identity) {
    dest->psk_identity = OPENSSL_strdup(src->psk_identity);
    if (dest->psk_identity == NULL) goto err;
  }
#endif
  ...
  if (src->ext.hostname) {
    ...
    if (dest->ext.hostname == NULL) goto err;
  }

  if (ticket != 0 && src->ext.tick != NULL) {
    dest->ext.tick = OPENSSL_memdup(src->ext.tick, src->ext.ticklen);
    if (dest->ext.tick == NULL) goto err;
  } else {
  ...
  if (src->ext.alpn_selected != NULL) {
    ...
    if (dest->ext.alpn_selected == NULL) goto err;
  }

#ifndef OPENSSL_NO_SRP
  if (src->srp_username) {
    dest->srp_username = OPENSSL_strdup(src->srp_username);
    if (dest->srp_username == NULL) goto err;
  }
#endif
  ...
  if (src->ticket_appdata != NULL) {
    ...
    if (dest->ticket_appdata == NULL) goto err;
  }
...
SSL_SESSION* ssl_session_dup(const SSL_SESSION* src, int ticket) {
  ...
  if (sess != NULL) sess->not_resumable = 0;
...
const unsigned char* SSL_SESSION_get0_id_context(const SSL_SESSION* s,
                                                 unsigned int* len) {
  if (len != NULL) *len = (unsigned int)s->sid_ctx_length;
  return s->sid_ctx;
}
...
static int def_generate_session_id(SSL* ssl, unsigned char* id,
  ...
  do {
    if (RAND_bytes_ex(ssl->ctx->libctx, id, *id_len, 0) <= 0) return 0;
  ...
  if (retry < MAX_SESS_ID_ATTEMPTS) return 1;
...
int ssl_generate_session_id(SSL_CONNECTION* s, SSL_SESSION* ss) {
  ...
  if (!CRYPTO_THREAD_read_lock(SSL_CONNECTION_GET_SSL(s)->lock)) return 0;
...
int ssl_get_new_session(SSL_CONNECTION* s, int session) {
  ...
  if (s->s3.flags & TLS1_FLAGS_RECEIVED_EXTMS) ss->flags |= SSL_SESS_FLAG_EXTMS;
...
SSL_SESSION* lookup_sess_in_cache(SSL_CONNECTION* s,
  ...
  if ((s->session_ctx->session_cache_mode &
    ...
    if (!ossl_assert(sess_id_len <= SSL_MAX_SSL_SESSION_ID_LENGTH)) return NULL;

    memcpy(data.session_id, sess_id, sess_id_len);
    data.session_id_length = sess_id_len;

    if (!CRYPTO_THREAD_read_lock(s->session_ctx->lock)) return NULL;
  ...
  if (ret == NULL && s->session_ctx->get_session_cb != NULL) {
    ...
    if (ret != NULL) {
       ...
       * thread-safe).
       */
      if (copy && !SSL_SESSION_up_ref(ret)) return NULL;
...
int ssl_get_prev_session(SSL_CONNECTION* s, CLIENTHELLO_MSG* hello) {
    ...
    switch (r) {
      ...
      case SSL_TICKET_EMPTY:
        if (hello->session_id_len > 0) {
          try_session_cache = 1;
          ret =
              lookup_sess_in_cache(s, hello->session_id, hello->session_id_len);
        }
        break;
  ...
  if (ret->ssl_version != s->version) goto err;
...
err:
  if (ret != NULL) {
    ...
    if (SSL_CONNECTION_IS_TLS13(s)) s->session = NULL;
...
int SSL_CTX_add_session(SSL_CTX* ctx, SSL_SESSION* c) {
   ...
   * an lhash
   */
  if (!SSL_SESSION_up_ref(c)) return 0;
  /*
...
static int remove_session_lock(SSL_CTX* ctx, SSL_SESSION* c, int lck) {
  ...
  if ((c != NULL) && (c->session_id_length != 0)) {
    ...
    if (lck) CRYPTO_THREAD_unlock(ctx->lock);

    if (ctx->remove_session_cb != NULL) ctx->remove_session_cb(ctx, c);

    if (ret) SSL_SESSION_free(r);
  }
  return ret;
}
...
int SSL_SESSION_up_ref(SSL_SESSION* ss) {
  ...
  if (CRYPTO_UP_REF(&ss->references, &i) <= 0) return 0;
...
int SSL_set_session(SSL* s, SSL_SESSION* session) {
  ...
  if (session != NULL) sc->verify_result = session->verify_result;
...
int SSL_SESSION_set1_id(SSL_SESSION* s, const unsigned char* sid,
  ...
  if (sid != s->session_id && sid_len > 0) memcpy(s->session_id, sid, sid_len);
...
long SSL_SESSION_set_timeout(SSL_SESSION* s, long t) {
  ...
  if (s == NULL || t < 0) return 0;
  if (s->owner != NULL) {
    if (!CRYPTO_THREAD_write_lock(s->owner->lock)) return 0;
...
int SSL_SESSION_set1_id_context(SSL_SESSION* s, const unsigned char* sid_ctx,
  ...
  if (sid_ctx != s->sid_ctx) memcpy(s->sid_ctx, sid_ctx, sid_ctx_len);
...
void SSL_CTX_flush_sessions_ex(SSL_CTX* s, time_t t) {
  ...
  if (!CRYPTO_THREAD_write_lock(s->lock)) return;
  ...
  while (s->session_cache_tail != NULL) {
    ...
    if (t == 0 || sess_timedout(timeout, current)) {
      ...
      if (s->remove_session_cb != NULL) s->remove_session_cb(s, current);
      /*
...
static void SSL_SESSION_list_add(SSL_CTX* ctx, SSL_SESSION* s) {
  ...
  if ((s->next != NULL) && (s->prev != NULL)) SSL_SESSION_list_remove(ctx, s);
