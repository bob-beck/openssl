...
sess_timedout(OSSL_TIME t, SSL_SESSION* ss)
{
 ...
 */
__owur static ossl_inline int
timeoutcmp(SSL_SESSION* a, SSL_SESSION* b)
{
 ...
 */
static SSL_SESSION*
ssl_session_dup_intern(const SSL_SESSION* src, int ticket)
{
  SSL_SESSION* dest;
  ...
  if (!CRYPTO_dup_ex_data(
        CRYPTO_EX_INDEX_SSL_SESSION, &dest->ex_data, &src->ex_data)) {
    ERR_raise(ERR_LIB_SSL, ERR_R_CRYPTO_LIB);
    goto err;
  }
...
static int
def_generate_session_id(SSL* ssl, unsigned char* id, unsigned int* id_len)
{
                     ...
                     size_t sess_id_len)
{
  ...
  if (ret == NULL && s->session_ctx->get_session_cb != NULL) {
    ...
    ret = s->session_ctx->get_session_cb(
      SSL_CONNECTION_GET_USER_SSL(s), sess_id, (int)sess_id_len, &copy);
...
int
SSL_SESSION_set1_id(SSL_SESSION* s,
...
int
SSL_SESSION_set1_hostname(SSL_SESSION* s, const char* hostname)
{
...
void
SSL_SESSION_get0_ticket(const SSL_SESSION* s,
...
void
SSL_SESSION_get0_alpn_selected(const SSL_SESSION* s,
...
int
SSL_SESSION_set1_alpn_selected(SSL_SESSION* s,
                               const unsigned char* alpn,
                               size_t len)
{
...
EVP_PKEY*
SSL_SESSION_get0_peer_rpk(SSL_SESSION* s)
{
  return s->peer_rpk;
}

int
SSL_SESSION_set1_id_context(SSL_SESSION* s,
...
void
SSL_CTX_flush_sessions_ex(SSL_CTX* s, time_t t)
{
...
SSL_CTX_sess_set_get_cb(
  SSL_CTX* ctx,
  SSL_SESSION* (*cb)(SSL* ssl, const unsigned char* data, int len, int* copy))
{
...
int
SSL_SESSION_set1_ticket_appdata(SSL_SESSION* ss, const void* data, size_t len)
{
  OPENSSL_free(ss->ticket_appdata);
...
int
SSL_SESSION_get0_ticket_appdata(SSL_SESSION* ss, void** data, size_t* len)
{
...
void
SSL_CTX_set_stateless_cookie_generate_cb(SSL_CTX* ctx,
                                         int (*cb)(SSL* ssl,
                                                   unsigned char* cookie,
                                                   size_t* cookie_len))
{
...
void
SSL_CTX_set_stateless_cookie_verify_cb(SSL_CTX* ctx,
                                       int (*cb)(SSL* ssl,
                                                 const unsigned char* cookie,
                                                 size_t cookie_len))
{
