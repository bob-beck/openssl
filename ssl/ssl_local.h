...
#define DTLS_VERSION_GE(v1, v2)                                               \
  (dtls_ver_ordinal (v1) <= dtls_ver_ordinal (v2))
#define DTLS_VERSION_LT(v1, v2) (dtls_ver_ordinal (v1) > dtls_ver_ordinal (v2))
#define DTLS_VERSION_LE(v1, v2)                                               \
  (dtls_ver_ordinal (v1) >= dtls_ver_ordinal (v2))
 ...
 * context Verify_result [ 5 ] EXPLICIT INTEGER,   -- X509_V_... code for
 * `Peer' HostName [ 6 ] EXPLICIT OCTET STRING,   -- optional HostName from
...
/*
 * The allowance we have between the client's calculated ticket age and our
 * own. We allow for 10 seconds. If a ticket is presented and the client's age
 * calculation is different by more than this than our own then we do not allow
 * that ticket for early_data.
 */
...
#define TLS_GROUP_FFDHE_FOR_TLS1_3                                            \
  (TLS_GROUP_FFDHE | TLS_GROUP_ONLY_FOR_TLS1_3)
...
#define ssl_close_construct_packet(s, pkt, htype)                             \
  SSL_CONNECTION_GET_SSL (s)->method->ssl3_enc->close_construct_packet (      \
      (s), (pkt), (htype))
#define ssl_do_write(s)                                                       \
  SSL_CONNECTION_GET_SSL (s)->method->ssl3_enc->do_write (s)
...
tls1_get_peer_groups (SSL_CONNECTION *s, const uint16_t **pgroups,
                      size_t *pgroupslen)
{
...
__owur int ssl_read_internal (SSL *s, void *buf, size_t num,
                              size_t *readbytes);
...
__owur int ssl_ctx_security (const SSL_CTX *ctx, int op, int bits, int nid,
                             void *other);
int ssl_get_security_level_bits (const SSL *s, const SSL_CTX *ctx,
                                 int *levelp);
...
__owur int ssl_gensecret (SSL_CONNECTION *s, unsigned char *pms,
                          size_t pmslen);
...
__owur int ssl_version_cmp (const SSL_CONNECTION *s, int versiona,
                            int versionb);
...
__owur int ssl_choose_server_version (SSL_CONNECTION *s,
                                      CLIENTHELLO_MSG *hello, DOWNGRADE *dgrd);
...
__owur int tls1_set_groups (uint16_t **grpext, size_t *grpextlen,
                            uint16_t **ksext, size_t *ksextlen,
                            size_t **tplext, size_t *tplextlen, int *curves,
                            size_t ncurves);
...
__owur int tls_valid_group (SSL_CONNECTION *s, uint16_t group_id,
                            int minversion, int maxversion, int isec,
                            int *okfortls13);
...
__owur int tls1_set_sigalgs_list (SSL_CTX *ctx, CERT *c, const char *str,
                                  int client);
__owur int tls1_set_raw_sigalgs (CERT *c, const uint16_t *psigs,
                                 size_t salglen, int client);
...
__owur int ssl_security_cert (SSL_CONNECTION *s, SSL_CTX *ctx, X509 *x,
                              int vfy, int is_ee);
...
__owur int tls1_set_peer_legacy_sigalg (SSL_CONNECTION *s,
                                        const EVP_PKEY *pkey);
...
__owur int custom_ext_add (SSL_CONNECTION *s, int context, WPACKET *pkt,
                           X509 *x, size_t chainidx, int maxversion);
