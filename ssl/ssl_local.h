...
int ssl_hmac_final(SSL_HMAC* ctx,
                   unsigned char* md,
                   size_t* len,
                   size_t max_size);
...
__owur int ossl_ssl_init(SSL* ssl,
                         SSL_CTX* ctx,
                         const SSL_METHOD* method,
                         int type);
__owur SSL* ossl_ssl_connection_new_int(SSL_CTX* ctx,
                                        SSL* user_ssl,
                                        const SSL_METHOD* method);
...
__owur int ssl_write_internal(SSL* s,
                              const void* buf,
                              size_t num,
                              uint64_t flags,
                              size_t* written);
...
__owur STACK_OF(SSL_CIPHER)* ssl_create_cipher_list(
    SSL_CTX* ctx,
    STACK_OF(SSL_CIPHER)* tls13_ciphersuites,
    STACK_OF(SSL_CIPHER)** cipher_list,
    STACK_OF(SSL_CIPHER)** cipher_list_by_id,
    const char* rule_str,
    CERT* c);
__owur int ssl_cache_cipherlist(SSL_CONNECTION* s,
...
__owur int ssl_cipher_get_evp_cipher(SSL_CTX* ctx,
...
__owur const SSL_CIPHER* ssl_get_cipher_by_char(SSL_CONNECTION* ssl,
                                                ...
                                                int all);
__owur int ssl_cert_set0_chain(SSL_CONNECTION* s,
                               SSL_CTX* ctx,
                               STACK_OF(X509)* chain);
__owur int ssl_cert_set1_chain(SSL_CONNECTION* s,
                               SSL_CTX* ctx,
                               STACK_OF(X509)* chain);
...
__owur int ssl_cert_set_cert_store(CERT* c,
                                   X509_STORE* store,
                                   int chain,
                                   int ref);
__owur int ssl_cert_get_cert_store(CERT* c, X509_STORE** pstore, int chain);

__owur int ssl_security(const SSL_CONNECTION* s,
                        int op,
                        int bits,
                        int nid,
                        void* other);
__owur int ssl_ctx_security(const SSL_CTX* ctx,
                            int op,
                            int bits,
                            int nid,
                            void* other);
...
__owur int ssl_fill_hello_random(SSL_CONNECTION* s,
                                 int server,
                                 unsigned char* field,
                                 size_t len,
                                 DOWNGRADE dgrd);
__owur int ssl_generate_master_secret(SSL_CONNECTION* s,
                                      unsigned char* pms,
                                      size_t pmslen,
                                      int free_pms);
...
__owur int ssl_derive(SSL_CONNECTION* s,
                      EVP_PKEY* privkey,
                      EVP_PKEY* pubkey,
                      int genmaster);
__owur int ssl_decapsulate(SSL_CONNECTION* s,
                           EVP_PKEY* privkey,
                           const unsigned char* ct,
                           size_t ctlen,
                           int gensecret);
__owur int ssl_encapsulate(SSL_CONNECTION* s,
                           EVP_PKEY* pubkey,
                           unsigned char** ctp,
                           size_t* ctlenp,
                           int gensecret);
...
__owur int ssl3_put_cipher_by_char(const SSL_CIPHER* c,
                                   WPACKET* pkt,
                                   size_t* len);
...
__owur int ssl3_generate_master_secret(SSL_CONNECTION* s,
                                       unsigned char* out,
                                       unsigned char* p,
                                       size_t len,
                                       size_t* secret_size);
...
__owur size_t ssl3_final_finish_mac(SSL_CONNECTION* s,
                                    const char* sender,
                                    size_t slen,
                                    unsigned char* p);
__owur int ssl3_finish_mac(SSL_CONNECTION* s,
                           const unsigned char* buf,
                           size_t len);
void ssl3_free_digest_list(SSL_CONNECTION* s);
__owur unsigned long ssl3_output_cert_chain(SSL_CONNECTION* s,
                                            WPACKET* pkt,
                                            CERT_PKEY* cpk,
                                            int for_comp);
...
__owur int ssl3_set_handshake_header(SSL_CONNECTION* s,
                                     WPACKET* pkt,
                                     int htype);
__owur int tls_close_construct_packet(SSL_CONNECTION* s,
                                      WPACKET* pkt,
                                      int htype);
__owur int tls_setup_handshake(SSL_CONNECTION* s);
__owur int dtls1_set_handshake_header(SSL_CONNECTION* s,
                                      WPACKET* pkt,
                                      int htype);
__owur int dtls1_close_construct_packet(SSL_CONNECTION* s,
                                        WPACKET* pkt,
                                        int htype);
...
__owur int ssl_version_supported(const SSL_CONNECTION* s,
                                 int version,
...
__owur int ssl_choose_server_version(SSL_CONNECTION* s,
                                     CLIENTHELLO_MSG* hello,
                                     DOWNGRADE* dgrd);
__owur int ssl_choose_client_version(SSL_CONNECTION* s,
                                     int version,
                                     RAW_EXTENSION* extensions);
__owur int ssl_get_min_max_version(const SSL_CONNECTION* s,
                                   int* min_version,
                                   int* max_version,
                                   int* real_max);
...
__owur int dtls1_retransmit_message(SSL_CONNECTION* s,
                                    unsigned short seq,
                                    int* found);
...
__owur int dtls_raw_hello_verify_request(WPACKET* pkt,
                                         unsigned char* cookie,
                                         size_t cookie_len);
...
__owur size_t tls1_final_finish_mac(SSL_CONNECTION* s,
                                    const char* str,
                                    size_t slen,
                                    unsigned char* p);
__owur int tls1_generate_master_secret(SSL_CONNECTION* s,
                                       unsigned char* out,
                                       unsigned char* p,
                                       size_t len,
                                       size_t* secret_size);
__owur int tls13_setup_key_block(SSL_CONNECTION* s);
__owur size_t tls13_final_finish_mac(SSL_CONNECTION* s,
                                     const char* str,
                                     size_t slen,
                                     unsigned char* p);
...
__owur int tls13_hkdf_expand_ex(OSSL_LIB_CTX* libctx,
                                ...
                                size_t outlen,
                                int raise_error);
__owur int tls13_derive_key(SSL_CONNECTION* s,
                            const EVP_MD* md,
                            const unsigned char* secret,
                            unsigned char* key,
                            size_t keylen);
__owur int tls13_derive_iv(SSL_CONNECTION* s,
                           const EVP_MD* md,
                           const unsigned char* secret,
                           unsigned char* iv,
                           size_t ivlen);
...
__owur int tls1_check_group_id(SSL_CONNECTION* s,
                               uint16_t group_id,
...
__owur int tls_valid_group(SSL_CONNECTION* s,
                           uint16_t group_id,
                           int minversion,
                           int maxversion,
                           int isec,
...
void tls1_get_formatlist(SSL_CONNECTION* s,
...
void tls1_get_supported_groups(SSL_CONNECTION* s,
                               const uint16_t** pgroups,
                               size_t* pgroupslen);
void tls1_get_requested_keyshare_groups(SSL_CONNECTION* s,
                                        const uint16_t** pgroups,
                                        size_t* pgroupslen);
void tls1_get_group_tuples(SSL_CONNECTION* s,
...
__owur int tls1_set_sigalgs_list(SSL_CTX* ctx,
                                 CERT* c,
                                 const char* str,
                                 int client);
__owur int tls1_set_raw_sigalgs(CERT* c,
                                const uint16_t* psigs,
                                size_t salglen,
                                int client);
__owur int tls1_set_sigalgs(CERT* c,
                            const int* salg,
                            size_t salglen,
                            int client);
int tls1_check_chain(SSL_CONNECTION* s,
                     X509* x,
                     EVP_PKEY* pk,
                     STACK_OF(X509)* chain,
                     int idx);
...
__owur int ssl_security_cert(SSL_CONNECTION* s,
                             SSL_CTX* ctx,
                             X509* x,
                             int vfy,
                             int is_ee);
__owur int ssl_security_cert_chain(SSL_CONNECTION* s,
                                   STACK_OF(X509)* sk,
                                   X509* ex,
                                   int vfy);
...
__owur int tls12_copy_sigalgs(SSL_CONNECTION* s,
                              WPACKET* pkt,
                              const uint16_t* psig,
                              size_t psiglen);
...
__owur int tls1_lookup_md(SSL_CTX* ctx,
                          const SIGALG_LOOKUP* lu,
                          const EVP_MD** pmd);
__owur size_t tls12_get_psigalgs(SSL_CONNECTION* s,
                                 int sent,
                                 const uint16_t** psigs);
...
__owur int ssl_cipher_disabled(const SSL_CONNECTION* s,
                               const SSL_CIPHER* c,
                               int op,
                               int echde);
 ...
 */
__owur int ssl_log_secret(SSL_CONNECTION* s,
                          const char* label,
...
__owur int custom_ext_parse(SSL_CONNECTION* s,
                            ...
                            X509* x,
                            size_t chainidx);
__owur int custom_ext_add(SSL_CONNECTION* s,
                          int context,
                          WPACKET* pkt,
                          X509* x,
                          size_t chainidx,
...
int tls_engine_load_ssl_client_cert(SSL_CONNECTION* s,
                                    X509** px509,
                                    EVP_PKEY** ppkey);
