...
typedef struct ssl3_enc_method {
  ...
  int (*generate_master_secret)(SSL_CONNECTION*, unsigned char*, unsigned char*,
                                size_t, size_t*);
...
static ossl_inline int ssl_has_cert(const SSL_CONNECTION* s, int idx) {
  if (idx < 0 || idx >= (int)s->ssl_pkey_num) return 0;
...
#ifdef TSAN_REQUIRES_LOCKING
  if (!CRYPTO_THREAD_write_lock(ctx->tsan_lock)) return 0;
#endif
  return 1;
}
