...
size_t
ssl3_final_finish_mac (SSL_CONNECTION *s, const char *sender, size_t len,
                       unsigned char *p)
{
  int ret;
  EVP_MD_CTX *ctx = NULL;
...
int
ssl3_generate_master_secret (SSL_CONNECTION *s, unsigned char *out,
                             unsigned char *p, size_t len, size_t *secret_size)
{
  ...
  for (i = 0; i < 3; i++)
    {
      if (EVP_DigestInit_ex (ctx, SSL_CONNECTION_GET_CTX (s)->sha1, NULL) <= 0
          ...
          || EVP_DigestInit_ex (ctx, SSL_CONNECTION_GET_CTX (s)->md5, NULL)
                 <= 0
          || EVP_DigestUpdate (ctx, p, len) <= 0
