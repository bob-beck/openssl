...
static int
cipher_compare(const void* a, const void* b)
{
...
{
  qsort(
    tls13_ciphers, TLS13_NUM_CIPHERS, sizeof(tls13_ciphers[0]), cipher_compare);
  qsort(
    ssl3_ciphers, SSL3_NUM_CIPHERS, sizeof(ssl3_ciphers[0]), cipher_compare);
  qsort(ssl3_scsvs, SSL3_NUM_SCSVS, sizeof(ssl3_scsvs[0]), cipher_compare);
}

static int
sslcon_undefined_function_1(SSL_CONNECTION* sc,
                            unsigned char* r,
                            size_t s,
                            const char* t,
                            size_t u,
...
static int
ssl3_set_req_cert_type(CERT* c, const unsigned char* p, size_t len);
...
ssl3_ctx_callback_ctrl(SSL_CTX* ctx, int cmd, void (*fp)(void))
{
  switch (cmd) {
    ...
    case SSL_CTRL_SET_TLSEXT_TICKET_KEY_CB:
      ctx->ext.ticket_key_cb = (int (*)(
        SSL*, unsigned char*, unsigned char*, EVP_CIPHER_CTX*, HMAC_CTX*, int))
        fp;
      break;
#endif
...
int
SSL_CTX_set_tlsext_ticket_key_evp_cb(SSL_CTX* ctx,
                                     int (*fp)(SSL*,
                                               unsigned char*,
                                               unsigned char*,
                                               EVP_CIPHER_CTX*,
                   ...
                   STACK_OF(SSL_CIPHER)* srvr)
{
  ...
  for (i = 0; i < sk_SSL_CIPHER_num(prio); i++) {
    ...
    if (ii >= 0) {
      ...
      if (!ssl_security(
            s, SSL_SECOP_CIPHER_SHARED, c->strength_bits, 0, (void*)c))
        continue;
...
ssl3_write(SSL* s, const void* buf, size_t len, size_t* written)
{
  ...
  return s->method->ssl_write_bytes(
    s, SSL3_RT_APPLICATION_DATA, buf, len, written);
}

static int
ssl3_read_internal(SSL* s, void* buf, size_t len, int peek, size_t* readbytes)
{
  int ret;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL_ONLY(s);
  ...
  ret = s->method->ssl_read_bytes(
    s, SSL3_RT_APPLICATION_DATA, NULL, buf, len, peek, readbytes);
 ...
 */
int
ssl3_renegotiate_check(SSL* s, int initok)
{
  int ret = 0;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL_ONLY(s);
...
const char*
SSL_group_to_name(SSL* s, int nid)
{
  int group_id = 0;
