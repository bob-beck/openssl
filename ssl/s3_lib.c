...
int ssl3_set_handshake_header(SSL_CONNECTION* s, WPACKET* pkt, int htype) {
  ...
  if (htype == SSL3_MT_CHANGE_CIPHER_SPEC) return 1;
...
void ssl3_free(SSL* s) {
  ...
  for (i = 0; i < sc->s3.tmp.num_ks_pkey; i++)
    if (sc->s3.tmp.ks_pkey[i] != NULL) {
      if (sc->s3.tmp.pkey == sc->s3.tmp.ks_pkey[i]) sc->s3.tmp.pkey = NULL;
...
int ssl3_clear(SSL* s) {
  ...
  if (!ssl_free_wbio_buffer(sc)) return 0;
  ...
  switch (cmd) {
    ...
    case SSL_CTRL_SET_TLSEXT_STATUS_REQ_OCSP_RESP:
      ...
      if (parg != NULL) {
        ...
        if (sc->ext.ocsp.resp_ex == NULL) return 0;

        p = parg;
        resp = d2i_OCSP_RESPONSE(NULL, (const unsigned char**)&p, larg);
        if (resp != NULL) sk_OCSP_RESPONSE_push(sc->ext.ocsp.resp_ex, resp);
      }
#endif
      break;
    ...
    case SSL_CTRL_SET_CURRENT_CERT:
      if (larg == SSL_CERT_SET_SERVER) {
         ...
         * authentication
         */
        if (cipher->algorithm_auth & (SSL_aNULL | SSL_aSRP)) return 2;
        if (sc->s3.tmp.cert == NULL) return 0;
    ...
    case SSL_CTRL_GET_SHARED_GROUP: {
      ...
      if (larg != -1) return tls1_group_id2nid(id, 1);
      return id;
    }
    ...
    case SSL_CTRL_GET_CLIENT_CERT_TYPES: {
      ...
      if (sc->server || !sc->s3.tmp.cert_req) return 0;
      if (pctype) *pctype = sc->s3.tmp.ctype;
    ...
    case SSL_CTRL_GET_PEER_SIGNATURE_NAME:
      if (parg == NULL || sc->s3.tmp.peer_sigalg == NULL) return 0;
    ...
    case SSL_CTRL_GET_SIGNATURE_NAME:
      if (parg == NULL || sc->s3.tmp.sigalg == NULL) return 0;
    ...
    case SSL_CTRL_GET_PEER_TMP_KEY:
      if (sc->session == NULL || sc->s3.peer_tmp == NULL) {
        ...
        if (!EVP_PKEY_up_ref(sc->s3.peer_tmp)) return 0;
    ...
    case SSL_CTRL_GET_TMP_KEY:
      if (sc->session == NULL || sc->s3.tmp.pkey == NULL) {
        ...
        if (!EVP_PKEY_up_ref(sc->s3.tmp.pkey)) return 0;
    ...
    case SSL_CTRL_GET_EC_POINT_FORMATS: {
      ...
      if (sc->ext.peer_ecpointformats == NULL) return 0;
...
long ssl3_ctx_ctrl(SSL_CTX* ctx, int cmd, long larg, void* parg) {
  switch (cmd) {
    ...
    case SSL_CTRL_GET_TLSEXT_TICKET_KEYS: {
      ...
      if (keys == NULL) return tick_keylen;
      if (larg != tick_keylen) {
    ...
    case SSL_CTRL_SET_TLS_EXT_SRP_PASSWORD:
      ...
      if (ctx->srp_ctx.info != NULL) OPENSSL_free(ctx->srp_ctx.info);
...
int ssl3_put_cipher_by_char(const SSL_CIPHER* c, WPACKET* pkt, size_t* len) {
  ...
  if (!WPACKET_put_bytes_u16(pkt, c->id & 0xffff)) return 0;
...
const SSL_CIPHER* ssl3_choose_cipher(SSL_CONNECTION* s,
  ...
  for (i = 0; i < sk_SSL_CIPHER_num(prio); i++) {
    ...
    if (!SSL_CONNECTION_IS_TLS13(s)) {
      ...
      if ((alg_k & SSL_PSK) && s->psk_server_callback == NULL) continue;
       ...
       * EC key check it
       */
      if (alg_k & SSL_kECDHE) ok = ok && tls1_check_ec_tmp_key(s, c->id);
...
int ssl3_get_req_cert_type(SSL_CONNECTION* s, WPACKET* pkt) {
  ...
  if ((s->version == SSL3_VERSION) && (alg_k & SSL_kDHE)) {
    if (!WPACKET_put_bytes_u8(pkt, SSL3_CT_RSA_EPHEMERAL_DH)) return 0;
    if (!(alg_a & SSL_aDSS) &&
...
int ssl3_shutdown(SSL* s) {
  ...
  if (!(sc->shutdown & SSL_SENT_SHUTDOWN)) {
     ...
     */
    if (sc->s3.alert_dispatch > 0) return -1; /* return WANT_WRITE */
...
int ssl3_write(SSL* s, const void* buf, size_t len, size_t* written) {
  ...
  clear_sys_error();
  if (sc->s3.renegotiate) ssl3_renegotiate_check(s, 0);
...
int ssl3_renegotiate(SSL* s) {
  ...
  if (sc->handshake_func == NULL) return 1;
...
int ssl_generate_master_secret(SSL_CONNECTION* s, unsigned char* pms,
  ...
  if (alg_k & SSL_PSK) {
    ...
    if (alg_k & SSL_kPSK) pmslen = psklen;

    pskpmslen = 4 + pmslen + psklen;
    pskpms = OPENSSL_malloc(pskpmslen);
    if (pskpms == NULL) goto err;
    t = pskpms;
    s2n(pmslen, t);
    if (alg_k & SSL_kPSK)
      memset(t, 0, pmslen);
...
const char* SSL_group_to_name(SSL* s, int nid) {
  ...
  if (cinf != NULL) return cinf->tlsname;
  return NULL;
}
