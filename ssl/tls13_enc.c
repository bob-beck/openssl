...
int tls13_hkdf_expand_ex(OSSL_LIB_CTX* libctx, const char* propq,
  ...
  if ((ret = EVP_MD_get_size(md)) <= 0) {
    EVP_KDF_CTX_free(kctx);
    if (raise_error) ERR_raise(ERR_LIB_SSL, ERR_R_INTERNAL_ERROR);
    return 0;
  }
  hashlen = (size_t)ret;
...
int tls13_generate_secret(SSL_CONNECTION* s, const EVP_MD* md,
  ...
  *p++ = OSSL_PARAM_construct_end();
  ...
  if (ret != 0) SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
...
static int tls13_store_hash(SSL_CONNECTION* s, unsigned char* hash,
                            size_t len) {
  size_t hashlen;
...
int tls13_change_cipher_state(SSL_CONNECTION* s, int which) {
  ...
  if (!ossl_assert(cipher != NULL)) goto err;
...
err:
  ...
  if (iv != iv_intern) OPENSSL_free(iv);
  return ret;
}
...
int tls13_export_keying_material(SSL_CONNECTION* s, unsigned char* out,
  ...
  if (ctx == NULL || md == NULL || !ossl_statem_export_allowed(s)) goto err;

  if (!use_context) contextlen = 0;
...
int tls13_export_keying_material_early(SSL_CONNECTION* s, unsigned char* out,
  ...
  if (ctx == NULL || !ossl_statem_export_early_allowed(s)) goto err;
