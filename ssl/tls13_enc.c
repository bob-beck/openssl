                 ...
                 size_t keylen)
{
  ...
  return tls13_hkdf_expand(
    s, md, secret, keylabel, sizeof(keylabel) - 1, NULL, 0, key, keylen, 1);
}
                ...
                size_t ivlen)
{
  ...
  return tls13_hkdf_expand(
    s, md, secret, ivlabel, sizeof(ivlabel) - 1, NULL, 0, iv, ivlen, 1);
}
                       ...
                       unsigned char* out)
{
  ...
  if (sctx->propq != NULL)
    *p++ = OSSL_PARAM_construct_utf8_string(
      OSSL_ALG_PARAM_PROPERTIES, (char*)sctx->propq, 0);
  *p = OSSL_PARAM_construct_end();
  ...
  if (str ==
    ...
    if (!tls13_derive_finishedkey(
          s, md, s->client_app_traffic_secret, finsecret, hashlen))
      goto err;
    key = finsecret;
  }
...
tls13_setup_key_block(SSL_CONNECTION* s)
{
  ...
  if (!ssl_cipher_get_evp(SSL_CONNECTION_GET_CTX(s),
                          s->session,
                          &c,
                          &hash,
                          &mac_type,
...
static int
derive_secret_key_and_iv(SSL_CONNECTION* s,
                         const EVP_MD* md,
                         ...
                         size_t* taglen)
{
  ...
  if (!tls13_hkdf_expand(
        s, md, insecret, label, labellen, hash, hashlen, secret, hashlen, 1)) {
    /* SSLfatal() already called */
    return 0;
  }
...
static int
tls13_store_hash(SSL_CONNECTION* s, unsigned char* hash, size_t len)
{
  size_t hashlen;
...
{
  return tls13_store_hash(
    s, s->handshake_traffic_hash, sizeof(s->handshake_traffic_hash));
}
...
{
  return tls13_store_hash(
    s, s->server_finished_hash, sizeof(s->server_finished_hash));
}
...
tls13_change_cipher_state(SSL_CONNECTION* s, int which)
{
    ...
    if ((which & SSL3_CC_EARLY) != 0) {
      ...
      if (((EVP_CIPHER_flags(cipher) & EVP_CIPH_FLAG_AEAD_CIPHER) == 0) &&
          (!ssl_cipher_get_evp_md_mac(
            sctx, sslcipher, &mac_md, &mac_pkey_type, NULL))) {
        SSLfatal_alert(s, SSL_AD_INTERNAL_ERROR);
        goto err;
      }
  ...
  if (label == server_application_traffic) {
    ...
    if (!ssl_log_secret(
          s, EXPORTER_SECRET_LABEL, s->exporter_master_secret, hashlen)) {
      /* SSLfatal() already called */
      goto err;
    }
  ...
  if (finsecret != NULL &&
      !tls13_derive_finishedkey(
        s, ssl_handshake_md(s), secret, finsecret, (size_t)finsecretlen)) {
    /* SSLfatal() already called */
    goto err;
  }
...
int
tls13_update_key(SSL_CONNECTION* s, int sending)
{
