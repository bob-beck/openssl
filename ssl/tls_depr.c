...
#ifndef OPENSSL_NO_ENGINE
int
tls_engine_load_ssl_client_cert (SSL_CONNECTION *s, X509 **px509,
                                 EVP_PKEY **ppkey)
{
 ...
 * SSL_CTX_set_tlsext_ticket_key_cb() is removed, then all of this code can
 * also be removed.
 */
...
{
  return HMAC_Init_ex (ctx->old_ctx, key, (int)len, EVP_get_digestbyname (md),
                       NULL);
}
...
int
ssl_set_tmp_ecdh_groups (uint16_t **pext, size_t *pextlen, uint16_t **ksext,
