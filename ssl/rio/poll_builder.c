...
static int rpb_ensure_alloc(RIO_POLL_BUILDER* rpb, size_t alloc) {
  ...
  if (alloc <= rpb->pfd_alloc) return 1;
...
int ossl_rio_poll_builder_add_fd(RIO_POLL_BUILDER* rpb, int fd, int want_read,
  ...
  if (want_read) openssl_fdset(fd, &rpb->rfd);

  if (want_write) openssl_fdset(fd, &rpb->wfd);

  openssl_fdset(fd, &rpb->efd);
  if (fd > rpb->hwm_fd) rpb->hwm_fd = fd;
  ...
  if (want_read) pfds[i].events |= POLLIN;
  if (want_write) pfds[i].events |= POLLOUT;

  if (i == rpb->pfd_num) ++rpb->pfd_num;
