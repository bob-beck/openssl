...
static int
poll_translate_ssl_quic(SSL* ssl,
...
static void
postpoll_translation_cleanup(SSL_POLL_ITEM* items,
                             size_t num_items,
                             size_t stride,
                             QUIC_REACTOR_WAIT_CTX* wctx)
{
...
static int
poll_translate(SSL_POLL_ITEM* items,
               size_t num_items,
               size_t stride,
               QUIC_REACTOR_WAIT_CTX* wctx,
               RIO_POLL_BUILDER* rpb,
        ...
        switch (ssl->type) {
          ...
          case SSL_TYPE_QUIC_XSO:
            if (!poll_translate_ssl_quic(
                  ssl, wctx, rpb, item->events, abort_blocking))
              FAIL_ITEM(i);
...
static int
poll_block(SSL_POLL_ITEM* items,
           size_t num_items,
           size_t stride,
...
static int
poll_readout(SSL_POLL_ITEM* items,
             size_t num_items,
             size_t stride,
             int do_tick,
             size_t* p_result_count)
