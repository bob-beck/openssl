...
#define FAIL_FROM(n)                                                        \
  do {                                                                      \
    size_t j;                                                               \
                                                                            \
    for (j = (n); j < num_items; ++j) ITEM_N(items, stride, j).revents = 0; \
                                                                            \
    ok = 0;                                                                 \
    goto out;                                                               \
  } while (0)
...
static int poll_translate_ssl_quic(SSL* ssl, QUIC_REACTOR_WAIT_CTX* wctx,
  ...
  if (fd1 != -1)
    if (!ossl_rio_poll_builder_add_fd(rpb, fd1, fd1_r, fd1_w)) return 0;

  if (fd2 != -1 && fd2_w)
    if (!ossl_rio_poll_builder_add_fd(rpb, fd2, /*r = */ 0, fd2_w)) return 0;
...
out:
  if (!ok) postpoll_translation_cleanup(items, i, stride, wctx);
...
out:
  if (p_result_count != NULL) *p_result_count = result_count;
