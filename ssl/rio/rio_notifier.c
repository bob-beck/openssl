...
/*
 * Sets a socket as close-on-exec, except that this is a no-op if we are
 * certain we do not need to do this or the OS does not support the concept.
 */
static int
set_cloexec (int fd)
{
...
static int
create_socket (int domain, int socktype, int protocol)
{
  int fd;
#if defined(OPENSSL_SYS_WINDOWS)
 ...
 * it.
 *
 * Win32 does not support socketpair(2), and Win32 pipes are not compatible
 * with Winsock select(2). This means our only means of making select(2)
 * wakeable is to artifically create a loopback TCP connection and send bytes
 * to it.
 */
int
ossl_rio_notifier_init (RIO_NOTIFIER *nfy)
{
...
void
ossl_rio_notifier_cleanup (RIO_NOTIFIER *nfy)
{
  if (nfy->rfd < 0)
    return;
...
int
ossl_rio_notifier_signal (RIO_NOTIFIER *nfy)
{
...
int
ossl_rio_notifier_unsignal (RIO_NOTIFIER *nfy)
{
