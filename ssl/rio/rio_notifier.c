...
DEFINE_RUN_ONCE_STATIC(do_wsa_startup) {
  ...
  if (WSAStartup(versionreq, &wsadata) != 0) return 0;
  wsa_started = 1;
...
err:
  if (lfd != INVALID_SOCKET) BIO_closesocket(lfd);
  if (wfd != INVALID_SOCKET) BIO_closesocket(wfd);
  if (rfd != INVALID_SOCKET) BIO_closesocket(rfd);
  return 0;
}
...
int ossl_rio_notifier_unsignal(RIO_NOTIFIER* nfy) {
  ...
  if (rd < 0 && !BIO_fd_non_fatal_error(get_last_socket_error())) return 0;
