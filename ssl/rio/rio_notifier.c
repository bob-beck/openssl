...
create_socket(int domain, int socktype, int protocol)
{
  ...
  fd = (int)WSASocketA(
    domain, socktype, protocol, NULL, 0, WSA_FLAG_NO_HANDLE_INHERIT);
...
ossl_rio_notifier_init(RIO_NOTIFIER* nfy)
{
  ...
  if (!ensure_wsa_startup()) {
    ERR_raise_data(
      ERR_LIB_SSL, ERR_R_INTERNAL_ERROR, "Cannot start Windows sockets");
    return 0;
  }
#endif
  /* Create a close-on-exec socket. */
  ...
  if (lfd == INVALID_SOCKET) {
    ERR_raise_data(
      ERR_LIB_SYS, get_last_sys_error(), "calling create_socket()");
    return 0;
  }
  ...
  if (!BIO_set_tcp_ndelay(wfd, 1)) {
    ERR_raise_data(
      ERR_LIB_SYS, get_last_sys_error(), "calling BIO_set_tcp_ndelay()");
    goto err;
  }
  ...
  if (!BIO_socket_nbio(rfd, 1)) {
    ERR_raise_data(
      ERR_LIB_SYS, get_last_sys_error(), "calling BIO_socket_nbio()");
    goto err;
  }
