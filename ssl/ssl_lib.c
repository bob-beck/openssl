...
static int
ssl_undefined_function_3 (SSL_CONNECTION *sc, unsigned char *r,
...
static size_t
ssl_undefined_function_5 (SSL_CONNECTION *sc, const char *r, size_t s,
                          unsigned char *t)
{
  return ssl_undefined_function (SSL_CONNECTION_GET_SSL (sc));
}
...
static int
ssl_undefined_function_7 (SSL_CONNECTION *sc, unsigned char *r, size_t s,
                          const char *t, size_t u, const unsigned char *v,
                          size_t w, int x)
{
  return ssl_undefined_function (SSL_CONNECTION_GET_SSL (sc));
}
...
static int
dane_mtype_set (struct dane_ctx_st *dctx, const EVP_MD *md, uint8_t mtype,
                uint8_t ord)
{
  int i;
...
static int
dane_tlsa_add (SSL_DANE *dane, uint8_t usage, uint8_t selector, uint8_t mtype,
...
SSL *
ossl_ssl_connection_new_int (SSL_CTX *ctx, SSL *user_ssl,
  ...
  if (ctx->ext.tuples != NULL)
    {
      ...
      if (ctx->ext.tuples_len == 0)
        ...
        add = 1;
      s->ext.tuples
          = OPENSSL_memdup (ctx->ext.tuples, (ctx->ext.tuples_len + add)
...
int
SSL_get0_dane_tlsa (SSL *s, uint8_t *usage, uint8_t *selector, uint8_t *mtype,
                    const unsigned char **data, size_t *dlen)
{
  SSL_DANE *dane;
  SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL (s);
...
int
SSL_dane_tlsa_add (SSL *s, uint8_t usage, uint8_t selector, uint8_t mtype,
                   const unsigned char *data, size_t dlen)
{
  SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL (s);
...
int
SSL_CTX_dane_mtype_set (SSL_CTX *ctx, const EVP_MD *md, uint8_t mtype,
                        uint8_t ord)
{
...
int
SSL_get_changed_async_fds (SSL *s, OSSL_ASYNC_FD *addfd, size_t *numaddfds,
...
int
ssl_write_internal (SSL *s, const void *buf, size_t num, uint64_t flags,
                    size_t *written)
{
  SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL (s);
...
int
SSL_write_ex2 (SSL *s, const void *buf, size_t num, uint64_t flags,
               size_t *written)
{
 ...
 */
void
SSL_CTX_set_npn_advertised_cb (SSL_CTX *ctx, SSL_CTX_npn_advertised_cb_func cb,
                               void *arg)
{
  if (IS_QUIC_CTX (ctx))
 ...
 */
void
SSL_CTX_set_npn_select_cb (SSL_CTX *ctx, SSL_CTX_npn_select_cb_func cb,
                           void *arg)
{
  if (IS_QUIC_CTX (ctx))
    /* NPN not allowed for QUIC */
 ...
 */
void
SSL_CTX_set_alpn_select_cb (SSL_CTX *ctx, SSL_CTX_alpn_select_cb_func cb,
                            void *arg)
{
...
int
SSL_export_keying_material_early (SSL *s, unsigned char *out, size_t olen,
                                  const char *label, size_t llen,
 ...
 */
static long
check_keylog_bio_free (BIO *b, int oper, const char *argp, size_t len,
...
SSL_CTX *
SSL_CTX_new_ex (OSSL_LIB_CTX *libctx, const char *propq,
...
size_t
SSL_SESSION_get_master_key (const SSL_SESSION *session, unsigned char *out,
                            size_t outlen)
{
  if (outlen == 0)
...
int
SSL_SESSION_set1_master_key (SSL_SESSION *sess, const unsigned char *in,
                             size_t len)
{
...
void
SSL_CTX_set_record_padding_callback (SSL_CTX *ctx,
...
int
SSL_CTX_set_block_padding_ex (SSL_CTX *ctx, size_t app_block_size,
...
int
SSL_set_record_padding_callback (SSL *ssl,
...
int
SSL_set_block_padding_ex (SSL *ssl, size_t app_block_size,
                          size_t hs_block_size)
{
  SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL (ssl);
...
int
ssl_handshake_hash (SSL_CONNECTION *s, unsigned char *out, size_t outlen,
                    size_t *hashlen)
{
  EVP_MD_CTX *ctx = NULL;
...
void
SSL_set_security_callback (SSL *s, int (*cb) (const SSL *s, const SSL_CTX *ctx,
...
int (*SSL_CTX_get_security_callback (const SSL_CTX *ctx)) (
    const SSL *s, const SSL_CTX *ctx, int op, int bits, int nid, void *other,
    void *ex)
{
...
/*
 * Checks for an OCSP response and then attempts to extract any SCTs found if
 * it contains an SCT X509 extension. They will be stored in |s->scts|.
 * Returns:
      ...
      for (i = 0; i < OCSP_resp_count (br); ++i)
        {
          ...
          scts = OCSP_SINGLERESP_get1_ext_d2i (single, NID_ct_cert_scts, NULL,
                                               NULL);
          if (scts == NULL)
            {
              scts_extracted = -1;
...
static int
ct_permissive (const CT_POLICY_EVAL_CTX *ctx, const STACK_OF (SCT) *scts,
               void *unused_arg)
{
  return 1;
}
...
static int
ct_strict (const CT_POLICY_EVAL_CTX *ctx, const STACK_OF (SCT) *scts,
           void *unused_arg)
{
...
int
SSL_set_ct_validation_callback (SSL *s, ssl_ct_validation_cb callback,
                                void *arg)
{
  SSL_CONNECTION *sc = SSL_CONNECTION_FROM_SSL (s);
...
void
SSL_CTX_set_new_pending_conn_cb (SSL_CTX *c, SSL_new_pending_conn_cb_fn cb,
                                 void *arg)
{
...
static int
nss_keylog_int (const char *prefix, SSL_CONNECTION *sc,
...
int
ssl_log_secret (SSL_CONNECTION *sc, const char *label, const uint8_t *secret,
                size_t secret_len)
{
...
int
ssl_cache_cipherlist (SSL_CONNECTION *s, PACKET *cipher_suites,
                      int sslv2format)
{
  int n;
...
int
SSL_bytes_to_cipher_list (SSL *s, const unsigned char *bytes, size_t len,
...
int
ossl_bytes_to_cipher_list (SSL_CONNECTION *s, PACKET *cipher_suites,
...
void
SSL_CTX_set_allow_early_data_cb (SSL_CTX *ctx, SSL_allow_early_data_cb_fn cb,
                                 void *arg)
{
...
int
SSL_shutdown_ex (SSL *ssl, uint64_t flags, const SSL_SHUTDOWN_EX_ARGS *args,
                 size_t args_len)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC (ssl))
...
static int
set_cert_type (unsigned char **cert_type, size_t *cert_type_len,
...
int
SSL_CTX_set1_client_cert_type (SSL_CTX *ctx, const unsigned char *val,
                               size_t len)
{
...
int
SSL_CTX_set1_server_cert_type (SSL_CTX *ctx, const unsigned char *val,
                               size_t len)
{
...
int
SSL_CTX_get0_client_cert_type (const SSL_CTX *ctx, unsigned char **t,
                               size_t *len)
{
  if (t == NULL || len == NULL)
    return 0;
...
int
SSL_CTX_get0_server_cert_type (const SSL_CTX *ctx, unsigned char **t,
                               size_t *len)
{
  if (t == NULL || len == NULL)
    return 0;
