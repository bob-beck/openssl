...
static int
ssl_undefined_function_3(SSL_CONNECTION* sc,
                         unsigned char* r,
                         unsigned char* s,
                         size_t t,
                         size_t* u)
...
static int
ssl_undefined_function_4(SSL_CONNECTION* sc, int r)
{
  return ssl_undefined_function(SSL_CONNECTION_GET_SSL(sc));
}

static size_t
ssl_undefined_function_5(SSL_CONNECTION* sc,
                         const char* r,
                         size_t s,
                         unsigned char* t)
{
...
static int
ssl_undefined_function_6(int r)
{
  return ssl_undefined_function(NULL);
}

static int
ssl_undefined_function_7(SSL_CONNECTION* sc,
                         unsigned char* r,
                         size_t s,
                         const char* t,
                         size_t u,
...
static int
ssl_undefined_function_8(SSL_CONNECTION* sc)
{
  return ssl_undefined_function(SSL_CONNECTION_GET_SSL(sc));
}
...
static int
dane_ctx_enable(struct dane_ctx_st* dctx)
{
  const EVP_MD** mdevp;
  uint8_t* mdord;
...
static void
dane_ctx_final(struct dane_ctx_st* dctx)
{
 ...
 */
static int
ssl_dane_dup(SSL_CONNECTION* to, SSL_CONNECTION* from)
{
  int num;
  int i;
  ...
  for (i = 0; i < num; ++i) {
    ...
    if (SSL_dane_tlsa_add(SSL_CONNECTION_GET_SSL(to),
                          t->usage,
                          t->selector,
                          t->mtype,
                          t->data,
...
static int
dane_mtype_set(struct dane_ctx_st* dctx,
               const EVP_MD* md,
               uint8_t mtype,
               uint8_t ord)
{
...
static const EVP_MD*
tlsa_md_get(SSL_DANE* dane, uint8_t mtype)
{
 ...
 */
static int
ssl_check_allowed_versions(int min_version, int max_version)
{
...
int
ossl_ssl_connection_reset(SSL* s)
{
  ...
  memset(sc->ext.compress_certificate_from_peer,
         0,
...
int
ossl_ssl_init(SSL* ssl, SSL_CTX* ctx, const SSL_METHOD* method, int type)
{
...
SSL*
ossl_ssl_connection_new_int(SSL_CTX* ctx,
                            SSL* user_ssl,
                            const SSL_METHOD* method)
{
  ...
  if (!SSL_set_ct_validation_callback(
        ssl, ctx->ct_validation_callback, ctx->ct_validation_callback_arg))
    goto sslerr;
#endif
...
SSL*
ossl_ssl_connection_new(SSL_CTX* ctx)
{
...
int
SSL_set_session_id_context(SSL* ssl,
                           const unsigned char* sid_ctx,
                           unsigned int sid_ctx_len)
{
...
int
SSL_has_matching_session_id(const SSL* ssl,
...
int
SSL_set1_host(SSL* s, const char* host)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_add1_host(SSL* s, const char* host)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
unsigned long
SSL_CTX_dane_set_flags(SSL_CTX* ctx, unsigned long flags)
{
...
unsigned long
SSL_CTX_dane_clear_flags(SSL_CTX* ctx, unsigned long flags)
{
  unsigned long orig = ctx->dane.flags;
...
int
SSL_dane_enable(SSL* s, const char* basedomain)
{
  SSL_DANE* dane;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
unsigned long
SSL_dane_set_flags(SSL* ssl, unsigned long flags)
{
  unsigned long orig;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(ssl);
...
unsigned long
SSL_dane_clear_flags(SSL* ssl, unsigned long flags)
{
  unsigned long orig;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(ssl);
...
int
SSL_get0_dane_authority(SSL* s, X509** mcert, EVP_PKEY** mspki)
{
  SSL_DANE* dane;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_CTX_dane_mtype_set(SSL_CTX* ctx,
                       const EVP_MD* md,
                       uint8_t mtype,
                       uint8_t ord)
{
...
void
ossl_ssl_connection_free(SSL* ssl)
{
  SSL_CONNECTION* s;
...
int
SSL_get_all_async_fds(SSL* s, OSSL_ASYNC_FD* fds, size_t* numfds)
{
  ASYNC_WAIT_CTX* ctx;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_get_changed_async_fds(SSL* s,
                          OSSL_ASYNC_FD* addfd,
                          size_t* numaddfds,
                          OSSL_ASYNC_FD* delfd,
                          size_t* numdelfds)
{
  ...
  if ((ctx = sc->waitctx) == NULL)
    return 0;
  return ASYNC_WAIT_CTX_get_changed_fds(
    ctx, addfd, numaddfds, delfd, numdelfds);
}

int
SSL_CTX_set_async_callback(SSL_CTX* ctx, SSL_async_callback_fn callback)
{
...
int
SSL_CTX_set_async_callback_arg(SSL_CTX* ctx, void* arg)
{
...
int
SSL_set_async_callback(SSL* s, SSL_async_callback_fn callback)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_set_async_callback_arg(SSL* s, void* arg)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_get_async_status(SSL* s, int* status)
{
  ASYNC_WAIT_CTX* ctx;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
static int
ssl_async_wait_ctx_cb(void* arg)
{
  SSL* s = (SSL*)arg;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
static int
ssl_start_async_job(SSL* s, struct ssl_async_args* args, int (*func)(void*))
{
  int ret;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
  ...
  sc->rwstate = SSL_NOTHING;
  switch (ASYNC_start_job(
    &sc->job, sc->waitctx, &ret, func, args, sizeof(struct ssl_async_args))) {
    case ASYNC_ERR:
      sc->rwstate = SSL_NOTHING;
...
int
ssl_read_internal(SSL* s, void* buf, size_t num, size_t* readbytes)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_read_ex(SSL* s, void* buf, size_t num, size_t* readbytes)
{
...
static int
ssl_peek_internal(SSL* s, void* buf, size_t num, size_t* readbytes)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_peek_ex(SSL* s, void* buf, size_t num, size_t* readbytes)
{
...
#endif
      ERR_raise_data(
        ERR_LIB_SYS, get_last_sys_error(), "ktls_sendfile failure");
    return ret;
  }
  sc->rwstate = SSL_NOTHING;
  return ret;
...
int
SSL_write_ex(SSL* s, const void* buf, size_t num, size_t* written)
{
...
int
SSL_key_update(SSL* s, int updatetype)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
 ...
 */
static int
can_renegotiate(const SSL_CONNECTION* sc)
{
  if (SSL_CONNECTION_IS_TLS13(sc)) {
    ERR_raise(ERR_LIB_SSL, SSL_R_WRONG_SSL_VERSION);
    return 0;
...
int
SSL_renegotiate_pending(const SSL* s)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL_ONLY(s);
...
long
ossl_ctrl_internal(SSL* s, int cmd, long larg, void* parg, int no_quic)
{
  long l;
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
static int
ssl_tsan_load(SSL_CTX* ctx, TSAN_QUALIFIER int* stat)
{
  int res = 0;
  ...
  switch (cmd) {
    ...
    case SSL_CTRL_SET_MIN_PROTO_VERSION:
      return ssl_check_allowed_versions(larg, ctx->max_proto_version) &&
             ssl_set_version_bound(
               ctx->method->version, (int)larg, &ctx->min_proto_version);
    case SSL_CTRL_GET_MIN_PROTO_VERSION:
      return ctx->min_proto_version;
    case SSL_CTRL_SET_MAX_PROTO_VERSION:
      return ssl_check_allowed_versions(ctx->min_proto_version, larg) &&
             ssl_set_version_bound(
               ctx->method->version, (int)larg, &ctx->max_proto_version);
    case SSL_CTRL_GET_MAX_PROTO_VERSION:
...
SSL_CTX_callback_ctrl(SSL_CTX* ctx, int cmd, void (*fp)(void))
{
  switch (cmd) {
    case SSL_CTRL_SET_MSG_CALLBACK:
      ctx->msg_callback = (void (*)(int write_p,
                                    int version,
                                    int content_type,
                                    const void* buf,
                                    size_t len,
...
int
ssl_cipher_ptr_id_cmp(const SSL_CIPHER* const* ap, const SSL_CIPHER* const* bp)
{
  if ((*ap)->id > (*bp)->id)
    return 1;
  if ((*ap)->id < (*bp)->id)
 ...
 * algorithm id */
STACK_OF(SSL_CIPHER)*
ssl_get_ciphers_by_id(SSL_CONNECTION* s)
{
  if (s != NULL) {
 ...
 */
static int
cipher_list_tls12_num(STACK_OF(SSL_CIPHER)* sk)
{
  int i, num = 0;
  const SSL_CIPHER* c;
...
char*
SSL_get_shared_ciphers(const SSL* s, char* buf, int size)
{
  char* p;
 ...
 */
int
SSL_select_next_proto(unsigned char** out,
    ...
    while (PACKET_get_length_prefixed_1(&spkt, &ssubpkt)) {
      ...
      if (PACKET_buf_init(&cpkt, client, client_len)) {
        while (PACKET_get_length_prefixed_1(&cpkt, &csubpkt)) {
          if (PACKET_equal(
                &csubpkt, PACKET_data(&ssubpkt), PACKET_remaining(&ssubpkt))) {
            /* We found a match */
 ...
 */
void
SSL_get0_next_proto_negotiated(const SSL* s,
                               const unsigned char** data,
                               unsigned* len)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL(s);
 ...
 */
void
SSL_CTX_set_npn_advertised_cb(SSL_CTX* ctx,
 ...
 */
void
SSL_CTX_set_npn_select_cb(SSL_CTX* ctx,
...
static int
alpn_value_ok(const unsigned char* protos, unsigned int protos_len)
{
  unsigned int idx;
 ...
 */
int
SSL_CTX_set_alpn_protos(SSL_CTX* ctx,
 ...
 */
void
SSL_get0_alpn_selected(const SSL* ssl,
                       const unsigned char** data,
                       unsigned int* len)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL(ssl);
...
int
SSL_export_keying_material_early(SSL* s,
                                 unsigned char* out,
                                 size_t olen,
                                 const char* label,
                                 size_t llen,
                                 ...
                                 size_t contextlen)
{
  ...
  return tls13_export_keying_material_early(
    sc, out, olen, label, llen, context, contextlen);
}
 ...
 */
static void
do_sslkeylogfile(const SSL* ssl, const char* line)
{
  if (keylog_lock == NULL)
    return;
...
SSL_CTX_new_ex(OSSL_LIB_CTX* libctx, const char* propq, const SSL_METHOD* meth)
{
  ...
  if ((RAND_bytes_ex(
         libctx, ret->ext.tick_key_name, sizeof(ret->ext.tick_key_name), 0) <=
       0) ||
      (RAND_priv_bytes_ex(libctx,
                          ret->ext.secure->tick_hmac_key,
                          sizeof(ret->ext.secure->tick_hmac_key),
                          0) <= 0) ||
      (RAND_priv_bytes_ex(libctx,
                          ret->ext.secure->tick_aes_key,
                          sizeof(ret->ext.secure->tick_aes_key),
                          0) <= 0))
    ret->options |= SSL_OP_NO_TICKET;

  if (RAND_priv_bytes_ex(libctx,
                         ret->ext.cookie_hmac_key,
...
void
SSL_set_default_passwd_cb(SSL* s, pem_password_cb* cb)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
void
SSL_set_default_passwd_cb_userdata(SSL* s, void* u)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
pem_password_cb*
SSL_get_default_passwd_cb(SSL* s)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
void*
SSL_get_default_passwd_cb_userdata(SSL* s)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
void
ssl_set_masks(SSL_CONNECTION* s)
{
  CERT* c = s->cert;
...
int
ssl_check_srvr_ecc_cert_and_alg(X509* x, SSL_CONNECTION* s)
{
...
int
ssl_get_server_cert_serverinfo(SSL_CONNECTION* s,
...
void
ssl_update_cache(SSL_CONNECTION* s, int mode)
{
  int i;
...
const SSL_METHOD*
SSL_get_ssl_method(const SSL* s)
{
  return s->method;
}
...
int
ossl_ssl_get_error(const SSL* s, int i, int check_err)
{
  int reason;
  unsigned long l;
  BIO* bio;
...
static int
ssl_do_handshake_intern(void* vargs)
{
...
const char*
ssl_protocol_to_string(int version)
{
  switch (version) {
    case TLS1_3_VERSION:
      return "TLSv1.3";
...
__owur int
SSL_get_handshake_rtt(const SSL* s, uint64_t* rtt)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL(s);
...
static int
dup_ca_names(STACK_OF(X509_NAME)** dst, STACK_OF(X509_NAME)* src)
{
...
const COMP_METHOD*
SSL_get_current_compression(const SSL* s)
{
...
const COMP_METHOD*
SSL_get_current_expansion(const SSL* s)
{
#ifndef OPENSSL_NO_COMP
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL_ONLY(s);
...
int
ssl_init_wbio_buffer(SSL_CONNECTION* s)
{
  BIO* bbio;
...
int
ssl_free_wbio_buffer(SSL_CONNECTION* s)
{
...
int
SSL_client_version(const SSL* s)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL(s);
...
{
  return X509_STORE_set_default_paths_ex(
    ctx->cert_store, ctx->libctx, ctx->propq);
}

int
SSL_CTX_set_default_verify_dir(SSL_CTX* ctx)
{
  X509_LOOKUP* lookup;
...
SSL_CTX_set_default_verify_file(SSL_CTX* ctx)
{
  ...
  X509_LOOKUP_load_file_ex(
    lookup, NULL, X509_FILETYPE_DEFAULT, ctx->libctx, ctx->propq);
...
int
SSL_CTX_set_default_verify_store(SSL_CTX* ctx)
{
  X509_LOOKUP* lookup;
...
{
  return X509_STORE_load_file_ex(
    ctx->cert_store, CAfile, ctx->libctx, ctx->propq);
}

int
SSL_CTX_load_verify_dir(SSL_CTX* ctx, const char* CApath)
{
  return X509_STORE_load_path(ctx->cert_store, CApath);
}

int
SSL_CTX_load_verify_store(SSL_CTX* ctx, const char* CAstore)
{
  return X509_STORE_load_store_ex(
    ctx->cert_store, CAstore, ctx->libctx, ctx->propq);
}
...
int
SSL_SESSION_set1_master_key(SSL_SESSION* sess,
...
void
SSL_set_psk_client_callback(SSL* s, SSL_psk_client_cb_func cb)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
void
SSL_CTX_set_psk_client_callback(SSL_CTX* ctx, SSL_psk_client_cb_func cb)
{
  ctx->psk_client_callback = cb;
}

void
SSL_set_psk_server_callback(SSL* s, SSL_psk_server_cb_func cb)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
void
SSL_set_psk_find_session_callback(SSL* s, SSL_psk_find_session_cb_func cb)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
void
SSL_CTX_set_psk_find_session_callback(SSL_CTX* ctx,
...
void
SSL_CTX_set_psk_use_session_callback(SSL_CTX* ctx,
...
void
SSL_CTX_set_not_resumable_session_callback(SSL_CTX* ctx,
                                           int (*cb)(SSL* ssl,
                                                     int is_forward_secure))
{
  SSL_CTX_callback_ctrl(
    ctx, SSL_CTRL_SET_NOT_RESUMABLE_SESS_CB, (void (*)(void))cb);
}

void
SSL_set_not_resumable_session_callback(SSL* ssl,
                                       int (*cb)(SSL* ssl,
                                                 int is_forward_secure))
{
  SSL_callback_ctrl(
    ssl, SSL_CTRL_SET_NOT_RESUMABLE_SESS_CB, (void (*)(void))cb);
}
...
void
SSL_CTX_set_record_padding_callback_arg(SSL_CTX* ctx, void* arg)
{
  ctx->record_padding_arg = arg;
}

void*
SSL_CTX_get_record_padding_callback_arg(const SSL_CTX* ctx)
{
  return ctx->record_padding_arg;
}

int
SSL_CTX_set_block_padding_ex(SSL_CTX* ctx,
...
void
SSL_set_record_padding_callback_arg(SSL* ssl, void* arg)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(ssl);
...
void*
SSL_get_record_padding_callback_arg(const SSL* ssl)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL(ssl);
...
int
SSL_set_block_padding_ex(SSL* ssl, size_t app_block_size, size_t hs_block_size)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(ssl);
...
int
SSL_set_block_padding(SSL* ssl, size_t block_size)
{
...
int
ssl_handshake_hash(SSL_CONNECTION* s,
                   unsigned char* out,
                   size_t outlen,
                   size_t* hashlen)
{
...
int
SSL_session_reused(const SSL* s)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL(s);
...
void
SSL_set_security_callback(SSL* s,
                          int (*cb)(const SSL* s,
                                    const SSL_CTX* ctx,
                                    int op,
                                    int bits,
...
void
SSL_set0_security_ex_data(SSL* s, void* ex)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
void*
SSL_get0_security_ex_data(const SSL* s)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL(s);
...
void
SSL_CTX_set_security_callback(SSL_CTX* ctx,
                              int (*cb)(const SSL* s,
                                        const SSL_CTX* ctx,
                                        int op,
                                        int bits,
...
void
SSL_CTX_set0_security_ex_data(SSL_CTX* ctx, void* ex)
{
  ctx->cert->sec_ex = ex;
}

void*
SSL_CTX_get0_security_ex_data(const SSL_CTX* ctx)
{
...
uint64_t
SSL_CTX_set_options(SSL_CTX* ctx, uint64_t op)
{
  return ctx->options |= op;
}

uint64_t
SSL_set_options(SSL* s, uint64_t op)
{
  SSL_CONNECTION* sc;
  OSSL_PARAM options[2], *opts = options;
...
uint64_t
SSL_CTX_clear_options(SSL_CTX* ctx, uint64_t op)
{
  return ctx->options &= ~op;
}

uint64_t
SSL_clear_options(SSL* s, uint64_t op)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
  OSSL_PARAM options[2], *opts = options;
 ...
 */
static int
ct_move_scts(STACK_OF(SCT)** dst, STACK_OF(SCT)* src, sct_source_t origin)
{
  int scts_moved = 0;
  SCT* sct = NULL;
 ...
 */
static int
ct_extract_tls_extension_scts(SSL_CONNECTION* s)
{
 ...
 */
static int
ct_extract_ocsp_response_scts(SSL_CONNECTION* s)
{
#ifndef OPENSSL_NO_OCSP
  int scts_extracted = 0;
  ...
  for (j = 0; j < sk_OCSP_RESPONSE_num(s->ext.ocsp.resp_ex); j++) {
    ...
    for (i = 0; i < OCSP_resp_count(br); ++i) {
      ...
      ret = ct_move_scts(&s->scts, scts, SCT_SOURCE_OCSP_STAPLED_RESPONSE);
 ...
 */
static int
ct_extract_x509v3_extension_scts(SSL_CONNECTION* s)
{
  int scts_extracted = 0;
...
static int
ct_permissive(const CT_POLICY_EVAL_CTX* ctx,
...
static int
ct_strict(const CT_POLICY_EVAL_CTX* ctx,
          const STACK_OF(SCT)* scts,
          void* unused_arg)
{
...
int
SSL_set_ct_validation_callback(SSL* s, ssl_ct_validation_cb callback, void* arg)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_CTX_set_ct_validation_callback(SSL_CTX* ctx,
...
int
SSL_CTX_ct_is_enabled(const SSL_CTX* ctx)
{
  return ctx->ct_validation_callback != NULL;
}

int
ssl_validate_ct(SSL_CONNECTION* s)
{
  int ret = 0;
  X509* cert = s->session != NULL ? s->session->peer : NULL;
  X509* issuer;
...
int
SSL_CTX_enable_ct(SSL_CTX* ctx, int validation_mode)
{
...
int
SSL_enable_ct(SSL* s, int validation_mode)
{
  switch (validation_mode) {
    default:
      ERR_raise(ERR_LIB_SSL, SSL_R_INVALID_CT_VALIDATION_TYPE);
...
int
SSL_CTX_set_ctlog_list_file(SSL_CTX* ctx, const char* path)
{
  return CTLOG_STORE_load_file(ctx->ctlog_store, path);
}

void
SSL_CTX_set0_ctlog_store(SSL_CTX* ctx, CTLOG_STORE* logs)
{
...
const CTLOG_STORE*
SSL_CTX_get0_ctlog_store(const SSL_CTX* ctx)
{
...
void
SSL_CTX_set_client_hello_cb(SSL_CTX* c, SSL_client_hello_cb_fn cb, void* arg)
{
...
void
SSL_CTX_set_new_pending_conn_cb(SSL_CTX* c,
...
size_t
SSL_client_hello_get0_random(SSL* s, const unsigned char** out)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
size_t
SSL_client_hello_get0_session_id(SSL* s, const unsigned char** out)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
size_t
SSL_client_hello_get0_compression_methods(SSL* s, const unsigned char** out)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_client_hello_get_extension_order(SSL* s, uint16_t* exts, size_t* num_exts)
{
  RAW_EXTENSION* ext;
  size_t num = 0, i;
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_client_hello_get0_ext(SSL* s,
                          unsigned int type,
...
int
ssl_log_rsa_client_key_exchange(SSL_CONNECTION* sc,
                                ...
                                size_t premaster_len)
{
  ...
  return nss_keylog_int(
    "RSA", sc, encrypted_premaster, 8, premaster, premaster_len);
}

int
ssl_log_secret(SSL_CONNECTION* sc,
               const char* label,
               const uint8_t* secret,
               size_t secret_len)
{
  return nss_keylog_int(
    label, sc, sc->s3.client_random, SSL3_RANDOM_SIZE, secret, secret_len);
}

#define SSLV2_CIPHER_LEN 3

int
ssl_cache_cipherlist(SSL_CONNECTION* s, PACKET* cipher_suites, int sslv2format)
{
  int n;
  ...
  } else if (!PACKET_memdup(cipher_suites,
                            &s->s3.tmp.ciphers_raw,
...
int
ossl_bytes_to_cipher_list(SSL_CONNECTION* s,
...
int
SSL_CTX_set_recv_max_early_data(SSL_CTX* ctx, uint32_t recv_max_early_data)
{
...
uint32_t
SSL_CTX_get_recv_max_early_data(const SSL_CTX* ctx)
{
  return ctx->recv_max_early_data;
}

int
SSL_set_recv_max_early_data(SSL* s, uint32_t recv_max_early_data)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL_ONLY(s);
...
uint32_t
SSL_get_recv_max_early_data(const SSL* s)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL(s);
...
__owur unsigned int
ssl_get_max_send_fragment(const SSL_CONNECTION* sc)
{
...
__owur unsigned int
ssl_get_split_send_fragment(const SSL_CONNECTION* sc)
{
...
int
SSL_CTX_set_session_ticket_cb(SSL_CTX* ctx,
...
void
SSL_CTX_set_allow_early_data_cb(SSL_CTX* ctx,
...
void
SSL_set_allow_early_data_cb(SSL* s, SSL_allow_early_data_cb_fn cb, void* arg)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL_ONLY(s);
...
const EVP_CIPHER*
ssl_evp_cipher_fetch(OSSL_LIB_CTX* libctx, int nid, const char* properties)
{
...
int
ssl_evp_cipher_up_ref(const EVP_CIPHER* cipher)
{
...
void
ssl_evp_cipher_free(const EVP_CIPHER* cipher)
{
  if (cipher == NULL)
    return;
...
const EVP_MD*
ssl_evp_md_fetch(OSSL_LIB_CTX* libctx, int nid, const char* properties)
{
  const EVP_MD* md;
...
int
ssl_evp_md_up_ref(const EVP_MD* md)
{
...
void
ssl_evp_md_free(const EVP_MD* md)
{
  if (md == NULL)
    return;
...
int
SSL_set0_tmp_dh_pkey(SSL* s, EVP_PKEY* dhpkey)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
  ...
  if (!ssl_security(
        sc, SSL_SECOP_TMP_DH, EVP_PKEY_get_security_bits(dhpkey), 0, dhpkey)) {
...
{
  if (!ssl_ctx_security(
        ctx, SSL_SECOP_TMP_DH, EVP_PKEY_get_security_bits(dhpkey), 0, dhpkey)) {
    ERR_raise(ERR_LIB_SSL, SSL_R_DH_KEY_TOO_SMALL);
    return 0;
  }
...
int
SSL_get_event_timeout(SSL* s, struct timeval* tv, int* is_infinite)
{
  SSL_CONNECTION* sc;
...
int
SSL_get_rpoll_descriptor(SSL* s, BIO_POLL_DESCRIPTOR* desc)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_get_wpoll_descriptor(SSL* s, BIO_POLL_DESCRIPTOR* desc)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_set_blocking_mode(SSL* s, int blocking)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return 0;
...
int
SSL_set1_initial_peer_addr(SSL* s, const BIO_ADDR* peer_addr)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return 0;
...
int
SSL_stream_conclude(SSL* ssl, uint64_t flags)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(ssl))
    return 0;
...
SSL*
SSL_new_stream(SSL* s, uint64_t flags)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return NULL;
...
int
SSL_is_connection(SSL* s)
{
  return SSL_get0_connection(s) == s;
}

SSL*
SSL_get0_listener(SSL* s)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return NULL;
...
int
SSL_set_default_stream_mode(SSL* s, uint32_t mode)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return 0;
...
int
SSL_set_incoming_stream_policy(SSL* s, int policy, uint64_t aec)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return 0;
...
SSL*
SSL_accept_stream(SSL* s, uint64_t flags)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return NULL;
...
int
SSL_stream_reset(SSL* s, const SSL_STREAM_RESET_ARGS* args, size_t args_len)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return 0;
...
int
SSL_get_stream_read_state(SSL* s)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
...
int
SSL_get_stream_write_state(SSL* s)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return SSL_STREAM_STATE_NONE;
...
int
SSL_get_stream_read_error_code(SSL* s, uint64_t* app_error_code)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return -1;
...
int
SSL_get_stream_write_error_code(SSL* s, uint64_t* app_error_code)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return -1;
...
int
SSL_get_conn_close_info(SSL* s, SSL_CONN_CLOSE_INFO* info, size_t info_len)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s))
    return -1;
...
int
SSL_get_value_uint(SSL* s, uint32_t class_, uint32_t id, uint64_t* value)
{
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s))
...
int
SSL_set_value_uint(SSL* s, uint32_t class_, uint32_t id, uint64_t value)
{
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s))
...
SSL*
SSL_new_listener(SSL_CTX* ctx, uint64_t flags)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC_CTX(ctx))
    return NULL;
...
SSL*
SSL_new_listener_from(SSL* ssl, uint64_t flags)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(ssl))
    return NULL;
...
SSL*
SSL_new_from_listener(SSL* ssl, uint64_t flags)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(ssl))
    return NULL;
...
SSL*
SSL_accept_connection(SSL* ssl, uint64_t flags)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(ssl))
    return NULL;
...
SSL*
SSL_new_domain(SSL_CTX* ctx, uint64_t flags)
{
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC_CTX(ctx))
    return NULL;
...
int
ossl_adjust_domain_flags(uint64_t domain_flags, uint64_t* p_domain_flags)
{
  if ((domain_flags & ~OSSL_QUIC_SUPPORTED_DOMAIN_FLAGS) != 0) {
    ERR_raise_data(
      ERR_LIB_SSL, ERR_R_UNSUPPORTED, "unsupported domain flag requested");
    return 0;
  }
...
int
SSL_CTX_set_domain_flags(SSL_CTX* ctx, uint64_t domain_flags)
{
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC_CTX(ctx)) {
...
int
SSL_CTX_get_domain_flags(const SSL_CTX* ctx, uint64_t* domain_flags)
{
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC_CTX(ctx)) {
...
int
SSL_get_domain_flags(const SSL* ssl, uint64_t* domain_flags)
{
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(ssl))
...
int
SSL_add_expected_rpk(SSL* s, EVP_PKEY* rpk)
{
...
int
SSL_get_negotiated_client_cert_type(const SSL* s)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
int
SSL_get_negotiated_server_cert_type(const SSL* s)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
...
static int
validate_cert_type(const unsigned char* val, size_t len)
{
  size_t i;
  int saw_rpk = 0;
  int saw_x509 = 0;
...
static int
set_cert_type(unsigned char** cert_type,
...
int
SSL_set1_client_cert_type(SSL* s, const unsigned char* val, size_t len)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
  ...
  return set_cert_type(
    &sc->client_cert_type, &sc->client_cert_type_len, val, len);
}

int
SSL_set1_server_cert_type(SSL* s, const unsigned char* val, size_t len)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(s);
  ...
  return set_cert_type(
    &sc->server_cert_type, &sc->server_cert_type_len, val, len);
}

int
SSL_CTX_set1_client_cert_type(SSL_CTX* ctx,
                              const unsigned char* val,
                              size_t len)
{
  return set_cert_type(
    &ctx->client_cert_type, &ctx->client_cert_type_len, val, len);
}

int
SSL_CTX_set1_server_cert_type(SSL_CTX* ctx,
                              const unsigned char* val,
                              size_t len)
{
  return set_cert_type(
    &ctx->server_cert_type, &ctx->server_cert_type_len, val, len);
}

int
SSL_get0_client_cert_type(const SSL* s, unsigned char** t, size_t* len)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL(s);
...
int
SSL_get0_server_cert_type(const SSL* s, unsigned char** t, size_t* len)
{
  const SSL_CONNECTION* sc = SSL_CONNECTION_FROM_CONST_SSL(s);
...
int
SSL_CTX_get0_client_cert_type(const SSL_CTX* ctx,
                              unsigned char** t,
                              size_t* len)
{
  if (t == NULL || len == NULL)
...
int
SSL_CTX_get0_server_cert_type(const SSL_CTX* ctx,
                              unsigned char** t,
                              size_t* len)
{
  if (t == NULL || len == NULL)
