...
static int ssl_dane_dup(SSL_CONNECTION* to, SSL_CONNECTION* from) {
  ...
  if (!DANETLS_ENABLED(&from->dane)) return 1;
...
static const EVP_MD* tlsa_md_get(SSL_DANE* dane, uint8_t mtype) {
  if (mtype > dane->dctx->mdmax) return NULL;
...
static int dane_tlsa_add(SSL_DANE* dane, uint8_t usage, uint8_t selector,
  ...
  if ((t = OPENSSL_zalloc(sizeof(*t))) == NULL) return -1;
  ...
  for (i = 0; i < num; ++i) {
    ...
    if (rec->usage > usage) continue;
    if (rec->usage < usage) break;
    if (rec->selector > selector) continue;
    if (rec->selector < selector) break;
    if (dane->dctx->mdord[rec->mtype] > dane->dctx->mdord[mtype]) continue;
    break;
  }
...
static int ssl_check_allowed_versions(int min_version, int max_version) {
  ...
  if (minisdtls || maxisdtls) {
    ...
    if (min_version == 0) /* Ignore DTLS1_BAD_VER */
      min_version = DTLS1_VERSION;
    if (max_version == 0) max_version = DTLS1_2_VERSION;
#ifdef OPENSSL_NO_DTLS1_2
    if (max_version == DTLS1_2_VERSION) max_version = DTLS1_VERSION;
#endif
#ifdef OPENSSL_NO_DTLS1
    if (min_version == DTLS1_VERSION) min_version = DTLS1_2_VERSION;
#endif
    ...
    if (min_version == 0) min_version = SSL3_VERSION;
    if (max_version == 0) max_version = TLS1_3_VERSION;
#ifdef OPENSSL_NO_TLS1_3
    if (max_version == TLS1_3_VERSION) max_version = TLS1_2_VERSION;
#endif
#ifdef OPENSSL_NO_TLS1_2
    if (max_version == TLS1_2_VERSION) max_version = TLS1_1_VERSION;
#endif
#ifdef OPENSSL_NO_TLS1_1
    if (max_version == TLS1_1_VERSION) max_version = TLS1_VERSION;
#endif
#ifdef OPENSSL_NO_TLS1
    if (max_version == TLS1_VERSION) max_version = SSL3_VERSION;
#endif
...
#endif
#ifdef OPENSSL_NO_TLS1_1
    if (min_version == TLS1_1_VERSION) min_version = TLS1_2_VERSION;
#endif
#ifdef OPENSSL_NO_TLS1_2
...
int ossl_ssl_connection_reset(SSL* s) {
  ...
  if (s->method != s->defltmeth) {
    ...
    if (!s->method->ssl_init(s)) return 0;
  } else {
...
int ossl_ssl_init(SSL* ssl, SSL_CTX* ctx, const SSL_METHOD* method, int type) {
  if (!SSL_CTX_up_ref(ctx)) return 0;

  ssl->lock = CRYPTO_THREAD_lock_new();

  if (ssl->lock == NULL || !CRYPTO_NEW_REF(&ssl->references, 1)) goto err;
...
SSL* ossl_ssl_connection_new_int(SSL_CTX* ctx, SSL* user_ssl,
  ...
  if (s->tls13_ciphersuites == NULL) goto cerr;
  ...
  if (!ossl_assert(s->sid_ctx_length <= sizeof(s->sid_ctx))) goto err;
  ...
  if (!SSL_CTX_up_ref(ctx)) goto err;
  ...
  if (ctx->ext.supportedgroups != NULL) {
    ...
    if (ctx->ext.supportedgroups_len == 0) /* Add 1 so allocation won't fail */
      add = 1;
  ...
  if (ctx->ext.keyshares != NULL) {
    ...
    if (ctx->ext.keyshares_len == 0) /* Add 1 so allocation won't fail */
      add = 1;
    s->ext.keyshares =
  ...
  if (ctx->ext.tuples != NULL) {
    ...
    if (ctx->ext.tuples_len == 0) /* Add 1 so allocation won't fail */
      add = 1;
    s->ext.tuples =
  ...
  if (!method->ssl_init(ssl)) goto sslerr;

  s->server = (method->ssl_accept == ssl_undefined_function) ? 0 : 1;

  if (!method->ssl_reset(ssl)) goto sslerr;
  ...
  if (ctx->client_cert_type != NULL) {
    ...
    if (s->client_cert_type == NULL) goto sslerr;
  ...
  if (ctx->server_cert_type != NULL) {
    ...
    if (s->server_cert_type == NULL) goto sslerr;
...
int SSL_up_ref(SSL* s) {
  ...
  if (CRYPTO_UP_REF(&s->references, &i) <= 0) return 0;
...
int SSL_set_generate_session_id(SSL* ssl, GEN_SESSION_CB cb) {
  ...
  if (sc == NULL || !CRYPTO_THREAD_write_lock(ssl->lock)) return 0;
...
int SSL_has_matching_session_id(const SSL* ssl, const unsigned char* id,
  ...
  if (sc == NULL || id_len > sizeof(r.session_id)) return 0;

  r.ssl_version = sc->version;
  r.session_id_length = id_len;
  memcpy(r.session_id, id, id_len);

  if (!CRYPTO_THREAD_read_lock(sc->session_ctx->lock)) return 0;
...
int SSL_get0_dane_authority(SSL* s, X509** mcert, EVP_PKEY** mspki) {
  ...
  if (!DANETLS_ENABLED(dane) || sc->verify_result != X509_V_OK) return -1;
  if (dane->mtlsa) {
    if (mcert) *mcert = dane->mcert;
    if (mspki) *mspki = (dane->mcert == NULL) ? dane->mtlsa->spki : NULL;
  }
  return dane->mdpth;
}
...
int SSL_get0_dane_tlsa(SSL* s, uint8_t* usage, uint8_t* selector,
  ...
  if (!DANETLS_ENABLED(dane) || sc->verify_result != X509_V_OK) return -1;
  if (dane->mtlsa) {
    if (usage) *usage = dane->mtlsa->usage;
    if (selector) *selector = dane->mtlsa->selector;
    if (mtype) *mtype = dane->mtlsa->mtype;
    if (data) *data = dane->mtlsa->data;
    if (dlen) *dlen = dane->mtlsa->dlen;
  }
  return dane->mdpth;
}
...
void ossl_ssl_connection_free(SSL* ssl) {
  ...
  if (s->clienthello != NULL) OPENSSL_free(s->clienthello->pre_proc_exts);
  ...
  if (ssl->method != NULL) ssl->method->ssl_deinit(ssl);
...
void SSL_set0_wbio(SSL* s, BIO* wbio) {
   ...
   */
  if (sc->bbio != NULL) sc->wbio = BIO_pop(sc->wbio);

  BIO_free_all(sc->wbio);
  sc->wbio = wbio;

  /* Re-attach |bbio| to the new |wbio|. */
  if (sc->bbio != NULL) sc->wbio = BIO_push(sc->bbio, sc->wbio);
...
void SSL_set_bio(SSL* s, BIO* rbio, BIO* wbio) {
  ...
  if (rbio == SSL_get_rbio(s) && wbio == SSL_get_wbio(s)) return;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_conn_get_net_rbio(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_conn_get_net_wbio(s);
#endif
...
static const BIO_METHOD* fd_method(SSL* s) {
  ...
  if (IS_QUIC(s)) return BIO_s_datagram();
#endif
...
void SSL_set_verify(SSL* s, int mode,
  ...
  sc->verify_mode = mode;
  if (callback != NULL) sc->verify_callback = callback;
}
...
int SSL_has_pending(const SSL* s) {
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_has_pending(s);
#endif
  ...
  if (SSL_CONNECTION_IS_DTLS(sc)) {
    ...
    while ((item = pqueue_next(&iter)) != NULL) {
      rdata = item->data;
      if (rdata->length > 0) return 1;
    }
  }

  if (RECORD_LAYER_processed_read_pending(&sc->rlayer)) return 1;
...
X509* SSL_get1_peer_certificate(const SSL* s) {
  ...
  if (r != NULL && !X509_up_ref(r)) return NULL;
...
int SSL_copy_session_id(SSL* t, const SSL* f) {
  ...
  if (t->method != f->method) {
    ...
    t->method = f->method;
    if (t->method->ssl_init(t) == 0) return 0;
  }
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return s->method->ssl_accept(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return s->method->ssl_connect(s);
#endif
...
static int ssl_io_intern(void* vargs) {
  ...
  buf = args->buf;
  num = args->num;
  if ((sc = SSL_CONNECTION_FROM_SSL(s)) == NULL) return -1;
...
int ssl_read_internal(SSL* s, void* buf, size_t num, size_t* readbytes) {
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return s->method->ssl_read(s, buf, num, readbytes);
#endif
   ...
   * better do that
   */
  if (!ossl_statem_check_finish_init(sc, 0)) return -1;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return s->method->ssl_peek(s, buf, num, readbytes);
#endif
...
int ssl_write_internal(SSL* s, const void* buf, size_t num, uint64_t flags,
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_write_flags(s, buf, num, flags, written);
#endif
  ...
  if (!ossl_statem_check_finish_init(sc, 1)) return -1;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_conn_shutdown(s, 0, NULL, 0);
#endif
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_key_update(s, updatetype);
#endif
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_get_key_update_type(s);
#endif
...
long ossl_ctrl_internal(SSL* s, int cmd, long larg, void* parg, int no_quic) {
   ...
   */
  if (!no_quic && IS_QUIC(s)) return s->method->ssl_ctrl(s, cmd, larg, parg);
  ...
  switch (cmd) {
    ...
    case SSL_CTRL_SET_MAX_SEND_FRAGMENT:
      if (larg < 512 || larg > SSL3_RT_MAX_PLAIN_LENGTH) return 0;
#ifndef OPENSSL_NO_KTLS
      if (sc->wbio != NULL && BIO_get_ktls_send(sc->wbio)) return 0;
#endif /* OPENSSL_NO_KTLS */
    ...
    case SSL_CTRL_SET_SPLIT_SEND_FRAGMENT:
      if ((size_t)larg > sc->max_send_fragment || larg == 0) return 0;
      sc->split_send_fragment = larg;
      return 1;
    case SSL_CTRL_SET_MAX_PIPELINES:
      if (larg < 1 || larg > SSL_MAX_PIPELINES) return 0;
    ...
    case SSL_CTRL_GET_RAW_CIPHERLIST:
      if (parg) {
        if (sc->s3.tmp.ciphers_raw == NULL) return 0;
...
long SSL_CTX_ctrl(SSL_CTX* ctx, int cmd, long larg, void* parg) {
  ...
  switch (cmd) {
    ...
    case SSL_CTRL_SET_MAX_SEND_FRAGMENT:
      ...
      return 1;
    case SSL_CTRL_SET_SPLIT_SEND_FRAGMENT:
      if ((size_t)larg > ctx->max_send_fragment || larg == 0) return 0;
...
STACK_OF(SSL_CIPHER)* SSL_get_client_ciphers(const SSL* s) {
  ...
  if (sc == NULL || !sc->server) return NULL;
...
STACK_OF(SSL_CIPHER)* SSL_get1_supported_ciphers(SSL* s) {
  ...
  if (!ssl_set_client_disabled(sc)) return NULL;
  for (i = 0; i < sk_SSL_CIPHER_num(ciphers); i++) {
    const SSL_CIPHER* c = sk_SSL_CIPHER_value(ciphers, i);
    if (!ssl_cipher_disabled(sc, c, SSL_SECOP_CIPHER_SUPPORTED, 0)) {
      if (!sk) sk = sk_SSL_CIPHER_new_null();
      if (!sk) return NULL;
...
const char* SSL_get_cipher_list(const SSL* s, int n) {
  ...
  if ((sk == NULL) || (sk_SSL_CIPHER_num(sk) <= n)) return NULL;
...
STACK_OF(SSL_CIPHER)* SSL_CTX_get_ciphers(const SSL_CTX* ctx) {
  if (ctx != NULL) return ctx->cipher_list;
  return NULL;
}
...
static int cipher_list_tls12_num(STACK_OF(SSL_CIPHER)* sk) {
  ...
  for (i = 0; i < sk_SSL_CIPHER_num(sk); ++i) {
    ...
    if (c->min_tls >= TLS1_3_VERSION) continue;
    num++;
  }
  return num;
}
...
char* SSL_get_shared_ciphers(const SSL* s, char* buf, int size) {
  ...
  if (!sc->server || sc->peer_ciphers == NULL || size < 2) return NULL;

  p = buf;
  clntsk = sc->peer_ciphers;
  srvrsk = SSL_get_ciphers(s);
  if (clntsk == NULL || srvrsk == NULL) return NULL;
  ...
  for (i = 0; i < sk_SSL_CIPHER_num(clntsk); i++) {
    ...
    if (sk_SSL_CIPHER_find(srvrsk, c) < 0) continue;
...
const char* SSL_get_servername(const SSL* s, const int type) {
  ...
  if (type != TLSEXT_NAMETYPE_host_name) return NULL;
  ...
  *outlen = (unsigned char)PACKET_remaining(&csubpkt);
  ...
  if (PACKET_buf_init(&spkt, server, server_len)) {
    while (PACKET_get_length_prefixed_1(&spkt, &ssubpkt)) {
      if (PACKET_remaining(&ssubpkt) == 0) continue; /* Invalid - ignore it */
      if (PACKET_buf_init(&cpkt, client, client_len)) {
...
void SSL_CTX_set_npn_advertised_cb(SSL_CTX* ctx,
                                   ...
                                   void* arg) {
  if (IS_QUIC_CTX(ctx)) /* NPN not allowed for QUIC */
    return;
...
static int alpn_value_ok(const unsigned char* protos, unsigned int protos_len) {
  ...
  if (protos_len < 2 || protos == NULL) return 0;

  for (idx = 0; idx < protos_len; idx += protos[idx] + 1) {
    if (protos[idx] == 0) return 0;
  }
...
int SSL_CTX_set_alpn_protos(SSL_CTX* ctx, const unsigned char* protos,
  ...
  /* Not valid per RFC */
  if (!alpn_value_ok(protos, protos_len)) return 1;

  alpn = OPENSSL_memdup(protos, protos_len);
  if (alpn == NULL) return 1;
  OPENSSL_free(ctx->ext.alpn);
  ctx->ext.alpn = alpn;
...
int SSL_export_keying_material_early(SSL* s, unsigned char* out, size_t olen,
  ...
  if (sc->version != TLS1_3_VERSION) return 0;
...
static long check_keylog_bio_free(BIO* b, int oper, const char* argp,
   ...
   */
  if (oper == BIO_CB_FREE) keylog_bio = NULL;
  return ret;
}
...
static void do_sslkeylogfile(const SSL* ssl, const char* line) {
  if (keylog_lock == NULL) return;

  if (!CRYPTO_THREAD_write_lock(keylog_lock)) return;
  if (keylog_bio != NULL) {
...
static BIO* get_sslkeylog_bio(const char* keylogfile) {
  ...
  if ((b = BIO_new_fp(fp, BIO_CLOSE)) == NULL) fclose(fp);
  return b;
#else
...
SSL_CTX* SSL_CTX_new_ex(OSSL_LIB_CTX* libctx, const char* propq,
  ...
  if (!OPENSSL_init_ssl(OPENSSL_INIT_LOAD_SSL_STRINGS, NULL)) return NULL;
...
#define eng_str(x) eng_strx(x)
  /* Use specific client engine automatically... ignore errors */
  {
    ...
    if (!eng || !SSL_CTX_set_client_cert_engine(ret, eng)) ERR_clear_error();
  }
#endif
#endif
  ...
  if (meth == OSSL_QUIC_client_method())
    if ((ret->tokencache = ossl_quic_new_token_store()) == NULL) goto err;
  ret->domain_flags = 0;
  ...
  if (keylogfile != NULL && strlen(keylogfile) != 0) {
    ...
    if (!CRYPTO_THREAD_write_lock(keylog_lock)) {
      ...
      if (keylog_bio != NULL) ret->do_sslkeylog = 1;
...
int SSL_CTX_up_ref(SSL_CTX* ctx) {
  ...
  if (CRYPTO_UP_REF(&ctx->references, &i) <= 0) return 0;
...
void SSL_CTX_free(SSL_CTX* a) {
  ...
  if (keylog_lock != NULL && CRYPTO_THREAD_write_lock(keylog_lock)) {
    if (a->do_sslkeylog == 1) BIO_free(keylog_bio);
    a->do_sslkeylog = 0;
    CRYPTO_THREAD_unlock(keylog_lock);
  }
#endif
   ...
   */
  if (a->sessions != NULL) SSL_CTX_flush_sessions_ex(a, 0);
...
void ssl_set_masks(SSL_CONNECTION* s) {
  ...
  if (rsa_enc) mask_k |= SSL_kRSA;

  if (dh_tmp) mask_k |= SSL_kDHE;
  ...
  if (pvalid[SSL_PKEY_ECC] & CERT_PKEY_RPK) mask_a |= SSL_aECDSA;
  if (TLS1_get_version(&s->ssl) == TLS1_2_VERSION) {
    if (pvalid[SSL_PKEY_RSA_PSS_SIGN] & CERT_PKEY_RPK) mask_a |= SSL_aRSA;
  ...
  if (have_ecc_cert) {
    ...
    if (!(pvalid[SSL_PKEY_ECC] & CERT_PKEY_SIGN)) ecdsa_ok = 0;
    if (ecdsa_ok) mask_a |= SSL_aECDSA;
  }
  ...
  mask_k |= SSL_kPSK;
  mask_a |= SSL_aPSK;
  if (mask_k & SSL_kRSA) mask_k |= SSL_kRSAPSK;
  if (mask_k & SSL_kDHE) mask_k |= SSL_kDHEPSK;
  if (mask_k & SSL_kECDHE) mask_k |= SSL_kECDHEPSK;
#endif
...
int ssl_get_server_cert_serverinfo(SSL_CONNECTION* s,
  ...
  *serverinfo_length = 0;
  ...
  if (cpk == NULL || cpk->serverinfo == NULL) return 0;
...
void ssl_update_cache(SSL_CONNECTION* s, int mode) {
   ...
   */
  if (s->session->session_id_length == 0 || s->session->not_resumable) return;
...
int ossl_ssl_get_error(const SSL* s, int i, int check_err) {
  ...
  if (IS_QUIC(s)) {
    ...
    if (reason != SSL_ERROR_NONE) return reason;
  }
#endif

  if (sc == NULL) return SSL_ERROR_SSL;
  ...
  if (SSL_want_x509_lookup(s)) return SSL_ERROR_WANT_X509_LOOKUP;
  if (SSL_want_retry_verify(s)) return SSL_ERROR_WANT_RETRY_VERIFY;
  if (SSL_want_async(s)) return SSL_ERROR_WANT_ASYNC;
  if (SSL_want_async_job(s)) return SSL_ERROR_WANT_ASYNC_JOB;
  if (SSL_want_client_hello_cb(s)) return SSL_ERROR_WANT_CLIENT_HELLO_CB;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_do_handshake(s);
#endif
...
__owur int SSL_get_handshake_rtt(const SSL* s, uint64_t* rtt) {
  ...
  if (sc->ts_msg_read.t < sc->ts_msg_write.t) return -1;
...
static int dup_ca_names(STACK_OF(X509_NAME)** dst, STACK_OF(X509_NAME)* src) {
  ...
  if ((sk = sk_X509_NAME_new_null()) == NULL) return 0;
...
SSL* SSL_dup(SSL* s) {
  ...
  if ((retsc = SSL_CONNECTION_FROM_SSL_ONLY(ret)) == NULL) goto err;
  ...
  if (sc->session != NULL) {
     ...
     */
    if (!SSL_copy_session_id(ret, s)) goto err;
  } else {
    /*
     ...
     * SSL_copy_session_id.
     */
    if (!SSL_set_ssl_method(ret, s->method)) goto err;

    if (sc->cert != NULL) {
      ssl_cert_free(retsc->cert);
      retsc->cert = ssl_cert_dup(sc->cert);
      if (retsc->cert == NULL) goto err;
    }
  ...
  if (!ssl_dane_dup(retsc, sc)) goto err;
...
int SSL_get_shutdown(const SSL* s) {
  ...
  if (IS_QUIC(s)) return ossl_quic_get_shutdown(s);
#endif
...
SSL_CTX* SSL_set_SSL_CTX(SSL* ssl, SSL_CTX* ctx) {
  ...
  if (ssl->ctx == ctx) return ssl->ctx;
  if (ctx == NULL) ctx = sc->session_ctx;
  new_cert = ssl_cert_dup(ctx->cert);
  if (new_cert == NULL) goto err;
  if (!custom_exts_copy_conn(&new_cert->custext, &sc->cert->custext)) goto err;
  if (!custom_exts_copy_flags(&new_cert->custext, &sc->cert->custext)) goto err;

  /*
   * Program invariant: |sid_ctx| has fixed size (SSL_MAX_SID_CTX_LENGTH),
   * so setter APIs must prevent invalid lengths from entering the system.
   */
  if (!ossl_assert(sc->sid_ctx_length <= sizeof(sc->sid_ctx))) goto err;
  if (!SSL_CTX_up_ref(ctx)) goto err;
...
int SSL_CTX_load_verify_locations(SSL_CTX* ctx, const char* CAfile,
                                  const char* CApath) {
  if (CAfile == NULL && CApath == NULL) return 0;
...
size_t SSL_get_client_random(const SSL* ssl, unsigned char* out,
  ...
  if (outlen == 0) return sizeof(sc->s3.client_random);
...
size_t SSL_get_server_random(const SSL* ssl, unsigned char* out,
  ...
  if (outlen == 0) return sizeof(sc->s3.server_random);
...
size_t SSL_SESSION_get_master_key(const SSL_SESSION* session,
  ...
  if (outlen == 0) return session->master_key_length;
  if (outlen > session->master_key_length) outlen = session->master_key_length;
...
int SSL_SESSION_set1_master_key(SSL_SESSION* sess, const unsigned char* in,
                                size_t len) {
  if (len > sizeof(sess->master_key)) return 0;
...
void SSL_CTX_set1_cert_store(SSL_CTX* ctx, X509_STORE* store) {
  if (store != NULL && !X509_STORE_up_ref(store)) return;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_want(s);
#endif

  if (sc == NULL) return SSL_NOTHING;
...
int SSL_CTX_use_psk_identity_hint(SSL_CTX* ctx, const char* identity_hint) {
  ...
  if (identity_hint != NULL) {
    ...
    if (ctx->cert->psk_identity_hint == NULL) return 0;
  } else
...
int SSL_use_psk_identity_hint(SSL* s, const char* identity_hint) {
  ...
  if (identity_hint != NULL) {
    ...
    if (sc->cert->psk_identity_hint == NULL) return 0;
  } else
...
const char* SSL_get_psk_identity_hint(const SSL* s) {
  ...
  if (sc == NULL || sc->session == NULL) return NULL;
...
int SSL_CTX_set_block_padding_ex(SSL_CTX* ctx, size_t app_block_size,
  ...
  if (IS_QUIC_CTX(ctx) && (app_block_size > 1 || hs_block_size > 1)) return 0;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_get_options(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_set_options(s, op);
#endif
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_clear_options(s, op);
#endif
...
static int ct_move_scts(STACK_OF(SCT)** dst, STACK_OF(SCT)* src,
  ...
  while ((sct = sk_SCT_pop(src)) != NULL) {
    if (SCT_set_source(sct, origin) != 1) goto err;

    if (!sk_SCT_push(*dst, sct)) goto err;
    scts_moved += 1;
  }
...
static int ct_strict(const CT_POLICY_EVAL_CTX* ctx, const STACK_OF(SCT)* scts,
  ...
  for (i = 0; i < count; ++i) {
    ...
    if (status == SCT_VALIDATION_STATUS_VALID) return 1;
  }
...
int SSL_set_ct_validation_callback(SSL* s, ssl_ct_validation_cb callback,
  ...
  if (callback != NULL) {
     ...
     */
    if (!SSL_set_tlsext_status_type(s, TLSEXT_STATUSTYPE_ocsp)) return 0;
  }
...
int ssl_validate_ct(SSL_CONNECTION* s) {
  ...
  if (ret < 0) ret = 0; /* This function returns 0 on failure */
  if (!ret) SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_CALLBACK_FAILED);
   ...
   */
  if (ret <= 0) s->verify_result = X509_V_ERR_NO_VALID_SCTS;
  return ret;
}
...
size_t SSL_client_hello_get0_random(SSL* s, const unsigned char** out) {
  ...
  if (sc->clienthello == NULL) return 0;
  if (out != NULL) *out = sc->clienthello->random;
...
size_t SSL_client_hello_get0_session_id(SSL* s, const unsigned char** out) {
  ...
  if (sc->clienthello == NULL) return 0;
  if (out != NULL) *out = sc->clienthello->session_id;
...
size_t SSL_client_hello_get0_ciphers(SSL* s, const unsigned char** out) {
  ...
  if (sc->clienthello == NULL) return 0;
  if (out != NULL) *out = PACKET_data(&sc->clienthello->ciphersuites);
...
size_t SSL_client_hello_get0_compression_methods(SSL* s,
  ...
  if (sc->clienthello == NULL) return 0;
  if (out != NULL) *out = sc->clienthello->compressions;
...
int SSL_client_hello_get1_extensions_present(SSL* s, int** out,
  ...
  if ((present = OPENSSL_malloc_array(num, sizeof(*present))) == NULL) return 0;
  for (i = 0; i < sc->clienthello->pre_proc_exts_len; i++) {
    ext = sc->clienthello->pre_proc_exts + i;
    if (ext->present) {
      if (ext->received_order >= num) goto err;
...
int SSL_client_hello_get_extension_order(SSL* s, uint16_t* exts,
  ...
  if (*num_exts < num) return 0;
  for (i = 0; i < sc->clienthello->pre_proc_exts_len; i++) {
    ext = sc->clienthello->pre_proc_exts + i;
    if (ext->present) {
      if (ext->received_order >= num) return 0;
...
int SSL_client_hello_get0_ext(SSL* s, unsigned int type,
  ...
  for (i = 0; i < sc->clienthello->pre_proc_exts_len; ++i) {
    ...
    if (r->present && r->type == type) {
      if (out != NULL) *out = PACKET_data(&r->data);
      if (outlen != NULL) *outlen = PACKET_remaining(&r->data);
      return 1;
    }
  }
  return 0;
...
static int nss_keylog_int(const char* prefix, SSL_CONNECTION* sc,
...
#ifndef OPENSSL_NO_SSLKEYLOG
  if (sctx->keylog_callback == NULL && sctx->do_sslkeylog == 0) return 1;
#else
  if (sctx->keylog_callback == NULL) return 1;
#endif
  ...
  if ((out = cursor = OPENSSL_malloc(out_len)) == NULL) return 0;
...
int ssl_cache_cipherlist(SSL_CONNECTION* s, PACKET* cipher_suites,
  ...
  if (sslv2format) {
    ...
    for (s->s3.tmp.ciphers_rawlen = 0; PACKET_remaining(&sslv2ciphers) > 0;
      ...
      if (leadbyte == 0) s->s3.tmp.ciphers_rawlen += TLS_CIPHER_LEN;
    }
...
int ossl_bytes_to_cipher_list(SSL_CONNECTION* s, PACKET* cipher_suites,
  ...
  while (PACKET_copy_bytes(cipher_suites, cipher, n)) {
     ...
     */
    if (sslv2format && cipher[0] != '\0') continue;
...
int SSL_stateless(SSL* s) {
  ...
  if (ret > 0 && sc->ext.cookieok) return 1;
...
int ssl_evp_cipher_up_ref(const EVP_CIPHER* cipher) {
  ...
  if (EVP_CIPHER_get0_provider(cipher) == NULL) return 1;
...
int ssl_evp_md_up_ref(const EVP_MD* md) {
  ...
  if (EVP_MD_get0_provider(md) == NULL) return 1;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_handle_events(s);
#endif
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_get_event_timeout(s, tv, is_infinite);
#endif
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_get_rpoll_descriptor(s, desc);
#endif

  if (sc == NULL || sc->rbio == NULL) return 0;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_get_wpoll_descriptor(s, desc);
#endif

  if (sc == NULL || sc->wbio == NULL) return 0;
...
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s)) return SSL_want_read(s);
...
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s)) return SSL_want_write(s);
...
int SSL_shutdown_ex(SSL* ssl, uint64_t flags, const SSL_SHUTDOWN_EX_ARGS* args,
                    size_t args_len) {
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(ssl)) return SSL_shutdown(ssl);
...
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s)) return SSL_STREAM_TYPE_BIDI;
...
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s)) return UINT64_MAX;
...
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC(s)) return SSL_STREAM_STATE_NONE;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_get_value_uint(s, class_, id, value);
#endif
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(s)) return ossl_quic_set_value_uint(s, class_, id, value);
#endif
...
#ifndef OPENSSL_NO_QUIC
  if (!IS_QUIC_CTX(ctx)) return NULL;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC_CTX(ctx)) {
    if (!ossl_adjust_domain_flags(domain_flags, &domain_flags)) return 0;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC_CTX(ctx)) {
    if (domain_flags != NULL) *domain_flags = ctx->domain_flags;
...
#ifndef OPENSSL_NO_QUIC
  if (IS_QUIC(ssl)) return ossl_quic_get_domain_flags(ssl, domain_flags);
#endif
...
int SSL_get0_client_cert_type(const SSL* s, unsigned char** t, size_t* len) {
  ...
  if (t == NULL || len == NULL || sc == NULL) return 0;
