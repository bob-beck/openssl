...
static int ssl_undefined_function_3(SSL_CONNECTION* sc,
                                    unsigned char* r,
                                    unsigned char* s,
                                    size_t t,
                                    size_t* u) {
...
static size_t ssl_undefined_function_5(SSL_CONNECTION* sc,
                                       const char* r,
                                       size_t s,
                                       unsigned char* t) {
  return ssl_undefined_function(SSL_CONNECTION_GET_SSL(sc));
...
SSL* ossl_ssl_connection_new_int(SSL_CTX* ctx,
                                 SSL* user_ssl,
...
int SSL_CTX_dane_mtype_set(SSL_CTX* ctx,
                           const EVP_MD* md,
                           uint8_t mtype,
                           uint8_t ord) {
...
int SSL_get_changed_async_fds(SSL* s,
                              OSSL_ASYNC_FD* addfd,
                              size_t* numaddfds,
                              OSSL_ASYNC_FD* delfd,
                              size_t* numdelfds) {
...
static int ssl_start_async_job(SSL* s,
...
ossl_ssize_t SSL_sendfile(SSL* s,
                          int fd,
                          off_t offset,
                          size_t size,
                          int flags) {
 ...
 * @returns 1 always
 */
static long check_keylog_bio_free(BIO* b,
                                  int oper,
                                  const char* argp,
                                  size_t len,
                                  int argi,
...
SSL_CTX* SSL_CTX_new_ex(OSSL_LIB_CTX* libctx,
                        const char* propq,
...
int SSL_CTX_set_block_padding_ex(SSL_CTX* ctx,
...
int SSL_set_block_padding_ex(SSL* ssl,
                             size_t app_block_size,
                             size_t hs_block_size) {
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(ssl);
 ...
 */
static int ct_move_scts(STACK_OF(SCT)** dst,
                        STACK_OF(SCT)* src,
                        sct_source_t origin) {
  int scts_moved = 0;
  SCT* sct = NULL;
...
int SSL_client_hello_get_extension_order(SSL* s,
                                         uint16_t* exts,
                                         size_t* num_exts) {
  RAW_EXTENSION* ext;
  size_t num = 0, i;
...
int ssl_cache_cipherlist(SSL_CONNECTION* s,
...
int SSL_get_conn_close_info(SSL* s,
...
int SSL_CTX_set1_client_cert_type(SSL_CTX* ctx,
                                  const unsigned char* val,
                                  size_t len) {
...
int SSL_CTX_set1_server_cert_type(SSL_CTX* ctx,
                                  const unsigned char* val,
                                  size_t len) {
...
int SSL_CTX_get0_server_cert_type(const SSL_CTX* ctx,
                                  unsigned char** t,
                                  size_t* len) {
  if (t == NULL || len == NULL)
    return 0;
