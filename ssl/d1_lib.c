...
int dtls1_new(SSL* ssl) {
  ...
  if (!ssl->method->ssl_clear(ssl)) return 0;
...
int dtls1_get_timeout(const SSL_CONNECTION* s, OSSL_TIME* timeleft) {
  ...
  if (ossl_time_is_zero(s->d1->next_timeout)) return 0;
...
int dtls1_is_timer_expired(SSL_CONNECTION* s) {
  ...
  if (!dtls1_get_timeout(s, &timeleft)) return 0;
...
size_t DTLS_get_data_mtu(const SSL* ssl) {
  ...
  if (ext_overhead + DTLS1_RT_HEADER_LENGTH >= mtu) return 0;
  ...
  if (blocksize) mtu -= (mtu % blocksize);

  /* Subtract internal overhead (e.g. CBC padding len byte) */
  if (int_overhead >= mtu) return 0;
  mtu -= int_overhead;
