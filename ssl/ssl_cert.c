...
static int ssl_security_default_callback(const SSL* s,
                                         const SSL_CTX* ctx,
                                         int op,
                                         int bits,
                                         int nid,
...
int ssl_cert_set0_chain(SSL_CONNECTION* s,
                        SSL_CTX* ctx,
                        STACK_OF(X509)* chain) {
  int i, r;
...
int ssl_cert_set1_chain(SSL_CONNECTION* s,
                        SSL_CTX* ctx,
                        STACK_OF(X509)* chain) {
 ...
 * -1: Retry required
 */
static int ssl_verify_internal(SSL_CONNECTION* s,
                               STACK_OF(X509)* sk,
                               EVP_PKEY* rpk) {
  X509* x;
  int i = 0;
...
int ssl_security(const SSL_CONNECTION* s,
                 int op,
                 int bits,
                 int nid,
                 void* other) {
...
int ssl_ctx_security(const SSL_CTX* ctx,
                     int op,
                     int bits,
                     int nid,
                     void* other) {
