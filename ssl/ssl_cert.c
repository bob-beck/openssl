...
int SSL_get_ex_data_X509_STORE_CTX_idx(void) {
  if (!RUN_ONCE(&ssl_x509_store_ctx_once, ssl_x509_store_ctx_init)) return -1;
...
CERT* ssl_cert_new(size_t ssl_pkey_num) {
  ...
  /* Should never happen */
  if (!ossl_assert(ssl_pkey_num >= SSL_PKEY_NUM)) return NULL;
...
CERT* ssl_cert_dup(CERT* cert) {
  ...
  if (cert->dh_tmp != NULL) {
    if (!EVP_PKEY_up_ref(cert->dh_tmp)) goto err;
  ...
  for (i = 0; i < ret->ssl_pkey_num; i++) {
    ...
    if (cpk->x509 != NULL) {
      if (!X509_up_ref(cpk->x509)) goto err;
      rpk->x509 = cpk->x509;
    }

    if (cpk->privatekey != NULL) {
      if (!EVP_PKEY_up_ref(cpk->privatekey)) goto err;
  ...
  if (cert->conf_sigalgs) {
    ...
    if (ret->conf_sigalgs == NULL) goto err;
  ...
  if (cert->client_sigalgs) {
    ...
    if (ret->client_sigalgs == NULL) goto err;
  ...
  if (cert->verify_store) {
    if (!X509_STORE_up_ref(cert->verify_store)) goto err;
    ret->verify_store = cert->verify_store;
  }

  if (cert->chain_store) {
    if (!X509_STORE_up_ref(cert->chain_store)) goto err;
  ...
  if (!custom_exts_copy(&ret->custext, &cert->custext)) goto err;
#ifndef OPENSSL_NO_PSK
  if (cert->psk_identity_hint) {
    ret->psk_identity_hint = OPENSSL_strdup(cert->psk_identity_hint);
    if (ret->psk_identity_hint == NULL) goto err;
  }
#endif
  return ret;
...
int ssl_cert_set1_chain(SSL_CONNECTION* s, SSL_CTX* ctx,
  ...
  if (!chain) return ssl_cert_set0_chain(s, ctx, NULL);
...
int ssl_cert_add0_chain_cert(SSL_CONNECTION* s, SSL_CTX* ctx, X509* x) {
  ...
  if (!cpk->chain) cpk->chain = sk_X509_new_null();
  if (!cpk->chain || !sk_X509_push(cpk->chain, x)) return 0;
  return 1;
}

int ssl_cert_add1_chain_cert(SSL_CONNECTION* s, SSL_CTX* ctx, X509* x) {
  if (!X509_up_ref(x)) return 0;
...
int ssl_cert_set_current(CERT* c, long op) {
  ...
  if (op == SSL_CERT_SET_FIRST)
    ...
    if (idx >= c->ssl_pkey_num) return 0;
  } else
    return 0;
...
static int ssl_verify_internal(SSL_CONNECTION* s, STACK_OF(X509)* sk,
  ...
  if ((sk == NULL || sk_X509_num(sk) == 0) && rpk == NULL) return 0;
  ...
  if (DANETLS_ENABLED(&s->dane)) X509_STORE_CTX_set0_dane(ctx, &s->dane);
  ...
  if (s->verify_callback) X509_STORE_CTX_set_verify_cb(ctx, s->verify_callback);
...
STACK_OF(X509_NAME)* SSL_get_client_CA_list(const SSL* s) {
  ...
  if (!sc->server) return sc->s3.tmp.peer_ca_names;
...
static int add_ca_name(STACK_OF(X509_NAME)** sk, const X509* x) {
  ...
  if (x == NULL) return 0;
  if (*sk == NULL && ((*sk = sk_X509_NAME_new_null()) == NULL)) return 0;
...
STACK_OF(X509_NAME)* SSL_load_client_CA_file_ex(const char* file,
  ...
  for (;;) {
    ...
    if (lh_X509_NAME_retrieve(name_hash, xn) != NULL) {
      ...
      if (!sk_X509_NAME_push(ret, xn)) goto err;
    }
  }
  goto done;
...
#ifndef OPENSSL_NO_POSIX_IO
    /* Skip subdirectories */
    if (!stat(buf, &st) && S_ISDIR(st.st_mode)) continue;
#endif
    if (r <= 0 || r >= (int)sizeof(buf)) goto err;
    if (!add_file_cert_subjects_to_stack(stack, buf, name_hash)) goto err;
  }
...
static int add_uris_recursive(STACK_OF(X509_NAME)* stack, const char* uri,
  ...
  if ((ctx = OSSL_STORE_open(uri, NULL, NULL, NULL, NULL)) == NULL) goto err;

  while (!OSSL_STORE_eof(ctx) && !OSSL_STORE_error(ctx)) {
    int infotype;

    if ((info = OSSL_STORE_load(ctx)) == NULL) continue;
...
int ssl_build_cert_chain(SSL_CONNECTION* s, SSL_CTX* ctx, int flags) {
  ...
  if (flags & SSL_BUILD_CHAIN_FLAG_CHECK) {
    ...
    if (chain_store == NULL) goto err;
    for (i = 0; i < sk_X509_num(cpk->chain); i++) {
      x = sk_X509_value(cpk->chain, i);
      if (!X509_STORE_add_cert(chain_store, x)) goto err;
    }
    /* Add EE cert too: it might be self signed */
    if (!X509_STORE_add_cert(chain_store, cpk->x509)) goto err;
  } else {
    ...
    if (flags & SSL_BUILD_CHAIN_FLAG_UNTRUSTED) untrusted = cpk->chain;
  }
  ...
  if (i <= 0 && flags & SSL_BUILD_CHAIN_FLAG_IGNORE_ERROR) {
    if (flags & SSL_BUILD_CHAIN_FLAG_CLEAR_ERROR) ERR_clear_error();
    i = 1;
    rv = 2;
  }
  if (i > 0) chain = X509_STORE_CTX_get1_chain(xs_ctx);
  if (i <= 0) {
  ...
  if (rv == 0) rv = 1;
err:
  if (flags & SSL_BUILD_CHAIN_FLAG_CHECK) X509_STORE_free(chain_store);
...
static int ssl_security_default_callback(const SSL* s, const SSL_CTX* ctx,
  ...
  if (level == 0) {
     ...
     * anything goes.
     */
    if (op == SSL_SECOP_TMP_DH && bits < 80) return 0;
    return 1;
  }
  switch (op) {
    ...
    case SSL_SECOP_CIPHER_CHECK: {
      ...
      if (minbits > 160 && c->algorithm_mac & SSL_SHA1) return 0;
    ...
    case SSL_SECOP_VERSION:
      if ((sc = SSL_CONNECTION_FROM_CONST_SSL(s)) == NULL) return 0;
      if (!SSL_CONNECTION_IS_DTLS(sc)) {
        /* SSLv3, TLS v1.0 and TLS v1.1 only allowed at level 0 */
        if (nid <= TLS1_1_VERSION && level > 0) return 0;
      } else {
        /* DTLS v1.0 only allowed at level 0 */
        if (DTLS_VERSION_LT(nid, DTLS1_2_VERSION) && level > 0) return 0;
      }
      break;
...
const SSL_CERT_LOOKUP* ssl_cert_lookup_by_pkey(const EVP_PKEY* pk, size_t* pidx,
  ...
  for (i = 0; i < ctx->sigalg_list_len; i++) {
    ...
    if (EVP_PKEY_is_a(pk, OBJ_nid2sn(tmp_lu->nid)) ||
        EVP_PKEY_is_a(pk, OBJ_nid2ln(tmp_lu->nid))) {
      if (pidx != NULL) *pidx = SSL_PKEY_NUM + i;
