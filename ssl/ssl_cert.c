...
static int
ssl_security_default_callback(const SSL* s,
                              const SSL_CTX* ctx,
                              int op,
                              int bits,
                              int nid,
...
ssl_cert_dup(CERT* cert)
{
  ...
  if (cert->conf_sigalgs) {
    ret->conf_sigalgs =
      OPENSSL_malloc_array(cert->conf_sigalgslen, sizeof(*cert->conf_sigalgs));
    if (ret->conf_sigalgs == NULL)
      goto err;
    memcpy(ret->conf_sigalgs,
           cert->conf_sigalgs,
...
int
ssl_cert_set_current(CERT* c, long op)
{
  size_t i, idx;
 ...
 */
static int
ssl_verify_internal(SSL_CONNECTION* s, STACK_OF(X509)* sk, EVP_PKEY* rpk)
{
  X509* x;
  int i = 0;
...
static void
set0_CA_list(STACK_OF(X509_NAME)** ca_list, STACK_OF(X509_NAME)* name_list)
{
...
void
SSL_set0_CA_list(SSL* s, STACK_OF(X509_NAME)* name_list)
{
...
static int
add_ca_name(STACK_OF(X509_NAME)** sk, const X509* x)
{
  X509_NAME* name;
...
int
SSL_add1_to_CA_list(SSL* ssl, const X509* x)
{
...
int
SSL_CTX_add1_to_CA_list(SSL_CTX* ctx, const X509* x)
{
...
static int
xname_cmp(const X509_NAME* a, const X509_NAME* b)
{
...
static int
xname_sk_cmp(const X509_NAME* const* a, const X509_NAME* const* b)
{
  return xname_cmp(*a, *b);
}

static unsigned long
xname_hash(const X509_NAME* a)
{
...
static int
add_file_cert_subjects_to_stack(STACK_OF(X509_NAME)* stack,
                                const char* file,
...
SSL_add_dir_cert_subjects_to_stack(STACK_OF(X509_NAME)* stack, const char* dir)
{
  ...
  if (errno) {
    ERR_raise_data(
      ERR_LIB_SYS, get_last_sys_error(), "calling OPENSSL_dir_read(%s)", dir);
...
static int
add_uris_recursive(STACK_OF(X509_NAME)* stack, const char* uri, int depth)
{
  int ok = 1;
...
{
  return s->cert->sec_cb(SSL_CONNECTION_GET_USER_SSL(s),
                         NULL,
                         op,
                         bits,
                         nid,
