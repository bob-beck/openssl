...
static int serverinfo_find_extension(const unsigned char* serverinfo,
  ...
  *extension_data = NULL;
  *extension_length = 0;
  if (serverinfo == NULL || serverinfo_length == 0) return -1;

  if (!PACKET_buf_init(&pkt, serverinfo, serverinfo_length)) return -1;
  ...
  for (;;) {
    ...
    /* end of serverinfo */
    if (PACKET_remaining(&pkt) == 0) return 0; /* Extension not found */
...
static int serverinfoex_srv_add_cb(SSL* s, unsigned int ext_type,
  ...
  if ((context & SSL_EXT_TLS1_3_CERTIFICATE) != 0 && chainidx > 0) return 0;
  ...
  if ((ssl_get_server_cert_serverinfo(sc, &serverinfo, &serverinfo_length)) !=
    ...
    if (retval == 0) return 0; /* No extension found, don't send extension */
...
static int serverinfo_process_buffer(unsigned int version,
  ...
  if (!PACKET_buf_init(&pkt, serverinfo, serverinfo_length)) return 0;
...
static int ssl_set_cert_and_key(SSL* ssl, SSL_CTX* ctx, X509* x509,
  ...
  if (ctx == NULL && (sc = SSL_CONNECTION_FROM_SSL(ssl)) == NULL) return 0;
