...
static int
ssl_set_cert(CERT* c, X509* x509, SSL_CTX* ctx);
static int
ssl_set_pkey(CERT* c, EVP_PKEY* pkey, SSL_CTX* ctx);
...
SSL_use_PrivateKey_file(SSL* ssl, const char* file, int type)
{
  ...
  if (type == SSL_FILETYPE_PEM) {
    ...
    j = ERR_R_PEM_LIB;
    pkey = PEM_read_bio_PrivateKey_ex(in,
                                      NULL,
                                      sc->default_passwd_callback,
                                      sc->default_passwd_callback_userdata,
                                      ssl->ctx->libctx,
...
SSL_use_PrivateKey_ASN1(int type, SSL* ssl, const unsigned char* d, long len)
{
  ...
  p = d;
  if ((pkey = d2i_PrivateKey_ex(
         type, NULL, &p, (long)len, ssl->ctx->libctx, ssl->ctx->propq)) ==
      NULL) {
    ERR_raise(ERR_LIB_SSL, ERR_R_ASN1_LIB);
    return 0;
  }
...
static int
ssl_set_cert(CERT* c, X509* x, SSL_CTX* ctx)
{
  EVP_PKEY* pkey;
  size_t i;
...
int
SSL_CTX_use_PrivateKey_ASN1(int type,
                            SSL_CTX* ctx,
                            ...
                            long len)
{
  ...
  p = d;
  if ((pkey = d2i_PrivateKey_ex(
         type, NULL, &p, (long)len, ctx->libctx, ctx->propq)) == NULL) {
    ERR_raise(ERR_LIB_SSL, ERR_R_ASN1_LIB);
    return 0;
  }
...
use_certificate_chain_file(SSL_CTX* ctx, SSL* ssl, const char* file)
{
  ...
  if (PEM_read_bio_X509_AUX(
        in, &x, passwd_callback, passwd_callback_userdata) == NULL) {
  ...
  if (ret) {
    ...
    while (1) {
      ...
      if (PEM_read_bio_X509(
            in, &ca, passwd_callback, passwd_callback_userdata) != NULL) {
        if (ctx)
...
static int
serverinfo_find_extension(const unsigned char* serverinfo,
...
static int
serverinfoex_srv_parse_cb(SSL* s,
 ...
 */
static int
serverinfo_process_buffer(unsigned int version,
                          ...
                          SSL_CTX* ctx)
{
  ...
  while (PACKET_remaining(&pkt)) {
    ...
    if (version == SSL_SERVERINFOV1 || context == SYNTHV1CONTEXT) {
      if (!SSL_CTX_add_server_custom_ext(ctx,
                                         ext_type,
...
static size_t
extension_contextoff(unsigned int version)
{
  return version == SSL_SERVERINFOV1 ? 4 : 0;
}

static size_t
extension_append_length(unsigned int version, size_t extension_length)
{
  return extension_length + extension_contextoff(version);
}

static void
extension_append(unsigned int version,
...
int
SSL_CTX_use_serverinfo_ex(SSL_CTX* ctx,
                          ...
                          size_t serverinfo_length)
{
  ...
  if (!serverinfo_process_buffer(
        version, serverinfo, serverinfo_length, NULL)) {
...
int
SSL_CTX_use_serverinfo(SSL_CTX* ctx,
                       const unsigned char* serverinfo,
                       size_t serverinfo_length)
{
  return SSL_CTX_use_serverinfo_ex(
    ctx, SSL_SERVERINFOV1, serverinfo, serverinfo_length);
}
...
SSL_CTX_use_serverinfo_file(SSL_CTX* ctx, const char* file)
{
  ...
  for (num_extensions = 0;; num_extensions++) {
    ...
    serverinfo = tmp;
    extension_append(
      version, extension, extension_length, serverinfo + serverinfo_length);
  ...
  ret = SSL_CTX_use_serverinfo_ex(
    ctx, SSL_SERVERINFOV2, serverinfo, serverinfo_length);
end:
...
int
SSL_CTX_use_cert_and_key(SSL_CTX* ctx,
                         X509* x509,
                         EVP_PKEY* privatekey,
                         STACK_OF(X509)* chain,
                         int override)
