...
OSSL_COMP_CERT_new(unsigned char* data, size_t len, size_t orig_len, int alg)
{
...
OSSL_COMP_CERT_from_compressed_data(unsigned char* data,
                                    size_t len,
                                    size_t orig_len,
                                    int alg)
{
...
OSSL_COMP_CERT_from_uncompressed_data(unsigned char* data, size_t len, int alg)
{
  ...
  comp_length =
    COMP_compress_block(comp_ctx, comp_data, (int)max_length, data, (int)len);
  if (comp_length <= 0)
    goto err;
...
void
OSSL_COMP_CERT_free(OSSL_COMP_CERT* cc)
{
  int i;
...
int
OSSL_COMP_CERT_up_ref(OSSL_COMP_CERT* cc)
{
  int i;
...
static int
ssl_set_cert_comp_pref(int* prefs, int* algs, size_t len)
{
  size_t j = 0;
  size_t i;
  int found = 0;
...
static size_t
ssl_get_cert_to_compress(SSL* ssl, CERT_PKEY* cpk, unsigned char** data)
{
...
static int
ssl_compress_one_cert(SSL* ssl, CERT_PKEY* cpk, int alg)
{
...
static int
ssl_compress_certs(SSL* ssl, CERT_PKEY* cpks, int alg_in)
{
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(ssl);
  int i;
  int j;
...
static size_t
ssl_get_compressed_cert(SSL* ssl,
                        CERT_PKEY* cpk,
                        int alg,
                        unsigned char** data,
                        size_t* orig_len)
...
static int
ossl_set1_compressed_cert(CERT* cert,
                          int algorithm,
                          ...
                          size_t orig_length)
{
  ...
  comp_cert = OSSL_COMP_CERT_from_compressed_data(
    comp_data, comp_length, orig_length, algorithm);
  if (comp_cert == NULL)
    return 0;
 ...
 */
int
SSL_CTX_set1_cert_comp_preference(SSL_CTX* ctx, int* algs, size_t len)
{
#ifndef OPENSSL_NO_COMP_ALG
...
int
SSL_set1_cert_comp_preference(SSL* ssl, int* algs, size_t len)
{
#ifndef OPENSSL_NO_COMP_ALG
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(ssl);
...
int
SSL_compress_certs(SSL* ssl, int alg)
{
#ifndef OPENSSL_NO_COMP_ALG
  SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(ssl);
...
int
SSL_CTX_compress_certs(SSL_CTX* ctx, int alg)
{
  int ret = 0;
#ifndef OPENSSL_NO_COMP_ALG
  SSL* new = SSL_new(ctx);
...
size_t
SSL_get1_compressed_cert(SSL* ssl,
                         int alg,
                         unsigned char** data,
                         size_t* orig_len)
{
...
size_t
SSL_CTX_get1_compressed_cert(SSL_CTX* ctx,
                             int alg,
                             unsigned char** data,
                             size_t* orig_len)
{
...
int
SSL_CTX_set1_compressed_cert(SSL_CTX* ctx,
                             int algorithm,
                             unsigned char* comp_data,
                             size_t comp_length,
                             size_t orig_length)
{
  ...
  return ossl_set1_compressed_cert(
    ctx->cert, algorithm, comp_data, comp_length, orig_length);
#else
  return 0;
#endif
}
...
int
SSL_set1_compressed_cert(SSL* ssl,
                         int algorithm,
                         unsigned char* comp_data,
                         size_t comp_length,
                         size_t orig_length)
{
  ...
  return ossl_set1_compressed_cert(
    sc->cert, algorithm, comp_data, comp_length, orig_length);
#else
  return 0;
#endif
}
