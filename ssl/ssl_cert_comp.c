...
int ossl_comp_has_alg(int a) {
  ...
  if ((a == 0 || a == TLSEXT_comp_cert_brotli) && BIO_f_brotli() != NULL)
    return 1;
  if ((a == 0 || a == TLSEXT_comp_cert_zstd) && BIO_f_zstd() != NULL) return 1;
  if ((a == 0 || a == TLSEXT_comp_cert_zlib) && BIO_f_zlib() != NULL) return 1;
#endif
  return 0;
}
...
__owur static OSSL_COMP_CERT* OSSL_COMP_CERT_from_compressed_data(
    unsigned char* data, size_t len, size_t orig_len, int alg) {
...
__owur static OSSL_COMP_CERT* OSSL_COMP_CERT_from_uncompressed_data(
    unsigned char* data, size_t len, int alg) {
  OSSL_COMP_CERT* ret = NULL;
  size_t max_length;
  int comp_length;
  COMP_METHOD* method;
...
int OSSL_COMP_CERT_up_ref(OSSL_COMP_CERT* cc) {
  ...
  if (CRYPTO_UP_REF(&cc->references, &i) <= 0) return 0;
...
static int ssl_set_cert_comp_pref(int* prefs, int* algs, size_t len) {
  ...
  for (i = 0; i < len; i++) {
    if (algs[i] != 0 && ossl_comp_has_alg(algs[i])) {
      /* Check for duplicate */
      if (already_set[algs[i]]) return 0;
      tmp_prefs[j++] = algs[i];
      already_set[algs[i]] = 1;
      found = 1;
    }
  }
  if (found) memcpy(prefs, tmp_prefs, sizeof(tmp_prefs));
  return found;
}
...
static int ssl_compress_certs(SSL* ssl, CERT_PKEY* cpks, int alg_in) {
  ...
  if (sc == NULL || cpks == NULL || !ossl_comp_has_alg(alg_in)) return 0;
  ...
  for (i = 0; i < TLSEXT_comp_cert_limit; i++) {
    ...
    if ((alg_in == 0 && alg != TLSEXT_comp_cert_none) ||
      ...
      for (j = 0; j < SSL_PKEY_NUM; j++) {
        /* No cert, move on */
        if (cpks[j].x509 == NULL) continue;

        if (!ssl_compress_one_cert(ssl, &cpks[j], alg)) return 0;
...
static int ossl_set1_compressed_cert(CERT* cert, int algorithm,
  ...
  if (cert == NULL || cert->key == NULL) return 0;
...
int SSL_compress_certs(SSL* ssl, int alg) {
  ...
  if (sc == NULL || sc->cert == NULL) return 0;
