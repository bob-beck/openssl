...
static int
tls1_generate_key_block(SSL_CONNECTION* s, unsigned char* km, size_t num)
{
  int ret;
...
static int
tls_iv_length_within_key_block(const EVP_CIPHER* c)
{
...
tls1_setup_key_block(SSL_CONNECTION* s)
{
  ...
  OSSL_TRACE_BEGIN(TLS)
  {
    ...
    BIO_dump_indent(
      trc_out, s->session->master_key, (int)s->session->master_key_length, 4);
  }
  OSSL_TRACE_END(TLS);
                            ...
                            size_t* secret_size)
{
  ...
  OSSL_TRACE_BEGIN(TLS)
  {
    ...
    BIO_dump_indent(
      trc_out, s->session->master_key, SSL3_MASTER_SECRET_SIZE, 4);
  }
  OSSL_TRACE_END(TLS);
                            ...
                            int use_context)
{
   ...
   */
  if (memcmp(
        val, TLS_MD_CLIENT_FINISH_CONST, TLS_MD_CLIENT_FINISH_CONST_SIZE) == 0)
    goto err1;
  if (memcmp(
        val, TLS_MD_SERVER_FINISH_CONST, TLS_MD_SERVER_FINISH_CONST_SIZE) == 0)
    goto err1;
  if (memcmp(
        val, TLS_MD_MASTER_SECRET_CONST, TLS_MD_MASTER_SECRET_CONST_SIZE) == 0)
    goto err1;
  if (memcmp(val,
             TLS_MD_EXTENDED_MASTER_SECRET_CONST,
    ...
    goto err1;
  if (memcmp(
        val, TLS_MD_KEY_EXPANSION_CONST, TLS_MD_KEY_EXPANSION_CONST_SIZE) == 0)
    goto err1;
