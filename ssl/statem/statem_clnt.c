 ...
 * handshake state transitions when a TLS1.3 client is reading messages from
 * the server. The message type that the server has sent is provided in |mt|.
 ...
 */
int
ossl_statem_client_construct_message (SSL_CONNECTION *s, confunc_f *confunc,
                                      int *mt)
{
  OSSL_STATEM *st = &s->statem;
...
tls_process_ske_dhe (SSL_CONNECTION *s, PACKET *pkt, EVP_PKEY **pkey)
{
  ...
  if (!ssl_security (s, SSL_SECOP_TMP_DH,
                     EVP_PKEY_get_security_bits (peer_tmp), 0, peer_tmp))
    {
 ...
 * server's initial flight. In TLS1.3 this is after the Server Finished
 * message. In <=TLS1.2 this is after the ServerDone message. Returns 1 on
...
tls_construct_client_compressed_certificate (SSL_CONNECTION *sc, WPACKET *pkt)
{
  ...
  comp_len
      = COMP_compress_block (comp, WPACKET_get_curr (pkt), (int)max_length,
...
int
ssl_cipher_list_to_bytes (SSL_CONNECTION *s, STACK_OF (SSL_CIPHER) *sk,
                          WPACKET *pkt)
{
  int i;
