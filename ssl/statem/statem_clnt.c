    ...
    case TLS_ST_CW_FINISHED:
        ...
        if (SSL_CONNECTION_IS_TLS13(s))
        {
            ...
            if (s->post_handshake_auth != SSL_PHA_REQUESTED)
            {
                 ...
                 * keys.
                 */
                if (SSL_IS_QUIC_HANDSHAKE(s) && !ssl->method->ssl3_enc->change_cipher_state(
...
MSG_PROCESS_RETURN tls_process_server_certificate(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if ((SSL_CONNECTION_IS_TLS13(s) && !PACKET_get_1(pkt, &context)) || context != 0 ||
        !PACKET_get_net_3(pkt, &cert_list_len) || PACKET_remaining(pkt) != cert_list_len || PACKET_remaining(pkt) == 0)
    {
        SSLfatal(s, SSL_AD_DECODE_ERROR, SSL_R_LENGTH_MISMATCH);
        goto err;
    }
...
int tls_process_cert_status_body(SSL_CONNECTION *s, size_t chainidx, PACKET *pkt)
{
    ...
    if (PACKET_remaining(pkt) > 0)
    {
        ...
        if (resplen > 0)
        {
            ...
            if (resp == NULL)
            {
                SSLfatal(s, TLS1_AD_BAD_CERTIFICATE_STATUS_RESPONSE, SSL_R_TLSV1_BAD_CERTIFICATE_STATUS_RESPONSE);
                return 0;
            }
