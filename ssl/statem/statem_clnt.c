...
int ossl_statem_client_read_transition(SSL_CONNECTION* s, int mt) {
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    if (!ossl_statem_client13_read_transition(s, mt)) goto err;
    return 1;
  }
...
WORK_STATE ossl_statem_client_post_work(SSL_CONNECTION* s, WORK_STATE wst) {
  ...
  switch (st->hand_state) {
...
#endif
      if (statem_flush(s) != 1) return WORK_MORE_B;
    ...
    case TLS_ST_CW_KEY_UPDATE:
      if (statem_flush(s) != 1) return WORK_MORE_A;
...
size_t ossl_statem_client_max_message_size(SSL_CONNECTION* s) {
  ...
  switch (st->hand_state) {
    ...
    case TLS_ST_CR_CHANGE:
      if (s->version == DTLS1_BAD_VER) return 3;
...
MSG_PROCESS_RETURN tls_process_server_hello(SSL_CONNECTION* s, PACKET* pkt) {
  ...
  if (SSL_CONNECTION_IS_DTLS(s) && s->hit) {
    ...
    if (s->mode & SSL_MODE_DTLS_SCTP_LABEL_LENGTH_BUG) labellen += 1;
...
static WORK_STATE tls_post_process_server_rpk(SSL_CONNECTION* sc,
  ...
  if (sc->rwstate == SSL_RETRY_VERIFY) sc->rwstate = SSL_NOTHING;
...
WORK_STATE tls_post_process_server_certificate(SSL_CONNECTION* s,
  ...
  if (s->rwstate == SSL_RETRY_VERIFY) s->rwstate = SSL_NOTHING;
  ...
  ERR_pop_to_mark(); /* but we keep s->verify_result */
  if (i > 0 && s->rwstate == SSL_RETRY_VERIFY) return WORK_MORE_A;
...
MSG_PROCESS_RETURN tls_process_new_session_ticket(SSL_CONNECTION* s,
   ...
   */
  if (ticklen == 0) return MSG_PROCESS_CONTINUE_READING;
 ...
 */
int tls_process_cert_status_body(SSL_CONNECTION* s, size_t chainidx,
                                 PACKET* pkt) {
  unsigned int type;
...
CON_FUNC_RETURN tls_construct_client_key_exchange(SSL_CONNECTION* s,
   ...
   */
  if ((alg_k & SSL_PSK) && !tls_construct_cke_psk_preamble(s, pkt)) goto err;

  if (alg_k & (SSL_kRSA | SSL_kRSAPSK)) {
    if (!tls_construct_cke_rsa(s, pkt)) goto err;
  } else if (alg_k & (SSL_kDHE | SSL_kDHEPSK)) {
    if (!tls_construct_cke_dhe(s, pkt)) goto err;
  } else if (alg_k & (SSL_kECDHE | SSL_kECDHEPSK)) {
    if (!tls_construct_cke_ecdhe(s, pkt)) goto err;
  } else if (alg_k & SSL_kGOST) {
    if (!tls_construct_cke_gost(s, pkt)) goto err;
  } else if (alg_k & SSL_kGOST18) {
    if (!tls_construct_cke_gost18(s, pkt)) goto err;
  } else if (alg_k & SSL_kSRP) {
    if (!tls_construct_cke_srp(s, pkt)) goto err;
...
static int ssl3_check_client_certificate(SSL_CONNECTION* s) {
  ...
  if (!tls_choose_sigalg(s, 0) || s->s3.tmp.sigalg == NULL) return 0;
  /*
...
WORK_STATE tls_prepare_client_certificate(SSL_CONNECTION* s, WORK_STATE wst) {
  ...
  if (wst == WORK_MORE_B) {
    ...
    X509_free(x509);
    EVP_PKEY_free(pkey);
    if (i && !ssl3_check_client_certificate(s)) i = 0;
    if (i == 0) {
      if (s->version == SSL3_VERSION) {
        s->s3.tmp.cert_req = 0;
    ...
    if (s->post_handshake_auth == SSL_PHA_REQUESTED) return WORK_FINISHED_STOP;
    return WORK_FINISHED_CONTINUE;
  }
...
CON_FUNC_RETURN tls_construct_client_certificate(SSL_CONNECTION* s,
  ...
  if (s->s3.tmp.cert_req != 2) cpk = s->cert->key;
...
CON_FUNC_RETURN tls_construct_client_compressed_certificate(SSL_CONNECTION* sc,
  ...
  if ((buf = BUF_MEM_new()) == NULL || !WPACKET_init(&tmppkt, buf)) goto err;

  /* Use the |tmppkt| for the to-be-compressed data */
  if (sc->pha_context == NULL) {
    /* no context available, add 0-length context */
    if (!WPACKET_put_bytes_u8(&tmppkt, 0)) goto err;
...
int ssl3_check_cert_and_algorithm(SSL_CONNECTION* s) {
  ...
  if (s->session->peer_rpk != NULL) return 1;

  if (clu->amask & SSL_aECDSA) {
    if (ssl_check_srvr_ecc_cert_and_alg(s->session->peer, s)) return 1;
...
#error Max cipher length too short
  ...
  if (empty_reneg_info_scsv) maxlen -= 2;
  if (s->mode & SSL_MODE_SEND_FALLBACK_SCSV) maxlen -= 2;
