...
tls_process_as_hello_retry_request(SSL_CONNECTION* s, PACKET* pkt);
static MSG_PROCESS_RETURN
tls_process_encrypted_extensions(SSL_CONNECTION* s, PACKET* pkt);

static ossl_inline int
cert_req_allowed(SSL_CONNECTION* s);
static int
key_exchange_expected(SSL_CONNECTION* s);
static int
ssl_cipher_list_to_bytes(SSL_CONNECTION* s,
                         STACK_OF(SSL_CIPHER)* sk,
                         WPACKET* pkt);

static ossl_inline int
received_server_cert(SSL_CONNECTION* sc)
{
 ...
 */
static int
ossl_statem_client13_read_transition(SSL_CONNECTION* s, int mt)
{
...
static int
do_compressed_cert(SSL_CONNECTION* sc)
{
 ...
 */
static WRITE_TRAN
ossl_statem_client13_write_transition(SSL_CONNECTION* s)
{
  OSSL_STATEM* st = &s->statem;
...
tls_construct_client_hello(SSL_CONNECTION* s, WPACKET* pkt)
{
  ...
  if (i && ssl_fill_hello_random(
             s, 0, p, sizeof(s->s3.client_random), DOWNGRADE_NONE) <= 0) {
    SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
    return CON_FUNC_ERROR;
  }
  ...
  if (!ssl_cipher_list_to_bytes(
        s, SSL_get_ciphers(SSL_CONNECTION_GET_SSL(s)), pkt)) {
    /* SSLfatal() already called */
    return CON_FUNC_ERROR;
  }
...
static int
set_client_ciphersuite(SSL_CONNECTION* s, const unsigned char* cipherchars)
{
  STACK_OF(SSL_CIPHER)* sk;
  const SSL_CIPHER* c;
  int i;
  ...
  if (s->hit && (s->session->cipher_id != c->id)) {
    if (SSL_CONNECTION_IS_TLS13(s)) {
      ...
      if (md == NULL || md != ssl_md(sctx, s->session->cipher->algorithm2)) {
        SSLfatal(
          s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_CIPHERSUITE_DIGEST_HAS_CHANGED);
        return 0;
      }
    } else {
      /*
       ...
       */
      SSLfatal(
        s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_OLD_SESSION_CIPHER_NOT_RETURNED);
      return 0;
    }
  }
...
tls_process_server_hello(SSL_CONNECTION* s, PACKET* pkt)
{
  ...
  if (s->hit) {
    ...
    if (!SSL_CONNECTION_IS_TLS13(s)) {
      ...
      if (session_id_len > 0)
        memcpy(
          s->session->session_id, PACKET_data(&session_id), session_id_len);
    }
  }
  ...
  if (compression != 0) {
    SSLfatal(
      s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_UNSUPPORTED_COMPRESSION_ALGORITHM);
    goto err;
  }
  /*
...
static MSG_PROCESS_RETURN
tls_process_as_hello_retry_request(SSL_CONNECTION* s, PACKET* extpkt)
{
  RAW_EXTENSION* extensions = NULL;
  ...
  if (!tls_collect_extensions(
        s, extpkt, SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST, &extensions, NULL, 1) ||
      !tls_parse_all_extensions(
        s, SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST, extensions, NULL, 0, 1)) {
    /* SSLfatal() already called */
    goto err;
  }
...
static WORK_STATE
tls_post_process_server_rpk(SSL_CONNECTION* sc, WORK_STATE wst)
{
  size_t certidx;
  ...
  if (v_ok <= 0 && sc->verify_mode != SSL_VERIFY_NONE) {
    ERR_clear_last_mark();
    SSLfatal(sc,
             ssl_x509err2alert(sc->verify_result),
  ...
  if ((clu = ssl_cert_lookup_by_pkey(sc->session->peer_rpk,
                                     &certidx,
...
tls_post_process_server_certificate(SSL_CONNECTION* s, WORK_STATE wst)
{
  ...
  if (i <= 0 && s->verify_mode != SSL_VERIFY_NONE) {
    ERR_clear_last_mark();
    SSLfatal(
      s, ssl_x509err2alert(s->verify_result), SSL_R_CERTIFICATE_VERIFY_FAILED);
    return WORK_ERROR;
  }
  ERR_pop_to_mark(); /* but we keep s->verify_result */
  ...
  if ((clu = ssl_cert_lookup_by_pkey(
         pkey, &certidx, SSL_CONNECTION_GET_CTX(s))) == NULL) {
...
static int
tls_process_ske_psk_preamble(SSL_CONNECTION* s, PACKET* pkt)
{
...
static int
tls_process_ske_srp(SSL_CONNECTION* s, PACKET* pkt, EVP_PKEY** pkey)
{
  ...
  if ((s->srp_ctx.N = BN_bin2bn(
      ...
      (s->srp_ctx.B = BN_bin2bn(PACKET_data(&server_pub),
                                (int)PACKET_remaining(&server_pub),
                                NULL)) == NULL) {
...
static int
tls_process_ske_dhe(SSL_CONNECTION* s, PACKET* pkt, EVP_PKEY** pkey)
{
...
static int
tls_process_ske_ecdhe(SSL_CONNECTION* s, PACKET* pkt, EVP_PKEY** pkey)
{
  PACKET encoded_pt;
...
tls_process_key_exchange(SSL_CONNECTION* s, PACKET* pkt)
{
  ...
  if (pkey != NULL) {
    ...
    tbslen = construct_key_exchange_tbs(
      s, &tbs, PACKET_data(&params), PACKET_remaining(&params));
    if (tbslen == 0) {
      /* SSLfatal() already called */
      goto err;
    }

    rv = EVP_DigestVerify(md_ctx,
...
tls_process_certificate_request(SSL_CONNECTION* s, PACKET* pkt)
{
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    if (!tls_collect_extensions(s,
                                ...
                                1) ||
        !tls_parse_all_extensions(
          s, SSL_EXT_TLS1_3_CERTIFICATE_REQUEST, rawexts, NULL, 0, 1)) {
      /* SSLfatal() already called */
      OPENSSL_free(rawexts);
      return MSG_PROCESS_ERROR;
    }
...
tls_process_new_session_ticket(SSL_CONNECTION* s, PACKET* pkt)
{
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    if (!tls_collect_extensions(
        ...
        !tls_parse_all_extensions(
          s, SSL_EXT_TLS1_3_NEW_SESSION_TICKET, exts, NULL, 0, 1)) {
      /* SSLfatal() already called */
      goto err;
    }
  }
   ...
   */
  if (!EVP_Digest(s->session->ext.tick,
                  ticklen,
                  s->session->session_id,
                  &sess_len,
                  sha256,
...
static int
tls_construct_cke_psk_preamble(SSL_CONNECTION* s, WPACKET* pkt)
{
#ifndef OPENSSL_NO_PSK
  int ret = 0;
  /*
...
static int
tls_construct_cke_rsa(SSL_CONNECTION* s, WPACKET* pkt)
{
...
static int
tls_construct_cke_dhe(SSL_CONNECTION* s, WPACKET* pkt)
{
...
static int
tls_construct_cke_ecdhe(SSL_CONNECTION* s, WPACKET* pkt)
{
...
static int
tls_construct_cke_gost(SSL_CONNECTION* s, WPACKET* pkt)
{
  ...
  if ((pkey = tls_get_peer_pkey(s)) == NULL) {
    SSLfatal(
      s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_NO_GOST_CERTIFICATE_SENT_BY_PEER);
    return 0;
  }
...
static int
tls_construct_cke_gost18(SSL_CONNECTION* s, WPACKET* pkt)
{
#ifndef OPENSSL_NO_GOST
...
static int
tls_construct_cke_srp(SSL_CONNECTION* s, WPACKET* pkt)
{
#ifndef OPENSSL_NO_SRP
 ...
 */
static int
ssl3_check_client_certificate(SSL_CONNECTION* s)
{
...
tls_construct_client_certificate(SSL_CONNECTION* s, WPACKET* pkt)
{
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    } else if (!WPACKET_sub_memcpy_u8(
                 pkt, s->pha_context, s->pha_context_len)) {
      SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
      return CON_FUNC_ERROR;
    }
  }
...
tls_construct_client_compressed_certificate(SSL_CONNECTION* sc, WPACKET* pkt)
{
  ...
  if (sc->pha_context == NULL) {
    ...
    if (!WPACKET_put_bytes_u8(&tmppkt, 0))
      goto err;
               ...
               &tmppkt, sc->pha_context, sc->pha_context_len))
    goto err;
  ...
  comp_len = COMP_compress_block(comp,
...
tls_process_encrypted_extensions(SSL_CONNECTION* s, PACKET* pkt)
{
  ...
  if (!tls_collect_extensions(s,
                              ...
                              1) ||
      !tls_parse_all_extensions(
        s, SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS, rawexts, NULL, 0, 1)) {
    /* SSLfatal() already called */
    goto err;
  }
...
#error Max cipher length too short
  ...
  if (totlen == 0 || !maxverok) {
    ...
    SSLfatal_data(
      s, SSL_AD_INTERNAL_ERROR, SSL_R_NO_CIPHERS_AVAILABLE, maxvertext);
    return 0;
  }
