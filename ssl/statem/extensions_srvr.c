 ...
 * EVP_MAX_MD_SIZE for transcript hash, 1 byte for app cookie length, app
 * cookie length bytes, SHA256_DIGEST_LENGTH bytes for the HMAC of the whole
 * thing.
 */
 ...
 */
int
tls_parse_ctos_renegotiate (SSL_CONNECTION *s, PACKET *pkt,
 ...
 */
int
tls_parse_ctos_server_name (SSL_CONNECTION *s, PACKET *pkt,
                            unsigned int context, X509 *x, size_t chainidx)
{
...
int
tls_parse_ctos_maxfragmentlen (SSL_CONNECTION *s, PACKET *pkt,
                               unsigned int context, X509 *x, size_t chainidx)
{
  unsigned int value;
...
int
tls_parse_ctos_srp (SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
...
int
tls_parse_ctos_ec_pt_formats (SSL_CONNECTION *s, PACKET *pkt,
                              unsigned int context, X509 *x, size_t chainidx)
{
...
int
tls_parse_ctos_session_ticket (SSL_CONNECTION *s, PACKET *pkt,
                               unsigned int context, X509 *x, size_t chainidx)
{
  if (s->ext.session_ticket_cb
      && !s->ext.session_ticket_cb (
          SSL_CONNECTION_GET_USER_SSL (s), PACKET_data (pkt),
          (int)PACKET_remaining (pkt), s->ext.session_ticket_cb_arg))
    {
      SSLfatal (s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
      return 0;
    }
...
int
tls_parse_ctos_sig_algs_cert (SSL_CONNECTION *s, PACKET *pkt,
...
int
tls_parse_ctos_sig_algs (SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
                         X509 *x, size_t chainidx)
{
  PACKET supported_sig_algs;
...
int
tls_parse_ctos_status_request (SSL_CONNECTION *s, PACKET *pkt,
                               unsigned int context, X509 *x, size_t chainidx)
{
  ...
  if (PACKET_remaining (&exts) > 0)
    {
      ...
      s->ext.ocsp.exts = d2i_X509_EXTENSIONS (NULL, &ext_data,
...
int
tls_parse_ctos_npn (SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
                    X509 *x, size_t chainidx)
{
  /*
...
/*
 * Save the ALPN extension in a ClientHello.|pkt| holds the contents of the
 * ALPN extension, not including type and length. Returns: 1 on success, 0 on
 * error.
 */
int
tls_parse_ctos_alpn (SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
                     X509 *x, size_t chainidx)
{
...
int
tls_parse_ctos_use_srtp (SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
                         X509 *x, size_t chainidx)
{
...
int
tls_parse_ctos_etm (SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
                    X509 *x, size_t chainidx)
{
...
/*
 * Process a psk_kex_modes extension received in the ClientHello. |pkt|
 * contains the raw PACKET data for the extension. Returns 1 on success or 0 on
 * failure.
 */
int
tls_parse_ctos_psk_kex_modes (SSL_CONNECTION *s, PACKET *pkt,
                              unsigned int context, X509 *x, size_t chainidx)
{
 ...
 */
static int
tls_accept_ksgroup (SSL_CONNECTION *s, uint16_t ksgroup,
                   ...
                   size_t *keyshares_cnt, size_t *keyshares_max)
{
   ...
   */
  *keyshares_arr
      = OPENSSL_malloc_array (*keyshares_max, sizeof (**keyshares_arr));
  ...
  *encoded_pubkey_arr
      = OPENSSL_malloc_array (*keyshares_max, sizeof (**encoded_pubkey_arr));
      ...
      if (*keyshares_cnt == *keyshares_max)
        {
          ...
          uint16_t *tmp = OPENSSL_realloc_array (
              *keyshares_arr, *keyshares_max + GROUPLIST_INCREMENT,
              sizeof (**keyshares_arr));

          if (tmp == NULL)
            goto failure;
          *keyshares_arr = tmp;
          tmp_pkt = OPENSSL_realloc_array (
              *encoded_pubkey_arr, *keyshares_max + GROUPLIST_INCREMENT,
...
#ifndef OPENSSL_NO_TLS1_3
static void
check_overlap (SSL_CONNECTION *s, const uint16_t *prio_groups,
               size_t prio_num_groups, const uint16_t *candidate_groups,
...
int
tls_parse_ctos_key_share (SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
                          X509 *x, size_t chainidx)
{
#ifndef OPENSSL_NO_TLS1_3
  ...
  ks_extraction_result = extract_keyshares (
      s, &key_share_list, clntgroups, clnt_num_groups, srvrgroups,
      srvr_num_groups, &keyshares_arr, &encoded_pubkey_arr, &keyshares_cnt,
      &keyshares_max);
      ...
      if (s->options & SSL_OP_SERVER_PREFERENCE)
        {
          ...
          if (group_id_candidate > 0)
            { /* Overlap found -> accept the key share group */
...
int
tls_parse_ctos_cookie (SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
                       X509 *x, size_t chainidx)
{
#ifndef OPENSSL_NO_TLS1_3
...
int
tls_parse_ctos_supported_groups (SSL_CONNECTION *s, PACKET *pkt,
...
int
tls_parse_ctos_ems (SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
                    X509 *x, size_t chainidx)
{
...
int
tls_parse_ctos_early_data (SSL_CONNECTION *s, PACKET *pkt,
                           unsigned int context, X509 *x, size_t chainidx)
{
  if (PACKET_remaining (pkt) != 0)
    {
...
int
tls_parse_ctos_psk (SSL_CONNECTION *s, PACKET *pkt, unsigned int context,
                    X509 *x, size_t chainidx)
{
  ...
  for (id = 0; PACKET_remaining (&identities) != 0; id++)
    {
      ...
      if (!EVP_MD_is_a (md, EVP_MD_get0_name (ssl_md (
                                sctx, s->s3.tmp.new_cipher->algorithm2))))
        {
...
int
tls_parse_ctos_post_handshake_auth (SSL_CONNECTION *s, PACKET *pkt,
                                    ossl_unused unsigned int context,
                                    ossl_unused X509 *x,
 ...
 */
EXT_RETURN
tls_construct_stoc_renegotiate (SSL_CONNECTION *s, WPACKET *pkt,
                                unsigned int context, X509 *x, size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_server_name (SSL_CONNECTION *s, WPACKET *pkt,
                                unsigned int context, X509 *x, size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_maxfragmentlen (SSL_CONNECTION *s, WPACKET *pkt,
                                   unsigned int context, X509 *x,
                                   size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_ec_pt_formats (SSL_CONNECTION *s, WPACKET *pkt,
                                  unsigned int context, X509 *x,
                                  size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_supported_groups (SSL_CONNECTION *s, WPACKET *pkt,
                                     unsigned int context, X509 *x,
                                     size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_session_ticket (SSL_CONNECTION *s, WPACKET *pkt,
                                   unsigned int context, X509 *x,
                                   size_t chainidx)
{
...
#ifndef OPENSSL_NO_OCSP
EXT_RETURN
tls_construct_stoc_status_request (SSL_CONNECTION *s, WPACKET *pkt,
                                   unsigned int context, X509 *x,
                                   size_t chainidx)
{
...
#ifndef OPENSSL_NO_NEXTPROTONEG
EXT_RETURN
tls_construct_stoc_next_proto_neg (SSL_CONNECTION *s, WPACKET *pkt,
                                   unsigned int context, X509 *x,
                                   size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_alpn (SSL_CONNECTION *s, WPACKET *pkt, unsigned int context,
                         X509 *x, size_t chainidx)
{
...
#ifndef OPENSSL_NO_SRTP
EXT_RETURN
tls_construct_stoc_use_srtp (SSL_CONNECTION *s, WPACKET *pkt,
                             unsigned int context, X509 *x, size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_etm (SSL_CONNECTION *s, WPACKET *pkt, unsigned int context,
                        X509 *x, size_t chainidx)
{
  if (!s->ext.use_etm)
    return EXT_RETURN_NOT_SENT;
...
EXT_RETURN
tls_construct_stoc_ems (SSL_CONNECTION *s, WPACKET *pkt, unsigned int context,
                        X509 *x, size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_supported_versions (SSL_CONNECTION *s, WPACKET *pkt,
                                       unsigned int context, X509 *x,
                                       size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_key_share (SSL_CONNECTION *s, WPACKET *pkt,
                              unsigned int context, X509 *x, size_t chainidx)
{
#ifndef OPENSSL_NO_TLS1_3
  ...
  if ((ginf
       = tls1_group_id_lookup (SSL_CONNECTION_GET_CTX (s), s->s3.group_id))
      == NULL)
    {
      SSLfatal (s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
      return EXT_RETURN_FAIL;
...
EXT_RETURN
tls_construct_stoc_cookie (SSL_CONNECTION *s, WPACKET *pkt,
                           unsigned int context, X509 *x, size_t chainidx)
{
#ifndef OPENSSL_NO_TLS1_3
...
EXT_RETURN
tls_construct_stoc_cryptopro_bug (SSL_CONNECTION *s, WPACKET *pkt,
                                  unsigned int context, X509 *x,
                                  size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_early_data (SSL_CONNECTION *s, WPACKET *pkt,
                               unsigned int context, X509 *x, size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_psk (SSL_CONNECTION *s, WPACKET *pkt, unsigned int context,
                        X509 *x, size_t chainidx)
{
  if (!s->hit)
    return EXT_RETURN_NOT_SENT;
...
EXT_RETURN
tls_construct_stoc_client_cert_type (SSL_CONNECTION *sc, WPACKET *pkt,
                                     unsigned int context, X509 *x,
                                     size_t chainidx)
{
...
static int
reconcile_cert_type (const unsigned char *pref, size_t pref_len,
...
int
tls_parse_ctos_client_cert_type (SSL_CONNECTION *sc, PACKET *pkt,
                                 unsigned int context, X509 *x,
                                 size_t chainidx)
{
...
EXT_RETURN
tls_construct_stoc_server_cert_type (SSL_CONNECTION *sc, WPACKET *pkt,
                                     unsigned int context, X509 *x,
                                     size_t chainidx)
{
...
int
tls_parse_ctos_server_cert_type (SSL_CONNECTION *sc, PACKET *pkt,
                                 unsigned int context, X509 *x,
                                 size_t chainidx)
{
  PACKET supported_cert_types;
