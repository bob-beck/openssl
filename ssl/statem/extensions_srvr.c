                           ...
                           size_t chainidx)
{
  ...
  ok = memcmp(
    data, s->s3.previous_client_finished, s->s3.previous_client_finished_len);
                              ...
                              size_t chainidx)
{
  ...
  if (!IS_MAX_FRAGMENT_LENGTH_EXT_VALID(value)) {
    SSLfatal(
      s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_SSL3_EXT_INVALID_MAX_FRAGMENT_LENGTH);
    return 0;
  }
                    ...
                    size_t chainidx)
{
  ...
  if (!PACKET_memdup(
        &save_protocol_list, &s->s3.alpn_proposed, &s->s3.alpn_proposed_len)) {
    SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
    return 0;
  }
 ...
 */
static int
tls_accept_ksgroup(SSL_CONNECTION* s, uint16_t ksgroup, PACKET* encoded_pubkey)
{
...
extract_keyshares(SSL_CONNECTION* s,
                  ...
                  size_t* keyshares_max)
{
  ...
  *encoded_pubkey_arr =
  ...
  while (PACKET_remaining(key_share_list) > 0) {
     ...
     */
    if (!check_in_list(
          s, group_id, clntgroups, clnt_num_groups, 0, &key_share_pos)) {
      SSLfatal(s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_BAD_KEY_SHARE);
      goto failure;
    }
    ...
    if (key_share_pos < previous_key_share_pos)
      OSSL_TRACE1(
        TLS, "key share group id %d is out of RFC 8446 order\n", group_id);
    ...
    if (!check_in_list(s, group_id, srvrgroups, srvr_num_groups, 1, NULL) ||
        ...
        !tls_valid_group(
          s, group_id, TLS1_3_VERSION, TLS1_3_VERSION, 0, NULL)) {
...
#ifndef OPENSSL_NO_TLS1_3
static void
check_overlap(SSL_CONNECTION* s,
  ...
  *selected_group = 0;
  ...
  for (current_group = 0; current_group < candidate_num_groups;
    ...
    if (!check_in_list(s,
                       ...
                       &new_group_idx) ||
        !tls_group_allowed(
          s, candidate_groups[current_group], SSL_SECOP_CURVE_SUPPORTED) ||
        !tls_valid_group(s,
                         candidate_groups[current_group],
                         TLS1_3_VERSION,
                         TLS1_3_VERSION,
                         ...
                         size_t chainidx)
{
  ...
  if (clnt_num_groups == 0) {
     ...
     */
    SSLfatal(
      s, SSL_AD_MISSING_EXTENSION, SSL_R_MISSING_SUPPORTED_GROUPS_EXTENSION);
    return 0;
  }
    ...
    if (s->options & SSL_OP_SERVER_PREFERENCE) {
      ...
      if (group_id_candidate >
        ...
        if (!tls_accept_ksgroup(
              s, group_id_candidate, &encoded_pubkey_arr[candidate_group_idx]))
      ...
      if (group_id_candidate > 0) {
        if (!tls_accept_ksgroup(
              s, group_id_candidate, &encoded_pubkey_arr[prio_group_idx]))
          goto err;
        goto end;
      } else {
        check_overlap(s,
                      ...
                      size_t chainidx)
{
  ...
  if (EVP_DigestSignInit_ex(
        hctx, NULL, "SHA2-256", sctx->libctx, sctx->propq, pkey, NULL) <= 0 ||
      EVP_DigestSign(
        hctx, hmac, &hmaclen, data, rawlen - SHA256_DIGEST_LENGTH) <= 0 ||
      hmaclen != SHA256_DIGEST_LENGTH) {
    EVP_MD_CTX_free(hctx);
    EVP_PKEY_free(pkey);
    SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
  ...
  if (!WPACKET_put_bytes_u8(&hrrpkt, SSL3_MT_SERVER_HELLO) ||
      ...
      !WPACKET_sub_memcpy_u8(
        &hrrpkt, s->tmp_session_id, s->tmp_session_id_len) ||
      !ssl->method->put_cipher_by_char(
        s->s3.tmp.new_cipher, &hrrpkt, &ciphlen) ||
                   ...
                   size_t chainidx)
{
  ...
  for (id = 0; PACKET_remaining(&identities) != 0; id++) {
    ...
    if (!EVP_MD_is_a(
          md,
          EVP_MD_get0_name(ssl_md(sctx, s->s3.tmp.new_cipher->algorithm2)))) {
...
int
tls_parse_ctos_post_handshake_auth(SSL_CONNECTION* s,
                                   PACKET* pkt,
                                   ossl_unused unsigned int context,
                                   ossl_unused X509* x,
                                   ossl_unused size_t chainidx)
                                  ...
                                  size_t chainidx)
{
  ...
  ret = sctx->ext.npn_advertised_cb(SSL_CONNECTION_GET_USER_SSL(s),
                                    &npa,
                                    &npalen,
                        ...
                        size_t chainidx)
{
  ...
  if (!WPACKET_put_bytes_u16(
      ...
      !WPACKET_start_sub_packet_u16(pkt) ||
      !WPACKET_sub_memcpy_u8(
        pkt, s->s3.alpn_selected, s->s3.alpn_selected_len) ||
      !WPACKET_close(pkt) || !WPACKET_close(pkt)) {
    SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
    return EXT_RETURN_FAIL;
  }
...
static int
reconcile_cert_type(const unsigned char* pref,
                    size_t pref_len,
                                ...
                                size_t chainidx)
{
  ...
  sc->ext.server_cert_type_ctos =
    reconcile_cert_type(sc->server_cert_type,
