...
int tls_parse_ctos_npn(SSL_CONNECTION* s, PACKET* pkt, unsigned int context,
   ...
   * renegotiation.
   */
  if (SSL_IS_FIRST_HANDSHAKE(s)) s->s3.npn_seen = 1;
...
int tls_parse_ctos_alpn(SSL_CONNECTION* s, PACKET* pkt, unsigned int context,
  ...
  if (!SSL_IS_FIRST_HANDSHAKE(s)) return 1;
...
int tls_parse_ctos_use_srtp(SSL_CONNECTION* s, PACKET* pkt,
  ...
  if (SSL_get_srtp_profiles(ssl) == NULL) return 1;
...
int tls_parse_ctos_etm(SSL_CONNECTION* s, PACKET* pkt, unsigned int context,
                       X509* x, size_t chainidx) {
  if (!(s->options & SSL_OP_NO_ENCRYPT_THEN_MAC)) s->ext.use_etm = 1;
...
static KS_EXTRACTION_RESULT extract_keyshares(
    SSL_CONNECTION* s, PACKET* key_share_list, const uint16_t* clntgroups,
    size_t clnt_num_groups, const uint16_t* srvrgroups, size_t srvr_num_groups,
...
int tls_parse_ctos_cookie(SSL_CONNECTION* s, PACKET* pkt, unsigned int context,
  ...
  if (format != COOKIE_STATE_FORMAT_VERSION) return 1;
...
int tls_parse_ctos_ems(SSL_CONNECTION* s, PACKET* pkt, unsigned int context,
  ...
  if (s->options & SSL_OP_NO_EXTENDED_MASTER_SECRET) return 1;
...
static SSL_TICKET_STATUS tls_get_stateful_ticket(SSL_CONNECTION* s,
  ...
  if (tmpsess == NULL) return SSL_TICKET_NO_DECRYPT;
...
int tls_parse_ctos_psk(SSL_CONNECTION* s, PACKET* pkt, unsigned int context,
  ...
  for (id = 0; PACKET_remaining(&identities) != 0; id++) {
    ...
    if (sess != NULL) {
      ...
      if (ret == SSL_TICKET_NONE || ret == SSL_TICKET_NO_DECRYPT) continue;
...
EXT_RETURN tls_construct_stoc_renegotiate(SSL_CONNECTION* s, WPACKET* pkt,
                                          unsigned int context, X509* x,
                                          size_t chainidx) {
  if (!s->s3.send_connection_binding) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_server_name(SSL_CONNECTION* s, WPACKET* pkt,
                                          unsigned int context, X509* x,
                                          size_t chainidx) {
  if (s->servername_done != 1) return EXT_RETURN_NOT_SENT;

  /*
   * Prior to TLSv1.3 we ignore any SNI in the current handshake if resuming.
   * We just use the servername from the initial handshake.
   */
  if (s->hit && !SSL_CONNECTION_IS_TLS13(s)) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_maxfragmentlen(SSL_CONNECTION* s, WPACKET* pkt,
                                             unsigned int context, X509* x,
                                             size_t chainidx) {
  if (!USE_MAX_FRAGMENT_LENGTH_EXT(s->session)) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_ec_pt_formats(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (!using_ecc) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_supported_groups(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (s->s3.group_id == 0) return EXT_RETURN_NOT_SENT;
  ...
  for (i = 0; i < numgroups; i++) {
    ...
    if (tls_valid_group(s, group, version, version, 0, NULL) &&
      ...
      if (first) {
         ...
         */
        if (s->s3.group_id == group) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_status_request(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (context == SSL_EXT_TLS1_3_CERTIFICATE_REQUEST) return EXT_RETURN_NOT_SENT;

  if (!s->ext.status_expected) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_alpn(SSL_CONNECTION* s, WPACKET* pkt,
                                   unsigned int context, X509* x,
                                   size_t chainidx) {
  if (s->s3.alpn_selected == NULL) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_use_srtp(SSL_CONNECTION* s, WPACKET* pkt,
                                       unsigned int context, X509* x,
                                       size_t chainidx) {
  if (s->srtp_profile == NULL) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_etm(SSL_CONNECTION* s, WPACKET* pkt,
                                  unsigned int context, X509* x,
                                  size_t chainidx) {
  if (!s->ext.use_etm) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_cookie(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if ((s->s3.flags & TLS1_FLAGS_STATELESS) == 0) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_early_data(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (context == SSL_EXT_TLS1_3_NEW_SESSION_TICKET) {
    if (s->max_early_data == 0) return EXT_RETURN_NOT_SENT;
  ...
  if (s->ext.early_data != SSL_EARLY_DATA_ACCEPTED) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_stoc_psk(SSL_CONNECTION* s, WPACKET* pkt,
                                  unsigned int context, X509* x,
                                  size_t chainidx) {
  if (!s->hit) return EXT_RETURN_NOT_SENT;
...
int tls_parse_ctos_server_cert_type(SSL_CONNECTION* sc, PACKET* pkt,
  ...
  if (sc->ext.server_cert_type_ctos == OSSL_CERT_TYPE_CTOS_GOOD) return 1;
