...
static int
init_sig_algs_cert(SSL_CONNECTION* s, unsigned int context);
static int
...
static EXT_RETURN
tls_construct_certificate_authorities(SSL_CONNECTION* s,
                                      WPACKET* pkt,
                                      unsigned int context,
                                      X509* x,
                                      size_t chainidx);
static int
tls_parse_certificate_authorities(SSL_CONNECTION* s,
                                  PACKET* pkt,
                                  unsigned int context,
                                  X509* x,
                                  size_t chainidx);
...
static int
init_post_handshake_auth(SSL_CONNECTION* s, unsigned int context);
static int
...
static EXT_RETURN
tls_construct_compress_certificate(SSL_CONNECTION* sc,
                                   WPACKET* pkt,
                                   unsigned int context,
                                   X509* x,
                                   size_t chainidx);
static int
tls_parse_compress_certificate(SSL_CONNECTION* sc,
                               PACKET* pkt,
                               unsigned int context,
                               X509* x,
                               size_t chainidx);
...
typedef struct extensions_definition_st
{
  ...
  int (*parse_ctos)(SSL_CONNECTION* s,
                    PACKET* pkt,
                    unsigned int context,
                    X509* x,
                    size_t chainidx);
  /* Parse extension send from server to client */
  int (*parse_stoc)(SSL_CONNECTION* s,
                    PACKET* pkt,
                    unsigned int context,
                    X509* x,
                    size_t chainidx);
  /* Construct extension sent from server to client */
  EXT_RETURN (*construct_stoc)(SSL_CONNECTION* s,
                               WPACKET* pkt,
                               unsigned int context,
                               X509* x,
                               size_t chainidx);
  /* Construct extension sent from client to server */
  EXT_RETURN (*construct_ctos)(SSL_CONNECTION* s,
                               WPACKET* pkt,
                               unsigned int context,
                               X509* x,
                               size_t chainidx);
 ...
 */
static int
verify_extension(SSL_CONNECTION* s,
                 unsigned int context,
                 unsigned int type,
                         ...
                         size_t chainidx)
{
  ...
  for (i = 0, thisexd = ext_defs; i < OSSL_NELEM(ext_defs); i++, thisexd++) {
    EXT_RETURN (*construct)(SSL_CONNECTION* s,
                            WPACKET* pkt,
                            unsigned int context,
                            X509* x,
                            size_t chainidx);
...
final_renegotiate(SSL_CONNECTION* s, unsigned int context, int sent)
{
  ...
  if (s->renegotiate &&
      !(s->options & SSL_OP_ALLOW_UNSAFE_LEGACY_RENEGOTIATION) && !sent) {
    SSLfatal(
      s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_UNSAFE_LEGACY_RENEGOTIATION_DISABLED);
    return 0;
  }
...
final_ec_pt_formats(SSL_CONNECTION* s, unsigned int context, int sent)
{
  ...
  if (s->ext.ecpointformats != NULL && s->ext.ecpointformats_len > 0 &&
    ...
    if (i == s->ext.peer_ecpointformats_len) {
      SSLfatal(
        s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_TLS_INVALID_ECPOINTFORMAT_LIST);
      return 0;
    }
  }
                  ...
                  int external)
{
  ...
  if (!tls13_generate_secret(
        s, md, NULL, sess->master_key, sess->master_key_length, early_secret)) {
    /* SSLfatal() already called */
    goto err;
  }
...
final_psk(SSL_CONNECTION* s, unsigned int context, int sent)
{
  if (s->server && sent && s->clienthello != NULL &&
    ...
    SSLfatal(
      s, TLS13_AD_MISSING_EXTENSION, SSL_R_MISSING_PSK_KEX_MODES_EXTENSION);
    return 0;
  }
