...
unsigned int ossl_get_extension_type(size_t idx) {
  ...
  if (idx >= num_exts) return TLSEXT_TYPE_out_of_range;
...
static int validate_context(SSL_CONNECTION* s, unsigned int extctx,
  ...
  if ((thisctx & extctx) == 0) return 0;

  if (SSL_CONNECTION_IS_DTLS(s)) {
    if ((extctx & SSL_EXT_TLS_ONLY) != 0) return 0;
...
int tls_validate_all_contexts(SSL_CONNECTION* s, unsigned int thisctx,
  ...
  for (thisext = exts, i = 0; i < num_exts; i++, thisext++) {
    ...
    if (i < builtin_num) {
      ...
      if (!ossl_assert(meth != NULL)) return 0;
      context = meth->context;
    }

    if (!validate_context(s, context, thisctx)) return 0;
  }
...
static int verify_extension(SSL_CONNECTION* s, unsigned int context,
  ...
  for (i = 0, thisext = ext_defs; i < builtin_num; i++, thisext++) {
    if (type == thisext->type) {
      if (!validate_context(s, thisext->context, context)) return 0;
  ...
  if (meths != NULL) {
    ...
    if (meth != NULL) {
      if (!validate_context(s, meth->context, context)) return 0;
...
int tls_collect_extensions(SSL_CONNECTION* s, PACKET* packet,
  ...
  *res = NULL;
   ...
   */
  if ((context & SSL_EXT_CLIENT_HELLO) != 0) custom_ext_init(&s->cert->custext);
    ...
    if (thisex != NULL) {
      ...
      if (s->ext.debug_cb)
        s->ext.debug_cb(SSL_CONNECTION_GET_USER_SSL(s), !s->server,
                        ...
                        (int)PACKET_remaining(&thisex->data), s->ext.debug_arg);
    }
  }
...
int tls_parse_extension(SSL_CONNECTION* s, TLSEXT_INDEX idx, int context,
  ...
  if (!currext->present) return 1;

  /* Skip if we've already parsed this extension */
  if (currext->parsed) return 1;
  ...
  if (idx < OSSL_NELEM(ext_defs)) {
    ...
    if (!extension_is_relevant(s, extdef->context, context)) return 1;

    parser = s->server ? extdef->parse_ctos : extdef->parse_stoc;

    if (parser != NULL) return parser(s, &currext->data, context, x, chainidx);
...
int tls_construct_extensions(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if ((context & SSL_EXT_CLIENT_HELLO) != 0) {
    ...
    if (reason != 0) {
      if (!for_comp) SSLfatal(s, SSL_AD_INTERNAL_ERROR, reason);
      return 0;
    }
  }
...
static int final_ec_pt_formats(SSL_CONNECTION* s, unsigned int context,
  ...
  if (s->ext.ecpointformats != NULL && s->ext.ecpointformats_len > 0 &&
    ...
    for (i = 0; i < s->ext.peer_ecpointformats_len; i++) {
      if (*list++ == TLSEXT_ECPOINTFORMAT_uncompressed) break;
    }
...
static int init_session_ticket(SSL_CONNECTION* s, unsigned int context) {
  if (!s->server) s->ext.ticket_expected = 0;
...
static int final_alpn(SSL_CONNECTION* s, unsigned int context, int sent) {
  ...
  if (!s->server || !SSL_CONNECTION_IS_TLS13(s)) return 1;
...
static EXT_RETURN tls_construct_certificate_authorities(SSL_CONNECTION* s,
  ...
  if (ca_sk == NULL || sk_X509_NAME_num(ca_sk) == 0) return EXT_RETURN_NOT_SENT;
...
static int tls_parse_certificate_authorities(SSL_CONNECTION* s, PACKET* pkt,
                                             unsigned int context, X509* x,
                                             size_t chainidx) {
  if (!parse_ca_names(s, pkt)) return 0;
...
#ifndef OPENSSL_NO_SRTP
static int init_srtp(SSL_CONNECTION* s, unsigned int context) {
  if (s->server) s->srtp_profile = NULL;
...
static int final_key_share(SSL_CONNECTION* s, unsigned int context, int sent) {
  ...
  if (!SSL_CONNECTION_IS_TLS13(s)) return 1;

  /* Nothing to do for key_share in an HRR */
  if ((context & SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST) != 0) return 1;
...
int tls_psk_do_binder(SSL_CONNECTION* s, const EVP_MD* md,
  ...
  if (sign) {
    ...
    if (!ret) SSLfatal(s, SSL_AD_DECRYPT_ERROR, SSL_R_BINDER_DOES_NOT_VERIFY);
  }
...
static EXT_RETURN tls_construct_compress_certificate(SSL_CONNECTION* sc,
  ...
  if (!ossl_comp_has_alg(0)) return EXT_RETURN_NOT_SENT;
  ...
  for (i = 0; sc->cert_comp_prefs[i] != TLSEXT_comp_cert_none; i++) {
    if (!WPACKET_put_bytes_u16(pkt, sc->cert_comp_prefs[i])) goto err;
  }
  if (!WPACKET_close(pkt) || !WPACKET_close(pkt)) goto err;
...
static int tls_comp_in_pref(SSL_CONNECTION* sc, int alg) {
  ...
  if (sc->cert_comp_prefs[0] == TLSEXT_comp_cert_none) return 1;
  /* Find the algorithm */
  for (i = 0; i < TLSEXT_comp_cert_limit; i++)
    if (sc->cert_comp_prefs[i] == alg) return 1;
  return 0;
}
#endif
