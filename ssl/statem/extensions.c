...
static int final_supported_versions (SSL_CONNECTION *s, unsigned int context,
                                     int sent);
static int final_early_data (SSL_CONNECTION *s, unsigned int context,
                             int sent);
...
static int
validate_context (SSL_CONNECTION *s, unsigned int extctx, unsigned int thisctx)
{
...
int
tls_validate_all_contexts (SSL_CONNECTION *s, unsigned int thisctx,
                           RAW_EXTENSION *exts)
{
 ...
 * indicate the extension is not allowed. If returning 1 then |*found| is set
 * to the definition for the extension we found.
 */
static int
verify_extension (SSL_CONNECTION *s, unsigned int context, unsigned int type,
 ...
 */
int
extension_is_relevant (SSL_CONNECTION *s, unsigned int extctx,
 ...
 * This function returns 1 if all extensions are unique and we have parsed
 * their types, and 0 if the extensions contain duplicates, could not be
 * successfully found, or an internal error occurred. We only check duplicates
 * for extensions that we know about. We ignore others.
 */
int
tls_collect_extensions (SSL_CONNECTION *s, PACKET *packet,
                        unsigned int context, RAW_EXTENSION **res, size_t *len,
                        int init)
{
 ...
 */
int
tls_parse_extension (SSL_CONNECTION *s, TLSEXT_INDEX idx, int context,
 ...
 */
int
tls_parse_all_extensions (SSL_CONNECTION *s, int context, RAW_EXTENSION *exts,
...
int
should_add_extension (SSL_CONNECTION *s, unsigned int extctx,
 ...
 * |chainidx| will indicate the position in the chainidx we are processing
 * (with 0 being the first in the chain). Returns 1 on success or 0 on failure.
 * On a failure construction stops at the first extension to fail to construct.
 */
int
tls_construct_extensions (SSL_CONNECTION *s, WPACKET *pkt,
...
static int
final_renegotiate (SSL_CONNECTION *s, unsigned int context, int sent)
{
  if (!s->server)
    {
      /*
...
ssl_tsan_decr (const SSL_CTX *ctx, TSAN_QUALIFIER int *stat)
{
...
static int
init_server_name (SSL_CONNECTION *s, unsigned int context)
{
  if (s->server)
    {
      s->servername_done = 0;
...
static int
final_server_name (SSL_CONNECTION *s, unsigned int context, int sent)
{
...
static int
final_ec_pt_formats (SSL_CONNECTION *s, unsigned int context, int sent)
{
...
static int
init_session_ticket (SSL_CONNECTION *s, unsigned int context)
{
  if (!s->server)
    s->ext.ticket_expected = 0;
...
#ifndef OPENSSL_NO_OCSP
static int
init_status_request (SSL_CONNECTION *s, unsigned int context)
{
  if (s->server)
    {
...
#ifndef OPENSSL_NO_NEXTPROTONEG
static int
init_npn (SSL_CONNECTION *s, unsigned int context)
{
  s->s3.npn_seen = 0;
...
static int
init_alpn (SSL_CONNECTION *s, unsigned int context)
{
...
static int
final_alpn (SSL_CONNECTION *s, unsigned int context, int sent)
{
...
static int
init_sig_algs (SSL_CONNECTION *s, unsigned int context)
{
...
static int
init_sig_algs_cert (SSL_CONNECTION *s, ossl_unused unsigned int context)
{
  /* Clear any signature algorithms extension received */
...
#ifndef OPENSSL_NO_SRP
static int
init_srp (SSL_CONNECTION *s, unsigned int context)
{
...
static int
init_ec_point_formats (SSL_CONNECTION *s, unsigned int context)
{
...
static int
init_etm (SSL_CONNECTION *s, unsigned int context)
{
  s->ext.use_etm = 0;
...
static int
init_ems (SSL_CONNECTION *s, unsigned int context)
{
...
static int
final_ems (SSL_CONNECTION *s, unsigned int context, int sent)
{
  /*
...
static int
init_certificate_authorities (SSL_CONNECTION *s, unsigned int context)
{
...
static EXT_RETURN
tls_construct_certificate_authorities (SSL_CONNECTION *s, WPACKET *pkt,
                                       unsigned int context, X509 *x,
                                       size_t chainidx)
{
...
static int
tls_parse_certificate_authorities (SSL_CONNECTION *s, PACKET *pkt,
                                   unsigned int context, X509 *x,
                                   size_t chainidx)
{
...
#ifndef OPENSSL_NO_SRTP
static int
init_srtp (SSL_CONNECTION *s, unsigned int context)
{
  if (s->server)
    s->srtp_profile = NULL;
...
static int
final_sig_algs (SSL_CONNECTION *s, unsigned int context, int sent)
{
...
static int
final_supported_versions (SSL_CONNECTION *s, unsigned int context, int sent)
{
...
static int
final_key_share (SSL_CONNECTION *s, unsigned int context, int sent)
{
...
static int
init_psk_kex_modes (SSL_CONNECTION *s, unsigned int context)
{
...
int
tls_psk_do_binder (SSL_CONNECTION *s, const EVP_MD *md,
...
static int
final_early_data (SSL_CONNECTION *s, unsigned int context, int sent)
{
  if (!sent)
    return 1;
...
static int
final_maxfragmentlen (SSL_CONNECTION *s, unsigned int context, int sent)
{
  if (s->session == NULL)
    return 1;
...
static int
init_post_handshake_auth (SSL_CONNECTION *s, ossl_unused unsigned int context)
{
 ...
 */
static int
final_psk (SSL_CONNECTION *s, unsigned int context, int sent)
{
...
static int
tls_init_compress_certificate (SSL_CONNECTION *sc, unsigned int context)
{
 ...
 * zlib, zstd] */
static EXT_RETURN
tls_construct_compress_certificate (SSL_CONNECTION *sc, WPACKET *pkt,
                                    unsigned int context, X509 *x,
                                    size_t chainidx)
{
...
#ifndef OPENSSL_NO_COMP_ALG
static int
tls_comp_in_pref (SSL_CONNECTION *sc, int alg)
{
  int i;
...
int
tls_parse_compress_certificate (SSL_CONNECTION *sc, PACKET *pkt,
                                unsigned int context, X509 *x, size_t chainidx)
{
#ifndef OPENSSL_NO_COMP_ALG
...
static int
init_server_cert_type (SSL_CONNECTION *sc, unsigned int context)
{
...
static int
init_client_cert_type (SSL_CONNECTION *sc, unsigned int context)
{
  /* Only reset when parsing client hello */
  if (sc->server)
    {
