...
static int custom_ext_add_old_cb_wrap(SSL* s, unsigned int ext_type,
  ...
  if (add_cb_wrap->add_cb == NULL) return 1;
...
static void custom_ext_free_old_cb_wrap(SSL* s, unsigned int ext_type,
  ...
  if (add_cb_wrap->free_cb == NULL) return;
...
static int custom_ext_parse_old_cb_wrap(SSL* s, unsigned int ext_type,
  ...
  if (parse_cb_wrap->parse_cb == NULL) return 1;
...
int custom_ext_parse(SSL_CONNECTION* s, unsigned int context,
  ...
  if (!extension_is_relevant(s, meth->context, context)) return 1;
  ...
  if (meth->parse_cb == NULL) return 1;
...
int custom_ext_add(SSL_CONNECTION* s, int context, WPACKET* pkt, X509* x,
  ...
  for (i = 0; i < exts->meths_count; i++) {
    ...
    if (!should_add_extension(s, meth->context, context, maxversion)) continue;
    ...
    if ((context & (SSL_EXT_TLS1_2_SERVER_HELLO | SSL_EXT_TLS1_3_SERVER_HELLO |
      ...
      if (!(meth->ext_flags & SSL_EXT_FLAG_RECEIVED)) continue;
    }
    /*
     * We skip it if the callback is absent - except for a ClientHello where
     * we add an empty extension.
     */
    if ((context & SSL_EXT_CLIENT_HELLO) == 0 && meth->add_cb == NULL) continue;
    ...
    if (meth->add_cb != NULL) {
      ...
      if (cb_retval < 0) {
        if (!for_comp) SSLfatal(s, al, SSL_R_CALLBACK_FAILED);
        return 0; /* error */
      }
      if (cb_retval == 0) continue; /* skip this extension */
    }
...
static void custom_ext_copy_old_cb(custom_ext_method* methdst,
  ...
  if (methsrc->add_cb != custom_ext_add_old_cb_wrap) return;
  ...
  if (methdst->add_arg == NULL || methdst->parse_arg == NULL) *err = 1;
...
int custom_exts_copy_conn(custom_ext_methods* dst,
  ...
  if (src->meths_count > 0) {
    ...
    for (i = 0; i < src->meths_count; i++)
      if ((src->meths[i].ext_flags & SSL_EXT_FLAG_CONN) != 0) meths_count++;

    if (meths_count > 0) {
      custom_ext_method* methdst =
          OPENSSL_realloc(dst->meths, (dst->meths_count + meths_count) *
                                          sizeof(custom_ext_method));

      if (methdst == NULL) return 0;
      ...
      for (i = 0; i < src->meths_count; i++) {
        ...
        if ((methsrc->ext_flags & SSL_EXT_FLAG_CONN) == 0) continue;
...
void custom_exts_free(custom_ext_methods* exts) {
  ...
  for (i = 0, meth = exts->meths; i < exts->meths_count; i++, meth++) {
    if (meth->add_cb != custom_ext_add_old_cb_wrap) continue;
...
int ossl_tls_add_custom_ext_intern(
   ...
   * called.
   */
  if (add_cb == NULL && free_cb != NULL) return 0;

  if (exts == NULL) exts = &ctx->cert->custext;
  ...
  if (ext_type > 0xffff) return 0;
  /* Search for duplicate */
  if (custom_ext_find(exts, role, ext_type, NULL)) return 0;
