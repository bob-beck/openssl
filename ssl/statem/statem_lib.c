...
MSG_PROCESS_RETURN tls_process_cert_verify(SSL_CONNECTION *s, PACKET *pkt)
{
     ...
     */
#ifndef OPENSSL_NO_GOST
    if (!SSL_USE_SIGALGS(s) && ((PACKET_remaining(pkt) == 64 && (EVP_PKEY_get_id(pkey) == NID_id_GostR3410_2001 ||
...
MSG_PROCESS_RETURN tls_process_finished(SSL_CONNECTION *s, PACKET *pkt)
{
    ...
    if (SSL_CONNECTION_IS_TLS13(s))
    {
        ...
        {
            if (s->post_handshake_auth != SSL_PHA_REQUESTED &&
                !ssl->method->ssl3_enc->change_cipher_state(s, SSL3_CC_APPLICATION | SSL3_CHANGE_CIPHER_SERVER_READ))
            {
                /* SSLfatal() already called */
                return MSG_PROCESS_ERROR;
            }
        ...
        else
        {
             ...
             */
            if (!SSL_IS_QUIC_HANDSHAKE(s) &&
                !ssl->method->ssl3_enc->change_cipher_state(s, SSL3_CC_APPLICATION | SSL3_CHANGE_CIPHER_CLIENT_READ))
            {
                /* SSLfatal() already called */
                return MSG_PROCESS_ERROR;
            }
...
int ssl_version_supported(const SSL_CONNECTION *s, int version, const SSL_METHOD **meth)
{
    ...
    for (vent = table; vent->version != 0 && ssl_version_cmp(s, version, vent->version) <= 0; ++vent)
    {
        ...
        if (thismeth != NULL && ssl_version_cmp(s, version, vent->version) == 0 &&
            ssl_method_error(s, thismeth()) == 0 && (!s->server || version != TLS1_3_VERSION || is_tls13_capable(s)))
        {
            if (meth != NULL)
                *meth = thismeth();
            return 1;
                                  ...
                                  const unsigned char *hrr, size_t hrrlen)
{
    ...
    if (hrr != NULL &&
        (!ssl3_finish_mac(s, hrr, hrrlen) ||
         !ssl3_finish_mac(s, (unsigned char *)s->init_buf->data, s->s3.tmp.message_size + SSL3_HM_HEADER_LENGTH)))
    {
        /* SSLfatal() already called */
        return 0;
    }
    ...
    if ((ca_sk != NULL) && !(s->options & SSL_OP_DISABLE_TLSEXT_CA_NAMES))
    {
        ...
        for (i = 0; i < sk_X509_NAME_num(ca_sk); i++)
        {
            ...
            if (name == NULL || (namelen = i2d_X509_NAME(name, NULL)) < 0 ||
                !WPACKET_sub_allocate_bytes_u16(pkt, namelen, &namebytes) || i2d_X509_NAME(name, &namebytes) != namelen)
            {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
                return 0;
            }
...
MSG_PROCESS_RETURN tls13_process_compressed_certificate(SSL_CONNECTION *sc, PACKET *pkt, PACKET *tmppkt, BUF_MEM *buf)
{
    ...
    if (!BUF_MEM_grow(buf, expected_length) || !PACKET_buf_init(tmppkt, (unsigned char *)buf->data, expected_length) ||
        COMP_expand_block(comp, (unsigned char *)buf->data, (int)expected_length, (unsigned char *)PACKET_data(pkt),
