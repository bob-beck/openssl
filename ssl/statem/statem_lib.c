...
ssl3_do_write(SSL_CONNECTION* s, uint8_t type)
{
  ...
  ret = ssl3_write_bytes(
    ssl, type, &s->init_buf->data[s->init_off], s->init_num, &written);
  if (ret <= 0)
    return -1;
...
static int
get_cert_verify_tbs_data(SSL_CONNECTION* s,
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    ...
    if (s->statem.hand_state == TLS_ST_CR_CERT_VRFY ||
      ...
      hashlen = s->cert_verify_hash_len;
                                   ...
                                   tls13tbs + TLS13_TBS_PREAMBLE_SIZE,
                                   EVP_MAX_MD_SIZE,
                                   &hashlen)) {
      /* SSLfatal() already called */
      return 0;
...
tls_process_cert_verify(SSL_CONNECTION* s, PACKET* pkt)
{
  ...
  if (ssl_cert_lookup_by_pkey(pkey, NULL, sctx) == NULL) {
    SSLfatal(
      s, SSL_AD_ILLEGAL_PARAMETER, SSL_R_SIGNATURE_FOR_NON_SIGNING_CERTIFICATE);
    goto err;
  }
  ...
  if (SSL_USE_SIGALGS(s)) {
    ...
    SSLfatal(
      s, SSL_AD_INTERNAL_ERROR, SSL_R_LEGACY_SIGALG_DISALLOWED_OR_UNSUPPORTED);
    goto err;
  }
  ...
  if (EVP_DigestVerifyInit_ex(mctx,
                              &pctx,
                              md == NULL ? NULL : EVP_MD_get0_name(md),
                              sctx->libctx,
                              sctx->propq,
...
static int
ssl_add_cert_to_wpacket(SSL_CONNECTION* s,
                        WPACKET* pkt,
                        X509* x,
                        int chain,
                        int for_comp)
...
static int
ssl_add_cert_chain(SSL_CONNECTION* s,
                   WPACKET* pkt,
                   CERT_PKEY* cpk,
                   int for_comp)
{
...
tls_process_rpk(SSL_CONNECTION* sc, PACKET* pkt, EVP_PKEY** peer_rpk)
{
  ...
  spkistart = spki;
  if ((pkey = d2i_PUBKEY_ex(
         NULL, &spki, spki_len, sctx->libctx, sctx->propq)) == NULL ||
  ...
  if (EVP_PKEY_missing_parameters(pkey)) {
    SSLfatal(
      sc, SSL_AD_INTERNAL_ERROR, SSL_R_UNABLE_TO_FIND_PUBLIC_KEY_PARAMETERS);
    goto err;
  }
  ...
  if (SSL_CONNECTION_IS_TLS13(sc)) {
    ...
    if (!tls_collect_extensions(
          sc, &extensions, SSL_EXT_TLS1_3_RAW_PUBLIC_KEY, &rawexts, NULL, 1)) {
      /* SSLfatal already called */
      goto err;
    }
    /* chain index is always zero and fin always 1 for RPK */
    if (!tls_parse_all_extensions(
          sc, SSL_EXT_TLS1_3_RAW_PUBLIC_KEY, rawexts, NULL, 0, 1)) {
      /* SSLfatal already called */
      goto err;
    }
  }
...
tls_get_message_header(SSL_CONNECTION* s, int* mt)
{
  ...
  do {
    while (s->init_num < SSL3_HM_HEADER_LENGTH) {
      i = ssl->method->ssl_read_bytes(ssl,
                                      SSL3_RT_HANDSHAKE,
                                      &recvd_type,
                                      &p[s->init_num],
...
tls_get_message_body(SSL_CONNECTION* s, size_t* len)
{
  ...
  while (n > 0) {
    i = ssl->method->ssl_read_bytes(
      ssl, SSL3_RT_HANDSHAKE, NULL, &p[s->init_num], n, 0, &readbytes);
    if (i <= 0) {
      s->rwstate = SSL_READING;
      *len = 0;
      return 0;
