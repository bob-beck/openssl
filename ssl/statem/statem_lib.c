...
static int
get_cert_verify_tbs_data (SSL_CONNECTION *s, unsigned char *tls13tbs,
                          void **hdata, size_t *hdatalen)
{
  ...
  static const char servercontext[]
      = "\x54\x4c\x53\x20\x31\x2e\x33\x2c\x20\x73\x65\x72"
        "\x76\x65\x72\x20\x43\x65\x72\x74\x69\x66\x69\x63\x61\x74\x65\x56\x65"
        "\x72\x69\x66\x79";
  ...
  static const char clientcontext[]
      = "\x54\x4c\x53\x20\x31\x2e\x33\x2c\x20\x63\x6c\x69"
        "\x65\x6e\x74\x20\x43\x65\x72\x74\x69\x66\x69\x63\x61\x74\x65\x56\x65"
        "\x72\x69\x66\x79";
...
static int
ssl_add_cert_to_wpacket (SSL_CONNECTION *s, WPACKET *pkt, X509 *x, int chain,
                         int for_comp)
{
  int len;
...
static int
ssl_add_cert_chain (SSL_CONNECTION *s, WPACKET *pkt, CERT_PKEY *cpk,
                    int for_comp)
{
  int i, chain_count;
  X509 *x;
...
int
tls_process_rpk (SSL_CONNECTION *sc, PACKET *pkt, EVP_PKEY **peer_rpk)
{
  EVP_PKEY *pkey = NULL;
  int ret = 0;
...
unsigned long
ssl3_output_cert_chain (SSL_CONNECTION *s, WPACKET *pkt, CERT_PKEY *cpk,
                        int for_comp)
{
  if (!WPACKET_start_sub_packet_u24 (pkt))
    {
 ...
 */
WORK_STATE
tls_finish_handshake (SSL_CONNECTION *s, ossl_unused WORK_STATE wst,
 ...
 */
static int
ssl_method_error (const SSL_CONNECTION *s, const SSL_METHOD *method)
{
 ...
 */
static int
is_tls13_capable (const SSL_CONNECTION *s)
{
  size_t i;
  int curve;
  SSL_CTX *sctx = SSL_CONNECTION_GET_CTX (s);
 ...
 */
int
ssl_version_supported (const SSL_CONNECTION *s, int version,
...
static void
check_for_downgrade (SSL_CONNECTION *s, int vers, DOWNGRADE *dgrd)
{
 ...
 */
int
ssl_choose_server_version (SSL_CONNECTION *s, CLIENTHELLO_MSG *hello,
                           DOWNGRADE *dgrd)
{
  /*-
 ...
 */
int
ssl_choose_client_version (SSL_CONNECTION *s, int version,
 ...
 * version is initially (D)TLS_ANY_VERSION.  We apply any explicit
 * SSL_OP_NO_xxx options, the MinProtocol and MaxProtocol configuration
 * commands, any Suite B constraints and any floor imposed by the security
 * level here, so we don't advertise the wrong protocol version to only reject
 * the outcome later.
 *
 ...
 */
int
ssl_get_min_max_version (const SSL_CONNECTION *s, int *min_version,
 ...
 */
int
check_in_list (SSL_CONNECTION *s, uint16_t group_id, const uint16_t *groups,
...
int
create_synthetic_message_hash (SSL_CONNECTION *s, const unsigned char *hashval,
...
int
construct_ca_names (SSL_CONNECTION *s, const STACK_OF (X509_NAME) *ca_sk,
                    WPACKET *pkt)
{
...
size_t
construct_key_exchange_tbs (SSL_CONNECTION *s, unsigned char **ptbs,
...
MSG_PROCESS_RETURN
tls13_process_compressed_certificate (SSL_CONNECTION *sc, PACKET *pkt,
                                      PACKET *tmppkt, BUF_MEM *buf)
{
  ...
  if (!BUF_MEM_grow (buf, expected_length)
      ...
      || COMP_expand_block (
             comp, (unsigned char *)buf->data, (int)expected_length,
             (unsigned char *)PACKET_data (pkt), (int)comp_length)
             != (int)expected_length)
    {
      SSLfatal (sc, SSL_AD_BAD_CERTIFICATE, SSL_R_BAD_DECOMPRESSION);
      goto err;
