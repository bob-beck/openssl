...
static int get_cert_verify_tbs_data(SSL_CONNECTION* s,
...
static int ssl_add_cert_to_wpacket(SSL_CONNECTION* s,
                                   WPACKET* pkt,
                                   X509* x,
                                   int chain,
                                   int for_comp) {
...
static int ssl_add_cert_chain(SSL_CONNECTION* s,
                              WPACKET* pkt,
                              CERT_PKEY* cpk,
                              int for_comp) {
  int i, chain_count;
...
unsigned long ssl3_output_cert_chain(SSL_CONNECTION* s,
                                     WPACKET* pkt,
                                     CERT_PKEY* cpk,
                                     int for_comp) {
  if (!WPACKET_start_sub_packet_u24(pkt)) {
 ...
 * freed up as well.
 */
WORK_STATE tls_finish_handshake(SSL_CONNECTION* s,
 ...
 */
int ssl_version_supported(const SSL_CONNECTION* s,
                          int version,
 ...
 */
int ssl_choose_server_version(SSL_CONNECTION* s,
 ...
 */
int ssl_choose_client_version(SSL_CONNECTION* s,
                              int version,
 ...
 */
int ssl_get_min_max_version(const SSL_CONNECTION* s,
                            int* min_version,
                            int* max_version,
                            int* real_max) {
