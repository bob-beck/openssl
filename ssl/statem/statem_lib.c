...
int tls_setup_handshake(SSL_CONNECTION* s) {
  ...
  if (s->server) {
    ...
    if (SSL_CONNECTION_IS_DTLS(s)) s->statem.use_timer = 1;
  }
...
MSG_PROCESS_RETURN tls_process_cert_verify(SSL_CONNECTION* s, PACKET* pkt) {
...
#ifndef OPENSSL_NO_GOST
  {
    ...
    if (pktype == NID_id_GostR3410_2001 ||
        pktype == NID_id_GostR3410_2012_256 ||
        pktype == NID_id_GostR3410_2012_512) {
      if ((gost_data = OPENSSL_malloc(len)) == NULL) goto err;
  ...
  if (s->version == SSL3_VERSION) {
    ...
    if (SSL_IS_QUIC_HANDSHAKE(s)) j = 1;
#endif
    if (j <= 0) {
      SSLfatal(s, SSL_AD_DECRYPT_ERROR, SSL_R_BAD_SIGNATURE);
      goto err;
...
CON_FUNC_RETURN tls_construct_finished(SSL_CONNECTION* s, WPACKET* pkt) {
  ...
  finish_md_len = ssl->method->ssl3_enc->final_finish_mac(s, sender, slen,
                                                          s->s3.tmp.finish_md);
  if (finish_md_len == 0) {
    /* SSLfatal() already called */
    return CON_FUNC_ERROR;
...
MSG_PROCESS_RETURN tls_process_finished(SSL_CONNECTION* s, PACKET* pkt) {
  ...
  if (s->server) {
    ...
    if (s->post_handshake_auth != SSL_PHA_REQUESTED) s->statem.cleanuphand = 1;
...
static int ssl_add_cert_to_wpacket(SSL_CONNECTION* s, WPACKET* pkt, X509* x,
  ...
  if (for_comp) context |= SSL_EXT_TLS1_3_CERTIFICATE_COMPRESSION;

  len = i2d_X509(x, NULL);
  if (len < 0) {
    if (!for_comp) SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_BUF_LIB);
    return 0;
  }
  if (!WPACKET_sub_allocate_bytes_u24(pkt, len, &outbytes) ||
      i2d_X509(x, &outbytes) != len) {
    if (!for_comp) SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
    return 0;
  }
...
static int ssl_add_cert_chain(SSL_CONNECTION* s, WPACKET* pkt, CERT_PKEY* cpk,
  ...
  if (cpk == NULL || cpk->x509 == NULL) return 1;
  ...
  if (chain_store != NULL) {
    ...
    if (xs_ctx == NULL) {
      if (!for_comp) SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_X509_LIB);
      return 0;
    }
...
#endif
      X509_STORE_CTX_free(xs_ctx);
      if (!for_comp) SSLfatal(s, SSL_AD_INTERNAL_ERROR, i);
      return 0;
    }
...
EVP_PKEY* tls_get_peer_pkey(const SSL_CONNECTION* sc) {
  if (sc->session->peer_rpk != NULL) return sc->session->peer_rpk;
  if (sc->session->peer != NULL) return X509_get0_pubkey(sc->session->peer);
  return NULL;
}
...
unsigned long ssl3_output_cert_chain(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (!ssl_add_cert_chain(s, pkt, cpk, for_comp)) return 0;
...
int ssl_version_cmp(const SSL_CONNECTION* s, int versiona, int versionb) {
  ...
  if (!dtls) return versiona < versionb ? -1 : 1;
...
static int is_tls13_capable(const SSL_CONNECTION* s) {
  ...
  if (s->psk_server_callback != NULL) return 1;
#endif

  if (s->psk_find_session_cb != NULL || s->cert->cert_cb != NULL) return 1;
  ...
  for (i = 0; i < s->ssl_pkey_num; i++) {
    ...
    if (!ssl_has_cert(s, (int)i)) continue;
    if (i != SSL_PKEY_ECC) return 1;
    /*
    ...
    if (tls_check_sigalg_curve(s, curve)) return 1;
  }
...
int ssl_check_version_downgrade(SSL_CONNECTION* s) {
   ...
   * s->method).
   */
  if (s->version == ssl->defltmeth->version) return 1;
...
int ssl_set_version_bound(int method_version, int version, int* bound) {
  ...
  if (!valid_tls && !valid_dtls) return 0;
  ...
  switch (method_version) {
    ...
    case TLS_ANY_VERSION:
      if (valid_tls) *bound = version;
      break;

    case DTLS_ANY_VERSION:
      if (valid_dtls) *bound = version;
      break;
  }
  return 1;
}
...
int ssl_choose_server_version(SSL_CONNECTION* s, CLIENTHELLO_MSG* hello,
  ...
  if (suppversions->present && !SSL_CONNECTION_IS_DTLS(s)) {
     ...
     */
    if (client_version <= SSL3_VERSION) return SSL_R_BAD_LEGACY_VERSION;

    while (PACKET_get_net_2(&versionslist, &candidate_vers)) {
      if (ssl_version_cmp(s, candidate_vers, best_vers) <= 0) continue;
    ...
    if (best_vers > 0) {
      if (s->hello_retry_request != SSL_HRR_NONE) {
         ...
         * negotiated TLSv1.3
         */
        if (best_vers != TLS1_3_VERSION) return SSL_R_UNSUPPORTED_PROTOCOL;
        return 0;
      }
...
int ssl_choose_client_version(SSL_CONNECTION* s, int version,
  ...
  if (!tls_parse_extension(
          s, TLSEXT_IDX_supported_versions,
  ...
  if ((s->mode & SSL_MODE_SEND_FALLBACK_SCSV) == 0) real_max = ver_max;
  ...
  for (vent = table; vent->version != 0; ++vent) {
    if (vent->cmeth == NULL || s->version != vent->version) continue;
...
int ssl_get_min_max_version(const SSL_CONNECTION* s, int* min_version,
  ...
  switch (ssl->method->version) {
    default:
      ...
      *min_version = *max_version = s->version;
       ...
       * flexible method.
       */
      if (!ossl_assert(real_max == NULL)) return ERR_R_INTERNAL_ERROR;
      return 0;
    case TLS_ANY_VERSION:
      table = tls_version_table;
      break;
  ...
  *min_version = version = 0;
  ...
  for (vent = table; vent->version != 0; ++vent) {
    ...
    if (hole == 1 && tmp_real_max == 0) tmp_real_max = vent->version;
    ...
    if (ssl_method_error(s, method) != 0) {
      ...
      *min_version = method->version;
      ...
      if (real_max != NULL && tmp_real_max != 0) *real_max = tmp_real_max;
...
int ssl_set_client_hello_version(SSL_CONNECTION* s) {
   ...
   */
  if (!SSL_IS_FIRST_HANDSHAKE(s)) return 0;
  ...
  if (SSL_CONNECTION_IS_DTLS(s)) {
    if (ver_max == DTLS1_BAD_VER) {
       ...
       */
      if (!ssl_set_record_protocol_version(s, ver_max)) return 0;
    }
...
int check_in_list(SSL_CONNECTION* s, uint16_t group_id, const uint16_t* groups,
  ...
  if (groups == NULL || num_groups == 0) return 0;
...
const STACK_OF(X509_NAME)* get_ca_names(SSL_CONNECTION* s) {
  ...
  if (s->server) {
    ...
    if (ca_sk != NULL && sk_X509_NAME_num(ca_sk) == 0) ca_sk = NULL;
  }

  if (ca_sk == NULL) ca_sk = SSL_get0_CA_list(ssl);
...
int tls13_save_handshake_digest_for_pha(SSL_CONNECTION* s) {
  if (s->pha_dgst == NULL) {
    if (!ssl3_digest_cached_records(s, 1)) /* SSLfatal() already called */
      return 0;
