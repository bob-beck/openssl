...
EXT_RETURN tls_construct_ctos_srp(SSL_CONNECTION *s, WPACKET *pkt, unsigned int context, X509 *x, size_t chainidx)
{
    ...
    if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_srp)
        ...
        !WPACKET_memcpy(pkt, s->srp_ctx.login, strlen(s->srp_ctx.login)) || !WPACKET_close(pkt) || !WPACKET_close(pkt))
    {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        return EXT_RETURN_FAIL;
    }
                                         ...
                                         size_t chainidx)
{
    ...
    if (s->psk_use_session_cb != NULL && (!s->psk_use_session_cb(ussl, handmd, &id, &idlen, &psksess) ||
    ...
    if (psksess == NULL && s->psk_client_callback != NULL)
    {
        ...
        else if (psklen > 0)
        {
            ...
            if (psksess == NULL || !SSL_SESSION_set1_master_key(psksess, psk, psklen) ||
                !SSL_SESSION_set_cipher(psksess, cipher) || !SSL_SESSION_set_protocol_version(psksess, TLS1_3_VERSION))
            {
                SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
