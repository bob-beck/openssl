...
EXT_RETURN tls_construct_ctos_server_name(SSL_CONNECTION* s, WPACKET* pkt,
                                          unsigned int context, X509* x,
                                          size_t chainidx) {
  if (s->ext.hostname == NULL) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_srp(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (s->srp_ctx.login == NULL) return EXT_RETURN_NOT_SENT;
...
static int use_ecc(SSL_CONNECTION* s, int min_version, int max_version) {
  ...
  if (s->version == SSL3_VERSION) return 0;
...
EXT_RETURN tls_construct_ctos_ec_pt_formats(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (!use_ecc(s, min_version, max_version)) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_supported_groups(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  for (i = 0; i < num_groups; i++) {
    ...
    if (tls_valid_group(s, ctmp, min_version, max_version, 0, &okfortls13) &&
      ...
      if (okfortls13 && max_version == TLS1_3_VERSION) tls13added++;
      added++;
    }
  }
...
EXT_RETURN tls_construct_ctos_session_ticket(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (!tls_use_ticket(s)) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_status_request(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (x != NULL) return EXT_RETURN_NOT_SENT;

  if (s->ext.status_type != TLSEXT_STATUSTYPE_ocsp) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_use_srtp(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (clnt == NULL) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_etm(SSL_CONNECTION* s, WPACKET* pkt,
                                  unsigned int context, X509* x,
                                  size_t chainidx) {
  if (s->options & SSL_OP_NO_ENCRYPT_THEN_MAC) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_sct(SSL_CONNECTION* s, WPACKET* pkt,
                                  unsigned int context, X509* x,
                                  size_t chainidx) {
  if (s->ct_validation_callback == NULL) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_ems(SSL_CONNECTION* s, WPACKET* pkt,
                                  unsigned int context, X509* x,
                                  size_t chainidx) {
  if (s->options & SSL_OP_NO_EXTENDED_MASTER_SECRET) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_supported_versions(SSL_CONNECTION* s,
   ...
   */
  if (max_version < TLS1_3_VERSION) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_psk_kex_modes(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (nodhe) s->ext.psk_kex_mode |= TLSEXT_KEX_MODE_FLAG_KE;
#endif
...
static int add_key_share(SSL_CONNECTION* s, WPACKET* pkt, unsigned int group_id,
  ...
  if (loop_num >= s->s3.tmp.num_ks_pkey) s->s3.tmp.num_ks_pkey++;
...
EXT_RETURN tls_construct_ctos_cookie(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (s->ext.tls13_cookie_len == 0) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_early_data(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (s->hello_retry_request == SSL_HRR_PENDING) handmd = ssl_handshake_md(s);
...
EXT_RETURN tls_construct_ctos_padding(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if ((s->options & SSL_OP_TLSEXT_PADDING) == 0) return EXT_RETURN_NOT_SENT;
  ...
  if (s->session->ssl_version == TLS1_3_VERSION &&
    ...
    if (md != NULL) {
      ...
      if (md_size <= 0) return EXT_RETURN_FAIL;
...
EXT_RETURN tls_construct_ctos_psk(SSL_CONNECTION* s, WPACKET* pkt,
  ...
  if (s->session->ext.ticklen != 0) {
    ...
    if (reshashsize <= 0) goto dopsksess;
    s->ext.tick_identity++;
    dores = 1;
  }

dopsksess:
  if (!dores && s->psksession == NULL) return EXT_RETURN_NOT_SENT;
...
#ifndef OPENSSL_NO_TLS1_3
  if (!s->pha_enabled) return EXT_RETURN_NOT_SENT;
...
int tls_parse_stoc_alpn(SSL_CONNECTION* s, PACKET* pkt, unsigned int context,
  ...
  while (PACKET_get_length_prefixed_1(&confpkt, &protpkt)) {
    if (PACKET_remaining(&protpkt) != len) continue;
...
int tls_parse_stoc_supported_versions(SSL_CONNECTION* s, PACKET* pkt,
  ...
  if (context == SSL_EXT_TLS1_3_HELLO_RETRY_REQUEST) return 1;
...
int tls_parse_stoc_psk(SSL_CONNECTION* s, PACKET* pkt, unsigned int context,
  ...
  if (identity != 0) s->ext.early_data_ok = 0;
#endif
...
EXT_RETURN tls_construct_ctos_client_cert_type(SSL_CONNECTION* sc, WPACKET* pkt,
  ...
  if (sc->client_cert_type == NULL) return EXT_RETURN_NOT_SENT;
...
EXT_RETURN tls_construct_ctos_server_cert_type(SSL_CONNECTION* sc, WPACKET* pkt,
  ...
  if (sc->server_cert_type == NULL) return EXT_RETURN_NOT_SENT;
