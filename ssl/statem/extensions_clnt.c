...
#ifndef OPENSSL_NO_TLS1_3
static int
add_key_share(SSL_CONNECTION* s,
              WPACKET* pkt,
  ...
  if (s->s3.group_id != 0 && s->s3.tmp.pkey == NULL) {
    ...
    for (i = 0; i < num_groups; i++) {
      if (!tls_group_allowed(s, pgroups[i], SSL_SECOP_CURVE_SUPPORTED))
        continue;
      if (!tls_valid_group(
            s, pgroups[i], TLS1_3_VERSION, TLS1_3_VERSION, 0, NULL))
        continue;
                          ...
                          size_t chainidx)
{
  ...
  if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_cookie)
      ...
      !WPACKET_sub_memcpy_u16(
        pkt, s->ext.tls13_cookie, s->ext.tls13_cookie_len) ||
      !WPACKET_close(pkt)) {
    SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
    goto end;
  }
                              ...
                              size_t chainidx)
{
  ...
  if (edsess->ext.alpn_selected != NULL) {
    ...
    while (PACKET_get_length_prefixed_1(&prots, &alpnpkt)) {
      if (PACKET_equal(&alpnpkt,
                       edsess->ext.alpn_selected,
...
dopsksess:
  ...
  if (dores) {
    if (!WPACKET_sub_memcpy_u16(
          pkt, s->session->ext.tick, s->session->ext.ticklen) ||
  ...
  if (dores &&
      tls_psk_do_binder(
        s, mdres, msgstart, binderoffset, NULL, resbinder, s->session, 1, 0) !=
        1) {
    /* SSLfatal() already called */
    return EXT_RETURN_FAIL;
  }

  if (s->psksession != NULL && tls_psk_do_binder(s,
                                                 mdpsk,
                                                 msgstart,
                                                 binderoffset,
                                                 NULL,
                             ...
                             size_t chainidx)
{
  ...
  if (!s->hit) {
    ...
    if (!PACKET_copy_bytes(
          &ecptformatlist, s->ext.peer_ecpointformats, ecpointformats_len)) {
      SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
      return 0;
    }
  }
                   ...
                   size_t chainidx)
{
  ...
  if (s->ct_validation_callback != NULL) {
     ...
     */
    if (custom_ext_find(&s->cert->custext,
                        role,
                         ...
                         size_t chainidx)
{
  ...
  if (!ginf->is_kem) {
    ...
    if (tls13_set_encoded_pub_key(
          skey, PACKET_data(&encoded_pt), PACKET_remaining(&encoded_pt)) <= 0) {
                                    ...
                                    size_t chainidx)
{
  ...
  if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_client_cert_type) ||
      ...
      !WPACKET_sub_memcpy_u8(
        pkt, sc->client_cert_type, sc->client_cert_type_len) ||
      !WPACKET_close(pkt)) {
                                    ...
                                    size_t chainidx)
{
  ...
  if (!WPACKET_put_bytes_u16(pkt, TLSEXT_TYPE_server_cert_type) ||
      !WPACKET_start_sub_packet_u16(pkt) ||
      !WPACKET_sub_memcpy_u8(
        pkt, sc->server_cert_type, sc->server_cert_type_len) ||
      !WPACKET_close(pkt)) {
    SSLfatal(sc, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
    return EXT_RETURN_FAIL;
  }
...
int
tls_parse_stoc_server_cert_type(SSL_CONNECTION* sc,
                                PACKET* pkt,
                                unsigned int context,
                                X509* x,
                                size_t chainidx)
