...
static int ossl_statem_server13_read_transition(SSL_CONNECTION* s, int mt) {
  ...
  switch (st->hand_state) {
    ...
    case TLS_ST_OK:
       ...
       */
      if (s->early_data_state == SSL_EARLY_DATA_READING) break;
...
int ossl_statem_server_read_transition(SSL_CONNECTION* s, int mt) {
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    if (!ossl_statem_server13_read_transition(s, mt)) goto err;
    return 1;
  }
...
static int get_compressed_certificate_alg(SSL_CONNECTION* sc) {
  ...
  if (sc->s3.tmp.cert == NULL) return TLSEXT_comp_cert_none;

  for (; *alg != TLSEXT_comp_cert_none; alg++) {
    if (sc->s3.tmp.cert->comp_cert[*alg] != NULL) return *alg;
  }
#endif
...
WORK_STATE ossl_statem_server_pre_work(SSL_CONNECTION* s, WORK_STATE wst) {
  ...
  switch (st->hand_state) {
    ...
    case TLS_ST_SW_HELLO_REQ:
      s->shutdown = 0;
      if (SSL_CONNECTION_IS_DTLS(s)) dtls1_clear_sent_buffer(s);
      break;
    ...
    case TLS_ST_SW_CHANGE:
      if (SSL_CONNECTION_IS_TLS13(s)) break;
...
WORK_STATE ossl_statem_server_post_work(SSL_CONNECTION* s, WORK_STATE wst) {
  ...
  switch (st->hand_state) {
    ...
    case TLS_ST_SW_CHANGE:
      if (s->hello_retry_request == SSL_HRR_PENDING) {
        if (!statem_flush(s)) return WORK_MORE_A;
        break;
      }
...
static void ssl_check_for_safari(SSL_CONNECTION* s,
  ...
  if (type != TLSEXT_TYPE_server_name) return;
...
err:
  if (clienthello != NULL) OPENSSL_free(clienthello->pre_proc_exts);
  OPENSSL_free(clienthello);
...
static int tls_early_post_process_client_hello(SSL_CONNECTION* s) {
  ...
  /* This only happens if we have a cache hit */
    ...
    for (k = 0; k < clienthello->compressions_len; k++) {
      if (clienthello->compressions[k] == comp_id) break;
    }
  ...
  if (wst == WORK_MORE_B) {
    if (!s->hit || SSL_CONNECTION_IS_TLS13(s)) {
      ...
      if (!s->hit) {
        ...
        if (s->session->not_resumable) /* do not send a session ticket */
...
CON_FUNC_RETURN tls_construct_server_hello(SSL_CONNECTION* s, WPACKET* pkt) {
  ...
  if (!tls_construct_extensions(
          ...
          s->hello_retry_request == SSL_HRR_PENDING
              ...
              : (SSL_CONNECTION_IS_TLS13(s) ? SSL_EXT_TLS1_3_SERVER_HELLO
...
static void tls_update_ticket_counts(SSL_CONNECTION* s) {
  ...
  s->sent_tickets++;
  s->next_ticket_nonce++;
  if (s->ext.extra_tickets_expected > 0) s->ext.extra_tickets_expected--;
}
 ...
 */
int tls_construct_cert_status_body(SSL_CONNECTION* s, size_t chainidx,
                                   WPACKET* pkt) {
  ...
  if (X509_self_signed(x, 0)) return 1;
    ...
    if (OCSP_response_status(resp) == OCSP_RESPONSE_STATUS_SUCCESSFUL) {
      ...
      if (((bs = OCSP_response_get1_basic(resp)) != NULL) &&
        ...
        for (i = 0; i < num; i++) {
          ...
          OCSP_id_get0_info(&respIssuerNameHash, NULL, NULL, &respSerial,
                            sr_cert_id);
  ...
  if (resp != NULL) resplen = i2d_OCSP_RESPONSE(resp, &respder);
#endif
