...
/*
 * ossl_statem_server13_read_transition() encapsulates the logic for the
 * allowed handshake state transitions when a TLSv1.3 server is reading
 * messages from the client. The message type that the client has sent is
 * provided in |mt|. The current state is in |s->statem.hand_state|.
 *
 ...
 */
int
ossl_statem_server_construct_message (SSL_CONNECTION *s, confunc_f *confunc,
                                      int *mt)
{
  OSSL_STATEM *st = &s->statem;
...
int
dtls_raw_hello_verify_request (WPACKET *pkt, unsigned char *cookie,
                               size_t cookie_len)
{
 ...
 *
 * We wish to fingerprint Safari because they broke ECDHE-ECDSA support
 * in 10.8, but they advertise support. So enabling ECDHE-ECDSA ciphers breaks
 * them. Sadly we cannot differentiate 10.6, 10.7 and 10.8.4 (which work), from
...
tls_process_cke_psk_preamble (SSL_CONNECTION *s, PACKET *pkt)
{
  ...
  psklen
      = s->psk_server_callback (SSL_CONNECTION_GET_USER_SSL (s),
...
tls_process_cke_ecdhe (SSL_CONNECTION *s, PACKET *pkt)
{
  ...
  if (PACKET_remaining (pkt) == 0L)
  ...
  else
    {
       ...
       */
      if (!PACKET_get_1 (pkt, &i) || i == 0
          || !PACKET_get_bytes (pkt, &data, i) || PACKET_remaining (pkt) != 0)
        {
          SSLfatal (s, SSL_AD_DECODE_ERROR, SSL_R_LENGTH_MISMATCH);
          goto err;
        }
...
static int
create_ticket_prequel (SSL_CONNECTION *s, WPACKET *pkt, uint32_t age_add,
...
construct_stateless_ticket (SSL_CONNECTION *s, WPACKET *pkt, uint32_t age_add,
                            unsigned char *tick_nonce)
{
...
static int
construct_stateful_ticket (SSL_CONNECTION *s, WPACKET *pkt, uint32_t age_add,
                           unsigned char *tick_nonce)
{
  if (!create_ticket_prequel (s, pkt, age_add, tick_nonce))
    {
 ...
 */
int
tls_construct_cert_status_body (SSL_CONNECTION *s, size_t chainidx,
                                WPACKET *pkt)
{
  ...
  if ((resp = sk_OCSP_RESPONSE_value (s->ext.ocsp.resp_ex, (int)chainidx))
    ...
    {
      /*
       * check if its the right response in the case it is a successful
       * response as not every time the issuer certificate is available the
       * check just uses the issuer name and the serial number from the current
       * certificate
       */
      if (OCSP_response_status (resp) == OCSP_RESPONSE_STATUS_SUCCESSFUL)
        {
          ...
          if (((bs = OCSP_response_get1_basic (resp)) != NULL)
              && ((sr = OCSP_resp_get0 (bs, 0)) != NULL))
            {
              ...
              OCSP_id_get0_info (&respIssuerNameHash, &cert_id_md_oid, NULL,
                                 &respSerial, cid);
              ...
              /* get serial number and issuer name hash of the certificate from
               * the chain */
              ...
              for (i = 0; i < num; i++)
                {
                  ...
                  /* determine the md algorithm which was used to create cert
                   * id */
                  sr_cert_id = (OCSP_CERTID *)OCSP_SINGLERESP_get0_id (sr);

                  OCSP_id_get0_info (&respIssuerNameHash, NULL, NULL,
                                     &respSerial, sr_cert_id);

                  if (!ASN1_INTEGER_cmp (certSerial, respSerial)
                      && !ASN1_OCTET_STRING_cmp (certIssuerNameHash,
                                                 respIssuerNameHash))
                    break;
                }
              ...
              /*
               * if we did not find the right single response in the OCSP
               * response we construct an empty message
               */
              if (i == num)
                resp = NULL;
            }
          ...
          /*
           * in a test case a response without a basic response is used the
           * error set could be ignored here
           */
          ERR_pop_to_mark ();
        }
    }
