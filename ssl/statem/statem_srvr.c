...
received_client_cert(const SSL_CONNECTION* sc)
{
 ...
 */
static int
ossl_statem_server13_read_transition(SSL_CONNECTION* s, int mt)
{
 ...
 */
static int
send_server_key_exchange(SSL_CONNECTION* s)
{
 ...
 */
static int
get_compressed_certificate_alg(SSL_CONNECTION* sc)
{
#ifndef OPENSSL_NO_COMP_ALG
 ...
 */
static WRITE_TRAN
ossl_statem_server13_write_transition(SSL_CONNECTION* s)
{
  OSSL_STATEM* st = &s->statem;
  ...
  switch (st->hand_state) {
    ...
    case TLS_ST_SW_SRVR_HELLO:
      ...
      if (SSL_CONNECTION_IS_DTLS(s) && s->hit) {
         ...
         */
        memcpy(
          labelbuffer, DTLS1_SCTP_AUTH_LABEL, sizeof(DTLS1_SCTP_AUTH_LABEL));
...
static int
ssl_check_srp_ext_ClientHello(SSL_CONNECTION* s)
{
  int ret;
...
dtls_construct_hello_verify_request(SSL_CONNECTION* s, WPACKET* pkt)
{
  ...
  if (sctx->app_gen_cookie_cb == NULL ||
      sctx->app_gen_cookie_cb(
        SSL_CONNECTION_GET_USER_SSL(s), s->d1->cookie, &cookie_leni) == 0 ||
 ...
 */
static void
ssl_check_for_safari(SSL_CONNECTION* s, const CLIENTHELLO_MSG* hello)
{
...
tls_process_client_hello(SSL_CONNECTION* s, PACKET* pkt)
{
  ...
  if (clienthello->isv2) {
    ...
    if (!PACKET_get_sub_packet(
          pkt, &clienthello->ciphersuites, ciphersuite_len) ||
...
static int
tls_early_post_process_client_hello(SSL_CONNECTION* s)
{
  unsigned int j;
    ...
    if (SSL_get_options(ssl) & SSL_OP_COOKIE_EXCHANGE) {
      if (sctx->app_verify_cookie_cb != NULL) {
        ...
        /* default verification */
                 ...
                 memcmp(clienthello->dtls_cookie,
                        s->d1->cookie,
                        s->d1->cookie_len) != 0) {
        SSLfatal(s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_COOKIE_MISMATCH);
        goto err;
  ...
  if (scsvs != NULL) {
    for (i = 0; i < sk_SSL_CIPHER_num(scsvs); i++) {
      ...
      if (SSL_CIPHER_get_id(c) == SSL3_CK_SCSV) {
         ...
         */
        SSLfatal(
          s, SSL_AD_INAPPROPRIATE_FALLBACK, SSL_R_INAPPROPRIATE_FALLBACK);
        goto err;
      }
    }
  }
  ...
  if (!tls_parse_all_extensions(
        s, SSL_EXT_CLIENT_HELLO, clienthello->pre_proc_exts, NULL, 0, 1)) {
    /* SSLfatal() already called */
    goto err;
  }
  ...
  /* This only happens if we have a cache hit */
    ...
    if (s->s3.tmp.new_compression == NULL) {
      SSLfatal(
        s, SSL_AD_HANDSHAKE_FAILURE, SSL_R_INVALID_COMPRESSION_ALGORITHM);
      goto err;
    }
 ...
 */
static int
tls_handle_status_request(SSL_CONNECTION* s)
{
  SSL_CTX* sctx = SSL_CONNECTION_GET_CTX(s);
...
tls_handle_alpn(SSL_CONNECTION* s)
{
  ...
  if (sctx->ext.alpn_select_cb != NULL && s->s3.alpn_proposed != NULL) {
    ...
    if (r == SSL_TLSEXT_ERR_OK) {
      ...
      SSLfatal(
        s, SSL_AD_NO_APPLICATION_PROTOCOL, SSL_R_NO_APPLICATION_PROTOCOL);
      return 0;
    }
    /*
...
tls_construct_server_key_exchange(SSL_CONNECTION* s, WPACKET* pkt)
{
  ...
  if (type & (SSL_kPSK | SSL_kRSAPSK)) {
    ...
    if (type & (SSL_kDHE | SSL_kDHEPSK)) {
      ...
      if (!ssl_security(
            s, SSL_SECOP_TMP_DH, EVP_PKEY_get_security_bits(pkdhp), 0, pkdhp)) {
      ...
      if (!EVP_PKEY_get_bn_param(
            s->s3.tmp.pkey, OSSL_PKEY_PARAM_FFC_P, &r[0]) ||
          !EVP_PKEY_get_bn_param(
            s->s3.tmp.pkey, OSSL_PKEY_PARAM_FFC_G, &r[1]) ||
          !EVP_PKEY_get_bn_param(
            s->s3.tmp.pkey, OSSL_PKEY_PARAM_PUB_KEY, &r[2])) {
        SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
        goto err;
      }
...
static int
tls_process_cke_psk_preamble(SSL_CONNECTION* s, PACKET* pkt)
{
#ifndef OPENSSL_NO_PSK
  ...
  psklen = s->psk_server_callback(
    SSL_CONNECTION_GET_USER_SSL(s), s->session->psk_identity, psk, sizeof(psk));
...
static int
tls_process_cke_rsa(SSL_CONNECTION* s, PACKET* pkt)
{
  size_t outlen;
...
static int
tls_process_cke_dhe(SSL_CONNECTION* s, PACKET* pkt)
{
  EVP_PKEY* skey = NULL;
  unsigned int i;
...
static int
tls_process_cke_ecdhe(SSL_CONNECTION* s, PACKET* pkt)
{
...
static int
tls_process_cke_srp(SSL_CONNECTION* s, PACKET* pkt)
{
#ifndef OPENSSL_NO_SRP
  unsigned int i;
  const unsigned char* data;
...
static int
tls_process_cke_gost(SSL_CONNECTION* s, PACKET* pkt)
{
...
static int
tls_process_cke_gost18(SSL_CONNECTION* s, PACKET* pkt)
{
#ifndef OPENSSL_NO_GOST
...
tls_process_client_certificate(SSL_CONNECTION* s, PACKET* pkt)
{
  ...
  if (sk_X509_num(sk) <= 0) {
    ...
    if (i <= 0) {
      SSLfatal(s,
               ssl_x509err2alert(s->verify_result),
               SSL_R_CERTIFICATE_VERIFY_FAILED);
      goto err;
    }
...
static int
create_ticket_prequel(SSL_CONNECTION* s,
                      WPACKET* pkt,
                      uint32_t age_add,
...
static CON_FUNC_RETURN
construct_stateless_ticket(SSL_CONNECTION* s,
                           WPACKET* pkt,
                           uint32_t age_add,
                           unsigned char* tick_nonce)
{
...
#endif
  {
    ...
    if (tctx->ext.ticket_key_evp_cb != NULL)
      ...
      ret = tctx->ext.ticket_key_cb(
        ssl, key_name, iv, ctx, ssl_hmac_get0_HMAC_CTX(hctx), 1);
#endif
    ...
    if (iv_len < 0 || RAND_bytes_ex(sctx->libctx, iv, iv_len, 0) <= 0 ||
        !EVP_EncryptInit_ex(
          ctx, cipher, NULL, tctx->ext.secure->tick_aes_key, iv) ||
        !ssl_hmac_init(hctx,
...
static int
construct_stateful_ticket(SSL_CONNECTION* s,
                          WPACKET* pkt,
                          uint32_t age_add,
                          unsigned char* tick_nonce)
{
  ...
  if (!WPACKET_memcpy(
        pkt, s->session->session_id, s->session->session_id_length) ||
      !WPACKET_close(pkt)) {
    SSLfatal(s, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
    return 0;
  }
...
static void
tls_update_ticket_counts(SSL_CONNECTION* s)
{
  /*
...
tls_construct_new_session_ticket(SSL_CONNECTION* s, WPACKET* pkt)
{
  ...
  if (SSL_CONNECTION_IS_TLS13(s)) {
    if (!tls_construct_extensions(
          s, pkt, SSL_EXT_TLS1_3_NEW_SESSION_TICKET, NULL, 0)) {
      /* SSLfatal() already called */
      goto err;
    }
    tls_update_ticket_counts(s);
 ...
 */
int
tls_construct_cert_status_body(SSL_CONNECTION* s, size_t chainidx, WPACKET* pkt)
{
   ...
   */
  if (chain_certs != NULL && (int)chainidx <= sk_X509_num(chain_certs) &&
      chainidx > 0)
  ...
  if ((resp = sk_OCSP_RESPONSE_value(s->ext.ocsp.resp_ex, (int)chainidx)) !=
      NULL) {
    /*
    ...
    if (OCSP_response_status(resp) == OCSP_RESPONSE_STATUS_SUCCESSFUL) {
      ...
      if (((bs = OCSP_response_get1_basic(resp)) != NULL) &&
        ...
        OCSP_id_get0_info(
          &respIssuerNameHash, &cert_id_md_oid, NULL, &respSerial, cid);
        if (cert_id_md_oid != NULL)
          cert_id_md = EVP_get_digestbyobj(cert_id_md_oid);
        else
          cert_id_md = EVP_sha1();

        /* get serial number and issuer name hash of the certificate from the
         * chain */
        ...
        for (i = 0; i < num; i++) {
          ...
          OCSP_id_get0_info(
            &respIssuerNameHash, NULL, NULL, &respSerial, sr_cert_id);
      ...
      /*
       * in a test case a response without a basic response is used the error
       * set could be ignored here
       */
      ERR_pop_to_mark();
    }
  }
...
{
  if (!tls_construct_extensions(
        s, pkt, SSL_EXT_TLS1_3_ENCRYPTED_EXTENSIONS, NULL, 0)) {
    /* SSLfatal() already called */
    return CON_FUNC_ERROR;
  }
