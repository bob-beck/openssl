        ...
        if ((s->s3.tmp.new_cipher->algorithm_mkey & SSL_kSRP) && (s->srp_ctx.TLS_ext_srp_username_callback != NULL)) {
            if (s->srp_ctx.login == NULL) {
                ...
                if (ret == SSL3_AL_FATAL) {
                    SSLfatal(s,
                             al,
                             al == SSL_AD_UNKNOWN_PSK_IDENTITY ? SSL_R_PSK_IDENTITY_NOT_FOUND
...
#ifndef OPENSSL_NO_SRP
                if (type & SSL_kSRP) {
                if ((s->srp_ctx.N == NULL) || (s->srp_ctx.g == NULL) || (s->srp_ctx.s == NULL)
                    || (s->srp_ctx.B == NULL)) {
    ...
    static int tls_process_cke_rsa(SSL_CONNECTION * s, PACKET * pkt)
    {
        ...
        *p++ = OSSL_PARAM_construct_uint(OSSL_ASYM_CIPHER_PARAM_TLS_CLIENT_VERSION, (unsigned int *)&s->client_version);
        if ((s->options & SSL_OP_TLS_ROLLBACK_BUG) != 0)
            *p++ =
                OSSL_PARAM_construct_uint(OSSL_ASYM_CIPHER_PARAM_TLS_NEGOTIATED_VERSION, (unsigned int *)&s->version);
