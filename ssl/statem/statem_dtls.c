...
dtls1_write_message_header(SSL_CONNECTION* s, unsigned char* p);
static void
dtls1_set_message_header_int(SSL_CONNECTION* s,
                             unsigned char mt,
                             size_t len,
                             ...
                             size_t frag_len);
static int
dtls_get_reassembled_message(SSL_CONNECTION* s, int* errtype, size_t* len);
...
dtls1_do_write(SSL_CONNECTION* s, uint8_t type)
{
  ...
  while (s->init_num > 0) {
    ...
    ret = dtls1_write_bytes(
      s, type, &s->init_buf->data[s->init_off], len, &written);

    if (type == SSL3_RT_HANDSHAKE && s->msg_callback && s->init_off != 0)
      memcpy(
        &s->init_buf->data[s->init_off], saved_payload, sizeof(saved_payload));
    ...
    if (ret <= 0) {
       ...
       */
      if (retry &&
          BIO_ctrl(SSL_get_wbio(ssl), BIO_CTRL_DGRAM_MTU_EXCEEDED, 0, NULL) >
            0) {
 ...
 */
static size_t
dtls1_max_handshake_message_len(const SSL_CONNECTION* s)
{
...
static int
dtls1_preprocess_fragment(SSL_CONNECTION* s, struct hm_header_st* msg_hdr)
{
 ...
 */
static int
dtls1_retrieve_buffered_fragment(SSL_CONNECTION* s, size_t* len)
{
  /*-
...
static int
dtls1_reassemble_fragment(SSL_CONNECTION* s, const struct hm_header_st* msg_hdr)
{
  hm_fragment* frag = NULL;
  pitem* item = NULL;
  int i = -1, is_complete;
  ...
  RSMBLY_BITMASK_MARK(frag->reassembly,
                      (long)msg_hdr->frag_off,
                      (long)(msg_hdr->frag_off + frag_len));

  if (!ossl_assert(msg_hdr->msg_len > 0))
    goto err;
  RSMBLY_BITMASK_IS_COMPLETE(
    frag->reassembly, (long)msg_hdr->msg_len, is_complete);
                                 ...
                                 const struct hm_header_st* msg_hdr)
{
  ...
  if (msg_hdr->seq <= s->d1->handshake_read_seq ||
    ...
    if (frag_len) {
       ...
       */
      i = ssl->method->ssl_read_bytes(
        ssl, SSL3_RT_HANDSHAKE, NULL, frag->fragment, frag_len, 0, &readbytes);
      if (i <= 0 || readbytes != frag_len)
        i = -1;
      if (i <= 0)
        goto err;
...
redo:
  ...
  if (frag_len > 0) {
    ...
    i = ssl->method->ssl_read_bytes(
      ssl, SSL3_RT_HANDSHAKE, NULL, &p[frag_off], frag_len, 0, &readbytes);
...
int
dtls1_buffer_message(SSL_CONNECTION* s, int is_ccs)
{
  pitem* item;
  hm_fragment* frag;
  unsigned char seq64be[8];
...
dtls1_retransmit_message(SSL_CONNECTION* s, unsigned short seq, int* found)
{
  ...
  *found = 1;
  ...
  memcpy(s->init_buf->data,
         frag->fragment,
         frag->msg_header.msg_len + header_length);
  s->init_num = frag->msg_header.msg_len + header_length;

  dtls1_set_message_header_int(s,
                         ...
                         size_t frag_len)
{
  ...
  dtls1_set_message_header_int(
    s, mt, len, s->d1->handshake_write_seq, frag_off, frag_len);
}
...
dtls1_set_handshake_header(SSL_CONNECTION* s, WPACKET* pkt, int htype)
{
  ...
  if (htype == SSL3_MT_CHANGE_CIPHER_SPEC) {
    s->d1->handshake_write_seq = s->d1->next_handshake_write_seq;
    dtls1_set_message_header_int(
      s, SSL3_MT_CCS, 0, s->d1->handshake_write_seq, 0, 0);
