...
static hm_fragment* dtls1_hm_fragment_new(size_t frag_len, int reassembly) {
  ...
  if ((frag = OPENSSL_zalloc(sizeof(*frag))) == NULL) return NULL;
...
int dtls1_do_write(SSL_CONNECTION* s, uint8_t type) {
  ...
  while (s->init_num > 0) {
    ...
    if (len > ssl_get_max_send_fragment(s)) len = ssl_get_max_send_fragment(s);
    ...
    if (ret <= 0) {
       ...
       */
      if (!ossl_assert(len == written)) return -1;
      ...
      if (written == s->init_num) {
        if (s->msg_callback)
          s->msg_callback(1, s->version, type, s->init_buf->data,
                          s->init_off + s->init_num, ussl, s->msg_callback_arg);
...
static size_t dtls1_max_handshake_message_len(const SSL_CONNECTION* s) {
  ...
  if (max_len < s->max_cert_list) return s->max_cert_list;
  return max_len;
}
...
static int dtls1_reassemble_fragment(SSL_CONNECTION* s,
  ...
  i = ssl->method->ssl_read_bytes(ssl, SSL3_RT_HANDSHAKE, NULL,
                                  ...
                                  0, &readbytes);
  if (i <= 0 || readbytes != frag_len) i = -1;
  if (i <= 0) goto err;

  RSMBLY_BITMASK_MARK(frag->reassembly, (long)msg_hdr->frag_off,
                      (long)(msg_hdr->frag_off + frag_len));

  if (!ossl_assert(msg_hdr->msg_len > 0)) goto err;
  ...
  if (item == NULL) {
     ...
     * branch.
     */
    if (!ossl_assert(item != NULL)) goto err;
  }
...
static int dtls1_process_out_of_seq_message(
    SSL_CONNECTION* s, const struct hm_header_st* msg_hdr) {
  int i = -1;
  hm_fragment* frag = NULL;
  pitem* item = NULL;
  unsigned char seq64be[8];
...
int dtls1_buffer_message(SSL_CONNECTION* s, int is_ccs) {
   ...
   * serialized
   */
  if (!ossl_assert(s->init_off == 0)) return 0;
...
int dtls1_set_handshake_header(SSL_CONNECTION* s, WPACKET* pkt, int htype) {
  ...
  if (htype == SSL3_MT_CHANGE_CIPHER_SPEC) {
    ...
    dtls1_set_message_header_int(s, SSL3_MT_CCS, 0, s->d1->handshake_write_seq,
                                 0, 0);
    if (!WPACKET_put_bytes_u8(pkt, SSL3_MT_CCS)) return 0;
  } else {
