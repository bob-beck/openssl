 ...
 * ownership of the reference. If the QUIC connection SSL object has not
 * already been connected, HTTP/3 ALPN is set automatically. If it has already
 * been connected, HTTP/3 ALPN ("h3") must have been configured and no streams
 * must have been created yet.
 *
 ...
 *
 * This works analogously to nghttp3_conn_submit_request(). The stream user
 * data pointer passed to the callbacks is a OSSL_DEMO_H3_STREAM object
 * pointer; to retrieve the stream user data pointer passed to this function,
 * use OSSL_DEMO_H3_STREAM_get_user_data.
 */
