...
static OSSL_DEMO_H3_STREAM* h3_conn_create_stream(OSSL_DEMO_H3_CONN* conn,
  ...
  if ((s = OPENSSL_zalloc(sizeof(OSSL_DEMO_H3_STREAM))) == NULL) return NULL;

  if (type != OSSL_DEMO_H3_STREAM_TYPE_REQ) flags |= SSL_STREAM_FLAG_UNI;
...
static int h3_conn_recv_data(nghttp3_conn* h3conn, int64_t stream_id,
  ...
  if (conn->recv_data_cb == NULL) return 0;
...
static int h3_conn_reset_stream(nghttp3_conn* h3conn, int64_t stream_id,
  ...
  if (stream->s != NULL) {
    ...
    if (!SSL_stream_reset(stream->s, &args, sizeof(args))) return 1;
  }
...
OSSL_DEMO_H3_CONN* OSSL_DEMO_H3_CONN_new_for_conn(
  ...
  if ((conn = OPENSSL_zalloc(sizeof(OSSL_DEMO_H3_CONN))) == NULL) return NULL;
  ...
  if (callbacks != NULL) intl_callbacks = *callbacks;
...
OSSL_DEMO_H3_CONN* OSSL_DEMO_H3_CONN_new_for_addr(
  ...
  if ((qconn_bio = BIO_new_ssl_connect(ctx)) == NULL) goto err;

  /* Pass the 'hostname:port' string into the ssl_connect BIO. */
  if (BIO_set_conn_hostname(qconn_bio, addr) == 0) goto err;
  ...
  if (bare_hostname == NULL) goto err;

  if (BIO_get_ssl(qconn_bio, &qconn) == 0) goto err;

  /* Set the hostname we will validate the X.509 certificate against. */
  if (SSL_set1_host(qconn, bare_hostname) <= 0) goto err;

  /* Configure SNI */
  if (!SSL_set_tlsext_host_name(qconn, bare_hostname)) goto err;
...
static void h3_conn_pump_stream(OSSL_DEMO_H3_STREAM* s, void* conn_) {
  ...
  for (;;) {
    ...
    if (s->buf_cur == s->buf_total) {
      ...
      if (ec <= 0) {
        ...
        if (SSL_get_error(s->s, ec) == SSL_ERROR_ZERO_RETURN) {
          ...
          if (!SSL_get_stream_read_error_code(s->s, &aec)) goto err;
    ...
    if (s->buf_cur == s->buf_total) break;
