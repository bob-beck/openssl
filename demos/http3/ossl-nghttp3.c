...
static void
h3_stream_free(OSSL_DEMO_H3_STREAM* s)
{
  if (s == NULL)
    return;
...
h3_stream_hash(const OSSL_DEMO_H3_STREAM* s)
{
  return (unsigned long)s->id;
}

static int
h3_stream_eq(const OSSL_DEMO_H3_STREAM* a, const OSSL_DEMO_H3_STREAM* b)
{
  if (a->id < b->id)
    return -1;
  if (a->id > b->id)
...
void*
OSSL_DEMO_H3_STREAM_get_user_data(const OSSL_DEMO_H3_STREAM* s)
{
  return s->user_data;
}
...
void
OSSL_DEMO_H3_CONN_free(OSSL_DEMO_H3_CONN* conn)
{
  if (conn == NULL)
    return;
...
h3_conn_create_stream(OSSL_DEMO_H3_CONN* conn, int type)
{
...
static OSSL_DEMO_H3_STREAM*
h3_conn_accept_stream(OSSL_DEMO_H3_CONN* conn, SSL* qstream)
{
  OSSL_DEMO_H3_STREAM* s;
...
static void
h3_conn_remove_stream(OSSL_DEMO_H3_CONN* conn, OSSL_DEMO_H3_STREAM* s)
{
  if (s == NULL)
    return;
...
static int
h3_conn_recv_data(nghttp3_conn* h3conn,
                  int64_t stream_id,
                  const uint8_t* data,
                  size_t datalen,
                  void* conn_user_data,
                  void* stream_user_data)
{
  ...
  return conn->recv_data_cb(
    h3conn, stream_id, data, datalen, conn_user_data, stream_user_data);
}

static int
h3_conn_stream_close(nghttp3_conn* h3conn,
                     int64_t stream_id,
                     ...
                     void* stream_user_data)
{
  ...
  if (conn->stream_close_cb != NULL)
    ret = conn->stream_close_cb(
      h3conn, stream_id, app_error_code, conn_user_data, stream_user_data);
...
static int
h3_conn_stop_sending(nghttp3_conn* h3conn,
                     int64_t stream_id,
                     uint64_t app_error_code,
                     void* conn_user_data,
                     void* stream_user_data)
...
static int
h3_conn_reset_stream(nghttp3_conn* h3conn,
                     int64_t stream_id,
                     uint64_t app_error_code,
                     void* conn_user_data,
                     void* stream_user_data)
...
static int
h3_conn_deferred_consume(nghttp3_conn* h3conn,
                         int64_t stream_id,
                         size_t consumed,
                         void* conn_user_data,
                         void* stream_user_data)
{
  ...
  if (conn->deferred_consume_cb != NULL)
    ret = conn->deferred_consume_cb(
      h3conn, stream_id, consumed, conn_user_data, stream_user_data);
...
OSSL_DEMO_H3_CONN*
OSSL_DEMO_H3_CONN_new_for_conn(BIO* qconn_bio,
                               ...
                               void* user_data)
{
  ...
  if (BIO_get_ssl(qconn_bio, &conn->qconn) == 0) {
    ERR_raise_data(
      ERR_LIB_USER, ERR_R_PASSED_INVALID_ARGUMENT, "BIO must be an SSL BIO");
    goto err;
  }
  ...
  if (SSL_in_before(conn->qconn))
    if (SSL_set_alpn_protos(conn->qconn, alpn, sizeof(alpn))) {
      ...
      ERR_raise_data(
        ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "failed to configure ALPN");
      goto err;
    }
  ...
  ec = nghttp3_conn_client_new(
    &conn->h3conn, &intl_callbacks, settings, NULL, conn);
  if (ec < 0) {
    ERR_raise_data(ERR_LIB_USER,
                   ERR_R_INTERNAL_ERROR,
  ...
  ec = nghttp3_conn_bind_qpack_streams(
    conn->h3conn, s_qpenc_send->id, s_qpdec_send->id);
  if (ec < 0) {
    ERR_raise_data(ERR_LIB_USER,
                   ERR_R_INTERNAL_ERROR,
...
OSSL_DEMO_H3_CONN*
OSSL_DEMO_H3_CONN_new_for_addr(SSL_CTX* ctx,
                               const char* addr,
                               const nghttp3_callbacks* callbacks,
                               const nghttp3_settings* settings,
                               void* user_data)
...
int
OSSL_DEMO_H3_CONN_connect(OSSL_DEMO_H3_CONN* conn)
{
  return SSL_connect(OSSL_DEMO_H3_CONN_get0_connection(conn));
}

void*
OSSL_DEMO_H3_CONN_get_user_data(const OSSL_DEMO_H3_CONN* conn)
{
  return conn->user_data;
}

SSL*
OSSL_DEMO_H3_CONN_get0_connection(const OSSL_DEMO_H3_CONN* conn)
{
  return conn->qconn;
}
...
static void
h3_conn_pump_stream(OSSL_DEMO_H3_STREAM* s, void* conn_)
{
  int ec;
...
int
OSSL_DEMO_H3_CONN_handle_events(OSSL_DEMO_H3_CONN* conn)
{
  int ec, fin;
  ...
  for (;;) {
     ...
     */
    ec = nghttp3_conn_writev_stream(
      conn->h3conn, &stream_id, &fin, vecs, ARRAY_LEN(vecs));
    if (ec < 0)
      return 0;
    if (ec == 0)
      break;
    ...
    if ((s = lh_OSSL_DEMO_H3_STREAM_retrieve(conn->streams, &key)) == NULL) {
      ERR_raise_data(
        ERR_LIB_USER, ERR_R_INTERNAL_ERROR, "no stream for ID %zd", stream_id);
      return 0;
    }
    ...
    for (i = 0; i < num_vecs; ++i) {
      ...
      if (s->s == NULL) {
        ...
        written = vecs[i].len;
                   ...
                   s->s, vecs[i].base, vecs[i].len, flags, &written)) {
...
int
OSSL_DEMO_H3_CONN_submit_request(OSSL_DEMO_H3_CONN* conn,
