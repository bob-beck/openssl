...
static int are_all_clientid_closed(struct h3ssl* h3ssl) {
  ...
  ssl_ids = h3ssl->ssl_ids;
  for (i = 0; i < MAXSSL_IDS; i++) {
    if (ssl_ids[i].id == UINT64_MAX) continue;
...
static int quic_server_read(nghttp3_conn* h3conn, SSL* stream, uint64_t id,
  ...
  if (!SSL_has_pending(stream)) return 0; /* Nothing to read */
  ...
  if (r != ret) {
    ...
    if (!nghttp3_err_is_fatal(r)) {
      ...
      if (id == 2) h3ssl->received_from_two = 1;
      return 1;
    }
    return -1;
  }
...
static int read_from_ssl_ids(nghttp3_conn** curh3conn, struct h3ssl* h3ssl) {
  ...
  for (i = 0, item = items; i < numitem; i++, item++) {
    ...
    if (item->revents == SSL_POLL_EVENT_NONE) continue;
    processed_event = 0;
    /* get the stream */
  ...
  ret = hassomething;
err:
  if (has_ids_to_remove) remove_marked_ids(h3ssl);
  return ret;
}
...
static void handle_events_from_ids(struct h3ssl* h3ssl) {
  ...
  for (i = 0; i < MAXSSL_IDS; i++) {
    if (ssl_ids[i].s != NULL &&
      ...
      if (SSL_handle_events(ssl_ids[i].s)) ERR_print_errors_fp(stderr);
    }
  }
}
...
static size_t get_file_length(struct h3ssl* h3ssl) {
  ...
  if (h3ssl->fileprefix != NULL) strcat(filename, h3ssl->fileprefix);
...
static int wait_for_activity(SSL* ssl) {
   ...
   */
  if (SSL_net_write_desired(ssl)) FD_SET(sock, &write_fd);
  if (SSL_net_read_desired(ssl)) FD_SET(sock, &read_fd);
  ...
  restart:
    ...
    for (;;) {
      ...
      for (i = 0; i < sveccnt; i++) {
        ...
        if (fin && i == sveccnt - 1) flagwrite = SSL_WRITE_FLAG_CONCLUDE;
  ...
  wait_close:
    ...
    for (;;) {
      if (!hasnothing) {
        ...
        if (newssl == NULL) newssl = listener;
        ret = wait_for_activity(newssl);
        if (ret == -1) goto err;
        if (ret == 0) printf("hasnothing timeout\n");
...
int main(int argc, char** argv) {
  ...
  /* Create SSL_CTX. */
  if ((ctx = create_ctx(argv[2], argv[3])) == NULL) goto err;
  ...
  /* Create UDP socket. */
  if ((fd = create_socket((uint16_t)port)) < 0) goto err;

  /* Enter QUIC server connection acceptance loop. */
  if (!run_quic_server(ctx, fd)) goto err;
