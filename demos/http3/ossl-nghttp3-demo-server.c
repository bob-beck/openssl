...
static void add_id_status(uint64_t id,
                          SSL* ssl,
                          struct h3ssl* h3ssl,
                          int status) {
  struct ssl_id* ssl_ids;
...
static void replace_ids_connection(struct h3ssl* h3ssl,
                                   SSL* oldstream,
                                   SSL* newstream) {
  struct ssl_id* ssl_ids;
  int i;
...
static int on_recv_header(nghttp3_conn* conn,
                          int64_t stream_id,
                          int32_t token,
                          nghttp3_rcbuf* name,
                          nghttp3_rcbuf* value,
...
static int on_recv_data(nghttp3_conn* conn,
                        int64_t stream_id,
                        const uint8_t* data,
                        size_t datalen,
                        void* conn_user_data,
...
static int on_end_stream(nghttp3_conn* h3conn,
                         int64_t stream_id,
                         void* conn_user_data,
                         void* stream_user_data) {
...
static int quic_server_read(nghttp3_conn* h3conn,
                            SSL* stream,
                            uint64_t id,
                            struct h3ssl* h3ssl) {
  int ret, r;
...
static nghttp3_ssize step_read_data(nghttp3_conn* conn,
                                    int64_t stream_id,
                                    nghttp3_vec* vec,
                                    size_t veccnt,
                                    uint32_t* pflags,
...
static int quic_server_write(struct h3ssl* h3ssl,
                             uint64_t streamid,
                             uint8_t* buff,
                             size_t len,
                             uint64_t flags,
