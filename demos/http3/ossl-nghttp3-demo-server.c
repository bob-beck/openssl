...
static int
on_recv_header(nghttp3_conn* conn,
               int64_t stream_id,
               int32_t token,
               nghttp3_rcbuf* name,
               nghttp3_rcbuf* value,
...
static int
on_end_headers(nghttp3_conn* conn,
               int64_t stream_id,
               int fin,
               void* user_data,
               void* stream_user_data)
...
static int
on_recv_data(nghttp3_conn* conn,
             int64_t stream_id,
             const uint8_t* data,
             size_t datalen,
             void* conn_user_data,
...
static int
on_end_stream(nghttp3_conn* h3conn,
              int64_t stream_id,
              void* conn_user_data,
              void* stream_user_data)
{
...
static int
quic_server_read(nghttp3_conn* h3conn,
                 SSL* stream,
                 uint64_t id,
                 struct h3ssl* h3ssl)
{
  ...
  for (i = 0, item = items; i < numitem; i++, item++) {
    ...
    if (item->revents & SSL_POLL_EVENT_IC) {
      ...
      if (nghttp3_conn_server_new(
            curh3conn, &callbacks, &settings, mem, h3ssl)) {
...
step_read_data(nghttp3_conn* conn,
               int64_t stream_id,
               nghttp3_vec* vec,
               size_t veccnt,
               uint32_t* pflags,
...
static int
quic_server_write(struct h3ssl* h3ssl,
                  uint64_t streamid,
                  uint8_t* buff,
                  size_t len,
                  uint64_t flags,
...
run_quic_server(SSL_CTX* ctx, int fd)
{
  ...
  restart:
    ...
    if (nghttp3_conn_submit_response(
          h3conn, h3ssl.id_bidi, resp, num_nv, &dr)) {
    ...
    for (;;) {
      ...
      sveccnt = nghttp3_conn_writev_stream(
        h3conn, &streamid, &fin, vec, nghttp3_arraylen(vec));
      if (sveccnt <= 0) {
      ...
      printf(
        "nghttp3_conn_writev_stream: %ld fin: %d\n", (long int)sveccnt, fin);
      for (i = 0; i < sveccnt; i++) {
