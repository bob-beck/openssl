...
static int aria_cbc_encrypt(void) {
  ...
  if (!EVP_EncryptFinal_ex(ctx, outbuf + outlen, &tmplen)) goto err;
  outlen += tmplen;
...
static int aria_cbc_decrypt(void) {
  ...
  /* Finalise: there can be some additional output from padding */
  if (!EVP_DecryptFinal_ex(ctx, outbuf + outlen, &tmplen)) goto err;
  outlen += tmplen;
...
int main(int argc, char** argv) {
  if (!aria_cbc_encrypt()) return EXIT_FAILURE;

  if (!aria_cbc_decrypt()) return EXIT_FAILURE;
