...
static int aes_gcm_encrypt(void) {
   ...
   * would be NULL.
   */
  if (!EVP_EncryptInit_ex2(ctx, cipher, gcm_key, gcm_iv, params)) goto err;
  ...
  if (!EVP_EncryptFinal_ex(ctx, outbuf, &tmplen)) goto err;
...
static int aes_gcm_decrypt(void) {
   ...
   * IV length parameter.
   */
  if (!EVP_DecryptInit_ex2(ctx, cipher, gcm_key, gcm_iv, params)) goto err;
  ...
  if (!EVP_CIPHER_CTX_set_params(ctx, params)) goto err;
...
int main(int argc, char** argv) {
  if (!aes_gcm_encrypt()) return EXIT_FAILURE;

  if (!aes_gcm_decrypt()) return EXIT_FAILURE;
