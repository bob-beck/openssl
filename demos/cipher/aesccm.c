...
static int aes_ccm_encrypt(void) {
   ...
   */
  if (!EVP_EncryptInit_ex2(ctx, cipher, NULL, NULL, params)) goto err;

  /* Initialise key and nonce */
  if (!EVP_EncryptInit_ex(ctx, NULL, NULL, ccm_key, ccm_nonce)) goto err;

  /* Set plaintext length: only needed if AAD is used */
  if (!EVP_EncryptUpdate(ctx, NULL, &outlen, NULL, sizeof(ccm_pt))) goto err;
  ...
  if (!EVP_EncryptFinal_ex(ctx, NULL, &tmplen)) goto err;
...
static int aes_ccm_decrypt(void) {
   ...
   */
  if (!EVP_DecryptInit_ex2(ctx, cipher, NULL, NULL, params)) goto err;

  /* Specify key and IV */
  if (!EVP_DecryptInit_ex(ctx, NULL, NULL, ccm_key, ccm_nonce)) goto err;

  /* Set ciphertext length: only needed if we have AAD */
  if (!EVP_DecryptUpdate(ctx, NULL, &outlen, NULL, sizeof(ccm_ct))) goto err;
...
int main(int argc, char** argv) {
  if (!aes_ccm_encrypt()) return EXIT_FAILURE;

  if (!aes_ccm_decrypt()) return EXIT_FAILURE;
