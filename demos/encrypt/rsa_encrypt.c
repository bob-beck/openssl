...
get_key(OSSL_LIB_CTX* libctx, const char* propq, int public)
{
  ...
  dctx = OSSL_DECODER_CTX_new_for_pkey(
    &pkey, "DER", NULL, "RSA", selection, libctx, propq);
 ...
 */
static int
do_encrypt(OSSL_LIB_CTX* libctx,
...
static int
do_decrypt(OSSL_LIB_CTX* libctx,
           const unsigned char* in,
           size_t in_len,
           unsigned char** out,
           size_t* out_len)
...
main(void)
{
  ...
  if (!do_decrypt(
        libctx, encrypted, encrypted_len, &decrypted, &decrypted_len)) {
