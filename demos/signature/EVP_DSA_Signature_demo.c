...
static int generate_dsa_params(OSSL_LIB_CTX* libctx, EVP_PKEY** p_params) {
  ...
  if (pkey_ctx == NULL) goto end;

  if (EVP_PKEY_paramgen_init(pkey_ctx) <= 0) goto end;

  if (EVP_PKEY_CTX_set_dsa_paramgen_bits(pkey_ctx, NUMBITS) <= 0) goto end;
  if (EVP_PKEY_paramgen(pkey_ctx, &params) <= 0) goto end;
  if (params == NULL) goto end;
...
static int generate_dsa_key(OSSL_LIB_CTX* libctx, EVP_PKEY* params,
  ...
  if (EVP_PKEY_keygen(ctx, &pkey) <= 0) goto end;
...
static int extract_public_key(const EVP_PKEY* pkey, OSSL_PARAM** p_public_key) {
  ...
  if (EVP_PKEY_todata(pkey, EVP_PKEY_PUBLIC_KEY, &public_key) != 1) goto end;
...
static int extract_keypair(const EVP_PKEY* pkey, OSSL_PARAM** p_keypair) {
  ...
  if (EVP_PKEY_todata(pkey, EVP_PKEY_KEYPAIR, &keypair) != 1) goto end;
...
static int demo_sign(OSSL_LIB_CTX* libctx, size_t* p_sig_len,
  ...
  pkey_ctx = EVP_PKEY_CTX_new_from_name(libctx, ALG, PROPQUERY);
  if (pkey_ctx == NULL) goto end;
  if (EVP_PKEY_fromdata_init(pkey_ctx) != 1) goto end;
  ...
  if (EVP_DigestSignUpdate(ctx, hamlet_1, sizeof(hamlet_1)) != 1) goto end;

  if (EVP_DigestSignUpdate(ctx, hamlet_2, sizeof(hamlet_2)) != 1) goto end;

  /* Calculate the signature size */
  if (EVP_DigestSignFinal(ctx, NULL, &sig_len) != 1) goto end;
  if (sig_len == 0) goto end;

  sig_value = OPENSSL_malloc(sig_len);
  if (sig_value == NULL) goto end;

  /* Calculate the signature */
  if (EVP_DigestSignFinal(ctx, sig_value, &sig_len) != 1) goto end;
...
static int demo_verify(OSSL_LIB_CTX* libctx, size_t sig_len,
  ...
  if (EVP_DigestVerifyUpdate(ctx, hamlet_1, sizeof(hamlet_1)) != 1) goto end;

  if (EVP_DigestVerifyUpdate(ctx, hamlet_2, sizeof(hamlet_2)) != 1) goto end;

  if (EVP_DigestVerifyFinal(ctx, sig_value, sig_len) != 1) goto end;
...
int main(void) {
  ...
  if (libctx == NULL) goto end;

  if (generate_dsa_params(libctx, &params) != 1) goto end;

  if (generate_dsa_key(libctx, params, &pkey) != 1) goto end;

  if (extract_public_key(pkey, &public_key) != 1) goto end;

  if (extract_keypair(pkey, &keypair) != 1) goto end;

  /* The signer signs with his private key, and distributes his public key */
  if (demo_sign(libctx, &sig_len, &sig_value, keypair) != 1) goto end;

  /* A verifier uses the signers public key to verify the signature */
  if (demo_verify(libctx, sig_len, sig_value, public_key) != 1) goto end;

  ret = EXIT_SUCCESS;
end:
  if (ret != EXIT_SUCCESS) ERR_print_errors_fp(stderr);
