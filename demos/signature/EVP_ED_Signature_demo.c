          ...
          size_t* sig_out_len)
{
  ...
  if (!EVP_DigestSignInit_ex(
        sign_context, NULL, NULL, libctx, NULL, priv, NULL)) {
...
static int
demo_verify(EVP_PKEY* pub,
            const unsigned char* tbs,
            size_t tbs_len,
            const unsigned char* sig_value,
            size_t sig_len,
            OSSL_LIB_CTX* libctx)
{
  int ret = 0;
  ...
  if (!EVP_DigestVerifyInit_ex(
        verify_context, NULL, NULL, libctx, NULL, pub, NULL)) {
...
create_key(OSSL_LIB_CTX* libctx, EVP_PKEY** privout, EVP_PKEY** pubout)
{
  ...
  pub = EVP_PKEY_new_raw_public_key_ex(
    libctx, "ED25519", NULL, pubdata, pubdata_len);
  if (pub == NULL) {
