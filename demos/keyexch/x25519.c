...
/*
 * Prepare for X25519 key exchange. The public key to be sent to the remote
 * peer is put in pubk_data, which should be a 32-byte buffer. Returns 1 on
 * success.
 */
static int
keyexch_x25519_before (OSSL_LIB_CTX *libctx,
 ...
 * public key value received from the remote peer. On success, returns 1 and
 * the secret is pointed to by *secret. The caller must free it.
 */
static int
keyexch_x25519_after (OSSL_LIB_CTX *libctx, int use_kat, PEER_DATA *local_peer,
                      const unsigned char *remote_peer_pubk_data)
{
  ...
  BIO_dump_indent_fp (stdout, local_peer->secret, (int)local_peer->secret_len,
                      2);
  putchar ('\n');
