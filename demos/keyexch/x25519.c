 ...
 */
static int
keyexch_x25519_after(OSSL_LIB_CTX* libctx,
                     int use_kat,
                     PEER_DATA* local_peer,
                     const unsigned char* remote_peer_pubk_data)
{
  ...
  BIO_dump_indent_fp(
    stdout, local_peer->secret, (int)local_peer->secret_len, 2);
  putchar('\n');
...
keyexch_x25519(int use_kat)
{
   ...
   * later use.
   */
  if (keyexch_x25519_before(
        libctx, use_kat ? peer1_privk_data : NULL, &peer1) == 0)
    return 0;

  if (keyexch_x25519_before(
        libctx, use_kat ? peer2_privk_data : NULL, &peer2) == 0)
    return 0;
