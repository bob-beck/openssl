...
static int generate_secret(PEER_DATA* peerA, EVP_PKEY* peerBpub,
  ...
  if (derivectx == NULL) return 0;

  if (EVP_PKEY_derive_init(derivectx) <= 0) goto cleanup;
  /* Set up peerB's public key */
  if (EVP_PKEY_derive_set_peer(derivectx, peerBpub) <= 0) goto cleanup;
  ...
  if (EVP_PKEY_derive(derivectx, NULL, &secretlen) <= 0) goto cleanup;
   ...
   */
  if (EVP_PKEY_derive(derivectx, secret, &secretlen) <= 0) goto cleanup;
  peerA->secret = secret;
