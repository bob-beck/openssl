 ...
 */
static int
get_peer_public_key(PEER_DATA* peer, OSSL_LIB_CTX* libctx)
{
  int ret = 0;
  EVP_PKEY_CTX* ctx;
  OSSL_PARAM params[3];
  ...
  if (!EVP_PKEY_get_octet_string_param(peer->priv,
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
    OSSL_PKEY_PARAM_GROUP_NAME, (char*)peer->curvename, 0);
  params[1] = OSSL_PARAM_construct_octet_string(
    OSSL_PKEY_PARAM_PUB_KEY, pubkeydata, pubkeylen);
...
static int
create_peer(PEER_DATA* peer, OSSL_LIB_CTX* libctx)
{
  int ret = 0;
...
static int
generate_secret(PEER_DATA* peerA, EVP_PKEY* peerBpub, OSSL_LIB_CTX* libctx)
{
   ...
   * 0); params[1] =
   * OSSL_PARAM_construct_utf8_string(OSSL_EXCHANGE_PARAM_KDF_DIGEST, "SHA256",
   * 0); params[2] = OSSL_PARAM_construct_size_t(OSSL_EXCHANGE_PARAM_KDF_OUTLEN,
   *                                           &outlen);
   *   params[3] =
