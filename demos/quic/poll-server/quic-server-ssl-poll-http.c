...
static void
destroy_pe(struct poll_event*);
static int
pe_return_error(struct poll_event*);
static void
pe_return_void(struct poll_event*);
...
static unsigned int
rb_txt_simple_read_cb(struct response_buffer* rb,
                      char* buf,
                      unsigned int buf_sz)
{
  struct response_txt_simple* rts = rb_to_txt_simple(rb);
 ...
 */
static int
request_new_stream(struct poll_event_connection* pec,
                   uint64_t qsflag,
                   void* peccx_arg)
{
...
app_write_cb(struct poll_event* pe)
{
  ...
  if (wlen == 0) {
    DPRINTF(stderr,
            "%s no more data to write to %p (%s)\n",
            __func__,
            pe,
            pe_type_to_name(pe));
...
app_setup_response(struct poll_event_stream* pes)
{
  ...
  switch (pe->pe_type) {
    case PE_STREAM_UNI_IN:
      rv =
        request_new_stream(pes->pes_conn, SSL_STREAM_FLAG_UNI, pe->pe_appdata);
      break;
    case PE_STREAM:
...
app_read_cb(struct poll_event* pe)
{
  ...
  rv = SSL_read_ex(
    get_ssl_from_pe(pe), pes->pes_wpos, pes->pes_wpos_sz, &read_len);
  if (rv == 0) {
    pe_disable_read(pe);
    /*
