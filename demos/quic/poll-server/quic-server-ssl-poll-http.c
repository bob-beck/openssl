 ...
 * However SSL_poll(9ossl) expects an array of SSL_POLL_ITEM structures.
 * Therefore the poll manager needs to construct array of poll_event structures
 * from list. We get two copies of poll_event structures:
...
#define SSL_POLL_ERROR                                        \
  (SSL_POLL_EVENT_F | SSL_POLL_EVENT_EL | SSL_POLL_EVENT_EC | \
   SSL_POLL_EVENT_ECD | SSL_POLL_EVENT_ER | SSL_POLL_EVENT_EW)

#define SSL_POLL_IN                                            \
  (SSL_POLL_EVENT_R | SSL_POLL_EVENT_IC | SSL_POLL_EVENT_ISB | \
   SSL_POLL_EVENT_ISU)

#define SSL_POLL_OUT \
  (SSL_POLL_EVENT_W | SSL_POLL_EVENT_OSB | SSL_POLL_EVENT_OSU)
...
#define response_buffer_base                                                \
  ...
  unsigned int (*rb_read_cb)(struct response_buffer*, char*, unsigned int); \
 ...
 */
static char* basename(char* path) { return path; }
...
static struct response_txt_simple* rb_to_txt_simple(
    struct response_buffer* rb) {
  if (rb == NULL || rb->rb_type != RB_TYPE_TEXT_SIMPLE) return NULL;

  return (struct response_txt_simple*)rb;
}

static struct response_txt_full* rb_to_txt_full(struct response_buffer* rb) {
  if (rb == NULL || rb->rb_type != RB_TYPE_TEXT_FULL) return NULL;
...
static void rb_ondestroy_cb(struct response_buffer* rb) { OPENSSL_free(rb); }
...
static int rb_eof_cb(struct response_buffer* rb) { return 1; }
...
static void rb_advrpos(struct response_buffer* rb, unsigned int rpos) {
  if (rb != NULL) rb->rb_advrpos_cb(rb, rpos);
}
...
static unsigned int rb_txt_simple_read_cb(struct response_buffer* rb, char* buf,
  ...
  if (rts == NULL || rb_eof(rb)) return 0;
...
static void rb_txt_simple_advrpos_cb(struct response_buffer* rb,
  ...
  if (rts != NULL) {
    rb->rb_rpos += sz;
    if (rb->rb_rpos >= rts->rts_len) rb->rb_rpos = rts->rts_len;
  }
}
...
static unsigned int rb_txt_full_read_cb(struct response_buffer* rb, char* buf,
  ...
  if (rtf == NULL || rb_eof(rb)) return 0;
...
static void rb_txt_full_advrpos_cb(struct response_buffer* rb,
                                   unsigned int sz) {
  struct response_txt_full* rtf = rb_to_txt_full(rb);

  if (rtf != NULL) {
    rb->rb_rpos += sz;
    if (rb->rb_rpos >= rtf->rtf_len) rb->rb_rpos = rtf->rtf_len;
  }
}

static struct response_txt_full* new_txt_full_respoonse(
    const char* fill_pattern, unsigned int fsize) {
...
static ossl_unused const char* pe_type_to_name(const struct poll_event* pe) {
  static const char* names[] = {"none",          "listener",    "connection",
                                "stream (bidi)", "stream (in)", "stream (out)",
                                "invalid"};

  if (pe->pe_type >= PE_INVALID) return (names[PE_INVALID]);

  return names[pe->pe_type];
}

static struct poll_event_connection* pe_to_connection(struct poll_event* pe) {
  if ((pe == NULL) || (pe->pe_type != PE_CONNECTION)) return NULL;
...
static SSL* get_ssl_from_pe(struct poll_event* pe) {
  ...
  if (pe != NULL) ssl = pe->pe_poll_item.desc.value.ssl;
...
static struct poll_manager* create_poll_manager(void) {
  ...
  pm->pm_poll_set = OPENSSL_malloc_array(POLL_GROW, sizeof(struct poll_event));
...
static int rebuild_poll_set(struct poll_manager* pm) {
  ...
  if (pm->pm_need_rebuild == 0) return 0;
  ...
  if (pe_num > pm->pm_poll_set_sz) {
     ...
     */
    new_poll_set =
        OPENSSL_realloc_array(pm->pm_poll_set, pm->pm_poll_set_sz + POLL_GROW,
                              sizeof(struct poll_event));
    if (new_poll_set == NULL) return -1;
...
static void destroy_pe(struct poll_event* pe) {
  ...
  if (pe->pe_my_pm) remove_pe_from_pm(pe->pe_my_pm, pe);
...
static int pe_return_error(struct poll_event* pe) { return -1; }

static void pe_return_void(struct poll_event* ctx) { return; }

static int pe_handle_listener_error(struct poll_event* pe) {
  pe->pe_my_pm->pm_continue = 0;
  if (pe->pe_poll_item.revents & SSL_POLL_EVENT_EL) return -1;
...
static void* get_response_from_pec(struct poll_event_connection* pec,
                                   int stype) {
  struct poll_event_context* peccx;
  void* rv;
...
static unsigned int get_fsize(const char* file_name) {
  ...
  if (fsize == 0) fsize = 12345; /* ? may be random ? */
...
static int parse_request(struct poll_event_stream* pes) {
  ...
  /* got request already */
  if (pe->pe_appdata != NULL) return -1;
  ...
  if (*pos != '/') return -1; /* this will reset the stream */
  ...
  *dst = '\0';
  ...
  if (file_name_buf[0] == '\0') {
     ...
     */
    if (file_name == NULL || *file_name == '\0') file_name = "foo";
  }
...
static int app_read_cb(struct poll_event* pe) {
  ...
  if (rv == 0) {
     ...
     */
    rv = handle_ssl_error(pe, rv, __func__);
    if (rv == 0) rv = handle_read_stream_state(pe);
    return rv;
  }
...
static int run_quic_server(SSL_CTX* ctx, struct poll_manager* pm, int fd) {
  ...
  if ((listener = SSL_new_listener(ctx, 0)) == NULL) goto err;

  if (!SSL_set_fd(listener, fd)) goto err;

  /*
   * Set the listener mode to non-blocking, which is inherited by
   * child objects.
   */
  if (!SSL_set_blocking_mode(listener, 0)) goto err;
   ...
   */
  if (!SSL_listen(listener)) goto err;

  listener_pe = new_listener_pe(listener);
  if (listener_pe == NULL) goto err;
  ...
  while (pm->pm_continue) {
    ...
    if (ok == 0 && poll_items == 0) break;

    for (i = 0; i < pm->pm_event_count; i++) {
      pe = &pm->pm_poll_set[i];
      if (pe->pe_poll_item.revents == 0) continue;
      DPRINTF(stderr, "%s %s (%p) " POLL_FMT "\n", __func__,
              pe_type_to_name(pe), pe, POLL_PRINTA(pe->pe_poll_item.revents));
