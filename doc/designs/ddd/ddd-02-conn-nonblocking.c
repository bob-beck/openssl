...
int get_conn_fd(APP_CONN* conn) {
  ...
  if (!BIO_get_rpoll_descriptor(conn->ssl_bio, &d)) return -1;
...
int get_conn_pump_timeout(APP_CONN* conn) {
  ...
  if (!SSL_get_event_timeout(conn->ssl, &tv, &is_infinite)) return -1;
...
int main(int argc, char** argv) {
  ...
  while (tx_len != 0) {
    ...
    if (l > 0) {
      ...
      if (t.tv_sec < 0 || timercmp(&t, &timeout, >)) t = timeout;
