...
static void dequeue_upper_write_op(APP_CONN* conn) {
  if (conn->pending_upper_write_head == NULL) return;
...
static void net_read_alloc(uv_handle_t* handle, size_t suggested_size,
                           uv_buf_t* buf) {
#ifdef USE_QUIC
  if (suggested_size < 1472) suggested_size = 1472;
#endif
...
static void net_error(APP_CONN* conn) {
  ...
  if (conn->app_read_cb) conn->app_read_cb(conn, NULL, 0, conn->app_read_arg);
}
...
static void set_timer(APP_CONN* conn) {
  ...
  if (!SSL_get_event_timeout(conn->ssl, &tv, &is_infinite)) return;

  ms = is_infinite ? -1 : timeval_to_ms(&tv);
  if (ms > 0) uv_timer_start(&conn->timer, timer_done, ms, 0);
}
...
static int try_write(APP_CONN* conn, UPPER_WRITE_OP* op) {
  ...
  while (written < op->buf_len) {
    ...
    if (rc <= 0) {
      ...
      if (rcx == SSL_ERROR_WANT_READ) {
        ...
        if (op->cb != NULL) op->cb(conn, -rcx, op->cb_arg);
  ...
  if (op->cb != NULL) op->cb(conn, 0, op->cb_arg);
