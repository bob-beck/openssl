...
APP_CONN *
new_conn (SSL_CTX *ctx, const char *hostname, struct sockaddr *sa,
 ...
 */
int
app_read_start (APP_CONN *conn, app_read_cb *cb, void *arg)
{
  conn->app_read_cb = cb;
 ...
 */
int
app_write (APP_CONN *conn, const void *buf, size_t buf_len, app_write_cb *cb,
           void *arg)
{
 ...
 */
static void
enqueue_upper_write_op (APP_CONN *conn, UPPER_WRITE_OP *op)
{
...
static void
net_read_alloc (uv_handle_t *handle, size_t suggested_size, uv_buf_t *buf)
{
#ifdef USE_QUIC
...
static void
handle_pending_writes (APP_CONN *conn)
{
  int rc;
...
#ifdef USE_QUIC
static void
net_read_done (uv_udp_t *stream, ssize_t nr, const uv_buf_t *buf,
               const struct sockaddr *addr, unsigned int flags)
#else
static void
net_read_done (uv_stream_t *stream, ssize_t nr, const uv_buf_t *buf)
#endif
{
  int rc;
...
#ifdef USE_QUIC
static void
net_write_done (uv_udp_send_t *req, int status)
#else
static void
net_write_done (uv_write_t *req, int status)
#endif
{
...
static void
handshake_done_ssl (APP_CONN *conn)
{
#ifdef USE_QUIC
  set_timer (conn);
#endif
...
static int
setup_ssl (APP_CONN *conn, const char *hostname)
{
...
#ifndef USE_QUIC
static void
tcp_connect_done (uv_connect_t *tcp_connect, int status)
{
  int rc;
...
static void
net_connect_fail_close_done (uv_handle_t *handle)
{
...
static int
try_write (APP_CONN *conn, UPPER_WRITE_OP *op)
{
  int rc, rcx;
...
static int
write_deferred (APP_CONN *conn, const void *buf, size_t buf_len,
                app_write_cb *cb, void *arg)
{
...
static void
teardown_continued (uv_handle_t *handle)
{
  APP_CONN *conn = (APP_CONN *)handle->data;
 ...
 */
static void
post_read (APP_CONN *conn, void *buf, size_t buf_len, void *arg)
{
  if (!buf_len)
    {
      free (buf);
...
static void
post_write_get (APP_CONN *conn, int status, void *arg)
{
  if (status < 0)
    {
...
static void
post_connect (APP_CONN *conn, int status, void *arg)
{
  int wr;
