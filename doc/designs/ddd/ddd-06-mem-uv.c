...
main(int argc, char** argv)
{
  ...
  mlen = snprintf(
    tx_msg, sizeof(tx_msg), "GET / HTTP/1.0\r\nHost: %s\r\n\r\n", argv[1]);
  ...
  conn = new_conn(
    ctx, argv[1], result->ai_addr, result->ai_addrlen, post_connect, NULL);
  if (!conn)
    goto fail;
