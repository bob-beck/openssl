 ...
 * asynchronous, nonblocking fashion. The client is responsible for creating
 * the socket and passing it to libssl. The functions show all interactions
 * with libssl the application makes, and would hypothetically be linked into a
 * larger application.
 */
 ...
 *              if application does not want to read yet, it should call
 * pump().
 *
