...
static int pump(APP_CONN* conn, int fd, int events, int timeout) {
  ...
  if (net_rx_space(conn) == 0) pfd.events &= ~POLLIN;
  if (net_tx_avail(conn) > 0) pfd.events |= POLLOUT;

  if ((pfd.events & (POLLIN | POLLOUT)) == 0) return 1;
  ...
  if (pfd.revents & POLLIN) {
    while ((wspace = net_rx_space(conn)) > 0) {
      ...
      if (l2 < l) fprintf(stderr, "short write %d %d\n", l2, l);
    }
  stop:;
  }
  ...
  if (pfd.revents & POLLOUT) {
    for (;;) {
      ...
      if (l <= 0) break;
      l2 = write(fd, buf, l);
      if (l2 < l) fprintf(stderr, "short read %d %d\n", l2, l);
    }
  }
...
int main(int argc, char** argv) {
  ...
  res = 0;
fail:
  if (conn != NULL) teardown(conn);
  if (ctx != NULL) teardown_ctx(ctx);
