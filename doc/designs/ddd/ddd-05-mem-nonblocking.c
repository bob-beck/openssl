 ...
 * object on the decrypted side but also when encrypted data from the network
 * is shunted to/from OpenSSL. In this way OpenSSL is used as a pure state
 * machine which does not make its own network I/O calls. OpenSSL never sees or
 * creates any file descriptor for a network socket. The functions below show
 * all interactions with libssl the application makes, and would hypothetically
 * be linked into a larger application.
 */
 ...
 */
APP_CONN *
new_conn (SSL_CTX *ctx, const char *bare_hostname)
{
 ...
 */
int
read_net_tx (APP_CONN *conn, void *buf, int buf_len)
{
 ...
 *
 * QUIC: buf must contain the entirety of a single datagram. It will be
 * consumed entirely (return value == buf_len) or not at all.
 */
int
write_net_rx (APP_CONN *conn, const void *buf, int buf_len)
{
...
static int
pump (APP_CONN *conn, int fd, int events, int timeout)
{
  int l, l2;
