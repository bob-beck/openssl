...
=for openssl generic
 ...
 int sk_TYPE_find_sorted(STACK_OF(TYPE) *sk, TYPE *ptr);
...
B<sk_I<TYPE>_find_sorted() operates like B<sk_I<TYPE>_find>() but I<sk>
is required to be sorted. In the case where a comparison function
has been specified, if I<sk> is not sorted B<-1> will be returned.
