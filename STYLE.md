...
The basic style for indenting and whitespace is enforced by
clang-format using the .clang-format file in this directory. Your
changes should be formatted with clang-format and whatever indentation
and line wrapping clang-format does to them should be viewed as
correct.
...
Place comments above or to the right of the code they refer to. Comments
referring to the code line after should be indented equally to that code line.
...
an indication of whether the computation succeeded, are not subject to these
rules.

When constrained by legacy code, you should follow the existing API's convention
for return values. Be certain you are aware of what this is when modifying such code.
...
```
    /**
     * SNUFFLE_thingamabob snuffles a thingamabob value between
     * 0 and 60000 from |input| of |input_len| bytes.
     * If a valid thingamabob is successfully snuffled, the result
     * is stored in |out_thingamabob| and 1 is returned. On error or
     * failure 0 is returned.
     */
    int SNUFFLE_thingamabob(uint8_t *input, size_t input_len,
      uint16_t *out_thingamabob);
```
...
include their prerequisites, and that should compile all on their own.
...
header guards and mith not be self contained. Name such files with the
.inc extension, Use them sparingly and always prefer self contained
...
can put conditional includes after other includes as necessary as long
as such conditional code is kept small and localized.
