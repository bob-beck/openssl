 ...
 *
 *   - The reactor can export these FDs to the user, as well as flags
 * indicating whether the user should listen for readability, writeability, or
 * neither.
 *
 ...
 *
 * The intention is that ALL I/O-related SSL_* functions with side effects
 * (e.g. SSL_read/SSL_write) consist of three phases:
 *
 ...
 * immediately, though not strictly needed if transmission is being deferred
 * due to Nagle's algorithm, etc.).
 *
 * SSL_read is also a mutation and in principle does not need to tick the
 * reactor, but it generally will anyway to ensure that the reactor is
 * regularly ticked by an application which is only reading and not writing.
 *
 ...
 * ossl_quic_reactor_(enter/leave)_blocking_section() as this function also
 * uses those functions (see below); it is assumed if a caller is using those
 * functions it is implementing blocking semantics itself. There is no need to
 * use those functions if using this function.
 *
 * Precondition:   If a reactor mutex is being used, it must be held
 * (unchecked) Postcondition:  If a reactor mutex is being used, it is held
 ...
 * calls which have yet to be matched with a subsequent leave call) must *at
 * all times* equal the number of threads blocking on the reactor. In other
 * words, a single thread is not permitted to use these functions
 * "recursively". Failing to adhere to this rule will result in deadlock.
 *
