...
/*
 * Peek ahead and initialize |subpkt| with the next |len| bytes read from
 * |pkt|. Data is not copied: the |subpkt| packet will share its underlying
...
/**
 * @brief Get 4 bytes in network order from |pkt| and store the value in
 * |*data| Similar to PACKET_get_net_4() except the data is uint32_t
 *
...
PACKET_get_net_4_len_u32 (PACKET *pkt, uint32_t *data)
{
  size_t i = 0;
...
/*
 * Peek ahead at 4 bytes in reverse network order from |pkt| and store the
 * value in |*data|
 */
__owur static ossl_inline int
 ...
 * not free this data directly (it will be freed when the underlying buffer
 * gets freed
 */
__owur static ossl_inline int
 ...
 */
__owur static ossl_inline int
PACKET_copy_all (const PACKET *pkt, unsigned char *dest, size_t dest_len,
                 size_t *len)
{
 ...
 * Data is not copied: the |subpkt| packet will share its underlying buffer
 * with the original |pkt|, so data wrapped by |pkt| must outlive the |subpkt|.
 ...
 * for the whole time that the WPACKET is being used. Additionally |lenbytes|
 * of data is preallocated at the start of the buffer to store the length of
 * the WPACKET once we know it.
 */
 ...
 * maximum size will be. If this function is used, then it should be
 * immediately followed by a WPACKET_allocate_bytes() call before any other
 * WPACKET functions are called (unless the write to the allocated bytes is
 * abandoned).
 *
