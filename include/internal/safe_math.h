...
#define OSSL_SAFE_MATH_ADDS(type_name, type, min, max)                                                                 \
    static ossl_inline ossl_unused type safe_add_##type_name(type a, type b, int *err)                                 \
    {                                                                                                                  \
        type r;                                                                                                        \
                                                                                                                       \
...
#define OSSL_SAFE_MATH_ADDS(type_name, type, min, max)                                                                 \
    static ossl_inline ossl_unused type safe_add_##type_name(type a, type b, int *err)                                 \
    {                                                                                                                  \
        if ((a < 0) ^ (b < 0) || (a > 0 && b <= max - a) || (a < 0 && b >= min - a) || a == 0)                         \
            return a + b;                                                                                              \
        *err |= 1;                                                                                                     \
        return a < 0 ? min : max;                                                                                      \
    }
...
#define OSSL_SAFE_MATH_SUBS(type_name, type, min, max)                                                                 \
    static ossl_inline ossl_unused type safe_sub_##type_name(type a, type b, int *err)                                 \
    {                                                                                                                  \
        type r;                                                                                                        \
                                                                                                                       \
...
#define OSSL_SAFE_MATH_SUBS(type_name, type, min, max)                                                                 \
    static ossl_inline ossl_unused type safe_sub_##type_name(type a, type b, int *err)                                 \
    {                                                                                                                  \
        if (!((a < 0) ^ (b < 0)) || (b > 0 && a >= min + b) || (b < 0 && a <= max + b) || b == 0)                      \
            return a - b;                                                                                              \
        *err |= 1;                                                                                                     \
        return a < 0 ? min : max;                                                                                      \
    }
...
#define OSSL_SAFE_MATH_MULS(type_name, type, min, max)                                                                 \
    static ossl_inline ossl_unused type safe_mul_##type_name(type a, type b, int *err)                                 \
    {                                                                                                                  \
        type r;                                                                                                        \
                                                                                                                       \
...
#define OSSL_SAFE_MATH_DIVS(type_name, type, min, max)                                                                 \
    static ossl_inline ossl_unused type safe_div_##type_name(type a, type b, int *err)                                 \
    {                                                                                                                  \
        if (b == 0)                                                                                                    \
        {                                                                                                              \
            *err |= 1;                                                                                                 \
...
#define OSSL_SAFE_MATH_MODS(type_name, type, min, max)                                                                 \
    static ossl_inline ossl_unused type safe_mod_##type_name(type a, type b, int *err)                                 \
    {                                                                                                                  \
        if (b == 0)                                                                                                    \
        {                                                                                                              \
            *err |= 1;                                                                                                 \
...
#define OSSL_SAFE_MATH_MULDIVS(type_name, type, max)                                                                   \
    static ossl_inline ossl_unused type safe_muldiv_##type_name(type a, type b, type c, int *err)                      \
    {                                                                                                                  \
        int e2 = 0;                                                                                                    \
        type q, r, x, y;                                                                                               \
                                                                                                                       \
...
#define OSSL_SAFE_MATH_SIGNED(type_name, type)                                                                         \
    OSSL_SAFE_MATH_ADDS(type_name, type, OSSL_SAFE_MATH_MINS(type), OSSL_SAFE_MATH_MAXS(type))                         \
    OSSL_SAFE_MATH_SUBS(type_name, type, OSSL_SAFE_MATH_MINS(type), OSSL_SAFE_MATH_MAXS(type))                         \
    OSSL_SAFE_MATH_MULS(type_name, type, OSSL_SAFE_MATH_MINS(type), OSSL_SAFE_MATH_MAXS(type))                         \
    OSSL_SAFE_MATH_DIVS(type_name, type, OSSL_SAFE_MATH_MINS(type), OSSL_SAFE_MATH_MAXS(type))                         \
    OSSL_SAFE_MATH_MODS(type_name, type, OSSL_SAFE_MATH_MINS(type), OSSL_SAFE_MATH_MAXS(type))                         \
