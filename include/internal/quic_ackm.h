...
void ossl_ackm_set_ack_deadline_callback (
    OSSL_ACKM *ackm, void (*fn) (OSSL_TIME deadline, int pkt_space, void *arg),
    void *arg);
 ...
 * we are allowed to delay sending an ACK frame after receiving an
 * ACK-eliciting packet. Note that this cannot be changed after a connection is
 * established as it must be accurately reported in the transport parameters we
 * send to our peer.
 */
 ...
 * applicable. If the deadline has already passed, this function may return
 * that deadline, or may return OSSL_TIME_ZERO.
 */
 ...
 * whether or not this returns 1, so it is suggested that you call this
 * function first to determine whether you need to generate an ACK frame.
 *
 ...
 *
 *   - written off, meaning that the PN is so old we have stopped tracking
 * state for it (meaning that we cannot tell whether it is a duplicate and
 * cannot process it safely).
 *
 * This should be called for a packet before attempting to process its
 * contents. Failure to do so may result in processing a duplicated packet in
 * violation of the RFC.
 *
 * The return value of this function transitions from 1 to 0 for a given PN
 * once that PN is passed to ossl_ackm_on_rx_packet, thus this function must be
 * used before calling ossl_ackm_on_rx_packet.
 */
int ossl_ackm_is_rx_pn_processable (OSSL_ACKM *ackm, QUIC_PN pn,
                                    int pkt_space);
...
int ossl_ackm_get_largest_unacked (OSSL_ACKM *ackm, int pkt_space,
                                   QUIC_PN *pn);

/*
 * Forces the ACKM to consider a packet with the given PN in the given PN space
 * as having been pseudo-lost. The main reason to use this is during a Retry,
 * to force any resources sent in the first Initial packet to be resent.
 *
...
/*
 * Returns the PTO duration as currently calculated. This is a quantity of
 * time. This duration is used in various parts of QUIC besides the ACKM.
 */
