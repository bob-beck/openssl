 ...
 */
static ossl_inline int
ktls_start (int fd, ktls_crypto_info_t *tls_en, int is_tx)
{
  if (is_tx)
...
static ossl_inline int
ktls_enable_tx_zerocopy_sendfile (int fd)
{
  return 0;
}
 ...
 */
static ossl_inline int
ktls_send_ctrl_message (int fd, unsigned char record_type, const void *data,
                        size_t length)
{
  struct msghdr msg = { 0 };
...
static ossl_inline int
ktls_read_record (int fd, void *data, size_t length)
{
  return -1;
}
 ...
 * TLS.
 */
static ossl_inline ossl_ssize_t
ktls_sendfile (int s, int fd, off_t off, size_t size, int flags)
{
  off_t sbytes = 0;
  int ret;
 ...
 * and encapsulated in TLS records using the crypto_info provided here. The
 * TLS_RX socket option changes the recv/recvmsg handlers of the TCP socket. If
 * successful, then data received using this socket will be decrypted,
 * authenticated and decapsulated using the crypto_info provided here.
 */
static ossl_inline int
ktls_start (int fd, ktls_crypto_info_t *crypto_info, int is_tx)
{
