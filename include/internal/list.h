...
#define OSSL_LIST_FOREACH_FROM(p, name, init) for ((p) = (init); (p) != NULL; (p) = ossl_list_##name##_next(p))
#define OSSL_LIST_FOREACH(p, name, l) OSSL_LIST_FOREACH_FROM(p, name, ossl_list_##name##_head(l))

#define OSSL_LIST_FOREACH_REV_FROM(p, name, init) for ((p) = (init); (p) != NULL; (p) = ossl_list_##name##_prev(p))
#define OSSL_LIST_FOREACH_REV(p, name, l) OSSL_LIST_FOREACH_FROM(p, name, ossl_list_##name##_tail(l))

#define OSSL_LIST_FOREACH_DELSAFE_FROM(p, pn, name, init)                                                              \
    for ((p) = (init); (p) != NULL && (((pn) = ossl_list_##name##_next(p)), 1); (p) = (pn))
...
#define OSSL_LIST_FOREACH_REV_DELSAFE_FROM(p, pn, name, init)                                                          \
    for ((p) = (init); (p) != NULL && (((pn) = ossl_list_##name##_prev(p)), 1); (p) = (pn))
...
#define DEFINE_LIST_OF_IMPL(name, type)                                                                                \
    ...
    {                                                                                                                  \
        assert(elem->ossl_list_##name.next == NULL || elem->ossl_list_##name.next->ossl_list_##name.prev == elem);     \
        return elem->ossl_list_##name.next;                                                                            \
    }                                                                                                                  \
    static ossl_unused ossl_inline type *ossl_list_##name##_prev(const type *elem)                                     \
    {                                                                                                                  \
        assert(elem->ossl_list_##name.prev == NULL || elem->ossl_list_##name.prev->ossl_list_##name.next == elem);     \
        return elem->ossl_list_##name.prev;                                                                            \
    }                                                                                                                  \
    static ossl_unused ossl_inline void ossl_list_##name##_remove(OSSL_LIST(name) * list, type * elem)                 \
    {                                                                                                                  \
    ...
    static ossl_unused ossl_inline void ossl_list_##name##_insert_head(OSSL_LIST(name) * list, type * elem)            \
    {                                                                                                                  \
    ...
    static ossl_unused ossl_inline void ossl_list_##name##_insert_tail(OSSL_LIST(name) * list, type * elem)            \
    {                                                                                                                  \
        assert(elem->ossl_list_##name.list == NULL);                                                                   \
        OSSL_LIST_DBG(elem->ossl_list_##name.list = list)                                                              \
        if (list->omega != NULL)                                                                                       \
    ...
    static ossl_unused ossl_inline void ossl_list_##name##_insert_before(OSSL_LIST(name) * list, type * e,             \
                                                                         type * elem)                                  \
    {                                                                                                                  \
        assert(elem->ossl_list_##name.list == NULL);                                                                   \
        OSSL_LIST_DBG(elem->ossl_list_##name.list = list)                                                              \
    ...
    static ossl_unused ossl_inline void ossl_list_##name##_insert_after(OSSL_LIST(name) * list, type * e, type * elem) \
    {                                                                                                                  \
        assert(elem->ossl_list_##name.list == NULL);                                                                   \
        OSSL_LIST_DBG(elem->ossl_list_##name.list = list)                                                              \
