...
/*
 * Set the token used in Initial packets. The callback is called when the
 * buffer is no longer needed; for example, when the TXP is freed or when this
 * function is called again with a new buffer. Fails returning 0 if the token
 * is too big to ever be reasonably encapsulated in an outgoing packet based on
 * our current understanding of our PMTU.
 */
 ...
 *
 * The TXP will no longer reference the crypto[enc_level] QUIC_SSTREAM which
 * was provided in the TXP arguments. However, it is the callers responsibility
 * to free that QUIC_SSTREAM if desired.
 */
 ...
 * irreversible and causes all further packets generated by the TXP to contain
 * a CONNECTION_CLOSE frame. This function fails if it has already been called
