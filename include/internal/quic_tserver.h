 ...
 * =======================
 *
 * This implements a QUIC test server. Since full QUIC server support is not
 * yet implemented this server is limited in features and scope. It exists to
 ...
 *
 * Note that this test server is not suitable for production use because it
 * does not implement address verification, anti-amplification or retry logic.
 */
 ...
 * *bytes_read and returns 1 on success. If no bytes are available, 0 is
 * written to *bytes_read and 1 is returned (this is considered a success
 * case).
 *
...
/*
 * Attempts to write to the given stream. Writes the number of bytes consumed
 * to *bytes_written and returns 1 on success. If there is no space currently
...
/*
 * Returns 1 if all data sent on the given stream_id has been acked by the
 * peer.
 */
...
void ossl_quic_tserver_set_msg_callback (
    QUIC_TSERVER *srv,
