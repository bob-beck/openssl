 ...
 * part.
 *
 * Invariants in each state are noted in comments below. In particular, once
 * all data has been acknowledged received, or we have reset the stream, we
 * don't need to keep the QUIC_SSTREAM and data buffers around. Of course, we
 * also don't have a QUIC_SSTREAM on a receive-only stream.
 */
 ...
 * --------------------------
 *
 * These correspond to the states defined in RFC 9000 s. 3.2, with the
 * exception of the NONE state which represents the absence of a receive stream
 * part.
 *
 * Invariants in each state are noted in comments below. In particular, once
 * all data has been read by the application, we don't need to keep the
 * QUIC_RSTREAM and data buffers around. If the receive part is instead reset
 * before it is finished, we also don't need to keep the QUIC_RSTREAM around.
 * Finally, we don't need a QUIC_RSTREAM on a send-only stream.
 */
 ...
 * even if this function returns 1, s->sstream might be NULL if the
 * QUIC_SSTREAM has been deemed no longer needed, for example due to a
 * RESET_STREAM.
 */
static ossl_inline ossl_unused int
 ...
 * QUIC_RSTREAM has been deemed no longer needed, for example if the receive
 * stream is completely finished with.
 */
static ossl_inline ossl_unused int
 ...
 * been *sent*; the absence of DATA_SENT does not mean a FIN has not been
 * queued (meaning no more application data can be appended). This is enforced
 * by QUIC_SSTREAM.
 */
static ossl_inline ossl_unused int
 ...
 * converse is not necessarily true; erasure of a receive stream buffer which
 * is no longer required is an optimisation which the QSM may, but is not
 * obliged, to perform.
 *
 * This call should be used where it is desired to do something with the
 * receive stream buffer but there is no more specific receive state
 * restriction which is applicable.
 */
static ossl_inline ossl_unused int
 ...
 *   - tracks which streams are 'active' (currently have data for
 * transmission);
...
/*
 * get_stream_limit is a callback which is called to retrieve the current
 * stream limit for streams created by us. This mechanism is not used for
 ...
 *
 * If uni is 1, get the limit for locally-initiated unidirectional streams,
 * else get the limit for locally-initiated bidirectional streams.
 *
 ...
 * given stream object, but iterators not currently pointing at the given
 * stream object are not invalidated.
 */
 ...
 * started transmitting application data, in practice the meaningful
 * distinction between the two states is whether we have allocated a stream ID
 * to the stream or not. QUIC permits us to defer stream ID allocation until
 * first STREAM (or STREAM_DATA_BLOCKED) frame transmission for
 ...
 * (i.e., the time at which ossl_quic_sstream_fin()) is called as it occurs
 * when we have SENT all data on a given stream send part, not merely buffered
 * it. Note that this transition is NOT reversed in the event of some of that
 * data being lost.
 *
 ...
 *
 * It returns 0 if not in the RESET_SENT or RESET_RECVD states, as this
 * function should only be called after we have already sent a RESET_STREAM
 * frame and entered the RESET_SENT state. It also returns 0 if there is no
 * send part (caller error).
 */
...
/*
 * Transitions from the SIZE_KNOWN receive stream state to the DATA_RECVD
 * state. This should be called once all data for a receive stream is received.
 *
 * Returns 1 if the transition was taken.
 */
...
/*
 * Marks the stream as wanting a STOP_SENDING frame transmitted. It is not
 * valid to call this if ossl_quic_stream_map_stop_sending_recv_part() has not
 * been called. For TXP use.
 */
