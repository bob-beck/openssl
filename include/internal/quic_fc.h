 ...
 * value. This is not an error per se but may indicate a local programming
 * error or a protocol error in a remote peer.
 */
 ...
 * controlled bytes we are allowed to send. (Thus if this function returns 0,
 ...
 * returned. The consumed value is the amount already consumed on the
...
/*
 * Like ossl_quic_txfc_consume_credit(), but when called on a stream-level
 * TXFC, consumes only from the stream-level credit and does not inform the
 * connection-level TXFC.
 */
int ossl_quic_txfc_consume_credit_local (QUIC_TXFC *txfc, uint64_t num_bytes);

/*
 * This flag is provided for convenience. A caller is not required to use it.
 * It is a boolean flag set whenever our credit drops to zero. If clear is 1,
 * the flag is cleared. The old value of the flag is returned. Callers may use
 * this to determine if they need to send a DATA_BLOCKED or STREAM_DATA_BLOCKED
 ...
 * expressed in bytes and determine how much credit the RXFC extends to the
 * peer to transmit more data at a time.
 */
 ...
 * function, as it may have caused the RXFC to decide to grant more flow
 * control credit to the peer.
 *
 ...
 * size for the stream in bytes. If this is the case and final_size is
 * non-NULL, writes the final size to *final_size. Otherwise, returns 0.
 */
int ossl_quic_rxfc_get_final_size (const QUIC_RXFC *rxfc,
                                   uint64_t *final_size);
