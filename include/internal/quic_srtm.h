 ...
 *
 * The (opaque pointer, sequence number) tuple is used to refer to an entry
 * (for example for the purposes of removing it later when it is no longer
 * needed). Likewise, an entry can be looked up using SRT to get the opaque
 * pointer and sequence number.
 *
 * It is important to note that the same SRT may exist multiple times and map
 * to multiple (opaque pointer, sequence number) tuples, for example, if we
 * initiate multiple connections to the same peer using the same local
 * QUIC_PORT and the peer decides to behave bizarrely and issue the same SRT
 * for both connections. It should not do this, but we have to be resilient
 * against byzantine peer behaviour. Thus we are capable of storing multiple
 * identical SRTs for different (opaque pointer, sequence number) keys.
 *
 ...
 * identify zero or more corresponding (opaque pointer, sequence number)
 * tuples.
 *
 * The opaque pointer may be used for any purpose but is intended to represent
 * a connection identity and must therefore be consistent (usefully
 * comparable).
 */
