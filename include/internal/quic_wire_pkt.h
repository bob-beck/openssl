...
/*
 * Determine if the packet type must come at the end of the datagram (due to
 * the lack of a length field).
 */
static ossl_inline ossl_unused int
...
/*
 * Smallest possible QUIC packet size as per RFC (aside from version
 * negotiation packets).
 */
 ...
 * be encrypted (i.e., you must remove header protection before decrypting
 * packets received). The function examines the header buffer to determine
 * which bytes of the header need to be decrypted.
 *
 ...
 *
 * If nodata is 1, the input is assumed to have no payload data in it.
 * Otherwise payload data must be present.
 *
 ...
 * buffer.
 *
 * In order to decode short packets, the connection ID length being used must
 * be known contextually, and should be passed as short_conn_id_len. If
 ...
 * is not copied; the caller should write hdr->len bytes of encrypted payload
 * to the WPACKET immediately after the call to this function. However,
 ...
 * occur, causing these pointers to become invalid. Therefore, you must not
 * call any write WPACKET function between this call and the call to
 ...
 * write at the WPACKET cursor location once this function returns
 * successfully. It is recommended that you call this function, write the
 * encrypted payload, call ossl_quic_hdr_protector_encrypt, and then call
 * WPACKET_allocate_bytes(hdr->len).
 *
 * Version Negotiation and Retry packets do not use header protection; for
 * these header types, the fields in *ptrs are all written as zero. Version
 ...
 *
 * Information on packet length is not decoded, as this only needs to be used
 * on the first packet in a datagram, therefore this takes a buffer and not a
 * PACKET.
 *
 * Returns 1 on success and 0 on failure.
 */
...
/*
 * Determine how many bytes should be used to encode a PN. Returns the number
 * of bytes (which will be in range [1, 4]).
 */
 ...
 * pointer.
 *
 * client_initial_dcid must be the original DCID used by the client in its
 * first Initial packet, as this is used to calculate the Retry Integrity Tag.
 *
...
/*
 * Calculates a retry integrity tag. Returns 0 on error, for example if hdr
 * does not have a type of QUIC_PKT_TYPE_RETRY.
 *
 * client_initial_dcid must be the original DCID used by the client in its
 * first Initial packet, as this is used to calculate the Retry Integrity Tag.
 *
 ...
 * function and passing (hdr->data + hdr->len - QUIC_RETRY_INTEGRITY_TAG_LEN)
 * as the tag argument.) This function fails if hdr->len is too short to
 * contain a Retry Integrity Tag.
 */
