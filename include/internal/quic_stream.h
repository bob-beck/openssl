 ...
 * of the stream and 0 otherwise. hdr->stream_id is not set; the caller must
 * set it.
 *
 ...
 * buffer organisation. The sum of the lengths of the IOVs and the value
 * written to hdr->len will always match. If the caller decides to send less
 * than hdr->len of stream data, it must adjust the IOVs accordingly. This may
 * be done by updating hdr->len and then calling the utility function
 ...
 * - 1). If you do not call ossl_quic_sstream_mark_transmitted, the next call
 * to ossl_quic_sstream_get_stream_frame will return the same data (or
 * potentially the same and more, if more data has been appended by the
 * application).
 *
 ...
 * 0, 1 or 2. A *num_iov value of 0 can only occurs when hdr->is_fin is set
 * (for example, when a stream is closed after all existing data has been sent,
 * and without sending any more data); otherwise the function returns 0 as
 ...
 * are both inclusive, therefore all calls to this function always mark at
 * least one byte as being transmitted; if no bytes have been transmitted, do
 ...
 * been transmitted. final_size is the final size of the stream (i.e., the
 * value offset + len of the transmitted STREAM frame).
 *
 ...
 * The send stream will return the lost data for retransmission on a future
 * call to ossl_quic_sstream_get_stream_frame. The start and end values denote
...
/*
 * (Front end use.) Appends user data to the stream. The data is copied into
 * the stream. The amount of data consumed from buf is written to *consumed on
...
/*
 * Returns 1 iff all bytes (and any FIN, if any) which have been appended to
 * the QUIC_SSTREAM so far, and any FIN (if any), have been both sent and
 * acked.
 */
 ...
 *
 * The QUIC_RSTREAM is instantiated once for every stream that can receive
 * data. (i.e., for a unidirectional receiving stream or for the receiving
 * component of a bidirectional stream).
 */
