 ...
 * first they are popped, and then they are culled. Bytes which have been
 * popped but not yet culled will not be overwritten, and can be restored.
 */
struct ring_buf
{
  void *start;
...
ring_buf_init (struct ring_buf *r)
{
  r->start = NULL;
  r->alloc = 0;
...
ring_buf_destroy (struct ring_buf *r, int cleanse)
{
  if (cleanse)
...
ring_buf_used (struct ring_buf *r)
{
  return (size_t)(r->head_offset - r->ctail_offset);
}

static ossl_inline size_t
ring_buf_avail (struct ring_buf *r)
{
  return r->alloc - ring_buf_used (r);
}

static ossl_inline int
ring_buf_write_at (struct ring_buf *r, uint64_t logical_offset,
...
static ossl_inline size_t
ring_buf_push (struct ring_buf *r, const unsigned char *buf, size_t buf_len)
{
 ...
 * of (logical_offset + *buf_len) by calling this function again. If the
 * logical offset is out of the range retained by the ring buffer, returns 0,
 * else returns 1. A logical offset at the end of the range retained by the
 * ring buffer is not considered an error and is returned with a *buf_len of 0.
 *
 * The ring buffer state is not changed.
 */
static ossl_inline int
ring_buf_get_buf_at (const struct ring_buf *r, uint64_t logical_offset,
                     const unsigned char **buf, size_t *buf_len)
{
...
static ossl_inline void
ring_buf_cpop_range (struct ring_buf *r, uint64_t start, uint64_t end,
                     int cleanse)
{
  assert (end >= start);
...
static ossl_inline int
ring_buf_resize (struct ring_buf *r, size_t num_bytes, int cleanse)
{
