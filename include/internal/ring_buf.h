...
static ossl_inline int
ring_buf_write_at(struct ring_buf* r,
...
ring_buf_get_ptr(const struct ring_buf* r,
                 uint64_t logical_offset,
                 size_t* max_len)
{
  unsigned char* start = r->start;
 ...
 */
static ossl_inline int
ring_buf_get_buf_at(const struct ring_buf* r,
                    uint64_t logical_offset,
                    const unsigned char** buf,
                    size_t* buf_len)
{
...
static ossl_inline void
ring_buf_cpop_range(struct ring_buf* r,
                    uint64_t start,
                    uint64_t end,
                    int cleanse)
{
