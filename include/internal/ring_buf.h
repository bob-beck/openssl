		 ...
		 size_t *max_len)
{
	...
	if (logical_offset >= r->head_offset ||
	    logical_offset < r->ctail_offset)
		return NULL;
	idx = logical_offset % r->alloc;
	*max_len = r->alloc - idx;
	return start + idx;
					    ...
					    uint64_t end, int cleanse)
{
	...
	if (cleanse && r->alloc > 0 && end > r->ctail_offset) {
		...
		if (l > r->alloc - idx) {
			OPENSSL_cleanse((unsigned char *)r->start + idx,
					r->alloc - idx);
			l -= r->alloc - idx;
			idx = 0;
		}
				       ...
				       int cleanse)
{
	...
	for (;;) {
		if (!ring_buf_get_buf_at(r, r->ctail_offset + copied, &src,
					 &src_len)) {
