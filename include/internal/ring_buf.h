...
static ossl_inline const unsigned char* ring_buf_get_ptr(
    const struct ring_buf* r, uint64_t logical_offset, size_t* max_len) {
  unsigned char* start = r->start;
  size_t idx;
...
static ossl_inline int ring_buf_get_buf_at(const struct ring_buf* r,
  ...
  if (l > r->alloc - idx) l = r->alloc - idx;
...
static ossl_inline void ring_buf_cpop_range(struct ring_buf* r, uint64_t start,
  ...
  if (start > r->ctail_offset || end >= MAX_OFFSET) return;
  ...
  if (cleanse && r->alloc > 0 && end > r->ctail_offset) {
    ...
    if (cleanse_end > r->head_offset) cleanse_end = r->head_offset;
    ...
    if (l > 0) OPENSSL_cleanse((unsigned char*)r->start + idx, l);
  }

  r->ctail_offset = end + 1;
  /* Allow culling unpushed data */
  if (r->head_offset < r->ctail_offset) r->head_offset = r->ctail_offset;
}
...
static ossl_inline int ring_buf_resize(struct ring_buf* r, size_t num_bytes,
  ...
  if (num_bytes == r->alloc) return 1;

  if (num_bytes < ring_buf_used(r)) return 0;

  rnew.start = OPENSSL_malloc(num_bytes);
  if (rnew.start == NULL) return 0;
