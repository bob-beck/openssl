...
PACKET_get_quic_vlint (PACKET *pkt, uint64_t *data)
{
  size_t enclen;
 ...
 * position. If was_minimal is non-NULL, *was_minimal is set to 1 if the
 * integer was encoded using the minimal possible number of bytes and 0
 * otherwise.
 */
__owur static ossl_inline int
...
__owur static ossl_inline int
PACKET_peek_quic_vlint (PACKET *pkt, uint64_t *data)
{
 ...
 */
__owur static ossl_inline int
PACKET_skip_quic_vlint (PACKET *pkt)
{
  size_t enclen;
 ...
 */
__owur static ossl_inline int
PACKET_get_quic_length_prefixed (PACKET *pkt, PACKET *subpkt)
{
  uint64_t length;
 ...
 * QUIC variable-length integer. The pointer to the payload buffer is output
 * and must be filled by the caller. This function assures optimal selection of
