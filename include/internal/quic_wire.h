...
/*
 * Encodes a QUIC ACK frame to the packet writer, given a logical
 * representation of the ACK frame.
 *
 ...
 * bidirectional streams; else it specifies the maximum number of
 ...
 *
 * Returns a pointer to the start of the payload on success, or NULL on
 * failure.
 */
 ...
 *
 *   - A variable-length field in the encoded frame appears to exceed the
 * bounds of the PACKET's buffer.
 *
 * These functions should be called with the PACKET pointing to the start of
 * the frame (including the initial type field), and consume an entire frame
...
/*
 * Decodes a QUIC ACK frame. The ack_ranges field of the passed structure
 * should point to a preallocated array of ACK ranges and the num_ack_ranges
 * field should specify the length of allocation.
 *
 ...
 * allocated.
 *
 * The ack_delay_exponent argument specifies the index of a power of two used
 * to decode the ack_delay field. This must match the ack_delay_exponent value
 ...
 *
 * f->data is set to point inside the packet buffer inside the PACKET,
 * therefore it is safe to access for as long as the packet buffer exists. If
 * nodata is set to 1 then reading the PACKET stops after the frame header and
 * f->data is set to NULL.
 */
 ...
 * *max_streams.
 *
 * Whether the limit concerns bidirectional streams or unidirectional streams
 * is denoted by the frame type; the caller should examine the frame type to
 * determine this.
 */
...
int ossl_quic_wire_decode_frame_stream_data_blocked (
    PACKET *pkt, uint64_t *stream_id, uint64_t *max_stream_data);

/*
 * Decodes a QUIC STREAMS_BLOCKED frame. The Maximum Streams field is written
 * to *max_streams.
 *
 * Whether the limit concerns bidirectional streams or unidirectional streams
 * is denoted by the frame type; the caller should examine the frame type to
 ...
 * valid. The conn_id_len field is set to the length of the connection ID
 * string in bytes.
 */
