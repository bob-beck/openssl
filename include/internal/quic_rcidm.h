 ...
 * the retry. This SCID must be enrolled using this function. May only be
 * called once. The added RCID has no sequence number associated with it as it
 * is essentially a new ODCID (hereafter a Retry ODCID).
 *
 * Not for server use.
 */
...
/*
 * Processes an incoming NEW_CONN_ID frame, recording the new CID as a
 * potential RCID. The RCIDM retirement mechanism is ratcheted according to the
 * ncid->retire_prior_to field. The stateless_reset field is ignored; the
 * caller is responsible for handling it separately.
 */
 ...
 * written to *seq_num (if seq_num is non-NULL) and this function returns 1.
 * The queue entry is popped (and the caller is thus assumed to have taken
 ...
 * changed since the last call to this function with clear set. If clear is
 * set, clears the changed flag. Returns the old value of the changed flag.
 */
 ...
 * ossl_quic_rcidm_pop_retire_seq_num() as these are still active until
 * actually retired.
 */
