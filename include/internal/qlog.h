...
/* Configuration */
int
ossl_qlog_set_event_type_enabled(QLOG* qlog, uint32_t event_type, int enable);
int
...
int
ossl_qlog_set_sink_file(QLOG* qlog, FILE* file, int close_flag);
#endif
int
...
#define QLOG_EVENT_BEGIN(qlog, cat, name)                                      \
  {                                                                            \
    ...
    uint32_t qlog_event_type = QLOG_EVENT_TYPE_##cat##_##name;                 \
                                                                               \
    if (ossl_qlog_event_try_begin(                                             \
          qlog_instance, qlog_event_type, #cat, #name, #cat ":" #name)) {
