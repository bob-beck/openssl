...
#define HT_SET_KEY_STRING(key, member, value)                                  \
  do {                                                                         \
    if ((value) != NULL)                                                       \
      strncpy(                                                                 \
        (key)->keyfields.member, value, sizeof((key)->keyfields.member) - 1);  \
  } while (0)
...
#define DECLARE_HT_VALUE_TYPE_FNS(vtype, name)                                 \
  int ossl_ht_##name##_##vtype##_insert(                                       \
    HT* h, HT_KEY* key, vtype* data, vtype** olddata);                         \
  vtype* ossl_ht_##name##_##vtype##_from_value(HT_VALUE* v);                   \
  vtype* ossl_unused ossl_ht_##name##_##vtype##_get(                           \
    HT* h, HT_KEY* key, HT_VALUE** v);                                         \
 ...
 */
static void ossl_unused
ossl_ht_strcase(char* tgt, const char* src, int len)
{
  int i;
 ...
 */
int
ossl_ht_insert(HT* htable, HT_KEY* key, HT_VALUE* data, HT_VALUE** olddata);
 ...
 */
int
ossl_ht_delete(HT* htable, HT_KEY* key);

/*
 * Returns number of elements in the hash table
 */
size_t
ossl_ht_count(HT* htable);
 ...
 */
void
ossl_ht_foreach_until(HT* htable,
                      int (*cb)(HT_VALUE* obj, void* arg),
                      void* arg);
/*
 ...
 */
HT_VALUE*
ossl_ht_get(HT* htable, HT_KEY* key);
