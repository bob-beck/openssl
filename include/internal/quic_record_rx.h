 ...
 * will unregister the QRX from the demuxer for all registered destination
 * connection IDs (DCIDs) automatically.
 */
 ...
 * connection, it is important that all ELs no longer being used (i.e.,
 * INITIAL, 0-RTT, 1-RTT) are transitioned to the DISCARDED state. Otherwise,
 * the QRX will assume that keys for these ELs will arrive in future, and will
 * buffer any received packets for those ELs perpetually. This can be done by
 * calling ossl_qrx_discard_enc_level for all non-1-RTT ELs immediately after
...
/*
 * Provides a secret to the QRX, which arises due to an encryption level
 * change. enc_level is a QUIC_ENC_LEVEL_* value. To initialise the INITIAL
 * encryption level, it is recommended to use ossl_quic_provide_initial_secret
 * instead.
 *
 ...
 * are managed semi-automatically by the QRX but do require some caller
 * handling (see below).
 *
 ...
 * If we don't then PN decoding fails in cases where the initial PN is a large
 * value. This function does that migration for us
 */
...
/*
 * Returns 1 if there are any already processed (i.e. decrypted) packets
 * waiting to be read from the QRX.
 */
 ...
 * unprocessed data is buffered by the QRX, not whether any data is available
 * in a kernel socket buffer.
 */
 ...
 * validated before being queued in the read queue. This is called after packet
 * body decryption and authentication to prevent exposing side channels.
 * pn_space is a QUIC_PN_SPACE_* value denoting which PN space the PN belongs
 * to.
 *
 ...
 * would not be correctly routed to the QRX via standard DCID-based routing;
 * for example, when handling an incoming Initial packet which is attempting to
 ...
 * called. Calling this function will then requeue any such deferred packets
 * for processing.
 */
 ...
 * different Key Phase value. This could be caused by an attacker in the
 * network flipping random bits, therefore such a key update is tentative until
 * the packet payload is successfully decrypted and authenticated by the AEAD
 * with the 'next' keys. These 'next' keys then become the 'current' keys and
 * the 'current' keys then become the 'previous' keys. The 'previous' keys must
 * be kept around temporarily as some packets may still be in flight in the
 * network encrypted with the old keys. If the old Key Phase value is X and the
 * new Key Phase Value is Y (where obviously X != Y), this creates an ambiguity
 * as any new packet received with a KP of X could either be an attempt to
 * initiate yet another key update right after the last one, or an old packet
 * encrypted before the key update.
 *
 ...
 *       previous packet protection keys; a recovered PN that is higher than
 * any PN from the current KP requires use of the next packet protection keys."
 *
 *   Strategy 2:
 ...
 *       allow for reordering in the network. In this case, the KP bit alone
 * can be used to select keys."
 *
 * Strategy 2 is more efficient (we can keep fewer cipher contexts around) and
 * should cover all actually possible network conditions. It also allows a
 * delay after we make the 'next' keys our 'current' keys before we generate
 * new 'next' keys, which allows us to mitigate against malicious peers who try
 * to initiate an excessive number of key updates.
 *
 ...
 * decrypt and authenticate the received packet with the 'next' keys rather
 * than the 'current' keys. If (and only if) this authentication is successful,
 * we move to the UPDATING state. (An attacker in the network could flip the
 * Key Phase bit randomly, so it is essential we do nothing until AEAD
 * authentication is complete.)
 *
 * In the UPDATING state, we know a key update is occurring and record
 * the new Key Phase bit value as the newly current value, but we still keep
 * the old keys around so that we can still process any packets which were
 * still in flight when the key update was initiated. In the UPDATING state, a
 * Key Phase bit value different to the current expected value is treated not
 * as the initiation of another key update, but a reference to our old keys.
 *
 ...
 * set of keys can be used: the current keys. If a packet is received with a
 * Key Phase bit value different to the current Key Phase Bit value, this is
 * treated as a request for a Key Update, but this request is ignored and the
 * packet is treated as malformed. We do this to allow mitigation against
 * malicious peers trying to initiate an excessive number of Key Updates. The
 * timeout for the transition from UPDATING to COOLDOWN is recommended as
 * adequate for this purpose in itself by the RFC, so the normal additional
 * timeout value for the transition from COOLDOWN to normal is zero (immediate
 * transition).
 *
 * A summary of each state:
 *
 *                 Epoch  Exp KP  Uses Keys KS0    KS1    If Non-Expected KP
 * Bit
 *                 -----  ------  --------- ------ ----- ----------------------
 ...
 * Update as per the RFC. The caller is responsible for detecting this
 * condition and making the necessary calls to the TX side by detecting changes
 * to the return value of ossl_qrx_get_key_epoch().
 *
 ...
 * current substate of the PROVISIONED state is to the DISCARDED state, which
 ...
 *
 * The value returned by this function increments specifically at the
 * transition from the NORMAL to the UPDATING state discussed above.
 */
 ...
 * has been dropped.
 *
 * It is essential that the caller call this within a few PTO intervals of a
 * key update occurring (as detected by the caller in a call to
 ...
 * exact limit imposed does vary by EL due to the possibility that different
 * ELs use different AEADs.
 */
