...
ossl_assert_int (int expr, const char *exprstr, const char *file, int line)
{
  if (!expr)
...
ossl_ends_with_dirsep (const char *path)
{
  if (*path != '\0')
    path += strlen (path) - 1;
#if defined __VMS
...
static ossl_inline char
ossl_determine_dirsep (const char *path)
{
...
static ossl_inline int
ossl_is_absolute_path (const char *path)
{
#if defined __VMS
  if (strchr (path, ':') != NULL
