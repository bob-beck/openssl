 ...
 * various pieces of QUIC into a single top-level object, and handles
 * connection state which is not specific to the client or server roles. In
 * particular, it is strictly separated from the libssl front end I/O API
 * personality layer, and is not an SSL object.
 *
 ...
 * layer and common code for handling connections, and between the
 * functionality which is specific to clients and which is specific to servers,
 * and the functionality which is common to both.
 *
 * The API personality layer provides SSL objects (e.g. a QUIC_CONNECTION)
 * which consume a QUIC channel and implement a specific public API. Things
 * which are handled by the API personality layer include emulation of blocking
 * semantics, handling of SSL object mode flags like non-partial write mode,
 * etc.
 *
 * Where the QUIC_CHANNEL is used in a server role, there is one QUIC_CHANNEL
 * per connection. In the future a QUIC Channel Manager will probably be
 * defined to handle ownership of resources which are shared between
 * connections (e.g. demuxers). Since we only use server-side functionality for
 * dummy test servers for now, which only need to handle one connection at a
 * time, this is not currently modelled.
 *
 * Synchronisation
 * ---------------
 *
 ...
 * implementation. Some of these are suitable for general use by any part of
 * the code (e.g. ossl_quic_channel_raise_protocol_error), others are for very
 ...
 * channel, this starts sending the first handshake layer message, etc. Can
 * only be called in the idle state; successive calls are ignored.
 */
 ...
 *
 * Usually the ossl_quic_channel_raise_protocol_error() function should be
 * used. The ossl_quic_channel_raise_protocol_error_loc() function can be used
 ...
 * channel was instantiated. In order to allow locks to be acquired and
 * released with the correct granularity, it is the caller's responsibility to
 * ensure this lock is held for write while calling any QUIC_CHANNEL method,
 * except for methods explicitly designed otherwise.
 *
 * This method is thread safe and does not require prior locking. It can also
 * be called while the lock is already held. Note that this is simply a
 * convenience function to access the mutex which was passed to the channel at
 * instantiation time; it does not belong to the channel but rather is presumed
 * to belong to the owner of the channel.
 */
 ...
 * is used to confirm the initiator and determine the stream type. The stream
 * is automatically added to the QSM's accept queue. A pointer to the stream is
 ...
 */
void ossl_quic_channel_get_diag_local_cid (QUIC_CHANNEL *ch,
                                           QUIC_CONN_ID *cid);
