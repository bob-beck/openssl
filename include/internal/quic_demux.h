 ...
 * ============
 *
 * The QUIC connection demuxer is the entity responsible for receiving
 * datagrams from the network via a datagram BIO. It parses the headers of the
 * first packet in the datagram to determine that packet's DCID and hands off
 ...
 * any QRX and is not coupled to it. All CID knowledge is also externalised
 * into a LCIDM or other CID state tracking object, without the DEMUX being
 * coupled to any particular DCID resolution mechanism.
 *
 * URX Queue
 * ---------
 *
 * Since the demuxer must handle the initial reception of datagrams from the
 * OS, RX queue management for new, unprocessed datagrams is also handled by
 * the demuxer.
 *
 ...
 * packets, however, this is not the demuxer's concern. QUIC prohibits
 * different packets in the same datagram from containing different DCIDs; the
 * demuxer only considers the DCID of the first packet in a datagram when
 * deciding how to route a received datagram, and it is the responsibility of
 * the QRX to enforce this rule. Packets other than the first packet in a
 * datagram are not examined by the demuxer, and the demuxer does not perform
 * validation of packet headers other than to the minimum extent necessary to
 * extract the DCID; further parsing and validation of packet headers is the
 * responsibility of the QRX.
 *
 ...
 * -------------------
 *
 * The URXE queue is maintained as a simple doubly-linked list. URXE entries
 * are moved between different lists in their lifecycle (for example, from a
 * free list to a pending list and vice versa). The buffer into which datagrams
 * are received immediately follows this URXE header structure and is part of
 * the same allocation.
 */
 ...
 *
 * e is a URXE containing the datagram payload. It is permissible for the
 * callee to mutate this buffer; once the demuxer calls this callback, it will
 * never read the buffer again.
 *
...
/*
 * Set the default packet handler. This is used for incoming packets which
 * don't match a registered DCID. This is only needed for servers. If a default
 * packet handler is not set, a packet which doesn't match a registered DCID is
 * silently dropped. A default packet handler may be unset by passing NULL.
 *
 * The handler is responsible for ensuring that ossl_quic_demux_reinject_urxe
 * or ossl_quic_demux_release_urxe is called on the passed packet at some point
 * in the future, which may or may not be before the handler returns.
 */
...
/*
 * Releases a URXE back to the demuxer. No reference must be made to the URXE
 * or its buffer after calling this function. The URXE must not be in any
 * queue; that is, its prev and next pointers must be NULL.
 */
 ...
 *         Either the network read BIO has failed in a non-transient fashion,
 * or the QUIC implementation has encountered an internal state, assertion or
 * allocation error. The caller should tear down the connection similarly to in
 * the case of a protocol violation.
 *
 */
 ...
 *
 * If peer or local are NULL, their respective fields are zeroed in the
 * injected URXE.
 *
