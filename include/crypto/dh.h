...
int ossl_dh_generate_ffc_parameters(DH* dh,
                                    int type,
                                    int pbits,
                                    int qbits,
                                    BN_GENCB* cb);
int ossl_dh_generate_public_key(BN_CTX* ctx,
                                const DH* dh,
...
int ossl_dh_check_pub_key_partial(const DH* dh,
...
size_t ossl_dh_key2buf(const DH* dh,
                       unsigned char** pbuf,
                       size_t size,
                       int alloc);
