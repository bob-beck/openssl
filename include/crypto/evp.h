...
#define BLOCK_CIPHER_func_ecb(cname, cprefix, kstruct, ksched)                                                         \
    ...
    {                                                                                                                  \
        BLOCK_CIPHER_ecb_loop() cprefix##_ecb_encrypt(in + i, out + i, &EVP_C_DATA(kstruct, ctx)->ksched,              \
...
#define BLOCK_CIPHER_func_ofb(cname, cprefix, cbits, kstruct, ksched)                                                  \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            cprefix##_ofb##cbits##_encrypt(in, out, (long)EVP_MAXCHUNK, &EVP_C_DATA(kstruct, ctx)->ksched, ctx->iv,    \
                                           &num);                                                                      \
...
#define BLOCK_CIPHER_func_cbc(cname, cprefix, kstruct, ksched)                                                         \
    ...
    {                                                                                                                  \
        while (inl >= EVP_MAXCHUNK)                                                                                    \
        {                                                                                                              \
            cprefix##_cbc_encrypt(in, out, (long)EVP_MAXCHUNK, &EVP_C_DATA(kstruct, ctx)->ksched, ctx->iv,             \
                                  EVP_CIPHER_CTX_is_encrypting(ctx));                                                  \
            inl -= EVP_MAXCHUNK;                                                                                       \
            in += EVP_MAXCHUNK;                                                                                        \
            out += EVP_MAXCHUNK;                                                                                       \
...
#define BLOCK_CIPHER_func_cfb(cname, cprefix, cbits, kstruct, ksched)                                                  \
    static int cname##_cfb##cbits##_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, const unsigned char *in,           \
                                           size_t inl)                                                                 \
    {                                                                                                                  \
        ...
        {                                                                                                              \
            ...
            cprefix##_cfb##cbits##_encrypt(                                                                            \
                in, out,                                                                                               \
                (long)((cbits == 1) && !EVP_CIPHER_CTX_test_flags(ctx, EVP_CIPH_FLAG_LENGTH_BITS) ? chunk * 8          \
                                                                                                  : chunk),            \
...
#define BLOCK_CIPHER_def_ecb(cname, kstruct, nid, block_size, key_len, flags, init_key, cleanup, set_asn1, get_asn1,   \
                             ctrl)                                                                                     \
    BLOCK_CIPHER_def1(cname, ecb, ecb, ECB, kstruct, nid, block_size, key_len, 0, flags, init_key, cleanup, set_asn1,  \
                      get_asn1, ctrl)
...
#define BLOCK_CIPHER_defs(cname, kstruct, nid, block_size, key_len, iv_len, cbits, flags, init_key, cleanup, set_asn1, \
    ...
    BLOCK_CIPHER_def_cbc(cname, kstruct, nid, block_size, key_len, iv_len, flags, init_key, cleanup, set_asn1,         \
                         get_asn1, ctrl) BLOCK_CIPHER_def_cfb(cname, kstruct, nid, key_len, iv_len, cbits, flags,      \
                                                              init_key, cleanup, set_asn1, get_asn1, ctrl)             \
        BLOCK_CIPHER_def_ofb(cname, kstruct, nid, key_len, iv_len, cbits, flags, init_key, cleanup, set_asn1,          \
                             get_asn1, ctrl) BLOCK_CIPHER_def_ecb(cname, kstruct, nid, block_size, key_len, flags,     \
                                                                  init_key, cleanup, set_asn1, get_asn1, ctrl)
...
#define EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx) (((ctx)->operation & EVP_PKEY_OP_TYPE_SIG) != 0)

#define EVP_PKEY_CTX_IS_DERIVE_OP(ctx) (((ctx)->operation & EVP_PKEY_OP_TYPE_DERIVE) != 0)

#define EVP_PKEY_CTX_IS_ASYM_CIPHER_OP(ctx) (((ctx)->operation & EVP_PKEY_OP_TYPE_CRYPT) != 0)

#define EVP_PKEY_CTX_IS_GEN_OP(ctx) (((ctx)->operation & EVP_PKEY_OP_TYPE_GEN) != 0)

#define EVP_PKEY_CTX_IS_FROMDATA_OP(ctx) (((ctx)->operation & EVP_PKEY_OP_TYPE_DATA) != 0)

#define EVP_PKEY_CTX_IS_KEM_OP(ctx) (((ctx)->operation & EVP_PKEY_OP_TYPE_KEM) != 0)
