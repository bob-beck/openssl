...
__owur ML_DSA_KEY* ossl_ml_dsa_key_new(OSSL_LIB_CTX* libctx,
                                       const char* propq,
                                       int evp_type);
...
__owur int ossl_ml_dsa_key_equal(const ML_DSA_KEY* key1,
                                 const ML_DSA_KEY* key2,
                                 int selection);
...
__owur int ossl_ml_dsa_pk_decode(ML_DSA_KEY* key,
                                 const uint8_t* in,
                                 size_t in_len);
__owur int ossl_ml_dsa_sk_decode(ML_DSA_KEY* key,
                                 const uint8_t* in,
                                 size_t in_len);

EVP_MD_CTX* ossl_ml_dsa_mu_init(const ML_DSA_KEY* key,
                                int encode,
                                const uint8_t* ctx,
                                size_t ctx_len);
__owur int ossl_ml_dsa_mu_update(EVP_MD_CTX* md_ctx,
                                 const uint8_t* msg,
                                 size_t msg_len);
__owur int ossl_ml_dsa_mu_finalize(EVP_MD_CTX* md_ctx,
                                   uint8_t* mu,
                                   size_t mu_len);
