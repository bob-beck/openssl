...
#include <openssl/params.h>
  ...
  /** Copies EC_GROUP objects. Note: both EC_GROUPs must use the same
   * EC_METHOD.
  ...
  /** Sets the parameters of an ec curve defined by y^2 = x^3 + a*x + b (for
   * GFp) or y^2 + x*y = x^3 + a*x^2 + b (for GF2m)
   *  \param  group  EC_GROUP object
  ...
  /** Gets the parameters of the ec curve defined by y^2 = x^3 + a*x + b (for
   * GFp) or y^2 + x*y = x^3 + a*x^2 + b (for GF2m)
   *  \param  group  EC_GROUP object
   *  \param  p      BIGNUM with the prime number (GFp) or the polynomial
   *                 defining the underlying field (GF2m)
   ...
   * EC_GROUP.
   * The resulting parameters may contain an explicit or a named curve
   * depending on the EC_GROUP.
   ...
   *          if an error occurred
   */
  EC_GROUP *EC_GROUP_new_by_curve_name_ex (OSSL_LIB_CTX *libctx,
...
/* other interfaces to point2oct/oct2point: */
   ...
   *  \param  group  underlying EC_GROUP object
   *  \param  a      EC_POINT object to be inverted (it's used for the result
   * as well)
   *  \param  ctx    BN_CTX object (optional)
   *  \return 1 on success and 0 if an error occurred
   */
   ...
   *  \param  n      BIGNUM with the multiplier for the group generator
   * (optional)
   *  \param  q      EC_POINT object with the first factor of the second
   * summand
  ...
  EC_GROUP *d2i_ECPKParameters (EC_GROUP **, const unsigned char **in,
                                long len);
...
#define EC_FLAG_FIPS_CHECKED 0x0000
   ...
   *  \param  ctx  The library context for to use for this EC_KEY. May be NULL
   * in which case the default library context is used.
   ...
   */
  OSSL_DEPRECATEDIN_3_0 const EC_POINT *
  ...
  EC_KEY_set_conv_form (EC_KEY *eckey, point_conversion_form_t cform);
#endif /*OPENSSL_NO_DEPRECATED_3_0 */
...
#define EC_KEY_get_ex_new_index(l, p, newf, dupf, freef)                      \
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
  OSSL_DEPRECATEDIN_3_0 int EC_KEY_set_ex_data (EC_KEY *key, int idx,
                                                void *arg);
  OSSL_DEPRECATEDIN_3_0 void *EC_KEY_get_ex_data (const EC_KEY *key, int idx);

  /* wrapper functions for the underlying EC_GROUP object */
  OSSL_DEPRECATEDIN_3_0 void EC_KEY_set_asn1_flag (EC_KEY *eckey,
                                                   int asn1_flag);
  ...
  OSSL_DEPRECATEDIN_3_0 int
  EC_KEY_oct2priv (EC_KEY *key, const unsigned char *buf, size_t len);
   ...
   *  \return 1 on success and 0 if an error occurred
   */
  OSSL_DEPRECATEDIN_3_0 int ECParameters_print_fp (FILE *fp,
                                                   const EC_KEY *key);
   ...
   *  \return 1 on success and 0 if an error occurred
   */
  OSSL_DEPRECATEDIN_3_0 int EC_KEY_print_fp (FILE *fp, const EC_KEY *key,
                                             int off);
  ...
  OSSL_DEPRECATEDIN_3_0 const EC_KEY_METHOD *
  ...
  /** i2d_ECDSA_SIG encodes content of ECDSA_SIG (note: this function modifies
   * *pp
   *  (*pp += length of the DER encoded signature)).
   *  \param  sig  pointer to the ECDSA_SIG object
   *  \param  pp   pointer to a unsigned char pointer for the output or NULL
   *  \return the length of the DER encoded ECDSA_SIG object or a negative
   * value on error
   */
  ...
  OSSL_DEPRECATEDIN_3_0 void EC_KEY_METHOD_set_init (
      EC_KEY_METHOD *meth, int (*init) (EC_KEY *key),
