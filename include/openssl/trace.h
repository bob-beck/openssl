 ...
 * categories. For every trace category, the application can register a
 * separate tracer callback. When a callback is registered, a so called trace
 * channel is created for this category. This channel consists essentially of
 * an internal BIO which sends all trace output it receives to the registered
...
#define OSSL_TRACE_CATEGORY_NUM 21
   ...
   * The |data| argument points to whatever data was provided by the
   * application when registering the tracer function.
   *
   * The |category| number is given, as well as a |cmd| number, described
   * below.
   */
...
#define OSSL_TRACE_CTRL_END 2
   ...
   * Returns 1 on success and 0 on failure
   */
  int OSSL_trace_set_callback (int category, OSSL_trace_cb callback,
                               void *data);
  ...
  /*
   * Wrap a group of tracing output calls.  OSSL_trace_begin() locks tracing
   * and returns the trace channel associated with the given category, or NULL
   * if no channel is associated with the category.  OSSL_trace_end() unlocks
   * tracing.
   *
   * Usage:
   *
