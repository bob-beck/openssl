...
extern "C"
{
  ...
  /*
   * Typedef for the OSSL_STORE_INFO post processing callback.  This can be
   * used to massage the given OSSL_STORE_INFO, or to drop it entirely (by
   * returning NULL).
   */
  typedef OSSL_STORE_INFO *(*OSSL_STORE_post_process_info_fn) (
      OSSL_STORE_INFO *, void *);

  /*
   * Open a channel given a URI.  The given UI method will be used any time the
   * loader needs extra input, for example when a password or pin is needed,
   * and will be passed the same user data every time it's needed in this
   * context.
   *
...
#define OSSL_STORE_C_CUSTOM_START 100
  ...
  /*
   * Read one data item (a key, a cert, a CRL) that is supported by the
   * OSSL_STORE functionality, given a context. Returns a OSSL_STORE_INFO
   * pointer, from which OpenSSL typed data can be extracted with
   * OSSL_STORE_INFO_get0_PKEY(), OSSL_STORE_INFO_get0_CERT(), ... NULL is
   * returned on error, which may include that the data found at the URI can't
   * be figured out for certain or is ambiguous.
   */
   ...
   */
  int OSSL_STORE_delete (const char *uri, OSSL_LIB_CTX *libctx,
   ...
   * BIO actually reads.
   */
  OSSL_STORE_CTX *OSSL_STORE_attach (
      BIO *bio, const char *scheme, OSSL_LIB_CTX *libctx, const char *propq,
      const UI_METHOD *ui_method, void *ui_data, const OSSL_PARAM params[],
      OSSL_STORE_post_process_info_fn post_process, void *post_process_data);
...
#define OSSL_STORE_INFO_CRL 6    /* X509_CRL * */
   ...
   *
   * In all cases, ownership of the object is transferred to the
   * OSSL_STORE_INFO and will therefore be freed when the OSSL_STORE_INFO is
   * freed.
   */
  OSSL_STORE_INFO *OSSL_STORE_INFO_new (int type, void *data);
  OSSL_STORE_INFO *OSSL_STORE_INFO_new_NAME (char *name);
  int OSSL_STORE_INFO_set0_NAME_description (OSSL_STORE_INFO *info,
                                             char *desc);
...
#define OSSL_STORE_SEARCH_BY_ALIAS 4
   ...
   * to the loading channel.  This MUST happen before the first
   * OSSL_STORE_load().
   */
  ...
  const char *
  OSSL_STORE_LOADER_get0_properties (const OSSL_STORE_LOADER *loader);
  const char *
  OSSL_STORE_LOADER_get0_description (const OSSL_STORE_LOADER *loader);
  int OSSL_STORE_LOADER_is_a (const OSSL_STORE_LOADER *loader,
                              const char *scheme);
  void OSSL_STORE_LOADER_do_all_provided (
      OSSL_LIB_CTX *libctx, void (*fn) (OSSL_STORE_LOADER *loader, void *arg),
      void *arg);
  ...
  typedef int (*OSSL_STORE_ctrl_fn) (OSSL_STORE_LOADER_CTX *ctx, int cmd,
                                     va_list args);
  typedef int (*OSSL_STORE_expect_fn) (OSSL_STORE_LOADER_CTX *ctx,
                                       int expected);
  ...
  OSSL_DEPRECATEDIN_3_0
  const ENGINE *
  OSSL_STORE_LOADER_get0_engine (const OSSL_STORE_LOADER *loader);
  OSSL_DEPRECATEDIN_3_0
