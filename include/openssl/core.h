 ...
 *
 * The difference between this and OSSL_PARAM_OCTET_STRING is that only
 * pointers are manipulated for this type.
 *
 * This is more relevant for parameter requests, where the responding
 * function doesn't need to copy the data to the provided buffer, but
 * sets the provided buffer to point at the actual data instead.
...
#define OSSL_PARAM_OCTET_PTR 7
   ...
   * callback using the OSSL_FUNC_CORE_THREAD_START function in the |in|
   * dispatch table passed to OSSL_provider_init(). The arg passed back to a
   * provider will be the provider side context object.
   */
   ...
   * |in|         is the array of functions that the Core passes to the
   * provider. |out|        will be the array of base functions that the
   * provider passes back to the Core. |provctx|    a provider side context
   * object, optionally created if the provider needs it.  This value is passed
   * to other provider functions, notably other context constructors.
   */
