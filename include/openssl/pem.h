...
#define PEM_write_cb_fnsig(name, type, OUTTYPE, writename)                     \
  int PEM_##writename##_##name(OUTTYPE* out,                                   \
                               const type* x,                                  \
...
#define PEM_write_ex_fnsig(name, type, OUTTYPE, writename)                     \
  int PEM_##writename##_##name##_ex(                                           \
    OUTTYPE* out, const type* x, OSSL_LIB_CTX* libctx, const char* propq)
#define PEM_write_cb_ex_fnsig(name, type, OUTTYPE, writename)                  \
  int PEM_##writename##_##name##_ex(OUTTYPE* out,                              \
                                    const type* x,                             \
                                    const EVP_CIPHER* enc,                     \
                                    const unsigned char* kstr,                 \
                                    int klen,                                  \
...
#define IMPLEMENT_PEM_write_fp(name, type, str, asn1)                          \
  ...
  {                                                                            \
    return PEM_ASN1_write(                                                     \
      (i2d_of_void*)i2d_##asn1, str, out, x, NULL, NULL, 0, NULL, NULL);       \
  }
...
#define IMPLEMENT_PEM_write_cb_fp(name, type, str, asn1)                       \
  ...
  {                                                                            \
    return PEM_ASN1_write(                                                     \
      (i2d_of_void*)i2d_##asn1, str, out, x, enc, kstr, klen, cb, u);          \
  }
...
#include <openssl/symhacks.h>
  ...
  int PEM_write_PKCS8PrivateKey(FILE* fp,
                                ...
                                void* u);
#endif
  EVP_PKEY* PEM_read_bio_Parameters_ex(BIO* bp,
                                       EVP_PKEY** x,
                                       OSSL_LIB_CTX* libctx,
                                       const char* propq);
