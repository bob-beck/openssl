...
static int pref_cmp(const void* va, const void* vb) {
   ...
   */
  if (a->pref > 0 && b->pref > 0) return a->pref - b->pref;
  /* A preference of 0 is "larger" than (sorts after) any nonzero value. */
  return b->pref - a->pref;
}

ML_COMMON_PKCS8_FMT_PREF* ossl_ml_common_pkcs8_fmt_order(
    const char* algorithm_name, const ML_COMMON_PKCS8_FMT* p8fmt,
