...
void *
ossl_prov_import_key (const OSSL_DISPATCH *fns, void *provctx, int selection,
...
void
ossl_prov_free_key (const OSSL_DISPATCH *fns, void *key)
{
  OSSL_FUNC_keymgmt_free_fn *kmgmt_free = ossl_prov_get_keymgmt_free (fns);
...
int
ossl_read_der (PROV_CTX *provctx, OSSL_CORE_BIO *cin, unsigned char **data,
               long *len)
{
  BUF_MEM *mem = NULL;
