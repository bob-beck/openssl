...
ML_KEM_KEY*
ossl_ml_kem_d2i_PUBKEY(const uint8_t* pubenc,
                       int publen,
                       int evp_type,
                       PROV_CTX* provctx,
                       const char* propq)
...
ML_KEM_KEY*
ossl_ml_kem_d2i_PKCS8(const uint8_t* prvenc,
                      int prvlen,
                      int evp_type,
                      PROV_CTX* provctx,
                      const char* propq)
{
  ...
  if (p8fmt->seed_length > 0) {
    if (!ossl_ml_kem_set_seed(
          buf + p8fmt->seed_offset, ML_KEM_SEED_BYTES, key)) {
...
int
ossl_ml_kem_i2d_prvkey(const ML_KEM_KEY* key, uint8_t** out, PROV_CTX* provctx)
{
...
int
ossl_ml_kem_key_to_text(BIO* out, const ML_KEM_KEY* key, int selection)
{
