...
ML_KEM_KEY *
ossl_ml_kem_d2i_PKCS8 (const uint8_t *prvenc, int prvlen, int evp_type,
                       PROV_CTX *provctx, const char *propq)
{
  const ML_KEM_VINFO *v;
  const ML_COMMON_CODEC *codec;
  ...
  if (p8fmt->priv_length > 0)
    {
      if ((key->encoded_dk = OPENSSL_secure_malloc (p8fmt->priv_length))
          == NULL)
        {
...
int
ossl_ml_kem_i2d_prvkey (const ML_KEM_KEY *key, uint8_t **out,
                        PROV_CTX *provctx)
{
