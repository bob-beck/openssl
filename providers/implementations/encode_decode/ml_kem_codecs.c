...
ML_KEM_KEY* ossl_ml_kem_d2i_PUBKEY(const uint8_t* pubenc, int publen,
  ...
  if ((ret = ossl_ml_kem_key_new(libctx, propq, evp_type)) == NULL) return NULL;
...
ML_KEM_KEY* ossl_ml_kem_d2i_PKCS8(const uint8_t* prvenc, int prvlen,
  ...
  if ((ossl_ssize_t)len < (ossl_ssize_t)sizeof(magic)) goto end;
  ...
  for (slot = fmt_slots; (p8fmt = slot->fmt) != NULL; ++slot) {
    if (len != (ossl_ssize_t)p8fmt->p8_bytes) continue;
  ...
  if (p8fmt->seed_length > 0) {
    ...
    if (pos + sizeof(uint16_t) == buf + p8fmt->seed_offset) {
      ...
      if (seed_magic != p8fmt->seed_magic) goto end;
  ...
  if (p8fmt->priv_length > 0) {
    ...
    if (pos + sizeof(uint32_t) == buf + p8fmt->priv_offset) {
      ...
      if (magic != p8fmt->priv_magic) goto end;
  ...
  if (p8fmt->pub_length > 0) {
    if (pos != buf + p8fmt->pub_offset) goto end;
    pos += v->pubkey_bytes;
  }
  if (pos != buf + len) goto end;

  /*
   * Collect the seed and/or key into a "decoded" private key object,
   * to be turned into a real key on provider "load" or "import".
   */
  if ((key = ossl_prov_ml_kem_new(provctx, propq, evp_type)) == NULL) goto end;
  ...
  if (ret == NULL) ossl_ml_kem_key_free(key);
  return ret;
}
...
int ossl_ml_kem_i2d_pubkey(const ML_KEM_KEY* key, unsigned char** out) {
  ...
  if (out != NULL && (*out = OPENSSL_malloc(publen)) == NULL) return 0;
...
int ossl_ml_kem_i2d_prvkey(const ML_KEM_KEY* key, uint8_t** out,
  ...
  /* Not ours to handle */
  if ((codec = ml_kem_get_codec(v->evp_type)) == NULL) return 0;
  ...
  fmt_slots = ossl_ml_common_pkcs8_fmt_order(v->algorithm_name, codec->p8fmt,
                                             "output", formats);
  if (fmt_slots == NULL) return 0;

  /* If we don't have a seed, skip seedful entries */
  for (slot = fmt_slots; (p8fmt = slot->fmt) != NULL; ++slot)
    if (ossl_ml_kem_have_seed(key) || p8fmt->seed_length == 0) break;
  ...
  if ((pos = buf = OPENSSL_malloc((size_t)len)) == NULL) goto end;
...
int ossl_ml_kem_key_to_text(BIO* out, const ML_KEM_KEY* key, int selection) {
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0 &&
    ...
    if (ossl_ml_kem_have_seed(key)) {
      if (!ossl_ml_kem_encode_seed(seed, sizeof(seed), key)) goto end;
      if (!ossl_bio_print_labeled_buf(out, "seed:", seed, sizeof(seed)))
        goto end;
    }
    if (ossl_ml_kem_have_prvkey(key)) {
      if ((prvenc = OPENSSL_malloc(prvlen)) == NULL) return 0;
      if (!ossl_ml_kem_encode_private_key(prvenc, prvlen, key)) goto end;
      if (!ossl_bio_print_labeled_buf(out, "dk:", prvenc, prvlen)) goto end;
    }
    ret = 1;
  }
