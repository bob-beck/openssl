...
static struct msblob2key_ctx_st* msblob2key_newctx(
...
static int msblob2key_decode(void* vctx, OSSL_CORE_BIO* cin, int selection,
  ...
  if ((selection == 0 || (selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) &&
    ...
    if (!ossl_pw_set_ossl_passphrase_cb(&pwdata, pw_cb, pw_cbarg)) goto end;
    p = buf;
    key = ctx->desc->read_private_key(&p, bitlen, ispub);
    if (selection != 0 && key == NULL) goto next;
  }
  if (key == NULL &&
...
static int msblob2key_export_object(void* vctx, const void* reference,
  ...
  if (reference_sz == sizeof(keydata) && export != NULL) {
    ...
    if (selection == 0) selection = OSSL_KEYMGMT_SELECT_ALL;
