...
ML_DSA_KEY *
ossl_ml_dsa_d2i_PKCS8 (const uint8_t *prvenc, int prvlen, int evp_type,
                       PROV_CTX *provctx, const char *propq)
{
  ...
  fmt_slots = ossl_ml_common_pkcs8_fmt_order (v->alg, codec->p8fmt, "input",
                                              formats);
  if (fmt_slots == NULL)
    goto end;
...
int
ossl_ml_dsa_i2d_prvkey (const ML_DSA_KEY *key, uint8_t **out,
                        PROV_CTX *provctx)
{
  const ML_DSA_PARAMS *params = ossl_ml_dsa_key_params (key);
  const ML_COMMON_CODEC *codec;
