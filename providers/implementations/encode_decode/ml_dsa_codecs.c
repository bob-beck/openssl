...
static const ML_COMMON_CODEC*
ml_dsa_get_codec(int evp_type)
{
  switch (evp_type) {
...
ML_DSA_KEY*
ossl_ml_dsa_d2i_PUBKEY(const uint8_t* pk,
                       int pk_len,
                       int evp_type,
                       PROV_CTX* provctx,
                       const char* propq)
...
ML_DSA_KEY*
ossl_ml_dsa_d2i_PKCS8(const uint8_t* prvenc,
                      int prvlen,
                      int evp_type,
                      PROV_CTX* provctx,
                      const char* propq)
{
  ...
  if (ossl_ml_dsa_set_prekey(
        key, 0, 0, seed, ML_DSA_SEED_BYTES, priv, v->sk_len))
    ret = key;
...
int
ossl_ml_dsa_i2d_prvkey(const ML_DSA_KEY* key, uint8_t** out, PROV_CTX* provctx)
{
  const ML_DSA_PARAMS* params = ossl_ml_dsa_key_params(key);
  const ML_COMMON_CODEC* codec;
  ...
  fmt_slots = ossl_ml_common_pkcs8_fmt_order(
    params->alg, codec->p8fmt, "output", formats);
  if (fmt_slots == NULL)
    return 0;
...
int
ossl_ml_dsa_key_to_text(BIO* out, const ML_DSA_KEY* key, int selection)
{
  const ML_DSA_PARAMS* params;
