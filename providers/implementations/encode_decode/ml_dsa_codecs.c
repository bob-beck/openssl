...
ML_DSA_KEY* ossl_ml_dsa_d2i_PUBKEY(const uint8_t* pk, int pk_len, int evp_type,
  ...
  if ((ret = ossl_ml_dsa_key_new(libctx, propq, evp_type)) == NULL) return NULL;
...
ML_DSA_KEY* ossl_ml_dsa_d2i_PKCS8(const uint8_t* prvenc, int prvlen,
  ...
  formats = ossl_prov_ctx_get_param(provctx,
                                    OSSL_PKEY_PARAM_ML_DSA_INPUT_FORMATS, NULL);
  fmt_slots =
   ...
   */
  if ((key = ossl_prov_ml_dsa_new(provctx, propq, evp_type)) == NULL) goto end;
  if (p8fmt->seed_length > 0) seed = buf + p8fmt->seed_offset;
  if (p8fmt->priv_length > 0) priv = buf + p8fmt->priv_offset;
  ...
  if (ret == NULL) ossl_ml_dsa_key_free(key);
  return ret;
}
...
int ossl_ml_dsa_i2d_prvkey(const ML_DSA_KEY* key, uint8_t** out,
  ...
  /* Not ours to handle */
  if ((codec = ml_dsa_get_codec(params->evp_type)) == NULL) return 0;
  ...
  for (slot = fmt_slots; (p8fmt = slot->fmt) != NULL; ++slot)
    if (seed != NULL || p8fmt->seed_length == 0) break;
