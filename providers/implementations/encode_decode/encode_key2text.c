...
static int ec_param_explicit_curve_to_text(BIO* out, const EC_GROUP* group,
  ...
  a = BN_CTX_get(ctx);
  b = BN_CTX_get(ctx);
  if (b == NULL || !EC_GROUP_get_curve(group, p, a, b, ctx)) return 0;
...
static int ec_param_explicit_to_text(BIO* out, const EC_GROUP* group,
  ...
  if (seed != NULL) seed_len = EC_GROUP_get_seed_len(group);
...
static int ec_to_text(BIO* out, const void* key, int selection) {
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0)
    ...
    if (EC_GROUP_get_curve_name(group) != NID_sm2) type_label = "EC-Parameters";
...
static int rsa_to_text(BIO* out, const void* key, int selection) {
  ...
  if (!ossl_bio_print_labeled_bignum(out, modulus_label, rsa_n)) goto err;
  if (!ossl_bio_print_labeled_bignum(out, exponent_label, rsa_e)) goto err;
  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
    int i;
    ...
    for (i = 2; i < sk_BIGNUM_const_num(factors); i++) {
      if (BIO_printf(out, "prime%d:", i + 1) <= 0) goto err;
      if (!ossl_bio_print_labeled_bignum(out, NULL,
                                         sk_BIGNUM_const_value(factors, i)))
        goto err;
      if (BIO_printf(out, "exponent%d:", i + 1) <= 0) goto err;
      if (!ossl_bio_print_labeled_bignum(out, NULL,
                                         sk_BIGNUM_const_value(exps, i)))
        goto err;
      if (BIO_printf(out, "coefficient%d:", i + 1) <= 0) goto err;
      if (!ossl_bio_print_labeled_bignum(out, NULL,
