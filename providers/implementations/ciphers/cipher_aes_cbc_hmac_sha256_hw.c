...
const PROV_CIPHER_HW_AES_HMAC_SHA* ossl_prov_cipher_hw_aes_cbc_hmac_sha256(
    void) {
  return NULL;
}
#else
...
static void sha256_update(SHA256_CTX* c, const void* data, size_t len) {
  ...
  if (res) SHA256_Update(c, ptr, res);
}
...
static int aesni_cbc_hmac_sha256_cipher(PROV_CIPHER_CTX* vctx,
  ...
  if (ctx->base.enc) {
    ...
    if (((OPENSSL_ia32cap_P[2] & (1 << 29)) ||         /* SHAEXT? */
      ...
      if (sctx->md.Nl < (unsigned int)blocks) sctx->md.Nh++;
    } else {
      sha_off = 0;
    }
    sha_off += iv;
    ...
    if (plen != len) { /* "TLS" mode of operation */
      if (in != out) memcpy(out + aes_off, in + aes_off, plen - aes_off);
    ...
    if (plen != NO_PAYLOAD_LENGTH) { /* "TLS" mode of operation */
      ...
      if (len < (iv + SHA256_DIGEST_LENGTH + 1)) return 0;
...
static void aesni_cbc_hmac_sha256_set_mac_key(void* vctx,
  ...
  for (i = 0; i < sizeof(hmac_key); i++) hmac_key[i] ^= 0x36 ^ 0x5c; /* opad */
...
static int aesni_cbc_hmac_sha256_set_tls1_aad(void* vctx,
  ...
  if (aad_len != EVP_AEAD_TLS1_AAD_LEN) return -1;
...
static int aesni_cbc_hmac_sha256_tls1_multiblock_aad(
  ...
  if (ctx->base.enc) {
    if ((param->inp[9] << 8 | param->inp[10]) < TLS1_1_VERSION) return -1;
