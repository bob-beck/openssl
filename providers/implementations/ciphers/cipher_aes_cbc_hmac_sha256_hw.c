...
int
ossl_cipher_capable_aes_cbc_hmac_sha256(void)
{
  return 0;
}
...
void
sha256_block_data_order(void* c, const void* p, size_t len);
int
aesni_cbc_sha256_enc(const void* inp,
                     void* out,
                     size_t blocks,
                     const AES_KEY* key,
                     unsigned char iv[16],
...
static int
aesni_cbc_hmac_sha256_init_key(PROV_CIPHER_CTX* vctx,
...
void
sha256_multi_block(SHA256_MB_CTX*, const HASH_DESC*, int);
void
aesni_multi_cbc_encrypt(CIPH_DESC*, void*, int);

static size_t
tls1_multi_block_encrypt(void* vctx,
                         unsigned char* out,
                         const unsigned char* inp,
                         size_t inp_len,
                         int n4x)
...
static int
aesni_cbc_hmac_sha256_cipher(PROV_CIPHER_CTX* vctx,
                             unsigned char* out,
                             ...
                             size_t len)
{
  ...
  if (ctx->base.enc) {
    ...
    if (((OPENSSL_ia32cap_P[2] & (1 << 29)) ||         /* SHAEXT? */
      ...
      (void)aesni_cbc_sha256_enc(
        in, out, blocks, &ctx->ks, ctx->base.iv, &sctx->md, in + iv + sha_off);
    ...
    if (plen != len) { /* "TLS" mode of operation */
      ...
      aesni_cbc_encrypt(
        out + aes_off, out + aes_off, len - aes_off, &ctx->ks, ctx->base.iv, 1);
    } else {
      aesni_cbc_encrypt(
        in + aes_off, out + aes_off, len - aes_off, &ctx->ks, ctx->base.iv, 1);
    }
  } else {
    union
    {
...
static void
aesni_cbc_hmac_sha256_set_mac_key(void* vctx,
...
static int
aesni_cbc_hmac_sha256_set_tls1_aad(void* vctx,
...
static int
aesni_cbc_hmac_sha256_tls1_multiblock_max_bufsize(void* vctx)
{
  PROV_AES_HMAC_SHA_CTX* ctx = (PROV_AES_HMAC_SHA_CTX*)vctx;
...
aesni_cbc_hmac_sha256_tls1_multiblock_encrypt(
  ...
  EVP_CTRL_TLS1_1_MULTIBLOCK_PARAM* param)
{
  return (int)tls1_multi_block_encrypt(
    ctx, param->out, param->inp, param->len, param->interleave / 4);
}
#endif
