...
static int aes_xts_init(void* vctx, const unsigned char* key, size_t keylen,
  ...
  if (iv != NULL) {
    if (!ossl_cipher_generic_initiv(vctx, iv, ivlen)) return 0;
  }
  if (key != NULL) {
    ...
    if (!aes_xts_check_keys_differ(key, keylen / 2, enc)) return 0;
    if (!ctx->hw->init(ctx, key, keylen)) return 0;
  }
...
#ifdef AES_XTS_S390X
  if (s390x_aes_xts_einit(vctx, key, keylen, iv, ivlen, params) == 1) return 1;
#endif
...
static int aes_xts_dinit(void* vctx, const unsigned char* key, size_t keylen,
                         ...
                         const OSSL_PARAM params[]) {
#ifdef AES_XTS_S390X
  if (s390x_aes_xts_dinit(vctx, key, keylen, iv, ivlen, params) == 1) return 1;
#endif
...
#ifdef AES_XTS_S390X
  if (in->plat.s390x.fc) return s390x_aes_xts_dupctx(vctx);
#endif

  if (in->xts.key1 != NULL) {
    if (in->xts.key1 != &in->ks1) return NULL;
  }
  if (in->xts.key2 != NULL) {
    if (in->xts.key2 != &in->ks2) return NULL;
  }
