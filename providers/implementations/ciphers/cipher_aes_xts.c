               ...
               size_t inl)
{
  ...
  if (ctx->stream != NULL)
    (*ctx->stream)(in, out, inl, ctx->xts.key1, ctx->xts.key2, ctx->base.iv);
             ...
             &ctx->xts, ctx->base.iv, in, out, inl, ctx->base.enc))
    return 0;
...
static int
aes_xts_stream_update(void* vctx,
                      unsigned char* out,
                      size_t* outl,
                      size_t outsize,
                      const unsigned char* in,
...
static int
aes_xts_stream_final(void* vctx,
                     unsigned char* out,
                     size_t* outl,
                     size_t outsize)
{
