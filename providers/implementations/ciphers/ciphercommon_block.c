 ...
 */
size_t ossl_cipher_fillblock(unsigned char* buf,
                             size_t* buflen,
                             size_t blocksize,
 ...
 */
int ossl_cipher_trailingdata(unsigned char* buf,
                             size_t* buflen,
                             size_t blocksize,
                             const unsigned char** in,
                             size_t* inlen) {
...
void ossl_cipher_padblock(unsigned char* buf,
                          size_t* buflen,
                          size_t blocksize) {
  size_t i;
...
int ossl_cipher_unpadblock(unsigned char* buf,
                           size_t* buflen,
                           size_t blocksize) {
  size_t pad, i;
  size_t len = *buflen;
 ...
 *      the mac is random
 */
int ossl_cipher_tlsunpadblock(OSSL_LIB_CTX* libctx,
  ...
  switch (tlsversion) {
    case SSL3_VERSION:
      return ssl3_cbc_remove_padding_and_mac(buflen, *buflen, buf, mac, alloced,
                                             blocksize, macsize, libctx);
    ...
    case DTLS1_BAD_VER:
      ...
      *buflen -= blocksize;
      /* Fall through */
    case TLS1_VERSION:
      ret = tls1_cbc_remove_padding_and_mac(buflen, *buflen, buf, mac, alloced,
