...
static int
aes_gcm_initkey(PROV_GCM_CTX* ctx, const unsigned char* key, size_t keylen)
{
  ...
  if (HWAES_CAPABLE) {
    ...
    GCM_HW_SET_KEY_CTR_FN(
      ks, HWAES_set_encrypt_key, HWAES_encrypt, HWAES_ctr32_encrypt_blocks);
#else
    ...
    if (BSAES_CAPABLE) {
    GCM_HW_SET_KEY_CTR_FN(
      ks, AES_set_encrypt_key, AES_encrypt, ossl_bsaes_ctr32_encrypt_blocks);
  } else
#endif /* BSAES_CAPABLE */
...
#ifdef AES_CTR_ASM
    GCM_HW_SET_KEY_CTR_FN(
      ks, AES_set_encrypt_key, AES_encrypt, AES_ctr32_encrypt);
#else
...
static int
generic_aes_gcm_cipher_update(PROV_GCM_CTX* ctx,
  ...
  if (ctx->enc) {
    if (ctx->ctr != NULL) {
      ...
      if (CRYPTO_gcm128_encrypt_ctr32(
            &ctx->gcm, in + bulk, out + bulk, len - bulk, ctx->ctr))
        return 0;
#else
