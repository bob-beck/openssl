...
int ossl_gcm_cipher_update(PROV_GCM_CTX* ctx, const unsigned char* in,
  ...
  if (ctx->enc) {
    ...
    if (CRYPTO_gcm128_decrypt(&ctx->gcm, in, out, len)) return 0;
  }
  return 1;
}
...
int ossl_gcm_cipher_final(PROV_GCM_CTX* ctx, unsigned char* tag) {
  if (ctx->enc) {
    ...
    if (CRYPTO_gcm128_finish(&ctx->gcm, tag, ctx->taglen) != 0) return 0;
  }
  return 1;
}
...
int ossl_gcm_one_shot(PROV_GCM_CTX* ctx, unsigned char* aad, size_t aad_len,
  ...
  /* Use saved AAD */
  if (!ctx->hw->aadupdate(ctx, aad, aad_len)) goto err;
  if (!ctx->hw->cipherupdate(ctx, in, in_len, out)) goto err;
  ctx->taglen = GCM_TAG_MAX_SIZE;
  if (!ctx->hw->cipherfinal(ctx, tag)) goto err;
  ret = 1;
