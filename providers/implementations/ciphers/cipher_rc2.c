...
static int rc2_einit(void* ctx, const unsigned char* key, size_t keylen,
  ...
  if (!ossl_cipher_generic_einit(ctx, key, keylen, iv, ivlen, NULL)) return 0;
...
static int rc2_dinit(void* ctx, const unsigned char* key, size_t keylen,
                     const unsigned char* iv, size_t ivlen,
                     const OSSL_PARAM params[]) {
  if (!ossl_cipher_generic_dinit(ctx, key, keylen, iv, ivlen, NULL)) return 0;
  return rc2_set_ctx_params(ctx, params);
}
...
static int rc2_get_ctx_params(void* vctx, OSSL_PARAM params[]) {
  ...
  if (!ossl_cipher_generic_get_ctx_params(vctx, params)) return 0;
  ...
  if (p1 != NULL || p2 != NULL) {
    ...
    if (p1 != NULL && i >= 0) p1->return_size = (size_t)i;

    /*
     * If the buffers differ, redo the i2d on the second buffer.
     * Otherwise, just use |i| as computed above
     */
    if (d1 != d2) i = i2d_ASN1_TYPE(type, dd2);
    if (p2 != NULL && i >= 0) p2->return_size = (size_t)i;
...
static int rc2_set_ctx_params(void* vctx, const OSSL_PARAM params[]) {
  ...
  if (!ossl_cipher_var_keylen_set_ctx_params(vctx, params)) return 0;
  ...
  if (p != NULL) {
    ...
    if (p->data_type != OSSL_PARAM_OCTET_STRING ||
        ...
        ((size_t)ASN1_TYPE_get_int_octetstring(
             type, &num, iv, (int)ctx->base.ivlen) != ctx->base.ivlen) ||
