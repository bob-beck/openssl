...
static int
rc2_keybits_to_magic(int keybits)
{
  switch (keybits) {
    case 128:
...
static int
rc2_magic_to_keybits(int magic)
{
  switch (magic) {
    case RC2_128_MAGIC:
      return 128;
...
static int
rc2_get_ctx_params(void* vctx, OSSL_PARAM params[])
{
  PROV_RC2_CTX* ctx = (PROV_RC2_CTX*)vctx;
  OSSL_PARAM *p, *p1, *p2;
  ...
  if (p1 != NULL || p2 != NULL) {
    ...
    if (!ASN1_TYPE_set_int_octetstring(
          type, num, ctx->base.iv, (int)ctx->base.ivlen)) {
      ASN1_TYPE_free(type);
      ERR_raise(ERR_LIB_PROV, ERR_R_ASN1_LIB);
      return 0;
    }
...
static int
rc2_set_ctx_params(void* vctx, const OSSL_PARAM params[])
{
  PROV_RC2_CTX* ctx = (PROV_RC2_CTX*)vctx;
  const OSSL_PARAM* p;
