...
#define SM4_HW_CCM_SET_KEY_FN(fn_set_enc_key, fn_blk, fn_ccm_enc, fn_ccm_dec)  \
  ...
  CRYPTO_ccm128_init(&ctx->ccm_ctx,                                            \
                     (unsigned int)ctx->m,                                     \
                     (unsigned int)ctx->l,                                     \
                     &actx->ks.ks,                                             \
                     (block128_f)fn_blk);                                      \
...
static int
ccm_sm4_initkey(PROV_CCM_CTX* ctx, const unsigned char* key, size_t keylen)
{
  ...
  if (HWSM4_CAPABLE) {
    ...
    if (VPSM4_EX_CAPABLE) {
    SM4_HW_CCM_SET_KEY_FN(
      vpsm4_ex_set_encrypt_key, vpsm4_ex_encrypt, NULL, NULL);
  } else
...
#else
const PROV_CCM_HW*
ossl_prov_sm4_hw_ccm(size_t keybits)
{
  return &ccm_sm4;
}
#endif
