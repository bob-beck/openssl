 ...
 * stolen ciphertext to the partial block, which can then be decrypted as
 usual.

 * AES_CBC_CTS has 3 variants:
 *  (1) CS1 The NIST variant.
 *      If the length is a multiple of the blocksize it is the same as CBC
 mode.
...
const char *
ossl_cipher_cbc_cts_mode_id2name (unsigned int id)
{
  size_t i;
...
int
ossl_cipher_cbc_cts_mode_name2id (const char *name)
{
  size_t i;
...
static size_t
cts128_cs1_encrypt (PROV_CIPHER_CTX *ctx, const unsigned char *in,
...
static void
do_xor (const unsigned char *in1, const unsigned char *in2, size_t len,
        unsigned char *out)
{
  size_t i;
...
static size_t
cts128_cs1_decrypt (PROV_CIPHER_CTX *ctx, const unsigned char *in,
                    unsigned char *out, size_t len)
{
...
static size_t
cts128_cs3_encrypt (PROV_CIPHER_CTX *ctx, const unsigned char *in,
                    unsigned char *out, size_t len)
{
  aligned_16bytes tmp_in;
  size_t residue;
 ...
 */
static size_t
cts128_cs3_decrypt (PROV_CIPHER_CTX *ctx, const unsigned char *in,
                    unsigned char *out, size_t len)
{
  aligned_16bytes mid_iv, ct_mid, cn, pt_last;
  size_t residue;
...
static size_t
cts128_cs2_encrypt (PROV_CIPHER_CTX *ctx, const unsigned char *in,
                    unsigned char *out, size_t len)
{
...
static size_t
cts128_cs2_decrypt (PROV_CIPHER_CTX *ctx, const unsigned char *in,
                    unsigned char *out, size_t len)
{
  if (len % CTS_BLOCK_SIZE == 0)
    {
...
int
ossl_cipher_cbc_cts_block_update (void *vctx, unsigned char *out, size_t *outl,
...
int
ossl_cipher_cbc_cts_block_final (void *vctx, unsigned char *out, size_t *outl,
                                 size_t outsize)
{
  *outl = 0;
  return 1;
