...
static int aes_set_ctx_params(void* vctx, const OSSL_PARAM params[]) {
  ...
  if (p != NULL) {
    ...
    if (hw->tls1_multiblock_aad(vctx, &mb_param) <= 0) return 0;
  }
  ...
  if (p != NULL) {
    ...
    if (hw->tls1_multiblock_encrypt(vctx, &mb_param) <= 0) return 0;
  }
#endif /* !defined(OPENSSL_NO_MULTIBLOCK) */
  ...
  if (p != NULL) {
    ...
    if (hw->set_tls1_aad(ctx, p->data, (int)p->data_size) <= 0) return 0;
  }
