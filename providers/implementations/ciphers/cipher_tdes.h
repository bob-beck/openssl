...
#define IMPLEMENT_tdes_cipher(                                                 \
  type, UCTYPE, lcmode, UCMODE, flags, kbits, blkbits, ivbits, block)          \
  static OSSL_FUNC_cipher_newctx_fn tdes_##type##_##lcmode##_newctx;           \
  static void* tdes_##type##_##lcmode##_newctx(void* provctx)                  \
  {                                                                            \
    return ossl_tdes_newctx(provctx,                                           \
...
int
ossl_tdes_get_params(OSSL_PARAM params[],
                     unsigned int md,
                     uint64_t flags,
                     size_t kbits,
                     size_t blkbits,
...
int
ossl_cipher_hw_tdes_ede3_initkey(PROV_CIPHER_CTX* ctx,
                                 const unsigned char* key,
                                 size_t keylen);
void
ossl_cipher_hw_tdes_copyctx(PROV_CIPHER_CTX* dst, const PROV_CIPHER_CTX* src);
int
ossl_cipher_hw_tdes_cbc(PROV_CIPHER_CTX* ctx,
                        unsigned char* out,
                        const unsigned char* in,
                        size_t inl);
int
ossl_cipher_hw_tdes_ecb(PROV_CIPHER_CTX* ctx,
                        unsigned char* out,
                        const unsigned char* in,
                        size_t len);

const PROV_CIPHER_HW*
ossl_prov_cipher_hw_tdes_ede3_cbc(void);
const PROV_CIPHER_HW*
ossl_prov_cipher_hw_tdes_ede3_ecb(void);
