           ...
           const PROV_CIPHER_HW* hw)
{
  ...
  if (ctx != NULL)
    ossl_cipher_generic_initkey(
      ctx, kbits, blkbits, ivbits, mode, flags, hw, provctx);
  return ctx;
}
...
#define IMPLEMENT_des_cipher(                                                  \
  type, lcmode, UCMODE, flags, kbits, blkbits, ivbits, block)                  \
  static OSSL_FUNC_cipher_newctx_fn type##_##lcmode##_newctx;                  \
  static void* des_##lcmode##_newctx(void* provctx)                            \
  {                                                                            \
    return des_newctx(provctx,                                                 \
                      kbits,                                                   \
                      blkbits,                                                 \
                      ivbits,                                                  \
