  ...
  if (ctx->xts_standard) {
    ...
    if (ctx->stream_gb != NULL)
      (*ctx->stream_gb)(in,
                        ...
                        ctx->base.iv,
                        ctx->base.enc);
    else if (ossl_crypto_xts128gb_encrypt(
               &ctx->xts, ctx->base.iv, in, out, inl, ctx->base.enc))
      return 0;
  }
  *outl = inl;
...
static int
sm4_xts_stream_update(void* vctx,
                      unsigned char* out,
                      size_t* outl,
                      size_t outsize,
                      const unsigned char* in,
...
static int
sm4_xts_stream_final(void* vctx,
                     unsigned char* out,
                     size_t* outl,
                     size_t outsize)
{
