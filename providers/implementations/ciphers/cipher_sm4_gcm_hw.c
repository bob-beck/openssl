...
static int
sm4_gcm_initkey(PROV_GCM_CTX* ctx, const unsigned char* key, size_t keylen)
{
  ...
  if (HWSM4_CAPABLE) {
    ...
    SM4_GCM_HW_SET_KEY_CTR_FN(
      ks, HWSM4_set_encrypt_key, HWSM4_encrypt, HWSM4_ctr32_encrypt_blocks);
    ...
    if (VPSM4_CAPABLE) {
    SM4_GCM_HW_SET_KEY_CTR_FN(
      ks, vpsm4_set_encrypt_key, vpsm4_encrypt, vpsm4_ctr32_encrypt_blocks);
  } else
#endif /* VPSM4_CAPABLE */
  {
...
static int
hw_gcm_cipher_update(PROV_GCM_CTX* ctx,
...
#else
const PROV_GCM_HW*
ossl_prov_sm4_hw_gcm(size_t keybits)
{
  return &sm4_gcm;
}
#endif
