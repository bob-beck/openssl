...
static int aes_siv_initkey(void* vctx, const unsigned char* key,
  ...
  if (ctx->cbc == NULL || ctx->ctr == NULL) return 0;
  /*
...
static int aes_siv_dupctx(void* in_vctx, void* out_vctx) {
  ...
  if (in->cbc != NULL && !EVP_CIPHER_up_ref(in->cbc)) return 0;
  ...
  *out = *in;
  ...
  out->siv.mac = NULL;
  if (!ossl_siv128_copy_ctx(&out->siv, &in->siv)) return 0;
...
static int aes_siv_cipher(void* vctx, unsigned char* out,
  ...
  if (in == NULL) return ossl_siv128_finish(sctx) == 0;

  /* Deal with associated data */
  if (out == NULL) return (ossl_siv128_aad(sctx, in, len) == 1);

  if (ctx->enc) return ossl_siv128_encrypt(sctx, in, out, len) > 0;
