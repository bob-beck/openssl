...
static ossl_inline int aes_generic_ocb_cipher(PROV_AES_OCB_CTX* ctx,
  ...
  if (ctx->base.enc) {
    if (!CRYPTO_ocb128_encrypt(&ctx->ocb, in, out, len)) return 0;
  } else {
    if (!CRYPTO_ocb128_decrypt(&ctx->ocb, in, out, len)) return 0;
  }
  return 1;
}
...
static int aes_ocb_init(void* vctx, const unsigned char* key, size_t keylen,
  ...
  if (iv != NULL) {
    ...
    if (!ossl_cipher_generic_initiv(&ctx->base, iv, ivlen)) return 0;
...
static int aes_ocb_block_final(void* vctx, unsigned char* out, size_t* outl,
  ...
  *outl = 0;
  ...
  if (ctx->aad_buf_len > 0) {
    if (!aes_generic_ocb_setaad(ctx, ctx->aad_buf, ctx->aad_buf_len)) return 0;
    ctx->aad_buf_len = 0;
  }
  if (ctx->base.enc) {
    /* If encrypting then just get the tag */
    if (!aes_generic_ocb_gettag(ctx, ctx->tag, ctx->taglen)) return 0;
  } else {
    /* If decrypting then verify */
    if (ctx->taglen == 0) return 0;
...
static int aes_ocb_set_ctx_params(void* vctx, const OSSL_PARAM params[]) {
  ...
  if (p != NULL) {
    ...
    /* IV len must be 1 to 15 */
    if (sz < OCB_MIN_IV_LEN || sz > OCB_MAX_IV_LEN) return 0;
...
static const OSSL_PARAM cipher_ocb_known_gettable_ctx_params[] = {
    ...
    OSSL_PARAM_END};
static const OSSL_PARAM* cipher_ocb_gettable_ctx_params(
    ossl_unused void* cctx, ossl_unused void* p_ctx) {
...
static const OSSL_PARAM cipher_ocb_known_settable_ctx_params[] = {
    ...
    OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_AEAD_TAG, NULL, 0),
    OSSL_PARAM_END};
static const OSSL_PARAM* cipher_ocb_settable_ctx_params(
    ossl_unused void* cctx, ossl_unused void* p_ctx) {
