...
aes_generic_ocb_setiv(PROV_AES_OCB_CTX* ctx,
...
static ossl_inline int
aes_generic_ocb_setaad(PROV_AES_OCB_CTX* ctx,
...
static ossl_inline int
aes_generic_ocb_gettag(PROV_AES_OCB_CTX* ctx, unsigned char* tag, size_t tlen)
{
  return CRYPTO_ocb128_tag(&ctx->ocb, tag, tlen) > 0;
}

static ossl_inline int
aes_generic_ocb_final(PROV_AES_OCB_CTX* ctx)
{
  return (CRYPTO_ocb128_finish(&ctx->ocb, ctx->tag, ctx->taglen) == 0);
}

static ossl_inline void
aes_generic_ocb_cleanup(PROV_AES_OCB_CTX* ctx)
{
  CRYPTO_ocb128_cleanup(&ctx->ocb);
}

static ossl_inline int
aes_generic_ocb_cipher(PROV_AES_OCB_CTX* ctx,
                       const unsigned char* in,
                       unsigned char* out,
                       size_t len)
{
...
static ossl_inline int
aes_generic_ocb_copy_ctx(PROV_AES_OCB_CTX* dst, PROV_AES_OCB_CTX* src)
{
  return CRYPTO_ocb128_copy_ctx(
    &dst->ocb, &src->ocb, &dst->ksenc.ks, &dst->ksdec.ks);
}
 ...
 */
static int
aes_ocb_block_update_internal(PROV_AES_OCB_CTX* ctx,
                              unsigned char* buf,
                              size_t* bufsz,
                              unsigned char* out,
                              size_t* outl,
...
static int
cipher_updateaad(PROV_AES_OCB_CTX* ctx,
                 const unsigned char* in,
                 unsigned char* out,
                 size_t len)
{
...
static int
update_iv(PROV_AES_OCB_CTX* ctx)
{
...
static int
aes_ocb_block_update(void* vctx,
                     unsigned char* out,
                     size_t* outl,
                     size_t outsize,
                     const unsigned char* in,
                     size_t inl)
{
  ...
  return aes_ocb_block_update_internal(
    ctx, buf, buflen, out, outl, outsize, in, inl, fn);
}

static int
aes_ocb_block_final(void* vctx,
                    unsigned char* out,
                    size_t* outl,
                    size_t outsize)
{
               ...
               uint64_t flags)
{
  ...
  if (ctx != NULL) {
    ossl_cipher_generic_initkey(ctx,
                                kbits,
                                blkbits,
                                ivbits,
                                mode,
...
static int
aes_ocb_set_ctx_params(void* vctx, const OSSL_PARAM params[])
{
  PROV_AES_OCB_CTX* ctx = (PROV_AES_OCB_CTX*)vctx;
  const OSSL_PARAM* p;
  size_t sz;
...
static int
aes_ocb_get_ctx_params(void* vctx, OSSL_PARAM params[])
{
  PROV_AES_OCB_CTX* ctx = (PROV_AES_OCB_CTX*)vctx;
  OSSL_PARAM* p;
  ...
  if (p != NULL) {
    ...
    if (!OSSL_PARAM_set_octet_string_or_ptr(
          p, ctx->base.oiv, ctx->base.ivlen)) {
      ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_TO_SET_PARAMETER);
      return 0;
    }
  }
...
cipher_ocb_gettable_ctx_params(ossl_unused void* cctx, ossl_unused void* p_ctx)
{
...
static const OSSL_PARAM*
cipher_ocb_settable_ctx_params(ossl_unused void* cctx, ossl_unused void* p_ctx)
{
  return cipher_ocb_known_settable_ctx_params;
}

static int
aes_ocb_cipher(void* vctx,
               unsigned char* out,
               size_t* outl,
               size_t outsize,
               const unsigned char* in,
...
#define IMPLEMENT_cipher(mode, UCMODE, flags, kbits, blkbits, ivbits)          \
  ...
  {                                                                            \
    return aes_##mode##_newctx(                                                \
      provctx, kbits, blkbits, ivbits, EVP_CIPH_##UCMODE##_MODE, flags);       \
  }                                                                            \
