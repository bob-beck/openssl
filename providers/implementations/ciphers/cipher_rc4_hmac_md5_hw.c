...
static int cipher_hw_rc4_hmac_md5_cipher(PROV_CIPHER_CTX* bctx,
  ...
  if (ctx->base.enc) {
    ...
    if (plen > md5_off && (blocks = (plen - md5_off) / MD5_CBLOCK) &&
      ...
      if (ctx->md.Nl < (unsigned int)blocks) ctx->md.Nh++;
    } else {
      rc4_off = 0;
      md5_off = 0;
    }
    ...
    if (plen != len) { /* "TLS" mode of operation */
      if (in != out) memcpy(out + rc4_off, in + rc4_off, plen - rc4_off);
    ...
    if (plen != NO_PAYLOAD_LENGTH) {
      ...
      if (CRYPTO_memcmp(out + plen, mac, MD5_DIGEST_LENGTH)) return 0;
    } else {
...
static int cipher_hw_rc4_hmac_md5_tls_init(PROV_CIPHER_CTX* bctx,
  ...
  if (aad_len != EVP_AEAD_TLS1_AAD_LEN) return 0;

  len = aad[aad_len - 2] << 8 | aad[aad_len - 1];

  if (!bctx->enc) {
    if (len < MD5_DIGEST_LENGTH) return 0;
