...
static int
cipher_hw_rc4_hmac_md5_initkey(PROV_CIPHER_CTX* bctx,
                               const uint8_t* key,
                               size_t keylen)
{
...
static int
cipher_hw_rc4_hmac_md5_cipher(PROV_CIPHER_CTX* bctx,
                              unsigned char* out,
                              ...
                              size_t len)
{
  ...
  if (ctx->base.enc) {
    ...
    if (plen > md5_off && (blocks = (plen - md5_off) / MD5_CBLOCK) &&
      ...
      rc4_md5_enc(
        ks, in + rc4_off, out + rc4_off, &ctx->md, in + md5_off, blocks);
      blocks *= MD5_CBLOCK;
      rc4_off += blocks;
      md5_off += blocks;
    ...
    if (len > rc4_off && (blocks = (len - rc4_off) / MD5_CBLOCK) &&
      ...
      rc4_md5_enc(
        ks, in + rc4_off, out + rc4_off, &ctx->md, out + md5_off, blocks);
      blocks *= MD5_CBLOCK;
      rc4_off += blocks;
      md5_off += blocks;
...
static int
cipher_hw_rc4_hmac_md5_tls_init(PROV_CIPHER_CTX* bctx,
                                unsigned char* aad,
                                size_t aad_len)
{
  PROV_RC4_HMAC_MD5_CTX* ctx = (PROV_RC4_HMAC_MD5_CTX*)bctx;
...
static void
cipher_hw_rc4_hmac_md5_init_mackey(PROV_CIPHER_CTX* bctx,
...
ossl_prov_cipher_hw_rc4_hmac_md5(size_t keybits)
{
