...
static int aes_gcm_siv_aad(PROV_AES_GCM_SIV_CTX* ctx, const unsigned char* aad,
  ...
  len64 = to_alloc;
  if (len64 > ((uint64_t)1 << 36)) return 0;
...
static int aes_gcm_siv_finish(PROV_AES_GCM_SIV_CTX* ctx) {
  ...
  if (ctx->enc) return ctx->generated_tag;
...
static int aes_gcm_siv_encrypt(PROV_AES_GCM_SIV_CTX* ctx,
  ...
  ctx->generated_tag = 0;
  if (!ctx->speed && ctx->used_enc) return 0;
  /* need to check the size of the input! */
  if (len64 > ((int64_t)1 << 36)) return 0;
...
static int aes_gcm_siv_decrypt(PROV_AES_GCM_SIV_CTX* ctx,
  ...
  ctx->generated_tag = 0;
  if (!ctx->speed && ctx->used_dec) return 0;
  /* need to check the size of the input! */
  if (len64 > ((int64_t)1 << 36)) return 0;
...
static int aes_gcm_siv_cipher(void* vctx, unsigned char* out,
  ...
  if (in == NULL) return aes_gcm_siv_finish(ctx);

  /* Deal with associated data */
  if (out == NULL) return aes_gcm_siv_aad(ctx, in, len);

  if (ctx->enc) return aes_gcm_siv_encrypt(ctx, in, out, len);
...
static int aes_gcm_siv_dup_ctx(void* vdst, void* vsrc) {
  ...
  dst->ecb_ctx = NULL;
  if (src->ecb_ctx != NULL) {
    if ((dst->ecb_ctx = EVP_CIPHER_CTX_new()) == NULL) goto err;
    if (!EVP_CIPHER_CTX_copy(dst->ecb_ctx, src->ecb_ctx)) goto err;
  }
  return 1;
...
static int aes_gcm_siv_ctr32(PROV_AES_GCM_SIV_CTX* ctx,
  ...
  for (i = 0; i < len; i += sizeof(block)) {
    ...
    todo = len - i;
    if (todo > sizeof(keystream)) todo = sizeof(keystream);
