...
static int
aes_gcm_siv_ctr32(PROV_AES_GCM_SIV_CTX* ctx,
...
aes_gcm_siv_initkey(void* vctx)
{
  ...
  for (i = 0; i < BLOCK_SIZE; i += 8) {
    ...
    if (!EVP_EncryptUpdate(
          ctx->ecb_ctx, output, &out_len, data.block, BLOCK_SIZE))
      goto err;
...
static int
aes_gcm_siv_aad(PROV_AES_GCM_SIV_CTX* ctx, const unsigned char* aad, size_t len)
{
  size_t to_alloc;
  uint8_t* ptr;
  uint64_t len64;
...
static int
aes_gcm_siv_finish(PROV_AES_GCM_SIV_CTX* ctx)
{
  int ret = 0;
...
static int
aes_gcm_siv_encrypt(PROV_AES_GCM_SIV_CTX* ctx,
                    const unsigned char* in,
                    unsigned char* out,
                    size_t len)
{
...
static int
aes_gcm_siv_decrypt(PROV_AES_GCM_SIV_CTX* ctx,
                    const unsigned char* in,
                    unsigned char* out,
                    size_t len)
{
  ...
  if (!IS16(len)) {
    ...
    ossl_polyval_ghash_hash(
      ctx->Htable, S_s, (uint8_t*)padding, sizeof(padding));
  }
...
static void
aes_gcm_siv_clean_ctx(void* vctx)
{
  PROV_AES_GCM_SIV_CTX* ctx = (PROV_AES_GCM_SIV_CTX*)vctx;
...
static int
aes_gcm_siv_dup_ctx(void* vdst, void* vsrc)
{
                  ...
                  size_t len)
{
  ...
  for (i = 0; i < len; i += sizeof(block)) {
    ...
    error |= !EVP_EncryptUpdate(
      ctx->ecb_ctx, keystream, &out_len, (uint8_t*)&block, sizeof(block));
    if (IS_LITTLE_ENDIAN) {
      block.x32[0]++;
    } else {
      counter++;
