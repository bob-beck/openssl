...
typedef size_t (*aeswrap_fn)(void* key,
                ...
                uint64_t flags)
{
  ...
  if (ctx != NULL) {
    ossl_cipher_generic_initkey(
      ctx, kbits, blkbits, ivbits, mode, flags, NULL, NULL);
...
static int
aes_wrap_cipher_internal(void* vctx,
                         unsigned char* out,
                         const unsigned char* in,
                         size_t inlen)
{
  ...
  rv = wctx->wrapfn(
    &wctx->ks.ks, ctx->iv_set ? ctx->iv : NULL, out, in, inlen, ctx->block);
  if (!rv) {
...
static int
aes_wrap_final(void* vctx, unsigned char* out, size_t* outl, size_t outsize)
{
  if (!ossl_prov_is_running())
    return 0;
...
static int
aes_wrap_set_ctx_params(void* vctx, const OSSL_PARAM params[])
{
  PROV_CIPHER_CTX* ctx = (PROV_CIPHER_CTX*)vctx;
  const OSSL_PARAM* p;
  size_t keylen = 0;
...
#define IMPLEMENT_cipher(mode, fname, UCMODE, flags, kbits, blkbits, ivbits)   \
  ...
  {                                                                            \
    return aes_##mode##_newctx(                                                \
      kbits, blkbits, ivbits, EVP_CIPH_##UCMODE##_MODE, flags);                \
  }                                                                            \
