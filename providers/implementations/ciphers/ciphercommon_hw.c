 ...
 */
int ossl_cipher_hw_generic_cbc(PROV_CIPHER_CTX* dat,
                               unsigned char* out,
...
int ossl_cipher_hw_generic_ecb(PROV_CIPHER_CTX* dat,
                               unsigned char* out,
                               const unsigned char* in,
                               size_t len) {
...
int ossl_cipher_hw_generic_ofb128(PROV_CIPHER_CTX* dat,
                                  unsigned char* out,
                                  const unsigned char* in,
                                  size_t len) {
  int num = dat->num;
...
int ossl_cipher_hw_generic_cfb128(PROV_CIPHER_CTX* dat,
                                  unsigned char* out,
                                  const unsigned char* in,
                                  size_t len) {
  int num = dat->num;
...
int ossl_cipher_hw_generic_cfb8(PROV_CIPHER_CTX* dat,
                                unsigned char* out,
                                const unsigned char* in,
                                size_t len) {
  int num = dat->num;
...
int ossl_cipher_hw_generic_cfb1(PROV_CIPHER_CTX* dat,
                                unsigned char* out,
                                const unsigned char* in,
                                size_t len) {
  int num = dat->num;
  ...
  while (len >= MAXBITCHUNK) {
    CRYPTO_cfb128_1_encrypt(in, out, MAXBITCHUNK * 8, dat->ks, dat->iv, &num,
                            dat->enc, dat->block);
    len -= MAXBITCHUNK;
    out += MAXBITCHUNK;
    in += MAXBITCHUNK;
  ...
  if (len)
    CRYPTO_cfb128_1_encrypt(in, out, len * 8, dat->ks, dat->iv, &num, dat->enc,
                            dat->block);
...
int ossl_cipher_hw_generic_ctr(PROV_CIPHER_CTX* dat,
                               unsigned char* out,
                               const unsigned char* in,
                               size_t len) {
  ...
  if (dat->stream.ctr)
    CRYPTO_ctr128_encrypt_ctr32(in, out, len, dat->ks, dat->iv, dat->buf, &num,
                                dat->stream.ctr);
  else
...
int ossl_cipher_hw_chunked_cbc(PROV_CIPHER_CTX* ctx,
                               unsigned char* out,
                               const unsigned char* in,
                               size_t inl) {
  while (inl >= MAXCHUNK) {
...
int ossl_cipher_hw_chunked_cfb8(PROV_CIPHER_CTX* ctx,
                                unsigned char* out,
                                const unsigned char* in,
                                size_t inl) {
...
int ossl_cipher_hw_chunked_cfb128(PROV_CIPHER_CTX* ctx,
                                  unsigned char* out,
                                  const unsigned char* in,
                                  size_t inl) {
  size_t chunk = MAXCHUNK;
...
int ossl_cipher_hw_chunked_ofb128(PROV_CIPHER_CTX* ctx,
                                  unsigned char* out,
                                  const unsigned char* in,
                                  size_t inl) {
  while (inl >= MAXCHUNK) {
