...
static int
aesni_cbc_hmac_sha1_init_key (PROV_CIPHER_CTX *vctx, const unsigned char *key,
                              size_t keylen)
{
  int ret;
...
static size_t
tls1_multi_block_encrypt (void *vctx, unsigned char *out,
                          const unsigned char *inp, size_t inp_len, int n4x)
{ /* n4x is 1 or 2 */
  PROV_AES_HMAC_SHA_CTX *ctx = (PROV_AES_HMAC_SHA_CTX *)vctx;
  PROV_AES_HMAC_SHA1_CTX *sctx = (PROV_AES_HMAC_SHA1_CTX *)vctx;
...
static int
aesni_cbc_hmac_sha1_cipher (PROV_CIPHER_CTX *vctx, unsigned char *out,
  ...
  else
    {
      ...
      if (plen != NO_PAYLOAD_LENGTH)
        { /* "TLS" mode of operation */
          ...
          bitlen = sctx->md.Nl
                   + (unsigned int)(inp_len << 3); /* at most 18 bits */
#ifdef BSWAP4
...
static void
aesni_cbc_hmac_sha1_set_mac_key (void *vctx, const unsigned char *mac,
                                 size_t len)
{
...
static int
aesni_cbc_hmac_sha1_set_tls1_aad (void *vctx, unsigned char *aad_rec,
                                  int aad_len)
{
  PROV_AES_HMAC_SHA_CTX *ctx = (PROV_AES_HMAC_SHA_CTX *)vctx;
  PROV_AES_HMAC_SHA1_CTX *sctx = (PROV_AES_HMAC_SHA1_CTX *)vctx;
  ...
  if (ctx->base.enc)
    {
      ...
      ctx->tls_aad_pad = (int)(((len + SHA_DIGEST_LENGTH + AES_BLOCK_SIZE)
                                & -AES_BLOCK_SIZE)
                               - len);
      return 1;
    }
