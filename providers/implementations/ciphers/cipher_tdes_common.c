...
static int tdes_init(void* vctx, const unsigned char* key, size_t keylen,
  ...
  if (iv != NULL) {
    if (!ossl_cipher_generic_initiv(ctx, iv, ivlen)) return 0;
  ...
  if (key != NULL) {
    ...
    if (!ctx->hw->init(ctx, key, ctx->keylen)) return 0;
    ctx->key_set = 1;
  }
  if (!ossl_tdes_set_ctx_params(ctx, params)) return 0;
#ifdef FIPS_MODULE
  if (!tdes_encrypt_check_approved((PROV_TDES_CTX*)ctx, enc)) return 0;
#endif
  return 1;
}
...
OSSL_PARAM_octet_string(OSSL_CIPHER_PARAM_RANDOM_KEY, NULL, 0),
  ...
  if (kl == 0 || RAND_priv_bytes_ex(ctx->libctx, ptr, kl, 0) <= 0) return 0;
