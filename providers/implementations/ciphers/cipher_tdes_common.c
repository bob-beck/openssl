...
static int tdes_encrypt_check_approved(PROV_TDES_CTX *ctx, int enc)
{
    ...
    if (enc && !OSSL_FIPS_IND_ON_UNAPPROVED(ctx, OSSL_FIPS_IND_SETTABLE0, ctx->base.libctx, "Triple-DES", "Encryption",
    ...
    if (iv != NULL)
    {
        if (!ossl_cipher_generic_initiv(ctx, iv, ivlen))
            return 0;
             ...
             (ctx->mode == EVP_CIPH_CBC_MODE || ctx->mode == EVP_CIPH_CFB_MODE || ctx->mode == EVP_CIPH_OFB_MODE))
    {
