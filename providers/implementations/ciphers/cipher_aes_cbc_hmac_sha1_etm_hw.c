...
int
ossl_cipher_capable_aes_cbc_hmac_sha1_etm(void)
{
  return 0;
}
...
int
asm_aescbc_sha1_hmac(const uint8_t* csrc,
                     uint8_t* cdst,
                     uint64_t clen,
                     uint8_t* dsrc,
                     uint8_t* ddst,
                     ...
                     CIPH_DIGEST* arg);
void
asm_sha1_hmac_aescbc_dec(const uint8_t* csrc,
                         uint8_t* cdst,
                         uint64_t clen,
...
static int
aes_cbc_hmac_sha1_init_key(PROV_CIPHER_CTX* vctx,
...
static void
ciph_digest_arg_init(CIPH_DIGEST* arg, PROV_CIPHER_CTX* vctx)
{
  PROV_AES_HMAC_SHA_ETM_CTX* ctx = (PROV_AES_HMAC_SHA_ETM_CTX*)vctx;
  PROV_AES_HMAC_SHA1_ETM_CTX* sctx = (PROV_AES_HMAC_SHA1_ETM_CTX*)vctx;
...
static int
hwaes_cbc_hmac_sha1_etm(PROV_CIPHER_CTX* vctx,
                        unsigned char* out,
...
static void
aes_cbc_hmac_sha1_set_mac_key(void* vctx, const unsigned char* mac, size_t len)
{
...
static int
aes_cbc_hmac_sha1_cipher(PROV_CIPHER_CTX* vctx,
                         unsigned char* out,
                         const unsigned char* in,
                         size_t len)
{
