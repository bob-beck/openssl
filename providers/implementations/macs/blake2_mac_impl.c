...
static int blake2_mac_init(void* vmacctx, const unsigned char* key,
  ...
  if (!ossl_prov_is_running() || !blake2_mac_set_ctx_params(macctx, params))
    return 0;
  if (key != NULL) {
    if (!blake2_setkey(macctx, key, keylen)) return 0;
...
static int blake2_get_ctx_params(void* vmacctx, OSSL_PARAM params[]) {
  ...
  if (macctx == NULL || !blake2_get_ctx_decoder(params, &p)) return 0;

  if (p.size != NULL && !OSSL_PARAM_set_size_t(p.size, blake2_mac_size(macctx)))
    return 0;
...
static int blake2_mac_set_ctx_params(void* vmacctx, const OSSL_PARAM params[]) {
  ...
  if (macctx == NULL || !blake2_mac_set_ctx_decoder(params, &p)) return 0;

  if (p.size != NULL) {
    size_t size;

    if (!OSSL_PARAM_get_size_t(p.size, &size) || size < 1 ||
  ...
  if (p.cust != NULL) {
    if (p.cust->data_type != OSSL_PARAM_OCTET_STRING) return 0;
    /*
    ...
    BLAKE2_PARAM_SET_PERSONAL(&macctx->params, p.cust->data, p.cust->data_size);
  }

  if (p.salt != NULL) {
    if (p.salt->data_type != OSSL_PARAM_OCTET_STRING) return 0;
    /*
     * The OSSL_PARAM API doesn't provide direct pointer use, so we
