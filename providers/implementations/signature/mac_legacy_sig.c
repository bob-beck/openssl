...
static void* mac_newctx(void* provctx, const char* propq, const char* macname) {
  ...
  if (pmacctx->macctx == NULL) goto err;
...
static int mac_digest_sign_init(void* vpmacctx, const char* mdname, void* vkey,
  ...
  if (!ossl_prov_is_running() || pmacctx == NULL) return 0;
  ...
  if (vkey != NULL) {
    if (!ossl_mac_key_up_ref(vkey)) return 0;
  ...
  if (!ossl_prov_set_macctx(pmacctx->macctx, (char*)ciphername, (char*)mdname,
                            (char*)engine, pmacctx->key->properties))
    return 0;
...
int mac_digest_sign_update(void* vpmacctx, const unsigned char* data,
  ...
  if (pmacctx == NULL || pmacctx->macctx == NULL) return 0;
...
static void* mac_dupctx(void* vpmacctx) {
  ...
  *dstctx = *srcctx;
  ...
  if (srcctx->key != NULL && !ossl_mac_key_up_ref(srcctx->key)) goto err;
  dstctx->key = srcctx->key;

  if (srcctx->macctx != NULL) {
    dstctx->macctx = EVP_MAC_CTX_dup(srcctx->macctx);
    if (dstctx->macctx == NULL) goto err;
  }
