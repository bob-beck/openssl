...
static ssize_t sysctl_random(char* buf, size_t buflen) {
  ...
  do {
    ...
    if (sysctl(mib, 2, buf, &len, NULL, 0) == -1) return done > 0 ? done : -1;
    done += len;
    buf += len;
    buflen -= len;
  } while (buflen > 0);
...
static ssize_t syscall_random(void* buf, size_t buflen) {
  ...
  if (CCRandomGenerateBytes(buf, buflen) == kCCSuccess) return (ssize_t)buflen;
...
static int wait_random_seeded(void) {
  ...
  if (!seeded) {
    ...
    if (shm_id != -1) {
      ...
      if (shm_addr != (void*)-1) OPENSSL_atexit(&cleanup_shm);
    }
  }
  return seeded;
}
...
static int get_random_device(size_t n) {
  ...
  if (check_random_device(rd)) return rd->fd;

  /* open the random device ... */
  if ((rd->fd = open(random_device_paths[n], O_RDONLY)) == -1) return rd->fd;
...
static void close_random_device(size_t n) {
  ...
  if (check_random_device(rd)) close(rd->fd);
  rd->fd = -1;
}
...
void ossl_rand_pool_keep_random_devices_open(int keep) {
  if (!keep) ossl_rand_pool_cleanup();
...
size_t ossl_pool_acquire_entropy(RAND_POOL* pool) {
  ...
  if (entropy_available > 0) return entropy_available;
#endif
  ...
  if (wait_random_seeded()) {
    ...
    for (i = 0; bytes_needed > 0 && i < OSSL_NELEM(random_device_paths); i++) {
      ...
      if (bytes < 0 || !keep_random_devices_open) close_random_device(i);
...
#if defined(OPENSSL_RAND_SEED_EGD)
  {
    ...
    for (i = 0; bytes_needed > 0 && paths[i] != NULL; i++) {
      ...
      if (num == (int)bytes_needed) bytes = bytes_needed;
...
static uint64_t get_time_stamp(void) {
    ...
    (defined(_POSIX_C_SOURCE) && _POSIX_C_SOURCE >= 200112L)
  {
    ...
    if (gettimeofday(&tv, NULL) == 0) return TWO32TO64(tv.tv_sec, tv.tv_usec);
  }
#endif
  return time(NULL);
}
