 ...
 */
static ssize_t
sysctl_random (char *buf, size_t buflen)
{
  int mib[2];
  size_t done = 0;
  size_t len;
  ...
  if (!ossl_assert (buflen % sizeof (long) == 0))
     ...
     * supporting up to 256 bytes. Just return an error on older NetBSD
     * versions.
     */
 ...
 */
static ssize_t
syscall_random (void *buf, size_t buflen)
{
  /*
 ...
 */
static int
check_random_device (struct random_device *rd)
{
  struct stat st;
