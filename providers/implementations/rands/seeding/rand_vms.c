 ...
 */
static size_t
prepare_item_list (const struct item_st *items_input, size_t items_input_num,
                   ILE3 *items, uint32_t__ptr32 databuffer)
{
  size_t data_sz = 0;
...
size_t
data_collect_method (RAND_POOL *pool)
{
  ...
  total_elems += prepare_item_list (
      JPI_item_data_64bit, OSSL_NELEM (JPI_item_data_64bit), JPI_items_64bit,
      &data.buffer[total_elems]);
  total_elems += prepare_item_list (
      RMI_item_data_64bit, OSSL_NELEM (RMI_item_data_64bit), RMI_items_64bit,
      &data.buffer[total_elems]);
  ...
  /* Fill data.buffer with various info bits from this process */
  {
    ...
    if ((status
         = sys$getdviw (EFN$C_ENF, 0, &SYSDEVICE, DVI_items, 0, 0, 0, 0, 0))
        != SS$_NORMAL)
      {
        lib$signal (status);
        return 0;
...
size_t
get_entropy_method (RAND_POOL *pool)
{
  /*
