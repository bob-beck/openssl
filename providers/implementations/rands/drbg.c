...
find_call(const OSSL_DISPATCH* dispatch, int function);

static int
rand_drbg_restart(PROV_DRBG* drbg);
...
static int
ossl_drbg_lock_parent(PROV_DRBG* drbg)
{
...
static void
ossl_drbg_unlock_parent(PROV_DRBG* drbg)
{
  void* parent = drbg->parent;
...
static int
get_parent_strength(PROV_DRBG* drbg, unsigned int* str)
{
...
static unsigned int
get_parent_reseed_count(PROV_DRBG* drbg)
{
  OSSL_PARAM params[2] = { OSSL_PARAM_END, OSSL_PARAM_END };
  void* parent = drbg->parent;
  unsigned int r = 0;
...
void
ossl_drbg_clear_seed(ossl_unused void* vdrbg, unsigned char* out, size_t outlen)
{
  OPENSSL_secure_clear_free(out, outlen);
}

static size_t
get_entropy(PROV_DRBG* drbg,
            unsigned char** pout,
            int entropy,
            size_t min_len,
            size_t max_len,
            int prediction_resistance)
{
  ...
  if (drbg->parent == NULL)
     ...
     */
    return ossl_prov_get_entropy(
      drbg->provctx, pout, entropy, min_len, max_len);
...
static void
cleanup_entropy(PROV_DRBG* drbg, unsigned char* out, size_t outlen)
{
  if (drbg->parent == NULL) {
 ...
 */
void*
ossl_prov_drbg_nonce_ctx_new(OSSL_LIB_CTX* libctx)
{
...
void
ossl_prov_drbg_nonce_ctx_free(void* vdngbl)
{
...
static size_t
prov_drbg_get_nonce(PROV_DRBG* drbg,
                    unsigned char** pout,
                    size_t min_len,
                    size_t max_len)
{
  ...
  if (drbg->parent != NULL && drbg->parent_nonce != NULL) {
    n = drbg->parent_nonce(
      drbg->parent, NULL, 0, drbg->min_noncelen, drbg->max_noncelen);
    if (n > 0 && (buf = OPENSSL_malloc(n)) != NULL) {
      ret = drbg->parent_nonce(
        drbg->parent, buf, 0, drbg->min_noncelen, drbg->max_noncelen);
      if (ret == n) {
        *pout = buf;
        return ret;
      }
  ...
  if (!CRYPTO_atomic_add(
        &dngbl->rand_nonce_count, 1, &data.count, dngbl->rand_nonce_lock))
    return 0;
  return ossl_prov_get_nonce(
    drbg->provctx, pout, min_len, max_len, &data, sizeof(data));
}
 ...
 */
int
ossl_prov_drbg_instantiate(PROV_DRBG* drbg,
                           ...
                           size_t perslen)
{
  ...
  if (drbg->min_noncelen > 0) {
    if (drbg->parent_nonce != NULL) {
      ...
      if (noncelen != drbg->parent_nonce(drbg->parent,
                                         nonce,
                                         drbg->strength,
                                         drbg->min_noncelen,
                                         drbg->max_noncelen)) {
      ...
      noncelen = prov_drbg_get_nonce(
        drbg, &nonce, drbg->min_noncelen, drbg->max_noncelen);
  ...
  if (!drbg->instantiate(
        drbg, entropy, entropylen, nonce, noncelen, pers, perslen)) {
 ...
 */
int
ossl_prov_drbg_uninstantiate(PROV_DRBG* drbg)
{
...
static int
ossl_prov_drbg_reseed_unlocked(PROV_DRBG* drbg,
                               int prediction_resistance,
                      ...
                      size_t adinlen)
{
  ...
  ret = ossl_prov_drbg_reseed_unlocked(
    drbg, prediction_resistance, ent, ent_len, adin, adinlen);
                        ...
                        size_t adinlen)
{
  ...
  if (reseed_required || prediction_resistance) {
    if (!ossl_prov_drbg_reseed_unlocked(
          drbg, prediction_resistance, NULL, 0, adin, adinlen)) {
 ...
 */
PROV_DRBG*
ossl_rand_drbg_new(void* provctx,
                   void* parent,
 ...
 */
int
ossl_drbg_get_ctx_params(PROV_DRBG* drbg,
...
int
ossl_drbg_set_ctx_params(PROV_DRBG* drbg,
...
#ifdef FIPS_MODULE
static int
digest_allowed(const EVP_MD* md)
{
...
int
ossl_drbg_verify_digest(PROV_DRBG* drbg, OSSL_LIB_CTX* libctx, const EVP_MD* md)
{
#ifdef FIPS_MODULE
