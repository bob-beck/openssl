...
static unsigned int get_parent_reseed_count(PROV_DRBG* drbg) {
  ...
  *params = OSSL_PARAM_construct_uint(OSSL_DRBG_PARAM_RESEED_COUNTER, &r);
  ...
  if (!drbg->parent_get_ctx_params(parent, params)) r = 0;
  ossl_drbg_unlock_parent(drbg);
  return r;
...
size_t ossl_drbg_get_seed(void* vdrbg, unsigned char** pout, int entropy,
  ...
  if (bytes_needed < min_len) bytes_needed = min_len;
  if (bytes_needed > max_len) bytes_needed = max_len;
...
static size_t get_entropy(PROV_DRBG* drbg, unsigned char** pout, int entropy,
  ...
  if (!get_parent_strength(drbg, &p_str)) return 0;
   ...
   */
  if (!ossl_drbg_lock_parent(drbg)) return 0;
  /*
...
static void cleanup_entropy(PROV_DRBG* drbg, unsigned char* out,
  ...
  if (drbg->parent == NULL) {
    ...
    if (!ossl_drbg_lock_parent(drbg)) return;
...
int ossl_prov_drbg_instantiate(PROV_DRBG* drbg, unsigned int strength,
  ...
  if (drbg->reseed_next_counter) {
    ...
    if (!drbg->reseed_next_counter) drbg->reseed_next_counter = 1;
  }
  ...
  if (nonce != NULL) ossl_prov_cleanup_nonce(drbg->provctx, nonce, noncelen);
  if (drbg->state == EVP_RAND_STATE_READY) return 1;
  return 0;
}
...
static int ossl_prov_drbg_reseed_unlocked(
    PROV_DRBG* drbg, int prediction_resistance, const unsigned char* ent,
  ...
  if (!drbg->reseed(drbg, entropy, entropylen, adin, adinlen)) goto end;
...
int ossl_prov_drbg_reseed(PROV_DRBG* drbg, int prediction_resistance,
  ...
  if (drbg->lock != NULL) CRYPTO_THREAD_unlock(drbg->lock);
...
int ossl_prov_drbg_generate(PROV_DRBG* drbg, unsigned char* out, size_t outlen,
  ...
  if (reseed_time_interval > 0) now = time(NULL);
  ...
  if (drbg->reseed_interval > 0) {
    if (drbg->generate_counter >= drbg->reseed_interval) reseed_required = 1;
  }
...
static int rand_drbg_restart(PROV_DRBG* drbg) {
  /* repair error state */
  if (drbg->state == EVP_RAND_STATE_ERROR) drbg->uninstantiate(drbg);

  /* repair uninitialized state */
  if (drbg->state == EVP_RAND_STATE_UNINITIALISED) /* reinstantiate drbg */
...
static const OSSL_DISPATCH* find_call(const OSSL_DISPATCH* dispatch,
  ...
  if (dispatch != NULL)
    while (dispatch->function_id != 0) {
      if (dispatch->function_id == function) return dispatch;
      dispatch++;
    }
  return NULL;
}
...
PROV_DRBG* ossl_rand_drbg_new(
  ...
  if (!dnew(drbg)) goto err;

  if (parent != NULL) {
    if (!get_parent_strength(drbg, &p_str)) goto err;
    if (drbg->strength > p_str) {
      /*
       * We currently don't support the algorithm from NIST SP 800-90C
       * 10.1.2 to use a weaker DRBG as source
  ...
  if (!ossl_drbg_enable_locking(drbg)) goto err;
#endif
  return drbg;
...
int ossl_drbg_get_ctx_params(PROV_DRBG* drbg,
  ...
  if (p->state != NULL && !OSSL_PARAM_set_int(p->state, drbg->state)) return 0;

  if (p->str != NULL && !OSSL_PARAM_set_int(p->str, drbg->strength)) return 0;
  ...
  if (!OSSL_FIPS_IND_GET_CTX_FROM_PARAM(drbg, p->ind)) return 0;
...
int ossl_drbg_get_ctx_params_no_lock(PROV_DRBG* drbg,
  ...
  if (p->maxreq != NULL) {
    if (!OSSL_PARAM_set_size_t(p->maxreq, drbg->max_request)) return 0;
    cnt++;
  }
...
static int digest_allowed(const EVP_MD* md) {
  ...
  for (i = 0; i < OSSL_NELEM(allowed_digests); i++) {
    if (EVP_MD_is_a(md, allowed_digests[i])) return 1;
  }
  return 0;
}
#endif
