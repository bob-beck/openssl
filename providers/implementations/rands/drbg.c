 ...
 */
size_t
ossl_drbg_get_seed (void *vdrbg, unsigned char **pout, int entropy,
...
void
ossl_drbg_clear_seed (ossl_unused void *vdrbg, unsigned char *out,
                      size_t outlen)
{
...
static size_t
get_entropy (PROV_DRBG *drbg, unsigned char **pout, int entropy,
             size_t min_len, size_t max_len, int prediction_resistance)
{
  size_t bytes;
  unsigned int p_str;
...
static size_t
prov_drbg_get_nonce (PROV_DRBG *drbg, unsigned char **pout, size_t min_len,
                     size_t max_len)
{
  size_t ret = 0, n;
 ...
 */
int
ossl_prov_drbg_instantiate (PROV_DRBG *drbg, unsigned int strength,
 ...
 */
int
ossl_prov_drbg_reseed (PROV_DRBG *drbg, int prediction_resistance,
                       const unsigned char *ent, size_t ent_len,
                       const unsigned char *adin, size_t adinlen)
{
  int ret;
 ...
 */
int
ossl_prov_drbg_generate (PROV_DRBG *drbg, unsigned char *out, size_t outlen,
                                  ...
                                  const OSSL_PARAM params[], int *complete)
{
  ...
  if (p->reseed_cnt != NULL)
    {
      if (!OSSL_PARAM_set_uint (p->reseed_cnt,
                                tsan_load (&drbg->reseed_counter)))
        return 0;
      cnt++;
    }
...
int
ossl_drbg_verify_digest (PROV_DRBG *drbg, OSSL_LIB_CTX *libctx,
                         const EVP_MD *md)
{
#ifdef FIPS_MODULE
