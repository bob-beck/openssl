...
blake2s_set_lastblock (BLAKE2S_CTX *S)
{
  S->f[0] = -1;
}
...
static void
blake2s_init_param (BLAKE2S_CTX *S, const BLAKE2S_PARAM *P)
{
  size_t i;
...
void
ossl_blake2s_param_set_personal (BLAKE2S_PARAM *P, const uint8_t *personal,
                                 size_t len)
{
...
void
ossl_blake2s_param_set_salt (BLAKE2S_PARAM *P, const uint8_t *salt, size_t len)
{
...
static void
blake2s_compress (BLAKE2S_CTX *S, const uint8_t *blocks, size_t len)
{
  uint32_t m[16];
  uint32_t v[16];
  size_t i;
