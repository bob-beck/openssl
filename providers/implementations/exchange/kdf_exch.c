...
static void *
kdf_newctx (const char *kdfname, void *provctx)
{
  PROV_KDF_CTX *kdfctx;
  EVP_KDF *kdf = NULL;
...
static int
kdf_init (void *vpkdfctx, void *vkdf, const OSSL_PARAM params[])
{
...
static int
kdf_derive (void *vpkdfctx, unsigned char *secret, size_t *secretlen,
            size_t outlen)
{
  PROV_KDF_CTX *pkdfctx = (PROV_KDF_CTX *)vpkdfctx;
  size_t kdfsize;
...
static int
kdf_set_ctx_params (void *vpkdfctx, const OSSL_PARAM params[])
{
  PROV_KDF_CTX *pkdfctx = (PROV_KDF_CTX *)vpkdfctx;
...
static int
kdf_get_ctx_params (void *vpkdfctx, OSSL_PARAM params[])
{
  PROV_KDF_CTX *pkdfctx = (PROV_KDF_CTX *)vpkdfctx;
...
kdf_settable_ctx_params (ossl_unused void *vpkdfctx, void *provctx,
                         const char *kdfname)
{
...
static const OSSL_PARAM *
kdf_gettable_ctx_params (ossl_unused void *vpkdfctx, void *provctx,
                         const char *kdfname)
{
  EVP_KDF *kdf = EVP_KDF_fetch (PROV_LIBCTX_OF (provctx), kdfname, NULL);
  const OSSL_PARAM *params;
...
#define KDF_KEYEXCH_FUNCTIONS(funcname)                                       \
  const OSSL_DISPATCH ossl_kdf_##funcname##_keyexch_functions[]               \
      = { { OSSL_FUNC_KEYEXCH_NEWCTX,                                         \
            (void (*) (void))kdf_##funcname##_newctx },                       \
