...
key_to_params (const EC_KEY *eckey, OSSL_PARAM_BLD *tmpl, OSSL_PARAM params[],
...
static ossl_inline int
otherparams_to_params (const EC_KEY *ec, OSSL_PARAM_BLD *tmpl,
                       OSSL_PARAM params[])
{
...
static int
common_import (void *keydata, int selection, const OSSL_PARAM params[],
               int sm2_wanted)
{
  EC_KEY *ec = keydata;
  int ok = 1;
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0)
    {
      ...
      ok = ok
           && ossl_ec_group_todata (
               EC_KEY_get0_group (ec), tmpl, NULL, ossl_ec_key_get_libctx (ec),
...
common_get_params (void *key, OSSL_PARAM params[], int sm2)
{
  ...
  if ((p = OSSL_PARAM_locate (params, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY))
      != NULL)
    {
      ...
      p->return_size
          = EC_POINT_point2oct (ecg, ecp, POINT_CONVERSION_UNCOMPRESSED,
