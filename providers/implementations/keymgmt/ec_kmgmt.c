...
static ossl_inline int key_to_params(const EC_KEY* eckey, OSSL_PARAM_BLD* tmpl,
  ...
  if (eckey == NULL || (ecg = EC_KEY_get0_group(eckey)) == NULL) return 0;
  ...
  if (pub_point != NULL) {
    ...
    if (p != NULL || tmpl != NULL) {
      ...
      if ((pub_key_len = EC_POINT_point2buf(ecg, pub_point, format, pub_key,
                                            bnctx)) == 0 ||
...
static int ec_has(const void* keydata, int selection) {
  ...
  if (!ossl_prov_is_running() || ec == NULL) return 0;
...
static int common_import(void* keydata, int selection,
   ...
   */
  if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) == 0) return 0;

  ok = ok && ossl_ec_group_fromdata(ec, params);

  if (!common_check_sm2(ec, sm2_wanted)) return 0;
...
static ossl_inline const OSSL_PARAM* ec_imexport_types(int selection) {
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) type_select += 1;
  if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) type_select += 2;
  if ((selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0)
    type_select += 4;
  if ((selection & OSSL_KEYMGMT_SELECT_OTHER_PARAMETERS) != 0) type_select += 8;
...
static int ec_get_ecm_params(const EC_GROUP* group, OSSL_PARAM params[]) {
  ...
  if (fid != NID_X9_62_characteristic_two_field) return 1;
...
static int common_get_params(void* key, OSSL_PARAM params[], int sm2) {
  ...
  if ((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_SECURITY_BITS)) != NULL) {
    ...
    if (!OSSL_PARAM_set_int(p, sec_bits)) goto err;
  }
  if ((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_SECURITY_CATEGORY)) !=
      NULL)
    if (!OSSL_PARAM_set_int(p, 0)) goto err;
  ...
  if ((p = OSSL_PARAM_locate(
    ...
    if (explicitparams < 0 || !OSSL_PARAM_set_int(p, explicitparams)) goto err;
  }
  ...
  if (!sm2) {
    ...
    if (p != NULL) {
      ...
      if (!OSSL_PARAM_set_int(p, ecdh_cofactor_mode)) goto err;
    }
  }
...
static void* sm2_gen_init(void* provctx, int selection,
  ...
  if (gctx != NULL) {
    if (gctx->group_name != NULL) return gctx;
    if ((gctx->group_name = OPENSSL_strdup("sm2")) != NULL) return gctx;
    ec_gen_cleanup(gctx);
  }
  return NULL;
}
...
static int ec_gen_set_template(void* genctx, void* templ) {
  ...
  if (!ossl_prov_is_running() || gctx == NULL || ec == NULL) return 0;
...
#define COPY_INT_PARAM(params, key, val)    \
  ...
  if (p != NULL && !OSSL_PARAM_get_int(p, &val)) goto err;

#define COPY_UTF8_PARAM(params, key, val)                 \
  p = OSSL_PARAM_locate_const(params, key);               \
  if (p != NULL) {                                        \
    if (p->data_type != OSSL_PARAM_UTF8_STRING) goto err; \
    OPENSSL_free(val);                                    \
...
#define COPY_OCTET_PARAM(params, key, val, len)            \
  p = OSSL_PARAM_locate_const(params, key);                \
  if (p != NULL) {                                         \
    if (p->data_type != OSSL_PARAM_OCTET_STRING) goto err; \
    OPENSSL_free(val);                                     \
    len = p->data_size;                                    \
...
#define COPY_BN_PARAM(params, key, bn)                      \
  p = OSSL_PARAM_locate_const(params, key);                 \
  if (p != NULL) {                                          \
    if (bn == NULL) bn = BN_new();                          \
    if (bn == NULL || !OSSL_PARAM_get_BN(p, &bn)) goto err; \
  }
...
static void* ec_gen(void* genctx, OSSL_CALLBACK* osslcb, void* cbarg) {
  ...
  if (gctx->gen_group == NULL) {
    if (!ec_gen_set_group_from_params(gctx)) goto err;
  } else {
    if (gctx->encoding != NULL) {
...
static void* common_load(const void* reference, size_t reference_sz,
  ...
  if (ossl_prov_is_running() && reference_sz == sizeof(ec)) {
    ...
    if (!common_check_sm2(ec, sm2_wanted)) return NULL;
...
static void* ec_dup(const void* keydata_from, int selection) {
  if (ossl_prov_is_running()) return ossl_ec_key_dup(keydata_from, selection);
  return NULL;
}
