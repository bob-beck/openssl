...
static int
pss_params_fromdata(RSA_PSS_PARAMS_30* pss_params,
                    int* defaults_set,
                    const OSSL_PARAM params[],
                    int rsa_type,
                    OSSL_LIB_CTX* libctx)
{
  if (!ossl_rsa_pss_params_30_fromdata(
        pss_params, defaults_set, params, libctx))
    return 0;
...
static void
rsa_freedata(void* keydata)
{
  RSA_free(keydata);
}

static int
rsa_has(const void* keydata, int selection)
{
...
static int
rsa_match(const void* keydata1, const void* keydata2, int selection)
{
...
static int
rsa_import(void* keydata, int selection, const OSSL_PARAM params[])
{
  RSA* rsa = keydata;
  int rsa_type;
  int ok = 1;
  ...
  if (ok > 0 && !ossl_fips_self_testing()) {
    ...
    if (n != NULL && e != NULL)
      ...
      if (d != NULL ||
          (p != NULL && q != NULL && dp != NULL && dq != NULL && iq != NULL))
...
static int
rsa_get_params(void* key, OSSL_PARAM params[])
{
  RSA* rsa = key;
  const RSA_PSS_PARAMS_30* pss_params = ossl_rsa_get0_pss_params_30(rsa);
...
static int
rsa_validate(const void* keydata, int selection, int checktype)
{
  const RSA* rsa = keydata;
  int ok = 1;
...
static int
rsa_gencb(int p, int n, BN_GENCB* cb)
{
...
static void*
gen_init(void* provctx, int selection, int rsa_type, const OSSL_PARAM params[])
{
  OSSL_LIB_CTX* libctx = PROV_LIBCTX_OF(provctx);
...
static void*
rsa_gen_init(void* provctx, int selection, const OSSL_PARAM params[])
{
  return gen_init(provctx, selection, RSA_FLAG_TYPE_RSA, params);
}

static void*
rsapss_gen_init(void* provctx, int selection, const OSSL_PARAM params[])
{
 ...
 */
static int
rsa_gen_set_params(void* genctx, const OSSL_PARAM params[])
{
  ...
  if (gctx->rsa_type == RSA_FLAG_TYPE_RSASSAPSS &&
      !pss_params_fromdata(&gctx->pss_params,
                           &gctx->pss_defaults_set,
                           params,
                           gctx->rsa_type,
                           gctx->libctx))
    return 0;
...
static const OSSL_PARAM*
rsa_gen_settable_params(ossl_unused void* genctx, ossl_unused void* provctx)
{
...
static const OSSL_PARAM*
rsapss_gen_settable_params(ossl_unused void* genctx, ossl_unused void* provctx)
{
...
static void*
rsa_gen(void* genctx, OSSL_CALLBACK* osslcb, void* cbarg)
{
  struct rsa_gen_ctx* gctx = genctx;
  ...
  if (!RSA_generate_multi_prime_key(
        rsa_tmp, (int)gctx->nbits, (int)gctx->primes, gctx->pub_exp, gencb))
    goto err;
...
static void*
common_load(const void* reference, size_t reference_sz, int expected_rsa_type)
{
  RSA* rsa = NULL;
...
static void*
rsa_load(const void* reference, size_t reference_sz)
{
  return common_load(reference, reference_sz, RSA_FLAG_TYPE_RSA);
}

static void*
rsapss_load(const void* reference, size_t reference_sz)
{
  return common_load(reference, reference_sz, RSA_FLAG_TYPE_RSASSAPSS);
}

static void*
rsa_dup(const void* keydata_from, int selection)
{
  if (ossl_prov_is_running()
...
static const char*
rsa_query_operation_name(int operation_id)
{
  return "RSA";
}
