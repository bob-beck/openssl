...
static int rsa_has(const void* keydata, int selection) {
  ...
  if (rsa == NULL || !ossl_prov_is_running()) return 0;
...
static int rsa_import(void* keydata, int selection, const OSSL_PARAM params[]) {
  ...
  if (!ossl_prov_is_running() || rsa == NULL) return 0;

  if ((selection & RSA_POSSIBLE_SELECTIONS) == 0) return 0;
...
static const OSSL_PARAM* rsa_imexport_types(int selection) {
  if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) return rsa_key_types;
  return NULL;
}
...
static int rsa_get_params(void* key, OSSL_PARAM params[]) {
  ...
  if ((p = OSSL_PARAM_locate(params, OSSL_PKEY_PARAM_MANDATORY_DIGEST)) !=
    ...
    if (mdname == NULL || !OSSL_PARAM_set_utf8_string(p, mdname)) return 0;
  }
...
static void* gen_init(void* provctx, int selection, int rsa_type,
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) == 0) return NULL;
  ...
  if (!rsa_gen_set_params(gctx, params)) goto err;
  return gctx;

err:
  if (gctx != NULL) BN_free(gctx->pub_exp);
  OPENSSL_free(gctx);
  return NULL;
}
...
static int rsa_gen_set_params(void* genctx, const OSSL_PARAM params[]) {
  ...
  if ((p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_RSA_BITS)) != NULL) {
    if (!OSSL_PARAM_get_size_t(p, &gctx->nbits)) return 0;
...
static void* rsa_gen(void* genctx, OSSL_CALLBACK* osslcb, void* cbarg) {
  ...
  switch (gctx->rsa_type) {
    case RSA_FLAG_TYPE_RSA:
      ...
      if (!ossl_rsa_pss_params_30_is_unrestricted(&gctx->pss_params)) goto err;
      break;
  ...
  if ((rsa_tmp = ossl_rsa_new_with_ctx(gctx->libctx)) == NULL) return NULL;

  gctx->cb = osslcb;
  gctx->cbarg = cbarg;
  gencb = BN_GENCB_new();
  if (gencb != NULL) BN_GENCB_set(gencb, rsa_gencb, genctx);
