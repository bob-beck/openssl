...
static int rsa_match(const void* keydata1,
                     const void* keydata2,
                     int selection) {
...
static int rsa_export(void* keydata,
                      int selection,
...
static void* gen_init(void* provctx,
                      int selection,
                      int rsa_type,
                      const OSSL_PARAM params[]) {
  OSSL_LIB_CTX* libctx = PROV_LIBCTX_OF(provctx);
...
static void* rsa_gen_init(void* provctx,
                          int selection,
                          const OSSL_PARAM params[]) {
  return gen_init(provctx, selection, RSA_FLAG_TYPE_RSA, params);
}

static void* rsapss_gen_init(void* provctx,
                             int selection,
                             const OSSL_PARAM params[]) {
