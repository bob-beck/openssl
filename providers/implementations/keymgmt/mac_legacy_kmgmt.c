...
static int mac_match(const void* keydata1,
                     const void* keydata2,
                     int selection) {
...
static int key_to_params(MAC_KEY* key,
                         OSSL_PARAM_BLD* tmpl,
                         OSSL_PARAM params[]) {
  if (key == NULL)
    return 0;
...
static int mac_export(void* keydata,
                      int selection,
...
static void* mac_gen_init(void* provctx,
                          int selection,
...
static void* cmac_gen_init(void* provctx,
                           int selection,
                           const OSSL_PARAM params[]) {
  struct mac_gen_ctx* gctx = mac_gen_init_common(provctx, selection);
