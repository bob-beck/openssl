...
#ifdef FIPS_MODULE
    if (type == DH_FLAG_TYPE_DHX) return DH_PARAMGEN_TYPE_FIPS_186_4;

    return DH_PARAMGEN_TYPE_GROUP;
#else
    if (type == DH_FLAG_TYPE_DHX) return DH_PARAMGEN_TYPE_FIPS_186_2;
...
static int dh_has(const void* keydata, int selection) {
  ...
  if (!ossl_prov_is_running() || dh == NULL) return 0;
...
static int dh_import(void* keydata, int selection, const OSSL_PARAM params[]) {
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
    ...
    if (ok > 0 && !ossl_fips_self_testing()) {
      ...
      if (ok <= 0) ossl_set_error_state(OSSL_SELF_TEST_TYPE_PCT);
    }
#endif /* FIPS_MODULE */
  }
...
static const OSSL_PARAM* dh_imexport_types(int selection) {
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_ALL_PARAMETERS) != 0) type_select += 1;
  if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) type_select += 2;
...
static int dh_gen_set_template(void* genctx, void* templ) {
  ...
  if (!ossl_prov_is_running() || gctx == NULL || dh == NULL) return 0;
...
static int dh_gen_common_set_params(void* genctx, const OSSL_PARAM params[]) {
  ...
  if (ossl_param_is_empty(params)) return 1;
  ...
  if (p != NULL) {
    ...
    if (gen_type != -1) gctx->gen_type = gen_type;
  }
  ...
  if (p != NULL && !OSSL_PARAM_get_int(p, &gctx->priv_len)) return 0;
  return 1;
}
...
static int dh_gen_set_params(void* genctx, const OSSL_PARAM params[]) {
  ...
  p = OSSL_PARAM_locate_const(params, OSSL_PKEY_PARAM_DH_GENERATOR);
  if (p != NULL && !OSSL_PARAM_get_int(p, &gctx->generator)) return 0;
...
static void* dh_gen(void* genctx, OSSL_CALLBACK* osslcb, void* cbarg) {
  ...
  if (!ossl_prov_is_running() || gctx == NULL) return NULL;
   ...
   * compatibility.
   */
  if (gctx->group_nid != NID_undef) gctx->gen_type = DH_PARAMGEN_TYPE_GROUP;
  ...
  if (gctx->gen_type == DH_PARAMGEN_TYPE_GROUP && gctx->ffc_params == NULL) {
    ...
    if (gctx->group_nid == NID_undef) return NULL;
    ...
    if (!ossl_ffc_params_set_seed(ffc, gctx->seed, gctx->seedlen)) goto end;
    if (gctx->gindex != -1) {
    ...
    gctx->cbarg = cbarg;
    gencb = BN_GENCB_new();
    if (gencb != NULL) BN_GENCB_set(gencb, dh_gencb, genctx);
    ...
    if ((gctx->selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
      ...
      if (gctx->gen_type == DH_PARAMGEN_TYPE_GENERATOR)
        ret = DH_generate_parameters_ex(dh, (int)gctx->pbits, gctx->generator,
                                        gencb);
      else
  ...
  if ((gctx->selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
    if (ffc->p == NULL || ffc->g == NULL) goto end;
    if (gctx->priv_len > 0) DH_set_length(dh, (long)gctx->priv_len);
    ossl_ffc_params_enable_flags(ffc, FFC_PARAM_FLAG_VALIDATE_LEGACY,
                                 gctx->gen_type == DH_PARAMGEN_TYPE_FIPS_186_2);
    if (DH_generate_key(dh) <= 0) goto end;
#ifdef FIPS_MODULE
...
static void* dh_dup(const void* keydata_from, int selection) {
  if (ossl_prov_is_running()) return ossl_dh_dup(keydata_from, selection);
  return NULL;
}
