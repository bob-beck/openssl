...
static int dsa_has(const void* keydata, int selection) {
  ...
  if (!ossl_prov_is_running() || dsa == NULL) return 0;
...
static int dsa_import(void* keydata, int selection, const OSSL_PARAM params[]) {
  ...
  if ((selection & DSA_POSSIBLE_SELECTIONS) == 0) return 0;
...
static int dsa_gen_set_template(void* genctx, void* templ) {
  ...
  if (!ossl_prov_is_running() || gctx == NULL || dsa == NULL) return 0;
...
static void* dsa_gen(void* genctx, OSSL_CALLBACK* osslcb, void* cbarg) {
  ...
  gctx->cbarg = cbarg;
  gencb = BN_GENCB_new();
  if (gencb != NULL) BN_GENCB_set(gencb, dsa_gencb, genctx);
  ...
  if (gctx->gindex != -1) {
    ...
    if (gctx->pcounter != -1) ossl_ffc_params_set_pcounter(ffc, gctx->pcounter);
  ...
  if ((gctx->selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
    if (ossl_dsa_generate_ffc_parameters(dsa, gctx->gen_type, (int)gctx->pbits,
  ...
  if ((gctx->selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
    if (ffc->p == NULL || ffc->q == NULL || ffc->g == NULL) goto end;
    if (DSA_generate_key(dsa) <= 0) goto end;
  }
  ret = 1;
end:
  if (ret <= 0) {
...
static void* dsa_dup(const void* keydata_from, int selection) {
  if (ossl_prov_is_running()) return ossl_dsa_dup(keydata_from, selection);
  return NULL;
}
