...
static int
dsa_set_gen_seed(struct dsa_gen_ctx* gctx, unsigned char* seed, size_t seedlen)
{
...
dsa_gen_set_params(void* genctx, const OSSL_PARAM params[])
{
  ...
  if (!OSSL_FIPS_IND_SET_CTX_PARAM(
        gctx, OSSL_FIPS_IND_SETTABLE0, params, OSSL_PKEY_PARAM_FIPS_SIGN_CHECK))
    return 0;
...
dsa_gen(void* genctx, OSSL_CALLBACK* osslcb, void* cbarg)
{
   ...
   */
  if (!OSSL_FIPS_IND_ON_UNAPPROVED(gctx,
  ...
  if ((gctx->selection & OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) != 0) {
    ...
    if (ossl_dsa_generate_ffc_parameters(
          dsa, gctx->gen_type, (int)gctx->pbits, (int)gctx->qbits, gencb) <= 0)
      goto end;
  }
