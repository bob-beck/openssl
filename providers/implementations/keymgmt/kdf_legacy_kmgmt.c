...
KDF_DATA *
ossl_kdf_data_new (void *provctx)
{
  KDF_DATA *kdfdata;
...
int
ossl_kdf_data_up_ref (KDF_DATA *kdfdata)
{
  int ref = 0;
...
static void *
kdf_newdata (void *provctx)
{
  return ossl_kdf_data_new (provctx);
}

static void
kdf_freedata (void *kdfdata)
{
  ossl_kdf_data_free (kdfdata);
}

static int
kdf_has (const void *keydata, int selection)
{
