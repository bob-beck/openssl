...
static void
lms_free_key (void *keydata)
{
  ossl_lms_key_free ((LMS_KEY *)keydata);
}

static int
lms_has (const void *keydata, int selection)
{
...
static int
lms_match (const void *keydata1, const void *keydata2, int selection)
{
...
static int
lms_import (void *keydata, int selection, const OSSL_PARAM params[])
{
  LMS_KEY *key = keydata;
...
lms_imexport_types (int selection)
{
  if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0)
...
static int
lms_export (void *keydata, int selection, OSSL_CALLBACK *param_cb, void *cbarg)
{
  ...
  if (!ossl_param_build_set_octet_string (
          tmpl, params, OSSL_PKEY_PARAM_PUB_KEY, lmskey->pub.encoded,
...
static int
lms_validate (const void *keydata, int selection, int checktype)
{
...
static void *
lms_load (const void *reference, size_t reference_sz)
{
  LMS_KEY *key = NULL;
