...
static void debug_print(char* fmt, ...) {
  ...
  va_end(argptr);
  if (getenv("TEMPLATEKEM")) fprintf(stderr, "TEMPLATE_KEM: %s", out);
}
#endif
...
static int template_decapsulate(void* vctx, unsigned char* out, size_t* outlen,
  ...
  if (outlen != NULL) *outlen = 0; /* replace with shared secret length */

  if (out == NULL) {
    if (outlen != NULL) debug_print("decaps outlen set to %zu \n", *outlen);
    return 1;
  }
