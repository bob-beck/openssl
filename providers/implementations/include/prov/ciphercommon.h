...
typedef int(PROV_CIPHER_HW_FN)(PROV_CIPHER_CTX* dat,
                               unsigned char* out,
...
void
ossl_cipher_generic_reset_ctx(PROV_CIPHER_CTX* ctx);
...
int
ossl_cipher_generic_get_params(OSSL_PARAM params[],
                               unsigned int md,
                               uint64_t flags,
                               size_t kbits,
                               size_t blkbits,
...
#define IMPLEMENT_CIPHER_HW_CFB(MODE, NAME, CTX_NAME, KEY_NAME, FUNC_PREFIX)   \
  ...
  {                                                                            \
    ...
    while (len > 0 && len >= chunk) {                                          \
      FUNC_PREFIX##_encrypt(                                                   \
        in, out, (long)chunk, key, ctx->iv, &num, ctx->enc);                   \
      len -= chunk;                                                            \
      in += chunk;                                                             \
      out += chunk;                                                            \
      if (len < chunk)                                                         \
...
int
ossl_cipher_common_set_ctx_params(
  PROV_CIPHER_CTX* ctx,
  const struct ossl_cipher_set_ctx_param_list_st* p);

int
ossl_cipher_generic_initiv(PROV_CIPHER_CTX* ctx,
                           const unsigned char* iv,
                           size_t ivlen);

size_t
ossl_cipher_fillblock(unsigned char* buf,
                      size_t* buflen,
                      size_t blocksize,
                      const unsigned char** in,
                      size_t* inlen);
int
ossl_cipher_trailingdata(unsigned char* buf,
                         size_t* buflen,
                         size_t blocksize,
                         const unsigned char** in,
                         size_t* inlen);
