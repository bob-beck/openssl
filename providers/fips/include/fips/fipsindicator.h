...
/*
 * Use the copy if an algorithm has a dup function that does not copy the src
 * to the dst.
 */
...
/*
 * This should be called if a FIPS check fails, to indicate the operation is
 * not approved If there is more than 1 strict check flag per algorithm ctx,
 * the id represents the index.
 */
...
int ossl_fips_ind_ec_key_check (OSSL_FIPS_IND *ind, int id,
                                OSSL_LIB_CTX *libctx, const EC_GROUP *group,
                                const char *desc, int protect);
#endif
