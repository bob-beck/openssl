...
set_kat_drbg(OSSL_LIB_CTX* ctx,
             ...
             size_t persstr_len);
static int
reset_main_drbg(OSSL_LIB_CTX* ctx);

static int
self_test_digest(const ST_KAT_DIGEST* t,
                 OSSL_SELF_TEST* st,
                 OSSL_LIB_CTX* libctx)
{
  int ok = 0;
            ...
            int enc)
{
  ...
  return EVP_CipherInit_ex(ctx, cipher, NULL, NULL, NULL, enc) &&
         (EVP_CIPHER_CTX_ctrl(
            ctx, EVP_CTRL_AEAD_SET_IVLEN, (int)t->iv_len, NULL) > 0) &&
         (in_tag == NULL ||
          EVP_CIPHER_CTX_ctrl(
            ctx, EVP_CTRL_AEAD_SET_TAG, (int)t->tag_len, in_tag) > 0) &&
...
static int
self_test_cipher(const ST_KAT_CIPHER* t,
                 OSSL_SELF_TEST* st,
                 OSSL_LIB_CTX* libctx)
{
  ...
  if ((t->mode & CIPHER_MODE_ENCRYPT) != 0) {
    ...
    if (t->tag != NULL) {
      ...
      if (EVP_CIPHER_CTX_ctrl(
            ctx, EVP_CTRL_AEAD_GET_TAG, (int)t->tag_len, tag) <= 0 ||
  ...
  if ((t->mode & CIPHER_MODE_DECRYPT) != 0) {
    if (!(cipher_init(ctx, cipher, t, !encrypt) &&
          EVP_CipherUpdate(
            ctx, pt_buf, &len, t->base.expected, (int)t->base.expected_len) &&
...
static int
add_params(OSSL_PARAM_BLD* bld, const ST_KAT_PARAM* params, BN_CTX* ctx)
{
  int ret = 0;
...
static SENTINEL OSSL_PARAM*
kat_params_to_ossl_params(OSSL_LIB_CTX* libctx, ...)
{
  BN_CTX* bnc = NULL;
...
static int
self_test_kdf(const ST_KAT_KDF* t, OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int ret = 0;
...
static int
self_test_drbg(const ST_KAT_DRBG* t, OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int ret = 0;
  ...
  OSSL_PARAM drbg_params[3] = { OSSL_PARAM_END,
                                OSSL_PARAM_END,
                                OSSL_PARAM_END };
...
static int
self_test_ka(const ST_KAT_KAS* t, OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int ret = 0;
...
static int
digest_signature(const uint8_t* sig,
                 size_t sig_len,
                 uint8_t* out,
                 size_t* out_len,
...
static int
self_test_LMS(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int ret = 0;
  OSSL_PARAM pm[2];
  ...
  OSSL_SELF_TEST_onbegin(
    st, OSSL_SELF_TEST_TYPE_KAT_SIGNATURE, OSSL_SELF_TEST_DESC_SIGN_LMS);
...
static int
self_test_digest_sign(const ST_KAT_SIGN* t,
                      OSSL_SELF_TEST* st,
                      OSSL_LIB_CTX* libctx)
{
  int ret = 0;
  ...
  if ((t->mode & SIGNATURE_MODE_VERIFY_ONLY) != 0) {
    ...
    if (t->sig_expected != NULL) {
      if ((t->mode & SIGNATURE_MODE_SIG_DIGESTED) != 0) {
        ...
        if (!digest_signature(
              psig, siglen, digested_sig, &digested_sig_len, libctx) ||
 ...
 */
static int
self_test_asym_keygen(const ST_KAT_ASYM_KEYGEN* t,
                      OSSL_SELF_TEST* st,
                      OSSL_LIB_CTX* libctx)
{
  int ret = 0;
  ...
  for (expected = t->expected_params; expected->data != NULL; ++expected) {
    if (expected->type != OSSL_PARAM_OCTET_STRING ||
        !EVP_PKEY_get_octet_string_param(
          key, expected->name, out, sizeof(out), &out_len))
      goto err;
    OSSL_SELF_TEST_oncorrupt_byte(st, out);
    /* Check the KAT */
 ...
 */
static int
self_test_kem_encapsulate(const ST_KAT_KEM* t,
                          OSSL_SELF_TEST* st,
                          OSSL_LIB_CTX* libctx,
                          EVP_PKEY* pkey)
{
  ...
  OSSL_SELF_TEST_onbegin(
    st, OSSL_SELF_TEST_TYPE_KAT_KEM, OSSL_SELF_TEST_DESC_ENCAP_KEM);
 ...
 */
static int
self_test_kem_decapsulate(const ST_KAT_KEM* t,
                          OSSL_SELF_TEST* st,
                          OSSL_LIB_CTX* libctx,
                          EVP_PKEY* pkey,
                          int reject)
{
  ...
  if (EVP_PKEY_decapsulate(
        ctx, secret, &secretlen, cipher_text, t->cipher_text_len) <= 0)
    goto err;
 ...
 */
static int
self_test_kem(const ST_KAT_KEM* t, OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int ret = 0;
  EVP_PKEY* pkey = NULL;
  EVP_PKEY_CTX* ctx;
 ...
 */
static int
self_test_digests(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int i, ret = 1;
...
static int
self_test_ciphers(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int i, ret = 1;
...
static int
self_test_kems(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int ret = 1;
#ifndef OPENSSL_NO_ML_KEM
  int i;
...
static int
self_test_kdfs(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int i, ret = 1;
...
static int
self_test_drbgs(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int i, ret = 1;
...
static int
self_test_kas(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int ret = 1;
#if !defined(OPENSSL_NO_DH) || !defined(OPENSSL_NO_EC)
  int i;
...
static int
self_test_signatures(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
  int i, ret = 1;
...
static int
setup_main_random(OSSL_LIB_CTX* libctx)
{
  OSSL_PARAM drbg_params[3] = { OSSL_PARAM_END,
                                OSSL_PARAM_END,
                                OSSL_PARAM_END };
...
static int
self_test_asym_keygens(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
#if !defined(OPENSSL_NO_ML_DSA) || !defined(OPENSSL_NO_SLH_DSA)
  int i, ret = 1;
 ...
 */
int
SELF_TEST_kats(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx)
{
