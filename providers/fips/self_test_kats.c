...
static int cipher_init(EVP_CIPHER_CTX* ctx, const EVP_CIPHER* cipher,
  ...
  if (!enc) in_tag = (unsigned char*)t->tag;

  return EVP_CipherInit_ex(ctx, cipher, NULL, NULL, NULL, enc) &&
         (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, (int)t->iv_len,
                              NULL) > 0) &&
         (in_tag == NULL || EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG,
                                                (int)t->tag_len, in_tag) > 0) &&
  ...
  for (p = params; p->data != NULL; ++p) {
    switch (p->type) {
      ...
      case OSSL_PARAM_INTEGER: {
        if (!OSSL_PARAM_BLD_push_int(bld, p->name, *(int*)p->data)) goto err;
        break;
      }
      default:
        break;
...
static SENTINEL OSSL_PARAM* kat_params_to_ossl_params(OSSL_LIB_CTX* libctx,
  ...
  if (bnc == NULL) goto err;
  bld = OSSL_PARAM_BLD_new();
  if (bld == NULL) goto err;
...
static int self_test_kdf(const ST_KAT_KDF* t, OSSL_SELF_TEST* st,
  ...
  if (t->expected_len > sizeof(out)) goto err;
  if (EVP_KDF_derive(ctx, out, t->expected_len, params) <= 0) goto err;

  OSSL_SELF_TEST_oncorrupt_byte(st, out);

  if (memcmp(out, t->expected, t->expected_len) != 0) goto err;
...
static int self_test_drbg(const ST_KAT_DRBG* t, OSSL_SELF_TEST* st,
  ...
  if (!EVP_RAND_CTX_set_params(test, drbg_params)) goto err;
  ...
  if (!EVP_RAND_CTX_set_params(drbg, drbg_params)) goto err;
  ...
  if (!EVP_RAND_instantiate(test, strength, 0, NULL, 0, drbg_params)) goto err;
  ...
  if (memcmp(out, t->expected, t->expectedlen) != 0) goto err;

  if (!EVP_RAND_uninstantiate(drbg)) goto err;
  /*
   * Check that the DRBG data has been zeroized after
   * ossl_prov_drbg_uninstantiate.
   */
  if (!EVP_RAND_verify_zeroization(drbg)) goto err;
...
static int self_test_ka(const ST_KAT_KAS* t, OSSL_SELF_TEST* st,
  ...
  if (secret_len > sizeof(secret)) goto err;
...
static int self_test_digest_sign(const ST_KAT_SIGN* t, OSSL_SELF_TEST* st,
  ...
  if (t->sig_expected_len > sizeof(sig)) goto err;

  if (t->sig_expected == NULL) typ = OSSL_SELF_TEST_TYPE_PCT_SIGNATURE;
  ...
  if ((t->mode & SIGNATURE_MODE_VERIFY_ONLY) != 0) {
    ...
    if (digested) {
      if (EVP_PKEY_sign_init_ex2(ctx, sigalg, paramsinit) <= 0) goto err;
    } else {
      if (EVP_PKEY_sign_message_init(ctx, sigalg, paramsinit) <= 0) goto err;
    }
    siglen = sizeof(sig);
    if ((t->mode & SIGNATURE_MODE_SIG_DIGESTED) != 0) {
      if (EVP_PKEY_sign(ctx, NULL, &siglen, t->msg, t->msg_len) <= 0) goto err;
      if (siglen > sizeof(sig)) {
        psig = OPENSSL_malloc(siglen);
        if (psig == NULL) goto err;
      }
    }
    if (EVP_PKEY_sign(ctx, psig, &siglen, t->msg, t->msg_len) <= 0) goto err;
  ...
  if ((t->mode & SIGNATURE_MODE_SIGN_ONLY) == 0) {
    if (digested) {
      if (EVP_PKEY_verify_init_ex2(ctx, sigalg, paramsverify) <= 0) goto err;
    } else {
      if (EVP_PKEY_verify_message_init(ctx, sigalg, paramsverify) <= 0)
        goto err;
    }
    OSSL_SELF_TEST_oncorrupt_byte(st, psig);
    if (EVP_PKEY_verify(ctx, psig, siglen, t->msg, t->msg_len) <= 0) goto err;
  }
  ret = 1;
err:
  if (psig != sig) OPENSSL_free(psig);
  EVP_PKEY_free(pkey);
  ...
  if (t->entropy != NULL) {
    if (!reset_main_drbg(libctx)) ret = 0;
  }
  OSSL_SELF_TEST_onend(st, ret);
  return ret;
}
...
static int self_test_kem_encapsulate(const ST_KAT_KEM* t, OSSL_SELF_TEST* st,
  ...
  *params = OSSL_PARAM_construct_octet_string(
  ...
  if (EVP_PKEY_encapsulate_init(ctx, params) <= 0) goto err;

  /* Allocate output buffers */
  wrapped = OPENSSL_malloc(wrappedlen);
  secret = OPENSSL_malloc(secretlen);
  if (wrapped == NULL || secret == NULL) goto err;
...
static int self_test_kem_decapsulate(const ST_KAT_KEM* t, OSSL_SELF_TEST* st,
  ...
  if (EVP_PKEY_decapsulate_init(ctx, NULL) <= 0) goto err;

  /* Allocate output buffer */
  secret = OPENSSL_malloc(secretlen);
  if (secret == NULL) goto err;
...
static int self_test_digests(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx) {
  ...
  for (i = 0; i < (int)OSSL_NELEM(st_kat_digest_tests); ++i) {
    if (!self_test_digest(&st_kat_digest_tests[i], st, libctx)) ret = 0;
  }
  return ret;
}
...
static int self_test_ciphers(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx) {
  ...
  for (i = 0; i < (int)OSSL_NELEM(st_kat_cipher_tests); ++i) {
    if (!self_test_cipher(&st_kat_cipher_tests[i], st, libctx)) ret = 0;
  }
  return ret;
}
...
static int self_test_kems(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx) {
  ...
  for (i = 0; i < (int)OSSL_NELEM(st_kat_kem_tests); ++i) {
    if (!self_test_kem(&st_kat_kem_tests[i], st, libctx)) ret = 0;
  }
#endif
  return ret;
}
...
static int self_test_kdfs(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx) {
  ...
  for (i = 0; i < (int)OSSL_NELEM(st_kat_kdf_tests); ++i) {
    if (!self_test_kdf(&st_kat_kdf_tests[i], st, libctx)) ret = 0;
  }
  return ret;
}
...
static int self_test_drbgs(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx) {
  ...
  for (i = 0; i < (int)OSSL_NELEM(st_kat_drbg_tests); ++i) {
    if (!self_test_drbg(&st_kat_drbg_tests[i], st, libctx)) ret = 0;
  }
  return ret;
}
...
static int self_test_kas(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx) {
  ...
  for (i = 0; i < (int)OSSL_NELEM(st_kat_kas_tests); ++i) {
    if (!self_test_ka(&st_kat_kas_tests[i], st, libctx)) ret = 0;
  }
#endif
...
static int self_test_signatures(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx) {
  ...
  for (i = 0; i < (int)OSSL_NELEM(st_kat_sign_tests); ++i) {
    if (!self_test_digest_sign(&st_kat_sign_tests[i], st, libctx)) ret = 0;
  }
  return ret;
}
...
static int set_kat_drbg(OSSL_LIB_CTX* ctx, const unsigned char* entropy,
  ...
  EVP_RAND_free(rand);
  if (parent_rand == NULL) goto err;

  drbg_params[0] =
      OSSL_PARAM_construct_uint(OSSL_RAND_PARAM_STRENGTH, &strength);
  if (!EVP_RAND_CTX_set_params(parent_rand, drbg_params)) goto err;
  ...
  EVP_RAND_free(rand);
  if (kat_rand == NULL) goto err;

  drbg_params[0] = OSSL_PARAM_construct_utf8_string("digest", "SHA256", 0);
  if (!EVP_RAND_CTX_set_params(kat_rand, drbg_params)) goto err;
...
static int reset_main_drbg(OSSL_LIB_CTX* ctx) {
  ...
  if (!RAND_set0_private(ctx, main_rand)) ret = 0;
  if (kat_rand != NULL) {
...
int SELF_TEST_kats(OSSL_SELF_TEST* st, OSSL_LIB_CTX* libctx) {
  ...
  if (!self_test_digests(st, libctx)) ret = 0;
  if (!self_test_ciphers(st, libctx)) ret = 0;
#ifndef OPENSSL_NO_LMS
  /*
   ...
   */
  if (!self_test_LMS(st, libctx)) ret = 0;
#endif /* OPENSSL_NO_LMS */
  if (!self_test_signatures(st, libctx)) ret = 0;
  if (!self_test_kdfs(st, libctx)) ret = 0;
  if (!self_test_drbgs(st, libctx)) ret = 0;
  if (!self_test_kas(st, libctx)) ret = 0;
  if (!self_test_asym_keygens(st, libctx)) ret = 0;
  if (!self_test_kems(st, libctx)) ret = 0;
