...
static void
init_fips_option(FIPS_OPTION* opt, int enabled)
{
  opt->enabled = enabled;
...
void*
ossl_fips_prov_ossl_ctx_new(OSSL_LIB_CTX* libctx)
{
...
void
ossl_fips_prov_ossl_ctx_free(void* fgbl)
{
  OPENSSL_free(fgbl);
}

static int
fips_random_bytes(ossl_unused void* vprov,
                  int which,
                  void* buf,
                  size_t n,
 ...
 */
static int
fips_get_params_from_core(FIPS_GLOBAL* fgbl)
{
  OSSL_PARAM core_params[32], *p = core_params;

#define OSSL_FIPS_PARAM(structname, paramname)                                 \
  *p++ =                                                                       \
    OSSL_PARAM_construct_utf8_ptr(paramname,                                   \
                                  (char**)&fgbl->selftest_params.structname,   \
...
#define OSSL_FIPS_PARAM(structname, paramname, initvalue)                      \
  *p++ =                                                                       \
    OSSL_PARAM_construct_utf8_ptr(OSSL_PROV_PARAM_##paramname,                 \
...
static int
fips_get_params(void* provctx, OSSL_PARAM params[])
{
  OSSL_PARAM* p;
...
static const OSSL_ALGORITHM fips_digests_internal[] = {
   ...
   * KMAC128 and KMAC256.
   */
  { PROV_NAMES_KECCAK_KMAC_128,
    FIPS_DEFAULT_PROPERTIES,
    ossl_keccak_kmac_128_functions },
  { PROV_NAMES_KECCAK_KMAC_256,
    FIPS_DEFAULT_PROPERTIES,
...
static const OSSL_ALGORITHM fips_keymgmt[] = {
  ...
  { PROV_NAMES_ML_DSA_44,
    FIPS_DEFAULT_PROPERTIES,
    ossl_ml_dsa_44_keymgmt_functions,
  ...
  { PROV_NAMES_ML_DSA_87,
    FIPS_DEFAULT_PROPERTIES,
    ossl_ml_dsa_87_keymgmt_functions,
  ...
  { PROV_NAMES_LMS,
    FIPS_DEFAULT_PROPERTIES,
    ossl_lms_keymgmt_functions,
    PROV_DESCS_LMS },
#endif
#ifndef OPENSSL_NO_ML_KEM
  { PROV_NAMES_ML_KEM_512,
    FIPS_DEFAULT_PROPERTIES,
    ossl_ml_kem_512_keymgmt_functions,
  ...
  { PROV_NAMES_ML_KEM_1024,
    FIPS_DEFAULT_PROPERTIES,
    ossl_ml_kem_1024_keymgmt_functions,
  ...
  { PROV_NAMES_X448MLKEM1024,
    FIPS_DEFAULT_PROPERTIES,
    ossl_mlx_x448_kem_kmgmt_functions,
...
fips_query(void* provctx, int operation_id, int* no_cache)
{
  *no_cache = 0;
...
static void
fips_intern_teardown(void* provctx)
{
  /*
...
#endif
int
OSSL_provider_init_int(const OSSL_CORE_HANDLE* handle,
...
int
ossl_fips_intern_provider_init(const OSSL_CORE_HANDLE* handle,
                               const OSSL_DISPATCH* in,
                               const OSSL_DISPATCH** out,
                               void** provctx)
{
...
FIPS_get_core_handle(OSSL_LIB_CTX* libctx)
{
  FIPS_GLOBAL* fgbl =
...
void*
CRYPTO_clear_realloc(void* addr,
                     size_t old_num,
                     size_t num,
                     const char* file,
                     int line)
...
void*
CRYPTO_aligned_alloc(size_t num,
                     size_t align,
                     void** freeptr,
                     const char* file,
                     int line)
...
void
OSSL_SELF_TEST_get_callback(OSSL_LIB_CTX* libctx,
                            OSSL_CALLBACK** cb,
                            void** cbarg)
{
  assert(libctx != NULL);
...
void
OSSL_INDICATOR_get_callback(OSSL_LIB_CTX* libctx, OSSL_INDICATOR_CALLBACK** cb)
{
  assert(libctx != NULL);
