...
static int fips_get_params(void* provctx, OSSL_PARAM params[]) {
  ...
  if (p != NULL && !OSSL_PARAM_set_utf8_ptr(p, FIPS_VENDOR)) return 0;
  p = OSSL_PARAM_locate(params, OSSL_PROV_PARAM_VERSION);
  if (p != NULL && !OSSL_PARAM_set_utf8_ptr(p, OPENSSL_VERSION_STR)) return 0;
  p = OSSL_PARAM_locate(params, OSSL_PROV_PARAM_BUILDINFO);
  if (p != NULL && !OSSL_PARAM_set_utf8_ptr(p, OPENSSL_FULL_VERSION_STR))
    return 0;
  p = OSSL_PARAM_locate(params, OSSL_PROV_PARAM_STATUS);
  if (p != NULL && !OSSL_PARAM_set_int(p, ossl_prov_is_running())) return 0;
...
static const OSSL_ALGORITHM* fips_query_internal(void* provctx,
                                                 int operation_id,
                                                 int* no_cache) {
  *no_cache = 0;
...
int OSSL_provider_init_int(const OSSL_CORE_HANDLE* handle,
  ...
  if (!ossl_prov_seeding_from_dispatch(in)) goto err;
...
#define set_func(c, f) \
   ...
   */
  if (!ossl_thread_register_fips(libctx)) goto err;
   ...
   * we need it.
   */
  if (!ossl_provider_activate_fallbacks(libctx)) goto err;
...
int ossl_fips_intern_provider_init(const OSSL_CORE_HANDLE* handle,
  ...
  if (c_internal_get_libctx == NULL) return 0;
