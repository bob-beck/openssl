...
static int
integrity_self_test(OSSL_SELF_TEST* ev, OSSL_LIB_CTX* libctx)
{
  int ok = 0;
  ...
  OSSL_SELF_TEST_onbegin(
    ev, OSSL_SELF_TEST_TYPE_KAT_INTEGRITY, OSSL_SELF_TEST_DESC_INTEGRITY_HMAC);
 ...
 */
static int
verify_integrity(OSSL_CORE_BIO* bio,
...
int
SELF_TEST_post(SELF_TEST_POST_PARAMS* st, int on_demand_test)
{
  int loclstate;
#if !defined(OPENSSL_NO_FIPS_POST)
  int ok = 0;
...
void
SELF_TEST_disable_conditional_error_state(void)
{
  FIPS_conditional_error_check = 0;
}

void
ossl_set_error_state(const char* type)
{
