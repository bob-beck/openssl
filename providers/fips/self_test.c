...
static int verify_integrity(OSSL_CORE_BIO* bio,
  ...
  if (!integrity_self_test(ev, libctx)) goto err;

  OSSL_SELF_TEST_onbegin(ev, event_type, OSSL_SELF_TEST_DESC_INTEGRITY_HMAC);

  mac = EVP_MAC_fetch(libctx, MAC_NAME, NULL);
  if (mac == NULL) goto err;
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!EVP_MAC_init(ctx, fixed_key, sizeof(fixed_key), params)) goto err;

  while (1) {
    status = read_ex_cb(bio, buf, sizeof(buf), &bytes_read);
    if (status != 1) break;
    if (!EVP_MAC_update(ctx, buf, bytes_read)) goto err;
  }
  if (!EVP_MAC_final(ctx, out, &out_len, sizeof(out))) goto err;

  OSSL_SELF_TEST_oncorrupt_byte(ev, out);
  if (expected_len != out_len || memcmp(expected, out, out_len) != 0) goto err;
  ret = 1;
err:
...
int SELF_TEST_post(SELF_TEST_POST_PARAMS* st, int on_demand_test) {
  ...
  if (st != NULL) (*st->bio_free_cb)(bio_module);
