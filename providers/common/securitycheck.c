...
int ossl_ec_check_security_strength(const EC_GROUP* group, int protect) {
   ...
   * security strength
   */
  if (protect && strength < OSSL_FIPS_MIN_SECURITY_STRENGTH_BITS) return 0;
  return 1;
}
...
int ossl_dsa_check_key(const DSA* dsa, int sign) {
   ...
   */
  if (!sign) {
    if (L < 512 || N < 160) return 0;
    if (L < 2048 || N < 224) return 1;
  }
