...
static int set_propq(const OSSL_PARAM* propq, const char** propquery) {
  *propquery = NULL;
  if (propq != NULL) {
    if (propq->data_type != OSSL_PARAM_UTF8_STRING) return 0;
...
#if !defined(FIPS_MODULE) && !defined(OPENSSL_NO_ENGINE)
  if (e != NULL) {
    if (e->data_type != OSSL_PARAM_UTF8_STRING) return 0;
...
int ossl_prov_cipher_load(PROV_CIPHER* pc, const OSSL_PARAM* cipher,
  ...
  if (cipher->data_type != OSSL_PARAM_UTF8_STRING) return 0;
...
int ossl_prov_digest_load(PROV_DIGEST* pd, const OSSL_PARAM* digest,
  ...
  if (digest->data_type != OSSL_PARAM_UTF8_STRING) return 0;
  ...
  if (pd->md == NULL) {
    ...
    if (md != NULL && md->origin != EVP_ORIG_GLOBAL) pd->md = md;
  }
#endif
  if (pd->md != NULL)
    ERR_pop_to_mark();
...
int ossl_prov_set_macctx(EVP_MAC_CTX* macctx, const char* ciphername,
...
int ossl_prov_macctx_load(EVP_MAC_CTX** macctx, const OSSL_PARAM* pmac,
  ...
  if (macname == NULL && pmac != NULL)
    if (!OSSL_PARAM_get_utf8_string_ptr(pmac, &macname)) return 0;
   ...
   */
  if (*macctx == NULL) return 1;

  if (ciphername == NULL && pcipher != NULL)
    if (!OSSL_PARAM_get_utf8_string_ptr(pcipher, &ciphername)) return 0;
  if (mdname == NULL && pdigest != NULL)
    if (!OSSL_PARAM_get_utf8_string_ptr(pdigest, &mdname)) return 0;
...
void ossl_prov_cache_exported_algorithms(const OSSL_ALGORITHM_CAPABLE* in,
  ...
  if (out[0].algorithm_names == NULL) {
    for (i = j = 0; in[i].alg.algorithm_names != NULL; ++i) {
      if (in[i].capable == NULL || in[i].capable()) out[j++] = in[i].alg;
    }
    out[j++] = in[i].alg;
  }
}
...
int ossl_prov_memdup(const void* src, size_t src_len, unsigned char** dest,
                     size_t* dest_len) {
  if (src != NULL) {
    if ((*dest = OPENSSL_memdup(src, src_len)) == NULL) return 0;
    *dest_len = src_len;
  } else {
    *dest = NULL;
    *dest_len = 0;
