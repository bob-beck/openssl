...
void
ossl_prov_cipher_reset (PROV_CIPHER *pc)
{
...
int
ossl_prov_cipher_copy (PROV_CIPHER *dst, const PROV_CIPHER *src)
{
...
static int
set_propq (const OSSL_PARAM *propq, const char **propquery)
{
  *propquery = NULL;
  if (propq != NULL)
    {
...
static int
set_engine (const OSSL_PARAM *e, ENGINE **engine)
{
#if !defined(FIPS_MODULE) && !defined(OPENSSL_NO_ENGINE)
...
int
ossl_prov_cipher_load (PROV_CIPHER *pc, const OSSL_PARAM *cipher,
                       ...
                       OSSL_LIB_CTX *ctx)
{
  ...
  pc->cipher = pc->alloc_cipher
      = EVP_CIPHER_fetch (ctx, cipher->data, propquery);
#ifndef FIPS_MODULE /* Inside the FIPS module, we don't support legacy        \
                       ciphers */
  if (pc->cipher == NULL)
    {
...
int
ossl_prov_cipher_load_from_params (PROV_CIPHER *pc, const OSSL_PARAM params[],
                                   OSSL_LIB_CTX *ctx)
{
  return ossl_prov_cipher_load (
      pc, OSSL_PARAM_locate_const (params, OSSL_ALG_PARAM_CIPHER),
      OSSL_PARAM_locate_const (params, OSSL_ALG_PARAM_PROPERTIES),
      OSSL_PARAM_locate_const (params, OSSL_ALG_PARAM_ENGINE), ctx);
}

const EVP_CIPHER *
ossl_prov_cipher_cipher (const PROV_CIPHER *pc)
{
  return pc->cipher;
}

ENGINE *
ossl_prov_cipher_engine (const PROV_CIPHER *pc)
{
  return pc->engine;
}

void
ossl_prov_digest_reset (PROV_DIGEST *pd)
{
...
int
ossl_prov_digest_copy (PROV_DIGEST *dst, const PROV_DIGEST *src)
{
...
const EVP_MD *
ossl_prov_digest_fetch (PROV_DIGEST *pd, OSSL_LIB_CTX *libctx,
...
int
ossl_prov_digest_load (PROV_DIGEST *pd, const OSSL_PARAM *digest,
                       const OSSL_PARAM *propq, const OSSL_PARAM *engine,
                       OSSL_LIB_CTX *ctx)
{
  const char *propquery;
...
int
ossl_prov_digest_load_from_params (PROV_DIGEST *pd, const OSSL_PARAM params[],
                                   OSSL_LIB_CTX *ctx)
{
  return ossl_prov_digest_load (
      pd, OSSL_PARAM_locate_const (params, OSSL_ALG_PARAM_DIGEST),
      OSSL_PARAM_locate_const (params, OSSL_ALG_PARAM_PROPERTIES),
      OSSL_PARAM_locate_const (params, OSSL_ALG_PARAM_ENGINE), ctx);
}

void
ossl_prov_digest_set_md (PROV_DIGEST *pd, EVP_MD *md)
{
...
const EVP_MD *
ossl_prov_digest_md (const PROV_DIGEST *pd)
{
  return pd->md;
}

ENGINE *
ossl_prov_digest_engine (const PROV_DIGEST *pd)
{
  return pd->engine;
}

int
ossl_prov_set_macctx (EVP_MAC_CTX *macctx, const char *ciphername,
                      const char *mdname, const char *engine,
...
int
ossl_prov_macctx_load (EVP_MAC_CTX **macctx, const OSSL_PARAM *pmac,
                       const OSSL_PARAM *pcipher, const OSSL_PARAM *pdigest,
                       const OSSL_PARAM *propq, const OSSL_PARAM *pengine,
...
int
ossl_prov_macctx_load_from_params (EVP_MAC_CTX **macctx,
                                   ...
                                   const char *mdname, OSSL_LIB_CTX *libctx)
{
  return ossl_prov_macctx_load (
      ...
      OSSL_PARAM_locate_const (params, OSSL_ALG_PARAM_PROPERTIES),
      OSSL_PARAM_locate_const (params, OSSL_ALG_PARAM_ENGINE), macname,
      ciphername, mdname, libctx);
}

void
ossl_prov_cache_exported_algorithms (const OSSL_ALGORITHM_CAPABLE *in,
                                     OSSL_ALGORITHM *out)
{
  int i, j;
...
int
ossl_prov_memdup (const void *src, size_t src_len, unsigned char **dest,
                  size_t *dest_len)
{
  if (src != NULL)
    {
