...
static const TLS_GROUP_CONSTANTS group_list[] = {
  ...
  { OSSL_TLS_GROUP_ID_SecP256r1MLKEM768,
    ...
    1 },
  /* 43 */
  { OSSL_TLS_GROUP_ID_SecP384r1MLKEM1024,
    ML_KEM_1024_SECBITS,
    TLS1_3_VERSION,
    0,
    -1,
...
#define TLS_GROUP_ENTRY(tlsname, realname, algorithm, idx)                     \
  { OSSL_PARAM_utf8_string(                                                    \
      OSSL_CAPABILITY_TLS_GROUP_NAME, tlsname, sizeof(tlsname)),               \
    OSSL_PARAM_utf8_string(                                                    \
      OSSL_CAPABILITY_TLS_GROUP_NAME_INTERNAL, realname, sizeof(realname)),    \
    OSSL_PARAM_utf8_string(                                                    \
      OSSL_CAPABILITY_TLS_GROUP_ALG, algorithm, sizeof(algorithm)),            \
...
static int
tls_group_capability(OSSL_CALLBACK* cb, void* arg)
{
#if !defined(OPENSSL_NO_EC) || !defined(OPENSSL_NO_DH) ||                      \
  !defined(OPENSSL_NO_ML_KEM)
  size_t i;
...
#define TLS_SIGALG_ENTRY(tlsname, algorithm, oid, idx)                         \
  { OSSL_PARAM_utf8_string(                                                    \
      OSSL_CAPABILITY_TLS_SIGALG_IANA_NAME, tlsname, sizeof(tlsname)),         \
    OSSL_PARAM_utf8_string(                                                    \
      OSSL_CAPABILITY_TLS_SIGALG_NAME, algorithm, sizeof(algorithm)),          \
...
static int
tls_sigalg_capability(OSSL_CALLBACK* cb, void* arg)
{
#if !defined(OPENSSL_NO_ML_DSA)
  size_t i;
...
int
ossl_prov_get_capabilities(void* provctx,
