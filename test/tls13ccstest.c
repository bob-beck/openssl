...
watchccs_free(BIO* a);
static int
watchccs_read(BIO* b, char* out, int outl);
static int
watchccs_write(BIO* b, const char* in, int inl);
static long
watchccs_ctrl(BIO* b, int cmd, long num, void* ptr);
static int
watchccs_gets(BIO* bp, char* buf, int size);
static int
watchccs_puts(BIO* bp, const char* str);
...
static int
watchccs_read(BIO* bio, char* out, int outl)
{
  int ret = 0;
...
static int
watchccs_write(BIO* bio, const char* in, int inl)
{
  int ret = 0;
  BIO* next = BIO_next(bio);
...
static long
watchccs_ctrl(BIO* bio, int cmd, long num, void* ptr)
{
  long ret;
  BIO* next = BIO_next(bio);
...
static int
watchccs_gets(BIO* bio, char* buf, int size)
{
...
static int
watchccs_puts(BIO* bio, const char* str)
{
  return watchccs_write(bio, str, (int)strlen(str));
}

static int
test_tls13ccs(int tst)
{
