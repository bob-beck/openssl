...
static int watchccs_read(BIO* bio, char* out, int outl) {
  ...
  if (ret <= 0 && BIO_should_read(next)) BIO_set_retry_read(bio);
...
static int watchccs_write(BIO* bio, const char* in, int inl) {
  ...
  if (!PACKET_buf_init(&pkt, (const unsigned char*)in, inl)) return 0;
  ...
  while (PACKET_remaining(&pkt)) {
    ...
    if (rectype == SSL3_RT_HANDSHAKE) {
      ...
      if (msgtype == SSL3_MT_CLIENT_HELLO) {
        ...
        if (chseen == 1) {
          ...
          if (!PACKET_copy_bytes(&sessionid, chsessid, chsessidlen)) return 0;
        } else {
          /*
    ...
    if (recvers != expectedrecvers) badvers = 1;
  }

  ret = BIO_write(next, in, inl);
  if (ret <= 0 && BIO_should_write(next)) BIO_set_retry_write(bio);
...
static int test_tls13ccs(int tst) {
  ...
  if ((tst >= 3 && tst <= 5) || tst >= 9) {
    ...
    if (!TEST_true(SSL_CTX_set1_groups_list(sctx, "ffdhe3072"))) goto err;
#endif
#else
    if (!TEST_true(SSL_CTX_set1_groups_list(sctx, "P-384"))) goto err;
#endif
  }
  ...
  if (tst >= 6) {
    ...
    if (!TEST_true(SSL_set_session(cssl, sess)) ||
      ...
      goto err;
    if (tst <= 8) {
      if (!TEST_int_gt(SSL_connect(cssl), 0)) goto err;
    } else {
      if (!TEST_int_le(SSL_connect(cssl), 0)) goto err;
    }
  ...
  if (!TEST_true(create_ssl_connection(sssl, cssl, SSL_ERROR_NONE))) goto err;
