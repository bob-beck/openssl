...
static int
tls_corrupt_read(BIO* bio, char* out, int outl)
{
  int ret;
  BIO* next = BIO_next(bio);
...
static int
tls_corrupt_write(BIO* bio, const char* in, int inl)
{
  int ret;
  BIO* next = BIO_next(bio);
  char* copy;
...
static long
tls_corrupt_ctrl(BIO* bio, int cmd, long num, void* ptr)
{
  long ret;
  BIO* next = BIO_next(bio);
...
static int
tls_corrupt_gets(BIO* bio, char* buf, int size)
{
...
static int
tls_corrupt_puts(BIO* bio, const char* str)
{
  /* We don't support this - not needed anyway */
  return -1;
}
...
static void
bio_f_tls_corrupt_filter_free(void)
{
...
setup_cipher_list(void)
{
   ...
   */
  cipher_list =
    OPENSSL_malloc_array(sk_SSL_CIPHER_num(sk_ciphers), sizeof(cipher_list[0]));
