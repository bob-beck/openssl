...
static int tls_corrupt_write(BIO* bio, const char* in, int inl) {
  ...
  if (docorrupt) {
    if (!TEST_ptr(copy = OPENSSL_memdup(in, inl))) return 0;
...
static int setup_cipher_list(void) {
  ...
  if (!TEST_ptr(cipher_list)) goto err;
  ...
  if (TEST_int_ne(j, 0)) numciphers = j;
...
static int test_ssl_corrupt(int testidx) {
   ...
   */
  if (!TEST_true(SSL_CTX_set_max_proto_version(cctx, TLS1_2_VERSION))) goto end;

  if (!TEST_ptr(c_to_s_fbio = BIO_new(bio_f_tls_corrupt_filter()))) goto end;
  ...
  if (!TEST_int_ge(SSL_write(client, junk, sizeof(junk)), 0)) goto end;

  if (!TEST_int_lt(SSL_read(server, junk, sizeof(junk)), 0)) goto end;
...
int setup_tests(void) {
  ...
  n = setup_cipher_list();
  if (n > 0) ADD_ALL_TESTS(test_ssl_corrupt, n);
  return 1;
}
