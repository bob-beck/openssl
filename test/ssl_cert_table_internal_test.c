...
static int
do_test_cert_table(int nid, uint32_t amask, size_t idx, const char* idxname)
{
  ...
  TEST_error(
    "Invalid table entry for certificate type %s, index %zu", idxname, idx);
  if (clu->nid != nid)
