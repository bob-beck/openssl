...
static int
fetch_sig (OSSL_LIB_CTX *ctx, const char *alg, const char *propq,
...
static int
reset_ctx_providers (OSSL_LIB_CTX **ctx, OSSL_PROVIDER *providers[2],
                     const char *prop)
{
  ...
  for (i = 0; i < OSSL_NELEM (properties_test); i++)
    {
       ...
       */
      if (!TEST_int_eq (
              reset_ctx_providers (&my_libctx, providers, libctx_prop), 1))
        goto end;
      if (!TEST_int_ge (BIO_seek (bio_priv, 0), 0)
          || !TEST_ptr (
              pkey = PEM_read_bio_PrivateKey_ex (bio_priv, NULL, NULL, NULL,
                                                 my_libctx, explicit_prop))
