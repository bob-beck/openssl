...
static int
test_alternative_keygen_init(void)
{
  EVP_PKEY_CTX* ctx = NULL;
  OSSL_PROVIDER* deflt = NULL;
  OSSL_PROVIDER* fake_rsa = NULL;
...
test_pkey_store(int idx)
{
  ...
  if (!TEST_ptr(
        ctx = OSSL_STORE_open_ex(
          "fake_rsa:test", libctx, propq, NULL, NULL, NULL, NULL, NULL)))
    goto end;
...
static int
fake_pw_read_string(UI* ui, UI_STRING* uis)
{
...
test_pkey_store_open_ex(void)
{
  ...
  if (!TEST_ptr(ctx = OSSL_STORE_open_ex("fake_rsa:openpwtest",
                                         libctx,
                                         propq,
                                         ui_method,
                                         NULL,
  ...
  if (!TEST_ptr_null(
        ctx = OSSL_STORE_open_ex(
          "fake_rsa:openpwtest", libctx, propq, NULL, NULL, NULL, NULL, NULL)))
    goto end;
...
static int
reset_ctx_providers(OSSL_LIB_CTX** ctx,
...
static int
test_pkey_provider_decoder_props(void)
{
  ...
  if (!TEST_ptr(bio_priv = BIO_new(BIO_s_mem())) ||
      ...
      !TEST_ptr(pkey = PEM_read_bio_PrivateKey_ex(
                  bio_priv, NULL, NULL, NULL, NULL, NULL)) ||
  ...
  for (i = 0; i < OSSL_NELEM(properties_test); i++) {
    ...
    p = encoded_pub;
    if (!TEST_ptr(
          pkey = d2i_PUBKEY_ex(NULL, &p, len_pub, my_libctx, explicit_prop)) ||
        !TEST_ptr_eq(EVP_PKEY_get0_provider(pkey), *curr_provider))
      goto end;
    EVP_PKEY_free(pkey);
    pkey = NULL;
