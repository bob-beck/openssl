...
static int fetch_sig(OSSL_LIB_CTX* ctx, const char* alg, const char* propq,
  ...
  if (!TEST_ptr(sig)) return 0;

  if (!TEST_ptr(prov = EVP_SIGNATURE_get0_provider(sig))) goto end;
...
static int test_pkey_sig(void) {
  ...
  if (!TEST_ptr(deflt = OSSL_PROVIDER_load(libctx, "default"))) goto end;
  ...
  for (i = 0; i < 3; i++) {
     ...
     */
    if (!TEST_int_eq(EVP_PKEY_sign_init(ctx), 1)) goto end;
...
static int test_alternative_keygen_init(void) {
  ...
  if (!TEST_int_gt(EVP_PKEY_keygen_init(ctx), 0)) goto end;

  if (!TEST_ptr(provider = EVP_PKEY_CTX_get0_provider(ctx))) goto end;

  if (!TEST_ptr(provname = OSSL_PROVIDER_get0_name(provider))) goto end;

  if (!TEST_str_eq(provname, "default")) goto end;
  ...
  if (!TEST_str_eq(provname, "fake-rsa")) goto end;
...
static int test_pkey_eq(void) {
  ...
  if (!TEST_int_eq(EVP_PKEY_eq(pkey_fake, pkey_dflt), 1)) goto end;
...
static int test_pkey_store(int idx) {
  ...
  if (!TEST_ptr(fake_rsa = fake_rsa_start(libctx))) goto end;
  ...
  if (!TEST_ptr(pkey) || !TEST_int_eq(EVP_PKEY_is_a(pkey, "RSA"), 1)) goto end;
...
static int test_pkey_delete(void) {
  ...
  if (!TEST_int_eq(
          ...
          1))
    goto end;
  if (!TEST_int_eq(OSSL_STORE_close(ctx), 1)) goto end;
  ...
  while (!OSSL_STORE_eof(ctx)) {
    ...
    if (!TEST_ptr_null(info)) goto end;
  }
...
static int test_pkey_store_open_ex(void) {
  ...
  if (!TEST_ptr(ui_method = UI_create_method("PW Callbacks"))) goto end;

  if (UI_method_set_reader(ui_method, fake_pw_read_string)) goto end;
...
static int test_pkey_provider_decoder_props(void) {
  ...
  /* Load private key BIO, DER-encoded public key and PKCS#8 private key for
   * testing */
  if (!TEST_ptr(bio_priv = BIO_new(BIO_s_mem())) ||
      !TEST_int_gt(
          BIO_write(bio_priv, pem_rsa_priv_key, sizeof(pem_rsa_priv_key)), 0) ||
      !TEST_ptr(pkey = PEM_read_bio_PrivateKey_ex(bio_priv, NULL, NULL, NULL,
                                                  NULL, NULL)) ||
  ...
  for (i = 0; i < OSSL_NELEM(properties_test); i++) {
    ...
    /*
     * Decoding a PEM-encoded key uses the properties to select the right
     * provider. Using a PEM-encoding adds an extra decoder before the key is
     * created.
     */
    if (!TEST_int_eq(reset_ctx_providers(&my_libctx, providers, libctx_prop),
                     1))
      goto end;
    ...
    /* Decoding a DER-encoded X509_PUBKEY uses the properties to select the
     * right provider */
    if (!TEST_int_eq(reset_ctx_providers(&my_libctx, providers, libctx_prop),
                     1))
      goto end;
    ...
    /* Decoding a PKCS8_PRIV_KEY_INFO uses the properties to select the right
     * provider */
    if (!TEST_int_eq(reset_ctx_providers(&my_libctx, providers, libctx_prop),
                     1))
      goto end;
