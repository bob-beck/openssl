...
static EVP_PKEY*
make_template(const char* type, OSSL_PARAM* genparams)
{
  EVP_PKEY* pkey = NULL;
...
static EVP_PKEY*
make_key(const char* type, EVP_PKEY* template, OSSL_PARAM* genparams)
{
  EVP_PKEY* pkey = NULL;
  EVP_PKEY_CTX* ctx =
...
typedef int(encoder)(const char* file,
                     const int line,
                     void** encoded,
                     long* encoded_len,
                     void* object,
                     ...
                     const char* pass,
                     const char* pcipher);
typedef int(decoder)(const char* file,
                     const int line,
                     void** object,
                     void* encoded,
                     long encoded_len,
                     ...
                     int selection,
                     const char* pass);
typedef int(tester)(const char* file,
                    const int line,
                    const void* data1,
                    size_t data1_len,
                    const void* data2,
                    size_t data2_len);
typedef int(checker)(const char* file,
                     const int line,
                     const char* type,
                     const void* data,
                     size_t data_len);
...
static int
test_encode_decode(const char* file,
                   const int line,
                   const char* type,
                   EVP_PKEY* pkey,
                   int selection,
...
static int
encode_EVP_PKEY_prov(const char* file,
                     const int line,
                     void** encoded,
                     long* encoded_len,
                     void* object,
                     ...
                     const char* pcipher)
{
  ...
  if (!TEST_FL_ptr(
        ectx = OSSL_ENCODER_CTX_new_for_pkey(
          pkey, selection, output_type, output_structure, testpropq)) ||
...
static int
encode_EVP_PKEY_i2d(const char* unused_file,
...
static int
decode_EVP_PKEY_prov(const char* file,
                     const int line,
                     void** object,
                     void* encoded,
                     long encoded_len,
...
static int
encode_EVP_PKEY_legacy_PEM(const char* file,
                           const int line,
                           void** encoded,
                           long* encoded_len,
                           void* object,
...
static int
encode_EVP_PKEY_MSBLOB(const char* file,
                       const int line,
                       void** encoded,
                       long* encoded_len,
                       void* object,
...
static int
pass_pw(char* buf, int size, int rwflag, void* userdata)
{
...
static int
encode_EVP_PKEY_PVK(const char* file,
                    const int line,
                    void** encoded,
                    long* encoded_len,
                    void* object,
                    ...
                    ossl_unused const char* pcipher)
{
  ...
  if (!TEST_FL_true(
      ...
      !TEST_FL_int_ge(
        i2b_PVK_bio_ex(
          mem_ser, pkey, enc, pass_pw, (void*)pass, testctx, testpropq),
        0) ||
      !TEST_FL_true(BIO_get_mem_ptr(mem_ser, &mem_buf) > 0) ||
      !TEST_FL_ptr(*encoded = mem_buf->data) ||
      !TEST_FL_long_gt(*encoded_len = (long)mem_buf->length, 0))
...
static void
collect_name(const char* name, void* arg)
{
  char** namelist = arg;
  char* new_namelist;
  size_t space;
...
static void
dump_der(const char* label, const void* data, size_t data_len)
{
  test_output_memory(label, data, data_len);
}

static void
dump_pem(const char* label, const void* data, size_t data_len)
{
  test_output_string(label, data, data_len - 1);
}

static int
check_unprotected_PKCS8_DER(const char* file,
                            const int line,
                            const char* type,
                            const void* data,
                            size_t data_len)
...
static int
test_unprotected_via_DER(const char* type, EVP_PKEY* key, int fips)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
                            key,
...
static int
test_unprotected_via_i2d(const char* type, EVP_PKEY* key, int fips)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
...
static int
check_unprotected_PKCS8_PEM(const char* file,
                            const int line,
                            const char* type,
                            const void* data,
                            size_t data_len)
{
  ...
  return TEST_FL_strn_eq(
    data, expected_pem_header, sizeof(expected_pem_header) - 1);
}

static int
test_unprotected_via_PEM(const char* type, EVP_PKEY* key, int fips)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
...
#ifndef OPENSSL_NO_KEYPARAMS
static int
check_params_DER(const char* file,
                 const int line,
                 const char* type,
                 const void* data,
                 size_t data_len)
...
static int
check_params_PEM(const char* file,
                 const int line,
                 const char* type,
                 const void* data,
                 size_t data_len)
{
  ...
  return TEST_FL_int_gt(BIO_snprintf(expected_pem_header,
                        ...
                        0) &&
         TEST_FL_strn_eq(
           data, expected_pem_header, strlen(expected_pem_header));
}

static int
test_params_via_DER(const char* type, EVP_PKEY* key)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
...
static int
test_params_via_PEM(const char* type, EVP_PKEY* key)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
...
static int
check_unprotected_legacy_PEM(const char* file,
                             const int line,
                             const char* type,
                             const void* data,
                             size_t data_len)
...
static int
test_unprotected_via_legacy_PEM(const char* type, EVP_PKEY* key)
{
...
static int
test_unprotected_via_MSBLOB(const char* type, EVP_PKEY* key)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
          ...
          size_t data_len)
{
  ...
  return ossl_do_PVK_header(
    &in, (unsigned int)data_len, 0, &isdss, &saltlen, &keylen);
}

static int
test_unprotected_via_PVK(const char* type, EVP_PKEY* key)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
...
static int
check_protected_PKCS8_DER(const char* file,
                          const int line,
                          const char* type,
                          const void* data,
                          size_t data_len)
...
static int
test_protected_via_DER(const char* type, EVP_PKEY* key, int fips)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
...
static int
check_protected_PKCS8_PEM(const char* file,
                          const int line,
                          const char* type,
                          const void* data,
                          size_t data_len)
...
static int
test_protected_via_PEM(const char* type, EVP_PKEY* key, int fips)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
...
static int
check_protected_legacy_PEM(const char* file,
                           const int line,
                           const char* type,
                           const void* data,
                           size_t data_len)
...
static int
test_protected_via_legacy_PEM(const char* type, EVP_PKEY* key)
{
  if (!default_libctx || is_fips)
    return TEST_skip("Test not available if using a non-default library "
                     "context or FIPS provider");
...
static int
test_protected_via_PVK(const char* type, EVP_PKEY* key)
{
  int ret = 0;
  ...
  ret = test_encode_decode(__FILE__,
                           __LINE__,
                           type,
                           key,
                           OSSL_KEYMGMT_SELECT_KEYPAIR |
...
static int
check_public_DER(const char* file,
                 const int line,
                 const char* type,
                 const void* data,
                 size_t data_len)
...
static int
test_public_via_DER(const char* type, EVP_PKEY* key, int fips)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
...
static int
check_public_PEM(const char* file,
                 const int line,
                 const char* type,
                 const void* data,
                 size_t data_len)
...
static int
test_public_via_PEM(const char* type, EVP_PKEY* key, int fips)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
...
static int
check_public_MSBLOB(const char* file,
                    const int line,
                    const char* type,
                    const void* data,
                    size_t data_len)
...
static int
test_public_via_MSBLOB(const char* type, EVP_PKEY* key)
{
  return test_encode_decode(__FILE__,
                            __LINE__,
                            type,
...
static int
do_create_ec_explicit_prime_params(OSSL_PARAM_BLD* bld,
...
static int
create_ec_explicit_prime_params_namedcurve(OSSL_PARAM_BLD* bld)
{
  ...
  return do_create_ec_explicit_prime_params(
    bld, prime256v1_gen, sizeof(prime256v1_gen));
}

static int
create_ec_explicit_prime_params(OSSL_PARAM_BLD* bld)
{
  /* 2G */
  ...
  return do_create_ec_explicit_prime_params(
    bld, prime256v1_gen2, sizeof(prime256v1_gen2));
}

#ifndef OPENSSL_NO_EC2M
static int
do_create_ec_explicit_trinomial_params(OSSL_PARAM_BLD* bld,
                                       const unsigned char* gen,
                                       size_t gen_len)
{
  ...
  static const unsigned char a_data[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
  static const unsigned char b_data[] = { 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
                                          0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
...
static int
create_ec_explicit_trinomial_params_namedcurve(OSSL_PARAM_BLD* bld)
{
...
static int
create_ec_explicit_trinomial_params(OSSL_PARAM_BLD* bld)
{
...
setup_tests(void)
{
  ...
  OSSL_PARAM EC_params[] = { OSSL_PARAM_utf8_string(
                               "group", groupname, sizeof(groupname) - 1),
                             OSSL_PARAM_END };
#endif
  ...
  if (default_libctx) {
    ...
    if (!test_get_libctx(
          &testctx, &nullprov, config_file, &deflprov, prov_name))
      return 0;
  }
