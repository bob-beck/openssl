...
static EVP_PKEY* make_template(const char* type, OSSL_PARAM* genparams) {
   ...
   */
  if (strcmp(type, "DH") == 0) return get_dh512(keyctx);

  if (strcmp(type, "X9.42 DH") == 0) return get_dhx512(keyctx);
#endif
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (i == 0) {
      ...
      if (selection == OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS) {
        ...
        if (!TEST_FL_int_eq(EVP_PKEY_eq(pkey, testpkey), 1)) goto end;
      }
    }
  }
  ok = 1;
...
static int encode_EVP_PKEY_MSBLOB(const char* file, const int line,
  ...
  if (!TEST_FL_ptr(mem_ser = BIO_new(BIO_s_mem()))) goto end;

  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
    if (!TEST_FL_int_ge(i2b_PrivateKey_bio(mem_ser, pkey), 0)) goto end;
  } else {
    if (!TEST_FL_int_ge(i2b_PublicKey_bio(mem_ser, pkey), 0)) goto end;
  }
  ...
  if (TEST_FL_ptr(p8inf)) {
    ...
    if (TEST_FL_ptr(pkey)) {
      if (!(ok = TEST_FL_true(EVP_PKEY_is_a(pkey, type)))) {
        ...
        if (namelist != NULL) TEST_note("%s isn't any of %s", type, namelist);
...
static int test_protected_via_PVK(const char* type, EVP_PKEY* key) {
  ...
  if (lgcyprov == NULL) return TEST_skip("Legacy provider not available");
...
int setup_tests(void) {
  ...
  if (!TEST_ptr(keyctx = OSSL_LIB_CTX_new())) return 0;
