...
static int test_text(const char* file,
                     const int line,
                     const void* data1,
                     size_t data1_len,
                     const void* data2,
...
static int test_mem(const char* file,
                    const int line,
                    const void* data1,
                    size_t data1_len,
                    const void* data2,
...
static int check_unprotected_PKCS8_DER(const char* file,
                                       const int line,
                                       const char* type,
                                       const void* data,
                                       size_t data_len) {
...
static int check_unprotected_PKCS8_PEM(const char* file,
                                       const int line,
                                       const char* type,
                                       const void* data,
                                       size_t data_len) {
...
#ifndef OPENSSL_NO_KEYPARAMS
static int check_params_DER(const char* file,
                            const int line,
                            const char* type,
                            const void* data,
                            size_t data_len) {
...
static int check_params_PEM(const char* file,
                            const int line,
                            const char* type,
                            const void* data,
                            size_t data_len) {
  ...
  return TEST_FL_int_gt(
             BIO_snprintf(expected_pem_header, sizeof(expected_pem_header),
...
static int check_unprotected_legacy_PEM(const char* file,
                                        const int line,
                                        const char* type,
                                        const void* data,
                                        size_t data_len) {
...
static int test_unprotected_via_legacy_PEM(const char* type, EVP_PKEY* key) {
  if (!default_libctx || is_fips)
    return TEST_skip(
        "Test not available if using a non-default library context or FIPS "
        "provider");
...
static int check_MSBLOB(const char* file,
                        const int line,
                        const char* type,
                        const void* data,
                        size_t data_len) {
...
static int check_PVK(const char* file,
                     const int line,
                     const char* type,
                     const void* data,
                     size_t data_len) {
...
static int check_protected_PKCS8_DER(const char* file,
                                     const int line,
                                     const char* type,
                                     const void* data,
                                     size_t data_len) {
...
static int test_protected_via_DER(const char* type, EVP_PKEY* key, int fips) {
  return test_encode_decode(
      __FILE__, __LINE__, type, key,
      OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
      "DER", "EncryptedPrivateKeyInfo", pass, pass_cipher, encode_EVP_PKEY_prov,
      decode_EVP_PKEY_prov, test_mem, check_protected_PKCS8_DER, dump_der,
      fips ? 0 : FLAG_FAIL_IF_FIPS);
}

static int check_protected_PKCS8_PEM(const char* file,
                                     const int line,
                                     const char* type,
                                     const void* data,
                                     size_t data_len) {
...
static int test_protected_via_PEM(const char* type, EVP_PKEY* key, int fips) {
  return test_encode_decode(
      __FILE__, __LINE__, type, key,
      OSSL_KEYMGMT_SELECT_KEYPAIR | OSSL_KEYMGMT_SELECT_DOMAIN_PARAMETERS,
      "PEM", "EncryptedPrivateKeyInfo", pass, pass_cipher, encode_EVP_PKEY_prov,
      decode_EVP_PKEY_prov, test_text, check_protected_PKCS8_PEM, dump_pem,
      fips ? 0 : FLAG_FAIL_IF_FIPS);
}

static int check_protected_legacy_PEM(const char* file,
                                      const int line,
                                      const char* type,
                                      const void* data,
                                      size_t data_len) {
...
static int check_public_DER(const char* file,
                            const int line,
                            const char* type,
                            const void* data,
                            size_t data_len) {
...
static int check_public_PEM(const char* file,
                            const int line,
                            const char* type,
                            const void* data,
                            size_t data_len) {
...
static int check_public_MSBLOB(const char* file,
                               const int line,
                               const char* type,
                               const void* data,
                               size_t data_len) {
