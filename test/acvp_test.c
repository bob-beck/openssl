...
static int pkey_get_bn_bytes(EVP_PKEY* pkey, const char* name,
  ...
  if (!EVP_PKEY_get_bn_param(pkey, name, &bn)) goto err;
  sz = BN_num_bytes(bn);
  buf = OPENSSL_zalloc(sz);
  if (buf == NULL) goto err;
  if (BN_bn2binpad(bn, buf, sz) <= 0) goto err;
...
static int get_ecdsa_sig_rs_bytes(const unsigned char* sig, size_t sig_len,
  ...
  if (r1 == NULL || s1 == NULL) goto err;

  r1_len = BN_num_bytes(r1);
  s1_len = BN_num_bytes(s1);
  rbuf = OPENSSL_zalloc(r1_len);
  sbuf = OPENSSL_zalloc(s1_len);
  if (rbuf == NULL || sbuf == NULL) goto err;
  if (BN_bn2binpad(r1, rbuf, r1_len) <= 0) goto err;
  if (BN_bn2binpad(s1, sbuf, s1_len) <= 0) goto err;
  *r = rbuf;
  *s = sbuf;
  *rlen = r1_len;
  *slen = s1_len;
...
static int ecdh_cofactor_derive_test(int tstid) {
  ...
  if (!ec_cofactors) return TEST_skip("not supported by FIPS provider version");
  ...
  if (!TEST_ptr(peer2 = EVP_PKEY_Q_keygen(libctx, NULL, "EC", curve))) goto err;
...
err:
  if (ret == 0) {
    ...
    TEST_note(
        "ECDH derive() was expected to %s if key cofactor is"
...
static int pkey_get_octet_bytes(EVP_PKEY* pkey, const char* name,
  ...
  if (!EVP_PKEY_get_octet_string_param(pkey, name, NULL, 0, &len)) goto err;

  buf = OPENSSL_zalloc(len);
  if (buf == NULL) goto err;

  if (!EVP_PKEY_get_octet_string_param(pkey, name, buf, len, out_len)) goto err;
  *out = buf;
  return 1;
err:
  OPENSSL_free(buf);
...
static int dsa_keygen_test(int id) {
  ...
  if (!dsasign_allowed) return TEST_skip("DSA signing is not allowed");
...
static int dsa_paramgen_test(int id) {
  ...
  if (!dsasign_allowed) {
    if (!TEST_false(EVP_PKEY_paramgen(paramgen_ctx, &param_key))) goto err;
  } else {
...
static int dsa_create_pkey(EVP_PKEY** pkey, const unsigned char* p,
  ...
  if (counter != -1) {
    if (!TEST_true(OSSL_PARAM_BLD_push_int(bld, OSSL_PKEY_PARAM_FFC_PCOUNTER,
                                           counter)))
      goto err;
  }
  if (pub != NULL) {
...
static int dsa_siggen_test(int id) {
  ...
  if (!dsasign_allowed) {
    if (!TEST_ptr_null(pkey = dsa_keygen(tst->L, tst->N))) goto err;
  } else {
    if (!TEST_ptr(pkey = dsa_keygen(tst->L, tst->N))) goto err;
    if (!TEST_true(sig_gen(pkey, NULL, tst->digest_alg, tst->msg, tst->msg_len,
                           &sig, &sig_len)) ||
...
static int cipher_enc(const char* alg, const unsigned char* pt, size_t pt_len,
  ...
  if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()) ||
    ...
    goto err;
  out_len += len;
  if (!TEST_mem_eq(out, out_len, ct, ct_len)) goto err;
  ret = 1;
err:
...
static int aes_ccm_enc_dec(const char* alg, const unsigned char* pt,
  ...
  if (!TEST_true(EVP_CipherFinal_ex(ctx, out + len, &out_len))) goto err;
  if (enc) {
    out_len += len;
    ...
    if (!TEST_mem_eq(out, out_len + len, ct, ct_len)) goto err;
  }
...
static int aes_gcm_enc_dec(const char* alg, const unsigned char* pt,
  ...
  if (!TEST_int_eq(EVP_CipherFinal_ex(ctx, out + len, &olen), pass)) goto err;
  if (!pass) {
    ret = 1;
    goto err;
  }
  ...
  if (enc) {
    ...
    if (ct != NULL && !TEST_mem_eq(out, olen, ct, ct_len)) goto err;
  }

  {
    ...
    if (ivgen != 0) *p++ = OSSL_PARAM_construct_uint(ivgenkey, &iv_generated);
    if (outiv != NULL)
...
static int self_test_events(const OSSL_PARAM params[], void* varg) {
  ...
  if (!args->enable) return 1;

  args->called++;
  p = OSSL_PARAM_locate_const(params, OSSL_PROV_PARAM_SELF_TEST_PHASE);
  if (p == NULL || p->data_type != OSSL_PARAM_UTF8_STRING) goto err;
...
static int drbg_test(int id) {
  ...
  params[1] = OSSL_PARAM_construct_end();
  if (!TEST_true(EVP_RAND_CTX_set_params(parent, params))) goto err;
  ...
  if (!TEST_true(EVP_RAND_CTX_set_params(ctx, params))) goto err;
...
static int aes_cfb1_bits_test(void) {
  ...
  if (!TEST_int_gt(EVP_CipherUpdate(ctx, out, &outlen, pt, 7), 0)) goto err;
  if (!TEST_int_eq(outlen, 7)) goto err;
  if (!TEST_mem_eq(out, (outlen + 7) / 8, expected, sizeof(expected))) goto err;
  ret = 1;
err:
  EVP_CIPHER_free(cipher);
  EVP_CIPHER_CTX_free(ctx);
