...
static int
ecdsa_create_pkey(EVP_PKEY **pkey, const char *curve_name, const unsigned char *pub, size_t pub_len, int expected)
{
    int             ret    = 0;
...
static int ecdsa_sigver_test(int id)
{
    ...
    if (!TEST_ptr(sign = ECDSA_SIG_new()) || !TEST_ptr(rbn = BN_bin2bn(tst->r, (int)tst->r_len, NULL))
        || !TEST_ptr(sbn = BN_bin2bn(tst->s, (int)tst->s_len, NULL)) || !TEST_true(ECDSA_SIG_set0(sign, rbn, sbn)))
        goto err;
    rbn = sbn = NULL;
...
static int
eddsa_create_pkey(EVP_PKEY **pkey, const char *algname, const unsigned char *pub, size_t pub_len, int expected)
{
    int             ret    = 0;
    EVP_PKEY_CTX   *ctx    = NULL;
    OSSL_PARAM_BLD *bld    = NULL;
                           ...
                           BN_CTX              *bn_ctx)
{
    ...
    if (g != NULL) {
        if (!TEST_ptr(g_bn = BN_CTX_get(bn_ctx)) || !TEST_ptr(BN_bin2bn(g, (int)g_len, g_bn))
    ...
    if (pub != NULL) {
        if (!TEST_ptr(pub_bn = BN_CTX_get(bn_ctx)) || !TEST_ptr(BN_bin2bn(pub, (int)pub_len, pub_bn))
...
static int get_dsa_sig_rs_bytes(const unsigned char *sig,
                                size_t               sig_len,
                                unsigned char      **r,
                                unsigned char      **s,
                                size_t              *r_len,
...
static int dsa_sigver_test(int id)
{
    ...
    if (!TEST_ptr(sign = DSA_SIG_new()) || !TEST_ptr(rbn = BN_bin2bn(tst->r, (int)tst->r_len, NULL))
        || !TEST_ptr(sbn = BN_bin2bn(tst->s, (int)tst->s_len, NULL)) || !TEST_true(DSA_SIG_set0(sign, rbn, sbn)))
        goto err;
    rbn = sbn = NULL;
                           ...
                           int                  pass)
{
    ...
    if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()) || !TEST_ptr(cipher = EVP_CIPHER_fetch(libctx, alg, ""))
        ...
        || !TEST_int_gt(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_IVLEN, (int)iv_len, NULL), 0)
        || !TEST_int_gt(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, (int)tag_len, enc ? NULL : (void *)tag), 0)
    ...
    if (enc) {
        out_len += len;
        if (!TEST_int_gt(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, (int)tag_len, out + out_len), 0)
                           ...
                           unsigned char       *outiv)
{
    ...
    if (!enc) {
        if (!TEST_int_gt(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, (int)tag_len, (void *)tag), 0))
            goto err;
    }
    /*
    ...
    if (enc) {
        if ((ct != NULL && !TEST_mem_eq(out, olen, ct, ct_len))
            || !TEST_int_gt(EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, (int)tag_len, out + olen), 0)
...
static int dh_create_pkey(EVP_PKEY           **pkey,
                          ...
                          int                  pass)
{
    ...
    if (priv != NULL) {
        if (!TEST_ptr(priv_bn = BN_CTX_get(bn_ctx)) || !TEST_ptr(BN_bin2bn(priv, (int)priv_len, priv_bn))
                           ...
                           BN_CTX              *bn_ctx)
{
    ...
    if (e != NULL) {
        if (!TEST_ptr(e_bn = BN_CTX_get(bn_ctx)) || !TEST_ptr(BN_bin2bn(e, (int)e_len, e_bn))
            || !TEST_true(OSSL_PARAM_BLD_push_BN(bld, OSSL_PKEY_PARAM_RSA_E, e_bn)))
            goto err;
    }
    if (d != NULL) {
        if (!TEST_ptr(d_bn = BN_CTX_get(bn_ctx)) || !TEST_ptr(BN_bin2bn(d, (int)d_len, d_bn))
...
static int rsa_keygen_test(int id)
{
    ...
    if (!TEST_ptr(bld = OSSL_PARAM_BLD_new()) || !TEST_ptr(xp1_bn = BN_bin2bn(tst->xp1, (int)tst->xp1_len, NULL))
    ...
    if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(libctx, "RSA", NULL))
        || !TEST_ptr(e_bn = BN_bin2bn(tst->e, (int)tst->e_len, NULL)) || !TEST_int_gt(EVP_PKEY_keygen_init(ctx), 0)
...
static int aes_cfb1_bits_test(void)
{
    ...
    static const unsigned char key[] =
        {0x12, 0x22, 0x58, 0x2F, 0x1C, 0x1A, 0x8A, 0x88, 0x30, 0xFC, 0x18, 0xB7, 0x24, 0x89, 0x7F, 0xC0};
    static const unsigned char iv[] =
        {0x05, 0x28, 0xB5, 0x2B, 0x58, 0x27, 0x63, 0x5C, 0x81, 0x86, 0xD3, 0x63, 0x60, 0xB0, 0xAA, 0x2B};
