...
static int
pkey_get_bn_bytes (EVP_PKEY *pkey, const char *name, unsigned char **out,
                   size_t *out_len)
{
...
static int
sig_gen (EVP_PKEY *pkey, OSSL_PARAM *params, const char *digest_name,
         const unsigned char *msg, size_t msg_len, unsigned char **sig_out,
         size_t *sig_out_len)
{
  int ret = 0;
  ...
  if (!TEST_ptr (sig = OPENSSL_malloc (sz))
      ...
      || !TEST_int_eq (EVP_DigestSignInit_ex (md_ctx, NULL, digest_name,
                                              libctx, NULL, pkey, p),
                       1)
      || !TEST_int_gt (EVP_DigestSign (md_ctx, sig, &sig_len, msg, msg_len),
                       0))
    goto err;
  *sig_out = sig;
  *sig_out_len = sig_len;
...
static int
check_verify_message (EVP_PKEY_CTX *pkey_ctx, int expected)
{
...
static int
ecdsa_create_pkey (EVP_PKEY **pkey, const char *curve_name,
                   const unsigned char *pub, size_t pub_len, int expected)
{
  int ret = 0;
...
static int
get_ecdsa_sig_rs_bytes (const unsigned char *sig, size_t sig_len,
                        unsigned char **r, unsigned char **s, size_t *rlen,
                        size_t *slen)
{
  int ret = 0;
...
static int
ecdh_cofactor_derive_test (int tstid)
{
  int ret = 0;
...
static int
pkey_get_octet_bytes (EVP_PKEY *pkey, const char *name, unsigned char **out,
                      size_t *out_len)
{
  size_t len = 0;
  unsigned char *buf = NULL;
...
static int
eddsa_create_pkey (EVP_PKEY **pkey, const char *algname,
                   const unsigned char *pub, size_t pub_len, int expected)
{
  int ret = 0;
  EVP_PKEY_CTX *ctx = NULL;
...
static int
dsa_create_pkey (EVP_PKEY **pkey, const unsigned char *p, size_t p_len,
                 const unsigned char *q, size_t q_len, const unsigned char *g,
                 size_t g_len, const unsigned char *seed, size_t seed_len,
                 int counter, int validate_pq, int validate_g,
                 const unsigned char *pub, size_t pub_len, BN_CTX *bn_ctx)
{
  int ret = 0;
  EVP_PKEY_CTX *ctx = NULL;
  OSSL_PARAM_BLD *bld = NULL;
...
static int
get_dsa_sig_rs_bytes (const unsigned char *sig, size_t sig_len,
                      unsigned char **r, unsigned char **s, size_t *r_len,
                      size_t *s_len)
{
  int ret = 0;
  unsigned char *rbuf = NULL, *sbuf = NULL;
...
dsa_sigver_test (int id)
{
  ...
  if (!TEST_ptr (bn_ctx = BN_CTX_new ())
      || !TEST_true (dsa_create_pkey (
          &pkey, tst->p, tst->p_len, tst->q, tst->q_len, tst->g, tst->g_len,
          NULL, 0, 0, 0, 0, tst->pub, tst->pub_len, bn_ctx)))
    goto err;
...
static int
cipher_enc (const char *alg, const unsigned char *pt, size_t pt_len,
            const unsigned char *key, size_t key_len, const unsigned char *iv,
            size_t iv_len, const unsigned char *ct, size_t ct_len, int enc)
{
...
static int
aes_ccm_enc_dec (const char *alg, const unsigned char *pt, size_t pt_len,
                 const unsigned char *key, size_t key_len,
                 const unsigned char *iv, size_t iv_len,
                 const unsigned char *aad, size_t aad_len,
                 const unsigned char *ct, size_t ct_len,
                 const unsigned char *tag, size_t tag_len, int enc, int pass)
{
  int ret = 0;
  EVP_CIPHER_CTX *ctx;
  EVP_CIPHER *cipher = NULL;
  ...
  if (!TEST_ptr (ctx = EVP_CIPHER_CTX_new ())
      ...
      || !TEST_int_eq (EVP_CipherUpdate (ctx, out, &len, pt, (int)pt_len),
                       pass))
    goto err;
...
aes_ccm_enc_dec_test (int id)
{
  ...
  return aes_ccm_enc_dec (tst->alg, tst->pt, tst->pt_len, tst->key,
         ...
         && aes_ccm_enc_dec (tst->alg, tst->ct, ct_len, tst->key, tst->key_len,
                             tst->iv, tst->iv_len, tst->aad, tst->aad_len,
                             tst->pt, tst->pt_len, tag - 1, tag_len, !enc,
                             !pass);
}

static int
aes_gcm_enc_dec (const char *alg, const unsigned char *pt, size_t pt_len,
                 const unsigned char *key, size_t key_len,
                 const unsigned char *iv, size_t iv_len,
                 const unsigned char *aad, size_t aad_len,
                 const unsigned char *ct, size_t ct_len,
                 const unsigned char *tag, size_t tag_len, int enc, int pass,
                 unsigned char *out, int *out_len, unsigned char *outiv)
{
  int ret = 0;
  EVP_CIPHER_CTX *ctx;
  EVP_CIPHER *cipher = NULL;
...
static int
aes_gcm_gen_iv_internal_test (void)
{
  ...
  return aes_gcm_enc_dec (tst->alg, tst->pt, tst->pt_len, tst->key,
         ...
         && aes_gcm_enc_dec (tst->alg, out, out_len, tst->key, tst->key_len,
                             iv, tst->iv_len, tst->aad, tst->aad_len, tst->pt,
                             tst->pt_len, out + out_len, tst->tag_len, !enc,
                             pass, out, NULL, NULL);
}

#ifndef OPENSSL_NO_DH
static int
dh_create_pkey (EVP_PKEY **pkey, const char *group_name,
                const unsigned char *pub, size_t pub_len,
                const unsigned char *priv, size_t priv_len, BN_CTX *bn_ctx,
                int pass)
{
  int ret = 0;
  EVP_PKEY_CTX *ctx = NULL;
...
static int
dh_safe_prime_keygen_test (int id)
{
  int ret = 0;
  EVP_PKEY_CTX *ctx = NULL;
  EVP_PKEY *pkey = NULL;
...
static int
dh_safe_prime_keyver_test (int id)
{
  int ret = 0;
  BN_CTX *bn_ctx = NULL;
  EVP_PKEY_CTX *key_ctx = NULL;
...
static int
rsa_create_pkey (EVP_PKEY **pkey, const unsigned char *n, size_t n_len,
                 const unsigned char *e, size_t e_len, const unsigned char *d,
                 size_t d_len, BN_CTX *bn_ctx)
{
  int ret = 0;
  EVP_PKEY_CTX *ctx = NULL;
  OSSL_PARAM_BLD *bld = NULL;
...
rsa_siggen_test (int id)
{
  ...
  *p++ = OSSL_PARAM_construct_end ();
  ...
  if (!TEST_ptr (pkey
                 = EVP_PKEY_Q_keygen (libctx, NULL, "RSA", (size_t)tst->mod))
      || !TEST_true (
          pkey_get_bn_bytes (pkey, OSSL_PKEY_PARAM_RSA_N, &n, &n_len))
      || !TEST_true (
...
static int
rsa_decryption_primitive_test (int id)
{
  int ret = 0;
  EVP_PKEY_CTX *ctx = NULL;
  EVP_PKEY *pkey = NULL;
...
static int
self_test_events (const OSSL_PARAM params[], void *varg)
{
