...
static int
pkey_get_bn_bytes(EVP_PKEY* pkey,
                  const char* name,
                  unsigned char** out,
                  size_t* out_len)
{
...
static int
ecdsa_create_pkey(EVP_PKEY** pkey,
...
ecdsa_pub_verify_test(int id)
{
  ...
  if (!TEST_true(ecdsa_create_pkey(
        &pkey, tst->curve_name, tst->pub, tst->pub_len, tst->pass)))
    goto err;
...
ecdsa_siggen_test(int id)
{
  ...
  if (!TEST_true(sig_gen(
        pkey, NULL, tst->digest_alg, tst->msg, tst->msg_len, &sig, &sig_len)) ||
...
ecdsa_sigver_test(int id)
{
  ...
  if (!TEST_int_gt((sig_len = i2d_ECDSA_SIG(sign, &sig)), 0) ||
      ...
      !TEST_true(EVP_DigestVerifyInit_ex(
        md_ctx, NULL, tst->digest_alg, libctx, NULL, pkey, NULL)) ||
...
static int
pkey_get_octet_bytes(EVP_PKEY* pkey,
                     const char* name,
                     unsigned char** out,
                     size_t* out_len)
{
...
static int
eddsa_create_pkey(EVP_PKEY** pkey,
                  const char* algname,
                  const unsigned char* pub,
                  size_t pub_len,
                  int expected)
...
eddsa_keygen_test(int id)
{
  ...
  if (!TEST_ptr(pkey = EVP_PKEY_Q_keygen(libctx, NULL, tst->curve_name)) ||
      ...
      !TEST_true(pkey_get_octet_bytes(
        pkey, OSSL_PKEY_PARAM_PRIV_KEY, &priv, &priv_len)) ||
      !TEST_true(
...
dsa_pqver_test(int id)
{
  ...
  if (!TEST_ptr(bn_ctx = BN_CTX_new_ex(libctx)) ||
      !TEST_true(dsa_create_pkey(&param_key,
                                 tst->p,
                                 tst->p_len,
                                 tst->q,
                                 tst->q_len,
...
static int
get_dsa_sig_rs_bytes(const unsigned char* sig,
                     size_t sig_len,
                     unsigned char** r,
                     unsigned char** s,
                     size_t* r_len,
...
dsa_sigver_test(int id)
{
  ...
  if (!TEST_ptr(bn_ctx = BN_CTX_new()) ||
      !TEST_true(dsa_create_pkey(&pkey,
                                 tst->p,
                                 tst->p_len,
                                 tst->q,
                                 tst->q_len,
...
cipher_enc_dec_test(int id)
{
  ...
  return TEST_true(cipher_enc(tst->alg,
                              tst->pt,
                              tst->pt_len,
                              tst->key,
                              tst->key_len,
...
static int
aes_ccm_enc_dec(const char* alg,
                const unsigned char* pt,
                size_t pt_len,
                const unsigned char* key,
                size_t key_len,
                ...
                int pass)
{
  ...
  if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()) ||
      ...
      !TEST_int_gt(
        EVP_CIPHER_CTX_ctrl(
          ctx, EVP_CTRL_AEAD_SET_TAG, (int)tag_len, enc ? NULL : (void*)tag),
        0) ||
  ...
  if (enc) {
    ...
    if (!TEST_int_gt(EVP_CIPHER_CTX_ctrl(
                       ctx, EVP_CTRL_AEAD_GET_TAG, (int)tag_len, out + out_len),
                     0) ||
...
aes_ccm_enc_dec_test(int id)
{
  ...
  return aes_ccm_enc_dec(tst->alg,
                         tst->pt,
                         tst->pt_len,
                         tst->key,
                         tst->key_len,
...
static int
aes_gcm_enc_dec(const char* alg,
                const unsigned char* pt,
                size_t pt_len,
                const unsigned char* key,
                size_t key_len,
  ...
  if (enc) {
    if ((ct != NULL && !TEST_mem_eq(out, olen, ct, ct_len)) ||
        !TEST_int_gt(EVP_CIPHER_CTX_ctrl(
                       ctx, EVP_CTRL_AEAD_GET_TAG, (int)tag_len, out + olen),
                     0) ||
...
aes_gcm_enc_dec_test(int id)
{
  ...
  return aes_gcm_enc_dec(tst->alg,
                         tst->pt,
                         tst->pt_len,
                         tst->key,
                         tst->key_len,
...
dh_safe_prime_keygen_test(int id)
{
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
    OSSL_PKEY_PARAM_GROUP_NAME, (char*)tst->group_name, 0);
  params[1] = OSSL_PARAM_construct_end();
...
rsa_siggen_test(int id)
{
  ...
  *p++ = OSSL_PARAM_construct_utf8_string(
    OSSL_SIGNATURE_PARAM_PAD_MODE, (char*)tst->sig_pad_mode, 0);
  *p++ = OSSL_PARAM_construct_utf8_string(
    OSSL_SIGNATURE_PARAM_DIGEST, (char*)tst->digest_alg, 0);
  if (salt_len >= 0)
    *p++ =
  ...
  *p++ = OSSL_PARAM_construct_end();
  ...
  if (!TEST_ptr(pkey =
      ...
      !TEST_true(sig_gen(
        pkey, params, tst->digest_alg, tst->msg, tst->msg_len, &sig, &sig_len)))
    goto err;
  test_output_memory("n", n, n_len);
...
rsa_sigver_test(int id)
{
  ...
  TEST_note("RSA %s Signature Verify : expected to %s ",
            tst->sig_pad_mode,
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!TEST_ptr(bn_ctx = BN_CTX_new()) ||
      !TEST_true(rsa_create_pkey(
        &pkey, tst->n, tst->n_len, tst->e, tst->e_len, NULL, 0, bn_ctx)) ||
      !TEST_ptr(md_ctx = EVP_MD_CTX_new()) ||
      !TEST_true(EVP_DigestVerifyInit_ex(
        md_ctx, &pkey_ctx, tst->digest_alg, libctx, NULL, pkey, NULL)) ||
      !check_verify_message(pkey_ctx, 1) ||
      !TEST_true(EVP_PKEY_CTX_set_params(pkey_ctx, params)) ||
      !TEST_int_eq(EVP_DigestVerify(
                     md_ctx, tst->sig, tst->sig_len, tst->msg, tst->msg_len),
                   tst->pass) ||
      !check_verify_message(pkey_ctx, 1) ||
      !TEST_true(EVP_PKEY_verify_init(pkey_ctx)) ||
      !check_verify_message(pkey_ctx, 0))
...
drbg_test(int id)
{
  ...
  params[1] = OSSL_PARAM_construct_utf8_string(
    OSSL_DRBG_PARAM_CIPHER, (char*)tst->cipher, 0);
  ...
  if (!TEST_true(EVP_RAND_instantiate(ctx, 0, 0, (void*)"", 0, NULL)) ||
      !TEST_true(EVP_RAND_generate(
        ctx, returned_bits, returned_bits_len, 0, 0, NULL, 0)) ||
      !TEST_true(EVP_RAND_generate(
        ctx, returned_bits, returned_bits_len, 0, 0, NULL, 0)))
    goto err;
