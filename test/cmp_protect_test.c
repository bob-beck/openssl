...
static void
tear_down(CMP_PROTECT_TEST_FIXTURE* fixture)
{
  if (fixture != NULL) {
...
static CMP_PROTECT_TEST_FIXTURE*
 ...
 */
static int
verify_signature(OSSL_CMP_MSG* msg,
                 ASN1_BIT_STRING* protection,
                 EVP_PKEY* pkey,
                 EVP_MD* digest)
{
  ...
  int ret =
    (TEST_ptr(protection) &&
     TEST_true(verify_signature(
       fixture->msg, protection, fixture->pubkey, fixture->cmp_ctx->digest)));
...
static int
test_cmp_calc_protection_no_key_no_secret(void)
{
...
static int
test_cmp_calc_protection_pkey(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
...
#ifndef OPENSSL_NO_ECX
static int
test_cmp_calc_protection_pkey_Ed(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
...
static int
test_cmp_calc_protection_pbmac(void)
{
...
static int
execute_MSG_protect_test(CMP_PROTECT_TEST_FIXTURE* fixture)
{
...
static int
test_MSG_protect_unprotected_request(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
...
static int
test_MSG_protect_with_msg_sig_alg_protection_plus_rsa_key(void)
{
  ...
  if (!TEST_ptr(fixture->msg = OSSL_CMP_MSG_dup(ir_unprotected)) ||
      ...
      || !TEST_true(OSSL_CMP_CTX_set1_referenceValue(
           fixture->cmp_ctx, rand_data, size)) ||
      !TEST_true(OSSL_CMP_CTX_set1_secretValue(
        fixture->cmp_ctx, rand_data + size, size))) {
    tear_down(fixture);
    fixture = NULL;
  }
  EXECUTE_TEST(execute_MSG_protect_test, tear_down);
...
static int
test_MSG_protect_with_certificate_and_key(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
  fixture->expected = 1;
...
static int
test_MSG_protect_certificate_based_without_cert(void)
{
  OSSL_CMP_CTX* ctx;
...
static int
test_MSG_protect_no_key_no_secret(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
  fixture->expected = 0;
  if (!TEST_ptr(fixture->msg = OSSL_CMP_MSG_dup(ir_unprotected)) ||
...
static int
test_MSG_protect_pbmac_no_sender(int with_ref)
{
  ...
  if (!TEST_ptr(fixture->msg = OSSL_CMP_MSG_dup(ir_unprotected)) ||
      ...
      !OSSL_CMP_CTX_set1_secretValue(
        fixture->cmp_ctx, secret, sizeof(secret)) ||
...
static int
test_MSG_protect_pbmac_no_sender_with_ref(void)
{
  return test_MSG_protect_pbmac_no_sender(1);
}

static int
test_MSG_protect_pbmac_no_sender_no_ref(void)
{
  return test_MSG_protect_pbmac_no_sender(0);
}

static int
execute_MSG_add_extraCerts_test(CMP_PROTECT_TEST_FIXTURE* fixture)
{
  return TEST_true(ossl_cmp_msg_add_extraCerts(fixture->cmp_ctx, fixture->msg));
}

static int
test_MSG_add_extraCerts(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
...
execute_cmp_build_cert_chain_test(CMP_PROTECT_TEST_FIXTURE* fixture)
{
  ...
  X509_STORE* store;
  STACK_OF(X509)* chain = X509_build_chain(fixture->cert,
                                           fixture->certs,
                                           NULL,
                                           fixture->with_ss,
                                           ctx->libctx,
  ...
  if (TEST_ptr(store = X509_STORE_new()) &&
    ...
    chain = X509_build_chain(fixture->cert,
                             fixture->certs,
                             store,
                             fixture->with_ss,
                             ctx->libctx,
...
static int
test_cmp_build_cert_chain(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
  fixture->expected = 1;
  fixture->with_ss = 0;
...
static int
test_cmp_build_cert_chain_missing_intermediate(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
  fixture->expected = 0;
  fixture->with_ss = 0;
...
static int
test_cmp_build_cert_chain_no_root(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
  fixture->expected = 1;
  fixture->with_ss = 0;
...
static int
test_cmp_build_cert_chain_only_root(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
  fixture->expected = 1;
...
static int
test_cmp_build_cert_chain_no_certs(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
  fixture->expected = 0;
  fixture->with_ss = 0;
...
static int
execute_X509_STORE_test(CMP_PROTECT_TEST_FIXTURE* fixture)
{
  X509_STORE* store = X509_STORE_new();
  STACK_OF(X509)* sk = NULL;
  int res = 0;

  if (!TEST_true(ossl_cmp_X509_STORE_add1_certs(
...
static int
test_X509_STORE_only_self_issued(void)
{
  SETUP_TEST_FIXTURE(CMP_PROTECT_TEST_FIXTURE, set_up);
