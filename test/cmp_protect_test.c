...
static int execute_calc_protection_fails_test(
    CMP_PROTECT_TEST_FIXTURE* fixture) {
...
static int execute_calc_protection_signature_test(
    CMP_PROTECT_TEST_FIXTURE* fixture) {
  ASN1_BIT_STRING* protection =
      ossl_cmp_calc_protection(fixture->cmp_ctx, fixture->msg);
  int ret =
...
static int execute_cmp_build_cert_chain_test(
    CMP_PROTECT_TEST_FIXTURE* fixture) {
  int ret = 0;
...
static int execute_X509_STORE_test(CMP_PROTECT_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_int_eq(0, STACK_OF_X509_cmp(sk, fixture->chain))) goto err;
  res = 1;
err:
  X509_STORE_free(store);
...
#ifndef OPENSSL_NO_ECX
  if (!TEST_ptr(prot_Ed_key = load_pkey_pem(prot_Ed_f, libctx))) return 0;
#endif
