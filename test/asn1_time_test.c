...
static ASN1_TIME gtime
    = { 15, V_ASN1_GENERALIZEDTIME, (unsigned char *)"19991231000000Z", 0 };
static time_t gtime_t = 946598400;

static int
test_table (struct testdata *tbl, int idx)
{
  int error = 0;
  ASN1_TIME atime;
  ASN1_TIME *ptime;
  ...
  if (!TEST_ptr (ptime))
  ...
  else
    {
      ...
      if (!TEST_int_eq (ASN1_TIME_cmp_time_t (ptime, gtime_t), td->cmp_result))
        {
          TEST_info ("ASN1_TIME_cmp_time_t(%s) after "
                     "ASN1_TIME_set_string_gnt() to baseline bad comparison",
                     td->data);
          local_error = error = 1;
        }
      if (local_error)
  ...
  if (!TEST_ptr (ptime))
  ...
  else
    {
      ...
      if (!TEST_int_eq (ASN1_TIME_cmp_time_t (ptime, gtime_t), td->cmp_result))
        {
          TEST_info ("ASN1_TIME_cmp_time_t(%s) after ASN1_TIME_set_string() "
                     "to baseline bad comparison",
                     td->data);
          local_error = error = 1;
        }
      if (local_error)
...
#define TOMORROW_GEN                                                          \
  { sizeof (TOMORROW_GEN_STR) - 1, V_ASN1_GENERALIZEDTIME, TOMORROW_GEN_STR,  \
    0 }
#define TODAY_UTC                                                             \
...
static int
convert_asn1_to_time_t (int idx)
{
  time_t testdateutc;
