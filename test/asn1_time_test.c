...
static int test_table(struct testdata* tbl, int idx) {
  ...
  if (!TEST_ptr(ptime)) {
    ...
    if (!TEST_true(ASN1_TIME_diff(&day, &sec, ptime, &atime)) ||
      ...
      TEST_info(
          "ASN1_TIME_diff(day=%d, sec=%d, %s) after ASN1_TIME_set_string_gmt() "
          "failed",
          day, sec, td->data);
      local_error = error = 1;
    }
  ...
  if (!TEST_ptr(ptime)) {
    ...
    if (!TEST_true(ASN1_TIME_diff(&day, &sec, ptime, &atime)) ||
        !TEST_int_eq(day, 0) || !TEST_int_eq(sec, 0)) {
      TEST_info(
          "ASN1_TIME_diff(day=%d, sec=%d, %s) after ASN1_TIME_set_string() "
          "failed",
          day, sec, td->data);
      local_error = error = 1;
    }
    if (!TEST_int_eq(ASN1_TIME_cmp_time_t(ptime, gtime_t), td->cmp_result)) {
      TEST_info(
          "ASN1_TIME_cmp_time_t(%s) after ASN1_TIME_set_string() to baseline "
          "bad comparison",
          td->data);
      local_error = error = 1;
    }
  ...
  if (error) TEST_error("atime=%s", atime.data);
