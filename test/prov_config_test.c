...
static int test_double_config(void) {
  ...
  if (!TEST_true(OSSL_LIB_CTX_load_config(ctx, configfile))) goto err;
  if (!TEST_true(OSSL_LIB_CTX_load_config(ctx, configfile))) goto err;
...
static int test_recursive_config(void) {
  ...
  if (!TEST_false(OSSL_LIB_CTX_load_config(ctx, recurseconfigfile))) goto err;

  err = ERR_peek_error();
  /* We expect to get a recursion error here */
  if (ERR_GET_REASON(err) == CONF_R_RECURSIVE_SECTION_REFERENCE) testresult = 1;
err:
  OSSL_LIB_CTX_free(ctx);
  return testresult;
}
...
static int test_path_config(void) {
  ...
  if (!TEST_ptr(pathedconfig)) return 0;

  ctx = OSSL_LIB_CTX_new();
  if (!TEST_ptr(ctx)) return 0;

  if (!TEST_true(OSSL_LIB_CTX_load_config(ctx, pathedconfig))) goto err;

  /* attempt to manually load the test provider */
  if (!TEST_ptr(prov = OSSL_PROVIDER_load(ctx, "test"))) goto err;
...
int setup_tests(void) {
  ...
  if (!TEST_ptr(configfile = test_get_argument(0))) return 0;

  if (!TEST_ptr(recurseconfigfile = test_get_argument(1))) return 0;
