...
static int verify_chain(SSL* ssl, STACK_OF(X509)* chain) {
  ...
  if (!TEST_int_ge(ret = X509_STORE_CTX_verify(store_ctx), 0)) ret = 0;
...
static STACK_OF(X509)* load_chain(BIO* fp, int nelem) {
  ...
  if (!TEST_ptr(chain = sk_X509_new_null())) goto err;
  ...
  for (count = 0; count < nelem && errtype == 0 &&
    ...
    if (strcmp(name, PEM_STRING_X509) == 0 ||
      ...
      if (!TEST_true(sk_X509_push(chain, cert))) goto err;
    } else {
...
static char* read_to_eol(BIO* f) {
  ...
  if (BIO_gets(f, buf, sizeof(buf)) <= 0) return NULL;
...
static ossl_ssize_t hexdecode(const char* in, void* result) {
  ...
  if (!TEST_ptr(ret = OPENSSL_malloc(strlen(in) / 2))) return -1;
  cp = ret;
...
static int test_tlsafile(SSL_CTX* ctx, const char* base_name, BIO* f,
  ...
  while (ret > 0 && (line = read_to_eol(f)) != NULL) {
    ...
    if (noncheck) SSL_dane_set_flags(ssl, DANE_FLAG_NO_DANE_EE_NAMECHECKS);
