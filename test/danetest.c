 ...
 */
static void
store_ctx_dane_init(X509_STORE_CTX*, SSL*);
...
static int
verify_chain(SSL* ssl, STACK_OF(X509)* chain)
{
 ...
 */
static ossl_ssize_t
hexdecode(const char* in, void* result)
{
...
static ossl_ssize_t
checked_uint8(const char* in, void* out)
{
...
static int
tlsa_import_rr(SSL* ssl, const char* rrdata)
{
...
static int
test_tlsafile(SSL_CTX* ctx, const char* base_name, BIO* f, const char* path)
{
  char* line;
  int testno = 0;
  int ret = 1;
...
static void
store_ctx_dane_init(X509_STORE_CTX* store_ctx, SSL* ssl)
{
