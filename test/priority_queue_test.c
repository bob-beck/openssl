...
static int test_size_t_priority_queue_int(int reserve, int order, int count,
  ...
  if (!TEST_size_t_le(count, MAX_SAMPLES)) return 0;
  ...
  if (reserve && !TEST_true(ossl_pqueue_size_t_reserve(pq, count))) goto err;

  for (i = 0; i < count; i++)
    if (!TEST_true(ossl_pqueue_size_t_push(pq, values + i, ref + i))) goto err;
  ...
  if (popfree) {
    ...
    pq = NULL;
    if (!TEST_size_t_eq(num_rec_freed, n)) goto err;
  }
  res = 1;
err:
...
static int cmp(const INFO* a, const INFO* b) {
  ...
  if (a->seq_num > b->seq_num) return 1;
  if (a->sub_seq < b->sub_seq) return -1;
  if (a->sub_seq > b->sub_seq) return 1;
  return 0;
}
