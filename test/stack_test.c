...
static int test_int_stack(int reserve) {
  ...
  if (!TEST_int_eq(sk_sint_num(s), n)) goto end;
  ...
  /* sorting */
  if (!TEST_false(sk_sint_is_sorted(s))) goto end;
  (void)sk_sint_set_cmp_func(s, &int_compare);
  sk_sint_sort(s);
  if (!TEST_true(sk_sint_is_sorted(s))) goto end;
  ...
  /* shift */
  if (!TEST_ptr_eq(sk_sint_shift(s), v + 6)) goto end;
...
static int test_uchar_stack(int reserve) {
  ...
  if (!TEST_int_eq(sk_uchar_num(s), n)) goto end;

  /* dup */
  r = sk_uchar_dup(NULL);
  if (sk_uchar_num(r) != 0) goto end;
  sk_uchar_free(r);
  r = sk_uchar_dup(s);
  if (!TEST_int_eq(sk_uchar_num(r), n)) goto end;
  sk_uchar_sort(r);
  ...
  /* zero */
  sk_uchar_zero(r);
  if (!TEST_int_eq(sk_uchar_num(r), 0)) goto end;
  ...
  /* delete */
  if (!TEST_ptr_null(sk_uchar_delete(r, 12))) goto end;
  if (!TEST_ptr_eq(sk_uchar_delete(r, 1), v + 1)) goto end;
...
static int test_SS_stack(void) {
  ...
  if (!TEST_int_eq(sk_SS_num(s), n)) goto end;

  /* deepcopy */
  r = sk_SS_deep_copy(NULL, &SS_copy, &SS_free);
  if (sk_SS_num(r) != 0) goto end;
  sk_SS_free(r);
  r = sk_SS_deep_copy(s, &SS_copy, &SS_free);
  if (!TEST_ptr(r)) goto end;
  for (i = 0; i < n; i++) {
    p = sk_SS_value(r, i);
  ...
  SS_free(p);
  if (!TEST_int_eq(sk_SS_num(s), n - 1)) goto end;
  for (i = 0; i < n - 1; i++)
...
static int test_SU_stack(void) {
  ...
  if (!TEST_int_eq(sk_SU_num(s), n)) goto end;
