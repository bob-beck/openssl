...
static int test_qlog(void) {
  ...
  if (!TEST_ptr(qlog = ossl_qlog_new(&qti))) goto err;
  ...
  if (!TEST_true(ossl_qlog_set_sink_bio(qlog, bio))) goto err;
  ...
  if (!TEST_true(ossl_qlog_flush(qlog))) goto err;

  buf_len = BIO_get_mem_data(bio, &buf);
  if (!TEST_size_t_gt(buf_len, 0)) goto err;

  if (!TEST_mem_eq(buf, buf_len, expected, sizeof(expected))) goto err;
