...
static int set_fromdata_string(EVP_PKEY_CTX* ctx, const char* name,
  ...
  if (EVP_PKEY_fromdata_init(ctx) != 1) return -1;
...
static int test_new_keytype(void) {
  ...
  if (TEST_ptr(key = EVP_PKEY_Q_keygen(mainctx, NULL, "XOR"))) goto err;
...
static int test_d2i_AutoPrivateKey_ex(int i) {
  ...
  p = input;
  if (!TEST_ptr(pkey = d2i_AutoPrivateKey_ex(NULL, &p, (long)input_len, mainctx,
                                             NULL)) ||
...
static int test_alternative_default(void) {
   ...
   */
  if (!TEST_ptr_null(sha256 = EVP_MD_fetch(NULL, "SHA2-256", NULL))) goto err;
...
static int test_d2i_PrivateKey_ex(int testid) {
  ...
  if (!TEST_ptr(provider = OSSL_PROVIDER_load(NULL, "default"))) goto err;
  if (!TEST_ptr(key_bio =
  ...
  ERR_clear_error();
  if (!TEST_int_ge(BIO_seek(key_bio, 0), 0)) goto err;
...
static int test_PEM_read_bio_negative(int testid) {
  ...
  if (!TEST_ptr_null(pkey = PEM_read_bio_PrivateKey(key_bio, NULL, NULL, NULL)))
    goto err;
  if (!TEST_int_ne(ERR_peek_error(), 0)) goto err;
  if (!TEST_ptr(provider = OSSL_PROVIDER_load(NULL, "default"))) goto err;
  if (!TEST_int_ge(BIO_seek(key_bio, 0), 0)) goto err;
  ERR_clear_error();
  if (!TEST_ptr_null(pkey = PEM_read_bio_PrivateKey(key_bio, NULL, NULL, NULL)))
...
static int test_PEM_read_bio_negative_wrong_password(int testid) {
  ...
  if (testid > 0) ui_method = UI_null();

  if (!TEST_ptr(provider)) goto err;
  if (!TEST_ptr(key_bio)) goto err;
  if (!TEST_ptr(write_pkey)) goto err;
  ...
  if (!TEST_ptr_null(read_pkey)) goto err;
...
static int test_dsa_todata(void) {
  ...
  if (!do_fromdata_key_is_equal(to_params, pkey, "DSA")) goto err;
   ...
   */
  dsa_gindex++;
  if (!do_check_params(all_params, 0)) goto err;
  dsa_gindex--;
  dsa_pcounter++;
  if (!do_check_params(all_params, 0)) goto err;
  dsa_pcounter--;
...
static int test_pkey_export_null(void) {
  ...
  ret = TEST_ptr(pkey = d2i_AutoPrivateKey_ex(
                     NULL, &pdata, (long)keydata[0].size, mainctx, NULL)) &&
        TEST_int_eq(
            EVP_PKEY_export(NULL, EVP_PKEY_KEYPAIR, test_pkey_export_cb, NULL),
            0) &&
...
static int test_rsa_pss_sign(void) {
  ...
  ret =
      TEST_ptr(pkey = d2i_AutoPrivateKey_ex(NULL, &pdata, (long)keydata[0].size,
                                            mainctx, NULL)) &&
      TEST_ptr(pctx = EVP_PKEY_CTX_new_from_pkey(mainctx, pkey, NULL)) &&
      TEST_int_gt(EVP_PKEY_sign_init_ex(pctx, sig_params), 0) &&
      TEST_int_gt(EVP_PKEY_sign(pctx, NULL, &sig_len, mdbuf, sizeof(mdbuf)),
                  0) &&
...
static int test_evp_md_ctx_copy2(void) {
  ...
  if (!TEST_ptr(inctx) || !TEST_ptr(outctx)) goto end;

  /* init inctx and outctx, now the contexts are from same providers */
  if (!TEST_true(EVP_DigestInit_ex2(inctx, md, NULL))) goto end;
  if (!TEST_true(EVP_DigestInit_ex2(outctx, md, NULL))) goto end;
...
static int test_evp_pbe_alg_add(void) {
  ...
  if (!TEST_true(keygen != NULL)) goto err;
  if (!TEST_true(keygen_ex == NULL)) goto err;
