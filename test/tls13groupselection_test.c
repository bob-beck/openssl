...
static void
server_response_check_cb(int write_p,
                         int version,
                         int content_type,
                         const void* buf,
                         size_t len,
  ...
  int ssl_or_ctx)
{
  ...
  TEST_true_or_end(create_ssl_objects(
    server_ctx, client_ctx, &serverssl, &clientssl, NULL, NULL));
...
static int
tls13groupselection_test(int i)
{
  ...
  if (test_type == TEST_SYNTAX_FAILURE)
    ...
    res = test_groupnegotiation(
      &tls13groupselection_tests[i], WORK_ON_SSL_OBJECT, test_type);
  ...
  if (test_type == TEST_SYNTAX_FAILURE)
    ...
    res = test_groupnegotiation(
      &tls13groupselection_tests[i], WORK_ON_CONTEXT, test_type);
