...
static void MYOBJ_sethello(MYOBJ* obj, char* cp) {
  ...
  if (!TEST_int_eq(obj->st, 1)) gbl_result = 0;
}
...
static char* MYOBJ_gethello2(MYOBJ* obj) {
  ...
  if (TEST_ptr(ex_data)) return ex_data->hello;
...
static int test_exdata(void) {
  ...
  if (!TEST_ptr(p = OPENSSL_strdup("hello world"))) return 0;
  saved_argl = 21;
  if (!TEST_ptr(saved_argp = OPENSSL_malloc(1))) goto err;
  ...
  t1 = MYOBJ_new();
  t2 = MYOBJ_new();
  if (!TEST_int_eq(t1->st, 1) || !TEST_int_eq(t2->st, 1)) goto err;
  if (!TEST_ptr(CRYPTO_get_ex_data(&t1->ex_data, saved_idx2))) goto err;
  ...
  saved_idx3 = CRYPTO_get_ex_new_index(CRYPTO_EX_INDEX_APP, saved_argl,
                                       saved_argp, exnew2, exdup2, exfree2);
  if (!TEST_ptr_null(CRYPTO_get_ex_data(&t1->ex_data, saved_idx3))) goto err;

  MYOBJ_sethello(t1, p);
  cp = MYOBJ_gethello(t1);
  if (!TEST_ptr_eq(cp, p)) goto err;
  ...
  if (!TEST_ptr_null(cp)) goto err;

  cp = MYOBJ_gethello2(t2);
  if (!TEST_ptr_null(cp)) goto err;

  t3 = MYOBJ_dup(t1);
  if (!TEST_int_eq(t3->st, 1)) goto err;

  ex_data = CRYPTO_get_ex_data(&t3->ex_data, saved_idx2);
  if (!TEST_ptr(ex_data)) goto err;
  if (!TEST_int_eq(ex_data->dup, 1)) goto err;
