...
static int ec2m_field_sanity(void) {
  ...
  if (!TEST_true(BN_set_word(p, 0xf2))) goto out;
  if (!TEST_ptr_null(group1 = EC_GROUP_new_curve_GF2m(p, a, b, ctx)))
    TEST_error("Zero constant term accepted in GF2m polynomial");

  /* Odd hexanomial should also be rejected */
  if (!TEST_true(BN_set_word(p, 0xf3))) goto out;
...
static int underflow_test(void) {
  ...
  y2 = BN_CTX_get(ctx);
  k = BN_CTX_get(ctx);
  if (!TEST_ptr(k)) goto err;

  grp = EC_GROUP_new_by_curve_name(NID_secp521r1);
  P = EC_POINT_new(grp);
  Q = EC_POINT_new(grp);
  R = EC_POINT_new(grp);
  if (!TEST_ptr(grp) || !TEST_ptr(P) || !TEST_ptr(Q) || !TEST_ptr(R)) goto err;
...
static int set_private_key(void) {
  ...
  if (!TEST_int_eq(EC_KEY_set_private_key(key, aux_key->priv_key), 1)) goto err;
...
static int ecpkparams_i2d2i_test(int n) {
  ...
  /* create group */
  if (!TEST_ptr(g1 = EC_GROUP_new_by_curve_name(nid))) goto end;
...
static int check_bn_mont_ctx(BN_MONT_CTX* mont, BIGNUM* mod, BN_CTX* ctx) {
  ...
  if (!TEST_ptr(regenerated)) return ret;
  if (!TEST_ptr(mont)) goto err;

  if (!TEST_true(BN_MONT_CTX_set(regenerated, mod, ctx))) goto err;

  if (!TEST_true(ossl_bn_mont_ctx_eq(regenerated, mont))) goto err;
