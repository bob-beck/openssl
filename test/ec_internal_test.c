...
static int
group_field_tests(const EC_GROUP* group, BN_CTX* ctx)
{
...
static int
field_tests(const EC_METHOD* meth, const unsigned char* params, int len)
{
  BN_CTX* ctx = NULL;
...
static int
field_tests_ecp_simple(void)
{
  TEST_info("Testing EC_GFp_simple_method()\n");
  return field_tests(
    EC_GFp_simple_method(), params_p256, sizeof(params_p256) / 3);
}

/* test EC_GFp_mont_method directly */
static int
field_tests_ecp_mont(void)
{
  TEST_info("Testing EC_GFp_mont_method()\n");
  return field_tests(
    EC_GFp_mont_method(), params_p256, sizeof(params_p256) / 3);
}

#ifndef OPENSSL_NO_EC2M
/* Test that decoding of invalid GF2m field parameters fails. */
static int
ec2m_field_sanity(void)
{
  int ret = 0;
...
static int
field_tests_ec2_simple(void)
{
  TEST_info("Testing EC_GF2m_simple_method()\n");
  return field_tests(
    EC_GF2m_simple_method(), params_b283, sizeof(params_b283) / 3);
}
#endif

/* test default method for a named curve */
static int
field_tests_default(int n)
{
  BN_CTX* ctx = NULL;
  EC_GROUP* group = NULL;
...
static int
check_bn_mont_ctx(BN_MONT_CTX* mont, BIGNUM* mod, BN_CTX* ctx)
{
  int ret = 0;
...
static int
montgomery_correctness_test(EC_GROUP* group)
{
  int ret = 0;
  BN_CTX* ctx = NULL;
...
static int
named_group_creation_test(void)
{
  int ret = 0;
  EC_GROUP* group = NULL;
