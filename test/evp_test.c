                           ...
                           const OSSL_PARAM initparams[])
{
    ...
    if (expected->mac_key != NULL &&
        EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_MAC_KEY, (int)expected->mac_key_len, (void *)expected->mac_key) <= 0)
    {
    ...
    {
        if (expected->tls_version >= TLS1_1_VERSION && (EVP_CIPHER_is_a(expected->cipher, "AES-128-CBC-HMAC-SHA1") ||
...
retry:
    ...
    if (xof)
    {
        ...
        if (!EVP_MAC_finalXOF(ctx, got, expected->output_len) ||
            !memory_err_compare(t, "TEST_MAC_ERR", expected->output, expected->output_len, got, expected->output_len))
        {
...
static int pbe_test_run(EVP_TEST *t)
{
    ...
    {
        if (PKCS5_PBKDF2_HMAC((char *)expected->pass, (int)expected->pass_len, expected->salt, (int)expected->salt_len,
                              expected->iter, expected->md, (int)expected->key_len, key) == 0)
        {
            t->err = "PBKDF2_ERROR";
            goto err;
        }
    ...
    else if (expected->pbe_type == PBE_TYPE_SCRYPT)
    {
        if (EVP_PBE_scrypt((const char *)expected->pass, expected->pass_len, expected->salt, expected->salt_len,
                           expected->N, expected->r, expected->p, expected->maxmem, key, expected->key_len) == 0)
        {
            t->err = "SCRYPT_ERROR";
            goto err;
        }
    ...
    else if (expected->pbe_type == PBE_TYPE_PKCS12)
    {
        ...
        if (PKCS12_key_gen_uni(expected->pass, (int)expected->pass_len, expected->salt, (int)expected->salt_len,
                               expected->id, expected->iter, (int)expected->key_len, key, fetched_digest) == 0)
        {
            t->err = "PKCS12_ERROR";
            goto err;
        }
...
static int encode_test_run(EVP_TEST *t)
{
    ...
    do
    {
        ...
        if (data_chunk_size == 0 || (size_t)data_chunk_size > input_len)
            current_len = input_len;
        if (EVP_DecodeUpdate(decode_ctx, decode_out + output_len, &chunk_len, expected->output + donelen,
                             (int)current_len) < 0)
        {
            t->err = "DECODE_ERROR";
            goto err;
