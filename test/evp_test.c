...
static int get_faked_bytes(unsigned char* buf, size_t num,
  ...
  while (num-- > 0) {
    if (fake_rand_bytes_offset >= fake_rand_size) fake_rand_bytes_offset = 0;
...
static int mac_check_fips_approved(EVP_MAC_CTX* ctx, EVP_TEST* t) {
  ...
  if (EVP_MAC_CTX_gettable_params(ctx) == NULL) return 1;

  params[0] = OSSL_PARAM_construct_int(OSSL_MAC_PARAM_FIPS_APPROVED_INDICATOR,
                                       &approved);
  if (!EVP_MAC_CTX_get_params(ctx, params)) return 0;
...
static int pkey_check_fips_approved(EVP_PKEY_CTX* ctx, EVP_TEST* t) {
  ...
  if (EVP_PKEY_CTX_gettable_params(ctx) == NULL) return 1;

  params[0] = OSSL_PARAM_construct_int(OSSL_ALG_PARAM_FIPS_APPROVED_INDICATOR,
                                       &approved);
  if (!EVP_PKEY_CTX_get_params(ctx, params)) return 0;
  return check_fips_approved(t, approved);
}
...
static int rand_check_fips_approved(EVP_RAND_CTX* ctx, EVP_TEST* t) {
  ...
  if (EVP_RAND_CTX_gettable_params(ctx) == NULL) return 1;

  params[0] = OSSL_PARAM_construct_int(OSSL_DRBG_PARAM_FIPS_APPROVED_INDICATOR,
                                       &approved);
  if (!EVP_RAND_CTX_get_params(ctx, params)) return 0;
  return check_fips_approved(t, approved);
}
...
static int check_security_category(EVP_TEST* t, void* alg_obj,
  ...
  if (t->security_category < 0) return 1;
...
static int ctrl2params(EVP_TEST* t, STACK_OF(OPENSSL_STRING)* controls,
  ...
  if (settables == NULL) settables = settable_ctx_params;
  /* check bounds */
...
static int kdf_check_fips_approved(EVP_KDF_CTX* ctx, EVP_TEST* t) {
  ...
  params[0] = OSSL_PARAM_construct_int(OSSL_KDF_PARAM_FIPS_APPROVED_INDICATOR,
                                       &approved);
  if (!EVP_KDF_CTX_get_params(ctx, params)) return 0;
  return check_fips_approved(t, approved);
}
...
static int cipher_check_fips_approved(EVP_CIPHER_CTX* ctx, EVP_TEST* t) {
  ...
  if (!EVP_CIPHER_CTX_get_params(ctx, params)) return 0;
  return check_fips_approved(t, approved);
}
...
static int evp_test_buffer_append(const char* value, size_t max_len,
  ...
  if (*sk == NULL && !TEST_ptr(*sk = sk_EVP_TEST_BUFFER_new_null())) goto err;

  do {
    if (!TEST_ptr(db = OPENSSL_zalloc(sizeof(*db)))) goto err;
    if (max_len == 0) {
      /* parse all in one shot */
      if ((rv = parse_bin(value, &db->buf, &db->buflen)) != 1) goto err;
    } else {
      /* parse in chunks */
      size_t new_offset = 0;
...
static int evp_test_buffer_ncopy(const char* value,
  ...
  if (!TEST_ptr(tbuf = OPENSSL_malloc(tbuflen))) return 0;
...
static int evp_test_buffer_do(STACK_OF(EVP_TEST_BUFFER)* sk,
  ...
  for (i = 0; i < sk_EVP_TEST_BUFFER_num(sk); i++) {
    ...
    for (j = 0; j < tb->count; j++) {
      if (fn(ctx, tb->buf, tb->buflen) <= 0) return 0;
    }
  }
  return 1;
}
...
static unsigned char* unescape(const char* input, size_t input_len,
  ...
  if (!TEST_ptr(ret = p = OPENSSL_malloc(input_len))) return NULL;
...
static int parse_bin(const char* value, unsigned char** buf, size_t* buflen) {
  ...
  if (value[0] == '"') {
    ...
    if (vlen == 0 || value[vlen - 1] != '"') return 0;
    vlen--;
...
static int parse_bin_chunk(const char* value, size_t offset, size_t max,
  ...
  if (value[0] == '"') {
    ...
    if (vlen == 1 && value_str[0] != '"') /* Missing ending quotation mark */
      return -1;
    if (vlen == 1 && value_str[0] == '"') /* End of value */
      return 0;
    vlen--;
    chunk_len = max > vlen ? vlen : max;
    if ((*buf = unescape(value_str, chunk_len, buflen)) == NULL) return -1;
  } else {
...
static int digest_test_init(EVP_TEST* t, const char* alg) {
  ...
  mdat->pad_type = 0;
  mdat->xof = 0;
  if (fetched_digest != NULL) TEST_info("%s is fetched", alg);
  return 1;
}
...
static int digest_test_parse(EVP_TEST* t, const char* keyword,
  ...
  if (strcmp(keyword, "XOF") == 0) return (mdata->xof = atoi(value)) > 0;
...
static int digest_test_run(EVP_TEST* t) {
  ...
  t->err = "TEST_FAILURE";
  if (!TEST_ptr(mctx = EVP_MD_CTX_new())) goto err;

  got = OPENSSL_malloc(expected->output_len > EVP_MAX_MD_SIZE
                           ? expected->output_len
                           : EVP_MAX_MD_SIZE);
  if (!TEST_ptr(got)) goto err;
  ...
  *p++ = OSSL_PARAM_construct_end();
  ...
  if (xof) {
    ...
    if (!test_duplicate_md_ctx(t, EVP_MD_CTX_dup(mctx))) goto err;
...
static int cipher_test_init(EVP_TEST* t, const char* alg) {
  ...
  if (!TEST_ptr(cdat = OPENSSL_zalloc(sizeof(*cdat)))) return 0;
  ...
  t->data = cdat;
  if (fetched_cipher != NULL) TEST_info("%s is fetched", alg);
  return 1;
}
...
static int cipher_test_parse(EVP_TEST* t, const char* keyword,
  ...
  if (cdat->aead) {
    ...
    if (strcmp(keyword, "TLSAAD") == 0) cdat->tls_aad = tls_aad = 1;
...
static int cipher_test_enc(EVP_TEST* t, int enc, size_t out_misalign,
  ...
  t->err = "TEST_FAILURE";
  if (!TEST_ptr(ctx_base = EVP_CIPHER_CTX_new())) goto err;
  ...
  if (in_place == 1) {
    ...
    if (!tmp) goto err;
    in = memcpy(
        tmp + out_misalign + in_len + 2 * EVP_MAX_BLOCK_LENGTH + inp_misalign,
        in, in_len);
  }
  ...
  if (expected->tls_aad) {
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_SET_TAG, (int)expected->tag_len,
                            expected->tag) <= 0) {
      t->err = "TAG_SET_ERROR";
      goto err;
    }
  ...
  if (!enc && expected->tls_aad) {
    ...
    if ((int)out_len > tmplen + tmpflen) out_len = tmplen + tmpflen;
  }
  ...
  if (enc && expected->aead && !expected->tls_aad) {
    ...
    if (EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, (int)expected->tag_len,
                            rtag) <= 0) {
...
err:
  OPENSSL_free(tmp);
  if (ctx != ctx_base) EVP_CIPHER_CTX_free(ctx_base);
  EVP_CIPHER_CTX_free(ctx);
  return ok;
}
...
static int cipher_test_run(EVP_TEST* t) {
  ...
  for (in_place = 1; in_place >= 0; in_place--) {
    ...
    if (process_mode_in_place == 1 && in_place == 0) break;

    for (frag = 0; frag <= fragmax; frag++) {
      if (frag == 1 && data_chunk_size != 0) break;
      for (out_misalign = 0; out_misalign <= 1; out_misalign++) {
        for (inp_misalign = 0; inp_misalign <= 1; inp_misalign++) {
          ...
          if (inp_misalign == 1 && in_place == 1) break;
          if (in_place == 1) {
...
static int mac_test_parse(EVP_TEST* t, const char* keyword, const char* value) {
  ...
  if (strcmp(keyword, "Output") == 0)
    return parse_bin(value, &mdata->output, &mdata->output_len);
  if (strcmp(keyword, "XOF") == 0) return mdata->xof = 1;
  if (strcmp(keyword, "NoReinit") == 0) return mdata->no_reinit = 1;
  if (strcmp(keyword, "Ctrl") == 0) return ctrladd(mdata->controls, value);
  if (strcmp(keyword, "OutputSize") == 0) {
    mdata->output_size = atoi(value);
    if (mdata->output_size < 0) return -1;
    return 1;
  }
  if (strcmp(keyword, "BlockSize") == 0) {
    mdata->block_size = atoi(value);
    if (mdata->block_size < 0) return -1;
    return 1;
  }
  return 0;
}
...
static int mac_test_ctrl_pkey(EVP_TEST* t, EVP_PKEY_CTX* pctx,
  ...
  if (!TEST_ptr(tmpval = OPENSSL_strdup(value))) return 0;
  p = strchr(tmpval, ':');
  if (p != NULL) {
    *p++ = '\0';
...
static int mac_test_run_mac(EVP_TEST* t) {
  ...
  if (p != NULL) {
    if (!OSSL_PARAM_get_size_t(p, &size_val)) goto err;
  }
...
retry:
  ...
  if (xof) {
    ...
    if (!mac_check_fips_approved(ctx, t)) goto err;
  }
...
static int mac_test_run(EVP_TEST* t) {
  ...
  if (expected->mac != NULL) return mac_test_run_mac(t);
...
static int encapsulate(EVP_TEST* t, EVP_PKEY_CTX* ctx, const char* op,
  ...
  if (!TEST_size_t_lt(params_n, params_max)) goto err;

  if (kdata->entropy != NULL) /* Input key material a.k.a entropy */
    params[params_n++] = OSSL_PARAM_construct_octet_string(
...
static int kem_test_run(EVP_TEST* t) {
  ...
  if (!pkey_check_security_category(t, pkey)) goto err;
...
ok:
  ret = 1;
err:
  if (!found_key) EVP_PKEY_free(pkey);
  OPENSSL_free(wrapped);
  OPENSSL_free(secret);
  return ret;
}
...
static int pkey_test_init_keyctx(EVP_TEST* t, const char* keyname,
  ...
  if (use_public) rv = find_key(&pkey, keyname, public_keys);
  if (rv == 0) rv = find_key(&pkey, keyname, private_keys);
  if (rv == 0 || pkey == NULL) {
...
static int pkey_test_run_init(EVP_TEST* t) {
  ...
  for (i = 0; i < sk_OPENSSL_STRING_num(data->controls); i++) {
    ...
    if (!pkey_test_ctrl(t, data->ctx, value) || t->err != NULL) goto err;
  }
  ret = 1;
err:
  ctrl2params_free(params, params_n, params_n_allocstart);
...
static int pkey_test_run(EVP_TEST* t) {
  ...
  if (!pkey_test_run_init(t)) goto err;
  ...
  if (pkey_check_fips_approved(expected->ctx, t) <= 0) goto err;
...
static int pkey_fromdata_test_init(EVP_TEST* t, const char* name) {
  ...
  if (kdata->ctx == NULL) goto err;
  if (EVP_PKEY_fromdata_init(kdata->ctx) <= 0) goto err;
  kdata->controls = sk_OPENSSL_STRING_new_null();
  if (kdata->controls == NULL) goto err;
  t->data = kdata;
  return 1;
err:
  EVP_PKEY_CTX_free(kdata->ctx);
...
static int pkey_fromdata_test_run(EVP_TEST* t) {
  ...
  if (!pkey_check_security_category(t, key)) goto err;
...
static int verify_test_run(EVP_TEST* t) {
  ...
  if (!pkey_check_fips_approved(kdata->ctx, t)) ret = 0;
err:
  return ret;
}
...
static int pderive_test_parse(EVP_TEST* t, const char* keyword,
  ...
  if (strcmp(keyword, "PeerKeyValidate") == 0) validate = 1;

  if (validate || strcmp(keyword, "PeerKey") == 0) {
    EVP_PKEY* peer = NULL;

    kdata->validate = validate;
    if (find_key(&peer, value, public_keys) == 0) return -1;
    kdata->peer = peer;
    return 1;
  }
...
static int scrypt_test_parse(EVP_TEST* t, const char* keyword,
  ...
  if (strcmp(keyword, "N") == 0) return parse_uint64(value, &pdata->N);
  if (strcmp(keyword, "p") == 0) return parse_uint64(value, &pdata->p);
  if (strcmp(keyword, "r") == 0) return parse_uint64(value, &pdata->r);
...
static int pbe_test_run(EVP_TEST* t) {
  ...
  if (expected->pbe_type == PBE_TYPE_PBKDF2) {
    if (PKCS5_PBKDF2_HMAC((char*)expected->pass, (int)expected->pass_len,
                          ...
                          expected->iter, expected->md, (int)expected->key_len,
                          key) == 0) {
      t->err = "PBKDF2_ERROR";
      goto err;
    }
...
static int encode_test_init(EVP_TEST* t, const char* encoding) {
  ...
  if (!TEST_ptr(edata = OPENSSL_zalloc(sizeof(*edata)))) return 0;
  if (strcmp(encoding, "canonical") == 0) {
    ...
    if (!TEST_ptr(t->expected_err = OPENSSL_strdup("DECODE_ERROR"))) goto err;
  } else {
    TEST_error(
        "Bad encoding: %s."
...
static int encode_test_run(EVP_TEST* t) {
  ...
  if (!TEST_ptr(decode_out = OPENSSL_malloc(decode_length))) goto err;
  /*
...
static int rand_test_init(EVP_TEST* t, const char* name) {
  ...
  if (!TEST_ptr(rdata = OPENSSL_zalloc(sizeof(*rdata)))) return 0;
...
static int rand_test_parse(EVP_TEST* t, const char* keyword,
  ...
  if ((p = strchr(keyword, '.')) != NULL) {
    ...
    if (strcmp(keyword, "GenerateBits") == 0) {
      if ((n = atoi(value)) <= 0 || n % 8 != 0) return 0;
...
static int rand_test_run(EVP_TEST* t) {
  ...
  if (!TEST_ptr(got = OPENSSL_malloc(got_len))) return 0;
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  for (i = 0; i <= expected->n; i++) {
    ...
    *p++ = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_NONCE, z,
                                             item->nonce_len);
    *p = OSSL_PARAM_construct_end();
    if (!TEST_true(EVP_RAND_instantiate(expected->parent, strength, 0, NULL, 0,
                                        params)))
      goto err;
    ...
    if (!TEST_true(EVP_RAND_generate(expected->ctx, got, got_len, strength,
      ...
      goto err;
    if (!TEST_mem_eq(got, got_len, item->output, item->output_len)) goto err;
    if (!rand_check_fips_approved(expected->ctx, t)) goto err;
...
static int kdf_test_ctrl(EVP_TEST* t, EVP_KDF_CTX* kctx, const char* value) {
  ...
  if (!TEST_ptr(name = OPENSSL_strdup(value))) return 0;
  p = strchr(name, ':');
  if (p == NULL)
    p = "";
  else
...
static int kdf_test_parse(EVP_TEST* t, const char* keyword, const char* value) {
  ...
  if (strcmp(keyword, "CtrlInit") == 0)
    return ctrladd(kdata->init_controls, value);
  if (HAS_PREFIX(keyword, "Ctrl")) return kdf_test_ctrl(t, kdata->ctx, value);
  return 0;
}
...
static int pkey_kdf_test_init(EVP_TEST* t, const char* name) {
  ...
  if (kdata->ctx == NULL || EVP_PKEY_derive_init(kdata->ctx) <= 0) goto err;
...
static int pkey_kdf_test_parse(EVP_TEST* t, const char* keyword,
  ...
  if (strcmp(keyword, "Output") == 0)
    return parse_bin(value, &kdata->output, &kdata->output_len);
  if (HAS_PREFIX(keyword, "Ctrl")) return pkey_test_ctrl(t, kdata->ctx, value);
  return 0;
}
...
static int keypair_test_init(EVP_TEST* t, const char* pair) {
  ...
  *pub++ = '\0';
  ...
  if (!TEST_ptr(data = OPENSSL_malloc(sizeof(*data)))) goto end;
  data->privk = pk;
  data->pubk = pubk;
  t->data = data;
  rv = 1;
...
static int keygen_test_init(EVP_TEST* t, const char* alg) {
  ...
  if (!TEST_ptr(data = OPENSSL_zalloc(sizeof(*data)))) return 0;
  data->keyname = NULL;
  data->paramname = NULL;
...
static int keygen_test_parse(EVP_TEST* t, const char* keyword,
  ...
  if (strcmp(keyword, "Ctrl") == 0) return ctrladd(keygen->in_controls, value);
...
static int keygen_test_run(EVP_TEST* t) {
  ...
  if (keygen->keyname != NULL) {
    ...
    if (!TEST_ptr(key = OPENSSL_malloc(sizeof(*key)))) goto err;
...
static int digestsigver_test_parse(EVP_TEST* t, const char* keyword,
  ...
  if (strcmp(keyword, "Key") == 0) {
    ...
    if (mdata->is_verify) rv = find_key(&pkey, value, public_keys);
    if (rv == 0) rv = find_key(&pkey, value, private_keys);
    if (rv == 0 || pkey == NULL) {
      t->skip = 1;
      return 1;
    }
  ...
  if (strcmp(keyword, "NonceType") == 0) {
    if (strcmp(value, "deterministic") == 0) mdata->deterministic_noncetype = 1;
    return 1;
  }
  return 0;
}
...
static int signverify_init(EVP_TEST* t, DIGESTSIGN_DATA* data) {
  ...
  for (i = 0; i < sk_OPENSSL_STRING_num(data->controls); i++) {
    ...
    if (!pkey_test_ctrl(t, data->pctx, value) || t->err != NULL) goto err;
  }
  ret = 1;
err:
  ctrl2params_free(params, params_n, params_allocated_n);
...
static int digestsign_test_run(EVP_TEST* t) {
  ...
  if (!signverify_init(t, expected)) goto err;
  if (!check_deterministic_noncetype(t, expected)) goto err;

  if (!pkey_check_security_category(t, expected->key)) goto err;

  for (i = 0; i < sk_OPENSSL_STRING_num(expected->controls); i++) {
    char* value = sk_OPENSSL_STRING_value(expected->controls, i);
    if (!pkey_test_ctrl(t, expected->pctx, value) || t->err != NULL) return 0;
  }
...
static int digestverify_test_run(EVP_TEST* t) {
  ...
  if (!signverify_init(t, mdata)) return 1;
...
static const EVP_TEST_METHOD* find_test(const char* name) {
  ...
  for (tt = evp_test_list; *tt; tt++) {
    if (strcmp(name, (*tt)->name) == 0) return *tt;
  }
  return NULL;
}
...
static void clear_test(EVP_TEST* t) {
  ...
  ERR_clear_error();
  if (t->data != NULL) {
    if (t->meth != NULL) t->meth->cleanup(t);
    OPENSSL_free(t->data);
    t->data = NULL;
  }
...
static int check_test_error(EVP_TEST* t) {
  ...
  if (t->err == NULL && t->expected_err == NULL) return 1;
  ...
  if (reason == NULL) {
    TEST_info(
        "%s:%d: Expected error \"%s\", no strings available."
        " Assuming ok.",
        t->s.test_file, t->s.start, t->reason);
    return 1;
  }

  if (strcmp(reason, t->reason) == 0) return 1;
...
static int prov_available(char* providers) {
  ...
  while (more) {
    ...
    if (*providers == '\0') break; /* End of the road */
    ...
    if (*p == '\0')
      ...
      *p = '\0';
    if (OSSL_PROVIDER_available(libctx, providers)) return 1; /* Found one */
  }
  return 0;
}
...
top:
  do {
    if (BIO_eof(t->s.fp)) return EOF;
    clear_test(t);
    if (!test_readstanza(&t->s)) return 0;
...
start:
  ...
  if (!TEST_ptr(t->meth = find_test(pp->key))) return 0;
...
static int run_file_tests(int i) {
  ...
  if (!TEST_ptr(t = OPENSSL_zalloc(sizeof(*t)))) return 0;
...
int setup_tests(void) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_IN_PLACE:
        ...
        case OPT_DATA_CHUNK:
          if (!opt_int(opt_arg(), &data_chunk_size)) return 0;
        break;
   ...
   */
  if (config_file == NULL && provider_name == NULL) provider_name = "default";
  ...
  if ((fake_rand = fake_rand_start(libctx)) == NULL) return 0;
...
static int is_digest_disabled(const char* name) {
  ...
  if (HAS_CASE_PREFIX(name, "BLAKE")) return 1;
#endif
#ifdef OPENSSL_NO_MD2
  if (OPENSSL_strcasecmp(name, "MD2") == 0) return 1;
#endif
#ifdef OPENSSL_NO_MDC2
  if (OPENSSL_strcasecmp(name, "MDC2") == 0) return 1;
#endif
#ifdef OPENSSL_NO_MD4
  if (OPENSSL_strcasecmp(name, "MD4") == 0) return 1;
#endif
#ifdef OPENSSL_NO_MD5
  if (OPENSSL_strcasecmp(name, "MD5") == 0) return 1;
#endif
#ifdef OPENSSL_NO_RMD160
  if (OPENSSL_strcasecmp(name, "RIPEMD160") == 0) return 1;
#endif
#ifdef OPENSSL_NO_SM3
  if (OPENSSL_strcasecmp(name, "SM3") == 0) return 1;
#endif
#ifdef OPENSSL_NO_WHIRLPOOL
  if (OPENSSL_strcasecmp(name, "WHIRLPOOL") == 0) return 1;
#endif
  return 0;
}
...
static int is_pkey_disabled(const char* name) {
#ifdef OPENSSL_NO_EC
  if (HAS_CASE_PREFIX(name, "EC")) return 1;
#endif
#ifdef OPENSSL_NO_DH
  if (HAS_CASE_PREFIX(name, "DH")) return 1;
#endif
#ifdef OPENSSL_NO_DSA
  if (HAS_CASE_PREFIX(name, "DSA")) return 1;
#endif
#ifdef OPENSSL_NO_SM2
  if (HAS_CASE_PREFIX(name, "SM2")) return 1;
#endif

  /* For sigalgs we use, we also check for digest suffixes */
#ifdef OPENSSL_NO_RMD160
  if (HAS_CASE_SUFFIX(name, "-RIPEMD160")) return 1;
#endif
#ifdef OPENSSL_NO_SM3
  if (HAS_CASE_SUFFIX(name, "-SM3")) return 1;
#endif
#ifdef OPENSSL_NO_ML_DSA
  if (HAS_CASE_PREFIX(name, "ML-DSA")) return 1;
#endif
  return 0;
}
...
static int is_mac_disabled(const char* name) {
  ...
  if (HAS_CASE_PREFIX(name, "CMAC")) return 1;
#endif
#ifdef OPENSSL_NO_POLY1305
  if (HAS_CASE_PREFIX(name, "Poly1305")) return 1;
#endif
#ifdef OPENSSL_NO_SIPHASH
  if (HAS_CASE_PREFIX(name, "SipHash")) return 1;
#endif
  return 0;
}
static int is_kdf_disabled(const char* name) {
#ifdef OPENSSL_NO_SCRYPT
  if (HAS_CASE_SUFFIX(name, "SCRYPT")) return 1;
#endif
#ifdef OPENSSL_NO_ARGON2
  if (HAS_CASE_SUFFIX(name, "ARGON2")) return 1;
#endif
  return 0;
}
...
static int is_cipher_disabled(const char* name) {
  ...
  if (HAS_CASE_PREFIX(name, "ARIA")) return 1;
#endif
#ifdef OPENSSL_NO_BF
  if (HAS_CASE_PREFIX(name, "BF")) return 1;
#endif
#ifdef OPENSSL_NO_CAMELLIA
  if (HAS_CASE_PREFIX(name, "CAMELLIA")) return 1;
#endif
#ifdef OPENSSL_NO_CAST
  if (HAS_CASE_PREFIX(name, "CAST")) return 1;
#endif
#ifdef OPENSSL_NO_CHACHA
  if (HAS_CASE_PREFIX(name, "CHACHA")) return 1;
#endif
#ifdef OPENSSL_NO_POLY1305
  if (HAS_CASE_SUFFIX(name, "Poly1305")) return 1;
#endif
#ifdef OPENSSL_NO_DES
  if (HAS_CASE_PREFIX(name, "DES")) return 1;
  if (HAS_CASE_SUFFIX(name, "3DESwrap")) return 1;
#endif
#ifdef OPENSSL_NO_OCB
  if (HAS_CASE_SUFFIX(name, "OCB")) return 1;
#endif
#ifdef OPENSSL_NO_IDEA
  if (HAS_CASE_PREFIX(name, "IDEA")) return 1;
#endif
#ifdef OPENSSL_NO_RC2
  if (HAS_CASE_PREFIX(name, "RC2")) return 1;
#endif
#ifdef OPENSSL_NO_RC4
  if (HAS_CASE_PREFIX(name, "RC4")) return 1;
#endif
#ifdef OPENSSL_NO_RC5
  if (HAS_CASE_PREFIX(name, "RC5")) return 1;
#endif
#ifdef OPENSSL_NO_SEED
  if (HAS_CASE_PREFIX(name, "SEED")) return 1;
#endif
#ifdef OPENSSL_NO_SIV
  if (HAS_CASE_SUFFIX(name, "SIV")) return 1;
#endif
#ifdef OPENSSL_NO_SM4
  if (HAS_CASE_PREFIX(name, "SM4")) return 1;
#endif
  return 0;
}
