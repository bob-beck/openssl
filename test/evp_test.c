...
static int parse_bin_chunk(const char* value,
                           size_t offset,
                           size_t max,
                           unsigned char** buf,
                           size_t* buflen,
...
static int get_faked_bytes(unsigned char* buf,
                           size_t num,
...
static int fips_indicator_cb(const char* type,
                             const char* desc,
...
static int check_security_category(EVP_TEST* t,
                                   void* alg_obj,
 ...
 */
static unsigned char* unescape(const char* input,
                               size_t input_len,
                               size_t* out_len) {
  unsigned char *ret, *p;
  size_t i;
...
static int digest_update_fn(void* ctx,
                            const unsigned char* buf,
                            size_t buflen) {
...
static int cipher_test_parse(EVP_TEST* t,
                             const char* keyword,
                             const char* value) {
  CIPHER_DATA* cdat = t->data;
  int i;
...
static int mac_test_ctrl_pkey(EVP_TEST* t,
                              EVP_PKEY_CTX* pctx,
                              const char* value) {
  int rv = 0;
  char *p, *tmpval;
 ...
 */
static int pkey_test_init_keyctx(EVP_TEST* t,
                                 const char* keyname,
                                 int use_public) {
  PKEY_DATA* kdata;
  EVP_PKEY* pkey = NULL;
...
static int pkey_test_init(EVP_TEST* t,
                          const char* name,
                          int use_public,
...
static int pkey_test_init_ex2(EVP_TEST* t,
                              const char* name,
                              int use_public,
                              int (*keyopinit)(EVP_PKEY_CTX* ctx,
                                               EVP_SIGNATURE* algo,
...
static int pkey_add_control(EVP_TEST* t,
                            STACK_OF(OPENSSL_STRING)* controls,
                            const char* value) {
  char* p;
 ...
 */
static int void_test_parse(EVP_TEST* t,
                           const char* keyword,
                           const char* value) {
  return 0;
}
...
static int check_pkey_expected_values(EVP_TEST* t,
                                      const EVP_PKEY* pkey,
                                      STACK_OF(OPENSSL_STRING)* out_controls) {
  int ret = 0;
...
static int digestsigver_test_init(EVP_TEST* t,
                                  const char* alg,
                                  int is_verify,
                                  int is_oneshot) {
...
static int digestsign_update_fn(void* ctx,
                                const unsigned char* buf,
                                size_t buflen) {
...
static int digestverify_update_fn(void* ctx,
                                  const unsigned char* buf,
                                  size_t buflen) {
