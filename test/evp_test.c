...
static int
parse_bin_chunk(const char* value,
                size_t offset,
                size_t max,
                unsigned char** buf,
                size_t* buflen,
...
static int
get_faked_bytes(unsigned char* buf,
                size_t num,
...
fips_indicator_cb(const char* type, const char* desc, const OSSL_PARAM params[])
{
  ...
  TEST_info(
    "(Indicator Callback received %s : %s is not approved)", type, desc);
  return 1;
}
...
static int
check_security_category(EVP_TEST* t,
                        void* alg_obj,
...
static int
pkey_check_security_category(EVP_TEST* t, EVP_PKEY* pkey)
{
            ...
            size_t* params_n)
{
  ...
  for (i = 0; i < sk_OPENSSL_STRING_num(controls); i++) {
    ...
    if (tmpval == NULL || !OSSL_PARAM_allocate_from_text(&params[*params_n],
                                                         settables,
                                                         tmpkey,
                                                         tmpval,
                                                         strlen(tmpval),
...
static void
ctrl2params_free(OSSL_PARAM params[],
                 size_t params_n,
...
static int
evp_test_buffer_append(const char* value,
                       size_t max_len,
                       STACK_OF(EVP_TEST_BUFFER)** sk)
{
  ...
  do {
    ...
    if (max_len == 0) {
      ...
      if ((rv = parse_bin_chunk(
             value, offset, max_len, &db->buf, &db->buflen, &new_offset)) == -1)
        goto err;
      offset = new_offset;
    }
                ...
                const OSSL_PARAM initparams[])
{
  ...
  if (!EVP_CipherInit_ex2(
        ctx_base, expected->cipher, NULL, NULL, enc, initparams)) {
  ...
  if (expected->iv) {
    if (expected->aead) {
      if (EVP_CIPHER_CTX_ctrl(
            ctx_base, EVP_CTRL_AEAD_SET_IVLEN, (int)expected->iv_len, 0) <= 0) {
  ...
  if (expected->aead && !expected->tls_aad) {
    ...
    if (tag || expected->aead != EVP_CIPH_GCM_MODE) {
      if (EVP_CIPHER_CTX_ctrl(
            ctx_base, EVP_CTRL_AEAD_SET_TAG, (int)expected->tag_len, tag) <= 0)
        goto err;
    }
  }
  ...
  if (!EVP_CipherInit_ex(
        ctx_base, NULL, NULL, expected->key, expected->iv, -1)) {
    t->err = "KEY_SET_ERROR";
    goto err;
  }
    ...
    if (!frag) {
      ...
      for (i = 0; expected->aad[i] != NULL; i++) {
        ...
        if (expected->aad_len[i] > 1 &&
            !EVP_CipherUpdate(
              ctx, NULL, &chunklen, expected->aad[i] + donelen, 1))
          goto err;
      }
    }
  }
  ...
  if (expected->tls_aad) {
    ...
    if (EVP_CIPHER_CTX_ctrl(
          ctx, EVP_CTRL_AEAD_SET_TAG, (int)expected->tag_len, expected->tag) <=
        0) {
      t->err = "TAG_SET_ERROR";
      goto err;
    }
  ...
  if (!frag) {
    ...
    if (in_len > 1) {
      if (!EVP_CipherUpdate(
            ctx, tmp + out_misalign + tmplen, &chunklen, in, (int)(in_len - 1)))
        goto err;
      tmplen += chunklen;
      in += in_len - 1;
      in_len = 1;
  ...
  if (enc && expected->aead && !expected->tls_aad) {
    ...
    if (EVP_CIPHER_CTX_ctrl(
          ctx, EVP_CTRL_AEAD_GET_TAG, (int)expected->tag_len, rtag) <= 0) {
  ...
  if (expected->next_iv != NULL) {
    ...
    if (!TEST_true(EVP_CIPHER_CTX_get_updated_iv(ctx, iv, sizeof(iv))) ||
        !TEST_mem_eq(
          expected->next_iv, expected->iv_len, iv, expected->iv_len)) {
    ...
    for (frag = 0; frag <= fragmax; frag++) {
      ...
      for (out_misalign = 0; out_misalign <= 1; out_misalign++) {
        for (inp_misalign = 0; inp_misalign <= 1; inp_misalign++) {
          ...
          if (cdat->enc) {
            rv = cipher_test_enc(
              t, 1, out_misalign, inp_misalign, frag, in_place, initparams);
            if (rv != 1)
              goto end;
          }
          if (cdat->enc != 1) {
            rv = cipher_test_enc(
              t, 0, out_misalign, inp_misalign, frag, in_place, initparams);
            if (rv != 1)
              goto end;
          }
        }
...
mac_test_run_mac(EVP_TEST* t)
{
  ...
  if (expected->alg == NULL)
    ...
    TEST_info(
      "Trying the EVP_MAC %s test with %s", expected->mac_name, expected->alg);
...
static const EVP_TEST_METHOD mac_test_method = { "MAC",
                                                 mac_test_init,
                                                 mac_test_cleanup,
                                                 mac_test_parse,
                                                 mac_test_run };
...
kem_test_parse(EVP_TEST* t, const char* keyword, const char* value)
{
  ...
  if (strcmp(keyword, "EncodedPublicKey") == 0)
    return parse_bin(
      value, &kdata->encoded_pub_key, &kdata->encoded_pub_key_len);
  if (strcmp(keyword, "EncodedPrivateKey") == 0)
    return parse_bin(
      value, &kdata->encoded_priv_key, &kdata->encoded_priv_key_len);
            ...
            size_t* outsecretlen)
{
  ...
  if (kdata->ciphertext != NULL &&
      !TEST_mem_eq(
        wrapped, wrappedlen, kdata->ciphertext, kdata->ciphertext_len)) {
    ret = 0;
    goto err;
  }
...
kem_test_run(EVP_TEST* t)
{
  ...
  if (kdata->input == NULL) {
    ...
    ret = encapsulate(
      t, kdata->ctx, kdata->op, &wrapped, &wrappedlen, &secret, &secretlen);
    if (ret == 0 || t->err != NULL)
      goto err;
...
typedef struct pkey_data_st
{
  ...
  int (*keyopinit_ex2)(EVP_PKEY_CTX* ctx,
                       EVP_SIGNATURE* algo,
...
pkey_test_run_init(EVP_TEST* t)
{
  ...
  int i, ret = 0;
  OSSL_PARAM params[5] = { OSSL_PARAM_END,
                           OSSL_PARAM_END,
                           OSSL_PARAM_END,
                           OSSL_PARAM_END,
                           OSSL_PARAM_END };
  ...
  if (sk_OPENSSL_STRING_num(data->init_controls) > 0) {
    if (!ctrl2params(
          t, data->init_controls, NULL, params, OSSL_NELEM(params), &params_n))
      goto err;
    p = params;
  }
...
pkey_test_run(EVP_TEST* t)
{
  ...
  if (expected->keyop(
        expected->ctx, NULL, &got_len, expected->input, expected->input_len) <=
        0 ||
  ...
  if (expected->keyop(
        expected->ctx, got, &got_len, expected->input, expected->input_len) <=
      0) {
    t->err = "KEYOP_ERROR";
    goto err;
  }
  ...
  if (expected->keyop(
        copy, NULL, &got_len, expected->input, expected->input_len) <= 0 ||
      !TEST_ptr(got = OPENSSL_malloc(got_len))) {
    t->err = "KEYOP_LENGTH_ERROR";
    goto err;
  }
  if (expected->keyop(
        copy, got, &got_len, expected->input, expected->input_len) <= 0) {
    t->err = "KEYOP_ERROR";
    goto err;
  }
  if (!memory_err_compare(t,
...
{
  if (strchr(name, ':') != NULL)
    return pkey_test_init_ex2(
      t, name, 0, EVP_PKEY_sign_init_ex2, EVP_PKEY_sign);
...
{
  return pkey_test_init_ex2(
    t, name, 0, EVP_PKEY_sign_message_init, EVP_PKEY_sign);
}
...
verify_recover_test_init(EVP_TEST* t, const char* name)
{
  if (strchr(name, ':') != NULL)
    return pkey_test_init_ex2(
      t, name, 1, EVP_PKEY_verify_recover_init_ex2, EVP_PKEY_verify_recover);
  return pkey_test_init(
    t, name, 1, EVP_PKEY_verify_recover_init_ex, EVP_PKEY_verify_recover);
}
...
pderive_test_run(EVP_TEST* t)
{
  ...
  if (!pkey_check_security_category(t, EVP_PKEY_CTX_get0_pkey(expected->ctx)))
    goto err;
  if (EVP_PKEY_derive_set_peer_ex(
        expected->ctx, expected->peer, expected->validate) <= 0) {
...
static const EVP_TEST_METHOD pbe_test_method = { "PBE",
                                                 pbe_test_init,
                                                 pbe_test_cleanup,
                                                 pbe_test_parse,
                                                 pbe_test_run };
  ...
  if (expected->encoding == BASE64_CANONICAL_ENCODING) {
    ...
    do {
      ...
      if (data_chunk_size == 0 || current_len > input_len)
        current_len = input_len;
      if (!TEST_true(EVP_EncodeUpdate(encode_ctx,
                                      encode_out,
                                      &chunk_len,
  ...
  *p++ = OSSL_PARAM_construct_int(OSSL_DRBG_PARAM_USE_DF, &expected->use_df);
  if (expected->cipher != NULL)
    *p++ = OSSL_PARAM_construct_utf8_string(
      OSSL_DRBG_PARAM_CIPHER, expected->cipher, 0);
  if (expected->digest != NULL)
    *p++ = OSSL_PARAM_construct_utf8_string(
      OSSL_DRBG_PARAM_DIGEST, expected->digest, 0);
...
static const EVP_TEST_METHOD kdf_test_method = { "KDF",
                                                 kdf_test_init,
                                                 kdf_test_cleanup,
                                                 kdf_test_parse,
                                                 kdf_test_run };
...
static int
check_pkey_expected_values(EVP_TEST* t,
                           const EVP_PKEY* pkey,
                           STACK_OF(OPENSSL_STRING)* out_controls)
{
  int ret = 0;
    ...
    for (p = out_params; p->key != NULL; ++p) {
      if (p->data_type == OSSL_PARAM_OCTET_STRING) {
        ...
        ret = EVP_PKEY_get_octet_string_param(
                pkey, p->key, data, p->data_size, &len) &&
              len == p->data_size &&
...
static int
digestsigver_test_init(EVP_TEST* t,
                       const char* alg,
                       int is_verify,
                       int is_oneshot)
{
...
signverify_init(EVP_TEST* t, DIGESTSIGN_DATA* data)
{
  ...
  if (data->is_verify) {
    if (!EVP_DigestVerifyInit_ex(
          data->ctx, &data->pctx, name, libctx, NULL, data->key, p)) {
...
digestsign_test_run(EVP_TEST* t)
{
  ...
  if (!evp_test_buffer_do(
        expected->input, digestsign_update_fn, expected->ctx)) {
    t->err = "DIGESTUPDATE_ERROR";
    goto err;
  }
...
oneshot_digestsign_test_run(EVP_TEST* t)
{
  ...
  if (!EVP_DigestSign(
        expected->ctx, NULL, &got_len, expected->osin, expected->osin_len)) {
  ...
  got_len *= 2;
  if (!EVP_DigestSign(
        expected->ctx, got, &got_len, expected->osin, expected->osin_len)) {
...
setup_tests(void)
{
  ...
  if (!test_get_libctx(
        &libctx, &prov_null, config_file, &libprov, provider_name))
    return 0;
