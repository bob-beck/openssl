...
static int
get_faked_bytes (unsigned char *buf, size_t num, ossl_unused const char *name,
...
static int
fips_indicator_cb (const char *type, const char *desc,
...
static int
check_security_category (EVP_TEST *t, void *alg_obj,
...
static int
ctrl2params (EVP_TEST *t, STACK_OF (OPENSSL_STRING) *controls,
...
static void
ctrl2params_free (OSSL_PARAM params[], size_t params_n,
 ...
 */
static int
memory_err_compare (EVP_TEST *t, const char *err, const void *expected,
                    size_t expected_len, const void *got, size_t got_len)
{
  int r;
...
static int
evp_test_buffer_append (const char *value, size_t max_len,
 ...
 */
static int
parse_bin_chunk (const char *value, size_t offset, size_t max,
  ...
  if (sk_EVP_TEST_BUFFER_num (expected->input) == 1
      ...
      && !inbuf->count_set)
    {
      ...
      if (!TEST_true (EVP_Q_digest (
              libctx, EVP_MD_get0_name (expected->fetched_digest), NULL,
...
static int
cipher_test_enc (EVP_TEST *t, int enc, size_t out_misalign,
...
static int
encapsulate (EVP_TEST *t, EVP_PKEY_CTX *ctx, const char *op,
...
static int
decapsulate (EVP_TEST *t, EVP_PKEY_CTX *ctx, const char *op,
...
static int
pkey_add_control (EVP_TEST *t, STACK_OF (OPENSSL_STRING) *controls,
                  const char *value)
{
  char *p;
...
pkey_test_run (EVP_TEST *t)
{
  ...
  if (!pkey_check_security_category (t,
                                     EVP_PKEY_CTX_get0_pkey (expected->ctx)))
    goto err;
...
static const EVP_TEST_METHOD psign_test_method
    = { "Sign", sign_test_init, pkey_test_cleanup, pkey_test_parse,
        pkey_test_run };
...
/*
 * "VerifyRecover" implies EVP_PKEY_verify_recover_init_ex2() if the argument
 * is a colon-separated pair, {algorithm}:{key}. If not, it implies
  ...
  if (expected->pbe_type == PBE_TYPE_PBKDF2)
  ...
  else if (expected->pbe_type == PBE_TYPE_PKCS12)
    {
      ...
      if (PKCS12_key_gen_uni (expected->pass, (int)expected->pass_len,
                              ...
                              (int)expected->key_len, key, fetched_digest)
          == 0)
        {
          t->err = "PKCS12_ERROR";
          goto err;
  ...
  for (i = 0; i <= expected->n; i++)
    {
      ...
      *p++ = OSSL_PARAM_construct_octet_string (OSSL_RAND_PARAM_TEST_NONCE, z,
                                                item->nonce_len);
      *p = OSSL_PARAM_construct_end ();
      if (!TEST_true (EVP_RAND_instantiate (expected->parent, strength, 0,
                                            NULL, 0, params)))
        goto err;
...
static const EVP_TEST_METHOD rand_test_method
    = { "RAND", rand_test_init, rand_test_cleanup, rand_test_parse,
        rand_test_run };
...
static int
digestsigver_test_init (EVP_TEST *t, const char *alg, int is_verify,
                        int is_oneshot)
{
  ...
  static const OPTIONS test_options[]
      = { OPT_TEST_OPTIONS_WITH_EXTRA_USAGE ("[file...]\n"),
          ...
          { "process", OPT_IN_PLACE, 's',
            "Mode for data processing by cipher tests [in_place/both], both "
            "by default" },
          { "provider", OPT_PROVIDER_NAME, 's',
            "The provider to load (when no configuration file, the default "
            "value is 'default')" },
