...
static void tear_down(OSSL_CMP_CTX_TEST_FIXTURE* fixture) {
  if (fixture != NULL) OSSL_CMP_CTX_free(fixture->ctx);
...
static int execute_CTX_reinit_test(OSSL_CMP_CTX_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_true(OSSL_CMP_CTX_reinit(ctx))) goto err;
...
static int execute_CTX_print_errors_test(OSSL_CMP_CTX_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_true(OSSL_CMP_CTX_set_log_cb(ctx, NULL))) res = 0;
  if (!TEST_true(ctx->log_cb == NULL)) res = 0;
  ...
  if (!TEST_true(OSSL_CMP_CTX_set_log_cb(ctx, msg_total_size_log_cb))) res = 0;
  if (!TEST_true(ctx->log_cb == msg_total_size_log_cb)) {
    ...
    if (!TEST_int_eq(msg_total_size, expected_size)) res = 0;
...
static int execute_CTX_reqExtensions_have_SAN_test(
  ...
  if (!TEST_false(OSSL_CMP_CTX_reqExtensions_have_SAN(ctx))) return 0;
...
static int execute_cmp_ctx_log_cb_test(OSSL_CMP_CTX_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_true(OSSL_CMP_CTX_set_log_cb(ctx, test_log_cb))) {
    ...
    if (!TEST_int_eq(test_log_cb_res, 1)) res = 0;
...
#define DEFINE_SET_GET_BASE_TEST(PREFIX, SETN, GETN, DUP, FIELD, TYPE, ERR,  \
  ...
  static int execute_CTX_##SETN##_##GETN##_##FIELD(                          \
    ...
    int res = 1;                                                             \
                                                                             \
    if (!TEST_int_eq(ERR_peek_error(), 0)) res = 0;                          \
    ...
    if (SETN == 0) val1_to_free = 0; /* 0 works for any type */              \
                                                                             \
    if (GETN == 1) FREE(val1_read);                                          \
    val1_read = (*get_fn)(ctx);                                              \
    if (SETN == 0) {                                                         \
      if (val1_read != val1) {                                               \
...
#define DEFINE_PUSH_BASE_TEST(PUSHN, DUP, FIELD, ELEM, TYPE, T, DEFAULT, NEW, \
  ...
  static int execute_CTX_##PUSHN##_##ELEM(                                    \
    ...
    if (PUSHN == 0) val1_to_free = 0; /* 0 works for any type */              \
                                                                              \
