...
#define STR509                                                                                                         \
    STR64 STR64 STR64 STR64 STR64 STR64 STR64 "This is a 61 bytes loooooooooooooooooooooooooooooong string.\n"
...
static int test_log_cb(const char *func, const char *file, int line, OSSL_CMP_severity level, const char *msg)
{
    test_log_cb_res =
        ...
        (TEST_int_eq(line, test_log_line_start) || TEST_int_eq(line, 0) || TEST_int_eq(line, test_log_line_end)) &&
...
#define DEFINE_SET_GET_BASE_TEST(PREFIX, SETN, GETN, DUP, FIELD, TYPE, ERR, DEFAULT, NEW, FREE)                        \
    static int execute_CTX_##SETN##_##GETN##_##FIELD(OSSL_CMP_CTX_TEST_FIXTURE *fixture)                               \
    {                                                                                                                  \
        ...
        if (!TEST_int_eq(ERR_peek_error(), 0))                                                                         \
            res = 0;                                                                                                   \
        if (PREFIX == 1)                                                                                               \
        { /* exported setter functions must test ctx == NULL */                                                        \
...
#define DEFINE_SET_GET_TEST(OSSL_CMP, CTX, N, M, DUP, FIELD, TYPE)                                                     \
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX, set##N, get##M, DUP, FIELD, TYPE *, NULL, IS_0, TYPE##_new(),           \
                             TYPE##_free)

#define DEFINE_SET_GET_SK_TEST_DEFAULT(OSSL_CMP, CTX, N, M, FIELD, ELEM_TYPE, DEFAULT, NEW, FREE)                      \
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX, set##N, get##M, 1, FIELD, STACK_OF(ELEM_TYPE) *, NULL, DEFAULT, NEW,    \
                             FREE)
#define DEFINE_SET_GET_SK_TEST(OSSL_CMP, CTX, N, M, FIELD, T)                                                          \
    DEFINE_SET_GET_SK_TEST_DEFAULT(OSSL_CMP, CTX, N, M, FIELD, T, IS_0, sk_##T##_new_null(), sk_##T##_free)
#define DEFINE_SET_GET_SK_X509_TEST(OSSL_CMP, CTX, N, M, FNAME)                                                        \
    DEFINE_SET_GET_SK_TEST_DEFAULT(OSSL_CMP, CTX, N, M, FNAME, X509, EMPTY_SK_X509, sk_X509_new_1(),                   \
                                   sk_X509_pop_X509_free)

#define DEFINE_SET_GET_TEST_DEFAULT(OSSL_CMP, CTX, N, M, DUP, FIELD, TYPE, DEFAULT)                                    \
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX, set##N, get##M, DUP, FIELD, TYPE *, NULL, DEFAULT, TYPE##_new(),        \
                             TYPE##_free)
...
#define DEFINE_SET_SK_TEST(OSSL_CMP, CTX, N, FIELD, TYPE)                                                              \
    ...
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX, set##N, get0, 1, FIELD, STACK_OF(TYPE) *, NULL, IS_0,                   \
...
#define DEFINE_SET_GET_P_VOID_TEST(FIELD)                                                                              \
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP_CTX, set, get, 0, FIELD, void *, NULL, IS_0, ((void *)1), DROP)

#define DEFINE_SET_GET_INT_TEST_DEFAULT(OSSL_CMP, CTX, FIELD, DEFAULT)                                                 \
    DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX, set, get, 0, FIELD, int, -1, DEFAULT, 1, DROP)
...
#define DEFINE_SET_GET1_STR_FN(SETN, FIELD)                                                                            \
    ...
    {                                                                                                                  \
        return OSSL_CMP_CTX_##SETN##_##FIELD(ctx, (unsigned char *)val, (int)strlen(val));                             \
    }                                                                                                                  \
                                                                                                                       \
...
#define DEFINE_PUSH_TEST(N, DUP, FIELD, ELEM, TYPE)                                                                    \
    DEFINE_PUSH_BASE_TEST(push##N, DUP, FIELD, ELEM, TYPE *, TYPE, IS_0, TYPE##_new(), TYPE##_free)
