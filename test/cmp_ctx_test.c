...
static void
tear_down(OSSL_CMP_CTX_TEST_FIXTURE* fixture)
{
  if (fixture != NULL)
...
static OSSL_CMP_CTX_TEST_FIXTURE*
...
static void
sk_X509_pop_X509_free(STACK_OF(X509)* sk)
{
  OSSL_STACK_OF_X509_free(sk);
}

static int
execute_CTX_reinit_test(OSSL_CMP_CTX_TEST_FIXTURE* fixture)
{
...
static int
msg_total_size_log_cb(const char* func,
                      const char* file,
                      int line,
...
static int
execute_CTX_print_errors_test(OSSL_CMP_CTX_TEST_FIXTURE* fixture)
{
  OSSL_CMP_CTX* ctx = fixture->ctx;
...
static int
test_CTX_reqExtensions_have_SAN(void)
{
  SETUP_TEST_FIXTURE(OSSL_CMP_CTX_TEST_FIXTURE, set_up);
...
static int
execute_cmp_ctx_log_cb_test(OSSL_CMP_CTX_TEST_FIXTURE* fixture)
{
  int res = 1;
  OSSL_CMP_CTX* ctx = fixture->ctx;
...
static BIO*
test_http_cb(BIO* bio, void* arg, int use_ssl, int detail)
{
  return NULL;
}
#endif
...
static OSSL_CMP_MSG*
test_transfer_cb(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* req)
{
  return NULL;
}
...
#define DEFINE_SET_GET_BASE_TEST(                                              \
  PREFIX, SETN, GETN, DUP, FIELD, TYPE, ERR, DEFAULT, NEW, FREE)               \
...
#define DEFINE_SET_GET_TEST(OSSL_CMP, CTX, N, M, DUP, FIELD, TYPE)             \
  DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX,                                   \
                           set##N,                                             \
                           get##M,                                             \
                           DUP,                                                \
                           FIELD,                                              \
...
#define DEFINE_SET_GET_SK_TEST_DEFAULT(                                        \
  OSSL_CMP, CTX, N, M, FIELD, ELEM_TYPE, DEFAULT, NEW, FREE)                   \
  DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX,                                   \
                           set##N,                                             \
                           get##M,                                             \
                           1,                                                  \
...
#define DEFINE_SET_GET_SK_TEST(OSSL_CMP, CTX, N, M, FIELD, T)                  \
  DEFINE_SET_GET_SK_TEST_DEFAULT(                                              \
    OSSL_CMP, CTX, N, M, FIELD, T, IS_0, sk_##T##_new_null(), sk_##T##_free)
#define DEFINE_SET_GET_SK_X509_TEST(OSSL_CMP, CTX, N, M, FNAME)                \
  DEFINE_SET_GET_SK_TEST_DEFAULT(OSSL_CMP,                                     \
                                 CTX,                                          \
                                 N,                                            \
                                 M,                                            \
                                 FNAME,                                        \
...
#define DEFINE_SET_GET_TEST_DEFAULT(                                           \
  OSSL_CMP, CTX, N, M, DUP, FIELD, TYPE, DEFAULT)                              \
  DEFINE_SET_GET_BASE_TEST(OSSL_CMP##_##CTX,                                   \
                           set##N,                                             \
                           get##M,                                             \
                           DUP,                                                \
...
#define DEFINE_SET_GET_P_VOID_TEST(FIELD)                                      \
  DEFINE_SET_GET_BASE_TEST(                                                    \
    OSSL_CMP_CTX, set, get, 0, FIELD, void*, NULL, IS_0, ((void*)1), DROP)

#define DEFINE_SET_GET_INT_TEST_DEFAULT(OSSL_CMP, CTX, FIELD, DEFAULT)         \
  DEFINE_SET_GET_BASE_TEST(                                                    \
    OSSL_CMP##_##CTX, set, get, 0, FIELD, int, -1, DEFAULT, 1, DROP)
...
#define push0 0
#define push1 1
#define DEFINE_PUSH_BASE_TEST(                                                 \
  PUSHN, DUP, FIELD, ELEM, TYPE, T, DEFAULT, NEW, FREE)                        \
...
#define DEFINE_PUSH_TEST(N, DUP, FIELD, ELEM, TYPE)                            \
  DEFINE_PUSH_BASE_TEST(                                                       \
    push##N, DUP, FIELD, ELEM, TYPE*, TYPE, IS_0, TYPE##_new(), TYPE##_free)
...
DEFINE_SET_GET_TEST_DEFAULT(OSSL_CMP,
                            CTX,
                            1,
                            1,
                            1,
...
DEFINE_SET_TEST_DEFAULT(OSSL_CMP,
                        CTX,
                        1,
                        1,
                        transactionID,
