...
static int test_punycode(int n) {
  ...
  if (!TEST_true(ossl_punycode_decode(tc->encoded, strlen(tc->encoded), buffer,
                                      &bsize)))
    return 0;
...
static int test_a2ulabel(void) {
  ...
  if (!TEST_int_eq(ossl_a2ulabel("xn--e28h.com", out, 10), 1)) return 0;
  ...
  memset(in + 4, 'e', 513);
  memcpy(in + 517, "-3ya", 4);
  if (!TEST_int_eq(ossl_a2ulabel(in, out, 50), -1)) return 0;
...
static int test_dotted_overflow(void) {
  ...
  if (!TEST_ptr(in = BUF_MEM_new())) return 0;
  if (!TEST_true(WPACKET_init_len(&p, in, 0))) {
    BUF_MEM_free(in);
    return 0;
  }
  for (i = 0; i < num_reps; i++) {
    if (i > 1 && !TEST_true(WPACKET_put_bytes_u8(&p, '.'))) goto err;
    if (!TEST_true(WPACKET_memcpy(&p, "xn--a", sizeof("xn--a") - 1))) goto err;
  }
  if (!TEST_true(WPACKET_put_bytes_u8(&p, '\0'))) goto err;
  if (!TEST_ptr(out = OPENSSL_malloc(in->length))) goto err;
