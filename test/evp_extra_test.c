...
static APK_DATA keycheckdata[] = {
  { kExampleRSAKeyDER, sizeof (kExampleRSAKeyDER), "RSA", EVP_PKEY_RSA, 1, 1,
    1, 0 },
...
#ifndef OPENSSL_NO_EC
  { kExampleECKeyDER, sizeof (kExampleECKeyDER), "EC", EVP_PKEY_EC, 1, 1, 1,
    0 },
  /* group is also associated in our pub key */
  { kExampleECPubKeyDER, sizeof (kExampleECPubKeyDER), "EC", EVP_PKEY_EC, 0, 1,
    1, 1 },
  { pExampleECParamDER, sizeof (pExampleECParamDER), "EC", EVP_PKEY_EC, 0, 0,
    1, 2 },
#ifndef OPENSSL_NO_ECX
...
static EVP_PKEY *
load_example_key (const char *keytype, const unsigned char *data,
                  size_t data_len)
{
...
test_EC_priv_pub (void)
{
  ...
  if (!TEST_int_gt (EVP_PKEY_parameters_eq (just_params, just_params), 0)
      ...
      || !TEST_int_gt (EVP_PKEY_parameters_eq (just_params, params_and_priv),
                       0)
      || !TEST_int_gt (
          EVP_PKEY_parameters_eq (just_params, params_and_keypair), 0)
  ...
  encoded = NULL;
  if (!TEST_size_t_eq (
          EVP_PKEY_get1_encoded_public_key (just_params, &encoded), 0))
    {
      OPENSSL_free (encoded);
      encoded = NULL;
      goto err;
 ...
 * instead (RSA) Test 7: Use an MD BIO to do the Update calls instead (DSA)
  ...
  if (tst % 3 != 2)
    {
      ...
      if (!TEST_int_gt (EVP_DigestVerifyFinal (md_ctx_verify, sig, sig_len),
                        0))
        goto out;
...
test_EVP_SM2_verify (void)
{
  ...
  if (!TEST_int_gt (
          EVP_DigestVerifyFinal (mctx, signature, sizeof (signature)), 0))
    goto done;
  rc = 1;
...
test_EVP_SM2 (void)
{
  ...
  if (!TEST_ptr (pctx
                 = EVP_PKEY_CTX_new_from_name (testctx, "SM2", testpropq)))
    goto done;
...
#ifndef OPENSSL_NO_ML_KEM
static int
ml_kem_seed_to_priv (const char *alg, const unsigned char *seed,
                     size_t seedlen, unsigned char **ret, size_t *retlen)
{
  ...
  if (pub)
    {
      ...
      if (uselibctx || keys[tst].name != NULL)
        {
          pkey = EVP_PKEY_new_raw_public_key_ex (testctx, name, NULL, in,
                                                 inlen);
        }
      else
        {
  ...
  else
    {
      ...
      if (uselibctx || keys[tst].name != NULL)
        {
          pkey = EVP_PKEY_new_raw_private_key_ex (testctx, name, NULL, in,
                                                  inlen);
        }
      else
        {
...
test_HKDF (void)
{
  ...
  const unsigned char expected[]
      = { 0xe5, 0x07, 0x70, 0x7f, 0xc6, 0x78, 0xd6, 0x54, 0x32, 0x5f,
...
test_emptyikm_HKDF (void)
{
  ...
  const unsigned char expected[]
      = { 0x68, 0x81, 0xa5, 0x3e, 0x5b, 0x9c, 0x7b, 0x6f, 0x2e, 0xec,
...
test_decrypt_null_chunks (void)
{
  ...
  if (!TEST_ptr (cipher
      ...
      || !TEST_true (
          EVP_EncryptUpdate (ctx, ciphertext + ctlen, &tmp, NULL, 0))
      || !TEST_int_eq (tmp, 0)
...
test_rand_agglomeration (void)
{
  ...
  *p = OSSL_PARAM_construct_end ();
  res = TEST_true (EVP_RAND_CTX_set_params (ctx, params))
        && TEST_true (
            EVP_RAND_generate (ctx, out, sizeof (out), 0, 1, NULL, 0))
...
test_EVP_rsa_pss_set_saltlen (void)
{
  ...
  ret = TEST_ptr (pkey = load_example_rsa_key ())
        ...
        && TEST_int_gt (
            EVP_PKEY_CTX_set_rsa_pss_saltlen (pkey_ctx, test_value), 0)
        && TEST_int_gt (EVP_PKEY_CTX_get_rsa_pss_saltlen (pkey_ctx, &saltlen),
                        0)
...
test_EVP_rsa_invalid_key (void)
{
  ...
  pkey = load_example_key ("RSA", kInvalidRSAKeyDER,
                           sizeof (kInvalidRSAKeyDER));
...
static const EVP_INIT_TEST_st evp_init_tests[]
    = { { "aes-128-cfb", kCFBDefaultKey, iCFBIV, cfbPlaintext, cfbCiphertext,
          NULL, 0, sizeof (cfbPlaintext), sizeof (cfbCiphertext), 0, 1, 0, 1 },
        ...
        { "aes-128-cfb", kCFBDefaultKey, iCFBIV, cfbPlaintext, cfbCiphertext,
          NULL, 0, sizeof (cfbPlaintext), sizeof (cfbCiphertext), 0, 0, 0, 1 },
        { "aes-256-gcm", kGCMDefaultKey, iGCMDefaultIV, gcmDefaultPlaintext,
          gcmDefaultCiphertext, gcmDefaultTag, sizeof (iGCMDefaultIV),
          sizeof (gcmDefaultPlaintext), sizeof (gcmDefaultCiphertext),
          sizeof (gcmDefaultTag), 0, 0, 1 },
        { "aes-128-cfb", kCFBDefaultKey, iCFBIV, cfbCiphertext, cfbPlaintext,
          NULL, 0, sizeof (cfbCiphertext), sizeof (cfbPlaintext), 0, 1, 1, 0 },
        ...
        { "aes-128-cfb", kCFBDefaultKey, iCFBIV, cfbCiphertext, cfbPlaintext,
          NULL, 0, sizeof (cfbCiphertext), sizeof (cfbPlaintext), 0, 0, 1, 0 },
        { "aes-256-gcm", kGCMDefaultKey, iGCMDefaultIV, gcmDefaultCiphertext,
          gcmDefaultPlaintext, gcmDefaultTag, sizeof (iGCMDefaultIV),
          sizeof (gcmDefaultCiphertext), sizeof (gcmDefaultPlaintext),
...
static const EVP_RESET_TEST_st evp_reset_tests[]
    = { { cfbPlaintext, cfbCiphertext, sizeof (cfbPlaintext),
          sizeof (cfbCiphertext), 1 },
        { cfbCiphertext, cfbPlaintext, sizeof (cfbCiphertext),
          sizeof (cfbPlaintext), 0 } };
...
static int
custom_pmeth_sign (EVP_PKEY_CTX *ctx, unsigned char *out, size_t *outlen,
...
static int
custom_pmeth_digestsign (EVP_MD_CTX *ctx, unsigned char *sig, size_t *siglen,
                         const unsigned char *tbs, size_t tbslen)
{
  ...
  if (doderive)
  ...
  else
    {
      ...
      if (id == EVP_PKEY_ED25519)
        {
          if (!TEST_true (
                  EVP_DigestSign (ctx, res, &reslen, msg, sizeof (msg))))
            goto err;
        }
      else
        {
...
static int
custom_ciph_init (EVP_CIPHER_CTX *ctx, const unsigned char *key,
...
test_custom_ciph_meth (void)
{
  ...
  nid = OBJ_create ("1.3.6.1.4.1.16604.998866.2", "custom-ciph",
                    "custom-ciph");
  if (!TEST_int_ne (nid, NID_undef))
    goto err;
  ...
  if (!TEST_true (EVP_CIPHER_meth_set_init (tmp, custom_ciph_init))
      || !TEST_true (
          EVP_CIPHER_meth_set_flags (tmp, EVP_CIPH_ALWAYS_CALL_INIT))
...
test_cipher_with_engine (void)
{
  ...
  if (!TEST_true (
          EVP_EncryptInit_ex (ctx, EVP_aes_128_cbc (), e, keyiv, keyiv)))
    goto err;
...
static int
aes_gcm_encrypt (const unsigned char *gcm_key, size_t gcm_key_s,
                 ...
                 const unsigned char *gcm_tag, size_t gcm_tag_s)
{
  ...
  if (!TEST_true (EVP_EncryptInit_ex2 (ctx, cipher, gcm_key, gcm_iv, params))
      || (gcm_aad != NULL
          && !TEST_true (
              EVP_EncryptUpdate (ctx, NULL, &outlen, gcm_aad, (int)gcm_aad_s)))
      || !TEST_true (
...
static int
aes_gcm_decrypt (const unsigned char *gcm_key, size_t gcm_key_s,
                 const unsigned char *gcm_iv, size_t gcm_ivlen,
                 const unsigned char *gcm_pt, size_t gcm_pt_s,
                 const unsigned char *gcm_aad, size_t gcm_aad_s,
                 const unsigned char *gcm_ct, size_t gcm_ct_s,
                 const unsigned char *gcm_tag, size_t gcm_tag_s)
{
  ...
  if (!TEST_true (EVP_DecryptInit_ex2 (ctx, cipher, gcm_key, gcm_iv, params))
      || (gcm_aad != NULL
          && !TEST_true (
              EVP_DecryptUpdate (ctx, NULL, &outlen, gcm_aad, (int)gcm_aad_s)))
      || !TEST_true (
...
test_aes_gcm_ivlen_change_cve_2023_5363 (void)
{
  ...
  return aes_gcm_encrypt (gcm_key, sizeof (gcm_key), gcm_iv, sizeof (gcm_iv),
         ...
         && aes_gcm_decrypt (gcm_key, sizeof (gcm_key), gcm_iv,
                             sizeof (gcm_iv), gcm_pt, sizeof (gcm_pt), NULL, 0,
...
#ifndef OPENSSL_NO_RC4
static int
rc4_encrypt (const unsigned char *rc4_key, size_t rc4_key_s,
...
static int
rc4_decrypt (const unsigned char *rc4_key, size_t rc4_key_s,
             const unsigned char *rc4_pt, size_t rc4_pt_s,
             const unsigned char *rc4_ct, size_t rc4_ct_s)
{
  int ret = 0;
...
test_aes_rc4_keylen_change_cve_2023_5363 (void)
{
  ...
  static const struct
  {
    ...
    unsigned char padding[11];
                ...
                { /* Random padding to 16 bytes */
                  0x80, 0xad, 0x97, 0xbd, 0xc9, 0x73, 0xdf, 0x8a, 0xaa, 0x32,
                  0x91 } };
  ...
  return rc4_encrypt (rc4_key.key, sizeof (rc4_key.key), rc4_pt,
                      sizeof (rc4_pt), rc4_ct, sizeof (rc4_ct))
         && rc4_decrypt (rc4_key.key, sizeof (rc4_key.key), rc4_pt,
                         sizeof (rc4_pt), rc4_ct, sizeof (rc4_ct));
}
#endif
          ...
          for (i = 0; i < numpipes; i++)
            {
              ...
              memset (plaintext_array[i], (unsigned char)(i + 1),
                      plaintextlen);
  ...
  while ((o = opt_next ()) != OPT_EOF)
    {
      switch (o)
        ...
        case OPT_CONTEXT:
           ...
           * against libcrypto. Instead we use a built-in version of the legacy
           * provider.
           */
