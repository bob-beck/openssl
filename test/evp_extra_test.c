...
static int test_EVP_set_default_properties(void) {
  ...
  if (!TEST_true(EVP_default_properties_enable_fips(ctx, 1))) goto err;
  fetched_properties = EVP_get1_default_properties(ctx);
  if (!TEST_ptr(fetched_properties) ||
  ...
  if (!TEST_true(EVP_default_properties_enable_fips(ctx, 0))) goto err;
...
static int test_selection(EVP_PKEY* pkey, int selection) {
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
    if (!TEST_true(ret)) goto err;
  } else {
    if (!TEST_false(ret)) goto err;
  }
...
static int test_EVP_PKEY_ffc_priv_pub(char* keytype) {
  ...
  if (!test_selection(params_and_keypair, EVP_PKEY_KEYPAIR)) goto err;
...
static int test_EC_priv_pub(void) {
   ...
   * have to actually be *valid* parameters. We just need to set something.
   */
  if (!TEST_ptr(priv = BN_bin2bn(ec_priv, sizeof(ec_priv), NULL))) goto err;
  ...
  if (!TEST_size_t_gt(
          EVP_PKEY_get1_encoded_public_key(params_and_pub, &encoded), 0))
    goto err;
  OPENSSL_free(encoded);
  encoded = NULL;
  if (!TEST_size_t_eq(EVP_PKEY_get1_encoded_public_key(just_params, &encoded),
                      0)) {
    OPENSSL_free(encoded);
    encoded = NULL;
    goto err;
  ...
  len = 0;
  if (!TEST_int_eq(EVP_PKEY_get_octet_string_param(
                       params_and_pub, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, NULL,
                       0, &len),
                   1) ||
      !TEST_size_t_eq(len, 65))
    goto err;
...
static int test_evp_get_ec_pub(void) {
  ...
  if (!test_selection(keypair, EVP_PKEY_KEYPAIR)) goto err;

  if (!EVP_PKEY_get_bn_param(keypair, OSSL_PKEY_PARAM_EC_PUB_X, &x) ||
      !EVP_PKEY_get_bn_param(keypair, OSSL_PKEY_PARAM_EC_PUB_Y, &y))
    goto err;

  if (!TEST_ptr(pad = OPENSSL_zalloc(sizeof(ec_pub)))) goto err;

  pad[0] = ec_pub[0];
  BN_bn2bin(x, &pad[1]);
  BN_bn2bin(y, &pad[33]);
  if (!TEST_true(memcmp(ec_pub, pad, sizeof(ec_pub)) == 0)) goto err;
...
static int test_EC_priv_only_legacy(void) {
  ...
  if (!TEST_ptr(eckey)) goto err;

  if (!TEST_true(EC_KEY_set_private_key(eckey, priv))) goto err;

  pkey = EVP_PKEY_new();
  if (!TEST_ptr(pkey)) goto err;

  if (!TEST_true(EVP_PKEY_assign_EC_KEY(pkey, eckey))) goto err;
  eckey = NULL;

  for (;;) {
    ret = 0;
    ctx = EVP_MD_CTX_new();
    if (!TEST_ptr(ctx)) goto err;
    ...
    if (dup_pk != NULL) break;

    if (!TEST_ptr(dup_pk = EVP_PKEY_dup(pkey))) goto err;
...
static int test_evp_get_ec_pub_legacy(void) {
  ...
  if (!TEST_ptr(libctx = OSSL_LIB_CTX_new())) goto err;
  ...
  if (!TEST_ptr(x = BN_bin2bn(&ec_pub[1], 32, NULL))) goto err;

  if (!TEST_ptr(y = BN_bin2bn(&ec_pub[33], 32, NULL))) goto err;

  if (!TEST_true(EC_KEY_set_public_key_affine_coordinates(eckey, x, y)))
    goto err;

  if (!TEST_ptr(pkey = EVP_PKEY_new())) goto err;
...
static int test_EVP_PKEY_sign(int tst) {
  ...
  if (tst == 0) {
    if (!TEST_ptr(pkey = load_example_rsa_key())) goto out;
  } else if (tst == 1) {
#ifndef OPENSSL_NO_DSA
    if (!TEST_ptr(pkey = load_example_dsa_key())) goto out;
#else
    ret = 1;
    goto out;
#endif
...
#ifndef OPENSSL_NO_EC
    if (!TEST_ptr(pkey = load_example_ec_key())) goto out;
#else
    ret = 1;
    goto out;
#endif
...
static int test_EVP_PKEY_sign_with_app_method(int tst) {
  ...
  if (tst == 0) {
    if (!TEST_ptr(pkey = load_example_rsa_key())) goto out;
    if (!TEST_ptr(rsa_meth = RSA_meth_dup(RSA_get_default_method()))) goto out;
...
#ifndef OPENSSL_NO_DSA
    if (!TEST_ptr(pkey = load_example_dsa_key())) goto out;
    if (!TEST_ptr(dsa_meth = DSA_meth_dup(DSA_get_default_method()))) goto out;
...
static int test_EVP_Enveloped(int n) {
  ...
  plaintext_len += len;
  if (!TEST_mem_eq(msg, sizeof(msg), plaintext, plaintext_len)) goto err;

  ret = 1;
err:
  if (n != 0) EVP_CIPHER_free(type);
  OPENSSL_free(kek);
  EVP_PKEY_free(keypair);
...
static int test_EVP_DigestSignInit(int tst) {
  ...
  if (tst >= 6 && tst <= 8) {
    ...
    if (!TEST_ptr(membio) || !TEST_ptr(mdbio)) goto out;
    BIO_push(mdbio, membio);
    if (!TEST_int_gt(BIO_get_md_ctx(mdbio, &md_ctx), 0)) goto out;
  } else {
  ...
  if (tst % 3 == 0) {
    ...
    if (!TEST_ptr(pkey = load_example_hmac_key())) goto out;
  }
  ...
  if (!TEST_true(EVP_DigestSignInit(md_ctx, NULL, md, NULL, pkey))) goto out;

  if (reinit && !TEST_true(EVP_DigestSignInit(md_ctx, NULL, NULL, NULL, NULL)))
    goto out;

  if (tst >= 6 && tst <= 8) {
    if (!BIO_write_ex(mdbio, kMsg, sizeof(kMsg), &written)) goto out;
  } else if (tst < 6) {
    if (!TEST_true(EVP_DigestSignUpdate(md_ctx, kMsg, sizeof(kMsg)))) goto out;
  }
  ...
  if (tst % 3 != 2) {
    ...
    if (!TEST_mem_eq(sig, sig_len, sig2, sig2_len)) goto out;
  }
...
static int test_ecdsa_digestsign_keccak(void) {
   ...
   */
  if (!TEST_true(EVP_DigestSignInit(ctx, NULL, md, NULL, pkey))) goto err;
...
static int test_siphash_digestsign(void) {
  ...
  if (!TEST_ptr(mdctx = EVP_MD_CTX_create())) goto out;

  if (!TEST_true(EVP_DigestSignInit(mdctx, &ctx, NULL, NULL, pkey))) goto out;
  if (!TEST_int_eq(EVP_PKEY_CTX_ctrl(ctx, -1, EVP_PKEY_OP_SIGNCTX,
                                     EVP_PKEY_CTRL_SET_DIGEST_SIZE, 8, NULL),
                   1))
    goto out;
  /* reinitialize */
  if (!TEST_true(EVP_DigestSignInit(mdctx, NULL, NULL, NULL, NULL))) goto out;
  if (!TEST_true(EVP_DigestSignUpdate(mdctx, buf, 8))) goto out;
  if (!TEST_true(EVP_DigestSignFinal(mdctx, digest, &len))) goto out;
  if (!TEST_mem_eq(digest, len, expected, sizeof(expected))) goto out;
...
static int test_EVP_Digest(void) {
  ...
  if (!TEST_ptr(md_ctx = EVP_MD_CTX_new())) goto out;
...
static int test_EVP_md_null(void) {
  ...
  if (!TEST_ptr(md_null) || !TEST_ptr(md_ctx = EVP_MD_CTX_new())) goto out;
  ...
  if (!TEST_uint_eq(md_len, 0)) goto out;
...
static int test_EVP_PKCS82PKEY(void) {
  ...
  if (!TEST_ptr_null(pkey = EVP_PKCS82PKEY(p8inf))) goto done;
...
static int test_EVP_PKCS82PKEY_v2(int i) {
  ...
  if (!TEST_ptr(p8inf) || !TEST_true(p == input + input_len)) goto done;
...
static int test_EVP_PKCS82PKEY_wrong_tag(void) {
  ...
  if (testctx != NULL) /* test not supported with non-default context */
    return 1;

  if (!TEST_ptr(membio = BIO_new(BIO_s_mem())) ||
      !TEST_ptr(pkey = load_example_rsa_key()) ||
      !TEST_int_gt(
          i2d_PKCS8PrivateKey_bio(membio, pkey, NULL, NULL, 0, NULL, NULL),
          0) ||
...
static int test_privatekey_to_pkcs8(void) {
  ...
  if (!TEST_ptr(membio = BIO_new(BIO_s_mem())) ||
       ...
       */
      || !TEST_int_gt(PEM_write_bio_PKCS8PrivateKey(membio, pkey, NULL, NULL, 0,
                                                    NULL, NULL),
                      0))
    goto done;
...
static int test_EVP_SM2_verify(void) {
  ...
  if (!TEST_true(bio != NULL)) goto done;

  pkey = PEM_read_bio_PUBKEY_ex(bio, NULL, NULL, NULL, testctx, testpropq);
  if (!TEST_true(pkey != NULL)) goto done;

  if (!TEST_true(EVP_PKEY_is_a(pkey, "SM2"))) goto done;

  if (!TEST_ptr(mctx = EVP_MD_CTX_new())) goto done;
  ...
  if (!TEST_ptr(sm3 = EVP_MD_fetch(testctx, "sm3", testpropq))) goto done;

  if (!TEST_true(EVP_DigestVerifyInit(mctx, NULL, sm3, NULL, pkey))) goto done;

  if (!TEST_int_gt(EVP_PKEY_CTX_set1_id(pctx, id, (int)strlen(id)), 0))
    goto done;

  if (!TEST_true(EVP_DigestVerifyUpdate(mctx, msg, strlen(msg)))) goto done;
...
static int test_EVP_SM2(void) {
  ...
  if (!TEST_true(EVP_PKEY_paramgen_init(pctx) == 1)) goto done;

  if (!TEST_int_gt(EVP_PKEY_CTX_set_ec_paramgen_curve_nid(pctx, NID_sm2), 0))
    goto done;

  if (!TEST_true(EVP_PKEY_paramgen(pctx, &pkeyparams))) goto done;

  if (!TEST_ptr(kctx =
                    EVP_PKEY_CTX_new_from_pkey(testctx, pkeyparams, testpropq)))
    goto done;

  if (!TEST_int_gt(EVP_PKEY_keygen_init(kctx), 0)) goto done;

  if (!TEST_true(EVP_PKEY_keygen(kctx, &pkey))) goto done;

  if (!TEST_ptr(md_ctx = EVP_MD_CTX_new())) goto done;

  if (!TEST_ptr(md_ctx_verify = EVP_MD_CTX_new())) goto done;
  ...
  if (!TEST_ptr(check_md = EVP_MD_fetch(testctx, "sm3", testpropq))) goto done;
  ...
  if (!TEST_true(EVP_DigestSignUpdate(md_ctx, kMsg, sizeof(kMsg)))) goto done;

  /* Determine the size of the signature. */
  if (!TEST_true(EVP_DigestSignFinal(md_ctx, NULL, &sig_len))) goto done;

  if (!TEST_ptr(sig = OPENSSL_malloc(sig_len))) goto done;

  if (!TEST_true(EVP_DigestSignFinal(md_ctx, sig, &sig_len))) goto done;
  ...
  if (!TEST_int_gt(EVP_PKEY_CTX_set1_id(sctx, NULL, 0), 0)) goto done;
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (!TEST_true(EVP_PKEY_encrypt_init(cctx))) goto done;

    if (!TEST_true(EVP_PKEY_CTX_set_params(cctx, sparams))) goto done;

    if (!TEST_true(
            EVP_PKEY_encrypt(cctx, ciphertext, &ctext_len, kMsg, sizeof(kMsg))))
      goto done;

    if (!TEST_int_gt(EVP_PKEY_decrypt_init(cctx), 0)) goto done;
    ...
    if (!TEST_true(EVP_PKEY_CTX_get_params(cctx, gparams))) goto done;
    ...
    if (!TEST_true(ptext_len == sizeof(kMsg))) goto done;

    if (!TEST_true(memcmp(plaintext, kMsg, sizeof(kMsg)) == 0)) goto done;
  }
...
static int ml_kem_seed_to_priv(const char* alg, const unsigned char* seed,
                               size_t seedlen, unsigned char** ret,
                               size_t* retlen) {
  ...
  if (!TEST_ptr(ctx) || !TEST_int_gt(EVP_PKEY_fromdata_init(ctx), 0)) goto done;
...
static int test_set_get_raw_keys_int(int tst, int pub, int uselibctx) {
  ...
  if (pub && keys[tst].pub == NULL) return 1;
  ...
  if (!TEST_ptr(buf = OPENSSL_zalloc(len <= 80 ? 80 : len))) goto done;
...
static int test_EVP_PKEY_check(int i) {
  ...
  if (!TEST_ptr(pkey = load_example_key(ak->keytype, input, input_len)))
    goto done;
  if (type == 0 && !TEST_int_eq(EVP_PKEY_get_id(pkey), expected_id)) goto done;

  if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_pkey(testctx, pkey, testpropq)))
    goto done;

  if (!TEST_int_eq(EVP_PKEY_check(ctx), expected_check)) goto done;

  if (!TEST_int_eq(EVP_PKEY_public_check(ctx), expected_pub_check)) goto done;

  if (!TEST_int_eq(EVP_PKEY_param_check(ctx), expected_param_check)) goto done;

#ifndef OPENSSL_NO_DEPRECATED_3_0
  ctx2 = EVP_PKEY_CTX_new_id(0xdefaced, NULL);
  /* assign the pkey directly, as an internal test */
  if (!EVP_PKEY_up_ref(pkey)) goto done;

  ctx2->pkey = pkey;

  if (!TEST_int_eq(EVP_PKEY_check(ctx2), 0xbeef)) goto done;

  if (!TEST_int_eq(EVP_PKEY_public_check(ctx2), 0xbeef)) goto done;

  if (!TEST_int_eq(EVP_PKEY_param_check(ctx2), 0xbeef)) goto done;
#endif
...
static int test_CMAC_keygen(void) {
  ...
  if (!TEST_int_gt(EVP_PKEY_keygen_init(kctx), 0) ||
      !TEST_int_gt(
          EVP_PKEY_CTX_ctrl(kctx, -1, EVP_PKEY_OP_KEYGEN, EVP_PKEY_CTRL_CIPHER,
                            0, (void*)EVP_aes_256_cbc()),
          0) ||
      !TEST_int_gt(
          EVP_PKEY_CTX_ctrl(kctx, -1, EVP_PKEY_OP_KEYGEN,
                            EVP_PKEY_CTRL_SET_MAC_KEY, sizeof(key), (void*)key),
          0) ||
...
static int test_X509_PUBKEY_inplace(void) {
  ...
  if (!TEST_ptr(xp)) goto done;
  if (!TEST_ptr(d2i_X509_PUBKEY(&xp, &p, (long)input_len))) goto done;

  if (!TEST_ptr(X509_PUBKEY_get0(xp))) goto done;

  p = kExampleBadECPubKeyDER;
  input_len = sizeof(kExampleBadECPubKeyDER);

  if (!TEST_ptr(xp = d2i_X509_PUBKEY(&xp, &p, (long)input_len))) goto done;

  if (!TEST_true(X509_PUBKEY_get0(xp) == NULL)) goto done;
...
static int test_X509_PUBKEY_dup(void) {
  ...
  if (!TEST_ptr(xp) || !TEST_ptr(d2i_X509_PUBKEY(&xp, &p, (long)input_len)) ||
  ...
  X509_PUBKEY_free(xp);
  xp = NULL;
  if (!TEST_true(X509_PUBKEY_get0(xq) == NULL)) goto done;
...
static int test_EVP_PKEY_CTX_get_set_params(EVP_PKEY* pkey) {
  ...
  if (!TEST_ptr(ctx) || !TEST_int_gt(EVP_PKEY_sign_init(ctx), 0)) goto err;
  ...
  *param++ = OSSL_PARAM_construct_end();
  ...
  if (!TEST_true(EVP_PKEY_CTX_set_params(ctx, ourparams))) goto err;
  ...
  *param++ = OSSL_PARAM_construct_end();
  ...
  if (!TEST_true(EVP_MD_CTX_set_params(mdctx, ourparams))) goto err;
...
static int test_DSA_get_set_params(void) {
  ...
  if (!TEST_true(OSSL_PARAM_BLD_push_BN(bld, OSSL_PKEY_PARAM_FFC_P, p)) ||
      ...
      !TEST_true(OSSL_PARAM_BLD_push_BN(bld, OSSL_PKEY_PARAM_PRIV_KEY, priv)))
    goto err;
  if (!TEST_ptr(params = OSSL_PARAM_BLD_to_param(bld))) goto err;
...
static int test_RSA_OAEP_set_get_params(void) {
...

  {
    ...
    if (!TEST_int_gt(EVP_PKEY_encrypt_init_ex(key_ctx, params), 0)) goto err;
  }
  {
    OSSL_PARAM params[3];
    char oaepmd[30] = {'\0'};
    ...
    if (!TEST_true(EVP_PKEY_CTX_get_params(key_ctx, params))) goto err;
...
static int test_RSA_OAEP_set_null_label(void) {
  ...
  if (!TEST_ptr(label = OPENSSL_strdup("foo"))) goto err;
  ...
  if (!TEST_true(EVP_PKEY_CTX_set0_rsa_oaep_label(key_ctx, NULL, 0))) goto err;
...
static int test_RSA_legacy(void) {
  ...
  if (!TEST_true(RSA_set0_factors(rsa, p, q))) goto err;
  p = NULL;
  q = NULL;

  if (!TEST_true(RSA_set0_key(rsa, n, e, d))) goto err;
  n = NULL;
  e = NULL;
  d = NULL;

  if (!TEST_true(EVP_PKEY_assign_RSA(pkey, rsa))) goto err;
...
static int test_decrypt_null_chunks(void) {
  ...
  if (!TEST_ptr(
          cipher = EVP_CIPHER_fetch(testctx, "ChaCha20-Poly1305", testpropq)) ||
      !TEST_ptr(ctx = EVP_CIPHER_CTX_new()) ||
...
static int test_keygen_with_empty_template(int n) {
  ...
  switch (n) {
    case 0:
      ...
      if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_id(EVP_PKEY_RSA, NULL))) goto err;
      break;
    case 1:
...
static int test_pkey_ctx_fail_without_provider(int tst) {
  ...
  if (!TEST_ptr(tmpctx)) goto err;

  tmpnullprov = OSSL_PROVIDER_load(tmpctx, "null");
  if (!TEST_ptr(tmpnullprov)) goto err;
  ...
  if (expect_null ? !TEST_ptr_null(pctx) : !TEST_ptr(pctx)) goto err;
...
static int test_evp_iv_aes(int idx) {
  ...
  if (!TEST_size_t_gt(ivlen, 0)) goto err;
  ...
  if (!TEST_true(EVP_EncryptInit_ex(ctx, NULL, NULL, NULL, NULL)) ||
    ...
    goto err;
  if (iv_reset) {
    if (!TEST_mem_eq(init_iv, ivlen, iv, ivlen)) goto err;
  } else {
    if (!TEST_mem_eq(ref_iv, ivlen, iv, ivlen)) goto err;
  }

  ret = 1;
err:
  EVP_CIPHER_CTX_free(ctx);
  if (idx >= 6) EVP_CIPHER_free((EVP_CIPHER*)type);
  return ret;
}
...
static int test_ecpub(int idx) {
  ...
  savelen = len;
  if (!TEST_int_ge(len, 1) || !TEST_int_lt(len, 1024)) goto done;
  p = buf;
  len = i2d_PublicKey(pkey, &p);
  if (!TEST_int_ge(len, 1) || !TEST_int_eq(len, savelen)) goto done;
  ...
  ec = NULL;
  if (!TEST_ptr(d2i_PublicKey(EVP_PKEY_EC, &pkey2, &q, savelen))) goto done;
  /* The keys should match. */
  if (!TEST_int_eq(EVP_PKEY_eq(pkey, pkey2), 1)) goto done;
#endif
...
static int test_names_do_all(void) {
  ...
  if (!TEST_ptr(sha256)) goto err;

  /*
   * We loop through all the names for a given digest. This should still work
   * even if the namemap changes part way through.
   */
  if (!TEST_true(EVP_MD_names_do_all(sha256, md_names, ctx))) goto err;

  if (!TEST_true(success)) goto err;
...
static int evp_init_seq_set_iv(EVP_CIPHER_CTX* ctx, const EVP_INIT_TEST_st* t) {
  ...
  if (!TEST_true(EVP_CipherInit_ex(ctx, NULL, NULL, NULL, t->iv, -1))) goto err;
  res = 1;
err:
  return res;
}
...
static int test_evp_init_seq(int idx) {
  ...
  if (!TEST_true(
          EVP_CipherUpdate(ctx, outbuf, &outlen1, t->input, (int)t->inlen))) {
  ...
  if (t->finalenc != 0 && t->tag != NULL) {
    if (!TEST_int_gt(
            EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_AEAD_GET_TAG, (int)taglen, tag),
            0)) {
      errmsg = "GET_TAG";
      goto err;
    }
  ...
  testresult = 1;
err:
  if (errmsg != NULL) TEST_info("evp_init_test %d: %s", idx, errmsg);
  EVP_CIPHER_CTX_free(ctx);
  EVP_CIPHER_free(type);
  return testresult;
}
...
static int test_evp_reinit_seq(int idx) {
  ...
  if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new()) ||
      ...
      /* second iteration */
      || !TEST_true(EVP_CipherUpdate(ctx, outbuf2, &outlen2, t->input,
                                     (int)t->inlen)) ||
...
static int test_evp_reset(int idx) {
  ...
  testresult = 1;
err:
  if (errmsg != NULL) TEST_info("test_evp_reset %d: %s", idx, errmsg);
  EVP_CIPHER_CTX_free(ctx);
  EVP_CIPHER_free(type);
  return testresult;
}
...
ok:
  testresult = 1;
err:
  if (errmsg != NULL) TEST_info("test_evp_updated_iv %d: %s", idx, errmsg);
  EVP_CIPHER_CTX_free(ctx);
  EVP_CIPHER_free(type);
  return testresult;
}
...
static int test_ivlen_change(int idx) {
  ...
  if (!TEST_ptr(ctx = EVP_CIPHER_CTX_new())) goto err;
  ...
  if (!TEST_true(EVP_CIPHER_CTX_set_params(ctx, params))) goto err;
...
static int test_keylen_change(int idx) {
  ...
  if (!TEST_true(EVP_CipherInit_ex(ctx, ciph, NULL, key, NULL, 1))) goto err;
...
static int test_custom_pmeth(int idx) {
  ...
  if (idx < 6) {
    if (!TEST_true(evp_pkey_is_provided(pkey))) goto err;
  } else {
    EVP_PKEY* tmp = pkey;
    ...
    EVP_PKEY_free(tmp);
    if (!TEST_true(evp_pkey_is_legacy(pkey))) goto err;
  }
  ...
  if (!TEST_true(EVP_PKEY_meth_add0(custom_pmeth))) goto err;
...
err:
  OPENSSL_free(res);
  EVP_MD_CTX_free(ctx);
  if (doderive) EVP_PKEY_CTX_free(pctx);
  EVP_PKEY_free(pkey);
...
static int test_evp_md_cipher_meth(void) {
  ...
  if (!TEST_ptr(md) || !TEST_ptr(ciph)) goto err;
...
static int test_custom_md_meth(void) {
   ...
   */
  if (testctx != NULL) return TEST_skip("Non-default libctx");

  custom_md_init_called = custom_md_cleanup_called = 0;

  nid = OBJ_create("1.3.6.1.4.1.16604.998866.1", "custom-md", "custom-md");
  if (!TEST_int_ne(nid, NID_undef)) goto err;
  if (!TEST_int_eq(OBJ_txt2nid("1.3.6.1.4.1.16604.998866.1"), nid)) goto err;
  tmp = EVP_MD_meth_new(nid, NID_undef);
  if (!TEST_ptr(tmp)) goto err;
  ...
  o = ASN1_OBJECT_create(
      ...
      "custom-md", "custom-md");
  if (!TEST_int_eq(OBJ_add_object(o), nid)) goto err;
...
static int test_custom_ciph_meth(void) {
  ...
  if (!TEST_int_ne(nid, NID_undef)) goto err;
  if (!TEST_int_eq(OBJ_txt2nid("1.3.6.1.4.1.16604.998866.2"), nid)) goto err;
...
static int test_signatures_with_engine(int tst) {
  ...
  if (!TEST_ptr(ctx = EVP_MD_CTX_new())) goto err;
  ...
  if (tst == 0) {
    ...
    if (!TEST_ptr(mac = OPENSSL_malloc(maclen))) goto err;

    if (!TEST_true(EVP_DigestSignFinal(ctx, mac, &maclen))) goto err;
  } else {
...
static int test_cipher_with_engine(void) {
  ...
  if (!TEST_true(EVP_CIPHER_CTX_copy(ctx2, ctx))) goto err;
...
static int test_ecx_not_private_key(int tst) {
  ...
  /* Check if this algorithm supports public keys */
  if (keys[tst].pub == NULL) return TEST_skip("no public key present");
  ...
  if (EVP_DigestSignInit(ctx, NULL, NULL, NULL, pkey) != 1) goto check_err;

  if (EVP_DigestSign(ctx, NULL, &maclen, msg, sizeof(msg)) != 1) goto check_err;
...
static int test_sign_continuation(void) {
  ...
  if (!TEST_ptr(fake_rsa = fake_rsa_start(testctx))) return 0;
...
static int aes_gcm_encrypt(const unsigned char* gcm_key, size_t gcm_key_s,
  ...
  if (!TEST_true(EVP_EncryptInit_ex2(ctx, cipher, gcm_key, gcm_iv, params)) ||
      (gcm_aad != NULL && !TEST_true(EVP_EncryptUpdate(
                              ctx, NULL, &outlen, gcm_aad, (int)gcm_aad_s))) ||
      !TEST_true(
          EVP_EncryptUpdate(ctx, outbuf, &outlen, gcm_pt, (int)gcm_pt_s)) ||
...
static int aes_gcm_decrypt(const unsigned char* gcm_key, size_t gcm_key_s,
  ...
  if ((cipher = EVP_CIPHER_fetch(testctx, "AES-256-GCM", "")) == NULL) goto err;
  ...
  if (!TEST_true(EVP_DecryptInit_ex2(ctx, cipher, gcm_key, gcm_iv, params)) ||
      (gcm_aad != NULL && !TEST_true(EVP_DecryptUpdate(
                              ctx, NULL, &outlen, gcm_aad, (int)gcm_aad_s))) ||
      !TEST_true(
          EVP_DecryptUpdate(ctx, outbuf, &outlen, gcm_ct, (int)gcm_ct_s)) ||
...
static int rc4_encrypt(const unsigned char* rc4_key, size_t rc4_key_s,
  ...
  if (!TEST_true(EVP_EncryptInit_ex2(ctx, cipher, rc4_key, NULL, params)) ||
      !TEST_true(
          EVP_EncryptUpdate(ctx, outbuf, &outlen, rc4_pt, (int)rc4_pt_s)) ||
      !TEST_true(EVP_EncryptFinal_ex(ctx, outbuf, &tmplen)))
    goto err;

  if (!TEST_mem_eq(outbuf, outlen, rc4_ct, rc4_ct_s)) goto err;
...
static int rc4_decrypt(const unsigned char* rc4_key, size_t rc4_key_s,
  ...
  if ((cipher = EVP_CIPHER_fetch(testctx, "RC4", "")) == NULL) goto err;

  params[0] = OSSL_PARAM_construct_size_t(OSSL_CIPHER_PARAM_KEYLEN, &rc4_key_s);

  if (!TEST_true(EVP_DecryptInit_ex2(ctx, cipher, rc4_key, NULL, params)) ||
      !TEST_true(
          EVP_DecryptUpdate(ctx, outbuf, &outlen, rc4_ct, (int)rc4_ct_s)) ||
...
static int test_evp_cipher_pipeline(void) {
  ...
  if (!TEST_ptr(fake_pipeline = fake_pipeline_start(testctx))) return 0;
  ...
  /* Negative tests */
  if (!TEST_false(EVP_CIPHER_can_pipeline(cipher, 1))) goto end;
  if (!TEST_false(EVP_CIPHER_can_pipeline(EVP_aes_256_gcm(), 1))) goto end;
  ...
  for (numpipes = 1; numpipes <= EVP_MAX_PIPES; numpipes++) {
    for (plaintextlen = 1; plaintextlen <= 256; plaintextlen++) {
      ...
      for (i = 0; i < numpipes; i++) ciphertextlen_array[i] += outlen_array[i];

      params[0] = OSSL_PARAM_construct_octet_ptr(
          OSSL_CIPHER_PARAM_PIPELINE_AEAD_TAG, (void**)&aead_tags, taglen);
      if (!TEST_true(EVP_CIPHER_CTX_get_params(ctx, params))) goto err;

      /* Encrypt using non-pipeline API and compare */
      if (!TEST_true(EVP_CIPHER_CTX_reset(ctx))) goto err;
...
int setup_tests(void) {
  ...
  if (!TEST_ptr(custom_pmeth)) return 0;
  ...
  if (!TEST_int_eq(EVP_PKEY_meth_add0(custom_pmeth), 1)) return 0;
#endif
