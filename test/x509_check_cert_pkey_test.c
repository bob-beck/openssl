...
static int test_x509_check_cert_pkey(void) {
  ...
  /* process private key */
  if (!TEST_ptr(bio = BIO_new_file(k, "r"))) goto failed;
  ...
  if (!TEST_ptr(bio = BIO_new_file(c, "r"))) goto failed;
...
static int test_PEM_X509_INFO_read_bio(void) {
  ...
  if (!TEST_ptr((in = BIO_new_file(file, "r")))) return 0;
  sk = PEM_X509_INFO_read_bio(in, NULL, NULL, "");
  BIO_free(in);
  for (i = 0; i < sk_X509_INFO_num(sk); i++) {
    it = sk_X509_INFO_value(sk, i);
    if (it->x509 != NULL) count++;
    if (it->crl != NULL) count++;
    if (it->x_pkey != NULL) count++;
  }
...
int setup_tests(void) {
  ...
  if (test_get_argument_count() == 2) {
    ...
    if (!TEST_ptr(file = test_get_argument(0)) ||
      ...
      return 0;
    if (!TEST_int_eq(sscanf(num, "%d", &expected), 1)) return 0;
