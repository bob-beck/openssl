...
static int set_cn(X509* crt, ...) {
  ...
  if (!X509_set_subject_name(crt, n)) goto out;
  ret = 1;
out:
  X509_NAME_free(n);
  va_end(ap);
...
static int set_altname(X509* crt, ...) {
  ...
  while (1) {
    ...
    if (gen == NULL) goto out;
    ia5 = ASN1_IA5STRING_new();
    if (ia5 == NULL) goto out;
    if (!ASN1_STRING_set(ia5, name, -1)) goto out;
    switch (type) {
      case GEN_EMAIL:
      case GEN_DNS:
    ...
    if (!sk_GENERAL_NAME_push(gens, gen)) goto out;
    gen = NULL;
  }
  if (!X509_add1_ext_i2d(crt, NID_subject_alt_name, gens, 0, 0)) goto out;
  ret = 1;
out:
...
static X509* make_cert(void) {
  ...
  if (!TEST_ptr(crt = X509_new())) return NULL;
...
static int run_cert(X509* crt, const char* nameincert,
  ...
  for (; *pname != NULL; ++pname) {
    ...
    if (!TEST_int_ge(ret = X509_check_host(crt, name, namelen, 0, NULL), 0)) {
      ...
      if (ret == 1 && !samename) match = 1;
      if (ret == 0 && samename) match = 0;
    } else if (ret == 1)
      match = 1;
    ...
    if (fn->email) {
      if (ret && !samename) match = 1;
      if (!ret && samename && strchr(nameincert, '@') != NULL) match = 0;
    } else if (ret)
      match = 1;
...
static int test_GENERAL_NAME_cmp(void) {
  ...
  if (!TEST_ptr(namesa) || !TEST_ptr(namesb)) goto end;
  ...
  for (i = 0; i < OSSL_NELEM(gennames); i++) {
    ...
    if (!TEST_ptr(namesa[i]) || !TEST_ptr(namesb[i])) goto end;
  }
  ...
  for (i = 0; i < OSSL_NELEM(gennames); i++) {
    for (j = 0; j < OSSL_NELEM(gennames); j++) {
      if (i == j) {
        if (!TEST_int_eq(GENERAL_NAME_cmp(namesa[i], namesb[j]), 0)) goto end;
      } else {
        if (!TEST_int_ne(GENERAL_NAME_cmp(namesa[i], namesb[j]), 0)) goto end;
      }
    }
  }
  testresult = 1;
  ...
  for (i = 0; i < OSSL_NELEM(gennames); i++) {
    if (namesa != NULL) GENERAL_NAME_free(namesa[i]);
    if (namesb != NULL) GENERAL_NAME_free(namesb[i]);
  }
  OPENSSL_free(namesa);
  OPENSSL_free(namesb);
