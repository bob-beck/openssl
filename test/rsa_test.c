...
#define SetKey                                                      \
  ...
  if (c != NULL) memcpy(c, ctext_ex, sizeof(ctext_ex) - 1);         \
...
static int test_rsa_simple(int idx, int en_pad_type, int de_pad_type,
  ...
  if (clen != NULL) *clen = clentmp;

  num = RSA_public_encrypt(plen, ptext_ex, ctext, key, en_pad_type);
  if (!TEST_int_eq(num, clentmp)) goto err;
...
static int test_rsa_oaep(int idx) {
  ...
  if (num <= 0 || !TEST_mem_eq(ptext, num, ptext_ex, plen)) goto err;
...
static int test_rsa_security_bit(int n) {
  ...
  if (!TEST_ptr(key) || !TEST_int_le(bytes, (int)sizeof(num))) goto err;
...
static RSA* load_key(int priv) {
  ...
  if (!TEST_ptr(rsa = RSA_new())) return NULL;
  pn = BN_bin2bn(n, sizeof(n), NULL);
  pe = BN_bin2bn(e, sizeof(e), NULL);
  if (priv) pd = BN_bin2bn(d, sizeof(d), NULL);
...
static int test_rsa_saos(void) {
  ...
  if (!TEST_ptr(rsa_priv = load_key(1))) goto err;
  if (!TEST_ptr(rsa_pub = load_key(0))) goto err;
  if (!TEST_int_ge((int)sizeof(sig), RSA_size(rsa_priv))) goto err;
