...
static int dh_test(void) {
  ...
  if (!TEST_true(DH_check(dh, &i))) goto err2;
  ...
  if (!TEST_false(DH_generate_key(dh))) goto err3;
  ...
  if (!TEST_true(DH_check(dh, &i))) goto err3;
   ...
   */
  if (!TEST_false(DH_check(dh, &i))) goto err3;
  ...
  /* generate a DH group ... */
  if (!TEST_ptr(_cb = BN_GENCB_new())) goto err3;
  ...
  if (!TEST_true(DH_check(a, &i))) goto err3;
  if (!TEST_false(i & DH_CHECK_P_NOT_PRIME) ||
      !TEST_false(i & DH_CHECK_P_NOT_SAFE_PRIME) ||
      !TEST_false(i & DH_UNABLE_TO_CHECK_GENERATOR) ||
      !TEST_false(i & DH_NOT_SUITABLE_GENERATOR) ||
  ...
  if (!TEST_ptr(b = DH_new())) goto err3;
  ...
  if (!DH_generate_key(a)) goto err3;
  DH_get0_key(a, &apub_key, NULL);

  if (!DH_generate_key(b)) goto err3;
...
static int dh_computekey_range_test(void) {
  ...
  /* Test that z == 1 fails */
  if (!TEST_int_le(ossl_dh_compute_key(buf, pub, dh), 0)) goto err;
  /* Test that z == 0 fails */
...
static int rfc5114_test(void) {
  ...
  for (i = 0; i < (int)OSSL_NELEM(rfctd); i++) {
    ...
    if (!TEST_int_ne(DH_compute_key(Z1, pub_key_tmp, dhA), -1)) goto bad_err;

    DH_get0_key(dhA, &pub_key_tmp, NULL);
    if (!TEST_int_ne(DH_compute_key(Z2, pub_key_tmp, dhB), -1)) goto bad_err;
...
static int rfc7919_test(void) {
  ...
  if (!TEST_ptr(a = DH_new_by_nid(NID_ffdhe2048))) goto err;
  ...
  /* now create another copy of the DH group for the peer */
  if (!TEST_ptr(b = DH_new_by_nid(NID_ffdhe2048))) goto err;
  ...
  if (!TEST_true(aout >= 20) || !TEST_mem_eq(abuf, aout, bbuf, bout)) goto err;
...
static int dh_test_prime_groups(int index) {
  ...
  if (!TEST_ptr(dh = DH_new_by_nid(prime_groups[index]))) goto err;
  DH_get0_pqg(dh, &p, &q, &g);
  if (!TEST_ptr(p) || !TEST_ptr(q) || !TEST_ptr(g)) goto err;

  if (!TEST_int_eq(DH_get_nid(dh), prime_groups[index])) goto err;

  /* Since q is set there is no need for the private length to be set */
  if (!TEST_int_eq((int)DH_get_length(dh), 0)) goto err;
...
static int dh_rfc5114_fix_nid_test(void) {
  ...
  if (!TEST_ptr(paramgen_ctx)) goto err;
  if (!TEST_int_eq(EVP_PKEY_paramgen_init(paramgen_ctx), 1)) goto err;
  /* Tested function is called here */
  if (!TEST_int_eq(EVP_PKEY_CTX_set_dhx_rfc5114(paramgen_ctx, 3), 1)) goto err;
  /* Negative test */
  if (!TEST_int_eq(EVP_PKEY_CTX_set_dhx_rfc5114(paramgen_ctx, 99), 0)) goto err;
...
static int dh_get_nid(void) {
  ...
  if (!TEST_ptr(dh1) || !TEST_ptr(dh2)) goto err;
  ...
  if (!TEST_true(DH_set0_pqg(dh2, pcpy, NULL, gcpy))) goto err;
  pcpy = gcpy = NULL;
  /* Test q is set if p and g are provided */
  if (!TEST_ptr(DH_get0_q(dh2))) goto err;

  /* Test that setting p & g manually returns that it is a named group */
  if (!TEST_int_eq(DH_get_nid(dh2), NID_ffdhe2048)) goto err;

  /* Test that after changing g it is no longer a named group */
  if (!TEST_ptr(gcpy = BN_dup(BN_value_one()))) goto err;
  if (!TEST_true(DH_set0_pqg(dh2, NULL, NULL, gcpy))) goto err;
  gcpy = NULL;
  if (!TEST_int_eq(DH_get_nid(dh2), NID_undef)) goto err;
...
static int dh_load_pkcs3_namedgroup_privlen_test(void) {
  ...
  ret = TEST_ptr(pkey =
                     d2i_PUBKEY_ex(NULL, &p, sizeof(dh_pub_der), NULL, NULL)) &&
