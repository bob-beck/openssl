...
static int execute_exec_certrequest_ses_test(CMP_SES_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_ptr(res) || !TEST_int_eq(X509_cmp(res, client_cert), 0)) return 0;
...
static int test_exec_RR_ses(int request_error) {
  ...
  if (request_error) OSSL_CMP_CTX_set1_oldCert(fixture->cmp_ctx, NULL);
  fixture->expected =
...
static int test_exec_KUR_ses(int transfer_error, int pubkey, int raverified) {
  ...
  if (transfer_error) OSSL_CMP_CTX_set_transfer_cb_arg(fixture->cmp_ctx, NULL);
  if (pubkey) {
