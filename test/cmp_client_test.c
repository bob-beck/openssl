...
static int
test_exec_REQ_ses_poll (int req_type, int check_after, int poll_count,
...
static int
test_certConf_cb (OSSL_CMP_CTX *ctx, X509 *cert, int fail_info,
                  const char **txt)
{
...
execute_try_certreq_poll_test (CMP_SES_TEST_FIXTURE *fixture)
{
  ...
  return TEST_int_eq (-1, OSSL_CMP_try_certreq (ctx, TYPE, NULL, &check_after))
         ...
         && TEST_int_eq (
             0, X509_cmp (OSSL_CMP_CTX_get0_newCert (ctx), client_cert));
}
...
execute_exchange_error_test (CMP_SES_TEST_FIXTURE *fixture)
{
  int res = ossl_cmp_exchange_error (
      ...
      1 << OSSL_CMP_PKIFAILUREINFO_unsupportedVersion, "foo_status", 999,
      "foo_details");
