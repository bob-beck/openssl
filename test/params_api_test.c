...
static void
swap_copy(unsigned char* out, const void* in, size_t len)
{
  size_t j;
 ...
 */
static void
le_copy(unsigned char* out, size_t outlen, const void* in, size_t inlen)
{
  DECLARE_IS_ENDIAN;
...
static int
test_param_type_null(OSSL_PARAM* param)
{
  int rc = 0;
  uint64_t intval;
  double dval;
...
static int
test_param_type_extra(OSSL_PARAM* param, const unsigned char* cmp, size_t width)
{
  int32_t i32;
  int64_t i64;
  size_t s, sz;
...
test_param_uint(int n)
{
  ...
  if (!TEST_mem_eq(cmp, sizeof(in), raw_values[n].value, sizeof(in)))
    return 0;
  param.data = &out;
  return test_param_type_extra(
    &param, raw_values[n].value, sizeof(unsigned int));
}

static int
test_param_ulong(int n)
{
  ...
  param.data = &out;
  return test_param_type_extra(
    &param, raw_values[n].value, sizeof(unsigned long int));
}

static int
test_param_int32(int n)
{
  int32_t in, out;
...
static int
test_param_signed_bignum(int n)
{
  unsigned char buf[MAX_LEN], bnbuf[MAX_LEN];
  const size_t len = raw_values[n].len;
  BIGNUM *b = NULL, *c = NULL;
...
static int
test_param_construct(int tstid)
{
