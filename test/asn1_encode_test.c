...
static size_t make_custom_der(const TEST_CUSTOM_DATA* custom_data,
  ...
  for (i = custom_data->nbytes2; i > 0; i--) {
    if (custom_data->bytes2[i - 1] != '\0') break;
  }
...
#define DATA_BUF_SIZE 256
  ...
  if ((o = OPENSSL_malloc(DATA_BUF_SIZE)) == NULL) return 0;
...
ASN1_SEQUENCE(INVALIDTEMPLATE) =
    {
         ...
         * but we deliberately use implicit here, which makes this template
         * invalid.
         */
