 ...
 */
typedef int
i2d_fn(void* a, unsigned char** pp);
typedef void*
d2i_fn(void** a, unsigned char** pp, long length);
typedef void
ifree_fn(void* a);
typedef struct
{
...
ASN1_SEQUENCE(
  ASN1_INT32_DATA) = { ASN1_SIMPLE(ASN1_INT32_DATA, success, ASN1_BOOLEAN),
...
ASN1_SEQUENCE(
  ASN1_UINT32_DATA) = { ASN1_SIMPLE(ASN1_UINT32_DATA, success, ASN1_BOOLEAN),
...
ASN1_SEQUENCE(
  ASN1_INT64_DATA) = { ASN1_SIMPLE(ASN1_INT64_DATA, success, ASN1_BOOLEAN),
...
ASN1_SEQUENCE(
  ASN1_UINT64_DATA) = { ASN1_SIMPLE(ASN1_UINT64_DATA, success, ASN1_BOOLEAN),
...
static int
do_enc_dec(EXPECTED* bytes, long nbytes, const TEST_PACKAGE* package)
{
  unsigned char* data = NULL;
  int len;
  int ret = 0;
...
static size_t
der_encode_length(size_t len, unsigned char** pp)
{
  size_t lenbytes;
...
static size_t
make_custom_der(const TEST_CUSTOM_DATA* custom_data,
...
static int
do_decode_custom(const TEST_CUSTOM_DATA* custom_data,
                 const EXPECTED* expected,
                 size_t expected_size,
                 const TEST_PACKAGE* package)
{
  ...
  ret = do_decode(
    encoding, (long)encoding_length, expected, expected_size, package);
...
static int
do_encode_custom(EXPECTED* input,
...
static int
do_print_item(const TEST_PACKAGE* package)
{
...
static int
test_intern(const TEST_PACKAGE* package)
{
  unsigned int i;
  size_t nelems;
  int fail = 0;
  ...
  for (i = 0; i < nelems; i++) {
    ...
    switch (do_encode_custom(expected, &test_custom_data[i], package)) {
      case -1:
        if (expected->success) {
          TEST_error(
            "Failed custom encode round trip %u of %s", i, package->name);
          TEST_openssl_errors();
          fail++;
        }
        break;
      case 0:
        TEST_error(
          "Custom encode round trip %u of %s mismatch", i, package->name);
        TEST_openssl_errors();
        fail++;
        break;
      case 1:
      ...
      default:
        OPENSSL_die(
          "do_encode_custom() return unknown value", __FILE__, __LINE__);
    }
    switch (do_decode_custom(&test_custom_data[i],
      ...
      case -1:
        if (expected->success) {
          TEST_error(
            "Failed custom decode round trip %u of %s", i, package->name);
          TEST_openssl_errors();
          fail++;
        }
        break;
      case 0:
        TEST_error(
          "Custom decode round trip %u of %s mismatch", i, package->name);
        TEST_openssl_errors();
        fail++;
        break;
      case 1:
      ...
      default:
        OPENSSL_die(
          "do_decode_custom() return unknown value", __FILE__, __LINE__);
    }
  }
  ...
  for (i = 0; i < nelems; i++) {
    ...
    switch (
      do_enc_dec(expected, (long)package->encdec_data_elem_size, package)) {
      case -1:
        if (expected->success) {
          TEST_error(
            "Failed encode/decode round trip %u of %s", i, package->name);
          TEST_openssl_errors();
          fail++;
        }
        break;
      case 0:
        TEST_error(
          "Encode/decode round trip %u of %s mismatch", i, package->name);
        fail++;
        break;
      case 1:
        break;
