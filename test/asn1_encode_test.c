 ...
 */
static int
do_decode (unsigned char *bytes, long nbytes, const EXPECTED *expected,
 ...
 */
static int
do_encode (EXPECTED *input, const unsigned char *expected, size_t expected_len,
...
static size_t
make_custom_der (const TEST_CUSTOM_DATA *custom_data, unsigned char **encoding,
                 int explicit_default)
{
  ...
  seqbytes
      = 1
        + der_encode_length (sizeof (t_true) + firstbytes + secondbytes, NULL)
...
static int
do_encode_custom (EXPECTED *input, const TEST_CUSTOM_DATA *custom_data,
                  const TEST_PACKAGE *package)
{
