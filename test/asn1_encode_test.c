...
#define ENCDEC_ARRAY(max, zmax, min, zmin)                                                                             \
    ENCDEC_DATA(max, zmax), ENCDEC_DATA(min, zmin), ENCDEC_DATA(1, 1), ENCDEC_DATA(-1, -1),                            \
...
ASN1_SEQUENCE(ASN1_LONG_DATA) = {ASN1_SIMPLE(ASN1_LONG_DATA, success, ASN1_BOOLEAN),
...
ASN1_SEQUENCE(ASN1_INT32_DATA) = {ASN1_SIMPLE(ASN1_INT32_DATA, success, ASN1_BOOLEAN),
...
ASN1_SEQUENCE(ASN1_UINT32_DATA) = {ASN1_SIMPLE(ASN1_UINT32_DATA, success, ASN1_BOOLEAN),
...
ASN1_SEQUENCE(ASN1_INT64_DATA) = {ASN1_SIMPLE(ASN1_INT64_DATA, success, ASN1_BOOLEAN),
...
ASN1_SEQUENCE(ASN1_UINT64_DATA) = {ASN1_SIMPLE(ASN1_UINT64_DATA, success, ASN1_BOOLEAN),
 ...
 */
static int do_decode(unsigned char *bytes, long nbytes, const EXPECTED *expected, size_t expected_size,
 ...
 */
static int do_encode(EXPECTED *input, const unsigned char *expected, size_t expected_len, const TEST_PACKAGE *package)
{
...
static size_t make_custom_der(const TEST_CUSTOM_DATA *custom_data, unsigned char **encoding, int explicit_default)
{
     ...
     * items
     */
    seqbytes = 1 + der_encode_length(sizeof(t_true) + firstbytes + secondbytes, NULL) + sizeof(t_true) + firstbytes +
               secondbytes;
...
static int do_decode_custom(const TEST_CUSTOM_DATA *custom_data, const EXPECTED *expected, size_t expected_size,
                            const TEST_PACKAGE *package)
{
...
static int do_encode_custom(EXPECTED *input, const TEST_CUSTOM_DATA *custom_data, const TEST_PACKAGE *package)
{
...
static int test_intern(const TEST_PACKAGE *package)
{
    ...
    for (i = 0; i < nelems; i++)
    {
        ...
        switch (do_decode_custom(&test_custom_data[i], expected, package->encode_expectations_elem_size, package))
        {
        case -1:
            if (expected->success)
            {
...
ASN1_SEQUENCE(INVALIDTEMPLATE) =
    {
         ...
         */
        ASN1_IMP(INVALIDTEMPLATE, invalidDirString, DIRECTORYSTRING, 12)} static_ASN1_SEQUENCE_END(INVALIDTEMPLATE)
