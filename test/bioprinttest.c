...
static const struct int_data
{
     ...
     * mid-string.
     */
    {{.i = 0x1337}, AT_INT, "|%2147483647.x|", "|", .skip_libc_check = true, .exp_ret = -1},
    {{.i = 0x1337},
     AT_INT,
     ...
     .skip_libc_check = true,
     .exp_ret = -1},
    {{.i = 0x1337}, AT_INT, "%4294967302.x", "", .skip_libc_check = true, .exp_ret = -1},
    ...
    {{.ll = 0}, AT_LLONG, "#%#.0x#", "##"},
    {{.ll = 0}, AT_LLONG, "#%.0x#", "##"},
    {{.ll = 0xffffFFFFffffFFFFULL}, AT_LLONG, "%#-032llo", "01777777777777777777777         "},
    {{.ll = 0xbadc0deddeadfaceULL}, AT_LLONG, "%022lld", "-004982091772484257074"},
};
...
static int test_int(int i)
{
    ...
    const int exp_ret = data->exp_ret ? data->exp_ret : (int)strlen(data->expected);
...
#define DO_PRINT(field_)                                                                                               \
    do                                                                                                                 \
    {                                                                                                                  \
        bio_ret = BIO_snprintf(bio_buf, sizeof(bio_buf), data->format, data->value.field_);                            \
        if (!data->skip_libc_check)                                                                                    \
            std_ret = snprintf(std_buf, sizeof(std_buf), data->format, data->value.field_);                            \
    } while (0)
    case AT_CHAR:
        DO_PRINT(hh);
        break;
...
#undef DO_PRINT
    ...
    if (!TEST_str_eq(bio_buf, data->expected) + !TEST_int_eq(bio_ret, exp_ret))
    {
    ...
    if (!data->skip_libc_check)
    {
        if (!TEST_str_eq(bio_buf, std_buf) + !(data->skip_libc_ret_check || TEST_int_eq(bio_ret, std_ret)))
        {
            TEST_note("Format: \"%s\"", data->format);
...
static const struct wp_data
{
    ...
    {{.s = "DEF"},
     ...
     .skip_libc_check = true,
     .exp_ret = -1},
    {{.s = "DEF"}, "%-2147483640s", "", .skip_libc_check = true, .exp_ret = -1},
    {{.s = "DEF"},
     "%*s",
     "                                                               ",
     1,
...
static int test_width_precision(int i)
{
    ...
    {
    case 2:
        bio_ret = BIO_snprintf(bio_buf, sizeof(bio_buf), data->format, data->arg1, data->arg2, data->value.i);
        if (!data->skip_libc_check)
            std_ret = snprintf(std_buf, sizeof(std_buf), data->format, data->arg1, data->arg2, data->value.i);
        break;

    case 1:
        bio_ret = BIO_snprintf(bio_buf, sizeof(bio_buf), data->format, data->arg1, data->value.i);
        if (!data->skip_libc_check)
            std_ret = snprintf(std_buf, sizeof(std_buf), data->format, data->arg1, data->value.i);
        break;

    case 0:
    default:
        bio_ret = BIO_snprintf(bio_buf, sizeof(bio_buf), data->format, data->value.i);
        if (!data->skip_libc_check)
            std_ret = snprintf(std_buf, sizeof(std_buf), data->format, data->value.i);
    }
...
static const struct n_data
{
    ...
    {"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz%n",
     "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz", AT_INT, 62, AT_NONE},
    {"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz+=%n",
     "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz+", AT_INT, 64, AT_NONE,
     .skip_libc_ret_check = true, .exp_ret = -1},
    {"%" PRIdPTR "%hhn", "1234567890", AT_CHAR, 10, AT_INT, {.i = 1234567890}},
    {"%#.200" PRIXPTR "%hhn",
    ...
    {"%#10000" PRIoPTR "%hhn1234567890",
     ...
     .skip_libc_ret_check = true,
     .exp_ret = -1},
    {"%.0s%hn0987654321", "0987654321", AT_SHORT, 0, AT_INT, {.s = "1234567890"}},
...
#define DO_PRINT(field_)                                                                                               \
    ...
    {                                                                                                                  \
        ...
        {                                                                                                              \
            bio_ret = BIO_snprintf(bio_buf, sizeof(bio_buf), data->format, &n.field_);                                 \
            if (!skip_libc_check)                                                                                      \
                std_ret = snprintf(std_buf, sizeof(std_buf), data->format, &std_n.field_);                             \
        }                                                                                                              \
        else if (data->arg2_type == AT_NONE)                                                                           \
        {                                                                                                              \
            bio_ret = BIO_snprintf(bio_buf, sizeof(bio_buf), data->format, data->arg1.i, &n.field_);                   \
            if (!skip_libc_check)                                                                                      \
                std_ret = snprintf(std_buf, sizeof(std_buf), data->format, data->arg1.i, &std_n.field_);               \
        }                                                                                                              \
        else                                                                                                           \
        {                                                                                                              \
            bio_ret = BIO_snprintf(bio_buf, sizeof(bio_buf), data->format, data->arg1.i, data->arg2.i, &n.field_);     \
            if (!skip_libc_check)                                                                                      \
                std_ret = snprintf(std_buf, sizeof(std_buf), data->format, data->arg1.i, data->arg2.i, &std_n.field_); \
        }                                                                                                              \
    } while (0)
    case AT_CHAR:
        DO_PRINT(hh);
...
#undef DO_PRINT
    ...
    if (!TEST_str_eq(bio_buf, data->expected) + !TEST_uint64_t_eq(n.val, data->exp_n) + !TEST_int_eq(bio_ret, exp_ret))
    {
        TEST_note("Format: \"%s\"", data->format);
        return 0;
    }
    ...
    if (!data->skip_libc_check)
    {
        if (!TEST_str_eq(bio_buf, std_buf) + !TEST_uint64_t_eq(n.val, std_n.val) +
...
static const struct t_data
{
    ...
    {PTRDIFF_MAX, "%+td", sizeof(ptrdiff_t) == 4 ? "+2147483647" : "+9223372036854775807"},
    {PTRDIFF_MIN, "%+ti", sizeof(ptrdiff_t) == 4 ? "-2147483648" : "-9223372036854775808"},
    {0, "%tu", "0"},
    {0, "%+09ti", "+00000000"},
};
...
static const pw pw_params[] = {{4, ""}, {5, ""}, {4, "12"}, {5, "12"}, {0, ""}, {-1, ""}, {4, "08"}};
...
static int dofptest(int test, int sub, double val, const char *width, int prec)
{
    ...
    for (i = 0; i < (int)OSSL_NELEM(fspecs); i++)
    {
        ...
        {
            if (!TEST_str_eq(fpexpected[test][sub][i], result) + !TEST_int_eq(bio_ret, exp_ret))
            {
             ...
             * but not fatal.
             */
            if (!TEST_str_eq(result, std_result) + !TEST_int_eq(bio_ret, std_ret))
            {
                TEST_info("test %d format=|%s| exp=|%s|, ret=|%s|"
                          ", stdlib_ret=|%s|",
                          test, format, fpexpected[test][sub][i], result, std_result);
