...
test_int(int i)
{
  ...
  const int exp_ret =
    data->exp_ret ? data->exp_ret : (int)strlen(data->expected);
...
#define DO_PRINT(field_)                                                       \
  do {                                                                         \
    bio_ret = BIO_snprintf(                                                    \
      bio_buf, sizeof(bio_buf), data->format, data->value.field_);             \
    if (!data->skip_libc_check)                                                \
      std_ret =                                                                \
        snprintf(std_buf, sizeof(std_buf), data->format, data->value.field_);  \
  } while (0)
    case AT_CHAR:
      DO_PRINT(hh);
      break;
...
test_width_precision(int i)
{
  ...
  switch (data->num_args) {
    ...
    case 1:
      bio_ret = BIO_snprintf(
        bio_buf, sizeof(bio_buf), data->format, data->arg1, data->value.i);
      if (!data->skip_libc_check)
        std_ret = snprintf(
          std_buf, sizeof(std_buf), data->format, data->arg1, data->value.i);
      break;
...
#define DO_PRINT(field_)                                                       \
  do {                                                                         \
    if (data->arg1_type == AT_NONE) {                                          \
      bio_ret =                                                                \
        BIO_snprintf(bio_buf, sizeof(bio_buf), data->format, &n.field_);       \
      if (!skip_libc_check)                                                    \
        std_ret =                                                              \
          snprintf(std_buf, sizeof(std_buf), data->format, &std_n.field_);     \
    } else if (data->arg2_type == AT_NONE) {                                   \
      bio_ret = BIO_snprintf(                                                  \
        bio_buf, sizeof(bio_buf), data->format, data->arg1.i, &n.field_);      \
      if (!skip_libc_check)                                                    \
        std_ret = snprintf(std_buf,                                            \
                           sizeof(std_buf),                                    \
                           data->format,                                       \
...
#undef DO_PRINT
  ...
  if (!TEST_str_eq(bio_buf, data->expected) +
      !TEST_uint64_t_eq(n.val, data->exp_n) + !TEST_int_eq(bio_ret, exp_ret)) {
    TEST_note("Format: \"%s\"", data->format);
    return 0;
  }
...
static int
dofptest(int test, int sub, double val, const char* width, int prec)
{
