...
static int test_width_precision(int i) {
  ...
  case 2:
    bio_ret = BIO_snprintf(bio_buf, sizeof(bio_buf), data->format, data->arg1,
                           data->arg2, data->value.i);
    if (!data->skip_libc_check)
      std_ret = snprintf(std_buf, sizeof(std_buf), data->format, data->arg1,
                         data->arg2, data->value.i);
    break;
...
static const struct n_data {
  ...
  int exp_ret;
    ...
    {"0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz+=%n",
     "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz+", AT_INT,
     64, AT_NONE, .skip_libc_ret_check = true, .exp_ret = -1},
