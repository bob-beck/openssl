...
static int
get_sni_from_client_hello (BIO *bio, char **sni)
{
  long len;
  unsigned char *data;
...
static int
client_setup_sni_before_state (void)
{
  SSL_CTX *ctx;
  SSL *con = NULL;
  BIO *rbio;
...
static int
client_setup_sni_after_state (void)
{
  SSL_CTX *ctx;
  SSL *con = NULL;
  BIO *rbio;
...
static sni_test_fn sni_test_fns[3]
    = { client_setup_sni_before_state, client_setup_sni_after_state,
        server_setup_sni };

static int
test_servername (int test)
{
  /*
