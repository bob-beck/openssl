...
static int test_sanity_range(void)
{
    ...
    if (!TEST_size_t_eq(sizeof(int8_t), 1) || !TEST_size_t_eq(sizeof(uint8_t), 1) ||
        !TEST_size_t_eq(sizeof(int16_t), 2) || !TEST_size_t_eq(sizeof(uint16_t), 2) ||
        !TEST_size_t_eq(sizeof(int32_t), 4) || !TEST_size_t_eq(sizeof(uint32_t), 4) ||
        !TEST_size_t_eq(sizeof(int64_t), 8) || !TEST_size_t_eq(sizeof(uint64_t), 8)
#ifdef UINT128_MAX
