...
static int test_lib(void) {
  ...
  switch (test_type) {
    ...
    case CRYPTO_FIRST:
      ...
      if (test_type != CRYPTO_FIRST) break;
      /* Fall through */
  ...
  if (cryptolib != SD_INIT) sd_close(cryptolib);
  if (ssllib != SD_INIT) sd_close(ssllib);
  return result;
}
#endif
