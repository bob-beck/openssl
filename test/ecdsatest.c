...
static int
fbytes (unsigned char *buf, size_t num, ossl_unused const char *name,
        EVP_RAND_CTX *ctx)
{
  int ret = 0;
...
x9_62_tests (int n)
{
  ...
  if (!TEST_true (EC_KEY_generate_key (key))
      || !TEST_true (p_len = EC_KEY_key2buf (
                         key, POINT_CONVERSION_UNCOMPRESSED, &pbuf, NULL))
 ...
 */
static int
set_sm2_id (EVP_MD_CTX *mctx, EVP_PKEY *pkey)
{
  ...
  if (!TEST_ptr (pctx = EVP_MD_CTX_get_pkey_ctx (mctx))
      || !TEST_int_gt (EVP_PKEY_CTX_set1_id (pctx, sm2_id, sizeof (sm2_id)),
                       0))
    return 0;
  return 1;
}
...
test_builtin (int n, int as)
{
  ...
  if (!TEST_int_ge (temp, 0)
      ...
      || !TEST_int_eq (
          EVP_DigestVerify (mctx, sig, sig_len, tbs, sizeof (tbs)), 0)
      || !TEST_true (EVP_MD_CTX_reset (mctx))
      ...
      || (as == EVP_PKEY_SM2 && !set_sm2_id (mctx, pkey))
      || !TEST_int_eq (
          EVP_DigestVerify (mctx, sig, sig_len, tbs, sizeof (tbs)), 1)
      || !TEST_true (EVP_MD_CTX_reset (mctx)))
    goto err;
...
test_ecdsa_sig_NULL (void)
{
  ...
  ret = TEST_ptr (eckey = EC_KEY_new_by_curve_name (NID_X9_62_prime256v1))
        ...
        && TEST_int_eq (
            ECDSA_sign (0, dgst, sizeof (dgst), NULL, &siglen0, eckey), 1)
        && TEST_int_gt (siglen0, 0)
        && TEST_ptr (sig = OPENSSL_malloc (siglen0))
        && TEST_int_eq (
            ECDSA_sign (0, dgst, sizeof (dgst), sig, &siglen, eckey), 1)
        && TEST_int_gt (siglen, 0) && TEST_int_le (siglen, siglen0)
        && TEST_int_eq (
            ECDSA_verify (0, dgst, sizeof (dgst), sig, siglen, eckey), 1)
