...
typedef void* PEM_read_bio_of_void(BIO* out,
                                   void** obj,
                                   pem_password_cb* cb,
                                   void* u);
typedef int EVP_PKEY_print_fn(BIO* out,
                              const EVP_PKEY* pkey,
                              int indent,
                              ASN1_PCTX* pctx);
...
static int test_protected_PEM(const char* keytype,
  ...
  if (!TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(provided_pkey, selection,
      ...
      !TEST_true(pem_write_bio(membio_legacy, legacy_key, NULL, NULL, 0, NULL,
                               NULL)) ||
...
static int test_unprotected_PEM(
    const char* keytype,
    int evp_type,
    const void* legacy_key,
