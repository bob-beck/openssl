...
static struct key_st* lookup_key(const char* type) {
  ...
  for (i = 0; i < OSSL_NELEM(keys); i++) {
    if (strcmp(keys[i].keytype, type) == 0) return &keys[i];
  }
  return NULL;
}
...
static int test_unprotected_PEM(
    const char* keytype, int evp_type, const void* legacy_key,
    PEM_write_bio_of_void_unprotected* pem_write_bio,
    PEM_read_bio_of_void* pem_read_bio, EVP_PKEY_eq_fn* evp_pkey_eq,
    EVP_PKEY_print_fn* evp_pkey_print, EVP_PKEY* provided_pkey, int selection,
    const char* structure) {
  int ok = 0;
  BIO* membio_legacy = NULL;
  BIO* membio_provided = NULL;
