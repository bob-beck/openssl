...
static int
test_protected_PEM (const char *keytype, int evp_type, const void *legacy_key,
...
static int
test_unprotected_PEM (const char *keytype, int evp_type,
...
static int
test_DER (const char *keytype, int evp_type, const void *legacy_key,
          i2d_of_void *i2d, d2i_of_void *d2i, EVP_PKEY_eq_fn *evp_pkey_eq,
          EVP_PKEY_print_fn *evp_pkey_print, EVP_PKEY *provided_pkey,
          int selection, const char *structure)
{
  int ok = 0;
      ...
      for (i = 0; i < OSSL_NELEM (test_stanza->structure); i++)
        {
          ...
          TEST_info ("Test OSSL_ENCODER against PEM_write_bio_{TYPE}params "
                     "for %s, %s",
                     test_stanza->keytype, structure);
