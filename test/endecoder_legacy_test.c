...
typedef int
PEM_write_bio_of_void_protected(BIO* out,
                                const void* obj,
                                ...
                                void* u);
typedef int
PEM_write_bio_of_void_unprotected(BIO* out, const void* obj);
typedef void*
PEM_read_bio_of_void(BIO* out, void** obj, pem_password_cb* cb, void* u);
typedef int
EVP_PKEY_print_fn(BIO* out, const EVP_PKEY* pkey, int indent, ASN1_PCTX* pctx);
typedef int
EVP_PKEY_eq_fn(const EVP_PKEY* a, const EVP_PKEY* b);
...
static EVP_PKEY*
make_key(const char* type, const OSSL_PARAM* gen_template_params)
{
...
static int
test_membio_str_eq(BIO* bio_provided, BIO* bio_legacy)
{
...
static int
test_protected_PEM(const char* keytype,
                   int evp_type,
...
static int
test_unprotected_PEM(const char* keytype,
                     int evp_type,
                     const void* legacy_key,
...
test_key(int idx)
{
  ...
  if (test_stanza->pem_write_bio_PrivateKey != NULL) {
    ...
    for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
      ...
      TEST_info(
        ...
        test_stanza->keytype,
        structure);
      if (!test_protected_PEM(key->keytype,
                              key->evp_type,
                              legacy_obj,
  ...
  if (test_stanza->pem_write_bio_PublicKey != NULL) {
    ...
    for (i = 0; i < OSSL_NELEM(test_stanza->structure); i++) {
      ...
      TEST_info(
        ...
        test_stanza->keytype,
        structure);
      if (!test_unprotected_PEM(key->keytype,
                                key->evp_type,
                                legacy_obj,
  ...
  if (test_stanza->i2d_PUBKEY != NULL) {
    ...
    if (!test_DER(key->keytype,
                  key->evp_type,
                  legacy_obj,
                  test_stanza->i2d_PUBKEY,
                  test_stanza->d2i_PUBKEY,
                  EVP_PKEY_eq,
                  EVP_PKEY_print_public,
                  pkey,
                  selection,
