...
static const struct rx_test_op rx_script_5[] = {
  ...
  RX_OP_SET_SCID_LEN (0) RX_OP_SET_RX_DCID (empty_conn_id) RX_OP_INJECT_N (5)
      RX_OP_CHECK_NO_PKT () RX_OP_PROVIDE_SECRET_INITIAL (
          rx_script_5_c2s_init_dcid) RX_OP_CHECK_PKT_N (5a)
  ...
  for (; op->op != RX_TEST_OP_END; ++op)
    switch (op->op)
      ...
      case RX_TEST_OP_PROVIDE_SECRET_INITIAL:
        if (!TEST_true (rx_state_ensure (&s)))
          goto err;
        if (!TEST_true (ossl_quic_provide_initial_secret (NULL, NULL, op->dcid,
                                                          0, s.qrx, NULL)))
          goto err;
        break;
...
static const struct tx_test_op tx_script_6[]
    = { TX_OP_WRITE_CHECK (6) TX_OP_END };

static const struct tx_test_op *const tx_scripts[]
    = { tx_script_1, tx_script_2,
#if !defined(OPENSSL_NO_CHACHA) && !defined(OPENSSL_NO_POLY1305)
        tx_script_3,
#endif
