...
static void demux_default_handler(QUIC_URXE* e, void* arg,
  ...
  if (dcid == NULL || !ossl_quic_conn_id_eq(dcid, &s->rx_dcid)) return;
...
static int rx_state_ensure(struct rx_state* s) {
  ...
  /* Initialise OSSL_QRX */
  if (s->qrx == NULL && !TEST_ptr(s->qrx = ossl_qrx_new(&s->args))) return 0;

  ossl_quic_demux_set_default_handler(s->demux, demux_default_handler, s);

  if (s->allow_1rtt) ossl_qrx_allow_1rtt_processing(s->qrx);
...
static int rx_run_script(const struct rx_test_op* script) {
  ...
  for (; op->op != RX_TEST_OP_END; ++op) switch (op->op) {
      ...
      case RX_TEST_OP_SET_RX_DCID:
        if (!TEST_true(rx_state_ensure(&s))) goto err;
        s.rx_dcid = *op->dcid;
        break;
      ...
      case RX_TEST_OP_CHECK_PKT:
        ...
        if (!TEST_true(ossl_qrx_read_pkt(s.qrx, &pkt))) goto err;

        if (!TEST_ptr(pkt) || !TEST_ptr(pkt->hdr)) goto err;
      ...
      case RX_TEST_OP_CHECK_NO_PKT:
        ...
        if (!TEST_false(ossl_qrx_read_pkt(s.qrx, &pkt))) goto err;
      ...
      case RX_TEST_OP_CHECK_PKT_EPOCH:
        ...
        if (!TEST_uint64_t_eq(last_key_epoch, op->largest_pn)) goto err;
...
static int test_wire_pkt_hdr_actual(int tidx, int repeat, int cipher,
  ...
  if (!TEST_ptr(buf = OPENSSL_malloc(TEST_PKT_BUF_LEN))) goto err;

  if (!TEST_true(WPACKET_init_static_len(&wpkt, buf, TEST_PKT_BUF_LEN, 0)))
    goto err;

  if (!TEST_true(PACKET_buf_init(&pkt, t->expected, trunc_len))) goto err;
  ...
  if (!expect_fail && !is_trunc) {
    ...
    if (!TEST_ptr_eq(ptrs.raw_start, t->expected)) goto err;

    if (t->pn_offset == SIZE_MAX) {
      if (!TEST_ptr_null(ptrs.raw_pn)) goto err;
    } else {
      if (!TEST_ptr_eq(ptrs.raw_pn, t->expected + t->pn_offset)) goto err;
    }
    ...
    if (!TEST_true(WPACKET_memcpy(&wpkt, t->payload, t->payload_len))) goto err;

    if (!TEST_true(WPACKET_get_total_written(&wpkt, &l))) goto err;

    if (!TEST_mem_eq(buf, l, t->expected, t->expected_len)) goto err;
    ...
    if (t->sample_offset != SIZE_MAX) { /* if packet type has protection */
      ...
      ptrs.raw_start = hbuf;
      if (!TEST_true(ossl_quic_hdr_protector_encrypt(&hpr, &ptrs))) goto err;
      ...
      for (i = 0; i < t->expected_len; ++i) {
        ...
        if (!TEST_int_eq(d & rej_mask, 0)) goto err;
      }

      /* Decrypt and check matches original. */
      if (!TEST_true(ossl_quic_hdr_protector_decrypt(&hpr, &ptrs))) goto err;
  ...
  testresult = 1;
err:
  if (have_hpr) ossl_quic_hdr_protector_cleanup(&hpr);
  WPACKET_finish(&wpkt);
  OPENSSL_free(buf);
  OPENSSL_free(hbuf);
  return testresult;
...
static int test_wire_pkt_hdr_inner(int tidx, int repeat, int cipher) {
  ...
  for (i = 0; i < t->expected_len; ++i)
    if (!TEST_true(test_wire_pkt_hdr_actual(tidx, repeat, cipher, i))) goto err;
...
static int test_hdr_prot_stats(void) {
  ...
  for (cipher = 0; cipher < HPR_CIPHER_COUNT; ++cipher)
    for (i = 0; i < OSSL_NELEM(counts_u[0]); ++i) {
      if (!TEST_uint_gt(counts_u[cipher][i], 0)) goto err;
      if (!TEST_uint_gt(counts_c[cipher][i], 0)) goto err;
    }
...
static int test_wire_pkt_hdr(int idx) {
  ...
  if (idx == NUM_WIRE_PKT_HDR_TESTS) return test_hdr_prot_stats();
...
static int tx_run_script(const struct tx_test_op* script) {
  ...
  if (!TEST_ptr(qtx = ossl_qtx_new(&args))) goto err;
  ...
  for (; op->op != TX_TEST_OP_END; ++op) switch (op->op) {
      ...
      case TX_TEST_OP_WRITE: {
        ...
        if (enc_level < QUIC_ENC_LEVEL_NUM) { /* encrypted packet */
          ...
          if (!TEST_uint64_t_lt(max_value, UINT64_MAX)) goto err;

          old_value = ossl_qtx_get_cur_epoch_pkt_count(qtx, enc_level);
          if (!TEST_uint64_t_lt(old_value, UINT64_MAX)) goto err;
        }

        if (!TEST_true(ossl_qtx_write_pkt(qtx, op->pkt))) goto err;

        if (enc_level < QUIC_ENC_LEVEL_NUM) {
          new_value = ossl_qtx_get_cur_epoch_pkt_count(qtx, enc_level);
          if (!TEST_uint64_t_eq(old_value + 1, new_value)) goto err;
        }
      } break;
      case TX_TEST_OP_CHECK_DGRAM:
        if (!TEST_true(ossl_qtx_pop_net(qtx, &msg))) goto err;
      ...
      case TX_TEST_OP_CHECK_NO_DGRAM:
        if (!TEST_false(ossl_qtx_pop_net(qtx, &msg))) goto err;
        break;
      case TX_TEST_OP_KEY_UPDATE:
        if (!TEST_true(ossl_qtx_trigger_key_update(qtx))) goto err;
        break;
      default:
        OPENSSL_assert(0);
        goto err;
