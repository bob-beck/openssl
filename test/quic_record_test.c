...
static const struct rx_test_op rx_script_5[] = {
  ...
  RX_OP_SET_SCID_LEN(0) RX_OP_SET_RX_DCID(empty_conn_id) RX_OP_INJECT_N(5)
    RX_OP_CHECK_NO_PKT() RX_OP_PROVIDE_SECRET_INITIAL(rx_script_5_c2s_init_dcid)
      RX_OP_CHECK_PKT_N(5a) RX_OP_CHECK_NO_PKT()
        RX_OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_HANDSHAKE,
                             QRL_SUITE_AES128GCM,
                             rx_script_5_handshake_secret) RX_OP_CHECK_PKT_N(5b)
          RX_OP_CHECK_NO_PKT() RX_OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT,
                                                    QRL_SUITE_AES128GCM,
...
static const struct rx_test_op rx_script_6[] = {
  ...
  RX_OP_SET_SCID_LEN(0) RX_OP_SET_RX_DCID(empty_conn_id) RX_OP_INJECT_N(6)
    RX_OP_CHECK_NO_PKT() RX_OP_PROVIDE_SECRET_INITIAL(rx_script_6_c2s_init_dcid)
      RX_OP_CHECK_PKT_N(6a) RX_OP_CHECK_NO_PKT()
        RX_OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_HANDSHAKE,
                             QRL_SUITE_AES256GCM,
                             rx_script_6_handshake_secret) RX_OP_CHECK_PKT_N(6b)
          RX_OP_CHECK_NO_PKT() RX_OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT,
...
static void
rx_state_teardown(struct rx_state* s)
{
  if (s->quic_conn != NULL) {
...
static OSSL_TIME
expected_time(uint64_t counter)
{
...
static int
rx_state_ensure(struct rx_state* s)
{
  if (s->demux == NULL &&
...
static int
rx_run_script(const struct rx_test_op* script)
{
  int testresult = 0;
  struct rx_state s = { 0 };
  size_t i;
    ...
    switch (op->op) {
      ...
      case RX_TEST_OP_PROVIDE_SECRET:
        ...
        if (!TEST_true(ossl_qrx_provide_secret(
              s.qrx, op->enc_level, op->suite_id, NULL, op->buf, op->buf_len)))
          goto err;
        break;
...
static const unsigned char pkt_hdr_test_12_payload[] = { 0x11,
                                                         0x22,
                                                         0x33,
                                                         0x44 };
...
static const unsigned char pkt_hdr_test_13_payload[] = { 0x11,
                                                         0x22,
                                                         0x33,
                                                         0x44 };
...
static int
test_wire_pkt_hdr_actual(int tidx, int repeat, int cipher, size_t trunc_len)
{
  int testresult = 0;
  ...
  if (!TEST_int_eq(ossl_quic_wire_decode_pkt_hdr(
                     &pkt, t->short_conn_id_len, 0, 0, &hdr, &ptrs, NULL),
                   !expect_fail))
    goto err;
...
static int
test_wire_pkt_hdr_inner(int tidx, int repeat, int cipher)
{
  int testresult = 0;
  const struct pkt_hdr_test* t = pkt_hdr_tests[tidx];
  size_t i;
...
static const struct tx_test_op tx_script_4[] = {
  TX_OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT,
                       QRL_SUITE_AES128GCM,
...
static int
tx_run_script(const struct tx_test_op* script)
{
  int testresult = 0;
