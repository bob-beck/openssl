...
static int test_thread_reported_flags(void) {
  ...
  if (!TEST_int_eq(flags, 0)) return 0;
#endif

#if defined(OPENSSL_NO_THREAD_POOL)
  if (!TEST_int_eq(flags & OSSL_THREAD_SUPPORT_FLAG_THREAD_POOL, 0)) return 0;
#else
  ...
  if (!TEST_int_eq(flags & OSSL_THREAD_SUPPORT_FLAG_DEFAULT_SPAWN, 0)) return 0;
#else
...
static int test_thread_native(void) {
   ...
   * if we do).
   */
  if (!TEST_int_eq(ossl_crypto_thread_native_join(t, &retval), 1)) return 0;
  if (!TEST_int_eq(ossl_crypto_thread_native_join(t, &retval), 1)) return 0;

  if (!TEST_int_eq(retval, 1) || !TEST_int_eq(local, 2)) return 0;

  if (!TEST_int_eq(ossl_crypto_thread_native_clean(t), 1)) return 0;
  t = NULL;

  if (!TEST_int_eq(ossl_crypto_thread_native_clean(t), 0)) return 0;
...
static int test_thread_internal(void) {
  ...
  if (threads_supported == 0) {
    if (!TEST_uint64_t_eq(OSSL_get_max_threads(NULL), 0)) goto cleanup;
    if (!TEST_uint64_t_eq(OSSL_get_max_threads(cust_ctx), 0)) goto cleanup;

    if (!TEST_int_eq(OSSL_set_max_threads(NULL, 1), 0)) goto cleanup;
    if (!TEST_int_eq(OSSL_set_max_threads(cust_ctx, 1), 0)) goto cleanup;

    if (!TEST_uint64_t_eq(OSSL_get_max_threads(NULL), 0)) goto cleanup;
    if (!TEST_uint64_t_eq(OSSL_get_max_threads(cust_ctx), 0)) goto cleanup;

    t[0] = ossl_crypto_thread_start(NULL, test_thread_native_fn, &local[0]);
    if (!TEST_ptr_null(t[0])) goto cleanup;
  ...
  if (!TEST_uint64_t_eq(OSSL_get_max_threads(cust_ctx), 0)) goto cleanup;
  if (!TEST_int_eq(OSSL_set_max_threads(cust_ctx, 1), 1)) goto cleanup;
  if (!TEST_uint64_t_eq(OSSL_get_max_threads(NULL), 0)) goto cleanup;
  if (!TEST_uint64_t_eq(OSSL_get_max_threads(cust_ctx), 1)) goto cleanup;
  t[0] = ossl_crypto_thread_start(NULL, test_thread_native_fn, &local[0]);
  if (!TEST_ptr_null(t[0])) goto cleanup;
  if (!TEST_int_eq(OSSL_set_max_threads(cust_ctx, 0), 1)) goto cleanup;

  /* sequential startup */

  if (!TEST_int_eq(OSSL_set_max_threads(NULL, 1), 1)) goto cleanup;
  if (!TEST_uint64_t_eq(OSSL_get_max_threads(NULL), 1)) goto cleanup;
  if (!TEST_uint64_t_eq(OSSL_get_max_threads(cust_ctx), 0)) goto cleanup;
  ...
  for (i = 0; i < OSSL_NELEM(t); ++i) {
    ...
    if (!TEST_int_eq(ossl_crypto_thread_clean(t[i]), 1)) goto cleanup;
    t[i] = NULL;

    if (!TEST_int_eq(ossl_crypto_thread_clean(t[i]), 0)) goto cleanup;
  }

  /* parallel startup */

  if (!TEST_int_eq(OSSL_set_max_threads(NULL, OSSL_NELEM(t)), 1)) goto cleanup;
  ...
  if (!TEST_int_eq(OSSL_set_max_threads(NULL, 0), 1)) goto cleanup;
...
static int test_thread_native_multiple_joins(void) {
  ...
  t = ossl_crypto_thread_native_start(test_thread_native_multiple_joins_fn1,
                                      NULL, 1);
  t1 = ossl_crypto_thread_native_start(test_thread_native_multiple_joins_fn2, t,
                                       1);
  t2 = ossl_crypto_thread_native_start(test_thread_native_multiple_joins_fn3, t,
                                       1);

  if (!TEST_ptr(t) || !TEST_ptr(t1) || !TEST_ptr(t2)) return 0;

  if (!TEST_int_eq(ossl_crypto_thread_native_join(t2, NULL), 1)) return 0;
  if (!TEST_int_eq(ossl_crypto_thread_native_join(t1, NULL), 1)) return 0;

  if (!TEST_int_eq(ossl_crypto_thread_native_clean(t2), 1)) return 0;

  if (!TEST_int_eq(ossl_crypto_thread_native_clean(t1), 1)) return 0;
