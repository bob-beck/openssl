...
static char* change_path(const char* file) {
  ...
  if (last == NULL) goto err;

  last[DIRSEP_PRESERVE] = 0;
  TEST_note("changing path to %s", s);

  ret = chdir(s);
  if (ret == 0) new_config_name = OPENSSL_strdup(last + DIRSEP_PRESERVE + 1);
err:
  OPENSSL_free(s);
...
static int test_check_overflow(void) {
  ...
  setenv("FNORD", max, 1);
  if (!TEST_false(NCONF_get_number(NULL, "missing", "FNORD", &val))) return 0;
#endif
  return 1;
}
...
int setup_tests(void) {
  ...
  if (test_providers != 0) ADD_TEST(test_available_providers);
