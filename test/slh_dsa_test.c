...
static EVP_PKEY *
slh_dsa_key_from_data (const char *alg, const unsigned char *data,
...
static int
slh_dsa_create_keypair (EVP_PKEY **pkey, const char *name, const uint8_t *priv,
 ...
 * test (KAT) using the digest of the signature, so this should be sufficient
 * to run as a KAT for the verify.
 */
static int
do_slh_dsa_verify (const SLH_DSA_SIG_TEST_DATA *td, uint8_t *sig,
                   size_t sig_len)
{
  int ret = 0;
  EVP_PKEY_CTX *vctx = NULL;
