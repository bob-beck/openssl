...
static EVP_PKEY*
slh_dsa_key_from_data(const char* alg,
...
static int
slh_dsa_create_keypair(EVP_PKEY** pkey,
                       const char* name,
                       const uint8_t* priv,
                       size_t priv_len,
                       const uint8_t* pub,
...
static int
slh_dsa_key_validate_test(void)
{
  int ret = 0;
  SLH_DSA_SIG_TEST_DATA* td = &slh_dsa_sig_testdata[0];
...
static int
slh_dsa_key_validate_failure_test(void)
{
  int ret = 0;
  EVP_PKEY_CTX* vctx = NULL;
  EVP_PKEY* key = NULL;
   ...
   */
  if (!TEST_ptr(key =
                  slh_dsa_key_from_data("SLH-DSA-SHA2-128f",
 ...
 */
static int
do_slh_dsa_verify(const SLH_DSA_SIG_TEST_DATA* td, uint8_t* sig, size_t sig_len)
{
  int ret = 0;
  EVP_PKEY_CTX* vctx = NULL;
  EVP_PKEY* key = NULL;
...
static int
slh_dsa_sign_verify_test(int tst_id)
{
  int ret = 0;
  ...
  *p = OSSL_PARAM_construct_end();
   ...
   */
  if (!slh_dsa_create_keypair(
        &pkey, td->alg, td->priv, td->priv_len, td->pub, td->pub_len))
    goto err;
...
static int
slh_dsa_keygen_test(int tst_id)
{
  int ret = 0;
...
slh_dsa_usage_test(void)
{
  ...
  if (!TEST_ptr(PEM_read_bio_PrivateKey_ex(
        priv_bio, &priv, NULL, pass, lib_ctx, NULL)) ||
...
static int
slh_dsa_deterministic_usage_test(void)
{
  int ret = 0;
  EVP_CIPHER* cipher = NULL; /* Used to encrypt the private key */
  char* pass = "Password";
...
static int
slh_dsa_digest_sign_verify_test(void)
{
  int ret = 0;
  EVP_PKEY* key = NULL;
  uint8_t* sig = NULL;
...
static int
slh_dsa_keygen_invalid_test(void)
{
  int ret = 0;
  const SLH_DSA_KEYGEN_TEST_DATA* tst = &slh_dsa_keygen_testdata[0];
  EVP_PKEY* pkey = NULL;
  ...
  p[0] = OSSL_PARAM_construct_octet_string(
    OSSL_PKEY_PARAM_SLH_DSA_SEED, seed, n * 3 - 1);
  p[1] = OSSL_PARAM_construct_end();
