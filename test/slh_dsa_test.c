...
static EVP_PKEY *slh_dsa_key_from_data(const char *alg, const unsigned char *data, size_t datalen, int public)
{
    ...
    ret = TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(lib_ctx, alg, NULL)) &&
          TEST_int_eq(EVP_PKEY_fromdata_init(ctx), 1) && (EVP_PKEY_fromdata(ctx, &key, selection, params) == 1);
    if (ret == 0)
    {
        EVP_PKEY_free(key);
        key = NULL;
...
static int slh_dsa_usage_test(void)
{
    ...
    if (!TEST_int_eq(EVP_PKEY_sign(sctx, NULL, &sig_len, msg, msg_len), 1) ||
        !TEST_ptr(sig = OPENSSL_malloc(sig_len)) || !TEST_int_eq(EVP_PKEY_sign(sctx, sig, &sig_len, msg, msg_len), 1))
        goto err;
...
static int slh_dsa_deterministic_usage_test(void)
{
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_int_eq(EVP_PKEY_sign(sctx, NULL, &sig_len, msg, msg_len), 1))
        goto err;
    len = sig_len;
    if (!TEST_ptr(sig = OPENSSL_calloc(2, sig_len)) || !TEST_int_eq(EVP_PKEY_sign(sctx, sig, &len, msg, msg_len), 1) ||
