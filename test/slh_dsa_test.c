...
static int slh_dsa_bad_pub_len_test(void) {
  ...
  if (!TEST_size_t_le(td->pub_len, sizeof(pubdata))) goto end;
...
static int slh_dsa_key_eq_test(void) {
  ...
  if (!TEST_ptr(key[0] = slh_dsa_key_from_data(td1->alg, td1->pub, td1->pub_len,
                                               1)) ||
      !TEST_ptr(key[1] = slh_dsa_key_from_data(td1->alg, td1->pub, td1->pub_len,
                                               1)) ||
      !TEST_ptr(key[2] = slh_dsa_key_from_data(td2->alg, td2->pub, td2->pub_len,
                                               1)) ||
...
static int slh_dsa_key_validate_test(void) {
  ...
  if (!TEST_ptr(vctx = EVP_PKEY_CTX_new_from_pkey(lib_ctx, key, NULL)))
    goto end;
  if (!TEST_int_eq(EVP_PKEY_public_check(vctx), 1)) goto end;
  if (!TEST_int_eq(EVP_PKEY_private_check(vctx), 0)) goto end;
  if (!TEST_int_eq(EVP_PKEY_pairwise_check(vctx), 0)) goto end;
  ret = 1;
end:
...
static int slh_dsa_sign_verify_test(int tst_id) {
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!TEST_mem_eq(digest, digest_len, td->sig_digest, td->sig_digest_len))
    goto err;
  if (!do_slh_dsa_verify(td, psig, psig_len)) goto err;
  ret = 1;
err:
  EVP_SIGNATURE_free(sig_alg);
  EVP_PKEY_free(pkey);
...
static int slh_dsa_keygen_test(int tst_id) {
  ...
  if (!TEST_ptr(pkey = do_gen_key(tst->name, tst->priv, key_len - n))) goto err;
  ...
  if (!TEST_size_t_eq(priv_len, key_len) ||
    ...
    goto err;
  if (!TEST_mem_eq(pub, pub_len, tst->priv + 2 * n, 2 * n)) goto err;
  ret = 1;
err:
  EVP_PKEY_free(pkey);
  return ret;
...
static int slh_dsa_usage_test(void) {
  ...
  /* Generate a key */
  if (!TEST_ptr(gctx = EVP_PKEY_CTX_new_from_name(lib_ctx, "SLH-DSA-SHA2-128s",
                                                  NULL)) ||
  ...
  if (!TEST_ptr(pub_bio = BIO_new(BIO_s_mem())) ||
      ...
      !TEST_true(PEM_write_bio_PrivateKey_ex(priv_bio, gkey, cipher, NULL, 0,
                                             NULL, (void*)pass, lib_ctx, NULL)))
    goto err;

  /* Read the private key and add to a signing ctx */
  if (!TEST_ptr(PEM_read_bio_PrivateKey_ex(priv_bio, &priv, NULL, pass, lib_ctx,
                                           NULL)) ||
...
static int slh_dsa_deterministic_usage_test(void) {
  ...
  /* Generate a key */
  if (!TEST_ptr(gkey = do_gen_key(tst->name, tst->priv, key_len + n))) goto err;
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!TEST_ptr(dupctx = EVP_PKEY_CTX_dup(sctx))) goto err;
...
static int slh_dsa_digest_sign_verify_test(void) {
  ...
  if (!TEST_ptr(key = do_gen_key(alg, NULL, 0))) goto err;
...
static int slh_dsa_keygen_invalid_test(void) {
  ...
  if (!TEST_int_eq(EVP_PKEY_CTX_set_params(ctx, params), 0)) goto err;
