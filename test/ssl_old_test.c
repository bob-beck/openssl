...
static int
verify_servername(SSL* client, SSL* server)
{
               ...
               void* arg)
{
  ...
  if (protos == NULL) {
    fprintf(
      stderr, "failed to parser ALPN server protocol string: %s\n", alpn_str);
    abort();
  }
...
err:
  ...
  BIO_printf(bio_stdout, "'\n");
  BIO_printf(
    bio_stdout, "ALPN configured: client: '%s', server: '", alpn_client);
...
static int
custom_ext_0_cli_parse_cb(SSL* s,
                          unsigned int ext_type,
                          const unsigned char* in,
                          size_t inlen,
                          int* al,
...
static int
custom_ext_1_cli_parse_cb(SSL* s,
                          unsigned int ext_type,
                          const unsigned char* in,
                          size_t inlen,
                          int* al,
...
static int
custom_ext_2_cli_parse_cb(SSL* s,
                          unsigned int ext_type,
                          const unsigned char* in,
                          size_t inlen,
                          int* al,
...
static int
custom_ext_3_cli_parse_cb(SSL* s,
                          unsigned int ext_type,
                          const unsigned char* in,
                          size_t inlen,
                          int* al,
 ...
 */
static int
custom_ext_0_srv_parse_cb(SSL* s,
                          unsigned int ext_type,
                          const unsigned char* in,
                          size_t inlen,
                          int* al,
...
static int
custom_ext_1_srv_parse_cb(SSL* s,
                          unsigned int ext_type,
                          const unsigned char* in,
                          size_t inlen,
                          int* al,
...
static int
custom_ext_2_srv_parse_cb(SSL* s,
                          unsigned int ext_type,
                          const unsigned char* in,
                          size_t inlen,
                          int* al,
...
static int
custom_ext_3_srv_parse_cb(SSL* s,
                          unsigned int ext_type,
                          const unsigned char* in,
                          size_t inlen,
                          int* al,
...
doit_localhost(SSL* s_ssl,
               ...
               clock_t* c_time);
int
doit_biopair(SSL* s_ssl,
             SSL* c_ssl,
             long bytes,
             clock_t* s_time,
             clock_t* c_time);
int
doit(SSL* s_ssl, SSL* c_ssl, long bytes);
...
static void
print_key_details(BIO* out, EVP_PKEY* key)
{
...
static int
write_session(const char* filename, SSL_SESSION* sess)
{
  BIO* f;
 ...
 */
static int
set_protocol_version(const char* version, SSL* ssl, int setting)
{
  if (version != NULL) {
...
main(int argc, char* argv[])
{
  ...
  if (client_auth) {
    ...
    SSL_CTX_set_cert_verify_callback(
      s_ctx, app_verify_callback, &app_verify_arg);
    SSL_CTX_set_cert_verify_callback(
      s_ctx2, app_verify_callback, &app_verify_arg);
  }
  if (server_auth) {
    printf("server authentication\n");
    SSL_CTX_set_verify(c_ctx, SSL_VERIFY_PEER, verify_callback);
    SSL_CTX_set_cert_verify_callback(
      c_ctx, app_verify_callback, &app_verify_arg);
  }

  {
    ...
    if (!SSL_CTX_set_session_id_context(
        ...
        !SSL_CTX_set_session_id_context(
          s_ctx2, (void*)&session_id_context, sizeof(session_id_context))) {
      ERR_print_errors(bio_err);
      goto end;
    }
  }
  ...
  if (custom_ext) {
    if (!SSL_CTX_add_client_custom_ext(c_ctx,
                                       ...
                                       NULL) ||
        !SSL_CTX_add_client_custom_ext(c_ctx,
                                       CUSTOM_EXT_TYPE_1,
        ...
        !SSL_CTX_add_client_custom_ext(c_ctx,
                                       ...
                                       NULL) ||
        !SSL_CTX_add_server_custom_ext(s_ctx,
                                       CUSTOM_EXT_TYPE_0,
                                       ...
                                       NULL) ||
        !SSL_CTX_add_server_custom_ext(s_ctx2,
                                       CUSTOM_EXT_TYPE_0,
                                       custom_ext_0_srv_add_cb,
                                       NULL,
                                       NULL,
  ...
  if (!set_protocol_version(
        server_min_proto, s_ssl, SSL_CTRL_SET_MIN_PROTO_VERSION))
    goto end;
  if (!set_protocol_version(
        server_max_proto, s_ssl, SSL_CTRL_SET_MAX_PROTO_VERSION))
    goto end;
  if (!set_protocol_version(
        client_min_proto, c_ssl, SSL_CTRL_SET_MIN_PROTO_VERSION))
    goto end;
  if (!set_protocol_version(
        client_max_proto, c_ssl, SSL_CTRL_SET_MAX_PROTO_VERSION))
    goto end;
  ...
  for (i = 0; i < number; i++) {
    ...
    switch (bio_type) {
      ...
      case BIO_IPV4:
        ret = doit_localhost(
          s_ssl, c_ssl, BIO_FAMILY_IPV4, bytes, &s_time, &c_time);
        break;
      case BIO_IPV6:
        ret = doit_localhost(
          s_ssl, c_ssl, BIO_FAMILY_IPV6, bytes, &s_time, &c_time);
        break;
#else
      case BIO_IPV4:
      case BIO_IPV6:
