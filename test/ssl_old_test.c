...
static int verify_npn(SSL* client, SSL* server) {
  ...
  if (!npn_client && client_len) return -1;
  if (!npn_server && server_len) return -1;
  if (npn_server_reject && server_len) return -1;
  if (npn_client && npn_server && (!client_len || !server_len)) return -1;
...
static int verify_serverinfo(void) {
  if (serverinfo_sct != serverinfo_sct_seen) return -1;
  if (serverinfo_tack != serverinfo_tack_seen) return -1;
  if (serverinfo_other_seen) return -1;
  return 0;
}
...
static int custom_ext_0_cli_add_cb(SSL* s, unsigned int ext_type,
                                   ...
                                   int* al, void* arg) {
  if (ext_type != CUSTOM_EXT_TYPE_0) custom_ext_error = 1;
...
static int custom_ext_1_cli_add_cb(SSL* s, unsigned int ext_type,
                                   const unsigned char** out, size_t* outlen,
                                   int* al, void* arg) {
  if (ext_type != CUSTOM_EXT_TYPE_1) custom_ext_error = 1;
...
static int custom_ext_2_cli_add_cb(SSL* s, unsigned int ext_type,
                                   const unsigned char** out, size_t* outlen,
                                   int* al, void* arg) {
  if (ext_type != CUSTOM_EXT_TYPE_2) custom_ext_error = 1;
  *out = (const unsigned char*)custom_ext_cli_string;
  *outlen = strlen(custom_ext_cli_string);
  return 1; /* Send "abc" */
}
...
static int custom_ext_2_cli_parse_cb(SSL* s, unsigned int ext_type,
                                     ...
                                     int* al, void* arg) {
  if (ext_type != CUSTOM_EXT_TYPE_2) custom_ext_error = 1;
  if (inlen != 0) custom_ext_error = 1; /* Should be empty response */
  return 1;
}
...
static int custom_ext_3_cli_add_cb(SSL* s, unsigned int ext_type,
                                   const unsigned char** out, size_t* outlen,
                                   int* al, void* arg) {
  if (ext_type != CUSTOM_EXT_TYPE_3) custom_ext_error = 1;
  *out = (const unsigned char*)custom_ext_cli_string;
  *outlen = strlen(custom_ext_cli_string);
  return 1; /* Send "abc" */
}
...
static int custom_ext_3_cli_parse_cb(SSL* s, unsigned int ext_type,
                                     ...
                                     int* al, void* arg) {
  if (ext_type != CUSTOM_EXT_TYPE_3) custom_ext_error = 1;
  if (inlen != strlen(custom_ext_srv_string)) custom_ext_error = 1;
...
static int custom_ext_1_srv_parse_cb(SSL* s, unsigned int ext_type,
  ...
  if (ext_type != CUSTOM_EXT_TYPE_1) custom_ext_error = 1;
  /* Check for "abc" */
  if (inlen != strlen(custom_ext_cli_string)) custom_ext_error = 1;
  if (memcmp(in, custom_ext_cli_string, inlen) != 0) custom_ext_error = 1;
  return 1;
}
...
static void sv_usage(void) {
  ...
  fprintf(stderr,
          " -dhe2048      - use 2048 bit key (safe prime) for DHE (default, "
          "no-op)\n");
  ...
  fprintf(stderr,
          " -alpn_expected <string> - the ALPN protocol that should be "
          "negotiated\n");
  fprintf(stderr,
          " -server_min_proto <string> - Minimum version the server should "
          "support\n");
  fprintf(stderr,
          " -server_max_proto <string> - Maximum version the server should "
          "support\n");
  fprintf(stderr,
          " -client_min_proto <string> - Minimum version the client should "
          "support\n");
  fprintf(stderr,
          " -client_max_proto <string> - Maximum version the client should "
          "support\n");
  fprintf(stderr,
          " -should_negotiate <string> - The version that should be "
  ...
  fprintf(stderr,
          " -sn_server1 <string> - have server context 1 respond to this "
          "servername\n");
  fprintf(stderr,
          " -sn_server2 <string> - have server context 2 respond to this "
          "servername\n");
  ...
  fprintf(stderr,
          " -should_reuse <number>     - The expected state of reusing the "
          "session\n");
  fprintf(stderr, " -no_ticket    - do not issue TLS session ticket\n");
  fprintf(stderr, " -client_ktls  - try to enable client KTLS\n");
  fprintf(stderr, " -server_ktls  - try to enable server KTLS\n");
  fprintf(stderr,
          " -provider <name>    - Load the given provider into the library "
          "context\n");
  fprintf(stderr,
          " -config <cnf>    - Load the given config file into the library "
          "context\n");
}
...
static int protocol_from_string(const char* value) {
  ...
  for (i = 0; i < n; i++)
    if (strcmp(versions[i].name, value) == 0) return versions[i].version;
  return -1;
}
...
int main(int argc, char* argv[]) {
  ...
  while (argc >= 1) {
    if (strcmp(*argv, "-F") == 0) {
      ...
      if (rv == 0) rv = SSL_CONF_cmd_argv(s_cctx, &argc, &argv);
      /* Recognised: store it for later use */
      if (rv > 0) {
        if (rv == 1) argn = NULL;
        if (!conf_args) {
          conf_args = sk_OPENSSL_STRING_new_null();
          if (!conf_args) goto end;
        }
        if (!sk_OPENSSL_STRING_push(conf_args, arg)) goto end;
        if (!sk_OPENSSL_STRING_push(conf_args, argn)) goto end;
        continue;
      }
      if (rv == -3)
  ...
  if (ssl3 + tls1 + tls1_1 + tls1_2 + dtls + dtls1 + dtls12 > 1) {
    fprintf(stderr,
            "At most one of -ssl3, -tls1, -tls1_1, -tls1_2, -dtls, -dtls1 or "
            "-dtls12 should "
            "be requested.\n");
    goto end;
  }
  ...
  if (!ssl3 && !tls1 && !tls1_1 && !tls1_2 && !dtls && !dtls1 && !dtls12 &&
    ...
    fprintf(stderr,
            ...
            "or add one of -ssl3, -tls1, -tls1_1, -tls1_2, -dtls, -dtls1, "
            "-dtls12, -reuse\n"
  ...
  if (print_time) {
    ...
    if (number < 50 && !force)
      fprintf(stderr,
              "Warning: For accurate timings, use more connections (e.g. -num "
              "1000)\n");
  }
  ...
  if (SSL_CTX_set_min_proto_version(c_ctx, min_version) == 0) goto end;
  if (SSL_CTX_set_max_proto_version(c_ctx, max_version) == 0) goto end;
  if (SSL_CTX_set_min_proto_version(s_ctx, min_version) == 0) goto end;
  if (SSL_CTX_set_max_proto_version(s_ctx, max_version) == 0) goto end;
  ...
  for (i = 0; i < sk_OPENSSL_STRING_num(conf_args); i += 2) {
    ...
    if (rv == -2) {
      ...
      if (rv > 0) rv = SSL_CONF_cmd(s_cctx, arg, argn);
    }
    if (rv <= 0) {
  ...
  if (!no_dhe) {
    ...
    if (!SSL_CTX_set0_tmp_dh_pkey(s_ctx, dhpkey)) EVP_PKEY_free(dhpkey);
    if (!SSL_CTX_set0_tmp_dh_pkey(s_ctx2, dhpkey)) EVP_PKEY_free(dhpkey);
  }
#endif
  ...
  c_ssl = SSL_new(c_ctx);
  s_ssl = SSL_new(s_ctx);
  if (c_ssl == NULL || s_ssl == NULL) goto end;

  if (sn_client) SSL_set_tlsext_host_name(c_ssl, sn_client);
  if (client_ktls) SSL_set_options(c_ssl, SSL_OP_ENABLE_KTLS);
  if (server_ktls) SSL_set_options(s_ssl, SSL_OP_ENABLE_KTLS);
...
int doit_localhost(SSL* s_ssl, SSL* c_ssl, int family, long count,
  ...
  if (BIO_set_nbio(client, 1) <= 0) goto err;
  if (BIO_set_nbio(acpt, 1) <= 0) goto err;
    ...
    while (!st_connect || !st_accept) {
      if (!st_connect) {
        if (BIO_do_connect(client) <= 0) {
          if (!BIO_should_retry(client)) goto err;
        } else {
          st_connect = 1;
        }
      }
      if (!st_accept) {
        if (BIO_do_accept(acpt) <= 0) {
          if (!BIO_should_retry(acpt)) goto err;
        } else {
          st_accept = 1;
        }
      }
  ...
  if (!s_ssl_bio) goto err;

  c_ssl_bio = BIO_new(BIO_f_ssl());
  if (!c_ssl_bio) goto err;
...

    {
      ...
      if (cw_num > 0) {
        ...
        if (r < 0) {
          ...
          if (debug) printf("client wrote %d\n", r);
          cw_num -= r;
        }
      }
...

    {
      ...
      if (sw_num > 0) {
        ...
        if (r < 0) {
          ...
          if (debug) printf("server wrote %d\n", r);
          sw_num -= r;
        }
      }
...
#ifndef OPENSSL_NO_NEXTPROTONEG
  if (verify_npn(c_ssl, s_ssl) < 0) goto end;
#endif
...
int doit_biopair(SSL* s_ssl, SSL* c_ssl, long count, clock_t* s_time,
  ...
  if (!BIO_new_bio_pair(&server, bufsiz, &server_io, bufsiz)) goto err;
  if (!BIO_new_bio_pair(&client, bufsiz, &client_io, bufsiz)) goto err;
...

    {
      ...
      /* io2 to io1 */
      {
        ...
        if (num) {
          ...
          if (num > 1) --num; /* test restartability even more thoroughly */
  ...
  if (verbose) print_details(c_ssl, "DONE via BIO pair: ");
#ifndef OPENSSL_NO_NEXTPROTONEG
  if (verify_npn(c_ssl, s_ssl) < 0) goto end;
#endif
  if (verify_serverinfo() < 0) {
...
int doit(SSL* s_ssl, SSL* c_ssl, long count) {
  ...
  if ((cbuf = OPENSSL_zalloc(bufsiz)) == NULL) goto err;
  if ((sbuf = OPENSSL_zalloc(bufsiz)) == NULL) goto err;
      ...
      if (c_write) {
        ...
        if (i < 0) {
          ...
          if (BIO_should_retry(c_bio)) {
            if (BIO_should_read(c_bio)) c_r = 1;
            if (BIO_should_write(c_bio)) c_w = 1;
          } else {
            fprintf(stderr, "ERROR in CLIENT\n");
            err_in_client = 1;
            ERR_print_errors(bio_err);
          ...
          c_write = 0;
          cw_num -= i;
          if (max_frag > 1029) SSL_set_max_send_fragment(c_ssl, max_frag -= 5);
        }
      } else {
      ...
      if (!s_write) {
        ...
        if (i < 0) {
          ...
          if (BIO_should_retry(s_bio)) {
            if (BIO_should_read(s_bio)) s_r = 1;
            if (BIO_should_write(s_bio)) s_w = 1;
          } else {
            fprintf(stderr, "ERROR in SERVER\n");
            err_in_server = 1;
            ERR_print_errors(bio_err);
        ...
        if (i < 0) {
          ...
          s_write = 0;
          c_r = 1;
          if (sw_num <= 0) done |= S_DONE;
          if (max_frag > 1029) SSL_set_max_send_fragment(s_ssl, max_frag -= 5);
        }
      }
    }
  ...
  if (verbose) print_details(c_ssl, "DONE: ");
#ifndef OPENSSL_NO_NEXTPROTONEG
  if (verify_npn(c_ssl, s_ssl) < 0) goto err;
#endif
  if (verify_serverinfo() < 0) {
    fprintf(stderr, "Server info verify error\n");
    goto err;
...
static int app_verify_callback(X509_STORE_CTX* ctx, void* arg) {
  ...
  if (cb_arg->app_verify) {
    ...
    printf("Finished printing do we have a context? 0x%p a cert? 0x%p\n",
           (void*)ctx, (void*)c);
    if (c) s = X509_NAME_oneline(X509_get_subject_name(c), buf, 256);
    if (s != NULL) {
