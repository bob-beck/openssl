...
static int
test_spki_aid(X509_PUBKEY* pubkey, const char* filename)
{
  ...
  OSSL_PARAM params[] = { OSSL_PARAM_octet_string(
                            ALGORITHMID_NAME, &algid_prov, sizeof(algid_prov)),
                          OSSL_PARAM_END };
  int ret = 0;
...
static int
test_x509_spki_aid(X509* cert, const char* filename)
{
                  ...
                  const char* ca_filename)
{
  ...
  const OSSL_PARAM* gettable_params = NULL;
  OSSL_PARAM params[] = { OSSL_PARAM_octet_string(
                            "algorithm-id", &algid_prov, sizeof(algid_prov)),
                          OSSL_PARAM_END };
  int ret = 0;
  ...
  if (!TEST_ptr(mdctx = EVP_MD_CTX_new()) ||
      !TEST_true(EVP_DigestVerifyInit_ex(
        mdctx, &pctx, OBJ_nid2sn(dig_nid), NULL, NULL, pkey, NULL))) {
