...
static int test_spki_aid(X509_PUBKEY* pubkey, const char* filename) {
  ...
  OSSL_PARAM params[] = {OSSL_PARAM_octet_string(ALGORITHMID_NAME, &algid_prov,
                                                 sizeof(algid_prov)),
                         OSSL_PARAM_END};
  int ret = 0;
  ...
  if (!TEST_int_gt(OBJ_obj2txt(name, sizeof(name), oid, 0), 0)) goto end;
  ...
  algid_prov[0] = '\0';
  if (!TEST_true(evp_keymgmt_get_params(keymgmt, keydata, params))) goto end;
...
static int test_x509_sig_aid(X509* eecert, const char* ee_filename,
  ...
  if (!TEST_ptr(mdctx = EVP_MD_CTX_new()) ||
      !TEST_true(EVP_DigestVerifyInit_ex(mdctx, &pctx, OBJ_nid2sn(dig_nid),
                                         NULL, NULL, pkey, NULL))) {
    TEST_info(
        "Couldn't initialize a DigestVerify operation with "
  ...
  algid_prov[0] = '\0';
  if (!TEST_true(EVP_PKEY_CTX_get_params(pctx, params))) goto end;
  algid_prov_len = params[0].return_size;
...
int setup_tests(void) {
  ...
  if (x509) ADD_TEST(test_x509_files);
  if (spki) ADD_TEST(test_spki_file);
  return 1;
}
