...
static int test_bf_ecb_raw(int n) {
  ...
  data[1] = bf_plain[n][1];
  BF_encrypt(data, &key);
  if (!TEST_mem_eq(&(bf_cipher[n][0]), BF_BLOCK, &(data[0]), BF_BLOCK)) ret = 0;

  BF_decrypt(&(data[0]), &key);
  if (!TEST_mem_eq(&(bf_plain[n][0]), BF_BLOCK, &(data[0]), BF_BLOCK)) ret = 0;
...
static int test_bf_ecb(int n) {
  ...
  if (!TEST_mem_eq(&(cipher_data[n][0]), BF_BLOCK, out, BF_BLOCK)) ret = 0;

  BF_ecb_encrypt(out, out, &key, BF_DECRYPT);
  if (!TEST_mem_eq(&(plain_data[n][0]), BF_BLOCK, out, BF_BLOCK)) ret = 0;
...
static int test_bf_set_key(int n) {
  ...
  if (!TEST_mem_eq(out, 8, &(key_out[n][0]), 8)) ret = 0;
...
static int test_bf_cbc(void) {
  ...
  if (!TEST_mem_eq(cbc_out, 32, cbc_ok, 32)) ret = 0;

  memcpy(iv, cbc_iv, 8);
  BF_cbc_encrypt(cbc_out, cbc_in, len, &key, iv, BF_DECRYPT);
  if (!TEST_mem_eq(cbc_in, len, cbc_data, strlen(cbc_data) + 1)) ret = 0;
...
static int test_bf_cfb64(void) {
  ...
  BF_cfb64_encrypt((unsigned char*)&(cbc_data[13]), &(cbc_out[13]), len - 13,
                   &key, iv, &n, BF_ENCRYPT);
  if (!TEST_mem_eq(cbc_out, (int)len, cfb64_ok, (int)len)) ret = 0;
  ...
  BF_cfb64_encrypt(&(cbc_out[17]), &(cbc_in[17]), len - 17, &key, iv, &n,
                   BF_DECRYPT);
  if (!TEST_mem_eq(cbc_in, (int)len, cbc_data, (int)len)) ret = 0;
...
static int test_bf_ofb64(void) {
  ...
  BF_ofb64_encrypt((unsigned char*)&(cbc_data[13]), &(cbc_out[13]), len - 13,
                   &key, iv, &n);
  if (!TEST_mem_eq(cbc_out, (int)len, ofb64_ok, (int)len)) ret = 0;
