...
static int self_test_on_pairwise_fail(const OSSL_PARAM params[], void* arg) {
  ...
  if (p == NULL || p->data_type != OSSL_PARAM_UTF8_STRING) return 0;
  phase = (const char*)p->data;
  if (strcmp(phase, OSSL_SELF_TEST_PHASE_CORRUPT) == 0) {
    p = OSSL_PARAM_locate_const(params, OSSL_PROV_PARAM_SELF_TEST_TYPE);
    if (p == NULL || p->data_type != OSSL_PARAM_UTF8_STRING) return 0;
    type = (const char*)p->data;
    if (strcmp(type, args->type) == 0) return 0;
  }
  return 1;
}
...
static int setup_selftest_pairwise_failure(const char* type) {
  ...
  if (!TEST_ptr(prov = OSSL_PROVIDER_load(libctx, "fips"))) goto err;
...
static int test_keygen_pairwise_failure(void) {
  ...
  if (strcmp(pairwise_name, "rsa") == 0) {
    if (!TEST_true(setup_selftest_pairwise_failure(type))) goto err;
    if (!TEST_ptr_null(
            pkey = EVP_PKEY_Q_keygen(libctx, NULL, "RSA", (size_t)2048)))
      goto err;
  } else if (strncmp(pairwise_name, "ec", 2) == 0) {
    if (strcmp(pairwise_name, "eckat") == 0) type = OSSL_SELF_TEST_TYPE_PCT_KAT;
    if (!TEST_true(setup_selftest_pairwise_failure(type))) goto err;
    ...
    if (!TEST_true(setup_selftest_pairwise_failure(type))) goto err;
    if (!TEST_ptr(bio = BIO_new_file(dsaparam_file, "r"))) goto err;
    if (!TEST_ptr(pParams =
                      PEM_read_bio_Parameters_ex(bio, NULL, libctx, NULL)))
      goto err;
    if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_pkey(libctx, pParams, NULL)))
      goto err;
    if (!TEST_int_eq(EVP_PKEY_keygen_init(ctx), 1)) goto err;
    if (!TEST_int_le(EVP_PKEY_keygen(ctx, &pkey), 0)) goto err;
    if (!TEST_ptr_null(pkey)) goto err;
    ...
    if (!TEST_true(setup_selftest_pairwise_failure(type))) goto err;
    if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(libctx, "SLH-DSA-SHA2-256f",
                                                   NULL)))
      goto err;
    if (!TEST_int_eq(EVP_PKEY_keygen_init(ctx), 1)) goto err;
    if (!TEST_int_le(EVP_PKEY_keygen(ctx, &pkey), 0)) goto err;
