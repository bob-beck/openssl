...
static int test_x509_req_detect_invalid_version(void) {
  ...
  if (certFilePath == NULL) goto err;
  if (!TEST_ptr(bio = BIO_new_file(certFilePath, "r"))) goto err;
  ...
  if (!TEST_ptr(pkey = X509_REQ_get_pubkey(req))) goto err;
  /* Verification MUST fail at this point. ret != 1. */
  if (!TEST_int_ne(X509_REQ_verify(req, pkey), 1)) goto err;
  ret = 1; /* success */
err:
  EVP_PKEY_free(pkey);
  X509_REQ_free(req);
