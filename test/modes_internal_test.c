...
static int test_gcm128(int idx) {
  ...
  if (P.size == 1) P.data = NULL;
  if (C.size == 1) C.data = NULL;
  ...
  memset(out, 0, P.size);
  if (A.data != NULL) CRYPTO_gcm128_aad(&ctx, A.data, A.size);
  if (P.data != NULL)
  ...
  memset(out, 0, P.size);
  if (A.data != NULL) CRYPTO_gcm128_aad(&ctx, A.data, A.size);
  if (C.data != NULL) CRYPTO_gcm128_decrypt(&ctx, C.data, out, P.size);
  if (!TEST_false(CRYPTO_gcm128_finish(&ctx, T.data, 16)) ||
