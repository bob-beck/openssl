...
static int
validate_client_hello(BIO* wbio)
{
  PACKET pkt, pkt2;
  long len;
  unsigned char* data;
...
#define SH_SESSID_OFS 60 /* Session ID goes here */
  ...
  if (!EVP_DigestUpdate(handshake_md,
                        server_hello + MAC_OFFSET,
...
send_finished(SSL* s, BIO* rbio)
{
  ...
  /* Derive key material */
  do_PRF(TLS_MD_KEY_EXPANSION_CONST,
  ...
  do_PRF(TLS_MD_SERVER_FINISH_CONST,
         ...
         NULL,
         0,
         finished_msg + DTLS1_HM_HEADER_LENGTH,
         TLS1_FINISH_MAC_LENGTH);

  return send_record(
    rbio, SSL3_RT_HANDSHAKE, 0, finished_msg, sizeof(finished_msg));
}

static int
validate_ccs(BIO* wbio)
{
  PACKET pkt;
  long len;
  unsigned char* data;
