...
static int validate_client_hello(BIO* wbio) {
  ...
  if ((len = BIO_get_mem_data(wbio, (char**)&data)) < 0) return 0;
  if (!PACKET_buf_init(&pkt, data, len)) return 0;

  /* Check record header type */
  if (!PACKET_get_1(&pkt, &u) || u != SSL3_RT_HANDSHAKE) return 0;
  /* Version */
  if (!PACKET_get_net_2(&pkt, &u) || u != DTLS1_BAD_VER) return 0;
  /* Skip the rest of the record header */
  if (!PACKET_forward(&pkt, DTLS1_RT_HEADER_LENGTH - 3)) return 0;

  /* Check it's a ClientHello */
  if (!PACKET_get_1(&pkt, &u) || u != SSL3_MT_CLIENT_HELLO) return 0;
  /* Skip the rest of the handshake message header */
  if (!PACKET_forward(&pkt, DTLS1_HM_HEADER_LENGTH - 1)) return 0;

  /* Check client version */
  if (!PACKET_get_net_2(&pkt, &u) || u != DTLS1_BAD_VER) return 0;

  /* Store random */
  if (!PACKET_copy_bytes(&pkt, client_random, SSL3_RANDOM_SIZE)) return 0;
  ...
  if (PACKET_remaining(&pkt2)) {
    if (!PACKET_equal(&pkt2, cookie, sizeof(cookie))) return 0;
    cookie_found = 1;
  }
  ...
  /* Skip compression */
  if (!PACKET_get_1(&pkt, &u) || !PACKET_forward(&pkt, u)) return 0;

  /* Skip extensions */
  if (!PACKET_get_net_2(&pkt, &u) || !PACKET_forward(&pkt, u)) return 0;

  /* Now we are at the end */
  if (PACKET_remaining(&pkt)) return 0;
...
static int send_record(BIO* rbio, unsigned char type, uint64_t seqnr,
  ...
  if (!TEST_int_gt(RAND_bytes(iv, sizeof(iv)), 0) ||
      ...
      !TEST_true(EVP_CipherInit_ex(enc_ctx, EVP_aes_128_cbc(), NULL, enc_key,
                                   iv, 1)) ||
...
static int send_finished(SSL* s, BIO* rbio) {
  ...
  if (!EVP_DigestFinal_ex(handshake_md, handshake_hash, NULL)) return 0;
...
static int validate_ccs(BIO* wbio) {
  ...
  /* Check record header type */
  if (!PACKET_get_1(&pkt, &u) || u != SSL3_RT_CHANGE_CIPHER_SPEC) return 0;
  /* Version */
  if (!PACKET_get_net_2(&pkt, &u) || u != DTLS1_BAD_VER) return 0;
  /* Skip the rest of the record header */
  if (!PACKET_forward(&pkt, DTLS1_RT_HEADER_LENGTH - 3)) return 0;
  ...
  if (!PACKET_get_1(&pkt, &u) || u != SSL3_MT_CCS) return 0;
...
static int test_bad_dtls(void) {
  ...
  con = SSL_new(ctx);
  if (!TEST_ptr(con) || !TEST_true(SSL_set_session(con, sess))) goto end;

  rbio = BIO_new(BIO_s_mem());
  wbio = BIO_new(BIO_s_mem());

  if (!TEST_ptr(rbio) || !TEST_ptr(wbio)) goto end;
  ...
  ret = SSL_do_handshake(con);
  if (!TEST_int_gt(ret, 0) || !TEST_true(validate_ccs(wbio))) goto end;
  ...
  for (i = 0; i < (int)OSSL_NELEM(tests); i++) {
    ...
    if (!TEST_true(recv_buf[0] == tests[i].seq)) goto end;
  }

  /* The last test cannot be DROP() */
  if (!TEST_false(tests[i - 1].drop)) goto end;
