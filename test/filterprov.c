...
static void filter_unquery(void* provctx, int operation_id,
  ...
  for (i = 0; i < globs->num_dispatch; i++)
    if (globs->dispatch[i].alg == algs) return;
...
int filter_provider_init(const OSSL_CORE_HANDLE* handle,
  ...
  if (!ossl_prov_bio_from_dispatch(in)) return 0;
  ...
  if (ourglobals.libctx == NULL) goto err;

  ourglobals.deflt = OSSL_PROVIDER_load(ourglobals.libctx, "default");
  if (ourglobals.deflt == NULL) goto err;
...
int filter_provider_set_filter(int operation, const char* filterstr) {
  ...
  if (filterstrtmp == NULL) goto err;

  /* Nothing to filter */
  if (provalgs == NULL) goto err;

  if (globs->num_dispatch >= MAX_FILTERS) goto err;
  ...
  for (name = filterstrtmp; !last; name = (sep == NULL ? NULL : sep + 1)) {
    ...
    for (algs = provalgs; algs->algorithm_names != NULL; algs++) {
      ...
      if (found[namelen] != '\0' && found[namelen] != ':') continue;
      if (found != algs->algorithm_names && found[-1] != ':') continue;

      /* We found a match */
      if (algnum >= MAX_ALG_FILTERS) goto err;
