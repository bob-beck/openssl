...
static void
tear_down(CT_TEST_FIXTURE* fixture)
{
  if (fixture != NULL) {
    CTLOG_STORE_free(fixture->ctlog_store);
...
static X509*
load_pem_cert(const char* dir, const char* file)
{
  X509* cert = NULL;
...
static int
compare_sct_list_printout(STACK_OF(SCT)* sct, const char* expected_output)
{
...
static int
compare_extension_printout(X509_EXTENSION* extension,
...
static int
assert_validity(CT_TEST_FIXTURE* fixture,
                STACK_OF(SCT)* scts,
...
static int
execute_cert_test(CT_TEST_FIXTURE* fixture)
{
  int success = 0;
  ...
  if (fixture->sct_text_file != NULL) {
    sct_text_len = read_text_file(fixture->sct_dir,
  ...
  if (fixture->tls_sct_list != NULL) {
    ...
    if (!TEST_mem_eq(fixture->tls_sct_list,
...
static int
test_no_scts_in_certificate(void)
{
...
static int
test_one_sct_in_certificate(void)
{
  SETUP_CT_TEST_FIXTURE();
  fixture->certs_dir = certs_dir;
...
static int
test_multiple_scts_in_certificate(void)
{
  SETUP_CT_TEST_FIXTURE();
  fixture->certs_dir = certs_dir;
...
static int
test_verify_multiple_scts(void)
{
  SETUP_CT_TEST_FIXTURE();
  fixture->certs_dir = certs_dir;
  fixture->certificate_file = "embeddedSCTs3.pem";
...
static int
test_verify_fails_for_future_sct(void)
{
  SETUP_CT_TEST_FIXTURE();
...
test_encode_tls_sct(void)
{
  ...
  if (!TEST_ptr(sct = SCT_new_from_base64(SCT_VERSION_V1,
                                          log_id,
 ...
 */
static int
test_default_ct_policy_eval_ctx_time_is_now(void)
{
  int success = 0;
  CT_POLICY_EVAL_CTX* ct_policy_ctx = CT_POLICY_EVAL_CTX_new();
