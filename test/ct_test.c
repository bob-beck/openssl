...
set_up (const char *const test_case_name)
{
  ...
  if (!TEST_ptr (fixture->ctlog_store = CTLOG_STORE_new ())
      || !TEST_int_eq (CTLOG_STORE_load_default_file (fixture->ctlog_store),
                       1))
    goto end;
  return fixture;
...
static int
read_text_file (const char *dir, const char *file, char *buffer,
                int buffer_length)
{
  int len = -1;
  char *file_path = test_mk_file_path (dir, file);
...
static int
assert_validity (CT_TEST_FIXTURE *fixture, STACK_OF (SCT) *scts,
