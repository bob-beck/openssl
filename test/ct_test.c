...
static CT_TEST_FIXTURE* set_up(const char* const test_case_name) {
  ...
  if (!TEST_ptr(fixture = OPENSSL_zalloc(sizeof(*fixture)))) goto end;
  ...
  if (fixture != NULL) CTLOG_STORE_free(fixture->ctlog_store);
...
static X509* load_pem_cert(const char* dir, const char* file) {
  ...
  if (file_path != NULL) {
    ...
    if (cert_io != NULL) cert = PEM_read_bio_X509(cert_io, NULL, NULL, NULL);
    BIO_free(cert_io);
  }
...
static int read_text_file(const char* dir, const char* file, char* buffer,
  ...
  if (file_path != NULL) {
    ...
    if (file_io != NULL) len = BIO_read(file_io, buffer, buffer_length);
    BIO_free(file_io);
  }
...
static int compare_sct_list_printout(STACK_OF(SCT)* sct,
  ...
  if (!TEST_ptr(text_buffer = BIO_new(BIO_s_mem()))) goto end;

  SCT_LIST_print(sct, text_buffer, 0, "\n", NULL);

  /* Append \0 because we're about to use the buffer contents as a string. */
  if (!TEST_true(BIO_write(text_buffer, "\0", 1))) goto end;

  BIO_get_mem_data(text_buffer, &actual_output);
  if (!TEST_str_eq(actual_output, expected_output)) goto end;
  result = 1;
...
static int assert_validity(CT_TEST_FIXTURE* fixture, STACK_OF(SCT)* scts,
  ...
  if (!TEST_int_ge(SCT_LIST_validate(scts, policy_ctx), 0)) return 0;
...
static int execute_cert_test(CT_TEST_FIXTURE* fixture) {
  ...
  if (fixture->sct_text_file != NULL) {
    ...
    if (!TEST_int_ge(sct_text_len, 0)) goto end;
  ...
  if (fixture->certificate_file != NULL) {
    ...
    if (fixture->expected_sct_count > 0) {
      if (!TEST_ptr(sct_extension)) goto end;
  ...
  if (fixture->tls_sct_list != NULL) {
    ...
    if (!TEST_ptr(o2i_SCT_LIST(&scts, &p, fixture->tls_sct_list_len))) goto end;
...
#ifndef OPENSSL_NO_CT
  if ((ct_dir = getenv("CT_DIR")) == NULL) ct_dir = "ct";
  if ((certs_dir = getenv("CERTS_DIR")) == NULL) certs_dir = "certs";
