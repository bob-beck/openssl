...
#define IS_X_86 1
size_t
OPENSSL_ia32_rdrand_bytes(unsigned char* buf, size_t len);
size_t
OPENSSL_ia32_rdseed_bytes(unsigned char* buf, size_t len);
#else
#define IS_X_86 0
#endif
...
static int
sanity_check_bytes(size_t (*rng)(unsigned char*, size_t),
                   int rounds,
                   int min_failures,
                   int max_retries,
                   int max_zero_words)
...
#elif IS_AARCH_64
static int
sanity_check_rndr_bytes(void)
{
  return sanity_check_bytes(OPENSSL_rndr_bytes, 1000, 0, 10, 10);
}

static int
sanity_check_rndrrs_bytes(void)
{
