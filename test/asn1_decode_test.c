...
static int test_gentime(void) {
  ...
  if (!TEST_ptr_null(gentime)) rc = 0; /* fail */
...
static int test_utctime(void) {
  ...
  if (!TEST_ptr_null(utctime)) rc = 0; /* fail */
...
static int test_reuse_asn1_object(void) {
  ...
  if (!TEST_ptr(d2i_ASN1_OBJECT(&obj, &p, sizeof(oid_der)))) goto err;
  ret = 1;
err:
