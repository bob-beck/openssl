...
static int test_digestbyname(void) {
  ...
  if (!TEST_ptr(nm)) return 0;
  id = ossl_namemap_add_name(nm, 0, "SHA256");
  if (!TEST_int_ne(id, 0)) return 0;
  ...
  if (!TEST_ptr_eq(sha256, foo)) return 0;
...
static int test_cipherbyname(void) {
  ...
  if (!TEST_ptr(aes128)) return 0;
  bar = EVP_get_cipherbyname("bar");
  if (!TEST_ptr_eq(aes128, bar)) return 0;
