...
static int tmpmd_get_params(OSSL_PARAM params[]) {
  ...
  if (p != NULL && !OSSL_PARAM_set_size_t(p, 1)) return 0;
...
static int test_default_props_and_providers(int propsorder) {
  ...
  if (!TEST_ptr(testprov = OSSL_PROVIDER_load(libctx, "testprov"))) goto err;
  ...
  if (propsorder == DEFAULT_PROPS_AFTER_FETCH) {
    if (!TEST_true(EVP_set_default_properties(libctx, MYPROPERTIES))) goto err;
