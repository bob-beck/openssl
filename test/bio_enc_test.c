...
static int
do_bio_cipher(const EVP_CIPHER* cipher,
...
static int
do_test_bio_cipher(const EVP_CIPHER* cipher, int idx)
{
  switch (idx) {
    case 0:
...
static int
test_bio_enc_chacha20_poly1305(int idx)
{
...
static int
test_bio_enc_eof_read_flush(void)
{
  ...
  if (!TEST_ptr(cipher) ||
      ...
      !TEST_int_gt(
        EVP_CIPHER_CTX_ctrl(ctx, EVP_CTRL_GCM_GET_TAG, sizeof(tag), tag), 0))
    goto end;
  BIO_free(cbio);
  BIO_free(b64);
  b64 = cbio = NULL;
