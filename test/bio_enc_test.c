...
static int do_bio_cipher(const EVP_CIPHER* cipher, const unsigned char* key,
  ...
  if (!TEST_int_gt(RAND_bytes(inp, DATA_SIZE), 0)) return 0;
  ...
  if (!TEST_true(BIO_set_cipher(b, cipher, key, iv, ENCRYPT))) goto err;
  mem = BIO_new_mem_buf(inp, DATA_SIZE);
  if (!TEST_ptr(mem)) goto err;
  BIO_push(b, mem);
  ...
  b = BIO_new(BIO_f_cipher());
  if (!TEST_ptr(b)) return 0;
  if (!TEST_true(BIO_set_cipher(b, cipher, key, iv, DECRYPT))) goto err;
  ...
  if (!TEST_mem_eq(inp, DATA_SIZE, out, len)) return 0;
