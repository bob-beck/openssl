...
static int
check_session_ticket_app_data (HANDSHAKE_RESULT *result,
...
static int
check_ca_names (const char *name, STACK_OF (X509_NAME) *expected_names,
      ...
      if (test_ctx->handshake_mode == SSL_TEST_HANDSHAKE_RESUME)
        {
          ...
          if (!TEST_true (
                  SSL_CTX_set_max_proto_version (resume_client_ctx, 0)))
            goto err;
...
setup_tests (void)
{
  ...
  if (!TEST_ptr (conf = NCONF_new (NULL))
      ...
      || !TEST_int_ne (
          NCONF_get_number_e (conf, NULL, "num_tests", &num_tests), 0))
    {
