...
static const char*
print_alert(int alert)
{
  return alert ? SSL_alert_desc_string_long(alert) : "no alert";
}

static int
check_result(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
...
static int
check_alerts(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
...
static int
check_protocol(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
...
static int
check_servername(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
...
static int
check_session_ticket(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
...
static int
check_session_id(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
...
static int
check_compression(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
...
static int
check_npn(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  int ret = 1;
...
static int
check_alpn(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  int ret = 1;
...
static int
check_session_ticket_app_data(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  size_t result_len = 0;
...
static int
check_resumption(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
...
static int
check_nid(const char* name, int expected_nid, int nid)
{
  if (expected_nid == 0 || expected_nid == nid)
    return 1;
  TEST_error("%s type mismatch, %s vs %s\n",
             name,
             OBJ_nid2ln(expected_nid),
             nid == NID_undef ? "absent" : OBJ_nid2ln(nid));
  return 0;
}

static void
print_ca_names(STACK_OF(X509_NAME)* names)
{
  int i;
  ...
  for (i = 0; i < sk_X509_NAME_num(names); i++) {
    X509_NAME_print_ex(
      bio_err, sk_X509_NAME_value(names, i), 4, XN_FLAG_ONELINE);
    BIO_puts(bio_err, "\n");
  }
}

static int
check_ca_names(const char* name,
...
static int
check_tmp_key(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  return check_nid(
    "Tmp key", test_ctx->expected_tmp_key_type, result->tmp_key_type);
}

static int
check_server_cert_type(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  return check_nid("Server certificate",
                   test_ctx->expected_server_cert_type,
                   result->server_cert_type);
}

static int
check_server_sign_hash(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  return check_nid("Server signing hash",
                   test_ctx->expected_server_sign_hash,
                   result->server_sign_hash);
}

static int
check_server_sign_type(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  return check_nid("Server signing",
                   test_ctx->expected_server_sign_type,
                   result->server_sign_type);
}

static int
check_server_ca_names(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
...
static int
check_client_cert_type(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  return check_nid("Client certificate",
                   test_ctx->expected_client_cert_type,
                   result->client_cert_type);
}

static int
check_client_sign_hash(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  return check_nid("Client signing hash",
                   test_ctx->expected_client_sign_hash,
                   result->client_sign_hash);
}

static int
check_client_sign_type(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  return check_nid("Client signing",
                   test_ctx->expected_client_sign_type,
                   result->client_sign_type);
}

static int
check_client_ca_names(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
...
static int
check_cipher(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
 ...
 */
static int
check_test(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx)
{
  int ret = 1;
...
test_handshake(int idx)
{
  ...
  result = do_handshake(server_ctx,
                        server2_ctx,
                        client_ctx,
                        resume_server_ctx,
                        resume_client_ctx,
