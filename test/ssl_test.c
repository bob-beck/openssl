...
static int check_alerts(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx) {
  ...
  if (!TEST_int_le(result->client_num_fatal_alerts_sent, 1)) return 0;
  if (!TEST_int_le(result->server_num_fatal_alerts_sent, 1)) return 0;
  return 1;
}
...
static int check_session_id(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx) {
  if (test_ctx->session_id_expected == SSL_TEST_SESSION_ID_IGNORE) return 1;
...
static int check_session_ticket_app_data(HANDSHAKE_RESULT* result,
  ...
  if (result_len == 0 && expected_len == 0) return 1;
...
static int check_resumption(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx) {
  if (!TEST_int_eq(result->client_resumed, result->server_resumed)) return 0;
...
static int check_nid(const char* name, int expected_nid, int nid) {
  if (expected_nid == 0 || expected_nid == nid) return 1;
...
static int check_ca_names(const char* name, STACK_OF(X509_NAME)* expected_names,
  ...
  if (expected_names == NULL) return 1;
  if (names == NULL || sk_X509_NAME_num(names) == 0) {
    if (TEST_int_eq(sk_X509_NAME_num(expected_names), 0)) return 1;
    goto err;
  }
  if (sk_X509_NAME_num(names) != sk_X509_NAME_num(expected_names)) goto err;
  for (i = 0; i < sk_X509_NAME_num(names); i++) {
...
static int check_cipher(HANDSHAKE_RESULT* result, SSL_TEST_CTX* test_ctx) {
  if (test_ctx->expected_cipher == NULL) return 1;
  if (!TEST_ptr(result->cipher)) return 0;
  if (!TEST_str_eq(test_ctx->expected_cipher, result->cipher)) return 0;
  return 1;
}
...
static int test_handshake(int idx) {
  ...
  if (test_ctx->method == SSL_TEST_METHOD_DTLS) {
    ...
    if (!TEST_true(SSL_CTX_set_max_proto_version(client_ctx, 0))) goto err;
  ...
  if (!TEST_true(OPENSSL_init_ssl(OPENSSL_INIT_LOAD_CONFIG, NULL))) goto err;
#endif
  ...
  if (result != NULL) ret = check_test(result, test_ctx);
