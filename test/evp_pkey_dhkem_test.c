...
static int
test_dhkem_encapsulate(int tstid)
{
  int ret = 0;
...
static int
test_dhkem_decapsulate(int tstid)
{
  int ret = 0;
  ...
  if (!TEST_ptr(rpriv = new_raw_private_key(
                  t->curve, t->rpriv, t->rprivlen, t->rpub, t->rpublen)))
    goto err;
  if (t->spub != NULL) {
...
static int
test_ec_dhkem_derivekey_fail(void)
{
  int ret = 0;
  EVP_PKEY* pkey = NULL;
  OSSL_PARAM params[3];
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
    OSSL_PKEY_PARAM_GROUP_NAME, "secp256k1", 0);
  params[1] = OSSL_PARAM_construct_octet_string(
    OSSL_PKEY_PARAM_DHKEM_IKM, (char*)t->ikm, t->ikmlen);
...
static int
test_no_operation_set(int tstid)
{
  EVP_PKEY_CTX* ctx = rctx[tstid];
  const TEST_ENCAPDATA* t = &ec_encapdata[tstid];
  size_t len = 0;
  ...
  return TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1) &&
         ...
         TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, NULL), 1) &&
         TEST_int_eq(EVP_PKEY_decapsulate(
                       ctx, NULL, &len, t->expected_enc, t->expected_enclen),
                     1);
}

/* Fail if the ikm is too small */
static int
test_ikm_small(int tstid)
{
...
static int
test_input_size_small(int tstid)
{
  int ret = 0;
...
static int
test_ec_auth_key_curve_mismatch(void)
{
  int ret = 0;
  EVP_PKEY* auth = NULL;
...
static int
test_auth_key_type_mismatch(int tstid)
{
  int id1 = tstid;
  int id2 = !tstid;
...
static int
test_ec_invalid_private_key(void)
{
  int ret = 0;
  EVP_PKEY* priv = NULL;
  ...
  ret = TEST_ptr(priv = new_raw_private_key(
                   "P-256", order, sizeof(order), t->rpub, t->rpublen)) &&
...
static int
test_ec_public_key_infinity(void)
{
  int ret = 0;
  EVP_PKEY* key = NULL;
  ...
  ret = TEST_ptr(key = new_raw_private_key(
                   t->curve, t->rpriv, t->rprivlen, tmp, sizeof(tmp))) &&
        ...
        TEST_int_eq(EVP_PKEY_decapsulate(
                      keyctx, s, &slen, t->expected_enc, t->expected_enclen),
                    0)
...
test_null_params(int tstid)
{
  ...
  return TEST_int_eq(EVP_PKEY_auth_encapsulate_init(ctx, NULL, opparam), 0) &&
         ...
         TEST_int_eq(EVP_PKEY_decapsulate(
                       ctx, NULL, NULL, t->expected_enc, t->expected_enclen),
                     0)
...
test_noauthpublic(int tstid)
{
  ...
  TEST_note("%s %s", t->curve, encap ? "Encap" : "Decap");
  if (!TEST_ptr(auth = new_raw_private_key(
                  t->curve, t->rpriv, t->rprivlen, NULL, expected)))
    goto err;
...
static int
test_ec_dhkem_derivekey(int tstid)
{
  int ret = 0;
  EVP_PKEY* pkey = NULL;
  OSSL_PARAM params[3];
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
    OSSL_PKEY_PARAM_GROUP_NAME, (char*)t->curvename, 0);
  params[1] = OSSL_PARAM_construct_octet_string(
    OSSL_PKEY_PARAM_DHKEM_IKM, (char*)t->ikm, t->ikmlen);
  params[2] = OSSL_PARAM_construct_end();
...
test_ec_noikme(int tstid)
{
  ...
  TEST_note(
    "testing encap/decap of curve %s%s\n", curve, auth ? " with auth" : "");
  ...
  ret =
    ...
    TEST_int_eq(
      EVP_PKEY_encapsulate(
        ctx, sender_pub, &sender_publen, sender_secret, &sender_secretlen),
      1) &&
    (sender_auth == NULL ||
    ...
    TEST_int_eq(
      EVP_PKEY_decapsulate(
        ctx, recip_secret, &recip_secretlen, sender_pub, sender_publen),
      1) &&
...
static int
do_ec_curve_failtest(const char* curve)
{
  int ret;
  EVP_PKEY* key = NULL;
  EVP_PKEY_CTX* ctx = NULL;
...
static int
test_ec_curve_nonnist(void)
{
  return do_ec_curve_failtest("secp256k1");
}

static int
test_ec_curve_unsupported(void)
{
...
test_ec_badpublic(int tstid)
{
  ...
  if (encap) {
    ...
    if (!TEST_int_eq(
          EVP_PKEY_decapsulate(
            ctx, secret, &secretlen, t->expected_enc, t->expected_enclen),
          0))
      goto err;
  }
...
static int
test_ec_invalid_decap_enc_buffer(void)
{
  const TEST_ENCAPDATA* t = &ec_encapdata[TEST_KEYTYPE_P256];
  unsigned char enc[256];
  unsigned char secret[256];
  ...
  return TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, opparam), 1) &&
         TEST_int_eq(EVP_PKEY_decapsulate(
                       ctx, secret, &secretlen, enc, t->expected_enclen),
                     0);
}
...
static int
test_ecx_dhkem_derivekey(int tstid)
{
  int ret;
  OSSL_PARAM params[2];
  EVP_PKEY_CTX* genctx;
  ...
  ret =
    ...
    TEST_int_eq(
      EVP_PKEY_get_octet_string_param(
        pkey, OSSL_PKEY_PARAM_PRIV_KEY, privkey, sizeof(privkey), &privkeylen),
      1) &&
...
/* Fail if the auth key has a different curve */
static int
test_ecx_auth_key_curve_mismatch(void)
{
  int ret = 0;
  EVP_PKEY* auth = NULL;
...
static int
test_ed_curve_unsupported(void)
{
  int ret;
  EVP_PKEY* key = NULL;
  EVP_PKEY_CTX* ctx = NULL;
