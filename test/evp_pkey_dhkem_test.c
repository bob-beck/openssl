...
static int test_input_size_small(int tstid) {
  ...
  if (!TEST_int_eq(EVP_PKEY_encapsulate(ctx, enc, &enclen, sec, &seclen), 0))
    goto err;
  seclen++;
  if (!TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, opparam), 1)) goto err;
...
static int test_set_params(int tstid) {
  ...
  if (!TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, badparams), 0)) goto err;
  ...
  badparams[1] = OSSL_PARAM_construct_end();
  if (!TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, badparams), 1)) goto err;
...
static int test_nopublic(int tstid) {
  ...
  if (!TEST_ptr(
    ...
    goto err;
  if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_pkey(libctx, priv, NULL))) goto err;
...
static int test_ec_noikme(int tstid) {
  ...
  ret = TEST_ptr(ctx = EVP_PKEY_CTX_new_from_pkey(libctx, recip, NULL)) &&
        (sender_auth == NULL ||
         TEST_int_eq(EVP_PKEY_auth_encapsulate_init(ctx, sender_auth, NULL),
                     1)) &&
        (sender_auth != NULL ||
         TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, NULL), 1)) &&
        ...
        TEST_int_eq(EVP_PKEY_encapsulate(ctx, sender_pub, &sender_publen,
                    ...
                    1) &&
        (sender_auth == NULL ||
         TEST_int_eq(EVP_PKEY_auth_decapsulate_init(ctx, sender_auth, NULL),
                     1)) &&
        (sender_auth != NULL ||
         TEST_int_eq(EVP_PKEY_decapsulate_init(ctx, NULL), 1)) &&
        TEST_int_eq(EVP_PKEY_CTX_set_kem_op(ctx, op), 1) &&
...
static int test_ec_badpublic(int tstid) {
  ...
  if (encap) {
    ...
    if (!TEST_int_eq(EVP_PKEY_encapsulate_init(ctx, opparam), 1)) goto err;
    if (!TEST_int_eq(
            EVP_PKEY_encapsulate(ctx, enc, &enclen, secret, &secretlen), 0))
      goto err;
  } else {
...
static int test_ec_badauth(int tstid) {
  ...
  pub[0] = 0;
  if (!TEST_ptr(
          auth = new_raw_private_key(t->curve, t->rpriv, t->rprivlen, pub, 1)))
    goto err;
  if (encap) {
...
static int test_ecx_auth_key_curve_mismatch(void) {
  ...
  if (!TEST_ptr(auth = EVP_PKEY_Q_keygen(libctx, NULL, "X448"))) return 0;
