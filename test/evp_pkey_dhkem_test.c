...
test_dhkem_decapsulate (int tstid)
{
  ...
  if (t->spub != NULL)
    {
      if (!TEST_ptr (spub
                     = new_raw_public_key (t->curve, t->spub, t->spublen)))
        goto err;
    }
...
test_input_size_small (int tstid)
{
  ...
  if (!TEST_int_eq (EVP_PKEY_encapsulate_init (ctx, opparam), 1)
      || !TEST_int_eq (
          EVP_PKEY_encapsulate (ctx, NULL, &enclen, NULL, &seclen), 1))
    goto err;
...
test_ec_auth_key_curve_mismatch (void)
{
  ...
  ret = TEST_int_eq (EVP_PKEY_auth_encapsulate_init (rctx[0], auth, opparam),
                     0);
  EVP_PKEY_free (auth);
  return ret;
}
...
test_null_params (int tstid)
{
  ...
  return TEST_int_eq (EVP_PKEY_auth_encapsulate_init (ctx, NULL, opparam), 0)
         && TEST_int_eq (EVP_PKEY_auth_decapsulate_init (ctx, NULL, opparam),
                         0)
         ...
         && TEST_int_eq (EVP_PKEY_encapsulate (ctx, NULL, NULL, NULL, NULL),
                         0);
}
...
test_ec_noikme (int tstid)
{
  ...
  ret = TEST_ptr (ctx = EVP_PKEY_CTX_new_from_pkey (libctx, recip, NULL))
        ...
        && (sender_auth != NULL
            || TEST_int_eq (EVP_PKEY_decapsulate_init (ctx, NULL), 1))
        && TEST_int_eq (EVP_PKEY_CTX_set_kem_op (ctx, op), 1)
        && TEST_int_eq (EVP_PKEY_decapsulate (ctx, recip_secret,
                                              &recip_secretlen, sender_pub,
                                              sender_publen),
                        1)
