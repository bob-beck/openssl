...
#else
static const char alpn_prots[] =
    "O1234567890123456789012345678901234567890123456789012345678901234567890123"
    "456789"
    "O1234567890123456789012345678901234567890123456789012345678901234567890123"
    "456789";
#endif
...
static int test_client_hello(int currtest) {
  ...
  if (!TEST_true(SSL_CTX_set_max_proto_version(ctx, 0))) goto end;
  ...
  if (currtest == TEST_SET_SESSION_TICK_DATA_VER_NEG) {
    if (!TEST_true(
            SSL_set_session_ticket_ext(con, dummytick, (int)strlen(dummytick))))
      goto end;
  }
  ...
  if (currtest == TEST_PADDING_NOT_NEEDED) testresult = 1;
