...
static int gen_bytes(EVP_RAND_CTX* drbg, unsigned char* buf, int num) {
  ...
  if (meth != NULL && meth != RAND_OpenSSL()) {
    if (meth->bytes != NULL) return meth->bytes(buf, num);
    return -1;
  }
#endif

  if (drbg != NULL) return EVP_RAND_generate(drbg, buf, num, 0, 0, NULL, 0);
  return 0;
}
...
static unsigned int query_rand_uint(EVP_RAND_CTX* drbg, const char* name) {
  ...
  *params = OSSL_PARAM_construct_uint(name, &n);
  if (EVP_RAND_CTX_get_params(drbg, params)) return n;
  return 0;
}
...
static time_t reseed_time(EVP_RAND_CTX* drbg) {
  ...
  *params = OSSL_PARAM_construct_time_t(OSSL_DRBG_PARAM_RESEED_TIME, &t);
  if (EVP_RAND_CTX_get_params(drbg, params)) return t;
  return 0;
}
...
static int test_drbg_reseed(int expect_success, EVP_RAND_CTX* primary,
  ...
  if (public_random == NULL) public_random = dummy;

  if (private_random == NULL) private_random = dummy;
  ...
  if (reseed_when == 0) reseed_when = time(NULL);
  ...
  if (!TEST_int_eq(rand_bytes((unsigned char*)public_random, RANDOM_SIZE),
                   expect_success) ||
      !TEST_int_eq(rand_priv_bytes((unsigned char*)private_random, RANDOM_SIZE),
                   expect_success))
    return 0;
  ...
  if (expect_primary_reseed >= 0) {
    ...
    if (!TEST_int_ge(reseed_counter(primary), primary_reseed)) return 0;
  }
...
static int compare_drbg_fork_result(const void* left, const void* right) {
  ...
  if (result == 0) result = memcmp(l->random, r->random, RANDOM_SIZE);

  if (result == 0) result = l->pindex - r->pindex;
...
static int test_drbg_reseed_in_child(EVP_RAND_CTX* primary,
  ...
  if (!TEST_int_ge(pipe(fd), 0)) return 0;
...
static int test_rand_reseed_on_fork(EVP_RAND_CTX* primary, EVP_RAND_CTX* public,
  ...
  for (i = 2, psample = sample + 2; i < sizeof(sample); i += 2, psample += 2) {
    if (compare_rand_chunk(psample - 2, psample) == 0) ++duplicate[0];
  }
...
static int test_rand_reseed(void) {
  ...
  if (using_fips_rng()) return TEST_skip("CRNGT cannot be disabled");

#ifndef OPENSSL_NO_DEPRECATED_3_0
  /* Check whether RAND_OpenSSL() is the default method */
  if (!TEST_ptr_eq(RAND_get_rand_method(), RAND_OpenSSL())) return 0;
#endif
   ...
   */
  if (!TEST_true(test_drbg_reseed(1, primary, public, private, NULL, NULL, 0, 0,
                                  0, 0)))
    goto error;
  ...
  if (!TEST_true(test_drbg_reseed(1, primary, public, private, NULL, NULL, 0, 1,
                                  1, 0)))
    goto error;
...
static int test_multi_thread(void) {
  ...
  if (!TEST_true(multi_thread_rand_bytes_succeeded)) return 0;
  if (!TEST_true(multi_thread_rand_priv_bytes_succeeded)) return 0;
