...
static int test_evp_cipher_api_safety(void) {
   ...
   */
  if (!TEST_int_eq(EVP_CIPHER_CTX_get_block_size(ctx), 0)) goto err_free;

  /*
   * Ensure that EVP_CIPHER_get_iv_length returns 0
   * if we haven't initialized the cipher in this context
   */
  if (!TEST_int_eq(EVP_CIPHER_CTX_get_iv_length(ctx), 0)) goto err_free;
...
static int test_dsa_param_keygen(int tstid) {
  ...
  if (!TEST_true(EVP_PKEY_assign_DSA(pkey_parm, dsa))) goto err;
  dsa = NULL;
...
static int do_dh_param_keygen(int tstid, const BIGNUM** bn) {
  ...
  if (!TEST_true(EVP_PKEY_assign_DH(pkey_parm, dh))) goto err;
  dh = NULL;
...
static int test_cipher_reinit(int test_id) {
  ...
  if (!TEST_ptr(cipher = EVP_CIPHER_fetch(libctx, name, NULL))) goto err;
...
static int test_cipher_reinit_partialupdate(int test_id) {
  ...
  if (!TEST_int_gt(in_len, 0)) goto err;
  ...
  if (EVP_CIPHER_get_iv_length(cipher) != 0)
    if (!TEST_mem_eq(out1, out1_len, out2, out2_len)) goto err;
  ...
  if (EVP_CIPHER_get_mode(cipher) != EVP_CIPH_SIV_MODE) {
    ...
    if (EVP_CIPHER_get_iv_length(cipher) != 0)
      if (!TEST_mem_eq(out1, out1_len, out3, out3_len)) goto err;
  }
  ret = 1;
err:
  EVP_CIPHER_free(cipher);
...
static int rsa_keygen(int bits, EVP_PKEY** pub, EVP_PKEY** priv) {
  ...
  if (!TEST_ptr(*priv = EVP_PKEY_Q_keygen(libctx, NULL, "RSA", (size_t)bits)) ||
      !TEST_ptr(ectx = OSSL_ENCODER_CTX_new_for_pkey(*priv, EVP_PKEY_PUBLIC_KEY,
                                                     "DER", "type-specific",
                                                     NULL)) ||
      !TEST_true(OSSL_ENCODER_to_data(ectx, &pub_der, &len)))
    goto err;
  pp = pub_der;
  if (!TEST_ptr(d2i_PublicKey(EVP_PKEY_RSA, pub, &pp, (long)len))) goto err;
  ret = 1;
err:
...
static int kem_rsa_gen_recover(void) {
  ...
  ret =
      ...
      TEST_int_eq(EVP_PKEY_encapsulate(dctx, NULL, &ctlen, NULL, &secretlen),
                  1) &&
      TEST_size_t_eq(ctlen, secretlen) && TEST_size_t_eq(ctlen, bits / 8) &&
      ...
      TEST_int_eq(EVP_PKEY_decapsulate(rctx, unwrap, &unwraplen, ct, ctlen),
                  1) &&
...
static int kem_rsa_params(void) {
  ...
  ret =
      ...
      /* Unless older FIPS, test encapsulate succeeds even if the mode is not
         set */
      ...
      (is_fips_lt_3_5 ||
       (TEST_int_eq(
        ...
        TEST_int_eq(
            EVP_PKEY_encapsulate(pubctx, ct, &ctlen, secret, &secretlen), 1)))
      ...
      && TEST_int_eq(EVP_PKEY_decapsulate(privctx, NULL, NULL, ct, sizeof(ct)),
                     0) &&
...
static int kem_invalid_keytype(void) {
  ...
  if (!TEST_ptr(key = gen_dh_key())) goto done;

  if (!TEST_ptr(sctx = EVP_PKEY_CTX_new_from_pkey(libctx, key, NULL)))
    goto done;
  if (!TEST_int_eq(EVP_PKEY_encapsulate_init(sctx, NULL), -2)) goto done;
...
int setup_tests(void) {
  ...
  if (strcmp(prov_name, "fips") == 0) is_fips = 1;
  ...
  if (!TEST_ptr(cipher_names = sk_OPENSSL_STRING_new(name_cmp))) return 0;
