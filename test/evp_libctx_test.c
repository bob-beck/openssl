...
static int
test_evp_cipher_api_safety (void)
{
  int ret = 0;
...
static int
test_dsa_param_keygen (int tstid)
{
  int ret = 0;
  int expected;
...
#ifndef OPENSSL_NO_DH
static int
do_dh_param_keygen (int tstid, const BIGNUM **bn)
{
  int ret = 0;
  int expected;
  EVP_PKEY_CTX *gen_ctx = NULL;
 ...
 */
static int
test_dh_safeprime_param_keygen (int tstid)
{
  static const BIGNUM *bn[]
...
static int
test_cipher_reinit (int test_id)
{
 ...
 * EVP_EncryptUpdate() in order to test that the second init/update is not
 * using a leftover buffer from the first init/update. Note: some ciphers don't
 * need a full block to produce output.
 */
static int
test_cipher_reinit_partialupdate (int test_id)
{
  int ret = 0, in_len;
  int out1_len = 0, out2_len = 0, out3_len = 0;
  EVP_CIPHER *cipher = NULL;
...
static int
name_cmp (const char *const *a, const char *const *b)
{
  return OPENSSL_strcasecmp (*a, *b);
}

static void
collect_cipher_names (EVP_CIPHER *cipher, void *cipher_names_list)
{
...
static int
rsa_keygen (int bits, EVP_PKEY **pub, EVP_PKEY **priv)
{
  int ret = 0;
...
kem_rsa_gen_recover (void)
{
  ...
  ret = TEST_true (rsa_keygen (bits, &pub, &priv))
        ...
        && TEST_int_eq (
            EVP_PKEY_decapsulate (rctx, NULL, &unwraplen, ct, ctlen), 1)
        && TEST_int_eq (
  ...
  ret = TEST_true (rsa_keygen (2048, &pub, &priv))
        ...
        && (is_fips_lt_3_5
            || (TEST_int_eq (EVP_PKEY_encapsulate (pubctx, NULL, &ctlen, NULL,
                ...
                && TEST_int_eq (EVP_PKEY_encapsulate (pubctx, ct, &ctlen,
                                                      secret, &secretlen),
                                1)))
        ...
        && TEST_int_eq (
            EVP_PKEY_encapsulate (pubctx, ct, &ctlen, secret, NULL), 1)
        && TEST_int_eq (
            EVP_PKEY_encapsulate (pubctx, NULL, &ctlen, NULL, NULL), 1)
        ...
        && TEST_int_eq (EVP_PKEY_decapsulate (privctx, secret, &secretlen,
                        ...
                        0)
        && TEST_int_eq (
            EVP_PKEY_decapsulate (privctx, secret, &secretlen, ct, 0), 0);
