...
async_free(BIO* a);
static int
async_read(BIO* b, char* out, int outl);
static int
async_write(BIO* b, const char* in, int inl);
static long
async_ctrl(BIO* b, int cmd, long num, void* ptr);
static int
async_gets(BIO* bp, char* buf, int size);
static int
async_puts(BIO* bp, const char* str);
...
static int
async_read(BIO* bio, char* out, int outl)
{
  struct async_ctrs* ctrs;
  int ret = 0;
...
static int
async_write(BIO* bio, const char* in, int inl)
{
  struct async_ctrs* ctrs;
  int ret = 0;
  int written = 0;
...
static long
async_ctrl(BIO* bio, int cmd, long num, void* ptr)
{
  long ret;
  BIO* next = BIO_next(bio);
...
static int
async_gets(BIO* bio, char* buf, int size)
{
...
static int
async_puts(BIO* bio, const char* str)
{
...
test_asyncio(int test)
{
  ...
  if (!TEST_true(create_ssl_objects(serverctx,
                                    clientctx,
                                    &serverssl,
                                    &clientssl,
                                    s_to_c_fbio,
