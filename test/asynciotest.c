...
static int async_write(BIO* bio, const char* in, int inl) {
  ...
  if (ctrs->wctr > 0) {
    ...
    if (fragment) {
      ...
      if (!PACKET_buf_init(&pkt, (const unsigned char*)in, inl)) return -1;
...
static int test_asyncio(int test) {
  ...
  for (j = 0; j < 2; j++) {
    ...
    if (!TEST_mem_eq(testdata, sizeof(testdata), buf, len)) goto end;
  }
