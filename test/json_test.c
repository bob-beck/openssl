...
static int helper_ensure(struct helper* h) {
  ...
  if (!TEST_ptr(h->mem_bio = BIO_new(BIO_s_mem()))) return 0;
...
OPJ_END_A()
END_SCRIPT_EXPECTING_S(
    "[1,-1,9007199254740991,\"9007199254740992\",-9007199254740991,\"-"
    "9007199254740992\"]")
...
#define GET_P() (GET_WORD().p)
  ...
  for (;;) switch (GET_U64()) {
      ...
      case OPK_CALL: {
        ...
        if (!TEST_true(helper_ensure(&h))) goto err;
      ...
      case OPK_ASSERT_ERROR: {
        ...
        if (!TEST_int_eq(ossl_json_in_error(&h.j), asserted)) goto err;
...
stop:
  ...
  if (!TEST_true(ossl_json_flush(&h.j))) goto err;

  /* Implicit error check if not done explicitly. */
  if (asserted < 0 && !TEST_false(ossl_json_in_error(&h.j))) goto err;

  if (!TEST_true(BIO_get_mem_ptr(h.mem_bio, &bufp))) goto err;
  ...
  ok = 1;
err:
  if (!ok) TEST_error("script '%s' failed", info->name);
...
static int test_json_enc(void) {
  ...
  for (i = 0; i < OSSL_NELEM(scripts); ++i)
    if (!TEST_true(run_script(scripts[i]()))) ok = 0;
