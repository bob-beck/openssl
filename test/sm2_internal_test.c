...
static int
test_sm2_crypt (const EC_GROUP *group, const EVP_MD *digest,
...
static int
test_sm2_sign (const EC_GROUP *group, const char *userid,
               const char *privkey_hex, const char *message, const char *k_hex,
               const char *r_hex, const char *s_hex, int omit_pubkey)
{
  const size_t msg_len = strlen (message);
  int ok = 0;
  BIGNUM *priv = NULL;
