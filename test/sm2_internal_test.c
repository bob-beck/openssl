...
static EC_GROUP* create_EC_group(const char* p_hex, const char* a_hex,
  ...
  if (!TEST_ptr(group)) goto done;

  generator = EC_POINT_new(group);
  if (!TEST_ptr(generator)) goto done;
...
static int test_sm2_crypt(const EC_GROUP* group, const EVP_MD* digest,
  ...
  if (!TEST_ptr(ctext)) goto done;

  start_fake_rand(k_hex);
  if (!TEST_true(ossl_sm2_encrypt(key, digest, (const uint8_t*)message, msg_len,
                                  ctext, &ctext_len))) {
    restore_rand();
    goto done;
  }
  ...
  if (!TEST_mem_eq(ctext, ctext_len, expected, ctext_len)) goto done;
...
static int sm2_crypt_test(void) {
  ...
  if (!TEST_ptr(test_group)) goto done;
  ...
  if (!TEST_ptr(gm_group)) goto done;
...
static int test_sm2_sign(const EC_GROUP* group, const char* userid,
  ...
  if (!TEST_true(BN_hex2bn(&priv, privkey_hex))) goto done;
  ...
  start_fake_rand(k_hex);
  sig = ossl_sm2_do_sign(key, EVP_sm3(), (const uint8_t*)userid, strlen(userid),
