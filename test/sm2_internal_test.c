...
static int
start_fake_rand(const char* hex_bytes)
{
...
static int
test_sm2_crypt(const EC_GROUP* group,
               const EVP_MD* digest,
               ...
               const char* ctext_hex)
{
  ...
  if (!TEST_true(ossl_sm2_encrypt(
        key, digest, (const uint8_t*)message, msg_len, ctext, &ctext_len))) {
    restore_rand();
    goto done;
  }
  restore_rand();
  ...
  if (!TEST_ptr(recovered) ||
      !TEST_true(ossl_sm2_decrypt(
        key, digest, ctext, ctext_len, recovered, &recovered_len)) ||
...
static int
test_sm2_sign(const EC_GROUP* group,
              const char* userid,
              const char* privkey_hex,
              const char* message,
              const char* k_hex,
