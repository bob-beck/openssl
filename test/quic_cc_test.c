...
static int net_sim_init(struct net_sim* s, const OSSL_CC_METHOD* ccm,
  ...
  if (!TEST_ptr(s->pkts = ossl_pqueue_NET_PKT_new(net_pkt_cmp))) return 0;
...
static int net_sim_send(struct net_sim* s, size_t sz) {
   ...
   */
  if (!TEST_true(net_sim_process(s, 0))) goto err;
  ...
  if (!TEST_true(s->ccm->on_data_sent(s->cc, sz))) goto err;

  if (!TEST_true(ossl_pqueue_NET_PKT_push(s->pkts, pkt, &pkt->idx))) goto err;
...
static int net_sim_process_one(struct net_sim* s, int skip_forward) {
  ...
  if (pkt->success && !pkt->arrived &&
    ...
    pkt->next_time = pkt->determination_time;
    if (!ossl_pqueue_NET_PKT_push(s->pkts, pkt, &pkt->idx)) return 0;

    return 1;
  }

  if (ossl_time_compare(fake_time, pkt->determination_time) < 0) return 2;

  if (!TEST_true(!pkt->success || pkt->arrived)) return 0;
  ...
  if (!pkt->success) {
    ...
    if (!TEST_true(s->ccm->on_data_lost(s->cc, &loss_info))) return 0;
    ...
    if (!TEST_true(s->ccm->on_data_acked(s->cc, &ack_info))) return 0;
...
static int net_sim_process(struct net_sim* s, size_t skip_forward) {
  ...
  while ((rc = net_sim_process_one(s, skip_forward > 0 ? 1 : 0)) == 1)
    if (skip_forward > 0) --skip_forward;
...
static int test_simulate(void) {
  ...
  if (!TEST_ptr(cc = ccm->new(fake_now, NULL))) goto err;

  if (!TEST_true(net_sim_init(&sim, ccm, cc, actual_capacity, 100))) goto err;
  ...
  *p++ = OSSL_PARAM_construct_end();
  ...
  if (!TEST_true(ccm->set_input_params(cc, params))) goto err;
  ...
  *p++ = OSSL_PARAM_construct_end();
  ...
  if (!TEST_true(ccm->bind_diagnostics(cc, params))) goto err;

  ccm->reset(cc);

  if (!TEST_uint64_t_ge(allowance = ccm->get_tx_allowance(cc), mdpl)) goto err;
  ...
  while (total_sent < total_to_send) {
    ...
    for (;;) {
      ...
      if (!TEST_true(net_sim_send(&sim, (size_t)sz))) goto err;
     ...
     */
    if (rc == 3) {
      if (!TEST_uint64_t_eq(diag_cur_bytes_in_flight, 0)) goto err;

      if (!TEST_uint64_t_ge(ccm->get_tx_allowance(cc), mdpl)) goto err;
    }
   ...
   */
  {
    ...
    /* Max 5% error */
    if (!TEST_double_le(error, 0.05)) goto err;
  }

  testresult = 1;
err:
  if (have_sim) net_sim_cleanup(&sim);

  if (cc != NULL) ccm->free(cc);
...
static int test_sanity(void) {
  ...
  *p++ = OSSL_PARAM_construct_end();
  ...
  if (!TEST_uint64_t_ge(allowance = ccm->get_tx_allowance(cc), 1472)) goto err;

  /* There is TX allowance so wakeup should be immediate */
  if (!TEST_true(ossl_time_is_zero(ccm->get_wakeup_deadline(cc)))) goto err;

  /* No bytes should currently be in flight. */
  if (!TEST_uint64_t_eq(diag_cur_bytes_in_flight, 0)) goto err;

  /* Tell the CC we have sent some data. */
  if (!TEST_true(ccm->on_data_sent(cc, 1200))) goto err;

  /* Allowance should have decreased. */
  if (!TEST_uint64_t_eq(ccm->get_tx_allowance(cc), allowance - 1200)) goto err;

  /* Acknowledge the data. */
  ack_info.tx_time = fake_time;
  ack_info.tx_size = 1200;
  step_time(100);
  if (!TEST_true(ccm->on_data_acked(cc, &ack_info))) goto err;
  ...
  if (!TEST_true(ccm->on_data_invalidated(cc, 1200))) goto err;

  /* Allowance should have returned. */
  if (!TEST_uint64_t_eq(ccm->get_tx_allowance(cc), allowance2)) goto err;
  ...
  if (!TEST_true(ccm->on_data_sent(cc, 1300))) goto err;
  ...
  if (!TEST_true(ccm->on_data_lost(cc, &loss_info))) goto err;

  loss_info.tx_size = 1300;
  if (!TEST_true(ccm->on_data_lost(cc, &loss_info))) goto err;

  if (!TEST_true(ccm->on_data_lost_finished(cc, 0))) goto err;

  /* Allowance should have changed due to the lost calls */
  if (!TEST_uint64_t_ne(ccm->get_tx_allowance(cc), allowance2)) goto err;

  /* But it should not be as high as the original value */
  if (!TEST_uint64_t_lt(ccm->get_tx_allowance(cc), allowance)) goto err;
