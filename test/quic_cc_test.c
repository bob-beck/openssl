 ...
 * bandwidth and latency. Sending a packet into the network causes it to
 * consume some capacity of the network until the packet exits the network.
 * Note that the capacity is not known to the congestion controller as the
 * entire point of a congestion controller is to correctly estimate this
 * capacity and this is what we are testing. The network simulator does take
 * care of informing the congestion controller of ack/loss events automatically
 * but the caller is responsible for querying the congestion controller and
 * choosing the size of simulated transmitted packets.
 */
...
static int
net_pkt_cmp (const NET_PKT *a, const NET_PKT *b)
{
...
static int
net_sim_init (struct net_sim *s, const OSSL_CC_METHOD *ccm, OSSL_CC_DATA *cc,
...
static void
net_sim_cleanup (struct net_sim *s)
{
...
static int
net_sim_send (struct net_sim *s, size_t sz)
{
...
static int
net_sim_process_one (struct net_sim *s, int skip_forward)
{
...
static int
net_sim_process (struct net_sim *s, size_t skip_forward)
{
  int rc;
...
static int
dump_state (const OSSL_CC_METHOD *ccm, OSSL_CC_DATA *cc, struct net_sim *s)
{
#ifdef GENERATE_LOG
