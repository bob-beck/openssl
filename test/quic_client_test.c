...
static int test_quic_client_ex(int fd_arg) {
  ...
  if (fd_arg == INVALID_SOCKET) {
    ...
    if (!TEST_int_ne(c_fd, INVALID_SOCKET)) goto err;
  ...
  if (s_addr_ != NULL && !BIO_dgram_set_peer(c_net_bio, s_addr_)) goto err;

  if (!TEST_ptr(c_ctx = SSL_CTX_new(OSSL_QUIC_client_method()))) goto err;
  ...
  for (;;) {
    ...
    if (c_write_done && !c_shutdown && c_total_read < sizeof(msg2) - 1) {
      ...
      if (ret != 1) {
        ...
        if (!TEST_size_t_lt(c_total_read, sizeof(msg2) - 1)) goto err;
      }
    }
...
err:
  ...
  if (fd_arg == INVALID_SOCKET && c_fd != INVALID_SOCKET) BIO_closesocket(c_fd);
  return testresult;
}
...
err:
  if (c_fd != INVALID_SOCKET) close(c_fd);
  return (0);
}
