...
static int test_bad_issuer_crl(void)
{
    ...
    r = TEST_ptr(bad_issuer_crl)
        && TEST_int_eq(
            verify(test_leaf, test_root, make_CRL_stack(bad_issuer_crl, NULL), X509_V_FLAG_CRL_CHECK, PARAM_TIME),
            X509_V_ERR_UNABLE_TO_GET_CRL);
...
static int test_crl_empty_idp(void)
{
    ...
    r = TEST_ptr(empty_idp_crl)
        && TEST_int_eq(
            verify(test_leaf2, test_root2, make_CRL_stack(empty_idp_crl, NULL), X509_V_FLAG_CRL_CHECK, PARAM_TIME2),
            X509_V_ERR_UNABLE_TO_GET_CRL);
