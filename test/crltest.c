...
static const char *kCRLTestRoot2[] = {"-----BEGIN CERTIFICATE-----\n",
...
static const char *kCRLTestLeaf2[] = {"-----BEGIN CERTIFICATE-----\n",
...
static const char *kEmptyIdpCRL[] = {"-----BEGIN X509 CRL-----\n",
 ...
 */
static int verify(X509 *leaf, X509 *root, STACK_OF(X509_CRL) *crls, unsigned long flags, time_t verification_time)
{
...
static int test_basic_crl(void)
{
    ...
    r = TEST_ptr(basic_crl) && TEST_ptr(revoked_crl) &&
        TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(basic_crl, NULL), X509_V_FLAG_CRL_CHECK, PARAM_TIME),
                    X509_V_OK) &&
        TEST_int_eq(
            verify(test_leaf, test_root, make_CRL_stack(basic_crl, revoked_crl), X509_V_FLAG_CRL_CHECK, PARAM_TIME),
            X509_V_ERR_CERT_REVOKED);
    if (r)
    {
        X509_CRL_get0_signature(basic_crl, NULL, &alg);
        tbsalg = X509_CRL_get0_tbs_sigalg(basic_crl);
        r = TEST_ptr(alg) && TEST_ptr(tbsalg) && TEST_int_eq(X509_ALGOR_cmp(alg, tbsalg), 0);
    }
...
{
    return TEST_int_eq(verify(test_leaf, test_root, NULL, X509_V_FLAG_CRL_CHECK, PARAM_TIME),
...
static int test_bad_issuer_crl(void)
{
    ...
    r = TEST_ptr(bad_issuer_crl) && TEST_int_eq(verify(test_leaf, test_root, make_CRL_stack(bad_issuer_crl, NULL),
...
static int test_crl_empty_idp(void)
{
    ...
    r = TEST_ptr(empty_idp_crl) && TEST_int_eq(verify(test_leaf2, test_root2, make_CRL_stack(empty_idp_crl, NULL),
...
static int test_known_critical_crl(void)
{
    ...
    r = TEST_ptr(known_critical_crl) &&
        TEST_int_eq(
            verify(test_leaf, test_root, make_CRL_stack(known_critical_crl, NULL), X509_V_FLAG_CRL_CHECK, PARAM_TIME),
            X509_V_OK);
...
static int test_unknown_critical_crl(int n)
{
    ...
    r = TEST_ptr(unknown_critical_crl) &&
        TEST_int_eq(
            verify(test_leaf, test_root, make_CRL_stack(unknown_critical_crl, NULL), X509_V_FLAG_CRL_CHECK, PARAM_TIME),
