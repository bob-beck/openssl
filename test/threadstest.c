...
static int get_new_uid(void) {
  ...
  if (!TEST_true(CRYPTO_THREAD_write_lock(tsan_lock))) return 0;
  r = ++current_uid;
  if (!TEST_true(CRYPTO_THREAD_unlock(tsan_lock))) return 0;
  return r;
...
static void rwwriter_fn(int id, int* iterations) {
  ...
  for (count = 0;; count++) {
    ...
    if (contention == 0) OSSL_sleep(1000);
    if (!CRYPTO_THREAD_write_lock(rwtorturelock)) abort();
    ...
    old = rwwriter_ptr;
    rwwriter_ptr = new;
    if (!CRYPTO_THREAD_unlock(rwtorturelock)) abort();
    if (old != NULL) CRYPTO_free(old, __FILE__, __LINE__);
    t2 = ossl_time_now();
    if ((ossl_time2seconds(t2) - ossl_time2seconds(t1)) >= 4) break;
  }
  *iterations = count;
  return;
}
...
static void rwreader_fn(int* iterations) {
  ...
  if (CRYPTO_THREAD_read_lock(rwtorturelock) == 0) abort();
  ...
  while (lw1 != 1 || lw2 != 1) {
    ...
    if (CRYPTO_THREAD_unlock(rwtorturelock) == 0) abort();
    *iterations = count;
...
static int _torture_rw(void) {
  ...
  if (!TEST_ptr(rwtorturelock) || !TEST_ptr(atomiclock)) goto out;
  ...
  TEST_info(
      "performed %d reads and %d writes over 2 read and 2 write threads in %e "
      "seconds",
  ...
  if (TEST_int_eq(rw_torture_result, 1)) ret = 1;
out:
...
static void writer_fn(int id, int* iterations) {
  ...
  for (count = 0;; count++) {
    ...
    *new = global_ctr++;
    ...
    if (contention == 0) ossl_rcu_call(rcu_lock, free_old_rcu_data, old);
...
static int _torture_rcu(void) {
  ...
  atomiclock = CRYPTO_THREAD_lock_new();
  if (!TEST_ptr(atomiclock)) goto out;
  ...
  if (!TEST_int_eq(rcu_torture_result, 1)) goto out;

  rc = 1;
out:
  ossl_rcu_lock_free(rcu_lock);
  CRYPTO_THREAD_lock_free(atomiclock);
  if (!TEST_int_eq(rcu_torture_result, 1)) return 0;
...
static void thread_local_thread_cb(void) {
  ...
  ptr = CRYPTO_THREAD_get_local(&thread_local_key);
  if (!TEST_ptr_eq(ptr, &destructor_run_count)) return;
...
static int test_thread_local(void) {
  ...
  ptr = CRYPTO_THREAD_get_local(&thread_local_key);
  if (!TEST_ptr_null(ptr)) return 0;

#if !defined(OPENSSL_SYS_WINDOWS)
  if (!TEST_int_eq(destructor_run_count, 1)) return 0;
#endif
#endif

  if (!TEST_true(CRYPTO_THREAD_cleanup_local(&thread_local_key))) return 0;
  return 1;
}
...
static int test_atomic(void) {
  ...
  if (CRYPTO_atomic_add(&val, 1, &ret, NULL)) {
    ...
    if (!TEST_int_eq(val, 1) || !TEST_int_eq(val, ret)) goto err;
  } else {
    /* This failed therefore we're on a platform without lockless atomics */
    if (!TEST_int_eq(val, 0) || !TEST_int_eq(val, ret)) goto err;
  }
  val = 0;
  ret = 0;

  if (!TEST_true(CRYPTO_atomic_add(&val, 1, &ret, lock))) goto err;
  if (!TEST_int_eq(val, 1) || !TEST_int_eq(val, ret)) goto err;
  ...
  if (!TEST_true(CRYPTO_atomic_or(&val64, 2, &ret64, lock))) goto err;
  ...
  ret64 = 0;
  if (!TEST_true(CRYPTO_atomic_load(&val64, &ret64, lock))) goto err;
  ...
  if (!TEST_true(CRYPTO_atomic_and(&val64, 5, &ret64, lock))) goto err;
  ...
  if (!TEST_true(CRYPTO_atomic_add64(&val64, 2, &ret64, lock))) goto err;
...
static int teardown_threads(void) {
  ...
  for (i = 0; i < multi_num_threads; i++)
    if (!TEST_true(wait_for_thread(multi_threads[i]))) return 0;
  return 1;
}
...
static int start_threads(size_t n, void (*thread_func)(void)) {
  ...
  if (!TEST_size_t_le(multi_num_threads + n, MAXIMUM_THREADS)) return 0;
...
static int thread_run_test(void (*main_func)(void), size_t num_threads,
  ...
  if (main_func != NULL) main_func();

  if (!teardown_threads() || !TEST_true(multi_success)) goto err;
  testresult = 1;
err:
  thead_teardown_libctx();
  return testresult;
  ...
  for (i = 0; i < 5; i++) {
    if (!TEST_true(EVP_EncryptInit_ex(cipherctx, ciph, NULL, key, iv)) ||
        !TEST_true(EVP_EncryptUpdate(cipherctx, out, &ciphoutl,
                                     (unsigned char*)message, (int)messlen)) ||
...
err:
  ...
  EVP_PKEY_free(pkey);
  if (!testresult) multi_set_success(0);
}
...
err:
  EVP_PKEY_CTX_free(ctx);
  if (!success) multi_set_success(0);
}
...
static int test_multi_general_worker_fips_provider(void) {
  if (!do_fips) return TEST_skip("FIPS not supported");
  return thread_run_test(&thread_general_worker, 2, &thread_general_worker, 1,
                         fips_provider);
}
...
static void thread_downgrade_shared_evp_pkey(void) {
   ...
   * downgrading
   */
  if (EVP_PKEY_get0_RSA(shared_evp_pkey) == NULL) multi_set_success(0);
}
...
static int test_multi_shared_pkey_release(void) {
  ...
  if (!thread_setup_libctx(
    ...
    goto err;
  for (; i < 10; ++i) {
    if (!TEST_true(EVP_PKEY_up_ref(shared_evp_pkey))) goto err;
  }

  if (!start_threads(10, &thread_release_shared_pkey)) goto err;
  i = 0;
...
static int test_multi_load_unload_provider(void) {
  ...
  if (!start_threads(2, &thread_provider_load_unload)) goto err;
...
static void test_bio_dgram_pair_worker(void) {
  ...
  if (!TEST_int_eq(RAND_bytes_ex(multi_libctx, &ch, 1, 64), 1)) goto err;
...
static void test_pem_read_one(void) {
  ...
  if (key == NULL) multi_set_success(0);
...
static void test_x509_store_by_subject(void) {
  ...
  if (!TEST_true(X509_STORE_CTX_init(ctx, store, NULL, NULL))) goto err;

  name = X509_NAME_new();
  if (!TEST_ptr(name)) goto err;
  if (!TEST_true(X509_NAME_add_entry_by_txt(
          name, "CN", MBSTRING_ASC, (unsigned char*)"Root CA", -1, -1, 0)))
    goto err;
  obj = X509_STORE_CTX_get_obj_by_subject(ctx, X509_LU_X509, name);
  if (!TEST_ptr(obj)) goto err;
...
static int test_x509_store(void) {
  ...
  if (!TEST_ptr(store)) return 0;
  if (!TEST_true(X509_STORE_load_store(store, storedir))) goto err;
...
int setup_tests(void) {
  ...
  if (!TEST_ptr(storedir)) return 0;

  if (!TEST_ptr(global_lock = CRYPTO_THREAD_lock_new())) return 0;

#ifdef TSAN_REQUIRES_LOCKING
  if (!TEST_ptr(tsan_lock = CRYPTO_THREAD_lock_new())) return 0;
#endif
