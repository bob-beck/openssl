...
static void
rwwriter_fn(int id, int* iterations)
{
  int count;
  int *old, *new;
  OSSL_TIME t1, t2;
...
static void
free_old_rcu_data(void* data)
{
  CRYPTO_free(data, NULL, 0);
}

static void
writer_fn(int id, int* iterations)
{
  int count;
  OSSL_TIME t1, t2;
  uint64_t *old, *new;
...
static void
thread_local_destructor(void* arg)
{
  unsigned* count;
 ...
 */
static int
test_thread_local_multi_key(void)
{
  int dummy;
  int i;
...
static int
thread_setup_libctx(int libctx, const char* providers[])
{
  size_t n;
...
static int
start_threads(size_t n, void (*thread_func)(void))
{
  size_t i;
...
static int
thread_run_test(void (*main_func)(void),
                size_t num_threads,
  ...
  for (i = 0; i < 5; i++) {
    if (!TEST_true(EVP_EncryptInit_ex(cipherctx, ciph, NULL, key, iv)) ||
        !TEST_true(EVP_EncryptUpdate(
          cipherctx, out, &ciphoutl, (unsigned char*)message, (int)messlen)) ||
...
static void
thread_multi_simple_fetch(void)
{
  EVP_MD* md = EVP_MD_fetch(multi_libctx, "SHA2-256", NULL);
...
thread_shared_evp_pkey(void)
{
  ...
  for (i = 0; i < 1 + do_fips; i++) {
    ...
    if (!TEST_int_ge(EVP_PKEY_encrypt_init(ctx), 0) ||
        !TEST_int_ge(EVP_PKEY_encrypt(
                       ctx, ctbuf, &ctlen, (unsigned char*)msg, strlen(msg)),
                     0))
      goto err;
...
static void
thread_provider_load_unload(void)
{
...
static int
test_multi_general_worker_default_provider(void)
{
  return thread_run_test(
    &thread_general_worker, 2, &thread_general_worker, 1, default_provider);
}

static int
test_multi_general_worker_fips_provider(void)
{
  if (!do_fips)
    return TEST_skip("FIPS not supported");
  return thread_run_test(
    &thread_general_worker, 2, &thread_general_worker, 1, fips_provider);
}

static int
test_multi_fetch_worker(void)
{
...
static int
test_multi_shared_pkey_common(void (*worker)(void))
{
  int testresult = 0;
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
static void
thread_downgrade_shared_evp_pkey(void)
{
  /*
...
static int
test_multi_downgrade_shared_pkey(void)
{
...
static int
test_multi_shared_pkey_release(void)
{
  int testresult = 0;
  size_t i = 1;
...
static int
test_multi_load_unload_provider(void)
{
  EVP_MD* sha256 = NULL;
...
test_multi_default(void)
{
  ...
  return thread_run_test(
    &thread_multi_simple_fetch, 2, &thread_multi_simple_fetch, 0, NULL);
}

static int
test_multi_load(void)
{
  int res = 1;
  OSSL_PROVIDER* prov;
  ...
  return thread_run_test(
           NULL, MAXIMUM_THREADS, &test_multi_load_worker, 0, NULL) &&
         res;
}

static void
test_obj_create_one(void)
{
...
static void
test_bio_dgram_pair_worker(void)
{
  ossl_unused int r;
  int ok = 0;
  uint8_t ch = 0;
   ...
   */
  if (ch & 2)
    r = BIO_sendmmsg(ch & 1 ? multi_bio2 : multi_bio1,
                     &msg,
                     sizeof(BIO_MSG),
                     1,
                     0,
    ...
    r = BIO_recvmmsg(ch & 1 ? multi_bio2 : multi_bio1,
                     &msg,
                     sizeof(BIO_MSG),
                     1,
                     0,
...
test_pem_read(void)
{
  return thread_run_test(&test_pem_read_one,
                         MAXIMUM_THREADS,
                         &test_pem_read_one,
                         1,
                         default_provider);
...
static void
test_x509_store_by_subject(void)
{
  X509_STORE_CTX* ctx;
...
test_x509_store(void)
{
  ...
  ret = thread_run_test(&test_x509_store_by_subject,
                        MAXIMUM_THREADS,
