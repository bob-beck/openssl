...
_torture_rw (void)
{
  ...
  TEST_info ("performed %d reads and %d writes over 2 read and 2 write "
             "threads in %e seconds",
...
static int
thread_run_test (void (*main_func) (void), size_t num_threads,
...
test_bio_dgram_pair_worker (void)
{
   ...
   */
  if (ch & 2)
    r = BIO_sendmmsg (ch & 1 ? multi_bio2 : multi_bio1, &msg, sizeof (BIO_MSG),
                      1, 0, &num_processed);
  else
    r = BIO_recvmmsg (ch & 1 ? multi_bio2 : multi_bio1, &msg, sizeof (BIO_MSG),
                      1, 0, &num_processed);
