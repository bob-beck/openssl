    ...
    RX_OP_RX(0, INIT_WINDOW_SIZE * 2, 0)
      RX_OP_RETIRE(0, INIT_WINDOW_SIZE, 50 * OSSL_TIME_MS, 0)
        RX_OP_CHECK_SWM(INIT_WINDOW_SIZE * 2) RX_OP_CHECK_CHANGED(1, 0)
          RX_OP_CHECK_CHANGED(1, 1) RX_OP_CHECK_CHANGED(0, 0)
            RX_OP_CHECK_ERROR(0, 0) RX_OP_CHECK_CWM(INIT_WINDOW_SIZE * 3)
    ...
    RX_OP_RX(0, INIT_WINDOW_SIZE * 3, 0)
      RX_OP_RETIRE(0, INIT_WINDOW_SIZE, 50 * OSSL_TIME_MS, 0)
        RX_OP_CHECK_SWM(INIT_WINDOW_SIZE * 3) RX_OP_CHECK_CHANGED(1, 0)
          RX_OP_CHECK_CHANGED(1, 1) RX_OP_CHECK_CHANGED(0, 0)
            RX_OP_CHECK_ERROR(0, 0) RX_OP_CHECK_CWM(INIT_WINDOW_SIZE * 5)
        ...
        RX_OP_RX(0, 10, 0) RX_OP_CHECK_CWM_CONN(INIT_WINDOW_SIZE)
          RX_OP_CHECK_CWM_STREAM(0, INIT_S_WINDOW_SIZE)
            RX_OP_CHECK_CWM_STREAM(1, INIT_S_WINDOW_SIZE)
              RX_OP_CHECK_SWM_CONN(10) RX_OP_CHECK_SWM_STREAM(0, 10)
                RX_OP_CHECK_SWM_STREAM(1, 0) RX_OP_CHECK_RWM_CONN(0)
                  RX_OP_CHECK_RWM_STREAM(0, 0) RX_OP_CHECK_RWM_STREAM(1, 0)
                    ...
                    RX_OP_CHECK_CWM_STREAM(0, INIT_S_WINDOW_SIZE)
                      RX_OP_CHECK_CWM_STREAM(1, INIT_S_WINDOW_SIZE)
                        ...
                        RX_OP_RETIRE(1, 42, 50 * OSSL_TIME_MS, 0)
                          RX_OP_CHECK_RWM_CONN(52) RX_OP_CHECK_RWM_STREAM(1, 42)
                            RX_OP_CHECK_CWM_CONN(INIT_WINDOW_SIZE)
                              RX_OP_CHECK_CWM_STREAM(0, INIT_S_WINDOW_SIZE)
                                RX_OP_CHECK_CWM_STREAM(1, INIT_S_WINDOW_SIZE)
  ...
  RX_OP_CHECK_CWM_CONN(INIT_S_WINDOW_SIZE + INIT_WINDOW_SIZE + 42)
    RX_OP_CHECK_RWM_CONN(INIT_S_WINDOW_SIZE + 42)
      RX_OP_CHECK_RWM_STREAM(0, INIT_S_WINDOW_SIZE)
        RX_OP_CHECK_RWM_STREAM(1, 42) RX_OP_CHECK_CHANGED_CONN(1, 0)
          RX_OP_CHECK_CHANGED_CONN(1, 1) RX_OP_CHECK_CHANGED_CONN(0, 0)
            RX_OP_CHECK_ERROR_CONN(0, 0) RX_OP_CHECK_ERROR_STREAM(0, 0, 0)
...
static int
run_rxfc_script(const struct rx_test_op* script)
{
    ...
    switch (op->op) {
      case RX_OPC_INIT_CONN:
        if (!TEST_true(ossl_quic_rxfc_init(
              &conn_rxfc, 0, op->arg0, op->arg1, fake_now, NULL)))
          goto err;
