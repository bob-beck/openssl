...
static int key2048p3_v1(RSA* key) {
  ...
  if (!TEST_ptr(pris) || !TEST_ptr(exps) || !TEST_ptr(coeffs)) goto err;
...
ret:
  ...
  return rv;
err:
  if (pris != NULL) BN_free(pris[0]);
  if (exps != NULL) BN_free(exps[0]);
  if (coeffs != NULL) BN_free(coeffs[0]);
  rv = 0;
  goto ret;
}
...
static int key2048p3_v2(RSA* key) {
  ...
  if (!TEST_true(ossl_rsa_set0_all_params(key, primes, exps, coeffs))) goto err;
...
static int test_rsa_mp_gen_bad_input(void) {
  ...
  if (!TEST_ptr(ebn = BN_new())) goto err;
  if (!TEST_true(BN_set_word(ebn, 65537))) goto err;
