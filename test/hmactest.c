...
static int test_hmac_run(void) {
  ...
  if (!TEST_ptr(ctx = HMAC_CTX_new())) return 0;
  HMAC_CTX_reset(ctx);
  ...
  p = pt(buf, len);
  if (!TEST_ptr(p) || !TEST_str_eq(p, test[4].digest)) goto err;

  if (!TEST_false(HMAC_Init_ex(ctx, NULL, 0, EVP_sha256(), NULL))) goto err;
  ...
  p = pt(buf, len);
  if (!TEST_ptr(p) || !TEST_str_eq(p, test[5].digest)) goto err;
  ...
  if (!TEST_true(HMAC_Init_ex(ctx, test[6].key, test[6].key_len, NULL, NULL)) ||
    ...
    goto err;
  p = pt(buf, len);
  if (!TEST_ptr(p) || !TEST_str_eq(p, test[6].digest)) goto err;
...
static int test_hmac_final_update_fail(void) {
  ...
  if (!TEST_ptr(ctx = HMAC_CTX_new())) goto err;
  if (!TEST_true(HMAC_Init_ex(ctx, test[5].key, test[5].key_len, EVP_sha256(),
                              NULL)) ||
      !TEST_true(HMAC_Update(ctx, test[5].data, test[5].data_len)) ||
      !TEST_true(HMAC_Final(ctx, buf, &len)) ||
...
static int test_hmac_copy(void) {
  ...
  p = pt(buf, len);
  if (!TEST_ptr(p) || !TEST_str_eq(p, test[7].digest)) goto err;
...
static int test_hmac_copy_uninited(void) {
  ...
  if (!TEST_true(EVP_DigestSignUpdate(ctx, ct, sizeof(ct)))) goto err;
  res = 1;
err:
  EVP_MD_CTX_free(ctx);
...
static int test_hmac_chunks(int idx) {
  ...
  if (!TEST_ptr(md = EVP_get_digestbyname(test_chunks[idx].md_name))) goto err;
  ...
  if (!TEST_true(HMAC_Final(ctx, buf, &len))) goto err;

  p = pt(buf, len);
  if (!TEST_ptr(p) || !TEST_str_eq(p, test_chunks[idx].digest)) goto err;
