...
static int
test_hmac_final_update_fail(void)
{
  HMAC_CTX* ctx = NULL;
  unsigned char buf[EVP_MAX_MD_SIZE];
  unsigned int len;
...
#ifdef CHARSET_EBCDIC
  ebcdic2ascii(
    test_chunks[idx].key, test_chunks[idx].key, test_chunks[idx].key_len);
#endif

  if (!TEST_true(HMAC_Init_ex(
        ctx, test_chunks[idx].key, test_chunks[idx].key_len, md, NULL)))
    goto err;
