...
static OSSL_TIME fake_now(void* arg) {
  ...
  if (!CRYPTO_THREAD_read_lock(fake_time_lock)) return ossl_time_zero();
...
static int do_test(int use_thread_assist, int use_fake_time, int use_inject) {
  ...
  if (!TEST_int_ge(s_fd, 0)) goto err;

  if (!TEST_true(BIO_socket_nbio(s_fd, 1))) goto err;

  if (!TEST_ptr(s_addr_ = BIO_ADDR_new())) goto err;

  if (!TEST_true(BIO_ADDR_rawmake(s_addr_, AF_INET, &ina, sizeof(ina), 0)))
    goto err;

  if (!TEST_true(BIO_bind(s_fd, s_addr_, 0))) goto err;

  s_info.addr = s_addr_;
  if (!TEST_true(BIO_sock_info(s_fd, BIO_SOCK_INFO_ADDRESS, &s_info))) goto err;

  if (!TEST_int_gt(BIO_ADDR_rawport(s_addr_), 0)) goto err;

  if (!TEST_ptr(s_net_bio = s_net_bio_own = BIO_new_dgram(s_fd, 0))) goto err;

  if (!BIO_up_ref(s_net_bio)) goto err;
  ...
  if (use_fake_time) tserver_args.now_cb = fake_now;
  ...
  if (!TEST_int_ge(c_fd, 0)) goto err;

  if (!TEST_true(BIO_socket_nbio(c_fd, 1))) goto err;

  if (!TEST_ptr(c_net_bio = c_net_bio_own = BIO_new_dgram(c_fd, 0))) goto err;

  if (!BIO_dgram_set_peer(c_net_bio, s_addr_)) goto err;
  ...
  if (!TEST_ptr(c_ssl = SSL_new(c_ctx))) goto err;
  ...
  if (!TEST_false(SSL_set_alpn_protos(c_ssl, alpn, sizeof(alpn)))) goto err;
  ...
  if (!TEST_true(SSL_set_blocking_mode(c_ssl, 0))) goto err;
  ...
  for (;;) {
    ...
    if (!c_start_idle_test) {
      ...
      if (!TEST_true(ret == 1 || is_want(c_ssl, ret))) goto err;
      ...
      if (ret == 1) {
         ...
         * to hear back.
         */
        if (!TEST_true(CRYPTO_THREAD_write_lock(fake_time_lock))) goto err;
    ...
    if (c_connected && !c_write_done) {
      ...
      if (!TEST_true(SSL_stream_conclude(c_ssl, 0))) goto err;
    ...
    if (c_connected && c_write_done && !s_read_done) {
      if (!ossl_quic_tserver_read(tserver, 0,
        ...
        if (!TEST_true(ossl_quic_tserver_has_read_ended(tserver, 0))) goto err;

        if (!TEST_mem_eq(msg1, sizeof(msg1) - 1, msg2, s_total_read)) goto err;

        s_begin_write = 1;
        s_read_done = 1;
      } else {
        s_total_read += l;
        if (!TEST_size_t_le(s_total_read, sizeof(msg1) - 1)) goto err;
      }
    }
    ...
    if (c_begin_read && c_total_read < sizeof(msg1) - 1) {
      ...
      if (c_total_read == sizeof(msg1) - 1) {
        if (!TEST_mem_eq(msg1, sizeof(msg1) - 1, msg3, c_total_read)) goto err;
    ...
    if (c_wait_eos && !c_done_eos) {
      ...
      if (SSL_get_error(c_ssl, ret) != SSL_ERROR_WANT_READ) {
        ...
        if (use_thread_assist && use_fake_time) {
          if (!TEST_true(ossl_quic_tserver_is_connected(tserver))) goto err;
          c_start_idle_test = 1;
    ...
    if (c_start_idle_test && !c_done_idle_test) {
      ...
      if (idle_units_done < 6000) {
         ...
         */
        if (!TEST_true(SSL_get_event_timeout(c_ssl, &tv, &isinf))) goto err;
...
err:
  ...
  BIO_free(c_pair_own);
  BIO_free(s_pair_own);
  if (s_fd >= 0) BIO_closesocket(s_fd);
  if (c_fd >= 0) BIO_closesocket(c_fd);
  return testresult;
}
...
static int test_tserver(int idx) {
  ...
  if (use_fake_time && !thread_assisted) return 1;
...
int setup_tests(void) {
  ...
  if ((fake_time_lock = CRYPTO_THREAD_lock_new()) == NULL) return 0;
