...
static int test_bio_memleak(void) {
  ...
  if (!TEST_ptr(bio)) goto finish;
  bufmem.length = sizeof(str);
  bufmem.data = (char*)str;
  bufmem.max = bufmem.length;
  BIO_set_mem_buf(bio, &bufmem, BIO_NOCLOSE);
  BIO_set_flags(bio, BIO_FLAGS_MEM_RDONLY);
  if (!TEST_int_eq(BIO_read(bio, buf, sizeof(buf)), sizeof(str))) goto finish;
  if (!TEST_mem_eq(buf, sizeof(str), str, sizeof(str))) goto finish;
  ok = 1;
...
static int test_bio_get_mem(void) {
  ...
  if (!TEST_ptr(bufmem)) goto finish;
  if (!TEST_int_gt(BIO_set_close(bio, BIO_NOCLOSE), 0)) goto finish;
  BIO_free(bio);
  bio = NULL;
...
static int test_bio_new_mem_buf(void) {
  ...
  if (!TEST_ptr(bio)) goto finish;
  if (!TEST_int_eq(BIO_read(bio, data, 5), 5)) goto finish;
  if (!TEST_mem_eq(data, 5, "Hello", 5)) goto finish;
  if (!TEST_int_gt(BIO_get_mem_ptr(bio, &bufmem), 0)) goto finish;
  if (!TEST_int_lt(BIO_write(bio, "test", 4), 0)) goto finish;
  if (!TEST_int_eq(BIO_read(bio, data, 16), 7)) goto finish;
  if (!TEST_mem_eq(data, 7, " World\n", 7)) goto finish;
  if (!TEST_int_gt(BIO_reset(bio), 0)) goto finish;
  if (!TEST_int_eq(BIO_read(bio, data, 16), 12)) goto finish;
...
static int test_bio_rdonly_mem_buf(void) {
  ...
  if (!TEST_ptr(bio2)) goto finish;
  BIO_set_mem_buf(bio2, bufmem, BIO_CLOSE);
  BIO_set_flags(bio2, BIO_FLAGS_MEM_RDONLY);

  if (!TEST_int_eq(BIO_read(bio2, data, 16), 7)) goto finish;
  if (!TEST_mem_eq(data, 7, " World\n", 7)) goto finish;
  if (!TEST_int_gt(BIO_reset(bio2), 0)) goto finish;
  if (!TEST_int_eq(BIO_read(bio2, data, 16), 7)) goto finish;
  if (!TEST_mem_eq(data, 7, " World\n", 7)) goto finish;
  ok = 1;
...
static int test_bio_rdwr_rdonly(void) {
  ...
  if (!TEST_int_eq(BIO_read(bio, data, 16), 16)) goto finish;
...
static int test_bio_nonclear_rst(void) {
  ...
  if (!TEST_int_lt(BIO_read(bio, data, 16), 1)) goto finish;
...
static int test_bio_i2d_ASN1_mime(void) {
  ...
  if (!TEST_ptr(bio = BIO_new(BIO_s_mem()))) goto finish;
  ...
  if (!TEST_ptr(out = BIO_new(BIO_s_mem()))) goto finish;
  if (!TEST_ptr(p7 = PKCS7_new())) goto finish;
  if (!TEST_true(PKCS7_set_type(p7, NID_pkcs7_data))) goto finish;
  ...
  if (!TEST_int_eq(error_callback_fired, 1)) goto finish;
