...
static int test_asid(void) {
  ...
  if (!TEST_ptr(asid1) || !TEST_ptr(asid2) || !TEST_ptr(asid3)) goto err;
...
static int check_addr(IPAddrBlocks* addr, int type) {
  ...
  if (!TEST_int_eq(sk_IPAddressFamily_num(addr), 1)) return 0;

  fam = sk_IPAddressFamily_value(addr, 0);
  if (!TEST_ptr(fam)) return 0;
...
static int test_addr_ranges(void) {
  ...
  for (i = 0; i < OSSL_NELEM(ranges); i++) {
    ...
    if (!TEST_ptr(addr)) goto end;
    /*
     * Has the side effect of installing the comparison function onto the
     * stack.
     */
    if (!TEST_true(X509v3_addr_canonize(addr))) goto end;

    ip1 = a2i_IPADDRESS(ranges[i].ip1);
    if (!TEST_ptr(ip1)) goto end;
    if (!TEST_true(ip1->length == 4 || ip1->length == 16)) goto end;
    ip2 = a2i_IPADDRESS(ranges[i].ip2);
    if (!TEST_ptr(ip2)) goto end;
    if (!TEST_int_eq(ip2->length, ip1->length)) goto end;
    if (!TEST_true(memcmp(ip1->data, ip2->data, ip1->length) <= 0)) goto end;

    if (!TEST_true(X509v3_addr_add_range(addr, ranges[i].afi, NULL, ip1->data,
                                         ip2->data)))
      goto end;

    if (!TEST_true(X509v3_addr_is_canonical(addr))) goto end;

    if (!check_addr(addr, ranges[i].rorp)) goto end;
...
static int test_addr_fam_len(void) {
  ...
  key[5] = 0xD;
  keylen = 6;
  if ((f1 = IPAddressFamily_new()) == NULL) goto end;
  ...
  if (f1->addressFamily == NULL &&
    ...
    goto end;
  if (!ASN1_OCTET_STRING_set(f1->addressFamily, key, keylen)) goto end;

  /* Push and transfer memory ownership to stack */
  if (!sk_IPAddressFamily_push(addr, f1)) goto end;
  f1 = NULL;

  /* Shouldn't be able to canonize this as the len is > 3*/
  if (!TEST_false(X509v3_addr_canonize(addr))) goto end;
...
static int test_ext_syntax(void) {
  ...
  for (i = 0; i < OSSL_NELEM(extvalues); i++) {
    X509V3_CTX ctx;
    BIO* extbio =
        BIO_new_mem_buf(extvalues[i].value, (int)strlen(extvalues[i].value));
    CONF* conf;
    long eline;

    if (!TEST_ptr(extbio)) return 0;
