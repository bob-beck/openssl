...
static int
check_addr(IPAddrBlocks* addr, int type)
{
  IPAddressFamily* fam;
...
test_addr_ranges(void)
{
  ...
  for (i = 0; i < OSSL_NELEM(ranges); i++) {
    ...
    if (!TEST_true(X509v3_addr_add_range(
          addr, ranges[i].afi, NULL, ip1->data, ip2->data)))
      goto end;
...
test_addr_subset(void)
{
  ...
  for (i = 0; i < sz; ++i) {
    ...
    if (!TEST_ptr(addr[i] = sk_IPAddressFamily_new_null()) ||
        ...
        !TEST_true(X509v3_addr_add_range(
          addr[i], ranges[i].afi, NULL, ip1[i]->data, ip2[i]->data)))
      goto end;
  }
