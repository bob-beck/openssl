...
static int test_ml_kem(void) {
  ...
  if (!TEST_ptr(akey)) goto err;

  /* Get the raw public key */
  publen = EVP_PKEY_get1_encoded_public_key(akey, &rawpub);
  if (!TEST_size_t_gt(publen, 0)) goto err;

  /* Create Bob's key and populate it with Alice's public key data */
  bkey = EVP_PKEY_new();
  if (!TEST_ptr(bkey)) goto err;

  if (!TEST_int_gt(EVP_PKEY_copy_parameters(bkey, akey), 0)) goto err;
  ...
  if (!TEST_int_gt(EVP_PKEY_encapsulate_init(ctx, NULL), 0)) goto err;

  if (!TEST_int_gt(
          EVP_PKEY_encapsulate(ctx, NULL, &wrpkeylen, NULL, &bgenkeylen), 0))
    goto err;

  if (!TEST_size_t_gt(wrpkeylen, 0) || !TEST_size_t_gt(bgenkeylen, 0)) goto err;

  wrpkey = OPENSSL_zalloc(wrpkeylen);
  bgenkey = OPENSSL_zalloc(bgenkeylen);
  if (!TEST_ptr(wrpkey) || !TEST_ptr(bgenkey)) goto err;
  ...
  if (!TEST_int_gt(EVP_PKEY_decapsulate_init(ctx, NULL), 0)) goto err;

  if (!TEST_int_gt(
          EVP_PKEY_decapsulate(ctx, NULL, &agenkeylen, wrpkey, wrpkeylen), 0))
    goto err;

  if (!TEST_size_t_gt(agenkeylen, 0)) goto err;

  agenkey = OPENSSL_zalloc(agenkeylen);
  if (!TEST_ptr(agenkey)) goto err;
  ...
  if (!TEST_mem_eq(agenkey, agenkeylen, bgenkey, bgenkeylen)) goto err;

  /* Verify we generated a non-zero shared key */
  for (i = 0; i < agenkeylen; i++)
    if (agenkey[i] != 0) break;
  if (!TEST_size_t_ne(i, agenkeylen)) goto err;
...
static int test_non_derandomised_ml_kem(void) {
  ...
  for (i = 0; i < (int)OSSL_NELEM(alg); ++i) {
    ...
    if ((v = ossl_ml_kem_get_vinfo(alg[i])) == NULL) goto done;
    ...
    *p = OSSL_PARAM_construct_end();
    if (!TEST_true(EVP_RAND_CTX_set_params(privctx, params))) goto done;

    res = -2;
    /* Generate Alice's key */
    akey = EVP_PKEY_Q_keygen(testctx, NULL, v->algorithm_name);
    if (!TEST_ptr(akey)) goto done;

    /* Check that no more entropy is available! */
    if (!TEST_int_le(RAND_priv_bytes(&c, 1), 0)) goto done;

    /* Get the raw public key */
    publen = EVP_PKEY_get1_encoded_public_key(akey, &rawpub);
    if (!TEST_size_t_eq(publen, v->pubkey_bytes)) goto done;
    ...
    /* Create Bob's key and populate it with Alice's public key data */
    bkey = EVP_PKEY_new();
    if (!TEST_ptr(bkey)) goto done;
    if (!TEST_int_gt(EVP_PKEY_copy_parameters(bkey, akey), 0)) goto done;
    if (!TEST_true(EVP_PKEY_set1_encoded_public_key(bkey, rawpub, publen)))
      goto done;
    ...
    *p = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_ENTROPY,
    ...
    if (!TEST_true(EVP_RAND_CTX_set_params(pubctx, params))) goto done;

    /* Encapsulate Bob's key */
    res = -5;
    ctx = EVP_PKEY_CTX_new_from_pkey(testctx, bkey, NULL);
    if (!TEST_ptr(ctx)) goto done;
    if (!TEST_int_gt(EVP_PKEY_encapsulate_init(ctx, NULL), 0)) goto done;
    if (!TEST_int_gt(
            EVP_PKEY_encapsulate(ctx, NULL, &wrpkeylen, NULL, &bgenkeylen), 0))
      goto done;
    ...
    wrpkey = OPENSSL_zalloc(wrpkeylen);
    bgenkey = OPENSSL_zalloc(bgenkeylen);
    if (!TEST_ptr(wrpkey) || !TEST_ptr(bgenkey)) goto done;
    ...
    if (!TEST_int_le(RAND_bytes(&c, 1), 0)) goto done;
    ...
    ctx = EVP_PKEY_CTX_new_from_pkey(testctx, akey, NULL);
    if (!TEST_ptr(ctx)) goto done;
    if (!TEST_int_gt(EVP_PKEY_decapsulate_init(ctx, NULL), 0)) goto done;
    if (!TEST_true(
            EVP_PKEY_decapsulate(ctx, NULL, &agenkeylen, wrpkey, wrpkeylen)))
      goto done;
    if (!TEST_size_t_eq(agenkeylen, ML_KEM_SHARED_SECRET_BYTES)) goto done;
    agenkey = OPENSSL_zalloc(agenkeylen);
    if (!TEST_ptr(agenkey)) goto done;
    if (!TEST_true(
            EVP_PKEY_decapsulate(ctx, agenkey, &agenkeylen, wrpkey, wrpkeylen)))
      goto done;
    /* Hopefully we ended up with a shared key */
    if (!TEST_mem_eq(agenkey, agenkeylen, bgenkey, bgenkeylen)) goto done;
    ...
    if (!TEST_true(
            EVP_PKEY_decapsulate(ctx, agenkey, &agenkeylen, wrpkey, wrpkeylen)))
      goto done;
    if (!TEST_mem_ne(agenkey, agenkeylen, bgenkey, bgenkeylen)) goto done;
