...
static int
test_non_derandomised_ml_kem(void)
{
  static const int alg[3] = { EVP_PKEY_ML_KEM_512,
                              EVP_PKEY_ML_KEM_768,
  ...
  for (i = 0; i < (int)OSSL_NELEM(alg); ++i) {
    ...
    *p++ = OSSL_PARAM_construct_octet_string(
      OSSL_RAND_PARAM_TEST_ENTROPY, gen_seed, sizeof(gen_seed));
    ...
    *p = OSSL_PARAM_construct_end();
    ...
    if (!TEST_mem_eq(rawpub + v->vector_bytes,
                     ML_KEM_RANDOM_BYTES,
                     expected_rho[i],
                     ML_KEM_RANDOM_BYTES))
      goto done;
    ...
    p = params;
    *p = OSSL_PARAM_construct_octet_string(
      OSSL_RAND_PARAM_TEST_ENTROPY, enc_seed, sizeof(enc_seed));
    ...
    if (!TEST_mem_eq(bgenkey,
                     bgenkeylen,
                     expected_shared_secret[i],
    ...
    p = params;
    *p = OSSL_PARAM_construct_octet_string(
      OSSL_RAND_PARAM_TEST_ENTROPY, dec_seed, sizeof(dec_seed));
    if (!TEST_true(EVP_RAND_CTX_set_params(pubctx, params)))
      goto done;

    /* This time decap should fail, and return the decap entropy */
    if (!TEST_false(EVP_PKEY_decapsulate(
          ctx, agenkey, &agenkeylen, wrpkey, wrpkeylen - 1)))
      goto done;
