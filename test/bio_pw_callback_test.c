...
static int
full_cycle_test(KEY_ENCODING key_encoding,
...
{
  return full_cycle_test(
    KE_PEM, CB_TEST_ZERO_LENGTH, CB_TEST_ZERO_LENGTH, ER_SUCCESS);
}
...
{
  return full_cycle_test(
    KE_PEM, CB_TEST_MATCH_SIZE, CB_TEST_MATCH_SIZE, ER_SUCCESS);
}
...
{
  return full_cycle_test(
    KE_PEM, CB_TEST_MATCH_SIZE, CB_TEST_EXCEED_SIZE, ER_FAILURE);
}
...
{
  return full_cycle_test(
    KE_PKCS8, CB_TEST_ZERO_LENGTH, CB_TEST_ZERO_LENGTH, ER_SUCCESS);
}
...
{
  return full_cycle_test(
    KE_PKCS8, CB_TEST_MATCH_SIZE, CB_TEST_MATCH_SIZE, ER_SUCCESS);
}
...
{
  return full_cycle_test(
    KE_PKCS8, CB_TEST_MATCH_SIZE, CB_TEST_EXCEED_SIZE, ER_FAILURE);
}
...
setup_tests(void)
{
  ...
  if (!TEST_ptr(bio = BIO_new_file(key_file, "r")))
    return 0;
  if (!TEST_ptr(PEM_read_bio_PrivateKey(
        bio, &original_pkey, callback_original_pw, NULL))) {
    BIO_free(bio);
    return 0;
  }
  BIO_free(bio);
