...
static int read_callback(char* buf, int size, int rwflag, void* u) {
  ...
  if (!TEST_ptr(cb_data)) goto err;
  if (!TEST_char_eq(cb_data->magic, cb_magic)) goto err;
  if (!TEST_ptr(buf)) goto err;
  if (!TEST_int_gt(size, 0)) goto err;
  if (!TEST_int_eq(rwflag, 0)) goto err;
...
static int write_callback(char* buf, int size, int rwflag, void* u) {
  ...
  if (!TEST_ptr(buf)) goto err;
  if (!TEST_int_gt(size, 0)) goto err;
  if (!TEST_int_eq(rwflag, 1)) goto err;
  ret = callback_copy_password(buf, size);
  cb_data->result = 1;
err:
  return ret;
...
static int re_encrypt_key(char** enc_data, int* enc_data_size,
  ...
  if (!TEST_ptr(enc_data)) goto err;
  if (!TEST_ptr(enc_data_size)) goto err;
  ...
  if (!TEST_int_ne(w_ret, 0)) goto err;
  if (!TEST_char_eq(cb_data.magic, cb_magic)) goto err;
  if (!TEST_int_eq(cb_data.result, 1)) goto err;
  *enc_data_size = BIO_get_mem_data(bio, enc_data);
  BIO_get_mem_ptr(bio, &bptr);
  if (!BIO_set_close(bio, BIO_NOCLOSE)) goto err;
  bptr->data = NULL;
  ret = 1;
err:
  BUF_MEM_free(bptr);
...
static int decrypt_key(char* enc_data, int enc_data_size,
  ...
  if (!TEST_ptr(bio = BIO_new_mem_buf(enc_data, enc_data_size))) goto err;
  cb_data.magic = cb_magic;
  cb_data.result = 0;
  switch (key_encoding) {
    case KE_PEM:
  ...
  if (expected_result == ER_SUCCESS) {
    if (!TEST_ptr(r_ret)) goto err;
  } else {
    if (!TEST_ptr_null(r_ret)) goto err;
  }
  if (!TEST_char_eq(cb_data.magic, cb_magic)) goto err;
  if (!TEST_int_eq(cb_data.result, 1)) goto err;
  ret = 1;
...
static int full_cycle_test(KEY_ENCODING key_encoding, CALLBACK_TEST write_test,
  ...
  if (!re_encrypt_key(&enc_data, &enc_data_size, key_encoding)) goto err;
  callback_test = read_test;
...
int setup_tests(void) {
  ...
  if (!TEST_ptr(bio = BIO_new_file(key_file, "r"))) return 0;
