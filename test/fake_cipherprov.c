...
static int
fake_skeymgmt_export(void* keydata,
                     int selection,
                     ...
                     void* cbarg)
{
  ...
  if (selection & OSSL_SKEYMGMT_SELECT_SECRET_KEY) {
    *p = OSSL_PARAM_construct_octet_string(
      OSSL_SKEY_PARAM_RAW_BYTES, ctx->key, sizeof(ctx->key));
    p++;
  }
...
static int
fake_skey_init(PROV_CIPHER_FAKE_CTX* ctx,
               void* pkeyparam,
...
fake_cipher_start(OSSL_LIB_CTX* libctx)
{
  ...
  if (!TEST_true(OSSL_PROVIDER_add_builtin(
        libctx, FAKE_PROV_NAME, fake_cipher_provider_init)) ||
