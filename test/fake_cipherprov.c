...
static int ctx_from_key_params(PROV_CIPHER_FAKE_CTX* pctx,
  ...
  if (p != NULL) {
    ...
    if (data_size > FAKE_KEY_LEN) data_size = FAKE_KEY_LEN;
...
static void* fake_skeymgmt_import(void* provctx, int selection,
  ...
  if (!TEST_ptr(ctx = OPENSSL_zalloc(sizeof(PROV_CIPHER_FAKE_CTX)))) return 0;
...
static int fake_skey_init(PROV_CIPHER_FAKE_CTX* ctx, void* pkeyparam,
                          ...
                          const OSSL_PARAM params[]) {
  if (pkeyparam != NULL) memcpy(ctx, pkeyparam, sizeof(PROV_CIPHER_FAKE_CTX));
...
static int fake_skey_einit(void* vctx, void* pkeyparam, const unsigned char* iv,
  ...
  if (fake_skey_init(ctx, pkeyparam, iv, ivlen, params) != 1) return 0;
