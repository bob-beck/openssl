...
static int
skip_time_ms (struct helper *h, struct helper_local *hl)
{
...
static int
check_rejected (struct helper *h, struct helper_local *hl)
{
...
static int
check_stream_reset (struct helper *h, struct helper_local *hl)
{
...
static int
check_stream_stopped (struct helper *h, struct helper_local *hl)
{
  uint64_t stream_id = hl->check_op->arg2;
...
static int
override_key_update (struct helper *h, struct helper_local *hl)
{
...
static int
trigger_key_update (struct helper *h, struct helper_local *hl)
{
...
static int
check_key_update_ge (struct helper *h, struct helper_local *hl)
{
  QUIC_CHANNEL *ch = ossl_quic_conn_get_channel (h->c_conn);
...
static int
check_key_update_lt (struct helper *h, struct helper_local *hl)
{
  QUIC_CHANNEL *ch = ossl_quic_conn_get_channel (h->c_conn);
...
stream_info_hash (const STREAM_INFO *info)
{
  return OPENSSL_LH_strhash (info->name);
}

static int
stream_info_cmp (const STREAM_INFO *a, const STREAM_INFO *b)
{
  return strcmp (a->name, b->name);
}

static void
cleanup_stream (STREAM_INFO *info)
{
...
static void
helper_cleanup_streams (LHASH_OF (STREAM_INFO) **lh)
{
  if (*lh == NULL)
    return;
...
static int
join_threads (struct child_thread_args *threads, size_t num_threads)
{
  int ok = 1;
  size_t i;
...
static int
join_server_thread (struct helper *h)
{
  CRYPTO_THREAD_RETVAL rv;
...
static int *
s_checked_out_p (struct helper *h, int thread_idx)
{
...
static QUIC_TSERVER *
s_lock (struct helper *h, struct helper_local *hl)
{
...
static void
s_unlock (struct helper *h, struct helper_local *hl)
{
...
static void
helper_cleanup (struct helper *h)
{
#if defined(OPENSSL_THREADS)
...
static int
helper_init (struct helper *h, const char *script_name, int free_order,
             int blocking, int need_injector)
{
...
static int
helper_local_init (struct helper_local *hl, struct helper *h, int thread_idx)
{
  hl->h = h;
  hl->c_streams = NULL;
...
static void
helper_local_cleanup (struct helper_local *hl)
{
  if (hl->h == NULL)
    return;
...
static STREAM_INFO *
get_stream_info (LHASH_OF (STREAM_INFO) *lh, const char *stream_name)
{
...
static int
helper_local_set_c_stream (struct helper_local *hl, const char *stream_name,
                           SSL *c_stream)
{
...
static SSL *
helper_local_get_c_stream (struct helper_local *hl, const char *stream_name)
{
  STREAM_INFO *info;
...
static int
helper_packet_plain_listener (QTEST_FAULT *qtf, QUIC_PKT_HDR *hdr,
                              unsigned char *buf, size_t buf_len, void *arg)
{
  struct helper *h = arg;
...
static int
helper_handshake_listener (QTEST_FAULT *fault, unsigned char *buf,
...
static int
helper_datagram_listener (QTEST_FAULT *fault, BIO_MSG *msg, size_t stride,
                          void *arg)
{
  struct helper *h = arg;
...
static int
check_consistent_want (SSL *s, int ret)
{
  int ec = SSL_get_error (s, ret);
  int w = SSL_want (s);
...
static int
run_script_worker (struct helper *h, const struct script_op *script,
                  ...
                  if (!done)
                    {
                      if (!end_wait_warning)
                        {
                          TEST_info ("still waiting for other threads to "
                                     "finish (%zu)",
                                     i);
                          end_wait_warning = 1;
                        }
        ...
        case OPK_C_SET_ALPN:
          {
            ...
            if (!TEST_size_t_le (alpn_len, UINT8_MAX)
                || !TEST_ptr (
                    tmp_buf = (unsigned char *)OPENSSL_malloc (alpn_len + 1)))
              goto out;
            ...
            if (!TEST_false (SSL_set_alpn_protos (
                    h->c_conn, tmp_buf, (unsigned int)(alpn_len + 1))))
              goto out;
        ...
        case OPK_S_NEW_STREAM:
          {
            ...
            if (!TEST_true (
                    helper_set_s_stream (h, op->stream_name, stream_id)))
              goto out;
          }
          break;
        ...
        case OPK_C_SET_DEFAULT_STREAM_MODE:
          {
            ...
            if (!TEST_true (
                    SSL_set_default_stream_mode (c_tgt, (uint32_t)op->arg1)))
              goto out;
          }
          break;
        ...
        case OPK_C_SET_INCOMING_STREAM_POLICY:
          {
            ...
            if (!TEST_true (
                    SSL_set_incoming_stream_policy (c_tgt, (int)op->arg1, 0)))
              goto out;
          }
          break;
        ...
        case OPK_S_BIND_STREAM_ID:
          {
            ...
            if (!TEST_true (
                    helper_set_s_stream (h, op->stream_name, op->arg2)))
              goto out;
          }
          break;
        ...
        case OPK_S_UNBIND_STREAM_ID:
          {
            ...
            if (!TEST_true (
                    helper_set_s_stream (h, op->stream_name, UINT64_MAX)))
              goto out;
          }
          break;
        ...
        case OPK_S_WRITE_FAIL:
          {
            ...
            if (!TEST_false (ossl_quic_tserver_write (
                    ACQUIRE_S (), s_stream_id, (const unsigned char *)"apple",
                    5, &bytes_written)))
              goto out;
          }
          break;
        ...
        case OPK_C_STREAM_RESET_FAIL:
          {
            ...
            if (op->op == OPK_C_STREAM_RESET)
              {
                if (!TEST_true (
                        SSL_stream_reset (c_tgt, &args, sizeof (args))))
                  goto out;
              }
            else
              {
        ...
        case OPK_NEW_THREAD:
          {
            ...
            h->threads
                = OPENSSL_calloc (op->arg1, sizeof (struct child_thread_args));
        ...
        case OPK_C_INHIBIT_TICK:
          {
            ...
            ossl_quic_engine_set_inhibit_tick (
                ossl_quic_channel_get0_engine (ch), (int)op->arg1);
          }
          break;
  ...
  if (!testresult)
    {
      ...
      tcause = (h->s != NULL ? ossl_quic_tserver_get_terminate_cause (h->s)
                             : NULL);
      if (tcause != NULL)
        {
...
static int
run_script (const struct script_op *script, const char *script_name,
...
static const struct script_op script_2[] = {
  OP_C_SET_ALPN ("ossltest") OP_C_CONNECT_WAIT () OP_C_SET_INCOMING_STREAM_POLICY (
      ...
      OP_S_BIND_STREAM_ID (a, C_BIDI_ID (0)) OP_S_READ_EXPECT (
          a, "apple", 5) OP_S_WRITE (a, "orange",
                                     6) OP_C_READ_EXPECT (DEFAULT, "orange", 6)

          OP_C_NEW_STREAM_BIDI (b, C_BIDI_ID (1)) OP_C_WRITE (
              b, "flamingo",
              8) OP_C_CONCLUDE (b) OP_S_BIND_STREAM_ID (b, C_BIDI_ID (1))
              OP_S_READ_EXPECT (b, "flamingo", 8) OP_S_EXPECT_FIN (b) OP_S_WRITE (
                  b, "gargoyle",
                  8) OP_S_CONCLUDE (b) OP_C_READ_EXPECT (b, "gargoyle",
                                                         8) OP_C_EXPECT_FIN (b)

                  OP_C_NEW_STREAM_UNI (c, C_UNI_ID (0)) OP_C_WRITE (
                      c, "elephant", 8) OP_C_CONCLUDE (c)
                      OP_S_BIND_STREAM_ID (c, C_UNI_ID (0)) OP_S_READ_EXPECT (
                          c, "elephant",
                          ...
                          OP_C_ACCEPT_STREAM_NONE ()
                              ...
                              OP_S_NEW_STREAM_BIDI (
                                  ...
                                  OP_C_ACCEPT_STREAM_WAIT (
                                      d) OP_C_ACCEPT_STREAM_NONE ()
                                      OP_C_READ_EXPECT (d, "frog",
                                                        4) OP_C_EXPECT_FIN (d)
                    ...
                    OP_S_READ_EXPECT (a, "appleby", 7)
                        ...
                        OP_S_WRITE (a, "hello", 5)
                            OP_C_READ_EXPECT (a, "hello", 5)
                                ...
                                OP_C_WRITE_FAIL (DEFAULT) OP_C_ATTACH (a)
                                    OP_C_WRITE (DEFAULT, "is here", 7)
                                        OP_S_READ_EXPECT (a, "is here", 7)

                                            OP_C_DETACH (a) OP_C_CONCLUDE (a)
                                                OP_S_EXPECT_FIN (a)
  ...
  OP_C_SET_ALPN ("ossltest") OP_C_CONNECT_WAIT ()
      OP_C_SET_DEFAULT_STREAM_MODE (SSL_DEFAULT_STREAM_MODE_NONE)
          ...
          OP_NEW_THREAD (5, script_12_child)
              ...
              OP_S_BIND_STREAM_ID (a, C_BIDI_ID (0)) OP_S_READ_EXPECT (
                  a, "foo",
                  3) OP_S_EXPECT_FIN (a) OP_S_BIND_STREAM_ID (b, C_BIDI_ID (1))
                ...
                OP_NEW_THREAD (5, script_13_child)
                    ...
                    OP_BEGIN_REPEAT (50)
                        ...
                        OP_S_NEW_STREAM_BIDI (a, ANY_ID)
                            OP_S_WRITE (a, "foo", 3) OP_S_CONCLUDE (a)
                ...
                OP_NEW_THREAD (5, script_14_child)
                    ...
                    OP_BEGIN_REPEAT (50)
                        ...
                        OP_S_ACCEPT_STREAM_WAIT (a)
                            OP_S_READ_EXPECT (a, "foo", 3) OP_S_EXPECT_FIN (a)
                                OP_S_UNBIND_STREAM_ID (a)
...
static const struct script_op script_15[]
    = { OP_C_SET_ALPN ("ossltest") OP_C_CONNECT_WAIT ()
        ...
        /*
         * This will cause a protocol violation to be raised by the server if
         * we are not handling the stream limit correctly on the TX side.
         */
        OP_BEGIN_REPEAT (200)
        ...
        /*
         * Drain the queue of incoming streams. We should be able to get all
         * 200 even though only 100 can be initiated at a time.
         */
        OP_BEGIN_REPEAT (200)
...
static const struct script_op script_16[]
    = { OP_C_SET_ALPN ("ossltest") OP_C_CONNECT_WAIT ()
        ...
        /*
         * This will cause a protocol violation to be raised by the client if
         * we are not handling the stream limit correctly on the TX side.
         */
        OP_BEGIN_REPEAT (200)
        ...
        OP_C_NEW_STREAM_BIDI (a, ANY_ID) OP_C_WRITE (a, "bar", 3)
            OP_C_CONCLUDE (a)
...
static const struct script_op script_17[]
    = { OP_C_SET_ALPN ("ossltest") OP_C_CONNECT_WAIT ()
         ...
         * waiting this long, we eliminate any such concern and ensure as many
         * key updates as possible can occur for the purposes of this test.
         */
...
static const struct script_op script_18[]
    = { OP_C_SET_ALPN ("ossltest") OP_C_CONNECT_WAIT ()
        ...
        /*
         * This time we simulate far less time passing between writes, so there
         * are fewer opportunities to initiate TXKUs. Note that we ask for a
         * TXKU every 1 packet above, which is absurd; thus this ensures we
         * only actually generate TXKUs when we are allowed to.
         */
...
static const struct script_op script_19[] = {
  OP_C_SET_ALPN ("ossltest") OP_C_CONNECT_WAIT ()
      ...
      OP_C_WRITE (DEFAULT, "apple", 5)
          ...
          OP_S_BIND_STREAM_ID (a, C_BIDI_ID (0)) OP_S_READ_EXPECT (a, "apple",
                                                                   5)
 ...
 */
static int
script_20_trigger (struct helper *h, volatile uint64_t *counter)
{
#if defined(OPENSSL_THREADS)
...
static int
script_20_wait (struct helper *h, volatile uint64_t *counter,
                uint64_t threshold)
{
#if defined(OPENSSL_THREADS)
  int stop = 0;
...
static int
script_20_trigger1 (struct helper *h, struct helper_local *hl)
{
  return script_20_trigger (h, &h->scratch0);
}

static int
script_20_wait1 (struct helper *h, struct helper_local *hl)
{
  return script_20_wait (h, &h->scratch0, hl->check_op->arg2);
}

static int
script_20_trigger2 (struct helper *h, struct helper_local *hl)
{
  return script_20_trigger (h, &h->scratch1);
}

static int
script_20_wait2 (struct helper *h, struct helper_local *hl)
{
            ...
            OP_C_SET_DEFAULT_STREAM_MODE (SSL_DEFAULT_STREAM_MODE_NONE)
                ...
                OP_NEW_THREAD (5, script_20_child)
                    ...
                    OP_BEGIN_REPEAT (5)
                        ...
                        OP_S_NEW_STREAM_BIDI (a, ANY_ID)
                            OP_S_WRITE (a, "foo", 3) OP_S_UNBIND_STREAM_ID (a)
...
static int
script_21_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
...
static int
script_22_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  if (h->inject_word0 == 0)
    return 1;
...
static int
script_23_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  WPACKET wpkt;
...
static int
script_24_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  WPACKET wpkt;
...
static int
script_28_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  WPACKET wpkt;
  ...
  if (!TEST_true (WPACKET_quic_write_vlint (&wpkt, h->inject_word1))
      ...
      || (h->inject_word1 == OSSL_QUIC_FRAME_TYPE_RESET_STREAM
          && !TEST_true (
              WPACKET_quic_write_vlint (&wpkt, 5)))) /* final size */
    goto err;
...
static int
script_32_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  WPACKET wpkt;
...
static int
script_39_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  WPACKET wpkt;
...
static const struct script_op script_39[]
    = { OP_S_SET_INJECT_PLAIN (script_39_inject_plain) OP_C_SET_ALPN (
        "ossltest") OP_C_CONNECT_WAIT ()
            OP_C_SET_DEFAULT_STREAM_MODE (SSL_DEFAULT_STREAM_MODE_NONE)

                OP_C_NEW_STREAM_BIDI (a, C_BIDI_ID (0)) OP_C_WRITE (
                    a, "apple", 5) OP_S_BIND_STREAM_ID (a, C_BIDI_ID (0))
                    OP_S_READ_EXPECT (a, "apple", 5)
...
static int
script_41_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  WPACKET wpkt;
...
static void
script_41_trace (int write_p, int version, int content_type, const void *buf,
...
static int
script_41_setup (struct helper *h, struct helper_local *hl)
{
...
static int
script_41_check (struct helper *h, struct helper_local *hl)
{
...
static int
script_42_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  unsigned char frame_buf[64];
...
static int
script_44_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  WPACKET wpkt;
...
static int
force_ping (struct helper *h, struct helper_local *hl)
{
...
static int
wait_incoming_acks_increased (struct helper *h, struct helper_local *hl)
{
  QUIC_CHANNEL *ch = ossl_quic_tserver_get_channel (ACQUIRE_S ());
  uint16_t count;
...
static int
script_46_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  WPACKET wpkt;
...
static int
script_52_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  unsigned char frame_buf[64];
...
static int
script_53_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  size_t written;
...
static int
script_54_inject_handshake (struct helper *h, unsigned char *buf,
                            size_t buf_len)
{
  size_t i;
...
static const struct script_op script_57[]
    = { OP_S_SET_INJECT_PLAIN (script_39_inject_plain) OP_C_SET_ALPN (
        ...
        /*
         * Now we send a NEW_CONN_ID with a bogus CID. However the sequence
         * number is old so it should be ignored and we should still be able to
         * communicate.
         */
...
static int
script_58_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  unsigned char frame_buf[64];
...
static int
init_reason (struct helper *h, struct helper_local *hl)
{
...
static int
check_shutdown_reason (struct helper *h, struct helper_local *hl)
{
  const QUIC_TERMINATE_CAUSE *tc
...
static int
script_61_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  WPACKET wpkt;
  ...
  if (!TEST_true (WPACKET_quic_write_vlint (&wpkt, h->inject_word0))
      ...
      || (h->inject_word0 == OSSL_QUIC_FRAME_TYPE_RESET_STREAM
          && !TEST_true (
              WPACKET_quic_write_vlint (&wpkt, 0)))) /* final size */
    goto err;
...
static int
script_65_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  unsigned char frame_buf[64];
...
static int
script_66_inject_plain (struct helper *h, QUIC_PKT_HDR *hdr,
                        unsigned char *buf, size_t len)
{
  int ok = 0;
  WPACKET wpkt;
...
static int
script_68_inject_handshake (struct helper *h, unsigned char *msg,
                            size_t msglen)
{
...
static int
set_max_early_data (struct helper *h, struct helper_local *hl)
{
...
static int
script_72_check (struct helper *h, struct helper_local *hl)
{
...
static const struct script_op script_72[]
    = { OP_C_SET_ALPN ("ossltest") OP_C_CONNECT_WAIT ()
        ...
        /*
         * Request more streams than a server will initially hand out and test
...
static int
generate_version_neg (WPACKET *wpkt, uint32_t version)
{
  QUIC_PKT_HDR hdr = { 0 };
...
static int
server_gen_version_neg (struct helper *h, BIO_MSG *msg, size_t stride)
{
 ...
 */
static int
script_74_alter_version (const QUIC_PKT_HDR *hdrin,
...
static void
script_74_finish_mutation (void *arg)
{
 ...
 */
static int
script_74_arm_packet_mutator (struct helper *h, struct helper_local *hl)
{
  QUIC_CHANNEL *ch = ossl_quic_conn_get_channel (h->c_conn);
...
static int
script_76_check (struct helper *h, struct helper_local *hl)
{
...
static int
on_new_session (SSL *s, SSL_SESSION *sess)
{
  ++new_session_count;
...
static int
setup_session (struct helper *h, struct helper_local *hl)
{
...
static int
trigger_late_session_ticket (struct helper *h, struct helper_local *hl)
{
  new_session_count = 0;
...
static int
check_got_session_ticket (struct helper *h, struct helper_local *hl)
{
 ...
 */
static int
script_80_send_stateless_reset (struct helper *h, QUIC_PKT_HDR *hdr,
                                unsigned char *buf, size_t len)
{
...
static int
script_80_gen_new_conn_id (struct helper *h, QUIC_PKT_HDR *hdr,
                           unsigned char *buf, size_t len)
{
  int rc = 0;
  size_t l;
...
static int
script_80_inject_pkt (struct helper *h, QUIC_PKT_HDR *hdr, unsigned char *buf,
                      size_t len)
{
  if (h->inject_word1 == 1)
                ...
                OP_C_CONCLUDE (DEFAULT) OP_S_BIND_STREAM_ID (a, C_BIDI_ID (0))
                    OP_S_READ_EXPECT (a, "apple", 5) OP_SET_INJECT_WORD (1, 0)
                        OP_S_WRITE (a, "apple", 5) OP_C_READ_EXPECT (
                            DEFAULT, "apple", 5) OP_SET_INJECT_WORD (0, 1)
                            OP_S_WRITE (a, "apple", 5)
                                OP_C_EXPECT_CONN_CLOSE_INFO (0, 0, 1) OP_END };

/* 81. Idle timeout configuration */
static int
modify_idle_timeout (struct helper *h, struct helper_local *hl)
{
  uint64_t v = 0;
...
static int
check_idle_timeout (struct helper *h, struct helper_local *hl)
{
  uint64_t v = 0;
...
static int
cannot_change_idle_timeout (struct helper *h, struct helper_local *hl)
{
  uint64_t v = 0;
...
static int
check_avail_streams (struct helper *h, struct helper_local *hl)
{
  uint64_t v = 0;
...
static int
check_write_buf_stat (struct helper *h, struct helper_local *hl)
{
  SSL *c_a;
                                  ...
                                  OP_C_ACCEPT_STREAM_WAIT (
                                      ...
                                      OP_CHECK2 (check_avail_streams, 0, 99) OP_CHECK2 (
                                          ...
                                          OP_S_NEW_STREAM_UNI (
                                              ...
                                              OP_C_ACCEPT_STREAM_WAIT (
                                                  d) OP_C_READ_EXPECT (d, "x",
                                                                       1)

                                                  OP_CHECK2 (
                                                      check_avail_streams, 0,
                                                      99) OP_CHECK2 (check_avail_streams,
                                                                     1, 99)
                                                      OP_CHECK2 (
                                                          check_avail_streams,
                                                          2, 99) OP_CHECK2 (check_avail_streams,
                                                                            3, 99)
                                                          ...
                                                          OP_CHECK2 (
                                                              ...
                                                              OP_C_WRITE (
                                                                  a, "apple",
                                                                  5) OP_CHECK2 (check_write_buf_stat,
                                                                                5, 0)
...
ossl_unused static int
script_85_poll (struct helper *h, struct helper_local *hl)
{
  ...
  ret = SSL_poll (items, OSSL_NELEM (items), sizeof (SSL_POLL_ITEM), &timeout,
                  0, &result_count);
  ...
  OP_C_SET_ALPN ("ossltest") OP_C_CONNECT_WAIT ()
      ...
      OP_C_SET_DEFAULT_STREAM_MODE (SSL_DEFAULT_STREAM_MODE_NONE)
          ...
          OP_C_NEW_STREAM_BIDI (a, C_BIDI_ID (0)) OP_C_WRITE (a, "flamingo", 8)
              ...
              OP_C_NEW_STREAM_BIDI (b, C_BIDI_ID (1)) OP_C_WRITE (b, "orange",
                                                                  6)

                  OP_C_NEW_STREAM_BIDI (c, C_BIDI_ID (2)) OP_C_WRITE (
                      c, "Strawberry", 10)
...
ossl_unused static int
script_88_poll (struct helper *h, struct helper_local *hl)
{
  int ok = 1, ret, expected_ret = 1;
  static const struct timeval timeout = { 0 };
...
ossl_unused static int
script_88_poll_conly (struct helper *h, struct helper_local *hl)
{
  int ok = 1;
  static const struct timeval timeout = { 0 };
  size_t result_count;
...
static const struct script_op script_88[]
    = { OP_SKIP_IF_BLOCKING (16) OP_C_SET_ALPN ("ossltest")
            OP_C_CONNECT_WAIT ()
...
static int
set_event_handling_mode_conn (struct helper *h, struct helper_local *hl)
{
...
static int
reenable_test_event_handling (struct helper *h, struct helper_local *hl)
{
  hl->explicit_event_handling = 0;
  return 1;
}
...
set_event_handling_mode_stream (struct helper *h, struct helper_local *hl)
{
