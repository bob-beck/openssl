...
static int helper_local_init(struct helper_local* hl,
                             struct helper* h,
                             int thread_idx) {
  hl->h = h;
  hl->c_streams = NULL;
...
static int helper_set_s_stream(struct helper* h,
                               const char* stream_name,
                               uint64_t s_stream_id) {
  STREAM_INFO* info;
...
static int helper_packet_plain_listener(QTEST_FAULT* qtf,
                                        QUIC_PKT_HDR* hdr,
                                        unsigned char* buf,
                                        size_t buf_len,
                                        void* arg) {
...
static int run_script_worker(struct helper* h,
                             const struct script_op* script,
                             const char* script_name,
                             int thread_idx) {
  int testresult = 0;
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        OP_C_SET_INCOMING_STREAM_POLICY(
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
                a,
                "apple",
                5) OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(DEFAULT,
                                                               "orange",
                                                               6)
    ...
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_WRITE(DEFAULT, "apple", 5)
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                ...
                OP_C_WRITE(DEFAULT, "orange", 6) OP_S_READ_EXPECT(a,
                                                                  "orange",
                                                                  6)
                    ...
                    OP_S_WRITE(a, "strawberry", 10)
                        OP_C_READ_EXPECT(DEFAULT, "strawberry", 10)
                            ...
                            OP_CHECK(check_key_update_lt,
                                     1) OP_CHECK(trigger_key_update, 0)
...
static int script_20_wait(struct helper* h,
...
static int script_21_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_22_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  if (h->inject_word0 == 0)
...
static int script_23_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_24_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_28_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
    ...
    OP_S_SET_INJECT_PLAIN(script_28_inject_plain) OP_C_SET_ALPN("ossltest")
        OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "orange", 6)
                ...
                OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a,
                                                                      "orange",
                                                                      6)

                    OP_C_NEW_STREAM_UNI(b,
                                        C_UNI_ID(0)) OP_C_WRITE(b, "apple", 5)
...
static int script_32_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_39_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_41_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static void script_41_trace(int write_p,
                            int version,
                            int content_type,
                            const void* buf,
                            size_t len,
...
static int script_42_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_44_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_46_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_52_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
    ...
    OP_S_SET_INJECT_PLAIN(script_52_inject_plain) OP_C_SET_ALPN(
        ...
        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
            ...
            OP_SET_INJECT_WORD(1, OSSL_QUIC_FRAME_TYPE_DATA_BLOCKED)
                ...
                OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(DEFAULT,
                                                            "orange",
                                                            6)
...
static int script_53_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_58_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_61_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_65_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_66_inject_plain(struct helper* h,
                                  QUIC_PKT_HDR* hdr,
                                  unsigned char* buf,
                                  size_t len) {
  int ok = 0;
...
static int script_68_inject_handshake(struct helper* h,
                                      unsigned char* msg,
                                      size_t msglen) {
...
static int server_gen_version_neg(struct helper* h,
                                  BIO_MSG* msg,
                                  size_t stride) {
 ...
 *  }
 */
static int script_80_send_stateless_reset(struct helper* h,
                                          QUIC_PKT_HDR* hdr,
                                          unsigned char* buf,
                                          size_t len) {
...
static int script_80_gen_new_conn_id(struct helper* h,
                                     QUIC_PKT_HDR* hdr,
                                     unsigned char* buf,
                                     size_t len) {
  int rc = 0;
...
static int script_80_inject_pkt(struct helper* h,
                                QUIC_PKT_HDR* hdr,
                                unsigned char* buf,
                                size_t len) {
  if (h->inject_word1 == 1)
                                                        ...
                                                        OP_CHECK2(
                                                            ...
                                                            OP_CHECK2(check_avail_streams,
                                                                ...
                                                                OP_CHECK2(
                                                                    ...
                                                                    0,
                                                                    0) OP_CHECK(set_event_handling_mode_conn, SSL_VALUE_EVENT_HANDLING_MODE_EXPLICIT)
                                                                    OP_C_WRITE(a,
                                                                               "apple",
            ...
            OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "flamingo", 8)
                ...
                OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1)) OP_C_WRITE(b, "orange", 6)
                    ...
                    OP_C_NEW_STREAM_BIDI(
                        c,
                        C_BIDI_ID(2)) OP_C_WRITE(c, "Strawberry", 10)
