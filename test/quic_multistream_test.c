...
static int skip_time_ms(struct helper* h, struct helper_local* hl) {
  if (!TEST_true(CRYPTO_THREAD_write_lock(h->time_lock))) return 0;
...
static int trigger_key_update(struct helper* h, struct helper_local* hl) {
  if (!TEST_true(SSL_key_update(h->c_conn, SSL_KEY_UPDATE_REQUESTED))) return 0;
...
static int check_key_update_ge(struct helper* h, struct helper_local* hl) {
   ...
   */
  if (!TEST_int64_t_ge(diff, 0) || !TEST_int64_t_le(diff, 1)) return 0;

  /* Caller specifies a minimum number of RXKEs which must have happened. */
  if (!TEST_uint64_t_ge((uint64_t)rxke, hl->check_op->arg2)) return 0;
...
static int check_key_update_lt(struct helper* h, struct helper_local* hl) {
  ...
  if (!TEST_uint64_t_lt(txke, hl->check_op->arg2)) return 0;
...
static int join_server_thread(struct helper* h) {
  ...
  if (h->server_thread.t == NULL) return 1;
...
static QUIC_TSERVER* s_lock(struct helper* h, struct helper_local* hl) {
  ...
  if (h->server_thread.m == NULL || *p_checked_out) return h->s;
...
static void s_unlock(struct helper* h, struct helper_local* hl) {
  ...
  if (h->server_thread.m == NULL || !*p_checked_out) return;
...
static int helper_init(struct helper* h, const char* script_name,
  ...
  if (bdata == NULL) goto err;

  if (!TEST_ptr(h->time_lock = CRYPTO_THREAD_lock_new())) goto err;
  ...
  if (!TEST_int_ge(h->s_fd, 0)) goto err;

  if (!TEST_true(BIO_socket_nbio(h->s_fd, 1))) goto err;
  ...
  if (!TEST_true(BIO_bind(h->s_fd, h->s_net_bio_orig_addr, 0))) goto err;

  info.addr = h->s_net_bio_addr;
  if (!TEST_true(BIO_sock_info(h->s_fd, BIO_SOCK_INFO_ADDRESS, &info)))
    goto err;

  if (!TEST_int_gt(BIO_ADDR_rawport(h->s_net_bio_addr), 0)) goto err;

  if (!TEST_ptr(h->s_net_bio = h->s_net_bio_own = BIO_new_dgram(h->s_fd, 0)))
    goto err;

  if (!BIO_up_ref(h->s_net_bio)) goto err;

  if (need_injector) {
    h->s_qtf_wbio = h->s_qtf_wbio_own = BIO_new(qtest_get_bio_method());
    if (!TEST_ptr(h->s_qtf_wbio)) goto err;

    if (!TEST_ptr(BIO_push(h->s_qtf_wbio, h->s_net_bio))) goto err;
  ...
  if (!blocking) h->s = h->s_priv;

  if (need_injector) {
    h->qtf = qtest_create_injector(h->s_priv);
    if (!TEST_ptr(h->qtf)) goto err;
    bdata->fault = h->qtf;
  ...
  if (!TEST_int_ge(h->c_fd, 0)) goto err;

  if (!TEST_true(BIO_socket_nbio(h->c_fd, 1))) goto err;

  if (!TEST_ptr(h->c_net_bio = h->c_net_bio_own = BIO_new_dgram(h->c_fd, 0)))
    goto err;

  if (!TEST_true(BIO_dgram_set_peer(h->c_net_bio, h->s_net_bio_addr))) goto err;

  if (!TEST_ptr(h->c_ctx = SSL_CTX_new(OSSL_QUIC_client_method()))) goto err;

  /* Set title for qlog purposes. */
  BIO_snprintf(title, sizeof(title), "quic_multistream_test: %s", script_name);
  if (!TEST_true(ossl_quic_set_diag_title(h->c_ctx, title))) goto err;

  if (!TEST_ptr(h->c_conn = SSL_new(h->c_ctx))) goto err;
  ...
  if (!TEST_true(BIO_up_ref(h->c_net_bio))) goto err;

  SSL_set0_wbio(h->c_conn, h->c_net_bio);

  if (!TEST_true(SSL_set_blocking_mode(h->c_conn, h->blocking))) goto err;

#if defined(OPENSSL_THREADS)
  if (!TEST_ptr(h->misc_m = ossl_crypto_mutex_new())) goto err;
  if (!TEST_ptr(h->misc_cv = ossl_crypto_condvar_new())) goto err;
#endif

  if (h->blocking) {
#if defined(OPENSSL_THREADS)
    if (!TEST_ptr(h->server_thread.m = ossl_crypto_mutex_new())) goto err;

    if (!TEST_ptr(h->server_thread.c = ossl_crypto_condvar_new())) goto err;

    h->server_thread.t =
        ossl_crypto_thread_native_start(server_helper_thread, h, 1);
    if (!TEST_ptr(h->server_thread.t)) goto err;
#else
...
static void helper_local_cleanup(struct helper_local* hl) {
  ...
  if (hl->thread_idx >= 0) helper_cleanup_streams(&hl->c_streams);
...
static STREAM_INFO* get_stream_info(LHASH_OF(STREAM_INFO)* lh,
  ...
  if (!TEST_ptr(stream_name)) return NULL;

  if (!strcmp(stream_name, "DEFAULT")) return NULL;
...
static SSL* helper_local_get_c_stream(struct helper_local* hl,
  ...
  if (!strcmp(stream_name, "DEFAULT")) return hl->h->c_conn;
...
static int helper_set_s_stream(struct helper* h, const char* stream_name,
  ...
  if (!strcmp(stream_name, "DEFAULT")) return 0;
...
static uint64_t helper_get_s_stream(struct helper* h, const char* stream_name) {
  ...
  if (!strcmp(stream_name, "DEFAULT")) return UINT64_MAX;

  info = get_stream_info(h->s_streams, stream_name);
  if (info == NULL) return UINT64_MAX;
...
static int check_consistent_want(SSL* s, int ret) {
  ...
  if (!ok) TEST_error("got error=%d, want=%d", ec, w);
...
#define REPEAT_SLOTS 8
  ...
  if (!TEST_true(helper_local_init(hl, h, thread_idx))) goto out;
...
#define C_SPIN_AGAIN()                           \
  ...
  for (;;) {
    ...
    switch (op->op) {
      case OPK_END:
        if (!TEST_size_t_eq(repeat_stack_len, 0)) goto out;
        ...
        if (thread_idx < 0) {
          ...
          for (i = 0; i < h->num_threads; ++i) {
            if (h->threads[i].m == NULL) continue;
      ...
      case OPK_BEGIN_REPEAT:
        ...
        if (!TEST_size_t_gt(op->arg1, 0)) goto out;
      ...
      case OPK_END_REPEAT:
        if (!TEST_size_t_gt(repeat_stack_len, 0)) goto out;
      ...
      case OPK_C_CONNECT_WAIT: {
        ...
        if (!check_consistent_want(c_tgt, ret)) goto out;
        if (ret != 1) {
          if (!h->blocking && is_want(h->c_conn, ret)) C_SPIN_AGAIN();

          if (op->arg1 == 0 && !TEST_int_eq(ret, 1)) goto out;
        }
      } break;
      ...
      case OPK_S_WRITE: {
        ...
        if (!TEST_uint64_t_ne(s_stream_id, UINT64_MAX)) goto out;
      ...
      case OPK_C_CONCLUDE: {
        if (!TEST_true(SSL_stream_conclude(c_tgt, 0))) goto out;
      } break;
      ...
      case OPK_C_READ_EXPECT: {
        ...
        r = SSL_read_ex(c_tgt, tmp_buf + offset, op->arg1 - offset,
                        &bytes_read);
        if (!check_consistent_want(c_tgt, r)) goto out;
      ...
      case OPK_C_EXPECT_FIN: {
        ...
        if (is_want(c_tgt, 0)) C_SPIN_AGAIN();

        if (!TEST_int_eq(SSL_get_error(c_tgt, 0), SSL_ERROR_ZERO_RETURN))
          goto out;

        if (!TEST_int_eq(SSL_want(c_tgt), SSL_NOTHING)) goto out;
      } break;
      ...
      case OPK_C_DETACH: {
        ...
        if (!TEST_ptr(op->stream_name)) goto out;

        if (!TEST_ptr(c_stream = ossl_quic_detach_stream(h->c_conn))) goto out;
      ...
      case OPK_C_ATTACH: {
        ...
        if (!TEST_true(ossl_quic_attach_stream(h->c_conn, c_tgt))) goto out;
      ...
      case OPK_C_NEW_STREAM: {
        ...
        if (!allow_fail && !TEST_ptr(c_stream)) goto out;
      ...
      case OPK_S_NEW_STREAM: {
        ...
        if (!TEST_true(ossl_quic_tserver_stream_new(ACQUIRE_S(), op->arg1 > 0,
                                                    &stream_id)))
          goto out;
      ...
      case OPK_S_ACCEPT_STREAM_WAIT: {
        ...
        if (!TEST_uint64_t_eq(s_stream_id, UINT64_MAX)) goto out;

        if (!TEST_ptr(op->stream_name)) goto out;

        new_stream_id = ossl_quic_tserver_pop_incoming_stream(ACQUIRE_S());
        if (new_stream_id == UINT64_MAX) S_SPIN_AGAIN();
      ...
      case OPK_C_FREE_STREAM: {
        if (!TEST_ptr(c_tgt) || !TEST_true(!SSL_is_connection(c_tgt))) goto out;
      ...
      case OPK_C_EXPECT_CONN_CLOSE_INFO: {
        ...
        if (!TEST_int_eq(expect_app, (cc_info.flags &
      ...
      case OPK_C_WRITE_FAIL: {
        ...
        if (!TEST_false(r) || !check_consistent_want(c_tgt, r)) goto out;
      } break;
      ...
      case OPK_S_READ_FAIL: {
        ...
        ret = ossl_quic_tserver_read(ACQUIRE_S(), s_stream_id, buf, sizeof(buf),
                                     &bytes_read);
        if (!TEST_true(ret == 0 || (op->arg1 && bytes_read == 0))) goto out;
      } break;
      ...
      case OPK_NEW_THREAD: {
        ...
        if (!TEST_ptr(h->threads)) goto out;
      ...
      case OPK_S_NEW_TICKET:
        if (!TEST_true(ossl_quic_tserver_new_ticket(ACQUIRE_S()))) goto out;
        break;
...
out:
  ...
  if (!testresult) {
    ...
    if (tcause != NULL) {
      ...
      TEST_info(
          "server side is closed: %llu(%s)/%llu(%s), "
          "%s, %s, reason: \"%s\"",
...
static int run_script(const struct script_op* script, const char* script_name,
  ...
  if (!TEST_true(run_script_worker(&h, script, script_name, -1))) goto out;

#if defined(OPENSSL_THREADS)
  if (!TEST_true(join_threads(h.threads, h.num_threads))) goto out;
#endif
        ...
        OP_C_SET_INCOMING_STREAM_POLICY(
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
                a, "apple", 5) OP_S_WRITE(a, "orange",
                                          6) OP_C_READ_EXPECT(DEFAULT, "orange",
                                                              6)

                OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1)) OP_C_WRITE(
                    b, "flamingo",
                    8) OP_C_CONCLUDE(b) OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1))
                    OP_S_READ_EXPECT(b, "flamingo", 8) OP_S_EXPECT_FIN(
                        b) OP_S_WRITE(b, "gargoyle", 8) OP_S_CONCLUDE(b)
...
static const struct script_op script_9[] = {
    OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
        ...
        OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_AUTO_UNI)
            OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "apple", 5)
                OP_C_READ_EXPECT(DEFAULT, "apple", 5) OP_C_WRITE(
                    DEFAULT, "orange", 6) OP_S_READ_EXPECT(a, "orange", 6)
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "foo", 3)
                OP_S_EXPECT_FIN(a) OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1))
                    OP_S_READ_EXPECT(b, "foo", 3) OP_S_EXPECT_FIN(b)
                        OP_S_BIND_STREAM_ID(c, C_BIDI_ID(2)) OP_S_READ_EXPECT(
                            c, "foo", 3) OP_S_EXPECT_FIN(c)
            ...
            OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
                ...
                OP_CHECK(override_key_update, 1)
                    ...
                    OP_BEGIN_REPEAT(200)
                        ...
                        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_READ_EXPECT(
                            a, "apple", 5) OP_CHECK(skip_time_ms, 8)
                ...
                OP_C_WRITE(DEFAULT, "orange", 6)
                    OP_S_READ_EXPECT(a, "orange", 6)
                        ...
                        OP_S_WRITE(a, "strawberry", 10) OP_C_READ_EXPECT(
                            DEFAULT, "strawberry", 10)

                            OP_CHECK(check_key_update_lt, 1) OP_CHECK(
                                trigger_key_update, 0)
...
static int script_21_inject_plain(struct helper* h, QUIC_PKT_HDR* hdr,
  ...
  if (h->inject_word0 == 0 || hdr->type != h->inject_word0) return 1;

  if (!TEST_true(
          WPACKET_init_static_len(&wpkt, frame_buf, sizeof(frame_buf), 0)))
    return 0;

  if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, h->inject_word1))) goto err;
  ...
  switch (h->inject_word1) {
    ...
    case OSSL_QUIC_FRAME_TYPE_RETIRE_CONN_ID:
       ...
       */
      if (!TEST_true(WPACKET_put_bytes_u64(&wpkt, (uint64_t)0))) goto err;
      break;
    ...
    case OSSL_QUIC_FRAME_TYPE_NEW_TOKEN:
      ...
      if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, (uint64_t)1))) goto err;

      /* 1 bytes of token data, to match the above length */
      if (!TEST_true(WPACKET_put_bytes_u8(&wpkt, (uint8_t)0))) goto err;
      break;
    case OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID:
      ...
      if (!TEST_true(WPACKET_put_bytes_u8(&wpkt, (uint8_t)1))) goto err;

      /* The connection id, to match the above length */
      if (!TEST_true(WPACKET_put_bytes_u8(&wpkt, (uint8_t)0))) goto err;

      /* 16 bytes total for the SRT */
      if (!TEST_true(WPACKET_memset(&wpkt, 0, 16))) goto err;
  ...
  if (!qtest_fault_prepend_frame(h->qtf, frame_buf, written)) goto err;
...
static int script_23_inject_plain(struct helper* h, QUIC_PKT_HDR* hdr,
  ...
  if (h->inject_word0 == 0 || hdr->type != QUIC_PKT_TYPE_1RTT) return 1;
...
static int script_32_inject_plain(struct helper* h, QUIC_PKT_HDR* hdr,
  ...
  if (hdr->type != QUIC_PKT_TYPE_1RTT) return 1;
  ...
  for (i = 0; i < flen; ++i)
    if (!TEST_true(WPACKET_put_bytes_u8(&wpkt, 0x42))) goto err;
                ...
                OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
                    ...
                    OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
                        OP_S_READ_EXPECT(a, "apple", 5)
                            ...
                            OP_SET_INJECT_WORD(C_BIDI_ID(0) + 1, 2) OP_S_WRITE(
                                a, "orange", 6)
...
static int script_39_inject_plain(struct helper* h, QUIC_PKT_HDR* hdr,
  ...
  for (i = 0; i < new_cid.id_len && i < OSSL_NELEM(new_cid.id); ++i)
    if (!TEST_true(WPACKET_put_bytes_u8(&wpkt, new_cid.id[i]))) goto err;

  for (; i < new_cid.id_len; ++i)
    if (!TEST_true(WPACKET_put_bytes_u8(&wpkt, 0x55))) goto err;
...
static void script_41_trace(int write_p, int version, int content_type,
  ...
  if (frame_type != OSSL_QUIC_FRAME_TYPE_PATH_RESPONSE) return;
...
static int script_41_check(struct helper* h, struct helper_local* hl) {
  ...
  if (!TEST_uint64_t_gt(h->scratch0, 0)) return 0;
...
static int script_44_inject_plain(struct helper* h, QUIC_PKT_HDR* hdr,
  ...
  if (!TEST_true(ossl_quic_wire_encode_padding(&wpkt, 1))) goto err;
...
static int script_52_inject_plain(struct helper* h, QUIC_PKT_HDR* hdr,
  ...
  if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, type))) goto err;

  if (type == OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED)
    if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, C_BIDI_ID(0)))) goto err;

  if (!TEST_true(WPACKET_quic_write_vlint(&wpkt, 0xFFFFFF))) goto err;
        ...
        OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
            ...
            OP_SET_INJECT_WORD(1, OSSL_QUIC_FRAME_TYPE_DATA_BLOCKED)
                ...
                OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(DEFAULT, "orange",
                    ...
                    OP_C_WRITE(DEFAULT, "Strawberry", 10) OP_S_READ_EXPECT(
                        a, "Strawberry", 10)
...
static int script_53_inject_plain(struct helper* h, QUIC_PKT_HDR* hdr,
  ...
  if (!TEST_ptr(frame_buf = OPENSSL_malloc(frame_len))) return 0;
            ...
            OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
                ...
                OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
                    ...
                    OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                        ...
                        OP_SET_INJECT_WORD(S_BIDI_ID(20) + 1, 1) OP_S_WRITE(
                            a, "orange", 6) OP_C_READ_EXPECT(a, "orange", 6)
                ...
                OP_S_NEW_STREAM_BIDI(a, S_BIDI_ID(0)) OP_S_WRITE(a, "apple", 5)
                    ...
                    OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
                        ...
                        OP_SET_INJECT_WORD(S_BIDI_ID(0) + 1,
                                           OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA)
                            OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(
                                a, "orange", 6) OP_C_WRITE(a, "Strawberry", 10)
                                OP_S_READ_EXPECT(a, "Strawberry", 10)
...
static int script_72_check(struct helper* h, struct helper_local* hl) {
  if (!TEST_uint64_t_ge(h->fail_count, 50)) return 0;
...
static int generate_version_neg(WPACKET* wpkt, uint32_t version) {
  ...
  if (!TEST_true(ossl_quic_wire_encode_pkt_hdr(wpkt, 0, &hdr, NULL))) return 0;
...
static int server_gen_version_neg(struct helper* h, BIO_MSG* msg,
  ...
  if (!TEST_ptr(buf = BUF_MEM_new())) goto err;

  if (!TEST_true(WPACKET_init(&wpkt, buf))) goto err;

  have_wpkt = 1;

  generate_version_neg(&wpkt, version);

  if (!TEST_true(WPACKET_get_total_written(&wpkt, &l))) goto err;

  if (!TEST_true(qtest_fault_resize_datagram(h->qtf, l))) return 0;
...
static int script_74_alter_version(const QUIC_PKT_HDR* hdrin,
  ...
  *numout = numin;
  ...
  if (do_mutation == 0) return 1;
  do_mutation = 0;

  if (hdrin->type == QUIC_PKT_TYPE_INITIAL) (*hdrout)->version = 0xdeadbeef;
  return 1;
}
...
static int trigger_late_session_ticket(struct helper* h,
  ...
  if (!TEST_true(ossl_quic_tserver_new_ticket(ACQUIRE_S()))) return 0;

  return 1;
}

static int check_got_session_ticket(struct helper* h, struct helper_local* hl) {
  if (!TEST_size_t_gt(new_session_count, 0)) return 0;
...
static int script_80_gen_new_conn_id(struct helper* h, QUIC_PKT_HDR* hdr,
  ...
  if (!qtest_fault_prepend_frame(h->qtf, frame_buf, l)) goto err;
...
static int check_write_buf_stat(struct helper* h, struct helper_local* hl) {
  ...
  if (!TEST_ptr(c_a = helper_local_get_c_stream(hl, "a"))) return 0;
...
ossl_unused static int script_88_poll(struct helper* h,
                                      struct helper_local* hl) {
  int ok = 1, ret, expected_ret = 1;
  static const struct timeval timeout = {0};
  ...
  if (!TEST_int_eq(ret, expected_ret)) ok = 0;
  ...
  if (!TEST_size_t_eq(processed, result_count)) ok = 0;
...
ossl_unused static int script_88_poll_conly(struct helper* h,
  ...
  while (done == 0 && ok == 1) {
    ok = SSL_poll(items, OSSL_NELEM(items), sizeof(SSL_POLL_ITEM), &timeout, 0,
                  &result_count);

    if (!TEST_int_eq(ok, 1)) continue;

    if (result_count == 0) OSSL_sleep(10);
