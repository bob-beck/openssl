...
struct helper
{
  ...
  int (*qtf_packet_plain_cb)(struct helper* h,
                             QUIC_PKT_HDR* hdr,
                             unsigned char* buf,
                             size_t buf_len);
...
check_rejected(struct helper* h, struct helper_local* hl)
{
  ...
  if (!ossl_quic_tserver_stream_has_peer_stop_sending(
        ACQUIRE_S(), stream_id, NULL) ||
...
check_stream_reset(struct helper* h, struct helper_local* hl)
{
  ...
  if (!ossl_quic_tserver_stream_has_peer_reset_stream(
        ACQUIRE_S(), stream_id, &aec)) {
    h->check_spin_again = 1;
    return 0;
  }
            ...
            int need_injector)
{
  ...
  if (!TEST_true(BIO_ADDR_rawmake(
        h->s_net_bio_orig_addr, AF_INET, &ina, sizeof(ina), 0)))
    goto err;
...
static int
helper_local_set_c_stream(struct helper_local* hl,
                          const char* stream_name,
                          SSL* c_stream)
{
...
static int
helper_set_s_stream(struct helper* h,
                    const char* stream_name,
                    uint64_t s_stream_id)
{
  STREAM_INFO* info;
...
static int
helper_packet_plain_listener(QTEST_FAULT* qtf,
                             QUIC_PKT_HDR* hdr,
                             unsigned char* buf,
                             size_t buf_len,
                             void* arg)
...
static int
helper_handshake_listener(QTEST_FAULT* fault,
                          unsigned char* buf,
                          size_t buf_len,
                          void* arg)
{
...
static int
helper_datagram_listener(QTEST_FAULT* fault,
                         BIO_MSG* msg,
                         size_t stride,
                         void* arg)
{
...
static int
run_script_worker(struct helper* h,
                  const struct script_op* script,
                  const char* script_name,
                  int thread_idx)
{
  ...
  for (;;) {
    ...
    switch (op->op) {
      case OPK_END:
        ...
        TEST_info(
          "script \"%s\" finished on thread %d", script_name, thread_idx);
        testresult = 1;
        goto out;
      ...
      case OPK_S_WRITE: {
        ...
        if (!TEST_true(ossl_quic_tserver_write(
              ACQUIRE_S(), s_stream_id, op->arg0, op->arg1, &bytes_written)) ||
            !TEST_size_t_eq(bytes_written, op->arg1))
          goto out;
      } break;
      ...
      case OPK_C_READ_EXPECT: {
        ...
        r =
          SSL_read_ex(c_tgt, tmp_buf + offset, op->arg1 - offset, &bytes_read);
      ...
      case OPK_S_READ_EXPECT: {
        ...
        if (!TEST_true(ossl_quic_tserver_read(ACQUIRE_S(),
                                              s_stream_id,
                                              tmp_buf + offset,
                                              op->arg1 - offset,
                                              &bytes_read)))
      ...
      case OPK_S_READ_FAIL: {
        ...
        ret = ossl_quic_tserver_read(
          ACQUIRE_S(), s_stream_id, buf, sizeof(buf), &bytes_read);
...
static int
run_script(const struct script_op* script,
           const char* script_name,
           int free_order,
           int blocking)
{
...
run_script_child_thread(void* arg)
{
  ...
  testresult = run_script_worker(
    args->h, args->script, args->script_name, args->thread_idx);
...
static const struct script_op script_2[] = {
  OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT() OP_C_SET_INCOMING_STREAM_POLICY(
    ...
    OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(
      a,
      "apple",
      5) OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(DEFAULT, "orange", 6)

      OP_C_NEW_STREAM_BIDI(b, C_BIDI_ID(1)) OP_C_WRITE(b, "flamingo", 8)
        OP_C_CONCLUDE(b) OP_S_BIND_STREAM_ID(b, C_BIDI_ID(1)) OP_S_READ_EXPECT(
          b,
          "flamingo",
          8) OP_S_EXPECT_FIN(b) OP_S_WRITE(b, "gargoyle", 8) OP_S_CONCLUDE(b)
          OP_C_READ_EXPECT(b, "gargoyle", 8) OP_C_EXPECT_FIN(b)

            OP_C_NEW_STREAM_UNI(c, C_UNI_ID(0)) OP_C_WRITE(c, "elephant", 8)
              OP_C_CONCLUDE(c) OP_S_BIND_STREAM_ID(c, C_UNI_ID(0))
                OP_S_READ_EXPECT(c, "elephant", 8) OP_S_EXPECT_FIN(
                  c) OP_S_WRITE_FAIL(c)
                  ...
                  OP_C_ACCEPT_STREAM_NONE()
                    ...
                    OP_S_NEW_STREAM_BIDI(
                      d,
                      S_BIDI_ID(0)) OP_S_WRITE(d, "frog", 4) OP_S_CONCLUDE(d)
                        ...
                        OP_C_READ_EXPECT(d, "frog", 4) OP_C_EXPECT_FIN(d)
                          ...
                          OP_S_NEW_STREAM_BIDI(e, S_BIDI_ID(1)) OP_S_WRITE(
                            ...
                            OP_C_ACCEPT_STREAM_WAIT(e) OP_C_READ_EXPECT(
                              e,
                              "mixture",
                              7) OP_C_EXPECT_FIN(e) OP_C_WRITE(e, "ramble", 6)
                              OP_S_READ_EXPECT(e, "ramble", 6) OP_C_CONCLUDE(
                                e) OP_S_EXPECT_FIN(e)
                                ...
                                OP_S_NEW_STREAM_UNI(f, S_UNI_ID(0)) OP_S_WRITE(
                                  ...
                                  OP_C_ACCEPT_STREAM_WAIT(f)
                                    OP_C_ACCEPT_STREAM_NONE() OP_C_READ_EXPECT(
                                      f,
                                      "yonder",
      ...
      OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "appleby", 7)
        ...
        OP_S_WRITE(a, "hello", 5) OP_C_READ_EXPECT(a, "hello", 5)
          ...
          OP_C_WRITE_FAIL(DEFAULT) OP_C_ATTACH(a)
            OP_C_WRITE(DEFAULT, "is here", 7) OP_S_READ_EXPECT(a, "is here", 7)
        ...
        OP_S_NEW_STREAM_BIDI(a, ANY_ID) OP_S_WRITE(a, "foo", 3) OP_S_CONCLUDE(a)
          ...
          OP_S_NEW_STREAM_BIDI(b, ANY_ID) OP_S_WRITE(b, "foo", 3)
            OP_S_CONCLUDE(b)
              ...
              OP_S_NEW_STREAM_BIDI(c, ANY_ID) OP_S_WRITE(c, "foo", 3)
                OP_S_CONCLUDE(c)

                  OP_S_NEW_STREAM_BIDI(d, ANY_ID) OP_S_WRITE(d, "foo", 3)
                    OP_S_CONCLUDE(d)

                      OP_S_NEW_STREAM_BIDI(e, ANY_ID) OP_S_WRITE(e, "foo", 3)
                        OP_S_CONCLUDE(e)
                ...
                OP_S_EXPECT_FIN(c) OP_S_BIND_STREAM_ID(d, C_BIDI_ID(3))
                  OP_S_READ_EXPECT(d, "foo", 3) OP_S_EXPECT_FIN(d)
                    OP_S_BIND_STREAM_ID(e, C_BIDI_ID(4))
                      OP_S_READ_EXPECT(e, "foo", 3) OP_S_EXPECT_FIN(e)
            ...
            OP_C_READ_EXPECT(DEFAULT, "strawberry", 10)
              ...
              OP_CHECK(check_key_update_lt, 1) OP_CHECK(trigger_key_update, 0)
                ...
                OP_C_WRITE(DEFAULT, "orange", 6)
                  OP_S_READ_EXPECT(a, "orange", 6) OP_S_WRITE(a, "ok", 2)
...
static int
script_20_wait(struct helper* h, volatile uint64_t* counter, uint64_t threshold)
{
#if defined(OPENSSL_THREADS)
  int stop = 0;
...
static int
script_21_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_22_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_23_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_24_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_28_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
            ...
            OP_S_BIND_STREAM_ID(b, C_UNI_ID(0)) OP_S_READ_EXPECT(b, "apple", 5)
              ...
              OP_SET_INJECT_WORD(C_UNI_ID(0) + 1,
                ...
                OP_S_WRITE(a, "fruit", 5)
                  ...
                  OP_C_EXPECT_CONN_CLOSE_INFO(OSSL_QUIC_ERR_STREAM_STATE_ERROR,
                                              0,
                                              0)
        ...
        OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "orange", 6)
          ...
          OP_C_NEW_STREAM_UNI(b, C_UNI_ID(0)) OP_C_WRITE(b, "apple", 5)
            ...
            OP_S_BIND_STREAM_ID(b, C_UNI_ID(0)) OP_S_READ_EXPECT(b, "apple", 5)
              ...
              OP_SET_INJECT_WORD(C_UNI_ID(1) + 1,
                                 OSSL_QUIC_FRAME_TYPE_RESET_STREAM)
                OP_S_WRITE(a, "fruit", 5)
    ...
    OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
      ...
      OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
        ...
        OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
          ...
          OP_SET_INJECT_WORD(
            S_UNI_ID(0) + 1,
            OSSL_QUIC_FRAME_TYPE_STOP_SENDING) OP_S_WRITE(a, "orange", 6)
...
static int
script_32_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
    ...
    OP_C_CONNECT_WAIT() OP_C_SET_DEFAULT_STREAM_MODE(
      ...
      OP_S_NEW_STREAM_UNI(a, S_UNI_ID(0)) OP_S_WRITE(a, "apple", 5)
        ...
        OP_C_ACCEPT_STREAM_WAIT(a) OP_C_READ_EXPECT(a, "apple", 5)
          ...
          OP_SET_INJECT_WORD(
            S_UNI_ID(0) + 1,
            OSSL_QUIC_FRAME_TYPE_MAX_STREAM_DATA) OP_S_WRITE(a, "orange", 6)
      ...
      OP_C_NEW_STREAM_UNI(a, C_UNI_ID(0)) OP_C_WRITE(a, "apple", 5)
        ...
        OP_S_BIND_STREAM_ID(a, C_UNI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
          ...
          OP_S_NEW_STREAM_UNI(b, S_UNI_ID(0)) OP_SET_INJECT_WORD(
            C_UNI_ID(0) + 1,
            OSSL_QUIC_FRAME_TYPE_STREAM_DATA_BLOCKED) OP_S_WRITE(b, "orange", 5)
...
static int
script_39_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_41_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_42_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
      ...
      OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
        ...
        OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
          ...
          OP_SET_INJECT_WORD(1, 0x100000 /* 1 MiB */) OP_S_WRITE(a, "orange", 6)
            ...
            OP_C_EXPECT_CONN_CLOSE_INFO(OSSL_QUIC_ERR_CRYPTO_BUFFER_EXCEEDED,
                                        0,
                                        0)
...
static int
script_44_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_46_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_52_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static const struct script_op script_52[] = {
  OP_S_SET_INJECT_PLAIN(script_52_inject_plain) OP_C_SET_ALPN(
    ...
    OP_C_WRITE(DEFAULT, "apple", 5) OP_S_BIND_STREAM_ID(
      a,
      C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)

      OP_SET_INJECT_WORD(1, OSSL_QUIC_FRAME_TYPE_DATA_BLOCKED)

        OP_S_WRITE(a, "orange", 6) OP_C_READ_EXPECT(DEFAULT, "orange", 6)

          OP_C_WRITE(DEFAULT, "Strawberry", 10) OP_S_READ_EXPECT(a,
                                                                 "Strawberry",
                                                                 10)
...
static int
script_53_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_54_inject_handshake(struct helper* h, unsigned char* buf, size_t buf_len)
{
  size_t i;
...
static int
script_58_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_61_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_65_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_66_inject_plain(struct helper* h,
                       QUIC_PKT_HDR* hdr,
                       unsigned char* buf,
                       size_t len)
{
...
static int
script_68_inject_handshake(struct helper* h, unsigned char* msg, size_t msglen)
{
      ...
      OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_C_WRITE(a, "apple", 5)
        OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
          ...
          OP_CHECK(set_max_early_data,
                   0xfffffffe) OP_S_NEW_TICKET() OP_S_WRITE(a, "orange", 6)
...
script_74_arm_packet_mutator(struct helper* h, struct helper_local* hl)
{
  ...
  do_mutation = 1;
  if (!ossl_quic_channel_set_mutator(
        ch, script_74_alter_version, script_74_finish_mutation, NULL))
    return 0;
  return 1;
}
...
static QUIC_STATELESS_RESET_TOKEN test_reset_token = { { 0xde,
                                                         0xad,
                                                         0xbe,
                                                         0xef,
                                                         0xde,
 ...
 */
static int
script_80_send_stateless_reset(struct helper* h,
                               QUIC_PKT_HDR* hdr,
                               unsigned char* buf,
                               size_t len)
{
  ...
  if (!TEST_int_eq(
        SSL_inject_net_dgram(
          h->c_conn, databuf, sizeof(databuf), NULL, h->s_net_bio_addr),
        1))
    return 0;
...
static int
script_80_gen_new_conn_id(struct helper* h,
                          QUIC_PKT_HDR* hdr,
                          unsigned char* buf,
                          size_t len)
{
  ...
  memcpy(ncid.stateless_reset.token,
...
static int
script_80_inject_pkt(struct helper* h,
                     QUIC_PKT_HDR* hdr,
                     unsigned char* buf,
                     size_t len)
{
      ...
      OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0)) OP_S_READ_EXPECT(a, "apple", 5)
        OP_SET_INJECT_WORD(1, 0) OP_S_WRITE(a, "apple", 5)
          OP_C_READ_EXPECT(DEFAULT, "apple", 5) OP_SET_INJECT_WORD(0, 1)
            OP_S_WRITE(a, "apple", 5) OP_C_EXPECT_CONN_CLOSE_INFO(0, 0, 1)
              OP_END
};
...
static const struct script_op script_83[] = {
  OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
    ...
    OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
      ...
      OP_CHECK(cannot_change_idle_timeout,
               0) OP_CHECK2(check_idle_timeout,
...
script_85_poll(struct helper* h, struct helper_local* hl)
{
  ...
  for (i = 0; i < OSSL_NELEM(items); ++i)
    if (!TEST_uint64_t_eq(items[i].revents, expected_revents[i])) {
      TEST_error(
        "mismatch at index %zu in poll results, mode %d", i, (int)mode);
      ok = 0;
    }
...
script_88_poll_conly(struct helper* h, struct helper_local* hl)
{
  ...
  t_limit =
    ossl_time_add(ossl_time_now(), ossl_ticks2time(5 * OSSL_TIME_SECOND));
  while (done == 0 && ok == 1) {
    ok = SSL_poll(items,
                  OSSL_NELEM(items),
                  sizeof(SSL_POLL_ITEM),
...
static const struct script_op script_88[] = {
  OP_SKIP_IF_BLOCKING(16) OP_C_SET_ALPN("ossltest") OP_C_CONNECT_WAIT()
    ...
    OP_C_SET_DEFAULT_STREAM_MODE(SSL_DEFAULT_STREAM_MODE_NONE)
      ...
      OP_C_NEW_STREAM_BIDI(a, C_BIDI_ID(0)) OP_S_BIND_STREAM_ID(a, C_BIDI_ID(0))
...
test_script(int idx)
{
  ...
  TEST_info("Running script %d (order=%d, blocking=%d)",
            script_idx + 1,
            free_order,
            blocking);
