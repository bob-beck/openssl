...
#define OP_C_WRITE(stream_name, buf, buf_len) {OPK_C_WRITE, (buf), (buf_len), NULL, #stream_name},
#define OP_S_WRITE(stream_name, buf, buf_len) {OPK_S_WRITE, (buf), (buf_len), NULL, #stream_name},
#define OP_C_READ_EXPECT(stream_name, buf, buf_len) {OPK_C_READ_EXPECT, (buf), (buf_len), NULL, #stream_name},
#define OP_S_READ_EXPECT(stream_name, buf, buf_len) {OPK_S_READ_EXPECT, (buf), (buf_len), NULL, #stream_name},
#define OP_C_EXPECT_FIN(stream_name) {OPK_C_EXPECT_FIN, NULL, 0, NULL, #stream_name},
#define OP_S_EXPECT_FIN(stream_name) {OPK_S_EXPECT_FIN, NULL, 0, NULL, #stream_name},
#define OP_C_CONCLUDE(stream_name) {OPK_C_CONCLUDE, NULL, 0, NULL, #stream_name},
#define OP_S_CONCLUDE(stream_name) {OPK_S_CONCLUDE, NULL, 0, NULL, #stream_name},
...
#define OP_C_NEW_STREAM_BIDI(stream_name, expect_id) {OPK_C_NEW_STREAM, NULL, 0, NULL, #stream_name, (expect_id)},
...
#define OP_C_NEW_STREAM_UNI_EX(stream_name, expect_id, flags)                                                          \
    {OPK_C_NEW_STREAM, NULL, (flags) | SSL_STREAM_FLAG_UNI, NULL, #stream_name, (expect_id)},
#define OP_S_NEW_STREAM_BIDI(stream_name, expect_id) {OPK_S_NEW_STREAM, NULL, 0, NULL, #stream_name, (expect_id)},
#define OP_S_NEW_STREAM_UNI(stream_name, expect_id) {OPK_S_NEW_STREAM, NULL, 1, NULL, #stream_name, (expect_id)},
#define OP_C_ACCEPT_STREAM_WAIT(stream_name) {OPK_C_ACCEPT_STREAM_WAIT, NULL, 0, NULL, #stream_name},
#define OP_C_ACCEPT_STREAM_NONE() {OPK_C_ACCEPT_STREAM_NONE, NULL, 0, NULL, NULL},
#define OP_C_FREE_STREAM(stream_name) {OPK_C_FREE_STREAM, NULL, 0, NULL, #stream_name},
#define OP_C_SET_DEFAULT_STREAM_MODE(mode) {OPK_C_SET_DEFAULT_STREAM_MODE, NULL, (mode), NULL, NULL},
#define OP_C_SET_INCOMING_STREAM_POLICY(policy) {OPK_C_SET_INCOMING_STREAM_POLICY, NULL, (policy), NULL, NULL},
#define OP_C_SHUTDOWN(reason, flags) {OPK_C_SHUTDOWN, (reason), (flags), NULL, NULL},
#define OP_C_SHUTDOWN_WAIT(reason, flags) {OPK_C_SHUTDOWN_WAIT, (reason), (flags), NULL, NULL},
...
#define OP_S_EXPECT_CONN_CLOSE_INFO(ec, app, remote)                                                                   \
    {OPK_S_EXPECT_CONN_CLOSE_INFO,                                                                                     \
     ...
     NULL,                                                                                                             \
     (ec)},
#define OP_S_BIND_STREAM_ID(stream_name, stream_id) {OPK_S_BIND_STREAM_ID, NULL, 0, NULL, #stream_name, (stream_id)},
#define OP_C_WAIT_FOR_DATA(stream_name) {OPK_C_WAIT_FOR_DATA, NULL, 0, NULL, #stream_name},
#define OP_C_WRITE_FAIL(stream_name) {OPK_C_WRITE_FAIL, NULL, 0, NULL, #stream_name},
#define OP_S_WRITE_FAIL(stream_name) {OPK_S_WRITE_FAIL, NULL, 0, NULL, #stream_name},
#define OP_C_READ_FAIL(stream_name) {OPK_C_READ_FAIL, NULL, 0, NULL, #stream_name},
#define OP_S_READ_FAIL(stream_name, allow_zero_len) {OPK_S_READ_FAIL, NULL, (allow_zero_len), NULL, #stream_name},
#define OP_C_STREAM_RESET(stream_name, aec) {OPK_C_STREAM_RESET, NULL, 0, NULL, #stream_name, (aec)},
#define OP_C_STREAM_RESET_FAIL(stream_name, aec) {OPK_C_STREAM_RESET_FAIL, NULL, 0, NULL, #stream_name, (aec)},
#define OP_S_ACCEPT_STREAM_WAIT(stream_name) {OPK_S_ACCEPT_STREAM_WAIT, NULL, 0, NULL, #stream_name},
#define OP_NEW_THREAD(num_threads, script) {OPK_NEW_THREAD, (script), (num_threads), NULL, NULL, 0},
#define OP_BEGIN_REPEAT(n) {OPK_BEGIN_REPEAT, NULL, (n)},
#define OP_END_REPEAT() {OPK_END_REPEAT},
#define OP_S_UNBIND_STREAM_ID(stream_name) {OPK_S_UNBIND_STREAM_ID, NULL, 0, NULL, #stream_name},
#define OP_C_READ_FAIL_WAIT(stream_name) {OPK_C_READ_FAIL_WAIT, NULL, 0, NULL, #stream_name},
#define OP_C_CLOSE_SOCKET() {OPK_C_CLOSE_SOCKET},
#define OP_C_EXPECT_SSL_ERR(stream_name, err) {OPK_C_EXPECT_SSL_ERR, NULL, (err), NULL, #stream_name},
...
#define OP_S_SET_INJECT_PLAIN(f) {OPK_S_SET_INJECT_PLAIN, NULL, 0, NULL, NULL, 0, (f)},
#define OP_SET_INJECT_WORD(w0, w1) {OPK_SET_INJECT_WORD, NULL, (w0), NULL, NULL, (w1), NULL},
#define OP_C_INHIBIT_TICK(inhibit) {OPK_C_INHIBIT_TICK, NULL, (inhibit), NULL, NULL, 0, NULL},
#define OP_C_SET_WRITE_BUF_SIZE(stream_name, size) {OPK_C_SET_WRITE_BUF_SIZE, NULL, (size), NULL, #stream_name},
#define OP_S_SET_INJECT_HANDSHAKE(f) {OPK_S_SET_INJECT_HANDSHAKE, NULL, 0, NULL, NULL, 0, NULL, (f)},
#define OP_S_NEW_TICKET() {OPK_S_NEW_TICKET},
#define OP_C_SKIP_IF_UNBOUND(stream_name, n) {OPK_C_SKIP_IF_UNBOUND, NULL, (n), NULL, #stream_name},
#define OP_S_SET_INJECT_DATAGRAM(f) {OPK_S_SET_INJECT_DATAGRAM, NULL, 0, NULL, NULL, 0, NULL, NULL, (f)},
...
#define OP_CHECK2(func, arg1, arg2) {OPK_CHECK, NULL, (arg1), (func), NULL, (arg2)},
...
ossl_unused static int script_88_poll(struct helper *h, struct helper_local *hl)
{
    ...
    for (i = 0; i < OSSL_NELEM(items); ++i)
    {
        ...
        if (!TEST_uint64_t_eq(items[i].revents, expected_revents[i]))
        {
            TEST_info("wanted: " POLL_FMT " got: " POLL_FMT, POLL_PRINTA(expected_revents[i]),
...
ossl_unused static int script_88_poll_conly(struct helper *h, struct helper_local *hl)
{
    ...
    t_limit = ossl_time_add(ossl_time_now(), ossl_ticks2time(5 * OSSL_TIME_SECOND));
    while (done == 0 && ok == 1)
    {
        ok = SSL_poll(items, OSSL_NELEM(items), sizeof(SSL_POLL_ITEM), &timeout, 0, &result_count);
...
static const struct script_op script_88[] = {
    ...
    /* Send something that will make client sockets readable. */
    OP_S_READ_EXPECT(a, "flamingo", 8) OP_S_WRITE(a, "flamingo", 8) OP_S_CONCLUDE(a)
...
static const struct script_op *const scripts[] = {
    ...
    script_81, script_82, script_83, script_84, script_85, script_86, script_87, script_88,
};
