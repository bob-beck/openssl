...
int test_start_file(STANZA* s, const char* testfile) {
  ...
  memset(s, 0, sizeof(*s));
  if (!TEST_ptr(s->fp = BIO_new_file(testfile, "r"))) return 0;
...
static int read_key(STANZA* s) {
  ...
  if (s->key == NULL) {
    if (!TEST_ptr(s->key = BIO_new(BIO_s_mem()))) return 0;
  ...
  while (BIO_gets(s->fp, tmpbuf, sizeof(tmpbuf))) {
    s->curr++;
    if (!TEST_int_gt(BIO_puts(s->key, tmpbuf), 0)) return 0;
    if (HAS_PREFIX(tmpbuf, "-----END")) return 1;
  }
...
int test_readstanza(STANZA* s) {
  ...
  for (s->numpairs = 0; BIO_gets(s->fp, buff, sizeof(buff));) {
    ...
    *equals++ = '\0';
    ...
    if ((value = strip_spaces(equals)) == NULL) value = "";
