...
X509* load_cert_pem(const char* file, OSSL_LIB_CTX* libctx) {
  ...
  if (!TEST_ptr(file) || !TEST_ptr(bio = BIO_new(BIO_s_file()))) return NULL;
...
STACK_OF(X509)* load_certs_pem(const char* file) {
  ...
  if (!TEST_ptr(file) || (bio = BIO_new_file(file, "r")) == NULL) return NULL;
...
X509_REQ* load_csr_der(const char* file, OSSL_LIB_CTX* libctx) {
  ...
  if (!TEST_ptr(file) || !TEST_ptr(bio = BIO_new_file(file, "rb"))) return NULL;

  csr = X509_REQ_new_ex(libctx, NULL);
  if (TEST_ptr(csr)) (void)TEST_ptr(d2i_X509_REQ_bio(bio, &csr));
  BIO_free(bio);
  return csr;
}
