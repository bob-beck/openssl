 ...
 */
void
test_fail_message_prefix(const char* prefix,
                         const char* file,
                         int line,
                         const char* type,
                         const char* left,
 ...
 */
static void
test_fail_message(const char* prefix,
                  const char* file,
                  int line,
                  const char* type,
                  const char* left,
...
static void
test_fail_message_va(const char* prefix,
                     const char* file,
                     int line,
                     const char* type,
                     const char* left,
...
#define DEFINE_COMPARISON(type, name, opname, op, fmt, cast)                   \
  int test_##name##_##opname(const char* file,                                 \
                             int line,                                         \
                             const char* s1,                                   \
                             const char* s2,                                   \
                             const type t1,                                    \
             ...
             size_t n2)
{
  ...
  if (n1 != n2 || s1 == NULL || s2 == NULL || strncmp(s1, s2, n1) != 0) {
    test_fail_string_message(NULL,
                             ...
                             "==",
                             s1,
                             s1 == NULL ? 0 : OPENSSL_strnlen(s1, n1),
                             s2,
            ...
            size_t n2)
{
  ...
  if (n1 != n2 || s1 == NULL || s2 == NULL || memcmp(s1, s2, n1) != 0) {
    test_fail_memory_message(
      NULL, file, line, "memory", st1, st2, "==", s1, n1, s2, n2);
    return 0;
  }
  return 1;
}
...
#define DEFINE_BN_COMPARISONS(opname, op, zero_cond)                           \
  int test_BN_##opname(const char* file,                                       \
                       int line,                                               \
                       const char* s1,                                         \
                       const char* s2,                                         \
                       const BIGNUM* t1,                                       \
  ...
  {                                                                            \
    if (BN_cmp(t1, t2) op 0)                                                   \
      return 1;                                                                \
    test_fail_bignum_message(NULL, file, line, "BIGNUM", s1, s2, #op, t1, t2); \
    return 0;                                                                  \
  }                                                                            \
  int test_BN_##opname##_zero(                                                 \
    const char* file, int line, const char* s, const BIGNUM* a)                \
  {                                                                            \
    if (a != NULL && (zero_cond))                                              \
      return 1;                                                                \
    test_fail_bignum_mono_message(NULL, file, line, "BIGNUM", s, "0", #op, a); \
    return 0;                                                                  \
  }
                    ...
                    BN_ULONG w)
{
  ...
  test_fail_bignum_message(
    NULL, file, line, "BIGNUM", bns, ws, "abs==", aa, bw);
  BN_free(bw);
  BN_free(aa);
  return 0;
}
...
#define DEFINE_TIME_T_COMPARISON(opname, op)                                   \
  int test_time_t_##opname(const char* file,                                   \
                           int line,                                           \
                           const char* s1,                                     \
                           const char* s2,                                     \
                           const time_t t1,                                    \
