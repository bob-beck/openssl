 ...
 */
void test_fail_message_prefix(const char* prefix,
                              const char* file,
                              int line,
                              const char* type,
                              const char* left,
 ...
 */
static void test_fail_message(const char* prefix,
                              const char* file,
                              int line,
                              const char* type,
                              const char* left,
...
static void test_fail_message_va(const char* prefix,
                                 const char* file,
                                 int line,
                                 const char* type,
                                 const char* left,
...
#define DEFINE_COMPARISON(type, name, opname, op, fmt, cast)                  \
  int test_##name##_##opname(const char* file, int line, const char* s1,      \
    ...
    test_fail_message(NULL, file, line, #type, s1, s2, #op,                   \
                      "[" fmt "] compared to [" fmt "]", (cast)t1, (cast)t2); \
    return 0;                                                                 \
  }
...
int test_str_eq(const char* file,
                int line,
                const char* st1,
                const char* st2,
                const char* s1,
...
int test_str_ne(const char* file,
                int line,
                const char* st1,
                const char* st2,
                const char* s1,
...
int test_strn_eq(const char* file,
                 int line,
                 const char* st1,
                 const char* st2,
                 const char* s1,
...
int test_strn_ne(const char* file,
                 int line,
                 const char* st1,
                 const char* st2,
                 const char* s1,
...
int test_mem_eq(const char* file,
                int line,
                const char* st1,
                const char* st2,
                const void* s1,
  ...
  if (n1 != n2 || s1 == NULL || s2 == NULL || memcmp(s1, s2, n1) != 0) {
    test_fail_memory_message(NULL, file, line, "memory", st1, st2, "==", s1, n1,
                             s2, n2);
    return 0;
  }
  return 1;
...
int test_mem_ne(const char* file,
                int line,
                const char* st1,
                const char* st2,
                const void* s1,
...
int test_BN_eq_word(const char* file,
                    int line,
                    const char* bns,
                    const char* ws,
                    const BIGNUM* a,
...
int test_BN_abs_eq_word(const char* file,
                        int line,
                        const char* bns,
                        const char* ws,
                        const BIGNUM* a,
