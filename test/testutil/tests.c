...
#define DEFINE_COMPARISON(type, name, opname, op, fmt, cast)                                                           \
    int test_##name##_##opname(const char *file, int line, const char *s1, const char *s2, const type t1,              \
                               const type t2)                                                                          \
    {                                                                                                                  \
        if (t1 op t2)                                                                                                  \
            return 1;                                                                                                  \
        test_fail_message(NULL, file, line, #type, s1, s2, #op, "[" fmt "] compared to [" fmt "]", (cast)t1,           \
                          (cast)t2);                                                                                   \
        return 0;                                                                                                      \
    }
...
#define DEFINE_BN_COMPARISONS(opname, op, zero_cond)                                                                   \
    int test_BN_##opname(const char *file, int line, const char *s1, const char *s2, const BIGNUM *t1,                 \
                         const BIGNUM *t2)                                                                             \
    {                                                                                                                  \
    ...
    int test_BN_##opname##_zero(const char *file, int line, const char *s, const BIGNUM *a)                            \
    {                                                                                                                  \
        if (a != NULL && (zero_cond))                                                                                  \
            return 1;                                                                                                  \
...
#define DEFINE_TIME_T_COMPARISON(opname, op)                                                                           \
    int test_time_t_##opname(const char *file, int line, const char *s1, const char *s2, const time_t t1,              \
                             const time_t t2)                                                                          \
    {                                                                                                                  \
        ...
        if (!r)                                                                                                        \
            test_fail_message(NULL, file, line, "time_t", s1, s2, #op, "[%s] compared to [%s]", print_time(at1),       \
                              print_time(at2));                                                                        \
