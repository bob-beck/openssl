...
int setup_test_framework(int argc, char* argv[]) {
  ...
  if (TAP_levels != NULL) level = 4 * atoi(TAP_levels);
  ...
  if (!opt_init(argc, argv, test_get_options())) return 0;
  return 1;
}
...
static int check_single_test_params(char* name, char* testname, char* itname) {
  if (name != NULL) {
    ...
    if (i >= num_tests) single_test = atoi(name);
  }

  /* if only iteration is specified, assume we want the first test */
  if (single_test == -1 && single_iter != -1) single_test = 1;
...
static int process_shared_options(void) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_TEST_ITERATION:
        flag_iter = opt_flag();
        if (!opt_int(opt_arg(), &single_iter)) goto end;
        break;
      case OPT_TEST_INDENT:
        if (!opt_int(opt_arg(), &value)) goto end;
        level = 4 * value;
        test_adjust_streams_tap_level(level);
        break;
      case OPT_TEST_SEED:
  ...
  if (!check_single_test_params(testname, flag_test, flag_iter)) goto end;
  ret = 1;
end:
  return ret;
}
...
static void test_verdict(int verdict, const char* description, ...) {
  ...
  va_end(ap);
  if (verdict == TEST_SKIP_CODE) test_printf_tapout(" # skipped");
  test_printf_tapout("\n");
  test_flush_tapout();
}
...
int run_tests(const char* test_prog_name) {
  ...
  for (ii = 0; ii != num_tests; ++ii) {
    ...
    if (single_test != -1 && ((i + 1) != single_test)) {
      ...
      for (jj = 0; jj < all_tests[i].num; jj++) {
        ...
        if (single_iter != -1 && ((jj + 1) != single_iter)) continue;
        ERR_clear_error();
  ...
  if (num_failed != 0) return EXIT_FAILURE;
  return EXIT_SUCCESS;
}
...
char* glue_strings(const char* list[], size_t* out_len) {
  ...
  if (!TEST_ptr(ret = p = OPENSSL_malloc(len + 1))) return NULL;
