...
static void test_fail_string_common(const char* prefix, const char* file,
  ...
  if (l1 != l2 || strncmp(m1, m2, l1) != 0) test_diff_header(left, right);
  ...
  while (l1 > 0 || l2 > 0) {
    ...
    if (n1 == n2 && !diff) {
      ...
      if (cnt == 0 && (m2 == NULL || *m2 == '\0'))
        ...
        test_printf_stderr("%4u:+ '%s'\n", cnt, b2);
      if (diff && i > 0) test_printf_stderr("%4s    %s\n", "", bdiff);
    }
    if (m1 != NULL) m1 += n1;
    if (m2 != NULL) m2 += n2;
    l1 -= n1;
...
static const char* test_bignum_zero_null(const BIGNUM* bn) {
  if (bn != NULL) return BN_is_negative(bn) ? "-0" : "0";
  return "NULL";
}
...
fin:
  test_flush_stderr();
  if (bufp != buffer) OPENSSL_free(bufp);
}
