...
int test_arg_libctx(OSSL_LIB_CTX** libctx, OSSL_PROVIDER** default_null_prov,
  ...
  if (strcmp(module_name, "none") == 0) return 1;
...
int fips_provider_version_eq(OSSL_LIB_CTX* libctx, int major, int minor,
  ...
  if ((res = fips_provider_version(libctx, &prov)) <= 0) return res == 0;
