...
static void
fake_rand_freectx(void* vrng)
{
  OPENSSL_free(vrng);
}

static int
fake_rand_instantiate(void* vrng,
...
static int
fake_rand_uninstantiate(void* vrng)
{
  FAKE_RAND* frng = (FAKE_RAND*)vrng;
...
static int
fake_rand_enable_locking(void* vrng)
{
  return 1;
}

static int
fake_rand_get_ctx_params(ossl_unused void* vrng, OSSL_PARAM params[])
{
  FAKE_RAND* frng = (FAKE_RAND*)vrng;
  OSSL_PARAM* p;
...
fake_rand_query(void* provctx, int operation_id, int* no_cache)
{
  *no_cache = 0;
  switch (operation_id) {
    case OSSL_OP_RAND:
...
static int
fake_rand_provider_init(const OSSL_CORE_HANDLE* handle,
...
static int
check_rng(EVP_RAND_CTX* rng, const char* name)
{
  FAKE_RAND* f;
...
fake_rand_start(OSSL_LIB_CTX* libctx)
{
  ...
  if (!TEST_true(OSSL_PROVIDER_add_builtin(
        libctx, "fake-rand", fake_rand_provider_init)) ||
...
void
fake_rand_set_callback(EVP_RAND_CTX* rng,
