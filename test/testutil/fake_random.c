...
static void* fake_rand_newctx(void* provctx, void* parent,
  ...
  if (r != NULL) r->state = EVP_RAND_STATE_UNINITIALISED;
  return r;
}
...
static int fake_rand_generate(void* vrng, unsigned char* out, size_t outlen,
  ...
  if (frng->cb != NULL) return (*frng->cb)(out, outlen, frng->name, frng->ctx);
  while (outlen > 0) {
    r = test_random();
...
static int fake_rand_get_ctx_params(ossl_unused void* vrng,
  ...
  if (p != NULL && !OSSL_PARAM_set_int(p, frng->state)) return 0;

  p = OSSL_PARAM_locate(params, OSSL_RAND_PARAM_STRENGTH);
  if (p != NULL && !OSSL_PARAM_set_int(p, 256)) return 0;

  p = OSSL_PARAM_locate(params, OSSL_RAND_PARAM_MAX_REQUEST);
  if (p != NULL && !OSSL_PARAM_set_size_t(p, INT_MAX)) return 0;
  return 1;
}

static const OSSL_PARAM* fake_rand_gettable_ctx_params(
    ossl_unused void* vrng, ossl_unused void* provctx) {
...
static int fake_rand_provider_init(const OSSL_CORE_HANDLE* handle,
  ...
  if (!TEST_ptr(*provctx = OSSL_LIB_CTX_new())) return 0;
...
void fake_rand_set_callback(EVP_RAND_CTX* rng,
  ...
  if (rng != NULL) ((FAKE_RAND*)rng->algctx)->cb = cb;
}
