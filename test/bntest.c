...
static const char* findattr(STANZA* s, const char* key) {
  ...
  for (; --i >= 0; pp++)
    if (OPENSSL_strcasecmp(pp->key, key) == 0) return pp->value;
  return NULL;
}
...
static int equalBN(const char* op, const BIGNUM* expected,
                   const BIGNUM* actual) {
  if (BN_cmp(expected, actual) == 0) return 1;
...
static int test_swap(void) {
  ...
  /* regular swap */
  BN_swap(a, b);
  if (!equalBN("swap", a, d) || !equalBN("swap", b, c)) goto err;

  /* regular swap: same pointer */
  BN_swap(a, a);
  if (!equalBN("swap with same pointer", a, d)) goto err;

  /* conditional swap: true */
  cond = 1;
  BN_consttime_swap(cond, a, b, top);
  if (!equalBN("cswap true", a, c) || !equalBN("cswap true", b, d)) goto err;

  /* conditional swap: true, same pointer */
  BN_consttime_swap(cond, a, a, top);
  if (!equalBN("cswap true", a, c)) goto err;

  /* conditional swap: false */
  cond = 0;
  BN_consttime_swap(cond, a, b, top);
  if (!equalBN("cswap false", a, c) || !equalBN("cswap false", b, d)) goto err;

  /* conditional swap: false, same pointer */
  BN_consttime_swap(cond, a, a, top);
  if (!equalBN("cswap false", a, c)) goto err;
...
static int test_sub(void) {
  ...
  for (i = 0; i < NUM0 + NUM1; i++) {
    if (i < NUM1) {
      ...
      if (!TEST_true(BN_bntest_rand(b, 400 + i - NUM1, 0, 0))) goto err;
...
static int test_div_recip(void) {
  ...
  for (i = 0; i < NUM0 + NUM1; i++) {
    if (i < NUM1) {
      ...
      if (!(TEST_true(BN_bntest_rand(b, 50 + 3 * (i - NUM1), 0, 0)))) goto err;
    }
    BN_set_negative(a, rand_neg());
    BN_set_negative(b, rand_neg());
...
static int test_mod(void) {
  ...
  if (!(TEST_true(BN_bntest_rand(a, 1024, 0, 0)))) goto err;
  for (i = 0; i < NUM0; i++) {
    if (!(TEST_true(BN_bntest_rand(b, 450 + i * 10, 0, 0)))) goto err;
    BN_set_negative(a, rand_neg());
    BN_set_negative(b, rand_neg());
...
static int test_modexp_mont5(void) {
  ...
  if (!TEST_true(BN_mod_exp_mont_consttime(d, a, p, m, ctx, NULL))) goto err;
  if (!TEST_BN_eq_one(d)) goto err;
  ...
  if (!(TEST_true(BN_hex2bn(
        ...
        TEST_true(BN_hex2bn(
            ...
            "9CC638A5EE8CDD703719F8000E6A9F63BEED5F2FCD52FF293EA05A251BB4AB8"
            "1")) &&
        TEST_true(BN_hex2bn(
            &n,
            ...
            "2C9A15D84D4954425F0B7642FC0BD9D7B24E2618D2DCC9B729D944BADACFDDA"
            "F"))))
    goto err;
  ...
  if (!(TEST_true(BN_hex2bn(
            ...
            "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"
            "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"
            "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF2020202020D"
            "F")) &&
        TEST_true(BN_hex2bn(
            &b,
        ...
        TEST_true(BN_hex2bn(
            ...
            "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"
            "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"
            "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF2020202020F"
            "F")) &&
        TEST_true(BN_MONT_CTX_set(mont, n, ctx)) &&
  ...
  if (!(TEST_true(BN_hex2bn(
        ...
        TEST_true(BN_hex2bn(
            ...
            "E38E38E38E38E38E38E38E38E38E38E38E38E38E38E38E38E38E38E38E38E38"
            "E")) &&
        TEST_true(BN_hex2bn(
            &n,
            "FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF"
  ...
  /* Zero input */
  if (!TEST_true(BN_bntest_rand(p, 1024, 0, 0))) goto err;
  BN_zero(a);
...
static int test_gf2m_modinv(void) {
  ...
  if (!TEST_true(BN_one(b[0]))) goto err;
  if (!TEST_true(BN_bntest_rand(a, 512, 0, 0))) goto err;
  if (!TEST_false(BN_GF2m_mod_inv(c, a, b[0], ctx))) goto err;
...
static int test_kronecker(void) {
  ...
  if (!TEST_true(BN_generate_prime_ex(b, 512, 0, NULL, NULL, NULL))) goto err;
  BN_set_negative(b, rand_neg());
  ...
  for (i = 0; i < NUM0; i++) {
    ...
    if (!TEST_true(BN_copy(t, b))) goto err;
    BN_set_negative(t, 0);
    if (!TEST_true(BN_sub_word(t, 1))) goto err;
    if (!TEST_true(BN_rshift1(t, t))) goto err;
    /* r := a^t mod b */
    BN_set_negative(b, 0);

    if (!TEST_true(BN_mod_exp_recp(r, a, t, b, ctx))) goto err;
    BN_set_negative(b, 1);
    ...
    if (BN_is_word(r, 1))
      ...
      if (!TEST_true(BN_add_word(r, 1))) goto err;
    ...
    if (!TEST_int_ge(kronecker = BN_kronecker(a, b, ctx), -1)) goto err;
    /* we actually need BN_kronecker(a, |b|) */
    if (BN_is_negative(a) && BN_is_negative(b)) kronecker = -kronecker;

    if (!TEST_int_eq(legendre, kronecker)) goto err;
  }
...
static int file_square(STANZA* s) {
  ...
  if (!TEST_BN_eq_zero(square)) {
    if (!TEST_ptr(tmp = BN_new()) || !TEST_true(BN_copy(tmp, square))) goto err;
    BN_set_negative(tmp, 1);

    if (!TEST_int_eq(BN_sqrt(ret, tmp, ctx), 0)) goto err;
    ERR_clear_error();

    BN_set_negative(tmp, 0);
    if (BN_add(tmp, tmp, BN_value_one())) goto err;
    if (!TEST_int_eq(BN_sqrt(ret, tmp, ctx))) goto err;
    ERR_clear_error();
  }
#endif
...
static int file_quotient(STANZA* s) {
  ...
  if (!BN_is_negative(b) && b_word != (BN_ULONG)-1) {
    ...
    if (!TEST_ptr(BN_copy(ret, a))) goto err;
    ...
    if (!equalBN("A / B (word)", quotient, ret)) goto err;
...
static int file_modsqrt(STANZA* s) {
  ...
  if (BN_is_negative(mod_sqrt)) {
    /* A negative testcase */
    if (!TEST_ptr_null(BN_mod_sqrt(ret, a, p, ctx))) goto err;
...
static int test_bn2padded(void) {
  ...
  /* Test edge case at 0. */
  if (!TEST_ptr((n = BN_new()))) goto err;
  if (!TEST_int_eq(BN_bn2binpad(n, NULL, 0), 0)) goto err;
  memset(out, -1, sizeof(out));
  if (!TEST_int_eq(BN_bn2binpad(n, out, sizeof(out)), sizeof(out))) goto err;
  memset(zeros, 0, sizeof(zeros));
  if (!TEST_mem_eq(zeros, sizeof(zeros), out, sizeof(out))) goto err;
...
#define BOTTOM_BIT_NOTOUCH 0
    ...
    if (!TEST_int_eq(BN_bn2binpad(n, NULL, 0), -1)) goto err;
    /* One byte short should fail. */
    if (!TEST_int_eq(BN_bn2binpad(n, out, bytes - 1), -1)) goto err;
...
static int test_asc2bn(void) {
  ...
  if (!TEST_ptr(bn = BN_new())) goto err;
...
static int test_mpi(int i) {
  ...
  if (!TEST_size_t_le(mpi_len, sizeof(scratch))) goto err;

  if (!TEST_size_t_eq(mpi_len2 = BN_bn2mpi(bn, scratch), mpi_len) ||
      !TEST_mem_eq(test->mpi, test->mpi_len, scratch, mpi_len))
    goto err;

  if (!TEST_ptr(bn2 = BN_mpi2bn(scratch, (int)mpi_len, NULL))) goto err;
...
static int test_bin2zero(void) {
  ...
  if (!TEST_ptr(zbn = BN_new())) goto err;
...
static int test_rand(void) {
  ...
  if (!TEST_ptr(bn = BN_new())) return 0;
...
static int test_rand_range(void) {
  ...
  if (TEST_int_ge(n_success, binomial_critical)) return 1;
...
static int test_negzero(void) {
  ...
  if (!TEST_true(BN_set_word(a, 1))) goto err;
  BN_set_negative(a, 1);
  BN_zero(b);
  if (!TEST_true(BN_mul(c, a, b, ctx))) goto err;
  if (!TEST_BN_eq_zero(c) || !TEST_BN_ge_zero(c)) goto err;
...
static int test_badmod(void) {
  ...
  if (!TEST_false(BN_div(a, b, BN_value_one(), zero, ctx))) goto err;
  ERR_clear_error();
  ...
  if (!TEST_false(BN_MONT_CTX_set(mont, zero, ctx))) goto err;
  ERR_clear_error();

  /* Some operations also may not be used with an even modulus. */
  if (!TEST_true(BN_set_word(b, 16))) goto err;

  if (!TEST_false(BN_MONT_CTX_set(mont, b, ctx))) goto err;
  ERR_clear_error();
...
static int test_ctx_set_ct_flag(BN_CTX* c) {
  ...
  for (i = 0; i < OSSL_NELEM(b); i++) {
    if (!TEST_ptr(b[i] = BN_CTX_get(c))) goto err;
    if (i % 2 == 1) BN_set_flags(b[i], BN_FLG_CONSTTIME);
  }
...
static int test_ctx_check_ct_flag(BN_CTX* c) {
  ...
  for (i = 0; i < OSSL_NELEM(b); i++) {
    if (!TEST_ptr(b[i] = BN_CTX_get(c))) goto err;
    if (!TEST_false(BN_get_flags(b[i], BN_FLG_CONSTTIME))) goto err;
  }
...
static int test_gcd_prime(void) {
  ...
  if (!TEST_true(BN_generate_prime_ex(a, 1024, 0, NULL, NULL, NULL))) goto err;
...
static int test_mod_exp(int i) {
  ...
  if (!TEST_ptr(s = BN_bn2dec(result))) goto err;

  if (!TEST_mem_eq(s, strlen(s), test->res, strlen(test->res))) goto err;
...
static int test_mod_inverse(void) {
  ...
  if (!TEST_true(BN_dec2bn(&a, "5193817943"))) goto err;
  if (!TEST_true(BN_dec2bn(&b, "3259122431"))) goto err;
  if (!TEST_ptr(r = BN_new())) goto err;
  if (!TEST_ptr_eq(BN_mod_inverse(r, a, b, ctx), r)) goto err;
  if (!TEST_ptr_ne(str = BN_bn2dec(r), NULL)) goto err;
  if (!TEST_int_eq(strcmp(str, "2609653924"), 0)) goto err;

  /* Note that this aliases the result with the modulus. */
  if (!TEST_ptr_null(BN_mod_inverse(b, a, b, ctx))) goto err;
...
static int test_mod_exp_alias(int idx) {
  ...
  if (!TEST_true(BN_dec2bn(&a, "15"))) goto err;
  if (!TEST_true(BN_dec2bn(&b, "10"))) goto err;
  if (!TEST_true(BN_dec2bn(&c, "39"))) goto err;
  if (!TEST_ptr(r = BN_new())) goto err;
  ...
  if (!TEST_int_eq(
    ...
    goto err;
  if (!TEST_ptr_ne(str = BN_bn2dec(r), NULL)) goto err;
  if (!TEST_str_eq(str, "36")) goto err;
  ...
  if (idx == 0) {
    if (!TEST_int_eq(BN_mod_exp_simple(c, a, b, c, ctx), 0)) goto err;
  } else {
    if (!TEST_int_eq(BN_mod_exp_recp(c, a, b, c, ctx), 1)) goto err;
    if (!TEST_ptr_ne(str = BN_bn2dec(c), NULL)) goto err;
    if (!TEST_str_eq(str, "36")) goto err;
  }
...
static int run_file_tests(int i) {
  ...
  while (!BIO_eof(s->fp) && test_readstanza(s)) {
    if (s->numpairs == 0) continue;
    if (!file_test_run(s)) s->errors++;
    s->numtests++;
    test_clearstanza(s);
  }
  test_end_file(s);
...
int setup_tests(void) {
  ...
  if (!TEST_ptr(ctx = BN_CTX_new())) return 0;
  ...
  if (n == 0) {
    ...
    if (stochastic) ADD_TEST(test_rand_range);
  } else {
