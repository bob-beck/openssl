 ...
 */
static const char *
findattr (STANZA *s, const char *key)
{
  int i = s->numpairs;
  PAIR *pp = s->pairs;
 ...
 */
static int
parse_bigBN (BIGNUM **out, const char *bn_strings[])
{
 ...
 */
static int
parseBN (BIGNUM **out, const char *in)
{
  *out = NULL;
...
static int
parsedecBN (BIGNUM **out, const char *in)
{
  *out = NULL;
...
static BIGNUM *
getBN (STANZA *s, const char *attribute)
{
  const char *hex;
  BIGNUM *ret = NULL;
...
static int
getint (STANZA *s, int *out, const char *attribute)
{
  BIGNUM *ret;
  BN_ULONG word;
  int st = 0;
...
static int
equalBN (const char *op, const BIGNUM *expected, const BIGNUM *actual)
{
  ...
  for (i = 0; i < NUM0 + NUM1; i++)
    {
      ...
      if (!(TEST_true (BN_RECP_CTX_set (recp, b, ctx))
            ...
            && TEST_true (BN_add (d, e, c)) && TEST_true (BN_sub (d, d, a))
            && TEST_BN_eq_zero (d)))
        goto err;
    }
  st = 1;
...
static int
test_signed_mod_replace_ab (int n)
{
  BIGNUM *a = NULL, *b = NULL, *c = NULL, *d = NULL;
  int st = 0;
...
static int
test_signed_mod_replace_ba (int n)
{
  BIGNUM *a = NULL, *b = NULL, *c = NULL, *d = NULL;
  int st = 0;
  ...
  for (i = 0; i < NUM0; i++)
    {
      ...
      if (!(TEST_true (BN_mod (c, a, b, ctx))
            ...
            && TEST_true (BN_add (d, c, e)) && TEST_BN_eq (d, a)))
        goto err;
    }
  st = 1;
err:
...
file_sum (STANZA *s)
{
  ...
  if (!TEST_true (BN_copy (ret, a)) || !TEST_true (BN_add (ret, ret, b))
      ...
      || !TEST_true (BN_copy (ret, sum)) || !TEST_true (BN_sub (ret, ret, b))
...
static int
copy_reversed (uint8_t *dst, uint8_t *src, size_t len)
{
...
test_dec2bn (void)
{
  ...
  if (!TEST_int_eq (parsedecBN (&bn, "0"), 1) || !TEST_BN_eq_word (bn, 0)
      || !TEST_BN_eq_zero (bn) || !TEST_BN_le_zero (bn)
  ...
  if (!TEST_int_eq (parsedecBN (&bn, "256"), 3) || !TEST_BN_eq_word (bn, 256)
      || !TEST_BN_ge_zero (bn) || !TEST_BN_gt_zero (bn)
      || !TEST_BN_ne_zero (bn) || !TEST_BN_even (bn))
    goto err;
  BN_free (bn);
  bn = NULL;
  ...
  if (!TEST_int_eq (parsedecBN (&bn, "1"), 1) || !TEST_BN_eq_word (bn, 1)
      || !TEST_BN_ne_zero (bn) || !TEST_BN_gt_zero (bn)
...
test_hex2bn (void)
{
  ...
  if (!TEST_int_eq (parseBN (&bn, "-42"), 3) || !TEST_BN_abs_eq_word (bn, 0x42)
      || !TEST_BN_lt_zero (bn) || !TEST_BN_le_zero (bn)
      || !TEST_BN_ne_zero (bn) || !TEST_BN_even (bn))
    goto err;
  BN_free (bn);
  bn = NULL;
  ...
  if (!TEST_int_eq (parseBN (&bn, "cb"), 2) || !TEST_BN_eq_word (bn, 0xCB)
      || !TEST_BN_ge_zero (bn) || !TEST_BN_gt_zero (bn)
      || !TEST_BN_ne_zero (bn) || !TEST_BN_odd (bn))
    goto err;
  BN_free (bn);
  bn = NULL;
...
static int
test_rand_range_single (size_t n)
{
...
test_badmod (void)
{
  ...
  if (!TEST_false (
          BN_mod_mul (a, BN_value_one (), BN_value_one (), zero, ctx)))
    goto err;
  ERR_clear_error ();
  ...
  if (!TEST_false (BN_mod_exp_mont_consttime (
          a, BN_value_one (), BN_value_one (), zero, ctx, NULL)))
    goto err;
  ERR_clear_error ();
  ...
  if (!TEST_false (BN_mod_exp_mont_consttime (a, BN_value_one (),
                                              BN_value_one (), b, ctx, NULL)))
    goto err;
  ERR_clear_error ();
...
static int
test_ctx_check_ct_flag (BN_CTX *c)
{
  int st = 0;
  size_t i;
  BIGNUM *b[30];
  ...
  if (n == 0)
    {
      ...
      ADD_ALL_TESTS (test_signed_mod_replace_ab,
                     OSSL_NELEM (signed_mod_tests));
      ADD_ALL_TESTS (test_signed_mod_replace_ba,
                     OSSL_NELEM (signed_mod_tests));
      ADD_TEST (test_mod);
