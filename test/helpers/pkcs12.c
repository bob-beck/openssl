...
static void generate_p12(PKCS12_BUILDER* pb, const PKCS12_ENC* mac) {
  ...
  if (mac != NULL) {
    ...
    if (!TEST_true(PKCS12_set_mac(p12, mac->pass, (int)strlen(mac->pass), NULL,
                                  0, mac->iter, md))) {
      pb->success = 0;
      goto err;
    }
  ...
  if (write_files) write_p12(p12, pb->filename);
err:
  if (!legacy && md != NULL) EVP_MD_free(md);
  PKCS12_free(p12);
}
...
static int write_p12(PKCS12* p12, const char* outfile) {
  ...
  if (!TEST_int_eq(i2d_PKCS12_bio(out, p12), 1)) goto err;
  ret = 1;
err:
  BIO_free(out);
  return ret;
...
static PKCS12* from_bio_p12(BIO* bio, const PKCS12_ENC* mac) {
  ...
  if (!legacy) {
    ...
    if (!TEST_ptr(p12)) goto err;
  }
  p12 = d2i_PKCS12_bio(bio, &p12);
  BIO_free(bio);
  if (!TEST_ptr(p12)) goto err;
  if (mac == NULL) {
    if (!TEST_false(PKCS12_mac_present(p12))) goto err;
  } else {
    if (!check_p12_mac(p12, mac)) goto err;
  }
  return p12;
err:
  PKCS12_free(p12);
...
static STACK_OF(PKCS12_SAFEBAG)* decode_contentinfo(STACK_OF(PKCS7)* safes,
                                                    int idx,
  ...
  if (!TEST_ptr(p7)) goto err;

  bagnid = OBJ_obj2nid(p7->type);
  if (enc) {
    if (!TEST_int_eq(bagnid, NID_pkcs7_encrypted)) goto err;
    bags = PKCS12_unpack_p7encdata(p7, enc->pass, (int)strlen(enc->pass));
  } else {
    if (!TEST_int_eq(bagnid, NID_pkcs7_data)) goto err;
    bags = PKCS12_unpack_p7data(p7);
  }
  if (!TEST_ptr(bags)) goto err;
...
static int add_attributes(PKCS12_SAFEBAG* bag, const PKCS12_ATTR* attr) {
  ...
  while (p_attr->oid != NULL) {
    ...
    if (attr_nid == NID_friendlyName) {
      if (!TEST_true(PKCS12_add_friendlyname(bag, p_attr->value, -1))) goto err;
...
void add_keybag(PKCS12_BUILDER* pb, const unsigned char* bytes, int len,
  ...
  if (!add_attributes(bag, attrs)) pb->success = 0;
err:
  EVP_PKEY_free(pkey);
}
...
static int check_asn1_string(const ASN1_TYPE* av, const char* txt) {
  ...
  if (!TEST_ptr(av)) goto err;

  switch (av->type) {
    case V_ASN1_BMPSTRING:
      value = OPENSSL_uni2asc(av->value.bmpstring->data,
                              av->value.bmpstring->length);
      if (!TEST_str_eq(txt, (char*)value)) goto err;
      break;
  ...
  for (i = 0; i < sk_X509_ATTRIBUTE_num(bag_attrs); i++) {
    ...
    while (p_attr->oid != NULL) {
      ...
      if (strcmp(p_attr->oid, attr_txt) == 0) {
        if (!TEST_int_eq(X509_ATTRIBUTE_count(attr), 1)) goto err;

        for (j = 0; j < X509_ATTRIBUTE_count(attr); j++) {
          av = X509_ATTRIBUTE_get0_type(attr, j);
          if (!TEST_true(check_asn1_string(av, p_attr->value))) goto err;
        }
        break;
      }
      p_attr++;
...
void check_certbag(PKCS12_BUILDER* pb, const unsigned char* bytes, int len,
  ...
  if (!TEST_false(X509_cmp(x509, ref_x509))) pb->success = 0;
err:
  X509_free(x509);
  X509_free(ref_x509);
}
...
void check_keybag(PKCS12_BUILDER* pb, const unsigned char* bytes, int len,
  ...
  if (!TEST_true(EVP_PKEY_eq(pkey, ref_pkey))) pb->success = 0;
err:
  EVP_PKEY_free(pkey);
...
void start_check_pkcs12(PKCS12_BUILDER* pb) {
  ...
  if (!TEST_ptr(pb->safes)) pb->success = 0;
