...
check_p12_mac (PKCS12 *p12, const PKCS12_ENC *mac)
{
  return TEST_true (PKCS12_mac_present (p12))
         && TEST_true (
             PKCS12_verify_mac (p12, mac->pass, (int)strlen (mac->pass)));
}
  ...
  while (p_attr->oid != NULL)
    {
      ...
      if (attr_nid == NID_friendlyName)
        ...
        {
          if (!TEST_true (
                  PKCS12_add_localkeyid (bag, (unsigned char *)p_attr->value,
      ...
      else
        {
          ...
          if (!TEST_true (
                  PKCS12_add1_attr_by_txt (bag, p_attr->oid, MBSTRING_ASC,
...
void
add_certbag (PKCS12_BUILDER *pb, const unsigned char *bytes, int len,
             const PKCS12_ATTR *attrs)
{
...
void
add_keybag (PKCS12_BUILDER *pb, const unsigned char *bytes, int len,
...
void
add_secretbag (PKCS12_BUILDER *pb, int secret_nid, const char *secret,
               const PKCS12_ATTR *attrs)
{
  PKCS12_SAFEBAG *bag = NULL;
...
void
check_certbag (PKCS12_BUILDER *pb, const unsigned char *bytes, int len,
               const PKCS12_ATTR *attrs)
{
  X509 *x509 = NULL;
  X509 *ref_x509 = NULL;
...
void
check_keybag (PKCS12_BUILDER *pb, const unsigned char *bytes, int len,
              const PKCS12_ATTR *attrs, const PKCS12_ENC *enc)
{
  EVP_PKEY *pkey = NULL;
...
void
check_secretbag (PKCS12_BUILDER *pb, int secret_nid, const char *secret,
                 const PKCS12_ATTR *attrs)
{
  const PKCS12_SAFEBAG *bag;
