...
static int
add_attributes(PKCS12_SAFEBAG* bag, const PKCS12_ATTR* attrs);

static void
generate_p12(PKCS12_BUILDER* pb, const PKCS12_ENC* mac);
static int
write_p12(PKCS12* p12, const char* outfile);

static PKCS12*
from_bio_p12(BIO* bio, const PKCS12_ENC* mac);
static PKCS12*
read_p12(const char* infile, const PKCS12_ENC* mac);
static int
check_p12_mac(PKCS12* p12, const PKCS12_ENC* mac);
static int
check_asn1_string(const ASN1_TYPE* av, const char* txt);
static int
check_attrs(const STACK_OF(X509_ATTRIBUTE)* bag_attrs,
...
void
PKCS12_helper_set_write_files(int enable)
{
  write_files = enable;
}

void
PKCS12_helper_set_legacy(int enable)
{
  legacy = enable;
}

void
PKCS12_helper_set_libctx(OSSL_LIB_CTX* libctx)
{
  test_ctx = libctx;
}

void
PKCS12_helper_set_propq(const char* propq)
{
  test_propq = propq;
}
...
static X509*
load_cert_asn1(const unsigned char* bytes, int len)
{
  X509* cert = NULL;
...
static EVP_PKEY*
load_pkey_asn1(const unsigned char* bytes, int len)
{
  EVP_PKEY* pkey = NULL;
...
int
end_pkcs12_builder(PKCS12_BUILDER* pb)
{
...
void
start_pkcs12(PKCS12_BUILDER* pb)
{
  pb->safes = NULL;
}

void
end_pkcs12(PKCS12_BUILDER* pb)
{
  if (!pb->success)
    return;
...
void
end_pkcs12_with_mac(PKCS12_BUILDER* pb, const PKCS12_ENC* mac)
{
  if (!pb->success)
    return;
  generate_p12(pb, mac);
...
generate_p12(PKCS12_BUILDER* pb, const PKCS12_ENC* mac)
{
  ...
  if (mac != NULL) {
    ...
    if (!TEST_true(PKCS12_set_mac(
          p12, mac->pass, (int)strlen(mac->pass), NULL, 0, mac->iter, md))) {
      pb->success = 0;
      goto err;
    }
  }
...
void
start_contentinfo(PKCS12_BUILDER* pb)
{
  pb->bags = NULL;
}

void
end_contentinfo(PKCS12_BUILDER* pb)
{
...
void
end_contentinfo_encrypted(PKCS12_BUILDER* pb, const PKCS12_ENC* enc)
{
  if (pb->success && pb->bags != NULL) {
    if (legacy) {
      if (!TEST_true(PKCS12_add_safe(
            &pb->safes, pb->bags, enc->nid, enc->iter, enc->pass)))
        pb->success = 0;
    } else {
      if (!TEST_true(PKCS12_add_safe_ex(&pb->safes,
                                        pb->bags,
                                        enc->nid,
                                        enc->iter,
                                        enc->pass,
...
static int
add_attributes(PKCS12_SAFEBAG* bag, const PKCS12_ATTR* attr)
{
  int ret = 0;
  int attr_nid;
  ...
  while (p_attr->oid != NULL) {
    ...
    if (attr_nid == NID_friendlyName) {
      ...
      if (!TEST_true(PKCS12_add_localkeyid(
            bag, (unsigned char*)p_attr->value, (int)strlen(p_attr->value))))
        goto err;
    } else if (attr_nid == NID_oracle_jdk_trustedkeyusage) {
      attrs = (STACK_OF(X509_ATTRIBUTE)*)PKCS12_SAFEBAG_get0_attrs(bag);
      x509_attr = X509_ATTRIBUTE_create(
        attr_nid, V_ASN1_OBJECT, OBJ_txt2obj(p_attr->value, 0));
           ...
           const PKCS12_ENC* enc)
{
  ...
  if (legacy)
    bag = PKCS12_add_key(
      &pb->bags, pkey, 0 /*keytype*/, enc->iter, enc->nid, enc->pass);
  else
    bag = PKCS12_add_key_ex(&pb->bags,
                            pkey,
                            0 /*keytype*/,
                            enc->iter,
                            enc->nid,
...
void
add_secretbag(PKCS12_BUILDER* pb,
              int secret_nid,
              const char* secret,
              const PKCS12_ATTR* attrs)
{
  ...
  bag = PKCS12_add_secret(
    &pb->bags, secret_nid, (const unsigned char*)secret, (int)strlen(secret));
  if (!TEST_ptr(bag)) {
    pb->success = 0;
    return;
  }
...
static int
check_attrs(const STACK_OF(X509_ATTRIBUTE)* bag_attrs, const PKCS12_ATTR* attrs)
{
  int ret = 0;
  X509_ATTRIBUTE* attr;
  ASN1_TYPE* av;
...
void
check_certbag(PKCS12_BUILDER* pb,
              const unsigned char* bytes,
              int len,
              const PKCS12_ATTR* attrs)
{
...
void
check_keybag(PKCS12_BUILDER* pb,
             const unsigned char* bytes,
             int len,
             const PKCS12_ATTR* attrs,
             const PKCS12_ENC* enc)
  ...
  switch (PKCS12_SAFEBAG_get_nid(bag)) {
    ...
    case NID_pkcs8ShroudedKeyBag:
      if (legacy)
        ...
        p8 = PKCS12_decrypt_skey_ex(
          bag, enc->pass, (int)strlen(enc->pass), test_ctx, test_propq);
      if (!TEST_ptr(p8)) {
        pb->success = 0;
        goto err;
      }
...
void
check_secretbag(PKCS12_BUILDER* pb,
                int secret_nid,
                const char* secret,
                const PKCS12_ATTR* attrs)
{
...
void
start_check_pkcs12(PKCS12_BUILDER* pb)
{
  PKCS12* p12;
...
void
start_check_pkcs12_with_mac(PKCS12_BUILDER* pb, const PKCS12_ENC* mac)
{
  PKCS12* p12;
...
void
start_check_pkcs12_file(PKCS12_BUILDER* pb)
{
  PKCS12* p12;
...
void
start_check_pkcs12_file_with_mac(PKCS12_BUILDER* pb, const PKCS12_ENC* mac)
{
  PKCS12* p12;
...
void
end_check_pkcs12(PKCS12_BUILDER* pb)
{
  if (!pb->success)
    return;
...
void
start_check_contentinfo(PKCS12_BUILDER* pb)
{
  if (!pb->success)
    return;
...
void
start_check_contentinfo_encrypted(PKCS12_BUILDER* pb, const PKCS12_ENC* enc)
{
  if (!pb->success)
    return;
...
void
end_check_contentinfo(PKCS12_BUILDER* pb)
{
  if (!pb->success)
    return;
