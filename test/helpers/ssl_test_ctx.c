...
static int
parse_boolean(const char* value, int* result)
{
...
__owur static int
parse_enum(const test_enum* enums,
           size_t num_enums,
           int* value,
           const char* name)
{
...
static const char*
enum_name(const test_enum* enums, size_t num_enums, int value)
{
  size_t i;
  for (i = 0; i < num_enums; i++) {
...
__owur static int
parse_expected_result(SSL_TEST_CTX* test_ctx, const char* value)
{
  int ret_value;
  if (!parse_enum(
        ssl_test_results, OSSL_NELEM(ssl_test_results), &ret_value, value)) {
    return 0;
  }
...
const char*
ssl_test_result_name(ssl_test_result_t result)
{
...
__owur static int
parse_alert(int* alert, const char* value)
{
  return parse_enum(ssl_alerts, OSSL_NELEM(ssl_alerts), alert, value);
}

__owur static int
parse_client_alert(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_alert(&test_ctx->expected_client_alert, value);
}

__owur static int
parse_server_alert(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_alert(&test_ctx->expected_server_alert, value);
}

const char*
ssl_alert_name(int alert)
{
...
__owur static int
parse_protocol(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_enum(ssl_protocols,
                    OSSL_NELEM(ssl_protocols),
                    &test_ctx->expected_protocol,
                    value);
}

const char*
ssl_protocol_name(int protocol)
{
                             ...
                             const char* value)
{
  ...
  if (!parse_enum(ssl_verify_callbacks,
                  OSSL_NELEM(ssl_verify_callbacks),
                  &ret_value,
                  value)) {
    return 0;
  }
...
const char*
ssl_verify_callback_name(ssl_verify_callback_t callback)
{
  return enum_name(
    ssl_verify_callbacks, OSSL_NELEM(ssl_verify_callbacks), callback);
}
...
__owur static int
parse_servername(SSL_TEST_CLIENT_CONF* client_conf, const char* value)
{
  int ret_value;
  if (!parse_enum(
        ssl_servername, OSSL_NELEM(ssl_servername), &ret_value, value)) {
    return 0;
  }
...
__owur static int
parse_expected_servername(SSL_TEST_CTX* test_ctx, const char* value)
{
  int ret_value;
  if (!parse_enum(
        ssl_servername, OSSL_NELEM(ssl_servername), &ret_value, value)) {
...
const char*
ssl_servername_name(ssl_servername_t server)
{
...
__owur static int
parse_servername_callback(SSL_TEST_SERVER_CONF* server_conf, const char* value)
{
  int ret_value;
  if (!parse_enum(ssl_servername_callbacks,
                  OSSL_NELEM(ssl_servername_callbacks),
                  &ret_value,
                  value)) {
    return 0;
  }
...
const char*
ssl_servername_callback_name(ssl_servername_callback_t callback)
{
  return enum_name(
    ssl_servername_callbacks, OSSL_NELEM(ssl_servername_callbacks), callback);
}
...
__owur static int
parse_session_ticket(SSL_TEST_CTX* test_ctx, const char* value)
{
  int ret_value;
  if (!parse_enum(ssl_session_ticket,
...
const char*
ssl_session_ticket_name(ssl_session_ticket_t server)
{
...
__owur static int
parse_session_id(SSL_TEST_CTX* test_ctx, const char* value)
{
  int ret_value;
  if (!parse_enum(
        ssl_session_id, OSSL_NELEM(ssl_session_id), &ret_value, value)) {
    return 0;
  }
...
const char*
ssl_session_id_name(ssl_session_id_t server)
{
...
__owur static int
parse_test_method(SSL_TEST_CTX* test_ctx, const char* value)
{
  int ret_value;
  if (!parse_enum(
        ssl_test_methods, OSSL_NELEM(ssl_test_methods), &ret_value, value)) {
    return 0;
  }
...
const char*
ssl_test_method_name(ssl_test_method_t method)
{
...
IMPLEMENT_SSL_TEST_STRING_OPTION(SSL_TEST_CTX,
                                 test,
                                 expected_session_ticket_app_data)
IMPLEMENT_SSL_TEST_STRING_OPTION(SSL_TEST_SERVER_CONF,
                                 server,
...
__owur static int
parse_handshake_mode(SSL_TEST_CTX* test_ctx, const char* value)
{
  int ret_value;
  if (!parse_enum(ssl_handshake_modes,
                  OSSL_NELEM(ssl_handshake_modes),
                  &ret_value,
                  value)) {
    return 0;
  }
...
const char*
ssl_handshake_mode_name(ssl_handshake_mode_t mode)
{
...
__owur static int
parse_key_update_type(SSL_TEST_CTX* test_ctx, const char* value)
{
  int ret_value;
  if (!parse_enum(ssl_key_update_types,
                  OSSL_NELEM(ssl_key_update_types),
                  &ret_value,
                  value)) {
    return 0;
  }
...
__owur static int
parse_ct_validation(SSL_TEST_CLIENT_CONF* client_conf, const char* value)
{
  int ret_value;
  if (!parse_enum(ssl_ct_validation_modes,
                  OSSL_NELEM(ssl_ct_validation_modes),
                  &ret_value,
                  value)) {
    return 0;
  }
...
const char*
ssl_ct_validation_name(ssl_ct_validation_t mode)
{
  return enum_name(
    ssl_ct_validation_modes, OSSL_NELEM(ssl_ct_validation_modes), mode);
}

IMPLEMENT_SSL_TEST_BOOL_OPTION(SSL_TEST_CTX, test, resumption_expected)
IMPLEMENT_SSL_TEST_BOOL_OPTION(SSL_TEST_SERVER_CONF,
                               server,
...
__owur static int
parse_certstatus(SSL_TEST_SERVER_CONF* server_conf, const char* value)
{
  int ret_value;
  if (!parse_enum(
        ssl_certstatus, OSSL_NELEM(ssl_certstatus), &ret_value, value)) {
    return 0;
  }
...
const char*
ssl_certstatus_name(ssl_cert_status_t cert_status)
{
...
__owur static int
parse_max_fragment_len_mode(SSL_TEST_CLIENT_CONF* client_conf,
                            const char* value)
{
  int ret_value;

  if (!parse_enum(ssl_max_fragment_len_mode,
                  OSSL_NELEM(ssl_max_fragment_len_mode),
                  &ret_value,
                  value)) {
    return 0;
  }
...
const char*
ssl_max_fragment_len_name(int MFL_mode)
{
  return enum_name(
    ssl_max_fragment_len_mode, OSSL_NELEM(ssl_max_fragment_len_mode), MFL_mode);
}

/* Expected key and signature types */

__owur static int
parse_expected_key_type(int* ptype, const char* value)
{
  int nid;
...
__owur static int
parse_expected_tmp_key_type(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_expected_key_type(&test_ctx->expected_tmp_key_type, value);
}

__owur static int
parse_expected_server_cert_type(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_expected_key_type(&test_ctx->expected_server_cert_type, value);
}

__owur static int
parse_expected_server_sign_type(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_expected_key_type(&test_ctx->expected_server_sign_type, value);
}

__owur static int
parse_expected_client_cert_type(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_expected_key_type(&test_ctx->expected_client_cert_type, value);
}

__owur static int
parse_expected_client_sign_type(SSL_TEST_CTX* test_ctx, const char* value)
{
...
__owur static int
parse_expected_sign_hash(int* ptype, const char* value)
{
  int nid;
...
__owur static int
parse_expected_server_sign_hash(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_expected_sign_hash(&test_ctx->expected_server_sign_hash, value);
}

__owur static int
parse_expected_client_sign_hash(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_expected_sign_hash(&test_ctx->expected_client_sign_hash, value);
}

__owur static int
parse_expected_ca_names(STACK_OF(X509_NAME)** pnames,
                        const char* value,
                        OSSL_LIB_CTX* libctx)
{
  if (value == NULL)
...
__owur static int
parse_expected_server_ca_names(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_expected_ca_names(
    &test_ctx->expected_server_ca_names, value, test_ctx->libctx);
}
__owur static int
parse_expected_client_ca_names(SSL_TEST_CTX* test_ctx, const char* value)
{
  return parse_expected_ca_names(
    &test_ctx->expected_client_ca_names, value, test_ctx->libctx);
}
...
static void
ssl_test_extra_conf_free_data(SSL_TEST_EXTRA_CONF* conf)
{
...
static void
ssl_test_ctx_free_extra_data(SSL_TEST_CTX* ctx)
{
...
void
SSL_TEST_CTX_free(SSL_TEST_CTX* ctx)
{
  if (ctx == NULL)
    return;
...
static int
parse_client_options(SSL_TEST_CLIENT_CONF* client,
                     const CONF* conf,
...
static int
parse_server_options(SSL_TEST_SERVER_CONF* server,
                     const CONF* conf,
...
SSL_TEST_CTX*
SSL_TEST_CTX_create(const CONF* conf,
                    ...
                    OSSL_LIB_CTX* libctx)
{
  ...
  for (i = 0; i < sk_CONF_VALUE_num(sk_conf); i++) {
    ...
    if (strcmp(option->name, "client") == 0) {
      ...
      if (!parse_server_options(
            &ctx->resume_extra.server2, conf, option->value))
        goto err;
    } else {
      for (j = 0; j < OSSL_NELEM(ssl_test_ctx_options); j++) {
        if (strcmp(option->name, ssl_test_ctx_options[j].name) == 0) {
          if (!ssl_test_ctx_options[j].parse(ctx, option->value)) {
            TEST_info(
              "Bad value %s for option %s", option->value, option->name);
            goto err;
          }
          found = 1;
          break;
