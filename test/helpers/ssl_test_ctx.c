...
__owur static int parse_client_verify_callback(
    SSL_TEST_CLIENT_CONF* client_conf, const char* value) {
  int ret_value;
...
__owur static int parse_expected_key_type(int* ptype, const char* value) {
  ...
  if (nid == NID_undef) nid = EC_curve_nist2nid(value);
#endif
  switch (nid) {
...
static int parse_client_options(SSL_TEST_CLIENT_CONF* client, const CONF* conf,
  ...
  if (!TEST_ptr(sk_conf = NCONF_get_section(conf, client_section))) return 0;
...
static int parse_server_options(SSL_TEST_SERVER_CONF* server, const CONF* conf,
  ...
  if (!TEST_ptr(sk_conf = NCONF_get_section(conf, server_section))) return 0;
