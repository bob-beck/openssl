...
static long
pkt_split_dgram_ctrl (BIO *bio, int cmd, long num, void *ptr)
{
  long ret;
...
static int
pkt_split_dgram_sendmmsg (BIO *bio, BIO_MSG *msg, size_t stride,
...
static int
pkt_split_dgram_recvmmsg (BIO *bio, BIO_MSG *msg, size_t stride,
                          size_t num_msg, uint64_t flags,
                          size_t *msgs_processed)
{
  BIO *next = BIO_next (bio);
