...
static int pkt_split_dgram_recvmmsg(BIO* bio, BIO_MSG* msg, size_t stride,
  ...
  if (!TEST_ptr(next) || !TEST_ptr(bdata)) return 0;
  ...
  for (i = 0, thismsg = msg; i < msg_cnt; i++, thismsg++) {
    ...
    if (!PACKET_buf_init(&pkt, thismsg->data, thismsg->data_len)) return 0;
    ...
    if (remain > 0) {
      ...
      memmove(msg[i + 1].data,
              (unsigned char*)msg[i + 1].data + thismsg->data_len, remain);
      msg_cnt++;
    }
  }
