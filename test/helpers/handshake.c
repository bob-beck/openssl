...
static int parse_protos(const char* protos,
                        unsigned char** out,
                        size_t* outlen) {
  size_t len, i, prefix;
...
static int server_npn_cb(SSL* s,
...
static void configure_handshake_ssl(SSL* server,
                                    SSL* client,
                                    const SSL_TEST_EXTRA_CONF* extra) {
...
static void do_connect_step(const SSL_TEST_CTX* test_ctx,
                            PEER* peer,
                            connect_phase_t phase) {
  switch (phase) {
    case HANDSHAKE:
