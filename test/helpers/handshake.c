...
static int client_hello_select_server_ctx(SSL* s, void* arg, int ignore) {
  ...
  len = (*(p++) << 8);
  len += *(p++);
  if (len + 2 != remaining) return 0;
  remaining = len;
  /*
   * The list in practice only has a single element, so we only consider
   * the first one.
   */
  if (remaining == 0 || *p++ != TLSEXT_NAMETYPE_host_name) return 0;
  remaining--;
...
static int parse_protos(const char* protos, unsigned char** out,
  ...
  *outlen = len + 1;
  ...
  while (i <= len) {
    if ((*out)[i] == ',') {
      if (!TEST_size_t_gt(i - 1, prefix)) goto err;
      (*out)[prefix] = (unsigned char)(i - 1 - prefix);
      prefix = i;
    }
    i++;
  }
  if (!TEST_size_t_gt(len, prefix)) goto err;
...
static int generate_session_ticket_cb(SSL* s, void* arg) {
  ...
  if (ss == NULL || app_data == NULL) return 0;
...
static int configure_handshake_ctx(SSL_CTX* server_ctx, SSL_CTX* server2_ctx,
  ...
  if (extra->server.session_ticket_app_data != NULL) {
    ...
    if (!TEST_ptr(server_ctx_data->session_ticket_app_data)) goto err;
  ...
  if (extra->server2.session_ticket_app_data != NULL) {
    if (!TEST_ptr(server2_ctx)) goto err;
    server2_ctx_data->session_ticket_app_data =
        OPENSSL_strdup(extra->server2.session_ticket_app_data);
    if (!TEST_ptr(server2_ctx_data->session_ticket_app_data)) goto err;
  ...
  if (!TEST_ptr(ticket_keys = OPENSSL_zalloc(ticket_key_len)) ||
      !TEST_long_eq(SSL_CTX_set_tlsext_ticket_keys(server_ctx, ticket_keys,
                                                   ticket_key_len),
                    1)) {
  ...
  if (!TEST_true(SSL_CTX_set_default_ctlog_list_file(client_ctx))) goto err;
  ...
  if (test->compress_certificates) {
    if (!TEST_true(SSL_CTX_compress_certs(server_ctx, 0))) goto err;
    if (server2_ctx != NULL &&
...
static void configure_handshake_ssl(SSL* server, SSL* client,
  ...
  if (extra->client.enable_pha) SSL_set_post_handshake_auth(client, 1);
}
...
static void do_reneg_setup_step(const SSL_TEST_CTX* test_ctx, PEER* peer) {
  ...
  if ((test_ctx->handshake_mode == SSL_TEST_HANDSHAKE_RENEG_SERVER &&
     ...
     * PEER_SUCCESS
     */
    if (peer->status != PEER_SUCCESS) peer->status = PEER_ERROR;
    return;
  ...
  if (ret >= 0) {
    ...
    if (!SSL_in_init(peer->ssl)) return;
  }
...
static connect_phase_t next_phase(const SSL_TEST_CTX* test_ctx,
  ...
  switch (phase) {
    ...
    case RENEG_APPLICATION_DATA:
      return RENEG_SETUP;
    case RENEG_SETUP:
      if (post_handshake_op(test_ctx)) return APPLICATION_DATA;
...
static int peer_pkey_type(SSL* s) {
  ...
  if (x != NULL) return pkey_type(X509_get0_pubkey(x));
  return NID_undef;
}
...
static int create_sctp_socks(int* ssock, int* csock) {
  ...
  if (lsock == INVALID_SOCKET) goto err;
  ...
  if (consock == INVALID_SOCKET) goto err;
  ...
  if (asock == INVALID_SOCKET) goto err;
...
err:
  BIO_ADDRINFO_free(res);
  if (consock != INVALID_SOCKET) BIO_closesocket(consock);
  if (lsock != INVALID_SOCKET) BIO_closesocket(lsock);
  if (asock != INVALID_SOCKET) BIO_closesocket(asock);
  return ret;
}
#endif
...
static HANDSHAKE_RESULT* do_handshake_internal(
  ...
  if (!TEST_ptr(client_to_server) || !TEST_ptr(server_to_client)) goto err;
  ...
  for (;;) {
    if (client_turn) {
      ...
      status = handshake_status(client.status, server.status,
                                1 /* client went last */);
      if (server.status == PEER_WAITING) server.status = PEER_RETRY;
    } else {
...
err:
  ...
  if (session_out != NULL) *session_out = SSL_get1_session(client.ssl);
  if (serv_sess_out != NULL) {
     ...
     * list.
     */
    if (tmp != NULL) *serv_sess_out = SSL_SESSION_dup(tmp);
  }
