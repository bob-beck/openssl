...
void
HANDSHAKE_RESULT_free (HANDSHAKE_RESULT *result)
{
  if (result == NULL)
    return;
...
static void
ctx_data_free_data (CTX_DATA *ctx_data)
{
...
static void
info_cb (const SSL *s, int where, int ret)
{
 ...
 */
static int
select_server_ctx (SSL *s, void *arg, int ignore)
{
...
static int
client_hello_select_server_ctx (SSL *s, void *arg, int ignore)
{
 ...
 */
static int
servername_ignore_cb (SSL *s, int *ad, void *arg)
{
  return select_server_ctx (s, arg, 1);
}

static int
servername_reject_cb (SSL *s, int *ad, void *arg)
{
  return select_server_ctx (s, arg, 0);
}

static int
client_hello_ignore_cb (SSL *s, int *al, void *arg)
{
...
static int
client_hello_reject_cb (SSL *s, int *al, void *arg)
{
...
static int
client_hello_nov12_cb (SSL *s, int *al, void *arg)
{
  int ret;
  unsigned int v;
  const unsigned char *p;
...
static int
server_ocsp_cb_ext (SSL *s, void *arg)
{
...
static int
client_ocsp_cb_ext (SSL *s, void *arg)
{
  int len, status;
  STACK_OF (OCSP_RESPONSE) *sk_resp = NULL;
  OCSP_RESPONSE *rsp;
...
static int
verify_reject_cb (X509_STORE_CTX *ctx, void *arg)
{
...
static int
verify_accept_cb (X509_STORE_CTX *ctx, void *arg)
{
  return 1;
}

static int
broken_session_ticket_cb (SSL *s, unsigned char *key_name, unsigned char *iv,
...
static int
do_not_call_session_ticket_cb (SSL *s, unsigned char *key_name,
...
static int
parse_protos (const char *protos, unsigned char **out, size_t *outlen)
{
  size_t len, i, prefix;
 ...
 */
static int
client_npn_cb (SSL *s, unsigned char **out, unsigned char *outlen,
...
static int
server_npn_cb (SSL *s, const unsigned char **data, unsigned int *len,
               void *arg)
{
  CTX_DATA *ctx_data = (CTX_DATA *)(arg);
 ...
 */
static int
server_alpn_cb (SSL *s, const unsigned char **out, unsigned char *outlen,
                const unsigned char *in, unsigned int inlen, void *arg)
{
  CTX_DATA *ctx_data = (CTX_DATA *)(arg);
  int ret;
  ...
  ret = SSL_select_next_proto (&tmp_out, outlen, ctx_data->alpn_protocols,
                               (unsigned int)ctx_data->alpn_protocols_len, in,
                               inlen);
...
static int
generate_session_ticket_cb (SSL *s, void *arg)
{
...
static int
decrypt_session_ticket_cb (SSL *s, SSL_SESSION *ss,
 ...
 */
static int
configure_handshake_ctx (SSL_CTX *server_ctx, SSL_CTX *server2_ctx,
                         SSL_CTX *client_ctx, const SSL_TEST_CTX *test,
                         const SSL_TEST_EXTRA_CONF *extra,
                         CTX_DATA *server_ctx_data, CTX_DATA *server2_ctx_data,
  ...
  if (extra->server.cert_status != SSL_TEST_CERT_STATUS_NONE)
    {
      ...
      switch (extra->server.cert_status)
        ...
        case SSL_TEST_CERT_STATUS_GOOD_RESPONSE:
          ...
          dummy_ocsp_resp
              = OCSP_response_create (OCSP_RESPONSE_STATUS_SUCCESSFUL, NULL);
  ...
  if (extra->client.alpn_protocols != NULL)
    {
      ...
      if (!TEST_true (parse_protos (extra->client.alpn_protocols, &alpn_protos,
          ...
          || !TEST_int_eq (
              SSL_CTX_set_alpn_protos (client_ctx, alpn_protos,
...
static void
configure_handshake_ssl (SSL *server, SSL *client,
                         const SSL_TEST_EXTRA_CONF *extra)
{
...
static int
create_peer (PEER *peer, SSL_CTX *ctx)
{
...
static void
do_reneg_setup_step (const SSL_TEST_CTX *test_ctx, PEER *peer)
{
  int ret;
  char buf;
 ...
 *
 * However,
 * (a) OpenSSL requires that a connection be shutdown for all protocol
 * versions. (b) We test lower versions, too. So we just implement shutdown. We
 * do a full bidirectional shutdown so that we can compare sent and received
 * close_notify alerts and get some test coverage for SSL_shutdown as a bonus.
 */
static void
do_shutdown_step (PEER *peer)
{
  int ret;
...
static int
renegotiate_op (const SSL_TEST_CTX *test_ctx)
{
...
static int
post_handshake_op (const SSL_TEST_CTX *test_ctx)
{
  switch (test_ctx->handshake_mode)
    {
...
static connect_phase_t
next_phase (const SSL_TEST_CTX *test_ctx, connect_phase_t phase)
{
  switch (phase)
    {
    case HANDSHAKE:
...
static void
do_connect_step (const SSL_TEST_CTX *test_ctx, PEER *peer,
...
handshake_status (peer_status_t last_status, peer_status_t previous_status,
...
static char *
dup_str (const unsigned char *in, size_t len)
{
  char *ret = NULL;
...
static int
create_sctp_socks (int *ssock, int *csock)
{
...
HANDSHAKE_RESULT *
do_handshake (SSL_CTX *server_ctx, SSL_CTX *server2_ctx, SSL_CTX *client_ctx,
