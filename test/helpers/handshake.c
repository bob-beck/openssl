...
static int
broken_session_ticket_cb(SSL* s,
...
static int
do_not_call_session_ticket_cb(SSL* s,
                              unsigned char* key_name,
                              unsigned char* iv,
                              EVP_CIPHER_CTX* ctx,
                              EVP_MAC_CTX* hctx,
...
static int
server_npn_cb(SSL* s, const unsigned char** data, unsigned int* len, void* arg)
{
  CTX_DATA* ctx_data = (CTX_DATA*)(arg);
...
static int
decrypt_session_ticket_cb(SSL* s,
                          SSL_SESSION* ss,
 ...
 */
static int
configure_handshake_ctx(SSL_CTX* server_ctx,
                        SSL_CTX* server2_ctx,
                        SSL_CTX* client_ctx,
                        ...
                        CTX_DATA* client_ctx_data)
{
  ...
  switch (extra->server.servername_callback) {
    ...
    case SSL_TEST_SERVERNAME_CLIENT_HELLO_IGNORE_MISMATCH:
      SSL_CTX_set_client_hello_cb(
        server_ctx, client_hello_ignore_cb, server2_ctx);
      break;
    case SSL_TEST_SERVERNAME_CLIENT_HELLO_REJECT_MISMATCH:
      SSL_CTX_set_client_hello_cb(
        server_ctx, client_hello_reject_cb, server2_ctx);
      break;
    case SSL_TEST_SERVERNAME_CLIENT_HELLO_NO_V12:
      SSL_CTX_set_client_hello_cb(
        server_ctx, client_hello_nov12_cb, server2_ctx);
  }
  ...
  if (extra->client.npn_protocols != NULL) {
    ...
    SSL_CTX_set_next_proto_select_cb(
      client_ctx, client_npn_cb, client_ctx_data);
  }
#endif
  ...
  if (extra->client.alpn_protocols != NULL) {
    ...
    if (!TEST_true(parse_protos(
          extra->client.alpn_protocols, &alpn_protos, &alpn_protos_len))
        /* Reversed return value convention... */
        ||
        !TEST_int_eq(SSL_CTX_set_alpn_protos(
                       client_ctx, alpn_protos, (unsigned int)alpn_protos_len),
                     0))
      goto err;
...
static void
configure_handshake_ssl(SSL* server,
                        SSL* client,
                        const SSL_TEST_EXTRA_CONF* extra)
{
...
static void
do_connect_step(const SSL_TEST_CTX* test_ctx, PEER* peer, connect_phase_t phase)
{
  switch (phase) {
    case HANDSHAKE:
      do_handshake_step(peer);
...
do_handshake_internal(SSL_CTX* server_ctx,
                      SSL_CTX* server2_ctx,
                      SSL_CTX* client_ctx,
  ...
  for (;;) {
    if (client_turn) {
      ...
      status = handshake_status(
        client.status, server.status, 1 /* client went last */);
      if (server.status == PEER_WAITING)
        server.status = PEER_RETRY;
    } else {
      do_connect_step(test_ctx, &server, phase);
      status = handshake_status(
        server.status, client.status, 0 /* server went last */);
    }
...
HANDSHAKE_RESULT*
do_handshake(SSL_CTX* server_ctx,
             SSL_CTX* server2_ctx,
             SSL_CTX* client_ctx,
