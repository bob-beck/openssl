...
int STACK_OF_X509_cmp(const STACK_OF(X509)* sk1, const STACK_OF(X509)* sk2) {
  ...
  if ((res = sk_X509_num(sk1) - sk_X509_num(sk2))) return res;
  for (i = 0; i < sk_X509_num(sk1); i++) {
    a = sk_X509_value(sk1, i);
    b = sk_X509_value(sk2, i);
    if (a != b)
      if ((res = X509_cmp(a, b)) != 0) return res;
  }
  return 0;
}
...
int STACK_OF_X509_push1(STACK_OF(X509)* sk, X509* cert) {
  ...
  if (sk == NULL || cert == NULL) return -1;
  if (!X509_up_ref(cert)) return -1;
  res = sk_X509_push(sk, cert);
  if (res <= 0) X509_free(cert); /* down-ref */
  return res;
}
