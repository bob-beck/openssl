...
int qtest_create_quic_objects (OSSL_LIB_CTX *libctx, SSL_CTX *clientctx,
                               SSL_CTX *serverctx, char *certfile,
                               char *keyfile, int flags, QUIC_TSERVER **qtserv,
                               SSL **cssl, QTEST_FAULT **fault,
                               BIO **tracebio);
...
/*
 * Confirm the server has received a frame encoding error. Equivalent to
 * calling qtest_check_server_transport_err with a code of
 ...
 * wants to resize the packet (either to add new data to it, or to truncate
 * it). The buf provided to packet_plain_listener is over allocated, so this
 * just changes the logical size and never changes the actual address of the
 * buf. This will fail if a large resize is attempted that exceeds the over
 * allocation.
 */
 ...
 * to resize the handshake message (either to add new data to it, or to
 * truncate it). newlen must include the length of the handshake message
 * header. The handshake message buffer is over allocated, so this just changes
 * the logical size and never changes the actual address of the buf. This will
 * fail if a large resize is attempted that exceeds the over allocation.
 */
...
/*
 * Helper function to be called from message specific listener callbacks.
 * newlen is the new length of the specific message excluding the handshake
 * message header.  The buffers provided to the message specific listeners are
 * over allocated, so this just changes the logical size and never changes the
 * actual address of the buffer. This will fail if a large resize is attempted
 * that exceeds the over allocation.
 */
 ...
 * the type of the extension to be deleted. |ext| points to the extension
 * block. On entry |*extlen| contains the length of the extension block. It is
 * updated with the new length on exit. If old_ext is non-NULL, the deleted
 * extension is appended to the given BUF_MEM.
 */
 ...
 * finding or adding them). We could also provide a "listener" API for
 * listening for specific extension types
 */
 ...
 * allocated, so this just changes the logical size and never changes the
 * actual address of the buffer. This will fail if a large resize is attempted
 * that exceeds the over allocation.
 */
...
int qtest_fault_set_bw_limit (QTEST_FAULT *fault, size_t ctos_bw,
                              size_t stoc_bw, int noise_rate);
