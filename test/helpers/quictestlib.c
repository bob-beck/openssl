...
static void
noise_msg_callback(int write_p,
                   int version,
                   int content_type,
                   const void* buf,
                   size_t len,
                   ...
                   void* arg)
{
  ...
  if ((noiseargs->flags & QTEST_FLAG_CLIENT_TRACE) != 0 && !SSL_is_server(ssl))
    SSL_trace(
      write_p, version, content_type, buf, len, ssl, noiseargs->tracebio);
#endif
}

int
qtest_create_quic_objects(OSSL_LIB_CTX* libctx,
                          SSL_CTX* clientctx,
                          SSL_CTX* serverctx,
                          char* certfile,
                          char* keyfile,
  ...
  *qtserv = NULL;
  ...
  if ((flags & QTEST_FLAG_NOISE) != 0)
    ossl_quic_tserver_set_msg_callback(
      *qtserv, noise_msg_callback, &(*fault)->noiseargs);
...
int
qtest_create_quic_connection_ex(QUIC_TSERVER* qtserv,
                                SSL* clientssl,
                                int wanterr)
{
...
static int
packet_plain_mutate(const QUIC_PKT_HDR* hdrin,
                    ...
                    void* arg)
{
  ...
  if (fault->pplaincb != NULL)
    fault->pplaincb(fault,
                    &fault->pplainhdr,
                    (unsigned char*)fault->pplainio.buf,
                    fault->pplainio.buf_len,
                    fault->pplaincbarg);
 ...
 */
int
qtest_fault_prepend_frame(QTEST_FAULT* fault,
...
static int
handshake_mutate(const unsigned char* msgin,
                 size_t msginlen,
                             ...
                             BUF_MEM* old_ext)
{
  ...
  if (old_ext != NULL) {
    ...
    if (!WPACKET_memcpy(
          &old_ext_wpkt, PACKET_data(&subext), PACKET_remaining(&subext)) ||
                 ...
                 size_t* num_processed)
{
  ...
  for (i = 0; i < num_msg; ++i) {
    ...
    if (bdata->fault->pciphercb != NULL) {
      if (!PACKET_buf_init(
            &pkt, bdata->fault->msg.data, bdata->fault->msg.data_len))
        return 0;
      ...
      do {
         ...
         * const is safe
         */
        if (!bdata->fault->pciphercb(bdata->fault,
                                     &hdr,
    ...
    if (bdata->fault->datagramcb != NULL &&
        !bdata->fault->datagramcb(bdata->fault,
                                  &bdata->fault->msg,
                                  stride,
...
qtest_fault_resize_datagram(QTEST_FAULT* fault, size_t newlen)
{
  ...
  if (newlen > fault->msg.data_len)
    memset((unsigned char*)fault->msg.data + fault->msg.data_len,
           0,
...
int
qtest_fault_set_bw_limit(QTEST_FAULT* fault,
                         size_t ctos_bw,
                         size_t stoc_bw,
                         int noise_rate)
{
