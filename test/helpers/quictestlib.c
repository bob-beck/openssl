...
static void
noise_msg_callback (int write_p, int version, int content_type,
...
int
qtest_create_quic_objects (OSSL_LIB_CTX *libctx, SSL_CTX *clientctx,
...
void
qtest_add_time (uint64_t millis)
{
...
void
qtest_start_stopwatch (void)
{
  start_time = qtest_get_time ();
}

uint64_t
qtest_get_stopwatch_time (void)
{
  return ossl_time2ms (ossl_time_subtract (qtest_get_time (), start_time));
}

QTEST_FAULT *
qtest_create_injector (QUIC_TSERVER *ts)
{
  QTEST_FAULT *f;
...
int
qtest_wait_for_timeout (SSL *s, QUIC_TSERVER *qtserv)
{
  struct timeval tv;
...
int
qtest_create_quic_connection_ex (QUIC_TSERVER *qtserv, SSL *clientssl,
                                 int wanterr)
{
...
int
qtest_create_quic_connection (QUIC_TSERVER *qtserv, SSL *clientssl)
{
...
static void
run_server_shutdown_thread (void)
{
  /*
   * This will operate in a busy loop because the server does not block,
   * but should be acceptable because it is local and we expect this to be
...
int
qtest_shutdown (QUIC_TSERVER *qtserv, SSL *clientssl)
{
  int tickserver = 1;
  int ret = 0;
#if defined(OPENSSL_THREADS) && !defined(CRYPTO_TDEBUG)
...
int
qtest_check_server_transport_err (QUIC_TSERVER *qtserv, uint64_t code)
{
...
int
qtest_check_server_protocol_err (QUIC_TSERVER *qtserv)
{
...
int
qtest_check_server_frame_encoding_err (QUIC_TSERVER *qtserv)
{
  return qtest_check_server_transport_err (qtserv,
...
void
qtest_fault_free (QTEST_FAULT *fault)
{
  if (fault == NULL)
    return;
...
static int
packet_plain_mutate (const QUIC_PKT_HDR *hdrin, const OSSL_QTX_IOVEC *iovecin,
...
int
qtest_fault_set_packet_plain_listener (QTEST_FAULT *fault,
                                       qtest_fault_on_packet_plain_cb pplaincb,
                                       void *pplaincbarg)
{
...
int
qtest_fault_resize_plain_packet (QTEST_FAULT *fault, size_t newlen)
{
  unsigned char *buf;
 ...
 */
int
qtest_fault_prepend_frame (QTEST_FAULT *fault, const unsigned char *frame,
                           size_t frame_len)
{
  unsigned char *buf;
  size_t old_len;
  ...
  if (!TEST_true (
          qtest_fault_resize_plain_packet (fault, old_len + frame_len)))
    return 0;
...
static int
handshake_mutate (const unsigned char *msgin, size_t msginlen,
                  unsigned char **msgout, size_t *msgoutlen, void *arg)
{
  QTEST_FAULT *fault = arg;
  unsigned char *buf;
...
int
qtest_fault_set_handshake_listener (QTEST_FAULT *fault,
                                    qtest_fault_on_handshake_cb handshakecb,
                                    void *handshakecbarg)
{
...
int
qtest_fault_set_hand_enc_ext_listener (QTEST_FAULT *fault,
                                       qtest_fault_on_enc_ext_cb encextcb,
                                       void *encextcbarg)
{
...
int
qtest_fault_resize_handshake (QTEST_FAULT *fault, size_t newlen)
{
  unsigned char *buf;
...
int
qtest_fault_resize_message (QTEST_FAULT *fault, size_t newlen)
{
...
int
qtest_fault_delete_extension (QTEST_FAULT *fault, unsigned int exttype,
...
static int
pcipher_sendmmsg (BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg,
...
static long
pcipher_ctrl (BIO *b, int cmd, long larg, void *parg)
{
  BIO *next = BIO_next (b);
...
int
qtest_fault_set_datagram_listener (QTEST_FAULT *fault,
                                   qtest_fault_on_datagram_cb datagramcb,
                                   void *datagramcbarg)
{
...
int
qtest_fault_resize_datagram (QTEST_FAULT *fault, size_t newlen)
{
...
int
qtest_fault_set_bw_limit (QTEST_FAULT *fault, size_t ctos_bw, size_t stoc_bw,
                          int noise_rate)
{
...
int
bio_msg_copy (BIO_MSG *dst, BIO_MSG *src)
{
  /*
