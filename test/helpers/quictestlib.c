...
static void noise_msg_callback(int write_p, int version, int content_type,
  ...
  if (content_type == SSL3_RT_QUIC_FRAME_FULL) {
    ...
    if (!PACKET_buf_init(&pkt, buf, len)) return;

    if (!ossl_quic_wire_peek_frame_header(&pkt, &frame_type, NULL)) return;
...
int qtest_create_quic_objects(OSSL_LIB_CTX* libctx, SSL_CTX* clientctx,
  ...
  if (client_ready_cond == NULL) {
    ...
    if (client_ready_cond == NULL) return 0;
  }
  ...
  *qtserv = NULL;
  ...
  if (fault != NULL) {
    *fault = OPENSSL_zalloc(sizeof(**fault));
    if (*fault == NULL) goto err;
    bdata->fault = *fault;
  }

  if (*cssl == NULL) {
    *cssl = SSL_new(clientctx);
    if (!TEST_ptr(*cssl)) goto err;
  }

#ifndef OPENSSL_NO_SSL_TRACE
  if ((flags & QTEST_FLAG_CLIENT_TRACE) != 0) {
    tmpbio = BIO_new_fp(stdout, BIO_NOCLOSE);
    if (!TEST_ptr(tmpbio)) goto err;

    SSL_set_msg_callback(*cssl, SSL_trace);
    SSL_set_msg_callback_arg(*cssl, tmpbio);
  }
#endif
  if (tracebio != NULL) *tracebio = tmpbio;

  /* SSL_set_alpn_protos returns 0 for success! */
  if (!TEST_false(SSL_set_alpn_protos(*cssl, alpn, sizeof(alpn)))) goto err;

  if (!TEST_ptr(peeraddr = BIO_ADDR_new())) goto err;
  ...
  if ((flags & QTEST_FLAG_BLOCK) != 0) {
    ...
    if (!TEST_ptr(localaddr = BIO_ADDR_new())) goto err;
  ...
  if ((flags & QTEST_FLAG_PACKET_SPLIT) != 0) {
    ...
    if (!TEST_ptr(pktsplitbio)) goto err;
  ...
  if ((flags & QTEST_FLAG_NOISE) != 0) {
     ...
     */
    if (!TEST_ptr(fault)) goto err;

    noisebio = BIO_new(bio_f_noisy_dgram_filter());

    if (!TEST_ptr(noisebio)) goto err;
  ...
  if (!TEST_true(SSL_set1_initial_peer_addr(*cssl, peeraddr))) goto err;

  fisbio = BIO_new(qtest_get_bio_method());
  if (!TEST_ptr(fisbio)) goto err;

  BIO_set_data(fisbio, bdata);

  if (!BIO_up_ref(sbio)) goto err;
  ...
  if (serverctx != NULL && !TEST_true(SSL_CTX_up_ref(serverctx))) goto err;
  tserver_args.ctx = serverctx;
  if (fake_now_lock == NULL) {
    fake_now_lock = CRYPTO_THREAD_lock_new();
    if (fake_now_lock == NULL) goto err;
  }
  if ((flags & QTEST_FLAG_FAKE_TIME) != 0) {
    using_fake_time = 1;
    qtest_reset_time();
...
err:
  ...
  *cssl = NULL;
  ...
  if (fault != NULL) OPENSSL_free(*fault);
  OPENSSL_free(bdata);
  BIO_free(tmpbio);
  if (tracebio != NULL) *tracebio = NULL;

  return 0;
}

void qtest_add_time(uint64_t millis) {
  if (!CRYPTO_THREAD_write_lock(fake_now_lock)) return;
...
static OSSL_TIME qtest_get_time(void) {
  ...
  if (!CRYPTO_THREAD_read_lock(fake_now_lock)) return ossl_time_zero();
  ret = fake_now;
  CRYPTO_THREAD_unlock(fake_now_lock);
  return ret;
}
...
int qtest_wait_for_timeout(SSL* s, QUIC_TSERVER* qtserv) {
  ...
  if (s == NULL || SSL_get_blocking_mode(s)) return 1;
   ...
   */
  if (!SSL_get_event_timeout(s, &tv, &cinf)) return 0;
  ...
  if (ossl_time_is_infinite(mintimeout)) return 0;
...
int qtest_create_quic_connection_ex(QUIC_TSERVER* qtserv, SSL* clientssl,
  ...
  if (clientssl != NULL) abortserverthread = 0;
  ...
  if (!TEST_ptr(qtserv)) {
     ...
     */
    globtserv = qtserv;
    if (!TEST_true(run_thread(&t, run_server_thread))) goto err;
  ...
  do {
    ...
    qtest_add_time(1);
    if (clientssl != NULL) SSL_handle_events(clientssl);
    if (qtserv != NULL) {
#if defined(OPENSSL_THREADS) && !defined(CRYPTO_TDEBUG)
      ossl_crypto_mutex_lock(client_ready_mutex);
      for (;;) {
        if (client_ready == 1) break;
    ...
    if ((retc <= 0 && !clienterr) || (rets <= 0 && !servererr)) {
      if (!qtest_wait_for_timeout(clientssl, qtserv)) goto err;
    }
  ...
  if (qtserv == NULL && rets > 0) {
    ...
    ossl_crypto_condvar_broadcast(client_ready_cond);
    ossl_crypto_mutex_unlock(client_ready_mutex);
    if (!TEST_true(wait_for_thread(t)) || !TEST_true(globserverret)) goto err;
#else
...
int qtest_shutdown(QUIC_TSERVER* qtserv, SSL* clientssl) {
  ...
  if (SSL_get_blocking_mode(clientssl) > 0) {
    ...
    globtserv = qtserv;
    shutdowndone = 0;
    if (!TEST_true(run_thread(&t, run_server_shutdown_thread))) return 0;
  ...
  for (;;) {
    ...
    if (tickserver) ossl_quic_tserver_tick(qtserv);
  }

#if defined(OPENSSL_THREADS) && !defined(CRYPTO_TDEBUG)
  tsan_store(&shutdowndone, 1);
  if (!tickserver) {
    if (!TEST_true(wait_for_thread(t))) ret = 0;
  }
#endif
...
int qtest_fault_resize_plain_packet(QTEST_FAULT* fault, size_t newlen) {
   ...
   * incorrectly called
   */
  if (fault->pplainbuf_alloc == 0) return 0;
...
int qtest_fault_resize_handshake(QTEST_FAULT* fault, size_t newlen) {
   ...
   * incorrectly called
   */
  if (fault->handbufalloc == 0) return 0;
...
int qtest_fault_delete_extension(QTEST_FAULT* fault, unsigned int exttype,
  ...
  if (!PACKET_buf_init(&pkt, ext, *extlen)) return 0;

  /* Extension block starts with 2 bytes for extension block length */
  if (!PACKET_as_length_prefixed_2(&pkt, &sub)) return 0;
  ...
  if (old_ext != NULL) {
    if (!WPACKET_init(&old_ext_wpkt, old_ext)) return 0;
   ...
   */
  if (end < ext + *extlen) memmove((unsigned char*)start, end, end - start);
  ...
  *extlen = newlen + 2;
  ...
  if (!qtest_fault_resize_message(fault, msglen)) return 0;
...
BIO_METHOD* qtest_get_bio_method(void) {
  ...
  if (pcipherbiometh != NULL) return pcipherbiometh;

  tmp = BIO_meth_new(BIO_TYPE_CIPHER_PACKET_FILTER, "Cipher Packet Filter");

  if (!TEST_ptr(tmp)) return NULL;
...
int qtest_fault_resize_datagram(QTEST_FAULT* fault, size_t newlen) {
  if (newlen > fault->msgalloc) return 0;
...
int qtest_fault_set_bw_limit(QTEST_FAULT* fault, size_t ctos_bw, size_t stoc_bw,
  ...
  if (!TEST_ptr(sbio) || !TEST_ptr(cbio)) return 0;
...
int bio_msg_copy(BIO_MSG* dst, BIO_MSG* src) {
  ...
  if (dst->local != NULL) {
    if (src->local != NULL) {
      if (!TEST_true(BIO_ADDR_copy(dst->local, src->local))) return 0;
    } else {
