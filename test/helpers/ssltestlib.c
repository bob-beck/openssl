...
static void mempacket_free(MEMPACKET* pkt) {
  if (pkt->data != NULL) OPENSSL_free(pkt->data);
  OPENSSL_free(pkt);
}
...
static int mempacket_test_new(BIO* bio) {
  ...
  if (!TEST_ptr(ctx = OPENSSL_zalloc(sizeof(*ctx)))) return 0;
...
static int mempacket_test_read(BIO* bio, char* out, int outl) {
  ...
  if (outl > thispkt->len) outl = thispkt->len;
  ...
  if (thispkt->type != INJECT_PACKET_IGNORE_REC_SEQ &&
    ...
    for (rem = thispkt->len, rec = thispkt->data; rem > 0; rem -= len) {
      if (rem < DTLS1_RT_HEADER_LENGTH) return -1;
      ...
      if (rem < (int)len) return -1;
      if (ctx->droprec == (int)ctx->currrec && ctx->dropepoch == epoch) {
        if (rem > (int)len) memmove(rec, rec + len, rem - len);
        outl -= len;
        ctx->droprec = -1;
...
int mempacket_swap_epoch(BIO* bio) {
  ...
  if (thispkt == NULL) return 0;

  for (rem = thispkt->len, rec = thispkt->data; rem > 0;
       rem -= len, rec += len) {
    if (rem < DTLS1_RT_HEADER_LENGTH) return 0;
    epoch = (rec[EPOCH_HI] << 8) | rec[EPOCH_LO];
    len = ((rec[RECORD_LEN_HI] << 8) | rec[RECORD_LEN_LO]) +
          DTLS1_RT_HEADER_LENGTH;
    if (rem < len) return 0;
...
int mempacket_move_packet(BIO* bio, int d, int s) {
  ...
  if (sk_MEMPACKET_delete(ctx->pkts, s) != thispkt) return 0;

  thispkt->num -= (s - d);
  if (sk_MEMPACKET_insert(ctx->pkts, thispkt, d) <= 0) return 0;
...
int mempacket_test_inject(BIO* bio, const char* in, int inl, int pktnum,
  ...
  for (i = 0; i < (duprec ? 3 : 1); i++) {
    if (!TEST_ptr(allpkts[i] = OPENSSL_malloc(sizeof(*thispkt)))) goto err;
    thispkt = allpkts[i];

    if (!TEST_ptr(thispkt->data = OPENSSL_malloc(inl))) goto err;
    /*
  ...
  for (i = 0; i < sk_MEMPACKET_num(ctx->pkts); i++) {
    if (!TEST_ptr(looppkt = sk_MEMPACKET_value(ctx->pkts, i))) goto err;
    /* Check if we found the right place to insert this packet */
    if (looppkt->num > thispkt->num) {
      if (sk_MEMPACKET_insert(ctx->pkts, thispkt, i) == 0) goto err;
      /* If we're doing up front injection then we're done */
      if (pktnum >= 0) return inl;
      /*
  ...
  for (i = 0; i < (duprec ? 3 : 1); i++) {
    thispkt = allpkts[i];
    if (!sk_MEMPACKET_push(ctx->pkts, thispkt)) goto err;

    if (pktnum < 0) ctx->lastpkt++;
  }
...
int create_ssl_ctx_pair(OSSL_LIB_CTX* libctx, const SSL_METHOD* sm,
  ...
  if (sctx != NULL) *sctx = serverctx;
  if (cctx != NULL) *cctx = clientctx;
  return 1;

err:
  if (sctx != NULL && *sctx == NULL) SSL_CTX_free(serverctx);
  if (cctx != NULL && *cctx == NULL) SSL_CTX_free(clientctx);
  return 0;
}
...
int create_test_sockets(int* cfdp, int* sfdp, int socktype, BIO_ADDR* saddr) {
  ...
  if (afd == INVALID_SOCKET) return 0;

  if (bind(afd, (struct sockaddr*)&sin, sizeof(sin)) < 0) goto out;

  if (getsockname(afd, (struct sockaddr*)&sin, &slen) < 0) goto out;

  if (saddr != NULL && !BIO_ADDR_rawmake(saddr, sin.sin_family, &sin.sin_addr,
                                         sizeof(sin.sin_addr), sin.sin_port))
    goto out;

  if (socktype == SOCK_STREAM && listen(afd, 1) < 0) goto out;

  cfd = BIO_socket(AF_INET, socktype,
                   socktype == SOCK_STREAM ? IPPROTO_TCP : IPPROTO_UDP, 0);
  if (cfd == INVALID_SOCKET) goto out;

  if (!BIO_socket_nbio(afd, 1)) goto out;
  ...
  while (sfd == -1 || !cfd_connected) {
    ...
    if (sfd == -1 && errno != EAGAIN) goto out;
  ...
  if (!BIO_socket_nbio(cfd, 1) || !BIO_socket_nbio(sfd, 1)) goto out;
  ret = 1;
  *cfdp = cfd;
  *sfdp = sfd;
  goto success;
...
int create_ssl_objects(SSL_CTX* serverctx, SSL_CTX* clientctx, SSL** sssl,
  ...
  if (!BIO_up_ref(s_to_c_bio)) goto error;
...
int create_bare_ssl_connection_ex(SSL* serverssl, SSL* clientssl, int want,
  ...
  do {
    ...
    while (!clienterr && retc <= 0 && err == SSL_ERROR_WANT_WRITE) {
      ...
      if (retc <= 0) err = SSL_get_error(clientssl, retc);
      icm_count++;
    }

    if (!clienterr && retc <= 0 && err != SSL_ERROR_WANT_READ) {
      TEST_info("SSL_connect() failed %d, %d", retc, err);
      if (want != SSL_ERROR_SSL) TEST_openssl_errors();
      clienterr = 1;
    }
    if (want != SSL_ERROR_NONE && err == want) goto err;
    ...
    while (!servererr && rets <= 0 && err == SSL_ERROR_WANT_WRITE) {
...
#endif
      {
        ...
        if (rets <= 0) err = SSL_get_error(serverssl, rets);
        ism_count++;
      }
    }
  ...
  ret = 1;
err:
  if (cm_count != NULL) *cm_count = icm_count;
  if (sm_count != NULL) *sm_count = ism_count;
#ifndef OPENSSL_NO_SOCK
  BIO_ADDR_free(peer);
#endif
  return ret;
...
int create_bare_ssl_connection(SSL* serverssl, SSL* clientssl, int want,
                               int read, int listen) {
  return create_bare_ssl_connection_ex(serverssl, clientssl, want, read, listen,
                                       NULL, NULL);
}
...
int create_ssl_connection_ex(SSL* serverssl, SSL* clientssl, int want,
  ...
  if (!create_bare_ssl_connection_ex(serverssl, clientssl, want, 1, 0, cm_count,
                                     sm_count))
    return 0;
...
int ssl_ctx_add_large_cert_chain(OSSL_LIB_CTX* libctx, SSL_CTX* sctx,
  ...
  if (!TEST_ptr(certbio = BIO_new_file(cert_file, "r"))) goto end;

  if (!TEST_ptr(chaincert = X509_new_ex(libctx, NULL))) goto end;

  if (PEM_read_bio_X509(certbio, &chaincert, NULL, NULL) == NULL) goto end;
  BIO_free(certbio);
  certbio = NULL;
  ...
  for (i = 0; i < NUM_EXTRA_CERTS; i++) {
    if (!X509_up_ref(chaincert)) goto end;
...
ENGINE* load_dasync(void) {
  ...
  if (!TEST_ptr(e = ENGINE_by_id("dasync"))) return NULL;
