  ...
  if (meth_maybe_retry == NULL)
    {
      if (!TEST_ptr (meth_maybe_retry
          ...
          || !TEST_true (
              BIO_meth_set_ctrl (meth_maybe_retry, maybe_retry_ctrl))
          || !TEST_true (
...
int
create_ssl_ctx_pair (OSSL_LIB_CTX *libctx, const SSL_METHOD *sm,
...
int
create_ssl_objects2 (SSL_CTX *serverctx, SSL_CTX *clientctx, SSL **sssl,
 ...
 */
int
create_ssl_objects (SSL_CTX *serverctx, SSL_CTX *clientctx, SSL **sssl,
 ...
 * the connection once we've completed one half of it, to ensure any
 * retransmits get triggered. We stop the connection attempt (and return a
 * failure value) if either peer has SSL_get_error() return the value in the
 * |want| parameter. The connection attempt could be restarted by a subsequent
 * call to this function.
 */
int
create_bare_ssl_connection_ex (SSL *serverssl, SSL *clientssl, int want,
...
int
create_bare_ssl_connection (SSL *serverssl, SSL *clientssl, int want, int read,
                            int listen)
{
 ...
 */
int
create_ssl_connection_ex (SSL *serverssl, SSL *clientssl, int want,
...
int
ssl_ctx_add_large_cert_chain (OSSL_LIB_CTX *libctx, SSL_CTX *sctx,
