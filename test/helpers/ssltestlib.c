...
tls_dump_free(BIO* a);
static int
tls_dump_read(BIO* b, char* out, int outl);
static int
tls_dump_write(BIO* b, const char* in, int inl);
static long
tls_dump_ctrl(BIO* b, int cmd, long num, void* ptr);
static int
tls_dump_gets(BIO* bp, char* buf, int size);
static int
tls_dump_puts(BIO* bp, const char* str);
...
void
bio_f_tls_dump_filter_free(void)
{
  BIO_meth_free(method_tls_dump);
}

static int
tls_dump_new(BIO* bio)
{
  BIO_set_init(bio, 1);
  return 1;
}
...
static void
dump_data(const char* data, int len)
{
...
static int
tls_dump_read(BIO* bio, char* out, int outl)
{
  int ret;
  BIO* next = BIO_next(bio);
...
static int
tls_dump_write(BIO* bio, const char* in, int inl)
{
  int ret;
  BIO* next = BIO_next(bio);
...
static long
tls_dump_ctrl(BIO* bio, int cmd, long num, void* ptr)
{
  long ret;
  BIO* next = BIO_next(bio);
...
static int
tls_dump_gets(BIO* bio, char* buf, int size)
{
...
static int
tls_dump_puts(BIO* bio, const char* str)
{
...
static int
mempacket_test_new(BIO* bi);
static int
mempacket_test_free(BIO* a);
static int
mempacket_test_read(BIO* b, char* out, int outl);
static int
mempacket_test_write(BIO* b, const char* in, int inl);
static long
mempacket_test_ctrl(BIO* b, int cmd, long num, void* ptr);
static int
mempacket_test_gets(BIO* bp, char* buf, int size);
static int
mempacket_test_puts(BIO* bp, const char* str);
...
void
bio_s_mempacket_test_free(void)
{
  BIO_meth_free(meth_mem);
}

static int
mempacket_test_new(BIO* bio)
{
...
static int
mempacket_test_read(BIO* bio, char* out, int outl)
{
  MEMPACKET_TEST_CTX* ctx = BIO_get_data(bio);
  MEMPACKET* thispkt;
  unsigned char* rec;
...
int
mempacket_move_packet(BIO* bio, int d, int s)
{
  MEMPACKET_TEST_CTX* ctx = BIO_get_data(bio);
  MEMPACKET* thispkt;
  int numpkts = sk_MEMPACKET_num(ctx->pkts);
...
int
mempacket_dup_last_packet(BIO* bio)
{
  MEMPACKET_TEST_CTX* ctx = BIO_get_data(bio);
...
int
mempacket_test_inject(BIO* bio, const char* in, int inl, int pktnum, int type)
{
  MEMPACKET_TEST_CTX* ctx = BIO_get_data(bio);
...
static int
mempacket_test_write(BIO* bio, const char* in, int inl)
{
  return mempacket_test_inject(bio, in, inl, -1, STANDARD_PACKET);
}

static long
mempacket_test_ctrl(BIO* bio, int cmd, long num, void* ptr)
{
  long ret = 1;
  MEMPACKET_TEST_CTX* ctx = BIO_get_data(bio);
  MEMPACKET* thispkt;
...
static int
mempacket_test_gets(BIO* bio, char* buf, int size)
{
  /* We don't support this - not needed anyway */
  return -1;
}
...
static int
mempacket_test_puts(BIO* bio, const char* str)
{
  return mempacket_test_write(bio, str, (int)strlen(str));
}

static int
always_retry_new(BIO* bi);
static int
always_retry_free(BIO* a);
static int
always_retry_read(BIO* b, char* out, int outl);
static int
always_retry_write(BIO* b, const char* in, int inl);
static long
always_retry_ctrl(BIO* b, int cmd, long num, void* ptr);
static int
always_retry_gets(BIO* bp, char* buf, int size);
static int
always_retry_puts(BIO* bp, const char* str);
...
void
bio_s_always_retry_free(void)
{
  BIO_meth_free(meth_always_retry);
}

static int
always_retry_new(BIO* bio)
{
  BIO_set_init(bio, 1);
  return 1;
}
...
void
set_always_retry_err_val(int err)
{
  retry_err = err;
}

static int
always_retry_read(BIO* bio, char* out, int outl)
{
  BIO_set_retry_read(bio);
  return retry_err;
}
...
static long
always_retry_ctrl(BIO* bio, int cmd, long num, void* ptr)
{
  long ret = 1;
...
static int
always_retry_gets(BIO* bio, char* buf, int size)
{
  BIO_set_retry_read(bio);
  return retry_err;
}
...
static int
always_retry_puts(BIO* bio, const char* str)
{
  BIO_set_retry_write(bio);
  return retry_err;
}
...
static int
maybe_retry_new(BIO* bi);
static int
maybe_retry_free(BIO* a);
static int
maybe_retry_write(BIO* b, const char* in, int inl);
static long
maybe_retry_ctrl(BIO* b, int cmd, long num, void* ptr);

const BIO_METHOD*
bio_s_maybe_retry(void)
{
...
void
bio_s_maybe_retry_free(void)
{
  BIO_meth_free(meth_maybe_retry);
}

static int
maybe_retry_new(BIO* bio)
{
...
static int
maybe_retry_write(BIO* bio, const char* in, int inl)
{
  struct maybe_retry_data_st* data = BIO_get_data(bio);
...
static long
maybe_retry_ctrl(BIO* bio, int cmd, long num, void* ptr)
{
  struct maybe_retry_data_st* data = BIO_get_data(bio);
...
int
create_ssl_ctx_pair(OSSL_LIB_CTX* libctx,
                    const SSL_METHOD* sm,
                    const SSL_METHOD* cm,
...
int
wait_until_sock_readable(int sock)
{
  fd_set readfds;
...
int
create_test_sockets(int* cfdp, int* sfdp, int socktype, BIO_ADDR* saddr)
{
  ...
  afd = BIO_socket(
    AF_INET, socktype, socktype == SOCK_STREAM ? IPPROTO_TCP : IPPROTO_UDP, 0);
  ...
  if (saddr != NULL && !BIO_ADDR_rawmake(saddr,
                                         sin.sin_family,
                                         &sin.sin_addr,
                                         sizeof(sin.sin_addr),
                                         sin.sin_port))
...
int
create_ssl_objects2(SSL_CTX* serverctx,
                    SSL_CTX* clientctx,
                    SSL** sssl,
                    SSL** cssl,
                    int sfd,
 ...
 */
int
create_ssl_objects(SSL_CTX* serverctx,
                   SSL_CTX* clientctx,
                   SSL** sssl,
                   SSL** cssl,
                   BIO* s_to_c_fbio,
 ...
 */
int
create_bare_ssl_connection_ex(SSL* serverssl,
                              SSL* clientssl,
                              int want,
                              int read,
                              int listen,
...
int
create_bare_ssl_connection(SSL* serverssl,
                           SSL* clientssl,
                           int want,
                           int read,
                           int listen)
{
  return create_bare_ssl_connection_ex(
    serverssl, clientssl, want, read, listen, NULL, NULL);
}
 ...
 */
int
create_ssl_connection_ex(SSL* serverssl,
                         SSL* clientssl,
                         int want,
                         int* cm_count,
                         int* sm_count)
{
  ...
  if (!create_bare_ssl_connection_ex(
        serverssl, clientssl, want, 1, 0, cm_count, sm_count))
    return 0;
...
int
create_ssl_connection(SSL* serverssl, SSL* clientssl, int want)
{
  return create_ssl_connection_ex(serverssl, clientssl, want, NULL, NULL);
}

void
shutdown_ssl_connection(SSL* serverssl, SSL* clientssl)
{
...
SSL_SESSION*
create_a_psk(SSL* ssl, size_t mdsize)
{
...
int
ssl_ctx_add_large_cert_chain(OSSL_LIB_CTX* libctx,
                             SSL_CTX* sctx,
