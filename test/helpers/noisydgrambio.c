...
static size_t
bandwidth_limit(struct bw_limiter_st* limit,
                OSSL_TIME now,
                BIO_MSG* msg,
                size_t num_msg)
{
...
static void
flip_bits(unsigned char* msg, size_t msg_len, uint16_t flip, size_t flip_offset)
{
  if (flip == 0)
    return;
