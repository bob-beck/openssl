...
static size_t bandwidth_limit(struct bw_limiter_st* limit, OSSL_TIME now,
  ...
  if (num_msg > MAX_PKTS_PER_WINDOW) num_msg = MAX_PKTS_PER_WINDOW;
...
static void flip_bits(unsigned char* msg, size_t msg_len, uint16_t flip,
  ...
  if (msg_len < 2) return;
  if (msg_len < flip_offset + 2) flip_offset = msg_len - 2;
...
static int noisy_dgram_recvmmsg(BIO* bio, BIO_MSG* msg, size_t stride,
  ...
  if (msg_cnt == 0) goto end;

  if (data->noise_rate == 0) goto end;
  ...
  for (i = 0, thismsg = msg; i < msg_cnt; i++, thismsg++, data->this_dgram++) {
    ...
    if (data->reinject_dgram > 0 && data->reinject_dgram == data->this_dgram) {
      if (msg_cnt < num_msg) {
        ...
        for (j = msg_cnt; j > i; j--) {
          if (!bio_msg_copy(&msg[j], &msg[j - 1])) return 0;
        }
    ...
    if (reinject > 0 && data->reinject_dgram == 0) {
       ...
       */
      if (!bio_msg_copy(&data->msg, thismsg)) return 0;
    ...
    if (should_drop) {
      ...
      for (j = i + 1; j < msg_cnt; j++) {
        if (!bio_msg_copy(&msg[j - 1], &msg[j])) return 0;
      }
      msg_cnt--;
    }
  }
