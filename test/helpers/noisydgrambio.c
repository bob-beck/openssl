...
static long
noisy_dgram_ctrl (BIO *bio, int cmd, long num, void *ptr)
{
  long ret;
...
static size_t
bandwidth_limit (struct bw_limiter_st *limit, OSSL_TIME now, BIO_MSG *msg,
                 size_t num_msg)
{
  size_t i;
...
static int
noisy_dgram_sendmmsg (BIO *bio, BIO_MSG *msg, size_t stride, size_t num_msg,
...
static void
get_noise (int noise_rate, int long_header, uint64_t *reinject,
...
static void
flip_bits (unsigned char *msg, size_t msg_len, uint16_t flip,
           size_t flip_offset)
{
  if (flip == 0)
    return;
...
static int
noisy_dgram_recvmmsg (BIO *bio, BIO_MSG *msg, size_t stride, size_t num_msg,
                      uint64_t flags, size_t *msgs_processed)
{
  BIO *next = BIO_next (bio);
...
static void
data_free (struct noisy_dgram_st *data)
{
  if (data == NULL)
    return;
