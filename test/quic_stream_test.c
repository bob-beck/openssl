...
static int
compare_iov(const unsigned char* ref,
            size_t ref_len,
            const OSSL_QTX_IOVEC* iov,
            size_t iov_len)
{
...
static int
test_single_copy_read(QUIC_RSTREAM* qrs,
                      unsigned char* buf,
                      size_t size,
                      size_t* readbytes,
                      int* fin)
...
test_rstream_simple(int idx)
{
  ...
  if (!TEST_true(ossl_quic_rstream_queue_data(
        rstream, NULL, 5, simple_data + 5, 10, 0)) ||
      !TEST_true(
        ossl_quic_rstream_queue_data(rstream,
                                     NULL,
      ...
      !TEST_true(read_fn(
        rstream, buf + 14 + 5, sizeof(buf) - 14 - 5, &readbytes, &fin)) ||
...
test_rstream_random(int idx)
{
  ...
  for (r = 0; r < 100; ++r) {
    ...
    if (idx % 2 == 0) {
      if (!TEST_true(test_single_copy_read(
