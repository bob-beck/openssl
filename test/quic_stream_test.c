...
static int
compare_iov (const unsigned char *ref, size_t ref_len,
...
test_sstream_simple (void)
{
  ...
  for (i = 0; i < 2; ++i)
    {
      num_iov = OSSL_NELEM (iov);
      if (!TEST_true (ossl_quic_sstream_get_stream_frame (sstream, 0, &hdr,
                                                          iov, &num_iov))
...
test_sstream_bulk (int idx)
{
  ...
  for (i = 0, rd = 0; rd < expected; ++i)
    {
      num_iov = OSSL_NELEM (iov);
      if (!TEST_true (ossl_quic_sstream_get_stream_frame (sstream, i, &hdr,
                                                          iov, &num_iov)))
        goto err;
...
static int
test_single_copy_read (QUIC_RSTREAM *qrs, unsigned char *buf, size_t size,
