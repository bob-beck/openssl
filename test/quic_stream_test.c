...
static int compare_iov(const unsigned char* ref, size_t ref_len,
  ...
  if (ref_len != total_len) return 0;

  for (i = 0; i < iov_len; ++i) {
    if (memcmp(cur, iov[i].buf, iov[i].buf_len)) return 0;
...
static int test_sstream_simple(void) {
  ...
  if (!TEST_ptr(sstream = ossl_quic_sstream_new(init_size))) goto err;

  /* A stream with nothing yet appended is totally acked */
  if (!TEST_true(ossl_quic_sstream_is_totally_acked(sstream))) goto err;
  ...
  if (!TEST_false(ossl_quic_sstream_is_totally_acked(sstream))) goto err;
  ...
  if (!TEST_true(compare_iov(data_1, sizeof(data_1), iov, num_iov))) goto err;

  /* Mark data as half transmitted */
  if (!TEST_true(ossl_quic_sstream_mark_transmitted(sstream, 0, 7))) goto err;
  ...
  if (!TEST_true(ossl_quic_sstream_mark_transmitted(sstream, 8, 15))) goto err;
  ...
  if (!TEST_true(ossl_quic_sstream_mark_lost(sstream, 4, 6))) goto err;
  ...
  if (!TEST_true(compare_iov(data_1 + 4, 3, iov, num_iov))) goto err;

  /* Retransmit */
  if (!TEST_true(ossl_quic_sstream_mark_transmitted(sstream, 4, 6))) goto err;
  ...
  if (!TEST_size_t_eq(ossl_quic_sstream_get_buffer_used(sstream), 16)) goto err;
  ...
  if (!TEST_true(ossl_quic_sstream_mark_transmitted_fin(sstream, 16))) goto err;
  ...
  /* Lose FIN frame */
  if (!TEST_true(ossl_quic_sstream_mark_lost_fin(sstream))) goto err;
  ...
  /* Acknowledge fin. */
  if (!TEST_true(ossl_quic_sstream_mark_acked_fin(sstream))) goto err;
...
static int test_sstream_bulk(int idx) {
  ...
  if (!TEST_ptr(src_buf = OPENSSL_zalloc(init_size))) goto err;

  if (!TEST_ptr(dst_buf = OPENSSL_malloc(init_size))) goto err;

  if (!TEST_ptr(ref_src_buf = OPENSSL_malloc(init_size))) goto err;

  if (!TEST_ptr(ref_dst_buf = OPENSSL_malloc(init_size))) goto err;
  ...
  for (i = 0, rd = 0; rd < expected; ++i) {
    ...
    cur_rd = 0;
    for (j = 0; j < num_iov; ++j) {
      if (!TEST_size_t_le(iov[j].buf_len + rd, expected)) goto err;
    ...
    if (!TEST_uint64_t_eq(cur_rd, hdr.len)) goto err;

    rd += cur_rd;
  }

  if (!TEST_mem_eq(dst_buf, rd, ref_dst_buf, expected)) goto err;
...
static int test_single_copy_read(QUIC_RSTREAM* qrs, unsigned char* buf,
  ...
  *readbytes = 0;
  ...
  for (;;) {
    if (!ossl_quic_rstream_get_record(qrs, &record, &rec_len, fin)) return 0;
    if (rec_len == 0) break;
    if (rec_len > size) {
      rec_len = size;
      *fin = 0;
    ...
    *readbytes += rec_len;
    ...
    if (!ossl_quic_rstream_release_record(qrs, rec_len)) return 0;
    if (*fin || size == 0) break;
  }
...
static int test_rstream_simple(int idx) {
  ...
  if (!TEST_ptr(rstream = ossl_quic_rstream_new(NULL, NULL, 0))) goto err;

  if (!TEST_true(ossl_quic_rstream_queue_data(rstream, NULL, 5, simple_data + 5,
                                              10, 0)) ||
      !TEST_true(ossl_quic_rstream_queue_data(
          rstream, NULL, sizeof(simple_data) - 1,
          simple_data + sizeof(simple_data) - 1, 1, 1)) ||
      !TEST_true(ossl_quic_rstream_peek(rstream, buf, sizeof(buf), &readbytes,
                                        &fin)) ||
...
static int test_rstream_random(int idx) {
  ...
  if (idx % 3 == 0) ossl_quic_rstream_set_cleanse(rstream, 1);
  ...
  for (r = 0; r < 100; ++r) {
    for (s = 0; s < 10; ++s) {
      ...
      if (test_random() % 10 == 0) /* drop packet */
        continue;

      if (off <= queued_min && off + size > queued_min) queued_min = off + size;

      if (!TEST_true(ossl_quic_rstream_queue_data(rstream, NULL, off,
                                                  bulk_data + off, size, 0)))
        goto err;
      if (queued_max < off + size) queued_max = off + size;

      if (test_random() % 5 != 0) continue;

      /* random overlapping retransmit */
      off = read_off + test_random() % 50;
      if (off > 50) off -= 50;
      size = test_random() % 100 + 1;
      if (off + size > data_size) off = data_size - size;
      if (off <= queued_min && off + size > queued_min) queued_min = off + size;
  ...
  if (idx % 3 == 0)
    for (i = 0; i < read_off; i++)
      if (!TEST_uchar_eq(bulk_data[i], 0)) goto err;
  ...
  if (read_off == data_size && fin_set && !fin) {
    ...
    if (!TEST_size_t_eq(readbytes, 0) || !TEST_true(fin)) goto err;
  }
