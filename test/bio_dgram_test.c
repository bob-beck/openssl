...
static int
compare_addr(const BIO_ADDR* a, const BIO_ADDR* b)
{
            ...
            size_t* num_processed)
{
  ...
  for (done = 0; done < num_msg;) {
    if (!BIO_sendmmsg(
          b, msg + done, sizeof(BIO_MSG), num_msg - done, flags, num_processed))
      return 0;
...
static int
test_bio_dgram_impl(int af, int use_local)
{
  int testresult = 0;
  BIO *b1 = NULL, *b2 = NULL;
  int fd1 = -1, fd2 = -1;
...
static int
random_data(const uint32_t* key, uint8_t* data, size_t data_len, size_t offset)
{
  int ret = 0, outl;
  ...
  if (EVP_EncryptInit_ex2(
        ctx, cipher, (uint8_t*)key, (uint8_t*)counter, NULL) == 0)
    goto err;
...
test_bio_dgram_pair(int idx)
{
  ...
  if (!TEST_true(BIO_sendmmsg(
        bio1, msgs, sizeof(BIO_MSG), OSSL_NELEM(msgs), 0, &num_processed)) ||
      !TEST_size_t_eq(num_processed, 2))
    goto err;
  ...
  rmsgs[1].data_len = 64;
  if (!TEST_true(BIO_recvmmsg(
        bio2, rmsgs, sizeof(BIO_MSG), OSSL_NELEM(rmsgs), 0, &num_processed)) ||
      !TEST_size_t_eq(num_processed, 2))
    goto err;
  ...
  if (!TEST_mem_eq(
        rmsgs[0].data, rmsgs[0].data_len, msgs[0].data, msgs[0].data_len))
    goto err;
