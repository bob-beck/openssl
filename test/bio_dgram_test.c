...
static int
do_sendmmsg (BIO *b, BIO_MSG *msg, size_t num_msg, uint64_t flags,
             size_t *num_processed)
{
  ...
  for (done = 0; done < num_msg;)
    {
      if (!BIO_sendmmsg (b, msg + done, sizeof (BIO_MSG), num_msg - done,
                         flags, num_processed))
        return 0;
...
static int
do_recvmmsg (BIO *b, BIO_MSG *msg, size_t num_msg, uint64_t flags,
             size_t *num_processed)
{
  size_t done;
  ...
  for (done = 0; done < num_msg;)
    {
      if (!BIO_recvmmsg (b, msg + done, sizeof (BIO_MSG), num_msg - done,
                         flags, num_processed))
        return 0;
...
static int
random_data (const uint32_t *key, uint8_t *data, size_t data_len,
             size_t offset)
{
  int ret = 0, outl;
...
test_bio_dgram_pair (int idx)
{
  ...
  if (!TEST_true (BIO_sendmmsg (bio1, msgs, sizeof (BIO_MSG),
  ...
  rmsgs[1].data_len = 64;
  if (!TEST_true (BIO_recvmmsg (bio2, rmsgs, sizeof (BIO_MSG),
                                OSSL_NELEM (rmsgs), 0, &num_processed))
      || !TEST_size_t_eq (num_processed, 2))
    goto err;
