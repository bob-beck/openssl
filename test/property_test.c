...
static int
add_property_names(const char* n, ...)
{
  va_list args;
  int res = 1;
...
test_property_parse(int n)
{
  ...
  if (TEST_ptr(store = ossl_method_store_new(NULL)) &&
      add_property_names(
        "sky", "groan", "cold", "today", "tomorrow", "n", NULL) &&
...
static int
test_property_query_value_create(void)
{
  OSSL_METHOD_STORE* store;
...
static int
test_property_parse_error(int n)
{
  OSSL_METHOD_STORE* store;
...
test_property_merge(int n)
{
  ...
  if (TEST_ptr(store = ossl_method_store_new(NULL)) &&
      add_property_names(
        "colour", "urn", "clouds", "pot", "day", "night", NULL) &&
...
static int
test_definition_compares(int n)
{
  OSSL_METHOD_STORE* store;
...
test_register_deregister(void)
{
  ...
  for (i = 0; i < OSSL_NELEM(impls); i++)
    if (!TEST_true(ossl_method_store_add(store,
                                         &prov,
                                         impls[i].nid,
                                         impls[i].prop,
                                         impls[i].impl,
...
static int
test_query_cache_stochastic(void)
{
...
static int
test_property_list_to_string(int i)
{
