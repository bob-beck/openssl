...
static int add_property_names(const char* n, ...) {
  ...
  va_start(args, n);
  do {
    if (!TEST_int_ne(ossl_property_name(NULL, n, 1), 0)) res = 0;
...
static int test_property_parse_error(int n) {
  ...
  if (parse_error_tests[n].query) {
    if (!TEST_ptr_null(p = ossl_parse_query(NULL, ps, 1))) goto err;
...
static int test_fips_mode(void) {
  ...
  if (!TEST_ptr(ctx = OSSL_LIB_CTX_new())) goto err;
...
static int test_property_list_to_string(int i) {
  ...
  if (!TEST_size_t_gt(bufsize, 0)) goto err;
