...
static int test_int_lhash(void) {
  ...
  /* num_items */
  if (!TEST_size_t_eq((size_t)lh_int_num_items(h), n_int_tests)) goto end;
  ...
  j = 1;
  if (!TEST_ptr_eq(lh_int_retrieve(h, &j), int_tests + 2)) goto end;

  /* replace */
  j = 13;
  if (!TEST_ptr(p = lh_int_insert(h, &j))) goto end;
  if (!TEST_ptr_eq(p, int_tests + 1)) goto end;
  if (!TEST_ptr_eq(lh_int_retrieve(h, int_tests + 1), &j)) goto end;
  ...
  /* error */
  if (!TEST_int_eq(lh_int_error(h), 0)) goto end;
...
static int test_int_hashtable(void) {
  ...
  /* num_items */
  if (!TEST_size_t_eq(ossl_ht_count(ht), n_int_tests)) goto end;
  ...
  if (!TEST_size_t_eq(list->list_len, n_int_tests)) goto end;
...
static int test_stress(void) {
  ...
  /* num_items */
  if (!TEST_int_eq(lh_int_num_items(h), n)) goto end;
...
static int test_hashtable_stress(int idx) {
  ...
  if (!TEST_size_t_eq(ossl_ht_count(h), n)) goto end;
...
static void do_mt_hash_work(void) {
  ...
  for (iter = 0; iter < TEST_THREAD_ITERATIONS; iter++) {
    if (!TEST_true(CRYPTO_THREAD_write_lock(testrand_lock))) return;
    ...
    switch (behavior) {
      case DO_LOOKUP:
        if (!ossl_ht_read_lock(m_ht)) break;
        ...
        if (worker_exits[num] != NULL) return;
        break;
      case DO_INSERT:
      case DO_REPLACE:
        ...
        if (expected_rc == 1) expected_m->in_table = 1;
      ...
      case DO_DELETE:
        ...
        if (expected_rc != ossl_ht_delete(m_ht, TO_HT_KEY(&key))) {
          TEST_info(
              "Iteration %d Expected rc %d on delete of element %u which is "
              "%s\n",
...
static int test_hashtable_multithread(void) {
  ...
  if (!TEST_ptr(m_ht)) goto end;

  if (!TEST_ptr(worker_lock = CRYPTO_THREAD_lock_new())) goto end_free;
  if (!TEST_ptr(testrand_lock = CRYPTO_THREAD_lock_new())) goto end_free;
#ifdef MEASURE_HASH_PERFORMANCE
  gettimeofday(&start, NULL);
#endif

  for (i = 0; i < NUM_WORKERS; i++) {
    if (!run_thread(&workers[i], do_mt_hash_work)) goto shutdown;
  }
