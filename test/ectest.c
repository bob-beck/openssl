 ...
 *
 * Note that it is possible to retrieve an alternative alias that does not
 * match the original nid.
 *
...
check_named_curve_from_ecparameters (int id)
{
  ...
  /* Ensure that the OPENSSL_EC_EXPLICIT_CURVE ASN1 flag is set. */
  if (!TEST_int_eq (EC_GROUP_get_asn1_flag (tgroup),
...
check_ec_key_field_public_range_test (int id)
{
  ...
  if (!TEST_ptr (x = BN_new ()) || !TEST_ptr (y = BN_new ())
      ...
      || !TEST_int_gt (
          EC_POINT_get_affine_coordinates (group, pub, x, y, NULL), 0))
    goto err;
...
static int
do_test_custom_explicit_fromdata (EC_GROUP *group, BN_CTX *ctx,
                                  unsigned char *gen, size_t gen_size)
{
  ...
  if (!TEST_true (EVP_PKEY_get_octet_string_param (
          pkeyparam, OSSL_PKEY_PARAM_EC_GENERATOR, buf, sizeof (buf),
          &buf_len))
  ...
  if (!TEST_ptr (gettable = EVP_PKEY_gettable_params (pkeyparam))
      ...
      || !TEST_ptr (OSSL_PARAM_locate_const (
          gettable, OSSL_PKEY_PARAM_EC_CHAR2_TP_BASIS))
      || !TEST_ptr (
