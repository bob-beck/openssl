...
prime_field_tests(void)
{
  ...
  len = EC_POINT_point2oct(
    group, Q, POINT_CONVERSION_COMPRESSED, buf, sizeof(buf), ctx);
  ...
  len = EC_POINT_point2oct(
    group, Q, POINT_CONVERSION_UNCOMPRESSED, buf, sizeof(buf), ctx);
  if (!TEST_size_t_ne(len, 0) ||
      !TEST_true(EC_POINT_oct2point(group, P, buf, len, ctx)) ||
      !TEST_int_eq(0, EC_POINT_cmp(group, P, Q, ctx)))
    goto err;
  ...
  len = EC_POINT_point2oct(
    group, Q, POINT_CONVERSION_HYBRID, buf, sizeof(buf), ctx);
  if (!TEST_size_t_ne(len, 0) ||
      !TEST_true(EC_POINT_oct2point(group, P, buf, len, ctx)) ||
      !TEST_int_eq(0, EC_POINT_cmp(group, P, Q, ctx)))
    goto err;
...
static int
hybrid_point_encoding_test(void)
{
  ...
  if (!TEST_true(BN_dec2bn(&x, "0")) || !TEST_true(BN_dec2bn(&y, "1")) ||
      ...
      !TEST_size_t_ne(
        0,
        (len = EC_POINT_point2oct(
           group, point, POINT_CONVERSION_HYBRID, NULL, 0, NULL))) ||
      !TEST_ptr(buf = OPENSSL_malloc(len)) ||
      !TEST_size_t_eq(len,
                      EC_POINT_point2oct(
                        group, point, POINT_CONVERSION_HYBRID, buf, len, NULL)))
    goto err;
...
static int
internal_curve_test_method(int n)
{
...
check_named_curve_test(int id)
{
  ...
  if (!TEST_true(EC_GROUP_set_generator(
        gtest, other_gen, group_order, group_cofactor)) ||
      !TEST_int_eq(EC_GROUP_check_named_curve(gtest, 0, NULL), 0) ||
      !TEST_true(EC_GROUP_set_generator(
        gtest, group_gen, other_order, group_cofactor)) ||
      !TEST_int_eq(EC_GROUP_check_named_curve(gtest, 0, NULL), 0)
      /* The order is not an optional field, so this should fail */
      || !TEST_false(
           EC_GROUP_set_generator(gtest, group_gen, NULL, group_cofactor)) ||
      !TEST_true(EC_GROUP_set_generator(
        gtest, group_gen, group_order, other_cofactor)) ||
      !TEST_int_eq(EC_GROUP_check_named_curve(gtest, 0, NULL), 0)
      ...
      || !TEST_true(EC_GROUP_set_generator(
           gtest, group_gen, group_order, group_cofactor)) ||
      !TEST_int_eq(EC_GROUP_check_named_curve(gtest, 0, NULL), nid))
    goto err;
 ...
 */
static int
check_named_curve_lookup_test(int id)
{
  int ret = 0, nid, rv = 0;
...
are_ec_nids_compatible(int n1d, int n2d)
{
  int ret = 0;
  switch (n1d) {
#ifndef OPENSSL_NO_EC2M
 ...
 */
static int
check_named_curve_from_ecparameters(int id)
{
  int ret = 0, nid, tnid;
  ...
  if (/* Other gen, same group order & cofactor */
      ...
      || !TEST_true(EC_GROUP_set_generator(
           tmpg, group_gen, other_order, group_cofactor)) ||
      !TEST_ptr(other_params = *p_next++ =
                  EC_GROUP_get_ecparameters(tmpg, NULL)) ||
      !TEST_ptr(tgroup = *g_next++ =
                  EC_GROUP_new_from_ecparameters(other_params)) ||
      ...
      || !TEST_true(EC_GROUP_set_generator(
           tmpg, group_gen, group_order, other_cofactor)) ||
      !TEST_ptr(other_params = *p_next++ =
                  EC_GROUP_get_ecparameters(tmpg, NULL)) ||
      !TEST_ptr(tgroup = *g_next++ =
                  EC_GROUP_new_from_ecparameters(other_params)) ||
      ...
      /* check that restoring the generator passes */
      || !TEST_true(EC_GROUP_set_generator(
           tmpg, group_gen, group_order, group_cofactor)) ||
      !TEST_ptr(other_params = *p_next++ =
                  EC_GROUP_get_ecparameters(tmpg, NULL)) ||
      !TEST_ptr(tgroup = *g_next++ =
                  EC_GROUP_new_from_ecparameters(other_params)) ||
...
ossl_parameter_test(void)
{
  ...
  if (EC_GROUP_get0_cofactor(group_nmd) != NULL) {
    if (!TEST_true(OSSL_PARAM_BLD_push_BN(
          bld, OSSL_PKEY_PARAM_EC_COFACTOR, EC_GROUP_get0_cofactor(group_nmd))))
      goto err;
  }

  if (!TEST_ptr(group_gen = EC_GROUP_get0_generator(group_nmd)) ||
      !TEST_size_t_gt(bsize =
                        EC_POINT_point2oct(group_nmd,
...
static int
check_ec_key_field_public_range_test(int id)
{
  int ret = 0, type = 0;
 ...
 */
static ossl_inline int
ec_point_hex2point_test_helper(const EC_GROUP* group,
                               const EC_POINT* P,
 ...
 */
static int
ec_point_hex2point_test(int id)
{
  int ret = 0, nid;
  EC_GROUP* group = NULL;
  const EC_POINT* G = NULL;
...
static int
do_test_custom_explicit_fromdata(EC_GROUP* group,
                                 BN_CTX* ctx,
                                 unsigned char* gen,
                                 size_t gen_size)
{
  ...
  if (EC_GROUP_get0_cofactor(group) != NULL) {
    if (!TEST_true(OSSL_PARAM_BLD_push_BN(
          bld, OSSL_PKEY_PARAM_EC_COFACTOR, EC_GROUP_get0_cofactor(group))))
      goto err;
  }
  ...
  if (!TEST_true(OSSL_PARAM_BLD_push_octet_string(
      ...
      !TEST_true(OSSL_PARAM_BLD_push_BN(
        bld, OSSL_PKEY_PARAM_EC_ORDER, EC_GROUP_get0_order(group))))
    goto err;
  ...
  if (!TEST_true(
      ...
      !TEST_true(EVP_PKEY_get_bn_param(
        pkeyparam, OSSL_PKEY_PARAM_EC_ORDER, &order_out)) ||
  ...
  if (EC_GROUP_get_field_type(group) == NID_X9_62_prime_field) {
    ...
    if (!TEST_false(EVP_PKEY_get_int_param(
        ...
        !TEST_false(
          EVP_PKEY_get_utf8_string_param(pkeyparam,
...
custom_generator_test(int id)
{
  ...
  if (!TEST_ptr(k = BN_CTX_get(ctx))
      ...
      ||
      !TEST_true(BN_rand(
        k, EC_GROUP_order_bits(group) - 1, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ANY))
      /* make k even */
      ...
      !TEST_true(EC_GROUP_set_generator(group,
                                        G2,
...
custom_params_test(int id)
{
  ...
  if (!TEST_true(EC_GROUP_get_curve(group, p, a, b, ctx)) ||
      ...
      !TEST_size_t_eq(
        EC_POINT_point2oct(
          group, G2, POINT_CONVERSION_UNCOMPRESSED, buf1, bsize, ctx),
        bsize) ||
  ...
  if (/* allocate temporary points on group and altgroup */
      ...
      !TEST_size_t_eq(
        EC_POINT_point2oct(
          group, Q1, POINT_CONVERSION_UNCOMPRESSED, buf1, bsize, ctx),
        bsize)
      /* k := k/2 */
      || !TEST_true(BN_rshift1(k, k))
      ...
      !TEST_size_t_eq(
        EC_POINT_point2oct(
          altgroup, Q2, POINT_CONVERSION_UNCOMPRESSED, buf2, bsize, ctx),
        bsize)
      /* Q1 = kG = k/2 G2 = Q2 should hold */
  ...
  if (!TEST_true(EC_POINT_mul(group, Q1, priv1, NULL, NULL, ctx)) ||
      ...
      !TEST_size_t_eq(
        EC_POINT_point2oct(
          group, Q1, POINT_CONVERSION_UNCOMPRESSED, pub1, bsize, ctx),
        bsize))
    goto err;
  ...
  if (!TEST_ptr(Q = EC_KEY_get0_public_key(eckey2)) ||
      ...
      !TEST_size_t_eq(
        EC_POINT_point2oct(
          altgroup, Q, POINT_CONVERSION_UNCOMPRESSED, pub2, bsize, ctx),
        bsize))
    goto err;
