...
static int prime_field_tests(void) {
  ...
  if (!TEST_ptr(scalar3 = BN_new())) goto err;
  BN_zero(scalar3);
  scalars[3] = scalar3;
...
/* Change test based on whether binary point compression is enabled or not. */
  ...
  test_output_bignum("y", y);
  /* G_y value taken from the standard: */
  if (!TEST_true(BN_hex2bn(&z, test->y)) || !TEST_BN_eq(y, z)) goto err;
#else
  /*
   * When (x, y) is on the curve, (x, y + 1) is, as it happens, not,
   * and therefore setting the coordinates should fail.
...
static int hybrid_point_encoding_test(void) {
  ...
  if (!TEST_true(BN_dec2bn(&x, "0")) || !TEST_true(BN_dec2bn(&y, "1")) ||
      ...
      !TEST_size_t_eq(
          len, EC_POINT_point2oct(group, point, POINT_CONVERSION_HYBRID, buf,
                                  len, NULL)))
    goto err;
  ...
  if (!TEST_true(EC_POINT_oct2point(group, point, buf, len, NULL))) r = 0;

  /* Flip the y_bit and verify that the invalid encoding is rejected. */
  buf[0] ^= 1;
  if (!TEST_false(EC_POINT_oct2point(group, point, buf, len, NULL))) r = 0;
...
static int group_field_test(void) {
  ...
  if (BN_cmp(secp521r1_field, EC_GROUP_get0_field(secp521r1_group))) r = 0;

#ifndef OPENSSL_NO_EC2M
  sect163r2_group = EC_GROUP_new_by_curve_name(NID_sect163r2);
  if (BN_cmp(sect163r2_field, EC_GROUP_get0_field(sect163r2_group))) r = 0;
#endif
...
static int nistp_single_test(int idx) {
  ...
  if (!TEST_true(BN_hex2bn(&n, test->d))) goto err;
  /* fixed point multiplication */
  EC_POINT_mul(NISTP, Q, n, NULL, NULL, ctx);
  if (!TEST_int_eq(0, EC_POINT_cmp(NISTP, Q, Q_CHECK, ctx))) goto err;
...
static int check_named_curve_test(int id) {
  ...
  if (field_nid == NID_X9_62_characteristic_two_field) {
    ...
    if (!TEST_ptr(other_p = BN_dup(group_p))) goto err;
    /*
  ...
  ERR_set_mark();
  if (!TEST_int_le(EC_GROUP_check_named_curve(group, 0, NULL), 0)) goto err;
  ERR_pop_to_mark();

  /* Restore curve name and ensure it's passing */
  EC_GROUP_set_curve_name(group, nid);
  if (!TEST_int_eq(EC_GROUP_check_named_curve(group, 0, NULL), nid)) goto err;
  ...
  if (has_seed) {
     ...
     */
    if (!TEST_int_eq(EC_GROUP_check_named_curve(group, 0, NULL), 0)) goto err;
  } else {
    /*
  ...
  if (!TEST_int_eq(EC_GROUP_check_named_curve(gtest, 0, NULL), nid)) goto err;
  ...
  if (EC_GROUP_set_curve(gtest, other_p, group_a, group_b, NULL)) {
    if (!TEST_int_le(EC_GROUP_check_named_curve(gtest, 0, NULL), 0)) goto err;
  } else {
...
static int check_named_curve_lookup_test(int id) {
  ...
  EC_GROUP_free(g);
  if (!TEST_ptr(g = EC_GROUP_new_from_ecparameters(p))) goto err;

  if (!TEST_int_gt(rv = EC_GROUP_check_named_curve(g, 0, NULL), 0)) goto err;
  if (rv != nid) {
    /*
     * Found an alias:
...
static int check_named_curve_from_ecparameters(int id) {
  ...
  if (!TEST_ptr(bn_ctx = BN_CTX_new())) return ret;
  BN_CTX_start(bn_ctx);
...
static int ossl_parameter_test(void) {
  ...
  if (!TEST_ptr(bn_ctx = BN_CTX_new())) goto err;
  ...
  /* test explicit curve */
  if (!TEST_ptr(bld = OSSL_PARAM_BLD_new())) goto err;
...
static int check_ec_key_field_public_range_test(int id) {
  ...
  if (type == NID_X9_62_characteristic_two_field) {
    ...
    if (!TEST_true(BN_GF2m_add(x, x, field))) goto err;
  } else
#endif
      if (type == NID_X9_62_prime_field) {
    /* test for prime curves */
    if (!TEST_true(BN_add(x, x, field))) goto err;
  } else {
 ...
 */
static ossl_inline int ec_point_hex2point_test_helper(
    const EC_GROUP* group, const EC_POINT* P, point_conversion_form_t form,
    BN_CTX* bnctx) {
  int ret = 0;
   ...
   */
  if (Pinf != NULL && !TEST_true(EC_POINT_is_at_infinity(group, Q))) goto err;
...
static int do_test_custom_explicit_fromdata(EC_GROUP* group, BN_CTX* ctx,
  ...
  if (!TEST_ptr(b) || !TEST_ptr(bld = OSSL_PARAM_BLD_new())) goto err;
  ...
  if (EC_GROUP_get_field_type(group) == NID_X9_62_prime_field) {
    ...
    if (EC_GROUP_get_basis_type(group) == NID_X9_62_tpBasis) {
      ...
      if (!TEST_true(EC_GROUP_get_trinomial_basis(group, &k1))) goto err;
    } else {
  ...
  if (!TEST_false(
          EVP_PKEY_get_utf8_string_param(pkeyparam, OSSL_PKEY_PARAM_GROUP_NAME,
...
static int custom_generator_test(int id) {
  ...
  if (!TEST_ptr(group = EC_GROUP_new_by_curve_name(nid))) goto err;
  ...
  if (!TEST_ptr(k = BN_CTX_get(ctx))
      ...
      || !TEST_size_t_eq(
             EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, NULL,
                                0, ctx),
             bsize) ||
      !TEST_ptr(b1 = OPENSSL_malloc(bsize)) ||
      !TEST_size_t_eq(
          EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, b1,
                             bsize, ctx),
          bsize)
      ...
      !TEST_size_t_eq(
          EC_POINT_point2oct(group, Q2, POINT_CONVERSION_UNCOMPRESSED, NULL, 0,
                             ctx),
          bsize) ||
      !TEST_ptr(b2 = OPENSSL_malloc(bsize)) ||
      !TEST_size_t_eq(
          EC_POINT_point2oct(group, Q2, POINT_CONVERSION_UNCOMPRESSED, b2,
                             bsize, ctx),
          bsize)
  ...
  if (!do_test_custom_explicit_fromdata(group, ctx, b1, bsize)) goto err;
...
static int custom_params_test(int id) {
  ...
  if (nid == NID_sm2) return TEST_skip("custom params not supported with SM2");
  ...
  if (!TEST_true(EC_GROUP_get_curve(group, p, a, b, ctx)) ||
      ...
      /* pull out the bytes of that */
      || !TEST_size_t_eq(
             EC_POINT_point2oct(group, G2, POINT_CONVERSION_UNCOMPRESSED, NULL,
                                0, ctx),
             bsize) ||
      !TEST_ptr(buf1 = OPENSSL_malloc(bsize)) ||
      !TEST_size_t_eq(
          EC_POINT_point2oct(group, G2, POINT_CONVERSION_UNCOMPRESSED, buf1,
                             bsize, ctx),
          bsize) ||
  ...
  if (is_prime) {
    if (!TEST_ptr(altgroup = EC_GROUP_new_curve_GFp(p, a, b, ctx))) goto err;
  }
#ifndef OPENSSL_NO_EC2M
  else {
    if (!TEST_ptr(altgroup = EC_GROUP_new_curve_GF2m(p, a, b, ctx))) goto err;
  }
#endif
  ...
  if (/* allocate temporary points on group and altgroup */
      ...
      || !TEST_size_t_eq(
             ...
             bsize) ||
      !TEST_size_t_eq(
          EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, buf1,
                             bsize, ctx),
          bsize)
      /* k := k/2 */
      || !TEST_true(BN_rshift1(k, k))
      ...
      /* pull out the bytes of that */
      || !TEST_size_t_eq(
             EC_POINT_point2oct(altgroup, Q2, POINT_CONVERSION_UNCOMPRESSED,
                                NULL, 0, ctx),
             bsize) ||
      !TEST_ptr(buf2 = OPENSSL_malloc(bsize)) ||
      !TEST_size_t_eq(
          EC_POINT_point2oct(altgroup, Q2, POINT_CONVERSION_UNCOMPRESSED, buf2,
                             bsize, ctx),
          bsize)
      /* Q1 = kG = k/2 G2 = Q2 should hold */
  ...
  if (!TEST_ptr(priv1 = EC_KEY_get0_private_key(eckey1))) goto err;
  ...
  if (!TEST_true(EC_POINT_mul(group, Q1, priv1, NULL, NULL, ctx)) ||
      !TEST_size_t_eq(
          EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, NULL, 0,
                             ctx),
          bsize) ||
      !TEST_ptr(pub1 = OPENSSL_malloc(bsize)) ||
      !TEST_size_t_eq(
          EC_POINT_point2oct(group, Q1, POINT_CONVERSION_UNCOMPRESSED, pub1,
                             bsize, ctx),
          bsize))
    goto err;
  ...
  if (!TEST_ptr(Q = EC_KEY_get0_public_key(eckey2)) ||
      !TEST_size_t_eq(
          EC_POINT_point2oct(altgroup, Q, POINT_CONVERSION_UNCOMPRESSED, NULL,
                             0, ctx),
          bsize) ||
      !TEST_ptr(pub2 = OPENSSL_malloc(bsize)) ||
      !TEST_size_t_eq(
          EC_POINT_point2oct(altgroup, Q, POINT_CONVERSION_UNCOMPRESSED, pub2,
                             bsize, ctx),
          bsize))
    goto err;
...
static int ec_d2i_publickey_test(void) {
  ...
  if (!TEST_ptr(gen_key = EVP_EC_gen("P-256"))) goto err;

  if (!TEST_int_gt(pklen = i2d_PublicKey(gen_key, &pubkey_enc), 0)) goto err;
  ...
  if (!TEST_true(EVP_PKEY_eq(gen_key, decoded_key))) goto err;
  ret = 1;
