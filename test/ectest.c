...
static int group_order_tests(EC_GROUP *group)
{
    ...
    if (!TEST_true(EC_GROUP_get_order(group, order, ctx)) ||
        ...
        !TEST_true(EC_POINT_is_at_infinity(group, Q)) || !TEST_true(EC_POINT_copy(P, G)) || !TEST_true(BN_one(n1)) ||
...
static int prime_field_tests(void)
{
    ...
    if (!TEST_ptr(ctx = BN_CTX_new()) || !TEST_ptr(p = BN_new()) || !TEST_ptr(a = BN_new()) ||
        !TEST_ptr(b = BN_new()) || !TEST_true(BN_hex2bn(&p, "17")) || !TEST_true(BN_hex2bn(&a, "1")) ||
        !TEST_true(BN_hex2bn(&b, "1")) || !TEST_ptr(group = EC_GROUP_new_curve_GFp(p, a, b, ctx)) ||
    ...
    if (!TEST_ptr(P = EC_POINT_new(group)) || !TEST_ptr(Q = EC_POINT_new(group)) ||
        !TEST_ptr(R = EC_POINT_new(group)) || !TEST_true(EC_POINT_set_to_infinity(group, P)) ||
        !TEST_true(EC_POINT_is_at_infinity(group, P)) || !TEST_true(EC_POINT_oct2point(group, Q, buf, 1, ctx)) ||
        !TEST_true(EC_POINT_add(group, P, P, Q, ctx)) || !TEST_true(EC_POINT_is_at_infinity(group, P)) ||
        !TEST_ptr(x = BN_new()) || !TEST_ptr(y = BN_new()) || !TEST_ptr(z = BN_new()) ||
        !TEST_ptr(yplusone = BN_new()) || !TEST_true(BN_hex2bn(&x, "D")) ||
    ...
    if (!TEST_true(BN_hex2bn(&z, "118"
        ...
        !TEST_int_gt(EC_POINT_is_on_curve(group, P, ctx), 0) || !TEST_true(EC_POINT_invert(group, Q, ctx)) /* P = -2Q */
...
static int char2_curve_test(int n)
{
    ...
    if (!TEST_ptr(ctx = BN_CTX_new()) || !TEST_ptr(p = BN_new()) || !TEST_ptr(a = BN_new()) ||
        !TEST_ptr(b = BN_new()) || !TEST_ptr(x = BN_new()) || !TEST_ptr(y = BN_new()) || !TEST_ptr(z = BN_new()) ||
...
static int char2_field_tests(void)
{
    ...
    if (!TEST_ptr(ctx = BN_CTX_new()) || !TEST_ptr(p = BN_new()) || !TEST_ptr(a = BN_new()) ||
        !TEST_ptr(b = BN_new()) || !TEST_true(BN_hex2bn(&p, "13")) || !TEST_true(BN_hex2bn(&a, "3")) ||
...
static int nistp_single_test(int idx)
{
    ...
    if (!TEST_ptr(ctx = BN_CTX_new()) || !TEST_ptr(p = BN_new()) || !TEST_ptr(a = BN_new()) ||
        !TEST_ptr(b = BN_new()) || !TEST_ptr(x = BN_new()) || !TEST_ptr(y = BN_new()) || !TEST_ptr(m = BN_new()) ||
        !TEST_ptr(n = BN_new()) || !TEST_ptr(order = BN_new()) || !TEST_ptr(yplusone = BN_new())
...
static int check_named_curve_test(int id)
{
    ...
    else
    {
         ...
         */
        if (!TEST_ptr(BN_copy(other_p, BN_ucmp(BN_get0_nist_prime_192(), other_p) == 0 ? BN_get0_nist_prime_256()
...
static int ossl_parameter_test(void)
{
    ...
    if (!TEST_ptr(group_gen = EC_GROUP_get0_generator(group_nmd)) ||
        ...
        !TEST_size_t_eq(EC_POINT_point2oct(group_nmd, EC_GROUP_get0_generator(group_nmd), POINT_CONVERSION_UNCOMPRESSED,
                                           buf2, bsize, bn_ctx),
                        bsize) ||
...
static int check_ec_key_field_public_range_test(int id)
{
    ...
    if (!TEST_ptr(x = BN_new()) || !TEST_ptr(y = BN_new()) ||
        !TEST_ptr(key = EC_KEY_new_by_curve_name(curves[id].nid)) || !TEST_ptr(group = EC_KEY_get0_group(key)) ||
        !TEST_ptr(field = EC_GROUP_get0_field(group)) || !TEST_int_gt(EC_KEY_generate_key(key), 0) ||
        !TEST_int_gt(EC_KEY_check_key(key), 0) || !TEST_ptr(pub = EC_KEY_get0_public_key(key)) ||
                                                      ...
                                                      point_conversion_form_t form, BN_CTX *bnctx)
{
    ...
    if (!TEST_ptr(hex = EC_POINT_point2hex(group, P, form, bnctx)) ||
        !TEST_ptr(Q = EC_POINT_hex2point(group, hex, NULL, bnctx)) || !TEST_int_eq(0, EC_POINT_cmp(group, Q, P, bnctx)))
        goto err;
...
static int do_test_custom_explicit_fromdata(EC_GROUP *group, BN_CTX *ctx, unsigned char *gen, size_t gen_size)
{
    ...
    else
    {
        ...
        if (!TEST_true(EVP_PKEY_get_int_param(pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_M, &i_out)) ||
            ...
            !TEST_true(EVP_PKEY_get_utf8_string_param(pkeyparam, OSSL_PKEY_PARAM_EC_CHAR2_TYPE, name, sizeof(name),
                                                      &name_len)) ||
