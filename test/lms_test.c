...
static EVP_PKEY* lms_pubkey_from_data(const unsigned char* data,
                                      size_t datalen) {
  int ret;
...
static int lms_bad_pub_len_test(void) {
  ...
  if (!TEST_size_t_le(td->publen + 16, sizeof(pubdata))) goto end;

  OPENSSL_cleanse(pubdata, sizeof(pubdata));
  memcpy(pubdata, td->pub, td->publen);

  for (publen = 0; publen <= td->publen + 16; publen += 3) {
    if (publen == td->publen) continue;
    if (!TEST_ptr_null(pkey = lms_pubkey_from_data(pubdata, publen))) goto end;
  }
  ret = 1;
end:
  if (ret == 0)
...
static int lms_pubkey_decoder_fail_test(void) {
  ...
  size_t pdatalen;
  static const unsigned char pub_bad_LMSType[] = {0x00, 0x00, 0x00, 0xAA};
  static const unsigned char pub_bad_OTSType[] = {0x00, 0x00, 0x00, 0x01,
                                                  0x00, 0x00, 0x00, 0xAA};
  ...
  pdata = td->pub;
  pdatalen = 3;
  if (!TEST_false(OSSL_DECODER_from_data(dctx, &pdata, &pdatalen))) goto end;
...
static EVP_PKEY* key_decode_from_bio(BIO* bio, const char* keytype) {
  ...
  if (!TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(&pkey, NULL, NULL, keytype,
                                                     selection, libctx, NULL)))
    return NULL;
...
static EVP_PKEY* key_decode_from_data(const unsigned char* data, size_t datalen,
  ...
  if (!TEST_ptr(bio = BIO_new_mem_buf(data, (int)datalen))) return NULL;
...
static int lms_pubkey_decoder_test(void) {
  ...
  if (!TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
                    &pub, "xdr", NULL, "LMS", OSSL_KEYMGMT_SELECT_PUBLIC_KEY,
                    libctx, NULL)))
    goto err;
  data = td->pub;
  data_len = td->publen;
  if (!TEST_true(OSSL_DECODER_from_data(dctx, &data, &data_len))) goto err;
  ret = 1;
err:
  EVP_PKEY_free(pub);
  OSSL_DECODER_CTX_free(dctx);
...
static int lms_key_eq_test(void) {
  ...
  for (i = 0; i < OSSL_NELEM(key); i++) key[i] = NULL;
  ...
  if (!TEST_ptr(key[0] = lms_pubkey_from_data(td1->pub, td1->publen)) ||
      ...
      !TEST_ptr(key[3] = key_decode_from_data(td1->pub, td1->publen, NULL)))
    goto end;
...
static int lms_key_validate_test(void) {
  ...
  if (!TEST_ptr(key = lms_pubkey_from_data(td->pub, td->publen))) return 0;
  if (!TEST_ptr(vctx = EVP_PKEY_CTX_new_from_pkey(libctx, key, NULL))) goto end;
...
static int lms_verify_test(int tst) {
  ...
  ret = TEST_ptr(pkey = lms_pubkey_from_data(td->pub, td->publen)) &&
        ...
        TEST_int_eq(
            EVP_PKEY_verify(ctx, td->sig, td->siglen, td->msg, td->msglen), 1);
...
static int lms_digest_verify_fail_test(void) {
  ...
  if (!TEST_ptr(pub = lms_pubkey_from_data(td->pub, td->publen))) return 0;
  if (!TEST_ptr(vctx = EVP_MD_CTX_new())) goto err;
  /* Only one shot mode is supported, streaming fails to initialise */
  if (!TEST_int_eq(
          EVP_DigestVerifyInit_ex(vctx, NULL, NULL, libctx, NULL, pub, NULL),
          0))
    goto err;
  ret = 1;
err:
...
static int lms_digest_signing_fail_test(void) {
  ...
  if (!TEST_ptr(vctx = EVP_MD_CTX_new())) goto err;
  if (!TEST_int_eq(
          EVP_DigestSignInit_ex(vctx, NULL, NULL, libctx, NULL, pub, NULL), 0))
    goto err;
  ret = 1;
err:
  EVP_PKEY_free(pub);
...
static int lms_verify_fail_test(void) {
  ...
  if (!TEST_ptr(pkey = lms_pubkey_from_data(td->pub, td->publen)) ||
    ...
    goto end;
  /* Only one shot mode is supported, streaming fails to initialise */
  if (!TEST_int_eq(EVP_PKEY_verify_init(ctx), -2)) goto end;
  ret = 1;
end:
  EVP_PKEY_CTX_free(ctx);
  EVP_PKEY_free(pkey);
...
static int lms_verify_bad_sig_test(void) {
  ...
  if (!TEST_int_eq(EVP_PKEY_verify_message_init(ctx, sig, NULL), 1)) goto end;
  ...
  for (i = 0; i < (int)td->siglen; i += step) {
    ...
    if (!TEST_int_eq(
            EVP_PKEY_verify(ctx, sig_data, td->siglen, td->msg, td->msglen),
            0)) {
      TEST_note(
...
static int lms_verify_bad_sig_len_test(void) {
  ...
  if (!TEST_size_t_le(td->siglen + 16, sizeof(sigdata))) goto end;
  ...
  for (siglen = 0; siglen < td->siglen + 16; siglen += step) {
    if (siglen == td->siglen) /* ignore the size that should pass */
      continue;
    if (!TEST_int_eq(EVP_PKEY_verify_message_init(ctx, sig, NULL), 1)) goto end;
    if (!TEST_int_eq(EVP_PKEY_verify(ctx, sigdata, siglen, td->msg, td->msglen),
                     0)) {
      TEST_note(
          "Incorrectly accepted signature key of length"
          " %u (expected %u)",
...
static int lms_verify_bad_pub_sig_test(void) {
  ...
  if (!TEST_ptr(pub = OPENSSL_memdup(td->pub, td->publen))) return 0;

  if (!TEST_ptr(sig = EVP_SIGNATURE_fetch(libctx, "LMS", NULL))) goto end;
  ...
  for (i = 0; i < (int)td->publen; i += step) {
    ...
    if (pkey != NULL) {
      ...
      if ((EVP_PKEY_verify_message_init(ctx, sig, NULL) == 1) &&
          !TEST_int_eq(
              EVP_PKEY_verify(ctx, td->sig, td->siglen, td->msg, td->msglen),
              0)) {
        TEST_note(
