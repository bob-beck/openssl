...
static EVP_PKEY *
key_decode_from_data (const unsigned char *data, size_t datalen,
                      const char *keytype)
{
  BIO *bio;
  EVP_PKEY *key = NULL;
...
lms_key_eq_test (void)
{
  ...
  if (!TEST_ptr (key[0] = lms_pubkey_from_data (td1->pub, td1->publen))
      ...
      || !TEST_ptr (key[3]
                    = key_decode_from_data (td1->pub, td1->publen, NULL)))
    goto end;
...
lms_digest_signing_fail_test (void)
{
  ...
  if (!TEST_ptr (vctx = EVP_MD_CTX_new ()))
    goto err;
  if (!TEST_int_eq (
          EVP_DigestSignInit_ex (vctx, NULL, NULL, libctx, NULL, pub, NULL),
          0))
    goto err;
  ret = 1;
err:
  ...
  for (siglen = 0; siglen < td->siglen + 16; siglen += step)
    {
      ...
      if (!TEST_int_eq (EVP_PKEY_verify_message_init (ctx, sig, NULL), 1))
        goto end;
      if (!TEST_int_eq (
              EVP_PKEY_verify (ctx, sigdata, siglen, td->msg, td->msglen), 0))
        {
  ...
  for (i = 0; i < (int)td->publen; i += step)
    {
      ...
      if (pkey != NULL)
        {
          if (!TEST_ptr (ctx
                         = EVP_PKEY_CTX_new_from_pkey (libctx, pkey, NULL)))
            goto end;
