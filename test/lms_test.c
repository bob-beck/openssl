...
static EVP_PKEY*
lms_pubkey_from_data(const unsigned char* data, size_t datalen)
{
  int ret;
...
static int
lms_pubkey_decoder_fail_test(void)
{
  int ret = 0;
  EVP_PKEY* pkey = NULL;
  ...
  if (!TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
                  &pkey, NULL, NULL, "LMS", selection, libctx, NULL)))
    return 0;
...
static EVP_PKEY*
key_decode_from_bio(BIO* bio, const char* keytype)
{
  EVP_PKEY* pkey = NULL;
  OSSL_DECODER_CTX* dctx = NULL;
  int selection = 0;
  ...
  if (!TEST_ptr(dctx = OSSL_DECODER_CTX_new_for_pkey(
                  &pkey, NULL, NULL, keytype, selection, libctx, NULL)))
    return NULL;
...
static EVP_PKEY*
key_decode_from_data(const unsigned char* data,
                     size_t datalen,
                     const char* keytype)
{
  BIO* bio;
...
lms_pubkey_decoder_test(void)
{
  ...
  if (!TEST_ptr(dctx =
                  OSSL_DECODER_CTX_new_for_pkey(&pub,
                                                "xdr",
                                                NULL,
                                                "LMS",
...
static int
lms_digest_verify_fail_test(void)
{
  int ret = 0;
  LMS_ACVP_TEST_DATA* td = &lms_testdata[0];
  EVP_PKEY* pub = NULL;
  ...
  if (!TEST_int_eq(
        EVP_DigestVerifyInit_ex(vctx, NULL, NULL, libctx, NULL, pub, NULL), 0))
    goto err;
  ret = 1;
err:
  EVP_PKEY_free(pub);
...
static int
lms_digest_signing_fail_test(void)
{
  int ret = 0;
  LMS_ACVP_TEST_DATA* td = &lms_testdata[0];
  EVP_PKEY* pub = NULL;
...
static int
lms_message_signing_fail_test(void)
{
  int ret = 0;
  LMS_ACVP_TEST_DATA* td = &lms_testdata[0];
  EVP_PKEY_CTX* ctx = NULL;
...
lms_verify_bad_sig_test(void)
{
  ...
  for (i = 0; i < (int)td->siglen; i += step) {
    ...
    if (!TEST_int_eq(
          EVP_PKEY_verify(ctx, sig_data, td->siglen, td->msg, td->msglen), 0)) {
 ...
 */
static int
lms_verify_bad_sig_len_test(void)
{
  int ret = 0;
  LMS_ACVP_TEST_DATA* td = &lms_testdata[1];
  EVP_PKEY* pkey = NULL;
...
static int
lms_verify_bad_pub_sig_test(void)
{
  LMS_ACVP_TEST_DATA* td = &lms_testdata[1];
  int ret = 0, i = 0;
  EVP_PKEY* pkey = NULL;
