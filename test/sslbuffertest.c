...
static int
checkbuffers (SSL *s, int isalloced)
{
...
/*
 * Test that attempting to free the buffers at points where they cannot be
 * freed works as expected Test 0: Attempt to free buffers after a full record
 * has been processed, but the application has only performed a partial read
 * Test 1: Attempt to free buffers after only a partial record header has been
 *         received
 * Test 2: Attempt to free buffers after a full record header but no record
 * body Test 3: Attempt to free buffers after a full record hedaer and partial
 * record body Test 4-7: We repeat tests 0-3 but including data from a second
 * pipelined record
 */
static int
test_free_buffers (int test)
{
  int result = 0;
  SSL *serverssl = NULL, *clientssl = NULL;
  const char testdata[] = "Test data";
