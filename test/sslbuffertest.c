...
static int checkbuffers(SSL* s, int isalloced) {
  ...
  if (isalloced) return rrl->rbuf.buf != NULL && wrl->wbuf[0].buf != NULL;
...
static int test_func(int test) {
  ...
  for (j = 0; j < 2; j++) {
    ...
    if (!TEST_size_t_eq(len, sizeof(testdata))) goto end;
    /*
    ...
    if (!TEST_mem_eq(buf, len, testdata, sizeof(testdata))) goto end;
  }
...
static int test_free_buffers(int test) {
  ...
  if (test == 0) {
     ...
     */
    if (pipeline) readlen += strlen(testdata);
    ...
    if (pipeline) {
      ...
      if (test != 3) partial_len += first_rec_len;

      /*
       * Sanity check. If we got the record len right then this should
       * never fail.
       */
      if (!TEST_int_eq(buf[first_rec_len], SSL3_RT_APPLICATION_DATA)) goto end;
    }
     ...
     * the pipelining case)
     */
    if (!TEST_true(BIO_write_ex(tmp, buf, partial_len, &written))) goto end;
   ...
   * still in use
   */
  if (!TEST_false(SSL_free_buffers(serverssl))) goto end;
