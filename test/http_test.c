 ...
 * Response hdr has HTTP version 1.|version| and |keep_alive| (unless
 * implicit).
 */
static int
mock_http_server (BIO *in, BIO *out, char version, int keep_alive,
                  const char *content_type, const char *txt, ASN1_VALUE *rsp,
                  const ASN1_ITEM *it)
{
  const char *req, *path;
...
static long
http_bio_cb_ex (BIO *bio, int oper, const char *argp, size_t len, int cmd,
...
test_http_method (int do_get, int do_txt, int suggested_status)
{
  ...
  rsp = do_get ? OSSL_HTTP_get (
                     real_server ? REAL_SERVER_URL : path, NULL /* proxy */,
                     ...
                     0 /* timeout */)
               : OSSL_HTTP_transfer (
                     NULL, NULL /* host */, NULL /* port */, path,
                     0 /* use_ssl */, NULL /* proxy */, NULL /* no_pr */, wbio,
                     rbio, NULL /* bio_fn */, NULL /* arg */, 0 /* buf_size */,
...
static int
test_http_url_ok (const char *url, int exp_ssl, const char *exp_host,
...
test_http_url_invalid (const char *url)
{
  ...
  res = TEST_false (OSSL_HTTP_parse_url (url, &ssl, NULL, &host, &port, &num,
                                         &path, NULL, NULL))
        && TEST_ptr_null (host) && TEST_ptr_null (port)
        && TEST_ptr_null (path);
  if (!res)
    {
      OPENSSL_free (host);
