...
static int mock_http_server(BIO* in, BIO* out, char version, int keep_alive,
  ...
  if (!TEST_strn_eq(++hdr, "HTTP/1.", len)) return 0;
  hdr += len;
  /* check for HTTP version 1.0 .. 1.1 */
  if (!TEST_char_le('0', *hdr) || !TEST_char_le(*hdr++, '1')) return 0;
  ...
  if (is_get) { /* construct new header and body */
    ...
    if (BIO_printf(out,
                   ...
                   content_type, len) <= 0)
      return 0;
    if (txt != NULL) return BIO_puts(out, txt);
...
static int test_http_method(int do_get, int do_txt, int suggested_status) {
  ...
  if (wbio == NULL || rbio == NULL || req == NULL) goto err;
  ...
  if (!TEST_int_eq(suggested_status == HTTP_STATUS_CODES_FATAL_ERROR,
                   rsp == NULL))
    goto err;
  if (suggested_status == HTTP_STATUS_CODES_FATAL_ERROR) res = 1;
  if (rsp != NULL) {
...
static int test_http_keep_alive(char version, int keep_alive, int kept_alive) {
  ...
  if (wbio == NULL || rbio == NULL) goto err;
  mock_args.out = rbio;
  mock_args.content_type = content_type;
...
static int test_http_url_ok(const char* url, int exp_ssl, const char* exp_host,
  ...
  if (!TEST_int_eq(sscanf(exp_port, "%d", &exp_num), 1)) return 0;
  ...
  if (res && *user != '\0') res = TEST_str_eq(user, "user:pass");
  if (res && *frag != '\0') res = TEST_str_eq(frag, "fr");
  if (res && *query != '\0') res = TEST_str_eq(query, "q");
  OPENSSL_free(user);
  OPENSSL_free(host);
  OPENSSL_free(port);
  OPENSSL_free(path);
...
static int test_http_resp_hdr_limit(size_t limit) {
  ...
  if (TEST_ptr(wbio) == 0 || TEST_ptr(rbio) == 0) goto err;
...
int setup_tests(void) {
  if (!test_skip_common_options()) return 0;

  x509_it = ASN1_ITEM_rptr(X509);
  if (!TEST_ptr((x509 = load_cert_pem(test_get_argument(0), NULL)))) return 0;
