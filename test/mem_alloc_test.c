...
#if USE_CUSTOM_ALLOC_FNS
static void* my_malloc(const size_t num, const char* const file,
                       const int line) {
  ...
  if (cur_custom_counts.malloc < INT_MAX) cur_custom_counts.malloc++;
...
static void* my_realloc(void* const addr, const size_t num,
  ...
  if (cur_custom_counts.realloc < INT_MAX) cur_custom_counts.realloc++;
...
static void my_free(void* const addr, const char* const file, const int line) {
  ...
  if (cur_custom_counts.free < INT_MAX) cur_custom_counts.free++;
  ...
  switch (exp) {
    case EXP_OOM:
      ...
      if (!TEST_ptr_null(ret) || !TEST_int_eq(num_errs, 1) ||
          ...
          !(TEST_int_eq(err_line, start_ln) || TEST_int_eq(err_line, end_ln)) ||
    ...
    case EXP_NONNULL:
      if (!TEST_ptr(ret) || !TEST_int_eq(num_errs, 0)) {
        ...
        if (!check_zero_mem(ret, sz)) test_result = 0;
      }
    ...
    case EXP_ZERO_SIZE:
       ...
       */
      if (!TEST_int_eq(num_errs, 0)) test_result = 0;
...
static int test_xalloc(const bool secure, const bool array, const bool zero,
   ...
   * allocation function.
   */
  if (!really_secure) exp_cnt += !!(exp == EXP_OOM || !IS_FAIL(exp));
...
static int test_xrealloc(const bool clear, const bool array, const bool macro,
  ...
  for (i = 0; i < 2; i++) {
    ...
    if (clear && array && i && exp2 == EXP_INT_OF) exp = EXP_INT_OF;
    ...
    if (exp != EXP_INT_OF) {
      if (clear) {
         ...
         * requested.
         */
        if (ret == NULL || sz > old_sz) exp_malloc_cnt++;
      } else {
        exp_realloc_cnt++;
#if !USE_CUSTOM_ALLOC_FNS
        /* CRYPTO_malloc() is called explicitly when p is NULL. */
        if (ret == NULL) exp_malloc_cnt++;
#endif
      }
    } else {
      if (!macro || OPENSSL_FILE[0] != '\0') exp_malloc_cnt++;
    }
    ...
    if (macro) {
      ...
      if (array) {
        if (clear)
          ret = CRYPTO_clear_realloc_array(ret, old_nmemb, nmemb, td->size,
                                           test_fn, test_line);
        else
    ...
    res = check_exp(macro ? OPENSSL_FILE : test_fn, start_ln, end_ln, sz, false,
                    false, ret, exp, exp_malloc_cnt, exp_realloc_cnt);
    if (res == 0)
      TEST_error(
      ...
      if (i != 0) {
        for (j = 0; j < check_sz; j++) {
          ...
          if (ret[j] != exp_val) {
            if (!num_err)
              TEST_error(
                  "Memory mismatch at byte %zu of %zu: "
                  "%#04hhx != %#04hhx",
        ...
        if (num_err != 0) TEST_error("Total errors: %zu", num_err);
      } else {
        for (j = 0; j < check_sz; j++)
    ...
    if (ret == NULL && exp != EXP_ZERO_SIZE) OPENSSL_free(old_ret);
...
static int test_xaligned_alloc(const bool array, const bool macro,
  ...
  if (macro) {
    if (array) {
      start_ln = OPENSSL_LINE + 1;
      ret =
          OPENSSL_aligned_alloc_array(td->nmemb, td->size, td->align, &freeptr);
      end_ln = OPENSSL_LINE - 1;
    } else {
      start_ln = OPENSSL_LINE + 1;
    ...
    if (array)
      ret = CRYPTO_aligned_alloc_array(td->nmemb, td->size, td->align, &freeptr,
                                       test_fn, test_line);
    else
  ...
  exp_cnt += IS_FAIL(exp) && (!macro || (bool)OPENSSL_FILE[0]);
  res &= check_exp(macro ? OPENSSL_FILE : test_fn, start_ln, end_ln, sz, false,
                   false, ret, exp, exp_cnt, 0);

  /* Check the pointer's alignment */
  if (exp == EXP_NONNULL) {
    if (!TEST_uint64_t_eq((uintptr_t)ret & (td->align - 1), 0)) res = 0;
  }

#if !defined(OPENSSL_SMALL_FOOTPRINT)
  if (IS_FAIL(exp) && !TEST_ptr_null(freeptr)) res = 0;
  if ((exp == EXP_NONNULL) && !TEST_ptr(freeptr)) res = 0;
#else  /* OPENSSL_SMALL_FOOTPRINT */
  if (!TEST_ptr_null(ret) || !TEST_ptr_null(freeptr)) res = 0;
#endif /* !OPENSSL_SMALL_FOOTPRINT */
...
static int test_free(void) {
  ...
  save_counts();
  OPENSSL_free(NULL);
  if (!TEST_int_eq(check_counts(0, 0, 1), 1)) test_result = 0;
  ...
  p = OPENSSL_malloc(42);
  OPENSSL_free(p);
  if (!TEST_int_eq(check_counts(1, 0, 1), 1)) test_result = 0;
