...
static const struct array_realloc_vector
{
    ...
    {SQSQRT_SIZE_T, SIZE_MAX / SQSQRT_SIZE_T + 1, SIZE_MAX / SQSQRT_SIZE_T + 2, EXP_ZERO_SIZE, EXP_NONNULL, EXP_INT_OF,
     EXP_INT_OF},
    {SQSQRT_SIZE_T, SIZE_MAX / SQSQRT_SIZE_T + 2, SIZE_MAX / SQSQRT_SIZE_T + 1, EXP_NONNULL, EXP_ZERO_SIZE, EXP_INT_OF,
     EXP_INT_OF},
 ...
 */
static int check_exp(const char *const fn, const int start_ln, const int end_ln, const size_t sz, const bool secure,
                     const bool zero, char *const ret, const enum exp_ret exp, int exp_mallocs, int exp_reallocs)
...
static int test_xalloc(const bool secure, const bool array, const bool zero, const bool macro,
...
static int test_xrealloc(const bool clear, const bool array, const bool macro, const struct array_realloc_vector *td)
{
    ...
    for (i = 0; i < 2; i++)
    {
        ...
        res = check_exp(macro ? OPENSSL_FILE : test_fn, start_ln, end_ln, sz, false, false, ret, exp, exp_malloc_cnt,
                        exp_realloc_cnt);
        if (res == 0)
