 ...
 */
static int
check_exp (const char *const fn, const int start_ln, const int end_ln,
          ...
          if (array)
            {
              if (clear)
                {
                  start_ln = OPENSSL_LINE + 1;
                  ret = OPENSSL_clear_realloc_array (ret, old_nmemb, nmemb,
                                                     td->size);
                  end_ln = OPENSSL_LINE - 1;
                }
              else
  ...
  if (macro)
    {
      if (array)
        {
          start_ln = OPENSSL_LINE + 1;
          ret = OPENSSL_aligned_alloc_array (td->nmemb, td->size, td->align,
                                             &freeptr);
          end_ln = OPENSSL_LINE - 1;
        }
      else
