...
save_counts(void)
{
  ...
  CRYPTO_get_alloc_counts(
    &mdebug_counts.malloc, &mdebug_counts.realloc, &mdebug_counts.free);
#endif
...

{
  ...
  switch (exp) {
    ...
    case EXP_INT_OF:
      if (!TEST_ptr_null(ret) || !TEST_int_eq(num_errs, 1) ||
          !TEST_ulong_eq(
            err_code, ERR_PACK(ERR_LIB_CRYPTO, 0, CRYPTO_R_INTEGER_OVERFLOW)) ||
          !TEST_str_eq(err_file, fn) ||
          !(TEST_int_eq(err_line, start_ln) || TEST_int_eq(err_line, end_ln)) ||
          !TEST_str_eq(err_func, "") || !TEST_str_eq(err_data, "") ||
          !TEST_int_eq(err_flags, 0))
...
static int
test_xrealloc(const bool clear,
              const bool array,
              const bool macro,
...
static int
test_xaligned_alloc(const bool array,
                    const bool macro,
                    const struct array_aligned_alloc_vector* td)
{
  ...
  if (macro) {
    ...
    if (array)
      ret = CRYPTO_aligned_alloc_array(
        td->nmemb, td->size, td->align, &freeptr, test_fn, test_line);
    else
