...
static const struct array_realloc_vector
{
  ...
  { 0, 0, SIZE_MAX, EXP_ZERO_SIZE, EXP_ZERO_SIZE, EXP_ZERO_SIZE,
    EXP_ZERO_SIZE },
  { 0, 1, 0, EXP_ZERO_SIZE, EXP_ZERO_SIZE, EXP_ZERO_SIZE, EXP_ZERO_SIZE },
  { 0, SIZE_MAX, 0, EXP_ZERO_SIZE, EXP_ZERO_SIZE, EXP_ZERO_SIZE,
    EXP_ZERO_SIZE },
  { 0, 1, SIZE_MAX, EXP_ZERO_SIZE, EXP_ZERO_SIZE, EXP_ZERO_SIZE,
    EXP_ZERO_SIZE },
  { 0, SIZE_MAX, 1, EXP_ZERO_SIZE, EXP_ZERO_SIZE, EXP_ZERO_SIZE,
    EXP_ZERO_SIZE },
  { 0, SIZE_MAX, SIZE_MAX, EXP_ZERO_SIZE, EXP_ZERO_SIZE, EXP_ZERO_SIZE,
    EXP_ZERO_SIZE },
  ...
  { 123, 12, SIZE_MAX / 123 + 12, EXP_NONNULL, EXP_NONNULL, EXP_NONNULL,
    EXP_INT_OF },
  { 123, SIZE_MAX / 123 + 12, 12, EXP_NONNULL, EXP_NONNULL, EXP_INT_OF,
    EXP_NONNULL },
};
...
#if USE_CUSTOM_ALLOC_FNS
static void *
my_malloc (const size_t num, const char *const file, const int line)
{
...
static void *
my_realloc (void *const addr, const size_t num, const char *const file,
            const int line)
{
#if CUSTOM_FN_PRINT_CALLS
  ...
  if (file == test_fn || file == NULL
      || (strcmp (file, OPENSSL_FILE) == 0 && file[0] != '\0'))
    TEST_note ("[%s:%d]: realloc(%#" PRIxPTR ", %#zx) -> %p", file, line,
               old_addr, num, p);
#endif
...
static void
my_free (void *const addr, const char *const file, const int line)
{
#if CUSTOM_FN_PRINT_CALLS
  if (file == test_fn || file == NULL
      || (strcmp (file, OPENSSL_FILE) == 0 && file[0] != '\0'))
...
static bool
check_zero_mem (char *p, size_t sz)
{
...
{
#if !defined(OPENSSL_NO_CRYPTO_MDEBUG)
  CRYPTO_get_alloc_counts (&mdebug_counts.malloc, &mdebug_counts.realloc,
                           &mdebug_counts.free);
#endif
 ...
 */
static bool
check_counts (int exp_mallocs, int exp_reallocs, int exp_frees)
{
  int test_result = 1;
...
static int
check_exp (const char *const fn, const int ln, const size_t sz,
  ...
  switch (exp)
    ...
    case EXP_INVAL:
      if (!TEST_ptr_null (ret) || !TEST_int_eq (num_errs, 1)
          || !TEST_ulong_eq (
              err_code,
              ERR_PACK (ERR_LIB_CRYPTO, 0, ERR_R_PASSED_INVALID_ARGUMENT))
          || !TEST_str_eq (err_file, fn) || !TEST_int_eq (err_line, ln)
          || !TEST_str_eq (err_func, "") || !TEST_str_eq (err_data, "")
          || !TEST_int_eq (err_flags, 0))
        test_result = 0;
...
static int
test_xalloc (const bool secure, const bool array, const bool zero,
      ...
      if (array)
        {
          ret = (secure ? (zero ? CRYPTO_secure_calloc
                        ...
                        : (zero ? CRYPTO_calloc : CRYPTO_malloc_array)) (
              td->nmemb, td->size, test_fn, test_line);
        }
      else
        {
          ret = (secure ? (zero ? CRYPTO_secure_zalloc : CRYPTO_secure_malloc)
                        : (zero ? CRYPTO_zalloc : CRYPTO_malloc)) (sz, test_fn,
                                                                   test_line);
        }
    }
...
static int
test_xrealloc (const bool clear, const bool array, const bool macro,
  ...
  for (i = 0; i < 2; i++)
    {
      ...
      if (macro)
        {
          if (array)
            {
              if (clear)
                ln = OPENSSL_LINE, ret = OPENSSL_clear_realloc_array (
                                       ret, old_nmemb, nmemb, td->size);
              else
                ln = OPENSSL_LINE,
      ...
      else
        {
          if (array)
            {
              if (clear)
                ret = CRYPTO_clear_realloc_array (
                    ret, old_nmemb, nmemb, td->size, test_fn, test_line);
              else
                ret = CRYPTO_realloc_array (ret, nmemb, td->size, test_fn,
                                            test_line);
            }
          else
            {
              if (clear)
                ret = CRYPTO_clear_realloc (ret, old_sz, sz, test_fn,
                                            test_line);
              else
      ...
      if (res != 0 && exp == EXP_NONNULL && exp2 == EXP_NONNULL)
        {
          size_t check_sz
              = MIN (td->orig_nmemb * td->size, td->new_nmemb * td->size);
          size_t j;
          size_t num_err = 0;
...
static int
test_xaligned_alloc (const bool array, const bool macro,
                     const struct array_aligned_alloc_vector *td)
{
  ...
  if (macro)
    {
      if (array)
        {
          ln = OPENSSL_LINE, ret = OPENSSL_aligned_alloc_array (
                                 td->nmemb, td->size, td->align, &freeptr);
        }
      else
        {
          ln = OPENSSL_LINE,
  ...
  else
    {
      if (array)
        ...
        ret = CRYPTO_aligned_alloc (sz, td->align, &freeptr, test_fn,
                                    test_line);
    }
   ...
   */
  exp_cnt += IS_FAIL (exp) && (!macro || (bool)OPENSSL_FILE[0]);
  res &= check_exp (macro ? OPENSSL_FILE : test_fn, ln, sz, false, false, ret,
                    exp, exp_cnt, 0);
...
static int
test_secure_malloc (const int i)
{
...
static int
test_secure_zalloc (const int i)
{
...
static int
test_secure_malloc_array (const int i)
{
...
static int
test_secure_calloc (const int i)
{
...
static int
test_clear_realloc (const int i)
{
...
static int
test_realloc_array (const int i)
{
...
static int
test_clear_realloc_array (const int i)
{
...
static int
test_aligned_alloc (const int i)
{
...
static int
test_aligned_alloc_array (const int i)
{
