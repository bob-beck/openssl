...
static const struct array_realloc_vector {
	...
	{ SQSQRT_SIZE_T, SIZE_MAX / SQSQRT_SIZE_T + 1,
	  SIZE_MAX / SQSQRT_SIZE_T + 2, EXP_ZERO_SIZE, EXP_NONNULL, EXP_INT_OF,
	  EXP_INT_OF },
	{ SQSQRT_SIZE_T, SIZE_MAX / SQSQRT_SIZE_T + 2,
	  SIZE_MAX / SQSQRT_SIZE_T + 1, EXP_NONNULL, EXP_ZERO_SIZE, EXP_INT_OF,
	  EXP_INT_OF },
			...
			const char *const file, const int line)
{
	...
	if (file == test_fn || file == NULL ||
	    (strcmp(file, OPENSSL_FILE) == 0 && file[0] != '\0'))
		TEST_note("[%s:%d]: realloc(%#" PRIxPTR ", %#zx) -> %p", file,
			  line, old_addr, num, p);
#endif
...
static bool check_zero_mem(char *p, size_t sz)
{
	for (size_t i = 0; i < sz; i++) {
		if (p[i] != 0) {
			TEST_error("Non-zero byte %zu of %zu (%#04hhx)", i, sz,
				   p[i]);
...
static bool check_counts(int exp_mallocs, int exp_reallocs, int exp_frees)
{
...
#if !defined(OPENSSL_NO_CRYPTO_MDEBUG)
	{
		...
		if (exp_mallocs >= 0 &&
		    !TEST_int_eq(cur.malloc - mdebug_counts.malloc,
				 exp_mallocs))
			test_result = 0;
		if (exp_reallocs >= 0 &&
		    !TEST_int_eq(cur.realloc - mdebug_counts.realloc,
				 exp_reallocs))
			test_result = 0;
		if (exp_frees >= 0 &&
	...
	if (exp_mallocs >= 0 &&
		...
		test_result = 0;
	if (exp_reallocs >= 0 &&
	    !TEST_int_eq(cur_custom_counts.realloc -
				 saved_custom_counts.realloc,
			 exp_reallocs))
		test_result = 0;
	if (exp_frees >= 0 &&
...

{
	...
	if (num_errs > 0) {
		err_code = ERR_peek_last_error_all(
			&err_file, &err_line, &err_func, &err_data, &err_flags);
	}
	...
	if (macro) {
		...
		if (array) {
			...
			ret = (secure ? (zero ? CRYPTO_secure_zalloc :
					...
					(zero ? CRYPTO_zalloc : CRYPTO_malloc))(
				sz, test_fn, test_line);
		}
	}
	...
	for (i = 0; i < 2; i++) {
		...
		if (macro) {
			...
			if (array) {
				if (clear)
					...
					ret = CRYPTO_realloc_array(ret, nmemb,
								   td->size,
								   test_fn,
								   test_line);
			} else {
				if (clear)
					ret = CRYPTO_clear_realloc(ret, old_sz,
								   sz, test_fn,
								   test_line);
				else
					ret = CRYPTO_realloc(ret, sz, test_fn,
							     test_line);
			}
		}
		...
		if (res != 0 && exp == EXP_NONNULL && exp2 == EXP_NONNULL) {
			...
			if (i != 0) {
				...
				if (num_err != 0)
					TEST_error("Total errors: %zu",
						   num_err);
			} else {
				for (j = 0; j < check_sz; j++)
					ret[j] = (uint8_t)((uintptr_t)td * 253 +
							   j * 17);
			}
		}
			       ...
			       const struct array_aligned_alloc_vector *td)
{
	...
	if (macro) {
		...
		if (array)
			ret = CRYPTO_aligned_alloc_array(td->nmemb, td->size,
							 td->align, &freeptr,
							 test_fn, test_line);
		else
...
{
	return test_xalloc(false, false, false, false,
			   array_alloc_vectors + i) &&
	       test_xalloc(false, false, false, true, array_alloc_vectors + i);
}

static int test_zalloc(const int i)
{
	return test_xalloc(false, false, true, false,
			   array_alloc_vectors + i) &&
...
{
	return test_xalloc(false, true, false, false,
			   array_alloc_vectors + i) &&
...
{
	return test_xalloc(true, false, false, false,
			   array_alloc_vectors + i) &&
...
{
	return test_xaligned_alloc(false, false,
				   array_aligned_alloc_vectors + i) &&
	       test_xaligned_alloc(false, true,
				   array_aligned_alloc_vectors + i);
}

static int test_aligned_alloc_array(const int i)
{
	return test_xaligned_alloc(true, false,
				   array_aligned_alloc_vectors + i) &&
...
int setup_tests(void)
{
	...
	ADD_ALL_TESTS(test_secure_malloc_array,
		      OSSL_NELEM(array_alloc_vectors));
	...
	ADD_ALL_TESTS(test_clear_realloc_array,
		      OSSL_NELEM(array_realloc_vectors));

	ADD_ALL_TESTS(test_aligned_alloc,
