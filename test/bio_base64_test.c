...
static int
memout (BIO *mem, char c, int llen, int *pos)
{
...
static int
memoutws (BIO *mem, char c, unsigned wscnt, unsigned llen, int *pos)
{
 ...
 */
static int
encode (unsigned const char *buf, unsigned buflen, char *encoded, int trunc,
...
static int
genb64 (char *prefix, char *suffix, unsigned const char *buf, unsigned buflen,
        int trunc, char *encoded, unsigned llen, unsigned wscnt, char **out)
{
...
static int
test_bio_base64_run (test_case *t, int llen, int wscnt)
{
  unsigned char *raw;
  unsigned char *out;
  unsigned out_len;
...
static int
generic_case (test_case *t, int verbose)
{
  unsigned *llen;
  unsigned *wscnt;
  int ok = 1;
...
static int
quotrem (int i, unsigned int m, int *q)
{
  *q = i / m;
  return i - *q * m;
}
...
static int
test_bio_base64_generated (int idx)
{
  test_case t;
  int variant;
  int lencase;
...
static int
test_bio_base64_corner_case_bug (int idx)
{
  test_case t;
  int q = idx;
