 ...
 */
static int
get_ulong_via_BN (const OSSL_PARAM *p, unsigned long *goal)
{
  BIGNUM *n = NULL;
...
static int
export_cb (const OSSL_PARAM *params, void *arg)
{
  ...
  for (;;)
    {
      ...
      for (i = 0; i < OSSL_NELEM (expected); i++)
        {
          int rv
              = TEST_uint64_t_eq ((uint64_t)expected[i], (uint64_t)keydata[i]);
...
static int
test_evp_pkey_export_to_provider (int n)
{
