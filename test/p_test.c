...
static int
local_snprintf (char *buf, size_t n, const char *format, ...)
{
  va_list args;
  int ret;
...
static void
p_set_error (int lib, int reason, const char *file, int line, const char *func,
             const char *fmt, ...)
{
  va_list ap;
...
static int
p_get_params (void *provctx, OSSL_PARAM params[])
{
  ...
  for (; ok && p->key != NULL; p++)
    {
      if (strcmp (p->key, "greeting") == 0)
      ...
      else if (strcmp (p->key, "digest-check") == 0)
        {
          ...
          /*
           * "default" has not been loaded into the parent libctx. We should be
           * able to explicitly load it as a non-child provider.
           */
...
p_query (OSSL_PROVIDER *prov, int operation_id, int *no_cache)
{
  *no_cache = 1;
  return NULL;
}
...
int
OSSL_provider_init (const OSSL_CORE_HANDLE *handle, const OSSL_DISPATCH *oin,
