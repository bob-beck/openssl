  ...
  for (; ok && p->key != NULL; p++) {
    if (strcmp(p->key, "greeting") == 0) {
      ...
      if (deflt == NULL || !OSSL_PROVIDER_available(ctx->libctx, "default")) {
        ...
        p_set_error(
          ERR_LIB_PROV, 3, ctx->thisfile, OPENSSL_LINE, ctx->thisfunc, NULL);
        ok = 0;
      }
                   ...
                   void** provctx)
{
  ...
  if (ctx->libctx == NULL) {
    ...
    p_set_error(
      ERR_LIB_PROV, 2, ctx->thisfile, OPENSSL_LINE, ctx->thisfunc, NULL);
    p_teardown(ctx);
    return 0;
  }
  /*
   ...
   */
  p_set_error(
    ERR_LIB_PROV, 1, ctx->thisfile, OPENSSL_LINE, ctx->thisfunc, NULL);
