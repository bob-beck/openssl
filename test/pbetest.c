...
static int test_pkcs5_pbe(const EVP_CIPHER* cipher, const EVP_MD* md,
  ...
  if (!TEST_true(PKCS5_pbe_set0_algor(algor, EVP_CIPHER_nid(cipher), pbe_iter,
      ...
      !TEST_true(PKCS5_PBE_keyivgen(ctx, pbe_password,
                                    (int)strlen(pbe_password), algor->parameter,
                                    cipher, md, 1)) ||
      !TEST_true(
  ...
  if (!TEST_true(EVP_CipherFinal_ex(ctx, out + i, &i))) goto err;
  outlen += i;

  if (!TEST_mem_eq(out, outlen, exp, exp_len)) goto err;
  ...
  if (!TEST_mem_eq(out, outlen, pbe_plaintext, sizeof(pbe_plaintext))) goto err;
