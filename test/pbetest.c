...
static const unsigned char pbe_ciphertext_des_sha1[] = {
  ...
  !defined OPENSSL_NO_DES && !defined OPENSSL_NO_SHA1
static int
test_pkcs5_pbe(const EVP_CIPHER* cipher,
               const EVP_MD* md,
               ...
               const int exp_len)
{
  ...
  if (!TEST_true(PKCS5_pbe_set0_algor(
        algor, EVP_CIPHER_nid(cipher), pbe_iter, pbe_salt, sizeof(pbe_salt))) ||
      !TEST_true(PKCS5_PBE_keyivgen(ctx,
                                    pbe_password,
  ...
  if (!TEST_true(PKCS5_PBE_keyivgen(ctx,
                                    pbe_password,
                                    (int)strlen(pbe_password),
                                    algor->parameter,
                                    cipher,
...
#if !defined OPENSSL_NO_RC4 && !defined OPENSSL_NO_MD5
static int
test_pkcs5_pbe_rc4_md5(void)
{
  return test_pkcs5_pbe(EVP_rc4(),
                        EVP_md5(),
