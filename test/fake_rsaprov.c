...
static int fake_rsa_keymgmt_export(void* keydata, int selection,
  ...
  if (selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) return 0;

  if (!TEST_ptr(params = fake_rsa_key_params(0))) return 0;
...
static void* fake_rsa_keymgmt_load(const void* reference, size_t reference_sz) {
  ...
  if (reference_sz != sizeof(key)) return NULL;
...
static void* fake_rsa_gen_init(void* provctx, int selection,
  ...
  if (!TEST_ptr(gctx = OPENSSL_malloc(1))) return NULL;
...
static void* fake_rsa_gen(void* genctx, OSSL_CALLBACK* osslcb, void* cbarg) {
  ...
  if (!TEST_ptr(keydata = fake_rsa_keymgmt_new(NULL))) return NULL;
...
static int fake_rsa_sig_sign(void* ctx, unsigned char* sig, size_t* siglen,
  ...
  if (!TEST_ptr(sigctx) || !TEST_int_eq(*sigctx, 1)) return 0;

  *siglen = 256;
  /* record that the real sign operation was called */
  if (sig != NULL) {
    if (!TEST_size_t_ge(sigsize, *siglen)) return 0;
    *sigctx = 2;
...
static void* fake_rsa_sig_dupctx(void* ctx) {
  ...
  if ((*sigctx & FAKE_DGSTSGN_NO_DUP) != 0) return NULL;

  if (!TEST_ptr(newctx = OPENSSL_zalloc(1))) return NULL;
...
static int fake_rsa_dgstsgn(void* ctx, unsigned char* sig, size_t* siglen,
                            size_t sigsize, const unsigned char* tbs,
                            size_t tbslen) {
  if (!fake_rsa_dgstsgnvfy_update(ctx, tbs, tbslen)) return 0;
...
static int fake_rsa_dgstvfy(void* ctx, unsigned char* sig, size_t siglen,
  ...
  if (!fake_rsa_dgstsgnvfy_update(ctx, tbv, tbvlen)) return 0;
...
static void* fake_rsa_st_open_ex(void* provctx, const char* uri,
  ...
  if (!TEST_ptr(storectx)) return NULL;
...
static int fake_rsa_st_load(void* loaderctx, OSSL_CALLBACK* object_cb,
  ...
  switch (*storectx) {
    case 0:
      ...
      if (!TEST_ptr(key = fake_rsa_keymgmt_new(NULL))) break;
      if (!TEST_int_gt(fake_rsa_keymgmt_import(key, 0, NULL), 0)) break;
      params[0] =
...
static struct der2key_ctx_st* der2key_newctx(void* provctx,
  ...
  if (ctx != NULL) {
    ctx->provctx = provctx;
    ctx->desc = desc;
    if (desc->evp_type == 0) ctx->desc->evp_type = OBJ_sn2nid(tls_name);
  }
  return ctx;
}
...
static OSSL_FUNC_keymgmt_export_fn* fake_rsa_prov_get_keymgmt_export(
...
static struct fake_rsa_keydata* fake_rsa_key_from_pkcs8(
    const PKCS8_PRIV_KEY_INFO* p8inf, OSSL_LIB_CTX* libctx, const char* propq) {
  struct fake_rsa_keydata* key = fake_rsa_keymgmt_new(NULL);

  if (key) key->status = FAKE_RSA_STATUS_DECODED;
  return key;
}
...
static int fake_rsa_provider_init(const OSSL_CORE_HANDLE* handle,
  ...
  if (!TEST_ptr(libctx = OSSL_LIB_CTX_new_from_dispatch(handle, in))) return 0;
