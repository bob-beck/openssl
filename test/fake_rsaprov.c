...
static int fake_rsa_keymgmt_import(void* keydata,
                                   int selection,
                                   const OSSL_PARAM* p) {
...
static int fake_rsa_keymgmt_export(void* keydata,
                                   int selection,
...
static void* fake_rsa_gen_init(void* provctx,
                               int selection,
...
static int fake_rsa_sig_sign_init(void* ctx,
                                  void* provkey,
                                  const OSSL_PARAM params[]) {
...
static int fake_rsa_sig_sign(void* ctx,
                             unsigned char* sig,
                             size_t* siglen,
                             size_t sigsize,
...
static int fake_rsa_dgstsgnvfy_init(void* ctx,
                                    unsigned char type,
                                    void* provkey,
                                    const OSSL_PARAM params[]) {
  unsigned char* sigctx = ctx;
...
static int fake_rsa_dgstsgn_init(void* ctx,
                                 const char* mdname,
                                 void* provkey,
                                 const OSSL_PARAM params[]) {
...
static int fake_rsa_dgstvfy_init(void* ctx,
                                 const char* mdname,
                                 void* provkey,
                                 const OSSL_PARAM params[]) {
...
static int fake_rsa_dgstsgnvfy_update(void* ctx,
...
static int fake_rsa_dgstsgnvfy_final(void* ctx,
                                     unsigned char* sig,
                                     size_t* siglen,
                                     size_t sigsize) {
  unsigned char* sigctx = ctx;
...
static int fake_rsa_dgstvfy_final(void* ctx,
                                  unsigned char* sig,
                                  size_t siglen) {
  return fake_rsa_dgstsgnvfy_final(ctx, sig, NULL, siglen);
}

static int fake_rsa_dgstsgn(void* ctx,
                            unsigned char* sig,
                            size_t* siglen,
                            size_t sigsize,
                            const unsigned char* tbs,
...
static int fake_rsa_dgstvfy(void* ctx,
                            unsigned char* sig,
                            size_t siglen,
...
static void* fake_rsa_st_open_ex(void* provctx,
                                 const char* uri,
                                 const OSSL_PARAM params[],
...
ASN1_SEQUENCE(X509_PUBKEY_INTERNAL) =
    {ASN1_SIMPLE(X509_PUBKEY, algor, X509_ALGOR),
     ASN1_SIMPLE(
        ...
        static X509_PUBKEY
    * fake_rsa_d2i_X509_PUBKEY_INTERNAL(const unsigned char** pp,
                                        long len,
                                        OSSL_LIB_CTX* libctx) {
...
static int fake_rsa_read_der(PROV_FAKE_RSA_CTX* provctx,
                             OSSL_CORE_BIO* cin,
                             unsigned char** data,
                             long* len) {
  BUF_MEM* mem = NULL;
...
static int fake_rsa_der2key_decode(void* vctx,
                                   OSSL_CORE_BIO* cin,
                                   int selection,
...
static void* fake_rsa_d2i_PKCS8(void** key,
