...
void
fake_rsa_restore_store_state (void)
{
  key_deleted = 0;
}

static void *
fake_rsa_keymgmt_new (void *provctx)
{
...
static void
fake_rsa_keymgmt_free (void *keydata)
{
  OPENSSL_free (keydata);
}

static int
fake_rsa_keymgmt_has (const void *key, int selection)
{
...
static int
fake_rsa_keymgmt_import (void *keydata, int selection, const OSSL_PARAM *p)
{
...
static int
fake_rsa_keymgmt_export (void *keydata, int selection,
...
fake_rsa_keymgmt_imptypes (int selection)
{
  /* record global for checking */
...
static const OSSL_PARAM *
fake_rsa_keymgmt_exptypes (int selection)
{
  /* record global for checking */
...
static void *
fake_rsa_keymgmt_load (const void *reference, size_t reference_sz)
{
...
static void *
fake_rsa_gen_init (void *provctx, int selection, const OSSL_PARAM params[])
{
...
static void *
fake_rsa_gen (void *genctx, OSSL_CALLBACK *osslcb, void *cbarg)
{
...
static void
fake_rsa_gen_cleanup (void *genctx)
{
  OPENSSL_free (genctx);
}
...
static void *
fake_rsa_sig_newctx (void *provctx, const char *propq)
{
...
static void
fake_rsa_sig_freectx (void *sigctx)
{
  OPENSSL_free (sigctx);
}

static int
fake_rsa_sig_sign_init (void *ctx, void *provkey, const OSSL_PARAM params[])
{
...
static int
fake_rsa_sig_sign (void *ctx, unsigned char *sig, size_t *siglen,
...
static int
fake_rsa_dgstsgnvfy_init (void *ctx, unsigned char type, void *provkey,
...
static int
fake_rsa_dgstsgn_init (void *ctx, const char *mdname, void *provkey,
                       const OSSL_PARAM params[])
{
...
static int
fake_rsa_dgstvfy_init (void *ctx, const char *mdname, void *provkey,
                       const OSSL_PARAM params[])
{
...
static int
fake_rsa_dgstsgnvfy_update (void *ctx, const unsigned char *data,
                            size_t datalen)
{
  unsigned char *sigctx = ctx;
...
static int
fake_rsa_dgstsgnvfy_final (void *ctx, unsigned char *sig, size_t *siglen,
                           size_t sigsize)
{
  unsigned char *sigctx = ctx;
...
static int
fake_rsa_dgstvfy_final (void *ctx, unsigned char *sig, size_t siglen)
{
  return fake_rsa_dgstsgnvfy_final (ctx, sig, NULL, siglen);
}

static int
fake_rsa_dgstsgn (void *ctx, unsigned char *sig, size_t *siglen,
                  size_t sigsize, const unsigned char *tbs, size_t tbslen)
{
...
static int
fake_rsa_dgstvfy (void *ctx, unsigned char *sig, size_t siglen,
...
static void *
fake_rsa_st_open_ex (void *provctx, const char *uri, const OSSL_PARAM params[],
...
static void *
fake_rsa_st_open (void *provctx, const char *uri)
{
  unsigned char *storectx = NULL;
...
static const OSSL_PARAM *
fake_rsa_st_settable_ctx_params (void *provctx)
{
...
static int
fake_rsa_st_set_ctx_params (void *loaderctx, const OSSL_PARAM params[])
{
  return 1;
}

static int
fake_rsa_st_load (void *loaderctx, OSSL_CALLBACK *object_cb,
...
static int
fake_rsa_st_delete (void *loaderctx, const char *uri,
                    const OSSL_PARAM params[], OSSL_PASSPHRASE_CALLBACK *pw_cb,
                    void *pw_cbarg)
{
  key_deleted = 1;
  return 1;
...
static int
fake_rsa_st_eof (void *loaderctx)
{
  unsigned char *storectx = loaderctx;
...
static int
fake_rsa_st_close (void *loaderctx)
{
...
ASN1_SEQUENCE (X509_PUBKEY_INTERNAL)
    = { ASN1_SIMPLE (X509_PUBKEY, algor, X509_ALGOR),
        ASN1_SIMPLE (
          ...
          static X509_PUBKEY
      * fake_rsa_d2i_X509_PUBKEY_INTERNAL (const unsigned char **pp, long len,
                                           OSSL_LIB_CTX *libctx)
{
...
static int
fake_rsa_read_der (PROV_FAKE_RSA_CTX *provctx, OSSL_CORE_BIO *cin,
...
static void *
fake_rsa_der2key_decode_p8 (const unsigned char **input_der,
...
fake_rsa_d2i_PUBKEY (struct fake_rsa_keydata **a, const unsigned char **pp,
                     long length)
{
  struct fake_rsa_keydata *key = NULL;
  X509_PUBKEY *xpk;
...
static int
fake_rsa_der2key_decode (void *vctx, OSSL_CORE_BIO *cin, int selection,
...
static struct fake_rsa_keydata *
fake_rsa_key_from_pkcs8 (const PKCS8_PRIV_KEY_INFO *p8inf,
                         OSSL_LIB_CTX *libctx, const char *propq)
{
...
static void *
fake_rsa_d2i_PKCS8 (void **key, const unsigned char **der, long der_len,
...
static void
fake_rsa_key_adjust (void *key, struct der2key_ctx_st *ctx)
{
}
...
fake_rsa_query (void *provctx, int operation_id, int *no_cache)
{
  *no_cache = 0;
  switch (operation_id)
    {
...
static void
fake_rsa_prov_teardown (void *provctx)
{
...
static int
fake_rsa_provider_init (const OSSL_CORE_HANDLE *handle,
...
OSSL_PROVIDER *
fake_rsa_start (OSSL_LIB_CTX *libctx)
{
  OSSL_PROVIDER *p;
...
void
fake_rsa_finish (OSSL_PROVIDER *p)
{
