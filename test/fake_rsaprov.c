...
fake_rsa_key_params(int priv)
{
  if (priv) {
    OSSL_PARAM params[] = {
      ...
      OSSL_PARAM_BN(
        OSSL_PKEY_PARAM_RSA_FACTOR1, fake_rsa_p, sizeof(fake_rsa_p) - 1),
      OSSL_PARAM_BN(
        OSSL_PKEY_PARAM_RSA_FACTOR2, fake_rsa_q, sizeof(fake_rsa_q) - 1),
...
static int
fake_rsa_keymgmt_export(void* keydata,
                        int selection,
...
static int
fake_rsa_dgstsgnvfy_init(void* ctx,
                         unsigned char type,
                         void* provkey,
...
static int
fake_rsa_dgstsgn_init(void* ctx,
                      const char* mdname,
                      void* provkey,
                      const OSSL_PARAM params[])
{
...
static int
fake_rsa_dgstvfy_init(void* ctx,
                      const char* mdname,
                      void* provkey,
                      const OSSL_PARAM params[])
{
...
static int
fake_rsa_dgstsgnvfy_update(void* ctx, const unsigned char* data, size_t datalen)
{
  unsigned char* sigctx = ctx;
...
static int
fake_rsa_dgstsgnvfy_final(void* ctx,
                          unsigned char* sig,
                          size_t* siglen,
                          size_t sigsize)
{
...
static int
fake_rsa_dgstvfy(void* ctx,
                 unsigned char* sig,
                 size_t siglen,
                 const unsigned char* tbv,
                 size_t tbvlen)
{
...
static void*
fake_rsa_st_open_ex(void* provctx,
                    const char* uri,
                    const OSSL_PARAM params[],
...
static int
fake_rsa_st_load(void* loaderctx,
...
static int
fake_rsa_st_delete(void* loaderctx,
                   const char* uri,
                   const OSSL_PARAM params[],
                   OSSL_PASSPHRASE_CALLBACK* pw_cb,
                   void* pw_cbarg)
...
static int
fake_rsa_read_der(PROV_FAKE_RSA_CTX* provctx,
                  OSSL_CORE_BIO* cin,
                  unsigned char** data,
                  long* len)
{
...
fake_rsa_d2i_PUBKEY(struct fake_rsa_keydata** a,
                    const unsigned char** pp,
                    long length)
{
  struct fake_rsa_keydata* key = NULL;
...
static int
fake_rsa_der2key_decode(void* vctx,
                        OSSL_CORE_BIO* cin,
                        int selection,
...
#define MAKE_DECODER(keytype_name, keytype, type, kind)                        \
  ...
  static const OSSL_DISPATCH                                                   \
    fake_rsa_##kind##_der_to_##keytype##_decoder_functions[] = {               \
      ...
      { OSSL_FUNC_DECODER_DECODE, (void (*)(void))fake_rsa_der2key_decode },   \
...
fake_rsa_start(OSSL_LIB_CTX* libctx)
{
  ...
  if (!TEST_true(OSSL_PROVIDER_add_builtin(
        libctx, "fake-rsa", fake_rsa_provider_init)) ||
