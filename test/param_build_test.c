...
static int template_public_single_zero_test(int idx) {
  ...
  res = 1;
err:
  if (params != params_blt) OPENSSL_free(params);
...
err:
  OSSL_PARAM_free(p1);
  if (params != params_blt) OSSL_PARAM_free(params);
  OSSL_PARAM_free(params_blt);
  OSSL_PARAM_BLD_free(bld);
...
static int builder_limit_test(void) {
  ...
  if (!TEST_ptr(bld)) goto err;

  for (i = 0; i < n; i++) {
    names[i][0] = 'A' + (i / 26) - 1;
    names[i][1] = 'a' + (i % 26) - 1;
    names[i][2] = '\0';
    if (!TEST_true(OSSL_PARAM_BLD_push_int(bld, names[i], 3 * i + 1))) goto err;
  }
  if (!TEST_ptr(params = OSSL_PARAM_BLD_to_param(bld))) goto err;
  /* Count the elements in the params array, expecting n */
  for (i = 0; params[i].key != NULL; i++);
  if (!TEST_int_eq(i, n)) goto err;
...
static int builder_merge_test(void) {
  ...
  if (!TEST_ptr(params = OSSL_PARAM_merge(params_blt, params2_blt))) goto err;
