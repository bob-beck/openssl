...
static int execute_certrep_create(CMP_MSG_TEST_FIXTURE* fixture) {
  ...
  if (crepmsg == NULL || cresp == NULL) goto err;
  if (!ASN1_INTEGER_set(cresp->certReqId, 99)) goto err;
  ...
  if (!TEST_ptr(read_cresp)) goto err;
...
static int execute_rp_create(CMP_MSG_TEST_FIXTURE* fixture) {
  ...
  if (si == NULL || issuer == NULL || serial == NULL) goto err;
