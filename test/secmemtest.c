...
static int test_sec_mem(void) {
  ...
  /* s = non-secure 20 */
  if (!TEST_ptr(s) || !TEST_false(CRYPTO_secure_allocated(s))) goto end;
  ...
  if (!TEST_ptr(p)
    ...
    goto end;
  q = OPENSSL_malloc(20);
  if (!TEST_ptr(q)) goto end;
  /* r = non-secure 20, p = secure 20, q = non-secure 20, s = non-secure 20 */
  if (!TEST_false(CRYPTO_secure_allocated(q))) goto end;
  ...
  if (!TEST_size_t_eq(CRYPTO_secure_used(), 32)) goto end;
  OPENSSL_free(q);
  q = NULL;
  ...
  if (!TEST_true(CRYPTO_secure_malloc_init(32768, 16))) goto end;
...
static int test_sec_mem_clear(void) {
  ...
  for (i = 0; i < size; i++)
    if (!TEST_uchar_eq(p[i], 0)) goto err;
  ...
  for (i = sizeof(void*) * 2; i < size; i++)
    if (!TEST_uchar_eq(p[i], 0)) return 0;
