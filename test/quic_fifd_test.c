...
static void (*regen_frame_p)(uint64_t frame_type,
                             uint64_t stream_id,
                             QUIC_TXPIM_PKT* pkt,
                             void* arg);

static void
regen_frame(uint64_t frame_type,
            uint64_t stream_id,
            QUIC_TXPIM_PKT* pkt,
            void* arg)
{
...
static void
confirm_frame(uint64_t frame_type,
              uint64_t stream_id,
              QUIC_TXPIM_PKT* pkt,
              void* arg)
{
...
static void
regen_expect(uint64_t frame_type,
             uint64_t stream_id,
             QUIC_TXPIM_PKT* pkt,
             void* arg)
{
...
test_generic(INFO* info, int kind)
{
  ...
  if (!TEST_ptr(cfq_item =
                  ossl_quic_cfq_add_frame(info->cfq,
                                          10,
                                          pn_space,
...
test_fifd(int idx)
{
  ...
  if (!TEST_true(ossl_statm_init(&info.statm)) ||
      ...
      !TEST_ptr(info.ackm = ossl_ackm_new(fake_now,
                                          NULL,
                                          &info.statm,
