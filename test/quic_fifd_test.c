...
static int test_generic(INFO* info, int kind) {
  ...
  if (!TEST_ptr(pkt = ossl_quic_txpim_pkt_alloc(info->txpim))) goto err;
  ...
  for (i = 0; i < 2; ++i) {
    ...
    if (i == 1) ossl_quic_sstream_fin(info->sstream[i]);
    ...
    if (!TEST_true(ossl_quic_txpim_pkt_append_chunk(pkt, &chunk))) goto err;
  }

  cfq_freed = 0;
  if (!TEST_ptr(cfq_item = ossl_quic_cfq_add_frame(
                    info->cfq, 10, pn_space, OSSL_QUIC_FRAME_TYPE_NEW_CONN_ID,
                    0, placeholder_data, sizeof(placeholder_data), cfq_free_cb_,
                    NULL)) ||
  ...
  if (!TEST_true(ossl_quic_fifd_pkt_commit(&info->fifd, pkt))) goto err;
  ...
  switch (kind) {
    case TEST_KIND_ACK:
      ...
      if (!TEST_true(cfq_freed)) goto err;

      /* No regen calls should have been made */
      if (!TEST_size_t_eq(regen_count, 0)) goto err;

      break;

    case TEST_KIND_LOSS:
      /* Trigger loss detection via packet threshold. */
      if (!TEST_ptr(pkt2 = ossl_quic_txpim_pkt_alloc(info->txpim))) goto err;
  ...
  if (!TEST_size_t_eq(ossl_quic_txpim_get_in_use(info->txpim), 0)) goto err;
...
static int test_fifd(int idx) {
  ...
  if (!TEST_true(ossl_statm_init(&info.statm)) ||
      ...
      !TEST_ptr(info.ackm = ossl_ackm_new(fake_now, NULL, &info.statm,
  ...
  for (i = 0; i < OSSL_NELEM(info.sstream); ++i)
    if (!TEST_ptr(info.sstream[i] = ossl_quic_sstream_new(1024))) goto err;

  ossl_statm_update_rtt(&info.statm, ossl_time_zero(), ossl_ms2time(1));

  if (!TEST_true(test_generic(&info, idx)) || !TEST_false(cb_fail)) goto err;
...
err:
  ...
  if (info.ccdata != NULL) ossl_cc_dummy_method.free(info.ccdata);
  for (i = 0; i < OSSL_NELEM(info.sstream); ++i)
