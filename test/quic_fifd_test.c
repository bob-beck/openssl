...
static void
regen_frame (uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt,
             void *arg)
{
...
static void
confirm_frame (uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt,
               void *arg)
{
}

static void
sstream_updated (uint64_t stream_id, void *arg)
{
}
 ...
 */
static QUIC_SSTREAM *
sstream_expect (uint64_t stream_id, uint32_t pn_space, void *arg)
{
...
static void
regen_expect (uint64_t frame_type, uint64_t stream_id, QUIC_TXPIM_PKT *pkt,
              void *arg)
{
...
static void
cfq_free_cb_ (unsigned char *buf, size_t buf_len, void *arg)
{
...
static int
test_generic (INFO *info, int kind)
{
  int testresult = 0;
  size_t i, consumed = 0;
  ...
  for (i = 0; i < 2; ++i)
    {
      ...
      if (!TEST_true (ossl_quic_sstream_append (
              info->sstream[i], (unsigned char *)"Test message", 12,
              &consumed))
...
test_fifd (int idx)
{
  ...
  if (!TEST_true (ossl_statm_init (&info.statm))
      ...
      || !TEST_true (ossl_quic_fifd_init (
          &info.fifd, info.cfq, info.ackm, info.txpim, get_sstream_by_id, NULL,
          regen_frame, NULL, confirm_frame, NULL, sstream_updated, NULL, NULL,
          NULL)))
    goto err;
