...
test_client_auth(int idx)
{
  ...
  if (idx > 0 && (!TEST_true(SSL_CTX_use_certificate_chain_file(cctx, ccert)) ||
                  !TEST_true(SSL_CTX_use_PrivateKey_file(
                    cctx, cprivkey, SSL_FILETYPE_PEM))))
    goto err;
  ...
  ossl_quic_tserver_tick(qtserv);
  if (!TEST_true(ossl_quic_tserver_write(
        qtserv, 0, (unsigned char*)msg, msglen, &numbytes)))
    goto err;
 ...
 */
static int
unreliable_client_read(SSL* clientquic,
                       SSL** stream,
                       void* buf,
                       size_t buflen,
                       size_t* readbytes,
...
static int
unreliable_server_read(QUIC_TSERVER* qtserv,
                       uint64_t sid,
                       void* buf,
                       size_t buflen,
                       size_t* readbytes,
...
test_noisy_dgram(int idx)
{
  ...
  if (!TEST_ptr(cctx) || !TEST_true(qtest_create_quic_objects(libctx,
                                                              cctx,
                                                              NULL,
                                                              cert,
                                                              privkey,
  ...
  for (j = 0; j < 2; j++) {
    ...
    for (i = 0; i < 20; i++) {
      ...
      if (!TEST_true(unreliable_client_read(
            clientquic, &stream[j], buf, sizeof(buf), &readbytes, qtserv)) ||
          !TEST_mem_eq(msg, msglen, buf, readbytes))
        goto err;
    }
...
static int
tparam_handle(struct tparam_ctx* ctx,
              uint64_t id,
              unsigned char* data,
              size_t data_len,
              WPACKET* wpkt)
...
static int
tparam_on_enc_ext(QTEST_FAULT* qtf,
                  ...
                  void* arg)
{
   ...
   */
  if (!TEST_true(
        qtest_fault_delete_extension(qtf,
  ...
  if (!TEST_true(PACKET_buf_init(
        &pkt, (unsigned char*)old_bufm->data, old_bufm->length)) ||
            ...
            void* arg)
{
  ...
  if (SSL_select_next_proto(
        (unsigned char**)out, out_len, alpn, sizeof(alpn), in, in_len) ==
...
create_addr(struct in_addr* ina, short int port)
{
  ...
  if (!TEST_true(BIO_ADDR_rawmake(
        addr, AF_INET, ina, sizeof(struct in_addr), htons(port)))) {
    BIO_ADDR_free(addr);
    return NULL;
  }
 ...
 */
static int
create_accept_stream(SSL* serverssl,
                     SSL* clientssl,
...
test_accept_stream(void)
{
  ...
  for (j = 0; j < sizeof(accept_flags) / sizeof(accept_flags[0]); j++) {
    if (!TEST_int_eq(
          create_accept_stream(serverssl, clientssl, accept_flags[j], 0), 1) ||
        !TEST_int_eq(
          create_accept_stream(serverssl, clientssl, accept_flags[j], 1), 1) ||
        !TEST_int_eq(
