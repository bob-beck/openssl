...
/*
 * Test that sending FIN with no data to a client blocking in SSL_read_ex()
 * will wake up the client.
 */
static int
test_fin_only_blocking (void)
{
  SSL_CTX *cctx = SSL_CTX_new_ex (libctx, NULL, OSSL_QUIC_client_method ());
  SSL_CTX *sctx = NULL;
  SSL *clientquic = NULL;
  ...
  if (!TEST_true (ossl_quic_tserver_stream_new (qtserv, 0, &sid))
      || !TEST_true (ossl_quic_tserver_write (
          qtserv, sid, (unsigned char *)msg, strlen (msg), &numbytes))
...
static int
find_new_token_data (BIO *membio)
{
  char buf[1024];
  int state = INITIAL;
  char *tmpstring;
...
static int
ensure_valid_ciphers (const STACK_OF (SSL_CIPHER) *ciphers)
{
  int i;
 ...
 */
static int
test_quic_forbidden_apis_ctx (void)
{
  int testresult = 0;
  SSL_CTX *ctx = NULL;
...
static int
test_quic_forbidden_options (void)
{
  int testresult = 0;
  SSL_CTX *ctx = NULL;
  SSL *ssl = NULL;
...
static void
dgram_cb (int write_p, int version, int content_type, const void *buf,
...
static int
non_io_retry_cert_verify_cb (X509_STORE_CTX *ctx, void *arg)
{
 ...
 */
static int
unreliable_client_read (SSL *clientquic, SSL **stream, void *buf,
                        size_t buflen, size_t *readbytes, QUIC_TSERVER *qtserv)
{
  int abortctr;
...
static int
unreliable_server_read (QUIC_TSERVER *qtserv, uint64_t sid, void *buf,
                        size_t buflen, size_t *readbytes, SSL *clientquic)
{
  int abortctr;
...
/*
 * Create a connection and send data using an unreliable transport. We
 * introduce random noise to drop, delay and duplicate datagrams. Test 0:
 * Introduce random noise to datagrams Test 1: As with test 0 but also split
 * datagrams containing multiple packets into individual datagrams so that
 * individual packets can be affected by noise - not just a whole datagram.
 */
static int
test_noisy_dgram (int idx)
{
  SSL_CTX *cctx = SSL_CTX_new_ex (libctx, NULL, OSSL_QUIC_client_method ());
      ...
      for (i = 0; i < 20; i++)
        {
          ...
          if (!TEST_true (unreliable_server_read (
                  qtserv, sid, buf, sizeof (buf), &readbytes, clientquic))
              || !TEST_mem_eq (msg, msglen, buf, readbytes))
            goto err;
        }
    }
...
static const struct tparam_test tparam_tests[]
    = { TPARAM_CHECK_DUP (
        ORIG_DCID,
        "ORIG_DCID appears multiple times") TPARAM_CHECK_DUP (INITIAL_SCID,
                                                              "INITIAL_SCID "
                                                              "appears "
                                                              "multiple times")
            TPARAM_CHECK_DUP (INITIAL_MAX_DATA, "INITIAL_MAX_DATA appears "
                                                "multiple times") TPARAM_CHECK_DUP (
                ...
                TPARAM_CHECK_DUP (
                    ...
                    "times") TPARAM_CHECK_DUP (INITIAL_MAX_STREAM_DATA_UNI,
                                               "INITIAL_MAX_STREAM_DATA_UNI "
                                               "appears multiple times")
                    TPARAM_CHECK_DUP (INITIAL_MAX_STREAMS_BIDI, "INITIAL_MAX_"
                                                                "STREAMS_BIDI "
                                                                "appears "
                                                                "multiple "
                                                                "times") TPARAM_CHECK_DUP (
                        INITIAL_MAX_STREAMS_UNI,
                        "INITIAL_MAX_STREAMS_UNI appears multiple times")
                        TPARAM_CHECK_DUP (MAX_IDLE_TIMEOUT, "MAX_IDLE_TIMEOUT "
                                                            "appears multiple "
                                                            "times") TPARAM_CHECK_DUP (
                            MAX_UDP_PAYLOAD_SIZE,
                            "MAX_UDP_PAYLOAD_SIZE appears multiple times")
                            TPARAM_CHECK_DUP (ACTIVE_CONN_ID_LIMIT, "ACTIVE_"
                                                                    "CONN_ID_"
                                                                    "LIMIT "
                                                                    "appears "
                                                                    "multiple "
                                ...
                                TPARAM_CHECK_DROP (
                                    INITIAL_SCID,
                                    "INITIAL_SCID was not sent but is "
                                    "required") TPARAM_CHECK_DROP (ORIG_DCID,
                                                                   "ORIG_DCID "
                                                                   "was not "
                                                                   "sent but "
                                                                   "is "
                                    ...
                                    TPARAM_CHECK_DROP_INJECT_A (
                                        ...
                                        "DISABLE_ACTIVE_MIGRATION is "
                                        "malformed")
                                        TPARAM_CHECK_INJECT (UNKNOWN_1, NULL,
                                                             0, NULL)
                                            TPARAM_CHECK_INJECT_RAW_A (
                                                malformed_stateless_reset_token_1,
                                                "STATELESS_RESET_TOKEN is "
                                                "malformed") TPARAM_CHECK_INJECT_A (STATELESS_RESET_TOKEN,
                                                                                    malformed_stateless_reset_token_2, "STATELESS_RESET_TOKEN is malformed") TPARAM_CHECK_INJECT_A (STATELESS_RESET_TOKEN, malformed_stateless_reset_token_3, "STATELESS_RESET_TOKEN is malformed") TPARAM_CHECK_INJECT_A (STATELESS_RESET_TOKEN, malformed_stateless_reset_token_4, "STATELESS_RESET_TOKEN is malformed") TPARAM_CHECK_INJECT (STATELESS_RESET_TOKEN, NULL, 0, "STATELESS_RESET_TOKEN is malformed") TPARAM_CHECK_INJECT_RAW_A (malformed_preferred_addr_1,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 "PREFERRED_ADDR is malformed") TPARAM_CHECK_INJECT_RAW_A (malformed_preferred_addr_2, "PREFERRED_ADDR is malformed") TPARAM_CHECK_INJECT_RAW_A (malformed_preferred_addr_3, "PREFERRED_ADDR is malformed") TPARAM_CHECK_INJECT_RAW_A (malformed_preferred_addr_4,
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       "PREFERRED_ADDR is malformed") TPARAM_CHECK_INJECT_RAW_A (malformed_unknown_1, "bad transport parameter") TPARAM_CHECK_INJECT_RAW_A (malformed_unknown_2,
                                                ...
                                                TPARAM_CHECK_INJECT_RAW_A (malformed_unknown_3, "bad transport parameter")
                                                    ...
                                                    TPARAM_CHECK_INJECT_A (ACK_DELAY_EXP, excess_ack_delay_exp, "ACK_DELAY_EXP is malformed") TPARAM_CHECK_INJECT_A (
                                                        ...
                                                        "MAX_ACK_DELAY is "
                                                        "malformed") TPARAM_CHECK_DROP_INJECT_A (INITIAL_MAX_STREAMS_BIDI,
                                                                                                 excess_initial_max_streams,
                                                                                                 "INITIAL_MAX_STREAMS_BIDI is malformed")
                                                        TPARAM_CHECK_DROP_INJECT_A (
                                                            INITIAL_MAX_STREAMS_UNI, excess_initial_max_streams, "INITIAL_MAX_STREAMS_UNI is malformed")
                                                            ...
                                                            TPARAM_CHECK_DROP_INJECT_A (MAX_UDP_PAYLOAD_SIZE, undersize_udp_payload_size, "MAX_UDP_PAYLOAD_SIZE is malformed")
                                                                TPARAM_CHECK_DROP_INJECT_A (
                                                                    ...
                                                                    TPARAM_CHECK_INJECT_TWICE_A (
                                                                        ...
                                                                        "ple "
                                                                        "time"
                                                                        "s") TPARAM_CHECK_INJECT_TWICE_A (MAX_ACK_DELAY, ack_delay_exp, "MAX_ACK_DELAY appears multiple times")
                                                                        TPARAM_CHECK_INJECT_TWICE_A (
                                                                                    ...
                                                                                    TPARAM_CHECK_MUTATE (
                                                                                        ...
                                                                                        TPARAM_CHECK_DROP_INJECT_A (
                                                                                            ...
                                                                                            TPARAM_CHECK_DROP_INJECT_A (
                                                                                                ...
                                                                                                TPARAM_CHECK_INT (INITIAL_MAX_DATA, "INITIAL_MAX_DATA is malformed") TPARAM_CHECK_INT (INITIAL_MAX_STREAM_DATA_BIDI_LOCAL, "INITIAL_MAX_STREAM_DATA_BIDI_LOCAL is malformed") TPARAM_CHECK_INT (
                                                                                                    INITIAL_MAX_STREAM_DATA_BIDI_REMOTE,
...
static int
tparam_handle (struct tparam_ctx *ctx, uint64_t id, unsigned char *data,
               size_t data_len, WPACKET *wpkt)
{
...
static int
tparam_on_enc_ext (QTEST_FAULT *qtf, QTEST_ENCRYPTED_EXTENSIONS *ee,
...
test_tparam (int idx)
{
  ...
  if (!TEST_true (qtest_create_quic_objects (
          libctx, c_ctx, NULL, cert, privkey, 0, &s, &c_ssl, &qtf, NULL)))
    goto err;
...
static BIO_ADDR *
create_addr (struct in_addr *ina, short int port)
{
  BIO_ADDR *addr = NULL;
...
static int
bio_addr_bind (BIO *bio, BIO_ADDR *addr)
{
  int bio_caps
...
static SSL *
ql_create (SSL_CTX *ssl_ctx, BIO *bio)
{
  SSL *qserver;
...
static int
qc_init (SSL *qconn, BIO_ADDR *dst_addr)
{
  static unsigned char alpn[] = { 8, 'o', 's', 's', 'l', 't', 'e', 's', 't' };
...
static int
test_ssl_new_from_listener (void)
{
...
static int
test_server_method_with_ssl_new (void)
{
  SSL_CTX *ctx = NULL;
  SSL *ssl = NULL;
  int ret = 0;
...
static int
create_quic_ssl_objects (SSL_CTX *sctx, SSL_CTX *cctx, SSL **lssl, SSL **cssl)
{
  BIO_ADDR *addr = NULL;
  struct in_addr ina;
...
static int
test_ssl_accept_connection (void)
{
...
static int
quic_verify_cb (int ok, X509_STORE_CTX *ctx)
{
...
/*
 * Creates server-initiated streams (1 uni stream if stream_opt=0, 1 bidi
 * stream if stream_opt=1, and both a uni and bidi stream if stream_opt=2) and
 * tests that client calling SSL_accept_stream with accept_flags behaves as
 * expected.
 */
static int
create_accept_stream (SSL *serverssl, SSL *clientssl, uint64_t accept_flags,
                      int stream_opt)
{
  ...
  if (create_uni
      && (!TEST_ptr (serverstream
          ...
          || !TEST_int_gt (
              SSL_write_ex (serverstream, msg, sizeof (msg), &nwritten), 0)
  ...
  if (create_bidi
      && (!TEST_ptr (serverstream2 = SSL_new_stream (serverssl, 0))
          || !TEST_int_gt (
              SSL_write_ex (serverstream2, msg, sizeof (msg), &nwritten), 0)
          || !TEST_size_t_eq (nwritten, sizeof (msg))
          || !TEST_int_eq (SSL_handle_events (clientssl), 1)))
    goto err;
  ...
  for (j = 0; j < sizeof (accept_flags) / sizeof (accept_flags[0]); j++)
    {
      if (!TEST_int_eq (
          ...
          || !TEST_int_eq (
              ...
              1)
          || !TEST_int_eq (
              create_accept_stream (serverssl, clientssl, accept_flags[j], 2),
              1))
        goto err;
    }
