...
static int test_quic_write_read(int idx) {
  ...
  for (k = 0; k < 2; k++) {
    ...
    if (sess != NULL && !TEST_true(SSL_set_session(clientquic, sess))) goto end;

    if (!TEST_true(qtest_create_quic_connection(qtserv, clientquic))) goto end;

    if (idx >= 1) {
      if (!TEST_true(BIO_get_fd(ossl_quic_tserver_get0_rbio(qtserv), &ssock)))
        goto end;
      if (!TEST_int_gt(csock = SSL_get_rfd(clientquic), 0)) goto end;
    }
    ...
    for (j = 0; j < 2; j++) {
      ...
      if (idx >= 1) {
        do {
          if (!TEST_true(wait_until_sock_readable(ssock))) goto end;
        ...
        if (!TEST_mem_eq(buf, numbytes, msg, msglen)) goto end;
      }

      if (idx >= 2 && j > 0) /* Introduce permanent socket error */
    ...
    if (sess == NULL) {
      ...
      if (!TEST_false(SSL_session_reused(clientquic))) goto end;
      ...
      if (!TEST_true(SSL_session_reused(clientquic))) goto end;
    }

    if (!TEST_true(qtest_shutdown(qtserv, clientquic))) goto end;
...
static int test_fin_only_blocking(void) {
  ...
  if (!TEST_true(ossl_quic_tserver_conclude(qtserv, sid))) goto end;
...
static int test_ciphersuites(void) {
   ...
   * aren't used in QUIC.
   */
  if (!TEST_true(SSL_CTX_set_cipher_list(ctx, "DEFAULT"))) goto err;

  ssl = SSL_new(ctx);
  if (!TEST_ptr(ssl)) goto err;

  if (!TEST_true(SSL_set_cipher_list(ssl, "DEFAULT"))) goto err;

  ciphers = SSL_get_ciphers(ssl);

  for (i = 0, j = 0; i < OSSL_NELEM(cipherids); i++) {
    if (cipherids[i] == TLS1_3_CK_CHACHA20_POLY1305_SHA256 && is_fips) continue;
    cipher = sk_SSL_CIPHER_value(ciphers, j++);
    if (!TEST_ptr(cipher)) goto err;
    if (!TEST_uint_eq(SSL_CIPHER_get_id(cipher), cipherids[i])) goto err;
  }

  /* We should have checked all the ciphers in the stack */
  if (!TEST_int_eq(sk_SSL_CIPHER_num(ciphers), j)) goto err;
...
static int test_cipher_find(void) {
  ...
  if (!TEST_ptr(cctx)) goto err;

  clientquic = SSL_new(cctx);
  if (!TEST_ptr(clientquic)) goto err;
...
static int compare_with_file(BIO* membio) {
...
#endif
  if (!TEST_ptr(reffile)) goto err;

  file = BIO_new_file(reffile, "rb");
  if (!TEST_ptr(file)) goto err;

  newfile = BIO_new_file("ssltraceref-new.txt", "wb");
  if (!TEST_ptr(newfile)) goto err;
  ...
  if (!TEST_int_ge(BIO_seek(membio, 0), 0)) goto err;
  ...
  while (BIO_gets(file, buf1, sizeof(buf1)) > 0) {
    ...
    if (line_len > 0 && buf1[line_len - 1] == '?') {
      ...
      if (strlen(buf2) > line_len) buf2[line_len] = '\0';
    }
    ...
    if (!TEST_str_eq(buf1, buf2)) goto err;
  }
  if (!TEST_true(BIO_eof(file)) || !TEST_true(BIO_eof(membio))) goto err;
...
static int test_ssl_trace(void) {
  ...
  if (!TEST_true(qtest_create_quic_connection(qtserv, clientquic))) goto err;

  /* Skip the comparison of the trace when the fips provider is used. */
  if (is_fips) {
    /* Check whether there was something written. */
    if (!TEST_int_gt(BIO_pending(bio), 0)) goto err;
  } else {
    if (!TEST_true(compare_with_file(bio))) goto err;
  }
...
static int find_new_token_data(BIO* membio) {
  ...
  while (state != SUCCESS && state != FAILED &&
    ...
    switch (state) {
      case INITIAL:
        if (strstr(buf, "Received Frame: New token")) state = GATHER_TOKEN;
        break;
      case GATHER_TOKEN:
        ...
        if (tmpstring == NULL) {
          ...
          if (!TEST_ptr(tokenval = OPENSSL_strdup(tmpstring))) return 0;
          state = CHECK_TOKEN;
...
static int test_new_token(void) {
  ...
  if (!SSL_write_ex(clientquic, msg, sizeof(msg), &written)) goto err;

  if (written != sizeof(msg)) goto err;
  ...
  if (!TEST_true(qtest_create_quic_connection(qtserv2, clientquic2))) goto err;

  /* Skip the comparison of the trace when the fips provider is used. */
  if (!TEST_true(find_new_token_data(bio))) goto err;
...
static int test_quic_forbidden_apis(void) {
  ...
  if (!TEST_ptr(ssl = SSL_new(ctx))) goto err;
...
static int test_quic_forbidden_options(void) {
  ...
  if (!TEST_uint64_t_eq(SSL_CTX_get_options(ctx), UINT64_MAX)) goto err;
  ...
  if (!TEST_uint64_t_eq(SSL_get_options(ssl), 0)) goto err;

  /* Readahead */
  if (!TEST_false(SSL_get_read_ahead(ssl))) goto err;
  ...
  /* HRR */
  if (!TEST_false(SSL_stateless(ssl))) goto err;

  /* Quiet Shutdown */
  if (!TEST_false(SSL_get_quiet_shutdown(ssl))) goto err;

  /* No duplication */
  if (!TEST_ptr_null(SSL_dup(ssl))) goto err;

  /* No clear */
  if (!TEST_false(SSL_clear(ssl))) goto err;
...
static int test_quic_set_fd(int idx) {
  ...
  if (idx == 0) {
    if (!TEST_true(SSL_set_fd(ssl, fd))) goto err;
    if (!TEST_ptr(bio = SSL_get_rbio(ssl))) goto err;
    if (!TEST_ptr_eq(bio, SSL_get_wbio(ssl))) goto err;
  } else if (idx == 1) {
    if (!TEST_true(SSL_set_rfd(ssl, fd))) goto err;
    if (!TEST_ptr(bio = SSL_get_rbio(ssl))) goto err;
    if (!TEST_ptr_null(SSL_get_wbio(ssl))) goto err;
  } else {
    if (!TEST_true(SSL_set_wfd(ssl, fd))) goto err;
    if (!TEST_ptr(bio = SSL_get_wbio(ssl))) goto err;
    if (!TEST_ptr_null(SSL_get_rbio(ssl))) goto err;
  }

  if (!TEST_int_eq(BIO_method_type(bio), BIO_TYPE_DGRAM)) goto err;

  if (!TEST_true(BIO_get_fd(bio, &resfd)) || !TEST_int_eq(resfd, fd)) goto err;
...
static int test_bio_ssl(void) {
   ...
   */
  if (!TEST_int_eq(BIO_get_ssl(cbio, &clientquic), 1)) goto err;
  ...
  do {
    ...
    if (!servererr && rets <= 0) {
      ...
      if (!servererr) rets = ossl_quic_tserver_is_handshake_confirmed(qtserv);
    }

    if (clienterr && servererr) goto err;
  ...
  for (i = 0, thisbio = cbio; i < 2; i++) {
    ...
    if (!TEST_true(SSL_set_mode(clientquic, 0))) goto err;
    ...
    if (!TEST_ptr(stream)) goto err;

    if (!TEST_true(SSL_set_mode(stream, 0))) goto err;

    thisbio = strbio = BIO_new(BIO_f_ssl());
    if (!TEST_ptr(strbio)) goto err;

    if (!TEST_int_eq(BIO_set_ssl(thisbio, stream, BIO_CLOSE), 1)) goto err;
    stream = NULL;

    if (!TEST_true(BIO_write_ex(thisbio, msg, msglen, &written))) goto err;
...
static int test_back_pressure(void) {
  ...
  if (!TEST_ptr(msg)) goto err;
  if (!TEST_int_eq(RAND_bytes_ex(libctx, msg, msglen, 0), 1)) goto err;
  ...
  for (i = 0; i < BACK_PRESSURE_NUM_LOOPS; i++) {
    ...
    if (!SSL_write_ex(clientquic, msg, msglen, &written)) {
      ...
      if (SSL_get_error(clientquic, 0) == SSL_ERROR_WANT_WRITE) break;
...
static void dgram_cb(int write_p, int version, int content_type,
  ...
  if (content_type != SSL3_RT_QUIC_DATAGRAM) return;
...
static int non_io_retry_cert_verify_cb(X509_STORE_CTX* ctx, void* arg) {
  ...
  if (*allow == 0) return SSL_set_retry_verify(ssl);
...
static int find_session_cb(SSL* ssl, const unsigned char* identity,
  ...
  if (serverpsk == NULL || !SSL_SESSION_up_ref(serverpsk)) return 0;
...
static int test_client_auth(int idx) {
  ...
  if (!TEST_ptr(cctx) || !TEST_ptr(sctx)) goto err;
  ...
  if (!TEST_true(SSL_CTX_load_verify_file(sctx, cauthca))) goto err;
  ...
  if (idx == 0) {
    if (!TEST_false(qtest_create_quic_connection(qtserv, clientquic))) goto err;
  ...
  if (!TEST_true(qtest_shutdown(qtserv, clientquic))) goto err;
...
static int test_alpn(int idx) {
  ...
  if (idx == 0) {
     ...
     */
    if (!TEST_false(SSL_set_alpn_protos(clientquic, NULL, 0))) goto err;
  }

  ret = SSL_connect(clientquic);
  if (!TEST_int_le(ret, 0)) goto err;
  if (idx == 0) {
    /* We expect an immediate error due to lack of ALPN */
    if (!TEST_int_eq(SSL_get_error(clientquic, ret), SSL_ERROR_SSL)) goto err;
  } else {
...
static int test_get_shutdown(void) {
  ...
  if (!TEST_int_eq(SSL_get_shutdown(clientquic), 0)) goto err;

  if (!TEST_int_eq(SSL_shutdown(clientquic), 0)) goto err;

  if (!TEST_int_eq(SSL_get_shutdown(clientquic), SSL_SENT_SHUTDOWN)) goto err;
...
static int unreliable_client_read(SSL* clientquic, SSL** stream, void* buf,
  ...
  for (abortctr = 0; abortctr < MAX_LOOPS; abortctr++) {
    ...
    if (*stream != NULL) {
      if (SSL_read_ex(*stream, buf, buflen, readbytes)) return 1;
...
static int test_noisy_dgram(int idx) {
  ...
  if (idx == 1) flags |= QTEST_FLAG_PACKET_SPLIT;
  ...
  for (j = 0; j < 2; j++) {
    if (!TEST_true(ossl_quic_tserver_stream_new(qtserv, 0, &sid))) goto err;
    ossl_quic_tserver_tick(qtserv);
    qtest_add_time(1);
...
static int test_bw_limit(void) {
  ...
  if (!TEST_true(qtest_fault_set_bw_limit(fault, 1000, 1000, 0))) goto err;
  ...
  if (!TEST_uint64_t_lt(real_bw, TEST_BW_LIMIT)) goto err;
                    ...
                    TPARAM_CHECK_DUP(MAX_UDP_PAYLOAD_SIZE,
                        ...
                        TPARAM_CHECK_DUP(
                            ...
                            TPARAM_CHECK_DROP(INITIAL_SCID,
                                              "INITIAL_SCID was not sent but "
                                              "is required") TPARAM_CHECK_DROP(ORIG_DCID,
                                ...
                                TPARAM_CHECK_DROP_INJECT_A(
                                    ...
                                    TPARAM_CHECK_INJECT(
                                        UNKNOWN_1, NULL, 0,
                                        NULL) TPARAM_CHECK_INJECT_RAW_A(malformed_stateless_reset_token_1,
                                                                        "STATEL"
                                                                        "ESS_"
                                                                        "RESET_"
                                                                        "TOKEN "
                                                                        ...
                                                                        "malfor"
                                                                        "med")
                                        TPARAM_CHECK_INJECT_A(
                                            STATELESS_RESET_TOKEN,
                                            malformed_stateless_reset_token_2,
                                            "STATELESS_RESET_TOKEN is "
                                            "malformed")
                                            TPARAM_CHECK_INJECT_A(
                                                STATELESS_RESET_TOKEN,
                                                                                                ...
                                                                                                TPARAM_CHECK_DROP_INJECT_A(
                                                                                                    ...
                                                                                                    TPARAM_CHECK_DROP_INJECT_A(
                                                                                                        ...
                                                                                                        TPARAM_CHECK_DROP_INJECT_A(
                                                                                                            ...
                                                                                                            TPARAM_CHECK_INJECT_TWICE_A(ACK_DELAY_EXP,
                                                                                                                                        ack_delay_exp,
                                                                                                                                        "ACK_DELAY_EXP appears multiple times") TPARAM_CHECK_INJECT_TWICE_A(MAX_ACK_DELAY, ack_delay_exp,
                                                                                                                                                                                                            "MAX_ACK_DELAY appears multiple times") TPARAM_CHECK_INJECT_TWICE_A(STATELESS_RESET_TOKEN,
                                                                                                                                                                                                                                                                                stateless_reset_token,
                                                                                                                                                                                                                                                                                "STATELESS_RESET_TOKEN appears multiple times") TPARAM_CHECK_INJECT_TWICE_A(PREFERRED_ADDR,
                                                                                                                                                                                                                                                                                                                                                            preferred_addr,
                                                                                                                                                                                                                                                                                                                                                            "PREFERRED_ADDR appears multiple times")

                                                                                                                TPARAM_CHECK_MUTATE(
                                                                                                                    ORIG_DCID,
                                                                                                                    "ORIG_DCID does not match expected value") TPARAM_CHECK_MUTATE(INITIAL_SCID,
                                                                                                                                                                                   "INITIAL_SCID does not match expected value")

                                                                                                                    TPARAM_CHECK_DROP_INJECT_A(
                                                                                                                        ORIG_DCID, long_cid,
                                                                                                                        "ORIG_DCID is malformed") TPARAM_CHECK_DROP_INJECT_A(INITIAL_SCID,
                                                                                                                                                                             long_cid,
                                                                                                                        ...
                                                                                                                        TPARAM_CHECK_INT(
                                                                                                                            ...
                                                                                                                            TPARAM_CHECK_INT(
                                                                                                                                ...
                                                                                                                                "INITIAL_MAX_STREAM_DATA_BIDI_LOCAL is malformed") TPARAM_CHECK_INT(INITIAL_MAX_STREAM_DATA_BIDI_REMOTE,
                                                                                                                                                                                                    "INITIAL_MAX_STREAM_DATA_BIDI_REMOTE is malformed") TPARAM_CHECK_INT(INITIAL_MAX_STREAM_DATA_UNI,
                                                                                                                                                                                                                                                                         "INITIAL_MAX_STREAM_DATA_UNI is malformed")
                                                                                                                                TPARAM_CHECK_INT(ACK_DELAY_EXP,
                                                                                                                                                 "ACK_DELAY_EXP is malformed") TPARAM_CHECK_INT(MAX_ACK_DELAY,
                                                                                                                                                                                                "MAX_ACK_DELAY is malformed") TPARAM_CHECK_INT(INITIAL_MAX_STREAMS_BIDI,
...
static int tparam_handle(struct tparam_ctx* ctx, uint64_t id,
  ...
  switch (t->op) {
    case TPARAM_OP_DUP:
      if (!TEST_ptr(ossl_quic_wire_encode_transport_param_bytes(wpkt, id, data,
                                                                data_len)))
        return 0;
...
static int tparam_on_enc_ext(QTEST_FAULT* qtf, QTEST_ENCRYPTED_EXTENSIONS* ee,
  ...
  if (!TEST_ptr(old_bufm = BUF_MEM_new())) goto err;
  ...
  for (; PACKET_remaining(&pkt) > 0;) {
    ...
    if (!TEST_true(tparam_handle(ctx, id, tp_p, tp_len, &wpkt))) goto err;
  }
  ...
  if (!TEST_true(WPACKET_get_total_written(&wpkt, &written))) goto err;
  ...
  if (!qtest_fault_resize_message(qtf, ee->extensionslen + written)) goto err;
  ...
  rc = 1;
err:
  if (have_wpkt) WPACKET_cleanup(&wpkt);
  ...
  if (ctx.t->expect_fail != NULL) {
    ...
    if (!TEST_true((info.flags & SSL_CONN_CLOSE_FLAG_TRANSPORT) != 0) ||
      ...
      TEST_error(
          ...
          (unsigned long long)info.flags, (unsigned long long)info.error_code,
          info.reason);
      goto err;
    }
  }
...
static int test_session_cb(void) {
  ...
  if (!TEST_true(SSL_handle_events(clientquic))) goto err;
   ...
   */
  if (!TEST_int_eq(new_called, 2) || !TEST_ptr_eq(clientquic, cbssl)) goto err;
...
static int test_early_ticks(void) {
  ...
  if (!TEST_true(SSL_in_before(clientquic))) goto err;
  ...
  if (!TEST_false(SSL_in_before(clientquic))) goto err;
...
static BIO_ADDR* create_addr(struct in_addr* ina, short int port) {
  ...
  if (!TEST_ptr(addr = BIO_ADDR_new())) return NULL;
...
static int bio_addr_bind(BIO* bio, BIO_ADDR* addr) {
  ...
  if (!TEST_true(BIO_dgram_set_caps(bio, bio_caps))) return 0;

  if (!TEST_int_eq(BIO_dgram_set0_local_addr(bio, addr), 1)) return 0;
...
static int qc_init(SSL* qconn, BIO_ADDR* dst_addr) {
  ...
  if (!TEST_true(SSL_set1_initial_peer_addr(qconn, dst_addr))) return 0;
...
static int test_ssl_new_from_listener(void) {
  ...
  if (!TEST_ptr(addr = create_addr(&ina, 8040))) goto err;

  if (!TEST_true(bio_addr_bind(lbio, addr))) goto err;
  addr = NULL;

  if (!TEST_ptr(addr = create_addr(&ina, 4080))) goto err;

  if (!TEST_true(bio_addr_bind(sbio, addr))) goto err;
  addr = NULL;

  qlistener = ql_create(lctx, lbio);
  lbio = NULL;
  if (!TEST_ptr(qlistener)) goto err;

  qserver = ql_create(sctx, sbio);
  sbio = NULL;
  if (!TEST_ptr(qserver)) goto err;

  if (!TEST_ptr(qconn = SSL_new_from_listener(qlistener, 0))) goto err;

  if (!TEST_ptr(addr = create_addr(&ina, 4080))) goto err;

  chk = qc_init(qconn, addr);
  if (!TEST_true(chk)) goto err;
...
static int create_quic_ssl_objects(SSL_CTX* sctx, SSL_CTX* cctx, SSL** lssl,
  ...
  *cssl = *lssl = NULL;
  ...
  if (!TEST_true(BIO_new_bio_dgram_pair(&cbio, 0, &sbio, 0))) goto err;
  ...
  *lssl = ql_create(sctx, sbio);
  sbio = NULL;
  if (!TEST_ptr(*lssl)) goto err;

  if (!TEST_ptr(*cssl = SSL_new(cctx))) goto err;

  if (!TEST_ptr(addr = create_addr(&ina, 8040))) goto err;
  if (!TEST_true(bio_addr_bind(cbio, addr))) goto err;
...
static int test_ssl_accept_connection(void) {
  ...
  if (!create_quic_ssl_objects(sctx, cctx, &qlistener, &clientssl)) goto err;
...
static int create_accept_stream(SSL* serverssl, SSL* clientssl,
  ...
  if (stream_opt < 0 || stream_opt > 2) goto err;
  ...
  if (create_uni &&
      (!TEST_ptr(serverstream =
                     SSL_new_stream(serverssl, SSL_STREAM_FLAG_UNI)) ||
       !TEST_int_gt(SSL_write_ex(serverstream, msg, sizeof(msg), &nwritten),
                    0) ||
  ...
  if (create_bidi &&
      (!TEST_ptr(serverstream2 = SSL_new_stream(serverssl, 0)) ||
       !TEST_int_gt(SSL_write_ex(serverstream2, msg, sizeof(msg), &nwritten),
                    0) ||
       !TEST_size_t_eq(nwritten, sizeof(msg)) ||
       !TEST_int_eq(SSL_handle_events(clientssl), 1)))
    goto err;
...
static int test_accept_stream(void) {
  ...
  int ret, i;
  long unsigned int j;
  uint64_t accept_flags[] = {0, SSL_ACCEPT_STREAM_UNI, SSL_ACCEPT_STREAM_BIDI,
  ...
  if (!TEST_int_eq(
          SSL_set_default_stream_mode(clientssl, SSL_DEFAULT_STREAM_MODE_NONE),
          1) ||
      !TEST_int_eq(
          SSL_set_default_stream_mode(serverssl, SSL_DEFAULT_STREAM_MODE_NONE),
          1) ||
      !TEST_int_eq(SSL_set_incoming_stream_policy(
                       clientssl, SSL_INCOMING_STREAM_POLICY_ACCEPT, 0),
                   1) ||
      !TEST_int_eq(SSL_set_incoming_stream_policy(
                       serverssl, SSL_INCOMING_STREAM_POLICY_ACCEPT, 0),
                   1))
    goto err;

  for (j = 0; j < sizeof(accept_flags) / sizeof(accept_flags[0]); j++) {
    if (!TEST_int_eq(
            create_accept_stream(serverssl, clientssl, accept_flags[j], 0),
            1) ||
        !TEST_int_eq(
            create_accept_stream(serverssl, clientssl, accept_flags[j], 1),
            1) ||
        !TEST_int_eq(
            create_accept_stream(serverssl, clientssl, accept_flags[j], 2), 1))
      goto err;
  }
...
static int test_client_hello_retry(void) {
  ...
  if (!TEST_true(SSL_CTX_set1_groups_list(cctx, "secp384r1:secp256r1")))
    goto err;
  if (!TEST_true(SSL_CTX_set1_groups_list(sctx, "secp256r1"))) goto err;
...
int setup_tests(void) {
  ...
  if (!TEST_true(OSSL_LIB_CTX_load_config(libctx, configfile))) goto err;

  /* Check we have the expected provider available */
  if (!TEST_true(OSSL_PROVIDER_available(libctx, modulename))) goto err;
