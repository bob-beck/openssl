...
/* The ssltrace test assumes some options are switched on/off */
    ...
    !defined(OPENSSL_NO_ECX) && !defined(OPENSSL_NO_DH) && !defined(OPENSSL_NO_ML_DSA) && !defined(OPENSSL_NO_ML_KEM)
...
static int test_fin_only_blocking(void)
{
    ...
    if (!TEST_ptr(cctx) ||
        !TEST_true(qtest_create_quic_objects(libctx, cctx, sctx, cert, privkey, QTEST_FLAG_BLOCK, &qtserv, &clientquic,
                                             NULL, NULL)) ||
        !TEST_true(SSL_set_tlsext_host_name(clientquic, "localhost")))
        goto end;
...
static int test_alpn(int idx)
{
    ...
    if (!TEST_ptr(cctx) || !TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert, privkey, QTEST_FLAG_FAKE_TIME,
                                                                &qtserv, &clientquic, NULL, NULL)))
        goto err;
...
static int test_noisy_dgram(int idx)
{
    ...
    if (!TEST_ptr(cctx) || !TEST_true(qtest_create_quic_objects(libctx, cctx, NULL, cert, privkey, flags, &qtserv,
...
#define TPARAM_CHECK_DUP(name, reason) {QUIC_TPARAM_##name, TPARAM_OP_DUP, (reason)},
#define TPARAM_CHECK_DROP(name, reason) {QUIC_TPARAM_##name, TPARAM_OP_DROP, (reason)},
#define TPARAM_CHECK_INJECT(name, buf, buf_len, reason)                                                                \
    {QUIC_TPARAM_##name, TPARAM_OP_INJECT, (reason), (buf), (buf_len)},
#define TPARAM_CHECK_INJECT_A(name, buf, reason) TPARAM_CHECK_INJECT(name, buf, sizeof(buf), reason)
#define TPARAM_CHECK_DROP_INJECT(name, buf, buf_len, reason)                                                           \
    {QUIC_TPARAM_##name, TPARAM_OP_DROP_INJECT, (reason), (buf), (buf_len)},
#define TPARAM_CHECK_DROP_INJECT_A(name, buf, reason) TPARAM_CHECK_DROP_INJECT(name, buf, sizeof(buf), reason)
#define TPARAM_CHECK_INJECT_TWICE(name, buf, buf_len, reason)                                                          \
    {QUIC_TPARAM_##name, TPARAM_OP_INJECT_TWICE, (reason), (buf), (buf_len)},
#define TPARAM_CHECK_INJECT_TWICE_A(name, buf, reason) TPARAM_CHECK_INJECT_TWICE(name, buf, sizeof(buf), reason)
#define TPARAM_CHECK_INJECT_RAW(buf, buf_len, reason) {0, TPARAM_OP_INJECT_RAW, (reason), (buf), (buf_len)},
#define TPARAM_CHECK_INJECT_RAW_A(buf, reason) TPARAM_CHECK_INJECT_RAW(buf, sizeof(buf), reason)
#define TPARAM_CHECK_MUTATE(name, reason) {QUIC_TPARAM_##name, TPARAM_OP_MUTATE, (reason)},
...
static const struct tparam_test tparam_tests[] = {
    TPARAM_CHECK_DUP(ORIG_DCID, "ORIG_DCID appears multiple times") TPARAM_CHECK_DUP(
        ...
        TPARAM_CHECK_DUP(INITIAL_MAX_STREAM_DATA_BIDI_LOCAL, "INITIAL_MAX_STREAM_DATA_BIDI_LOCAL appears multiple "
            ...
            "INITIAL_MAX_STREAM_DATA_BIDI_REMOTE appears multiple times") TPARAM_CHECK_DUP(INITIAL_MAX_STREAM_DATA_UNI,
                                                                                           ...
                                                                                           "UNI appears multiple times")
            TPARAM_CHECK_DUP(
                INITIAL_MAX_STREAMS_BIDI,
                "INITIAL_MAX_STREAMS_BIDI appears multiple times") TPARAM_CHECK_DUP(INITIAL_MAX_STREAMS_UNI,
                                                                                    "INITIAL_MAX_STREAMS_UNI appears "
                                                                                    "multiple times")
                TPARAM_CHECK_DUP(MAX_IDLE_TIMEOUT, "MAX_IDLE_TIMEOUT appears multiple times") TPARAM_CHECK_DUP(
                    MAX_UDP_PAYLOAD_SIZE,
                    "MAX_UDP_PAYLOAD_SIZE appears multiple times") TPARAM_CHECK_DUP(ACTIVE_CONN_ID_LIMIT,
                                                                                    "ACTIVE_CONN_ID_LIMIT appears "
                                                                                    "multiple times")
                    TPARAM_CHECK_DUP(DISABLE_ACTIVE_MIGRATION, "DISABLE_ACTIVE_MIGRATION appears multiple times")
                        ...
                        TPARAM_CHECK_DROP(INITIAL_SCID, "INITIAL_SCID was not sent but is required") TPARAM_CHECK_DROP(
                            ...
                            TPARAM_CHECK_DROP_INJECT_A(
                                DISABLE_ACTIVE_MIGRATION, disable_active_migration_1,
                                "DISABLE_ACTIVE_MIGRATION is malformed") TPARAM_CHECK_INJECT(UNKNOWN_1, NULL, 0, NULL)
                                TPARAM_CHECK_INJECT_RAW_A(malformed_stateless_reset_token_1, "STATELESS_RESET_TOKEN is "
                                                                                             "malformed") TPARAM_CHECK_INJECT_A(
                                    ...
                                    TPARAM_CHECK_INJECT_A(STATELESS_RESET_TOKEN, malformed_stateless_reset_token_3, "STATELESS_RESET_TOKEN is malformed") TPARAM_CHECK_INJECT_A(
                                        ...
                                        TPARAM_CHECK_INJECT_RAW_A(malformed_preferred_addr_1, "PREFERRED_ADDR is "
                                                                                              "malformed") TPARAM_CHECK_INJECT_RAW_A(
                                            malformed_preferred_addr_2,
                                            "PREFERRED_ADDR is malformed")
                                            TPARAM_CHECK_INJECT_RAW_A(malformed_preferred_addr_3, "PREFERRED_ADDR is "
                                                                                                  "malformed") TPARAM_CHECK_INJECT_RAW_A(
                                                malformed_preferred_addr_4,
                                                "PREFERRED_ADDR is malformed")
                                                TPARAM_CHECK_INJECT_RAW_A(malformed_unknown_1, "bad transport "
                                                                                               "parameter") TPARAM_CHECK_INJECT_RAW_A(
                                                    malformed_unknown_2,
                                                                    ...
                                                                    TPARAM_CHECK_DROP_INJECT_A(
                                                                        ...
                                                                        TPARAM_CHECK_INJECT_TWICE_A(ACK_DELAY_EXP, ack_delay_exp, "ACK_DELAY_EXP appears multiple times") TPARAM_CHECK_INJECT_TWICE_A(
                                                                            ...
                                                                            TPARAM_CHECK_INJECT_TWICE_A(
                                                                                STATELESS_RESET_TOKEN,
                                                                                stateless_reset_token,
                                                                                "STATELESS_RESET_TOKEN appears "
                                                                                "multiple times") TPARAM_CHECK_INJECT_TWICE_A(PREFERRED_ADDR,
                                                                                                                              preferred_addr,
                                                                                                                              "PREFERRED_ADDR appears multiple times")

                                                                                TPARAM_CHECK_MUTATE(
                                                                                    ORIG_DCID,
                                                                                    "ORIG_DCID does not match expected "
                                                                                    "value") TPARAM_CHECK_MUTATE(INITIAL_SCID, "INITIAL_SCID does not match expected value")
                                                                                            ...
                                                                                            TPARAM_CHECK_INT(
                                                                                                ...
                                                                                                TPARAM_CHECK_INT(
                                                                                                    ...
                                                                                                    TPARAM_CHECK_INT(
                                                                                                        ...
                                                                                                        "STREAM_DATA_"
                                                                                                        "UNI is "
                                                                                                        "malformed") TPARAM_CHECK_INT(ACK_DELAY_EXP, "ACK_DELAY_EXP is malformed")
                                                                                                        TPARAM_CHECK_INT(
...
static int create_accept_stream(SSL *serverssl, SSL *clientssl, uint64_t accept_flags, int stream_opt)
{
    ...
    if (create_uni && (!TEST_ptr(serverstream = SSL_new_stream(serverssl, SSL_STREAM_FLAG_UNI)) ||
    ...
    if (create_bidi && (!TEST_ptr(serverstream2 = SSL_new_stream(serverssl, 0)) ||
