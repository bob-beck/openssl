...
static int test_print_acert(int idx) {
  ...
  if (!TEST_ptr(acert_file = test_get_argument(idx))) return 0;

  if (!TEST_ptr(bp = BIO_new_file(acert_file, "r"))) return 0;

  if (!TEST_ptr(bout = BIO_new_fp(stderr, BIO_NOCLOSE))) goto err;
...
static int test_acert_sign(void) {
  ...
  if (!TEST_ptr(acert_file = test_get_argument(0))) return 0;

  if (!TEST_ptr(pkey = EVP_RSA_gen(2048))) return 0;

  if (!TEST_ptr(bp = BIO_new_file(acert_file, "r"))) goto err;
...
static int test_object_group_attr(int idx) {
  ...
  if (!TEST_ptr(bout = BIO_new_fp(stderr, BIO_NOCLOSE))) goto done;
