...
test_aes_raw_skey (void)
{
  ...
  fin_len = 0;
  if (!TEST_int_gt (
          EVP_CipherInit_ex2 (ctx, aes_cbc, aes_key, aes_iv, 1, NULL), 0)
      || !TEST_int_gt (
          EVP_CipherUpdate (ctx, encrypted_raw, &enc_len, data, DATA_SIZE), 0)
      || !TEST_int_gt (
          EVP_CipherFinal (ctx, encrypted_raw + enc_len, &fin_len), 0)
...
test_des_raw_skey (void)
{
  ...
  fin_len = 0;
  if (!TEST_int_gt (
          EVP_CipherInit_ex2 (ctx, des_cbc, des_key, des_iv, 1, NULL), 0)
      || !TEST_int_gt (
          EVP_CipherUpdate (ctx, encrypted_raw, &enc_len, data, DATA_SIZE), 0)
      || !TEST_int_gt (
          EVP_CipherFinal (ctx, encrypted_raw + enc_len, &fin_len), 0)
