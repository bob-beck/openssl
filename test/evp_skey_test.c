...
test_skey_cipher(void)
{
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
    FAKE_CIPHER_PARAM_KEY_NAME, "fake key name", 0);
  params[1] = OSSL_PARAM_construct_octet_string(
    OSSL_SKEY_PARAM_RAW_BYTES, (void*)import_key, KEY_SIZE);
  params[2] = OSSL_PARAM_construct_end();
  key = EVP_SKEY_import(libctx,
                        "fake_cipher",
...
test_skey_skeymgmt(void)
{
  ...
  if (!TEST_ptr(key = EVP_SKEY_import(libctx,
                                      EVP_SKEYMGMT_get0_name(skeymgmt),
                                      NULL,
                                      OSSL_SKEYMGMT_SELECT_ALL,
                                      params)))
    goto end;
...
{
  const unsigned char data[DATA_SIZE] = { 0x2, 0x2, 0x2, 0x2, 0x2, 0x2, 0x2,
