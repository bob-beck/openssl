...
test_basic(void)
{
  ...
  if (!TEST_true(qtest_create_quic_objects(
        NULL, cctx, NULL, cert, privkey, 0, &qtserv, &cssl, NULL, NULL)))
    goto err;
 ...
 */
static int
add_unknown_frame_cb(QTEST_FAULT* fault,
                     QUIC_PKT_HDR* hdr,
                     unsigned char* buf,
                     size_t len,
                     void* cbarg)
...
test_unknown_frame(void)
{
  ...
  if (!TEST_true(qtest_create_quic_objects(
        NULL, cctx, NULL, cert, privkey, 0, &qtserv, &cssl, &fault, NULL)))
    goto err;
  ...
  if (!TEST_true(ossl_quic_tserver_write(
        qtserv, sid, (unsigned char*)msg, msglen, &byteswritten)))
    goto err;
 ...
 */
static int
drop_extensions_cb(QTEST_FAULT* fault,
                   ...
                   void* encextcbarg)
{
  ...
  if (!qtest_fault_delete_extension(
        fault, *ext, ee->extensions, &ee->extensionslen, NULL))
    return 0;
...
static int
on_packet_cipher_cb(QTEST_FAULT* fault,
                    QUIC_PKT_HDR* hdr,
                    unsigned char* buf,
                    size_t len,
                    void* cbarg)
...
static int
on_datagram_cb(QTEST_FAULT* fault, BIO_MSG* m, size_t stride, void* cbarg)
{
...
test_corrupted_data(int idx)
{
  ...
  if (!TEST_true(qtest_create_quic_objects(NULL,
                                           cctx,
                                           NULL,
                                           cert,
                                           privkey,
   ...
   */
  if (!TEST_true(ossl_quic_tserver_write(
        qtserv, sid, (unsigned char*)msg, 5, &byteswritten)))
    goto err;
