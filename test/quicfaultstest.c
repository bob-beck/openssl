...
static int test_basic(void) {
  ...
  if (!TEST_true(qtest_create_quic_connection(qtserv, cssl))) goto err;

  if (!TEST_int_eq(SSL_write(cssl, msg, (int)msglen), (int)msglen)) goto err;
   ...
   * not.
   */
  if (!TEST_mem_eq(msg, msglen, buf, bytesread)) goto err;
...
static int test_unknown_frame(void) {
  ...
  if (!TEST_size_t_eq(msglen, byteswritten)) goto err;

  ossl_quic_tserver_tick(qtserv);
  if (!TEST_true(SSL_handle_events(cssl))) goto err;

  if (!TEST_int_le(ret = SSL_read(cssl, buf, sizeof(buf)), 0)) goto err;

  if (!TEST_int_eq(SSL_get_error(cssl, ret), SSL_ERROR_SSL)) goto err;

  if (!TEST_int_eq(ERR_GET_REASON(ERR_peek_error()), SSL_R_QUIC_PROTOCOL_ERROR))
    goto err;

  if (!TEST_true(qtest_check_server_frame_encoding_err(qtserv))) goto err;
...
static int test_drop_extensions(int idx) {
   ...
   */
  if (!TEST_false(qtest_create_quic_connection(qtserv, cssl))) goto err;

  if (!TEST_true(qtest_check_server_transport_err(qtserv, err))) goto err;
...
static int on_datagram_cb(QTEST_FAULT* fault, BIO_MSG* m, size_t stride,
                          void* cbarg) {
  if (!docorrupt || m->data_len == 0) return 1;

  if (!qtest_fault_resize_datagram(fault, m->data_len - 1)) return 1;
...
static int test_corrupted_data(int idx) {
  ...
  if (!TEST_size_t_eq(byteswritten, 5)) goto err;
  ...
  if (!TEST_size_t_eq(byteswritten, msglen - 5)) goto err;
  ...
  if (!TEST_true(SSL_read_ex(cssl, buf, sizeof(buf), &bytesread))) goto err;
   ...
   * docorrupt was reset
   */
  if (!TEST_false(docorrupt)) goto err;
