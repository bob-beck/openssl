...
static void helper_cleanup(struct helper* h) {
  ...
  if (h->cc_data != NULL) h->cc_method->free(h->cc_data);
  if (h->have_statm) ossl_statm_destroy(&h->statm);
  if (h->have_qsm) ossl_quic_stream_map_cleanup(&h->qsm);
...
static void demux_default_handler(QUIC_URXE* e, void* arg,
  ...
  if (dcid == NULL || !ossl_quic_conn_id_eq(dcid, &dcid_1)) return;
...
static int helper_init(struct helper* h) {
  ...
  /* Initialisation */
  if (!TEST_true(BIO_new_bio_dgram_pair(&h->bio1, 0, &h->bio2, 0))) goto err;

  h->qtx_args.bio = h->bio1;
  h->qtx_args.mdpl = 1200;

  if (!TEST_ptr(h->args.qtx = ossl_qtx_new(&h->qtx_args))) goto err;

  if (!TEST_ptr(h->args.txpim = ossl_quic_txpim_new())) goto err;

  if (!TEST_ptr(h->args.cfq = ossl_quic_cfq_new())) goto err;

  if (!TEST_true(ossl_quic_txfc_init(&h->conn_txfc, NULL))) goto err;
  ...
  if (!TEST_true(ossl_quic_rxfc_init(&h->max_streams_uni_rxfc, NULL, 100, 100,
    ...
    if (!TEST_true(ossl_statm_init(&h->statm))) goto err;

  h->have_statm = 1;

  h->cc_method = &ossl_cc_dummy_method;
  if (!TEST_ptr(h->cc_data = h->cc_method->new(fake_now, NULL))) goto err;
  ...
  if (!TEST_ptr(h->client_ch)) goto err;
  ...
  for (i = 0; i < QUIC_PN_SPACE_NUM; ++i)
    if (!TEST_ptr(h->args.crypto[i] = ossl_quic_sstream_new(4096))) goto err;
  ...
  if (!TEST_ptr(h->qrx = ossl_qrx_new(&h->qrx_args))) goto err;

  ossl_qrx_allow_1rtt_processing(h->qrx);

  rc = 1;
err:
  if (!rc) helper_cleanup(h);
...
static int schedule_cfq_new_conn_id(struct helper* h) {
  ...
  if (!TEST_ptr(buf_mem = BUF_MEM_new())) goto err;

  if (!TEST_true(WPACKET_init(&wpkt, buf_mem))) goto err;
...
static int schedule_ack(struct helper* h) {
  ...
  for (i = 0; i < 5; ++i) {
    ...
    if (!TEST_true(ossl_ackm_on_rx_packet(h->args.ackm, &rx_pkt))) return 0;
  }
...
static const struct script_op script_10[] = {
    OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT, QRL_SUITE_AES128GCM, secret_1)
        OP_HANDSHAKE_COMPLETE() OP_TXP_GENERATE_NONE() OP_STREAM_NEW(42)
            OP_STREAM_NEW(43) OP_CONN_TXFC_BUMP(10000) OP_STREAM_TXFC_BUMP(
                42, 5000) OP_STREAM_TXFC_BUMP(43, 5000)
...
static const struct script_op script_13[] = {
    OP_PROVIDE_SECRET(QUIC_ENC_LEVEL_1RTT, QRL_SUITE_AES128GCM, secret_1)
        OP_HANDSHAKE_COMPLETE() OP_TXP_GENERATE_NONE() OP_STREAM_NEW(42)
            OP_CONN_TXFC_BUMP(8) OP_STREAM_TXFC_BUMP(42, 8) OP_STREAM_SEND(
                42, stream_13) OP_RESET_STREAM(42, 4568) OP_TXP_GENERATE()
...
static int try_big_token(struct helper* h) {
  ...
  for (i = sizeof(big_token) - 1;; --i) {
    if (!TEST_size_t_gt(i, 0)) return 0;
  ...
  for (op = script, opn = 0; op->opcode != OPK_END; ++op, ++opn) {
    switch (op->opcode) {
      ...
      case OPK_RX_PKT:
        ...
        ossl_qrx_pkt_release(h.qrx_pkt);
        h.qrx_pkt = NULL;
        if (!TEST_true(ossl_qrx_read_pkt(h.qrx, &h.qrx_pkt))) goto err;
      ...
      case OPK_RX_PKT_NONE:
        ossl_quic_demux_pump(h.demux);
        if (!TEST_false(ossl_qrx_read_pkt(h.qrx, &h.qrx_pkt))) goto err;
        h.frame_type = UINT64_MAX;
        break;
      ...
      case OPK_EXPECT_FRAME:
        if (!TEST_uint64_t_eq(h.frame_type, op->arg0)) goto err;
        break;
      ...
      case OPK_EXPECT_HDR:
        ...
        break;
      case OPK_CHECK:
        if (!TEST_true(op->check_func(&h))) goto err;
        break;
      case OPK_NEXT_FRAME:
        skip_padding(&h);
        ...
        switch (h.frame_type) {
          ...
          case OSSL_QUIC_FRAME_TYPE_PING:
            if (!TEST_true(ossl_quic_wire_decode_frame_ping(&h.pkt))) goto err;
            break;
          ...
          case OSSL_QUIC_FRAME_TYPE_CRYPTO:
            if (!TEST_true(ossl_quic_wire_decode_frame_crypto(&h.pkt, 0,
                                                              &h.frame.crypto)))
              goto err;
            break;
          ...
          case OSSL_QUIC_FRAME_TYPE_STREAM_OFF_LEN_FIN:
            if (!TEST_true(ossl_quic_wire_decode_frame_stream(&h.pkt, 0,
                                                              &h.frame.stream)))
              goto err;
            break;
      ...
      case OPK_EXPECT_NO_FRAME:
        skip_padding(&h);
        if (!TEST_size_t_eq(PACKET_remaining(&h.pkt), 0)) goto err;
        break;
      ...
      case OPK_CRYPTO_SEND: {
        ...
        if (!TEST_size_t_eq(consumed, op->buf_len)) goto err;
      } break;
      case OPK_STREAM_NEW: {
        QUIC_STREAM* s;
      ...
      case OPK_STOP_SENDING: {
        ...
        if (!TEST_true(s->active)) goto err;
      } break;
      ...
      case OPK_STREAM_TXFC_BUMP: {
        ...
        if (!TEST_true(ossl_quic_txfc_bump_cwm(&s->txfc, op->arg0))) goto err;
...
err:
  ...
  if (have_helper) helper_cleanup(&h);
  return testresult;
}
