...
static int bio_addr_is_eq(const BIO_ADDR* a, const BIO_ADDR* b) {
  ...
  if (!BIO_ADDR_rawaddress(b, NULL, &blen)) goto err;
  ...
  if (!TEST_ptr(adata) || !BIO_ADDR_rawaddress(a, adata, &alen)) goto err;

  bdata = OPENSSL_malloc(blen);
  if (!TEST_ptr(bdata) || !BIO_ADDR_rawaddress(b, bdata, &blen)) goto err;
...
static int test_bio_addr_copy_dup(int idx) {
  ...
  if (!TEST_ptr(src)) return 0;

  if (docopy) {
    dst = BIO_ADDR_new();
    if (!TEST_ptr(dst)) goto err;

    if (!TEST_true(BIO_ADDR_copy(dst, src))) goto err;
  } else {
    dst = BIO_ADDR_dup(src);
    if (!TEST_ptr(dst)) goto err;
  }

  if (!TEST_true(bio_addr_is_eq(src, dst))) goto err;
