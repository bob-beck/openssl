...
static int test_do_updatedb(void) {
  ...
  if (rv > 0) {
    if (!TEST_true(save_index(indexfile, "new", db))) goto end;

    if (!TEST_true(rotate_index(indexfile, "new", "old"))) goto end;
  }
end:
  free_index(db);
  return 1;
...
int setup_tests(void) {
  ...
  if (strcmp(command, "do_updatedb") == 0) return test_do_updatedb();
