...
static int test_print_error_format(void) {
  ...
  if (!TEST_int_gt(len = BIO_get_mem_data(bio, &out), 0)) goto err;
  /* Skip over the variable thread id at the start of the string */
  for (p = out; *p != ':' && *p != 0; ++p) {
    if (!TEST_true(IS_HEX(*p))) goto err;
  }
...
static int test_marks(void) {
  ...
  if (!TEST_ulong_gt(mallocfail, 0)) return 0;
  ...
  /* Test popping errors */
  if (!TEST_true(ERR_set_mark())) return 0;
  ...
  if (!TEST_true(ERR_set_mark()) || !TEST_true(ERR_set_mark())) return 0;
  ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
  if (!TEST_ulong_ne(mallocfail, ERR_peek_last_error()) ||
   ...
   */
  if (!TEST_false(ERR_set_mark())) return 0;
...
static int test_clear_error(void) {
  ...
  ERR_raise(0, 0);
  ERR_peek_error_data(&data, &flags);
  if (!TEST_str_eq(data, "") || !TEST_int_eq(flags, ERR_TXT_MALLOCED)) goto err;
  ERR_clear_error();
...
static int test_save_restore(int idx) {
  ...
  if (!TEST_ptr(es = OSSL_ERR_STATE_new())) goto err;

  ERR_raise(ERR_LIB_CRYPTO, ERR_R_MALLOC_FAILURE);
  mallocfail = ERR_peek_last_error();
  if (!TEST_ulong_gt(mallocfail, 0)) goto err;

  if (idx == 1 && !TEST_int_eq(ERR_set_mark(), 1)) goto err;

  ERR_raise_data(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR, testdata);
  interr = ERR_peek_last_error();
  if (!TEST_ulong_ne(mallocfail, ERR_peek_last_error())) goto err;

  if (idx == 0) {
    OSSL_ERR_STATE_save(es);

    if (!TEST_ulong_eq(ERR_peek_last_error(), 0)) goto err;
  } else {
    OSSL_ERR_STATE_save_to_mark(es);

    if (!TEST_ulong_ne(ERR_peek_last_error(), 0)) goto err;
  }

  for (i = 0; i < 2; i++) {
    OSSL_ERR_STATE_restore(es);

    if (!TEST_ulong_eq(ERR_peek_last_error(), interr)) goto err;
    ...
    if (!TEST_ulong_eq(ERR_get_error(), 0)) goto err;
  }
