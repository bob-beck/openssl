...
static int test_WPACKET_init(void) {
  ...
  if (!TEST_true(WPACKET_init_len(&pkt, buf, 1))) return cleanup(&pkt);
  for (i = 1; i < 257; i++) {
    /*
  ...
  if (!TEST_true(WPACKET_finish(&pkt))) return cleanup(&pkt);
...
static int test_WPACKET_init_der(void) {
  ...
  for (i = 0; i < 2; i++) {
    if (i == 0) {
      if (!TEST_true(WPACKET_init_null_der(&pkt))) return 0;
    } else {
      if (!TEST_true(WPACKET_init_der(&pkt, sbuf, sizeof(sbuf)))) return 0;
    }
...
int setup_tests(void) {
  if (!TEST_ptr(buf = BUF_MEM_new())) return 0;
