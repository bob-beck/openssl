...
static int test_WPACKET_quic(void)
{
    ...
    if (!TEST_true(WPACKET_init(&pkt, buf)) ||
        ...
        !TEST_true(WPACKET_get_total_written(&pkt, &written)) || !TEST_mem_eq(buf->data, written, quic2, sizeof(quic2)))
        return cleanup(&pkt);
    ...
    if (!TEST_true(WPACKET_init(&pkt, buf)) ||
        ...
        !TEST_true(WPACKET_close(&pkt)) || !TEST_false(WPACKET_close(&pkt)) || !TEST_true(WPACKET_finish(&pkt)) ||
        !TEST_true(WPACKET_get_total_written(&pkt, &written)) || !TEST_mem_eq(buf->data, written, quic3, sizeof(quic3)))
        return cleanup(&pkt);
    ...
    if (!TEST_true(WPACKET_init(&pkt, buf)) ||
        ...
        !TEST_true(WPACKET_close(&pkt)) || !TEST_false(WPACKET_close(&pkt)) || !TEST_true(WPACKET_finish(&pkt)) ||
        !TEST_true(WPACKET_get_total_written(&pkt, &written)) || !TEST_mem_eq(buf->data, written, quic4, sizeof(quic4)))
        return cleanup(&pkt);
    ...
    if (!TEST_true(WPACKET_init(&pkt, buf)) ||
        ...
        !TEST_true(WPACKET_close(&pkt)) || !TEST_false(WPACKET_close(&pkt)) || !TEST_true(WPACKET_finish(&pkt)) ||
        !TEST_true(WPACKET_get_total_written(&pkt, &written)) || !TEST_mem_eq(buf->data, written, quic5, sizeof(quic5)))
        return cleanup(&pkt);
