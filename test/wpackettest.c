...
static int
test_WPACKET_set_max_size(void)
{
  WPACKET pkt;
  size_t written;
...
static int
test_WPACKET_start_sub_packet(void)
{
  WPACKET pkt;
  size_t written;
  size_t len;
...
static int
test_WPACKET_allocate_bytes(void)
{
  WPACKET pkt;
  size_t written;
  unsigned char* bytes;
...
test_WPACKET_init_der(void)
{
  ...
  if (!TEST_true(WPACKET_init_der(&pkt, sbuf, sizeof(sbuf))) ||
      ...
      !TEST_mem_eq(
        WPACKET_get_curr(&pkt), written[0], simpleder, sizeof(simpleder)))
    return cleanup(&pkt);
  ...
  if (!TEST_size_t_eq(written[0], written[1]) ||
      !TEST_mem_eq(
        WPACKET_get_curr(&pkt), written[1], testdata2, sizeof(testdata2)))
    return 0;
...
static int
test_WPACKET_quic_vlint_random(void)
{
  size_t i, written;
