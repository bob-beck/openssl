...
encode_case_3_dec (PACKET *pkt, ossl_ssize_t fail)
{
  ...
  if (!TEST_int_eq (
          ossl_quic_wire_decode_frame_ack (pkt, 3, &f, &total_ranges),
          fail < 0))
    return 0;
...
encode_case_4_dec (PACKET *pkt, ossl_ssize_t fail)
{
  ...
  if (!TEST_mem_eq (&f, sizeof (f), &encode_case_4_f,
...
encode_case_5_dec (PACKET *pkt, ossl_ssize_t fail)
{
  ...
  if (!TEST_mem_eq (&f, sizeof (f), &encode_case_5_f,
...
{
  if (!TEST_int_eq (
          ossl_quic_wire_encode_frame_new_token (pkt, encode_case_7_token,
...
static const OSSL_QUIC_FRAME_NEW_CONN_ID encode_case_16_f
    = { 0x9781,
        ...
        { { 0xde, 0x06, 0xcb, 0x76, 0x5d, 0xb1, 0xa7, 0x71, 0x78, 0x09, 0xbb,
            0xe8, 0x50, 0x19, 0x12, 0x9a } } };
...
encode_case_19_dec (PACKET *pkt, ossl_ssize_t fail)
{
  ...
  if (!TEST_int_eq (
          ossl_quic_wire_decode_frame_path_response (pkt, &challenge),
          fail < 0))
    return 0;
...
encode_case_20_dec (PACKET *pkt, ossl_ssize_t fail)
{
  ...
  if (!TEST_int_eq (ossl_quic_wire_decode_frame_conn_close (pkt, &f),
                    fail < 0))
    return 0;
...
encode_case_21_dec (PACKET *pkt, ossl_ssize_t fail)
{
  ...
  if (!TEST_int_eq (ossl_quic_wire_decode_frame_handshake_done (pkt),
                    fail < 0))
    return 0;
...
static const unsigned char encode_case_22_data[]
...
static const struct encode_test_case encode_cases[]
    = { ENCODE_CASE (1) ENCODE_CASE (2) ENCODE_CASE (3) ENCODE_CASE (4)
            ENCODE_CASE (5) ENCODE_CASE (6) ENCODE_CASE (7) ENCODE_CASE (8)
                ENCODE_CASE (9) ENCODE_CASE (10) ENCODE_CASE (11)
                    ENCODE_CASE (12) ENCODE_CASE (13) ENCODE_CASE (14)
                        ENCODE_CASE (15) ENCODE_CASE (16) ENCODE_CASE (16b)
                            ENCODE_CASE (17) ENCODE_CASE (18) ENCODE_CASE (19)
                                ENCODE_CASE (20) ENCODE_CASE (21)
...
test_wire_encode (int idx)
{
  ...
  if (!TEST_int_eq (
          PACKET_buf_init (&pkt, (unsigned char *)buf->data, written), 1))
    goto err;
...
#define ACK_CASE(n, expect_fail, dec)                                         \
  { ack_case_##n##_input, sizeof (ack_case_##n##_input), (dec),               \
    (expect_fail) },
...
test_wire_retry_integrity_tag (void)
{
  ...
  if (!TEST_true (
          PACKET_buf_init (&pkt, retry_encoded, sizeof (retry_encoded))))
    goto err;
