...
static int
encode_case_1_dec(PACKET* pkt, ossl_ssize_t fail)
{
  if (fail >= 0)
...
static int
encode_case_2_dec(PACKET* pkt, ossl_ssize_t fail)
{
...
/* 3. ACK */
static const OSSL_QUIC_ACK_RANGE encode_case_3_ranges[] = { { 20, 30 },
                                                            { 0, 10 } };
...
static int
encode_case_3_dec(PACKET* pkt, ossl_ssize_t fail)
{
...
/* 4. RESET_STREAM */
static const OSSL_QUIC_FRAME_RESET_STREAM encode_case_4_f = { 0x1234,
                                                              0x9781,
                                                              0x11717 };

static int
encode_case_4_enc(WPACKET* pkt)
{
  if (!TEST_int_eq(
...
static int
encode_case_4_dec(PACKET* pkt, ossl_ssize_t fail)
{
...
static int
encode_case_5_dec(PACKET* pkt, ossl_ssize_t fail)
{
...
static const OSSL_QUIC_FRAME_CRYPTO encode_case_6_f = { 0x1234,
                                                        sizeof(
                                                          encode_case_6_data),
                                                        encode_case_6_data };

static int
encode_case_6_enc(WPACKET* pkt)
{
...
static int
encode_case_6_dec(PACKET* pkt, ossl_ssize_t fail)
{
  ...
  if (!TEST_mem_eq(
        f.data, (size_t)f.len, encode_case_6_data, sizeof(encode_case_6_data)))
    return 0;
...
/* 7. NEW_TOKEN */
static const unsigned char encode_case_7_token[] = { 0xde, 0x06, 0xcb, 0x76,
                                                     0x5d, 0xb1, 0xa7, 0x71,
                                                     0x78, 0x09, 0xbb, 0xe8,
                                                     0x50, 0x19, 0x12, 0x9a };

static int
encode_case_7_enc(WPACKET* pkt)
{
...
static int
encode_case_7_dec(PACKET* pkt, ossl_ssize_t fail)
{
  ...
  if (!TEST_mem_eq(
        token, token_len, encode_case_7_token, sizeof(encode_case_7_token)))
    return 0;
...
static const unsigned char encode_case_8_data[] = { 0xde,
                                                    0x06,
                                                    0xcb,
                                                    0x76,
                                                    0x5d };
...
static int
encode_case_8_dec(PACKET* pkt, ossl_ssize_t fail)
{
  ...
  if (!TEST_mem_eq(
        f.data, (size_t)f.len, encode_case_8_data, sizeof(encode_case_8_data)))
    return 0;
...
static const unsigned char encode_case_9_data[] = { 0xde,
                                                    0x06,
                                                    0xcb,
                                                    0x76,
                                                    0x5d };
...
static int
encode_case_9_dec(PACKET* pkt, ossl_ssize_t fail)
{
  OSSL_QUIC_FRAME_STREAM f = { 0 };
  ...
  if (!TEST_mem_eq(
        f.data, (size_t)f.len, encode_case_9_data, sizeof(encode_case_9_data)))
    return 0;
...
/* 10. MAX_DATA */
static int
encode_case_10_enc(WPACKET* pkt)
{
...
static int
encode_case_10_dec(PACKET* pkt, ossl_ssize_t fail)
{
  uint64_t max_data = 0;
...
/* 11. MAX_STREAM_DATA */
static int
encode_case_11_enc(WPACKET* pkt)
{
  if (!TEST_int_eq(
...
static int
encode_case_11_dec(PACKET* pkt, ossl_ssize_t fail)
{
...
/* 12. MAX_STREAMS */
static int
encode_case_12_enc(WPACKET* pkt)
{
...
static int
encode_case_12_dec(PACKET* pkt, ossl_ssize_t fail)
{
...
/* 13. DATA_BLOCKED */
static int
encode_case_13_enc(WPACKET* pkt)
{
...
static int
encode_case_13_dec(PACKET* pkt, ossl_ssize_t fail)
{
  uint64_t max_data = 0;
...
static int
encode_case_14_enc(WPACKET* pkt)
{
  if (!TEST_int_eq(
...
static int
encode_case_14_dec(PACKET* pkt, ossl_ssize_t fail)
{
  uint64_t stream_id = 0, max_data = 0;
...
/* 15. STREAMS_BLOCKED */
static int
encode_case_15_enc(WPACKET* pkt)
{
...
static int
encode_case_15_dec(PACKET* pkt, ossl_ssize_t fail)
{
  uint64_t max_streams_1 = 0, max_streams_2 = 0, frame_type_1 = 0,
           frame_type_2 = 0;
  int is_minimal = 1, success_if;
...
static const unsigned char encode_case_16_conn_id[] = { 0x33,
                                                        0x44,
                                                        0x55,
                                                        0x66 };
...
static int
encode_case_16_enc(WPACKET* pkt)
{
  if (!TEST_int_eq(
...
static int
encode_case_16_dec(PACKET* pkt, ossl_ssize_t fail)
{
...
static int
encode_case_16b_enc(WPACKET* pkt)
{
  if (!TEST_int_eq(
...
static int
encode_case_16b_dec(PACKET* pkt, ossl_ssize_t fail)
{
  OSSL_QUIC_FRAME_NEW_CONN_ID f = { 0 };
...
static int
encode_case_17_enc(WPACKET* pkt)
{
...
static int
encode_case_17_dec(PACKET* pkt, ossl_ssize_t fail)
{
  uint64_t seq_num = 0;
...
static int
encode_case_18_enc(WPACKET* pkt)
{
  if (!TEST_int_eq(
...
static int
encode_case_18_dec(PACKET* pkt, ossl_ssize_t fail)
{
...
static int
encode_case_19_enc(WPACKET* pkt)
{
  if (!TEST_int_eq(
...
static int
encode_case_19_dec(PACKET* pkt, ossl_ssize_t fail)
{
  uint64_t challenge = 0;
...
static int
encode_case_20_enc(WPACKET* pkt)
{
  if (!TEST_int_eq(
...
static int
encode_case_20_dec(PACKET* pkt, ossl_ssize_t fail)
{
  ...
  if (!TEST_mem_eq(f.reason,
                   f.reason_len,
                   encode_case_20_f.reason,
...
/* 21. HANDSHAKE_DONE */
static int
encode_case_21_enc(WPACKET* pkt)
{
...
static int
encode_case_21_dec(PACKET* pkt, ossl_ssize_t fail)
{
...
static const unsigned char encode_case_22_data[] = { 0x55,
                                                     0x77,
                                                     0x32,
                                                     0x46,
                                                     0x99 };
...
static int
encode_case_22_enc(WPACKET* pkt)
{
  unsigned char* p;
...
static int
encode_case_22_dec(PACKET* pkt, ossl_ssize_t fail)
{
  uint64_t id = 0;
  size_t len = 0;
...
static int
encode_case_23_enc(WPACKET* pkt)
{
  if (!TEST_int_eq(
...
static int
encode_case_23_dec(PACKET* pkt, ossl_ssize_t fail)
{
...
static const struct encode_test_case encode_cases[] = {
  ENCODE_CASE(1) ENCODE_CASE(2) ENCODE_CASE(3) ENCODE_CASE(4) ENCODE_CASE(5)
    ENCODE_CASE(6) ENCODE_CASE(7) ENCODE_CASE(8) ENCODE_CASE(9) ENCODE_CASE(10)
      ENCODE_CASE(11) ENCODE_CASE(12) ENCODE_CASE(13) ENCODE_CASE(14)
        ENCODE_CASE(15) ENCODE_CASE(16) ENCODE_CASE(16b) ENCODE_CASE(17)
          ENCODE_CASE(18) ENCODE_CASE(19) ENCODE_CASE(20) ENCODE_CASE(21)
            ENCODE_CASE(22) ENCODE_CASE(23)
};

static int
test_wire_encode(int idx)
{
  int testresult = 0;
  WPACKET wpkt;
  PACKET pkt;
...
test_wire_pkt_hdr_pn(int tidx)
{
  ...
  if (!TEST_true(ossl_quic_wire_decode_pkt_hdr_pn(
        buf, pn_len, t->rx_largest_pn, &res_pn)))
    goto err;
...
static int
test_wire_retry_integrity_tag(void)
{
  int testresult = 0;
  PACKET pkt = { 0 };
  QUIC_PKT_HDR hdr = { 0 };
  ...
  if (!TEST_true(ossl_quic_validate_retry_integrity_tag(
        NULL, NULL, &hdr, &retry_orig_dcid)))
    goto err;
