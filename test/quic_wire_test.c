...
static int encode_case_1_enc(WPACKET* pkt) {
  if (!TEST_int_eq(ossl_quic_wire_encode_padding(pkt, 3), 1)) return 0;

  return 1;
}

static int encode_case_1_dec(PACKET* pkt, ossl_ssize_t fail) {
  if (fail >= 0) /* No failure modes for padding */
    return 1;

  if (!TEST_size_t_eq(ossl_quic_wire_decode_padding(pkt), 3)) return 0;
...
static int encode_case_2_enc(WPACKET* pkt) {
  if (!TEST_int_eq(ossl_quic_wire_encode_frame_ping(pkt), 1)) return 0;

  return 1;
}

static int encode_case_2_dec(PACKET* pkt, ossl_ssize_t fail) {
  if (!TEST_int_eq(ossl_quic_wire_decode_frame_ping(pkt), fail < 0)) return 0;
...
static int encode_case_3_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (fail < 0 && !TEST_int_eq(ret, 1)) return 0;
...
static int encode_case_6_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_uint64_t_eq(f.offset, 0x1234)) return 0;
...
static int encode_case_8_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_int_eq(f.has_explicit_len, 0)) return 0;
...
static int encode_case_9_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_uint64_t_eq(f.offset, 0x39)) return 0;

  if (!TEST_int_eq(f.has_explicit_len, 1)) return 0;

  if (!TEST_int_eq(f.is_fin, 1)) return 0;
...
static int encode_case_10_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_uint64_t_eq(max_data, 0x1234)) return 0;
...
static int encode_case_11_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_int_eq(ossl_quic_wire_decode_frame_max_stream_data(pkt, &stream_id,
                                                               &max_data),
                   fail < 0))
    return 0;
  ...
  if (!TEST_uint64_t_eq(stream_id, 0x1234)) return 0;

  if (!TEST_uint64_t_eq(max_data, 0x9781)) return 0;
...
static int encode_case_16_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_uint64_t_eq(f.seq_num, 0x9781)) return 0;
...
static int encode_case_16b_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_true(PACKET_forward(pkt, PACKET_remaining(pkt)))) return 0;
...
static int encode_case_17_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_uint64_t_eq(seq_num, 0x1234)) return 0;
...
static int encode_case_18_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_uint64_t_eq(challenge, encode_case_18_data)) return 0;
...
static int encode_case_19_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_uint64_t_eq(challenge, encode_case_19_data)) return 0;
...
static int encode_case_20_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (!TEST_int_eq(f.is_app, 0)) return 0;

  if (!TEST_uint64_t_eq(f.error_code, 0x1234)) return 0;

  if (!TEST_uint64_t_eq(f.frame_type, 0x9781)) return 0;
...
static int encode_case_22_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if (fail < 0 || fail >= 8) {
    if (!TEST_ptr(p)) return 0;
  } else {
    if (!TEST_ptr_null(p)) return 0;
  }
  ...
  if ((fail < 0 || fail >= 12) && !TEST_uint64_t_eq(id, 0x9781)) return 0;
  ...
  if ((fail < 0 || fail >= 15) && !TEST_uint64_t_eq(id, 0x9781)) return 0;
...
static int encode_case_23_dec(PACKET* pkt, ossl_ssize_t fail) {
  ...
  if ((fail < 0 || fail >= 7) && !TEST_uint64_t_eq(id, 0x1234)) return 0;

  if ((fail < 0 || fail >= 7) && !TEST_uint64_t_eq(value, 0x9781)) return 0;
...
static int test_wire_encode(int idx) {
  ...
  if (!TEST_int_eq(WPACKET_init(&wpkt, buf), 1)) goto err;

  have_wpkt = 1;
  if (!TEST_int_eq(c->serializer(&wpkt), 1)) goto err;

  if (!TEST_int_eq(WPACKET_get_total_written(&wpkt, &written), 1)) goto err;
  ...
  if (!TEST_int_eq(c->deserializer(&pkt, -1), 1)) goto err;

  if (!TEST_false(PACKET_remaining(&pkt))) goto err;
  ...
  for (i = 0; i < c->expect_buf_len; ++i) {
    ...
    if (!TEST_int_eq(c->deserializer(&pkt2, i), 1)) goto err;
  }
...
static int ack_generic_decode(PACKET* pkt) {
  ...
  for (i = 0; i < f.num_ack_ranges; ++i) {
    if (!TEST_uint64_t_le(f.ack_ranges[i].start, f.ack_ranges[i].end)) return 0;
    if (!TEST_uint64_t_lt(f.ack_ranges[i].end, 1000)) return 0;
  }
...
static const struct ack_test_case ack_cases[] = {
    ACK_CASE(1, 1, ack_generic_decode) ACK_CASE(2, 0, ack_generic_decode)
        ACK_CASE(3, 1, ack_generic_decode) ACK_CASE(4, 0, ack_generic_decode)
            ACK_CASE(5, 1, ack_generic_decode) ACK_CASE(
                6, 1, ack_generic_decode) ACK_CASE(7, 0, ack_generic_decode)};
...
static int test_wire_pkt_hdr_pn(int tidx) {
  ...
  if (!TEST_int_eq(pn_len, (int)t->expected_len)) goto err;

  if (!TEST_true(ossl_quic_wire_encode_pkt_hdr_pn(t->pn, buf, pn_len)))
    goto err;

  if (!TEST_mem_eq(t->expected_bytes, t->expected_len, buf, pn_len)) goto err;

  if (!TEST_true(ossl_quic_wire_decode_pkt_hdr_pn(buf, pn_len, t->rx_largest_pn,
                                                  &res_pn)))
    goto err;

  if (!TEST_uint64_t_eq(res_pn, t->pn)) goto err;
...
static int test_wire_retry_integrity_tag(void) {
  ...
  if (!TEST_int_eq(hdr.type, QUIC_PKT_TYPE_RETRY)) goto err;
...
static int test_wire_minimal(int idx) {
  ...
  if (!TEST_false(is_minimal)) goto err;
