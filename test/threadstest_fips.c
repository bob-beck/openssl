...
static void thread_fips_rand_fetch(void) {
  ...
  if (!TEST_true(md = EVP_MD_fetch(NULL, "SHA2-256", NULL))) success = 0;
  EVP_MD_free(md);
}
...
static int test_fips_rand_leak(void) {
  ...
  if (!TEST_true(run_thread(&thread, thread_fips_rand_fetch))) return 0;
  if (!TEST_true(wait_for_thread(thread))) return 0;
