...
static int
param_conversion_load_stanza(PARAM_CONVERSION* pc, const STANZA* s)
{
  ...
  for (i = 0; i < s->numpairs; i++, pp++) {
    ...
    if (*p != '\0') {
      TEST_info(
        "Line %d: extra characters at end '%s' for %s", s->curr, p, pp->key);
      return 0;
    }
  }
...
static int
param_conversion_test(const PARAM_CONVERSION* pc, int line)
{
  int32_t i32;
  int64_t i64;
  uint32_t u32;
