...
static int
get_cert_and_key (X509 **cert_out, EVP_PKEY **key_out)
{
  BIO *certbio, *keybio;
  X509 *cert = NULL;
  EVP_PKEY *key = NULL;
...
make_dummy_resp (void)
{
  ...
  if (!TEST_ptr (bs) || !TEST_ptr (thisupd) || !TEST_ptr (nextupd)
      || !TEST_ptr (cid)
      || !TEST_true (OCSP_basic_add1_status (
...
static int
test_access_description (int testcase)
{
