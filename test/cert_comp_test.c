...
static void cert_comp_info_cb(const SSL* s, int where, int ret) {
  ...
  if (SSL_is_server(s)) {
    /* TLS_ST_SR_COMP_CERT */
    if (!strcmp(SSL_state_string(s), "TRCCC") && seen != NULL) *seen = 1;
  } else {
    /* TLS_ST_CR_COMP_CERT */
    if (!strcmp(SSL_state_string(s), "TRSCC") && seen != NULL) *seen = 1;
  }
}
...
static int test_ssl_cert_comp(int test) {
  ...
  if (test == 1 || test == 2 || test == 3) {
    if (!TEST_true(SSL_CTX_compress_certs(sctx, expected_server))) goto end;
  }
  ...
  if (!TEST_true(SSL_set_app_data(clientssl, &client_seen))) goto end;
  if (!TEST_true(SSL_set_app_data(serverssl, &server_seen))) goto end;
  ...
  if (test == 3) {
    ...
    if (!TEST_int_eq(sc->cert->key->cert_comp_used, 0)) goto end;

    if (!TEST_false(*(int*)SSL_get_app_data(clientssl))) goto end;
  } else {
    SSL_CONNECTION* sc = SSL_CONNECTION_FROM_SSL(serverssl);

    if (!TEST_int_gt(sc->cert->key->cert_comp_used, 0)) goto end;

    if (!TEST_true(*(int*)SSL_get_app_data(clientssl))) goto end;
  }

  if (test == 2) {
    /* Only for client auth */
    if (!TEST_true(*(int*)SSL_get_app_data(serverssl))) goto end;
  }
