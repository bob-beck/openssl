...
client_cert_cb(SSL* ssl, X509** x509, EVP_PKEY** pkey)
{
  ...
  if (!TEST_ptr(xcert = X509_new_ex(NULL, NULL)) ||
      ...
      !TEST_ptr(privpkey = PEM_read_bio_PrivateKey_ex(
                  priv_in, NULL, NULL, NULL, NULL, NULL)))
    goto err;
...
static void
cert_comp_info_cb(const SSL* s, int where, int ret)
{
...
test_ssl_cert_comp(int test)
{
  ...
  int server_pref[] = { TLSEXT_comp_cert_zstd,
                        TLSEXT_comp_cert_zlib,
                        TLSEXT_comp_cert_brotli };
  /* default order */
  int client_pref[] = { TLSEXT_comp_cert_brotli,
                        TLSEXT_comp_cert_zlib,
