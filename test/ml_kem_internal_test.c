...
sanity_test(void)
{
  ...
  for (i = 0; i < (int)OSSL_NELEM(alg); ++i) {
    ...
    if (!ossl_ml_kem_parse_public_key(
          encoded_public_key, v->pubkey_bytes, public_key))
      goto done;
    ...
    ret2 = -5;
    if (!ossl_ml_kem_encap_rand(ciphertext,
                                v->ctext_bytes,
                                shared_secret,
    ...
    ret2 = -7;
    if (!ossl_ml_kem_decap(shared_secret2,
    ...
    if (!TEST_true(ossl_ml_kem_decap(shared_secret2,
                                     sizeof(shared_secret2),
                                     ciphertext,
                                     v->ctext_bytes,
                                     private_key)))
