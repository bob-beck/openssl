...
static int compare_hex_encoded_buffer(const char* hex_encoded,
  ...
  if (!TEST_size_t_eq(raw_length * 2, hex_length)) return 1;
...
static int test_keylog_output(char* buffer, const SSL* ssl,
  ...
  for (token = strtok(buffer, " \n"); token != NULL;
    ...
    if (strcmp(token, "RSA") == 0) {
      ...
      if (!TEST_size_t_eq(strlen(token), 16)) return 0;
      if (!TEST_ptr(token = strtok(NULL, " \n"))) return 0;
      /*
      ...
      if (!TEST_size_t_eq(client_random_size, SSL3_RANDOM_SIZE)) return 0;

      if (!TEST_ptr(token = strtok(NULL, " \n"))) return 0;
      if (!TEST_size_t_eq(strlen(token), 64)) return 0;
      if (!TEST_false(compare_hex_encoded_buffer(
              token, 64, actual_client_random, client_random_size)))
        return 0;

      if (!TEST_ptr(token = strtok(NULL, " \n"))) return 0;
      master_key_size = SSL_SESSION_get_master_key(session, actual_master_key,
                                                   master_key_size);
      if (!TEST_size_t_ne(master_key_size, 0)) return 0;
      if (!TEST_false(compare_hex_encoded_buffer(
...
static int test_keylog(void) {
  ...
  if (!TEST_true(SSL_CTX_set_cipher_list(cctx, "RSA"))) goto end;
...
static int verify_retry_cb(X509_STORE_CTX* ctx, void* arg) {
  ...
  if (idx < 0 || (ssl = X509_STORE_CTX_get_ex_data(ctx, idx)) == NULL) return 0;
...
static int test_client_cert_verify_cb(void) {
  ...
  if (!TEST_int_eq(SSL_CTX_use_certificate_chain_file(sctx, leaf), 1) ||
    ...
    goto end;
  if (!TEST_true(SSL_CTX_load_verify_locations(cctx, root, NULL))) goto end;
  ...
  if (!TEST_true(sk_X509_push(server_chain, crt1))) goto end;
  crt1 = NULL;
  if (!TEST_true(sk_X509_push(server_chain, crt2))) goto end;
  crt2 = NULL;
...
static int test_ssl_build_cert_chain(void) {
  ...
  if (!TEST_ptr(ssl_ctx = SSL_CTX_new_ex(libctx, NULL, TLS_server_method())))
    goto end;
  if (!TEST_ptr(ssl = SSL_new(ssl_ctx))) goto end;
...
static int get_password_cb(char* buf, int size, int rw_flag, void* userdata) {
  ...
  if (!TEST_int_eq(size, PEM_BUFSIZE)) return -1;
...
static int full_client_hello_callback(SSL* s, int* al, void* arg) {
  ...
  if ((*ctr)++ == 0) return SSL_CLIENT_HELLO_RETRY;
...
static int test_ccs_change_cipher(void) {
  ...
  for (i = 0; i < 3; i++) {
    if (SSL_read_ex(clientssl, &buf, sizeof(buf), &readbytes) > 0) {
      if (!TEST_size_t_eq(readbytes, 0)) goto end;
...
static int execute_test_large_message(const SSL_METHOD* smeth,
  ...
  if (!ssl_ctx_add_large_cert_chain(libctx, sctx, cert)) goto end;
   ...
   * doesn't leak.
   */
  if (!TEST_true(SSL_clear(serverssl))) goto end;
...
static int ping_pong_query(SSL* clientssl, SSL* serverssl) {
  ...
  /* verify the payload */
  if (!TEST_mem_eq(cbuf, sizeof(cbuf), sbuf, sizeof(sbuf))) goto end;
...
static int execute_test_ktls(int cis_ktls, int sis_ktls, int tls_version,
  ...
  if (!TEST_true(create_test_sockets(&cfd, &sfd, SOCK_STREAM, NULL))) goto end;
  ...
  if (cis_ktls) {
    if (!TEST_true(SSL_set_options(clientssl, SSL_OP_ENABLE_KTLS))) goto end;
  }

  if (sis_ktls) {
    if (!TEST_true(SSL_set_options(serverssl, SSL_OP_ENABLE_KTLS))) goto end;
  }
   ...
   */
  if (!cis_ktls) {
    if (!TEST_false(BIO_get_ktls_send(clientsc->wbio))) goto end;
  } else {
    if (BIO_get_ktls_send(clientsc->wbio)) ktls_used = 1;
  }

  if (!sis_ktls) {
    if (!TEST_false(BIO_get_ktls_send(serversc->wbio))) goto end;
  } else {
    if (BIO_get_ktls_send(serversc->wbio)) ktls_used = 1;
  }
  ...
  if (!cis_ktls || !rx_supported) {
    if (!TEST_false(BIO_get_ktls_recv(clientsc->rbio))) goto end;
  } else {
    if (BIO_get_ktls_send(clientsc->rbio)) ktls_used = 1;
  }

  if (!sis_ktls || !rx_supported) {
    if (!TEST_false(BIO_get_ktls_recv(serversc->rbio))) goto end;
  } else {
    if (BIO_get_ktls_send(serversc->rbio)) ktls_used = 1;
  }
  ...
  if (!TEST_true(ping_pong_query(clientssl, serverssl))) goto end;

  buf = OPENSSL_zalloc(bufsz);
  if (!TEST_ptr(buf)) goto end;
  ...
  if (!TEST_true(offset == bufsz)) goto end;
  for (i = 0; i < bufsz; i++)
    if (!TEST_true(buf[i] == 0)) goto end;
...
static int execute_test_ktls_sendfile(int tls_version, const char* cipher,
  ...
  if (!TEST_ptr(serversc = SSL_CONNECTION_FROM_SSL_ONLY(serverssl))) goto end;
  ...
  if (!TEST_int_gt(RAND_bytes_ex(libctx, buf, SENDFILE_SZ, 0), 0)) goto end;

  out = BIO_new_file(tmpfilename, "wb");
  if (!TEST_ptr(out)) goto end;

  if (BIO_write(out, buf, SENDFILE_SZ) != SENDFILE_SZ) goto end;
  ...
  while (chunk_off < SENDFILE_SZ) {
    ...
    while ((err = SSL_sendfile(serverssl, ffd, chunk_off, chunk_size, 0)) !=
           chunk_size) {
      if (SSL_get_error(serverssl, err) != SSL_ERROR_WANT_WRITE) goto end;
    }
    while ((err = SSL_read(clientssl, buf_dst + chunk_off, chunk_size)) !=
           chunk_size) {
      if (SSL_get_error(clientssl, err) != SSL_ERROR_WANT_READ) goto end;
    }
...
static int test_large_app_data(int tst) {
  ...
  switch (tst >> 2) {
    ...
    case 6:
      ...
      if (is_fips) return TEST_skip("DTLS 1 not supported by FIPS provider");
      prot = DTLS1_VERSION;
      smeth = DTLS_server_method();
      cmeth = DTLS_client_method();
      break;
  ...
  if (!TEST_mem_eq(msg, written, buf, readbytes)) goto end;
...
static int execute_cleanse_plaintext(const SSL_METHOD* smeth,
  ...
  zbuf = &rr->data[rr->off];
  if (!TEST_size_t_eq(rr->length, sizeof(cbuf))) goto end;

  /*
   * After SSL_peek() the plaintext must still be stored in the
   * record.
   */
  if (!TEST_mem_eq(cbuf, sizeof(cbuf), zbuf, sizeof(cbuf))) goto end;

  memset(sbuf, 0, sizeof(sbuf));
  if (!TEST_int_eq(SSL_read(serverssl, &sbuf, sizeof(sbuf)), sizeof(sbuf)))
    goto end;

  if (!TEST_mem_eq(cbuf, sizeof(cbuf), sbuf, sizeof(cbuf))) goto end;
...
static OCSP_RESPONSE* create_ocsp_resp(X509* ssl_cert, X509* issuer, int status,
                                       char* signer_key_files,
                                       char* signer_cert_files) {
...
static int ocsp_server_cb_single(SSL* s, void* arg) {
  ...
  ocsp_resp = create_ocsp_resp(ssl_cert, issuer, V_OCSP_CERTSTATUS_GOOD,
                               "subinterCA.key", "subinterCA.pem");
  if (!TEST_ptr(ocsp_resp)) return SSL_TLSEXT_ERR_ALERT_FATAL;
...
static int ocsp_client_cb_single(SSL* s, void* arg) {
  ...
  if (resp_status != OCSP_RESPONSE_STATUS_SUCCESSFUL) return 0;
...
static int test_tlsext_status_type(void) {
  ...
  if (!create_ssl_ctx_pair(libctx, TLS_server_method(), TLS_client_method(),
    ...
    goto end;
  if (SSL_CTX_use_certificate_chain_file(sctx, leaf_chain) <= 0) goto end;
  if (SSL_CTX_get_tlsext_status_type(cctx) != -1) goto end;

  /* First just do various checks getting and setting tlsext_status_type */

  clientssl = SSL_new(cctx);
  if (!TEST_ptr(clientssl)) goto end;
  ...
  clientssl = SSL_new(cctx);
  if (!TEST_ptr(clientssl)) goto end;
  if (SSL_get_tlsext_status_type(clientssl) != TLSEXT_STATUSTYPE_ocsp) goto end;
  SSL_free(clientssl);
  clientssl = NULL;
...
static int ocsp_server_cb_multi(SSL* s, void* arg) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (ocsp_server_called == 3 && i == 0)
      ocsp_resp = create_ocsp_resp(
          ssl_cert, issuer, OCSP_RESPONSE_STATUS_MALFORMEDREQUEST, NULL, NULL);
...
static int ocsp_client_cb_multi(SSL* s, void* arg) {
  ...
  if (*argi < 1 || *argi > 3 || num != *argi) return 0;
   ...
   */
  for (i = 0; i < num; i++) {
    if ((ssl_cert = sk_X509_value(server_certs, i)) == NULL) return 0;

    /* for a selfsigned certificate we expect no OCSP response */
    if (X509_self_signed(ssl_cert, 0)) continue;

    if ((resp = sk_OCSP_RESPONSE_value(sk_resp, i)) == NULL) break;
    ...
    if (ocsp_client_called != 3) {
      ...
      if ((bs = OCSP_response_get1_basic(resp)) == NULL) break;
      /* we send a OCSP response with one single response so we check only the
       * first */
      ...
      if (OCSP_resp_find(bs, cert_id, -1) < 0) resp = NULL;
...
static int verify_cb_multi_stapling(int preverify_ok,
  ...
  if (ocsp_server_called == 3 || ocsp_server_called == 4)
    if (ocsp_verify_cb_called == 0 && preverify_ok == 0) res = 1;
...
static int test_tlsext_status_type_multi(void) {
  ...
  if (TEST_int_lt(SSL_CTX_use_certificate_chain_file(sctx, leaf_chain), 0))
    goto end;
  if (!TEST_true(SSL_CTX_load_verify_locations(cctx, root, NULL))) goto end;
  if (TEST_int_ne(SSL_CTX_get_tlsext_status_type(cctx), -1)) goto end;
  ...
  if (!TEST_true(
          SSL_CTX_set_tlsext_status_type(cctx, TLSEXT_STATUSTYPE_ocsp)) ||
  ...
  X509_VERIFY_PARAM_set_flags(
      vpm, X509_V_FLAG_OCSP_RESP_CHECK | X509_V_FLAG_OCSP_RESP_CHECK_ALL);
  if (!TEST_true(SSL_CTX_set1_param(cctx, vpm))) goto end;
  if (!TEST_true(
          create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)) ||
      !TEST_true(create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE)) ||
      !TEST_int_eq(ocsp_client_called, 1) ||
      !TEST_int_eq(ocsp_server_called, 1) || !TEST_true(ocsp_verify_cb_called))
    goto end;
  ...
  if (!TEST_true(
          create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)) ||
      !TEST_true(create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE)) ||
      !TEST_int_eq(ocsp_client_called, 2) ||
      !TEST_int_eq(ocsp_server_called, 2) || !TEST_true(ocsp_verify_cb_called))
    goto end;
  ...
  if (!TEST_true(
          create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)) ||
      !TEST_true(create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE)) ||
      !TEST_int_eq(ocsp_client_called, 3) ||
      !TEST_int_eq(ocsp_server_called, 3) || !TEST_true(ocsp_verify_cb_called))
    goto end;
  ...
  /*
   * In the third test case we set the status of the server certificate to
   * REVOKED. The SSL connection should fail and the ocsp_client_cb_multi should
   * not be called.
   */
  ocsp_verify_cb_called = 0;
  cdummyarg = 3; /* expect three OCSP responses */
  if (!TEST_true(
          create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)) ||
      !TEST_true(create_ssl_connection(serverssl, clientssl, SSL_ERROR_NONE)) ||
      !TEST_int_eq(ocsp_client_called, 4) ||
      !TEST_int_eq(ocsp_server_called, 4) || !TEST_true(ocsp_verify_cb_called))
    goto end;
...
static int execute_test_session(int maxprot, int use_int_cache,
  ...
  if (maxprot == TLS1_3_VERSION) numnewsesstick = 2;
  ...
  if (use_int_cache && !TEST_false(SSL_CTX_add_session(cctx, sess1))) goto end;
  ...
  new_called = remove_called = 0;
  if (!TEST_true(create_ssl_objects(sctx, cctx, &serverssl2, &clientssl2, NULL,
                                    NULL)) ||
  ...
  if (!TEST_ptr(sess1 = SSL_get1_session(clientssl2))) goto end;
  ...
  if (!TEST_ptr(sess2 = SSL_get1_session(clientssl2))) goto end;
   ...
   */
  if (!TEST_true(SSL_set_session(clientssl2, sess1))) goto end;
  if (use_ext_cache &&
      (!TEST_int_eq(new_called, 0) || !TEST_int_eq(remove_called, 1)))
    goto end;
  if (!TEST_ptr_eq(SSL_get_session(clientssl2), sess1)) goto end;
  ...
  if (!TEST_false(SSL_CTX_remove_session(cctx, sess2))) goto end;
  ...
  if (!TEST_true(create_ssl_objects(sctx, cctx, &serverssl3, &clientssl3, NULL,
                                    NULL)) ||
  ...
  if (use_int_cache && !TEST_true(SSL_CTX_add_session(cctx, sess2))) goto end;
#endif
  ...
  if (maxprot == TLS1_2_VERSION) SSL_CTX_set_options(sctx, SSL_OP_NO_TICKET);
  ...
  if (use_int_cache) {
    if (maxprot == TLS1_3_VERSION && !use_ext_cache) {
       ...
       */
      if (!TEST_false(SSL_CTX_remove_session(sctx, sess2))) goto end;
    } else {
      /* Should fail because it should already be in the cache */
      if (!TEST_false(SSL_CTX_add_session(sctx, sess2))) goto end;
    }
  }
  ...
  if (use_ext_cache) {
    if (!TEST_int_eq(remove_called, 0)) goto end;

    if (maxprot == TLS1_3_VERSION) {
      if (!TEST_int_eq(new_called, 1) || !TEST_int_eq(get_called, 0)) goto end;
    } else {
      if (!TEST_int_eq(new_called, 0) || !TEST_int_eq(get_called, 1)) goto end;
    }
  }
  /*
  ...
  if (!TEST_long_ne(SSL_CTX_sess_set_cache_size(sctx, 1), 0)) goto end;
...
#ifndef OSSL_NO_USABLE_TLS1_3
  if (!execute_test_session(TLS1_3_VERSION, 1, 0, 0)) return 0;
#endif
...
#ifndef OSSL_NO_USABLE_TLS1_3
  if (!execute_test_session(TLS1_3_VERSION, 0, 1, 0)) return 0;
#endif
...
#ifndef OSSL_NO_USABLE_TLS1_3
  if (!execute_test_session(TLS1_3_VERSION, 1, 1, 0)) return 0;
#endif
...
static int post_handshake_verify(SSL* sssl, SSL* cssl) {
  ...
  if (!TEST_true(SSL_verify_client_post_handshake(sssl))) return 0;
...
static int setup_ticket_test(int stateful, int idx, SSL_CTX** sctx,
  ...
  if (stateful) SSL_CTX_set_options(*sctx, SSL_OP_NO_TICKET);
...
static int test_tickets(int stateful, int idx) {
  ...
  if (!setup_ticket_test(stateful, idx, &sctx, &cctx)) goto end;
  ...
  if (!check_resumption(idx, sctx, cctx, 0)) goto end;
   ...
   */
  if (!check_resumption(idx, sctx, cctx, 1)) goto end;
...
static int test_extra_tickets(int idx) {
  ...
  if (!TEST_true(SSL_write_ex(serverssl, &c, 1, &nbytes)) ||
      ...
      !TEST_true(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)) ||
      !TEST_int_eq(0, new_called) || !TEST_size_t_eq(sizeof(buf), nbytes) ||
      !TEST_int_eq(c, buf[0]) ||
      !TEST_false(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)))
    goto end;
  ...
  if (!TEST_true(SSL_write_ex(serverssl, &c, 1, &nbytes)) ||
      ...
      !TEST_true(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)) ||
      !TEST_int_eq(4, new_called) || !TEST_size_t_eq(sizeof(buf), nbytes) ||
      !TEST_int_eq(c, buf[0]) ||
      !TEST_false(SSL_read_ex(clientssl, buf, sizeof(buf), &nbytes)))
    goto end;
...
static int test_ssl_set_bio(int idx) {
  ...
  if (initrbio == USE_BIO_1 || initwbio == USE_BIO_1 || newrbio == USE_BIO_1 ||
      newwbio == USE_BIO_1) {
    if (!TEST_ptr(bio1 = BIO_new(BIO_s_mem()))) goto end;
  }

  if (initrbio == USE_BIO_2 || initwbio == USE_BIO_2 || newrbio == USE_BIO_2 ||
      newwbio == USE_BIO_2) {
    if (!TEST_ptr(bio2 = BIO_new(BIO_s_mem()))) goto end;
  }
  ...
  if (initrbio != USE_DEFAULT) {
     ...
     * call
     */
    if (irbio != NULL && !BIO_up_ref(irbio)) goto end;
  ...
  if (nrbio != NULL && nrbio != irbio && (nwbio != iwbio || nrbio != nwbio))
    if (!TEST_true(BIO_up_ref(nrbio))) goto end;
  if (nwbio != NULL && nwbio != nrbio &&
      (nwbio != iwbio || (nwbio == iwbio && irbio == iwbio)))
    if (!TEST_true(BIO_up_ref(nwbio))) {
      if (nrbio != irbio && (nwbio != iwbio || nrbio != nwbio)) BIO_free(nrbio);
      goto end;
    }
...
static int test_set_sigalgs(int idx) {
  ...
  /* Should never happen */
  if (!TEST_size_t_le((size_t)idx, OSSL_NELEM(testsigalgs) * 2)) return 0;
...
static unsigned int psk_client_cb(SSL* ssl, const char* hint, char* id,
  ...
  if (psk_client_cb_cnt > 2) return 0;

  if (clientpsk == NULL) return 0;

  /* We'll reuse the PSK we set up for TLSv1.3 */
  if (SSL_SESSION_get_master_key(clientpsk, NULL, 0) > max_psk_len) return 0;
...
static int find_session_cb(SSL* ssl, const unsigned char* identity,
  ...
  /* We should only ever be called a maximum of twice per connection */
  if (find_session_cb_cnt > 2) return 0;
  ...
  if (!SSL_SESSION_up_ref(serverpsk)) return 0;
...
static unsigned int psk_server_cb(SSL* ssl, const char* identity,
  ...
  /* We'll reuse the PSK we set up for TLSv1.3 */
  if (SSL_SESSION_get_master_key(serverpsk, NULL, 0) > max_psk_len) return 0;
...
static int ed_gen_cb(SSL* s, void* arg) {
   ...
   */
  if (artificial_ticket_time == 0) return 1;
...
static int setupearly_data_test(SSL_CTX** cctx, SSL_CTX** sctx, SSL** clientssl,
  ...
  if (artificial) SSL_CTX_set_session_ticket_cb(*sctx, ed_gen_cb, NULL, NULL);
  ...
  *serverssl = *clientssl = NULL;
   ...
   */
  if (artificial && !TEST_true(sub_session_time(*sess))) return 0;
...
static int test_early_data_read_write(int idx) {
  ...
  if (idx != 2) artificial_ticket_time = 2;
...
static int early_data_skip_helper(int testtype, int cipher, int idx) {
  ...
  if (is_fips && cipher >= 4) return 1;

  if (ciphersuites[cipher] == NULL) return TEST_skip("Cipher not supported");
  ...
  if (testtype == 1 || testtype == 2) {
    ...
    if (!TEST_true(SSL_set1_groups_list(serverssl, "ffdhe3072"))) goto end;
#else
    if (!TEST_true(SSL_set1_groups_list(serverssl, "P-384"))) goto end;
#endif
  } else if (idx == 2) {
    /*
     ...
     */
    if (!TEST_true(SSL_SESSION_set_time_ex(sess, time(NULL) - 20))) goto end;
  }
  ...
  switch (testtype) {
    ...
    case 2: {
       ...
       */
      if (!TEST_true(BIO_write_ex(wbio, bad_early_data, sizeof(bad_early_data),
                                  &written)))
        goto end;
    }
      /* FALLTHROUGH */
   ...
   * on the error stack
   */
  if (!TEST_long_eq(ERR_peek_error(), 0)) goto end;
...
static int alpn_select_cb(SSL* ssl, const unsigned char** out,
  ...
  for (prot = in; prot < in + inlen; prot += protlen) {
    protlen = *prot++;
    if (in + inlen < prot + protlen) return SSL_TLSEXT_ERR_NOACK;
...
static int test_early_data_psk_with_all_ciphers(int idx) {
  ...
  SSL_SESSION_free(serverpsk);
  clientpsk = serverpsk = NULL;
  if (clientssl != NULL) SSL_shutdown(clientssl);
  if (serverssl != NULL) SSL_shutdown(serverssl);
  SSL_free(serverssl);
  SSL_free(clientssl);
  SSL_CTX_free(sctx);
  SSL_CTX_free(cctx);
...
static int test_key_exchange(int idx) {
  ...
  switch (idx) {
    ...
    case 12:
      kexch_groups = NULL;
      if (kexch_names == NULL) kexch_names = "MLKEM512:MLKEM768:MLKEM1024";
...
#ifndef OPENSSL_NO_TLS1_2
  if (!TEST_true(SSL_CTX_set_cipher_list(
          cctx, TLS1_TXT_ECDHE_RSA_WITH_AES_128_GCM_SHA256
          ":" TLS1_TXT_DHE_RSA_WITH_AES_128_GCM_SHA256)))
    goto end;
#endif
  ...
  switch (idx) {
    case 19:
      ...
      if (!TEST_int_eq(shared_group0, NID_X9_62_prime256v1)) goto end;
      break;
#endif
      /* Fall through */
    case 20:
      if (!TEST_int_eq(shared_group0, 0)) goto end;
      break;
    default:
...
static int set_ssl_groups(SSL* serverssl, SSL* clientssl, int clientmulti,
  ...
  if (clientmulti) {
    if (!TEST_true(SSL_set1_groups(serverssl, kexch_groups, 1))) return 0;
    if (isecdhe) {
    ...
    if (!TEST_true(SSL_set1_groups(clientssl, kexch_groups, 1))) return 0;
    if (isecdhe) {
...
static int test_negotiated_group(int idx) {
  ...
  if (!istls13) max_version = TLS1_2_VERSION;
  ...
  if (!TEST_ptr((origsess = SSL_get1_session(clientssl)))) goto end;
  ...
  if (istls13) {
    ...
    if (!TEST_int_ne(expectednid, kexch_alg)) goto end;
  } else {
...
static int test_tls13_ciphersuite(int idx) {
  ...
  for (max_ver = TLS1_2_VERSION; max_ver <= TLS1_3_VERSION; max_ver++) {
    ...
    if (max_ver == TLS1_2_VERSION) continue;
#endif
    for (i = 0; i < OSSL_NELEM(t13_ciphers); i++) {
      if (is_fips && !t13_ciphers[i].fipscapable) continue;
      ...
      if (!TEST_str_eq(negotiated_scipher, negotiated_ccipher)) goto end;
...
static int test_tls13_no_dhe_kex(const int idx) {
   ...
   */
  if (!TEST_true(SSL_session_reused(clientssl))) goto end;
...
static int test_custom_exts(int tst) {
  ...
  if (tst < 3) {
    SSL_CTX_set_options(cctx, SSL_OP_NO_TLSv1_3);
    SSL_CTX_set_options(sctx, SSL_OP_NO_TLSv1_3);
    if (sctx2 != NULL) SSL_CTX_set_options(sctx2, SSL_OP_NO_TLSv1_3);
  }
...
static int test_serverinfo_custom(const int idx) {
  ...
  if (call_use_serverinfo_ex) {
    ...
    if (!TEST_true(SSL_CTX_use_serverinfo(sctx, si, si_len))) goto end;
  }
  ...
  if (!TEST_true(cb_result)) goto end;
...
static int test_export_key_mat(int tst) {
  ...
  if (tst == 1) return 1;
#endif
  if (is_fips && (tst == 0 || tst == 1)) return 1;
#ifdef OPENSSL_NO_TLS1_2
  if (tst == 2) return 1;
#endif
  ...
  if (!TEST_int_eq(SSL_export_keying_material(clientssl, ckeymat1,
      ...
      !TEST_int_eq(
          ...
          1) ||
      !TEST_int_eq(
          SSL_export_keying_material(clientssl, ckeymat3, sizeof(ckeymat3),
                                     label, labellen, NULL, 0, 0),
          1) ||
      (test_longcontext &&
      ...
      !TEST_int_eq(
          ...
          1) ||
      !TEST_int_eq(
          SSL_export_keying_material(serverssl, skeymat3, sizeof(skeymat3),
                                     label, labellen, NULL, 0, 0),
          1) ||
      (test_longcontext &&
       !TEST_int_eq(SSL_export_keying_material(
...
static int test_key_update(void) {
  ...
  for (j = 0; j < 2; j++) {
    ...
    for (i = 0; i < NUM_KEY_UPDATE_MESSAGES; i++) {
      if (!TEST_true(
              SSL_key_update(clientssl, (j == 0) ? SSL_KEY_UPDATE_NOT_REQUESTED
    ...
    if (!TEST_int_eq(SSL_write(serverssl, mess, (int)strlen(mess)),
                     (int)strlen(mess)) ||
...
static int test_key_update_peer_in_write(int tst) {
   ...
   * endpoint
   */
  if (!TEST_int_eq(SSL_write(peerwrite, mess, (int)strlen(mess)),
                   (int)strlen(mess)) ||
...
static int test_key_update_peer_in_read(int tst) {
  ...
  if (!TEST_int_eq(BIO_new_bio_pair(&lbio, 512, &pbio, 512), 1)) goto end;
  ...
  if (!TEST_int_eq(SSL_write(local, mess, (int)strlen(mess)),
                   (int)strlen(mess)) ||
...
static int test_key_update_local_in_write(int tst) {
  ...
  if (!TEST_int_eq(SSL_write(peer, mess, (int)strlen(mess)),
                   (int)strlen(mess)) ||
...
static int test_ssl_clear(int idx) {
...
#ifdef OPENSSL_NO_TLS1_2
  if (tls12test == 1) return TEST_skip("No TLSv1.2 in this build");
#endif
  ...
  if (servertest) {
    ...
    if (cleartest) {
      if (!TEST_true(SSL_clear(clientssl))) goto end;
    } else {
      SSL_set_connect_state(clientssl);
    }
    SSL_free(serverssl);
...
static int get_MFL_from_client_hello(BIO* bio, int* mfl_codemfl_code) {
  ...
  if (!TEST_uint_gt(len = BIO_get_mem_data(bio, (char**)&data), 0)) goto end;
...
static int test_max_fragment_len_ext(int idx_tst) {
  ...
  if (!TEST_true(get_MFL_from_client_hello(wbio, &MFL_mode)))
    ...
    goto end;
  if (!TEST_true(max_fragment_len_test[idx_tst] == MFL_mode)) goto end;
...
static int test_pha_key_update(void) {
  ...
  if (!TEST_true(SSL_verify_client_post_handshake(serverssl))) goto end;

  if (!TEST_true(SSL_key_update(clientssl, SSL_KEY_UPDATE_NOT_REQUESTED)))
    goto end;

  /* Start handshake on the server */
  if (!TEST_int_eq(SSL_do_handshake(serverssl), 1)) goto end;
...
static int create_new_vfile(char* userid, char* password,
  ...
  if (TXT_DB_write(out, db) <= 0) goto end;
...
static int create_new_vbase(char* userid, char* password) {
  ...
  if (!TEST_ptr(user_pwd)) goto end;

  user_pwd->N = lgN->N;
  user_pwd->g = lgN->g;
  user_pwd->id = OPENSSL_strdup(userid);
  if (!TEST_ptr(user_pwd->id)) goto end;

  user_pwd->v = verifier;
  user_pwd->s = salt;
  verifier = salt = NULL;

  if (sk_SRP_user_pwd_insert(vbase->users_pwd, user_pwd, 0) == 0) goto end;
  user_pwd = NULL;
...
static int test_srp(int tst) {
  ...
  if (!TEST_ptr(vbase)) goto end;

  if (tst == 0 || tst == 1) {
    if (!TEST_true(create_new_vbase(userid, password))) goto end;
  } else {
    if (tst == 4 || tst == 5) {
      if (!TEST_true(create_new_vfile(userid, password, tmpfilename))) goto end;
      tstsrpfile = tmpfilename;
    } else {
      tstsrpfile = srpvfile;
    }
    if (!TEST_int_eq(SRP_VBASE_init(vbase, tstsrpfile), SRP_NO_ERROR)) goto end;
  }
  ...
  if (tst % 2 == 1) {
    if (!TEST_int_gt(SSL_CTX_set_srp_password(cctx, "badpass"), 0)) goto end;
  } else {
    if (!TEST_int_gt(SSL_CTX_set_srp_password(cctx, password), 0)) goto end;
  }
  ...
  if (ret) {
    if (!TEST_true(tst % 2 == 0)) goto end;
  } else {
    if (!TEST_true(tst % 2 == 1)) goto end;
  }
...
/* We need either ECDHE or DHE for the TLSv1.2 test to work */
  ...
  if (!TEST_true(SSL_CTX_set_dh_auto(sctx, 1))) goto end;
  ...
  if (tst >= 6) {
    if (!SSL_CTX_compress_certs(sctx, 0)) goto end;
  }
...
static int int_test_ssl_get_shared_ciphers(int tst, int clnt) {
  ...
  if (!TEST_ptr(tmplibctx)) goto end;
  ...
  if (!TEST_true(SSL_CTX_set_cipher_list(
          cctx, shared_ciphers_data[tst].clntciphers)) ||
      ...
      !TEST_true(SSL_CTX_set_cipher_list(
          sctx, shared_ciphers_data[tst].srvrciphers)) ||
...
static SSL_TICKET_RETURN dec_tick_cb(SSL* s, SSL_SESSION* ss,
  ...
  if (status == SSL_TICKET_EMPTY) return SSL_TICKET_RETURN_IGNORE_RENEW;
...
static int tick_key_cb(SSL* s, unsigned char key_name[16],
  ...
  if (tick_key_renew == -1) return 0;

  aes128cbc = EVP_CIPHER_fetch(libctx, "AES-128-CBC", NULL);
  if (!TEST_ptr(aes128cbc)) return 0;
...
static int test_ticket_callbacks(int tst) {
  ...
  if (tick_dec_ret == SSL_TICKET_RETURN_IGNORE ||
    ...
    if (!TEST_false(SSL_session_reused(clientssl))) goto end;
  } else {
    if (!TEST_true(SSL_session_reused(clientssl))) goto end;
  }
...
static int test_incorrect_shutdown(int tst) {
  ...
  if (!TEST_false(SSL_read(serverssl, buf, sizeof(buf)))) goto end;
...
static int test_shutdown(int tst) {
  ...
  if (tst == 5) SSL_CTX_set_post_handshake_auth(cctx, 1);
  ...
  if (!TEST_int_eq(SSL_shutdown(clientssl), 0)) goto end;
  ...
  if (tst >= 4) {
    ...
    if ((tst == 4 || tst == 5) &&
        !TEST_true(SSL_write(serverssl, msg, sizeof(msg))))
      goto end;
    if (!TEST_int_eq(SSL_shutdown(serverssl), 1)) goto end;
    if (tst == 4 || tst == 5) {
...
static int cert_cb(SSL* s, void* arg) {
  ...
  if (cert_cb_cnt == 0) {
     ...
     */
    if (ctx != NULL && !TEST_ptr(SSL_set_SSL_CTX(s, ctx))) return 0;
    ...
    if ((rv & (CERT_PKEY_VALID | CERT_PKEY_CA_SIGNATURE)) ==
      ...
      if (!SSL_use_cert_and_key(s, x509, pkey, NULL, 1)) goto out;
    }
...
static int client_cert_cb(SSL* ssl, X509** x509, EVP_PKEY** pkey) {
  ...
  if (!TEST_ptr(SSL_get0_peer_certificate(ssl))) return 0;

  in = BIO_new_file(cert, "r");
  if (!TEST_ptr(in)) return 0;
...
static int test_multiblock_write(int test_index) {
  ...
  if (!TEST_true(SSL_CTX_set_cipher_list(cctx, cipherlist))) goto end;
  ...
  if (!TEST_mem_eq(msg, sizeof(msg), buf, sizeof(buf))) goto end;
...
static int test_session_cache_overflow(int idx) {
  ...
  if (idx % 2 == 0) return TEST_skip("No TLSv1.3 available");
#endif
#ifdef OPENSSL_NO_TLS1_2
  /* If no TLSv1.2 available then do nothing in this case */
  if (idx % 2 == 1) return TEST_skip("No TLSv1.2 available");
#endif
  ...
  if (!TEST_ptr(get_sess_val)) goto end;
  sess = SSL_get1_session(clientssl);
  if (!TEST_ptr(sess)) goto end;
  ...
  if (!TEST_true(SSL_set_session(clientssl, sess))) goto end;
...
static int test_servername(int tst) {
  ...
  if (tst != 3 && tst != 8) {
    if (!TEST_true(SSL_set_tlsext_host_name(clientssl, "goodhost"))) goto end;
  ...
  if (tst == 2 || tst == 7) {
     ...
     */
    if (tst == 7) sexpectedhost = cexpectedhost = "altgoodhost";
     ...
     */
    if (tst == 9) sexpectedhost = cexpectedhost = NULL;
     ...
     * is as we set it.
     */
    if (tst == 1 || tst == 3) sexpectedhost = NULL;
...
static int test_unknown_sigalgs_groups(void) {
  ...
  if (!TEST_int_le(SSL_CTX_set1_groups_list(ctx, "nonexistent"), 0)) goto end;
...
static int test_sigalgs_available(int idx) {
  ...
  if (idx != 0 && idx != 3) {
    ...
    if (!TEST_ptr(filterprov)) goto end;
  ...
  if (!TEST_ptr(cctx) || !TEST_ptr(sctx)) goto end;
...
static int test_pluggable_group(int idx) {
  ...
  if (!TEST_true(SSL_set1_groups_list(
          serverssl,
          "xorgroup:xorkemgroup:dummy1:dummy2:dummy3:dummy4:dummy5:dummy6:"
          "dummy7:dummy8:dummy9:dummy10:dummy11:dummy12:dummy13:dummy14:"
          "dummy15:dummy16:dummy17:dummy18:dummy19:dummy20:dummy21:dummy22:"
          "dummy23:dummy24:dummy25:dummy26:dummy27:dummy28:dummy29:dummy30:"
          "dummy31:dummy32:dummy33:dummy34:dummy35:dummy36:dummy37:dummy38:"
          "dummy39:dummy40:dummy41:dummy42:dummy43"))
...
static int test_pluggable_signature(int idx) {
  ...
  if (!TEST_int_eq(SSL_CTX_check_private_key(sctx), 1)) goto end;
...
static int test_ssl_dup(void) {
  ...
  if (!TEST_ptr(rbio) || !TEST_true(BIO_up_ref(rbio))) goto end;
  SSL_set0_rbio(client2ssl, rbio);
  rbio = NULL;

  wbio = SSL_get_wbio(clientssl);
  if (!TEST_ptr(wbio) || !TEST_true(BIO_up_ref(wbio))) goto end;
...
static int test_session_secret_cb(void) {
  ...
  if (!TEST_ptr(secret_sess = SSL_get1_session(clientssl))) goto end;
   ...
   * confused.
   */
  if (!TEST_true(SSL_SESSION_set1_id(secret_sess, NULL, 0))) goto end;
...
static EVP_PKEY* get_tmp_dh_params(void) {
  ...
  if (tmp_dh_params != NULL && !EVP_PKEY_up_ref(tmp_dh_params)) return NULL;
...
static DH* tmp_dh_callback(SSL* s, int is_export, int keylen) {
  ...
  if (!TEST_ptr(dhpkey)) return NULL;
...
static int test_set_tmp_dh(int idx) {
  ...
  if (idx >= 5 && idx <= 8) {
    ...
    if (!TEST_ptr(dhpkey)) goto end;
  }
#ifndef OPENSSL_NO_DEPRECATED_3_0
  if (idx == 7 || idx == 8) {
    dh = EVP_PKEY_get1_DH(dhpkey);
    if (!TEST_ptr(dh)) goto end;
  }
#endif
  ...
  if ((idx & 1) == 1) {
    if (!TEST_true(SSL_CTX_set_dh_auto(sctx, dhauto))) goto end;
  }

  if (idx == 5) {
    if (!TEST_true(SSL_CTX_set0_tmp_dh_pkey(sctx, dhpkey))) goto end;
    dhpkey = NULL;
  }
#ifndef OPENSSL_NO_DEPRECATED_3_0
  else if (idx == 7) {
    if (!TEST_true(SSL_CTX_set_tmp_dh(sctx, dh))) goto end;
  } else if (idx == 9) {
  ...
  if ((idx & 1) == 0 && idx != 0) {
    if (!TEST_true(SSL_set_dh_auto(serverssl, dhauto))) goto end;
  }
  if (idx == 6) {
    if (!TEST_true(SSL_set0_tmp_dh_pkey(serverssl, dhpkey))) goto end;
    dhpkey = NULL;
  }
#ifndef OPENSSL_NO_DEPRECATED_3_0
  else if (idx == 8) {
    if (!TEST_true(SSL_set_tmp_dh(serverssl, dh))) goto end;
  } else if (idx == 10) {
...
static int test_dh_auto(int idx) {
  ...
  if (!TEST_ptr(sctx) || !TEST_ptr(cctx)) goto end;
  ...
  if (!TEST_int_gt(SSL_get_tmp_key(serverssl, &tmpkey), 0)) goto end;
  if (!TEST_size_t_eq(EVP_PKEY_get_bits(tmpkey), expdhsize)) goto end;
...
static int test_sni_tls13(void) {
  ...
  if (!TEST_int_eq(snicb, 1)) goto end;
...
static int test_set_alpn(void) {
  ...
  if (!TEST_ptr(ctx)) goto end;

  /* the set_alpn functions return 0 (false) on success, non-zero (true) on
   * failure */
  if (!TEST_false(SSL_CTX_set_alpn_protos(ctx, NULL, 2))) goto end;
  if (!TEST_false(SSL_CTX_set_alpn_protos(ctx, good, 0))) goto end;
  if (!TEST_false(SSL_CTX_set_alpn_protos(ctx, good, sizeof(good)))) goto end;
  if (!TEST_true(SSL_CTX_set_alpn_protos(ctx, good, 1))) goto end;
  if (!TEST_true(SSL_CTX_set_alpn_protos(ctx, bad0, sizeof(bad0)))) goto end;
  if (!TEST_true(SSL_CTX_set_alpn_protos(ctx, bad1, sizeof(bad1)))) goto end;
  if (!TEST_true(SSL_CTX_set_alpn_protos(ctx, bad2, sizeof(bad2)))) goto end;
  if (!TEST_true(SSL_CTX_set_alpn_protos(ctx, bad3, sizeof(bad3)))) goto end;
  if (!TEST_true(SSL_CTX_set_alpn_protos(ctx, bad4, sizeof(bad4)))) goto end;

  ssl = SSL_new(ctx);
  if (!TEST_ptr(ssl)) goto end;

  if (!TEST_false(SSL_set_alpn_protos(ssl, NULL, 2))) goto end;
  if (!TEST_false(SSL_set_alpn_protos(ssl, good, 0))) goto end;
  if (!TEST_false(SSL_set_alpn_protos(ssl, good, sizeof(good)))) goto end;
  if (!TEST_true(SSL_set_alpn_protos(ssl, good, 1))) goto end;
  if (!TEST_true(SSL_set_alpn_protos(ssl, bad0, sizeof(bad0)))) goto end;
  if (!TEST_true(SSL_set_alpn_protos(ssl, bad1, sizeof(bad1)))) goto end;
  if (!TEST_true(SSL_set_alpn_protos(ssl, bad2, sizeof(bad2)))) goto end;
  if (!TEST_true(SSL_set_alpn_protos(ssl, bad3, sizeof(bad3)))) goto end;
  if (!TEST_true(SSL_set_alpn_protos(ssl, bad4, sizeof(bad4)))) goto end;
...
static int test_legacy_ec_point_formats(void) {
  ...
  if (!TEST_int_eq(nformats, 3)) goto end;
...
static int test_set_verify_cert_store_ssl_ctx(void) {
  ...
  if (!TEST_true(SSL_CTX_get0_verify_cert_store(ctx, &store))) goto end;

  /* Retrieve chain store pointer. */
  if (!TEST_true(SSL_CTX_get0_chain_cert_store(ctx, &cstore))) goto end;

  /* We haven't set any yet, so this should be NULL. */
  if (!TEST_ptr_null(store) || !TEST_ptr_null(cstore)) goto end;

  /* Create stores. We use separate stores so pointers are different. */
  new_store = X509_STORE_new();
  if (!TEST_ptr(new_store)) goto end;

  new_cstore = X509_STORE_new();
  if (!TEST_ptr(new_cstore)) goto end;

  /* Set stores. */
  if (!TEST_true(SSL_CTX_set1_verify_cert_store(ctx, new_store))) goto end;

  if (!TEST_true(SSL_CTX_set1_chain_cert_store(ctx, new_cstore))) goto end;
  ...
  if (!TEST_true(SSL_CTX_set1_verify_cert_store(ctx, NULL))) goto end;

  if (!TEST_true(SSL_CTX_set1_chain_cert_store(ctx, NULL))) goto end;
...
static int test_set_verify_cert_store_ssl(void) {
  ...
  /* Retrieve verify store pointer. */
  if (!TEST_true(SSL_get0_verify_cert_store(ssl, &store))) goto end;

  /* Retrieve chain store pointer. */
  if (!TEST_true(SSL_get0_chain_cert_store(ssl, &cstore))) goto end;
  ...
  /* Set stores. */
  if (!TEST_true(SSL_set1_verify_cert_store(ssl, new_store))) goto end;

  if (!TEST_true(SSL_set1_chain_cert_store(ssl, new_cstore))) goto end;
  ...
  /* Should be able to unset again. */
  if (!TEST_true(SSL_set1_verify_cert_store(ssl, NULL))) goto end;

  if (!TEST_true(SSL_set1_chain_cert_store(ssl, NULL))) goto end;
...
static int test_inherit_verify_param(void) {
  ...
  if (!TEST_ptr(cp)) goto end;
  if (!TEST_int_eq(X509_VERIFY_PARAM_get_hostflags(cp), 0)) goto end;
  ...
  if (!TEST_ptr(sp)) goto end;
  if (!TEST_int_eq(X509_VERIFY_PARAM_get_hostflags(sp), hostflags)) goto end;
...
static int test_load_dhfile(void) {
  ...
  if (!TEST_int_eq(SSL_CONF_cmd(cctx, "DHParameters", dhfile), 2)) goto end;
...
static int test_read_ahead_key_change(void) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    buf[readbytes] = '\0';
    if (!TEST_str_eq(buf, msg)) goto end;
  }
...
static int test_tls13_record_padding(int idx) {
  ...
  if (idx == 0) {
    ...
    if (!TEST_false(
      ...
      goto end;
    if (!TEST_true(SSL_CTX_set_block_padding(cctx, 512))) goto end;
  } else if (idx == 4) {
    /* pad only handshake/alert messages */
    if (!TEST_true(SSL_CTX_set_block_padding_ex(cctx, 0, 512))) goto end;
  }
  ...
  if (idx == 1) {
    ...
    if (!TEST_false(
      ...
      goto end;
    if (!TEST_true(SSL_set_block_padding(clientssl, 512))) goto end;
  } else if (idx == 5) {
    /* Exceeding the max plain length should fail */
    ...
    if (!TEST_true(SSL_set_block_padding_ex(clientssl, 0, 512))) goto end;
    if (!TEST_true(SSL_set_block_padding_ex(serverssl, 0, 512))) goto end;
  }
  ...
  if ((idx == 0 || idx == 1) && !TEST_int_eq(called, 4)) goto end;
...
static int test_pipelining(int idx) {
  ...
  if (!TEST_true(SSL_set_cipher_list(clientssl, "AES128-SHA"))) goto end;
  ...
  if (idx == 5) {
    ...
    if (!TEST_ptr(msg)) goto end;
    if (!TEST_int_gt(RAND_bytes_ex(libctx, msg, msglen, 0), 0)) goto end;
  } else if (idx == 4) {
    msglen = 55;
  } else {
    msglen = 50;
  ...
  for (offset = 0; offset < msglen; offset += fragsize) {
    ...
    if (sendlen > fragsize) sendlen = fragsize;
  ...
  if (idx == 4) {
    ...
    if (!TEST_size_t_le(readbytes, msglen)) goto end;
  }

  if (!TEST_mem_eq(msg, msglen, buf, readbytes)) goto end;
  ...
  OPENSSL_free(buf);
  if (fragsize == SSL3_RT_MAX_PLAIN_LENGTH) OPENSSL_free(msg);
  return testresult;
}
...
static int test_rstate_string(void) {
  ...
  if (version == TLS1_3_VERSION) version = TLS1_2_VERSION;
...
static int test_handshake_retry(int idx) {
...
#ifndef OPENSSL_NO_TLS1_2
  if ((idx & 8) == 8) maxversion = TLS1_2_VERSION;
#else
  if ((idx & 8) == 8) return TEST_skip("No TLSv1.2");
#endif
   ...
   * isn't present.
   */
  if ((idx & 2) == 2) SSL_CTX_set_verify(sctx, SSL_VERIFY_PEER, NULL);

  if ((idx & 4) == 4) set_always_retry_err_val(0);
  ...
  if (!TEST_int_eq(SSL_connect(clientssl), -1)) goto end;
...
static int test_data_retry(void) {
  ...
  if (!TEST_true(SSL_set_max_send_fragment(clientssl, 512))) goto end;

  tmp = SSL_get_wbio(clientssl);
  if (!TEST_ptr(tmp)) goto end;
  if (!TEST_true(BIO_up_ref(tmp))) goto end;
  BIO_push(bretry, tmp);
  tmp = NULL;
  ...
  while (SSL_read_ex(serverssl, outbuf + totread, sizeof(outbuf) - totread,
                     &readbytes))
    totread += readbytes;
  if (!TEST_mem_eq(inbuf, sizeof(inbuf), outbuf, totread)) goto end;
...
static int resume_servername_cb(SSL* s, int* ad, void* arg) {
  ...
  if ((cbdata->i % 3) != 1) return SSL_TLSEXT_ERR_OK;
...
static int test_multi_resume(int idx) {
...
#if defined(OPENSSL_NO_TLS1_2)
  if (idx == 4) return TEST_skip("TLSv1.2 is disabled in this build");
#else
  if (idx == 4) max_version = TLS1_2_VERSION;
#endif
#if defined(OSSL_NO_USABLE_TLS1_3)
  if (idx != 4) return TEST_skip("No usable TLSv1.3 in this build");
#endif
   ...
   */
  if (idx == 0 || idx == 2) {
    if (!TEST_true(SSL_CTX_set_max_early_data(sctx, 1024))) goto end;
  }
  ...
  for (i = 0; i < 30; i++) {
    ...
    if ((i % 3) == 2) {
      ...
      if (!TEST_ptr((sess = SSL_get1_session(clientssl)))) goto end;
    }
  ...
  if (!TEST_long_le(SSL_CTX_sess_number(sctx), 5)) goto end;
...
static int test_select_next_proto(int idx) {
  ...
  if (np->selectedlen == 0) {
    if (!TEST_ptr_null(out) || !TEST_uchar_eq(outlen, 0)) goto err;
  } else {
    if (!TEST_mem_eq(out, outlen, np->selected, np->selectedlen)) goto err;
  }
...
static int test_npn(int idx) {
  ...
  if (idx == 4) {
    ...
    switch (idx) {
      case 0:
      case 1:
        if (!TEST_mem_eq(prot, protlen, fooprot + 1, *fooprot)) goto end;
        break;
      case 2:
        if (!TEST_uint_eq(protlen, 0)) goto end;
        break;
      case 3:
        if (!TEST_mem_eq(prot, protlen, barprot + 1, *barprot)) goto end;
        break;
      default:
...
static int test_alpn(int idx) {
  ...
  if (!TEST_false(SSL_set_alpn_protos(clientssl, prots, protslen))) goto end;
...
static int crypto_send_cb(SSL* s, const unsigned char* buf, size_t buf_len,
  ...
  if (buf_len > max_len) buf_len = max_len;
  ...
  memcpy(
      peer->rcd_data[data->wenc_level] + peer->rcd_data_len[data->wenc_level],
      buf, buf_len);
...
static int check_secret_history(SSL* s) {
  ...
  for (i = 0; secret_history[i].recorded == 1; i++) {
    if (secret_history[i].ssl != s) continue;
    TEST_info(
        "Got %s(%d) secret for level %d, last level %d, last state %d, gen "
        "%d\n",
      ...
      case 1:
        ...
        if (last_prot_level == secret_history[i].prot_level &&
          ...
          if (last_generation == secret_history[i].sm_generation) {
            TEST_info(
                "Read before write key in same SSL state machine iteration is "
                "ok");
          } else {
...
static int yield_secret_cb(SSL* s, uint32_t prot_level, int direction,
  ...
  switch (direction) {
    case 0: /* read */
      if (!TEST_size_t_le(secret_len, sizeof(data->rsecret))) goto err;
    ...
    case 1: /* write */
      if (!TEST_size_t_le(secret_len, sizeof(data->wsecret))) goto err;
...
static int test_quic_tls(int idx) {
  ...
  if (idx == 4) qtdis[3].function = (void (*)(void))yield_secret_cb_fail;
  ...
  if (idx == 5) {
    if (!TEST_true(create_ssl_ctx_pair(libctx, TLS_server_method(), NULL,
                                       TLS1_3_VERSION, 0, &sctx2, NULL, cert,
                                       privkey)))
      goto end;
  ...
  if (idx != 1 && idx != 4) {
    ...
    if (!TEST_false(create_ssl_connection_ex(serverssl, clientssl,
                                             SSL_ERROR_NONE, &cdata.sm_count,
                                             &sdata.sm_count)))
      goto end;
    testresult = 1;
   ...
   */
  if (!TEST_int_eq(check_secret_history(serverssl), 1)) goto end;
  if (!TEST_int_eq(check_secret_history(clientssl), 1)) goto end;
  ...
  if (!TEST_false(sdata.err) || !TEST_false(cdata.err)) testresult = 0;
...
static int test_quic_tls_early_data(void) {
  ...
  if (!TEST_int_eq(end_of_early_data, 0)) goto end;
...
static int test_no_renegotiation(int idx) {
  ...
  if (!TEST_true(create_ssl_ctx_pair(libctx, sm, cm, 0, max_proto, &sctx, &cctx,
                                     cert, privkey)))
    goto end;
   ...
   */
  if (!TEST_int_le(ret = SSL_read(serverssl, buf, sizeof(buf)), 0)) goto end;
...
int setup_tests(void) {
  ...
  if (!TEST_true(OSSL_LIB_CTX_load_config(libctx, configfile))) return 0;
