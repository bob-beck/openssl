...
static int use_session_cb(SSL* ssl,
                          const EVP_MD* md,
                          const unsigned char** id,
                          size_t* idlen,
                          SSL_SESSION** sess);
static int find_session_cb(SSL* ssl,
...
static int test_keylog_output(char* buffer,
                              const SSL* ssl,
...
static int execute_test_ktls(int cis_ktls,
                             int sis_ktls,
                             int tls_version,
                             const char* cipher) {
  SSL_CTX *cctx = NULL, *sctx = NULL;
...
static int execute_test_ktls_sendfile(int tls_version,
                                      const char* cipher,
                                      int zerocopy) {
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
...
#ifndef OPENSSL_NO_OCSP
static OCSP_RESPONSE* create_ocsp_resp(X509* ssl_cert,
                                       X509* issuer,
                                       int status,
...
static SSL_SESSION* get_session_cb(SSL* ssl,
                                   const unsigned char* id,
                                   int len,
                                   int* copy) {
  get_called++;
...
static int execute_test_session(int maxprot,
                                int use_int_cache,
                                int use_ext_cache,
                                long s_options) {
...
static int setup_ticket_test(int stateful,
                             int idx,
                             SSL_CTX** sctx,
                             SSL_CTX** cctx) {
  int sess_id_ctx = 1;
...
static int set_ssl_groups(SSL* serverssl,
                          SSL* clientssl,
                          int clientmulti,
                          int isecdhe,
                          int idx) {
...
static int generate_cookie_callback(SSL* ssl,
...
static int verify_cookie_callback(SSL* ssl,
...
static int generate_stateless_cookie_callback(SSL* ssl,
                                              unsigned char* cookie,
                                              size_t* cookie_len) {
  unsigned int temp;
...
static int create_new_vfile(char* userid,
                            char* password,
                            const char* filename) {
  char* gNid = NULL;
...
static int load_chain(const char* file,
                      EVP_PKEY** pkey,
                      X509** x509,
                      STACK_OF(X509)* chain) {
...
static int npn_advert_cb(SSL* ssl,
                         const unsigned char** out,
                         unsigned int* outlen,
                         void* arg) {
  int* idx = (int*)arg;
...
static int crypto_send_cb(SSL* s,
...
static int crypto_recv_rcd_cb(SSL* s,
                              const unsigned char** buf,
                              size_t* bytes_read,
                              void* arg) {
  struct quic_tls_test_data* data = (struct quic_tls_test_data*)arg;
...
static int yield_secret_cb(SSL* s,
                           uint32_t prot_level,
                           int direction,
...
static int got_transport_params_cb(SSL* s,
