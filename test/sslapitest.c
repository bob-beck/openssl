...
static int use_session_cb (SSL *ssl, const EVP_MD *md,
...
static int
hostname_cb (SSL *s, int *al, void *arg)
{
...
static void
client_keylog_callback (const SSL *ssl, const char *line)
{
...
static void
server_keylog_callback (const SSL *ssl, const char *line)
{
  int line_length = (int)strlen (line);
...
static int
compare_hex_encoded_buffer (const char *hex_encoded, size_t hex_length,
...
static int
test_keylog_output (char *buffer, const SSL *ssl, const SSL_SESSION *session,
...
#ifndef OSSL_NO_USABLE_TLS1_3
static int
test_keylog_no_master_key (void)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
...
static int
verify_retry_cb (X509_STORE_CTX *ctx, void *arg)
{
...
static int
test_client_cert_verify_cb (void)
{
...
static int
test_ssl_build_cert_chain (void)
{
  int ret = 0;
  ...
  if (!TEST_int_eq (SSL_use_certificate_chain_file (ssl, leaf_chain), 1)
      || !TEST_int_eq (SSL_use_PrivateKey_file (ssl, skey, SSL_FILETYPE_PEM),
                       1)
...
static int
get_password_cb (char *buf, int size, int rw_flag, void *userdata)
{
...
static int
test_ssl_ctx_build_cert_chain (void)
{
  int ret = 0;
  SSL_CTX *ctx = NULL;
...
static int
full_client_hello_callback (SSL *s, int *al, void *arg)
{
  int *ctr = arg;
  ...
  if (fips_ems_check)
    {
      if (status == 1)
        {
          printf ("When FIPS uses the EMS check a connection that doesn't use "
                  "EMS should fail\n");
          goto end;
        }
    }
...
static int
execute_test_large_message (const SSL_METHOD *smeth, const SSL_METHOD *cmeth,
                            int min_version, int max_version, int read_ahead)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
...
static int
ping_pong_query (SSL *clientssl, SSL *serverssl)
{
  static char count = 1;
...
static int
execute_test_ktls (int cis_ktls, int sis_ktls, int tls_version,
                   const char *cipher)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
...
static int
execute_test_ktls_sendfile (int tls_version, const char *cipher, int zerocopy)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
...
static int
test_large_message_tls (void)
{
  return execute_test_large_message (TLS_server_method (),
                                     TLS_client_method (), TLS1_VERSION, 0, 0);
}

static int
test_large_message_tls_read_ahead (void)
{
  return execute_test_large_message (TLS_server_method (),
                                     TLS_client_method (), TLS1_VERSION, 0, 1);
}

#ifndef OPENSSL_NO_DTLS
static int
test_large_message_dtls (void)
{
#ifdef OPENSSL_NO_DTLS1_2
 ...
 * TLSv1.3 doesn't use EtM so technically it is redundant to test both but it
 * is simpler this way. We also test all combinations with and without the
 * SSL_OP_DONT_INSERT_EMPTY_FRAGMENTS option which affects the size of the
 * underlying buffer.
 */
static int
test_large_app_data (int tst)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
...
static int
execute_cleanse_plaintext (const SSL_METHOD *smeth, const SSL_METHOD *cmeth,
...
create_ocsp_resp (X509 *ssl_cert, X509 *issuer, int status,
  ...
  if (signer_key_files != NULL && signer_cert_files != NULL)
    {
      ...
      OCSP_basic_add1_status (bs, cert_id, status, 0, thisupd, thisupd,
                              nextupd);
      ...
      if (!TEST_ptr (signer_cert = load_cert_pem (signer_cert_file, libctx))
          || !TEST_true (OCSP_basic_sign (bs, signer_cert, signer_key,
                                          EVP_sha256 (), NULL, OCSP_NOCERTS)))
        goto end;
...
static int
ocsp_server_cb_single (SSL *s, void *arg)
{
  int *argi = (int *)arg;
...
static int
ocsp_client_cb_single (SSL *s, void *arg)
{
  int *argi = (int *)arg;
...
#ifndef OSSL_NO_USABLE_TLS1_3
static int
ocsp_server_cb_multi (SSL *s, void *arg)
{
  int *argi = (int *)arg;
  ...
  char *signer_cert_files[]
      = { "subinterCA.pem", "interCA.pem", "rootCA.pem" };
  ...
  for (i = 0; i < num; i++)
    {
      ...
      if (ocsp_server_called == 3 && i == 0)
        ocsp_resp = create_ocsp_resp (ssl_cert, issuer,
                                      OCSP_RESPONSE_STATUS_MALFORMEDREQUEST,
                                      NULL, NULL);
      else if (ocsp_server_called == 4 && i == 0)
        ocsp_resp
            = create_ocsp_resp (ssl_cert, issuer, V_OCSP_CERTSTATUS_REVOKED,
                                signer_key_files[i], signer_cert_files[i]);
      else
        ocsp_resp
            = create_ocsp_resp (ssl_cert, issuer, V_OCSP_CERTSTATUS_GOOD,
                                signer_key_files[i], signer_cert_files[i]);
...
static int
ocsp_client_cb_multi (SSL *s, void *arg)
{
  int *argi = (int *)arg;
  STACK_OF (OCSP_RESPONSE) *sk_resp = NULL;
      ...
      if (ocsp_client_called != 3)
        {
          ...
          if ((bs = OCSP_response_get1_basic (resp)) == NULL)
            break;
          /* we send a OCSP response with one single response so we check only
           * the first */
...
static int
verify_cb_multi_stapling (int preverify_ok, X509_STORE_CTX *x509_ctx)
{
...
static int
test_tlsext_status_type_multi (void)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
  ...
  if (!TEST_true (
      ...
      || TEST_int_ne (SSL_CTX_get_tlsext_status_type (cctx),
                      TLSEXT_STATUSTYPE_ocsp))
    goto end;
  ...
  if (!TEST_true (
      ...
      || !TEST_int_eq (ocsp_server_called, 2)
      || !TEST_true (ocsp_verify_cb_called))
    goto end;
  ...
  if (!TEST_true (
      ...
      || !TEST_int_eq (ocsp_server_called, 3)
      || !TEST_true (ocsp_verify_cb_called))
    goto end;
   ...
   * REVOKED. The SSL connection should fail and the ocsp_client_cb_multi
  ...
  if (!TEST_true (
      ...
      || !TEST_int_eq (ocsp_server_called, 4)
      || !TEST_true (ocsp_verify_cb_called))
    goto end;
...
static void
remove_session_cb (SSL_CTX *ctx, SSL_SESSION *sess)
{
  remove_called++;
}
...
static SSL_SESSION *
get_session_cb (SSL *ssl, const unsigned char *id, int len, int *copy)
{
  get_called++;
  *copy = 1;
  return get_sess_val;
...
static int
execute_test_session (int maxprot, int use_int_cache, int use_ext_cache,
                      long s_options)
{
...
static int
test_session_with_only_int_cache (void)
{
#ifndef OSSL_NO_USABLE_TLS1_3
...
static int
test_session_with_only_ext_cache (void)
{
#ifndef OSSL_NO_USABLE_TLS1_3
...
static int
test_session_with_both_cache (void)
{
#ifndef OSSL_NO_USABLE_TLS1_3
...
static int
new_cachesession_cb (SSL *ssl, SSL_SESSION *sess)
{
  if (do_cache)
    {
...
static int
post_handshake_verify (SSL *sssl, SSL *cssl)
{
...
static int
setup_ticket_test (int stateful, int idx, SSL_CTX **sctx, SSL_CTX **cctx)
{
  int sess_id_ctx = 1;
...
static int
check_resumption (int idx, SSL_CTX *sctx, SSL_CTX *cctx, int succ)
{
...
static int
test_tickets (int stateful, int idx)
{
  SSL_CTX *sctx = NULL, *cctx = NULL;
  SSL *serverssl = NULL, *clientssl = NULL;
  int testresult = 0;
...
static int
test_stateless_tickets (int idx)
{
  return test_tickets (0, idx);
}

static int
test_stateful_tickets (int idx)
{
  return test_tickets (1, idx);
}

static int
test_psk_tickets (void)
{
  SSL_CTX *sctx = NULL, *cctx = NULL;
  SSL *serverssl = NULL, *clientssl = NULL;
  int testresult = 0;
...
static void
setupbio (BIO **res, BIO *bio1, BIO *bio2, int type)
{
  switch (type)
    {
    case USE_NULL:
 ...
 * that SSL_set_bio() functions correctly in the case where s->bbio is not
 * NULL.
 */
static int
test_ssl_set_bio (int idx)
{
  SSL_CTX *sctx = NULL, *cctx = NULL;
  BIO *bio1 = NULL;
  BIO *bio2 = NULL;
...
static int
execute_test_ssl_bio (int pop_ssl, bio_change_t change_bio)
{
...
static const int validlist2[]
    = { NID_sha256, EVP_PKEY_RSA, NID_sha512, EVP_PKEY_EC };
...
static int
use_session_cb (SSL *ssl, const EVP_MD *md, const unsigned char **id,
...
static int
find_session_cb (SSL *ssl, const unsigned char *identity, size_t identity_len,
                 SSL_SESSION **sess)
{
  find_session_cb_cnt++;
...
static int
sub_session_time (SSL_SESSION *sess)
{
  OSSL_TIME tick_time;
 ...
 */
static int
setupearly_data_test (SSL_CTX **cctx, SSL_CTX **sctx, SSL **clientssl,
...
static int
check_early_data_timeout (OSSL_TIME timer)
{
  int res = 0;
...
static int
test_early_data_read_write (int idx)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
...
static int
allow_early_data_cb (SSL *s, void *arg)
{
  int *usecb = (int *)arg;
 ...
 */
static int
test_early_data_replay_int (int idx, int usecb, int confopt)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
...
static int
test_early_data_replay (int idx)
{
 ...
 */
static int
early_data_skip_helper (int testtype, int cipher, int idx)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
 ...
 */
static int
test_early_data_skip_hrr (int idx)
{
 ...
 */
static int
test_early_data_skip_hrr_fail (int idx)
{
 ...
 */
static int
test_early_data_skip_abort (int idx)
{
 ...
 */
static int
test_early_data_not_sent (int idx)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
  ...
  switch (idx)
    ...
    case 1:
      ...
      if (!TEST_true (
          ...
          || !TEST_false (
              SSL_set_alpn_protos (clientssl, BADALPN, BADALPNLEN)))
        goto end;
      break;
    ...
    case 7:
      ...
      if (!TEST_ptr (serverpsk)
          || !TEST_true (SSL_SESSION_set1_alpn_selected (
              clientpsk, BADALPN + 1, BADALPNLEN - 1))
          || !TEST_true (SSL_SESSION_set1_alpn_selected (
 ...
 */
static int
test_early_data_psk_with_all_ciphers (int idx)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
 ...
 */
static int
test_early_data_not_expected (int idx)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
 ...
 */
static int
test_early_data_tls1_2 (int idx)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
...
static int
set_ssl_groups (SSL *serverssl, SSL *clientssl, int clientmulti, int isecdhe,
                int idx)
{
  int kexch_alg;
  int *kexch_groups = &kexch_alg;
 ...
 */
static int
test_tls13_ciphersuite (int idx)
{
  SSL_CTX *sctx = NULL, *cctx = NULL;
  SSL *serverssl = NULL, *clientssl = NULL;
  static const struct
 ...
 */
static int
test_tls13_no_dhe_kex (const int idx)
{
  SSL_CTX *sctx = NULL, *cctx = NULL;
  SSL *serverssl = NULL, *clientssl = NULL;
  int testresult = 0;
...
static int
generate_stateless_cookie_callback (SSL *ssl, unsigned char *cookie,
                                    size_t *cookie_len)
{
  unsigned int temp;
...
static int
verify_stateless_cookie_callback (SSL *ssl, const unsigned char *cookie,
                                  size_t cookie_len)
{
...
static int
old_add_cb (SSL *s, unsigned int ext_type, const unsigned char **out,
...
static void
old_free_cb (SSL *s, unsigned int ext_type, const unsigned char *out,
             void *add_arg)
{
...
static int
old_parse_cb (SSL *s, unsigned int ext_type, const unsigned char *in,
  ...
  else
    {
      ...
      if (sctx2 != NULL
          && !TEST_true (SSL_CTX_add_custom_ext (
              sctx2, TEST_EXT_TYPE1, context, new_add_cb, new_free_cb, &server,
              new_parse_cb, &server)))
        goto end;
    }
...
static int
serverinfo_custom_parse_cb (SSL *s, unsigned int ext_type,
...
static int
test_serverinfo_custom (const int idx)
{
  SSL_CTX *sctx = NULL, *cctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
  ...
  if (tst == 5)
    {
      ...
      if (!TEST_int_le (SSL_export_keying_material (
                            clientssl, ckeymat1, sizeof (ckeymat1), label,
                            LONG_LABEL_LEN + 1, context, sizeof (context) - 1,
                            1),
                        0))
        goto end;
 ...
 */
static int
test_export_key_mat_early (int idx)
{
 ...
 */
static int
test_key_update_peer_in_write (int tst)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
  ...
  if (!TEST_int_eq (SSL_write (peerwrite, mess, (int)strlen (mess)),
                    (int)strlen (mess))
      || !TEST_int_eq (SSL_read (peerupdate, buf, sizeof (buf)),
                       (int)strlen (mess)))
    goto end;
 ...
 */
static int
test_key_update_peer_in_read (int tst)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
  ...
  if (!TEST_int_eq (SSL_write (local, mess, (int)strlen (mess)),
                    (int)strlen (mess))
      || !TEST_int_eq (SSL_read (peer, prbuf, sizeof (prbuf)),
                       (int)strlen (mess)))
    goto end;
 ...
 */
static int
test_key_update_local_in_write (int tst)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
  ...
  if (!TEST_int_eq (SSL_write (peer, mess, (int)strlen (mess)),
                    (int)strlen (mess))
      || !TEST_int_eq (SSL_read (local, buf, sizeof (buf)),
                       (int)strlen (mess)))
    goto end;
 ...
 */
static int
test_key_update_local_in_read (int tst)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
  ...
  if (!TEST_int_eq (SSL_write (peer, pwbuf, sizeof (pwbuf)), sizeof (pwbuf))
      || !TEST_int_eq (SSL_read (local, lrbuf, sizeof (lrbuf)),
                       sizeof (lrbuf)))
    goto end;
...
static int
get_MFL_from_client_hello (BIO *bio, int *mfl_codemfl_code)
{
  long len;
  unsigned char *data;
...
static int
test_max_fragment_len_ext (int idx_tst)
{
  SSL_CTX *ctx = NULL;
  SSL *con = NULL;
...
static int
ssl_srp_cb (SSL *s, int *ad, void *arg)
{
...
static int
create_new_vfile (char *userid, char *password, const char *filename)
{
  char *gNid = NULL;
  ...
  gNid = SRP_create_verifier_ex (userid, password, &row[DB_srpsalt],
                                 &row[DB_srpverifier], NULL, NULL, libctx,
                                 NULL);
  if (!TEST_ptr (gNid))
    goto end;
...
static int
create_new_vbase (char *userid, char *password)
{
  ...
  if (!TEST_true (SRP_create_verifier_BN_ex (
          userid, password, &salt, &verifier, lgN->N, lgN->g, libctx, NULL)))
    goto end;
...
static void
sslapi_info_callback (const SSL *s, int where, int ret)
{
...
#ifndef OPENSSL_NO_DTLS
      if (!TEST_true (create_ssl_ctx_pair (
              libctx, DTLS_server_method (), DTLS_client_method (),
              DTLS1_VERSION, 0, &sctx, &cctx, cert, privkey)))
        goto end;
...
static int
int_test_ssl_get_shared_ciphers (int tst, int clnt)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
  ...
  if (!TEST_ptr (SSL_get_shared_ciphers (serverssl, buf, sizeof (buf)))
      || !TEST_int_eq (strcmp (buf, is_fips
...
static int
test_ssl_get_shared_ciphers (int tst)
{
...
static SSL_TICKET_RETURN
dec_tick_cb (SSL *s, SSL_SESSION *ss, const unsigned char *keyname,
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
static int
tick_key_cb (SSL *s, unsigned char key_name[16],
...
static int
tick_key_evp_cb (SSL *s, unsigned char key_name[16],
                 unsigned char iv[EVP_MAX_IV_LENGTH], EVP_CIPHER_CTX *ctx,
 ...
 */
static int
test_incorrect_shutdown (int tst)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
 ...
 * Test 4: TLSv1.3, server continues to read/write after client shutdown,
 * server sends key update, client reads it Test 5: TLSv1.3, server continues
 * to read/write after client shutdown, server sends CertificateRequest, client
 * reads and ignores it Test 6: TLSv1.3, server continues to read/write after
 * client shutdown, client doesn't read it
 */
static int
test_shutdown (int tst)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
...
static int
load_chain (const char *file, EVP_PKEY **pkey, X509 **x509,
            STACK_OF (X509) *chain)
{
  ...
  if (cert_cb_cnt == 0)
  ...
  else if (cert_cb_cnt == 1)
    {
      ...
      if (!TEST_true (SSL_use_certificate_file (s, cert, SSL_FILETYPE_PEM))
          || !TEST_true (
              SSL_use_PrivateKey_file (s, privkey, SSL_FILETYPE_PEM))
 ...
 */
static int
test_cert_cb_int (int prot, int tst)
{
...
client_cert_cb (SSL *ssl, X509 **x509, EVP_PKEY **pkey)
{
  ...
  if (!TEST_ptr (xcert = X509_new_ex (libctx, NULL))
      ...
      || !TEST_ptr (privpkey = PEM_read_bio_PrivateKey_ex (
                        priv_in, NULL, NULL, NULL, libctx, NULL)))
    goto err;
 ...
 */
static int
test_ca_names_int (int prot, int tst)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
...
static int
test_multiblock_write (int test_index)
{
...
#if !defined(OSSL_NO_USABLE_TLS1_3) || !defined(OPENSSL_NO_TLS1_2)
static int
test_session_cache_overflow (int idx)
{
  SSL_CTX *sctx = NULL, *cctx = NULL;
  SSL *serverssl = NULL, *clientssl = NULL;
  int testresult = 0;
...
static int
test_unknown_sigalgs_groups (void)
{
  int ret = 0;
  SSL_CTX *ctx = NULL;
  ...
  if (!TEST_int_gt (
          SSL_CTX_set1_groups_list (ctx, "P-384:?nonexistent:?P-521"), 0))
    goto end;
...
static int
test_configuration_of_groups (void)
{
  int ret = 0;
  SSL_CTX *ctx = NULL;
  size_t groups_len;
 ...
 */
static int
test_sigalgs_available (int idx)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
      ...
      if (idx < 3)
      ...
      else
        {
          if (!TEST_true (
              ...
              || !TEST_true (
                  filter_provider_set_filter (OSSL_OP_KEYMGMT, "EC"))
#else
 ...
 */
static int
create_cert_key (int idx, char *certfilename, char *privkeyfilename)
{
 ...
 * "xorhmacsig" Test 1: Signature algorithm using external SHA2 hashing:
 * "xorhmacsha2sig" Test 2: Signature algorithm with built-in hashing
 * configured via SSL_CONF_cmd Test 3: Test 0 using RPK Test 4: Test 1 using
 * RPK Test 5: Test 2 using RPK
 */
static int
test_pluggable_signature (int idx)
{
...
static int
secret_cb (SSL *s, void *secretin, int *secret_len,
           STACK_OF (SSL_CIPHER) *peer_ciphers, const SSL_CIPHER **cipher,
           void *arg)
{
  int i;
 ...
 */
static int
test_legacy_ec_point_formats (void)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
 ...
 */
static int
test_set_verify_cert_store_ssl_ctx (void)
{
  SSL_CTX *ctx = NULL;
  int testresult = 0;
 ...
 */
static int
test_set_verify_cert_store_ssl (void)
{
  SSL_CTX *ctx = NULL;
  SSL *ssl = NULL;
  int testresult = 0;
...
static int
test_inherit_verify_param (void)
{
  int testresult = 0;
...
static int
test_read_ahead_key_change (void)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
...
static size_t
record_pad_cb (SSL *s, int type, size_t len, void *arg)
{
  int *called = arg;
 ...
 */
static int
test_tls13_record_padding (int idx)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
  int testresult = 0;
  ...
  if (idx == 1)
    ...
    {
      /* Exceeding the max plain length should fail */
      if (!TEST_false (SSL_set_block_padding_ex (
              clientssl, 0, SSL3_RT_MAX_PLAIN_LENGTH + 1)))
        goto end;
 ...
 * Test 5: Client has pipelining enabled, server does not: Maximum size
 * pipeline Test 6: Repeat of test 0, but the engine is loaded late (after the
 * SSL_CTX is created)
 */
static int
test_pipelining (int idx)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  ...
  if (idx == 5)
    {
       ...
       */
      if (!TEST_false (
              SSL_set_split_send_fragment (peera, (long)(fragsize + 1))))
        goto end;
    }
...
static int
check_version_string (SSL *s, int version)
{
 ...
 * SSL_is_dtls return the expected results for a (D)TLS connection. Compare
 * with test_version() in quicapitest.c which does the same thing for QUIC
 * connections.
 */
static int
test_version (int idx)
{
  SSL_CTX *cctx = NULL, *sctx = NULL;
  SSL *clientssl = NULL, *serverssl = NULL;
 ...
 * also try a BIO that indicates retry via a 0 return. BIO_write() is
 * documented to indicate retry via -1 - but sometimes BIOs don't do that.
 *
  ...
  for (i = 0; i < 3; i++)
    {
      ...
      if (!TEST_true (BIO_ctrl (bretry, MAYBE_RETRY_CTRL_SET_RETRY_AFTER_CNT,
                                1, NULL)))
        goto end;
 ...
 */
static int
resume_servername_cb (SSL *s, int *ad, void *arg)
{
...
static int
test_select_next_proto (int idx)
{
...
static int
npn_advert_cb (SSL *ssl, const unsigned char **out, unsigned int *outlen,
               void *arg)
{
  int *idx = (int *)arg;
...
static int
npn_select_cb (SSL *s, unsigned char **out, unsigned char *outlen,
               const unsigned char *in, unsigned int inlen, void *arg)
{
  int *idx = (int *)arg;
...
static int
alpn_select_cb2 (SSL *ssl, const unsigned char **out, unsigned char *outlen,
                 const unsigned char *in, unsigned int inlen, void *arg)
{
  int *idx = (int *)arg;
...
static int
crypto_send_cb (SSL *s, const unsigned char *buf, size_t buf_len,
...
static int
crypto_recv_rcd_cb (SSL *s, const unsigned char **buf, size_t *bytes_read,
                    void *arg)
{
  struct quic_tls_test_data *data = (struct quic_tls_test_data *)arg;
...
static int
crypto_release_rcd_cb (SSL *s, size_t bytes_read, void *arg)
{
  struct quic_tls_test_data *data = (struct quic_tls_test_data *)arg;
  ...
  for (i = 0; secret_history[i].recorded == 1; i++)
    {
      if (secret_history[i].ssl != s)
        continue;
      TEST_info ("Got %s(%d) secret for level %d, last level %d, last state "
                 "%d, gen %d\n",
      ...
      switch (secret_history[i].direction)
        ...
        case 1:
           ...
           * in a single iteration of the state machine, the read key is
           * yielded prior to the write key for the same level.  This is
           * undesireable for quic, but it is ok, as the general implementation
           * of every 3rd party quic stack while prefering write keys before
           * read, allows for read before write if both keys are yielded in the
           * same call to SSL_do_handshake, as the tls adaptation code for that
           * quic stack can then cache keys until both are available, so we
           * allow read before write here iff they occur in the same iteration
           * of SSL_do_handshake as represented by the recorded sm_generation
           * value.
           */
          if (last_prot_level == secret_history[i].prot_level
              && last_state == LAST_DIR_READ)
            {
              if (last_generation == secret_history[i].sm_generation)
                {
                  TEST_info ("Read before write key in same SSL state machine "
                             "iteration is ok");
                }
              else
                {
...
static int
yield_secret_cb (SSL *s, uint32_t prot_level, int direction,
...
static int
yield_secret_cb_fail (SSL *s, uint32_t prot_level, int direction,
...
static int
got_transport_params_cb (SSL *s, const unsigned char *params,
...
static int
alert_cb (SSL *s, unsigned char alert_code, void *arg)
{
  struct quic_tls_test_data *data = (struct quic_tls_test_data *)arg;
...
static void
assert_no_end_of_early_data (int write_p, int version, int content_type,
                             const void *buf, size_t msglen, SSL *ssl,
                             void *arg)
{
