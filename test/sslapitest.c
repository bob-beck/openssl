...
int
tls_provider_init(const OSSL_CORE_HANDLE* handle,
...
static int
use_session_cb(SSL* ssl,
               const EVP_MD* md,
               const unsigned char** id,
               size_t* idlen,
               SSL_SESSION** sess);
static int
find_session_cb(SSL* ssl,
...
static int
compare_hex_encoded_buffer(const char* hex_encoded,
                           size_t hex_length,
                           const uint8_t* raw,
                           size_t raw_length)
{
...
static int
test_keylog_output(char* buffer,
                   const SSL* ssl,
...
test_keylog(void)
{
  ...
  if (!TEST_true(test_keylog_output(
        client_log_buffer, clientssl, SSL_get_session(clientssl), &expected)))
    goto end;

  expected.rsa_key_exchange_count = 0;
  if (!TEST_true(test_keylog_output(
        server_log_buffer, serverssl, SSL_get_session(serverssl), &expected)))
    goto end;
...
test_client_cert_verify_cb(void)
{
  ...
  if (!TEST_false(create_ssl_connection(
        serverssl, clientssl, SSL_ERROR_WANT_RETRY_VERIFY)))
    goto end;
...
test_client_hello_cb(void)
{
  ...
  if (!TEST_true(SSL_CTX_set_cipher_list(
      ...
      !TEST_true(
        create_ssl_objects(sctx, cctx, &serverssl, &clientssl, NULL, NULL)) ||
      !TEST_false(create_ssl_connection(
        serverssl, clientssl, SSL_ERROR_WANT_CLIENT_HELLO_CB))
      /*
...
static int
execute_test_large_message(const SSL_METHOD* smeth,
...
ping_pong_query(SSL* clientssl, SSL* serverssl)
{
  ...
  if (!BIO_get_ktls_send(clientsc->wbio)) {
    if (!TEST_mem_ne(
          crec_wseq_before, SEQ_NUM_SIZE, crec_wseq_after, SEQ_NUM_SIZE))
      goto end;
  } else {
    if (!TEST_mem_eq(
          crec_wseq_before, SEQ_NUM_SIZE, crec_wseq_after, SEQ_NUM_SIZE))
  ...
  if (!BIO_get_ktls_send(serversc->wbio)) {
    if (!TEST_mem_ne(
          srec_wseq_before, SEQ_NUM_SIZE, srec_wseq_after, SEQ_NUM_SIZE))
      goto end;
  } else {
    if (!TEST_mem_eq(
          srec_wseq_before, SEQ_NUM_SIZE, srec_wseq_after, SEQ_NUM_SIZE))
  ...
  if (!BIO_get_ktls_recv(clientsc->wbio)) {
    if (!TEST_mem_ne(
          crec_rseq_before, SEQ_NUM_SIZE, crec_rseq_after, SEQ_NUM_SIZE))
      goto end;
  } else {
    if (!TEST_mem_eq(
          crec_rseq_before, SEQ_NUM_SIZE, crec_rseq_after, SEQ_NUM_SIZE))
  ...
  if (!BIO_get_ktls_recv(serversc->wbio)) {
    if (!TEST_mem_ne(
          srec_rseq_before, SEQ_NUM_SIZE, srec_rseq_after, SEQ_NUM_SIZE))
      goto end;
  } else {
    if (!TEST_mem_eq(
          srec_rseq_before, SEQ_NUM_SIZE, srec_rseq_after, SEQ_NUM_SIZE))
...
static int
execute_test_ktls(int cis_ktls,
                  int sis_ktls,
                  int tls_version,
                  const char* cipher)
{
...
execute_test_ktls_sendfile(int tls_version, const char* cipher, int zerocopy)
{
  ...
  while (chunk_off < SENDFILE_SZ) {
    ...
    /* verify the payload */
    if (!TEST_mem_eq(
          buf_dst + chunk_off, chunk_size, buf + chunk_off, chunk_size))
      goto end;
...
test_ktls(int test)
{
  ...
  return execute_test_ktls(
    cis_ktls, sis_ktls, cipher->tls_version, cipher->cipher);
}
...
test_ktls_sendfile(int test)
{
  ...
  return execute_test_ktls_sendfile(
    cipher->tls_version, cipher->cipher, test & 1);
}
#endif
...
test_large_message_tls(void)
{
  return execute_test_large_message(
    TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0, 0);
}
...
{
  return execute_test_large_message(
    TLS_server_method(), TLS_client_method(), TLS1_VERSION, 0, 1);
}
...
test_large_app_data(int tst)
{
  ...
  if (!TEST_true(create_ssl_ctx_pair(
        libctx, smeth, cmeth, prot, prot, &sctx, &cctx, cert, privkey)))
    goto end;
...
static int
execute_cleanse_plaintext(const SSL_METHOD* smeth,
                          const SSL_METHOD* cmeth,
                          int min_version,
                          int max_version)
{
...
create_ocsp_resp(X509* ssl_cert,
                 X509* issuer,
                 int status,
                 ...
                 char* signer_cert_files)
{
  ...
  if (signer_key_files != NULL && signer_cert_files != NULL) {
    ...
    if (!TEST_ptr(signer_cert = load_cert_pem(signer_cert_file, libctx)) ||
        !TEST_true(OCSP_basic_sign(
          bs, signer_cert, signer_key, EVP_sha256(), NULL, OCSP_NOCERTS)))
      goto end;
...
ocsp_server_cb_single(SSL* s, void* arg)
{
  ...
  ocsp_resp = create_ocsp_resp(ssl_cert,
                               issuer,
...
static int
execute_test_session(int maxprot,
                     int use_int_cache,
                     int use_ext_cache,
                     long s_options)
{
...
{
#ifndef OSSL_NO_USABLE_TLS1_3
  if (!execute_test_session(
        TLS1_3_VERSION, 1, 0, SSL_OP_DISABLE_TLSEXT_CA_NAMES))
    return 0;
#endif

#ifndef OPENSSL_NO_TLS1_2
  return execute_test_session(
    TLS1_2_VERSION, 1, 0, SSL_OP_DISABLE_TLSEXT_CA_NAMES);
#else
  return 1;
#endif
}
...
#ifndef OPENSSL_NO_EC
static const int validlist2[] = { NID_sha256,
                                  EVP_PKEY_RSA,
                                  NID_sha512,
                                  EVP_PKEY_EC };
 ...
 */
static int
setupearly_data_test(SSL_CTX** cctx,
                     SSL_CTX** sctx,
                     SSL** clientssl,
                     SSL** serverssl,
                     SSL_SESSION** sess,
                     ...
                     size_t mdsize)
{
  ...
  if (*sctx == NULL && !TEST_true(create_ssl_ctx_pair(libctx,
                                                      TLS_server_method(),
                                                      TLS_client_method(),
                                                      TLS1_VERSION,
                                                      0,
  ...
  switch (testtype) {
    ...
    case 2: {
      ...
      if (!TEST_true(BIO_write_ex(
            wbio, bad_early_data, sizeof(bad_early_data), &written)))
        goto end;
    }
      /* FALLTHROUGH */
...
{
  return early_data_skip_helper(
    0, idx % OSSL_NELEM(ciphersuites), idx / OSSL_NELEM(ciphersuites));
}
...
{
  return early_data_skip_helper(
    1, idx % OSSL_NELEM(ciphersuites), idx / OSSL_NELEM(ciphersuites));
}
...
{
  return early_data_skip_helper(
    2, idx % OSSL_NELEM(ciphersuites), idx / OSSL_NELEM(ciphersuites));
}
...
{
  return early_data_skip_helper(
    3, idx % OSSL_NELEM(ciphersuites), idx / OSSL_NELEM(ciphersuites));
}
...
#define BADALPN (alpnlist + GOODALPNLEN)
  ...
  if (!TEST_true(setupearly_data_test(
        &cctx, &sctx, &clientssl, &serverssl, &sess, 2, SHA384_DIGEST_LENGTH)))
    goto end;
...
test_early_data_tls1_2(int idx)
{
  ...
  if (!TEST_true(setupearly_data_test(
        &cctx, &sctx, &clientssl, &serverssl, NULL, idx, SHA384_DIGEST_LENGTH)))
    goto end;
...
static int ffdhe_kexch_groups[] = { NID_ffdhe2048,
                                    NID_ffdhe3072,
                                    NID_ffdhe4096,
                                    NID_ffdhe6144,
                                    NID_ffdhe8192 };
  ...
  for (max_ver = TLS1_2_VERSION; max_ver <= TLS1_3_VERSION; max_ver++) {
    ...
    for (i = 0; i < OSSL_NELEM(t13_ciphers); i++) {
      ...
      if (max_ver == TLS1_3_VERSION &&
          !TEST_strn_eq(
            t13_cipher, negotiated_scipher, strlen(negotiated_scipher)))
        goto end;
...
static int
generate_cookie_callback(SSL* ssl,
...
static int
verify_cookie_callback(SSL* ssl,
...
static int
generate_stateless_cookie_callback(SSL* ssl,
                                   unsigned char* cookie,
                                   size_t* cookie_len)
{
  unsigned int temp;
...
static int
verify_stateless_cookie_callback(SSL* ssl,
                                 const unsigned char* cookie,
                                 size_t cookie_len)
{
...
test_custom_exts(int tst)
{
  ...
  if (tst == 2 && !TEST_true(create_ssl_ctx_pair(libctx,
                                                 TLS_server_method(),
                                                 NULL,
                                                 TLS1_VERSION,
                                                 0,
  ...
  if (tst == 0) {
    ...
    if (!TEST_true(SSL_CTX_add_custom_ext(sctx,
                                          ...
                                          &server)))
      goto end;
    if (sctx2 != NULL && !TEST_true(SSL_CTX_add_custom_ext(sctx2,
                                                           TEST_EXT_TYPE1,
                                                           context,
                                                           new_add_cb,
                                                           new_free_cb,
...
static int
serverinfo_custom_parse_cb(SSL* s,
                           unsigned int ext_type,
                           unsigned int context,
                           const unsigned char* in,
                           size_t inlen,
  ...
  if (!TEST_int_eq(SSL_export_keying_material(clientssl,
      ...
      !TEST_int_eq(
        SSL_export_keying_material(
          clientssl, ckeymat3, sizeof(ckeymat3), label, labellen, NULL, 0, 0),
        1) ||
      (test_longcontext &&
       !TEST_int_eq(SSL_export_keying_material(clientssl,
                                               ckeymat4,
      ...
      !TEST_int_eq(
        SSL_export_keying_material(
          serverssl, skeymat3, sizeof(skeymat3), label, labellen, NULL, 0, 0),
        1) ||
      (test_longcontext &&
       !TEST_int_eq(SSL_export_keying_material(serverssl,
                                               skeymat4,
  ...
  for (j = 0; j < 2; j++) {
    ...
    for (i = 0; i < NUM_KEY_UPDATE_MESSAGES; i++) {
      if (!TEST_true(SSL_key_update(clientssl,
                                    (j == 0) ? SSL_KEY_UPDATE_NOT_REQUESTED
...
ssl_srp_cb(SSL* s, int* ad, void* arg)
{
  ...
  if (SSL_set_srp_server_param(
        s, user->N, user->g, user->s, user->v, user->info) <= 0) {
    *ad = SSL_AD_INTERNAL_ERROR;
    goto err;
  }
...
create_new_vfile(char* userid, char* password, const char* filename)
{
  ...
  gNid = SRP_create_verifier_ex(userid,
                                password,
                                &row[DB_srpsalt],
                                &row[DB_srpverifier],
                                NULL,
...
gen_tick_cb(SSL* s, void* arg)
{
  ...
  return SSL_SESSION_set1_ticket_appdata(
    SSL_get_session(s), appdata, strlen(appdata));
}
...
test_shutdown(int tst)
{
  ...
  if (tst == 3) {
    if (!TEST_true(create_bare_ssl_connection(
          serverssl, clientssl, SSL_ERROR_NONE, 1, 0)) ||
...
test_pluggable_group(int idx)
{
  ...
  if (!TEST_true(SSL_set1_groups_list(
        ...
        "dummy17:dummy18:dummy19:dummy20:dummy21:dummy22:dummy23:dummy24:"
        "dummy25:dummy26:dummy27:dummy28:dummy29:dummy30:dummy31:dummy32:"
        "dummy33:dummy34:dummy35:dummy36:dummy37:dummy38:dummy39:dummy40:"
        "dummy41:dummy42:dummy43"))
...
test_pluggable_signature(int idx)
{
  ...
  if (rpkidx) {
    if (!TEST_true(SSL_set1_server_cert_type(
          serverssl, cert_type_rpk, sizeof(cert_type_rpk))) ||
        !TEST_true(SSL_set1_server_cert_type(
          clientssl, cert_type_rpk, sizeof(cert_type_rpk))))
      goto end;
  }
...
test_dh_auto(int idx)
{
  ...
  if (!TEST_true(create_ssl_ctx_pair(
        libctx, NULL, NULL, 0, 0, &sctx, &cctx, thiscert, thiskey)))
    goto end;
...
test_rstate_string(void)
{
  ...
  if (!TEST_true(create_ssl_ctx_pair(
        libctx, servmeth, clientmeth, 0, 0, &sctx, &cctx, cert, privkey)))
    goto end;
   ...
   */
  if (!TEST_true(BIO_write_ex(
        SSL_get_rbio(serverssl), dummyheader, sizeof(dummyheader), &written)) ||
...
test_data_retry(void)
{
  ...
  while (SSL_read_ex(
    serverssl, outbuf + totread, sizeof(outbuf) - totread, &readbytes))
    totread += readbytes;
...
resume_servername_cb(SSL* s, int* ad, void* arg)
{
  ...
  if (!TEST_true(create_ssl_objects(
        cbdata->sctx, cbdata->cctx, &serverssl, &clientssl, NULL, NULL)) ||
...
test_select_next_proto(int idx)
{
  ...
  if (!TEST_int_eq(SSL_select_next_proto(
                     &out, &outlen, server, serverlen, client, clientlen),
                   np->expected_ret))
    goto err;
...
test_quic_tls(int idx)
{
  ...
  if (!TEST_true(SSL_set_quic_tls_cbs(clientssl, qtdis, &cdata)) ||
      ...
      !TEST_true(SSL_set_quic_tls_transport_params(
        clientssl, cparams, sizeof(cparams))) ||
      !TEST_true(
        SSL_set_quic_tls_transport_params(serverssl, sparams, sizeof(sparams))))
    goto end;

  if (idx != 1 && idx != 4) {
    if (!TEST_true(create_ssl_connection_ex(serverssl,
                                            clientssl,
                                            SSL_ERROR_NONE,
                                            &cdata.sm_count,
                                            &sdata.sm_count)))
    ...
    if (!TEST_false(create_ssl_connection_ex(serverssl,
                                             clientssl,
                                             SSL_ERROR_NONE,
                                             &cdata.sm_count,
                                             &sdata.sm_count)))
  ...
  for (i = OSSL_RECORD_PROTECTION_LEVEL_EARLY - 1;
       ...
       i++) {
    if (!TEST_mem_eq(sdata.wsecret[i],
                     sdata.wsecret_len[i],
                     cdata.rsecret[i],
                     cdata.rsecret_len[i]))
      goto end;
...
static void
assert_no_end_of_early_data(int write_p,
                            int version,
                            int content_type,
                            const void* buf,
                            size_t msglen,
...
test_no_renegotiation(int idx)
{
  ...
  if (!TEST_true(create_ssl_ctx_pair(
        libctx, sm, cm, 0, max_proto, &sctx, &cctx, cert, privkey)))
    goto end;
