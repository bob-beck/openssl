...
/*
 * If a version is valid for *any* protocol then setting the min/max protocol
 * is expected to return success, even if that version is not valid for *this*
...
static int
test_set_min_max_version (int idx_tst)
{
  SSL_CTX *ctx = NULL;
  SSL *ssl = NULL;
  int testresult = 0;
