...
/* helper function */
static BIGNUM *
bn_load_new (const unsigned char *data, int sz)
{
  BIGNUM *ret = BN_new ();
  if (ret != NULL)
...
static int
test_check_public_exponent (void)
{
  int ret = 0;
  BIGNUM *e = NULL;
...
static int
test_check_prime_factor_range (void)
{
  int ret = 0;
  BN_CTX *ctx = NULL;
  BIGNUM *p = NULL;
...
static int
test_check_private_exponent (void)
{
  int ret = 0;
  RSA *key = NULL;
  BN_CTX *ctx = NULL;
...
static int
test_check_crt_components (void)
{
  const int P = 15;
  const int Q = 17;
  const int E = 5;
...
static int
test_derive_params_from_pq_fail (int tst)
{
  int ret = 0;
  RSA *key = NULL;
  BN_CTX *ctx = NULL;
