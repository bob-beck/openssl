...
static int do_bio_comp_test(const BIO_METHOD* meth, size_t size) {
  ...
  /* Compress */
  if (!TEST_ptr(meth)) goto err;
  if (!TEST_ptr(bcomp = BIO_new(meth))) goto err;
  if (!TEST_ptr(bmem = BIO_new(BIO_s_mem()))) goto err;
  BIO_push(bcomp, bmem);
  osize = BIO_write(bcomp, original, (int)size);
  if (!TEST_int_eq(osize, (int)size) || !TEST_true(BIO_flush(bcomp))) goto err;
  BIO_free(bcomp);
  bcomp = NULL;

  /* decompress */
  if (!TEST_ptr(bexp = BIO_new(meth))) goto err;
  BIO_push(bexp, bmem);
...
static int do_bio_comp(const BIO_METHOD* meth, int n) {
  ...
  if (!TEST_ptr(original) || !TEST_ptr(result)) goto err;
  ...
  switch (type) {
    ...
    case 3:
      ...
      if (!TEST_int_gt(RAND_bytes(original, BUFFER_SIZE), 0)) goto err;
      break;
    default:
      goto err;
  }

  if (!TEST_true(do_bio_comp_test(meth, size))) goto err;
  success = 1;
err:
