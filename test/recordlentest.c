...
static int write_record(BIO* b, size_t len, uint8_t rectype, int recversion) {
  ...
  while (len > 0) {
    ...
    if (!BIO_write_ex(b, buf, outlen, &written) || written != outlen) return 0;
...
static int test_record_overflow(int idx) {
  ...
  if (idx == TEST_PLAINTEXT_OVERFLOW_OK ||
    ...
    if (idx == TEST_PLAINTEXT_OVERFLOW_NOT_OK) len++;

    if (!TEST_true(
            write_record(serverbio, len, SSL3_RT_HANDSHAKE, TLS1_VERSION)))
      goto end;

    if (!TEST_int_le(SSL_accept(serverssl), 0)) goto end;

    overf_expected = (idx == TEST_PLAINTEXT_OVERFLOW_OK) ? 0 : 1;
    if (!TEST_int_eq(fail_due_to_record_overflow(0), overf_expected)) goto end;
  ...
  if (!TEST_int_eq(fail_due_to_record_overflow(1), overf_expected)) goto end;
