...
static void* init_object(void) {
  ...
  obj->p1 = p1_init;
  obj->p2 = p2_init;
  if (!TEST_true(BN_hex2bn(&obj->p3, p3_init))) goto fail;
  if (!TEST_ptr(obj->p4 = OPENSSL_strdup(p4_init))) goto fail;
...
static int raw_set_params(void* vobj, const OSSL_PARAM* params) {
  ...
  for (; params->key != NULL; params++)
    if (strcmp(params->key, "p1") == 0) {
      ...
      BN_free(obj->p3);
      if (!TEST_ptr(obj->p3 = BN_native2bn(params->data, (int)params->data_size,
                                           NULL)))
        return 0;
      ...
      if (!TEST_size_t_lt(data_length, sizeof(obj->p5))) return 0;
  ...
  for (; params->key != NULL; params++)
    if (strcmp(params->key, "p1") == 0) {
      ...
      *(double*)params->data = obj->p2;
      ...
      if (!TEST_size_t_ge(params->data_size, params->return_size)) return 0;
      BN_bn2nativepad(obj->p3, params->data, (int)params->return_size);
    } else if (strcmp(params->key, "p4") == 0) {
      params->return_size = strlen(obj->p4);
      if (!TEST_size_t_gt(params->data_size, params->return_size)) return 0;
...
static int api_set_params(void* vobj, const OSSL_PARAM* params) {
  ...
  if ((p = OSSL_PARAM_locate_const(params, "p6")) != NULL) {
    if (!TEST_true(OSSL_PARAM_get_utf8_ptr(p, &obj->p6))) return 0;
