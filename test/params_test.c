...
static int
raw_set_params(void* vobj, const OSSL_PARAM* params)
{
  struct object_st* obj = vobj;
  ...
  for (; params->key != NULL; params++)
    if (strcmp(params->key, "p1") == 0) {
      ...
      BN_free(obj->p3);
      if (!TEST_ptr(obj->p3 =
                      BN_native2bn(params->data, (int)params->data_size, NULL)))
        return 0;
...
static int
raw_get_params(void* vobj, OSSL_PARAM* params)
{
  struct object_st* obj = vobj;
...
static int
api_set_params(void* vobj, const OSSL_PARAM* params)
{
  struct object_st* obj = vobj;
...
static int
api_get_params(void* vobj, OSSL_PARAM* params)
{
  struct object_st* obj = vobj;
  OSSL_PARAM* p = NULL;
...
static int
test_case_variant(OSSL_PARAM* params, const struct provider_dispatch_st* prov)
{
...
static int
check_int_from_text(const struct int_from_text_test_st a)
{
  OSSL_PARAM param;
  long int val = 0;
  int res;
  ...
  if (!OSSL_PARAM_allocate_from_text(
        &param, params_from_text, a.argname, a.strval, 0, NULL)) {
    if (a.expected_res)
      TEST_error(
...
static int
check_octetstr_from_hexstr(void)
{
  OSSL_PARAM param;
...
static int
test_allocate_from_text(int i)
{
  return check_int_from_text(int_from_text_test_cases[i]);
}

static int
test_more_allocate_from_text(void)
{
