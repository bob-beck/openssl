...
static unsigned char cbc_ok[32] = { 0xcc, 0xd1, 0x73, 0xff, 0xab, 0x20, 0x39,
                                    0xf4, 0xac, 0xd8, 0xae, 0xfd, 0xdf, 0xd8,
                                    0xa1, 0xeb, 0x46, 0x8e, 0x91, 0x15, 0x78,
                                    0x88, 0xba, 0x68, 0x1d, 0x26, 0x93, 0x97,
                                    0xf7, 0xfe, 0x62, 0xb4 };
...
static unsigned char cbc3_ok[32] = { 0x3F, 0xE3, 0x01, 0xC9, 0x62, 0xAC, 0x01,
                                     0xD0, 0x22, 0x13, 0x76, 0x3C, 0x1C, 0xBD,
                                     0x4C, 0xDC, 0x79, 0x96, 0x57, 0xC0, 0x64,
                                     0xEC, 0xF5, 0xD4, 0x1C, 0x67, 0x38, 0x12,
                                     0xCF, 0xDE, 0x96, 0x75 };

static unsigned char pcbc_ok[32] = { 0xcc, 0xd1, 0x73, 0xff, 0xab, 0x20, 0x39,
                                     0xf4, 0x6d, 0xec, 0xb4, 0x70, 0xa0, 0xe5,
                                     0x6b, 0x15, 0xae, 0xa6, 0xbf, 0x61, 0xed,
                                     0x7d, 0x9c, 0x9f, 0xf7, 0x17, 0x46, 0x3b,
                                     0x8a, 0xb3, 0xcc, 0x88 };
...
static unsigned char plain[24] = { 0x4e, 0x6f, 0x77, 0x20, 0x69, 0x73,
                                   0x20, 0x74, 0x68, 0x65, 0x20, 0x74,
                                   0x69, 0x6d, 0x65, 0x20, 0x66, 0x6f,
                                   0x72, 0x20, 0x61, 0x6c, 0x6c, 0x20 };

static unsigned char cfb_cipher8[24] = { 0xf3, 0x1f, 0xda, 0x07, 0x01, 0x14,
                                         0x62, 0xee, 0x18, 0x7f, 0x43, 0xd8,
                                         0x0a, 0x7c, 0xd9, 0xb5, 0xb0, 0xd2,
                                         0x90, 0xda, 0x6e, 0x5b, 0x9a, 0x87 };

static unsigned char cfb_cipher16[24] = { 0xF3, 0x09, 0x87, 0x87, 0x7F, 0x57,
                                          0xF7, 0x3C, 0x36, 0xB6, 0xDB, 0x70,
                                          0xD8, 0xD5, 0x34, 0x19, 0xD3, 0x86,
                                          0xB2, 0x23, 0xB7, 0xB2, 0xAD, 0x1B };

static unsigned char cfb_cipher32[24] = { 0xF3, 0x09, 0x62, 0x49, 0xA4, 0xDF,
                                          0xA4, 0x9F, 0x33, 0xDC, 0x7B, 0xAD,
                                          0x4C, 0xC8, 0x9F, 0x64, 0xE4, 0x53,
                                          0xE5, 0xEC, 0x67, 0x20, 0xDA, 0xB6 };

static unsigned char cfb_cipher48[24] = { 0xF3, 0x09, 0x62, 0x49, 0xC7, 0xF4,
                                          0x30, 0xB5, 0x15, 0xEC, 0xBB, 0x85,
                                          0x97, 0x5A, 0x13, 0x8C, 0x68, 0x60,
                                          0xE2, 0x38, 0x34, 0x3C, 0xDC, 0x1F };

static unsigned char cfb_cipher64[24] = { 0xF3, 0x09, 0x62, 0x49, 0xC7, 0xF4,
...
static unsigned char ofb_cipher[24] = { 0xf3, 0x09, 0x62, 0x49, 0xc7, 0xf4,
                                        0x6e, 0x51, 0x35, 0xf2, 0x4a, 0x24,
                                        0x2e, 0xeb, 0x3d, 0x3f, 0x3d, 0x6d,
                                        0x5b, 0xe3, 0x25, 0x5a, 0xf8, 0xc3 };
static DES_LONG cbc_cksum_ret = 0xF7FE62B4L;
static unsigned char cbc_cksum_data[8] = { 0x1D, 0x26, 0x93, 0x97,
                                           0xf7, 0xfe, 0x62, 0xb4 };

static char*
pt(const unsigned char* p, char buf[DATA_BUF_SIZE])
{
  char* ret;
  int i;
...
test_des_ecb(int i)
{
  ...
  if (!TEST_mem_eq(out, 8, cipher_data[i], 8)) {
    TEST_info(
      "Encryption error %2d k=%s p=%s", i + 1, pt(key_data[i], b1), pt(in, b2));
    return 0;
  }
  if (!TEST_mem_eq(in, 8, outin, 8)) {
    TEST_info("Decryption error %2d k=%s p=%s",
              i + 1,
              pt(key_data[i], b1),
              pt(out, b2));
    return 0;
...
test_des_cbc(void)
{
  ...
  DES_ncbc_encrypt(
    cbc_data, cbc_out, (long)(cbc_data_len + 1), &ks, &iv3, DES_ENCRYPT);
  if (!TEST_mem_eq(cbc_out, 32, cbc_ok, 32))
    return 0;

  memcpy(iv3, cbc_iv, sizeof(cbc_iv));
  DES_ncbc_encrypt(
    cbc_out, cbc_in, (long)(cbc_data_len + 1), &ks, &iv3, DES_DECRYPT);
  return TEST_mem_eq(cbc_in, cbc_data_len, cbc_data, cbc_data_len);
}

static int
test_des_ede_cbc(void)
{
  DES_cblock iv3;
  DES_key_schedule ks;
  unsigned char cbc_in[40];
  ...
  memcpy(iv3, cbc_iv, sizeof(cbc_iv));
  DES_xcbc_encrypt(
    cbc_data, cbc_out, (long)n, &ks, &iv3, &cbc2_key, &cbc3_key, DES_ENCRYPT);
  if (!TEST_mem_eq(cbc_out, sizeof(xcbc_ok), xcbc_ok, sizeof(xcbc_ok)))
    return 0;
  memcpy(iv3, cbc_iv, sizeof(cbc_iv));
  DES_xcbc_encrypt(
    cbc_out, cbc_in, (long)n, &ks, &iv3, &cbc2_key, &cbc3_key, DES_DECRYPT);
...
test_ede_cbc(void)
{
  ...
  DES_ede3_cbc_encrypt(
    cbc_data, cbc_out, 16L, &ks, &ks2, &ks3, &iv3, DES_ENCRYPT);
  DES_ede3_cbc_encrypt(&cbc_data[16],
                       &cbc_out[16],
                       (long)(i - 16),
                       &ks,
                       &ks2,
  ...
  memcpy(iv3, cbc_iv, sizeof(cbc_iv));
  DES_ede3_cbc_encrypt(
    cbc_out, cbc_in, (long)i, &ks, &ks2, &ks3, &iv3, DES_DECRYPT);
  return TEST_mem_eq(cbc_in, i, cbc_data, i);
}

static int
test_input_align(int i)
{
  unsigned char cbc_out[40];
  DES_cblock iv;
  DES_key_schedule ks;
...
cfb_test(int bits, unsigned char* cfb_cipher)
{
  ...
  DES_cfb_encrypt(
    plain, cfb_buf1, bits, sizeof(plain), &ks, &cfb_tmp, DES_ENCRYPT);
  if (!TEST_mem_eq(cfb_cipher, sizeof(plain), cfb_buf1, sizeof(plain)))
    return 0;
  memcpy(cfb_tmp, cfb_iv, sizeof(cfb_iv));
  DES_cfb_encrypt(
    cfb_buf1, cfb_buf2, bits, sizeof(plain), &ks, &cfb_tmp, DES_DECRYPT);
...
test_des_cfb64(void)
{
  ...
  for (i = 0; i < sizeof(plain); i++)
    DES_cfb_encrypt(
      &cfb_buf1[i], &cfb_buf2[i], 8, 1, &ks, &cfb_tmp, DES_DECRYPT);
  return TEST_mem_eq(plain, sizeof(plain), cfb_buf2, sizeof(plain));
}

static int
test_des_ede_cfb64(void)
{
  DES_key_schedule ks;
  int n;
  ...
  DES_ede3_cfb64_encrypt(
    plain, cfb_buf1, 12, &ks, &ks, &ks, &cfb_tmp, &n, DES_ENCRYPT);
  DES_ede3_cfb64_encrypt(&plain[12],
                         &cfb_buf1[12],
                         sizeof(plain) - 12,
                         &ks,
                         &ks,
  ...
  n = 0;
  DES_ede3_cfb64_encrypt(
    cfb_buf1, cfb_buf2, (long)17, &ks, &ks, &ks, &cfb_tmp, &n, DES_DECRYPT);
  DES_ede3_cfb64_encrypt(&cfb_buf1[17],
                         &cfb_buf2[17],
                         sizeof(plain) - 17,
                         &ks,
                         &ks,
...
test_des_ede_ofb64(void)
{
  ...
  for (i = 0; i < sizeof(plain); i++) {
    DES_ede3_ofb64_encrypt(
      &plain[i], &ofb_buf1[i], 1, &ks, &ks, &ks, &ofb_tmp, &num);
  }
  if (!TEST_mem_eq(ofb_cipher, sizeof(ofb_buf1), ofb_buf1, sizeof(ofb_buf1)))
    return 0;
  memcpy(ofb_tmp, ofb_iv, sizeof(ofb_iv));
  num = 0;
  DES_ede3_ofb64_encrypt(
    ofb_buf1, ofb_buf2, sizeof(ofb_buf1), &ks, &ks, &ks, &ofb_tmp, &num);
  return TEST_mem_eq(plain, sizeof(ofb_buf2), ofb_buf2, sizeof(ofb_buf2));
}

static int
test_des_cbc_cksum(void)
{
  DES_LONG cs;
  DES_key_schedule ks;
  ...
  cs =
    DES_cbc_cksum(cbc_data, &cret, (long)strlen((char*)cbc_data), &ks, &cbc_iv);
...
static int
test_des_quad_cksum(void)
{
  DES_LONG cs, lqret[4];

  cs = DES_quad_cksum(cbc_data,
                      (DES_cblock*)lqret,
                      (long)strlen((char*)cbc_data),
                      2,
                      (DES_cblock*)cbc_iv);
...
static int
test_des_check_bad_parity(int n)
{
