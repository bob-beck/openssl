...
static int test_des_cbc(void) {
  ...
  if (!TEST_int_eq(DES_set_key_checked(&cbc_key, &ks), 0)) return 0;
  memset(cbc_out, 0, sizeof(cbc_out));
  memset(cbc_in, 0, sizeof(cbc_in));
  memcpy(iv3, cbc_iv, sizeof(cbc_iv));
  DES_ncbc_encrypt(cbc_data, cbc_out, (long)(cbc_data_len + 1), &ks, &iv3,
                   DES_ENCRYPT);
  if (!TEST_mem_eq(cbc_out, 32, cbc_ok, 32)) return 0;
...
static int test_ede_cbc(void) {
  ...
  DES_ede3_cbc_encrypt(&cbc_data[16], &cbc_out[16], (long)(i - 16), &ks, &ks2,
                       &ks3, &iv3, DES_ENCRYPT);
  if (!TEST_mem_eq(cbc_out, n, cbc3_ok, n)) return 0;
...
static int test_des_crypt(void) {
  if (!TEST_str_eq("efGnQx2725bI2", DES_crypt("testing", "ef"))) return 0;
  if (!TEST_str_eq("yA1Rp/1hZXIJk", DES_crypt("bca76;23", "yA"))) return 0;
...
static int test_des_cfb64(void) {
  ...
  if (!cfb_test(64, cfb_cipher64)) return 0;
  ...
  if (!TEST_mem_eq(plain, sizeof(plain), cfb_buf2, sizeof(plain))) return 0;
...
static int test_des_cbc_cksum(void) {
  ...
  cs = DES_cbc_cksum(cbc_data, &cret, (long)strlen((char*)cbc_data), &ks,
                     &cbc_iv);
  if (!TEST_cs_eq(cs, cbc_cksum_ret)) return 0;
...
static int test_des_key_wrap(int idx) {
  ...
  if (!TEST_int_eq(clen_upd, expect)) goto err;
  ...
  if (!TEST_true(EVP_DecryptFinal(ctx, p_txt + plen_upd, &plen_fin))) goto err;
  plen = plen_upd + plen_fin;

  if (!TEST_mem_eq(in, in_bytes, p_txt, plen)) goto err;
  res = 1;
err:
