...
static int self_test_events(const OSSL_PARAM params[], void* arg,
  ...
  if (args->count == 0) BIO_printf(bio_out, "\n%s\n", title);
  args->count++;
   ...
   */
  if (corrupt && strcmp(phase, OSSL_SELF_TEST_PHASE_CORRUPT) == 0) goto err;
  ret = 1;
err:
  return ret;
}
...
static int get_provider_params(const OSSL_PROVIDER* prov) {
  ...
  if (!TEST_true(OSSL_PROVIDER_get_params(prov, params))) goto end;
...
static int test_provider_status(void) {
  ...
  if (!TEST_ptr(prov = OSSL_PROVIDER_load(libctx, provider_name))) goto err;
  if (!get_provider_params(prov)) goto err;
  ...
  if (!TEST_true(OSSL_PROVIDER_get_params(prov, params)) ||
      !TEST_true(status == 1))
    goto err;
  if (!TEST_ptr(fetch = EVP_MD_fetch(libctx, "SHA256", NULL))) goto err;
  EVP_MD_free(fetch);
  fetch = NULL;
  ...
  if (!TEST_true(OSSL_PROVIDER_self_test(prov))) goto err;
  ...
  OSSL_SELF_TEST_set_callback(libctx, self_test_on_demand_fail,
                              &self_test_args);
  if (!TEST_false(OSSL_PROVIDER_self_test(prov))) goto err;
  if (!TEST_true(OSSL_PROVIDER_get_params(prov, params)) ||
      !TEST_uint_eq(status, 0))
    goto err;
  if (!TEST_ptr_null(fetch = EVP_MD_fetch(libctx, "SHA256", NULL))) goto err;
...
static int test_provider_gettable_params(void) {
  ...
  if (!TEST_ptr(prov = OSSL_PROVIDER_load(libctx, provider_name))) return 0;
