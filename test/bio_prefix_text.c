...
static int
setup_bio_chain(const char* progname)
{
  BIO* next = NULL;
  size_t n = amount;
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_INDENT:
        ...
        if (endptr[0] != '\0') {
          BIO_printf(
            bio_err, "%s: -i value isn't a decimal number: %s", progname, arg);
          return 0;
        }
        if (idx >= amount) {
          BIO_printf(bio_err,
                     "%s: index (%zu) not within range 0..%zu",
                     progname,
                     idx,
                     amount - 1);
          return 0;
        ...
        if (BIO_set_indent(chain[idx], (long)indent) <= 0) {
          BIO_printf(
            bio_err, "%s: failed setting indentation: %s", progname, arg);
          return 0;
        }
        break;
      case OPT_PREFIX:
