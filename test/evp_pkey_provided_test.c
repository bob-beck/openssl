...
static int
compare_with_file(const char* alg, int type, BIO* membio)
{
  char filename[80];
  BIO* file = NULL;
  char buf[4096];
...
static int
pass_cb_error(char* buf, int size, int rwflag, void* u)
{
  return -1;
}

static int
test_print_key_using_pem(const char* alg, const EVP_PKEY* pk)
{
...
static int
test_print_key_type_using_encoder(const char* alg, int type, const EVP_PKEY* pk)
{
...
static int
test_print_key_using_encoder(const char* alg, const EVP_PKEY* pk)
{
  int i;
  int ret = 1;
...
static int
test_print_key_using_encoder_public(const char* alg, const EVP_PKEY* pk)
{
  int i;
  int ret = 1;
...
static int
do_fromdata_rsa_derive(OSSL_PARAM* fromdata_params,
...
static int
test_fromdata_rsa_derive_from_pq_sp800(void)
{
  ...
  static unsigned char p_data[] = { 0x00, 0xf6, 0x61, 0x38, 0x0e, 0x1f, 0x82,
                                    0x7c, 0xb8, 0xba, 0x00, 0xd3, 0xac, 0xdc,
                                    0x4e, 0x6b, 0x7e, 0xf7, 0x58, 0xf3, 0xd9,
                                    0xd8, 0x21, 0xed, 0x54, 0xa3, 0x36, 0xd2,
                                    0x2c, 0x5f, 0x06, 0x7d, 0xc5 };
  static unsigned char q_data[] = { 0x00, 0xce, 0xcc, 0x4a, 0xa5, 0x4f, 0xd6,
                                    0x73, 0xd0, 0x20, 0xc3, 0x98, 0x64, 0x20,
                                    0x9b, 0xc1, 0x23, 0xd8, 0x5c, 0x82, 0x4f,
                                    0xe8, 0xa5, 0x32, 0xcd, 0x7e, 0x97, 0xb4,
                                    0xde, 0xf6, 0x4c, 0x80, 0xdb };
  static unsigned char dmp1_data[] = { 0x00, 0xd1, 0x07, 0xb6, 0x79, 0x34, 0xfe,
                                       0x8e, 0x36, 0x63, 0x88, 0xa4, 0x0e, 0x3a,
                                       0x73, 0x45, 0xfc, 0x58, 0x7a, 0x5d, 0x98,
                                       0xeb, 0x28, 0x0d, 0xa5, 0x0b, 0x3c, 0x4d,
                                       0xa0, 0x5b, 0x96, 0xb4, 0x49 };
  static unsigned char dmq1_data[] = { 0x5b, 0x47, 0x02, 0xdf, 0xaa, 0xb8, 0xae,
                                       0x8f, 0xbc, 0x16, 0x79, 0x6a, 0x20, 0x96,
                                       0x7f, 0x0e, 0x92, 0x4e, 0x6a, 0xda, 0x58,
                                       0x86, 0xaa, 0x40, 0xd7, 0xd2, 0xa0, 0x6c,
                                       0x15, 0x6c, 0xb9, 0x27 };
  static unsigned char iqmp_data[] = { 0x00, 0xa0, 0xd6, 0xf0, 0xe8, 0x17, 0x9e,
                                       0xe7, 0xe6, 0x99, 0x12, 0xd6, 0xd9, 0x43,
                                       0xcf, 0xed, 0x37, 0x29, 0xf5, 0x6c, 0x3e,
                                       0xc1, 0x7f, 0x2e, 0x31, 0x3f, 0x64, 0x34,
                                       0x66, 0x68, 0x5c, 0x22, 0x08 };
...
static int
test_fromdata_rsa_derive_from_pq_multiprime(void)
{
  OSSL_PARAM_BLD* bld = NULL;
...
static int
test_evp_pkey_get_bn_param_large(void)
{
  int ret = 0;
  EVP_PKEY_CTX *ctx = NULL, *key_ctx = NULL;
  EVP_PKEY* pk = NULL;
...
static int
test_fromdata_dh_named_group(void)
{
  int ret = 0;
...
static int
test_fromdata_dh_fips186_4(void)
{
  int ret = 0;
  int gindex = 0, pcounter = 0, hindex = 0;
  EVP_PKEY_CTX *ctx = NULL, *key_ctx = NULL;
...
test_fromdata_ecx(int tst)
{
  ...
  OSSL_PARAM x25519_fromdata_params[] = {
    ...
    OSSL_PARAM_octet_string(
      OSSL_PKEY_PARAM_PUB_KEY, key_numbers[X25519_IDX][PUB_KEY], X25519_KEYLEN),
    OSSL_PARAM_END
  };
  OSSL_PARAM x448_fromdata_params[] = {
    OSSL_PARAM_octet_string(
      OSSL_PKEY_PARAM_PRIV_KEY, key_numbers[X448_IDX][PRIV_KEY], X448_KEYLEN),
    OSSL_PARAM_octet_string(
      OSSL_PKEY_PARAM_PUB_KEY, key_numbers[X448_IDX][PUB_KEY], X448_KEYLEN),
    OSSL_PARAM_END
  };
  ...
  OSSL_PARAM ed448_fromdata_params[] = {
    OSSL_PARAM_octet_string(
      OSSL_PKEY_PARAM_PRIV_KEY, key_numbers[ED448_IDX][PRIV_KEY], ED448_KEYLEN),
    OSSL_PARAM_octet_string(
      OSSL_PKEY_PARAM_PUB_KEY, key_numbers[ED448_IDX][PUB_KEY], ED448_KEYLEN),
    OSSL_PARAM_END
  };
  OSSL_PARAM* fromdata_params = NULL;
  ...
  for (;;) {
    ...
    if (tst <= 7) {
      if (!TEST_int_gt(EVP_PKEY_check(ctx2), 0))
        goto err;
      if (!TEST_true(
            EVP_PKEY_get_octet_string_param(pk,
...
test_fromdata_ec(void)
{
  ...
  if (OSSL_PARAM_BLD_push_utf8_string(
        bld, OSSL_PKEY_PARAM_GROUP_NAME, curve, 0) <= 0)
    goto err;
  /*
  ...
  for (;;) {
    ...
    if (!EVP_PKEY_get_utf8_string_param(pk,
        ...
        !EVP_PKEY_get_octet_string_param(
          pk, OSSL_PKEY_PARAM_PUB_KEY, out_pub, sizeof(out_pub), &len)
        ...
        !TEST_mem_eq(
          out_pub + 1, len - 1, ec_pub_keydata + 1, sizeof(ec_pub_keydata) - 1)
...
static int
test_ec_dup_keygen_operation(void)
{
  int ret = 0;
  EVP_PKEY_CTX *pctx = NULL, *ctx = NULL, *kctx = NULL;
  EVP_PKEY *param = NULL, *pkey = NULL;
...
static int
test_fromdata_dsa_fips186_4(void)
{
  int ret = 0;
  EVP_PKEY_CTX *ctx = NULL, *key_ctx = NULL;
  EVP_PKEY *pk = NULL, *copy_pk = NULL, *dup_pk = NULL;
...
static OSSL_PARAM*
do_construct_hkdf_params(char* digest, char* key, size_t keylen, char* salt)
{
...
static int
test_evp_pkey_ctx_dup_kdf(void)
{
  int ret = 0;
  size_t len = 0, dlen = 0;
