...
static int compare_with_file(const char* alg, int type, BIO* membio) {
  ...
  if (!TEST_mem_eq(memdata, slen, buf, readbytes)) goto err;
...
static int test_print_key_using_pem(const char* alg, const EVP_PKEY* pk) {
  ...
  if (/* Output Encrypted private key in PEM form */
      ...
      /* NULL key */
      || !TEST_false(PEM_write_bio_PrivateKey(membio, NULL, NULL, NULL, 0, NULL,
                                              NULL)) ||
...
static int test_fromdata_rsa(void) {
  ...
  if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(NULL, "RSA", NULL))) goto err;
  ...
  for (;;) {
    ...
    if (!TEST_ptr(key_ctx = EVP_PKEY_CTX_new_from_pkey(NULL, pk, ""))) goto err;
    ...
    if (!ret || dup_pk != NULL) break;

    if (!TEST_ptr(dup_pk = EVP_PKEY_dup(pk))) goto err;
...
static int do_fromdata_rsa_derive(OSSL_PARAM* fromdata_params,
  ...
  for (i = 0; check[i].pname != NULL; i++) {
    if (!TEST_ptr(check_param =
      ...
      goto err;
    if (!TEST_int_eq(OSSL_PARAM_get_BN(check_param, &check_bn), 1)) goto err;
...
static int test_fromdata_dh_named_group(void) {
  ...
  if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(NULL, "DH", NULL))) goto err;
  ...
  if (!TEST_true(EVP_PKEY_get_utf8_string_param(
      ...
      || !TEST_true(EVP_PKEY_get_utf8_string_param(
             pk, OSSL_PKEY_PARAM_GROUP_NAME, name_out, sizeof(group_name),
             &len)) ||
      !TEST_size_t_eq(len, sizeof(group_name) - 1)
...
static int test_fromdata_ecx(int tst) {
  ...
  for (;;) {
    ...
    if (!TEST_ptr(ctx2 = EVP_PKEY_CTX_new_from_pkey(NULL, pk, NULL))) goto err;
    if (tst <= 7) {
      if (!TEST_int_gt(EVP_PKEY_check(ctx2), 0)) goto err;
...
static int test_fromdata_ec(void) {
  ...
  if (OSSL_PARAM_BLD_push_BN(bld, OSSL_PKEY_PARAM_PRIV_KEY, ec_priv_bn) <= 0)
    goto err;
  if (!TEST_ptr(fromdata_params = OSSL_PARAM_BLD_to_param(bld))) goto err;
  ctx = EVP_PKEY_CTX_new_from_name(NULL, alg, NULL);
  if (!TEST_ptr(ctx)) goto err;
...
static int test_fromdata_dsa_fips186_4(void) {
  ...
  if (!TEST_ptr(ctx = EVP_PKEY_CTX_new_from_name(NULL, "DSA", NULL))) goto err;
...
static int test_evp_pkey_ctx_dup_kdf(void) {
  ...
  if (!TEST_ptr(pctx = EVP_PKEY_CTX_new_from_name(NULL, "HKDF", NULL)))
    goto err;
  if (!TEST_int_eq(EVP_PKEY_derive_init_ex(pctx, params), 1)) goto err;
  if (!TEST_ptr(dctx = EVP_PKEY_CTX_dup(pctx))) goto err;
