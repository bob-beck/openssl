...
static int
test_print_key_type_using_encoder (const char *alg, int type,
                                   const EVP_PKEY *pk)
{
...
static int
do_fromdata_rsa_derive (OSSL_PARAM *fromdata_params, struct check_data check[],
...
test_fromdata_rsa_derive_from_pq_multiprime (void)
{
  ...
  if (!TEST_ptr (bld = OSSL_PARAM_BLD_new ())
      ...
      || !TEST_ptr (coeff2
                    = BN_bin2bn (coeff2_data, sizeof (coeff2_data), NULL))
      || !TEST_ptr (dmp1 = BN_bin2bn (dmp1_data, sizeof (dmp1_data), NULL))
      || !TEST_ptr (dmq1 = BN_bin2bn (dmq1_data, sizeof (dmq1_data), NULL))
      || !TEST_ptr (iqmp = BN_bin2bn (iqmp_data, sizeof (iqmp_data), NULL))
      || !TEST_true (OSSL_PARAM_BLD_push_BN (bld, OSSL_PKEY_PARAM_RSA_N, n))
