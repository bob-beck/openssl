...
static int test_alt_chains_cert_forgery(void) {
  ...
  if (store == NULL) goto err;

  lookup = X509_STORE_add_lookup(store, X509_LOOKUP_file());
  if (lookup == NULL) goto err;
  if (X509_LOOKUP_load_file(lookup, roots_f, X509_FILETYPE_PEM) <= 0) goto err;

  untrusted = load_certs_pem(untrusted_f);

  if ((x = load_cert_from_file(bad_f)) == NULL) goto err;

  sctx = X509_STORE_CTX_new();
  if (sctx == NULL) goto err;

  if (!X509_STORE_CTX_init(sctx, store, x, untrusted)) goto err;
...
static int test_distinguishing_id(void) {
  ...
  if (!TEST_ptr(v2) || !TEST_int_eq(ASN1_OCTET_STRING_cmp(v, v2), 0)) goto err;
...
static int test_self_signed(const char* filename, int use_trusted,
  ...
  if (cert != NULL) {
    if (use_trusted) ret = ret && TEST_true(sk_X509_push(trusted, cert));
...
static int do_test_purpose(int purpose, int expected) {
  ...
  if (!TEST_true(sk_X509_push(trusted, trcert))) goto err;
  trcert = NULL;
  if (!TEST_true(sk_X509_push(untrusted, untrcert))) goto err;
  untrcert = NULL;

  if (!TEST_true(X509_STORE_CTX_init(ctx, NULL, eecert, untrusted))) goto err;

  if (!TEST_true(X509_STORE_CTX_set_purpose(ctx, purpose))) goto err;
  ...
  if (!TEST_int_eq(X509_verify_cert(ctx), expected)) goto err;
