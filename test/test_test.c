...
static int
test_case (int expected, const char *test, int result)
{
  if (result != expected)
    {
...
test_pointer (void)
{
  ...
  if (!TEST (1, TEST_ptr (&y)) || !TEST (0, TEST_ptr (NULL))
      ...
      || !TEST (0, TEST_ptr_eq (NULL, &y)) || !TEST (0, TEST_ptr_eq (&y, NULL))
      || !TEST (0, TEST_ptr_eq (&y, &x)) || !TEST (1, TEST_ptr_eq (&x, &x))
...
test_bignum (void)
{
  ...
  if (!TEST (1, TEST_int_eq (BN_dec2bn (&a, "0"), 1))
      ...
      || !TEST (0, TEST_BN_eq_zero (b)) || !TEST (1, TEST_BN_ne_zero (b))
      || !TEST (0, TEST_BN_le_zero (b)) || !TEST (0, TEST_BN_lt_zero (b))
      || !TEST (1, TEST_BN_ge_zero (b)) || !TEST (1, TEST_BN_gt_zero (b))
      || !TEST (0, TEST_BN_even (b)) || !TEST (1, TEST_BN_odd (b))
