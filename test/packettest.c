...
static int
test_PACKET_get_sub_packet(void)
{
  PACKET pkt, subpkt;
  unsigned long i = 0;
...
static int
test_PACKET_contains_zero_byte(void)
{
  char buf1[10], buf2[10];
  PACKET pkt;
...
static int
test_PACKET_get_length_prefixed_1(void)
{
...
static int
test_PACKET_get_length_prefixed_2(void)
{
...
static int
test_PACKET_get_length_prefixed_3(void)
{
  unsigned char buf1[1024];
...
static int
test_PACKET_as_length_prefixed_1(void)
{
  unsigned char buf1[BUF_LEN];
  const size_t len = 16;
  unsigned int i;
...
static int
test_PACKET_as_length_prefixed_2(void)
{
...
static int
test_PACKET_get_quic_vlint(void)
{
...
static int
test_PACKET_get_quic_length_prefixed(void)
{
  struct quic_test_case
  {
    unsigned char buf[16];
  ...
  for (i = 0; i < OSSL_NELEM(cases); ++i) {
    ...
    if (!TEST_true(PACKET_buf_init(&pkt,
                                   cases[i].buf,
                                   cases[i].fail
                                     ? sizeof(cases[i].buf)
