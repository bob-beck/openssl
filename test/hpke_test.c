 ...
 */
static int
cmpkey(const EVP_PKEY* pkey, const unsigned char* pub, size_t publen)
{
...
static int
do_testhpke(const TEST_BASEDATA* base,
            const TEST_AEADDATA* aead,
            size_t aeadsz,
            const TEST_EXPORTDATA* export,
            size_t exportsz)
{
  ...
  if (base->mode == OSSL_HPKE_MODE_PSK ||
    ...
    if (!TEST_true(OSSL_HPKE_CTX_set1_psk(
          sealctx, base->pskid, base->psk, base->psklen)))
      goto end;
  }
  if (!TEST_true(OSSL_HPKE_encap(
        sealctx, enc, &enclen, rpub, rpublen, base->ksinfo, base->ksinfolen)))
    goto end;
  ...
  for (i = 0; i < aeadsz; ++i) {
    ctlen = sizeof(ct);
    memset(ct, 0, ctlen);
    if (!TEST_true(OSSL_HPKE_seal(sealctx,
                                  ct,
                                  &ctlen,
                                  aead[i].aad,
                                  aead[i].aadlen,
  ...
  if (!TEST_ptr(
        openctx = OSSL_HPKE_CTX_new(
          base->mode, base->suite, OSSL_HPKE_ROLE_RECEIVER, libctx, propq)))
    goto end;
  if (base->mode == OSSL_HPKE_MODE_PSK ||
      base->mode == OSSL_HPKE_MODE_PSKAUTH) {
    if (!TEST_true(base->pskid != NULL && base->psk != NULL &&
      ...
      goto end;
    if (!TEST_true(OSSL_HPKE_CTX_set1_psk(
          openctx, base->pskid, base->psk, base->psklen)))
      goto end;
  }
  if (base->mode == OSSL_HPKE_MODE_AUTH ||
      base->mode == OSSL_HPKE_MODE_PSKAUTH) {
  ...
  if (!TEST_true(OSSL_HPKE_decap(
        openctx, enc, enclen, privR, base->ksinfo, base->ksinfolen)))
    goto end;
  for (i = 0; i < aeadsz; ++i) {
    ptoutlen = sizeof(ptout);
    memset(ptout, 0, ptoutlen);
    if (!TEST_true(OSSL_HPKE_open(openctx,
                                  ptout,
                                  &ptoutlen,
                                  aead[i].aad,
                                  aead[i].aadlen,
  ...
  for (i = 0; i < exportsz; ++i) {
    ...
    if (!TEST_false(
      ...
      goto end;
    /* good export call */
    if (!TEST_true(OSSL_HPKE_export(
          sealctx, eval, len, export[i].context, export[i].contextlen)))
      goto end;
    if (!TEST_mem_eq(
          eval, len, export[i].expected_secret, export[i].expected_secretlen))
      goto end;
...
static int
x25519kdfsha256_hkdfsha256_aes128gcm_psk_test(void)
{
  ...
  const TEST_AEADDATA aeaddata[] = { { 0,
                                       pt,
                                       sizeof(pt),
                                       first_aad0,
                                       sizeof(first_aad0),
...
static int
x25519kdfsha256_hkdfsha256_aes128gcm_base_test(void)
{
...
static int
P256kdfsha256_hkdfsha256_aes128gcm_base_test(void)
{
  const TEST_BASEDATA basedata = {
    OSSL_HPKE_MODE_BASE,
...
static int hpke_mode_list[] = { OSSL_HPKE_MODE_BASE,
                                OSSL_HPKE_MODE_PSK,
                                OSSL_HPKE_MODE_AUTH,
                                OSSL_HPKE_MODE_PSKAUTH };
static uint16_t hpke_kem_list[] = { OSSL_HPKE_KEM_ID_P256,
      ...
      for (kdfind = 0; overallresult == 1 && kdfind < OSSL_NELEM(hpke_kdf_list);
        ...
        for (aeadind = 0;
          ...
          if (!TEST_true(OSSL_HPKE_keygen(
                hpke_suite, pub, &publen, &privp, NULL, 0, testctx, NULL)))
            overallresult = 0;
          if (!TEST_ptr(
          ...
          if (!TEST_true(OSSL_HPKE_encap(
                ctx, senderpub, &senderpublen, pub, publen, infop, infolen)))
            overallresult = 0;
          /* throw in a call with a too-short cipherlen */
          cipherlen = 15;
          if (!TEST_false(OSSL_HPKE_seal(
                ctx, cipher, &cipherlen, aadp, aadlen, plain, plainlen)))
            overallresult = 0;
          ...
          if (!TEST_false(OSSL_HPKE_open(
                rctx, clear, &clearlen, aadp, aadlen, cipher, cipherlen)))
            overallresult = 0;
...
test_hpke_export(void)
{
  ...
  if (!TEST_false(OSSL_HPKE_export(
        NULL, exp, sizeof(exp), (unsigned char*)estr, strlen(estr))))
    goto end;
  /* ctx before encap should fail too */
  if (!TEST_false(OSSL_HPKE_export(
        ctx, exp, sizeof(exp), (unsigned char*)estr, strlen(estr))))
    goto end;
  ...
  if (!TEST_true(OSSL_HPKE_export(
        ctx, exp2, sizeof(exp2), (unsigned char*)estr, strlen(estr))))
    goto end;
  ...
  if (!TEST_true(
    ...
    goto end;
  if (!TEST_true(OSSL_HPKE_export(
        rctx, rexp, sizeof(rexp), (unsigned char*)estr, strlen(estr))))
    goto end;
  /* check a 2nd call with same input gives same output */
  if (!TEST_true(OSSL_HPKE_export(
        rctx, rexp2, sizeof(rexp2), (unsigned char*)estr, strlen(estr))))
    goto end;
...
test_hpke_grease(void)
{
  ...
  g_pub_len = 10;
  if (TEST_false(OSSL_HPKE_get_grease_value(NULL,
                                            &g_suite,
                                            g_pub,
                                            &g_pub_len,
                                            g_cipher,
  ...
  if (TEST_true(OSSL_HPKE_get_grease_value(NULL,
                                           &g_suite,
                                           g_pub,
                                           &g_pub_len,
                                           g_cipher,
...
test_hpke_oddcalls(void)
{
  ...
  if (!TEST_false(OSSL_HPKE_get_ciphertext_size(bad_suite, 0)))
    goto end;
  if (!TEST_false(OSSL_HPKE_keygen(
        bad_suite, pub, &publen, &privp, NULL, 0, testctx, badpropq)))
    goto end;
  if (!TEST_false(OSSL_HPKE_keygen(
        bad_suite, pub, &publen, &privp, NULL, 0, testctx, NULL)))
    goto end;

  /* dodgy keygen calls */
  /* no pub */
  if (!TEST_false(OSSL_HPKE_keygen(
        hpke_suite, NULL, &publen, &privp, NULL, 0, testctx, NULL)))
    goto end;
  /* ikmlen but NULL ikm */
  if (!TEST_false(OSSL_HPKE_keygen(
        hpke_suite, pub, &publen, &privp, NULL, 80, testctx, NULL)))
    goto end;
  /* zero ikmlen but ikm */
  if (!TEST_false(OSSL_HPKE_keygen(
        hpke_suite, pub, &publen, &privp, fake_ikm, 0, testctx, NULL)))
    goto end;
  /* GIANT ikmlen */
  if (!TEST_false(OSSL_HPKE_keygen(
        hpke_suite, pub, &publen, &privp, fake_ikm, -1, testctx, NULL)))
    goto end;
  /* short publen */
  publen = 10;
  if (!TEST_false(OSSL_HPKE_keygen(
  ...
  if (!TEST_ptr(ctx = OSSL_HPKE_CTX_new(OSSL_HPKE_MODE_PSK,
                                        hpke_suite,
  ...
  if (!TEST_ptr_null(
        ctx = OSSL_HPKE_CTX_new(
          hpke_mode, bad_suite, OSSL_HPKE_ROLE_SENDER, testctx, NULL)))
    goto end;
  /* bad mode */
  if (!TEST_ptr_null(
        ctx = OSSL_HPKE_CTX_new(
          bad_mode, hpke_suite, OSSL_HPKE_ROLE_SENDER, testctx, NULL)))
    goto end;
  /* make good ctx */
  if (!TEST_ptr(ctx = OSSL_HPKE_CTX_new(
                  hpke_mode, hpke_suite, OSSL_HPKE_ROLE_SENDER, testctx, NULL)))
  ...
  if (!TEST_ptr(rctx = OSSL_HPKE_CTX_new(OSSL_HPKE_MODE_PSK,
                                         hpke_suite,
...
test_hpke_random_suites(void)
{
  ...
  if (!TEST_false(OSSL_HPKE_get_grease_value(
        NULL, NULL, NULL, NULL, NULL, 0, testctx, NULL)))
    return 0;
  enclen = 10;
  if (!TEST_false(OSSL_HPKE_get_grease_value(
  ...
  enclen = sizeof(enc); /* reset, 'cause get_grease() will have set */
  if (!TEST_true(OSSL_HPKE_get_grease_value(
        NULL, &suite2, enc, &enclen, ct, ctlen, testctx, NULL)))
    return 0;
 ...
 */
static int
test_hpke_one_ikm_gen(uint16_t kem_id,
                      ...
                      size_t publen)
{
  ...
  hpke_suite.kem_id = kem_id;
  if (!TEST_true(OSSL_HPKE_keygen(
        hpke_suite, lpub, &lpublen, &sk, ikm, ikmlen, testctx, NULL)))
    return 0;
  if (!TEST_ptr(sk))
    return 0;
  EVP_PKEY_free(sk);
...
test_hpke_ikms(void)
{
  ...
  res = test_hpke_one_ikm_gen(
    OSSL_HPKE_KEM_ID_P521, ikmp521, sizeof(ikmp521), pubp521, sizeof(pubp521));
  if (res != 1)
    return res;

  res = test_hpke_one_ikm_gen(
    OSSL_HPKE_KEM_ID_P256, ikmp256, sizeof(ikmp256), pubp256, sizeof(pubp256));
  if (res != 1)
    return res;

  res = test_hpke_one_ikm_gen(
    OSSL_HPKE_KEM_ID_P256, ikmiter, sizeof(ikmiter), pubiter, sizeof(pubiter));
  if (res != 1)
    return res;
...
test_hpke_compressed(void)
{
  ...
  if (!TEST_true(OSSL_HPKE_keygen(
        hpke_suite, authpub, &authpublen, &authpriv, NULL, 0, testctx, NULL)))
    goto end;
