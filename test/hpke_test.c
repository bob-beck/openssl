...
static int
do_testhpke (const TEST_BASEDATA *base, const TEST_AEADDATA *aead,
             size_t aeadsz, const TEST_EXPORTDATA *export, size_t exportsz)
{
  ...
  if (!TEST_ptr (sealctx
    ...
    goto end;
  if (!TEST_true (
          OSSL_HPKE_CTX_set1_ikme (sealctx, base->ikmE, base->ikmElen)))
    goto end;
...
static uint16_t hpke_kdf_list[]
    = { OSSL_HPKE_KDF_ID_HKDF_SHA256, OSSL_HPKE_KDF_ID_HKDF_SHA384,
        OSSL_HPKE_KDF_ID_HKDF_SHA512 };
static uint16_t hpke_aead_list[]
    = { OSSL_HPKE_AEAD_ID_AES_GCM_128, OSSL_HPKE_AEAD_ID_AES_GCM_256,
      ...
      for (kemind = 0; /* iterate over the kems, kdfs and aeads */
           overallresult == 1 && kemind < OSSL_NELEM (hpke_kem_list); kemind++)
        {
          ...
          if (hpke_mode == OSSL_HPKE_MODE_AUTH
              || hpke_mode == OSSL_HPKE_MODE_PSKAUTH)
            {
              if (TEST_true (OSSL_HPKE_keygen (hpke_suite, authpub,
                                               &authpublen, &authpriv, NULL, 0,
                                               testctx, NULL))
                  != 1)
                {
                  overallresult = 0;
...
test_hpke_grease (void)
{
  ...
  g_pub_len = 10;
  if (TEST_false (OSSL_HPKE_get_grease_value (NULL, &g_suite, g_pub,
                                              &g_pub_len, g_cipher,
                                              g_cipher_len, testctx, NULL))
      != 1)
    {
      overallresult = 0;
    }
...
test_hpke_oddcalls (void)
{
  ...
  /* dodgy keygen calls */
  /* no pub */
  if (!TEST_false (OSSL_HPKE_keygen (hpke_suite, NULL, &publen, &privp, NULL,
                                     0, testctx, NULL)))
    goto end;
  /* ikmlen but NULL ikm */
 ...
 */
static int
test_hpke_one_ikm_gen (uint16_t kem_id, const unsigned char *ikm,
                       size_t ikmlen, const unsigned char *pub, size_t publen)
{
  OSSL_HPKE_SUITE hpke_suite = OSSL_HPKE_SUITE_DEFAULT;
