...
static int do_testhpke(const TEST_BASEDATA* base, const TEST_AEADDATA* aead,
  ...
  if (base->mode == OSSL_HPKE_MODE_AUTH ||
    ...
    if (!TEST_true(base->ikmAuth != NULL && base->ikmAuthlen > 0)) goto end;
    if (!TEST_true(OSSL_HPKE_keygen(base->suite, authpub, &authpublen,
                                    &authpriv, base->ikmAuth, base->ikmAuthlen,
                                    libctx, propq)))
      goto end;
    if (!TEST_true(OSSL_HPKE_CTX_set1_authpriv(sealctx, authpriv))) goto end;
  }
  ...
  if (!TEST_true(OSSL_HPKE_encap(sealctx, enc, &enclen, rpub, rpublen,
    ...
    goto end;
  if (!TEST_true(cmpkey(privE, enc, enclen))) goto end;
  for (i = 0; i < aeadsz; ++i) {
    ctlen = sizeof(ct);
    memset(ct, 0, ctlen);
    ...
    if (!TEST_mem_eq(ct, ctlen, aead[i].expected_ct, aead[i].expected_ctlen))
      goto end;
    if (!TEST_true(OSSL_HPKE_CTX_get_seq(sealctx, &lastseq))) goto end;
    if (lastseq != (uint64_t)(i + 1)) goto end;
  }
  if (!TEST_ptr(openctx =
                    OSSL_HPKE_CTX_new(base->mode, base->suite,
  ...
  for (i = 0; i < aeadsz; ++i) {
    ...
    if (!TEST_true(OSSL_HPKE_open(openctx, ptout, &ptoutlen, aead[i].aad,
      ...
      goto end;
    if (!TEST_mem_eq(aead[i].pt, aead[i].ptlen, ptout, ptoutlen)) goto end;
    /* check the sequence is being incremented as expected */
    if (!TEST_true(OSSL_HPKE_CTX_get_seq(openctx, &lastseq))) goto end;
    if (lastseq != (uint64_t)(i + 1)) goto end;
  }
  /* check exporters */
    ...
    for (kemind = 0; /* iterate over the kems, kdfs and aeads */
      ...
      for (kdfind = 0; overallresult == 1 && kdfind < OSSL_NELEM(hpke_kdf_list);
        ...
        for (aeadind = 0;
          ...
          if (!TEST_ptr(rctx)) overallresult = 0;
          if (hpke_mode == OSSL_HPKE_MODE_PSK ||
              hpke_mode == OSSL_HPKE_MODE_PSKAUTH) {
          ...
          if (verbose || overallresult != 1) {
            ...
            TEST_note(
                ...
                kem_id, kdf_str_list[kdfind], kdf_id, aead_str_list[aeadind],
                aead_id);
          }
        }
      }
...
static int test_hpke_export(void) {
  ...
  if (!TEST_false(OSSL_HPKE_export(NULL, exp, sizeof(exp), (unsigned char*)estr,
                                   strlen(estr))))
    goto end;
  ...
  if (!TEST_true(OSSL_HPKE_export(ctx, exp2, sizeof(exp2), (unsigned char*)estr,
                                  strlen(estr))))
    goto end;
  if (!TEST_mem_eq(exp, sizeof(exp), exp2, sizeof(exp2))) goto end;
  if (!TEST_ptr(rctx =
                    OSSL_HPKE_CTX_new(hpke_mode, hpke_suite,
                                      OSSL_HPKE_ROLE_RECEIVER, testctx, NULL)))
    goto end;
  if (!TEST_true(OSSL_HPKE_decap(rctx, enc, enclen, privp, NULL, 0))) goto end;
  if (!TEST_true(
  ...
  if (!TEST_true(OSSL_HPKE_export(rctx, rexp2, sizeof(rexp2),
                                  (unsigned char*)estr, strlen(estr))))
    goto end;
  if (!TEST_mem_eq(rexp, sizeof(rexp), rexp2, sizeof(rexp2))) goto end;
  if (!TEST_mem_eq(exp, sizeof(exp), rexp, sizeof(rexp))) goto end;
  erv = 1;
end:
  OSSL_HPKE_CTX_free(ctx);
  OSSL_HPKE_CTX_free(rctx);
...
static int test_hpke_suite_strs(void) {
  ...
  /* check a few errors */
  if (!TEST_false(OSSL_HPKE_str2suite("", &stirred))) overallresult = 0;
  if (!TEST_false(OSSL_HPKE_str2suite(NULL, &stirred))) overallresult = 0;
  if (!TEST_false(OSSL_HPKE_str2suite("", NULL))) overallresult = 0;
  memset(giant, 'A', sizeof(giant) - 1);
  giant[sizeof(giant) - 1] = '\0';
  if (!TEST_false(OSSL_HPKE_str2suite(giant, &stirred))) overallresult = 0;
...
static int test_hpke_oddcalls(void) {
  ...
  if (!TEST_false(OSSL_HPKE_CTX_set_seq(NULL, 1))) goto end;
  if (!TEST_false(OSSL_HPKE_CTX_get_seq(NULL, &lseq))) goto end;
  if (!TEST_false(OSSL_HPKE_CTX_set1_authpub(NULL, pub, publen))) goto end;
  if (!TEST_false(OSSL_HPKE_CTX_set1_authpriv(NULL, privp))) goto end;
  if (!TEST_false(OSSL_HPKE_CTX_set1_ikme(NULL, NULL, 0))) goto end;
  if (!TEST_false(OSSL_HPKE_CTX_set1_psk(NULL, NULL, NULL, 0))) goto end;

  /* bad suite calls */
  hpke_suite.aead_id = 0xbad;
  if (!TEST_false(OSSL_HPKE_suite_check(hpke_suite))) goto end;
  hpke_suite.aead_id = OSSL_HPKE_AEAD_ID_AES_GCM_128;
  if (!TEST_false(OSSL_HPKE_suite_check(bad_suite))) goto end;
  if (!TEST_false(OSSL_HPKE_get_recommended_ikmelen(bad_suite))) goto end;
  if (!TEST_false(OSSL_HPKE_get_public_encap_size(bad_suite))) goto end;
  if (!TEST_false(OSSL_HPKE_get_ciphertext_size(bad_suite, 0))) goto end;
  ...
  if (!TEST_false(OSSL_HPKE_encap(NULL, NULL, NULL, NULL, 0, NULL, 0)))
    goto end;
  if (!TEST_false(OSSL_HPKE_decap(NULL, NULL, 0, NULL, NULL, 0))) goto end;
  ...
  if (!TEST_ptr(ctx = OSSL_HPKE_CTX_new(hpke_mode, hpke_suite,
    ...
    goto end;
  /* too long ikm */
  if (!TEST_false(OSSL_HPKE_CTX_set1_ikme(ctx, fake_ikm, -1))) goto end;
  /* zero length ikm */
  if (!TEST_false(OSSL_HPKE_CTX_set1_ikme(ctx, fake_ikm, 0))) goto end;
  /* NULL authpub */
  if (!TEST_false(OSSL_HPKE_CTX_set1_authpub(ctx, NULL, 0))) goto end;
  /* NULL auth priv */
  if (!TEST_false(OSSL_HPKE_CTX_set1_authpriv(ctx, NULL))) goto end;
  /* priv good, but mode is bad */
  if (!TEST_false(OSSL_HPKE_CTX_set1_authpriv(ctx, privp))) goto end;
  /* bad mode for psk */
  ...
  if (!TEST_ptr(rctx =
                    OSSL_HPKE_CTX_new(OSSL_HPKE_MODE_PSK, hpke_suite,
                                      OSSL_HPKE_ROLE_RECEIVER, testctx, NULL)))
    goto end;
  if (!TEST_false(OSSL_HPKE_decap(rctx, enc, enclen, privp, NULL, 0))) goto end;
  /* done with PSK mode */
  OSSL_HPKE_CTX_free(rctx);
  ...
  if (!TEST_true(OSSL_HPKE_CTX_set_seq(rctx, -1))) goto end;
  if (!TEST_false(
          OSSL_HPKE_open(rctx, clear, &clearlen, NULL, 0, cipher, cipherlen)))
    goto end;
  if (!TEST_true(OSSL_HPKE_CTX_set_seq(rctx, 0))) goto end;
  if (!TEST_true(
          OSSL_HPKE_open(rctx, clear, &clearlen, NULL, 0, cipher, cipherlen)))
    goto end;
  if (!TEST_mem_eq(plain, plainlen, clear, clearlen)) goto end;
  erv = 1;
end:
  OSSL_HPKE_CTX_free(ctx);
  OSSL_HPKE_CTX_free(rctx);
...
static int test_hpke_one_ikm_gen(uint16_t kem_id, const unsigned char* ikm,
  ...
  EVP_PKEY_free(sk);
  if (!TEST_mem_eq(pub, publen, lpub, lpublen)) return 0;
  return 1;
}
...
static int test_hpke_compressed(void) {
  ...
  if (!TEST_ptr(ctx = OSSL_HPKE_CTX_new(hpke_mode, hpke_suite,
                                        OSSL_HPKE_ROLE_SENDER, testctx, NULL)))
    goto end;
  if (!TEST_true(OSSL_HPKE_CTX_set1_authpriv(ctx, authpriv))) goto end;
  if (!TEST_true(OSSL_HPKE_encap(ctx, enc, &enclen, pub, publen, NULL, 0)))
    goto end;
  if (!TEST_true(
          OSSL_HPKE_seal(ctx, cipher, &cipherlen, NULL, 0, plain, plainlen)))
...
static int test_hpke_noncereuse(void) {
  ...
  if (!TEST_false(OSSL_HPKE_CTX_set_seq(ctx, seq))) goto end;
  if (!TEST_true(OSSL_HPKE_encap(ctx, enc, &enclen, pub, publen, NULL, 0)))
    goto end;
  /* set seq will also fail after some crypto done */
  if (!TEST_false(OSSL_HPKE_CTX_set_seq(ctx, seq + 1))) goto end;
  if (!TEST_true(
          OSSL_HPKE_seal(ctx, cipher, &cipherlen, NULL, 0, plain, plainlen)))
    goto end;
  ...
  if (!TEST_true(OSSL_HPKE_CTX_set_seq(rctx, seq))) goto end;
  if (!TEST_true(OSSL_HPKE_decap(rctx, enc, enclen, privp, NULL, 0))) goto end;
