...
static int test_fatalerr(void) {
  ...
  if (!TEST_false(create_ssl_connection(sssl, cssl, SSL_ERROR_NONE))) goto err;

  ERR_clear_error();

  /* Inject a plaintext record from client to server */
  if (!TEST_int_gt(BIO_write(wbio, dummyrec, sizeof(dummyrec)), 0)) goto err;
  ...
  if (!TEST_int_le(SSL_write(sssl, msg, (int)strlen(msg)), 0)) goto err;
