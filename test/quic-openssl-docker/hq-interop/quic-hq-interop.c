 ...
 */
static BIO*
create_socket_bio(const char* hostname, const char* port, BIO_ADDR** peer_addr)
{
  int sock = -1;
  BIO_ADDRINFO* res;
   ...
   */
  if (!BIO_lookup_ex(
        hostname, port, BIO_LOOKUP_CLIENT, AF_UNSPEC, SOCK_DGRAM, 0, &res))
    return NULL;
...
build_request_set(SSL* ssl)
{
  ...
  while (req_idx < total_requests) {
     ...
     */
    poll_list =
      OPENSSL_realloc_array(poll_list, poll_count, sizeof(SSL_POLL_ITEM));
    if (poll_list == NULL) {
      fprintf(stderr, "Unable to realloc poll_list\n");
      goto err;
    }

    outbiolist = OPENSSL_realloc_array(outbiolist, poll_count, sizeof(BIO*));
    if (outbiolist == NULL) {
...
main(int argc, char* argv[])
{
  ...
  while (!BIO_eof(req_bio)) {
    if (!BIO_read_ex(
          req_bio, &reqnames[read_offset], REQ_STRING_SZ, &bytes_read)) {
  ...
  while (req != NULL) {
    total_requests++;
    req_array = OPENSSL_realloc_array(req_array, total_requests, sizeof(char*));
    if (req_array == NULL)
      goto end;
  ...
  while (this_poll_count != 0 && poll_done < this_poll_count) {
    ...
    for (poll_idx = 0; poll_idx < this_poll_count; poll_idx++) {
      ...
      if (poll_list[poll_idx].revents == SSL_POLL_EVENT_R) {
        ...
        if (!SSL_read_ex(poll_list[poll_idx].desc.value.ssl,
                         buf,
                         sizeof(buf),
                         &readbytes)) {
