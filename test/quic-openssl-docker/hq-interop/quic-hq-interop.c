 ...
 *
 * The file includes functions for setting up SSL/TLS contexts, managing
 * session caches, and handling I/O failures. It supports both IPv4 and IPv6
 * connections and uses non-blocking mode for QUIC operations.
 *
 ...
 *         The returned BIO object will be associated with the connected
 * socket. If the BIO object is successfully created, it will take ownership of
 *         the socket and automatically close it when the BIO is freed.
 *
 * @note The function uses OpenSSL's socket-related functions (e.g.,
 * BIO_socket, BIO_connect) or portability and to integrate with OpenSSL's
 ...
 */
static BIO *
create_socket_bio (const char *hostname, const char *port,
                   BIO_ADDR **peer_addr)
{
  int sock = -1;
  BIO_ADDRINFO *res;
 ...
 */
static int
cache_new_session (struct ssl_st *ssl, SSL_SESSION *sess)
{
 ...
 */
static int
setup_session_cache (SSL *ssl, SSL_CTX *ctx, const char *filename)
{
  ...
  while (req_idx < total_requests)
    {
      ...
      outbiolist
          = OPENSSL_realloc_array (outbiolist, poll_count, sizeof (BIO *));
      if (outbiolist == NULL)
        {
 ...
 */
static int
setup_connection (char *hostname, char *port, SSL_CTX **ctx, SSL **ssl)
{
  unsigned char alpn[]
  ...
  while (req != NULL)
    {
      total_requests++;
      req_array
          = OPENSSL_realloc_array (req_array, total_requests, sizeof (char *));
      if (req_array == NULL)
        goto end;
