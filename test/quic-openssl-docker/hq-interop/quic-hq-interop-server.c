...
static void process_new_stream(SSL* stream) {
  ...
  for (;;) {
    nread = 0;
    ret = SSL_read_ex(stream, &buf[total_read], sizeof(buf) - total_read - 1,
                      &nread);
    total_read += nread;
    if (ret <= 0) {
...
static int run_quic_server(SSL_CTX* ctx, BIO* sock) {
   ...
   * child objects.
   */
  if ((listener = SSL_new_listener(ctx, flags)) == NULL) goto err;
...
int main(int argc, char* argv[]) {
  ...
  if (fileprefix == NULL) fileprefix = "./downloads";
