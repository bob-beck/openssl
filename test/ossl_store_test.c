...
static int get_params(const char* uri, const char* type) {
  ...
  if (pkey != NULL) ret = EVP_PKEY_is_a(pkey, type);
  EVP_PKEY_free(pkey);
...
static int test_store_get_params(int idx) {
  ...
  if (!TEST_true(get_params(uri, type))) return 0;
