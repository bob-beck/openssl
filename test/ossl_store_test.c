...
test_store_open(void)
{
  ...
  ret = TEST_ptr(input) &&
        ...
        TEST_ptr(sctx = OSSL_STORE_open_ex(
                   input, NULL, NULL, ui_method, NULL, NULL, NULL, NULL)) &&
...
test_store_open_winstore(void)
{
  ...
  ret = TEST_ptr(testcert = d2i_X509(NULL, &certptr, TEST_CERT_LEN)) &&
        ...
        TEST_ptr(ui_method = UI_create_method("DummyUI")) &&
        TEST_ptr(sctx = OSSL_STORE_open_ex("org.openssl.winstore:",
                                           NULL,
                                           NULL,
                                           ui_method,
                                           NULL,
...
static int
test_store_search_by_key_fingerprint_fail(void)
{
  int ret;
  OSSL_STORE_SEARCH* search = NULL;
...
static int
get_params(const char* uri, const char* type)
{
  EVP_PKEY* pkey = NULL;
 ...
 */
static int
test_store_attach_unregistered_scheme(void)
{
  int ret;
