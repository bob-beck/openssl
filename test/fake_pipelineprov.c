...
static void fake_pipeline_freectx(void* vctx) {
  ...
  for (i = 0; i < ctx->numpipes; i++) EVP_CIPHER_CTX_free(ctx->cipher_ctxs[i]);
...
static int fake_pipeline_init(void* vctx, const unsigned char* key,
  ...
  for (i = 0; i < numpipes; i++) {
    ...
    if (ctx->cipher_ctxs[i] == NULL) return 0;
...
int fake_pipeline_final(void* vctx, size_t numpipes, unsigned char** out,
  ...
  for (i = 0; i < numpipes; i++) {
    if (!EVP_CipherFinal(ctx->cipher_ctxs[i], out[i], &ioutl)) return 0;
    outl[i] = (size_t)ioutl;
  }
  return 1;
}
...
static const OSSL_PARAM fake_pipeline_aead_known_settable_ctx_params[] = {
    OSSL_PARAM_octet_ptr(OSSL_CIPHER_PARAM_PIPELINE_AEAD_TAG, NULL, 0),
    OSSL_PARAM_END};
const OSSL_PARAM* fake_pipeline_aead_settable_ctx_params(
    ossl_unused void* cctx, ossl_unused void* provctx) {
