...
static int run_srp(const char* username, const char* client_pass,
  ...
  if (!TEST_ptr(GN = SRP_get_default_gN("1024"))) return 0;
  ...
  if (!TEST_true(SRP_Verify_B_mod_N(Bpub, GN->N))) goto end;

  /* Client random */
  RAND_bytes(rand_tmp, sizeof(rand_tmp));
  a = BN_bin2bn(rand_tmp, sizeof(rand_tmp), NULL);
  if (!TEST_BN_ne_zero(a)) goto end;
  ...
  if (!TEST_true(SRP_Verify_A_mod_N(Apub, GN->N))) goto end;
  ...
  if (!TEST_BN_eq(Kclient, Kserver)) goto end;
...
static int check_bn(const char* name, const BIGNUM* bn, const char* hexbn) {
  ...
  if (!TEST_true(BN_hex2bn(&tmp, hexbn))) return 0;

  if (BN_cmp(bn, tmp) != 0) TEST_error("unexpected %s value", name);
  r = TEST_BN_eq(bn, tmp);
  BN_free(tmp);
  return r;
}
...
static int run_srp_kat(void) {
  ...
  if (!TEST_ptr(GN = SRP_get_default_gN("1024"))) goto err;
  ...
  /* Server's first message */
  Bpub = SRP_Calc_B(b, GN->N, GN->g, v);
  if (!TEST_true(SRP_Verify_B_mod_N(Bpub, GN->N))) goto err;
  ...
  /* Client's response */
  Apub = SRP_Calc_A(a, GN->N, GN->g);
  if (!TEST_true(SRP_Verify_A_mod_N(Apub, GN->N))) goto err;
...
static int run_srp_tests(void) {
  ...
  if (!TEST_false(run_srp("alice", "password1", "password2"))) return 0;

  /* "Positive" test, should pass */
  TEST_info("run_srp: expecting a match");
  if (!TEST_true(run_srp("alice", "password", "password"))) return 0;
