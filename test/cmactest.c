...
static int test_cmac_run(void) {
  ...
  if (!TEST_true(CMAC_Init(ctx, test[0].key, test[0].key_len, EVP_aes_128_cbc(),
                           NULL)) ||
      !TEST_true(CMAC_Update(ctx, test[0].data, test[0].data_len)) ||
  ...
  p = pt(buf, len);
  if (!TEST_str_eq(p, test[0].mac)) goto err;

  if (!TEST_true(CMAC_Init(ctx, test[1].key, test[1].key_len, EVP_aes_256_cbc(),
                           NULL)) ||
  ...
  p = pt(buf, len);
  if (!TEST_str_eq(p, test[1].mac)) goto err;
  ...
  if (!TEST_true(CMAC_Init(ctx, test[2].key, test[2].key_len, NULL, NULL)) ||
    ...
    goto err;
  p = pt(buf, len);
  if (!TEST_str_eq(p, test[2].mac)) goto err;
  /* Test reusing a key */
  ...
  if (!TEST_true(CMAC_Init(ctx, test[3].key, test[3].key_len, EVP_aes_128_cbc(),
                           NULL)) ||
      !TEST_true(CMAC_Update(ctx, test[3].data, test[3].data_len)) ||
      !TEST_true(CMAC_Final(ctx, buf, &len)))
    goto err;
  p = pt(buf, len);
  if (!TEST_str_eq(p, test[3].mac)) goto err;

  if (!TEST_true(CMAC_Init(ctx, test[4].key, test[4].key_len, EVP_aes_192_cbc(),
                           NULL)) ||
      !TEST_true(CMAC_Update(ctx, test[4].data, test[4].data_len)) ||
      !TEST_true(CMAC_Final(ctx, buf, &len)))
    goto err;
  p = pt(buf, len);
  if (!TEST_str_eq(p, test[4].mac)) goto err;

  if (!TEST_true(CMAC_Init(ctx, test[5].key, test[5].key_len, EVP_aes_256_cbc(),
                           NULL)) ||
      !TEST_true(CMAC_Update(ctx, test[5].data, test[5].data_len)) ||
      !TEST_true(CMAC_Final(ctx, buf, &len)))
    goto err;
  p = pt(buf, len);
  if (!TEST_str_eq(p, test[5].mac)) goto err;
  ...
  if (!TEST_true(CMAC_Init(ctx, test[6].key, test[6].key_len,
    ...
    goto err;
  p = pt(buf, len);
  if (!TEST_str_eq(p, test[6].mac)) goto err;
#endif
  ...
  if (!TEST_true(
    ...
    goto err;
  p = pt(buf, len);
  if (!TEST_str_eq(p, test[7].mac)) goto err;
#endif
...
static int test_cmac_copy(void) {
  ...
  ctx = CMAC_CTX_new();
  ctx2 = CMAC_CTX_new();
  if (!TEST_ptr(ctx) || !TEST_ptr(ctx2)) goto err;
