...
static int test_empty_nonoptional_content(void) {
   ...
   */
  if (TEST_int_le(i2d_RSAPrivateKey(rsa, NULL), 0)) ok = 1;
...
static int test_unicode_range(void) {
  const unsigned char univ_ok[] =
      "\0\0\0\0"
      "\0\0\xd7\xff"
      "\0\0\xe0\x00"
      "\0\x10\xff\xff";
  const unsigned char univ_bad[] =
      "\0\0\xd8\x00"
      "\0\0\xdf\xff"
      "\0\x11\x00\x00"
      "\x80\x00\x00\x00"
  ...
  if (!test_unicode(univ_ok, sizeof univ_ok - 1, V_ASN1_UTF8STRING)) ok = 0;
  if (!test_unicode(univ_bad, sizeof univ_bad - 1, -1)) ok = 0;
  return ok;
}
