...
static int test_dtls_unprocessed(int testidx) {
  ...
  if (!TEST_true(SSL_CTX_set_cipher_list(cctx, "AES128-SHA"))) goto end;
#else
  ...
  if (!TEST_ptr(c_to_s_fbio)) goto end;
  ...
  if (testidx == 1) certstatus[RECORD_SEQUENCE] = 0xff;
  ...
  mempacket_test_inject(c_to_s_mempacket, (char*)certstatus, sizeof(certstatus),
...
static int test_dtls_drop_records(int idx) {
  ...
  if (sess != NULL && !TEST_true(SSL_session_reused(clientssl))) goto end;
...
static int test_just_finished(void) {
  ...
  if (!TEST_ptr(serverssl) || !TEST_ptr(rbio) || !TEST_ptr(wbio)) goto end;
  ...
  if (!TEST_int_eq(BIO_write(sbio, buf, sizeof(buf)), sizeof(buf))) goto end;

  /* We expect the attempt to process the message to fail */
  if (!TEST_int_le(ret = SSL_accept(serverssl), 0)) goto end;

  /* Check that we got the error we were expecting */
  if (!TEST_int_eq(SSL_get_error(serverssl, ret), SSL_ERROR_SSL)) goto end;
...
static int test_swap_records(int idx) {
  ...
  if (!TEST_int_le(SSL_connect(cssl), 0)) goto end;

  /* Recv flight 1, send flight 2: ServerHello, Certificate, ServerHelloDone */
  if (!TEST_int_le(SSL_accept(sssl), 0)) goto end;
  ...
  if (idx == 0) {
    ...
    if (!TEST_ptr(bio) || !TEST_true(mempacket_swap_epoch(bio))) goto end;
  }

  /* Recv flight 3, send flight 4: datagram 0(NST, CCS) datagram 1(Finished) */
  if (!TEST_int_gt(SSL_accept(sssl), 0)) goto end;
  ...
  if (!TEST_ptr(bio)) goto end;
  if (idx == 1) {
    /* Finished comes before NST/CCS */
    if (!TEST_true(mempacket_move_packet(bio, 0, 1))) goto end;
  } else if (idx == 2) {
    /* App data comes before NST/CCS */
    if (!TEST_true(mempacket_move_packet(bio, 0, 2))) goto end;
  } else if (idx == 3) {
    /* App data comes before Finished */
    bio = SSL_get_wbio(sssl);
    if (!TEST_true(mempacket_move_packet(bio, 1, 2))) goto end;
  }
   ...
   */
  if (!TEST_int_gt(SSL_connect(cssl), 0)) goto end;
...
static int test_duplicate_app_data(void) {
  ...
  if (!TEST_true(mempacket_dup_last_packet(bio))) goto end;
  ...
  if (!TEST_mem_eq(buf, sizeof(msg), msg, sizeof(msg))) goto end;
