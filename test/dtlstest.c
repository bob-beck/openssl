...
static int
generate_cookie_cb(SSL* ssl, unsigned char* cookie, unsigned int* cookie_len)
{
...
static int
verify_cookie_cb(SSL* ssl, const unsigned char* cookie, unsigned int cookie_len)
{
...
static int
test_dtls_unprocessed(int testidx)
{
  ...
  if (!TEST_true(create_ssl_objects(
        sctx, cctx, &serverssl1, &clientssl1, NULL, c_to_s_fbio)))
    goto end;
  ...
  mempacket_test_inject(c_to_s_mempacket,
                        (char*)certstatus,
                        sizeof(certstatus),
                        1,
  ...
  if (!TEST_true(create_bare_ssl_connection(
        serverssl1, clientssl1, SSL_ERROR_NONE, 0, 0)))
    goto end;
...
static int
test_dtls_drop_records(int idx)
{
  SSL_CTX *sctx = NULL, *cctx = NULL;
  ...
  /* BIO is freed by create_ssl_connection on error */
  if (!TEST_true(create_ssl_objects(
        sctx, cctx, &serverssl, &clientssl, NULL, c_to_s_fbio)))
    goto end;
...
static int
test_dtls_duplicate_records(void)
{
  SSL_CTX *sctx = NULL, *cctx = NULL;
  SSL *serverssl = NULL, *clientssl = NULL;
  int testresult = 0;
...
test_just_finished(void)
{
  ...
  unsigned char buf[] = {
    ...
    0,
    0,
    SHA_DIGEST_LENGTH, /* message length */
    0,
    0, /* message sequence */
    0,
    0,
    ...
    0,
    0,
    SHA_DIGEST_LENGTH, /* fragment length */
  ...
  if (!TEST_true(create_ssl_ctx_pair(
        NULL, DTLS_server_method(), NULL, 0, 0, &sctx, NULL, cert, privkey)))
    return 0;
