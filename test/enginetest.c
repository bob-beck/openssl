...
static int
test_encrypt (EVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen,
...
static int
test_pkey_meths (ENGINE *e, EVP_PKEY_METHOD **pmeth, const int **pnids,
                 int nid)
{
...
test_x509_dup_w_engine (void)
{
  ...
  if (!TEST_ptr (pubkey = X509_get_X509_PUBKEY (cert))
      ...
      || !TEST_ptr_ne (X509_PUBKEY_get0 (duppubkey),
