...
static unsigned int clnt_psk_callback(SSL* ssl, const char* hint, char* ident,
  ...
  if (max_psk_len > 20) max_psk_len = 20;
...
static int mtu_test(SSL_CTX* ctx, const char* cs, int no_etm) {
  ...
  if (no_etm) SSL_set_options(srvr_ssl, SSL_OP_NO_ENCRYPT_THEN_MAC);
  ...
  if (debug) TEST_info("Channel established");
  ...
  for (s = mtus[0]; s <= mtus[29]; s++) {
    ...
    if (!TEST_int_eq(SSL_write(clnt_ssl, buf, (int)s), (int)s)) goto end;
    reclen = BIO_read(sc_bio, buf, sizeof(buf));
    if (debug) TEST_info("record %zu for payload %zu", reclen, s);
  ...
  if (!TEST_ptr(clnt_sc = SSL_CONNECTION_FROM_SSL_ONLY(clnt_ssl))) goto end;
  rv = 1;
  if (SSL_READ_ETM(clnt_sc)) rv = 2;
end:
  SSL_free(clnt_ssl);
  SSL_free(srvr_ssl);
  return rv;
...
static int run_mtu_tests(void) {
  ...
  if (!TEST_ptr(ctx = SSL_CTX_new(DTLS_method()))) goto end;
   ...
   * only do (non-DH) PSK.
   */
  if (!TEST_true(SSL_CTX_set_cipher_list(ctx, "PSK"))) goto end;
  ...
  for (i = 0; i < sk_SSL_CIPHER_num(ciphers); i++) {
    ...
    if (!HAS_PREFIX(cipher_name, "PSK-")) continue;

    if (!TEST_int_gt(ret = mtu_test(ctx, cipher_name, 0), 0)) break;
    TEST_info("%s OK", cipher_name);
    if (ret == 1) continue;

    /* mtu_test() returns 2 if it used Encrypt-then-MAC */
    if (!TEST_int_gt(ret = mtu_test(ctx, cipher_name, 1), 0)) break;
...
static int test_server_mtu_larger_than_max_fragment_length(void) {
  ...
  if (!TEST_true(SSL_CTX_set_dh_auto(ctx, 1))) goto end;
#endif
