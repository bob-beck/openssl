...
static unsigned int
clnt_psk_callback (SSL *ssl, const char *hint, char *ident,
                   unsigned int max_ident_len, unsigned char *psk,
...
static unsigned int
srvr_psk_callback (SSL *ssl, const char *identity, unsigned char *psk,
                   unsigned int max_psk_len)
{
  if (max_psk_len > 20)
    max_psk_len = 20;
...
static int
mtu_test (SSL_CTX *ctx, const char *cs, int no_etm)
{
...
static int
test_server_mtu_larger_than_max_fragment_length (void)
{
  SSL_CTX *ctx = NULL;
  SSL *srvr_ssl = NULL, *clnt_ssl = NULL;
  int rv = 0;
