...
static int
ml_dsa_create_keypair (EVP_PKEY **pkey, const char *name, const uint8_t *priv,
                       size_t priv_len, const uint8_t *pub, size_t pub_len,
                       int expected)
{
...
ml_dsa_key_dup_test (void)
{
  ...
  if (!TEST_ptr (pkey = do_gen_key (tst->name, tst->seed, tst->seed_len))
      ...
      || !TEST_ptr (ctx
                    = EVP_PKEY_CTX_new_from_pkey (lib_ctx, pkey_copy, NULL))
...
from_data_bad_input_test (void)
{
  ...
  if (!TEST_int_ne (EVP_PKEY_fromdata (
                        ctx, &pkey, OSSL_KEYMGMT_SELECT_PRIVATE_KEY, params),
                    1))
    goto err;
...
ml_dsa_digest_sign_verify_test (void)
{
  ...
  *p++ = OSSL_PARAM_construct_end ();
  ...
  if (!TEST_int_eq (
          EVP_DigestSign (mctx, NULL, &sig_len, sp->msg, sp->msg_len), 1)
      || !TEST_ptr (sig = OPENSSL_zalloc (sig_len)))
    goto err;
  sig_len--;
