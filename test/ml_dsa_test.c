...
static EVP_PKEY*
do_gen_key(const char* alg, const uint8_t* seed, size_t seed_len)
{
  EVP_PKEY* pkey = NULL;
  ...
  *p++ = OSSL_PARAM_construct_utf8_string(
    OSSL_PKEY_PARAM_PROPERTIES, "?fips=yes", 0);
...
static int
ml_dsa_create_keypair(EVP_PKEY** pkey,
                      const char* name,
                      const uint8_t* priv,
                      size_t priv_len,
                      const uint8_t* pub,
...
ml_dsa_siggen_test(int tst_id)
{
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!TEST_true(ml_dsa_create_keypair(
        &pkey, td->alg, td->priv, td->priv_len, NULL, 0, 1)) ||
      ...
      !TEST_int_eq(
        EVP_Q_digest(
          lib_ctx, "SHA256", NULL, psig, psig_len, digest, &digest_len),
        1) ||
...
ml_dsa_sigver_test(int tst_id)
{
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!TEST_true(ml_dsa_create_keypair(
        &pkey, td->alg, NULL, 0, td->pub, td->pub_len, 1)))
    goto err;
...
static int
ml_dsa_siggen_upd_test(int tst_id)
{
  int ret = 0;
  const ML_DSA_SIG_GEN_TEST_DATA* td = &ml_dsa_siggen_testdata[tst_id];
  EVP_PKEY_CTX* sctx = NULL;
...
static int
ml_dsa_sigver_upd_test(int tst_id)
{
  int ret = 0;
  const ML_DSA_SIG_VER_TEST_DATA* td = &ml_dsa_sigver_testdata[tst_id];
  EVP_PKEY_CTX* vctx = NULL;
...
static int
from_data_invalid_public_test(void)
{
  int ret = 0;
  const ML_DSA_KEYGEN_TEST_DATA* tst = &ml_dsa_keygen_testdata[0];
  uint8_t* pub = NULL;
  ...
  if (!TEST_true(ml_dsa_create_keypair(&pkey,
                                       tst->name,
                                       tst->priv,
                                       tst->priv_len,
                                       tst->pub,
      ...
      !TEST_true(ml_dsa_create_keypair(&pkey,
                                       tst->name,
                                       tst->priv,
                                       tst->priv_len,
                                       tst->pub,
...
static int
do_ml_dsa_sign_verify(const char* alg, int tstid)
{
  int ret = 0;
...
static int
ml_dsa_44_sign_verify_test(int tstid)
{
  return do_ml_dsa_sign_verify("ML-DSA-44", tstid);
}
static int
ml_dsa_65_sign_verify_test(int tstid)
{
  return do_ml_dsa_sign_verify("ML-DSA-65", tstid);
}
static int
ml_dsa_87_sign_verify_test(int tstid)
{
  return do_ml_dsa_sign_verify("ML-DSA-87", tstid);
}

static int
ml_dsa_digest_sign_verify_test(void)
{
  int ret = 0;
  ...
  *p++ = OSSL_PARAM_construct_end();
  ...
  if (!TEST_ptr(mctx = EVP_MD_CTX_new()) ||
      !TEST_int_eq(EVP_DigestSignInit_ex(
                     mctx, NULL, "SHA256", lib_ctx, "?fips=true", key, params),
                   0) ||
      !TEST_int_eq(EVP_DigestSignInit_ex(
                     mctx, NULL, NULL, lib_ctx, "?fips=true", key, params),
                   1))
    goto err;
  if (sp->expected == 0) {
    ret = 1; /* return true as we expected to fail */
  ...
  if (!TEST_int_eq(EVP_DigestSign(mctx, sig, &sig_len, sp->msg, sp->msg_len),
    ...
    goto err;
  sig_len++;
  if (!TEST_int_eq(EVP_DigestSignInit_ex(
                     mctx, NULL, NULL, lib_ctx, "?fips=true", key, params),
                   1) ||
      !TEST_int_eq(EVP_DigestSign(mctx, sig, &sig_len, sp->msg, sp->msg_len),
                   1) ||
      !TEST_int_eq(EVP_DigestVerifyInit_ex(
                     mctx, NULL, "SHA256", lib_ctx, "?fips=true", key, params),
                   0) ||
      !TEST_int_eq(EVP_DigestVerifyInit_ex(
                     mctx, NULL, NULL, lib_ctx, "?fips=true", key, params),
...
static int
ml_dsa_priv_pub_bad_t0_test(void)
{
  int ret = 0;
  EVP_PKEY* key = NULL;
