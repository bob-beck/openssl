...
static void*
xor_prov_import_key(const OSSL_DISPATCH* fns,
                    void* provctx,
                    int selection,
...
static void
xor_prov_free_key(const OSSL_DISPATCH* fns, void* key)
{
  OSSL_FUNC_keymgmt_free_fn* kmgmt_free = xor_prov_get_keymgmt_free(fns);
...
static int
tls_prov_get_capabilities(void* provctx,
    ...
    for (i = 0; i < NUM_DUMMY_GROUPS; i++) {
      ...
      if (dummy_group_names[i] == NULL) {
        ...
        if (dummy_group_names[i] == NULL)
          return 0;
        BIO_snprintf(
          dummy_group_names[i], dummy_name_max_size, "%s%d", dummy_base, i);
      }
...
static PROV_XOR_CTX*
xor_newprovctx(OSSL_LIB_CTX* libctx)
{
...
static int
xor_init(void* vpxorctx, void* vkey, ossl_unused const OSSL_PARAM params[])
{
...
static int
xor_set_peer(void* vpxorctx, void* vpeerkey)
{
  PROV_XORKEMKEX_CTX* pxorctx = (PROV_XORKEMKEX_CTX*)vpxorctx;
...
static void
xor_freectx(void* pxorctx)
{
  OPENSSL_free(pxorctx);
}

static void*
xor_dupctx(void* vpxorctx)
{
...
static int
xor_has(const void* vkey, int selection)
{
...
static void*
xor_dup(const void* vfromkey, int selection)
{
...
xor_get_params(void* vkey, OSSL_PARAM params[])
{
  XORKEY* key = vkey;
  OSSL_PARAM* p;
...
static int
xor_set_params(void* vkey, const OSSL_PARAM params[])
{
  XORKEY* key = vkey;
  const OSSL_PARAM* p;
...
static void*
xor_load(const void* reference, size_t reference_sz)
{
  XORKEY* key = NULL;
...
static int
xor_recreate(const unsigned char* kd1, const unsigned char* kd2)
{
  int i;
...
static int
xor_match(const void* keydata1, const void* keydata2, int selection)
{
...
static void*
xor_gen_init(void* provctx, int selection, const OSSL_PARAM params[])
{
...
static int
xor_gen_set_params(void* genctx, const OSSL_PARAM params[])
{
...
static const OSSL_PARAM*
xor_gen_settable_params(ossl_unused void* genctx, ossl_unused void* provctx)
{
...
static void*
xor_gen(void* genctx, OSSL_CALLBACK* osslcb, void* cbarg)
{
  struct xor_gen_ctx* gctx = genctx;
...
static int
xor_import(void* vkey, int select, const OSSL_PARAM params[])
{
  XORKEY* key = vkey;
  ...
  if ((param_priv_key != NULL &&
       !OSSL_PARAM_get_octet_string(
         param_priv_key, &pprivkey, sizeof(privkey), &priv_len)) ||
      (param_pub_key != NULL &&
       !OSSL_PARAM_get_octet_string(
         param_pub_key, &ppubkey, sizeof(pubkey), &pub_len)))
    goto err;
...
static int
xor_export(void* vkey, int select, OSSL_CALLBACK* param_cb, void* cbarg)
{
  XORKEY* key = vkey;
...
static const OSSL_PARAM*
xor_import_types_ex(void* provctx, int select)
{
  if (provctx == NULL)
    return NULL;
...
static const OSSL_PARAM*
xor_export_types_ex(void* provctx, int select)
{
  if (provctx == NULL)
    return NULL;
...
static void*
xor_xorhmacsig_gen(void* genctx, OSSL_CALLBACK* osslcb, void* cbarg)
{
...
static void*
xor_xorhmacsha2sig_gen(void* genctx, OSSL_CALLBACK* osslcb, void* cbarg)
{
  XORKEY* k = xor_gen(genctx, osslcb, cbarg);
...
static XORKEY*
xor_key_op(const X509_ALGOR* palg,
...
static XORKEY*
xor_key_from_x509pubkey(const X509_PUBKEY* xpk,
                        OSSL_LIB_CTX* libctx,
                        const char* propq)
{
  const unsigned char* p;
...
static XORKEY*
xor_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO* p8inf,
                   OSSL_LIB_CTX* libctx,
                   const char* propq)
{
  XORKEY* xork = NULL;
...
typedef int
check_key_type_fn(const void* key, int nid);
typedef int
key_to_paramstring_fn(const void* key,
                      int nid,
                      int save,
                      void** str,
                      int* strtype);
typedef int
key_to_der_fn(BIO* out,
              const void* key,
              int key_nid,
              const char* pemname,
...
typedef int
write_bio_of_void_fn(BIO* bp, const void* x);

/* Free the blob allocated during key_to_paramstring_fn */
static void
free_asn1_data(int type, void* data)
{
  switch (type) {
    case V_ASN1_OBJECT:
              ...
              i2d_of_void* k2d)
{
  ...
  if ((p8info = PKCS8_PRIV_KEY_INFO_new()) == NULL ||
      ...
      !PKCS8_pkey_set0(
        p8info, OBJ_nid2obj(key_nid), 0, V_ASN1_UNDEF, NULL, der, derlen)) {
    ERR_raise(ERR_LIB_USER, ERR_R_MALLOC_FAILURE);
...
static X509_SIG*
p8info_to_encp8(PKCS8_PRIV_KEY_INFO* p8info, struct key2any_ctx_st* ctx)
{
  X509_SIG* p8 = NULL;
  ...
  p8 = PKCS8_encrypt_ex(
    -1, ctx->cipher, kstr, (int)klen, NULL, 0, 0, p8info, libctx, NULL);
                   ...
                   i2d_of_void k2d)
{
  ...
  if ((xpk = X509_PUBKEY_new()) == NULL || (derlen = k2d(key, &der)) <= 0 ||
      !X509_PUBKEY_set0_param(
        xpk, OBJ_nid2obj(key_nid), V_ASN1_UNDEF, NULL, der, derlen)) {
    ERR_raise(ERR_LIB_USER, ERR_R_MALLOC_FAILURE);
...
static int
key_to_epki_der_priv_bio(BIO* out,
                         const void* key,
                         int key_nid,
...
static int
key_to_epki_pem_priv_bio(BIO* out,
                         const void* key,
                         int key_nid,
                         ossl_unused const char* pemname,
                         key_to_paramstring_fn* p2s,
...
static int
key_to_pki_der_priv_bio(BIO* out,
                        const void* key,
                        int key_nid,
                        ossl_unused const char* pemname,
                        key_to_paramstring_fn* p2s,
...
static int
key_to_pki_pem_priv_bio(BIO* out,
                        const void* key,
                        int key_nid,
                        ossl_unused const char* pemname,
                        key_to_paramstring_fn* p2s,
...
static int
key_to_spki_der_pub_bio(BIO* out,
                        const void* key,
                        int key_nid,
                        ossl_unused const char* pemname,
                        key_to_paramstring_fn* p2s,
...
static int
key_to_spki_pem_pub_bio(BIO* out,
                        const void* key,
                        int key_nid,
                        ossl_unused const char* pemname,
                        key_to_paramstring_fn* p2s,
...
static int
prepare_xorx_params(const void* xorxkey,
                    int nid,
                    int save,
                    void** pstr,
                    int* pstrtype)
...
static int
xorx_spki_pub_to_der(const void* vecxkey, unsigned char** pder)
{
...
static int
xorx_pki_priv_to_der(const void* vecxkey, unsigned char** pder)
{
...
static int
key2any_set_ctx_params(void* vctx, const OSSL_PARAM params[])
{
  struct key2any_ctx_st* ctx = vctx;
  OSSL_LIB_CTX* libctx = PROV_XOR_LIBCTX_OF(ctx->provctx);
...
static int
key2any_check_selection(int selection, int selection_mask)
{
  /*
...
static int
key2any_encode(struct key2any_ctx_st* ctx,
               OSSL_CORE_BIO* cout,
               const void* key,
               const char* typestr,
               const char* pemname,
...
#define MAKE_ENCODER(impl, type, kind, output)                                 \
  ...
  {                                                                            \
    struct key2any_ctx_st* ctx = vctx;                                         \
                                                                               \
    return xor_prov_import_key(                                                \
      xor_##impl##_keymgmt_functions, ctx->provctx, selection, params);        \
  }                                                                            \
...
#define ENCODER_w_structure(_name, _sym, _output, _structure)                  \
  ...
  ENCODER_w_structure(XORSIGALG_HASH_NAME,
                      xorhmacsha2sig,
                      der,
...
typedef int
check_key_fn(void*, struct der2key_ctx_st* ctx);
typedef void
adjust_key_fn(void*, struct der2key_ctx_st* ctx);
typedef void
free_key_fn(void*);
typedef void*
d2i_PKCS8_fn(void**, const unsigned char**, long, struct der2key_ctx_st*);
                              ...
                              OSSL_LIB_CTX* libctx)
{
  ...
  if (xpub == NULL)
    return NULL;
  return (X509_PUBKEY*)ASN1_item_d2i_ex((ASN1_VALUE**)&xpub,
                                        pp,
                                        len,
...
static int
xor_read_der(PROV_XOR_CTX* provctx,
             OSSL_CORE_BIO* cin,
             unsigned char** data,
             long* len)
{
...
typedef void*
key_from_pkcs8_t(const PKCS8_PRIV_KEY_INFO* p8inf,
                 OSSL_LIB_CTX* libctx,
                 const char* propq);
static void*
xor_der2key_decode_p8(const unsigned char** input_der,
                      long input_der_len,
...
static XORKEY*
xor_d2i_PUBKEY(XORKEY** a, const unsigned char** pp, long length)
{
  XORKEY* key = NULL;
  X509_PUBKEY* xpk;
...
static int
der2key_check_selection(int selection, const struct keytype_desc_st* desc)
{
  /*
   * The selections are kinda sorta "levels", i.e. each selection given
   * here is assumed to include those following.
...
static int
der2key_export_object(void* vctx,
...
static void*
xorx_d2i_PKCS8(void** key,
               const unsigned char** der,
               long der_len,
               struct der2key_ctx_st* ctx)
{
  return xor_der2key_decode_p8(
    der, der_len, ctx, (key_from_pkcs8_t*)xor_key_from_pkcs8);
}

static void
xorx_key_adjust(void* key, struct der2key_ctx_st* ctx)
{
}
...
#define DECODER_w_structure(_name, _input, _structure, _output)                \
  ...
  DECODER_w_structure(XORSIGALG_HASH_NAME,
                      der,
                      SubjectPublicKeyInfo,
                      xorhmacsha2sig),
...
static int
xor_get_aid(unsigned char** oidbuf, const char* tls_name)
{
...
static void*
xor_sig_newctx(void* provctx, const char* propq)
{
...
static int
xor_sig_setup_md(PROV_XORSIG_CTX* ctx, const char* mdname, const char* mdprops)
{
  EVP_MD* md;
...
static int
xor_sig_signverify_init(void* vpxor_sigctx, void* vxorsig, int operation)
{
...
static int
xor_sig_sign_init(void* vpxor_sigctx, void* vxorsig, const OSSL_PARAM params[])
{
  return xor_sig_signverify_init(vpxor_sigctx, vxorsig, EVP_PKEY_OP_SIGN);
}

static int
xor_sig_verify_init(void* vpxor_sigctx,
                    void* vxorsig,
                    const OSSL_PARAM params[])
{
...
static int
xor_sig_sign(void* vpxor_sigctx,
             unsigned char* sig,
             size_t* siglen,
             size_t sigsize,
             ...
             size_t tbslen)
{
   ...
   */
  if (!EVP_Q_mac(pxor_sigctx->libctx,
                 "HMAC",
                 NULL,
                 "sha1",
                 NULL,
...
static int
xor_sig_verify(void* vpxor_sigctx,
...
static int
xor_sig_digest_signverify_init(void* vpxor_sigctx,
                               const char* mdname,
                               void* vxorsig,
                               int operation)
{
...
static int
xor_sig_digest_sign_init(void* vpxor_sigctx,
                         const char* mdname,
                         void* vxorsig,
                         const OSSL_PARAM params[])
{
  return xor_sig_digest_signverify_init(
    vpxor_sigctx, mdname, vxorsig, EVP_PKEY_OP_SIGN);
}

static int
xor_sig_digest_verify_init(void* vpxor_sigctx,
                           const char* mdname,
                           void* vxorsig,
                           const OSSL_PARAM params[])
{
  return xor_sig_digest_signverify_init(
    vpxor_sigctx, mdname, vxorsig, EVP_PKEY_OP_VERIFY);
}

int
xor_sig_digest_signverify_update(void* vpxor_sigctx,
...
int
xor_sig_digest_sign_final(void* vpxor_sigctx,
                          unsigned char* sig,
                          size_t* siglen,
                          size_t sigsize)
{
...
int
xor_sig_digest_verify_final(void* vpxor_sigctx,
                            const unsigned char* sig,
                            size_t siglen)
{
  PROV_XORSIG_CTX* pxor_sigctx = (PROV_XORSIG_CTX*)vpxor_sigctx;
...
static void
xor_sig_freectx(void* vpxor_sigctx)
{
...
static void*
xor_sig_dupctx(void* vpxor_sigctx)
{
...
static int
xor_sig_get_ctx_params(void* vpxor_sigctx, OSSL_PARAM* params)
{
  PROV_XORSIG_CTX* pxor_sigctx = (PROV_XORSIG_CTX*)vpxor_sigctx;
  OSSL_PARAM* p;
...
static int
xor_sig_set_ctx_params(void* vpxor_sigctx, const OSSL_PARAM params[])
{
  PROV_XORSIG_CTX* pxor_sigctx = (PROV_XORSIG_CTX*)vpxor_sigctx;
  const OSSL_PARAM* p;
...
static int
xor_sig_get_ctx_md_params(void* vpxor_sigctx, OSSL_PARAM* params)
{
  PROV_XORSIG_CTX* pxor_sigctx = (PROV_XORSIG_CTX*)vpxor_sigctx;
...
static int
xor_sig_set_ctx_md_params(void* vpxor_sigctx, const OSSL_PARAM params[])
{
  PROV_XORSIG_CTX* pxor_sigctx = (PROV_XORSIG_CTX*)vpxor_sigctx;
...
tls_prov_query(void* provctx, int operation_id, int* no_cache)
{
  *no_cache = 0;
  switch (operation_id) {
...
static void
tls_prov_teardown(void* provctx)
{
  int i;
...
static unsigned int
randomize_tls_alg_id(OSSL_LIB_CTX* libctx)
{
  /*
                  ...
                  void** provctx)
{
  ...
  *provctx = xor_prov_ctx;
  ...
  if (!c_obj_add_sigid(
        handle, XORSIGALG_HASH_OID, XORSIGALG_HASH, XORSIGALG_HASH_OID)) {
    ERR_raise(ERR_LIB_USER, XORPROV_R_OBJ_CREATE_ERR);
    goto err;
  }
