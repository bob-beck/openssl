...
static OSSL_FUNC_keymgmt_free_fn* xor_prov_get_keymgmt_free(
    const OSSL_DISPATCH* fns) {
  /* Pilfer the keymgmt dispatch table */
  for (; fns->function_id != 0; fns++)
...
static OSSL_FUNC_keymgmt_import_fn* xor_prov_get_keymgmt_import(
    const OSSL_DISPATCH* fns) {
  /* Pilfer the keymgmt dispatch table */
  for (; fns->function_id != 0; fns++)
...
static OSSL_FUNC_keymgmt_export_fn* xor_prov_get_keymgmt_export(
    const OSSL_DISPATCH* fns) {
  /* Pilfer the keymgmt dispatch table */
  for (; fns->function_id != 0; fns++)
...
static void xor_prov_free_key(const OSSL_DISPATCH* fns, void* key) {
  ...
  if (kmgmt_free != NULL) kmgmt_free(key);
}
  ...
  if (strcmp(capability, "TLS-GROUP") == 0) {
    ...
    for (i = 0; i < NUM_DUMMY_GROUPS; i++) {
      ...
      if (dummy_group_names[i] == NULL) {
        ...
        if (dummy_group_names[i] == NULL) return 0;
...
static int xor_init(void* vpxorctx, void* vkey,
  ...
  if (pxorctx == NULL || vkey == NULL) return 0;
  pxorctx->key = vkey;
  return 1;
}
...
static int xor_set_peer(void* vpxorctx, void* vpeerkey) {
  ...
  if (pxorctx == NULL || vpeerkey == NULL) return 0;
...
static int xor_derive(void* vpxorctx, unsigned char* secret, size_t* secretlen,
  ...
  if (pxorctx->key == NULL || pxorctx->peerkey == NULL) return 0;

  *secretlen = XOR_KEY_SIZE;
  if (secret == NULL) return 1;

  if (outlen < XOR_KEY_SIZE) return 0;
...
static int xor_encapsulate(void* vpxorctx, unsigned char* ct, size_t* ctlen,
  ...
  if (ct == NULL || ss == NULL) {
    ...
    if (ctlen == NULL && sslen == NULL) return 0;
    if (ctlen != NULL) *ctlen = XOR_KEY_SIZE;
    if (sslen != NULL) *sslen = XOR_KEY_SIZE;
    return 1;
  }
...
static int xor_decapsulate(void* vpxorctx, unsigned char* ss, size_t* sslen,
  ...
  if (ss == NULL) {
    /* Just return size */
    if (sslen == NULL) return 0;
    *sslen = XOR_KEY_SIZE;
    return 1;
  }

  if (ctlen != XOR_KEY_SIZE) return 0;
  peerkey = xor_newkey(pxorctx->provctx);
  if (peerkey == NULL) goto end;
...
static void xor_freekey(void* keydata) {
  ...
  if (CRYPTO_DOWN_REF(&key->references, &refcnt) <= 0) return;
...
static int xor_key_up_ref(XORKEY* key) {
  ...
  if (CRYPTO_UP_REF(&key->references, &refcnt) <= 0) return 0;
...
static int xor_recreate(const unsigned char* kd1, const unsigned char* kd2) {
  ...
  for (i = 0; i < XOR_KEY_SIZE; i++) {
    if ((kd1[i] & 0xff) != ((kd2[i] ^ private_constant[i]) & 0xff)) return 0;
  }
  return 1;
}
...
static int xor_match(const void* keydata1, const void* keydata2,
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
    if (key1->hasprivkey) {
      if (key2->hasprivkey)
        ok = ok &
             (CRYPTO_memcmp(key1->privkey, key2->privkey, XOR_KEY_SIZE) == 0);
      else
...
static void* xor_gen_init(void* provctx, int selection,
  ...
  if ((gctx = OPENSSL_zalloc(sizeof(*gctx))) == NULL) return NULL;
...
static int xor_import(void* vkey, int select, const OSSL_PARAM params[]) {
  ...
  if (key == NULL || (select & OSSL_KEYMGMT_SELECT_KEYPAIR) == 0) return 0;
...
static XORKEY* xor_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO* p8inf,
  ...
  if (!PKCS8_pkey_get0(NULL, &p, &plen, &palg, p8inf)) return 0;
...
static X509_SIG* p8info_to_encp8(PKCS8_PRIV_KEY_INFO* p8info,
  ...
  if (ctx->cipher == NULL || ctx->pwcb == NULL) return NULL;
...
static int key_to_epki_pem_priv_bio(BIO* out, const void* key, int key_nid,
  ...
  p8 = key_to_encp8(key, key_nid, str, strtype, k2d, ctx);
  if (p8 != NULL) ret = PEM_write_bio_PKCS8(out, p8);
...
static int key_to_spki_der_pub_bio(BIO* out, const void* key, int key_nid,
  ...
  if (xpk != NULL) ret = i2d_X509_PUBKEY_bio(out, xpk);
...
static const OSSL_PARAM* key2any_settable_ctx_params(
    ossl_unused void* provctx) {
...
static int key2any_set_ctx_params(void* vctx, const OSSL_PARAM params[]) {
  ...
  if (cipherp != NULL) {
    ...
    if (!OSSL_PARAM_get_utf8_string_ptr(cipherp, &ciphername)) return 0;
...
static int xor_der2key_decode(void* vctx, OSSL_CORE_BIO* cin, int selection,
   ...
   */
  if (selection == 0) selection = ctx->desc->selection_mask;
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
    ...
    if (ctx->desc->d2i_PKCS8 != NULL) {
      ...
      if (ctx->flag_fatal) goto end;
    } else if (ctx->desc->d2i_private_key != NULL) {
      key = ctx->desc->d2i_private_key(NULL, &derp, der_len);
    }
    if (key == NULL && ctx->selection != 0) goto next;
  }
...
static void* xor_sig_newctx(void* provctx, const char* propq) {
  ...
  if (pxor_sigctx == NULL) return NULL;
...
static int xor_sig_signverify_init(void* vpxor_sigctx, void* vxorsig,
  ...
  if (pxor_sigctx == NULL || vxorsig == NULL) return 0;
...
static int xor_sig_digest_signverify_init(void* vpxor_sigctx,
  ...
  if (rmdname == NULL) rmdname = "sha256";

  pxor_sigctx->flag_allow_md = 0;
  if (!xor_sig_signverify_init(vpxor_sigctx, vxorsig, operation)) return 0;

  if (!xor_sig_setup_md(pxor_sigctx, rmdname, NULL)) return 0;

  pxor_sigctx->mdctx = EVP_MD_CTX_new();
  if (pxor_sigctx->mdctx == NULL) goto error;

  if (!EVP_DigestInit_ex(pxor_sigctx->mdctx, pxor_sigctx->md, NULL)) goto error;
...
int xor_sig_digest_signverify_update(void* vpxor_sigctx,
  ...
  if (pxor_sigctx == NULL || pxor_sigctx->mdctx == NULL) return 0;
...
int xor_sig_digest_sign_final(void* vpxor_sigctx, unsigned char* sig,
  ...
  if (sig != NULL) {
    ...
    if (!EVP_DigestFinal_ex(pxor_sigctx->mdctx, digest, &dlen)) return 0;
...
static void* xor_sig_dupctx(void* vpxor_sigctx) {
  ...
  *dstctx = *srcctx;
  ...
  if ((srcctx->sig != NULL) && !xor_key_up_ref(srcctx->sig)) goto err;
  dstctx->sig = srcctx->sig;

  if (srcctx->md != NULL && !EVP_MD_up_ref(srcctx->md)) goto err;
  dstctx->md = srcctx->md;
...
static int xor_sig_get_ctx_params(void* vpxor_sigctx, OSSL_PARAM* params) {
  ...
  if (pxor_sigctx == NULL || params == NULL) return 0;
...
static int xor_sig_set_ctx_params(void* vpxor_sigctx,
  ...
  if (p != NULL && !pxor_sigctx->flag_allow_md) return 0;
  if (p != NULL) {
...
static int xor_sig_get_ctx_md_params(void* vpxor_sigctx, OSSL_PARAM* params) {
  ...
  if (pxor_sigctx->mdctx == NULL) return 0;
...
static const OSSL_PARAM* xor_sig_gettable_ctx_md_params(void* vpxor_sigctx) {
  ...
  if (pxor_sigctx->md == NULL) return 0;
...
retry:
  if (RAND_bytes_ex(libctx, (unsigned char*)&id, sizeof(id), 0) <= 0) return 0;
  /*
...
int tls_provider_init(const OSSL_CORE_HANDLE* handle, const OSSL_DISPATCH* in,
  ...
  if (libctx == NULL || xor_prov_ctx == NULL) goto err;
