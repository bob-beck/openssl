...
static void* xor_prov_import_key(const OSSL_DISPATCH* fns,
                                 void* provctx,
                                 int selection,
...
static int tls_prov_get_capabilities(void* provctx,
...
static int xor_init(void* vpxorctx,
                    void* vkey,
...
static int xor_derive(void* vpxorctx,
...
static int xor_match(const void* keydata1,
                     const void* keydata2,
                     int selection) {
...
static void* xor_gen_init(void* provctx,
                          int selection,
                          const OSSL_PARAM params[]) {
...
static int xor_export(void* vkey,
                      int select,
...
static void* xor_xorhmacsig_gen(void* genctx,
...
static void* xor_xorhmacsha2sig_gen(void* genctx,
                                    OSSL_CALLBACK* osslcb,
                                    void* cbarg) {
  XORKEY* k = xor_gen(genctx, osslcb, cbarg);
...
typedef int key_to_paramstring_fn(const void* key,
                                  int nid,
                                  int save,
                                  void** str,
                                  int* strtype);
...
static PKCS8_PRIV_KEY_INFO* key_to_p8info(const void* key,
                                          int key_nid,
                                          void* params,
                                          int params_type,
                                          i2d_of_void* k2d) {
...
static X509_SIG* key_to_encp8(const void* key,
                              int key_nid,
                              void* params,
                              int params_type,
                              i2d_of_void* k2d,
...
static X509_PUBKEY* xorx_key_to_pubkey(const void* key,
                                       int key_nid,
                                       void* params,
                                       int params_type,
                                       i2d_of_void k2d) {
...
static int prepare_xorx_params(const void* xorxkey,
                               int nid,
                               int save,
                               void** pstr,
                               int* pstrtype) {
...
#define ENCODER_w_structure(_name, _sym, _output, _structure) \
  {_name,                                                     \
   ...
   (xor_##_sym##_to_##_structure##_##_output##_encoder_functions)}
    ...
    ENCODER_w_structure(XORSIGALG_NAME,
                        xorhmacsig,
                        der,
...
typedef void* d2i_PKCS8_fn(void**,
                           const unsigned char**,
                           long,
...
static int xor_read_der(PROV_XOR_CTX* provctx,
                        OSSL_CORE_BIO* cin,
                        unsigned char** data,
                        long* len) {
  BUF_MEM* mem = NULL;
...
static int xor_der2key_decode(void* vctx,
                              OSSL_CORE_BIO* cin,
                              int selection,
...
static void* xorx_d2i_PKCS8(void** key,
...
static int xor_sig_signverify_init(void* vpxor_sigctx,
                                   void* vxorsig,
                                   int operation) {
...
static int xor_sig_digest_sign_init(void* vpxor_sigctx,
                                    const char* mdname,
                                    void* vxorsig,
                                    const OSSL_PARAM params[]) {
...
static const OSSL_ALGORITHM* tls_prov_query(void* provctx,
                                            int operation_id,
                                            int* no_cache) {
  *no_cache = 0;
  switch (operation_id) {
