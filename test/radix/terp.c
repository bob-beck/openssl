...
#define TERP_STK_PUSH(terp, v)                                        \
  do {                                                                \
    if (!TEST_true(TERP_stk_push((terp), &(v), sizeof(v)))) goto err; \
  } while (0)

#define TERP_STK_POP(terp, v)                                        \
  do {                                                               \
    memset(&(v), 0, sizeof(v)); /* quiet warnings */                 \
    if (!TEST_true(TERP_stk_pop((terp), &(v), sizeof(v)))) goto err; \
  } while (0)
...
static int GEN_SCRIPT_init(GEN_SCRIPT* gen_script, SCRIPT_INFO* script_info) {
  ...
  if (!TEST_true(GEN_CTX_init(&gctx, script_info))) return 0;

  script_info->gen_func(&gctx);
  opgen_END(&gctx);

  if (!TEST_false(gctx.error)) goto err;

  if (!TEST_true(GEN_CTX_finish(&gctx, gen_script))) goto err;
...
#define GET_OPERAND(srdr, v)                                           \
  do {                                                                 \
    ...
    if (!TEST_true(SRDR_get_operand(srdr, &(v), sizeof(v)))) goto err; \
  } while (0)
...
static int GEN_SCRIPT_print(GEN_SCRIPT* gen_script, BIO* bio,
  ...
  for (i = 0; !was_end; ++i) {
    ...
    if (!TEST_true(SRDR_print_one(srdr, bio, i, &was_end))) goto err;
  }
...
static int TERP_init(TERP* terp, const TERP_CONFIG* cfg,
                     const SCRIPT_INFO* script_info,
                     const GEN_SCRIPT* gen_script) {
  if (!TEST_true(cfg->now_cb != NULL)) return 0;
...
static void TERP_cleanup(TERP* terp) {
  if (terp->script_info == NULL) return;
...
static ossl_inline int TERP_stk_push(TERP* terp, const void* buf,
                                     size_t buf_len) {
  if (!TEST_true(TERP_stk_ensure_capacity(terp, buf_len))) return 0;
...
static ossl_inline int TERP_stk_pop(TERP* terp, void* buf, size_t buf_len) {
  if (!TEST_size_t_ge(terp->stk_end - terp->stk_cur, buf_len)) return 0;
...
static int TERP_execute(TERP* terp) {
  ...
  spin_again:
    if (ossl_time_compare(TERP_now(terp), terp->deadline_time) >= 0) {
      ...
      if (terp->log_execute) TERP_log_spin(terp, spin_count);
      goto err;
    }
    ...
    switch (opc) {
      ...
      case OPK_FUNC: {
        ...
        if (!TEST_true(v != NULL)) goto err;

        ret = v(&terp->fctx);

        if (terp->fctx.skip_rest) {
          if (!TEST_int_eq(ret, F_RET_SKIP_REST)) goto err;
          ...
          if (!TEST_int_eq(ret, F_RET_SPIN_AGAIN)) goto err;

          terp->fctx.spin_again = 0;
          TERP_SPIN_AGAIN();
        } else {
          if (!TEST_false(terp->fctx.spin_again)) goto err;
...
static int TERP_run(SCRIPT_INFO* script_info, TERP_CONFIG* cfg) {
  ...
  /* Execute the script. */
  if (!TEST_true(TERP_init(&terp, cfg, script_info, &gen_script))) goto err;

  have_terp = 1;

  SCRIPT_INFO_print(script_info, debug_bio, /*error=*/0, "executing script");

  if (!TERP_execute(&terp)) goto err;
