...
static int
GEN_CTX_init(GEN_CTX* ctx, SCRIPT_INFO* script_info)
{
...
TERP_stk_pop(TERP* terp, void* buf, size_t buf_len);
...
#define DEF_SCRIPT(name, desc)                                                 \
  ...
  static SCRIPT_INFO script_info_##name = {                                    \
    #name, desc, __FILE__, __LINE__, script_gen_##name                         \
  };                                                                           \
  static void script_gen_##name(GEN_CTX* ctx)
...
static void*
openc_alloc_space(GEN_CTX* ctx, size_t num_bytes);
...
static void
opgen_END(GEN_CTX* ctx)
{
  openc_opcode(ctx, OPK_END);
}

static ossl_unused void
opgen_PUSH_P(GEN_CTX* ctx, void* p)
{
...
static void
opgen_PUSH_PZ(GEN_CTX* ctx, void* p)
{
...
static void
opgen_PUSH_U64(GEN_CTX* ctx, uint64_t v)
{
...
opgen_PUSH_SIZE(GEN_CTX* ctx, size_t v)
{
...
ossl_unused static void
opgen_FUNC(GEN_CTX* ctx, helper_func_t f, const char* f_name)
{
...
ossl_unused static void
opgen_LABEL(GEN_CTX* ctx, const char* name)
{
...
static void
opgen_set_line(GEN_CTX* ctx, const char* file, int line)
{
  ctx->cur_file = file;
  ctx->cur_line = line;
}
...
static ossl_unused void
opgen_fail(GEN_CTX* ctx, const char* msg)
{
  if (!ctx->error) {
...
static int
GEN_CTX_finish(GEN_CTX* ctx, GEN_SCRIPT* script)
{
...
static void
GEN_SCRIPT_cleanup(GEN_SCRIPT* script)
{
...
static int
GEN_SCRIPT_init(GEN_SCRIPT* gen_script, SCRIPT_INFO* script_info)
{
  int ok = 0;
  GEN_CTX gctx;
...
static void
SRDR_init(SRDR* rdr, const uint8_t* buf, size_t buf_len)
{
  rdr->beg = rdr->cur = buf;
...
static ossl_inline int
SRDR_get_operand(SRDR* srdr, void* buf, size_t buf_len)
{
...
static void
print_opc(BIO* bio, size_t op_num, size_t offset, const char* name)
{
  if (op_num != SIZE_MAX)
...
static int
SRDR_print_one(SRDR* srdr, BIO* bio, size_t i, int* was_end)
{
  int ok = 0;
...
#define PRINT_OPC(name)                                                        \
  ...
  switch (opc) {
    ...
    case OPK_PUSH_U64: {
      ...
      PRINT_OPC(PUSH_U64);
      BIO_printf(
        bio, "%#20llx (%llu)", (unsigned long long)v, (unsigned long long)v);
    } break;
    case OPK_PUSH_SIZE: {
      size_t v;
...
static int
GEN_SCRIPT_print(GEN_SCRIPT* gen_script,
                 BIO* bio,
...
static void
SCRIPT_INFO_print(SCRIPT_INFO* script_info,
                  BIO* bio,
                  int error,
                  const char* msg)
{
  if (error)
    TEST_error(
      "%s: script '%s' (%s)", msg, script_info->name, script_info->desc);
  else
    TEST_info(
      "%s: script '%s' (%s)", msg, script_info->name, script_info->desc);
}
...
static int
TERP_stk_ensure_capacity(TERP* terp, size_t spare)
{
  uint8_t* p;
...
static ossl_inline int
TERP_stk_push(TERP* terp, const void* buf, size_t buf_len)
{
...
static void
TERP_print_stack(TERP* terp, BIO* bio, const char* header)
{
...
static void
TERP_log_spin(TERP* terp, size_t spin_count)
{
  if (spin_count > 0)
    BIO_printf(
      terp->cfg.debug_bio, "           \t\t(span %zu times)\n", spin_count);
}

static int
TERP_execute(TERP* terp)
{
  int ok = 0;
  uint64_t opc;
  size_t op_num = 0;
...
static int
TERP_run(SCRIPT_INFO* script_info, TERP_CONFIG* cfg)
{
  ...
  if (!TEST_true(GEN_SCRIPT_init(&gen_script, script_info))) {
    SCRIPT_INFO_print(
      script_info, debug_bio, /*error=*/1, "error while generating script");
    goto err;
  }
  ...
  if (!TEST_true(GEN_SCRIPT_print(&gen_script, debug_bio, script_info))) {
    SCRIPT_INFO_print(
      script_info, debug_bio, /*error=*/1, "error while printing script");
    goto err;
  }
