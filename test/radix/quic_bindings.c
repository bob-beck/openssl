...
static RADIX_OBJ* RADIX_OBJ_new(const char* name, SSL* ssl) {
  ...
  if (!TEST_ptr(name) || !TEST_ptr(ssl)) return NULL;

  if (!TEST_ptr(obj = OPENSSL_zalloc(sizeof(*obj)))) return NULL;
...
static int RADIX_PROCESS_init(RADIX_PROCESS* rp, size_t node_idx,
  ...
  if (!TEST_ptr(rp->gm = ossl_crypto_mutex_new())) goto err;
#endif

  if (!TEST_ptr(rp->objs = lh_RADIX_OBJ_new(RADIX_OBJ_hash, RADIX_OBJ_cmp)))
    goto err;

  if (!TEST_ptr(rp->threads = sk_RADIX_THREAD_new(NULL))) goto err;
...
static void report_ssl_state(BIO* bio, const char* pfx, int is_write, int state,
  ...
  if (ec != UINT64_MAX) BIO_printf(bio, ", %llu", (unsigned long long)ec);
  BIO_printf(bio, "\n");
}
...
static void report_ssl(SSL* ssl, BIO* bio, const char* pfx) {
  ...
  if (is_quic && !is_listener) {
    ...
    if (SSL_get_stream_read_error_code(ssl, &rec) != 1) rec = UINT64_MAX;

    if (SSL_get_stream_write_error_code(ssl, &wec) != 1) wec = UINT64_MAX;
...
static void RADIX_PROCESS_report_thread_results(RADIX_PROCESS* rp, BIO* bio) {
  ...
  for (i = 1; i < sk_RADIX_THREAD_num(rp->threads); ++i) {
    ...
    if (!rt_testresult) RADIX_THREAD_report_state(rt, bio);
  }
...
static int RADIX_PROCESS_join_all_threads(RADIX_PROCESS* rp, int* testresult) {
  ...
  for (i = 1; i < sk_RADIX_THREAD_num(rp->threads); ++i) {
    ...
    if (!TEST_true(RADIX_THREAD_join(rt))) ok = 0;

    if (!rt->testresult) composite_testresult = 0;
  }
...
static int RADIX_PROCESS_set_obj(RADIX_PROCESS* rp, const char* name,
  ...
  if (obj != NULL && !TEST_false(obj->registered)) return 0;

  existing = RADIX_PROCESS_get_obj(rp, name);
  if (existing != NULL && obj != existing) {
    if (!TEST_true(existing->registered)) return 0;
...
static int RADIX_PROCESS_set_ssl(RADIX_PROCESS* rp, const char* name,
  ...
  if (!TEST_ptr(obj = RADIX_OBJ_new(name, ssl))) return 0;
...
static RADIX_THREAD* RADIX_THREAD_new(RADIX_PROCESS* rp) {
  ...
  if (!TEST_ptr(rp) || !TEST_ptr(rt = OPENSSL_zalloc(sizeof(*rt)))) return 0;
...
static int RADIX_THREAD_join(RADIX_THREAD* rt) {
  ...
  if (rt->t != NULL) ossl_crypto_thread_native_join(rt->t, &rv);

  ossl_crypto_thread_native_clean(rt->t);
  rt->t = NULL;

  if (!TEST_true(rt->done)) return 0;
...
static void radix_thread_cleanup(void) {
  ...
  if (!TEST_ptr(rt)) return;

  if (!TEST_true(CRYPTO_THREAD_set_local(&radix_thread, NULL))) return;
}
...
static int bindings_process_init(size_t node_idx, size_t process_idx) {
  ...
  if (!TEST_ptr(rt = RADIX_THREAD_new(&radix_process))) return 0;
...
static void per_op_tick_obj(RADIX_OBJ* obj) {
  if (obj->active) SSL_handle_events(obj->ssl);
}
...
#define REQUIRE_SSL_N(idx, ssl)                                     \
  do {                                                              \
    ...
    if (!TEST_true(expect_slot_ssl(fctx, (idx), &(ssl)))) goto err; \
  } while (0)
...
static int RADIX_THREAD_worker_run(RADIX_THREAD* rt) {
  ...
  if (!TEST_true(bindings_adjust_terp_config(&cfg))) goto err;

  if (!TERP_run(rt->child_script_info, &cfg)) goto err;
...
static unsigned int RADIX_THREAD_worker_main(void* p) {
  ...
  if (!TEST_true(radix_thread_init(rt))) return 0;
...
DEF_FUNC(hf_spawn_thread) {
  ...
  F_POP(script_info);
  if (!TEST_ptr(script_info)) goto err;

#if !defined(OPENSSL_THREADS)
  TEST_skip("threading not supported, skipping");
  F_SKIP_REST();
#else
  if (!TEST_ptr(child_rt = RADIX_THREAD_new(&radix_process))) return 0;

  if (!TEST_ptr(child_rt->debug_bio = BIO_new(BIO_s_mem()))) goto err;
...
#endif
err:
  if (!ok) RADIX_THREAD_free(child_rt);
