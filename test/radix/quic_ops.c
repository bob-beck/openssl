...
static int ssl_ctx_configure(SSL_CTX* ctx, int is_server) {
  if (!TEST_true(ossl_quic_set_diag_title(ctx, "quic_radix_test"))) return 0;

  if (!is_server) return 1;

  if (RP()->keylog_out != NULL) SSL_CTX_set_keylog_callback(ctx, keylog_cb);
...
static int ssl_create_bound_socket(uint16_t listen_port, int* p_fd,
  ...
  if (!TEST_int_ge(fd, 0)) goto err;

  if (!TEST_true(BIO_socket_nbio(fd, 1))) goto err;

  if (!TEST_ptr(addr = BIO_ADDR_new())) goto err;

  if (!TEST_true(BIO_ADDR_rawmake(addr, AF_INET, &ina, sizeof(ina), 0)))
    goto err;

  if (!TEST_true(BIO_bind(fd, addr, 0))) goto err;

  info.addr = addr;
  if (!TEST_true(BIO_sock_info(fd, BIO_SOCK_INFO_ADDRESS, &info))) goto err;

  if (!TEST_int_gt(BIO_ADDR_rawport(addr), 0)) goto err;
...
err:
  if (!ok && fd >= 0)
    ...
    *p_fd = fd;
    if (p_result_port != NULL) *p_result_port = BIO_ADDR_rawport(addr);
  }
  BIO_ADDR_free(addr);
  return ok;
}
...
static int ssl_attach_bio_dgram(SSL* ssl, uint16_t local_port,
  ...
  if (!TEST_true(BIO_up_ref(bio))) return 0;
...
DEF_FUNC(hf_new_ssl) {
  ...
  if (!TEST_ptr(ctx = SSL_CTX_new(method))) goto err;
  ...
  if (!TEST_true(ssl_ctx_configure(ctx, is_server))) goto err;

  if (is_domain) {
    if (!TEST_ptr(ssl = SSL_new_domain(ctx, 0))) goto err;
    ...
    if (!TEST_ptr(ssl = SSL_new_listener(ctx, 0))) goto err;
  } else {
    if (!TEST_ptr(ssl = SSL_new(ctx))) goto err;
  }

  if (!is_domain && !TEST_true(ssl_attach_bio_dgram(ssl, 0, NULL))) goto err;
...
DEF_FUNC(hf_new_ssl_listener_from) {
  ...
  if (!TEST_ptr_null(RADIX_PROCESS_get_obj(RP(), listener_name))) goto err;

  if (!TEST_ptr(listener = SSL_new_listener_from(domain, flags))) goto err;

  if (!TEST_true(ssl_attach_bio_dgram(listener, 0, NULL))) goto err;
...
DEF_FUNC(hf_listen) {
  ...
  r = SSL_listen(ssl);
  if (!TEST_true(r)) goto err;

  if (SSL_get0_domain(ssl) == NULL) radix_activate_slot(0);
...
DEF_FUNC(hf_new_stream) {
  ...
  if (!TEST_ptr_null(RADIX_PROCESS_get_obj(RP(), stream_name))) goto err;

  if (do_accept) {
    stream = SSL_accept_stream(conn, flags);

    if (stream == NULL) F_SPIN_AGAIN();
  } else {
...
DEF_FUNC(hf_accept_conn) {
  ...
  if (!TEST_ptr_null(RADIX_PROCESS_get_obj(RP(), conn_name))) goto err;

  conn = SSL_accept_connection(listener, flags);
  if (conn == NULL) F_SPIN_AGAIN();
...
DEF_FUNC(hf_accept_stream_none) {
  ...
  if (!TEST_ptr(conn = RADIX_PROCESS_get_ssl(RP(), conn_name))) goto err;
...
DEF_FUNC(hf_stream_reset) {
  ...
  if (!TEST_true(SSL_stream_reset(ssl, &args, sizeof(args)))) goto err;
...
DEF_FUNC(hf_set_default_stream_mode) {
  ...
  if (!TEST_true(SSL_set_default_stream_mode(ssl, (uint32_t)mode))) goto err;
...
DEF_FUNC(hf_shutdown_wait) {
  ...
  if (!TEST_int_ge(ret, 0)) goto err;

  if (ret == 0) F_SPIN_AGAIN();
...
DEF_FUNC(hf_conclude) {
  ...
  if (!TEST_true(SSL_stream_conclude(ssl, 0))) goto err;
...
DEF_FUNC(hf_write_rand) {
  ...
  while (buf_len > 0) {
    ...
    if (buf == NULL) buf = OPENSSL_malloc(thislen);
    if (!TEST_ptr(buf)) goto err;
    if (!TEST_int_eq(RAND_bytes(buf, (int)thislen), 1)) goto err;
...
DEF_FUNC(hf_read_expect) {
  ...
  if (!TEST_true(check_consistent_want(ssl, r))) goto err;
...
DEF_FUNC(hf_read_fail) {
  ...
  if (do_wait && is_want(ssl, 0)) F_SPIN_AGAIN();
...
DEF_FUNC(hf_connect_wait) {
  ...
  /* if not started */
  if (RT()->scratch0 == 0) {
    if (!TEST_true(SSL_set_blocking_mode(ssl, 0))) return 0;
  ...
  ret = SSL_connect(ssl);
  radix_activate_slot(0);
  if (!TEST_true(check_consistent_want(ssl, ret))) goto err;

  if (ret != 1) {
    if (is_want(ssl, ret)) F_SPIN_AGAIN();

    if (!TEST_int_eq(ret, 1)) goto err;
  }
...
DEF_FUNC(hf_detach) {
  ...
  if (!TEST_ptr(stream = ossl_quic_detach_stream(conn))) goto err;
...
DEF_FUNC(hf_attach) {
  ...
  if (!TEST_ptr(stream = RADIX_PROCESS_get_ssl(RP(), stream_name))) goto err;

  if (!TEST_true(ossl_quic_attach_stream(conn, stream))) goto err;

  if (!TEST_true(RADIX_PROCESS_set_ssl(RP(), stream_name, NULL))) goto err;
...
DEF_FUNC(hf_expect_fin) {
  ...
  if (is_want(ssl, 0)) F_SPIN_AGAIN();

  if (!TEST_int_eq(SSL_get_error(ssl, 0), SSL_ERROR_ZERO_RETURN)) goto err;

  if (!TEST_int_eq(SSL_want(ssl), SSL_NOTHING)) goto err;
...
DEF_FUNC(hf_expect_conn_close_info) {
  ...
  if (!SSL_get_conn_close_info(ssl, &cc_info, sizeof(cc_info))) F_SPIN_AGAIN();
...
DEF_FUNC(hf_expect_stream_id) {
  ...
  if (!TEST_uint64_t_eq(actual, expected)) goto err;
...
DEF_FUNC(hf_select_ssl) {
  ...
  F_POP2(slot, name);
  if (!TEST_ptr(obj = RADIX_PROCESS_get_obj(RP(), name))) goto err;

  if (!TEST_uint64_t_lt(slot, NUM_SLOTS)) goto err;
...
DEF_FUNC(hf_set_peer_addr_from) {
  ...
  if (!TEST_ptr(dst_bio) || !TEST_ptr(src_bio)) goto err;

  if (!TEST_ptr(src_addr = BIO_ADDR_new())) goto err;
   ...
   */
  if (!TEST_true(BIO_dgram_set_peer(dst_bio, src_addr))) goto err;
