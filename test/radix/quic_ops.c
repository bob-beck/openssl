...
#define OP_SELECT_SSL(slot, name) (OP_PUSH_U64(slot), OP_PUSH_PZ(#name), OP_FUNC(hf_select_ssl))
...
#define OP_NEW_SSL_C(name) (OP_PUSH_PZ(#name), OP_PUSH_U64(0), OP_FUNC(hf_new_ssl))

#define OP_NEW_SSL_L(name) (OP_PUSH_PZ(#name), OP_PUSH_U64(1), OP_FUNC(hf_new_ssl))

#define OP_NEW_SSL_D(name) (OP_PUSH_PZ(#name), OP_PUSH_U64(3), OP_FUNC(hf_new_ssl))

#define OP_NEW_SSL_L_LISTEN(name) (OP_NEW_SSL_L(name), OP_LISTEN(name))

#define OP_NEW_SSL_L_FROM(domain_name, listener_name, flags)                                                           \
    (OP_SELECT_SSL(0, domain_name), OP_PUSH_PZ(#listener_name), OP_PUSH_U64(flags), OP_FUNC(hf_new_ssl_listener_from))

#define OP_NEW_SSL_L_FROM_LISTEN(domain_name, listener_name, flags)                                                    \
    (OP_NEW_SSL_L_FROM(domain_name, listener_name, flags), OP_LISTEN(listener_name))

#define OP_SET_PEER_ADDR_FROM(dst_name, src_name)                                                                      \
    (OP_SELECT_SSL(0, dst_name), OP_SELECT_SSL(1, src_name), OP_FUNC(hf_set_peer_addr_from))

#define OP_SIMPLE_PAIR_CONN() (OP_NEW_SSL_L_LISTEN(L), OP_NEW_SSL_C(C), OP_SET_PEER_ADDR_FROM(C, L), OP_CONNECT_WAIT(C))

#define OP_SIMPLE_PAIR_CONN_D()                                                                                        \
    (OP_NEW_SSL_D(Ds), OP_NEW_SSL_L_FROM_LISTEN(Ds, L, 0), OP_NEW_SSL_C(C), OP_SET_PEER_ADDR_FROM(C, L),               \
     OP_CONNECT_WAIT(C))

#define OP_SIMPLE_PAIR_CONN_ND() (OP_SIMPLE_PAIR_CONN(), OP_SET_DEFAULT_STREAM_MODE(C, SSL_DEFAULT_STREAM_MODE_NONE))

#define OP_NEW_STREAM(conn_name, stream_name, flags)                                                                   \
    (OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(#stream_name), OP_PUSH_U64(flags), OP_PUSH_U64(0), OP_FUNC(hf_new_stream))

#define OP_ACCEPT_STREAM_WAIT(conn_name, stream_name, flags)                                                           \
    (OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(#stream_name), OP_PUSH_U64(flags), OP_PUSH_U64(1), OP_FUNC(hf_new_stream))

#define OP_ACCEPT_STREAM_NONE(conn_name) (OP_SELECT_SSL(0, conn_name), OP_FUNC(hf_accept_stream_none))

#define OP_ACCEPT_CONN_WAIT(listener_name, conn_name, flags)                                                           \
    (OP_SELECT_SSL(0, listener_name), OP_PUSH_PZ(#conn_name), OP_PUSH_U64(flags), OP_FUNC(hf_accept_conn))
...
#define OP_ACCEPT_CONN_NONE(listener_name) (OP_SELECT_SSL(0, listener_name), OP_FUNC(hf_accept_conn_none))

#define OP_ACCEPT_CONN_WAIT1(listener_name, conn_name, flags)                                                          \
    (OP_ACCEPT_CONN_WAIT(listener_name, conn_name, flags), OP_ACCEPT_CONN_NONE(listener_name))

#define OP_ACCEPT_CONN_WAIT1_ND(listener_name, conn_name, flags)                                                       \
    (OP_ACCEPT_CONN_WAIT_ND(listener_name, conn_name, flags), OP_ACCEPT_CONN_NONE(listener_name))

#define OP_WRITE(name, buf, buf_len) (OP_SELECT_SSL(0, name), OP_PUSH_BUFP(buf, buf_len), OP_FUNC(hf_write))

#define OP_WRITE_RAND(name, buf_len) (OP_SELECT_SSL(0, name), OP_PUSH_SIZE(buf_len), OP_FUNC(hf_write_rand))

#define OP_WRITE_B(name, buf) OP_WRITE(name, (buf), sizeof(buf))

#define OP_WRITE_EX2(name, buf, buf_len, flags)                                                                        \
    (OP_SELECT_SSL(0, name), OP_PUSH_BUFP(buf, buf_len), OP_PUSH_U64(flags), OP_FUNC(hf_write_ex2))
...
#define OP_READ_EXPECT(name, buf, buf_len) (OP_SELECT_SSL(0, name), OP_PUSH_BUFP(buf, buf_len), OP_FUNC(hf_read_expect))

#define OP_READ_EXPECT_B(name, buf) OP_READ_EXPECT(name, (buf), sizeof(buf))

#define OP_READ_FAIL() (OP_SELECT_SSL(0, name), OP_PUSH_U64(0), OP_FUNC(hf_read_fail))
...
#define OP_SET_DEFAULT_STREAM_MODE(name, mode)                                                                         \
    (OP_SELECT_SSL(0, name), OP_PUSH_U64(mode), OP_FUNC(hf_set_default_stream_mode))

#define OP_SET_INCOMING_STREAM_POLICY(name, policy, error_code)                                                        \
    (OP_SELECT_SSL(0, name), OP_PUSH_U64(policy), OP_PUSH_U64(error_code), OP_FUNC(hf_set_incoming_stream_policy))

#define OP_STREAM_RESET(name, error_code)                                                                              \
    (OP_SELECT_SSL(0, name), OP_PUSH_U64(flags), OP_PUSH_U64(error_code), OP_FUNC(hf_stream_reset))

#define OP_SHUTDOWN_WAIT(name, flags, error_code, reason)                                                              \
    (OP_SELECT_SSL(0, name), OP_PUSH_U64(flags), OP_PUSH_U64(error_code), OP_PUSH_PZ(reason), OP_FUNC(hf_shutdown_wait))

#define OP_DETACH(conn_name, stream_name) (OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(#stream_name), OP_FUNC(hf_detach))

#define OP_ATTACH(conn_name, stream_name) (OP_SELECT_SSL(0, conn_name), OP_PUSH_PZ(stream_name), OP_FUNC(hf_attach))
...
#define OP_EXPECT_CONN_CLOSE_INFO(name, error_code, expect_app, expect_remote)                                         \
    (OP_SELECT_SSL(0, name), OP_PUSH_U64(expect_app), OP_PUSH_U64(expect_remote), OP_PUSH_U64(error_code),             \
     OP_FUNC(hf_expect_conn_close_info))

#define OP_WAIT_FOR_DATA(name) (OP_SELECT_SSL(0, name), OP_FUNC(hf_wait_for_data))

#define OP_EXPECT_ERR(lib, reason) (OP_PUSH_U64(lib), OP_PUSH_U64(reason), OP_FUNC(hf_expect_err))

#define OP_EXPECT_SSL_ERR(name, expected) (OP_SELECT_SSL(0, name), OP_PUSH_U64(expected), OP_FUNC(hf_expect_ssl_err))

#define OP_EXPECT_STREAM_ID(expected) (OP_PUSH_U64(expected), OP_FUNC(hf_expect_stream_id))
