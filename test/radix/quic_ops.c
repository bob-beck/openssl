...
static int
ssl_ctx_select_alpn (SSL *ssl, const unsigned char **out,
...
static int
ssl_ctx_configure (SSL_CTX *ctx, int is_server)
{
...
static int
ssl_create_bound_socket (uint16_t listen_port, int *p_fd,
...
static int
ssl_attach_bio_dgram (SSL *ssl, uint16_t local_port, uint16_t *actual_port)
{
  int s_fd = -1;
  BIO *bio;
 ...
 * connection acceptance via the user_ssl.  This is just a quic test to
 * validate that: 1) The new callback to inform the user of a new pending ssl
 * acceptance works properly 2) That the object returned from
 * SSL_accept_connection matches the one passed to various callbacks
 *
...
static int
new_pending_cb (SSL_CTX *ctx, SSL *new_ssl, void *arg)
{
...
static int
client_hello_cb (SSL *s, int *al, void *arg)
{
...
#define OP_EXPECT_CONN_CLOSE_INFO(name, error_code, expect_app,               \
                                  expect_remote)                              \
