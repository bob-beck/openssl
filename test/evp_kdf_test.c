...
static EVP_KDF_CTX *
get_kdfbyname_libctx (OSSL_LIB_CTX *libctx, const char *name)
{
...
static OSSL_PARAM *
construct_tls1_prf_params (const char *digest, const char *secret,
                           const char *seed)
{
...
static int
test_kdf_tls1_prf_invalid_digest (void)
{
  int ret;
  EVP_KDF_CTX *kctx = NULL;
  OSSL_PARAM *params;
...
static int
test_kdf_tls1_prf_zero_output_size (void)
{
  int ret;
  EVP_KDF_CTX *kctx = NULL;
  unsigned char out[16];
...
static int
test_kdf_tls1_prf_empty_secret (void)
{
  int ret;
  EVP_KDF_CTX *kctx = NULL;
  unsigned char out[16];
...
static int
test_kdf_tls1_prf_1byte_secret (void)
{
  int ret;
  EVP_KDF_CTX *kctx = NULL;
  unsigned char out[16];
...
static int
test_kdf_tls1_prf_empty_seed (void)
{
  int ret;
  EVP_KDF_CTX *kctx = NULL;
  unsigned char out[16];
...
static int
test_kdf_tls1_prf_1byte_seed (void)
{
  int ret;
  EVP_KDF_CTX *kctx = NULL;
  unsigned char out[16];
...
static OSSL_PARAM *
construct_hkdf_params (char *digest, char *key, size_t keylen, char *salt,
                       char *info)
{
...
static int
do_kdf_hkdf_gettables (int extract_only, int has_digest)
{
  int ret = 0;
  size_t sz = 0;
  OSSL_PARAM *params;
  ...
  params_get[1] = OSSL_PARAM_construct_octet_string (OSSL_KDF_PARAM_SALT, salt,
                                                     sizeof (salt));
  params_get[2] = OSSL_PARAM_construct_octet_string (OSSL_KDF_PARAM_INFO, info,
                                                     sizeof (info));
...
static int
test_kdf_hkdf_gettables (void)
{
  return do_kdf_hkdf_gettables (0, 1);
}

static int
test_kdf_hkdf_gettables_extractonly (void)
{
  return do_kdf_hkdf_gettables (1, 1);
}

static int
test_kdf_hkdf_gettables_no_digest (void)
{
  return do_kdf_hkdf_gettables (1, 0);
}

static int
test_kdf_hkdf_invalid_digest (void)
{
  int ret;
  EVP_KDF_CTX *kctx = NULL;
  OSSL_PARAM *params;
...
static int
kdf_hkdf_fixed_digest_change_digest (const char *kdf_name, char *bad_digest)
{
  int ret;
  EVP_KDF_CTX *kctx = NULL;
  ...
  /* In a fixed-digest KDF it is not allowed to set the same digest nor change
   * it */
  ret = TEST_ptr (params_get) && TEST_ptr (kctx = get_kdfbyname (kdf_name))
...
static int
test_kdf_hkdf_fixed_digest_change_digest (void)
{
...
static int
kdf_hkdf_fixed_digest_preserve_digest (const char *kdf_name,
...
static int
test_kdf_hkdf_fixed_digest_preserve_digest (void)
{
...
test_kdf_hkdf_reset (void)
{
  ...
  params_get[0] = OSSL_PARAM_construct_octet_string (OSSL_KDF_PARAM_SALT, salt,
                                                     sizeof (salt));
  params_get[1] = OSSL_PARAM_construct_octet_string (OSSL_KDF_PARAM_INFO, info,
                                                     sizeof (info));
...
static int
test_kdf_hkdf_derive_set_params_fail (void)
{
  int ret = 0, i = 0;
  EVP_KDF_CTX *kctx = NULL;
  OSSL_PARAM params[2];
...
static int
test_kdf_hkdf_set_invalid_mode (void)
{
...
static int
do_kdf_hkdf_set_invalid_param (const char *key, int type)
{
  int ret = 0;
  EVP_KDF_CTX *kctx = NULL;
  OSSL_PARAM params[2];
...
static int
test_kdf_hkdf_set_ctx_param_fail (void)
{
...
static int
test_kdf_hkdf_zero_output_size (void)
{
  int ret;
  EVP_KDF_CTX *kctx = NULL;
  unsigned char out[10];
...
static OSSL_PARAM *
construct_pbkdf1_params (char *pass, char *digest, char *salt,
                         unsigned int *iter)
{
  OSSL_PARAM *params = OPENSSL_malloc_array (5, sizeof (OSSL_PARAM));
  OSSL_PARAM *p = params;
...
static int
test_kdf_pbkdf1_key_too_long (void)
{
  int ret = 0;
  EVP_KDF_CTX *kctx = NULL;
...
static OSSL_PARAM *
construct_pbkdf2_params (char *pass, char *digest, char *salt,
...
static int
test_kdf_pbkdf2_small_output (void)
{
  int ret = 0;
  EVP_KDF_CTX *kctx = NULL;
  unsigned char out[25];
...
static int
test_kdf_pbkdf2_large_output (void)
{
  int ret = 0;
  EVP_KDF_CTX *kctx = NULL;
  unsigned char out[25];
...
static int
test_kdf_pbkdf2_small_salt (void)
{
  int ret = 0;
  EVP_KDF_CTX *kctx = NULL;
  unsigned int iterations = 4096;
...
static int
test_kdf_pbkdf2_small_iterations (void)
{
  int ret = 0;
  EVP_KDF_CTX *kctx = NULL;
...
static int
test_kdf_pbkdf2_small_salt_pkcs5 (void)
{
  int ret = 0;
  EVP_KDF_CTX *kctx = NULL;
  unsigned char out[25];
...
static int
test_kdf_pbkdf2_small_iterations_pkcs5 (void)
{
  int ret = 0;
  EVP_KDF_CTX *kctx = NULL;
  unsigned char out[25];
...
static int
test_kdf_pbkdf2_invalid_digest (void)
{
  int ret = 0;
  EVP_KDF_CTX *kctx = NULL;
  unsigned int iterations = 4096;
  ...
  for (i = 0; ret && i < 2; ++i)
    {
      ...
      if (i == 0)
        ret = ret
              && TEST_int_le (EVP_KDF_derive (kctx, out, sizeof (out), NULL),
                              0)
...
test_kdf_ss_hash (void)
{
  ...
  static unsigned char z[]
      = { 0x6d, 0xbd, 0xc2, 0x3f, 0x04, 0x54, 0x88, 0xe4, 0x06, 0x27,
          0x57, 0xb0, 0x6b, 0x9e, 0xba, 0xe1, 0x83, 0xfc, 0x5a, 0x59,
          0x46, 0xd8, 0x0d, 0xb9, 0x3f, 0xec, 0x6f, 0x62, 0xec, 0x07,
          0xe3, 0x72, 0x7f, 0x01, 0x26, 0xae, 0xd1, 0x2c, 0xe4, 0xb2,
          0x62, 0xf4, 0x7d, 0x48, 0xd5, 0x42, 0x87, 0xf8, 0x1d, 0x47,
          0x4c, 0x7c, 0x3b, 0x18, 0x50, 0xe9 };
...
test_kdf_x963 (void)
{
  ...
  static const unsigned char expected[sizeof (out)]
      = {  };
...
test_kdf_kbkdf_6803_128 (void)
{
  ...
  static unsigned char outputs[][16]
      = { { 0xD1, 0x55, 0x77, 0x5A, 0x20, 0x9D, 0x05, 0xF0, 0x2B, 0x38, 0xD4,
            0x2A, 0x38, 0x9E, 0x5A, 0x56 },
          { 0x64, 0xDF, 0x83, 0xF8, 0x5A, 0x53, 0x2F, 0x17, 0x57, 0x7D, 0x8C,
            0x37, 0x03, 0x57, 0x96, 0xAB },
          { 0x3E, 0x4F, 0xBD, 0xF3, 0x0F, 0xB8, 0x25, 0x9C, 0x42, 0x5C, 0xB6,
            0xC9, 0x6F, 0x1F, 0x46, 0x35 } };
  ...
  for (i = 0; i < 3; i++)
    {
      ...
      ret = TEST_ptr (kctx)
            && TEST_int_gt (
                EVP_KDF_derive (kctx, result, sizeof (result), params), 0)
...
static OSSL_PARAM *
construct_kbkdf_params (char *digest, char *mac, unsigned char *key,
...
static int
test_kdf_kbkdf_invalid_digest (void)
{
  int ret;
  EVP_KDF_CTX *kctx;
  OSSL_PARAM *params;
...
static int
test_kdf_kbkdf_invalid_mac (void)
{
  int ret;
  EVP_KDF_CTX *kctx;
  OSSL_PARAM *params;
...
static int
test_kdf_kbkdf_zero_output_size (void)
{
  int ret;
  EVP_KDF_CTX *kctx;
  OSSL_PARAM *params;
...
test_kdf_ss_hmac (void)
{
  ...
  static unsigned char salt[]
      = { 0x36, 0x38, 0x27, 0x1c, 0xcd, 0x68, 0xa2, 0x5d,
...
test_kdf_sshkdf (void)
{
  ...
  static unsigned char key[]
      = {  };
...
static int
test_kdfs_same (EVP_KDF *kdf1, EVP_KDF *kdf2)
{
...
static int
test_kdf_hmac_drbg_settables (void)
{
  int ret = 0, i = 0, j = 0;
  EVP_KDF_CTX *kctx = NULL;
  ...
  for (i = 0; settableparams[i].key != NULL; ++i)
    {
      ...
      if (OPENSSL_strcasecmp (settableparams[i].key, OSSL_KDF_PARAM_PROPERTIES)
              != 0
          && OPENSSL_strcasecmp (settableparams[i].key, OSSL_ALG_PARAM_ENGINE)
                 != 0)
        {
...
static int
test_kdf_hmac_drbg_gettables (void)
{
  int ret = 0, i, j = 0;
  EVP_KDF_CTX *kctx = NULL;
