...
static int kdf_hkdf_fixed_digest_change_digest(const char *kdf_name, char *bad_digest)
{
    ...
    params_get[0] = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_DIGEST, digestname, sizeof(digestname));
    params_get[1] = OSSL_PARAM_construct_end();
    ...
    ret = TEST_ptr(params_get) && TEST_ptr(kctx = get_kdfbyname(kdf_name)) &&
          ...
          TEST_false(EVP_KDF_CTX_set_params(kctx, params_get)) && TEST_false(EVP_KDF_CTX_set_params(kctx, params_set));
...
static int kdf_hkdf_fixed_digest_preserve_digest(const char *kdf_name, const char *expected_digest, char *bad_digest)
{
    int ret = 0;
    EVP_KDF_CTX *kctx = NULL;
    ...
    ret = TEST_ptr(kctx = get_kdfbyname(kdf_name)) && TEST_true(EVP_KDF_CTX_set_params(kctx, params_init));
    if (!ret)
        goto end;

    params_set[0] = OSSL_PARAM_construct_utf8_string(OSSL_ALG_PARAM_DIGEST, bad_digest, strlen(bad_digest));
    params_set[1] = OSSL_PARAM_construct_end();

    /* Reset context and ensure it's still fixed-digest */
    EVP_KDF_CTX_reset(kctx);
    if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1) || !TEST_size_t_gt(params_get[0].return_size, 0) ||
        !TEST_str_eq(digestname, expected_digest) || !TEST_size_t_eq(params_get[1].return_size, 0) ||
        !TEST_size_t_eq(params_get[2].return_size, 0) || !TEST_false(EVP_KDF_CTX_set_params(kctx, params_set)))
        goto end;
    ...
    if (!TEST_ptr(kctx_copy = EVP_KDF_CTX_dup(kctx)) ||
        !TEST_int_eq(EVP_KDF_CTX_get_params(kctx_copy, params_get), 1) || !TEST_str_eq(digestname, expected_digest) ||
...
static int test_kdf_hkdf_reset(void)
{
    ...
    if (!TEST_ptr(params_set) || !TEST_ptr(kctx = get_kdfbyname(OSSL_KDF_NAME_HKDF)) ||
    ...
    params_get[0] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, salt, sizeof(salt));
    params_get[1] = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_INFO, info, sizeof(info));
    params_get[2] = OSSL_PARAM_construct_end();
    /* Getting other unset params gives empty params */
    if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1) || !TEST_size_t_eq(params_get[0].return_size, 0) ||
