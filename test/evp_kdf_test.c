...
static int kdf_hkdf_fixed_digest_preserve_digest(const char *kdf_name, const char *expected_digest, char *bad_digest)
{
    ...
    if (!TEST_ptr(kctx_copy = EVP_KDF_CTX_dup(kctx)) || !TEST_int_eq(EVP_KDF_CTX_get_params(kctx_copy, params_get), 1)
        || !TEST_str_eq(digestname, expected_digest) || !TEST_false(EVP_KDF_CTX_set_params(kctx_copy, params_set)))
        goto end;
