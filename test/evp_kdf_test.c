...
static int do_kdf_hkdf_gettables(int extract_only, int has_digest) {
  ...
  if (has_digest) {
    ...
    if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 0)) goto err;
  }
  ...
  if (!TEST_int_eq(EVP_KDF_CTX_get_params(kctx, params_get), 1)) goto err;
  ret = 1;
err:
  EVP_KDF_CTX_free(kctx);
  OPENSSL_free(params);
...
static int test_kdf_hkdf_reset(void) {
  ...
  if (!TEST_int_le(EVP_KDF_CTX_get_params(kctx, params_get), 0)) goto end;
...
static int test_kdf_hkdf_derive_set_params_fail(void) {
  ...
  if (!TEST_ptr(kctx = get_kdfbyname(OSSL_KDF_NAME_HKDF))) goto end;
  /*
  ...
  if (!TEST_int_eq(EVP_KDF_derive(kctx, out, sizeof(out), params), 0)) goto end;
  ret = 1;
end:
  EVP_KDF_CTX_free(kctx);
  return ret;
...
static int test_kdf_hkdf_set_invalid_mode(void) {
  ...
  params[1] = OSSL_PARAM_construct_end();
  if (!TEST_int_eq(EVP_KDF_CTX_set_params(kctx, params), 0)) goto end;
...
static int test_kdf_pbkdf1(void) {
  ...
  if (!TEST_ptr(defprov = OSSL_PROVIDER_load(libctx, "default"))) goto err;
...
static int test_kdf_hmac_drbg_settables(void) {
  ...
  if (!TEST_int_le(EVP_KDF_derive(kctx, out, sizeof(out), NULL), 0)) goto err;
  ...
  for (i = 0; settableparams[i].key != NULL; ++i) {
    ...
    if (OPENSSL_strcasecmp(settableparams[i].key, OSSL_KDF_PARAM_PROPERTIES) !=
      ...
      if (!TEST_int_le(EVP_KDF_CTX_set_params(kctx, params), 0)) goto err;
    }
  }
  ...
  if (!TEST_int_eq(EVP_KDF_CTX_set_params(kctx, params), 1)) goto err;
  if (!TEST_int_eq(EVP_KDF_CTX_set_params(kctx, params), 1)) goto err;
  ...
  /* Test the derive */
  if (!TEST_int_eq(EVP_KDF_derive(kctx, out, sizeof(out), NULL), 1)) goto err;
...
static int test_kdf_hmac_drbg_gettables(void) {
  ...
  for (i = 0; gettableparams[i].key != NULL; ++i) {
    ...
    if (!TEST_int_le(EVP_KDF_CTX_get_params(kctx, params), 0)) goto err;
  }
...
#endif
  if (fips_provider_version_ge(NULL, 3, 1, 0)) ADD_TEST(test_kdf_kbkdf_kmac);
