...
test_kdf_tls1_prf(void)
{
  ...
  OSSL_PARAM* params;
  static const unsigned char expected[sizeof(out)] = { 0x8e, 0x4d, 0x93, 0x25,
                                                       0x30, 0xd7, 0x65, 0xa0,
                                                       0xaa, 0xe9, 0x74, 0xc3,
                                                       0x04, 0x73, 0x5e, 0xcc };
                      ...
                      char* info)
{
  ...
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_KEY, (unsigned char*)key, keylen);
  if (info != NULL)
    *p++ = OSSL_PARAM_construct_octet_string(
...
test_kdf_hkdf(void)
{
  ...
  OSSL_PARAM* params;
  static const unsigned char expected[sizeof(out)] = { 0x2a, 0xc4, 0x36, 0x9f,
                                                       0x52, 0x59, 0x96, 0xf8,
                                                       0xde, 0x13 };
...
do_kdf_hkdf_gettables(int extract_only, int has_digest)
{
  ...
  if (!TEST_ptr(params =
                  construct_hkdf_params(has_digest ? "sha256" : NULL,
                                        "secret",
                                        6,
                                        "salt",
...
{
  return kdf_hkdf_fixed_digest_preserve_digest(
           OSSL_KDF_NAME_HKDF_SHA256, "SHA2-256", "SHA2-512") &&
         kdf_hkdf_fixed_digest_preserve_digest(
           OSSL_KDF_NAME_HKDF_SHA384, "SHA2-384", "SHA2-512") &&
         kdf_hkdf_fixed_digest_preserve_digest(
           OSSL_KDF_NAME_HKDF_SHA512, "SHA2-512", "SHA2-256");
}
...
test_kdf_pbkdf1(void)
{
  ...
  const unsigned char expected[sizeof(out)] = { 0xfb, 0x83, 0x4d, 0x36, 0x6d,
                                                0xbc, 0x53, 0x87, 0x35, 0x1b,
                                                0x34, 0x75, 0x95, 0x88, 0x32,
                                                0x4f, 0x3e, 0x82, 0x81, 0x01,
                                                0x21, 0x93, 0x64, 0x00, 0xcc };
...
test_kdf_pbkdf2(void)
{
  ...
  int mode = 0;
  OSSL_PARAM* params;
  const unsigned char expected[sizeof(out)] = { 0x34, 0x8c, 0x89, 0xdb, 0xcb,
                                                0xd3, 0x2b, 0x2f, 0x32, 0xd8,
                                                0x14, 0xb8, 0x11, 0x6e, 0x84,
                                                0xcf, 0x2b, 0x17, 0x34, 0x7e,
                                                0xbc, 0x18, 0x00, 0x18, 0x1c };
...
test_kdf_pbkdf2_small_salt(void)
{
  ...
  params = construct_pbkdf2_params(
    "passwordPASSWORDpassword", "sha256", "saltSALT", &iterations, &mode);
...
test_kdf_scrypt(void)
{
  ...
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_PASSWORD, (char*)"password", 8);
  *p++ =
...
test_kdf_ss_hash(void)
{
  ...
  static unsigned char z[] = { 0x6d, 0xbd, 0xc2, 0x3f, 0x04, 0x54, 0x88, 0xe4,
  ...
  static const unsigned char expected[sizeof(out)] = { 0xa4, 0x62, 0xde, 0x16,
                                                       0xa8, 0x9d, 0xe8, 0x46,
                                                       0x6e, 0xf5, 0x46, 0x0b,
                                                       0x47, 0xb8 };
  ...
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_INFO, other, sizeof(other));
  *p = OSSL_PARAM_construct_end();
...
test_kdf_x963(void)
{
  ...
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, z, sizeof(z));
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_INFO, shared, sizeof(shared));
  *p = OSSL_PARAM_construct_end();
...
test_kdf_kbkdf_6803_128(void)
{
  ...
  static unsigned char outputs[][16] = { { 0xD1,
                                           0x55,
                                           0x77,
                                           0x5A,
                                           0x20,
...
test_kdf_kbkdf_kmac(void)
{
  ...
  static unsigned char input_key[] = { 0xDD, 0x81, 0xEF, 0xC8, 0x2C, 0xDD, 0xEC,
                                       0x51, 0xC4, 0x09, 0xBD, 0x8C, 0xCB, 0xAF,
                                       0x94, 0xF6, 0x5F, 0xFA, 0x7B, 0x92, 0xF1,
                                       0x11, 0xF9, 0x40, 0x2B, 0x0D, 0x6A, 0xE0,
                                       0x5E, 0x44, 0x92, 0x34, 0xF0, 0x3B, 0xBA,
                                       0xF5, 0x4F, 0xEF, 0x19, 0x45, 0xDA };
  ...
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_KEY, input_key, sizeof(input_key));
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_INFO, context, sizeof(context));
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_SALT, label, sizeof(label));
  *p = OSSL_PARAM_construct_end();
...
test_kdf_ss_hmac(void)
{
  ...
  static unsigned char salt[] = { 0x36, 0x38, 0x27, 0x1c, 0xcd, 0x68,
                                  ...
                                  0x39, 0xef, 0x3f, 0x89 };
  static const unsigned char expected[sizeof(out)] = { 0x44, 0xf6, 0x76, 0xe8,
                                                       0x5c, 0x1b, 0x1a, 0x8b,
                                                       0xbc, 0x3d, 0x31, 0x92,
                                                       0x18, 0x63, 0x1c, 0xa3 };

  *p++ = OSSL_PARAM_construct_utf8_string(
    OSSL_KDF_PARAM_MAC, (char*)OSSL_MAC_NAME_HMAC, 0);
  *p++ =
...
test_kdf_sshkdf(void)
{
  ...
  static unsigned char xcghash[] = { 0x0e, 0x68, 0x3f, 0xc8, 0xa9, 0xed, 0x7c,
                                     0x2f, 0xf0, 0x2d, 0xef, 0x23, 0xb2, 0x74,
                                     0x5e, 0xbc, 0x99, 0xb2, 0x67, 0xda, 0xa8,
                                     0x6a, 0x4a, 0xa7, 0x69, 0x72, 0x39, 0x08,
                                     0x82, 0x53, 0xf6, 0x42 };
  static unsigned char sessid[] = { 0x0e, 0x68, 0x3f, 0xc8, 0xa9, 0xed, 0x7c,
                                    0x2f, 0xf0, 0x2d, 0xef, 0x23, 0xb2, 0x74,
                                    0x5e, 0xbc, 0x99, 0xb2, 0x67, 0xda, 0xa8,
                                    0x6a, 0x4a, 0xa7, 0x69, 0x72, 0x39, 0x08,
                                    0x82, 0x53, 0xf6, 0x42 };
  ...
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_SSHKDF_XCGHASH, xcghash, sizeof(xcghash));
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_SSHKDF_SESSION_ID, sessid, sizeof(sessid));
  *p++ = OSSL_PARAM_construct_utf8_string(
    OSSL_KDF_PARAM_SSHKDF_TYPE, &kdftype, sizeof(kdftype));
  *p = OSSL_PARAM_construct_end();
...
test_kdf_krb5kdf(void)
{
  ...
  static const unsigned char expected[sizeof(out)] = { 0x34, 0x28, 0x0A, 0x38,
                                                       0x2B, 0xC9, 0x27, 0x69,
                                                       0xB2, 0xDA, 0x2F, 0x9E,
                                                       0xF0, 0x66, 0x85, 0x4B };

  *p++ = OSSL_PARAM_construct_utf8_string(
    OSSL_KDF_PARAM_CIPHER, (char*)"AES-128-CBC", 0);
  *p++ =
    OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_KEY, key, sizeof(key));
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_CONSTANT, constant, sizeof(constant));
  *p = OSSL_PARAM_construct_end();
...
test_kdf_hmac_drbg_settables(void)
{
  ...
  for (i = 0; settableparams[i].key != NULL; ++i) {
    ...
    if (OPENSSL_strcasecmp(settableparams[i].key, OSSL_KDF_PARAM_PROPERTIES) !=
          0 &&
        OPENSSL_strcasecmp(settableparams[i].key, OSSL_ALG_PARAM_ENGINE) != 0) {
  ...
  params[1] = OSSL_PARAM_construct_utf8_string(
    OSSL_ALG_PARAM_MAC, macname, sizeof(macname));
...
test_kbkdf_mac_change(void)
{
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
    OSSL_KDF_PARAM_MAC, OSSL_MAC_NAME_KMAC128, 0);
  params[1] = OSSL_PARAM_construct_end();
