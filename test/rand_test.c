...
static int test_rand(void) {
  ...
  *params = OSSL_PARAM_construct_octet_string(OSSL_RAND_PARAM_TEST_NONCE, nonce,
                                              sizeof(nonce));
  if (!TEST_true(EVP_RAND_CTX_set_params(privctx, params)) ||
...
static int test_rand_uniform(void) {
  ...
  if (!test_get_libctx(&ctx, NULL, NULL, NULL, NULL)) goto err;
...
static int fips_health_tests(void) {
  ...
  if (!TEST_true(fips_health_test_one(buf, i, i))) return 0;

  /* Verify RCT can fail */
  for (i = 0; i < 20; i++) buf[i] = 0xff & (i > 10 ? 200 : i);
  if (!TEST_false(fips_health_test_one(buf, i, i))) return 0;
...
static int test_rand_get0_primary(void) {
  ...
  if (!TEST_ptr(RAND_get0_primary(ctx))) goto err;
