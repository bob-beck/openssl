...
test_rand(void)
{
  ...
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_RAND_PARAM_TEST_ENTROPY, entropy1, sizeof(entropy1));
  *p = OSSL_PARAM_construct_end();
  ...
  if (!TEST_ptr(privctx = RAND_get0_private(NULL)) ||
      ...
      !TEST_int_gt(RAND_priv_bytes(outbuf, sizeof(outbuf)), 0) ||
      !TEST_mem_eq(
        outbuf, sizeof(outbuf), entropy1 + sizeof(outbuf), sizeof(outbuf)))
    return 0;
  ...
  *params = OSSL_PARAM_construct_octet_string(
    OSSL_RAND_PARAM_TEST_NONCE, nonce, sizeof(nonce));
  if (!TEST_true(EVP_RAND_CTX_set_params(privctx, params)) ||
...
static int
fips_health_test_one(const uint8_t* buf, size_t n, size_t gen)
{
  int res = 0;
  ...
  p[0] = OSSL_PARAM_construct_octet_string(
    OSSL_RAND_PARAM_TEST_ENTROPY, (void*)buf, n);
...
static int
r_random_bytes(ossl_unused void* vprov,
...
static int
r_init(const OSSL_CORE_HANDLE* handle,
...
static int
test_rand_random_provider(void)
{
