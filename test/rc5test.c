...
test_rc5_cbc(int n)
{
  ...
  if (i >= 8) {
    ...
    RC5_32_cbc_encrypt(
      &rc5_cbc_plain[n][0], buf, 8, &key, &ivb[0], RC5_ENCRYPT);

    if (!TEST_mem_eq(
          &rc5_cbc_cipher[n][0], sizeof(rc5_cbc_cipher[0]), buf, sizeof(buf)))
      testresult = 0;

    memcpy(ivb, &rc5_cbc_iv[n][0], 8);
    RC5_32_cbc_encrypt(buf, buf2, 8, &key, &ivb[0], RC5_DECRYPT);
    if (!TEST_mem_eq(
          &rc5_cbc_plain[n][0], sizeof(rc5_cbc_plain[0]), buf2, sizeof(buf2)))
      testresult = 0;
  }
