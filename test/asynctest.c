...
static int
test_ASYNC_callback_status(void)
{
  ASYNC_WAIT_CTX* waitctx = NULL;
  int set_arg = 100;
...
static int
test_ASYNC_get_current_job(void)
{
  ASYNC_JOB* job = NULL;
  int funcret;
  ASYNC_WAIT_CTX* waitctx = NULL;
...
static int
test_ASYNC_WAIT_CTX_get_all_fds(void)
{
  ASYNC_JOB* job = NULL;
  int funcret;
  ASYNC_WAIT_CTX* waitctx = NULL;
  ...
  if (!ASYNC_init_thread(1, 0) ||
      ...
      !ASYNC_WAIT_CTX_get_changed_fds(
        waitctx, NULL, &numfds, NULL, &numdelfds) ||
      numfds != 0 ||
      numdelfds != 0
      ...
      !ASYNC_WAIT_CTX_get_changed_fds(
        waitctx, &fd, &numfds, NULL, &numdelfds) ||
      fd != MAGIC_WAIT_FD
      ...
      !ASYNC_WAIT_CTX_get_changed_fds(
        waitctx, NULL, &numfds, &delfd, &numdelfds) ||
      delfd != MAGIC_WAIT_FD
...
static int
test_ASYNC_set_mem_functions(void)
{
  ...
  if ((alloc_fn != test_alloc_stack) || (free_fn != test_free_stack)) {
    fprintf(stderr,
            "test_ASYNC_set_mem_functions() - setting and retrieving custom "
            "allocators failed\n");
    return 0;
  }
  ...
  if (!custom_alloc_used || !custom_free_used) {
    fprintf(stderr,
            "test_ASYNC_set_mem_functions() - custom allocation functions not "
            "used\n");
