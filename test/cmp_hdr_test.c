...
static CMP_HDR_TEST_FIXTURE* set_up(const char* const test_case_name) {
  ...
  if (!TEST_ptr(fixture = OPENSSL_zalloc(sizeof(*fixture)))) return NULL;
  fixture->test_case_name = test_case_name;
  if (!TEST_ptr(fixture->cmp_ctx = OSSL_CMP_CTX_new(NULL, NULL))) goto err;
  if (!TEST_ptr(fixture->hdr = OSSL_CMP_PKIHEADER_new())) goto err;
  return fixture;
...
static int execute_HDR_set_get_pvno_test(CMP_HDR_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_int_eq(ossl_cmp_hdr_set_pvno(fixture->hdr, pvno), 1)) return 0;
...
#define X509_NAME_ADD(n, rd, s)                                                \
  X509_NAME_add_entry_by_txt((n), (rd), MBSTRING_ASC, (unsigned char*)(s), -1, \
                             -1, 0)
...
static int execute_HDR_set1_recipient_test(CMP_HDR_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_int_eq(fixture->hdr->recipient->type, GEN_DIRNAME)) goto err;
...
static int execute_HDR_update_messageTime_test(CMP_HDR_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_true(ossl_cmp_hdr_update_messageTime(fixture->hdr))) return 0;
  if (!TEST_true(ASN1_TIME_to_tm(fixture->hdr->messageTime, &hdrtm))) return 0;

  hdrtime = mktime(&hdrtm);

  if (!TEST_time_t_le(before, hdrtime)) return 0;
  now = time(NULL);
  tmptm = *gmtime(&now);
  after = mktime(&tmptm);
...
static int execute_HDR_set1_senderKID_test(CMP_HDR_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_ptr(senderKID)) goto err;
...
static int execute_HDR_push0_freeText_test(CMP_HDR_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_ptr(text)) return 0;

  if (!ASN1_STRING_set(text, "A free text", -1)) goto err;
...
static int execute_HDR_push1_freeText_test(CMP_HDR_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_int_eq(ASN1_STRING_cmp(text, pushed_text), 0)) goto err;
...
static int execute_HDR_generalInfo_push0_item_test(
    CMP_HDR_TEST_FIXTURE* fixture) {
  OSSL_CMP_ITAV* itav = OSSL_CMP_ITAV_new();

  if (!TEST_ptr(itav)) return 0;
...
static int execute_HDR_generalInfo_push1_items_test(
    CMP_HDR_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_int_eq(memcmp(oid, buf, sizeof(oid)), 0)) goto err;
...
static int execute_HDR_set_and_check_implicitConfirm_test(
    CMP_HDR_TEST_FIXTURE* fixture) {
...
static int execute_HDR_init_test(CMP_HDR_TEST_FIXTURE* fixture) {
  ...
  if (!TEST_int_eq(fixture->expected,
    ...
    return 0;
  if (fixture->expected == 0) return 1;
