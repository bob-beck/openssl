...
static int
test_x509_cmp_time_current(void)
{
...
static int
test_X509_cmp_timeframe_vpm(const X509_VERIFY_PARAM* vpm,
...
test_x509_time(int idx)
{
  ...
  if (!TEST_int_eq(result, x509_format_tests[idx].expected)) {
    TEST_info("test_x509_time(%d) failed: expected %d, got %d\n",
              idx,
              x509_format_tests[idx].expected,
              result);
    goto out;
  }
...
static int
test_x509_time_print_rfc_822(int idx)
{
  BIO* m;
  int ret = 0, rv;
  char* pp;
  ...
  rv = ASN1_TIME_print_ex(
    m, &x509_print_tests_rfc_822[idx].asn1, ASN1_DTFLGS_RFC822);
...
static int
test_x509_time_print_iso_8601(int idx)
{
  BIO* m;
  int ret = 0, rv;
  char* pp;
  ...
  rv = ASN1_TIME_print_ex(
    m, &x509_print_tests_iso_8601[idx].asn1, ASN1_DTFLGS_ISO8601);
