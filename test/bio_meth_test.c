...
static int test_bio_meth(void) {
  ...
  if (!TEST_int_eq(id, BIO_TYPE_START + 1)) goto err;
  ...
  for (i = id + 1; i <= BIO_TYPE_MASK; ++i) {
    if (!TEST_int_eq(BIO_get_new_index(), i)) goto err;
  }
  if (!TEST_int_eq(BIO_get_new_index(), -1)) goto err;
  ...
  if (!TEST_ptr_null(BIO_find_type(bio3, BIO_TYPE_NONE))) goto err;
  /* Check searching for BIO_TYPE_NONE + BIO_TYPE_FILTER works */
  if (!TEST_ptr_eq(BIO_find_type(bio3, BIO_TYPE_FILTER), bio3)) goto err;
  ret = 1;
err:
  BIO_free(membio);
  BIO_free(bio3);
