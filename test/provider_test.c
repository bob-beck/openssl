...
static int test_provider(OSSL_LIB_CTX** libctx, const char* name,
   ...
   * a child lib ctx.
   */
  if (!TEST_ptr(base = OSSL_PROVIDER_load(*libctx, "base"))) goto err;
  if (!TEST_ptr(prov = OSSL_PROVIDER_load(*libctx, name))) goto err;
  ...
  if (!TEST_int_gt(err, 0) || !TEST_int_eq(ERR_GET_REASON(err), 1)) goto err;
  ...
  if (!TEST_true(OSSL_PROVIDER_unload(base))) goto err;
  base = NULL;
  if (!TEST_true(OSSL_PROVIDER_unload(prov))) goto err;
  prov = NULL;
...
static int test_provider_ex(OSSL_LIB_CTX** libctx, const char* name) {
  ...
  if (!TEST_ptr(prov = OSSL_PROVIDER_load_ex(*libctx, name, params))) goto err;
