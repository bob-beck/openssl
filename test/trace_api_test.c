...
#undef SET_EXPECTED_CAT_NAME
    ...
    if (!TEST_str_eq(cat_name, expected_cat_name)) return 0;
    ret_cat_num = OSSL_trace_get_category_num(cat_name);
    if (cat_num < OSSL_TRACE_CATEGORY_NUM)
      if (!TEST_int_eq(cat_num, ret_cat_num)) return 0;
  }
...
static int put_trace_output(void) {
  ...
  OSSL_TRACE_BEGIN(HTTP) {
    ...
    res += TEST_int_eq(trace_string(0, 1, OSSL_MASKED),
                       (int)strlen(OSSL_MASKED) + 1); /* newline added */
...
static int test_trace_channel(void) {
  ...
  if (!TEST_true(OSSL_trace_enabled(OSSL_TRACE_CATEGORY_HTTP))) goto end;
  ...
  if (!TEST_strn2_eq(p_buf, len, expected, expected_len)) ret = 0;
  ret =
...
static size_t trace_cb(const char* buffer, size_t count, int category, int cmd,
                       void* data) {
  trace_cb_called = 1;
  if (!TEST_true(category == OSSL_TRACE_CATEGORY_TRACE)) trace_cb_failure = 1;
  return count;
}
...
static int test_trace_callback(void) {
  ...
  if (!TEST_false(trace_cb_failure) || !TEST_true(trace_cb_called)) goto end;
