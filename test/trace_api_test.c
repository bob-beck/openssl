...
test_trace_channel (void)
{
  ...
  if (!TEST_strn2_eq (p_buf, len, expected, expected_len))
    ret = 0;
  ret = TEST_int_eq (OSSL_trace_set_channel (OSSL_TRACE_CATEGORY_HTTP, NULL),
                     1)
        && ret;
...
static size_t
trace_cb (const char *buffer, size_t count, int category, int cmd, void *data)
{
  trace_cb_called = 1;
