...
static int test_readbuffer_file_bio(int tstid) {
  ...
  if (!TEST_int_eq(BIO_tell(in_bio), 0)) goto err;
  ...
  if (tstid != 0) {
    ...
    while (!BIO_eof(in_bio)) {
      ...
      if (len == 0) {
        if (!TEST_true(BIO_eof(in_bio))) goto err;
      } else {
        if (!TEST_int_gt(len, 0) || !TEST_int_le(len, (int)sizeof(buf) - 1))
          goto err;
        if (!TEST_true(buf[len] == 0)) goto err;
        if (len > 1 && !BIO_eof(in_bio) && len != ((int)sizeof(buf) - 1) &&
            !TEST_true(buf[len - 1] == '\n'))
          goto err;
      }
      if (tstid == 1 && --partial == 0) break;
    }
  }
  if (!TEST_int_eq(BIO_seek(in_bio, 0), 1)) goto err;

  len = 8; /* Do a small partial read to start with */
  while (!BIO_eof(in_bio)) {
    if (!TEST_int_eq(BIO_read_ex(in_bio, buf, len, &bytes), 1)) break;
    if (!TEST_mem_eq(buf, bytes, expected + count, bytes)) goto err;
    count += bytes;
    len = sizeof(buf); /* fill the buffer on subsequent reads */
  }
  if (!TEST_size_t_eq(count, readbytes)) goto err;
  ret = 1;
err:
  BIO_free(in);
  BIO_free_all(in_bio);
