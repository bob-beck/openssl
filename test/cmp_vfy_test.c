...
test_validate_with_sender (const X509_NAME *name, int expected)
{
  ...
  if (!TEST_ptr (fixture->msg = load_pkimsg (ir_protected_f, libctx))
      || !TEST_true (
          OSSL_CMP_CTX_set1_expected_sender (fixture->cmp_ctx, name))
...
execute_msg_check_test (CMP_VFY_TEST_FIXTURE *fixture)
{
  ...
  if (fixture->expected == 0) /* error expected already during above check */
    return 1;
  return TEST_int_eq (
             0, ASN1_OCTET_STRING_cmp (ossl_cmp_hdr_get0_senderNonce (hdr),
...
static int
allow_unprotected (const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg,
...
static void
setup_check_update (CMP_VFY_TEST_FIXTURE **fixture, int expected,
