...
static int test_ncid_frame(int fail) {
   ...
   */
  if (!TEST_true(qtest_fault_set_packet_plain_listener(fault, add_ncid_frame_cb,
                                                       NULL)))
    goto err;
  if (!fail &&
  ...
  if (!TEST_true(ncid_injected)) goto err;
  ...
  if (!TEST_int_eq(SSL_read(cssl, buf, sizeof(buf)), (int)msglen)) goto err;
  ...
  if (fail) {
    if (!TEST_size_t_eq(bytesread, 0)) goto err;
  } else {
    if (!TEST_mem_eq(msg, msglen, buf, bytesread)) goto err;
  }
