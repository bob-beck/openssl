...
static const char*
do_table2string(long s, const OCSP_TBLSTR* ts, size_t len)
{
  size_t i;
  for (i = 0; i < len; i++, ts++)
    if (ts->t == s)
...
OCSP_REQUEST_print(BIO* bp, OCSP_REQUEST* o, unsigned long flags)
{
  ...
  if (!X509V3_extensions_print(
        bp, "Request Extensions", inf->requestExtensions, flags, 4))
    goto err;
  if (sig) {
...
OCSP_RESPONSE_print(BIO* bp, OCSP_RESPONSE* o, unsigned long flags)
{
  ...
  for (i = 0; i < sk_OCSP_SINGLERESP_num(rd->responses); i++) {
    ...
    if (BIO_printf(
          bp, "    Cert Status: %s", OCSP_cert_status_str(cst->type)) <= 0)
      goto err;
    ...
    if (BIO_write(bp, "\n", 1) <= 0)
      goto err;
    if (!X509V3_extensions_print(
          bp, "Response Single Extensions", single->singleExtensions, flags, 8))
      goto err;
    if (BIO_write(bp, "\n", 1) <= 0)
      goto err;
  }
  if (!X509V3_extensions_print(
        bp, "Response Extensions", rd->responseExtensions, flags, 4))
    goto err;
