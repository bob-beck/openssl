...
int
OCSP_request_set1_name(OCSP_REQUEST* req, const X509_NAME* nm)
{
...
OCSP_request_add1_cert(OCSP_REQUEST* req, X509* cert)
{
  ...
  if (cert == NULL)
    return 1;
  return ossl_x509_add_cert_new(
    &req->optionalSignature->certs, cert, X509_ADD_FLAG_UP_REF);
}
 ...
 */
int
OCSP_request_sign(OCSP_REQUEST* req,
                  X509* signer,
                  EVP_PKEY* key,
                  const EVP_MD* dgst,
                  STACK_OF(X509)* certs,
                  unsigned long flags)
{
  ...
  if ((flags & OCSP_NOCERTS) == 0) {
    if (!OCSP_request_add1_cert(req, signer) ||
        !X509_add_certs(
          req->optionalSignature->certs, certs, X509_ADD_FLAG_UP_REF))
      goto err;
  }
...
int
OCSP_resp_get0_id(const OCSP_BASICRESP* bs,
...
int
OCSP_resp_get1_id(const OCSP_BASICRESP* bs,
 ...
 */
int
OCSP_single_get0_status(OCSP_SINGLERESP* single,
                        int* reason,
 ...
 */
int
OCSP_resp_find_status(OCSP_BASICRESP* bs,
                      OCSP_CERTID* id,
                      int* status,
                      int* reason,
                      ASN1_GENERALIZEDTIME** revtime,
