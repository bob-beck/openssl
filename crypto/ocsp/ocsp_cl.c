...
OCSP_ONEREQ* OCSP_request_add0_id(OCSP_REQUEST* req, OCSP_CERTID* cid) {
  ...
  if ((one = OCSP_ONEREQ_new()) == NULL) return NULL;
...
int OCSP_request_sign(OCSP_REQUEST* req, X509* signer, EVP_PKEY* key,
                      ...
                      unsigned long flags) {
  if (!OCSP_request_set1_name(req, X509_get_subject_name(signer))) goto err;

  if ((req->optionalSignature = OCSP_SIGNATURE_new()) == NULL) goto err;
  if (key != NULL) {
...
int OCSP_single_get0_status(OCSP_SINGLERESP* single, int* reason,
  ...
  if (thisupd != NULL) *thisupd = single->thisUpdate;
  if (nextupd != NULL) *nextupd = single->nextUpdate;
  return ret;
}
