...
int
OCSP_id_get0_info(ASN1_OCTET_STRING** piNameHash,
                  ASN1_OBJECT** pmd,
...
OCSP_SINGLERESP*
OCSP_basic_add1_status(OCSP_BASICRESP* rsp,
                       OCSP_CERTID* cid,
                       int status,
                       int reason,
                       ASN1_TIME* revtime,
 ...
 */
int
OCSP_basic_sign_ctx(OCSP_BASICRESP* brsp,
                    X509* signer,
                    EVP_MD_CTX* ctx,
                    STACK_OF(X509)* certs,
                    unsigned long flags)
...
int
OCSP_basic_sign(OCSP_BASICRESP* brsp,
                X509* signer,
                EVP_PKEY* key,
                const EVP_MD* dgst,
                STACK_OF(X509)* certs,
...
int
OCSP_RESPID_set_by_name(OCSP_RESPID* respid, X509* cert)
{
...
int
OCSP_RESPID_set_by_key_ex(OCSP_RESPID* respid,
                          X509* cert,
                          OSSL_LIB_CTX* libctx,
                          const char* propq)
{
...
int
OCSP_RESPID_set_by_key(OCSP_RESPID* respid, X509* cert)
{
  if (cert == NULL)
    return 0;
...
int
OCSP_RESPID_match_ex(OCSP_RESPID* respid,
                     X509* cert,
                     OSSL_LIB_CTX* libctx,
                     const char* propq)
{
  ...
  if (respid->type == V_OCSP_RESPID_KEY) {
    ...
    ret = (ASN1_STRING_length(respid->value.byKey) == SHA_DIGEST_LENGTH) &&
          (memcmp(ASN1_STRING_get0_data(respid->value.byKey),
                  md,
                  SHA_DIGEST_LENGTH) == 0);
...
int
OCSP_RESPID_match(OCSP_RESPID* respid, X509* cert)
{
  if (cert == NULL)
    return 0;
