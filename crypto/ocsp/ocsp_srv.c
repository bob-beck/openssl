...
OCSP_RESPONSE* OCSP_response_create(int status, OCSP_BASICRESP* bs) {
  ...
  if ((rsp = OCSP_RESPONSE_new()) == NULL) goto err;
  if (!(ASN1_ENUMERATED_set(rsp->responseStatus, status))) goto err;
  if (!bs) return rsp;
  if ((rsp->responseBytes = OCSP_RESPBYTES_new()) == NULL) goto err;
...
OCSP_SINGLERESP* OCSP_basic_add1_status(OCSP_BASICRESP* rsp, OCSP_CERTID* cid,
  ...
  if ((single = OCSP_SINGLERESP_new()) == NULL) goto err;

  if (!ASN1_TIME_to_generalizedtime(thisupd, &single->thisUpdate)) goto err;
  if (nextupd && !ASN1_TIME_to_generalizedtime(nextupd, &single->nextUpdate))
    goto err;

  OCSP_CERTID_free(single->certId);

  if ((single->certId = OCSP_CERTID_dup(cid)) == NULL) goto err;
  ...
  switch (cs->type = status) {
    case V_OCSP_CERTSTATUS_REVOKED:
      ...
      if ((cs->value.revoked = ri = OCSP_REVOKEDINFO_new()) == NULL) goto err;
      if (!ASN1_TIME_to_generalizedtime(revtime, &ri->revocationTime)) goto err;
      if (reason != OCSP_REVOKED_STATUS_NOSTATUS) {
        if ((ri->revocationReason = ASN1_ENUMERATED_new()) == NULL) goto err;
        if (!(ASN1_ENUMERATED_set(ri->revocationReason, reason))) goto err;
      }
      break;

    case V_OCSP_CERTSTATUS_GOOD:
      if ((cs->value.good = ASN1_NULL_new()) == NULL) goto err;
      break;

    case V_OCSP_CERTSTATUS_UNKNOWN:
      if ((cs->value.unknown = ASN1_NULL_new()) == NULL) goto err;
      break;
...
int OCSP_basic_sign_ctx(OCSP_BASICRESP* brsp, X509* signer, EVP_MD_CTX* ctx,
  ...
  if (flags & OCSP_RESPID_KEY) {
    if (!OCSP_RESPID_set_by_key(rid, signer)) goto err;
...
int OCSP_RESPID_set_by_key_ex(OCSP_RESPID* respid, X509* cert,
  ...
  if (sha1 == NULL) return 0;

  /* RFC2560 requires SHA1 */
  if (!X509_pubkey_digest(cert, sha1, md, NULL)) goto err;

  byKey = ASN1_OCTET_STRING_new();
  if (byKey == NULL) goto err;
...
int OCSP_RESPID_match_ex(OCSP_RESPID* respid, X509* cert, OSSL_LIB_CTX* libctx,
  ...
  if (respid->type == V_OCSP_RESPID_KEY) {
    ...
    if (respid->value.byKey == NULL) goto err;
    ...
    if (respid->value.byName == NULL) return 0;
