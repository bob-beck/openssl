...
int OCSP_check_nonce(OCSP_REQUEST* req, OCSP_BASICRESP* bs) {
  ...
  /* Check both absent */
  if ((req_idx < 0) && (resp_idx < 0)) return 2;
  /* Check in request only */
  if ((req_idx >= 0) && (resp_idx < 0)) return -1;
  /* Check in response but not request */
  if ((req_idx < 0) && (resp_idx >= 0)) return 3;
  /*
...
X509_EXTENSION* OCSP_crlID_new(const char* url, long* n, char* tim) {
  ...
  if ((cid = OCSP_CRLID_new()) == NULL) goto err;
  if (url) {
    if ((cid->crlUrl = ASN1_IA5STRING_new()) == NULL) goto err;
    if (!(ASN1_STRING_set(cid->crlUrl, url, -1))) goto err;
  }
  if (n) {
    if ((cid->crlNum = ASN1_INTEGER_new()) == NULL) goto err;
    if (!(ASN1_INTEGER_set(cid->crlNum, *n))) goto err;
  }
  if (tim) {
    if ((cid->crlTime = ASN1_GENERALIZEDTIME_new()) == NULL) goto err;
    if (!(ASN1_GENERALIZEDTIME_set_string(cid->crlTime, tim))) goto err;
  }
...
X509_EXTENSION* OCSP_accept_responses_new(char** oids) {
  ...
  if ((sk = sk_ASN1_OBJECT_new_null()) == NULL) goto err;
  while (oids && *oids) {
    if ((nid = OBJ_txt2nid(*oids)) != NID_undef && (o = OBJ_nid2obj(nid)))
      if (!sk_ASN1_OBJECT_push(sk, o)) goto err;
    oids++;
  }
...
X509_EXTENSION* OCSP_archive_cutoff_new(char* tim) {
  ...
  if ((gt = ASN1_GENERALIZEDTIME_new()) == NULL) goto err;
...
X509_EXTENSION* OCSP_url_svcloc_new(const X509_NAME* issuer,
  ...
  if ((sloc = OCSP_SERVICELOC_new()) == NULL) goto err;
  X509_NAME_free(sloc->issuer);
  if ((sloc->issuer = X509_NAME_dup(issuer)) == NULL) goto err;
  if (urls && *urls &&
      (sloc->locator = sk_ACCESS_DESCRIPTION_new_null()) == NULL)
    goto err;
  while (urls && *urls) {
    if ((ad = ACCESS_DESCRIPTION_new()) == NULL) goto err;
    if ((ad->method = OBJ_nid2obj(NID_ad_OCSP)) == NULL) goto err;
    if ((ia5 = ASN1_IA5STRING_new()) == NULL) goto err;
