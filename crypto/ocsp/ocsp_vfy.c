...
static int
ocsp_match_issuerid(X509* cert,
                    OCSP_CERTID* cid,
                    STACK_OF(OCSP_SINGLERESP)* sresp);
static int
ocsp_check_delegated(X509* x);
static int
ocsp_req_find_signer(X509** psigner,
                     OCSP_REQUEST* req,
                     const X509_NAME* nm,
                     STACK_OF(X509)* certs,
                     unsigned long flags);
...
static int
ocsp_verify_signer(X509* signer,
                   int response,
                   X509_STORE* st,
                   unsigned long flags,
...
int
OCSP_basic_verify(OCSP_BASICRESP* bs,
                  STACK_OF(X509)* certs,
                  X509_STORE* st,
                  unsigned long flags)
{
...
int
OCSP_resp_get0_signer(OCSP_BASICRESP* bs,
                      X509** signer,
 ...
 */
int
OCSP_request_verify(OCSP_REQUEST* req,
                    STACK_OF(X509)* certs,
                    X509_STORE* store,
                    unsigned long flags)
{
