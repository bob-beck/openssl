...
static int ocsp_verify_signer(X509* signer, int response, X509_STORE* st,
  ...
  if ((vp = X509_STORE_CTX_get0_param(ctx)) == NULL) goto end;
...
static int ocsp_verify(OCSP_REQUEST* req, OCSP_BASICRESP* bs, X509* signer,
  ...
  if ((flags & OCSP_NOSIGS) == 0) {
    ...
    if (ret <= 0) ERR_raise(ERR_LIB_OCSP, OCSP_R_SIGNATURE_FAILURE);
  }
  return ret;
}
...
int OCSP_basic_verify(OCSP_BASICRESP* bs, STACK_OF(X509)* certs, X509_STORE* st,
  ...
  if ((ret == 2) && (flags & OCSP_TRUSTOTHER) != 0) flags |= OCSP_NOVERIFY;

  if ((ret = ocsp_verify(NULL, bs, signer, flags)) <= 0) goto end;
  if ((flags & OCSP_NOVERIFY) == 0) {
    ret = -1;
    if ((flags & OCSP_NOCHAIN) == 0) {
      if ((untrusted = sk_X509_dup(bs->certs)) == NULL) goto end;
      if (!X509_add_certs(untrusted, certs, X509_ADD_FLAG_DEFAULT)) goto end;
    }
     ...
     * trust
     */
    if ((flags & OCSP_NOEXPLICIT) != 0) goto end;
...
static X509* ocsp_find_signer_sk(STACK_OF(X509)* certs, OCSP_RESPID* id) {
  ...
  for (i = 0; i < sk_X509_num(certs); i++) {
    if ((x = sk_X509_value(certs, i)) != NULL) {
      if ((md = EVP_MD_fetch(x->libctx, SN_sha1, x->propq)) == NULL) break;
      r = X509_pubkey_digest(x, md, tmphash, NULL);
      EVP_MD_free(md);
      if (!r) break;
      if (memcmp(keyhash, tmphash, SHA_DIGEST_LENGTH) == 0) return x;
    }
  }
  return NULL;
}
...
static int ocsp_check_issuer(OCSP_BASICRESP* bs, STACK_OF(X509)* chain) {
  ...
  if (sk_X509_num(chain) > 1) {
    ...
    if (ret != 0) {
      ...
      if (ocsp_check_delegated(signer)) return 1;
      return 0;
    }
  }
...
static int ocsp_match_issuerid(X509* cert, OCSP_CERTID* cid,
  ...
  if (cid != NULL) {
    ...
    if (dgst == NULL) dgst = (EVP_MD*)EVP_get_digestbyname(name);
    ...
    if (!X509_NAME_digest(iname, dgst, md, NULL)) goto end;
...
int OCSP_request_verify(OCSP_REQUEST* req, STACK_OF(X509)* certs,
  ...
  if ((flags & OCSP_NOVERIFY) != 0) return 1;
