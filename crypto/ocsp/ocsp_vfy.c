...
static int
ocsp_verify_signer (X509 *signer, int response, X509_STORE *st,
...
static int
ocsp_verify (OCSP_REQUEST *req, OCSP_BASICRESP *bs, X509 *signer,
             unsigned long flags)
{
  EVP_PKEY *skey;
  int ret = 1;
 ...
 * Just find the signer's certificate and verify it against a given trust
 * value. Returns 1 on success, 0 on failure and on fatal error.
 */
int
OCSP_request_verify (OCSP_REQUEST *req, STACK_OF (X509) *certs,
...
static int
ocsp_req_find_signer (X509 **psigner, OCSP_REQUEST *req, const X509_NAME *nm,
                      STACK_OF (X509) *certs, unsigned long flags)
{
  X509 *signer;
