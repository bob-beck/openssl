...
static int int_engine_init(ENGINE* e) {
  if (!ENGINE_init(e)) return 0;
  if (!initialized_engines) initialized_engines = sk_ENGINE_new_null();
...
static int int_engine_configure(const char* name, const char* value,
  ...
  for (i = 0; i < sk_CONF_VALUE_num(ecmds); i++) {
    ...
    /* Load a dynamic ENGINE */
      ...
      if (!e) goto err;
      if (!ENGINE_ctrl_cmd_string(e, "SO_PATH", ctrlvalue, 0)) goto err;
      if (!ENGINE_ctrl_cmd_string(e, "LIST_ADD", "2", 0)) goto err;
    ...
    /* ... add other pseudos here ... */
       ...
       */
      if (strcmp(ctrlvalue, "EMPTY") == 0) ctrlvalue = NULL;
      if (strcmp(ctrlname, "init") == 0) {
        if (!NCONF_get_number_e(cnf, value, "init", &do_init)) goto err;
        if (do_init == 1) {
          if (!int_engine_init(e)) goto err;
        } else if (do_init != 0) {
          ERR_raise(ERR_LIB_ENGINE, ENGINE_R_INVALID_INIT_VALUE);
          goto err;
        }
      } else if (strcmp(ctrlname, "default_algorithms") == 0) {
        if (!ENGINE_set_default_string(e, ctrlvalue)) goto err;
...
static int int_engine_module_init(CONF_IMODULE* md, const CONF* cnf) {
  ...
  for (i = 0; i < sk_CONF_VALUE_num(elist); i++) {
    ...
    if (!int_engine_configure(cval->name, cval->value, cnf)) return 0;
  }
