...
static int dynamic_set_data_ctx(ENGINE* e, dynamic_data_ctx** ctx) {
  ...
  c->dir_load = 1;
  if (!CRYPTO_THREAD_write_lock(global_engine_lock)) goto end;
  ...
  if (c != NULL) sk_OPENSSL_STRING_free(c->dirs);
  OPENSSL_free(c);
  return ret;
}
...
static int int_load(dynamic_data_ctx* ctx) {
  ...
  if (!ctx->dir_load || (num = sk_OPENSSL_STRING_num(ctx->dirs)) < 1) return 0;
...
static int dynamic_load(ENGINE* e, dynamic_data_ctx* ctx) {
  ...
  if (ctx->dynamic_dso == NULL) ctx->dynamic_dso = DSO_new();
  if (ctx->dynamic_dso == NULL) return 0;
  if (!ctx->DYNAMIC_LIBNAME) {
    if (!ctx->engine_id) return 0;
  ...
  if (!ctx->no_vcheck) {
    ...
    if (ctx->v_check) vcheck_res = ctx->v_check(OSSL_DYNAMIC_VERSION);
    /*
