...
#ifdef TEST_ENG_OPENSSL_RC4
/*-
 * This section of code compiles an "alternative implementation" of two modes
 * of RC4 into this ENGINE. The result is that EVP_CIPHER operation for "rc4"
 * should under normal circumstances go via this support rather than the
 * default EVP support. There are other symbols to tweak the testing;
 *    TEST_ENC_OPENSSL_RC4_OTHERS - print a one line message to stderr each
 * time we're asked for a cipher we don't support (should not happen).
 *    TEST_ENG_OPENSSL_RC4_P_INIT - print a one line message to stderr each
 * time the "init_key" handler is called. TEST_ENG_OPENSSL_RC4_P_CIPHER - ditto
 * for the "cipher" handler.
 */
...
static void
test_r4_40_cipher_destroy (void)
{
  EVP_CIPHER_meth_free (r4_40_cipher);
  r4_40_cipher = NULL;
}
static int
test_cipher_nids (const int **nids)
{
...
static int
openssl_ciphers (ENGINE *e, const EVP_CIPHER **cipher, const int **nids,
                 int nid)
{
  if (!cipher)
    {
...
static void
test_sha_md_destroy (void)
{
  EVP_MD_meth_free (sha1_md);
  sha1_md = NULL;
}
static int
test_digest_nids (const int **nids)
{
...
static int
ossl_hmac_init (EVP_PKEY_CTX *ctx)
{
...
static int
ossl_hmac_copy (EVP_PKEY_CTX *dst, EVP_PKEY_CTX *src)
{
...
static void
ossl_hmac_cleanup (EVP_PKEY_CTX *ctx)
{
...
static int
ossl_hmac_keygen (EVP_PKEY_CTX *ctx, EVP_PKEY *pkey)
{
...
static int
ossl_int_update (EVP_MD_CTX *ctx, const void *data, size_t count)
{
  OSSL_HMAC_PKEY_CTX *hctx
...
static int
ossl_hmac_signctx_init (EVP_PKEY_CTX *ctx, EVP_MD_CTX *mctx)
{
...
static int
ossl_hmac_signctx (EVP_PKEY_CTX *ctx, unsigned char *sig, size_t *siglen,
                   EVP_MD_CTX *mctx)
{
  unsigned int hlen;
  OSSL_HMAC_PKEY_CTX *hctx = EVP_PKEY_CTX_get_data (ctx);
...
static int
ossl_hmac_ctrl (EVP_PKEY_CTX *ctx, int type, int p1, void *p2)
{
  OSSL_HMAC_PKEY_CTX *hctx = EVP_PKEY_CTX_get_data (ctx);
  EVP_PKEY *pk;
...
static int
ossl_hmac_ctrl_str (EVP_PKEY_CTX *ctx, const char *type, const char *value)
{
  if (!value)
    {
      return 0;
...
static int
ossl_pkey_meths (ENGINE *e, EVP_PKEY_METHOD **pmeth, const int **nids, int nid)
{
