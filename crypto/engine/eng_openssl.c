...
static int bind_fn(ENGINE* e, const char* id) {
  if (id && (strcmp(id, engine_openssl_id) != 0)) return 0;
...
static int test_digest_nids(const int** nids) {
  ...
  if (!init) {
    const EVP_MD* md;
    if ((md = test_sha_md()) != NULL) digest_nids[pos++] = EVP_MD_get_type(md);
    digest_nids[pos] = 0;
    init = 1;
  }
  *nids = digest_nids;
...
static int ossl_hmac_init(EVP_PKEY_CTX* ctx) {
  ...
  if ((hctx = OPENSSL_zalloc(sizeof(*hctx))) == NULL) return 0;
...
static int ossl_hmac_copy(EVP_PKEY_CTX* dst, EVP_PKEY_CTX* src) {
  ...
  if (!ossl_hmac_init(dst)) return 0;
  sctx = EVP_PKEY_CTX_get_data(src);
  dctx = EVP_PKEY_CTX_get_data(dst);
  dctx->md = sctx->md;
  if (!HMAC_CTX_copy(dctx->ctx, sctx->ctx)) goto err;
  if (sctx->ktmp.data) {
...
static int ossl_int_update(EVP_MD_CTX* ctx, const void* data, size_t count) {
  ...
  if (!HMAC_Update(hctx->ctx, data, count)) return 0;
  return 1;
}
...
static int ossl_hmac_signctx(EVP_PKEY_CTX* ctx, unsigned char* sig,
  ...
  *siglen = l;
  ...
  if (!HMAC_Final(hctx->ctx, sig, &hlen)) return 0;
  *siglen = (size_t)hlen;
  return 1;
}
...
static int ossl_hmac_ctrl(EVP_PKEY_CTX* ctx, int type, int p1, void* p2) {
  ...
  switch (type) {
    case EVP_PKEY_CTRL_SET_MAC_KEY:
      if ((!p2 && p1 > 0) || (p1 < -1)) return 0;
      if (!ASN1_OCTET_STRING_set(&hctx->ktmp, p2, p1)) return 0;
      break;
