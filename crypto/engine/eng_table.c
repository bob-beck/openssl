...
int engine_table_register(ENGINE_TABLE** table, ENGINE_CLEANUP_CB* cleanup,
  ...
  if (!(*table)) added = 1;
  if (!int_table_check(table, 1)) goto end;
  ...
  while (num_nids--) {
     ...
     */
    if (!sk_ENGINE_push(fnd->sk, e)) goto end;
    ...
    if (setdefault) {
      ...
      if (fnd->funct) engine_unlocked_finish(fnd->funct, 0);
      fnd->funct = e;
      fnd->uptodate = 1;
    }
    nids++;
...
static void int_cleanup_cb_doall(ENGINE_PILE* p) {
  if (p == NULL) return;
  sk_ENGINE_free(p->sk);
  if (p->funct) engine_unlocked_finish(p->funct, 0);
  OPENSSL_free(p);
}

void engine_table_cleanup(ENGINE_TABLE** table) {
  if (!CRYPTO_THREAD_write_lock(global_engine_lock)) return;
  if (*table) {
