...
static void
int_unregister_cb(ENGINE_PILE* pile, ENGINE* e)
{
  int n;
...
ossl_engine_table_select(ENGINE_TABLE** table, int nid, const char* f, int l)
{
  ...
  if (!(*table)) {
    OSSL_TRACE3(
      ENGINE_TABLE, "%s:%d, nid=%d, nothing registered!\n", f, l, nid);
    return NULL;
  }
...
trynext:
  ...
  if (ret)
    OSSL_TRACE4(
      ENGINE_TABLE, "%s:%d, nid=%d, caching ENGINE '%s'\n", f, l, nid, ret->id);
  else
    OSSL_TRACE3(
      ENGINE_TABLE, "%s:%d, nid=%d, caching 'no matching ENGINE'\n", f, l, nid);
  CRYPTO_THREAD_unlock(global_engine_lock);
  /*
...
static void
int_dall(const ENGINE_PILE* pile, ENGINE_PILE_DOALL* dall)
{
