...
CRYPTO_THREAD* ossl_crypto_thread_native_start(CRYPTO_THREAD_ROUTINE routine,
  ...
  if ((handle->lock = ossl_crypto_mutex_new()) == NULL) goto fail;
  if ((handle->statelock = ossl_crypto_mutex_new()) == NULL) goto fail;
  if ((handle->condvar = ossl_crypto_condvar_new()) == NULL) goto fail;

  handle->data = data;
  handle->routine = routine;
  handle->joinable = joinable;

  if (ossl_crypto_thread_native_spawn(handle) == 1) return handle;
...
int ossl_crypto_thread_native_join(CRYPTO_THREAD* thread,
  ...
  if (CRYPTO_THREAD_GET_STATE(thread, CRYPTO_THREAD_JOINED)) goto pass;
  ...
  if (ossl_crypto_thread_native_perform_join(thread, retval) == 0) goto fail;
