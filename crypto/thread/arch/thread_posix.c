...
int ossl_crypto_thread_native_perform_join(CRYPTO_THREAD* thread,
  ...
  if (pthread_join(*handle, &thread_retval) != 0) return 0;
   ...
   */
  if (thread_retval != NULL) return 0;
...
int ossl_crypto_mutex_try_lock(CRYPTO_MUTEX* mutex) {
  ...
  if (pthread_mutex_trylock(mutex_p) == EBUSY) return 0;
...
void ossl_crypto_mutex_free(CRYPTO_MUTEX** mutex) {
  ...
  mutex_p = (pthread_mutex_t**)mutex;
  if (*mutex_p != NULL) pthread_mutex_destroy(*mutex_p);
...
void ossl_crypto_condvar_free(CRYPTO_CONDVAR** cv) {
  ...
  cv_p = (pthread_cond_t**)cv;
  if (*cv_p != NULL) pthread_cond_destroy(*cv_p);
  OPENSSL_free(*cv_p);
  *cv_p = NULL;
}
