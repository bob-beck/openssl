...
int ossl_crypto_thread_native_perform_join(CRYPTO_THREAD* thread,
  ...
  if (thread == NULL || thread->handle == NULL) return 0;

  handle = (HANDLE*)thread->handle;
  if (WaitForSingleObject(*handle, INFINITE) != WAIT_OBJECT_0) return 0;

  if (GetExitCodeThread(*handle, &thread_retval) == 0) return 0;
   ...
   */
  if (thread_retval != 0) return 0;

  if (CloseHandle(*handle) == 0) return 0;
...
CRYPTO_MUTEX* ossl_crypto_mutex_new(void) {
  ...
  if ((mutex = OPENSSL_zalloc(sizeof(*mutex))) == NULL) return NULL;
...
int ossl_crypto_mutex_try_lock(CRYPTO_MUTEX* mutex) {
  ...
  mutex_p = (CRITICAL_SECTION*)mutex;
  if (TryEnterCriticalSection(mutex_p)) return 1;
...
void ossl_crypto_mutex_free(CRYPTO_MUTEX** mutex) {
  ...
  mutex_p = (CRITICAL_SECTION**)mutex;
  if (*mutex_p != NULL) DeleteCriticalSection(*mutex_p);
...
static int determine_timeout(OSSL_TIME deadline, DWORD* w_timeout_p) {
  ...
  if (ossl_time_is_zero(delta)) return 0;
...
CRYPTO_CONDVAR* ossl_crypto_condvar_new(void) {
  ...
  if ((cv = OPENSSL_malloc(sizeof(LEGACY_CONDVAR))) == NULL) return NULL;
...
static uint32_t obj_wait(HANDLE h, OSSL_TIME deadline) {
  ...
  if (!determine_timeout(deadline, &timeout)) timeout = 1;
  ...
  do {
    ...
    if (closed)
      if (obj_wait(cv->prewait_sema, deadline) != WAIT_OBJECT_0) {
         ...
         */
        ossl_crypto_mutex_lock(cv->int_m);
        if (set_prewait && cv->gen == orig_gen) --cv->num_prewait;
        ossl_crypto_mutex_unlock(cv->int_m);
        return;
      }
  } while (closed);
...
CRYPTO_CONDVAR* ossl_crypto_condvar_new(void) {
  ...
  if ((cv_p = OPENSSL_zalloc(sizeof(*cv_p))) == NULL) return NULL;
