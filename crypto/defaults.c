...
static char* get_windows_regdirs(char* dst, DWORD dstsizebytes,
  ...
  if (ret != ERROR_SUCCESS) goto out;
  if (ktype != REG_EXPAND_SZ && ktype != REG_SZ) goto out;
  if (keysizebytes > MAX_PATH * sizeof(WCHAR)) goto out;
...
DEFINE_RUN_ONCE_STATIC(do_defaults_setup) {
   ...
   */
  if (strlen(openssldir) > 0) openssldirptr = openssldir;

  if (strlen(enginesdir) > 0) enginesdirptr = enginesdir;

  if (strlen(modulesdir) > 0) modulesdirptr = modulesdir;
...
#if defined(_WIN32) && defined(OSSL_WINCTX)
  if (!RUN_ONCE(&defaults_setup_init, do_defaults_setup)) return NULL;
