...
static __owur int single_keccak(uint8_t* out, size_t outlen, const uint8_t* in,
  ...
  if (!EVP_DigestUpdate(mdctx, in, inlen)) return 0;
...
static int hash_h_pubkey(uint8_t pkhash[ML_KEM_PKHASH_BYTES], EVP_MD_CTX* mdctx,
  ...
  if (!EVP_DigestInit_ex(mdctx, key->sha3_256_md, NULL)) return 0;

  do {
    uint8_t buf[3 * DEGREE / 2];

    scalar_encode(buf, t++, 12);
    if (!EVP_DigestUpdate(mdctx, buf, sizeof(buf))) return 0;
  } while (t < end);
...
static __owur int sample_scalar(scalar* out, EVP_MD_CTX* mdctx) {
  ...
  do {
    if (!EVP_DigestSqueeze(mdctx, in = buf, sizeof(buf))) return 0;
    do {
      b1 = *in++;
      b2 = *in++;
      b3 = *in++;

      if (curr >= endout) break;
      if ((d = ((b2 & 0x0f) << 8) + b1) < kPrime) *curr++ = d;
      if (curr >= endout) break;
      if ((d = (b3 << 4) + (b2 >> 4)) < kPrime) *curr++ = d;
    } while (in < endin);
  } while (curr < endout);
  return 1;
}
...
static __owur int scalar_decode_12(scalar* out,
  ...
  for (i = 0; i < DEGREE / 2; ++i) {
    ...
    if (outOfRange1 | outOfRange2) return 0;
  }
  return 1;
}
...
static __owur int vector_decode_12(scalar* out,
  ...
  for (; rank-- > 0; in += stride)
    if (!scalar_decode_12(out++, in)) return 0;
  return 1;
}
...
static __owur int cbd_2(scalar* out, uint8_t in[ML_KEM_RANDOM_BYTES + 1],
  ...
  if (!prf(randbuf, sizeof(randbuf), in, mdctx, key)) return 0;
...
static __owur int gencbd_vector(scalar* out, CBD_FUNC cbd, uint8_t* counter,
  ...
  do {
    ...
    if (!cbd(out++, input, mdctx, key)) return 0;
  } while (--rank > 0);
  return 1;
}
...
static __owur int encrypt_cpa(uint8_t out[ML_KEM_SHARED_SECRET_BYTES],
  ...
  /* FIPS 203 "y" vector */
  if (!gencbd_vector_ntt(y, cbd_1, &counter, r, rank, mdctx, key)) return 0;
  /* FIPS 203 "v" scalar */
  ...
  if (!gencbd_vector(e1, cbd_2, &counter, r, rank, mdctx, key)) return 0;
  ...
  input[ML_KEM_RANDOM_BYTES] = counter;
  if (!cbd_2(e2, input, mdctx, key)) return 0;
  scalar_add(&v, e2);
...
static __owur int genkey(const uint8_t seed[ML_KEM_SEED_BYTES],
  ...
  if (pubenc == NULL) {
    ...
    if (!hash_h_pubkey(key->pkhash, mdctx, key)) goto end;
  } else {
    encode_pubkey(pubenc, key);
    if (!hash_h(key->pkhash, pubenc, vinfo->pubkey_bytes, mdctx, key)) goto end;
  }
...
static __owur int add_storage(scalar* pub, scalar* priv, int private,
  ...
  if (pub == NULL || (private && priv == NULL)) {
    /*
     * One of these could be allocated correctly. It is legal to call free with
     * a NULL pointer, so always attempt to free both allocations here
     */
    OPENSSL_free(pub);
   ...
   * non-NULL |d| pointer.
   */
  if (private) key->z = (uint8_t*)(rank + (key->s = priv));
  return 1;
}
...
ML_KEM_KEY* ossl_ml_kem_key_new(OSSL_LIB_CTX* libctx, const char* properties,
  ...
  if ((key = OPENSSL_malloc(sizeof(*key))) == NULL) return NULL;
...
ML_KEM_KEY* ossl_ml_kem_key_dup(const ML_KEM_KEY* key, int selection) {
  ...
  switch (selection & OSSL_KEYMGMT_SELECT_KEYPAIR) {
    ...
    case OSSL_KEYMGMT_SELECT_PRIVATE_KEY:
      ...
      if (key->d != NULL) ret->d = ret->z + ML_KEM_RANDOM_BYTES;
      break;
  }
...
int ossl_ml_kem_encode_seed(uint8_t* out, size_t len, const ML_KEM_KEY* key) {
  if (key == NULL || key->d == NULL || len != ML_KEM_SEED_BYTES) return 0;
  /*
...
ML_KEM_KEY* ossl_ml_kem_set_seed(const uint8_t* seed, size_t seedlen,
  ...
  if (key->seedbuf == NULL) {
    ...
    if (key->seedbuf == NULL) return NULL;
  }
...
int ossl_ml_kem_parse_public_key(const uint8_t* in, size_t len,
  ...
  if (!ret) ossl_ml_kem_key_reset(key);
...
int ossl_ml_kem_genkey(uint8_t* pubenc, size_t publen, ML_KEM_KEY* key) {
  ...
  if (key->seedbuf != NULL) {
    if (!ossl_ml_kem_encode_seed(seed, sizeof(seed), key)) return 0;
    ossl_ml_kem_key_reset(key);
...
int ossl_ml_kem_encap_seed(uint8_t* ctext, size_t clen, uint8_t* shared_secret,
  ...
  if (key == NULL || !ossl_ml_kem_have_pubkey(key)) return 0;
  vinfo = key->vinfo;
...
int ossl_ml_kem_decap(uint8_t* shared_secret, size_t slen, const uint8_t* ctext,
  ...
  if (!ossl_ml_kem_have_prvkey(key)) return 0;
  vinfo = key->vinfo;
