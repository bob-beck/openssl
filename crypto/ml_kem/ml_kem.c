 ...
 */
static const ML_KEM_VINFO vinfo_map[3] = { { "ML-KEM-512",
                                             PRVKEY_BYTES(512),
 ...
 */
static __owur int
hash_h(uint8_t out[ML_KEM_PKHASH_BYTES],
       const uint8_t* in,
       size_t len,
       EVP_MD_CTX* mdctx,
       const ML_KEM_KEY* key)
 ...
 */
static __owur int
hash_g(uint8_t out[ML_KEM_SEED_BYTES],
       const uint8_t* in,
       size_t len,
       EVP_MD_CTX* mdctx,
       const ML_KEM_KEY* key)
...
static void
matrix_mult_transpose_add(scalar* out,
                          const scalar* m,
                          const scalar* a,
                          int rank)
{
 ...
 */
static __owur int
genkey(const uint8_t seed[ML_KEM_SEED_BYTES],
       EVP_MD_CTX* mdctx,
       uint8_t* pubenc,
       ML_KEM_KEY* key)
{
 ...
 */
static int
decap(uint8_t secret[ML_KEM_SHARED_SECRET_BYTES],
      const uint8_t* ctext,
      uint8_t* tmp_ctext,
      scalar* tmp,
      EVP_MD_CTX* mdctx,
...
ossl_ml_kem_key_dup(const ML_KEM_KEY* key, int selection)
{
  ...
  switch (selection & OSSL_KEYMGMT_SELECT_KEYPAIR) {
    ...
    case OSSL_KEYMGMT_SELECT_PUBLIC_KEY:
      ok =
        add_storage(OPENSSL_memdup(key->t, key->vinfo->puballoc), NULL, 0, ret);
      break;
...
ossl_ml_kem_parse_private_key(const uint8_t* in, size_t len, ML_KEM_KEY* key)
{
  ...
  if (add_storage(OPENSSL_malloc(vinfo->puballoc),
                  OPENSSL_secure_malloc(vinfo->prvalloc),
                  1,
                  key))
...
ossl_ml_kem_genkey(uint8_t* pubenc, size_t publen, ML_KEM_KEY* key)
{
  ...
  if (key->seedbuf != NULL) {
    ...
    ossl_ml_kem_key_reset(key);
               ...
               key->libctx, seed, sizeof(seed), key->vinfo->secbits) <= 0) {
    return 0;
  }
 ...
 */
int
ossl_ml_kem_encap_seed(uint8_t* ctext,
                       size_t clen,
...
int
ossl_ml_kem_encap_rand(uint8_t* ctext,
                       size_t clen,
                       uint8_t* shared_secret,
                       size_t slen,
                       const ML_KEM_KEY* key)
{
  ...
  return ossl_ml_kem_encap_seed(
    ctext, clen, shared_secret, slen, r, sizeof(r), key);
}

int
ossl_ml_kem_decap(uint8_t* shared_secret,
                  size_t slen,
                  const uint8_t* ctext,
                  size_t clen,
                  const ML_KEM_KEY* key)
{
  ...
  if (shared_secret == NULL || slen != ML_KEM_SHARED_SECRET_BYTES ||
    ...
    (void)RAND_bytes_ex(
      key->libctx, shared_secret, ML_KEM_SHARED_SECRET_BYTES, vinfo->secbits);
    return 0;
  }
#if defined(OPENSSL_CONSTANT_TIME_VALIDATION)
  /*
