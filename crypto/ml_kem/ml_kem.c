 ...
 */
static __owur int single_keccak(uint8_t* out,
                                size_t outlen,
                                const uint8_t* in,
                                size_t inlen,
                                EVP_MD_CTX* mdctx) {
 ...
 */
static __owur int prf(uint8_t* out,
                      size_t len,
 ...
 */
static __owur int hash_h(uint8_t out[ML_KEM_PKHASH_BYTES],
                         const uint8_t* in,
                         size_t len,
                         EVP_MD_CTX* mdctx,
                         const ML_KEM_KEY* key) {
...
static int hash_h_pubkey(uint8_t pkhash[ML_KEM_PKHASH_BYTES],
                         EVP_MD_CTX* mdctx,
                         ML_KEM_KEY* key) {
 ...
 */
static __owur int hash_g(uint8_t out[ML_KEM_SEED_BYTES],
                         const uint8_t* in,
                         size_t len,
                         EVP_MD_CTX* mdctx,
                         const ML_KEM_KEY* key) {
...
static ossl_inline void scalar_mult_add(scalar* out,
                                        const scalar* lhs,
                                        const scalar* rhs) {
  uint16_t *curr = out->c, *end = curr + DEGREE;
  const uint16_t *lc = lhs->c, *rc = rhs->c;
 ...
 */
static void vector_decode_decompress_ntt(scalar* out,
                                         const uint8_t* in,
                                         int bits,
                                         int rank) {
  int stride = bits * DEGREE / 8;
...
static void inner_product(scalar* out,
                          const scalar* lhs,
                          const scalar* rhs,
                          int rank) {
 ...
 */
static void matrix_mult_intt(scalar* out,
                             const scalar* m,
                             const scalar* a,
                             int rank) {
  const scalar* ar;
...
static void matrix_mult_transpose_add(scalar* out,
                                      const scalar* m,
                                      const scalar* a,
                                      int rank) {
 ...
 */
static __owur int cbd_2(scalar* out,
                        uint8_t in[ML_KEM_RANDOM_BYTES + 1],
                        EVP_MD_CTX* mdctx,
                        const ML_KEM_KEY* key) {
  uint16_t *curr = out->c, *end = curr + DEGREE;
 ...
 */
static __owur int cbd_3(scalar* out,
                        uint8_t in[ML_KEM_RANDOM_BYTES + 1],
                        EVP_MD_CTX* mdctx,
                        const ML_KEM_KEY* key) {
  uint16_t *curr = out->c, *end = curr + DEGREE;
 ...
 */
static __owur int add_storage(scalar* pub,
                              scalar* priv,
                              int private,
                              ML_KEM_KEY* key) {
  int rank = key->vinfo->rank;
...
int ossl_ml_kem_encode_public_key(uint8_t* out,
                                  size_t len,
                                  const ML_KEM_KEY* key) {
...
int ossl_ml_kem_encode_private_key(uint8_t* out,
                                   size_t len,
                                   const ML_KEM_KEY* key) {
 ...
 */
ML_KEM_KEY* ossl_ml_kem_set_seed(const uint8_t* seed,
                                 size_t seedlen,
                                 ML_KEM_KEY* key) {
...
int ossl_ml_kem_encap_rand(uint8_t* ctext,
                           size_t clen,
                           uint8_t* shared_secret,
                           size_t slen,
                           const ML_KEM_KEY* key) {
