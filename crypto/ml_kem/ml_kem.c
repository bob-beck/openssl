...
static const ML_KEM_VINFO vinfo_map[3]
    = { { "ML-KEM-512", PRVKEY_BYTES (512), sizeof (struct prvkey_512_alloc),
          PUBKEY_BYTES (512), sizeof (struct pubkey_512_alloc),
          CTEXT_BYTES (512), VECTOR_BYTES (512), U_VECTOR_BYTES (512),
          EVP_PKEY_ML_KEM_512, ML_KEM_512_BITS, ML_KEM_512_RANK, ML_KEM_512_DU,
          ML_KEM_512_DV, ML_KEM_512_SECBITS, ML_KEM_512_SECURITY_CATEGORY },
        { "ML-KEM-768", PRVKEY_BYTES (768), sizeof (struct prvkey_768_alloc),
          PUBKEY_BYTES (768), sizeof (struct pubkey_768_alloc),
          CTEXT_BYTES (768), VECTOR_BYTES (768), U_VECTOR_BYTES (768),
          EVP_PKEY_ML_KEM_768, ML_KEM_768_BITS, ML_KEM_768_RANK, ML_KEM_768_DU,
          ML_KEM_768_DV, ML_KEM_768_SECBITS, ML_KEM_768_SECURITY_CATEGORY },
        { "ML-KEM-1024", PRVKEY_BYTES (1024),
          sizeof (struct prvkey_1024_alloc), PUBKEY_BYTES (1024),
          sizeof (struct pubkey_1024_alloc), CTEXT_BYTES (1024),
 ...
 */
static __owur int
single_keccak (uint8_t *out, size_t outlen, const uint8_t *in, size_t inlen,
               EVP_MD_CTX *mdctx)
{
 ...
 */
static __owur int
prf (uint8_t *out, size_t len, const uint8_t in[ML_KEM_RANDOM_BYTES + 1],
     EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
 ...
 */
static __owur int
hash_h (uint8_t out[ML_KEM_PKHASH_BYTES], const uint8_t *in, size_t len,
        EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
...
static int
hash_h_pubkey (uint8_t pkhash[ML_KEM_PKHASH_BYTES], EVP_MD_CTX *mdctx,
               ML_KEM_KEY *key)
{
 ...
 */
static __owur int
hash_g (uint8_t out[ML_KEM_SEED_BYTES], const uint8_t *in, size_t len,
        EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
 ...
 */
static __owur int
kdf (uint8_t out[ML_KEM_SHARED_SECRET_BYTES],
     const uint8_t z[ML_KEM_RANDOM_BYTES], const uint8_t *ctext, size_t len,
     EVP_MD_CTX *mdctx, const ML_KEM_KEY *key)
{
  return EVP_DigestInit_ex (mdctx, key->shake256_md, NULL)
 ...
 */
static void
vector_decode_decompress_ntt (scalar *out, const uint8_t *in, int bits,
                              int rank)
{
  int stride = bits * DEGREE / 8;
...
static void
matrix_mult_transpose_add (scalar *out, const scalar *m, const scalar *a,
                           int rank)
{
 ...
 * and setting the coefficient to the count of the first bits minus the count
 * of the second bits, resulting in a centered binomial distribution. Since eta
 * is two this gives -2/2 with a probability of 1/16, -1/1 with probability
 * 1/4, and 0 with probability 3/8.
 */
static __owur int
cbd_2 (scalar *out, uint8_t in[ML_KEM_RANDOM_BYTES + 1], EVP_MD_CTX *mdctx,
 ...
 * and setting the coefficient to the count of the first bits minus the count
 * of the second bits, resulting in a centered binomial distribution.
 */
static __owur int
cbd_3 (scalar *out, uint8_t in[ML_KEM_RANDOM_BYTES + 1], EVP_MD_CTX *mdctx,
       const ML_KEM_KEY *key)
{
  uint16_t *curr = out->c, *end = curr + DEGREE;
 ...
 */
static __owur int
gencbd_vector (scalar *out, CBD_FUNC cbd, uint8_t *counter,
 ...
 */
static __owur int
encrypt_cpa (uint8_t out[ML_KEM_SHARED_SECRET_BYTES],
 ...
 */
static void
decrypt_cpa (uint8_t out[ML_KEM_SHARED_SECRET_BYTES], const uint8_t *ctext,
 ...
 * 32-byte seeds public "rho", used to generate the matrix, and private
 * "sigma", used to generate the secret vector |s|.
 *
 ...
 */
static __owur int
genkey (const uint8_t seed[ML_KEM_SEED_BYTES], EVP_MD_CTX *mdctx,
 ...
 */
static int
decap (uint8_t secret[ML_KEM_SHARED_SECRET_BYTES], const uint8_t *ctext,
       uint8_t *tmp_ctext, scalar *tmp, EVP_MD_CTX *mdctx,
       const ML_KEM_KEY *key)
{
 ...
 */
static __owur int
add_storage (scalar *pub, scalar *priv, int private, ML_KEM_KEY *key)
{
  int rank = key->vinfo->rank;
  ...
  if (pub == NULL || (private && priv == NULL))
    {
      /*
       * One of these could be allocated correctly. It is legal to call free
       * with a NULL pointer, so always attempt to free both allocations here
       */
      OPENSSL_free (pub);
...
ML_KEM_KEY *
ossl_ml_kem_key_new (OSSL_LIB_CTX *libctx, const char *properties,
                     int evp_type)
{
  ...
  key->d = key->z = key->rho = key->pkhash = key->encoded_dk = key->seedbuf
      = NULL;
...
ossl_ml_kem_key_dup (const ML_KEM_KEY *key, int selection)
{
  ...
  switch (selection & OSSL_KEYMGMT_SELECT_KEYPAIR)
    ...
    case OSSL_KEYMGMT_SELECT_PUBLIC_KEY:
      ok = add_storage (OPENSSL_memdup (key->t, key->vinfo->puballoc), NULL, 0,
                        ret);
      break;
...
int
ossl_ml_kem_encode_private_key (uint8_t *out, size_t len,
                                const ML_KEM_KEY *key)
{
 ...
 */
int
ossl_ml_kem_encap_seed (uint8_t *ctext, size_t clen, uint8_t *shared_secret,
...
int
ossl_ml_kem_encap_rand (uint8_t *ctext, size_t clen, uint8_t *shared_secret,
...
int
ossl_ml_kem_decap (uint8_t *shared_secret, size_t slen, const uint8_t *ctext,
