...
alloc_new_neighborhood_list(size_t len, void** freeptr)
{
  ...
  ret = OPENSSL_aligned_alloc_array(
    len, sizeof(struct ht_neighborhood_st), CACHE_LINE_BYTES, freeptr);
...
not_found:
  ...
  if (!CRYPTO_atomic_store(
        &md->neighborhoods[neigh_idx].entries[empty_idx].hash,
        hash,
        h->atomic_lock))
    return 0;
  h->wpd.value_count++;
