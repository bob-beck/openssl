...
HT* ossl_ht_new(const HT_CONFIG* conf) {
  ...
  if (new->atomic_lock == NULL) goto err;
...
err:
  ...
  if (new->md != NULL) OPENSSL_free(new->md->neighborhood_ptr_to_free);
  OPENSSL_free(new->md);
  OPENSSL_free(new);
  return NULL;
}

int ossl_ht_read_lock(HT* htable) { return ossl_rcu_read_lock(htable->lock); }
...
void ossl_ht_write_unlock(HT* htable) {
  ...
  if (need_sync) ossl_synchronize_rcu(htable->lock);
}
  ...
  for (i = 0; i < md->neighborhood_mask + 1; i++) {
    ...
    for (j = 0; j < NEIGHBORHOOD_LEN; j++) {
      ...
      if (v != NULL && filter((HT_VALUE*)v, arg)) {
        ...
        if (list->list_len == max_len) goto out;
      }
    }
  }
out:
...
static int grow_hashtable(HT* h, size_t oldsize) {
  ...
  if (h->config.lockless_reads) goto out;

  if ((newmd = OPENSSL_zalloc(sizeof(*newmd))) == NULL) goto out;

  /* bucket list is always a power of 2 */
  newmd->neighborhoods = alloc_new_neighborhood_list(
      oldsize * 2, &newmd->neighborhood_ptr_to_free);
  if (newmd->neighborhoods == NULL) goto out_free;
  ...
  do {
    ...
    for (j = 0; j < NEIGHBORHOOD_LEN; j++) {
      ...
      if (ival == NULL) {
        ...
        if (lockless_reads) goto not_found;
        continue;
      }
...
not_found:
  ...
  if (empty_idx == SIZE_MAX) return -1; /* out of space */
...
static struct ht_internal_value_st* alloc_new_value(HT* h, HT_KEY* key,
  ...
  if (h->config.collision_check == 1) nvsize += key->keysize;
...
int ossl_ht_delete(HT* h, HT_KEY* key) {
  ...
  if (h->config.lockless_reads) return 0;
