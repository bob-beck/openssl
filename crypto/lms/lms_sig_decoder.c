...
/**
 * @brief Decode a byte array containing XDR signature data into a LMS_SIG
 * object.
 *
 ...
 */
LMS_SIG*
ossl_lms_sig_from_pkt(PACKET* pkt, const LMS_KEY* pub)
{
  ...
  if (!PACKET_get_bytes(
        pkt, (const unsigned char**)&lsig->sig.C, sig_params->n) ||
      !PACKET_get_bytes(pkt,
                        (const unsigned char**)&lsig->sig.y,
      ...
      !PACKET_get_bytes(
        pkt, (const unsigned char**)&lsig->paths, lparams->h * lparams->n) ||
 ...
 */
int
ossl_lms_sig_decode(LMS_SIG** out,
                    LMS_KEY* pub,
