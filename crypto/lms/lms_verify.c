...
static int lms_sig_compute_tc_from_path(const unsigned char* paths, uint32_t n,
  ...
  while (node_num > 1) {
     ...
     */
    if (!EVP_DigestFinal_ex(ctx, Tc, NULL)) goto err;
    path += n;
  }
  ret = 1;
err:
...
int ossl_lms_sig_verify(const LMS_SIG* lms_sig, const LMS_KEY* pub,
  ...
  if (ctx == NULL || ctxIq == NULL) goto err;

  if (!lms_evp_md_ctx_init(ctxIq, md, lms_sig->params)) goto err;
  /*
   * Algorithm 6a: Step 3.
   * Calculate a candidate public key |Kc| using the lmots_signature, message,
   * and the identifiers I, q
   */
  if (!ossl_lm_ots_compute_pubkey(ctx, ctxIq, &lms_sig->sig, pub->ots_params,
                                  pub->Id, lms_sig->q, msg, msglen, Kc))
    goto err;
   ...
   */
  if (!ossl_assert(lms_sig->q < (uint32_t)(1 << lms_params->h))) return 0;
  ...
  if (!EVP_DigestInit_ex2(ctx, NULL, NULL) ||
      ...
      !EVP_DigestUpdate(ctx, Kc, n) || !EVP_DigestFinal_ex(ctx, Tc, NULL) ||
