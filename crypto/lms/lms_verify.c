 ...
 * @param Tc Contains H(I || u32str(node_num) || u16str(D_LEAF) || Kc) on
 * input, and on output returns the calculated candidate public key.
 * @returns 1 on success, or 0 otherwise.
 */
static int
lms_sig_compute_tc_from_path (const unsigned char *paths, uint32_t n,
 ...
 */
int
ossl_lms_sig_verify (const LMS_SIG *lms_sig, const LMS_KEY *pub,
                     const EVP_MD *md, const unsigned char *msg, size_t msglen)
{
  int ret = 0;
  ...
  if (!EVP_DigestInit_ex2 (ctx, NULL, NULL)
      ...
      || !lms_sig_compute_tc_from_path (lms_sig->paths, n, node_num, ctx, ctxI,
                                        Tc))
    goto err;
  /* Algorithm 6: Step 4 */
