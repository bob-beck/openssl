...
static int lms_pubkey_from_pkt(LMS_KEY* lmskey, const unsigned char* pubkeydata,
  ...
  if (!PACKET_buf_init(&pkt, pubkeydata, publen)) goto err;
  key->encoded = (unsigned char*)PACKET_data(&pkt);
  if (!PACKET_get_net_4_len_u32(&pkt, &lms_type)) goto err;
  ...
  if (lmskey->ots_params == NULL) goto err;
...
int ossl_lms_pubkey_decode(const unsigned char* pub, size_t publen,
  ...
  if (pkey->encoded == NULL) return 0;

  if (!lms_pubkey_from_pkt(lmskey, pkey->encoded, publen)) goto err;
