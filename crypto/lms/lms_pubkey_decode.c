...
size_t ossl_lms_pubkey_length(const unsigned char *data, size_t datalen)
{
    ...
    if (!PACKET_buf_init(&pkt, data, datalen) || !PACKET_get_net_4_len_u32(&pkt, &lms_type)
 ...
 */
static int lms_pubkey_from_pkt(LMS_KEY *lmskey, const unsigned char *pubkeydata, size_t publen)
{
    uint32_t     lms_type;
    uint32_t     ots_type;
    ...
    if (HASH_NOT_MATCHED(lmskey->ots_params, lmskey->lms_params)
        ...
        || !PACKET_get_bytes(&pkt, (const unsigned char **)&key->K, lmskey->lms_params->n))
        goto err;
 ...
 */
int ossl_lms_pubkey_decode(const unsigned char *pub, size_t publen, LMS_KEY *lmskey)
{
