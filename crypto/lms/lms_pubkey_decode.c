 ...
 */
static int
lms_pubkey_from_pkt(LMS_KEY* lmskey,
                    ...
                    size_t publen)
{
  ...
  if (lmskey->lms_params == NULL || !PACKET_get_net_4_len_u32(&pkt, &ots_type))
    goto err;
  ...
  if (HASH_NOT_MATCHED(lmskey->ots_params, lmskey->lms_params) ||
      !PACKET_get_bytes(&pkt, (const unsigned char**)&lmskey->Id, LMS_SIZE_I) ||
      !PACKET_get_bytes(
        &pkt, (const unsigned char**)&key->K, lmskey->lms_params->n))
    goto err;
 ...
 * invalid or |publen| is not the correct size (i.e. trailing data is not
 * allowed)
 */
int
ossl_lms_pubkey_decode(const unsigned char* pub, size_t publen, LMS_KEY* lmskey)
{
