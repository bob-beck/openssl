...
void* ossl_stored_namemap_new(OSSL_LIB_CTX* libctx) {
  ...
  if (namemap != NULL) namemap->stored = 1;
...
int ossl_namemap_empty(OSSL_NAMEMAP* namemap) {
  ...
  if (namemap == NULL) return 1;

  if (!CRYPTO_THREAD_read_lock(namemap->lock)) return -1;
...
int ossl_namemap_doall_names(const OSSL_NAMEMAP* namemap, int number,
  ...
  if (namemap == NULL || number <= 0) return 0;
  ...
  if (names != NULL) names = sk_OPENSSL_STRING_dup(names);
...
#ifndef FIPS_MODULE
  if (namemap == NULL) namemap = ossl_namemap_stored(NULL);
#endif
...
const char* ossl_namemap_num2name(const OSSL_NAMEMAP* namemap, int number,
  ...
  if (namemap == NULL || number <= 0) return NULL;

  if (!CRYPTO_THREAD_read_lock(namemap->lock)) return NULL;

  names = sk_NAMES_value(namemap->numnames, number - 1);
  if (names != NULL) ret = sk_OPENSSL_STRING_value(names, idx);
...
static int numname_insert(OSSL_NAMEMAP* namemap, int number, const char* name) {
  ...
  if ((tmpname = OPENSSL_strdup(name)) == NULL) goto err;

  if (!sk_OPENSSL_STRING_push(names, tmpname)) goto err;
  tmpname = NULL;

  if (number <= 0) {
    if (!sk_NAMES_push(namemap->numnames, names)) goto err;
    number = sk_NAMES_num(namemap->numnames);
  }
  return number;

err:
  if (number <= 0) sk_OPENSSL_STRING_pop_free(names, name_string_free);
...
static int namemap_add_name(OSSL_NAMEMAP* namemap, int number,
  ...
  if ((ret = ossl_namemap_name2num(namemap, name)) != 0) return ret;

  if ((number = numname_insert(namemap, number, name)) == 0) return 0;
  ...
  if (ret <= 0) {
    ...
    ERR_raise(ERR_LIB_CRYPTO,
              (ret < 0) ? CRYPTO_R_TOO_MANY_NAMES : ERR_R_INTERNAL_ERROR);
    return 0;
  }
  return number;
}
...
int ossl_namemap_add_name(OSSL_NAMEMAP* namemap, int number, const char* name) {
  ...
  if (name == NULL || *name == 0 || namemap == NULL) return 0;
...
static void get_legacy_evp_names(int base_nid, int nid, const char* pem_name,
  ...
  if (pem_name != NULL) num = ossl_namemap_add_name(arg, num, pem_name);
}
...
static void get_legacy_md_names(const OBJ_NAME* on, void* arg) {
  ...
  if (md != NULL) get_legacy_evp_names(0, EVP_MD_get_type(md), NULL, arg);
}
...
OSSL_NAMEMAP* ossl_namemap_new(OSSL_LIB_CTX* libctx) {
  ...
  if ((namemap = OPENSSL_zalloc(sizeof(*namemap))) == NULL) goto err;

  if ((namemap->lock = CRYPTO_THREAD_lock_new()) == NULL) goto err;

  if ((namemap->namenum_ht = ossl_ht_new(&htconf)) == NULL) goto err;

  if ((namemap->numnames = sk_NAMES_new_null()) == NULL) goto err;
...
void ossl_namemap_free(OSSL_NAMEMAP* namemap) {
  if (namemap == NULL || namemap->stored) return;
