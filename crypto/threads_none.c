...
int ossl_rcu_read_lock(CRYPTO_RCU_LOCK* lock) { return 1; }
...
__owur int CRYPTO_THREAD_read_lock(CRYPTO_RWLOCK* lock) {
  if (!ossl_assert(*(unsigned int*)lock == 1)) return 0;
  return 1;
}
...
int CRYPTO_THREAD_init_local(CRYPTO_THREAD_LOCAL* key, void (*cleanup)(void*)) {
  ...
  for (entry_idx = 0; entry_idx < OPENSSL_CRYPTO_THREAD_LOCAL_KEY_MAX;
       entry_idx++) {
    if (!thread_local_storage[entry_idx].used) break;
  }

  if (entry_idx == OPENSSL_CRYPTO_THREAD_LOCAL_KEY_MAX) return 0;
...
void* CRYPTO_THREAD_get_local(CRYPTO_THREAD_LOCAL* key) {
  if (*key >= OPENSSL_CRYPTO_THREAD_LOCAL_KEY_MAX) return NULL;

  return thread_local_storage[*key].data;
}

int CRYPTO_THREAD_set_local(CRYPTO_THREAD_LOCAL* key, void* val) {
  if (*key >= OPENSSL_CRYPTO_THREAD_LOCAL_KEY_MAX) return 0;
