...
int
ossl_pw_set_passphrase(struct ossl_passphrase_data_st* data,
...
int
ossl_pw_set_pem_password_cb(struct ossl_passphrase_data_st* data,
                            pem_password_cb* cb,
                            void* cbarg)
{
...
int
ossl_pw_set_ossl_passphrase_cb(struct ossl_passphrase_data_st* data,
...
int
ossl_pw_set_ui_method(struct ossl_passphrase_data_st* data,
                 ...
                 void* ui_data)
{
  ...
  prompt_idx =
    UI_add_input_string(
      ui, prompt, UI_INPUT_FLAG_DEFAULT_PWD, ipass, 0, (int)pass_size) -
    1;
  if (prompt_idx < 0) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_UI_LIB);
    goto end;
  ...
  if (verify) {
    ...
    if (vpass == NULL)
      goto end;
    verify_idx = UI_add_verify_string(ui,
                                      prompt,
...
int
ossl_pw_get_passphrase(char* pass,
                       size_t pass_size,
                       size_t* pass_len,
                       ...
                       struct ossl_passphrase_data_st* data)
{
  ...
  ret = do_ui_passphrase(
    pass, pass_size, pass_len, prompt_info, verify, ui_method, ui_data);
                     ...
                     const char* info)
{
  ...
  if (ossl_pw_get_passphrase(
        buf, (size_t)size, &password_len, params, rwflag, userdata))
...
int
ossl_pw_passphrase_callback_enc(char* pass,
                                size_t pass_size,
                                size_t* pass_len,
                                const OSSL_PARAM params[],
                                void* arg)
...
int
ossl_pw_passphrase_callback_dec(char* pass,
                                size_t pass_size,
                                size_t* pass_len,
                                const OSSL_PARAM params[],
                                void* arg)
