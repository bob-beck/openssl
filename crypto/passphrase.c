 ...
 */
static int do_ui_passphrase(char            *pass,
                            size_t           pass_size,
                            size_t          *pass_len,
                            ...
                            void            *ui_data)
{
    ...
    prompt_idx = UI_add_input_string(ui, prompt, UI_INPUT_FLAG_DEFAULT_PWD, ipass, 0, (int)pass_size) - 1;
    if (prompt_idx < 0) {
        ERR_raise(ERR_LIB_CRYPTO, ERR_R_UI_LIB);
        goto end;
    }
    ...
    if (verify) {
        ...
        if (vpass == NULL)
            goto end;
        verify_idx = UI_add_verify_string(ui, prompt, UI_INPUT_FLAG_DEFAULT_PWD, vpass, 0, (int)pass_size, ipass) - 1;
        if (verify_idx < 0) {
            ERR_raise(ERR_LIB_CRYPTO, ERR_R_UI_LIB);
            goto end;
        }
...
int ossl_pw_get_passphrase(char                           *pass,
                           size_t                          pass_size,
                           size_t                         *pass_len,
...
int ossl_pw_passphrase_callback_enc(char            *pass,
                                    size_t           pass_size,
                                    size_t          *pass_len,
                                    const OSSL_PARAM params[],
                                    void            *arg)
...
int ossl_pw_passphrase_callback_dec(char            *pass,
                                    size_t           pass_size,
                                    size_t          *pass_len,
                                    const OSSL_PARAM params[],
                                    void            *arg)
