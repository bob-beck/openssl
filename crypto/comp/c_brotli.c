...
typedef BROTLI_BOOL (
    *encode_oneshot_ft)(int, int, BrotliEncoderMode, size_t, const uint8_t in[], size_t *, uint8_t out[]);

typedef BrotliDecoderState *(*decode_init_ft)(brotli_alloc_func, brotli_free_func, void *);
typedef BROTLI_BOOL (
    *decode_stream_ft)(BrotliDecoderState *, size_t *, const uint8_t **, size_t *, uint8_t **, size_t *);
...
static ossl_ssize_t
brotli_stateful_compress_block(COMP_CTX *ctx, unsigned char *out, size_t olen, unsigned char *in, size_t ilen)
{
    BROTLI_BOOL          done;
    struct brotli_state *state     = ctx->data;
...
static ossl_ssize_t
brotli_stateful_expand_block(COMP_CTX *ctx, unsigned char *out, size_t olen, unsigned char *in, size_t ilen)
{
...
static ossl_ssize_t
brotli_oneshot_compress_block(COMP_CTX *ctx, unsigned char *out, size_t olen, unsigned char *in, size_t ilen)
{
...
static ossl_ssize_t
brotli_oneshot_expand_block(COMP_CTX *ctx, unsigned char *out, size_t olen, unsigned char *in, size_t ilen)
{
    size_t       out_size = olen;
    ossl_ssize_t ret;
...
static const BIO_METHOD bio_meth_brotli = {BIO_TYPE_COMP,
                                           "brotli",
...
# endif
    ...
    for (;;) {
        ...
        while (ctx->decode.avail_in > 0 || BrotliDecoderHasMoreOutput(ctx->decode.state)) {
            bret = BrotliDecoderDecompressStream(ctx->decode.state,
                                                 &ctx->decode.avail_in,
