...
typedef BROTLI_BOOL (*encode_oneshot_ft)(int,
                                         int,
                                         BrotliEncoderMode,
                                         size_t,
                                         const uint8_t in[],
...
typedef BrotliDecoderState* (*decode_init_ft)(brotli_alloc_func,
                                              brotli_free_func,
                                              void*);
typedef BROTLI_BOOL (*decode_stream_ft)(BrotliDecoderState*,
                                        size_t*,
                                        const uint8_t**,
                                        size_t*,
                                        uint8_t**,
...
typedef BrotliDecoderResult (*decode_oneshot_ft)(size_t,
                                                 const uint8_t in[],
                                                 size_t*,
                                                 uint8_t out[]);
...
static int
brotli_stateful_init(COMP_CTX* ctx)
{
...
static void
brotli_stateful_finish(COMP_CTX* ctx)
{
...
static ossl_ssize_t
brotli_stateful_compress_block(COMP_CTX* ctx,
                               unsigned char* out,
                               size_t olen,
                               unsigned char* in,
                               size_t ilen)
...
static ossl_ssize_t
brotli_stateful_expand_block(COMP_CTX* ctx,
                             unsigned char* out,
                             size_t olen,
                             unsigned char* in,
                             size_t ilen)
...
static int
brotli_oneshot_init(COMP_CTX* ctx)
{
  return 1;
}

static void
brotli_oneshot_finish(COMP_CTX* ctx)
{
}

static ossl_ssize_t
brotli_oneshot_compress_block(COMP_CTX* ctx,
                              unsigned char* out,
                              size_t olen,
                              unsigned char* in,
                              size_t ilen)
...
static ossl_ssize_t
brotli_oneshot_expand_block(COMP_CTX* ctx,
                            unsigned char* out,
                            size_t olen,
                            unsigned char* in,
                            size_t ilen)
...
static int
bio_brotli_new(BIO* bi);
static int
bio_brotli_free(BIO* bi);
static int
bio_brotli_read(BIO* b, char* out, int outl);
static int
bio_brotli_write(BIO* b, const char* in, int inl);
static long
bio_brotli_ctrl(BIO* b, int cmd, long num, void* ptr);
static long
bio_brotli_callback_ctrl(BIO* b, int cmd, BIO_info_cb* fp);
...
bio_brotli_read(BIO* b, char* out, int outl)
{
  ...
  for (;;) {
    ...
    while (ctx->decode.avail_in > 0 ||
      ...
      bret =
        BrotliDecoderDecompressStream(ctx->decode.state,
                                      &ctx->decode.avail_in,
                                      (const uint8_t**)&ctx->decode.next_in,
                                      &ctx->decode.avail_out,
                                      &ctx->decode.next_out,
                                      NULL);
...
bio_brotli_write(BIO* b, const char* in, int inl)
{
  ...
  for (;;) {
    ...
    ctx->encode.avail_out = ctx->encode.bufsize;
    /* Compress some more */
    brret = BrotliEncoderCompressStream(ctx->encode.state,
                                        BROTLI_OPERATION_FLUSH,
                                        &ctx->encode.avail_in,
                                        (const uint8_t**)&ctx->encode.next_in,
                                        &ctx->encode.avail_out,
                                        &ctx->encode.next_out,
                                        NULL);
