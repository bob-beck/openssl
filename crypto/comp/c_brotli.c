...
static ossl_ssize_t brotli_stateful_compress_block(COMP_CTX* ctx,
                                                   unsigned char* out,
                                                   size_t olen,
                                                   unsigned char* in,
                                                   size_t ilen) {
  ...
  if (state == NULL || olen > OSSL_SSIZE_MAX) return -1;
...
static ossl_ssize_t brotli_oneshot_compress_block(COMP_CTX* ctx,
                                                  unsigned char* out,
                                                  size_t olen,
                                                  unsigned char* in,
                                                  size_t ilen) {
...
#ifndef OPENSSL_NO_BROTLI
  if (RUN_ONCE(&brotli_once, ossl_comp_brotli_init)) return &bio_meth_brotli;
#endif
  return NULL;
}
...
static int bio_brotli_read(BIO* b, char* out, int outl) {
  ...
  for (;;) {
    ...
    /* If EOF */
    if (BrotliDecoderIsFinished(ctx->decode.state)) return 0;
