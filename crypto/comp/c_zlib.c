...
static ossl_ssize_t
zlib_stateful_compress_block(COMP_CTX* ctx,
                             unsigned char* out,
                             size_t olen,
                             unsigned char* in,
                             size_t ilen);
static ossl_ssize_t
zlib_stateful_expand_block(COMP_CTX* ctx,
                           unsigned char* out,
                           size_t olen,
                           unsigned char* in,
                           size_t ilen);
...
/* Function pointers */
typedef int (*compress_ft)(Bytef* dest,
                           uLongf* destLen,
                           const Bytef* source,
                           uLong sourceLen);
typedef int (*uncompress_ft)(Bytef* dest,
                             uLongf* destLen,
                             const Bytef* source,
                             uLong sourceLen);
...
typedef int (*deflateInit__ft)(z_streamp strm,
                               int level,
                               const char* version,
                               int stream_size);
...
zlib_stateful_init(COMP_CTX* ctx)
{
  ...
  err = deflateInit_(
    &state->ostream, Z_DEFAULT_COMPRESSION, ZLIB_VERSION, sizeof(z_stream));
  if (err != Z_OK)
    goto err;
...
static int
zlib_oneshot_init(COMP_CTX* ctx)
{
  return 1;
}

static void
zlib_oneshot_finish(COMP_CTX* ctx)
{
}

static ossl_ssize_t
zlib_oneshot_compress_block(COMP_CTX* ctx,
                            unsigned char* out,
                            size_t olen,
                            unsigned char* in,
                            size_t ilen)
...
static ossl_ssize_t
zlib_oneshot_expand_block(COMP_CTX* ctx,
                          unsigned char* out,
                          size_t olen,
                          unsigned char* in,
                          size_t ilen)
...
bio_zlib_read(BIO* b, char* out, int outl)
{
  ...
  if (!ctx->ibuf) {
    ...
    if ((ret = inflateInit(zin)) != Z_OK) {
      ERR_raise_data(
        ERR_LIB_COMP, COMP_R_ZLIB_INFLATE_ERROR, "zlib error: %s", zError(ret));
      return 0;
    }
...
bio_zlib_write(BIO* b, const char* in, int inl)
{
  ...
  if (!ctx->obuf) {
    ...
    if ((ret = deflateInit(zout, ctx->comp_level)) != Z_OK) {
      ERR_raise_data(
        ERR_LIB_COMP, COMP_R_ZLIB_DEFLATE_ERROR, "zlib error: %s", zError(ret));
      return 0;
    }
