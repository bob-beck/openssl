...
static ossl_ssize_t zlib_stateful_compress_block(COMP_CTX* ctx,
  ...
  if (ilen > 0) err = deflate(&state->ostream, Z_SYNC_FLUSH);
  if (err != Z_OK) return -1;
  if (state->ostream.avail_out > olen) return -1;
...
static ossl_ssize_t zlib_stateful_expand_block(COMP_CTX* ctx,
  ...
  if (ilen > 0) err = inflate(&state->istream, Z_SYNC_FLUSH);
  if (err != Z_OK) return -1;
  if (state->istream.avail_out > olen) return -1;
...
static ossl_ssize_t zlib_oneshot_compress_block(COMP_CTX* ctx,
  ...
  if (olen > ULONG_MAX) return -1;
  out_size = (uLongf)olen;

  if (compress(out, &out_size, in, ilen) != Z_OK) return -1;

  if (out_size > OSSL_SSIZE_MAX) return -1;
...
static ossl_ssize_t zlib_oneshot_expand_block(COMP_CTX* ctx, unsigned char* out,
  ...
  if (uncompress(out, &out_size, in, ilen) != Z_OK) return -1;
...
COMP_METHOD* COMP_zlib(void) {
  ...
  if (RUN_ONCE(&zlib_once, ossl_comp_zlib_init)) meth = &zlib_stateful_method;
#endif
...
#ifndef OPENSSL_NO_ZLIB
  if (RUN_ONCE(&zlib_once, ossl_comp_zlib_init)) meth = &zlib_oneshot_method;
#endif
...
#ifndef OPENSSL_NO_ZLIB
  if (RUN_ONCE(&zlib_once, ossl_comp_zlib_init)) return &bio_meth_zlib;
#endif
  return NULL;
}
  ...
  switch (cmd) {
    ...
    case BIO_CTRL_WPENDING:
      ...
      if (ctx->odone) {
        ...
        ret = ctx->ocount;
        if (ret == 0) /* Unknown amount pending but we are not finished */
          ret = 1;
      }
      if (ret == 0) ret = BIO_ctrl(next, cmd, num, ptr);
      break;
