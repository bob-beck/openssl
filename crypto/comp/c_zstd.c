...
static ossl_ssize_t
zstd_stateful_compress_block (COMP_CTX *ctx, unsigned char *out, size_t olen,
...
static ossl_ssize_t
zstd_stateful_expand_block (COMP_CTX *ctx, unsigned char *out, size_t olen,
                            unsigned char *in, size_t ilen)
{
  ZSTD_inBuffer inbuf;
  ZSTD_outBuffer outbuf;
...
static ossl_ssize_t
zstd_oneshot_compress_block (COMP_CTX *ctx, unsigned char *out, size_t olen,
                             unsigned char *in, size_t ilen)
{
  size_t out_size;
  ossl_ssize_t ret;
...
static ossl_ssize_t
zstd_oneshot_expand_block (COMP_CTX *ctx, unsigned char *out, size_t olen,
                           unsigned char *in, size_t ilen)
{
  size_t out_size;
  ossl_ssize_t ret;
...
#define LIBZSTD "zstd"
  ...
  if (p_createCStream == NULL || p_initCStream == NULL || p_freeCStream == NULL
      || p_compressStream2 == NULL || p_flushStream == NULL
      || p_endStream == NULL || p_compress == NULL || p_createDStream == NULL
