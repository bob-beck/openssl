...
#endif
  if (state->compressor == NULL) goto err;
...
#endif
  if (state->decompressor == NULL) goto err;
...
static ossl_ssize_t zstd_stateful_compress_block(COMP_CTX* ctx,
  ...
  if (outbuf.pos > OSSL_SSIZE_MAX) return -1;
...
static ossl_ssize_t zstd_oneshot_compress_block(COMP_CTX* ctx,
  ...
  if (ZSTD_isError(out_size)) return -1;
...
COMP_METHOD* COMP_zstd(void) {
  ...
  if (RUN_ONCE(&zstd_once, ossl_comp_zstd_init)) meth = &zstd_stateful_method;
#endif
  return meth;
}
...
#ifndef OPENSSL_NO_ZSTD
  if (RUN_ONCE(&zstd_once, ossl_comp_zstd_init)) meth = &zstd_oneshot_method;
#endif
  return meth;
}
...
#ifndef OPENSSL_NO_ZSTD
  if (RUN_ONCE(&zstd_once, ossl_comp_zstd_init)) return &bio_meth_zstd;
#endif
  return NULL;
}
...
#endif
  if (ctx->decompress.state == NULL) goto err;
...
#endif
  if (ctx->compress.state == NULL) goto err;
...
static int bio_zstd_read(BIO* b, char* out, int outl) {
  ...
  for (;;) {
    ...
    do {
      ...
      if (outBuf.pos == outBuf.size) return (int)outBuf.pos;
    ...
    if (ret <= 0) {
      ...
      if (ret < 0 && outBuf.pos == 0) return ret;
...
static int bio_zstd_write(BIO* b, const char* in, int inl) {
  ...
  for (;;) {
    ...
    while (ctx->compress.write_pos < ctx->compress.outbuf.pos) {
      ...
      if (ret <= 0) {
        BIO_copy_next_retry(b);
        if (ret < 0 && inBuf.pos == 0) return ret;
...
static int bio_zstd_flush(BIO* b) {
  ...
  if (ctx->compress.outbuf.dst == NULL) return 1;
