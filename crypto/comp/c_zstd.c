...
typedef size_t (*compressStream2_ft)(ZSTD_CCtx*,
                                     ZSTD_outBuffer*,
                                     ZSTD_inBuffer*,
                                     ZSTD_EndDirective);
...
typedef size_t (*decompressStream_ft)(ZSTD_DStream*,
                                      ZSTD_outBuffer*,
                                      ZSTD_inBuffer*);
...
static int
zstd_stateful_init(COMP_CTX* ctx)
{
...
static void
zstd_stateful_finish(COMP_CTX* ctx)
{
...
static ossl_ssize_t
zstd_stateful_compress_block(COMP_CTX* ctx,
                             unsigned char* out,
                             size_t olen,
                             unsigned char* in,
                             size_t ilen)
...
static ossl_ssize_t
zstd_stateful_expand_block(COMP_CTX* ctx,
                           unsigned char* out,
                           size_t olen,
                           unsigned char* in,
                           size_t ilen)
...
static int
zstd_oneshot_init(COMP_CTX* ctx)
{
  return 1;
}

static void
zstd_oneshot_finish(COMP_CTX* ctx)
{
}

static ossl_ssize_t
zstd_oneshot_compress_block(COMP_CTX* ctx,
                            unsigned char* out,
                            size_t olen,
                            unsigned char* in,
                            size_t ilen)
...
static ossl_ssize_t
zstd_oneshot_expand_block(COMP_CTX* ctx,
                          unsigned char* out,
                          size_t olen,
                          unsigned char* in,
                          size_t ilen)
...
bio_zstd_new(BIO* bi);
static int
bio_zstd_free(BIO* bi);
static int
bio_zstd_read(BIO* b, char* out, int outl);
static int
bio_zstd_write(BIO* b, const char* in, int inl);
static long
bio_zstd_ctrl(BIO* b, int cmd, long num, void* ptr);
static long
bio_zstd_callback_ctrl(BIO* b, int cmd, BIO_info_cb* fp);
  ...
  for (;;) {
    ...
    do {
      zret = ZSTD_decompressStream(
        ctx->decompress.state, &outBuf, &ctx->decompress.inbuf);
...
bio_zstd_write(BIO* b, const char* in, int inl)
{
  ...
  for (;;) {
    ...
    while (ctx->compress.write_pos < ctx->compress.outbuf.pos) {
      ret = BIO_write(
        next,
        (unsigned char*)ctx->compress.outbuf.dst + ctx->compress.write_pos,
    ...
    ctx->compress.write_pos = 0;
    /* Compress some more */
    zret = ZSTD_compressStream2(
      ctx->compress.state, &ctx->compress.outbuf, &inBuf, ZSTD_e_end);
    if (ZSTD_isError(zret)) {
