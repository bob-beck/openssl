...
static void
ssl_module_free (CONF_IMODULE *md)
{
  size_t i, j;
  if (ssl_names == NULL)
    return;
...
static int
ssl_module_init (CONF_IMODULE *md, const CONF *cnf)
{
  size_t i, j, cnt;
  int rv = 0;
 ...
 */
const SSL_CONF_CMD *
conf_ssl_get (size_t idx, const char **name, size_t *cnt)
{
 ...
 */
int
conf_ssl_name_find (const char *name, size_t *idx)
{
  size_t i;
...
/*
 * Given a command set |cmd|, return details on the command at index |idx|
 * which must be less than the number of commands in the set (as returned by
 * conf_ssl_get). The name of the command will be returned in |*cmdstr| and the
 * argument is returned in |*arg|.
 */
void
conf_ssl_get_cmd (const SSL_CONF_CMD *cmd, size_t idx, char **cmdstr,
                  char **arg)
{
  *cmdstr = cmd[idx].cmd;
  *arg = cmd[idx].arg;
...
void
ossl_config_add_ssl_module (void)
{
