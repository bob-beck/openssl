...
static int
str_copy(CONF* conf, char* section, char** to, char* from);
static char*
...
static int
def_load(CONF* conf, const char* name, long* eline);
static int
def_load_bio(CONF* conf, BIO* bp, long* eline);
static int
def_dump(const CONF* conf, BIO* bp);
static int
...
process_include(char* include, OPENSSL_DIR_CTX** dirctx, char** dirpath)
{
  ...
  if (S_ISDIR(st.st_mode)) {
    if (*dirctx != NULL) {
      ERR_raise_data(
        ERR_LIB_CONF, CONF_R_RECURSIVE_DIRECTORY_INCLUDE, "%s", include);
      return NULL;
    }
