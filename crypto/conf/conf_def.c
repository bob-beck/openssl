...
#define CONFBUFSIZE 512
  ...
  for (;;) {
    ...
    *p = '\0';
  read_retry:
    if (in != NULL && BIO_gets(in, p, CONFBUFSIZE - 1) < 0) goto err;
    p[CONFBUFSIZE - 1] = '\0';
    ii = i = (int)strlen(p);
    if (first_call) {
    ...
    again:
      ...
      if (CHECK_AND_SKIP_PREFIX(pname, ".pragma") &&
        ...
        *pval++ = '\0';
        ...
        if (strcmp(p, "dollarid") == 0) {
          if (!parsebool(pval, &conf->flag_dollarid)) goto err;
        } else if (strcmp(p, "abspath") == 0) {
          if (!parsebool(pval, &conf->flag_abspath)) goto err;
        } else if (strcmp(p, "includedir") == 0) {
          OPENSSL_free(conf->includedir);
          if ((conf->includedir = OPENSSL_strdup(pval)) == NULL) goto err;
        }
         ...
         */
        if (numincludes++ > 10) goto err;
#endif

        if (include_dir == NULL) include_dir = conf->includedir;
        ...
        trim_ws(conf, p);
        if (!str_copy(conf, psection, &include, p)) goto err;
      ...
      *end = '\0';
      ...
      if ((v = OPENSSL_malloc(sizeof(*v))) == NULL) goto err;
      v->name = OPENSSL_strdup(pname);
      v->value = NULL;
      if (v->name == NULL) goto err;
...
#ifndef OPENSSL_NO_POSIX_IO
  OPENSSL_free(dirpath);
  if (dirctx != NULL) OPENSSL_DIR_end(&dirctx);
#endif
...
static int str_copy(CONF* conf, char* section, char** pto, char* from) {
  ...
  if (!BUF_MEM_grow(buf, strlen(from) + 1)) goto err;
...
static BIO* process_include(char* include, OPENSSL_DIR_CTX** dirctx,
  ...
  if (S_ISDIR(st.st_mode)) {
    ...
    if ((next = get_next_file(include, dirctx)) != NULL) *dirpath = include;
    return next;
  }
