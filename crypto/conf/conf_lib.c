...
void CONF_set_nconf(CONF* conf, LHASH_OF(CONF_VALUE)* hash) {
  if (default_CONF_method == NULL) default_CONF_method = NCONF_default();
...
CONF* NCONF_new_ex(OSSL_LIB_CTX* libctx, CONF_METHOD* meth) {
  ...
  if (meth == NULL) meth = NCONF_default();
...
STACK_OF(OPENSSL_CSTRING)* NCONF_get_section_names(const CONF* cnf) {
  ...
  if ((names = sk_OPENSSL_CSTRING_new(section_name_cmp)) == NULL) return NULL;
...
int NCONF_get_number_e(const CONF* conf, const char* group, const char* name,
  ...
  if (conf != NULL) {
    if (conf->meth->is_number != NULL) is_number = conf->meth->is_number;
    if (conf->meth->to_int != NULL) to_int = conf->meth->to_int;
  }
