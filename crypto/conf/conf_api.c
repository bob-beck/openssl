...
int _CONF_add_string(CONF* conf, CONF_VALUE* section, CONF_VALUE* value) {
  ...
  if (!sk_CONF_VALUE_push(ts, value)) return 0;
...
char* _CONF_get_string(const CONF* conf, const char* section,
  ...
  if (conf == NULL) return ossl_safe_getenv(name);
  if (conf->data == NULL) return NULL;
  if (section != NULL) {
    vv.name = (char*)name;
    vv.section = (char*)section;
...
static int conf_value_cmp(const CONF_VALUE* a, const CONF_VALUE* b) {
  ...
  if (a->name != NULL && b->name != NULL) return strcmp(a->name, b->name);
...
static void value_free_hash(const CONF_VALUE* a, LHASH_OF(CONF_VALUE)* conf) {
  if (a->name != NULL) (void)lh_CONF_VALUE_delete(conf, a);
}
...
CONF_VALUE* _CONF_new_section(CONF* conf, const char* section) {
  ...
  if ((sk = sk_CONF_VALUE_new_null()) == NULL) goto err;
  if ((v = OPENSSL_malloc(sizeof(*v))) == NULL) goto err;
  i = (int)strlen(section) + 1;
  if ((v->section = OPENSSL_malloc(i)) == NULL) goto err;
  ...
  if (vv != NULL || lh_CONF_VALUE_error(conf->data) > 0) goto err;
  return v;

err:
  sk_CONF_VALUE_free(sk);
  if (v != NULL) OPENSSL_free(v->section);
  OPENSSL_free(v);
  return NULL;
}
