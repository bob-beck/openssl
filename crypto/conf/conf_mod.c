...
static int
module_init(CONF_MODULE* pmod,
            const char* name,
            const char* value,
            const CONF* cnf);
static CONF_MODULE*
module_load_dso(const CONF* cnf, const char* name, const char* value);

static int
conf_modules_finish_int(void);

static void
module_lists_free(void)
{
...
static int
conf_diagnostics(const CONF* cnf)
{
  int status;
  long result = 0;
...
int
CONF_modules_load_file_ex(OSSL_LIB_CTX* libctx,
                          const char* filename,
                          const char* appname,
                          unsigned long flags)
{
...
int
CONF_modules_load_file(const char* filename,
                       const char* appname,
                       unsigned long flags)
{
           ...
           unsigned long flags)
{
  ...
  if (!md) {
    if (!(flags & CONF_MFLAGS_SILENT)) {
      ERR_raise_data(
        ERR_LIB_CONF, CONF_R_UNKNOWN_MODULE_NAME, "module=%s", name);
    }
    return -1;
  }
...
int
CONF_module_add(const char* name,
                conf_init_func* ifunc,
                conf_finish_func* ffunc)
{
...
int
CONF_parse_list(const char* list_,
                int sep,
                int nospc,
