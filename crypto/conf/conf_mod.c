...
int CONF_modules_load(const CONF* cnf, const char* appname,
  ...
  ERR_set_mark();
  if (appname) vsection = NCONF_get_string(cnf, NULL, appname);
...
err:
  if (filename == NULL) OPENSSL_free(file);
  NCONF_free(conf);

  if ((flags & CONF_MFLAGS_IGNORE_RETURN_CODES) != 0 && !diagnostics) ret = 1;
...
static int module_run(const CONF* cnf, const char* name, const char* value,
  ...
  if (!RUN_ONCE(&load_builtin_modules, do_load_builtin_modules)) return -1;
...
static CONF_MODULE* module_add(DSO* dso, const char* name,
  ...
  if (!RUN_ONCE(&init_module_list_lock, do_init_module_list_lock)) return NULL;
  ...
  if (new_modules == NULL) goto err;

  if ((tmod = OPENSSL_zalloc(sizeof(*tmod))) == NULL) goto err;

  tmod->dso = dso;
  tmod->name = OPENSSL_strdup(name);
  tmod->init = ifunc;
  tmod->finish = ffunc;
  if (tmod->name == NULL) goto err;

  if (!sk_CONF_MODULE_push(new_modules, tmod)) goto err;
...
static CONF_MODULE* module_find(const char* name) {
  ...
  if (!ossl_rcu_read_lock(module_list_lock)) return NULL;
...
static int module_init(CONF_MODULE* pmod, const char* name, const char* value,
  ...
  if (!imod->name || !imod->value) goto memerr;
  ...
  if (!RUN_ONCE(&init_module_list_lock, do_init_module_list_lock)) goto err;
...
err:
  ...
  if (pmod->finish && init_called) pmod->finish(imod);
...
void CONF_modules_unload(int all) {
  ...
  for (i = sk_CONF_MODULE_num(new_modules) - 1; i >= 0; i--) {
    ...
    if (((md->links > 0) || !md->dso) && !all) continue;
...
static int conf_modules_finish_int(void) {
  ...
  if (!RUN_ONCE(&init_module_list_lock, do_init_module_list_lock)) return 0;
...
static void module_finish(CONF_IMODULE* imod) {
  if (!imod) return;
  if (imod->pmod->finish) imod->pmod->finish(imod);
  imod->pmod->links--;
  OPENSSL_free(imod->name);
  OPENSSL_free(imod->value);
  OPENSSL_free(imod);
...
char* CONF_get1_default_config_file(void) {
   ...
   * empty config file
   */
  if (t == NULL) return OPENSSL_strdup("");
