...
static STACK_OF (CONF_MODULE) *supported_modules
...
int
CONF_modules_load_file_ex (OSSL_LIB_CTX *libctx, const char *filename,
...
static int
module_run (const CONF *cnf, const char *name, const char *value,
            unsigned long flags)
{
  CONF_MODULE *md;
  int ret;
...
/* initialize a module */
static int
module_init (CONF_MODULE *pmod, const char *name, const char *value,
             const CONF *cnf)
{
  int ret = 1;
  int init_called = 0;
