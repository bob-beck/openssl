 ...
 */
RAND_POOL *
ossl_rand_pool_new (int entropy_requested, int secure, size_t min_len,
                    size_t max_len)
{
 ...
 */
RAND_POOL *
ossl_rand_pool_attach (const unsigned char *buffer, size_t len, size_t entropy)
{
  RAND_POOL *pool = OPENSSL_zalloc (sizeof (*pool));
 ...
 */
void
ossl_rand_pool_free (RAND_POOL *pool)
{
  if (pool == NULL)
    return;
 ...
 */
const unsigned char *
ossl_rand_pool_buffer (RAND_POOL *pool)
{
  return pool->buffer;
}
 ...
 */
size_t
ossl_rand_pool_entropy (RAND_POOL *pool)
{
  return pool->entropy;
}
 ...
 */
size_t
ossl_rand_pool_length (RAND_POOL *pool)
{
  return pool->len;
}
 ...
 */
unsigned char *
ossl_rand_pool_detach (RAND_POOL *pool)
{
 ...
 */
void
ossl_rand_pool_reattach (RAND_POOL *pool, unsigned char *buffer)
{
  pool->buffer = buffer;
 ...
 */
size_t
ossl_rand_pool_entropy_available (RAND_POOL *pool)
{
...
size_t
ossl_rand_pool_entropy_needed (RAND_POOL *pool)
{
  if (pool->entropy < pool->entropy_requested)
...
static int
rand_pool_grow (RAND_POOL *pool, size_t len)
{
...
size_t
ossl_rand_pool_bytes_needed (RAND_POOL *pool, unsigned int entropy_factor)
{
  size_t bytes_needed;
...
size_t
ossl_rand_pool_bytes_remaining (RAND_POOL *pool)
{
 ...
 */
int
ossl_rand_pool_add (RAND_POOL *pool, const unsigned char *buffer, size_t len,
                    size_t entropy)
{
 ...
 */
unsigned char *
ossl_rand_pool_add_begin (RAND_POOL *pool, size_t len)
{
  if (len == 0)
    return NULL;
 ...
 */
int
ossl_rand_pool_add_end (RAND_POOL *pool, size_t len, size_t entropy)
{
  if (len > pool->alloc_len - pool->len)
    {
      ERR_raise (ERR_LIB_RAND, RAND_R_RANDOM_POOL_OVERFLOW);
...
int
ossl_rand_pool_adin_mix_in (RAND_POOL *pool, const unsigned char *adin,
                            size_t adin_len)
{
