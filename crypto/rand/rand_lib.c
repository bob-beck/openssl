...
static int
set_random_provider_name (RAND_GLOBAL *dgbl, const char *name)
{
 ...
 */
void
RAND_keep_random_devices_open (int keep)
{
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
static int
rand_set_rand_method_internal (const RAND_METHOD *meth, ossl_unused ENGINE *e)
{
  if (!RUN_ONCE (&rand_init, do_rand_init))
    return 0;
 ...
 */
int
RAND_priv_bytes_ex (OSSL_LIB_CTX *ctx, unsigned char *buf, size_t num,
...
int
RAND_priv_bytes (unsigned char *buf, int num)
{
  if (num < 0)
    return 0;
...
int
RAND_bytes_ex (OSSL_LIB_CTX *ctx, unsigned char *buf, size_t num,
               unsigned int strength)
{
  RAND_GLOBAL *dgbl;
  EVP_RAND_CTX *rand;
 ...
 */
void *
ossl_rand_ctx_new (OSSL_LIB_CTX *libctx)
{
...
static void
rand_delete_thread_state (void *arg)
{
...
static EVP_RAND_CTX *
rand_new_seed (OSSL_LIB_CTX *libctx)
{
  EVP_RAND *rand;
  const char *propq;
  char *name;
...
#ifndef FIPS_MODULE
EVP_RAND_CTX *
ossl_rand_get0_seed_noncreating (OSSL_LIB_CTX *ctx)
{
  RAND_GLOBAL *dgbl = rand_get_global (ctx);
  EVP_RAND_CTX *ret;
...
static EVP_RAND_CTX *
rand_new_drbg (OSSL_LIB_CTX *libctx, EVP_RAND_CTX *parent,
...
static EVP_RAND_CTX *
rand_new_crngt (OSSL_LIB_CTX *libctx, EVP_RAND_CTX *parent)
{
  EVP_RAND *rand;
  EVP_RAND_CTX *ctx;
 ...
 */
static EVP_RAND_CTX *
rand_get0_primary (OSSL_LIB_CTX *ctx, RAND_GLOBAL *dgbl)
{
...
static EVP_RAND_CTX *
rand_get0_public (OSSL_LIB_CTX *ctx, RAND_GLOBAL *dgbl)
{
  ...
  if (rand == NULL)
    {
      ...
      if (!CRYPTO_THREAD_set_local_ex (CRYPTO_THREAD_LOCAL_DRBG_PUB_KEY, ctx,
                                       rand))
        {
          EVP_RAND_CTX_free (rand);
          rand = NULL;
...
static EVP_RAND_CTX *
rand_get0_private (OSSL_LIB_CTX *ctx, RAND_GLOBAL *dgbl)
{
  EVP_RAND_CTX *rand, *primary;
  OSSL_LIB_CTX *origctx = ctx;
  ...
  if (rand == NULL)
    {
      ...
      rand = rand_new_drbg (ctx, primary, SECONDARY_RESEED_INTERVAL,
                            SECONDARY_RESEED_TIME_INTERVAL);
      if (!CRYPTO_THREAD_set_local_ex (CRYPTO_THREAD_LOCAL_DRBG_PRIV_KEY, ctx,
                                       rand))
        {
          EVP_RAND_CTX_free (rand);
          rand = NULL;
 ...
 */
EVP_RAND_CTX *
RAND_get0_private (OSSL_LIB_CTX *ctx)
{
  RAND_GLOBAL *dgbl = rand_get_global (ctx);
...
#ifdef FIPS_MODULE
EVP_RAND_CTX *
ossl_rand_get0_private_noncreating (OSSL_LIB_CTX *ctx)
{
  RAND_GLOBAL *dgbl = rand_get_global (ctx);
...
int
RAND_set0_public (OSSL_LIB_CTX *ctx, EVP_RAND_CTX *rand)
{
  RAND_GLOBAL *dgbl = rand_get_global (ctx);
  EVP_RAND_CTX *old;
  int r;
...
int
RAND_set0_private (OSSL_LIB_CTX *ctx, EVP_RAND_CTX *rand)
{
  RAND_GLOBAL *dgbl = rand_get_global (ctx);
  EVP_RAND_CTX *old;
  int r;
...
#ifndef FIPS_MODULE
static int
random_set_string (char **p, const char *s)
{
  char *d = NULL;
 ...
 */
static int
random_conf_init (CONF_IMODULE *md, const CONF *cnf)
{
          ...
          if (prov != NULL)
            {
               ...
               * freed. The provider load code, conversely, checks the provider
               * name and re-hooks our reference if required.  This means that
               * a load, hook random provider, use, unload, reload, reuse
...
static void
random_conf_deinit (CONF_IMODULE *md)
{
  OSSL_TRACE (CONF, "Cleaned up random\n");
}

void
ossl_random_add_conf_module (void)
{
...
int
RAND_set_DRBG_type (OSSL_LIB_CTX *ctx, const char *drbg, const char *propq,
...
int
RAND_set_seed_source_type (OSSL_LIB_CTX *ctx, const char *seed,
                           const char *propq)
{
  RAND_GLOBAL *dgbl = rand_get_global (ctx);
...
int
RAND_set1_random_provider (OSSL_LIB_CTX *ctx, OSSL_PROVIDER *prov)
{
  RAND_GLOBAL *dgbl = rand_get_global (ctx);
 ...
 */
int
ossl_rand_check_random_provider_on_load (OSSL_LIB_CTX *ctx,
                                         OSSL_PROVIDER *prov)
{
  RAND_GLOBAL *dgbl = rand_get_global (ctx);
 ...
 */
int
ossl_rand_check_random_provider_on_unload (OSSL_LIB_CTX *ctx,
                                           OSSL_PROVIDER *prov)
{
  RAND_GLOBAL *dgbl = rand_get_global (ctx);
