...
DEFINE_RUN_ONCE_STATIC(do_rand_init) {
  ...
  if (rand_engine_lock == NULL) return 0;
#endif /* !OPENSSL_NO_ENGINE */

#ifndef OPENSSL_NO_DEPRECATED_3_0
  rand_meth_lock = CRYPTO_THREAD_lock_new();
  if (rand_meth_lock == NULL) goto err;
#endif /* !OPENSSL_NO_DEPRECATED_3_0 */

  if (!ossl_rand_pool_init()) goto err;
...
void ossl_rand_cleanup_int(void) {
  ...
  if (!rand_inited) return;

  if (meth != NULL && meth->cleanup != NULL) meth->cleanup();
...
int RAND_poll(void) {
  ...
  if (!ret) {
    ...
    if (ossl_pool_acquire_entropy(pool) == 0) goto err;

    if (meth->add == NULL ||
        meth->add(ossl_rand_pool_buffer(pool), (int)ossl_rand_pool_length(pool),
...
const RAND_METHOD* RAND_get_rand_method(void) {
  ...
  if (!RUN_ONCE(&rand_init, do_rand_init)) return NULL;

  if (rand_meth_lock == NULL) return NULL;

  if (!CRYPTO_THREAD_read_lock(rand_meth_lock)) return NULL;
  tmp_meth = default_RAND_meth;
  CRYPTO_THREAD_unlock(rand_meth_lock);
  if (tmp_meth != NULL) return tmp_meth;

  if (!CRYPTO_THREAD_write_lock(rand_meth_lock)) return NULL;
...
void RAND_seed(const void* buf, int num) {
  ...
  if (drbg != NULL && num > 0) EVP_RAND_reseed(drbg, 0, NULL, 0, buf, num);
}
...
int RAND_status(void) {
  ...
  if ((rand = RAND_get0_primary(NULL)) == NULL) return 0;
...
int RAND_priv_bytes_ex(OSSL_LIB_CTX* ctx, unsigned char* buf, size_t num,
  ...
  if (meth != NULL && meth != RAND_OpenSSL()) {
    ...
    if (meth->bytes != NULL) return meth->bytes(buf, (int)num);
    ERR_raise(ERR_LIB_RAND, RAND_R_FUNC_NOT_IMPLEMENTED);
    return -1;
  }
#endif
...
void* ossl_rand_ctx_new(OSSL_LIB_CTX* libctx) {
  ...
  if (dgbl->random_provider_name == NULL) goto err0;
#endif

  dgbl->lock = CRYPTO_THREAD_lock_new();
  if (dgbl->lock == NULL) goto err1;
...
EVP_RAND_CTX* ossl_rand_get0_seed_noncreating(OSSL_LIB_CTX* ctx) {
  ...
  if (!CRYPTO_THREAD_read_lock(dgbl->lock)) return NULL;
  ret = dgbl->seed;
...
static EVP_RAND_CTX* rand_get0_primary(OSSL_LIB_CTX* ctx, RAND_GLOBAL* dgbl) {
  ...
  if (ret == NULL || !EVP_RAND_enable_locking(ret)) {
    ...
    if (newseed == NULL) return NULL;
    /* else carry on and store seed */
    ret = NULL;
  }

  if (!CRYPTO_THREAD_write_lock(dgbl->lock)) return NULL;
  ...
  if (newseed != NULL) dgbl->seed = newseed;
  dgbl->primary = ret;
  CRYPTO_THREAD_unlock(dgbl->lock);
...
static int random_conf_init(CONF_IMODULE* md, const CONF* cnf) {
  ...
  for (i = 0; i < sk_CONF_VALUE_num(elist); i++) {
    ...
    if (OPENSSL_strcasecmp(cval->name, "random") == 0) {
      if (!random_set_string(&dgbl->rng_name, cval->value)) return 0;
    } else if (OPENSSL_strcasecmp(cval->name, "cipher") == 0) {
      if (!random_set_string(&dgbl->rng_cipher, cval->value)) return 0;
    } else if (OPENSSL_strcasecmp(cval->name, "digest") == 0) {
      if (!random_set_string(&dgbl->rng_digest, cval->value)) return 0;
    } else if (OPENSSL_strcasecmp(cval->name, "properties") == 0) {
      if (!random_set_string(&dgbl->rng_propq, cval->value)) return 0;
    } else if (OPENSSL_strcasecmp(cval->name, "seed") == 0) {
      if (!random_set_string(&dgbl->seed_name, cval->value)) return 0;
    } else if (OPENSSL_strcasecmp(cval->name, "seed_properties") == 0) {
      if (!random_set_string(&dgbl->seed_propq, cval->value)) return 0;
...
int RAND_set1_random_provider(OSSL_LIB_CTX* ctx, OSSL_PROVIDER* prov) {
  ...
  if (dgbl->random_provider == prov) return 1;

  if (!set_random_provider_name(dgbl, OSSL_PROVIDER_get0_name(prov))) return 0;
...
int ossl_rand_check_random_provider_on_unload(OSSL_LIB_CTX* ctx,
  ...
  if (dgbl->random_provider == prov) dgbl->random_provider = NULL;
  return 1;
}
