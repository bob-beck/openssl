...
RAND_poll(void)
{
  ...
  if (!ret) {
    ...
    RAND_POOL* pool = ossl_rand_pool_new(RAND_DRBG_STRENGTH,
                                         1,
 ...
 */
int
RAND_priv_bytes_ex(OSSL_LIB_CTX* ctx,
                   unsigned char* buf,
                   size_t num,
...
int
RAND_bytes_ex(OSSL_LIB_CTX* ctx,
              unsigned char* buf,
              size_t num,
              unsigned int strength)
{
...
static EVP_RAND_CTX*
rand_new_drbg(OSSL_LIB_CTX* libctx,
              EVP_RAND_CTX* parent,
              ...
              time_t reseed_time_interval)
{
  ...
  if (dgbl->rng_digest != NULL &&
    ...
    *p++ = OSSL_PARAM_construct_utf8_string(
      OSSL_DRBG_PARAM_DIGEST, dgbl->rng_digest, 0);
  if (prov_name != NULL)
    *p++ = OSSL_PARAM_construct_utf8_string(
...
#else  /* FIPS_MODULE */
  ret = rand_new_drbg(
    ctx, seed, PRIMARY_RESEED_INTERVAL, PRIMARY_RESEED_TIME_INTERVAL);
#endif /* FIPS_MODULE */
...
rand_get0_public(OSSL_LIB_CTX* ctx, RAND_GLOBAL* dgbl)
{
  ...
  if (rand == NULL) {
    ...
    if (CRYPTO_THREAD_get_local_ex(CRYPTO_THREAD_LOCAL_DRBG_PRIV_KEY, ctx) ==
      ...
      return NULL;
    rand = rand_new_drbg(
      ctx, primary, SECONDARY_RESEED_INTERVAL, SECONDARY_RESEED_TIME_INTERVAL);
...
RAND_set0_public(OSSL_LIB_CTX* ctx, EVP_RAND_CTX* rand)
{
  ...
  if ((r = CRYPTO_THREAD_set_local_ex(
         CRYPTO_THREAD_LOCAL_DRBG_PUB_KEY, ctx, rand)) > 0)
...
RAND_set0_private(OSSL_LIB_CTX* ctx, EVP_RAND_CTX* rand)
{
  ...
  if ((r = CRYPTO_THREAD_set_local_ex(
         CRYPTO_THREAD_LOCAL_DRBG_PRIV_KEY, ctx, rand)) > 0)
    EVP_RAND_CTX_free(old);
  return r;
}
...
int
RAND_set_DRBG_type(OSSL_LIB_CTX* ctx,
                   const char* drbg,
                   const char* propq,
                   const char* cipher,
                   const char* digest)
...
int
RAND_set_seed_source_type(OSSL_LIB_CTX* ctx,
                          const char* seed,
                          const char* propq)
{
  RAND_GLOBAL* dgbl = rand_get_global(ctx);
 ...
 */
int
ossl_rand_check_random_provider_on_load(OSSL_LIB_CTX* ctx, OSSL_PROVIDER* prov)
{
  RAND_GLOBAL* dgbl = rand_get_global(ctx);
