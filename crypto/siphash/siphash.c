...
static size_t siphash_adjust_hash_size(size_t hash_size) {
  if (hash_size == 0) hash_size = SIPHASH_MAX_DIGEST_SIZE;
  return hash_size;
}
...
int SipHash_Init(SIPHASH* ctx, const unsigned char* k, int crounds,
  ...
  if (drounds == 0) drounds = SIPHASH_D_ROUNDS;
  if (crounds == 0) crounds = SIPHASH_C_ROUNDS;
  ...
  if (ctx->hash_size == SIPHASH_MAX_DIGEST_SIZE) ctx->v1 ^= 0xee;
...
void SipHash_Update(SIPHASH* ctx, const unsigned char* in, size_t inlen) {
  ...
  if (left) memcpy(ctx->leavings, end, left);
  ctx->len = left;
...
int SipHash_Final(SIPHASH* ctx, unsigned char* out, size_t outlen) {
  ...
  b = v0 ^ v1 ^ v2 ^ v3;
  U64TO8_LE(out, b);
  if (ctx->hash_size == SIPHASH_MIN_DIGEST_SIZE) return 1;
  v1 ^= 0xdd;
  for (i = 0; i < ctx->drounds; ++i) SIPROUND;
  b = v0 ^ v1 ^ v2 ^ v3;
  U64TO8_LE(out + 8, b);
