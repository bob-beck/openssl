...
int WPACKET_allocate_bytes(WPACKET* pkt, size_t len,
  ...
  if (!WPACKET_reserve_bytes(pkt, len, allocbytes)) return 0;
...
int WPACKET_reserve_bytes(WPACKET* pkt, size_t len,
  ...
  if (pkt->buf != NULL && (pkt->buf->length - pkt->written < len)) {
    ...
    if (reflen > SIZE_MAX / 2) {
      ...
      newlen = reflen * 2;
      if (newlen < DEFAULT_BUF_SIZE) newlen = DEFAULT_BUF_SIZE;
    }
    if (BUF_MEM_grow(pkt->buf, newlen) == 0) return 0;
  }
  if (allocbytes != NULL) {
    *allocbytes = WPACKET_get_curr(pkt);
    if (pkt->endfirst && *allocbytes != NULL) *allocbytes -= len;
  }
...
int WPACKET_sub_reserve_bytes__(WPACKET* pkt, size_t len,
                                unsigned char** allocbytes, size_t lenbytes) {
  if (pkt->endfirst && lenbytes > 0) return 0;

  if (!WPACKET_reserve_bytes(pkt, lenbytes + len, allocbytes)) return 0;

  if (*allocbytes != NULL) *allocbytes += lenbytes;

  return 1;
}

static size_t maxmaxsize(size_t lenbytes) {
  if (lenbytes >= sizeof(size_t) || lenbytes == 0) return SIZE_MAX;
...
static int wpacket_intern_init_len(WPACKET* pkt, size_t lenbytes) {
  ...
  if ((pkt->subs = OPENSSL_zalloc(sizeof(*pkt->subs))) == NULL) return 0;
...
static int put_quic_value(unsigned char* data, size_t value, size_t len) {
  ...
  if (ossl_quic_vlint_encode_len(value) > len) return 0;
  ...
  if (sub->lenbytes > 0) {
    ...
    if (buf != NULL) {
      ...
      if ((sub->flags & WPACKET_FLAGS_QUIC_VLINT) == 0) {
        if (!put_value(&buf[sub->packet_len], packlen, sub->lenbytes)) return 0;
      } else {
    ...
    if (!WPACKET_put_bytes__(pkt, packlen, numlenbytes)) return 0;
    if (packlen > 0x7f) {
      numlenbytes |= 0x80;
      if (!WPACKET_put_bytes_u8(pkt, numlenbytes)) return 0;
    }
  }
...
int WPACKET_fill_lengths(WPACKET* pkt) {
  ...
  for (sub = pkt->subs; sub != NULL; sub = sub->parent) {
    if (!wpacket_intern_close(pkt, sub, 0)) return 0;
  }
...
int WPACKET_start_sub_packet_len__(WPACKET* pkt, size_t lenbytes) {
  ...
  if (lenbytes > 0 && pkt->endfirst) return 0;
  ...
  if (!WPACKET_allocate_bytes(pkt, lenbytes, &lenchars)) return 0;
...
int WPACKET_set_max_size(WPACKET* pkt, size_t maxsize) {
  ...
  if (lenbytes == 0) lenbytes = sizeof(pkt->maxsize);

  if (maxmaxsize(lenbytes) < maxsize || maxsize < pkt->written) return 0;
...
int WPACKET_memset(WPACKET* pkt, int ch, size_t len) {
  ...
  if (!WPACKET_allocate_bytes(pkt, len, &dest)) return 0;

  if (dest != NULL) memset(dest, ch, len);
...
int WPACKET_memcpy(WPACKET* pkt, const void* src, size_t len) {
  ...
  if (dest != NULL) memcpy(dest, src, len);
...
unsigned char* WPACKET_get_curr(WPACKET* pkt) {
  ...
  if (pkt->endfirst) return buf + pkt->maxsize - pkt->curr;
...
int WPACKET_start_quic_sub_packet_bound(WPACKET* pkt, size_t max_len) {
  ...
  if (WPACKET_start_sub_packet_len__(pkt, enclen) == 0) return 0;
