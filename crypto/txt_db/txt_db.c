...
TXT_DB* TXT_DB_read(BIO* in, int num) {
  ...
  ret->index = NULL;
  ret->qual = NULL;
  if ((ret->data = sk_OPENSSL_PSTRING_new_null()) == NULL) goto err;
  ...
  for (;;) {
    ...
    if (buf->data[offset - 1] != '\n')
      ...
      if ((p = OPENSSL_malloc(add + offset)) == NULL) goto err;
      offset = 0;
    }
    pp = (char**)p;
    p += add;
...
long TXT_DB_write(BIO* out, TXT_DB* db) {
  ...
  for (i = 0; i < n; i++) {
    ...
    l = 0;
    for (j = 0; j < nn; j++) {
      if (pp[j] != NULL) l += (long)strlen(pp[j]);
    }
    ...
    p[-1] = '\n';
    j = (long)(p - buf->data);
    if (BIO_write(out, buf->data, (int)j) != j) goto err;
    tot += j;
  }
  ret = tot;
err:
...
int TXT_DB_insert(TXT_DB* db, OPENSSL_STRING* row) {
  ...
  for (i = 0; i < db->num_fields; i++) {
    if (db->index[i] != NULL) {
      if ((db->qual[i] != NULL) && (db->qual[i](row) == 0)) continue;
  ...
  for (i = 0; i < db->num_fields; i++) {
    if (db->index[i] != NULL) {
      ...
      if (lh_OPENSSL_STRING_retrieve(db->index[i], row) == NULL) goto err1;
    }
  }
  if (!sk_OPENSSL_PSTRING_push(db->data, row)) goto err1;
  return 1;
    ...
    for (i = sk_OPENSSL_PSTRING_num(db->data) - 1; i >= 0; i--) {
      ...
      if (max == NULL) {       /* new row */
        ...
        for (n = 0; n < db->num_fields; n++) {
          if (((p[n] < (char*)p) || (p[n] > max))) OPENSSL_free(p[n]);
        }
      }
