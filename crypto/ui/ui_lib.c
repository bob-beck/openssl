...
{
  int ret = -1;
  UI_STRING* s = general_allocate_prompt(
    ui, prompt, prompt_freeable, type, input_flags, result_buf);
...
UI_add_input_string(UI* ui,
                    ...
                    int maxsize)
{
  return general_allocate_string(
    ui, prompt, 0, UIT_PROMPT, flags, result_buf, minsize, maxsize, NULL);
}
                    ...
                    int maxsize)
{
  ...
  ret = general_allocate_string(
    ui, prompt_copy, 1, UIT_PROMPT, flags, result_buf, minsize, maxsize, NULL);
  if (ret <= 0)
                     ...
                     const char* test_buf)
{
  ...
  ret = general_allocate_string(ui,
                                prompt_copy,
                                1,
                                UIT_VERIFY,
                                flags,
...
UI_add_input_boolean(UI* ui,
                     ...
                     char* result_buf)
{
  return general_allocate_boolean(ui,
                                  prompt,
                                  action_desc,
                                  ok_chars,
                                  cancel_chars,
                     ...
                     char* result_buf)
{
  ...
  ret = general_allocate_boolean(ui,
                                 prompt_copy,
                                 action_desc_copy,
                                 ok_chars_copy,
                                 cancel_chars_copy,
...
char*
UI_construct_prompt(UI* ui, const char* phrase_desc, const char* object_name)
{
  char* prompt = NULL;
...
int
UI_dup_user_data(UI* ui, void* user_data)
{
  void* duplicate = NULL;
...
int
UI_get_result_length(UI* ui, int i)
{
  if (i < 0) {
    ERR_raise(ERR_LIB_UI, UI_R_INDEX_TOO_SMALL);
    return -1;
...
void*
UI_get_ex_data(const UI* r, int idx)
{
...
UI_create_method(const char* name)
{
  ...
  if ((ui_method = OPENSSL_zalloc(sizeof(*ui_method))) == NULL ||
      ...
      !CRYPTO_new_ex_data(
        CRYPTO_EX_INDEX_UI_METHOD, ui_method, &ui_method->ex_data)) {
...
int
UI_method_set_data_duplicator(UI_METHOD* method,
...
int
UI_method_set_ex_data(UI_METHOD* method, int idx, void* data)
{
...
const void*
UI_method_get_ex_data(const UI_METHOD* method, int idx)
{
...
int
UI_get_result_string_length(UI_STRING* uis)
{
  switch (uis->type) {
    case UIT_PROMPT:
    case UIT_VERIFY:
...
int
UI_set_result_ex(UI* ui, UI_STRING* uis, const char* result, int len)
{
  ui->flags &= ~UI_FLAG_REDOABLE;
