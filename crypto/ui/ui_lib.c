...
UI* UI_new_method(const UI_METHOD* method) {
  ...
  if (method == NULL) method = UI_get_default_method();
  if (method == NULL) method = UI_null();
  ret->meth = method;
...
int UI_dup_input_string(UI* ui, const char* prompt, int flags, char* result_buf,
  ...
  if (ret <= 0) OPENSSL_free(prompt_copy);
...
int UI_dup_info_string(UI* ui, const char* text) {
  ...
  if (ret <= 0) OPENSSL_free(text_copy);
  return ret;
}
...
int UI_process(UI* ui) {
  ...
  if (ui->meth->ui_flush != NULL) switch (ui->meth->ui_flush(ui)) {
...
err:
  if (ui->meth->ui_close_session != NULL &&
    ...
    if (state == NULL) state = "closing session";
    ok = -1;
  }
...
int (*UI_method_get_opener(const UI_METHOD* method))(UI*) {
  if (method != NULL) return method->ui_open_session;
  return NULL;
}

int (*UI_method_get_writer(const UI_METHOD* method))(UI*, UI_STRING*) {
  if (method != NULL) return method->ui_write_string;
  return NULL;
}

int (*UI_method_get_flusher(const UI_METHOD* method))(UI*) {
  if (method != NULL) return method->ui_flush;
  return NULL;
}

int (*UI_method_get_reader(const UI_METHOD* method))(UI*, UI_STRING*) {
  if (method != NULL) return method->ui_read_string;
  return NULL;
}

int (*UI_method_get_closer(const UI_METHOD* method))(UI*) {
  if (method != NULL) return method->ui_close_session;
  return NULL;
}

char* (*UI_method_get_prompt_constructor(const UI_METHOD* method))(
    UI*, const char*, const char*) {
  if (method != NULL) return method->ui_construct_prompt;
  return NULL;
}

void* (*UI_method_get_data_duplicator(const UI_METHOD* method))(UI*, void*) {
  if (method != NULL) return method->ui_duplicate_data;
  return NULL;
}

void (*UI_method_get_data_destructor(const UI_METHOD* method))(UI*, void*) {
  if (method != NULL) return method->ui_destroy_data;
  return NULL;
}
...
int UI_set_result_ex(UI* ui, UI_STRING* uis, const char* result, int len) {
  ...
  switch (uis->type) {
    ...
    case UIT_VERIFY:
      ...
      if (len <= uis->_.string_data.result_maxsize) uis->result_buf[len] = '\0';
      uis->result_len = len;
      break;
    case UIT_BOOLEAN: {
      const char* p;
