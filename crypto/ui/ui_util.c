...
int
UI_UTIL_read_pw (char *buf, char *buff, int size, const char *prompt,
                 int verify)
{
  int ok = -2;
  UI *ui;
...
static void
ui_new_method_data (void *parent, void *ptr, CRYPTO_EX_DATA *ad, int idx,
                    long argl, void *argp)
{
  /*
...
static int
ui_dup_method_data (CRYPTO_EX_DATA *to, const CRYPTO_EX_DATA *from,
...
static void
ui_free_method_data (void *parent, void *ptr, CRYPTO_EX_DATA *ad, int idx,
                     long argl, void *argp)
{
  OPENSSL_free (ptr);
}
...
UI_METHOD *
UI_UTIL_wrap_read_pem_callback (pem_password_cb *cb, int rwflag)
{
