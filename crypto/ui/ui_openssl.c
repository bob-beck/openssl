...
static int
noecho_fgets(char* buf, int size, FILE* tty);
#endif
static int
...
read_string_inner(UI* ui, UI_STRING* uis, int echo, int strip_nl)
{
  ...
  if (is_a_tty) {
    ...
    if (GetEnvironmentVariableW(L"OPENSSL_WIN32_UTF8", NULL, 0) != 0) {
      ...
      if (ReadConsoleW(
            GetStdHandle(STD_INPUT_HANDLE), wresult, maxsize, &numread, NULL)) {
        ...
        if (WideCharToMultiByte(
              CP_UTF8, 0, wresult, -1, result, sizeof(result), NULL, 0) > 0)
          p = result;
...
#endif
      if (ReadConsoleA(
            GetStdHandle(STD_INPUT_HANDLE), result, maxsize, &numread, NULL)) {
