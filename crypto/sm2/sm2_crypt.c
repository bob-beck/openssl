...
int ossl_sm2_ciphertext_size(const EC_KEY* key, const EVP_MD* digest,
  ...
  if (field_size == 0 || md_size <= 0 || msg_len > INT_MAX / 2) return 0;
...
int ossl_sm2_encrypt(const EC_KEY* key, const EVP_MD* digest,
  ...
  if (x2y2 == NULL || C3 == NULL) goto done;

  memset(ciphertext_buf, 0, *ciphertext_len);

  msg_mask = OPENSSL_zalloc(msg_len);
  if (msg_mask == NULL) goto done;
...
int ossl_sm2_decrypt(const EC_KEY* key, const EVP_MD* digest,
  ...
  if (field_size == 0 || hash_size <= 0 || ciphertext_len > LONG_MAX) goto done;
  ...
  if (msg_mask == NULL || x2y2 == NULL || computed_C3 == NULL) goto done;
  ...
  *ptext_len = msg_len;
  ...
  if (rc == 0) memset(ptext_buf, 0, *ptext_len);
