...
int ossl_lib_ctx_write_lock(OSSL_LIB_CTX* ctx) {
  if ((ctx = ossl_lib_ctx_get_concrete(ctx)) == NULL) return 0;
...
static int context_init(OSSL_LIB_CTX* ctx) {
  ...
  if (ctx->provider_conf == NULL) goto err;
#endif

  /* P2. */
  ctx->drbg = ossl_rand_ctx_new(ctx);
  if (ctx->drbg == NULL) goto err;
...
err:
  ...
  if (exdata_done) ossl_crypto_cleanup_all_ex_data_int(ctx);
...
DEFINE_RUN_ONCE_STATIC(default_context_do_init) {
  if (!CRYPTO_THREAD_init_local(&default_context_thread_local, NULL)) goto err;

  if (!context_init(&default_context_int)) goto deinit_thread;
...
void ossl_lib_ctx_default_deinit(void) {
  if (!default_context_inited) return;
...
static OSSL_LIB_CTX* get_thread_default_context(void) {
  if (!RUN_ONCE(&default_context_init, default_context_do_init)) return NULL;
...
static int set_default_context(OSSL_LIB_CTX* defctx) {
  if (defctx == &default_context_int) defctx = NULL;
...
void OSSL_LIB_CTX_free(OSSL_LIB_CTX* ctx) {
  if (ctx == NULL || ossl_lib_ctx_is_default(ctx)) return;

#ifndef FIPS_MODULE
  if (ctx->ischild) ossl_provider_deinit_child(ctx);
#endif
  context_deinit(ctx);
  OPENSSL_free(ctx);
}
...
OSSL_LIB_CTX* OSSL_LIB_CTX_set0_default(OSSL_LIB_CTX* libctx) {
  ...
  if ((current_defctx = get_default_context()) != NULL) {
    if (libctx != NULL) set_default_context(libctx);
    return current_defctx;
  }
...
#ifndef FIPS_MODULE
  if (ctx == NULL) return get_default_context();
#endif
  return ctx;
}
...
#ifndef FIPS_MODULE
  if (ctx == NULL || ctx == get_default_context()) return 1;
#endif
  return 0;
}
...
#ifndef FIPS_MODULE
  if (ossl_lib_ctx_get_concrete(ctx) == &default_context_int) return 1;
#endif
  return 0;
}
