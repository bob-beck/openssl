                                      ...
                                      BN_GENCB* cb)
{
  ...
  if (Xp1 == NULL) {
    ...
    if (!BN_priv_rand_ex(
          Xp1i, bitlen, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ODD, 0, ctx))
      goto err;
  }
  ...
  if (Xp2 == NULL) {
    /* Set the top and bottom bits to make it odd and the correct size */
    if (!BN_priv_rand_ex(
          Xp2i, bitlen, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ODD, 0, ctx))
      goto err;
  }
  ...
  if (!ossl_bn_rsa_fips186_4_derive_prime(
                                   ...
                                   BN_GENCB* cb)
{
  ...
  if (Xin == NULL) {
    if (bits < BN_num_bits(&ossl_bn_inv_sqrt_2))
      goto err;
    if (!BN_lshift(
          base, &ossl_bn_inv_sqrt_2, bits - BN_num_bits(&ossl_bn_inv_sqrt_2)) ||
