 ...
 * below any attempt to generate 1024 bit RSA keys will result in an error
 * (Note that digital signature verification can still use deprecated 1024 bit
 * keys).
 *
 ...
 */
static int
bn_rsa_fips186_5_aux_prime_MR_rounds (int nbits)
{
  if (nbits >= 4096)
    return 44;
  if (nbits >= 3072)
 ...
 */
static int
bn_rsa_fips186_5_aux_prime_min_size (int nbits)
{
  if (nbits >= 4096)
    return 201;
  if (nbits >= 3072)
 ...
 */
static int
bn_rsa_fips186_4_find_aux_prob_prime (const BIGNUM *Xp1, BIGNUM *p1,
                                      BN_CTX *ctx, int rounds, BN_GENCB *cb)
{
  int ret = 0;
  int i = 0;
  int tmp = 0;
 ...
 */
int
ossl_bn_rsa_fips186_4_gen_prob_primes (BIGNUM *p, BIGNUM *Xpout, BIGNUM *p1,
 ...
 */
int
ossl_bn_rsa_fips186_4_derive_prime (BIGNUM *Y, BIGNUM *X, const BIGNUM *Xin,
