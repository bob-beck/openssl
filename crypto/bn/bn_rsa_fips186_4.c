...
static int bn_rsa_fips186_5_aux_prime_MR_rounds(int nbits) {
  if (nbits >= 4096) return 44;
  if (nbits >= 3072) return 41;
  if (nbits >= 2048) return 38;
  return 0; /* Error */
}
...
static int bn_rsa_fips186_5_prime_MR_rounds(int nbits) {
  if (nbits >= 3072) return 4;
  if (nbits >= 2048) return 5;
  return 0; /* Error */
}
...
static int bn_rsa_fips186_5_aux_prime_min_size(int nbits) {
  if (nbits >= 4096) return 201;
  if (nbits >= 3072) return 171;
  if (nbits >= 2048) return 141;
  return 0;
}
...
static int bn_rsa_fips186_5_aux_prime_max_sum_size_for_prob_primes(int nbits) {
  if (nbits >= 4096) return 2030;
  if (nbits >= 3072) return 1518;
  if (nbits >= 2048) return 1007;
  return 0;
}
...
static int bn_rsa_fips186_4_find_aux_prob_prime(const BIGNUM* Xp1, BIGNUM* p1,
  ...
  if (BN_copy(p1, Xp1) == NULL) return 0;
...
int ossl_bn_rsa_fips186_4_gen_prob_primes(BIGNUM* p, BIGNUM* Xpout, BIGNUM* p1,
  ...
  if (p1i == NULL || p2i == NULL || Xp1i == NULL || Xp2i == NULL) goto err;

  bitlen = bn_rsa_fips186_5_aux_prime_min_size(nlen);
  if (bitlen == 0) goto err;
...
err:
  ...
  if (p1 == NULL) BN_clear(p1i);
  if (p2 == NULL) BN_clear(p2i);
  if (Xp1 == NULL) BN_clear(Xp1i);
  if (Xp2 == NULL) BN_clear(Xp2i);
  BN_CTX_end(ctx);
  return ret;
}
...
int ossl_bn_rsa_fips186_4_derive_prime(BIGNUM* Y, BIGNUM* X, const BIGNUM* Xin,
  ...
  y1 = BN_CTX_get(ctx);
  r1x2 = BN_CTX_get(ctx);
  if (r1x2 == NULL) goto err;

  if (Xin != NULL && BN_copy(X, Xin) == NULL) goto err;
   ...
   */
  if (Xin == NULL) {
    if (bits < BN_num_bits(&ossl_bn_inv_sqrt_2)) goto err;
  ...
  if (BN_is_negative(R) && !BN_add(R, R, r1r2x2)) goto err;
  ...
  for (;;) {
    ...
    if (!BN_mod_sub(Y, R, X, r1r2x2, ctx) || !BN_add(Y, Y, X)) goto err;
    /* (Step 5) */
    i = 0;
    for (;;) {
      /* (Step 6) */
      ...
      if (BN_copy(y1, Y) == NULL || !BN_sub_word(y1, 1)) goto err;
