 ...
 */
static int
BN_GF2m_mod_inv_vartime(BIGNUM* r,
                        const BIGNUM* a,
                        const BIGNUM* p,
                        BN_CTX* ctx)
{
...
BN_GF2m_mod_inv(BIGNUM* r, const BIGNUM* a, const BIGNUM* p, BN_CTX* ctx)
{
  ...
  do {
    if (!BN_priv_rand_ex(
          b, numbits - 1, BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ANY, 0, ctx))
      goto err;
  } while (BN_is_zero(b));
 ...
 */
int
BN_GF2m_mod_solve_quad_arr(BIGNUM* r,
                           const BIGNUM* a_,
                           const int p[],
                           BN_CTX* ctx)
{
  ...
  if (p[0] & 0x1) { /* m is odd */
    ...
    do {
      if (!BN_priv_rand_ex(
            rho, p[0], BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ANY, 0, ctx))
        goto err;
