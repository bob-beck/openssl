...
int BN_GF2m_add(BIGNUM* r, const BIGNUM* a, const BIGNUM* b) {
  ...
  if (bn_wexpand(r, at->top) == NULL) return 0;
...
int BN_GF2m_mod_arr(BIGNUM* r, const BIGNUM* a, const int p[]) {
  ...
  for (j = r->top - 1; j > dN;) {
    ...
    d1 = BN_BITS2 - d0;
    z[j - n] ^= (zz >> d0);
    if (ossl_likely(d0)) z[j - n - 1] ^= (zz << d1);
  }
  ...
  while (j == dN) {
    ...
    for (k = 1; p[k] != 0; k++) {
      ...
      d1 = BN_BITS2 - d0;
      z[n] ^= (zz << d0);
      if (d0 && (tmp_ulong = zz >> d1)) z[n + 1] ^= tmp_ulong;
    }
  }
...
int BN_GF2m_mod_mul_arr(BIGNUM* r, const BIGNUM* a, const BIGNUM* b,
  ...
  bn_correct_top(s);
  if (BN_GF2m_mod_arr(r, s, p)) ret = 1;
  bn_check_top(r);
...
static int BN_GF2m_mod_inv_vartime(BIGNUM* r, const BIGNUM* a, const BIGNUM* p,
...
#else
  {
    ...
    if (!bn_wexpand(u, top)) goto err;
    udp = u->d;
    for (i = u->top; i < top; i++) udp[i] = 0;
    u->top = top;
    if (!bn_wexpand(b, top)) goto err;
    bdp = b->d;
    bdp[0] = 1;
    for (i = 1; i < top; i++) bdp[i] = 0;
    b->top = top;
    if (!bn_wexpand(c, top)) goto err;
    cdp = c->d;
...
int BN_GF2m_mod_inv(BIGNUM* r, const BIGNUM* a, const BIGNUM* p, BN_CTX* ctx) {
  ...
  /* r := a * b */
  if (!BN_GF2m_mod_mul(r, a, b, p, ctx)) goto err;

  /* r := 1/(a * b) */
  if (!BN_GF2m_mod_inv_vartime(r, r, p, ctx)) goto err;

  /* r := b/(a * b) = 1/a */
  if (!BN_GF2m_mod_mul(r, r, b, p, ctx)) goto err;
...
int BN_GF2m_mod_exp_arr(BIGNUM* r, const BIGNUM* a, const BIGNUM* b,
  ...
  if (BN_is_zero(b)) return BN_one(r);

  if (BN_abs_is_word(b, 1)) return (BN_copy(r, a) != NULL);
...
int BN_GF2m_poly2arr(const BIGNUM* a, int p[], int max) {
  ...
  if (!BN_is_odd(a)) return 0;

  for (i = a->top - 1; i >= 0; i--) {
    if (!a->d[i]) /* skip word if a->d[i] == 0 */
      continue;
    mask = BN_TBIT;
  ...
  if (k > 0 && p[0] > OPENSSL_ECC_MAX_FIELD_BITS) return 0;
...
int BN_GF2m_arr2poly(const int p[], BIGNUM* a) {
  ...
  BN_zero(a);
  for (i = 0; p[i] != -1; i++) {
    if (BN_set_bit(a, p[i]) == 0) return 0;
  }
  bn_check_top(a);
