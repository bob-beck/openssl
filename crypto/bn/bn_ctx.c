...
static void ctxdbg(BIO* channel, const char* text, BN_CTX* ctx) {
  ...
  while (bnidx < ctx->used) {
    ...
    if (!(bnidx % BN_CTX_POOL_SIZE)) item = item->next;
  }
  BIO_printf(channel, "\n");
  bnidx = 0;
...
void BN_CTX_start(BN_CTX* ctx) {
  ...
  if (ctx->err_stack || ctx->too_many) ctx->err_stack++;
...
static int BN_STACK_push(BN_STACK* st, unsigned int idx) {
  if (st->depth == st->size) {
    ...
    if ((newitems = OPENSSL_malloc_array(newsize, sizeof(*newitems))) == NULL)
      return 0;
    if (st->depth) memcpy(newitems, st->indexes, sizeof(*newitems) * st->depth);
    OPENSSL_free(st->indexes);
    st->indexes = newitems;
    st->size = newsize;
  }
...
static BIGNUM* BN_POOL_get(BN_POOL* p, int flag) {
  ...
  if (p->used == p->size) {
    ...
    if ((item = OPENSSL_malloc(sizeof(*item))) == NULL) return NULL;
    for (loop = 0, bn = item->vals; loop++ < BN_CTX_POOL_SIZE; bn++) {
      bn_init(bn);
      if ((flag & BN_FLG_SECURE) != 0) BN_set_flags(bn, BN_FLG_SECURE);
    }
    item->prev = p->tail;
    item->next = NULL;
