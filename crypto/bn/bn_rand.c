             ...
             BN_CTX* ctx)
{
  ...
  if (n == 1)
    ...
    do {
      if (!bnrand(
            flag, r, n + 1, BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ANY, strength, ctx))
        return 0;
    ...
    do {
      ...
      if (!bnrand(
            flag, r, n, BN_RAND_TOP_ANY, BN_RAND_BOTTOM_ANY, strength, ctx))
        return 0;
...
int
BN_priv_rand_range_ex(BIGNUM* r,
                      const BIGNUM* range,
                      unsigned int strength,
                      BN_CTX* ctx)
{
...
int
BN_generate_dsa_nonce(BIGNUM* out,
                      const BIGNUM* range,
                      const BIGNUM* priv,
                      const unsigned char* message,
                      size_t message_len,
                      BN_CTX* ctx)
{
  ...
  ret = ossl_bn_gen_dsa_nonce_fixed_top(
    out, range, priv, message, message_len, ctx);
  /*
