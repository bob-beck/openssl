...
static int bnrand(BNRAND_FLAG flag, BIGNUM* rnd, int bits, int top, int bottom,
  ...
  if (bits == 0) {
    if (top != BN_RAND_TOP_ANY || bottom != BN_RAND_BOTTOM_ANY) goto toosmall;
    BN_zero(rnd);
    return 1;
  }
  if (bits < 0 || (bits == 1 && top > 0)) goto toosmall;
  ...
  if (flag == TESTING) {
    ...
    for (i = 0; i < bytes; i++) {
      if (RAND_bytes_ex(libctx, &c, 1, strength) <= 0) goto err;
      if (c >= 128 && i > 0)
        buf[i] = buf[i - 1];
      else if (c < 42)
        buf[i] = 0;
...
int ossl_bn_gen_dsa_nonce_fixed_top(BIGNUM* out, const BIGNUM* range,
  ...
  for (n = 0; n < max_n; n++) {
    ...
    for (done = 1; done < num_k_bytes;) {
      ...
      if (todo > SHA512_DIGEST_LENGTH) todo = SHA512_DIGEST_LENGTH;
      memcpy(k_bytes + done, digest, todo);
      done += todo;
      ++i;
    }

    if (!BN_bin2bn(k_bytes, num_k_bytes, out)) goto end;
