 ...
 */
BIGNUM *
int_bn_mod_inverse (BIGNUM *in, const BIGNUM *a, const BIGNUM *n, BN_CTX *ctx,
                    int *pnoinv)
{
  BIGNUM *A, *B, *X, *Y, *M, *D, *T, *R = NULL;
  BIGNUM *ret = NULL;
      ...
      while (!BN_is_zero (B))
        {
           ...
           * at -sign*X*a  ==  B   (mod |n|), sign*Y*a  ==  A   (mod |n|). Note
           * that  X  and  Y  stay non-negative all the time.
           */
 ...
 */
int
BN_are_coprime (BIGNUM *a, const BIGNUM *b, BN_CTX *ctx)
{
  int ret = 0;
  BIGNUM *tmp;
