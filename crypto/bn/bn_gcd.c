...
static ossl_inline BIGNUM* bn_mod_inverse_no_branch(BIGNUM* in, const BIGNUM* a,
  ...
  if (B->neg || (BN_ucmp(B, A) >= 0)) {
     ...
     */
    {
      ...
      if (!BN_nnmod(B, &local_B, A, ctx)) goto err;
  ...
  while (!BN_is_zero(B)) {
     ...
     */
    {
      ...
      /* (D, M) := (A/B, A%B) ... */
      if (!BN_div(D, M, &local_A, B, ctx)) goto err;
...
BIGNUM* BN_mod_inverse(BIGNUM* in, const BIGNUM* a, const BIGNUM* n,
  ...
  if (noinv) ERR_raise(ERR_LIB_BN, BN_R_NO_INVERSE);
  BN_CTX_free(new_ctx);
  return rv;
}
...
int BN_gcd(BIGNUM* r, const BIGNUM* in_a, const BIGNUM* in_b, BN_CTX* ctx) {
  ...
  if (g == NULL || !BN_lshift1(g, in_b) || !BN_lshift1(r, in_a)) goto err;
  ...
  if (!BN_rshift(r, r, pow2_shifts) || !BN_rshift(g, g, pow2_shifts)) goto err;
