...
void BN_set_params(int mult, int high, int low, int mont) {
  if (mult >= 0) {
    if (mult > (int)(sizeof(int) * 8) - 1) mult = sizeof(int) * 8 - 1;
    bn_limit_bits = mult;
    bn_limit_num = 1 << mult;
  }
  if (high >= 0) {
    if (high > (int)(sizeof(int) * 8) - 1) high = sizeof(int) * 8 - 1;
    bn_limit_bits_high = high;
    bn_limit_num_high = 1 << high;
  }
  if (low >= 0) {
    if (low > (int)(sizeof(int) * 8) - 1) low = sizeof(int) * 8 - 1;
    bn_limit_bits_low = low;
    bn_limit_num_low = 1 << low;
  }
  if (mont >= 0) {
    if (mont > (int)(sizeof(int) * 8) - 1) mont = sizeof(int) * 8 - 1;
...
int BN_num_bits(const BIGNUM* a) {
  ...
  if (ossl_unlikely(BN_is_zero(a))) return 0;
...
void BN_clear_free(BIGNUM* a) {
  if (a == NULL) return;
  if (a->d != NULL && !BN_get_flags(a, BN_FLG_STATIC_DATA)) bn_free_d(a, 1);
...
void BN_free(BIGNUM* a) {
  if (a == NULL) return;
  if (!BN_get_flags(a, BN_FLG_STATIC_DATA)) bn_free_d(a, 0);
  if (a->flags & BN_FLG_MALLOCED) OPENSSL_free(a);
}
...
BIGNUM* BN_new(void) {
  ...
  if ((ret = OPENSSL_zalloc(sizeof(*ret))) == NULL) return NULL;
...
BIGNUM* BN_secure_new(void) {
  ...
  if (ret != NULL) ret->flags |= BN_FLG_SECURE;
  return ret;
}
...
static BN_ULONG* bn_expand_internal(const BIGNUM* b, int words) {
  ...
  if (ossl_unlikely(a == NULL)) return NULL;

  assert(b->top <= words);
  if (b->top > 0) memcpy(a, b->d, sizeof(*a) * b->top);
...
BIGNUM* bn_expand2(BIGNUM* b, int words) {
  if (ossl_likely(words > b->dmax)) {
    ...
    if (ossl_unlikely(!a)) return NULL;
    if (b->d != NULL) bn_free_d(b, 1);
    b->d = a;
    b->dmax = words;
  }
...
BIGNUM* BN_copy(BIGNUM* a, const BIGNUM* b) {
  ...
  if (ossl_unlikely(a == b)) return a;
  if (ossl_unlikely(bn_wexpand(a, bn_words) == NULL)) return NULL;

  if (ossl_likely(b->top > 0)) memcpy(a->d, b->d, sizeof(b->d[0]) * bn_words);
...
void BN_clear(BIGNUM* a) {
  if (a == NULL) return;
  bn_check_top(a);
  if (a->d != NULL) OPENSSL_cleanse(a->d, sizeof(*a->d) * a->dmax);
  a->neg = 0;
  a->top = 0;
...
static int bn2binpad(const BIGNUM* a, unsigned char* to, int tolen,
  ...
  if (tolen == -1) {
    ...
    n8 = BN_num_bits(&temp);
    n = (n8 + 7) / 8; /* This is what BN_num_bytes() does */
    if (tolen < n + ext) return -1;
  }

  /* Swipe through whole available data and don't give away padded zero. */
  atop = a->dmax * BN_BYTES;
  if (atop == 0) {
    if (tolen != 0) memset(to, '\0', tolen);
    return tolen;
  }
...
BIGNUM* BN_native2bn(const unsigned char* s, int len, BIGNUM* ret) {
  ...
  if (IS_LITTLE_ENDIAN) return BN_lebin2bn(s, len, ret);
...
BIGNUM* BN_signed_native2bn(const unsigned char* s, int len, BIGNUM* ret) {
  ...
  if (IS_LITTLE_ENDIAN) return BN_signed_lebin2bn(s, len, ret);
...
int BN_bn2nativepad(const BIGNUM* a, unsigned char* to, int tolen) {
  ...
  if (IS_LITTLE_ENDIAN) return BN_bn2lebinpad(a, to, tolen);
...
int BN_signed_bn2native(const BIGNUM* a, unsigned char* to, int tolen) {
  ...
  if (IS_LITTLE_ENDIAN) return BN_signed_bn2lebin(a, to, tolen);
...
int BN_ucmp(const BIGNUM* a, const BIGNUM* b) {
  ...
  if (BN_get_flags(a, BN_FLG_CONSTTIME) && a->top == b->top) {
    ...
    for (i = 0; i < b->top; i++) {
      res = constant_time_select_int((int)constant_time_lt_bn(ap[i], bp[i]), -1,
                                     res);
      res = constant_time_select_int((int)constant_time_lt_bn(bp[i], ap[i]), 1,
                                     res);
    }
    return res;
  }
...
int BN_is_bit_set(const BIGNUM* a, int n) {
  ...
  bn_check_top(a);
  if (ossl_unlikely(n < 0)) return 0;
  i = n / BN_BITS2;
  j = n % BN_BITS2;
  if (ossl_unlikely(a->top <= i)) return 0;
...
int bn_cmp_words(const BN_ULONG* a, const BN_ULONG* b, int n) {
  ...
  aa = a[n - 1];
  bb = b[n - 1];
  if (ossl_likely(aa != bb)) return ((aa > bb) ? 1 : -1);
  for (i = n - 2; i >= 0; i--) {
    aa = a[i];
    bb = b[i];
...
int ossl_bn_is_word_fixed_top(const BIGNUM* a, const BN_ULONG w) {
  ...
  if (a->neg || a->top == 0) return 0;

  res = constant_time_select_int((int)constant_time_eq_bn(ap[0], w), 1, 0);

  for (i = 1; i < a->top; i++)
    res =
        constant_time_select_int((int)constant_time_is_zero_bn(ap[i]), res, 0);
  return res;
}
...
BN_GENCB* BN_GENCB_new(void) {
  ...
  if ((ret = OPENSSL_malloc(sizeof(*ret))) == NULL) return NULL;
