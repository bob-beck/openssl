...
static ossl_inline int bn_num_bits_consttime(const BIGNUM *a)
{
	...
	for (j = 0, past_i = 0, ret = 0; j < a->dmax; j++) {
		mask = constant_time_eq_int(
			i, j); /* 0xff..ff if i==j, 0x0 otherwise */
	...
	if (BN_get_flags(a, BN_FLG_CONSTTIME) && a->top == b->top) {
		...
		for (i = 0; i < b->top; i++) {
			res = constant_time_select_int(
				(int)constant_time_lt_bn(ap[i], bp[i]), -1,
				res);
			res = constant_time_select_int(
...
int ossl_bn_is_word_fixed_top(const BIGNUM *a, const BN_ULONG w)
{
	...
	res = constant_time_select_int((int)constant_time_eq_bn(ap[0], w), 1,
				       0);

	for (i = 1; i < a->top; i++)
		res = constant_time_select_int(
			(int)constant_time_is_zero_bn(ap[i]), res, 0);
	return res;
}
