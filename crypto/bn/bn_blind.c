...
int BN_BLINDING_update(BN_BLINDING* b, BN_CTX* ctx) {
  ...
  if (++b->counter == BN_BLINDING_COUNTER && b->e != NULL &&
    ...
    if (!BN_BLINDING_create_param(b, NULL, NULL, ctx, NULL, NULL)) goto err;
  ...
  ret = 1;
err:
  if (b->counter == BN_BLINDING_COUNTER) b->counter = 0;
  return ret;
}
...
BN_BLINDING* BN_BLINDING_create_param(
  ...
  if (ret->A == NULL && (ret->A = BN_new()) == NULL) goto err;
  if (ret->Ai == NULL && (ret->Ai = BN_new()) == NULL) goto err;

  if (e != NULL) {
    BN_free(ret->e);
    ret->e = BN_dup(e);
  }
  if (ret->e == NULL) goto err;

  if (bn_mod_exp != NULL) ret->bn_mod_exp = bn_mod_exp;
  if (m_ctx != NULL) ret->m_ctx = m_ctx;

  do {
    int rv;
    if (!BN_priv_rand_range_ex(ret->A, ret->mod, 0, ctx)) goto err;
    if (int_bn_mod_inverse(ret->Ai, ret->A, ret->mod, ctx, &rv)) break;
  ...
  if (ret->bn_mod_exp != NULL && ret->m_ctx != NULL) {
    ...
    if (!BN_mod_exp(ret->A, ret->A, ret->e, ret->mod, ctx)) goto err;
  }
