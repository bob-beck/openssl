...
int BN_mod_exp_recp(BIGNUM* r, const BIGNUM* a, const BIGNUM* p,
  ...
  if (r == p) {
    ...
    if (p_dup == NULL || BN_copy(p_dup, p) == NULL) goto err;
    p = p_dup;
  }
  ...
  for (;;) {
    ...
    if (BN_is_bit_set(p, wstart) == 0) {
      if (!start)
        if (!BN_mod_mul_reciprocal(r, r, r, &recp, ctx)) goto err;
      if (wstart == 0) break;
      wstart--;
      continue;
    }
    ...
    if (!BN_mod_mul_reciprocal(r, r, val[wvalue >> 1], &recp, ctx)) goto err;
...
int BN_mod_exp_mont(BIGNUM* rr, const BIGNUM* a, const BIGNUM* p,
  ...
  if (a->neg || BN_ucmp(a, m) >= 0) {
    ...
    aa = a;
  if (!bn_to_mont_fixed_top(val[0], aa, mont, ctx)) goto err; /* 1 */

  window = BN_window_bits_for_exponent_size(bits);
  if (window > 1) {
    if (!bn_mul_mont_fixed_top(d, val[0], val[0], mont, ctx)) goto err; /* 2 */
    j = 1 << (window - 1);
    for (i = 1; i < j; i++) {
      if (((val[i] = BN_CTX_get(ctx)) == NULL) ||
  ...
  for (;;) {
    ...
    /* wvalue will be an odd number < 2^window */
    if (!bn_mul_mont_fixed_top(r, r, val[wvalue >> 1], mont, ctx)) goto err;
  ...
  ret = 1;
err:
  if (in_mont == NULL) BN_MONT_CTX_free(mont);
  BN_CTX_end(ctx);
  bn_check_top(rr);
  return ret;
}
...
static BN_ULONG bn_get_bits(const BIGNUM* a, int bitpos) {
  ...
  if (wordpos >= 0 && wordpos < a->top) {
    ...
    if (bitpos) {
      ret >>= bitpos;
      if (++wordpos < a->top) ret |= a->d[wordpos] << (BN_BITS2 - bitpos);
    }
  }
...
int bn_mod_exp_mont_fixed_top(BIGNUM* rr, const BIGNUM* a, const BIGNUM* p,
...
#ifdef alloca
  if (powerbufLen < 3072) powerbufFree = NULL;
#endif
  ...
  if (!bn_to_mont_fixed_top(&am, a, mont, ctx)) goto err;

  if (top > BN_SOFT_LIMIT) goto fallback;
  ...
  fallback:
    ...
    if (window > 1) {
      ...
      for (i = 3; i < numPowers; i++) {
        /* Calculate a^i = a^(i-1) * a */
        if (!bn_mul_mont_fixed_top(&tmp, &am, &tmp, mont, ctx)) goto err;
...
#define BN_TO_MONTGOMERY_WORD(r, w, mont) \
  ...
  if (m->top == 1) a %= m->d[0]; /* make sure that 'a' is reduced */
...
int BN_mod_exp_simple(BIGNUM* r, const BIGNUM* a, const BIGNUM* p,
  ...
  window = BN_window_bits_for_exponent_size(bits);
  if (window > 1) {
    if (!BN_mod_mul(d, val[0], val[0], m, ctx)) goto err; /* 2 */
    j = 1 << (window - 1);
    for (i = 1; i < j; i++) {
      if (((val[i] = BN_CTX_get(ctx)) == NULL) ||
  ...
  for (;;) {
    ...
    /* wvalue will be an odd number < 2^window */
    if (!BN_mod_mul(r, r, val[wvalue >> 1], m, ctx)) goto err;
...
int BN_mod_exp_mont_consttime_x2(BIGNUM* rr1, const BIGNUM* a1,
  ...
  if ((ossl_rsaz_avx512ifma_eligible() || ossl_rsaz_avxifma_eligible()) &&
    ...
    if (bn_wexpand(rr1, topn) == NULL) goto err;
    if (bn_wexpand(rr2, topn) == NULL) goto err;

    /*  Ensure that montgomery contexts are initialized */
    if (in_mont1 != NULL) {
      mont1 = in_mont1;
    } else {
      if ((mont1 = BN_MONT_CTX_new()) == NULL) goto err;
      if (!BN_MONT_CTX_set(mont1, m1, ctx)) goto err;
    }
    if (in_mont2 != NULL) {
      mont2 = in_mont2;
    } else {
      if ((mont2 = BN_MONT_CTX_new()) == NULL) goto err;
      if (!BN_MONT_CTX_set(mont2, m2, ctx)) goto err;
    }
...
#ifdef RSAZ_ENABLED
err:
  if (in_mont2 == NULL) BN_MONT_CTX_free(mont2);
  if (in_mont1 == NULL) BN_MONT_CTX_free(mont1);
#endif
