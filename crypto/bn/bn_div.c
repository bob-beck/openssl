...
int bn_div_fixed_top(BIGNUM* dv, BIGNUM* rm, const BIGNUM* num,
   ...
   */
  if (!(bn_lshift_fixed_top(snum, num, norm_shift))) goto err;
  ...
  if (num_n <= div_n) {
    ...
    if (bn_wexpand(snum, div_n + 1) == NULL) goto err;
  ...
  /* Setup quotient */
  if (!bn_wexpand(res, loop)) goto err;
  num_neg = num->neg;
  ...
  for (i = 0; i < loop; i++, wnumtop--) {
    ...
    if (n0 == d0)
      ...
      for (;;) {
        if (t2 <= ((((BN_ULLONG)rem) << BN_BITS2) | n2)) break;
        q--;
        rem += d0;
      ...
      for (;;) {
        if ((t2h < rem) || ((t2h == rem) && (t2l <= n2))) break;
        q--;
        rem += d0;
        if (rem < d0) break; /* don't let rem overflow */
        if (t2l < d1) t2h--;
  ...
  if (rm != NULL && bn_rshift_fixed_top(rm, snum, norm_shift) == 0) goto err;
