...
loop:
  ...
  if (add == NULL) {
    if (!probable_prime(ret, bits, safe, mods, ctx)) goto err;
  } else {
    if (!probable_prime_dh(ret, bits, safe, mods, add, rem, ctx)) goto err;
  }

  if (!BN_GENCB_call(cb, 0, c1++)) /* aborted */
    goto err;
...
int ossl_bn_check_prime(const BIGNUM* w, int checks, BN_CTX* ctx,
  ...
  if (checks < min_checks) checks = min_checks;
...
static int bn_is_prime_int(const BIGNUM* w, int checks, BN_CTX* ctx,
  ...
  if (do_trial_division) {
    ...
    for (i = 1; i < trial_divisions; i++) {
      ...
      if (mod == (BN_ULONG)-1) return -1;
      if (mod == 0) return BN_is_word(w, primes[i]);
    }
    if (!BN_GENCB_call(cb, 1, -1)) return -1;
  }
#ifndef FIPS_MODULE
  if (ctx == NULL && (ctxlocal = ctx = BN_CTX_new()) == NULL) goto err;
#endif
...
int ossl_bn_miller_rabin_is_prime(const BIGNUM* w, int iterations, BN_CTX* ctx,
  ...
  for (i = 0; i < iterations; ++i) {
    ...
    if (BN_is_one(z) || BN_cmp(z, w1) == 0) goto outer_loop;
    /* (Step 4.7) for j = 1 to a-1 */
    for (j = 1; j < a; ++j) {
      /* (Step 4.7.1 - 4.7.2) x = z. z = x^2 mod w */
      if (!BN_copy(x, z) || !BN_mod_mul(z, x, x, w, ctx)) goto err;
      /* (Step 4.7.3) */
      if (BN_cmp(z, w1) == 0) goto outer_loop;
      /* (Step 4.7.4) */
      if (BN_is_one(z)) goto composite;
    }
    ...
    if (!BN_copy(x, z)) goto err;
  composite:
    if (enhanced) {
      /* (Step 4.1.2) g = GCD(x-1, w) */
      if (!BN_sub_word(x, 1) || !BN_gcd(g, x, w, ctx)) goto err;
      /* (Steps 4.1.3 - 4.1.4) */
      if (BN_is_one(g))
...
again:
  if (!BN_priv_rand_ex(rnd, bits, BN_RAND_TOP_TWO, BN_RAND_BOTTOM_ODD, 0, ctx))
    return 0;
  if (safe && !BN_set_bit(rnd, 1)) return 0;
  /* we now have a random number 'rnd' to test. */
  for (i = 1; i < trial_divisions; i++) {
    BN_ULONG mod = BN_mod_word(rnd, (BN_ULONG)primes[i]);
    if (mod == (BN_ULONG)-1) return 0;
    mods[i] = (prime_t)mod;
  }
  delta = 0;
loop:
...
again:
  ...
  if (rem == NULL) {
    if (!BN_add_word(rnd, safe ? 3u : 1u)) goto err;
  } else {
