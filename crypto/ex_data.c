...
static EX_CALLBACKS* get_and_lock(OSSL_EX_DATA_GLOBAL* global, int class_index,
  ...
  if (read) {
    if (!CRYPTO_THREAD_read_lock(global->ex_data_lock)) return NULL;
  } else {
    if (!CRYPTO_THREAD_write_lock(global->ex_data_lock)) return NULL;
  }
...
int ossl_crypto_free_ex_index_ex(OSSL_LIB_CTX* ctx, int class_index, int idx) {
  ...
  if (idx < 0 || idx >= sk_EX_CALLBACK_num(ip->meth)) goto err;
...
int ossl_crypto_new_ex_data_ex(OSSL_LIB_CTX* ctx, int class_index, void* obj,
  ...
  if (mx > 0) {
    ...
    if (storage != NULL)
      for (i = 0; i < mx; i++) storage[i] = sk_EX_CALLBACK_value(ip->meth, i);
  }
  CRYPTO_THREAD_unlock(global->ex_data_lock);

  if (mx > 0 && storage == NULL) return 0;
  for (i = 0; i < mx; i++) {
  ...
  if (storage != stack) OPENSSL_free(storage);
  return 1;
}
...
int CRYPTO_dup_ex_data(int class_index, CRYPTO_EX_DATA* to,
  ...
  to->ctx = from->ctx;
  if (from->sk == NULL) /* Nothing to copy over */
    return 1;
   ...
   * proper size
   */
  if (!CRYPTO_set_ex_data(to, mx - 1, CRYPTO_get_ex_data(to, mx - 1))) goto err;
...
static int ex_callback_compare(const void* a, const void* b) {
  ...
  if (ap->excb == bp->excb) return 0;

  if (ap->excb == NULL) return 1;
  if (bp->excb == NULL) return -1;
  if (ap->excb->priority == bp->excb->priority) return 0;
...
void* CRYPTO_get_ex_data(const CRYPTO_EX_DATA* ad, int idx) {
  if (ad->sk == NULL || idx >= sk_void_num(ad->sk)) return NULL;
