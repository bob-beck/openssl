...
static void free_all_params(OSSL_PARAM_BLD* bld) {
  ...
  for (i = 0; i < n; i++) OPENSSL_free(sk_OSSL_PARAM_BLD_DEF_pop(bld->params));
}
...
static int push_BN(OSSL_PARAM_BLD* bld, const char* key, const BIGNUM* bn,
  ...
  if (bn != NULL) {
    ...
    if (BN_get_flags(bn, BN_FLG_SECURE) == BN_FLG_SECURE) secure = 1;
    ...
    if (pd->bn != NULL) {
      ...
      *(const void**)p = pd->string;
      ...
      if (pd->string != NULL)
        ...
        memset(p, 0, pd->size);
      if (pd->type == OSSL_PARAM_UTF8_STRING) ((char*)p)[pd->size] = '\0';
    } else {
