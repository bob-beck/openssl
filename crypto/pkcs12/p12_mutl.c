...
void PKCS12_get0_mac(const ASN1_OCTET_STRING** pmac, const X509_ALGOR** pmacalg,
  ...
  if (p12->mac) {
    ...
    if (psalt) *psalt = p12->mac->salt;
    if (piter) *piter = p12->mac->iter;
  } else {
    if (pmac) *pmac = NULL;
    if (pmacalg) *pmacalg = NULL;
    if (psalt) *psalt = NULL;
    if (piter) *piter = NULL;
  }
}
...
static int pkcs12_gen_mac(
  ...
  if (md == NULL) md = EVP_get_digestbynid(OBJ_obj2nid(macoid));
  ...
  if (pbmac1_md_nid != NID_undef && pkcs12_key_gen == NULL) {
    ...
    if (pkcs12_key_gen != NULL) {
      ...
      if (fetched) EVP_MD_free(hmac_md);
      if (res != 1) {
        ERR_raise(ERR_LIB_PKCS12, PKCS12_R_KEY_GEN_ERROR);
        goto err;
      }
...
static int pkcs12_setup_mac(PKCS12* p12, int iter, unsigned char* salt,
  ...
  if ((p12->mac = PKCS12_MAC_DATA_new()) == NULL) return PKCS12_ERROR;
  if (iter > 1) {
...
int PKCS12_set_pbmac1_pbkdf2(PKCS12* p12, const char* pass, int passlen,
  ...
  if (iter == 0) iter = PKCS12_DEFAULT_ITER;
  ...
  alg = PKCS5_pbkdf2_set(iter, salt ? salt : known_salt, saltlen, prf_nid,
                         keylen);
  if (param == NULL || hmac_alg == NULL || alg == NULL) goto err;
