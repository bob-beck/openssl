...
static int
pkcs12_pbmac1_pbkdf2_key_gen(const char* pass,
                             int passlen,
                             unsigned char* salt,
                             int saltlen,
                             int id,
...
void
PKCS12_get0_mac(const ASN1_OCTET_STRING** pmac,
...
static int
pkcs12_gen_gost_mac_key(const char* pass,
                        int passlen,
                        ...
                        const EVP_MD* digest)
{
  ...
  if (!PKCS5_PBKDF2_HMAC(
        pass, passlen, salt, saltlen, iter, digest, sizeof(out), out)) {
    return 0;
  }
...
PBKDF2PARAM*
PBMAC1_get1_pbkdf2_param(const X509_ALGOR* macalg)
{
...
static int
PBMAC1_PBKDF2_HMAC(OSSL_LIB_CTX* ctx,
                   const char* propq,
                   const char* pass,
                   int passlen,
                                     ...
                                     const EVP_MD* md_type))
{
  ...
  md = md_fetch = EVP_MD_fetch(
    p12->authsafes->ctx.libctx, md_name, p12->authsafes->ctx.propq);
  if (md == NULL)
  ...
  if (pbmac1_md_nid != NID_undef && pkcs12_key_gen == NULL) {
    ...
    if (!pkcs12_gen_gost_mac_key(
          pass, passlen, salt, saltlen, iter, keylen, key, md)) {
    ...
    if (pkcs12_key_gen != NULL) {
      ...
      if (!PKCS12_key_gen_utf8_ex(pass,
                                  passlen,
                                  salt,
                                  saltlen,
                                  PKCS12_MAC_ID,
  ...
  if ((hmac = HMAC_CTX_new()) == NULL ||
      ...
      !HMAC_Update(
        hmac, p12->authsafes->d.data->data, p12->authsafes->d.data->length) ||
...
PKCS12_gen_mac(PKCS12* p12,
               ...
               unsigned int* maclen)
{
  return pkcs12_gen_mac(
    p12, pass, passlen, mac, maclen, NID_undef, NID_undef, NULL);
}
...
PKCS12_verify_mac(PKCS12* p12, const char* pass, int passlen)
{
  ...
  if (OBJ_obj2nid(macoid) == NID_pbmac1) {
    ...
    if (!pkcs12_gen_mac(
          p12, pass, passlen, mac, &maclen, md_nid, NID_undef, NULL)) {
                             ...
                             const EVP_MD* md_type)
{
  return PKCS5_PBKDF2_HMAC(
    pass, passlen, salt, saltlen, iter, md_type, keylen, out);
}

static int
pkcs12_setup_mac(PKCS12* p12,
                 int iter,
                 unsigned char* salt,
                 int saltlen,
                 int nid)
{
  ...
  if (salt == NULL) {
    if (RAND_bytes_ex(p12->authsafes->ctx.libctx,
                      p12->mac->salt->data,
                      (size_t)saltlen,
                      0) <= 0)
      return 0;
...
int
PKCS12_set_pbmac1_pbkdf2(PKCS12* p12,
                         const char* pass,
                         int passlen,
                         unsigned char* salt,
                         int saltlen,
                         ...
                         const char* prf_md_name)
{
  ...
  if (pkcs12_setup_mac(
        p12, iter, salt ? salt : known_salt, saltlen, NID_pbmac1) ==
      PKCS12_ERROR) {
    ERR_raise(ERR_LIB_PKCS12, PKCS12_R_MAC_SETUP_ERROR);
    goto err;
  }
  ...
  if (!ASN1_TYPE_pack_sequence(
        ASN1_ITEM_rptr(PBMAC1PARAM), param, &macalg->parameter))
    goto err;

  /*
   * Note that output mac is forced to UTF-8...
   */
  if (!pkcs12_gen_mac(p12,
                      pass,
                      passlen,
                      mac,
                      &maclen,
