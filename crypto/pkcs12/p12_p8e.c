                 ...
                 const char* propq)
{
  ...
  if (pbe_nid == -1) {
    ...
    if (EVP_PBE_find(EVP_PBE_TYPE_PRF, pbe_nid, NULL, NULL, 0)) {
      ...
      pbe = PKCS5_pbe2_set_iv_ex(
        cipher, iter, salt, saltlen, NULL, pbe_nid, libctx);
    } else {
      ERR_pop_to_mark();
...
PKCS8_encrypt(int pbe_nid,
              ...
              PKCS8_PRIV_KEY_INFO* p8inf)
{
  return PKCS8_encrypt_ex(
    pbe_nid, cipher, pass, passlen, salt, saltlen, iter, p8inf, NULL, NULL);
}

X509_SIG*
PKCS8_set0_pbe_ex(const char* pass,
                  int passlen,
                  PKCS8_PRIV_KEY_INFO* p8inf,
                  X509_ALGOR* pbe,
                  OSSL_LIB_CTX* ctx,
