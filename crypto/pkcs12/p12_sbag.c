...
const ASN1_OBJECT*
PKCS12_SAFEBAG_get0_bag_type(const PKCS12_SAFEBAG* bag)
{
  return bag->value.bag->type;
}

const ASN1_TYPE*
PKCS12_SAFEBAG_get0_bag_obj(const PKCS12_SAFEBAG* bag)
{
...
X509*
PKCS12_SAFEBAG_get1_cert_ex(const PKCS12_SAFEBAG* bag,
                            OSSL_LIB_CTX* libctx,
                            const char* propq)
{
  X509* ret = NULL;
  ...
  if (OBJ_obj2nid(bag->value.bag->type) != NID_x509Certificate)
    return NULL;
  ret = ASN1_item_unpack_ex(
    bag->value.bag->value.octet, ASN1_ITEM_rptr(X509), libctx, propq);
...
X509_CRL*
PKCS12_SAFEBAG_get1_crl_ex(const PKCS12_SAFEBAG* bag,
                           OSSL_LIB_CTX* libctx,
                           const char* propq)
{
  X509_CRL* ret = NULL;
  ...
  if (OBJ_obj2nid(bag->value.bag->type) != NID_x509Crl)
    return NULL;
  ret = ASN1_item_unpack_ex(
    bag->value.bag->value.octet, ASN1_ITEM_rptr(X509_CRL), libctx, propq);
...
{
  return PKCS12_item_pack_safebag(
    x509, ASN1_ITEM_rptr(X509), NID_x509Certificate, NID_certBag);
}
...
{
  return PKCS12_item_pack_safebag(
    crl, ASN1_ITEM_rptr(X509_CRL), NID_x509Crl, NID_crlBag);
}

PKCS12_SAFEBAG*
PKCS12_SAFEBAG_create_secret(int type,
                             int vtype,
...
PKCS12_SAFEBAG*
PKCS12_SAFEBAG_create_pkcs8_encrypt_ex(int pbe_nid,
                                       const char* pass,
                                       int passlen,
                                       unsigned char* salt,
                                       int saltlen,
