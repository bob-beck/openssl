 ...
 */
unsigned char*
PKCS12_pbe_crypt_ex(const X509_ALGOR* algor,
                    const char* pass,
                    int passlen,
                    ...
                    const char* propq)
{
  ...
  /* Process data */
  if (!EVP_PBE_CipherInit_ex(algor->algorithm,
                             pass,
                             passlen,
                             algor->parameter,
                             ctx,
  ...
  if ((EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(ctx)) &
    ...
    if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
      if (EVP_CIPHER_CTX_ctrl(
            ctx, EVP_CTRL_AEAD_GET_TAG, (int)mac_len, out + outlen) < 0) {
        OPENSSL_free(out);
        out = NULL;
        ERR_raise(ERR_LIB_PKCS12, ERR_R_INTERNAL_ERROR);
        goto err;
...
unsigned char*
PKCS12_pbe_crypt(const X509_ALGOR* algor,
                 const char* pass,
                 int passlen,
                 const unsigned char* in,
                 int inlen,
                 ...
                 int en_de)
{
  return PKCS12_pbe_crypt_ex(
    algor, pass, passlen, in, inlen, data, datalen, en_de, NULL, NULL);
}
...
void*
PKCS12_item_decrypt_d2i_ex(const X509_ALGOR* algor,
                           const ASN1_ITEM* it,
                           const char* pass,
                           int passlen,
...
void*
PKCS12_item_decrypt_d2i(const X509_ALGOR* algor,
                        const ASN1_ITEM* it,
                        const char* pass,
                        int passlen,
                        const ASN1_OCTET_STRING* oct,
                        int zbuf)
{
  return PKCS12_item_decrypt_d2i_ex(
    algor, it, pass, passlen, oct, zbuf, NULL, NULL);
}
...
ASN1_OCTET_STRING*
PKCS12_item_i2d_encrypt_ex(X509_ALGOR* algor,
                           const ASN1_ITEM* it,
                           const char* pass,
                           int passlen,
                           void* obj,
...
ASN1_OCTET_STRING*
PKCS12_item_i2d_encrypt(X509_ALGOR* algor,
                        const ASN1_ITEM* it,
                        const char* pass,
                        int passlen,
                        void* obj,
                        int zbuf)
{
  return PKCS12_item_i2d_encrypt_ex(
    algor, it, pass, passlen, obj, zbuf, NULL, NULL);
}
