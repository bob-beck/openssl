...
unsigned char* PKCS12_pbe_crypt_ex(const X509_ALGOR* algor, const char* pass,
  ...
  /* Process data */
  if (!EVP_PBE_CipherInit_ex(algor->algorithm, pass, passlen, algor->parameter,
                             ctx, en_de, libctx, propq))
    goto err;
  ...
  if ((out = OPENSSL_malloc(max_out_len)) == NULL) goto err;
...
void* PKCS12_item_decrypt_d2i_ex(const X509_ALGOR* algor, const ASN1_ITEM* it,
  ...
  if (!ret) ERR_raise(ERR_LIB_PKCS12, PKCS12_R_DECODE_ERROR);
  OPENSSL_free(out);
  return ret;
}
...
ASN1_OCTET_STRING* PKCS12_item_i2d_encrypt_ex(
    X509_ALGOR* algor, const ASN1_ITEM* it, const char* pass, int passlen,
