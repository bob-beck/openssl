...
static int
pkcs12_remove_bag(STACK_OF(PKCS12_SAFEBAG)** pbags, PKCS12_SAFEBAG* bag);
static PKCS12_SAFEBAG*
pkcs12_add_cert_bag(STACK_OF(PKCS12_SAFEBAG)** pbags,
                    X509* cert,
                    const char* name,
                    int namelen,
                    unsigned char* keyid,
...
PKCS12*
PKCS12_create_ex2(const char* pass,
                  const char* name,
                  EVP_PKEY* pkey,
                  X509* cert,
                  STACK_OF(X509)* ca,
...
PKCS12*
PKCS12_create_ex(const char* pass,
                 const char* name,
                 EVP_PKEY* pkey,
                 X509* cert,
                 STACK_OF(X509)* ca,
...
PKCS12_SAFEBAG*
PKCS12_add_key_ex(STACK_OF(PKCS12_SAFEBAG)** pbags,
                  EVP_PKEY* key,
                  int key_usage,
                  int iter,
                  int nid_key,
                  ...
                  const char* propq)
{
  ...
  if (nid_key != -1) {
    ...
    bag = PKCS12_SAFEBAG_create_pkcs8_encrypt_ex(
      nid_key, pass, -1, NULL, 0, iter, p8, ctx, propq);
  } else {
...
PKCS12_SAFEBAG*
PKCS12_add_key(STACK_OF(PKCS12_SAFEBAG)** pbags,
               EVP_PKEY* key,
               int key_usage,
               int iter,
               int nid_key,
               const char* pass)
{
  return PKCS12_add_key_ex(
    pbags, key, key_usage, iter, nid_key, pass, NULL, NULL);
}

PKCS12_SAFEBAG*
PKCS12_add_secret(STACK_OF(PKCS12_SAFEBAG)** pbags,
                  int nid_type,
                  ...
                  int len)
{
  ...
  if ((bag = PKCS12_SAFEBAG_create_secret(
         nid_type, V_ASN1_OCTET_STRING, value, len)) == NULL)
    goto err;
...
int
PKCS12_add_safe_ex(STACK_OF(PKCS7)** psafes,
                   ...
                   const char* propq)
{
  ...
  if (nid_safe == -1)
    ...
    p7 = PKCS12_pack_p7encdata_ex(
      nid_safe, pass, -1, NULL, 0, iter, bags, ctx, propq);
  if (p7 == NULL)
    goto err;
...
int
PKCS12_add_safe(STACK_OF(PKCS7)** psafes,
                STACK_OF(PKCS12_SAFEBAG)* bags,
                int nid_safe,
                int iter,
                const char* pass)
...
PKCS12*
PKCS12_add_safes_ex(STACK_OF(PKCS7)* safes,
                    int nid_p7,
                    OSSL_LIB_CTX* ctx,
                    const char* propq)
{
