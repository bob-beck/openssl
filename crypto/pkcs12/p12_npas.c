...
static int newpass_p12(PKCS12* p12, const char* oldpass, const char* newpass) {
  ...
  if ((asafes = PKCS12_unpack_authsafes(p12)) == NULL) goto err;
  if ((newsafes = sk_PKCS7_new_null()) == NULL) goto err;
  for (i = 0; i < sk_PKCS7_num(asafes); i++) {
    ...
    if (bagnid == NID_pkcs7_data)
      ...
      p7new = PKCS12_pack_p7encdata_ex(pbe_nid, newpass, -1, NULL, pbe_saltlen,
                                       pbe_iter, bags, p7->ctx.libctx,
                                       p7->ctx.propq);
  ...
  if ((p12->authsafes->d.data = ASN1_OCTET_STRING_new()) == NULL) goto err;
  if (!PKCS12_pack_authsafes(p12, newsafes)) goto err;

  if (p12->mac != NULL) {
    if (!PKCS12_gen_mac(p12, newpass, -1, mac, &maclen)) goto err;
    X509_SIG_getm(p12->mac->dinfo, NULL, &macoct);
    if (!ASN1_OCTET_STRING_set(macoct, mac, maclen)) goto err;
  }
...
static int newpass_bag(PKCS12_SAFEBAG* bag, const char* oldpass,
  ...
  if (PKCS12_SAFEBAG_get_nid(bag) != NID_pkcs8ShroudedKeyBag) return 1;
...
static int alg_get(const X509_ALGOR* alg, int* pnid, int* piter, int* psaltlen,
  ...
  switch (pbenid) {
    case NID_pbes2:
      ...
      if (kdf->salt->type != V_ASN1_OCTET_STRING) goto done;
  ...
  if (kdf != NULL) PBKDF2PARAM_free(kdf);
  if (pbe2 != NULL) PBE2PARAM_free(pbe2);
