                         ...
                         const char* propq)
{
  ...
  if (!(p7->d.encrypted->enc_data->enc_data =
          PKCS12_item_i2d_encrypt_ex(pbe,
                                     ASN1_ITEM_rptr(PKCS12_SAFEBAGS),
                                     pass,
                                     passlen,
                                     bags,
...
PKCS12_pack_p7encdata(int pbe_nid,
                      ...
                      STACK_OF(PKCS12_SAFEBAG)* bags)
{
  return PKCS12_pack_p7encdata_ex(
    pbe_nid, pass, passlen, salt, saltlen, iter, bags, NULL, NULL);
}
...
PKCS8_PRIV_KEY_INFO*
PKCS12_decrypt_skey_ex(const PKCS12_SAFEBAG* bag,
                       const char* pass,
                       int passlen,
                       OSSL_LIB_CTX* ctx,
                       const char* propq)
...
{
  if (ASN1_item_pack(
        safes, ASN1_ITEM_rptr(PKCS12_AUTHSAFES), &p12->authsafes->d.data))
    return 1;
  return 0;
}
