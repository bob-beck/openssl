...
static int
parse_bags(const STACK_OF(PKCS12_SAFEBAG)* bags,
           const char* pass,
           int passlen,
           EVP_PKEY** pkey,
           STACK_OF(X509)* ocerts,
           ...
           STACK_OF(X509)* ocerts)
{
  ...
  for (i = 0; i < sk_PKCS7_num(asafes); i++) {
    ...
    if (!parse_bags(
          bags, pass, passlen, pkey, ocerts, p7->ctx.libctx, p7->ctx.propq)) {
          ...
          const char* propq)
{
  ...
  switch (PKCS12_SAFEBAG_get_nid(bag)) {
    ...
    case NID_safeContentsBag:
      return parse_bags(PKCS12_SAFEBAG_get0_safes(bag),
                        pass,
                        passlen,
                        pkey,
                        ocerts,
