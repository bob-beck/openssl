...
unsigned char* OPENSSL_asc2uni(const char* asc, int asclen, unsigned char** uni,
  ...
  if (asclen < 0) return NULL;
  ulen = asclen * 2 + 2;
  if ((unitmp = OPENSSL_malloc(ulen)) == NULL) return NULL;
...
char* OPENSSL_uni2asc(const unsigned char* uni, int unilen) {
  ...
  if (unilen & 1) return NULL;
  if (unilen < 0) return NULL;
  asclen = unilen / 2;
  /* If no terminating zero allow for one */
  if (!unilen || uni[unilen - 1]) asclen++;
  uni++;
  if ((asctmp = OPENSSL_malloc(asclen)) == NULL) return NULL;
...
unsigned char* OPENSSL_utf82uni(const char* asc, int asclen,
  ...
  for (ulen = 0, i = 0; i < asclen; i += j) {
     ...
     * decoding failure...
     */
    if (j < 0) return OPENSSL_asc2uni(asc, asclen, uni, unilen);
  ...
  if ((ret = OPENSSL_malloc(ulen)) == NULL) return NULL;
...
char* OPENSSL_uni2utf8(const unsigned char* uni, int unilen) {
  ...
  /* If no terminating zero allow for one */
  if (!unilen || (uni[unilen - 2] || uni[unilen - 1])) asclen++;
  ...
  if (!unilen || (uni[unilen - 2] || uni[unilen - 1])) asctmp[asclen] = '\0';
