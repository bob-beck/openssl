 ...
 */
static int
ossl_cms_signerinfo_get_signing_cert(const CMS_SignerInfo* si,
 ...
 */
static int
ossl_cms_signerinfo_get_signing_cert_v2(const CMS_SignerInfo* si,
...
CMS_ReceiptRequest*
CMS_ReceiptRequest_create0_ex(unsigned char* id,
                              int idlen,
                              int allorfirst,
...
CMS_ReceiptRequest*
CMS_ReceiptRequest_create0(unsigned char* id,
                           int idlen,
                           int allorfirst,
                           STACK_OF(GENERAL_NAMES)* receiptList,
                           STACK_OF(GENERAL_NAMES)* receiptsTo)
{
  return CMS_ReceiptRequest_create0_ex(
    id, idlen, allorfirst, receiptList, receiptsTo, NULL);
}
...
CMS_add1_ReceiptRequest(CMS_SignerInfo* si, CMS_ReceiptRequest* rr)
{
  ...
  if (!CMS_signed_add1_attr_by_NID(
        si, NID_id_smime_aa_receiptRequest, V_ASN1_SEQUENCE, rrder, rrderlen)) {
    ERR_raise(ERR_LIB_CMS, ERR_R_CMS_LIB);
    goto err;
  }
...
void
CMS_ReceiptRequest_get0_values(CMS_ReceiptRequest* rr,
                               ASN1_STRING** pcid,
                               int* pallorfirst,
...
cms_msgSigDigest(CMS_SignerInfo* si, unsigned char* dig, unsigned int* diglen)
{
  ...
  if (md == NULL)
    return 0;
  if (!ossl_asn1_item_digest_ex(ASN1_ITEM_rptr(CMS_Attributes_Verify),
                                md,
                                si->signedAttrs,
                                dig,
                                diglen,
...
ossl_cms_msgSigDigest_add1(CMS_SignerInfo* dest, CMS_SignerInfo* src)
{
  ...
  if (!CMS_signed_add1_attr_by_NID(
        dest, NID_id_smime_aa_msgSigDigest, V_ASN1_OCTET_STRING, dig, diglen)) {
    ERR_raise(ERR_LIB_CMS, ERR_R_CMS_LIB);
    return 0;
  }
  return 1;
