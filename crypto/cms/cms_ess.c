...
void CMS_ReceiptRequest_get0_values(CMS_ReceiptRequest* rr, ASN1_STRING** pcid,
  ...
  if (pcid != NULL) *pcid = rr->signedContentIdentifier;
  if (rr->receiptsFrom->type == 0) {
    if (pallorfirst != NULL)
      *pallorfirst = (int)rr->receiptsFrom->d.allOrFirstTier;
    if (plist != NULL) *plist = NULL;
  } else {
    if (pallorfirst != NULL) *pallorfirst = -1;
    if (plist != NULL) *plist = rr->receiptsFrom->d.receiptList;
  }
  if (prto != NULL) *prto = rr->receiptsTo;
}
...
int ossl_cms_Receipt_verify(CMS_ContentInfo* cms, CMS_ContentInfo* req_cms) {
  ...
  for (i = 0; i < sk_CMS_SignerInfo_num(osis); i++) {
    ...
    if (!ASN1_STRING_cmp(osi->signature, rct->originatorSignatureValue)) break;
  }
