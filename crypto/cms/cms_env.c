...
static int cms_get_enveloped_type(const CMS_ContentInfo* cms) {
  ...
  if (ret == 0) ERR_raise(ERR_LIB_CMS, CMS_R_CONTENT_TYPE_NOT_ENVELOPED_DATA);
  return ret;
}
...
static CMS_AuthEnvelopedData* cms_auth_enveloped_data_init(
    CMS_ContentInfo* cms) {
  if (cms->d.other == NULL) {
...
int ossl_cms_env_asn1_ctrl(CMS_RecipientInfo* ri, int cmd) {
  ...
  if (pkey->ameth == NULL || pkey->ameth->pkey_ctrl == NULL) return 1;
...
CMS_EncryptedContentInfo* ossl_cms_get0_env_enc_content(
...
CMS_ContentInfo* CMS_AuthEnvelopedData_create_ex(const EVP_CIPHER* cipher,
  ...
  cms = CMS_ContentInfo_new_ex(libctx, propq);
  if (cms == NULL) goto merr;
  aenv = cms_auth_enveloped_data_init(cms);
  if (aenv == NULL) goto merr;
...
static int cms_RecipientInfo_ktri_init(CMS_RecipientInfo* ri, X509* recip,
  ...
  if (!ossl_cms_set1_SignerIdentifier(ktri->rid, recip, idtype, ctx)) return 0;

  if (!X509_up_ref(recip)) return 0;
...
CMS_RecipientInfo* CMS_add1_recipient(CMS_ContentInfo* cms, X509* recip,
  ...
  switch (ossl_cms_pkey_get_ri_type(pk)) {
    case CMS_RECIPINFO_TRANS:
      if (!cms_RecipientInfo_ktri_init(ri, recip, pk, flags, ctx)) goto err;
      break;
...
int CMS_RecipientInfo_ktri_get0_algs(CMS_RecipientInfo* ri, EVP_PKEY** pk,
  ...
  if (pk) *pk = ktri->pkey;
  if (recip) *recip = ktri->recip;
  if (palg) *palg = ktri->keyEncryptionAlgorithm;
  return 1;
}
...
static int cms_RecipientInfo_ktri_encrypt(const CMS_ContentInfo* cms,
  ...
  if (pctx) {
    if (!ossl_cms_env_asn1_ctrl(ri, 0)) goto err;
  } else {
  ...
  if (EVP_PKEY_encrypt(pctx, NULL, &eklen, ec->key, ec->keylen) <= 0) goto err;

  ek = OPENSSL_malloc(eklen);
  if (ek == NULL) goto err;

  if (EVP_PKEY_encrypt(pctx, ek, &eklen, ec->key, ec->keylen) <= 0) goto err;
...
static int cms_RecipientInfo_ktri_decrypt(CMS_ContentInfo* cms,
  ...
  if (ktri->pctx == NULL) goto err;

  if (EVP_PKEY_decrypt_init(ktri->pctx) <= 0) goto err;
...
CMS_RecipientInfo* CMS_add0_recipient_key(CMS_ContentInfo* cms, int nid,
  ...
  if (ris == NULL || idlen > INT_MAX) goto err;
...
int CMS_RecipientInfo_kekri_get0_id(CMS_RecipientInfo* ri, X509_ALGOR** palg,
  ...
  if (palg) *palg = ri->d.kekri->keyEncryptionAlgorithm;
  if (pid) *pid = rkid->keyIdentifier;
  if (pdate) *pdate = rkid->date;
  if (potherid) {
    if (rkid->other)
...
static void cms_env_set_originfo_version(CMS_EnvelopedData* env) {
  ...
  for (i = 0; i < sk_CMS_CertificateChoices_num(org->certificates); i++) {
    ...
    if (cch->type == CMS_CERTCHOICE_OTHER) {
      ...
      if (env->version < 3) env->version = 3;
    }
  }
...
static void cms_env_set_version(CMS_EnvelopedData* env) {
   ...
   */
  if (env->version >= 4) return;

  cms_env_set_originfo_version(env);

  if (env->version >= 3) return;
  ...
  if (env->originatorInfo || env->unprotectedAttrs) env->version = 2;
...
static int cms_env_encrypt_content_key(const CMS_ContentInfo* cms,
  ...
  for (i = 0; i < sk_CMS_RecipientInfo_num(ris); i++) {
    ...
    if (CMS_RecipientInfo_encrypt(cms, ri) <= 0) return -1;
  }
  return 1;
}
...
static BIO* cms_EnvelopedData_Decryption_init_bio(CMS_ContentInfo* cms) {
  ...
  if (contentBio == NULL) return NULL;
...
BIO* ossl_cms_AuthEnvelopedData_init_bio(CMS_ContentInfo* cms) {
  ...
  if (ret == NULL || ec->cipher == NULL) return ret;
...
int ossl_cms_AuthEnvelopedData_final(CMS_ContentInfo* cms, BIO* cmsbio) {
   ...
   * decryption.
   */
  if (!EVP_CIPHER_CTX_is_encrypting(ctx)) return 1;
...
int ossl_cms_RecipientInfo_wrap_init(CMS_RecipientInfo* ri,
  ...
  if (kekcipher != NULL) {
    if (EVP_CIPHER_CTX_get_mode(ctx) != EVP_CIPH_WRAP_MODE) return 0;
    return 1;
  }
  ...
  if ((EVP_CIPHER_get_flags(cipher) & EVP_CIPH_FLAG_GET_WRAP_CIPHER) != 0) {
    ...
    if (kekcipher != NULL) {
      if (EVP_CIPHER_get_mode(kekcipher) != EVP_CIPH_WRAP_MODE) return 0;
