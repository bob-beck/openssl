...
static int
cms_get_enveloped_type_simple(const CMS_ContentInfo* cms)
{
...
static int
cms_get_enveloped_type(const CMS_ContentInfo* cms)
{
                            ...
                            const char* propq)
{
  ...
  if (!ossl_cms_EncryptedContent_init(
        env->encryptedContentInfo, cipher, NULL, 0, ossl_cms_get0_cmsctx(cms)))
    goto err;
  return cms;
err:
...
BIO*
CMS_EnvelopedData_decrypt(CMS_EnvelopedData* env,
                          BIO* detached_data,
                          EVP_PKEY* pkey,
                          X509* cert,
                          ...
                          const char* propq)
{
  ...
  if (secret != NULL &&
      CMS_decrypt_set1_password(ci,
                                (unsigned char*)ASN1_STRING_get0_data(secret),
                                ASN1_STRING_length(secret)) != 1)
    goto end;
  res = CMS_decrypt(ci,
                    secret == NULL ? pkey : NULL,
                    secret == NULL ? cert : NULL,
                    detached_data,
                    bio,
                    flags);
...
static int
cms_RecipientInfo_ktri_init(CMS_RecipientInfo* ri,
                            X509* recip,
                            EVP_PKEY* pk,
                            unsigned int flags,
                            const CMS_CTX* ctx)
...
CMS_RecipientInfo*
CMS_add1_recipient(CMS_ContentInfo* cms,
                   X509* recip,
                   ...
                   unsigned int flags)
{
  ...
  switch (ossl_cms_pkey_get_ri_type(pk)) {
    ...
    case CMS_RECIPINFO_AGREE:
      if (!ossl_cms_RecipientInfo_kari_init(
            ri, recip, pk, originator, originatorPrivKey, flags, ctx))
        goto err;
      break;
...
int
CMS_RecipientInfo_ktri_get0_algs(CMS_RecipientInfo* ri,
                                 EVP_PKEY** pk,
                                 X509** recip,
                                 X509_ALGOR** palg)
{
                                      ...
                                      ASN1_INTEGER** sno)
{
  ...
  return ossl_cms_SignerIdentifier_get0_signer_id(
    ktri->rid, keyid, issuer, sno);
}
...
static int
cms_RecipientInfo_ktri_encrypt(const CMS_ContentInfo* cms,
                               CMS_RecipientInfo* ri)
{
  CMS_KeyTransRecipientInfo* ktri;
...
cms_RecipientInfo_ktri_decrypt(CMS_ContentInfo* cms, CMS_RecipientInfo* ri)
{
  ...
  if (evp_pkey_decrypt_alloc(ktri->pctx,
                             &ek,
                             &eklen,
                             fixlen,
...
int
CMS_RecipientInfo_kekri_id_cmp(CMS_RecipientInfo* ri,
...
CMS_RecipientInfo*
CMS_add0_recipient_key(CMS_ContentInfo* cms,
                       int nid,
                       unsigned char* key,
                       size_t keylen,
                       unsigned char* id,
                       ...
                       ASN1_TYPE* otherType)
{
  ...
  (void)X509_ALGOR_set0(kekri->keyEncryptionAlgorithm,
                        OBJ_nid2obj(nid),
                        V_ASN1_UNDEF,
                        NULL); /* cannot fail */
...
int
CMS_RecipientInfo_kekri_get0_id(CMS_RecipientInfo* ri,
                                X509_ALGOR** palg,
...
int
CMS_RecipientInfo_set0_key(CMS_RecipientInfo* ri,
                           unsigned char* key,
                           size_t keylen)
{
  CMS_KEKRecipientInfo* kekri;
...
static EVP_CIPHER*
cms_get_key_wrap_cipher(size_t keylen, const CMS_CTX* ctx)
{
  const char* alg = NULL;
  ...
  return EVP_CIPHER_fetch(
    ossl_cms_ctx_get0_libctx(ctx), alg, ossl_cms_ctx_get0_propq(ctx));
}

/* Encrypt content key in KEK recipient info */

static int
cms_RecipientInfo_kekri_encrypt(const CMS_ContentInfo* cms,
                                CMS_RecipientInfo* ri)
{
  CMS_EncryptedContentInfo* ec;
  CMS_KEKRecipientInfo* kekri;
...
int
CMS_RecipientInfo_encrypt(const CMS_ContentInfo* cms, CMS_RecipientInfo* ri)
{
  switch (ri->type) {
    case CMS_RECIPINFO_TRANS:
...
static int
cms_env_encrypt_content_key(const CMS_ContentInfo* cms,
                            STACK_OF(CMS_RecipientInfo)* ris)
{
  int i;
  CMS_RecipientInfo* ri;
...
static void
cms_env_clear_ec(CMS_EncryptedContentInfo* ec)
{
  ec->cipher = NULL;
  OPENSSL_clear_free(ec->key, ec->keylen);
  ec->key = NULL;
...
static BIO*
cms_EnvelopedData_Decryption_init_bio(CMS_ContentInfo* cms)
{
...
static BIO*
cms_EnvelopedData_Encryption_init_bio(CMS_ContentInfo* cms)
{
  CMS_EncryptedContentInfo* ec;
...
ossl_cms_EnvelopedData_final(CMS_ContentInfo* cms, BIO* chain)
{
  ...
  if ((EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(ctx)) &
    ...
    if (EVP_CIPHER_CTX_ctrl(
          ctx, EVP_CTRL_PROCESS_UNPROTECTED, 1, env->unprotectedAttrs) <= 0) {
      ERR_raise(ERR_LIB_CMS, CMS_R_CTRL_FAILURE);
      return 0;
    }
  }
...
ossl_cms_pkey_is_ri_type_supported(EVP_PKEY* pk, int ri_type)
{
  ...
  if (pk->ameth != NULL && pk->ameth->pkey_ctrl != NULL) {
    ...
    i = pk->ameth->pkey_ctrl(
      pk, ASN1_PKEY_CTRL_CMS_IS_RI_TYPE_SUPPORTED, ri_type, &r);
    if (i > 0)
      return r;
  }
                                 ...
                                 const EVP_CIPHER* cipher)
{
  ...
  if ((EVP_CIPHER_get_flags(cipher) & EVP_CIPH_FLAG_GET_WRAP_CIPHER) != 0) {
    ret = EVP_CIPHER_meth_get_ctrl(cipher)(
      NULL, EVP_CTRL_GET_WRAP_CIPHER, 0, &kekcipher);
    if (ret <= 0)
      return 0;
