                               ...
                               ASN1_OCTET_STRING *secret, unsigned int flags, OSSL_LIB_CTX *libctx, const char *propq)
{
    ...
    if (secret != NULL &&
        CMS_decrypt_set1_password(ci, (unsigned char *)ASN1_STRING_get0_data(secret), ASN1_STRING_length(secret)) != 1)
        goto end;
...
static void cms_env_set_version(CMS_EnvelopedData *env)
{
    ...
    for (i = 0; i < sk_CMS_RecipientInfo_num(env->recipientInfos); i++)
    {
        ...
        if (ri->type == CMS_RECIPINFO_PASS || ri->type == CMS_RECIPINFO_OTHER || ri->type == CMS_RECIPINFO_KEM)
        {
            env->version = 3;
            return;
        }
...
int ossl_cms_pkey_get_ri_type(EVP_PKEY *pk)
{
     ...
     * and/or RSA-KEM.
     */
    if (evp_pkey_is_provided(pk) && EVP_PKEY_get_int_param(pk, OSSL_PKEY_PARAM_CMS_RI_TYPE, &ri_type))
        return ri_type;
