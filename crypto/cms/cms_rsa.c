...
static int rsa_cms_decrypt(CMS_RecipientInfo* ri) {
  ...
  if (pkctx == NULL) return 0;
  if (!CMS_RecipientInfo_ktri_get0_algs(ri, NULL, NULL, &cmsalg)) return -1;
  nid = OBJ_obj2nid(cmsalg->algorithm);
  if (nid == NID_rsaEncryption) return 1;
  ...
  if (EVP_PKEY_CTX_set_rsa_padding(pkctx, RSA_PKCS1_OAEP_PADDING) <= 0)
    goto err;
  if (EVP_PKEY_CTX_set_rsa_oaep_md(pkctx, md) <= 0) goto err;
...
static int rsa_cms_encrypt(CMS_RecipientInfo* ri) {
  ...
  if (CMS_RecipientInfo_ktri_get0_algs(ri, NULL, NULL, &alg) <= 0) return 0;
  if (pkctx != NULL) {
  ...
  /* Not supported */
  if (pad_mode != RSA_PKCS1_OAEP_PADDING) return 0;
  if (EVP_PKEY_CTX_get_rsa_oaep_md(pkctx, &md) <= 0) goto err;
  if (EVP_PKEY_CTX_get_rsa_mgf1_md(pkctx, &mgf1md) <= 0) goto err;
  labellen = EVP_PKEY_CTX_get0_rsa_oaep_label(pkctx, &label);
  if (labellen < 0) goto err;
  oaep = RSA_OAEP_PARAMS_new();
  if (oaep == NULL) goto err;
  if (!ossl_x509_algor_new_from_md(&oaep->hashFunc, md)) goto err;
  if (!ossl_x509_algor_md_to_mgf1(&oaep->maskGenFunc, mgf1md)) goto err;
  if (labellen > 0) {
    los = ASN1_OCTET_STRING_new();

    if (los == NULL) goto err;
    if (!ASN1_OCTET_STRING_set(los, label, labellen)) goto err;

    oaep->pSourceFunc =
        ossl_X509_ALGOR_from_nid(NID_pSpecified, V_ASN1_OCTET_STRING, los);
    if (oaep->pSourceFunc == NULL) goto err;

    los = NULL;
  }
  /* create string with oaep parameter encoding. */
  if (!ASN1_item_pack(oaep, ASN1_ITEM_rptr(RSA_OAEP_PARAMS), &os)) goto err;
...
int ossl_cms_rsa_envelope(CMS_RecipientInfo* ri, int decrypt) {
  ...
  if (decrypt == 1) return rsa_cms_decrypt(ri);

  if (decrypt == 0) return rsa_cms_encrypt(ri);
...
static int rsa_cms_sign(CMS_SignerInfo* si) {
  ...
  /* We don't support it */
  if (pad_mode != RSA_PKCS1_PSS_PADDING) return 0;
  ...
  if (d2i_X509_ALGOR(&alg, &pp, (long)aid_len) == NULL) return 0;
  return 1;
}
...
static int rsa_cms_verify(CMS_SignerInfo* si) {
  ...
  if (OBJ_find_sigid_algs(nid, NULL, &nid2)) {
    if (nid2 == NID_rsaEncryption) return 1;
  }
  return 0;
}
...
int ossl_cms_rsa_sign(CMS_SignerInfo* si, int verify) {
  ...
  if (verify == 1) return rsa_cms_verify(si);

  if (verify == 0) return rsa_cms_sign(si);
