...
int
CMS_digest_verify(CMS_ContentInfo* cms,
                  BIO* dcont,
                  BIO* out,
                  unsigned int flags)
{
...
int
CMS_EncryptedData_decrypt(CMS_ContentInfo* cms,
...
CMS_ContentInfo*
CMS_EncryptedData_encrypt_ex(BIO* in,
...
CMS_ContentInfo*
CMS_EncryptedData_encrypt(BIO* in,
                          const EVP_CIPHER* cipher,
                          const unsigned char* key,
                          size_t keylen,
                          unsigned int flags)
{
  return CMS_EncryptedData_encrypt_ex(
    in, cipher, key, keylen, flags, NULL, NULL);
}

static int
cms_signerinfo_verify_cert(CMS_SignerInfo* si,
                           X509_STORE* store,
...
int
CMS_verify(CMS_ContentInfo* cms,
           STACK_OF(X509)* certs,
           X509_STORE* store,
           BIO* dcont,
           BIO* out,
  ...
  if ((flags & CMS_NO_SIGNER_CERT_VERIFY) == 0 || cadesVerify) {
    ...
    for (i = 0; i < scount; i++) {
      ...
      if (!cms_signerinfo_verify_cert(
            si, store, untrusted, crls, si_chains ? &si_chains[i] : NULL, ctx))
        goto err;
    }
  }
...
int
CMS_verify_receipt(CMS_ContentInfo* rcms,
...
CMS_ContentInfo*
CMS_sign_receipt(CMS_SignerInfo* si,
                 X509* signcert,
                 EVP_PKEY* pkey,
                 STACK_OF(X509)* certs,
                 unsigned int flags)
...
CMS_ContentInfo*
CMS_encrypt_ex(STACK_OF(X509)* certs,
               BIO* data,
               const EVP_CIPHER* cipher,
               unsigned int flags,
               OSSL_LIB_CTX* libctx,
...
CMS_ContentInfo*
CMS_encrypt(STACK_OF(X509)* certs,
            BIO* data,
            const EVP_CIPHER* cipher,
            unsigned int flags)
{
...
static int
cms_kari_set1_pkey_and_peer(CMS_ContentInfo* cms,
...
int
CMS_decrypt_set1_pkey_and_peer(CMS_ContentInfo* cms,
                               EVP_PKEY* pk,
                               X509* cert,
                               X509* peer)
{
...
int
CMS_decrypt_set1_key(CMS_ContentInfo* cms,
                     unsigned char* key,
                     size_t keylen,
...
int
CMS_decrypt_set1_password(CMS_ContentInfo* cms,
                          unsigned char* pass,
...
int
CMS_decrypt(CMS_ContentInfo* cms,
            EVP_PKEY* pk,
            X509* cert,
            BIO* dcont,
            BIO* out,
...
int
CMS_final_digest(CMS_ContentInfo* cms,
