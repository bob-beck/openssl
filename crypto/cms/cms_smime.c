  ...
  for (;;) {
    ...
    if (i <= 0) {
      if (BIO_method_type(in) == BIO_TYPE_CIPHER) {
        if (BIO_get_cipher_status(in) <= 0) goto err;
      }
      if (i < 0) goto err;
      break;
    }

    if (tmpout != NULL && (BIO_write(tmpout, buf, i) != i)) goto err;
  }
...
CMS_ContentInfo* CMS_data_create_ex(BIO* in, unsigned int flags,
  ...
  if ((flags & CMS_STREAM) || CMS_final(cms, in, NULL, flags)) return cms;
...
int CMS_digest_verify(CMS_ContentInfo* cms, BIO* dcont, BIO* out,
  ...
  r = cms_copy_content(out, cont, flags);
  if (r) r = ossl_cms_DigestedData_do_final(cms, cont, 1);
...
CMS_ContentInfo* CMS_digest_create_ex(BIO* in, const EVP_MD* md,
  ...
  if (!(flags & CMS_DETACHED)) CMS_set_detached(cms, 0);
...
CMS_ContentInfo* CMS_EncryptedData_encrypt_ex(BIO* in, const EVP_CIPHER* cipher,
  ...
  if (cms == NULL) return NULL;
  if (!CMS_EncryptedData_set1_key(cms, cipher, key, keylen)) goto err;
...
static int cms_signerinfo_verify_cert(CMS_SignerInfo* si, X509_STORE* store,
  ...
  if (crls != NULL) X509_STORE_CTX_set0_crls(ctx, crls);
  ...
  if (chain != NULL) *chain = X509_STORE_CTX_get1_chain(ctx);
err:
...
int CMS_verify(CMS_ContentInfo* cms, STACK_OF(X509)* certs, X509_STORE* store,
  ...
  if ((flags & CMS_NO_SIGNER_CERT_VERIFY) == 0 || cadesVerify) {
    if (cadesVerify) {
      ...
      if (si_chains == NULL) goto err;
    }
    if (!ossl_cms_get1_certs_ex(cms, &untrusted)) goto err;
  ...
  if ((flags & CMS_NO_ATTR_VERIFY) == 0 || cadesVerify) {
    for (i = 0; i < scount; i++) {
      si = sk_CMS_SignerInfo_value(sinfos, i);
      if (CMS_signed_get_attr_count(si) < 0) continue;
      if (CMS_SignerInfo_verify(si) <= 0) goto err;
      if (cadesVerify) {
        STACK_OF(X509)* si_chain = si_chains ? si_chains[i] : NULL;

        if (ossl_cms_check_signing_certs(si, si_chain) <= 0) goto err;
      }
    }
  }
  ...
  if (!(flags & SMIME_BINARY) && dcont) {
    ...
    if (cmsbio == NULL) goto err;
    /*
     * Don't use SMIME_TEXT for verify: it adds headers and we want to
     * remove them.
     */
    if (!SMIME_crlf_copy(dcont, cmsbio, flags & ~SMIME_TEXT)) goto err;
    ...
    if (!cms_copy_content(out, cmsbio, flags)) goto err;
  }
...
err:
  ...
  if (out != tmpout) BIO_free_all(tmpout);
...
CMS_ContentInfo* CMS_sign_receipt(CMS_SignerInfo* si, X509* signcert,
  ...
  if (rct_cont == NULL) goto err;

  /* Add msgSigDigest attribute */

  if (!ossl_cms_msgSigDigest_add1(rct_si, si)) goto err;

  /* Finalize structure */
  if (!CMS_final(cms, rct_cont, NULL, flags)) goto err;
...
static int cms_kari_set1_pkey_and_peer(CMS_ContentInfo* cms,
  ...
  for (i = 0; i < sk_CMS_RecipientEncryptedKey_num(reks); i++) {
    ...
    if (cert != NULL && CMS_RecipientEncryptedKey_cert_cmp(rek, cert)) continue;
...
int CMS_decrypt_set1_pkey_and_peer(CMS_ContentInfo* cms, EVP_PKEY* pk,
  ...
  if (ris != NULL && ec != NULL) debug = ec->debug;
  ...
  for (i = 0; i < sk_CMS_RecipientInfo_num(ris); i++) {
    ...
    if (!ossl_cms_pkey_is_ri_type_supported(pk, ri_type)) continue;
    match_ri = 1;
  ...
  if (!match_ri) ERR_raise(ERR_LIB_CMS, CMS_R_NO_MATCHING_RECIPIENT);
  return 0;
}
...
int CMS_decrypt_set1_key(CMS_ContentInfo* cms, unsigned char* key,
  ...
  for (i = 0; i < sk_CMS_RecipientInfo_num(ris); i++) {
    ri = sk_CMS_RecipientInfo_value(ris, i);
    if (CMS_RecipientInfo_type(ri) != CMS_RECIPINFO_KEK) continue;
...
int CMS_decrypt_set1_password(CMS_ContentInfo* cms, unsigned char* pass,
  ...
  for (i = 0; i < sk_CMS_RecipientInfo_num(ris); i++) {
    ri = sk_CMS_RecipientInfo_value(ris, i);
    if (CMS_RecipientInfo_type(ri) != CMS_RECIPINFO_PASS) continue;
...
int CMS_decrypt(CMS_ContentInfo* cms, EVP_PKEY* pk, X509* cert, BIO* dcont,
  ...
  if (pk == NULL && cert == NULL && dcont == NULL && out == NULL) return 1;
  if (pk != NULL && !CMS_decrypt_set1_pkey(cms, pk, cert)) return 0;
  cont = CMS_dataInit(cms, dcont);
  if (cont == NULL) return 0;
  r = cms_copy_content(out, cont, flags);
  do_free_upto(cont, dcont);
...
CMS_ContentInfo* CMS_compress(BIO* in, int comp_nid, unsigned int flags) {
  ...
  if (comp_nid <= 0) comp_nid = NID_zlib_compression;
