...
BIO* ossl_cms_EncryptedContent_init_bio(CMS_EncryptedContentInfo* ec,
  ...
  if (cipher != NULL) {
    ...
    if (fetched_ciph != NULL) cipher = fetched_ciph;
  }
  if (cipher == NULL) {
  ...
  if (!enc || !ec->key) {
    ...
    if (EVP_CIPHER_CTX_rand_key(ctx, tkey) <= 0) goto err;
  }
...
int ossl_cms_EncryptedContent_init(CMS_EncryptedContentInfo* ec,
  ...
  ec->cipher = cipher;
  if (key) {
    if ((ec->key = OPENSSL_malloc(keylen)) == NULL) return 0;
    memcpy(ec->key, key, keylen);
  }
  ec->keylen = keylen;
  if (cipher != NULL) ec->contentType = OBJ_nid2obj(NID_pkcs7_data);
  return 1;
}
