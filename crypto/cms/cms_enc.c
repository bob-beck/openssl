                                   ...
                                   const CMS_CTX* cms_ctx)
{
  ...
  if (enc) {
    ...
    if ((EVP_CIPHER_get_flags(cipher) & EVP_CIPH_FLAG_AEAD_CIPHER)) {
      ...
      if (ec->taglen > 0 &&
          EVP_CIPHER_CTX_ctrl(
            ctx, EVP_CTRL_AEAD_SET_TAG, (int)ec->taglen, ec->tag) <= 0) {
...
int
CMS_EncryptedData_set1_key(CMS_ContentInfo* cms,
                           ...
                           size_t keylen)
{
  ...
  return ossl_cms_EncryptedContent_init(
    ec, ciph, key, keylen, ossl_cms_get0_cmsctx(cms));
}
