...
static int ecdh_cms_set_peerkey(EVP_PKEY_CTX* pctx, X509_ALGOR* alg,
  ...
  if (OBJ_obj2nid(aoid) != NID_X9_62_id_ecPublicKey) goto err;
  ...
  if (atype == V_ASN1_UNDEF || atype == V_ASN1_NULL) {
    ...
    pkpeer = EVP_PKEY_new();
    if (pkpeer == NULL) goto err;
    if (!EVP_PKEY_copy_parameters(pkpeer, pk)) goto err;
  } else {
  ...
  if (EVP_PKEY_set1_encoded_public_key(pkpeer, p, plen) <= 0) goto err;
...
static int ecdh_cms_set_kdf_param(EVP_PKEY_CTX* pctx, int eckdf_nid) {
  ...
  if (eckdf_nid == NID_undef) return 0;

  /* Lookup KDF type, cofactor mode and digest */
  if (!OBJ_find_sigid_algs(eckdf_nid, &kdfmd_nid, &kdf_nid)) return 0;
...
static int ecdh_cms_set_shared_info(EVP_PKEY_CTX* pctx, CMS_RecipientInfo* ri) {
  ...
  if (!CMS_RecipientInfo_kari_get0_alg(ri, &alg, &ukm)) return 0;
  ...
  if (EVP_PKEY_CTX_set_ecdh_kdf_outlen(pctx, keylen) <= 0) goto err;

  plen = CMS_SharedInfo_encode(&der, kekalg, ukm, keylen);

  if (plen <= 0) goto err;

  if (EVP_PKEY_CTX_set0_ecdh_kdf_ukm(pctx, der, plen) <= 0) goto err;
  der = NULL;
...
static int ecdh_cms_encrypt(CMS_RecipientInfo* ri) {
  ...
  if (aoid == OBJ_nid2obj(NID_undef)) {
    ...
    if (enckeylen > INT_MAX || enckeylen == 0) goto err;
  ...
  if (kdf_type <= 0) goto err;
  if (EVP_PKEY_CTX_get_ecdh_kdf_md(pctx, &kdf_md) <= 0) goto err;
  ...
  if (kdf_type == EVP_PKEY_ECDH_KDF_NONE) {
    ...
    if (EVP_PKEY_CTX_set_ecdh_kdf_type(pctx, kdf_type) <= 0) goto err;
  } else
    /* Unknown KDF */
    goto err;
  if (kdf_md == NULL) {
    ...
    kdf_md = EVP_sha1();
    if (EVP_PKEY_CTX_set_ecdh_kdf_md(pctx, kdf_md) <= 0) goto err;
  }
  ...
  if (EVP_PKEY_CTX_set0_ecdh_kdf_ukm(pctx, penc, penclen) <= 0) goto err;
  penc = NULL;
...
int ossl_cms_ecdh_envelope(CMS_RecipientInfo* ri, int decrypt) {
  ...
  if (decrypt == 1) return ecdh_cms_decrypt(ri);

  if (decrypt == 0) return ecdh_cms_encrypt(ri);
