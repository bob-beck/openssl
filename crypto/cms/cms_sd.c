...
static void cms_sd_set_version(CMS_SignedData* sd) {
  ...
  for (i = 0; i < sk_CMS_CertificateChoices_num(sd->certificates); i++) {
    ...
    if (cch->type == CMS_CERTCHOICE_OTHER) {
      if (sd->version < 5) sd->version = 5;
    } else if (cch->type == CMS_CERTCHOICE_V2ACERT) {
      if (sd->version < 4) sd->version = 4;
    } else if (cch->type == CMS_CERTCHOICE_V1ACERT) {
      if (sd->version < 3) sd->version = 3;
    }
  }
...
static int cms_copy_messageDigest(CMS_ContentInfo* cms, CMS_SignerInfo* si) {
  ...
  for (i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
    ...
    if (sitmp == si) continue;
    if (CMS_signed_get_attr_count(sitmp) < 0) continue;
...
int ossl_cms_set1_SignerIdentifier(CMS_SignerIdentifier* sid, X509* cert,
  ...
  switch (type) {
    case CMS_SIGNERINFO_ISSUER_SERIAL:
      if (!ossl_cms_set1_ias(&sid->d.issuerAndSerialNumber, cert)) return 0;
      break;
...
int ossl_cms_SignerIdentifier_get0_signer_id(CMS_SignerIdentifier* sid,
  ...
  if (sid->type == CMS_SIGNERINFO_ISSUER_SERIAL) {
    if (issuer) *issuer = sid->d.issuerAndSerialNumber->issuer;
    if (sno) *sno = sid->d.issuerAndSerialNumber->serialNumber;
  } else if (sid->type == CMS_SIGNERINFO_KEYIDENTIFIER) {
    if (keyid) *keyid = sid->d.subjectKeyIdentifier;
  } else {
    return 0;
  }
  return 1;
...
static int cms_generic_sign(CMS_SignerInfo* si, int verify) {
  if (!ossl_assert(verify == 0 || verify == 1)) return -1;
  ...
  if (!verify) {
    ...
    if (pknid <= 0) { /* check whether a provider registered a NID */
      ...
      if (typename != NULL) pknid = OBJ_txt2nid(typename);
    }
...
static int ossl_cms_add1_signing_cert(CMS_SignerInfo* si,
  ...
  if (len <= 0 || (pp = OPENSSL_malloc(len)) == NULL) return 0;
...
CMS_SignerInfo* CMS_add1_signer(CMS_ContentInfo* cms, X509* signer,
  ...
  if (!X509_up_ref(signer)) goto err;
  ...
  if (!ossl_cms_set1_SignerIdentifier(si->sid, signer, type, ctx)) goto err;
  ...
  if (!(flags & CMS_NOATTR)) {
    ...
    if (!(flags & CMS_NOSMIMECAP)) {
      ...
      if (i) i = CMS_add_smimecap(si, smcap);
    ...
    if (flags & CMS_REUSE_DIGEST) {
      if (!cms_copy_messageDigest(cms, si)) goto err;
      if (!cms_set_si_contentType_attr(cms, si)) goto err;
  ...
  if (flags & CMS_KEY_PARAM) {
    if (flags & CMS_NOATTR) {
      ...
      if (si->pctx == NULL) goto err;
      if (EVP_PKEY_sign_init(si->pctx) <= 0) goto err;
      if (EVP_PKEY_CTX_set_signature_md(si->pctx, md) <= 0) goto err;
  ...
  if (sd->signerInfos == NULL) sd->signerInfos = sk_CMS_SignerInfo_new_null();
...
int CMS_set1_signers_certs(CMS_ContentInfo* cms, STACK_OF(X509)* scerts,
  ...
  for (i = 0; i < sk_CMS_SignerInfo_num(sd->signerInfos); i++) {
    ...
    if (si->signer != NULL || (flags & CMS_NOINTERN)) continue;

    for (j = 0; j < sk_CMS_CertificateChoices_num(certs); j++) {
      cch = sk_CMS_CertificateChoices_value(certs, j);
      if (cch->type != CMS_CERTCHOICE_CERT) continue;
      x = cch->d.certificate;
      if (CMS_SignerInfo_cert_cmp(si, x) == 0) {
        CMS_SignerInfo_set1_signer_cert(si, x);
        ret++;
...
void CMS_SignerInfo_get0_algs(CMS_SignerInfo* si, EVP_PKEY** pk, X509** signer,
  ...
  if (pk != NULL) *pk = si->pkey;
  if (signer != NULL) *signer = si->signer;
  if (pdig != NULL) *pdig = si->digestAlgorithm;
  if (psig != NULL) *psig = si->signatureAlgorithm;
}
...
static int cms_SignerInfo_content_sign(CMS_ContentInfo* cms, CMS_SignerInfo* si,
  ...
  if (si->pctx && !cms_sd_asn1_ctrl(si, 0)) goto err;
  ...
  if (CMS_signed_get_attr_count(si) >= 0) {
    ...
    if (md == NULL) {
      if (!EVP_DigestFinal_ex(mctx, computed_md, &mdlen)) goto err;
      md = computed_md;
    }
    if (!CMS_signed_add1_attr_by_NID(si, NID_pkcs9_messageDigest,
    ...
    if (!CMS_SignerInfo_sign(si)) goto err;
  } else if (si->pctx) {
    unsigned char* sig;
    size_t siglen;
    unsigned char computed_md[EVP_MAX_MD_SIZE];
    ...
    if (siglen == 0 || (sig = OPENSSL_malloc(siglen)) == NULL) goto err;
...
int CMS_SignerInfo_sign(CMS_SignerInfo* si) {
  ...
  if (!si->omit_signing_time &&
    ...
    if (!cms_add1_signingTime(si, NULL)) goto err;
  }

  if (!ossl_cms_si_check_attributes(si)) goto err;
  ...
  if (alen < 0 || abuf == NULL) goto err;
  if (EVP_DigestSignUpdate(mctx, abuf, alen) <= 0) goto err;
  if (EVP_DigestSignFinal(mctx, NULL, &siglen) <= 0) goto err;
  OPENSSL_free(abuf);
  abuf = OPENSSL_malloc(siglen);
  if (abuf == NULL) goto err;
  if (EVP_DigestSignFinal(mctx, abuf, &siglen) <= 0) goto err;
...
int CMS_SignerInfo_verify(CMS_SignerInfo* si) {
  ...
  if (!ossl_cms_si_check_attributes(si)) return -1;
  ...
  if (cms_signature_nomd(si->pkey)) {
    ...
    if (r <= 0) ERR_raise(ERR_LIB_CMS, CMS_R_VERIFICATION_FAILURE);
    return r;
  }
  ...
  if (!cms_sd_asn1_ctrl(si, 1)) goto err;

  alen = ASN1_item_i2d((ASN1_VALUE*)si->signedAttrs, &abuf,
                       ASN1_ITEM_rptr(CMS_Attributes_Verify));
  if (abuf == NULL || alen < 0) goto err;
...
BIO* ossl_cms_SignedData_init_bio(CMS_ContentInfo* cms) {
  ...
  if (cms->d.signedData->encapContentInfo->partial) cms_sd_set_version(sd);
  for (i = 0; i < sk_X509_ALGOR_num(sd->digestAlgorithms); i++) {
...
int CMS_SignerInfo_verify_content(CMS_SignerInfo* si, BIO* chain) {
  ...
  if (os != NULL) {
    ...
    pkctx = EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(ctx), si->pkey,
                                       ossl_cms_ctx_get0_propq(ctx));
    if (pkctx == NULL) goto err;
    if (EVP_PKEY_verify_init(pkctx) <= 0) goto err;
    if (EVP_PKEY_CTX_set_signature_md(pkctx, md) <= 0) goto err;
    si->pctx = pkctx;
...
BIO* CMS_SignedData_verify(CMS_SignedData* sd, BIO* detached_data,
  ...
  if ((ci = CMS_ContentInfo_new_ex(libctx, propq)) == NULL) return NULL;
  if ((bio = BIO_new(BIO_s_mem())) == NULL) goto end;
  ci->contentType = OBJ_nid2obj(NID_pkcs7_signed);
  ci->d.signedData = sd;

  for (i = 0; i < sk_X509_num(extra); i++)
    if (!CMS_add1_cert(ci, sk_X509_value(extra, i))) goto end;
  for (i = 0; i < sk_X509_CRL_num(crls); i++)
    if (!CMS_add1_crl(ci, sk_X509_CRL_value(crls, i))) goto end;
  res = CMS_verify(ci, scerts, store, detached_data, bio, flags);

end:
  if (ci != NULL) ci->d.signedData = NULL; /* do not indirectly free |sd| */
...
int CMS_add_simple_smimecap(STACK_OF(X509_ALGOR)** algs, int algnid,
  ...
  if (*algs == NULL) *algs = sk_X509_ALGOR_new_null();
...
static int cms_add_cipher_smcap(STACK_OF(X509_ALGOR)** sk, int nid, int arg) {
  if (EVP_get_cipherbynid(nid)) return CMS_add_simple_smimecap(sk, nid, arg);
  return 1;
}

static int cms_add_digest_smcap(STACK_OF(X509_ALGOR)** sk, int nid, int arg) {
  if (EVP_get_digestbynid(nid)) return CMS_add_simple_smimecap(sk, nid, arg);
  return 1;
}
