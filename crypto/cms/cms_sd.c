 ...
 */
static int
cms_set_si_contentType_attr(CMS_ContentInfo* cms, CMS_SignerInfo* si)
{
  ASN1_OBJECT* ctype = cms->d.signedData->encapContentInfo->eContentType;

  /* Add the contentType attribute */
  return CMS_signed_add1_attr_by_NID(
           si, NID_pkcs9_contentType, V_ASN1_OBJECT, ctype, -1) > 0;
}
...
cms_copy_messageDigest(CMS_ContentInfo* cms, CMS_SignerInfo* si)
{
  ...
  for (i = 0; i < sk_CMS_SignerInfo_num(sinfos); i++) {
    ...
    if (CMS_signed_add1_attr_by_NID(
          si, NID_pkcs9_messageDigest, V_ASN1_OCTET_STRING, messageDigest, -1))
      return 1;
    else
      return 0;
  }
...
static int
cms_signature_nomd(EVP_PKEY* pkey)
{
  char def_md[80];
...
static int
cms_generic_sign(CMS_SignerInfo* si, int verify)
{
...
static int
ossl_cms_add1_signing_cert(CMS_SignerInfo* si, const ESS_SIGNING_CERT* sc)
{
  ...
  OPENSSL_free(pp);
  ret = CMS_signed_add1_attr_by_NID(
    si, NID_id_smime_aa_signingCertificate, V_ASN1_SEQUENCE, seq, -1);
  ASN1_STRING_free(seq);
  return ret;
}
...
static int
ossl_cms_add1_signing_cert_v2(CMS_SignerInfo* si, const ESS_SIGNING_CERT_V2* sc)
{
  ASN1_STRING* seq = NULL;
  unsigned char *p, *pp = NULL;
  ...
  OPENSSL_free(pp);
  ret = CMS_signed_add1_attr_by_NID(
    si, NID_id_smime_aa_signingCertificateV2, V_ASN1_SEQUENCE, seq, -1);
  ASN1_STRING_free(seq);
  return ret;
}

CMS_SignerInfo*
CMS_add1_signer(CMS_ContentInfo* cms,
                X509* signer,
                EVP_PKEY* pk,
                const EVP_MD* md,
                unsigned int flags)
{
  ...
  if (md == NULL) {
    ...
    if (md == NULL) {
      ERR_raise_data(
        ERR_LIB_CMS, CMS_R_NO_DEFAULT_DIGEST, "default md nid=%d", def_nid);
      goto err;
    }
  }
  ...
  if (flags & CMS_KEY_PARAM) {
    if (flags & CMS_NOATTR) {
      ...
      if (EVP_PKEY_CTX_set_signature_md(si->pctx, md) <= 0)
        goto err;
    } else if (EVP_DigestSignInit_ex(si->mctx,
                                     &si->pctx,
                                     EVP_MD_get0_name(md),
...
cms_add1_signingTime(CMS_SignerInfo* si, ASN1_TIME* t)
{
  ...
  if (CMS_signed_add1_attr_by_NID(
        si, NID_pkcs9_signingTime, tt->type, tt, -1) <= 0) {
    ERR_raise(ERR_LIB_CMS, ERR_R_CMS_LIB);
    goto err;
  }
...
int
CMS_SignerInfo_get0_signer_id(CMS_SignerInfo* si,
                              ASN1_OCTET_STRING** keyid,
                              X509_NAME** issuer,
                              ASN1_INTEGER** sno)
{
...
int
CMS_set1_signers_certs(CMS_ContentInfo* cms,
...
void
CMS_SignerInfo_get0_algs(CMS_SignerInfo* si,
                         EVP_PKEY** pk,
                         X509** signer,
                         X509_ALGOR** pdig,
                         X509_ALGOR** psig)
...
static int
cms_SignerInfo_content_sign(CMS_ContentInfo* cms,
                            CMS_SignerInfo* si,
                            BIO* chain,
                            ...
                            unsigned int mdlen)
{
  ...
  if (CMS_signed_get_attr_count(si) >= 0) {
    ...
    if (!CMS_signed_add1_attr_by_NID(
          si, NID_pkcs9_messageDigest, V_ASN1_OCTET_STRING, md, mdlen))
      goto err;
...
CMS_SignerInfo_sign(CMS_SignerInfo* si)
{
  ...
  if (OBJ_obj2txt(
        md_name_buf, sizeof(md_name_buf), si->digestAlgorithm->algorithm, 0) <=
      0)
    return 0;
  ...
  alen = ASN1_item_i2d(
    (ASN1_VALUE*)si->signedAttrs, &abuf, ASN1_ITEM_rptr(CMS_Attributes_Sign));
  if (alen < 0 || abuf == NULL)
    goto err;
...
CMS_SignerInfo_verify(CMS_SignerInfo* si)
{
  ...
  if (EVP_DigestVerifyInit_ex(
        mctx, &si->pctx, EVP_MD_get0_name(md), libctx, propq, si->pkey, NULL) <=
      0) {
    si->pctx = NULL;
    goto err;
  }
  ...
  alen = ASN1_item_i2d(
    (ASN1_VALUE*)si->signedAttrs, &abuf, ASN1_ITEM_rptr(CMS_Attributes_Verify));
  if (abuf == NULL || alen < 0)
    goto err;
...
CMS_SignerInfo_verify_content(CMS_SignerInfo* si, BIO* chain)
{
  ...
  if (os != NULL) {
    ...
    si->pctx = NULL;
    r = EVP_PKEY_verify(
      pkctx, si->signature->data, si->signature->length, mval, mlen);
    if (r <= 0) {
      ERR_raise(ERR_LIB_CMS, CMS_R_VERIFICATION_FAILURE);
      r = 0;
    }
...
BIO*
CMS_SignedData_verify(CMS_SignedData* sd,
                      BIO* detached_data,
                      STACK_OF(X509)* scerts,
                      X509_STORE* store,
                      STACK_OF(X509)* extra,
...
CMS_add_smimecap(CMS_SignerInfo* si, STACK_OF(X509_ALGOR)* algs)
{
  ...
  if (smderlen <= 0)
    return 0;
  r = CMS_signed_add1_attr_by_NID(
    si, NID_SMIMECapabilities, V_ASN1_SEQUENCE, smder, smderlen);
  OPENSSL_free(smder);
  return r;
}
