...
CMS_RecipientInfo_kemri_set0_pkey(CMS_RecipientInfo* ri, EVP_PKEY* pk)
{
  ...
  if (pk != NULL) {
    pctx = EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(kemri->cms_ctx),
                                      pk,
...
create_kdf_ctx(CMS_KEMRecipientInfo* kemri)
{
  ...
  kdf = EVP_KDF_fetch(ossl_cms_ctx_get0_libctx(kemri->cms_ctx),
                      kdf_alg,
                      ossl_cms_ctx_get0_propq(kemri->cms_ctx));
  if (kdf == NULL)
    goto err;
           ...
           CMS_KEMRecipientInfo* kemri)
{
  ...
  infolen = CMS_CMSORIforKEMOtherInfo_encode(
    &infoder, kemri->wrap, kemri->ukm, kemri->kekLength);
  if (infolen <= 0)
    goto err;
  ...
  params[0] = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_KEY, (unsigned char*)ss, sslen);
  params[1] = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_INFO, (char*)infoder, infolen);
...
static int
cms_kek_cipher(unsigned char** pout,
               size_t* poutlen,
               const unsigned char* ss,
               size_t sslen,
                                     ...
                                     CMS_RecipientInfo* ri)
{
  ...
  if (EVP_PKEY_encapsulate(
        kemri->pctx, NULL, &kem_ct_len, NULL, &kem_secret_len) <= 0)
    return 0;
  ...
  if (EVP_PKEY_encapsulate(
        kemri->pctx, kem_ct, &kem_ct_len, kem_secret, &kem_secret_len) <= 0)
    goto err;

  ASN1_STRING_set0(kemri->kemct, kem_ct, (int)kem_ct_len);
  kem_ct = NULL;

  if (!cms_kek_cipher(&enckey,
                      &enckeylen,
                      kem_secret,
                      kem_secret_len,
                      ec->key,
                                     ...
                                     CMS_RecipientInfo* ri)
{
  ...
  if (EVP_PKEY_decapsulate(
        kemri->pctx, NULL, &kem_secret_len, kem_ct, kem_ct_len) <= 0)
    return 0;
  kem_secret = OPENSSL_malloc(kem_secret_len);
  if (kem_secret == NULL)
    goto err;
  ...
  if (EVP_PKEY_decapsulate(
        kemri->pctx, kem_secret, &kem_secret_len, kem_ct, kem_ct_len) <= 0)
    goto err;
  ...
  if (!cms_kek_cipher(
        &cek, &ceklen, kem_secret, kem_secret_len, enckey, enckeylen, kemri, 0))
    goto err;
  ec = ossl_cms_get0_env_enc_content(cms);
