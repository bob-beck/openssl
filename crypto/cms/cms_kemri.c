...
int ossl_cms_RecipientInfo_kemri_get0_alg(CMS_RecipientInfo* ri,
  ...
  if (pkekLength) *pkekLength = &ri->d.ori->d.kemri->kekLength;
  if (pwrap) *pwrap = ri->d.ori->d.kemri->wrap;
  return 1;
}
...
int CMS_RecipientInfo_kemri_set0_pkey(CMS_RecipientInfo* ri, EVP_PKEY* pk) {
  ...
  if (pk != NULL) {
    pctx =
        EVP_PKEY_CTX_new_from_pkey(ossl_cms_ctx_get0_libctx(kemri->cms_ctx), pk,
                                   ossl_cms_ctx_get0_propq(kemri->cms_ctx));
    if (pctx == NULL || EVP_PKEY_decapsulate_init(pctx, NULL) <= 0) goto err;
...
int ossl_cms_RecipientInfo_kemri_init(CMS_RecipientInfo* ri, X509* recip,
  ...
  idtype = (flags & CMS_USE_KEYID) ? CMS_RECIPINFO_KEYIDENTIFIER
                                   : CMS_RECIPINFO_ISSUER_SERIAL;
  if (!ossl_cms_set1_SignerIdentifier(kemri->rid, recip, idtype, ctx)) return 0;

  x_pubkey = X509_get_X509_PUBKEY(recip);
  if (x_pubkey == NULL) return 0;
  if (!X509_PUBKEY_get0_param(NULL, NULL, NULL, &x_alg, x_pubkey)) return 0;
  if (!X509_ALGOR_copy(kemri->kem, x_alg)) return 0;

  kemri->pctx = EVP_PKEY_CTX_new_from_pkey(
      ossl_cms_ctx_get0_libctx(ctx), recipPubKey, ossl_cms_ctx_get0_propq(ctx));
  if (kemri->pctx == NULL) return 0;
  if (EVP_PKEY_encapsulate_init(kemri->pctx, NULL) <= 0) return 0;

  return 1;
}

EVP_CIPHER_CTX* CMS_RecipientInfo_kemri_get0_ctx(CMS_RecipientInfo* ri) {
  if (ri->type == CMS_RECIPINFO_KEM) return ri->d.ori->d.kemri->ctx;
  return NULL;
}

X509_ALGOR* CMS_RecipientInfo_kemri_get0_kdf_alg(CMS_RecipientInfo* ri) {
  if (ri->type == CMS_RECIPINFO_KEM) return ri->d.ori->d.kemri->kdf;
  return NULL;
}

int CMS_RecipientInfo_kemri_set_ukm(CMS_RecipientInfo* ri,
                                    const unsigned char* ukm, int ukmLength) {
  CMS_KEMRecipientInfo* kemri;
...
static EVP_KDF_CTX* create_kdf_ctx(CMS_KEMRecipientInfo* kemri) {
  ...
  if (OBJ_obj2txt(kdf_alg, sizeof(kdf_alg), kdf_oid, 1) < 0) goto err;
...
static int kdf_derive(unsigned char* kek, size_t keklen,
  ...
  if (EVP_KDF_derive(kctx, kek, keklen, params) <= 0) goto err;
...
static int cms_kek_cipher(unsigned char** pout, size_t* poutlen,
  ...
  if (!kdf_derive(kek, keklen, ss, sslen, kemri)) goto err;

  /* Set KEK in context */
  if (!EVP_CipherInit_ex(kemri->ctx, NULL, NULL, kek, NULL, enc)) goto err;
  /* obtain output length of ciphered key */
  if (!EVP_CipherUpdate(kemri->ctx, NULL, &outlen, in, (int)inlen)) goto err;
  out = OPENSSL_malloc(outlen);
  if (out == NULL) goto err;
  if (!EVP_CipherUpdate(kemri->ctx, out, &outlen, in, (int)inlen)) goto err;
  *pout = out;
  out = NULL;
...
int ossl_cms_RecipientInfo_kemri_encrypt(const CMS_ContentInfo* cms,
  ...
  if (!ossl_cms_RecipientInfo_wrap_init(ri, ec->cipher)) return 0;

  /* Initialise KDF algorithm */
  if (!ossl_cms_env_asn1_ctrl(ri, 0)) return 0;

  if (EVP_PKEY_encapsulate(kemri->pctx, NULL, &kem_ct_len, NULL,
                           &kem_secret_len) <= 0)
    return 0;
  kem_ct = OPENSSL_malloc(kem_ct_len);
  kem_secret = OPENSSL_malloc(kem_secret_len);
  if (kem_ct == NULL || kem_secret == NULL) goto err;

  if (EVP_PKEY_encapsulate(kemri->pctx, kem_ct, &kem_ct_len, kem_secret,
                           &kem_secret_len) <= 0)
    goto err;
  ...
  if (!cms_kek_cipher(&enckey, &enckeylen, kem_secret, kem_secret_len, ec->key,
                      ec->keylen, kemri, 1))
    goto err;
...
int ossl_cms_RecipientInfo_kemri_decrypt(const CMS_ContentInfo* cms,
  ...
  if (!ossl_cms_env_asn1_ctrl(ri, 1)) goto err;

  kem_ct = ASN1_STRING_get0_data(kemri->kemct);
  kem_ct_len = ASN1_STRING_length(kemri->kemct);

  if (EVP_PKEY_decapsulate(kemri->pctx, NULL, &kem_secret_len, kem_ct,
                           kem_ct_len) <= 0)
    return 0;
  kem_secret = OPENSSL_malloc(kem_secret_len);
  if (kem_secret == NULL) goto err;

  if (EVP_PKEY_decapsulate(kemri->pctx, kem_secret, &kem_secret_len, kem_ct,
                           kem_ct_len) <= 0)
    goto err;
  ...
  if (!cms_kek_cipher(&cek, &ceklen, kem_secret, kem_secret_len, enckey,
                      enckeylen, kemri, 0))
    goto err;
  ec = ossl_cms_get0_env_enc_content(cms);
