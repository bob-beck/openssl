...
int
CMS_RecipientInfo_kemri_cert_cmp (CMS_RecipientInfo *ri, X509 *cert)
{
  if (ri->type != CMS_RECIPINFO_KEM)
    {
      ERR_raise (ERR_LIB_CMS, CMS_R_NOT_KEM);
...
int
CMS_RecipientInfo_kemri_set0_pkey (CMS_RecipientInfo *ri, EVP_PKEY *pk)
{
  ...
  if (pk != NULL)
    {
      pctx = EVP_PKEY_CTX_new_from_pkey (
          ossl_cms_ctx_get0_libctx (kemri->cms_ctx), pk,
...
int
ossl_cms_RecipientInfo_kemri_init (CMS_RecipientInfo *ri, X509 *recip,
...
EVP_CIPHER_CTX *
CMS_RecipientInfo_kemri_get0_ctx (CMS_RecipientInfo *ri)
{
  if (ri->type == CMS_RECIPINFO_KEM)
...
X509_ALGOR *
CMS_RecipientInfo_kemri_get0_kdf_alg (CMS_RecipientInfo *ri)
{
  if (ri->type == CMS_RECIPINFO_KEM)
...
int
CMS_RecipientInfo_kemri_set_ukm (CMS_RecipientInfo *ri,
...
static EVP_KDF_CTX *
create_kdf_ctx (CMS_KEMRecipientInfo *kemri)
{
...
static int
kdf_derive (unsigned char *kek, size_t keklen, const unsigned char *ss,
            size_t sslen, CMS_KEMRecipientInfo *kemri)
{
  EVP_KDF_CTX *kctx = NULL;
  OSSL_PARAM params[3];
  ...
  infolen = CMS_CMSORIforKEMOtherInfo_encode (&infoder, kemri->wrap,
                                              kemri->ukm, kemri->kekLength);
  if (infolen <= 0)
    goto err;
...
static int
cms_kek_cipher (unsigned char **pout, size_t *poutlen, const unsigned char *ss,
                size_t sslen, const unsigned char *in, size_t inlen,
                                      ...
                                      CMS_RecipientInfo *ri)
{
  ...
  if (!cms_kek_cipher (&enckey, &enckeylen, kem_secret, kem_secret_len,
                       ec->key, ec->keylen, kemri, 1))
    goto err;
