...
int
CMS_RecipientInfo_set0_password(CMS_RecipientInfo* ri,
                                unsigned char* pass,
...
CMS_RecipientInfo*
CMS_add0_recipient_password(CMS_ContentInfo* cms,
                            int iter,
                            int wrap_nid,
                            int pbe_nid,
                            unsigned char* pass,
...
static int
kek_unwrap_key(unsigned char* out,
               size_t* outlen,
...
static int
kek_wrap_key(unsigned char* out,
             size_t* outlen,
             const unsigned char* in,
             size_t inlen,
             EVP_CIPHER_CTX* ctx,
             const CMS_CTX* cms_ctx)
{
  ...
  if (out) {
    ...
    if (olen > inlen + 4 && RAND_bytes_ex(ossl_cms_ctx_get0_libctx(cms_ctx),
                                          out + 4 + inlen,
                                          olen - 4 - inlen,
                                          0) <= 0)
      return 0;
                                  ...
                                  int en_de)
{
  ...
  kekcipher = EVP_CIPHER_fetch(
    ossl_cms_ctx_get0_libctx(cms_ctx), name, ossl_cms_ctx_get0_propq(cms_ctx));
  ...
  if (en_de) {
    ...
    if (key == NULL)
      goto err;
    if (!kek_unwrap_key(key,
                        &keylen,
                        pwri->encryptedKey->data,
                        pwri->encryptedKey->length,
                        kekctx)) {
