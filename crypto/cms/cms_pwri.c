...
int CMS_RecipientInfo_set0_password(CMS_RecipientInfo* ri, unsigned char* pass,
  ...
  pwri = ri->d.pwri;
  pwri->pass = pass;
  if (pass && passlen < 0) passlen = strlen((char*)pass);
...
CMS_RecipientInfo* CMS_add0_recipient_password(CMS_ContentInfo* cms, int iter,
  ...
  if (wrap_nid <= 0) wrap_nid = NID_id_alg_PWRI_KEK;

  /* Get from enveloped data */
  if (kekciph == NULL) kekciph = ec->cipher;
  ...
  if (pwri->keyDerivationAlgorithm == NULL) goto err;
...
err:
  EVP_CIPHER_CTX_free(ctx);
  if (ri) M_ASN1_free_of(ri, CMS_RecipientInfo);
  X509_ALGOR_free(encalg);
  return NULL;
}
...
int ossl_cms_RecipientInfo_pwri_crypt(const CMS_ContentInfo* cms,
  ...
  if (!EVP_CipherInit_ex(kekctx, kekcipher, NULL, NULL, NULL, en_de)) goto err;
