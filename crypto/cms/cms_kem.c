...
static int kem_cms_decrypt(CMS_RecipientInfo* ri) {
  ...
  if (!ossl_cms_RecipientInfo_kemri_get0_alg(ri, &kekLength, &wrap)) goto err;
  ...
  if (EVP_CIPHER_asn1_to_param(kekctx, wrap->parameter) <= 0) goto err;
...
static int kem_cms_encrypt(CMS_RecipientInfo* ri) {
  ...
  if (kdf_obj == NULL || OBJ_obj2nid(kdf_obj) == NID_undef) {
    ...
    params[0] = OSSL_PARAM_construct_octet_string(
        OSSL_PKEY_PARAM_CMS_KEMRI_KDF_ALGORITHM, kemri_x509_algor,
    ...
    if (OSSL_PARAM_modified(&params[0])) {
      ...
      if (x509_algor == NULL) goto err;
      if (!X509_ALGOR_copy(kdf, x509_algor)) goto err;
    } else {
      if (!X509_ALGOR_set0(kdf, OBJ_nid2obj(NID_HKDF_SHA256), V_ASN1_UNDEF,
                           NULL))
        return 0;
    }
  }
  ...
  *kekLength = EVP_CIPHER_CTX_get_key_length(kekctx);
  ...
  if (wrap->parameter == NULL) goto err;
...
int ossl_cms_kem_envelope(CMS_RecipientInfo* ri, int decrypt) {
  ...
  if (decrypt == 1) return kem_cms_decrypt(ri);

  if (decrypt == 0) return kem_cms_encrypt(ri);
