...
static int
kem_cms_decrypt(CMS_RecipientInfo* ri)
{
  uint32_t* kekLength;
  X509_ALGOR* wrap;
  EVP_PKEY_CTX* pctx;
...
static int
kem_cms_encrypt(CMS_RecipientInfo* ri)
{
  uint32_t* kekLength;
  X509_ALGOR* wrap;
  X509_ALGOR* kdf;
  ...
  if (kdf_obj == NULL || OBJ_obj2nid(kdf_obj) == NID_undef) {
     ...
     */
    params[0] =
      OSSL_PARAM_construct_octet_string(OSSL_PKEY_PARAM_CMS_KEMRI_KDF_ALGORITHM,
                                        kemri_x509_algor,
    ...
    if (OSSL_PARAM_modified(&params[0])) {
      ...
      if (!X509_ALGOR_set0(
            kdf, OBJ_nid2obj(NID_HKDF_SHA256), V_ASN1_UNDEF, NULL))
        return 0;
    }
  }
