...
int CMS_RecipientInfo_kari_get0_alg(CMS_RecipientInfo* ri, X509_ALGOR** palg,
  ...
  if (palg) *palg = ri->d.kari->keyEncryptionAlgorithm;
  if (pukm) *pukm = ri->d.kari->ukm;
  return 1;
}
...
int CMS_RecipientInfo_kari_get0_orig_id(
  ...
  if (sno) *sno = NULL;
  if (keyid) *keyid = NULL;
  if (pubalg) *pubalg = NULL;
  if (pubkey) *pubkey = NULL;
  if (oik->type == CMS_OIK_ISSUER_SERIAL) {
    if (issuer) *issuer = oik->d.issuerAndSerialNumber->issuer;
    if (sno) *sno = oik->d.issuerAndSerialNumber->serialNumber;
  } else if (oik->type == CMS_OIK_KEYIDENTIFIER) {
    if (keyid) *keyid = oik->d.subjectKeyIdentifier;
  } else if (oik->type == CMS_OIK_PUBKEY) {
    if (pubalg) *pubalg = oik->d.originatorKey->algorithm;
    if (pubkey) *pubkey = oik->d.originatorKey->publicKey;
  } else
    return 0;
  return 1;
}
...
int CMS_RecipientEncryptedKey_get0_id(CMS_RecipientEncryptedKey* rek,
  ...
  if (rid->type == CMS_REK_ISSUER_SERIAL) {
    if (issuer) *issuer = rid->d.issuerAndSerialNumber->issuer;
    if (sno) *sno = rid->d.issuerAndSerialNumber->serialNumber;
    if (keyid) *keyid = NULL;
    if (tm) *tm = NULL;
    if (other) *other = NULL;
  } else if (rid->type == CMS_REK_KEYIDENTIFIER) {
    if (keyid) *keyid = rid->d.rKeyId->subjectKeyIdentifier;
    if (tm) *tm = rid->d.rKeyId->date;
    if (other) *other = rid->d.rKeyId->other;
    if (issuer) *issuer = NULL;
    if (sno) *sno = NULL;
  } else
    return 0;
...
int CMS_RecipientInfo_kari_set0_pkey_and_peer(CMS_RecipientInfo* ri,
  ...
  if (pctx == NULL || EVP_PKEY_derive_init(pctx) <= 0) goto err;

  if (peer != NULL) {
    EVP_PKEY* pub_pkey = X509_get0_pubkey(peer);

    if (EVP_PKEY_derive_set_peer(pctx, pub_pkey) <= 0) goto err;
  }
...
EVP_CIPHER_CTX* CMS_RecipientInfo_kari_get0_ctx(CMS_RecipientInfo* ri) {
  if (ri->type == CMS_RECIPINFO_AGREE) return ri->d.kari->ctx;
  return NULL;
}
...
static int cms_kek_cipher(unsigned char** pout, size_t* poutlen,
  ...
  if (keklen > EVP_MAX_KEY_LENGTH || inlen > INT_MAX) return 0;
  /* Derive KEK */
  if (EVP_PKEY_derive(kari->pctx, kek, &keklen) <= 0) goto err;
  /* Set KEK in context */
  if (!EVP_CipherInit_ex(kari->ctx, NULL, NULL, kek, NULL, enc)) goto err;
  /* obtain output length of ciphered key */
  if (!EVP_CipherUpdate(kari->ctx, NULL, &outlen, in, (int)inlen)) goto err;
  out = OPENSSL_malloc(outlen);
  if (out == NULL) goto err;
  if (!EVP_CipherUpdate(kari->ctx, out, &outlen, in, (int)inlen)) goto err;
  *pout = out;
...
static int cms_kari_create_ephemeral_key(CMS_KeyAgreeRecipientInfo* kari,
  ...
  if (EVP_PKEY_keygen(pctx, &ekey) <= 0) goto err;
  EVP_PKEY_CTX_free(pctx);
...
static int cms_kari_set_originator_private_key(CMS_KeyAgreeRecipientInfo* kari,
  ...
  rv = 1;
err:
  if (rv == 0) EVP_PKEY_CTX_free(pctx);
  return rv;
}
...
int ossl_cms_RecipientInfo_kari_init(CMS_RecipientInfo* ri, X509* recip,
  ...
  if (!EVP_PKEY_up_ref(recipPubKey)) return 0;
...
int ossl_cms_RecipientInfo_kari_encrypt(const CMS_ContentInfo* cms,
  ...
  for (i = 0; i < sk_CMS_RecipientEncryptedKey_num(reks); i++) {
    ...
    if (EVP_PKEY_derive_set_peer(kari->pctx, rek->pkey) <= 0) return 0;
