...
static STACK_OF(CMS_CertificateChoices)** cms_get0_certificate_choices(
    CMS_ContentInfo* cms);
...
BIO* ossl_cms_content_bio(CMS_ContentInfo* cms) {
  ...
  if (*pos == NULL) return BIO_new(BIO_s_null());
  /*
...
BIO* CMS_dataInit(CMS_ContentInfo* cms, BIO* icont) {
  ...
  if (cmsbio) return BIO_push(cmsbio, cont);
err:
  if (!icont) BIO_free(cont);
  return NULL;
}
...
int CMS_set_detached(CMS_ContentInfo* cms, int detached) {
  ...
  if (*pos == NULL) *pos = ASN1_OCTET_STRING_new();
  if (*pos != NULL) {
    /*
...
BIO* ossl_cms_DigestAlgorithm_init_bio(X509_ALGOR* digestAlgorithm,
  ...
  if (EVP_MD_xof(digest)) {
    ...
    if (xof_len > 0) {
      ...
      if (BIO_get_md_ctx(mdbio, &mdctx) <= 0 || mdctx == NULL) goto err;
      params[0] =
          OSSL_PARAM_construct_size_t(OSSL_DIGEST_PARAM_XOFLEN, &xof_len);
      params[1] = OSSL_PARAM_construct_end();
      if (!EVP_MD_CTX_set_params(mdctx, params)) goto err;
    }
  }
...
static STACK_OF(CMS_CertificateChoices)** cms_get0_certificate_choices(
  ...
  switch (OBJ_obj2nid(cms->contentType)) {
    ...
    case NID_pkcs7_enveloped:
      if (cms->d.envelopedData->originatorInfo == NULL) return NULL;
      return &cms->d.envelopedData->originatorInfo->certificates;

    case NID_id_smime_ct_authEnvelopedData:
      if (cms->d.authEnvelopedData->originatorInfo == NULL) return NULL;
...
CMS_CertificateChoices* CMS_add0_CertificateChoices(CMS_ContentInfo* cms) {
  ...
  if (pcerts == NULL) return NULL;
  if (*pcerts == NULL) *pcerts = sk_CMS_CertificateChoices_new_null();
  if (*pcerts == NULL) return NULL;
...
int CMS_add1_cert(CMS_ContentInfo* cms, X509* cert) {
  if (!X509_up_ref(cert)) return 0;
  if (CMS_add0_cert(cms, cert)) return 1;
  X509_free(cert);
  return 0;
}
...
CMS_RevocationInfoChoice* CMS_add0_RevocationInfoChoice(CMS_ContentInfo* cms) {
  ...
  if (pcrls == NULL) return NULL;
  if (*pcrls == NULL) *pcrls = sk_CMS_RevocationInfoChoice_new_null();
  if (*pcrls == NULL) return NULL;
...
int CMS_add1_crl(CMS_ContentInfo* cms, X509_CRL* crl) {
  ...
  if (CMS_add0_crl(cms, crl)) return 1;
  X509_CRL_free(crl);
  return 0;
}
...
STACK_OF(X509)* CMS_get1_certs(CMS_ContentInfo* cms) {
  ...
  if (!ossl_cms_get1_certs_ex(cms, &certs)) return NULL;
...
int ossl_cms_get1_certs_ex(CMS_ContentInfo* cms, STACK_OF(X509)** certs) {
  ...
  *certs = NULL;
  ...
  if ((*certs = sk_X509_new_reserve(NULL, n)) == NULL) return 0;
...
STACK_OF(X509_CRL)* CMS_get1_crls(CMS_ContentInfo* cms) {
  ...
  if (!ossl_cms_get1_crls_ex(cms, &crls)) return NULL;
...
int ossl_cms_get1_crls_ex(CMS_ContentInfo* cms, STACK_OF(X509_CRL)** crls) {
  ...
  if (crls == NULL) return 0;
  *crls = NULL;
  pcrls = cms_get0_revocation_choices(cms);
  if (pcrls == NULL) return 0;

  /* make sure to return NULL *crls only on error */
  n = sk_CMS_RevocationInfoChoice_num(*pcrls);
  if ((*crls = sk_X509_CRL_new_reserve(NULL, n)) == NULL) return 0;
...
CMS_EnvelopedData* ossl_cms_sign_encrypt(
    ...
    unsigned int sign_flags, STACK_OF(X509)* enc_recip,
    const EVP_CIPHER* cipher, unsigned int enc_flags, OSSL_LIB_CTX* libctx,
    const char* propq) {
