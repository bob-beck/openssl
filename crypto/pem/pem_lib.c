...
static int check_pem(const char* nm, const char* name) {
  ...
  if (strcmp(name, PEM_STRING_EVP_PKEY) == 0) {
    ...
    if (strcmp(nm, PEM_STRING_PKCS8) == 0) return 1;
    if (strcmp(nm, PEM_STRING_PKCS8INF) == 0) return 1;
    ...
    if (slen > 0) {
      ...
      if (ameth && ameth->old_priv_decode) return 1;
    }
    return 0;
  }
...
err:
  if (!ret || pnm == NULL) PEM_FREE(nm, flags, 0);
  PEM_FREE(header, flags, 0);
  if (!ret) PEM_FREE(data, flags, len);
  return ret;
}
...
static int PEM_ASN1_write_bio_internal(i2d_of_void* i2d,
  ...
  if (enc != NULL) {
    ...
    /* Generate a salt */
    if (RAND_bytes(iv, EVP_CIPHER_get_iv_length(enc)) <= 0) goto err;
    /*
     * The 'iv' is used as the iv and as a salt.  It is NOT taken from
     * the BytesToKey function
     */
    if (!EVP_BytesToKey(enc, EVP_md5(), iv, kstr, klen, 1, key, NULL)) goto err;
...
int PEM_do_header(EVP_CIPHER_INFO* cipher, unsigned char* data, long* plen,
  ...
  if (ok) ok = EVP_DecryptUpdate(ctx, data, &ilen, data, ilen);
  if (ok) {
...
int PEM_get_EVP_CIPHER_INFO(char* header, EVP_CIPHER_INFO* cipher) {
  ...
  *header = c;
  ...
  if (!load_iv(&header, cipher->iv, EVP_CIPHER_get_iv_length(enc))) return 0;
...
err:
  if (retval == 0 && reason != 0) ERR_raise(ERR_LIB_PEM, reason);
...
static int sanitize_line(char* linebuf, int len, unsigned int flags,
  ...
  if (flags & PEM_FLAG_EAY_COMPATIBLE) {
    ...
    for (i = 0; i < len; ++i) {
      if (linebuf[i] == '\n' || linebuf[i] == '\r') break;
      if (ossl_iscntrl(linebuf[i])) linebuf[i] = ' ';
    }
    len = i;
  }
...
static int get_header_and_data(BIO* bp, BIO** header, BIO** data, char* name,
  ...
  while (1) {
    ...
    if (got_header == MAYBE_HEADER) {
      if (memchr(linebuf, ':', len) != NULL) got_header = IN_HEADER;
    }
     ...
     */
    if (BIO_puts(tmp, linebuf) < 0) goto err;
    /*
...
int PEM_read_bio_ex(BIO* bp, char** name_out, char** header,
  ...
  *data = NULL;
  ...
  if (!get_name(bp, &name, flags)) goto end;
  if (!get_header_and_data(bp, &headerB, &dataB, name, flags)) goto end;
  ...
  *data = PEM_MALLOC(len, flags);
  if (*header == NULL || *data == NULL) goto out_free;
  if (headerlen != 0 && BIO_read(headerB, *header, headerlen) != headerlen)
    goto out_free;
  (*header)[headerlen] = '\0';
  if (BIO_read(dataB, *data, len) != len) goto out_free;
  *len_out = len;
  *name_out = name;
  name = NULL;
  ret = 1;
