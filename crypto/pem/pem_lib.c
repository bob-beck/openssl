...
int
ossl_pem_check_suffix(const char* pem_str, const char* suffix);

int
PEM_def_callback(char* buf, int num, int rwflag, void* userdata)
{
  int i, min_len;
  const char* prompt;
...
void
PEM_dek_info(char* buf, const char* type, int len, const char* str)
{
  long i;
  char* p = buf + strlen(buf);
...
static void
pem_free(void* p, unsigned int flags, size_t num, const char* file, int line)
{
...
static void*
pem_malloc(int num, unsigned int flags, const char* file, int line)
{
...
static int
pem_bytes_read_bio_flags(unsigned char** pdata,
                         long* plen,
                         char** pnm,
                         const char* name,
                         BIO* bp,
...
int
PEM_bytes_read_bio(unsigned char** pdata,
                   long* plen,
                   char** pnm,
                   const char* name,
                   BIO* bp,
                   ...
                   void* u)
{
  return pem_bytes_read_bio_flags(
    pdata, plen, pnm, name, bp, cb, u, PEM_FLAG_EAY_COMPATIBLE);
}

int
PEM_bytes_read_bio_secmem(unsigned char** pdata,
                          long* plen,
                          char** pnm,
                          const char* name,
                          BIO* bp,
                          ...
                          void* u)
{
  return pem_bytes_read_bio_flags(pdata,
                                  plen,
                                  pnm,
                                  name,
                                  bp,
...
#ifndef OPENSSL_NO_STDIO
int
PEM_ASN1_write(i2d_of_void* i2d,
               const char* name,
               FILE* fp,
               const void* x,
...
static int
PEM_ASN1_write_bio_internal(i2d_of_void* i2d,
...
int
PEM_ASN1_write_bio(i2d_of_void* i2d,
                   const char* name,
                   BIO* bp,
                   const void* x,
                   const EVP_CIPHER* enc,
                   ...
                   void* u)
{
  return PEM_ASN1_write_bio_internal(
    i2d, NULL, NULL, name, bp, x, enc, kstr, klen, callback, u);
}

int
PEM_ASN1_write_bio_ctx(OSSL_i2d_of_void_ctx* i2d,
                       void* vctx,
                       const char* name,
                       BIO* bp,
                       const void* x,
...
{
  return PEM_ASN1_write_bio_internal(
    NULL, i2d, vctx, name, bp, x, enc, kstr, klen, callback, u);
}

int
PEM_do_header(EVP_CIPHER_INFO* cipher,
              unsigned char* data,
              long* plen,
              pem_password_cb* callback,
              void* u)
{
  ...
  if (!EVP_BytesToKey(cipher->cipher,
                      EVP_md5(),
                      &(cipher->iv[0]),
                      (unsigned char*)buf,
                      keylen,
...
static int
sanitize_line(char* linebuf, int len, unsigned int flags, int first_call)
{
  int i;
  if (first_call) {
...
static int
get_name(BIO* bp, char** name, unsigned int flags)
{
  char* linebuf;
  int ret = 0;
  int len;
