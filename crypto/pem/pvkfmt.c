 ...
 */
int
ossl_do_blob_header(const unsigned char** in,
                    unsigned int length,
                    unsigned int* pmagic,
...
static void*
do_b2i_key(const unsigned char** in,
           unsigned int length,
           int* isdss,
           int* ispub)
{
...
static int
check_bitlen_rsa(const RSA* rsa, int ispub, unsigned int* magic);
static void
write_rsa(unsigned char** out, const RSA* rsa, int ispub);

#ifndef OPENSSL_NO_DSA
static int
check_bitlen_dsa(const DSA* dsa, int ispub, unsigned int* magic);
static void
...
int
ossl_do_PVK_header(const unsigned char** in,
                   unsigned int length,
                   int skip_magic,
                   int* isdss,
                   unsigned int* psaltlen,
...
static int
derive_pvk_key(unsigned char* key,
               size_t keylen,
...
static void*
do_PVK_body_key(const unsigned char** in,
                unsigned int saltlen,
                unsigned int keylen,
                pem_password_cb* cb,
                void* u,
