...
static EVP_PKEY* evp_pkey_new0_key(void* key, int evp_type) {
  ...
  if ((pkey = EVP_PKEY_new()) != NULL) {
    switch (evp_type) {
      case EVP_PKEY_RSA:
        if (EVP_PKEY_set1_RSA(pkey, key)) break;
      ...
      case EVP_PKEY_DSA:
        if (EVP_PKEY_set1_DSA(pkey, key)) break;
        ERR_raise(ERR_LIB_PEM, ERR_R_EVP_LIB);
        EVP_PKEY_free(pkey);
        pkey = NULL;
        break;
...
static void* do_b2i_key(const unsigned char** in, unsigned int length,
  ...
  if (!*isdss) key = ossl_b2i_RSA_after_header(&p, bitlen, *ispub);
#ifndef OPENSSL_NO_DSA
  else
...
DSA* ossl_b2i_DSA_after_header(const unsigned char** in, unsigned int bitlen,
  ...
  dsa = DSA_new();
  if (dsa == NULL) goto dsaerr;
  if (!read_lebn(&p, nbyte, &pbn)) goto bnerr;

  if (!read_lebn(&p, 20, &qbn)) goto bnerr;

  if (!read_lebn(&p, nbyte, &gbn)) goto bnerr;

  if (ispub) {
    if (!read_lebn(&p, nbyte, &pub_key)) goto bnerr;
  } else {
    if (!read_lebn(&p, 20, &priv_key)) goto bnerr;
    ...
    pub_key = BN_new();
    if (pub_key == NULL) goto bnerr;
    if ((ctx = BN_CTX_new()) == NULL) goto bnerr;

    if (!BN_mod_exp(pub_key, gbn, priv_key, pbn, ctx)) goto bnerr;

    BN_CTX_free(ctx);
    ctx = NULL;
  }
  if (!DSA_set0_pqg(dsa, pbn, qbn, gbn)) goto dsaerr;
  pbn = qbn = gbn = NULL;
  if (!DSA_set0_key(dsa, pub_key, priv_key)) goto dsaerr;
...
RSA* ossl_b2i_RSA_after_header(const unsigned char** in, unsigned int bitlen,
  ...
  rsa = RSA_new();
  if (rsa == NULL) goto rsaerr;
  e = BN_new();
  if (e == NULL) goto bnerr;
  if (!BN_set_word(e, read_ledword(&pin))) goto bnerr;
  if (!read_lebn(&pin, nbyte, &n)) goto bnerr;
  if (!ispub) {
    if (!read_lebn(&pin, hnbyte, &p)) goto bnerr;
    if (!read_lebn(&pin, hnbyte, &q)) goto bnerr;
    if (!read_lebn(&pin, hnbyte, &dmp1)) goto bnerr;
    if (!read_lebn(&pin, hnbyte, &dmq1)) goto bnerr;
    if (!read_lebn(&pin, hnbyte, &iqmp)) goto bnerr;
    if (!read_lebn(&pin, nbyte, &d)) goto bnerr;
    if (!RSA_set0_factors(rsa, p, q)) goto rsaerr;
    p = q = NULL;
    if (!RSA_set0_crt_params(rsa, dmp1, dmq1, iqmp)) goto rsaerr;
    dmp1 = dmq1 = iqmp = NULL;
  }
  if (!RSA_set0_key(rsa, n, e, d)) goto rsaerr;
  n = e = d = NULL;
...
static int do_i2b(unsigned char** out, const EVP_PKEY* pk, int ispub) {
  ...
  outlen = 16 + ossl_blob_length(bitlen, keyalg == MS_KEYALG_DSS_SIGN ? 1 : 0,
                                 ispub);
  ...
  *p++ = 0;
  ...
  if (keyalg == MS_KEYALG_RSA_KEYX) write_rsa(&p, EVP_PKEY_get0_RSA(pk), ispub);
#ifndef OPENSSL_NO_DSA
  else
...
static int check_bitlen_rsa(const RSA* rsa, int ispub, unsigned int* pmagic) {
  ...
  if (BN_num_bits(e) > 32) goto badkey;
  bitlen = RSA_bits(rsa);
  nbyte = RSA_size(rsa);
  hnbyte = (bitlen + 15) >> 4;
  if (ispub) {
    ...
    *pmagic = MS_RSA2MAGIC;
    ...
    if (BN_num_bytes(d) > nbyte) goto badkey;
...
static int check_bitlen_dsa(const DSA* dsa, int ispub, unsigned int* pmagic) {
  ...
  if ((bitlen & 7) || (BN_num_bits(q) != 160) || (BN_num_bits(g) > bitlen))
    goto badkey;
  if (ispub) {
    if (BN_num_bits(pub_key) > bitlen) goto badkey;
    *pmagic = MS_DSS1MAGIC;
  } else {
    if (BN_num_bits(priv_key) > 160) goto badkey;
    *pmagic = MS_DSS2MAGIC;
  }
...
int ossl_do_PVK_header(const unsigned char** in, unsigned int length,
  ...
  *pkeylen = read_ledword(&p);
  ...
  if (*pkeylen > PVK_MAX_KEYLEN || *psaltlen > PVK_MAX_SALTLEN) return 0;
...
static int derive_pvk_key(unsigned char* key, size_t keylen,
  ...
  if ((kdf = EVP_KDF_fetch(libctx, "PVKKDF", propq)) == NULL) return 0;
  ...
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_SALT, (void*)salt,
                                           saltlen);
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD, (void*)pass,
                                           passlen);
...
static void* do_PVK_body_key(const unsigned char** in, unsigned int saltlen,
  ...
  if (saltlen) {
    ...
    inlen = keylen - 8;
    q = enctmp + 8;
    if ((rc4 = EVP_CIPHER_fetch(libctx, "RC4", propq)) == NULL) goto err;
    if (!EVP_DecryptInit_ex(cctx, rc4, NULL, keybuf, NULL)) goto err;
    if (!EVP_DecryptUpdate(cctx, q, &enctmplen, p, inlen)) goto err;
    if (!EVP_DecryptFinal_ex(cctx, q + enctmplen, &enctmplen)) goto err;
...
static void* do_PVK_key_bio(BIO* in, pem_password_cb* cb, void* u, int* isdss,
  ...
  if (!ossl_do_PVK_header(&p, 24, 0, isdss, &saltlen, &keylen)) return 0;
...
static int i2b_PVK(unsigned char** out, const EVP_PKEY* pk, int enclevel,
  ...
  if (enclevel) outlen += PVK_SALTLEN;
  pklen = do_i2b(NULL, pk, 0);
  if (pklen < 0) return -1;
  outlen += pklen;
  if (out == NULL) return outlen;
  if (*out != NULL) {
    p = *out;
  } else {
  ...
  if (cctx == NULL) goto error;

  write_ledword(&p, MS_PVKMAGIC);
  write_ledword(&p, 0);
  if (EVP_PKEY_get_id(pk) == EVP_PKEY_RSA) write_ledword(&p, MS_KEYTYPE_KEYX);
#ifndef OPENSSL_NO_DSA
  else
  ...
  if (enclevel) {
#ifndef OPENSSL_NO_RC4
    if (RAND_bytes_ex(libctx, p, PVK_SALTLEN, 0) <= 0) goto error;
    salt = p;
    p += PVK_SALTLEN;
#endif
  }
  ...
  if (enclevel != 0) {
    ...
    if (!derive_pvk_key(keybuf, sizeof(keybuf), salt, PVK_SALTLEN,
                        (unsigned char*)psbuf, inlen, libctx, propq))
      goto error;
    if ((rc4 = EVP_CIPHER_fetch(libctx, "RC4", propq)) == NULL) goto error;
    if (enclevel == 1) memset(keybuf + 5, 0, 11);
    p = salt + PVK_SALTLEN + 8;
    if (!EVP_EncryptInit_ex(cctx, rc4, NULL, keybuf, NULL)) goto error;
    OPENSSL_cleanse(keybuf, 20);
    if (!EVP_EncryptUpdate(cctx, p, &enctmplen, p, pklen - 8)) goto error;
    if (!EVP_EncryptFinal_ex(cctx, p + enctmplen, &enctmplen)) goto error;
#else
    ERR_raise(ERR_LIB_PEM, PEM_R_UNSUPPORTED_CIPHER);
    goto error;
#endif
...
error:
  ...
  EVP_CIPHER_free(rc4);
#endif
  if (*out == NULL) OPENSSL_free(start);
