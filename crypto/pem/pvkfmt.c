...
static EVP_PKEY *
evp_pkey_new0_key (void *key, int evp_type)
{
  EVP_PKEY *pkey = NULL;
 ...
 */
int
ossl_do_blob_header (const unsigned char **in, unsigned int length,
                     unsigned int *pmagic, unsigned int *pbitlen, int *pisdss,
                     int *pispub)
{
  const unsigned char *p = *in;
...
unsigned int
ossl_blob_length (unsigned bitlen, int isdss, int ispub)
{
...
static void *
do_b2i_key (const unsigned char **in, unsigned int length, int *isdss,
            int *ispub)
{
  const unsigned char *p = *in;
...
EVP_PKEY *
ossl_b2i (const unsigned char **in, unsigned int length, int *ispub)
{
  int isdss = -1;
...
#ifndef OPENSSL_NO_DSA
DSA *
ossl_b2i_DSA_after_header (const unsigned char **in, unsigned int bitlen,
                           int ispub)
{
  const unsigned char *p = *in;
  DSA *dsa = NULL;
...
RSA *
ossl_b2i_RSA_after_header (const unsigned char **in, unsigned int bitlen,
                           int ispub)
{
...
static int
do_i2b (unsigned char **out, const EVP_PKEY *pk, int ispub)
{
  unsigned char *p;
  ...
  outlen = 16
           + ossl_blob_length (bitlen, keyalg == MS_KEYALG_DSS_SIGN ? 1 : 0,
                               ispub);
  if (out == NULL)
    goto end;
  if (*out)
...
static int
do_i2b_bio (BIO *out, const EVP_PKEY *pk, int ispub)
{
...
static int
check_bitlen_rsa (const RSA *rsa, int ispub, unsigned int *pmagic)
{
...
static void
write_rsa (unsigned char **out, const RSA *rsa, int ispub)
{
  int nbyte, hnbyte;
...
#ifndef OPENSSL_NO_DSA
static int
check_bitlen_dsa (const DSA *dsa, int ispub, unsigned int *pmagic)
{
  int bitlen;
...
static void
write_dsa (unsigned char **out, const DSA *dsa, int ispub)
{
  int nbyte;
  const BIGNUM *p = NULL, *q = NULL, *g = NULL;
  const BIGNUM *pub_key = NULL, *priv_key = NULL;
...
int
i2b_PrivateKey_bio (BIO *out, const EVP_PKEY *pk)
{
  return do_i2b_bio (out, pk, 0);
}

int
i2b_PublicKey_bio (BIO *out, const EVP_PKEY *pk)
{
  return do_i2b_bio (out, pk, 1);
}

int
ossl_do_PVK_header (const unsigned char **in, unsigned int length,
...
static int
derive_pvk_key (unsigned char *key, size_t keylen, const unsigned char *salt,
...
static void *
do_PVK_body_key (const unsigned char **in, unsigned int saltlen,
                 unsigned int keylen, pem_password_cb *cb, void *u, int *isdss,
...
static void *
do_PVK_key_bio (BIO *in, pem_password_cb *cb, void *u, int *isdss, int *ispub,
                OSSL_LIB_CTX *libctx, const char *propq)
{
...
#ifndef OPENSSL_NO_DSA
DSA *
b2i_DSA_PVK_bio_ex (BIO *in, pem_password_cb *cb, void *u,
                    OSSL_LIB_CTX *libctx, const char *propq)
{
  int isdss = 1;
...
DSA *
b2i_DSA_PVK_bio (BIO *in, pem_password_cb *cb, void *u)
{
...
RSA *
b2i_RSA_PVK_bio_ex (BIO *in, pem_password_cb *cb, void *u,
                    OSSL_LIB_CTX *libctx, const char *propq)
{
  int isdss = 0;
  int ispub = 0; /* PVK keys are always private */
...
RSA *
b2i_RSA_PVK_bio (BIO *in, pem_password_cb *cb, void *u)
{
  return b2i_RSA_PVK_bio_ex (in, cb, u, NULL, NULL);
}

EVP_PKEY *
b2i_PVK_bio_ex (BIO *in, pem_password_cb *cb, void *u, OSSL_LIB_CTX *libctx,
                const char *propq)
{
  int isdss = -1;
  int ispub = -1;
...
static int
i2b_PVK (unsigned char **out, const EVP_PKEY *pk, int enclevel,
...
int
i2b_PVK_bio_ex (BIO *out, const EVP_PKEY *pk, int enclevel,
...
int
i2b_PVK_bio (BIO *out, const EVP_PKEY *pk, int enclevel, pem_password_cb *cb,
             void *u)
{
