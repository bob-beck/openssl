...
static EVP_PKEY*
pem_read_bio_key_decoder(BIO* bp,
                         EVP_PKEY** x,
                         pem_password_cb* cb,
                         void* u,
                         OSSL_LIB_CTX* libctx,
                         ...
                         int selection)
{
  ...
  dctx = OSSL_DECODER_CTX_new_for_pkey(
    &pkey, "PEM", NULL, NULL, selection, libctx, propq);
...
static EVP_PKEY*
pem_read_bio_key_legacy(BIO* bp,
                        EVP_PKEY** x,
                        pem_password_cb* cb,
                        void* u,
                        OSSL_LIB_CTX* libctx,
                 ...
                 int selection)
{
  ...
  ERR_set_mark();
  ret = pem_read_bio_key_decoder(
    bp, x, ossl_pw_pem_password, &pwdata, libctx, propq, selection);
  if (ret == NULL &&
      (BIO_seek(bp, pos) < 0 ||
...
{
  return PEM_write_bio_PrivateKey_ex(
    out, x, enc, kstr, klen, cb, u, NULL, NULL);
}
 ...
 */
int
PEM_write_bio_PrivateKey_traditional(BIO* bp,
                                     const EVP_PKEY* x,
                                     ...
                                     void* u)
{
  ...
  ret = PEM_ASN1_write_bio(
    (i2d_of_void*)i2d_PrivateKey, pem_str, bp, x, enc, kstr, klen, cb, u);
...
EVP_PKEY*
PEM_read_bio_Parameters_ex(BIO* bp,
                           EVP_PKEY** x,
                           OSSL_LIB_CTX* libctx,
                           const char* propq)
{
   ...
   */
  return pem_read_bio_key(
    bp, x, no_password_cb, NULL, libctx, propq, EVP_PKEY_KEY_PARAMETERS);
}
