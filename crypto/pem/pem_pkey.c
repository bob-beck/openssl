...
static EVP_PKEY* pem_read_bio_key_decoder(BIO* bp, EVP_PKEY** x,
  ...
  if (cb == NULL) cb = PEM_def_callback;

  if (!OSSL_DECODER_CTX_set_pem_password_cb(dctx, cb, u)) goto err;
...
static EVP_PKEY* pem_read_bio_key_legacy(BIO* bp, EVP_PKEY** x,
  ...
  if (strcmp(nm, PEM_STRING_PKCS8INF) == 0) {
    ...
    if ((p8inf = d2i_PKCS8_PRIV_KEY_INFO(NULL, &p, len)) == NULL) goto p8err;
    ...
    if ((p8 = d2i_X509_SIG(NULL, &p, len)) == NULL) goto p8err;
    if (cb != NULL)
    ...
    if (ameth == NULL || ameth->old_priv_decode == NULL) goto p8err;
...
legacy:
  if (!x->ameth || !x->ameth->param_encode) return 0;
