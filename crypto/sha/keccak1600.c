 ...
 */
static void
Round (uint64_t R[5][5], uint64_t A[5][5], size_t i)
{
  uint64_t C[5], D[5];
 ...
 */
static void
FourRounds (uint64_t A[5][5], size_t i)
{
 ...
 * |len| bytes.
 * If multiple SHA3_squeeze calls are required the output length |len| must be
 * a multiple of the blocksize, with |next| being 0 on the first call and 1 on
...
void
SHA3_sponge (const unsigned char *inp, size_t len, unsigned char *out,
             size_t d, size_t r)
{
  uint64_t A[5][5];
