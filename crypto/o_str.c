...
char* CRYPTO_strdup(const char* str, const char* file, int line) {
  ...
  if (ret != NULL) memcpy(ret, str, len);
  return ret;
}
...
void* CRYPTO_memdup(const void* data, size_t siz, const char* file, int line) {
  ...
  if (data == NULL || siz >= INT_MAX) return NULL;
...
static int hexstr2buf_sep(unsigned char* buf, size_t buf_n, size_t* buflen,
  ...
  for (p = (const unsigned char*)str, q = buf, cnt = 0; *p;) {
    ...
    if (ch == sep && sep != CH_ZERO) continue;
    cl = *p++;
    if (!cl) {
...
unsigned char* ossl_hexstr2buf_sep(const char* str, long* buflen,
  ...
  buf_n /= 2;
  if ((buf = OPENSSL_malloc(buf_n)) == NULL) return NULL;

  if (buflen != NULL) *buflen = 0;
  tmp_buflen = 0;
  if (hexstr2buf_sep(buf, buf_n, &tmp_buflen, str, sep)) {
    if (buflen != NULL) *buflen = (long)tmp_buflen;
    return buf;
  }
  OPENSSL_free(buf);
  return NULL;
...
char* ossl_buf2hexstr_sep(const unsigned char* buf, long buflen, char sep) {
  ...
  if (buflen == 0) return OPENSSL_zalloc(1);

  tmp_n = (sep != CH_ZERO) ? buflen * 3 : 1 + buflen * 2;
  if ((tmp = OPENSSL_malloc(tmp_n)) == NULL) return NULL;

  if (buf2hexstr_sep(tmp, tmp_n, NULL, buf, buflen, sep)) return tmp;
  OPENSSL_free(tmp);
  return NULL;
}
...
int openssl_strerror_r(int errnum, char* buf, size_t buflen) {
   ...
   */
  if (err != buf) OPENSSL_strlcpy(buf, err, buflen);
  return 1;
