...
static char*
dl_name_converter(DSO* dso, const char* filename);
static char*
dl_merger(DSO* dso, const char* filespec1, const char* filespec2);
static int
dl_pathbyaddr(void* addr, char* path, int sz);
static void*
dl_globallookup(const char* name);
...
dl_load(DSO* dso)
{
  ...
  if (ptr == NULL) {
    ...
    if (openssl_strerror_r(errno, errbuf, sizeof(errbuf)))
      ERR_raise_data(
        ERR_LIB_DSO, DSO_R_LOAD_FAILED, "filename(%s): %s", filename, errbuf);
    else
...
dl_bind_func(DSO* dso, const char* symname)
{
  ...
  if (shl_findsym(&ptr, symname, TYPE_UNDEFINED, &sym) < 0) {
    ...
    if (openssl_strerror_r(errno, errbuf, sizeof(errbuf)))
      ERR_raise_data(
        ERR_LIB_DSO, DSO_R_SYM_FAILURE, "symname(%s): %s", symname, errbuf);
    else
      ERR_raise_data(ERR_LIB_DSO,
                     DSO_R_SYM_FAILURE,
                     "symname(%s): errno %d",
