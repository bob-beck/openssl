...
#ifdef RTLD_GLOBAL
  if (dso->flags & DSO_FLAG_GLOBAL_SYMBOLS) flags |= RTLD_GLOBAL;
#endif
#ifdef _AIX
  if (filename[strlen(filename) - 1] == ')') flags |= RTLD_MEMBER;
#endif
...
static int dlfcn_unload(DSO* dso) {
  ...
  if (sk_void_num(dso->meth_data) < 1) return 1;
...
static int dladdr(void* ptr, Dl_info* dl) {
  ...
  do {
    ...
    if (((addr >= (uintptr_t)this_ldi->ldinfo_textorg) &&
      ...
      if ((member_len = strlen(member)) > 0) buffer_sz += 1 + member_len + 1;
      found = 1;
