...
static int
init_thread_deregister(void* arg, int all);
#endif
static void
init_thread_stop(void* arg, THREAD_EVENT_HANDLER** hands);

static THREAD_EVENT_HANDLER**
get_thread_event_handler(OSSL_LIB_CTX* ctx)
{
#ifdef FIPS_MODULE
...
static int
set_thread_event_handler(OSSL_LIB_CTX* ctx, THREAD_EVENT_HANDLER** hands)
{
#ifdef FIPS_MODULE
...
static ossl_inline THREAD_EVENT_HANDLER**
clear_thread_local(OSSL_LIB_CTX* ctx)
{
...
static ossl_inline THREAD_EVENT_HANDLER**
alloc_thread_local(OSSL_LIB_CTX* ctx)
{
...
{
  return c_thread_start(
    FIPS_get_core_handle(libctx), ossl_arg_thread_stop, libctx);
}
...
ossl_thread_event_ctx_new(OSSL_LIB_CTX* libctx)
{
  ...
  if (!CRYPTO_THREAD_set_local_ex(
        CRYPTO_THREAD_LOCAL_TEVENT_KEY, libctx, hands))
    goto err;
...
void
ossl_thread_event_ctx_free(OSSL_LIB_CTX* ctx)
{
...
int
ossl_init_thread_start(const void* index,
                       void* arg,
                       OSSL_thread_stop_handler_fn handfn)
{
  THREAD_EVENT_HANDLER** hands;
