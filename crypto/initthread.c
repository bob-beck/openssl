...
DEFINE_RUN_ONCE_STATIC(create_global_tevent_register) {
  ...
  if (glob_tevent_reg == NULL) return 0;
...
static THREAD_EVENT_HANDLER** manage_thread_local(OSSL_LIB_CTX* ctx, int alloc,
  ...
  if (alloc) {
    if (hands == NULL) {
      if ((hands = OPENSSL_zalloc(sizeof(*hands))) == NULL) return NULL;
...
static ossl_inline THREAD_EVENT_HANDLER** clear_thread_local(
    OSSL_LIB_CTX* ctx) {
  return manage_thread_local(ctx, 0, 0);
}

static ossl_inline ossl_unused THREAD_EVENT_HANDLER** fetch_thread_local(
    OSSL_LIB_CTX* ctx) {
  return manage_thread_local(ctx, 0, 1);
}

static ossl_inline THREAD_EVENT_HANDLER** alloc_thread_local(
    OSSL_LIB_CTX* ctx) {
...
static void init_thread_remove_handlers(THREAD_EVENT_HANDLER** handsin) {
  ...
  gtr = get_global_tevent_register();
  if (gtr == NULL) return;
  if (!CRYPTO_THREAD_write_lock(gtr->lock)) return;
...
int ossl_init_thread_start(const void* index, void* arg,
  ...
  if (*hands == NULL) {
     ...
     */
    if (!ossl_thread_register_fips(ctx)) return 0;
  }
#endif
...
static int init_thread_deregister(void* index, int all) {
  ...
  for (i = 0; i < sk_THREAD_EVENT_HANDLER_PTR_num(gtr->skhands); i++) {
    ...
    if (hands == NULL) {
      if (!all) CRYPTO_THREAD_unlock(gtr->lock);
      return 0;
    }
    curr = *hands;
    while (curr != NULL) {
