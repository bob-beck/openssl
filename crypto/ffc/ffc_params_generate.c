...
static int
generate_unverifiable_g(BN_CTX* ctx,
                        BN_MONT_CTX* mont,
                        BIGNUM* g,
                        BIGNUM* hbn,
                        const BIGNUM* p,
 ...
 */
static int
generate_canonical_g(BN_CTX* ctx,
                     BN_MONT_CTX* mont,
                     const EVP_MD* evpmd,
                     BIGNUM* g,
                     BIGNUM* tmp,
...
static int
generate_q_fips186_4(BN_CTX* ctx,
                     BIGNUM* q,
                     const EVP_MD* evpmd,
                     int qsize,
                     unsigned char* seed,
...
static int
generate_q_fips186_2(BN_CTX* ctx,
                     BIGNUM* q,
                     const EVP_MD* evpmd,
                     unsigned char* buf,
                     unsigned char* seed,
                                     ...
                                     BN_GENCB* cb)
{
  ...
  *res = 0;
  ...
  for (;;) {
    if (!generate_q_fips186_4(
          ctx, q, md, qsize, seed, seedlen, seed != params->seed, &m, res, cb))
      goto err;
    ...
    r = generate_p(
      ctx, md, counter, n, seed_tmp, seedlen, q, p, (int)L, cb, &pcounter, res);
    if (r > 0)
      break; /* found p */
    if (r < 0)
      goto err;
...
g_only:
  ...
  if ((seed != NULL) && (params->gindex != FFC_UNVERIFIABLE_GINDEX)) {
    canonical_g = 1;
    if (!generate_canonical_g(
          ctx, mont, md, g, tmp, p, e, params->gindex, seed, seedlen)) {
      *res = FFC_CHECK_INVALID_G;
      goto err;
    }
                                     ...
                                     BN_GENCB* cb)
{
  ...
  *res = 0;
  ...
  for (;;) {
    if (!generate_q_fips186_2(
          ctx, q, md, buf, seed, qsize, use_random_seed, &m, res, cb))
      goto err;
    ...
    rv = generate_p(
      ctx, md, counter, n, buf, qsize, q, p, (int)L, cb, &pcounter, res);
    if (rv > 0)
      break; /* found it */
    if (rv == -1)
      goto err;
