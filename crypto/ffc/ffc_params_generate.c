...
static int ffc_validate_LN(size_t L, size_t N, int type, int verify) {
  if (type == FFC_PARAM_TYPE_DH) {
    ...
    if (L == 2048 && (N == 224 || N == 256)) return 112;
    ...
    if (verify && L == 1024 && N == 160) return 80;
    if (L == 2048 && (N == 224 || N == 256)) return 112;
    if (L == 3072 && N == 256) return 128;
...
static int generate_unverifiable_g(BN_CTX* ctx, BN_MONT_CTX* mont, BIGNUM* g,
  ...
  if (!BN_set_word(hbn, h)) return 0;

  for (;;) {
    /* Step (3): g = h^e % p */
    if (!BN_mod_exp_mont(g, hbn, e, p, ctx, mont)) return 0;
    /* Step (4): Finish if g > 1 */
    if (BN_cmp(g, BN_value_one()) > 0) break;

    /* Step (2) Choose any h in the range 1 < h < (p-1) */
    if (!BN_add_word(hbn, 1) || BN_cmp(hbn, pm1) >= 0) return 0;
    ++h;
  }
  *hret = h;
  return 1;
...
static int generate_p(BN_CTX* ctx, const EVP_MD* evpmd, int max_counter, int n,
  ...
  if (!BN_lshift(test, BN_value_one(), L - 1)) goto err;

  mdsize = EVP_MD_get_size(evpmd);
  if (mdsize <= 0) goto err;
  ...
  for (i = 0; i <= max_counter; i++) {
    if ((i != 0) && !BN_GENCB_call(cb, 0, i)) goto err;
...
static int generate_q_fips186_4(BN_CTX* ctx, BIGNUM* q, const EVP_MD* evpmd,
  ...
  /* find q */
  for (;;) {
    if (!BN_GENCB_call(cb, 0, m++)) goto err;

    /* A.1.1.2 Step (5) : generate seed with size seed_len */
    if (generate_seed && RAND_bytes_ex(libctx, seed, seedlen, 0) <= 0) goto err;
    /*
     * A.1.1.2 Step (6) AND
     * A.1.1.3 Step (7)
     * U = Hash(seed) % (2^(N-1))
     */
    if (!EVP_Digest(seed, seedlen, md, NULL, evpmd, NULL)) goto err;
    /* Take least significant bits of md */
    if (mdsize > qsize)
      pmd = md + mdsize - qsize;
    else
      pmd = md;
    if (mdsize < qsize) memset(md + mdsize, 0, qsize - mdsize);
    ...
    pmd[0] |= 0x80;
    pmd[qsize - 1] |= 0x01;
    if (!BN_bin2bn(pmd, qsize, q)) goto err;
...
static int generate_q_fips186_2(BN_CTX* ctx, BIGNUM* q, const EVP_MD* evpmd,
  ...
  for (;;) {
    ...
    if (generate_seed && RAND_bytes_ex(libctx, seed, qsize, 0) <= 0) goto err;
    ...
    /* step 2 */
    if (!EVP_Digest(seed, qsize, md, NULL, evpmd, NULL)) goto err;
    if (!EVP_Digest(buf, qsize, buf2, NULL, evpmd, NULL)) goto err;
    for (i = 0; i < (int)qsize; i++) md[i] ^= buf2[i];

    /* step 3 */
    md[0] |= 0x80;
    md[qsize - 1] |= 0x01;
    if (!BN_bin2bn(md, (int)qsize, q)) goto err;
...
int ossl_ffc_params_FIPS186_4_gen_verify(OSSL_LIB_CTX* libctx,
  ...
  *res = 0;
  ...
  if (params->mdname != NULL) {
    ...
    if (N == 0) N = (L >= 2048 ? SHA256_DIGEST_LENGTH : SHA_DIGEST_LENGTH) * 8;
  ...
  mctx = EVP_MD_CTX_new();
  if (mctx == NULL) goto err;

  if ((ctx = BN_CTX_new_ex(libctx)) == NULL) goto err;
  ...
  if (seedlen == 0) seedlen = (size_t)mdsize;
  ...
  if (!BN_lshift(test, BN_value_one(), (int)(L - 1))) goto err;
  ...
  for (;;) {
    ...
    r = generate_p(ctx, md, counter, n, seed_tmp, seedlen, q, p, (int)L, cb,
                   &pcounter, res);
    if (r > 0) break; /* found p */
    if (r < 0) goto err;
    /*
     * A.1.1.3 Step (14):
   ...
   */
  if (verify && (pcounter != counter || (BN_cmp(p, params->p) != 0))) goto err;
...
g_only:
  ...
  if ((seed != NULL) && (params->gindex != FFC_UNVERIFIABLE_GINDEX)) {
    ...
    if (!generate_unverifiable_g(ctx, mont, g, tmp, p, e, pm1, &hret)) goto err;
  }
  ...
  if (!verify) {
    ...
    if (params->p == NULL || params->q == NULL || params->g == NULL) goto err;
...
err:
  if (seed != params->seed) OPENSSL_free(seed);
...
int ossl_ffc_params_FIPS186_2_gen_verify(OSSL_LIB_CTX* libctx,
  ...
  *res = 0;
  ...
  if (seed_in != NULL) {
    ...
    if (seed_len > qsize) seed_len = qsize;
