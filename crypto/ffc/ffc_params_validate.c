...
int ossl_ffc_params_validate_unverifiable_g(BN_CTX* ctx, BN_MONT_CTX* mont,
   ...
   * Check g^q mod p = 1
   */
  if (!BN_mod_exp_mont(tmp, g, q, p, ctx, mont)) return 0;
...
int ossl_ffc_params_simple_validate(OSSL_LIB_CTX* libctx,
  ...
  if (res == NULL) res = &tmpres;

  if (!ossl_ffc_params_copy(&tmpparams, params)) return 0;
...
int ossl_ffc_params_full_validate(OSSL_LIB_CTX* libctx,
  ...
  if (params->seed != NULL) {
    ...
    if (ret) {
      ...
      if ((ctx = BN_CTX_new_ex(libctx)) == NULL) return 0;
