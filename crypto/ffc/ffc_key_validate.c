...
int ossl_ffc_validate_public_key_partial(const FFC_PARAMS* params,
  ...
  *ret = 0;
  ...
  if (tmp == NULL || !BN_set_word(tmp, 1)) goto err;
  if (BN_cmp(pub_key, tmp) <= 0) *ret |= FFC_ERROR_PUBKEY_TOO_SMALL;
  /* Step(1): Verify pub_key <=  p-2 */
  if (BN_copy(tmp, params->p) == NULL || !BN_sub_word(tmp, 1)) goto err;
  if (BN_cmp(pub_key, tmp) >= 0) *ret |= FFC_ERROR_PUBKEY_TOO_LARGE;
  ok = 1;
err:
  if (ctx != NULL) {
    BN_CTX_end(ctx);
...
int ossl_ffc_validate_public_key(const FFC_PARAMS* params,
  ...
  if (!ossl_ffc_validate_public_key_partial(params, pub_key, ret)) return 0;
  ...
  if (*ret == 0 && params->q != NULL) {
    ...
    if (tmp == NULL || !BN_mod_exp(tmp, pub_key, params->q, params->p, ctx))
      goto err;
    if (!BN_is_one(tmp)) *ret |= FFC_ERROR_PUBKEY_INVALID;
  }
