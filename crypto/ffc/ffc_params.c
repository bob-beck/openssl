...
void
ossl_ffc_params_get0_pqg (const FFC_PARAMS *d, const BIGNUM **p,
...
int
ossl_ffc_params_set_seed (FFC_PARAMS *params, const unsigned char *seed,
                          size_t seedlen)
{
  if (params->seed != NULL)
    {
...
void
ossl_ffc_params_enable_flags (FFC_PARAMS *params, unsigned int flags,
                              int enable)
{
  if (enable)
    params->flags |= flags;
...
int
ossl_ffc_params_todata (const FFC_PARAMS *ffc, OSSL_PARAM_BLD *bld,
                        OSSL_PARAM params[])
{
  int test_flags;
