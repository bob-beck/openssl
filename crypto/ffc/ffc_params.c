...
int ossl_ffc_params_set_seed(FFC_PARAMS* params, const unsigned char* seed,
                             size_t seedlen) {
  if (params->seed != NULL) {
    if (params->seed == seed) return 1;
    OPENSSL_free(params->seed);
  }

  if (seed != NULL && seedlen > 0) {
    params->seed = OPENSSL_memdup(seed, seedlen);
    if (params->seed == NULL) return 0;
...
int ossl_ffc_params_set_validate_params(FFC_PARAMS* params,
  ...
  if (!ossl_ffc_params_set_seed(params, seed, seedlen)) return 0;
...
void ossl_ffc_params_get_validate_params(const FFC_PARAMS* params,
                                         ...
                                         int* pcounter) {
  if (seed != NULL) *seed = params->seed;
  if (seedlen != NULL) *seedlen = params->seedlen;
  if (pcounter != NULL) *pcounter = params->pcounter;
}
...
int ossl_ffc_params_print(BIO* bp, const FFC_PARAMS* ffc, int indent) {
  if (!ASN1_bn_print(bp, "prime P:", ffc->p, NULL, indent)) goto err;
  if (!ASN1_bn_print(bp, "generator G:", ffc->g, NULL, indent)) goto err;
  if (ffc->q != NULL &&
  ...
  if (ffc->seed != NULL) {
    ...
    if (!BIO_indent(bp, indent, 128) || BIO_puts(bp, "seed:") <= 0) goto err;
