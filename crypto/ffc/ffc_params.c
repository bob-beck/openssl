...
void
ossl_ffc_params_set0_pqg(FFC_PARAMS* d, BIGNUM* p, BIGNUM* q, BIGNUM* g)
{
  if (p != NULL && p != d->p) {
    BN_free(d->p);
    d->p = p;
...
void
ossl_ffc_params_get0_pqg(const FFC_PARAMS* d,
                         const BIGNUM** p,
                         const BIGNUM** q,
                         const BIGNUM** g)
{
...
void
ossl_ffc_params_enable_flags(FFC_PARAMS* params, unsigned int flags, int enable)
{
  if (enable)
    params->flags |= flags;
  else
...
static int
ffc_bn_cpy(BIGNUM** dst, const BIGNUM* src)
{
  BIGNUM* a;
                       ...
                       OSSL_PARAM params[])
{
  ...
  if (!ossl_param_build_set_int(
        bld, params, OSSL_PKEY_PARAM_FFC_GINDEX, ffc->gindex))
    return 0;
  if (!ossl_param_build_set_int(
        bld, params, OSSL_PKEY_PARAM_FFC_PCOUNTER, ffc->pcounter))
    return 0;
...
ossl_ffc_params_print(BIO* bp, const FFC_PARAMS* ffc, int indent)
{
  ...
  if (ffc->seed != NULL) {
    ...
    for (i = 0; i < ffc->seedlen; i++) {
      ...
      if (BIO_printf(
            bp, "%02x%s", ffc->seed[i], ((i + 1) == ffc->seedlen) ? "" : ":") <=
          0)
        goto err;
    }
    if (BIO_write(bp, "\n", 1) <= 0)
