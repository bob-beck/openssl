...
const EVP_CIPHER* evp_get_cipherbyname_ex(OSSL_LIB_CTX* libctx,
  ...
  if (!OPENSSL_init_crypto(OPENSSL_INIT_ADD_ALL_CIPHERS, NULL)) return NULL;
...
const EVP_MD* evp_get_digestbyname_ex(OSSL_LIB_CTX* libctx, const char* name) {
  ...
  if (!OPENSSL_init_crypto(OPENSSL_INIT_ADD_ALL_DIGESTS, NULL)) return NULL;
