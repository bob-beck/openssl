...
static int evp_md_ctx_reset_ex(EVP_MD_CTX* ctx, int keep_fetched) {
  ...
  if (!keep_fetched) OPENSSL_cleanse(ctx, sizeof(*ctx));
...
int evp_md_ctx_free_algctx(EVP_MD_CTX* ctx) {
  if (ctx->algctx != NULL) {
    ...
    if (ctx->digest->freectx != NULL) ctx->digest->freectx(ctx->algctx);
    ctx->algctx = NULL;
  }
  return 1;
}
...
static int evp_md_init_internal(EVP_MD_CTX* ctx, const EVP_MD* type,
  ...
  if (impl == NULL) tmpimpl = ENGINE_get_digest_engine(type->type);
#endif
  ...
  if (impl != NULL
    ...
    if (ctx->digest == ctx->fetched_digest) ctx->digest = NULL;
    EVP_MD_free(ctx->fetched_digest);
    ctx->fetched_digest = NULL;
    goto legacy;
  }
...
#endif
  if (ctx->flags & EVP_MD_CTX_FLAG_NO_INIT) return 1;
...
int EVP_DigestUpdate(EVP_MD_CTX* ctx, const void* data, size_t count) {
  if (ossl_unlikely(count == 0)) return 1;
...
int EVP_DigestFinal_ex(EVP_MD_CTX* ctx, unsigned char* md,
  ...
  if (ossl_unlikely(ctx->digest == NULL)) return 0;

  sz = EVP_MD_CTX_get_size(ctx);
  if (ossl_unlikely(sz < 0)) return 0;
  mdsize = sz;
  if (ossl_unlikely(ctx->digest->prov == NULL)) goto legacy;
...
legacy:
  ...
  if (isize != NULL) *isize = (unsigned int)mdsize;
...
int EVP_MD_CTX_copy_ex(EVP_MD_CTX* out, const EVP_MD_CTX* in) {
  ...
  if (in->digest == NULL) {
    ...
    EVP_MD_CTX_reset(out);
    if (out->fetched_digest != NULL) EVP_MD_free(out->fetched_digest);
    *out = *in;
    goto clone_pkey;
  }
...
legacy:
  ...
  if (out->digest->copy) return out->digest->copy(out, in);
...
const OSSL_PARAM* EVP_MD_CTX_gettable_params(EVP_MD_CTX* ctx) {
  ...
  if (ossl_unlikely(ctx == NULL)) return NULL;
...
int EVP_MD_CTX_ctrl(EVP_MD_CTX* ctx, int cmd, int p1, void* p2) {
  ...
  if (ctx->digest != NULL && ctx->digest->prov == NULL) goto legacy;
...
static int evp_md_cache_constants(EVP_MD* md) {
  ...
  if (mdsize > INT_MAX || blksz > INT_MAX) ok = 0;
  if (ok) {
    md->block_size = (int)blksz;
    md->md_size = (int)mdsize;
    if (xof) md->flags |= EVP_MD_FLAG_XOF;
    if (algid_absent) md->flags |= EVP_MD_FLAG_DIGALGID_ABSENT;
  }
  return ok;
}
  ...
  for (; fns->function_id != 0; fns++) {
    switch (fns->function_id) {
      ...
      case OSSL_FUNC_DIGEST_DUPCTX:
        if (md->dupctx == NULL) md->dupctx = OSSL_FUNC_digest_dupctx(fns);
        break;
      ...
      case OSSL_FUNC_DIGEST_COPYCTX:
        if (md->copyctx == NULL) md->copyctx = OSSL_FUNC_digest_copyctx(fns);
        break;
    }
  }
...
int EVP_MD_up_ref(EVP_MD* md) {
  ...
  if (md->origin == EVP_ORIG_DYNAMIC) CRYPTO_UP_REF(&md->refcnt, &ref);
  return 1;
}
...
void EVP_MD_free(EVP_MD* md) {
  ...
  if (md == NULL || md->origin != EVP_ORIG_DYNAMIC) return;
