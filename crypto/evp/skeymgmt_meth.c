...
void *
evp_skeymgmt_import (const EVP_SKEYMGMT *skeymgmt, int selection,
...
int
evp_skeymgmt_export (const EVP_SKEYMGMT *skeymgmt, void *keydata,
...
void
evp_skeymgmt_freedata (const EVP_SKEYMGMT *skeymgmt, void *keydata)
{
  /* This is mandatory, no need to check for its presence */
...
static void *
skeymgmt_from_algorithm (int name_id, const OSSL_ALGORITHM *algodef,
                         OSSL_PROVIDER *prov)
{
...
EVP_SKEYMGMT *
evp_skeymgmt_fetch_from_prov (OSSL_PROVIDER *prov, const char *name,
...
EVP_SKEYMGMT *
EVP_SKEYMGMT_fetch (OSSL_LIB_CTX *ctx, const char *algorithm,
                    const char *properties)
{
...
int
EVP_SKEYMGMT_up_ref (EVP_SKEYMGMT *skeymgmt)
{
  int ref = 0;
...
void
EVP_SKEYMGMT_free (EVP_SKEYMGMT *skeymgmt)
{
  int ref = 0;
...
const OSSL_PROVIDER *
EVP_SKEYMGMT_get0_provider (const EVP_SKEYMGMT *skeymgmt)
{
  return (skeymgmt != NULL) ? skeymgmt->prov : NULL;
}

const char *
EVP_SKEYMGMT_get0_description (const EVP_SKEYMGMT *skeymgmt)
{
  return (skeymgmt != NULL) ? skeymgmt->description : NULL;
}

const char *
EVP_SKEYMGMT_get0_name (const EVP_SKEYMGMT *skeymgmt)
{
  return (skeymgmt != NULL) ? skeymgmt->type_name : NULL;
}

int
EVP_SKEYMGMT_is_a (const EVP_SKEYMGMT *skeymgmt, const char *name)
{
...
void
EVP_SKEYMGMT_do_all_provided (OSSL_LIB_CTX *libctx,
                              void (*fn) (EVP_SKEYMGMT *skeymgmt, void *arg),
                              void *arg)
{
  evp_generic_do_all (libctx, OSSL_OP_SKEYMGMT, (void (*) (void *, void *))fn,
...
int
EVP_SKEYMGMT_names_do_all (const EVP_SKEYMGMT *skeymgmt,
