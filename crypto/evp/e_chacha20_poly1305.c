...
static int
chacha_init_key(EVP_CIPHER_CTX* ctx,
...
static int
chacha_cipher(EVP_CIPHER_CTX* ctx,
              unsigned char* out,
              ...
              size_t len)
{
  ...
  if (rem) {
    ...
    ChaCha20_ctr32(
      key->buf, key->buf, CHACHA_BLK_SIZE, key->key.d, key->counter);
    for (n = 0; n < rem; n++)
...
static int
chacha20_poly1305_init_key(EVP_CIPHER_CTX* ctx,
...
void*
xor128_encrypt_n_pad(void* out, const void* inp, void* otp, size_t len);
void*
xor128_decrypt_n_pad(void* out, const void* inp, void* otp, size_t len);
...
static int
chacha20_poly1305_tls_cipher(EVP_CIPHER_CTX* ctx,
                             unsigned char* out,
...
static int
chacha20_poly1305_cipher(EVP_CIPHER_CTX* ctx,
                         unsigned char* out,
                         const unsigned char* in,
                         size_t len)
{
  ...
  if (!actx->mac_inited) {
    ...
    actx->key.counter[0] = 0;
    ChaCha20_ctr32(
      actx->key.buf, zero, CHACHA_BLK_SIZE, actx->key.key.d, actx->key.counter);
  ...
  if (in == NULL        /* explicit final */
    ...
    if (IS_LITTLE_ENDIAN) {
      Poly1305_Update(
        POLY1305_ctx(actx), (unsigned char*)&actx->len, POLY1305_BLOCK_SIZE);
    } else {
