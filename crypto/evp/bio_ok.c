...
static int ok_write(BIO* b, const char* in, int inl) {
  ...
  do {
    ...
    while (ctx->blockout && n > 0) {
      ...
      if (i <= 0) {
        BIO_copy_next_retry(b);
        if (!BIO_should_retry(b)) ctx->cont = 0;
        return i;
      }
      ctx->buf_off += i;
      n -= i;
...
static long ok_ctrl(BIO* b, int cmd, long num, void* ptr) {
  ...
  switch (cmd) {
    ...
    case BIO_C_SET_MD:
      md = ptr;
      if (!EVP_DigestInit_ex(ctx->md, md, NULL)) return 0;
      BIO_set_init(b, 1);
      break;
    case BIO_C_GET_MD:
      if (BIO_get_init(b)) {
...
static int sig_out(BIO* b) {
  ...
  if (md_size <= 0) goto berr;
  if (ctx->buf_len + 2 * md_size > OK_BLOCK_SIZE) return 1;

  if (!EVP_DigestInit_ex(md, digest, NULL)) goto berr;
  /*
   * FIXME: there's absolutely no guarantee this makes any sense at all,
   * particularly now EVP_MD_CTX has been restructured.
   */
  if (RAND_bytes(md_data, md_size) <= 0) goto berr;
  memcpy(&(ctx->buf[ctx->buf_len]), md_data, md_size);
  longswap(&(ctx->buf[ctx->buf_len]), md_size);
  ctx->buf_len += md_size;

  if (!EVP_DigestUpdate(md, WELLKNOWN, strlen(WELLKNOWN))) goto berr;
  if (!EVP_DigestFinal_ex(md, &(ctx->buf[ctx->buf_len]), NULL)) goto berr;
  ctx->buf_len += md_size;
  ctx->blockout = 1;
  ctx->sigio = 0;
  return 1;
...
static int sig_in(BIO* b) {
  ...
  ctx = BIO_get_data(b);
  if ((md = ctx->md) == NULL) goto berr;
  digest = EVP_MD_CTX_get0_md(md);
  if ((md_size = EVP_MD_get_size(digest)) <= 0) goto berr;
  md_data = EVP_MD_CTX_get0_md_data(md);

  if ((int)(ctx->buf_len - ctx->buf_off) < 2 * md_size) return 1;
  ...
  if (!EVP_DigestUpdate(md, WELLKNOWN, strlen(WELLKNOWN))) goto berr;
  if (!EVP_DigestFinal_ex(md, tmp, NULL)) goto berr;
...
static int block_in(BIO* b) {
  ...
  if (ctx->buf_len < tl + OK_BLOCK_BLOCK + md_size) return 1;
