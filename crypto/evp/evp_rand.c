...
static int evp_rand_up_ref(void* vrand) {
  ...
  if (rand != NULL) return CRYPTO_UP_REF(&rand->refcnt, &ref);
  return 1;
}
...
static int evp_rand_lock(EVP_RAND_CTX* rand) {
  if (rand->meth->lock != NULL) return rand->meth->lock(rand->algctx);
  return 1;
}

/* Unlock the underlying DRBG/RAND if available */
static void evp_rand_unlock(EVP_RAND_CTX* rand) {
  if (rand->meth->unlock != NULL) rand->meth->unlock(rand->algctx);
}
...
int EVP_RAND_get_params(EVP_RAND* rand, OSSL_PARAM params[]) {
  if (rand->get_params != NULL) return rand->get_params(params);
  return 1;
}
...
const OSSL_PARAM* EVP_RAND_gettable_params(const EVP_RAND* rand) {
  if (rand->gettable_params == NULL) return NULL;
...
const OSSL_PARAM* EVP_RAND_gettable_ctx_params(const EVP_RAND* rand) {
  ...
  if (rand->gettable_ctx_params == NULL) return NULL;
...
const OSSL_PARAM* EVP_RAND_settable_ctx_params(const EVP_RAND* rand) {
  ...
  if (rand->settable_ctx_params == NULL) return NULL;
  provctx = ossl_provider_ctx(EVP_RAND_get0_provider(rand));
...
const OSSL_PARAM* EVP_RAND_CTX_gettable_params(EVP_RAND_CTX* ctx) {
  ...
  if (ctx->meth->gettable_ctx_params == NULL) return NULL;
...
const OSSL_PARAM* EVP_RAND_CTX_settable_params(EVP_RAND_CTX* ctx) {
  ...
  if (ctx->meth->settable_ctx_params == NULL) return NULL;
  provctx = ossl_provider_ctx(EVP_RAND_get0_provider(ctx->meth));
...
static unsigned int evp_rand_strength_locked(EVP_RAND_CTX* ctx) {
  ...
  if (!evp_rand_get_ctx_params_locked(ctx, params)) return 0;
  return strength;
}
...
int EVP_RAND_get_state(EVP_RAND_CTX* ctx) {
  ...
  if (!EVP_RAND_CTX_get_params(ctx, params)) state = EVP_RAND_STATE_ERROR;
  return state;
}
