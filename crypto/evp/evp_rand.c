...
static int evp_rand_instantiate_locked(EVP_RAND_CTX        *ctx,
...
int EVP_RAND_instantiate(EVP_RAND_CTX        *ctx,
                         unsigned int         strength,
                         int                  prediction_resistance,
                         const unsigned char *pstr,
                         size_t               pstr_len,
...
static int evp_rand_generate_locked(EVP_RAND_CTX        *ctx,
                                    unsigned char       *out,
                                    size_t               outlen,
                                    unsigned int         strength,
                                    int                  prediction_resistance,
...
int EVP_RAND_generate(EVP_RAND_CTX        *ctx,
                      unsigned char       *out,
                      size_t               outlen,
                      unsigned int         strength,
                      int                  prediction_resistance,
...
static int evp_rand_reseed_locked(EVP_RAND_CTX        *ctx,
                                  int                  prediction_resistance,
                                       ...
                                       size_t               adin_len)
{
    if (ctx->meth->get_seed != NULL)
        return ctx->meth
            ->get_seed(ctx->algctx, buffer, entropy, min_len, max_len, prediction_resistance, adin, adin_len);
    return 0;
}
