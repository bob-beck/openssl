...
int
EVP_RAND_enable_locking(EVP_RAND_CTX* rand)
{
...
static int
evp_rand_lock(EVP_RAND_CTX* rand)
{
...
static void
evp_rand_unlock(EVP_RAND_CTX* rand)
{
...
static void*
evp_rand_from_algorithm(int name_id,
...
EVP_RAND*
EVP_RAND_fetch(OSSL_LIB_CTX* libctx,
               const char* algorithm,
               const char* properties)
{
  return evp_generic_fetch(libctx,
                           OSSL_OP_RAND,
                           algorithm,
                           properties,
...
const char*
EVP_RAND_get0_name(const EVP_RAND* rand)
{
  return rand->type_name;
}

const char*
EVP_RAND_get0_description(const EVP_RAND* rand)
{
  return rand->description;
}

int
EVP_RAND_is_a(const EVP_RAND* rand, const char* name)
{
...
int
EVP_RAND_get_params(EVP_RAND* rand, OSSL_PARAM params[])
{
  if (rand->get_params != NULL)
...
EVP_RAND_CTX*
EVP_RAND_CTX_new(EVP_RAND* rand, EVP_RAND_CTX* parent)
{
  EVP_RAND_CTX* ctx;
  ...
  if ((ctx->algctx = rand->newctx(
         ossl_provider_ctx(rand->prov), parent_ctx, parent_dispatch)) == NULL ||
...
static int
evp_rand_get_ctx_params_locked(EVP_RAND_CTX* ctx, OSSL_PARAM params[])
{
  return ctx->meth->get_ctx_params(ctx->algctx, params);
}

int
EVP_RAND_CTX_get_params(EVP_RAND_CTX* ctx, OSSL_PARAM params[])
{
  int res;
...
static int
evp_rand_set_ctx_params_locked(EVP_RAND_CTX* ctx, const OSSL_PARAM params[])
{
...
int
EVP_RAND_CTX_set_params(EVP_RAND_CTX* ctx, const OSSL_PARAM params[])
{
  int res;
...
const OSSL_PARAM*
EVP_RAND_gettable_params(const EVP_RAND* rand)
{
  if (rand->gettable_params == NULL)
    return NULL;
...
const OSSL_PARAM*
EVP_RAND_gettable_ctx_params(const EVP_RAND* rand)
{
  void* provctx;
...
const OSSL_PARAM*
EVP_RAND_settable_ctx_params(const EVP_RAND* rand)
{
  void* provctx;
...
const OSSL_PARAM*
EVP_RAND_CTX_settable_params(EVP_RAND_CTX* ctx)
{
  void* provctx;
...
void
EVP_RAND_do_all_provided(OSSL_LIB_CTX* libctx,
...
int
EVP_RAND_names_do_all(const EVP_RAND* rand,
...
static int
evp_rand_instantiate_locked(EVP_RAND_CTX* ctx,
...
{
  return ctx->meth->instantiate(
    ctx->algctx, strength, prediction_resistance, pstr, pstr_len, params);
}

int
EVP_RAND_instantiate(EVP_RAND_CTX* ctx,
                     unsigned int strength,
                     int prediction_resistance,
                     const unsigned char* pstr,
                     size_t pstr_len,
                     const OSSL_PARAM params[])
{
  ...
  if (!evp_rand_lock(ctx))
    return 0;
  res = evp_rand_instantiate_locked(
    ctx, strength, prediction_resistance, pstr, pstr_len, params);
  evp_rand_unlock(ctx);
  return res;
}

static int
evp_rand_uninstantiate_locked(EVP_RAND_CTX* ctx)
{
...
static int
evp_rand_generate_locked(EVP_RAND_CTX* ctx,
                         unsigned char* out,
                         size_t outlen,
                         unsigned int strength,
                         int prediction_resistance,
                         ...
                         size_t addin_len)
{
  ...
  for (; outlen > 0; outlen -= chunk, out += chunk) {
    ...
    if (!ctx->meth->generate(ctx->algctx,
                             out,
                             chunk,
                             strength,
...
int
EVP_RAND_generate(EVP_RAND_CTX* ctx,
                  unsigned char* out,
                  size_t outlen,
                  unsigned int strength,
                  int prediction_resistance,
                  ...
                  size_t addin_len)
{
  ...
  if (!evp_rand_lock(ctx))
    return 0;
  res = evp_rand_generate_locked(
    ctx, out, outlen, strength, prediction_resistance, addin, addin_len);
  evp_rand_unlock(ctx);
  return res;
}

static int
evp_rand_reseed_locked(EVP_RAND_CTX* ctx,
                       int prediction_resistance,
                       ...
                       size_t addin_len)
{
  if (ctx->meth->reseed != NULL)
    return ctx->meth->reseed(
      ctx->algctx, prediction_resistance, ent, ent_len, addin, addin_len);
  return 1;
}

int
EVP_RAND_reseed(EVP_RAND_CTX* ctx,
                int prediction_resistance,
                const unsigned char* ent,
                size_t ent_len,
                const unsigned char* addin,
                size_t addin_len)
{
  ...
  if (!evp_rand_lock(ctx))
    return 0;
  res = evp_rand_reseed_locked(
    ctx, prediction_resistance, ent, ent_len, addin, addin_len);
  evp_rand_unlock(ctx);
  return res;
}

static unsigned int
evp_rand_strength_locked(EVP_RAND_CTX* ctx)
{
  OSSL_PARAM params[2] = { OSSL_PARAM_END, OSSL_PARAM_END };
...
static int
evp_rand_nonce_locked(EVP_RAND_CTX* ctx, unsigned char* out, size_t outlen)
{
...
int
EVP_RAND_nonce(EVP_RAND_CTX* ctx, unsigned char* out, size_t outlen)
{
  int res;
...
static int
evp_rand_verify_zeroization_locked(EVP_RAND_CTX* ctx)
{
...
static size_t
evp_rand_get_seed_locked(EVP_RAND_CTX* ctx,
                         ...
                         size_t adin_len)
{
  if (ctx->meth->get_seed != NULL)
    return ctx->meth->get_seed(ctx->algctx,
                               buffer,
                               entropy,
                               min_len,
                               max_len,
...
static void
evp_rand_clear_seed_locked(EVP_RAND_CTX* ctx,
                           unsigned char* buffer,
                           size_t b_len)
{
