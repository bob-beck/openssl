...
static int default_fixup_args(enum state state,
  ...
  if ((ret = default_check(state, translation, ctx)) <= 0) return ret;
        ...
        switch (translation->param_data_type) {
          ...
          case OSSL_PARAM_UNSIGNED_INTEGER:
            if (ctx->p2 != NULL) {
              ...
              if (!OSSL_PARAM_get_BN(ctx->params, ctx->p2)) return 0;
            } else {
...
static int cleanup_translation_ctx(enum state state,
                                   const struct translation_st* translation,
                                   struct translation_ctx_st* ctx) {
  if (ctx->allocated_buf != NULL) OPENSSL_free(ctx->allocated_buf);
  ctx->allocated_buf = NULL;
  return 1;
}
...
static int fix_cipher_md(
  ...
  if ((ret = default_fixup_args(state, translation, ctx)) <= 0) return ret;
...
static int fix_kdf_type(enum state state,
  ...
  if (state == PRE_CTRL_TO_PARAMS) {
     ...
     * be OSSL_ACTION_NONE.
     */
    if (!ossl_assert(ctx->action_type == OSSL_ACTION_NONE)) return 0;
...
static int fix_dh_nid(enum state state,
  ...
  if (ctx->action_type != OSSL_ACTION_SET) return 0;
...
static int fix_rsa_padding_mode(enum state state,
  ...
  if (state == PRE_CTRL_TO_PARAMS && ctx->action_type == OSSL_ACTION_GET) {
    ...
    for (i = 0; i < OSSL_NELEM(str_value_map); i++) {
      if (ctx->p1 == (int)str_value_map[i].id) break;
    }
  ...
  if ((ctx->action_type == OSSL_ACTION_SET && state == PRE_PARAMS_TO_CTRL) ||
    ...
    for (i = 0; i < OSSL_NELEM(str_value_map); i++) {
      if (strcmp(ctx->p2, str_value_map[i].ptr) == 0) break;
    }
...
static int fix_hkdf_mode(enum state state,
  ...
  if ((ctx->action_type == OSSL_ACTION_SET && state == PRE_CTRL_TO_PARAMS) ||
    ...
    if (i == OSSL_NELEM(str_value_map)) return 0;
    ctx->p2 = str_value_map[i].ptr;
    ctx->p1 = (int)strlen(ctx->p2);
  }
...
static int get_payload_group_name(enum state state,
  ...
  switch (EVP_PKEY_get_base_id(pkey)) {
    ...
    case EVP_PKEY_EC: {
      ...
      if (grp != NULL) nid = EC_GROUP_get_curve_name(grp);
      if (nid != NID_undef) ctx->p2 = (char*)OSSL_EC_curve_nid2name(nid);
    } break;
#endif
    default:
...
static int get_payload_public_key_ec(enum state state,
  ...
  if (ctx->params->data_type != OSSL_PARAM_UNSIGNED_INTEGER) goto out;

  x = BN_CTX_get(bnctx);
  y = BN_CTX_get(bnctx);
  if (y == NULL) goto out;

  if (!EC_POINT_get_affine_coordinates(ecg, point, x, y, bnctx)) goto out;
...
static int get_payload_int(enum state state,
  ...
  if (ctx->params->data_type != OSSL_PARAM_INTEGER) return 0;
  ctx->p1 = val;
  ctx->p2 = NULL;
...
static const struct translation_st* lookup_translation(
    struct translation_st* tmpl, const struct translation_st* translations,
  ...
  for (i = 0; i < translations_num; i++) {
     ...
     */
    if (item->optype != -1 && (tmpl->optype & item->optype) == 0) continue;
    /*
     ...
     */
    if (tmpl->ctrl_num != 0) {
      if (tmpl->ctrl_num != item->ctrl_num) continue;
...
static const struct translation_st* lookup_evp_pkey_ctx_translation(
    struct translation_st* tmpl) {
  return lookup_translation(tmpl, evp_pkey_ctx_translations,
                            OSSL_NELEM(evp_pkey_ctx_translations));
}

static const struct translation_st* lookup_evp_pkey_translation(
    struct translation_st* tmpl) {
...
int evp_pkey_ctx_ctrl_to_param(EVP_PKEY_CTX* pctx, int keytype, int optype,
  ...
  if (keytype == -1) keytype = pctx->legacy_keytype;
  tmpl.ctrl_num = cmd;
  ...
  if (translation->fixup_args != NULL) fixup = translation->fixup_args;
...
int evp_pkey_ctx_ctrl_str_to_param(EVP_PKEY_CTX* pctx, const char* name,
  ...
  if (ret > 0) ret = fixup(POST_CTRL_STR_TO_PARAMS, translation, &ctx);
