 ...
 *                              necessary, when |action_type ==
 * OSSL_ACTION_SET|. With the POST_CTRL_TO_PARAMS state, the fixup_args
 * function is expected to modify the passed |p2| in whatever way necessary,
 ...
 *                              way necessary, when |action_type ==
 * OSSL_ACTION_SET|. With the POST_CTRL_STR_TO_PARAMS state, the fixup_args
 * function is only expected to return a value.
 *
 ...
 *                              necessary, when |action_type ==
 * OSSL_ACTION_SET|. With the POST_PARAMS_TO_CTRL state, the fixup_args
 * function is expected to modify the passed |*params| in whatever way
 * necessary, when |action_type == OSSL_ACTION_GET|.
 *
...
struct translation_ctx_st
{
  ...
  EVP_PKEY_CTX *pctx;
  /*
   * The action type (OSSL_ACTION_GET or OSSL_ACTION_SET). This may be 0 in
   * some cases, and should be modified by the fixup_args function in the PRE
   * states.  It should otherwise remain untouched once set.
   */
...
struct translation_st
{
   ...
   *
   * If the item has this set to 0, it means that both OSSL_ACTION_GET and
   * OSSL_ACTION_SET are supported, and |fixup_args| will determine which it
   * is.  This is to support translations of ctrls where the action type
   * depends on the value of |p1| or |p2| (ctrls are really bi-directional, but
   * are seldom used that way).
   *
   ...
   * Fixer functions
   *
   * |fixup_args| is always called before (for OSSL_ACTION_SET) or after (for
   * OSSL_ACTION_GET) the actual ctrl / OSSL_PARAM function.
   */
 ...
 */
static int
default_check (enum state state, const struct translation_st *translation,
 ...
 * data between p1+p2 (OSSL_ACTION_SET) or return value+p2 (OSSL_ACTION_GET) as
 * long as the values don't have extra semantics (such as NIDs, OIDs, that sort
 * of stuff). Extra semantics must be handled via specific fixup_args
 * functions.
 *
 ...
 */
static int
default_fixup_args (enum state state, const struct translation_st *translation,
                    struct translation_ctx_st *ctx)
{
  int ret;
  ...
  switch (state)
    ...
    case PRE_CTRL_TO_PARAMS:
      ...
      /*
       * OSSL_PARAM_construct_TYPE() works equally well for OSSL_ACTION_SET and
       * OSSL_ACTION_GET.
       */
            ...
            switch (translation->param_data_type)
              ...
              case OSSL_PARAM_OCTET_STRING:
                return OSSL_PARAM_get_octet_string (
                    ctx->params, &ctx->p2, ctx->sz, (size_t *)&ctx->p1);
              case OSSL_PARAM_OCTET_PTR:
                return OSSL_PARAM_get_octet_ptr (ctx->params, ctx->p2,
                                                 &ctx->sz);
              default:
                ERR_raise_data (ERR_LIB_EVP, ERR_R_UNSUPPORTED,
                                "[action:%d, state:%d] "
            ...
            switch (param_data_type)
              ...
              case OSSL_PARAM_OCTET_STRING:
                return OSSL_PARAM_set_octet_string (ctx->params, ctx->p2,
                                                    size);
              case OSSL_PARAM_OCTET_PTR:
                return OSSL_PARAM_set_octet_ptr (ctx->params,
                                                 *(void **)ctx->p2, size);
              default:
                ERR_raise_data (ERR_LIB_EVP, ERR_R_UNSUPPORTED,
                                "[action:%d, state:%d] "
...
/*
 * fix_cipher_md fixes up an EVP_CIPHER / EVP_MD to its name on
 * OSSL_ACTION_SET, and cipher / md name to EVP_MD on OSSL_ACTION_GET.
 */
static const char *
get_cipher_name (void *cipher)
{
...
static const void *
get_cipher_by_name (OSSL_LIB_CTX *libctx, const char *name)
{
  return evp_get_cipherbyname_ex (libctx, name);
}

static const void *
get_md_by_name (OSSL_LIB_CTX *libctx, const char *name)
{
  return evp_get_digestbyname_ex (libctx, name);
}

static int
fix_cipher_md (enum state state, const struct translation_st *translation,
               struct translation_ctx_st *ctx,
                                                ...
                                                const char *name))
{
  ...
  if (state == PRE_CTRL_TO_PARAMS && ctx->action_type == OSSL_ACTION_GET)
  ...
  else if (state == PRE_CTRL_TO_PARAMS && ctx->action_type == OSSL_ACTION_SET)
    {
       ...
       */
      ctx->p2 = (char *)(ctx->p2 == NULL ? OBJ_nid2sn (ctx->p1)
                                         : get_name (ctx->p2));
...
static int
fix_cipher (enum state state, const struct translation_st *translation,
            struct translation_ctx_st *ctx)
{
...
static int
fix_md (enum state state, const struct translation_st *translation,
        struct translation_ctx_st *ctx)
{
...
static int
fix_distid_len (enum state state, const struct translation_st *translation,
                struct translation_ctx_st *ctx)
{
...
static int
fix_kdf_type (enum state state, const struct translation_st *translation,
              struct translation_ctx_st *ctx,
...
static int
fix_dh_kdf_type (enum state state, const struct translation_st *translation,
                 struct translation_ctx_st *ctx)
{
...
static int
fix_ec_kdf_type (enum state state, const struct translation_st *translation,
                 struct translation_ctx_st *ctx)
{
  static const struct kdf_type_map_st kdf_type_map[]
...
static int
fix_oid (enum state state, const struct translation_st *translation,
         struct translation_ctx_st *ctx)
{
  int ret;
...
static int
fix_dh_nid (enum state state, const struct translation_st *translation,
            struct translation_ctx_st *ctx)
{
  int ret;
...
static int
fix_dh_nid5114 (enum state state, const struct translation_st *translation,
                struct translation_ctx_st *ctx)
{
  int ret;
...
static int
fix_dh_paramgen_type (enum state state,
                      const struct translation_st *translation,
                      struct translation_ctx_st *ctx)
{
  int ret;
...
static int
fix_ec_param_enc (enum state state, const struct translation_st *translation,
                  struct translation_ctx_st *ctx)
{
  int ret;
...
static int
fix_ec_paramgen_curve_nid (enum state state,
                           const struct translation_st *translation,
                           struct translation_ctx_st *ctx)
{
  char *p2 = NULL;
...
static int
fix_ecdh_cofactor (enum state state, const struct translation_st *translation,
                   struct translation_ctx_st *ctx)
{
  /*
...
static int
fix_rsa_padding_mode (enum state state,
                      const struct translation_st *translation,
                      struct translation_ctx_st *ctx)
{
  ...
  if (state == PRE_CTRL_TO_PARAMS && ctx->action_type == OSSL_ACTION_GET)
    {
       ...
       * of the rest for us, along with the POST_CTRL_TO_PARAMS &&
       * OSSL_ACTION_GET code section further down.
       */
      ctx->orig_p2 = ctx->p2;
      ctx->p2 = ctx->name_buf;
      ctx->p1 = sizeof (ctx->name_buf);
...
static int
fix_rsa_pss_saltlen (enum state state,
                     const struct translation_st *translation,
                     struct translation_ctx_st *ctx)
{
  static const OSSL_ITEM str_value_map[]
...
static int
fix_hkdf_mode (enum state state, const struct translation_st *translation,
               struct translation_ctx_st *ctx)
{
  static const OSSL_ITEM str_value_map[]
 ...
 * a post-ctrl OSSL_ACTION_GET fixup.  They all get NULL ctx, ctrl_cmd,
 * ctrl_str, p1, sz
 */

/* Pilfering DH, DSA and EC_KEY */
static int
get_payload_group_name (enum state state,
                        const struct translation_st *translation,
                        struct translation_ctx_st *ctx)
{
...
static int
get_payload_private_key (enum state state,
                         const struct translation_st *translation,
                         struct translation_ctx_st *ctx)
{
  EVP_PKEY *pkey = ctx->p2;
...
static int
get_payload_public_key (enum state state,
                        const struct translation_st *translation,
                        struct translation_ctx_st *ctx)
{
  EVP_PKEY *pkey = ctx->p2;
...
static int
get_payload_public_key_ec (enum state state,
                           const struct translation_st *translation,
                           struct translation_ctx_st *ctx)
{
#ifndef OPENSSL_NO_EC
...
static int
get_dh_dsa_payload_p (enum state state,
                      const struct translation_st *translation,
                      struct translation_ctx_st *ctx)
{
...
static int
get_dh_dsa_payload_q (enum state state,
                      const struct translation_st *translation,
                      struct translation_ctx_st *ctx)
{
  const BIGNUM *bn = NULL;
...
static int
get_dh_dsa_payload_g (enum state state,
                      const struct translation_st *translation,
                      struct translation_ctx_st *ctx)
{
  const BIGNUM *bn = NULL;
...
static int
get_payload_int (enum state state, const struct translation_st *translation,
...
static int
get_rsa_payload_n (enum state state, const struct translation_st *translation,
                   struct translation_ctx_st *ctx)
{
  const BIGNUM *bn = NULL;
...
static int
get_rsa_payload_e (enum state state, const struct translation_st *translation,
                   struct translation_ctx_st *ctx)
{
  const BIGNUM *bn = NULL;
...
static int
get_rsa_payload_d (enum state state, const struct translation_st *translation,
                   struct translation_ctx_st *ctx)
{
  const BIGNUM *bn = NULL;
...
static int
get_rsa_payload_factor (enum state state,
                        const struct translation_st *translation,
...
static int
get_rsa_payload_exponent (enum state state,
                          const struct translation_st *translation,
...
static int
get_rsa_payload_coefficient (enum state state,
                             const struct translation_st *translation,
                             struct translation_ctx_st *ctx,
...
static int
fix_group_ecx (enum state state, const struct translation_st *translation,
               struct translation_ctx_st *ctx)
{
...
int
evp_pkey_ctx_ctrl_to_param (EVP_PKEY_CTX *pctx, int keytype, int optype,
...
/* This must ONLY be called for provider side operations */
int
evp_pkey_ctx_ctrl_str_to_param (EVP_PKEY_CTX *pctx, const char *name,
                                const char *value)
{
  struct translation_ctx_st ctx = {
    0,
...
static int
evp_pkey_ctx_setget_params_to_ctrl (EVP_PKEY_CTX *pctx,
                                    enum action action_type,
                                    OSSL_PARAM *params)
{
  int keytype = pctx->legacy_keytype;
...
static int
evp_pkey_setget_params_to_ctrl (const EVP_PKEY *pkey, enum action action_type,
                                OSSL_PARAM *params)
{
  int ret = 1;
