...
#define IMPL_GET_RSA_PAYLOAD_FACTOR(n)                                                                                 \
    static int get_rsa_payload_f##n(enum state state, const struct translation_st *translation,                        \
                                    struct translation_ctx_st *ctx)                                                    \
    {                                                                                                                  \
        if (EVP_PKEY_get_base_id(ctx->p2) != EVP_PKEY_RSA && EVP_PKEY_get_base_id(ctx->p2) != EVP_PKEY_RSA_PSS)        \
            return 0;                                                                                                  \
...
#define IMPL_GET_RSA_PAYLOAD_EXPONENT(n)                                                                               \
    static int get_rsa_payload_e##n(enum state state, const struct translation_st *translation,                        \
                                    struct translation_ctx_st *ctx)                                                    \
    {                                                                                                                  \
        if (EVP_PKEY_get_base_id(ctx->p2) != EVP_PKEY_RSA && EVP_PKEY_get_base_id(ctx->p2) != EVP_PKEY_RSA_PSS)        \
            return 0;                                                                                                  \
...
#define IMPL_GET_RSA_PAYLOAD_COEFFICIENT(n)                                                                            \
    static int get_rsa_payload_c##n(enum state state, const struct translation_st *translation,                        \
                                    struct translation_ctx_st *ctx)                                                    \
    {                                                                                                                  \
        if (EVP_PKEY_get_base_id(ctx->p2) != EVP_PKEY_RSA && EVP_PKEY_get_base_id(ctx->p2) != EVP_PKEY_RSA_PSS)        \
            return 0;                                                                                                  \
