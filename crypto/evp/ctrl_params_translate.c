 ...
 * OSSL_ACTION_SET|. With the POST_CTRL_TO_PARAMS state, the fixup_args function
 * is expected to modify the passed |p2| in whatever way necessary, when
 * |action_type == OSSL_ACTION_GET|.
 *
 ...
 *                              necessary, when |action_type ==
 * OSSL_ACTION_SET|. With the POST_PARAMS_TO_CTRL state, the fixup_args function
 * is expected to modify the passed |*params| in whatever way necessary, when
 * |action_type == OSSL_ACTION_GET|.
 *
...
typedef int
fixup_args_fn(enum state state,
              const struct translation_st* translation,
              struct translation_ctx_st* ctx);
typedef int
cleanup_args_fn(enum state state,
                const struct translation_st* translation,
                struct translation_ctx_st* ctx);
...
struct translation_ctx_st
{
  ...
  EVP_PKEY_CTX* pctx;
  /*
   * The action type (OSSL_ACTION_GET or OSSL_ACTION_SET). This may be 0 in some
   * cases, and should be modified by the fixup_args function in the PRE states.
   * It should otherwise remain untouched once set.
   */
...
struct translation_st
{
   ...
   * OSSL_ACTION_SET are supported, and |fixup_args| will determine which it is.
   * This is to support translations of ctrls where the action type depends on
   * the value of |p1| or |p2| (ctrls are really bi-directional, but are seldom
   * used that way).
   *
 ...
 */
static int
default_check(enum state state,
              const struct translation_st* translation,
 ...
 * of stuff). Extra semantics must be handled via specific fixup_args functions.
 *
 ...
 */
static int
default_fixup_args(enum state state,
                   const struct translation_st* translation,
                   struct translation_ctx_st* ctx)
{
  int ret;
          ...
          if (ctx->p2 != NULL) {
            if (ctx->action_type == OSSL_ACTION_SET) {
              ...
              if ((ctx->allocated_buf = OPENSSL_malloc(ctx->buflen)) == NULL)
                return 0;
              if (BN_bn2nativepad(
                    ctx->p2, ctx->allocated_buf, (int)ctx->buflen) < 0) {
    ...
    case PRE_CTRL_STR_TO_PARAMS: {
      ...
      if (translation != NULL) {
        ...
        if (ctx->ishex) {
          ...
          if (OPENSSL_strlcat(
                ctx->name_buf, tmp_ctrl_str, sizeof(ctx->name_buf)) <= 3) {
            ERR_raise(ERR_LIB_EVP, ERR_R_INTERNAL_ERROR);
            return -1;
          }
      ...
      if (!OSSL_PARAM_allocate_from_text(ctx->params,
                                         settable,
                                         tmp_ctrl_str,
                                         ctx->p2,
                                         strlen(ctx->p2),
    ...
    case PRE_PARAMS_TO_CTRL: {
      ...
      if (state == PRE_PARAMS_TO_CTRL && ctx->action_type == OSSL_ACTION_SET) {
        ...
        switch (param_data_type) {
          ...
          case OSSL_PARAM_OCTET_PTR:
            return OSSL_PARAM_set_octet_ptr(
              ctx->params, *(void**)ctx->p2, size);
          default:
            ERR_raise_data(ERR_LIB_EVP,
                           ERR_R_UNSUPPORTED,
                           "[action:%d, state:%d] "
...
static int
fix_cipher_md(enum state state,
              const struct translation_st* translation,
              struct translation_ctx_st* ctx,
...
static int
fix_cipher(enum state state,
           const struct translation_st* translation,
           struct translation_ctx_st* ctx)
{
  return fix_cipher_md(
    state, translation, ctx, get_cipher_name, get_cipher_by_name);
}

static int
fix_md(enum state state,
       const struct translation_st* translation,
       struct translation_ctx_st* ctx)
{
...
static int
fix_distid_len(enum state state,
               const struct translation_st* translation,
               struct translation_ctx_st* ctx)
{
...
static int
fix_dh_kdf_type(enum state state,
                const struct translation_st* translation,
                struct translation_ctx_st* ctx)
{
...
static int
fix_ec_kdf_type(enum state state,
                const struct translation_st* translation,
                struct translation_ctx_st* ctx)
{
  static const struct kdf_type_map_st kdf_type_map[] = {
...
static int
fix_dh_nid5114(enum state state,
               const struct translation_st* translation,
               struct translation_ctx_st* ctx)
{
  int ret;
...
static int
fix_ec_param_enc(enum state state,
                 const struct translation_st* translation,
                 struct translation_ctx_st* ctx)
{
  int ret;
...
static int
fix_ecdh_cofactor(enum state state,
                  const struct translation_st* translation,
                  struct translation_ctx_st* ctx)
{
  /*
...
static int
fix_hkdf_mode(enum state state,
              const struct translation_st* translation,
              struct translation_ctx_st* ctx)
{
  static const OSSL_ITEM str_value_map[] = {
...
static int
get_payload_bn(enum state state,
               const struct translation_st* translation,
               struct translation_ctx_st* ctx,
               const BIGNUM* bn)
{
...
static int
get_payload_int(enum state state,
                const struct translation_st* translation,
                struct translation_ctx_st* ctx,
                const int val)
{
...
static int
get_rsa_payload_n(enum state state,
                  const struct translation_st* translation,
                  struct translation_ctx_st* ctx)
{
  const BIGNUM* bn = NULL;
...
static int
get_rsa_payload_e(enum state state,
                  const struct translation_st* translation,
                  struct translation_ctx_st* ctx)
{
  const BIGNUM* bn = NULL;
...
static int
get_rsa_payload_d(enum state state,
                  const struct translation_st* translation,
                  struct translation_ctx_st* ctx)
{
  const BIGNUM* bn = NULL;
...
static int
fix_group_ecx(enum state state,
              const struct translation_st* translation,
              struct translation_ctx_st* ctx)
{
...
lookup_evp_pkey_ctx_translation(struct translation_st* tmpl)
{
  return lookup_translation(
    tmpl, evp_pkey_ctx_translations, OSSL_NELEM(evp_pkey_ctx_translations));
}
...
{
  return lookup_translation(
    tmpl, evp_pkey_translations, OSSL_NELEM(evp_pkey_translations));
}

/* This must ONLY be called for provider side operations */
int
evp_pkey_ctx_ctrl_to_param(EVP_PKEY_CTX* pctx,
                           int keytype,
                           int optype,
                           int cmd,
                           int p1,
...
/* This must ONLY be called for provider side operations */
int
evp_pkey_ctx_ctrl_str_to_param(EVP_PKEY_CTX* pctx,
                               const char* name,
                               const char* value)
{
  struct translation_ctx_st ctx = {
...
evp_pkey_ctx_set_params_to_ctrl(EVP_PKEY_CTX* ctx, const OSSL_PARAM* params)
{
  ...
  return evp_pkey_ctx_setget_params_to_ctrl(
    ctx, OSSL_ACTION_SET, (OSSL_PARAM*)params);
}
...
static int
evp_pkey_setget_params_to_ctrl(const EVP_PKEY* pkey,
                               enum action action_type,
                               OSSL_PARAM* params)
{
  int ret = 1;
