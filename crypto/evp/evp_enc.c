...
nonlegacy:
    ...
    if (enc) {
        if (ctx->cipher->einit == NULL) {
            ...
            if (key == NULL && ctx->cipher->einit_skey != NULL) {
                return ctx->cipher->einit_skey(ctx->algctx,
                                               NULL,
                                               iv,
    ...
    if (ctx->cipher->dinit == NULL) {
        ...
        if (key == NULL && ctx->cipher->dinit_skey != NULL) {
            return ctx->cipher->dinit_skey(ctx->algctx,
                                           NULL,
                                           iv,
                                           iv == NULL ? 0 : EVP_CIPHER_CTX_get_iv_length(ctx),
                                           params);
...
int EVP_CipherInit_SKEY(EVP_CIPHER_CTX      *ctx,
                        const EVP_CIPHER    *cipher,
                        EVP_SKEY            *skey,
                        const unsigned char *iv,
                        size_t               iv_len,
...
int EVP_CipherInit_ex2(EVP_CIPHER_CTX      *ctx,
                       const EVP_CIPHER    *cipher,
                       const unsigned char *key,
                       const unsigned char *iv,
                       int                  enc,
...
static int
evp_EncryptDecryptUpdate(EVP_CIPHER_CTX *ctx, unsigned char *out, int *outl, const unsigned char *in, int inl)
{
    int i, j, bl, cmpl = inl;
