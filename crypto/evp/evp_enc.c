...
static int
evp_cipher_init_internal(EVP_CIPHER_CTX* ctx,
...
nonlegacy:
   ...
   */
  if (params != NULL) {
    OSSL_PARAM param_lens[3] = { OSSL_PARAM_END,
                                 OSSL_PARAM_END,
                                 OSSL_PARAM_END };
  ...
  if (enc) {
    if (ctx->cipher->einit == NULL) {
      ...
      if (key == NULL && ctx->cipher->einit_skey != NULL) {
        return ctx->cipher->einit_skey(
          ...
          NULL,
          iv,
          iv == NULL ? 0 : EVP_CIPHER_CTX_get_iv_length(ctx),
          params);
      }
      ERR_raise(ERR_LIB_EVP, EVP_R_INITIALIZATION_ERROR);
      return 0;
    ...
    return ctx->cipher->einit(
      ctx->algctx,
      key,
      key == NULL ? 0 : EVP_CIPHER_CTX_get_key_length(ctx),
      iv,
      iv == NULL ? 0 : EVP_CIPHER_CTX_get_iv_length(ctx),
      params);
  }
 ...
 */
static int
evp_cipher_init_skey_internal(EVP_CIPHER_CTX* ctx,
                              const EVP_CIPHER* cipher,
                              const EVP_SKEY* skey,
                              const unsigned char* iv,
                              size_t iv_len,
...
int
EVP_CipherInit_SKEY(EVP_CIPHER_CTX* ctx,
                    const EVP_CIPHER* cipher,
                    EVP_SKEY* skey,
                    const unsigned char* iv,
                    size_t iv_len,
                    ...
                    const OSSL_PARAM params[])
{
  return evp_cipher_init_skey_internal(
    ctx, cipher, skey, iv, iv_len, enc, params);
}

int
EVP_CipherInit_ex2(EVP_CIPHER_CTX* ctx,
                   const EVP_CIPHER* cipher,
                   const unsigned char* key,
                   const unsigned char* iv,
                   int enc,
...
int
EVP_CipherInit(EVP_CIPHER_CTX* ctx,
               const EVP_CIPHER* cipher,
               const unsigned char* key,
               const unsigned char* iv,
               int enc)
...
int
EVP_CipherInit_ex(EVP_CIPHER_CTX* ctx,
                  const EVP_CIPHER* cipher,
                  ENGINE* impl,
                  const unsigned char* key,
                  const unsigned char* iv,
...
int
EVP_CipherPipelineEncryptInit(EVP_CIPHER_CTX* ctx,
                              const EVP_CIPHER* cipher,
                              const unsigned char* key,
                              size_t keylen,
                              size_t numpipes,
                              ...
                              size_t ivlen)
{
  ...
  return ctx->cipher->p_einit(
    ctx->algctx, key, keylen, numpipes, iv, ivlen, NULL);
}

int
EVP_CipherPipelineDecryptInit(EVP_CIPHER_CTX* ctx,
                              const EVP_CIPHER* cipher,
                              const unsigned char* key,
                              size_t keylen,
                              size_t numpipes,
                              ...
                              size_t ivlen)
{
  ...
  return ctx->cipher->p_dinit(
    ctx->algctx, key, keylen, numpipes, iv, ivlen, NULL);
}

int
EVP_CipherUpdate(EVP_CIPHER_CTX* ctx,
                 unsigned char* out,
                 int* outl,
...
int
EVP_CipherPipelineUpdate(EVP_CIPHER_CTX* ctx,
                         unsigned char** out,
                         size_t* outl,
                         ...
                         const size_t* inl)
{
  ...
  return ctx->cipher->p_cupdate(
    ctx->algctx, ctx->numpipes, out, outl, outsize, in, inl);
}
...
int
EVP_EncryptInit_ex(EVP_CIPHER_CTX* ctx,
                   const EVP_CIPHER* cipher,
                   ENGINE* impl,
                   const unsigned char* key,
                   const unsigned char* iv)
...
int
EVP_EncryptInit_ex2(EVP_CIPHER_CTX* ctx,
                    const EVP_CIPHER* cipher,
                    const unsigned char* key,
                    const unsigned char* iv,
                    const OSSL_PARAM params[])
...
int
EVP_DecryptInit(EVP_CIPHER_CTX* ctx,
                const EVP_CIPHER* cipher,
                const unsigned char* key,
                const unsigned char* iv)
{
...
int
EVP_DecryptInit_ex(EVP_CIPHER_CTX* ctx,
                   const EVP_CIPHER* cipher,
                   ENGINE* impl,
                   const unsigned char* key,
                   const unsigned char* iv)
...
int
EVP_DecryptInit_ex2(EVP_CIPHER_CTX* ctx,
                    const EVP_CIPHER* cipher,
                    const unsigned char* key,
                    const unsigned char* iv,
                    const OSSL_PARAM params[])
...
int
ossl_is_partially_overlapping(const void* ptr1, const void* ptr2, int len)
{
...
static int
evp_EncryptDecryptUpdate(EVP_CIPHER_CTX* ctx,
                         unsigned char* out,
                         int* outl,
                         const unsigned char* in,
                         int inl)
...
int
EVP_EncryptUpdate(EVP_CIPHER_CTX* ctx,
                  unsigned char* out,
                  int* outl,
                  const unsigned char* in,
                  int inl)
...
EVP_EncryptFinal_ex(EVP_CIPHER_CTX* ctx, unsigned char* out, int* outl)
{
  ...
  ret = ctx->cipher->cfinal(
    ctx->algctx, out, &soutl, blocksize == 1 ? 0 : blocksize);
...
int
EVP_DecryptUpdate(EVP_CIPHER_CTX* ctx,
                  unsigned char* out,
                  int* outl,
                  const unsigned char* in,
                  int inl)
...
int
EVP_CIPHER_get_params(EVP_CIPHER* cipher, OSSL_PARAM params[])
{
...
int
EVP_CIPHER_CTX_set_params(EVP_CIPHER_CTX* ctx, const OSSL_PARAM params[])
{
  int r = 0;
  const OSSL_PARAM* p;
...
int
EVP_CIPHER_CTX_get_params(EVP_CIPHER_CTX* ctx, OSSL_PARAM params[])
{
...
const OSSL_PARAM*
EVP_CIPHER_gettable_params(const EVP_CIPHER* cipher)
{
...
const OSSL_PARAM*
EVP_CIPHER_settable_ctx_params(const EVP_CIPHER* cipher)
{
  void* provctx;
...
const OSSL_PARAM*
EVP_CIPHER_gettable_ctx_params(const EVP_CIPHER* cipher)
{
  void* provctx;
...
const OSSL_PARAM*
EVP_CIPHER_CTX_settable_params(EVP_CIPHER_CTX* cctx)
{
  void* alg;
...
const OSSL_PARAM*
EVP_CIPHER_CTX_gettable_params(EVP_CIPHER_CTX* cctx)
{
  void* provctx;
...
#ifndef FIPS_MODULE
static OSSL_LIB_CTX*
EVP_CIPHER_CTX_get_libctx(EVP_CIPHER_CTX* ctx)
{
...
static void
set_legacy_nid(const char* name, void* vlegacy_nid)
{
  int nid;
...
static void*
evp_cipher_from_algorithm(const int name_id,
...
static int
evp_cipher_up_ref(void* cipher)
{
  return EVP_CIPHER_up_ref(cipher);
}

static void
evp_cipher_free(void* cipher)
{
  EVP_CIPHER_free(cipher);
}

EVP_CIPHER*
EVP_CIPHER_fetch(OSSL_LIB_CTX* ctx,
                 const char* algorithm,
                 const char* properties)
{
  EVP_CIPHER* cipher = evp_generic_fetch(ctx,
                                         OSSL_OP_CIPHER,
                                         algorithm,
                                         properties,
...
EVP_CIPHER*
evp_cipher_fetch_from_prov(OSSL_PROVIDER* prov,
                           const char* algorithm,
                           const char* properties)
{
...
int
EVP_CIPHER_can_pipeline(const EVP_CIPHER* cipher, int enc)
{
...
void
evp_cipher_free_int(EVP_CIPHER* cipher)
{
...
void
EVP_CIPHER_do_all_provided(OSSL_LIB_CTX* libctx,
