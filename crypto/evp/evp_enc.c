...
int EVP_CIPHER_CTX_reset(EVP_CIPHER_CTX* ctx) {
  ...
  if (ctx->cipher == NULL || ctx->cipher->prov == NULL) goto legacy;

  if (ctx->algctx != NULL) {
    if (ctx->cipher->freectx != NULL) ctx->cipher->freectx(ctx->algctx);
    ctx->algctx = NULL;
  }
  if (ctx->fetched_cipher != NULL) EVP_CIPHER_free(ctx->fetched_cipher);
...
legacy:
  ...
  if (ctx->cipher != NULL) {
    if (ctx->cipher->cleanup && !ctx->cipher->cleanup(ctx)) return 0;
...
static int evp_cipher_init_internal(EVP_CIPHER_CTX* ctx,
  ...
  if (ctx->engine != NULL
    ...
    if (ctx->cipher == ctx->fetched_cipher) ctx->cipher = NULL;
   ...
   */
  if (cipher != NULL && ctx->cipher != NULL) {
    if (ctx->cipher->cleanup != NULL && !ctx->cipher->cleanup(ctx)) return 0;
...
nonlegacy:
  ...
  if (cipher == NULL) cipher = ctx->cipher;
  ...
  if ((ctx->flags & EVP_CIPH_NO_PADDING) != 0) {
     ...
     */
    if (!EVP_CIPHER_CTX_set_padding(ctx, 0)) return 0;
  }
  ...
  if ((EVP_CIPHER_get_flags(EVP_CIPHER_CTX_get0_cipher(ctx)) &
    ...
    switch (EVP_CIPHER_CTX_get_mode(ctx)) {
      ...
      case EVP_CIPH_CBC_MODE:
        ...
        if (iv != NULL) memcpy(ctx->oiv, iv, n);
...
int EVP_CipherInit(EVP_CIPHER_CTX* ctx, const EVP_CIPHER* cipher,
  ...
  if (cipher != NULL) EVP_CIPHER_CTX_reset(ctx);
...
int EVP_EncryptUpdate(EVP_CIPHER_CTX* ctx, unsigned char* out, int* outl,
  ...
  if (ossl_unlikely(ctx->cipher->prov == NULL)) goto legacy;
...
int EVP_EncryptFinal_ex(EVP_CIPHER_CTX* ctx, unsigned char* out, int* outl) {
  ...
  if (ctx->cipher->prov == NULL) goto legacy;
...
legacy:
  ...
  if (!evp_EncryptDecryptUpdate(ctx, out, outl, in, inl)) return 0;
...
int EVP_CIPHER_CTX_set_key_length(EVP_CIPHER_CTX* c, int keylen) {
  if (c->cipher->prov != NULL) {
    ...
    if (EVP_CIPHER_CTX_get_key_length(c) == keylen) return 1;
    ...
    if (!OSSL_PARAM_set_int(params, keylen)) return 0;
...
int EVP_CIPHER_CTX_set_padding(EVP_CIPHER_CTX* ctx, int pad) {
  ...
  if (ctx->cipher != NULL && ctx->cipher->prov == NULL) return 1;
...
int EVP_CIPHER_CTX_ctrl(EVP_CIPHER_CTX* ctx, int type, int arg, void* ptr) {
  ...
  switch (type) {
    case EVP_CTRL_SET_KEY_LENGTH:
      if (arg < 0) return 0;
      if (ctx->key_len == arg) /* Skip calling into provider if unchanged. */
        return 1;
    ...
    case EVP_CTRL_AEAD_SET_IVLEN:
      if (arg < 0) return 0;
      if (ctx->iv_len == arg) /* Skip calling into provider if unchanged. */
        return 1;
    ...
    case EVP_CTRL_GCM_IV_GEN:
      set_params = 0;
      if (arg < 0) sz = 0; /* special case that uses the iv length */
      params[0] = OSSL_PARAM_construct_octet_string(
    ...
    case EVP_CTRL_TLS1_1_MULTIBLOCK_MAX_BUFSIZE:
      ...
      ret = evp_do_ciph_ctx_getparams(ctx->cipher, ctx->algctx, params);
      if (ret <= 0 || sz > INT_MAX) return 0;
      return (int)sz;
    case EVP_CTRL_TLS1_1_MULTIBLOCK_AAD: {
      EVP_CTRL_TLS1_1_MULTIBLOCK_PARAM* p =
          (EVP_CTRL_TLS1_1_MULTIBLOCK_PARAM*)ptr;

      if (arg < (int)sizeof(EVP_CTRL_TLS1_1_MULTIBLOCK_PARAM)) return 0;
...
int EVP_CIPHER_CTX_rand_key(EVP_CIPHER_CTX* ctx, unsigned char* key) {
...
#else
  {
    ...
    if (kl <= 0 || RAND_priv_bytes_ex(libctx, key, kl, 0) <= 0) return 0;
    return 1;
  }
#endif /* FIPS_MODULE */
}
...
int EVP_CIPHER_CTX_copy(EVP_CIPHER_CTX* out, const EVP_CIPHER_CTX* in) {
  ...
  if (in->cipher->prov == NULL) goto legacy;
...
static void* evp_cipher_from_algorithm(const int name_id,
  ...
  if (prov != NULL && !ossl_provider_up_ref(prov)) goto err;
...
int EVP_CIPHER_up_ref(EVP_CIPHER* cipher) {
  ...
  if (cipher->origin == EVP_ORIG_DYNAMIC) CRYPTO_UP_REF(&cipher->refcnt, &ref);
  return 1;
}
...
void EVP_CIPHER_free(EVP_CIPHER* cipher) {
  ...
  if (cipher == NULL || cipher->origin != EVP_ORIG_DYNAMIC) return;
