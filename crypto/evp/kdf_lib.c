...
EVP_KDF_CTX* EVP_KDF_CTX_new(EVP_KDF* kdf) {
  ...
  if (ctx == NULL ||
    ...
    if (ctx != NULL) kdf->freectx(ctx->algctx);
    OPENSSL_free(ctx);
    ctx = NULL;
  } else {
    ctx->meth = kdf;
...
void EVP_KDF_CTX_reset(EVP_KDF_CTX* ctx) {
  ...
  if (ctx->meth->reset != NULL) ctx->meth->reset(ctx->algctx);
}
...
size_t EVP_KDF_CTX_get_kdf_size(EVP_KDF_CTX* ctx) {
  ...
  *params = OSSL_PARAM_construct_size_t(OSSL_KDF_PARAM_SIZE, &s);
  if (ctx->meth->get_ctx_params != NULL &&
    ...
    return s;
  if (ctx->meth->get_params != NULL && ctx->meth->get_params(params)) return s;
  return 0;
}
...
int EVP_KDF_get_params(EVP_KDF* kdf, OSSL_PARAM params[]) {
  if (kdf->get_params != NULL) return kdf->get_params(params);
  return 1;
}
