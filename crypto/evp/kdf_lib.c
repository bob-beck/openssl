...
EVP_KDF_CTX*
EVP_KDF_CTX_dup(const EVP_KDF_CTX* src)
{
  EVP_KDF_CTX* dst;
...
const char*
EVP_KDF_get0_name(const EVP_KDF* kdf)
{
  return kdf->type_name;
}

const char*
EVP_KDF_get0_description(const EVP_KDF* kdf)
{
  return kdf->description;
}

int
EVP_KDF_is_a(const EVP_KDF* kdf, const char* name)
{
  return kdf != NULL && evp_is_a(kdf->prov, kdf->name_id, NULL, name);
}

const OSSL_PROVIDER*
EVP_KDF_get0_provider(const EVP_KDF* kdf)
{
  return kdf->prov;
}
...
size_t
EVP_KDF_CTX_get_kdf_size(EVP_KDF_CTX* ctx)
{
...
int
EVP_KDF_derive(EVP_KDF_CTX* ctx,
               unsigned char* key,
               size_t keylen,
 ...
 */
int
EVP_KDF_get_params(EVP_KDF* kdf, OSSL_PARAM params[])
{
...
int
EVP_KDF_CTX_get_params(EVP_KDF_CTX* ctx, OSSL_PARAM params[])
{
  if (ctx->meth->get_ctx_params != NULL)
...
int
EVP_KDF_CTX_set_params(EVP_KDF_CTX* ctx, const OSSL_PARAM params[])
{
...
int
EVP_KDF_names_do_all(const EVP_KDF* kdf,
