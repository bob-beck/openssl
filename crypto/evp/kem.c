...
static void
evp_kem_free(void* data)
{
  EVP_KEM_free(data);
}

static int
evp_kem_up_ref(void* data)
{
  return EVP_KEM_up_ref(data);
}

static int
evp_kem_init(EVP_PKEY_CTX* ctx,
             int operation,
  ...
  for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
    ...
    switch (iter) {
      ...
      case 2:
        ...
        kem = evp_kem_fetch_from_prov(
          (OSSL_PROVIDER*)tmp_prov, supported_kem, ctx->propquery);
...
int
EVP_PKEY_auth_encapsulate_init(EVP_PKEY_CTX* ctx,
                               EVP_PKEY* authpriv,
                               const OSSL_PARAM params[])
{
  if (authpriv == NULL)
...
int
EVP_PKEY_encapsulate_init(EVP_PKEY_CTX* ctx, const OSSL_PARAM params[])
{
                     ...
                     size_t* secretlen)
{
  ...
  return ctx->op.encap.kem->encapsulate(
    ctx->op.encap.algctx, out, outlen, secret, secretlen);
}

int
EVP_PKEY_decapsulate_init(EVP_PKEY_CTX* ctx, const OSSL_PARAM params[])
{
  return evp_kem_init(ctx, EVP_PKEY_OP_DECAPSULATE, params, NULL);
}

int
EVP_PKEY_auth_decapsulate_init(EVP_PKEY_CTX* ctx,
                               EVP_PKEY* authpub,
                               const OSSL_PARAM params[])
{
  if (authpub == NULL)
                     ...
                     size_t inlen)
{
  ...
  return ctx->op.encap.kem->decapsulate(
    ctx->op.encap.algctx, secret, secretlen, in, inlen);
}

static EVP_KEM*
evp_kem_new(OSSL_PROVIDER* prov)
{
...
static void*
evp_kem_from_algorithm(int name_id,
...
OSSL_PROVIDER*
EVP_KEM_get0_provider(const EVP_KEM* kem)
{
  return kem->prov;
}

EVP_KEM*
EVP_KEM_fetch(OSSL_LIB_CTX* ctx, const char* algorithm, const char* properties)
{
...
int
EVP_KEM_is_a(const EVP_KEM* kem, const char* name)
{
  return kem != NULL && evp_is_a(kem->prov, kem->name_id, NULL, name);
}

int
evp_kem_get_number(const EVP_KEM* kem)
{
  return kem->name_id;
}

const char*
EVP_KEM_get0_name(const EVP_KEM* kem)
{
  return kem->type_name;
}

const char*
EVP_KEM_get0_description(const EVP_KEM* kem)
{
  return kem->description;
}

void
EVP_KEM_do_all_provided(OSSL_LIB_CTX* libctx,
...
int
EVP_KEM_names_do_all(const EVP_KEM* kem,
...
const OSSL_PARAM*
EVP_KEM_gettable_ctx_params(const EVP_KEM* kem)
{
  void* provctx;
...
const OSSL_PARAM*
EVP_KEM_settable_ctx_params(const EVP_KEM* kem)
{
  void* provctx;
