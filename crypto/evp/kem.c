  ...
  for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++) {
    ...
    switch (iter) {
      case 1:
        ...
        if (kem != NULL) tmp_prov = EVP_KEM_get0_provider(kem);
        break;
      case 2:
...
int EVP_PKEY_encapsulate(EVP_PKEY_CTX* ctx, unsigned char* out, size_t* outlen,
  ...
  if (out != NULL && secret == NULL) return 0;
...
const OSSL_PARAM* EVP_KEM_gettable_ctx_params(const EVP_KEM* kem) {
  ...
  if (kem == NULL || kem->gettable_ctx_params == NULL) return NULL;
...
const OSSL_PARAM* EVP_KEM_settable_ctx_params(const EVP_KEM* kem) {
  ...
  if (kem == NULL || kem->settable_ctx_params == NULL) return NULL;
