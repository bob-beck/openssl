...
static int dsa_paramgen_check(EVP_PKEY_CTX* ctx) {
  ...
  if (ctx->pmeth != NULL && ctx->pmeth->pkey_id != EVP_PKEY_DSA) return -1;
  return 1;
}
...
int EVP_PKEY_CTX_set_dsa_paramgen_type(EVP_PKEY_CTX* ctx, const char* name) {
  ...
  if ((ret = dsa_paramgen_check(ctx)) <= 0) return ret;

  *p++ = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_FFC_TYPE, (char*)name,
                                          0);
