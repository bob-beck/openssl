...
static void*
evp_signature_from_algorithm(int name_id,
...
EVP_SIGNATURE*
EVP_SIGNATURE_fetch(OSSL_LIB_CTX* ctx,
...
EVP_SIGNATURE*
evp_signature_fetch_from_prov(OSSL_PROVIDER* prov,
                              const char* algorithm,
                              const char* properties)
{
...
EVP_SIGNATURE_do_all_provided(OSSL_LIB_CTX* libctx,
                              void (*fn)(EVP_SIGNATURE* signature, void* arg),
                              void* arg)
{
...
static int
evp_pkey_signature_init(EVP_PKEY_CTX* ctx,
...
int
EVP_PKEY_sign_init_ex2(EVP_PKEY_CTX* ctx,
                       EVP_SIGNATURE* algo,
                       const OSSL_PARAM params[])
{
...
int
EVP_PKEY_sign_message_init(EVP_PKEY_CTX* ctx,
                           EVP_SIGNATURE* algo,
                           const OSSL_PARAM params[])
{
...
int
EVP_PKEY_sign_message_update(EVP_PKEY_CTX* ctx,
...
int
EVP_PKEY_sign_message_final(EVP_PKEY_CTX* ctx,
                            unsigned char* sig,
                            size_t* siglen)
{
  EVP_SIGNATURE* signature;
  ...
  ret = signature->sign_message_final(
              ...
              size_t tbslen)
{
  ...
  ret = signature->sign(
    ctx->op.sig.algctx, sig, siglen, (sig == NULL) ? 0 : *siglen, tbs, tbslen);
  if (ret <= 0)
    ERR_raise_data(ERR_LIB_EVP,
                   EVP_R_PROVIDER_SIGNATURE_FAILURE,
                   "%s sign:%s",
...
int
EVP_PKEY_verify_init_ex2(EVP_PKEY_CTX* ctx,
                         EVP_SIGNATURE* algo,
                         const OSSL_PARAM params[])
{
...
int
EVP_PKEY_verify_message_init(EVP_PKEY_CTX* ctx,
                             EVP_SIGNATURE* algo,
                             const OSSL_PARAM params[])
{
...
int
EVP_PKEY_CTX_set_signature(EVP_PKEY_CTX* ctx,
...
int
EVP_PKEY_verify_message_update(EVP_PKEY_CTX* ctx,
                               const unsigned char* in,
                               size_t inlen)
{
  EVP_SIGNATURE* signature;
...
int
EVP_PKEY_verify(EVP_PKEY_CTX* ctx,
                const unsigned char* sig,
                size_t siglen,
                const unsigned char* tbs,
                size_t tbslen)
...
int
EVP_PKEY_verify_recover_init_ex2(EVP_PKEY_CTX* ctx,
                                 EVP_SIGNATURE* algo,
                                 const OSSL_PARAM params[])
{
...
int
EVP_PKEY_verify_recover(EVP_PKEY_CTX* ctx,
                        unsigned char* rout,
                        size_t* routlen,
                        const unsigned char* sig,
                        size_t siglen)
{
  ...
  ret = signature->verify_recover(ctx->op.sig.algctx,
                                  rout,
                                  routlen,
                                  (rout == NULL ? 0 : *routlen),
                                  sig,
