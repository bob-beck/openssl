...
static void* evp_signature_from_algorithm(int name_id,
  ...
  if (valid &&
    ...
    ERR_raise_data(ERR_LIB_EVP, EVP_R_INVALID_PROVIDER_FUNCTIONS,
                   "missing %s verification function:%s", signature->type_name,
                   desc);
    valid = 0;
  }
  if (valid &&
  ...
  if (valid &&
    ...
    ERR_raise_data(ERR_LIB_EVP, EVP_R_INVALID_PROVIDER_FUNCTIONS,
                   "missing %s digest_sign function:%s", signature->type_name,
                   desc);
    valid = 0;
  }
  ...
  if (valid && ((signature->digest_verify_init != NULL &&
     ...
     */
    ERR_raise_data(ERR_LIB_EVP, EVP_R_INVALID_PROVIDER_FUNCTIONS,
                   "missing %s digest_verify function:%s", signature->type_name,
                   desc);
    valid = 0;
  }
...
void EVP_SIGNATURE_do_all_provided(OSSL_LIB_CTX* libctx,
                                   void (*fn)(EVP_SIGNATURE* signature,
                                              void* arg),
                                   void* arg) {
  evp_generic_do_all(libctx, OSSL_OP_SIGNATURE, (void (*)(void*, void*))fn, arg,
                     evp_signature_from_algorithm, evp_signature_up_ref,
                     evp_signature_free);
}
...
const OSSL_PARAM* EVP_SIGNATURE_gettable_ctx_params(const EVP_SIGNATURE* sig) {
  ...
  if (sig == NULL || sig->gettable_ctx_params == NULL) return NULL;
...
const OSSL_PARAM* EVP_SIGNATURE_settable_ctx_params(const EVP_SIGNATURE* sig) {
  ...
  if (sig == NULL || sig->settable_ctx_params == NULL) return NULL;
...
static int evp_pkey_signature_init(EVP_PKEY_CTX* ctx, EVP_SIGNATURE* signature,
  ...
  if (signature != NULL) {
    ...
    if (tmp_keymgmt == NULL) EVP_KEYMGMT_free(tmp_keymgmt_tofree);

    if (provkey == NULL) goto end;
    ...
    if (signature->query_key_types != NULL) {
      ...
      for (; *keytypes != NULL; keytypes++)
        if (EVP_PKEY_CTX_is_a(ctx, *keytypes)) break;
      if (*keytypes == NULL) {
    ...
    if (!EVP_SIGNATURE_up_ref(signature)) return 0;
  } else {
    /* Without a pre-fetched signature, it must be figured out somehow */
    ERR_set_mark();

    if (evp_pkey_ctx_is_legacy(ctx)) goto legacy;
...
legacy:
  ...
  switch (operation) {
    case EVP_PKEY_OP_SIGN:
      if (ctx->pmeth->sign_init == NULL) return 1;
      ret = ctx->pmeth->sign_init(ctx);
      break;
    case EVP_PKEY_OP_VERIFY:
      if (ctx->pmeth->verify_init == NULL) return 1;
      ret = ctx->pmeth->verify_init(ctx);
      break;
    case EVP_PKEY_OP_VERIFYRECOVER:
      if (ctx->pmeth->verify_recover_init == NULL) return 1;
...
#ifndef FIPS_MODULE
  if (ret > 0) ret = evp_pkey_ctx_use_cached_data(ctx);
#endif
...
int EVP_PKEY_sign(EVP_PKEY_CTX* ctx, unsigned char* sig, size_t* siglen,
  ...
  if (ctx->op.sig.algctx == NULL) goto legacy;
  ...
  if (ret <= 0)
    ERR_raise_data(ERR_LIB_EVP, EVP_R_PROVIDER_SIGNATURE_FAILURE, "%s sign:%s",
                   signature->type_name, desc);
  return ret;
legacy:
