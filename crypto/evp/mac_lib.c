...
static size_t
get_size_t_ctx_param (EVP_MAC_CTX *ctx, const char *name)
{
  size_t sz = 0;
...
int
EVP_MAC_init_SKEY (EVP_MAC_CTX *ctx, EVP_SKEY *skey, const OSSL_PARAM params[])
{
...
static int
evp_mac_final (EVP_MAC_CTX *ctx, int xof, unsigned char *out, size_t *outl,
               size_t outsize)
{
  size_t l;
  int res;
...
int
EVP_MAC_finalXOF (EVP_MAC_CTX *ctx, unsigned char *out, size_t outsize)
{
 ...
 */
int
EVP_MAC_get_params (EVP_MAC *mac, OSSL_PARAM params[])
{
...
int
EVP_MAC_CTX_get_params (EVP_MAC_CTX *ctx, OSSL_PARAM params[])
{
  if (ctx->meth->get_ctx_params != NULL)
...
int
EVP_MAC_CTX_set_params (EVP_MAC_CTX *ctx, const OSSL_PARAM params[])
{
...
const char *
EVP_MAC_get0_name (const EVP_MAC *mac)
{
  return mac->type_name;
}

const char *
EVP_MAC_get0_description (const EVP_MAC *mac)
{
  return mac->description;
}

int
EVP_MAC_is_a (const EVP_MAC *mac, const char *name)
{
  return mac != NULL && evp_is_a (mac->prov, mac->name_id, NULL, name);
}

int
EVP_MAC_names_do_all (const EVP_MAC *mac,
...
EVP_Q_mac (OSSL_LIB_CTX *libctx, const char *name, const char *propq,
           const char *subalg, const OSSL_PARAM *params, const void *key,
