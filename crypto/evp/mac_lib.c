...
EVP_MAC_CTX* EVP_MAC_CTX_dup(const EVP_MAC_CTX* src) {
  ...
  if (src->algctx == NULL) return NULL;
...
static size_t get_size_t_ctx_param(EVP_MAC_CTX* ctx, const char* name) {
  ...
  if (ctx->algctx != NULL) {
    ...
    if (ctx->meth->get_ctx_params != NULL) {
      if (ctx->meth->get_ctx_params(ctx->algctx, params)) return sz;
    } else if (ctx->meth->get_params != NULL) {
      if (ctx->meth->get_params(params)) return sz;
    }
  }
  /*
...
int EVP_MAC_get_params(EVP_MAC* mac, OSSL_PARAM params[]) {
  if (mac->get_params != NULL) return mac->get_params(params);
  return 1;
}
...
unsigned char* EVP_Q_mac(OSSL_LIB_CTX* libctx, const char* name,
  ...
  if ((ctx = EVP_MAC_CTX_new(mac)) != NULL &&
    ...
    res = out;
    if (res != NULL && outlen != NULL) *outlen = len;
  }
