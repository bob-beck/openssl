...
int EVP_SealInit(EVP_CIPHER_CTX* ctx, const EVP_CIPHER* type,
  ...
  if ((npubk <= 0) || !pubk) return 1;

  if (EVP_CIPHER_CTX_rand_key(ctx, key) <= 0) return 0;

  len = EVP_CIPHER_CTX_get_iv_length(ctx);
  if (len < 0 || RAND_priv_bytes_ex(libctx, iv, len, 0) <= 0) goto err;
...
int EVP_SealFinal(EVP_CIPHER_CTX* ctx, unsigned char* out, int* outl) {
  ...
  if (i) i = EVP_EncryptInit_ex(ctx, NULL, NULL, NULL, NULL);
  return i;
}
