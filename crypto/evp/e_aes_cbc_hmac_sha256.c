...
static int
aesni_cbc_hmac_sha256_init_key(EVP_CIPHER_CTX* ctx,
...
static size_t
tls1_1_multi_block_encrypt(EVP_AES_HMAC_SHA256* key,
                           unsigned char* out,
                           const unsigned char* inp,
                           size_t inp_len,
                           int n4x)
...
static int
aesni_cbc_hmac_sha256_cipher(EVP_CIPHER_CTX* ctx,
                             unsigned char* out,
  ...
  if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
    ...
    if (((OPENSSL_ia32cap_P[2] & (1 << 29)) ||         /* SHAEXT? */
      ...
      (void)aesni_cbc_sha256_enc(
        in, out, blocks, &key->ks, ctx->iv, &key->md, in + iv + sha_off);
    ...
    if (plen != len) { /* "TLS" mode of operation */
      ...
      aesni_cbc_encrypt(
        out + aes_off, out + aes_off, len - aes_off, &key->ks, ctx->iv, 1);
    } else {
      aesni_cbc_encrypt(
        in + aes_off, out + aes_off, len - aes_off, &key->ks, ctx->iv, 1);
    }
  } else {
    union
    {
...
static int
aesni_cbc_hmac_sha256_ctrl(EVP_CIPHER_CTX* ctx, int type, int arg, void* ptr)
{
  EVP_AES_HMAC_SHA256* key = data(ctx);
