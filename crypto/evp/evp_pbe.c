...
int
EVP_PBE_CipherInit_ex (ASN1_OBJECT *pbe_obj, const char *pass, int passlen,
...
int
EVP_PBE_find_ex (int type, int pbe_nid, int *pcnid, int *pmnid,
                 EVP_PBE_KEYGEN **pkeygen, EVP_PBE_KEYGEN_EX **pkeygen_ex)
{
  ...
  if (pbetmp == NULL)
    {
      pbetmp
          = OBJ_bsearch_pbe2 (&pbelu, builtin_pbe, OSSL_NELEM (builtin_pbe));
    }
  if (pbetmp == NULL)
    return 0;
  if (pcnid != NULL)
...
int
EVP_PBE_find (int type, int pbe_nid, int *pcnid, int *pmnid,
