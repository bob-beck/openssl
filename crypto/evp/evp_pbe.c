...
int
EVP_PBE_CipherInit_ex(ASN1_OBJECT* pbe_obj,
                      const char* pass,
                      int passlen,
                      ASN1_TYPE* param,
                      EVP_CIPHER_CTX* ctx,
                      ...
                      const char* propq)
{
  ...
  if (!EVP_PBE_find_ex(EVP_PBE_TYPE_OUTER,
                       OBJ_obj2nid(pbe_obj),
                       &cipher_nid,
                       &md_nid,
                       &keygen,
    ...
    ERR_raise_data(
      ERR_LIB_EVP, EVP_R_UNKNOWN_PBE_ALGORITHM, "TYPE=%s", obj_tmp);
    goto err;
  }
...
int
EVP_PBE_CipherInit(ASN1_OBJECT* pbe_obj,
                   const char* pass,
                   int passlen,
                   ASN1_TYPE* param,
                   EVP_CIPHER_CTX* ctx,
                   int en_de)
{
  return EVP_PBE_CipherInit_ex(
    pbe_obj, pass, passlen, param, ctx, en_de, NULL, NULL);
}
...
int
EVP_PBE_alg_add_type(int pbe_type,
                     int pbe_nid,
                     int cipher_nid,
                     int md_nid,
                     EVP_PBE_KEYGEN* keygen)
                ...
                EVP_PBE_KEYGEN* keygen)
{
  ...
  return EVP_PBE_alg_add_type(
    EVP_PBE_TYPE_OUTER, nid, cipher_nid, md_nid, keygen);
}

int
EVP_PBE_find_ex(int type,
                int pbe_nid,
                int* pcnid,
                int* pmnid,
...
int
EVP_PBE_get(int* ptype, int* ppbe_nid, size_t num)
{
