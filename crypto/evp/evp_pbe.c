...
int EVP_PBE_CipherInit_ex(ASN1_OBJECT* pbe_obj, const char* pass, int passlen,
  ...
  if (cipher_nid != -1) {
    ...
    if (cipher == NULL) cipher = EVP_get_cipherbynid(cipher_nid);
    if (cipher == NULL) {
  ...
  if (md_nid != -1) {
    ...
    /* Fallback to legacy method */
    if (md == NULL) md = EVP_get_digestbynid(md_nid);
...
int EVP_PBE_alg_add_type(int pbe_type, int pbe_nid, int cipher_nid, int md_nid,
  ...
  if ((pbe_tmp = OPENSSL_zalloc(sizeof(*pbe_tmp))) == NULL) goto err;
...
int EVP_PBE_find_ex(int type, int pbe_nid, int* pcnid, int* pmnid,
  ...
  if (pbetmp == NULL) return 0;
  if (pcnid != NULL) *pcnid = pbetmp->cipher_nid;
  if (pmnid != NULL) *pmnid = pbetmp->md_nid;
  if (pkeygen != NULL) *pkeygen = pbetmp->keygen;
  if (pkeygen_ex != NULL) *pkeygen_ex = pbetmp->keygen_ex;
  return 1;
}
...
int EVP_PBE_get(int* ptype, int* ppbe_nid, size_t num) {
  ...
  if (num >= OSSL_NELEM(builtin_pbe)) return 0;

  tpbe = builtin_pbe + num;
  if (ptype) *ptype = tpbe->pbe_type;
  if (ppbe_nid) *ppbe_nid = tpbe->pbe_nid;
  return 1;
}
