...
static int aria_gcm_ctrl(EVP_CIPHER_CTX* c, int type, int arg, void* ptr) {
  ...
  switch (type) {
    ...
    case EVP_CTRL_AEAD_SET_TAG:
      if (arg <= 0 || arg > 16 || EVP_CIPHER_CTX_is_encrypting(c)) return 0;
...
static int aria_gcm_tls_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
    /* Encrypt payload */
    if (CRYPTO_gcm128_encrypt(&gctx->gcm, in, out, len)) goto err;
    out += len;
    /* Finally write tag */
    ...
    /* Decrypt */
    if (CRYPTO_gcm128_decrypt(&gctx->gcm, in, out, len)) goto err;
    /* Retrieve tag */
...
static int aria_gcm_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (gctx->tls_aad_len >= 0) return aria_gcm_tls_cipher(ctx, out, in, len);
  ...
  if (in) {
    if (out == NULL) {
      ...
      if (CRYPTO_gcm128_encrypt(&gctx->gcm, in, out, len)) return -1;
    } else {
      if (CRYPTO_gcm128_decrypt(&gctx->gcm, in, out, len)) return -1;
    }
    return (int)len;
  }
  if (!EVP_CIPHER_CTX_is_encrypting(ctx)) {
...
static int aria_gcm_cleanup(EVP_CIPHER_CTX* ctx) {
  ...
  if (gctx->iv != ctx->iv) OPENSSL_free(gctx->iv);
...
static int aria_ccm_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (cctx->tls_aad_len >= 0) return aria_ccm_tls_cipher(ctx, out, in, len);
