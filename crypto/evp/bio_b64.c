...
static int
b64_write(BIO* h, const char* buf, int num);
static int
b64_read(BIO* h, char* buf, int size);
static int
b64_puts(BIO* h, const char* str);
static long
b64_ctrl(BIO* h, int cmd, long arg1, void* arg2);
static int
b64_new(BIO* h);
static int
b64_free(BIO* data);
static long
b64_callback_ctrl(BIO* h, int cmd, BIO_info_cb* fp);
...
b64_write(BIO* b, const char* in, int inl)
{
  ...
  while (inl > 0) {
    ...
    if ((BIO_get_flags(b) & BIO_FLAGS_BASE64_NO_NL) != 0) {
      ...
      if (!EVP_EncodeUpdate(
            ctx->base64, ctx->buf, &ctx->buf_len, (unsigned char*)in, n))
        return ret == 0 ? -1 : ret;
      if (!ossl_assert(ctx->buf_len <= (int)sizeof(ctx->buf))) {
        ERR_raise(ERR_LIB_BIO, ERR_R_INTERNAL_ERROR);
        return ret == 0 ? -1 : ret;
