...
static int md_read(BIO* b, char* out, int outl) {
  ...
  if ((ctx == NULL) || (next == NULL)) return 0;
...
static int md_write(BIO* b, const char* in, int inl) {
  ...
  ctx = BIO_get_data(b);
  next = BIO_next(b);
  if ((ctx != NULL) && (next != NULL)) ret = BIO_write(next, in, inl);
...
static long md_ctrl(BIO* b, int cmd, long num, void* ptr) {
  ...
  switch (cmd) {
    ...
    case BIO_C_SET_MD:
      ...
      if (ret > 0) BIO_set_init(b, 1);
      break;
    case BIO_CTRL_DUP:
      dbio = ptr;
      dctx = BIO_get_data(dbio);
      if (!EVP_MD_CTX_copy_ex(dctx, ctx)) return 0;
      BIO_set_init(b, 1);
      break;
    default:
      ret = BIO_ctrl(next, cmd, num, ptr);
...
static int md_gets(BIO* bp, char* buf, int size) {
  ...
  if (size < EVP_MD_CTX_get_size(ctx)) return 0;

  if (EVP_DigestFinal_ex(ctx, (unsigned char*)buf, &ret) <= 0) return -1;
