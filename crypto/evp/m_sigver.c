...
static const char* canon_mdname(const char* mdname) {
  if (mdname != NULL && strcmp(mdname, "UNDEF") == 0) return NULL;
...
static int do_sigver_init(EVP_MD_CTX* ctx, EVP_PKEY_CTX** pctx,
  ...
  if (!evp_md_ctx_free_algctx(ctx)) return 0;
  ...
  if (evp_pkey_ctx_is_legacy(locpctx)) goto legacy;
  ...
  if (props == NULL) props = locpctx->propquery;
...
reinitialize:
  if (pctx != NULL) *pctx = locpctx;

  if (type != NULL) {
    ctx->reqdigest = type;
    if (mdname == NULL) mdname = canon_mdname(EVP_MD_get0_name(type));
  } else {
...
legacy:
  ...
  if (ver) {
    if (ctx->pctx->pmeth->verifyctx_init) {
      if (ctx->pctx->pmeth->verifyctx_init(ctx->pctx, ctx) <= 0) return 0;
    ...
    if (ctx->pctx->pmeth->signctx_init) {
      if (ctx->pctx->pmeth->signctx_init(ctx->pctx, ctx) <= 0) return 0;
  ...
  if (EVP_PKEY_CTX_set_signature_md(ctx->pctx, type) <= 0) return 0;
  if (pctx) *pctx = ctx->pctx;
  if (ctx->pctx->pmeth->flags & EVP_PKEY_FLAG_SIGCTX_CUSTOM) return 1;
  if (!EVP_DigestInit_ex(ctx, type, e)) return 0;
  /*
  ...
  if (ret > 0) ret = evp_pkey_ctx_use_cached_data(locpctx);
...
legacy:
  ...
  if (pctx->pmeth->flags & EVP_PKEY_FLAG_SIGCTX_CUSTOM) {
    if (sigret == NULL) return pctx->pmeth->signctx(pctx, sigret, siglen, ctx);
  ...
  if (sigret != NULL) {
    ...
    if (sctx || !r) return r;
    if (EVP_PKEY_sign(pctx, sigret, siglen, md, mdlen) <= 0) return 0;
  } else {
    if (sctx) {
      if (pctx->pmeth->signctx(pctx, sigret, siglen, ctx) <= 0) return 0;
    } else {
      int s = EVP_MD_get_size(ctx->digest);

      if (s <= 0 || EVP_PKEY_sign(pctx, sigret, siglen, NULL, s) <= 0) return 0;
    }
  }
  return 1;
}
...
int EVP_DigestSign(EVP_MD_CTX* ctx, unsigned char* sigret, size_t* siglen,
  ...
  if (pctx->operation == EVP_PKEY_OP_SIGNCTX && pctx->op.sig.algctx != NULL &&
    ...
    if (signature->digest_sign != NULL) {
      ...
      if (sigret != NULL) ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
      ERR_set_mark();
  ...
  if (sigret != NULL && EVP_DigestSignUpdate(ctx, tbs, tbslen) <= 0) return 0;
...
legacy:
  ...
  if (ctx->flags & EVP_MD_CTX_FLAG_FINALISE) {
    ...
    EVP_MD_CTX* tmp_ctx = EVP_MD_CTX_new();
    if (tmp_ctx == NULL) return -1;
    if (!EVP_MD_CTX_copy_ex(tmp_ctx, ctx)) {
      EVP_MD_CTX_free(tmp_ctx);
      return -1;
    }
    if (vctx)
      r = tmp_ctx->pctx->pmeth->verifyctx(tmp_ctx->pctx, sig, (int)siglen,
                                          tmp_ctx);
    else
      r = EVP_DigestFinal_ex(tmp_ctx, md, &mdlen);
    EVP_MD_CTX_free(tmp_ctx);
...
int EVP_DigestVerify(EVP_MD_CTX* ctx, const unsigned char* sigret,
  ...
  if (pctx->operation == EVP_PKEY_OP_VERIFYCTX && pctx->op.sig.algctx != NULL &&
    ...
    if (pctx->op.sig.signature->digest_verify != NULL) {
      ...
      ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
      ERR_set_mark();
      ret = signature->digest_verify(pctx->op.sig.algctx, sigret, siglen, tbs,
                                     tbslen);
      if (ret <= 0 && ERR_count_to_mark() == 0)
        ERR_raise_data(ERR_LIB_EVP, EVP_R_PROVIDER_SIGNATURE_FAILURE,
  ...
  if (EVP_DigestVerifyUpdate(ctx, tbs, tbslen) <= 0) return -1;
