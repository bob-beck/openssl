...
static int
update(EVP_MD_CTX* ctx, const void* data, size_t datalen)
{
 ...
 */
static const char*
canon_mdname(const char* mdname)
{
    ...
    switch (iter) {
      case 1:
        signature = EVP_SIGNATURE_fetch(
          locpctx->libctx, supported_sig, locpctx->propquery);
        if (signature != NULL)
...
reinitialize:
  ...
  if (ver) {
    ...
    ret = signature->digest_verify_init(
    ...
    ret = signature->digest_sign_init(
      locpctx->op.sig.algctx, mdname, provkey, params);
  }
...
int
EVP_DigestSignInit_ex(EVP_MD_CTX* ctx,
                      EVP_PKEY_CTX** pctx,
                      const char* mdname,
                      OSSL_LIB_CTX* libctx,
                      const char* props,
                      ...
                      const OSSL_PARAM params[])
{
  return do_sigver_init(
    ctx, pctx, NULL, mdname, libctx, props, NULL, pkey, 0, params);
}

int
EVP_DigestSignInit(EVP_MD_CTX* ctx,
                   EVP_PKEY_CTX** pctx,
                   const EVP_MD* type,
                   ENGINE* e,
                   EVP_PKEY* pkey)
...
int
EVP_DigestVerifyInit_ex(EVP_MD_CTX* ctx,
                        EVP_PKEY_CTX** pctx,
                        const char* mdname,
                        OSSL_LIB_CTX* libctx,
                        const char* props,
...
{
  return do_sigver_init(
    ctx, pctx, NULL, mdname, libctx, props, NULL, pkey, 1, params);
}

int
EVP_DigestVerifyInit(EVP_MD_CTX* ctx,
                     EVP_PKEY_CTX** pctx,
                     const EVP_MD* type,
                     ENGINE* e,
                     EVP_PKEY* pkey)
...
int
EVP_DigestSignUpdate(EVP_MD_CTX* ctx, const void* data, size_t dsize)
{
...
int
EVP_DigestVerifyUpdate(EVP_MD_CTX* ctx, const void* data, size_t dsize)
{
  EVP_SIGNATURE* signature;
  const char* desc;
  EVP_PKEY_CTX* pctx = ctx->pctx;
  ...
  if (pctx->operation == EVP_PKEY_OP_SIGNCTX && pctx->op.sig.algctx != NULL &&
    ...
    if (signature->digest_sign != NULL) {
      ...
      if (sigret != NULL)
        ctx->flags |= EVP_MD_CTX_FLAG_FINALISED;
      ERR_set_mark();
      ret = signature->digest_sign(pctx->op.sig.algctx,
                                   sigret,
                                   siglen,
...
int
EVP_DigestVerify(EVP_MD_CTX* ctx,
