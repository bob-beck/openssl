 ...
 * export. We start by attempting to fetch the keymgmt with the same
 * name as |ctx->keymgmt| from |prov| using the property query from
 * |ctx|. If that succeeds, we then try to export |ctx->pkey| back to
 * |prov| (evp_pkey_export_to_provider() is smart enough to
 * only actually export it if |tmp_keymgmt| is different from
 * |ctx->pkey|'s keymgmt).
 *
 * returns 1 if |*out_keymgmt| and |*out_provkey| are set to the keymgmt
 * and provider key, 0, otherwise.
 */
static int ensure_ctx_key_is_provided(EVP_PKEY_CTX *ctx,
                                                const OSSL_PROVIDER *prov,
                                                EVP_KEYMGMT **out_keymgmt,
                                                void **out_provkey) {
    ...
    if (tmp_keymgmt == NULL)
        goto err;
    *out_keymgmt = tmp_keymgmt;
    *out_provkey = evp_pkey_export_to_provider(ctx->pkey, ctx->libctx,
                                                   out_keymgmt, ctx->propquery);
    if (*out_keymgmt == NULL || *out_provkey == NULL)
        goto err;
                          ...
                          const OSSL_PARAM params[])
{
    ...
    /*
     *  Do the normal signature fetch, using the fetching data given
     *  by the EVP_PKEY_CTX.
     */
    ...
    if (!ensure_ctx_key_is_provided(locpctx, tmp_prov, &tmp_keymgmt,
                                    &provkey)) {
        /*
         *  That did not work, so try the provider specific signature
         *  fetch, from the same provider as |locpctx->keymgmt|
         */
        ...
        /*
         * If we could not fetch the provider specific signature,
         * jump to legacy
         */
        if (signature == NULL)
            goto legacy;
