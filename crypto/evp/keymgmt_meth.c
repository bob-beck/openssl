...
static void
evp_keymgmt_free (void *data)
{
  EVP_KEYMGMT_free (data);
}

static int
evp_keymgmt_up_ref (void *data)
{
...
#ifndef FIPS_MODULE
static void
help_get_legacy_alg_type_from_keymgmt (const char *keytype, void *arg)
{
  int *type = arg;
...
static int
get_legacy_alg_type_from_keymgmt (const EVP_KEYMGMT *keymgmt)
{
  int type = NID_undef;
...
static void *
keymgmt_from_algorithm (int name_id, const OSSL_ALGORITHM *algodef,
                        OSSL_PROVIDER *prov)
{
...
EVP_KEYMGMT *
evp_keymgmt_fetch_from_prov (OSSL_PROVIDER *prov, const char *name,
...
EVP_KEYMGMT *
EVP_KEYMGMT_fetch (OSSL_LIB_CTX *ctx, const char *algorithm,
                   const char *properties)
{
...
const OSSL_PROVIDER *
EVP_KEYMGMT_get0_provider (const EVP_KEYMGMT *keymgmt)
{
  return keymgmt->prov;
}
...
const char *
EVP_KEYMGMT_get0_description (const EVP_KEYMGMT *keymgmt)
{
  return keymgmt->description;
}

const char *
EVP_KEYMGMT_get0_name (const EVP_KEYMGMT *keymgmt)
{
  return keymgmt->type_name;
}

int
EVP_KEYMGMT_is_a (const EVP_KEYMGMT *keymgmt, const char *name)
{
  return keymgmt != NULL
...
void
EVP_KEYMGMT_do_all_provided (OSSL_LIB_CTX *libctx,
...
int
EVP_KEYMGMT_names_do_all (const EVP_KEYMGMT *keymgmt,
...
void
evp_keymgmt_freedata (const EVP_KEYMGMT *keymgmt, void *keydata)
{
...
void *
evp_keymgmt_gen_init (const EVP_KEYMGMT *keymgmt, int selection,
...
int
evp_keymgmt_gen_set_template (const EVP_KEYMGMT *keymgmt, void *genctx,
                              void *templ)
{
  /*
...
int
evp_keymgmt_gen_set_params (const EVP_KEYMGMT *keymgmt, void *genctx,
                            const OSSL_PARAM params[])
{
  if (keymgmt->gen_set_params == NULL)
    return 0;
...
int
evp_keymgmt_gen_get_params (const EVP_KEYMGMT *keymgmt, void *genctx,
                            OSSL_PARAM params[])
{
  if (keymgmt->gen_get_params == NULL)
    return 0;
...
void *
evp_keymgmt_gen (const EVP_KEYMGMT *keymgmt, void *genctx, OSSL_CALLBACK *cb,
                 void *cbarg)
{
  void *ret;
...
void *
evp_keymgmt_load (const EVP_KEYMGMT *keymgmt, const void *objref,
                  size_t objref_sz)
{
...
int
evp_keymgmt_get_params (const EVP_KEYMGMT *keymgmt, void *keydata,
                        OSSL_PARAM params[])
{
  if (keymgmt->get_params == NULL)
    return 1;
...
const OSSL_PARAM *
EVP_KEYMGMT_gettable_params (const EVP_KEYMGMT *keymgmt)
{
  void *provctx = ossl_provider_ctx (EVP_KEYMGMT_get0_provider (keymgmt));
...
int
evp_keymgmt_set_params (const EVP_KEYMGMT *keymgmt, void *keydata,
                        const OSSL_PARAM params[])
{
  if (keymgmt->set_params == NULL)
    return 1;
...
const OSSL_PARAM *
EVP_KEYMGMT_settable_params (const EVP_KEYMGMT *keymgmt)
{
  void *provctx = ossl_provider_ctx (EVP_KEYMGMT_get0_provider (keymgmt));
...
int
evp_keymgmt_has (const EVP_KEYMGMT *keymgmt, void *keydata, int selection)
{
  /* This is mandatory, no need to check for its presence */
...
int
evp_keymgmt_validate (const EVP_KEYMGMT *keymgmt, void *keydata, int selection,
                      int checktype)
{
...
int
evp_keymgmt_match (const EVP_KEYMGMT *keymgmt, const void *keydata1,
...
int
evp_keymgmt_import (const EVP_KEYMGMT *keymgmt, void *keydata, int selection,
                    const OSSL_PARAM params[])
{
  if (keymgmt->import == NULL)
    return 0;
...
int
evp_keymgmt_export (const EVP_KEYMGMT *keymgmt, void *keydata, int selection,
...
void *
evp_keymgmt_dup (const EVP_KEYMGMT *keymgmt, const void *keydata_from,
                 int selection)
{
