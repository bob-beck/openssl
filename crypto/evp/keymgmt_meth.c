...
static void* keymgmt_new(void) {
  ...
  if ((keymgmt = OPENSSL_zalloc(sizeof(*keymgmt))) == NULL) return NULL;
...
static void help_get_legacy_alg_type_from_keymgmt(const char* keytype,
  ...
  if (*type == NID_undef) *type = evp_pkey_name2type(keytype);
}
...
static void* keymgmt_from_algorithm(int name_id, const OSSL_ALGORITHM* algodef,
  ...
  if ((keymgmt = keymgmt_new()) == NULL) return NULL;
  ...
  for (; fns->function_id != 0; fns++) {
    switch (fns->function_id) {
      case OSSL_FUNC_KEYMGMT_NEW:
        if (keymgmt->new == NULL) keymgmt->new = OSSL_FUNC_keymgmt_new(fns);
        break;
      ...
      case OSSL_FUNC_KEYMGMT_GEN:
        if (keymgmt->gen == NULL) keymgmt->gen = OSSL_FUNC_keymgmt_gen(fns);
        break;
      ...
      case OSSL_FUNC_KEYMGMT_FREE:
        if (keymgmt->free == NULL) keymgmt->free = OSSL_FUNC_keymgmt_free(fns);
        break;
      case OSSL_FUNC_KEYMGMT_LOAD:
        if (keymgmt->load == NULL) keymgmt->load = OSSL_FUNC_keymgmt_load(fns);
        break;
      ...
      case OSSL_FUNC_KEYMGMT_HAS:
        if (keymgmt->has == NULL) keymgmt->has = OSSL_FUNC_keymgmt_has(fns);
        break;
      case OSSL_FUNC_KEYMGMT_DUP:
        if (keymgmt->dup == NULL) keymgmt->dup = OSSL_FUNC_keymgmt_dup(fns);
        break;
      ...
      case OSSL_FUNC_KEYMGMT_IMPORT_TYPES:
        if (keymgmt->import_types == NULL) {
          if (importtypesfncnt == 0) importfncnt++;
          importtypesfncnt++;
      ...
      case OSSL_FUNC_KEYMGMT_EXPORT_TYPES:
        if (keymgmt->export_types == NULL) {
          if (exporttypesfncnt == 0) exportfncnt++;
          exporttypesfncnt++;
...
void* evp_keymgmt_newdata(const EVP_KEYMGMT* keymgmt) {
   ...
   */
  if (keymgmt->new == NULL) return NULL;
...
void* evp_keymgmt_gen_init(const EVP_KEYMGMT* keymgmt, int selection,
  ...
  if (keymgmt->gen_init == NULL) return NULL;
...
int evp_keymgmt_gen_set_template(const EVP_KEYMGMT* keymgmt, void* genctx,
   ...
   */
  if (keymgmt->gen_set_template == NULL) return 1;
  return keymgmt->gen_set_template(genctx, templ);
}

int evp_keymgmt_gen_set_params(const EVP_KEYMGMT* keymgmt, void* genctx,
                               const OSSL_PARAM params[]) {
  if (keymgmt->gen_set_params == NULL) return 0;
...
const OSSL_PARAM* EVP_KEYMGMT_gen_gettable_params(const EVP_KEYMGMT* keymgmt) {
  ...
  if (keymgmt->gen_gettable_params == NULL) return NULL;
...
void evp_keymgmt_gen_cleanup(const EVP_KEYMGMT* keymgmt, void* genctx) {
  if (keymgmt->gen_cleanup != NULL) keymgmt->gen_cleanup(genctx);
}
...
void* evp_keymgmt_load(const EVP_KEYMGMT* keymgmt, const void* objref,
                       size_t objref_sz) {
  if (evp_keymgmt_has_load(keymgmt)) return keymgmt->load(objref, objref_sz);
  return NULL;
}

int evp_keymgmt_get_params(const EVP_KEYMGMT* keymgmt, void* keydata,
                           OSSL_PARAM params[]) {
  if (keymgmt->get_params == NULL) return 1;
...
const OSSL_PARAM* EVP_KEYMGMT_settable_params(const EVP_KEYMGMT* keymgmt) {
  ...
  if (keymgmt->settable_params == NULL) return NULL;
...
int evp_keymgmt_validate(const EVP_KEYMGMT* keymgmt, void* keydata,
  ...
  if (keymgmt->validate == NULL) return 1;
...
int evp_keymgmt_match(const EVP_KEYMGMT* keymgmt, const void* keydata1,
  ...
  if (keymgmt->match == NULL) return 0;
...
int evp_keymgmt_import(const EVP_KEYMGMT* keymgmt, void* keydata, int selection,
                       const OSSL_PARAM params[]) {
  if (keymgmt->import == NULL) return 0;
...
const OSSL_PARAM* evp_keymgmt_export_types(const EVP_KEYMGMT* keymgmt,
  ...
  if (keymgmt->export_types == NULL) return NULL;
...
void* evp_keymgmt_dup(const EVP_KEYMGMT* keymgmt, const void* keydata_from,
  ...
  if (keymgmt->dup == NULL) return NULL;
