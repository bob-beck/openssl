...
try_provided_check(EVP_PKEY_CTX* ctx, int selection, int checktype)
{
  ...
  keydata = evp_pkey_export_to_provider(
    ctx->pkey, ctx->libctx, &keymgmt, ctx->propquery);
  if (keydata == NULL) {
...
evp_pkey_public_check_combined(EVP_PKEY_CTX* ctx, int checktype)
{
  ...
  if ((ok = try_provided_check(
         ctx, OSSL_KEYMGMT_SELECT_PUBLIC_KEY, checktype)) != -1)
    return ok;
...
evp_pkey_param_check_combined(EVP_PKEY_CTX* ctx, int checktype)
{
  ...
  if ((ok = try_provided_check(
         ctx, OSSL_KEYMGMT_SELECT_ALL_PARAMETERS, checktype)) != -1)
    return ok;
