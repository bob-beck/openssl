...
static int try_provided_check(EVP_PKEY_CTX* ctx, int selection, int checktype) {
  ...
  if (evp_pkey_ctx_is_legacy(ctx)) return -1;
...
static int evp_pkey_public_check_combined(EVP_PKEY_CTX* ctx, int checktype) {
  ...
  if (pkey->type == EVP_PKEY_NONE) goto not_supported;

#ifndef FIPS_MODULE
  /* legacy */
  /* call customized public key check function first */
  if (ctx->pmeth->public_check != NULL) return ctx->pmeth->public_check(pkey);
...
static int evp_pkey_param_check_combined(EVP_PKEY_CTX* ctx, int checktype) {
  ...
  if (ctx->pmeth->param_check != NULL) return ctx->pmeth->param_check(pkey);
...
int EVP_PKEY_pairwise_check(EVP_PKEY_CTX* ctx) {
  ...
  if (ctx->pmeth->check != NULL) return ctx->pmeth->check(pkey);
