...
EVP_PKEY* evp_pkcs82pkey_legacy(const PKCS8_PRIV_KEY_INFO* p8,
  ...
  if (!PKCS8_pkey_get0(&algoid, NULL, NULL, NULL, p8)) return NULL;
  ...
  if (pkey->ameth->priv_decode_ex != NULL) {
    if (!pkey->ameth->priv_decode_ex(pkey, p8, libctx, propq)) goto error;
...
const char* EVP_PKEY_get0_type_name(const EVP_PKEY* key) {
  ...
  if (key->keymgmt != NULL) return EVP_KEYMGMT_get0_name(key->keymgmt);
...
const OSSL_PROVIDER* EVP_PKEY_get0_provider(const EVP_PKEY* key) {
  if (evp_pkey_is_provided(key)) return EVP_KEYMGMT_get0_provider(key->keymgmt);
  return NULL;
}
