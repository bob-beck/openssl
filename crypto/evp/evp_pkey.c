...
EVP_PKEY*
evp_pkcs82pkey_legacy(const PKCS8_PRIV_KEY_INFO* p8,
                      OSSL_LIB_CTX* libctx,
                      const char* propq)
{
  EVP_PKEY* pkey = NULL;
  ...
  if (!EVP_PKEY_set_type(pkey, OBJ_obj2nid(algoid))) {
    ...
    ERR_raise_data(
      ERR_LIB_EVP, EVP_R_UNSUPPORTED_PRIVATE_KEY_ALGORITHM, "TYPE=%s", obj_tmp);
    goto error;
  }
...
EVP_PKEY*
EVP_PKCS82PKEY_ex(const PKCS8_PRIV_KEY_INFO* p8,
                  OSSL_LIB_CTX* libctx,
                  const char* propq)
{
  EVP_PKEY* pkey = NULL;
  ...
  dctx = OSSL_DECODER_CTX_new_for_pkey(
    &pkey, "DER", "PrivateKeyInfo", keytype, selection, libctx, propq);
  ...
  if (dctx != NULL && OSSL_DECODER_CTX_get_num_decoders(dctx) == 0) {
     ...
     */
    dctx = OSSL_DECODER_CTX_new_for_pkey(
      &pkey, "DER", "PrivateKeyInfo", NULL, selection, libctx, propq);
  }
...
PKCS8_PRIV_KEY_INFO*
EVP_PKEY2PKCS8(const EVP_PKEY* pkey)
{
...
int
EVP_PKEY_get_attr_by_OBJ(const EVP_PKEY* key,
...
int
EVP_PKEY_add1_attr_by_OBJ(EVP_PKEY* key,
                          const ASN1_OBJECT* obj,
                          int type,
...
int
EVP_PKEY_add1_attr_by_NID(EVP_PKEY* key,
                          int nid,
                          int type,
                          const unsigned char* bytes,
                          int len)
...
int
EVP_PKEY_add1_attr_by_txt(EVP_PKEY* key,
                          const char* attrname,
                          int type,
                          const unsigned char* bytes,
                          int len)
...
const char*
EVP_PKEY_get0_type_name(const EVP_PKEY* key)
{
...
const OSSL_PROVIDER*
EVP_PKEY_get0_provider(const EVP_PKEY* key)
{
