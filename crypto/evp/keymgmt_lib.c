 ...
 */
static int
match_type(const EVP_KEYMGMT* keymgmt1, const EVP_KEYMGMT* keymgmt2)
{
                        ...
                        void* export_cbarg)
{
  if (pk == NULL || export_cb == NULL)
    return 0;
  return evp_keymgmt_export(
    pk->keymgmt, pk->keydata, selection, export_cb, export_cbarg);
}
                                    ...
                                    int selection)
{
  ...
  if (!evp_keymgmt_util_export(
        pk, selection, &evp_keymgmt_util_try_import, &import_data))
  ...
  if (!evp_keymgmt_util_cache_keydata(
        pk, keymgmt, import_data.keydata, selection)) {
    CRYPTO_THREAD_unlock(pk->lock);
    evp_keymgmt_freedata(keymgmt, import_data.keydata);
    return NULL;
  }
                                       ...
                                       size_t mdname_sz)
{
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
    OSSL_PKEY_PARAM_DEFAULT_DIGEST, mddefault, sizeof(mddefault));
