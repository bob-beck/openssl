...
int evp_keymgmt_util_export(const EVP_PKEY* pk, int selection,
  ...
  if (pk == NULL || export_cb == NULL) return 0;
...
void* evp_keymgmt_util_export_to_provider(EVP_PKEY* pk, EVP_KEYMGMT* keymgmt,
  ...
  /* Export to where? */
  if (keymgmt == NULL) return NULL;

  /* If we have an unassigned key, give up */
  if (pk->keydata == NULL) return NULL;
  ...
  if (pk->keymgmt->export == NULL) return NULL;

  /*
   * Make sure that the type of the keymgmt to export to matches the type
   * of the "origin"
   */
  if (!ossl_assert(match_type(pk->keymgmt, keymgmt))) return NULL;
...
int evp_keymgmt_util_cache_keydata(EVP_PKEY* pk, EVP_KEYMGMT* keymgmt,
  ...
  if (keydata != NULL) {
    if (pk->operation_cache == NULL) {
      ...
      if (pk->operation_cache == NULL) return 0;
    }
...
int evp_keymgmt_util_match(EVP_PKEY* pk1, EVP_PKEY* pk2, int selection) {
  ...
  if (keydata1 == NULL && keydata2 == NULL) return 1;
  /* If only one of the keydata is NULL, then they're different keys */
  if (keydata1 == NULL || keydata2 == NULL) return 0;
...
int evp_keymgmt_util_copy(EVP_PKEY* to, EVP_PKEY* from, int selection) {
  ...
  if (from == NULL || from->keydata == NULL) return 0;
  ...
  if (to_keymgmt == from->keymgmt && to_keymgmt->dup != NULL &&
     ...
     */
    if (to_keydata == NULL) to_keydata = alloc_keydata = import_data.keydata;
  } else {
    ERR_raise(ERR_LIB_EVP, EVP_R_DIFFERENT_KEY_TYPES);
    return 0;
  }
...
int evp_keymgmt_util_get_deflt_digest_name(EVP_KEYMGMT* keymgmt, void* keydata,
  ...
  if (!evp_keymgmt_get_params(keymgmt, keydata, params)) return 0;
  ...
  if (rv > 0) OPENSSL_strlcpy(mdname, result, mdname_sz);
  return rv;
}
...
const char* evp_keymgmt_util_query_operation_name(EVP_KEYMGMT* keymgmt,
  ...
  if (keymgmt != NULL) {
    ...
    if (name == NULL) name = EVP_KEYMGMT_get0_name(keymgmt);
  }
  return name;
}
