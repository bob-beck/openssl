...
legacy:
  ...
  switch (ctx->operation) {
    case EVP_PKEY_OP_ENCRYPT:
      if (ctx->pmeth->encrypt_init == NULL) return 1;
      ret = ctx->pmeth->encrypt_init(ctx);
      break;
    case EVP_PKEY_OP_DECRYPT:
      if (ctx->pmeth->decrypt_init == NULL) return 1;
...
EVP_ASYM_CIPHER* evp_asym_cipher_fetch_from_prov(OSSL_PROVIDER* prov,
                                                 ...
                                                 const char* properties) {
  return evp_generic_fetch_from_prov(prov, OSSL_OP_ASYM_CIPHER, algorithm,
                                     properties, evp_asym_cipher_from_algorithm,
...
const OSSL_PARAM* EVP_ASYM_CIPHER_gettable_ctx_params(
    const EVP_ASYM_CIPHER* cip) {
  void* provctx;

  if (cip == NULL || cip->gettable_ctx_params == NULL) return NULL;
...
const OSSL_PARAM* EVP_ASYM_CIPHER_settable_ctx_params(
  ...
  if (cip == NULL || cip->settable_ctx_params == NULL) return NULL;
