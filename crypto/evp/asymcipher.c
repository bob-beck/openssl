...
static void
evp_asym_cipher_free(void* data)
{
  EVP_ASYM_CIPHER_free(data);
}

static int
evp_asym_cipher_up_ref(void* data)
{
  return EVP_ASYM_CIPHER_up_ref(data);
}

static int
evp_pkey_asym_cipher_init(EVP_PKEY_CTX* ctx,
                          int operation,
...
int
EVP_PKEY_encrypt_init_ex(EVP_PKEY_CTX* ctx, const OSSL_PARAM params[])
{
                 ...
                 size_t inlen)
{
  ...
  ERR_set_mark();
  ret = cipher->encrypt(
    ctx->op.ciph.algctx, out, outlen, (out == NULL ? 0 : *outlen), in, inlen);
...
int
EVP_PKEY_decrypt_init_ex(EVP_PKEY_CTX* ctx, const OSSL_PARAM params[])
{
  return evp_pkey_asym_cipher_init(ctx, EVP_PKEY_OP_DECRYPT, params);
}

int
EVP_PKEY_decrypt(EVP_PKEY_CTX* ctx,
                 unsigned char* out,
                 size_t* outlen,
                 const unsigned char* in,
                 size_t inlen)
...
static EVP_ASYM_CIPHER*
evp_asym_cipher_new(OSSL_PROVIDER* prov)
{
...
static void*
evp_asym_cipher_from_algorithm(int name_id,
...
OSSL_PROVIDER*
EVP_ASYM_CIPHER_get0_provider(const EVP_ASYM_CIPHER* cipher)
{
  return cipher->prov;
}

EVP_ASYM_CIPHER*
EVP_ASYM_CIPHER_fetch(OSSL_LIB_CTX* ctx,
...
int
EVP_ASYM_CIPHER_is_a(const EVP_ASYM_CIPHER* cipher, const char* name)
{
...
const char*
EVP_ASYM_CIPHER_get0_name(const EVP_ASYM_CIPHER* cipher)
{
  return cipher->type_name;
}

const char*
EVP_ASYM_CIPHER_get0_description(const EVP_ASYM_CIPHER* cipher)
{
  return cipher->description;
}

void
EVP_ASYM_CIPHER_do_all_provided(OSSL_LIB_CTX* libctx,
...
int
EVP_ASYM_CIPHER_names_do_all(const EVP_ASYM_CIPHER* cipher,
