...
int EVP_PKEY_save_parameters(EVP_PKEY* pkey, int mode) {
  ...
  if (pkey->type == EVP_PKEY_DSA) {
    ...
    if (mode >= 0) pkey->save_parameters = mode;
    return ret;
  }
#endif
...
int EVP_PKEY_copy_parameters(EVP_PKEY* to, const EVP_PKEY* from) {
  ...
  if (evp_pkey_is_legacy(to) && evp_pkey_is_provided(from)) {
    if (!evp_pkey_copy_downgraded(&downgraded_from, from)) goto end;
  ...
  if (evp_pkey_is_blank(to)) {
    ...
    if (evp_pkey_is_legacy(from)) {
      if (EVP_PKEY_set_type(to, from->type) == 0) goto end;
    } else
#endif /* !FIPS_MODULE */
    {
      if (EVP_PKEY_set_type_by_keymgmt(to, from->keymgmt) == 0) goto end;
    }
  }
#ifndef FIPS_MODULE
...
int EVP_PKEY_eq(const EVP_PKEY* a, const EVP_PKEY* b) {
  ...
  if (a->ameth != NULL) {
    ...
    if (a->ameth->pub_cmp != NULL) return a->ameth->pub_cmp(a, b);
  }
...
static EVP_PKEY* new_raw_key_int(OSSL_LIB_CTX* libctx, const char* strtype,
  ...
  if (!ossl_assert(pkey->ameth != NULL)) goto err;
...
static EVP_PKEY* new_cmac_key_int(const unsigned char* priv, size_t len,
  ...
  if (cipher != NULL) cipher_name = EVP_CIPHER_get0_name(cipher);
...
int EVP_PKEY_assign(EVP_PKEY* pkey, int type, void* key) {
  ...
  if (pkey == NULL || !EVP_PKEY_set_type(pkey, type)) return 0;
...
void* EVP_PKEY_get0(const EVP_PKEY* pkey) {
  ...
  if (!evp_pkey_is_provided(pkey)) return pkey->pkey.ptr;
...
DSA* EVP_PKEY_get1_DSA(EVP_PKEY* pkey) {
  ...
  if (ret != NULL && !DSA_up_ref(ret)) return NULL;
...
static ECX_KEY* evp_pkey_get1_ECX_KEY(EVP_PKEY* pkey, int type) {
  ...
  if (ret != NULL && !ossl_ecx_key_up_ref(ret)) ret = NULL;
  return ret;
}
...
int EVP_PKEY_set1_DH(EVP_PKEY* pkey, DH* dhkey) {
  ...
  if (!DH_up_ref(dhkey)) return 0;

  ret = EVP_PKEY_assign(pkey, type, dhkey);

  if (!ret) DH_free(dhkey);
...
DH* EVP_PKEY_get1_DH(EVP_PKEY* pkey) {
  ...
  if (ret != NULL && !DH_up_ref(ret)) ret = NULL;
...
int evp_pkey_name2type(const char* name) {
  ...
  if ((type = EVP_PKEY_type(OBJ_sn2nid(name))) != NID_undef) return type;
...
const char* evp_pkey_type2name(int type) {
  ...
  for (i = 0; i < OSSL_NELEM(standard_name2type); i++) {
    if (type == (int)standard_name2type[i].id) return standard_name2type[i].ptr;
  }
...
int EVP_PKEY_is_a(const EVP_PKEY* pkey, const char* name) {
  ...
  if (pkey->keymgmt == NULL) return pkey->type == evp_pkey_name2type(name);
...
int EVP_PKEY_type_names_do_all(const EVP_PKEY* pkey,
                               ...
                               void* data) {
  if (!evp_pkey_is_typed(pkey)) return 0;
...
static int print_pkey(const EVP_PKEY* pkey, BIO* out, int indent,
  ...
  if (!print_set_indent(&out, &pop_f_prefix, &saved_indent, indent)) return 0;
...
static void mdname2nid(const char* mdname, void* data) {
  ...
  *nid = OBJ_sn2nid(mdname);
  if (*nid == NID_undef) *nid = OBJ_ln2nid(mdname);
}

static int legacy_asn1_ctrl_to_param(EVP_PKEY* pkey, int op, int arg1,
                                     void* arg2) {
  if (pkey->keymgmt == NULL) return 0;
  switch (op) {
...
static int evp_pkey_asn1_ctrl(EVP_PKEY* pkey, int op, int arg1, void* arg2) {
  ...
  if (pkey->ameth->pkey_ctrl == NULL) return -2;
...
int EVP_PKEY_get_default_digest_name(EVP_PKEY* pkey, char* mdname,
...

  {
    ...
    if (rv > 0) OPENSSL_strlcpy(mdname, name, mdname_sz);
    return rv;
  }
}
...
int EVP_PKEY_digestsign_supports_digest(EVP_PKEY* pkey, OSSL_LIB_CTX* libctx,
  ...
  if ((ctx = EVP_MD_CTX_new()) == NULL) return -1;
...
size_t EVP_PKEY_get1_encoded_public_key(EVP_PKEY* pkey, unsigned char** ppub) {
...
#endif
  {
    ...
    EVP_PKEY_get_octet_string_param(pkey, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY,
                                    NULL, 0, &return_size);
    if (return_size == OSSL_PARAM_UNMODIFIED) return 0;
...
EVP_PKEY* EVP_PKEY_new(void) {
  ...
  if (!CRYPTO_NEW_REF(&ret->references, 1)) goto err;
...
static int pkey_set_type(EVP_PKEY* pkey, ENGINE* e, int type, const char* str,
  ...
  if (pkey != NULL) {
    ...
    if (free_it) evp_pkey_free_it(pkey);
#ifndef FIPS_MODULE
    /*
...
#ifndef OPENSSL_NO_ENGINE
  if (pkey == NULL && eptr != NULL) ENGINE_finish(e);
#endif
#endif
  ...
  if (pkey != NULL) {
     ...
     */
    if (keymgmt == NULL) pkey->ameth = ameth;
     ...
     */
    if (ameth != NULL) {
      if (type == EVP_PKEY_NONE) pkey->type = ameth->pkey_id;
    } else {
...
int EVP_PKEY_up_ref(EVP_PKEY* pkey) {
  ...
  if (CRYPTO_UP_REF(&pkey->references, &i) <= 0) return 0;
...
EVP_PKEY* EVP_PKEY_dup(EVP_PKEY* pkey) {
  ...
  if ((dup_pk = EVP_PKEY_new()) == NULL) return NULL;

  if (evp_pkey_is_blank(pkey)) goto done;

#ifndef FIPS_MODULE
  if (evp_pkey_is_provided(pkey))
#endif /* !FIPS_MODULE */
  {
    if (!evp_keymgmt_util_copy(dup_pk, pkey, OSSL_KEYMGMT_SELECT_ALL)) goto err;
    goto done;
  }
  ...
  if (evp_pkey_is_legacy(pkey)) {
    ...
    if (!ameth->copy(dup_pk, pkey)) goto err;
    goto done;
  }
#endif /* !FIPS_MODULE */
...
void evp_pkey_free_legacy(EVP_PKEY* x) {
  ...
  if (ameth != NULL) {
    ...
    if (ameth->pkey_free != NULL) ameth->pkey_free(x);
    x->pkey.ptr = NULL;
  }
#ifndef OPENSSL_NO_ENGINE
  ENGINE_finish(tmpe);
...
void* evp_pkey_export_to_provider(EVP_PKEY* pk, OSSL_LIB_CTX* libctx,
  ...
  if (pk->pkey.ptr != NULL) {
    ...
    if (pk->ameth->dirty_cnt(pk) == pk->dirty_cnt_copy) {
      if (!CRYPTO_THREAD_read_lock(pk->lock)) goto end;
    ...
    if (!EVP_KEYMGMT_is_a(tmp_keymgmt, OBJ_nid2sn(pk->type))) goto end;

    if ((keydata = evp_keymgmt_newdata(tmp_keymgmt)) == NULL) goto end;
    ...
    if (!CRYPTO_THREAD_write_lock(pk->lock)) goto end;
    if (pk->ameth->dirty_cnt(pk) != pk->dirty_cnt_copy &&
   ...
   */
  if (keydata == NULL) tmp_keymgmt = NULL;
...
int evp_pkey_copy_downgraded(EVP_PKEY** dest, const EVP_PKEY* src) {
  ...
  if (!ossl_assert(dest != NULL)) return 0;
  ...
  if (evp_pkey_is_assigned(src) && evp_pkey_is_provided(src)) {
    ...
    if (type != EVP_PKEY_KEYMGMT) keytype = OBJ_nid2sn(type);
    ...
    if (EVP_PKEY_set_type(*dest, type)) {
      ...
      if ((*dest)->ameth->import_from == NULL) {
        ...
        if (pctx == NULL) ERR_raise(ERR_LIB_EVP, ERR_R_EVP_LIB);
...
void* evp_pkey_get_legacy(EVP_PKEY* pk) {
  ...
  if (!ossl_assert(pk != NULL)) return NULL;

  /*
   * If this isn't an assigned provider side key, we just use any existing
   * origin legacy key.
   */
  if (!evp_pkey_is_assigned(pk)) return NULL;
  if (!evp_pkey_is_provided(pk)) return pk->pkey.ptr;

  if (!CRYPTO_THREAD_read_lock(pk->lock)) return NULL;

  ret = pk->legacy_cache_pkey.ptr;

  if (!CRYPTO_THREAD_unlock(pk->lock)) return NULL;

  if (ret != NULL) return ret;

  if (!evp_pkey_copy_downgraded(&tmp_copy, pk)) goto err;

  if (!CRYPTO_THREAD_write_lock(pk->lock)) goto err;
...
int EVP_PKEY_get_bn_param(const EVP_PKEY* pkey, const char* key_name,
  ...
  if (key_name == NULL || bn == NULL) return 0;

  memset(buffer, 0, sizeof(buffer));
  params[0] = OSSL_PARAM_construct_BN(key_name, buffer, sizeof(buffer));
  params[1] = OSSL_PARAM_construct_end();
  if (!EVP_PKEY_get_params(pkey, params)) {
    if (!OSSL_PARAM_modified(params) || params[0].return_size == 0) return 0;
    ...
    if (!EVP_PKEY_get_params(pkey, params)) goto err;
  }
  /* Fail if the param was not found */
  if (!OSSL_PARAM_modified(params)) goto err;
...
int EVP_PKEY_get_utf8_string_param(const EVP_PKEY* pkey, const char* key_name,
  ...
  if (ret2 && str != NULL) str[params[0].return_size] = '\0';
...
int EVP_PKEY_set_bn_param(EVP_PKEY* pkey, const char* key_name,
  ...
  if (!ossl_assert(bsize <= (int)sizeof(buffer))) return 0;

  if (BN_bn2nativepad(bn, buffer, bsize) < 0) return 0;
...
int EVP_PKEY_get_ec_point_conv_form(const EVP_PKEY* pkey) {
  ...
  if (strcmp(name, "uncompressed") == 0) return POINT_CONVERSION_UNCOMPRESSED;

  if (strcmp(name, "compressed") == 0) return POINT_CONVERSION_COMPRESSED;

  if (strcmp(name, "hybrid") == 0) return POINT_CONVERSION_HYBRID;
