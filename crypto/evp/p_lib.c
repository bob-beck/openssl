  ...
  if (to->keymgmt != NULL && to->keydata == NULL)
    {
      ...
      if (from_keydata == NULL)
        ...
        ok = (to->keydata
              = evp_keymgmt_dup (to->keymgmt, from_keydata, SELECT_PARAMETERS))
             != NULL;
      goto end;
    }
...
#ifndef FIPS_MODULE
static EVP_PKEY *
new_raw_key_int (OSSL_LIB_CTX *libctx, const char *strtype, const char *propq,
                 int nidtype, ENGINE *e, const unsigned char *key, size_t len,
                 int key_is_priv)
{
  EVP_PKEY *pkey = NULL;
...
EVP_PKEY *
EVP_PKEY_new_raw_private_key (int type, ENGINE *e, const unsigned char *priv,
                              size_t len)
{
...
EVP_PKEY *
EVP_PKEY_new_raw_public_key (int type, ENGINE *e, const unsigned char *pub,
                             size_t len)
{
...
int
EVP_PKEY_get_raw_private_key (const EVP_PKEY *pkey, unsigned char *priv,
                              size_t *len)
{
  if (pkey->keymgmt != NULL)
    {
...
static EVP_PKEY *
new_cmac_key_int (const unsigned char *priv, size_t len,
                  const char *cipher_name, const EVP_CIPHER *cipher,
                  OSSL_LIB_CTX *libctx, const char *propq, ENGINE *e)
{
...
static int
print_set_indent (BIO **out, int *pop_f_prefix, long *saved_indent,
                  long indent)
{
  *pop_f_prefix = 0;
  *saved_indent = 0;
...
static int
print_pkey (const EVP_PKEY *pkey, BIO *out, int indent,
            int selection /* For provided encoding */,
            const char *propquery /* For provided encoding */,
            int (*legacy_print) (BIO *out, const EVP_PKEY *pkey, int indent,
                                 ASN1_PCTX *pctx),
...
int
EVP_PKEY_print_public_fp (FILE *fp, const EVP_PKEY *pkey, int indent,
                          ASN1_PCTX *pctx)
{
  int ret;
...
int
EVP_PKEY_print_private_fp (FILE *fp, const EVP_PKEY *pkey, int indent,
                           ASN1_PCTX *pctx)
{
  int ret;
  BIO *b = BIO_new_fp (fp, BIO_NOCLOSE);
...
int
EVP_PKEY_print_params_fp (FILE *fp, const EVP_PKEY *pkey, int indent,
                          ASN1_PCTX *pctx)
{
  int ret;
  BIO *b = BIO_new_fp (fp, BIO_NOCLOSE);
    ...
    case ASN1_PKEY_CTRL_DEFAULT_MD_NID:
      {
        char mdname[80] = "";
        int rv
            = EVP_PKEY_get_default_digest_name (pkey, mdname, sizeof (mdname));
...
int
EVP_PKEY_get_default_digest_name (EVP_PKEY *pkey, char *mdname,
                                  size_t mdname_sz)
{
  if (pkey->ameth == NULL)
...
int
EVP_PKEY_get_group_name (const EVP_PKEY *pkey, char *gname, size_t gname_sz,
                         size_t *gname_len)
{
...
int
EVP_PKEY_digestsign_supports_digest (EVP_PKEY *pkey, OSSL_LIB_CTX *libctx,
...
int
EVP_PKEY_set1_encoded_public_key (EVP_PKEY *pkey, const unsigned char *pub,
                                  size_t publen)
{
  if (pkey == NULL)
    return 0;
...
void *
evp_pkey_export_to_provider (EVP_PKEY *pk, OSSL_LIB_CTX *libctx,
...
int
EVP_PKEY_get_octet_string_param (const EVP_PKEY *pkey, const char *key_name,
...
int
EVP_PKEY_get_utf8_string_param (const EVP_PKEY *pkey, const char *key_name,
...
int
EVP_PKEY_get_size_t_param (const EVP_PKEY *pkey, const char *key_name,
                           size_t *out)
{
  OSSL_PARAM params[2];
...
int
EVP_PKEY_set_utf8_string_param (EVP_PKEY *pkey, const char *key_name,
                                const char *str)
{
  OSSL_PARAM params[2];
...
int
EVP_PKEY_set_octet_string_param (EVP_PKEY *pkey, const char *key_name,
