...
#ifndef FIPS_MODULE
int
EVP_PKEY_get_bits(const EVP_PKEY* pkey)
{
  int size = 0;
...
int
EVP_PKEY_get_security_bits(const EVP_PKEY* pkey)
{
  int size = 0;
...
int
EVP_PKEY_get_security_category(const EVP_PKEY* pkey)
{
...
int
EVP_PKEY_set_ex_data(EVP_PKEY* key, int idx, void* arg)
{
  return CRYPTO_set_ex_data(&key->ex_data, idx, arg);
}

void*
EVP_PKEY_get_ex_data(const EVP_PKEY* key, int idx)
{
  ...
  if (to->keymgmt != NULL && to->keydata == NULL) {
    ...
    if (from_keydata == NULL)
      ...
      ok = (to->keydata = evp_keymgmt_dup(
              to->keymgmt, from_keydata, SELECT_PARAMETERS)) != NULL;
    goto end;
  }
 ...
 */
static int
evp_pkey_cmp_any(const EVP_PKEY* a, const EVP_PKEY* b, int selection)
{
#ifdef FIPS_MODULE
...
int
EVP_PKEY_parameters_eq(const EVP_PKEY* a, const EVP_PKEY* b)
{
#ifdef FIPS_MODULE
...
int
EVP_PKEY_eq(const EVP_PKEY* a, const EVP_PKEY* b)
{
  /*
   * This will just call evp_keymgmt_util_match when legacy support
   * is gone.
...
#ifndef FIPS_MODULE
static EVP_PKEY*
new_raw_key_int(OSSL_LIB_CTX* libctx,
                const char* strtype,
                const char* propq,
                int nidtype,
                ENGINE* e,
...
EVP_PKEY*
EVP_PKEY_new_raw_private_key_ex(OSSL_LIB_CTX* libctx,
                                const char* keytype,
                                const char* propq,
                                ...
                                size_t len)
{
  return new_raw_key_int(
    libctx, keytype, propq, EVP_PKEY_NONE, NULL, priv, len, 1);
}
...
EVP_PKEY*
EVP_PKEY_new_raw_public_key_ex(OSSL_LIB_CTX* libctx,
                               const char* keytype,
                               const char* propq,
...
{
  return new_raw_key_int(
    libctx, keytype, propq, EVP_PKEY_NONE, NULL, pub, len, 0);
}
...
static int
get_raw_key_details(const OSSL_PARAM params[], void* arg)
{
                             ...
                             size_t* len)
{
  if (pkey->keymgmt != NULL) {
    ...
    return evp_keymgmt_util_export(
      pkey, OSSL_KEYMGMT_SELECT_PRIVATE_KEY, get_raw_key_details, &raw_key);
  }
                            ...
                            size_t* len)
{
  if (pkey->keymgmt != NULL) {
    ...
    return evp_keymgmt_util_export(
      pkey, OSSL_KEYMGMT_SELECT_PUBLIC_KEY, get_raw_key_details, &raw_key);
  }
                 ...
                 ENGINE* e)
{
  ...
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_PKEY_PARAM_PRIV_KEY, (void*)priv, len);
  *p++ = OSSL_PARAM_construct_utf8_string(
    OSSL_PKEY_PARAM_CIPHER, (char*)cipher_name, 0);
  if (propq != NULL)
    *p++ = OSSL_PARAM_construct_utf8_string(
  ...
  if (engine_id != NULL)
    *p++ = OSSL_PARAM_construct_utf8_string(
      OSSL_PKEY_PARAM_ENGINE, (char*)engine_id, 0);
#endif
...
EVP_PKEY*
EVP_PKEY_new_CMAC_key(ENGINE* e,
                      const unsigned char* priv,
                      size_t len,
                      const EVP_CIPHER* cipher)
{
...
#ifndef OPENSSL_NO_ENGINE
int
EVP_PKEY_set1_engine(EVP_PKEY* pkey, ENGINE* e)
{
  if (e != NULL) {
    if (!ENGINE_init(e)) {
...
ENGINE*
EVP_PKEY_get0_engine(const EVP_PKEY* pkey)
{
  return pkey->engine;
}
#endif
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
static void
detect_foreign_key(EVP_PKEY* pkey)
{
  switch (pkey->type) {
    case EVP_PKEY_RSA:
...
const unsigned char*
EVP_PKEY_get0_poly1305(const EVP_PKEY* pkey, size_t* len)
{
  const ASN1_OCTET_STRING* os = NULL;
...
const unsigned char*
EVP_PKEY_get0_siphash(const EVP_PKEY* pkey, size_t* len)
{
  const ASN1_OCTET_STRING* os = NULL;
...
#ifndef OPENSSL_NO_DSA
static DSA*
evp_pkey_get0_DSA_int(const EVP_PKEY* pkey)
{
  if (pkey->type != EVP_PKEY_DSA) {
...
static const ECX_KEY*
evp_pkey_get0_ECX_KEY(const EVP_PKEY* pkey, int type)
{
...
static ECX_KEY*
evp_pkey_get1_ECX_KEY(EVP_PKEY* pkey, int type)
{
...
int
EVP_PKEY_set1_DH(EVP_PKEY* pkey, DH* dhkey)
{
  int ret, type;
...
int
EVP_PKEY_get_id(const EVP_PKEY* pkey)
{
  return pkey->type;
}

int
EVP_PKEY_get_base_id(const EVP_PKEY* pkey)
{
...
int
EVP_PKEY_is_a(const EVP_PKEY* pkey, const char* name)
{
  if (pkey == NULL)
    return 0;
  if (pkey->keymgmt == NULL)
...
int
EVP_PKEY_type_names_do_all(const EVP_PKEY* pkey,
...
int
EVP_PKEY_can_sign(const EVP_PKEY* pkey)
{
  if (pkey->keymgmt == NULL) {
...
static int
print_reset_indent(BIO** out, int pop_f_prefix, long saved_indent)
{
...
static int
print_set_indent(BIO** out, int* pop_f_prefix, long* saved_indent, long indent)
{
  *pop_f_prefix = 0;
  *saved_indent = 0;
  if (indent > 0) {
...
static int
print_pkey(const EVP_PKEY* pkey,
           BIO* out,
           int indent,
           int selection /* For provided encoding */,
           const char* propquery /* For provided encoding */,
           int (*legacy_print)(BIO* out,
                               const EVP_PKEY* pkey,
                               int indent,
                               ASN1_PCTX* pctx),
...
int
EVP_PKEY_print_private(BIO* out,
                       const EVP_PKEY* pkey,
                       int indent,
                       ASN1_PCTX* pctx)
{
...
int
EVP_PKEY_print_public_fp(FILE* fp,
                         const EVP_PKEY* pkey,
                         int indent,
                         ASN1_PCTX* pctx)
{
...
int
EVP_PKEY_print_private_fp(FILE* fp,
                          const EVP_PKEY* pkey,
                          int indent,
                          ASN1_PCTX* pctx)
{
...
int
EVP_PKEY_print_params_fp(FILE* fp,
                         const EVP_PKEY* pkey,
                         int indent,
                         ASN1_PCTX* pctx)
{
...
static void
mdname2nid(const char* mdname, void* data)
{
  int* nid = (int*)data;
...
static int
legacy_asn1_ctrl_to_param(EVP_PKEY* pkey, int op, int arg1, void* arg2)
{
  if (pkey->keymgmt == NULL)
    return 0;
  switch (op) {
...
static int
evp_pkey_asn1_ctrl(EVP_PKEY* pkey, int op, int arg1, void* arg2)
{
  if (pkey->ameth == NULL)
...
int
EVP_PKEY_get_default_digest_name(EVP_PKEY* pkey, char* mdname, size_t mdname_sz)
{
  if (pkey->ameth == NULL)
...
int
EVP_PKEY_get_group_name(const EVP_PKEY* pkey,
                        char* gname,
                        size_t gname_sz,
                        size_t* gname_len)
{
  return EVP_PKEY_get_utf8_string_param(
    pkey, OSSL_PKEY_PARAM_GROUP_NAME, gname, gname_sz, gname_len);
}

int
EVP_PKEY_digestsign_supports_digest(EVP_PKEY* pkey,
                                    OSSL_LIB_CTX* libctx,
                                    const char* name,
                                    const char* propq)
{
...
int
EVP_PKEY_set1_encoded_public_key(EVP_PKEY* pkey,
                                 const unsigned char* pub,
                                 size_t publen)
{
  if (pkey == NULL)
  ...
  if (evp_pkey_asn1_ctrl(
        pkey, ASN1_PKEY_CTRL_SET1_TLS_ENCPT, (int)publen, (void*)pub) <= 0)
    return 0;
  return 1;
#endif /* !FIPS_MODULE */
}
...
size_t
EVP_PKEY_get1_encoded_public_key(EVP_PKEY* pkey, unsigned char** ppub)
{
  if (pkey == NULL)
    return 0;
#ifndef FIPS_MODULE
...
#ifndef FIPS_MODULE
static void
find_ameth(const char* name, void* data)
{
  const char** str = data;
...
int
EVP_PKEY_set_type_by_keymgmt(EVP_PKEY* pkey, EVP_KEYMGMT* keymgmt)
{
#ifndef FIPS_MODULE
...
EVP_PKEY_dup(EVP_PKEY* pkey)
{
  ...
  if (!CRYPTO_dup_ex_data(
        CRYPTO_EX_INDEX_EVP_PKEY, &dup_pk->ex_data, &pkey->ex_data))
    goto err;
...
int
EVP_PKEY_get_size(const EVP_PKEY* pkey)
{
  int size = 0;
...
const char*
EVP_PKEY_get0_description(const EVP_PKEY* pkey)
{
                            ...
                            const char* propquery)
{
  ...
  if (pk->pkey.ptr != NULL) {
    ...
    if (!pk->ameth->export_to(
          pk, keydata, tmp_keymgmt->import, libctx, propquery)) {
  ...
  if (evp_pkey_is_assigned(src) && evp_pkey_is_provided(src)) {
    ...
    if (EVP_PKEY_set_type(*dest, type)) {
      ...
      if ((*dest)->ameth->import_from == NULL) {
        ERR_raise_data(
          ERR_LIB_EVP, EVP_R_NO_IMPORT_FUNCTION, "key type = %s", keytype);
      } else {
        /*
        ...
        if (pctx != NULL && evp_keymgmt_export(keymgmt,
                                               keydata,
      ...
      ERR_raise_data(
        ERR_LIB_EVP, EVP_R_KEYMGMT_EXPORT_FAILURE, "key type = %s", keytype);
    }
  }
...
int
EVP_PKEY_get_octet_string_param(const EVP_PKEY* pkey,
                                const char* key_name,
                                unsigned char* buf,
                                size_t max_buf_sz,
                                size_t* out_len)
...
int
EVP_PKEY_get_utf8_string_param(const EVP_PKEY* pkey,
                               const char* key_name,
                               char* str,
                               size_t max_buf_sz,
                               size_t* out_len)
...
int
EVP_PKEY_get_int_param(const EVP_PKEY* pkey, const char* key_name, int* out)
{
  OSSL_PARAM params[2];
...
int
EVP_PKEY_get_size_t_param(const EVP_PKEY* pkey,
                          const char* key_name,
                          size_t* out)
{
  OSSL_PARAM params[2];
...
int
EVP_PKEY_set_int_param(EVP_PKEY* pkey, const char* key_name, int in)
{
  OSSL_PARAM params[2];
...
int
EVP_PKEY_set_size_t_param(EVP_PKEY* pkey, const char* key_name, size_t in)
{
  OSSL_PARAM params[2];
...
int
EVP_PKEY_set_bn_param(EVP_PKEY* pkey, const char* key_name, const BIGNUM* bn)
{
  OSSL_PARAM params[2];
  unsigned char buffer[2048];
  int bsize = 0;
...
int
EVP_PKEY_set_utf8_string_param(EVP_PKEY* pkey,
                               const char* key_name,
                               const char* str)
{
  OSSL_PARAM params[2];
...
int
EVP_PKEY_set_octet_string_param(EVP_PKEY* pkey,
                                const char* key_name,
...
const OSSL_PARAM*
EVP_PKEY_settable_params(const EVP_PKEY* pkey)
{
...
int
EVP_PKEY_set_params(EVP_PKEY* pkey, OSSL_PARAM params[])
{
  if (pkey != NULL) {
    if (evp_pkey_is_provided(pkey)) {
      pkey->dirty_cnt++;
...
const OSSL_PARAM*
EVP_PKEY_gettable_params(const EVP_PKEY* pkey)
{
  return (pkey != NULL && evp_pkey_is_provided(pkey))
...
int
EVP_PKEY_get_params(const EVP_PKEY* pkey, OSSL_PARAM params[])
{
  if (pkey != NULL) {
    if (evp_pkey_is_provided(pkey))
...
#ifndef FIPS_MODULE
int
EVP_PKEY_get_ec_point_conv_form(const EVP_PKEY* pkey)
{
  char name[80];
  size_t name_len;
...
int
EVP_PKEY_get_field_type(const EVP_PKEY* pkey)
{
  char fstr[80];
  size_t fstrlen;
  ...
  if (!EVP_PKEY_get_utf8_string_param(
        pkey, OSSL_PKEY_PARAM_EC_FIELD_TYPE, fstr, sizeof(fstr), &fstrlen))
    return 0;
