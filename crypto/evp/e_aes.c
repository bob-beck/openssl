...
static int aesni_ocb_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key,
  ...
  if (key != NULL) {
     ...
     */
    if (iv == NULL && octx->iv_set) iv = octx->iv;
    if (iv) {
...
static int s390x_aes_gcm_aad(S390X_AES_GCM_CTX* ctx, const unsigned char* aad,
  ...
  if (ctx->kma.param.tpcl) return -2;

  alen = ctx->kma.param.taadl + len;
  if (alen > (U64(1) << 61) || (sizeof(len) == 8 && alen < len)) return -1;
...
static int s390x_aes_gcm_ctrl(EVP_CIPHER_CTX* c, int type, int arg, void* ptr) {
  ...
  switch (type) {
    ...
    case EVP_CTRL_AEAD_SET_IVLEN:
      ...
      if (arg != 12) {
        ...
        if (gctx->ivlen == 12 || len > S390X_gcm_ivpadlen(gctx->ivlen)) {
          if (gctx->iv != c->iv) OPENSSL_free(gctx->iv);

          if ((gctx->iv = OPENSSL_malloc(len)) == NULL) return 0;
        }
        /* Add padding. */
    ...
    case EVP_CTRL_AEAD_GET_TAG:
      enc = EVP_CIPHER_CTX_is_encrypting(c);
      if (arg <= 0 || arg > 16 || !enc || gctx->taglen < 0) return 0;
    ...
    case EVP_CTRL_GCM_IV_GEN:
      if (gctx->iv_gen == 0 || gctx->key_set == 0) return 0;
    ...
    case EVP_CTRL_GCM_SET_IV_INV:
      enc = EVP_CIPHER_CTX_is_encrypting(c);
      if (gctx->iv_gen == 0 || gctx->key_set == 0 || enc) return 0;
    ...
    case EVP_CTRL_AEAD_TLS1_AAD:
      ...
      if (arg != EVP_AEAD_TLS1_AAD_LEN) return 0;
    ...
    case EVP_CTRL_COPY:
      ...
      if (gctx->iv == c->iv) {
        ...
        if ((gctx_out->iv = OPENSSL_malloc(len)) == NULL) return 0;
...
static int s390x_aes_gcm_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (in != NULL) {
    if (out == NULL) {
      if (s390x_aes_gcm_aad(gctx, in, len)) return -1;
    } else {
      if (s390x_aes_gcm(gctx, in, out, len)) return -1;
    }
    return len;
  } else {
    ...
    if (enc) {
      ...
      if (CRYPTO_memcmp(buf, gctx->kma.param.t.b, gctx->taglen)) return -1;
    }
    return 0;
  }
}
...
static int s390x_aes_ccm(S390X_AES_CCM_CTX* ctx, const unsigned char* in,
  ...
  if (enc) {
    ...
    if (ctx->aes.ccm.blocks > (1ULL << 61)) return -2; /* too much data */
  }
  ...
  if (enc) {
    /* mac-then-encrypt */
    if (len) s390x_kmac(in, len, ctx->aes.ccm.fc, &ctx->aes.ccm.kmac_param);
    if (rem) {
      for (i = 0; i < rem; i++) ctx->aes.ccm.kmac_param.icv.b[i] ^= in[len + i];
    ...
    if (len) s390x_kmac(out, len, ctx->aes.ccm.fc, &ctx->aes.ccm.kmac_param);
    if (rem) {
      for (i = 0; i < rem; i++)
...
static int s390x_aes_ccm_tls_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (enc) {
    if (s390x_aes_ccm(cctx, in, out, len, enc)) return -1;
...
static int s390x_aes_ccm_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (!cctx->aes.ccm.key_set) return -1;
  ...
  if (!cctx->aes.ccm.iv_set) return -1;
  ...
  if (out == NULL) {
    ...
    /* Update(): Process aad. */
    if (!cctx->aes.ccm.len_set && len) return -1;
  ...
  if (!enc && !cctx->aes.ccm.tag_set) return -1;
...
static int s390x_aes_ccm_ctrl(EVP_CIPHER_CTX* c, int type, int arg, void* ptr) {
  ...
  switch (type) {
    ...
    case EVP_CTRL_AEAD_TLS1_AAD:
      ...
      enc = EVP_CIPHER_CTX_is_encrypting(c);
      if (!enc) {
        if (len < cctx->aes.ccm.m) return 0;
    ...
    case EVP_CTRL_CCM_SET_IV_FIXED:
      if (arg != EVP_CCM_TLS_FIXED_IV_LEN) return 0;
    ...
    case EVP_CTRL_AEAD_GET_TAG:
      enc = EVP_CIPHER_CTX_is_encrypting(c);
      if (!enc || !cctx->aes.ccm.tag_set) return 0;
  ...
  switch (type) {
    ...
    case EVP_CTRL_AEAD_SET_IVLEN:
      ...
      if ((arg > EVP_MAX_IV_LENGTH) && (arg > gctx->ivlen)) {
        if (gctx->iv != c->iv) OPENSSL_free(gctx->iv);
        if ((gctx->iv = OPENSSL_malloc(arg)) == NULL) return 0;
      }
      gctx->ivlen = arg;
      return 1;

    case EVP_CTRL_AEAD_SET_TAG:
      if (arg <= 0 || arg > 16 || c->encrypt) return 0;
      memcpy(c->buf, ptr, arg);
      gctx->taglen = arg;
      return 1;

    case EVP_CTRL_AEAD_GET_TAG:
      if (arg <= 0 || arg > 16 || !c->encrypt || gctx->taglen < 0) return 0;
    ...
    case EVP_CTRL_GCM_SET_IV_INV:
      if (gctx->iv_gen == 0 || gctx->key_set == 0 || c->encrypt) return 0;
      memcpy(gctx->iv + gctx->ivlen - arg, ptr, arg);
      CRYPTO_gcm128_setiv(&gctx->gcm, gctx->iv, gctx->ivlen);
      gctx->iv_set = 1;
      return 1;
    ...
    case EVP_CTRL_COPY: {
      ...
      if (gctx->iv == c->iv)
        ...
        if ((gctx_out->iv = OPENSSL_malloc(gctx->ivlen)) == NULL) return 0;
...
static int aes_gcm_iv_generate(EVP_AES_GCM_CTX* gctx, int offset) {
  ...
  if (sz <= 0 || gctx->ivlen < 12) return 0;

  /* Use DRBG to generate random iv */
  if (RAND_bytes(gctx->iv + offset, sz) <= 0) return 0;
  return 1;
}
#endif /* FIPS_MODULE */
...
BLOCK_CIPHER_custom(NID_aes, 128, 1, 16, xts, XTS, XTS_FLAGS)
  ...
  switch (type) {
    ...
    case EVP_CTRL_AEAD_SET_TAG:
      if ((arg & 1) || arg < 4 || arg > 16) return 0;
      if (EVP_CIPHER_CTX_is_encrypting(c) && ptr) return 0;
      if (ptr) {
        cctx->tag_set = 1;
        memcpy(EVP_CIPHER_CTX_buf_noconst(c), ptr, arg);
      }
    ...
    case EVP_CTRL_AEAD_GET_TAG:
      if (!EVP_CIPHER_CTX_is_encrypting(c) || !cctx->tag_set) return 0;
...
static int aes_ccm_tls_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (CRYPTO_ccm128_setiv(ccm, ctx->iv, 15 - cctx->L, len)) return -1;
  /* Use saved AAD */
  ...
  if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
    ...
    if (cctx->str ? !CRYPTO_ccm128_decrypt_ccm64(ccm, in, out, len, cctx->str)
      ...
      if (CRYPTO_ccm128_tag(ccm, tag, cctx->M)) {
        if (!CRYPTO_memcmp(tag, in + len, cctx->M)) return (int)len;
      }
    }
    OPENSSL_cleanse(out, len);
    return -1;
...
static int aes_ccm_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  /* The tag must be set before actually decrypting data */
  if (!EVP_CIPHER_CTX_is_encrypting(ctx) && !cctx->tag_set) return -1;
...
static int aes_wrap_init_key(EVP_CIPHER_CTX* ctx, const unsigned char* key,
  ...
  if (key != NULL) {
    ...
    if (iv == NULL) wctx->iv = NULL;
  }
  if (iv != NULL) {
    if ((len = EVP_CIPHER_CTX_get_iv_length(ctx)) < 0) return 0;
...
static int aes_ocb_ctrl(EVP_CIPHER_CTX* c, int type, int arg, void* ptr) {
  ...
  switch (type) {
    ...
    case EVP_CTRL_AEAD_SET_TAG:
      ...
      if (arg != octx->taglen || EVP_CIPHER_CTX_is_encrypting(c)) return 0;
...
static int aes_ocb_cipher(EVP_CIPHER_CTX* ctx, unsigned char* out,
  ...
  if (!octx->iv_set) return -1;

  if (!octx->key_set) return -1;
  ...
  if (in != NULL) {
    ...
    if (*buf_len > 0) {
      ...
      in += remaining;
      if (out == NULL) {
        if (!CRYPTO_ocb128_aad(&octx->ocb, buf, AES_BLOCK_SIZE)) return -1;
      } else if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
      ...
      *buf_len = 0;
      if (out != NULL) out += AES_BLOCK_SIZE;
    }
    ...
    if (len != trailing_len) {
      if (out == NULL) {
        if (!CRYPTO_ocb128_aad(&octx->ocb, in, len - trailing_len)) return -1;
      } else if (EVP_CIPHER_CTX_is_encrypting(ctx)) {
    ...
    if (CRYPTO_ocb128_tag(&octx->ocb, octx->tag, 16) != 1) return -1;
    /* Don't reuse the IV */
    octx->iv_set = 0;
    return written_len;
  }
