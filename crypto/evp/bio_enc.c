...
static int enc_read(BIO* b, char* out, int outl) {
  ...
  while (outl > 0) {
    ...
    if (i <= 0) {
      ...
      if (i > ENC_MIN_CHUNK) i = ENC_MIN_CHUNK;
  ...
  switch (cmd) {
    ...
    again:
      while (ctx->buf_len != ctx->buf_off) {
         ...
         */
        if (i < 0 || (ctx->buf_len - ctx->buf_off) == pend) return i;
      }
    ...
    case BIO_CTRL_DUP:
      ...
      if (dctx->cipher == NULL) return 0;
      ret = EVP_CIPHER_CTX_copy(dctx->cipher, ctx->cipher);
      if (ret) BIO_set_init(dbio, 1);
      break;
    default:
      ret = BIO_ctrl(next, cmd, num, ptr);
      break;
...
int BIO_set_cipher(BIO* b, const EVP_CIPHER* c, const unsigned char* k,
  ...
  if (!EVP_CipherInit_ex(ctx->cipher, c, NULL, k, i, e)) return 0;
