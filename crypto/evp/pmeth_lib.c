...
static int
evp_pkey_ctx_store_cached_data(EVP_PKEY_CTX* ctx,
                               int keytype,
                               int optype,
                               int cmd,
                               const char* name,
                               ...
                               size_t data_len);
static void
evp_pkey_ctx_free_cached_data(EVP_PKEY_CTX* ctx, int cmd, const char* name);
static void
evp_pkey_ctx_free_all_cached_data(EVP_PKEY_CTX* ctx);

typedef const EVP_PKEY_METHOD* (*pmeth_fn)(void);
typedef int
sk_cmp_fn_type(const char* const* a, const char* const* b);
...
static int
pmeth_func_cmp(const EVP_PKEY_METHOD* const* a, pmeth_fn const* b)
{
...
static EVP_PKEY_CTX*
int_ctx_new(OSSL_LIB_CTX* libctx,
            EVP_PKEY* pkey,
            ENGINE* e,
            const char* keytype,
...
EVP_PKEY_CTX*
EVP_PKEY_CTX_new_from_name(OSSL_LIB_CTX* libctx,
                           const char* name,
                           const char* propquery)
{
...
EVP_PKEY_CTX*
EVP_PKEY_CTX_new_from_pkey(OSSL_LIB_CTX* libctx,
                           EVP_PKEY* pkey,
                           const char* propquery)
{
...
void
evp_pkey_ctx_free_old_ops(EVP_PKEY_CTX* ctx)
{
...
EVP_PKEY_CTX*
EVP_PKEY_CTX_dup(const EVP_PKEY_CTX* pctx)
{
  EVP_PKEY_CTX* rctx;
...
int
EVP_PKEY_meth_remove(const EVP_PKEY_METHOD* pmeth)
{
...
int
EVP_PKEY_CTX_is_a(EVP_PKEY_CTX* ctx, const char* keytype)
{
#ifndef FIPS_MODULE
...
int
EVP_PKEY_CTX_set_params(EVP_PKEY_CTX* ctx, const OSSL_PARAM* params)
{
...
int
EVP_PKEY_CTX_get_params(EVP_PKEY_CTX* ctx, OSSL_PARAM* params)
{
  switch (evp_pkey_ctx_state(ctx)) {
    case EVP_PKEY_STATE_PROVIDER:
      if (EVP_PKEY_CTX_IS_DERIVE_OP(ctx) && ctx->op.kex.exchange != NULL &&
...
#ifndef FIPS_MODULE
const OSSL_PARAM*
EVP_PKEY_CTX_gettable_params(const EVP_PKEY_CTX* ctx)
{
  void* provctx;
...
const OSSL_PARAM*
EVP_PKEY_CTX_settable_params(const EVP_PKEY_CTX* ctx)
{
  void* provctx;
...
int
EVP_PKEY_CTX_get_signature_md(EVP_PKEY_CTX* ctx, const EVP_MD** md)
{
  ...
  if (ctx->op.sig.algctx == NULL)
    return EVP_PKEY_CTX_ctrl(
      ctx, -1, EVP_PKEY_OP_TYPE_SIG, EVP_PKEY_CTRL_GET_MD, 0, (void*)(md));

  *p++ = OSSL_PARAM_construct_utf8_string(
    OSSL_SIGNATURE_PARAM_DIGEST, name, sizeof(name));
...
static int
evp_pkey_ctx_set_md(EVP_PKEY_CTX* ctx,
                    const EVP_MD* md,
                    int fallback,
                    const char* param,
                    int op,
...
int
EVP_PKEY_CTX_set_tls1_prf_md(EVP_PKEY_CTX* ctx, const EVP_MD* md)
{
  return evp_pkey_ctx_set_md(ctx,
                             md,
                             ctx->op.kex.algctx == NULL,
...
static int
evp_pkey_ctx_set1_octet_string(EVP_PKEY_CTX* ctx,
                               int fallback,
                               const char* param,
                               int op,
                               int ctrl,
...
static int
evp_pkey_ctx_add1_octet_string(EVP_PKEY_CTX* ctx,
                               int fallback,
                               const char* param,
                               int op,
                               int ctrl,
                               ...
                               int datalen)
{
  ...
  if (gettables == NULL || OSSL_PARAM_locate_const(gettables, param) == NULL)
    return evp_pkey_ctx_set1_octet_string(
      ctx, fallback, param, op, ctrl, data, datalen);
...
int
EVP_PKEY_CTX_set1_tls1_prf_secret(EVP_PKEY_CTX* ctx,
                                  const unsigned char* sec,
                                  int seclen)
{
  return evp_pkey_ctx_set1_octet_string(ctx,
                                        ctx->op.kex.algctx == NULL,
...
int
EVP_PKEY_CTX_add1_tls1_prf_seed(EVP_PKEY_CTX* ctx,
...
int
EVP_PKEY_CTX_set_hkdf_md(EVP_PKEY_CTX* ctx, const EVP_MD* md)
{
  return evp_pkey_ctx_set_md(ctx,
                             md,
                             ctx->op.kex.algctx == NULL,
...
int
EVP_PKEY_CTX_set1_hkdf_salt(EVP_PKEY_CTX* ctx,
...
int
EVP_PKEY_CTX_set1_hkdf_key(EVP_PKEY_CTX* ctx,
...
int
EVP_PKEY_CTX_add1_hkdf_info(EVP_PKEY_CTX* ctx,
                            const unsigned char* info,
                            int infolen)
{
  return evp_pkey_ctx_add1_octet_string(ctx,
                                        ctx->op.kex.algctx == NULL,
                                        OSSL_KDF_PARAM_INFO,
                                        EVP_PKEY_OP_DERIVE,
...
int
EVP_PKEY_CTX_set_hkdf_mode(EVP_PKEY_CTX* ctx, int mode)
{
  ...
  if (ctx->op.kex.algctx == NULL)
    return EVP_PKEY_CTX_ctrl(
      ctx, -1, EVP_PKEY_OP_DERIVE, EVP_PKEY_CTRL_HKDF_MODE, mode, NULL);
  /* end of legacy support */
...
int
EVP_PKEY_CTX_set1_pbe_pass(EVP_PKEY_CTX* ctx, const char* pass, int passlen)
{
  return evp_pkey_ctx_set1_octet_string(ctx,
                                        ctx->op.kex.algctx == NULL,
...
int
EVP_PKEY_CTX_set1_scrypt_salt(EVP_PKEY_CTX* ctx,
                              const unsigned char* salt,
                              int saltlen)
{
  return evp_pkey_ctx_set1_octet_string(ctx,
...
static int
evp_pkey_ctx_set_uint64(EVP_PKEY_CTX* ctx,
                        const char* param,
                        int op,
                        int ctrl,
                        uint64_t val)
...
int
EVP_PKEY_CTX_set_scrypt_N(EVP_PKEY_CTX* ctx, uint64_t n)
{
  return evp_pkey_ctx_set_uint64(ctx,
...
int
EVP_PKEY_CTX_set_scrypt_r(EVP_PKEY_CTX* ctx, uint64_t r)
{
  return evp_pkey_ctx_set_uint64(ctx,
...
int
EVP_PKEY_CTX_set_scrypt_p(EVP_PKEY_CTX* ctx, uint64_t p)
{
  return evp_pkey_ctx_set_uint64(ctx,
...
int
EVP_PKEY_CTX_set_scrypt_maxmem_bytes(EVP_PKEY_CTX* ctx, uint64_t maxmem_bytes)
{
  return evp_pkey_ctx_set_uint64(ctx,
...
int
EVP_PKEY_CTX_set_mac_key(EVP_PKEY_CTX* ctx,
                         const unsigned char* key,
                         int keylen)
{
  return evp_pkey_ctx_set1_octet_string(ctx,
...
int
EVP_PKEY_CTX_set_kem_op(EVP_PKEY_CTX* ctx, const char* op)
{
...
int
EVP_PKEY_CTX_set1_id(EVP_PKEY_CTX* ctx, const void* id, int len)
{
  return EVP_PKEY_CTX_ctrl(
    ctx, -1, -1, EVP_PKEY_CTRL_SET1_ID, (int)len, (void*)(id));
}
...
{
  return EVP_PKEY_CTX_ctrl(
    ctx, -1, -1, EVP_PKEY_CTRL_GET1_ID_LEN, 0, (void*)id_len);
}

static int
evp_pkey_ctx_ctrl_int(EVP_PKEY_CTX* ctx,
                      int keytype,
                      int optype,
                      int cmd,
                      int p1,
...
int
EVP_PKEY_CTX_ctrl(EVP_PKEY_CTX* ctx,
                  int keytype,
                  int optype,
                  int cmd,
                  int p1,
...
int
EVP_PKEY_CTX_ctrl_uint64(EVP_PKEY_CTX* ctx,
                         int keytype,
                         int optype,
                         int cmd,
                         uint64_t value)
...
static int
evp_pkey_ctx_ctrl_str_int(EVP_PKEY_CTX* ctx,
                          const char* name,
                          const char* value)
{
  int ret = 0;
...
EVP_PKEY_CTX_ctrl_str(EVP_PKEY_CTX* ctx, const char* name, const char* value)
{
  ...
  /* If unsupported, we don't want that reported here */
  ERR_set_mark();
  ret = evp_pkey_ctx_store_cached_data(
    ctx, -1, -1, -1, name, value, strlen(value) + 1);
  if (ret == -2) {
    ERR_pop_to_mark();
  } else {
    ERR_clear_last_mark();
...
static int
decode_cmd(int cmd, const char* name)
{
  if (cmd == -1) {
    /*
...
evp_pkey_ctx_use_cached_data(EVP_PKEY_CTX* ctx)
{
  ...
  if (ret && ctx->cached_parameters.dist_id_set) {
    ...
    if (name != NULL)
      ...
      ret = evp_pkey_ctx_ctrl_int(
        ctx, -1, ctx->operation, EVP_PKEY_CTRL_SET1_ID, (int)len, (void*)val);
  }
...
const char*
EVP_PKEY_CTX_get0_propq(const EVP_PKEY_CTX* ctx)
{
  return ctx->propquery;
}

const OSSL_PROVIDER*
EVP_PKEY_CTX_get0_provider(const EVP_PKEY_CTX* ctx)
{
  if (EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx)) {
...
int
EVP_PKEY_CTX_hex2ctrl(EVP_PKEY_CTX* ctx, int cmd, const char* hex)
{
  unsigned char* bin;
  long binlen;
  int rv = -1;
...
int
EVP_PKEY_CTX_md(EVP_PKEY_CTX* ctx, int optype, int cmd, const char* md)
{
  const EVP_MD* m;
...
void*
EVP_PKEY_CTX_get_data(const EVP_PKEY_CTX* ctx)
{
  return ctx->data;
}
...
void
EVP_PKEY_meth_set_digestverify(EVP_PKEY_METHOD* pmeth,
...
void
EVP_PKEY_meth_set_digest_custom(EVP_PKEY_METHOD* pmeth,
...
void
EVP_PKEY_meth_get_copy(const EVP_PKEY_METHOD* pmeth,
...
void
EVP_PKEY_meth_get_paramgen(const EVP_PKEY_METHOD* pmeth,
...
void
EVP_PKEY_meth_get_sign(const EVP_PKEY_METHOD* pmeth,
...
void
EVP_PKEY_meth_get_verify(const EVP_PKEY_METHOD* pmeth,
...
void
EVP_PKEY_meth_get_verifyctx(const EVP_PKEY_METHOD* pmeth,
...
void
EVP_PKEY_meth_get_encrypt(const EVP_PKEY_METHOD* pmeth,
...
void
EVP_PKEY_meth_get_decrypt(const EVP_PKEY_METHOD* pmeth,
...
void
EVP_PKEY_meth_get_digest_custom(const EVP_PKEY_METHOD* pmeth,
