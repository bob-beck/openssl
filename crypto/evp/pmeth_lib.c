...
static const EVP_PKEY_METHOD* evp_pkey_meth_find_added_by_application(
    int type) {
  if (app_pkey_methods != NULL) {
    ...
    if (idx >= 0) return sk_EVP_PKEY_METHOD_value(app_pkey_methods, idx);
  }
  return NULL;
}
...
const EVP_PKEY_METHOD* EVP_PKEY_meth_find(int type) {
  ...
  if ((t = evp_pkey_meth_find_added_by_application(type)) != NULL) return t;

  tmp.pkey_id = type;
  t = &tmp;
  ret = OBJ_bsearch_pmeth_func(&t, standard_methods,
                               OSSL_NELEM(standard_methods));
  if (ret == NULL || *ret == NULL) return NULL;
  return (**ret)();
}
...
int evp_pkey_ctx_state(const EVP_PKEY_CTX* ctx) {
  if (ctx->operation == EVP_PKEY_OP_UNDEFINED) return EVP_PKEY_STATE_UNKNOWN;
...
common:
  ...
  if ((ret == NULL || pmeth == NULL) && e != NULL) ENGINE_finish(e);
#endif
...
void EVP_PKEY_CTX_free(EVP_PKEY_CTX* ctx) {
  if (ctx == NULL) return;
  if (ctx->pmeth && ctx->pmeth->cleanup) ctx->pmeth->cleanup(ctx);
...
void EVP_PKEY_meth_get0_info(int* ppkey_id, int* pflags,
  ...
  if (ppkey_id) *ppkey_id = meth->pkey_id;
  if (pflags) *pflags = meth->flags;
}
...
void EVP_PKEY_meth_free(EVP_PKEY_METHOD* pmeth) {
  if (pmeth && (pmeth->flags & EVP_PKEY_FLAG_DYNAMIC)) OPENSSL_free(pmeth);
}
...
EVP_PKEY_CTX* EVP_PKEY_CTX_dup(const EVP_PKEY_CTX* pctx) {
  ...
  if (pctx->pkey != NULL && !EVP_PKEY_up_ref(pctx->pkey)) goto err;
  ...
  if (pctx->propquery != NULL) {
    ...
    if (rctx->propquery == NULL) goto err;
  }
  rctx->legacy_keytype = pctx->legacy_keytype;

  if (pctx->keymgmt != NULL) {
    if (!EVP_KEYMGMT_up_ref(pctx->keymgmt)) goto err;
  ...
  if (EVP_PKEY_CTX_IS_DERIVE_OP(pctx)) {
    if (pctx->op.kex.exchange != NULL) {
      ...
      if (!EVP_KEYEXCH_up_ref(rctx->op.kex.exchange)) goto err;
    }
    if (pctx->op.kex.algctx != NULL) {
      if (!ossl_assert(pctx->op.kex.exchange != NULL)) goto err;
    ...
    if (pctx->op.sig.signature != NULL) {
      ...
      if (!EVP_SIGNATURE_up_ref(rctx->op.sig.signature)) goto err;
    }
    if (pctx->op.sig.algctx != NULL) {
      if (!ossl_assert(pctx->op.sig.signature != NULL)) goto err;
    ...
    if (pctx->op.ciph.cipher != NULL) {
      ...
      if (!EVP_ASYM_CIPHER_up_ref(rctx->op.ciph.cipher)) goto err;
    }
    if (pctx->op.ciph.algctx != NULL) {
      if (!ossl_assert(pctx->op.ciph.cipher != NULL)) goto err;
    ...
    if (pctx->op.encap.kem != NULL) {
      ...
      if (!EVP_KEM_up_ref(rctx->op.encap.kem)) goto err;
    }
    if (pctx->op.encap.algctx != NULL) {
      if (!ossl_assert(pctx->op.encap.kem != NULL)) goto err;
  ...
  if (pctx->peerkey != NULL && !EVP_PKEY_up_ref(pctx->peerkey)) goto err;
...
size_t EVP_PKEY_meth_get_count(void) {
  ...
  if (app_pkey_methods) rv += sk_EVP_PKEY_METHOD_num(app_pkey_methods);
  return rv;
}

const EVP_PKEY_METHOD* EVP_PKEY_meth_get0(size_t idx) {
  if (idx < OSSL_NELEM(standard_methods)) return (standard_methods[idx])();
  if (app_pkey_methods == NULL) return NULL;
  idx -= OSSL_NELEM(standard_methods);
  if (idx >= (size_t)sk_EVP_PKEY_METHOD_num(app_pkey_methods)) return NULL;
...
int evp_pkey_ctx_set_params_strict(EVP_PKEY_CTX* ctx, OSSL_PARAM* params) {
  if (ctx == NULL || params == NULL) return 0;
  ...
  if (evp_pkey_ctx_is_provided(ctx)) {
    ...
    for (p = params; p->key != NULL; p++) {
      ...
      if (OSSL_PARAM_locate_const(settable, p->key) == NULL) return -2;
    }
  }
...
int evp_pkey_ctx_get_params_strict(EVP_PKEY_CTX* ctx, OSSL_PARAM* params) {
  ...
  if (evp_pkey_ctx_is_provided(ctx)) {
    ...
    for (p = params; p->key != NULL; p++) {
      /* Check the ctx actually understands this parameter */
      if (OSSL_PARAM_locate_const(gettable, p->key) == NULL) return -2;
    }
  }
...
int EVP_PKEY_CTX_get_signature_md(EVP_PKEY_CTX* ctx, const EVP_MD** md) {
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!EVP_PKEY_CTX_get_params(ctx, sig_md_params)) return 0;
...
static int evp_pkey_ctx_set_md(EVP_PKEY_CTX* ctx, const EVP_MD* md,
  ...
  if (fallback) return EVP_PKEY_CTX_ctrl(ctx, -1, op, ctrl, 0, (void*)(md));
...
static int evp_pkey_ctx_add1_octet_string(EVP_PKEY_CTX* ctx, int fallback,
  ...
  if (!EVP_PKEY_CTX_get_params(ctx, os_params)) return 0;

  /* This should not happen but check to be sure. */
  if (os_params[0].return_size == OSSL_PARAM_UNMODIFIED) return 0;

  info_alloc = os_params[0].return_size + datalen;
  if (info_alloc == 0) return 0;
  ...
  if (info_len > 0) {
    if (!EVP_PKEY_CTX_get_params(ctx, os_params)) goto error;
  }
...
static int evp_pkey_ctx_ctrl_int(EVP_PKEY_CTX* ctx, int keytype, int optype,
  ...
  switch (evp_pkey_ctx_state(ctx)) {
    ...
    case EVP_PKEY_STATE_LEGACY:
      ...
      if ((keytype != -1) && (ctx->pmeth->pkey_id != keytype)) return -1;

      ret = ctx->pmeth->ctrl(ctx, cmd, p1, p2);

      if (ret == -2) ERR_raise(ERR_LIB_EVP, EVP_R_COMMAND_NOT_SUPPORTED);
      break;
  }
  return ret;
}
...
int EVP_PKEY_CTX_ctrl(EVP_PKEY_CTX* ctx, int keytype, int optype, int cmd,
  ...
  if (ret == -2) {
     ...
     */
    if (ret < 1 || ctx->operation == EVP_PKEY_OP_UNDEFINED) return ret;
  }
  ...
  switch (cmd) {
    case EVP_PKEY_CTRL_SET1_ID:
      ...
      if (name != NULL) {
        ...
        if (ctx->cached_parameters.dist_id_name == NULL) return 0;
      }
      if (data_len > 0) {
...
const OSSL_PROVIDER* EVP_PKEY_CTX_get0_provider(const EVP_PKEY_CTX* ctx) {
  if (EVP_PKEY_CTX_IS_SIGNATURE_OP(ctx)) {
    ...
    if (ctx->keymgmt != NULL) return EVP_KEYMGMT_get0_provider(ctx->keymgmt);
  }
...
int EVP_PKEY_CTX_hex2ctrl(EVP_PKEY_CTX* ctx, int cmd, const char* hex) {
  ...
  if (binlen <= INT_MAX) rv = ctx->pmeth->ctrl(ctx, cmd, binlen, bin);
  OPENSSL_free(bin);
  return rv;
}
...
void EVP_PKEY_meth_get_paramgen(const EVP_PKEY_METHOD* pmeth,
                                ...
                                int (**pparamgen)(EVP_PKEY_CTX* ctx,
                                                  EVP_PKEY* pkey)) {
  if (pparamgen_init) *pparamgen_init = pmeth->paramgen_init;
  if (pparamgen) *pparamgen = pmeth->paramgen;
}
...
void EVP_PKEY_meth_get_keygen(const EVP_PKEY_METHOD* pmeth,
                              ...
                              int (**pkeygen)(EVP_PKEY_CTX* ctx,
                                              EVP_PKEY* pkey)) {
  if (pkeygen_init) *pkeygen_init = pmeth->keygen_init;
  if (pkeygen) *pkeygen = pmeth->keygen;
}
...
void EVP_PKEY_meth_get_sign(
    ...
    int (**psign)(EVP_PKEY_CTX* ctx, unsigned char* sig, size_t* siglen,
                  const unsigned char* tbs, size_t tbslen)) {
  if (psign_init) *psign_init = pmeth->sign_init;
  if (psign) *psign = pmeth->sign;
}
...
void EVP_PKEY_meth_get_verify(
    ...
    int (**pverify)(EVP_PKEY_CTX* ctx, const unsigned char* sig, size_t siglen,
                    const unsigned char* tbs, size_t tbslen)) {
  if (pverify_init) *pverify_init = pmeth->verify_init;
  if (pverify) *pverify = pmeth->verify;
}
...
void EVP_PKEY_meth_get_verify_recover(
    ...
    int (**pverify_recover)(EVP_PKEY_CTX* ctx, unsigned char* sig,
                            size_t* siglen, const unsigned char* tbs,
                            size_t tbslen)) {
  if (pverify_recover_init) *pverify_recover_init = pmeth->verify_recover_init;
  if (pverify_recover) *pverify_recover = pmeth->verify_recover;
}
...
void EVP_PKEY_meth_get_signctx(
    ...
    int (**psignctx)(EVP_PKEY_CTX* ctx, unsigned char* sig, size_t* siglen,
                     EVP_MD_CTX* mctx)) {
  if (psignctx_init) *psignctx_init = pmeth->signctx_init;
  if (psignctx) *psignctx = pmeth->signctx;
}
...
void EVP_PKEY_meth_get_verifyctx(
    ...
    int (**pverifyctx)(EVP_PKEY_CTX* ctx, const unsigned char* sig, int siglen,
                       EVP_MD_CTX* mctx)) {
  if (pverifyctx_init) *pverifyctx_init = pmeth->verifyctx_init;
  if (pverifyctx) *pverifyctx = pmeth->verifyctx;
}
...
void EVP_PKEY_meth_get_encrypt(
    ...
    int (**pencryptfn)(EVP_PKEY_CTX* ctx, unsigned char* out, size_t* outlen,
                       const unsigned char* in, size_t inlen)) {
  if (pencrypt_init) *pencrypt_init = pmeth->encrypt_init;
  if (pencryptfn) *pencryptfn = pmeth->encrypt;
}
...
void EVP_PKEY_meth_get_decrypt(
    ...
    int (**pdecrypt)(EVP_PKEY_CTX* ctx, unsigned char* out, size_t* outlen,
                     const unsigned char* in, size_t inlen)) {
  if (pdecrypt_init) *pdecrypt_init = pmeth->decrypt_init;
  if (pdecrypt) *pdecrypt = pmeth->decrypt;
}
...
void EVP_PKEY_meth_get_derive(
  ...
  if (pderive_init) *pderive_init = pmeth->derive_init;
  if (pderive) *pderive = pmeth->derive;
}
...
void EVP_PKEY_meth_get_ctrl(
  ...
  if (pctrl) *pctrl = pmeth->ctrl;
  if (pctrl_str) *pctrl_str = pmeth->ctrl_str;
}
...
void EVP_PKEY_meth_get_digestsign(
    ...
    int (**digestsign)(EVP_MD_CTX* ctx, unsigned char* sig, size_t* siglen,
                       const unsigned char* tbs, size_t tbslen)) {
  if (digestsign) *digestsign = pmeth->digestsign;
}
...
void EVP_PKEY_meth_get_digestverify(
    ...
    int (**digestverify)(EVP_MD_CTX* ctx, const unsigned char* sig,
                         size_t siglen, const unsigned char* tbs,
                         size_t tbslen)) {
  if (digestverify) *digestverify = pmeth->digestverify;
}

void EVP_PKEY_meth_get_check(const EVP_PKEY_METHOD* pmeth,
                             int (**pcheck)(EVP_PKEY* pkey)) {
  if (pcheck != NULL) *pcheck = pmeth->check;
}

void EVP_PKEY_meth_get_public_check(const EVP_PKEY_METHOD* pmeth,
                                    int (**pcheck)(EVP_PKEY* pkey)) {
  if (pcheck != NULL) *pcheck = pmeth->public_check;
}

void EVP_PKEY_meth_get_param_check(const EVP_PKEY_METHOD* pmeth,
                                   int (**pcheck)(EVP_PKEY* pkey)) {
  if (pcheck != NULL) *pcheck = pmeth->param_check;
}

void EVP_PKEY_meth_get_digest_custom(const EVP_PKEY_METHOD* pmeth,
                                     int (**pdigest_custom)(EVP_PKEY_CTX* ctx,
                                                            EVP_MD_CTX* mctx)) {
  if (pdigest_custom != NULL) *pdigest_custom = pmeth->digest_custom;
}
