...
const char* OSSL_EC_curve_nid2name(int nid) {
  ...
  for (i = 0; i < OSSL_NELEM(curve_list); i++) {
    if (curve_list[i].nid == nid) return curve_list[i].name;
  }
  return NULL;
}
...
int ossl_ec_curve_name2nid(const char* name) {
  ...
  if (name != NULL) {
    if ((nid = ossl_ec_curve_nist2nid_int(name)) != NID_undef) return nid;
...
const char* ossl_ec_curve_nid2nist_int(int nid) {
  ...
  for (i = 0; i < OSSL_NELEM(nist_curves); i++) {
    if (nist_curves[i].nid == nid) return nist_curves[i].name;
  }
  return NULL;
}
...
int ossl_ec_curve_nist2nid_int(const char* name) {
  size_t i;
  for (i = 0; i < OSSL_NELEM(nist_curves); i++) {
    if (strcmp(nist_curves[i].name, name) == 0) return nist_curves[i].nid;
  }
  return NID_undef;
}
