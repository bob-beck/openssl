...
#define DEFINE_BLOCK_CIPHERS(nid, flags)                                       \
  BLOCK_CIPHER_generic(                                                        \
    nid, 16, 16, cbc, cbc, CBC, flags | EVP_CIPH_FLAG_DEFAULT_ASN1)            \
    BLOCK_CIPHER_generic(                                                      \
      nid, 16, 0, ecb, ecb, ECB, flags | EVP_CIPH_FLAG_DEFAULT_ASN1)           \
      BLOCK_CIPHER_generic(                                                    \
        nid, 1, 16, ofb128, ofb, OFB, flags | EVP_CIPH_FLAG_DEFAULT_ASN1)      \
        BLOCK_CIPHER_generic(                                                  \
          nid, 1, 16, cfb128, cfb, CFB, flags | EVP_CIPH_FLAG_DEFAULT_ASN1)    \
          BLOCK_CIPHER_generic(nid, 1, 16, ctr, ctr, CTR, flags)

static int
sm4_init_key(EVP_CIPHER_CTX* ctx,
...
static int
sm4_cbc_cipher(EVP_CIPHER_CTX* ctx,
               unsigned char* out,
               ...
               size_t len)
{
  ...
  if (dat->stream.cbc)
    (*dat->stream.cbc)(
      in, out, len, &dat->ks.ks, ctx->iv, EVP_CIPHER_CTX_is_encrypting(ctx));
...
static int
sm4_cfb_cipher(EVP_CIPHER_CTX* ctx,
               unsigned char* out,
               const unsigned char* in,
               size_t len)
{
...
static int
sm4_ecb_cipher(EVP_CIPHER_CTX* ctx,
               unsigned char* out,
               const unsigned char* in,
               size_t len)
{
  ...
  if (dat->stream.ecb != NULL)
    (*dat->stream.ecb)(
      in, out, len, &dat->ks.ks, EVP_CIPHER_CTX_is_encrypting(ctx));
  else
...
static int
sm4_ofb_cipher(EVP_CIPHER_CTX* ctx,
               unsigned char* out,
               const unsigned char* in,
               size_t len)
{
...
static int
sm4_ctr_cipher(EVP_CIPHER_CTX* ctx,
               unsigned char* out,
               const unsigned char* in,
               size_t len)
{
