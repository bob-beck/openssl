...
EVP_CIPHER* EVP_CIPHER_meth_dup(const EVP_CIPHER* cipher) {
   ...
   */
  if (cipher->prov != NULL) return NULL;
...
void EVP_CIPHER_meth_free(EVP_CIPHER* cipher) {
  if (cipher == NULL || cipher->origin != EVP_ORIG_METH) return;

  evp_cipher_free_int(cipher);
}

int EVP_CIPHER_meth_set_iv_length(EVP_CIPHER* cipher, int iv_len) {
  if (cipher->iv_len != 0) return 0;
...
int EVP_CIPHER_meth_set_do_cipher(EVP_CIPHER* cipher,
                                  int (*do_cipher)(EVP_CIPHER_CTX* ctx,
                                                   ...
                                                   size_t inl)) {
  if (cipher->do_cipher != NULL) return 0;
...
int EVP_CIPHER_meth_set_cleanup(EVP_CIPHER* cipher,
  ...
  if (cipher->cleanup != NULL) return 0;
...
int EVP_CIPHER_meth_set_set_asn1_params(
  ...
  if (cipher->set_asn1_parameters != NULL) return 0;
...
int EVP_CIPHER_meth_set_get_asn1_params(
  ...
  if (cipher->get_asn1_parameters != NULL) return 0;
