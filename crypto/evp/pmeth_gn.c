...
static int
gen_init (EVP_PKEY_CTX *ctx, int operation)
{
  int ret = 0;
...
static int
ossl_callback_to_pkey_gencb (const OSSL_PARAM params[], void *arg)
{
...
int
EVP_PKEY_generate (EVP_PKEY_CTX *ctx, EVP_PKEY **ppkey)
{
  int ret = 0;
...
EVP_PKEY *
EVP_PKEY_new_mac_key (int type, ENGINE *e, const unsigned char *key,
                      int keylen)
{
...
static int
fromdata_init (EVP_PKEY_CTX *ctx, int operation)
{
...
int
EVP_PKEY_fromdata (EVP_PKEY_CTX *ctx, EVP_PKEY **ppkey, int selection,
                   OSSL_PARAM params[])
{
  void *keydata = NULL;
  EVP_PKEY *allocated_pkey = NULL;
...
const OSSL_PARAM *
EVP_PKEY_fromdata_settable (EVP_PKEY_CTX *ctx, int selection)
{
...
int
EVP_PKEY_todata (const EVP_PKEY *pkey, int selection, OSSL_PARAM **params)
{
  if (params == NULL)
    return 0;
...
static int
pkey_fake_import (void *fake_keydata, int ignored_selection,
...
int
EVP_PKEY_export (const EVP_PKEY *pkey, int selection, OSSL_CALLBACK *export_cb,
                 void *export_cbarg)
{
  if (pkey == NULL)
    {
