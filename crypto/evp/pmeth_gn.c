...
EVP_PKEY_generate(EVP_PKEY_CTX* ctx, EVP_PKEY** ppkey)
{
  ...
  if (ctx->pkey != NULL) {
     ...
     */
    ret = evp_keymgmt_gen_set_template(
      ctx->keymgmt, ctx->op.keymgmt.genctx, keydata);
  }
   ...
   */
  ret = ret && (evp_keymgmt_util_gen(*ppkey,
                                     ctx->keymgmt,
                                     ctx->op.keymgmt.genctx,
...
int
EVP_PKEY_fromdata(EVP_PKEY_CTX* ctx,
                  EVP_PKEY** ppkey,
                  int selection,
                  OSSL_PARAM params[])
{
...
static int
pkey_fake_import(void* fake_keydata,
...
int
EVP_PKEY_export(const EVP_PKEY* pkey,
                int selection,
                OSSL_CALLBACK* export_cb,
                void* export_cbarg)
{
