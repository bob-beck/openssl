...
static int gen_init(EVP_PKEY_CTX* ctx, int operation) {
  ...
  if (ctx == NULL) goto not_supported;

  evp_pkey_ctx_free_old_ops(ctx);
  ctx->operation = operation;

  if (ctx->keymgmt == NULL || ctx->keymgmt->gen_init == NULL) goto legacy;
...
legacy:
  ...
  switch (operation) {
    case EVP_PKEY_OP_PARAMGEN:
      ...
      break;
    case EVP_PKEY_OP_KEYGEN:
      if (ctx->pmeth->keygen_init != NULL) ret = ctx->pmeth->keygen_init(ctx);
      break;
  }
#endif
...
static int ossl_callback_to_pkey_gencb(const OSSL_PARAM params[], void* arg) {
  ...
  if (ctx->pkey_gencb == NULL) return 1; /* No callback?  That's fine */
...
int EVP_PKEY_generate(EVP_PKEY_CTX* ctx, EVP_PKEY** ppkey) {
  ...
  if (ppkey == NULL) return -1;

  if (ctx == NULL) goto not_supported;

  if ((ctx->operation & EVP_PKEY_OP_TYPE_GEN) == 0) goto not_initialized;

  if (*ppkey == NULL) *ppkey = allocated_pkey = EVP_PKEY_new();
  ...
  if (ctx->op.keymgmt.genctx == NULL) goto legacy;
  ...
  if (ctx->pkey != NULL) {
    ...
    if (tmp_keymgmt == NULL) goto not_supported;
    /*
  ...
  if (ret) evp_pkey_free_legacy(*ppkey);
#endif
...
legacy:
  ...
  if (ret <= 0) {
    if (allocated_pkey != NULL) *ppkey = NULL;
...
int EVP_PKEY_CTX_get_keygen_info(EVP_PKEY_CTX* ctx, int idx) {
  if (idx == -1) return ctx->keygen_info_count;
  if (idx < 0 || idx > ctx->keygen_info_count) return 0;
...
EVP_PKEY* EVP_PKEY_new_mac_key(int type, ENGINE* e, const unsigned char* key,
  ...
  if (!mac_ctx) return NULL;
  if (EVP_PKEY_keygen_init(mac_ctx) <= 0) goto merr;
  if (EVP_PKEY_CTX_set_mac_key(mac_ctx, key, keylen) <= 0) goto merr;
  if (EVP_PKEY_keygen(mac_ctx, &mac_key) <= 0) goto merr;
merr:
...
static int fromdata_init(EVP_PKEY_CTX* ctx, int operation) {
  if (ctx == NULL || ctx->keytype == NULL) goto not_supported;

  evp_pkey_ctx_free_old_ops(ctx);
  if (ctx->keymgmt == NULL) goto not_supported;

  ctx->operation = operation;
  return 1;

not_supported:
  if (ctx != NULL) ctx->operation = EVP_PKEY_OP_UNDEFINED;
  ERR_raise(ERR_LIB_EVP, EVP_R_OPERATION_NOT_SUPPORTED_FOR_THIS_KEYTYPE);
  return -2;
}
...
int EVP_PKEY_fromdata(EVP_PKEY_CTX* ctx, EVP_PKEY** ppkey, int selection,
  ...
  if (*ppkey == NULL) allocated_pkey = *ppkey = EVP_PKEY_new();
