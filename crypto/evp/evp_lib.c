...
int
evp_cipher_get_asn1_aead_params(EVP_CIPHER_CTX* c,
                                ASN1_TYPE* type,
                                evp_cipher_aead_asn1_params* asn1_params)
{
  int i = 0;
...
int
evp_cipher_set_asn1_aead_params(EVP_CIPHER_CTX* c,
                                ASN1_TYPE* type,
                                evp_cipher_aead_asn1_params* asn1_params)
{
  if (type == NULL || asn1_params == NULL)
...
int
EVP_CIPHER_get_block_size(const EVP_CIPHER* cipher)
{
  return (cipher == NULL) ? 0 : cipher->block_size;
}

int
EVP_CIPHER_CTX_get_block_size(const EVP_CIPHER_CTX* ctx)
{
           ...
           unsigned int inl)
{
  ...
  if (ctx->cipher->prov != NULL) {
    ...
    if (ctx->cipher->ccipher != NULL)
      ret = ctx->cipher->ccipher(ctx->algctx,
                                 out,
                                 &outl,
      ...
      ret = ctx->cipher->cupdate(ctx->algctx,
                                 out,
                                 &outl,
                                 inl + (blocksize == 1 ? 0 : blocksize),
                                 in,
      ...
      ret = ctx->cipher->cfinal(
        ctx->algctx, out, &outl, blocksize == 1 ? 0 : blocksize);
...
const EVP_CIPHER*
EVP_CIPHER_CTX_get0_cipher(const EVP_CIPHER_CTX* ctx)
{
  if (ctx == NULL)
    return NULL;
  return ctx->cipher;
...
int
EVP_CIPHER_CTX_is_encrypting(const EVP_CIPHER_CTX* ctx)
{
  return ctx->encrypt;
}

unsigned long
EVP_CIPHER_get_flags(const EVP_CIPHER* cipher)
{
...
int
EVP_CIPHER_get_iv_length(const EVP_CIPHER* cipher)
{
  return (cipher == NULL) ? 0 : cipher->iv_len;
}

int
EVP_CIPHER_CTX_get_iv_length(const EVP_CIPHER_CTX* ctx)
{
  if (ctx->cipher == NULL)
    return 0;
...
int
EVP_CIPHER_CTX_get_tag_length(const EVP_CIPHER_CTX* ctx)
{
  int ret;
  size_t v = 0;
  OSSL_PARAM params[2] = { OSSL_PARAM_END, OSSL_PARAM_END };
...
int
EVP_CIPHER_CTX_get_updated_iv(EVP_CIPHER_CTX* ctx, void* buf, size_t len)
{
  OSSL_PARAM params[2] = { OSSL_PARAM_END, OSSL_PARAM_END };
...
int
EVP_CIPHER_CTX_get_original_iv(EVP_CIPHER_CTX* ctx, void* buf, size_t len)
{
  OSSL_PARAM params[2] = { OSSL_PARAM_END, OSSL_PARAM_END };
...
int
EVP_CIPHER_CTX_get_num(const EVP_CIPHER_CTX* ctx)
{
  int ok;
...
int
EVP_CIPHER_CTX_set_num(EVP_CIPHER_CTX* ctx, int num)
{
  int ok;
...
int
EVP_CIPHER_CTX_get_key_length(const EVP_CIPHER_CTX* ctx)
{
  if (ctx->cipher == NULL)
    return 0;
...
int
EVP_CIPHER_get_nid(const EVP_CIPHER* cipher)
{
  return (cipher == NULL) ? NID_undef : cipher->nid;
}

int
EVP_CIPHER_CTX_get_nid(const EVP_CIPHER_CTX* ctx)
{
  return EVP_CIPHER_get_nid(ctx->cipher);
}

int
EVP_CIPHER_is_a(const EVP_CIPHER* cipher, const char* name)
{
  if (cipher == NULL)
    return 0;
  if (cipher->prov != NULL)
...
const char*
EVP_CIPHER_get0_name(const EVP_CIPHER* cipher)
{
...
const char*
EVP_CIPHER_get0_description(const EVP_CIPHER* cipher)
{
...
int
EVP_CIPHER_names_do_all(const EVP_CIPHER* cipher,
...
int
EVP_CIPHER_get_mode(const EVP_CIPHER* cipher)
{
  return EVP_CIPHER_get_flags(cipher) & EVP_CIPH_MODE;
}

int
EVP_MD_is_a(const EVP_MD* md, const char* name)
{
  if (md == NULL)
    return 0;
  if (md->prov != NULL)
...
int
EVP_MD_names_do_all(const EVP_MD* md,
                    void (*fn)(const char* name, void* data),
                    void* data)
{
  if (md->prov != NULL)
...
const OSSL_PROVIDER*
EVP_MD_get0_provider(const EVP_MD* md)
{
  return md->prov;
}
...
int
EVP_MD_get_pkey_type(const EVP_MD* md)
{
  return md->pkey_type;
}

int
EVP_MD_get_block_size(const EVP_MD* md)
{
  if (md == NULL) {
...
unsigned long
EVP_MD_get_flags(const EVP_MD* md)
{
  return md->flags;
}
...
int
EVP_MD_meth_get_input_blocksize(const EVP_MD* md)
{
  return md->block_size;
}
int
EVP_MD_meth_get_result_size(const EVP_MD* md)
{
  return md->md_size;
}
int
EVP_MD_meth_get_app_datasize(const EVP_MD* md)
{
  return md->ctx_size;
}
unsigned long
...
int (*EVP_MD_meth_get_ctrl(const EVP_MD* md))(EVP_MD_CTX* ctx,
                                              int cmd,
                                              int p1,
                                              void* p2)
{
...
EVP_MD*
EVP_MD_CTX_get1_md(EVP_MD_CTX* ctx)
{
  EVP_MD* md;
...
int
EVP_MD_CTX_get_size_ex(const EVP_MD_CTX* ctx)
{
  ...
  if (gettables != NULL &&
    ...
    if (EVP_MD_CTX_get_params(c, params) != 1 || sz > INT_MAX || sz == 0)
      return -1;
    return (int)sz;
  }
...
EVP_PKEY_CTX*
EVP_MD_CTX_get_pkey_ctx(const EVP_MD_CTX* ctx)
{
  return ctx->pctx;
}
...
void*
EVP_MD_CTX_get0_md_data(const EVP_MD_CTX* ctx)
{
  return ctx->md_data;
}
...
void
EVP_MD_CTX_set_update_fn(EVP_MD_CTX* ctx,
                         int (*update)(EVP_MD_CTX* ctx,
                                       const void* data,
                                       size_t count))
{
...
static int
evp_cipher_ctx_enable_use_bits(EVP_CIPHER_CTX* ctx, unsigned int enable)
{
...
int
EVP_PKEY_CTX_set_group_name(EVP_PKEY_CTX* ctx, const char* name)
{
  OSSL_PARAM params[] = { OSSL_PARAM_END, OSSL_PARAM_END };
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
    OSSL_PKEY_PARAM_GROUP_NAME, (char*)name, 0);
  return EVP_PKEY_CTX_set_params(ctx, params);
}

int
EVP_PKEY_CTX_get_group_name(EVP_PKEY_CTX* ctx, char* name, size_t namelen)
{
  OSSL_PARAM params[] = { OSSL_PARAM_END, OSSL_PARAM_END };
  OSSL_PARAM* p = params;
 ...
 */
static EVP_PKEY*
evp_pkey_keygen(OSSL_LIB_CTX* libctx,
                const char* name,
                const char* propq,
...
EVP_PKEY*
EVP_PKEY_Q_keygen(OSSL_LIB_CTX* libctx,
                  const char* propq,
                  const char* type,
                  ...)
{
...
EVP_PKEY_CTX_get_algor_params(EVP_PKEY_CTX* ctx, X509_ALGOR* alg)
{
  ...
  if (OSSL_PARAM_modified(&params[0])
    ...
    if (EVP_PKEY_CTX_get_params(ctx, params) &&
        OSSL_PARAM_modified(&params[0]) &&
        d2i_ASN1_TYPE(&type, (const unsigned char**)&derp, (long)derl) !=
          NULL) {
      /*
       * Don't free alg->parameter, see comment further up.
       * Worst case, alg->parameter gets assigned its own value.
