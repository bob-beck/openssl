...
int EVP_CIPHER_get_asn1_iv(EVP_CIPHER_CTX* ctx, ASN1_TYPE* type) {
  ...
  if (type != NULL) {
    ...
    if (!ossl_assert(l <= sizeof(iv))) return -1;
    i = ASN1_TYPE_get_octetstring(type, iv, l);
    if (i != (int)l) return -1;

    if (!EVP_CipherInit_ex(ctx, NULL, NULL, NULL, iv, -1)) return -1;
  }
  return i;
}
...
int evp_cipher_param_to_asn1_ex(EVP_CIPHER_CTX* c, ASN1_TYPE* type,
  ...
  if (c == NULL || c->cipher == NULL) goto err;
...
int evp_cipher_get_asn1_aead_params(EVP_CIPHER_CTX* c, ASN1_TYPE* type,
  ...
  if (type == NULL || asn1_params == NULL) return 0;
...
int EVP_CIPHER_get_type(const EVP_CIPHER* cipher) {
  ...
  switch (nid) {
...
#else
    {
      ...
      if (OBJ_get0_data(otmp) == NULL) nid = NID_undef;
...
int evp_cipher_cache_constants(EVP_CIPHER* cipher) {
  ...
  if (ok) {
    ...
    cipher->flags = mode;
    if (aead) cipher->flags |= EVP_CIPH_FLAG_AEAD_CIPHER;
    if (custom_iv) cipher->flags |= EVP_CIPH_CUSTOM_IV;
    if (cts) cipher->flags |= EVP_CIPH_FLAG_CTS;
    if (multiblock) cipher->flags |= EVP_CIPH_FLAG_TLS1_1_MULTIBLOCK;
    if (cipher->ccipher != NULL) cipher->flags |= EVP_CIPH_FLAG_CUSTOM_CIPHER;
    if (randkey) cipher->flags |= EVP_CIPH_RAND_KEY;
    if (encrypt_then_mac) cipher->flags |= EVP_CIPH_FLAG_ENC_THEN_MAC;
...
EVP_CIPHER* EVP_CIPHER_CTX_get1_cipher(EVP_CIPHER_CTX* ctx) {
  ...
  if (ctx == NULL || ctx->cipher == NULL) return NULL;
  cipher = (EVP_CIPHER*)ctx->cipher;
  if (!EVP_CIPHER_up_ref(cipher)) return NULL;
  return cipher;
}
...
int EVP_CIPHER_CTX_get_iv_length(const EVP_CIPHER_CTX* ctx) {
  ...
  if (ctx->iv_len < 0) {
    ...
    /* Code below to be removed when legacy support is dropped. */
             ...
             0) {
      rv = EVP_CIPHER_CTX_ctrl((EVP_CIPHER_CTX*)ctx, EVP_CTRL_GET_IVLEN, 0,
                               &len);
      if (rv <= 0) return -1;
    }
    /*-
...
int EVP_CIPHER_CTX_get_key_length(const EVP_CIPHER_CTX* ctx) {
  ...
  if (ctx->key_len <= 0 && ctx->cipher->prov != NULL) {
    ...
    ok = evp_do_ciph_ctx_getparams(ctx->cipher, ctx->algctx, params);
    if (ok <= 0) return EVP_CTRL_RET_UNSUPPORTED;
...
const char* EVP_CIPHER_get0_name(const EVP_CIPHER* cipher) {
  if (cipher->type_name != NULL) return cipher->type_name;
#ifndef FIPS_MODULE
...
const char* EVP_CIPHER_get0_description(const EVP_CIPHER* cipher) {
  if (cipher->description != NULL) return cipher->description;
#ifndef FIPS_MODULE
...
int EVP_MD_is_a(const EVP_MD* md, const char* name) {
  if (md == NULL) return 0;
  if (md->prov != NULL) return evp_is_a(md->prov, md->name_id, NULL, name);
...
const char* EVP_MD_get0_description(const EVP_MD* md) {
  if (md->description != NULL) return md->description;
#ifndef FIPS_MODULE
...
const char* EVP_MD_get0_name(const EVP_MD* md) {
  ...
  if (md->type_name != NULL) return md->type_name;
#ifndef FIPS_MODULE
...
void EVP_MD_meth_free(EVP_MD* md) {
  if (md == NULL || md->origin != EVP_ORIG_METH) return;

  evp_md_free_int(md);
}

int EVP_MD_meth_set_input_blocksize(EVP_MD* md, int blocksize) {
  if (md->block_size != 0) return 0;

  md->block_size = blocksize;
  return 1;
}
int EVP_MD_meth_set_result_size(EVP_MD* md, int resultsize) {
  if (md->md_size != 0) return 0;
...
int EVP_MD_meth_set_flags(EVP_MD* md, unsigned long flags) {
  if (md->flags != 0) return 0;

  md->flags = flags;
  return 1;
}
int EVP_MD_meth_set_init(EVP_MD* md, int (*init)(EVP_MD_CTX* ctx)) {
  if (md->init != NULL) return 0;
...
int EVP_MD_meth_set_copy(EVP_MD* md,
  ...
  if (md->copy != NULL) return 0;

  md->copy = copy;
  return 1;
}
int EVP_MD_meth_set_cleanup(EVP_MD* md, int (*cleanup)(EVP_MD_CTX* ctx)) {
  if (md->cleanup != NULL) return 0;
...
EVP_MD* EVP_MD_CTX_get1_md(EVP_MD_CTX* ctx) {
  ...
  if (md == NULL || !EVP_MD_up_ref(md)) return NULL;
  return md;
}
...
int EVP_PKEY_CTX_get_group_name(EVP_PKEY_CTX* ctx, char* name, size_t namelen) {
  ...
  *p++ = OSSL_PARAM_construct_utf8_string(OSSL_PKEY_PARAM_GROUP_NAME, name,
                                          namelen);
  if (!EVP_PKEY_CTX_get_params(ctx, params)) return -1;
  return 1;
}
#endif /* !FIPS_MODULE */
...
int EVP_CIPHER_CTX_get_algor_params(EVP_CIPHER_CTX* ctx, X509_ALGOR* alg) {
  ...
  if (OSSL_PARAM_modified(&params[0]) && params[0].return_size != 0) i = 0;
  if (OSSL_PARAM_modified(&params[1]) && params[1].return_size != 0) i = 1;
  if (i < 0) goto err;
...
int EVP_CIPHER_CTX_get_algor(EVP_CIPHER_CTX* ctx, X509_ALGOR** alg) {
  ...
  if (EVP_CIPHER_CTX_get_params(ctx, params) <= 0) goto err;

  if (OSSL_PARAM_modified(&params[0])) aid_len = params[0].return_size;
  if (aid_len == 0) {
...
int EVP_PKEY_CTX_get_algor_params(EVP_PKEY_CTX* ctx, X509_ALGOR* alg) {
  ...
  if (!EVP_PKEY_CTX_get_params(ctx, params)) goto err;
...
int EVP_PKEY_CTX_get_algor(EVP_PKEY_CTX* ctx, X509_ALGOR** alg) {
  ...
  if (EVP_PKEY_CTX_get_params(ctx, params) <= 0) goto err;
