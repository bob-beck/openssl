...
int
EVP_SKEY_export(const EVP_SKEY* skey,
                int selection,
                ...
                void* export_cbarg)
{
  ...
  return evp_skeymgmt_export(
    skey->skeymgmt, skey->keydata, selection, export_cb, export_cbarg);
}

static EVP_SKEY*
evp_skey_alloc(EVP_SKEYMGMT* skeymgmt)
{
  EVP_SKEY* skey;
...
static EVP_SKEY*
evp_skey_alloc_fetch(OSSL_LIB_CTX* libctx,
...
EVP_SKEY*
EVP_SKEY_import(OSSL_LIB_CTX* libctx,
                const char* skeymgmtname,
                const char* propquery,
                int selection,
...
EVP_SKEY*
EVP_SKEY_generate(OSSL_LIB_CTX* libctx,
                  const char* skeymgmtname,
                  const char* propquery,
                  const OSSL_PARAM* params)
{
...
static int
get_secret_key(const OSSL_PARAM params[], void* arg)
{
...
int
EVP_SKEY_get0_raw_key(const EVP_SKEY* skey,
                      ...
                      size_t* len)
{
  ...
  return evp_skeymgmt_export(skey->skeymgmt,
                             skey->keydata,
...
EVP_SKEY*
EVP_SKEY_import_raw_key(OSSL_LIB_CTX* libctx,
                        const char* skeymgmtname,
                        unsigned char* key,
                        size_t keylen,
                        const char* propquery)
{
  ...
  params[0] = OSSL_PARAM_construct_octet_string(
    OSSL_SKEY_PARAM_RAW_BYTES, (void*)key, keylen);
  params[1] = OSSL_PARAM_construct_end();

  return EVP_SKEY_import(
    libctx, skeymgmtname, propquery, OSSL_SKEYMGMT_SELECT_SECRET_KEY, params);
}

int
EVP_SKEY_up_ref(EVP_SKEY* skey)
{
  int i;
...
const char*
EVP_SKEY_get0_key_id(const EVP_SKEY* skey)
{
  if (skey == NULL)
    return NULL;
...
const char*
EVP_SKEY_get0_skeymgmt_name(const EVP_SKEY* skey)
{
  if (skey == NULL)
    return NULL;
...
const char*
EVP_SKEY_get0_provider_name(const EVP_SKEY* skey)
{
  if (skey == NULL)
    return NULL;
...
int
EVP_SKEY_is_a(const EVP_SKEY* skey, const char* name)
{
  if (skey == NULL)
    return 0;
...
static int
transfer_cb(const OSSL_PARAM params[], void* arg)
{
...
EVP_SKEY*
EVP_SKEY_to_provider(EVP_SKEY* skey,
                     OSSL_LIB_CTX* libctx,
                     OSSL_PROVIDER* prov,
                     const char* propquery)
{
