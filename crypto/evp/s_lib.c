...
static EVP_SKEY* evp_skey_alloc(EVP_SKEYMGMT* skeymgmt) {
  ...
  if (!ossl_assert(skeymgmt != NULL)) return NULL;

  if ((skey = OPENSSL_zalloc(sizeof(*skey))) == NULL) return NULL;

  if (!CRYPTO_NEW_REF(&skey->references, 1)) goto err;
...
static EVP_SKEY* evp_skey_alloc_fetch(OSSL_LIB_CTX* libctx,
  ...
  if (skey == NULL) EVP_SKEYMGMT_free(skeymgmt);
...
int EVP_SKEY_up_ref(EVP_SKEY* skey) {
  ...
  if (CRYPTO_UP_REF(&skey->references, &i) <= 0) return 0;
...
EVP_SKEY* EVP_SKEY_to_provider(EVP_SKEY* skey, OSSL_LIB_CTX* libctx,
  ...
  if (skey->skeymgmt->name_id == skeymgmt->name_id &&
    ...
    if (!EVP_SKEY_up_ref(skey)) goto err;
  ...
  if (!EVP_SKEY_export(skey, ctx.selection, transfer_cb, &ctx)) goto err;

  if (ctx.keydata == NULL) goto err;
