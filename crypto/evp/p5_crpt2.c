                          ...
                          const char* propq)
{
  ...
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_KDF_PARAM_PASSWORD, (char*)pass, (size_t)passlen);
...
PKCS5_PBKDF2_HMAC(const char* pass,
                  ...
                  unsigned char* out)
{
  return ossl_pkcs5_pbkdf2_hmac_ex(
    pass, passlen, salt, saltlen, iter, digest, keylen, out, NULL, NULL);
}

int
PKCS5_PBKDF2_HMAC_SHA1(const char* pass,
                       int passlen,
                       const unsigned char* salt,
                       int saltlen,
                       int iter,
...
int
PKCS5_v2_PBE_keyivgen_ex(EVP_CIPHER_CTX* ctx,
                         const char* pass,
                         int passlen,
                         ASN1_TYPE* param,
                         const EVP_CIPHER* c,
                         ...
                         const char* propq)
{
   ...
   */
  if (OBJ_obj2txt(
        ciph_name, sizeof(ciph_name), pbe2->encryption->algorithm, 0) <= 0) {
...
int
PKCS5_v2_PBE_keyivgen(EVP_CIPHER_CTX* ctx,
                      const char* pass,
                      int passlen,
                      ASN1_TYPE* param,
                      const EVP_CIPHER* c,
                      ...
                      int en_de)
{
  return PKCS5_v2_PBE_keyivgen_ex(
    ctx, pass, passlen, param, c, md, en_de, NULL, NULL);
}
                            ...
                            const char* propq)
{
  ...
  if (!ossl_pkcs5_pbkdf2_hmac_ex(
        pass, passlen, salt, saltlen, iter, prfmd, keylen, key, libctx, propq))
    goto err;
...
PKCS5_v2_PBKDF2_keyivgen(EVP_CIPHER_CTX* ctx,
                         ...
                         int en_de)
{
  return PKCS5_v2_PBKDF2_keyivgen_ex(
    ctx, pass, passlen, param, c, md, en_de, NULL, NULL);
}
