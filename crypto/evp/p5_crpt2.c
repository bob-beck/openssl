...
int ossl_pkcs5_pbkdf2_hmac_ex(const char* pass, int passlen,
  ...
  if (salt == NULL && saltlen == 0) salt = (unsigned char*)empty;
  ...
  *p++ = OSSL_PARAM_construct_octet_string(OSSL_KDF_PARAM_PASSWORD, (char*)pass,
                                           (size_t)passlen);
  ...
  *p = OSSL_PARAM_construct_end();
  if (EVP_KDF_derive(kctx, out, keylen, params) != 1) rv = 0;
...
int PKCS5_v2_PBE_keyivgen_ex(EVP_CIPHER_CTX* ctx, const char* pass, int passlen,
  ...
  if (cipher == NULL) cipher = EVP_get_cipherbyname(ciph_name);
  ...
  if (!EVP_CipherInit_ex(ctx, cipher, NULL, NULL, NULL, en_de)) goto err;
...
int PKCS5_v2_PBKDF2_keyivgen_ex(EVP_CIPHER_CTX* ctx, const char* pass,
  ...
  if (prfmd == NULL) prfmd = EVP_get_digestbynid(hmac_md_nid);
  if (prfmd == NULL) {
    (void)ERR_clear_last_mark();
    ERR_raise(ERR_LIB_EVP, EVP_R_UNSUPPORTED_PRF);
    goto err;
