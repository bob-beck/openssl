...
static void* inner_evp_generic_fetch(
  ...
  if (meth_id == 0 ||
    ...
    if ((method = ossl_method_construct(methdata->libctx, operation_id, &prov,
                                        0 /* !force_cache */, &mcm,
                                        methdata)) != NULL) {
      /*
       ...
       */
      if (name_id == 0) name_id = ossl_namemap_name2num(namemap, name);
      if (name_id == 0) {
  ...
  if ((name_id != 0 || name != NULL) && method == NULL) {
    ...
    if (name == NULL) name = ossl_namemap_num2name(namemap, name_id, 0);
...
static int evp_set_parsed_default_properties(OSSL_LIB_CTX* libctx,
  ...
  if (plp != NULL && store != NULL) {
    ...
    if (mirrored) {
      if (ossl_global_properties_no_mirrored(libctx)) return 0;
    } else {
      /*
    ...
    if (strsz > 0) propstr = OPENSSL_malloc(strsz);
    if (propstr == NULL) {
      ERR_raise(ERR_LIB_EVP, ERR_R_INTERNAL_ERROR);
      return 0;
    }
...
char* evp_get_global_properties_str(OSSL_LIB_CTX* libctx, int loadconfig) {
  ...
  if (plp == NULL) return OPENSSL_strdup("");
...
int evp_is_a(OSSL_PROVIDER* prov, int number, const char* legacy_name,
  ...
  if (prov == NULL) number = ossl_namemap_name2num(namemap, legacy_name);
