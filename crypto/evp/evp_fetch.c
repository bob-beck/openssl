...
struct evp_method_data_st
{
  ...
  void* (*method_from_algorithm)(int name_id,
 ...
 */
static void*
get_tmp_evp_method_store(void* data)
{
  ...
  if (methdata->tmp_store == NULL) {
    ...
    OSSL_TRACE1(
      QUERY, "Allocating a new tmp_store %p\n", (void*)methdata->tmp_store);
  } else {
    OSSL_TRACE1(
      QUERY, "Using the existing tmp_store %p\n", (void*)methdata->tmp_store);
  }
  return methdata->tmp_store;
}

static void
dealloc_tmp_evp_method_store(void* store)
{
...
static OSSL_METHOD_STORE*
get_evp_method_store(OSSL_LIB_CTX* libctx)
{
  return ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_EVP_METHOD_STORE_INDEX);
}

static int
reserve_evp_method_store(void* store, void* data)
{
  struct evp_method_data_st* methdata = data;
...
static int
unreserve_evp_method_store(void* store, void* data)
{
  struct evp_method_data_st* methdata = data;
...
static uint32_t
evp_method_id(int name_id, unsigned int operation_id)
{
...
static void*
get_evp_method_from_store(void* store, const OSSL_PROVIDER** prov, void* data)
{
  struct evp_method_data_st* methdata = data;
  void* method = NULL;
  int name_id;
  ...
  if (!ossl_method_store_fetch(
        store, meth_id, methdata->propquery, prov, &method))
    return NULL;
  return method;
}

static int
put_evp_method_in_store(void* store,
                        void* method,
                        ...
                        void* data)
{
  ...
  OSSL_TRACE5(
    ...
    store,
    names,
    methdata->operation_id,
    meth_id,
    propdef ? propdef : "<null>");
  return ossl_method_store_add(store,
                               prov,
                               meth_id,
                               propdef,
                               method,
 ...
 */
static void*
construct_evp_method(const OSSL_ALGORITHM* algodef,
                     OSSL_PROVIDER* prov,
                     void* data)
{
  /*
...
static void
destruct_evp_method(void* method, void* data)
{
  struct evp_method_data_st* methdata = data;
...
inner_evp_generic_fetch(struct evp_method_data_st* methdata,
                        ...
                        int operation_id,
                        const char* name,
                        ossl_unused const char* properties,
                        ...
                        void (*free_method)(void*))
{
  ...
  if (meth_id == 0 ||
    ...
    if ((method = ossl_method_construct(methdata->libctx,
                                        operation_id,
                                        &prov,
                                        0 /* !force_cache */,
                                        &mcm,
...
void*
evp_generic_fetch(OSSL_LIB_CTX* libctx,
                  int operation_id,
                  const char* name,
 ...
 */
void*
evp_generic_fetch_from_prov(OSSL_PROVIDER* prov,
                            int operation_id,
                            const char* name,
                            const char* properties,
                            void* (*new_method)(int name_id,
...
static int
evp_set_parsed_default_properties(OSSL_LIB_CTX* libctx,
...
int
EVP_set_default_properties(OSSL_LIB_CTX* libctx, const char* propq)
{
  return evp_set_default_properties_int(libctx, propq, 1, 0);
}

static int
evp_default_properties_merge(OSSL_LIB_CTX* libctx,
                             const char* propq,
                             int loadconfig)
{
  OSSL_PROPERTY_LIST** plp = ossl_ctx_global_properties(libctx, loadconfig);
...
static int
evp_default_property_is_enabled(OSSL_LIB_CTX* libctx, const char* prop_name)
{
...
int
EVP_default_properties_enable_fips(OSSL_LIB_CTX* libctx, int enable)
{
...
char*
EVP_get1_default_properties(OSSL_LIB_CTX* libctx)
{
...
static void
filter_on_operation_id(int id, void* method, void* arg)
{
...
void
evp_generic_do_all(OSSL_LIB_CTX* libctx,
                   int operation_id,
                   void (*user_fn)(void* method, void* arg),
                   void* user_arg,
                   void* (*new_method)(int name_id,
                   ...
                   void (*free_method)(void*))
{
  ...
  if (methdata.tmp_store != NULL)
    ossl_method_store_do_all(
      methdata.tmp_store, &filter_on_operation_id, &data);
  ossl_method_store_do_all(
    get_evp_method_store(libctx), &filter_on_operation_id, &data);
  dealloc_tmp_evp_method_store(methdata.tmp_store);
}

int
evp_is_a(OSSL_PROVIDER* prov,
         int number,
...
int
evp_names_do_all(OSSL_PROVIDER* prov,
                 int number,
