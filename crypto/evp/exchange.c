...
static void *
evp_keyexch_from_algorithm (int name_id, const OSSL_ALGORITHM *algodef,
                            OSSL_PROVIDER *prov)
{
...
EVP_KEYEXCH *
EVP_KEYEXCH_fetch (OSSL_LIB_CTX *ctx, const char *algorithm,
...
EVP_KEYEXCH *
evp_keyexch_fetch_from_prov (OSSL_PROVIDER *prov, const char *algorithm,
                             const char *properties)
{
  ...
  for (iter = 1, provkey = NULL; iter < 3 && provkey == NULL; iter++)
    {
      ...
      switch (iter)
        {
        case 1:
          exchange = EVP_KEYEXCH_fetch (ctx->libctx, supported_exch,
                                        ctx->propquery);
          if (exchange != NULL)
...
int
EVP_PKEY_derive_set_peer_ex (EVP_PKEY_CTX *ctx, EVP_PKEY *peer,
                             int validate_peer)
{
  int ret = 0, check;
  void *provkey = NULL;
