...
static void
evp_keyexch_free(void* data)
{
  EVP_KEYEXCH_free(data);
}

static int
evp_keyexch_up_ref(void* data)
{
  return EVP_KEYEXCH_up_ref(data);
}

static EVP_KEYEXCH*
evp_keyexch_new(OSSL_PROVIDER* prov)
{
...
static void*
evp_keyexch_from_algorithm(int name_id,
...
void
EVP_KEYEXCH_free(EVP_KEYEXCH* exchange)
{
  int i;
...
OSSL_PROVIDER*
EVP_KEYEXCH_get0_provider(const EVP_KEYEXCH* exchange)
{
  return exchange->prov;
}

EVP_KEYEXCH*
EVP_KEYEXCH_fetch(OSSL_LIB_CTX* ctx,
...
int
EVP_PKEY_derive_init_ex(EVP_PKEY_CTX* ctx, const OSSL_PARAM params[])
{
  int ret;
  void* provkey = NULL;
  EVP_KEYEXCH* exchange = NULL;
...
int
EVP_PKEY_derive_set_peer_ex(EVP_PKEY_CTX* ctx,
                            EVP_PKEY* peer,
                            int validate_peer)
{
  int ret = 0, check;
...
EVP_PKEY_derive(EVP_PKEY_CTX* ctx, unsigned char* key, size_t* pkeylen)
{
  ...
  ret = ctx->op.kex.exchange->derive(
    ctx->op.kex.algctx, key, pkeylen, key != NULL ? *pkeylen : 0);
...
legacy:
  ...
  M_check_autoarg(ctx, key, pkeylen, EVP_F_EVP_PKEY_DERIVE) return ctx->pmeth
    ->derive(ctx, key, pkeylen);
}
...
const char*
EVP_KEYEXCH_get0_name(const EVP_KEYEXCH* keyexch)
{
  return keyexch->type_name;
}

const char*
EVP_KEYEXCH_get0_description(const EVP_KEYEXCH* keyexch)
{
  return keyexch->description;
}

int
EVP_KEYEXCH_is_a(const EVP_KEYEXCH* keyexch, const char* name)
{
  return keyexch != NULL &&
...
void
EVP_KEYEXCH_do_all_provided(OSSL_LIB_CTX* libctx,
...
int
EVP_KEYEXCH_names_do_all(const EVP_KEYEXCH* keyexch,
