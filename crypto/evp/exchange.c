...
legacy:
  ...
  if (ctx->pmeth->derive_init == NULL) return 1;
  ret = ctx->pmeth->derive_init(ctx);
  if (ret <= 0) ctx->operation = EVP_PKEY_OP_UNDEFINED;
  EVP_KEYMGMT_free(tmp_keymgmt);
  return ret;
#endif
}
...
int EVP_PKEY_derive_set_peer_ex(EVP_PKEY_CTX* ctx, EVP_PKEY* peer,
  ...
  if (tmp_keymgmt != NULL) /* A Coverity issue with up_ref/down_ref and free */
...
common:
  if (!EVP_PKEY_up_ref(peer)) return -1;
...
int EVP_PKEY_derive(EVP_PKEY_CTX* ctx, unsigned char* key, size_t* pkeylen) {
  ...
  if (ctx->op.kex.algctx == NULL) goto legacy;
...
const OSSL_PARAM* EVP_KEYEXCH_gettable_ctx_params(const EVP_KEYEXCH* keyexch) {
  ...
  if (keyexch == NULL || keyexch->gettable_ctx_params == NULL) return NULL;
...
const OSSL_PARAM* EVP_KEYEXCH_settable_ctx_params(const EVP_KEYEXCH* keyexch) {
  ...
  if (keyexch == NULL || keyexch->settable_ctx_params == NULL) return NULL;
  provctx = ossl_provider_ctx(EVP_KEYEXCH_get0_provider(keyexch));
