...
static void*
evp_mac_from_algorithm(int name_id,
...
EVP_MAC*
EVP_MAC_fetch(OSSL_LIB_CTX* libctx,
...
const OSSL_PROVIDER*
EVP_MAC_get0_provider(const EVP_MAC* mac)
{
  return mac->prov;
}

const OSSL_PARAM*
EVP_MAC_gettable_params(const EVP_MAC* mac)
{
  if (mac->gettable_params == NULL)
    return NULL;
...
const OSSL_PARAM*
EVP_MAC_gettable_ctx_params(const EVP_MAC* mac)
{
  void* alg;
...
const OSSL_PARAM*
EVP_MAC_settable_ctx_params(const EVP_MAC* mac)
{
  void* alg;
...
const OSSL_PARAM*
EVP_MAC_CTX_gettable_params(EVP_MAC_CTX* ctx)
{
  void* alg;
...
void
EVP_MAC_do_all_provided(OSSL_LIB_CTX* libctx,
