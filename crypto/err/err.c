...
static ERR_STRING_DATA* int_err_get_item(const ERR_STRING_DATA* d) {
  ...
  if (!CRYPTO_THREAD_read_lock(err_string_lock)) return NULL;
...
DEFINE_RUN_ONCE_STATIC(do_err_strings_init) {
  if (!OPENSSL_init_crypto(OPENSSL_INIT_BASE_ONLY, NULL)) return 0;
  err_string_lock = CRYPTO_THREAD_lock_new();
  if (err_string_lock == NULL) return 0;
#ifndef OPENSSL_NO_ERR
  int_error_hash =
...
#ifndef OPENSSL_NO_ERR
  if (ossl_err_load_ERR_strings() == 0) return 0;
...
void ossl_err_string_int(unsigned long e, const char* func, char* buf,
  ...
  if (ERR_SYSTEM_ERROR(e)) {
    if (openssl_strerror_r(r, rsbuf, sizeof(rsbuf))) rs = rsbuf;
  } else {
...
const char* ERR_reason_error_string(unsigned long e) {
   ...
   */
  if (ERR_SYSTEM_ERROR(e)) return NULL;
...
ERR_STATE* ossl_err_get_state_int(void) {
  ...
  state = CRYPTO_THREAD_get_local_ex(CRYPTO_THREAD_LOCAL_ERR_KEY,
                                     CRYPTO_THREAD_NO_CONTEXT);
  if (state == (ERR_STATE*)-1) return NULL;
...
void ERR_add_error_vdata(int num, va_list args) {
  ...
  if (!err_set_error_data_int(str, size, flags, 0)) OPENSSL_free(str);
}
