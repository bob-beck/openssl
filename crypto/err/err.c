...
static int
err_load_strings(const ERR_STRING_DATA* str);
#endif
...
get_error_values(ERR_GET_ACTION g,
                 const char** file,
                 int* line,
                 const char** func,
                 const char** data,
...
void
OSSL_ERR_STATE_free(ERR_STATE* state)
{
  int i;
 ...
 */
static void
err_patch(int lib, ERR_STRING_DATA* str)
{
...
int
ossl_err_load_ERR_strings(void)
{
#ifndef OPENSSL_NO_ERR
...
int
ERR_load_strings_const(const ERR_STRING_DATA* str)
{
#ifndef OPENSSL_NO_ERR
  if (ossl_err_load_ERR_strings() == 0)
    return 0;
...
unsigned long
ERR_get_error_all(const char** file,
                  int* line,
                  const char** func,
                  const char** data,
                  int* flags)
...
unsigned long
ERR_get_error_line_data(const char** file,
                        int* line,
                        const char** data,
                        int* flags)
{
...
unsigned long
ERR_peek_error_func(const char** func)
{
  return get_error_values(EV_PEEK, NULL, NULL, func, NULL, NULL);
}

unsigned long
ERR_peek_error_data(const char** data, int* flags)
{
  return get_error_values(EV_PEEK, NULL, NULL, NULL, data, flags);
}

unsigned long
ERR_peek_error_all(const char** file,
                   int* line,
                   const char** func,
                   const char** data,
                   int* flags)
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
unsigned long
ERR_peek_error_line_data(const char** file,
                         int* line,
                         const char** data,
                         int* flags)
{
...
unsigned long
ERR_peek_last_error_func(const char** func)
{
  return get_error_values(EV_PEEK_LAST, NULL, NULL, func, NULL, NULL);
}

unsigned long
ERR_peek_last_error_data(const char** data, int* flags)
{
  return get_error_values(EV_PEEK_LAST, NULL, NULL, NULL, data, flags);
}

unsigned long
ERR_peek_last_error_all(const char** file,
                        int* line,
                        const char** func,
                        const char** data,
                        int* flags)
...
void
ossl_err_string_int(unsigned long e, const char* func, char* buf, size_t len)
{
...
static void
err_delete_thread_state(void* unused)
{
  ...
  CRYPTO_THREAD_set_local_ex(
    CRYPTO_THREAD_LOCAL_ERR_KEY, CRYPTO_THREAD_NO_CONTEXT, NULL);
  OSSL_ERR_STATE_free(state);
}

#ifndef OPENSSL_NO_DEPRECATED_1_1_0
void
ERR_remove_thread_state(void* dummy)
{
}
#endif
...
ossl_err_get_state_int(void)
{
  ...
  if (state == NULL) {
    ...
    if (!ossl_init_thread_start(NULL, NULL, err_delete_thread_state) ||
        !CRYPTO_THREAD_set_local_ex(
          CRYPTO_THREAD_LOCAL_ERR_KEY, CRYPTO_THREAD_NO_CONTEXT, state)) {
      OSSL_ERR_STATE_free(state);
      CRYPTO_THREAD_set_local_ex(
        CRYPTO_THREAD_LOCAL_ERR_KEY, CRYPTO_THREAD_NO_CONTEXT, NULL);
      return NULL;
...
err_shelve_state(void** state)
{
  ...
  *state = CRYPTO_THREAD_get_local_ex(CRYPTO_THREAD_LOCAL_ERR_KEY,
  ...
  if (!CRYPTO_THREAD_set_local_ex(
        CRYPTO_THREAD_LOCAL_ERR_KEY, CRYPTO_THREAD_NO_CONTEXT, (ERR_STATE*)-1))
    return 0;
 ...
 */
void
err_unshelve_state(void* state)
{
  if (state != (void*)-1)
    CRYPTO_THREAD_set_local_ex(
      CRYPTO_THREAD_LOCAL_ERR_KEY, CRYPTO_THREAD_NO_CONTEXT, (ERR_STATE*)state);
}
...
static int
err_set_error_data_int(char* data, size_t size, int flags, int deallocate)
{
  ERR_STATE* es;
