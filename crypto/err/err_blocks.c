...
void ERR_vset_error(int lib, int reason, const char* fmt, va_list args) {
  ...
  if (fmt != NULL) {
    ...
    if (printed_len < 0) printed_len = 0;
    if (buf != NULL) buf[printed_len] = '\0';
    ...
    if (buf != NULL) flags = ERR_TXT_MALLOCED | ERR_TXT_STRING;
  }

  err_clear_data(es, es->top, 0);
  err_set_error(es, es->top, lib, reason);
  if (fmt != NULL) err_set_data(es, es->top, buf, buf_size, flags);
}
