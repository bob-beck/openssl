...
#define TOK_FUNC(NAME)                                                    \
  (sscanf(tok_begin,                                                      \
          ...
          tok[0], tok[1], tok[2]) == 2) {                                 \
    off = (tok[0][0] == '~') ? 1 : 0;                                     \
    if (sscanf(tok[0] + off, "%llx", &cap->NAME[0]) != 1) goto ret;       \
    if (off) cap->NAME[0] = ~cap->NAME[0];                                \
                                                                          \
    off = (tok[1][0] == '~') ? 1 : 0;                                     \
    if (sscanf(tok[1] + off, "%llx", &cap->NAME[1]) != 1) goto ret;       \
    if (off) cap->NAME[1] = ~cap->NAME[1];                                \
  }
...
void OPENSSL_cpuid_setup(void) {
  ...
  if (OPENSSL_s390xcap_P.stfle[0]) return;
...
#if defined(OSSL_IMPLEMENT_GETAUXVAL)
  {
    ...
    if (hwcap & HWCAP_S390_STFLE) OPENSSL_s390x_facilities();
...
#else
  {
    ...
    /* protection against missing store-facility-list-extended */
    if (sigsetjmp(ill_jmp, 1) == 0) OPENSSL_s390x_facilities();
...
static int parse_env(struct OPENSSL_s390xcap_st* cap, int* cex) {
  ...
  while (tok_begin != NULL) {
    ...
    if ((n = sscanf(tok_begin,
      ...
      for (i = 0; i < n; i++) {
        off = (tok[i][0] == '~') ? 1 : 0;
        if (sscanf(tok[i] + off, "%llx", &cap->stfle[i]) != 1) goto ret;
        if (off) cap->stfle[i] = ~cap->stfle[i];
      }
    }
    ...
    /* whitespace(ignored) or invalid tokens */
      ...
      while (*tok_begin != '\0') {
        if (!ossl_isspace(*tok_begin)) goto ret;
        tok_begin++;
      }
    }
