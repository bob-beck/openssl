...
static int
acpt_write(BIO* h, const char* buf, int num);
static int
acpt_read(BIO* h, char* buf, int size);
static int
acpt_puts(BIO* h, const char* str);
static long
acpt_ctrl(BIO* h, int cmd, long arg1, void* arg2);
static int
acpt_new(BIO* h);
static int
acpt_free(BIO* data);
  ...
  for (;;) {
    switch (c->state) {
      ...
      case ACPT_S_ACCEPT:
        ...
        s =
          BIO_accept_ex(c->accept_sock, &c->cache_peer_addr, c->accepted_mode);
