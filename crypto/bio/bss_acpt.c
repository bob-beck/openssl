...
static const BIO_METHOD methods_acceptp = {
    BIO_TYPE_ACCEPT, "socket accept", bwrite_conv, acpt_write, bread_conv,
    acpt_read,       acpt_puts,       NULL, /* connect_gets,         */
...
static int acpt_new(BIO* bi) {
  ...
  bi->flags = 0;
  if ((ba = BIO_ACCEPT_new()) == NULL) return 0;
  bi->ptr = (char*)ba;
  ...
  for (;;) {
    switch (c->state) {
      case ACPT_S_BEFORE:
        if (c->param_addr == NULL && c->param_serv == NULL) {
          ERR_raise_data(ERR_LIB_BIO, BIO_R_NO_ACCEPT_ADDR_OR_SERVICE_SPECIFIED,
                         "hostname=%s, service=%s", c->param_addr,
                         c->param_serv);
          goto exit_loop;
        }
      ...
      case ACPT_S_ACCEPT:
         ...
         */
        if (c->bio_chain != NULL) {
          if ((dbio = BIO_dup_chain(c->bio_chain)) == NULL) goto exit_loop;
          if (!BIO_push(dbio, bio)) goto exit_loop;
          bio = dbio;
        }
        if (BIO_push(b, bio) == NULL) goto exit_loop;
...
static long acpt_ctrl(BIO* b, int cmd, long num, void* ptr) {
  ...
  switch (cmd) {
      ...
      if (ptr != NULL) {
        if (num == 0) {
          ...
          ret = BIO_parse_hostserv(ptr, &data->param_addr, &data->param_serv,
                                   BIO_PARSE_PRIO_SERV);
          if (hold_serv != data->param_serv) OPENSSL_free(hold_serv);
          b->init = 1;
        } else if (num == 1) {
    ...
    case BIO_C_GET_FD:
      if (b->init) {
        ip = (int*)ptr;
        if (ip != NULL) *ip = data->accept_sock;
...
BIO* BIO_new_accept(const char* str) {
  ...
  if (BIO_set_accept_name(ret, str) > 0) return ret;
  BIO_free(ret);
  return NULL;
}
