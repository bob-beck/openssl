...
BIO* BIO_new_mem_buf(const void* buf, int len) {
  ...
  if ((ret = BIO_new(BIO_s_mem())) == NULL) return NULL;
...
static int mem_read(BIO* b, char* out, int outl) {
  ...
  if (b->flags & BIO_FLAGS_MEM_RDONLY) bm = bbm->buf;
...
static int mem_write(BIO* b, const char* in, int inl) {
  ...
  mem_buf_sync(b);
  if (BUF_MEM_grow_clean(bbm->buf, blen + inl) == 0) goto end;
...
static long mem_ctrl(BIO* b, int cmd, long num, void* ptr) {
  ...
  switch (cmd) {
    ...
    case BIO_C_GET_BUF_MEM_PTR:
      if (ptr != NULL) {
        if (!(b->flags & BIO_FLAGS_MEM_RDONLY)) mem_buf_sync(b);
        bm = bbm->buf;
        pptr = (char**)ptr;
        *pptr = (char*)bm;
      }
...
static int mem_gets(BIO* bp, char* buf, int size) {
  ...
  if (bp->flags & BIO_FLAGS_MEM_RDONLY) bm = bbm->buf;
