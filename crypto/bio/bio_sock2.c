 ...
 */
int
BIO_socket(int domain, int socktype, int protocol, int options)
{
  int sock = -1;
 ...
 */
int
BIO_connect(int sock, const BIO_ADDR* addr, int options)
{
  const int on = 1;
  ...
  if (options & BIO_SOCK_KEEPALIVE) {
    if (setsockopt(
          sock, SOL_SOCKET, SO_KEEPALIVE, (const void*)&on, sizeof(on)) != 0) {
      ERR_raise_data(
  ...
  if (options & BIO_SOCK_NODELAY) {
    if (setsockopt(
          sock, IPPROTO_TCP, TCP_NODELAY, (const void*)&on, sizeof(on)) != 0) {
      ERR_raise_data(
        ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
  ...
  if (options & BIO_SOCK_TFO) {
    ...
    if (connectx(sock,
      ...
      if (!BIO_sock_should_retry(-1)) {
        ERR_raise_data(
          ERR_LIB_SYS, get_last_socket_error(), "calling connectx()");
 ...
 */
int
BIO_bind(int sock, const BIO_ADDR* addr, int options)
{
  ...
  if (options & BIO_SOCK_REUSEADDR) {
    if (setsockopt(
          sock, SOL_SOCKET, SO_REUSEADDR, (const void*)&on, sizeof(on)) != 0) {
      ERR_raise_data(
        ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
  ...
  if (bind(sock, BIO_ADDR_sockaddr(addr), BIO_ADDR_sockaddr_size(addr)) != 0) {
    ERR_raise_data(
      ERR_LIB_SYS, get_last_socket_error() /* may be 0 */, "calling bind()");
 ...
 */
int
BIO_listen(int sock, const BIO_ADDR* addr, int options)
{
  int on = 1;
  int socktype;
  ...
  if (BIO_ADDR_family(addr) == AF_INET6) {
    ...
    if (setsockopt(
          sock, IPPROTO_IPV6, IPV6_V6ONLY, (const void*)&on, sizeof(on)) != 0) {
      ERR_raise_data(
        ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
...
#endif
    if (setsockopt(
          sock, IPPROTO_TCP, OSSL_TFO_SERVER_SOCKOPT, (void*)&q, sizeof(q)) <
        0) {
      ERR_raise_data(
        ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
      ERR_raise(ERR_LIB_BIO, BIO_R_UNABLE_TO_TFO);
 ...
 */
int
BIO_accept_ex(int accept_sock, BIO_ADDR* addr_, int options)
{
  socklen_t len;
  int accepted_sock;
  BIO_ADDR locaddr;
