...
BIO_sock_init(void)
{
  ...
  if (!wsa_init_done) {
    ...
    if (WSAStartup(0x0202, &wsa_state) != 0) {
      ERR_raise_data(
        ERR_LIB_SYS, get_last_socket_error(), "calling wsastartup()");
...
#define ARG arg
  ...
  if (i < 0)
    ERR_raise_data(
      ERR_LIB_SYS, get_last_socket_error(), "calling ioctlsocket()");
  return i;
}
...
BIO_get_accept_socket(char* host, int bind_mode)
{
  ...
  if (!BIO_listen(
        s, BIO_ADDRINFO_address(res), bind_mode ? BIO_SOCK_REUSEADDR : 0)) {
    BIO_closesocket(s);
    s = INVALID_SOCKET;
  }
              ...
              union BIO_sock_info_u* info)
{
  switch (type) {
    case BIO_SOCK_INFO_ADDRESS: {
      ...
      if (ret == -1) {
        ERR_raise_data(
          ERR_LIB_SYS, get_last_socket_error(), "calling getsockname()");
 ...
 */
int
BIO_socket_wait(int fd, int for_read, time_t max_time)
{
  ...
  return select(
    fd + 1, for_read ? &confds : NULL, for_read ? NULL : &confds, NULL, &tv);
#else
