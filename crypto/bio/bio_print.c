      ...
      case DP_S_CONV:
        switch (ch) {
          ...
          case 'i':
            ...
            if (!fmtint(&desc, value, 10, min, max, flags)) goto out;
            break;
          case 'X':
            flags |= DP_F_UP;
            /* FALLTHROUGH */
          ...
          case 'u':
            ...
            if (!fmtint(&desc, value, ch == 'o' ? 8 : (ch == 'u' ? 10 : 16),
                        min, max, flags))
              goto out;
            break;
#ifndef OPENSSL_SYS_UEFI
          case 'f':
            ...
            if (!fmtfp(&desc, fvalue, min, max, flags, F_FORMAT)) goto out;
            break;
          case 'E':
            flags |= DP_F_UP;
            /* fall through */
          case 'e':
            if (cflags == DP_C_LDOUBLE)
              ...
              fvalue = va_arg(args, double);
            if (!fmtfp(&desc, fvalue, min, max, flags, E_FORMAT)) goto out;
            break;
          case 'G':
            flags |= DP_F_UP;
            /* fall through */
          case 'g':
            if (cflags == DP_C_LDOUBLE)
              ...
              fvalue = va_arg(args, double);
            if (!fmtfp(&desc, fvalue, min, max, flags, G_FORMAT)) goto out;
            break;
#else
          case 'f':
          case 'E':
...
#endif
          case 'c':
            if (!doapr_outch(&desc, va_arg(args, int))) goto out;
            break;
          case 's':
            strvalue = va_arg(args, char*);
            if (max < 0) max = INT_MAX;
            if (!fmtstr(&desc, strvalue, flags, min, max)) goto out;
            break;
          case 'p':
            value = (size_t)va_arg(args, void*);
            if (!fmtint(&desc, value, 16, min, max, flags | DP_F_NUM)) goto out;
            break;
          case 'n':
            switch (cflags) {
...
#undef HANDLE_N
            ...
            break;
          case '%':
            if (!doapr_outch(&desc, ch)) goto out;
            break;
          case 'w':
...
out:
  ...
  if (buffer == NULL) {
    *truncated = (desc.currlen > desc.maxlen - 1);
    if (*truncated) desc.currlen = desc.maxlen - 1;
  }

  if (!doapr_outch(&desc, '\0')) ret = 0;
...
static int fmtstr(struct pr_desc* desc, const char* value, int flags, int min,
  ...
  while ((padlen > 0) && (max < 0 || cnt < max)) {
    if (!doapr_outch(desc, ' ')) return 0;
    --padlen;
    ++cnt;
  }
  while (strln > 0 && (max < 0 || cnt < max)) {
    if (!doapr_outch(desc, *value++)) return 0;
    --strln;
    ++cnt;
  }
  while ((padlen < 0) && (max < 0 || cnt < max)) {
...
static int fmtint(struct pr_desc* desc, int64_t value, int base, int min,
  ...
  if (flags & DP_F_NUM) {
    if (base == 8) prefix = oct_prefix;
    if (value != 0) {
      if (base == 16) prefix = flags & DP_F_UP ? "0X" : "0x";
    }
  }
  ...
  /* sign */
  if (signvalue)
    if (!doapr_outch(desc, signvalue)) return 0;
  ...
  if (zpadlen > 0) {
    while (zpadlen > 0) {
      if (!doapr_outch(desc, '0')) return 0;
      --zpadlen;
    }
  }
  /* digits */
  while (place > 0) {
    if (!doapr_outch(desc, convert[--place])) return 0;
  }
...
static int fmtfp(struct pr_desc* desc, LDOUBLE fvalue, int min, int max,
  ...
  if (style != F_FORMAT) {
    ...
    if (realstyle == E_FORMAT) ufvalue = tmpvalue;
  }
  ...
  if (realstyle == E_FORMAT) padlen -= 2 + eplace;
  zpadlen = max - fplace;
  if (zpadlen < 0) zpadlen = 0;
  ...
  if (signvalue && !doapr_outch(desc, signvalue)) return 0;

  while (iplace > 0) {
    if (!doapr_outch(desc, iconvert[--iplace])) return 0;
  }
  ...
  if (max > 0 || (flags & DP_F_NUM)) {
    ...
    while (fplace > 0) {
      if (!doapr_outch(desc, fconvert[--fplace])) return 0;
    }
  }
  while (zpadlen > 0) {
    if (!doapr_outch(desc, '0')) return 0;
  ...
  if (realstyle == E_FORMAT) {
    ...
    if ((flags & DP_F_UP) == 0)
      ...
      ech = 'E';
    if (!doapr_outch(desc, ech)) return 0;
    if (exp < 0) {
      if (!doapr_outch(desc, '-')) return 0;
    } else {
      if (!doapr_outch(desc, '+')) return 0;
    ...
    while (eplace > 0) {
      if (!doapr_outch(desc, econvert[--eplace])) return 0;
    }
  }
...
static int doapr_outch(struct pr_desc* desc, int c) {
  ...
  if (!ossl_assert(desc->sbuffer != NULL || desc->buffer != NULL)) return 0;

  /* |currlen| must always be <= |*maxlen| */
  if (!ossl_assert(desc->currlen <= desc->maxlen)) return 0;

  if (desc->buffer != NULL && desc->currlen == desc->maxlen) {
    if (desc->maxlen > INT_MAX - BUFFER_INC) return 0;

    desc->maxlen += BUFFER_INC;
    if (*(desc->buffer) == NULL) {
      if ((*(desc->buffer) = OPENSSL_malloc(desc->maxlen)) == NULL) return 0;
      if (desc->currlen > 0) {
        if (!ossl_assert(desc->sbuffer != NULL)) return 0;
