...
static int
fmtstr(struct pr_desc*, const char*, int, int, int);
static int
fmtint(struct pr_desc*, int64_t, int, int, int, int);
#ifndef OPENSSL_SYS_UEFI
static int
fmtfp(struct pr_desc*, LDOUBLE, int, int, int, int);
#endif
static int
doapr_outch(struct pr_desc*, int);
static int
_dopr(char** sbuffer,
      char** buffer,
      size_t* maxlen,
       ...
       int flags)
{
  ...
  spadlen =
    min -
    OSSL_MAX(max, place + zpadlen + (signvalue ? 1 : 0) + (int)strlen(prefix));
  if (spadlen < 0)
    spadlen = 0;
  if (flags & DP_F_MINUS) {
    spadlen = -spadlen;
...
BIO_vprintf(BIO* bio, const char* format, va_list args)
{
  ...
  dynbuf = NULL;
  if (!_dopr(
        &hugebufp, &dynbuf, &hugebufsize, &retlen, &ignored, format, args)) {
    OPENSSL_free(dynbuf);
    return -1;
  }
  if (dynbuf) {
