...
static int _dopr (char **sbuffer, char **buffer, size_t *maxlen,
                  size_t *retlen, int *truncated, const char *format,
                  va_list args);
...
static int
fmtint (struct pr_desc *desc, int64_t value, int base, int min, int max,
        int flags)
{
  ...
  if (zpadlen < 0)
    zpadlen = 0;
  spadlen = min
            - OSSL_MAX (max, place + zpadlen + (signvalue ? 1 : 0)
                                 + (int)strlen (prefix));
  if (spadlen < 0)
    spadlen = 0;
  if (flags & DP_F_MINUS)
...
static int
fmtfp (struct pr_desc *desc, LDOUBLE fvalue, int min, int max, int flags,
       int style)
{
  int signvalue = 0;
  LDOUBLE ufvalue;
