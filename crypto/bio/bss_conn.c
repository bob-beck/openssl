...
static int
conn_write(BIO* h, const char* buf, int num);
static int
conn_read(BIO* h, char* buf, int size);
static int
conn_puts(BIO* h, const char* str);
static int
conn_gets(BIO* h, char* buf, int size);
static long
conn_ctrl(BIO* h, int cmd, long arg1, void* arg2);
static int
conn_new(BIO* h);
static int
conn_free(BIO* data);
static long
conn_callback_ctrl(BIO* h, int cmd, BIO_info_cb*);
static int
conn_sendmmsg(BIO* h, BIO_MSG* m, size_t s, size_t n, uint64_t f, size_t* mp);
static int
conn_recvmmsg(BIO* h, BIO_MSG* m, size_t s, size_t n, uint64_t f, size_t* mp);

static int
conn_state(BIO* b, BIO_CONNECT* c);
static void
conn_close_socket(BIO* data);
static BIO_CONNECT*
...
static int
conn_create_dgram_bio(BIO* b, BIO_CONNECT* c)
{
  ...
  for (;;) {
    switch (c->state) {
      ...
      case BIO_CONN_S_GET_ADDR: {
        ...
        if (BIO_lookup(c->param_hostname,
                       c->param_service,
                       BIO_LOOKUP_CLIENT,
                       family,
                       c->connect_sock_type,
...
conn_write(BIO* b, const char* in, int inl)
{
  ...
  if (BIO_should_ktls_ctrl_msg_flag(b)) {
    ...
    ret = sendto(b->num,
                 ...
                 inl,
                 OSSL_TFO_SENDTO,
                 BIO_ADDRINFO_sockaddr(data->addr_iter),
                 peerlen);
    data->tfo_first = 0;
  } else
#endif
...
int
conn_gets(BIO* bio, char* buf, int size)
{
  BIO_CONNECT* data;
  char* ptr = buf;
  int ret = 0;
              ...
              size_t* msgs_processed)
{
  ...
  return BIO_sendmmsg(
    data->dgram_bio, msg, stride, num_msgs, flags, msgs_processed);
}

static int
conn_recvmmsg(BIO* bio,
              BIO_MSG* msg,
              size_t stride,
              size_t num_msgs,
              uint64_t flags,
