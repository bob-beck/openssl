...
static int conn_create_dgram_bio(BIO* b, BIO_CONNECT* c) {
  if (c->connect_sock_type != SOCK_DGRAM) return 1;

#ifndef OPENSSL_NO_DGRAM
  c->dgram_bio = BIO_new_dgram(b->num, 0);
  if (c->dgram_bio == NULL) goto err;
...
static int conn_state(BIO* b, BIO_CONNECT* c) {
  ...
  if (c->info_callback != NULL) cb = c->info_callback;
  ...
  for (;;) {
    switch (c->state) {
      ...
      case BIO_CONN_S_CONNECT:
        ...
        if (ret == 0) {
          ...
          ERR_clear_last_mark();
          if (!conn_create_dgram_bio(b, c)) break;
      ...
      case BIO_CONN_S_BLOCKED_CONNECT:
        ...
        if (BIO_socket_wait(b->num, 0, time(NULL)) == 0) break;
    ...
    if (cb != NULL) {
      if ((ret = cb((BIO*)b, c->state, ret)) == 0) goto end;
    }
  }

  /* Loop does not exit */
exit_loop:
  if (cb != NULL) ret = cb((BIO*)b, c->state, ret);
end:
  return ret;
}
...
static void conn_close_socket(BIO* bio) {
  ...
  if (bio->num != (int)INVALID_SOCKET) {
    ...
    if (c->state == BIO_CONN_S_OK) shutdown(bio->num, 2);
...
static int conn_write(BIO* b, const char* in, int inl) {
  ...
  BIO_clear_retry_flags(b);
  if (ret <= 0) {
    if (BIO_sock_should_retry(ret)) BIO_set_retry_write(b);
  }
  return ret;
}
...
static long conn_ctrl(BIO* b, int cmd, long num, void* ptr) {
  ...
  switch (cmd) {
    ...
    case BIO_C_SET_CONNECT:
      if (ptr != NULL) {
        ...
        if (num == 0) { /* BIO_set_conn_hostname */
          ...
          if (hold_service != data->param_service) OPENSSL_free(hold_service);
        } else if (num == 1) { /* BIO_set_conn_port */
          OPENSSL_free(data->param_service);
          if ((data->param_service = OPENSSL_strdup(ptr)) == NULL) ret = 0;
    ...
    case BIO_CTRL_DGRAM_DETECT_PEER_ADDR:
      if (data->state != BIO_CONN_S_OK) conn_state(b, data); /* best effort */
    ...
    case BIO_C_SET_NBIO:
      ...
      if (data->dgram_bio != NULL) ret = BIO_set_nbio(data->dgram_bio, num);
    ...
    case BIO_CTRL_DUP: {
      ...
      if (data->param_service) BIO_set_conn_port(dbio, data->param_service);
    ...
    case BIO_CTRL_SET_KTLS:
      ...
      if (ret) BIO_set_ktls_flag(b, num);
      break;
    ...
    case BIO_CTRL_SET_KTLS_TX_ZEROCOPY_SENDFILE:
      ...
      if (ret) BIO_set_ktls_zerocopy_sendfile_flag(b);
      break;
#endif
    default:
      ret = 0;
...
int conn_gets(BIO* bio, char* buf, int size) {
  ...
  *ptr = '\0';
  return ret > 0 || (bio->flags & BIO_FLAGS_IN_EOF) != 0 ? (int)(ptr - buf)
                                                         : ret;
}
...
BIO* BIO_new_connect(const char* str) {
  ...
  if (BIO_set_conn_hostname(ret, str)) return ret;
  BIO_free(ret);
  return NULL;
}
