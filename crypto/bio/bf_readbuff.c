...
static int
readbuffer_write(BIO* h, const char* buf, int num);
static int
readbuffer_read(BIO* h, char* buf, int size);
static int
readbuffer_puts(BIO* h, const char* str);
static int
readbuffer_gets(BIO* h, char* str, int size);
static long
readbuffer_ctrl(BIO* h, int cmd, long arg1, void* arg2);
static int
readbuffer_new(BIO* h);
static int
readbuffer_free(BIO* data);
static long
readbuffer_callback_ctrl(BIO* h, int cmd, BIO_info_cb* fp);
...
const BIO_METHOD*
BIO_f_readbuffer(void)
{
  return &methods_readbuffer;
}

static int
readbuffer_new(BIO* bi)
{
...
static int
readbuffer_resize(BIO_F_BUFFER_CTX* ctx, int sz)
{
  char* tmp;
...
static int
readbuffer_read(BIO* b, char* out, int outl)
{
  int i, num = 0;
...
static int
readbuffer_write(BIO* b, const char* in, int inl)
{
  return 0;
}
static int
readbuffer_puts(BIO* b, const char* str)
{
  return 0;
}

static long
readbuffer_ctrl(BIO* b, int cmd, long num, void* ptr)
{
  BIO_F_BUFFER_CTX* ctx;
  long ret = 1, sz;
...
static long
readbuffer_callback_ctrl(BIO* b, int cmd, BIO_info_cb* fp)
{
  if (b->next_bio == NULL)
    return 0;
...
static int
readbuffer_gets(BIO* b, char* buf, int size)
{
  BIO_F_BUFFER_CTX* ctx;
