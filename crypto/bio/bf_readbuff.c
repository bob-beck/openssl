...
static int readbuffer_read(BIO* b, char* out, int outl) {
  ...
  for (;;) {
    ...
    if (!readbuffer_resize(ctx, outl)) return 0;
...
static long readbuffer_ctrl(BIO* b, int cmd, long num, void* ptr) {
  ...
  switch (cmd) {
    case BIO_CTRL_EOF:
      if (ctx->ibuf_len > 0) return 0;
      if (b->next_bio == NULL) return 1;
...
static int readbuffer_gets(BIO* b, char* buf, int size) {
  ...
  if (!readbuffer_resize(ctx, 1 + size)) return 0;
  /*
