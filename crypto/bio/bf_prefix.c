...
static int prefix_write(BIO* b, const char* out, size_t outl,
  ...
  if ((ctx->prefix == NULL || *ctx->prefix == '\0') && ctx->indent == 0) {
     ...
     */
    if (outl > 0) ctx->linestart = (out[outl - 1] == '\n');
  ...
  *numwritten = 0;
  ...
  while (outl > 0) {
    ...
    while (i > 0) {
      ...
      if (!BIO_write_ex(BIO_next(b), out, i, &num)) return 0;
      out += num;
      outl -= num;
      *numwritten += num;
      i -= num;
...
static long prefix_ctrl(BIO* b, int cmd, long num, void* ptr) {
  ...
  if (b == NULL || (ctx = BIO_get_data(b)) == NULL) return -1;
  ...
  switch (cmd) {
    ...
    default:
      ...
      if (BIO_next(b) != NULL) ret = BIO_ctrl(BIO_next(b), cmd, num, ptr);
      break;
  }
  return ret;
}
