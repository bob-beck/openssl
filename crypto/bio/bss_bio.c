...
static int bio_free(BIO* bio) {
  ...
  if (b->peer) bio_destroy_pair(bio);
...
static int bio_read(BIO* bio, char* buf, int size_) {
  ...
  /* we can read */
  if (peer_b->len < size) size = peer_b->len;
  ...
  do { /* one or two iterations */
    ...
    if (peer_b->len) {
      ...
      if (peer_b->offset == peer_b->size) peer_b->offset = 0;
      buf += chunk;
    } else {
...
static ossl_ssize_t bio_nread0(BIO* bio, char** buf) {
  ...
  if (buf != NULL) *buf = peer_b->buf + peer_b->offset;
  return num;
}
...
static int bio_write(BIO* bio, const char* buf, int num_) {
  ...
  if (!bio->init || buf == NULL || num_ <= 0) return 0;
  ...
  /* we can write */
  if (num > b->size - b->len) num = b->size - b->len;
  ...
  do { /* one or two iterations */
    ...
    if (write_offset >= b->size) write_offset -= b->size;
...
static ossl_ssize_t bio_nwrite0(BIO* bio, char** buf) {
  ...
  if (buf != NULL) *buf = b->buf + write_offset;
