...
static void ring_buf_head_tail(struct ring_buf* r, int idx, uint8_t** buf,
  ...
  if (idx == 0 && max_len > r->len - r->count) max_len = r->len - r->count;
  if (idx == 1 && max_len > r->count) max_len = r->count;
...
static void ring_buf_push_pop(struct ring_buf* r, int idx, size_t num_bytes) {
   ...
   */
  if (!ossl_assert(num_bytes <= r->len - r->idx[idx])) return;
...
static int ring_buf_resize(struct ring_buf* r, size_t nbytes) {
  ...
  if (r->start == NULL) return ring_buf_init(r, nbytes);

  if (nbytes == r->len) return 1;
...
static int dgram_mem_init(BIO* bio) {
  ...
  if (!dgram_pair_init(bio)) return 0;
...
static int dgram_pair_free(BIO* bio) {
  ...
  b = bio->ptr;
  if (!ossl_assert(b != NULL)) return 0;
...
static int dgram_pair_ctrl_destroy_bio_pair(BIO* bio1) {
  ...
  if (b1->peer == NULL) return 1;

  bio2 = b1->peer;
  b2 = bio2->ptr;

  /* Invariant. */
  if (!ossl_assert(b2->peer == bio1)) return 0;
...
static int dgram_pair_ctrl_eof(BIO* bio) {
  ...
  if (!ossl_assert(b != NULL)) return -1;

  /* If we aren't initialized, we can never read anything */
  if (!bio->init) return 1;
  if (!is_dgram_pair(b)) return 0;

  peerb = b->peer->ptr;
  if (!ossl_assert(peerb != NULL)) return -1;
...
static int dgram_pair_ctrl_set_write_buf_size(BIO* bio, size_t len) {
  ...
  if (len < MIN_BUF_LEN) len = MIN_BUF_LEN;

  if (b->rbuf.start != NULL) {
    if (!ring_buf_resize(&b->rbuf, len)) return 0;
  }
...
static size_t dgram_pair_ctrl_pending(BIO* bio) {
  ...
  if (!bio->init) return 0;
  if (is_dgram_pair(b))
    readb = b->peer->ptr;
  else
    readb = b;

  if (CRYPTO_THREAD_write_lock(readb->lock) == 0) return 0;
  ...
  if (!ossl_assert(l == 0 || l == sizeof(hdr))) return 0;
...
static size_t dgram_pair_ctrl_get_write_guarantee(BIO* bio) {
  ...
  if (CRYPTO_THREAD_read_lock(b->lock) == 0) return 0;

  l = b->rbuf.len - b->rbuf.count;
  if (l >= sizeof(struct dgram_hdr)) l -= sizeof(struct dgram_hdr);
...
static int dgram_pair_ctrl_set_local_addr_enable(BIO* bio, int enable) {
  ...
  if (dgram_pair_ctrl_get_local_addr_cap(bio) == 0) return 0;
...
int BIO_new_bio_dgram_pair(BIO** pbio1, size_t writebuf1, BIO** pbio2,
  ...
  if (writebuf1 > LONG_MAX || writebuf2 > LONG_MAX) goto err;

  bio1 = BIO_new(BIO_s_dgram_pair());
  if (bio1 == NULL) goto err;

  bio2 = BIO_new(BIO_s_dgram_pair());
  if (bio2 == NULL) goto err;
...
static size_t dgram_pair_read_inner(struct bio_dgram_pair_st* b, uint8_t* buf,
  ...
  while (sz > 0) {
    ...
    if (src_len > sz) src_len = sz;

    if (buf != NULL) memcpy(buf, src_buf, src_len);

    ring_buf_pop(&b->rbuf, src_len);

    if (buf != NULL) buf += src_len;
    total_read += src_len;
    sz -= src_len;
  }
...
static ossl_ssize_t dgram_pair_read_actual(BIO* bio, char* buf, size_t sz,
  ...
  if (!is_multi) BIO_clear_retry_flags(bio);

  if (!bio->init) return -BIO_R_UNINITIALIZED;

  if (!ossl_assert(b != NULL)) return -BIO_R_TRANSFER_ERROR;
  ...
  if (sz > 0 && buf == NULL) return -BIO_R_INVALID_ARGUMENT;
  ...
  if (l == 0) {
    /* Buffer was empty. */
    if (!is_multi) BIO_set_retry_read(bio);
  ...
  if (local != NULL) *local = hdr.dst_addr;
  if (peer != NULL) *peer = hdr.src_addr;
...
static int dgram_pair_read(BIO* bio, char* buf, int sz_) {
  ...
  if (l < 0) {
    if (l != -BIO_R_NON_FATAL) ERR_raise(ERR_LIB_BIO, (int)-l);
    ret = -1;
  } else {
    ret = (int)l;
  }
...
static ossl_inline size_t compute_rbuf_growth(size_t target, size_t current) {
  ...
  while (current < target) {
    if (current >= max_rbuf_size) return 0;

    current = safe_muldiv_size_t(current, 8, 5, &err);
    if (err) return 0;
    if (current >= max_rbuf_size) current = max_rbuf_size;
  }
  return current;
}
...
static size_t dgram_pair_write_inner(struct bio_dgram_pair_st* b,
  ...
  while (sz > 0) {
    ...
    if (dst_len == 0) {
      ...
      if (new_len == 0 || !ring_buf_resize(&b->rbuf, new_len)) break;
      b->req_buf_len = new_len;
    }

    if (dst_len > sz) dst_len = sz;
...
static ossl_ssize_t dgram_pair_write_actual(BIO* bio, const char* buf,
  ...
  if (local == NULL) local = b->local_addr;
  ...
  if (dgram_pair_write_inner(b, (const uint8_t*)&hdr, sizeof(hdr)) !=
    ...
    if (!is_multi) BIO_set_retry_write(bio);
    return -BIO_R_NON_FATAL;
  }
...
static int dgram_pair_sendmmsg(BIO* bio, BIO_MSG* msg, size_t stride,
  ...
  for (i = 0; i < num_msg; ++i) {
    m = &BIO_MSG_N(msg, i);
    l = dgram_pair_write_actual(bio, m->data, m->data_len, m->local, m->peer,
                                1);
    if (l < 0) {
      *num_processed = i;
      if (i > 0) {
