...
dgram_pair_ctrl_make_bio_pair(BIO* bio1, BIO* bio2)
{
  ...
  if (bio1->method != &dgram_pair_method ||
    ...
    ERR_raise_data(
      ERR_LIB_BIO, BIO_R_INVALID_ARGUMENT, "both BIOs must be BIO_dgram_pair");
    return 0;
  }
...
int
BIO_new_bio_dgram_pair(BIO** pbio1,
                       size_t writebuf1,
                       BIO** pbio2,
                       size_t writebuf2)
{
 ...
 */
static ossl_ssize_t
dgram_pair_read_actual(BIO* bio,
                       char* buf,
                       size_t sz,
                       BIO_ADDR* local,
                       BIO_ADDR* peer,
...
static void
dgram_pair_unlock_both(struct bio_dgram_pair_st* a, struct bio_dgram_pair_st* b)
{
...
static size_t
dgram_pair_write_inner(struct bio_dgram_pair_st* b,
                       const uint8_t* buf,
                       size_t sz)
{
 ...
 */
static ossl_ssize_t
dgram_pair_write_actual(BIO* bio,
                        const char* buf,
                        size_t sz,
