...
static int
ring_buf_init (struct ring_buf *r, size_t nbytes)
{
...
static void
ring_buf_destroy (struct ring_buf *r)
{
...
/*
 * Get a pointer to the next place to write data to be pushed to the ring
 * buffer (idx=0), or the next data to be popped from the ring buffer (idx=1).
 * The pointer is written to *buf and the maximum number of bytes which can be
 ...
 */
static void
ring_buf_head_tail (struct ring_buf *r, int idx, uint8_t **buf, size_t *len)
{
 ...
 */
static void
ring_buf_push_pop (struct ring_buf *r, int idx, size_t num_bytes)
{
  size_t new_idx;
...
static void
ring_buf_clear (struct ring_buf *r)
{
  r->idx[0] = r->idx[1] = r->count = 0;
}

static int
ring_buf_resize (struct ring_buf *r, size_t nbytes)
{
...
static int
dgram_pair_ctrl_make_bio_pair (BIO *bio1, BIO *bio2)
{
...
static int
dgram_pair_ctrl_destroy_bio_pair (BIO *bio1)
{
  BIO *bio2;
...
static int
dgram_pair_ctrl_set_write_buf_size (BIO *bio, size_t len)
{
...
static size_t
dgram_pair_ctrl_pending (BIO *bio)
{
...
static size_t
dgram_pair_ctrl_get_write_guarantee (BIO *bio)
{
  size_t l;
  struct bio_dgram_pair_st *b = bio->ptr;
...
static int
dgram_pair_ctrl_get_local_addr_cap (BIO *bio)
{
  struct bio_dgram_pair_st *b = bio->ptr, *readb;
...
static int
dgram_pair_ctrl_get_effective_caps (BIO *bio)
{
  struct bio_dgram_pair_st *b = bio->ptr, *peerb;
...
static uint32_t
dgram_pair_ctrl_get_caps (BIO *bio)
{
  struct bio_dgram_pair_st *b = bio->ptr;
...
static int
dgram_pair_ctrl_set_caps (BIO *bio, uint32_t caps)
{
  struct bio_dgram_pair_st *b = bio->ptr;
...
static int
dgram_pair_ctrl_get_local_addr_enable (BIO *bio)
{
  struct bio_dgram_pair_st *b = bio->ptr;
...
static int
dgram_pair_ctrl_set_local_addr_enable (BIO *bio, int enable)
{
  struct bio_dgram_pair_st *b = bio->ptr;
...
static int
dgram_pair_ctrl_get_mtu (BIO *bio)
{
  struct bio_dgram_pair_st *b = bio->ptr;
...
static int
dgram_pair_ctrl_set_mtu (BIO *bio, size_t mtu)
{
  struct bio_dgram_pair_st *b = bio->ptr, *peerb;
...
static int
dgram_pair_ctrl_set0_local_addr (BIO *bio, BIO_ADDR *addr)
{
  struct bio_dgram_pair_st *b = bio->ptr;
...
static long
dgram_mem_ctrl (BIO *bio, int cmd, long num, void *ptr)
{
  long ret = 1;
  struct bio_dgram_pair_st *b = bio->ptr;
  ...
  switch (cmd)
     ...
     * until reads are performed. This cannot be used after a peer is
     * connected.
     */
...
static long
dgram_pair_ctrl (BIO *bio, int cmd, long num, void *ptr)
{
  long ret = 1;
...
int
BIO_new_bio_dgram_pair (BIO **pbio1, size_t writebuf1, BIO **pbio2,
                        size_t writebuf2)
{
  int ret = 0;
  long r;
...
static size_t
dgram_pair_read_inner (struct bio_dgram_pair_st *b, uint8_t *buf, size_t sz)
{
  size_t total_read = 0;
 ...
 */
static ossl_ssize_t
dgram_pair_read_actual (BIO *bio, char *buf, size_t sz, BIO_ADDR *local,
                        BIO_ADDR *peer, int is_multi)
{
...
/* Threadsafe */
static int
dgram_pair_lock_both_write (struct bio_dgram_pair_st *a,
                            struct bio_dgram_pair_st *b)
{
...
static void
dgram_pair_unlock_both (struct bio_dgram_pair_st *a,
                        struct bio_dgram_pair_st *b)
{
...
/* Threadsafe */
static int
dgram_pair_read (BIO *bio, char *buf, int sz_)
{
  int ret;
  ossl_ssize_t l;
  struct bio_dgram_pair_st *b = bio->ptr, *peerb;
...
/* Threadsafe */
static int
dgram_pair_recvmmsg (BIO *bio, BIO_MSG *msg, size_t stride, size_t num_msg,
...
/* Threadsafe */
static int
dgram_mem_read (BIO *bio, char *buf, int sz_)
{
  int ret;
  ossl_ssize_t l;
  struct bio_dgram_pair_st *b = bio->ptr;
...
compute_rbuf_growth (size_t target, size_t current)
{
  int err = 0;
...
static size_t
dgram_pair_write_inner (struct bio_dgram_pair_st *b, const uint8_t *buf,
                        size_t sz)
{
 ...
 */
static ossl_ssize_t
dgram_pair_write_actual (BIO *bio, const char *buf, size_t sz,
...
/* Threadsafe */
static int
dgram_pair_write (BIO *bio, const char *buf, int sz_)
{
  int ret;
  ossl_ssize_t l;
  struct bio_dgram_pair_st *b = bio->ptr;
...
/* Threadsafe */
static int
dgram_pair_sendmmsg (BIO *bio, BIO_MSG *msg, size_t stride, size_t num_msg,
                     uint64_t flags, size_t *num_processed)
{
  ossl_ssize_t l;
  BIO_MSG *m;
