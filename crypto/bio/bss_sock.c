...
static int
sock_write(BIO* h, const char* buf, int num);
static int
sock_read(BIO* h, char* buf, int size);
static int
sock_puts(BIO* h, const char* str);
static long
sock_ctrl(BIO* h, int cmd, long arg1, void* arg2);
static int
sock_new(BIO* h);
static int
sock_free(BIO* data);
int
BIO_sock_should_retry(int s);
...
sock_write(BIO* b, const char* in, int inl)
{
  ...
  if (BIO_should_ktls_ctrl_msg_flag(b)) {
    ...
    ret = sendto(b->num,
                 ...
                 inl,
                 OSSL_TFO_SENDTO,
                 BIO_ADDR_sockaddr(&data->tfo_peer),
                 peerlen);
    data->tfo_first = 0;
  } else
#endif
