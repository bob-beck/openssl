...
dgram_write (BIO *b, const char *in, int inl)
{
  ...
  if (data->connected)
  ...
  else
    {
      ...
      ret = sendto (b->num, in, inl, 0, BIO_ADDR_sockaddr (&data->peer),
                    peerlen);
    }
...
dgram_ctrl (BIO *b, int cmd, long num, void *ptr)
{
  ...
  switch (cmd)
    ...
    && defined(IP_PMTUDISC_PROBE)
          ...
          sockopt_val = num ? 1 : 0;
          if ((ret
               = setsockopt (b->num, IPPROTO_IP, IP_DONTFRAGMENT,
        ...
        case AF_INET6:
          ...
          sockopt_val = num ? 1 : 0;
          if ((ret
               = setsockopt (b->num, IPPROTO_IPV6, IPV6_DONTFRAG,
     ...
     * binary compatibility we now respond to both the old value and the new
     * one
     */
...
static void
translate_msg_win (BIO *b, WSAMSG *mh, WSABUF *iov, unsigned char *control,
                   BIO_MSG *msg)
{
...
static void
translate_msg (BIO *b, struct msghdr *mh, struct iovec *iov,
 ...
 * should mask out any system flags returned by this function you cannot
 * support in a particular circumstance. Currently no flags are defined.
 */
...
static int
dgram_sendmmsg (BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg,
                uint64_t flags, size_t *num_processed)
{
...
static int
dgram_recvmmsg (BIO *b, BIO_MSG *msg, size_t stride, size_t num_msg,
                uint64_t flags, size_t *num_processed)
{
#if M_METHOD != M_METHOD_NONE && M_METHOD != M_METHOD_RECVMSG
  int ret;
  ...
  for (i = 0; i < (size_t)ret; ++i)
    {
       ...
       */
      if (BIO_MSG_N (msg, stride, i).local != NULL)
        if (extract_local (b, &mh[i].msg_hdr, BIO_MSG_N (msg, stride, i).local)
            < 1)
          /*
  ...
  if (bio_WSARecvMsg != NULL)
    {
      ...
      ret = WSARecvMsg ((SOCKET)b->num, &wmsg, &num_bytes_received, NULL,
                        NULL);
      if (ret < 0)
        {
          ERR_raise (ERR_LIB_SYS, get_last_socket_error ());
