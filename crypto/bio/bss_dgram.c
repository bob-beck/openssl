...
static int dgram_read(BIO* b, char* out, int outl) {
  ...
  if (out != NULL) {
    ...
    if (data->peekmode) flags = MSG_PEEK;
    ret = recvfrom(b->num, out, outl, flags, BIO_ADDR_sockaddr_noconst(&peer),
                   &len);
  ...
  switch (cmd) {
    ...
    case BIO_C_SET_FD:
      ...
      if (data->local_addr_enabled) {
        if (enable_local_addr(b, 1) < 1) data->local_addr_enabled = 0;
      }
#endif
      break;
    case BIO_C_GET_FD:
...
#if defined(IP_PKTINFO)
      if (cmsg->cmsg_type != IP_PKTINFO) continue;

      local->s_in.sin_addr =
          ((struct in_pktinfo*)BIO_CMSG_DATA(cmsg))->ipi_addr;

#elif defined(IP_RECVDSTADDR)
      if (cmsg->cmsg_type != IP_RECVDSTADDR) continue;
...
#define BIO_MAX_MSGS_PER_CALL 64
  ...
  if (num_msg > OSSL_SSIZE_MAX) num_msg = OSSL_SSIZE_MAX;
   ...
   * be sent.
   */
  if (num_msg > BIO_MAX_MSGS_PER_CALL) num_msg = BIO_MAX_MSGS_PER_CALL;
...
static int dgram_sctp_new(BIO* bi) {
  ...
  bi->init = 0;
  bi->num = 0;
  if ((data = OPENSSL_zalloc(sizeof(*data))) == NULL) return 0;
...
static int dgram_sctp_read(BIO* b, char* out, int outl) {
  ...
  if (out != NULL) {
    ...
    if (ret > 0 && !(msg.msg_flags & MSG_EOR)) {
      ...
      if (ret >= 0) OPENSSL_assert(optval >= 18445);
...
static long dgram_sctp_ctrl(BIO* b, int cmd, long num, void* ptr) {
  ...
  switch (cmd) {
    ...
    case BIO_CTRL_DGRAM_SCTP_AUTH_CCS_RCVD:
       ...
       * fall-through?
       */
      if (cmd == BIO_CTRL_DGRAM_SCTP_AUTH_CCS_RCVD) data->ccs_rcvd = 1;
