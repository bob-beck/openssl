...
static int
dgram_write(BIO* h, const char* buf, int num);
static int
dgram_read(BIO* h, char* buf, int size);
static int
dgram_puts(BIO* h, const char* str);
static long
dgram_ctrl(BIO* h, int cmd, long arg1, void* arg2);
static int
dgram_new(BIO* h);
static int
dgram_free(BIO* data);
...
#ifndef OPENSSL_NO_SCTP
static int
dgram_sctp_write(BIO* h, const char* buf, int num);
static int
dgram_sctp_read(BIO* h, char* buf, int size);
static int
dgram_sctp_puts(BIO* h, const char* str);
static long
dgram_sctp_ctrl(BIO* h, int cmd, long arg1, void* arg2);
static int
dgram_sctp_new(BIO* h);
static int
dgram_sctp_free(BIO* data);
static int
dgram_sctp_wait_for_dry(BIO* b);
static int
dgram_sctp_msg_waiting(BIO* b);
...
static int
BIO_dgram_should_retry(int s);

static const BIO_METHOD methods_dgramp = {
  BIO_TYPE_DGRAM, "datagram socket", bwrite_conv, dgram_write, bread_conv,
  dgram_read,     dgram_puts,        NULL, /* dgram_gets,         */
...
dgram_adjust_rcv_timeout(BIO* b)
{
  ...
  if (!ossl_time_is_zero(data->next_timeout)) {
    ...
    if (getsockopt(b->num, SOL_SOCKET, SO_RCVTIMEO, (void*)&timeout, &sz) < 0)
      ERR_raise_data(
        ERR_LIB_SYS, get_last_socket_error(), "calling getsockopt()");
    else
    ...
    if (ossl_time_is_zero(data->socket_timeout) ||
      ...
      if (setsockopt(
            b->num, SOL_SOCKET, SO_RCVTIMEO, (void*)&timeout, sizeof(timeout)) <
          0)
        ERR_raise_data(
          ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
#else
...
dgram_read(BIO* b, char* out, int outl)
{
  ...
  if (out != NULL) {
    ...
    if (data->peekmode)
      flags = MSG_PEEK;
    ret = recvfrom(
      b->num, out, outl, flags, BIO_ADDR_sockaddr_noconst(&peer), &len);
...
static long
dgram_get_mtu_overhead(BIO_ADDR* addr)
{
  long ret;
...
static int
enable_local_addr(BIO* b, int enable)
{
  ...
  if (af == AF_INET) {
    ...
    if (setsockopt(
          b->num, IPPROTO_IP, IP_PKTINFO, (void*)&enable, sizeof(enable)) < 0)
      return 0;
    ...
    if (setsockopt(
          b->num, IPPROTO_IP, IP_RECVDSTADDR, &enable, sizeof(enable)) < 0)
      return 0;
  ...
  if (af == AF_INET6) {
    ...
    if (setsockopt(
          b->num, IPPROTO_IPV6, IPV6_RECVPKTINFO, &enable, sizeof(enable)) < 0)
      return 0;
...
dgram_ctrl(BIO* b, int cmd, long num, void* ptr)
{
  ...
  defined(IP_PMTUDISC_DO)
...
#else
      if ((ret = setsockopt(
             b->num, SOL_SOCKET, SO_RCVTIMEO, ptr, sizeof(struct timeval))) < 0)
        ERR_raise_data(
          ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
#endif
    break;
    case BIO_CTRL_DGRAM_GET_RECV_TIMEOUT: {
      ...
      sz = sizeof(timeout);
      if ((ret = getsockopt(
             b->num, SOL_SOCKET, SO_RCVTIMEO, (void*)&timeout, &sz)) < 0) {
        ERR_raise_data(
          ERR_LIB_SYS, get_last_socket_error(), "calling getsockopt()");
      } else {
...
#else
      if ((ret = setsockopt(
             b->num, SOL_SOCKET, SO_SNDTIMEO, ptr, sizeof(struct timeval))) < 0)
        ERR_raise_data(
          ERR_LIB_SYS, get_last_socket_error(), "calling setsockopt()");
#endif
    break;
    case BIO_CTRL_DGRAM_GET_SEND_TIMEOUT: {
      ...
      sz = sizeof(timeout);
      if ((ret = getsockopt(
             b->num, SOL_SOCKET, SO_SNDTIMEO, (void*)&timeout, &sz)) < 0) {
        ERR_raise_data(
          ERR_LIB_SYS, get_last_socket_error(), "calling getsockopt()");
      } else {
        tv->tv_sec = timeout / 1000;
...
static int
extract_local(BIO* b, MSGHDR_TYPE* mh, BIO_ADDR* local)
{
...
static int
pack_local(BIO* b, MSGHDR_TYPE* mh, const BIO_ADDR* local)
{
  int af = dgram_get_sock_family(b);
#if defined(IP_PKTINFO) || defined(IP_RECVDSTADDR) || defined(IPV6_PKTINFO)
  CMSGHDR_TYPE* cmsg;
...
static int
translate_flags(uint64_t flags)
{
  return 0;
}
#endif
...
#define BIO_MAX_MSGS_PER_CALL 64
  ...
  for (i = 0; i < num_msg; ++i) {
    translate_msg(
      b, &mh[i].msg_hdr, &iov[i], control[i], &BIO_MSG_N(msg, stride, i));
...
BIO_new_dgram_sctp(int fd, int close_flag)
{
  ...
  ret = setsockopt(
    fd, IPPROTO_SCTP, SCTP_AUTH_CHUNK, &auth, sizeof(struct sctp_authchunk));
  if (ret < 0) {
    BIO_vfree(bio);
  ...
  ret = getsockopt(
    fd, IPPROTO_SCTP, SCTP_LOCAL_AUTH_CHUNKS, authchunks, &sockopt_len);
  if (ret < 0) {
  ...
  ret = setsockopt(
    fd, IPPROTO_SCTP, SCTP_EVENTS, &event, sizeof(struct sctp_event_subscribe));
  if (ret < 0) {
    BIO_vfree(bio);
    return NULL;
  }
   ...
   */
  ret = setsockopt(
    fd, IPPROTO_SCTP, SCTP_PARTIAL_DELIVERY_POINT, &optval, sizeof(optval));
  if (ret < 0) {
    BIO_vfree(bio);
    return NULL;
  }
...
dgram_sctp_read(BIO* b, char* out, int outl)
{
  ...
  if (out != NULL) {
    ...
    if (ret > 0 && !(msg.msg_flags & MSG_EOR)) {
      ...
      optlen = (socklen_t)sizeof(int);
      ret = getsockopt(
        b->num, IPPROTO_SCTP, SCTP_PARTIAL_DELIVERY_POINT, &optval, &optlen);
      if (ret >= 0)
        OPENSSL_assert(optval >= 18445);
    ...
    if (!data->peer_auth_tested) {
      ...
      ii = getsockopt(
        b->num, IPPROTO_SCTP, SCTP_PEER_AUTH_CHUNKS, authchunks, &optlen);
...
dgram_sctp_ctrl(BIO* b, int cmd, long num, void* ptr)
{
  ...
  switch (cmd) {
    ...
    case BIO_CTRL_DGRAM_SCTP_SET_IN_HANDSHAKE:
      ...
      ret = setsockopt(
        b->num, IPPROTO_SCTP, SCTP_NODELAY, &data->in_handshake, sizeof(int));
      break;
    case BIO_CTRL_DGRAM_SCTP_ADD_AUTH_KEY:
      ...
      ret = getsockopt(
        b->num, IPPROTO_SCTP, SCTP_AUTH_ACTIVE_KEY, &authkeyid, &sockopt_len);
      if (ret < 0)
        break;
...
dgram_sctp_wait_for_dry(BIO* b)
{
  ...
  event.se_on = 1;
  ret = setsockopt(
    b->num, IPPROTO_SCTP, SCTP_EVENT, &event, sizeof(struct sctp_event));
#else
  eventsize = sizeof(struct sctp_event_subscribe);
