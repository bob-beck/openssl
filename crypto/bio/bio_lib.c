...
BIO* BIO_new_ex(OSSL_LIB_CTX* libctx, const BIO_METHOD* method) {
  ...
  if (!CRYPTO_NEW_REF(&bio->references, 1)) goto err;

  if (!CRYPTO_new_ex_data(CRYPTO_EX_INDEX_BIO, bio, &bio->ex_data)) goto err;
  ...
  if (method->create == NULL) bio->init = 1;
...
int BIO_up_ref(BIO* a) {
  ...
  if (CRYPTO_UP_REF(&a->references, &i) <= 0) return 0;
...
static int bio_read_intern(BIO* b, void* data, size_t dlen, size_t* readbytes) {
  ...
  if (ret > 0) b->num_read += (uint64_t)*readbytes;
...
BIO* BIO_pop(BIO* b) {
  ...
  if (b->prev_bio != NULL) b->prev_bio->next_bio = b->next_bio;
  if (b->next_bio != NULL) b->next_bio->prev_bio = b->prev_bio;
...
int BIO_do_connect_retry(BIO* bio, int timeout, int nap_milliseconds) {
  ...
  if (nap_milliseconds < 0) nap_milliseconds = 100;
