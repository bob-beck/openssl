...
BIO*
BIO_new_ex(OSSL_LIB_CTX* libctx, const BIO_METHOD* method)
{
 ...
 */
static int
bio_read_intern(BIO* b, void* data, size_t dlen, size_t* readbytes)
{
  int ret;
  ...
  if (HAS_CALLBACK(b))
    ret = (int)bio_call_callback(
      b, BIO_CB_READ | BIO_CB_RETURN, data, dlen, 0, 0L, ret, readbytes);
...
int
BIO_read(BIO* b, void* data, int dlen)
{
  size_t readbytes;
  int ret;
...
static int
bio_write_intern(BIO* b, const void* data, size_t dlen, size_t* written)
{
  ...
  if (HAS_CALLBACK(b))
    ret = (int)bio_call_callback(
      b, BIO_CB_WRITE | BIO_CB_RETURN, data, dlen, 0, 0L, ret, &local_written);
...
int
BIO_write(BIO* b, const void* data, int dlen)
{
  size_t written;
  int ret;
             ...
             size_t* msgs_processed)
{
  ...
  if (HAS_CALLBACK(b)) {
    ...
    ret = (size_t)bio_call_callback(
      b, BIO_CB_SENDMMSG, (void*)&args, 0, 0, 0, 1, NULL);
    if (ret <= 0)
      return 0;
  }
  ...
  if (HAS_CALLBACK(b))
    ret = (size_t)bio_call_callback(b,
...
int
BIO_get_rpoll_descriptor(BIO* b, BIO_POLL_DESCRIPTOR* desc)
{
  return BIO_ctrl(b, BIO_CTRL_GET_RPOLL_DESCRIPTOR, 0, desc);
}

int
BIO_get_wpoll_descriptor(BIO* b, BIO_POLL_DESCRIPTOR* desc)
{
...
BIO_puts(BIO* b, const char* buf)
{
  ...
  if (HAS_CALLBACK(b))
    ret = (int)bio_call_callback(
      b, BIO_CB_PUTS | BIO_CB_RETURN, buf, 0, 0, 0L, ret, &written);
...
BIO_gets(BIO* b, char* buf, int size)
{
  ...
  if (HAS_CALLBACK(b))
    ret = (int)bio_call_callback(
      b, BIO_CB_GETS | BIO_CB_RETURN, buf, size, 0, 0L, ret, &readbytes);
...
int
BIO_get_line(BIO* bio, char* buf, int size)
{
  int ret = 0;
  char* ptr = buf;
...
BIO_ctrl(BIO* b, int cmd, long larg, void* parg)
{
  ...
  if (HAS_CALLBACK(b))
    ret = bio_call_callback(
      b, BIO_CB_CTRL | BIO_CB_RETURN, parg, 0, cmd, larg, ret, NULL);
...
BIO_callback_ctrl(BIO* b, int cmd, BIO_info_cb* fp)
{
  ...
  if (HAS_CALLBACK(b))
    ret = bio_call_callback(
      b, BIO_CB_CTRL | BIO_CB_RETURN, (void*)&fp, 0, cmd, 0, ret, NULL);
...
BIO_dup_chain(BIO* in)
{
  ...
  for (bio = in; bio != NULL; bio = bio->next_bio) {
    ...
    if (!CRYPTO_dup_ex_data(
          CRYPTO_EX_INDEX_BIO, &new_bio->ex_data, &bio->ex_data)) {
      BIO_free(new_bio);
      goto err;
    }
...
void*
BIO_get_ex_data(const BIO* bio, int idx)
{
...
static int
bio_wait(BIO* bio, time_t max_time, unsigned int nap_milliseconds)
{
#ifndef OPENSSL_NO_SOCK
  int fd;
#endif
 ...
 */
int
BIO_do_connect_retry(BIO* bio, int timeout, int nap_milliseconds)
{
