 ...
 * old one, and translate between the two.
 *
...
bio_write_intern (BIO *b, const void *data, size_t dlen, size_t *written)
{
  ...
  if (HAS_CALLBACK (b))
    ret = (int)bio_call_callback (b, BIO_CB_WRITE | BIO_CB_RETURN, data, dlen,
                                  0, 0L, ret, &local_written);
...
bio_wait (BIO *bio, time_t max_time, unsigned int nap_milliseconds)
{
  ...
  if (sec_diff == 0)
    { /* we are below the 1 seconds resolution of max_time */
      if (nap_milliseconds > 1000)
        nap_milliseconds = 1000;
    }
  else
    { /* for sec_diff > 0, take min(sec_diff * 1000, nap_milliseconds) */
...
retry:
  ...
  if (rv <= 0)
    { /* could be timeout or retryable error or fatal error */
