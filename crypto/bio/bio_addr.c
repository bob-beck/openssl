...
#endif
  if (family == AF_INET) {
    if (wherelen != sizeof(struct in_addr)) return 0;
...
#if OPENSSL_USE_IPV6
  if (family == AF_INET6) {
    if (wherelen != sizeof(struct in6_addr)) return 0;
...
unsigned short BIO_ADDR_rawport(const BIO_ADDR* ap) {
  if (ap->sa.sa_family == AF_INET) return ap->s_in.sin_port;
#if OPENSSL_USE_IPV6
  if (ap->sa.sa_family == AF_INET6) return ap->s_in6.sin6_port;
#endif
  return 0;
}
...
static int addr_strings(const BIO_ADDR* ap, int numeric, char** hostname,
  ...
  if (1) {
    ...
    if (numeric) flags |= NI_NUMERICHOST | NI_NUMERICSERV;
    ...
    if (hostname != NULL) *hostname = OPENSSL_strdup(host);
    if (service != NULL) *service = OPENSSL_strdup(serv);
  } else {
#endif
    if (hostname != NULL)
...
char* BIO_ADDR_hostname_string(const BIO_ADDR* ap, int numeric) {
  ...
  if (addr_strings(ap, numeric, &hostname, NULL)) return hostname;
...
char* BIO_ADDR_service_string(const BIO_ADDR* ap, int numeric) {
  ...
  if (addr_strings(ap, numeric, NULL, &service)) return service;
...
#ifndef OPENSSL_NO_UNIX_SOCK
  if (ap->sa.sa_family == AF_UNIX) return OPENSSL_strdup(ap->s_un.sun_path);
#endif
  return NULL;
}
...
socklen_t BIO_ADDR_sockaddr_size(const BIO_ADDR* ap) {
  if (ap->sa.sa_family == AF_INET) return sizeof(ap->s_in);
#if OPENSSL_USE_IPV6
  if (ap->sa.sa_family == AF_INET6) return sizeof(ap->s_in6);
#endif
#ifndef OPENSSL_NO_UNIX_SOCK
  if (ap->sa.sa_family == AF_UNIX) return sizeof(ap->s_un);
#endif
  return sizeof(*ap);
}
...
const BIO_ADDRINFO* BIO_ADDRINFO_next(const BIO_ADDRINFO* bai) {
  if (bai != NULL) return bai->bai_next;
  return NULL;
}

int BIO_ADDRINFO_family(const BIO_ADDRINFO* bai) {
  if (bai != NULL) return bai->bai_family;
  return 0;
}

int BIO_ADDRINFO_socktype(const BIO_ADDRINFO* bai) {
  if (bai != NULL) return bai->bai_socktype;
  return 0;
}

int BIO_ADDRINFO_protocol(const BIO_ADDRINFO* bai) {
  if (bai != NULL) {
    if (bai->bai_protocol != 0) return bai->bai_protocol;

#ifndef OPENSSL_NO_UNIX_SOCK
    if (bai->bai_family == AF_UNIX) return 0;
#endif
...
socklen_t BIO_ADDRINFO_sockaddr_size(const BIO_ADDRINFO* bai) {
  if (bai != NULL) return (socklen_t)bai->bai_addrlen;
  return 0;
}
...
const struct sockaddr* BIO_ADDRINFO_sockaddr(const BIO_ADDRINFO* bai) {
  if (bai != NULL) return bai->bai_addr;
  return NULL;
}

const BIO_ADDR* BIO_ADDRINFO_address(const BIO_ADDRINFO* bai) {
  if (bai != NULL) return (BIO_ADDR*)bai->bai_addr;
  return NULL;
}
...
int BIO_parse_hostserv(const char* hostserv, char** host, char** service,
  ...
  if (*hostserv == '[') {
    if ((p = strchr(hostserv, ']')) == NULL) goto spec_err;
    h = hostserv + 1;
    hl = p - h;
    p++;
    if (*p == '\0')
  ...
  if (p != NULL && strchr(p, ':')) goto spec_err;
...
static int addrinfo_wrap(int family, int socktype, const void* where,
                         size_t wherelen, unsigned short port,
                         BIO_ADDRINFO** bai) {
  if ((*bai = OPENSSL_zalloc(sizeof(**bai))) == NULL) return 0;

  (*bai)->bai_family = family;
  (*bai)->bai_socktype = socktype;
  if (socktype == SOCK_STREAM) (*bai)->bai_protocol = IPPROTO_TCP;
  if (socktype == SOCK_DGRAM) (*bai)->bai_protocol = IPPROTO_UDP;
#ifndef OPENSSL_NO_UNIX_SOCK
  if (family == AF_UNIX) (*bai)->bai_protocol = 0;
#endif
  {
...
int BIO_lookup_ex(const char* host, const char* service, int lookup_type,
  ...
  if (1) {
    ...
    if (lookup_type == BIO_LOOKUP_SERVER) hints.ai_flags |= AI_PASSIVE;
...
#pragma pointer_size restore
    ...
    if (!CRYPTO_THREAD_write_lock(bio_lookup_lock)) return 0;
