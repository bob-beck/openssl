...
addr_strings(const BIO_ADDR* ap, int numeric, char** hostname, char** service)
{
  ...
  if (1) {
    ...
    if ((ret = getnameinfo(BIO_ADDR_sockaddr(ap),
      ...
      if (ret == EAI_SYSTEM) {
        ERR_raise_data(
          ERR_LIB_SYS, get_last_socket_error(), "calling getnameinfo()");
      } else
#endif
      {
 ...
 */
int
BIO_parse_hostserv(const char* hostserv,
                   char** host,
                   char** service,
              ...
              BIO_ADDRINFO** res)
{
  ...
  retry:
    ...
    switch ((gai_ret = getaddrinfo(host, service, &hints, res))) {
      ...
      case EAI_SYSTEM:
        ERR_raise_data(
          ERR_LIB_SYS, get_last_socket_error(), "calling getaddrinfo()");
...
#else
        ERR_raise_data(
          ERR_LIB_SYS, get_last_socket_error(), "calling gethostbyname()");
#endif
        ret = 0;
        goto err;
      }
...
#pragma pointer_size restore
      ...
      if (endp != service && *endp == '\0' && portnum > 0 && portnum < 65536) {
        ...
        if (se == NULL) {
          ERR_raise_data(
            ERR_LIB_SYS, get_last_socket_error(), "calling getservbyname()");
          goto err;
        }
      } else {
        ERR_raise(ERR_LIB_BIO, BIO_R_MALFORMED_HOST_OR_SERVICE);
...
#pragma pointer_size restore
      ...
      for (addresses = addrlistp - he->h_addr_list;
        ...
        if (!addrinfo_wrap(he->h_addrtype,
                           socktype,
                           *addrlistp,
                           he->h_length,
                           se->s_port,
