...
int
BIO_ADDR_copy (BIO_ADDR *dst, const BIO_ADDR *src)
{
...
BIO_ADDR *
BIO_ADDR_dup (const BIO_ADDR *ap)
{
  BIO_ADDR *ret = NULL;
 ...
 */
int
BIO_ADDR_make (BIO_ADDR *ap, const struct sockaddr *sa)
{
...
int
BIO_ADDR_rawmake (BIO_ADDR *ap, int family, const void *where, size_t wherelen,
                  unsigned short port)
{
#ifndef OPENSSL_NO_UNIX_SOCK
  if (family == AF_UNIX)
...
int
BIO_ADDR_family (const BIO_ADDR *ap)
{
  return ap->sa.sa_family;
}

int
BIO_ADDR_rawaddress (const BIO_ADDR *ap, void *p, size_t *l)
{
  size_t len = 0;
...
unsigned short
BIO_ADDR_rawport (const BIO_ADDR *ap)
{
  if (ap->sa.sa_family == AF_INET)
 ...
 */
static int
addr_strings (const BIO_ADDR *ap, int numeric, char **hostname, char **service)
{
  if (BIO_sock_init () != 1)
    return 0;
...
char *
BIO_ADDR_hostname_string (const BIO_ADDR *ap, int numeric)
{
  char *hostname = NULL;
...
char *
BIO_ADDR_service_string (const BIO_ADDR *ap, int numeric)
{
  char *service = NULL;
...
char *
BIO_ADDR_path_string (const BIO_ADDR *ap)
{
#ifndef OPENSSL_NO_UNIX_SOCK
...
BIO_ADDR_sockaddr (const BIO_ADDR *ap)
{
  return &(ap->sa);
}
 ...
 */
struct sockaddr *
BIO_ADDR_sockaddr_noconst (BIO_ADDR *ap)
{
  return &(ap->sa);
}
 ...
 */
socklen_t
BIO_ADDR_sockaddr_size (const BIO_ADDR *ap)
{
  if (ap->sa.sa_family == AF_INET)
...
const BIO_ADDRINFO *
BIO_ADDRINFO_next (const BIO_ADDRINFO *bai)
{
  if (bai != NULL)
    return bai->bai_next;
  return NULL;
...
int
BIO_ADDRINFO_family (const BIO_ADDRINFO *bai)
{
  if (bai != NULL)
...
int
BIO_ADDRINFO_socktype (const BIO_ADDRINFO *bai)
{
  if (bai != NULL)
...
int
BIO_ADDRINFO_protocol (const BIO_ADDRINFO *bai)
{
  if (bai != NULL)
    {
 ...
 */
socklen_t
BIO_ADDRINFO_sockaddr_size (const BIO_ADDRINFO *bai)
{
  if (bai != NULL)
 ...
 */
const struct sockaddr *
BIO_ADDRINFO_sockaddr (const BIO_ADDRINFO *bai)
{
  if (bai != NULL)
    return bai->bai_addr;
  return NULL;
...
const BIO_ADDR *
BIO_ADDRINFO_address (const BIO_ADDRINFO *bai)
{
  if (bai != NULL)
...
void
BIO_ADDRINFO_free (BIO_ADDRINFO *bai)
{
  if (bai == NULL)
    return;
 ...
 */
int
BIO_parse_hostserv (const char *hostserv, char **host, char **service,
 ...
 */
static int
addrinfo_wrap (int family, int socktype, const void *where, size_t wherelen,
               unsigned short port, BIO_ADDRINFO **bai)
{
...
int
BIO_lookup (const char *host, const char *service,
 ...
 *  or 0 for all.
 * @protocol: The protocol to use, e.g. IPPROTO_TCP or IPPROTO_UDP or 0 for
 * all. Note that some platforms may not return IPPROTO_SCTP without explicitly
 * requesting it (i.e. IPPROTO_SCTP may not be returned with 0 for the
 * protocol)
 ...
 *
 * When no longer needed you should call BIO_ADDRINFO_free() to free the
 * result.
 *
 * The return value is 1 on success or 0 in case of error.
 */
int
BIO_lookup_ex (const char *host, const char *service, int lookup_type,
