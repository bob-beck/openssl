...
static int
ossl_provider_cmp (const OSSL_PROVIDER *const *a,
...
int
ossl_provider_info_add_to_store (OSSL_LIB_CTX *libctx,
...
OSSL_PROVIDER *
ossl_provider_find (OSSL_LIB_CTX *libctx, const char *name,
 ...
 */
OSSL_PROVIDER *
ossl_provider_new (OSSL_LIB_CTX *libctx, const char *name,
                   OSSL_provider_init_fn *init_function, OSSL_PARAM *params,
                   int noconfig)
{
  struct provider_store_st *store = NULL;
...
int
ossl_provider_add_to_store (OSSL_PROVIDER *prov, OSSL_PROVIDER **actualprov,
...
static int
infopair_add (STACK_OF (INFOPAIR) **infopairsk, const char *name,
              const char *value)
{
  INFOPAIR *pair = NULL;
...
int
OSSL_PROVIDER_add_conf_parameter (OSSL_PROVIDER *prov, const char *name,
                                  const char *value)
{
...
int
OSSL_PROVIDER_get_conf_parameters (const OSSL_PROVIDER *prov,
                                   OSSL_PARAM params[])
{
  int i;
...
int
OSSL_PROVIDER_conf_get_bool (const OSSL_PROVIDER *prov, const char *name,
                             int defval)
{
  char *val = NULL;
...
int
ossl_provider_info_add_parameter (OSSL_PROVIDER_INFO *provinfo,
...
int
ossl_provider_doall_activated (OSSL_LIB_CTX *ctx,
 ...
 *
 * @param prov Pointer to the OSSL_PROVIDER structure representing the
 * provider.
 *
 ...
 * parameters.
 *
 * @return 1 on success, 0 if the `get_params` callback is not defined or
 * fails.
 *
 ...
 *
 * This function calls the `get_capabilities` callback of the specified
 * provider to retrieve capabilities information. The callback is invoked with
 * the provider context, capability name, a callback function, and an argument.
 *
 * @param prov Pointer to the OSSL_PROVIDER structure representing the
 * provider.
 ...
 */
int
ossl_provider_random_bytes (const OSSL_PROVIDER *prov, int which, void *buf,
...
int
ossl_provider_get_capabilities (const OSSL_PROVIDER *prov,
...
ossl_provider_query_operation (const OSSL_PROVIDER *prov, int operation_id,
                               int *no_cache)
{
...
int
ossl_provider_test_operation_bit (OSSL_PROVIDER *provider, size_t bitnum,
                                  int *result)
{
  size_t byte = bitnum / 8;
  unsigned char bit = (1 << (bitnum % 8)) & 0xFF;
...
static int
core_thread_start (const OSSL_CORE_HANDLE *handle,
...
static void
core_vset_error (const OSSL_CORE_HANDLE *handle, uint32_t reason,
...
static int
core_count_to_mark (const OSSL_CORE_HANDLE *handle)
{
  return ERR_count_to_mark ();
}

static void
core_indicator_get_callback (OPENSSL_CORE_CTX *libctx,
...
static void
core_self_test_get_callback (OPENSSL_CORE_CTX *libctx, OSSL_CALLBACK **cb,
                             void **cbarg)
{
...
static size_t
rand_get_entropy (const OSSL_CORE_HANDLE *handle, unsigned char **pout,
...
static size_t
rand_get_user_entropy (const OSSL_CORE_HANDLE *handle, unsigned char **pout,
                       int entropy, size_t min_len, size_t max_len)
{
...
static void
rand_cleanup_entropy (const OSSL_CORE_HANDLE *handle, unsigned char *buf,
                      size_t len)
{
...
static void
rand_cleanup_user_entropy (const OSSL_CORE_HANDLE *handle, unsigned char *buf,
                           size_t len)
{
...
static size_t
rand_get_nonce (const OSSL_CORE_HANDLE *handle, unsigned char **pout,
...
static size_t
rand_get_user_nonce (const OSSL_CORE_HANDLE *handle, unsigned char **pout,
                     size_t min_len, size_t max_len, const void *salt,
                     size_t salt_len)
{
...
static void
rand_cleanup_nonce (const OSSL_CORE_HANDLE *handle, unsigned char *buf,
                    size_t len)
{
...
static void
rand_cleanup_user_nonce (const OSSL_CORE_HANDLE *handle, unsigned char *buf,
                         size_t len)
{
...
static int
core_obj_create (const OSSL_CORE_HANDLE *prov, const char *oid, const char *sn,
                 const char *ln)
{
