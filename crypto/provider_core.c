...
static void provider_deactivate_free(OSSL_PROVIDER* prov) {
  if (prov->flag_activated) ossl_provider_deactivate(prov, 1);
...
static INFOPAIR* infopair_copy(const INFOPAIR* src) {
  ...
  if (src->name != NULL) {
    ...
    if (dest->name == NULL) goto err;
  }
  if (src->value != NULL) {
    dest->value = OPENSSL_strdup(src->value);
    if (dest->value == NULL) goto err;
  }
  return dest;
err:
...
static struct provider_store_st* get_provider_store(OSSL_LIB_CTX* libctx) {
  ...
  if (store == NULL) ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
  return store;
}
...
int ossl_provider_info_add_to_store(OSSL_LIB_CTX* libctx,
  ...
  if (store->provinfosz == 0) {
    store->provinfo =
        OPENSSL_calloc(BUILTINS_BLOCK_SIZE, sizeof(*store->provinfo));
    if (store->provinfo == NULL) goto err;
    ...
    tmpbuiltins =
        OPENSSL_realloc_array(store->provinfo, newsz, sizeof(*store->provinfo));
    if (tmpbuiltins == NULL) goto err;
...
OSSL_PROVIDER* ossl_provider_find(OSSL_LIB_CTX* libctx, const char* name,
  ...
  if ((store = get_provider_store(libctx)) != NULL) {
    ...
    tmpl.name = (char*)name;
    if (!CRYPTO_THREAD_write_lock(store->lock)) return NULL;
    sk_OSSL_PROVIDER_sort(store->providers);
    if ((i = sk_OSSL_PROVIDER_find(store->providers, &tmpl)) != -1)
      prov = sk_OSSL_PROVIDER_value(store->providers, i);
    CRYPTO_THREAD_unlock(store->lock);
    if (prov != NULL && !ossl_provider_up_ref(prov)) prov = NULL;
  }
...
static OSSL_PROVIDER* provider_new(const char* name,
  ...
  if ((prov = OPENSSL_zalloc(sizeof(*prov))) == NULL) return NULL;
...
int ossl_provider_up_ref(OSSL_PROVIDER* prov) {
  ...
  if (CRYPTO_UP_REF(&prov->refcnt, &ref) <= 0) return 0;
...
static int provider_up_ref_intern(OSSL_PROVIDER* prov, int activate) {
  if (activate) return ossl_provider_activate(prov, 1, 0);

  return ossl_provider_up_ref(prov);
}

static int provider_free_intern(OSSL_PROVIDER* prov, int deactivate) {
  if (deactivate) return ossl_provider_deactivate(prov, 1);
...
OSSL_PROVIDER* ossl_provider_new(OSSL_LIB_CTX* libctx, const char* name,
  ...
  if ((store = get_provider_store(libctx)) == NULL) return NULL;
  ...
  if (params != NULL) {
    ...
    for (i = 0; params[i].key != NULL; i++) {
      if (params[i].data_type != OSSL_PARAM_UTF8_STRING) continue;
...
int ossl_provider_add_to_store(OSSL_PROVIDER* prov, OSSL_PROVIDER** actualprov,
  ...
  if (actualprov != NULL) *actualprov = NULL;

  if ((store = get_provider_store(prov->libctx)) == NULL) return 0;
  ...
  if (idx == -1) {
    if (sk_OSSL_PROVIDER_push(store->providers, prov) == 0) goto err;
    prov->store = store;
    if (!create_provider_children(prov)) {
      sk_OSSL_PROVIDER_delete_ptr(store->providers, prov);
      goto err;
    }
    if (!retain_fallbacks) store->use_fallbacks = 0;
  }
...
int ossl_provider_set_module_path(OSSL_PROVIDER* prov,
  ...
  if ((prov->path = OPENSSL_strdup(module_path)) != NULL) return 1;
  return 0;
}
...
static int provider_init(OSSL_PROVIDER* prov) {
  ...
  if (prov->init_function == NULL) {
    ...
    if (prov->module == NULL) {
      ...
      if (store->default_path != NULL) {
        ...
        CRYPTO_THREAD_unlock(store->default_path_lock);
        if (allocated_load_dir == NULL) goto end;
      ...
      if (load_dir == NULL) {
        ...
        if (load_dir == NULL) load_dir = ossl_get_modulesdir();
      }
  ...
  if (p_get_reason_strings != NULL) {
    ...
    while (reasonstrings[cnt].id != 0) {
      if (ERR_GET_LIB(reasonstrings[cnt].id) != 0) goto end;
      cnt++;
    }
    ...
    if (prov->error_strings == NULL) goto end;
...
static int provider_deactivate(OSSL_PROVIDER* prov, int upcalls,
  ...
  if (!ossl_assert(prov != NULL)) return -1;
  ...
  if (store == NULL) lock = 0;

  if (lock && !CRYPTO_THREAD_read_lock(store->lock)) return -1;
  ...
  if (count < 1) prov->flag_activated = 0;
#ifndef FIPS_MODULE
  else
    removechildren = 0;
#endif
  ...
  if (lock) {
     ...
     */
#ifndef FIPS_MODULE
    if (count < 1) ossl_decoder_cache_flush(prov->libctx);
#endif
  }
#ifndef FIPS_MODULE
  if (freeparent) ossl_provider_free_parent(prov, 1);
#endif
...
static int provider_activate(OSSL_PROVIDER* prov, int lock, int upcalls) {
  ...
  if (store == NULL) {
    lock = 0;
    if (!provider_init(prov)) return -1;
  }
...
#ifndef FIPS_MODULE
    if (prov->ischild && upcalls) ossl_provider_free_parent(prov, 1);
#endif
    return -1;
  }
...
static int provider_remove_store_methods(OSSL_PROVIDER* prov) {
  ...
  if (!freeing) {
    ...
    if (!CRYPTO_THREAD_write_lock(prov->opbits_lock)) return 0;
    OPENSSL_free(prov->operation_bits);
    prov->operation_bits = NULL;
    prov->operation_bits_sz = 0;
...
int ossl_provider_activate(OSSL_PROVIDER* prov, int upcalls, int aschild) {
   ...
   */
  if (aschild && !prov->ischild) return 1;
#endif
...
static int provider_activate_fallbacks(struct provider_store_st* store) {
  ...
  for (p = ossl_predefined_providers; p->name != NULL; p++) {
    ...
    for (i = 0; i < store->numprovinfo; info++, i++) {
      if (strcmp(info->name, p->name) != 0) continue;
...
int ossl_provider_doall_activated(OSSL_LIB_CTX* ctx,
  ...
  if (!provider_activate_fallbacks(store)) return 0;
  ...
  for (curr = max - 1; curr >= 0; curr--) {
    ...
    if (!CRYPTO_THREAD_read_lock(prov->flag_lock)) goto err_unlock;
...
int OSSL_PROVIDER_available(OSSL_LIB_CTX* libctx, const char* name) {
  ...
  if (store == NULL || !provider_activate_fallbacks(store)) return 0;

  prov = ossl_provider_find(libctx, name, 0);
  if (prov != NULL) {
    if (!CRYPTO_THREAD_read_lock(prov->flag_lock)) return 0;
...
const OSSL_DISPATCH* ossl_provider_get0_dispatch(const OSSL_PROVIDER* prov) {
  if (prov != NULL) return prov->dispatch;
...
const OSSL_PARAM* ossl_provider_gettable_params(const OSSL_PROVIDER* prov) {
  ...
  if (prov->gettable_params != NULL) ret = prov->gettable_params(prov->provctx);
...
int ossl_provider_self_test(const OSSL_PROVIDER* prov) {
  ...
  if (prov->self_test != NULL) ret = prov->self_test(prov->provctx);
  ...
  OSSL_TRACE_END(PROVIDER);
#endif
  if (ret == 0) (void)provider_remove_store_methods((OSSL_PROVIDER*)prov);
  return ret;
}
...
int ossl_provider_set_operation_bit(OSSL_PROVIDER* provider, size_t bitnum) {
  ...
  if (!CRYPTO_THREAD_write_lock(provider->opbits_lock)) return 0;
...
int ossl_provider_test_operation_bit(OSSL_PROVIDER* provider, size_t bitnum,
  ...
  *result = 0;
  if (!CRYPTO_THREAD_read_lock(provider->opbits_lock)) return 0;
  if (provider->operation_bits_sz > byte)
...
int ossl_provider_default_props_update(OSSL_LIB_CTX* libctx,
  ...
  if ((store = get_provider_store(libctx)) == NULL) return 0;
...
static int ossl_provider_register_child_cb(
  ...
  for (i = 0; i < max; i++) {
    ...
    if (!CRYPTO_THREAD_read_lock(prov->flag_lock)) break;
     ...
     * the store lock.
     */
    if (activated && !create_cb((OSSL_CORE_HANDLE*)prov, cbdata)) break;
  }
  if (i == max) {
    /* Success */
...
static void ossl_provider_deregister_child_cb(const OSSL_CORE_HANDLE* handle) {
  ...
  if ((store = get_provider_store(libctx)) == NULL) return;

  if (!CRYPTO_THREAD_write_lock(store->lock)) return;
  max = sk_OSSL_PROVIDER_CHILD_CB_num(store->child_cbs);
  for (i = 0; i < max; i++) {
    child_cb = sk_OSSL_PROVIDER_CHILD_CB_value(store->child_cbs, i);
...
static const OSSL_DISPATCH* core_provider_get0_dispatch(
...
static int core_obj_add_sigid(const OSSL_CORE_HANDLE* prov,
  ...
  if (sign_nid == NID_undef) return 0;

  /*
   * Check if it already exists. This is a success if so (even if we don't
   * have nids for the digest/pkey)
   */
  if (OBJ_find_sigid_algs(sign_nid, NULL, NULL)) return 1;

  if (pkey_nid == NID_undef) return 0;
