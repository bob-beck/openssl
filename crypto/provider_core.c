...
OSSL_PROVIDER* ossl_provider_find(OSSL_LIB_CTX* libctx,
                                  const char* name,
...
static int infopair_add(STACK_OF(INFOPAIR)** infopairsk,
                        const char* name,
                        const char* value) {
  INFOPAIR* pair = NULL;
 ...
 * capabilities.
 */
int ossl_provider_random_bytes(const OSSL_PROVIDER* prov,
                               int which,
                               void* buf,
                               size_t n,
...
int ossl_provider_test_operation_bit(OSSL_PROVIDER* provider,
                                     size_t bitnum,
                                     int* result) {
  size_t byte = bitnum / 8;
  unsigned char bit = (1 << (bitnum % 8)) & 0xFF;
...
static const OSSL_PARAM param_types[] = {
    ...
    OSSL_PARAM_DEFN(OSSL_PROV_PARAM_CORE_PROV_NAME,
                    OSSL_PARAM_UTF8_PTR,
                    NULL,
                    0),
#ifndef FIPS_MODULE
...
static int core_obj_create(const OSSL_CORE_HANDLE* prov,
                           const char* oid,
                           const char* sn,
                           const char* ln) {
