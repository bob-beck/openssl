...
OSSL_PROVIDER*
ossl_provider_find(OSSL_LIB_CTX* libctx,
                   const char* name,
             ...
             STACK_OF(INFOPAIR)* parameters)
{
  ...
  if ((prov->opbits_lock = CRYPTO_THREAD_lock_new()) == NULL ||
      ...
      (prov->parameters = sk_INFOPAIR_deep_copy(
         parameters, infopair_copy, infopair_free)) == NULL) {
    ossl_provider_free(prov);
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_CRYPTO_LIB);
    return NULL;
  }
 ...
 */
OSSL_PROVIDER*
ossl_provider_new(OSSL_LIB_CTX* libctx,
                  const char* name,
                  OSSL_provider_init_fn* init_function,
                  OSSL_PARAM* params,
                  int noconfig)
  ...
  if (params != NULL) {
    ...
    for (i = 0; params[i].key != NULL; i++) {
      ...
      if (ossl_provider_info_add_parameter(
            &template, params[i].key, (char*)params[i].data) <= 0) {
...
int
ossl_provider_add_to_store(OSSL_PROVIDER* prov,
                           OSSL_PROVIDER** actualprov,
...
static int
infopair_add(STACK_OF(INFOPAIR)** infopairsk,
             const char* name,
             const char* value)
{
  INFOPAIR* pair = NULL;
...
int
OSSL_PROVIDER_add_conf_parameter(OSSL_PROVIDER* prov,
                                 const char* name,
                                 const char* value)
{
...
int
OSSL_PROVIDER_conf_get_bool(const OSSL_PROVIDER* prov,
                            const char* name,
                            int defval)
{
  char* val = NULL;
...
provider_init(OSSL_PROVIDER* prov)
{
  ...
  if (!prov->init_function((OSSL_CORE_HANDLE*)prov,
                           core_dispatch,
                           &provider_dispatch,
                           &tmp_provctx)) {
...
ossl_provider_teardown(const OSSL_PROVIDER* prov)
{
  if (prov->teardown != NULL
    ...
    {
      BIO_printf(
        trc_out, "(provider %s) calling teardown\n", ossl_provider_name(prov));
    }
    OSSL_TRACE_END(PROVIDER);
#endif
...
ossl_provider_gettable_params(const OSSL_PROVIDER* prov)
{
  ...
  OSSL_TRACE_BEGIN(PROVIDER)
  {
    ...
    BIO_printf(
      trc_out, "(provider %s) gettable params\n", ossl_provider_name(prov));
...
ossl_provider_self_test(const OSSL_PROVIDER* prov)
{
  ...
  OSSL_TRACE_BEGIN(PROVIDER)
  {
    if (prov->self_test != NULL)
      ...
      BIO_printf(
        trc_out, "(provider %s) doesn't implement self_test\n", prov->name);
  }
  OSSL_TRACE_END(PROVIDER);
#endif
  if (ret == 0)
 ...
 */
int
ossl_provider_random_bytes(const OSSL_PROVIDER* prov,
                           int which,
                           void* buf,
                           size_t n,
...
ossl_provider_query_operation(const OSSL_PROVIDER* prov,
                              int operation_id,
                              int* no_cache)
{
 ...
 */
void
ossl_provider_unquery_operation(const OSSL_PROVIDER* prov,
                                int operation_id,
...
int
ossl_provider_test_operation_bit(OSSL_PROVIDER* provider,
                                 size_t bitnum,
                                 int* result)
{
  size_t byte = bitnum / 8;
...
static void
core_set_error_debug(const OSSL_CORE_HANDLE* handle,
                     const char* file,
                     int line,
                     const char* func)
{
...
static void
core_vset_error(const OSSL_CORE_HANDLE* handle,
                uint32_t reason,
                const char* fmt,
                va_list args)
{
...
static void
core_self_test_get_callback(OPENSSL_CORE_CTX* libctx,
                            OSSL_CALLBACK** cb,
                            void** cbarg)
{
...
static size_t
rand_get_entropy(const OSSL_CORE_HANDLE* handle,
                 unsigned char** pout,
                 int entropy,
                 size_t min_len,
                 size_t max_len)
{
  return ossl_rand_get_entropy(
    (OSSL_LIB_CTX*)core_get_libctx(handle), pout, entropy, min_len, max_len);
}
#else
/*
 ...
 */
size_t
ossl_rand_jitter_get_seed(unsigned char**, int, size_t, size_t);
static size_t
rand_get_entropy(const OSSL_CORE_HANDLE* handle,
                 unsigned char** pout,
                 int entropy,
...
static size_t
rand_get_user_entropy(const OSSL_CORE_HANDLE* handle,
                      unsigned char** pout,
                      int entropy,
                      size_t min_len,
                      size_t max_len)
...
static void
rand_cleanup_entropy(const OSSL_CORE_HANDLE* handle,
                     unsigned char* buf,
                     size_t len)
{
...
static void
rand_cleanup_user_entropy(const OSSL_CORE_HANDLE* handle,
                          unsigned char* buf,
                          size_t len)
{
  ossl_rand_cleanup_user_entropy(
    (OSSL_LIB_CTX*)core_get_libctx(handle), buf, len);
}

static size_t
rand_get_nonce(const OSSL_CORE_HANDLE* handle,
               unsigned char** pout,
               size_t min_len,
               size_t max_len,
               const void* salt,
...
static size_t
rand_get_user_nonce(const OSSL_CORE_HANDLE* handle,
                    unsigned char** pout,
                    size_t min_len,
                    size_t max_len,
                    const void* salt,
...
static void
rand_cleanup_nonce(const OSSL_CORE_HANDLE* handle,
                   unsigned char* buf,
                   size_t len)
{
...
static void
rand_cleanup_user_nonce(const OSSL_CORE_HANDLE* handle,
                        unsigned char* buf,
                        size_t len)
{
...
static int
core_obj_add_sigid(const OSSL_CORE_HANDLE* prov,
...
static int
core_obj_create(const OSSL_CORE_HANDLE* prov,
                const char* oid,
                const char* sn,
                const char* ln)
{
