 ...
 */
static int
sh_init(size_t size, size_t minsize);
static void*
...
sh_done(void);
static size_t
sh_actual_size(char* ptr);
static int
sh_allocated(const char* ptr);
#endif

int
CRYPTO_secure_malloc_init(size_t size, size_t minsize)
{
#ifndef OPENSSL_NO_SECURE_MEMORY
  int ret = 0;
...
int
CRYPTO_secure_malloc_done(void)
{
#ifndef OPENSSL_NO_SECURE_MEMORY
...
static int
sh_testbit(char* ptr, int list, unsigned char* table)
{
  size_t bit;
...
static void
sh_clearbit(char* ptr, int list, unsigned char* table)
{
  size_t bit;
...
static void
sh_setbit(char* ptr, int list, unsigned char* table)
{
  size_t bit;
...
static void
sh_add_to_list(char** list, char* ptr)
{
  SH_LIST* temp;
...
#if !defined(_WIN32)
#ifdef MAP_ANON
  sh.map_result = mmap(NULL,
                       sh.map_size,
                       PROT_READ | PROT_WRITE,
                       MAP_ANON | MAP_PRIVATE | MAP_CONCEAL,
                       -1,
                       0);
#else
  {
...
#else
  if (VirtualProtect(
        sh.map_result + aligned, pgsize, PAGE_NOACCESS, &flOldProtect) == FALSE)
    ret = 2;
#endif
...
static char*
sh_find_my_buddy(char* ptr, int list)
{
  size_t bit;
  char* chunk = NULL;
