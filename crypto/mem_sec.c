...
int CRYPTO_secure_malloc_init(size_t size, size_t minsize) {
  ...
  if (!secure_mem_initialized) {
    ...
    if (sec_malloc_lock == NULL) return 0;
...
void CRYPTO_secure_free(void* ptr, const char* file, int line) {
  ...
  if (!CRYPTO_THREAD_write_lock(sec_malloc_lock)) return;
...
static void sh_remove_from_list(char* ptr) {
  ...
  temp = (SH_LIST*)ptr;
  if (temp->next != NULL) temp->next->p_next = temp->p_next;
...
static int sh_init(size_t size, size_t minsize) {
  ...
  if (size == 0 || (size & (size - 1)) != 0) goto err;
  ...
  if (minsize <= sizeof(SH_LIST)) {
    ...
    minsize |= minsize >> 1;
    minsize |= minsize >> 2;
    if (sizeof(SH_LIST) > 16) minsize |= minsize >> 4;
    if (sizeof(SH_LIST) > 256) minsize |= minsize >> 8;
    minsize++;
  } else {
    /* make sure minsize is a powers of 2 */
    OPENSSL_assert((minsize & (minsize - 1)) == 0);
    if ((minsize & (minsize - 1)) != 0) goto err;
  }
  ...
  if (sh.bittable_size >> 3 == 0) goto err;
  ...
  if (mprotect(sh.map_result, pgsize, PROT_NONE) < 0) ret = 2;
#else
...
#if !defined(_WIN32)
  if (mprotect(sh.map_result + aligned, pgsize, PROT_NONE) < 0) ret = 2;
#else
  ...
  if (syscall(SYS_mlock2, sh.arena, sh.arena_size, MLOCK_ONFAULT) < 0) {
    if (errno == ENOSYS) {
      if (mlock(sh.arena, sh.arena_size) < 0) ret = 2;
    } else {
      ret = 2;
    }
  }
#elif defined(_WIN32)
  if (VirtualLock(sh.arena, sh.arena_size) == FALSE) ret = 2;
#else
  if (mlock(sh.arena, sh.arena_size) < 0) ret = 2;
#endif
#ifndef NO_MADVISE
  if (madvise(sh.arena, sh.arena_size, MADV_DONTDUMP) < 0) ret = 2;
#endif
...
static void* sh_malloc(size_t size) {
  ...
  if (size > sh.arena_size) return NULL;
  ...
  for (slist = list; slist >= 0; slist--)
    if (sh.freelist[slist] != NULL) break;
