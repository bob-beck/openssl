...
static int loader_set_params(OSSL_STORE_LOADER* loader,
  ...
  if (params != NULL) {
    if (!loader->p_set_ctx_params(loader_ctx, params)) return 0;
  }
  ...
  if (propq != NULL) {
    ...
    if (!loader->p_set_ctx_params(loader_ctx, propp)) return 0;
  }
  return 1;
}
  ...
  if ((p = strchr(scheme_copy, ':')) != NULL) {
    *p++ = '\0';
    if (OPENSSL_strcasecmp(scheme_copy, "file") != 0) {
      if (HAS_PREFIX(p, "//")) schemes_n--; /* Invalidate the file scheme */
...
OSSL_STORE_INFO* OSSL_STORE_load(OSSL_STORE_CTX* ctx) {
  ...
  ctx->loading = 1;
again:
  if (OSSL_STORE_eof(ctx)) return NULL;

  if (ctx->loader != NULL) OSSL_TRACE(STORE, "Loading next object\n");
...
int OSSL_STORE_error(OSSL_STORE_CTX* ctx) {
  ...
  if (ctx->fetched_loader != NULL) ret = ctx->error_flag;
#ifndef OPENSSL_NO_DEPRECATED_3_0
  if (ctx->fetched_loader == NULL) ret = ctx->loader->error(ctx->loader_ctx);
#endif
  return ret;
}
...
int OSSL_STORE_eof(OSSL_STORE_CTX* ctx) {
  ...
  if (ctx->cached_info == NULL) {
    ret = 1;
    if (ctx->fetched_loader != NULL) ret = ctx->loader->p_eof(ctx->loader_ctx);
#ifndef OPENSSL_NO_DEPRECATED_3_0
    if (ctx->fetched_loader == NULL) ret = ctx->loader->eof(ctx->loader_ctx);
#endif
  }
  return ret != 0;
}
...
static int ossl_store_close_it(OSSL_STORE_CTX* ctx) {
  ...
  if (ctx->fetched_loader != NULL) ret = ctx->loader->p_close(ctx->loader_ctx);
#ifndef OPENSSL_NO_DEPRECATED_3_0
  if (ctx->fetched_loader == NULL) ret = ctx->loader->closefn(ctx->loader_ctx);
#endif
...
void* OSSL_STORE_INFO_get0_data(int type, const OSSL_STORE_INFO* info) {
  if (info->type == type) return info->_.data;
  return NULL;
}

const char* OSSL_STORE_INFO_get0_NAME(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_NAME) return info->_.name.name;
  return NULL;
}
...
const char* OSSL_STORE_INFO_get0_NAME_description(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_NAME) return info->_.name.desc;
  return NULL;
}
...
EVP_PKEY* OSSL_STORE_INFO_get0_PARAMS(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_PARAMS) return info->_.params;
  return NULL;
}

EVP_PKEY* OSSL_STORE_INFO_get1_PARAMS(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_PARAMS) {
    if (!EVP_PKEY_up_ref(info->_.params)) return NULL;
...
EVP_PKEY* OSSL_STORE_INFO_get0_PUBKEY(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_PUBKEY) return info->_.pubkey;
  return NULL;
}

EVP_PKEY* OSSL_STORE_INFO_get1_PUBKEY(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_PUBKEY) {
    if (!EVP_PKEY_up_ref(info->_.pubkey)) return NULL;
...
EVP_PKEY* OSSL_STORE_INFO_get0_PKEY(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_PKEY) return info->_.pkey;
  return NULL;
}

EVP_PKEY* OSSL_STORE_INFO_get1_PKEY(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_PKEY) {
    if (!EVP_PKEY_up_ref(info->_.pkey)) return NULL;
    return info->_.pkey;
  }
...
X509* OSSL_STORE_INFO_get0_CERT(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_CERT) return info->_.x509;
  return NULL;
}

X509* OSSL_STORE_INFO_get1_CERT(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_CERT) {
    if (!X509_up_ref(info->_.x509)) return NULL;
    return info->_.x509;
  }
...
X509_CRL* OSSL_STORE_INFO_get0_CRL(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_CRL) return info->_.crl;
  return NULL;
}

X509_CRL* OSSL_STORE_INFO_get1_CRL(const OSSL_STORE_INFO* info) {
  if (info->type == OSSL_STORE_INFO_CRL) {
    if (!X509_CRL_up_ref(info->_.crl)) return NULL;
    return info->_.crl;
  }
...
int OSSL_STORE_supports_search(OSSL_STORE_CTX* ctx, int search_type) {
  ...
  if (ctx->fetched_loader == NULL) {
    ...
    if (ctx->loader->find == NULL) return 0;
...
const EVP_MD* OSSL_STORE_SEARCH_get0_digest(
    const OSSL_STORE_SEARCH* criterion) {
