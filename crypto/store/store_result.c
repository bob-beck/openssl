...
static EVP_PKEY *
try_key_ref (struct extracted_param_data_st *data, OSSL_STORE_CTX *ctx,
             const OSSL_PROVIDER *provider, OSSL_LIB_CTX *libctx,
             const char *propq)
{
  EVP_PKEY *pk = NULL;
...
static EVP_PKEY *
try_key_value (struct extracted_param_data_st *data, OSSL_STORE_CTX *ctx,
               OSSL_PASSPHRASE_CALLBACK *cb, void *cbarg, OSSL_LIB_CTX *libctx,
               const char *propq, int *harderr)
{
  EVP_PKEY *pk = NULL;
...
try_key_value_legacy (struct extracted_param_data_st *data,
                      store_info_new_fn **store_info_new, OSSL_STORE_CTX *ctx,
      ...
      if (p8 != NULL)
        {
          ...
          if (!cb (pbuf, sizeof (pbuf), &plen, NULL, cbarg))
          ...
          else
            {
               ...
               */
              PKCS12_pbe_crypt (alg, pbuf, (int)plen, oct->data, oct->length,
                                &new_der, &len, 0);
              der_len = len;
              der = new_der;
            }
...
static int
try_key (struct extracted_param_data_st *data, OSSL_STORE_INFO **v,
...
static int
try_cert (struct extracted_param_data_st *data, OSSL_STORE_INFO **v,
          OSSL_LIB_CTX *libctx, const char *propq)
{
  if (data->object_type == OSSL_OBJECT_UNKNOWN
...
static int
try_crl (struct extracted_param_data_st *data, OSSL_STORE_INFO **v,
         OSSL_LIB_CTX *libctx, const char *propq)
{
  if (data->object_type == OSSL_OBJECT_UNKNOWN
...
static int
try_pkcs12 (struct extracted_param_data_st *data, OSSL_STORE_INFO **v,
            OSSL_STORE_CTX *ctx, OSSL_LIB_CTX *libctx, const char *propq)
{
  int ok = 1;
