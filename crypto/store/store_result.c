...
static int
try_name(struct extracted_param_data_st*, OSSL_STORE_INFO**);
static int
try_key(struct extracted_param_data_st*,
        OSSL_STORE_INFO**,
        OSSL_STORE_CTX*,
        const OSSL_PROVIDER*,
        OSSL_LIB_CTX*,
        const char*);
static int
try_cert(struct extracted_param_data_st*,
         OSSL_STORE_INFO**,
         OSSL_LIB_CTX*,
         const char*);
static int
try_crl(struct extracted_param_data_st*,
        OSSL_STORE_INFO**,
        OSSL_LIB_CTX*,
        const char*);
static int
try_pkcs12(struct extracted_param_data_st*,
           OSSL_STORE_INFO**,
           OSSL_STORE_CTX*,
           OSSL_LIB_CTX*,
           const char*);
...
int
ossl_store_handle_load_result(const OSSL_PARAM params[], void* arg)
{
  ...
  if (p != NULL &&
      !OSSL_PARAM_get_octet_string_ptr(
        p, &helper_data.octet_data, &helper_data.octet_data_size) &&
...
static int
try_name(struct extracted_param_data_st* data, OSSL_STORE_INFO** v)
{
...
static EVP_PKEY*
try_key_ref(struct extracted_param_data_st* data,
            OSSL_STORE_CTX* ctx,
            const OSSL_PROVIDER* provider,
            OSSL_LIB_CTX* libctx,
            const char* propq)
  ...
  while (keymgmt != NULL && keydata == NULL && try_fallback-- > 0) {
    ...
    if (keydata == NULL && try_fallback > 0) {
      ...
      keymgmt = evp_keymgmt_fetch_from_prov(
        (OSSL_PROVIDER*)provider, data->data_type, propq);
      if (keymgmt != NULL) {
        ERR_pop_to_mark();
        ERR_set_mark();
      }
...
static EVP_PKEY*
try_key_value(struct extracted_param_data_st* data,
              OSSL_STORE_CTX* ctx,
              ...
              int* harderr)
{
  ...
  decoderctx = OSSL_DECODER_CTX_new_for_pkey(&pk,
                                             data->input_type,
                                             data->data_structure,
                                             data->data_type,
                                             selection,
...
static EVP_PKEY*
try_key_value_legacy(struct extracted_param_data_st* data,
  ...
  if (pk == NULL &&
    ...
    if (p8 != NULL) {
      ...
      if (!cb(pbuf, sizeof(pbuf), &plen, NULL, cbarg)) {
         ...
         */
        PKCS12_pbe_crypt(
          alg, pbuf, (int)plen, oct->data, oct->length, &new_der, &len, 0);
        der_len = len;
        der = new_der;
      }
      X509_SIG_free(p8);
...
static int
try_key(struct extracted_param_data_st* data,
        OSSL_STORE_INFO** v,
        OSSL_STORE_CTX* ctx,
        const OSSL_PROVIDER* provider,
        OSSL_LIB_CTX* libctx,
  ...
  if (data->object_type == OSSL_OBJECT_UNKNOWN ||
    ...
    if (data->object_type == OSSL_OBJECT_PKEY && data->ref != NULL) {
      ...
      if (pk == NULL && harderr == 0)
        pk = try_key_value_legacy(
          data, &store_info_new, ctx, cb, cbarg, libctx, propq);
    }
...
static int
try_cert(struct extracted_param_data_st* data,
         OSSL_STORE_INFO** v,
         OSSL_LIB_CTX* libctx,
         const char* propq)
{
...
static int
try_crl(struct extracted_param_data_st* data,
        OSSL_STORE_INFO** v,
        OSSL_LIB_CTX* libctx,
        const char* propq)
{
...
static int
try_pkcs12(struct extracted_param_data_st* data,
           OSSL_STORE_INFO** v,
           OSSL_STORE_CTX* ctx,
           OSSL_LIB_CTX* libctx,
           const char* propq)
      ...
      if (!PKCS12_mac_present(p12) || PKCS12_verify_mac(p12, NULL, 0)) {
        ...
        OSSL_PARAM pw_params[] = {
          OSSL_PARAM_utf8_string(
            OSSL_PASSPHRASE_PARAM_INFO, prompt_info, sizeof(prompt_info) - 1),
          OSSL_PARAM_END
        };

        if (!ossl_pw_get_passphrase(tpass,
                                    sizeof(tpass) - 1,
                                    &tpass_len,
                                    pw_params,
                                    0,
