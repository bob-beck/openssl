...
int ossl_store_handle_load_result(const OSSL_PARAM params[], void* arg) {
   ...
   */
  ERR_set_mark();
  if (*v == NULL && !try_name(&helper_data, v)) goto err;
  ERR_pop_to_mark();
  ERR_set_mark();
  ...
  ERR_pop_to_mark();
  ERR_set_mark();
  if (*v == NULL && !try_cert(&helper_data, v, libctx, propq)) goto err;
  ERR_pop_to_mark();
  ERR_set_mark();
  if (*v == NULL && !try_crl(&helper_data, v, libctx, propq)) goto err;
  ERR_pop_to_mark();
  ERR_set_mark();
  if (*v == NULL && !try_pkcs12(&helper_data, v, ctx, libctx, propq)) goto err;
  ERR_pop_to_mark();
...
static int try_name(struct extracted_param_data_st* data, OSSL_STORE_INFO** v) {
  if (data->object_type == OSSL_OBJECT_NAME) {
    ...
    if (data->utf8_data == NULL) return 0;
...
static EVP_PKEY* try_key_ref(struct extracted_param_data_st* data,
  ...
  if (data->data_type == NULL) return 0;
...
static EVP_PKEY* try_key_value_legacy(struct extracted_param_data_st* data,
  ...
  if (ctx->expected_type == 0 || ctx->expected_type == OSSL_STORE_INFO_PUBKEY) {
    ...
    if (pk != NULL) *store_info_new = OSSL_STORE_INFO_new_PUBKEY;
  }
  ...
  if (pk == NULL &&
    ...
    if (p8 != NULL) {
      ...
      if (!cb(pbuf, sizeof(pbuf), &plen, NULL, cbarg)) {
         ...
         */
        PKCS12_pbe_crypt(alg, pbuf, (int)plen, oct->data, oct->length, &new_der,
                         &len, 0);
        der_len = len;
        der = new_der;
      }
    ...
    if (pk != NULL) *store_info_new = OSSL_STORE_INFO_new_PKEY;
...
static int try_crl(struct extracted_param_data_st* data, OSSL_STORE_INFO** v,
  ...
  if (data->object_type == OSSL_OBJECT_UNKNOWN ||
    ...
    if (crl != NULL) /* We determined the object type */
      data->object_type = OSSL_OBJECT_CRL;
    ...
    if (crl != NULL) *v = OSSL_STORE_INFO_new_CRL(crl);
    if (*v == NULL) X509_CRL_free(crl);
  }
