...
int OSSL_STORE_LOADER_up_ref(OSSL_STORE_LOADER* loader) {
  ...
  if (loader->prov != NULL) CRYPTO_UP_REF(&loader->refcnt, &ref);
  return 1;
}
...
static OSSL_STORE_LOADER* new_loader(OSSL_PROVIDER* prov) {
  ...
  if ((loader = OPENSSL_zalloc(sizeof(*loader))) == NULL ||
    ...
    if (loader != NULL) CRYPTO_FREE_REF(&loader->refcnt);
    OPENSSL_free(loader);
    return NULL;
  }
...
static void dealloc_tmp_loader_store(void* store) {
  if (store != NULL) ossl_method_store_free(store);
}
...
static void* loader_from_algorithm(int scheme_id, const OSSL_ALGORITHM* algodef,
  ...
  if ((loader = new_loader(prov)) == NULL) return NULL;
  ...
  for (; fns->function_id != 0; fns++) {
    switch (fns->function_id) {
      case OSSL_FUNC_STORE_OPEN:
        if (loader->p_open == NULL) loader->p_open = OSSL_FUNC_store_open(fns);
        break;
      ...
      case OSSL_FUNC_STORE_LOAD:
        if (loader->p_load == NULL) loader->p_load = OSSL_FUNC_store_load(fns);
        break;
      case OSSL_FUNC_STORE_EOF:
        if (loader->p_eof == NULL) loader->p_eof = OSSL_FUNC_store_eof(fns);
        break;
...
static void* construct_loader(const OSSL_ALGORITHM* algodef,
  ...
  if (id != 0) method = loader_from_algorithm(id, algodef, prov);
   ...
   */
  if (method == NULL) methdata->flag_construct_error_occurred = 1;
...
static OSSL_STORE_LOADER* inner_loader_fetch(struct loader_data_st* methdata,
  ...
  if (id == 0 ||
    ...
    if ((method = ossl_method_construct(methdata->libctx, OSSL_OP_STORE, &prov,
                                        0 /* !force_cache */, &mcm,
                                        methdata)) != NULL) {
      /*
       ...
       */
      if (id == 0) id = ossl_namemap_name2num(namemap, scheme);
  ...
  if ((id != 0 || scheme != NULL) && method == NULL) {
    ...
    if (scheme == NULL) scheme = ossl_namemap_num2name(namemap, id, 0);
...
int ossl_store_loader_store_cache_flush(OSSL_LIB_CTX* libctx) {
  ...
  if (store != NULL) return ossl_method_store_cache_flush_all(store);
  return 1;
}
...
int ossl_store_loader_store_remove_all_provided(const OSSL_PROVIDER* prov) {
  ...
  if (store != NULL) return ossl_method_store_remove_all_provided(store, prov);
  return 1;
}
...
const char* OSSL_STORE_LOADER_get0_description(
    const OSSL_STORE_LOADER* loader) {
...
const OSSL_PARAM* OSSL_STORE_LOADER_settable_ctx_params(
    const OSSL_STORE_LOADER* loader) {
