...
int
OSSL_STORE_LOADER_up_ref (OSSL_STORE_LOADER *loader)
{
  int ref = 0;
...
void
OSSL_STORE_LOADER_free (OSSL_STORE_LOADER *loader)
{
...
static int
up_ref_loader (void *method)
{
  return OSSL_STORE_LOADER_up_ref (method);
}

static void
free_loader (void *method)
{
...
static void *
get_tmp_loader_store (void *data)
{
...
static void
dealloc_tmp_loader_store (void *store)
{
  if (store != NULL)
...
get_loader_store (OSSL_LIB_CTX *libctx)
{
  return ossl_lib_ctx_get_data (libctx, OSSL_LIB_CTX_STORE_LOADER_STORE_INDEX);
}

static int
reserve_loader_store (void *store, void *data)
{
  struct loader_data_st *methdata = data;
...
static int
unreserve_loader_store (void *store, void *data)
{
  struct loader_data_st *methdata = data;
...
static void *
get_loader_from_store (void *store, const OSSL_PROVIDER **prov, void *data)
{
  struct loader_data_st *methdata = data;
  void *method = NULL;
  int id;
...
static int
put_loader_in_store (void *store, void *method, const OSSL_PROVIDER *prov,
                     const char *scheme, const char *propdef, void *data)
{
  struct loader_data_st *methdata = data;
...
static void *
loader_from_algorithm (int scheme_id, const OSSL_ALGORITHM *algodef,
                       OSSL_PROVIDER *prov)
{
 ...
 */
static void *
construct_loader (const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov,
                  void *data)
{
  /*
...
static void
destruct_loader (void *method, void *data)
{
  OSSL_STORE_LOADER_free (method);
}
...
static OSSL_STORE_LOADER *
inner_loader_fetch (struct loader_data_st *methdata, const char *scheme,
  ...
  if ((id != 0 || scheme != NULL) && method == NULL)
    {
      ...
      const char *helpful_msg
          = unsupported
                ? ("No store loader found. For standard store loaders you "
                   "need "
...
OSSL_STORE_LOADER *
OSSL_STORE_LOADER_fetch (OSSL_LIB_CTX *libctx, const char *scheme,
                         const char *properties)
{
...
int
ossl_store_loader_store_cache_flush (OSSL_LIB_CTX *libctx)
{
...
int
ossl_store_loader_store_remove_all_provided (const OSSL_PROVIDER *prov)
{
  OSSL_LIB_CTX *libctx = ossl_provider_libctx (prov);
  OSSL_METHOD_STORE *store = get_loader_store (libctx);
...
int
ossl_store_loader_get_number (const OSSL_STORE_LOADER *loader)
{
  if (!ossl_assert (loader != NULL))
    {
      ERR_raise (ERR_LIB_OSSL_STORE, ERR_R_PASSED_NULL_PARAMETER);
...
int
OSSL_STORE_LOADER_is_a (const OSSL_STORE_LOADER *loader, const char *name)
{
  if (loader->prov != NULL)
    {
...
void
OSSL_STORE_LOADER_do_all_provided (OSSL_LIB_CTX *libctx,
...
int
OSSL_STORE_LOADER_names_do_all (const OSSL_STORE_LOADER *loader,
