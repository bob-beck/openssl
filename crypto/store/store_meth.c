...
static int
put_loader_in_store(void* store,
                    void* method,
                    ...
                    void* data)
{
  ...
  return ossl_method_store_add(
    store, prov, id, propdef, method, up_ref_loader, free_loader);
}

static void*
loader_from_algorithm(int scheme_id,
 ...
 */
static void*
construct_loader(const OSSL_ALGORITHM* algodef, OSSL_PROVIDER* prov, void* data)
{
  /*
  ...
  if (id == 0 ||
    ...
    if ((method = ossl_method_construct(methdata->libctx,
      ...
      ossl_method_store_cache_set(
        store, prov, id, propq, method, up_ref_loader, free_loader);
    }
...
OSSL_STORE_LOADER*
OSSL_STORE_LOADER_fetch(OSSL_LIB_CTX* libctx,
                        const char* scheme,
                        const char* properties)
{
