...
int CRYPTO_set_mem_functions(CRYPTO_malloc_fn malloc_fn,
  ...
  if (malloc_fn != NULL) malloc_impl = malloc_fn;
  if (realloc_fn != NULL) realloc_impl = realloc_fn;
  if (free_fn != NULL) free_impl = free_fn;
  return 1;
}
...
void CRYPTO_get_mem_functions(CRYPTO_malloc_fn* malloc_fn,
                              CRYPTO_realloc_fn* realloc_fn,
                              CRYPTO_free_fn* free_fn) {
  if (malloc_fn != NULL) *malloc_fn = malloc_impl;
  if (realloc_fn != NULL) *realloc_fn = realloc_impl;
  if (free_fn != NULL) *free_fn = free_impl;
}

#if !defined(OPENSSL_NO_CRYPTO_MDEBUG) && !defined(FIPS_MODULE)
void CRYPTO_get_alloc_counts(int* mcount, int* rcount, int* fcount) {
  if (mcount != NULL) *mcount = LOAD(malloc_count);
  if (rcount != NULL) *rcount = LOAD(realloc_count);
  if (fcount != NULL) *fcount = LOAD(free_count);
}
...
static void parseit(void) {
  ...
  if (semi != NULL) md_failstring = semi;
}
...
static int shouldfail(void) {
  ...
  if (md_tracefd > 0) {
    ...
    len = strlen(buff);
    if (write(md_tracefd, buff, len) != len) perror("shouldfail write failed");
  }
#endif

  if (md_count) {
    /* If we used up this one, go to the next. */
    if (--md_count == 0) parseit();
  }
...
void ossl_malloc_setup_failures(void) {
  ...
  if ((cp = getenv("OPENSSL_MALLOC_FD")) != NULL) md_tracefd = atoi(cp);
  if ((cp = getenv("OPENSSL_MALLOC_SEED")) != NULL) srandom(atoi(cp));
}
#endif
...
void* CRYPTO_malloc(size_t num, const char* file, int line) {
  ...
  if (ossl_unlikely(num == 0)) return NULL;
...
void* CRYPTO_realloc(void* str, size_t num, const char* file, int line) {
  ...
  if (realloc_impl != CRYPTO_realloc) {
    ...
    if (num == 0 || ret != NULL) return ret;

    goto err;
  }

  if (str == NULL) return CRYPTO_malloc(num, file, line);
...
err:
  if (num != 0 && ret == NULL) ossl_report_alloc_err(file, line);
...
void CRYPTO_clear_free(void* str, size_t num, const char* file, int line) {
  if (str == NULL) return;
  if (num) OPENSSL_cleanse(str, num);
  CRYPTO_free(str, file, line);
}
