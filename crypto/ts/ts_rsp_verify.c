...
int TS_RESP_verify_signature(PKCS7* token, STACK_OF(X509)* certs,
  ...
  if (untrusted == NULL || !X509_add_certs(untrusted, certs, 0) ||
    ...
    goto err;
  if (!ts_verify_cert(store, untrusted, signer, &chain)) goto err;
...
static int ts_verify_cert(X509_STORE* store, STACK_OF(X509)* untrusted,
  ...
  *chain = NULL;
  ...
  if (!X509_STORE_CTX_init(cert_ctx, store, signer, untrusted)) goto end;
...
static ESS_SIGNING_CERT* ossl_ess_get_signing_cert(
...
static ESS_SIGNING_CERT_V2* ossl_ess_get_signing_cert_v2(
    const PKCS7_SIGNER_INFO* si) {
  ASN1_TYPE* attr;
  const unsigned char* p;
...
int TS_RESP_verify_response(TS_VERIFY_CTX* ctx, TS_RESP* response) {
  ...
  if (!int_ts_RESP_verify_token(ctx, token, tst_info)) goto err;
  ret = 1;
...
static int ts_check_status_info(TS_RESP* response) {
  ...
  if (failure_text[0] == '\0') strcpy(failure_text, "unspecified");
...
static int ts_compute_imprint(BIO* data, TS_TST_INFO* tst_info,
  ...
  *imprint = NULL;
  ...
  if ((*md_alg = X509_ALGOR_dup(md_alg_resp)) == NULL) goto err;
  ...
  if (md == NULL) md = (EVP_MD*)EVP_get_digestbyname(name);
  ...
  *imprint_len = length;
  if ((*imprint = OPENSSL_malloc(*imprint_len)) == NULL) goto err;
  ...
  if (!EVP_DigestFinal(md_ctx, *imprint, NULL)) goto err;
...
err:
  if (!ret) ERR_raise(ERR_LIB_TS, TS_R_MESSAGE_IMPRINT_MISMATCH);
  return ret;
}
