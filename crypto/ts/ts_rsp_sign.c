...
int TS_RESP_CTX_set_signer_key(TS_RESP_CTX* ctx, EVP_PKEY* key) {
  if (!EVP_PKEY_up_ref(key)) return 0;
...
int TS_RESP_CTX_set_def_policy(TS_RESP_CTX* ctx,
  ...
  ASN1_OBJECT_free(ctx->default_policy);
  if ((ctx->default_policy = OBJ_dup(def_policy)) == NULL) goto err;
  return 1;
err:
...
int TS_RESP_CTX_add_md(TS_RESP_CTX* ctx, const EVP_MD* md) {
  if (ctx->mds == NULL && (ctx->mds = sk_EVP_MD_new_null()) == NULL) goto err;
...
int TS_RESP_CTX_add_failure_info(TS_RESP_CTX* ctx, int failure) {
  ...
  if (si->failure_info == NULL &&
    ...
    goto err;
  if (!ASN1_BIT_STRING_set_bit(si->failure_info, failure, 1)) goto err;
  return 1;
err:
  ERR_raise(ERR_LIB_TS, ERR_R_ASN1_LIB);
  return 0;
...
int TS_RESP_CTX_set_clock_precision_digits(TS_RESP_CTX* ctx,
                                           unsigned precision) {
  if (precision > TS_MAX_CLOCK_PRECISION_DIGITS) return 0;
...
TS_RESP* TS_RESP_create_response(TS_RESP_CTX* ctx, BIO* req_bio) {
  ...
  if (!TS_RESP_CTX_set_status_info(ctx, TS_STATUS_GRANTED, NULL)) goto end;
  if (!ts_RESP_check_request(ctx)) goto end;
  if ((policy = ts_RESP_get_policy(ctx)) == NULL) goto end;
  if ((ctx->tst_info = ts_RESP_create_tst_info(ctx, policy)) == NULL) goto end;
...
static int ts_RESP_check_request(TS_RESP_CTX* ctx) {
  ...
  for (i = 0; !md && i < sk_EVP_MD_num(ctx->mds); ++i) {
    ...
    if (EVP_MD_is_a(current_md, md_alg_name)) md = current_md;
  }
  if (!md) {
    TS_RESP_CTX_set_status_info(ctx, TS_STATUS_REJECTION,
...
static ASN1_OBJECT* ts_RESP_get_policy(TS_RESP_CTX* ctx) {
  ...
  for (i = 0; !policy && i < sk_ASN1_OBJECT_num(ctx->policies); ++i) {
    ...
    if (!OBJ_cmp(requested, current)) policy = current;
  }
  if (policy == NULL) {
...
static TS_TST_INFO* ts_RESP_create_tst_info(TS_RESP_CTX* ctx,
  ...
  if ((tst_info = TS_TST_INFO_new()) == NULL) goto end;
  ...
  if (ctx->seconds && !TS_ACCURACY_set_seconds(accuracy, ctx->seconds))
    goto end;
  if (ctx->millis && !TS_ACCURACY_set_millis(accuracy, ctx->millis)) goto end;
  if (ctx->micros && !TS_ACCURACY_set_micros(accuracy, ctx->micros)) goto end;
  if (accuracy && !TS_TST_INFO_set_accuracy(tst_info, accuracy)) goto end;
  ...
  if (ctx->flags & TS_TSA_NAME) {
    if ((tsa_name = GENERAL_NAME_new()) == NULL) goto end;
...
static int ts_RESP_sign(TS_RESP_CTX* ctx) {
  ...
  ret = 1;
err:
  if (signer_md != ctx->signer_md) EVP_MD_free(signer_md);
...
static int ts_TST_INFO_content_new(PKCS7* p7) {
  ...
  if ((ret->d.other = ASN1_TYPE_new()) == NULL) goto err;
  ret->type = OBJ_nid2obj(NID_id_smime_ct_TSTInfo);
  if ((octet_string = ASN1_OCTET_STRING_new()) == NULL) goto err;
...
static ASN1_GENERALIZEDTIME* TS_RESP_set_genTime_with_precision(
    ASN1_GENERALIZEDTIME* asn1_time, long sec, long usec, unsigned precision) {
  ...
  if (precision > TS_MAX_CLOCK_PRECISION_DIGITS) goto err;

  if ((tm = OPENSSL_gmtime(&time_sec, &tm_result)) == NULL) goto err;
