...
int TS_RESP_CTX_set_status_info(TS_RESP_CTX *ctx, int status, const char *text)
{
    ...
    if (text) {
        if ((utf8_text = ASN1_UTF8STRING_new()) == NULL || !ASN1_STRING_set(utf8_text, text, (int)strlen(text))) {
            ERR_raise(ERR_LIB_TS, ERR_R_ASN1_LIB);
            goto err;
        }
...
TS_RESP_set_genTime_with_precision(ASN1_GENERALIZEDTIME *asn1_time, long sec, long usec, unsigned precision)
{
