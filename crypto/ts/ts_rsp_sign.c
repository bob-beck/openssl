...
static ASN1_INTEGER*
def_serial_cb(struct TS_resp_ctx*, void*);
static int
def_time_cb(struct TS_resp_ctx*, void*, long* sec, long* usec);
static int
def_extension_cb(struct TS_resp_ctx*, X509_EXTENSION*, void*);
...
def_time_cb(struct TS_resp_ctx* ctx, void* data, long* sec, long* usec)
{
  ...
  if (ossl_time_is_zero(t)) {
    ...
    TS_RESP_CTX_set_status_info(
      ctx, TS_STATUS_REJECTION, "Time is not available.");
...
{
  TS_RESP_CTX_set_status_info(
    ctx, TS_STATUS_REJECTION, "Unsupported extension.");
...
TS_RESP_CTX*
TS_RESP_CTX_new_ex(OSSL_LIB_CTX* libctx, const char* propq)
{
  TS_RESP_CTX* ctx;
...
int
TS_RESP_CTX_set_signer_digest(TS_RESP_CTX* ctx, const EVP_MD* md)
{
  ctx->signer_md = md;
  return 1;
}
...
TS_RESP_create_response(TS_RESP_CTX* ctx, BIO* req_bio)
{
  ...
  if ((ctx->request = d2i_TS_REQ_bio(req_bio, NULL)) == NULL) {
    TS_RESP_CTX_set_status_info(
      ctx, TS_STATUS_REJECTION, "Bad request format or system error.");
...
ts_RESP_check_request(TS_RESP_CTX* ctx)
{
  ...
  if (TS_REQ_get_version(request) != 1) {
    TS_RESP_CTX_set_status_info(
      ctx, TS_STATUS_REJECTION, "Bad request version.");
  ...
  if (digest->length != md_size) {
    TS_RESP_CTX_set_status_info(
      ctx, TS_STATUS_REJECTION, "Bad message digest.");
    TS_RESP_CTX_add_failure_info(ctx, TS_INFO_BAD_DATA_FORMAT);
    return 0;
  }
...
static int
ossl_ess_add1_signing_cert(PKCS7_SIGNER_INFO* si, const ESS_SIGNING_CERT* sc)
{
  ...
  if (!PKCS7_add_signed_attribute(
        si, NID_id_smime_aa_signingCertificate, V_ASN1_SEQUENCE, seq)) {
    ASN1_STRING_free(seq);
    return 0;
  }
  return 1;
...
static int
ossl_ess_add1_signing_cert_v2(PKCS7_SIGNER_INFO* si,
                              const ESS_SIGNING_CERT_V2* sc)
{
  ...
  OPENSSL_free(pp);
  if (!PKCS7_add_signed_attribute(
        si, NID_id_smime_aa_signingCertificateV2, V_ASN1_SEQUENCE, seq)) {
    ASN1_STRING_free(seq);
    return 0;
  }
  return 1;
...
ts_RESP_sign(TS_RESP_CTX* ctx)
{
  ...
  if ((si = PKCS7_add_signature(
         p7, ctx->signer_cert, ctx->signer_key, signer_md)) == NULL) {
  ...
  if (!PKCS7_add_signed_attribute(
        si, NID_pkcs9_contentType, V_ASN1_OBJECT, oid)) {
...
int
TS_RESP_CTX_set_ess_cert_id_digest(TS_RESP_CTX* ctx, const EVP_MD* md)
{
