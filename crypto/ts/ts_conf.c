...
X509* TS_CONF_load_cert(const char* file) {
  ...
  if (x == NULL) ERR_raise(ERR_LIB_TS, TS_R_CANNOT_LOAD_CERT);
  BIO_free(cert);
  return x;
}
...
STACK_OF(X509)* TS_CONF_load_certs(const char* file) {
...
#endif
    goto end;
  if ((othercerts = sk_X509_new_null()) == NULL) goto end;
  ...
  if (othercerts == NULL) ERR_raise(ERR_LIB_TS, TS_R_CANNOT_LOAD_CERT);
...
EVP_PKEY* TS_CONF_load_key(const char* file, const char* pass) {
  ...
  if (pkey == NULL) ERR_raise(ERR_LIB_TS, TS_R_CANNOT_LOAD_KEY);
  BIO_free(key);
  return pkey;
}
...
const char* TS_CONF_get_tsa_section(CONF* conf, const char* section) {
  if (!section) {
    ...
    if (!section) ts_CONF_lookup_fail(BASE_SECTION, ENV_DEFAULT_TSA);
  }
  return section;
}
...
int TS_CONF_set_default_engine(const char* name) {
  ...
  if ((e = ENGINE_by_id(name)) == NULL) goto err;
...
int TS_CONF_set_signer_cert(CONF* conf, const char* section, const char* cert,
  ...
  if ((cert_obj = TS_CONF_load_cert(cert)) == NULL) goto err;
  if (!TS_RESP_CTX_set_signer_cert(ctx, cert_obj)) goto err;
...
int TS_CONF_set_certs(CONF* conf, const char* section, const char* certs,
  ...
  if (certs == NULL) {
    ...
    if ((certs = NCONF_get_string(conf, section, ENV_CERTS)) == NULL) goto end;
  }
  if ((certs_obj = TS_CONF_load_certs(certs)) == NULL) goto err;
...
int TS_CONF_set_signer_key(CONF* conf, const char* section, const char* key,
  ...
  if (!key) key = NCONF_get_string(conf, section, ENV_SIGNER_KEY);
  if (!key) {
    ts_CONF_lookup_fail(section, ENV_SIGNER_KEY);
    goto err;
  }
  if ((key_obj = TS_CONF_load_key(key, pass)) == NULL) goto err;
...
int TS_CONF_set_signer_digest(CONF* conf, const char* section, const char* md,
  ...
  if (md == NULL) md = NCONF_get_string(conf, section, ENV_SIGNER_DIGEST);
  if (md == NULL) {
  ...
  if (!TS_RESP_CTX_set_signer_digest(ctx, sign_md)) goto err;
...
int TS_CONF_set_def_policy(CONF* conf, const char* section, const char* policy,
  ...
  if (!TS_RESP_CTX_set_def_policy(ctx, policy_obj)) goto err;
...
int TS_CONF_set_policies(CONF* conf, const char* section, TS_RESP_CTX* ctx) {
  ...
  for (i = 0; i < sk_CONF_VALUE_num(list); ++i) {
    ...
    if (!TS_RESP_CTX_add_policy(ctx, objtmp)) goto err;
...
int TS_CONF_set_digests(CONF* conf, const char* section, TS_RESP_CTX* ctx) {
  ...
  for (i = 0; i < sk_CONF_VALUE_num(list); ++i) {
    ...
    if (!TS_RESP_CTX_add_md(ctx, md)) goto err;
  }
...
int TS_CONF_set_accuracy(CONF* conf, const char* section, TS_RESP_CTX* ctx) {
  ...
  if (!TS_RESP_CTX_set_accuracy(ctx, secs, millis, micros)) goto err;
...
int TS_CONF_set_clock_precision_digits(const CONF* conf, const char* section,
  ...
  if (!TS_RESP_CTX_set_clock_precision_digits(ctx, digits)) goto err;
...
int TS_CONF_set_ess_cert_id_digest(CONF* conf, const char* section,
  ...
  if (!TS_RESP_CTX_set_ess_cert_id_digest(ctx, cert_md)) goto err;
