...
static int provider_conf_params_internal(OSSL_PROVIDER* prov,
  ...
  if (sect != NULL) {
     ...
     */
    if (!sk_OPENSSL_CSTRING_push(visited, value)) return -1;
...
static int provider_conf_activate(OSSL_LIB_CTX* libctx, const char* name,
  ...
  if (!prov_already_activated(name, pcgbl->activated_providers)) {
    ...
    if (prov == NULL) prov = ossl_provider_new(libctx, name, NULL, NULL, 1);
    if (prov == NULL) {
      CRYPTO_THREAD_unlock(pcgbl->lock);
      if (soft) ERR_clear_error();
      return (soft == 0) ? -1 : 0;
    }

    if (path != NULL) ossl_provider_set_module_path(prov, path);
    ...
    if (ok <= 0) ossl_provider_free(prov);
  }
  CRYPTO_THREAD_unlock(pcgbl->lock);
...
static int provider_conf_load(OSSL_LIB_CTX* libctx, const char* name,
  ...
  if (activate) {
    ...
    if (name != NULL) {
      ...
      if (entry.name == NULL) ok = 0;
    }
    if (ok && path != NULL) {
      entry.path = OPENSSL_strdup(path);
      if (entry.path == NULL) ok = 0;
    }
    if (ok) ok = provider_conf_params(NULL, &entry, NULL, value, cnf);
    if (ok >= 1 && (entry.path != NULL || entry.parameters != NULL)) {
      ok = ossl_provider_info_add_to_store(libctx, &entry);
      added = ok;
    }
    if (added == 0) ossl_provider_info_clear(&entry);
  }
