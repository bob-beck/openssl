...
void*
ossl_prov_conf_ctx_new(OSSL_LIB_CTX* libctx)
{
...
void
ossl_prov_conf_ctx_free(void* vpcgbl)
{
 ...
 */
static int
provider_conf_params_internal(OSSL_PROVIDER* prov,
                              ...
                              STACK_OF(OPENSSL_CSTRING)* visited)
{
  ...
  if (sect != NULL) {
    ...
    for (i = 0; i < sk_CONF_VALUE_num(sect); i++) {
      ...
      rc = provider_conf_params_internal(
        prov, provinfo, buffer, sectconf->value, cnf, visited);
      if (rc < 0) {
        sk_OPENSSL_CSTRING_pop(visited);
        return rc;
      }
 ...
 */
static int
provider_conf_params(OSSL_PROVIDER* prov,
                     OSSL_PROVIDER_INFO* provinfo,
                     const char* name,
                     const char* value,
                     const CONF* cnf)
...
static int
prov_already_activated(const char* name, STACK_OF(OSSL_PROVIDER)* activated)
{
  int i, max;
 ...
 */
static int
provider_conf_activate(OSSL_LIB_CTX* libctx,
                       const char* name,
                       const char* value,
                       const char* path,
                       int soft,
...
static int
provider_conf_parse_bool_setting(const char* confname,
...
static int
provider_conf_load(OSSL_LIB_CTX* libctx,
                   const char* name,
                   const char* value,
                   const CONF* cnf)
{
...
static int
provider_conf_init(CONF_IMODULE* md, const CONF* cnf)
{
  ...
  OSSL_TRACE1(
    CONF, "Loading providers module: section %s\n", CONF_imodule_get_value(md));
  ...
  for (i = 0; i < sk_CONF_VALUE_num(elist); i++) {
    ...
    if (!provider_conf_load(
          NCONF_get0_libctx((CONF*)cnf), cval->name, cval->value, cnf))
      return 0;
  }
...
void
ossl_provider_add_conf_module(void)
{
