...
int ossl_param_build_set_int(OSSL_PARAM_BLD* bld, OSSL_PARAM* p,
  ...
  if (bld != NULL) return OSSL_PARAM_BLD_push_int(bld, key, num);
  p = OSSL_PARAM_locate(p, key);
  if (p != NULL) return OSSL_PARAM_set_int(p, num);
  return 1;
}

int ossl_param_build_set_long(OSSL_PARAM_BLD* bld, OSSL_PARAM* p,
                              const char* key, long num) {
  if (bld != NULL) return OSSL_PARAM_BLD_push_long(bld, key, num);
  p = OSSL_PARAM_locate(p, key);
  if (p != NULL) return OSSL_PARAM_set_long(p, num);
  return 1;
}

int ossl_param_build_set_utf8_string(OSSL_PARAM_BLD* bld, OSSL_PARAM* p,
                                     const char* key, const char* buf) {
  if (bld != NULL) return OSSL_PARAM_BLD_push_utf8_string(bld, key, buf, 0);
  p = OSSL_PARAM_locate(p, key);
  if (p != NULL) return OSSL_PARAM_set_utf8_string(p, buf);
  return 1;
}
...
int ossl_param_build_set_octet_string(OSSL_PARAM_BLD* bld, OSSL_PARAM* p,
  ...
  p = OSSL_PARAM_locate(p, key);
  if (p != NULL) return OSSL_PARAM_set_octet_string(p, data, data_len);
  return 1;
}

int ossl_param_build_set_bn_pad(OSSL_PARAM_BLD* bld, OSSL_PARAM* p,
                                const char* key, const BIGNUM* bn, size_t sz) {
  if (bld != NULL) return OSSL_PARAM_BLD_push_BN_pad(bld, key, bn, sz);
  p = OSSL_PARAM_locate(p, key);
  if (p != NULL) {
    if (sz > p->data_size) {
...
int ossl_param_build_set_bn(OSSL_PARAM_BLD* bld, OSSL_PARAM* p, const char* key,
                            const BIGNUM* bn) {
  if (bld != NULL) return OSSL_PARAM_BLD_push_BN(bld, key, bn);

  p = OSSL_PARAM_locate(p, key);
  if (p != NULL) return OSSL_PARAM_set_BN(p, bn) > 0;
  return 1;
}
...
int ossl_param_build_set_multi_key_bn(OSSL_PARAM_BLD* bld, OSSL_PARAM* params,
  ...
  if (bld != NULL) {
    for (i = 0; i < sz && names[i] != NULL; ++i) {
      ...
      if (bn != NULL && !OSSL_PARAM_BLD_push_BN(bld, names[i], bn)) return 0;
    }
    return 1;
  }
  ...
  for (i = 0; i < sz && names[i] != NULL; ++i) {
    ...
    if (p != NULL && bn != NULL) {
      if (!OSSL_PARAM_set_BN(p, bn)) return 0;
    }
  }
  return 1;
}
