...
int CRYPTO_ocb128_init(OCB128_CONTEXT* ctx, void* keyenc, void* keydec,
  ...
  ctx->l_index = 0;
  ctx->max_l_index = 5;
  if ((ctx->l = OPENSSL_malloc_array(ctx->max_l_index, 16)) == NULL) return 0;
...
int CRYPTO_ocb128_copy_ctx(OCB128_CONTEXT* dest, OCB128_CONTEXT* src,
  ...
  if (keyenc) dest->keyenc = keyenc;
  if (keydec) dest->keydec = keydec;
  if (src->l) {
...
int CRYPTO_ocb128_encrypt(OCB128_CONTEXT* ctx, const unsigned char* in,
  ...
  if (num_blocks && all_num_blocks == (size_t)all_num_blocks &&
     ...
     */
    while (top >>= 1) max_idx++;
    if (ocb_lookup_l(ctx, max_idx) == NULL) return 0;
