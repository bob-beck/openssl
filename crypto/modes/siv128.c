...
__owur static ossl_inline uint32_t
rotl8 (uint32_t x)
{
  return (x << 8) | (x >> 24);
}
...
__owur static ossl_inline uint64_t
byteswap8 (uint64_t x)
{
...
__owur static ossl_inline uint64_t
siv128_getword (SIV_BLOCK const *b, size_t i)
{
  DECLARE_IS_ENDIAN;
...
siv128_putword (SIV_BLOCK *b, size_t i, uint64_t x)
{
  DECLARE_IS_ENDIAN;
...
static ossl_inline void
siv128_xorblock (SIV_BLOCK *x, SIV_BLOCK const *y)
{
  x->word[0] ^= y->word[0];
  x->word[1] ^= y->word[1];
}
...
siv128_do_s2v_p (SIV128_CONTEXT *ctx, SIV_BLOCK *out, unsigned char const *in,
                 size_t len)
{
  SIV_BLOCK t;
...
__owur static ossl_inline int
siv128_do_encrypt (EVP_CIPHER_CTX *ctx, unsigned char *out,
                   unsigned char const *in, size_t len, SIV_BLOCK *icv)
{
  int out_len = (int)len;
 ...
 */
SIV128_CONTEXT *
ossl_siv128_new (const unsigned char *key, int klen, EVP_CIPHER *cbc,
 ...
 */
int
ossl_siv128_init (SIV128_CONTEXT *ctx, const unsigned char *key, int klen,
 ...
 */
int
ossl_siv128_copy_ctx (SIV128_CONTEXT *dest, SIV128_CONTEXT *src)
{
 ...
 */
int
ossl_siv128_aad (SIV128_CONTEXT *ctx, const unsigned char *aad, size_t len)
{
  SIV_BLOCK mac_out;
  size_t out_len = SIV_LEN;
  EVP_MAC_CTX *mac_ctx;
 ...
 */
int
ossl_siv128_encrypt (SIV128_CONTEXT *ctx, const unsigned char *in,
 ...
 */
int
ossl_siv128_decrypt (SIV128_CONTEXT *ctx, const unsigned char *in,
                     unsigned char *out, size_t len)
{
  unsigned char *p;
  SIV_BLOCK t, q;
 ...
 */
int
ossl_siv128_finish (SIV128_CONTEXT *ctx)
{
 ...
 */
int
ossl_siv128_set_tag (SIV128_CONTEXT *ctx, const unsigned char *tag, size_t len)
{
  if (len != SIV_LEN)
    return 0;
 ...
 */
int
ossl_siv128_get_tag (SIV128_CONTEXT *ctx, unsigned char *tag, size_t len)
{
  if (len != SIV_LEN)
    return 0;
 ...
 */
int
ossl_siv128_cleanup (SIV128_CONTEXT *ctx)
{
  if (ctx != NULL)
    {
      EVP_CIPHER_CTX_free (ctx->cipher_ctx);
...
int
ossl_siv128_speed (SIV128_CONTEXT *ctx, int arg)
{
