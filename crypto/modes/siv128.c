...
__owur static ossl_inline uint64_t siv128_getword(SIV_BLOCK const* b,
  ...
  if (IS_LITTLE_ENDIAN) return byteswap8(b->word[i]);
  return b->word[i];
}
...
__owur static ossl_inline int siv128_do_s2v_p(SIV128_CONTEXT* ctx,
  ...
  if (mac_ctx == NULL) return 0;

  if (len >= SIV_LEN) {
    if (!EVP_MAC_update(mac_ctx, in, len - SIV_LEN)) goto err;
    memcpy(&t, in + (len - SIV_LEN), SIV_LEN);
    siv128_xorblock(&t, &ctx->d);
    if (!EVP_MAC_update(mac_ctx, t.byte, SIV_LEN)) goto err;
  } else {
    memset(&t, 0, sizeof(t));
    memcpy(&t, in, len);
    t.byte[len] = 0x80;
...
__owur static ossl_inline int siv128_do_encrypt(EVP_CIPHER_CTX* ctx,
  ...
  if (!EVP_CipherInit_ex(ctx, NULL, NULL, NULL, icv->byte, 1)) return 0;
...
int ossl_siv128_init(SIV128_CONTEXT* ctx, const unsigned char* key, int klen,
  ...
  if (key == NULL || cbc == NULL || ctr == NULL) return 0;
...
int ossl_siv128_copy_ctx(SIV128_CONTEXT* dest, SIV128_CONTEXT* src) {
  ...
  if (dest->cipher_ctx == NULL) {
    ...
    if (dest->cipher_ctx == NULL) return 0;
  }
  ...
  if (dest->mac_ctx_init == NULL) return 0;
  dest->mac = src->mac;
  if (dest->mac != NULL) EVP_MAC_up_ref(dest->mac);
  return 1;
}
...
int ossl_siv128_encrypt(SIV128_CONTEXT* ctx, const unsigned char* in,
  ...
  if (ctx->crypto_ok == 0) return 0;
  ctx->crypto_ok--;

  if (!siv128_do_s2v_p(ctx, &q, in, len)) return 0;

  memcpy(ctx->tag.byte, &q, SIV_LEN);
  q.byte[8] &= 0x7f;
  q.byte[12] &= 0x7f;

  if (!siv128_do_encrypt(ctx->cipher_ctx, out, in, len, &q)) return 0;
  ctx->final_ret = 0;
  return 1;
}
