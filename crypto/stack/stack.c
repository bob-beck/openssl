...
OPENSSL_STACK* OPENSSL_sk_dup(const OPENSSL_STACK* sk) {
  ...
  if ((ret = OPENSSL_malloc(sizeof(*ret))) == NULL) goto err;
...
OPENSSL_STACK* OPENSSL_sk_deep_copy(const OPENSSL_STACK* sk,
  ...
  for (i = 0; i < ret->num; ++i) {
    if (sk->data[i] == NULL) continue;
    if ((ret->data[i] = copy_func(sk->data[i])) == NULL) {
      while (--i >= 0)
        if (ret->data[i] != NULL) free_func((void*)ret->data[i]);
      goto err;
    }
  }
  return ret;
...
static int sk_reserve(OPENSSL_STACK* st, int n, int exact) {
  ...
  num_alloc = st->num + n;
  if (num_alloc < min_nodes) num_alloc = min_nodes;
  ...
  if (st->data == NULL) {
     ...
     */
    if ((st->data = OPENSSL_calloc(num_alloc, sizeof(void*))) == NULL) return 0;
    st->num_alloc = num_alloc;
    return 1;
  }

  if (!exact) {
    if (num_alloc <= st->num_alloc) return 1;
...
OPENSSL_STACK* OPENSSL_sk_set_thunks(OPENSSL_STACK* st,
  ...
  if (st != NULL) st->free_thunk = f_thunk;
...
void* OPENSSL_sk_delete_ptr(OPENSSL_STACK* st, const void* p) {
  ...
  for (i = 0; i < st->num; i++)
    if (st->data[i] == p) return internal_delete(st, i);
  return NULL;
}

void* OPENSSL_sk_delete(OPENSSL_STACK* st, int loc) {
  if (st == NULL || loc < 0 || loc >= st->num) return NULL;
...
static int internal_find(OPENSSL_STACK* st, const void* data,
  ...
  if (st == NULL || st->num == 0) return -1;

  if (pnum == NULL) pnum = &count;
...
void* OPENSSL_sk_shift(OPENSSL_STACK* st) {
  if (st == NULL || st->num == 0) return NULL;
...
void* OPENSSL_sk_value(const OPENSSL_STACK* st, int i) {
  if (st == NULL || i < 0 || i >= st->num) return NULL;
...
void OPENSSL_sk_sort(OPENSSL_STACK* st) {
  if (st != NULL && !st->sorted && st->comp != NULL) {
    if (st->num > 1) qsort(st->data, st->num, sizeof(void*), st->comp);
