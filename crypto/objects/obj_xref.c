...
static int sigx_cmp(const nid_triple* const* a, const nid_triple* const* b) {
   ...
   */
  if ((ret != 0) && ((*b)->hash_id != NID_undef)) return ret;
...
static int ossl_obj_find_sigid_algs(int signid, int* pdig_nid, int* ppkey_nid,
  ...
  if (rv == NULL) {
    ...
    if (sig_app != NULL) {
      ...
      if (idx >= 0) rv = sk_nid_triple_value(sig_app, idx);
    }
    if (lock) CRYPTO_THREAD_unlock(sig_lock);
    if (rv == NULL) return 0;
  }

  if (pdig_nid != NULL) *pdig_nid = rv->hash_id;
  if (ppkey_nid != NULL) *ppkey_nid = rv->pkey_id;
  return 1;
}
...
int OBJ_find_sigid_by_algs(int* psignid, int dig_nid, int pkey_nid) {
  ...
  if (psignid != NULL) *psignid = (*rv)->sign_id;
  return 1;
}
...
int OBJ_add_sigid(int signid, int dig_id, int pkey_id) {
  ...
  if (signid == NID_undef || pkey_id == NID_undef) return 0;
  ...
  if (sig_app == NULL) {
    ...
    if (sig_app == NULL) goto err;
  }
  if (sigx_app == NULL) {
    sigx_app = sk_nid_triple_new(sigx_cmp);
    if (sigx_app == NULL) goto err;
  }
   ...
   */
  if (!sk_nid_triple_push(sig_app, ntr)) goto err;
