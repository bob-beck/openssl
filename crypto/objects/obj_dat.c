...
DEFINE_RUN_ONCE_STATIC(obj_api_initialise) {
  ...
  if (ossl_obj_lock == NULL) return 0;
...
static void cleanup3_doall(ADDED_OBJ* a) {
  if (--a->obj->nid == 0) ASN1_OBJECT_free(a->obj);
  OPENSSL_free(a);
}
...
static int ossl_obj_obj2nid(const ASN1_OBJECT* a) {
  ...
  if (a->nid != NID_undef) return a->nid;
  if (a->length == 0) return NID_undef;

  op = OBJ_bsearch_obj(&a, obj_objs, NUM_OBJ);
  if (op != NULL) return nid_objs[*op].nid;
  if (!ossl_obj_read_lock()) {
    ERR_raise(ERR_LIB_OBJ, ERR_R_UNABLE_TO_GET_READ_LOCK);
    return NID_undef;
  }
...
int OBJ_obj2txt(char* buf, int buf_len, const ASN1_OBJECT* a, int no_name) {
  ...
  if (!no_name && (nid = OBJ_obj2nid(a)) != NID_undef) {
    s = OBJ_nid2ln(nid);
    if (s == NULL) s = OBJ_nid2sn(nid);
    if (s != NULL) {
      if (buf != NULL) OPENSSL_strlcpy(buf, s, buf_len);
      return (int)strlen(s);
    }
  }
  ...
  while (len > 0) {
    ...
    for (;;) {
      ...
      len--;
      if (len == 0 && (c & 0x80) != 0) goto err;
      if (use_bn) {
        if (!BN_add_word(bl, c & 0x7f)) goto err;
      } else {
        l |= c & 0x7f;
      }
      if ((c & 0x80) == 0) break;
      if (!use_bn && l > (ULONG_MAX >> 7L)) {
        if (bl == NULL && (bl = BN_new()) == NULL) goto err;
        if (!BN_set_word(bl, l)) goto err;
        use_bn = 1;
      }
      if (use_bn) {
        if (!BN_lshift(bl, bl, 7)) goto err;
      } else {
        l <<= 7L;
      }
    }
...
int OBJ_create_objects(BIO* in) {
  ...
  for (;;) {
    ...
    if (i <= 0) return num;
    buf[i - 1] = '\0';
    if (!ossl_isalnum(buf[0])) return num;
    o = s = buf;
...
int OBJ_create(const char* oid, const char* sn, const char* ln) {
  ...
  if (oid != NULL) {
    ...
    if (tmpoid == NULL) return NID_undef;
  } else {
  ...
  if (oid != NULL && ossl_obj_obj2nid(tmpoid) != NID_undef) {
    ERR_raise(ERR_LIB_OBJ, OBJ_R_OID_EXISTS);
    goto err;
  }
  ...
  if (tmpoid->nid == NID_undef) goto err;
...
int OBJ_add_object(const ASN1_OBJECT* obj) {
  ...
  if ((o = OBJ_dup(obj)) == NULL) return NID_undef;
...
int OBJ_obj2nid(const ASN1_OBJECT* a) { return ossl_obj_obj2nid(a); }
