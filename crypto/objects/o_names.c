...
DEFINE_RUN_ONCE_STATIC(o_names_init) {
  ...
  if (obj_lock != NULL) names_lh = lh_OBJ_NAME_new(obj_name_hash, obj_name_cmp);
  if (names_lh == NULL) {
...
int OBJ_NAME_new_index(unsigned long (*hash_func)(const char*),
  ...
  if (hash_func != NULL) name_funcs->hash_func = hash_func;
  if (cmp_func != NULL) name_funcs->cmp_func = cmp_func;
  if (free_func != NULL) name_funcs->free_func = free_func;
...
const char* OBJ_NAME_get(const char* name, int type) {
  ...
  if (!OBJ_NAME_init()) return NULL;
  if (!CRYPTO_THREAD_read_lock(obj_lock)) return NULL;
...
static void do_all_fn(const OBJ_NAME* name, OBJ_DOALL* d) {
  if (name->type == d->type) d->fn(name, d->arg);
}
