...
static void clean_master_key(void* data) {
  ...
  for (i = 0; i < CRYPTO_THREAD_LOCAL_KEY_MAX; i++) {
    if (mkey[i].ctx_table != NULL) clean_master_key_id(&mkey[i]);
  }
  OPENSSL_free(mkey);
}
...
static void init_master_key(void) {
   ...
   */
  if (!CRYPTO_THREAD_init_local(&master_key, clean_master_key)) return;
...
void* CRYPTO_THREAD_get_local_ex(CRYPTO_THREAD_LOCAL_KEY_ID id,
  ...
  ctx =
      (ctx == CRYPTO_THREAD_NO_CONTEXT) ? NULL : ossl_lib_ctx_get_concrete(ctx);
  /*
   ...
   * always defined.
   */
  if (!CRYPTO_THREAD_run_once(&master_once, init_master_key)) return NULL;

  if (!ossl_assert(id < CRYPTO_THREAD_LOCAL_KEY_MAX)) return NULL;
   ...
   */
  if (mkey[id].ctx_table == NULL) return NULL;
...
int CRYPTO_THREAD_set_local_ex(CRYPTO_THREAD_LOCAL_KEY_ID id, OSSL_LIB_CTX* ctx,
   ...
   */
  if (!CRYPTO_THREAD_run_once(&master_once, init_master_key)) return 0;

  if (!ossl_assert(id < CRYPTO_THREAD_LOCAL_KEY_MAX)) return 0;
  ...
  if (mkey[id].ctx_table == NULL) {
    ...
    if (mkey[id].ctx_table == NULL) return 0;
  }
...
void CRYPTO_THREAD_clean_local_for_fips(void) {
   ...
   */
  if (master_key_init == 0) return;

  mkey = CRYPTO_THREAD_get_local(&master_key);
  if (mkey != NULL) clean_master_key(mkey);
