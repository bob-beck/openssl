...
get_hold_current_qp(struct rcu_lock_st* lock)
{
  ...
  for (;;) {
    ...
    ATOMIC_SUB_FETCH(
      &lock->qp_group[qp_idx].users, (uint64_t)1, __ATOMIC_RELAXED);
  }
...
ossl_rcu_read_lock(CRYPTO_RCU_LOCK* lock)
{
  ...
  if (data == NULL) {
    ...
    if (!CRYPTO_THREAD_set_local_ex(
          CRYPTO_THREAD_LOCAL_RCU_KEY, lock->ctx, data)) {
      OPENSSL_free(data);
      return 0;
    }
    ...
    if (data->thread_qps[i].lock == lock) {
      ...
      if (data->thread_qps[i].depth == 0) {
        ret = ATOMIC_SUB_FETCH(
          &data->thread_qps[i].qp->users, (uint64_t)1, __ATOMIC_RELEASE);
...
update_qp(CRYPTO_RCU_LOCK* lock, uint32_t* curr_id)
{
  ...
  *curr_id = lock->id_ctr;
  ...
  ATOMIC_STORE_N(
    uint32_t, &lock->reader_idx, lock->current_alloc_idx, __ATOMIC_RELAXED);
   ...
   */
  ATOMIC_ADD_FETCH(
    &lock->qp_group[current_idx].users, (uint64_t)0, __ATOMIC_RELEASE);
...
static int
record_lock_contention(pthread_rwlock_t* lock,
                       struct stack_traces* traces,
                       bool write)
{
...
int
CRYPTO_atomic_add64(uint64_t* val,
                    uint64_t op,
                    uint64_t* ret,
                    CRYPTO_RWLOCK* lock)
{
...
int
CRYPTO_atomic_and(uint64_t* val,
                  uint64_t op,
                  uint64_t* ret,
                  CRYPTO_RWLOCK* lock)
{
...
int
CRYPTO_atomic_or(uint64_t* val, uint64_t op, uint64_t* ret, CRYPTO_RWLOCK* lock)
{
#if defined(__GNUC__) && defined(__ATOMIC_ACQ_REL) &&                          \
  !defined(BROKEN_CLANG_ATOMICS)
  if (__atomic_is_lock_free(sizeof(*val), val)) {
