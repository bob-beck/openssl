...
int ossl_rcu_read_lock(CRYPTO_RCU_LOCK* lock) {
  ...
  for (i = 0; i < MAX_QPS; i++) {
    if (data->thread_qps[i].qp == NULL && available_qp == -1) available_qp = i;
...
static ossl_inline pid_t get_tid(void) { return syscall(SYS_gettid); }
...
static ossl_inline int ossl_rwlock_unlock(pthread_rwlock_t* lock) {
...

  {
    ...
    if (traces != NULL) {
      ...
      if (traces->lock_depth == 0) print_stack_traces(traces);
    }
  }
...
#ifdef USE_RWLOCK
  if (!ossl_assert(ossl_rwlock_rdlock(lock) == 0)) return 0;
#else
...
#ifdef USE_RWLOCK
  if (!ossl_assert(ossl_rwlock_wrlock(lock) == 0)) return 0;
#else
  if (pthread_mutex_lock(lock) != 0) {
    assert(errno != EDEADLK && errno != EBUSY);
    return 0;
...
#ifdef USE_RWLOCK
  if (ossl_rwlock_unlock(lock) != 0) return 0;
#else
...
int CRYPTO_THREAD_run_once(CRYPTO_ONCE* once, void (*init)(void)) {
  if (ossl_unlikely(pthread_once(once, init) != 0)) return 0;

  return 1;
}

int CRYPTO_THREAD_init_local(CRYPTO_THREAD_LOCAL* key, void (*cleanup)(void*)) {
  if (pthread_key_create(key, cleanup) != 0) return 0;
...
int CRYPTO_THREAD_set_local(CRYPTO_THREAD_LOCAL* key, void* val) {
  if (pthread_setspecific(*key, val) != 0) return 0;

  return 1;
}

int CRYPTO_THREAD_cleanup_local(CRYPTO_THREAD_LOCAL* key) {
  if (pthread_key_delete(*key) != 0) return 0;
...
int CRYPTO_atomic_add(int* val, int amount, int* ret, CRYPTO_RWLOCK* lock) {
  ...
  *ret = *val;
  ...
  if (!CRYPTO_THREAD_unlock(lock)) return 0;
