...
#define ATOMIC_STORE(t, p, v, o) fallback_atomic_store_##t (p, v)
     ...
     * they are designed for uint64_t only.  If there comes a time when
     * multiple types need to be covered, it's relatively easy to refactor them
     * the same way as the fallbacks above.
     */
...
static ossl_inline uint64_t
fallback_atomic_sub_fetch (uint64_t *p, uint64_t v)
{
  uint64_t ret;
...
static struct rcu_qp *
get_hold_current_qp (struct rcu_lock_st *lock)
{
  uint32_t qp_idx;
...
static void
ossl_rcu_free_local_data (void *arg)
{
...
int
ossl_rcu_read_lock (CRYPTO_RCU_LOCK *lock)
{
  struct rcu_thr_data *data;
  ...
  if (data == NULL)
    {
      ...
      if (!CRYPTO_THREAD_set_local_ex (CRYPTO_THREAD_LOCAL_RCU_KEY, lock->ctx,
                                       data))
        {
          OPENSSL_free (data);
          return 0;
...
void
ossl_rcu_read_unlock (CRYPTO_RCU_LOCK *lock)
{
  int i;
  struct rcu_thr_data *data
 ...
 */
static struct rcu_qp *
update_qp (CRYPTO_RCU_LOCK *lock, uint32_t *curr_id)
{
...
static void
retire_qp (CRYPTO_RCU_LOCK *lock, struct rcu_qp *qp)
{
  pthread_mutex_lock (&lock->alloc_lock);
  lock->writers_alloced--;
  pthread_cond_signal (&lock->alloc_signal);
...
static struct rcu_qp *
allocate_new_qp_group (CRYPTO_RCU_LOCK *lock, uint32_t count)
{
...
void
ossl_rcu_write_lock (CRYPTO_RCU_LOCK *lock)
{
...
void
ossl_rcu_write_unlock (CRYPTO_RCU_LOCK *lock)
{
...
void
ossl_synchronize_rcu (CRYPTO_RCU_LOCK *lock)
{
  struct rcu_qp *qp;
  uint64_t count;
  uint32_t curr_id;
 ...
 */
int
ossl_rcu_call (CRYPTO_RCU_LOCK *lock, rcu_cb_fn cb, void *data)
{
...
void *
ossl_rcu_uptr_deref (void **p)
{
  return ATOMIC_LOAD_N (pvoid, p, __ATOMIC_ACQUIRE);
}

void
ossl_rcu_assign_uptr (void **p, void **v)
{
  ATOMIC_STORE (pvoid, p, v, __ATOMIC_RELEASE);
}

CRYPTO_RCU_LOCK *
ossl_rcu_lock_new (int num_writers, OSSL_LIB_CTX *ctx)
{
...
void
ossl_rcu_lock_free (CRYPTO_RCU_LOCK *lock)
{
 ...
 * library initialization, and creating a bio too early, creates a recursive
 * set of stack calls that leads us to call CRYPTO_thread_run_once while
 * currently executing the init routine for various run_once functions, which
 * leads to deadlock.  Avoid that by just using a FILE pointer.  Also note that
 * we directly use a pthread_mutex_t to protect access from multiple threads to
 * the contention log file.  We do this because we want to avoid use of the
 * CRYPTO_THREAD api so as to prevent recursive blocking reports.
 */
...
static void
destroy_contention_data (void *data)
{
...
static void
init_contention_data_once (void)
{
  /*
...
static struct stack_traces *
get_stack_traces (bool init)
{
  struct stack_traces *traces
...
static void
print_stack_traces (struct stack_traces *traces)
{
  unsigned int j;
  struct iovec *iov;
  int iovcnt;
  ...
  while (traces != NULL && traces->idx >= 1)
    {
      ...
      dprintf (traces->fd,
               ...
               ossl_time2us (traces->stacks[traces->idx].start), get_tid ());
          ...
          for (j = 0; j < traces->stacks[traces->idx].nptrs; j++)
            {
              ...
              iov[2 * j].iov_len
                  = strlen (traces->stacks[traces->idx].strings[j]);
...
ossl_init_rwlock_contention_data (void)
{
...
static int
record_lock_contention (pthread_rwlock_t *lock, struct stack_traces *traces,
                        bool write)
{
...
ossl_rwlock_rdlock (pthread_rwlock_t *lock)
{
...
static ossl_inline int
ossl_rwlock_wrlock (pthread_rwlock_t *lock)
{
  struct stack_traces *traces = get_stack_traces (true);
...
static ossl_inline int
ossl_rwlock_unlock (pthread_rwlock_t *lock)
{
  int ret;
...
static ossl_inline int
ossl_rwlock_rdlock (pthread_rwlock_t *rwlock)
{
  return pthread_rwlock_rdlock (rwlock);
}

static ossl_inline int
ossl_rwlock_wrlock (pthread_rwlock_t *rwlock)
{
  return pthread_rwlock_wrlock (rwlock);
}

static ossl_inline int
ossl_rwlock_unlock (pthread_rwlock_t *rwlock)
{
...
__owur int
CRYPTO_THREAD_read_lock (CRYPTO_RWLOCK *lock)
{
#ifdef USE_RWLOCK
...
int
CRYPTO_THREAD_unlock (CRYPTO_RWLOCK *lock)
{
#ifdef USE_RWLOCK
...
void
CRYPTO_THREAD_lock_free (CRYPTO_RWLOCK *lock)
{
  if (lock == NULL)
    return;
...
int
CRYPTO_THREAD_init_local (CRYPTO_THREAD_LOCAL *key, void (*cleanup) (void *))
{
...
int
CRYPTO_THREAD_set_local (CRYPTO_THREAD_LOCAL *key, void *val)
{
...
CRYPTO_THREAD_ID
CRYPTO_THREAD_get_current_id (void) { return pthread_self (); }

int
CRYPTO_THREAD_compare_id (CRYPTO_THREAD_ID a, CRYPTO_THREAD_ID b)
{
  return pthread_equal (a, b);
}

int
CRYPTO_atomic_add (int *val, int amount, int *ret, CRYPTO_RWLOCK *lock)
{
#if defined(__GNUC__) && defined(__ATOMIC_ACQ_REL)                            \
...
int
CRYPTO_atomic_add64 (uint64_t *val, uint64_t op, uint64_t *ret,
                     CRYPTO_RWLOCK *lock)
{
#if defined(__GNUC__) && defined(__ATOMIC_ACQ_REL)                            \
    && !defined(BROKEN_CLANG_ATOMICS)
...
int
CRYPTO_atomic_and (uint64_t *val, uint64_t op, uint64_t *ret,
                   CRYPTO_RWLOCK *lock)
{
#if defined(__GNUC__) && defined(__ATOMIC_ACQ_REL)                            \
    && !defined(BROKEN_CLANG_ATOMICS)
...
int
CRYPTO_atomic_or (uint64_t *val, uint64_t op, uint64_t *ret,
                  CRYPTO_RWLOCK *lock)
{
#if defined(__GNUC__) && defined(__ATOMIC_ACQ_REL)                            \
    && !defined(BROKEN_CLANG_ATOMICS)
...
int
CRYPTO_atomic_load (uint64_t *val, uint64_t *ret, CRYPTO_RWLOCK *lock)
{
#if defined(__GNUC__) && defined(__ATOMIC_ACQ_REL)                            \
    && !defined(BROKEN_CLANG_ATOMICS)
  if (__atomic_is_lock_free (sizeof (*val), val))
...
int
CRYPTO_atomic_store (uint64_t *dst, uint64_t val, CRYPTO_RWLOCK *lock)
{
#if defined(__GNUC__) && defined(__ATOMIC_ACQ_REL)                            \
    && !defined(BROKEN_CLANG_ATOMICS)
...
int
CRYPTO_atomic_load_int (int *val, int *ret, CRYPTO_RWLOCK *lock)
{
#if defined(__GNUC__) && defined(__ATOMIC_ACQ_REL)                            \
    && !defined(BROKEN_CLANG_ATOMICS)
  if (__atomic_is_lock_free (sizeof (*val), val))
...
#ifndef FIPS_MODULE
int
openssl_init_fork_handlers (void)
{
  return 1;
}
#endif /* FIPS_MODULE */
