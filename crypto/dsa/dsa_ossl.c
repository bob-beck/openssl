...
static int
dsa_sign_setup_no_digest(DSA* dsa, BN_CTX* ctx_in, BIGNUM** kinvp, BIGNUM** rp);
static int
...
dsa_finish(DSA* dsa);
static BIGNUM*
dsa_mod_inverse_fermat(const BIGNUM* k, const BIGNUM* q, BN_CTX* ctx);
...
DSA_SIG*
ossl_dsa_do_sign_int(const unsigned char* dgst,
                     int dlen,
                     DSA* dsa,
                     unsigned int nonce_type,
                     const char* digestname,
               ...
               const char* propq)
{
  ...
  do {
    if (dgst != NULL) {
      if (nonce_type == 1) {
#ifndef FIPS_MODULE
        if (!ossl_gen_deterministic_nonce_rfc6979(k,
                                                  dsa->params.q,
                                                  dsa->priv_key,
                                                  dgst,
                                                  dlen,
  ...
  if (dsa->flags & DSA_FLAG_CACHE_MONT_P) {
    if (!BN_MONT_CTX_set_locked(
          &dsa->method_mont_p, dsa->lock, dsa->params.p, ctx))
      goto err;
  }
  ...
  if ((dsa)->meth->bn_mod_exp != NULL) {
    if (!dsa->meth->bn_mod_exp(
          dsa, r, dsa->params.g, k, dsa->params.p, ctx, dsa->method_mont_p))
      goto err;
  } else {
    if (!BN_mod_exp_mont(
          r, dsa->params.g, k, dsa->params.p, ctx, dsa->method_mont_p))
      goto err;
  }
...
dsa_do_verify(const unsigned char* dgst, int dgst_len, DSA_SIG* sig, DSA* dsa)
{
  ...
  if (dsa->meth->dsa_mod_exp != NULL) {
    ...
    if (!BN_mod_exp2_mont(
          t1, dsa->params.g, u1, dsa->pub_key, u2, dsa->params.p, ctx, mont))
      goto err;
  }
