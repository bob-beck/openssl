...
DSA_SIG* ossl_dsa_do_sign_int(const unsigned char* dgst, int dlen, DSA* dsa,
  ...
  ret->r = BN_new();
  ret->s = BN_new();
  if (ret->r == NULL || ret->s == NULL) goto err;
...
redo:
  ...
  if (!BN_mod_mul(tmp, blind, dsa->priv_key, dsa->params.q, ctx)) goto err;
  if (!BN_mod_mul(tmp, tmp, ret->r, dsa->params.q, ctx)) goto err;

  /* blindm := blind * m mod q */
  if (!BN_mod_mul(blindm, blind, m, dsa->params.q, ctx)) goto err;

  /* s : = (blind * priv_key * r) + (blind * m) mod q */
  if (!BN_mod_add_quick(ret->s, tmp, blindm, dsa->params.q)) goto err;

  /* s := s * k^-1 mod q */
  if (!BN_mod_mul(ret->s, ret->s, kinv, dsa->params.q, ctx)) goto err;

  /* s:= s * blind^-1 mod q */
  if (BN_mod_inverse(blind, blind, dsa->params.q, ctx) == NULL) goto err;
  if (!BN_mod_mul(ret->s, ret->s, blind, dsa->params.q, ctx)) goto err;
...
static int dsa_sign_setup(DSA* dsa, BN_CTX* ctx_in, BIGNUM** kinvp, BIGNUM** rp,
  ...
  k = BN_new();
  l = BN_new();
  if (k == NULL || l == NULL) goto err;

  if (ctx_in == NULL) {
    /* if you don't pass in ctx_in you get a default libctx */
    if ((ctx = BN_CTX_new_ex(NULL)) == NULL) goto err;
  } else
    ctx = ctx_in;
   ...
   */
  if (!BN_add(l, k, dsa->params.q) || !BN_add(k, l, dsa->params.q)) goto err;
  ...
  if (!BN_mod(r, r, dsa->params.q, ctx)) goto err;

  /* Compute part of 's = inv(k) (m + xr) mod q' */
  if ((kinv = dsa_mod_inverse_fermat(k, dsa->params.q, ctx)) == NULL) goto err;
  ...
  *kinvp = kinv;
  ...
  ret = 1;
err:
  if (!ret) ERR_raise(ERR_LIB_DSA, ERR_R_BN_LIB);
  if (ctx != ctx_in) BN_CTX_free(ctx);
  BN_clear_free(k);
  BN_clear_free(l);
  return ret;
}
...
static int dsa_do_verify(const unsigned char* dgst, int dgst_len, DSA_SIG* sig,
  ...
  if (u1 == NULL || u2 == NULL || t1 == NULL || ctx == NULL) goto err;
   ...
   */
  if ((BN_mod_inverse(u2, s, dsa->params.q, ctx)) == NULL) goto err;
  ...
  /* u1 = M * w mod q */
  if (!BN_mod_mul(u1, u1, u2, dsa->params.q, ctx)) goto err;

  /* u2 = r * w mod q */
  if (!BN_mod_mul(u2, r, u2, dsa->params.q, ctx)) goto err;

  if (dsa->flags & DSA_FLAG_CACHE_MONT_P) {
    mont = BN_MONT_CTX_set_locked(&dsa->method_mont_p, dsa->lock, dsa->params.p,
                                  ctx);
    if (!mont) goto err;
  }
  ...
  /* let u1 = u1 mod q */
  if (!BN_mod(u1, t1, dsa->params.q, ctx)) goto err;
...
err:
  if (ret < 0) ERR_raise(ERR_LIB_DSA, ERR_R_BN_LIB);
  BN_CTX_free(ctx);
  BN_free(u1);
  BN_free(u2);
  BN_free(t1);
