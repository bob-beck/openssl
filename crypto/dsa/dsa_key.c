...
#ifndef FIPS_MODULE
  if (dsa->meth->dsa_keygen != NULL) return dsa->meth->dsa_keygen(dsa);
#endif
...
int ossl_dsa_generate_public_key(BN_CTX* ctx, const DSA* dsa,
  ...
  if (!BN_mod_exp(pub_key, dsa->params.g, prk, dsa->params.p, ctx)) goto err;
  ret = 1;
err:
  BN_clear_free(prk);
  return ret;
...
static int dsa_keygen_knownanswer_test(DSA* dsa, BN_CTX* ctx, OSSL_CALLBACK* cb,
  ...
  if (BN_num_bytes(pub_key2) > (int)sizeof(bytes)) goto err;
...
static int dsa_keygen_pairwise_test(DSA* dsa, OSSL_CALLBACK* cb, void* cbarg) {
  ...
  if (DSA_do_verify(dgst, dgst_len, sig, dsa) != 1) goto err;
...
static int dsa_keygen(DSA* dsa) {
  ...
  if ((ctx = BN_CTX_new_ex(dsa->libctx)) == NULL) goto err;
  ...
  if (!ossl_dsa_generate_public_key(ctx, dsa, priv_key, pub_key)) goto err;
...
err:
  if (pub_key != dsa->pub_key) BN_free(pub_key);
  if (priv_key != dsa->priv_key) BN_free(priv_key);
  BN_CTX_free(ctx);
