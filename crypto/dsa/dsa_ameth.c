...
static int dsa_pub_encode(X509_PUBKEY* pk, const EVP_PKEY* pkey) {
  ...
  if (aobj == NULL) goto err;

  if (X509_PUBKEY_set0_param(pk, aobj, ptype, str, penc, penclen)) return 1;
...
static int do_dsa_print(BIO* bp, const DSA* x, int off, int ptype) {
  ...
  if (x->params.p != NULL) mod_len = DSA_bits(x);
  ...
  if (priv_key != NULL) {
    ...
    if (BIO_printf(bp, "%s: (%d bit)\n", ktype, mod_len) <= 0) goto err;
  } else {
    if (BIO_printf(bp, "Public-Key: (%d bit)\n", mod_len) <= 0) goto err;
  }

  if (!ASN1_bn_print(bp, "priv:", priv_key, NULL, off)) goto err;
  if (!ASN1_bn_print(bp, "pub: ", pub_key, NULL, off)) goto err;
  if (!ossl_ffc_params_print(bp, &x->params, off)) goto err;
  ret = 1;
err:
  return ret;
}
...
static int dsa_param_decode(EVP_PKEY* pkey, const unsigned char** pder,
  ...
  if ((dsa = d2i_DSAparams(NULL, pder, derlen)) == NULL) return 0;
...
static int dsa_sig_print(BIO* bp, const X509_ALGOR* sigalg,
  ...
  if (dsa_sig != NULL) {
    ...
    if (!ASN1_bn_print(bp, "r:   ", r, NULL, indent)) goto err;
    if (!ASN1_bn_print(bp, "s:   ", s, NULL, indent)) goto err;
    rv = 1;
  err:
...
static int dsa_pkey_export_to(const EVP_PKEY* from, void* to_keydata,
  ...
  if (p == NULL || q == NULL || g == NULL) return 0;
