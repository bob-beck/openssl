...
static int
dsa_pub_decode(EVP_PKEY* pkey, const X509_PUBKEY* pubkey)
{
...
dsa_priv_encode(PKCS8_PRIV_KEY_INFO* p8, const EVP_PKEY* pkey)
{
  ...
  if (!PKCS8_pkey_set0(
        p8, OBJ_nid2obj(NID_dsa), 0, V_ASN1_SEQUENCE, params, dp, dplen)) {
...
static size_t
dsa_pkey_dirty_cnt(const EVP_PKEY* pkey)
{
  return pkey->pkey.dsa->dirty_cnt;
}

static int
dsa_pkey_export_to(const EVP_PKEY* from,
                   void* to_keydata,
...
static int
dsa_pkey_import_from(const OSSL_PARAM params[], void* vpctx)
{
...
static int
dsa_pkey_copy(EVP_PKEY* to, EVP_PKEY* from)
{
  DSA* dsa = from->pkey.dsa;
  DSA* dupkey = NULL;
  int ret;
