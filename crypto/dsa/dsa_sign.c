...
DSA_SIG*
d2i_DSA_SIG(DSA_SIG** psig, const unsigned char** ppin, long len)
{
  DSA_SIG* sig;
...
int
i2d_DSA_SIG(const DSA_SIG* sig, unsigned char** ppout)
{
  BUF_MEM* buf = NULL;
  size_t encoded_len;
  WPACKET pkt;
                  ...
                  const char* propq)
{
  ...
  if (dsa->libctx == NULL || dsa->meth != DSA_get_default_method())
    ...
    s = ossl_dsa_do_sign_int(
      dgst, dlen, dsa, nonce_type, digestname, libctx, propq);
  if (s == NULL) {
    *siglen = 0;
    return 0;
  }
...
DSA_sign(int type,
         ...
         DSA* dsa)
{
  return ossl_dsa_sign_int(
    type, dgst, dlen, sig, siglen, dsa, 0, NULL, NULL, NULL);
}
