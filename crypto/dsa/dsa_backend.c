...
int ossl_dsa_key_fromdata(DSA* dsa, const OSSL_PARAM params[],
  ...
  if (param_priv_key == NULL && param_pub_key == NULL) return 1;
  ...
  if (!DSA_set0_key(dsa, pub_key, priv_key)) goto err;
...
DSA* ossl_dsa_dup(const DSA* dsa, int selection) {
  ...
  if (ossl_dsa_is_foreign(dsa)) return NULL;

  if ((dupkey = ossl_dsa_new(dsa->libctx)) == NULL) return NULL;
