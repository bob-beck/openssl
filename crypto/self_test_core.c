...
#ifndef FIPS_MODULE
void*
ossl_self_test_set_callback_new(OSSL_LIB_CTX* ctx)
{
  SELF_TEST_CB* stcb;
...
void
ossl_self_test_set_callback_free(void* stcb)
{
  OPENSSL_free(stcb);
}

static SELF_TEST_CB*
get_self_test_callback(OSSL_LIB_CTX* libctx)
{
  return ossl_lib_ctx_get_data(libctx, OSSL_LIB_CTX_SELF_TEST_CB_INDEX);
}

void
OSSL_SELF_TEST_set_callback(OSSL_LIB_CTX* libctx,
                            OSSL_CALLBACK* cb,
                            void* cbarg)
{
...
static void
self_test_setparams(OSSL_SELF_TEST* st)
{
  size_t n = 0;
...
OSSL_SELF_TEST*
OSSL_SELF_TEST_new(OSSL_CALLBACK* cb, void* cbarg)
{
...
void
OSSL_SELF_TEST_free(OSSL_SELF_TEST* st)
{
  OPENSSL_free(st);
}
...
void
OSSL_SELF_TEST_onbegin(OSSL_SELF_TEST* st, const char* type, const char* desc)
{
 ...
 */
void
OSSL_SELF_TEST_onend(OSSL_SELF_TEST* st, int ret)
{
  if (st != NULL && st->cb != NULL) {
    st->phase =
 ...
 */
int
OSSL_SELF_TEST_oncorrupt_byte(OSSL_SELF_TEST* st, unsigned char* bytes)
{
  if (st != NULL && st->cb != NULL) {
