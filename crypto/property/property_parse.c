...
static int pd_compare(const OSSL_PROPERTY_DEFINITION* const* p1,
  ...
  if (pd1->name_idx < pd2->name_idx) return -1;
  if (pd1->name_idx > pd2->name_idx) return 1;
  return 0;
}
 ...
 * any exist.
 */
static OSSL_PROPERTY_LIST* stack_to_property_list(
    OSSL_LIB_CTX* ctx, STACK_OF(OSSL_PROPERTY_DEFINITION)* sk) {
...
OSSL_PROPERTY_LIST* ossl_parse_property(OSSL_LIB_CTX* ctx, const char* defn) {
  ...
  while (!done) {
    ...
    if (prop == NULL) goto err;
    memset(&prop->v, 0, sizeof(prop->v));
    prop->optional = 0;
    if (!parse_name(ctx, &s, 1, &prop->name_idx)) goto err;
    ...
    if (!sk_OSSL_PROPERTY_DEFINITION_push(sk, prop)) goto err;
    prop = NULL;
    done = !match_ch(&s, ',');
  }
  if (*s != '\0') {
...
OSSL_PROPERTY_LIST* ossl_property_merge(const OSSL_PROPERTY_LIST* a,
  ...
  for (i = j = n = 0; i < a->num_properties || j < b->num_properties; n++) {
    if (i >= a->num_properties) {
      ...
      if (ap[i].name_idx == bp[j].name_idx) j++;
      copy = &ap[i++];
    } else {
      copy = &bp[j++];
    }
...
int ossl_property_parse_init(OSSL_LIB_CTX* ctx) {
  ...
  for (i = 0; i < OSSL_NELEM(predefined_names); i++)
    if (ossl_property_name(ctx, predefined_names[i], 1) == 0) goto err;
...
static void put_str(const char* str, char** buf, size_t* remain,
  ...
  *needed += len;
  ...
  if (quotes) put_char(quote, buf, remain, needed);

  if (*remain < len + 1 + quotes) len = *remain - 1;
...
size_t ossl_property_list_to_string(OSSL_LIB_CTX* ctx,
  ...
  for (i = 0; i < list->num_properties; i++, prop--) {
    /* Skip invalid names */
    if (prop->name_idx == 0) continue;

    if (needed > 0) put_char(',', &buf, &bufsize, &needed);
