...
static int
match(const char* t[], const char m[], size_t m_len)
{
  const char* s = *t;
...
static int
parse_number(const char* t[], OSSL_PROPERTY_DEFINITION* res)
{
  const char* s = *t;
  int64_t v = 0;
  ...
  do {
    ...
    if (v > ((INT64_MAX - (*s - '0')) / 10)) {
      ERR_raise_data(
        ERR_LIB_PROP, PROP_R_PARSE_FAILED, "Property %s overflows", *t);
      return 0;
    }
    v = v * 10 + (*s++ - '0');
...
static int
parse_hex(const char* t[], OSSL_PROPERTY_DEFINITION* res)
{
  const char* s = *t;
  int64_t v = 0;
  int sval;
  ...
  if (!ossl_isspace(*s) && *s != '\0' && *s != ',') {
    ERR_raise_data(
      ERR_LIB_PROP, PROP_R_NOT_AN_HEXADECIMAL_DIGIT, "HERE-->%s", *t);
    return 0;
  }
  *t = skip_space(s);
  res->type = OSSL_PROPERTY_TYPE_NUMBER;
...
static int
parse_oct(const char* t[], OSSL_PROPERTY_DEFINITION* res)
{
  const char* s = *t;
  int64_t v = 0;
...
static int
parse_string(OSSL_LIB_CTX* ctx,
             const char* t[],
             char delim,
             OSSL_PROPERTY_DEFINITION* res,
             const int create)
{
  char v[1000];
  const char* s = *t;
...
static int
parse_unquoted(OSSL_LIB_CTX* ctx,
               const char* t[],
               OSSL_PROPERTY_DEFINITION* res,
               const int create)
{
...
static int
pd_compare(const OSSL_PROPERTY_DEFINITION* const* p1,
...
static void
pd_free(OSSL_PROPERTY_DEFINITION* pd)
{
  OPENSSL_free(pd);
}
...
ossl_parse_property(OSSL_LIB_CTX* ctx, const char* defn)
{
  ...
  while (!done) {
    ...
    if (prop->name_idx == 0) {
      ERR_raise_data(
        ERR_LIB_PROP, PROP_R_PARSE_FAILED, "Unknown name HERE-->%s", start);
      goto err;
    }
    if (match_ch(&s, '=')) {
...
static void
put_char(char ch, char** buf, size_t* remain, size_t* needed)
{
  if (*remain == 0) {
    ++*needed;
    return;
...
static void
put_str(const char* str, char** buf, size_t* remain, size_t* needed)
{
  size_t olen, len, i;
  char quote = '\0';
  int quotes;
...
static void
put_num(int64_t val, char** buf, size_t* remain, size_t* needed)
{
  int64_t tmpval = val;
  size_t len = 1;
