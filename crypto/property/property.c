...
void
ossl_ctx_global_properties_free (void *vglobp)
{
...
void *
ossl_ctx_global_properties_new (OSSL_LIB_CTX *ctx)
{
  return OPENSSL_zalloc (sizeof (OSSL_GLOBAL_PROPERTIES));
}

OSSL_PROPERTY_LIST **
ossl_ctx_global_properties (OSSL_LIB_CTX *libctx, ossl_unused int loadconfig)
{
...
#ifndef FIPS_MODULE
int
ossl_global_properties_no_mirrored (OSSL_LIB_CTX *libctx)
{
  OSSL_GLOBAL_PROPERTIES *globp
...
void
ossl_global_properties_stop_mirroring (OSSL_LIB_CTX *libctx)
{
  OSSL_GLOBAL_PROPERTIES *globp
      = ossl_lib_ctx_get_data (libctx, OSSL_LIB_CTX_GLOBAL_PROPERTIES);
...
static int
ossl_method_up_ref (METHOD *method)
{
  return (*method->up_ref) (method->method);
}

static void
ossl_method_free (METHOD *method)
{
  (*method->free) (method->method);
}

static __owur int
ossl_property_read_lock (OSSL_METHOD_STORE *p)
{
  return p != NULL ? CRYPTO_THREAD_read_lock (p->lock) : 0;
}

static __owur int
ossl_property_write_lock (OSSL_METHOD_STORE *p)
{
  return p != NULL ? CRYPTO_THREAD_write_lock (p->lock) : 0;
}

static int
ossl_property_unlock (OSSL_METHOD_STORE *p)
{
  return p != 0 ? CRYPTO_THREAD_unlock (p->lock) : 0;
}

static unsigned long
query_hash (const QUERY *a)
{
  return OPENSSL_LH_strhash (a->query);
}

static int
query_cmp (const QUERY *a, const QUERY *b)
{
...
static void
impl_cache_flush_alg (ossl_uintmax_t idx, ALGORITHM *alg)
{
...
static void
alg_cleanup (ossl_uintmax_t idx, ALGORITHM *a, void *arg)
{
 ...
 */
OSSL_METHOD_STORE *
ossl_method_store_new (OSSL_LIB_CTX *ctx)
{
...
int
ossl_method_lock_store (OSSL_METHOD_STORE *store)
{
  return store != NULL ? CRYPTO_THREAD_write_lock (store->biglock) : 0;
}

int
ossl_method_unlock_store (OSSL_METHOD_STORE *store)
{
  return store != NULL ? CRYPTO_THREAD_unlock (store->biglock) : 0;
}

static ALGORITHM *
ossl_method_store_retrieve (OSSL_METHOD_STORE *store, int nid)
{
  return ossl_sa_ALGORITHM_get (store->algs, nid);
}

static int
ossl_method_store_insert (OSSL_METHOD_STORE *store, ALGORITHM *alg)
{
 ...
 *
 * @param store Pointer to the OSSL_METHOD_STORE where the method will be
 * added. Must be non-null.
 ...
 *
 * NOTE: The nid parameter here is _not_ a nid in the sense of the NID_*
 * macros. It is an internal unique identifier.
 */
int
ossl_method_store_add (OSSL_METHOD_STORE *store, const OSSL_PROVIDER *prov,
...
int
ossl_method_store_remove (OSSL_METHOD_STORE *store, int nid,
                          const void *method)
{
  ALGORITHM *alg = NULL;
  int i;
 ...
 * @param alg Pointer to the ALGORITHM structure containing the
 * implementations.
...
int
ossl_method_store_remove_all_provided (OSSL_METHOD_STORE *store,
                                       const OSSL_PROVIDER *prov)
{
...
static void
alg_do_one (ALGORITHM *alg, IMPLEMENTATION *impl,
...
static void
alg_copy (ossl_uintmax_t idx, ALGORITHM *alg, void *arg)
{
...
void
ossl_method_store_do_all (OSSL_METHOD_STORE *store,
  ...
  if (store != NULL)
    {
      ...
      tmpalgs = sk_ALGORITHM_new_reserve (
          NULL, (int)ossl_sa_ALGORITHM_num (store->algs));
      if (tmpalgs == NULL)
        {
          ossl_property_unlock (store);
          return;
 ...
 * method, identified by its id (nid), and matching the given property query.
 * If successful, it returns the method and its associated provider.
 *
 * @param store Pointer to the OSSL_METHOD_STORE from which to fetch the
 ...
 *
 * If tracing is enabled, a message is printed indicating the property query
 * and the resolved provider.
 *
 * NOTE: The nid parameter here is _not_ a NID in the sense of the NID_*
 * macros. It is a unique internal identifier value.
 */
int
ossl_method_store_fetch (OSSL_METHOD_STORE *store, int nid,
                         const char *prop_query, const OSSL_PROVIDER **prov_rw,
                         void **method)
{
...
static void
ossl_method_cache_flush_alg (OSSL_METHOD_STORE *store, ALGORITHM *alg)
{
...
static void
ossl_method_cache_flush (OSSL_METHOD_STORE *store, int nid)
{
...
int
ossl_method_store_cache_flush_all (OSSL_METHOD_STORE *store)
{
  if (!ossl_property_write_lock (store))
    return 0;
 ...
 */
static void
impl_cache_flush_cache (QUERY *c, IMPL_CACHE_FLUSH *state)
{
  uint32_t n;
...
static void
impl_cache_flush_one_alg (ossl_uintmax_t idx, ALGORITHM *alg, void *v)
{
...
static void
ossl_method_cache_flush_some (OSSL_METHOD_STORE *store)
{
...
int
ossl_method_store_cache_get (OSSL_METHOD_STORE *store, OSSL_PROVIDER *prov,
...
int
ossl_method_store_cache_set (OSSL_METHOD_STORE *store, OSSL_PROVIDER *prov,
                             int nid, const char *prop_query, void *method,
                             int (*method_up_ref) (void *),
                             void (*method_destruct) (void *))
{
