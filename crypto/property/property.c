...
void ossl_global_properties_stop_mirroring(OSSL_LIB_CTX* libctx) {
  ...
  if (globp != NULL) globp->no_mirrored = 1;
}
#endif
...
static void alg_cleanup(ossl_uintmax_t idx, ALGORITHM* a, void* arg) {
  ...
  if (store != NULL) ossl_sa_ALGORITHM_set(store->algs, idx, NULL);
}
...
int ossl_method_store_add(OSSL_METHOD_STORE* store, const OSSL_PROVIDER* prov,
  ...
  if (nid <= 0 || method == NULL || store == NULL) return 0;
  ...
  if ((impl->properties = ossl_prop_defn_get(store->ctx, properties)) == NULL) {
    ...
    if (impl->properties == NULL) goto err;
  ...
  if (alg == NULL) {
    if ((alg = OPENSSL_zalloc(sizeof(*alg))) == NULL ||
      ...
      goto err;
    alg->nid = nid;
    if (!ossl_method_store_insert(store, alg)) goto err;
...
static void alg_cleanup_by_provider(ossl_uintmax_t idx, ALGORITHM* alg,
   ...
   */
  if (count > 0) ossl_method_cache_flush_alg(data->store, alg);
}
...
void ossl_method_store_do_all(OSSL_METHOD_STORE* store,
  ...
  if (store != NULL) {
    if (!ossl_property_read_lock(store)) return;
...
int ossl_method_store_fetch(OSSL_METHOD_STORE* store, int nid,
   ...
   */
  if (prop_query != NULL) p2 = pq = ossl_parse_query(store->ctx, prop_query, 0);
...
fin:
  if (ret && ossl_method_up_ref(&best_impl->method)) {
    *method = best_impl->method.method;
    if (prov_rw != NULL) *prov_rw = best_impl->provider;
  } else {
    ret = 0;
  }
...
static void ossl_method_cache_flush(OSSL_METHOD_STORE* store, int nid) {
  ...
  if (alg != NULL) ossl_method_cache_flush_alg(store, alg);
}
...
static void ossl_method_cache_flush_some(OSSL_METHOD_STORE* store) {
  ...
  if (state.using_global_seed) tsan_add(&global_seed, state.seed);
}
...
int ossl_method_store_cache_set(OSSL_METHOD_STORE* store, OSSL_PROVIDER* prov,
  ...
  if (!ossl_property_write_lock(store)) return 0;
  if (store->cache_need_flush) ossl_method_cache_flush_some(store);
  alg = ossl_method_store_retrieve(store, nid);
  if (alg == NULL) goto err;
  ...
  if (p != NULL) {
    ...
    if (!ossl_method_up_ref(&p->method)) goto err;
