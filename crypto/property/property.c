 ...
 */
int
ossl_method_store_add(OSSL_METHOD_STORE* store,
                      ...
                      void (*method_destruct)(void*))
{
  ...
  if (alg == NULL) {
    ...
    if (!ossl_method_store_insert(store, alg))
      goto err;
    OSSL_TRACE2(QUERY,
                "Inserted an alg with nid %d into the store %p\n",
                nid,
                (void*)store);
  }
...
int
ossl_method_store_remove(OSSL_METHOD_STORE* store, int nid, const void* method)
{
  ALGORITHM* alg = NULL;
  int i;
  ...
  for (count = 0, i = sk_IMPLEMENTATION_num(alg->impls); i-- > 0;) {
    ...
    if (impl->provider == data->prov) {
      ...
      OSSL_TRACE_BEGIN(QUERY)
      {
        ...
        size = ossl_property_list_to_string(
          NULL, impl->properties, buf, sizeof(buf));
        BIO_printf(trc_out,
                         ...
                         void* fnarg)
{
  ...
  if (store != NULL) {
    ...
    tmpalgs =
      sk_ALGORITHM_new_reserve(NULL, (int)ossl_sa_ALGORITHM_num(store->algs));
    if (tmpalgs == NULL) {
      ossl_property_unlock(store);
      return;
    }
 ...
 */
int
ossl_method_store_fetch(OSSL_METHOD_STORE* store,
                        int nid,
                        ...
                        void** method)
{
  ...
  if (alg == NULL) {
    ossl_property_unlock(store);
    OSSL_TRACE2(
      QUERY, "Failed to retrieve by nid %d from store %p\n", nid, (void*)store);
    return 0;
  }
...
int
ossl_method_store_cache_get(OSSL_METHOD_STORE* store,
                            OSSL_PROVIDER* prov,
                            int nid,
                            const char* prop_query,
                            void** method)
...
int
ossl_method_store_cache_set(OSSL_METHOD_STORE* store,
                            OSSL_PROVIDER* prov,
                            int nid,
                            const char* prop_query,
                            void* method,
