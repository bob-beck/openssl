...
int ossl_encode_der_length(WPACKET* pkt, size_t cont_len) {
  ...
  if (cont_len > 0xff) {
    ...
    if (cont_len > 0x7f && !WPACKET_put_bytes_u8(pkt, 0x81)) return 0;
    if (!WPACKET_put_bytes_u8(pkt, cont_len)) return 0;
  }
...
int ossl_encode_der_dsa_sig(WPACKET* pkt, const BIGNUM* r, const BIGNUM* s) {
  ...
  if (!WPACKET_start_sub_packet(pkt)) return 0;

  if (!isnull) {
    if (!WPACKET_init_null(&tmppkt, 0)) return 0;
    dummypkt = &tmppkt;
  } else {
  ...
  if (!ossl_encode_der_integer(dummypkt, r) ||
    ...
    if (!isnull) WPACKET_cleanup(dummypkt);
    return 0;
  }
...
int ossl_decode_der_length(PACKET* pkt, PACKET* subpkt) {
  ...
  if (!PACKET_get_1(pkt, &byte)) return 0;

  if (byte < 0x80) return PACKET_get_sub_packet(pkt, subpkt, (size_t)byte);
  if (byte == 0x81) return PACKET_get_length_prefixed_1(pkt, subpkt);
  if (byte == 0x82) return PACKET_get_length_prefixed_2(pkt, subpkt);
...
int ossl_decode_der_integer(PACKET* pkt, BIGNUM* n) {
  ...
  if (!PACKET_get_1(&tmppkt, &tmp) || (tmp & 0x80) != 0) return 0;
