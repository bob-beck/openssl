...
int ossl_DER_w_precompiled(WPACKET* pkt,
                           int tag,
...
int ossl_DER_w_octet_string(WPACKET* pkt,
                            int tag,
...
static int int_der_w_integer(WPACKET* pkt,
                             int tag,
                             int (*put_bytes)(WPACKET* pkt,
                                              const void* v,
...
static int int_put_bytes_uint32(WPACKET* pkt,
                                const void* v,
                                unsigned int* top_byte) {
...
static int int_put_bytes_bn(WPACKET* pkt,
                            const void* v,
                            unsigned int* top_byte) {
  ...
  return WPACKET_get_total_written(pkt, &size1) && WPACKET_close(pkt) &&
         ...
         (size1 == size2
              ...
              : WPACKET_put_bytes_u8(pkt,
                                     DER_F_CONSTRUCTED | DER_P_SEQUENCE)) &&
         int_end_context(pkt, tag);
}
