...
static int int_start_context(WPACKET* pkt, int tag) {
  if (tag < 0) return 1;
  if (!ossl_assert(tag <= 30)) return 0;
...
static int int_put_bytes_bn(WPACKET* pkt, const void* v,
  ...
  *top_byte =
  ...
  if (!WPACKET_allocate_bytes(pkt, n, &p)) return 0;
  if (p != NULL) BN_bn2bin(v, p);
  return 1;
}

int ossl_DER_w_bn(WPACKET* pkt, int tag, const BIGNUM* v) {
  if (v == NULL || BN_is_negative(v)) return 0;
  if (BN_is_zero(v)) return ossl_DER_w_uint32(pkt, tag, 0);
