...
static void s390x_call_kmac(HMAC_CTX* ctx, const unsigned char* in,
  ...
  if (ctx->plat.s390x.ikp) fc |= S390X_KMAC_IKP;

  if (ctx->plat.s390x.iimp) fc |= S390X_KMAC_IIMP;
...
int s390x_HMAC_init(HMAC_CTX* ctx, const void* key, int key_len, ENGINE* impl) {
  ...
  if (ctx->plat.s390x.blk_size < 0) return 0;
  ...
  if (ctx->plat.s390x.size !=
    ...
    if (ctx->plat.s390x.buf == NULL) return 0;
  ...
  if (key != NULL) {
    ...
    if (key_len > ctx->plat.s390x.blk_size) {
      ...
      if (key_len < 0 || key_len > (int)key_param_len) return 0;
...
int s390x_HMAC_update(HMAC_CTX* ctx, const unsigned char* data, size_t len) {
  ...
  if (len > remain) {
    ...
    if (!ossl_assert(ctx->plat.s390x.num == 0)) return 0;
  ...
  if (!ossl_assert(len <= ctx->plat.s390x.size)) return 0;
...
int s390x_HMAC_CTX_copy(HMAC_CTX* dctx, HMAC_CTX* sctx) {
  ...
  if (sctx->plat.s390x.buf != NULL) {
    ...
    if (dctx->plat.s390x.buf == NULL) return 0;
  }
