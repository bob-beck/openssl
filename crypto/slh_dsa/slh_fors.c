 ...
 */
static int
slh_fors_sk_gen (SLH_DSA_HASH_CTX *ctx, const uint8_t *sk_seed,
 ...
 */
static int
slh_fors_node (SLH_DSA_HASH_CTX *ctx, const uint8_t *sk_seed,
 ...
 * @param sig_len  The size of |sig| which is (2 * n + 3) * n + tree_height *
 * n.
 * @returns 1 on success, or 0 on error.
 */
int
ossl_slh_fors_sign (SLH_DSA_HASH_CTX *ctx, const uint8_t *md,
  ...
  for (tree_id = 0; tree_id < k; ++tree_id)
    {
       ...
       * level. For the next level up from the bottom there would be 2048 nodes
 ...
 */
int
ossl_slh_fors_pk_from_sig (SLH_DSA_HASH_CTX *ctx, PACKET *fors_sig_rpkt,
 ...
 */
static void
slh_base_2b (const uint8_t *in, uint32_t b, uint32_t *out, size_t out_len)
{
  size_t consumed = 0;
  uint32_t bits = 0;
  uint32_t total = 0;
