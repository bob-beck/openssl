 ...
 */
static int
slh_fors_sk_gen(SLH_DSA_HASH_CTX* ctx,
                ...
                size_t pk_out_len)
{
  ...
  return key->hash_func->PRF(
    ctx, pk_seed, sk_seed, sk_adrs, pk_out, pk_out_len);
}
 ...
 */
static int
slh_fors_node(SLH_DSA_HASH_CTX* ctx,
              const uint8_t* sk_seed,
              const uint8_t* pk_seed,
              uint8_t* adrs,
              uint32_t node_id,
