...
int ossl_slh_fors_sign(SLH_DSA_HASH_CTX* ctx, const uint8_t* md,
  ...
  for (tree_id = 0; tree_id < k; ++tree_id) {
    ...
    for (layer = 0; layer < a; ++layer) {
      ...
      if (!WPACKET_memcpy(sig_wpkt, out, n)) return 0;
    }
...
int ossl_slh_fors_pk_from_sig(SLH_DSA_HASH_CTX* ctx, PACKET* fors_sig_rpkt,
  ...
  if (!WPACKET_init_static_len(wroot_pkt, roots, sizeof(roots), 0)) return 0;
  ...
  for (i = 0; i < k; ++i) {
    ...
    for (j = 0; j < a; ++j) {
      ...
      if (!PACKET_get_bytes(fors_sig_rpkt, &authj, n)) goto err;
      /* Hash the children together to get the parent nodes public key */
      set_tree_height(adrs, j + 1);
      if ((id & 1) == 0) {
        node_id >>= 1;
        set_tree_index(adrs, node_id);
        if (!H(ctx, pk_seed, adrs, node0, authj, node1, n)) goto err;
      } else {
        node_id = (node_id - 1) >> 1;
        set_tree_index(adrs, node_id);
        if (!H(ctx, pk_seed, adrs, authj, node0, node1, n)) goto err;
      }
      id >>= 1;
    }
    aoff += two_power_a;
  }
  if (!WPACKET_get_total_written(wroot_pkt, &roots_len)) goto err;
...
err:
  if (!WPACKET_finish(wroot_pkt)) ret = 0;
  return ret;
}
