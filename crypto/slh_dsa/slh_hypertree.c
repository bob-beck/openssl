  ...
  for (layer = 0; layer < d; ++layer) {
    ...
    if (layer < d - 1) {
      if (!PACKET_buf_init(xmss_sig_rpkt, psig,
        ...
        return 0;
      if (!ossl_slh_xmss_pk_from_sig(ctx, leaf_id, xmss_sig_rpkt, root, pk_seed,
 ...
 */
int ossl_slh_ht_verify(SLH_DSA_HASH_CTX* ctx,
                       const uint8_t* msg,
                       PACKET* sig_pkt,
                       const uint8_t* pk_seed,
                       uint64_t tree_id,
  ...
  for (layer = 0; layer < d; ++layer) {
    adrsf->set_layer_address(adrs, layer);
    adrsf->set_tree_address(adrs, tree_id);
    if (!ossl_slh_xmss_pk_from_sig(ctx, leaf_id, sig_pkt, node, pk_seed, adrs,
                                   node, sizeof(node)))
      return 0;
    leaf_id = tree_id & mask;
    tree_id >>= tree_height;
