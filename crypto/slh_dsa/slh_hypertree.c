                 ...
                 WPACKET* sig_wpkt)
{
  ...
  for (layer = 0; layer < d; ++layer) {
    ...
    if (!ossl_slh_xmss_sign(
          ctx, root, sk_seed, leaf_id, pk_seed, adrs, sig_wpkt))
      return 0;
    /*
    ...
    if (layer < d - 1) {
      if (!PACKET_buf_init(
            xmss_sig_rpkt, psig, WPACKET_get_curr(sig_wpkt) - psig))
        return 0;
      if (!ossl_slh_xmss_pk_from_sig(ctx,
                                     leaf_id,
                                     xmss_sig_rpkt,
                                     root,
                                     pk_seed,
                   ...
                   const uint8_t* pk_root)
{
  ...
  for (layer = 0; layer < d; ++layer) {
    ...
    if (!ossl_slh_xmss_pk_from_sig(
          ctx, leaf_id, sig_pkt, node, pk_seed, adrs, node, sizeof(node)))
      return 0;
    leaf_id = tree_id & mask;
    tree_id >>= tree_height;
  }
