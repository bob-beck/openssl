...
static void slh_dsa_key_hash_cleanup(SLH_DSA_KEY* key) {
  ...
  if (key->md_big != key->md) EVP_MD_free(key->md_big);
  key->md_big = NULL;
  EVP_MD_free(key->md);
...
static int slh_dsa_key_hash_init(SLH_DSA_KEY* key) {
  ...
  if (is_shake == 0) {
    if (security_category == 1) {
      ...
      if (key->md_big == NULL) goto err;
    }
    key->hmac = EVP_MAC_fetch(key->libctx, "HMAC", key->propq);
    if (key->hmac == NULL) goto err;
  }
...
static void slh_dsa_key_hash_dup(SLH_DSA_KEY* dst, const SLH_DSA_KEY* src) {
  if (src->md_big != NULL && src->md_big != src->md) EVP_MD_up_ref(src->md_big);
  if (src->md != NULL) EVP_MD_up_ref(src->md);
  if (src->hmac != NULL) EVP_MAC_up_ref(src->hmac);
}
...
SLH_DSA_KEY* ossl_slh_dsa_key_new(OSSL_LIB_CTX* libctx, const char* propq,
  ...
  if (ret != NULL) {
    ...
    if (!slh_dsa_key_hash_init(ret)) goto err;
  }
  return ret;
err:
  ...
  if (ret != NULL) {
    *ret = *src; /* this copies everything including the keydata in priv[] */
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
      ...
      if (src->pub != NULL) ret->pub = SLH_DSA_PUB(ret);
...
int ossl_slh_dsa_key_equal(const SLH_DSA_KEY* key1, const SLH_DSA_KEY* key2,
  ...
  if (key1->params != key2->params) return 0;

  if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
    if ((selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
      if (key1->pub != NULL && key2->pub != NULL) {
        if (memcmp(key1->pub, key2->pub, key1->params->pk_len) != 0) return 0;
        key_checked = 1;
      }
    }
    if (!key_checked && (selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
      if (key1->has_priv && key2->has_priv) {
        if (memcmp(key1->priv, key2->priv, key1->params->pk_len) != 0) return 0;
        key_checked = 1;
      }
    }
    return key_checked;
...
int ossl_slh_dsa_key_has(const SLH_DSA_KEY* key, int selection) {
  if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
    if (key->pub == NULL) return 0; /* No public key */
    if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0 &&
        key->has_priv == 0)
...
int ossl_slh_dsa_key_pairwise_check(const SLH_DSA_KEY* key) {
  ...
  if (key->pub == NULL || key->has_priv == 0) return 0;
...
int ossl_slh_dsa_generate_key(SLH_DSA_HASH_CTX* ctx, SLH_DSA_KEY* out,
  ...
  if (entropy != NULL && entropy_len != 0) {
    if (entropy_len != entropy_len_expected) goto err;
  ...
  if (!slh_dsa_compute_pk_root(ctx, out, 0)) goto err;
  out->pub = pub;
  out->has_priv = 1;
  return 1;
err:
...
int ossl_slh_dsa_set_priv(SLH_DSA_KEY* key, const uint8_t* priv,
                          size_t priv_len) {
  if (ossl_slh_dsa_key_get_priv_len(key) != priv_len) return 0;
...
int ossl_slh_dsa_set_pub(SLH_DSA_KEY* key, const uint8_t* pub, size_t pub_len) {
  if (ossl_slh_dsa_key_get_pub_len(key) != pub_len) return 0;
  key->pub = SLH_DSA_PUB(key);
