...
static int
slh_dsa_compute_pk_root(SLH_DSA_HASH_CTX* ctx, SLH_DSA_KEY* out, int verify);
 ...
 */
int
ossl_slh_dsa_key_equal(const SLH_DSA_KEY* key1,
                       const SLH_DSA_KEY* key2,
                       int selection)
{
  int key_checked = 0;
 ...
 */
int
ossl_slh_dsa_key_fromdata(SLH_DSA_KEY* key,
 ...
 */
int
ossl_slh_dsa_generate_key(SLH_DSA_HASH_CTX* ctx,
                          SLH_DSA_KEY* out,
...
ossl_slh_dsa_key_to_text(BIO* out, const SLH_DSA_KEY* key, int selection)
{
  ...
  if (ossl_slh_dsa_key_get_pub(key) == NULL) {
    ...
    ERR_raise_data(
      ERR_LIB_PROV, PROV_R_MISSING_KEY, "no %s key material available", name);
    return 0;
  }
