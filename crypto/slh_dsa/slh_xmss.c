                   ...
                   size_t pk_out_len)
{
  ...
  if (h == 0) {
    ...
    if (!key->hash_func->H(
          ctx, pk_seed, adrs, lnode, rnode, pk_out, pk_out_len))
      return 0;
  }
  return 1;
}
                   ...
                   WPACKET* sig_wpkt)
{
  ...
  for (h = 0; h < hm; ++h) {
    if (!WPACKET_allocate_bytes(sig_wpkt, auth_path_len, &auth_path) ||
        !ossl_slh_xmss_node(
          ctx, sk_seed, id ^ 1, h, pk_seed, adrs, auth_path, auth_path_len))
      return 0;
    id >>= 1;
  }
  return 1;
                          ...
                          size_t pk_out_len)
{
  ...
  adrsf->set_type_and_clear(adrs, SLH_ADRS_TYPE_WOTS_HASH);
  adrsf->set_keypair_address(adrs, node_id);
  if (!ossl_slh_wots_pk_from_sig(
        ctx, sig_rpkt, msg, pk_seed, adrs, node, pk_out_len))
    return 0;
