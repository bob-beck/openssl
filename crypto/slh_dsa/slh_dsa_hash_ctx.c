...
SLH_DSA_HASH_CTX* ossl_slh_dsa_hash_ctx_new(const SLH_DSA_KEY* key) {
  ...
  if (ret->md_ctx == NULL) goto err;
  if (EVP_DigestInit_ex2(ret->md_ctx, key->md, NULL) != 1) goto err;
  if (key->md_big != NULL) {
    ...
    if (key->md_big == key->md) {
      ...
      if (ret->md_big_ctx == NULL) goto err;
      if (EVP_DigestInit_ex2(ret->md_big_ctx, key->md_big, NULL) != 1) goto err;
    }
    if (key->hmac != NULL) {
      ret->hmac_ctx = EVP_MAC_CTX_new(key->hmac);
      if (ret->hmac_ctx == NULL) goto err;
    }
  }
  return ret;
err:
...
SLH_DSA_HASH_CTX* ossl_slh_dsa_hash_ctx_dup(const SLH_DSA_HASH_CTX* src) {
  ...
  if (src->md_big_ctx != NULL) {
    if (src->md_big_ctx != src->md_ctx) {
      if ((ret->md_big_ctx = EVP_MD_CTX_dup(src->md_big_ctx)) == NULL) goto err;
    } else {
      ret->md_big_ctx = ret->md_ctx;
    }
  }
...
void ossl_slh_dsa_hash_ctx_free(SLH_DSA_HASH_CTX* ctx) {
  ...
  if (ctx->md_big_ctx != ctx->md_ctx) EVP_MD_CTX_free(ctx->md_big_ctx);
