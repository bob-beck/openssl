...
static int slh_wots_chain(SLH_DSA_HASH_CTX* ctx, const uint8_t* in,
  ...
  if (steps == 0) return WPACKET_memcpy(wpkt, in, n);

  if (!WPACKET_allocate_bytes(wpkt, tmp_len, &tmp)) return 0;

  set_hash_address(adrs, (uint32_t)(j++));
  if (!F(ctx, pk_seed, adrs, in, n, tmp, tmp_len)) return 0;

  end_index = start_index + steps;
  for (; j < end_index; ++j) {
    set_hash_address(adrs, (uint32_t)j);
    if (!F(ctx, pk_seed, adrs, tmp, n, tmp, tmp_len)) return 0;
  }
  return 1;
}
...
int ossl_slh_wots_pk_gen(SLH_DSA_HASH_CTX* ctx, const uint8_t* sk_seed,
  ...
  if (!WPACKET_init_static_len(tmp_wpkt, tmp, sizeof(tmp), 0)) return 0;
  ...
  for (i = 0; i < len; ++i) { /* len = 2n + 3 */
    ...
    if (!PRF(ctx, pk_seed, sk_seed, sk_adrs, sk, sizeof(sk))) goto end;
...
int ossl_slh_wots_sign(SLH_DSA_HASH_CTX* ctx, const uint8_t* msg,
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (!PRF(ctx, pk_seed, sk_seed, sk_adrs, sk, sizeof(sk))) goto err;
    set_chain_address(adrs, (uint32_t)i);
...
int ossl_slh_wots_pk_from_sig(SLH_DSA_HASH_CTX* ctx, PACKET* sig_rpkt,
  ...
  if (!WPACKET_init_static_len(tmp_pkt, tmp, sizeof(tmp), 0)) return 0;
  ...
  adrsf->set_type_and_clear(wots_pk_adrs, SLH_ADRS_TYPE_WOTS_PK);
  adrsf->copy_keypair_address(wots_pk_adrs, adrs);
  if (!WPACKET_get_total_written(tmp_pkt, &tmp_len)) goto err;
  ret = hashf->T(ctx, pk_seed, wots_pk_adrs, tmp, tmp_len, pk_out, pk_out_len);
err:
  if (!WPACKET_finish(tmp_pkt)) ret = 0;
  return ret;
}
