 ...
 * functions. It stores information related to the type of operation, as well
 * as information related to tree addresses and heights. SHAKE based algorithms
 * use 32 bytes for this object, whereas SHA2 based algorithms use a compressed
 * format of 22 bytes. For this reason there are different method tables to
 * support the different formats. FIPS 205 Section 4.2 describes the SHAKE
 * related functions. The compressed format is discussed in Section 11.2.
 */
 ...
 */
typedef void (OSSL_SLH_ADRS_FUNC_set_tree_address) (uint8_t *adrs,
                                                    uint64_t in);
...
typedef void (OSSL_SLH_ADRS_FUNC_set_keypair_address) (uint8_t *adrs,
                                                       uint32_t in);
typedef void (OSSL_SLH_ADRS_FUNC_set_chain_address) (uint8_t *adrs,
                                                     uint32_t in);
typedef void (OSSL_SLH_ADRS_FUNC_set_tree_height) (uint8_t *adrs, uint32_t in);
typedef void (OSSL_SLH_ADRS_FUNC_set_hash_address) (uint8_t *adrs,
                                                    uint32_t in);
