...
static int slh_sign_internal(SLH_DSA_HASH_CTX* hctx, const uint8_t* msg,
  ...
  if (!WPACKET_init_static_len(wpkt, sig, sig_len_expected, 0)) return 0;
  if (!PACKET_buf_init(rpkt, m_digest, params->m)) return 0;

  pk_seed = SLH_DSA_PK_SEED(priv);
  sk_seed = SLH_DSA_SK_SEED(priv);

  if (opt_rand == NULL) opt_rand = pk_seed;
...
static int slh_verify_internal(SLH_DSA_HASH_CTX* hctx, const uint8_t* msg,
  ...
  if (sig_len != params->sig_len || !PACKET_buf_init(sig_rpkt, sig, sig_len))
    return 0;
  if (!PACKET_get_bytes(sig_rpkt, &r, n)) return 0;
...
static uint8_t* msg_encode(const uint8_t* msg, size_t msg_len,
  ...
  if (ctx_len > SLH_DSA_MAX_CONTEXT_STRING_LEN) return NULL;
...
int ossl_slh_dsa_sign(SLH_DSA_HASH_CTX* slh_ctx, const uint8_t* msg,
  ...
  if (m != msg && m != m_tmp) OPENSSL_free(m);
  return ret;
}
