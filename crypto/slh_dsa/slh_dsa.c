 ...
 */
static int
slh_sign_internal(SLH_DSA_HASH_CTX* hctx,
                  const uint8_t* msg,
                  size_t msg_len,
                  uint8_t* sig,
                  size_t* sig_len,
                  ...
                  const uint8_t* opt_rand)
{
  ...
  ret =
    ...
    && ossl_slh_fors_pk_from_sig(
         hctx, rpkt, md, pk_seed, adrs, pk_fors, sizeof(pk_fors))
 ...
 */
static int
slh_verify_internal(SLH_DSA_HASH_CTX* hctx,
                    const uint8_t* msg,
                    size_t msg_len,
                    const uint8_t* sig,
                    size_t sig_len)
{
  ...
  if (!hashf->H_MSG(
        hctx, r, pk_seed, pk_root, msg, msg_len, m_digest, sizeof(m_digest)))
    return 0;
  ...
  adrsf->set_type_and_clear(adrs, SLH_ADRS_TYPE_FORS_TREE);
  adrsf->set_keypair_address(adrs, leaf_id);
  return ossl_slh_fors_pk_from_sig(
           hctx, sig_rpkt, md, pk_seed, adrs, pk_fors, sizeof(pk_fors)) &&
         ossl_slh_ht_verify(
           hctx, pk_fors, sig_rpkt, pk_seed, tree_id, leaf_id, pk_root) &&
                  ...
                  size_t sigsize)
{
  ...
  if (sig != NULL) {
    m = msg_encode(
      msg, msg_len, ctx, ctx_len, encode, m_tmp, sizeof(m_tmp), &m_len);
    if (m == NULL)
      return 0;
  }
 ...
 */
static uint64_t
bytes_to_u64_be(const uint8_t* in, size_t in_len)
{
