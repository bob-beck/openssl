...
static const struct trace_category_st trace_categories[OSSL_TRACE_CATEGORY_NUM]
    = {
      ...
      }; /* KEEP THIS LIST IN SYNC with #define OSSL_TRACE_CATEGORY_... in
            trace.h */
...
static int
set_trace_data (int category, int type, BIO **channel, const char **prefix,
                const char **suffix, int (*attach_cb) (int, int, const void *),
...
int
OSSL_trace_string (BIO *out, int text, int full, const unsigned char *data,
                   size_t size)
{
  ...
  if (!text)
    { /* mask control characters while preserving newlines */
