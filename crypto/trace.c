...
static int trace_puts(BIO* channel, const char* str) {
  ...
  if (trace_write(channel, str, strlen(str), &written)) return (int)written;
...
const char* OSSL_trace_get_category_name(int num) {
  if (num < 0 || (size_t)num >= OSSL_NELEM(trace_categories)) return NULL;
  /*
...
static int set_trace_data(int category, int type, BIO** channel,
  ...
  if (prefix != NULL && *prefix != NULL) {
    if ((curr_prefix = OPENSSL_strdup(*prefix)) == NULL) return 0;
    trace_channels[category].prefix = curr_prefix;
  }

  if (suffix != NULL && *suffix != NULL) {
    if ((curr_suffix = OPENSSL_strdup(*suffix)) == NULL) return 0;
...
void ossl_trace_cleanup(void) {
  ...
  for (category = 0; category < OSSL_TRACE_CATEGORY_NUM; category++) {
    ...
    if (category == OSSL_TRACE_CATEGORY_TRACE) continue;
...
int OSSL_trace_set_callback(int category, OSSL_trace_cb callback, void* data) {
  ...
  if (category < 0 || category >= OSSL_TRACE_CATEGORY_NUM) return 0;
...
static int ossl_trace_get_category(int category) {
  if (category < 0 || category >= OSSL_TRACE_CATEGORY_NUM) return -1;
  if (trace_channels[category].bio != NULL) return category;
...
int OSSL_trace_enabled(int category) {
  ...
  if (category >= 0) ret = trace_channels[category].bio != NULL;
#endif
  return ret;
}
...
BIO* OSSL_trace_begin(int category) {
  ...
  category = ossl_trace_get_category(category);
  if (category < 0 || !OSSL_trace_enabled(category)) return NULL;

  channel = trace_channels[category].bio;
  prefix = trace_channels[category].prefix;

  if (channel != NULL) {
    if (!CRYPTO_THREAD_write_lock(trace_lock)) return NULL;
