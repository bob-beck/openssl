 ...
 */
static int
trace_write(BIO* b, const char* buf, size_t num, size_t* written);
static int
trace_puts(BIO* b, const char* str);
static long
trace_ctrl(BIO* channel, int cmd, long argl, void* argp);
static int
trace_free(BIO* b);
...
static int
trace_write(BIO* channel, const char* buf, size_t num, size_t* written)
{
...
static int
trace_puts(BIO* channel, const char* str)
{
  size_t written;
...
int
OSSL_trace_get_category_num(const char* name)
{
  size_t i;
...
static int
trace_attach_cb(int category, int type, const void* data)
{
  switch (type) {
    case CHANNEL:
      OSSL_TRACE2(TRACE,
                  "Attach channel %p to category '%s'\n",
                  data,
                  trace_categories[category].name);
      break;
    case PREFIX:
      OSSL_TRACE2(TRACE,
                  "Attach prefix \"%s\" to category '%s'\n",
                  (const char*)data,
                  trace_categories[category].name);
      break;
    case SUFFIX:
      OSSL_TRACE2(TRACE,
                  "Attach suffix \"%s\" to category '%s'\n",
                  (const char*)data,
                  trace_categories[category].name);
      break;
    default: /* No clue */
...
static int
trace_detach_cb(int category, int type, const void* data)
{
  switch (type) {
    case CHANNEL:
      OSSL_TRACE2(TRACE,
                  "Detach channel %p from category '%s'\n",
                  data,
                  trace_categories[category].name);
      break;
    case PREFIX:
      OSSL_TRACE2(TRACE,
                  "Detach prefix \"%s\" from category '%s'\n",
                  (const char*)data,
                  trace_categories[category].name);
      break;
    case SUFFIX:
      OSSL_TRACE2(TRACE,
                  "Detach suffix \"%s\" from category '%s'\n",
                  (const char*)data,
                  trace_categories[category].name);
      break;
    default: /* No clue */
...
ossl_trace_cleanup(void)
{
  ...
  set_trace_data(OSSL_TRACE_CATEGORY_TRACE,
                 0,
                 &channel,
                 &prefix,
                 &suffix,
...
int
OSSL_trace_set_channel(int category, BIO* channel)
{
#ifndef OPENSSL_NO_TRACE
  if (category >= 0 && category < OSSL_TRACE_CATEGORY_NUM)
    return set_trace_data(category,
                          SIMPLE_CHANNEL,
                          &channel,
                          NULL,
                          NULL,
...
#ifndef OPENSSL_NO_TRACE
static int
trace_attach_w_callback_cb(int category, int type, const void* data)
{
  switch (type) {
    case CHANNEL:
      OSSL_TRACE2(TRACE,
...
int
OSSL_trace_set_callback(int category, OSSL_trace_cb callback, void* data)
{
#ifndef OPENSSL_NO_TRACE
  BIO* channel = NULL;
  ...
  if (!set_trace_data(category,
                      CALLBACK_CHANNEL,
                      &channel,
                      NULL,
                      NULL,
                      trace_attach_w_callback_cb,
                      trace_detach_cb))
    goto err;
...
int
OSSL_trace_set_prefix(int category, const char* prefix)
{
#ifndef OPENSSL_NO_TRACE
  if (category >= 0 && category < OSSL_TRACE_CATEGORY_NUM)
    return set_trace_data(
      category, 0, NULL, &prefix, NULL, trace_attach_cb, trace_detach_cb);
#endif
  return 0;
}

int
OSSL_trace_set_suffix(int category, const char* suffix)
{
#ifndef OPENSSL_NO_TRACE
  if (category >= 0 && category < OSSL_TRACE_CATEGORY_NUM)
    return set_trace_data(
      category, 0, NULL, NULL, &suffix, trace_attach_cb, trace_detach_cb);
#endif
  return 0;
}
...
#ifndef OPENSSL_NO_TRACE
static int
ossl_trace_get_category(int category)
{
  if (category < 0 || category >= OSSL_TRACE_CATEGORY_NUM)
    return -1;
...
int
OSSL_trace_enabled(int category)
{
  int ret = 0;
#ifndef OPENSSL_NO_TRACE
  ...
  if (channel != NULL) {
    ...
    switch (trace_channels[category].type) {
      ...
      case CALLBACK_CHANNEL:
        (void)BIO_ctrl(channel,
                       OSSL_TRACE_CTRL_BEGIN,
                       prefix == NULL ? 0 : (int)strlen(prefix),
                       prefix);
        break;
    }
  }
...
void
OSSL_trace_end(int category, BIO* channel)
{
#ifndef OPENSSL_NO_TRACE
  char* suffix = NULL;
  ...
  if (channel != NULL && ossl_assert(channel == current_channel)) {
    ...
    switch (trace_channels[category].type) {
      ...
      case CALLBACK_CHANNEL:
        (void)BIO_ctrl(channel,
                       OSSL_TRACE_CTRL_END,
                       suffix == NULL ? 0 : (int)strlen(suffix),
                       suffix);
        break;
    }
