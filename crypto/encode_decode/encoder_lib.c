...
int
OSSL_ENCODER_to_data(OSSL_ENCODER_CTX* ctx,
                     unsigned char** pdata,
                     size_t* pdata_len)
{
  BIO* out;
...
int
OSSL_ENCODER_CTX_set_output_type(OSSL_ENCODER_CTX* ctx, const char* output_type)
{
...
int
OSSL_ENCODER_CTX_add_extra(OSSL_ENCODER_CTX* ctx,
                           OSSL_LIB_CTX* libctx,
                           const char* propq)
{
  return 1;
...
int
OSSL_ENCODER_CTX_set_construct_data(OSSL_ENCODER_CTX* ctx, void* construct_data)
{
  if (!ossl_assert(ctx != NULL)) {
    ERR_raise(ERR_LIB_OSSL_ENCODER, ERR_R_PASSED_NULL_PARAMETER);
    return 0;
...
int
ossl_bio_print_labeled_buf(BIO* out,
                           const char* label,
