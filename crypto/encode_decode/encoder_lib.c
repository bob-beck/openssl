...
int OSSL_ENCODER_to_fp(OSSL_ENCODER_CTX* ctx, FILE* fp) {
  ...
  if (b != NULL) ret = OSSL_ENCODER_to_bio(ctx, b);
...
int OSSL_ENCODER_CTX_add_encoder(OSSL_ENCODER_CTX* ctx, OSSL_ENCODER* encoder) {
  ...
  if (!ossl_encoder_ctx_add_encoder_inst(ctx, encoder_inst)) goto err;

  return 1;
err:
  ossl_encoder_instance_free(encoder_inst);
  if (encoderctx != NULL) encoder->freectx(encoderctx);
  return 0;
}
...
int OSSL_ENCODER_CTX_get_num_encoders(OSSL_ENCODER_CTX* ctx) {
  if (ctx == NULL || ctx->encoder_insts == NULL) return 0;
...
OSSL_ENCODER* OSSL_ENCODER_INSTANCE_get_encoder(
    OSSL_ENCODER_INSTANCE* encoder_inst) {
  if (encoder_inst == NULL) return NULL;
...
const char* OSSL_ENCODER_INSTANCE_get_output_type(
    OSSL_ENCODER_INSTANCE* encoder_inst) {
  if (encoder_inst == NULL) return NULL;
...
static int encoder_process(struct encoder_process_data_st* data) {
  ...
  if (i < 0) {
    ...
    switch (ok) {
      ...
      case -1:
         ...
         */
        if (data->count_output_structure == 0) return 0;
...

        {
          ...
          *abstract_p++ = OSSL_PARAM_construct_utf8_string(
          ...
          if (prev_output_structure != NULL)
            *abstract_p++ = OSSL_PARAM_construct_utf8_string(
                OSSL_OBJECT_PARAM_DATA_STRUCTURE, (char*)prev_output_structure,
                0);
    ...
    if (ok) {
      ...
      if (ok) ok = (cbio = ossl_core_bio_new_from_bio(current_out)) != NULL;
      if (ok) {
  ...
  if (original_data != NULL) data->ctx->cleanup(data->ctx->construct_data);
  return ok;
}
...
int ossl_bio_print_labeled_bignum(BIO* out, const char* label,
  ...
  if (BN_is_zero(bn)) return BIO_printf(out, "%s%s0\n", label, post_label_spc);

  if (BN_num_bytes(bn) <= BN_BYTES) {
    BN_ULONG* words = bn_get_words(bn);

    if (BN_is_negative(bn)) neg = "-";
  ...
  if (BIO_printf(out, "%s%s\n", label, neg) <= 0) goto err;

  /* Keep track of how many bytes we have printed out so far */
  bytes = 0;

  if (BIO_printf(out, "%s", spaces) <= 0) goto err;

  /* Add a leading 00 if the top bit is set */
  if (*p >= '8') {
    if (BIO_printf(out, "%02x", 0) <= 0) goto err;
    ++bytes;
    use_sep = 1;
  }
  while (*p != '\0') {
...
int ossl_bio_print_ffc_params(BIO* out, const FFC_PARAMS* ffc) {
  ...
  if (!ossl_bio_print_labeled_bignum(out, "P:   ", ffc->p)) goto err;
  if (ffc->q != NULL) {
    if (!ossl_bio_print_labeled_bignum(out, "Q:   ", ffc->q)) goto err;
  }
  if (!ossl_bio_print_labeled_bignum(out, "G:   ", ffc->g)) goto err;
  if (ffc->j != NULL) {
    if (!ossl_bio_print_labeled_bignum(out, "J:   ", ffc->j)) goto err;
  }
  if (ffc->seed != NULL) {
