...
static int
decoder_process(const OSSL_PARAM params[], void* arg);

int
OSSL_DECODER_from_bio(OSSL_DECODER_CTX* ctx, BIO* in)
{
...
int
OSSL_DECODER_from_fp(OSSL_DECODER_CTX* ctx, FILE* fp)
{
...
int
OSSL_DECODER_from_data(OSSL_DECODER_CTX* ctx,
...
int
OSSL_DECODER_CTX_set_selection(OSSL_DECODER_CTX* ctx, int selection)
{
...
int
OSSL_DECODER_CTX_set_input_type(OSSL_DECODER_CTX* ctx, const char* input_type)
{
  if (!ossl_assert(ctx != NULL)) {
    ERR_raise(ERR_LIB_OSSL_DECODER, ERR_R_PASSED_NULL_PARAMETER);
    return 0;
...
int
OSSL_DECODER_CTX_set_input_structure(OSSL_DECODER_CTX* ctx,
                                     const char* input_structure)
{
  if (!ossl_assert(ctx != NULL)) {
    ERR_raise(ERR_LIB_OSSL_DECODER, ERR_R_PASSED_NULL_PARAMETER);
...
ossl_decoder_instance_new_forprov(OSSL_DECODER* decoder,
                                  void* provctx,
                                  const char* input_structure)
{
  void* decoderctx;
...
void
ossl_decoder_ctx_set_harderr(OSSL_DECODER_CTX* ctx)
{
  ctx->harderr = 1;
}

int
ossl_decoder_ctx_get_harderr(const OSSL_DECODER_CTX* ctx)
{
  return ctx->harderr;
}

int
ossl_decoder_ctx_add_decoder_inst(OSSL_DECODER_CTX* ctx,
...
int
OSSL_DECODER_CTX_add_decoder(OSSL_DECODER_CTX* ctx, OSSL_DECODER* decoder)
{
  OSSL_DECODER_INSTANCE* decoder_inst = NULL;
...
static void
collect_all_decoders(OSSL_DECODER* decoder, void* arg)
{
...
static void
collect_extra_decoder(OSSL_DECODER* decoder, void* arg)
{
  ...
  if (ossl_decoder_fast_is_a(
        decoder, data->output_type, &data->output_type_id)) {
    void* decoderctx = NULL;
...
static int
decoder_sk_cmp(const OSSL_DECODER_INSTANCE* const* a,
...
int
OSSL_DECODER_CTX_add_extra(OSSL_DECODER_CTX* ctx,
                           OSSL_LIB_CTX* libctx,
                           const char* propq)
{
  /*
...
int
OSSL_DECODER_CTX_set_construct(OSSL_DECODER_CTX* ctx,
...
int
OSSL_DECODER_CTX_set_construct_data(OSSL_DECODER_CTX* ctx, void* construct_data)
{
  if (!ossl_assert(ctx != NULL)) {
    ERR_raise(ERR_LIB_OSSL_DECODER, ERR_R_PASSED_NULL_PARAMETER);
    return 0;
...
int
OSSL_DECODER_CTX_set_cleanup(OSSL_DECODER_CTX* ctx,
...
int
OSSL_DECODER_export(OSSL_DECODER_INSTANCE* decoder_inst,
                    void* reference,
                    size_t reference_sz,
                    ...
                    void* export_cbarg)
{
  ...
  return decoder->export_object(
    decoderctx, reference, reference_sz, export_cb, export_cbarg);
}
...
#define LEVEL                                                                  \
  ...
  for (i = data->current_decoder_inst_index; i-- > 0;) {
    ...
    const char* new_input_structure = OSSL_DECODER_INSTANCE_get_input_structure(
    ...
    if (decoder != NULL &&
        !ossl_decoder_fast_is_a(
          decoder, new_input_type, &new_decoder_inst->input_type_id)) {
      OSSL_TRACE_BEGIN(DECODER)
      {
        BIO_printf(trc_out,
    ...
    if (data_structure != NULL &&
      ...
      OSSL_TRACE_BEGIN(DECODER)
      {
        BIO_printf(
          ...
          "match the input structure of the considered decoder, skipping...\n",
          (void*)new_data.ctx,
          LEVEL,
          (unsigned int)i);
      }
    ...
    ok = new_decoder->decode(new_decoderctx,
                             cbio,
