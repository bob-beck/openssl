...
int OSSL_DECODER_from_fp(OSSL_DECODER_CTX* ctx, FILE* fp) {
  ...
  if (b != NULL) ret = OSSL_DECODER_from_bio(ctx, b);
...
OSSL_DECODER_INSTANCE* ossl_decoder_instance_new_forprov(
    OSSL_DECODER* decoder, void* provctx, const char* input_structure) {
  void* decoderctx;
...
OSSL_DECODER_INSTANCE* ossl_decoder_instance_dup(
    const OSSL_DECODER_INSTANCE* src) {
  OSSL_DECODER_INSTANCE* dest;
  const OSSL_PROVIDER* prov;
  void* provctx;

  if ((dest = OPENSSL_zalloc(sizeof(*dest))) == NULL) return NULL;
...
int OSSL_DECODER_CTX_add_decoder(OSSL_DECODER_CTX* ctx, OSSL_DECODER* decoder) {
  ...
  if (!ossl_decoder_ctx_add_decoder_inst(ctx, decoder_inst)) goto err;

  return 1;
err:
  ossl_decoder_instance_free(decoder_inst);
  if (decoderctx != NULL) decoder->freectx(decoderctx);
  return 0;
}
...
static void collect_extra_decoder(OSSL_DECODER* decoder, void* arg) {
  ...
  if (ossl_decoder_fast_is_a(decoder, data->output_type,
    ...
    if ((decoderctx = decoder->newctx(provctx)) == NULL) return;
...
int OSSL_DECODER_CTX_add_extra(OSSL_DECODER_CTX* ctx, OSSL_LIB_CTX* libctx,
   ...
   */
  if (ctx->decoder_insts == NULL) return 1;
...
int OSSL_DECODER_CTX_get_num_decoders(OSSL_DECODER_CTX* ctx) {
  if (ctx == NULL || ctx->decoder_insts == NULL) return 0;
...
OSSL_DECODER* OSSL_DECODER_INSTANCE_get_decoder(
    OSSL_DECODER_INSTANCE* decoder_inst) {
  if (decoder_inst == NULL) return NULL;
...
const char* OSSL_DECODER_INSTANCE_get_input_type(
    OSSL_DECODER_INSTANCE* decoder_inst) {
  if (decoder_inst == NULL) return NULL;
  return decoder_inst->input_type;
}

const char* OSSL_DECODER_INSTANCE_get_input_structure(
    OSSL_DECODER_INSTANCE* decoder_inst, int* was_set) {
  if (decoder_inst == NULL) return NULL;
...
#define LEVEL                                                   \
  ...
  if (params == NULL) {
    ...
    if (p == NULL || p->data_type != OSSL_PARAM_OCTET_STRING) goto end;
    new_data.bio = BIO_new_mem_buf(p->data, (int)p->data_size);
    if (new_data.bio == NULL) goto end;
    bio = new_data.bio;

    /* Get the data type if there is one */
    p = OSSL_PARAM_locate_const(params, OSSL_OBJECT_PARAM_DATA_TYPE);
    if (p != NULL && !OSSL_PARAM_get_utf8_string_ptr(p, &data_type)) goto end;
    ...
    if (p != NULL) {
      if (!OSSL_PARAM_get_utf8_string_ptr(p, &ctx->start_input_type)) goto end;
      /*
   ...
   * we failed
   */
  if (data->current_decoder_inst_index == 0) goto end;
  ...
  for (i = data->current_decoder_inst_index; i-- > 0;) {
    ...
    if (data_structure != NULL &&
      ...
      OSSL_TRACE_BEGIN(DECODER) {
        BIO_printf(trc_out,
                   ...
                   "doesn't match the input structure of the considered "
                   "decoder, skipping...\n",
                   (void*)new_data.ctx, LEVEL, (unsigned int)i);
      }
      OSSL_TRACE_END(DECODER);
     ...
     */
    if (new_data.flag_next_level_called) break;
  }
