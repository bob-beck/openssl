...
int
OSSL_DECODER_from_data (OSSL_DECODER_CTX *ctx, const unsigned char **pdata,
                        size_t *pdata_len)
{
  BIO *membio;
  int ret = 0;
...
int
OSSL_DECODER_CTX_add_extra (OSSL_DECODER_CTX *ctx, OSSL_LIB_CTX *libctx,
                            const char *propq)
{
  /*
...
int
OSSL_DECODER_CTX_set_construct_data (OSSL_DECODER_CTX *ctx,
                                     void *construct_data)
{
  if (!ossl_assert (ctx != NULL))
    {
...
int
OSSL_DECODER_export (OSSL_DECODER_INSTANCE *decoder_inst, void *reference,
  ...
  for (i = data->current_decoder_inst_index; i-- > 0;)
    {
      ...
      if (decoder == NULL && ctx->start_input_type != NULL
          && OPENSSL_strcasecmp (ctx->start_input_type, new_input_type) != 0)
        {
          OSSL_TRACE_BEGIN (DECODER)
          {
            BIO_printf (
                trc_out,
                "(ctx %p) %s [%u] the start input type '%s' doesn't match the "
                "input type of the considered decoder, skipping...\n",
                (void *)new_data.ctx, LEVEL, (unsigned int)i,
      ...
      if (decoder != NULL
                                      ...
                                      &new_decoder_inst->input_type_id))
        {
          ...
          {
            BIO_printf (trc_out,
                        "(ctx %p) %s [%u] the input type doesn't match the "
                        "name of the previous decoder (%p), skipping...\n",
                        (void *)new_data.ctx, LEVEL, (unsigned int)i,
                        (void *)decoder);
          }
          OSSL_TRACE_END (DECODER);
                     ...
                     != 0))
        {
          OSSL_TRACE_BEGIN (DECODER)
          {
            BIO_printf (trc_out,
                        ...
                        "structure doesn't match the input structure of the "
                        "considered decoder, skipping...\n",
                        (void *)new_data.ctx, LEVEL, (unsigned int)i);
          }
          OSSL_TRACE_END (DECODER);
          continue;
          ...
          if (OPENSSL_strcasecmp (new_input_structure, ctx->input_structure)
              != 0)
            {
              OSSL_TRACE_BEGIN (DECODER)
              {
                BIO_printf (trc_out,
                            "(ctx %p) %s [%u] the previous decoder's data "
                            "structure doesn't match the input structure "
                            "given by the user, skipping...\n",
                            (void *)new_data.ctx, LEVEL, (unsigned int)i);
              }
              OSSL_TRACE_END (DECODER);
              continue;
