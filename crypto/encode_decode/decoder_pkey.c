...
int
OSSL_DECODER_CTX_set_passphrase (OSSL_DECODER_CTX *ctx,
...
int
OSSL_DECODER_CTX_set_passphrase_ui (OSSL_DECODER_CTX *ctx,
...
int
OSSL_DECODER_CTX_set_pem_password_cb (OSSL_DECODER_CTX *ctx,
...
int
OSSL_DECODER_CTX_set_passphrase_cb (OSSL_DECODER_CTX *ctx,
...
static int
decoder_construct_pkey (OSSL_DECODER_INSTANCE *decoder_inst,
  ...
  if (i < end)
    {
      ...
      if (!EVP_KEYMGMT_up_ref (keymgmt))
        return 0;
            ...
            = EVP_KEYMGMT_fetch (data->libctx, data->object_type, data->propq))
           != NULL)
    {
      keymgmt_prov = EVP_KEYMGMT_get0_provider (keymgmt);
    }
...
static void
decoder_clean_pkey_construct_arg (void *construct_data)
{
  struct decoder_pkey_data_st *data = construct_data;
 ...
 */
static int
collect_decoder_keymgmt (EVP_KEYMGMT *keymgmt, OSSL_DECODER *decoder,
...
static void
collect_decoder (OSSL_DECODER *decoder, void *arg)
{
 ...
 */
static int
check_keymgmt (EVP_KEYMGMT *keymgmt, struct collect_data_st *data)
{
...
static void
collect_keymgmt (EVP_KEYMGMT *keymgmt, void *arg)
{
  struct collect_data_st *data = arg;
...
/*
 * This function does the actual binding of decoders to the OSSL_DECODER_CTX.
 * It searches for decoders matching 'keytype', which is a string like "RSA",
 * "DH", etc. If 'keytype' is NULL, decoders for all keytypes are bound.
 */
static int
ossl_decoder_ctx_setup_for_pkey (OSSL_DECODER_CTX *ctx, const char *keytype,
...
static EVP_KEYMGMT *
keymgmt_dup (const EVP_KEYMGMT *keymgmt)
{
...
static void
decoder_cache_entry_free (DECODER_CACHE_ENTRY *entry)
{
  if (entry == NULL)
    return;
...
nullstrcmp (const char *a, const char *b, int casecmp)
{
  if (a == NULL || b == NULL)
    {
      if (a == NULL)
...
static int
decoder_cache_entry_cmp (const DECODER_CACHE_ENTRY *a,
...
void *
ossl_decoder_cache_new (OSSL_LIB_CTX *ctx)
{
...
void
ossl_decoder_cache_free (void *vcache)
{
 ...
 */
int
ossl_decoder_cache_flush (OSSL_LIB_CTX *libctx)
{
  DECODER_CACHE *cache
