...
static int decoder_construct_pkey(OSSL_DECODER_INSTANCE* decoder_inst,
  ...
  if (p != NULL) {
    ...
    if (!OSSL_PARAM_get_utf8_string(p, &object_type, 0)) return 0;
  ...
  if (p == NULL || p->data_type != OSSL_PARAM_OCTET_STRING) return 0;
  object_ref = p->data;
  ...
  if (i < end) {
    ...
    if (!EVP_KEYMGMT_up_ref(keymgmt)) return 0;
  ...
  if (keymgmt != NULL) {
     ...
     */
    if (keydata == NULL) ossl_decoder_ctx_set_harderr(data->ctx);
...
static void collect_decoder(OSSL_DECODER* decoder, void* arg) {
  ...
  for (i = 0; i < end_i; ++i) {
    ...
    if (collect_decoder_keymgmt(keymgmt, decoder, provctx, data)) break;
    if (data->error_occurred) return;
  }
}
...
static void collect_keymgmt(EVP_KEYMGMT* keymgmt, void* arg) {
  ...
  if (!check_keymgmt(keymgmt, data)) return;
   ...
   * frees it.
   */
  if (!EVP_KEYMGMT_up_ref(keymgmt)) return;
...
static int ossl_decoder_ctx_setup_for_pkey(OSSL_DECODER_CTX* ctx,
  ...
  /* Allocate data. */
  if ((process_data = OPENSSL_zalloc(sizeof(*process_data))) == NULL) goto err;
  if ((propquery != NULL &&
   ...
   */
  if (propquery != NULL) p2 = pq = ossl_parse_query(libctx, propquery, 1);
  ...
  if (collect_data.error_occurred) goto err;
...
static EVP_KEYMGMT* keymgmt_dup(const EVP_KEYMGMT* keymgmt) {
  if (!EVP_KEYMGMT_up_ref((EVP_KEYMGMT*)keymgmt)) return NULL;
 ...
 */
static OSSL_DECODER_CTX* ossl_decoder_ctx_for_pkey_dup(
    OSSL_DECODER_CTX* src, EVP_PKEY** pkey, const char* input_type,
...
static unsigned long decoder_cache_entry_hash(
    const DECODER_CACHE_ENTRY* cache) {
...
OSSL_DECODER_CTX* OSSL_DECODER_CTX_new_for_pkey(
    EVP_PKEY** pkey, const char* input_type, const char* input_structure,
  ...
  if (res == NULL) {
    ...
    if (input_type != NULL) {
      ...
      if (newcache->input_type == NULL) goto err;
    }
    if (input_structure != NULL) {
      newcache->input_structure = OPENSSL_strdup(input_structure);
      if (newcache->input_structure == NULL) goto err;
    }
    if (keytype != NULL) {
      newcache->keytype = OPENSSL_strdup(keytype);
      if (newcache->keytype == NULL) goto err;
    }
    if (propquery != NULL) {
      newcache->propquery = OPENSSL_strdup(propquery);
      if (newcache->propquery == NULL) goto err;
    }
