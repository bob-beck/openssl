...
static OSSL_DECODER* ossl_decoder_new(void) {
  ...
  if ((decoder = OPENSSL_zalloc(sizeof(*decoder))) == NULL) return NULL;
...
void* ossl_decoder_from_algorithm(int id, const OSSL_ALGORITHM* algodef,
  ...
  if ((decoder = ossl_decoder_new()) == NULL) return NULL;
  decoder->base.id = id;
...
static void* construct_decoder(const OSSL_ALGORITHM* algodef,
  ...
  if (id != 0) method = ossl_decoder_from_algorithm(id, algodef, prov);
...
static OSSL_DECODER* inner_ossl_decoder_fetch(struct decoder_data_st* methdata,
  ...
  if (id == 0 ||
    ...
    if ((method = ossl_method_construct(methdata->libctx, OSSL_OP_DECODER,
       ...
       */
      if (id == 0 && name != NULL) id = ossl_namemap_name2num(namemap, name);
      if (id != 0)
  ...
  if ((id != 0 || name != NULL) && method == NULL) {
    ...
    if (name == NULL) name = ossl_namemap_num2name(namemap, id, 0);
...
const OSSL_PROPERTY_LIST* ossl_decoder_parsed_properties(
...
int ossl_decoder_fast_is_a(OSSL_DECODER* decoder, const char* name,
  ...
  if (id <= 0) *id_cache = id = resolve_name(decoder, name);
...
int OSSL_DECODER_CTX_set_params(OSSL_DECODER_CTX* ctx,
  ...
  for (i = 0; i < l; i++) {
    ...
    if (decoderctx == NULL || decoder->set_ctx_params == NULL) continue;
    if (!decoder->set_ctx_params(decoderctx, params)) ok = 0;
  }
  return ok;
}
...
void OSSL_DECODER_CTX_free(OSSL_DECODER_CTX* ctx) {
  if (ctx != NULL) {
    if (ctx->cleanup != NULL) ctx->cleanup(ctx->construct_data);
