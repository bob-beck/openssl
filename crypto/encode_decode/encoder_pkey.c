...
int
OSSL_ENCODER_CTX_set_cipher(OSSL_ENCODER_CTX* ctx,
                            ...
                            const char* propquery)
{
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
    OSSL_ENCODER_PARAM_CIPHER, (void*)cipher_name, 0);
...
int
OSSL_ENCODER_CTX_set_passphrase(OSSL_ENCODER_CTX* ctx,
...
int
OSSL_ENCODER_CTX_set_passphrase_ui(OSSL_ENCODER_CTX* ctx,
...
int
OSSL_ENCODER_CTX_set_pem_password_cb(OSSL_ENCODER_CTX* ctx,
                                     pem_password_cb* cb,
                                     void* cbarg)
{
...
int
OSSL_ENCODER_CTX_set_passphrase_cb(OSSL_ENCODER_CTX* ctx,
...
static void
collect_encoder(OSSL_ENCODER* encoder, void* arg)
{
...
static int
encoder_import_cb(const OSSL_PARAM params[], void* arg)
{
  ...
  if (data->obj == NULL) {
    ...
    if (k_prov != e_prov) {
      ...
      if (!evp_keymgmt_export(
            pk->keymgmt, pk->keydata, selection, &encoder_import_cb, data))
        return NULL;
...
static void
encoder_destruct_pkey(void* arg)
{
  struct construct_data_st* data = arg;
 ...
 */
static int
ossl_encoder_ctx_setup_for_pkey(OSSL_ENCODER_CTX* ctx,
                                const EVP_PKEY* pkey,
                                int selection,
                                const char* propquery)
{
...
OSSL_ENCODER_CTX*
OSSL_ENCODER_CTX_new_for_pkey(const EVP_PKEY* pkey,
                              int selection,
                              const char* output_type,
