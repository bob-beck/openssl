...
static OSSL_ENCODER* ossl_encoder_new(void) {
  ...
  if ((encoder = OPENSSL_zalloc(sizeof(*encoder))) == NULL) return NULL;
...
static void* encoder_from_algorithm(int id, const OSSL_ALGORITHM* algodef,
  ...
  if ((encoder = ossl_encoder_new()) == NULL) return NULL;
  encoder->base.id = id;
...
static void* construct_encoder(const OSSL_ALGORITHM* algodef,
  ...
  if (id != 0) method = encoder_from_algorithm(id, algodef, prov);
...
static OSSL_ENCODER* inner_ossl_encoder_fetch(struct encoder_data_st* methdata,
  ...
  if (id == 0 ||
    ...
    if ((method = ossl_method_construct(methdata->libctx, OSSL_OP_ENCODER,
       ...
       */
      if (id == 0) id = ossl_namemap_name2num(namemap, name);
...
const OSSL_PROPERTY_LIST* ossl_encoder_parsed_properties(
...
int OSSL_ENCODER_CTX_set_params(OSSL_ENCODER_CTX* ctx,
  ...
  if (ctx->encoder_insts == NULL) return 1;
  ...
  for (i = 0; i < l; i++) {
    ...
    if (encoderctx == NULL || encoder->set_ctx_params == NULL) continue;
    if (!encoder->set_ctx_params(encoderctx, params)) ok = 0;
  }
  return ok;
}
