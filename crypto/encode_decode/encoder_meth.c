...
static int put_encoder_in_store(void                *store,
                                void                *method,
...
static OSSL_ENCODER *
inner_ossl_encoder_fetch(struct encoder_data_st *methdata, const char *name, const char *properties)
{
    ...
    if (id == 0 || !ossl_method_store_cache_get(store, NULL, id, propq, &method)) {
        OSSL_METHOD_CONSTRUCT_METHOD mcm        = {get_tmp_encoder_store,
