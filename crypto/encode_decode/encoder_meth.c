...
static int
put_encoder_in_store(void* store,
                     void* method,
                     ...
                     void* data)
{
  ...
  return ossl_method_store_add(
    store, prov, id, propdef, method, ossl_encoder_up_ref, ossl_encoder_free);
}

/* Create and populate a encoder method */
static void*
encoder_from_algorithm(int id,
 ...
 */
static void*
construct_encoder(const OSSL_ALGORITHM* algodef,
                  OSSL_PROVIDER* prov,
                  void* data)
{
  /*
  ...
  if (id == 0 ||
    ...
    if ((method = ossl_method_construct(methdata->libctx,
      ...
      ossl_method_store_cache_set(
        store, prov, id, propq, method, up_ref_encoder, free_encoder);
    }
...
OSSL_ENCODER*
OSSL_ENCODER_fetch(OSSL_LIB_CTX* libctx,
                   const char* name,
                   const char* properties)
{
