...
static void
ossl_encoder_free (void *data)
{
  OSSL_ENCODER_free (data);
}

static int
ossl_encoder_up_ref (void *data)
{
...
static void *
get_tmp_encoder_store (void *data)
{
...
static void
dealloc_tmp_encoder_store (void *store)
{
  if (store != NULL)
...
get_encoder_store (OSSL_LIB_CTX *libctx)
{
  return ossl_lib_ctx_get_data (libctx, OSSL_LIB_CTX_ENCODER_STORE_INDEX);
}

static int
reserve_encoder_store (void *store, void *data)
{
  struct encoder_data_st *methdata = data;
...
static int
unreserve_encoder_store (void *store, void *data)
{
  struct encoder_data_st *methdata = data;
...
static void *
get_encoder_from_store (void *store, const OSSL_PROVIDER **prov, void *data)
{
  struct encoder_data_st *methdata = data;
  void *method = NULL;
  int id;
...
static int
put_encoder_in_store (void *store, void *method, const OSSL_PROVIDER *prov,
...
static void *
encoder_from_algorithm (int id, const OSSL_ALGORITHM *algodef,
                        OSSL_PROVIDER *prov)
{
  OSSL_ENCODER *encoder = NULL;
 ...
 */
static void *
construct_encoder (const OSSL_ALGORITHM *algodef, OSSL_PROVIDER *prov,
                   void *data)
{
  /*
...
static void
destruct_encoder (void *method, void *data)
{
  OSSL_ENCODER_free (method);
}

static int
up_ref_encoder (void *method)
{
  return OSSL_ENCODER_up_ref (method);
}

static void
free_encoder (void *method)
{
  OSSL_ENCODER_free (method);
}
...
static OSSL_ENCODER *
inner_ossl_encoder_fetch (struct encoder_data_st *methdata, const char *name,
...
OSSL_ENCODER *
OSSL_ENCODER_fetch (OSSL_LIB_CTX *libctx, const char *name,
                    const char *properties)
{
...
int
ossl_encoder_store_cache_flush (OSSL_LIB_CTX *libctx)
{
...
int
ossl_encoder_store_remove_all_provided (const OSSL_PROVIDER *prov)
{
  OSSL_LIB_CTX *libctx = ossl_provider_libctx (prov);
  OSSL_METHOD_STORE *store = get_encoder_store (libctx);
...
const OSSL_PROVIDER *
OSSL_ENCODER_get0_provider (const OSSL_ENCODER *encoder)
{
...
const char *
OSSL_ENCODER_get0_properties (const OSSL_ENCODER *encoder)
{
  if (!ossl_assert (encoder != NULL))
    {
      ERR_raise (ERR_LIB_OSSL_ENCODER, ERR_R_PASSED_NULL_PARAMETER);
...
int
ossl_encoder_get_number (const OSSL_ENCODER *encoder)
{
  if (!ossl_assert (encoder != NULL))
    {
      ERR_raise (ERR_LIB_OSSL_ENCODER, ERR_R_PASSED_NULL_PARAMETER);
...
const char *
OSSL_ENCODER_get0_name (const OSSL_ENCODER *encoder)
{
  return encoder->base.name;
}

const char *
OSSL_ENCODER_get0_description (const OSSL_ENCODER *encoder)
{
  return encoder->base.algodef->algorithm_description;
}

int
OSSL_ENCODER_is_a (const OSSL_ENCODER *encoder, const char *name)
{
...
void
OSSL_ENCODER_do_all_provided (OSSL_LIB_CTX *libctx,
...
int
OSSL_ENCODER_names_do_all (const OSSL_ENCODER *encoder,
...
int
OSSL_ENCODER_get_params (OSSL_ENCODER *encoder, OSSL_PARAM params[])
{
...
int
OSSL_ENCODER_CTX_set_params (OSSL_ENCODER_CTX *ctx, const OSSL_PARAM params[])
{
  int ok = 1;
  int i;
  int l;
...
void
OSSL_ENCODER_CTX_free (OSSL_ENCODER_CTX *ctx)
{
  if (ctx != NULL)
    {
