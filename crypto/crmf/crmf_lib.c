 ...
 * NAMING
 *
 * The 0 functions use the supplied structure pointer directly in the parent
 * and it will be freed up when the parent is freed.
 *
 * The 1 functions use a copy of the supplied structure pointer (or in some
 * cases increases its link count) in the parent and so both should be freed
 * up.
 */
...
int
OSSL_CRMF_MSG_set0_SinglePubInfo (OSSL_CRMF_SINGLEPUBINFO *spi, int method,
                                  GENERAL_NAME *nm)
{
...
int
OSSL_CRMF_MSG_set0_validity (OSSL_CRMF_MSG *crm, ASN1_TIME *notBefore,
                             ASN1_TIME *notAfter)
{
...
create_popo_signature (OSSL_CRMF_POPOSIGNINGKEY *ps,
                       const OSSL_CRMF_CERTREQUEST *cr, EVP_PKEY *pkey,
...
int
OSSL_CRMF_MSG_create_popo (int meth, OSSL_CRMF_MSG *crm, EVP_PKEY *pkey,
                           const EVP_MD *digest, OSSL_LIB_CTX *libctx,
                           const char *propq)
{
 ...
 */
int
OSSL_CRMF_MSGS_verify_popo (const OSSL_CRMF_MSGS *reqs, int rid,
 ...
 */
int
OSSL_CRMF_CERTTEMPLATE_fill (OSSL_CRMF_CERTTEMPLATE *tmpl, EVP_PKEY *pubkey,
