...
#define IMPLEMENT_CRMF_CTRL_FUNC(atyp, valt, ctrlinf)                        \
  valt* OSSL_CRMF_MSG_get0_##ctrlinf##_##atyp(const OSSL_CRMF_MSG* msg) {    \
    ...
    OSSL_CRMF_ATTRIBUTETYPEANDVALUE* atav = NULL;                            \
                                                                             \
    if (msg == NULL || msg->certReq == NULL) return NULL;                    \
  ...
  int OSSL_CRMF_MSG_set1_##ctrlinf##_##atyp(OSSL_CRMF_MSG* msg,              \
    ...
    OSSL_CRMF_ATTRIBUTETYPEANDVALUE* atav = NULL;                            \
                                                                             \
    if (msg == NULL || in == NULL) goto err;                                 \
    if ((atav = OSSL_CRMF_ATTRIBUTETYPEANDVALUE_new()) == NULL) goto err;    \
    if ((atav->type = OBJ_nid2obj(NID_id_##ctrlinf##_##atyp)) == NULL)       \
      goto err;                                                              \
    if ((atav->value.atyp = valt##_dup(in)) == NULL) goto err;               \
    if (!OSSL_CRMF_MSG_push0_##ctrlinf(msg, atav)) goto err;                 \
    return 1;                                                                \
  err:                                                                       \
...
static int OSSL_CRMF_MSG_push0_regCtrl(OSSL_CRMF_MSG* crm,
  ...
  if (crm->certReq->controls == NULL) {
    ...
    if (crm->certReq->controls == NULL) goto err;
    new = 1;
  }
...
int OSSL_CRMF_MSG_set0_SinglePubInfo(OSSL_CRMF_SINGLEPUBINFO* spi, int method,
  ...
  if (!ASN1_INTEGER_set(spi->pubMethod, method)) return 0;
...
OSSL_CRMF_CERTID* OSSL_CRMF_CERTID_gen(const X509_NAME* issuer,
  ...
  if ((cid = OSSL_CRMF_CERTID_new()) == NULL) goto err;

  if (!X509_NAME_set(&cid->issuer->d.directoryName, issuer)) goto err;
  cid->issuer->type = GEN_DIRNAME;

  ASN1_INTEGER_free(cid->serialNumber);
  if ((cid->serialNumber = ASN1_INTEGER_dup(serial)) == NULL) goto err;
...
static int OSSL_CRMF_MSG_push0_regInfo(OSSL_CRMF_MSG* crm,
  ...
  if (crm->regInfo == NULL) goto err;
  if (!sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_push(crm->regInfo, ri)) goto err;
  return 1;

err:
  if (info != NULL) crm->regInfo = NULL;
...
int OSSL_CRMF_MSG_push0_extension(OSSL_CRMF_MSG* crm, X509_EXTENSION* ext) {
  ...
  if (tmpl->extensions == NULL) {
    if ((tmpl->extensions = sk_X509_EXTENSION_new_null()) == NULL) goto err;
    new = 1;
  }

  if (!sk_X509_EXTENSION_push(tmpl->extensions, ext)) goto err;
  return 1;
err:
  if (new != 0) {
...
static int create_popo_signature(OSSL_CRMF_POPOSIGNINGKEY* ps,
  ...
  if (!ossl_x509_check_private_key(pub, pkey)) return 0;
...
int OSSL_CRMF_MSG_create_popo(int meth, OSSL_CRMF_MSG* crm, EVP_PKEY* pkey,
  ...
  if (meth == OSSL_CRMF_POPO_NONE) goto end;
  if ((pp = OSSL_CRMF_POPO_new()) == NULL) goto err;
  pp->type = meth;

  switch (meth) {
    case OSSL_CRMF_POPO_RAVERIFIED:
      if ((pp->value.raVerified = ASN1_NULL_new()) == NULL) goto err;
      break;
  ...
  switch (req->popo->type) {
    ...
    case OSSL_CRMF_POPO_SIGNATURE:
      ...
      if (ASN1_item_verify_ex(it, sig->algorithmIdentifier, sig->signature, asn,
                              NULL, X509_PUBKEY_get0(pubkey), libctx,
                              propq) < 1)
        return 0;
      break;
    case OSSL_CRMF_POPO_KEYENC:
...
int OSSL_CRMF_CERTTEMPLATE_fill(OSSL_CRMF_CERTTEMPLATE* tmpl, EVP_PKEY* pubkey,
  ...
  if (serial != NULL) {
    ...
    if ((tmpl->serialNumber = ASN1_INTEGER_dup(serial)) == NULL) return 0;
  }
  if (pubkey != NULL && !X509_PUBKEY_set(&tmpl->publicKey, pubkey)) return 0;
  return 1;
}
...
static int check_cmKGA(ossl_unused const X509_PURPOSE* purpose, const X509* x,
  ...
  for (i = 0; i < sk_ASN1_OBJECT_num(ekus); i++) {
    if (OBJ_obj2nid(sk_ASN1_OBJECT_value(ekus, i)) == NID_cmKGA) goto end;
  }
  ret = 0;
...
EVP_PKEY* OSSL_CRMF_ENCRYPTEDKEY_get1_pkey(
  ...
  if ((vpm = X509_STORE_get0_param(ts)) == NULL) goto end;
...
unsigned char* OSSL_CRMF_ENCRYPTEDVALUE_decrypt(
    const OSSL_CRMF_ENCRYPTEDVALUE* enc, OSSL_LIB_CTX* libctx,
  ...
  *outlen = 0;
  ...
  if ((iv = OPENSSL_malloc(EVP_CIPHER_get_iv_length(cipher))) == NULL) goto end;
...
X509* OSSL_CRMF_ENCRYPTEDKEY_get1_encCert(const OSSL_CRMF_ENCRYPTEDKEY* ecert,
  ...
  if (cert == NULL) ERR_raise(ERR_LIB_CRMF, CRMF_R_ERROR_DECODING_CERTIFICATE);
  BIO_free(bio);
  return cert;
#else
