 ...
 */
static int
OSSL_CRMF_MSG_push0_regCtrl(OSSL_CRMF_MSG* crm,
...
int
OSSL_CRMF_MSG_set0_SinglePubInfo(OSSL_CRMF_SINGLEPUBINFO* spi,
                                 int method,
                                 GENERAL_NAME* nm)
{
...
int
OSSL_CRMF_MSG_set_PKIPublicationInfo_action(OSSL_CRMF_PKIPUBLICATIONINFO* pi,
                                            int action)
{
...
IMPLEMENT_CRMF_CTRL_FUNC(pkiPublicationInfo,
...
OSSL_CRMF_CERTID*
OSSL_CRMF_CERTID_gen(const X509_NAME* issuer, const ASN1_INTEGER* serial)
{
 ...
 */
static int
OSSL_CRMF_MSG_push0_regInfo(OSSL_CRMF_MSG* crm,
...
int
OSSL_CRMF_MSG_set0_validity(OSSL_CRMF_MSG* crm,
                            ASN1_TIME* notBefore,
                            ASN1_TIME* notAfter)
{
...
int
OSSL_CRMF_MSG_set_certReqId(OSSL_CRMF_MSG* crm, int rid)
{
...
static int
crmf_asn1_get_int(const ASN1_INTEGER* a)
{
  int64_t res;
...
int
OSSL_CRMF_MSG_set0_extensions(OSSL_CRMF_MSG* crm, X509_EXTENSIONS* exts)
{
  OSSL_CRMF_CERTTEMPLATE* tmpl = OSSL_CRMF_MSG_get0_tmpl(crm);
...
int
OSSL_CRMF_MSG_push0_extension(OSSL_CRMF_MSG* crm, X509_EXTENSION* ext)
{
  int new = 0;
  OSSL_CRMF_CERTTEMPLATE* tmpl = OSSL_CRMF_MSG_get0_tmpl(crm);
...
static int
create_popo_signature(OSSL_CRMF_POPOSIGNINGKEY* ps,
...
int
OSSL_CRMF_MSG_create_popo(int meth,
                          OSSL_CRMF_MSG* crm,
                          EVP_PKEY* pkey,
                          const EVP_MD* digest,
                          OSSL_LIB_CTX* libctx,
                          const char* propq)
{
  ...
  switch (meth) {
    ...
    case OSSL_CRMF_POPO_SIGNATURE: {
      ...
      if (ps == NULL)
        goto err;
      if (!create_popo_signature(
            ps, crm->certReq, pkey, digest, libctx, propq)) {
...
int
OSSL_CRMF_MSGS_verify_popo(const OSSL_CRMF_MSGS* reqs,
                           int rid,
...
int
OSSL_CRMF_MSG_centralkeygen_requested(const OSSL_CRMF_MSG* crm,
 ...
 */
int
OSSL_CRMF_CERTTEMPLATE_fill(OSSL_CRMF_CERTTEMPLATE* tmpl,
                            EVP_PKEY* pubkey,
...
static int
check_cmKGA(ossl_unused const X509_PURPOSE* purpose, const X509* x, int ca)
{
...
EVP_PKEY*
OSSL_CRMF_ENCRYPTEDKEY_get1_pkey(const OSSL_CRMF_ENCRYPTEDKEY* encryptedKey,
                                 X509_STORE* ts,
                                 STACK_OF(X509)* extra,
                                 EVP_PKEY* pkey,
                                 X509* cert,
                                 ...
                                 const char* propq)
{
  ...
  if (encryptedKey->type != OSSL_CRMF_ENCRYPTEDKEY_ENVELOPEDDATA) {
    ...
    p = OSSL_CRMF_ENCRYPTEDVALUE_decrypt(
      encryptedKey->value.encryptedValue, libctx, propq, pkey, &len);
    if ((p_copy = p) != NULL)
  ...
  if ((purpose_id = X509_PURPOSE_get_by_sname(SN_cmKGA)) < 0) {
    ...
    if (!X509_PURPOSE_add(purpose_id,
                          X509_TRUST_COMPAT,
                          0,
                          check_cmKGA,
                          LN_cmKGA,
  ...
  pkey_bio = CMS_SignedData_verify(
    sd, NULL, NULL /* scerts */, ts, extra, NULL, 0, libctx, propq);
                                 ...
                                 int* outlen)
{
  ...
  *outlen = 0;
  ...
  if ((iv = OPENSSL_malloc(EVP_CIPHER_get_iv_length(cipher))) == NULL)
    goto end;
  if (ASN1_TYPE_get_octetstring(
        enc->symmAlg->parameter, iv, EVP_CIPHER_get_iv_length(cipher)) !=
  ...
  if (!EVP_DecryptInit(evp_ctx, cipher, ek, iv) ||
      !EVP_DecryptUpdate(
        evp_ctx, out, outlen, enc->encValue->data, enc->encValue->length) ||
...
#ifndef OPENSSL_NO_CMS
  bio = CMS_EnvelopedData_decrypt(ecert->value.envelopedData,
                                  NULL,
                                  pkey,
                                  NULL /* cert */,
                                  NULL,
