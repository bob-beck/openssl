...
OSSL_CRMF_PBMPARAMETER* OSSL_CRMF_pbmp_new(OSSL_LIB_CTX* libctx, size_t slen,
  ...
  if ((pbm = OSSL_CRMF_PBMPARAMETER_new()) == NULL) goto err;
   ...
   * bits) long.
   */
  if ((salt = OPENSSL_malloc(slen)) == NULL) goto err;
  if (RAND_bytes_ex(libctx, salt, slen, 0) <= 0) {
    ERR_raise(ERR_LIB_CRMF, CRMF_R_FAILURE_OBTAINING_RANDOM);
    goto err;
  }
  if (!ASN1_OCTET_STRING_set(pbm->salt, salt, (int)slen)) goto err;
...
int OSSL_CRMF_pbm_new(OSSL_LIB_CTX* libctx, const char* propq,
  ...
  if ((mac_res = OPENSSL_malloc(EVP_MAX_MD_SIZE)) == NULL) goto err;
  ...
  if ((ctx = EVP_MD_CTX_new()) == NULL) goto err;

  /* compute the basekey of the salted secret */
  if (!EVP_DigestInit_ex(ctx, owf, NULL)) goto err;
  /* first the secret */
  if (!EVP_DigestUpdate(ctx, sec, seclen)) goto err;
  /* then the salt */
  if (!EVP_DigestUpdate(ctx, pbmp->salt->data, pbmp->salt->length)) goto err;
  if (!EVP_DigestFinal_ex(ctx, basekey, &bklen)) goto err;
  ...
  while (--iterations > 0) {
    if (!EVP_DigestInit_ex(ctx, owf, NULL)) goto err;
    if (!EVP_DigestUpdate(ctx, basekey, bklen)) goto err;
    if (!EVP_DigestFinal_ex(ctx, basekey, &bklen)) goto err;
  }
