...
static ossl_inline int digit_decoded(const unsigned char a) {
  if (a >= 0x41 && a <= 0x5A) return a - 0x41;

  if (a >= 0x61 && a <= 0x7A) return a - 0x61;

  if (a >= 0x30 && a <= 0x39) return a - 0x30 + 26;
...
int ossl_punycode_decode(const char* pEncoded, const size_t enc_len,
  ...
  if (enc_len >= UINT_MAX) return 0;
  for (loop = 0; loop < (unsigned int)enc_len; loop++) {
    if (pEncoded[loop] == delimiter) basic_count = loop;
  }

  if (basic_count > 0) {
    if (basic_count > max_out) return 0;

    for (loop = 0; loop < basic_count; loop++) {
      if (is_basic(pEncoded[loop]) == 0) return 0;
  ...
  for (loop = processed_in; loop < (unsigned int)enc_len;) {
    ...
    if (i / (written_out + 1) > maxint - n) return 0;
    n = n + i / (written_out + 1);
    i %= (written_out + 1);

    if (written_out >= max_out) return 0;
...
int ossl_a2ulabel(const char* in, char* out, size_t outlen) {
  ...
  if (!ossl_assert(out != NULL)) return -1;

  if (!WPACKET_init_static_len(&pkt, (unsigned char*)out, outlen, 0)) return -1;
  ...
  while (1) {
    ...
    if (!HAS_PREFIX(inptr, "xn--")) {
      if (!WPACKET_memcpy(&pkt, inptr, delta)) result = 0;
    } else {
      ...
      for (i = 0; i < bufsize; i++) {
        ...
        if (!WPACKET_memcpy(&pkt, seed, utfsize)) result = 0;
      }
    }

    if (tmpptr == NULL) break;

    if (!WPACKET_put_bytes_u8(&pkt, '.')) result = 0;

    inptr = tmpptr + 1;
  }

  if (!WPACKET_put_bytes_u8(&pkt, '\0')) result = 0;
end:
  WPACKET_cleanup(&pkt);
  return result;
}
