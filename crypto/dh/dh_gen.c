...
static int dh_builtin_genparams(DH* ret, int prime_len, int generator,
  ...
  if (ret->params.p == NULL && ((ret->params.p = BN_new()) == NULL)) goto err;
  if (ret->params.g == NULL && ((ret->params.g = BN_new()) == NULL)) goto err;
  ...
  if (generator == DH_GENERATOR_2) {
    ...
    if (!BN_set_word(t2, 23)) goto err;
    g = 2;
  } else if (generator == DH_GENERATOR_5) {
    if (!BN_set_word(t1, 60)) goto err;
    if (!BN_set_word(t2, 59)) goto err;
    g = 5;
  } else {
    /*
  ...
  if (!BN_set_word(ret->params.g, g)) goto err;
