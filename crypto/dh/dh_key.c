...
ossl_dh_compute_key(unsigned char* key, const BIGNUM* pub_key, DH* dh)
{
  ...
  if (!dh->meth->bn_mod_exp(
        dh, z, pub_key, dh->priv_key, dh->params.p, ctx, mont)) {
...
int
DH_compute_key_padded(unsigned char* key, const BIGNUM* pub_key, DH* dh)
{
  int rv, pad;
                            ...
                            BIGNUM* pub_key)
{
  ...
  if (!dh->meth->bn_mod_exp(
        dh, pub_key, dh->params.g, prk, dh->params.p, ctx, mont))
    goto err;
  ret = 1;
err:
  BN_clear_free(prk);
  ...
  if (generate_new_key) {
    ...
    if (DH_get_nid(dh) != NID_undef) {
      ...
      if (!ossl_ffc_generate_private_key(
            ctx, &dh->params, dh->length, max_strength, priv_key))
        goto err;
    } else {
#ifdef FIPS_MODULE
      if (dh->params.q == NULL)
      ...
      if (dh->params.q == NULL) {
        ...
        if (dh->length != 0 && dh->length < l)
          l = dh->length;
        if (!BN_priv_rand_ex(
              priv_key, l, BN_RAND_TOP_ONE, BN_RAND_BOTTOM_ANY, 0, ctx))
          goto err;
        /*
...
#endif
      {
        ...
        if (!ossl_ffc_params_simple_validate(
              dh->libctx, &dh->params, FFC_PARAM_TYPE_DH, NULL))
          goto err;
        /*
         ...
         */
        if (!ossl_ffc_generate_private_key(ctx,
                                           &dh->params,
...
int
ossl_dh_buf2key(DH* dh, const unsigned char* buf, size_t len)
{
...
size_t
ossl_dh_key2buf(const DH* dh, unsigned char** pbuf_out, size_t size, int alloc)
{
  const BIGNUM* pubkey;
