...
static int generate_key(DH* dh) {
...
#ifdef FIPS_MODULE
      if (dh->params.q == NULL) goto err;
#else
      if (dh->params.q == NULL) {
        /* secret exponent length, must satisfy 2^l < (p-1)/2 */
        l = BN_num_bits(dh->params.p);
        if (dh->length >= l) goto err;
        l -= 2;
        if (dh->length != 0 && dh->length < l) l = dh->length;
        ...
        if (BN_is_word(dh->params.g, DH_GENERATOR_2) &&
          ...
          if (!BN_clear_bit(priv_key, 0)) goto err;
        }
      } else
#endif
      {
  ...
  if (!ossl_dh_generate_public_key(ctx, dh, priv_key, pub_key)) goto err;
...
err:
  ...
  if (pub_key != dh->pub_key) BN_free(pub_key);
  if (priv_key != dh->priv_key) BN_free(priv_key);
  BN_CTX_free(ctx);
  return ok;
}
...
int ossl_dh_buf2key(DH* dh, const unsigned char* buf, size_t len) {
  ...
  if (DH_set0_key(dh, pubkey, NULL) != 1) goto err;
  return 1;
err:
...
size_t ossl_dh_key2buf(const DH* dh, unsigned char** pbuf_out, size_t size,
  ...
  if (pbuf_out != NULL && (alloc || *pbuf_out != NULL)) {
    if (!alloc) {
      if (size >= (size_t)p_size) pbuf = *pbuf_out;
      if (pbuf == NULL) ERR_raise(ERR_LIB_DH, DH_R_INVALID_SIZE);
    } else {
