...
static int dh_ffc_params_fromdata(DH* dh, const OSSL_PARAM params[]) {
  ...
  if (ret) ossl_dh_cache_named_group(dh); /* This increments dh->dirty_cnt */
  return ret;
}
...
int ossl_dh_params_fromdata(DH* dh, const OSSL_PARAM params[]) {
  ...
  if (!dh_ffc_params_fromdata(dh, params)) return 0;
...
int ossl_dh_key_fromdata(DH* dh, const OSSL_PARAM params[],
  ...
  if (!DH_set0_key(dh, pub_key, priv_key)) goto err;
...
int ossl_dh_params_todata(DH* dh, OSSL_PARAM_BLD* bld, OSSL_PARAM params[]) {
  ...
  if (!ossl_ffc_params_todata(ossl_dh_get0_params(dh), bld, params)) return 0;
  if (l > 0 &&
...
#ifndef FIPS_MODULE
  if (dh->engine != NULL || ossl_dh_get_method(dh) != DH_OpenSSL()) return 1;
#endif
  return 0;
}
...
DH* ossl_dh_dup(const DH* dh, int selection) {
  ...
  if (ossl_dh_is_foreign(dh)) return NULL;

  if ((dupkey = ossl_dh_new_ex(dh->libctx)) == NULL) return NULL;
...
DH* ossl_dh_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO* p8inf,
  ...
  if (!DH_set0_key(dh, NULL, privkey_bn)) goto dherr;
  /* Calculate public key, increments dirty_cnt */
  if (!DH_generate_key(dh)) goto dherr;
