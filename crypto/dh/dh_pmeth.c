...
static int pkey_dh_init(EVP_PKEY_CTX* ctx) {
  ...
  if ((dctx = OPENSSL_zalloc(sizeof(*dctx))) == NULL) return 0;
  dctx->prime_len = 2048;
  dctx->subprime_len = -1;
  dctx->generator = 2;
...
static int pkey_dh_copy(EVP_PKEY_CTX* dst, const EVP_PKEY_CTX* src) {
  ...
  if (dctx->kdf_oid == NULL) return 0;
  dctx->kdf_md = sctx->kdf_md;
  if (sctx->kdf_ukm != NULL) {
    dctx->kdf_ukm = OPENSSL_memdup(sctx->kdf_ukm, sctx->kdf_ukmlen);
    if (dctx->kdf_ukm == NULL) return 0;
...
static int pkey_dh_ctrl(EVP_PKEY_CTX* ctx, int type, int p1, void* p2) {
  ...
  switch (type) {
    ...
    case EVP_PKEY_CTRL_DH_PARAMGEN_SUBPRIME_LEN:
      if (dctx->paramgen_type == DH_PARAMGEN_TYPE_GENERATOR) return -2;
    ...
    case EVP_PKEY_CTRL_DH_PARAMGEN_TYPE:
      ...
      if (p1 != DH_PARAMGEN_TYPE_GENERATOR) return -2;
#else
      if (p1 < 0 || p1 > 2) return -2;
#endif
      dctx->paramgen_type = p1;
      return 1;

    case EVP_PKEY_CTRL_DH_RFC5114:
      if (p1 < 1 || p1 > 3 || dctx->param_nid != NID_undef) return -2;
      dctx->param_nid = p1;
      return 1;

    case EVP_PKEY_CTRL_DH_NID:
      if (p1 <= 0 || dctx->param_nid != NID_undef) return -2;
      dctx->param_nid = p1;
      return 1;
    ...
    case EVP_PKEY_CTRL_DH_KDF_TYPE:
      if (p1 == -2) return dctx->kdf_type;
      if (p1 != EVP_PKEY_DH_KDF_NONE && p1 != EVP_PKEY_DH_KDF_X9_42) return -2;
      dctx->kdf_type = p1;
      return 1;
...
static DH* ffc_params_generate(OSSL_LIB_CTX* libctx, DH_PKEY_CTX* dctx,
  ...
  if (dctx->paramgen_type > DH_PARAMGEN_TYPE_FIPS_186_4) return NULL;
  ret = DH_new();
...
static int pkey_dh_paramgen(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey) {
  ...
  if (dctx->param_nid != NID_undef) {
    ...
    if ((dh = DH_new_by_nid(dctx->param_nid)) == NULL) return 0;
...
static int pkey_dh_derive(EVP_PKEY_CTX* ctx, unsigned char* key,
  ...
  if (dctx->kdf_type == EVP_PKEY_DH_KDF_NONE) {
    ...
    *keylen = ret;
    ...
    if (!dctx->kdf_outlen || !dctx->kdf_oid) return 0;
    if (key == NULL) {
      *keylen = dctx->kdf_outlen;
      return 1;
    }
    if (*keylen != dctx->kdf_outlen) return 0;
    ret = 0;
    if ((Zlen = DH_size(dh)) <= 0) return 0;
    if ((Z = OPENSSL_malloc(Zlen)) == NULL) return 0;
