...
static int i2d_dhp(const EVP_PKEY* pkey, const DH* a, unsigned char** pp) {
  if (pkey->ameth == &ossl_dhx_asn1_meth) return i2d_DHxparams(a, pp);
...
static int dh_param_decode(EVP_PKEY* pkey, const unsigned char** pder,
  ...
  if ((dh = d2i_dhp(pkey, pder, derlen)) == NULL) return 0;
  dh->dirty_cnt++;
  EVP_PKEY_assign(pkey, pkey->ameth->pkey_id, dh);
  return 1;
}
...
static int do_dh_print(BIO* bp, const DH* x, int indent, int ptype) {
  ...
  if (!ASN1_bn_print(bp, "private-key:", priv_key, NULL, indent)) goto err;
  if (!ASN1_bn_print(bp, "public-key:", pub_key, NULL, indent)) goto err;

  if (!ossl_ffc_params_print(bp, &x->params, indent)) goto err;
...
static int int_dh_param_copy(DH* to, const DH* from, int is_x942) {
  if (is_x942 == -1) is_x942 = (from->params.q != NULL);
  if (!ossl_ffc_params_copy(&to->params, &from->params)) return 0;
  if (!is_x942) to->length = from->length;
  to->dirty_cnt++;
  return 1;
}
...
static int dh_pub_cmp(const EVP_PKEY* a, const EVP_PKEY* b) {
  if (dh_cmp_parameters(a, b) == 0) return 0;
...
static int dh_pkey_export_to(const EVP_PKEY* from, void* to_keydata,
  ...
  if (!OSSL_PARAM_BLD_push_BN(tmpl, OSSL_PKEY_PARAM_FFC_P, p) ||
    ...
    goto err;
  if (q != NULL) {
    if (!OSSL_PARAM_BLD_push_BN(tmpl, OSSL_PKEY_PARAM_FFC_Q, q)) goto err;
  }
