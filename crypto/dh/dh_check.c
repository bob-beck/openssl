...
int DH_check_params_ex(const DH* dh) {
  ...
  if (!DH_check_params(dh, &errflags)) return 0;
...
int DH_check_params(const DH* dh, int* ret) {
  ...
  *ret = 0;
  ...
  if (!BN_is_odd(dh->params.p)) *ret |= DH_CHECK_P_NOT_PRIME;
  if (BN_is_negative(dh->params.g) || BN_is_zero(dh->params.g) ||
      BN_is_one(dh->params.g))
    *ret |= DH_NOT_SUITABLE_GENERATOR;
  if (BN_copy(tmp, dh->params.p) == NULL || !BN_sub_word(tmp, 1)) goto err;
  if (BN_cmp(dh->params.g, tmp) >= 0) *ret |= DH_NOT_SUITABLE_GENERATOR;
...
int DH_check(const DH* dh, int* ret) {
  ...
  *ret = 0;
  ...
  if (!DH_check_params(dh, ret)) return 0;
    ...
    if (BN_cmp(dh->params.g, BN_value_one()) <= 0)
      ...
      *ret |= DH_NOT_SUITABLE_GENERATOR;
      ...
      if (!BN_mod_exp(t1, dh->params.g, dh->params.q, dh->params.p, ctx))
        goto err;
      if (!BN_is_one(t1)) *ret |= DH_NOT_SUITABLE_GENERATOR;
    }
    r = BN_check_prime(dh->params.q, ctx, NULL);
    if (r < 0) goto err;
    if (!r) *ret |= DH_CHECK_Q_NOT_PRIME;
    /* Check p == 1 mod q  i.e. q divides p - 1 */
    if (!BN_div(t1, t2, dh->params.p, dh->params.q, ctx)) goto err;
    if (!BN_is_one(t2)) *ret |= DH_CHECK_INVALID_Q_VALUE;
  ...
  if (!r)
    *ret |= DH_CHECK_P_NOT_PRIME;
    ...
    if (!BN_rshift1(t1, dh->params.p)) goto err;
    r = BN_check_prime(t1, ctx, NULL);
    if (r < 0) goto err;
    if (!r) *ret |= DH_CHECK_P_NOT_SAFE_PRIME;
  }
  ok = 1;
err:
  BN_CTX_end(ctx);
  ...
  *ret = 0;
  ...
  if (dh->params.q != NULL) {
    ...
    if (length == 0) {
      ...
      if (BN_num_bits(priv_key) <= length && BN_num_bits(priv_key) > 1) ok = 1;
  ...
  if (DH_get_nid((DH*)dh) != NID_undef && dh->length != 0) {
    if (!BN_lshift(two_powN, BN_value_one(), dh->length)) goto end;
    if (BN_cmp(two_powN, dh->params.q) < 0) upper = two_powN;
  }
  if (!ossl_ffc_validate_private_key(upper, priv_key, ret)) goto end;
...
int ossl_dh_check_pairwise(const DH* dh, int return_on_null_numbers) {
  ...
  if (!ossl_dh_generate_public_key(ctx, dh, dh->priv_key, pub_key)) goto err;
...
#ifdef FIPS_MODULE
  {
    ...
    if (BN_num_bytes(pub_key) > (int)sizeof(bytes)) goto err;
    len = BN_bn2bin(pub_key, bytes);
    OSSL_SELF_TEST_oncorrupt_byte(st, bytes);
    if (BN_bin2bn(bytes, len, pub_key) == NULL) goto err;
  }
#endif
