...
ossl_param_buf_alloc(OSSL_PARAM_BUF* out, size_t extra_blocks, int is_secure)
{
  ...
  if (ossl_unlikely(
        !ossl_size_add(
          extra_blocks, out->blocks, &num_blocks, OPENSSL_FILE, OPENSSL_LINE) ||
        !ossl_size_mul(
          num_blocks, OSSL_PARAM_ALIGN_SIZE, &sz, OPENSSL_FILE, OPENSSL_LINE)))
    return 0;
...
OSSL_PARAM_dup(const OSSL_PARAM* src)
{
  ...
  ossl_param_set_secure_block(last,
                              buf[OSSL_PARAM_BUF_SECURE].alloc,
