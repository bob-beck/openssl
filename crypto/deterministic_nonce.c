...
static int bits2int(BIGNUM* out, int qlen_bits, const unsigned char* in,
  ...
  if (BN_bin2bn(in, (int)inlen, out) == NULL) return 0;

  shift = blen_bits - qlen_bits;
  if (shift > 0) return BN_rshift(out, out, shift);
  return 1;
}
...
static int bits2int_consttime(BIGNUM* out, int qlen_bits,
  ...
  shift = blen_bits - qlen_bits;
  if (shift > 0) return bn_rshift_fixed_top(out, out, shift);
  return 1;
}
...
static int bits2octets(unsigned char* out, const BIGNUM* q, int qlen_bits,
  ...
  if (z == NULL || !bits2int(z, qlen_bits, in, inlen)) goto err;

  /* z2 = z1 mod q (Do a simple subtract, since z1 < 2^qlen_bits) */
  if (BN_cmp(z, q) >= 0 && !BN_usub(z, z, q)) goto err;
...
static EVP_KDF_CTX* kdf_setup(const char* digestname,
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (EVP_KDF_CTX_set_params(ctx, params) <= 0) goto err;
