...
err:
  if (erv != 1) OPENSSL_cleanse(pt, *ptlen);
...
err:
  if (erv != 1) OPENSSL_cleanse(ct, *ctlen);
  EVP_CIPHER_CTX_free(ctx);
  return erv;
}
...
static int hpke_suite_check(OSSL_HPKE_SUITE suite,
  ...
  if ((kem_info_ = ossl_HPKE_KEM_INFO_find_id(suite.kem_id)) == NULL) return 0;
  if ((kdf_info_ = ossl_HPKE_KDF_INFO_find_id(suite.kdf_id)) == NULL) return 0;
  if ((aead_info_ = ossl_HPKE_AEAD_INFO_find_id(suite.aead_id)) == NULL)
    return 0;

  if (kem_info != NULL) *kem_info = kem_info_;
  if (kdf_info != NULL) *kdf_info = kdf_info_;
  if (aead_info != NULL) *aead_info = aead_info_;
...
static size_t hpke_seqnonce2buf(OSSL_HPKE_CTX* ctx, unsigned char* buf,
  ...
  if (ctx == NULL || blen < sizeof(seq_copy) || blen != ctx->noncelen) return 0;
  seq_copy = ctx->seq;
  memset(buf, 0, blen);
...
static int hpke_encap(OSSL_HPKE_CTX* ctx, unsigned char* enc, size_t* enclen,
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (ctx->shared_secret == NULL) goto err;
  ...
  *p = OSSL_PARAM_construct_end();
  if (ctx->mode == OSSL_HPKE_MODE_AUTH || ctx->mode == OSSL_HPKE_MODE_PSKAUTH) {
    ...
    if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
      ...
      spub = EVP_PKEY_new_raw_public_key_ex(ctx->libctx, kem_info->keytype,
...
static int hpke_do_middle(OSSL_HPKE_CTX* ctx, const unsigned char* info,
  ...
  if (ctx->exportersec == NULL) goto err;
...
int OSSL_HPKE_export(OSSL_HPKE_CTX* ctx, unsigned char* secret,
  ...
  EVP_KDF_CTX_free(kctx);
  if (erv != 1) ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
  return erv;
}
...
size_t OSSL_HPKE_get_ciphertext_size(OSSL_HPKE_SUITE suite, size_t clearlen) {
  ...
  if (hpke_expansion(suite, &enclen, clearlen, &cipherlen) != 1) return 0;
  return cipherlen;
}
...
size_t OSSL_HPKE_get_recommended_ikmelen(OSSL_HPKE_SUITE suite) {
  ...
  if (hpke_suite_check(suite, &kem_info, NULL, NULL) != 1) return 0;
  if (kem_info == NULL) return 0;
