 ...
 */
static int
hpke_kem_id_nist_curve (uint16_t kem_id)
{
  const OSSL_HPKE_KEM_INFO *kem_info;
 ...
 */
static EVP_PKEY *
evp_pkey_new_raw_nist_public_key (OSSL_LIB_CTX *libctx, const char *propq,
                                  const char *gname, const unsigned char *buf,
                                  size_t buflen)
{
  OSSL_PARAM params[2];
  EVP_PKEY *ret = NULL;
 ...
 */
static int
hpke_aead_dec (OSSL_HPKE_CTX *hctx, const unsigned char *iv,
               const unsigned char *aad, size_t aadlen,
               const unsigned char *ct, size_t ctlen, unsigned char *pt,
               size_t *ptlen)
{
  int erv = 0;
  ...
  if (EVP_CIPHER_CTX_ctrl (ctx, EVP_CTRL_AEAD_SET_IVLEN, (int)hctx->noncelen,
                           NULL)
      != 1)
    {
      ERR_raise (ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
 ...
 */
static int
hpke_aead_enc (OSSL_HPKE_CTX *hctx, const unsigned char *iv,
               const unsigned char *aad, size_t aadlen,
               const unsigned char *pt, size_t ptlen, unsigned char *ct,
               size_t *ctlen)
{
  int erv = 0;
  EVP_CIPHER_CTX *ctx = NULL;
 ...
 */
static int
hpke_mode_check (unsigned int mode)
{
  switch (mode)
    {
 ...
 */
static int
hpke_suite_check (OSSL_HPKE_SUITE suite, const OSSL_HPKE_KEM_INFO **kem_info,
                  const OSSL_HPKE_KDF_INFO **kdf_info,
                  const OSSL_HPKE_AEAD_INFO **aead_info)
{
 ...
 */
static int
hpke_random_suite (OSSL_LIB_CTX *libctx, const char *propq,
 ...
 */
static int
hpke_expansion (OSSL_HPKE_SUITE suite, size_t *enclen, size_t clearlen,
                size_t *cipherlen)
{
  const OSSL_HPKE_AEAD_INFO *aead_info = NULL;
  const OSSL_HPKE_KEM_INFO *kem_info = NULL;
 ...
 */
static size_t
hpke_seqnonce2buf (OSSL_HPKE_CTX *ctx, unsigned char *buf, size_t blen)
{
  size_t i;
  uint64_t seq_copy;
 ...
 */
static int
hpke_encap (OSSL_HPKE_CTX *ctx, unsigned char *enc, size_t *enclen,
 ...
 */
static int
hpke_decap (OSSL_HPKE_CTX *ctx, const unsigned char *enc, size_t enclen,
            EVP_PKEY *priv)
{
  int erv = 0;
  EVP_PKEY_CTX *pctx = NULL;
 ...
 */
static int
hpke_do_middle (OSSL_HPKE_CTX *ctx, const unsigned char *info, size_t infolen)
{
  int erv = 0;
...
OSSL_HPKE_CTX *
OSSL_HPKE_CTX_new (int mode, OSSL_HPKE_SUITE suite, int role,
...
int
OSSL_HPKE_CTX_set1_psk (OSSL_HPKE_CTX *ctx, const char *pskid,
...
int
OSSL_HPKE_CTX_set1_ikme (OSSL_HPKE_CTX *ctx, const unsigned char *ikme,
                         size_t ikmelen)
{
...
int
OSSL_HPKE_CTX_set1_authpriv (OSSL_HPKE_CTX *ctx, EVP_PKEY *priv)
{
...
int
OSSL_HPKE_CTX_set1_authpub (OSSL_HPKE_CTX *ctx, const unsigned char *pub,
                            size_t publen)
{
  int erv = 0;
  EVP_PKEY *pubp = NULL;
...
int
OSSL_HPKE_CTX_get_seq (OSSL_HPKE_CTX *ctx, uint64_t *seq)
{
...
int
OSSL_HPKE_CTX_set_seq (OSSL_HPKE_CTX *ctx, uint64_t seq)
{
  if (ctx == NULL)
    {
      ERR_raise (ERR_LIB_CRYPTO, ERR_R_PASSED_NULL_PARAMETER);
...
int
OSSL_HPKE_encap (OSSL_HPKE_CTX *ctx, unsigned char *enc, size_t *enclen,
                 const unsigned char *pub, size_t publen,
...
int
OSSL_HPKE_decap (OSSL_HPKE_CTX *ctx, const unsigned char *enc, size_t enclen,
...
int
OSSL_HPKE_seal (OSSL_HPKE_CTX *ctx, unsigned char *ct, size_t *ctlen,
                const unsigned char *aad, size_t aadlen,
...
int
OSSL_HPKE_open (OSSL_HPKE_CTX *ctx, unsigned char *pt, size_t *ptlen,
                const unsigned char *aad, size_t aadlen,
...
int
OSSL_HPKE_export (OSSL_HPKE_CTX *ctx, unsigned char *secret, size_t secretlen,
...
int
OSSL_HPKE_keygen (OSSL_HPKE_SUITE suite, unsigned char *pub, size_t *publen,
...
int
OSSL_HPKE_get_grease_value (const OSSL_HPKE_SUITE *suite_in,
...
int
OSSL_HPKE_str2suite (const char *str, OSSL_HPKE_SUITE *suite)
{
  return ossl_hpke_str2suite (str, suite);
}

size_t
OSSL_HPKE_get_ciphertext_size (OSSL_HPKE_SUITE suite, size_t clearlen)
{
  size_t enclen = 0;
  size_t cipherlen = 0;
...
size_t
OSSL_HPKE_get_recommended_ikmelen (OSSL_HPKE_SUITE suite)
{
  const OSSL_HPKE_KEM_INFO *kem_info = NULL;
