 ...
 */
static EVP_PKEY*
evp_pkey_new_raw_nist_public_key(OSSL_LIB_CTX* libctx,
                                 const char* propq,
                                 const char* gname,
                                 ...
                                 size_t buflen)
{
  ...
  params[0] = OSSL_PARAM_construct_utf8_string(
    OSSL_PKEY_PARAM_GROUP_NAME, (char*)gname, 0);
 ...
 */
static int
hpke_aead_dec(OSSL_HPKE_CTX* hctx,
              ...
              size_t* ptlen)
{
  ...
  if (EVP_CIPHER_CTX_ctrl(
        ctx, EVP_CTRL_AEAD_SET_IVLEN, (int)hctx->noncelen, NULL) != 1) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
    goto err;
  }
 ...
 */
static int
hpke_aead_enc(OSSL_HPKE_CTX* hctx,
              const unsigned char* iv,
              const unsigned char* aad,
              size_t aadlen,
 ...
 */
static int
hpke_suite_check(OSSL_HPKE_SUITE suite,
                 const OSSL_HPKE_KEM_INFO** kem_info,
                 const OSSL_HPKE_KDF_INFO** kdf_info,
                 const OSSL_HPKE_AEAD_INFO** aead_info)
{
 ...
 */
static int
hpke_random_suite(OSSL_LIB_CTX* libctx,
                  const char* propq,
 ...
 */
static int
hpke_expansion(OSSL_HPKE_SUITE suite,
               size_t* enclen,
               size_t clearlen,
               size_t* cipherlen)
{
           ...
           size_t publen)
{
  ...
  if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
    ...
    pkR = EVP_PKEY_new_raw_public_key_ex(
      ctx->libctx, kem_info->keytype, ctx->propq, pub, publen);
  }
  if (pkR == NULL) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
    goto err;
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (EVP_PKEY_encapsulate(
        pctx, enc, enclen, ctx->shared_secret, &ctx->shared_secretlen) != 1) {
           ...
           EVP_PKEY* priv)
{
  ...
  *p = OSSL_PARAM_construct_end();
  if (ctx->mode == OSSL_HPKE_MODE_AUTH || ctx->mode == OSSL_HPKE_MODE_PSKAUTH) {
    ...
    if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
      spub = evp_pkey_new_raw_nist_public_key(ctx->libctx,
                                              ctx->propq,
                                              kem_info->groupname,
                                              ctx->authpub,
                                              ctx->authpublen);
...
hpke_do_middle(OSSL_HPKE_CTX* ctx, const unsigned char* info, size_t infolen)
{
  ...
  if (ossl_hpke_labeled_extract(kctx,
                                ks_context + 1,
                                halflen,
                                NULL,
                                0,
...
int
OSSL_HPKE_CTX_set1_psk(OSSL_HPKE_CTX* ctx,
                       const char* pskid,
...
int
OSSL_HPKE_CTX_set1_ikme(OSSL_HPKE_CTX* ctx,
...
int
OSSL_HPKE_CTX_set1_authpub(OSSL_HPKE_CTX* ctx,
                           const unsigned char* pub,
                           size_t publen)
{
  int erv = 0;
  ...
  if (hpke_kem_id_nist_curve(ctx->suite.kem_id) == 1) {
    ...
    pubp = EVP_PKEY_new_raw_public_key_ex(
      ctx->libctx, kem_info->keytype, ctx->propq, pub, publen);
  }
  if (pubp == NULL) {
...
int
OSSL_HPKE_encap(OSSL_HPKE_CTX* ctx,
                unsigned char* enc,
                size_t* enclen,
                const unsigned char* pub,
                size_t publen,
...
int
OSSL_HPKE_decap(OSSL_HPKE_CTX* ctx,
                const unsigned char* enc,
                size_t enclen,
                EVP_PKEY* recippriv,
                const unsigned char* info,
...
int
OSSL_HPKE_seal(OSSL_HPKE_CTX* ctx,
               unsigned char* ct,
               size_t* ctlen,
               const unsigned char* aad,
               size_t aadlen,
...
int
OSSL_HPKE_open(OSSL_HPKE_CTX* ctx,
               unsigned char* pt,
               size_t* ptlen,
               const unsigned char* aad,
               size_t aadlen,
...
int
OSSL_HPKE_export(OSSL_HPKE_CTX* ctx,
...
int
OSSL_HPKE_keygen(OSSL_HPKE_SUITE suite,
                 unsigned char* pub,
                 size_t* publen,
                 EVP_PKEY** priv,
                 ...
                 const char* propq)
{
  ...
  if (hpke_kem_id_nist_curve(suite.kem_id) == 1) {
    *p++ = OSSL_PARAM_construct_utf8_string(
      OSSL_PKEY_PARAM_GROUP_NAME, (char*)kem_info->groupname, 0);
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (EVP_PKEY_get_octet_string_param(
        skR, OSSL_PKEY_PARAM_ENCODED_PUBLIC_KEY, pub, *publen, publen) != 1) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
    goto err;
  }
  *priv = skR;
                           ...
                           const char* propq)
{
  ...
  *suite = chosen;
   ...
   */
  if (OSSL_HPKE_keygen(
        chosen, enc, enclen, &fakepriv, NULL, 0, libctx, propq) != 1) {
    ERR_raise(ERR_LIB_CRYPTO, ERR_R_INTERNAL_ERROR);
    goto err;
  }
