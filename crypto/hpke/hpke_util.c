...
const OSSL_HPKE_KEM_INFO *
ossl_HPKE_KEM_INFO_find_random (OSSL_LIB_CTX *ctx)
{
  uint32_t rval = 0;
  int err = 0;
...
const OSSL_HPKE_KDF_INFO *
ossl_HPKE_KDF_INFO_find_random (OSSL_LIB_CTX *ctx)
{
  uint32_t rval = 0;
  int err = 0;
...
const OSSL_HPKE_AEAD_INFO *
ossl_HPKE_AEAD_INFO_find_random (OSSL_LIB_CTX *ctx)
{
  uint32_t rval = 0;
  int err = 0;
...
static int
kdf_derive (EVP_KDF_CTX *kctx, unsigned char *out, size_t outlen, int mode,
            const unsigned char *salt, size_t saltlen,
            const unsigned char *ikm, size_t ikmlen, const unsigned char *info,
            size_t infolen)
{
  int ret;
  ...
  *p++ = OSSL_PARAM_construct_int (OSSL_KDF_PARAM_MODE, &mode);
  if (salt != NULL)
    *p++ = OSSL_PARAM_construct_octet_string (OSSL_KDF_PARAM_SALT,
                                              (char *)salt, saltlen);
  if (ikm != NULL)
    *p++ = OSSL_PARAM_construct_octet_string (OSSL_KDF_PARAM_KEY, (char *)ikm,
                                              ikmlen);
  if (info != NULL)
...
int
ossl_hpke_kdf_extract (EVP_KDF_CTX *kctx, unsigned char *prk, size_t prklen,
                       const unsigned char *salt, size_t saltlen,
...
int
ossl_hpke_kdf_expand (EVP_KDF_CTX *kctx, unsigned char *okm, size_t okmlen,
 ...
 */
int
ossl_hpke_labeled_extract (EVP_KDF_CTX *kctx, unsigned char *prk,
 ...
 */
int
ossl_hpke_labeled_expand (EVP_KDF_CTX *kctx, unsigned char *okm, size_t okmlen,
                          const unsigned char *prk, size_t prklen,
...
EVP_KDF_CTX *
ossl_kdf_ctx_create (const char *kdfname, const char *mdname,
 ...
 */
static uint16_t
synonyms_name2id (const char *st, const synonymttab_t *synp, size_t arrsize)
{
  size_t i, j;
 ...
 */
int
ossl_hpke_str2suite (const char *suitestr, OSSL_HPKE_SUITE *suite)
{
