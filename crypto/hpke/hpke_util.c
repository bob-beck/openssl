...
const OSSL_HPKE_KEM_INFO* ossl_HPKE_KEM_INFO_find_curve(const char* curve) {
  ...
  for (i = 0; i < sz; ++i) {
    ...
    if (group == NULL) group = hpke_kem_tab[i].keytype;
    if (OPENSSL_strcasecmp(curve, group) == 0) return &hpke_kem_tab[i];
  }
...
const OSSL_HPKE_KEM_INFO* ossl_HPKE_KEM_INFO_find_id(uint16_t kemid) {
  ...
  for (i = 0; i != sz; ++i) {
    if (hpke_kem_tab[i].kem_id == kemid) return &hpke_kem_tab[i];
  }
  ERR_raise(ERR_LIB_PROV, PROV_R_INVALID_CURVE);
  return NULL;
}
...
const OSSL_HPKE_KDF_INFO* ossl_HPKE_KDF_INFO_find_id(uint16_t kdfid) {
  ...
  for (i = 0; i != sz; ++i) {
    if (hpke_kdf_tab[i].kdf_id == kdfid) return &hpke_kdf_tab[i];
  }
...
const OSSL_HPKE_AEAD_INFO* ossl_HPKE_AEAD_INFO_find_id(uint16_t aeadid) {
  ...
  for (i = 0; i != sz; ++i) {
    if (hpke_aead_tab[i].aead_id == aeadid) return &hpke_aead_tab[i];
  }
...
static int kdf_derive(EVP_KDF_CTX* kctx, unsigned char* out, size_t outlen,
  ...
  *p = OSSL_PARAM_construct_end();
  ...
  if (!ret) ERR_raise(ERR_LIB_PROV, PROV_R_FAILED_DURING_DERIVATION);
  return ret;
}
...
static uint16_t synonyms_name2id(const char* st, const synonymttab_t* synp,
  ...
  for (i = 0; i < arrsize; ++i) {
    for (j = 0; j < OSSL_NELEM(synp[i].synonyms); ++j) {
      if (OPENSSL_strcasecmp(st, synp[i].synonyms[j]) == 0) return synp[i].id;
    }
  }
  return 0;
}
