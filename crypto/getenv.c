...
char* ossl_safe_getenv(const char* name) {
  ...
  if (GetEnvironmentVariableW(L"OPENSSL_WIN32_UTF8", NULL, 0) != 0) {
    ...
    if (rsize > 0) namew = _malloca(rsize * sizeof(WCHAR));
    ...
    if (NULL != namew) {
      ...
      if (fsize > 0) envlen = GetEnvironmentVariableW(namew, NULL, 0);
    }

    if (envlen > 0) valw = _malloca(envlen * sizeof(WCHAR));
    ...
    if (vallen > 0) val = OPENSSL_malloc(vallen);

    if (NULL != val) {
      /* convert value string from wide to utf-8 */
      if (WideCharToMultiByte(CP_UTF8, 0, valw, -1, val, vallen, NULL, NULL) ==
          0) {
        OPENSSL_free(val);
        val = NULL;
      }
    ...
    if (NULL != namew) _freea(namew);

    if (NULL != valw) _freea(valw);
...
#define SECURE_GETENV
  ...
  if (OPENSSL_issetugid()) return NULL;
  return getenv(name);
#endif
}
