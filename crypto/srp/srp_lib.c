...
static BIGNUM* srp_Calc_xy(const BIGNUM* x, const BIGNUM* y, const BIGNUM* N,
  ...
  if (x != N && BN_ucmp(x, N) >= 0) goto err;
  if (y != N && BN_ucmp(y, N) >= 0) goto err;
  if ((tmp = OPENSSL_malloc_array(numN, 2)) == NULL) goto err;
...
BIGNUM* SRP_Calc_server_key(const BIGNUM* A, const BIGNUM* v, const BIGNUM* u,
  ...
  if ((bn_ctx = BN_CTX_new()) == NULL || (tmp = BN_new()) == NULL) goto err;

  /* S = (A*v**u) ** b */

  if (!BN_mod_exp(tmp, v, u, N, bn_ctx)) goto err;
  if (!BN_mod_mul(tmp, A, tmp, N, bn_ctx)) goto err;
...
BIGNUM* SRP_Calc_x_ex(const BIGNUM* s, const char* user, const char* pass,
  ...
  if ((s == NULL) || (user == NULL) || (pass == NULL)) return NULL;

  ctxt = EVP_MD_CTX_new();
  if (ctxt == NULL) return NULL;
  if ((cs = OPENSSL_malloc(BN_num_bytes(s))) == NULL) goto err;

  sha1 = EVP_MD_fetch(libctx, "SHA1", propq);
  if (sha1 == NULL) goto err;
  ...
  if (!EVP_DigestInit_ex(ctxt, sha1, NULL) ||
    ...
    goto err;
  if (BN_bn2bin(s, cs) < 0) goto err;
  if (!EVP_DigestUpdate(ctxt, cs, BN_num_bytes(s))) goto err;
...
BIGNUM* SRP_Calc_client_key_ex(const BIGNUM* N, const BIGNUM* B,
  ...
  if (!BN_mod_exp(tmp, g, xtmp, N, bn_ctx)) goto err;
  if ((k = srp_Calc_k(N, g, libctx, propq)) == NULL) goto err;
  if (!BN_mod_mul(tmp2, tmp, k, N, bn_ctx)) goto err;
  if (!BN_mod_sub(tmp, B, tmp2, N, bn_ctx)) goto err;
  if (!BN_mul(tmp3, u, xtmp, bn_ctx)) goto err;
  if (!BN_add(tmp2, a, tmp3)) goto err;
  K = BN_new();
...
int SRP_Verify_B_mod_N(const BIGNUM* B, const BIGNUM* N) {
  ...
  if (B == NULL || N == NULL || (bn_ctx = BN_CTX_new()) == NULL) return 0;

  if ((r = BN_new()) == NULL) goto err;
  /* Checks if B % N == 0 */
  if (!BN_nnmod(r, B, N, bn_ctx)) goto err;
  ret = !BN_is_zero(r);
err:
  BN_CTX_free(bn_ctx);
  BN_free(r);
...
SRP_gN* SRP_get_default_gN(const char* id) {
  ...
  if (id == NULL) return knowngN;
  for (i = 0; i < KNOWN_GN_NUMBER; i++) {
    if (strcmp(knowngN[i].id, id) == 0) return knowngN + i;
  }
  return NULL;
}
#endif
