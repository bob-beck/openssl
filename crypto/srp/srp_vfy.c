 ...
 */
static int
t_fromb64 (unsigned char *a, size_t alen, const char *src)
{
  EVP_ENCODE_CTX *ctx;
  int outl = 0, outl2 = 0;
  size_t size, padsize;
  ...
  if (EVP_DecodeUpdate (ctx, a, &outl2, (const unsigned char *)src, (int)size)
      < 0)
    {
      outl = -1;
      goto err;
 ...
 */
static int
t_tob64 (char *dst, const unsigned char *src, int size)
{
...
void
SRP_user_pwd_set_gN (SRP_user_pwd *vinfo, const BIGNUM *g, const BIGNUM *N)
{
  vinfo->N = N;
  vinfo->g = g;
}
...
int
SRP_user_pwd_set1_ids (SRP_user_pwd *vinfo, const char *id, const char *info)
{
...
static int
SRP_user_pwd_set_sv (SRP_user_pwd *vinfo, const char *s, const char *v)
{
...
int
SRP_user_pwd_set0_sv (SRP_user_pwd *vinfo, BIGNUM *s, BIGNUM *v)
{
  BN_free (vinfo->s);
...
static SRP_user_pwd *
srp_user_pwd_dup (SRP_user_pwd *src)
{
  SRP_user_pwd *ret;
...
static void
SRP_gN_free (SRP_gN_cache *gN_cache)
{
  if (gN_cache == NULL)
    return;
...
static SRP_gN *
SRP_get_gN_by_id (const char *id, STACK_OF (SRP_gN) *gN_tab)
{
  int i;
...
static BIGNUM *
SRP_gN_place_bn (STACK_OF (SRP_gN_cache) *gN_cache, char *ch)
{
  int i;
  if (gN_cache == NULL)
    return NULL;
...
static SRP_user_pwd *
find_user (SRP_VBASE *vb, char *username)
{
  int i;
  SRP_user_pwd *user;
...
int
SRP_VBASE_add0_user (SRP_VBASE *vb, SRP_user_pwd *user_pwd)
{
 ...
 */
char *
SRP_create_verifier_ex (const char *user, const char *pass, char **salt,
...
char *
SRP_create_verifier (const char *user, const char *pass, char **salt,
                     char **verifier, const char *N, const char *g)
{
 ...
 */
int
SRP_create_verifier_BN_ex (const char *user, const char *pass, BIGNUM **salt,
...
int
SRP_create_verifier_BN (const char *user, const char *pass, BIGNUM **salt,
                        BIGNUM **verifier, const BIGNUM *N, const BIGNUM *g)
{
