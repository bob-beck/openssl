...
static int t_fromb64(unsigned char* a, size_t alen, const char* src) {
  ...
  if (size > INT_MAX || ((size + padsize) / 4) * 3 > alen) return -1;
...
int SRP_user_pwd_set1_ids(SRP_user_pwd* vinfo, const char* id,
  ...
  vinfo->id = NULL;
  vinfo->info = NULL;
  if (id != NULL && NULL == (vinfo->id = OPENSSL_strdup(id))) return 0;
...
static SRP_gN_cache* SRP_gN_new_init(const char* ch) {
  ...
  if ((newgN->b64_bn = OPENSSL_strdup(ch)) == NULL) goto err;
...
int SRP_VBASE_init(SRP_VBASE* vb, char* verifier_file) {
  ...
  for (i = 0; i < sk_OPENSSL_PSTRING_num(tmpdb->data); i++) {
    ...
    if (pp[DB_srptype][0] == DB_SRP_INDEX) {
      ...
      if ((gN = OPENSSL_malloc(sizeof(*gN))) == NULL) goto err;
...
int SRP_VBASE_add0_user(SRP_VBASE* vb, SRP_user_pwd* user_pwd) {
  if (sk_SRP_user_pwd_push(vb->users_pwd, user_pwd) <= 0) return 0;
  return 1;
}
...
SRP_user_pwd* SRP_VBASE_get1_by_user(SRP_VBASE* vb, char* username) {
  ...
  if (!SRP_user_pwd_set1_ids(user, username, NULL)) goto err;

  if (RAND_priv_bytes(digv, SHA_DIGEST_LENGTH) <= 0) goto err;
...
char* SRP_create_verifier_ex(const char* user, const char* pass, char** salt,
  ...
  if (N) {
    if ((len = t_fromb64(tmp, sizeof(tmp), N)) <= 0) goto err;
    N_bn_alloc = BN_bin2bn(tmp, len, NULL);
    if (N_bn_alloc == NULL) goto err;
    N_bn = N_bn_alloc;
    if ((len = t_fromb64(tmp, sizeof(tmp), g)) <= 0) goto err;
    g_bn_alloc = BN_bin2bn(tmp, len, NULL);
    if (g_bn_alloc == NULL) goto err;
    g_bn = g_bn_alloc;
    defgNid = "*";
  } else {
  ...
  if (*salt == NULL) {
    if (RAND_bytes_ex(libctx, tmp2, SRP_RANDOM_SALT_LEN, 0) <= 0) goto err;

    s = BN_bin2bn(tmp2, SRP_RANDOM_SALT_LEN, NULL);
  } else {
    if ((len = t_fromb64(tmp2, sizeof(tmp2), *salt)) <= 0) goto err;
  ...
  if (BN_bn2bin(v, tmp) < 0) goto err;
  vfsize = BN_num_bytes(v) * 2;
  if (((vf = OPENSSL_malloc(vfsize)) == NULL)) goto err;
  if (!t_tob64(vf, tmp, BN_num_bytes(v))) goto err;
...
err:
  if (salt != NULL && *salt != salttmp) BN_clear_free(salttmp);
  BN_clear_free(x);
  BN_CTX_free(bn_ctx);
  return result;
}
