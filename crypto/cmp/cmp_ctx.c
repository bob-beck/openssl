...
#define DEFINE_OSSL_set0_NAME(PREFIX, NAME, FIELD, TYPE)                       \
   ...
   * authentication. Any already existing store entry is freed. Given NULL, the
   * entry is reset.
   */
...
#define DEFINE_OSSL_get(PREFIX, FIELD, TYPE, ERR_RET)                          \
  ...
  DEFINE_OSSL_get(OSSL_CMP_CTX, certConf_cb_arg, void*, NULL)
    ...
    static size_t ossl_cmp_log_trace_cb(const char* buf,
                                        size_t cnt,
                                        int category,
                                        int cmd,
                                        void* vdata)
  ...
  if (OSSL_TRACE_ENABLED(CMP)) {
    OSSL_TRACE_BEGIN(CMP)
    {
      ...
      if (printed > 0 && (size_t)printed < sizeof(hugebuf)) {
        if (BIO_vsnprintf(
              hugebuf + printed, sizeof(hugebuf) - printed, format, args) > 0)
...
OSSL_CMP_CTX_set_log_cb(OSSL_CMP_CTX* ctx, OSSL_CMP_log_cb_t cb)
{
  ...
  if (!OSSL_trace_set_callback(
        OSSL_TRACE_CATEGORY_CMP, ossl_cmp_log_trace_cb, ctx))
    return 0;
#endif
 ...
 */
int
OSSL_CMP_CTX_set1_referenceValue(OSSL_CMP_CTX* ctx,
...
int
OSSL_CMP_CTX_set1_secretValue(OSSL_CMP_CTX* ctx,
                                    ...
                                    STACK_OF(X509)* candidates)
{
  ...
  if (!ossl_x509_add_certs_new(&ctx->untrusted,
                               candidates,
  ...
  chain = X509_build_chain(
    ctx->cert, ctx->untrusted, own_trusted, 0, ctx->libctx, ctx->propq);
  if (chain == NULL) {
