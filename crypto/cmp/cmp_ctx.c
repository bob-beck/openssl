...
#define DEFINE_OSSL_set0_NAME(PREFIX, NAME, FIELD, TYPE)                      \
  ...
  {                                                                           \
    ...
    /*
     * Set certificate store containing trusted (root) CA certs and possibly
     * CRLs and a cert verification callback function used for CMP server
     * authentication. Any already existing store entry is freed. Given NULL,
     * the entry is reset.
     */
...
static int
cmp_ctx_set_md (OSSL_CMP_CTX *ctx, EVP_MD **pmd, int nid)
{
 ...
 * values. Returns new context on success, NULL on error
 */
OSSL_CMP_CTX *
OSSL_CMP_CTX_new (OSSL_LIB_CTX *libctx, const char *propq)
{
 ...
 */
int
OSSL_CMP_CTX_reinit (OSSL_CMP_CTX *ctx)
{
  if (ctx == NULL)
    {
      ERR_raise (ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
...
#define DEFINE_OSSL_get(PREFIX, FIELD, TYPE, ERR_RET)                         \
  ...
  {                                                                           \
    ...
    /* Set callback function for checking if the cert is ok or should be
       rejected */
    DEFINE_OSSL_set (OSSL_CMP_CTX, certConf_cb, OSSL_CMP_certConf_cb_t)

    /*
     * Set argument, respectively a pointer to a structure containing
     * arguments, optionally to be used by the certConf callback.
     */
    DEFINE_OSSL_set (OSSL_CMP_CTX, certConf_cb_arg, void *)

    /*
     * Get argument, respectively the pointer to a structure containing
     * arguments, optionally to be used by certConf callback. Returns callback
     * argument set previously (NULL if not set or on error)
     */
    DEFINE_OSSL_get (OSSL_CMP_CTX, certConf_cb_arg, void *, NULL)

#ifndef OPENSSL_NO_TRACE
        static size_t
    ossl_cmp_log_trace_cb (const char *buf, size_t cnt, int category, int cmd,
                           void *vdata)
{
...
int
ossl_cmp_print_log (OSSL_CMP_severity level, const OSSL_CMP_CTX *ctx,
...
int
OSSL_CMP_CTX_set_log_cb (OSSL_CMP_CTX *ctx, OSSL_CMP_log_cb_t cb)
{
  if (ctx == NULL)
    {
      ERR_raise (ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
 ...
 */
void
OSSL_CMP_CTX_print_errors (const OSSL_CMP_CTX *ctx)
{
 ...
 */
int
OSSL_CMP_CTX_set1_referenceValue (OSSL_CMP_CTX *ctx, const unsigned char *ref,
                                  int len)
{
  if (ctx == NULL)
    {
...
int
OSSL_CMP_CTX_set1_secretValue (OSSL_CMP_CTX *ctx, const unsigned char *sec,
                               int len)
{
...
#define DEFINE_OSSL_set1_certs(PREFIX, FIELD)                                 \
  ...
  {                                                                           \
    ...
    /* Returns the stack of extraCerts received in CertRepMessage, NULL on
       error */
...
int
OSSL_CMP_CTX_push0_geninfo_ITAV (OSSL_CMP_CTX *ctx, OSSL_CMP_ITAV *itav)
{
  if (ctx == NULL)
    {
      ERR_raise (ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
...
#define DEFINE_OSSL_set1_up_ref(PREFIX, FIELD, TYPE)                          \
  ...
  {                                                                           \
    ...
    /*
     * Pins the server certificate to be directly trusted (even if it is
     * expired) for verifying response messages. Cert pointer is not consumed.
     * It may be NULL to clear the entry.
     */
...
int
OSSL_CMP_CTX_reqExtensions_have_SAN (OSSL_CMP_CTX *ctx)
{
  if (ctx == NULL)
    {
      ERR_raise (ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
 ...
 */
int
OSSL_CMP_CTX_push1_subjectAltName (OSSL_CMP_CTX *ctx, const GENERAL_NAME *name)
{
...
DEFINE_OSSL_set1_up_ref (OSSL_CMP_CTX, oldCert, X509)
    ...
    /*
     * Get the (newly received in IP/KUP/CP) client certificate from the
     * context This only permits for one client cert to be received...
     */
...
EVP_PKEY *
OSSL_CMP_CTX_get0_newPkey (const OSSL_CMP_CTX *ctx, int priv)
{
  if (ctx == NULL)
    {
      ERR_raise (ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
...
DEFINE_set1_ASN1_OCTET_STRING (OSSL_CMP_CTX, transactionID)
    ...
    /* Set argument optionally to be used by the http connect/disconnect
       callback */
    DEFINE_OSSL_set (OSSL_CMP_CTX, http_cb_arg, void *)

    /*
     * Get argument optionally to be used by the http connect/disconnect
     * callback Returns callback argument set previously (NULL if not set or on
     * error)
     */
 ...
 */
int
OSSL_CMP_CTX_get_option (const OSSL_CMP_CTX *ctx, int opt)
{
  if (ctx == NULL)
    {
      ERR_raise (ERR_LIB_CMP, CMP_R_NULL_ARGUMENT);
