...
OSSL_CMP_CTX* OSSL_CMP_CTX_new(OSSL_LIB_CTX* libctx, const char* propq) {
  ...
  ctx->libctx = libctx;
  if (propq != NULL && (ctx->propq = OPENSSL_strdup(propq)) == NULL) goto err;
  ...
  ctx->pbm_slen = 16;
  if (!cmp_ctx_set_md(ctx, &ctx->pbm_owf, NID_sha256)) goto err;
  ctx->pbm_itercnt = 500;
  ctx->pbm_mac = NID_hmac_sha1;

  if (!cmp_ctx_set_md(ctx, &ctx->digest, NID_sha256)) goto err;
...
#define DEFINE_OSSL_get(PREFIX, FIELD, TYPE, ERR_RET)  \
  ...
  if (buf == NULL || cnt == 0 || cmd != OSSL_TRACE_CTRL_WRITE || ctx == NULL)
    return 0;
  if (ctx->log_cb == NULL) return 1; /* silently drop message */
...
int ossl_cmp_print_log(OSSL_CMP_severity level, const OSSL_CMP_CTX* ctx,
  ...
  if (ctx == NULL || ctx->log_cb == NULL) return 1; /* silently drop message */
  ...
  if (func == NULL) func = "(unset function name)";
  if (file == NULL) file = "(unset file name)";
  if (level_str == NULL) level_str = "(unset level string)";
...
EVP_PKEY* ossl_cmp_ctx_get0_newPubkey(const OSSL_CMP_CTX* ctx) {
  ...
  if (ctx->newPkey != NULL) return ctx->newPkey;
  if (ctx->p10CSR != NULL) return X509_REQ_get0_pubkey(ctx->p10CSR);
  if (ctx->oldCert != NULL) return X509_get0_pubkey(ctx->oldCert);
  if (ctx->cert != NULL) return X509_get0_pubkey(ctx->cert);
  return ctx->pkey;
}
...
DEFINE_set1_ASN1_OCTET_STRING(OSSL_CMP_CTX, transactionID)
  ...
  switch (opt) {
    ...
    case OSSL_CMP_OPT_OWF_ALGNID:
      if (!cmp_ctx_set_md(ctx, &ctx->pbm_owf, val)) return 0;
      break;
