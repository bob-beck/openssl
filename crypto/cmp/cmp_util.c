...
int
OSSL_CMP_log_open (void) /* is designed to be idempotent */
{
...
void
OSSL_CMP_log_close (void) /* is designed to be idempotent */
{
...
#define max_level_len                                                         \
  5 /* = max length of the below strings, e.g., "EMERG"                       \
     */
static OSSL_CMP_severity
...
const char *
ossl_cmp_log_parse_metadata (const char *buf, OSSL_CMP_severity *level,
 ...
 */
static const char *
improve_location_name (const char *func, const char *fallback)
{
  if (fallback == NULL)
...
int
OSSL_CMP_print_to_bio (BIO *bio, const char *component, const char *file,
...
void
OSSL_CMP_print_errors_cb (OSSL_CMP_log_cb_t log_fn)
{
  unsigned long err;
...
int
ossl_cmp_X509_STORE_add1_certs (X509_STORE *store, STACK_OF (X509) *certs,
