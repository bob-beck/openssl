...
static const char* improve_location_name(const char* func,
                                         const char* fallback) {
  if (fallback == NULL) return func == NULL ? UNKNOWN_FUNC : func;
...
void OSSL_CMP_print_errors_cb(OSSL_CMP_log_cb_t log_fn) {
  ...
  while ((err = ERR_get_error_all(&file, &line, &func, &data, &flags)) != 0) {
    ...
    if (ERR_SYSTEM_ERROR(err)) {
      if (openssl_strerror_r(reason, rsbuf, sizeof(rsbuf))) rs = rsbuf;
    } else {
...
int ossl_cmp_sk_ASN1_UTF8STRING_push_str(STACK_OF(ASN1_UTF8STRING)* sk,
  ...
  if (!ossl_assert(sk != NULL && text != NULL)) return 0;
  if ((utf8string = ASN1_UTF8STRING_new()) == NULL) return 0;
  if (!ASN1_STRING_set(utf8string, text, len)) goto err;
  if (!sk_ASN1_UTF8STRING_push(sk, utf8string)) goto err;
  return 1;
...
int ossl_cmp_asn1_octet_string_set1(ASN1_OCTET_STRING** tgt,
  ...
  if (src != NULL) {
    if ((new = ASN1_OCTET_STRING_dup(src)) == NULL) return 0;
  } else {
    new = NULL;
  }
