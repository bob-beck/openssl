...
OSSL_CMP_SRV_CTX* OSSL_CMP_SRV_CTX_new(OSSL_LIB_CTX* libctx,
  ...
  if ((ctx->ctx = OSSL_CMP_CTX_new(libctx, propq)) == NULL) goto err;
...
static OSSL_CMP_MSG* process_cert_request(OSSL_CMP_SRV_CTX* srv_ctx,
  ...
  if (central_keygen < 0) return NULL;
  if (central_keygen == 0 &&
  ...
  if (msg == NULL) ERR_raise(ERR_LIB_CMP, CMP_R_ERROR_CREATING_CERTREP);
...
static int unprotected_exception(const OSSL_CMP_CTX* ctx,
  ...
  if (!ossl_assert(ctx != NULL && req != NULL)) return -1;
...
OSSL_CMP_MSG* OSSL_CMP_SRV_process_request(OSSL_CMP_SRV_CTX* srv_ctx,
  ...
  if (!OSSL_CMP_CTX_set1_recipient(ctx, hdr->sender->d.directoryName)) goto err;
...
err:
  if (rsp == NULL) {
    ...
    if ((flags & ERR_TXT_STRING) == 0 || *data == '\0') data = NULL;
  ...
  switch (rsp_type) {
    ...
    case OSSL_CMP_PKIBODY_ERROR:
      if (rsp != NULL && ossl_cmp_is_error_with_waiting(rsp)) break;
      /* fall through */
