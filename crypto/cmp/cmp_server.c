...
int
OSSL_CMP_SRV_CTX_init(OSSL_CMP_SRV_CTX* srv_ctx,
                      void* custom_ctx,
                      OSSL_CMP_SRV_cert_request_cb_t process_cert_request,
                      OSSL_CMP_SRV_rr_cb_t process_rr,
                      OSSL_CMP_SRV_genm_cb_t process_genm,
...
int
OSSL_CMP_SRV_CTX_init_trans(OSSL_CMP_SRV_CTX* srv_ctx,
...
delayed_delivery(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* req)
{
  ...
  msg = ossl_cmp_error_new(
    srv_ctx->ctx, si, errorCode, details, srv_ctx->sendUnprotectedErrors);
...
process_cert_request(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* req)
{
  ...
  if (central_keygen == 0 &&
    ...
    si = srv_ctx->process_cert_request(
      srv_ctx, req, certReqId, crm, p10cr, &certOut, &chainOut, &caPubs);
    if (si == NULL)
      goto err;
...
process_rr(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* req)
{
  ...
  if ((msg = ossl_cmp_rp_new(
         srv_ctx->ctx, si, certId, srv_ctx->sendUnprotectedErrors)) == NULL)
...
process_pollReq(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* req)
{
  ...
  if (!srv_ctx->process_pollReq(
        srv_ctx, req, certReqId, &orig_req, &check_after))
    return NULL;
...
OSSL_CMP_SRV_process_request(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* req)
{
  ...
  ossl_cmp_log1(
    DEBUG, ctx, "received %s", ossl_cmp_bodytype_to_string(req_type));
  ...
  req_verified = ossl_cmp_msg_check_update(
    ctx, req, unprotected_exception, srv_ctx->acceptUnprotected);
...
err:
  if (rsp == NULL) {
    ...
    if ((si = OSSL_CMP_STATUSINFO_new(
           OSSL_CMP_PKISTATUS_rejection, fail_info, reason)) != NULL) {
      rsp = ossl_cmp_error_new(
        srv_ctx->ctx, si, err, data, srv_ctx->sendUnprotectedErrors);
      OSSL_CMP_PKISI_free(si);
    }
  }
  ...
  if (rsp != NULL)
    ossl_cmp_log1(
      DEBUG, ctx, "sending %s", ossl_cmp_bodytype_to_string(rsp_type));
  else
