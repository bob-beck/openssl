...
int
OSSL_CMP_SRV_CTX_set_send_unprotected_errors (OSSL_CMP_SRV_CTX *srv_ctx,
                                              int val)
{
  if (srv_ctx == NULL)
    {
...
int
OSSL_CMP_SRV_CTX_set_grant_implicit_confirm (OSSL_CMP_SRV_CTX *srv_ctx,
                                             int val)
{
  if (srv_ctx == NULL)
    {
...
process_cert_request (OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *req)
{
  ...
  if (central_keygen == 0
      && !ossl_cmp_verify_popo (srv_ctx->ctx, req, srv_ctx->acceptRAVerified))
    {
      ...
      si = OSSL_CMP_STATUSINFO_new (
          OSSL_CMP_PKISTATUS_rejection, 1 << OSSL_CMP_PKIFAILUREINFO_badPOP,
...
static OSSL_CMP_MSG *
process_non_polling_request (OSSL_CMP_SRV_CTX *srv_ctx,
 ...
 */
static int
unprotected_exception (const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req,
                       int invalid_protection, int accept_unprotected_requests)
{
...
/*
 * Server interface that may substitute OSSL_CMP_MSG_http_perform at the
 * client. The OSSL_CMP_SRV_CTX must be set as client_ctx->transfer_cb_arg.
