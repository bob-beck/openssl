...
int ossl_cmp_pkisi_get_status(const OSSL_CMP_PKISI* si) {
  ...
  if (!ossl_assert(si != NULL && si->status != NULL)) return -1;
...
OSSL_CMP_PKIFREETEXT* ossl_cmp_pkisi_get0_statusString(
    const OSSL_CMP_PKISI* si) {
  if (!ossl_assert(si != NULL)) return NULL;
...
int ossl_cmp_pkisi_get_pkifailureinfo(const OSSL_CMP_PKISI* si) {
  ...
  if (!ossl_assert(si != NULL)) return -1;
  if (si->failInfo != NULL)
    for (i = 0; i <= OSSL_CMP_PKIFAILUREINFO_MAX; i++)
      if (ASN1_BIT_STRING_get_bit(si->failInfo, i)) res |= 1 << i;
  return res;
}
...
int ossl_cmp_pkisi_check_pkifailureinfo(const OSSL_CMP_PKISI* si,
                                        int bit_index) {
  if (!ossl_assert(si != NULL && si->failInfo != NULL)) return -1;
 ...
 */
static char* snprint_PKIStatusInfo_parts(
    int status, int fail_info, const OSSL_CMP_PKIFREETEXT* status_strings,
...
#define ADVANCE_BUFFER                                                    \
  if (printed_chars < 0 || (size_t)printed_chars >= bufsize) return NULL; \
...
OSSL_CMP_PKISI* OSSL_CMP_STATUSINFO_new(int status, int fail_info,
  ...
  if (text != NULL) {
    if ((utf8_text = ASN1_UTF8STRING_new()) == NULL ||
      ...
      goto err;
    if ((si->statusString = sk_ASN1_UTF8STRING_new_null()) == NULL) goto err;
    if (!sk_ASN1_UTF8STRING_push(si->statusString, utf8_text)) goto err;
    /* Ownership is lost. */
    utf8_text = NULL;
  }
  ...
  for (failure = 0; failure <= OSSL_CMP_PKIFAILUREINFO_MAX; failure++) {
    if ((fail_info & (1 << failure)) != 0) {
      if (si->failInfo == NULL &&
        ...
        goto err;
      if (!ASN1_BIT_STRING_set_bit(si->failInfo, failure, 1)) goto err;
    }
  }
  return si;
