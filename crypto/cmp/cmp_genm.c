...
static const X509_VERIFY_PARAM*
get0_trustedStore_vpm(const OSSL_CMP_CTX* ctx)
{
...
static int
ossl_X509_check(OSSL_CMP_CTX* ctx,
                const char* source,
                X509* cert,
                int type_CA,
...
static int
ossl_X509_check_all(OSSL_CMP_CTX* ctx,
                    const char* source,
                    STACK_OF(X509)* certs,
                    int type_CA,
                    const X509_VERIFY_PARAM* vpm)
...
get_genm_itav(OSSL_CMP_CTX* ctx,
              ...
              const char* desc)
{
  ...
  ERR_raise_data(
    ERR_LIB_CMP, CMP_R_INVALID_GENP, "could not find any ITAV for %s", desc);
...
int
OSSL_CMP_get1_caCerts(OSSL_CMP_CTX* ctx, STACK_OF(X509)** out)
{
  ...
  *out = NULL;
  ...
  if (!ossl_X509_check_all(
        ctx, "genp", certs, 1 /* CA */, get0_trustedStore_vpm(ctx))) {
    ret = 0;
    goto end;
  }
  *out = sk_X509_new_reserve(NULL, sk_X509_num(certs));
  if (!X509_add_certs(
        *out, certs, X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP)) {
...
static int
selfsigned_verify_cb(int ok, X509_STORE_CTX* store_ctx)
{
...
static int
verify_ss_cert(OSSL_LIB_CTX* libctx,
               const char* propq,
               X509_STORE* ts,
...
static int
verify_ss_cert_trans(OSSL_CMP_CTX* ctx,
                     X509* trusted /* may be NULL */,
...
int
OSSL_CMP_get1_rootCaKeyUpdate(OSSL_CMP_CTX* ctx,
                              ...
                              X509** oldWithNew)
{
  ...
  *newWithNew = NULL;
  ...
  if (!OSSL_CMP_ITAV_get0_rootCaKeyUpdate(
        itav, newWithNew, &my_newWithOld, &my_oldWithNew))
    goto end;
  ...
  if ((oldWithOld_copy = X509_dup(oldWithOld)) == NULL && oldWithOld != NULL)
    goto end;
  if (!verify_ss_cert_trans(
        ctx, oldWithOld_copy, my_newWithOld, *newWithNew, "newWithNew")) {
    ERR_raise(ERR_LIB_CMP, CMP_R_INVALID_ROOTCAKEYUPDATE);
    goto end;
  }
  if (oldWithOld != NULL && my_oldWithNew != NULL &&
      !verify_ss_cert_trans(
        ctx, *newWithNew, my_oldWithNew, oldWithOld_copy, "oldWithOld")) {
    ERR_raise(ERR_LIB_CMP, CMP_R_INVALID_ROOTCAKEYUPDATE);
    goto end;
  }
...
int
OSSL_CMP_get1_crlUpdate(OSSL_CMP_CTX* ctx,
                        const X509* crlcert,
...
int
OSSL_CMP_get1_certReqTemplate(OSSL_CMP_CTX* ctx,
                              ...
                              OSSL_CMP_ATAVS** keySpec)
{
  ...
  *certTemplate = NULL;
  ...
  if ((itav = get_genm_itav(
         ctx, req, NID_id_it_certReqTemplate, "certReqTemplate")) == NULL)
    return 0;
