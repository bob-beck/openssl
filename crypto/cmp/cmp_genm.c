...
static OSSL_CMP_ITAV* get_genm_itav(OSSL_CMP_CTX* ctx,
  ...
  if (!OSSL_CMP_CTX_push0_genm_ITAV(ctx, req)) goto err;
  req = NULL;
  ...
  if ((n = sk_OSSL_CMP_ITAV_num(itavs)) <= 0) {
    ERR_raise_data(ERR_LIB_CMP, CMP_R_INVALID_GENP,
                   "response on genm requesting infoType %s does not include "
                   "suitable value",
                   desc);
  ...
  for (i = 0; i < n; i++) {
    ...
    if (OBJ_obj2nid(obj) == expected) {
      for (i++; i < n; i++) OSSL_CMP_ITAV_free(sk_OSSL_CMP_ITAV_shift(itavs));
      sk_OSSL_CMP_ITAV_free(itavs);
      return itav;
    }
...
int OSSL_CMP_get1_caCerts(OSSL_CMP_CTX* ctx, STACK_OF(X509)** out) {
  ...
  *out = NULL;
  ...
  if ((req = OSSL_CMP_ITAV_new_caCerts(NULL)) == NULL) return 0;
  if ((itav = get_genm_itav(ctx, req, NID_id_it_caCerts, "caCerts")) == NULL)
    return 0;
  if (!OSSL_CMP_ITAV_get0_caCerts(itav, &certs)) goto end;
  ret = 1;
...
static int selfsigned_verify_cb(int ok, X509_STORE_CTX* store_ctx) {
  if (ok == 0 && X509_STORE_CTX_get_error_depth(store_ctx) == 0 &&
    ...
    for (i = 0; i < sk_X509_num(untrusted); i++) {
      ...
      if (!X509_add_cert(chain, cert, X509_ADD_FLAG_UP_REF)) return 0;
    }
    ...
    for (i = 0; i < sk_X509_num(trust); i++) {
      ...
      if ((*check_issued)(store_ctx, cert, issuer)) {
        if (X509_add_cert(chain, cert, X509_ADD_FLAG_UP_REF)) ok = 1;
        break;
      }
    }
...
static int verify_ss_cert_trans(
    OSSL_CMP_CTX* ctx, X509* trusted /* may be NULL */,
    X509* trans /* the only untrusted cert, may be NULL */, X509* target,
    const char* desc) {
  ...
  if (trusted != NULL) {
    ...
    if ((ts = X509_STORE_new()) == NULL) return 0;
...
err:
  ...
  if (trusted != NULL) X509_STORE_free(ts);
  return res;
}
...
int OSSL_CMP_get1_rootCaKeyUpdate(OSSL_CMP_CTX* ctx, const X509* oldWithOld,
  ...
  *newWithNew = NULL;
  ...
  if ((req = OSSL_CMP_ITAV_new_rootCaCert(oldWithOld)) == NULL) return 0;
  itav = get_genm_itav(ctx, req, NID_id_it_rootCaKeyUpdate, "rootCaKeyUpdate");
  if (itav == NULL) return 0;
  ...
  if (!X509_up_ref(*newWithNew)) goto end;
  ...
  if (newWithOld != NULL) X509_free(*newWithOld);
free:
...
int OSSL_CMP_get1_crlUpdate(OSSL_CMP_CTX* ctx, const X509* crlcert,
  ...
  *crl = NULL;
  ...
  if ((req = OSSL_CMP_ITAV_new0_crlStatusList(list)) == NULL) goto end;
  status = NULL;
  list = NULL;

  if ((itav = get_genm_itav(ctx, req, NID_id_it_crls, "crl")) == NULL) goto end;

  if (!OSSL_CMP_ITAV_get0_crls(itav, &crls)) goto end;
