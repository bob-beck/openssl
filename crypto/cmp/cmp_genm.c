...
static void
cert_msg (const char *func, const char *file, int lineno,
          OSSL_CMP_severity level, OSSL_CMP_CTX *ctx, const char *source,
...
static int
ossl_X509_check (OSSL_CMP_CTX *ctx, const char *source, X509 *cert,
...
static int
ossl_X509_check_all (OSSL_CMP_CTX *ctx, const char *source,
...
get_genm_itav (OSSL_CMP_CTX *ctx, OSSL_CMP_ITAV *req, /* gets consumed */
               int expected, const char *desc)
{
  ...
  if (n > 1)
    ossl_cmp_log2 (WARN, ctx,
                   "response on genm contains %d ITAVs; will use the first "
                   "ITAV with infoType id-it-%s",
                   n, desc);
  for (i = 0; i < n; i++)
    {
      ...
      if (OBJ_obj2txt (name + offset, (int)(sizeof (name) - offset), obj, 0)
          < 0)
        strcat (name, "<unknown>");
...
selfsigned_verify_cb (int ok, X509_STORE_CTX *store_ctx)
{
  if (ok == 0 && X509_STORE_CTX_get_error_depth (store_ctx) == 0
             ...
             == X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT)
    {
      ...
      trust
          = X509_STORE_get1_all_certs (X509_STORE_CTX_get0_store (store_ctx));
...
static int
verify_ss_cert (OSSL_LIB_CTX *libctx, const char *propq, X509_STORE *ts,
                STACK_OF (X509) *untrusted, X509 *target)
{
...
int
OSSL_CMP_get1_rootCaKeyUpdate (OSSL_CMP_CTX *ctx, const X509 *oldWithOld,
...
int
OSSL_CMP_get1_crlUpdate (OSSL_CMP_CTX *ctx, const X509 *crlcert,
