 ...
 */
int ossl_cmp_msg_set0_libctx(OSSL_CMP_MSG* msg,
                             OSSL_LIB_CTX* libctx,
                             const char* propq) {
  if (msg != NULL) {
    msg->libctx = libctx;
...
static int add1_extension(X509_EXTENSIONS** pexts,
                          int nid,
                          int crit,
                          void* ex) {
  X509_EXTENSION* ext;
...
OSSL_CMP_MSG* ossl_cmp_certreq_new(OSSL_CMP_CTX* ctx,
                                   int type,
                                   const OSSL_CRMF_MSG* crm) {
  OSSL_CMP_MSG* msg;
...
OSSL_CMP_MSG* ossl_cmp_certConf_new(OSSL_CMP_CTX* ctx,
                                    int certReqId,
                                    int fail_info,
                                    const char* text) {
  OSSL_CMP_MSG* msg = NULL;
...
OSSL_CMP_MSG* ossl_cmp_pollRep_new(OSSL_CMP_CTX* ctx,
                                   int crid,
                                   int64_t poll_after) {
  OSSL_CMP_MSG* msg;
...
OSSL_CMP_POLLREP* ossl_cmp_pollrepcontent_get0_pollrep(
    const OSSL_CMP_POLLREPCONTENT* prc,
    int rid) {
...
OSSL_CMP_CERTRESPONSE* ossl_cmp_certrepmessage_get0_certresponse(
    const OSSL_CMP_CERTREPMESSAGE* crm,
    int rid) {
...
OSSL_CMP_MSG* d2i_OSSL_CMP_MSG(OSSL_CMP_MSG** msg,
