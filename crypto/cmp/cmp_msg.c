...
OSSL_CMP_MSG_new(OSSL_LIB_CTX* libctx, const char* propq)
{
  ...
  msg = (OSSL_CMP_MSG*)ASN1_item_new_ex(
    ASN1_ITEM_rptr(OSSL_CMP_MSG), libctx, propq);
...
OSSL_CMP_PKIHEADER*
OSSL_CMP_MSG_get0_header(const OSSL_CMP_MSG* msg)
{
  if (msg == NULL) {
...
int
OSSL_CMP_MSG_get_bodytype(const OSSL_CMP_MSG* msg)
{
  if (!ossl_assert(msg != NULL && msg->body != NULL))
    return -1;
...
X509_PUBKEY*
OSSL_CMP_MSG_get0_certreq_publickey(const OSSL_CMP_MSG* msg)
{
...
static int
add1_extension(X509_EXTENSIONS** pexts, int nid, int crit, void* ex)
{
  X509_EXTENSION* ext;
  int res;
...
static int
add_crl_reason_extension(X509_EXTENSIONS** pexts, int reason_code)
{
...
static const X509_NAME*
determine_subj(OSSL_CMP_CTX* ctx, int for_KUR, const X509_NAME* ref_subj)
{
...
OSSL_CRMF_MSG*
OSSL_CMP_CTX_setup_CRM(OSSL_CMP_CTX* ctx, int for_KUR, int rid)
{
  ...
  if (!OSSL_CRMF_MSG_set_certReqId(crm, rid)
      ...
      !OSSL_CRMF_CERTTEMPLATE_fill(
        OSSL_CRMF_MSG_get0_tmpl(crm), rkey, subject, issuer, NULL /* serial */))
    goto err;
  if (rkey != NULL && central_keygen)
    X509_PUBKEY_set0_public_key(
      OSSL_CRMF_CERTTEMPLATE_get0_publicKey(tmpl), NULL, 0);
  ...
  if (!ctx->SubjectAltName_nodefault && !HAS_SAN(ctx) && refcert != NULL &&
      (default_sans = X509V3_get_d2i(
         X509_get0_extensions(refcert), NID_subject_alt_name, NULL, NULL)) !=
        NULL &&
  ...
  if (sk_GENERAL_NAME_num(ctx->subjectAltNames) > 0 &&
    ...
    goto err;
  if (ctx->policies != NULL && !add1_extension(&exts,
...
OSSL_CMP_MSG*
ossl_cmp_certreq_new(OSSL_CMP_CTX* ctx, int type, const OSSL_CRMF_MSG* crm)
{
  OSSL_CMP_MSG* msg;
  ...
  if (type != OSSL_CMP_PKIBODY_P10CR) {
    ...
    if (crm == NULL) {
      ...
      if (local_crm == NULL || !OSSL_CRMF_MSG_create_popo(ctx->popoMethod,
                                                          local_crm,
                                                          privkey,
                                                          ctx->digest,
                                                          ctx->libctx,
...
static OSSL_CRMF_ENCRYPTEDKEY*
enc_privkey(OSSL_CMP_CTX* ctx, const EVP_PKEY* pkey)
{
...
ossl_cmp_rr_new(OSSL_CMP_CTX* ctx)
{
  ...
  ret = OSSL_CRMF_CERTTEMPLATE_fill(
    rd->certDetails, pubkey, subject, issuer, serialNumber);
  if (!ret)
    goto err;
...
{
  return gen_new(
    ctx, ctx->genm_ITAVs, OSSL_CMP_PKIBODY_GENM, CMP_R_ERROR_CREATING_GENM);
}
...
static int
suitable_rid(const ASN1_INTEGER* certReqId, int rid)
{
  int trid;
                                     ...
                                     int rid)
{
  ...
  ERR_raise_data(
    ERR_LIB_CMP, CMP_R_CERTRESPONSE_NOT_FOUND, "expected certReqId = %d", rid);
  return NULL;
}
...
int
OSSL_CMP_MSG_update_transactionID(OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg)
{
...
int
OSSL_CMP_MSG_update_recipNonce(OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg)
{
...
OSSL_CMP_MSG*
OSSL_CMP_MSG_read(const char* file, OSSL_LIB_CTX* libctx, const char* propq)
{
  OSSL_CMP_MSG* msg;
  BIO* bio = NULL;
...
int
OSSL_CMP_MSG_write(const char* file, const OSSL_CMP_MSG* msg)
{
  BIO* bio;
  int res;
...
OSSL_CMP_MSG*
d2i_OSSL_CMP_MSG(OSSL_CMP_MSG** msg, const unsigned char** in, long len)
{
...
int
i2d_OSSL_CMP_MSG(const OSSL_CMP_MSG* msg, unsigned char** out)
{
  return ASN1_item_i2d(
    (const ASN1_VALUE*)msg, out, ASN1_ITEM_rptr(OSSL_CMP_MSG));
}

OSSL_CMP_MSG*
d2i_OSSL_CMP_MSG_bio(BIO* bio, OSSL_CMP_MSG** msg)
{
  OSSL_LIB_CTX* libctx = NULL;
  const char* propq = NULL;
  ...
  return ASN1_item_d2i_bio_ex(
    ASN1_ITEM_rptr(OSSL_CMP_MSG), bio, msg, libctx, propq);
}

int
i2d_OSSL_CMP_MSG_bio(BIO* bio, const OSSL_CMP_MSG* msg)
{
