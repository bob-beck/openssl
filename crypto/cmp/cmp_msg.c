...
const char* ossl_cmp_bodytype_to_string(int type) {
  ...
  if (type < 0 || type > OSSL_CMP_PKIBODY_TYPE_MAX) return "illegal body type";
  return type_names[type];
}

int ossl_cmp_msg_set_bodytype(OSSL_CMP_MSG* msg, int type) {
  if (!ossl_assert(msg != NULL && msg->body != NULL)) return 0;
...
int OSSL_CMP_MSG_get_bodytype(const OSSL_CMP_MSG* msg) {
  if (!ossl_assert(msg != NULL && msg->body != NULL)) return -1;
...
OSSL_CMP_MSG* ossl_cmp_msg_create(OSSL_CMP_CTX* ctx, int bodytype) {
  ...
  if (!ossl_assert(ctx != NULL)) return NULL;

  if ((msg = OSSL_CMP_MSG_new(ctx->libctx, ctx->propq)) == NULL) return NULL;
  ...
  switch (bodytype) {
    ...
    case OSSL_CMP_PKIBODY_CR:
    case OSSL_CMP_PKIBODY_KUR:
      if ((msg->body->value.ir = OSSL_CRMF_MSGS_new()) == NULL) goto err;
      return msg;
    ...
    case OSSL_CMP_PKIBODY_PKICONF:
      if ((msg->body->value.pkiconf = ASN1_TYPE_new()) == NULL) goto err;
...
OSSL_CRMF_MSG* OSSL_CMP_CTX_setup_CRM(OSSL_CMP_CTX* ctx, int for_KUR, int rid) {
  ...
  if ((crm = OSSL_CRMF_MSG_new()) == NULL) return NULL;
  ...
  if (ctx->policies != NULL &&
      !add1_extension(&exts, NID_certificate_policies, ctx->setPoliciesCritical,
                      ctx->policies))
    goto err;
  if (!OSSL_CRMF_MSG_set0_extensions(crm, exts)) goto err;
  exts = NULL;
...
OSSL_CMP_MSG* ossl_cmp_certreq_new(OSSL_CMP_CTX* ctx, int type,
  ...
  if ((msg = ossl_cmp_msg_create(ctx, type)) == NULL) goto err;
  ...
  if (type != OSSL_CMP_PKIBODY_P10CR) {
    ...
    if (crm == NULL) {
      ...
      if ((local_crm = OSSL_CRMF_MSG_dup(crm)) == NULL) goto err;
    }

    /* value.ir is same for cr and kur */
    if (!sk_OSSL_CRMF_MSG_push(msg->body->value.ir, local_crm)) goto err;
    local_crm = NULL;
  }

  if (!ossl_cmp_msg_protect(ctx, msg)) goto err;
...
static OSSL_CRMF_ENCRYPTEDKEY* enc_privkey(OSSL_CMP_CTX* ctx,
  ...
  if (privbio == NULL || i2d_PrivateKey_bio(privbio, pkey) <= 0) goto err;
  ...
  if (envData == NULL) goto err;
  ek = OSSL_CRMF_ENCRYPTEDKEY_init_envdata(envData);

err:
  sk_X509_pop_free(encryption_recips, X509_free);
  BIO_free(privbio);
  if (ek == NULL) M_ASN1_free_of(envData, CMS_EnvelopedData);

  return ek;
}
#endif

OSSL_CMP_MSG* ossl_cmp_certrep_new(
    OSSL_CMP_CTX* ctx, int bodytype, int certReqId, const OSSL_CMP_PKISI* si,
    X509* cert, const EVP_PKEY* pkey, const X509* encryption_recip,
  ...
  if (!ossl_assert(ctx != NULL && si != NULL)) return NULL;

  if ((msg = ossl_cmp_msg_create(ctx, bodytype)) == NULL) goto err;
  ...
  /* body */
  if ((resp = OSSL_CMP_CERTRESPONSE_new()) == NULL) goto err;
  ...
  if (!sk_OSSL_CMP_CERTRESPONSE_push(repMsg->response, resp)) goto err;
  resp = NULL;
...
OSSL_CMP_MSG* ossl_cmp_rr_new(OSSL_CMP_CTX* ctx) {
  ...
  if ((rd = OSSL_CMP_REVDETAILS_new()) == NULL) goto err;
  ...
  if ((msg = ossl_cmp_msg_create(ctx, OSSL_CMP_PKIBODY_RR)) == NULL) goto err;

  if (!sk_OSSL_CMP_REVDETAILS_push(msg->body->value.rr, rd)) goto err;
  rd = NULL;
...
OSSL_CMP_MSG* ossl_cmp_rp_new(OSSL_CMP_CTX* ctx, const OSSL_CMP_PKISI* si,
  ...
  if ((msg = ossl_cmp_msg_create(ctx, OSSL_CMP_PKIBODY_RP)) == NULL) goto err;
  ...
  if ((rep->revCerts = sk_OSSL_CRMF_CERTID_new_null()) == NULL) goto err;
  if (cid != NULL) {
...
OSSL_CMP_MSG* ossl_cmp_pkiconf_new(OSSL_CMP_CTX* ctx) {
  ...
  if ((msg = ossl_cmp_msg_create(ctx, OSSL_CMP_PKIBODY_PKICONF)) == NULL)
    goto err;
  if (ossl_cmp_msg_protect(ctx, msg)) return msg;
...
int ossl_cmp_msg_gen_push1_ITAVs(OSSL_CMP_MSG* msg,
  ...
  if (!ossl_assert(msg != NULL)) return 0;
...
static OSSL_CMP_MSG* gen_new(OSSL_CMP_CTX* ctx,
  ...
  if ((msg = ossl_cmp_msg_create(ctx, body_type)) == NULL) return NULL;

  if (itavs != NULL && !ossl_cmp_msg_gen_push1_ITAVs(msg, itavs)) goto err;
...
OSSL_CMP_MSG* ossl_cmp_error_new(OSSL_CMP_CTX* ctx, const OSSL_CMP_PKISI* si,
  ...
  if (lib != NULL || reason != NULL || details != NULL) {
    if ((ft = sk_ASN1_UTF8STRING_new_null()) == NULL) goto err;
  ...
  if (!unprotected && !ossl_cmp_msg_protect(ctx, msg)) goto err;
  return msg;
...
OSSL_CMP_MSG* ossl_cmp_certConf_new(OSSL_CMP_CTX* ctx, int certReqId,
  ...
  if ((certStatus = OSSL_CMP_CERTSTATUS_new()) == NULL) goto err;
  ...
  if (!ASN1_INTEGER_set(certStatus->certReqId, certReqId)) goto err;
  ...
  if ((certHash = X509_digest_sig(ctx->newCert, &md, &is_fallback)) == NULL)
    goto err;
  if (is_fallback) {
    if (!ossl_cmp_hdr_set_pvno(msg->header, OSSL_CMP_PVNO_3)) goto err;
    if ((certStatus->hashAlg = X509_ALGOR_new()) == NULL) goto err;
    X509_ALGOR_set_md(certStatus->hashAlg, md);
  }
  EVP_MD_free(md);

  if (!ossl_cmp_certstatus_set0_certHash(certStatus, certHash)) goto err;
  certHash = NULL;
  /*
   ...
   */
  sinfo = fail_info != 0
              ? OSSL_CMP_STATUSINFO_new(OSSL_CMP_PKISTATUS_rejection, fail_info,
                                        text)
              : OSSL_CMP_STATUSINFO_new(OSSL_CMP_PKISTATUS_accepted, 0, text);
  if (sinfo == NULL) goto err;
...
OSSL_CMP_MSG* ossl_cmp_pollRep_new(OSSL_CMP_CTX* ctx, int crid,
  ...
  if ((msg = ossl_cmp_msg_create(ctx, OSSL_CMP_PKIBODY_POLLREP)) == NULL)
    goto err;
  if ((prep = OSSL_CMP_POLLREP_new()) == NULL) goto err;
  if (!sk_OSSL_CMP_POLLREP_push(msg->body->value.pollRep, prep)) goto err;
  if (!ASN1_INTEGER_set(prep->certReqId, crid)) goto err;
  if (!ASN1_INTEGER_set_int64(prep->checkAfter, poll_after)) goto err;
...
OSSL_CMP_PKISI* ossl_cmp_revrepcontent_get_pkisi(OSSL_CMP_REVREPCONTENT* rrep,
  ...
  if (!ossl_assert(rrep != NULL)) return NULL;
 ...
 * returns NULL on error
 */
OSSL_CRMF_CERTID* ossl_cmp_revrepcontent_get_CertId(
    OSSL_CMP_REVREPCONTENT* rrep, int rsid) {
...
static int suitable_rid(const ASN1_INTEGER* certReqId, int rid) {
  ...
  if (rid == OSSL_CMP_CERTREQID_NONE) return 1;
 ...
 */
OSSL_CMP_POLLREP* ossl_cmp_pollrepcontent_get0_pollrep(
    const OSSL_CMP_POLLREPCONTENT* prc, int rid) {
  OSSL_CMP_POLLREP* pollRep = NULL;
  int i;

  if (!ossl_assert(prc != NULL)) return NULL;

  for (i = 0; i < sk_OSSL_CMP_POLLREP_num(prc); i++) {
    pollRep = sk_OSSL_CMP_POLLREP_value(prc, i);
    if (suitable_rid(pollRep->certReqId, rid)) return pollRep;
  }
...
OSSL_CMP_CERTRESPONSE* ossl_cmp_certrepmessage_get0_certresponse(
    const OSSL_CMP_CERTREPMESSAGE* crm, int rid) {
  OSSL_CMP_CERTRESPONSE* crep = NULL;
  int i;

  if (!ossl_assert(crm != NULL && crm->response != NULL)) return NULL;

  for (i = 0; i < sk_OSSL_CMP_CERTRESPONSE_num(crm->response); i++) {
    crep = sk_OSSL_CMP_CERTRESPONSE_value(crm->response, i);
    if (suitable_rid(crep->certReqId, rid)) return crep;
  }
...
X509* ossl_cmp_certresponse_get1_cert(const OSSL_CMP_CTX* ctx,
  ...
  if (!ossl_assert(crep != NULL && ctx != NULL)) return NULL;
...
int OSSL_CMP_MSG_update_transactionID(OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg) {
  ...
  if (!ossl_cmp_hdr_set_transactionID(ctx, msg->header)) return 0;
