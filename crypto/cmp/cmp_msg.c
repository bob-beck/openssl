 ...
 */
int
ossl_cmp_msg_set0_libctx (OSSL_CMP_MSG *msg, OSSL_LIB_CTX *libctx,
                          const char *propq)
{
  if (msg != NULL)
    {
...
ossl_cmp_certrep_new (OSSL_CMP_CTX *ctx, int bodytype, int certReqId,
                      const OSSL_CMP_PKISI *si, X509 *cert,
                      const EVP_PKEY *pkey, const X509 *encryption_recip,
...
OSSL_CMP_MSG *
ossl_cmp_rp_new (OSSL_CMP_CTX *ctx, const OSSL_CMP_PKISI *si,
 ...
 */
static OSSL_CMP_MSG *
gen_new (OSSL_CMP_CTX *ctx, const STACK_OF (OSSL_CMP_ITAV) *itavs,
...
OSSL_CMP_MSG *
ossl_cmp_error_new (OSSL_CMP_CTX *ctx, const OSSL_CMP_PKISI *si,
                    int64_t errorCode, const char *details, int unprotected)
{
  OSSL_CMP_MSG *msg = NULL;
...
OSSL_CMP_MSG *
ossl_cmp_certConf_new (OSSL_CMP_CTX *ctx, int certReqId, int fail_info,
                       const char *text)
{
  OSSL_CMP_MSG *msg = NULL;
...
/*-
 * Retrieve newly enrolled certificate and key from the given certResponse
 * crep. Stores any centrally generated key in ctx->newPkey. In case of
 * indirect POPO uses ctx->newPkey to decrypt the new certificate. Returns a
 * pointer to a copy of the found certificate, or NULL if not found.
 */
X509 *
