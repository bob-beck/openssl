...
int ossl_cmp_hdr_get_pvno(const OSSL_CMP_PKIHEADER* hdr) {
  ...
  if (!ossl_assert(hdr != NULL)) return -1;
...
int ossl_cmp_hdr_get_protection_nid(const OSSL_CMP_PKIHEADER* hdr) {
  if (!ossl_assert(hdr != NULL) || hdr->protectionAlg == NULL) return NID_undef;
...
ASN1_OCTET_STRING* ossl_cmp_hdr_get0_senderNonce(
    const OSSL_CMP_PKIHEADER* hdr) {
  if (!ossl_assert(hdr != NULL)) return NULL;
...
int ossl_cmp_hdr_push0_freeText(OSSL_CMP_PKIHEADER* hdr,
  ...
  if (!ossl_assert(hdr != NULL && text != NULL)) return 0;
...
int ossl_cmp_hdr_generalInfo_push0_item(OSSL_CMP_PKIHEADER* hdr,
                                        OSSL_CMP_ITAV* itav) {
  if (!ossl_assert(hdr != NULL && itav != NULL)) return 0;
...
int ossl_cmp_hdr_set_implicitConfirm(OSSL_CMP_PKIHEADER* hdr) {
  ...
  if ((itav = OSSL_CMP_ITAV_create(OBJ_nid2obj(NID_id_it_implicitConfirm),
                                   asn1null)) == NULL)
    goto err;
  if (!ossl_cmp_hdr_generalInfo_push0_item(hdr, itav)) goto err;
  return 1;
...
int ossl_cmp_hdr_init(OSSL_CMP_CTX* ctx, OSSL_CMP_PKIHEADER* hdr) {
  ...
  if (!ossl_assert(ctx != NULL && hdr != NULL)) return 0;

  /* set the CMP version */
  if (!ossl_cmp_hdr_set_pvno(hdr, OSSL_CMP_PVNO)) return 0;
  ...
  if (!ossl_cmp_hdr_set1_recipient(hdr, rcp)) return 0;

  /* set current time as message time */
  if (!ossl_cmp_hdr_update_messageTime(hdr)) return 0;

  if (ctx->recipNonce != NULL &&
      !ossl_cmp_asn1_octet_string_set1(&hdr->recipNonce, ctx->recipNonce))
    return 0;

  if (!ossl_cmp_hdr_set_transactionID(ctx, hdr)) return 0;
  ...
  if (!OSSL_CMP_CTX_set1_senderNonce(ctx, hdr->senderNonce)) return 0;
