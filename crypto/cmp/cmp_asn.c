...
void
OSSL_CMP_ITAV_set0 (OSSL_CMP_ITAV *itav, ASN1_OBJECT *type, ASN1_TYPE *value)
{
  itav->infoType = type;
...
ASN1_OBJECT *
OSSL_CMP_ITAV_get0_type (const OSSL_CMP_ITAV *itav)
{
  if (itav == NULL)
    return NULL;
...
ASN1_TYPE *
OSSL_CMP_ITAV_get0_value (const OSSL_CMP_ITAV *itav)
{
  if (itav == NULL)
    return NULL;
...
int
OSSL_CMP_ITAV_push0_stack_item (STACK_OF (OSSL_CMP_ITAV) **itav_sk_p,
                                OSSL_CMP_ITAV *itav)
{
  int created = 0;
...
int
OSSL_CMP_ITAV_get0_certProfile (const OSSL_CMP_ITAV *itav,
...
OSSL_CMP_ITAV *
OSSL_CMP_ITAV_new_caCerts (const STACK_OF (X509) *caCerts)
{
...
int
OSSL_CMP_ITAV_get0_caCerts (const OSSL_CMP_ITAV *itav, STACK_OF (X509) **out)
{
  if (itav == NULL || out == NULL)
    {
      ERR_raise (ERR_LIB_CMP, ERR_R_PASSED_NULL_PARAMETER);
...
OSSL_CMP_ITAV *
OSSL_CMP_ITAV_new_rootCaCert (const X509 *rootCaCert)
{
  OSSL_CMP_ITAV *itav = OSSL_CMP_ITAV_new ();
...
int
OSSL_CMP_ITAV_get0_rootCaCert (const OSSL_CMP_ITAV *itav, X509 **out)
{
  if (itav == NULL || out == NULL)
    {
      ERR_raise (ERR_LIB_CMP, ERR_R_PASSED_NULL_PARAMETER);
...
OSSL_CMP_ITAV *
OSSL_CMP_ITAV_new_rootCaKeyUpdate (const X509 *newWithNew,
...
int
OSSL_CMP_ITAV_get0_rootCaKeyUpdate (const OSSL_CMP_ITAV *itav,
...
int
OSSL_CMP_ITAV_get1_certReqTemplate (const OSSL_CMP_ITAV *itav,
          ...
          if (nid != NID_id_regCtrl_algId && nid != NID_id_regCtrl_rsaKeyLen)
            {
              ...
              ERR_raise_data (ERR_LIB_CMP, CMP_R_INVALID_KEYSPEC,
                              "keySpec with index %d in certReqTemplate has "
                              "invalid type %s",
                              i, name);
              goto err;
            }
...
OSSL_CMP_ATAV *
OSSL_CMP_ATAV_create (ASN1_OBJECT *type, ASN1_TYPE *value)
{
  OSSL_CMP_ATAV *atav;
...
void
OSSL_CMP_ATAV_set0 (OSSL_CMP_ATAV *atav, ASN1_OBJECT *type, ASN1_TYPE *value)
{
  atav->type = type;
...
ASN1_OBJECT *
OSSL_CMP_ATAV_get0_type (const OSSL_CMP_ATAV *atav)
{
  if (atav == NULL)
    return NULL;
  return atav->type;
...
OSSL_CMP_ATAV *
OSSL_CMP_ATAV_new_algId (const X509_ALGOR *alg)
{
  X509_ALGOR *dup;
  OSSL_CMP_ATAV *res;
...
X509_ALGOR *
OSSL_CMP_ATAV_get0_algId (const OSSL_CMP_ATAV *atav)
{
...
OSSL_CMP_ATAV *
OSSL_CMP_ATAV_new_rsaKeyLen (int len)
{
  ASN1_INTEGER *aint;
...
int
OSSL_CMP_ATAV_get_rsaKeyLen (const OSSL_CMP_ATAV *atav)
{
  int64_t val;
...
ASN1_TYPE *
OSSL_CMP_ATAV_get0_value (const OSSL_CMP_ATAV *atav)
{
  if (atav == NULL)
    return NULL;
...
int
OSSL_CMP_ATAV_push1 (OSSL_CMP_ATAVS **sk_p, const OSSL_CMP_ATAV *atav)
{
  int created = 0;
  OSSL_CMP_ATAV *dup;
...
int
OSSL_CMP_ITAV_get0_crlStatusList (const OSSL_CMP_ITAV *itav,
...
OSSL_CMP_CRLSTATUS *
OSSL_CMP_CRLSTATUS_new1 (const DIST_POINT_NAME *dpn,
...
gennames_new (const X509_NAME *nm)
{
  GENERAL_NAMES *names;
...
static int
gennames_allowed (GENERAL_NAMES *names, int only_DN)
{
  if (names == NULL)
    return 0;
  if (!only_DN)
...
OSSL_CMP_CRLSTATUS *
OSSL_CMP_CRLSTATUS_create (const X509_CRL *crl, const X509 *cert, int only_DN)
{
...
int
OSSL_CMP_CRLSTATUS_get0 (const OSSL_CMP_CRLSTATUS *crlstatus,
...
OSSL_CMP_ITAV *
OSSL_CMP_ITAV_new_crls (const X509_CRL *crl)
{
  OSSL_CMP_ITAV *itav;
...
int
OSSL_CMP_ITAV_get0_crls (const OSSL_CMP_ITAV *itav, STACK_OF (X509_CRL) **out)
{
  if (itav == NULL || out == NULL)
    {
      ERR_raise (ERR_LIB_CMP, ERR_R_PASSED_NULL_PARAMETER);
...
static int
ossl_cmp_msg_cb (int operation, ASN1_VALUE **pval,
...
ASN1_SEQUENCE (OSSL_CMP_POLLREQ)
    = { ASN1_SIMPLE (OSSL_CMP_POLLREQ, certReqId,
...
ASN1_CHOICE (OSSL_CMP_PKIBODY) = {
  ...
  ASN1_EXP (OSSL_CMP_PKIBODY, value.popdecc, OSSL_CMP_POPODECKEYCHALLCONTENT,
            5),
  ASN1_EXP (OSSL_CMP_PKIBODY, value.popdecr, OSSL_CMP_POPODECKEYRESPCONTENT,
            6),
