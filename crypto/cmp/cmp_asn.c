...
OSSL_CMP_ITAV* OSSL_CMP_ITAV_create(ASN1_OBJECT* type, ASN1_TYPE* value) {
  ...
  if (type == NULL || (itav = OSSL_CMP_ITAV_new()) == NULL) return NULL;
...
int OSSL_CMP_ITAV_push0_stack_item(STACK_OF(OSSL_CMP_ITAV)** itav_sk_p,
  ...
  if (*itav_sk_p == NULL) {
    if ((*itav_sk_p = sk_OSSL_CMP_ITAV_new_null()) == NULL) goto err;
    created = 1;
  }
  if (!sk_OSSL_CMP_ITAV_push(*itav_sk_p, itav)) goto err;
  return 1;
...
*OSSL_CMP_ITAV_new0_certProfile(STACK_OF(ASN1_UTF8STRING)* certProfile) {
  ...
  if ((itav = OSSL_CMP_ITAV_new()) == NULL) return NULL;
...
OSSL_CMP_ITAV* OSSL_CMP_ITAV_new_rootCaKeyUpdate(const X509* newWithNew,
  ...
  if (newWithNew != NULL) {
    ...
    if ((upd->newWithNew = X509_dup(newWithNew)) == NULL) goto err;
  ...
  if ((itav = OSSL_CMP_ITAV_new()) == NULL) goto err;
...
int OSSL_CMP_ITAV_get0_rootCaKeyUpdate(const OSSL_CMP_ITAV* itav,
  ...
  *newWithNew = upd != NULL ? upd->newWithNew : NULL;
  if (newWithOld != NULL) *newWithOld = upd != NULL ? upd->newWithOld : NULL;
  if (oldWithNew != NULL) *oldWithNew = upd != NULL ? upd->oldWithNew : NULL;
  return 1;
}
...
*OSSL_CMP_ITAV_new0_certReqTemplate(OSSL_CRMF_CERTTEMPLATE* certTemplate,
  ...
  if (certTemplate == NULL) return itav;
...
OSSL_CMP_ATAV* OSSL_CMP_ATAV_create(ASN1_OBJECT* type, ASN1_TYPE* value) {
  ...
  if ((atav = OSSL_CRMF_ATTRIBUTETYPEANDVALUE_new()) == NULL) return NULL;
...
OSSL_CMP_ATAV* OSSL_CMP_ATAV_new_algId(const X509_ALGOR* alg) {
  ...
  if ((dup = X509_ALGOR_dup(alg)) == NULL) return NULL;
  res =
      OSSL_CMP_ATAV_create(OBJ_nid2obj(NID_id_regCtrl_algId), (ASN1_TYPE*)dup);
  if (res == NULL) X509_ALGOR_free(dup);
  return res;
}
...
OSSL_CMP_ATAV* OSSL_CMP_ATAV_new_rsaKeyLen(int len) {
  ...
  if ((aint = ASN1_INTEGER_new()) == NULL) return NULL;
...
int OSSL_CMP_ATAV_push1(OSSL_CMP_ATAVS** sk_p, const OSSL_CMP_ATAV* atav) {
  ...
  if ((dup = OSSL_CRMF_ATTRIBUTETYPEANDVALUE_dup(
    ...
    goto err;
  if (sk_OSSL_CRMF_ATTRIBUTETYPEANDVALUE_push(*sk_p, dup)) return 1;
...
OSSL_CMP_CRLSTATUS* OSSL_CMP_CRLSTATUS_new1(const DIST_POINT_NAME* dpn,
  ...
  if ((crlstatus = OSSL_CMP_CRLSTATUS_new()) == NULL) return NULL;
  crlsource = crlstatus->source;

  if (dpn != NULL) {
    crlsource->type = OSSL_CMP_CRLSOURCE_DPN;
    if ((crlsource->value.dpn = DIST_POINT_NAME_dup(dpn)) == NULL) goto err;
  } else {
...
static GENERAL_NAMES* gennames_new(const X509_NAME* nm) {
  ...
  if ((names = sk_GENERAL_NAME_new_reserve(NULL, 1)) == NULL) return NULL;
...
OSSL_CMP_CRLSTATUS* OSSL_CMP_CRLSTATUS_create(const X509_CRL* crl,
  ...
  if (cert != NULL) {
    ...
    if (idp != NULL && idp->distpoint != NULL) dpn = idp->distpoint;
  }
...
static int ossl_cmp_msg_cb(int operation, ASN1_VALUE** pval,
  ...
  switch (operation) {
    ...
    case ASN1_OP_DUP_POST: {
      ...
      if (!ossl_cmp_msg_set0_libctx(msg, old->libctx, old->propq)) return 0;
    } break;
...
ASN1_SEQUENCE(OSSL_CMP_CERTIFIEDKEYPAIR) = {
    ...
    ASN1_EXP_OPT(OSSL_CMP_CERTIFIEDKEYPAIR, privateKey, OSSL_CRMF_ENCRYPTEDKEY,
                 0),
