...
ASN1_BIT_STRING* ossl_cmp_calc_protection(const OSSL_CMP_CTX* ctx,
  ...
  if (!ossl_assert(ctx != NULL && msg != NULL)) return NULL;
  ...
  if (OBJ_obj2nid(algorOID) == NID_id_PasswordBasedMAC) {
    ...
    if (sig_len > INT_MAX || (prot = ASN1_BIT_STRING_new()) == NULL) goto end;
    ...
    if ((prot = ASN1_BIT_STRING_new()) == NULL) return NULL;
...
void ossl_cmp_set_own_chain(OSSL_CMP_CTX* ctx) {
  if (!ossl_assert(ctx != NULL)) return;
...
int ossl_cmp_msg_add_extraCerts(OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg) {
  if (!ossl_assert(ctx != NULL && msg != NULL)) return 0;
...
static X509_ALGOR* pbmac_algor(const OSSL_CMP_CTX* ctx) {
  ...
  if (pbm == NULL || pbm_str == NULL) goto err;
  if ((pbm_der_len = i2d_OSSL_CRMF_PBMPARAMETER(pbm, &pbm_der)) < 0) goto err;
  if (!ASN1_STRING_set(pbm_str, pbm_der, pbm_der_len)) goto err;
  alg = ossl_X509_ALGOR_from_nid(NID_id_PasswordBasedMAC, V_ASN1_SEQUENCE,
                                 pbm_str);
err:
  if (alg == NULL) ASN1_STRING_free(pbm_str);
...
int ossl_cmp_msg_protect(OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg) {
  ...
  if (ctx->unprotectedSend) {
    if (!set_senderKID(ctx, msg, NULL)) goto err;
  } else if (ctx->secretValue != NULL) {
    /* use PasswordBasedMac according to 5.1.3.1 if secretValue is given */
    if ((msg->header->protectionAlg = pbmac_algor(ctx)) == NULL) goto err;
    if (!set_senderKID(ctx, msg, NULL)) goto err;
    ...
    if ((msg->header->protectionAlg = X509_ALGOR_new()) == NULL) goto err;
    /* set senderKID to keyIdentifier of the cert according to 5.1.1 */
    if (!set_senderKID(ctx, msg, X509_get0_subject_key_id(ctx->cert))) goto err;
   ...
   * the option to do this might be handy for certain use cases.
   */
  if (!ossl_cmp_msg_add_extraCerts(ctx, msg)) goto err;
