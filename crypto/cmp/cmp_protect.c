...
ossl_cmp_set_own_chain(OSSL_CMP_CTX* ctx)
{
  ...
  if (ctx->chain == NULL) {
    ...
    ctx->chain = X509_build_chain(
      ctx->cert, ctx->untrusted, NULL, 0, ctx->libctx, ctx->propq);
    if (ctx->chain != NULL) {
...
ossl_cmp_msg_add_extraCerts(OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg)
{
  ...
  if (!ossl_x509_add_certs_new(&msg->extraCerts,
                               ctx->extraCertsOut,
...
pbmac_algor(const OSSL_CMP_CTX* ctx)
{
  ...
  pbm = OSSL_CRMF_pbmp_new(ctx->libctx,
                           ctx->pbm_slen,
...
static int
set_senderKID(const OSSL_CMP_CTX* ctx,
              OSSL_CMP_MSG* msg,
