...
static int
keep_alive (int keep_alive, int body_type, BIO **bios)
{
  if (keep_alive != 0
      && bios == NULL
      /*
 ...
 */
OSSL_CMP_MSG *
OSSL_CMP_MSG_http_perform (OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req)
{
    ...
    { /* using existing connection or yet not set up own connection */
      ...
      if (bios == NULL)
        ...
        ossl_cmp_log3 (DEBUG, ctx,
                       "using existing connection with CMP server %s:%s and "
                       "HTTP path /%s",
