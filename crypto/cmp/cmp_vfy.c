...
static int
verify_signature (const OSSL_CMP_CTX *cmp_ctx, const OSSL_CMP_MSG *msg,
                  X509 *cert)
{
...
static int
verify_PBMAC (OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg)
{
 ...
 */
int
OSSL_CMP_validate_cert_path (const OSSL_CMP_CTX *ctx,
...
static int
verify_cb_cert (X509_STORE *ts, X509 *cert, int err)
{
...
static int
check_name (const OSSL_CMP_CTX *ctx, int log_success, const char *actual_desc,
            const X509_NAME *actual_name, const char *expect_desc,
...
static int
check_kid (const OSSL_CMP_CTX *ctx, const ASN1_OCTET_STRING *ckid,
...
static int
already_checked (const X509 *cert, const STACK_OF (X509) *already_checked)
{
  int i;
 ...
 */
static int
cert_acceptable (const OSSL_CMP_CTX *ctx, const char *desc1, const char *desc2,
                 X509 *cert, const STACK_OF (X509) *already_checked1,
...
static int
check_cert_path (const OSSL_CMP_CTX *ctx, X509_STORE *store, X509 *scrt)
{
 ...
 */
static int
check_cert_path_3gpp (const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg,
                      X509 *scrt)
{
  int valid = 0;
  X509_STORE *store;
...
static int
check_msg_given_cert (const OSSL_CMP_CTX *ctx, X509 *cert,
                      const OSSL_CMP_MSG *msg)
{
 ...
 * If already_checked1 == NULL then certs are assumed to be the
 * msg->extraCerts. On success cache the found cert using
 * ossl_cmp_ctx_set1_validatedSrvCert().
 */
static int
check_msg_with_certs (OSSL_CMP_CTX *ctx, const STACK_OF (X509) *certs,
                      const char *desc,
 ...
 */
static int
check_msg_all_certs (OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg, int mode_3gpp)
{
  int ret = 0;
 ...
 */
static int
check_msg_find_cert (OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg)
{
 ...
 * based mac (PBM) or a signature algorithm. In the case of signature
 * algorithm, the sender certificate can have been pinned by providing it in
 * ctx->srvCert, else it is searched in msg->extraCerts, ctx->untrusted, in
 * ctx->trusted (in this order) and is path is validated against ctx->trusted.
 * On success cache the found cert using ossl_cmp_ctx_set1_validatedSrvCert().
 *
 ...
 */
int
OSSL_CMP_validate_msg (OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg)
{
  X509 *scrt;
      ...
      if (scrt == NULL)
        {
          ...
          if (check_msg_find_cert (ctx, msg))
            {
              ossl_cmp_debug (ctx, "successfully validated signature-based "
                                   "CMP message protection using trust store");
              return 1;
            }
        }
      else
        { /* use pinned sender cert */
...
static int
check_transactionID_or_nonce (ASN1_OCTET_STRING *expected,
 ...
 *     is present and yields a positive result using also the supplied
 * argument) its transaction ID matches the previous transaction ID stored in
 * ctx (if any) its recipNonce matches the previous senderNonce stored in the
 * ctx (if any)
 *
 ...
 */
int
ossl_cmp_msg_check_update (OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg,
...
int
ossl_cmp_verify_popo (const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg,
