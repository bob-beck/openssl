...
static int verify_signature(const OSSL_CMP_CTX *cmp_ctx, const OSSL_CMP_MSG *msg, X509 *cert)
{
    ...
    if (ASN1_item_verify_ex(ASN1_ITEM_rptr(OSSL_CMP_PROTECTEDPART), msg->header->protectionAlg, msg->protection,
                            &prot_part, NULL, pubkey, cmp_ctx->libctx, cmp_ctx->propq) > 0)
    {
        res = 1;
        goto end;
    }
...
int OSSL_CMP_validate_msg(OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *msg)
{
    ...
    default:
        ...
        if (scrt == NULL)
        {
            ...
            {
                ossl_cmp_info(ctx, "no trust store nor pinned server cert available for verifying signature-based CMP "
                                   "message protection");
