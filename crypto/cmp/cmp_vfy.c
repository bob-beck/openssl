...
static int
verify_signature(const OSSL_CMP_CTX* cmp_ctx,
...
verify_PBMAC(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* msg)
{
  ...
  valid = msg->protection != NULL && msg->protection->length >= 0 &&
          ...
          CRYPTO_memcmp(
            msg->protection->data, protection->data, protection->length) == 0;
...
static int
check_name(const OSSL_CMP_CTX* ctx,
           int log_success,
           ...
           const X509_NAME* expect_name)
{
  ...
  if (X509_NAME_cmp(actual_name, expect_name) == 0) {
    if (log_success && str != NULL)
      ossl_cmp_log3(
        INFO, ctx, " %s matches %s: %s", actual_desc, expect_desc, str);
    OPENSSL_free(str);
    return 1;
  }
...
static int
check_kid(const OSSL_CMP_CTX* ctx,
 ...
 */
static int
cert_acceptable(const OSSL_CMP_CTX* ctx,
                const char* desc1,
                const char* desc2,
                X509* cert,
 ...
 */
static int
check_cert_path_3gpp(const OSSL_CMP_CTX* ctx,
                     const OSSL_CMP_MSG* msg,
                     X509* scrt)
{
  int valid = 0;
  ...
  if ((store = X509_STORE_new()) == NULL ||
      !ossl_cmp_X509_STORE_add1_certs(
        store, msg->extraCerts, 1 /* self-issued only */))
    goto err;
...
static int
check_msg_given_cert(const OSSL_CMP_CTX* ctx,
                     X509* cert,
                     const OSSL_CMP_MSG* msg)
{
  return cert_acceptable(
           ctx, "previously validated", "sender cert", cert, NULL, NULL, msg) &&
 ...
 */
static int
check_msg_with_certs(OSSL_CMP_CTX* ctx,
...
check_msg_all_certs(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* msg, int mode_3gpp)
{
  ...
  if (check_msg_with_certs(
        ctx, msg->extraCerts, "extraCerts", NULL, NULL, msg, mode_3gpp))
    return 1;
  if (check_msg_with_certs(ctx,
                           ctx->untrusted,
                           "untrusted certs",
                           msg->extraCerts,
                           NULL,
    ...
    default:
      ...
      if (scrt == NULL) {
        ...
        if (check_msg_find_cert(ctx, msg)) {
          ossl_cmp_debug(ctx,
                         "successfully validated signature-based CMP message "
                         "protection using trust store");
          return 1;
        }
        ...
        if (verify_signature(ctx, msg, scrt)) {
          ossl_cmp_debug(ctx,
                         "successfully validated signature-based CMP message "
                         "protection using pinned server cert");
                          ...
                          int cb_arg)
{
  ...
  if (expected_sender != NULL) {
    ...
    if (!check_name(ctx,
      ...
      ERR_raise_data(
        ERR_LIB_CMP, CMP_R_UNEXPECTED_SENDER, str != NULL ? str : "<unknown>");
      OPENSSL_free(str);
      return 0;
    }
  }
  ...
  if (num_added > 10)
    ossl_cmp_log1(
      WARN, ctx, "received CMP message contains %d extraCerts", num_added);
  /*
  ...
  if (!check_transactionID_or_nonce(
        ctx->transactionID, hdr->transactionID, CMP_R_TRANSACTIONID_UNMATCHED))
    return 0;
  ...
  if (!check_transactionID_or_nonce(
        ctx->senderNonce, hdr->recipNonce, CMP_R_RECIPNONCE_UNMATCHED)) {
                     ...
                     int acceptRAVerified)
{
  ...
  switch (msg->body->type) {
    case OSSL_CMP_PKIBODY_P10CR: {
      ...
      if (X509_REQ_verify_ex(
            req, X509_REQ_get0_pubkey(req), ctx->libctx, ctx->propq) <= 0) {
#ifndef FUZZING_BUILD_MODE_UNSAFE_FOR_PRODUCTION
