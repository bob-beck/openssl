...
static int verify_signature(const OSSL_CMP_CTX* cmp_ctx,
  ...
  if (!ossl_assert(cmp_ctx != NULL && msg != NULL && cert != NULL)) return 0;
...
static int verify_PBMAC(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* msg) {
  ...
  if (!valid) ERR_raise(ERR_LIB_CMP, CMP_R_WRONG_PBM_VALUE);
...
static int check_kid(const OSSL_CMP_CTX* ctx, const ASN1_OCTET_STRING* ckid,
  ...
  if (skid == NULL) return 1; /* no expectation, thus trivially fulfilled */
...
static int already_checked(const X509* cert,
  ...
  for (i = sk_X509_num(already_checked /* may be NULL */); i > 0; i--)
    if (X509_cmp(sk_X509_value(already_checked, i - 1), cert) == 0) return 1;
  return 0;
}
...
static int cert_acceptable(const OSSL_CMP_CTX* ctx, const char* desc1,
  ...
  if (!self_issued) {
    ...
    if (str != NULL) ossl_cmp_log1(INFO, ctx, "  issuer  = %s", str);
    OPENSSL_free(str);
  }
...
static int check_cert_path(const OSSL_CMP_CTX* ctx, X509_STORE* store,
                           X509* scrt) {
  if (OSSL_CMP_validate_cert_path(ctx, store, scrt)) return 1;
...
static int check_cert_path_3gpp(const OSSL_CMP_CTX* ctx,
  ...
  if (!ctx->permitTAInExtraCertsForIR) return 0;
...
static int check_msg_with_certs(OSSL_CMP_CTX* ctx, const STACK_OF(X509)* certs,
  ...
  for (i = 0; i < sk_X509_num(certs); i++) { /* certs may be NULL */
    ...
    if (!ossl_assert(cert != NULL)) return 0;
  ...
  switch (ossl_cmp_hdr_get_protection_nid(msg->header)) {
    ...
    case NID_id_PasswordBasedMAC:
      if (ctx->secretValue == NULL) {
        ossl_cmp_info(ctx,
                      "no secret available for verifying PBM-based CMP message "
                      "protection");
...
int ossl_cmp_msg_check_update(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* msg,
  ...
  if (!ossl_assert(ctx != NULL && msg != NULL && msg->header != NULL)) return 0;
  ...
  res = ossl_x509_add_certs_new(
      &ctx->untrusted, msg->extraCerts,
   ...
   */
  if (!ossl_cmp_ctx_set1_recipNonce(ctx, hdr->senderNonce)) return 0;
