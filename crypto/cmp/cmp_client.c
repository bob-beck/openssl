...
/*-
 * Evaluate whether there's an exception (violating the standard) configured
 * for handling negative responses without protection or with invalid
 * protection. Returns 1 on acceptance, 0 on rejection, or -1 on (internal)
 * error.
 */
static int
unprotected_exception (const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *rep,
                       int invalid_protection, ossl_unused int expected_type)
{
...
static int
save_statusInfo (OSSL_CMP_CTX *ctx, OSSL_CMP_PKISI *si)
{
  int i;
...
static int
is_crep_with_waiting (const OSSL_CMP_MSG *resp, int rid)
{
 ...
 */
static int
send_receive_check (OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req,
 ...
 */
static int
poll_for_response (OSSL_CMP_CTX *ctx, int sleep, int rid, OSSL_CMP_MSG **rep,
                   int *checkAfter)
{
...
static int
save_senderNonce_if_waiting (OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *rep,
                             int rid)
{
  /*
 ...
 */
static int
send_receive_also_delayed (OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req,
                           OSSL_CMP_MSG **rep, int expected_type)
{
 ...
 */
int
ossl_cmp_exchange_certConf (OSSL_CMP_CTX *ctx, int certReqId, int fail_info,
                            const char *txt)
{
...
int
ossl_cmp_exchange_error (OSSL_CMP_CTX *ctx, int status, int fail_info,
 ...
 */
static X509 *
get1_cert_status (OSSL_CMP_CTX *ctx, int bodytype, OSSL_CMP_CERTRESPONSE *crep)
{
  char buf[OSSL_CMP_PKISI_BUFLEN];
  X509 *crt = NULL;
  ...
  switch (ossl_cmp_pkisi_get_status (crep->status))
    ...
    case OSSL_CMP_PKISTATUS_revocationNotification:
      ossl_cmp_warn (ctx, "received \"revocationNotification\" - a revocation "
                          "of the cert has occurred");
      break;
 ...
 */
int
OSSL_CMP_certConf_cb (OSSL_CMP_CTX *ctx, X509 *cert, int fail_info,
                      const char **text)
{
  ...
  if (out_trusted != NULL)
    ...
    {
      ossl_cmp_warn (ctx, "could not build approximate chain for newly "
                          "enrolled cert, resorting to received extraCerts");
 ...
 * Regardless of success, caller is responsible for freeing *resp (unless
 * NULL).
 */
static int
cert_response (OSSL_CMP_CTX *ctx, int sleep, int rid, OSSL_CMP_MSG **resp,
               int *checkAfter, ossl_unused int req_type,
...
static int
initial_certreq (OSSL_CMP_CTX *ctx, int req_type, const OSSL_CRMF_MSG *crm,
...
int
OSSL_CMP_try_certreq (OSSL_CMP_CTX *ctx, int req_type,
 ...
 */
X509 *
OSSL_CMP_exec_certreq (OSSL_CMP_CTX *ctx, int req_type,
