 ...
 */
static int
unprotected_exception(const OSSL_CMP_CTX* ctx,
 ...
 */
static int
send_receive_check(OSSL_CMP_CTX* ctx,
                   ...
                   int expected_type)
{
  ...
  *rep = (*transfer_cb)(ctx, req);
  ...
  if (!ossl_cmp_msg_check_update(
        ctx, *rep, unprotected_exception, expected_type))
    return 0;
  ...
  if (bt != OSSL_CMP_PKIBODY_ERROR) {
    ERR_add_error_data(
      3, "message type is '", ossl_cmp_bodytype_to_string(bt), "'");
  } else {
 ...
 */
static int
poll_for_response(OSSL_CMP_CTX* ctx,
                  int sleep,
                  int rid,
                  OSSL_CMP_MSG** rep,
                  int* checkAfter)
  ...
  for (;;) {
    ...
    if (OSSL_CMP_MSG_get_bodytype(prep) == OSSL_CMP_PKIBODY_POLLREP) {
      ...
      if (check_after < 0 ||
        ...
        if (BIO_snprintf(
              str, OSSL_CMP_PKISI_BUFLEN, "value = %jd", check_after) >= 0)
...
static int
save_senderNonce_if_waiting(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* rep, int rid)
{
  /*
 ...
 */
static int
send_receive_also_delayed(OSSL_CMP_CTX* ctx,
                          const OSSL_CMP_MSG* req,
                          OSSL_CMP_MSG** rep,
                          int expected_type)
{
                           ...
                           const char* txt)
{
  ...
  res = send_receive_also_delayed(
    ctx, certConf, &PKIconf, OSSL_CMP_PKIBODY_PKICONF);
 ...
 */
int
OSSL_CMP_certConf_cb(OSSL_CMP_CTX* ctx,
                     X509* cert,
                     int fail_info,
                     const char** text)
{
  ...
  if (out_trusted == NULL) {
    ...
    chain = X509_build_chain(
      cert, ctx->untrusted, out_trusted, 0, ctx->libctx, ctx->propq);
  } else {
 ...
 */
static int
cert_response(OSSL_CMP_CTX* ctx,
              int sleep,
              int rid,
              OSSL_CMP_MSG** resp,
              int* checkAfter,
...
retry:
  ...
  if (fail_info != 0) /* immediately log error before any certConf exchange */
    ossl_cmp_log1(
      ERROR, ctx, "rejecting newly enrolled cert with subject: %s", subj);
  /*
...
static int
initial_certreq(OSSL_CMP_CTX* ctx,
                int req_type,
...
int
OSSL_CMP_try_certreq(OSSL_CMP_CTX* ctx,
                     int req_type,
                     const OSSL_CRMF_MSG* crm,
                     int* checkAfter)
{
  ...
  res = cert_response(
    ctx, 0 /* no sleep */, rid, &rep, checkAfter, req_type, rep_type);
