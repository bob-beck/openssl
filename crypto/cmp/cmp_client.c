...
static int unprotected_exception(const OSSL_CMP_CTX* ctx,
  ...
  if (!ossl_assert(ctx != NULL && rep != NULL)) return -1;

  if (!ctx->unprotectedErrors) return 0;
  ...
  switch (rcvd_type) {
    ...
    default:
      if (IS_CREP(rcvd_type)) {
        ...
        if (sk_OSSL_CMP_CERTRESPONSE_num(crepmsg->response) > 1) return -1;
        if (crep == NULL) return -1;
        if (ossl_cmp_pkisi_get_status(crep->status) ==
            OSSL_CMP_PKISTATUS_rejection)
          msg_type = "CertRepMessage with rejection status";
      }
  }
  if (msg_type == NULL) return 0;
...
static int save_statusInfo(OSSL_CMP_CTX* ctx, OSSL_CMP_PKISI* si) {
  ...
  if (!ossl_assert(ctx != NULL && si != NULL)) return 0;

  ctx->status = ossl_cmp_pkisi_get_status(si);
  if (ctx->status < OSSL_CMP_PKISTATUS_accepted) return 0;
...
static int send_receive_check(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* req,
  ...
  if (transfer_cb == NULL) transfer_cb = OSSL_CMP_MSG_http_perform;
#endif
  *rep = NULL;

  if (ctx->total_timeout != 0 /* not waiting indefinitely */) {
    if (begin_transaction) ctx->end_time = now + ctx->total_timeout;
    if (now >= ctx->end_time) {
  ...
  if (ctx->server != NULL) ossl_cmp_log1(INFO, ctx, "sending %s", req_type_str);
...
static int poll_for_response(OSSL_CMP_CTX* ctx, int sleep, int rid,
  ...
  *rep = NULL;
  for (;;) {
    if ((preq = ossl_cmp_pollReq_new(ctx, rid)) == NULL) goto err;
    ...
    if (OSSL_CMP_MSG_get_bodytype(prep) == OSSL_CMP_PKIBODY_POLLREP) {
      ...
      if (ctx->total_timeout != 0) { /* timeout is not infinite */
        ...
        if (time_left < check_after) check_after = time_left;
      ...
      if (sleep) {
        ...
        if (checkAfter != NULL) *checkAfter = (int)check_after;
...
static int send_receive_also_delayed(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* req,
                                     OSSL_CMP_MSG** rep, int expected_type) {
  if (!send_receive_check(ctx, req, rep, expected_type)) return 0;
...
int ossl_cmp_exchange_error(OSSL_CMP_CTX* ctx, int status, int fail_info,
  ...
  if ((si = OSSL_CMP_STATUSINFO_new(status, fail_info, txt)) == NULL) goto err;
...
static X509* get1_cert_status(OSSL_CMP_CTX* ctx, int bodytype,
  ...
  if (!ossl_assert(ctx != NULL && crep != NULL)) return NULL;

  switch (ossl_cmp_pkisi_get_status(crep->status)) {
    case OSSL_CMP_PKISTATUS_waiting:
      ossl_cmp_err(ctx,
                   "received \"waiting\" status for cert when actually aiming "
                   "to extract cert");
    ...
    case OSSL_CMP_PKISTATUS_revocationWarning:
      ossl_cmp_warn(ctx,
                    "received \"revocationWarning\" - a revocation of the cert "
                    "is imminent");
      break;
    case OSSL_CMP_PKISTATUS_revocationNotification:
      ossl_cmp_warn(ctx,
                    "received \"revocationNotification\" - a revocation of the "
                    "cert has occurred");
      break;
...
int OSSL_CMP_certConf_cb(OSSL_CMP_CTX* ctx, X509* cert, int fail_info,
  ...
  if (out_trusted == NULL) {
    ...
    if (csc == NULL) goto err;
    if (!X509_STORE_CTX_init(csc, out_trusted, cert, ctx->untrusted)) goto err;
    ...
    if (X509_verify_cert(csc) <= 0) goto err;
  ...
  if (out_trusted != NULL) {
    ...
    ossl_cmp_warn(ctx,
                  "could not build approximate chain for newly enrolled cert, "
                  "resorting to received extraCerts");
...
static int cert_response(OSSL_CMP_CTX* ctx, int sleep, int rid,
  ...
  if (!ossl_assert(ctx != NULL)) return 0;
...
retry:
  ...
  if (!ctx->disableConfirm &&
    ...
    if (!ossl_cmp_exchange_certConf(ctx, rid, fail_info, txt)) ret = 0;
  }
...
static int initial_certreq(OSSL_CMP_CTX* ctx, int req_type,
  ...
  if (!ossl_cmp_ctx_set0_newCert(ctx, NULL)) return 0;

  /* also checks if all necessary options are set */
  if ((req = ossl_cmp_certreq_new(ctx, req_type, crm)) == NULL) return 0;
...
int OSSL_CMP_try_certreq(OSSL_CMP_CTX* ctx, int req_type,
  ...
  if (ctx->status != OSSL_CMP_PKISTATUS_waiting) { /* not polling already */
    if (!initial_certreq(ctx, req_type, crm, &rep, rep_type)) goto err;

    if (!save_senderNonce_if_waiting(ctx, rep, rid)) return 0;
  } else {
    if (req_type < 0)
...
int OSSL_CMP_exec_RR_ses(OSSL_CMP_CTX* ctx) {
  ...
  if ((rr = ossl_cmp_rr_new(ctx)) == NULL) goto end;

  ctx->status = OSSL_CMP_PKISTATUS_trans;
  if (!send_receive_also_delayed(ctx, rr, &rp, OSSL_CMP_PKIBODY_RP)) goto end;
  ...
  if (!save_statusInfo(ctx, si)) goto err;
...
STACK_OF(OSSL_CMP_ITAV)* OSSL_CMP_exec_GENM_ses(OSSL_CMP_CTX* ctx) {
  ...
  if ((genm = ossl_cmp_genm_new(ctx)) == NULL) goto err;
  ...
  if (itavs == NULL) itavs = sk_OSSL_CMP_ITAV_new_null();
