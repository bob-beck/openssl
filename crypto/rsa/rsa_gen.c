...
int
RSA_generate_multi_prime_key (RSA *rsa, int bits, int primes, BIGNUM *e_value,
                              BN_GENCB *cb)
{
#ifndef FIPS_MODULE
...
#ifndef FIPS_MODULE
int
ossl_rsa_multiprime_derive (RSA *rsa, int bits, int primes, BIGNUM *e_value,
...
static int
rsa_multiprime_keygen (RSA *rsa, int bits, int primes, BIGNUM *e_value,
                       BN_GENCB *cb)
{
...
static int
rsa_keygen (OSSL_LIB_CTX *libctx, RSA *rsa, int bits, int primes,
            BIGNUM *e_value, BN_GENCB *cb, int pairwise_test)
{
  int ok = 0;
...
/*
 * AS10.35 (and its VEs/TEs) of the FIPS 140-3 standard requires a PCT for
 * every generated key pair. There are 3 options: 1) If the key pair is to be
 * used for key transport (asymmetric cipher), the PCT consists of encrypting a
 * plaintext, verifying that the result (ciphertext) is not equal to the
 * plaintext, decrypting the ciphertext, and verifying that the result is equal
 * to the plaintext. 2) If the key pair is to be used for digital signatures,
 * the PCT consists of computing and verifying a signature. 3) If the key pair
 * is to be used for key agreement, the exact PCT is defined in the applicable
 * standards. For RSA-based schemes, this is defined in SP 800-56Br2
 * (Section 6.4.1.1) as: "The owner shall perform a pair-wise consistency test
 * by verifying that m = (m^e)^d mod n for some integer m satisfying 1 < m < (n
 * - 1)."
 *
 ...
 */
static int
rsa_keygen_pairwise_test (RSA *rsa, OSSL_CALLBACK *cb, void *cbarg)
{
  int ret = 0;
