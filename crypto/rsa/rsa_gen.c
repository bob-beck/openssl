...
static int
rsa_keygen(OSSL_LIB_CTX* libctx,
           RSA* rsa,
           int bits,
           int primes,
           BIGNUM* e_value,
...
RSA_generate_key_ex(RSA* rsa, int bits, BIGNUM* e_value, BN_GENCB* cb)
{
  ...
  return RSA_generate_multi_prime_key(
    rsa, bits, RSA_DEFAULT_PRIME_NUM, e_value, cb);
}

int
RSA_generate_multi_prime_key(RSA* rsa,
                             int bits,
                             int primes,
                             BIGNUM* e_value,
                             BN_GENCB* cb)
                           ...
                           STACK_OF(BIGNUM)* coeffs)
{
  ...
  if (BN_mod_inverse(
        iqmp, sk_BIGNUM_value(factors, 1), sk_BIGNUM_value(factors, 0), ctx) ==
      NULL)
    goto err;
                      ...
                      BN_GENCB* cb)
{
  ...
  if (!ossl_rsa_multiprime_derive(
        rsa, bits, primes, e_value, factors, exps, coeffs))
    goto err;
