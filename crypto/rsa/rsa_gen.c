...
int ossl_rsa_multiprime_derive(RSA* rsa, int bits, int primes, BIGNUM* e_value,
  ...
  if (pplist == NULL) goto err;

  pdlist = sk_BIGNUM_new_null();
  if (pdlist == NULL) goto err;
  ...
  if (BN_copy(r1, rsa->n) == NULL) goto err;
  ...
  for (i = 0; i < sk_BIGNUM_num(factors); i++) {
    switch (i) {
      case 0:
        ...
        BN_set_flags(r2, BN_FLG_CONSTTIME);
        if (BN_mod_inverse(r1, r2, rsa->e, ctx) == NULL) goto err;
        break;
      case 1:
        /* second prime q */
        if (!BN_mul(r1, p, q, ctx)) goto err;
        tmp = BN_dup(r1);
        if (tmp == NULL) goto err;
        if (!sk_BIGNUM_insert(pplist, tmp, sk_BIGNUM_num(pplist))) goto err;
        tmp = NULL;
        break;
      default:
        factor = sk_BIGNUM_value(factors, i);
        /* all other primes */
        if (!BN_mul(r1, r1, factor, ctx)) goto err;
        tmp = BN_dup(r1);
        if (tmp == NULL) goto err;
        if (!sk_BIGNUM_insert(pplist, tmp, sk_BIGNUM_num(pplist))) goto err;
        tmp = NULL;
  ...
  for (i = 2; i < sk_BIGNUM_num(factors); i++) {
    factor = sk_BIGNUM_value(factors, i);
    dval = BN_new();
    if (dval == NULL) goto err;
    BN_set_flags(dval, BN_FLG_CONSTTIME);
    if (!BN_sub(dval, factor, BN_value_one())) goto err;
    if (!BN_mul(r0, r0, dval, ctx)) goto err;
    if (!sk_BIGNUM_insert(pdlist, dval, sk_BIGNUM_num(pdlist))) goto err;
    dval = NULL;
  }
  ...
  dmq1 = BN_secure_new();
  if (dmq1 == NULL) goto err;

  if (!BN_mod(dmp1, rsa->d, r1, ctx)) goto err;
  if (!sk_BIGNUM_insert(exps, dmp1, sk_BIGNUM_num(exps))) goto err;
  dmp1 = NULL;

  if (!BN_mod(dmq1, rsa->d, r2, ctx)) goto err;
  if (!sk_BIGNUM_insert(exps, dmq1, sk_BIGNUM_num(exps))) goto err;
  dmq1 = NULL;

  for (i = 2; i < sk_BIGNUM_num(factors); i++) {
    newpd = sk_BIGNUM_value(pdlist, i - 2);
    newexp = BN_new();
    if (newexp == NULL) goto err;
    if (!BN_mod(newexp, rsa->d, newpd, ctx)) goto err;
    if (!sk_BIGNUM_insert(exps, newexp, sk_BIGNUM_num(exps))) goto err;
    newexp = NULL;
  }

  /* Calculate iqmp and additional coefficients */
  iqmp = BN_new();
  if (iqmp == NULL) goto err;

  if (BN_mod_inverse(iqmp, sk_BIGNUM_value(factors, 1),
                     sk_BIGNUM_value(factors, 0), ctx) == NULL)
    goto err;
  if (!sk_BIGNUM_insert(coeffs, iqmp, sk_BIGNUM_num(coeffs))) goto err;
  iqmp = NULL;

  for (i = 2; i < sk_BIGNUM_num(factors); i++) {
    newpp = sk_BIGNUM_value(pplist, i - 2);
    newcoeff = BN_new();
    if (newcoeff == NULL) goto err;
    if (BN_mod_inverse(newcoeff, newpp, sk_BIGNUM_value(factors, i), ctx) ==
        NULL)
      goto err;
    if (!sk_BIGNUM_insert(coeffs, newcoeff, sk_BIGNUM_num(coeffs))) goto err;
    newcoeff = NULL;
  }
...
static int rsa_multiprime_keygen(RSA* rsa, int bits, int primes,
  ...
  if (exps == NULL) goto err;

  coeffs = sk_BIGNUM_new_null();
  if (coeffs == NULL) goto err;
  ...
  if (!rsa->d && ((rsa->d = BN_secure_new()) == NULL)) goto err;
  BN_set_flags(rsa->d, BN_FLG_CONSTTIME);
  if (!rsa->e && ((rsa->e = BN_new()) == NULL)) goto err;
  if (!rsa->p && ((rsa->p = BN_secure_new()) == NULL)) goto err;
  BN_set_flags(rsa->p, BN_FLG_CONSTTIME);
  if (!rsa->q && ((rsa->q = BN_secure_new()) == NULL)) goto err;
  ...
  if (BN_copy(rsa->e, e_value) == NULL) goto err;
  ...
  for (i = 0; i < primes; i++) {
    ...
    redo:
      ...
      if (!BN_sub(r2, prime, BN_value_one())) goto err;
      ERR_set_mark();
      BN_set_flags(r2, BN_FLG_CONSTTIME);
    ...
    if (i == 1) {
      ...
      if (!BN_mul(r1, rsa->p, rsa->q, ctx)) goto err;
    } else if (i != 0) {
      /* modulus n = p * q * r_3 * r_4 ... */
      if (!BN_mul(r1, rsa->n, prime, ctx)) goto err;
    } else {
      /* i == 0, do nothing */
      if (!BN_GENCB_call(cb, 3, i)) goto err;
      tmp = BN_dup(prime);
      if (tmp == NULL) goto err;
      if (!sk_BIGNUM_insert(factors, tmp, sk_BIGNUM_num(factors))) goto err;
      continue;
    }
     ...
     */
    if (!BN_rshift(r2, r1, bitse - 4)) goto err;
    ...
    if (i > 1 && BN_copy(pinfo->pp, rsa->n) == NULL) goto err;
    if (BN_copy(rsa->n, r1) == NULL) goto err;
    if (!BN_GENCB_call(cb, 3, i)) goto err;
    tmp = BN_dup(prime);
    if (tmp == NULL) goto err;
    if (!sk_BIGNUM_insert(factors, tmp, sk_BIGNUM_num(factors))) goto err;
  ...
  if (BN_cmp(rsa->p, rsa->q) < 0) {
    ...
    if (!sk_BIGNUM_insert(factors, sk_BIGNUM_delete(factors, 0), 1)) goto err;
  }
  ...
  for (i = 2; i < primes; i++) {
    ...
    if (!BN_sub(pinfo->d, pinfo->r, BN_value_one())) goto err;
    if (!BN_mul(r0, r0, pinfo->d, ctx)) goto err;
  }
...
int ossl_rsa_key_pairwise_test(RSA* rsa) {
  ...
  if (res <= 0) ossl_set_error_state(OSSL_SELF_TEST_TYPE_PCT);
  return res;
}
#endif /* FIPS_MODULE */
