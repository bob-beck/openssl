...
static RSA* rsa_new_intern(ENGINE* engine, OSSL_LIB_CTX* libctx) {
  ...
  if (ret->blindings_sa == NULL) goto err;
...
int RSA_security_bits(const RSA* rsa) {
  ...
  if (rsa->version == RSA_ASN1_VERSION_MULTI) {
    ...
    if (ex_primes <= 0 || (ex_primes + 2) > ossl_rsa_multip_cap(bits)) return 0;
  }
#endif
...
int RSA_set0_multi_prime_params(RSA* r, BIGNUM* primes[], BIGNUM* exps[],
  ...
  if (primes == NULL || exps == NULL || coeffs == NULL || pnum == 0) return 0;

  prime_infos = sk_RSA_PRIME_INFO_new_reserve(NULL, pnum);
  if (prime_infos == NULL) return 0;

  if (r->prime_infos != NULL) old = r->prime_infos;
...
int RSA_get0_multi_prime_factors(const RSA* r, const BIGNUM* primes[]) {
  ...
  if ((pnum = RSA_get_multi_prime_extra_count(r)) == 0) return 0;
...
int RSA_get0_multi_prime_crt_params(const RSA* r, const BIGNUM* exps[],
  ...
  if (exps != NULL || coeffs != NULL) {
    ...
    for (i = 0; i < pnum; i++) {
      pinfo = sk_RSA_PRIME_INFO_value(r->prime_infos, i);
      if (exps != NULL) exps[i] = pinfo->d;
      if (coeffs != NULL) coeffs[i] = pinfo->t;
    }
  }
...
int ossl_rsa_set0_all_params(RSA* r, STACK_OF(BIGNUM)* primes,
  ...
  if (primes == NULL || exps == NULL || coeffs == NULL) return 0;
  ...
  if (pnum > 2) {
    ...
    for (i = 2; i < pnum; i++) {
      ...
      if (!ossl_assert(prime != NULL && exp != NULL && coeff != NULL)) goto err;

      /* Using ossl_rsa_multip_info_new() is wasteful, so allocate directly */
      if ((pinfo = OPENSSL_zalloc(sizeof(*pinfo))) == NULL) goto err;
...
int ossl_rsa_check_factors(RSA* r) {
  ...
  if (factors == NULL || exps == NULL || coeffs == NULL) goto done;
  ...
  if (safe_BN_num_bits(RSA_get0_d(r)) > n) goto done;
...
static int int_set_rsa_md_name(EVP_PKEY_CTX* ctx,
  ...
  switch (keytype) {
    case -1:
      ...
      break;
    default:
      if (!EVP_PKEY_CTX_is_a(ctx, evp_pkey_type2name(keytype))) return -1;
      break;
  }
...
int EVP_PKEY_CTX_set_rsa_oaep_md(EVP_PKEY_CTX* ctx, const EVP_MD* md) {
  /* If key type not RSA return error */
  if (!EVP_PKEY_CTX_is_a(ctx, "RSA")) return -1;
...
int EVP_PKEY_CTX_set0_rsa_oaep_label(EVP_PKEY_CTX* ctx, void* label, int llen) {
  ...
  if (label == NULL && llen == 0) plabel = (void*)empty;
...
int EVP_PKEY_CTX_get0_rsa_oaep_label(EVP_PKEY_CTX* ctx, unsigned char** label) {
  ...
  *p++ = OSSL_PARAM_construct_end();
  ...
  if (!EVP_PKEY_CTX_get_params(ctx, rsa_params)) return -1;

  labellen = rsa_params[0].return_size;
  if (labellen > INT_MAX) return -1;
...
int EVP_PKEY_CTX_set_rsa_pss_keygen_saltlen(EVP_PKEY_CTX* ctx, int saltlen) {
  ...
  if (!EVP_PKEY_CTX_is_a(ctx, "RSA-PSS")) return -1;
...
int EVP_PKEY_CTX_set1_rsa_keygen_pubexp(EVP_PKEY_CTX* ctx, BIGNUM* pubexp) {
  ...
  if (evp_pkey_ctx_is_legacy(ctx)) {
    ...
    if (pubexp == NULL) return 0;
  }
  ret = EVP_PKEY_CTX_ctrl(ctx, EVP_PKEY_RSA, EVP_PKEY_OP_KEYGEN,
                          EVP_PKEY_CTRL_RSA_KEYGEN_PUBEXP, 0, pubexp);
  if (evp_pkey_ctx_is_legacy(ctx) && ret <= 0) BN_free(pubexp);
  return ret;
}
