...
static RSA*
rsa_new_intern(ENGINE* engine, OSSL_LIB_CTX* libctx);
 ...
 */
uint16_t
ossl_ifc_ffc_compute_security_bits(int n)
{
  uint64_t x;
  uint32_t lx;
  uint16_t y, cap;
 ...
 */
int
RSA_set0_multi_prime_params(RSA* r,
                            BIGNUM* primes[],
                            BIGNUM* exps[],
                            BIGNUM* coeffs[],
                            int pnum)
...
#ifndef FIPS_MODULE
int
RSA_get_multi_prime_extra_count(const RSA* r)
{
  int pnum;
...
int
RSA_get0_multi_prime_factors(const RSA* r, const BIGNUM* primes[])
{
  int pnum, i;
  RSA_PRIME_INFO* pinfo;
...
#ifndef FIPS_MODULE
int
RSA_get0_multi_prime_crt_params(const RSA* r,
                                const BIGNUM* exps[],
                                const BIGNUM* coeffs[])
{
  int pnum;
                         ...
                         STACK_OF(BIGNUM)* coeffs)
{
  ...
  if (!RSA_set0_factors(
        r, sk_BIGNUM_value(primes, 0), sk_BIGNUM_value(primes, 1)))
    return 0;
  ...
  if (pnum == sk_BIGNUM_num(exps) && pnum == sk_BIGNUM_num(coeffs) + 1) {
    ...
    if (!RSA_set0_crt_params(r,
                             sk_BIGNUM_value(exps, 0),
...
static int
int_set_rsa_md_name(EVP_PKEY_CTX* ctx,
                    /* For checks */
                    int keytype,
                    int optype,
...
/* Helpers to set or get diverse hash algorithm names */
static int
int_get_rsa_md_name(EVP_PKEY_CTX* ctx,
                    /* For checks */
                    int keytype,
                    int optype,
 ...
 */
int
EVP_PKEY_CTX_set_rsa_padding(EVP_PKEY_CTX* ctx, int pad_mode)
{
 ...
 */
int
EVP_PKEY_CTX_get_rsa_padding(EVP_PKEY_CTX* ctx, int* pad_mode)
{
 ...
 */
int
EVP_PKEY_CTX_set_rsa_pss_keygen_md(EVP_PKEY_CTX* ctx, const EVP_MD* md)
{
...
int
EVP_PKEY_CTX_set_rsa_pss_keygen_md_name(EVP_PKEY_CTX* ctx,
                                        const char* mdname,
                                        const char* mdprops)
{
...
int
EVP_PKEY_CTX_set_rsa_oaep_md_name(EVP_PKEY_CTX* ctx,
                                  const char* mdname,
                                  const char* mdprops)
{
  return int_set_rsa_md_name(ctx,
...
int
EVP_PKEY_CTX_get_rsa_oaep_md_name(EVP_PKEY_CTX* ctx,
                                  char* name,
                                  size_t namesize)
{
 ...
 */
int
EVP_PKEY_CTX_get_rsa_oaep_md(EVP_PKEY_CTX* ctx, const EVP_MD** md)
{
  /* If key type not RSA return error */
  if (!EVP_PKEY_CTX_is_a(ctx, "RSA"))
    return -1;
...
int
EVP_PKEY_CTX_set_rsa_mgf1_md_name(EVP_PKEY_CTX* ctx,
                                  const char* mdname,
                                  const char* mdprops)
{
  return int_set_rsa_md_name(ctx,
...
int
EVP_PKEY_CTX_get_rsa_mgf1_md_name(EVP_PKEY_CTX* ctx,
                                  char* name,
                                  size_t namesize)
{
  return int_get_rsa_md_name(ctx,
 ...
 */
int
EVP_PKEY_CTX_set_rsa_pss_keygen_mgf1_md(EVP_PKEY_CTX* ctx, const EVP_MD* md)
{
  return EVP_PKEY_CTX_ctrl(ctx,
                           EVP_PKEY_RSA_PSS,
                           EVP_PKEY_OP_KEYGEN,
...
int
EVP_PKEY_CTX_set_rsa_pss_keygen_mgf1_md_name(EVP_PKEY_CTX* ctx,
                                             const char* mdname)
{
  return int_set_rsa_md_name(ctx,
                             EVP_PKEY_RSA_PSS,
 ...
 */
int
EVP_PKEY_CTX_get_rsa_mgf1_md(EVP_PKEY_CTX* ctx, const EVP_MD** md)
{
  return RSA_pkey_ctx_ctrl(ctx,
                           EVP_PKEY_OP_TYPE_SIG | EVP_PKEY_OP_TYPE_CRYPT,
...
int
EVP_PKEY_CTX_set0_rsa_oaep_label(EVP_PKEY_CTX* ctx, void* label, int llen)
{
  ...
  *p++ = OSSL_PARAM_construct_octet_string(
    OSSL_ASYM_CIPHER_PARAM_OAEP_LABEL, (void*)plabel, (size_t)llen);
  *p++ = OSSL_PARAM_construct_end();
...
int
EVP_PKEY_CTX_get0_rsa_oaep_label(EVP_PKEY_CTX* ctx, unsigned char** label)
{
  OSSL_PARAM rsa_params[2], *p = rsa_params;
  size_t labellen;
  ...
  *p++ = OSSL_PARAM_construct_octet_ptr(
    OSSL_ASYM_CIPHER_PARAM_OAEP_LABEL, (void**)label, 0);
  *p++ = OSSL_PARAM_construct_end();
 ...
 */
int
EVP_PKEY_CTX_set_rsa_pss_saltlen(EVP_PKEY_CTX* ctx, int saltlen)
{
  /*
  ...
  return RSA_pkey_ctx_ctrl(
    ctx, EVP_PKEY_OP_TYPE_SIG, EVP_PKEY_CTRL_RSA_PSS_SALTLEN, saltlen, NULL);
}
 ...
 */
int
EVP_PKEY_CTX_get_rsa_pss_saltlen(EVP_PKEY_CTX* ctx, int* saltlen)
{
  /*
   ...
   */
  return RSA_pkey_ctx_ctrl(
    ctx, EVP_PKEY_OP_TYPE_SIG, EVP_PKEY_CTRL_GET_RSA_PSS_SALTLEN, 0, saltlen);
}

int
EVP_PKEY_CTX_set_rsa_pss_keygen_saltlen(EVP_PKEY_CTX* ctx, int saltlen)
{
...
int
EVP_PKEY_CTX_set_rsa_keygen_bits(EVP_PKEY_CTX* ctx, int bits)
{
  OSSL_PARAM params[2], *p = params;
  size_t bits2 = bits;
...
{
  int ret = RSA_pkey_ctx_ctrl(
    ctx, EVP_PKEY_OP_KEYGEN, EVP_PKEY_CTRL_RSA_KEYGEN_PUBEXP, 0, pubexp);
...
int
EVP_PKEY_CTX_set_rsa_keygen_primes(EVP_PKEY_CTX* ctx, int primes)
{
  OSSL_PARAM params[2], *p = params;
