...
static int
collect_numbers(STACK_OF(BIGNUM)* numbers,
...
ossl_rsa_fromdata(RSA* rsa, const OSSL_PARAM params[], int include_private)
{
  ...
  if (is_private) {
    if (!collect_numbers(
          factors = sk_BIGNUM_new_null(), params, ossl_rsa_mp_factor_names) ||
        !collect_numbers(
          exps = sk_BIGNUM_new_null(), params, ossl_rsa_mp_exp_names) ||
        !collect_numbers(
          coeffs = sk_BIGNUM_new_null(), params, ossl_rsa_mp_coeff_names))
      goto err;
    ...
    if (derive_from_pq && sk_BIGNUM_num(exps) == 0 &&
      ...
      if (sk_BIGNUM_num(factors) == 2) {
        ...
        if (!RSA_set0_factors(
              rsa, sk_BIGNUM_value(factors, 0), sk_BIGNUM_value(factors, 1))) {
         ...
         */
        if (!ossl_rsa_multiprime_derive(rsa,
                                        RSA_bits(rsa),
...
rsa_bn_dup_check(BIGNUM** out, const BIGNUM* f)
{
...
static int
ossl_rsa_sync_to_pss_params_30(RSA* rsa)
{
  ...
  if (rsa != NULL && (legacy_pss = RSA_get0_pss_params(rsa)) != NULL &&
    ...
    if (!ossl_rsa_pss_get_param_unverified(
          legacy_pss, &md, &mgf1md, &saltlen, &trailerField))
      return 0;
