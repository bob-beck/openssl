...
static int collect_numbers(STACK_OF(BIGNUM)* numbers, const OSSL_PARAM params[],
  ...
  for (i = 0; names[i] != NULL; i++) {
    ...
    if (p != NULL) {
      ...
      if (!OSSL_PARAM_get_BN(p, &tmp)) return 0;
...
int ossl_rsa_todata(RSA* rsa, OSSL_PARAM_BLD* bld, OSSL_PARAM params[],
  ...
  if (bld == NULL) ossl_rsa_acvp_test_get_params(rsa, params);
#endif
  ret = 1;
err:
...
int ossl_rsa_pss_params_30_fromdata(RSA_PSS_PARAMS_30* pss_params,
  ...
  if (!*defaults_set && (param_md != NULL || param_mgf != NULL ||
    ...
    if (!ossl_rsa_pss_params_30_set_defaults(pss_params)) return 0;
    *defaults_set = 1;
  }
...
static ossl_inline int rsa_bn_dup_check(BIGNUM** out, const BIGNUM* f) {
  if (f != NULL && (*out = BN_dup(f)) == NULL) return 0;
  return 1;
}
...
RSA* ossl_rsa_dup(const RSA* rsa, int selection) {
  ...
  if (ossl_rsa_is_foreign(rsa)) return NULL;

  if ((dupkey = ossl_rsa_new_with_ctx(rsa->libctx)) == NULL) return NULL;

  /* public key */
  if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
    if (!rsa_bn_dup_check(&dupkey->n, rsa->n)) goto err;
    if (!rsa_bn_dup_check(&dupkey->e, rsa->e)) goto err;
  }

  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
    /* private key */
    if (!rsa_bn_dup_check(&dupkey->d, rsa->d)) goto err;

    /* factors and crt params */
    if (!rsa_bn_dup_check(&dupkey->p, rsa->p)) goto err;
    if (!rsa_bn_dup_check(&dupkey->q, rsa->q)) goto err;
    if (!rsa_bn_dup_check(&dupkey->dmp1, rsa->dmp1)) goto err;
    if (!rsa_bn_dup_check(&dupkey->dmq1, rsa->dmq1)) goto err;
    if (!rsa_bn_dup_check(&dupkey->iqmp, rsa->iqmp)) goto err;
  }
  ...
  if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0 &&
    ...
    if (dupkey->prime_infos == NULL) goto err;
    for (i = 0; i < pnum; i++) {
      const RSA_PRIME_INFO* pinfo = NULL;
      RSA_PRIME_INFO* duppinfo = NULL;

      if ((duppinfo = OPENSSL_zalloc(sizeof(*duppinfo))) == NULL) goto err;
      /* push first so cleanup in error case works */
      (void)sk_RSA_PRIME_INFO_push(dupkey->prime_infos, duppinfo);

      pinfo = sk_RSA_PRIME_INFO_value(rsa->prime_infos, i);
      if (!rsa_bn_dup_check(&duppinfo->r, pinfo->r)) goto err;
      if (!rsa_bn_dup_check(&duppinfo->d, pinfo->d)) goto err;
      if (!rsa_bn_dup_check(&duppinfo->t, pinfo->t)) goto err;
    }
    if (!ossl_rsa_multip_calc_product(dupkey)) goto err;
  }
  ...
  if (rsa->pss != NULL) {
    ...
    if (rsa->pss->maskGenAlgorithm != NULL &&
      ...
      if (dupkey->pss->maskHash == NULL) goto err;
    }
  }
...
int ossl_rsa_param_decode(RSA* rsa, const X509_ALGOR* alg) {
  ...
  if (OBJ_obj2nid(algoid) != EVP_PKEY_RSA_PSS) return 1;
  if (algptype == V_ASN1_UNDEF) return 1;
...
RSA* ossl_rsa_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO* p8inf,
  ...
  if (!PKCS8_pkey_get0(NULL, &p, &pklen, &alg, p8inf)) return 0;
