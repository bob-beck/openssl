...
static int
collect_numbers (STACK_OF (BIGNUM) *numbers, const OSSL_PARAM params[],
                 const char *names[])
{
      ...
      if (sk_BIGNUM_num (factors) != 0 || sk_BIGNUM_num (exps) != 0
          || sk_BIGNUM_num (coeffs) != 0)
        {
          ERR_raise_data (ERR_LIB_RSA, ERR_R_INTERNAL_ERROR,
                          "There are %d, %d, %d elements left on our factors, "
                          "exps, coeffs stacks\n",
...
int
ossl_rsa_todata (RSA *rsa, OSSL_PARAM_BLD *bld, OSSL_PARAM params[],
                 int include_private)
{
  int ret = 0;
...
ossl_rsa_pss_params_30_fromdata (RSA_PSS_PARAMS_30 *pss_params,
                                 int *defaults_set, const OSSL_PARAM params[],
                                 OSSL_LIB_CTX *libctx)
{
