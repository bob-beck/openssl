...
rsa_pub_encode(X509_PUBKEY* pk, const EVP_PKEY* pkey)
{
  ...
  if (X509_PUBKEY_set0_param(
        pk, OBJ_nid2obj(pkey->ameth->pkey_id), strtype, str, penc, penclen))
    return 1;
...
static int
rsa_pub_decode(EVP_PKEY* pkey, const X509_PUBKEY* pubkey)
{
...
rsa_priv_encode(PKCS8_PRIV_KEY_INFO* p8, const EVP_PKEY* pkey)
{
  ...
  if (!PKCS8_pkey_set0(
        p8, OBJ_nid2obj(pkey->ameth->pkey_id), 0, strtype, str, rk, rklen)) {
    ERR_raise(ERR_LIB_RSA, ERR_R_ASN1_LIB);
    ASN1_STRING_free(str);
  ...
  switch (op) {
    case ASN1_PKEY_CTRL_DEFAULT_MD_NID:
      if (pkey->pkey.rsa->pss != NULL) {
        if (!ossl_rsa_pss_get_param(
              pkey->pkey.rsa->pss, &md, &mgf1md, &min_saltlen)) {
...
RSA_PSS_PARAMS*
ossl_rsa_pss_params_create(const EVP_MD* sigmd,
                           const EVP_MD* mgf1md,
                           int saltlen)
{
...
static int
rsa_pss_verify_param(const EVP_MD** pmd,
...
int
ossl_rsa_pss_get_param(const RSA_PSS_PARAMS* pss,
                       const EVP_MD** pmd,
                       const EVP_MD** pmgf1md,
                       int* psaltlen)
{
  ...
  return ossl_rsa_pss_get_param_unverified(
           pss, pmd, pmgf1md, psaltlen, &trailerField) &&
...
static int
rsa_item_verify(EVP_MD_CTX* ctx,
                const ASN1_ITEM* it,
                const void* asn,
                const X509_ALGOR* sigalg,
  ...
  if (pad_mode == RSA_PKCS1_PSS_PADDING) {
    ...
    if (evp_pkey_ctx_is_legacy(pkctx)) {
      ...
      if (alg2 != NULL) {
        ...
        if (!X509_ALGOR_set0(
              alg2, OBJ_nid2obj(EVP_PKEY_RSA_PSS), V_ASN1_SEQUENCE, os2)) {
          ASN1_STRING_free(os1);
      ...
      if (!X509_ALGOR_set0(
            alg1, OBJ_nid2obj(EVP_PKEY_RSA_PSS), V_ASN1_SEQUENCE, os1)) {
        ASN1_STRING_free(os1);
        return 0;
      }
      return 3;
...
static int
rsa_sig_info_set(X509_SIG_INFO* siginf,
                 const X509_ALGOR* sigalg,
                 const ASN1_STRING* sig)
{
  int rv = 0;
...
static size_t
rsa_pkey_dirty_cnt(const EVP_PKEY* pkey)
{
 ...
 */
static int
rsa_int_export_to(const EVP_PKEY* from,
                  int rsa_type,
                  void* to_keydata,
                  ...
                  const char* propq)
{
  ...
  if (rsa->pss != NULL) {
    ...
    if (!ossl_rsa_pss_get_param_unverified(
          rsa->pss, &md, &mgf1md, &saltlen, &trailerfield))
      goto err;
...
static int
rsa_int_import_from(const OSSL_PARAM params[], void* vpctx, int rsa_type)
{
  ...
  if (!ossl_rsa_pss_params_30_fromdata(
        &rsa_pss_params, &pss_defaults_set, params, pctx->libctx))
    goto err;
...
static int
rsa_pkey_export_to(const EVP_PKEY* from,
                   void* to_keydata,
                   OSSL_FUNC_keymgmt_import_fn* importer,
                   OSSL_LIB_CTX* libctx,
                   const char* propq)
{
  return rsa_int_export_to(
    from, RSA_FLAG_TYPE_RSA, to_keydata, importer, libctx, propq);
}

static int
rsa_pss_pkey_export_to(const EVP_PKEY* from,
                       void* to_keydata,
                       OSSL_FUNC_keymgmt_import_fn* importer,
                       OSSL_LIB_CTX* libctx,
                       const char* propq)
{
  return rsa_int_export_to(
    from, RSA_FLAG_TYPE_RSASSAPSS, to_keydata, importer, libctx, propq);
}

static int
rsa_pkey_import_from(const OSSL_PARAM params[], void* vpctx)
{
  return rsa_int_import_from(params, vpctx, RSA_FLAG_TYPE_RSA);
}

static int
rsa_pss_pkey_import_from(const OSSL_PARAM params[], void* vpctx)
{
  return rsa_int_import_from(params, vpctx, RSA_FLAG_TYPE_RSASSAPSS);
}

static int
rsa_pkey_copy(EVP_PKEY* to, EVP_PKEY* from)
{
  RSA* rsa = from->pkey.rsa;
  RSA* dupkey = NULL;
  int ret;
