...
static int rsa_pub_encode(X509_PUBKEY* pk, const EVP_PKEY* pkey) {
  ...
  if (!rsa_param_encode(pkey, &str, &strtype)) return 0;
...
static int rsa_pub_decode(EVP_PKEY* pkey, const X509_PUBKEY* pubkey) {
  ...
  if (!X509_PUBKEY_get0_param(NULL, &p, &pklen, &alg, pubkey)) return 0;
...
static int old_rsa_priv_decode(EVP_PKEY* pkey, const unsigned char** pder,
  ...
  if ((rsa = d2i_RSAPrivateKey(NULL, pder, derlen)) == NULL) return 0;
...
static int rsa_pss_param_print(BIO* bp, int pss_key, RSA_PSS_PARAMS* pss,
  ...
  if (!BIO_indent(bp, indent, 128)) goto err;
  if (pss_key) {
    if (pss == NULL) {
      if (BIO_puts(bp, "No PSS parameter restrictions\n") <= 0) return 0;
      return 1;
    } else {
      if (BIO_puts(bp, "PSS parameter restrictions:") <= 0) return 0;
    }
  } else if (pss == NULL) {
  ...
  if (pss_key) indent += 2;
  if (!BIO_indent(bp, indent, 128)) goto err;
  if (BIO_puts(bp, "Hash Algorithm: ") <= 0) goto err;

  if (pss->hashAlgorithm) {
    if (i2a_ASN1_OBJECT(bp, pss->hashAlgorithm->algorithm) <= 0) goto err;
  ...
  if (BIO_puts(bp, "Mask Algorithm: ") <= 0) goto err;
  if (pss->maskGenAlgorithm) {
    if (i2a_ASN1_OBJECT(bp, pss->maskGenAlgorithm->algorithm) <= 0) goto err;
    if (BIO_puts(bp, " with ") <= 0) goto err;
    maskHash = ossl_x509_algor_mgf1_decode(pss->maskGenAlgorithm);
    if (maskHash != NULL) {
      if (i2a_ASN1_OBJECT(bp, maskHash->algorithm) <= 0) goto err;
  ...
  if (BIO_printf(bp, "%s Salt Length: 0x", pss_key ? "Minimum" : "") <= 0)
    goto err;
  if (pss->saltLength) {
    if (i2a_ASN1_INTEGER(bp, pss->saltLength) <= 0) goto err;
  ...
  if (!BIO_indent(bp, indent, 128)) goto err;
  if (BIO_puts(bp, "Trailer Field: 0x") <= 0) goto err;
  if (pss->trailerField) {
    if (i2a_ASN1_INTEGER(bp, pss->trailerField) <= 0) goto err;
...
static int pkey_rsa_print(BIO* bp, const EVP_PKEY* pkey, int off, int priv) {
  ...
  if (x->n != NULL) mod_len = BN_num_bits(x->n);
  ...
  if (!ASN1_bn_print(bp, str, x->n, NULL, off)) goto err;
  if (!ASN1_bn_print(bp, s, x->e, NULL, off)) goto err;
  if (priv) {
    int i;

    if (!ASN1_bn_print(bp, "privateExponent:", x->d, NULL, off)) goto err;
    if (!ASN1_bn_print(bp, "prime1:", x->p, NULL, off)) goto err;
    if (!ASN1_bn_print(bp, "prime2:", x->q, NULL, off)) goto err;
    if (!ASN1_bn_print(bp, "exponent1:", x->dmp1, NULL, off)) goto err;
    if (!ASN1_bn_print(bp, "exponent2:", x->dmq1, NULL, off)) goto err;
    if (!ASN1_bn_print(bp, "coefficient:", x->iqmp, NULL, off)) goto err;
    for (i = 0; i < sk_RSA_PRIME_INFO_num(x->prime_infos); i++) {
      ...
      for (j = 0; j < 3; j++) {
        ...
        switch (j) {
          case 0:
            if (BIO_printf(bp, "prime%d:", i + 3) <= 0) goto err;
            bn = pinfo->r;
            break;
          case 1:
            if (BIO_printf(bp, "exponent%d:", i + 3) <= 0) goto err;
            bn = pinfo->d;
            break;
          case 2:
            if (BIO_printf(bp, "coefficient%d:", i + 3) <= 0) goto err;
            bn = pinfo->t;
            break;
          default:
            break;
        }
        if (!ASN1_bn_print(bp, "", bn, NULL, off)) goto err;
      }
    }
  }
  if (pkey_is_pss(pkey) && !rsa_pss_param_print(bp, 1, x->pss, off)) goto err;
  ret = 1;
err:
  return ret;
}
...
static int rsa_sig_print(BIO* bp, const X509_ALGOR* sigalg,
  ...
  if (sig) return X509_signature_dump(bp, sig, indent);
  return 1;
}
...
static RSA_PSS_PARAMS* rsa_ctx_to_pss(EVP_PKEY_CTX* pkctx) {
  ...
  if (EVP_PKEY_CTX_get_signature_md(pkctx, &sigmd) <= 0) return NULL;
  md_size = EVP_MD_get_size(sigmd);
  if (md_size <= 0) return NULL;
  if (EVP_PKEY_CTX_get_rsa_mgf1_md(pkctx, &mgf1md) <= 0) return NULL;
  if (EVP_PKEY_CTX_get_rsa_pss_saltlen(pkctx, &saltlen) <= 0) return NULL;
  ...
  if (saltlen == RSA_PSS_SALTLEN_MAX || saltlen == RSA_PSS_SALTLEN_AUTO) {
    ...
    if ((EVP_PKEY_get_bits(pk) & 0x7) == 1) saltlen--;
    if (saltlen < 0) return NULL;
    if (saltlenMax >= 0 && saltlen > saltlenMax) saltlen = saltlenMax;
  }
...
RSA_PSS_PARAMS* ossl_rsa_pss_params_create(const EVP_MD* sigmd,
  ...
  if (pss == NULL) goto err;
  if (saltlen != 20) {
    pss->saltLength = ASN1_INTEGER_new();
    if (pss->saltLength == NULL) goto err;
    if (!ASN1_INTEGER_set(pss->saltLength, saltlen)) goto err;
  }
  if (!ossl_x509_algor_new_from_md(&pss->hashAlgorithm, sigmd)) goto err;
  if (mgf1md == NULL) mgf1md = sigmd;
  if (!ossl_x509_algor_md_to_mgf1(&pss->maskGenAlgorithm, mgf1md)) goto err;
  if (!ossl_x509_algor_new_from_md(&pss->maskHash, mgf1md)) goto err;
  return pss;
err:
  RSA_PSS_PARAMS_free(pss);
  return NULL;
...
int ossl_rsa_pss_to_ctx(EVP_MD_CTX* ctx, EVP_PKEY_CTX* pkctx,
  ...
  if (pkey) {
    if (!EVP_DigestVerifyInit(ctx, &pkctx, md, NULL, pkey)) goto err;
  } else {
    const EVP_MD* checkmd;
    if (EVP_PKEY_CTX_get_signature_md(pkctx, &checkmd) <= 0) goto err;
  ...
  if (EVP_PKEY_CTX_set_rsa_padding(pkctx, RSA_PKCS1_PSS_PADDING) <= 0) goto err;

  if (EVP_PKEY_CTX_set_rsa_pss_saltlen(pkctx, saltlen) <= 0) goto err;

  if (EVP_PKEY_CTX_set_rsa_mgf1_md(pkctx, mgf1md) <= 0) goto err;
  /* Carry on */
  rv = 1;
...
static int rsa_item_sign(EVP_MD_CTX* ctx, const ASN1_ITEM* it, const void* asn,
  ...
  if (EVP_PKEY_CTX_get_rsa_padding(pkctx, &pad_mode) <= 0) return 0;
  if (pad_mode == RSA_PKCS1_PADDING) return 2;
  if (pad_mode == RSA_PKCS1_PSS_PADDING) {
    ...
    if (EVP_PKEY_CTX_get_params(pkctx, params) <= 0) return 0;
    if ((aid_len = params[0].return_size) == 0) return 0;

    if (alg1 != NULL) {
      const unsigned char* pp = aid;

      if (d2i_X509_ALGOR(&alg1, &pp, (long)aid_len) == NULL) return 0;
    }
    if (alg2 != NULL) {
      const unsigned char* pp = aid;

      if (d2i_X509_ALGOR(&alg2, &pp, (long)aid_len) == NULL) return 0;
    }
...
static int rsa_sig_info_set(X509_SIG_INFO* siginf, const X509_ALGOR* sigalg,
  ...
  /* Sanity check: make sure it is PSS */
  if (OBJ_obj2nid(sigalg->algorithm) != EVP_PKEY_RSA_PSS) return 0;
  /* Decode PSS parameters */
  pss = ossl_rsa_pss_decode(sigalg);
  if (!ossl_rsa_pss_get_param(pss, &md, &mgf1md, &saltlen)) goto err;
  md_size = EVP_MD_get_size(md);
  if (md_size <= 0) goto err;
...
static int rsa_int_export_to(const EVP_PKEY* from, int rsa_type,
  ...
  if (RSA_get0_n(rsa) == NULL || RSA_get0_e(rsa) == NULL) goto err;

  if (!ossl_rsa_todata(rsa, tmpl, NULL, 1)) goto err;

  selection |= OSSL_KEYMGMT_SELECT_PUBLIC_KEY;
  if (RSA_get0_d(rsa) != NULL) selection |= OSSL_KEYMGMT_SELECT_PRIVATE_KEY;
  ...
  if ((params = OSSL_PARAM_BLD_to_param(tmpl)) == NULL) goto err;
...
static int rsa_int_import_from(const OSSL_PARAM params[], void* vpctx,
  ...
  switch (rsa_type) {
    case RSA_FLAG_TYPE_RSA:
       ...
       */
      if (!ossl_rsa_pss_params_30_is_unrestricted(&rsa_pss_params)) goto err;
      break;
  ...
  if (!ossl_rsa_fromdata(rsa, params, 1)) goto err;
