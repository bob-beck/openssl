...
static int rsa_pss_param_print(BIO* bp,
                               int pss_key,
                               RSA_PSS_PARAMS* pss,
                               int indent) {
  int rv = 0;
...
static int rsa_sig_print(BIO* bp,
...
int ossl_rsa_pss_to_ctx(EVP_MD_CTX* ctx,
                        EVP_PKEY_CTX* pkctx,
                        const X509_ALGOR* sigalg,
                        EVP_PKEY* pkey) {
  int rv = -1;
...
static int rsa_pss_verify_param(const EVP_MD** pmd,
...
int ossl_rsa_pss_get_param(const RSA_PSS_PARAMS* pss,
                           const EVP_MD** pmd,
                           const EVP_MD** pmgf1md,
                           int* psaltlen) {
  /*
...
static int rsa_sig_info_set(X509_SIG_INFO* siginf,
                            const X509_ALGOR* sigalg,
                            const ASN1_STRING* sig) {
  int rv = 0;
...
static int rsa_int_import_from(const OSSL_PARAM params[],
                               void* vpctx,
                               int rsa_type) {
