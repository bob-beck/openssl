...
static int rsa_ossl_public_encrypt(int flen, const unsigned char* from,
  ...
  if ((ctx = BN_CTX_new_ex(rsa->libctx)) == NULL) goto err;
  BN_CTX_start(ctx);
  f = BN_CTX_get(ctx);
  ret = BN_CTX_get(ctx);
  num = BN_num_bytes(rsa->n);
  buf = OPENSSL_malloc(num);
  if (ret == NULL || buf == NULL) goto err;
...
void* ossl_rsa_alloc_blinding(void) { return ossl_sa_BN_BLINDING_new(); }
...
static BN_BLINDING* rsa_get_blinding(RSA* rsa, BN_CTX* ctx) {
  ...
  if (!CRYPTO_THREAD_read_lock(rsa->lock)) return NULL;
...
static int rsa_ossl_private_encrypt(int flen, const unsigned char* from,
  ...
  if (!(rsa->flags & RSA_FLAG_NO_BLINDING)) {
    ...
    if (!rsa_blinding_convert(blinding, f, ctx)) goto err;
  }
  ...
  if (blinding)
    if (!rsa_blinding_invert(blinding, ret, ctx)) goto err;

  if (padding == RSA_X931_PADDING) {
    if (!BN_sub(f, rsa->n, ret)) goto err;
    if (BN_cmp(ret, f) > 0)
      res = f;
    else
      res = ret;
...
static int rsa_ossl_private_decrypt(int flen, const unsigned char* from,
  ...
  if (padding == RSA_PKCS1_PADDING) {
    if (derive_kdk(flen, from, rsa, buf, num, kdk) == 0) goto err;
  }
  ...
  switch (padding) {
    case RSA_PKCS1_NO_IMPLICIT_REJECT_PADDING:
      ...
      break;
    case RSA_PKCS1_PADDING:
      r = ossl_rsa_padding_check_PKCS1_type_2(rsa->libctx, to, num, buf, j, num,
                                              kdk);
      break;
    case RSA_PKCS1_OAEP_PADDING:
...
static int rsa_ossl_public_decrypt(int flen, const unsigned char* from,
  ...
  if (r < 0) ERR_raise(ERR_LIB_RSA, RSA_R_PADDING_CHECK_FAILED);
