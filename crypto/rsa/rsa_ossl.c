...
static int
rsa_ossl_public_encrypt(int flen,
                        const unsigned char* from,
                        unsigned char* to,
                        RSA* rsa,
                        int padding);
static int
rsa_ossl_private_encrypt(int flen,
                         const unsigned char* from,
                         unsigned char* to,
                         RSA* rsa,
                         int padding);
static int
rsa_ossl_public_decrypt(int flen,
                        const unsigned char* from,
                        unsigned char* to,
                        RSA* rsa,
                        int padding);
static int
rsa_ossl_private_decrypt(int flen,
                         const unsigned char* from,
                         unsigned char* to,
                         RSA* rsa,
                         int padding);
static int
rsa_ossl_mod_exp(BIGNUM* r0, const BIGNUM* i, RSA* rsa, BN_CTX* ctx);
static int
rsa_ossl_init(RSA* rsa);
static int
rsa_ossl_finish(RSA* rsa);
#ifdef S390X_MOD_EXP
static int
rsa_ossl_s390x_mod_exp(BIGNUM* r0, const BIGNUM* i, RSA* rsa, BN_CTX* ctx);
...
static void
free_bn_blinding(ossl_uintmax_t idx, BN_BLINDING* b, void* arg)
{
  BN_BLINDING_free(b);
}
...
static BN_BLINDING*
ossl_rsa_get_thread_bn_blinding(RSA* rsa)
{
  SPARSE_ARRAY_OF(BN_BLINDING)* blindings = rsa->blindings_sa;
...
static int
ossl_rsa_set_thread_bn_blinding(RSA* rsa, BN_BLINDING* b)
{
  SPARSE_ARRAY_OF(BN_BLINDING)* blindings = rsa->blindings_sa;
  uintptr_t tid = (uintptr_t)CRYPTO_THREAD_get_current_id();
...
static BN_BLINDING*
rsa_get_blinding(RSA* rsa, BN_CTX* ctx)
{
  BN_BLINDING* ret;
...
static int
rsa_blinding_convert(BN_BLINDING* b, BIGNUM* f, BN_CTX* ctx)
{
  /*
...
static int
rsa_blinding_invert(BN_BLINDING* b, BIGNUM* f, BN_CTX* ctx)
{
  /*
  ...
  switch (padding) {
    ...
    case RSA_PKCS1_PADDING:
      r = ossl_rsa_padding_check_PKCS1_type_2(
        rsa->libctx, to, num, buf, j, num, kdk);
      break;
    case RSA_PKCS1_OAEP_PADDING:
...
rsa_ossl_mod_exp(BIGNUM* r0, const BIGNUM* I, RSA* rsa, BN_CTX* ctx)
{
  ...
  if (rsa->flags & RSA_FLAG_CACHE_PRIVATE) {
    ...
    if (!(BN_with_flags(factor, rsa->p, BN_FLG_CONSTTIME),
          BN_MONT_CTX_set_locked(
            &rsa->_method_mod_p, rsa->lock, factor, ctx)) ||
        !(BN_with_flags(factor, rsa->q, BN_FLG_CONSTTIME),
          BN_MONT_CTX_set_locked(
            &rsa->_method_mod_q, rsa->lock, factor, ctx))) {
      BN_free(factor);
      goto err;
    }
#ifndef FIPS_MODULE
  ...
  if (smooth) {
    ...
    if (/* m1 = I moq q */
         ...
         *    r1 = r1^dmp1 mod p
         */
        || !BN_mod_exp_mont_consttime_x2(m1,
                                         m1,
                                         rsa->dmq1,
                                         rsa->q,
                                         rsa->_method_mod_q,
  ...
  /* compute I mod q */
  {
...

    {
      ...
      if (!rsa->meth->bn_mod_exp(
            m1, r1, dmq1, rsa->q, ctx, rsa->_method_mod_q)) {
        BN_free(c);
        BN_free(dmq1);
        goto err;
      }
