...
static int
pkey_rsa_copy(EVP_PKEY_CTX* dst, const EVP_PKEY_CTX* src)
{
...
static int
pkey_rsa_sign(EVP_PKEY_CTX* ctx,
              unsigned char* sig,
              size_t* siglen,
  ...
  if (rctx->md) {
    ...
    if (EVP_MD_get_type(rctx->md) == NID_mdc2) {
      ...
      ret = RSA_private_encrypt(
        (int)(tbslen + 1), rctx->tbuf, sig, rsa, RSA_X931_PADDING);
    } else if (rctx->pad_mode == RSA_PKCS1_PADDING) {
      unsigned int sltmp;
      ret = RSA_sign(
        EVP_MD_get_type(rctx->md), tbs, (unsigned int)tbslen, sig, &sltmp, rsa);
      if (ret <= 0)
        return ret;
      ret = sltmp;
      ...
      if (!RSA_padding_add_PKCS1_PSS_mgf1(
            rsa, rctx->tbuf, tbs, rctx->md, rctx->mgf1md, rctx->saltlen))
        return -1;
      ret = RSA_private_encrypt(
        RSA_size(rsa), rctx->tbuf, sig, rsa, RSA_NO_PADDING);
    } else {
      return -1;
    }
  } else {
...
static int
pkey_rsa_verifyrecover(EVP_PKEY_CTX* ctx,
                       unsigned char* rout,
                       size_t* routlen,
  ...
  if (rctx->md) {
    if (rctx->pad_mode == RSA_X931_PADDING) {
      if (!setup_tbuf(rctx, ctx))
        return -1;
      ret =
        RSA_public_decrypt((int)siglen, sig, rctx->tbuf, rsa, RSA_X931_PADDING);
      if (ret <= 0)
        return 0;
      ret--;
      ...
      size_t sltmp;
      ret = ossl_rsa_verify(
        EVP_MD_get_type(rctx->md), NULL, 0, rout, &sltmp, sig, siglen, rsa);
      if (ret <= 0)
        return 0;
      ret = (int)sltmp;
    } else {
                ...
                size_t tbslen)
{
  ...
  if (rctx->md) {
    if (rctx->pad_mode == RSA_PKCS1_PADDING)
      return RSA_verify(EVP_MD_get_type(rctx->md),
                        tbs,
                        (unsigned int)tbslen,
                        sig,
                        (unsigned int)siglen,
    ...
    if (rctx->pad_mode == RSA_X931_PADDING) {
      ...
      if (ret <= 0)
        return 0;
      ret = RSA_verify_PKCS1_PSS_mgf1(
        rsa, tbs, rctx->md, rctx->mgf1md, rctx->tbuf, rctx->saltlen);
      if (ret <= 0)
        return 0;
      return 1;
    } else {
...
static int
pkey_rsa_encrypt(EVP_PKEY_CTX* ctx,
                 unsigned char* out,
                 size_t* outlen,
...
static int
pkey_rsa_decrypt(EVP_PKEY_CTX* ctx,
                 unsigned char* out,
                 size_t* outlen,
                 const unsigned char* in,
                 size_t inlen)
...
pkey_rsa_ctrl_str(EVP_PKEY_CTX* ctx, const char* type, const char* value)
{
  ...
  if (pkey_ctx_is_pss(ctx)) {
    ...
    if (strcmp(type, "rsa_pss_keygen_mgf1_md") == 0)
      return EVP_PKEY_CTX_md(
        ctx, EVP_PKEY_OP_KEYGEN, EVP_PKEY_CTRL_RSA_MGF1_MD, value);
  ...
  if (strcmp(type, "rsa_oaep_md") == 0)
    return EVP_PKEY_CTX_md(
      ctx, EVP_PKEY_OP_TYPE_CRYPT, EVP_PKEY_CTRL_RSA_OAEP_MD, value);
...
pkey_rsa_keygen(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey)
{
  ...
  ret = RSA_generate_multi_prime_key(
    rsa, rctx->nbits, rctx->primes, rctx->pub_exp, pcb);
  BN_GENCB_free(pcb);
