...
static int pkey_rsa_copy(EVP_PKEY_CTX* dst, const EVP_PKEY_CTX* src) {
  ...
  if (!pkey_rsa_init(dst)) return 0;
  sctx = src->data;
  dctx = dst->data;
  dctx->nbits = sctx->nbits;
  if (sctx->pub_exp) {
    dctx->pub_exp = BN_dup(sctx->pub_exp);
    if (!dctx->pub_exp) return 0;
  }
...
static int pkey_rsa_sign(EVP_PKEY_CTX* ctx, unsigned char* sig, size_t* siglen,
  ...
  if (rctx->md) {
    ...
    if (EVP_MD_get_type(rctx->md) == NID_mdc2) {
      unsigned int sltmp;
      if (rctx->pad_mode != RSA_PKCS1_PADDING) return -1;
      ...
      ret = RSA_private_encrypt((int)(tbslen + 1), rctx->tbuf, sig, rsa,
                                RSA_X931_PADDING);
    } else if (rctx->pad_mode == RSA_PKCS1_PADDING) {
      unsigned int sltmp;
      ret = RSA_sign(EVP_MD_get_type(rctx->md), tbs, (unsigned int)tbslen, sig,
                     &sltmp, rsa);
      if (ret <= 0) return ret;
      ret = sltmp;
    } else if (rctx->pad_mode == RSA_PKCS1_PSS_PADDING) {
      if (!setup_tbuf(rctx, ctx)) return -1;
...
static int pkey_rsa_verifyrecover(EVP_PKEY_CTX* ctx, unsigned char* rout,
  ...
  if (rctx->md) {
    if (rctx->pad_mode == RSA_X931_PADDING) {
      ...
      if (rout) memcpy(rout, rctx->tbuf, ret);
    } else if (rctx->pad_mode == RSA_PKCS1_PADDING) {
      size_t sltmp;
...
static int pkey_rsa_verify(EVP_PKEY_CTX* ctx, const unsigned char* sig,
  ...
  if (rctx->md) {
    ...
    if (rctx->pad_mode == RSA_X931_PADDING) {
      if (pkey_rsa_verifyrecover(ctx, NULL, &rslen, sig, siglen) <= 0) return 0;
    } else if (rctx->pad_mode == RSA_PKCS1_PSS_PADDING) {
      int ret;
      if (!setup_tbuf(rctx, ctx)) return -1;
      ret =
          RSA_public_decrypt((int)siglen, sig, rctx->tbuf, rsa, RSA_NO_PADDING);
      if (ret <= 0) return 0;
    ...
    if (!setup_tbuf(rctx, ctx)) return -1;
    rslen =
        RSA_public_decrypt((int)siglen, sig, rctx->tbuf, rsa, rctx->pad_mode);
    if (rslen <= 0) return 0;
  }
  ...
  switch (type) {
    case EVP_PKEY_CTRL_RSA_PADDING:
      if ((p1 >= RSA_PKCS1_PADDING) && (p1 <= RSA_PKCS1_PSS_PADDING)) {
        if (!check_padding_md(rctx->md, p1)) return 0;
        if (p1 == RSA_PKCS1_PSS_PADDING) {
          if (!(ctx->operation & (EVP_PKEY_OP_SIGN | EVP_PKEY_OP_VERIFY)))
            goto bad_pad;
          if (!rctx->md) rctx->md = EVP_sha1();
        } else if (pkey_ctx_is_pss(ctx)) {
          goto bad_pad;
        }
        if (p1 == RSA_PKCS1_OAEP_PADDING) {
          if (!(ctx->operation & EVP_PKEY_OP_TYPE_CRYPT)) goto bad_pad;
          if (!rctx->md) rctx->md = EVP_sha1();
        }
        rctx->pad_mode = p1;
        return 1;
    ...
    case EVP_PKEY_CTRL_GET_RSA_PSS_SALTLEN:
      ...
      if (type == EVP_PKEY_CTRL_GET_RSA_PSS_SALTLEN) {
        *(int*)p2 = rctx->saltlen;
        ...
        if (p1 < RSA_PSS_SALTLEN_MAX) return -2;
    ...
    case EVP_PKEY_CTRL_MD:
      if (!check_padding_md(p2, rctx->pad_mode)) return 0;
      if (rsa_pss_restricted(rctx)) {
        if (EVP_MD_get_type(rctx->md) == EVP_MD_get_type(p2)) return 1;
    ...
    case EVP_PKEY_CTRL_GET_RSA_MGF1_MD:
      ...
      if (type == EVP_PKEY_CTRL_GET_RSA_MGF1_MD) {
        ...
        if (rsa_pss_restricted(rctx)) {
          if (EVP_MD_get_type(rctx->mgf1md) == EVP_MD_get_type(p2)) return 1;
...
#endif
      if (!pkey_ctx_is_pss(ctx)) return 1;
    /* fall through */
...
static int pkey_rsa_keygen(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey) {
  ...
  if (rctx->pub_exp == NULL) {
    rctx->pub_exp = BN_new();
    if (rctx->pub_exp == NULL || !BN_set_word(rctx->pub_exp, RSA_F4)) return 0;
  }
  rsa = RSA_new();
...
static int pkey_pss_init(EVP_PKEY_CTX* ctx) {
  ...
  /* Should never happen */
  if (!pkey_ctx_is_pss(ctx)) return 0;
  rsa = EVP_PKEY_get0_RSA(ctx->pkey);
  /* If no restrictions just return */
  if (rsa->pss == NULL) return 1;
  /* Get and check parameters */
  if (!ossl_rsa_pss_get_param(rsa->pss, &md, &mgf1md, &min_saltlen)) return 0;
  ...
  if ((RSA_bits(rsa) & 0x7) == 1) max_saltlen--;
