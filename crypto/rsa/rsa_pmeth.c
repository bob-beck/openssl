      ...
      if (EVP_MD_get_type (rctx->md) == NID_mdc2)
        {
          ...
          if (rctx->pad_mode != RSA_PKCS1_PADDING)
            return -1;
          ret = RSA_sign_ASN1_OCTET_STRING (0, tbs, (int)tbslen, sig, &sltmp,
                                            rsa);
...
static int
pkey_rsa_verifyrecover (EVP_PKEY_CTX *ctx, unsigned char *rout,
  ...
  if (rctx->md)
    {
      if (rctx->pad_mode == RSA_PKCS1_PADDING)
        return RSA_verify (EVP_MD_get_type (rctx->md), tbs,
                           (unsigned int)tbslen, sig, (unsigned int)siglen,
                           rsa);
      md_size = EVP_MD_get_size (rctx->md);
      if (md_size <= 0)
        {
