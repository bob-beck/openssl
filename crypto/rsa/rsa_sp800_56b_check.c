...
ossl_rsa_check_private_exponent (const RSA *rsa, int nbits, BN_CTX *ctx)
{
  ...
  ret = (ret
         /* LCM(p - 1, q - 1) */
         && (ossl_rsa_get_lcm (ctx, rsa->p, rsa->q, lcm, gcd, p1, q1, p1q1)
             == 1)
 ...
 */
int
ossl_rsa_check_pminusq_diff (BIGNUM *diff, const BIGNUM *p, const BIGNUM *q,
                             int nbits)
{
 ...
 */
int
ossl_rsa_get_lcm (BN_CTX *ctx, const BIGNUM *p, const BIGNUM *q, BIGNUM *lcm,
 ...
 */
int
ossl_rsa_sp800_56b_check_keypair (const RSA *rsa, const BIGNUM *efixed,
