...
int ossl_rsa_check_crt_components(const RSA* rsa, BN_CTX* ctx) {
  ...
  if (rsa->dmp1 == NULL || rsa->dmq1 == NULL || rsa->iqmp == NULL) {
    if (rsa->dmp1 != NULL || rsa->dmq1 != NULL || rsa->iqmp != NULL) return 0;
...
int ossl_rsa_check_prime_factor_range(const BIGNUM* p, int nbits, BN_CTX* ctx) {
  ...
  /* Upper bound check */
  if (BN_num_bits(p) != nbits) return 0;

  BN_CTX_start(ctx);
  low = BN_CTX_get(ctx);
  if (low == NULL) goto err;

  /* set low = (√2)(2^(nbits/2 - 1) */
  if (!BN_copy(low, &ossl_bn_inv_sqrt_2)) goto err;
  ...
  if (shift >= 0) {
     ...
     * value.
     */
    if (!BN_lshift(low, low, shift)) goto err;
  } else if (!BN_rshift(low, low, -shift)) {
    goto err;
  }
  if (BN_cmp(p, low) <= 0) goto err;
  ret = 1;
err:
  BN_CTX_end(ctx);
  return ret;
...
int ossl_rsa_check_private_exponent(const RSA* rsa, int nbits, BN_CTX* ctx) {
  ...
  /* (Step 6a) 2^(nbits/2) < d */
  if (BN_num_bits(rsa->d) <= (nbits >> 1)) return 0;
...
int ossl_rsa_check_pminusq_diff(BIGNUM* diff, const BIGNUM* p, const BIGNUM* q,
  ...
  if (!BN_sub(diff, p, q)) return -1;
  BN_set_negative(diff, 0);

  if (BN_is_zero(diff)) return 0;

  if (!BN_sub_word(diff, 1)) return -1;
...
int ossl_rsa_sp800_56b_check_public(const RSA* rsa) {
  ...
  if (rsa->n == NULL || rsa->e == NULL) return 0;
...
int ossl_rsa_sp800_56b_check_private(const RSA* rsa) {
  if (rsa->d == NULL || rsa->n == NULL) return 0;
...
int ossl_rsa_sp800_56b_check_keypair(const RSA* rsa, const BIGNUM* efixed,
  ...
  /* (Step 1): Check Ranges */
  if (!ossl_rsa_sp800_56b_validate_strength(nbits, strength)) return 0;
  ...
  BN_CTX_start(ctx);
  r = BN_CTX_get(ctx);
  if (r == NULL || !BN_mul(r, rsa->p, rsa->q, ctx)) goto err;
  /* (Step 4.c): Check n = pq */
  if (BN_cmp(rsa->n, r) != 0) {
    ERR_raise(ERR_LIB_RSA, RSA_R_INVALID_REQUEST);
    goto err;
  ...
  if (ret != 1) ERR_raise(ERR_LIB_RSA, RSA_R_INVALID_KEYPAIR);
