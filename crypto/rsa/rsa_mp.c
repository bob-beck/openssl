...
RSA_PRIME_INFO* ossl_rsa_multip_info_new(void) {
  ...
  if ((pinfo = OPENSSL_zalloc(sizeof(RSA_PRIME_INFO))) == NULL) return NULL;
  if ((pinfo->r = BN_secure_new()) == NULL) goto err;
  if ((pinfo->d = BN_secure_new()) == NULL) goto err;
  if ((pinfo->t = BN_secure_new()) == NULL) goto err;
  if ((pinfo->pp = BN_secure_new()) == NULL) goto err;
...
int ossl_rsa_multip_calc_product(RSA* rsa) {
  ...
  for (i = 0; i < ex_primes; i++) {
    ...
    if (pinfo->pp == NULL) {
      ...
      if (pinfo->pp == NULL) goto err;
    }
    if (!BN_mul(pinfo->pp, p1, p2, ctx)) goto err;
    /* save previous one */
    p1 = pinfo->pp;
    p2 = pinfo->r;
  }
...
int ossl_rsa_multip_cap(int bits) {
  ...
  if (cap > RSA_MAX_PRIME_NUM) cap = RSA_MAX_PRIME_NUM;
