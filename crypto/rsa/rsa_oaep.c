...
int ossl_rsa_padding_add_PKCS1_OAEP_mgf1_ex(OSSL_LIB_CTX* libctx,
  ...
  if (RAND_bytes_ex(libctx, seed, mdlen, 0) <= 0) goto err;

  dbmask_len = emlen - mdlen;
  dbmask = OPENSSL_malloc(dbmask_len);
  if (dbmask == NULL) goto err;

  /* step 3e: dbMask = MGF(mgfSeed, nLen - HLen - 1) */
  if (PKCS1_MGF1(dbmask, dbmask_len, seed, mdlen, mgf1md) < 0) goto err;
  /* step 3f: maskedDB = DB XOR dbMask */
  for (i = 0; i < dbmask_len; i++) db[i] ^= dbmask[i];

  /* step 3g: mgfSeed = MGF(maskedDB, HLen) */
  if (PKCS1_MGF1(seedmask, mdlen, db, dbmask_len, mgf1md) < 0) goto err;
...
int RSA_padding_check_PKCS1_OAEP_mgf1(unsigned char* to, int tlen,
  ...
  if (tlen <= 0 || flen <= 0 || mdlen <= 0) return -1;
  /*
  ...
  if (PKCS1_MGF1(seed, mdlen, maskeddb, dblen, mgf1md)) goto cleanup;
  for (i = 0; i < mdlen; i++) seed[i] ^= maskedseed[i];

  if (PKCS1_MGF1(db, dblen, seed, mdlen, mgf1md)) goto cleanup;
  for (i = 0; i < dblen; i++) db[i] ^= maskeddb[i];

  if (!EVP_Digest((void*)param, plen, phash, NULL, md, NULL)) goto cleanup;
