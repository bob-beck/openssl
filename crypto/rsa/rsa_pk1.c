...
int
ossl_rsa_padding_add_PKCS1_type_2_ex (OSSL_LIB_CTX *libctx, unsigned char *to,
                                ...
                                const unsigned char *from, int flen, int num)
{
  ...
  for (i = 2; i < num; i++)
    {
      ...
      zero_index = constant_time_select_int (~found_zero_byte & equals0, i,
                                             zero_index);
   ...
   * buffer multiple times based on the bits set in the real length. Clear bits
   * do a non-copy with identical access pattern. The loop below has overall
  ...
  for (i = 0; i < tlen; i++)
    {
      ...
      to[i] = constant_time_select_8 (mask, em[i + RSA_PKCS1_PADDING_SIZE],
                                      to[i]);
    }
...
static int
ossl_rsa_prf (OSSL_LIB_CTX *ctx, unsigned char *to, int tlen,
 ...
 */
int
ossl_rsa_padding_check_PKCS1_type_2 (OSSL_LIB_CTX *ctx, unsigned char *to,
                                     int tlen, const unsigned char *from,
...
/*
 * ossl_rsa_padding_check_PKCS1_type_2_TLS() checks and removes the PKCS1 type
 * 2 padding from a decrypted RSA message in a TLS signature. The result is
 * stored in the buffer pointed to by |to| which should be |tlen| bytes long.
 * |tlen| must be at least SSL_MAX_MASTER_KEY_LENGTH. The original decrypted
 * message should be stored in |from| which must be |flen| bytes in length and
 * padded such that |flen == RSA_size()|. The TLS protocol version that the
 * client originally requested should be passed in |client_version|. Some buggy
 * clients can exist which use the negotiated version instead of the originally
 ...
 */
int
ossl_rsa_padding_check_PKCS1_type_2_TLS (OSSL_LIB_CTX *libctx,
