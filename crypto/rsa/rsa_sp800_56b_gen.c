...
int ossl_rsa_fips186_4_gen_prob_primes(RSA* rsa, RSA_ACVP_TEST* test, int nbits,
  ...
  Xpo = BN_CTX_get(ctx);
  Xqo = BN_CTX_get(ctx);
  if (tmp == NULL || Xpo == NULL || Xqo == NULL) goto err;
  BN_set_flags(Xpo, BN_FLG_CONSTTIME);
  BN_set_flags(Xqo, BN_FLG_CONSTTIME);

  if (rsa->p == NULL) rsa->p = BN_secure_new();
  if (rsa->q == NULL) rsa->q = BN_secure_new();
  if (rsa->p == NULL || rsa->q == NULL) goto err;
...
int ossl_rsa_sp800_56b_derive_params_from_pq(RSA* rsa, int nbits,
  ...
  if (e != NULL) {
    ...
    if (BN_mod_inverse(rsa->d, e, lcm, ctx) == NULL) goto err;
    ...
    /* (Step 4) n = pq */
    if (rsa->n == NULL) rsa->n = BN_new();
    if (rsa->n == NULL || !BN_mul(rsa->n, rsa->p, rsa->q, ctx)) goto err;
  }

  /* (Step 5a) dP = d mod (p-1) */
  if (rsa->dmp1 == NULL) rsa->dmp1 = BN_secure_new();
  if (rsa->dmp1 == NULL) goto err;
  BN_set_flags(rsa->dmp1, BN_FLG_CONSTTIME);
  if (!BN_mod(rsa->dmp1, rsa->d, p1, ctx)) goto err;

  /* (Step 5b) dQ = d mod (q-1) */
  if (rsa->dmq1 == NULL) rsa->dmq1 = BN_secure_new();
  if (rsa->dmq1 == NULL) goto err;
  BN_set_flags(rsa->dmq1, BN_FLG_CONSTTIME);
  if (!BN_mod(rsa->dmq1, rsa->d, q1, ctx)) goto err;
  ...
  if (BN_mod_inverse(rsa->iqmp, rsa->q, rsa->p, ctx) == NULL) goto err;
...
int ossl_rsa_sp800_56b_generate_key(RSA* rsa, int nbits, const BIGNUM* efixed,
  ...
  if (!ossl_rsa_sp800_56b_validate_strength(nbits, -1)) return 0;
  ...
  if (efixed == NULL) {
    e = BN_new();
    if (e == NULL || !BN_set_word(e, 65537)) goto err;
  } else {
    e = (BIGNUM*)efixed;
  }
...
int ossl_rsa_sp800_56b_pairwise_test(RSA* rsa, BN_CTX* ctx) {
  ...
  if (ret == 0) ERR_raise(ERR_LIB_RSA, RSA_R_PAIRWISE_TEST_FAILURE);
err:
  BN_CTX_end(ctx);
  return ret;
}
