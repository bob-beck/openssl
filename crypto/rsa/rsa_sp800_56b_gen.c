 ...
 */
int
ossl_rsa_fips186_4_gen_prob_primes (RSA *rsa, RSA_ACVP_TEST *test, int nbits,
 ...
 */
static int
rsa_validate_rng_strength (EVP_RAND_CTX *rng, int nbits)
{
  if (rng == NULL)
    return 0;
#ifdef FIPS_MODULE
 ...
 */
int
ossl_rsa_sp800_56b_derive_params_from_pq (RSA *rsa, int nbits, const BIGNUM *e,
                                          BN_CTX *ctx)
{
  int ret = -1;
 ...
 */
int
ossl_rsa_sp800_56b_generate_key (RSA *rsa, int nbits, const BIGNUM *efixed,
                                 BN_GENCB *cb)
{
  int ret = 0;
  int ok;
