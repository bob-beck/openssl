...
int ossl_rsa_verify_PKCS1_PSS_mgf1(RSA* rsa, const unsigned char* mHash,
  ...
  if (mgf1Hash == NULL) mgf1Hash = Hash;
  ...
  if (DB == NULL) goto err;
  if (PKCS1_MGF1(DB, maskedDBLen, H, hLen, mgf1Hash) < 0) goto err;
  for (i = 0; i < maskedDBLen; i++) DB[i] ^= EM[i];
  if (MSBits) DB[0] &= 0xFF >> (8 - MSBits);
  ...
  if (!EVP_DigestInit_ex(ctx, Hash, NULL) ||
    ...
    goto err;
  if (sLen != 0) {
    if (!EVP_DigestUpdate(ctx, DB + i, sLen)) goto err;
  }
  if (!EVP_DigestFinal_ex(ctx, H_, NULL)) goto err;
  if (memcmp(H_, H, hLen)) {
...
int ossl_rsa_padding_add_PKCS1_PSS_mgf1(RSA* rsa, unsigned char* EM,
  ...
  if (sLen == RSA_PSS_SALTLEN_MAX) {
    sLen = emLen - hLen - 2;
    if (sLenMax >= 0 && sLen > sLenMax) sLen = sLenMax;
  ...
  if (sLen > 0) {
    ...
    if (salt == NULL) goto err;
    if (RAND_bytes_ex(rsa->libctx, salt, sLen, 0) <= 0) goto err;
  }
  maskedDBLen = emLen - hLen - 1;
  H = EM + maskedDBLen;
  ctx = EVP_MD_CTX_new();
  ...
  if (!EVP_DigestInit_ex(ctx, Hash, NULL) ||
      ...
      !EVP_DigestUpdate(ctx, mHash, hLen))
    goto err;
  if (sLen && !EVP_DigestUpdate(ctx, salt, sLen)) goto err;
  if (!EVP_DigestFinal_ex(ctx, H, NULL)) goto err;

  /* Generate dbMask in place then perform XOR on it */
  if (PKCS1_MGF1(EM, maskedDBLen, H, hLen, mgf1Hash)) goto err;
...
int ossl_rsa_pss_params_30_saltlen(const RSA_PSS_PARAMS_30* rsa_pss_params) {
  if (rsa_pss_params == NULL) return default_RSASSA_PSS_params.salt_len;
  return rsa_pss_params->salt_len;
}

int ossl_rsa_pss_params_30_trailerfield(
    const RSA_PSS_PARAMS_30* rsa_pss_params) {
  if (rsa_pss_params == NULL) return default_RSASSA_PSS_params.trailer_field;
