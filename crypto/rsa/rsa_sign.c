 ...
 */
static int
encode_pkcs1(unsigned char** out,
             size_t* out_len,
             int type,
         ...
         RSA* rsa)
{
  ...
  encrypt_len = RSA_private_encrypt(
    (int)encoded_len, encoded, sigret, rsa, RSA_PKCS1_PADDING);
  if (encrypt_len <= 0)
    goto err;
                ...
                RSA* rsa)
{
  ...
  len = RSA_public_decrypt(
    (int)siglen, sigbuf, decrypt_buf, rsa, RSA_PKCS1_PADDING);
  if (len <= 0)
    goto err;
  decrypt_len = len;
