...
int OSSL_parse_url(const char* url, char** pscheme, char** puser, char** phost,
  ...
  if (p == NULL) {
    ...
    scheme_end = p;
    if (scheme_end == scheme) goto parse_err;
    p += strlen("://");
  }
  ...
  if (host[0] == '[') {
    ...
    if (host_end == NULL) goto parse_err;
    p = ++host_end;
  } else {
  ...
  if (!copy_substring(pscheme, scheme, scheme_end) ||
    ...
    goto err;
  if (pport_num != NULL) *pport_num = (int)portnum;
  if (*path == '/') {
    if (!copy_substring(ppath, path, path_end)) goto err;
  } else if (ppath != NULL) { /* must prepend '/' */
    size_t buflen = 1 + path_end - path + 1;

    if ((*ppath = OPENSSL_malloc(buflen)) == NULL) goto err;
...
int OSSL_HTTP_parse_url(const char* url, int* pssl, char** puser, char** phost,
  ...
  if (strcmp(port, "0") == 0) {
    ...
    if (!ossl_assert(sscanf(port, "%d", &portnum) == 1)) goto err;
    if (pport_num != NULL) *pport_num = portnum;
    if (pport != NULL) {
      *pport = OPENSSL_strdup(port);
      if (*pport == NULL) goto err;
    }
  } else {
    if (pport != NULL)
      *pport = port;
...
static int use_proxy(const char* no_proxy, const char* server) {
  ...
  if (!ossl_assert(server != NULL)) return 0;
  sl = strlen(server);
   ...
   */
  if (no_proxy == NULL) no_proxy = ossl_safe_getenv("no_proxy");
  if (no_proxy == NULL) no_proxy = ossl_safe_getenv(OPENSSL_NO_PROXY);

  if (no_proxy != NULL) found = strstr(no_proxy, server);
  while (found != NULL &&
