 ...
 */
int
OSSL_HTTP_REQ_CTX_set_request_line(OSSL_HTTP_REQ_CTX* rctx,
                                   int method_POST,
                                   const char* server,
                                   const char* port,
                                   const char* path)
...
int
OSSL_HTTP_REQ_CTX_add1_header(OSSL_HTTP_REQ_CTX* rctx,
                              const char* name,
                              const char* value)
{
...
int
OSSL_HTTP_REQ_CTX_set1_req(OSSL_HTTP_REQ_CTX* rctx,
                           const char* content_type,
                           const ASN1_ITEM* it,
...
static int
add1_headers(OSSL_HTTP_REQ_CTX* rctx,
...
http_req_ctx_new(int free_wbio,
                 BIO* wbio,
                 BIO* rbio,
  ...
  switch (retcode) {
    ...
    default:
      if (retcode < HTTP_STATUS_CODES_NONFATAL_ERROR) {
        ERR_raise_data(
          ERR_LIB_HTTP, HTTP_R_STATUS_CODE_UNSUPPORTED, "code=%s", code);
        if (*reason != '\0')
  ...
  switch (rctx->state) {
    ...
    case OHS_WRITE_REQ:
      ...
      if (rctx->req != NULL && !BIO_eof(rctx->req)) {
        if (OSSL_TRACE_ENABLED(HTTP))
          OSSL_TRACE1(HTTP,
                      "Sending request content (likely %s)\n",
    ...
    next_line:
      ...
      if (rctx->state == OHS_ERROR_CONTENT) {
        resp_len += n;
        if (!check_max_len(
              "error response content", rctx->max_resp_len, resp_len))
          return 0;
          ...
          if (rctx->state == OHS_HEADERS && rctx->expected_ct != NULL) {
            ...
            if (OPENSSL_strcasecmp(rctx->expected_ct, value) != 0
                ...
                && (strchr(rctx->expected_ct, ';') != NULL ||
                    ...
                    OPENSSL_strncasecmp(
                      rctx->expected_ct, value, semicolon - value) != 0)) {
              ERR_raise_data(ERR_LIB_HTTP,
...
int
OSSL_HTTP_REQ_CTX_nbio_d2i(OSSL_HTTP_REQ_CTX* rctx,
                           ASN1_VALUE** pval,
                           const ASN1_ITEM* it)
{
  const unsigned char* p;
...
OSSL_HTTP_REQ_CTX*
OSSL_HTTP_open(const char* server,
               const char* port,
               const char* proxy,
               const char* no_proxy,
               int use_ssl,
               ...
               int overall_timeout)
{
  ...
  if (bio != NULL) {
    ...
    if (proxy != NULL && !OSSL_HTTP_parse_url(proxy,
                                              NULL /* use_ssl */,
                                              NULL /* user */,
                                              &proxy_host,
                                              &proxy_port,
  ...
  rctx = http_req_ctx_new(bio == NULL,
                          cbio,
                          rbio != NULL ? rbio : cbio,
                          bio_update_fn,
                          arg,
...
int
OSSL_HTTP_set1_request(OSSL_HTTP_REQ_CTX* rctx,
                       const char* path,
                       const STACK_OF(CONF_VALUE)* headers,
                       const char* content_type,
                       BIO* req,
                       ...
                       int keep_alive)
{
  ...
  return OSSL_HTTP_REQ_CTX_set_request_line(rctx,
                                            req != NULL,
         ...
         OSSL_HTTP_REQ_CTX_set_expected(
           rctx, expected_content_type, expect_asn1, timeout, keep_alive) &&
              ...
              int timeout)
{
  ...
  for (;;) {
    ...
    if (!OSSL_HTTP_parse_url(current_url,
                             &use_ssl,
                             NULL /* user */,
                             &host,
                             &port,
  ...
  new_rpath:
    ...
    if (rctx != NULL) {
      if (!OSSL_HTTP_set1_request(rctx,
                                  path,
                                  headers,
                                  NULL /* content_type */,
                                  NULL /* req */,
...
BIO*
OSSL_HTTP_transfer(OSSL_HTTP_REQ_CTX** prctx,
                   const char* server,
                   const char* port,
                   const char* path,
                   int use_ssl,
 ...
 */
int
OSSL_HTTP_proxy_connect(BIO* bio,
                        const char* server,
                        const char* port,
