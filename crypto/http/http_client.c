...
#define HTTP_VERSION_STR_LEN                                                  \
  sizeof (HTTP_VERSION_PATT) /* == strlen("1.0")                              \
                              */
...
#define OHS_WRITE_REQ                                                         \
  (5 | OHS_NOREAD)                 /* Request content (body) being sent       \
                                    */
...
OSSL_HTTP_REQ_CTX *
OSSL_HTTP_REQ_CTX_new (BIO *wbio, BIO *rbio, int buf_size)
{
...
BIO *
OSSL_HTTP_REQ_CTX_get0_mem_bio (const OSSL_HTTP_REQ_CTX *rctx)
{
  if (rctx == NULL)
    {
      ERR_raise (ERR_LIB_HTTP, ERR_R_PASSED_NULL_PARAMETER);
...
size_t
OSSL_HTTP_REQ_CTX_get_resp_len (const OSSL_HTTP_REQ_CTX *rctx)
{
  if (rctx == NULL)
    {
      ERR_raise (ERR_LIB_HTTP, ERR_R_PASSED_NULL_PARAMETER);
 ...
 */
int
OSSL_HTTP_REQ_CTX_set_request_line (OSSL_HTTP_REQ_CTX *rctx, int method_POST,
                                    ...
                                    const char *path)
{
  ...
  if (server != NULL)
    { /* HTTP (but not HTTPS) proxy is used */
      /*
...
int
OSSL_HTTP_REQ_CTX_add1_header (OSSL_HTTP_REQ_CTX *rctx, const char *name,
                               const char *value)
{
...
int
OSSL_HTTP_REQ_CTX_set_expected (OSSL_HTTP_REQ_CTX *rctx,
...
static int
set1_content (OSSL_HTTP_REQ_CTX *rctx, const char *content_type, BIO *req)
{
  long req_len = 0;
  ...
  if (content_type == NULL)
    {
      rctx->text = 1; /* assuming request to be text by default, used just for
                         tracing */
    }
  else
    {
...
int
OSSL_HTTP_REQ_CTX_set1_req (OSSL_HTTP_REQ_CTX *rctx, const char *content_type,
...
void
OSSL_HTTP_REQ_CTX_set_max_response_hdr_lines (OSSL_HTTP_REQ_CTX *rctx,
                                              size_t count)
{
  if (rctx == NULL)
    {
...
static int
add1_headers (OSSL_HTTP_REQ_CTX *rctx, const STACK_OF (CONF_VALUE) *headers,
              const char *host)
{
  int i;
...
http_req_ctx_new (int free_wbio, BIO *wbio, BIO *rbio,
                  OSSL_HTTP_bio_cb_t bio_update_fn, void *arg, int use_ssl,
                  const char *proxy, const char *server, const char *port,
...
static int
parse_http_line1 (char *line, int *found_keep_alive)
{
  int i, retcode;
...
static int
check_max_len (const char *desc, size_t max_len, size_t len)
{
...
static int
check_set_resp_len (const char *desc, OSSL_HTTP_REQ_CTX *rctx, size_t len)
{
...
static int
may_still_retry (time_t max_time, int *ptimeout)
{
...
int
OSSL_HTTP_REQ_CTX_nbio_d2i (OSSL_HTTP_REQ_CTX *rctx, ASN1_VALUE **pval,
                            const ASN1_ITEM *it)
{
  const unsigned char *p;
  int rv;
...
static const char *
explict_or_default_port (const char *hostserv, const char *port, int use_ssl)
{
  if (port == NULL)
    {
      char *service = NULL;
...
static BIO *
http_new_bio (const char *server /* optionally includes ":port" */,
...
int
OSSL_HTTP_is_alive (const OSSL_HTTP_REQ_CTX *rctx)
{
...
OSSL_HTTP_REQ_CTX *
OSSL_HTTP_open (const char *server, const char *port, const char *proxy,
                const char *no_proxy, int use_ssl, BIO *bio, BIO *rbio,
...
int
OSSL_HTTP_set1_request (OSSL_HTTP_REQ_CTX *rctx, const char *path,
 ...
 */
BIO *
OSSL_HTTP_exchange (OSSL_HTTP_REQ_CTX *rctx, char **redirection_url)
{
  BIO *resp;
...
static int
redirection_ok (int n_redir, const char *old_url, const char *new_url)
{
...
OSSL_HTTP_get (const char *url, const char *proxy, const char *no_proxy,
               ...
               const char *expected_ct, int expect_asn1, size_t max_resp_len,
               int timeout)
{
  char *current_url;
  int n_redirs = 0;
...
BIO *
OSSL_HTTP_transfer (OSSL_HTTP_REQ_CTX **prctx, const char *server,
...
int
OSSL_HTTP_close (OSSL_HTTP_REQ_CTX *rctx, int ok)
{
  BIO *wbio;
  int ret = 1;
  ...
  if (rctx != NULL && rctx->upd_fn != NULL)
    {
      wbio = (*rctx->upd_fn) (rctx->wbio, rctx->upd_arg, 0 /* disconnect */,
                              ok);
      ret = wbio != NULL;
      if (ret)
        rctx->wbio = wbio;
...
static char *
base64encode (const void *buf, size_t len)
{
  int i;
  size_t outl;
  char *out;
 ...
 */
int
OSSL_HTTP_proxy_connect (BIO *bio, const char *server, const char *port,
