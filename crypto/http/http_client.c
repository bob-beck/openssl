...
OSSL_HTTP_REQ_CTX* OSSL_HTTP_REQ_CTX_new(BIO* wbio, BIO* rbio, int buf_size) {
  ...
  if ((rctx = OPENSSL_zalloc(sizeof(*rctx))) == NULL) return NULL;
...
void OSSL_HTTP_REQ_CTX_free(OSSL_HTTP_REQ_CTX* rctx) {
   ...
   */
  if (rctx->free_wbio) BIO_free_all(rctx->wbio);
...
int OSSL_HTTP_REQ_CTX_set_request_line(OSSL_HTTP_REQ_CTX* rctx, int method_POST,
  ...
  BIO_free(rctx->mem);
  if ((rctx->mem = BIO_new(BIO_s_mem())) == NULL) return 0;
  ...
  if (server != NULL) { /* HTTP (but not HTTPS) proxy is used */
     ...
     */
    if (BIO_printf(rctx->mem, OSSL_HTTP_PREFIX "%s", server) <= 0) return 0;
...
int OSSL_HTTP_REQ_CTX_add1_header(OSSL_HTTP_REQ_CTX* rctx, const char* name,
  ...
  if (value != NULL) {
    if (BIO_write(rctx->mem, ": ", 2) != 2) return 0;
...
static int set1_content(OSSL_HTTP_REQ_CTX* rctx, const char* content_type,
  ...
  if (content_type == NULL) {
    ...
    if (HAS_CASE_PREFIX(content_type, "text/")) rctx->text = 1;
...
int OSSL_HTTP_REQ_CTX_set1_req(OSSL_HTTP_REQ_CTX* rctx,
  ...
  if (req != NULL) res = (mem = ASN1_item_i2d_mem_bio(it, req)) != NULL;
...
static int add1_headers(OSSL_HTTP_REQ_CTX* rctx,
  ...
  for (i = 0; i < sk_CONF_VALUE_num(headers); i++) {
    ...
    if (add_host && OPENSSL_strcasecmp("host", hdr->name) == 0) add_host = 0;
    if (!OSSL_HTTP_REQ_CTX_add1_header(rctx, hdr->name, hdr->value)) return 0;
  }
...
static OSSL_HTTP_REQ_CTX* http_req_ctx_new(int free_wbio, BIO* wbio, BIO* rbio,
  ...
  if (proxy != NULL && (rctx->proxy = OPENSSL_strdup(proxy)) == NULL) goto err;
  if (server != NULL && (rctx->server = OPENSSL_strdup(server)) == NULL)
    goto err;
  if (port != NULL && (rctx->port = OPENSSL_strdup(port)) == NULL) goto err;
...
static int parse_http_line1(char* line, int* found_keep_alive) {
  ...
  if (!CHECK_AND_SKIP_PREFIX(line, HTTP_PREFIX_VERSION)) goto err;
    ...
    default:
      if (retcode < HTTP_STATUS_CODES_NONFATAL_ERROR) {
        ERR_raise_data(ERR_LIB_HTTP, HTTP_R_STATUS_CODE_UNSUPPORTED, "code=%s",
                       code);
        if (*reason != '\0') ERR_add_error_data(2, ", reason=", reason);
...
static int check_set_resp_len(const char* desc, OSSL_HTTP_REQ_CTX* rctx,
                              size_t len) {
  if (!check_max_len(desc, rctx->max_resp_len, len)) return 0;
...
next_io:
  ...
  if ((rctx->state & OHS_NOREAD) == 0) {
    ...
    if (n <= 0) {
      ...
      if (BIO_should_retry(rctx->rbio)) return -1;
      ERR_raise(ERR_LIB_HTTP, HTTP_R_FAILED_READING_DATA);
      return 0;
    }

    /* Write data to memory BIO */
    if (BIO_write(rctx->mem, buf, n) != n) return 0;
  }
  ...
  switch (rctx->state) {
    ...
    case OHS_WRITE_REQ:
      ...
      if (rctx->len_to_send > 0) {
        ...
        if (!BIO_write_ex(rctx->wbio, rctx->pos, rctx->len_to_send, &sz)) {
          if (BIO_should_retry(rctx->wbio)) return -1;
          rctx->state = OHS_ERROR;
          return 0;
        }
        if (OSSL_TRACE_ENABLED(HTTP)) {
        ...
        if (rctx->state == OHS_WRITE_HDR1) rctx->state = OHS_WRITE_HDR;
        rctx->pos += sz;
      ...
      if (rctx->req != NULL && !BIO_eof(rctx->req)) {
        ...
        if (n <= 0) {
          if (BIO_should_retry(rctx->req)) return -1;
          ERR_raise(ERR_LIB_HTTP, HTTP_R_FAILED_READING_DATA);
          return 0;
        }
        rctx->pos = rctx->buf;
    ...
    next_line:
      ...
      if (n <= 0) {
        if (BIO_should_retry(rctx->mem)) goto next_io;
        rctx->state = OHS_ERROR;
        return 0;
      }
      ...
      if (value != NULL) {
        *(value++) = '\0';
        ...
        if (line_end == NULL) line_end = strchr(value, '\n');
      ...
      if (value != NULL && line_end != NULL) {
        if (rctx->state == OHS_REDIRECT &&
          ...
          if (OSSL_TRACE_ENABLED(HTTP)) OSSL_TRACE(HTTP, "]\n");
          ...
          if (rctx->state == OHS_HEADERS && rctx->expected_ct != NULL) {
            ...
            if (OPENSSL_strcasecmp(rctx->expected_ct, value) != 0
              ...
              ERR_raise_data(ERR_LIB_HTTP, HTTP_R_UNEXPECTED_CONTENT_TYPE,
                             "expected=%s, actual=%s", rctx->expected_ct,
                             value);
              return 0;
            }
            found_expected_ct = 1;
    ...
    case OHS_ASN1_HEADER:
      ...
      if (!check_set_resp_len("ASN.1 DER content", rctx, resp_len)) return 0;
    ...
    case OHS_ASN1_CONTENT:
      ...
      if (n < 0 || (size_t)n < rctx->resp_len) goto next_io;
...
int OSSL_HTTP_REQ_CTX_nbio_d2i(OSSL_HTTP_REQ_CTX* rctx, ASN1_VALUE** pval,
  ...
  *pval = NULL;
  if ((rv = OSSL_HTTP_REQ_CTX_nbio(rctx)) != 1) return rv;
...
static BIO* http_new_bio(const char* server /* optionally includes ":port" */,
  ...
  if (!ossl_assert(server != NULL)) return NULL;
  ...
  if (cbio == NULL) goto end;
  if (port != NULL) (void)BIO_set_conn_port(cbio, port);
...
BIO* OSSL_HTTP_exchange(OSSL_HTTP_REQ_CTX* rctx, char** redirection_url) {
  ...
  if (resp == NULL) {
    if (rctx->redirection_url != NULL) {
      ...
      if (lib == ERR_LIB_SSL || lib == ERR_LIB_HTTP ||
        ...
        if (rctx->proxy != NULL) ERR_add_error_data(2, " proxy=", rctx->proxy);
        if (err == 0) {
  ...
  if (resp != NULL && !BIO_up_ref(resp)) resp = NULL;
  return resp;
}
...
BIO* OSSL_HTTP_get(const char* url, const char* proxy, const char* no_proxy,
  ...
  if ((current_url = OPENSSL_strdup(url)) == NULL) return NULL;
...
#define BUF_SIZE (8 * 1024)
  ...
  if (port == NULL || *port == '\0') port = OSSL_HTTPS_PORT;
  ...
  if (proxyuser != NULL) {
    ...
    if (proxypass != NULL) len += strlen(proxypass);
    proxyauth = OPENSSL_malloc(len + 1);
    if (proxyauth == NULL) goto end;
  ...
  for (;;) {
    if (BIO_flush(fbio) != 0) break;
    /* potentially needs to be retried if BIO is non-blocking */
    if (!BIO_should_retry(fbio)) break;
  }
  ...
  for (;;) {
    ...
    if (read_len < (int)HTTP_LINE1_MINLEN) continue;
