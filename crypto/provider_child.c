...
void*
ossl_child_prov_ctx_new(OSSL_LIB_CTX* libctx)
{
  return OPENSSL_zalloc(sizeof(struct child_prov_globals));
}

void
ossl_child_prov_ctx_free(void* vgbl)
{
...
static int
ossl_child_provider_init(const OSSL_CORE_HANDLE* handle,
...
static int
provider_create_child_cb(const OSSL_CORE_HANDLE* prov, void* cbdata)
{
  ...
  if ((cprov = ossl_provider_find(ctx, provname, 1)) != NULL) {
     ...
     * init children
     */
    if ((cprov = ossl_provider_new(
           ctx, provname, ossl_child_provider_init, NULL, 1)) == NULL)
      goto err;
...
static int
provider_remove_child_cb(const OSSL_CORE_HANDLE* prov, void* cbdata)
{
  OSSL_LIB_CTX* ctx = cbdata;
  struct child_prov_globals* gbl;
  const char* provname;
...
static int
provider_global_props_cb(const char* props, void* cbdata)
{
  OSSL_LIB_CTX* ctx = cbdata;
...
int
ossl_provider_init_as_child(OSSL_LIB_CTX* ctx,
                            const OSSL_CORE_HANDLE* handle,
                            const OSSL_DISPATCH* in)
{
  struct child_prov_globals* gbl;
...
void
ossl_provider_deinit_child(OSSL_LIB_CTX* ctx)
{
  struct child_prov_globals* gbl =
...
int
ossl_provider_up_ref_parent(OSSL_PROVIDER* prov, int activate)
{
  struct child_prov_globals* gbl;
...
int
ossl_provider_free_parent(OSSL_PROVIDER* prov, int deactivate)
{
  struct child_prov_globals* gbl;
  const OSSL_CORE_HANDLE* parent_handle;
