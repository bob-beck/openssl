 ...
 * all values from the array and doesn't try to optimize this away. The
 * standard doesn't actually require this behavior if the original data pointed
 * to is not volatile, but compilers do this in practice anyway.
 *
...
/*
 * For systems that don't provide an instruction counter register or
 * equivalent.
 */
uint32_t
OPENSSL_rdtsc (void)
