...
ASN1_OCTET_STRING* PKCS7_get_octet_string(PKCS7* p7) {
  if (PKCS7_type_is_data(p7)) return p7->d.data;
...
static int pkcs7_encode_rinfo(PKCS7_RECIP_INFO* ri, unsigned char* key,
  ...
  if (EVP_PKEY_encrypt_init(pctx) <= 0) goto err;

  if (EVP_PKEY_encrypt(pctx, NULL, &eklen, key, keylen) <= 0) goto err;

  ek = OPENSSL_malloc(eklen);
  if (ek == NULL) goto err;

  if (EVP_PKEY_encrypt(pctx, ek, &eklen, key, keylen) <= 0) goto err;
...
BIO* PKCS7_dataInit(PKCS7* p7, BIO* bio) {
  ...
  if (xa && !pkcs7_bio_add_digest(&out, xa, p7_ctx)) goto err;
  ...
  if (evp_cipher != NULL) {
    ...
    if (ivlen > 0)
      if (RAND_bytes_ex(libctx, iv, ivlen, 0) <= 0) goto err;
    ...
    if (EVP_CipherInit_ex(ctx, cipher, NULL, NULL, NULL, 1) <= 0) goto err;

    EVP_CIPHER_free(fetched_cipher);
    fetched_cipher = NULL;

    if (EVP_CIPHER_CTX_rand_key(ctx, key) <= 0) goto err;
    if (EVP_CipherInit_ex(ctx, NULL, NULL, key, iv, 1) <= 0) goto err;

    if (ivlen > 0) {
      if (xalg->parameter == NULL) {
        xalg->parameter = ASN1_TYPE_new();
        if (xalg->parameter == NULL) goto err;
      }
    ...
    for (i = 0; i < sk_PKCS7_RECIP_INFO_num(rsk); i++) {
      ...
      if (pkcs7_encode_rinfo(ri, key, keylen) <= 0) goto err;
    }
...
BIO* PKCS7_dataDecode(PKCS7* p7, EVP_PKEY* pkey, BIO* in_bio, X509* pcert) {
  ...
  if (cipher != NULL) {
    ...
    if (pcert) {
      for (i = 0; i < sk_PKCS7_RECIP_INFO_num(rsk); i++) {
        ri = sk_PKCS7_RECIP_INFO_value(rsk, i);
        if (!pkcs7_cmp_ri(ri, pcert)) break;
        ri = NULL;
      }
      if (ri == NULL) {
    ...
    if (pcert == NULL) {
      ...
      if (pkcs7_decrypt_rinfo(&ek, &eklen, ri, pkey, 0) < 0) goto err;
      ERR_clear_error();
    }

    evp_ctx = NULL;
    BIO_get_cipher_ctx(etmp, &evp_ctx);
    if (EVP_CipherInit_ex(evp_ctx, cipher, NULL, NULL, NULL, 0) <= 0) goto err;
    if (EVP_CIPHER_asn1_to_param(evp_ctx, enc_alg->parameter) <= 0) goto err;
    /* Generate random key as MMA defence */
    len = EVP_CIPHER_CTX_get_key_length(evp_ctx);
    if (len <= 0) goto err;
    tkeylen = (size_t)len;
    tkey = OPENSSL_malloc(tkeylen);
    if (tkey == NULL) goto err;
    if (EVP_CIPHER_CTX_rand_key(evp_ctx, tkey) <= 0) goto err;
    if (ek == NULL) {
      ek = tkey;
      eklen = tkeylen;
      tkey = NULL;
    ...
    ERR_clear_error();
    if (EVP_CipherInit_ex(evp_ctx, NULL, NULL, ek, NULL, 0) <= 0) goto err;
...
static BIO* PKCS7_find_digest(EVP_MD_CTX** pmd, BIO* bio, int nid) {
  for (;;) {
    ...
    if (EVP_MD_CTX_get_type(*pmd) == nid) return bio;
    bio = BIO_next(bio);
  }
  return NULL;
}
...
static int do_pkcs7_signed_attrib(PKCS7_SIGNER_INFO* si, EVP_MD_CTX* mctx) {
  ...
  if (!PKCS7_SIGNER_INFO_sign(si)) return 0;
...
int PKCS7_dataFinal(PKCS7* p7, BIO* bio) {
  ...
  if (si_sk != NULL) {
    for (i = 0; i < sk_PKCS7_SIGNER_INFO_num(si_sk); i++) {
      ...
      if (btmp == NULL) goto err;

      /*
       * We now have the EVP_MD_CTX, lets do the signing.
       */
      if (!EVP_MD_CTX_copy_ex(ctx_tmp, mdc)) goto err;
      ...
      if (sk_X509_ATTRIBUTE_num(sk) > 0) {
        if (!do_pkcs7_signed_attrib(si, ctx_tmp)) goto err;
      } else {
        unsigned char* abuf = NULL;
        unsigned int abuflen = EVP_PKEY_get_size(si->pkey);

        if (abuflen == 0 || (abuf = OPENSSL_malloc(abuflen)) == NULL) goto err;
    ...
    if (!PKCS7_find_digest(&mdc, bio, OBJ_obj2nid(p7->d.digest->md->algorithm)))
      goto err;
    if (!EVP_DigestFinal_ex(mdc, md_data, &md_len)) goto err;
    if (!ASN1_OCTET_STRING_set(p7->d.digest->digest, md_data, md_len)) goto err;
  }
...
int PKCS7_signatureVerify(BIO* bio, PKCS7* p7, PKCS7_SIGNER_INFO* si,
  ...
  for (;;) {
    ...
    if (EVP_MD_CTX_get_type(mdc) == md_type) break;
    /*
     * Workaround for some broken clients that put the signature OID
     * instead of the digest OID in digest_alg->algorithm
     */
    if (EVP_MD_get_pkey_type(EVP_MD_CTX_get0_md(mdc)) == md_type) break;
    btmp = BIO_next(btmp);
  }
   ...
   */
  if (!EVP_MD_CTX_copy_ex(mdc_tmp, mdc)) goto err;
  ...
  if ((sk != NULL) && (sk_X509_ATTRIBUTE_num(sk) != 0)) {
    ...
    if (!EVP_DigestFinal_ex(mdc_tmp, md_dat, &md_len)) goto err;
    ...
    if (!EVP_VerifyUpdate(mdc_tmp, abuf, alen)) goto err;
  }
...
PKCS7_ISSUER_AND_SERIAL* PKCS7_get_issuer_and_serial(PKCS7* p7, int idx) {
  ...
  if (p7->d.signed_and_enveloped == NULL) return NULL;
  rsk = p7->d.signed_and_enveloped->recipientinfo;
  if (rsk == NULL) return NULL;
...
int PKCS7_set_signed_attributes(PKCS7_SIGNER_INFO* p7si,
  ...
  if (p7si->auth_attr == NULL) return 0;
  return 1;
}
...
int PKCS7_set_attributes(PKCS7_SIGNER_INFO* p7si,
  ...
  p7si->unauth_attr =
      sk_X509_ATTRIBUTE_deep_copy(sk, X509_ATTRIBUTE_dup, X509_ATTRIBUTE_free);
  if (p7si->unauth_attr == NULL) return 0;
  return 1;
}
...
static int add_attribute(STACK_OF(X509_ATTRIBUTE)** sk, int nid, int atrtype,
  ...
  if (*sk == NULL) {
    if ((*sk = sk_X509_ATTRIBUTE_new_null()) == NULL) return 0;
  }
  n = sk_X509_ATTRIBUTE_num(*sk);
  for (i = 0; i < n; i++) {
    attr = sk_X509_ATTRIBUTE_value(*sk, i);
    if (OBJ_obj2nid(X509_ATTRIBUTE_get0_object(attr)) == nid) goto end;
  }
  if (!sk_X509_ATTRIBUTE_push(*sk, NULL)) return 0;

end:
  attr = X509_ATTRIBUTE_create(nid, atrtype, value);
  if (attr == NULL) {
    if (i == n) sk_X509_ATTRIBUTE_pop(*sk);
    return 0;
  }
