...
static ASN1_TYPE*
get_attribute(const STACK_OF(X509_ATTRIBUTE)* sk, int nid);
...
pkcs7_get1_data(PKCS7* p7)
{
  ...
  if (PKCS7_type_is_other(p7) && (p7->d.other != NULL) &&
    ...
    if (os == NULL)
      return NULL;
    inf = ASN1_get_object(
      &data, &len, &tag, &class, p7->d.other->value.sequence->length);
...
pkcs7_bio_add_digest(BIO** pbio, X509_ALGOR* alg, const PKCS7_CTX* ctx)
{
  ...
  fetched = EVP_MD_fetch(
    ossl_pkcs7_ctx_get0_libctx(ctx), name, ossl_pkcs7_ctx_get0_propq(ctx));
  if (fetched != NULL)
    md = fetched;
  else
...
pkcs7_encode_rinfo(PKCS7_RECIP_INFO* ri, unsigned char* key, int keylen)
{
  ...
  pctx = EVP_PKEY_CTX_new_from_pkey(
    ossl_pkcs7_ctx_get0_libctx(ctx), pkey, ossl_pkcs7_ctx_get0_propq(ctx));
  if (pctx == NULL)
    return 0;
...
static int
pkcs7_decrypt_rinfo(unsigned char** pek,
                    int* peklen,
                    PKCS7_RECIP_INFO* ri,
                    EVP_PKEY* pkey,
                    size_t fixlen)
{
  ...
  ret = evp_pkey_decrypt_alloc(
    pctx, &ek, &eklen, fixlen, ri->enc_key->data, ri->enc_key->length);
  if (ret <= 0)
    goto err;
    ...
    if (pcert == NULL) {
      ...
      for (i = 0; i < sk_PKCS7_RECIP_INFO_num(rsk); i++) {
        ...
        if (pkcs7_decrypt_rinfo(
              &ek, &eklen, ri, pkey, EVP_CIPHER_get_key_length(cipher)) < 0)
          goto err;
        ERR_clear_error();
      }
    } else {
...
PKCS7_SIGNER_INFO_sign(PKCS7_SIGNER_INFO* si)
{
  ...
  alen = ASN1_item_i2d(
    (ASN1_VALUE*)si->auth_attr, &abuf, ASN1_ITEM_rptr(PKCS7_ATTR_SIGN));
  if (alen < 0 || abuf == NULL)
    goto err;
...
int
PKCS7_dataVerify(X509_STORE* cert_store,
                 X509_STORE_CTX* ctx,
                 BIO* bio,
                 PKCS7* p7,
                 PKCS7_SIGNER_INFO* si)
...
int
PKCS7_signatureVerify(BIO* bio, PKCS7* p7, PKCS7_SIGNER_INFO* si, X509* signer)
{
...
ASN1_TYPE*
PKCS7_get_signed_attribute(const PKCS7_SIGNER_INFO* si, int nid)
{
  return get_attribute(si->auth_attr, nid);
}

ASN1_TYPE*
PKCS7_get_attribute(const PKCS7_SIGNER_INFO* si, int nid)
{
...
int
PKCS7_add_signed_attribute(PKCS7_SIGNER_INFO* p7si,
                           int nid,
                           int atrtype,
                           void* value)
{
