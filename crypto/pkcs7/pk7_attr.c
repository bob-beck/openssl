...
PKCS7_add_attrib_smimecap(PKCS7_SIGNER_INFO* si, STACK_OF(X509_ALGOR)* cap)
{
  ...
  if (!PKCS7_add_signed_attribute(
        si, NID_SMIMECapabilities, V_ASN1_SEQUENCE, seq)) {
    ASN1_STRING_free(seq);
    return 0;
  }
  return 1;
...
PKCS7_add_attrib_content_type(PKCS7_SIGNER_INFO* si, ASN1_OBJECT* coid)
{
  ...
  return PKCS7_add_signed_attribute(
    si, NID_pkcs9_contentType, V_ASN1_OBJECT, coid);
}
...
PKCS7_add0_attrib_signing_time(PKCS7_SIGNER_INFO* si, ASN1_TIME* t)
{
  ...
  if (!PKCS7_add_signed_attribute(
        si, NID_pkcs9_signingTime, V_ASN1_UTCTIME, t)) {
    ASN1_TIME_free(tmp);
    return 0;
  }
  return 1;
...
int
PKCS7_add1_attrib_digest(PKCS7_SIGNER_INFO* si,
                         ...
                         int mdlen)
{
  ...
  if (!ASN1_STRING_set(os, md, mdlen) ||
      !PKCS7_add_signed_attribute(
        si, NID_pkcs9_messageDigest, V_ASN1_OCTET_STRING, os)) {
