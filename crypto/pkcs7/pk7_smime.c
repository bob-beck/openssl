...
PKCS7* PKCS7_sign_ex(X509* signcert, EVP_PKEY* pkey, STACK_OF(X509)* certs,
  ...
  if (!(flags & PKCS7_NOCERTS)) {
    for (i = 0; i < sk_X509_num(certs); i++) {
      if (!PKCS7_add_certificate(p7, sk_X509_value(certs, i))) goto err;
    }
  }

  if (flags & PKCS7_DETACHED) PKCS7_set_detached(p7, 1);

  if (flags & (PKCS7_STREAM | PKCS7_PARTIAL)) return p7;
...
int PKCS7_final(PKCS7* p7, BIO* data, int flags) {
  ...
  if (!SMIME_crlf_copy(data, p7bio, flags)) goto err;
...
static int add_cipher_smcap(STACK_OF(X509_ALGOR)* sk, int nid, int arg) {
  if (EVP_get_cipherbynid(nid)) return PKCS7_simple_smimecap(sk, nid, arg);
  return 1;
}

static int add_digest_smcap(STACK_OF(X509_ALGOR)* sk, int nid, int arg) {
  if (EVP_get_digestbynid(nid)) return PKCS7_simple_smimecap(sk, nid, arg);
  return 1;
}
...
PKCS7_SIGNER_INFO* PKCS7_sign_add_signer(PKCS7* p7, X509* signcert,
  ...
  if (!(flags & PKCS7_NOCERTS)) {
    if (!PKCS7_add_certificate(p7, signcert)) goto err;
  }

  if (!(flags & PKCS7_NOATTR)) {
    if (!PKCS7_add_attrib_content_type(si, NULL)) goto err;
    ...
    if (flags & PKCS7_REUSE_DIGEST) {
      if (!pkcs7_copy_existing_digest(p7, si)) goto err;
      if (!(flags & PKCS7_PARTIAL) && !PKCS7_SIGNER_INFO_sign(si)) goto err;
    }
  }
  return si;
err:
...
static int pkcs7_copy_existing_digest(PKCS7* p7, PKCS7_SIGNER_INFO* si) {
  ...
  for (i = 0; i < sk_PKCS7_SIGNER_INFO_num(sinfos); i++) {
    ...
    if (si == sitmp) break;
    if (sk_X509_ATTRIBUTE_num(sitmp->auth_attr) <= 0) continue;
...
int PKCS7_verify(PKCS7* p7, STACK_OF(X509)* certs, X509_STORE* store,
  ...
  if ((p7bio = PKCS7_dataInit(p7, indata)) == NULL) goto err;
  ...
  if ((buf = OPENSSL_malloc(BUFFERSIZE)) == NULL) goto err;
  for (;;) {
...
err:
  if (flags & PKCS7_TEXT) BIO_free(tmpout);
  X509_STORE_CTX_free(cert_ctx);
  OPENSSL_free(buf);
  if (indata != NULL) BIO_pop(p7bio);
  BIO_free_all(p7bio);
...
PKCS7* PKCS7_encrypt_ex(STACK_OF(X509)* certs, BIO* in,
  ...
  if (!PKCS7_set_type(p7, NID_pkcs7_enveloped)) goto err;
  ...
  if (flags & PKCS7_STREAM) return p7;

  if (PKCS7_final(p7, in, flags)) return p7;
...
int PKCS7_decrypt(PKCS7* p7, EVP_PKEY* pkey, X509* cert, BIO* data, int flags) {
  ...
  if (flags & PKCS7_TEXT) {
    ...
    if (ret > 0 && BIO_method_type(tmpmem) == BIO_TYPE_CIPHER) {
      if (BIO_get_cipher_status(tmpmem) <= 0) ret = 0;
    }
    BIO_free_all(bread);
    return ret;
  }
