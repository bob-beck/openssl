  ...
  for (i = 0; i < sk_PKCS7_SIGNER_INFO_num(sinfos); i++) {
    ...
    if (signer == NULL && (flags & PKCS7_NOINTERN) == 0)
      signer = X509_find_by_issuer_and_serial(
        included_certs, ias->issuer, ias->serial);
    if (signer == NULL) {
...
PKCS7*
PKCS7_encrypt_ex(STACK_OF(X509)* certs,
                 BIO* in,
...
PKCS7*
PKCS7_encrypt(STACK_OF(X509)* certs,
              BIO* in,
              const EVP_CIPHER* cipher,
              int flags)
{
