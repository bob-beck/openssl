...
static int
pkcs7_ecdsa_or_dsa_sign_verify_setup(PKCS7_SIGNER_INFO* si, int verify)
{
  if (!verify) {
    int snid, hnid;
...
static int
pkcs7_rsa_sign_verify_setup(PKCS7_SIGNER_INFO* si, int verify)
{
  if (!verify) {
    X509_ALGOR* alg = NULL;
    ...
    if (alg != NULL)
      return X509_ALGOR_set0(
        alg, OBJ_nid2obj(NID_rsaEncryption), V_ASN1_NULL, NULL);
  }
  return 1;
}

int
PKCS7_SIGNER_INFO_set(PKCS7_SIGNER_INFO* p7i,
                      X509* x509,
                      EVP_PKEY* pkey,
                      const EVP_MD* dgst)
{
  ...
  if (!X509_ALGOR_set0(
        p7i->digest_alg, OBJ_nid2obj(EVP_MD_get_type(dgst)), V_ASN1_NULL, NULL))
    return 0;
...
STACK_OF(X509)*
pkcs7_get0_certificates(const PKCS7* p7)
{
  if (p7->d.ptr == NULL)
    return NULL;
...
static STACK_OF(PKCS7_RECIP_INFO)*
pkcs7_get_recipient_info(const PKCS7* p7)
{
  if (p7->d.ptr == NULL)
    return NULL;
  if (PKCS7_type_is_signedAndEnveloped(p7))
 ...
 */
void
ossl_pkcs7_resolve_libctx(PKCS7* p7)
{
  int i;
...
const PKCS7_CTX*
ossl_pkcs7_get0_ctx(const PKCS7* p7)
{
  return p7 != NULL ? &p7->ctx : NULL;
}

void
ossl_pkcs7_set0_libctx(PKCS7* p7, OSSL_LIB_CTX* ctx)
{
  p7->ctx.libctx = ctx;
}

int
ossl_pkcs7_set1_propq(PKCS7* p7, const char* propq)
{
...
int
ossl_pkcs7_ctx_propagate(const PKCS7* from, PKCS7* to)
{
...
OSSL_LIB_CTX*
ossl_pkcs7_ctx_get0_libctx(const PKCS7_CTX* ctx)
{
  return ctx != NULL ? ctx->libctx : NULL;
}
const char*
ossl_pkcs7_ctx_get0_propq(const PKCS7_CTX* ctx)
{
...
void
PKCS7_SIGNER_INFO_get0_algs(PKCS7_SIGNER_INFO* si,
                            EVP_PKEY** pk,
                            X509_ALGOR** pdig,
                            X509_ALGOR** psig)
{
...
static int
pkcs7_rsa_encrypt_decrypt_setup(PKCS7_RECIP_INFO* ri, int decrypt)
{
  X509_ALGOR* alg = NULL;
