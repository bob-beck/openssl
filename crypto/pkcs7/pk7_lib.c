...
int PKCS7_set_type(PKCS7* p7, int type) {
  ...
  switch (type) {
    case NID_pkcs7_signed:
      p7->type = obj;
      if ((p7->d.sign = PKCS7_SIGNED_new()) == NULL) goto err;
    ...
    case NID_pkcs7_data:
      p7->type = obj;
      if ((p7->d.data = ASN1_OCTET_STRING_new()) == NULL) goto err;
      break;
    case NID_pkcs7_signedAndEnveloped:
      p7->type = obj;
      if ((p7->d.signed_and_enveloped = PKCS7_SIGN_ENVELOPE_new()) == NULL)
        goto err;
      if (!ASN1_INTEGER_set(p7->d.signed_and_enveloped->version, 1)) goto err;
      p7->d.signed_and_enveloped->enc_data->content_type =
          OBJ_nid2obj(NID_pkcs7_data);
      break;
    case NID_pkcs7_enveloped:
      p7->type = obj;
      if ((p7->d.enveloped = PKCS7_ENVELOPE_new()) == NULL) goto err;
      if (!ASN1_INTEGER_set(p7->d.enveloped->version, 0)) goto err;
      p7->d.enveloped->enc_data->content_type = OBJ_nid2obj(NID_pkcs7_data);
      break;
    case NID_pkcs7_encrypted:
      p7->type = obj;
      if ((p7->d.encrypted = PKCS7_ENCRYPT_new()) == NULL) goto err;
      if (!ASN1_INTEGER_set(p7->d.encrypted->version, 0)) goto err;
      p7->d.encrypted->enc_data->content_type = OBJ_nid2obj(NID_pkcs7_data);
      break;

    case NID_pkcs7_digest:
      p7->type = obj;
      if ((p7->d.digest = PKCS7_DIGEST_new()) == NULL) goto err;
      if (!ASN1_INTEGER_set(p7->d.digest->version, 0)) goto err;
      break;
    default:
      ERR_raise(ERR_LIB_PKCS7, PKCS7_R_UNSUPPORTED_CONTENT_TYPE);
      goto err;
...
int PKCS7_add_crl(PKCS7* p7, X509_CRL* crl) {
  ...
  if (*sk == NULL) *sk = sk_X509_CRL_new_null();
  if (*sk == NULL) {
    ERR_raise(ERR_LIB_PKCS7, ERR_R_CRYPTO_LIB);
    return 0;
  }

  if (!X509_CRL_up_ref(crl)) return 0;
...
static int pkcs7_ecdsa_or_dsa_sign_verify_setup(PKCS7_SIGNER_INFO* si,
  ...
  if (!verify) {
    ...
    if (!OBJ_find_sigid_by_algs(&snid, hnid, EVP_PKEY_get_id(pkey))) return -1;
...
int PKCS7_SIGNER_INFO_set(PKCS7_SIGNER_INFO* p7i, X509* x509, EVP_PKEY* pkey,
  ...
  if (!ASN1_INTEGER_set(p7i->version, 1)) return 0;
  ...
  if (EVP_PKEY_is_a(pkey, "RSA")) return pkcs7_rsa_sign_verify_setup(p7i, 0);
...
PKCS7_SIGNER_INFO* PKCS7_add_signature(PKCS7* p7, X509* x509, EVP_PKEY* pkey,
  ...
  if (dgst == NULL) {
    int def_nid;
    if (EVP_PKEY_get_default_digest_nid(pkey, &def_nid) <= 0) goto err;
  ...
  if (PKCS7_SIGNER_INFO_set(si, x509, pkey, dgst) <= 0) goto err;
...
STACK_OF(X509)* pkcs7_get0_certificates(const PKCS7* p7) {
  if (p7->d.ptr == NULL) return NULL;
  if (PKCS7_type_is_signed(p7)) return p7->d.sign->cert;
...
static STACK_OF(PKCS7_RECIP_INFO)* pkcs7_get_recipient_info(const PKCS7* p7) {
  ...
  if (PKCS7_type_is_enveloped(p7)) return p7->d.enveloped->recipientinfo;
  return NULL;
}
...
void ossl_pkcs7_resolve_libctx(PKCS7* p7) {
  ...
  if (ctx == NULL || p7->d.ptr == NULL) return;
...
int ossl_pkcs7_ctx_propagate(const PKCS7* from, PKCS7* to) {
  ...
  if (!ossl_pkcs7_set1_propq(to, from->ctx.propq)) return 0;
...
STACK_OF(PKCS7_SIGNER_INFO)* PKCS7_get_signer_info(PKCS7* p7) {
  if (p7 == NULL || p7->d.ptr == NULL) return NULL;
...
void PKCS7_SIGNER_INFO_get0_algs(PKCS7_SIGNER_INFO* si, EVP_PKEY** pk,
  ...
  if (pk) *pk = si->pkey;
  if (pdig) *pdig = si->digest_alg;
  if (psig) *psig = si->digest_enc_alg;
}

void PKCS7_RECIP_INFO_get0_alg(PKCS7_RECIP_INFO* ri, X509_ALGOR** penc) {
  if (penc) *penc = ri->key_enc_algor;
}
...
PKCS7_RECIP_INFO* PKCS7_add_recipient(PKCS7* p7, X509* x509) {
  ...
  if (PKCS7_RECIP_INFO_set(ri, x509) <= 0) goto err;
...
int PKCS7_RECIP_INFO_set(PKCS7_RECIP_INFO* p7i, X509* x509) {
  ...
  if (EVP_PKEY_is_a(pkey, "RSA-PSS")) return -2;
  if (EVP_PKEY_is_a(pkey, "RSA")) {
    if (pkcs7_rsa_encrypt_decrypt_setup(p7i, 0) <= 0) goto err;
    goto finished;
  }
