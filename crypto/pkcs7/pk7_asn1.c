...
static int pk7_cb(int operation,
                  ASN1_VALUE** pval,
                  const ASN1_ITEM* it,
                  void* exarg) {
...
PKCS7* d2i_PKCS7(PKCS7** a, const unsigned char** in, long len) {
  ...
  ret = (PKCS7*)ASN1_item_d2i_ex((ASN1_VALUE**)a, in, len, (PKCS7_it()), libctx,
                                 propq);
  if (ret != NULL)
...
static int si_cb(int operation,
                 ASN1_VALUE** pval,
                 const ASN1_ITEM* it,
                 void* exarg) {
...
ASN1_SEQUENCE(PKCS7_ISSUER_AND_SERIAL) = {
    ...
    ASN1_SIMPLE(PKCS7_ISSUER_AND_SERIAL,
                serial,
...
static int ri_cb(int operation,
                 ASN1_VALUE** pval,
                 const ASN1_ITEM* it,
                 void* exarg) {
  if (operation == ASN1_OP_FREE_POST) {
...
ASN1_NDEF_SEQUENCE(PKCS7_ENC_CONTENT) = {
    ...
    ASN1_IMP_OPT(PKCS7_ENC_CONTENT,
                 enc_data,
...
ASN1_NDEF_SEQUENCE(PKCS7_SIGN_ENVELOPE) = {
    ...
    ASN1_SET_OF(PKCS7_SIGN_ENVELOPE,
                signer_info,
