...
int OSSL_PARAM_set_BN(OSSL_PARAM* p, const BIGNUM* val) {
  ...
  if (p->data_type == OSSL_PARAM_INTEGER) bytes++;
...
OSSL_PARAM OSSL_PARAM_construct_utf8_string(const char* key, char* buf,
                                            size_t bsize) {
  if (buf != NULL && bsize == 0) bsize = strlen(buf);
...
static int get_ptr_internal_skip_checks(const OSSL_PARAM* p, const void** val,
                                        size_t* used_len) {
  if (used_len != NULL) *used_len = p->data_size;
...
static int set_ptr_internal(OSSL_PARAM* p, const void* val, unsigned int type,
  ...
  p->return_size = len;
  if (p->data != NULL) *(const void**)p->data = val;
  return 1;
}
...
static int setbuf_fromparams(size_t n, OSSL_PARAM* p[], unsigned char* out,
  ...
  if (out == NULL) {
    if (!WPACKET_init_null(&pkt, 0)) return 0;
  } else {
    if (!WPACKET_init_static_len(&pkt, out, *outlen, 0)) return 0;
  }

  for (i = 0; i < n; i++) {
    if (p[i]->data_type != OSSL_PARAM_OCTET_STRING) goto err;
...
int ossl_param_get1_concat_octet_string(size_t n, OSSL_PARAM* params[],
  ...
  if (!setbuf_fromparams(n, params, NULL, &sz)) return 0;

  /* Special case zero length */
  if (sz == 0) {
    if ((res = OPENSSL_zalloc(1)) == NULL) return 0;
    goto fin;
  }
