...
static OSSL_PARAM ossl_param_construct(const char* key,
...
static int signed_from_signed(void* dest,
                              size_t dest_len,
                              const void* src,
                              size_t src_len) {
...
static int signed_from_unsigned(void* dest,
                                size_t dest_len,
                                const void* src,
                                size_t src_len) {
...
static int unsigned_from_signed(void* dest,
                                size_t dest_len,
                                const void* src,
                                size_t src_len) {
...
static int unsigned_from_unsigned(void* dest,
                                  size_t dest_len,
                                  const void* src,
                                  size_t src_len) {
  return copy_integer(dest, dest_len, src, src_len, 0, 0);
...
OSSL_PARAM OSSL_PARAM_construct_BN(const char* key,
                                   unsigned char* buf,
                                   size_t bsize) {
...
static int get_string_internal(const OSSL_PARAM* p,
                               void** val,
                               size_t* max_len,
                               size_t* used_len,
                               unsigned int type) {
...
int OSSL_PARAM_get_utf8_string(const OSSL_PARAM* p,
                               char** val,
                               size_t max_len) {
...
int OSSL_PARAM_get_octet_string(const OSSL_PARAM* p,
                                void** val,
                                size_t max_len,
                                size_t* used_len) {
...
static int set_string_internal(OSSL_PARAM* p,
                               const void* val,
                               size_t len,
                               unsigned int type) {
  if (p->data_type != type) {
...
OSSL_PARAM OSSL_PARAM_construct_octet_string(const char* key,
                                             void* buf,
                                             size_t bsize) {
  return ossl_param_construct(key, OSSL_PARAM_OCTET_STRING, buf, bsize);
}

static int get_ptr_internal_skip_checks(const OSSL_PARAM* p,
                                        const void** val,
                                        size_t* used_len) {
  if (used_len != NULL)
...
static int get_ptr_internal(const OSSL_PARAM* p,
                            const void** val,
                            size_t* used_len,
                            unsigned int type) {
  if (val == NULL || p == NULL) {
...
int OSSL_PARAM_get_octet_ptr(const OSSL_PARAM* p,
                             const void** val,
                             size_t* used_len) {
  return get_ptr_internal(p, val, used_len, OSSL_PARAM_OCTET_PTR);
}

static int set_ptr_internal(OSSL_PARAM* p,
                            const void* val,
                            unsigned int type,
                            size_t len) {
  if (p->data_type != type) {
...
OSSL_PARAM OSSL_PARAM_construct_utf8_ptr(const char* key,
                                         char** buf,
                                         size_t bsize) {
  return ossl_param_construct(key, OSSL_PARAM_UTF8_PTR, buf, bsize);
}

OSSL_PARAM OSSL_PARAM_construct_octet_ptr(const char* key,
                                          void** buf,
                                          size_t bsize) {
...
int ossl_param_get1_octet_string(const OSSL_PARAM* params,
                                 const char* name,
                                 unsigned char** out,
                                 size_t* out_len) {
...
static int setbuf_fromparams(size_t n,
                             OSSL_PARAM* p[],
                             unsigned char* out,
                             size_t* outlen) {
  int ret = 0;
...
int ossl_param_get1_concat_octet_string(size_t n,
                                        OSSL_PARAM* params[],
                                        unsigned char** out,
                                        size_t* out_len) {
  unsigned char* res;
...
int OSSL_PARAM_get_octet_string_ptr(const OSSL_PARAM* p,
                                    const void** val,
                                    size_t* used_len) {
