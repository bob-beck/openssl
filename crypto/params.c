 ...
 */
static int
copy_integer(unsigned char* dest,
             size_t dest_len,
...
{
  return copy_integer(
    dest, dest_len, src, src_len, is_negative(src, src_len) ? 0xff : 0, 1);
}

/* Copy an unsigned number to a signed number of possibly different length */
static int
signed_from_unsigned(void* dest,
                     size_t dest_len,
                     const void* src,
                     size_t src_len)
{
...
static int
unsigned_from_signed(void* dest,
                     size_t dest_len,
                     const void* src,
                     size_t src_len)
{
...
static int
unsigned_from_unsigned(void* dest,
                       size_t dest_len,
                       const void* src,
                       size_t src_len)
{
...
{
  return ossl_param_construct(
    key, OSSL_PARAM_UNSIGNED_INTEGER, buf, sizeof(unsigned int));
}
...
{
  return ossl_param_construct(
    key, OSSL_PARAM_UNSIGNED_INTEGER, buf, sizeof(unsigned long int));
}
...
{
  return ossl_param_construct(
    key, OSSL_PARAM_UNSIGNED_INTEGER, buf, sizeof(uint32_t));
}
...
{
  return ossl_param_construct(
    key, OSSL_PARAM_UNSIGNED_INTEGER, buf, sizeof(uint64_t));
}
...
{
  return ossl_param_construct(
    key, OSSL_PARAM_UNSIGNED_INTEGER, buf, sizeof(size_t));
}
...
static int
get_string_internal(const OSSL_PARAM* p,
                    void** val,
                    size_t* max_len,
                    size_t* used_len,
                    unsigned int type)
...
int
OSSL_PARAM_get_octet_string(const OSSL_PARAM* p,
                            void** val,
                            size_t max_len,
                            size_t* used_len)
{
  return get_string_internal(
    p, val, &max_len, used_len, OSSL_PARAM_OCTET_STRING);
}

static int
set_string_internal(OSSL_PARAM* p,
                    const void* val,
                    size_t len,
                    unsigned int type)
{
...
static int
get_ptr_internal_skip_checks(const OSSL_PARAM* p,
                             const void** val,
                             size_t* used_len)
{
  if (used_len != NULL)
...
static int
get_ptr_internal(const OSSL_PARAM* p,
                 const void** val,
                 size_t* used_len,
                 unsigned int type)
{
...
int
OSSL_PARAM_get_octet_ptr(const OSSL_PARAM* p,
                         const void** val,
                         size_t* used_len)
{
...
OSSL_PARAM_set_utf8_ptr(OSSL_PARAM* p, const char* val)
{
  ...
  p->return_size = 0;
  return set_ptr_internal(
    p, val, OSSL_PARAM_UTF8_PTR, val == NULL ? 0 : strlen(val));
}
 ...
 */
int
ossl_param_get1_octet_string_from_param(const OSSL_PARAM* p,
                                        unsigned char** out,
                                        size_t* out_len)
{
  void* buf = NULL;
...
int
ossl_param_get1_octet_string(const OSSL_PARAM* params,
                             const char* name,
                             unsigned char** out,
                             size_t* out_len)
{
...
static int
setbuf_fromparams(size_t n, OSSL_PARAM* p[], unsigned char* out, size_t* outlen)
{
  int ret = 0;
  WPACKET pkt;
  size_t i;
...
int
ossl_param_get1_concat_octet_string(size_t n,
                                    OSSL_PARAM* params[],
                                    unsigned char** out,
                                    size_t* out_len)
{
...
static int
get_string_ptr_internal(const OSSL_PARAM* p,
                        const void** val,
                        size_t* used_len,
...
{
  return get_string_ptr_internal(
    p, (const void**)val, NULL, OSSL_PARAM_UTF8_PTR, OSSL_PARAM_UTF8_STRING);
}

int
OSSL_PARAM_get_octet_string_ptr(const OSSL_PARAM* p,
                                const void** val,
                                size_t* used_len)
{
  return get_string_ptr_internal(p,
                                 (const void**)val,
                                 used_len,
