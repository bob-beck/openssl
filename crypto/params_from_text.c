...
static int
prepare_from_text(const OSSL_PARAM* paramdefs,
                  const char* key,
                  const char* value,
                  size_t value_n,
                  /* Output parameters */
...
static int
construct_from_text(OSSL_PARAM* to,
                    const OSSL_PARAM* paramdef,
                    const char* value,
                    size_t value_n,
                    int ishex,
...
OSSL_PARAM_print_to_bio(const OSSL_PARAM* p, BIO* bio, int print_values)
{
  ...
  for (; p->key != NULL; p++) {
    ...
    switch (p->data_type) {
      ...
      default:
        ok = BIO_printf(
          bio, "unknown type (%u) of %zu bytes\n", p->data_type, p->data_size);
        break;
    }
    if (ok == -1)
      goto end;
...
int
OSSL_PARAM_allocate_from_text(OSSL_PARAM* to,
                              ...
                              int* found)
{
  ...
  if (!prepare_from_text(paramdefs,
                         key,
                         value,
                         value_n,
                         &paramdef,
