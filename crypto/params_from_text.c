...
static int
prepare_from_text (const OSSL_PARAM *paramdefs, const char *key,
                   const char *value, size_t value_n,
                   /* Output parameters */
                   const OSSL_PARAM **paramdef, int *ishex, size_t *buf_n,
                   BIGNUM **tmpbn, int *found)
{
  const OSSL_PARAM *p;
  size_t buf_bits;
  int r;
...
static int
construct_from_text (OSSL_PARAM *to, const OSSL_PARAM *paramdef,
                     const char *value, size_t value_n, int ishex, void *buf,
 ...
 */
int
OSSL_PARAM_print_to_bio (const OSSL_PARAM *p, BIO *bio, int print_values)
{
  int64_t i;
  uint64_t u;
  BIGNUM *bn;
...
int
OSSL_PARAM_allocate_from_text (OSSL_PARAM *to, const OSSL_PARAM *paramdefs,
