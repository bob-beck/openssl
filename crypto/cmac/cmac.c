...
static void
make_kn (unsigned char *k1, const unsigned char *l, int bl)
{
  int i;
...
int
ossl_cmac_init (CMAC_CTX *ctx, const void *key, size_t keylen,
  ...
  if (max_burst_blocks == 0)
  ...
  else
    {
      while (cipher_blocks > max_burst_blocks)
        {
          if (EVP_Cipher (ctx->cctx, buf, data, (int)(max_burst_blocks * bl))
              <= 0)
            return 0;
      ...
      if (cipher_blocks > 0)
        {
          if (EVP_Cipher (ctx->cctx, buf, data, (int)(cipher_blocks * bl))
              <= 0)
            return 0;
