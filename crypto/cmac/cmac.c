...
CMAC_CTX* CMAC_CTX_new(void) {
  ...
  if ((ctx = OPENSSL_malloc(sizeof(*ctx))) == NULL) return NULL;
...
int CMAC_CTX_copy(CMAC_CTX* out, const CMAC_CTX* in) {
  ...
  if (in->nlast_block == -1) return 0;
  if ((bl = EVP_CIPHER_CTX_get_block_size(in->cctx)) == 0) return 0;
...
int ossl_cmac_init(CMAC_CTX* ctx, const void* key, size_t keylen,
  ...
  if (!key && !cipher && !impl && keylen == 0) {
    ...
    if (!EVP_EncryptInit_ex2(ctx->cctx, NULL, NULL, zero_iv, param)) return 0;
  ...
  if (cipher != NULL) {
    ...
    ctx->nlast_block = -1;
    if (impl != NULL) {
      if (!EVP_EncryptInit_ex(ctx->cctx, cipher, impl, NULL, NULL)) return 0;
    } else {
      if (!EVP_EncryptInit_ex2(ctx->cctx, cipher, NULL, NULL, param)) return 0;
    }
  }
  ...
  if (key != NULL) {
    ...
    ctx->nlast_block = -1;
    if (EVP_CIPHER_CTX_get0_cipher(ctx->cctx) == NULL) return 0;
    if (keylen > INT_MAX ||
        EVP_CIPHER_CTX_set_key_length(ctx->cctx, (int)keylen) <= 0)
      return 0;
    if (!EVP_EncryptInit_ex2(ctx->cctx, NULL, key, zero_iv, param)) return 0;
    if ((bl = EVP_CIPHER_CTX_get_block_size(ctx->cctx)) < 0) return 0;
    if (EVP_Cipher(ctx->cctx, ctx->tbl, zero_iv, bl) <= 0) return 0;
...
int CMAC_Update(CMAC_CTX* ctx, const void* in, size_t dlen) {
  ...
  if (ctx->nlast_block > 0) {
    ...
    if (EVP_Cipher(ctx->cctx, ctx->tbl, ctx->last_block, bl) <= 0) return 0;
  }
  ...
  if (max_burst_blocks == 0) {
    ...
    while (dlen > (size_t)bl) {
      if (EVP_Cipher(ctx->cctx, ctx->tbl, data, bl) <= 0) return 0;
      dlen -= bl;
      data += bl;
    }
  } else {
...
int CMAC_Final(CMAC_CTX* ctx, unsigned char* out, size_t* poutlen) {
  ...
  if (lb == bl) {
    ...
    if (bl - lb > 1) memset(ctx->last_block + lb + 1, 0, bl - lb - 1);
