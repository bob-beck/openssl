...
EC_KEY_copy(EC_KEY* dest, const EC_KEY* src)
{
  ...
  if (!CRYPTO_dup_ex_data(
        CRYPTO_EX_INDEX_EC_KEY, &dest->ex_data, &src->ex_data))
    return NULL;
#endif
 ...
 */
static int
ecdsa_keygen_knownanswer_test(EC_KEY* eckey,
                              BN_CTX* ctx,
                              OSSL_CALLBACK* cb,
                              void* cbarg)
{
  ...
  OSSL_SELF_TEST_onbegin(
    st, OSSL_SELF_TEST_TYPE_PCT_KAT, OSSL_SELF_TEST_DESC_PCT_ECDSA);
 ...
 */
int
ossl_ec_generate_key_dhkem(EC_KEY* eckey,
...
ossl_ec_key_simple_generate_public_key(EC_KEY* eckey)
{
   ...
   */
  ret = EC_POINT_mul(
    eckey->group, eckey->pub_key, eckey->priv_key, NULL, NULL, ctx);
...
size_t
ossl_ec_key_simple_priv2oct(const EC_KEY* eckey, unsigned char* buf, size_t len)
{
  int buf_len;
...
int
ossl_ec_key_simple_oct2priv(EC_KEY* eckey, const unsigned char* buf, size_t len)
{
  if (len > INT_MAX) {
...
ecdsa_keygen_pairwise_test(EC_KEY* eckey, OSSL_CALLBACK* cb, void* cbarg)
{
  ...
  OSSL_SELF_TEST_onbegin(
    st, OSSL_SELF_TEST_TYPE_PCT, OSSL_SELF_TEST_DESC_PCT_ECDSA);
