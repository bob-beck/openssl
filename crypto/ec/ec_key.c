...
void EC_KEY_free(EC_KEY* r) {
  ...
  if (r->meth != NULL && r->meth->finish != NULL) r->meth->finish(r);

#if !defined(OPENSSL_NO_ENGINE) && !defined(FIPS_MODULE)
  ENGINE_finish(r->engine);
#endif

  if (r->group && r->group->meth->keyfinish) r->group->meth->keyfinish(r);
...
EC_KEY* EC_KEY_copy(EC_KEY* dest, const EC_KEY* src) {
  ...
  if (src->meth != dest->meth) {
    if (dest->meth->finish != NULL) dest->meth->finish(dest);
  ...
  if (src->group != NULL) {
    ...
    if (dest->group == NULL) return NULL;
    if (!EC_GROUP_copy(dest->group, src->group)) return NULL;

    /*  copy the public key */
    if (src->pub_key != NULL) {
      EC_POINT_free(dest->pub_key);
      dest->pub_key = EC_POINT_new(src->group);
      if (dest->pub_key == NULL) return NULL;
      if (!EC_POINT_copy(dest->pub_key, src->pub_key)) return NULL;
    }
    /* copy the private key */
    if (src->priv_key != NULL) {
      if (dest->priv_key == NULL) {
        dest->priv_key = BN_new();
        if (dest->priv_key == NULL) return NULL;
      }
      if (!BN_copy(dest->priv_key, src->priv_key)) return NULL;
  ...
  if (src->meth != dest->meth) {
#if !defined(OPENSSL_NO_ENGINE) && !defined(FIPS_MODULE)
    if (src->engine != NULL && ENGINE_init(src->engine) == 0) return NULL;
    dest->engine = src->engine;
#endif
    dest->meth = src->meth;
  }

  if (src->meth->copy != NULL && src->meth->copy(dest, src) == 0) return NULL;
...
int EC_KEY_up_ref(EC_KEY* r) {
  ...
  if (CRYPTO_UP_REF(&r->references, &i) <= 0) return 0;
...
static int ecdsa_keygen_knownanswer_test(EC_KEY* eckey, BN_CTX* ctx,
  ...
  if ((pub_key2 = EC_POINT_new(eckey->group)) == NULL) goto err;

  /* pub_key = priv_key * G (where G is a point on the curve) */
  if (!EC_POINT_mul(eckey->group, pub_key2, eckey->priv_key, NULL, NULL, ctx))
    goto err;

  if (BN_num_bytes(pub_key2->X) > (int)sizeof(bytes)) goto err;
...
static int ec_generate_key(EC_KEY* eckey, int pairwise_test) {
  ...
  if (sm2) {
    order = BN_new();
    if (order == NULL || !BN_sub(order, tmp, BN_value_one())) goto err;
  } else {
    order = BN_dup(tmp);
  ...
  do
    if (!BN_priv_rand_range_ex(priv_key, order, 0, ctx)) goto err;
...
err:
  ...
  if (!ok) {
    ...
    if (eckey->pub_key != NULL) EC_POINT_set_to_infinity(group, eckey->pub_key);
  }
...
int ossl_ec_generate_key_dhkem(EC_KEY* eckey, const unsigned char* ikm,
  ...
  if (eckey->priv_key == NULL) {
    ...
    if (eckey->priv_key == NULL) goto err;
  }
  if (ossl_ec_dhkem_derive_private(eckey, eckey->priv_key, ikm, ikmlen) <= 0)
    goto err;
  if (eckey->pub_key == NULL) {
    eckey->pub_key = EC_POINT_new(eckey->group);
    if (eckey->pub_key == NULL) goto err;
  }
  if (!ossl_ec_key_simple_generate_public_key(eckey)) goto err;
...
int ossl_ec_key_simple_generate_public_key(EC_KEY* eckey) {
   ...
   * pub_key = priv_key * G (where G is a point on the curve)
   */
  ret = EC_POINT_mul(eckey->group, eckey->pub_key, eckey->priv_key, NULL, NULL,
                     ctx);
...
int ossl_ec_key_public_check(const EC_KEY* eckey, BN_CTX* ctx) {
  ...
  if (!ossl_ec_key_public_check_quick(eckey, ctx)) return 0;
...
int ossl_ec_key_simple_check_key(const EC_KEY* eckey) {
  ...
  if ((ctx = BN_CTX_new_ex(eckey->libctx)) == NULL) return 0;

  if (!ossl_ec_key_public_check(eckey, ctx)) goto err;
...
int EC_KEY_set_public_key_affine_coordinates(EC_KEY* key, BIGNUM* x,
  ...
  if (!EC_POINT_set_affine_coordinates(key->group, point, x, y, ctx)) goto err;
  ...
  if (!EC_KEY_set_public_key(key, point)) goto err;

  if (EC_KEY_check_key(key) == 0) goto err;
...
int EC_KEY_set_private_key(EC_KEY* key, const BIGNUM* priv_key) {
  ...
  if (key->group == NULL || key->group->meth == NULL) return 0;
...
void EC_KEY_set_asn1_flag(EC_KEY* key, int flag) {
  if (key->group != NULL) EC_GROUP_set_asn1_flag(key->group, flag);
}

#ifndef OPENSSL_NO_DEPRECATED_3_0
int EC_KEY_precompute_mult(EC_KEY* key, BN_CTX* ctx) {
  if (key->group == NULL) return 0;
...
int EC_KEY_decoded_from_explicit_params(const EC_KEY* key) {
  if (key == NULL || key->group == NULL) return -1;
...
size_t ossl_ec_key_simple_priv2oct(const EC_KEY* eckey, unsigned char* buf,
  ...
  if (eckey->priv_key == NULL) return 0;
  if (buf == NULL)
    return buf_len;
...
int ossl_ec_key_simple_oct2priv(EC_KEY* eckey, const unsigned char* buf,
  ...
  if (eckey->priv_key == NULL) eckey->priv_key = BN_secure_new();
  if (eckey->priv_key == NULL) {
...
static int ecdsa_keygen_pairwise_test(EC_KEY* eckey, OSSL_CALLBACK* cb,
  ...
  if (ECDSA_do_verify(dgst, dgst_len, sig, eckey) != 1) goto err;
