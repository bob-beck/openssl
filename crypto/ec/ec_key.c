...
EC_KEY *
EC_KEY_new_ex (OSSL_LIB_CTX *ctx, const char *propq)
{
 ...
 */
static int
ecdsa_keygen_knownanswer_test (EC_KEY *eckey, BN_CTX *ctx, OSSL_CALLBACK *cb,
                               void *cbarg)
{
  int len, ret = 0;
 ...
 *     eckey An EC key object that contains domain params. The generated
 * keypair is stored in this object. pairwise_test Set to non zero to perform a
 * pairwise test. If the test fails then the keypair is not generated, Returns
 * 1 if the keypair was generated or 0 otherwise.
 */
static int
ec_generate_key (EC_KEY *eckey, int pairwise_test)
{
  int ok = 0;
 ...
 */
int
ossl_ec_generate_key_dhkem (EC_KEY *eckey, const unsigned char *ikm,
                            size_t ikmlen)
{
  int ok = 0;
 ...
 */
static int
ec_key_public_range_check (BN_CTX *ctx, const EC_KEY *key)
{
  int ret = 0;
  BIGNUM *x, *y;
 ...
 */
int
ossl_ec_key_public_check_quick (const EC_KEY *eckey, BN_CTX *ctx)
{
  if (eckey == NULL || eckey->group == NULL || eckey->pub_key == NULL)
    {
      ERR_raise (ERR_LIB_EC, ERR_R_PASSED_NULL_PARAMETER);
 ...
 */
int
ossl_ec_key_public_check (const EC_KEY *eckey, BN_CTX *ctx)
{
  int ret = 0;
  EC_POINT *point = NULL;
 ...
 */
int
ossl_ec_key_private_check (const EC_KEY *eckey)
{
 ...
 */
int
ossl_ec_key_pairwise_check (const EC_KEY *eckey, BN_CTX *ctx)
{
  int ret = 0;
  EC_POINT *point = NULL;
 ...
 * NOTES:
 *    Before calling this method in fips mode, there should be an assurance
 * that an approved elliptic-curve group is used. Returns 1 if the key is
...
OSSL_LIB_CTX *
ossl_ec_key_get_libctx (const EC_KEY *key)
{
  return key->libctx;
}

const char *
ossl_ec_key_get0_propq (const EC_KEY *key)
{
  return key->propq;
}

void
ossl_ec_key_set0_libctx (EC_KEY *key, OSSL_LIB_CTX *libctx)
{
  key->libctx = libctx;
...
size_t
EC_KEY_key2buf (const EC_KEY *key, point_conversion_form_t form,
...
int
EC_KEY_oct2key (EC_KEY *key, const unsigned char *buf, size_t len, BN_CTX *ctx)
{
  if (key == NULL || key->group == NULL)
    return 0;
  if (key->pub_key == NULL)
...
size_t
ossl_ec_key_simple_priv2oct (const EC_KEY *eckey, unsigned char *buf,
                             size_t len)
{
  int buf_len;
...
int
EC_KEY_oct2priv (EC_KEY *eckey, const unsigned char *buf, size_t len)
{
  int ret;
...
int
ossl_ec_key_simple_oct2priv (EC_KEY *eckey, const unsigned char *buf,
                             size_t len)
{
  if (len > INT_MAX)
    {
...
int
EC_KEY_can_sign (const EC_KEY *eckey)
{
  if (eckey->group == NULL || eckey->group->meth == NULL
 ...
 */
static int
ecdsa_keygen_pairwise_test (EC_KEY *eckey, OSSL_CALLBACK *cb, void *cbarg)
{
  int ret = 0;
