...
ASN1_ADB_TEMPLATE(char_two_def) = ASN1_SIMPLE(X9_62_CHARACTERISTIC_TWO,
                                              p.other,
                                              ASN1_ANY);
...
ASN1_ADB(X9_62_CHARACTERISTIC_TWO) = {
  ...
  ADB_ENTRY(NID_X9_62_ppBasis,
            ASN1_SIMPLE(X9_62_CHARACTERISTIC_TWO, p.ppBasis, X9_62_PENTANOMIAL))
} ASN1_ADB_END(X9_62_CHARACTERISTIC_TWO, 0, type, 0, &char_two_def_tt, NULL);

ASN1_SEQUENCE(
  X9_62_CHARACTERISTIC_TWO) = { ASN1_EMBED(X9_62_CHARACTERISTIC_TWO, m, INT32),
...
static int
ec_asn1_group2fieldid(const EC_GROUP*, X9_62_FIELDID*);
/* ec_asn1_group2curve() sets the values in a X9_62_CURVE object */
static int
ec_asn1_group2curve(const EC_GROUP*, X9_62_CURVE*);
...
ECPARAMETERS*
EC_GROUP_get_ecparameters(const EC_GROUP* group, ECPARAMETERS* params)
{
  size_t len = 0;
...
ECPKPARAMETERS*
EC_GROUP_get_ecpkparameters(const EC_GROUP* group, ECPKPARAMETERS* params)
{
  int ok = 1, tmp;
...
EC_GROUP_new_from_ecparameters(const ECPARAMETERS* params)
{
  ...
  if (!EC_POINT_oct2point(
        ret, point, params->base->data, params->base->length, NULL)) {
    ERR_raise(ERR_LIB_EC, ERR_R_EC_LIB);
    goto err;
  }
...
d2i_ECPrivateKey(EC_KEY** a, const unsigned char** in, long len)
{
  ...
  if (priv_key->privateKey) {
    ...
    if (EC_KEY_oct2priv(
          ret, ASN1_STRING_get0_data(pkey), ASN1_STRING_length(pkey)) == 0)
      goto err;
  } else {
...
int
i2d_ECPrivateKey(const EC_KEY* a, unsigned char** out)
{
  int ret = 0, ok = 0;
...
int
i2d_ECParameters(const EC_KEY* a, unsigned char** out)
{
  if (a == NULL) {
    ERR_raise(ERR_LIB_EC, ERR_R_PASSED_NULL_PARAMETER);
    return 0;
...
i2o_ECPublicKey(const EC_KEY* a, unsigned char** out)
{
  ...
  if (!EC_POINT_point2oct(
        a->group, a->pub_key, a->conv_form, *out, buf_len, NULL)) {
    ERR_raise(ERR_LIB_EC, ERR_R_EC_LIB);
    if (new_buffer) {
      OPENSSL_free(*out);
      *out = NULL;
...
ECDSA_SIG*
d2i_ECDSA_SIG(ECDSA_SIG** psig, const unsigned char** ppin, long len)
{
  ECDSA_SIG* sig;
...
int
i2d_ECDSA_SIG(const ECDSA_SIG* sig, unsigned char** ppout)
{
  BUF_MEM* buf = NULL;
  size_t encoded_len;
  WPACKET pkt;
