            ...
            {ASN1_SIMPLE(X9_62_CURVE, a, ASN1_OCTET_STRING),
             ...
             ASN1_OPT(X9_62_CURVE, seed,
                ...
                ASN1_SEQUENCE(ECPARAMETERS) = {
                    ...
                    ASN1_OPT(ECPARAMETERS, cofactor,
...
err:
  if (params == NULL) ECPARAMETERS_free(ret);
  return NULL;
}
...
EC_GROUP* EC_GROUP_new_from_ecparameters(const ECPARAMETERS* params) {
  ...
  if ((curve_name = ossl_ec_curve_nid_from_params(dup, ctx)) != NID_undef) {
     ...
     */
    if (curve_name == NID_wap_wsg_idm_ecid_wtls12) curve_name = NID_secp224r1;
     ...
     */
    if (params->curve->seed == NULL) {
      if (EC_GROUP_set_seed(ret, NULL, 0) != 1) goto err;
    }
  }
...
EC_KEY* d2i_ECPrivateKey(EC_KEY** a, const unsigned char** in, long len) {
  ...
  if ((priv_key = d2i_EC_PRIVATEKEY(NULL, &p, len)) == NULL) return NULL;
...
err:
  if (a == NULL || *a != ret) EC_KEY_free(ret);
  EC_PRIVATEKEY_free(priv_key);
  return NULL;
}
...
int i2o_ECPublicKey(const EC_KEY* a, unsigned char** out) {
  ...
  if (*out == NULL) {
    if ((*out = OPENSSL_malloc(buf_len)) == NULL) return 0;
    new_buffer = 1;
  }
...
ECDSA_SIG* d2i_ECDSA_SIG(ECDSA_SIG** psig, const unsigned char** ppin,
  ...
  if (sig->r == NULL) sig->r = BN_new();
  if (sig->s == NULL) sig->s = BN_new();
  if (sig->r == NULL || sig->s == NULL ||
      ossl_decode_der_dsa_sig(sig->r, sig->s, ppin, (size_t)len) == 0) {
    if (psig == NULL || *psig == NULL) ECDSA_SIG_free(sig);
    return NULL;
  }
  if (psig != NULL && *psig == NULL) *psig = sig;
  return sig;
}
...
int i2d_ECDSA_SIG(const ECDSA_SIG* sig, unsigned char** ppout) {
  ...
  if (ppout == NULL) {
    if (!WPACKET_init_null(&pkt, 0)) return -1;
  } else if (*ppout == NULL) {
    ...
    if (!WPACKET_init_static_len(&pkt, *ppout, SIZE_MAX, 0)) return -1;
  }
