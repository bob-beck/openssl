...
int ECPKParameters_print(BIO* bp, const EC_GROUP* x, int off) {
  ...
  if (EC_GROUP_get_asn1_flag(x)) {
    ...
    if (nid == 0) goto err;
    if (BIO_printf(bp, "ASN1 OID: %s", OBJ_nid2sn(nid)) <= 0) goto err;
    if (BIO_printf(bp, "\n") <= 0) goto err;
    ...
    if (tmp_nid == NID_X9_62_characteristic_two_field) is_char_two = 1;
    ...
    if (BIO_printf(bp, "Field Type: %s\n", OBJ_nid2sn(tmp_nid)) <= 0) goto err;

    if (is_char_two) {
      /* print the 'short name' of the base type OID */
      int basis_type = EC_GROUP_get_basis_type(x);
      if (basis_type == 0) goto err;
      ...
      if ((p != NULL) && !ASN1_bn_print(bp, "Prime:", p, NULL, off)) goto err;
    }
    if ((a != NULL) && !ASN1_bn_print(bp, "A:   ", a, NULL, off)) goto err;
    if ((b != NULL) && !ASN1_bn_print(bp, "B:   ", b, NULL, off)) goto err;
    ...
    if ((cofactor != NULL) &&
      ...
      goto err;
    if (seed && !print_bin(bp, "Seed:", seed, seed_len, off)) goto err;
  }
  ret = 1;
err:
  if (!ret) ERR_raise(ERR_LIB_EC, reason);
  BN_free(p);
  BN_free(a);
  BN_free(b);
...
static int print_bin(BIO* fp, const char* name, const unsigned char* buf,
  ...
  if (off > 0) {
    if (off > 128) off = 128;
    memset(str, ' ', off);
    if (BIO_write(fp, str, off) <= 0) return 0;
  } else {
    off = 0;
  }
  ...
  for (i = 0; i < len; i++) {
    if ((i % 15) == 0) {
      str[0] = '\n';
      memset(&(str[1]), ' ', off + 4);
      if (BIO_write(fp, str, off + 1 + 4) <= 0) return 0;
    }
