  ...
  switch (key->type) {
    ...
    case ECX_KEY_TYPE_ED25519:
      if (!ossl_ed25519_public_from_private(
            key->libctx, key->pubkey, key->privkey, key->propq)) {
                      ...
                      int include_private)
{
  ...
  if (include_private && param_priv_key != NULL) {
    if (!OSSL_PARAM_get_octet_string(
          param_priv_key, (void**)&ecx->privkey, ecx->keylen, &privkeylen))
      return 0;
  ...
  if (param_pub_key != NULL &&
      !OSSL_PARAM_get_octet_string(
        param_pub_key, (void**)&pubkey, sizeof(ecx->pubkey), &pubkeylen))
    return 0;
                        ...
                        const char* propq)
{
   ...
   */
  ecx = ossl_ecx_key_op(
    palg, p, plen, EVP_PKEY_NONE, KEY_OP_PRIVATE, libctx, propq);
