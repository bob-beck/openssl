...
int ossl_ecx_key_fromdata(ECX_KEY* ecx, const OSSL_PARAM* param_pub_key,
  ...
  if (ecx == NULL) return 0;

  if (param_pub_key == NULL && param_priv_key == NULL) return 0;
  ...
  if (param_pub_key == NULL && !ossl_ecx_public_from_private(ecx)) return 0;
...
ECX_KEY* ossl_ecx_key_op(const X509_ALGOR* palg, const unsigned char* p,
  ...
  if (op == KEY_OP_PUBLIC) {
    ...
    if (op == KEY_OP_KEYGEN) {
      if (id != EVP_PKEY_NONE) {
        if (RAND_priv_bytes_ex(libctx, privkey, KEYLENID(id), 0) <= 0) goto err;
