...
static void ecp_nistz256_point_add(P256_POINT* r, const P256_POINT* a,
  ...
  if (P256_LIMBS == 8) in1infty |= (in1_z[4] | in1_z[5] | in1_z[6] | in1_z[7]);

  in2infty = (in2_z[0] | in2_z[1] | in2_z[2] | in2_z[3]);
  if (P256_LIMBS == 8) in2infty |= (in2_z[4] | in2_z[5] | in2_z[6] | in2_z[7]);
...
__owur static int ecp_nistz256_windowed_mul(const EC_GROUP* group,
  ...
  for (i = 0; i < num; i++) {
    ...
    if ((BN_num_bits(scalar[i]) > 256) || BN_is_negative(scalar[i])) {
      ...
      if ((mod = BN_CTX_get(ctx)) == NULL) goto err;
...
__owur static int ecp_nistz256_mult_precompute(EC_GROUP* group, BN_CTX* ctx) {
  ...
  if ((pre_comp = ecp_nistz256_pre_comp_new(group)) == NULL) return 0;
  ...
  if (P == NULL || T == NULL) goto err;

  /*
   * The zero entry is implicitly infinity, and we skip it, storing other
   * values with -1 offset.
   */
  if (!EC_POINT_copy(T, generator)) goto err;

  for (k = 0; k < 64; k++) {
    if (!EC_POINT_copy(P, T)) goto err;
    for (j = 0; j < 37; j++) {
      ...
      for (i = 0; i < 7; i++) {
        if (!EC_POINT_dbl(group, P, P, ctx)) goto err;
      }
    }
    if (!EC_POINT_add(group, T, T, generator, ctx)) goto err;
  }
...
__owur static int ecp_nistz256_points_mul(const EC_GROUP* group, EC_POINT* r,
  ...
  if (scalar) {
    ...
    if (pre_comp) {
      ...
      if (pre_comp_generator == NULL) goto err;
    ...
    if (preComputedTable) {
      ...
      if ((BN_num_bits(scalar) > 256) || BN_is_negative(scalar)) {
        if ((tmp_scalar = BN_CTX_get(ctx)) == NULL) goto err;
  ...
  if (no_precomp_for_generator) {
    ...
    if (new_scalars == NULL) goto err;

    new_points = OPENSSL_malloc_array(num + 1, sizeof(EC_POINT*));
    if (new_points == NULL) goto err;
  ...
  if (num) {
    P256_POINT* out = &t.p;
    if (p_is_infinity) out = &p.p;

    if (!ecp_nistz256_windowed_mul(group, out, scalars, points, num, ctx))
      goto err;

    if (!p_is_infinity) ecp_nistz256_point_add(&p.p, &p.p, out);
  }
...
__owur static int ecp_nistz256_get_affine(const EC_GROUP* group,
  ...
  if (x != NULL) {
    ...
    if (!bn_set_words(x, x_ret, P256_LIMBS)) return 0;
  }
  ...
  if (y != NULL) {
    ...
    if (!bn_set_words(y, y_ret, P256_LIMBS)) return 0;
  }
...
static int ecp_nistz256_inv_mod_ord(const EC_GROUP* group, BIGNUM* r,
   ...
   */
  if (!bn_set_words(r, out, P256_LIMBS)) goto err;
...
static int ecp_nistz256group_full_init(EC_GROUP* group,
  ...
  if (!EC_POINT_copy(group->generator, P)) goto err;
  if (!BN_copy(group->order, order)) goto err;
  if (!BN_set_word(group->cofactor, 1)) goto err;

  ordmont = BN_MONT_CTX_new();
  if (ordmont == NULL) goto err;
