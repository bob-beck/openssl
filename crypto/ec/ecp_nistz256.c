                                          ...
                                          BN_CTX *ctx)
{
    ...
    if (!ecp_nistz256_bignum_to_field_elem(point_x, point->X) ||
        !ecp_nistz256_bignum_to_field_elem(point_y, point->Y) || !ecp_nistz256_bignum_to_field_elem(point_z, point->Z))
    {
        ERR_raise(ERR_LIB_EC, EC_R_COORDINATES_OUT_OF_RANGE);
        return 0;
    }
...
static int ecp_nistz256group_full_init(EC_GROUP *group, const unsigned char *params)
{
    ...
    if ((order = BN_bin2bn(params + 5 * param_len, param_len, NULL)) == NULL || !BN_set_word(x, (BN_ULONG)1))
    { /* cofactor is 1 */
        ERR_raise(ERR_LIB_EC, ERR_R_BN_LIB);
        goto err;
    }
