...
static void ecp_sm2p256_point_P_mul_by_scalar(P256_POINT* R, const BN_ULONG* k,
  ...
  for (i = 64 - 1; i >= 0; --i) {
    ...
    if (init == 0) {
      ...
      ecp_sm2p256_point_double(R, R);
      ecp_sm2p256_point_double(R, R);
      if (index) ecp_sm2p256_point_add(R, R, &precomputed[index]);
    }
  }
}
...
static int ecp_sm2p256_get_affine(const EC_GROUP* group, const EC_POINT* point,
  ...
  if (x != NULL) {
    ...
    if (!bn_set_words(x, x_aff, P256_LIMBS)) return 0;
  }

  if (y != NULL) {
    ecp_sm2p256_mul(z_inv3, z_inv3, z_inv2);
    ecp_sm2p256_mul(y_aff, point_y, z_inv3);
    if (!bn_set_words(y, y_aff, P256_LIMBS)) return 0;
  }
...
static int ecp_sm2p256_windowed_mul(const EC_GROUP* group, P256_POINT* r,
  ...
  for (i = 0; i < num; i++) {
    if (EC_POINT_is_at_infinity(group, point[i])) continue;

    if ((BN_num_bits(scalar[i]) > 256) || BN_is_negative(scalar[i])) {
      BIGNUM* tmp;

      if ((tmp = BN_CTX_get(ctx)) == NULL) goto err;
...
static int ecp_sm2p256_points_mul(const EC_GROUP* group, EC_POINT* r,
  ...
  if (num) {
    ...
    if (!p_is_infinity) ecp_sm2p256_point_add(&p.p, &p.p, out);
  }
...
static int ecp_sm2p256_field_mul(const EC_GROUP* group, BIGNUM* r,
  ...
  if (!bn_set_words(r, r_fe, P256_LIMBS)) return 0;
