...
#define is_point_equal(a, b) is_equal(a->X, b->X) && is_equal(a->Y, b->Y) && is_equal(a->Z, b->Z)
...
static int ecp_sm2p256_get_affine(const EC_GROUP *group, const EC_POINT *point, BIGNUM *x, BIGNUM *y, BN_CTX *ctx)
{
    ...
    if (ecp_sm2p256_bignum_field_elem(point_x, point->X) <= 0 ||
        ecp_sm2p256_bignum_field_elem(point_y, point->Y) <= 0 || ecp_sm2p256_bignum_field_elem(point_z, point->Z) <= 0)
    {
                                    ...
                                    size_t num, BN_CTX *ctx)
{
    ...
    if (num > OPENSSL_MALLOC_MAX_NELEMS(P256_POINT) || (scalars = OPENSSL_malloc_array(num, sizeof(BIGNUM *))) == NULL)
    {
