...
void EC_GROUP_free(EC_GROUP* group) {
  ...
  if (group->meth->group_finish != 0) group->meth->group_finish(group);
...
int EC_GROUP_copy(EC_GROUP* dest, const EC_GROUP* src) {
  ...
  if (src->seed) {
    ...
    if ((dest->seed = OPENSSL_malloc(src->seed_len)) == NULL) return 0;
...
static int ec_guess_cofactor(EC_GROUP* group) {
  ...
  if ((ctx = BN_CTX_new_ex(group->libctx)) == NULL) return 0;

  BN_CTX_start(ctx);
  if ((q = BN_CTX_get(ctx)) == NULL) goto err;

  /* set q = 2**m for binary fields; q = p otherwise */
  if (group->meth->field_type == NID_X9_62_characteristic_two_field) {
    BN_zero(q);
    if (!BN_set_bit(q, BN_num_bits(group->field) - 1)) goto err;
  } else {
    if (!BN_copy(q, group->field)) goto err;
  }
...
int EC_GROUP_get_cofactor(const EC_GROUP* group, BIGNUM* cofactor,
                          BN_CTX* ctx) {
  if (group->cofactor == NULL) return 0;
  if (!BN_copy(cofactor, group->cofactor)) return 0;
...
size_t EC_GROUP_set_seed(EC_GROUP* group, const unsigned char* p, size_t len) {
  ...
  if ((group->seed = OPENSSL_malloc(len)) == NULL) return 0;
...
int EC_GROUP_cmp(const EC_GROUP* a, const EC_GROUP* b, BN_CTX* ctx) {
  ...
  if (EC_GROUP_get_field_type(a) != EC_GROUP_get_field_type(b)) return 1;
  /* compare the curve name (if present in both) */
  if (EC_GROUP_get_curve_name(a) && EC_GROUP_get_curve_name(b) &&
      EC_GROUP_get_curve_name(a) != EC_GROUP_get_curve_name(b))
    return 1;
  if (a->meth->flags & EC_FLAGS_CUSTOM_CURVE) return 0;

#ifndef FIPS_MODULE
  if (ctx == NULL) ctx_new = ctx = BN_CTX_new();
#endif
  ...
  if (!r) {
    ...
    if (!BN_is_zero(ac) && !BN_is_zero(bc) && BN_cmp(ac, bc) != 0) r = 1;
...
#else
  if (point->meth->point_finish != 0) point->meth->point_finish(point);
  OPENSSL_free(point);
#endif
}
...
int EC_POINTs_mul(const EC_GROUP* group, EC_POINT* r, const BIGNUM* scalar,
  ...
  if (scalar == NULL && num == 0) return EC_POINT_set_to_infinity(group, r);
...
#ifndef FIPS_MODULE
  if (ctx == NULL) ctx = new_ctx = BN_CTX_secure_new();
#endif
  if (ctx == NULL) {
...
int EC_GROUP_precompute_mult(EC_GROUP* group, BN_CTX* ctx) {
  if (group->meth->mul == 0) /* use default */
...
static int ec_field_inverse_mod_ord(const EC_GROUP* group, BIGNUM* r,
  ...
  BN_CTX_start(ctx);
  if ((e = BN_CTX_get(ctx)) == NULL) goto err;

  /*-
   * We want inverse in constant time, therefore we utilize the fact
   * order must be prime and use Fermats Little Theorem instead.
   */
  if (!BN_set_word(e, 2)) goto err;
  if (!BN_sub(e, group->order, e)) goto err;
  /*-
...
static EC_GROUP* ec_group_explicit_to_named(const EC_GROUP* group,
  ...
  if ((curve_name_nid = ossl_ec_curve_nid_from_params(dup, ctx)) != NID_undef) {
     ...
     */
    if (no_seed) {
      if (EC_GROUP_set_seed(ret_group, NULL, 0) != 1) goto err;
    }
  } else {
...
EC_GROUP* EC_GROUP_new_from_params(const OSSL_PARAM params[],
  ...
  if (ptmp != NULL) {
    ...
    if ((group = group_new_from_name(ptmp, libctx, propq)) == NULL) return NULL;
  ...
  if (ptmp != NULL) {
    ...
    if (!EC_GROUP_set_seed(group, ptmp->data, ptmp->data_size)) goto err;
  }
...
OSSL_PARAM* EC_GROUP_to_params(const EC_GROUP* group, OSSL_LIB_CTX* libctx,
  ...
  if (tmpl == NULL) goto err;

  if (bnctx == NULL) bnctx = new_bnctx = BN_CTX_new_ex(libctx);
