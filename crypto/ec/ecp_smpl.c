...
int
ossl_ec_GFp_simple_group_set_curve(EC_GROUP* group,
                                   const BIGNUM* p,
                                   const BIGNUM* a,
                                   const BIGNUM* b,
                                   BN_CTX* ctx)
...
int
ossl_ec_GFp_simple_group_get_curve(const EC_GROUP* group,
                                   BIGNUM* p,
                                   BIGNUM* a,
                                   BIGNUM* b,
                                   BN_CTX* ctx)
...
int
ossl_ec_GFp_simple_group_check_discriminant(const EC_GROUP* group, BN_CTX* ctx)
{
  int ret = 0;
...
int
ossl_ec_GFp_simple_point_set_to_infinity(const EC_GROUP* group, EC_POINT* point)
{
  point->Z_is_one = 0;
  BN_zero(point->Z);
  return 1;
...
int
ossl_ec_GFp_simple_set_Jprojective_coordinates_GFp(const EC_GROUP* group,
                                                   EC_POINT* point,
                                                   const BIGNUM* x,
                                                   const BIGNUM* y,
                                                   const BIGNUM* z,
...
int
ossl_ec_GFp_simple_add(const EC_GROUP* group,
                       EC_POINT* r,
                       const EC_POINT* a,
                       const EC_POINT* b,
                       BN_CTX* ctx)
{
  int (*field_mul)(
    const EC_GROUP*, BIGNUM*, const BIGNUM*, const BIGNUM*, BN_CTX*);
...
int
ossl_ec_GFp_simple_dbl(const EC_GROUP* group,
                       EC_POINT* r,
                       const EC_POINT* a,
                       BN_CTX* ctx)
{
...
int
ossl_ec_GFp_simple_is_at_infinity(const EC_GROUP* group, const EC_POINT* point)
{
  return BN_is_zero(point->Z);
}

int
ossl_ec_GFp_simple_is_on_curve(const EC_GROUP* group,
                               const EC_POINT* point,
                               BN_CTX* ctx)
{
  int (*field_mul)(
...
int
ossl_ec_GFp_simple_cmp(const EC_GROUP* group,
                       const EC_POINT* a,
                       const EC_POINT* b,
                       BN_CTX* ctx)
{
...
int
ossl_ec_GFp_simple_make_affine(const EC_GROUP* group,
                               EC_POINT* point,
                               BN_CTX* ctx)
{
  BN_CTX* new_ctx = NULL;
...
int
ossl_ec_GFp_simple_points_make_affine(const EC_GROUP* group,
                                      size_t num,
                                      EC_POINT* points[],
                                      BN_CTX* ctx)
{
  ...
  for (i = 1; i < num; i++) {
    if (!BN_is_zero(points[i]->Z)) {
      if (!group->meth->field_mul(
            group, prod_Z[i], prod_Z[i - 1], points[i]->Z, ctx))
        goto err;
    } else {
...
int
ossl_ec_GFp_simple_field_mul(const EC_GROUP* group,
                             BIGNUM* r,
                             const BIGNUM* a,
                             const BIGNUM* b,
                             BN_CTX* ctx)
...
int
ossl_ec_GFp_simple_field_sqr(const EC_GROUP* group,
                             BIGNUM* r,
                             const BIGNUM* a,
                             BN_CTX* ctx)
{
 ...
 */
int
ossl_ec_GFp_simple_field_inv(const EC_GROUP* group,
                             BIGNUM* r,
                             const BIGNUM* a,
                             BN_CTX* ctx)
{
