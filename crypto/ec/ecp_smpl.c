...
int ossl_ec_GFp_simple_add(const EC_GROUP* group, EC_POINT* r,
  ...
  if (a == b) return EC_POINT_dbl(group, r, a, ctx);
  if (EC_POINT_is_at_infinity(group, a)) return EC_POINT_copy(r, b);
  if (EC_POINT_is_at_infinity(group, b)) return EC_POINT_copy(r, a);
...
int ossl_ec_GFp_simple_is_on_curve(const EC_GROUP* group, const EC_POINT* point,
  ...
  if (EC_POINT_is_at_infinity(group, point)) return 1;
...
int ossl_ec_GFp_simple_cmp(const EC_GROUP* group, const EC_POINT* a,
  ...
  if (EC_POINT_is_at_infinity(group, b)) return 1;
...
int ossl_ec_GFp_simple_make_affine(const EC_GROUP* group, EC_POINT* point,
  ...
  if (point->Z_is_one || EC_POINT_is_at_infinity(group, point)) return 1;
  ...
  if (!EC_POINT_get_affine_coordinates(group, point, x, y, ctx)) goto err;
  if (!EC_POINT_set_affine_coordinates(group, point, x, y, ctx)) goto err;
  if (!point->Z_is_one) {
...
int ossl_ec_GFp_simple_field_inv(const EC_GROUP* group, BIGNUM* r,
  ...
  do {
    if (!BN_priv_rand_range_ex(e, group->field, 0, ctx)) goto err;
  } while (BN_is_zero(e));

  /* r := a * e */
  if (!group->meth->field_mul(group, r, a, e, ctx)) goto err;
  /* r := 1/(a * e) */
  ...
  /* r := e/(a * e) = 1/a */
  if (!group->meth->field_mul(group, r, r, e, ctx)) goto err;
...
int ossl_ec_GFp_simple_ladder_pre(const EC_GROUP* group, EC_POINT* r,
  ...
  do {
    if (!BN_priv_rand_range_ex(r->Y, group->field, 0, ctx)) return 0;
  } while (BN_is_zero(r->Y));

  /* make sure lambda (s->Z here for storage) is not zero */
  do {
    if (!BN_priv_rand_range_ex(s->Z, group->field, 0, ctx)) return 0;
...
int ossl_ec_GFp_simple_ladder_post(const EC_GROUP* group, EC_POINT* r,
  ...
  if (BN_is_zero(r->Z)) return EC_POINT_set_to_infinity(group, r);

  if (BN_is_zero(s->Z)) {
    if (!EC_POINT_copy(r, p) || !EC_POINT_invert(group, r, ctx)) return 0;
    return 1;
  }
  ...
  if (group->meth->field_set_to_one != NULL) {
    if (!group->meth->field_set_to_one(group, r->Z, ctx)) goto err;
  } else {
