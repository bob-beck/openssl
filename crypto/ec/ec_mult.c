 ...
 */
int
ossl_ec_scalar_mul_ladder (const EC_GROUP *group, EC_POINT *r,
 ...
 */
int
ossl_ec_wNAF_mul (const EC_GROUP *group, EC_POINT *r, const BIGNUM *scalar,
  ...
  for (i = 0; i < num + num_scalar; i++)
    {
      ...
      wNAF[i] = bn_compute_wNAF ((i < num ? scalars[i] : scalar),
                                 (int)wsize[i], &wNAF_len[i]);
      if (wNAF[i] == NULL)
        goto err;
                  ...
                  if (r_is_at_infinity)
                    {
                       ...
                       *
                       * The underlying EC_METHOD can optionally implement this
                       * function: ossl_ec_point_blind_coordinates() returns 0
                       * in case of errors or 1 on success or if coordinate
                       * blinding is not implemented for this group.
                       */
 ...
 */
int
ossl_ec_wNAF_precompute_mult (EC_GROUP *group, BN_CTX *ctx)
{
