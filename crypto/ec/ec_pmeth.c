...
static int pkey_ec_copy(EVP_PKEY_CTX* dst, const EVP_PKEY_CTX* src) {
  ...
  if (!pkey_ec_init(dst)) return 0;
  sctx = src->data;
  dctx = dst->data;
  if (sctx->gen_group) {
    dctx->gen_group = EC_GROUP_dup(sctx->gen_group);
    if (!dctx->gen_group) return 0;
  }
  dctx->md = sctx->md;

  if (sctx->co_key) {
    dctx->co_key = EC_KEY_dup(sctx->co_key);
    if (!dctx->co_key) return 0;
  }
...
static int pkey_ec_sign(EVP_PKEY_CTX* ctx, unsigned char* sig, size_t* siglen,
  ...
  if (!ossl_assert(sig_sz > 0)) return 0;
...
static int pkey_ec_kdf_derive(EVP_PKEY_CTX* ctx, unsigned char* key,
  ...
  if (!pkey_ec_derive(ctx, ktmp, &ktmplen)) goto err;
  /* Do KDF stuff */
  ...
  switch (type) {
    ...
    case EVP_PKEY_CTRL_EC_ECDH_COFACTOR:
      ...
      if (p1 != -1) {
        ...
        if (!ec_key->group) return -2;
        /* If cofactor is 1 cofactor mode does nothing */
        if (BN_is_one(ec_key->group->cofactor)) return 1;
        if (!dctx->co_key) {
...
static int pkey_ec_ctrl_str(EVP_PKEY_CTX* ctx, const char* type,
  ...
  if (strcmp(type, "ec_paramgen_curve") == 0) {
    ...
    if (nid == NID_undef) nid = OBJ_sn2nid(value);
    if (nid == NID_undef) nid = OBJ_ln2nid(value);
    if (nid == NID_undef) {
      ERR_raise(ERR_LIB_EC, EC_R_INVALID_CURVE);
      return 0;
    }
