...
static void felem_inv(felem out, const felem in) {
  ...
  for (i = 0; i < 3; i++) felem_square_reduce(ftmp, ftmp); /* 2^5 + 2^4 + 2^3 */
...
static void felem_contract(felem out, const felem in) {
   ...
   */
  for (i = 0; i < 5; i++) cond[i] = ((cond[i] & 0x00ffffffffffffff) - 1) >> 63;
...
static void batch_mul(felem x_out, felem y_out, felem z_out,
  ...
  for (i = (num_points ? 380 : 98); i >= 0; --i) {
    /* double */
    if (!skip) point_double(nq[0], nq[1], nq[2], nq[0], nq[1], nq[2]);
...
int ossl_ec_GFp_nistp384_group_set_curve(EC_GROUP* group, const BIGNUM* p,
  ...
  if (ctx == NULL) ctx = new_ctx = BN_CTX_new();
#endif
...
int ossl_ec_GFp_nistp384_points_mul(const EC_GROUP* group, EC_POINT* r,
  ...
  if (tmp_scalar == NULL) goto err;

  if (scalar != NULL) {
    pre = group->pre_comp.nistp384;
    if (pre) /* we have precomputation, try to use it */
      g_pre_comp = &pre->g_pre_comp[0];
    else
      /* try to use the standard precomputation */
      g_pre_comp = (felem(*)[3])gmul;
    generator = EC_POINT_new(group);
    if (generator == NULL) goto err;
  ...
  if (num_points > 0) {
    ...
    if (mixed) make_points_affine(num_points * 17, pre_comp[0], tmp_felems);
  }
...
int ossl_ec_GFp_nistp384_precompute_mult(EC_GROUP* group, BN_CTX* ctx) {
  ...
  if (!EC_POINT_set_affine_coordinates(group, generator, x, y, ctx)) goto err;
  if ((pre = nistp384_pre_comp_new()) == NULL) goto err;
  /*
