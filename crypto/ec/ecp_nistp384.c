 ...
 * Reduction causes all but the final limb to be reduced to contain a value
 * less than 2^56, with the final value represented allowed to be larger than
 * 2^384, inasmuch as we can be sure that arithmetic overflow remains
...
static void felem_mul_wrapper (widefelem out, const felem in1,
                               const felem in2);
...
felem_inv (felem out, const felem in)
{
  ...
  for (i = 0; i < 2; i++)
    felem_square_reduce (ftmp, ftmp); /* 2^383 + ... + 2^129 + 2^127 + ... +
                                         2^96 + 2^31 + ... + 2^2 */
 ...
 */
static void
select_point (const limb idx, unsigned int size, const felem pre_comp[][3],
              felem out[3])
{
  unsigned int i, j;
...
batch_mul (felem x_out, felem y_out, felem z_out,
           const felem_bytearray scalars[], const unsigned int num_points,
...
int
ossl_ec_GFp_nistp384_group_set_curve (EC_GROUP *group, const BIGNUM *p,
 ...
 */
int
ossl_ec_GFp_nistp384_points_mul (const EC_GROUP *group, EC_POINT *r,
