...
static void
bin48_to_felem(felem out, const u8 in[48])
{
  memset(out, 0, 56);
...
static void
felem_to_bin48(u8 out[48], const felem in)
{
  memset(out, 0, 48);
...
static void
felem_scalar128(widefelem out, limb scalar)
{
  unsigned int i;
...
void
p384_felem_diff64(felem out, const felem in);
void
p384_felem_diff128(widefelem out, const widefelem in);
void
p384_felem_diff_128_64(widefelem out, const felem in);
 ...
 */
static void
felem_diff128(widefelem out, const widefelem in)
{
  /*
   * In order to prevent underflow, we add a multiple of p before subtracting.
...
static void
felem_square_ref(widefelem out, const felem in)
{
  felem inx2;
...
static void
felem_mul_ref(widefelem out, const felem in1, const felem in2)
{
 ...
 */
static void
felem_reduce_ref(felem out, const widefelem in)
{
  /*
   * In order to prevent underflow, we add a multiple of p before subtracting.
...
felem_square_reduce_ref(felem out, const felem in)
{
  widefelem tmp;
...
static ossl_inline void
felem_mul_reduce_ref(felem out, const felem in1, const felem in2)
{
  widefelem tmp;
...
#if defined(ECP_NISTP384_ASM)
static void
felem_square_wrapper(widefelem out, const felem in);
static void
felem_mul_wrapper(widefelem out, const felem in1, const felem in2);

static void (*felem_square_p)(widefelem out,
                              const felem in) = felem_square_wrapper;
static void (*felem_mul_p)(widefelem out,
                           const felem in1,
...
static void (*felem_mul_reduce_p)(felem out,
                                  const felem in1,
                                  const felem in2) = felem_mul_reduce_ref;

void
p384_felem_square(widefelem out, const felem in);
void
p384_felem_mul(widefelem out, const felem in1, const felem in2);
void
p384_felem_reduce(felem out, const widefelem in);

void
p384_felem_square_reduce(felem out, const felem in);
void
p384_felem_mul_reduce(felem out, const felem in1, const felem in2);
...
felem_inv(felem out, const felem in)
{
  ...
  felem_mul_reduce(
    ftmp, ftmp6, ftmp); /* 2^285 + ... + 2^31 + 2^29 + ... + 2^0 */

  for (i = 0; i < 2; i++)
    felem_square_reduce(ftmp, ftmp); /* 2^287 + ... + 2^33 + 2^31 + ... + 2^2 */
  felem_mul_reduce(
    ftmp, ftmp2, ftmp); /* 2^287 + ... + 2^33 + 2^31 + ... + 2^0 */
  ...
  felem_mul_reduce(ftmp, in, ftmp); /* 2^383 + ... + 2^129 + 2^127 + ... + 2^96
                                       + 2^31 + ... + 2^2 + 2^0 */
  ...
  for (i = (num_points ? 380 : 98); i >= 0; --i) {
    ...
    if (gen_mul && (i <= 98)) {
      ...
      if (!skip) {
        ...
        point_add(
          nq[0], nq[1], nq[2], nq[0], nq[1], nq[2], 1, tmp[0], tmp[1], tmp[2]);
      } else {
...
static NISTP384_PRE_COMP*
nistp384_pre_comp_new(void)
{
...
NISTP384_PRE_COMP*
ossl_ec_nistp384_pre_comp_dup(NISTP384_PRE_COMP* p)
{
  int i;
...
void
ossl_ec_nistp384_pre_comp_free(NISTP384_PRE_COMP* p)
{
  int i;
...
static void
make_points_affine(size_t num, felem points[][3], felem tmp_felems[])
{
  /*
