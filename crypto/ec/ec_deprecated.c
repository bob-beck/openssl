...
BIGNUM* EC_POINT_point2bn(const EC_GROUP* group, const EC_POINT* point,
  ...
  if (buf_len == 0 || buf_len > INT_MAX) return NULL;
...
EC_POINT* EC_POINT_bn2point(const EC_GROUP* group, const BIGNUM* bn,
  ...
  if ((buf_len = BN_num_bytes(bn)) == 0) buf_len = 1;
  if ((buf = OPENSSL_malloc(buf_len)) == NULL) return NULL;
  ...
  if (!EC_POINT_oct2point(group, ret, buf, buf_len, ctx)) {
    if (ret != point) EC_POINT_clear_free(ret);
    OPENSSL_free(buf);
    return NULL;
  }
