...
static int
ec_GFp_s390x_nistp_mul(const EC_GROUP* group,
                       EC_POINT* r,
                       const BIGNUM* scalar,
                       size_t num,
                           ...
                           int len)
{
  ...
  if (r == NULL || kinv == NULL) {
    ...
    if (RAND_priv_bytes_ex(
          eckey->libctx, param + S390X_OFF_RN(len), (size_t)len, 0) != 1) {
...
static int
ecdsa_s390x_nistp_verify_sig(const unsigned char* dgst,
                             int dgstlen,
                             const ECDSA_SIG* sig,
                             EC_KEY* eckey,
                             unsigned int fc,
...
#define EC_GFP_S390X_NISTP_METHOD(bits)                                        \
  ...
  static int ec_GFp_s390x_nistp##bits##_mul(const EC_GROUP* group,             \
                                            ...
                                            BN_CTX* ctx)                       \
  {                                                                            \
    return ec_GFp_s390x_nistp_mul(group,                                       \
                                  r,                                           \
                                  scalar,                                      \
                                  num,                                         \
                                  points,                                      \
  ...
  static ECDSA_SIG* ecdsa_s390x_nistp##bits##_sign_sig(                        \
    ...
    EC_KEY* eckey)                                                             \
  {                                                                            \
    return ecdsa_s390x_nistp_sign_sig(dgst,                                    \
                                      dgstlen,                                 \
                                      kinv,                                    \
                                      r,                                       \
                                      eckey,                                   \
  ...
  static int ecdsa_s390x_nistp##bits##_verify_sig(const unsigned char* dgst,   \
                                                  ...
                                                  EC_KEY* eckey)               \
  {                                                                            \
    return ecdsa_s390x_nistp_verify_sig(dgst,                                  \
                                        dgstlen,                               \
                                        sig,                                   \
                                        eckey,                                 \
