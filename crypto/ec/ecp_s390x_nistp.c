                                        ...
                                        unsigned int fc, int len)
{
    ...
    if (group->meth->point_get_affine_coordinates(group, pubkey, x, y, ctx) != 1 ||
        ...
        BN_bn2binpad(x, param + S390X_OFF_X(len), len) == -1 || BN_bn2binpad(y, param + S390X_OFF_Y(len), len) == -1)
    {
        ERR_raise(ERR_LIB_EC, ERR_R_BN_LIB);
        goto ret;
    }
...
#define EC_GFP_S390X_NISTP_METHOD(bits)                                                                                \
                                                                                                                       \
    static int ec_GFp_s390x_nistp##bits##_mul(const EC_GROUP *group, EC_POINT *r, const BIGNUM *scalar, size_t num,    \
                                              const EC_POINT *points[], const BIGNUM *scalars[], BN_CTX *ctx)          \
    {                                                                                                                  \
        return ec_GFp_s390x_nistp_mul(group, r, scalar, num, points, scalars, ctx, S390X_SCALAR_MULTIPLY_P##bits,      \
                                      S390X_SIZE_P##bits);                                                             \
    }                                                                                                                  \
                                                                                                                       \
    static ECDSA_SIG *ecdsa_s390x_nistp##bits##_sign_sig(const unsigned char *dgst, int dgstlen, const BIGNUM *kinv,   \
                                                         const BIGNUM *r, EC_KEY *eckey)                               \
    {                                                                                                                  \
        return ecdsa_s390x_nistp_sign_sig(dgst, dgstlen, kinv, r, eckey, S390X_ECDSA_SIGN_P##bits,                     \
                                          S390X_SIZE_P##bits);                                                         \
    }                                                                                                                  \
                                                                                                                       \
    static int ecdsa_s390x_nistp##bits##_verify_sig(const unsigned char *dgst, int dgstlen, const ECDSA_SIG *sig,      \
                                                    EC_KEY *eckey)                                                     \
    {                                                                                                                  \
        return ecdsa_s390x_nistp_verify_sig(dgst, dgstlen, sig, eckey, S390X_ECDSA_VERIFY_P##bits,                     \
                                            S390X_SIZE_P##bits);                                                       \
    }                                                                                                                  \
                                                                                                                       \
    const EC_METHOD *EC_GFp_s390x_nistp##bits##_method(void)                                                           \
    {                                                                                                                  \
        static const EC_METHOD EC_GFp_s390x_nistp##bits##_meth = {EC_FLAGS_DEFAULT_OCT,                                \
