...
int ossl_ec_GF2m_simple_add(const EC_GROUP* group, EC_POINT* r,
  ...
  if (a->Z_is_one) {
    ...
    if (!EC_POINT_get_affine_coordinates(group, a, x0, y0, ctx)) goto err;
  }
  if (b->Z_is_one) {
    if (!BN_copy(x1, b->X)) goto err;
    if (!BN_copy(y1, b->Y)) goto err;
  } else {
    if (!EC_POINT_get_affine_coordinates(group, b, x1, y1, ctx)) goto err;
  }
  ...
  if (!EC_POINT_set_affine_coordinates(group, r, x2, y2, ctx)) goto err;
...
int ossl_ec_GF2m_simple_cmp(const EC_GROUP* group, const EC_POINT* a,
  ...
  if (!EC_POINT_get_affine_coordinates(group, a, aX, aY, ctx)) goto err;
  if (!EC_POINT_get_affine_coordinates(group, b, bX, bY, ctx)) goto err;
...
static int ec_GF2m_simple_points_mul(const EC_GROUP* group, EC_POINT* r,
  ...
  if (scalar != NULL && num == 0) /* Fixed point multiplication */
    return ossl_ec_scalar_mul_ladder(group, r, scalar, NULL, ctx);

  if (scalar == NULL && num == 1) /* Variable point multiplication */
