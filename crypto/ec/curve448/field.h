...
static INLINE_UNUSED void
gf_copy(gf out, const gf a)
{
  *out = *a;
}
...
static INLINE_UNUSED void
gf_bias(gf inout, int amount);
static INLINE_UNUSED void
gf_weak_reduce(gf inout);

void
gf_strong_reduce(gf inout);
void
gf_add(gf out, const gf a, const gf b);
void
gf_sub(gf out, const gf a, const gf b);
void
ossl_gf_mul(gf_s* RESTRICT out, const gf a, const gf b);
void
ossl_gf_mulw_unsigned(gf_s* RESTRICT out, const gf a, uint32_t b);
void
ossl_gf_sqr(gf_s* RESTRICT out, const gf a);
mask_t
gf_isr(
  gf a,
  const gf x); /** a^2 x = 1, QNR, or 0 if x=0.  Return true if successful */
mask_t
gf_eq(const gf x, const gf y);
mask_t
gf_lobit(const gf x);
mask_t
gf_hibit(const gf x);
...
void
gf_serialize(uint8_t serial[SER_BYTES], const gf x, int with_highbit);
mask_t
gf_deserialize(gf x,
...
gf_sqrn(gf_s* RESTRICT y, const gf x, int n)
{
  gf tmp;
...
static ossl_inline void
gf_sub_nr(gf c, const gf a, const gf b)
{
  gf_sub_RAW(c, a, b);
  gf_bias(c, 2);
  if (GF_HEADROOM < 3)
...
static ossl_inline void
gf_subx_nr(gf c, const gf a, const gf b, int amt)
{
  gf_sub_RAW(c, a, b);
  gf_bias(c, amt);
...
static ossl_inline void
gf_mulw(gf c, const gf a, int32_t w)
{
  if (w > 0) {
...
static ossl_inline void
gf_cond_sel(gf x, const gf y, const gf z, mask_t is_z)
{
  size_t i;
...
static ossl_inline void
gf_cond_swap(gf x, gf_s* RESTRICT y, mask_t swap)
{
  size_t i;
