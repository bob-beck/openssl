...
static void
clamp(uint8_t secret_scalar_ser[EDDSA_448_PRIVATE_BYTES])
{
...
static c448_error_t
hash_init_with_dom(OSSL_LIB_CTX* ctx,
                   EVP_MD_CTX* hashctx,
                   uint8_t prehashed,
                   uint8_t for_prehash,
  ...
  const char* propq)
{
  ...
  return oneshot_hash(
    ctx, x, X448_PRIVATE_BYTES, ed, EDDSA_448_PRIVATE_BYTES, propq);
}
  ...
  const char* propq)
{
  ...
  ossl_curve448_scalar_decode_long(
    secret_scalar, secret_scalar_ser, sizeof(secret_scalar_ser));
  ...
  ossl_curve448_precomputed_scalarmul(
    p, ossl_curve448_precomputed_base, secret_scalar);
                     ...
                     const char* propq)
{
...

  {
    ...
    clamp(expanded);
    ossl_curve448_scalar_decode_long(
      secret_scalar, expanded, EDDSA_448_PRIVATE_BYTES);

    /* Hash to create the nonce */
    if (!hash_init_with_dom(
          ctx, hashctx, prehashed, 0, context, context_len, propq) ||
...

  {
    ...
    ossl_curve448_scalar_decode_long(
      challenge_scalar, challenge, sizeof(challenge));
...
ossl_c448_ed448_sign_prehash(OSSL_LIB_CTX* ctx,
                             ...
                             const char* propq)
{
  return ossl_c448_ed448_sign(
    ctx, signature, privkey, pubkey, hash, 64, 1, context, context_len, propq);
}
                       ...
                       const char* propq)
{
  ...
  ossl_curve448_scalar_sub(
    challenge_scalar, ossl_curve448_scalar_zero, challenge_scalar);
  ...
  const char* propq)
{
  return ossl_c448_ed448_verify(
    ctx, signature, pubkey, hash, 64, 1, context, context_len, propq);
}
...
ossl_ed448_sign(OSSL_LIB_CTX* ctx,
                ...
                const char* propq)
{
  return ossl_c448_ed448_sign(ctx,
                              out_sig,
                              private_key,
                              public_key,
                              message,
...
ossl_ed448_verify(OSSL_LIB_CTX* ctx,
                  ...
                  const char* propq)
{
  return ossl_c448_ed448_verify(ctx,
                                signature,
                                public_key,
                                message,
                                message_len,
