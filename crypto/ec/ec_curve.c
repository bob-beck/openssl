...
static const ec_list_element* ec_curve_nid2curve(int nid) {
  ...
  for (i = 0; i < curve_list_length; i++) {
    if (curve_list[i].nid == nid) return &curve_list[i];
  }
  return NULL;
}
...
size_t EC_get_builtin_curves(EC_builtin_curve* r, size_t nitems) {
  ...
  if (r == NULL || nitems == 0) return curve_list_length;
...
int ossl_ec_curve_nid_from_params(const EC_GROUP* group, BN_CTX* ctx) {
  ...
  if (param_bytes == NULL) goto end;

  /* Create the bignums */
  for (i = 0; i < NUM_BN_FIELDS; ++i) {
    if ((bn[i] = BN_CTX_get(ctx)) == NULL) goto end;
  }
  /*
