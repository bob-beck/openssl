...
int ossl_ec_GFp_nist_group_set_curve(EC_GROUP* group, const BIGNUM* p,
  ...
  if (ctx == NULL)
    if ((ctx = new_ctx = BN_CTX_new_ex(group->libctx)) == NULL) return 0;
...
int ossl_ec_GFp_nist_field_mul(const EC_GROUP* group, BIGNUM* r,
  ...
  if (!ctx)
    if ((ctx_new = ctx = BN_CTX_new_ex(group->libctx)) == NULL) goto err;

  if (!BN_mul(r, a, b, ctx)) goto err;
  if (!group->field_mod_func(r, r, group->field, ctx)) goto err;
