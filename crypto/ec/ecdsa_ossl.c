                    ...
                    EC_KEY* eckey)
{
  ...
  return eckey->group->meth->ecdsa_sign_sig(
...
int
ossl_ecdsa_deterministic_sign(const unsigned char* dgst,
                              int dlen,
                              unsigned char* sig,
                              unsigned int* siglen,
                              EC_KEY* eckey,
      ...
      if (dgst != NULL) {
        if (nonce_type == 1) {
          ...
          res = ossl_bn_gen_dsa_nonce_fixed_top(
            k, order, priv_key, dgst, dlen, ctx);
        }
      } else {
...
ossl_ecdsa_simple_sign_setup(EC_KEY* eckey,
                             ...
                             BIGNUM** rp)
{
  return ecdsa_sign_setup(
    eckey, ctx_in, kinvp, rp, NULL, 0, 0, NULL, NULL, NULL);
}
                           ...
                           EC_KEY* eckey)
{
  ...
  do {
    if (in_kinv == NULL || in_r == NULL) {
      if (!ecdsa_sign_setup(
            eckey, ctx, &kinv, &ret->r, dgst, dgst_len, 0, NULL, NULL, NULL)) {
        ERR_raise(ERR_LIB_EC, ERR_R_ECDSA_LIB);
        goto err;
      }
      ckinv = kinv;
