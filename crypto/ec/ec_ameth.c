...
static int
eckey_priv_decode_ex (EVP_PKEY *pkey, const PKCS8_PRIV_KEY_INFO *p8,
...
static int
ec_pkey_export_to (const EVP_PKEY *from, void *to_keydata,
  ...
  if (pub_point != NULL)
    {
      ...
      if ((pub_key_buflen
           = EC_POINT_point2buf (ecg, pub_point, format, &pub_key_buf, bnctx))
              == 0
...
const EVP_PKEY_ASN1_METHOD ossl_sm2_asn1_meth
    = { EVP_PKEY_SM2, EVP_PKEY_EC, ASN1_PKEY_ALIAS };
#endif
