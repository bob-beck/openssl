...
eckey_pub_encode(X509_PUBKEY* pk, const EVP_PKEY* pkey)
{
  ...
  if (X509_PUBKEY_set0_param(
        pk, OBJ_nid2obj(EVP_PKEY_EC), ptype, pval, penc, penclen))
    return 1;
err:
...
static int
eckey_priv_decode_ex(EVP_PKEY* pkey,
...
eckey_priv_encode(PKCS8_PRIV_KEY_INFO* p8, const EVP_PKEY* pkey)
{
  ...
  if (!PKCS8_pkey_set0(
        p8, OBJ_nid2obj(NID_X9_62_id_ecPublicKey), 0, ptype, pval, ep, eplen)) {
...
ec_pkey_ctrl(EVP_PKEY* pkey, int op, long arg1, void* arg2)
{
  switch (op) {
    ...
    case ASN1_PKEY_CTRL_GET1_TLS_ENCPT:
      return (int)EC_KEY_key2buf(
        EVP_PKEY_get0_EC_KEY(pkey), POINT_CONVERSION_UNCOMPRESSED, arg2, NULL);
...
static int
ec_pkey_export_to(const EVP_PKEY* from,
                  void* to_keydata,
                  ...
                  const char* propq)
{
  ...
  if (pub_point != NULL) {
    ...
    if ((pub_key_buflen = EC_POINT_point2buf(
           ecg, pub_point, format, &pub_key_buf, bnctx)) == 0 ||
        !OSSL_PARAM_BLD_push_octet_string(
          tmpl, OSSL_PKEY_PARAM_PUB_KEY, pub_key_buf, pub_key_buflen))
      goto err;
  ...
  if (priv_key != NULL) {
    ...
    sz = (ecbits + 7) / 8;
    if (!OSSL_PARAM_BLD_push_BN_pad(
          tmpl, OSSL_PKEY_PARAM_PRIV_KEY, priv_key, sz))
      goto err;
    ...
    if (!OSSL_PARAM_BLD_push_int(
          tmpl, OSSL_PKEY_PARAM_USE_COFACTOR_ECDH, ecdh_cofactor_mode))
      goto err;
...
const EVP_PKEY_ASN1_METHOD ossl_sm2_asn1_meth = { EVP_PKEY_SM2,
                                                  EVP_PKEY_EC,
                                                  ASN1_PKEY_ALIAS };
#endif
