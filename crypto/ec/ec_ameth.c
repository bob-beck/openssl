...
static int eckey_pub_encode(X509_PUBKEY* pk, const EVP_PKEY* pkey) {
  ...
  if (X509_PUBKEY_set0_param(pk, OBJ_nid2obj(EVP_PKEY_EC), ptype, pval, penc,
    ...
    return 1;
err:
  if (ptype == V_ASN1_SEQUENCE) ASN1_STRING_free(pval);
  OPENSSL_free(penc);
  return 0;
}
...
static int eckey_pub_cmp(const EVP_PKEY* a, const EVP_PKEY* b) {
  ...
  if (group == NULL || pa == NULL || pb == NULL) return -2;
...
static int ec_security_bits(const EVP_PKEY* pkey) {
  ...
  if (ecbits >= 512) return 256;
  if (ecbits >= 384) return 192;
  if (ecbits >= 256) return 128;
  if (ecbits >= 224) return 112;
  if (ecbits >= 160) return 80;
  return ecbits / 2;
}
...
static int ec_copy_parameters(EVP_PKEY* to, const EVP_PKEY* from) {
  ...
  if (to->pkey.ec == NULL) {
    ...
    if (to->pkey.ec == NULL) goto err;
  }
  if (EC_KEY_set_group(to->pkey.ec, group) == 0) goto err;
  EC_GROUP_free(group);
  return 1;
err:
  EC_GROUP_free(group);
...
static int ec_cmp_parameters(const EVP_PKEY* a, const EVP_PKEY* b) {
  ...
  if (group_a == NULL || group_b == NULL) return -2;
...
static int do_EC_KEY_print(BIO* bp, const EC_KEY* x, int off,
  ...
  if (privlen != 0) {
    ...
    if (ASN1_buf_print(bp, priv, privlen, off + 4) == 0) goto err;
  }

  if (publen != 0) {
    if (BIO_printf(bp, "%*spub:\n", off, "") <= 0) goto err;
    if (ASN1_buf_print(bp, pub, publen, off + 4) == 0) goto err;
  }

  if (!ECPKParameters_print(bp, group, off)) goto err;
  ret = 1;
err:
  if (!ret) ERR_raise(ERR_LIB_EC, ERR_R_EC_LIB);
...
static int eckey_param_decode(EVP_PKEY* pkey, const unsigned char** pder,
  ...
  if ((eckey = d2i_ECParameters(NULL, pder, derlen)) == NULL) return 0;
  EVP_PKEY_assign_EC_KEY(pkey, eckey);
  return 1;
}
...
static int old_ec_priv_decode(EVP_PKEY* pkey, const unsigned char** pder,
  ...
  if ((ec = d2i_ECPrivateKey(NULL, pder, derlen)) == NULL) return 0;
...
static int ec_pkey_ctrl(EVP_PKEY* pkey, int op, long arg1, void* arg2) {
  switch (op) {
    ...
    case ASN1_PKEY_CTRL_SET1_TLS_ENCPT:
      ...
      if (!ossl_assert(evp_pkey_is_legacy(pkey))) return 0;
