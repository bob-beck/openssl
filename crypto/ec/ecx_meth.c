...
static int ecx_pub_cmp(const EVP_PKEY* a, const EVP_PKEY* b) {
  ...
  if (akey == NULL || bkey == NULL) return -2;
    ...
    case ASN1_PKEY_CTRL_GET1_TLS_ENCPT:
      if (pkey->pkey.ecx != NULL) {
        ...
        *ppt = OPENSSL_memdup(pkey->pkey.ecx->pubkey, KEYLEN(pkey));
        if (*ppt != NULL) return KEYLEN(pkey);
      }
      return 0;
...
static int ecx_get_pub_key(const EVP_PKEY* pkey, unsigned char* pub,
  ...
  if (key == NULL || *len < (size_t)KEYLENID(pkey->ameth->pkey_id)) return 0;
...
static int ecx_pkey_copy(EVP_PKEY* to, EVP_PKEY* from) {
  ...
  if (!ret) ossl_ecx_key_free(dupkey);
  return ret;
}
...
static int pkey_ecx_ctrl(EVP_PKEY_CTX* ctx, int type, int p1, void* p2) {
  ...
  if (type == EVP_PKEY_CTRL_PEER_KEY) return 1;
  return -2;
}
...
static int pkey_ecd_ctrl(EVP_PKEY_CTX* ctx, int type, int p1, void* p2) {
  switch (type) {
    case EVP_PKEY_CTRL_MD:
      ...
      if (p2 == NULL || (const EVP_MD*)p2 == EVP_md_null()) return 1;
...
static int s390x_pkey_ecx_keygen25519(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey) {
  ...
  if (RAND_priv_bytes_ex(ctx->libctx, privkey, X25519_KEYLEN, 0) <= 0) goto err;

  privkey[0] &= 248;
  privkey[31] &= 127;
  privkey[31] |= 64;

  if (s390x_x25519_mul(pubkey, generator, privkey) != 1) goto err;
...
static int s390x_pkey_ecx_keygen448(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey) {
  ...
  if (RAND_priv_bytes_ex(ctx->libctx, privkey, X448_KEYLEN, 0) <= 0) goto err;

  privkey[0] &= 252;
  privkey[55] |= 128;

  if (s390x_x448_mul(pubkey, generator, privkey) != 1) goto err;
...
static int s390x_pkey_ecd_keygen448(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey) {
  ...
  if (RAND_priv_bytes_ex(ctx->libctx, privkey, ED448_KEYLEN, 0) <= 0) goto err;

  hashctx = EVP_MD_CTX_new();
  if (hashctx == NULL) goto err;
  ...
  if (EVP_DigestUpdate(hashctx, privkey, 57) != 1) goto err;
  if (EVP_DigestFinalXOF(hashctx, buff, sizeof(buff)) != 1) goto err;
