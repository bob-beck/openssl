...
static int
ecx_pub_encode(X509_PUBKEY* pk, const EVP_PKEY* pkey)
{
...
static int
ecx_pub_decode(EVP_PKEY* pkey, const X509_PUBKEY* pubkey)
{
  ...
  if (!X509_PUBKEY_get0_param(NULL, &p, &pklen, &palg, pubkey))
    return 0;
  ecx = ossl_ecx_key_op(
    palg, p, pklen, pkey->ameth->pkey_id, KEY_OP_PUBLIC, NULL, NULL);
  if (ecx != NULL) {
    ret = 1;
...
static int
ecx_pub_cmp(const EVP_PKEY* a, const EVP_PKEY* b)
{
...
static int
ecx_priv_decode_ex(EVP_PKEY* pkey,
...
static int
ecx_priv_encode(PKCS8_PRIV_KEY_INFO* p8, const EVP_PKEY* pkey)
{
  const ECX_KEY* ecxkey = pkey->pkey.ecx;
...
static int
ecx_cmp_parameters(const EVP_PKEY* a, const EVP_PKEY* b)
{
  return 1;
}

static int
ecx_key_print(BIO* bp,
              const EVP_PKEY* pkey,
              int indent,
              ASN1_PCTX* ctx,
              ecx_key_op_t op)
...
static int
ecx_priv_print(BIO* bp, const EVP_PKEY* pkey, int indent, ASN1_PCTX* ctx)
{
  return ecx_key_print(bp, pkey, indent, ctx, KEY_OP_PRIVATE);
}

static int
ecx_pub_print(BIO* bp, const EVP_PKEY* pkey, int indent, ASN1_PCTX* ctx)
{
  ...
  switch (op) {
    ...
    case ASN1_PKEY_CTRL_SET1_TLS_ENCPT: {
      ECX_KEY* ecx = ossl_ecx_key_op(
        NULL, arg2, arg1, pkey->ameth->pkey_id, KEY_OP_PUBLIC, NULL, NULL);
...
static int
ecd_ctrl(EVP_PKEY* pkey, int op, long arg1, void* arg2)
{
  switch (op) {
...
static int
ecx_set_priv_key(EVP_PKEY* pkey, const unsigned char* priv, size_t len)
{
  ...
  ecx = ossl_ecx_key_op(
    NULL, priv, (int)len, pkey->ameth->pkey_id, KEY_OP_PRIVATE, libctx, NULL);
...
static int
ecx_set_pub_key(EVP_PKEY* pkey, const unsigned char* pub, size_t len)
{
  OSSL_LIB_CTX* libctx = NULL;
  ECX_KEY* ecx = NULL;
  ...
  ecx = ossl_ecx_key_op(
    NULL, pub, (int)len, pkey->ameth->pkey_id, KEY_OP_PUBLIC, libctx, NULL);
...
static int
ecx_get_priv_key(const EVP_PKEY* pkey, unsigned char* priv, size_t* len)
{
...
static int
ecx_get_pub_key(const EVP_PKEY* pkey, unsigned char* pub, size_t* len)
{
  const ECX_KEY* key = pkey->pkey.ecx;
...
static size_t
ecx_pkey_dirty_cnt(const EVP_PKEY* pkey)
{
  /*
...
static int
ecx_pkey_export_to(const EVP_PKEY* from,
                   void* to_keydata,
                   ...
                   const char* propq)
{
  ...
  if (!OSSL_PARAM_BLD_push_octet_string(
        tmpl, OSSL_PKEY_PARAM_PUB_KEY, key->pubkey, key->keylen))
    goto err;
  selection |= OSSL_KEYMGMT_SELECT_PUBLIC_KEY;

  if (key->privkey != NULL) {
    if (!OSSL_PARAM_BLD_push_octet_string(
          tmpl, OSSL_PKEY_PARAM_PRIV_KEY, key->privkey, key->keylen))
      goto err;
...
static int
ecx_generic_import_from(const OSSL_PARAM params[], void* vpctx, int keytype)
{
...
static int
ecx_pkey_copy(EVP_PKEY* to, EVP_PKEY* from)
{
...
static int
x25519_import_from(const OSSL_PARAM params[], void* vpctx)
{
...
static int
x448_import_from(const OSSL_PARAM params[], void* vpctx)
{
...
static int
ecd_size25519(const EVP_PKEY* pkey)
{
  return ED25519_SIGSIZE;
}

static int
ecd_size448(const EVP_PKEY* pkey)
{
  return ED448_SIGSIZE;
}

static int
ecd_item_verify(EVP_MD_CTX* ctx,
                const ASN1_ITEM* it,
                const void* asn,
...
static int
ecd_item_sign(X509_ALGOR* alg1, X509_ALGOR* alg2, int nid)
{
...
static int
ecd_item_sign25519(EVP_MD_CTX* ctx,
                   const ASN1_ITEM* it,
                   const void* asn,
                   X509_ALGOR* alg1,
                   X509_ALGOR* alg2,
...
static int
ecd_sig_info_set25519(X509_SIG_INFO* siginf,
                      const X509_ALGOR* alg,
                      const ASN1_STRING* sig)
{
  X509_SIG_INFO_set(
    siginf, NID_undef, NID_ED25519, X25519_SECURITY_BITS, X509_SIG_INFO_TLS);
  return 1;
}

static int
ecd_item_sign448(EVP_MD_CTX* ctx,
                 const ASN1_ITEM* it,
                 const void* asn,
                 X509_ALGOR* alg1,
                 X509_ALGOR* alg2,
...
static int
ecd_sig_info_set448(X509_SIG_INFO* siginf,
                    const X509_ALGOR* alg,
                    const ASN1_STRING* sig)
{
  X509_SIG_INFO_set(
    siginf, NID_undef, NID_ED448, X448_SECURITY_BITS, X509_SIG_INFO_TLS);
  return 1;
}

static int
ed25519_import_from(const OSSL_PARAM params[], void* vpctx)
{
...
static int
ed448_import_from(const OSSL_PARAM params[], void* vpctx)
{
...
static int
pkey_ecx_keygen(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey)
{
  ECX_KEY* ecx = ossl_ecx_key_op(
    NULL, NULL, 0, ctx->pmeth->pkey_id, KEY_OP_KEYGEN, NULL, NULL);
...
static int
validate_ecx_derive(EVP_PKEY_CTX* ctx,
                    unsigned char* key,
                    size_t* keylen,
...
static int
pkey_ecx_derive25519(EVP_PKEY_CTX* ctx, unsigned char* key, size_t* keylen)
{
...
static int
pkey_ecx_derive448(EVP_PKEY_CTX* ctx, unsigned char* key, size_t* keylen)
{
  const unsigned char *privkey, *pubkey;
...
static int
pkey_ecx_ctrl(EVP_PKEY_CTX* ctx, int type, int p1, void* p2)
{
...
static int
pkey_ecd_digestsign25519(EVP_MD_CTX* ctx,
                         unsigned char* sig,
                         size_t* siglen,
...
static int
pkey_ecd_digestsign448(EVP_MD_CTX* ctx,
                       unsigned char* sig,
                       size_t* siglen,
                       const unsigned char* tbs,
                       size_t tbslen)
{
  ...
  if (ossl_ed448_sign(edkey->libctx,
                      sig,
                      tbs,
                      tbslen,
                      edkey->pubkey,
...
static int
pkey_ecd_digestverify25519(EVP_MD_CTX* ctx,
                           ...
                           size_t tbslen)
{
  ...
  return ossl_ed25519_verify(tbs,
                             tbslen,
                             sig,
                             edkey->pubkey,
                             0,
...
static int
pkey_ecd_digestverify448(EVP_MD_CTX* ctx,
                         const unsigned char* sig,
                         size_t siglen,
                         const unsigned char* tbs,
                         size_t tbslen)
{
  ...
  return ossl_ed448_verify(
    edkey->libctx, tbs, tbslen, sig, edkey->pubkey, NULL, 0, 0, edkey->propq);
}

static int
pkey_ecd_ctrl(EVP_PKEY_CTX* ctx, int type, int p1, void* p2)
{
  switch (type) {
...
static int
s390x_pkey_ecx_keygen25519(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey)
{
...
static int
s390x_pkey_ecx_keygen448(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey)
{
  static const unsigned char generator[] = {
...
static int
s390x_pkey_ecd_keygen25519(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey)
{
...
static int
s390x_pkey_ecd_keygen448(EVP_PKEY_CTX* ctx, EVP_PKEY* pkey)
{
  static const unsigned char generator_x[] = {
...
static int
s390x_pkey_ecx_derive25519(EVP_PKEY_CTX* ctx,
                           unsigned char* key,
                           size_t* keylen)
{
  const unsigned char *privkey, *pubkey;
...
static int
s390x_pkey_ecx_derive448(EVP_PKEY_CTX* ctx, unsigned char* key, size_t* keylen)
{
  const unsigned char *privkey, *pubkey;
...
static int
s390x_pkey_ecd_digestsign25519(EVP_MD_CTX* ctx,
                               unsigned char* sig,
                               size_t* siglen,
                               const unsigned char* tbs,
                               size_t tbslen)
...
static int
s390x_pkey_ecd_digestsign448(EVP_MD_CTX* ctx,
                             unsigned char* sig,
                             size_t* siglen,
                             const unsigned char* tbs,
                             size_t tbslen)
...
static int
s390x_pkey_ecd_digestverify25519(EVP_MD_CTX* ctx,
                                 const unsigned char* sig,
                                 size_t siglen,
                                 const unsigned char* tbs,
                                 size_t tbslen)
...
static int
s390x_pkey_ecd_digestverify448(EVP_MD_CTX* ctx,
                               const unsigned char* sig,
                               size_t siglen,
                               const unsigned char* tbs,
                               size_t tbslen)
