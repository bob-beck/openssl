...
int
ossl_ec_encoding_name2id(const char* name)
{
  size_t i, sz;
...
static char*
ec_param_encoding_id2name(int id)
{
  size_t i, sz;
...
char*
ossl_ec_check_group_type_id2name(int id)
{
  size_t i, sz;
...
static int
ec_check_group_type_name2id(const char* name)
{
  size_t i, sz;
...
int
ossl_ec_set_check_group_type_from_name(EC_KEY* ec, const char* name)
{
...
static int
ec_set_check_group_type_from_param(EC_KEY* ec, const OSSL_PARAM* p)
{
...
int
ossl_ec_pt_format_name2id(const char* name)
{
  size_t i, sz;
...
char*
ossl_ec_pt_format_id2name(int id)
{
  size_t i, sz;
...
static int
ec_group_explicit_todata(const EC_GROUP* group,
                         OSSL_PARAM_BLD* tmpl,
                         OSSL_PARAM params[],
                         BN_CTX* bnctx,
                         unsigned char** genbuf)
{
  ...
  if (tmpl != NULL || param != NULL) {
    ...
    if (!ossl_param_build_set_bn(
          tmpl, params, OSSL_PKEY_PARAM_EC_ORDER, order)) {
      ERR_raise(ERR_LIB_EC, ERR_R_CRYPTO_LIB);
      goto err;
    }
  }
...
int
ossl_ec_group_todata(const EC_GROUP* group,
                     OSSL_PARAM_BLD* tmpl,
                     OSSL_PARAM params[],
                     OSSL_LIB_CTX* libctx,
                     const char* propq,
                     ...
                     unsigned char** genbuf)
{
  ...
  if (pt_form_name == NULL || !ossl_param_build_set_utf8_string(
                                tmpl,
                                params,
 ...
 */
int
ossl_ec_set_ecdh_cofactor_mode(EC_KEY* ec, int mode)
{
 ...
 */
int
ossl_ec_key_fromdata(EC_KEY* ec, const OSSL_PARAM params[], int include_private)
{
  ...
  if (param_pub_key != NULL)
    if (!OSSL_PARAM_get_octet_string(
          param_pub_key, (void**)&pub_key, 0, &pub_key_len) ||
...
int
ossl_ec_group_fromdata(EC_KEY* ec, const OSSL_PARAM params[])
{
  int ok = 0;
  EC_GROUP* group = NULL;
  ...
  group = EC_GROUP_new_from_params(
    params, ossl_ec_key_get_libctx(ec), ossl_ec_key_get0_propq(ec));
...
static int
ec_key_point_format_fromdata(EC_KEY* ec, const OSSL_PARAM params[])
{
  const OSSL_PARAM* p;
  int format = -1;
...
static int
ec_key_group_check_fromdata(EC_KEY* ec, const OSSL_PARAM params[])
{
  const OSSL_PARAM* p;
...
static int
ec_set_include_public(EC_KEY* ec, int include)
{
...
int
ossl_ec_key_otherparams_fromdata(EC_KEY* ec, const OSSL_PARAM params[])
{
  const OSSL_PARAM* p;
...
int
ossl_ec_key_is_foreign(const EC_KEY* ec)
{
#ifndef FIPS_MODULE
...
EC_KEY*
ossl_ec_key_dup(const EC_KEY* src, int selection)
{
  EC_KEY* ret;
  ...
  if ((ret = ossl_ec_key_new_method_int(
         src->libctx, src->propq, src->engine)) == NULL)
    return NULL;
...
int
ossl_ec_encoding_param2id(const OSSL_PARAM* p, int* id)
{
  const char* name = NULL;
  int status = 0;
...
int
ossl_ec_pt_format_param2id(const OSSL_PARAM* p, int* id)
{
  const char* name = NULL;
  int status = 0;
...
#ifndef FIPS_MODULE
int
ossl_x509_algor_is_sm2(const X509_ALGOR* palg)
{
  int ptype = 0;
...
EC_KEY*
ossl_ec_key_param_from_x509_algor(const X509_ALGOR* palg,
                                  OSSL_LIB_CTX* libctx,
                                  const char* propq)
{
  int ptype = 0;
...
EC_KEY*
ossl_ec_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO* p8inf,
                       OSSL_LIB_CTX* libctx,
                       const char* propq)
{
