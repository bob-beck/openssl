...
int ossl_ec_encoding_name2id(const char* name) {
  ...
  if (name == NULL) return OPENSSL_EC_NAMED_CURVE;
...
static char* ec_param_encoding_id2name(int id) {
  ...
  for (i = 0, sz = OSSL_NELEM(encoding_nameid_map); i < sz; i++) {
    if (id == (int)encoding_nameid_map[i].id) return encoding_nameid_map[i].ptr;
  }
  return NULL;
}
...
static int ec_set_check_group_type_from_param(EC_KEY* ec, const OSSL_PARAM* p) {
  ...
  if (status) return ossl_ec_set_check_group_type_from_name(ec, name);
  return 0;
}
...
int ossl_ec_pt_format_name2id(const char* name) {
  ...
  /* Return the default value if there is no name */
  if (name == NULL) return (int)POINT_CONVERSION_UNCOMPRESSED;
...
char* ossl_ec_pt_format_id2name(int id) {
  ...
  for (i = 0, sz = OSSL_NELEM(format_nameid_map); i < sz; i++) {
    if (id == (int)format_nameid_map[i].id) return format_nameid_map[i].ptr;
  }
  return NULL;
}
...
int ossl_ec_group_todata(const EC_GROUP* group, OSSL_PARAM_BLD* tmpl,
  ...
  if (tmpl == NULL || curve_nid == NID_undef)
    if (!ec_group_explicit_todata(group, tmpl, params, bnctx, genbuf)) goto err;
...
int ossl_ec_set_ecdh_cofactor_mode(EC_KEY* ec, int mode) {
  ...
  if (BN_is_one(cofactor)) return 1;
...
int ossl_ec_key_fromdata(EC_KEY* ec, const OSSL_PARAM params[],
  ...
  if (param_priv_key != NULL && include_private) {
    ...
    if (order == NULL || BN_is_zero(order)) goto err;

    fixed_words = bn_get_top(order) + 2;

    if ((priv_key = BN_secure_new()) == NULL) goto err;
    if (bn_wexpand(priv_key, fixed_words) == NULL) goto err;
    BN_set_flags(priv_key, BN_FLG_CONSTTIME);

    if (!OSSL_PARAM_get_BN(param_priv_key, &priv_key)) goto err;
  }

  if (priv_key != NULL && !EC_KEY_set_private_key(ec, priv_key)) goto err;

  if (pub_point != NULL && !EC_KEY_set_public_key(ec, pub_point)) goto err;
...
int ossl_ec_group_fromdata(EC_KEY* ec, const OSSL_PARAM params[]) {
  ...
  if (!EC_KEY_set_group(ec, group)) goto err;
  ok = 1;
err:
  EC_GROUP_free(group);
  return ok;
...
static int ec_key_group_check_fromdata(EC_KEY* ec, const OSSL_PARAM params[]) {
  ...
  if (p != NULL) return ec_set_check_group_type_from_param(ec, p);
  return 1;
}
...
int ossl_ec_key_otherparams_fromdata(EC_KEY* ec, const OSSL_PARAM params[]) {
  ...
  if (!ec_key_point_format_fromdata(ec, params)) return 0;
  if (!ec_key_group_check_fromdata(ec, params)) return 0;
  return 1;
}

int ossl_ec_key_is_foreign(const EC_KEY* ec) {
#ifndef FIPS_MODULE
  if (ec->engine != NULL || EC_KEY_get_method(ec) != EC_KEY_OpenSSL()) return 1;
#endif
  return 0;
}
...
EC_KEY* ossl_ec_key_dup(const EC_KEY* src, int selection) {
  ...
  if (src->group != NULL &&
    ...
    if (ret->group == NULL || !EC_GROUP_copy(ret->group, src->group)) goto err;

    if (src->meth != NULL) ret->meth = src->meth;
  }

  /*  copy the public key */
  if (src->pub_key != NULL &&
      (selection & OSSL_KEYMGMT_SELECT_PUBLIC_KEY) != 0) {
    if (ret->group == NULL) /* no parameter-less keys allowed */
      goto err;
  ...
  if (ret->meth != NULL && ret->meth->copy != NULL) {
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) !=
      ...
      goto err;
    if (ret->meth->copy(ret, src) == 0) goto err;
  }
...
int ossl_x509_algor_is_sm2(const X509_ALGOR* palg) {
  ...
  if (ptype == V_ASN1_OBJECT) return OBJ_obj2nid((ASN1_OBJECT*)pval) == NID_sm2;
...
EC_KEY* ossl_ec_key_param_from_x509_algor(const X509_ALGOR* palg,
  ...
  if (ptype == V_ASN1_SEQUENCE) {
    ...
    if (group == NULL) goto ecerr;
    EC_GROUP_set_asn1_flag(group, OPENSSL_EC_NAMED_CURVE);
    if (EC_KEY_set_group(eckey, group) == 0) goto ecerr;
    EC_GROUP_free(group);
  } else {
    ERR_raise(ERR_LIB_EC, EC_R_DECODE_ERROR);
    goto ecerr;
...
EC_KEY* ossl_ec_key_from_pkcs8(const PKCS8_PRIV_KEY_INFO* p8inf,
  ...
  if (!PKCS8_pkey_get0(NULL, &p, &pklen, &palg, p8inf)) return 0;
