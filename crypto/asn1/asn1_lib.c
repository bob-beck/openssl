...
int ASN1_get_object(const unsigned char** pp, long* plength, int* ptag,
  ...
  *pclass = xclass;
  ...
  if (inf && !(ret & V_ASN1_CONSTRUCTED)) goto err;
  ...
  if (*p == 0x80) {
    ...
    *inf = 0;
    ...
    if (*p++ & 0x80) {
      ...
      if (i > (int)sizeof(long)) return 0;
      while (i > 0) {
        ret <<= 8;
        ret |= *p++;
        i--;
      }
      if (ret > LONG_MAX) return 0;
    } else {
      ret = i;
    }
  }
...
void ossl_asn1_string_embed_free(ASN1_STRING* a, int embed) {
  if (a == NULL) return;
  if (!(a->flags & ASN1_STRING_FLAG_NDEF)) OPENSSL_free(a->data);
  if (embed == 0) OPENSSL_free(a);
}
...
int ASN1_STRING_cmp(const ASN1_STRING* a, const ASN1_STRING* b) {
  ...
  i = (a->length - b->length);
  if (i == 0) {
    if (a->length != 0) i = memcmp(a->data, b->data, a->length);
    if (i == 0)
...
char* ossl_sk_ASN1_UTF8STRING2text(STACK_OF(ASN1_UTF8STRING)* text,
  ...
  for (i = 0; i < sk_ASN1_UTF8STRING_num(text); i++) {
    ...
    if (i > 0) length += sep_len;
    length += ASN1_STRING_length(current);
    if (max_len != 0 && length > max_len) return NULL;
  }
  if ((result = OPENSSL_malloc(length + 1)) == NULL) return NULL;
