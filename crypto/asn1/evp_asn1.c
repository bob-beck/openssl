...
int ASN1_TYPE_get_octetstring(const ASN1_TYPE* a, unsigned char* data,
  ...
  if (ret < max_len)
    ...
    num = max_len;
  if (num > 0 && data != NULL) memcpy(data, p, num);
  return ret;
}
...
static int asn1_type_get_int_oct(ASN1_OCTET_STRING* oct, int32_t anum,
  ...
  if (data != NULL) memcpy(data, ASN1_STRING_get0_data(oct), n);
...
int ossl_asn1_type_get_octetstring_int(const ASN1_TYPE* a, long* num,
  ...
  if ((a->type != V_ASN1_SEQUENCE) || (a->value.sequence == NULL)) goto err;
