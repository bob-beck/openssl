...
static int uint64_new(ASN1_VALUE** pval, const ASN1_ITEM* it) {
  if ((*pval = (ASN1_VALUE*)OPENSSL_zalloc(sizeof(uint64_t))) == NULL) return 0;
  return 1;
}
...
static int uint64_c2i(ASN1_VALUE** pval, const unsigned char* cont, int len,
  ...
  if (*pval == NULL && !uint64_new(pval, it)) return 0;
   ...
   */
  if (len == 0) goto long_compat;

  if (!ossl_c2i_uint64_int(&utmp, &neg, &cont, len)) return 0;
  ...
  if (neg) /* ossl_c2i_uint64_int() returns positive values */
    utmp = 0 - utmp;
...
static int uint32_new(ASN1_VALUE** pval, const ASN1_ITEM* it) {
  if ((*pval = (ASN1_VALUE*)OPENSSL_zalloc(sizeof(uint32_t))) == NULL) return 0;
  return 1;
}
