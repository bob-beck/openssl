...
static int
asn1_print_info (BIO *bp, long offset, int depth, int hl, long len, int tag,
                 int xclass, int constructed, int indent)
{
  char str[128];
  const char *p;
  int pop_f_prefix = 0;
...
int
ASN1_parse_dump (BIO *bp, const unsigned char *pp, long len, int indent,
                 int dump)
{
          ...
          if ((j == 0x21) && (len == 0))
              ...
              for (;;)
                {
                  r = asn1_parse2 (bp, &p, (long)(tot - p),
                                   offset + (int)(p - *pp), depth + 1, indent,
                                   dump);
                  if (r == 0)
                    goto end;
                  if ((r == 2) || (p >= tot))
          ...
          else
            {
              ...
              while (p < ep)
                {
                  sp = p;
                  r = asn1_parse2 (bp, &p, tmp, offset + (int)(p - *pp),
                                   depth + 1, indent, dump);
                  if (r == 0)
                    goto end;
                  tmp -= (long)(p - sp);
