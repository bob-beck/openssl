...
err:
  if (saved_indent >= 0) BIO_set_indent(bp, saved_indent);
  if (pop_f_prefix) BIO_pop(bp);
  BIO_free(bio);
  return i;
}
...
static int asn1_parse2(BIO* bp, const unsigned char** pp, long length,
  ...
  while (length > 0) {
    ...
    if (j & V_ASN1_CONSTRUCTED) {
      ...
      if ((j == 0x21) && (len == 0)) {
        for (;;) {
          r = asn1_parse2(bp, &p, (long)(tot - p), offset + (int)(p - *pp),
                          depth + 1, indent, dump);
          if (r == 0) goto end;
          if ((r == 2) || (p >= tot)) {
            len = (long)(p - sp);
        ...
        while (p < ep) {
          sp = p;
          r = asn1_parse2(bp, &p, tmp, offset + (int)(p - *pp), depth + 1,
                          indent, dump);
          if (r == 0) goto end;
          tmp -= (long)(p - sp);
        }
      ...
      if ((tag == V_ASN1_PRINTABLESTRING) || (tag == V_ASN1_T61STRING) ||
        ...
        if (d2i_ASN1_OBJECT(&o, &opp, len + hl) != NULL) {
          ...
          if (BIO_puts(bp, ":BAD OBJECT") <= 0) goto end;
          dump_cont = 1;
        }
      } else if (tag == V_ASN1_BOOLEAN) {
        if (len != 1) {
          if (BIO_puts(bp, ":BAD BOOLEAN") <= 0) goto end;
          dump_cont = 1;
        }
        if (len > 0) BIO_printf(bp, ":%u", p[0]);
        ...
        if (os != NULL && os->length > 0) {
          ...
          {
            if (BIO_write(bp, ":", 1) <= 0) goto end;
            if (BIO_write(bp, (const char*)opp, os->length) <= 0) goto end;
          } else if (!dump)
          /*
           * not printable => print octet string as hex dump
           */
          {
            if (BIO_write(bp, "[HEX DUMP]:", 11) <= 0) goto end;
            for (i = 0; i < os->length; i++) {
              if (BIO_printf(bp, "%02X", opp[i]) <= 0) goto end;
            }
          } else
        ...
        if (ai != NULL) {
          ...
          for (i = 0; i < ai->length; i++) {
            if (BIO_printf(bp, "%02X", ai->data[i]) <= 0) goto end;
          }
          if (ai->length == 0) {
            if (BIO_write(bp, "00", 2) <= 0) goto end;
          }
        } else {
          if (BIO_puts(bp, ":BAD INTEGER") <= 0) goto end;
          dump_cont = 1;
        }
        ...
        if (ae != NULL) {
          ...
          for (i = 0; i < ae->length; i++) {
            if (BIO_printf(bp, "%02X", ae->data[i]) <= 0) goto end;
          }
          if (ae->length == 0) {
            if (BIO_write(bp, "00", 2) <= 0) goto end;
          }
        } else {
          if (BIO_puts(bp, ":BAD ENUMERATED") <= 0) goto end;
          dump_cont = 1;
        }
      ...
      if (dump_cont) {
        ...
        for (i = 0; i < len; i++) {
          if (BIO_printf(bp, "%02X", tmp[i]) <= 0) goto end;
        }
        if (BIO_puts(bp, "]") <= 0) goto end;
        dump_cont = 0;
      }
