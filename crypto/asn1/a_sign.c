...
int ASN1_item_sign_ex(const ASN1_ITEM* it, X509_ALGOR* algor1,
  ...
  if (!EVP_DigestSignInit(ctx, NULL, md, NULL, pkey)) goto err;
...
int ASN1_item_sign_ctx(const ASN1_ITEM* it, X509_ALGOR* algor1,
  ...
  if (pkey->ameth == NULL) {
    ...
    if (EVP_PKEY_CTX_get_params(pctx, params) <= 0) goto err;
     ...
     */
    if (rv <= 0) ERR_raise(ERR_LIB_ASN1, ERR_R_EVP_LIB);
    if (rv <= 1) goto err;
  } else {
    rv = 2;
  }
