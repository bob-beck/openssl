...
int ASN1_item_print(BIO* out, const ASN1_VALUE* ifld, int indent,
  ...
  const char* sname;
  if (pctx == NULL) pctx = &default_pctx;
...
static int asn1_item_print_ctx(BIO* out, const ASN1_VALUE** fld, int indent,
  ...
  switch (it->itype) {
    ...
    case ASN1_ITYPE_CHOICE:
      ...
      if (!asn1_template_print_ctx(out, tmpfld, indent, tt, pctx)) return 0;
      break;
...
static int asn1_print_fsname(BIO* out, int indent, const char* fname,
  ...
  while (indent > nspaces) {
    if (BIO_write(out, spaces, nspaces) != nspaces) return 0;
    indent -= nspaces;
  }
  if (BIO_write(out, spaces, indent) != indent) return 0;
  if (pctx->flags & ASN1_PCTX_FLAGS_NO_STRUCT_NAME) sname = NULL;
  if (pctx->flags & ASN1_PCTX_FLAGS_NO_FIELD_NAME) fname = NULL;
  if (!sname && !fname) return 1;
  if (fname) {
    if (BIO_puts(out, fname) <= 0) return 0;
  }
  if (sname) {
    if (fname) {
      if (BIO_printf(out, " (%s)", sname) <= 0) return 0;
    } else {
      if (BIO_puts(out, sname) <= 0) return 0;
    }
  }
...
static int asn1_print_boolean(BIO* out, int boolval) {
  ...
  if (BIO_puts(out, str) <= 0) return 0;
  return 1;
}
...
static int asn1_print_oid(BIO* out, const ASN1_OBJECT* oid) {
  ...
  if (BIO_printf(out, "%s (%s)", ln, objbuf) <= 0) return 0;
  return 1;
}
...
static int asn1_primitive_print(BIO* out, const ASN1_VALUE** fld,
  ...
  pf = it->funcs;
  if (!asn1_print_fsname(out, indent, fname, sname, pctx)) return 0;
  if (pf && pf->prim_print) return pf->prim_print(out, fld, it, indent, pctx);
  ...
  if (pname) {
    if (BIO_puts(out, pname) <= 0) return 0;
  ...
  switch (utype) {
    ...
    case V_ASN1_OTHER:
      if (BIO_puts(out, "\n") <= 0) return 0;
      if (ASN1_parse_dump(out, str->data, str->length, indent, 0) <= 0) ret = 0;
      needlf = 0;
      break;
