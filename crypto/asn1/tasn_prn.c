...
static int
asn1_template_print_ctx(BIO* out,
                        const ASN1_VALUE** fld,
                        int indent,
                        ...
                        const ASN1_PCTX* pctx)
{
  ...
  return asn1_item_print_ctx(
    out, fld, indent, ASN1_ITEM_ptr(tt->item), fname, sname, 0, pctx);
}
...
asn1_print_obstring(BIO* out, const ASN1_STRING* str, int indent)
{
  if (str->type == V_ASN1_BIT_STRING) {
    ...
    return 0;
  if ((str->length > 0) &&
      BIO_dump_indent(out, (const char*)str->data, str->length, indent + 2) <=
        0)
    return 0;
  return 1;
}
