...
static int asn1_item_flags_i2d(const ASN1_VALUE* val, unsigned char** out,
  ...
  if (out != NULL && *out == NULL) {
    ...
    if (len <= 0) return len;
    if ((buf = OPENSSL_malloc(len)) == NULL) return -1;
    p = buf;
...
int ASN1_item_ex_i2d(const ASN1_VALUE** pval, unsigned char** out,
  ...
  if ((it->itype != ASN1_ITYPE_PRIMITIVE) && *pval == NULL) return 0;
  ...
  switch (it->itype) {
    ...
    case ASN1_ITYPE_SEQUENCE:
      ...
      for (i = 0, tt = it->templates; i < it->tcount; tt++, i++) {
        ...
        if (tmplen == -1 || (tmplen > INT_MAX - seqcontlen)) return -1;
        seqcontlen += tmplen;
      }

      seqlen = ASN1_object_size(ndef, seqcontlen, tag);
      if (!out || seqlen == -1) return seqlen;
...
static int asn1_template_ex_i2d(const ASN1_VALUE** pval, unsigned char** out,
  ...
  if (flags & ASN1_TFLG_TAG_MASK) {
    ...
    if (tag != -1) /* FIXME: error code here */
      return -1;
  ...
  if (flags & ASN1_TFLG_SK_MASK) {
    ...
    for (i = 0; i < sk_const_ASN1_VALUE_num(sk); i++) {
      ...
      if (len == -1 || (skcontlen > INT_MAX - len)) return -1;
    ...
    if (ndef == 2) {
      ASN1_put_eoc(out);
      if (flags & ASN1_TFLG_EXPTAG) ASN1_put_eoc(out);
    }
...
static int asn1_set_seq_out(STACK_OF(const_ASN1_VALUE)* sk, unsigned char** out,
  ...
  if (do_sort) {
    ...
    if (sk_const_ASN1_VALUE_num(sk) < 2)
      ...
      if (derlst == NULL) return 0;
      tmpdat = OPENSSL_malloc(skcontlen);
      if (tmpdat == NULL) goto err;
    }
  }
