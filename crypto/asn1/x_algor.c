...
X509_ALGOR* ossl_X509_ALGOR_from_nid(int nid, int ptype, void* pval) {
  ...
  if (X509_ALGOR_set0(alg, algo, ptype, pval)) return alg;
...
void X509_ALGOR_get0(const ASN1_OBJECT** paobj, int* pptype, const void** ppval,
  ...
  if (paobj) *paobj = algor->algorithm;
  if (pptype) {
    ...
    if (ppval) *ppval = algor->parameter->value.ptr;
  }
}
...
int X509_ALGOR_copy(X509_ALGOR* dest, const X509_ALGOR* src) {
  ...
  if (dest->algorithm) ASN1_OBJECT_free(dest->algorithm);
  dest->algorithm = NULL;

  if (dest->parameter) ASN1_TYPE_free(dest->parameter);
  dest->parameter = NULL;

  if (src->algorithm)
    if ((dest->algorithm = OBJ_dup(src->algorithm)) == NULL) return 0;

  if (src->parameter != NULL) {
    dest->parameter = ASN1_TYPE_new();
    if (dest->parameter == NULL) return 0;
...
int ossl_x509_algor_new_from_md(X509_ALGOR** palg, const EVP_MD* md) {
  ...
  if (md == NULL || EVP_MD_is_a(md, "SHA1")) return 1;
  if ((alg = X509_ALGOR_new()) == NULL) return 0;
...
const EVP_MD* ossl_x509_algor_get_md(X509_ALGOR* alg) {
  ...
  if (alg == NULL) return EVP_sha1();
  md = EVP_get_digestbyobj(alg->algorithm);
  if (md == NULL) ERR_raise(ERR_LIB_ASN1, ASN1_R_UNKNOWN_DIGEST);
  return md;
}

X509_ALGOR* ossl_x509_algor_mgf1_decode(X509_ALGOR* alg) {
  if (OBJ_obj2nid(alg->algorithm) != NID_mgf1) return NULL;
...
int ossl_x509_algor_md_to_mgf1(X509_ALGOR** palg, const EVP_MD* mgf1md) {
  ...
  *palg = NULL;
  if (mgf1md == NULL || EVP_MD_is_a(mgf1md, "SHA1")) return 1;
  /* need to embed algorithm ID inside another */
  if (!ossl_x509_algor_new_from_md(&algtmp, mgf1md)) goto err;
  if (ASN1_item_pack(algtmp, ASN1_ITEM_rptr(X509_ALGOR), &stmp) == NULL)
    goto err;
  *palg = ossl_X509_ALGOR_from_nid(NID_mgf1, V_ASN1_SEQUENCE, stmp);
  if (*palg == NULL) goto err;
  stmp = NULL;
err:
