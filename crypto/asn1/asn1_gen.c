...
ASN1_TYPE* ASN1_generate_nconf(const char* str, CONF* nconf) {
  ...
  if (!nconf) return ASN1_generate_v3(str, NULL);
...
ASN1_TYPE* ASN1_generate_v3(const char* str, X509V3_CTX* cnf) {
  ...
  if (err) ERR_raise(ERR_LIB_ASN1, err);
  return ret;
}
...
static ASN1_TYPE* generate_v3(const char* str, X509V3_CTX* cnf, int depth,
  ...
  if ((asn1_tags.imp_tag == -1) && (asn1_tags.exp_count == 0)) return ret;
...
static int asn1_cb(const char* elem, int len, void* bitstr) {
  ...
  switch (utype) {
    ...
    case ASN1_GEN_FLAG_EXP:
      ...
      if (!parse_tagging(vstart, vlen, &tmp_tag, &tmp_class)) return -1;
      if (!append_exp(arg, tmp_tag, tmp_class, 1, 0, 0)) return -1;
      break;
    ...
    case ASN1_GEN_FLAG_SETWRAP:
      if (!append_exp(arg, V_ASN1_SET, V_ASN1_UNIVERSAL, 1, 0, 1)) return -1;
      break;
...
static int parse_tagging(const char* vstart, int vlen, int* ptag, int* pclass) {
  ...
  if (eptr && *eptr && (eptr > vstart + vlen)) return 0;
  if (tag_num < 0) {
...
static ASN1_TYPE* asn1_multi(int utype, const char* section, X509V3_CTX* cnf,
  ...
  if (section) {
    ...
    for (i = 0; i < sk_CONF_VALUE_num(sect); i++) {
      ASN1_TYPE* typ = generate_v3(sk_CONF_VALUE_value(sect, i)->value, cnf,
                                   depth + 1, perr);
      if (!typ) goto bad;
      if (!sk_ASN1_TYPE_push(sk, typ)) goto bad;
    }
  }
  ...
  if (derlen < 0) goto bad;
  if ((ret = ASN1_TYPE_new()) == NULL) goto bad;
  if ((ret->value.asn1_string = ASN1_STRING_type_new(utype)) == NULL) goto bad;
...
static int asn1_str2tag(const char* tagstr, int len) {
  ...
  if (len == -1) len = (int)strlen(tagstr);
...
static int mask_cb(const char* elem, int len, void* arg) {
  ...
  if (!tag || (tag & ASN1_GEN_FLAG)) return 0;
