...
generate_v3(const char* str, X509V3_CTX* cnf, int depth, int* perr)
{
  ...
  for (i = 0, etmp = asn1_tags.exp_list; i < asn1_tags.exp_count; i++, etmp++) {
    ASN1_put_object(
      &p, etmp->exp_constructed, etmp->exp_len, etmp->exp_tag, etmp->exp_class);
    if (etmp->exp_pad)
      *p++ = 0;
  }
  ...
  if (asn1_tags.imp_tag != -1) {
    ...
    ASN1_put_object(
      &p, hdr_constructed, hdr_len, asn1_tags.imp_tag, asn1_tags.imp_class);
  }
...
static int
mask_cb(const char* elem, int len, void* arg)
{
...
int
ASN1_str2mask(const char* str, unsigned long* pmask)
{
  *pmask = 0;
