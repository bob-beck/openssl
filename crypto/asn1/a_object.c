...
int i2d_ASN1_OBJECT(const ASN1_OBJECT* a, unsigned char** pp) {
  ...
  if (pp == NULL || objsize == -1) return objsize;

  if (*pp == NULL) {
    if ((p = allocated = OPENSSL_malloc(objsize)) == NULL) return 0;
  } else {
    p = *pp;
  }
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      if (!use_bn && l >= ((ULONG_MAX - 80) / 10L)) {
        use_bn = 1;
        if (bl == NULL) bl = BN_new();
        if (bl == NULL || !BN_set_word(bl, l)) goto err;
      }
      if (use_bn) {
        if (!BN_mul_word(bl, 10L) || !BN_add_word(bl, c - '0')) goto err;
      } else
        l = l * 10L + (long)(c - '0');
    }
    if (len == 0) {
      ...
      if (use_bn) {
        if (!BN_add_word(bl, first * 40)) goto err;
      } else
        l += (long)first * 40;
    }
    i = 0;
...
int i2a_ASN1_OBJECT(BIO* bp, const ASN1_OBJECT* a) {
  ...
  if ((a == NULL) || (a->data == NULL)) return BIO_write(bp, "NULL", 4);
  ...
  if (i > (int)(sizeof(buf) - 1)) {
    ...
    if ((p = OPENSSL_malloc(i + 1)) == NULL) return -1;
    i2t_ASN1_OBJECT(p, i + 1, a);
  }
  if (i <= 0) {
    i = BIO_write(bp, "<INVALID>", 9);
    if (i > 0) i += BIO_dump(bp, (const char*)a->data, a->length);
    return i;
  }
  BIO_write(bp, p, i);
  if (p != buf) OPENSSL_free(p);
  return i;
}
...
err:
  ERR_raise(ERR_LIB_ASN1, i);
  if ((a == NULL) || (*a != ret)) ASN1_OBJECT_free(ret);
  return NULL;
}
...
void ASN1_OBJECT_free(ASN1_OBJECT* a) {
  ...
  if (a->flags & ASN1_OBJECT_FLAG_DYNAMIC) OPENSSL_free(a);
}
