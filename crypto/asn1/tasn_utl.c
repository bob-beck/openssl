...
int ossl_asn1_do_lock(ASN1_VALUE** pval, int op, const ASN1_ITEM* it) {
  ...
  if ((it->itype != ASN1_ITYPE_SEQUENCE) &&
    ...
    return 0;
  aux = it->funcs;
  if (aux == NULL || (aux->flags & ASN1_AFLG_REFCOUNT) == 0) return 0;
  lock = offset2ptr(*pval, aux->ref_lock);
  refcnt = offset2ptr(*pval, aux->ref_offset);

  switch (op) {
    case 0:
      if (!CRYPTO_NEW_REF(refcnt, 1)) return -1;
      *lock = CRYPTO_THREAD_lock_new();
      ...
      break;
    case 1:
      if (!CRYPTO_UP_REF(refcnt, &ret)) return -1;
      break;
    case -1:
      if (!CRYPTO_DOWN_REF(refcnt, &ret)) return -1; /* failed */
...
static ASN1_ENCODING* asn1_get_enc_ptr(ASN1_VALUE** pval, const ASN1_ITEM* it) {
  ...
  if (pval == NULL || *pval == NULL) return NULL;
  aux = it->funcs;
  if (aux == NULL || (aux->flags & ASN1_AFLG_ENCODING) == 0) return NULL;
...
int ossl_asn1_enc_save(ASN1_VALUE** pval, const unsigned char* in, long inlen,
  ...
  if ((enc->enc = OPENSSL_malloc(inlen)) == NULL) return 0;
...
int ossl_asn1_enc_restore(int* len, unsigned char** out,
  ...
  if (enc == NULL || enc->modified) return 0;
  if (out) {
    memcpy(*out, enc->enc, enc->len);
    *out += enc->len;
  }
  if (len != NULL) *len = enc->len;
  return 1;
}
...
const ASN1_TEMPLATE* ossl_asn1_do_adb(const ASN1_VALUE* val,
  ...
  if ((tt->flags & ASN1_TFLG_ADB_MASK) == 0) return tt;
...
err:
  ...
  if (nullerr) ERR_raise(ERR_LIB_ASN1, ASN1_R_UNSUPPORTED_ANY_DEFINED_BY_TYPE);
  return NULL;
}
