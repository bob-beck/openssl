...
static int asn1_item_ex_d2i_intern(ASN1_VALUE** pval, const unsigned char** in,
  ...
  rv = asn1_item_embed_d2i(pval, in, len, it, tag, aclass, opt, ctx, 0, libctx,
                           propq);
  if (rv <= 0) ASN1_item_ex_free(pval, it);
  return rv;
}
...
static int asn1_item_embed_d2i(ASN1_VALUE** pval, const unsigned char** in,
  ...
  switch (it->itype) {
    ...
    case ASN1_ITYPE_CHOICE:
      ...
      if (asn1_cb && !asn1_cb(ASN1_OP_D2I_PRE, pval, it, NULL)) goto auxerr;
      if (*pval) {
      ...
      for (i = 0, tt = it->templates; i < it->tcount; i++, tt++) {
         ...
         */
        ret = asn1_template_ex_d2i(pchptr, &p, len, tt, 1, ctx, depth, libctx,
                                   propq);
      ...
      if (asn1_cb && !asn1_cb(ASN1_OP_D2I_POST, pval, it, NULL)) goto auxerr;
      *in = p;
      return 1;
    ...
    case ASN1_ITYPE_SEQUENCE:
      ...
      /* Save encoding */
      if (!ossl_asn1_enc_save(pval, *in, (long)(p - *in), it)) goto auxerr;
      if (asn1_cb && !asn1_cb(ASN1_OP_D2I_POST, pval, it, NULL)) goto auxerr;
      *in = p;
      return 1;
...
static int asn1_d2i_ex_primitive(ASN1_VALUE** pval, const unsigned char** in,
  ...
  *in = p;
  ret = 1;
err:
  if (free_cont) OPENSSL_free(buf.data);
  return ret;
}
...
static int asn1_ex_c2i(ASN1_VALUE** pval, const unsigned char* cont, int len,
  ...
  switch (utype) {
    case V_ASN1_OBJECT:
      if (!ossl_c2i_ASN1_OBJECT((ASN1_OBJECT**)pval, &cont, len)) goto err;
      break;
    ...
    case V_ASN1_ENUMERATED:
      ...
      if (!ossl_c2i_ASN1_INTEGER(tint, &cont, len)) goto err;
...
static int asn1_collect(BUF_MEM* buf, const unsigned char** in, long len,
  ...
  while (len > 0) {
    ...
    if (cst) {
      ...
      if (!asn1_collect(buf, &p, plen, ininf, tag, aclass, depth + 1)) return 0;
...
static int asn1_check_tlen(long* olen, int* otag, unsigned char* oclass,
  ...
  if ((i & 1) != 0) plen = len - (long)(p - q);

  if (inf != NULL) *inf = i & 1;

  if (cst != NULL) *cst = i & V_ASN1_CONSTRUCTED;

  if (olen != NULL) *olen = plen;

  if (oclass != NULL) *oclass = pclass;

  if (otag != NULL) *otag = ptag;
