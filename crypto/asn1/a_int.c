...
static size_t i2c_ibuf(const unsigned char* b, size_t blen, int neg,
  ...
  if (pp == NULL || (p = *pp) == NULL) return ret;
...
static size_t c2i_ibuf(unsigned char* b, int* pneg, const unsigned char* p,
  ...
  if (b != NULL) twos_complement(b, p, plen, neg ? 0xffU : 0);
...
static int asn1_get_int64(int64_t* pr, const unsigned char* b, size_t blen,
                          int neg) {
  uint64_t r;
  if (asn1_get_uint64(&r, b, blen) == 0) return 0;
  if (neg) {
    if (r <= INT64_MAX) {
      /*
...
ASN1_INTEGER* ossl_c2i_ASN1_INTEGER(ASN1_INTEGER** a, const unsigned char** pp,
  ...
  *pp += len;
  ...
  return ret;
err:
  if (a == NULL || *a != ret) ASN1_INTEGER_free(ret);
  return NULL;
}
...
ASN1_INTEGER* d2i_ASN1_UINTEGER(ASN1_INTEGER** a, const unsigned char** pp,
  ...
  *pp = p;
  return ret;
err:
  if (i != 0) ERR_raise(ERR_LIB_ASN1, i);
  if ((a == NULL) || (*a != ret)) ASN1_INTEGER_free(ret);
  return NULL;
}
...
static ASN1_STRING* bn_to_asn1_string(const BIGNUM* bn, ASN1_STRING* ai,
  ...
  if (BN_is_negative(bn) && !BN_is_zero(bn)) ret->type |= V_ASN1_NEG_INTEGER;
...
static BIGNUM* asn1_string_to_bn(const ASN1_INTEGER* ai, BIGNUM* bn,
  ...
  if (ai->type & V_ASN1_NEG) BN_set_negative(ret, 1);
  return ret;
}
...
long ASN1_INTEGER_get(const ASN1_INTEGER* a) {
  ...
  if (i == 0) return -1;
  if (r > LONG_MAX || r < LONG_MIN) return -1;
  return (long)r;
}
...
long ASN1_ENUMERATED_get(const ASN1_ENUMERATED* a) {
  ...
  int64_t r;
  if (a == NULL) return 0;
  if ((a->type & ~V_ASN1_NEG) != V_ASN1_ENUMERATED) return -1;
  if (a->length > (int)sizeof(long)) return 0xffffffffL;
