...
static int asn1_bio_init(BIO_ASN1_BUF_CTX* ctx, int size) {
  ...
  if ((ctx->buf = OPENSSL_malloc(size)) == NULL) return 0;
  ctx->bufsize = size;
...
static int asn1_bio_write(BIO* b, const char* in, int inl) {
  ...
  ctx = BIO_get_data(b);
  next = BIO_next(b);
  if (in == NULL || inl < 0 || ctx == NULL || next == NULL) return 0;
  ...
  for (;;) {
    switch (ctx->state) {
      ...
      case ASN1_STATE_HEADER:
        ...
        if (!ossl_assert(ctx->buflen <= ctx->bufsize)) return -1;
        p = ctx->buf;
      ...
      case ASN1_STATE_DATA_COPY:
        ...
        if (ctx->copylen == 0) ctx->state = ASN1_STATE_HEADER;
...
static int asn1_bio_flush_ex(BIO* b, BIO_ASN1_BUF_CTX* ctx,
  ...
  for (;;) {
    ...
    if (ctx->ex_len > 0)
      ...
      if (cleanup) cleanup(b, &ctx->ex_buf, &ctx->ex_len, &ctx->ex_arg);
      ctx->state = next;
      ctx->ex_pos = 0;
      break;
    }
