...
static int
do_esc_char (unsigned long c, unsigned short flags, char *do_quotes,
...
static int
do_buf (unsigned char *buf, int buflen, int type, unsigned short flags,
          ...
          for (i = 0; i < utflen; i++)
            {
               ...
               */
              len = do_esc_char (utfbuf[i], flags | orflags, quotes, io_ch,
                                 arg);
              if (len < 0)
                return -1;
              outlen += len;
...
static int
do_dump (unsigned long lflags, char_io *io_ch, void *arg,
