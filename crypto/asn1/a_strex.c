...
static int do_dump(unsigned long lflags, char_io* io_ch, void* arg,
  ...
  if ((der_buf = OPENSSL_malloc(der_len)) == NULL) return -1;
  p = der_buf;
  i2d_ASN1_TYPE(&t, &p);
...
static int do_print_ex(char_io* io_ch, void* arg, unsigned long lflags,
  ...
  if (type == -1) {
    ...
    if (len < 0 || len > INT_MAX - outlen) return -1;
    outlen += len;
    return outlen;
  }
  ...
  if (len < 0 || len > INT_MAX - 2 - outlen) return -1;
  outlen += len;
  if (quotes) outlen += 2;
...
int X509_NAME_print_ex(BIO* out, const X509_NAME* nm, int indent,
                       unsigned long flags) {
  if (flags == XN_FLAG_COMPAT) return X509_NAME_print(out, nm, indent);
