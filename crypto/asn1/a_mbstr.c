...
static int
traverse_string(const unsigned char* p,
                int len,
                int inform,
...
int
ASN1_mbstring_copy(ASN1_STRING** out,
...
int
ASN1_mbstring_ncopy(ASN1_STRING** out,
                    const unsigned char* in,
                    int len,
                    int inform,
                    unsigned long mask,
                    ...
                    long maxsize)
{
  ...
  if ((minsize > 0) && (nchar < minsize)) {
    ERR_raise_data(
      ERR_LIB_ASN1, ASN1_R_STRING_TOO_SHORT, "minsize=%ld", minsize);
    return -1;
  }

  if ((maxsize > 0) && (nchar > maxsize)) {
    ERR_raise_data(
      ERR_LIB_ASN1, ASN1_R_STRING_TOO_LONG, "maxsize=%ld", maxsize);
    return -1;
  }
