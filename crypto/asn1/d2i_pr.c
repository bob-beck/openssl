...
static EVP_PKEY* d2i_PrivateKey_decoder(int keytype, EVP_PKEY** a,
  ...
  *pp = p;
  ...
  if (a != NULL && (bak_a = *a) != NULL) ppkey = a;
...
err:
  if (ppkey != a) EVP_PKEY_free(*ppkey);
  return NULL;
}
...
EVP_PKEY* ossl_d2i_PrivateKey_legacy(int keytype, EVP_PKEY** a,
  ...
  if (!ret->ameth->old_priv_decode ||
    ...
    if (ret->ameth->priv_decode != NULL || ret->ameth->priv_decode_ex != NULL) {
      ...
      ret = tmp;
      ERR_pop_to_mark();
      if (EVP_PKEY_type(keytype) != EVP_PKEY_get_base_id(ret)) goto err;
    } else {
      ERR_clear_last_mark();
  ...
  *pp = p;
  ...
  return ret;
err:
  if (a == NULL || *a != ret) EVP_PKEY_free(ret);
  return NULL;
}
