...
static EVP_PKEY*
d2i_PrivateKey_decoder(int keytype,
                       EVP_PKEY** a,
                       ...
                       const char* propq)
{
  ...
  *pp = p;
  ...
  dctx = OSSL_DECODER_CTX_new_for_pkey(
    ppkey, "DER", structure, key_name, EVP_PKEY_KEYPAIR, libctx, propq);
  if (a != NULL)
    *a = bak_a;
  if (dctx == NULL)
    goto err;
...
static EVP_PKEY*
d2i_AutoPrivateKey_legacy(EVP_PKEY** a,
                          const unsigned char** pp,
                          long length,
                          OSSL_LIB_CTX* libctx,
                          const char* propq)
 ...
 */
EVP_PKEY*
d2i_AutoPrivateKey_ex(EVP_PKEY** a,
                      const unsigned char** pp,
                      long length,
                      OSSL_LIB_CTX* libctx,
                      const char* propq)
