...
strip_end(char* name);
static MIME_HEADER*
mime_hdr_new(const char* name, const char* value);
static int
mime_hdr_addparam(MIME_HEADER* mhdr, const char* name, const char* value);
...
static int
mime_bound_check(char* line, int linelen, const char* bound, int blen);
static int
multi_split(BIO* bio, int flags, const char* bound, STACK_OF(BIO)** ret);
static int
strip_eol(char* linebuf, int* plen, int flags);
static MIME_HEADER*
mime_hdr_find(STACK_OF(MIME_HEADER)* hdrs, const char* name);
static MIME_PARAM*
mime_param_find(MIME_HEADER* hdr, const char* name);
static void
...
int
PEM_write_bio_ASN1_stream(BIO* out,
                          ASN1_VALUE* val,
                          BIO* in,
                          int flags,
                          const char* hdr,
                    ...
                    const char* propq)
{
  ...
  if ((flags & SMIME_DETACHED) && data) {
    ...
    if (!B64_write_ASN1(bio, val, NULL, 0, it) ||
        BIO_printf(
          bio, "%s------%s--%s%s", mime_eol, bound, mime_eol, mime_eol) < 0)
      return 0;
    return 1;
  }
                 ...
                 const ASN1_ITEM* it)
{
  return SMIME_write_ASN1_ex(
    bio, val, data, flags, ctype_nid, econt_nid, mdalgs, it, NULL, NULL);
}
                   ...
                   const char* propq)
{
  ...
  if (strcmp(hdr->value, "multipart/signed") == 0) {
    ...
    if (strcmp(hdr->value, "application/x-pkcs7-signature") &&
      ...
      ERR_raise_data(
        ERR_LIB_ASN1, ASN1_R_SIG_INVALID_MIME_TYPE, "type: %s", hdr->value);
      sk_MIME_HEADER_pop_free(headers, mime_hdr_free);
      sk_BIO_pop_free(parts, BIO_vfree);
      return NULL;
    }
  ...
  if (strcmp(hdr->value, "application/x-pkcs7-mime") &&
    ...
    ERR_raise_data(
      ERR_LIB_ASN1, ASN1_R_INVALID_MIME_TYPE, "type: %s", hdr->value);
    sk_MIME_HEADER_pop_free(headers, mime_hdr_free);
    return NULL;
  }
