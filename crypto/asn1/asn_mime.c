...
static ASN1_VALUE* b64_read_asn1(BIO* bio, const ASN1_ITEM* it, ASN1_VALUE** x,
  ...
  if (!val) ERR_raise(ERR_LIB_ASN1, ASN1_R_DECODE_ERROR);
  (void)BIO_flush(bio);
  BIO_pop(bio);
  BIO_free(b64);
  return val;
...
static int asn1_write_micalg(BIO* out, STACK_OF(X509_ALGOR)* mdalgs) {
  ...
  for (i = 0; i < sk_X509_ALGOR_num(mdalgs); i++) {
    ...
    switch (md_nid) {
      case NID_sha1:
        if (BIO_puts(out, "sha1") < 0) goto err;
        break;

      case NID_md5:
        if (BIO_puts(out, "md5") < 0) goto err;
        break;

      case NID_sha256:
        if (BIO_puts(out, "sha-256") < 0) goto err;
        break;

      case NID_sha384:
        if (BIO_puts(out, "sha-384") < 0) goto err;
        break;

      case NID_sha512:
        if (BIO_puts(out, "sha-512") < 0) goto err;
        break;

      case NID_id_GostR3411_94:
        if (BIO_puts(out, "gostr3411-94") < 0) goto err;
        break;

      case NID_id_GostR3411_2012_256:
        if (BIO_puts(out, "gostr3411-2012-256") < 0) goto err;
        break;

      case NID_id_GostR3411_2012_512:
        if (BIO_puts(out, "gostr3411-2012-512") < 0) goto err;
        break;
      ...
      default:
        if (have_unknown) {
          ...
          if (BIO_puts(out, "unknown") < 0) goto err;
          have_unknown = 1;
        }
        break;
    }
...
int SMIME_write_ASN1_ex(BIO* bio, ASN1_VALUE* val, BIO* data, int flags,
  ...
  if ((flags & SMIME_DETACHED) && data) {
    ...
    if (RAND_bytes_ex(libctx, (unsigned char*)bound, 32, 0) <= 0) return 0;
    for (i = 0; i < 32; i++) {
      c = bound[i] & 0xf;
      if (c < 10)
        c += '0';
    ...
    if (BIO_printf(
            ...
            mime_prefix, mime_eol, mime_eol, mime_eol, mime_eol) < 0)
      return 0;
...
static int asn1_output_data(BIO* out, BIO* data, ASN1_VALUE* val, int flags,
  ...
  if (!SMIME_crlf_copy(data, sarg.ndef_bio, flags)) rv = 0;

  /* Finalize structure */
  if (aux->asn1_cb(ASN1_OP_DETACHED_POST, &val, it, &sarg) <= 0) rv = 0;
...
int SMIME_crlf_copy(BIO* in, BIO* out, int flags) {
  ...
  if (flags & SMIME_BINARY) {
    while ((len = BIO_read(in, linebuf, MAX_SMLEN)) > 0) {
      if (BIO_write(out, linebuf, len) != len && out != NULL) goto err;
    }
  } else {
    int eolcnt = 0;
...
int SMIME_text(BIO* in, BIO* out) {
  ...
  while ((len = BIO_read(in, iobuf, sizeof(iobuf))) > 0)
    if (BIO_write(out, iobuf, len) != len && out != NULL) return 0;
  return len >= 0;
}
  ...
  *ret = parts;
  ...
  while ((len = BIO_get_line(bio, linebuf, MAX_SMLEN)) > 0) {
    ...
    if (state == 1) {
      ...
      eol = next_eol;
      if (len > 0 && BIO_write(bpart, linebuf, len) != len) return 0;
    }
  }
  BIO_free(bpart);
  return 0;
        ...
        case MIME_TYPE:
          if (c == ';') {
            ...
            *p = 0;
            ...
            if (new_hdr == NULL) goto err;
            if (!sk_MIME_HEADER_push(headers, new_hdr)) goto err;
            mhdr = new_hdr;
            new_hdr = NULL;
            ntmp = NULL;
            q = p + 1;
...
static MIME_HEADER* mime_hdr_new(const char* name, const char* value) {
  ...
  if (name) {
    if ((tmpname = OPENSSL_strdup(name)) == NULL) return NULL;
    for (p = tmpname; *p; p++) *p = ossl_tolower(*p);
  }
  if (value) {
    if ((tmpval = OPENSSL_strdup(value)) == NULL) goto err;
    for (p = tmpval; *p; p++) *p = ossl_tolower(*p);
  }
  mhdr = OPENSSL_malloc(sizeof(*mhdr));
  if (mhdr == NULL) goto err;
  mhdr->name = tmpname;
  mhdr->value = tmpval;
  if ((mhdr->params = sk_MIME_PARAM_new(mime_param_cmp)) == NULL) goto err;
  return mhdr;
...
static int mime_hdr_addparam(MIME_HEADER* mhdr, const char* name,
  ...
  if (mparam == NULL) goto err;
  mparam->param_name = tmpname;
  mparam->param_value = tmpval;
  if (!sk_MIME_PARAM_push(mhdr->params, mparam)) goto err;
  return 1;
err:
  OPENSSL_free(tmpname);
  OPENSSL_free(tmpval);
...
static int strip_eol(char* linebuf, int* plen, int flags) {
  ...
  if ((flags & CMS_BINARY) != 0) {
    ...
    if ((flags & SMIME_CRLFEOL) != 0) {
      if (len <= 1 || linebuf[len - 2] != '\r') return 0;
      len--;
    }
    len--;
    *plen = len;
