...
int PKCS5_pbe_set0_algor_ex(X509_ALGOR* algor, int alg, int iter,
  ...
  if (!saltlen) saltlen = PKCS5_DEFAULT_PBE1_SALT_LEN;
  if (saltlen < 0) goto err;

  sstr = OPENSSL_malloc(saltlen);
  if (sstr == NULL) goto err;
  if (salt)
...
X509_ALGOR* PKCS5_pbe_set_ex(int alg, int iter, const unsigned char* salt,
  ...
  if (PKCS5_pbe_set0_algor_ex(ret, alg, iter, salt, saltlen, ctx)) return ret;
