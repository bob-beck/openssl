...
X509_ALGOR*
PKCS5_pbe2_set_scrypt(const EVP_CIPHER* cipher,
                      ...
                      uint64_t p)
{
  ...
  if (ASN1_TYPE_pack_sequence(
        ASN1_ITEM_rptr(PBE2PARAM), pbe2, &ret->parameter) == NULL) {
    ERR_raise(ERR_LIB_ASN1, ERR_R_ASN1_LIB);
    goto err;
  }
                 ...
                 uint64_t p)
{
  ...
  if (ASN1_TYPE_pack_sequence(
        ASN1_ITEM_rptr(SCRYPT_PARAMS), sparam, &keyfunc->parameter) == NULL) {
    ERR_raise(ERR_LIB_ASN1, ERR_R_ASN1_LIB);
    goto err;
  }
...
int
PKCS5_v2_scrypt_keyivgen_ex(EVP_CIPHER_CTX* ctx,
                            const char* pass,
                            int passlen,
                            ASN1_TYPE* param,
                            const EVP_CIPHER* c,
                            ...
                            const char* propq)
{
  ...
  if (EVP_PBE_scrypt_ex(
        pass, passlen, salt, saltlen, N, r, p, 0, key, keylen, libctx, propq) ==
      0)
    goto err;
...
int
PKCS5_v2_scrypt_keyivgen(EVP_CIPHER_CTX* ctx,
                         const char* pass,
                         int passlen,
                         ASN1_TYPE* param,
                         const EVP_CIPHER* c,
                         ...
                         int en_de)
{
  return PKCS5_v2_scrypt_keyivgen_ex(
