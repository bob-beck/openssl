...
X509_ALGOR* PKCS5_pbe2_set_scrypt(const EVP_CIPHER* cipher,
  ...
  if (EVP_CipherInit_ex(ctx, cipher, NULL, NULL, iv, 0) == 0) goto err;
  ...
  if (alg_nid == NID_rc2_cbc) keylen = EVP_CIPHER_get_key_length(cipher);
...
static X509_ALGOR* pkcs5_scrypt_set(const unsigned char* salt, int saltlen,
  ...
  if (!saltlen) saltlen = PKCS5_DEFAULT_PBE2_SALT_LEN;
  ...
  if (salt == NULL && RAND_bytes(sparam->salt->data, saltlen) <= 0) goto err;
...
err:
  if (keylen) OPENSSL_cleanse(key, keylen);
  SCRYPT_PARAMS_free(sparam);
  return rv;
}
