...
int ossl_asn1_time_to_tm(struct tm* tm, const ASN1_TIME* d) {
  ...
  for (i = 0; i < end; i++) {
    ...
    if (!ossl_ascii_isdigit(a[o])) goto err;
    n = a[o] - num_zero;
  ...
  if (d->type == V_ASN1_GENERALIZEDTIME && a[o] == period) {
    if (strict) /* RFC 5280 forbids fractional seconds */
      goto err;
    if (++o == l) goto err;
    i = o;
  ...
  if (a[o] == upper_z) {
    ...
    if (offset && !OPENSSL_gmtime_adj(&tmp, 0, offset * offsign)) goto err;
  } else {
...
ASN1_TIME* ossl_asn1_time_from_tm(ASN1_TIME* s, struct tm* ts, int type) {
  ...
  if (!ASN1_STRING_set(tmps, NULL, len)) goto err;

  tmps->type = type;
  p = (char*)tmps->data;

  if (ts->tm_mon > INT_MAX - 1) goto err;

  if (type == V_ASN1_GENERALIZEDTIME) {
    if (ts->tm_year > INT_MAX - 1900) goto err;
  ...
  return tmps;
err:
  if (tmps != s) ASN1_STRING_free(tmps);
  return NULL;
}
...
ASN1_GENERALIZEDTIME* ASN1_TIME_to_generalizedtime(const ASN1_TIME* t,
  ...
  if (!ASN1_TIME_to_tm(t, &tm)) return NULL;
...
int ASN1_TIME_set_string(ASN1_TIME* s, const char* str) {
  ...
  if (ASN1_UTCTIME_set_string(s, str)) return 1;
...
int ASN1_TIME_set_string_X509(ASN1_TIME* s, const char* str) {
  ...
  if ((len = strlen(str)) >= INT_MAX) goto out;
  t.length = (int)len;
  ...
  if (s != NULL && t.type == V_ASN1_GENERALIZEDTIME) {
    if (!ossl_asn1_time_to_tm(&tm, &t)) goto out;
    if (is_utc(tm.tm_year)) {
      ...
      if (t.data == NULL) goto out;
      memcpy(t.data, str + 2, t.length);
      t.type = V_ASN1_UTCTIME;
    }
  }

  if (s == NULL || ASN1_STRING_copy((ASN1_STRING*)s, (ASN1_STRING*)&t)) rv = 1;

  if (t.data != (unsigned char*)str) OPENSSL_free(t.data);
out:
  return rv;
}
...
int ASN1_TIME_to_tm(const ASN1_TIME* s, struct tm* tm) {
  if (s == NULL) {
    ...
    if (OPENSSL_gmtime(&now_t, tm) != NULL) return 1;
    return 0;
  }
...
int ASN1_TIME_diff(int* pday, int* psec, const ASN1_TIME* from,
  ...
  if (!ASN1_TIME_to_tm(from, &tm_from)) return 0;
...
int ASN1_TIME_cmp_time_t(const ASN1_TIME* s, time_t t) {
  ...
  if (!ASN1_TIME_to_tm(s, &stm)) return -2;

  if (!OPENSSL_gmtime(&t, &ttm)) return -2;

  if (!OPENSSL_gmtime_diff(&day, &sec, &ttm, &stm)) return -2;
...
int ASN1_TIME_normalize(ASN1_TIME* t) {
  ...
  if (t == NULL || !ASN1_TIME_to_tm(t, &tm)) return 0;
...
int ASN1_TIME_compare(const ASN1_TIME* a, const ASN1_TIME* b) {
  ...
  if (!ASN1_TIME_diff(&day, &sec, b, a)) return -2;
  if (day > 0 || sec > 0) return 1;
  if (day < 0 || sec < 0) return -1;
  return 0;
}
