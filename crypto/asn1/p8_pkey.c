...
static int pkey_cb(int operation, ASN1_VALUE** pval, const ASN1_ITEM* it,
  ...
  switch (operation) {
    case ASN1_OP_FREE_PRE:
      ...
      if (key->pkey) OPENSSL_cleanse(key->pkey->data, key->pkey->length);
      break;
...
ASN1_SEQUENCE_cb(PKCS8_PRIV_KEY_INFO, pkey_cb) = {
    ...
    ASN1_IMP_OPT(
        PKCS8_PRIV_KEY_INFO, kpub, ASN1_BIT_STRING,
...
int PKCS8_pkey_set0(PKCS8_PRIV_KEY_INFO* priv, ASN1_OBJECT* aobj, int version,
  ...
  if (penc) ASN1_STRING_set0(priv->pkey, penc, penclen);
  return 1;
}
...
int PKCS8_pkey_get0(const ASN1_OBJECT** ppkalg, const unsigned char** pk,
  ...
  if (ppkalg) *ppkalg = p8->pkeyalg->algorithm;
  if (pk) {
...
const STACK_OF(X509_ATTRIBUTE)* PKCS8_pkey_get0_attrs(
    const PKCS8_PRIV_KEY_INFO* p8) {
