...
static int i2r_ALLOWED_ATTRIBUTES_CHOICE(X509V3_EXT_METHOD* method,
  ...
  switch (a->type) {
    case (OSSL_AAA_ATTRIBUTE_TYPE):
      ...
      if (i2a_ASN1_OBJECT(out, a->choice.attributeType) <= 0) return 0;
      return BIO_puts(out, "\n") > 0;
    case (OSSL_AAA_ATTRIBUTE_VALUES):
      attr = a->choice.attributeTypeandValues;
      attr_obj = X509_ATTRIBUTE_get0_object(attr);
      attr_nid = OBJ_obj2nid(attr_obj);
      if (BIO_printf(out, "%*sAttribute Values: ", indent, "") <= 0) return 0;
      if (i2a_ASN1_OBJECT(out, attr_obj) <= 0) return 0;
      if (BIO_puts(out, "\n") <= 0) return 0;
...
static int i2r_ALLOWED_ATTRIBUTES_ITEM(X509V3_EXT_METHOD* method,
  ...
  if (BIO_printf(out, "%*sHolder Domain: ", indent, "") <= 0) return 0;
  if (GENERAL_NAME_print(out, aai->holderDomain) <= 0) return 0;
  if (BIO_puts(out, "\n") <= 0) return 0;
  return 1;
}
