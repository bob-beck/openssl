...
static int x509v3_add_len_value(const char* name, const char* value,
  ...
  if (name != NULL && (tname = OPENSSL_strdup(name)) == NULL) goto err;
  if (value != NULL) {
    /* We don't allow embedded NUL characters */
    if (memchr(value, 0, vallen) != NULL) goto err;
    tvalue = OPENSSL_strndup(value, vallen);
    if (tvalue == NULL) goto err;
  }
  if ((vtmp = OPENSSL_malloc(sizeof(*vtmp))) == NULL) goto err;
...
static char* bignum_to_string(const BIGNUM* bn) {
   ...
   * numbers.
   */
  if (BN_num_bits(bn) < 128) return BN_bn2dec(bn);
...
int X509V3_add_value_int(const char* name, const ASN1_INTEGER* aint,
  ...
  if (!aint) return 1;
  if ((strtmp = i2s_ASN1_INTEGER(NULL, aint)) == NULL) return 0;
...
int X509V3_get_value_bool(const CONF_VALUE* value, int* asn1_bool) {
  ...
  if ((btmp = value->value) == NULL) goto err;
  ...
  for (i = 0; i < sk_ACCESS_DESCRIPTION_num(info); i++) {
    ...
    if (OBJ_obj2nid(ad->method) == NID_ad_OCSP) {
      if (ad->location->type == GEN_URI) {
        if (!append_ia5(&ret, ad->location->d.uniformResourceIdentifier)) break;
      }
    }
  }
...
static int append_ia5(STACK_OF(OPENSSL_STRING)** sk,
  ...
  if (email->data == NULL || email->length == 0) return 1;
  if (memchr(email->data, 0, email->length) != NULL) return 1;
  if (*sk == NULL) *sk = sk_OPENSSL_STRING_new(sk_strcmp);
  if (*sk == NULL) return 0;
...
static void skip_prefix(const unsigned char** p, size_t* plen,
   ...
   * no NULs.
   */
  if ((flags & _X509_CHECK_FLAG_DOT_SUBDOMAINS) == 0) return;
...
static int wildcard_match(const unsigned char* prefix, size_t prefix_len,
  ...
  if (prefix_len == 0 && *suffix == '.') {
    ...
    allow_idna = 1;
    if (flags & X509_CHECK_FLAG_MULTI_LABEL_WILDCARDS) allow_multi = 1;
  }
...
static const unsigned char* valid_star(const unsigned char* p, size_t len,
  ...
  for (i = 0; i < len; ++i) {
    ...
    if (p[i] == '*') {
       ...
       */
      if (star != NULL || (state & LABEL_IDNA) != 0 || dots) return NULL;
      ...
      if ((state & (LABEL_HYPHEN | LABEL_START)) != 0) return NULL;
      state = LABEL_START;
      ++dots;
    } else if (p[i] == '-') {
      /* no domain/subdomain starts with '-' */
      if ((state & LABEL_START) != 0) return NULL;
      state |= LABEL_HYPHEN;
    } else {
      return NULL;
    }
   ...
   */
  if ((state & (LABEL_START | LABEL_HYPHEN)) != 0 || dots < 2) return NULL;
  return star;
}
...
static int do_x509_check(X509* x, const char* chk, size_t chklen,
  ...
  if (check_type == GEN_EMAIL) {
    ...
    if (chklen > 1 && chk[0] == '.') flags |= _X509_CHECK_FLAG_DOT_SUBDOMAINS;
    alt_type = V_ASN1_IA5STRING;
    ...
    for (i = 0; i < sk_GENERAL_NAME_num(gens); i++) {
      ...
      switch (gen->type) {
        ...
        case GEN_OTHERNAME:
          ...
          break;
        case GEN_EMAIL:
          if (check_type != GEN_EMAIL) continue;
          cstr = gen->d.rfc822Name;
          break;
        case GEN_DNS:
          if (check_type != GEN_DNS) continue;
          cstr = gen->d.dNSName;
          break;
        case GEN_IPADD:
          if (check_type != GEN_IPADD) continue;
...
int X509_check_host(X509* x, const char* chk, size_t chklen, unsigned int flags,
  ...
  if (chklen == 0)
    ...
    return -2;
  if (chklen > 1 && chk[chklen - 1] == '\0') --chklen;
...
ASN1_OCTET_STRING* a2i_IPADDRESS_NC(const char* ipasc) {
  ...
  *p++ = 0;
  ...
  if (!iplen2 || (iplen1 != iplen2)) goto err;

  ret = ASN1_OCTET_STRING_new();
  if (ret == NULL) goto err;
  if (!ASN1_OCTET_STRING_set(ret, ipout, iplen1 + iplen2)) goto err;
...
int ossl_a2i_ipadd(unsigned char* ipout, const char* ipasc) {
  ...
  if (strchr(ipasc, ':')) {
    ...
    if (!ipv4_from_asc(ipout, ipasc)) return 0;
    return 4;
  }
}
...
static int ipv6_from_asc(unsigned char* v6, const char* in) {
   ...
   * elements.
   */
  if (!CONF_parse_list(in, ':', 0, ipv6_cb, &v6stat)) return 0;
  ...
  if (v6stat.zero_pos == -1) {
    ...
    if (v6stat.zero_cnt > 3) {
      ...
      if (v6stat.total > 0) return 0;
    } else if (v6stat.zero_cnt == 2) {
      /* Can only have two zeroes if at start or end */
      if ((v6stat.zero_pos != 0) && (v6stat.zero_pos != v6stat.total)) return 0;
    } else {
...
static int ipv6_cb(const char* elem, int len, void* usr) {
  ...
  if (s->total == 16) return 0;
  if (len == 0) {
    /* Zero length element, corresponds to '::' */
    if (s->zero_pos == -1) s->zero_pos = s->total;
    ...
    if (len > 4) {
      ...
      if (s->total > 12) return 0;
      /* Must be end of string */
      if (elem[len]) return 0;
      if (!ipv4_from_asc(s->tmp + s->total, elem)) return 0;
      s->total += 4;
    } else {
      if (!ipv6_hex(s->tmp + s->total, elem, len)) return 0;
      s->total += 2;
    }
  }
  return 1;
