  ...
  do {
    if ((*p == LIST_SEPARATOR_CHAR) || (*p == '\0')) {
      ...
      for (j = 0; j < sk_BY_DIR_ENTRY_num(ctx->dirs); j++) {
        ...
        if (strlen(ent->dir) == len && strncmp(ent->dir, ss, len) == 0) break;
      }
      if (j < sk_BY_DIR_ENTRY_num(ctx->dirs)) continue;
      if (ctx->dirs == NULL) {
...
static int get_cert_by_subject_ex(X509_LOOKUP* xl, X509_LOOKUP_TYPE type,
  ...
  for (i = 0; i < sk_BY_DIR_ENTRY_num(ctx->dirs); i++) {
    ...
    if (type == X509_LU_CRL && ent->hashes) {
      htmp.hash = h;
      if (!CRYPTO_THREAD_read_lock(ctx->lock)) goto finish;
...
#define stat _stat
      ...
      if (type == X509_LU_X509) {
        ...
        if ((X509_load_crl_file(xl, b->data, ent->dir_type)) == 0) break;
      }
     ...
     */
    if (k > 0) {
      if (!X509_STORE_lock(xl->store_ctx)) goto finish;
    ...
    if (type == X509_LU_CRL && k > 0) {
      if (!CRYPTO_THREAD_write_lock(ctx->lock)) goto finish;
      /*
