...
static int
dir_ctrl(X509_LOOKUP* ctx, int cmd, const char* argp, long argl, char** retp);
...
static int
get_cert_by_subject(X509_LOOKUP* xl,
                    X509_LOOKUP_TYPE type,
                    const X509_NAME* name,
                    X509_OBJECT* ret);
static int
get_cert_by_subject_ex(X509_LOOKUP* xl,
                       X509_LOOKUP_TYPE type,
                       const X509_NAME* name,
                       X509_OBJECT* ret,
                       OSSL_LIB_CTX* libctx,
...
static int
by_dir_hash_cmp(const BY_DIR_HASH* const* a, const BY_DIR_HASH* const* b)
{
  if ((*a)->hash > (*b)->hash)
    return 1;
  if ((*a)->hash < (*b)->hash)
                       ...
                       const char* propq)
{
  ...
  for (i = 0; i < sk_BY_DIR_ENTRY_num(ctx->dirs); i++) {
    ...
    for (;;) {
      ...
      {
        BIO_snprintf(
          b->data, b->max, "%s%c%08lx.%s%d", ent->dir, c, h, postfix, k);
      }
#ifndef OPENSSL_NO_POSIX_IO
#ifdef _WIN32
#define stat _stat
      ...
      if (type == X509_LU_X509) {
        if ((X509_load_cert_file_ex(
              xl, b->data, ent->dir_type, libctx, propq)) == 0)
          break;
      } else if (type == X509_LU_CRL) {
