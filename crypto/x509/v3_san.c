...
v2i_subject_alt(X509V3_EXT_METHOD* method,
                X509V3_CTX* ctx,
                STACK_OF(CONF_VALUE)* nval);
static GENERAL_NAMES*
v2i_issuer_alt(X509V3_EXT_METHOD* method,
               X509V3_CTX* ctx,
               STACK_OF(CONF_VALUE)* nval);
static int
                 ...
                 STACK_OF(CONF_VALUE)* ret)
{
  ...
  switch (gen->type) {
    ...
    case GEN_EMAIL:
      if (!x509v3_add_len_value_uchar(
            "email", gen->d.ia5->data, gen->d.ia5->length, &ret))
        return NULL;
      break;

    case GEN_DNS:
      if (!x509v3_add_len_value_uchar(
            "DNS", gen->d.ia5->data, gen->d.ia5->length, &ret))
        return NULL;
      break;

    case GEN_URI:
      if (!x509v3_add_len_value_uchar(
            "URI", gen->d.ia5->data, gen->d.ia5->length, &ret))
        return NULL;
      break;
...
GENERAL_NAME*
a2i_GENERAL_NAME(GENERAL_NAME* out,
                 ...
                 int is_nc)
{
  ...
  switch (gen_type) {
    ...
    case GEN_IPADD:
      ...
      if (gen->d.ip == NULL) {
        ERR_raise_data(
          ERR_LIB_X509V3, X509V3_R_BAD_IP_ADDRESS, "value=%s", value);
        goto err;
      }
      break;
  ...
  if (is_string) {
    if ((gen->d.ia5 = ASN1_IA5STRING_new()) == NULL ||
        !ASN1_STRING_set(
          gen->d.ia5, (unsigned char*)value, (int)strlen(value))) {
...
GENERAL_NAME*
v2i_GENERAL_NAME_ex(GENERAL_NAME* out,
                    const X509V3_EXT_METHOD* method,
                    X509V3_CTX* ctx,
                    CONF_VALUE* cnf,
                    int is_nc)
{
  ...
  if (!ossl_v3_name_cmp(name, "email"))
    ...
    ERR_raise_data(
      ERR_LIB_X509V3, X509V3_R_UNSUPPORTED_OPTION, "name=%s", name);
    return NULL;
  }
...
do_dirname(GENERAL_NAME* gen, const char* value, X509V3_CTX* ctx)
{
  ...
  if (!sk) {
    ERR_raise_data(
      ERR_LIB_X509V3, X509V3_R_SECTION_NOT_FOUND, "section=%s", value);
    goto err;
  }
