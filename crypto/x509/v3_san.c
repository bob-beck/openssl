...
STACK_OF(CONF_VALUE)* i2v_GENERAL_NAMES(X509V3_EXT_METHOD* method,
  ...
  for (i = 0; i < sk_GENERAL_NAME_num(gens); i++) {
    ...
    if (tmpret == NULL) {
      if (origret == NULL) sk_CONF_VALUE_pop_free(ret, X509V3_conf_free);
      return NULL;
    }
    ret = tmpret;
  }
  if (ret == NULL) return sk_CONF_VALUE_new_null();
  return ret;
}
...
STACK_OF(CONF_VALUE)* i2v_GENERAL_NAME(X509V3_EXT_METHOD* method,
  ...
  switch (gen->type) {
    case GEN_OTHERNAME:
      switch (OBJ_obj2nid(gen->d.otherName->type_id)) {
        ...
        default:
          ...
          if (!X509V3_add_value(othername, "<unsupported>", &ret)) return NULL;
          break;
      }
      break;

    case GEN_X400:
      if (!X509V3_add_value("X400Name", "<unsupported>", &ret)) return NULL;
      break;

    case GEN_EDIPARTY:
      if (!X509V3_add_value("EdiPartyName", "<unsupported>", &ret)) return NULL;
      break;
    ...
    case GEN_IPADD:
      ...
      if (tmp == NULL || !X509V3_add_value("IP Address", tmp, &ret)) ret = NULL;
      OPENSSL_free(tmp);
      break;

    case GEN_RID:
      i2t_ASN1_OBJECT(oline, 256, gen->d.rid);
      if (!X509V3_add_value("Registered ID", oline, &ret)) return NULL;
      break;
  }
  return ret;
}
...
static int copy_issuer(X509V3_CTX* ctx, GENERAL_NAMES* gens) {
  ...
  if (ctx != NULL && (ctx->flags & X509V3_CTX_TEST) != 0) return 1;
...
static int do_othername(GENERAL_NAME* gen, const char* value, X509V3_CTX* ctx) {
  ...
  if ((p = strchr(value, ';')) == NULL) return 0;
  ...
  if (objtmp == NULL) goto err;
  gen->d.otherName->type_id = OBJ_txt2obj(objtmp, 0);
  OPENSSL_free(objtmp);
  if (!gen->d.otherName->type_id) goto err;
  return 1;
...
static int do_dirname(GENERAL_NAME* gen, const char* value, X509V3_CTX* ctx) {
  ...
  if ((nm = X509_NAME_new()) == NULL) goto err;
