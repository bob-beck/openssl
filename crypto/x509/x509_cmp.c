...
unsigned long X509_issuer_and_serial_hash(X509* a) {
  ...
  if (digest == NULL) goto err;

  if (!EVP_DigestInit_ex(ctx, digest, NULL)) goto err;
  if (!EVP_DigestUpdate(ctx, (unsigned char*)f, strlen(f))) goto err;
  if (!EVP_DigestUpdate(ctx, (unsigned char*)a->cert_info.serialNumber.data,
                        (unsigned long)a->cert_info.serialNumber.length))
    goto err;
  if (!EVP_DigestFinal_ex(ctx, &(md[0]), NULL)) goto err;
...
int X509_cmp(const X509* a, const X509* b) {
  ...
  if (!a->cert_info.enc.modified && !b->cert_info.enc.modified) {
    if (a->cert_info.enc.len < b->cert_info.enc.len) return -1;
    if (a->cert_info.enc.len > b->cert_info.enc.len) return 1;
...
int X509_add_cert(STACK_OF(X509)* sk, X509* cert, int flags) {
  ...
  if ((flags & X509_ADD_FLAG_UP_REF) != 0 && !X509_up_ref(cert)) return 0;
  if (!sk_X509_insert(sk, cert,
                      (flags & X509_ADD_FLAG_PREPEND) != 0 ? 0 : -1)) {
    if ((flags & X509_ADD_FLAG_UP_REF) != 0) X509_free(cert);
    ERR_raise(ERR_LIB_X509, ERR_R_CRYPTO_LIB);
    return 0;
  }
  return 1;
...
/* compiler would allow 'const' for the certs, yet they may get up-ref'ed */
{
  ...
  for (i = 0; i < n; i++) {
    ...
    if (!ossl_x509_add_cert_new(p_sk, sk_X509_value(certs, j), flags)) return 0;
  }
  return 1;
}
...
int X509_NAME_cmp(const X509_NAME* a, const X509_NAME* b) {
  ...
  if (ret == 0 && a->canon_enclen == 0) return 0;

  if (ret == 0) {
    if (a->canon_enc == NULL || b->canon_enc == NULL) return -2;
...
unsigned long X509_NAME_hash_old(const X509_NAME* x) {
  ...
  if (md5 == NULL || md_ctx == NULL) goto end;

  /* Make sure X509_NAME structure contains valid cached encoding */
  if (i2d_X509_NAME(x, NULL) < 0) goto end;
...
X509* X509_find_by_issuer_and_serial(STACK_OF(X509)* sk, const X509_NAME* name,
  ...
  for (i = 0; i < sk_X509_num(sk); i++) {
    ...
    if (X509_issuer_and_serial_cmp(x509, &x) == 0) return x509;
  }
  return NULL;
}
...
X509* X509_find_by_subject(STACK_OF(X509)* sk, const X509_NAME* name) {
  ...
  for (i = 0; i < sk_X509_num(sk); i++) {
    x509 = sk_X509_value(sk, i);
    if (X509_NAME_cmp(X509_get_subject_name(x509), name) == 0) return x509;
  }
  return NULL;
}
...
int X509_chain_check_suiteb(int* perror_depth, X509* x, STACK_OF(X509)* chain,
  ...
  if (!(flags & X509_V_FLAG_SUITEB_128_LOS)) return X509_V_OK;
   ...
   */
  if (chain == NULL) return check_suite_b(pk, -1, &tflags);
  ...
  if (rv != X509_V_OK) {
    ...
    if (perror_depth) *perror_depth = i;
  }
  return rv;
}
