...
int
ossl_x509_add_cert_new(STACK_OF(X509)** p_sk, X509* cert, int flags)
{
...
int
X509_add_cert(STACK_OF(X509)* sk, X509* cert, int flags)
{
  if (sk == NULL) {
  ...
  if ((flags & X509_ADD_FLAG_UP_REF) != 0 && !X509_up_ref(cert))
    return 0;
  if (!sk_X509_insert(
        sk, cert, (flags & X509_ADD_FLAG_PREPEND) != 0 ? 0 : -1)) {
...
int
X509_add_certs(STACK_OF(X509)* sk, STACK_OF(X509)* certs, int flags)
...
int
ossl_x509_add_certs_new(STACK_OF(X509)** p_sk, STACK_OF(X509)* certs, int flags)
/* compiler would allow 'const' for the certs, yet they may get up-ref'ed */
{
...
unsigned long
X509_NAME_hash_ex(const X509_NAME* x,
                  OSSL_LIB_CTX* libctx,
                  const char* propq,
                  int* ok)
{
 ...
 */
unsigned long
X509_NAME_hash_old(const X509_NAME* x)
{
...
X509*
X509_find_by_issuer_and_serial(STACK_OF(X509)* sk,
...
X509*
X509_find_by_subject(STACK_OF(X509)* sk, const X509_NAME* name)
{
  X509* x509;
  int i;
...
int
X509_check_private_key(const X509* cert, const EVP_PKEY* pkey)
{
...
int
ossl_x509_check_private_key(const EVP_PKEY* x, const EVP_PKEY* pkey)
{
  if (x == NULL) {
    ERR_raise(ERR_LIB_X509, ERR_R_PASSED_NULL_PARAMETER);
    return 0;
...
static int
check_suite_b(EVP_PKEY* pkey, int sign_nid, unsigned long* pflags)
{
  char curve_name[80];
  ...
  if (!EVP_PKEY_get_group_name(
        pkey, curve_name, sizeof(curve_name), &curve_name_len))
...
int
X509_chain_check_suiteb(int* perror_depth,
                        X509* x,
                        STACK_OF(X509)* chain,
                        unsigned long flags)
{
...
int
X509_CRL_check_suiteb(X509_CRL* crl, EVP_PKEY* pk, unsigned long flags)
{
  int sign_nid;
  if (!(flags & X509_V_FLAG_SUITEB_128_LOS))
    return X509_V_OK;
