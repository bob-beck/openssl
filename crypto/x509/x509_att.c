                        ...
                        int len)
{
  ...
  if (*x != NULL && X509at_get_attr_by_NID(*x, nid, -1) != -1) {
    ERR_raise_data(
      ERR_LIB_X509, X509_R_DUPLICATE_ATTRIBUTE, "name=%s", OBJ_nid2sn(nid));
    return NULL;
  }
...
void*
X509at_get0_data_by_OBJ(const STACK_OF(X509_ATTRIBUTE)* x,
                        const ASN1_OBJECT* obj,
                        int lastpos,
                        int type)
{
...
X509_ATTRIBUTE*
X509_ATTRIBUTE_create_by_NID(X509_ATTRIBUTE** attr,
                             int nid,
                             int atrtype,
                             const void* data,
                             int len)
...
X509_ATTRIBUTE*
X509_ATTRIBUTE_create_by_OBJ(X509_ATTRIBUTE** attr,
                             const ASN1_OBJECT* obj,
                             int atrtype,
                             const void* data,
                             int len)
...
X509_ATTRIBUTE*
X509_ATTRIBUTE_create_by_txt(X509_ATTRIBUTE** attr,
                             const char* atrname,
                             int type,
                             const unsigned char* bytes,
                             int len)
...
int
X509_ATTRIBUTE_set1_data(X509_ATTRIBUTE* attr,
                         int attrtype,
                         const void* data,
                         int len)
{
  ...
  if ((attrtype & MBSTRING_FLAG) != 0) {
    stmp = ASN1_STRING_set_by_NID(
      NULL, data, len, attrtype, OBJ_obj2nid(attr->object));
    if (stmp == NULL) {
      ERR_raise(ERR_LIB_X509, ERR_R_ASN1_LIB);
      return 0;
    }
