...
int X509at_get_attr_by_OBJ(const STACK_OF(X509_ATTRIBUTE)* sk,
  ...
  for (; lastpos < n; lastpos++) {
    ...
    if (OBJ_cmp(ex->object, obj) == 0) return lastpos;
  }
  return -1;
}
...
STACK_OF(X509_ATTRIBUTE)* ossl_x509at_add1_attr(STACK_OF(X509_ATTRIBUTE)** x,
  ...
  if ((new_attr = X509_ATTRIBUTE_dup(attr)) == NULL) goto err;
...
err:
  ...
  if (*x == NULL) sk_X509_ATTRIBUTE_free(sk);
  return NULL;
}
...
STACK_OF(X509_ATTRIBUTE)* ossl_x509at_add1_attr_by_OBJ(
...
STACK_OF(X509_ATTRIBUTE)* ossl_x509at_add1_attr_by_NID(
...
STACK_OF(X509_ATTRIBUTE)* ossl_x509at_add1_attr_by_txt(
...
void* X509at_get0_data_by_OBJ(const STACK_OF(X509_ATTRIBUTE)* x,
  ...
  if (i == -1) return NULL;
  if (lastpos <= -2 && X509at_get_attr_by_OBJ(x, obj, i) != -1) return NULL;
  at = X509at_get_attr(x, i);
  if (lastpos <= -3 && X509_ATTRIBUTE_count(at) != 1) return NULL;
...
X509_ATTRIBUTE* X509_ATTRIBUTE_create_by_OBJ(X509_ATTRIBUTE** attr,
  ...
  if (!X509_ATTRIBUTE_set1_object(ret, obj)) goto err;
  if (!X509_ATTRIBUTE_set1_data(ret, atrtype, data, len)) goto err;

  if (attr != NULL && *attr == NULL) *attr = ret;
  return ret;
err:
  if (attr == NULL || ret != *attr) X509_ATTRIBUTE_free(ret);
  return NULL;
}
