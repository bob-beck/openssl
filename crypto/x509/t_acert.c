...
static int print_attribute(BIO* bp, X509_ATTRIBUTE* a) {
  ...
  if ((j = i2a_ASN1_OBJECT(bp, aobj)) <= 0) goto err;
  ...
  if (j < 25 && (BIO_printf(bp, "%*s", 25 - j, " ") <= 0)) goto err;
  ...
  for (i = 0; i < count; i++) {
    ...
    switch (type) {
      ...
      case V_ASN1_IA5STRING:
        ...
        if (BIO_write(bp, (char*)bs->data, bs->length) != bs->length) goto err;
        if (BIO_puts(bp, "\n") <= 0) goto err;
        break;
...
int X509_ACERT_print_ex(BIO* bp, X509_ACERT* x, unsigned long nmflags,
  ...
  if ((cflag & X509_FLAG_NO_SERIAL) == 0) {
    ...
    if (BIO_printf(bp, "%8sSerial Number: ", "") <= 0) goto err;

    if (i2a_ASN1_INTEGER(bp, serial) <= 0) goto err;
  ...
  if ((cflag & X509_FLAG_NO_SUBJECT) == 0) {
    ...
    if (holderEntities != NULL) {
      for (i = 0; i < sk_GENERAL_NAME_num(holderEntities); i++) {
        ...
        if (BIO_printf(bp, "%12sName:%c", "", mlch) <= 0) goto err;
        if (GENERAL_NAME_print(bp, entity) <= 0) goto err;
        if (BIO_write(bp, "\n", 1) <= 0) goto err;
      }
    }
    ...
    if (holderIssuer != NULL) {
      ...
      if (BIO_printf(bp, "%12sIssuer:%c", "", mlch) <= 0) goto err;

      if (X509_NAME_print_ex(bp, holderIssuer, 0, nmflags) <= 0) goto err;

      if (BIO_write(bp, "\n", 1) <= 0) goto err;

      if (BIO_printf(bp, "%12sSerial: ", "") <= 0) goto err;

      holder_serial = OSSL_ISSUER_SERIAL_get0_serial(holder_bcid);

      if (i2a_ASN1_INTEGER(bp, holder_serial) <= 0) goto err;

      iuid = OSSL_ISSUER_SERIAL_get0_issuerUID(holder_bcid);
      if (iuid != NULL) {
        if (BIO_printf(bp, "%12sIssuer UID: ", "") <= 0) goto err;
        if (X509_signature_dump(bp, iuid, 24) <= 0) goto err;
      }
      if (BIO_write(bp, "\n", 1) <= 0) goto err;
    }
  }
  ...
  if ((cflag & X509_FLAG_NO_ISSUER) == 0) {
    ...
    if (BIO_printf(bp, "%8sIssuer:%c", "", mlch) <= 0) goto err;
    issuer = X509_ACERT_get0_issuerName(x);
    if (issuer) {
      if (X509_NAME_print_ex(bp, issuer, 0, nmflags) < 0) goto err;
    } else {
      if (BIO_printf(bp, "Unsupported Issuer Type") <= 0) goto err;
    }
    if (BIO_write(bp, "\n", 1) <= 0) goto err;
  }
  ...
  if ((cflag & X509_FLAG_NO_VALIDITY) == 0) {
    ...
    if (BIO_printf(bp, "%12sNot Before: ", "") <= 0) goto err;
    if (ASN1_GENERALIZEDTIME_print(bp, X509_ACERT_get0_notBefore(x)) == 0)
      goto err;
    if (BIO_printf(bp, "\n%12sNot After : ", "") <= 0) goto err;
  ...
  if ((cflag & X509_FLAG_NO_ATTRIBUTES) == 0) {
    ...
    if (X509_ACERT_get_attr_count(x) == 0) {
      ...
      for (i = 0; i < X509_ACERT_get_attr_count(x); i++) {
        if (print_attribute(bp, X509_ACERT_get_attr(x, i)) == 0) goto err;
      }
    }
  }
  ...
  if ((cflag & X509_FLAG_NO_EXTENSIONS) == 0) {
    ...
    if (exts != NULL) {
      ...
      for (i = 0; i < sk_X509_EXTENSION_num(exts); i++) {
        ...
        if (i2a_ASN1_OBJECT(bp, obj) <= 0) goto err;
        critical = X509_EXTENSION_get_critical(ex);
        if (BIO_printf(bp, ": %s\n", critical ? "critical" : "") <= 0) goto err;
        if (X509V3_EXT_print(bp, ex, cflag, 20) <= 0) {
          if (BIO_printf(bp, "%16s", "") <= 0) goto err;
          if (ASN1_STRING_print(bp, X509_EXTENSION_get_data(ex)) <= 0) goto err;
        }
        if (BIO_write(bp, "\n", 1) <= 0) goto err;
      }
    }
  ...
  if ((cflag & X509_FLAG_NO_SIGDUMP) == 0) {
    ...
    if (X509_signature_print(bp, sig_alg, sig) <= 0) return 0;
  }
