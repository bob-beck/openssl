...
static void*
v2i_EXTENDED_KEY_USAGE(const X509V3_EXT_METHOD* method,
                       X509V3_CTX* ctx,
                       STACK_OF(CONF_VALUE)* nval);
static STACK_OF(CONF_VALUE)*
i2v_EXTENDED_KEY_USAGE(const X509V3_EXT_METHOD* method,
                       void* eku,
                       ...
                       STACK_OF(CONF_VALUE)* nval)
{
  ...
  for (i = 0; i < num; i++) {
    ...
    if ((objtmp = OBJ_txt2obj(extval, 0)) == NULL) {
      ...
      ERR_raise_data(
        ERR_LIB_X509V3, X509V3_R_INVALID_OBJECT_IDENTIFIER, "%s", extval);
      return NULL;
    }
