...
static void tree_print(BIO* channel, char* str, X509_POLICY_TREE* tree,
  ...
  for (plev = tree->levels; plev != curr; plev++) {
    ...
    if (plev->anyPolicy) X509_POLICY_NODE_print(channel, plev->anyPolicy, 2);
  }
}
...
static int tree_init(X509_POLICY_TREE** ptree, STACK_OF(X509)* certs,
  ...
  *ptree = NULL;
  ...
  if (n < 0) return X509_PCY_TREE_INTERNAL;
  /* Can't do anything with just a trust anchor */
  if (n == 0) return X509_PCY_TREE_EMPTY;
  ...
  for (i = n - 1; i >= 0; i--) {
    ...
    if (ossl_policy_cache_set(x) == NULL) return X509_PCY_TREE_INTERNAL;
  }
  ...
  for (i = n - 1;
    ...
    if (ex_flags & EXFLAG_INVALID_POLICY) return X509_PCY_TREE_INVALID;
    ...
    if (explicit_policy > 0) {
      if (!(ex_flags & EXFLAG_SI)) explicit_policy--;
  ...
  if (explicit_policy == 0) ret |= X509_PCY_TREE_EXPLICIT;
  if ((ret & X509_PCY_TREE_VALID) == 0) return ret;
  ...
  for (i = n - 1; i >= 0; i--) {
    ...
    if (!X509_up_ref(x)) goto bad_tree;

    (++level)->cert = x;

    if (!cache->anyPolicy) level->flags |= X509_V_FLAG_INHIBIT_ANY;
    ...
    if (any_skip == 0) {
      ...
      if (!(ex_flags & EXFLAG_SI)) any_skip--;
    ...
    if (map_skip == 0)
      ...
      if (!(ex_flags & EXFLAG_SI)) map_skip--;
...
static int tree_link_nodes(X509_POLICY_LEVEL* curr,
  ...
  for (i = 0; i < sk_X509_POLICY_DATA_num(cache->data); i++) {
    ...
    if (!tree_link_matching_nodes(curr, data, tree)) return 0;
  }
  return 1;
}
...
static int tree_add_unmatched(X509_POLICY_LEVEL* curr,
  ...
  if (id == NULL) id = node->data->valid_policy;
  /*
...
static int tree_link_unmatched(X509_POLICY_LEVEL* curr,
  ...
  if ((last->flags & X509_V_FLAG_INHIBIT_MAP) ||
    ...
    if (node->nchild) return 1;
    if (!tree_add_unmatched(curr, cache, NULL, node, tree)) return 0;
    /* Add it */
  } else {
    /* If mapping: matched if one child per expected policy set */
    STACK_OF(ASN1_OBJECT)* expset = node->data->expected_policy_set;
    if (node->nchild == sk_ASN1_OBJECT_num(expset)) return 1;
    /* Locate unmatched nodes */
    for (i = 0; i < sk_ASN1_OBJECT_num(expset); i++) {
      ASN1_OBJECT* oid = sk_ASN1_OBJECT_value(expset, i);
      if (ossl_policy_level_find_node(curr, node, oid)) continue;
...
static int tree_link_any(X509_POLICY_LEVEL* curr,
  ...
  for (i = 0; i < sk_X509_POLICY_NODE_num(last->nodes); i++) {
    ...
    if (!tree_link_unmatched(curr, cache, node, tree)) return 0;
  }
...
static int tree_prune(X509_POLICY_TREE* tree, X509_POLICY_LEVEL* curr) {
  ...
  for (;;) {
    ...
    if (curr->anyPolicy && !curr->anyPolicy->nchild) {
      if (curr->anyPolicy->parent) curr->anyPolicy->parent->nchild--;
      OPENSSL_free(curr->anyPolicy);
      curr->anyPolicy = NULL;
    }
    if (curr == tree->levels) {
      /* If we zapped anyPolicy at top then tree is empty */
      if (!curr->anyPolicy) return X509_PCY_TREE_EMPTY;
      break;
    }
  }
...
static int tree_add_auth_node(STACK_OF(X509_POLICY_NODE)** pnodes,
  ...
  if (*pnodes == NULL && (*pnodes = ossl_policy_node_cmp_new()) == NULL)
    return 0;
  if (sk_X509_POLICY_NODE_find(*pnodes, pcy) >= 0) return 1;
...
static int tree_calculate_authority_set(X509_POLICY_TREE* tree,
  ...
  for (i = 1; i < tree->nlevel; i++) {
     ...
     */
    if ((anyptr = curr->anyPolicy) == NULL) break;
    curr++;
  ...
  if (addnodes == pnodes) return TREE_CALC_OK_DOFREE;
...
static int tree_calculate_user_set(X509_POLICY_TREE* tree,
   ...
   */
  if (sk_ASN1_OBJECT_num(policy_oids) <= 0) return 1;
...
static int tree_evaluate(X509_POLICY_TREE* tree) {
  ...
  for (i = 1; i < tree->nlevel; i++, curr++) {
    ...
    if (!tree_link_nodes(curr, cache, tree)) return X509_PCY_TREE_INTERNAL;
    ...
    if (ret != X509_PCY_TREE_VALID) return ret;
  }
  return X509_PCY_TREE_VALID;
}
...
int X509_policy_check(X509_POLICY_TREE** ptree, int* pexplicit_policy,
  ...
  *pexplicit_policy = 0;
  ...
  if ((init_ret & X509_PCY_TREE_EXPLICIT) == 0) {
    ...
    *pexplicit_policy = 1;
    ...
    if (init_ret & X509_PCY_TREE_EMPTY) return X509_PCY_TREE_FAILURE;
  }
  ...
  if (ret == X509_PCY_TREE_EMPTY) {
    X509_policy_tree_free(tree);
    if (init_ret & X509_PCY_TREE_EXPLICIT) return X509_PCY_TREE_FAILURE;
    return X509_PCY_TREE_VALID;
  }
  ...
  if (calc_ret == TREE_CALC_OK_DOFREE) sk_X509_POLICY_NODE_free(auth_nodes);
  if (!ret) goto error;
  ...
  *ptree = tree;
  ...
  if (init_ret & X509_PCY_TREE_EXPLICIT) {
    ...
    if (sk_X509_POLICY_NODE_num(nodes) <= 0) return X509_PCY_TREE_FAILURE;
  }
  return X509_PCY_TREE_VALID;
