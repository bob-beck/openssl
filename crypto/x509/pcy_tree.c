           ...
           X509_POLICY_LEVEL* curr)
{
  ...
  BIO_printf(
    channel, "Printing Up to Level %ld\n", (long)(curr - tree->levels));
 ...
 */
static int
tree_link_nodes(X509_POLICY_LEVEL* curr,
                const X509_POLICY_CACHE* cache,
                X509_POLICY_TREE* tree)
{
  int i;
 ...
 */
static int
tree_add_unmatched(X509_POLICY_LEVEL* curr,
                   const X509_POLICY_CACHE* cache,
 ...
 */
static int
tree_link_unmatched(X509_POLICY_LEVEL* curr,
                    const X509_POLICY_CACHE* cache,
                    X509_POLICY_NODE* node,
                    X509_POLICY_TREE* tree)
{
 ...
 */
static int
tree_link_any(X509_POLICY_LEVEL* curr,
              const X509_POLICY_CACHE* cache,
              X509_POLICY_TREE* tree)
{
  int i;
  ...
  if (last->anyPolicy &&
      ossl_policy_level_add_node(
        curr, cache->anyPolicy, last->anyPolicy, tree, 0) == NULL)
    return 0;
  return 1;
}
 ...
 */
int
X509_policy_check(X509_POLICY_TREE** ptree,
