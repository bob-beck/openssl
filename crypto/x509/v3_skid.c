...
ASN1_OCTET_STRING* ossl_x509_pubkey_hash(X509_PUBKEY* pubkey) {
  ...
  if (!ossl_x509_PUBKEY_get0_libctx(&libctx, &propq, pubkey)) return NULL;
  if ((md = EVP_MD_fetch(libctx, SN_sha1, propq)) == NULL) return NULL;
  if ((oct = ASN1_OCTET_STRING_new()) == NULL) {
    EVP_MD_free(md);
    return NULL;
  }
...
static ASN1_OCTET_STRING* s2i_skey_id(X509V3_EXT_METHOD* method,
                                      X509V3_CTX* ctx, char* str) {
  if (strcmp(str, "none") == 0) return ASN1_OCTET_STRING_new(); /* dummy */
