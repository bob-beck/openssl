...
ASN1_SEQUENCE(X509_ACERT_ISSUER_V2FORM) = {
  ...
  ASN1_IMP_OPT(X509_ACERT_ISSUER_V2FORM,
               baseCertificateId,
               OSSL_ISSUER_SERIAL,
               0),
  ASN1_IMP_OPT(X509_ACERT_ISSUER_V2FORM,
...
void
X509_ACERT_get0_signature(const X509_ACERT* x,
...
int
X509_ACERT_get_attr_by_OBJ(const X509_ACERT* x,
...
int
X509_ACERT_add1_attr_by_OBJ(X509_ACERT* x,
                            const ASN1_OBJECT* obj,
                            int type,
                            const void* bytes,
                            int len)
...
int
X509_ACERT_add1_attr_by_NID(X509_ACERT* x,
                            int nid,
                            int type,
                            const void* bytes,
                            int len)
...
int
X509_ACERT_add1_attr_by_txt(X509_ACERT* x,
                            const char* attrname,
                            int type,
  ...
  for (i = 0; i < sk_CONF_VALUE_num(attr_sk); i++) {
    ...
    if (check_asn1_attribute(&value) == 1) {
      ...
      ret = X509_ACERT_add1_attr_by_txt(
        acert, v->name, V_ASN1_SEQUENCE, att_data, att_len);
...
int
X509_ACERT_add1_ext_i2d(X509_ACERT* x,
                        int nid,
                        void* value,
                        int crit,
                        unsigned long flags)
