...
static X509_NAME* get_dirName(const GENERAL_NAMES* names) {
  ...
  if (sk_GENERAL_NAME_num(names) != 1) return NULL;

  dirName = sk_GENERAL_NAME_value(names, 0);
  if (dirName->type != GEN_DIRNAME) return NULL;
...
void OSSL_OBJECT_DIGEST_INFO_get0_digest(const OSSL_OBJECT_DIGEST_INFO* o,
  ...
  if (digestAlgorithm != NULL) *digestAlgorithm = &o->digestAlgorithm;
  if (digest != NULL) *digest = &o->objectDigest;
}
...
void X509_ACERT_get0_signature(const X509_ACERT* x,
  ...
  if (psig != NULL) *psig = &x->signature;
  if (palg != NULL) *palg = &x->sig_alg;
}
...
static int check_asn1_attribute(const char** value) {
  ...
  if (strncmp(p, "ASN1:", 5) != 0) return 0;
...
const STACK_OF(X509_EXTENSION)* X509_ACERT_get0_extensions(
    const X509_ACERT* x) {
