...
void
OSSL_OBJECT_DIGEST_INFO_get0_digest (const OSSL_OBJECT_DIGEST_INFO *o,
...
long
X509_ACERT_get_version (const X509_ACERT *x)
{
  return ASN1_INTEGER_get (&x->acinfo->version);
}

void
X509_ACERT_get0_signature (const X509_ACERT *x, const ASN1_BIT_STRING **psig,
...
int
X509_ACERT_get_signature_nid (const X509_ACERT *x)
{
  return OBJ_obj2nid (x->sig_alg.algorithm);
}

const GENERAL_NAMES *
X509_ACERT_get0_holder_entityName (const X509_ACERT *x)
{
...
const X509_NAME *
X509_ACERT_get0_issuerName (const X509_ACERT *x)
{
...
const ASN1_BIT_STRING *
X509_ACERT_get0_issuerUID (const X509_ACERT *x)
{
  return x->acinfo->issuerUID;
}

const X509_ALGOR *
X509_ACERT_get0_info_sigalg (const X509_ACERT *x)
{
  return &x->acinfo->signature;
}

const ASN1_INTEGER *
X509_ACERT_get0_serialNumber (const X509_ACERT *x)
{
  return &x->acinfo->serialNumber;
}

const ASN1_GENERALIZEDTIME *
X509_ACERT_get0_notBefore (const X509_ACERT *x)
{
  return x->acinfo->validityPeriod.notBefore;
}

const ASN1_GENERALIZEDTIME *
X509_ACERT_get0_notAfter (const X509_ACERT *x)
{
...
int
X509_ACERT_get_attr_count (const X509_ACERT *x)
{
  return X509at_get_attr_count (x->acinfo->attributes);
}

int
X509_ACERT_get_attr_by_NID (const X509_ACERT *x, int nid, int lastpos)
{
  return X509at_get_attr_by_NID (x->acinfo->attributes, nid, lastpos);
}

int
X509_ACERT_get_attr_by_OBJ (const X509_ACERT *x, const ASN1_OBJECT *obj,
                            int lastpos)
{
...
X509_ATTRIBUTE *
X509_ACERT_get_attr (const X509_ACERT *x, int loc)
{
  return X509at_get_attr (x->acinfo->attributes, loc);
}

X509_ATTRIBUTE *
X509_ACERT_delete_attr (X509_ACERT *x, int loc)
{
  return X509at_delete_attr (x->acinfo->attributes, loc);
}

int
X509_ACERT_add1_attr (X509_ACERT *x, X509_ATTRIBUTE *attr)
{
...
int
X509_ACERT_add1_attr_by_OBJ (X509_ACERT *x, const ASN1_OBJECT *obj, int type,
...
int
X509_ACERT_add1_attr_by_NID (X509_ACERT *x, int nid, int type,
                             const void *bytes, int len)
{
  STACK_OF (X509_ATTRIBUTE) **attrs = &x->acinfo->attributes;
...
int
X509_ACERT_add1_attr_by_txt (X509_ACERT *x, const char *attrname, int type,
...
static int
check_asn1_attribute (const char **value)
{
  const char *p = *value;
...
int
X509_ACERT_add_attr_nconf (CONF *conf, const char *section, X509_ACERT *acert)
{
  int ret = 0, i;
...
void *
X509_ACERT_get_ext_d2i (const X509_ACERT *x, int nid, int *crit, int *idx)
{
  return X509V3_get_d2i (x->acinfo->extensions, nid, crit, idx);
}

int
X509_ACERT_add1_ext_i2d (X509_ACERT *x, int nid, void *value, int crit,
                         unsigned long flags)
{
