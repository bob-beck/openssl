...
ASN1_SEQUENCE(OSSL_IETF_ATTR_SYNTAX) = {
  ASN1_IMP_SEQUENCE_OF_OPT(OSSL_IETF_ATTR_SYNTAX,
                           policyAuthority,
                           GENERAL_NAME,
                           0),
                          ...
                          long len)
{
  ...
  ias = (OSSL_IETF_ATTR_SYNTAX*)ASN1_item_d2i(
    (ASN1_VALUE**)a, in, len, OSSL_IETF_ATTR_SYNTAX_it());
  if (ias == NULL)
    return ias;
...
int
i2d_OSSL_IETF_ATTR_SYNTAX(const OSSL_IETF_ATTR_SYNTAX* a, unsigned char** out)
{
  return ASN1_item_i2d((const ASN1_VALUE*)a, out, OSSL_IETF_ATTR_SYNTAX_it());
}

int
OSSL_IETF_ATTR_SYNTAX_get_value_num(const OSSL_IETF_ATTR_SYNTAX* a)
{
  if (a->values == NULL)
    return 0;
...
void
OSSL_IETF_ATTR_SYNTAX_set0_policyAuthority(OSSL_IETF_ATTR_SYNTAX* a,
                                           GENERAL_NAMES* names)
{
...
void*
OSSL_IETF_ATTR_SYNTAX_get0_value(const OSSL_IETF_ATTR_SYNTAX* a,
                                 int ind,
                                 int* type)
{
  OSSL_IETF_ATTR_SYNTAX_VALUE* val;
...
int
OSSL_IETF_ATTR_SYNTAX_add1_value(OSSL_IETF_ATTR_SYNTAX* a, int type, void* data)
{
  OSSL_IETF_ATTR_SYNTAX_VALUE* val;
...
int
OSSL_IETF_ATTR_SYNTAX_print(BIO* bp, OSSL_IETF_ATTR_SYNTAX* a, int indent)
{
  int i;
