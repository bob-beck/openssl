...
int X509_verify(X509* a, EVP_PKEY* r) {
  if (X509_ALGOR_cmp(&a->sig_alg, &a->cert_info.signature) != 0) return 0;
...
int X509_ACERT_verify(X509_ACERT* a, EVP_PKEY* r) {
  if (X509_ALGOR_cmp(&a->sig_alg, &a->acinfo->signature) != 0) return 0;
...
int X509_digest(const X509* cert, const EVP_MD* md, unsigned char* data,
  ...
  if (EVP_MD_is_a(md, SN_sha1) && (cert->ex_flags & EXFLAG_SET) != 0 &&
    ...
    if (len != NULL) *len = sizeof(cert->sha1_hash);
...
ASN1_OCTET_STRING* X509_digest_sig(const X509* cert, EVP_MD** md_used,
  ...
  if (md_used != NULL) *md_used = NULL;
  if (md_is_fallback != NULL) *md_is_fallback = 0;
  ...
  if (mdnid == NID_undef) {
    if (pknid == EVP_PKEY_RSA_PSS) {
      ...
      if ((md = EVP_MD_fetch(cert->libctx, md_name, cert->propq)) == NULL)
        return NULL;
      if (md_is_fallback != NULL) *md_is_fallback = 1;
    } else {
...
int X509_CRL_digest(const X509_CRL* data, const EVP_MD* type, unsigned char* md,
  ...
  if (EVP_MD_is_a(type, SN_sha1) && (data->flags & EXFLAG_SET) != 0 &&
    ...
    if (len != NULL) *len = sizeof(data->sha1_hash);
