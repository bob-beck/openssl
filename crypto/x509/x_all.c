...
int
X509_REQ_verify_ex (X509_REQ *a, EVP_PKEY *r, OSSL_LIB_CTX *libctx,
                    const char *propq)
{
...
X509_sign (X509 *x, EVP_PKEY *pkey, const EVP_MD *md)
{
  ...
  return ASN1_item_sign_ex (
      ASN1_ITEM_rptr (X509_CINF), &x->cert_info.signature, &x->sig_alg,
      &x->signature, &x->cert_info, NULL, pkey, md, x->libctx, x->propq);
}
...
static ASN1_VALUE *
simple_get_asn1 (const char *url, BIO *bio, BIO *rbio, int timeout,
                 const ASN1_ITEM *it)
{
...
int
X509_digest (const X509 *cert, const EVP_MD *md, unsigned char *data,
             unsigned int *len)
{
...
EVP_PKEY *
d2i_PrivateKey_ex_fp (FILE *fp, EVP_PKEY **a, OSSL_LIB_CTX *libctx,
                      const char *propq)
{
  BIO *b;
  void *ret;
...
EVP_PKEY *
d2i_PUBKEY_ex_fp (FILE *fp, EVP_PKEY **a, OSSL_LIB_CTX *libctx,
                  const char *propq)
{
  BIO *b;
  void *ret;
...
EVP_PKEY *
d2i_PrivateKey_ex_bio (BIO *bp, EVP_PKEY **a, OSSL_LIB_CTX *libctx,
                       const char *propq)
{
  BUF_MEM *b = NULL;
...
EVP_PKEY *
d2i_PUBKEY_ex_bio (BIO *bp, EVP_PKEY **a, OSSL_LIB_CTX *libctx,
                   const char *propq)
{
  BUF_MEM *b = NULL;
  const unsigned char *p;
