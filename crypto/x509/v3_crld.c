...
static void*
v2i_crld(const X509V3_EXT_METHOD* method,
         X509V3_CTX* ctx,
...
static void*
v2i_idp(const X509V3_EXT_METHOD* method,
        X509V3_CTX* ctx,
        STACK_OF(CONF_VALUE)* nval);
...
int
DIST_POINT_set_dpname(DIST_POINT_NAME* dpn, const X509_NAME* iname)
{
  int i;
...
static int
print_boolean(BIO* out, ASN1_BOOLEAN b)
{
  return BIO_puts(out, b ? "TRUE" : "FALSE");
}

static OSSL_AA_DIST_POINT*
aaidp_from_section(X509V3_CTX* ctx, STACK_OF(CONF_VALUE)* nval)
{
  int i, ret;
  CONF_VALUE* cnf;
...
static void*
v2i_aaidp(const X509V3_EXT_METHOD* method,
          X509V3_CTX* ctx,
          STACK_OF(CONF_VALUE)* nval)
{
  GENERAL_NAMES* gens = NULL;
...
static int
i2r_aaidp(const X509V3_EXT_METHOD* method, void* dp, BIO* out, int indent)
{
  ...
  if (pdp->containsUserAttributeCerts) {
    if (BIO_printf(
          out, "%*sContains User Attribute Certificates: ", indent, "") <= 0)
      return 0;
