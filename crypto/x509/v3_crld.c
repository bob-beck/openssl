...
static int
print_reasons (BIO *out, const char *rname, ASN1_BIT_STRING *rflags,
               int indent)
{
  int first = 1;
  const BIT_STRING_BITNAME *pbn;
...
static void *
v2i_aaidp (const X509V3_EXT_METHOD *method, X509V3_CTX *ctx,
           STACK_OF (CONF_VALUE) *nval)
{
  GENERAL_NAMES *gens = NULL;
  GENERAL_NAME *gen = NULL;
...
i2r_aaidp (const X509V3_EXT_METHOD *method, void *dp, BIO *out, int indent)
{
  ...
  if (pdp->containsSOAPublicKeyCerts)
    {
      if (BIO_printf (out,
                      "%*sContains Source Of Authority (SOA) Public Key "
                      "Certificates: ",
                      indent, "")
          <= 0)
        return 0;
