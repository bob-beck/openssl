...
static int set_dist_point_name(DIST_POINT_NAME** pdp, X509V3_CTX* ctx,
  ...
  if (HAS_PREFIX(cnf->name, "fullname")) {
    ...
    nm->entries = NULL;
    X509_NAME_free(nm);
    if (!ret || sk_X509_NAME_ENTRY_num(rnm) <= 0) goto err;
    /*
...
static int set_reasons(ASN1_BIT_STRING** preas, char* value) {
  ...
  if (rsk == NULL) return 0;
  if (*preas != NULL) goto err;
  for (i = 0; i < sk_CONF_VALUE_num(rsk); i++) {
    ...
    for (pbn = reason_flags; pbn->lname; pbn++) {
      if (strcmp(pbn->sname, bnam) == 0) {
        if (!ASN1_BIT_STRING_set_bit(*preas, pbn->bitnum, 1)) goto err;
        break;
      }
    }
    if (pbn->lname == NULL) goto err;
  }
  ret = 1;
...
static DIST_POINT* crldp_from_section(X509V3_CTX* ctx,
  ...
  for (i = 0; i < sk_CONF_VALUE_num(nval); i++) {
    ...
    if (strcmp(cnf->name, "reasons") == 0) {
      if (!set_reasons(&point->reasons, cnf->value)) goto err;
    } else if (strcmp(cnf->name, "CRLissuer") == 0) {
      point->CRLissuer = gnames_from_sectname(ctx, cnf->value);
      if (point->CRLissuer == NULL) goto err;
    }
  }
...
static void* v2i_crld(const X509V3_EXT_METHOD* method, X509V3_CTX* ctx,
  ...
  for (i = 0; i < num; i++) {
    ...
    if (cnf->value == NULL) {
      ...
      if ((gen = v2i_GENERAL_NAME(method, ctx, cnf)) == NULL) goto err;
...
ASN1_CHOICE_cb(DIST_POINT_NAME, dpn_cb) = {
    ...
    ASN1_IMP_SET_OF(
        DIST_POINT_NAME, name.relativename, X509_NAME_ENTRY,
...
static void* v2i_idp(const X509V3_EXT_METHOD* method, X509V3_CTX* ctx,
  ...
  for (i = 0; i < sk_CONF_VALUE_num(nval); i++) {
    ...
    if (strcmp(name, "onlyuser") == 0) {
      if (!X509V3_get_value_bool(cnf, &idp->onlyuser)) goto err;
    } else if (strcmp(name, "onlyCA") == 0) {
      if (!X509V3_get_value_bool(cnf, &idp->onlyCA)) goto err;
    } else if (strcmp(name, "onlyAA") == 0) {
      if (!X509V3_get_value_bool(cnf, &idp->onlyattr)) goto err;
    } else if (strcmp(name, "indirectCRL") == 0) {
      if (!X509V3_get_value_bool(cnf, &idp->indirectCRL)) goto err;
    } else if (strcmp(name, "onlysomereasons") == 0) {
      if (!set_reasons(&idp->onlysomereasons, val)) goto err;
    } else {
...
static int i2r_idp(const X509V3_EXT_METHOD* method, void* pidp, BIO* out,
  ...
  if (idp->distpoint) print_distpoint(out, idp->distpoint, indent);
  if (idp->onlyuser > 0)
    BIO_printf(out, "%*sOnly User Certificates\n", indent, "");
  if (idp->onlyCA > 0) BIO_printf(out, "%*sOnly CA Certificates\n", indent, "");
  if (idp->indirectCRL > 0) BIO_printf(out, "%*sIndirect CRL\n", indent, "");
...
static int i2r_crldp(const X509V3_EXT_METHOD* method, void* pcrldp, BIO* out,
  ...
  for (i = 0; i < sk_DIST_POINT_num(crld); i++) {
    ...
    if (point->distpoint) print_distpoint(out, point->distpoint, indent);
    if (point->reasons) print_reasons(out, "Reasons", point->reasons, indent);
    if (point->CRLissuer) {
...
int DIST_POINT_set_dpname(DIST_POINT_NAME* dpn, const X509_NAME* iname) {
  ...
  if (dpn == NULL || dpn->type != 1) return 1;
  frag = dpn->name.relativename;
  X509_NAME_free(dpn->dpname); /* just in case it was already set */
  dpn->dpname = X509_NAME_dup(iname);
  if (dpn->dpname == NULL) return 0;
  for (i = 0; i < sk_X509_NAME_ENTRY_num(frag); i++) {
    ne = sk_X509_NAME_ENTRY_value(frag, i);
    if (!X509_NAME_add_entry(dpn->dpname, ne, -1, i ? 0 : 1)) goto err;
  }
  /* generate cached encoding of name */
  if (i2d_X509_NAME(dpn->dpname, NULL) >= 0) return 1;
...
ASN1_SEQUENCE(OSSL_AA_DIST_POINT) = {
    ...
    ASN1_IMP_OPT(OSSL_AA_DIST_POINT, containsUserAttributeCerts, ASN1_TBOOLEAN,
                 3),
...
static OSSL_AA_DIST_POINT* aaidp_from_section(X509V3_CTX* ctx,
  ...
  for (i = 0; i < sk_CONF_VALUE_num(nval); i++) {
    ...
    if (strcmp(cnf->name, "reasons") == 0) {
      ...
      if (!X509V3_get_value_bool(cnf, &point->indirectCRL)) goto err;
      ...
      if (!X509V3_get_value_bool(cnf, &point->containsAACerts)) goto err;
...
static int i2r_aaidp(const X509V3_EXT_METHOD* method, void* dp, BIO* out,
  ...
  if (pdp->distpoint)
    if (print_distpoint(out, pdp->distpoint, indent) <= 0) return 0;
  if (pdp->reasons)
    if (print_reasons(out, "Reasons", pdp->reasons, indent) <= 0) return 0;
  if (pdp->indirectCRL) {
    if (BIO_printf(out, "%*sIndirect CRL: ", indent, "") <= 0) return 0;
    if (print_boolean(out, pdp->indirectCRL) <= 0) return 0;
    if (BIO_puts(out, "\n") <= 0) return 0;
  }
  if (pdp->containsUserAttributeCerts) {
    if (BIO_printf(out, "%*sContains User Attribute Certificates: ", indent,
                   "") <= 0)
      return 0;
    if (print_boolean(out, pdp->containsUserAttributeCerts) <= 0) return 0;
    if (BIO_puts(out, "\n") <= 0) return 0;
  }
  if (pdp->containsAACerts) {
    if (BIO_printf(out, "%*sContains Attribute Authority (AA) Certificates: ",
                   indent, "") <= 0)
      return 0;
    if (print_boolean(out, pdp->containsAACerts) <= 0) return 0;
    if (BIO_puts(out, "\n") <= 0) return 0;
  }
  if (pdp->containsSOAPublicKeyCerts) {
    if (BIO_printf(
            ...
            indent, "") <= 0)
      return 0;
    if (print_boolean(out, pdp->containsSOAPublicKeyCerts) <= 0) return 0;
    if (BIO_puts(out, "\n") <= 0) return 0;
  }
  return 1;
}
