...
void
OSSL_STACK_OF_X509_free(STACK_OF(X509)* certs)
{
...
X509_print_ex(BIO* bp, X509* x, unsigned long nmflags, unsigned long cflag)
{
  ...
  if (!(cflag & X509_FLAG_NO_VALIDITY)) {
    ...
    if (BIO_write(bp, "            Not Before: ", 24) <= 0)
      goto err;
    if (ossl_asn1_time_print_ex(
          bp, X509_get0_notBefore(x), ASN1_DTFLGS_RFC822) == 0)
      goto err;
    if (BIO_write(bp, "\n            Not After : ", 25) <= 0)
      goto err;
    if (ossl_asn1_time_print_ex(
          bp, X509_get0_notAfter(x), ASN1_DTFLGS_RFC822) == 0)
      goto err;
    if (BIO_write(bp, "\n", 1) <= 0)
      goto err;
  }
  ...
  if (!(cflag & X509_FLAG_NO_EXTENSIONS) &&
      !X509V3_extensions_print(
        bp, "X509v3 extensions", X509_get0_extensions(x), cflag, 8))
    goto err;
...
int
X509_aux_print(BIO* out, X509* x, int indent)
{
...
ossl_x509_print_ex_brief(BIO* bio, X509* cert, unsigned long neg_cflags)
{
  ...
  if (!X509_print_ex(
        bio, cert, flags, ~(X509_FLAG_NO_SERIAL | X509_FLAG_NO_VALIDITY)))
    return 0;
  ...
  if (X509_cmp_current_time(X509_get0_notAfter(cert)) < 0)
    if (BIO_printf(bio, "        no more valid\n") <= 0)
      return 0;
  return X509_print_ex(
    bio, cert, flags, ~neg_cflags & ~X509_FLAG_EXTENSIONS_ONLY_KID);
}

static int
print_certs(BIO* bio, const STACK_OF(X509)* certs)
{
  int i;
...
static int
print_store_certs(BIO* bio, X509_STORE* store)
{
  if (store != NULL) {
...
int
X509_STORE_CTX_print_verify_cb(int ok, X509_STORE_CTX* ctx)
{
  if (ok == 0 && ctx != NULL) {
    ...
    ossl_x509_print_ex_brief(
      bio, X509_STORE_CTX_get_current_cert(ctx), X509_FLAG_NO_EXTENSIONS);
