...
int X509_print_ex(BIO* bp, X509* x, unsigned long nmflags,
  ...
  if (nmflags == XN_FLAG_COMPAT) printok = 1;
  ...
  if (!(cflag & X509_FLAG_NO_SERIAL)) {
    ...
    if (ossl_serial_number_print(bp, bs, 12) != 0) goto err;
    if (BIO_puts(bp, "\n") <= 0) goto err;
  }
  ...
  if (!(cflag & X509_FLAG_NO_SIGNAME)) {
    ...
    if (X509_signature_print(bp, tsig_alg, NULL) <= 0) goto err;
  }
  ...
  if (!(cflag & X509_FLAG_NO_PUBKEY)) {
    ...
    if (BIO_printf(bp, "%12sPublic Key Algorithm: ", "") <= 0) goto err;
    if (i2a_ASN1_OBJECT(bp, xpoid) <= 0) goto err;
    if (BIO_puts(bp, "\n") <= 0) goto err;
  ...
  if (!(cflag & X509_FLAG_NO_IDS)) {
    ...
    if (iuid != NULL) {
      if (BIO_printf(bp, "%8sIssuer Unique ID: ", "") <= 0) goto err;
      if (!X509_signature_dump(bp, iuid, 12)) goto err;
    }
    if (suid != NULL) {
      if (BIO_printf(bp, "%8sSubject Unique ID: ", "") <= 0) goto err;
      if (!X509_signature_dump(bp, suid, 12)) goto err;
    }
  }
  ...
  if (!(cflag & X509_FLAG_NO_SIGDUMP)) {
    ...
    if (X509_signature_print(bp, sig_alg, sig) <= 0) goto err;
  }
  if (!(cflag & X509_FLAG_NO_AUX)) {
    if (!X509_aux_print(bp, x, 0)) goto err;
  }
  ret = 1;
err:
  return ret;
...
int X509_ocspid_print(BIO* bp, X509* x) {
  ...
  if (x == NULL || bp == NULL) return 0;
  /*
   * display the hash of the subject as it would appear in OCSP requests
   */
  if (BIO_printf(bp, "        Subject OCSP hash: ") <= 0) goto err;
  subj = X509_get_subject_name(x);
  derlen = i2d_X509_NAME(subj, NULL);
  if (derlen <= 0) goto err;
  if ((der = dertmp = OPENSSL_malloc(derlen)) == NULL) goto err;
  i2d_X509_NAME(subj, &dertmp);

  md = EVP_MD_fetch(x->libctx, SN_sha1, x->propq);
  if (md == NULL) goto err;
  if (!EVP_Digest(der, derlen, SHA1md, NULL, md, NULL)) goto err;
   ...
   */
  if (BIO_printf(bp, "\n        Public key OCSP hash: ") <= 0) goto err;

  keybstr = X509_get0_pubkey_bitstr(x);

  if (keybstr == NULL) goto err;
...
#endif
  int indent = 4;
  if (BIO_printf(bp, "%*sSignature Algorithm: ", indent, "") <= 0) return 0;
  if (i2a_ASN1_OBJECT(bp, sigalg->algorithm) <= 0) return 0;

  if (sig && BIO_printf(bp, "\n%*sSignature Value:", indent, "") <= 0) return 0;
#ifndef OPENSSL_NO_DEPRECATED_3_6
...
#endif
  if (BIO_write(bp, "\n", 1) != 1) return 0;
  if (sig) return X509_signature_dump(bp, sig, indent + 4);
  return 1;
}
...
int X509_aux_print(BIO* out, X509* x, int indent) {
  ...
  if (alias) BIO_printf(out, "%*sAlias: %.*s\n", indent, "", i, alias);
...
int ossl_x509_print_ex_brief(BIO* bio, X509* cert, unsigned long neg_cflags) {
  ...
  if (cert == NULL) return BIO_printf(bio, "    (no certificate)\n") > 0;
...
static int print_certs(BIO* bio, const STACK_OF(X509)* certs) {
  ...
  for (i = 0; i < sk_X509_num(certs); i++) {
    ...
    if (cert != NULL) {
      if (!ossl_x509_print_ex_brief(bio, cert, 0)) return 0;
               ...
               X509_verify_cert_error_string(cert_error));
    {
      ...
      switch (cert_error) {
        ...
        case X509_V_ERR_IP_ADDRESS_MISMATCH:
          ...
          if (str != NULL) BIO_printf(bio, "Expected IP address = %s\n", str);
          OPENSSL_free(str);
          break;
        default:
          break;
...
int ossl_serial_number_print(BIO* out, const ASN1_INTEGER* bs, int indent) {
  ...
  if (bs->length == 0) {
    if (BIO_puts(out, " (Empty)") <= 0) return -1;
    return 0;
  }
  ...
  if (ok) { /* Reading an int64_t succeeded: print decimal and hex. */
    ...
    if (BIO_printf(out, " %s%ju (%s0x%jx)", neg, ul, neg, ul) <= 0) return -1;
  } else { /* Reading an int64_t failed: just print hex. */
    neg = (bs->type == V_ASN1_NEG_INTEGER) ? " (Negative)" : "";
    if (BIO_printf(out, "\n%*s%s", indent, "", neg) <= 0) return -1;

    for (i = 0; i < bs->length - 1; i++) {
      if (BIO_printf(out, "%02x%c", bs->data[i], ':') <= 0) return -1;
    }
    if (BIO_printf(out, "%02x", bs->data[i]) <= 0) return -1;
  }
  return 0;
}
