...
int
X509_signature_print (BIO *bp, const X509_ALGOR *sigalg,
...
/*
 * Prints serial numbers in decimal and hexadecimal. The indent argument is
 * only used if the serial number is too large to fit in an int64_t.
 */
int
ossl_serial_number_print (BIO *out, const ASN1_INTEGER *bs, int indent)
{
  ...
  if (ok)
    ...
    { /* Reading an int64_t failed: just print hex. */
