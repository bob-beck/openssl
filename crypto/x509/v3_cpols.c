...
static int
i2r_certpol(X509V3_EXT_METHOD* method,
...
static int
displaytext_str2tag(const char* tagstr, unsigned int* tag_len);
static int
displaytext_get_tag_len(const char* tagstr);
...
ASN1_ADB_TEMPLATE(policydefault) = ASN1_SIMPLE(POLICYQUALINFO,
                                               d.other,
                                               ASN1_ANY);
...
r2i_certpol(X509V3_EXT_METHOD* method, X509V3_CTX* ctx, const char* value)
{
  ...
  for (i = 0; i < num; i++) {
    ...
    if (strcmp(pstr, "ia5org") == 0) {
      ...
      if (polsect == NULL) {
        ERR_raise_data(
          ERR_LIB_X509V3, X509V3_R_INVALID_SECTION, "%s", cnf->name);
        goto err;
      }
      ...
      if ((pobj = OBJ_txt2obj(cnf->name, 0)) == NULL) {
        ERR_raise_data(
          ERR_LIB_X509V3, X509V3_R_INVALID_OBJECT_IDENTIFIER, "%s", cnf->name);
        goto err;
      }
      pol = POLICYINFO_new();
      if (pol == NULL) {
...
policy_section(X509V3_CTX* ctx, STACK_OF(CONF_VALUE)* polstrs, int ia5org)
{
  ...
  for (i = 0; i < sk_CONF_VALUE_num(polstrs); i++) {
    ...
    if (strcmp(cnf->name, "policyIdentifier") == 0) {
      ...
      if (!ASN1_STRING_set(
            qual->d.cpsuri, cnf->value, (int)strlen(cnf->value))) {
        ERR_raise(ERR_LIB_X509V3, ERR_R_ASN1_LIB);
        goto err;
      }
...
notice_section(X509V3_CTX* ctx, STACK_OF(CONF_VALUE)* unot, int ia5org)
{
  ...
  for (i = 0; i < sk_CONF_VALUE_num(unot); i++) {
    ...
    if (strcmp(cnf->name, "explicitText") == 0) {
      ...
      if (!ASN1_STRING_set(
            nref->organization, cnf->value, (int)strlen(cnf->value))) {
        ERR_raise(ERR_LIB_X509V3, ERR_R_ASN1_LIB);
        goto err;
      }
