...
static STACK_OF(CONF_VALUE)* i2v_OSSL_BASIC_ATTR_CONSTRAINTS(
    X509V3_EXT_METHOD* method, OSSL_BASIC_ATTR_CONSTRAINTS* battcons,
...
static OSSL_BASIC_ATTR_CONSTRAINTS* v2i_OSSL_BASIC_ATTR_CONSTRAINTS(
  ...
  for (i = 0; i < sk_CONF_VALUE_num(values); i++) {
    ...
    if (strcmp(val->name, "authority") == 0) {
      if (!X509V3_get_value_bool(val, &battcons->authority)) goto err;
    } else if (strcmp(val->name, "pathlen") == 0) {
      if (!X509V3_get_value_int(val, &battcons->pathlen)) goto err;
    } else {
