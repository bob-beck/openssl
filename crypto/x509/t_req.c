...
int X509_REQ_print_ex(BIO* bp, X509_REQ* x, unsigned long nmflags,
  ...
  if (!(cflag & X509_FLAG_NO_PUBKEY)) {
    ...
    if (i2a_ASN1_OBJECT(bp, koid) <= 0) goto err;
    if (BIO_puts(bp, "\n") <= 0) goto err;
    ...
    if (pkey == NULL) {
      ...
      if (EVP_PKEY_print_public(bp, pkey, 16, NULL) <= 0) goto err;
    }
  }
  ...
  if (!(cflag & X509_FLAG_NO_ATTRIBUTES)) {
    ...
    if (X509_REQ_get_attr_count(x) == 0) {
      ...
      for (i = 0; i < X509_REQ_get_attr_count(x); i++) {
        ...
        if (X509_REQ_extension_nid(OBJ_obj2nid(aobj))) continue;
  ...
  if (!(cflag & X509_FLAG_NO_SIGDUMP)) {
    ...
    if (!X509_signature_print(bp, sig_alg, sig)) goto err;
  }
