...
int X509_check_trust(X509* x, int id, int flags) {
  ...
  if (idx < 0) return default_trust(id, x, flags);
...
int X509_TRUST_add(int id, int flags, int (*ck)(X509_TRUST*, X509*, int),
  ...
  /* dup supplied name */
  if ((trtmp->name = OPENSSL_strdup(name)) == NULL) goto err;
...
static void trtable_free(X509_TRUST* p) {
  ...
  if (p->flags & X509_TRUST_DYNAMIC) {
    if (p->flags & X509_TRUST_DYNAMIC_NAME) OPENSSL_free(p->name);
    OPENSSL_free(p);
  }
}
...
static int trust_compat(X509_TRUST* trust, X509* x, int flags) {
  ...
  if (X509_check_purpose(x, -1, 0) != 1) return X509_TRUST_UNTRUSTED;
...
static int obj_trust(int id, X509* x, int flags) {
  ...
  if ((flags & X509_TRUST_DO_SS_COMPAT) == 0) return X509_TRUST_UNTRUSTED;
