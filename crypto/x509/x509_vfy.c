...
int X509_self_signed(X509* cert, int verify_signature) {
  ...
  if (!ossl_x509v3_cache_extensions(cert)) return -1;
  if ((cert->ex_flags & EXFLAG_SS) == 0) return 0;
  if (!verify_signature) return 1;
...
static int verify_cb_cert(X509_STORE_CTX* ctx, X509* x, int depth, int err) {
  ...
  if (err != X509_V_OK) ctx->error = err;
  return ctx->verify_cb(0, ctx);
}

#define CB_FAIL_IF(cond, ctx, cert, depth, err) \
  if ((cond) && verify_cb_cert(ctx, cert, depth, err) == 0) return 0
...
static int check_auth_level(X509_STORE_CTX* ctx) {
  ...
  if (ctx->param->auth_level <= 0) return 1;
...
static int verify_rpk(X509_STORE_CTX* ctx) {
  ...
  if (ctx->verify != NULL) return ctx->verify(ctx);
...
static int verify_chain(X509_STORE_CTX* ctx) {
  ...
  if ((ok = check_name_constraints(ctx)) <= 0) return ok;

#ifndef OPENSSL_NO_RFC3779
  /* RFC 3779 path validation, now that CRL check has been done */
  if ((ok = X509v3_asid_validate_path(ctx)) <= 0) return ok;
  if ((ok = X509v3_addr_validate_path(ctx)) <= 0) return ok;
#endif
...
int X509_STORE_CTX_verify(X509_STORE_CTX* ctx) {
  ...
  if (ctx->rpk != NULL) return x509_verify_rpk(ctx);
...
static int x509_verify_rpk(X509_STORE_CTX* ctx) {
   ...
   */
  if (ret <= 0 && ctx->error == X509_V_OK) ctx->error = X509_V_ERR_UNSPECIFIED;
  return ret;
}
...
static int sk_X509_contains(STACK_OF(X509)* sk, X509* cert) {
  ...
  for (i = 0; i < n; i++)
    if (X509_cmp(sk_X509_value(sk, i), cert) == 0) return 1;
  return 0;
}
  ...
  for (i = 0; i < sk_X509_num(sk); i++) {
    ...
    if (ctx->check_issued(ctx, x, candidate)) {
      if (check_signing_allowed
        ...
        continue;
      if (ossl_x509_check_cert_time(ctx, candidate, -1)) return candidate;
      /*
...
int X509_STORE_CTX_get1_issuer(X509** issuer, X509_STORE_CTX* ctx, X509* x) {
  ...
  *issuer = NULL;
  ...
  ret = -1;
  if ((certs = X509_STORE_CTX_get1_certs(ctx, xn)) == NULL) goto end;
  *issuer = get0_best_issuer_sk(ctx, 0, 0 /* allow duplicates */, certs, x);
  ret = 0;
  if (*issuer != NULL) ret = X509_up_ref(*issuer) ? 1 : -1;
  OSSL_STACK_OF_X509_free(certs);
end:
    ...
    default: /* can only be X509_TRUST_UNTRUSTED */
      switch (X509_check_purpose(x, purpose, must_be_ca > 0)) {
        ...
        case 0:
          break;
        default:
          if ((ctx->param->flags & X509_V_FLAG_X509_STRICT) == 0) return 1;
      }
      break;
  }
...
static int check_extensions(X509_STORE_CTX* ctx) {
  ...
  for (i = 0; i < num; i++) {
    ...
    if (i > 0 && (x->ex_flags & EXFLAG_SI) == 0) plen++;
    /*
...
static int check_name_constraints(X509_STORE_CTX* ctx) {
  ...
  for (i = sk_X509_num(ctx->chain) - 1; i >= 0; i--) {
    ...
    if (i != 0 && (x->ex_flags & EXFLAG_SI) != 0) continue;
...
static int check_id(X509_STORE_CTX* ctx) {
  ...
  if (vpm->hosts != NULL && check_hosts(x, vpm) <= 0) {
    if (!check_id_error(ctx, X509_V_ERR_HOSTNAME_MISMATCH)) return 0;
  }
  if (vpm->email != NULL &&
      X509_check_email(x, vpm->email, vpm->emaillen, 0) <= 0) {
    if (!check_id_error(ctx, X509_V_ERR_EMAIL_MISMATCH)) return 0;
  }
  if (vpm->ip != NULL && X509_check_ip(x, vpm->ip, vpm->iplen, 0) <= 0) {
    if (!check_id_error(ctx, X509_V_ERR_IP_ADDRESS_MISMATCH)) return 0;
  }
  return 1;
}
...
static int check_trust(X509_STORE_CTX* ctx, int num_untrusted) {
  ...
  if (DANETLS_HAS_TA(dane) && num_untrusted > 0 && num_untrusted < num) {
    ...
    if (trust != X509_TRUST_UNTRUSTED) return trust;
  }
  ...
  for (i = num_untrusted; i < num; i++) {
    ...
    if (trust == X509_TRUST_TRUSTED) goto trusted;
    if (trust == X509_TRUST_REJECTED) goto rejected;
  }
  ...
  if (num_untrusted < num) {
    if ((ctx->param->flags & X509_V_FLAG_PARTIAL_CHAIN) != 0) goto trusted;
  ...
  if (num_untrusted == num &&
    ...
    if (res < 0) return res;
    if (res == 0) return X509_TRUST_UNTRUSTED;
...
trusted:
  if (!DANETLS_ENABLED(dane)) return X509_TRUST_TRUSTED;
  if (dane->pdpth < 0) dane->pdpth = num_untrusted;
  /* With DANE, PKIX alone is not trusted until we have both */
  if (dane->mdpth >= 0) return X509_TRUST_TRUSTED;
  return X509_TRUST_UNTRUSTED;
}
...
static int check_revocation(X509_STORE_CTX* ctx) {
  ...
  if (!crl_check_enabled && !ocsp_check_enabled) return 1;
  ...
  if (ocsp_check_enabled) {
    ...
    for (i = 0; i <= last; i++) {
      ...
      if (ctx->current_cert->ex_flags & EXFLAG_SS) continue;
       ...
       * certificate.
       */
      if (ok == V_OCSP_CERTSTATUS_GOOD) continue;
...
static int check_cert_ocsp_resp(X509_STORE_CTX* ctx) {
  ...
  if (num < 0 || num <= ctx->error_depth) return X509_V_ERR_OCSP_NO_RESPONSE;

  if ((resp = sk_OCSP_RESPONSE_value(ctx->ocsp_resp, ctx->error_depth)) ==
          NULL ||
  ...
  for (i = 0; i < num; i++) {
    ...
    cert_id =
        OCSP_cert_to_id(cert_id_md, ctx->current_cert, ctx->current_issuer);
    if (cert_id == NULL) {
      ret = X509_V_ERR_OCSP_RESP_INVALID;
      goto end;
    }

    if (!OCSP_id_cmp(cert_id, sr_cert_id)) break;
...
static int check_cert_crl(X509_STORE_CTX* ctx) {
  ...
  /* skip if cert is apparently self-signed */
  if (ctx->current_cert->ex_flags & EXFLAG_SS) return 1;
  if ((x->ex_flags & EXFLAG_PROXY) != 0) return 1;
...
static int check_crl_time(X509_STORE_CTX* ctx, X509_CRL* crl, int notify) {
  ...
  if (X509_CRL_get0_nextUpdate(crl)) {
    ...
    if (i < 0 && (ctx->current_crl_score & CRL_SCORE_TIME_DELTA) == 0) {
      if (!notify || !verify_cb_crl(ctx, X509_V_ERR_CRL_HAS_EXPIRED)) return 0;
    }
  }

  if (notify) ctx->current_crl = NULL;
...
static int get_crl_sk(X509_STORE_CTX* ctx, X509_CRL** pcrl, X509_CRL** pdcrl,
  ...
  for (i = 0; i < sk_X509_CRL_num(crls); i++) {
    ...
    if (crl_score < best_score || crl_score == 0) continue;
  ...
  if (best_crl != NULL) {
    if (!X509_CRL_up_ref(best_crl)) return 0;
    X509_CRL_free(*pcrl);
    *pcrl = best_crl;
  ...
  if (best_score >= CRL_SCORE_VALID) return 1;
...
static int crl_extension_match(X509_CRL* a, X509_CRL* b, int nid) {
  ...
  if (i >= 0) {
    ...
    if (X509_CRL_get_ext_by_NID(a, nid, i) != -1) return 0;
    exta = X509_EXTENSION_get_data(X509_CRL_get_ext(a, i));
  }

  i = X509_CRL_get_ext_by_NID(b, nid, -1);
  if (i >= 0) {
    if (X509_CRL_get_ext_by_NID(b, nid, i) != -1) return 0;
    extb = X509_EXTENSION_get_data(X509_CRL_get_ext(b, i));
  }

  if (exta == NULL && extb == NULL) return 1;
...
static int check_delta_base(X509_CRL* delta, X509_CRL* base) {
  ...
  if (delta->base_crl_number == NULL) return 0;
  /* Base must have a CRL number */
  if (base->crl_number == NULL) return 0;
  /* Issuer names must match */
  if (X509_NAME_cmp(X509_CRL_get_issuer(base), X509_CRL_get_issuer(delta)) != 0)
    return 0;
  /* AKID and IDP must match */
  if (!crl_extension_match(delta, base, NID_authority_key_identifier)) return 0;
  if (!crl_extension_match(delta, base, NID_issuing_distribution_point))
    return 0;
  /* Delta CRL base number must not exceed Full CRL number. */
  if (ASN1_INTEGER_cmp(delta->base_crl_number, base->crl_number) > 0) return 0;
...
static void get_delta_sk(X509_STORE_CTX* ctx, X509_CRL** dcrl, int* pscore,
  ...
  if ((ctx->param->flags & X509_V_FLAG_USE_DELTAS) == 0) return;
  ...
  for (i = 0; i < sk_X509_CRL_num(crls); i++) {
    ...
    if (check_delta_base(delta, base)) {
      ...
      *dcrl = delta;
      ...
      if (check_crl_time(ctx, delta, 0)) *pscore |= CRL_SCORE_TIME_DELTA;
...
static int get_crl_score(X509_STORE_CTX* ctx, X509** pissuer,
  ...
  if ((crl->idp_flags & IDP_INVALID) != 0) return 0;
  /* Reason codes or indirect CRLs need extended CRL support */
  if ((ctx->param->flags & X509_V_FLAG_EXTENDED_CRL_SUPPORT) == 0) {
    if (crl->idp_flags & (IDP_INDIRECT | IDP_REASONS)) return 0;
  } else if ((crl->idp_flags & IDP_REASONS) != 0) {
    /* If no new reasons reject */
    if ((crl->idp_reasons & ~tmp_reasons) == 0) return 0;
  }
  ...
  if (X509_NAME_cmp(X509_get_issuer_name(x), X509_CRL_get_issuer(crl)) != 0) {
    if ((crl->idp_flags & IDP_INDIRECT) == 0) return 0;
  } else {
    crl_score |= CRL_SCORE_ISSUER_NAME;
  }

  if ((crl->flags & EXFLAG_CRITICAL) == 0) crl_score |= CRL_SCORE_NOCRITICAL;
  ...
  if ((crl_score & CRL_SCORE_AKID) == 0) return 0;

  /* Check cert for matching CRL distribution points */
  if (crl_crldp_check(x, crl, crl_score, &crl_reasons)) {
    /* If no new reasons reject */
    if ((crl_reasons & ~tmp_reasons) == 0) return 0;
...
static void crl_akid_check(X509_STORE_CTX* ctx, X509_CRL* crl, X509** pissuer,
  ...
  if (cidx != sk_X509_num(ctx->chain) - 1) cidx++;
  ...
  for (cidx++; cidx < sk_X509_num(ctx->chain); cidx++) {
    crl_issuer = sk_X509_value(ctx->chain, cidx);
    if (X509_NAME_cmp(X509_get_subject_name(crl_issuer), cnm)) continue;
    if (X509_check_akid(crl_issuer, crl->akid) == X509_V_OK) {
  ...
  if ((ctx->param->flags & X509_V_FLAG_EXTENDED_CRL_SUPPORT) == 0) return;
  ...
  for (i = 0; i < sk_X509_num(ctx->untrusted); i++) {
    ...
    if (X509_NAME_cmp(X509_get_subject_name(crl_issuer), cnm) != 0) continue;
    if (X509_check_akid(crl_issuer, crl->akid) == X509_V_OK) {
      *pissuer = crl_issuer;
...
static int check_crl_path(X509_STORE_CTX* ctx, X509* x) {
  ...
  if (ctx->parent != NULL) return 0;
  if (!X509_STORE_CTX_init(&crl_ctx, ctx->store, x, ctx->untrusted)) return -1;
...
static int idp_check_dp(DIST_POINT_NAME* a, DIST_POINT_NAME* b) {
  ...
  if (a->type == 1) {
    if (a->dpname == NULL) return 0;
    /* Case 1: two X509_NAME */
    if (b->type == 1) {
      if (b->dpname == NULL) return 0;
  ...
  if (nm != NULL) {
    for (i = 0; i < sk_GENERAL_NAME_num(gens); i++) {
      ...
      if (gena->type != GEN_DIRNAME) continue;
      if (X509_NAME_cmp(nm, gena->d.directoryName) == 0) return 1;
    }
    return 0;
  }
  ...
  for (i = 0; i < sk_GENERAL_NAME_num(a->name.fullname); i++) {
    ...
    for (j = 0; j < sk_GENERAL_NAME_num(b->name.fullname); j++) {
      ...
      if (GENERAL_NAME_cmp(gena, genb) == 0) return 1;
    }
  }
...
static int crldp_check_crlissuer(DIST_POINT* dp, X509_CRL* crl, int crl_score) {
  ...
  if (dp->CRLissuer == NULL) return (crl_score & CRL_SCORE_ISSUER_NAME) != 0;
  for (i = 0; i < sk_GENERAL_NAME_num(dp->CRLissuer); i++) {
    GENERAL_NAME* gen = sk_GENERAL_NAME_value(dp->CRLissuer, i);

    if (gen->type != GEN_DIRNAME) continue;
    if (X509_NAME_cmp(gen->d.directoryName, nm) == 0) return 1;
  }
  return 0;
}
...
static int crl_crldp_check(X509* x, X509_CRL* crl, int crl_score,
  ...
  if ((crl->idp_flags & IDP_ONLYATTR) != 0) return 0;
  if ((x->ex_flags & EXFLAG_CA) != 0) {
    if ((crl->idp_flags & IDP_ONLYUSER) != 0) return 0;
  } else {
    if ((crl->idp_flags & IDP_ONLYCA) != 0) return 0;
  }
...
static int get_crl_delta(X509_STORE_CTX* ctx, X509_CRL** pcrl, X509_CRL** pdcrl,
  ...
  if (skcrl == NULL && crl != NULL) goto done;
...
static int check_crl(X509_STORE_CTX* ctx, X509_CRL* crl) {
  ...
  if (ctx->current_issuer != NULL) {
    ...
    if (!ossl_assert(issuer != NULL)) return 0;
  ...
  if (ikey != NULL) {
    ...
    if (rv != X509_V_OK && !verify_cb_crl(ctx, rv)) return 0;
...
static int cert_crl(X509_STORE_CTX* ctx, X509_CRL* crl, X509* x) {
  ...
  if (X509_CRL_get0_by_cert(crl, &rev, x)) {
    if (rev->reason == CRL_REASON_REMOVE_FROM_CRL) return 2;
    if (!verify_cb_crl(ctx, X509_V_ERR_CERT_REVOKED)) return 0;
  }
...
static int check_policy(X509_STORE_CTX* ctx) {
  ...
  if (ctx->bare_ta_signed) (void)sk_X509_pop(ctx->chain);
  ...
  if (ret == X509_PCY_TREE_INVALID) {
    ...
    for (i = 0; i < sk_X509_num(ctx->chain); i++) {
      ...
      if ((x->ex_flags & EXFLAG_INVALID_POLICY) != 0) cbcalled = 1;
  ...
  if ((ctx->param->flags & X509_V_FLAG_NOTIFY_POLICY) != 0) {
     ...
     */
    if (!ctx->verify_cb(2, ctx)) return 0;
  }
...
static int internal_verify(X509_STORE_CTX* ctx) {
  ...
  if (ctx->rpk != NULL) {
    if (!ctx->verify_cb(ctx->error == X509_V_OK, ctx)) return 0;
    return 1;
  }
  ...
  while (n >= 0) {
    ...
    if (!ossl_x509_check_cert_time(ctx, xs, n)) return 0;
    ...
    ctx->current_cert = xs;
    ctx->error_depth = n;
    if (!ctx->verify_cb(1, ctx)) return 0;
...
int X509_cmp_time(const ASN1_TIME* ctm, time_t* cmp_time) {
  ...
  switch (ctm->type) {
    case V_ASN1_UTCTIME:
      if (ctm->length != (int)(utctime_length)) return 0;
      break;
    case V_ASN1_GENERALIZEDTIME:
      if (ctm->length != (int)(generalizedtime_length)) return 0;
      break;
    default:
      return 0;
  }
  ...
  for (i = 0; i < ctm->length - 1; i++) {
    if (!ossl_ascii_isdigit(ctm->data[i])) return 0;
  }
  if (ctm->data[ctm->length - 1] != upper_z) return 0;
  ...
  if (asn1_cmp_time == NULL) goto err;
  if (ASN1_TIME_diff(&day, &sec, ctm, asn1_cmp_time) == 0) goto err;
...
int X509_cmp_timeframe(const X509_VERIFY_PARAM* vpm, const ASN1_TIME* start,
  ...
  if (end != NULL && X509_cmp_time(end, time) < 0) return 1;
  if (start != NULL && X509_cmp_time(start, time) > 0) return -1;
  return 0;
}
...
int X509_get_pubkey_parameters(EVP_PKEY* pkey, STACK_OF(X509)* chain) {
  ...
  for (i = 0; i < sk_X509_num(chain); i++) {
    ...
    if (!EVP_PKEY_missing_parameters(ktmp)) break;
    ktmp = NULL;
  }
  if (ktmp == NULL) {
  ...
  if (pkey != NULL) return EVP_PKEY_copy_parameters(pkey, ktmp);
  return 1;
}
...
#ifndef OPENSSL_NO_OCSP
void X509_STORE_CTX_set_ocsp_resp(X509_STORE_CTX* ctx,
                                  STACK_OF(OCSP_RESPONSE)* sk) {
  ctx->ocsp_resp = sk;
}
#endif
...
int X509_STORE_CTX_purpose_inherit(X509_STORE_CTX* ctx, int def_purpose,
  ...
  if (purpose == 0) purpose = def_purpose;
  /*
  ...
  if (ctx->param->purpose == 0 && purpose != 0) ctx->param->purpose = purpose;
  if (ctx->param->trust == 0 && trust != 0) ctx->param->trust = trust;
  return 1;
}
...
int X509_STORE_CTX_init_rpk(X509_STORE_CTX* ctx, X509_STORE* store,
                            EVP_PKEY* rpk) {
  if (!X509_STORE_CTX_init(ctx, store, NULL, NULL)) return 0;
  ctx->rpk = rpk;
  return 1;
}
...
int X509_STORE_CTX_init(X509_STORE_CTX* ctx, X509_STORE* store, X509* x509,
  ...
  if (!X509_STORE_CTX_set_default(ctx, "default")) goto err;
  ...
  if (ctx->param->trust == X509_TRUST_DEFAULT) {
    ...
    if (xp != NULL) ctx->param->trust = X509_PURPOSE_get_trust(xp);
  }
...
void X509_STORE_CTX_cleanup(X509_STORE_CTX* ctx) {
  ...
  if (ctx->param != NULL) {
    if (ctx->parent == NULL) X509_VERIFY_PARAM_free(ctx->param);
    ctx->param = NULL;
  }
...
static int dane_match_cert(X509_STORE_CTX* ctx, X509* cert, int depth) {
  ...
  if (depth >= ctx->num_untrusted) mask &= DANETLS_PKIX_MASK;
   ...
   */
  if (dane->mdpth >= 0) mask &= ~DANETLS_PKIX_MASK;
  ...
  for (i = 0; matched == 0 && i < recnum; ++i) {
    ...
    if ((DANETLS_USAGE_BIT(t->usage) & mask) == 0) continue;
    if (t->usage != usage) {
      usage = t->usage;
    ...
    if (t->selector != selector) {
       ...
       * other than "Full".
       */
      if (dane->dctx->mdord[t->mtype] < ordinal) continue;
    }
    ...
    if (cmplen == t->dlen && memcmp(cmpbuf, t->data, cmplen) == 0) {
      if (DANETLS_USAGE_BIT(usage) & DANETLS_DANE_MASK) matched = 1;
...
static int check_dane_issuer(X509_STORE_CTX* ctx, int depth) {
  ...
  if (!DANETLS_HAS_TA(dane) || depth == 0) return X509_TRUST_UNTRUSTED;
...
static int check_dane_pkeys(X509_STORE_CTX* ctx) {
  ...
  for (i = 0; i < recnum; ++i) {
    ...
    for (; num > ctx->num_untrusted; --num) X509_free(sk_X509_pop(ctx->chain));
...
static int dane_match_rpk(X509_STORE_CTX* ctx, EVP_PKEY* rpk) {
  ...
  if ((len = i2d_PUBKEY(rpk, &i2dbuf)) <= 0) return -1;
...
static int dane_verify(X509_STORE_CTX* ctx) {
  ...
  if (done && !X509_get_pubkey_parameters(NULL, ctx->chain)) return -1;

  if (matched > 0) {
    /* Callback invoked as needed */
    if (!check_leaf_suiteb(ctx, cert)) return 0;
    /* Callback invoked as needed */
...
static int build_chain(X509_STORE_CTX* ctx) {
  ...
  if (!ossl_assert(num == 1 && ctx->num_untrusted == num)) goto int_err;
...
#define S_DOALTERNATE (1 << 2) /* Retry with pruned alternate chain */
   ...
   * might be reasonable.
   */
  if (ctx->param->depth > INT_MAX / 2) ctx->param->depth = INT_MAX / 2;
  ...
  while (search != 0) {
    ...
    if ((search & S_DOTRUSTED) != 0) {
      ...
      if (ok > 0) {
        ...
        if (!self_signed) { /* untrusted not self-signed certificate */
          ...
          if ((self_signed = X509_self_signed(issuer, 0)) < 0) goto int_err;
        } else {
          /*
         ...
         */
        if (ok) {
          if (!ossl_assert(ctx->num_untrusted <= num)) goto int_err;
          search &= ~S_DOUNTRUSTED;
          trust = check_trust(ctx, num);
          if (trust != X509_TRUST_UNTRUSTED) break;
          if (!self_signed) continue;
        }
      }
      ...
      if ((search & S_DOUNTRUSTED) == 0) {
        ...
        if ((search & S_DOALTERNATE) != 0 && --alt_untrusted > 0) continue;
    ...
    if ((search & S_DOUNTRUSTED) != 0) {
      num = sk_X509_num(ctx->chain);
      if (!ossl_assert(num == ctx->num_untrusted)) goto int_err;
      ...
      if (trust == X509_TRUST_TRUSTED || trust == X509_TRUST_REJECTED) break;
    }
  }
...
STACK_OF(X509)* X509_build_chain(X509* target, STACK_OF(X509)* certs,
  ...
  if ((ctx = X509_STORE_CTX_new_ex(libctx, propq)) == NULL) return NULL;
  if (!X509_STORE_CTX_init(ctx, store, target, finish_chain ? certs : NULL))
    goto err;
  if (!finish_chain) X509_STORE_CTX_set0_trusted_stack(ctx, certs);
  ...
  if (!build_chain(ctx) && finish_chain) goto err;
...
static int check_key_level(X509_STORE_CTX* ctx, EVP_PKEY* pkey) {
  ...
  if (level > NUM_AUTH_LEVELS) level = NUM_AUTH_LEVELS;
...
static int check_curve(X509* cert) {
  ...
  if (EVP_PKEY_get_id(pkey) != EVP_PKEY_EC) return 1;
...
static int check_sig_level(X509_STORE_CTX* ctx, X509* cert) {
  ...
  if (!X509_get_signature_info(cert, NULL, NULL, &secbits, NULL)) return 0;
