...
static int get_crl_delta (X509_STORE_CTX *ctx, X509_CRL **pcrl,
                          X509_CRL **pdcrl, X509 *x);
 ...
 */
static int
verify_cb_ocsp (X509_STORE_CTX *ctx, int err)
{
  ctx->error = err;
  return ctx->verify_cb (0, ctx);
}
 ...
 */
static X509 *
get0_best_issuer_sk (X509_STORE_CTX *ctx, int check_signing_allowed,
      ...
      for (i = 0; i <= last; i++)
        {
           ...
           * If OCSP is not enabled or the client receives a
           * "ocsp_response_list" that does not contain a response for one or
           * more of the certificates in the completed certificate chain, the
           * client SHOULD attempt to validate the certificate using an
           * alternative retrieval method, such as downloading the relevant
           * CRL;
           */
...
#ifndef OPENSSL_NO_OCSP
static int
check_cert_ocsp_resp (X509_STORE_CTX *ctx)
{
  ...
  for (i = 0; i < num; i++)
    {
      ...
      cert_id = OCSP_cert_to_id (cert_id_md, ctx->current_cert,
                                 ctx->current_issuer);
      if (cert_id == NULL)
        {
          ret = X509_V_ERR_OCSP_RESP_INVALID;
...
/* Sadly, returns 0 also on internal error. */
static int
check_cert_crl (X509_STORE_CTX *ctx)
{
 ...
 */
static void
get_delta_sk (X509_STORE_CTX *ctx, X509_CRL **dcrl, int *pscore,
 ...
 */
int
X509_cmp_timeframe (const X509_VERIFY_PARAM *vpm, const ASN1_TIME *start,
                    const ASN1_TIME *end)
{
  time_t ref_time;
  time_t *time = NULL;
 ...
 */
X509_CRL *
X509_CRL_diff (X509_CRL *base, X509_CRL *newer, EVP_PKEY *skey,
...
STACK_OF (X509) *
X509_STORE_CTX_get0_chain (const X509_STORE_CTX *ctx)
{
  return ctx->chain;
}

STACK_OF (X509) *
X509_STORE_CTX_get1_chain (const X509_STORE_CTX *ctx)
{
  if (ctx->chain == NULL)
    return NULL;
...
#ifndef OPENSSL_NO_OCSP
void
X509_STORE_CTX_set_ocsp_resp (X509_STORE_CTX *ctx,
...
/*
 * Set alternative get_issuer method: just from a STACK of trusted
 * certificates. This avoids the complexity of X509_STORE where it is not
 * needed.
 */
void
...
void
X509_STORE_CTX_set_current_reasons (X509_STORE_CTX *ctx,
...
STACK_OF (X509) *
X509_STORE_CTX_get0_untrusted (const X509_STORE_CTX *ctx)
{
...
void
X509_STORE_CTX_set_verify (X509_STORE_CTX *ctx,
...
void
X509_STORE_CTX_set_get_crl (X509_STORE_CTX *ctx,
      ...
      if ((search & S_DOUNTRUSTED) != 0)
        {
          ...
          issuer = (X509_self_signed (curr, 0) > 0 || num > max_depth)
                       ? NULL
                       : get0_best_issuer_sk (ctx, 0, 1 /* no_dup */,
                                              sk_untrusted, curr);
          if (issuer == NULL)
            {
              /*
...
X509_build_chain (X509 *target, STACK_OF (X509) *certs, X509_STORE *store,
                  int with_self_signed, OSSL_LIB_CTX *libctx,
                  const char *propq)
{
