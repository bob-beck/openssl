...
static int
get_crl_score(X509_STORE_CTX* ctx,
              X509** pissuer,
...
static void
get_delta_sk(X509_STORE_CTX* ctx,
             X509_CRL** dcrl,
             int* pcrl_score,
             X509_CRL* base,
             STACK_OF(X509_CRL)* crls);
static void
crl_akid_check(X509_STORE_CTX* ctx,
               X509_CRL* crl,
               X509** pissuer,
               int* pcrl_score);
static int
...
static int
check_crl_chain(X509_STORE_CTX* ctx,
...
verify_chain(X509_STORE_CTX* ctx)
{
  ...
  err = X509_chain_check_suiteb(
    &ctx->error_depth, NULL, ctx->chain, ctx->param->flags);
 ...
 */
static X509*
get0_best_issuer_sk(X509_STORE_CTX* ctx,
                    int check_signing_allowed,
                    int no_dup,
                    STACK_OF(X509)* sk,
                    X509* x)
{
 ...
 */
static int
check_purpose(X509_STORE_CTX* ctx,
              X509* x,
              int purpose,
              int depth,
              int must_be_ca)
...
check_revocation(X509_STORE_CTX* ctx)
{
  ...
  int crl_check_enabled =
    (ctx->param->flags & (X509_V_FLAG_CRL_CHECK | X509_V_FLAG_CRL_CHECK_ALL)) !=
    0;
...
check_cert_ocsp_resp(X509_STORE_CTX* ctx)
{
  ...
  if (OCSP_resp_find_status(
        bs, cert_id, &cert_status, &crl_reason, &rev, &thisupd, &nextupd) <=
      0) {
    ret = X509_V_ERR_OCSP_RESP_INVALID;
    goto end;
  }
...
check_policy(X509_STORE_CTX* ctx)
{
  ...
  ret = X509_policy_check(&ctx->tree,
                          &ctx->explicit_policy,
                          ctx->chain,
                          ctx->param->policies,
                          ctx->param->flags);
 ...
 */
int
X509_cmp_timeframe(const X509_VERIFY_PARAM* vpm,
...
#ifndef OPENSSL_NO_OCSP
void
X509_STORE_CTX_set_ocsp_resp(X509_STORE_CTX* ctx, STACK_OF(OCSP_RESPONSE)* sk)
{
  ctx->ocsp_resp = sk;
}
#endif
 ...
 */
int
X509_STORE_CTX_purpose_inherit(X509_STORE_CTX* ctx,
                               int def_purpose,
                               int purpose,
                               int trust)
{
...
int
X509_STORE_CTX_init(X509_STORE_CTX* ctx,
                    X509_STORE* store,
                    X509* x509,
                    STACK_OF(X509)* chain)
{
...
#define S_DOALTERNATE (1 << 2) /* Retry with pruned alternate chain */
  ...
  switch (trust) {
    ...
    default:
      ...
      CB_FAIL_IF(
        num > max_depth, ctx, NULL, num - 1, X509_V_ERR_CERT_CHAIN_TOO_LONG);
