...
int X509_LOOKUP_by_alias(X509_LOOKUP* ctx, X509_LOOKUP_TYPE type,
  ...
  if ((ctx->method == NULL) || (ctx->method->get_by_alias == NULL)) return 0;
...
int X509_STORE_up_ref(X509_STORE* xs) {
  ...
  if (CRYPTO_UP_REF(&xs->references, &i) <= 0) return 0;
...
X509_LOOKUP* X509_STORE_add_lookup(X509_STORE* xs, X509_LOOKUP_METHOD* m) {
  ...
  lu->store_ctx = xs;
  if (sk_X509_LOOKUP_push(xs->get_cert_methods, lu)) return lu;
...
int ossl_x509_store_ctx_get_by_subject(const X509_STORE_CTX* ctx,
  ...
  if (ret != NULL) {
    if (!X509_OBJECT_up_ref_count(tmp)) return -1;
    ret->type = tmp->type;
    ret->data = tmp->data;
  }
  return 1;
...
X509* X509_OBJECT_get0_X509(const X509_OBJECT* a) {
  if (a == NULL || a->type != X509_LU_X509) return NULL;
  return a->data.x509;
}

X509_CRL* X509_OBJECT_get0_X509_CRL(const X509_OBJECT* a) {
  if (a == NULL || a->type != X509_LU_CRL) return NULL;
  return a->data.crl;
}
...
STACK_OF(X509_OBJECT)* X509_STORE_get1_objects(X509_STORE* store) {
  ...
  if (!ossl_x509_store_read_lock(store)) return NULL;
...
STACK_OF(X509)* X509_STORE_get1_all_certs(X509_STORE* store) {
  ...
  if ((sk = sk_X509_new_null()) == NULL) return NULL;
  if (!X509_STORE_lock(store)) goto out_free;
...
STACK_OF(X509)* X509_STORE_CTX_get1_certs(X509_STORE_CTX* ctx,
  ...
  if (store == NULL) return sk_X509_new_null();

  if (!X509_STORE_lock(store)) return NULL;
  ...
  if (idx < 0) {
    ...
    if (i <= 0) return i < 0 ? NULL : sk_X509_new_null();
    if (!X509_STORE_lock(store)) return NULL;
    sk_X509_OBJECT_sort(store->objs);
    idx = x509_object_idx_cnt(store->objs, X509_LU_X509, nm, &cnt);
  }
...
X509_OBJECT* X509_OBJECT_retrieve_match(STACK_OF(X509_OBJECT)* h,
  ...
  for (i = idx, num = sk_X509_OBJECT_num(h); i < num; i++) {
    ...
    if (x->type == X509_LU_X509) {
      if (!X509_cmp(obj->data.x509, x->data.x509)) return obj;
    } else if (x->type == X509_LU_CRL) {
      if (X509_CRL_match(obj->data.crl, x->data.crl) == 0) return obj;
    } else {
      return obj;
    }
  }
