...
int
X509_LOOKUP_ctrl_ex (X509_LOOKUP *ctx, int cmd, const char *argc, long argl,
...
int
X509_LOOKUP_by_subject_ex (X509_LOOKUP *ctx, X509_LOOKUP_TYPE type,
...
int
X509_LOOKUP_by_subject (X509_LOOKUP *ctx, X509_LOOKUP_TYPE type,
                        const X509_NAME *name, X509_OBJECT *ret)
{
...
X509_OBJECT *
X509_STORE_CTX_get_obj_by_subject (X509_STORE_CTX *ctx, X509_LOOKUP_TYPE type,
                                   const X509_NAME *name)
{
      ...
      for (i = 0; i < sk_X509_LOOKUP_num (store->get_cert_methods); i++)
        {
          ...
          j = X509_LOOKUP_by_subject_ex (lu, type, name, &stmp, ctx->libctx,
                                         ctx->propq);
          if (j != 0)
            { /* non-zero value is considered success here */
              tmp = &stmp;
              break;
            }
        }
...
X509_LOOKUP_TYPE
X509_OBJECT_get_type (const X509_OBJECT *a) { return a->type; }
...
void
X509_STORE_set_check_crl (X509_STORE *xs,
