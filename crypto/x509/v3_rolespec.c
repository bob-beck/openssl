...
ASN1_ITEM_TEMPLATE(OSSL_ROLE_SPEC_CERT_ID_SYNTAX) =
    ASN1_EX_TEMPLATE_TYPE(ASN1_TFLG_SEQUENCE_OF, 0,
                          OSSL_ROLE_SPEC_CERT_ID_SYNTAX, OSSL_ROLE_SPEC_CERT_ID)
...
static int i2r_OSSL_ROLE_SPEC_CERT_ID(X509V3_EXT_METHOD* method,
                                      ...
                                      int indent) {
  if (BIO_printf(out, "%*sRole Name: ", indent, "") <= 0) return 0;
  if (GENERAL_NAME_print(out, rscid->roleName) <= 0) return 0;
  if (BIO_puts(out, "\n") <= 0) return 0;
  if (BIO_printf(out, "%*sRole Certificate Issuer: ", indent, "") <= 0)
    return 0;
  if (GENERAL_NAME_print(out, rscid->roleCertIssuer) <= 0) return 0;
...
static int i2r_OSSL_ROLE_SPEC_CERT_ID_SYNTAX(
    X509V3_EXT_METHOD* method, OSSL_ROLE_SPEC_CERT_ID_SYNTAX* rscids, BIO* out,
    int indent) {
