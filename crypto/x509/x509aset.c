...
static int
replace_gentime (ASN1_STRING **dest, const ASN1_GENERALIZEDTIME *src)
{
  ASN1_STRING *s;
...
static int
replace_dirName (GENERAL_NAMES **names, const X509_NAME *dirName)
{
...
int
OSSL_OBJECT_DIGEST_INFO_set1_digest (OSSL_OBJECT_DIGEST_INFO *o,
...
int
OSSL_ISSUER_SERIAL_set1_issuer (OSSL_ISSUER_SERIAL *isss,
...
int
OSSL_ISSUER_SERIAL_set1_serial (OSSL_ISSUER_SERIAL *isss,
...
int
OSSL_ISSUER_SERIAL_set1_issuerUID (OSSL_ISSUER_SERIAL *isss,
...
int
X509_ACERT_set_version (X509_ACERT *x, long version)
{
  return ASN1_INTEGER_set (&x->acinfo->version, version);
}

void
X509_ACERT_set0_holder_entityName (X509_ACERT *x, GENERAL_NAMES *names)
{
...
void
X509_ACERT_set0_holder_baseCertId (X509_ACERT *x, OSSL_ISSUER_SERIAL *isss)
{
...
void
X509_ACERT_set0_holder_digest (X509_ACERT *x, OSSL_OBJECT_DIGEST_INFO *dinfo)
{
...
int
X509_ACERT_set1_issuerName (X509_ACERT *x, const X509_NAME *name)
{
...
int
X509_ACERT_set1_serialNumber (X509_ACERT *x, const ASN1_INTEGER *serial)
{
  return ASN1_STRING_copy (&x->acinfo->serialNumber, serial);
}

int
X509_ACERT_set1_notBefore (X509_ACERT *x, const ASN1_GENERALIZEDTIME *time)
{
  return replace_gentime (&x->acinfo->validityPeriod.notBefore, time);
}

int
X509_ACERT_set1_notAfter (X509_ACERT *x, const ASN1_GENERALIZEDTIME *time)
{
