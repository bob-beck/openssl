...
static_ASN1_ITEM_TEMPLATE_END (X509_NAME_INTERNAL)
    ...
    /*
     * Normally that's where it would end: we'd have two nested STACK
     * structures representing the ASN1. Unfortunately X509_NAME uses a
     * completely different form and caches encodings so we have to process the
     * internal form and convert to the external form.
     */
...
static int
x509_name_ex_i2d (const ASN1_VALUE **val, unsigned char **out,
                  const ASN1_ITEM *it, int tag, int aclass)
{
  int ret;
...
static int
x509_name_ex_print (BIO *out, const ASN1_VALUE **pval, int indent,
                    const char *fname, const ASN1_PCTX *pctx)
{
...
static int
asn1_string_canon (ASN1_STRING *out, const ASN1_STRING *in)
{
...
static int
i2d_name_canon (const STACK_OF (STACK_OF_X509_NAME_ENTRY) *_intname,
                unsigned char **in)
{
  int i, len, ltmp;
  const ASN1_VALUE *v;
  ...
  for (i = 0; i < sk_ASN1_VALUE_num (intname); i++)
    {
      ...
      ltmp = ASN1_item_ex_i2d (&v, in, ASN1_ITEM_rptr (X509_NAME_ENTRIES), -1,
                               -1);
...
int
X509_NAME_set (X509_NAME **xn, const X509_NAME *name)
{
  X509_NAME *name_copy;
...
int
X509_NAME_get0_der (const X509_NAME *nm, const unsigned char **pder,
                    size_t *pderlen)
{
