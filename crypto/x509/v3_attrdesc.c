...
ASN1_SEQUENCE(OSSL_ATTRIBUTE_DESCRIPTOR) = {
  ...
  ASN1_SIMPLE(OSSL_ATTRIBUTE_DESCRIPTOR,
              dominationRule,
...
static int
i2r_HASH(X509V3_EXT_METHOD* method, OSSL_HASH* hash, BIO* out, int indent)
{
  ...
  if (hash->hashValue == NULL)
    return 0;
  return ossl_bio_print_hex(
    out, hash->hashValue->data, hash->hashValue->length);
}

static int
i2r_INFO_SYNTAX_POINTER(X509V3_EXT_METHOD* method,
...
static int
i2r_OSSL_INFO_SYNTAX(X509V3_EXT_METHOD* method,
  ...
  switch (info->type) {
    ...
    case OSSL_INFO_SYNTAX_TYPE_POINTER:
      ...
      return i2r_INFO_SYNTAX_POINTER(
        method, info->choice.pointer, out, indent + 4);
    default:
      return 0;
  }
  return 0;
...
static int
i2r_OSSL_PRIVILEGE_POLICY_ID(X509V3_EXT_METHOD* method,
                             ...
                             int indent)
{
  ...
  if (OBJ_obj2txt(buf, sizeof(buf), ppid->privilegePolicy, 1) <= 0)
    return 0;
  if (BIO_printf(
        out, "%*sPrivilege Policy Identifier: %s\n", indent, "", buf) <= 0)
    return 0;
...
static int
i2r_OSSL_ATTRIBUTE_DESCRIPTOR(X509V3_EXT_METHOD* method,
                              ...
                              int indent)
{
  ...
  if (BIO_printf(out, "%*sDomination Rule:\n", indent, "") <= 0)
    return 0;
  return i2r_OSSL_PRIVILEGE_POLICY_ID(
    method, ad->dominationRule, out, indent + 4);
}
