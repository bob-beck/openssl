...
EVP_PKEY*
X509_REQ_get0_pubkey(const X509_REQ* req)
{
  if (req == NULL)
    return NULL;
...
int
X509_REQ_check_private_key(const X509_REQ* req, EVP_PKEY* pkey)
{
...
int
X509_REQ_add_extensions(X509_REQ* req, const STACK_OF(X509_EXTENSION)* exts)
{
                          ...
                          int len)
{
  ...
  if (!X509at_add1_attr_by_OBJ(
        &req->req_info.attributes, obj, type, bytes, len))
    return 0;
  req->req_info.enc.modified = 1;
  return 1;
}
                          ...
                          int len)
{
  ...
  if (!X509at_add1_attr_by_NID(
        &req->req_info.attributes, nid, type, bytes, len))
    return 0;
  req->req_info.enc.modified = 1;
  return 1;
}
                          ...
                          int len)
{
  ...
  if (!X509at_add1_attr_by_txt(
        &req->req_info.attributes, attrname, type, bytes, len))
    return 0;
  req->req_info.enc.modified = 1;
  return 1;
}
...
void
X509_REQ_set0_signature(X509_REQ* req, ASN1_BIT_STRING* psig)
{
  if (req->signature)
...
int
X509_REQ_set1_signature_algo(X509_REQ* req, X509_ALGOR* palg)
{
