...
STACK_OF(X509_EXTENSION)* X509_REQ_get_extensions(
  ...
  if (req == NULL || ext_nids == NULL) return NULL;
  for (pnid = ext_nids; *pnid != NID_undef; pnid++) {
    exts = get_extensions_by_nid(req, *pnid);
    if (exts == NULL) return NULL;
    if (sk_X509_EXTENSION_num(exts) > 0) return exts;
...
int X509_REQ_add_extensions_nid(X509_REQ* req,
  ...
  if (loc != -1) {
    if ((mod_exts = get_extensions_by_nid(req, nid)) == NULL) return 0;
    if (X509v3_add_extensions(&mod_exts, exts) == NULL) goto end;
  }
...
X509_ATTRIBUTE* X509_REQ_delete_attr(X509_REQ* req, int loc) {
  ...
  if (attr != NULL) req->req_info.enc.modified = 1;
  return attr;
}
...
int X509_REQ_add1_attr(X509_REQ* req, X509_ATTRIBUTE* attr) {
  ...
  if (!X509at_add1_attr(&req->req_info.attributes, attr)) return 0;
...
void X509_REQ_set0_signature(X509_REQ* req, ASN1_BIT_STRING* psig) {
  if (req->signature) ASN1_BIT_STRING_free(req->signature);
  req->signature = psig;
}
