  ...
  switch (cmd) {
    case X509_L_ADD_STORE:
      if (argp != NULL) {
        ...
        store->libctx = libctx;
        if (propq != NULL) store->propq = OPENSSL_strdup(propq);
        /*
         * We open this to check for errors now - so we can report those
         * errors early.
         */
        sctx = OSSL_STORE_open_ex(argp, libctx, propq, NULL, NULL, NULL, NULL,
                                  NULL);
        ...
        if (stores == NULL) {
          ...
          if (stores != NULL) X509_LOOKUP_set_method_data(ctx, stores);
        }
  ...
  if (tmp != NULL) {
    ...
    switch (type) {
      case X509_LU_X509:
        ...
        if (ok) X509_free(tmp->data.x509);
        break;
      case X509_LU_CRL:
        ok = X509_OBJECT_set1_X509_CRL(ret, tmp->data.crl);
        if (ok) X509_CRL_free(tmp->data.crl);
        break;
      case X509_LU_NONE:
        break;
    }
