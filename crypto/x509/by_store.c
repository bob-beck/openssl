...
static int
cache_objects(X509_LOOKUP* lctx,
              CACHED_STORE* store,
              ...
              int depth)
{
  ...
  if ((ctx = OSSL_STORE_open_ex(store->uri,
                                store->libctx,
                                store->propq,
                                NULL,
                                NULL,
...
static int
by_store_ctrl_ex(X509_LOOKUP* ctx,
                 int cmd,
                 const char* argp,
                 long argl,
                 char** retp,
         ...
         X509_OBJECT* ret)
{
  ...
  for (i = 0; i < sk_CACHED_STORE_num(stores); i++) {
    ok = cache_objects(
      ctx, sk_CACHED_STORE_value(stores, i), criterion, 1 /* depth */);
...
static int
by_store_subject(X509_LOOKUP* ctx,
                 X509_LOOKUP_TYPE type,
