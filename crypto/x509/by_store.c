...
static int
cache_objects (X509_LOOKUP *lctx, CACHED_STORE *store,
               const OSSL_STORE_SEARCH *criterion, int depth)
{
  ...
  if ((ctx = OSSL_STORE_open_ex (store->uri, store->libctx, store->propq, NULL,
                                 NULL, NULL, NULL, NULL))
      == NULL)
    return 0;
...
static int
by_store_ctrl_ex (X509_LOOKUP *ctx, int cmd, const char *argp, long argl,
      ...
      if (argp != NULL)
        {
           ...
           * errors early.
           */
          sctx = OSSL_STORE_open_ex (argp, libctx, propq, NULL, NULL, NULL,
                                     NULL, NULL);
          if (sctx == NULL || (propq != NULL && store->propq == NULL)
              || store->uri == NULL)
            {
...
static int
by_store_ctrl (X509_LOOKUP *ctx, int cmd, const char *argp, long argl,
               char **retp)
{
...
static int
by_store (X509_LOOKUP *ctx, X509_LOOKUP_TYPE type,
...
static int
by_store_subject (X509_LOOKUP *ctx, X509_LOOKUP_TYPE type,
