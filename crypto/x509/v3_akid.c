 ...
 * combined: 'none': do not add any authority key identifier. 'keyid': use the
 * issuer's subject keyid; the option 'always' means its is an error if the
 * issuer certificate doesn't have a subject key id. 'issuer': use the issuer's
 * cert issuer and serial number. The default is to only use this if 'keyid' is
 * not present. With the option 'always' this is always included.
 */
