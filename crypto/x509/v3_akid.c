...
static STACK_OF(CONF_VALUE)* i2v_AUTHORITY_KEYID(
    X509V3_EXT_METHOD* method, AUTHORITY_KEYID* akeyid,
    STACK_OF(CONF_VALUE)* extlist) {
  char* tmp = NULL;
  ...
  return extlist;
err:
  if (origextlist == NULL) sk_CONF_VALUE_pop_free(extlist, X509V3_conf_free);
  return NULL;
}
...
static AUTHORITY_KEYID* v2i_AUTHORITY_KEYID(X509V3_EXT_METHOD* method,
  ...
  for (i = 0; i < n; i++) {
    ...
    if (strcmp(cnf->name, "keyid") == 0 && keyid == 0) {
      keyid = 1;
      if (cnf->value != NULL) keyid = 2;
    } else if (strcmp(cnf->name, "issuer") == 0 && issuer == 0) {
      issuer = 1;
      if (cnf->value != NULL) issuer = 2;
  ...
  if (ctx != NULL && (ctx->flags & X509V3_CTX_TEST) != 0) return akeyid;
