...
int GENERAL_NAME_set1_X509_NAME(GENERAL_NAME** tgt, const X509_NAME* src) {
  ...
  if (src == NULL) { /* NULL-DN */
    if ((name->d.directoryName = X509_NAME_new()) == NULL) goto err;
...
static int edipartyname_cmp(const EDIPARTYNAME* a, const EDIPARTYNAME* b) {
  ...
  if (a->nameAssigner == NULL && b->nameAssigner != NULL) return -1;
  if (a->nameAssigner != NULL && b->nameAssigner == NULL) return 1;
   ...
   */
  if (a->partyName == NULL || b->partyName == NULL) return -1;
...
int OTHERNAME_cmp(OTHERNAME* a, OTHERNAME* b) {
  ...
  if ((result = OBJ_cmp(a->type_id, b->type_id)) != 0) return result;
  /* Check the value. */
...
int GENERAL_NAME_get0_otherName(const GENERAL_NAME* gen, ASN1_OBJECT** poid,
  ...
  if (poid) *poid = gen->d.otherName->type_id;
  if (pvalue) *pvalue = gen->d.otherName->value;
  return 1;
}
