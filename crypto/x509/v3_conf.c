...
static X509_EXTENSION*
do_ext_i2d(const X509V3_EXT_METHOD* method,
           int ext_nid,
           int crit,
           void* ext_struc);
...
static X509_EXTENSION*
X509V3_EXT_nconf_int(CONF* conf,
                     X509V3_CTX* ctx,
                     const char* section,
                     const char* name,
                     const char* value)
...
X509_EXTENSION*
X509V3_EXT_nconf_nid(CONF* conf,
                     X509V3_CTX* ctx,
                     int ext_nid,
                     const char* value)
{
  ...
  if ((ext_type = v3_check_generic(&value)))
    return v3_generic_extension(
      OBJ_nid2sn(ext_nid), value, crit, ext_type, ctx);
                     ...
                     X509V3_CTX* ctx)
{
  ...
  if ((obj = OBJ_txt2obj(ext, 0)) == NULL) {
    ERR_raise_data(
      ERR_LIB_X509V3, X509V3_R_EXTENSION_NAME_ERROR, "name=%s", ext);
    goto err;
  }
  ...
  if (ext_der == NULL) {
    ERR_raise_data(
      ERR_LIB_X509V3, X509V3_R_EXTENSION_VALUE_ERROR, "value=%s", value);
    goto err;
  }
...
static void
delete_ext(STACK_OF(X509_EXTENSION)* sk, X509_EXTENSION* dext)
{
  int idx;
  ASN1_OBJECT* obj;
 ...
 */
int
X509V3_EXT_add_nconf_sk(CONF* conf,
                        X509V3_CTX* ctx,
                        const char* section,
                        STACK_OF(X509_EXTENSION)** sk)
{
  ...
  for (i = 0; i < sk_CONF_VALUE_num(nval); i++) {
    ...
    if ((ext = X509V3_EXT_nconf_int(
           conf, ctx, val->section, val->name, val->value)) == NULL)
      return 0;
    if (sk != NULL) {
 ...
 */
int
X509V3_EXT_add_nconf(CONF* conf,
                     X509V3_CTX* ctx,
                     const char* section,
                     X509* cert)
{
 ...
 */
int
X509V3_EXT_CRL_add_nconf(CONF* conf,
                         X509V3_CTX* ctx,
                         const char* section,
                         X509_CRL* crl)
{
 ...
 */
int
X509V3_EXT_REQ_add_nconf(CONF* conf,
                         X509V3_CTX* ctx,
                         const char* section,
                         X509_REQ* req)
{
...
static X509V3_CONF_METHOD nconf_method = { nconf_get_string,
                                           nconf_get_section,
                                           NULL,
                                           NULL };
...
int
X509V3_set_issuer_pkey(X509V3_CTX* ctx, EVP_PKEY* pkey)
{
  if (ctx == NULL) {
    ERR_raise(ERR_LIB_X509V3, ERR_R_PASSED_NULL_PARAMETER);
    return 0;
...
X509_EXTENSION*
X509V3_EXT_conf(LHASH_OF(CONF_VALUE)* conf,
                X509V3_CTX* ctx,
                const char* name,
                const char* value)
{
...
X509_EXTENSION*
X509V3_EXT_conf_nid(LHASH_OF(CONF_VALUE)* conf,
                    X509V3_CTX* ctx,
                    int ext_nid,
                    const char* value)
{
...
static X509V3_CONF_METHOD conf_lhash_method = { conf_lhash_get_string,
...
int
X509V3_EXT_add_conf(LHASH_OF(CONF_VALUE)* conf,
                    X509V3_CTX* ctx,
                    const char* section,
                    X509* cert)
{
...
int
X509V3_EXT_CRL_add_conf(LHASH_OF(CONF_VALUE)* conf,
                        X509V3_CTX* ctx,
                        const char* section,
                        X509_CRL* crl)
{
...
int
X509V3_EXT_REQ_add_conf(LHASH_OF(CONF_VALUE)* conf,
                        X509V3_CTX* ctx,
                        const char* section,
                        X509_REQ* req)
{
