...
static X509_EXTENSION* do_ext_nconf(CONF* conf, X509V3_CTX* ctx, int ext_nid,
  ...
  if (method->v2i) {
    ...
    if (nval == NULL || sk_CONF_VALUE_num(nval) <= 0) {
      ...
      if (*value != '@') sk_CONF_VALUE_pop_free(nval, X509V3_conf_free);
      return NULL;
    }
...
static X509_EXTENSION* do_ext_i2d(const X509V3_EXT_METHOD* method, int ext_nid,
  ...
  if (method->it) {
    ...
    if ((ext_der = OPENSSL_malloc(ext_len)) == NULL) goto err;
    p = ext_der;
...
static int v3_check_critical(const char** value) {
  ...
  if (!CHECK_AND_SKIP_PREFIX(p, "critical,")) return 0;
...
int X509V3_EXT_add_nconf_sk(CONF* conf, X509V3_CTX* ctx, const char* section,
  ...
  if ((nval = NCONF_get_section(conf, section)) == NULL) return 0;
...
int X509V3_EXT_add_nconf(CONF* conf, X509V3_CTX* ctx, const char* section,
  ...
  if (cert != NULL) sk = &cert->cert_info.extensions;
...
int X509V3_EXT_CRL_add_nconf(CONF* conf, X509V3_CTX* ctx, const char* section,
                             X509_CRL* crl) {
  STACK_OF(X509_EXTENSION)** sk = NULL;
  if (crl != NULL) sk = &crl->crl.extensions;
  return X509V3_EXT_add_nconf_sk(conf, ctx, section, sk);
}
...
void X509V3_string_free(X509V3_CTX* ctx, char* str) {
  if (!str) return;
  if (ctx->db_meth->free_string) ctx->db_meth->free_string(ctx->db, str);
}

void X509V3_section_free(X509V3_CTX* ctx, STACK_OF(CONF_VALUE)* section) {
  if (!section) return;
  if (ctx->db_meth->free_section) ctx->db_meth->free_section(ctx->db, section);
}
...
X509_EXTENSION* X509V3_EXT_conf(LHASH_OF(CONF_VALUE)* conf, X509V3_CTX* ctx,
  ...
  if ((ctmp = NCONF_new(NULL)) == NULL) return NULL;
