...
static int policy_cache_create(X509* x, CERTIFICATEPOLICIES* policies,
  ...
  if ((num = sk_POLICYINFO_num(policies)) <= 0) goto bad_policy;
...
static int policy_cache_new(X509* x) {
  ...
  if (x->policy_cache != NULL) return 1;
...
const X509_POLICY_CACHE* ossl_policy_cache_set(X509* x) {
  if (x->policy_cache == NULL) {
    if (!CRYPTO_THREAD_write_lock(x->lock)) return NULL;
    policy_cache_new(x);
...
static int policy_cache_set_int(long* out, ASN1_INTEGER* value) {
  ...
  if (value->type == V_ASN1_NEG_INTEGER) return 0;
