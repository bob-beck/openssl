...
static int x509_cb(int operation, ASN1_VALUE** pval, const ASN1_ITEM* it,
  ...
  switch (operation) {
    ...
    case ASN1_OP_DUP_POST: {
      ...
      if (!ossl_x509_set0_libctx(ret, old->libctx, old->propq)) return 0;
    } break;
...
X509* d2i_X509_AUX(X509** a, const unsigned char** pp, long length) {
  ...
  if (a == NULL || *a == NULL) freeret = 1;
  ret = d2i_X509(a, &q, length);
  /* If certificate unreadable then forget it */
  if (ret == NULL) return NULL;
  /* update length */
  length -= (long)(q - *pp);
  if (length > 0 && !d2i_X509_CERT_AUX(&ret->aux, &q, length)) goto err;
  *pp = q;
  return ret;
err:
  if (freeret) {
...
static int i2d_x509_aux_internal(const X509* a, unsigned char** pp) {
  ...
  if (length <= 0 || a == NULL) return length;

  tmplen = i2d_X509_CERT_AUX(a->aux, pp);
  if (tmplen < 0) {
    if (start != NULL) *pp = start;
    return tmplen;
  }
  length += tmplen;
...
int i2d_X509_AUX(const X509* a, unsigned char** pp) {
  ...
  if (pp == NULL || *pp != NULL) return i2d_x509_aux_internal(a, pp);

  /* Obtain the combined length */
  if ((length = i2d_x509_aux_internal(a, NULL)) <= 0) return length;
