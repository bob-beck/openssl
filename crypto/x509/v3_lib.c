...
void* X509V3_EXT_d2i(X509_EXTENSION* ext) {
  ...
  if ((method = X509V3_EXT_get(ext)) == NULL) return NULL;
...
int X509V3_add1_i2d(STACK_OF(X509_EXTENSION)** x, int nid, void* value,
   ...
   * extension.
   */
  if (ext_op != X509V3_ADD_APPEND) extidx = X509v3_get_ext_by_NID(*x, nid, -1);

  /* See if extension exists */
  if (extidx >= 0) {
    /* If keep existing, nothing to do */
    if (ext_op == X509V3_ADD_KEEP_EXISTING) return 1;
  ...
  ret = *x;
  if (*x == NULL && (ret = sk_X509_EXTENSION_new_null()) == NULL) goto m_fail;
  if (!sk_X509_EXTENSION_push(ret, ext)) goto m_fail;
...
m_fail:
  /* ERR_raise(ERR_LIB_X509V3, ERR_R_CRYPTO_LIB); */
  if (ret != *x) sk_X509_EXTENSION_free(ret);
  X509_EXTENSION_free(ext);
  return -1;

err:
  if (!(flags & X509V3_ADD_SILENT)) ERR_raise(ERR_LIB_X509V3, errcode);
  return 0;
}
