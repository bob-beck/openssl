...
static int addr_expand(unsigned char* addr, const ASN1_BIT_STRING* bs,
  ...
  if (bs->length < 0 || bs->length > length) return 0;
  if (bs->length > 0) {
...
static int i2r_address(BIO* out, const unsigned afi, const unsigned char fill,
  ...
  switch (afi) {
    case IANA_AFI_IPV4:
      ...
      break;
    case IANA_AFI_IPV6:
      if (!addr_expand(addr, bs, 16, fill)) return 0;
      for (n = 16; n > 1 && addr[n - 1] == 0x00 && addr[n - 2] == 0x00; n -= 2);
      for (i = 0; i < n; i += 2)
        BIO_printf(out, "%x%s", (addr[i] << 8) | addr[i + 1],
                   (i < 14 ? ":" : ""));
      if (i < 16) BIO_puts(out, ":");
      if (i == 0) BIO_puts(out, ":");
      break;
    default:
  ...
  for (i = 0; i < sk_IPAddressOrRange_num(aors); i++) {
    ...
    switch (aor->type) {
      case IPAddressOrRange_addressPrefix:
        if (!i2r_address(out, afi, 0x00, aor->u.addressPrefix)) return 0;
        BIO_printf(out, "/%d\n", addr_prefixlen(aor->u.addressPrefix));
        continue;
      case IPAddressOrRange_addressRange:
        if (!i2r_address(out, afi, 0x00, aor->u.addressRange->min)) return 0;
        BIO_puts(out, "-");
        if (!i2r_address(out, afi, 0xFF, aor->u.addressRange->max)) return 0;
        BIO_puts(out, "\n");
        continue;
    }
  }
...
static int IPAddressOrRange_cmp(const IPAddressOrRange* a,
  ...
  switch (a->type) {
    case IPAddressOrRange_addressPrefix:
      if (!addr_expand(addr_a, a->u.addressPrefix, length, 0x00)) return -1;
      prefixlen_a = addr_prefixlen(a->u.addressPrefix);
      break;
    case IPAddressOrRange_addressRange:
      if (!addr_expand(addr_a, a->u.addressRange->min, length, 0x00)) return -1;
  ...
  switch (b->type) {
    case IPAddressOrRange_addressPrefix:
      if (!addr_expand(addr_b, b->u.addressPrefix, length, 0x00)) return -1;
      prefixlen_b = addr_prefixlen(b->u.addressPrefix);
      break;
    case IPAddressOrRange_addressRange:
      if (!addr_expand(addr_b, b->u.addressRange->min, length, 0x00)) return -1;
...
static int make_addressPrefix(IPAddressOrRange** result, unsigned char* addr,
  ...
  if (prefixlen < 0 || prefixlen > (afilen * 8)) return 0;
  if ((aor = IPAddressOrRange_new()) == NULL) return 0;
  aor->type = IPAddressOrRange_addressPrefix;
  if (aor->u.addressPrefix == NULL &&
      (aor->u.addressPrefix = ASN1_BIT_STRING_new()) == NULL)
    goto err;
  if (!ASN1_BIT_STRING_set(aor->u.addressPrefix, addr, bytelen)) goto err;
  if (bitlen > 0) aor->u.addressPrefix->data[bytelen - 1] &= ~(0xFF >> bitlen);
...
static int make_addressRange(IPAddressOrRange** result, unsigned char* min,
  ...
  if (memcmp(min, max, length) > 0) return 0;
  ...
  if ((aor->u.addressRange = IPAddressRange_new()) == NULL) goto err;
  ...
  if (!ASN1_BIT_STRING_set(aor->u.addressRange->min, min, i)) goto err;
  ...
  if (!ASN1_BIT_STRING_set(aor->u.addressRange->max, max, i)) goto err;
...
static IPAddressFamily* make_IPAddressFamily(IPAddrBlocks* addr,
  ...
  if ((f = IPAddressFamily_new()) == NULL) goto err;
  ...
  if (f->addressFamily == NULL &&
    ...
    goto err;
  if (!ASN1_OCTET_STRING_set(f->addressFamily, key, keylen)) goto err;
  if (!sk_IPAddressFamily_push(addr, f)) goto err;
...
static IPAddressOrRanges* make_prefix_or_range(IPAddrBlocks* addr,
  ...
  if (aors != NULL) return aors;
  if ((aors = sk_IPAddressOrRange_new_null()) == NULL) return NULL;
  switch (afi) {
    case IANA_AFI_IPV4:
...
int X509v3_addr_add_prefix(IPAddrBlocks* addr, const unsigned afi,
  ...
  if (aors == NULL ||
    ...
    return 0;
  if (sk_IPAddressOrRange_push(aors, aor)) return 1;
  IPAddressOrRange_free(aor);
  return 0;
}
...
static int extract_min_max(IPAddressOrRange* aor, unsigned char* min,
  ...
  if (aor == NULL || min == NULL || max == NULL) return 0;
  switch (aor->type) {
    case IPAddressOrRange_addressPrefix:
...
int X509v3_addr_is_canonical(IPAddrBlocks* addr) {
  ...
  for (i = 0; i < sk_IPAddressFamily_num(addr); i++) {
     ...
     */
    if (f == NULL || f->ipAddressChoice == NULL) return 0;
    switch (f->ipAddressChoice->type) {
      case IPAddressChoice_inherit:
        continue;
      case IPAddressChoice_addressesOrRanges:
...
static int IPAddressOrRanges_canonize(IPAddressOrRanges* aors,
  ...
  for (i = 0; i < sk_IPAddressOrRange_num(aors) - 1; i++) {
    ...
    if (memcmp(a_max, b_min, length) == 0) {
      ...
      if (!make_addressRange(&merged, a_min, b_max, length)) return 0;
...
int X509v3_addr_canonize(IPAddrBlocks* addr) {
  ...
  if (!ossl_assert(X509v3_addr_is_canonical(addr))) return 0;
  return 1;
}
...
static void* v2i_IPAddrBlocks(const struct v3_ext_method* method,
   ...
   */
  if (!X509v3_addr_canonize(addr)) goto err;
  return addr;
...
int X509v3_addr_inherits(IPAddrBlocks* addr) {
  ...
  for (i = 0; i < sk_IPAddressFamily_num(addr); i++) {
    ...
    if (f->ipAddressChoice->type == IPAddressChoice_inherit) return 1;
  }
  return 0;
}
...
static int addr_contains(IPAddressOrRanges* parent, IPAddressOrRanges* child,
  ...
  if (child == NULL || parent == child) return 1;
  ...
  for (c = 0; c < sk_IPAddressOrRange_num(child); c++) {
    if (!extract_min_max(sk_IPAddressOrRange_value(child, c), c_min, c_max,
      ...
      return 0;
    for (;; p++) {
      if (p >= sk_IPAddressOrRange_num(parent)) return 0;
      if (!extract_min_max(sk_IPAddressOrRange_value(parent, p), p_min, p_max,
                           length))
        return 0;
      if (memcmp(p_max, c_max, length) < 0) continue;
      if (memcmp(p_min, c_min, length) > 0) return 0;
      break;
    }
  }
...
int X509v3_addr_subset(IPAddrBlocks* a, IPAddrBlocks* b) {
  ...
  if (b == NULL || X509v3_addr_inherits(a) || X509v3_addr_inherits(b)) return 0;
...
static int addr_validate_path_internal(X509_STORE_CTX* ctx,
  ...
  if (!ossl_assert(chain != NULL && sk_X509_num(chain) > 0) ||
    ...
    if (ctx != NULL) ctx->error = X509_V_ERR_UNSPECIFIED;
    return 0;
  }
  ...
  if (ext != NULL) {
    ...
    if ((ext = x->rfc3779_addr) == NULL) return 1; /* Return success */
  }
  ...
  if ((child = sk_IPAddressFamily_dup(ext)) == NULL) {
    ERR_raise(ERR_LIB_X509V3, ERR_R_CRYPTO_LIB);
    if (ctx != NULL) ctx->error = X509_V_ERR_OUT_OF_MEM;
    goto done;
  }
  ...
  for (i++; i < sk_X509_num(chain); i++) {
    ...
    if (x->rfc3779_addr == NULL) {
      for (j = 0; j < sk_IPAddressFamily_num(child); j++) {
        ...
        if (!IPAddressFamily_check_len(fc)) goto done;
  ...
  if (x->rfc3779_addr != NULL) {
    for (j = 0; j < sk_IPAddressFamily_num(x->rfc3779_addr); j++) {
      ...
      if (!IPAddressFamily_check_len(fp)) goto done;
...
int X509v3_addr_validate_resource_set(STACK_OF(X509)* chain, IPAddrBlocks* ext,
  ...
  if (chain == NULL || sk_X509_num(chain) == 0) return 0;
