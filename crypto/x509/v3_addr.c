            ...
            const ASN1_BIT_STRING* bs)
{
  ...
  switch (afi) {
    ...
    case IANA_AFI_IPV6:
      ...
      for (i = 0; i < n; i += 2)
        BIO_printf(
          out, "%x%s", (addr[i] << 8) | addr[i + 1], (i < 14 ? ":" : ""));
      if (i < 16)
        BIO_puts(out, ":");
      if (i == 0)
        BIO_puts(out, ":");
 ...
 */
static int
i2r_IPAddrBlocks(const X509V3_EXT_METHOD* method,
                 void* ext,
                 BIO* out,
                 int indent)
{
  ...
  for (i = 0; i < sk_IPAddressFamily_num(addr); i++) {
    ...
    if (f->addressFamily->length > 2) {
      switch (f->addressFamily->data[2]) {
        ...
        default:
          BIO_printf(
            out, " (Unknown SAFI %u)", (unsigned)f->addressFamily->data[2]);
          break;
      }
    }
 ...
 */
static int
IPAddressOrRange_cmp(const IPAddressOrRange* a,
 ...
 */
static int
range_should_be_prefix(const unsigned char* min,
 ...
 */
static int
make_addressPrefix(IPAddressOrRange** result,
                   unsigned char* addr,
                   const int prefixlen,
                   const int afilen)
{
 ...
 */
static int
make_addressRange(IPAddressOrRange** result,
                  unsigned char* min,
                  unsigned char* max,
                  const int length)
{
...
make_IPAddressFamily(IPAddrBlocks* addr,
                     const unsigned afi,
                     const unsigned* safi)
{
  IPAddressFamily* f;
 ...
 */
int
X509v3_addr_add_inherit(IPAddrBlocks* addr,
                        const unsigned afi,
                        const unsigned* safi)
{
 ...
 */
int
X509v3_addr_add_prefix(IPAddrBlocks* addr,
                       const unsigned afi,
                       const unsigned* safi,
                       unsigned char* a,
                       const int prefixlen)
 ...
 */
int
X509v3_addr_add_range(IPAddrBlocks* addr,
                      const unsigned afi,
                      const unsigned* safi,
                      unsigned char* min,
                      unsigned char* max)
 ...
 */
static int
extract_min_max(IPAddressOrRange* aor,
                unsigned char* min,
                unsigned char* max,
                int length)
{
 ...
 */
int
X509v3_addr_get_range(IPAddressOrRange* aor,
                      const unsigned afi,
                      unsigned char* min,
                      unsigned char* max,
                      const int length)
...
static int
IPAddressFamily_check_len(const IPAddressFamily* f)
{
 ...
 */
static void*
v2i_IPAddrBlocks(const struct v3_ext_method* method,
                 ...
                 STACK_OF(CONF_VALUE)* values)
{
  ...
  for (i = 0; i < sk_CONF_VALUE_num(values); i++) {
    ...
    if (!ossl_v3_name_cmp(val->name, "IPv4")) {
      ...
      ERR_raise_data(
        ERR_LIB_X509V3, X509V3_R_EXTENSION_NAME_ERROR, "%s", val->name);
      goto err;
    }
...
addr_contains(IPAddressOrRanges* parent, IPAddressOrRanges* child, int length)
{
  ...
  for (c = 0; c < sk_IPAddressOrRange_num(child); c++) {
    if (!extract_min_max(
          sk_IPAddressOrRange_value(child, c), c_min, c_max, length))
      return 0;
    for (;; p++) {
      if (p >= sk_IPAddressOrRange_num(parent))
        return 0;
      if (!extract_min_max(
            sk_IPAddressOrRange_value(parent, p), p_min, p_max, length))
        return 0;
 ...
 */
static int
addr_validate_path_internal(X509_STORE_CTX* ctx,
                            STACK_OF(X509)* chain,
                            IPAddrBlocks* ext)
{
 ...
 */
int
X509v3_addr_validate_resource_set(STACK_OF(X509)* chain,
                                  IPAddrBlocks* ext,
