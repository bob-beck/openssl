...
static void get_windows_default_path(char* pathname, const char* suffix) {
  ...
  if (ossldir == NULL) return;

  OPENSSL_strlcpy(pathname, ossldir, MAX_PATH - 1);
  if (MAX_PATH - strlen(pathname) > strlen(suffix)) strcat(pathname, suffix);
}
...
DEFINE_RUN_ONCE_STATIC(do_openssldir_setup) {
  ...
  if (strlen(x509_private_dir) > 0) x509_private_dirptr = x509_private_dir;

  get_windows_default_path(x509_cert_area, "\\");
  if (strlen(x509_cert_area) > 0) x509_cert_areaptr = x509_cert_area;

  get_windows_default_path(x509_cert_dir, "\\certs");
  if (strlen(x509_cert_dir) > 0) x509_cert_dirptr = x509_cert_dir;

  get_windows_default_path(x509_cert_file, "\\cert.pem");
  if (strlen(x509_cert_file) > 0) x509_cert_fileptr = x509_cert_file;
