...
int
X509_STORE_set_default_paths_ex(X509_STORE* ctx,
                                OSSL_LIB_CTX* libctx,
                                const char* propq)
{
  X509_LOOKUP* lookup;
...
int
X509_STORE_load_file_ex(X509_STORE* ctx,
                        const char* file,
                        OSSL_LIB_CTX* libctx,
                        const char* propq)
{
  ...
  if (file == NULL ||
      ...
      X509_LOOKUP_load_file_ex(
        lookup, file, X509_FILETYPE_PEM, libctx, propq) <= 0)
    return 0;
...
int
X509_STORE_load_store_ex(X509_STORE* ctx,
                         const char* uri,
                         OSSL_LIB_CTX* libctx,
                         const char* propq)
{
...
int
X509_STORE_load_locations_ex(X509_STORE* ctx,
                             const char* file,
                             const char* path,
                             OSSL_LIB_CTX* libctx,
                             const char* propq)
