...
static int x509_pubkey_ex_new_ex(ASN1_VALUE** pval, const ASN1_ITEM* it,
  ...
  if ((ret = OPENSSL_zalloc(sizeof(*ret))) == NULL) return 0;
...
int X509_PUBKEY_set(X509_PUBKEY** x, EVP_PKEY* pkey) {
  ...
  *x = pk;
   ...
   * |pkey|.
   */
  if (pk->pkey != NULL) EVP_PKEY_free(pk->pkey);
...
static int x509_pubkey_decode(EVP_PKEY** ppkey, const X509_PUBKEY* key) {
  ...
  if (pkey->ameth->pub_decode) {
     ...
     */
    if (!pkey->ameth->pub_decode(pkey, key)) goto error;
  } else {
    ERR_raise(ERR_LIB_X509, X509_R_METHOD_NOT_SUPPORTED);
    goto error;
  }
...
static EVP_PKEY* d2i_PUBKEY_int(
  ...
  if (libctx != NULL || propq != NULL || force_legacy) {
    ...
    if (xpk2 == NULL) return NULL;
    if (!x509_pubkey_set0_libctx(xpk2, libctx, propq)) goto end;
    xpk2->flag_force_legacy = !!force_legacy;
    pxpk = &xpk2;
  }
  xpk = d2i_x509_pubkey(pxpk, &q, length);
  if (xpk == NULL) goto end;
  pktmp = X509_PUBKEY_get(xpk);
  X509_PUBKEY_free(xpk);
  xpk2 = NULL; /* We know that xpk == xpk2 */
  if (pktmp == NULL) goto end;
  *pp = q;
  if (a != NULL) {
    EVP_PKEY_free(*a);
    *a = pktmp;
...
int i2d_PUBKEY(const EVP_PKEY* a, unsigned char** pp) {
  ...
  if (a->ameth != NULL) {
    ...
    if ((xpk = X509_PUBKEY_new()) == NULL) return -1;
...
DH* ossl_d2i_DH_PUBKEY(DH** a, const unsigned char** pp, long length) {
  ...
  pkey = ossl_d2i_PUBKEY_legacy(NULL, &q, length);
  if (pkey == NULL) return NULL;
  if (EVP_PKEY_get_id(pkey) == EVP_PKEY_DH) key = EVP_PKEY_get1_DH(pkey);
  EVP_PKEY_free(pkey);
  if (key == NULL) return NULL;
  *pp = q;
  if (a != NULL) {
...
DH* ossl_d2i_DHx_PUBKEY(DH** a, const unsigned char** pp, long length) {
  ...
  pkey = ossl_d2i_PUBKEY_legacy(NULL, &q, length);
  if (pkey == NULL) return NULL;
  if (EVP_PKEY_get_id(pkey) == EVP_PKEY_DHX) key = EVP_PKEY_get1_DH(pkey);
  EVP_PKEY_free(pkey);
  if (key == NULL) return NULL;
  *pp = q;
  if (a != NULL) {
...
int X509_PUBKEY_set0_param(X509_PUBKEY* pub, ASN1_OBJECT* aobj, int ptype,
  ...
  if (!X509_ALGOR_set0(pub->algor, aobj, ptype, pval)) return 0;
  if (penc != NULL) X509_PUBKEY_set0_public_key(pub, penc, penclen);
  return 1;
}
...
int X509_PUBKEY_get0_param(ASN1_OBJECT** ppkalg, const unsigned char** pk,
  ...
  if (ppkalg) *ppkalg = pub->algor->algorithm;
  if (pk) {
...
int X509_PUBKEY_eq(const X509_PUBKEY* a, const X509_PUBKEY* b) {
  ...
  if (!X509_PUBKEY_get0_param(NULL, NULL, NULL, &algA, a) || algA == NULL ||
    ...
    return -2;
  if (X509_ALGOR_cmp(algA, algB) != 0) return 0;
...
int ossl_x509_PUBKEY_get0_libctx(OSSL_LIB_CTX** plibctx, const char** ppropq,
  ...
  if (plibctx) *plibctx = key->libctx;
  if (ppropq) *ppropq = key->propq;
  return 1;
}
