 ...
 */
int
X509_check_purpose (X509 *x, int id, int non_leaf)
{
  int idx;
...
int
X509_PURPOSE_get_unused_id (ossl_unused OSSL_LIB_CTX *libctx)
{
...
static int
setup_dp (const X509 *x, DIST_POINT *dp)
{
 ...
 */
static int
check_sig_alg_match (const EVP_PKEY *issuer_key, const X509 *subject)
{
  int subj_sig_nid;
 ...
 * 0 not a CA
 * 1 is a CA
 * 2 Only possible in older versions of openSSL when basicConstraints are
 * absent new versions will not return this value. May be a CA 3
 * basicConstraints absent but self-signed V1. 4 basicConstraints absent but
 * keyUsage present and keyCertSign asserted. 5 Netscape specific CA Flags
 * present
 */
...
void
X509_set_proxy_pathlen (X509 *x, long l)
{
  x->ex_pcpathlen = l;
}
...
static int
check_purpose_ssl_client (const X509_PURPOSE *xp, const X509 *x, int non_leaf)
{
...
static int
check_purpose_ssl_server (const X509_PURPOSE *xp, const X509 *x, int non_leaf)
{
...
static int
check_purpose_ns_ssl_server (const X509_PURPOSE *xp, const X509 *x,
                             int non_leaf)
{
...
static int
purpose_smime (const X509 *x, int non_leaf)
{
...
static int
check_purpose_smime_sign (const X509_PURPOSE *xp, const X509 *x, int non_leaf)
{
...
static int
check_purpose_smime_encrypt (const X509_PURPOSE *xp, const X509 *x,
                             int non_leaf)
{
  int ret = purpose_smime (x, non_leaf);
...
static int
check_purpose_crl_sign (const X509_PURPOSE *xp, const X509 *x, int non_leaf)
{
  if (non_leaf)
    {
      int ca_ret = check_ca (x);
 ...
 */
static int
check_purpose_ocsp_helper (const X509_PURPOSE *xp, const X509 *x, int non_leaf)
{
  /*
...
static int
check_purpose_timestamp_sign (const X509_PURPOSE *xp, const X509 *x,
                              int non_leaf)
{
  int i_ext;
...
static int
check_purpose_code_sign (const X509_PURPOSE *xp, const X509 *x, int non_leaf)
{
  int i_ext;
...
static int
no_check_purpose (const X509_PURPOSE *xp, const X509 *x, int non_leaf)
{
  return 1;
}
...
int
ossl_x509_likely_issued (X509 *issuer, X509 *subject)
{
  int ret;
 ...
 */
int
ossl_x509_signing_allowed (const X509 *issuer, const X509 *subject)
{
...
int
X509_check_akid (const X509 *issuer, const AUTHORITY_KEYID *akid)
{
  if (akid == NULL)
    return X509_V_OK;
...
X509_get0_subject_key_id (X509 *x)
{
  /* Call for side-effect of computing hash and caching extensions */
  if (X509_check_purpose (x, -1, 0) != 1)
    return NULL;
...
const ASN1_OCTET_STRING *
X509_get0_authority_key_id (X509 *x)
{
  /* Call for side-effect of computing hash and caching extensions */
  if (X509_check_purpose (x, -1, 0) != 1)
    return NULL;
...
const GENERAL_NAMES *
X509_get0_authority_issuer (X509 *x)
{
  /* Call for side-effect of computing hash and caching extensions */
  if (X509_check_purpose (x, -1, 0) != 1)
    return NULL;
...
const ASN1_INTEGER *
X509_get0_authority_serial (X509 *x)
{
  /* Call for side-effect of computing hash and caching extensions */
  if (X509_check_purpose (x, -1, 0) != 1)
    return NULL;
