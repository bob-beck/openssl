...
int X509_check_purpose(X509* x, int id, int non_leaf) {
  ...
  if (!ossl_x509v3_cache_extensions(x)) return -1;
  if (id == -1) return 1;
...
int X509_PURPOSE_get_by_sname(const char* sname) {
  ...
  for (i = 0; i < X509_PURPOSE_get_count(); i++) {
    ...
    if (strcmp(xptmp->sname, sname) == 0) return i;
  }
  return -1;
}
...
int X509_PURPOSE_add(int id, int trust, int flags,
  ...
  if (ptmp->name == NULL || ptmp->sname == NULL) goto err;
...
static int setup_dp(const X509* x, DIST_POINT* dp) {
  ...
  if (dp->reasons != NULL) {
    if (dp->reasons->length > 0) dp->dp_reasons = dp->reasons->data[0];
    if (dp->reasons->length > 1) dp->dp_reasons |= (dp->reasons->data[1] << 8);
    dp->dp_reasons &= CRLDP_ALL_REASONS;
  } else {
    dp->dp_reasons = CRLDP_ALL_REASONS;
  }
  if (dp->distpoint == NULL || dp->distpoint->type != 1) return 1;
  ...
  if (iname == NULL) iname = X509_get_issuer_name(x);
...
static int check_sig_alg_match(const EVP_PKEY* issuer_key,
  ...
  if (issuer_key == NULL) return X509_V_ERR_NO_ISSUER_PUBLIC_KEY;
...
int ossl_x509v3_cache_extensions(X509* x) {
  ...
  if (!CRYPTO_THREAD_write_lock(x->lock)) return 0;
  ...
  if (X509_get_version(x) == X509_VERSION_1) x->ex_flags |= EXFLAG_V1;
  ...
  if ((usage = X509_get_ext_d2i(x, NID_key_usage, &i, NULL)) != NULL) {
    ...
    if (usage->length > 0) {
      ...
      if (usage->length > 1) x->ex_kusage |= usage->data[1] << 8;
    }
  ...
  if (x->skid == NULL && i != -1) x->ex_flags |= EXFLAG_INVALID;

  x->akid = X509_get_ext_d2i(x, NID_authority_key_identifier, &i, NULL);
  if (x->akid == NULL && i != -1) x->ex_flags |= EXFLAG_INVALID;
  ...
  if (x->altname == NULL && i != -1) x->ex_flags |= EXFLAG_INVALID;
  x->nc = X509_get_ext_d2i(x, NID_name_constraints, &i, NULL);
  if (x->nc == NULL && i != -1) x->ex_flags |= EXFLAG_INVALID;

  /* Handle CRL distribution point entries */
  res = setup_crldp(x);
  if (res == 0) x->ex_flags |= EXFLAG_INVALID;

#ifndef OPENSSL_NO_RFC3779
  x->rfc3779_addr = X509_get_ext_d2i(x, NID_sbgp_ipAddrBlock, &i, NULL);
  if (x->rfc3779_addr == NULL && i != -1) x->ex_flags |= EXFLAG_INVALID;
  x->rfc3779_asid = X509_get_ext_d2i(x, NID_sbgp_autonomousSysNum, &i, NULL);
  if (x->rfc3779_asid == NULL && i != -1) x->ex_flags |= EXFLAG_INVALID;
#endif
  for (i = 0; i < X509_get_ext_count(x); i++) {
    X509_EXTENSION* ex = X509_get_ext(x, i);
    int nid = OBJ_obj2nid(X509_EXTENSION_get_object(ex));

    if (nid == NID_freshest_crl) x->ex_flags |= EXFLAG_FRESHEST;
    if (!X509_EXTENSION_get_critical(ex)) continue;
...
int X509_check_ca(X509* x) {
  ...
  if (!ossl_x509v3_cache_extensions(x)) return 0;
...
static int check_purpose_ssl_client(const X509_PURPOSE* xp, const X509* x,
  ...
  if (non_leaf) return check_ssl_ca(x);
...
static int purpose_smime(const X509* x, int non_leaf) {
  ...
  if ((x->ex_flags & EXFLAG_NSCERT) != 0) {
    if ((x->ex_nscert & NS_SMIME) != 0) return 1;
...
static int check_purpose_code_sign(const X509_PURPOSE* xp, const X509* x,
   ...
   */
  /* Key Usage */
  if ((x->ex_flags & EXFLAG_KUSAGE) == 0) return 0;
  ...
  if (i_ext >= 0) {
    ...
    if (!X509_EXTENSION_get_critical(ext)) return 0;
  }

  /* Extended Key Usage */
  if ((x->ex_flags & EXFLAG_XKUSAGE) == 0) return 0;
...
int X509_check_issued(X509* issuer, X509* subject) {
  ...
  if ((ret = ossl_x509_likely_issued(issuer, subject)) != X509_V_OK) return ret;
...
uint32_t X509_get_key_usage(X509* x) {
  /* Call for side-effect of computing hash and caching extensions */
  if (X509_check_purpose(x, -1, 0) != 1) return 0;
...
const ASN1_OCTET_STRING* X509_get0_subject_key_id(X509* x) {
  /* Call for side-effect of computing hash and caching extensions */
  if (X509_check_purpose(x, -1, 0) != 1) return NULL;
  return x->skid;
}
