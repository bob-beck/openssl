...
X509_POLICY_NODE* ossl_policy_level_find_node(const X509_POLICY_LEVEL* level,
  ...
  for (i = 0; i < sk_X509_POLICY_NODE_num(level->nodes); i++) {
    ...
    if (node->parent == parent) {
      if (!OBJ_cmp(node->data->valid_policy, id)) return node;
    }
  }
  return NULL;
}
...
X509_POLICY_NODE* ossl_policy_level_add_node(X509_POLICY_LEVEL* level,
  ...
  if (level != NULL) {
    if (OBJ_obj2nid(data->valid_policy) == NID_any_policy) {
      if (level->anyPolicy) goto node_error;
      level->anyPolicy = node;
    } else {
      if (level->nodes == NULL) level->nodes = ossl_policy_node_cmp_new();
      if (level->nodes == NULL) {
...
int ossl_policy_node_match(const X509_POLICY_LEVEL* lvl,
  ...
  if ((lvl->flags & X509_V_FLAG_INHIBIT_MAP) ||
    ...
    if (!OBJ_cmp(x->valid_policy, oid)) return 1;
    return 0;
  }

  for (i = 0; i < sk_ASN1_OBJECT_num(x->expected_policy_set); i++) {
    policy_oid = sk_ASN1_OBJECT_value(x->expected_policy_set, i);
    if (!OBJ_cmp(policy_oid, oid)) return 1;
  }
  return 0;
}
