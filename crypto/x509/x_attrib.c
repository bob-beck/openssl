...
static int
print_oid(BIO* out, const ASN1_OBJECT* oid)
{
  const char* ln;
  char objbuf[80];
  int rc;
    ...
    case V_ASN1_SEQUENCE:
      switch (obj_nid) {
        ...
        case NID_owner:
          ...
          xn = d2i_X509_NAME(
            NULL, (const unsigned char**)&value, av->value.sequence->length);
          if (xn == NULL) {
      ...
      return ASN1_parse_dump(out,
                             av->value.sequence->data,
                             av->value.sequence->length,
                             indent,
                             1) > 0;

    case V_ASN1_SET:
      return ASN1_parse_dump(
               out, av->value.set->data, av->value.set->length, indent, 1) > 0;
