...
X509_ATTRIBUTE* X509_ATTRIBUTE_create(int nid, int atrtype, void* value) {
  ...
  if ((oid = OBJ_nid2obj(nid)) == NULL) return NULL;
  if ((ret = X509_ATTRIBUTE_new()) == NULL) return NULL;
  ret->object = oid;
  if ((val = ASN1_TYPE_new()) == NULL) goto err;
  if (!sk_ASN1_TYPE_push(ret->set, val)) goto err;
...
static int print_oid(BIO* out, const ASN1_OBJECT* oid) {
  ...
  if (OBJ_obj2txt(objbuf, sizeof(objbuf), oid, 1) <= 0) return 0;
