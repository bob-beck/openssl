...
static int
def_crl_lookup(X509_CRL* crl,
               X509_REVOKED** ret,
               const ASN1_INTEGER* serial,
               const X509_NAME* issuer);

static X509_CRL_METHOD int_crl_meth = { 0,
                                        0,
                                        0,
                                        def_crl_lookup,
                                        def_crl_verify };
...
X509_CRL*
X509_CRL_new_ex(OSSL_LIB_CTX* libctx, const char* propq)
{
  X509_CRL* crl = NULL;
...
int
X509_CRL_get0_by_serial(X509_CRL* crl,
                        X509_REVOKED** ret,
                        const ASN1_INTEGER* serial)
{
...
{
  if (crl->meth->crl_lookup)
    return crl->meth->crl_lookup(
      crl, ret, X509_get0_serialNumber(x), X509_get_issuer_name(x));
  return 0;
}
...
static int
crl_revoked_issuer_match(X509_CRL* crl, const X509_NAME* nm, X509_REVOKED* rev)
{
  int i;
