...
static X509_CRL_METHOD int_crl_meth
    = { 0, 0, 0, def_crl_lookup, def_crl_verify };
...
crl_cb (int operation, ASN1_VALUE **pval, const ASN1_ITEM *it, void *exarg)
{
  ...
  switch (operation)
    ...
    case ASN1_OP_D2I_POST:
      ...
      crl->idp = X509_CRL_get_ext_d2i (crl, NID_issuing_distribution_point, &i,
                                       NULL);
      if (crl->idp != NULL)
        {
...
static int
crl_revoked_issuer_match (X509_CRL *crl, const X509_NAME *nm,
                          X509_REVOKED *rev)
{
  int i;
...
static int
def_crl_lookup (X509_CRL *crl, X509_REVOKED **ret, const ASN1_INTEGER *serial,
                const X509_NAME *issuer)
{
...
int
ossl_x509_crl_set0_libctx (X509_CRL *x, OSSL_LIB_CTX *libctx,
                           const char *propq)
{
  if (x != NULL)
    {
