...
ASN1_SEQUENCE_enc(X509_CRL_INFO, enc, crl_inf_cb) = {
    ...
    ASN1_EXP_SEQUENCE_OF_OPT(
        X509_CRL_INFO, extensions, X509_EXTENSION,
        0)} ASN1_SEQUENCE_END_enc(X509_CRL_INFO, X509_CRL_INFO)
  ...
  switch (operation) {
    case ASN1_OP_D2I_PRE:
      if (crl->meth->crl_free) {
        if (!crl->meth->crl_free(crl)) return 0;
      }
    ...
    case ASN1_OP_D2I_POST:
      ...
      if (crl->idp != NULL) {
        if (!setup_idp(crl, crl->idp)) crl->flags |= EXFLAG_INVALID;
      } else if (i != -1) {
        crl->flags |= EXFLAG_INVALID;
      }
      ...
      if (crl->akid == NULL && i != -1) crl->flags |= EXFLAG_INVALID;

      crl->crl_number = X509_CRL_get_ext_d2i(crl, NID_crl_number, &i, NULL);
      if (crl->crl_number == NULL && i != -1) crl->flags |= EXFLAG_INVALID;

      crl->base_crl_number = X509_CRL_get_ext_d2i(crl, NID_delta_crl, &i, NULL);
      if (crl->base_crl_number == NULL && i != -1) crl->flags |= EXFLAG_INVALID;
      ...
      for (idx = 0; idx < sk_X509_EXTENSION_num(exts); idx++) {
        ...
        if (nid == NID_freshest_crl) crl->flags |= EXFLAG_FRESHEST;
        if (X509_EXTENSION_get_critical(ext)) {
      ...
      if (!crl_set_issuers(crl)) return 0;

      if (crl->meth->crl_init) {
        if (crl->meth->crl_init(crl) == 0) return 0;
      }
    ...
    case ASN1_OP_DUP_POST: {
      ...
      if (!ossl_x509_crl_set0_libctx(crl, old->libctx, old->propq)) return 0;
    } break;
  }
  return 1;
}
...
static int setup_idp(X509_CRL* crl, ISSUING_DIST_POINT* idp) {
  ...
  if (idp_only > 1) crl->idp_flags |= IDP_INVALID;

  if (idp->indirectCRL > 0) crl->idp_flags |= IDP_INDIRECT;
...
int X509_CRL_verify(X509_CRL* crl, EVP_PKEY* r) {
  if (crl->meth->crl_verify) return crl->meth->crl_verify(crl, r);
  return 0;
}
...
static int crl_revoked_issuer_match(X509_CRL* crl, const X509_NAME* nm,
  ...
  if (!rev->issuer) {
    if (!nm) return 1;
    if (!X509_NAME_cmp(nm, X509_CRL_get_issuer(crl))) return 1;
    return 0;
  }

  if (!nm) nm = X509_CRL_get_issuer(crl);

  for (i = 0; i < sk_GENERAL_NAME_num(rev->issuer); i++) {
    GENERAL_NAME* gen = sk_GENERAL_NAME_value(rev->issuer, i);
    if (gen->type != GEN_DIRNAME) continue;
    if (!X509_NAME_cmp(nm, gen->d.directoryName)) return 1;
  }
  return 0;
}
...
static int def_crl_lookup(X509_CRL* crl, X509_REVOKED** ret,
  ...
  if (crl->crl.revoked == NULL) return 0;
  ...
  if (!sk_X509_REVOKED_is_sorted(crl->crl.revoked)) {
    if (!CRYPTO_THREAD_write_lock(crl->lock)) return 0;
  ...
  for (num = sk_X509_REVOKED_num(crl->crl.revoked); idx < num; idx++) {
    ...
    if (ASN1_INTEGER_cmp(&rev->serialNumber, serial)) return 0;
...
void X509_CRL_METHOD_free(X509_CRL_METHOD* m) {
  if (m == NULL || !(m->flags & X509_CRL_METHOD_DYNAMIC)) return;
  OPENSSL_free(m);
}
