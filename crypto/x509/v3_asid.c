...
ASN1_SEQUENCE(ASRange) = {
    ...
    ASN1_SIMPLE(ASRange, max, ASN1_INTEGER)} ASN1_SEQUENCE_END(ASRange)

ASN1_CHOICE(ASIdOrRange) = {
    ASN1_SIMPLE(ASIdOrRange, u.id, ASN1_INTEGER),
    ASN1_SIMPLE(ASIdOrRange, u.range, ASRange)} ASN1_CHOICE_END(ASIdOrRange)
  ...
  switch (choice->type) {
    ...
    case ASIdentifierChoice_asIdsOrRanges:
      for (i = 0; i < sk_ASIdOrRange_num(choice->u.asIdsOrRanges); i++) {
        ...
        switch (aor->type) {
          case ASIdOrRange_id:
            if ((s = i2s_ASN1_INTEGER(NULL, aor->u.id)) == NULL) return 0;
            BIO_printf(out, "%*s%s\n", indent + 2, "", s);
            OPENSSL_free(s);
            break;
...
int X509v3_asid_add_inherit(ASIdentifiers* asid, int which) {
  ...
  if (*choice == NULL) {
    if ((*choice = ASIdentifierChoice_new()) == NULL) return 0;
...
int X509v3_asid_add_id_or_range(ASIdentifiers* asid, int which,
  ...
  if ((aor = ASIdOrRange_new()) == NULL) return 0;
  if (!sk_ASIdOrRange_reserve((*choice)->u.asIdsOrRanges, 1)) goto err;
  if (max == NULL) {
    aor->type = ASIdOrRange_id;
    aor->u.id = min;
  } else {
    aor->type = ASIdOrRange_range;
    if ((aor->u.range = ASRange_new()) == NULL) goto err;
...
static int extract_min_max(ASIdOrRange* aor, ASN1_INTEGER** min,
                           ASN1_INTEGER** max) {
  if (!ossl_assert(aor != NULL)) return 0;
  switch (aor->type) {
    case ASIdOrRange_id:
      *min = aor->u.id;
      *max = aor->u.id;
...
static int ASIdentifierChoice_is_canonical(ASIdentifierChoice* choice) {
   ...
   */
  if (choice == NULL || choice->type == ASIdentifierChoice_inherit) return 1;
  ...
  for (i = 0; i < sk_ASIdOrRange_num(choice->u.asIdsOrRanges) - 1; i++) {
     ...
     */
    if (ASN1_INTEGER_cmp(a_max_plus_one, b_min) >= 0) goto done;
  }
...
static int ASIdentifierChoice_canonize(ASIdentifierChoice* choice) {
  ...
  for (i = 0; i < sk_ASIdOrRange_num(choice->u.asIdsOrRanges) - 1; i++) {
     ...
     */
    if (!ossl_assert(ASN1_INTEGER_cmp(a_min, b_min) <= 0)) goto done;
    ...
    if (ASN1_INTEGER_cmp(a_max_plus_one, b_min) == 0) {
      ...
      switch (a->type) {
        case ASIdOrRange_id:
          if ((r = OPENSSL_malloc(sizeof(*r))) == NULL) goto done;
          r->min = a_min;
          r->max = b_max;
  ...
  /* Paranoia */
  if (!ossl_assert(ASIdentifierChoice_is_canonical(choice))) goto done;
...
static void* v2i_ASIdentifiers(const struct v3_ext_method* method,
  ...
  for (i = 0; i < sk_CONF_VALUE_num(values); i++) {
    ...
    if (strcmp(val->value, "inherit") == 0) {
      if (X509v3_asid_add_inherit(asid, which)) continue;
   ...
   */
  if (!X509v3_asid_canonize(asid)) goto err;
  return asid;
...
int X509v3_asid_inherits(ASIdentifiers* asid) {
  return (asid != NULL && ((asid->asnum != NULL &&
...
static int asid_contains(ASIdOrRanges* parent, ASIdOrRanges* child) {
  ...
  for (c = 0; c < sk_ASIdOrRange_num(child); c++) {
    if (!extract_min_max(sk_ASIdOrRange_value(child, c), &c_min, &c_max))
      return 0;
    for (;; p++) {
      if (p >= sk_ASIdOrRange_num(parent)) return 0;
      if (!extract_min_max(sk_ASIdOrRange_value(parent, p), &p_min, &p_max))
        return 0;
      if (ASN1_INTEGER_cmp(p_max, c_max) < 0) continue;
      if (ASN1_INTEGER_cmp(p_min, c_min) > 0) return 0;
      break;
    }
  }
...
static int asid_validate_path_internal(X509_STORE_CTX* ctx,
  ...
  if (ext != NULL) {
    ...
    if ((ext = x->rfc3779_asid) == NULL) goto done;
  }
