    ...
    case X509_L_FILE_LOAD:
      if (argl == X509_FILETYPE_DEFAULT) {
        ...
        if (file)
          ...
          ok = (X509_load_cert_crl_file_ex(ctx, X509_get_default_cert_file(),
                                           X509_FILETYPE_PEM, libctx,
                                           propq) != 0);

        if (!ok) ERR_raise(ERR_LIB_X509, X509_R_LOADING_DEFAULTS);
      } else {
...
int X509_load_cert_crl_file_ex(X509_LOOKUP* ctx, const char* file, int type,
  ...
  if (count == 0) ERR_raise(ERR_LIB_X509, X509_R_NO_CERTIFICATE_OR_CRL_FOUND);
err:
