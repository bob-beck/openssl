...
static int
by_file_ctrl_ex (X509_LOOKUP *ctx, int cmd, const char *argp, long argl,
      ...
      if (argl == X509_FILETYPE_DEFAULT)
      ...
      else
        {
          if (argl == X509_FILETYPE_PEM)
            ...
            ok = (X509_load_cert_file_ex (ctx, argp, (int)argl, libctx, propq)
                  != 0);
        }
      break;
    }
...
int
X509_load_cert_file_ex (X509_LOOKUP *ctx, const char *file, int type,
...
int
X509_load_cert_crl_file_ex (X509_LOOKUP *ctx, const char *file, int type,
                            OSSL_LIB_CTX *libctx, const char *propq)
{
