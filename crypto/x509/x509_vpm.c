...
static int
int_x509_param_set_hosts(X509_VERIFY_PARAM* vpm,
                         int mode,
                         const char* name,
                         size_t namelen)
{
...
static int
int_x509_param_set1(char** pdest,
                    size_t* pdestlen,
                    const char* src,
                    size_t srclen)
{
...
unsigned long
X509_VERIFY_PARAM_get_flags(const X509_VERIFY_PARAM* param)
{
  return param->flags;
}

uint32_t
X509_VERIFY_PARAM_get_inh_flags(const X509_VERIFY_PARAM* param)
{
  return param->inh_flags;
}

int
X509_VERIFY_PARAM_set_inh_flags(X509_VERIFY_PARAM* param, uint32_t flags)
{
...
int
X509_VERIFY_PARAM_get_purpose(const X509_VERIFY_PARAM* param)
{
...
char*
X509_VERIFY_PARAM_get0_host(X509_VERIFY_PARAM* param, int idx)
{
...
char*
X509_VERIFY_PARAM_get0_peername(const X509_VERIFY_PARAM* param)
{
 ...
 */
void
X509_VERIFY_PARAM_move_peername(X509_VERIFY_PARAM* to, X509_VERIFY_PARAM* from)
{
...
char*
X509_VERIFY_PARAM_get0_email(X509_VERIFY_PARAM* param)
{
  return param->email;
}

int
X509_VERIFY_PARAM_set1_email(X509_VERIFY_PARAM* param,
                             const char* email,
                             size_t emaillen)
{
...
char*
X509_VERIFY_PARAM_get1_ip_asc(X509_VERIFY_PARAM* param)
{
  size_t iplen;
                          ...
                          size_t iplen)
{
  ...
  return int_x509_param_set1(
    (char**)&param->ip, &param->iplen, (char*)ip, iplen);
}
...
int
X509_VERIFY_PARAM_get_auth_level(const X509_VERIFY_PARAM* param)
{
...
static int
param_cmp(const X509_VERIFY_PARAM* const* a, const X509_VERIFY_PARAM* const* b)
{
  return strcmp((*a)->name, (*b)->name);
}
