...
static int int_x509_param_set_hosts(X509_VERIFY_PARAM* vpm, int mode,
  ...
  if (namelen == 0 || name == NULL)
    ...
    return 0;
  if (namelen > 0 && name[namelen - 1] == '\0') --namelen;
...
#define x509_verify_param_copy(field, def) \
  if (test_x509_verify_param_copy(field, def)) dest->field = src->field;
...
int X509_VERIFY_PARAM_inherit(X509_VERIFY_PARAM* dest,
  ...
  if ((inh_flags & X509_VP_FLAG_ONCE) != 0) dest->inh_flags = 0;

  if ((inh_flags & X509_VP_FLAG_LOCKED) != 0) return 1;
  ...
  if ((inh_flags & X509_VP_FLAG_RESET_FLAGS) != 0) dest->flags = 0;

  dest->flags |= src->flags;

  if (test_x509_verify_param_copy(policies, NULL)) {
    if (!X509_VERIFY_PARAM_set1_policies(dest, src->policies)) return 0;
  }
...
static int int_x509_param_set1(char** pdest, size_t* pdestlen, const char* src,
  ...
  OPENSSL_free(*pdest);
  *pdest = tmp;
  if (pdestlen != NULL) *pdestlen = srclen;
  return 1;
}
...
int X509_VERIFY_PARAM_add0_policy(X509_VERIFY_PARAM* param,
  ...
  if (param->policies == NULL) {
    ...
    if (param->policies == NULL) return 0;
  }

  if (sk_ASN1_OBJECT_push(param->policies, policy) <= 0) return 0;
  return 1;
}
...
void X509_VERIFY_PARAM_move_peername(X509_VERIFY_PARAM* to,
  ...
  if (from != NULL) from->peername = NULL;
}
...
int X509_VERIFY_PARAM_add0_table(X509_VERIFY_PARAM* param) {
  ...
  if (sk_X509_VERIFY_PARAM_push(param_table, param) <= 0) return 0;
  return 1;
}
...
int X509_VERIFY_PARAM_get_count(void) {
  ...
  if (param_table != NULL) num += sk_X509_VERIFY_PARAM_num(param_table);
  return num;
}
...
const X509_VERIFY_PARAM* X509_VERIFY_PARAM_get0(int id) {
  ...
  if (id < num) return default_table + id;
...
const X509_VERIFY_PARAM* X509_VERIFY_PARAM_lookup(const char* name) {
  ...
  if (param_table != NULL) {
    ...
    if (idx >= 0) return sk_X509_VERIFY_PARAM_value(param_table, idx);
  }
