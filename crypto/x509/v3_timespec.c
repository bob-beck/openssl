...
static const char* WEEK_NAMES[5] = { "first",
                                     "second",
                                     "third",
                                     "fourth",
                                     "final" };
...
static int
i2r_OSSL_TIME_SPEC_ABSOLUTE(X509V3_EXT_METHOD* method,
                            ...
                            int indent)
{
  if (time->startTime != NULL && time->endTime != NULL) {
    ...
    if (!ossl_asn1_time_print_ex(out, time->startTime, 0))
      return 0;
    if (BIO_printf(
          out, "%.*s", time->startTime->length, time->startTime->data) <= 0)
      return 0;
...
static int
i2r_OSSL_DAY_TIME(X509V3_EXT_METHOD* method,
                  OSSL_DAY_TIME* dt,
                  BIO* out,
                  int indent)
{
...
static int
i2r_OSSL_DAY_TIME_BAND(X509V3_EXT_METHOD* method,
...
static int
print_int_month(BIO* out, int64_t month)
{
  switch (month) {
...
static int
print_bit_month(BIO* out, ASN1_BIT_STRING* bs)
{
 ...
 */
static int
print_bit_week(BIO* out, ASN1_BIT_STRING* bs)
{
...
static int
print_day_of_week(BIO* out, ASN1_BIT_STRING* bs)
{
...
static int
print_int_day_of_week(BIO* out, int64_t dow)
{
  switch (dow) {
...
static int
print_int_named_day(BIO* out, int64_t nd)
{
  switch (nd) {
...
static int
print_bit_named_day(BIO* out, ASN1_BIT_STRING* bs)
{
  return print_day_of_week(out, bs);
}

static int
i2r_OSSL_PERIOD(X509V3_EXT_METHOD* method,
                OSSL_TIME_PERIOD* p,
                BIO* out,
                int indent)
{
...
static int
i2r_OSSL_TIME_SPEC_TIME(X509V3_EXT_METHOD* method,
  ...
  switch (time->type) {
    case (OSSL_TIME_SPEC_TIME_TYPE_ABSOLUTE):
      if (BIO_printf(out, "%*sAbsolute: ", indent, "") <= 0)
        return 0;
      if (i2r_OSSL_TIME_SPEC_ABSOLUTE(
            method, time->choice.absolute, out, indent + 4) <= 0)
        return 0;
      return BIO_puts(out, "\n");
...
static int
i2r_OSSL_TIME_SPEC(X509V3_EXT_METHOD* method,
                   OSSL_TIME_SPEC* time,
                   BIO* out,
                   int indent)
{
  ...
  if (time->timeZone) {
    if (ASN1_INTEGER_get_int64(&tz, time->timeZone) != 1)
      return 0;
    if (BIO_printf(
          out, "%*sTimezone: UTC%+03lld:00\n", indent, "", (long long int)tz) <=
        0)
      return 0;
  }
