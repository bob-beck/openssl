...
static int i2r_OSSL_DAY_TIME(X509V3_EXT_METHOD* method, OSSL_DAY_TIME* dt,
  ...
  if (!dt->hour || !ASN1_INTEGER_get_int64(&h, dt->hour)) return 0;
...
static int i2r_OSSL_DAY_TIME_BAND(X509V3_EXT_METHOD* method,
  ...
  if (band->startDayTime) {
    if (!i2r_OSSL_DAY_TIME(method, band->startDayTime, out, indent)) return 0;
  } else if (!BIO_puts(out, "00:00:00")) {
    return 0;
  }
  if (!BIO_puts(out, " - ")) return 0;
  if (band->endDayTime) {
    if (!i2r_OSSL_DAY_TIME(method, band->endDayTime, out, indent)) return 0;
...
static int print_bit_month(BIO* out, ASN1_BIT_STRING* bs) {
  ...
  for (; i <= OSSL_TIME_SPEC_BIT_MONTH_DEC; i++) {
    if (ASN1_BIT_STRING_get_bit(bs, i)) {
      if (j > 0 && !BIO_puts(out, ", ")) return 0;
      j++;
      if (!BIO_puts(out, MONTH_NAMES[i])) return 0;
    }
  }
  return 1;
}
...
static int print_bit_week(BIO* out, ASN1_BIT_STRING* bs) {
  ...
  for (; i <= OSSL_TIME_SPEC_BIT_WEEKS_5; i++) {
    if (ASN1_BIT_STRING_get_bit(bs, i)) {
      if (j > 0 && !BIO_puts(out, ", ")) return 0;
      j++;
      if (!BIO_puts(out, WEEK_NAMES[i])) return 0;
    }
  }
  return 1;
}
...
static int print_day_of_week(BIO* out, ASN1_BIT_STRING* bs) {
  ...
  for (; i <= OSSL_TIME_SPEC_DAY_BIT_SAT; i++) {
    if (ASN1_BIT_STRING_get_bit(bs, i)) {
      if (j > 0 && !BIO_puts(out, ", ")) return 0;
      j++;
      if (!BIO_puts(out, WEEKDAY_NAMES[i])) return 0;
    }
  }
  return 1;
}
...
static int i2r_OSSL_PERIOD(X509V3_EXT_METHOD* method, OSSL_TIME_PERIOD* p,
  ...
  if (p->timesOfDay) {
    ...
    for (i = 0; i < sk_OSSL_DAY_TIME_BAND_num(p->timesOfDay); i++) {
      ...
      if (BIO_printf(out, "%*s", indent + 8, "") <= 0) return 0;
      if (!i2r_OSSL_DAY_TIME_BAND(method, band, out, indent + 8)) return 0;
      if (!BIO_puts(out, "\n")) return 0;
    }
  }
  if (p->days) {
    if (p->days->type == OSSL_TIME_SPEC_DAY_TYPE_INT) {
      ...
      if (BIO_printf(out, "%*sDays: ", indent + 4, "") <= 0) return 0;
    }
    ...
    switch (p->days->type) {
      case (OSSL_TIME_SPEC_DAY_TYPE_INT):
        for (i = 0; i < sk_ASN1_INTEGER_num(p->days->choice.intDay); i++) {
          ...
          if (!ASN1_INTEGER_get_int64(&small_val, big_val)) return 0;
          if (i > 0 && !BIO_puts(out, ", ")) return 0;
          /* If weeks is defined, then print day of week by name. */
          if (p->weeks != NULL) {
            if (!print_int_day_of_week(out, small_val)) return 0;
      ...
      case (OSSL_TIME_SPEC_DAY_TYPE_DAY_OF):
        switch (p->days->choice.dayOf->type) {
          ...
          case (OSSL_TIME_SPEC_X_DAY_OF_THIRD):
            if (!BIO_puts(out, "THIRD ")) return 0;
          ...
          case (OSSL_TIME_SPEC_X_DAY_OF_FIFTH):
            if (!BIO_puts(out, "FIFTH ")) return 0;
        ...
        switch (nd->type) {
          case (OSSL_NAMED_DAY_TYPE_INT):
            if (!ASN1_INTEGER_get_int64(&small_val, nd->choice.intNamedDays))
              return 0;
            if (!print_int_named_day(out, small_val)) return 0;
            break;
  ...
  if (p->weeks) {
    if (p->weeks->type == OSSL_TIME_SPEC_WEEKS_TYPE_INT) {
      ...
      if (BIO_printf(out, "%*sWeeks: ", indent + 4, "") <= 0) return 0;
    }

    switch (p->weeks->type) {
      case (OSSL_TIME_SPEC_WEEKS_TYPE_ALL):
        if (!BIO_puts(out, "ALL")) return 0;
        break;
      case (OSSL_TIME_SPEC_WEEKS_TYPE_INT):
        for (i = 0; i < sk_ASN1_INTEGER_num(p->weeks->choice.intWeek); i++) {
          ...
          if (!ASN1_INTEGER_get_int64(&small_val, big_val)) return 0;
          if (i > 0 && !BIO_puts(out, ", ")) return 0;
          if (!BIO_printf(out, "%lld", (long long int)small_val)) return 0;
        }
        break;
  ...
  if (p->months) {
    if (BIO_printf(out, "%*sMonths: ", indent + 4, "") <= 0) return 0;
    switch (p->months->type) {
      ...
      case (OSSL_TIME_SPEC_MONTH_TYPE_INT):
        for (i = 0; i < sk_ASN1_INTEGER_num(p->months->choice.intMonth); i++) {
          ...
          if (!ASN1_INTEGER_get_int64(&small_val, big_val)) return 0;
          if (i > 0 && !BIO_puts(out, ", ")) return 0;
          if (!print_int_month(out, small_val)) return 0;
        }
        break;
      case (OSSL_TIME_SPEC_MONTH_TYPE_BIT):
        if (!print_bit_month(out, p->months->choice.bitMonth)) return 0;
        break;
      default:
        return 0;
    }
  ...
  if (p->years) {
    ...
    for (i = 0; i < sk_ASN1_INTEGER_num(p->years); i++) {
      ...
      if (!ASN1_INTEGER_get_int64(&small_val, big_val)) return 0;
      if (i > 0 && !BIO_puts(out, ", ")) return 0;
      if (BIO_printf(out, "%04lld", (long long int)small_val) <= 0) return 0;
    }
  }
  return 1;
}
...
static int i2r_OSSL_TIME_SPEC_TIME(X509V3_EXT_METHOD* method,
  ...
  switch (time->type) {
    case (OSSL_TIME_SPEC_TIME_TYPE_ABSOLUTE):
      if (BIO_printf(out, "%*sAbsolute: ", indent, "") <= 0) return 0;
    ...
    case (OSSL_TIME_SPEC_TIME_TYPE_PERIODIC):
      ...
      for (i = 0; i < sk_OSSL_TIME_PERIOD_num(time->choice.periodic); i++) {
        ...
        if (!i2r_OSSL_PERIOD(method, tp, out, indent + 4)) return 0;
      }
      return BIO_puts(out, "\n");
    default:
      return 0;
...
static int i2r_OSSL_TIME_SPEC(X509V3_EXT_METHOD* method, OSSL_TIME_SPEC* time,
  ...
  if (time->timeZone) {
    if (ASN1_INTEGER_get_int64(&tz, time->timeZone) != 1) return 0;
