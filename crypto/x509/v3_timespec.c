                                ...
                                ASN1_SEQUENCE(OSSL_TIME_PERIOD)
                                    ...
                                    ASN1_CHOICE(OSSL_TIME_SPEC_TIME)
                                        = {ASN1_SIMPLE(OSSL_TIME_SPEC_TIME, choice.absolute, OSSL_TIME_SPEC_ABSOLUTE),
                                           ASN1_SET_OF(OSSL_TIME_SPEC_TIME,
                                                       choice.periodic,
                                        ...
                                        ASN1_SEQUENCE(OSSL_TIME_SPEC)
                                            ...
                                            /*
                                             * It might seem like you could just print the bits of the string
                                             * numerically, but the fifth bit has the special meaning of "the final
                                             * week" imputed to it by the text of ITU-T Recommendation X.520.
                                             */
                                                                       ...
                                                                       int indent)
                                            {
                                                ...
                                                if (p->days) {
                                                    ...
                                                    case (OSSL_TIME_SPEC_DAY_TYPE_INT):
                                                        for (i = 0; i < sk_ASN1_INTEGER_num(p->days->choice.intDay);
                                                             i++) {
                                                ...
                                                if (p->weeks) {
                                                    ...
                                                    case (OSSL_TIME_SPEC_WEEKS_TYPE_INT):
                                                        for (i = 0; i < sk_ASN1_INTEGER_num(p->weeks->choice.intWeek);
                                                             i++) {
                                                            big_val =
                                                                sk_ASN1_INTEGER_value(p->weeks->choice.intWeek, i);
                                                            if (!ASN1_INTEGER_get_int64(&small_val, big_val))
                                                                return 0;
                                                            if (i > 0 && !BIO_puts(out, ", "))
                                                                return 0;
                                                ...
                                                if (p->months) {
                                                    ...
                                                    case (OSSL_TIME_SPEC_MONTH_TYPE_INT):
                                                        for (i = 0; i < sk_ASN1_INTEGER_num(p->months->choice.intMonth);
                                                             i++) {
                                                            big_val =
                                                                sk_ASN1_INTEGER_value(p->months->choice.intMonth, i);
                                                            if (!ASN1_INTEGER_get_int64(&small_val, big_val))
                                                                return 0;
                                                            if (i > 0 && !BIO_puts(out, ", "))
                                                                return 0;
                                                                               ...
                                                                               int indent)
                                            {
                                                ...
                                                case (OSSL_TIME_SPEC_TIME_TYPE_ABSOLUTE):
                                                    if (BIO_printf(out, "%*sAbsolute: ", indent, "") <= 0)
                                                        return 0;
                                                    if (i2r_OSSL_TIME_SPEC_ABSOLUTE(method,
                                                                                    time->choice.absolute,
                                                                                    out,
                                                                                    indent + 4)
                                                        <= 0)
                                                        return 0;
                                                ...
                                                case (OSSL_TIME_SPEC_TIME_TYPE_PERIODIC):
                                                    if (BIO_printf(out, "%*sPeriodic:\n", indent, "") <= 0)
                                                        return 0;
                                                    for (i = 0; i < sk_OSSL_TIME_PERIOD_num(time->choice.periodic);
                                                         i++) {
                                                        if (i > 0 && !BIO_puts(out, "\n"))
                                                            return 0;
