...
int X509_set_version(X509* x, long version) {
  ...
  if (x->cert_info.version == NULL) {
    if ((x->cert_info.version = ASN1_INTEGER_new()) == NULL) return 0;
  }
...
int X509_set_serialNumber(X509* x, ASN1_INTEGER* serial) {
  ...
  if (in != serial) return ASN1_STRING_copy(in, serial);
  x->cert_info.enc.modified = 1;
  return 1;
}

int X509_set_issuer_name(X509* x, const X509_NAME* name) {
  if (x == NULL || !X509_NAME_set(&x->cert_info.issuer, name)) return 0;
  x->cert_info.enc.modified = 1;
  return 1;
}

int X509_set_subject_name(X509* x, const X509_NAME* name) {
  if (x == NULL || !X509_NAME_set(&x->cert_info.subject, name)) return 0;
  x->cert_info.enc.modified = 1;
  return 1;
}
...
int X509_up_ref(X509* x) {
  ...
  if (CRYPTO_UP_REF(&x->references, &i) <= 0) return 0;
...
void X509_get0_uids(const X509* x, const ASN1_BIT_STRING** piuid,
  ...
  if (piuid != NULL) *piuid = x->cert_info.issuerUID;
  if (psuid != NULL) *psuid = x->cert_info.subjectUID;
}
...
int X509_SIG_INFO_get(const X509_SIG_INFO* siginf, int* mdnid, int* pknid,
  ...
  if (mdnid != NULL) *mdnid = siginf->mdnid;
  if (pknid != NULL) *pknid = siginf->pknid;
  if (secbits != NULL) *secbits = siginf->secbits;
  if (flags != NULL) *flags = siginf->flags;
