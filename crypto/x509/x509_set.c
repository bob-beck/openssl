...
int
X509_set_issuer_name(X509* x, const X509_NAME* name)
{
...
int
X509_set_subject_name(X509* x, const X509_NAME* name)
{
...
X509_set1_notBefore(X509* x, const ASN1_TIME* tm)
{
  ...
  return ossl_x509_set1_time(
    &x->cert_info.enc.modified, &x->cert_info.validity.notBefore, tm);
}
...
X509_set1_notAfter(X509* x, const ASN1_TIME* tm)
{
  if (x == NULL || tm == NULL)
    return 0;
  return ossl_x509_set1_time(
    &x->cert_info.enc.modified, &x->cert_info.validity.notAfter, tm);
}
...
int
X509_SIG_INFO_get(const X509_SIG_INFO* siginf,
                  int* mdnid,
                  int* pknid,
                  int* secbits,
                  uint32_t* flags)
...
void
X509_SIG_INFO_set(X509_SIG_INFO* siginf,
                  int mdnid,
                  int pknid,
                  int secbits,
                  uint32_t flags)
...
int
X509_get_signature_info(X509* x,
                        int* mdnid,
                        int* pknid,
                        int* secbits,
                        uint32_t* flags)
...
static int
x509_sig_info_init(X509_SIG_INFO* siginf,
...
{
  return x509_sig_info_init(
    &x->siginf, &x->sig_alg, &x->signature, X509_PUBKEY_get0(x->cert_info.key));
}
