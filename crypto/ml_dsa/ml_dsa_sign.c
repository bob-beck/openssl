...
EVP_MD_CTX* ossl_ml_dsa_mu_init(const ML_DSA_KEY* key, int encode,
  ...
  if (md_ctx == NULL) return NULL;

  /* H(.. */
  if (!EVP_DigestInit_ex2(md_ctx, key->shake256_md, NULL)) goto err;
  /* ..pk (= key->tr) */
  if (!EVP_DigestUpdate(md_ctx, key->tr, sizeof(key->tr))) goto err;
  /* M' = .. */
  if (encode) {
    if (ctx_len > ML_DSA_MAX_CONTEXT_STRING_LEN) goto err;
    /* IntegerToBytes(0, 1) .. */
    itb[0] = 0;
    /* || IntegerToBytes(|ctx|, 1) || .. */
    itb[1] = (uint8_t)ctx_len;
    if (!EVP_DigestUpdate(md_ctx, itb, 2)) goto err;
    /* ctx || .. */
    if (!EVP_DigestUpdate(md_ctx, ctx, ctx_len)) goto err;
...
static int ml_dsa_sign_internal(const ML_DSA_KEY* priv, const uint8_t* mu,
  ...
  if (!matrix_expand_A(md_ctx, priv->shake128_md, priv->rho, &a_ntt)) goto err;
...
int ossl_ml_dsa_sign(const ML_DSA_KEY* priv, int msg_is_mu, const uint8_t* msg,
  ...
  if (ossl_ml_dsa_key_get_priv(priv) == NULL) return 0;

  if (sig_len != NULL) *sig_len = priv->params->sig_len;

  if (sig == NULL) return (sig_len != NULL) ? 1 : 0;

  if (sig_size < priv->params->sig_len) return 0;
  ...
  if (msg_is_mu) {
    ...
    if (!ossl_ml_dsa_mu_update(md_ctx, msg, msg_len)) goto err;

    if (!ossl_ml_dsa_mu_finalize(md_ctx, mu, mu_len)) goto err;
  }
...
int ossl_ml_dsa_verify(const ML_DSA_KEY* pub, int msg_is_mu, const uint8_t* msg,
  ...
  if (ossl_ml_dsa_key_get_pub(pub) == NULL) return 0;
