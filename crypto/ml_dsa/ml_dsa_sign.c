 ...
 */
static void
signature_init(ML_DSA_SIG *sig, POLY *hint, uint32_t k, POLY *z, uint32_t l, uint8_t *c_tilde, size_t c_tilde_len)
{
                                ...
                                uint8_t          *out_sig)
{
    ...
    for (kappa = 0;; kappa += l) {
        ...
        vector_expand_mask(&y, rho_prime, sizeof(rho_prime), (uint32_t)kappa, gamma1, md_ctx, priv->shake256_md);
        vector_copy(y_ntt, &y);
        vector_ntt(y_ntt);
        ...
        if (!poly_sample_in_ball_ntt(c_ntt, c_tilde, (int)c_tilde_len, md_ctx, priv->shake256_md, params->tau))
            break;
                                  ...
                                  size_t            sig_enc_len)
{
    ...
    if (!poly_sample_in_ball_ntt(c_ntt, c_tilde_sig, (int)c_tilde_len, md_ctx, pub->shake256_md, params->tau))
        goto err;
