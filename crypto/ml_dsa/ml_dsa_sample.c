...
static int rej_ntt_poly(EVP_MD_CTX* g_ctx, const EVP_MD* md,
   ...
   */
  if (!shake_xof(g_ctx, md, seed, seed_len, blocks, sizeof(blocks))) return 0;

  while (1) {
    for (b = blocks; b < end; b += 3) {
      if (coeff_from_three_bytes(b, &(out->coeff[j]))) {
        if (++j >= ML_DSA_NUM_POLY_COEFFICIENTS) return 1; /* finished */
      }
    }
    if (!EVP_DigestSqueeze(g_ctx, blocks, sizeof(blocks))) return 0;
  }
}
...
static int rej_bounded_poly(EVP_MD_CTX* h_ctx, const EVP_MD* md,
  ...
  if (!shake_xof(h_ctx, md, seed, seed_len, blocks, sizeof(blocks))) return 0;
  ...
  while (1) {
    ...
    if (!EVP_DigestSqueeze(h_ctx, blocks, sizeof(blocks))) return 0;
  }
}
...
int ossl_ml_dsa_poly_sample_in_ball(POLY* out_c, const uint8_t* seed,
   ...
   */
  if (!shake_xof(h_ctx, md, seed, seed_len, block, sizeof(block))) return 0;
  ...
  for (end = 256 - tau; end < 256; end++) {
    ...
    for (;;) {
      if (offset == sizeof(block)) {
        ...
        if (!EVP_DigestSqueeze(h_ctx, block, sizeof(block))) return 0;
        offset = 0;
      }
