 ...
 * SHAKE128 is used to absorb the seed, and then sequences of 3 sample bytes
 * are squeezed to try to produce coefficients. The SHAKE128 stream is used to
 * get uniformly distributed elements. This algorithm is used for matrix
 ...
 */
static int
rej_ntt_poly (EVP_MD_CTX *g_ctx, const EVP_MD *md, const uint8_t *seed,
              size_t seed_len, POLY *out)
{
  int j = 0;
 ...
 */
static int
rej_bounded_poly (EVP_MD_CTX *h_ctx, const EVP_MD *md,
 ...
 */
int
ossl_ml_dsa_matrix_expand_A (EVP_MD_CTX *g_ctx, const EVP_MD *md,
 ...
 * @param md A pre-fetched SHAKE256 object.
 * @param eta Is either 2 or 4, and determines the range of the coefficients
 * for s1 and s2.
 ...
 */
int
ossl_ml_dsa_vector_expand_S (EVP_MD_CTX *h_ctx, const EVP_MD *md, int eta,
...
int
ossl_ml_dsa_poly_expand_mask (POLY *out, const uint8_t *seed, size_t seed_len,
 ...
 */
int
ossl_ml_dsa_poly_sample_in_ball (POLY *out_c, const uint8_t *seed,
