 ...
 * (but since r0 is kept positive it effectively adds q and then reduces by q
 * if needed) Similarly for the range r = 8192..8192+4096 r1=1 and r0=0..4096
 ...
 */
void
ossl_ml_dsa_key_compress_decompose (uint32_t r, uint32_t gamma2, uint32_t *r1,
                                    int32_t *r0)
{
 ...
 *             polynomial vector t0 (which encodes the least significant bits
 of each coefficient of the uncompressed public-key polynomial t)
 ...
 */
int32_t
ossl_ml_dsa_key_compress_make_hint (uint32_t ct0, uint32_t cs2,
