...
#define POLY_COEFF_NUM_BYTES(bits)                                            \
 ...
 * @param pkt A packet to write 128 bytes of encoded polynomial coefficients
 * to.
 *
 * @returns 1 on success, or 0 on error.
 */
 ...
 * Hint is composed of k polynomials with binary coefficients where only
 * 'omega' of all the coefficients are set to 1. This can be encoded as a byte
 * array of 'omega' polynomial coefficient index positions for the coefficients
 * that are set, followed by k values of the last coefficient index used in
 * each polynomial.
 */
static int
 ...
 */
int
ossl_ml_dsa_sig_encode (const ML_DSA_SIG *sig, const ML_DSA_PARAMS *params,
                        uint8_t *out)
{
  int ret = 0;
  size_t i;
...
int
ossl_ml_dsa_poly_decode_expand_mask (POLY *out, const uint8_t *in,
 ...
 */
int
ossl_ml_dsa_w1_encode (const VECTOR *w1, uint32_t gamma2, uint8_t *out,
                       size_t out_len)
{
  WPACKET pkt;
  ENCODE_FN *encode_fn;
