...
static int poly_encode_4_bits(const POLY* p, WPACKET* pkt) {
  ...
  if (!WPACKET_allocate_bytes(pkt, POLY_COEFF_NUM_BYTES(4), &out)) return 0;
...
static int poly_encode_6_bits(const POLY* p, WPACKET* pkt) {
  ...
  if (!WPACKET_allocate_bytes(pkt, POLY_COEFF_NUM_BYTES(6), &out)) return 0;
...
static int poly_encode_10_bits(const POLY* p, WPACKET* pkt) {
  ...
  if (!WPACKET_allocate_bytes(pkt, POLY_COEFF_NUM_BYTES(10), &out)) return 0;
...
static int poly_encode_signed_4(const POLY* p, WPACKET* pkt) {
  ...
  if (!WPACKET_allocate_bytes(pkt, 32 * 4, &out)) return 0;
...
static int poly_decode_signed_4(POLY* p, PACKET* pkt) {
  ...
  for (i = 0; i < (ML_DSA_NUM_POLY_COEFFICIENTS / 8); i++) {
    if (!PACKET_get_bytes(pkt, &in, 4)) goto err;
    in = OPENSSL_load_u32_le(&v, in);
     ...
     */
    if (value_barrier_32((mask & v) != 0)) goto err;
...
static int poly_encode_signed_2(const POLY* p, WPACKET* pkt) {
  ...
  if (!WPACKET_allocate_bytes(pkt, POLY_COEFF_NUM_BYTES(3), &out)) return 0;
...
static int poly_decode_signed_2(POLY* p, PACKET* pkt) {
  ...
  for (i = 0; i < (ML_DSA_NUM_POLY_COEFFICIENTS / 8); i++) {
    if (!PACKET_get_bytes(pkt, &in, 3)) goto err;
    memcpy(&u, in, 3);
...
static int poly_encode_signed_two_to_power_12(const POLY* p, WPACKET* pkt) {
  ...
  do {
    ...
    if (!WPACKET_allocate_bytes(pkt, 13, &out)) return 0;
...
static int poly_decode_signed_two_to_power_12(POLY* p, PACKET* pkt) {
  ...
  for (i = 0; i < (ML_DSA_NUM_POLY_COEFFICIENTS / 8); i++) {
    ...
    if (!PACKET_get_bytes(pkt, &in, 13)) goto err;
...
static int poly_encode_signed_two_to_power_19(const POLY* p, WPACKET* pkt) {
  ...
  do {
    ...
    if (!WPACKET_allocate_bytes(pkt, 10, &out)) return 0;
...
static int poly_decode_signed_two_to_power_19(POLY* p, PACKET* pkt) {
  ...
  for (i = 0; i < (ML_DSA_NUM_POLY_COEFFICIENTS / 4); i++) {
    ...
    if (!PACKET_get_bytes(pkt, &in, 10)) goto err;
...
static int poly_encode_signed_two_to_power_17(const POLY* p, WPACKET* pkt) {
  ...
  do {
    ...
    if (!WPACKET_allocate_bytes(pkt, 9, &out)) return 0;
...
static int poly_decode_signed_two_to_power_17(POLY* p, PACKET* pkt) {
  ...
  do {
    ...
    if (!PACKET_get_bytes(pkt, &in, 9)) return 0;
    in = OPENSSL_load_u32_le(&a1, in);
    in = OPENSSL_load_u32_le(&a2, in);
    a3 = (uint32_t)*in;
...
int ossl_ml_dsa_pk_encode(ML_DSA_KEY* key) {
  ...
  if (!WPACKET_init_static_len(&pkt, enc, enc_len, 0) ||
    ...
    goto err;
  for (i = 0; i < t1_len; i++)
    if (!poly_encode_10_bits(t1 + i, &pkt)) goto err;
...
int ossl_ml_dsa_pk_decode(ML_DSA_KEY* key, const uint8_t* in, size_t in_len) {
  ...
  if (in_len != key->params->pk_len) return 0;
  ...
  for (i = 0; i < key->t1.num_poly; i++)
    if (!poly_decode_10_bits(key->t1.poly + i, &pkt)) goto err;
...
int ossl_ml_dsa_sk_encode(ML_DSA_KEY* key) {
  ...
  if (!WPACKET_init_static_len(&pkt, enc, enc_len, 0) ||
    ...
    goto err;
  for (i = 0; i < l; ++i)
    if (!encode_fn(key->s1.poly + i, &pkt)) goto err;
  for (i = 0; i < k; ++i)
    if (!encode_fn(key->s2.poly + i, &pkt)) goto err;
  for (i = 0; i < k; ++i)
    if (!poly_encode_signed_two_to_power_12(t0++, &pkt)) goto err;
  if (!WPACKET_get_total_written(&pkt, &written) || written != enc_len)
    goto err;
...
err:
  WPACKET_finish(&pkt);
  if (ret == 0) OPENSSL_secure_clear_free(enc, enc_len);
  return ret;
}
...
int ossl_ml_dsa_sk_decode(ML_DSA_KEY* key, const uint8_t* in, size_t in_len) {
  ...
  if ((key->priv_encoding != NULL && key->priv_encoding != in) ||
    ...
    return 0; /* Do not allow key mutation */
  if (in_len != key->params->sk_len) return 0;
  ...
  for (i = 0; i < l; ++i)
    if (!decode_fn(key->s1.poly + i, &pkt)) goto err;
  for (i = 0; i < k; ++i)
    if (!decode_fn(key->s2.poly + i, &pkt)) goto err;
  for (i = 0; i < k; ++i)
    if (!poly_decode_signed_two_to_power_12(key->t0.poly + i, &pkt)) goto err;
  if (PACKET_remaining(&pkt) != 0) goto err;
  if (key->priv_encoding == NULL) {
    key->priv_encoding = OPENSSL_secure_malloc(in_len);
    if (key->priv_encoding == NULL) goto err;
...
static int hint_bits_encode(const VECTOR* hint, WPACKET* pkt, uint32_t omega) {
  ...
  if (!WPACKET_allocate_bytes(pkt, omega + k, &data)) return 0;
  memset(data, 0, omega + k);

  for (i = 0; i < k; i++, p++) {
    for (j = 0; j < ML_DSA_NUM_POLY_COEFFICIENTS; j++)
      if (p->coeff[j] != 0) data[coeff_index++] = j;
...
static int hint_bits_decode(VECTOR* hint, PACKET* pkt, uint32_t omega) {
  ...
  do {
    ...
    if (limit < coeff_index || limit > omega) return 0;

    while (coeff_index < limit) {
      int byte = in[coeff_index++];

      if (last >= 0 && byte <= last) return 0;
      last = byte;
      p->coeff[byte] = 1;
    }
  } while (++p < end);
...
int ossl_ml_dsa_sig_encode(const ML_DSA_SIG* sig, const ML_DSA_PARAMS* params,
  ...
  for (i = 0; i < sig->z.num_poly; ++i)
    if (!encode_fn(sig->z.poly + i, &pkt)) goto err;
  if (!hint_bits_encode(&sig->hint, &pkt, params->omega)) goto err;
  ret = 1;
err:
  WPACKET_finish(&pkt);
  return ret;
...
int ossl_ml_dsa_sig_decode(ML_DSA_SIG* sig, const uint8_t* in, size_t in_len,
  ...
  if (!PACKET_buf_init(&pkt, in, in_len) ||
    ...
    goto err;
  for (i = 0; i < sig->z.num_poly; ++i)
    if (!decode_fn(sig->z.poly + i, &pkt)) goto err;
...
int ossl_ml_dsa_poly_decode_expand_mask(POLY* out, const uint8_t* in,
  ...
  if (!PACKET_buf_init(&pkt, in, in_len)) return 0;
