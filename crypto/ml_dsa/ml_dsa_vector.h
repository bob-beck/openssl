...
static ossl_inline ossl_unused void vector_zero(VECTOR* va) {
  if (va->poly != NULL) memset(va->poly, 0, va->num_poly * sizeof(va->poly[0]));
}
...
static ossl_inline ossl_unused int vector_equal(const VECTOR* a,
  ...
  if (a->num_poly != b->num_poly) return 0;
  for (i = 0; i < a->num_poly; ++i) {
    if (!poly_equal(a->poly + i, b->poly + i)) return 0;
  }
  return 1;
}
...
static ossl_inline ossl_unused void vector_ntt_inverse(VECTOR* va) {
  ...
  for (i = 0; i < va->num_poly; i++) ossl_ml_dsa_poly_ntt_inverse(va->poly + i);
}

/* @brief multiply a vector by a SCALAR polynomial */
static ossl_inline ossl_unused void vector_mult_scalar(const VECTOR* lhs,
                                                       const POLY* rhs,
                                                       VECTOR* out) {
  size_t i;
...
static ossl_inline ossl_unused void vector_expand_mask(
    VECTOR* out, const uint8_t* rho_prime, size_t rho_prime_len, uint32_t kappa,
...
static ossl_inline ossl_unused void vector_scale_power2_round_ntt(
...
static ossl_inline ossl_unused void vector_high_bits(const VECTOR* in,
                                                     uint32_t gamma2,
                                                     VECTOR* out) {
  size_t i;
...
static ossl_inline ossl_unused void vector_low_bits(const VECTOR* in,
                                                    uint32_t gamma2,
                                                    VECTOR* out) {
  size_t i;
...
static ossl_inline ossl_unused void vector_make_hint(const VECTOR* ct0,
                                                     const VECTOR* cs2,
                                                     const VECTOR* w,
                                                     uint32_t gamma2,
                                                     VECTOR* out) {
