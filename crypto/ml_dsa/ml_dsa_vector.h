...
static ossl_inline ossl_unused int
vector_expand_S(EVP_MD_CTX* h_ctx,
                const EVP_MD* md,
                int eta,
                const uint8_t* seed,
                VECTOR* s1,
                   ...
                   const EVP_MD* md)
{
  ...
  for (i = 0; i < out->num_poly; i++) {
    ...
    poly_expand_mask(
      out->poly + i, derived_seed, sizeof(derived_seed), gamma1, h_ctx, md);
  }
}
...
static ossl_inline ossl_unused void
vector_make_hint(const VECTOR* ct0,
                 const VECTOR* cs2,
                 const VECTOR* w,
                 uint32_t gamma2,
                 VECTOR* out)
{
  ...
  for (i = 0; i < out->num_poly; i++)
    poly_make_hint(
      ct0->poly + i, cs2->poly + i, w->poly + i, gamma2, out->poly + i);
}
