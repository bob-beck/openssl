...
int ossl_ml_dsa_set_prekey(ML_DSA_KEY* key, int flags_set, int flags_clr,
  ...
  if (sk != NULL) {
    ...
    if (key->priv_encoding == NULL) goto end;
    memcpy(key->priv_encoding, sk, sk_len);
  }

  if (seed != NULL) {
    if ((key->seed = OPENSSL_secure_malloc(seed_len)) == NULL) goto end;
...
ML_DSA_KEY* ossl_ml_dsa_key_new(OSSL_LIB_CTX* libctx, const char* propq,
  ...
  if (ret != NULL) {
    ...
    if (ret->shake128_md == NULL || ret->shake256_md == NULL) goto err;
  }
  return ret;
err:
...
int ossl_ml_dsa_key_priv_alloc(ML_DSA_KEY* key) {
  ...
  if (key->s1.poly != NULL) return 0;
  if (!vector_secure_alloc(&key->s1, l + 2 * k)) return 0;
  ...
  if (ret != NULL) {
    ...
    if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
      if (src->pub_encoding != NULL) {
        ...
        if (src->t1.poly != NULL) {
          if (!ossl_ml_dsa_key_pub_alloc(ret)) goto err;
      ...
      if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0) {
        if (src->priv_encoding != NULL) {
          ...
          if (src->s1.poly != NULL) {
            if (!ossl_ml_dsa_key_priv_alloc(ret)) goto err;
...
int ossl_ml_dsa_key_has(const ML_DSA_KEY* key, int selection) {
  if ((selection & OSSL_KEYMGMT_SELECT_KEYPAIR) != 0) {
    ...
    if (ossl_ml_dsa_key_get_pub(key) == NULL) return 0; /* No public key */
    if ((selection & OSSL_KEYMGMT_SELECT_PRIVATE_KEY) != 0 &&
...
static int public_from_private(const ML_DSA_KEY* key, EVP_MD_CTX* md_ctx,
  ...
  if (!matrix_expand_A(md_ctx, key->shake128_md, key->rho, &a_ntt)) goto err;
...
int ossl_ml_dsa_key_pairwise_check(const ML_DSA_KEY* key) {
  ...
  if (key->pub_encoding == NULL || key->priv_encoding == 0) return 0;

  polys = OPENSSL_malloc_array(2 * k, sizeof(*polys));
  if (polys == NULL) return 0;
  md_ctx = EVP_MD_CTX_new();
  if (md_ctx == NULL) goto err;

  vector_init(&t1, polys, k);
  vector_init(&t0, polys + k, k);
  if (!public_from_private(key, md_ctx, &t1, &t0)) goto err;
...
int ossl_ml_dsa_generate_key(ML_DSA_KEY* out) {
  ...
  if (out->seed == NULL) {
    if ((out->seed = OPENSSL_secure_malloc(seed_len)) == NULL) return 0;
