...
int
ossl_ml_dsa_set_prekey (ML_DSA_KEY *key, int flags_set, int flags_clr,
 ...
 */
int
ossl_ml_dsa_key_equal (const ML_DSA_KEY *key1, const ML_DSA_KEY *key2,
                       int selection)
{
  int key_checked = 0;
 ...
 */
static int
public_from_private (const ML_DSA_KEY *key, EVP_MD_CTX *md_ctx, VECTOR *t1,
                     VECTOR *t0)
{
  const ML_DSA_PARAMS *params = key->params;
...
ossl_ml_dsa_key_public_from_private (ML_DSA_KEY *key)
{
  ...
  ret = ((md_ctx = EVP_MD_CTX_new ()) != NULL)
        ...
        && vector_equal (
            &t0, &key->t0) /* compare the generated t0 to the expected */
