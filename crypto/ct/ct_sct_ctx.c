...
SCT_CTX*
SCT_CTX_new(OSSL_LIB_CTX* libctx, const char* propq)
{
...
ct_x509_cert_fixup(X509* cert, X509* presigner)
{
  ...
  preidx = ct_x509_get_ext(
    presigner, NID_authority_key_identifier, &pre_akid_ext_is_dup);
  certidx =
...
__owur static int
ct_public_key_hash(SCT_CTX* sctx,
                   X509_PUBKEY* pkey,
                   unsigned char** hash,
                   size_t* hash_len)
{
