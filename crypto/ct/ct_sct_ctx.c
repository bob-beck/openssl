...
__owur static int ct_x509_cert_fixup(X509* cert, X509* presigner) {
  ...
  if (preidx < -1 || certidx < -1) return 0;
  /* Invalid certificate if they contain duplicate extensions */
  if (pre_akid_ext_is_dup || cert_akid_ext_is_dup) return 0;
  /* AKID must be present in both certificate or absent in both */
  if (preidx >= 0 && certidx == -1) return 0;
  if (preidx == -1 && certidx >= 0) return 0;
  /* Copy issuer name */
  if (!X509_set_issuer_name(cert, X509_get_issuer_name(presigner))) return 0;
  if (preidx != -1) {
    ...
    /* Should never happen */
    if (preext == NULL || certext == NULL) return 0;
...
int SCT_CTX_set1_cert(SCT_CTX* sctx, X509* cert, X509* presigner) {
  ...
  if (idx >= 0) {
    ...
    if (pretmp == NULL) goto err;

    X509_EXTENSION_free(X509_delete_ext(pretmp, idx));

    if (!ct_x509_cert_fixup(pretmp, presigner)) goto err;

    prederlen = i2d_re_X509_tbs(pretmp, &preder);
    if (prederlen <= 0) goto err;
  }
...
__owur static int ct_public_key_hash(SCT_CTX* sctx, X509_PUBKEY* pkey,
  ...
  if (!EVP_Digest(der, der_len, md, &md_len, sha256, NULL)) goto err;
