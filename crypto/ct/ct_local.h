...
/*
 * Sets the public key of the issuer of the certificate that the SCT was
 * created for. The public key must not be NULL. Returns 1 on success, 0 on
 * failure.
 */
 ...
 * hold the TLS-format signature. It is the responsibility of the caller to
 * free it. If |out| points to an allocated string, the signature will be
 * written to it. The length of the signature in TLS format will be returned.
 */
 ...
 * Returns the number of bytes parsed, or a negative integer if an error
 * occurs. If an error occurs, the SCT's signature NID may be updated whilst
 * the signature field itself remains unset.
 */
