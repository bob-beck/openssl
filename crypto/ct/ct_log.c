...
int CTLOG_STORE_load_file(CTLOG_STORE* store, const char* file) {
  ...
  if (load_ctx == NULL) return 0;
  load_ctx->log_store = store;
  load_ctx->conf = NCONF_new(NULL);
  if (load_ctx->conf == NULL) goto end;
...
CTLOG* CTLOG_new_ex(EVP_PKEY* public_key, const char* name,
  ...
  if (ret->name == NULL) goto err;

  if (ct_v1_log_id_from_pkey(ret, public_key) != 1) goto err;
...
const CTLOG* CTLOG_STORE_get0_log_by_id(const CTLOG_STORE* store,
  ...
  for (i = 0; i < sk_CTLOG_num(store->logs); ++i) {
    ...
    if (memcmp(log->log_id, log_id, log_id_len) == 0) return log;
  }
