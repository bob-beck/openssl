...
static int
ctlog_new_from_conf(CTLOG_STORE* store,
                    CTLOG** ct_log,
                    const CONF* conf,
                    const char* section)
{
  ...
  return CTLOG_new_from_base64_ex(
    ct_log, pkey_base64, description, store->libctx, store->propq);
}
 ...
 */
CTLOG*
CTLOG_new_ex(EVP_PKEY* public_key,
             const char* name,
             OSSL_LIB_CTX* libctx,
             const char* propq)
{
