...
static int
ctlog_new_from_conf (CTLOG_STORE *store, CTLOG **ct_log, const CONF *conf,
                     const char *section)
{
...
ctlog_store_load_log (const char *log_name, int log_name_len, void *arg)
{
  ...
  ret = ctlog_new_from_conf (load_ctx->log_store, &ct_log, load_ctx->conf,
                             tmp);
  OPENSSL_free (tmp);
...
void
CTLOG_get0_log_id (const CTLOG *log, const uint8_t **log_id,
                   size_t *log_id_len)
{
  *log_id = log->log_id;
