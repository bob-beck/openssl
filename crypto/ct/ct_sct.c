...
int SCT_validate(SCT* sct, const CT_POLICY_EVAL_CTX* ctx) {
  ...
  if (X509_PUBKEY_set(&log_pkey, CTLOG_get0_public_key(log)) != 1) goto err;
  if (SCT_CTX_set1_pubkey(sctx, log_pkey) != 1) goto err;
  ...
  if (SCT_get_log_entry_type(sct) == CT_LOG_ENTRY_TYPE_PRECERT) {
    ...
    if (X509_PUBKEY_set(&pub, issuer_pkey) != 1) goto err;
    if (SCT_CTX_set1_issuer_pubkey(sctx, pub) != 1) goto err;
  }
...
int SCT_LIST_validate(const STACK_OF(SCT)* scts, CT_POLICY_EVAL_CTX* ctx) {
  ...
  for (i = 0; i < sct_count; ++i) {
    ...
    if (is_sct_valid < 0) return is_sct_valid;
