...
static int sct_ctx_update(EVP_MD_CTX* ctx, const SCT_CTX* sctx,
   ...
   * }
   */
  if (sct->entry_type == CT_LOG_ENTRY_TYPE_NOT_SET) return 0;
  ...
  *p++ = SIGNATURE_TYPE_CERT_TIMESTAMP;
  ...
  if (!EVP_DigestUpdate(ctx, tmpbuf, p - tmpbuf)) return 0;

  if (sct->entry_type == CT_LOG_ENTRY_TYPE_X509) {
    der = sctx->certder;
    derlen = sctx->certderlen;
  } else {
    if (!EVP_DigestUpdate(ctx, sctx->ihash, sctx->ihashlen)) return 0;
    der = sctx->preder;
  ...
  if (!EVP_DigestUpdate(ctx, tmpbuf, 3)) return 0;
  if (!EVP_DigestUpdate(ctx, der, derlen)) return 0;

  /* Add any extensions */
  p = tmpbuf;
  s2n(sct->ext_len, p);
  if (!EVP_DigestUpdate(ctx, tmpbuf, 2)) return 0;

  if (sct->ext_len && !EVP_DigestUpdate(ctx, sct->ext, sct->ext_len)) return 0;
...
int SCT_CTX_verify(const SCT_CTX* sctx, const SCT* sct) {
  ...
  if (!EVP_DigestVerifyInit_ex(ctx, NULL, "SHA2-256", sctx->libctx, sctx->propq,
                               sctx->pkey, NULL))
    goto end;

  if (!sct_ctx_update(ctx, sctx, sct)) goto end;

  /* Verify signature */
  ret = EVP_DigestVerifyFinal(ctx, sct->sig, sct->sig_len);
  /* If ret < 0 some other error: fall through without setting error */
  if (ret == 0) ERR_raise(ERR_LIB_CT, CT_R_SCT_INVALID_SIGNATURE);
