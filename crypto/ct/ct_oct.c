...
int o2i_SCT_signature(SCT* sct, const unsigned char** in, size_t len) {
  ...
  if (SCT_set1_signature(sct, p, siglen) != 1) return -1;
...
int i2o_SCT(const SCT* sct, unsigned char** out) {
  ...
  if (sct->version == SCT_VERSION_V1) {
    *p++ = sct->version;
    ...
    if (i2o_SCT_signature(sct, &p) <= 0) goto err;
  } else {
...
STACK_OF(SCT)* o2i_SCT_LIST(STACK_OF(SCT)** a, const unsigned char** pp,
  ...
  while (list_len > 0) {
    ...
    if ((sct = o2i_SCT(NULL, pp, sct_len)) == NULL) goto err;
  ...
  if (a != NULL && *a == NULL) *a = sk;
  return sk;

err:
  if (a == NULL || *a == NULL) SCT_LIST_free(sk);
  return NULL;
}
...
int i2o_SCT_LIST(const STACK_OF(SCT)* a, unsigned char** pp) {
  ...
  if (pp != NULL) {
    if (*pp == NULL) {
      ...
      if ((*pp = OPENSSL_malloc(len)) == NULL) return -1;
      is_pp_new = 1;
    }
    p = *pp + 2;
  }
  ...
  for (i = 0; i < sk_SCT_num(a); i++) {
    if (pp != NULL) {
      p2 = p;
      p += 2;
      if ((sct_len = i2o_SCT(sk_SCT_value(a, i), &p)) == -1) goto err;
      s2n(sct_len, p2);
    } else {
      if ((sct_len = i2o_SCT(sk_SCT_value(a, i), NULL)) == -1) goto err;
    }
    len2 += 2 + sct_len;
  }

  if (len2 > MAX_SCT_LIST_SIZE) goto err;

  if (pp != NULL) {
    p = *pp;
    s2n(len2 - 2, p);
    if (!is_pp_new) *pp += len2;
  }
  return (int)len2;
...
STACK_OF(SCT)* d2i_SCT_LIST(STACK_OF(SCT)** a, const unsigned char** pp,
  ...
  p = *pp;
  if (d2i_ASN1_OCTET_STRING(&oct, &p, len) == NULL) return NULL;

  p = oct->data;
  if ((sk = o2i_SCT_LIST(a, &p, oct->length)) != NULL) *pp += len;
...
int i2d_SCT_LIST(const STACK_OF(SCT)* a, unsigned char** out) {
  ...
  oct.data = NULL;
  if ((oct.length = i2o_SCT_LIST(a, &oct.data)) == -1) return -1;
