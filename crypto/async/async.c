...
/*
 * Without this we start getting longjmp crashes because it thinks we're
 * jumping up the stack when in fact we are jumping to an entirely different
 * stack. The cost of this is not having certain buffer overrun/underrun checks
 * etc for this source file :-(
 */
...
int
ASYNC_start_job (ASYNC_JOB **job, ASYNC_WAIT_CTX *wctx, int *ret,
