...
static ASYNC_JOB* async_get_pool_job(void) {
  ...
  if (pool == NULL) {
     ...
     */
    if (ASYNC_init_thread(0, 0) == 0) return NULL;
    pool = (async_pool*)CRYPTO_THREAD_get_local_ex(
        CRYPTO_THREAD_LOCAL_ASYNC_POOL_KEY, CRYPTO_THREAD_NO_CONTEXT);
  }
...
int ASYNC_start_job(ASYNC_JOB** job, ASYNC_WAIT_CTX* wctx, int* ret,
  ...
  if (!OPENSSL_init_crypto(OPENSSL_INIT_ASYNC, NULL)) return ASYNC_ERR;

  ctx = async_get_ctx();
  if (ctx == NULL) ctx = async_ctx_new();
  if (ctx == NULL) return ASYNC_ERR;

  if (*job != NULL) ctx->currjob = *job;
  ...
  for (;;) {
    ...
    /* Start a new job */
    if ((ctx->currjob = async_get_pool_job()) == NULL) return ASYNC_NO_JOBS;
...
static void async_empty_pool(async_pool* pool) {
  ...
  if (pool == NULL || pool->jobs == NULL) return;
...
int async_init(void) { return async_local_init(); }

void async_deinit(void) { async_local_deinit(); }
...
void ASYNC_cleanup_thread(void) {
  if (!OPENSSL_init_crypto(OPENSSL_INIT_ASYNC, NULL)) return;
...
ASYNC_JOB* ASYNC_get_current_job(void) {
  ...
  if (!OPENSSL_init_crypto(OPENSSL_INIT_ASYNC, NULL)) return NULL;
