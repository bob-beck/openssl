...
static void
async_delete_thread_state(void* arg);
...
async_ctx_new(void)
{
  ...
  if (!CRYPTO_THREAD_set_local_ex(
        CRYPTO_THREAD_LOCAL_ASYNC_CTX_KEY, CRYPTO_THREAD_NO_CONTEXT, nctx))
    goto err;
...
async_ctx_free(void)
{
  ...
  if (!CRYPTO_THREAD_set_local_ex(
        CRYPTO_THREAD_LOCAL_ASYNC_CTX_KEY, CRYPTO_THREAD_NO_CONTEXT, NULL))
    return 0;
...
static void
async_release_job(ASYNC_JOB* job)
{
  async_pool* pool;
  ...
  for (;;) {
    if (ctx->currjob != NULL) {
      ...
      if (ctx->currjob->status == ASYNC_JOB_PAUSED) {
        ...
        /* Resume previous job */
        if (!async_fibre_swapcontext(
              &ctx->dispatcher, &ctx->currjob->fibrectx, 1)) {
...
static void
async_empty_pool(async_pool* pool)
{
  ASYNC_JOB* job;
...
int
ASYNC_init_thread(size_t max_size, size_t init_size)
{
  async_pool* pool;
  size_t curr_size = 0;
  ...
  if (!CRYPTO_THREAD_set_local_ex(
        CRYPTO_THREAD_LOCAL_ASYNC_POOL_KEY, CRYPTO_THREAD_NO_CONTEXT, pool)) {
...
async_delete_thread_state(void* arg)
{
  ...
  if (pool != NULL) {
    ...
    CRYPTO_THREAD_set_local_ex(
      CRYPTO_THREAD_LOCAL_ASYNC_POOL_KEY, CRYPTO_THREAD_NO_CONTEXT, NULL);
  }
  async_local_cleanup();
  async_ctx_free();
}
