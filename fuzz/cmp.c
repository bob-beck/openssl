...
static int
allow_unprotected(const OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *rep, int invalid_protection, int expected_type)
{
    return 1;
}
...
process_rr(OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *rr, const X509_NAME *issuer, const ASN1_INTEGER *serial)
{
    ERR_raise(ERR_LIB_CMP, CMP_R_ERROR_PROCESSING_MESSAGE);
    return NULL;
}
...
static void process_error(OSSL_CMP_SRV_CTX           *srv_ctx,
