...
static OSSL_CMP_MSG*
transfer_cb(OSSL_CMP_CTX* ctx, const OSSL_CMP_MSG* req)
{
  if (num_responses++ > 2)
...
static void
cmp_client_process_response(OSSL_CMP_CTX* ctx, OSSL_CMP_MSG* msg)
{
  ...
  if (!OSSL_CMP_CTX_set1_secretValue(
        ctx, (unsigned char*)"", 0) /* prevent too unspecific error */
...
static int
clean_transaction(ossl_unused OSSL_CMP_SRV_CTX* srv_ctx,
...
static int
delayed_delivery(ossl_unused OSSL_CMP_SRV_CTX* srv_ctx,
...
FuzzerTestOneInput(const uint8_t* buf, size_t len)
{
  ...
  if (msg != NULL) {
    ...
    ASN1_item_print(
      out, (ASN1_VALUE*)msg, 4, ASN1_ITEM_rptr(OSSL_CMP_MSG), NULL);
    BIO_free(out);
    ...
    if (srv_ctx != NULL &&
        ...
        OSSL_CMP_SRV_CTX_init_trans(
          srv_ctx, delayed_delivery, clean_transaction))
