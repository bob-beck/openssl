...
int FuzzerTestOneInput(const uint8_t* buf, size_t len) {
  ...
  if (!PACKET_buf_init(&pkt, buf, len)) goto err;

  if ((rcidm = ossl_quic_rcidm_new(NULL)) == NULL) goto err;

  while (PACKET_remaining(&pkt) > 0) {
    if (!PACKET_get_1(&pkt, &cmd)) goto err;
    ...
    switch (cmd) {
      case CMD_RESET_WITH_ODCID:
        ...
        if ((rcidm = ossl_quic_rcidm_new(&arg_cid)) == NULL) goto err;
