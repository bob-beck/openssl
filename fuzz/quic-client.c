...
int FuzzerTestOneInput(const uint8_t* buf, size_t len) {
  ...
  client = SSL_new(ctx);
  if (client == NULL) goto end;

  fake_now = ossl_ms2time(1);
  if (!ossl_quic_set_override_now_cb(client, fake_now_cb, NULL)) goto end;

  peer_addr = BIO_ADDR_new();
  if (peer_addr == NULL) goto end;
  ...
  if (SSL_set_alpn_protos(client, (const unsigned char*)"\x08ossltest", 9) != 0)
    goto end;
  if (SSL_set1_initial_peer_addr(client, peer_addr) != 1) goto end;
  ...
  for (;;) {
    ...
    if (len >= 2) {
      if (len >= 5 && buf[0] == 0xff && buf[1] == 0xff) {
        switch (buf[2]) {
          case 0x00:
            if (state == READING) state = ACCEPTING_STREAM;
            break;
          case 0x01:
            if (state == READING) state = CREATING_STREAM;
            break;
          case 0x02:
            if (state == READING) state = SWAPPING_STREAM;
            break;
          default:
            /*ignore*/
            break;
    ...
    for (;;) {
      switch (state) {
        ...
        case CREATING_STREAM:
          state = READING;
          ret = 1;
          if (numstreams == OSSL_NELEM(allstreams)) break;
        ...
        case SWAPPING_STREAM:
          state = READING;
          ret = 1;
          if (numstreams == 1) break;
          if (++thisstream == numstreams) thisstream = 0;
          stream = allstreams[thisstream];
          break;
      }
      assert(stream != NULL);
      ...
      if (!SSL_get_event_timeout(client, &tv, &isinf)) goto end;
    ...
    if (size > 0) BIO_write(in, buf + 2, (int)size);
    len -= size + 2;
    buf += size + 2;
  }
end:
