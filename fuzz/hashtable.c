...
static int filter_iterator(HT_VALUE* v, void* arg) {
  ...
  if (f != NULL && f == &prediction_table[keyval]) return 1;
...
static void fuzz_free_cb(HT_VALUE* v) {
  ...
  if (f != NULL) f->flags &= ~FZ_FLAG_ALLOCATED;
}
...
int FuzzerInitialize(int* argc, char*** argv) {
  ...
  if (prediction_table == NULL) return -1;
  ...
  switch (OPERATION(op_flags)) {
    case OP_INSERT:
      ...
      if (valptr->flags & FZ_FLAG_ALLOCATED) {
        if (!IS_REPLACE(op_flags)) rc_prediction = 0;
      }
       ...
       */
      if (rc_prediction == 1) IS_REPLACE(op_flags) ? replacements++ : inserts++;
      break;
    ...
    case OP_DELETE:
       ...
       * of 1
       */
      if (!(valptr->flags & FZ_FLAG_ALLOCATED)) rc_prediction = 0;
    ...
    case OP_LOOKUP:
      ...
      if (!ossl_ht_read_lock(fuzzer_table)) return 0;
       ...
       * of a pointer
       */
      if (!(valptr->flags & FZ_FLAG_ALLOCATED)) valptr = NULL;
    ...
    case OP_FOREACH:
      ...
      rc_prediction = 0;
      if (valptr->flags & FZ_FLAG_ALLOCATED) rc_prediction = 1;
