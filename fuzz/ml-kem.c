...
static uint8_t* consume_uint8t(const uint8_t* buf, size_t* len, uint8_t* val) {
  if (*len < sizeof(uint8_t)) return NULL;
  *val = *buf;
  *len -= sizeof(uint8_t);
...
static void create_mlkem_raw_key(uint8_t** buf, size_t* len, void** key1,
  ...
  if (!select_keytype_and_size(buf, len, &keytype, &keylen, 0)) return;
   ...
   */
  if (!RAND_bytes(key, (int)keylen)) return;
...
again:
   ...
   */
  if (!select_keytype_and_size(buf, len, &keytype, &keylen, 1)) return;
...
static void mlkem_encap_decap(uint8_t** buf, size_t* len, void* key1, void* in2,
  ...
  if (!RAND_bytes(genkey, (int)genkey_len)) goto err;
...
int FuzzerTestOneInput(const uint8_t* buf, size_t len) {
  ...
  if (buffer_cursor == NULL) return -1;
