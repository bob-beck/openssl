...
int FuzzerTestOneInput(const uint8_t* buf, size_t len) {
  ...
  if (x509_2 == NULL) goto err;

  len = orig_len - (p - buf);
  crl = d2i_X509_CRL(NULL, &p, (long)len);
  if (crl == NULL) goto err;
  ...
  if (crl != NULL) {
    ...
    if (crls == NULL || !sk_X509_CRL_push(crls, crl)) goto err;
  ...
  if (resp != NULL) bs = OCSP_response_get1_basic(resp);
