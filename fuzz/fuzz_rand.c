...
static void* fuzz_rand_newctx(void* provctx, void* parent,
  ...
  if (st != NULL) *st = EVP_RAND_STATE_UNINITIALISED;
  return st;
}
...
static int fuzz_rand_get_ctx_params(void* vrng, OSSL_PARAM params[]) {
  ...
  if (p != NULL && !OSSL_PARAM_set_int(p, *(int*)vrng)) return 0;

  p = OSSL_PARAM_locate(params, OSSL_RAND_PARAM_STRENGTH);
  if (p != NULL && !OSSL_PARAM_set_int(p, 500)) return 0;
...
static const OSSL_PARAM* fuzz_rand_gettable_ctx_params(
