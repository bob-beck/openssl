...
util_verbose(ENGINE* e, int verbose, BIO* out, const char* indent)
{
  ...
  do {
    ...
    if (!(flags & ENGINE_CMD_FLAG_INTERNAL) || verbose >= 4) {
      ...
      if ((len = ENGINE_ctrl(
             e, ENGINE_CTRL_GET_NAME_LEN_FROM_CMD, num, NULL, NULL)) <= 0)
        goto err;
      ...
      if ((len = ENGINE_ctrl(
             e, ENGINE_CTRL_GET_DESC_LEN_FROM_CMD, num, NULL, NULL)) < 0)
        goto err;
      if (len > 0) {
...
static void
util_do_cmds(ENGINE* e,
             STACK_OF(OPENSSL_STRING)* cmds,
             BIO* out,
             const char* indent)
{
...
util_store_cap(const OSSL_STORE_LOADER* loader, void* arg)
{
  ...
  if (OSSL_STORE_LOADER_get0_engine(loader) == ctx->engine) {
    char buf[256];
    BIO_snprintf(
      buf, sizeof(buf), "STORE(%s)", OSSL_STORE_LOADER_get0_scheme(loader));
