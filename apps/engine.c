...
static int util_flags(BIO* out, unsigned int flags, const char* indent) {
  ...
  if (err) BIO_printf(out, "  <illegal flags!>");
  BIO_printf(out, "\n");
  return 1;
}
  ...
  do {
    ...
    if (!(flags & ENGINE_CMD_FLAG_INTERNAL) || verbose >= 4) {
      ...
      if (verbose == 1) {
        ...
        if ((verbose >= 3) && !util_flags(out, flags, indent)) goto err;
        xpos = 0;
      }
    }
    OPENSSL_free(name);
...
static void util_do_cmds(ENGINE* e, STACK_OF(OPENSSL_STRING)* cmds, BIO* out,
  ...
  for (loop = 0; loop < num; loop++) {
    ...
    if ((arg = strchr(cmd, ':')) == NULL) {
      if (!ENGINE_ctrl_cmd_string(e, cmd, NULL, 0)) res = 0;
    } else {
      if ((int)(arg - cmd) > 254) {
      ...
      if (!ENGINE_ctrl_cmd_string(e, buf, arg, 0)) res = 0;
    }
    if (res) {
...
static void util_store_cap(const OSSL_STORE_LOADER* loader, void* arg) {
  ...
  if (OSSL_STORE_LOADER_get0_engine(loader) == ctx->engine) {
    ...
    if (!append_buf(ctx->cap_buf, ctx->cap_size, buf)) ctx->ok = 0;
  }
}
...
int engine_main(int argc, char** argv) {
  ...
  if (engines == NULL || pre_cmds == NULL || post_cmds == NULL) goto end;

  /* Remember the original command name, parse/skip any leading engine
   * names, and then setup to parse the rest of the line as flags. */
  prog = argv[0];
  while ((argv1 = argv[1]) != NULL && *argv1 != '-') {
    if (!sk_OPENSSL_CSTRING_push(engines, argv1)) goto end;
    argc--;
    argv++;
  }
  argv[0] = prog;
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_T:
        ...
        break;
      case OPT_PRE:
        if (sk_OPENSSL_STRING_push(pre_cmds, opt_arg()) <= 0) goto end;
        break;
      case OPT_POST:
        if (sk_OPENSSL_STRING_push(post_cmds, opt_arg()) <= 0) goto end;
        break;
    }
  }
  ...
  for (; *argv; argv++) {
    ...
    if (!sk_OPENSSL_CSTRING_push(engines, *argv)) goto end;
  }

  if (sk_OPENSSL_CSTRING_num(engines) == 0) {
    for (e = ENGINE_get_first(); e != NULL; e = ENGINE_get_next(e)) {
      if (!sk_OPENSSL_CSTRING_push(engines, ENGINE_get_id(e))) goto end;
    }
  }
  ...
  for (i = 0; i < sk_OPENSSL_CSTRING_num(engines); i++) {
    ...
    if ((e = ENGINE_by_id(id)) != NULL) {
      ...
      if (list_cap) {
        ...
        if (fn_c == NULL) goto skip_ciphers;
        n = fn_c(e, NULL, &nids, 0);
        for (k = 0; k < n; ++k)
          if (!append_buf(&cap_buf, &cap_size, OBJ_nid2sn(nids[k]))) goto end;

      skip_ciphers:
        fn_d = ENGINE_get_digests(e);
        if (fn_d == NULL) goto skip_digests;
      ...
      skip_digests:
        ...
        if (fn_pk == NULL) goto skip_pmeths;
      ...
      if (test_avail) {
        ...
        if (ENGINE_init(e)) {
          ...
          if (test_avail_noise) ERR_print_errors_fp(stdout);
          ERR_clear_error();
        }
      }
      if ((verbose > 0) && !util_verbose(e, verbose, out, indent)) goto end;
      ENGINE_free(e);
    } else {
