 ...
 */
static int get_ocsp_resp_from_responder_single(SSL *s, X509 *x, tlsextstatusctx *srctx, OCSP_RESPONSE **resp)
{
...
static int bring_ocsp_resp_in_correct_order(SSL *s, tlsextstatusctx *srctx, STACK_OF(OCSP_RESPONSE) *sk_resp_unordered,
                                            STACK_OF(OCSP_RESPONSE) **sk_resp)
{
    ...
    *sk_resp = sk_OCSP_RESPONSE_new_reserve(NULL, num);
    ...
    for (i = 0; i < num; i++)
    {
        ...
        if (issuer == NULL || (cert_id = OCSP_cert_to_id(NULL, ssl_cert, issuer)) == NULL)
        {
 ...
 */
static int get_ocsp_resp_from_files(SSL *s, tlsextstatusctx *srctx, STACK_OF(OCSP_RESPONSE) **sk_resp)
{
 ...
 */
static int get_ocsp_resp_from_responder(SSL *s, tlsextstatusctx *srctx, STACK_OF(OCSP_RESPONSE) **sk_resp)
{
    X509 *ssl_cert = NULL;
    int i, num = 0;
    STACK_OF(X509) *server_certs = NULL;
    ...
    if (srctx->verbose)
    {
        ...
        for (i = 0; i < sk_OCSP_RESPONSE_num(sk_resp); i++)
        {
            ...
            if (resp != NULL)
                ...
                BIO_printf(bio_err, "cert_status: no ocsp response for certificate with index %d\n", i);
        }
    }
...
#ifndef OPENSSL_NO_OCSP
    OPT_SECTION("OCSP"),
    {"status", OPT_STATUS, '-', "Provide certificate status response if requested, for server cert only"},
    {"status_all", OPT_STATUS_ALL, '-', "Provide certificate status response(s) if requested, for the whole chain"},
...
#define IS_PROT_FLAG(o)                                                                                                \
    (o == OPT_SSL3 || o == OPT_TLS1 || o == OPT_TLS1_1 || o == OPT_TLS1_2 || o == OPT_TLS1_3 || o == OPT_DTLS ||       \
...
#ifndef OPENSSL_NO_OCSP
            s_tlsextstatus = 1;
            if (tlscstatp.sk_resp_in == NULL && (tlscstatp.sk_resp_in = sk_OPENSSL_STRING_new_null()) == NULL)
                goto end;
            ...
            *e = '\0';
            ...
            if (www == 2)
            {
                ...
                if (((i > 5) && (strcmp(&(p[i - 5]), ".html") == 0)) ||
                    ((i > 4) && (strcmp(&(p[i - 4]), ".php") == 0)) || ((i > 4) && (strcmp(&(p[i - 4]), ".htm") == 0)))
                    BIO_puts(io, "HTTP/1.0 200 ok\r\nContent-type: text/html\r\n\r\n");
                else
                    BIO_puts(io, "HTTP/1.0 200 ok\r\nContent-type: text/plain\r\n\r\n");
            }
