...
static int
psk_find_session_cb (SSL *ssl, const unsigned char *identity,
 ...
 * the OCSP certificate IDs and minimise the number of OCSP responses by
 * caching them until they were considered "expired".
 */
static int
get_ocsp_resp_from_responder_single (SSL *s, X509 *x, tlsextstatusctx *srctx,
                                     OCSP_RESPONSE **resp)
{
 ...
 */
static int
get_ocsp_resp_from_files (SSL *s, tlsextstatusctx *srctx,
                          STACK_OF (OCSP_RESPONSE) **sk_resp)
{
 ...
 */
static int
get_ocsp_resp_from_responder (SSL *s, tlsextstatusctx *srctx,
                              STACK_OF (OCSP_RESPONSE) **sk_resp)
{
  X509 *ssl_cert = NULL;
  int i, num = 0;
      ...
      for (i = 0; i < sk_OCSP_RESPONSE_num (sk_resp); i++)
        {
          ...
          if (resp != NULL)
            ...
            BIO_printf (bio_err,
                        "cert_status: no ocsp response for certificate with "
                        "index %d\n",
                        i);
        }
    }
...
static int
next_proto_cb (SSL *s, const unsigned char **data, unsigned int *len,
               void *arg)
{
...
const OPTIONS s_server_options[] = {
  ...
  { "proxy", OPT_PROXY, 's',
    "[http[s]://]host[:port][/path] of HTTP(S) proxy to use; path is "
    "ignored" },
      ...
      switch (o)
        ...
        case OPT_STATUS_FILE:
          ...
          if (tlscstatp.sk_resp_in == NULL
              && (tlscstatp.sk_resp_in = sk_OPENSSL_STRING_new_null ())
                     == NULL)
            goto end;
...
static long int
count_reads_callback (BIO *bio, int cmd, const char *argp, size_t len,
          ...
          if (www == 1 && HAS_PREFIX (buf, "GET /reneg"))
            {
              if (HAS_PREFIX (buf, "GET /renegcert"))
                SSL_set_verify (con, SSL_VERIFY_PEER | SSL_VERIFY_CLIENT_ONCE,
                                NULL);
