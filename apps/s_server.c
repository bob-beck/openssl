...
static int
not_resumable_sess_cb(SSL* s, int is_forward_secure);
static int
sv_body(int s, int stype, int prot, unsigned char* context);
static int
www_body(int s, int stype, int prot, unsigned char* context);
static int
rev_body(int s, int stype, int prot, unsigned char* context);
static void
close_accept_socket(void);
static int
init_ssl_connection(SSL* s);
static void
print_stats(BIO* bp, SSL_CTX* ctx);
static int
generate_session_id(SSL* ssl, unsigned char* id, unsigned int* id_len);
static void
init_session_cache_ctx(SSL_CTX* sctx);
static void
free_sessions(void);
static void
print_connection_info(SSL* con);
...
static long
ebcdic_ctrl(BIO* b, int cmd, long num, void* ptr);
static int
ebcdic_gets(BIO* bp, char* buf, int size);
static int
ebcdic_puts(BIO* bp, const char* str);
 ...
 */
static int
get_ocsp_resp_from_responder_single(SSL* s,
                                    X509* x,
                                    ...
                                    OCSP_RESPONSE** resp)
{
  ...
  if (id == NULL) {
    ...
    if (!X509_STORE_CTX_init(
          inctx, SSL_CTX_get_cert_store(ssl_ctx), NULL, NULL))
      goto err;
...
static int
bring_ocsp_resp_in_correct_order(SSL* s,
                                 tlsextstatusctx* srctx,
 ...
 */
static int
get_ocsp_resp_from_files(SSL* s,
                         tlsextstatusctx* srctx,
                         STACK_OF(OCSP_RESPONSE)** sk_resp)
{
  ...
  for (i = 0; i < num; i++) {
    ...
    if (derbio == NULL) {
      BIO_printf(
        bio_err, "cert_status: Cannot open OCSP response file %s\n", respfile);
      ret = SSL_TLSEXT_ERR_ALERT_FATAL;
      goto err;
    }
 ...
 */
static int
get_ocsp_resp_from_responder(SSL* s,
                             tlsextstatusctx* srctx,
                             STACK_OF(OCSP_RESPONSE)** sk_resp)
{
  X509* ssl_cert = NULL;
...
s_server_main(int argc, char* argv[])
{
    ...
    switch (o) {
      ...
      case OPT_PORT:
        ...
        if (BIO_parse_hostserv(opt_arg(), NULL, &port, BIO_PARSE_PRIO_SERV) <
            1) {
          BIO_printf(
            bio_err, "%s: -port argument malformed or ambiguous\n", port);
          goto end;
        }
        break;
      case OPT_ACCEPT:
  ...
  if (nocert == 0) {
    ...
    if (s_chain_file != NULL) {
      if (!load_certs(
            s_chain_file, 0, &s_chain, NULL, "server certificate chain"))
        goto end;
    }

    if (tlsextcbp.servername != NULL) {
      s_key2 = load_key(s_key_file2,
                        s_key_format,
                        0,
                        pass,
                        engine,
      ...
      s_cert2 = load_cert_pass(
        s_cert_file2, s_cert_format, 1, pass, "second server certificate");
  ...
  if (s_dcert_file != NULL) {
    ...
    s_dcert = load_cert_pass(
      s_dcert_file, s_dcert_format, 1, dpass, "second server certificate");
  ...
  if (!ctx_set_verify_locations(
        ctx, CAfile, noCAfile, CApath, noCApath, CAstore, noCAstore)) {
    ERR_print_errors(bio_err);
    goto end;
  }
  ...
  if (ctx2 != NULL) {
    ...
    if (!ctx_set_verify_locations(
          ctx2, CAfile, noCAfile, CApath, noCApath, CAstore, noCAstore)) {
      ERR_print_errors(bio_err);
      goto end;
    }
  ...
  if (!no_dhe) {
    ...
    if (dhfile != NULL)
      ...
      dhpkey = load_keyparams_suppress(
        s_cert_file, FORMAT_UNDEF, 0, "DH", "DH parameters", 1);
  ...
  if (srp_verifier_file != NULL) {
    if (!set_up_srp_verifier_file(
          ctx, &srp_callback_parm, srpuserseed, srp_verifier_file))
      goto end;
  } else
#endif
    if (CAfile != NULL) {
  ...
  if (enable_server_rpk)
    if (!SSL_CTX_set1_server_cert_type(
          ctx, cert_type_rpk, sizeof(cert_type_rpk))) {
      BIO_printf(bio_s_out, "Error setting server certificate types\n");
      goto end;
    }
  if (enable_client_rpk)
    if (!SSL_CTX_set1_client_cert_type(
          ctx, cert_type_rpk, sizeof(cert_type_rpk))) {
      BIO_printf(bio_s_out, "Error setting server certificate types\n");
      goto end;
    }
...
print_stats(BIO* bio, SSL_CTX* ssl_ctx)
{
  ...
  BIO_printf(
    bio, "%4ld server accepts (SSL_accept())\n", SSL_CTX_sess_accept(ssl_ctx));
  BIO_printf(bio,
  ...
  BIO_printf(
    bio, "%4ld session cache timeouts\n", SSL_CTX_sess_timeouts(ssl_ctx));
    ...
    if (read_from_terminal) {
      ...
      if (!s_quiet && !s_brief) {
        ...
        if ((buf[0] == 'K' || buf[0] == 'k') &&
            ((buf[1] == '\n') || (buf[1] == '\r'))) {
          SSL_key_update(con,
                         buf[0] == 'K' ? SSL_KEY_UPDATE_REQUESTED
  ...
  for (;;) {
    ...
    if ((www == 1 && HAS_PREFIX(buf, "GET ")) ||
      ...
      if (www == 1 && HAS_PREFIX(buf, "GET /reneg")) {
        ...
        if (i <= 0) {
          BIO_printf(
            bio_s_out, "SSL_do_handshake() Retval %d\n", SSL_get_error(con, i));
          ERR_print_errors(bio_err);
          goto err;
        }
...
static int
add_session(SSL* ssl, SSL_SESSION* session)
{
...
static SSL_SESSION*
get_session(SSL* ssl, const unsigned char* id, int idlen, int* do_copy)
{
  ...
  *do_copy = 0;
  for (sess = first; sess; sess = sess->next) {
    if (idlen == (int)sess->idlen && !memcmp(sess->id, id, idlen)) {
      ...
      return d2i_SSL_SESSION_ex(
        NULL, &p, sess->derlen, app_get0_libctx(), app_get0_propq());
    }
  }
...
static void
del_session(SSL_CTX* sctx, SSL_SESSION* session)
{
