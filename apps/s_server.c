...
static unsigned int psk_server_cb(SSL* ssl, const char* identity,
  ...
  if (s_debug) BIO_printf(bio_s_out, "fetched PSK len=%ld\n", key_len);
  return key_len;
out_err:
  if (s_debug) BIO_printf(bio_err, "Error in PSK server callback\n");
...
static int psk_find_session_cb(SSL* ssl, const unsigned char* identity,
  ...
  if (psksess != NULL) {
    if (!SSL_SESSION_up_ref(psksess)) return 0;
...
static int ebcdic_read(BIO* b, char* out, int outl) {
  ...
  if (ret > 0) ascii2ebcdic(out, out, ret);
  return ret;
}
...
static int get_ocsp_resp_from_responder_single(SSL* s, X509* x,
  ...
  if (!SSL_CTX_get0_chain_certs(ssl_ctx, &chain)) goto err;
...
static int bring_ocsp_resp_in_correct_order(
    SSL* s, tlsextstatusctx* srctx, STACK_OF(OCSP_RESPONSE)* sk_resp_unordered,
  ...
  if (*sk_resp != NULL) sk_OCSP_RESPONSE_pop_free(*sk_resp, OCSP_RESPONSE_free);
   ...
   */
  if (ssl_cert == NULL) return SSL_TLSEXT_ERR_OK;

  /* reserve enough space so the pushes to the stack would not fail */
  *sk_resp = sk_OCSP_RESPONSE_new_reserve(NULL, num);

  if (*sk_resp == NULL) return SSL_TLSEXT_ERR_ALERT_FATAL;

  for (i = 0; i < num; i++) {
    if (i != 0) /* for each certificate in chain (except root) get the OCSP
                   response */
    ...
    for (j = 0; j < sk_OCSP_RESPONSE_num(sk_resp_unordered); j++) {
      if ((resp = sk_OCSP_RESPONSE_value(sk_resp_unordered, j)) == NULL)
        continue;
      if ((bs = OCSP_response_get1_basic(resp)) == NULL) continue;
      found = OCSP_resp_find(bs, cert_id, -1);
      OCSP_BASICRESP_free(bs);
      if (found > -1) {
        /* remove the found OCSP response to prevent freeing it with the
         * remaining list */
...
static int get_ocsp_resp_from_responder(SSL* s, tlsextstatusctx* srctx,
  ...
  *sk_resp = sk_OCSP_RESPONSE_new_reserve(NULL, num);
  ...
  for (i = 0; i < num; i++) {
    ...
    resp = NULL;
    if (get_ocsp_resp_from_responder_single(s, ssl_cert, srctx, &resp) !=
        SSL_TLSEXT_ERR_OK)
      resp = NULL;
...
static int cert_status_cb(SSL* s, void* arg) {
  ...
  if (sk_resp == NULL || sk_OCSP_RESPONSE_num(sk_resp) <= 0) {
    if (srctx->sk_resp_in != NULL) {
      ...
      if (ret != SSL_TLSEXT_ERR_OK) goto err;
    }
...
const OPTIONS s_server_options[] = {
    ...
    {"status_all", OPT_STATUS_ALL, '-',
     "Provide certificate status response(s) if requested, for the whole "
     "chain"},
    ...
    {"status_file", OPT_STATUS_FILE, '<',
     "File containing DER encoded OCSP Response (can be specified multiple "
     "times)"},
#endif
    ...
    {"http_server_binmode", OPT_HTTP_SERVER_BINMODE, '-',
     "opening files in binary mode when acting as http server (-WWW and "
     "-HTTP)"},
...
int s_server_main(int argc, char* argv[]) {
  ...
  if (port == NULL || cctx == NULL || vpm == NULL) goto end;
  ...
  while ((o = opt_next()) != OPT_EOF) {
    ...
    if (IS_NO_PROT_FLAG(o)) no_prot_opt++;
    ...
    switch (o) {
      ...
      case OPT_CERT:
        ...
        break;
      case OPT_NAMEOPT:
        if (!set_nameopt(opt_arg())) goto end;
        break;
      case OPT_CRL:
        crl_file = opt_arg();
        break;
      ...
      case OPT_SERVERINFO:
        ...
        break;
      case OPT_CERTFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &s_cert_format)) goto opthelp;
        break;
      case OPT_KEY:
        s_key_file = opt_arg();
        break;
      case OPT_KEYFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &s_key_format)) goto opthelp;
        break;
      case OPT_PASS:
        passarg = opt_arg();
        break;
      ...
      case OPT_DHPARAM:
        ...
        break;
      case OPT_DCERTFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &s_dcert_format)) goto opthelp;
        break;
      case OPT_DCERT:
        s_dcert_file = opt_arg();
        break;
      case OPT_DKEYFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &s_dkey_format)) goto opthelp;
        break;
      case OPT_DPASS:
        dpassarg = opt_arg();
        break;
      ...
      case OPT_EXT_CACHE:
        ...
        break;
      case OPT_CRLFORM:
        if (!opt_format(opt_arg(), OPT_FMT_PEMDER, &crl_format)) goto opthelp;
        break;
      case OPT_S_CASES:
      case OPT_S_NUM_TICKETS:
      case OPT_ANTI_REPLAY:
      case OPT_NO_ANTI_REPLAY:
        if (ssl_args == NULL) ssl_args = sk_OPENSSL_STRING_new_null();
        ...
        break;
      case OPT_V_CASES:
        if (!opt_verify(o, vpm)) goto end;
        vpmtouched++;
        break;
      case OPT_X_CASES:
        if (!args_excert(o, &exc)) goto end;
        break;
      ...
      case OPT_SRPVFILE:
        ...
        if (min_version < TLS1_VERSION) min_version = TLS1_VERSION;
#endif
        break;
      ...
      case OPT_ENGINE:
        ...
        break;
      case OPT_R_CASES:
        if (!opt_rand(o)) goto end;
        break;
      case OPT_PROV_CASES:
        if (!opt_provider(o)) goto end;
        break;
      case OPT_SERVERNAME:
      ...
      case OPT_EARLY_DATA:
        early_data = 1;
        if (max_early_data == -1) max_early_data = SSL3_RT_MAX_PLAIN_LENGTH;
        break;
  ...
  if (use_zc_sendfile && !use_sendfile) {
    BIO_printf(bio_out,
               "Warning: -zerocopy_sendfile depends on -sendfile, enabling "
               "-sendfile now.\n");
    use_sendfile = 1;
  }
  ...
  if (s_key_file2 == NULL) s_key_file2 = s_cert_file2;

  if (!load_excert(&exc)) goto end;

  if (nocert == 0) {
    s_key = load_key(s_key_file, s_key_format, 0, pass, engine,
                     "server certificate private key");
    if (s_key == NULL) goto end;

    s_cert = load_cert_pass(s_cert_file, s_cert_format, 1, pass,
                            "server certificate");

    if (s_cert == NULL) goto end;
  ...
  if (alpn_in) {
    ...
    if (alpn_ctx.data == NULL) goto end;
  }
  ...
  if (sdebug) ssl_ctx_security_debug(ctx, sdebug);

  if (!config_ctx(cctx, ssl_args, ctx)) goto end;
  ...
  if (session_id_prefix) {
    if (strlen(session_id_prefix) >= 32)
      BIO_printf(bio_err,
                 "warning: id_prefix is too long, only one new session will be "
                 "possible\n");
  ...
  if (exc != NULL) ssl_ctx_set_excert(ctx, exc);
...
#ifndef OPENSSL_NO_KTLS
  if (enable_ktls) SSL_CTX_set_options(ctx, SSL_OP_ENABLE_KTLS);
  if (use_zc_sendfile)
  ...
  if (ctx2 != NULL) {
    ...
    if (sdebug) ssl_ctx_security_debug(ctx2, sdebug);

    if (session_id_prefix) {
      if (strlen(session_id_prefix) >= 32)
        BIO_printf(bio_err,
                   "warning: id_prefix is too long, only one new session will "
                   "be possible\n");
    ...
    if (exc != NULL) ssl_ctx_set_excert(ctx2, exc);
    ...
    if (async) SSL_CTX_set_mode(ctx2, SSL_MODE_ASYNC);
    ...
    if (!config_ctx(cctx, ssl_args, ctx2)) goto end;
  }
#ifndef OPENSSL_NO_NEXTPROTONEG
  if (next_proto.data)
    SSL_CTX_set_next_protos_advertised_cb(ctx, next_proto_cb, &next_proto);
#endif
  if (alpn_ctx.data) SSL_CTX_set_alpn_select_cb(ctx, alpn_cb, &alpn_ctx);
  ...
  if (!set_cert_key_stuff(ctx, s_cert, s_key, s_chain, build_chain)) goto end;
  ...
  if (srp_verifier_file != NULL) {
    ...
    if (ctx2) SSL_CTX_set_client_CA_list(ctx2, SSL_load_client_CA_file(CAfile));
  }
#ifndef OPENSSL_NO_OCSP
  if (s_tlsextstatus) {
...
#endif
  if (set_keylog_file(ctx, keylog_file)) goto end;

  if (max_early_data >= 0) SSL_CTX_set_max_early_data(ctx, max_early_data);
  if (recv_max_early_data >= 0)
  ...
  if (rev)
    ...
    server_cb = sv_body;
#ifdef AF_UNIX
  if (socket_family == AF_UNIX && unlink_unix_path) unlink(host);
#endif
  if (tfo) BIO_printf(bio_s_out, "Listening for TFO\n");
...
static int sv_body(int s, int stype, int prot, unsigned char* context) {
  ...
  if (early_data) {
    ...
    if (SSL_is_init_finished(con)) print_connection_info(con);
  }
  ...
  for (;;) {
    ...
    if (!read_from_sslcon) {
      ...
      if (has_stdin_waiting()) read_from_terminal = 1;
      if ((i < 0) || (!i && !read_from_terminal)) continue;
#else
      ...
      if (i <= 0) continue;
      if (FD_ISSET(fileno_stdin(), &readfds)) read_from_terminal = 1;
#endif
    ...
    if (read_from_terminal) {
      ...
      if (!s_quiet && !s_brief) {
        ...
        if ((i <= 0) || (buf[0] == 'q')) {
          BIO_printf(bio_s_out, "DONE\n");
          (void)BIO_flush(bio_s_out);
          if (SSL_version(con) != DTLS1_VERSION) BIO_closesocket(s);
          /*
      ...
      again:
        ...
        switch (SSL_get_error(con, i)) {
          case SSL_ERROR_NONE:
            ...
            (void)BIO_flush(bio_s_out);
            if (SSL_has_pending(con)) goto again;
            break;
          case SSL_ERROR_WANT_ASYNC:
...
static int init_ssl_connection(SSL* con) {
  ...
  if (dtlslisten || stateless) {
    ...
    do {
      ...
      if (i <= 0) retry = is_retryable(con, i);
...
static void print_connection_info(SSL* con) {
  ...
  if (s_brief) print_ssl_summary(con);
...
#endif
  if (SSL_session_reused(con)) BIO_printf(bio_s_out, "Reused session-id\n");
...
static int www_body(int s, int stype, int prot, unsigned char* context) {
  ...
  if ((io == NULL) || (ssl_bio == NULL) || (edio == NULL)) goto err;
  ...
  if (BIO_set_write_buffer_size(io, bufsize) <= 0) goto err;
  ...
  if (early_data) {
    ...
    while (edret != SSL_READ_EARLY_DATA_FINISH) {
      ...
      if (readbytes > 0) BIO_write(edio, buf, (int)readbytes);
    }
  }
    ...
    if ((www == 1 && HAS_PREFIX(buf, "GET ")) ||
      ...
      *e = '\0';
      ...
      if (use_sendfile_for_req) {
        ...
        filesize = st.st_size;
        if (((int)BIO_flush(io)) < 0) goto write_error;
        ...
        for (;;) {
          ...
          if (filesize <= 0) {
            if (!s_quiet) BIO_printf(bio_err, "KTLS SENDFILE '%s' OK\n", p);
...
#ifdef RENEG
            static count = 0;
            if (++count == 13) SSL_renegotiate(con);
#endif
  ...
  for (;;) {
    ...
    if (i < 0) { /* error */
      ...
      for (;;) {
        i = BIO_flush(io);
        if (i > 0) break;
        if (!BIO_should_retry(io)) goto end;
      }
    }
  }
end:
...
static int generate_session_id(SSL* ssl, unsigned char* id,
  ...
  do {
    if (RAND_bytes(id, *id_len) <= 0) return 0;
    /*
