...
int ec_main(int argc, char** argv) {
  ...
  if (!opt_cipher(ciphername, &enc)) goto opthelp;
  ...
  release_engine(e);
  if (passin != NULL) OPENSSL_clear_free(passin, strlen(passin));
  if (passout != NULL) OPENSSL_clear_free(passout, strlen(passout));
  return ret;
}
