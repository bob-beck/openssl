...
static int add_entry(enum Type type, unsigned int hash, const char* filename,
  ...
  for (bp = hash_table[ndx]; bp; bp = bp->next)
    if (bp->type == type && bp->hash == hash) break;
  if (bp == NULL) {
  ...
  if (ep == NULL) {
    ...
    *ep = nilhentry;
    ...
    if (bp->last_entry) bp->last_entry->next = ep;
    if (bp->first_entry == NULL) bp->first_entry = ep;
    bp->last_entry = ep;
  }

  if (old_id < ep->old_id) ep->old_id = old_id;
...
static int handle_symlink(const char* filename, const char* fullpath) {
  ...
  if (n < 0 || n >= (int)sizeof(linktarget)) return -1;
  linktarget[n] = 0;
...
static int do_file(const char* filename, const char* fullpath, enum Hash h) {
  ...
  if ((ext = strrchr(filename, '.')) == NULL) goto end;
  for (i = 0; i < OSSL_NELEM(extensions); i++) {
    if (OPENSSL_strcasecmp(extensions[i], ext + 1) == 0) break;
  }
  if (i >= OSSL_NELEM(extensions)) goto end;
...
static int do_dir(const char* dirname, enum Hash h) {
  ...
  if (verbose) BIO_printf(bio_out, "Doing %s\n", dirname);
  ...
  while ((filename = OPENSSL_DIR_read(&d, dirname)) != NULL) {
    ...
    if (fname_len > fname_max_len) fname_max_len = fname_len;
  }
  OPENSSL_DIR_end(&d);
  ...
  for (n = 0; n < numfiles; ++n) {
    ...
    if (S_ISLNK(st.st_mode) && handle_symlink(filename, buf) == 0) continue;
  ...
  for (i = 0; i < OSSL_NELEM(hash_table); i++) {
    for (bp = hash_table[i]; bp; bp = nextbp) {
      ...
      for (ep = bp->first_entry; ep; ep = ep->next)
        if (ep->old_id < bp->num_needed) bit_set(idmask, ep->old_id);
      ...
      for (ep = bp->first_entry; ep; ep = nextep) {
        ...
        if (ep->old_id < bp->num_needed) {
          ...
          if (verbose) BIO_printf(bio_out, "unlink %s\n", &buf[dirlen]);
          if (unlink(buf) < 0 && errno != ENOENT) {
            BIO_printf(bio_err, "%s: Can't unlink %s, %s\n", opt_getprog(), buf,
                       strerror(errno));
            errs++;
...
int rehash_main(int argc, char** argv) {
  ...
  if (evpmdsize <= 0 || evpmdsize > EVP_MAX_MD_SIZE) goto end;
