...
verify_main(int argc, char** argv)
{
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_UNTRUSTED:
        /* Zero or more times */
        if (!load_certs(
              opt_arg(), 0, &untrusted, NULL, "untrusted certificates"))
          goto end;
        break;
      case OPT_TRUSTED:
        /* Zero or more times */
  ...
  if (argc < 1) {
    ...
    for (i = 0; i < argc; i++)
      if (check(
            store, argv[i], untrusted, trusted, crls, show_chain, vfyopts) != 1)
        ret = -1;
  }
    ...
    if (show_chain) {
      ...
      for (j = 0; j < sk_X509_num(chain); j++) {
        ...
        X509_NAME_print_ex_fp(
          stdout, X509_get_subject_name(cert), 0, get_nameopt());
        if (j < num_untrusted)
...
cb(int ok, X509_STORE_CTX* ctx)
{
  ...
  if (!ok) {
    if (current_cert != NULL) {
      X509_NAME_print_ex(
        bio_err, X509_get_subject_name(current_cert), 0, get_nameopt());
      BIO_printf(bio_err, "\n");
    }
    BIO_printf(bio_err,
