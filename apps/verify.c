...
int verify_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_CRLFILE:
        /* Zero or more times */
        if (!load_crls(opt_arg(), &crls, NULL, "other CRLs")) goto end;
        break;
  ...
  if (crl_download) store_setup_crl_download(store);
...
static int check(X509_STORE* ctx, const char* file, STACK_OF(X509)* uchain,
  ...
  if (tchain != NULL) X509_STORE_CTX_set0_trusted_stack(csc, tchain);
  if (crls != NULL) X509_STORE_CTX_set0_crls(csc, crls);
    ...
    if (show_chain) {
      ...
      for (j = 0; j < sk_X509_num(chain); j++) {
        ...
        X509_NAME_print_ex_fp(stdout, X509_get_subject_name(cert), 0,
                              get_nameopt());
        if (j < num_untrusted) BIO_printf(bio_out, " (untrusted)");
        BIO_printf(bio_out, "\n");
      }
...
static int cb(int ok, X509_STORE_CTX* ctx) {
  ...
  if (cert_error == X509_V_OK && ok == 2) policies_print(ctx);
