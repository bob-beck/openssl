...
static X509_REQ* x509_to_req(X509* cert, int ext_copy, const char* names) {
  ...
  if ((exts = sk_X509_EXTENSION_new_reserve(NULL, n)) == NULL) goto err;
  for (i = 0; i < n; i++) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_IN:
        ...
        break;
      case OPT_OUTFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &outformat)) goto opthelp;
        break;
      case OPT_KEYFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &keyformat)) goto opthelp;
        break;
      case OPT_CAFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &CAformat)) goto opthelp;
        break;
      case OPT_CAKEYFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &CAkeyformat)) goto opthelp;
        break;
      case OPT_OUT:
        outfile = opt_arg();
        break;
      ...
      case OPT_SET_SERIAL:
        ...
        if ((sno = s2i_ASN1_INTEGER(NULL, opt_arg())) == NULL) goto opthelp;
        break;
      case OPT_NEW:
        newcert = 1;
        break;
      ...
      case OPT_ADDTRUST:
        ...
        if (!sk_ASN1_OBJECT_push(trust, objtmp)) goto err;
        trustout = 1;
        break;
      case OPT_ADDREJECT:
        ...
        if (!sk_ASN1_OBJECT_push(reject, objtmp)) goto err;
        trustout = 1;
        break;
      case OPT_SETALIAS:
        alias = opt_arg();
        ...
        break;
      case OPT_CERTOPT:
        if (!set_cert_ex(&certflag, opt_arg())) goto opthelp;
        break;
      case OPT_NAMEOPT:
      ...
      case OPT_CHECKEND:
        checkend = 1;
        {
          ...
          if (!opt_intmax(opt_arg(), &temp)) goto opthelp;
...
#define WARN_NO_CA(opt)                        \
  BIO_printf(bio_err, "Warning: ignoring " opt \
    ...
    if (CAkeyfile != NULL) WARN_NO_CA("-CAkey");
    if (CAkeyformat != FORMAT_UNDEF) WARN_NO_CA("-CAkeyform");
    if (CAformat != FORMAT_UNDEF) WARN_NO_CA("-CAform");
    if (CAserial != NULL) WARN_NO_CA("-CAserial");
    if (CA_createserial) WARN_NO_CA("-CAcreateserial");
  }
  ...
  if (extfile == NULL) {
    ...
    if ((extconf = app_load_config(extfile)) == NULL) goto err;
    if (extsect == NULL) {
      extsect = app_conf_try_string(extconf, "default", "extensions");
      if (extsect == NULL) extsect = "default";
    }
  ...
  if (reqfile) {
    if (infile == NULL && isatty(fileno_stdin()))
      BIO_printf(bio_err,
                 "Warning: Reading cert request from stdin since no -in option "
                 "is given\n");
    ...
    BIO_printf(bio_err,
               "Warning: ignoring -copy_extensions since neither -x509toreq "
               "nor -req is given\n");
  }
  ...
  if (reqfile || newcert) {
    ...
    if (privkeyfile == NULL && CAkeyfile == NULL) {
      BIO_printf(bio_err,
                 "We need a private key to sign with, use -key or -CAkey or "
                 "-CA with private key\n");
      goto err;
    }
    ...
    if (CAfile == NULL && sno == NULL) {
      ...
      if (sno == NULL || !rand_serial(NULL, sno)) goto err;
    }
    ...
    if (infile == NULL && isatty(fileno_stdin()))
      BIO_printf(bio_err,
                 "Warning: Reading certificate(s) from stdin since no -in or "
                 "-new option is given\n");
    if (multi) {
      certs = sk_X509_new_null();
      if (certs == NULL) goto err;
      if (!load_certs(infile, 1, &certs, passin, NULL)) goto err;
      if (sk_X509_num(certs) <= 0) goto err;
    } else {
...
cert_loop:
  if (multi) x = sk_X509_value(certs, k);
  ...
  if (CAfile != NULL) {
    issuer_cert = xca;
    if (sno == NULL) sno = x509_load_serial(CAfile, CAserial, CA_createserial);
    if (sno == NULL) goto err;
    if (!x509toreq && !reqfile && !newcert && !self_signed(ctx, x)) goto err;
  } else {
  ...
  if (CAfile == NULL) {
    if (!X509V3_set_issuer_pkey(&ext_ctx, privkey)) goto err;
  }
  ...
  if (x509toreq) { /* also works in conjunction with -req */
    ...
    if ((rq = x509_to_req(x, ext_copy, ext_names)) == NULL) goto err;
    if (extconf != NULL) {
      X509V3_set_nconf(&ext_ctx, extconf);
    ...
    if (!do_X509_REQ_sign(rq, privkey, digest, sigopts)) goto err;
    if (!noout) {
    ...
    if (!do_X509_sign(x, 0, CAkey, digest, sigopts, &ext_ctx)) goto err;
  } else if (privkey != NULL) {
    if (!do_X509_sign(x, 0, privkey, digest, sigopts, &ext_ctx)) goto err;
  }
  if (badsig) {
  ...
  for (i = 1; i <= num; i++) {
    if (i == issuer) {
      ...
      if (fdigname == NULL) fdigname = "SHA1";
...
end_cert_loop:
  if (multi && ++k < sk_X509_num(certs)) goto cert_loop;
...
static int callb(int ok, X509_STORE_CTX* ctx) {
  ...
  if (err == X509_V_ERR_DEPTH_ZERO_SELF_SIGNED_CERT) return 1;
...
static int print_x509v3_exts(BIO* bio, X509* x, const char* ext_names) {
  ...
  if ((tmp_ext_names = OPENSSL_strdup(ext_names)) == NULL) goto end;
  if ((nn = parse_ext_names(tmp_ext_names, NULL)) == 0) {
    BIO_printf(bio, "Invalid extension names: %s\n", ext_names);
    goto end;
  }
  if ((names = OPENSSL_malloc_array(nn, sizeof(char*))) == NULL) goto end;
  ...
  for (i = 0; i < num; i++) {
    ...
    if (sn == NULL || strcmp(sn, "UNDEF") == 0) continue;
    ...
    for (j = 0; j < nn; j++) {
      if (strcmp(sn, names[j]) == 0) {
        ...
        if (exts2 == NULL && (exts2 = sk_X509_EXTENSION_new_null()) == NULL)
          goto end;
        if (!sk_X509_EXTENSION_push(exts2, ext)) goto end;
      }
    }
  }
