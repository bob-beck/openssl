...
x509_main(int argc, char** argv)
{
    ...
    switch (o) {
      ...
      case OPT_ADDTRUST:
        ...
        if ((objtmp = OBJ_txt2obj(opt_arg(), 0)) == NULL) {
          BIO_printf(
            bio_err, "%s: Invalid trust object value %s\n", prog, opt_arg());
          goto opthelp;
        }
      ...
      case OPT_ADDREJECT:
        ...
        if ((objtmp = OBJ_txt2obj(opt_arg(), 0)) == NULL) {
          BIO_printf(
            bio_err, "%s: Invalid reject object value %s\n", prog, opt_arg());
          goto opthelp;
        }
  ...
  if (!X509_STORE_set_default_paths_ex(
        ctx, app_get0_libctx(), app_get0_propq()))
    goto err;
  ...
  if (pubkeyfile != NULL) {
    if ((pubkey = load_pubkey(
           pubkeyfile, keyformat, 0, NULL, e, "explicitly set public key")) ==
        NULL)
      goto err;
  }
...
cert_loop:
  ...
  if (x509toreq) { /* also works in conjunction with -req */
    ...
    if ((CAkey = load_key(
           CAkeyfile, CAkeyformat, 0, passin, e, "CA private key")) == NULL)
      goto err;
  ...
  for (i = 1; i <= num; i++) {
    if (i == issuer) {
      ...
      if ((fdig = EVP_MD_fetch(
             app_get0_libctx(), fdigname, app_get0_propq())) == NULL) {
