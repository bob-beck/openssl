...
static void
warn_copying (ASN1_OBJECT *excluded, const char *names)
{
  const char *sn = OBJ_nid2sn (OBJ_obj2nid (excluded));

  if (names != NULL && strstr (names, sn) != NULL)
    BIO_printf (bio_err,
                "Warning: -ext should not specify copying %s extension to "
                "CSR; ignoring this\n",
                sn);
}

static X509_REQ *
x509_to_req (X509 *cert, int ext_copy, const char *names)
{
...
static int
self_signed (X509_STORE *ctx, X509 *cert)
{
  ...
  if (reqfile)
    {
      if (infile == NULL && isatty (fileno_stdin ()))
        BIO_printf (bio_err, "Warning: Reading cert request from stdin since "
                             "no -in option is given\n");
      ...
      if (i <= 0)
        {
          BIO_printf (bio_err, i < 0 ? "Error while verifying certificate "
                                       "request self-signature\n"
                                     : "Certificate request self-signature "
                                       "did not match the contents\n");
          goto err;
        }
  ...
  if (reqfile || newcert)
    {
      ...
      if (privkeyfile == NULL && CAkeyfile == NULL)
        {
          BIO_printf (bio_err, "We need a private key to sign with, use -key "
                               "or -CAkey or -CA with private key\n");
          goto err;
        }
    ...
    {
      if (infile == NULL && isatty (fileno_stdin ()))
        BIO_printf (bio_err, "Warning: Reading certificate(s) from stdin "
                             "since no -in or -new option is given\n");
      if (multi)
        {
...
cert_loop:
  ...
  for (i = X509_get_ext_count (x) - 1; i >= 0; i--)
    {
      ...
      const char *sn
          = OBJ_nid2sn (OBJ_obj2nid (X509_EXTENSION_get_object (ex)));
  ...
  if (x509toreq)
    { /* also works in conjunction with -req */
      if (privkey == NULL)
        {
          BIO_printf (bio_err,
  ...
  for (i = 1; i <= num; i++)
    {
      if (i == issuer)
      ...
      else if (i == next_serial)
        {
          ASN1_INTEGER *ser;
          BIGNUM *bnser
              = ASN1_INTEGER_to_BN (X509_get0_serialNumber (x), NULL);
...
static ASN1_INTEGER *
x509_load_serial (const char *CAfile, const char *serialfile, int create)
{
  char *buf = NULL;
...
static int
parse_ext_names (char *names, const char **result)
{
  char *p, *q;
  int cnt = 0, len = 0;
...
static int
print_x509v3_exts (BIO *bio, X509 *x, const char *ext_names)
{
