...
static char* alloc_mac_algorithm_name(STACK_OF(OPENSSL_STRING)** optp,
  ...
  if (*optp == NULL) *optp = sk_OPENSSL_STRING_new_null();
  if (*optp == NULL) return NULL;
...
int mac_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_CIPHER:
        ...
        if (cipher == NULL) goto opthelp;
        break;
      case OPT_DIGEST:
        OPENSSL_free(digest);
        digest = alloc_mac_algorithm_name(&opts, "digest", opt_arg());
        if (digest == NULL) goto opthelp;
        break;
      case OPT_PROV_CASES:
        if (!opt_provider(o)) goto err;
        break;
    }
  }

  /* One argument, the MAC name. */
  if (!opt_check_rest_arg("MAC name")) goto opthelp;
  argv = opt_rest();
  ...
  if (out_bin) {
    ...
    if (outfile == NULL) BIO_printf(out, "\n");
  }
