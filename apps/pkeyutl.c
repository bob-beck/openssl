...
int pkeyutl_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_PASSIN:
        ...
        break;
      case OPT_PEERFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &peerform)) goto opthelp;
        break;
      case OPT_KEYFORM:
  ...
  if (pkeyopts_passin != NULL) {
    ...
    for (i = 0; i < num; i++) {
      ...
      if (passin == NULL) {
        ...
        r = EVP_read_pw_string(passwd_buf, sizeof(passwd_buf) - 1, passwd_buf,
                               0);
        if (r < 0) {
          if (r == -2)
  ...
  if (pkey_op == EVP_PKEY_OP_DECAPSULATE && outfile != NULL) {
    ...
    if ((out = bio_open_owner(outfile, 'w', FORMAT_BINARY)) == NULL) goto end;
  } else {
  ...
  if (rawin) {
    ...
    if (kdflen != 0) {
      ...
      rv = do_keyop(ctx, pkey_op, NULL, &buf_outlen, buf_in, (size_t)buf_inlen,
                    NULL, &secretlen);
    }
    if (rv > 0 &&
      ...
      if (buf_outlen > 0) buf_out = app_malloc(buf_outlen, "buffer output");
      if (secretlen > 0) secret = app_malloc(secretlen, "secret output");
  ...
  if (secretlen > 0) BIO_write(secout ? secout : out, secret, (int)secretlen);
  ...
  if (rawin) {
    ...
    switch (pkey_op) {
      ...
      case EVP_PKEY_OP_ENCAPSULATE:
        ...
        if (rv > 0 && kemop != NULL) rv = EVP_PKEY_CTX_set_kem_op(ctx, kemop);
        break;
...
static int setup_peer(EVP_PKEY_CTX* ctx, int peerform, const char* file,
  ...
  if (peerform == FORMAT_ENGINE) engine = e;
