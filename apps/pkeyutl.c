...
static EVP_PKEY *get_pkey (const char *kdfalg, const char *keyfile,
                           int keyform, int key_type, char *passinarg,
...
static int do_raw_keyop (int pkey_op, EVP_MD_CTX *mctx, EVP_PKEY *pkey,
                         BIO *in, int filesize, unsigned char *sig, int siglen,
                         unsigned char **out, size_t *poutlen);

static int
only_nomd (EVP_PKEY *pkey)
{
...
const OPTIONS pkeyutl_options[] = {
  ...
  { "asn1parse", OPT_ASN1PARSE, '-',
    "parse the output as ASN.1 data to check its DER encoding and print "
    "errors" },
  ...
  { "digest", OPT_DIGEST, 's',
    "The digest algorithm to use for signing/verifying raw input data. "
    "Implies -rawin" },
  ...
  if (!rawin && (pkey_op == EVP_PKEY_OP_SIGN || pkey_op == EVP_PKEY_OP_VERIFY))
    {
      if (buf_inlen > EVP_MAX_MD_SIZE)
        {
          BIO_printf (bio_err,
                      "Error: The non-raw input data length %d is too long - "
  ...
  else
    {
      ...
      if (rv > 0
          ...
          && (buf_outlen > 0 || pkey_op == EVP_PKEY_OP_DECAPSULATE))
        {
          ...
          rv = do_keyop (ctx, pkey_op, buf_out, &buf_outlen, buf_in,
                         (size_t)buf_inlen, secret, &secretlen);
        }
    }
  if (rv <= 0)
    {
...
static EVP_PKEY *
get_pkey (const char *kdfalg, const char *keyfile, int keyform, int key_type,
...
static EVP_PKEY_CTX *
init_ctx (const char *kdfalg, int *pkeysize, int pkey_op, ENGINE *e,
          const int engine_impl, int rawin,
          EVP_PKEY *pkey /* ownership is passed to ctx */, EVP_MD_CTX *mctx,
          const char *digestname, const char *kemop, OSSL_LIB_CTX *libctx,
          const char *propq)
{
  EVP_PKEY_CTX *ctx = NULL;
  ENGINE *impl = NULL;
...
static int
setup_peer (EVP_PKEY_CTX *ctx, int peerform, const char *file, ENGINE *e)
{
  ...
  if (strcmp (EVP_PKEY_get0_type_name (peer), EVP_PKEY_get0_type_name (pkey))
    ...
    {
      BIO_printf (bio_err,
                  "Type of peer public key: %s does not match type of private "
                  "key: %s\n",
...
static int
do_keyop (EVP_PKEY_CTX *ctx, int pkey_op, unsigned char *out, size_t *poutlen,
          const unsigned char *in, size_t inlen, unsigned char *secret,
          size_t *pseclen)
{
  int rv = 0;
...
static int
do_raw_keyop (int pkey_op, EVP_MD_CTX *mctx, EVP_PKEY *pkey, BIO *in,
