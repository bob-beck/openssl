...
pkeyutl_main(int argc, char** argv)
{
  ...
  if (kdfalg != NULL) {
    if (kdflen == 0) {
      BIO_printf(
        bio_err, "%s: no KDF length given (-kdflen parameter).\n", prog);
      goto opthelp;
    }
  } else if (inkey == NULL) {
    ...
    BIO_printf(
      bio_err, "%s: -peerkey option not allowed without -derive.\n", prog);
    goto opthelp;
  } else if (peerkey == NULL && pkey_op == EVP_PKEY_OP_DERIVE) {
    BIO_printf(
  ...
  if (rawin) {
    ...
    rv = do_raw_keyop(
      pkey_op, mctx, pkey, in, filesize, NULL, 0, &buf_out, &buf_outlen);
  } else {
    if (kdflen != 0) {
      buf_outlen = kdflen;
      rv = 1;
  ...
  if (rawin) {
    ...
    switch (pkey_op) {
      case EVP_PKEY_OP_SIGN:
        rv = EVP_DigestSignInit_ex(
          mctx, NULL, digestname, libctx, propq, pkey, NULL);
        break;

      case EVP_PKEY_OP_VERIFY:
        rv = EVP_DigestVerifyInit_ex(
          mctx, NULL, digestname, libctx, propq, pkey, NULL);
        break;
    }
