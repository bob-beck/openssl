...
int passwd_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_HELP:
        ...
        goto end;
      case OPT_IN:
        if (pw_source_defined) goto opthelp;
        infile = opt_arg();
        pw_source_defined = 1;
        break;
      case OPT_NOVERIFY:
      ...
      case OPT_REVERSE:
        ...
        break;
      case OPT_1:
        if (mode != passwd_unset) goto opthelp;
        mode = passwd_md5;
        break;
      case OPT_5:
        if (mode != passwd_unset) goto opthelp;
  ...
  if ((in == NULL) && (passwds == NULL)) {
    ...
    if (1) {
      ...
      if (in == NULL) {
        if (EVP_read_pw_string(passwd_malloc, (int)passwd_malloc_size,
...
static char* md5crypt(const char* passwd, const char* magic, const char* salt) {
  ...
  if (ascii_passwd == NULL) return NULL;
  ...
  salt_out = out_buf;
  if (magic_len > 0) salt_out += 2 + magic_len;
  ...
  if (!EVP_DigestUpdate(md, ascii_salt, salt_len)) goto err;
  ...
  for (i = (unsigned int)passwd_len; i > sizeof(buf); i -= sizeof(buf)) {
    if (!EVP_DigestUpdate(md, buf, sizeof(buf))) goto err;
  }
  if (!EVP_DigestUpdate(md, buf, i)) goto err;

  n = (int)passwd_len;
  while (n) {
    if (!EVP_DigestUpdate(md, (n & 1) ? "\0" : passwd, 1)) goto err;
    n >>= 1;
  }
  if (!EVP_DigestFinal_ex(md, buf, NULL)) goto err;

  for (i = 0; i < 1000; i++) {
    if (!EVP_DigestInit_ex(md2, EVP_md5(), NULL)) goto err;
    if (!EVP_DigestUpdate(md2, (i & 1) ? (const unsigned char*)passwd : buf,
                          (i & 1) ? passwd_len : sizeof(buf)))
      goto err;
    if (i % 3) {
      if (!EVP_DigestUpdate(md2, ascii_salt, salt_len)) goto err;
    }
    if (i % 7) {
      if (!EVP_DigestUpdate(md2, passwd, passwd_len)) goto err;
    }
    if (!EVP_DigestUpdate(md2, (i & 1) ? buf : (const unsigned char*)passwd,
                          (i & 1) ? sizeof(buf) : passwd_len))
      goto err;
    if (!EVP_DigestFinal_ex(md2, buf, NULL)) goto err;
  }
  EVP_MD_CTX_free(md2);
  EVP_MD_CTX_free(md);
  md2 = NULL;
...
#define ROUNDS_MAX 999999999
  ...
  if (strlen(out_buf) > 3 + 17 * rounds_custom + salt_len) goto err;
  ...
  for (n = passwd_len; n > buf_size; n -= buf_size) {
    if (!EVP_DigestUpdate(md, buf, buf_size)) goto err;
  }
  if (!EVP_DigestUpdate(md, buf, n)) goto err;
  ...
  /* P sequence */
  if (!EVP_DigestInit_ex(md2, sha, NULL)) goto err;

  for (n = passwd_len; n > 0; n--)
    if (!EVP_DigestUpdate(md2, passwd, passwd_len)) goto err;

  if (!EVP_DigestFinal_ex(md2, temp_buf, NULL)) goto err;

  if ((p_bytes = OPENSSL_zalloc(passwd_len)) == NULL) goto err;
  ...
  if ((s_bytes = OPENSSL_zalloc(salt_len)) == NULL) goto err;
  ...
  for (n = 0; n < rounds; n++) {
    ...
    if (!EVP_DigestUpdate(md2, (n & 1) ? (const unsigned char*)p_bytes : buf,
      ...
      goto err;
    if (n % 3) {
      if (!EVP_DigestUpdate(md2, s_bytes, salt_len)) goto err;
    }
    if (n % 7) {
      if (!EVP_DigestUpdate(md2, p_bytes, passwd_len)) goto err;
    }
...
static int do_passwd(int passed_salt, char** salt_p, char** salt_malloc_p,
  ...
  if (!passed_salt) {
    ...
    if (mode == passwd_sha256 || mode == passwd_sha512) saltlen = 16;

    assert(saltlen != 0);

    if (*salt_malloc_p == NULL)
      *salt_p = *salt_malloc_p = app_malloc(saltlen + 1, "salt buffer");
    if (RAND_bytes((unsigned char*)*salt_p, (int)saltlen) <= 0) goto end;
  ...
  if (mode == passwd_aixmd5) hash = md5crypt(passwd, "", *salt_p);
