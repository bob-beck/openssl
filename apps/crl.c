...
int crl_main(int argc, char** argv) {
  ...
  if (!opt_md(digestname, &digest)) goto opthelp;
  ...
  if (num) {
    for (i = 1; i <= num; i++) {
      ...
      if (hash == i) {
        ...
        if (num > 1) BIO_printf(bio_out, "issuer name hash=");
        if (ok) {
...
#ifndef OPENSSL_NO_MD5
      if (hash_old == i) {
        if (num > 1) BIO_printf(bio_out, "issuer name old hash=");
        BIO_printf(bio_out, "%08lx\n",
  ...
  if (text) X509_CRL_print_ex(out, x, get_nameopt());
