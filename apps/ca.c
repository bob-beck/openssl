...
static char*
lookup_conf(const CONF* conf, const char* group, const char* tag);
...
do_body(X509** xret,
        ...
        unsigned long dateopt);
static int
get_certificate_status(const char* ser_status, CA_DB* db);
static int
check_time_format(const char* str);
static int
do_revoke(X509* x509, CA_DB* db, REVINFO_TYPE rev_type, const char* extval);
static char*
...
end_of_options:
  ...
  for (i = 0; i < sk_OPENSSL_PSTRING_num(db->db->data); i++) {
    ...
    if ((j & 1) || (j < 2)) {
      BIO_printf(
        bio_err, "entry %d: bad serial number length (%d)\n", i + 1, j);
      goto end;
    }
    for (; *p; p++) {
      if (!isxdigit(_UC(*p))) {
  ...
  if (gencrl) {
    ...
    if (crl_ext != NULL) {
      ...
      if (!X509V3_EXT_add_nconf(conf, &ctx, crl_ext, NULL)) {
        BIO_printf(
          bio_err, "Error checking CRL extension section %s\n", crl_ext);
        ret = 1;
        goto end;
      }
    }
    ...
    if (crl_ext != NULL || crlnumberfile != NULL) {
      ...
      if (crl_ext != NULL)
        if (!X509V3_EXT_CRL_add_nconf(conf, &crlctx, crl_ext, crl)) {
          BIO_printf(
            bio_err, "Error adding CRL extensions from section %s\n", crl_ext);
          goto end;
        }
      if (crlnumberfile != NULL) {
  ...
  if (dorevoke) {
    if (infile == NULL) {
      ...
      revcert = load_cert_pass(
        infile, informat, 1, passin, "certificate to be revoked");
      if (revcert == NULL)
        goto end;
      if (dorevoke == 2)
        rev_type = REV_VALID;
             ...
             unsigned long dateopt)
{
  ...
  if ((template_cert = load_cert_pass(
         infile, certformat, 1, passin, "template certificate")) == NULL)
    goto end;
  if (verbose)
        ...
        unsigned long dateopt)
{
  ...
  for (i = 0; i < sk_CONF_VALUE_num(policy); i++) {
    ...
    for (;;) {
      ...
      again2:
        ...
        BIO_printf(
          bio_err, "%s:invalid type in 'policy' configuration\n", cv->value);
        goto end;
      }
  ...
  if (db->attributes.unique_subject) {
    ...
    if (rrow != NULL) {
      BIO_printf(
        bio_err, "ERROR:There is already a certificate for %s\n", row[DB_name]);
    }
  }
  if (rrow == NULL) {
...
do_revoke(X509* x509, CA_DB* db, REVINFO_TYPE rev_type, const char* value)
{
  ...
  if (rrow == NULL) {
    ...
    BIO_printf(
      bio_err, "ERROR:Already present, serial number %s\n", row[DB_serial]);
    goto end;
  } else if (rrow[DB_type][0] == DB_TYPE_REV) {
    BIO_printf(
      bio_err, "ERROR:Already revoked, serial number %s\n", row[DB_serial]);
    goto end;
  } else {
