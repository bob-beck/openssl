...
static int certify(X509** xret, const char* infile, int informat,
                   EVP_PKEY* pkey, X509* x509, const char* dgst,
...
const OPTIONS ca_options[] = {
    ...
    {"dateopt", OPT_DATEOPT, 's',
     "Datetime format used for printing. (rfc_822/iso_8601). Default is "
     "rfc_822."},
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_IN:
        ...
        break;
      case OPT_INFORM:
        if (!opt_format(opt_arg(), OPT_FMT_PEMDER, &informat)) goto opthelp;
        break;
      case OPT_OUT:
        outfile = opt_arg();
        break;
      case OPT_DATEOPT:
        if (!set_dateopt(&dateopt, opt_arg())) goto opthelp;
        break;
      case OPT_VERBOSE:
        verbose = 1;
        break;
      ...
      case OPT_KEYFILE:
        ...
        break;
      case OPT_KEYFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &keyformat)) goto opthelp;
        break;
      case OPT_PASSIN:
        passinarg = opt_arg();
        break;
      ...
      case OPT_CERT:
        ...
        break;
      case OPT_CERTFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &certformat)) goto opthelp;
        break;
      case OPT_SELFSIGN:
        selfsign = 1;
        break;
      case OPT_OUTDIR:
        ...
        break;
      case OPT_SIGOPT:
        if (sigopts == NULL) sigopts = sk_OPENSSL_STRING_new_null();
        if (sigopts == NULL || !sk_OPENSSL_STRING_push(sigopts, opt_arg()))
          goto end;
        break;
      case OPT_VFYOPT:
        if (vfyopts == NULL) vfyopts = sk_OPENSSL_STRING_new_null();
...
end_of_options:
  ...
  if ((conf = app_load_config_verbose(configfile, 1)) == NULL) goto end;
  if (configfile != default_config_file && !app_load_modules(conf)) goto end;
  ...
  if (!add_oid_section(conf)) goto end;
  ...
  if (chtype != MBSTRING_UTF8) {
    ...
    if (f != NULL && strcmp(f, "yes") == 0) chtype = MBSTRING_UTF8;
  }

  db_attr.unique_subject = 1;
  p = app_conf_try_string(conf, section, ENV_UNIQUE_SUBJECT);
  if (p != NULL) db_attr.unique_subject = parse_yesno(p, 1);

  /*****************************************************************/
  /* report status of cert with serial number given on command line */
  if (ser_status) {
    dbfile = lookup_conf(conf, section, ENV_DATABASE);
    if (dbfile == NULL) goto end;
  ...
  cleanse(passin);
  if (pkey == NULL) /* load_key() has already printed an appropriate message */
    goto end;
  ...
  if ((outdir == NULL) && (req)) {
    ...
    if (outdir == NULL) {
      BIO_printf(bio_err,
                 "there needs to be defined a directory for new certificate to "
                 "be placed in\n");
      goto end;
    }
  ...
  if (doupdatedb) {
    ...
    if (i == -1) {
      ...
      if (verbose) BIO_printf(bio_err, "No entries found to mark expired\n");
    } else {
  ...
  if (extfile) {
    ...
    if (extensions == NULL) {
      ...
      if (extensions == NULL) extensions = "default";
    }
  }
  ...
  if (req) {
    if (email_dn == 1) {
      ...
      if (tmp_email_dn != NULL && strcmp(tmp_email_dn, "no") == 0) email_dn = 0;
    }
    if (verbose) BIO_printf(bio_err, "message digest is %s\n", dgst);
    if (policy == NULL &&
        (policy = lookup_conf(conf, section, ENV_POLICY)) == NULL)
      goto end;

    if (verbose) BIO_printf(bio_err, "policy is %s\n", policy);

    if (app_conf_try_string(conf, section, ENV_RAND_SERIAL) != NULL) {
      rand_ser = 1;
    } else {
      serialfile = lookup_conf(conf, section, ENV_SERIAL);
      if (serialfile == NULL) goto end;
    }
    ...
    if (spkac_file != NULL) {
      ...
      if (j > 0) {
        total_done++;
        BIO_printf(bio_err, "\n");
        if (!BN_add_word(serial, 1)) goto end;
    ...
    if (infile != NULL) {
      total++;
      j = certify(&x, infile, informat, pkey, x509p, dgst, sigopts, vfyopts,
                  attribs, db, serial, subj, chtype, multirdn, email_dn,
                  startdate, enddate, days, batch, extensions, conf, verbose,
    ...
    for (i = 0; i < argc; i++) {
      total++;
      j = certify(&x, argv[i], informat, pkey, x509p, dgst, sigopts, vfyopts,
                  attribs, db, serial, subj, chtype, multirdn, email_dn,
                  startdate, enddate, days, batch, extensions, conf, verbose,
                  certopt, get_nameopt(), default_op, ext_copy, selfsign,
                  dateopt);
    ...
    if (verbose) BIO_printf(bio_err, "writing new certificates\n");
    ...
    for (i = 0; i < sk_X509_num(cert_sk); i++) {
      ...
      *n = '\0'; /* closing new_cert */
      if (verbose) BIO_printf(bio_err, "writing %s\n", new_cert);

      Sout = bio_open_default(outfile, 'w',
                              output_der ? FORMAT_ASN1 : FORMAT_TEXT);
      if (Sout == NULL) goto end;
  ...
  if (gencrl) {
    ...
    if ((crl = X509_CRL_new_ex(app_get0_libctx(), app_get0_propq())) == NULL)
      goto end;
    if (!X509_CRL_set_issuer_name(crl, X509_get_subject_name(x509))) goto end;
    ...
    for (i = 0; i < sk_OPENSSL_PSTRING_num(db->db->data); i++) {
      pp = sk_OPENSSL_PSTRING_value(db->db->data, i);
      if (pp[DB_type][0] == DB_TYPE_REV) {
        if ((r = X509_REVOKED_new()) == NULL) goto end;
        j = make_revoked(r, pp[DB_rev_date]);
        if (!j) goto end;
        if (j == 2) crl_v2 = 1;
        if (!BN_hex2bn(&serial, pp[DB_serial])) goto end;
    ...
    if (crl_ext != NULL || crlnumberfile != NULL) {
      ...
      if (crlnumberfile != NULL) {
        ...
        ASN1_INTEGER_free(tmpser);
        crl_v2 = 1;
        if (!BN_add_word(crlnumber, 1)) goto end;
      }
    }
    if (crl_ext != NULL || crl_v2) {
      if (!X509_CRL_set_version(crl, X509_CRL_VERSION_2)) goto end;
    }
    ...
    if (!do_X509_CRL_sign(crl, pkey, dgst, sigopts)) goto end;
  ...
  if (dorevoke) {
    if (infile == NULL) {
      ...
      if (revcert == NULL) goto end;
      if (dorevoke == 2) rev_type = REV_VALID;
  ...
  cleanse(passin);
  if (free_passin) OPENSSL_free(passin);
  BN_free(serial);
  BN_free(crlnumber);
  free_index(db);
...
static int certify(X509** xret, const char* infile, int informat,
  ...
  if (verbose) X509_REQ_print_ex(bio_err, req, nameopt, X509_FLAG_COMPAT);
...
static int certify_cert(X509** xret, const char* infile, int certformat,
  ...
  if ((template_cert = load_cert_pass(infile, certformat, 1, passin,
    ...
    goto end;
  if (verbose) X509_print(bio_err, template_cert);
  ...
  if ((rreq = X509_to_X509_REQ(template_cert, NULL, NULL)) == NULL) goto end;
...
static int do_body(X509** xret, EVP_PKEY* pkey, X509* x509, const char* dgst,
  ...
  for (i = 0; i < X509_NAME_entry_count(name); i++) {
    ...
    if (nid == NID_pkcs9_emailAddress && !email_dn) continue;
    ...
    if (default_op) old_entry_print(obj, str);
  }
    ...
    for (;;) {
      ...
      again2:
        ...
        if (j >= 0) {
          ...
          last2 = j;
          if (ASN1_STRING_cmp(str, str2) != 0) goto again2;
        }
        if (j < 0) {
          BIO_printf(bio_err,
  ...
  if (BN_to_ASN1_INTEGER(serial, X509_get_serialNumber(ret)) == NULL) goto end;
  if (selfsign) {
    if (!X509_set_issuer_name(ret, subject)) goto end;
  } else {
    if (!X509_set_issuer_name(ret, X509_get_subject_name(x509))) goto end;
  }

  if (!set_cert_times(ret, startdate, enddate, days, 0)) goto end;

  if (enddate != NULL) {
    int tdays;

    if (!ASN1_TIME_diff(&tdays, NULL, NULL, X509_get0_notAfter(ret))) goto end;
    days = tdays;
  }

  if (!X509_set_subject_name(ret, subject)) goto end;
  ...
  if (selfsign) {
    if (!X509V3_set_issuer_pkey(&ext_ctx, pkey)) goto end;
  ...
  if (ext_sect) {
    if (extfile_conf != NULL) {
      if (verbose) BIO_printf(bio_err, "Extra configuration file found\n");
  ...
  if (!do_X509_sign(ret, 0, pkey, dgst, sigopts, &ext_ctx)) goto end;
...
static char* make_revocation_str(REVINFO_TYPE rev_type, const char* rev_arg) {
  ...
  if (reason) i += (int)(strlen(reason) + 1);
  if (other) i += (int)(strlen(other) + 1);
...
static int make_revoked(X509_REVOKED* rev, const char* str) {
  ...
  if (rev && !X509_REVOKED_set_revocationDate(rev, revDate)) goto end;

  if (rev && (reason_code != OCSP_REVOKED_STATUS_NOSTATUS)) {
    rtmp = ASN1_ENUMERATED_new();
    if (rtmp == NULL || !ASN1_ENUMERATED_set(rtmp, reason_code)) goto end;
