...
static char *lookup_conf (const CONF *conf, const char *group,
                          const char *tag);
...
certify (X509 **xret, const char *infile, int informat, EVP_PKEY *pkey,
         ...
         CONF *conf, int verbose, unsigned long certopt, unsigned long nameopt,
         int default_op, int ext_copy, int selfsign, unsigned long dateopt);
static int certify_cert (
    X509 **xret, const char *infile, int certformat, const char *passin,
    EVP_PKEY *pkey, X509 *x509, const char *dgst,
    STACK_OF (OPENSSL_STRING) *sigopts, STACK_OF (OPENSSL_STRING) *vfyopts,
    STACK_OF (CONF_VALUE) *policy, CA_DB *db, BIGNUM *serial, const char *subj,
    unsigned long chtype, int multirdn, int email_dn, const char *startdate,
    const char *enddate, long days, int batch, const char *ext_sect,
    CONF *conf, int verbose, unsigned long certopt, unsigned long nameopt,
    int default_op, int ext_copy, unsigned long dateopt);
static int
certify_spkac (X509 **xret, const char *infile, EVP_PKEY *pkey, X509 *x509,
               ...
               long days, const char *ext_sect, CONF *conf, int verbose,
               unsigned long certopt, unsigned long nameopt, int default_op,
               int ext_copy, unsigned long dateopt);
...
const OPTIONS ca_options[] = {
  ...
  { "enddate", OPT_ENDDATE, 's',
    "[CC]YYMMDDHHMMSSZ value for notAfter certificate field, overrides "
    "-days" },
  ...
  { "crl_hold", OPT_CRL_HOLD, 's',
    "the hold instruction, an OID. Sets revocation reason to "
    "certificateHold" },
  ...
  if (req)
    {
          ...
          if (extensions != NULL)
            {
              ...
              if (!X509V3_EXT_add_nconf (conf, &ctx, extensions, NULL))
                {
                  BIO_printf (bio_err,
                              "Error checking certificate extension config "
                              "section %s\n",
                              extensions);
                  ret = 1;
                  goto end;
      ...
      if (days != 0 && enddate != NULL)
        BIO_printf (bio_err, "Warning: -enddate or -not_after option "
                             "overriding -days option\n");
      ...
      if (spkac_file != NULL)
        {
          ...
          j = certify_spkac (&x, spkac_file, pkey, x509, dgst, sigopts,
                             ...
                             conf, verbose, certopt, get_nameopt (),
...
static char *
lookup_conf (const CONF *conf, const char *section, const char *tag)
{
...
static int
certify_cert (X509 **xret, const char *infile, int certformat,
              const char *passin, EVP_PKEY *pkey, X509 *x509, const char *dgst,
              STACK_OF (OPENSSL_STRING) *sigopts,
...
static int
do_body (X509 **xret, EVP_PKEY *pkey, X509 *x509, const char *dgst,
         STACK_OF (OPENSSL_STRING) *sigopts, STACK_OF (CONF_VALUE) *policy,
         CA_DB *db, BIGNUM *serial, const char *subj, unsigned long chtype,
         int multirdn, int email_dn, const char *startdate,
         const char *enddate, long days, int batch, int verbose, X509_REQ *req,
      ...
      if (extfile_conf != NULL)
        {
          ...
          if (!X509V3_EXT_add_nconf (extfile_conf, &ext_ctx, ext_sect, ret))
            {
              BIO_printf (bio_err,
                          "Error adding certificate extensions from extfile "
                          "section %s\n",
                          ext_sect);
              goto end;
            }
      ...
      else if (ext_sect)
        {
          ...
          if (!X509V3_EXT_add_nconf (lconf, &ext_ctx, ext_sect, ret))
            {
              BIO_printf (bio_err,
                          "Error adding certificate extensions from config "
                          "section %s\n",
                          ext_sect);
              goto end;
            }
...
certify_spkac (X509 **xret, const char *infile, EVP_PKEY *pkey, X509 *x509,
               ...
               const char *subj, unsigned long chtype, int multirdn,
               int email_dn, const char *startdate, const char *enddate,
               long days, const char *ext_sect, CONF *lconf, int verbose,
               unsigned long certopt, unsigned long nameopt, int default_op,
               int ext_copy, unsigned long dateopt)
{
...
static int
do_revoke (X509 *x509, CA_DB *db, REVINFO_TYPE rev_type, const char *value)
{
...
static char *
make_revocation_str (REVINFO_TYPE rev_type, const char *rev_arg)
{
  char *str;
...
static int
old_entry_print (const ASN1_OBJECT *obj, const ASN1_STRING *str)
{
  char buf[25], *pbuf;
  const char *p;
  int j;
