...
int pkcs8_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_SCRYPT:
        ...
        break;
      case OPT_SCRYPT_N:
        if (!opt_long(opt_arg(), &scrypt_N) || scrypt_N <= 0) goto opthelp;
        break;
      case OPT_SCRYPT_R:
        if (!opt_long(opt_arg(), &scrypt_r) || scrypt_r <= 0) goto opthelp;
        break;
      case OPT_SCRYPT_P:
        if (!opt_long(opt_arg(), &scrypt_p) || scrypt_p <= 0) goto opthelp;
        break;
#endif
      case OPT_SALTLEN:
  ...
  if (topk8) {
    ...
    if ((out = bio_open_owner(outfile, outformat, private)) == NULL) goto end;
    if (nocrypt) {
      assert(private);
