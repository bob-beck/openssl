...
pkcs8_main(int argc, char** argv)
{
    ...
    switch (o) {
      ...
      case OPT_V1:
        ...
        if (pbe_nid == NID_undef) {
          BIO_printf(
            bio_err, "%s: Unknown PBE algorithm %s\n", prog, opt_arg());
          goto opthelp;
        }
        break;
      case OPT_V2PRF:
        ...
        if (!EVP_PBE_find(EVP_PBE_TYPE_PRF, pbe_nid, NULL, NULL, 0)) {
          BIO_printf(
            bio_err, "%s: Unknown PRF algorithm %s\n", prog, opt_arg());
          goto opthelp;
        }
        if (cipher == NULL)
  ...
  in = bio_open_default(
    infile, 'r', informat == FORMAT_UNDEF ? FORMAT_PEM : informat);
  if (in == NULL)
    goto end;
      ...
      if (cipher) {
        ...
        if (scrypt_N && scrypt_r && scrypt_p)
          pbe = PKCS5_pbe2_set_scrypt(
            cipher, NULL, saltlen, NULL, scrypt_N, scrypt_r, scrypt_p);
        else
#endif
