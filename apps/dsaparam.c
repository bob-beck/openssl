...
const OPTIONS dsaparam_options[] = {
    ...
    {"numqbits", 0, 0,
     "Number of bits in the subprime parameter q if generating parameters or "
     "key (optional)"},
    {NULL}};
...
int dsaparam_main(int argc, char** argv) {
  ...
  argv = opt_rest();
  if (argc == 2) {
    if (!opt_int(argv[0], &num) || num < 0) goto opthelp;
    if (!opt_int(argv[1], &numqbits) || numqbits < 0) goto opthelp;
  } else if (argc == 1) {
    if (!opt_int(argv[0], &num) || num < 0) goto opthelp;
