...
opthelp:
	...
	ctx = EVP_PKEY_CTX_new_from_name(app_get0_libctx(), "DSA",
					 app_get0_propq());
	if (ctx == NULL) {
		BIO_printf(
			bio_err,
	...
	if (numbits > 0) {
		...
		params = load_keyparams(infile, informat, 1, "DSA",
					"DSA parameters");
	}
	if (params == NULL) {
	...
	if (!noout) {
		...
		if (!i) {
			BIO_printf(bio_err,
				   "Error, unable to write DSA parameters\n");
			goto end;
		}
	}
	if (genkey) {
		...
		if (outformat == FORMAT_ASN1)
			...
			i = PEM_write_bio_PrivateKey(out, pkey, NULL, NULL, 0,
						     NULL, NULL);
	}
	ret = 0;
end:
