 ...
 * pcbio: pointer to variable where to place the BIO for sending the response
 * to acbio: the listening bio (typically as returned by
 * http_server_init_bio()) found_keep_alive: for returning flag if client
 * requests persistent connection prog: the name of the current app, for
 * diagnostics only accept_get: whether to accept GET requests (in addition to
 * POST requests) timeout: connection timeout (in seconds), or 0 for
 * none/infinite returns 0 in case caller should retry, then *preq == *ppath ==
 * *pcbio == NULL returns -1 on fatal error; also then holds *preq == *ppath ==
 * *pcbio == NULL returns 1 otherwise. In this case it is guaranteed that
 * *pcbio != NULL while *ppath == NULL and *preq == NULL if and only if the
 * request is invalid, On return value 1 the caller is responsible for sending
 * an HTTP response, using http_server_send_asn1_resp() or
 * http_server_send_status(). The caller must free any non-NULL *preq, *ppath,
 * and *pcbio pointers.
 */
