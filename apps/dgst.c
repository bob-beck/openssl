...
const OPTIONS dgst_options[] = {
    ...
    {"xoflen", OPT_XOFLEN, 'p',
     "Output length for XOF algorithms. To obtain the maximum security "
     "strength set this to 32 (or greater) for SHAKE128, and 64 (or greater) "
     "for SHAKE256"},
...
int dgst_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_SIGOPT:
        ...
        break;
      case OPT_MACOPT:
        if (!macopts) macopts = sk_OPENSSL_STRING_new_null();
  ...
  if (mac_name != NULL) {
    ...
    if (!init_gen_str(&mac_ctx, mac_name, impl, 0, NULL, NULL)) goto end;
    if (macopts != NULL) {
  ...
  /* we use md as a filter, reading from 'in' */
    ...
    if (md == NULL) md = (EVP_MD*)EVP_sha256();
  ...
  if (!oneshot_sign) {
    ...
    if (md != NULL) md_name = EVP_MD_get0_name(md);
  }
  if (xoflen > 0) {
    if (!EVP_MD_xof(md)) {
  ...
  if (argc == 0) {
    ...
    if (out_bin == 0) {
      if (sigkey != NULL) sig_name = EVP_PKEY_get0_type_name(sigkey);
    }
    ret = EXIT_SUCCESS;
    for (i = 0; i < argc; i++) {
  ...
  if (ret != EXIT_SUCCESS) ERR_print_errors(bio_err);
...
static void show_digests(const OBJ_NAME* name, void* arg) {
  ...
  if (md == NULL) {
    if (EVP_get_digestbyname(name->name) == NULL) return;
  }
...
static void print_out(BIO* out, unsigned char* buf, size_t len, int sep,
  ...
  if (binout) {
    ...
    if (sig_name != NULL) {
      ...
      if (md_name != NULL) BIO_printf(out, "-%s", md_name);
...
int do_fp(BIO* out, unsigned char* buf, BIO* bp, int sep, int binout,
  ...
  if (key != NULL) {
    ...
    if ((int)len < 0) goto end;
  }
  print_out(out, buf, len, sep, binout, sig_name, md_name, file);
  ret = EXIT_SUCCESS;
end:
  if (allocated_buf != NULL) OPENSSL_clear_free(allocated_buf, len);
