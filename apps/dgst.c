...
static void
show_digests (const OBJ_NAME *name, void *arg)
{
 ...
 * to backslash. The backslash pointer is a flag to indicating whether a
 * newline is present in the filename.  If a newline is present, the backslash
 * flag is set and the output format will contain a backslash at the beginning
 * of the digest output. This output format is to replicate the output format
 * found in the '*sum' checksum programs. This aims to preserve backward
 * compatibility.
 */
static const char *
newline_escape_filename (const char *file, int *backslash)
{
...
static void
print_out (BIO *out, unsigned char *buf, size_t len, int sep, int binout,
           const char *sig_name, const char *md_name, const char *file)
{
  int i, backslash = 0;
...
static void
print_verify_result (BIO *out, int i)
{
  if (i > 0)
...
int
do_fp (BIO *out, unsigned char *buf, BIO *bp, int sep, int binout, int xoflen,
 ...
 */
static int
do_fp_oneshot_sign (BIO *out, EVP_MD_CTX *ctx, BIO *in, int sep, int binout,
