...
static int do_mac(EVP_MAC_CTX* ctx, unsigned char* tmp, BIO* in,
  ...
  if (!EVP_MAC_init(ctx, NULL, 0, NULL)) goto err;
  if (EVP_MAC_CTX_get_mac_size(ctx) > outsz) goto end;
  while ((i = BIO_read(in, (char*)tmp, BUFSIZE)) != 0) {
    if (i < 0 || !EVP_MAC_update(ctx, tmp, i)) goto err;
  }
end:
  if (!EVP_MAC_final(ctx, out, out_len, outsz)) goto err;
  ret = 1;
err:
  return ret;
}
...
static CONF* generate_config_and_load(const char* prov_name,
  ...
  if (CONF_modules_load(conf, NULL, 0) <= 0) goto end;
  BIO_free(mem_bio);
  return conf;
end:
  NCONF_free(conf);
...
int fipsinstall_main(int argc, char** argv) {
  ...
  if ((opts = sk_OPENSSL_STRING_new_null()) == NULL) goto end;
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_NO_SECURITY_CHECKS:
        if (!check_non_pedantic_fips(pedantic, "no_security_checks")) goto end;
      ...
      case OPT_CONFIG:
        ...
        break;
      case OPT_MACOPT:
        if (!sk_OPENSSL_STRING_push(opts, opt_arg())) goto opthelp;
      ...
      case OPT_SELF_TEST_ONINSTALL:
        if (!check_non_pedantic_fips(pedantic, "self_test_oninstall")) goto end;
        set_selftest_onload_option = 1;
  ...
  if (module_fname == NULL) goto opthelp;

  tail = opt_path_end(module_fname);
  if (tail != NULL) {
    module_path = OPENSSL_strdup(module_fname);
    if (module_path == NULL) goto end;
    module_path[tail - module_fname] = '\0';
    if (!OSSL_PROVIDER_set_default_search_path(NULL, module_path)) goto end;
  }
  ...
  if (!gotdigest && !sk_OPENSSL_STRING_push(opts, "digest:SHA256")) goto end;
  ...
  if (verify) {
    if (fips_opts.self_test_onload == 1) install_mac_len = 0;
    if (!verify_config(in_fname, section_name, module_mac, module_mac_len,
                       install_mac, install_mac_len))
      goto end;
    if (!quiet) BIO_printf(bio_err, "VERIFY PASSED\n");
  } else {
    ...
    if (!write_config_fips_section(fout, section_name, module_mac,
                                   ...
                                   install_mac_len))
      goto end;
    if (!quiet) BIO_printf(bio_err, "INSTALL PASSED\n");
  }
