...
static int
check_non_pedantic_fips(int pedantic, const char* name)
{
  if (pedantic) {
...
static int
load_fips_prov_and_run_self_test(const char* prov_name, int* is_fips_140_2_prov)
{
  int ret = 0;
  ...
  if (!quiet) {
    ...
    *p++ = OSSL_PARAM_construct_utf8_ptr(
      OSSL_PROV_PARAM_VERSION, &vers, sizeof(vers));
    *p++ = OSSL_PARAM_construct_utf8_ptr(
      OSSL_PROV_PARAM_BUILDINFO, &build, sizeof(build));
...
static int
print_mac(BIO* bio, const char* label, const unsigned char* mac, size_t len)
{
  int ret;
  char* hexstr = NULL;
...
static int
write_config_header(BIO* out, const char* prov_name, const char* section)
{
 ...
 */
static int
write_config_fips_section(BIO* out,
                          const char* section,
                          ...
                          size_t install_mac_len)
{
  ...
  if (BIO_printf(out, "[%s]\n", section) <= 0 ||
      ...
      !print_mac(
        out, OSSL_PROV_FIPS_PARAM_MODULE_MAC, module_mac, module_mac_len))
    goto end;
...
static CONF*
generate_config_and_load(const char* prov_name,
                         const char* section,
                         unsigned char* module_mac,
                         size_t module_mac_len,
                         const FIPS_OPTS* opts)
{
  ...
  if (!write_config_header(mem_bio, prov_name, section) ||
      !write_config_fips_section(
        mem_bio, section, module_mac, module_mac_len, opts, NULL, 0))
    goto end;
...
static void
free_config_and_unload(CONF* conf)
{
  if (conf != NULL) {
    NCONF_free(conf);
...
static int
verify_module_load(const char* parent_config_file)
{
 ...
 */
static int
verify_config(const char* infile,
              const char* section,
              unsigned char* module_mac,
              size_t module_mac_len,
              unsigned char* install_mac,
...
int
fipsinstall_main(int argc, char** argv)
{
  ...
  if (verify) {
    ...
    conf = generate_config_and_load(
      prov_name, section_name, module_mac, module_mac_len, &fips_opts);
    if (conf == NULL)
      goto end;
    ...
    if (!write_config_fips_section(fout,
                                   section_name,
                                   module_mac,
                                   module_mac_len,
                                   &fips_opts,
...
static int
self_test_events(const OSSL_PARAM params[], void* arg)
{
