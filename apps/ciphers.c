...
int ciphers_main(int argc, char** argv) {
  ...
  if (SSL_CTX_set_min_proto_version(ctx, min_version) == 0) goto err;
  if (SSL_CTX_set_max_proto_version(ctx, max_version) == 0) goto err;

#ifndef OPENSSL_NO_PSK
  if (psk) SSL_CTX_set_psk_client_callback(ctx, dummy_psk);
#endif
#ifndef OPENSSL_NO_SRP
  if (srp) set_up_dummy_srp(ctx);
#endif
  ...
  if (!verbose) {
    for (i = 0; i < sk_SSL_CIPHER_num(sk); i++) {
      ...
      if (!ossl_assert(c != NULL)) continue;
...
err:
  ...
  if (use_supported) sk_SSL_CIPHER_free(sk);
  SSL_CTX_free(ctx);
  SSL_free(ssl);
  return ret;
}
