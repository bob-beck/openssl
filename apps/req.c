...
static int duplicated(LHASH_OF(OPENSSL_STRING)* addexts, char* kv) {
  ...
  off = p - kv;
  if ((kv = OPENSSL_strdup(kv)) == NULL) return -1;

  /* Skip trailing space before the equal sign. */
  for (p = kv + off; p > kv; --p)
    if (!isspace(_UC(p[-1]))) break;
  if (p == kv) {
...
int req_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_INFORM:
        ...
        break;
      case OPT_OUTFORM:
        if (!opt_format(opt_arg(), OPT_FMT_PEMDER, &outformat)) goto opthelp;
        break;
      case OPT_ENGINE:
      ...
      case OPT_SECTION:
        ...
        break;
      case OPT_KEYFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &keyform)) goto opthelp;
        break;
      case OPT_IN:
        infile = opt_arg();
        break;
      ...
      case OPT_NEWKEY:
        ...
        break;
      case OPT_PKEYOPT:
        if (pkeyopts == NULL) pkeyopts = sk_OPENSSL_STRING_new_null();
      ...
      case OPT_SIGOPT:
        ...
        break;
      case OPT_VFYOPT:
        if (!vfyopts) vfyopts = sk_OPENSSL_STRING_new_null();
      ...
      case OPT_UTF8:
        ...
        break;
      case OPT_NAMEOPT:
        if (!set_nameopt(opt_arg())) goto opthelp;
        break;
      case OPT_REQOPT:
        if (!set_cert_ex(&reqflag, opt_arg())) goto opthelp;
        break;
      case OPT_TEXT:
        text = 1;
        break;
      ...
      case OPT_ADDEXT:
        ...
        if (addexts == NULL) {
          ...
          if (addexts == NULL || addext_bio == NULL) goto end;
        }
  ...
  if (!gen_x509) {
    if (days != UNSET_DAYS)
      BIO_printf(bio_err,
                 "Warning: Ignoring -days without -x509; not generating a "
                 "certificate\n");
    if (not_before != NULL)
      BIO_printf(bio_err,
                 "Warning: Ignoring -not_before without -x509; not generating "
                 "a certificate\n");
    if (not_after != NULL)
      BIO_printf(bio_err,
                 "Warning: Ignoring -not_after without -x509; not generating a "
                 "certificate\n");
    if (ext_copy == EXT_COPY_NONE)
      BIO_printf(bio_err,
                 "Warning: Ignoring -copy_extensions 'none' when -x509 is not "
                 "given\n");
  }
  if (infile == NULL) {
    if (gen_x509)
      ...
      BIO_printf(bio_err,
                 "Warning: Will read cert request from stdin since no -in "
                 "option is given\n");
  }
  ...
  if ((req_conf = app_load_config_verbose(template, verbose)) == NULL) goto end;
  if (addext_bio != NULL) {
    if (verbose)
      BIO_printf(bio_err,
                 "Using additional configuration from -addext options\n");
    if ((addext_conf = app_load_config_bio(addext_bio, NULL)) == NULL) goto end;
  }
  if (template != default_config_file && !app_load_modules(req_conf)) goto end;
  ...
  if (digest != NULL) {
    if (!opt_check_md(digest)) goto opthelp;
  } else {
  ...
  if (chtype != MBSTRING_UTF8) {
    ...
    if (p != NULL && strcmp(p, "yes") == 0) chtype = MBSTRING_UTF8;
  }
  ...
  if (keyalg != NULL && pkey != NULL) {
    BIO_printf(bio_err,
               "Warning: Not generating key via given -newkey option since "
               "-key is given\n");
  ...
  if (newreq && pkey == NULL) {
    ...
    if (progress) EVP_PKEY_CTX_set_cb(genctx, progress_cb);
  ...
  if (pkey != NULL && (keyfile == NULL || keyout != NULL)) {
    ...
    if (p == NULL) p = app_conf_try_string(req_conf, section, "encrypt_key");
    if (p != NULL && strcmp(p, "no") == 0) cipher = NULL;
    if (noenc) cipher = NULL;
  ...
  if (!newreq) {
    if (keyfile != NULL)
      BIO_printf(bio_err,
                 "Warning: Not placing -key in cert or request since request "
                 "is used\n");
    ...
    BIO_printf(bio_err,
               "Warning: Ignoring -in option since -new or -newkey or -precert "
               "is given\n");
    /* Better throw an error in this case, as done in the x509 app */
  }

  if (CAkeyfile == NULL) CAkeyfile = CAfile;
  if (CAkeyfile != NULL) {
    if (CAfile == NULL) {
      BIO_printf(
          bio_err,
  ...
  if (newreq || gen_x509) {
    ...
    if (gen_x509) {
      ...
      if (CAcert != NULL && keyfile != NULL)
        BIO_printf(bio_err,
                   "Warning: Not using -key or -newkey for signing since -CA "
                   "option is given\n");
      ...
      if (serial != NULL) {
        if (!X509_set_serialNumber(new_x509, serial)) goto end;
      } else {
        if (!rand_serial(NULL, X509_get_serialNumber(new_x509))) goto end;
      }

      if (!X509_set_issuer_name(new_x509, issuer)) goto end;
      if (days == UNSET_DAYS)
        days = DEFAULT_DAYS;
      else if (not_after != NULL)
        BIO_printf(bio_err,
                   "Warning: -not_after option overriding -days option\n");
      if (!set_cert_times(new_x509, not_before, not_after, days, 1)) goto end;
      if (!X509_set_subject_name(new_x509, n_subj)) goto end;
      if (!pub_key || !X509_set_pubkey(new_x509, pub_key)) goto end;
      if (ext_copy == EXT_COPY_UNSET) {
        if (infile != NULL)
          BIO_printf(bio_err,
                     "Warning: No -copy_extensions given; ignoring any "
      ...
      if (CAcert == NULL) {
        if (!X509V3_set_issuer_pkey(&ext_ctx, issuer_key)) goto end;
  ...
  if (modulus) {
    ...
    if (EVP_PKEY_is_a(tpubkey, "RSA") || EVP_PKEY_is_a(tpubkey, "RSA-PSS")) {
      ...
      if (!EVP_PKEY_get_bn_param(tpubkey, "n", &n)) goto end;
      BN_print(out, n);
      BN_free(n);
    } else {
  ...
  release_engine(e);
  if (passin != nofree_passin) OPENSSL_free(passin);
  if (passout != nofree_passout) OPENSSL_free(passout);
  return ret;
}
...
static int make_REQ(X509_REQ* req, EVP_PKEY* pkey, X509_NAME* fsubj,
  ...
  if (tmp != NULL && strcmp(tmp, "no") == 0) no_prompt = 1;
  ...
  if (!X509_REQ_set_version(req, X509_REQ_VERSION_1)) goto err;
...
static int prompt_info(X509_REQ* req, STACK_OF(CONF_VALUE)* dn_sk,
  ...
  start:
    for (;;) {
      ...
      if (!join(buf, sizeof(buf), v->name, "_default", "Name")) return 0;
      if ((def = app_conf_try_string(req_conf, dn_sect, buf)) == NULL) def = "";

      if (!join(buf, sizeof(buf), v->name, "_value", "Name")) return 0;
      if ((value = app_conf_try_string(req_conf, dn_sect, buf)) == NULL)
        value = NULL;

      if (!join(buf, sizeof(buf), v->name, "_min", "Name")) return 0;
      if (!app_conf_try_number(req_conf, dn_sect, buf, &n_min)) n_min = -1;

      if (!join(buf, sizeof(buf), v->name, "_max", "Name")) return 0;
      if (!app_conf_try_number(req_conf, dn_sect, buf, &n_max)) n_max = -1;
    ...
    start2:
      for (;;) {
        i++;
        if ((attr_sk == NULL) || (sk_CONF_VALUE_num(attr_sk) <= i)) break;

        v = sk_CONF_VALUE_value(attr_sk, i);
        type = v->name;
        if ((nid = OBJ_txt2nid(type)) == NID_undef) goto start2;

        if (!join(buf, sizeof(buf), type, "_default", "Name")) return 0;
        def = app_conf_try_string(req_conf, attr_sect, buf);
        if (def == NULL) def = "";

        if (!join(buf, sizeof(buf), type, "_value", "Name")) return 0;
        value = app_conf_try_string(req_conf, attr_sect, buf);

        if (!join(buf, sizeof(buf), type, "_min", "Name")) return 0;
        if (!app_conf_try_number(req_conf, attr_sect, buf, &n_min)) n_min = -1;

        if (!join(buf, sizeof(buf), type, "_max", "Name")) return 0;
        if (!app_conf_try_number(req_conf, attr_sect, buf, &n_max)) n_max = -1;
...
start:
  ...
  if (value != NULL) {
    ...
    buf[0] = '\0';
    if (!batch) {
      if (!fgets(buf, buf_size, stdin)) return 0;
    } else {
      buf[0] = '\n';
      buf[1] = '\0';
    }
