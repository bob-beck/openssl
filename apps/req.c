...
auto_info(X509_REQ* req,
          ...
          unsigned long chtype);
static int
add_attribute_object(X509_REQ* req,
                     char* text,
                     const char* def,
                     char* value,
                     int nid,
...
join(char buf[],
     ...
     const char* desc);
static EVP_PKEY_CTX*
set_keygen_ctx(const char* gstr,
               char** pkeytype,
               long* pkeylen,
...
req_main(int argc, char** argv)
{
  ...
  if (CAfile != NULL) {
    if ((CAcert = load_cert_pass(
           CAfile, FORMAT_UNDEF, 1, passin, "issuer cert from -CA arg")) ==
        NULL)
      goto end;
  ...
  if (newreq || gen_x509) {
    ...
    if (gen_x509) {
      ...
      if (extsect != NULL &&
        ...
        BIO_printf(
          bio_err, "Error adding x509 extensions from section %s\n", extsect);
        goto end;
      }
      if (addext_conf != NULL &&
            ...
            unsigned long chtype)
{
  ...
  start:
    for (;;) {
      ...
      if (!add_DN_object(
            subj, v->value, def, value, nid, n_min, n_max, chtype, mval))
        return 0;
    }
    ...
    start2:
      for (;;) {
        ...
        if (!add_attribute_object(
              req, v->value, def, value, nid, n_min, n_max, chtype))
          return 0;
      }
    }
  } else {
  ...
  if (attribs) {
    for (i = 0; i < sk_CONF_VALUE_num(attr_sk); i++) {
      ...
      if (!X509_REQ_add1_attr_by_txt(
            req, v->name, chtype, (unsigned char*)v->value, -1))
        return 0;
    }
  }
  return 1;
              ...
              int mval)
{
  ...
  ret = build_data(
    text, def, value, n_min, n_max, buf, sizeof(buf), "DN value", "DN default");
  if ((ret == 0) || (ret == 1))
    return ret;
  ret = 1;

  if (!X509_NAME_add_entry_by_NID(
        n, nid, chtype, (unsigned char*)buf, -1, -1, mval))
    ret = 0;
...
req_check_len(int len, int n_min, int n_max)
{
  ...
  if (n_max >= 0 && len > n_max) {
    BIO_printf(
      bio_err, "String too long, must be at most %d bytes long\n", n_max);
    return 0;
  }
  return 1;
}
               ...
               ENGINE* keygen_engine)
{
  ...
  if (gstr == NULL) {
    ...
    if (strncmp(gstr, "param", len) == 0) {
      ...
      if (p == NULL) {
        BIO_printf(
          bio_err, "Parameter file requested but no path given: %s\n", gstr);
        return NULL;
      }
    } else {
      keytype = gstr;
