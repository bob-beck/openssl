...
const OPTIONS req_options[] = {
  ...
  { "newkey", OPT_NEWKEY, 's',
    "Generate new key with [<alg>:]<nbits> or <alg>[:<file>] or "
    "param:<file>" },
...
req_main (int argc, char **argv)
{
  ...
  if (!gen_x509)
    {
      ...
      if (not_before != NULL)
        BIO_printf (bio_err, "Warning: Ignoring -not_before without -x509; "
                             "not generating a certificate\n");
      if (not_after != NULL)
        BIO_printf (bio_err, "Warning: Ignoring -not_after without -x509; not "
                             "generating a certificate\n");
      if (ext_copy == EXT_COPY_NONE)
        BIO_printf (bio_err, "Warning: Ignoring -copy_extensions 'none' when "
                             "-x509 is not given\n");
    }
  if (infile == NULL)
    {
      if (gen_x509)
        ...
        BIO_printf (bio_err, "Warning: Will read cert request from stdin "
                             "since no -in option is given\n");
    }
  ...
  if (keyalg != NULL && pkey != NULL)
    {
      BIO_printf (bio_err, "Warning: Not generating key via given -newkey "
                           "option since -key is given\n");
  ...
  if (!newreq)
    ...
    {
      BIO_printf (bio_err, "Warning: Ignoring -in option since -new or "
                           "-newkey or -precert is given\n");
  ...
  if (CAkeyfile != NULL)
    {
      if (CAfile == NULL)
        {
          BIO_printf (bio_err, "Warning: Ignoring -CAkey option since no -CA "
                               "option is given\n");
        }
      else
        {
      ...
      if (gen_x509)
        {
          ...
          if (CAcert != NULL && keyfile != NULL)
            BIO_printf (bio_err, "Warning: Not using -key or -newkey for "
                                 "signing since -CA option is given\n");
      ...
      else
        {
          ...
          if (precert)
            {
              BIO_printf (bio_err, "Warning: Ignoring -precert flag since no "
                                   "cert is produced\n");
            }
          /* Set up V3 context struct */
...
static int
make_REQ (X509_REQ *req, EVP_PKEY *pkey, X509_NAME *fsubj, int multirdn,
...
static int
prompt_info (X509_REQ *req, STACK_OF (CONF_VALUE) *dn_sk, const char *dn_sect,
...
static int
add_DN_object (X509_NAME *n, char *text, const char *def, char *value, int nid,
...
static int
add_attribute_object (X509_REQ *req, char *text, const char *def, char *value,
...
static int
build_data (char *text, const char *def, char *value, int n_min, int n_max,
            char *buf, const int buf_size, const char *desc1,
            const char *desc2)
{
  int i;
start:
 ...
 */
static int
join (char buf[], size_t buf_size, const char *name, const char *tail,
      const char *desc)
{
...
static EVP_PKEY_CTX *
set_keygen_ctx (const char *gstr, char **pkeytype, long *pkeylen,
