...
static OSSL_CMP_MSG *read_write_req_resp(OSSL_CMP_CTX *ctx,
  ...
  if (opt_rspin != NULL) {
    ...
    if (opt_use_mock_srv) {
      if (rspin_in_use)
        CMP_warn("too few -rspin filename arguments; resorting to using mock "
                 "server");
      ...
      if (rspin_in_use)
        CMP_warn("too few -rspin filename arguments; resorting to contacting "
                 "server");
...
#define OSSL_CMP_RR_MSG "since -issuer and -serial is given for command 'rr'"
  ...
  if (OSSL_CMP_CTX_reqExtensions_have_SAN(ctx) && opt_sans != NULL) {
    CMP_err("cannot have Subject Alternative Names both via -reqexts and via "
            "-sans");
    return 0;
  }
  ...
  if (opt_policy_oids_critical) {
    if (opt_policy_oids == NULL)
      CMP_warn("-opt_policy_oids_critical has no effect unless -policy_oids is "
               "given");
