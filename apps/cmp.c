...
const OPTIONS cmp_options[] = {
  ...
  { "geninfo", OPT_GENINFO, 's',
    "Comma-separated list of OID and value to place in generalInfo "
    "PKIHeader" },
  { OPT_MORE_STR, 0, 0,
  ...
  { "oldcert", OPT_OLDCERT, 's',
    "Certificate to be updated (defaulting to -cert) or to be revoked in "
    "rr;" },
  { OPT_MORE_STR, 0, 0,
  ...
  { "msg_timeout", OPT_MSG_TIMEOUT, 'N',
    "Number of seconds allowed per CMP message round trip, or 0 for "
    "infinite" },
  { "total_timeout", OPT_TOTAL_TIMEOUT, 'N',
    "Overall time an enrollment incl. polling may take. Default 0 = "
    "infinite" },
  ...
  { "untrusted", OPT_UNTRUSTED, 's',
    "Intermediate CA certs for chain construction for CMP/TLS/enrolled "
    "certs" },
...
#define CMP_print(bio, level, prefix, msg, a1, a2, a3)                        \
  ((void)(level > opt_verbosity                                               \
              ...
              : (BIO_printf (bio, "%s:%s:%d:CMP %s: " msg "\n", FUNC,         \
                             OPENSSL_FILE, OPENSSL_LINE, prefix, a1, a2,      \
                             a3))))
...
static int
print_to_bio_out (const char *func, const char *file, int line,
...
static int
print_to_bio_err (const char *func, const char *file, int line,
                  OSSL_CMP_severity level, const char *msg)
{
...
static EVP_PKEY *
load_key_pwd (const char *uri, int format, const char *pass, ENGINE *eng,
              const char *desc)
{
...
static X509 *
load_cert_pwd (const char *uri, const char *pass, const char *desc)
{
  X509 *cert;
  char *pass_string = get_passwd (pass, desc);
...
static int
truststore_set_host_etc (X509_STORE *ts, const char *host)
{
...
static int
write_PKIMESSAGE (const OSSL_CMP_MSG *msg, char **filenames)
{
  char *file;
...
static OSSL_CMP_MSG *
read_PKIMESSAGE (const char *desc, char **filenames)
{
  char *file;
  OSSL_CMP_MSG *ret;
 ...
 */
static OSSL_CMP_MSG *
read_write_req_resp (OSSL_CMP_CTX *ctx, const OSSL_CMP_MSG *req)
{
  ...
  if (opt_rspin != NULL)
  ...
  else
    {
      ...
      if (opt_use_mock_srv)
        {
          if (rspin_in_use)
            CMP_warn ("too few -rspin filename arguments; resorting to using "
                      "mock server");
      ...
      else
        {
          ...
          if (opt_server == NULL)
            {
              CMP_err ("missing -server or -use_mock_srv option, or too few "
                       "-rspin filename arguments");
              goto err;
            }
          if (rspin_in_use)
...
static int
set_gennames (OSSL_CMP_CTX *ctx, char *names, const char *desc)
{
  char *next;
...
static X509_STORE *
load_trusted (char *input, int for_new_cert, const char *desc)
{
...
static int
setup_cert (void *ctx, const char *file, const char *pass, const char *desc,
            add_X509_fn_t set1_fn)
{
  X509 *cert;
  int ok;
...
static int
setup_certs (char *files, const char *desc, void *ctx,
...
static int
setup_mock_crlout (void *ctx, const char *file, const char *desc)
{
  X509_CRL *crl;
  int ok;
...
setup_srv_ctx (ENGINE *engine)
{
  ...
  if (opt_srv_secret != NULL)
    ...
    {
      CMP_err ("server credentials (-srv_secret or -srv_cert) must be given "
               "if -use_mock_srv or -port is used");
      goto err;
    }
  else
    {
  ...
  if (opt_failure != INT_MIN)
    { /* option has been set explicitly */
...
/*
 * set up verification aspects of OSSL_CMP_CTX w.r.t. opts from config
 * file/CLI. Returns pointer on success, NULL on error
 */
static int
setup_verification_ctx (OSSL_CMP_CTX *ctx)
{
      ...
      if (opt_srvcert != NULL)
        {
          if (opt_trusted != NULL)
            {
              CMP_warn ("-trusted option is ignored since -srvcert option is "
                        "present");
              opt_trusted = NULL;
            }
 ...
 */
static SSL_CTX *
setup_ssl_ctx (OSSL_CMP_CTX *ctx, const char *host, ENGINE *engine)
{
 ...
 */
static int
setup_protection_ctx (OSSL_CMP_CTX *ctx, ENGINE *engine)
{
      ...
      if (pass_string != NULL)
        {
          ...
          res = OSSL_CMP_CTX_set1_secretValue (
              ctx, (unsigned char *)pass_string, (int)strlen (pass_string));
          clear_free (pass_string);
          if (res == 0)
            return 0;
        }
...
static int
set_fallback_pubkey (OSSL_CMP_CTX *ctx)
{
  ...
  *end = bak;
  ...
  if (pkey == NULL || !OSSL_CMP_CTX_set0_newPkey (ctx, 0 /* priv */, pkey1))
    {
      EVP_PKEY_free (pkey1);
      CMP_err1 ("failed to get fallback public key obtained from ir/cr/kur "
                "file '%s'",
                file);
      goto err;
    }
 ...
 */
static int
setup_request_ctx (OSSL_CMP_CTX *ctx, ENGINE *engine)
{
  X509_REQ *csr = NULL;
  ...
  if (opt_subject == NULL && opt_csr == NULL && opt_oldcert == NULL
    ...
    CMP_warn ("no -subject given; no -csr or -oldcert or -cert available for "
              "fallback");
  ...
  if (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR)
    {
      if (opt_reqin == NULL && opt_newkey == NULL && !opt_centralkeygen
        ...
        {
          CMP_err ("missing -newkey (or -key) to be certified and no -csr, "
                   "-oldcert, -cert, or -reqin option given, which could "
                   "provide fallback public key."
      ...
      if (opt_newkey == NULL && opt_popo != OSSL_CRMF_POPO_NONE
          && opt_popo != OSSL_CRMF_POPO_RAVERIFIED)
        {
          if (opt_csr != NULL)
            {
              CMP_err1 ("no -newkey option given with private key for POPO, "
                        "-csr option provides just public key%s",
                        opt_key == NULL
                            ? ""
                            : ", and -key option superseded by -csr");
              if (opt_reqin != NULL)
                CMP_info ("since -reqin is used, may use -popo -1 or -popo 0 "
                          "to disable the needless generation of a POPO");
              return 0;
            }
          if (opt_key == NULL)
            {
              CMP_err ("missing -newkey (or -key) option for key to be "
                       "certified and for POPO");
              return 0;
            }
        }
      if (opt_certout == NULL && opt_reqout_only == NULL)
        {
          CMP_err ("-certout not given, nowhere to save newly enrolled "
                   "certificate");
          return 0;
        }
  ...
  if (opt_cmd == CMP_KUR)
    {
      ...
      if (ref_cert == NULL && opt_csr == NULL)
        {
          CMP_err ("missing -oldcert for certificate to be updated and no "
                   "-csr given");
          return 0;
        }
      if (opt_subject != NULL)
      ...
      if (opt_issuer == NULL && opt_serial == NULL)
        {
          if (opt_oldcert == NULL && opt_csr == NULL)
            {
              CMP_err ("missing -oldcert or -issuer and -serial for "
                       "certificate to be revoked and no -csr given");
              return 0;
            }
          if (opt_oldcert != NULL && opt_csr != NULL)
            CMP_warn (
...
static int
add_certProfile (OSSL_CMP_CTX *ctx, const char *name)
{
...
static int
handle_opt_geninfo (OSSL_CMP_CTX *ctx)
{
 ...
 */
static int
setup_client_ctx (OSSL_CMP_CTX *ctx, ENGINE *engine)
{
  int ret = 0;
...
static int
write_crl (BIO *bio, X509_CRL *crl)
{
 ...
 */
static int
save_free_certs (STACK_OF (X509) *certs, const char *file, const char *desc)
{
  BIO *bio = NULL;
  int i;
...
static int
save_crl (X509_CRL *crl, const char *file, const char *desc)
{
  BIO *bio = NULL;
  int res = 0;
...
static int
delete_file (const char *file, const char *desc)
{
  if (file == NULL)
    return 1;
  ...
  if (unlink (file) != 0 && errno != ENOENT)
    {
      CMP_err2 ("Failed to delete %s, which should be done to indicate there "
                "is no %s",
                file, desc);
      return 0;
    }
...
static int
save_cert_or_delete (X509 *cert, const char *file, const char *desc)
{
  if (file == NULL)
    return 1;
  if (cert == NULL)
...
static int
save_crl_or_delete (X509_CRL *crl, const char *file, const char *desc)
{
  if (file == NULL)
    return 1;
...
static int
save_template (const char *file, const OSSL_CRMF_CERTTEMPLATE *tmpl)
{
...
static int
save_keyspec (const char *file, const OSSL_CMP_ATAVS *keyspec)
{
  BIO *bio = BIO_new_file (file, "wb");
...
static int
print_itavs (const STACK_OF (OSSL_CMP_ITAV) *itavs)
{
  int i, ret = 1;
  int n = sk_OSSL_CMP_ITAV_num (itavs);

  if (n <= 0)
    { /* also in case itavs == NULL */
...
static const char *
prev_item (const char *opt, const char *end)
{
  const char *beg;
  size_t len;
  ...
  if (len > SECTION_NAME_MAX)
    {
      CMP_warn3 ("using only first %d characters of section name starting "
                 "with \"%.*s\"",
 ...
 */
static char *
conf_get_string (const CONF *src_conf, const char *groups, const char *name)
{
  char *res = NULL;
...
static int
conf_get_number_e (const CONF *conf_, const char *groups, const char *name,
                   long *result)
{
...
#if !defined(OPENSSL_NO_SOCK) && !defined(OPENSSL_NO_HTTP)
static int
cmp_server (OSSL_CMP_CTX *srv_cmp_ctx)
{
  BIO *acbio;
  BIO *cbio = NULL;
  int keep_alive = 0;
...
static void
print_keyspec (OSSL_CMP_ATAVS *keySpec)
{
...
static int
handle_opts_upfront (int argc, char **argv)
{
  int i;
            ...
            { /* default */
              if (!NCONF_get_section (conf, opt_section))
                CMP_info2 ("no [%s] section found in config file '%s';"
                           " will thus use just [default] and unnamed section "
                           "if present",
                           opt_section, configfile);
            }
          else
  ...
  if (opt_rspin != NULL)
    {
      ...
      if (opt_use_mock_srv)
        CMP_warn ("-use_mock_srv option is not used if enough filenames given "
                  "for -rspin");
    }
#endif
              ...
              if (opt_newkeypass != NULL)
                {
                  ...
                  cipher = EVP_CIPHER_fetch (
                      app_get0_libctx (), SN_aes_256_cbc, app_get0_propq ());
                }
