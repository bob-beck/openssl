...
static int
print_to_bio_out(const char* func,
                 const char* file,
                 int line,
...
static int
print_to_bio_err(const char* func,
                 const char* file,
                 int line,
                 OSSL_CMP_severity level,
                 const char* msg)
...
setup_srv_ctx(ENGINE* engine)
{
  ...
  if (opt_srv_ref == NULL) {
    ...
    if (!OSSL_CMP_CTX_set1_referenceValue(
          ctx, (unsigned char*)opt_srv_ref, (int)strlen(opt_srv_ref)))
      goto err;
  }
  ...
  if (opt_srv_secret != NULL) {
    ...
    if (pass_str != NULL) {
      ...
      res = OSSL_CMP_CTX_set1_secretValue(
        ctx, (unsigned char*)pass_str, (int)strlen(pass_str));
      clear_free(pass_str);
      if (res == 0)
        goto err;
    }
  ...
  if (!setup_cert(ctx,
    ...
    goto err;
  if (opt_srv_key != NULL) {
    EVP_PKEY* pkey = load_key_pwd(opt_srv_key,
                                  opt_keyform,
                                  opt_srv_keypass,
                                  engine,
  ...
  if (!setup_certs(opt_srv_untrusted,
                   "untrusted certificates for mock server",
                   ctx,
  ...
  if (opt_rsp_key != NULL) {
    EVP_PKEY* pkey = load_key_pwd(opt_rsp_key,
                                  opt_keyform,
                                  opt_rsp_keypass,
                                  engine,
  ...
  if (!setup_mock_crlout(
        srv_ctx, opt_rsp_crl, "CRL to be returned by the mock server"))
    goto err;
  if (!setup_certs(opt_rsp_extracerts,
                   "CMP extra certificates for mock server",
                   srv_ctx,
                   (add_X509_stack_fn_t)ossl_cmp_mock_srv_set1_chainOut))
    goto err;
  if (!setup_certs(opt_rsp_capubs,
                   "caPubs for mock server",
                   srv_ctx,
...
setup_ssl_ctx(OSSL_CMP_CTX* ctx, const char* host, ENGINE* engine)
{
  ...
  if (opt_tls_cert != NULL && opt_tls_key != NULL) {
    ...
    if (!load_cert_certs(opt_tls_cert,
                         &cert,
                         &certs,
                         0,
                         opt_tls_keypass,
    ...
    CMP_warn("-tls_used given without -tls_key; cannot authenticate to the TLS "
             "server");
  }
  if (trust_store != NULL) {
    /*
...
setup_protection_ctx(OSSL_CMP_CTX* ctx, ENGINE* engine)
{
  ...
  if (opt_ref != NULL && !OSSL_CMP_CTX_set1_referenceValue(
                           ctx, (unsigned char*)opt_ref, (int)strlen(opt_ref)))
    return 0;

  if (opt_key != NULL) {
    EVP_PKEY* pkey = load_key_pwd(opt_key,
                                  opt_keyform,
                                  opt_keypass,
                                  engine,
  ...
  if (!setup_certs(opt_extracerts,
                   "extra certificates for CMP",
                   ctx,
...
set_fallback_pubkey(OSSL_CMP_CTX* ctx)
{
  ...
  *end = bak;
  ...
  if (req == NULL) {
    CMP_err1("failed to load ir/cr/kur file '%s' attempting to get fallback "
             "public key",
             file);
    return 0;
  }
  ...
  if (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR) {
    if (opt_reqin == NULL && opt_newkey == NULL && !opt_centralkeygen &&
      ...
      CMP_err("missing -newkey (or -key) to be certified and no -csr, "
              "-oldcert, -cert, or -reqin option given, which could provide "
              "fallback public key."
    ...
    if (opt_subject != NULL) {
      if (opt_ref == NULL && opt_cert == NULL) {
        ...
        if (!set_name(
              opt_subject, OSSL_CMP_CTX_set1_subjectName, ctx, "subject"))
          return 0;
      } else {
...
#define OSSL_CMP_RR_MSG "since -issuer and -serial is given for command 'rr'"
    ...
    if (opt_revreason > CRL_REASON_NONE)
      (void)OSSL_CMP_CTX_set_option(
        ctx, OSSL_CMP_OPT_REVOCATION_REASON, opt_revreason);
  } else {
    if (opt_serial != NULL)
...
set_path:
  ...
  if (opt_msg_timeout >= 0) /* must do this before setup_ssl_ctx() */
    (void)OSSL_CMP_CTX_set_option(
      ctx, OSSL_CMP_OPT_MSG_TIMEOUT, opt_msg_timeout);
  if (opt_total_timeout >= 0)
    (void)OSSL_CMP_CTX_set_option(
      ctx, OSSL_CMP_OPT_TOTAL_TIMEOUT, opt_total_timeout);
...
save_free_certs(STACK_OF(X509)* certs, const char* file, const char* desc)
{
  ...
  if (certs != NULL)
    CMP_info3(
      "received %d %s certificate(s), saving to file '%s'", n, desc, file);
...
save_template(const char* file, const OSSL_CRMF_CERTTEMPLATE* tmpl)
{
  ...
  if (!ASN1_i2d_bio_of(
        OSSL_CRMF_CERTTEMPLATE, i2d_OSSL_CRMF_CERTTEMPLATE, bio, tmpl)) {
...
static int
conf_get_number_e(const CONF* conf_,
                  const char* groups,
                  const char* name,
                  long* result)
{
...
cmp_server(OSSL_CMP_CTX* srv_cmp_ctx)
{
  ...
  while (opt_max_msgs <= 0 || msgs < opt_max_msgs) {
    ...
    if (req != NULL) {
      ...
      ret = http_server_send_asn1_resp(prog,
                                       cbio,
                                       keep_alive,
                                       "application/pkixcmp",
    ...
    switch (nid) {
      case NID_id_regCtrl_algId: {
        ...
        if (paramtype == V_ASN1_UNDEF || alg->parameter == NULL) {
          ...
          BIO_printf(mem, " - ");
          ASN1_item_print(
            mem, (ASN1_VALUE*)alg, 0, ASN1_ITEM_rptr(X509_ALGOR), NULL);
        }
      } break;
      case NID_id_regCtrl_rsaKeyLen:
        BIO_printf(
          mem, "Key algorithm: RSA %d\n", OSSL_CMP_ATAV_get_rsaKeyLen(atav));
        break;
      default:
...
do_genm(OSSL_CMP_CTX* ctx)
{
  if (opt_infotype == NID_id_it_caCerts) {
    ...
    if (opt_oldwithold == NULL) {
      ...
      oldwithold =
        load_cert_pwd(opt_oldwithold,
                      opt_otherpass,
                      "OldWithOld cert for genm with -infotype rootCaCert");
      if (oldwithold == NULL)
        goto end_upd;
    }
    if (!OSSL_CMP_get1_rootCaKeyUpdate(
          ctx, oldwithold, &newwithnew, &newwithold, &oldwithnew))
      goto end_upd;
    ...
    if (save_cert_or_delete(
          newwithnew, opt_newwithnew, "NewWithNew cert from genp") &&
        save_cert_or_delete(
          newwithold, opt_newwithold, "NewWithOld cert from genp") &&
        save_cert_or_delete(
          oldwithnew, opt_oldwithnew, "OldWithNew cert from genp"))
      res = 1;
  ...
  end_upd:
    ...
    if (opt_crlcert != NULL) {
      crlcert = load_cert_pwd(opt_crlcert,
                              opt_otherpass,
...
cmp_main(int argc, char** argv)
{
  ...
  if (opt_rspin != NULL) {
    ...
    if (opt_use_mock_srv)
      CMP_warn("-use_mock_srv option is not used if enough filenames given for "
               "-rspin");
  }
#endif
  ...
  for (i = 0; i < opt_repeat; i++) {
    ...
    if (newcert != NULL && (opt_cmd == CMP_IR || opt_cmd == CMP_CR ||
      ...
      if (newcert != NULL && newchain != NULL /* NULL is on error only */
        ...
        if (!save_free_certs(
              newchain, opt_chainout, "newly enrolled cert and chain"))
          goto err;
      } else {
      ...
      if (save_free_certs(
            OSSL_CMP_CTX_get1_caPubs(cmp_ctx), opt_cacertsout, "CA") < 0)
        goto err;
      if (opt_centralkeygen) {
        ...
        if (PEM_write_bio_PrivateKey(
              out, new_key, cipher, NULL, 0, NULL, (void*)pass_string) <= 0)
          result = 0;
