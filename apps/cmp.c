...
const OPTIONS cmp_options[] = {
    ...
    {"infotype", OPT_INFOTYPE, 's',
     "InfoType name for requesting specific info in genm, with specific "
     "support"},
    {OPT_MORE_STR, 0, 0, "for 'caCerts' and 'rootCaCert'"},
    {"profile", OPT_PROFILE, 's',
     "Certificate profile name to place in generalInfo field of request "
     "PKIHeader"},
    ...
    {"centralkeygen", OPT_CENTRALKEYGEN, '-',
     "Request central (server-side) key generation. Default is local "
     "generation"},
    ...
    {"sans", OPT_SANS, 's',
     "Subject Alt Names (IPADDR/DNS/URI) to add as (critical) cert req "
     "extension"},
    {"san_nodefault", OPT_SAN_NODEFAULT, '-',
     "Do not take default SANs from reference certificate (see -oldcert)"},
    {"policies", OPT_POLICIES, 's',
     "Name of config file section defining policies certificate request "
     "extension"},
...
#if defined(OPENSSL_NO_SOCK) || defined(OPENSSL_NO_HTTP)
    {OPT_MORE_STR, 0, 0,
     "NOTE: -server, -proxy, and -no_proxy not supported due to "
    ...
    {"trusted", OPT_TRUSTED, 's',
     "Certificates to use as trust anchors when verifying signed CMP "
     "responses"},
    ...
    {"srvcert", OPT_SRVCERT, 's',
     "Server cert to pin and trust directly when verifying signed CMP "
     "responses"},
    {"expect_sender", OPT_EXPECT_SENDER, 's',
     "DN of expected sender of responses. Defaults to subject of -srvcert, if "
     "any"},
    {"ignore_keyusage", OPT_IGNORE_KEYUSAGE, '-',
     "Ignore CMP signer cert key usage, else 'digitalSignature' must be "
     "allowed"},
    {"unprotected_errors", OPT_UNPROTECTED_ERRORS, '-',
     "Accept missing or invalid protection of regular error messages and "
     "negative"},
    {OPT_MORE_STR, 0, 0,
     "certificate responses (ip/cp/kup), revocation responses (rp), and "
     "PKIConf"},
    {OPT_MORE_STR, 0, 0,
    ...
    {"srvcertout", OPT_SRVCERTOUT, 's',
     "File to save the server cert used and validated for CMP response "
     "protection"},
    ...
    {"cacertsout", OPT_CACERTSOUT, 's',
     "File to save CA certs received in caPubs field or genp with "
     "id-it-caCerts"},
    ...
    {"secret", OPT_SECRET, 's',
     "Prefer PBM (over signatures) for protecting msgs with given password "
     "source"},
    {"cert", OPT_CERT, 's',
    ...
    {"digest", OPT_DIGEST, 's',
     "Digest to use in message protection and POPO signatures. Default "
     "\"sha256\""},
    {"mac", OPT_MAC, 's',
     "MAC algorithm to use in PBM-based message protection. Default "
     "\"hmac-sha1\""},
    ...
    {"certform", OPT_CERTFORM, 's',
     "Format (PEM or DER) to use when saving a certificate to a file. Default "
     "PEM"},
    ...
    {"otherpass", OPT_OTHERPASS, 's',
     "Pass phrase source potentially needed for loading certificates of "
     "others"},
#ifndef OPENSSL_NO_ENGINE
    {"engine", OPT_ENGINE, 's',
    ...
    {"tls_cert", OPT_TLS_CERT, 's',
     "Client's TLS certificate. May include chain to be provided to TLS "
     "server"},
    ...
    {"srv_ref", OPT_SRV_REF, 's',
     "Reference value to use as senderKID of server in case no -srv_cert is "
     "given"},
    ...
    {"rsp_key", OPT_RSP_KEY, 's',
     "Private key for the certificate to be returned as mock enrollment "
     "result"},
    ...
    {"failurebits", OPT_FAILUREBITS, 'N',
     "Number representing failure bits to include in server response, 0..2^27 "
     "- 1"},
...
static OSSL_CMP_MSG* read_write_req_resp(OSSL_CMP_CTX* ctx,
  ...
  if (opt_reqout != NULL && !write_PKIMESSAGE(req, &opt_reqout)) goto err;
  if (opt_reqin != NULL && opt_rspin == NULL) {
     ...
     */
    if (!OSSL_CMP_MSG_update_recipNonce(ctx, req_new)) goto err;
  }
  ...
  if (opt_rspin != NULL) {
    ...
    if (opt_use_mock_srv) {
      if (rspin_in_use)
        CMP_warn(
            "too few -rspin filename arguments; resorting to using mock "
            "server");
      ...
      if (rspin_in_use)
        CMP_warn(
            "too few -rspin filename arguments; resorting to contacting "
            "server");
...
static int setup_cert(void* ctx, const char* file, const char* pass,
  ...
  if ((cert = load_cert_pwd(file, pass, desc)) == NULL) return 0;
  ok = (*set1_fn)(ctx, cert);
  X509_free(cert);
  return ok;
}
...
static int setup_mock_crlout(void* ctx, const char* file, const char* desc) {
  ...
  if (file == NULL) return 1;
  if ((crl = load_crl(file, FORMAT_UNDEF, 0, desc)) == NULL) return 0;
...
static OSSL_CMP_SRV_CTX* setup_srv_ctx(ENGINE* engine) {
  ...
  if (opt_srv_trusted != NULL) {
    ...
    CMP_warn(
        "mock server will not be able to handle signature-protected requests "
        "since -srv_trusted is not given");
  }
  ...
  if (opt_send_error) (void)ossl_cmp_mock_srv_set_sendError(srv_ctx, 1);
...
static SSL_CTX* setup_ssl_ctx(OSSL_CMP_CTX* ctx, const char* host,
  ...
  if (opt_tls_trusted != NULL) {
    ...
    if (trust_store == NULL) goto err;
    SSL_CTX_set_cert_store(ssl_ctx, trust_store);
    SSL_CTX_set_verify(ssl_ctx, SSL_VERIFY_PEER, NULL);
  } else {
    CMP_warn(
        "-tls_used given without -tls_trusted; will not authenticate the TLS "
        "server");
  }
  ...
  if (opt_tls_cert != NULL && opt_tls_key != NULL) {
...

    {
      ...
      if (trust_store != NULL) X509_VERIFY_PARAM_set_flags(tls_vpm, bak_flags);
    }
    ...
    if (opt_tls_extra != NULL) {
      ...
      if (tls_extra == NULL) goto err;
      for (i = 0; i < sk_X509_num(tls_extra); i++) {
        cert = sk_X509_value(tls_extra, i);
        if (res != 0) res = SSL_CTX_add_extra_chain_cert(ssl_ctx, cert);
        if (res == 0) X509_free(cert);
      }
    ...
    CMP_warn(
        "-tls_used given without -tls_key; cannot authenticate to the TLS "
        "server");
  }
  if (trust_store != NULL) {
    /*
...
static int setup_protection_ctx(OSSL_CMP_CTX* ctx, ENGINE* engine) {
  ...
  if (opt_secret == NULL && opt_srvcert == NULL && opt_trusted == NULL)
    CMP_warn(
        "will not authenticate server due to missing -secret, -trusted, or "
        "-srvcert");
...
static int set_fallback_pubkey(OSSL_CMP_CTX* ctx) {
  ...
  *end = bak;
  ...
  if (req == NULL) {
    CMP_err1(
        "failed to load ir/cr/kur file '%s' attempting to get fallback public "
        "key",
        file);
    return 0;
  }
...
static int setup_request_ctx(OSSL_CMP_CTX* ctx, ENGINE* engine) {
  ...
  if (opt_subject == NULL && opt_csr == NULL && opt_oldcert == NULL &&
    ...
    CMP_warn(
        "no -subject given; no -csr or -oldcert or -cert available for "
        "fallback");

  if (!set_name(opt_issuer, OSSL_CMP_CTX_set1_issuer, ctx, "issuer")) return 0;
  if (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR) {
    if (opt_reqin == NULL && opt_newkey == NULL && !opt_centralkeygen &&
      ...
      CMP_err(
          ...
          "-cert, or -reqin option given, which could provide fallback public "
          "key."
    ...
    if (opt_newkey == NULL && opt_popo != OSSL_CRMF_POPO_NONE &&
      ...
      if (opt_csr != NULL) {
        CMP_err1(
            "no -newkey option given with private key for POPO, -csr option "
            "provides just public key%s",
            opt_key == NULL ? "" : ", and -key option superseded by -csr");
        if (opt_reqin != NULL)
          CMP_info(
              "since -reqin is used, may use -popo -1 or -popo 0 to disable "
              "the needless generation of a POPO");
        return 0;
      }
      if (opt_key == NULL) {
        CMP_err(
            "missing -newkey (or -key) option for key to be certified and for "
            "POPO");
        return 0;
      }
    }
    ...
    if (opt_reqexts != NULL) CMP_warn1("-reqexts %s", msg);
    if (opt_san_nodefault) CMP_warn1("-san_nodefault %s", msg);
    if (opt_sans != NULL) CMP_warn1("-sans %s", msg);
    if (opt_policies != NULL) CMP_warn1("-policies %s", msg);
    if (opt_policy_oids != NULL) CMP_warn1("-policy_oids %s", msg);
    if (opt_popo != OSSL_CRMF_POPO_NONE - 1) CMP_warn1("-popo %s", msg);
    if (opt_centralkeygen) CMP_warn1("-popo -1 or -centralkeygen %s", msg);
    if (opt_newkeyout != NULL) CMP_warn1("-newkeyout %s", msg);
    if (opt_cmd != CMP_P10CR) {
      if (opt_implicit_confirm)
        CMP_warn1("-implicit_confirm %s, and 'p10cr'", msg);
      if (opt_disable_confirm)
        CMP_warn1("-disable_confirm %s, and 'p10cr'", msg);
      if (opt_certout != NULL) CMP_warn1("-certout %s, and 'p10cr'", msg);
      if (opt_chainout != NULL) CMP_warn1("-chainout %s, and 'p10cr'", msg);
    }
  }
  if (opt_cmd == CMP_KUR) {
...
#define OSSL_CMP_RR_MSG "since -issuer and -serial is given for command 'rr'"
      ...
      if (opt_oldcert != NULL) CMP_warn("Ignoring -oldcert " OSSL_CMP_RR_MSG);
      if (opt_csr != NULL) CMP_warn("Ignoring -csr " OSSL_CMP_RR_MSG);
    }
    if (opt_serial != NULL) {
      ASN1_INTEGER* sno;
  ...
  if (opt_recipient == NULL && opt_srvcert == NULL && opt_issuer == NULL &&
    ...
    CMP_warn(
        ...
        "for any requests not covered by -reqin will be set to \"NULL-DN\"");

  if (opt_cmd == CMP_P10CR || opt_cmd == CMP_RR || opt_cmd == CMP_GENM) {
    const char* msg =
        "option is ignored for 'p10cr', 'rr', and 'genm' commands";

    if (opt_newkeypass != NULL) CMP_warn1("-newkeypass %s", msg);
    if (opt_newkey != NULL) CMP_warn1("-newkey %s", msg);
    if (opt_days != 0) CMP_warn1("-days %s", msg);
    if (opt_popo != OSSL_CRMF_POPO_NONE - 1) CMP_warn1("-popo %s", msg);
    if (opt_out_trusted != NULL) CMP_warn1("-out_trusted %s", msg);
    ...
    if (!set_fallback_pubkey(ctx)) return 0;
  }
  ...
  if (opt_csr != NULL) {
    if (opt_cmd == CMP_GENM) {
      ...
      if (!OSSL_CMP_CTX_set1_p10CSR(ctx, csr)) goto oom;
    }
  }
  if (opt_reqexts != NULL || opt_policies != NULL) {
    if ((exts = sk_X509_EXTENSION_new_null()) == NULL) goto oom;
  ...
  if (OSSL_CMP_CTX_reqExtensions_have_SAN(ctx) && opt_sans != NULL) {
    CMP_err(
        "cannot have Subject Alternative Names both via -reqexts and via "
        "-sans");
    return 0;
  }
  ...
  if (opt_policy_oids_critical) {
    if (opt_policy_oids == NULL)
      CMP_warn(
          "-opt_policy_oids_critical has no effect unless -policy_oids is "
          "given");
...
static int add_certProfile(OSSL_CMP_CTX* ctx, const char* name) {
  ...
  if ((utf8string = ASN1_UTF8STRING_new()) == NULL) goto err;
  ...
  if ((itav = OSSL_CMP_ITAV_new0_certProfile(sk)) == NULL) goto err;
  if (OSSL_CMP_CTX_push0_geninfo_ITAV(ctx, itav)) return 1;
...
static int handle_opt_geninfo(OSSL_CMP_CTX* ctx) {
  ...
  do {
    ...
    *ptr++ = '\0';
    ...
    if ((type = ASN1_TYPE_new()) == NULL) goto oom;
...
static int setup_client_ctx(OSSL_CMP_CTX* ctx, ENGINE* engine) {
  ...
  if (!opt_use_mock_srv) strcpy(server_buf, "no server");
...
#else
    CMP_err(
        "missing -use_mock_srv or -rspin option; -server option is not "
  ...
  if (opt_path == NULL) used_path = path;
  if (!OSSL_CMP_CTX_set1_server(ctx, host) ||
      !OSSL_CMP_CTX_set_serverPort(ctx, portnum))
    goto oom;
  if (opt_proxy != NULL && !OSSL_CMP_CTX_set1_proxy(ctx, opt_proxy)) goto oom;
...
set_path:
  ...
  if (!OSSL_CMP_CTX_set1_serverPath(ctx, used_path)) goto oom;
  if (!transform_opts()) goto err;

  if (opt_infotype_s == NULL) {
    if (opt_cmd == CMP_GENM) CMP_warn("no -infotype option given for genm");
  ...
  if (opt_cmd != CMP_GENM || opt_infotype != NID_id_it_rootCaCert) {
    const char* msg =
        "option is ignored unless -cmd 'genm' and -infotype rootCaCert is "
        "given";

    if (opt_oldwithold != NULL) CMP_warn1("-oldwithold %s", msg);
    if (opt_newwithnew != NULL) CMP_warn1("-newwithnew %s", msg);
    if (opt_newwithold != NULL) CMP_warn1("-newwithold %s", msg);
    if (opt_oldwithnew != NULL) CMP_warn1("-oldwithnew %s", msg);
  }
  if (opt_cmd != CMP_GENM || opt_infotype != NID_id_it_certReqTemplate) {
    const char* msg =
        "option is ignored unless -cmd 'genm' and -infotype 'certReqTemplate' "
        "is given";

    if (opt_template != NULL) CMP_warn1("-template %s", msg);
    if (opt_keyspec != NULL) CMP_warn1("-keyspec %s", msg);
  } else {
  ...
  if (!setup_verification_ctx(ctx)) goto err;
  ...
  if (opt_tls_used) {
    ...
    if ((info = OPENSSL_zalloc(sizeof(*info))) == NULL) goto err;
    ...
    if ((info->port = OPENSSL_strdup(server_port)) == NULL) goto err;
    /* workaround for callback design flaw, see #17088: */
    info->use_proxy = proxy_host != NULL;
    info->timeout = OSSL_CMP_CTX_get_option(ctx, OSSL_CMP_OPT_MSG_TIMEOUT);

    if (info->ssl_ctx == NULL) goto err;
    (void)OSSL_CMP_CTX_set_http_cb(ctx, app_http_tls_cb);
  }
#endif

  if (!setup_protection_ctx(ctx, engine)) goto err;

  if (!setup_request_ctx(ctx, engine)) goto err;
  ...
  if (opt_geninfo != NULL && !handle_opt_geninfo(ctx)) goto err;
  if (opt_profile != NULL && !add_certProfile(ctx, opt_profile)) goto err;
...
static int save_crl(X509_CRL* crl, const char* file, const char* desc) {
  ...
  if (file == NULL) return 1;
  if (crl != NULL) CMP_info2("received %s, saving to file '%s'", desc, file);
...
static const char* nid_name(int nid) {
  ...
  if (name == NULL) name = OBJ_nid2sn(nid);
  if (name == NULL) name = "<unknown OID>";
  return name;
}
...
static int conf_get_number_e(const CONF* conf_, const char* groups,
  ...
  if (res == LONG_MIN || res == LONG_MAX || *tailptr != '\0') return 0;
...
static int read_config(void) {
  ...
  for (opt = &cmp_options[start_opt], i = start_idx; opt->name != NULL;
    ...
    if (provider_option || rand_state_option || verification_option) i--;
    switch (opt->valtype) {
      case '-':
      case 'p':
      case 'n':
...
static int get_opts(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_SECTION: /* has already been handled */
        break;
      case OPT_VERBOSITY:
        if (!set_verbosity(opt_int_arg())) goto opthelp;
        break;
#if !defined(OPENSSL_NO_SOCK) && !defined(OPENSSL_NO_HTTP)
      case OPT_SERVER:
        opt_server = opt_str();
      ...
      case OPT_V_CASES:
        if (!opt_verify(o, vpm)) goto opthelp;
        break;
      case OPT_CMD:
        opt_cmd_s = opt_str();
        break;
...
#endif
      case OPT_PROV_CASES:
        if (!opt_provider(o)) goto opthelp;
        break;
      case OPT_R_CASES:
        if (!opt_rand(o)) goto opthelp;
        break;
  ...
  /* No extra args. */
  if (!opt_check_rest_arg(NULL)) goto opthelp;
  return 1;
}
...
static int cmp_server(OSSL_CMP_CTX* srv_cmp_ctx) {
  ...
  while (opt_max_msgs <= 0 || msgs < opt_max_msgs) {
    ...
    if (req != NULL) {
      ...
      if (!ret) break; /* treated as fatal error */
    }
  next:
...
static int do_genm(OSSL_CMP_CTX* ctx) {
  if (opt_infotype == NID_id_it_caCerts) {
    ...
    if (opt_oldwithold == NULL) {
      CMP_warn(
          "No -oldwithold given, will use all certs given with -trusted as "
          "trust anchors for verifying the newWithNew cert");
    } else {
      oldwithold =
          load_cert_pwd(opt_oldwithold, opt_otherpass,
                        "OldWithOld cert for genm with -infotype rootCaCert");
      if (oldwithold == NULL) goto end_upd;
    }
    ...
    if (newwithnew == NULL)
      ...
      CMP_warn(
          "oldWithNew certificate received in genp for verifying oldWithOld, "
          "but oldWithOld was not provided");
  ...
  end_upd:
    ...
    if (!OSSL_CMP_get1_crlUpdate(ctx, crlcert, oldcrl, &crl)) goto end_crlupd;

    if (crl == NULL) CMP_info("no CRL update available");
    if (!save_crl_or_delete(crl, opt_crlout, desc)) goto end_crlupd;
  ...
  end_crlupd:
    ...
    if (!OSSL_CMP_get1_certReqTemplate(ctx, &certTemplate, &keySpec)) return 0;

    if (certTemplate == NULL) {
      CMP_warn("no certificate request template available");
      if (!delete_file(opt_template, "certTemplate from genp")) return 0;
      if (opt_keyspec != NULL && !delete_file(opt_keyspec, "keySpec from genp"))
        return 0;
      return 1;
    }
    if (!save_template(opt_template, certTemplate)) goto tmpl_end;

    print_keyspec(keySpec);
    if (opt_keyspec != NULL) {
      if (keySpec == NULL) {
        CMP_warn("no key specifications available");
        if (!delete_file(opt_keyspec, "keySpec from genp")) goto tmpl_end;
...
int cmp_main(int argc, char** argv) {
  ...
  if (!handle_opts_upfront(argc, argv)) goto err;
  ...
  if (configfile != NULL && configfile[0] != '\0' /* non-empty string */
    ...
    if (conf == NULL) {
      ...
      if (!set_verbosity(opt_verbosity)) /* just for checking range */
        ret = -1;
      if (ret <= 0) {
        if (ret == -1) BIO_printf(bio_err, "Use -help for summary.\n");
        goto err;
      }
    }
  }
  ...
  if (cmp_ctx == NULL) goto err;

  ret = get_opts(argc, argv);
  if (ret <= 0) goto err;

  ret = 0;
  if (!app_RAND_load()) goto err;

  if (opt_batch) set_base_ui_method(UI_null());
  ...
  if (opt_tls_cert == NULL && opt_tls_key == NULL && opt_tls_keypass == NULL &&
      ...
      opt_tls_host == NULL) {
    if (opt_tls_used) CMP_warn("-tls_used given without any other TLS options");
  ...
  if (opt_reqout_only == NULL && (opt_use_mock_srv
    ...
    if ((srv_ctx = setup_srv_ctx(engine)) == NULL) goto err;
  ...
  if (opt_tls_used && (opt_use_mock_srv || opt_server == NULL)) {
    CMP_warn(
        "ignoring -tls_used option since -use_mock_srv is given or -server is "
        "not given");
    opt_tls_used = 0;
  }
...
#if !defined(OPENSSL_NO_SOCK) && !defined(OPENSSL_NO_HTTP)
    if (opt_server != NULL) CMP_warn1("-server %s", msg);
#endif
    if (opt_use_mock_srv) CMP_warn1("-use_mock_srv %s", msg);
    if (opt_reqout != NULL) CMP_warn1("-reqout %s", msg);
    if (opt_rspin != NULL) CMP_warn1("-rspin %s", msg);
    if (opt_rspout != NULL) CMP_warn1("-rspout %s", msg);
  ...
  if (opt_rspin != NULL) {
    ...
    if (opt_use_mock_srv)
      CMP_warn(
          "-use_mock_srv option is not used if enough filenames given for "
          "-rspin");
  }
#endif
  ...
  for (i = 0; i < opt_repeat; i++) {
    ...
    if (!OSSL_CMP_CTX_reinit(cmp_ctx)) goto err;
  }
  ret = 1;
...
err:
  ...
  if (ret != 1) OSSL_CMP_CTX_print_errors(cmp_ctx);
