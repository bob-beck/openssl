...
#else
            CMP_err("-server not supported on no-sock/no-http build; missing -use_mock_srv option or too few -rspin "
                    "filename arguments");
#endif
        }
        rspin_in_use = 0;
...
static OSSL_CMP_SRV_CTX *setup_srv_ctx(ENGINE *engine)
{
    ...
    {
        if (!OSSL_CMP_CTX_set1_referenceValue(ctx, (unsigned char *)opt_srv_ref, (int)strlen(opt_srv_ref)))
            goto err;
    }
    ...
    if (opt_srv_secret != NULL)
    {
        ...
        if (pass_str != NULL)
        {
            ...
            res = OSSL_CMP_CTX_set1_secretValue(ctx, (unsigned char *)pass_str, (int)strlen(pass_str));
            clear_free(pass_str);
            if (res == 0)
                goto err;
        }
    ...
    if (opt_tls_cert != NULL && opt_tls_key != NULL)
    {
...

        {
            ...
            if (trust_store != NULL)
            {
                ...
                X509_VERIFY_PARAM_clear_flags(tls_vpm, ~(X509_V_FLAG_USE_CHECK_TIME | X509_V_FLAG_NO_CHECK_TIME |
...
static int setup_protection_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
    ...
    if (opt_secret != NULL)
    {
        ...
        {
            cleanse(opt_secret);
            res = OSSL_CMP_CTX_set1_secretValue(ctx, (unsigned char *)pass_string, (int)strlen(pass_string));
            clear_free(pass_string);
            if (res == 0)
                return 0;
        }
    ...
    if (opt_ref != NULL && !OSSL_CMP_CTX_set1_referenceValue(ctx, (unsigned char *)opt_ref, (int)strlen(opt_ref)))
        return 0;
...
static int setup_request_ctx(OSSL_CMP_CTX *ctx, ENGINE *engine)
{
    ...
    if (opt_cmd == CMP_IR || opt_cmd == CMP_CR || opt_cmd == CMP_KUR)
    {
        if (opt_reqin == NULL && opt_newkey == NULL && !opt_centralkeygen && opt_key == NULL && opt_csr == NULL &&
            opt_oldcert == NULL)
        {
            CMP_err("missing -newkey (or -key) to be certified and no -csr, -oldcert, -cert, or -reqin option given, "
                    "which could provide fallback public key."
        ...
        if (opt_newkey == NULL && opt_popo != OSSL_CRMF_POPO_NONE && opt_popo != OSSL_CRMF_POPO_RAVERIFIED)
        {
            if (opt_csr != NULL)
            {
                ...
                if (opt_reqin != NULL)
                    CMP_info("since -reqin is used, may use -popo -1 or -popo 0 to disable the needless generation of "
                             "a POPO");
                return 0;
            }
            if (opt_key == NULL)
...
#define OSSL_CMP_RR_MSG "since -issuer and -serial is given for command 'rr'"
    ...
    if (opt_recipient == NULL && opt_srvcert == NULL && opt_issuer == NULL && opt_oldcert == NULL && opt_cert == NULL)
        CMP_warn("missing -recipient, -srvcert, -issuer, -oldcert or -cert; recipient for any requests not covered by "
                 "-reqin will be set to \"NULL-DN\"");
...
static int read_config(void)
{
    ...
    for (opt = &cmp_options[start_opt], i = start_idx; opt->name != NULL; i++, opt++)
        if (!strcmp(opt->name, OPT_SECTION_STR) || !strcmp(opt->name, OPT_MORE_STR))
            n_options--;
    OPENSSL_assert(OSSL_NELEM(cmp_vars) == n_options + OPT_PROV__FIRST + 1 - OPT_PROV__LAST + OPT_R__FIRST + 1 -
                                               OPT_R__LAST + OPT_V__FIRST + 1 - OPT_V__LAST);
    for (opt = &cmp_options[start_opt], i = start_idx; opt->name != NULL; i++, opt++)
    {
...
static int do_genm(OSSL_CMP_CTX *ctx)
{
    ...
    else if (opt_infotype == NID_id_it_rootCaCert)
    {
        ...
        if (opt_oldwithold == NULL)
        {
            CMP_warn("No -oldwithold given, will use all certs given with -trusted as trust anchors for verifying the "
                     "newWithNew cert");
        }
        else
        {
