...
static int print_to_bio_out(const char* func,
                            const char* file,
                            int line,
...
static int print_to_bio_err(const char* func,
                            const char* file,
                            int line,
                            OSSL_CMP_severity level,
                            const char* msg) {
...
static EVP_PKEY* load_key_pwd(const char* uri,
                              int format,
                              const char* pass,
                              ENGINE* eng,
                              const char* desc) {
...
static X509* load_cert_pwd(const char* uri,
                           const char* pass,
                           const char* desc) {
  X509* cert;
  char* pass_string = get_passwd(pass, desc);
...
static X509_STORE* load_trusted(char* input,
                                int for_new_cert,
                                const char* desc) {
...
static int setup_cert(void* ctx,
                      const char* file,
                      const char* pass,
                      const char* desc,
...
static int setup_certs(char* files,
                       const char* desc,
                       void* ctx,
 ...
 * Returns pointer on success, NULL on error
 */
static SSL_CTX* setup_ssl_ctx(OSSL_CMP_CTX* ctx,
                              const char* host,
                              ENGINE* engine) {
...
static int save_crl_or_delete(X509_CRL* crl,
                              const char* file,
                              const char* desc) {
  if (file == NULL)
    return 1;
...
static char* conf_get_string(const CONF* src_conf,
                             const char* groups,
                             const char* name) {
  char* res = NULL;
...
static int conf_get_number_e(const CONF* conf_,
                             const char* groups,
                             const char* name,
                             long* result) {
