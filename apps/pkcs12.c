...
static int
get_cert_chain(X509* cert,
               X509_STORE* store,
               STACK_OF(X509)* untrusted_certs,
               STACK_OF(X509)** chain);
int
dump_certs_keys_p12(BIO* out,
                    const PKCS12* p12,
                    const char* pass,
                    int passlen,
                    int options,
...
int
print_attribs(BIO* out,
              const STACK_OF(X509_ATTRIBUTE)* attrlst,
              const char* name);
void
hex_prin(BIO* out, unsigned char* buf, int len);
static int
alg_print(const X509_ALGOR* alg);
int
cert_load(BIO* in, STACK_OF(X509)* sk);
static int
set_pbe(int* ppbe, const char* str);
static int
jdk_trust(PKCS12_SAFEBAG* bag, void* cbarg);
  ...
  if (export_pkcs12) {
    ...
    if (!(options & NOCERTS)) {
      ...
      if (key != NULL) {
        ...
        if (ee_cert == NULL) {
          BIO_printf(
            bio_err, "No cert in -in file '%s' matches private key\n", infile);
          goto export_end;
        }
      }
    }
    ...
    if (csp_name != NULL && key != NULL)
      EVP_PKEY_add1_attr_by_NID(
        key, NID_ms_csp_name, MBSTRING_ASC, (unsigned char*)csp_name, -1);
    ...
    if (maciter != -1) {
      if (pbmac1_pbkdf2 == 1) {
        if (!PKCS12_set_pbmac1_pbkdf2(p12,
                                      mpass,
                                      -1,
                                      NULL,
                                      macsaltlen,
...
jdk_trust(PKCS12_SAFEBAG* bag, void* cbarg)
{
  ...
  attr = X509_ATTRIBUTE_create(
    NID_oracle_jdk_trustedkeyusage, V_ASN1_OBJECT, (ASN1_OBJECT*)cbarg);
                    ...
                    const EVP_CIPHER* enc)
{
  ...
  for (i = 0; i < sk_PKCS7_num(asafes); i++) {
    ...
    if (bags == NULL)
      goto err;
    if (!dump_certs_pkeys_bags(
          out, bags, pass, passlen, options, pempass, enc)) {
...
print_attribute(BIO* out, const ASN1_TYPE* av)
{
  ...
  switch (av->type) {
    ...
    case V_ASN1_OCTET_STRING:
      hex_prin(
        out, av->value.octet_string->data, av->value.octet_string->length);
      BIO_printf(out, "\n");
      break;
