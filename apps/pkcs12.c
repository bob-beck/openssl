...
int pkcs12_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_PBMAC1_PBKDF2_MD:
        ...
        break;
      case OPT_CERTPBE:
        if (!set_pbe(&cert_pbe, opt_arg())) goto opthelp;
        break;
      case OPT_KEYPBE:
        if (!set_pbe(&key_pbe, opt_arg())) goto opthelp;
        break;
      case OPT_R_CASES:
      ...
      case OPT_CANAME:
        if (canames == NULL && (canames = sk_OPENSSL_STRING_new_null()) == NULL)
          goto end;
        if (sk_OPENSSL_STRING_push(canames, opt_arg()) <= 0) goto end;
        break;
      case OPT_IN:
        infile = opt_arg();
        break;
  ...
  if (!opt_cipher_any(ciphername, &enc)) goto opthelp;
  if (export_pkcs12) {
    if ((options & INFO) != 0) WARN_EXPORT("info");
    if (macver == 0) WARN_EXPORT("nomacver");
    if ((options & CLCERTS) != 0) WARN_EXPORT("clcerts");
    if ((options & CACERTS) != 0) WARN_EXPORT("cacerts");
    if (enc != default_enc)
      BIO_printf(bio_err,
                 "Warning: output encryption option -%s ignored with -export\n",
                 enc_name);
  } else {
    if (keyname != NULL) WARN_NO_EXPORT("inkey");
    if (certfile != NULL) WARN_NO_EXPORT("certfile");
    if (passcertsarg != NULL) WARN_NO_EXPORT("passcerts");
    if (chain) WARN_NO_EXPORT("chain");
    if (untrusted != NULL) WARN_NO_EXPORT("untrusted");
    if (CAfile != NULL) WARN_NO_EXPORT("CAfile");
    if (CApath != NULL) WARN_NO_EXPORT("CApath");
    if (CAstore != NULL) WARN_NO_EXPORT("CAstore");
    if (noCAfile) WARN_NO_EXPORT("no-CAfile");
    if (noCApath) WARN_NO_EXPORT("no-CApath");
    if (noCAstore) WARN_NO_EXPORT("no-CAstore");
    if (name != NULL) WARN_NO_EXPORT("name");
    if (canames != NULL) WARN_NO_EXPORT("caname");
    if (csp_name != NULL) WARN_NO_EXPORT("CSP");
    if (add_lmk) WARN_NO_EXPORT("LMK");
    if (keytype == KEY_EX) WARN_NO_EXPORT("keyex");
    if (keytype == KEY_SIG) WARN_NO_EXPORT("keysig");
    if (key_pbe != NID_undef) WARN_NO_EXPORT("keypbe");
    if (cert_pbe != NID_undef && cert_pbe != -1)
      WARN_NO_EXPORT("certpbe and -descert");
    if (macalg != NULL) WARN_NO_EXPORT("macalg");
    if (iter != 0) WARN_NO_EXPORT("iter and -noiter");
    if (maciter == 1) WARN_NO_EXPORT("nomaciter");
    if (cert_pbe == -1 && maciter == -1) WARN_NO_EXPORT("nomac");
    if (macsaltlen != PKCS12_SALT_LEN) WARN_NO_EXPORT("macsaltlen");
  }
#ifndef OPENSSL_NO_DES
  if (use_legacy) {
    /* load the legacy provider if not loaded already*/
    if (!OSSL_PROVIDER_available(app_get0_libctx(), "legacy")) {
      if (!app_provider_load(app_get0_libctx(), "legacy")) goto end;
      /* load the default provider explicitly */
      if (!app_provider_load(app_get0_libctx(), "default")) goto end;
    }
    ...
    if (key_pbe == NID_undef) key_pbe = NID_pbe_WithSHA1And3_Key_TripleDES_CBC;
    if (enc == default_enc) enc = (EVP_CIPHER*)EVP_des_ede3_cbc();
    if (macalg == NULL) macalg = "sha1";
  }
#endif
  ...
  if (export_pkcs12) {
    ...
    if (!twopass) OPENSSL_strlcpy(macpass, pass, sizeof(macpass));
    ...
    if (macalg != NULL) {
      if (!opt_md(macalg, &macmd)) goto opthelp;
    }
    ...
    if (maciter != -1) {
      if (pbmac1_pbkdf2 == 1) {
        ...
        if (!PKCS12_set_mac(p12, mpass, -1, NULL, macsaltlen, maciter, macmd)) {
          ...
          BIO_printf(bio_err,
                     "Use -nomac or -pbmac1_pbkdf2 if PKCS12KDF support not "
                     "available\n");
          goto export_end;
        }
      }
...
int dump_certs_pkeys_bag(BIO* out, const PKCS12_SAFEBAG* bag, const char* pass,
  ...
  switch (PKCS12_SAFEBAG_get_nid(bag)) {
    case NID_keyBag:
      ...
      if ((pkey = EVP_PKCS82PKEY(p8c)) == NULL) return 0;
    ...
    case NID_pkcs8ShroudedKeyBag:
      ...
      if (options & NOKEYS) return 1;
      print_attribs(out, attrs, "Bag Attributes");
      if ((p8 = PKCS12_decrypt_skey(bag, pass, passlen)) == NULL) return 0;
    ...
    case NID_secretBag:
      if (options & INFO) BIO_printf(bio_err, "Secret bag\n");
      print_attribs(out, attrs, "Bag Attributes");
...
int cert_load(BIO* in, STACK_OF(X509)* sk) {
  ...
  while ((cert = PEM_read_bio_X509(in, NULL, NULL, NULL))) {
    ret = 1;
    if (!sk_X509_push(sk, cert)) return 0;
  }
