...
int storeutl_main(int argc, char* argv[]) {
  ...
  if (!opt_check_rest_arg("URI")) goto opthelp;
  argv = opt_rest();
...
static int process(const char* uri, const UI_METHOD* uimeth, PW_CB_DATA* uidata,
  ...
  for (;;) {
    ...
    if (info == NULL) {
      if (OSSL_STORE_error(store_ctx)) {
        if (recursive)
          ...
          ERR_print_errors(bio_err);
        if (OSSL_STORE_eof(store_ctx)) break;
        ret++;
        continue;
      }
    ...
    if (type == OSSL_STORE_INFO_NAME) {
      ...
      if (desc != NULL) indent_printf(indent, bio_out, "%s\n", desc);
    } else {
    ...
    switch (type) {
      ...
      case OSSL_STORE_INFO_CERT:
        if (text) X509_print(out, OSSL_STORE_INFO_get0_CERT(info));
        if (!noout) PEM_write_bio_X509(out, OSSL_STORE_INFO_get0_CERT(info));
        break;
      case OSSL_STORE_INFO_CRL:
        if (text) X509_CRL_print(out, OSSL_STORE_INFO_get0_CRL(info));
        if (!noout) PEM_write_bio_X509_CRL(out, OSSL_STORE_INFO_get0_CRL(info));
        break;
      default:
