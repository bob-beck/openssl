...
int
http_server_get_asn1_req(const ASN1_ITEM* it,
                         ASN1_VALUE** preq,
                         char** ppath,
                         BIO** pcbio,
                         BIO* acbio,
                         ...
                         int timeout)
{
  ...
  *preq = NULL;
  ...
  if (log_get_verbosity() < LOG_TRACE)
    trace_log_message(
      -1, prog, LOG_INFO, "received request, 1st line: %s", reqbuf);
  log_HTTP(prog, LOG_TRACE, "received request header:");
  log_HTTP1(prog, LOG_TRACE, "%s", reqbuf);
  if (end == NULL) {
    log_HTTP(
      prog, LOG_WARNING, "cannot parse HTTP header: missing end of line");
    (void)http_server_send_status(prog, cbio, 400, "Bad Request");
    goto out;
  }
    ...
    *end = '\0';
    ...
    if (strlen(meth) == 3) { /* GET */
      if ((getbio = BIO_new_mem_buf(url, len)) == NULL ||
        ...
        log_HTTP1(
          prog, LOG_ERR, "could not allocate base64 bio with size = %d", len);
        goto fatal;
      }
  ...
  *end = '\0';
  ...
  for (;;) {
    ...
    if (end == NULL) {
      log_HTTP(
        prog, LOG_WARNING, "error parsing HTTP header: missing end of line");
      (void)http_server_send_status(prog, cbio, 400, "Bad Request");
      goto out;
    }
  ...
  if (req == NULL) {
    ...
    log_HTTP1(
      prog, LOG_ERR, "out of memory allocating %zu bytes", strlen(url) + 1);
...
int
http_server_send_asn1_resp(const char* prog,
                           BIO* cbio,
                           int keep_alive,
...
int
http_server_send_status(const char* prog,
                        BIO* cbio,
                        int status,
                        const char* reason)
{
