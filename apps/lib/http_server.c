...
void socket_timeout(int signum) {
  if (acfd != (int)INVALID_SOCKET) (void)shutdown(acfd, SHUT_RD);
}
...
static void killall(int ret, pid_t* kidpids) {
  ...
  for (i = 0; i < n_responders; ++i)
    if (kidpids[i] != 0) (void)kill(kidpids[i], SIGTERM);
...
#ifdef WCOREDUMP
            if (WCOREDUMP(status)) dumped = " (core dumped)";
#endif
...
BIO* http_server_init(const char* prog, const char* port, int verb) {
  ...
  if (verb >= 0 && !log_set_verbosity(prog, verb)) return NULL;
  bufbio = BIO_new(BIO_f_buffer());
  if (bufbio == NULL) goto err;
...
int http_server_send_asn1_resp(const char* prog, BIO* cbio, int keep_alive,
  ...
  if (ret < 0 || (size_t)ret >= sizeof(buf)) return 0;
