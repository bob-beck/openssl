 ...
 * s_client, s_server and ciphers. All of those features are deprecated and
 * will eventually disappear. In the meantime, to continue to support them, we
...
static int
srp_Verify_N_and_g (const BIGNUM *N, const BIGNUM *g)
{
  BN_CTX *bn_ctx = BN_CTX_new ();
  BIGNUM *p = BN_new ();
  BIGNUM *r = BN_new ();
  int ret = g != NULL && N != NULL && bn_ctx != NULL && BN_is_odd (N)
            && BN_check_prime (N, bn_ctx, NULL) == 1 && p != NULL
            && BN_rshift1 (p, N) &&
            /* p = (N-1)/2 */
...
int
set_up_srp_arg (SSL_CTX *ctx, SRP_ARG *srp_arg, int srp_lateuser, int c_msg,
                int c_debug)
{
...
int
set_up_srp_verifier_file (SSL_CTX *ctx, srpsrvparm *srp_callback_parm,
