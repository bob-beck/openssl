...
static void
provider_free(OSSL_PROVIDER* prov)
{
...
static int
opt_provider_path(const char* path)
{
  if (path != NULL && *path == '\0')
    path = NULL;
...
static int
set_prov_param(OSSL_PROVIDER* prov, void* vp)
{
...
static int
opt_provider_param(const char* arg)
{
  ...
  *(tmp = p.val++) = '\0';
  ...
  if (ret == 0) {
    ...
    opt_printf_stderr(
      "%s: No provider named '%s' is loaded\n", opt_getprog(), p.name);
    ret = 0;
  }
