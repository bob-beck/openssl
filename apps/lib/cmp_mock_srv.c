...
static mock_srv_ctx* mock_srv_ctx_new(void) {
  ...
  if ((ctx->statusOut = OSSL_CMP_PKISI_new()) == NULL) goto err;
...
int ossl_cmp_mock_srv_set1_crlOut(OSSL_CMP_SRV_CTX* srv_ctx, X509_CRL* crl) {
  ...
  if (crl != NULL && !X509_CRL_up_ref(crl)) return 0;
...
static int delayed_delivery(OSSL_CMP_SRV_CTX* srv_ctx,
  ...
  if (ctx->pollCount > 0 && ctx->curr_pollCount == 0) {
    /* start polling */
    if ((ctx->req = OSSL_CMP_MSG_dup(req)) == NULL) return -1;
    return 1;
  }
  return 0;
}
...
static OSSL_CMP_PKISI* process_cert_request(
    OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* cert_req,
  ...
  *caPubs = NULL;
  ...
  if (ctx->pollCount > 0 && ctx->curr_pollCount == 0) {
    /* start polling */
    if ((ctx->req = OSSL_CMP_MSG_dup(cert_req)) == NULL) return NULL;
  ...
  if (OSSL_CMP_MSG_get_bodytype(cert_req) == OSSL_CMP_CR) {
    ...
    for (i = 0; i < sk_OSSL_CMP_ITAV_num(itavs); i++) {
      ...
      if (OBJ_obj2nid(obj) == NID_id_it_certProfile) {
        if (!OSSL_CMP_ITAV_get0_certProfile(itav, &strs)) return NULL;
...
static int check_client_crl(const STACK_OF(OSSL_CMP_CRLSTATUS)* crlStatusList,
  ...
  if (!OSSL_CMP_CRLSTATUS_get0(crlstatus, &dpn, &issuer, &thisupd)) return -1;
...
static OSSL_CMP_ITAV* process_genm_itav(mock_srv_ctx* ctx, int req_nid,
  ...
  switch (req_nid) {
    ...
    case NID_id_it_rootCaCert: {
      ...
      if (!OSSL_CMP_ITAV_get0_rootCaCert(req, &rootcacert)) return NULL;
    ...
    case NID_id_it_crlStatusList: {
      ...
      if (!OSSL_CMP_ITAV_get0_crlStatusList(req, &crlstatuslist)) return NULL;
    ...
    case NID_id_it_certReqTemplate: {
      ...
      if ((reqtemp = OSSL_CRMF_CERTTEMPLATE_new()) == NULL) return NULL;
...
static int process_genm(OSSL_CMP_SRV_CTX* srv_ctx, const OSSL_CMP_MSG* genm,
  ...
  if (sk_OSSL_CMP_ITAV_num(in) == 1) {
    ...
    if ((*out = sk_OSSL_CMP_ITAV_new_reserve(NULL, 1)) == NULL) return 0;
    rsp = process_genm_itav(ctx, OBJ_obj2nid(obj), req);
    if (rsp != NULL && sk_OSSL_CMP_ITAV_push(*out, rsp)) return 1;
