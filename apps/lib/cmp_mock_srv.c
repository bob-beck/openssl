...
static void
mock_srv_ctx_free (mock_srv_ctx *ctx)
{
  if (ctx == NULL)
    return;
...
int
ossl_cmp_mock_srv_set1_keyOut (OSSL_CMP_SRV_CTX *srv_ctx, EVP_PKEY *pkey)
{
  mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx (srv_ctx);
...
int
ossl_cmp_mock_srv_set1_crlOut (OSSL_CMP_SRV_CTX *srv_ctx, X509_CRL *crl)
{
  mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx (srv_ctx);
...
int
ossl_cmp_mock_srv_set1_chainOut (OSSL_CMP_SRV_CTX *srv_ctx,
                                 STACK_OF (X509) *chain)
{
  mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx (srv_ctx);
...
int
ossl_cmp_mock_srv_set1_caPubsOut (OSSL_CMP_SRV_CTX *srv_ctx,
...
int
ossl_cmp_mock_srv_set_statusInfo (OSSL_CMP_SRV_CTX *srv_ctx, int status,
...
int
ossl_cmp_mock_srv_set_sendError (OSSL_CMP_SRV_CTX *srv_ctx, int bodytype)
{
  mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx (srv_ctx);
...
int
ossl_cmp_mock_srv_set_pollCount (OSSL_CMP_SRV_CTX *srv_ctx, int count)
{
  mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx (srv_ctx);
...
int
ossl_cmp_mock_srv_set_checkAfterTime (OSSL_CMP_SRV_CTX *srv_ctx, int sec)
{
  mock_srv_ctx *ctx = OSSL_CMP_SRV_CTX_get0_custom_ctx (srv_ctx);
...
static int
refcert_cmp (const X509 *refcert, const X509_NAME *issuer,
...
static int
clean_transaction (OSSL_CMP_SRV_CTX *srv_ctx,
                      ...
                      STACK_OF (X509) **chainOut, STACK_OF (X509) **caPubs)
{
  ...
  *caPubs = NULL;
  ...
  if (OSSL_CMP_MSG_get_bodytype (cert_req) == OSSL_CMP_CR)
    {
      STACK_OF (OSSL_CMP_ITAV) *itavs = OSSL_CMP_HDR_get0_geninfo_ITAVs (
  ...
  if (central_keygen == 1
      && (ctx->keyOut == NULL || (keyOut = EVP_PKEY_dup (ctx->keyOut)) == NULL
          || !OSSL_CMP_CTX_set0_newPkey (
              OSSL_CMP_SRV_CTX_get0_cmp_ctx (srv_ctx), 1 /* priv */, keyOut)))
    {
...
static OSSL_CMP_PKISI *
process_rr (OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *rr,
...
static int
check_client_crl (const STACK_OF (OSSL_CMP_CRLSTATUS) *crlStatusList,
                  const X509_CRL *crl)
{
...
process_genm_itav (mock_srv_ctx *ctx, int req_nid, const OSSL_CMP_ITAV *req)
{
...
static int
process_genm (OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *genm,
...
static void
process_error (OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *error,
...
static int
process_certConf (OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *certConf,
                  ossl_unused int certReqId, const ASN1_OCTET_STRING *certHash,
...
static int
process_pollReq (OSSL_CMP_SRV_CTX *srv_ctx, const OSSL_CMP_MSG *pollReq,
...
OSSL_CMP_SRV_CTX *
ossl_cmp_mock_srv_new (OSSL_LIB_CTX *libctx, const char *propq)
{
...
void
ossl_cmp_mock_srv_free (OSSL_CMP_SRV_CTX *srv_ctx)
{
  if (srv_ctx != NULL)
