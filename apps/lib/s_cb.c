...
static const char*
lookup(int val, const STRINT_PAIR* list, const char* def)
{
  for (; list->name; ++list)
    if (list->retval == val)
      return list->name;
...
verify_callback(int ok, X509_STORE_CTX* ctx)
{
  ...
  if (!verify_args.quiet || !ok) {
    ...
    if (err_cert != NULL) {
      X509_NAME_print_ex(
        bio_err, X509_get_subject_name(err_cert), 0, get_nameopt());
      BIO_puts(bio_err, "\n");
    } else {
  ...
  switch (err) {
    case X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT:
      if (err_cert != NULL) {
        ...
        X509_NAME_print_ex(
          bio_err, X509_get_issuer_name(err_cert), 0, get_nameopt());
        BIO_puts(bio_err, "\n");
      }
      break;
...
int
set_cert_key_stuff(SSL_CTX* ctx,
                   X509* cert,
                   EVP_PKEY* key,
                   STACK_OF(X509)* chain,
                   int build_chain)
...
static void
ssl_print_client_cert_types(BIO* bio, SSL* s)
{
...
static int
do_print_sigalgs(BIO* out, SSL* s, int shared)
{
  int i, nsig, client;
...
int
ssl_print_sigalgs(BIO* out, SSL* s)
{
  const char* name;
  int nid;
...
int
ssl_print_point_formats(BIO* out, SSL* s)
{
  int i, nformats;
  const char* pformats;
...
int
ssl_print_groups(BIO* out, SSL* s, int noshared)
{
  ...
  switch (EVP_PKEY_get_id(key)) {
    ...
    case EVP_PKEY_EC: {
      ...
      if (!EVP_PKEY_get_utf8_string_param(
            key, OSSL_PKEY_PARAM_GROUP_NAME, name, sizeof(name), &name_len))
        strcpy(name, "?");
...
static STRINT_PAIR signature_tls13_scheme_list[] = {
  ...
  /*  {"rsa_pkcs1_sha224",       0x0301    TLSEXT_SIGALG_rsa_pkcs1_sha224}, not
     in rfc8446 */
...
static void
print_chain_flags(SSL* s, int flags)
{
  STRINT_PAIR* pp;
...
set_cert_cb(SSL* ssl, void* arg)
{
  ...
  if (retry_cnt < 5) {
    retry_cnt++;
    BIO_printf(
      bio_err, "Certificate callback retry test: count %d\n", retry_cnt);
    return -1;
  }
#endif
  SSL_certs_clear(ssl);
  ...
  while (exc != NULL) {
    ...
    X509_NAME_print_ex(
      bio_err, X509_get_subject_name(exc->cert), 0, get_nameopt());
    BIO_puts(bio_err, "\n");
...
void
ssl_ctx_set_excert(SSL_CTX* ctx, SSL_EXCERT* exc)
{
  SSL_CTX_set_cert_cb(ctx, set_cert_cb, exc);
}

static int
ssl_excert_prepend(SSL_EXCERT** pexc)
{
...
void
ssl_excert_free(SSL_EXCERT* exc)
{
  SSL_EXCERT* curr;
 ...
 */
static char*
hexencode(const unsigned char* data, size_t len)
{
...
void
print_verify_detail(SSL* s, BIO* bio)
{
  int mdpth;
  EVP_PKEY* mspki = NULL;
...
int
config_ctx(SSL_CONF_CTX* cctx, STACK_OF(OPENSSL_STRING)* str, SSL_CTX* ctx)
{
  int i;
...
static int
add_crls_store(X509_STORE* st, STACK_OF(X509_CRL)* crls)
{
  X509_CRL* crl;
  int i, ret = 1;
...
int
ssl_ctx_add_crls(SSL_CTX* ctx, STACK_OF(X509_CRL)* crls, int crl_download)
{
  X509_STORE* st;
...
static int
security_callback_debug(const SSL* s,
                        const SSL_CTX* ctx,
                        int op,
                        int bits,
                        int nid,
...
void
ssl_ctx_security_debug(SSL_CTX* ctx, int verbose)
{
...
int
set_keylog_file(SSL_CTX* ctx, const char* keylog_file)
{
...
void
ssl_print_secure_renegotiation_notes(BIO* bio, SSL* s)
{
