...
int verify_callback(int ok, X509_STORE_CTX* ctx) {
  ...
  if (!ok) {
    ...
    if (verify_args.depth < 0 || verify_args.depth >= depth) {
      if (!verify_args.return_error) ok = 1;
  ...
  switch (err) {
    ...
    case X509_V_ERR_NO_EXPLICIT_POLICY:
      if (!verify_args.quiet) policies_print(ctx);
      break;
  ...
  if (err == X509_V_OK && ok == 2 && !verify_args.quiet) policies_print(ctx);
  if (ok && !verify_args.quiet) BIO_printf(bio_err, "verify return:%d\n", ok);
  return ok;
}
...
static int do_print_sigalgs(BIO* out, SSL* s, int shared) {
  ...
  if (shared) BIO_puts(out, "Shared ");

  if (client) BIO_puts(out, "Requested ");
...
int ssl_print_sigalgs(BIO* out, SSL* s) {
  ...
  if (!SSL_is_server(s)) ssl_print_client_cert_types(out, s);
...
int ssl_print_groups(BIO* out, SSL* s, int noshared) {
  ...
  if (ngroups == 0) BIO_puts(out, "NONE");
  BIO_puts(out, "\n");
  return 1;
}
#endif
...
int ssl_print_tmp_key(BIO* out, SSL* s) {
  ...
  switch (EVP_PKEY_get_id(key)) {
    ...
    case EVP_PKEY_KEYMGMT:
      if ((keyname = EVP_PKEY_get0_type_name(key)) == NULL) keyname = "?";
      BIO_printf(out, "%s\n", keyname);
      break;
...
void msg_cb(int write_p, int version, int content_type, const void* buf,
  ...
  if (len > 0) {
    ...
    num = len;
    for (i = 0; i < num; i++) {
      if (i % 16 == 0 && i > 0) BIO_printf(bio, "\n   ");
      BIO_printf(bio, " %02x", ((const unsigned char*)buf)[i]);
    }
    if (i < len) BIO_printf(bio, " ...");
    BIO_printf(bio, "\n");
  }
  (void)BIO_flush(bio);
}
...
int generate_cookie_callback(SSL* ssl, unsigned char* cookie,
  ...
  if (res != 0) *cookie_len = (unsigned int)temp;
  return res;
}
...
static int set_cert_cb(SSL* ssl, void* arg) {
  ...
  while (exc != NULL) {
    ...
    if (rv & CERT_PKEY_VALID) {
       ...
       */
      if (exc->build_chain) {
        if (!SSL_build_cert_chain(ssl, 0)) return 0;
...
int args_excert(int opt, SSL_EXCERT** pexc) {
  ...
  switch ((enum range)opt) {
    ...
    case OPT_X_CHAIN_BUILD:
      ...
      break;
    case OPT_X_CERTFORM:
      if (!opt_format(opt_arg(), OPT_FMT_ANY, &exc->certform)) return 0;
      break;
    case OPT_X_KEYFORM:
      if (!opt_format(opt_arg(), OPT_FMT_ANY, &exc->keyform)) return 0;
      break;
  }
  return 1;
...
void print_ssl_summary(SSL* s) {
  ...
  if (SSL_is_server(s)) ssl_print_groups(bio_err, s, 1);
#endif
...
static int add_crls_store(X509_STORE* st, STACK_OF(X509_CRL)* crls) {
  ...
  for (i = 0; i < sk_X509_CRL_num(crls); i++) {
    ...
    if (!X509_STORE_add_crl(st, crl)) ret = 0;
  }
  return ret;
}
...
int ssl_ctx_add_crls(SSL_CTX* ctx, STACK_OF(X509_CRL)* crls, int crl_download) {
  ...
  if (crl_download) store_setup_crl_download(st);
  return 1;
}
...
int ssl_load_stores(SSL_CTX* ctx, const char* vfyCApath, const char* vfyCAfile,
  ...
  if (vfyCApath != NULL || vfyCAfile != NULL || vfyCAstore != NULL) {
    vfy = X509_STORE_new();
    if (vfy == NULL) goto err;
    if (vfyCAfile != NULL && !X509_STORE_load_file(vfy, vfyCAfile)) goto err;
    if (vfyCApath != NULL && !X509_STORE_load_path(vfy, vfyCApath)) goto err;
    if (vfyCAstore != NULL && !X509_STORE_load_store(vfy, vfyCAstore)) goto err;
    add_crls_store(vfy, crls);
    if (SSL_CTX_set1_verify_cert_store(ctx, vfy) == 0) goto err;
    if (crl_download) store_setup_crl_download(vfy);
  }
  if (chCApath != NULL || chCAfile != NULL || chCAstore != NULL) {
    ch = X509_STORE_new();
    if (ch == NULL) goto err;
    if (chCAfile != NULL && !X509_STORE_load_file(ch, chCAfile)) goto err;
    if (chCApath != NULL && !X509_STORE_load_path(ch, chCApath)) goto err;
    if (chCAstore != NULL && !X509_STORE_load_store(ch, chCAstore)) goto err;
    if (SSL_CTX_set1_chain_cert_store(ctx, ch) == 0) goto err;
  }
  rv = 1;
err:
  X509_STORE_free(vfy);
...
static int security_callback_debug(const SSL* s, const SSL_CTX* ctx, int op,
  ...
  if (rv == 1 && sdb->verbose < 2) return 1;
  ...
  if (show_nm) BIO_printf(sdb->out, "%s=", nm);
    ...
    case SSL_SECOP_OTHER_CERT: {
      if (cert_md) {
        ...
        if (pkey == NULL) {
          ...
          BIO_printf(sdb->out, "%s, bits=%d", EVP_PKEY_get0_type_name(pkey),
  ...
  if (show_bits) BIO_printf(sdb->out, ", security bits=%d", bits);
