...
long
bio_dump_callback (BIO *bio, int cmd, const char *argp, size_t len, int argi,
...
void
tlsext_cb (SSL *s, int client_server, int type, const unsigned char *data,
           int len, void *arg)
{
  BIO *bio = arg;
...
int
ssl_load_stores (SSL_CTX *ctx, const char *vfyCApath, const char *vfyCAfile,
...
static int
security_callback_debug (const SSL *s, const SSL_CTX *ctx, int op, int bits,
