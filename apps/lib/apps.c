      ...
      if (arg->argc >= arg->size)
        {
          ...
          arg->size += 20;
          tmp = OPENSSL_realloc_array (arg->argv, arg->size,
                                       sizeof (*arg->argv));
          if (tmp == NULL)
            return 0;
          arg->argv = tmp;
...
int
ctx_set_verify_locations (SSL_CTX *ctx, const char *CAfile, int noCAfile,
...
int
ctx_set_ctlog_list_file (SSL_CTX *ctx, const char *path)
{
  if (path == NULL)
...
int
wrap_password_callback (char *buf, int bufsiz, int verify, void *userdata)
{
...
char *
get_passwd (const char *pass, const char *desc)
{
  char *result = NULL;
...
int
app_passwd (const char *arg1, const char *arg2, char **pass1, char **pass2)
{
...
static char *
app_get_pass (const char *arg, int keepbio)
{
...
char *
app_conf_try_string (const CONF *conf, const char *group, const char *name)
{
  char *res;
...
int
app_conf_try_number (const CONF *conf, const char *group, const char *name,
                     long *result)
{
  int ok;
...
CONF *
app_load_config_bio (BIO *in, const char *filename)
{
  long errorline = -1;
  CONF *conf;
  int i;
...
CONF *
app_load_config_verbose (const char *filename, int verbose)
{
  if (verbose)
    {
      if (*filename == '\0')
...
CONF *
app_load_config_internal (const char *filename, int quiet)
{
  BIO *in;
  CONF *conf;
...
int
app_load_modules (const CONF *config)
{
  CONF *to_free = NULL;
...
X509 *
load_cert_pass (const char *uri, int format, int maybe_stdin, const char *pass,
                const char *desc)
{
  X509 *cert = NULL;
...
X509_CRL *
load_crl (const char *uri, int format, int maybe_stdin, const char *desc)
{
  X509_CRL *crl = NULL;
...
X509_REQ *
load_csr (const char *file, int format, const char *desc)
{
  X509_REQ *req = NULL;
  BIO *in;
...
X509_REQ *
load_csr_autofmt (const char *infile, int format,
...
EVP_PKEY *
load_key (const char *uri, int format, int may_stdin, const char *pass,
...
EVP_PKEY *
load_pubkey (const char *uri, int format, int maybe_stdin, const char *pass,
             ENGINE *e, const char *desc)
{
  EVP_PKEY *pkey = NULL;
  char *allocated_uri = NULL;
...
EVP_PKEY *
load_keyparams_suppress (const char *uri, int format, int maybe_stdin,
...
EVP_PKEY *
load_keyparams (const char *uri, int format, int maybe_stdin,
                const char *keytype, const char *desc)
{
...
char *
next_item (char *opt) /* in list separated by comma and/or space */
{
...
static void
warn_cert_msg (const char *uri, X509 *cert, const char *msg)
{
...
static void
warn_cert (const char *uri, X509 *cert, int warn_EE, X509_VERIFY_PARAM *vpm)
{
...
static void
warn_certs (const char *uri, STACK_OF (X509) *certs, int warn_EE,
...
int
load_cert_certs (const char *uri, X509 **pcert, STACK_OF (X509) **pcerts,
...
STACK_OF (X509) *
load_certs_multifile (char *files, const char *pass, const char *desc,
                      X509_VERIFY_PARAM *vpm)
{
...
static X509_STORE *
sk_X509_to_store (X509_STORE *store /* may be NULL */,
 ...
 */
X509_STORE *
load_certstore (char *input, const char *pass, const char *desc,
                X509_VERIFY_PARAM *vpm)
{
 ...
 */
int
load_certs (const char *uri, int maybe_stdin, STACK_OF (X509) **certs,
 ...
 */
int
load_crls (const char *uri, STACK_OF (X509_CRL) **crls, const char *pass,
           const char *desc)
{
 ...
 * For non-NULL ppkey, pcert, and pcrl the first suitable value found is
 * loaded. If pcerts is non-NULL and *pcerts == NULL then a new cert list is
 * allocated. If pcerts is non-NULL then all available certificates are
 * appended to *pcerts except any certificate assigned to *pcert. If pcrls is
 * non-NULL and *pcrls == NULL then a new list of CRLs is allocated. If pcrls
 * is non-NULL then all available CRLs are appended to *pcerts except any CRL
 * assigned to *pcrl. In any case (also on error) the caller is responsible for
 * freeing all members of *pcerts and *pcrls (as far as they are not NULL).
 */
int
load_key_certs_crls (const char *uri, int format, int maybe_stdin,
                     const char *pass, const char *desc, int quiet,
                     EVP_PKEY **ppkey, EVP_PKEY **ppubkey, EVP_PKEY **pparams,
                     X509 **pcert, STACK_OF (X509) **pcerts, X509_CRL **pcrl,
      ...
      if (bio != NULL)
        {
          ctx = OSSL_STORE_attach (bio, "file", libctx, propq,
                                   get_ui_method (), &uidata, params, NULL,
                                   NULL);
          BIO_free (bio);
        }
    }
...
int
set_dateopt (unsigned long *dateopt, const char *arg)
{
...
void
print_name (BIO *out, const char *title, const X509_NAME *nm)
{
  char *buf;
  char mline = 0;
  int indent = 0;
...
void
print_bignum_var (BIO *out, const BIGNUM *in, const char *var, int len,
...
void
print_array (BIO *out, const char *title, int len, const unsigned char *d)
{
  int i;
...
X509_STORE *
setup_verify (const char *CAfile, int noCAfile, const char *CApath,
...
#define BSIZE 256
BIGNUM *
load_serial (const char *serialfile, int *exists, int create,
             ASN1_INTEGER **retai)
{
  BIO *in = NULL;
  BIGNUM *ret = NULL;
...
int
save_serial (const char *serialfile, const char *suffix, const BIGNUM *serial,
             ASN1_INTEGER **retai)
{
  char buf[1][BSIZE];
  BIO *out = NULL;
...
int
rotate_serial (const char *serialfile, const char *new_suffix,
...
CA_DB *
load_index (const char *dbfile, DB_ATTR *db_attr)
{
  CA_DB *retdb = NULL;
  TXT_DB *tmpdb = NULL;
  BIO *in;
...
int
rotate_index (const char *dbfile, const char *new_suffix,
              const char *old_suffix)
{
  char buf[5][BSIZE];
  size_t i, j;
 ...
 */
X509_NAME *
parse_name (const char *cp, int chtype, int canmulti, const char *desc)
{
  int nextismulti = 0;
  char *work;
  X509_NAME *n;
...
int
pkey_ctrl_string (EVP_PKEY_CTX *ctx, const char *value)
{
  int rv = 0;
  char *stmp, *vtmp = NULL;
...
static void
nodes_print (const char *name, STACK_OF (X509_POLICY_NODE) *nodes)
{
...
void
policies_print (X509_STORE_CTX *ctx)
{
 ...
 *   err: (maybe NULL) on failure, an error message line is written to this
 * BIO. in: a NUL terminated string like "abc,def,ghi"
 *
 *   returns: a malloc'd buffer or NULL on failure.
 */
unsigned char *
next_protos_parse (size_t *outlen, const char *in)
{
  size_t len;
  unsigned char *out;
  size_t i, start = 0;
...
int
check_cert_attributes (BIO *bio, X509 *x, const char *checkhost,
...
static int
do_pkey_ctx_init (EVP_PKEY_CTX *pkctx, STACK_OF (OPENSSL_STRING) *opts)
{
  int i;
...
static int
do_x509_init (X509 *x, STACK_OF (OPENSSL_STRING) *opts)
{
  int i;
...
static int
do_x509_req_init (X509_REQ *x, STACK_OF (OPENSSL_STRING) *opts)
{
  int i;
...
static int
do_sign_init (EVP_MD_CTX *ctx, EVP_PKEY *pkey, const char *md,
...
static int
adapt_keyid_ext (X509 *cert, X509V3_CTX *ext_ctx, const char *name,
...
int
cert_matches_key (const X509 *cert, const EVP_PKEY *pkey)
{
  int match;
 ...
 */
int
do_X509_sign (X509 *cert, int force_v1, EVP_PKEY *pkey, const char *md,
...
int
do_X509_REQ_sign (X509_REQ *x, EVP_PKEY *pkey, const char *md,
                  STACK_OF (OPENSSL_STRING) *sigopts)
{
  int rv = 0;
  EVP_MD_CTX *mctx = EVP_MD_CTX_new ();
...
/* Sign the CRL info */
int
do_X509_CRL_sign (X509_CRL *x, EVP_PKEY *pkey, const char *md,
                  STACK_OF (OPENSSL_STRING) *sigopts)
{
  int rv = 0;
  EVP_MD_CTX *mctx = EVP_MD_CTX_new ();
 ...
 */
int
do_X509_verify (X509 *x, EVP_PKEY *pkey, STACK_OF (OPENSSL_STRING) *vfyopts)
{
  int rv = 0;
 ...
 */
int
do_X509_REQ_verify (X509_REQ *x, EVP_PKEY *pkey,
...
static X509_CRL *
load_crl_crldp (STACK_OF (DIST_POINT) *crldp)
{
  int i;
...
void
store_setup_crl_download (X509_STORE *st)
{
  ...
  switch (ERR_GET_REASON (err))
    ...
    case SSL_R_CERTIFICATE_VERIFY_FAILED:
      return "Cannot authenticate server via its TLS certificate, likely due "
             "to mismatch with our trusted TLS certs or missing revocation "
             "status";
    case SSL_AD_REASON_OFFSET + TLS1_AD_UNKNOWN_CA:
      return "Server did not accept our TLS certificate, likely due to "
             "mismatch with server's trust anchor or missing revocation "
             "status";
...
BIO *
app_http_tls_cb (BIO *bio, void *arg, int connect, int detail)
{
...
void
APP_HTTP_TLS_INFO_free (APP_HTTP_TLS_INFO *info)
{
  if (info != NULL)
    {
...
ASN1_VALUE *
app_http_post_asn1 (const char *host, const char *port, const char *path,
                    const char *proxy, const char *no_proxy, SSL_CTX *ssl_ctx,
                    const STACK_OF (CONF_VALUE) *headers,
                    const char *content_type, ASN1_VALUE *req,
                    const ASN1_ITEM *req_it, const char *expected_content_type,
...
WIN32_rename (const char *from, const char *to)
{
  ...
  if (sizeof (TCHAR) == 1)
    ...
    { /* UNICODE path */
      ...
      if (!MultiByteToWideChar (CP_ACP, 0, from, (int)flen, (WCHAR *)tfrom,
                                (int)flen))
#endif
        for (i = 0; i < flen; i++)
          tfrom[i] = (TCHAR)from[i];
#if !defined(_WIN32_WCE) || _WIN32_WCE >= 101
      if (!MultiByteToWideChar (CP_ACP, 0, to, (int)tlen, (WCHAR *)tto,
                                (int)tlen))
#endif
        for (i = 0; i < tlen; i++)
          tto[i] = (TCHAR)to[i];
...
int
app_access (const char *name, int flag)
{
#ifdef _WIN32
 ...
 */
BIO *
bio_open_owner (const char *filename, int format, int private)
{
  FILE *fp = NULL;
  BIO *b = NULL;
  int textmode, bflags;
...
static BIO *
bio_open_default_ (const char *filename, char mode, int format, int quiet)
{
  BIO *ret;
...
BIO *
bio_open_default (const char *filename, char mode, int format)
{
  return bio_open_default_ (filename, mode, format, 0);
}

BIO *
bio_open_default_quiet (const char *filename, char mode, int format)
{
  return bio_open_default_ (filename, mode, format, 1);
}

int
mem_bio_to_file (BIO *in, const char *filename, int format, int private)
{
  int rv = 0, ret = 0;
  BIO *out = NULL;
...
void
corrupt_signature (const ASN1_STRING *signature)
{
...
int
check_cert_time_string (const char *time, const char *desc)
{
  if (time == NULL || strcmp (time, "today") == 0
      || ASN1_TIME_set_string_X509 (NULL, time))
    return 1;
  BIO_printf (bio_err,
              "%s is invalid, it should be \"today\" or have format "
              "[CC]YYMMDDHHMMSSZ\n",
              desc);
  return 0;
}
...
int
set_cert_times (X509 *x, const char *startdate, const char *enddate, int days,
...
int
set_crl_lastupdate (X509_CRL *crl, const char *lastupdate)
{
  int ret = 0;
...
int
set_crl_nextupdate (X509_CRL *crl, const char *nextupdate, long days,
...
OSSL_PARAM *
app_params_new_from_opts (STACK_OF (OPENSSL_STRING) *opts,
...
void
app_params_free (OSSL_PARAM *params)
{
  int i;
...
EVP_PKEY *
app_keygen (EVP_PKEY_CTX *ctx, const char *alg, int bits, int verbose)
{
  EVP_PKEY *res = NULL;
...
EVP_PKEY *
app_paramgen (EVP_PKEY_CTX *ctx, const char *alg)
{
  EVP_PKEY *res = NULL;
