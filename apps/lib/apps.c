...
static BIO* bio_open_default_(const char* filename,
                              char mode,
                              int format,
                              int quiet);
...
static int set_table_opts(unsigned long* flags,
                          const char* arg,
                          const NAME_EX_TBL* in_tbl);
static int set_multi_opts(unsigned long* flags,
                          const char* arg,
                          const NAME_EX_TBL* in_tbl);
...
char* app_conf_try_string(const CONF* conf,
                          const char* group,
                          const char* name) {
  char* res;
...
int app_conf_try_number(const CONF* conf,
                        const char* group,
                        const char* name,
                        long* result) {
  int ok;
...
X509_REQ* load_csr_autofmt(const char* infile,
                           int format,
...
STACK_OF(X509)* load_certs_multifile(char* files,
                                     const char* pass,
                                     const char* desc,
                                     X509_VERIFY_PARAM* vpm) {
 ...
 */
X509_STORE* load_certstore(char* input,
                           const char* pass,
                           const char* desc,
                           X509_VERIFY_PARAM* vpm) {
...
#undef BSIZE
#define BSIZE 256
BIGNUM* load_serial(const char* serialfile,
                    int* exists,
                    int create,
                    ASN1_INTEGER** retai) {
  BIO* in = NULL;
...
int save_serial(const char* serialfile,
                const char* suffix,
                const BIGNUM* serial,
                ASN1_INTEGER** retai) {
  char buf[1][BSIZE];
...
int rotate_serial(const char* serialfile,
 ...
 */
X509_NAME* parse_name(const char* cp,
                      int chtype,
                      int canmulti,
                      const char* desc) {
  int nextismulti = 0;
...
static int do_sign_init(EVP_MD_CTX* ctx,
                        EVP_PKEY* pkey,
                        const char* md,
...
static int adapt_keyid_ext(X509* cert,
                           X509V3_CTX* ext_ctx,
                           const char* name,
                           const char* value,
                           int add_default) {
...
int do_X509_REQ_sign(X509_REQ* x,
                     EVP_PKEY* pkey,
                     const char* md,
                     STACK_OF(OPENSSL_STRING)* sigopts) {
  int rv = 0;
...
/* Sign the CRL info */
int do_X509_CRL_sign(X509_CRL* x,
                     EVP_PKEY* pkey,
                     const char* md,
                     STACK_OF(OPENSSL_STRING)* sigopts) {
  int rv = 0;
 ...
 * 0 if the signature check fails, or -1 if error occurs.
 */
int do_X509_REQ_verify(X509_REQ* x,
                       EVP_PKEY* pkey,
                       STACK_OF(OPENSSL_STRING)* vfyopts) {
  int rv = 0;
...
int set_crl_nextupdate(X509_CRL* crl,
