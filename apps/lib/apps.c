...
static int
set_table_opts(unsigned long* flags,
               const char* arg,
               const NAME_EX_TBL* in_tbl);
static int
set_multi_opts(unsigned long* flags,
               const char* arg,
               const NAME_EX_TBL* in_tbl);
int
...
chopup_args(ARGS* arg, char* buf)
{
  ...
  for (p = buf;;) {
    ...
    if (arg->argc >= arg->size) {
      ...
      arg->size += 20;
      tmp = OPENSSL_realloc_array(arg->argv, arg->size, sizeof(*arg->argv));
      if (tmp == NULL)
        return 0;
      arg->argv = tmp;
    }
...
int
ctx_set_verify_locations(SSL_CTX* ctx,
                         const char* CAfile,
                         int noCAfile,
                         const char* CApath,
                         int noCApath,
...
int
app_conf_try_number(const CONF* conf,
                    const char* group,
                    const char* name,
                    long* result)
{
...
add_oid_section(CONF* conf)
{
  ...
  for (i = 0; i < sk_CONF_VALUE_num(sktmp); i++) {
    ...
    if (OBJ_create(cnf->value, cnf->name, cnf->name) == NID_undef) {
      BIO_printf(
        bio_err, "problem creating object %s=%s\n", cnf->name, cnf->value);
      return 0;
    }
  }
  return 1;
                 ...
                 const char* desc)
{
  ...
  if (format != FORMAT_UNDEF) {
    ...
    if (csr == NULL) {
      BIO_printf(
        bio_err, "error: unable to load %s from file '%s'\n", desc, infile);
    }
  }
  if (csr != NULL) {
...
app_malloc(size_t sz, const char* what)
{
  ...
  if (vp == NULL)
    app_bail_out(
      "%s: Could not allocate %zu bytes for %s\n", opt_getprog(), sz, what);
  return vp;
}
...
warn_cert(const char* uri, X509* cert, int warn_EE, X509_VERIFY_PARAM* vpm)
{
  ...
  int res = X509_cmp_timeframe(
    vpm, X509_get0_notBefore(cert), X509_get0_notAfter(cert));
...
int
load_cert_certs(const char* uri,
                X509** pcert,
                     ...
                     X509_VERIFY_PARAM* vpm)
{
  ...
  while (files != NULL) {
    ...
    if (!load_cert_certs(files, NULL, &certs, 0, pass, desc, vpm))
      goto err;
    if (!X509_add_certs(
          result, certs, X509_ADD_FLAG_UP_REF | X509_ADD_FLAG_NO_DUP))
      goto oom;
 ...
 */
int
load_key_certs_crls(const char* uri,
                    int format,
                    int maybe_stdin,
                    const char* pass,
                    const char* desc,
                    ...
                    STACK_OF(X509_CRL)** pcrls)
{
  ...
  if ((input_type = format2string(format)) != NULL) {
    itp[0] = OSSL_PARAM_construct_utf8_string(
      OSSL_STORE_PARAM_INPUT_TYPE, (char*)input_type, 0);
  ...
  if (uri == NULL) {
    ...
    ctx = OSSL_STORE_open_ex(
      uri, libctx, propq, get_ui_method(), &uidata, params, NULL, NULL);
  }
  if (ctx == NULL) {
    if (!quiet)
    ...
    switch (type) {
      ...
      case OSSL_STORE_INFO_CERT:
        if (pcert != NULL) {
          ...
          ok = X509_add_cert(
            *pcerts, OSSL_STORE_INFO_get1_CERT(info), X509_ADD_FLAG_DEFAULT);
        }
        ncerts += ok;
        break;
             ...
             int noCAstore)
{
  ...
  if (CAfile != NULL || !noCAfile) {
    ...
    if (lookup == NULL)
      goto end;
    if (CAfile != NULL) {
      if (X509_LOOKUP_load_file_ex(
            lookup, CAfile, X509_FILETYPE_PEM, libctx, propq) <= 0) {
        ERR_clear_error();
        if (X509_LOOKUP_load_file_ex(
              lookup, CAfile, X509_FILETYPE_ASN1, libctx, propq) <= 0) {
      ...
      X509_LOOKUP_load_file_ex(
        lookup, NULL, X509_FILETYPE_DEFAULT, libctx, propq);
    }
  }
            ...
            ASN1_INTEGER** retai)
{
  ...
  if (in == NULL) {
    ...
    if (ret == NULL) {
      ...
      BIO_printf(
        bio_err, "Error creating random number to store in %s\n", serialfile);
      BN_free(ret);
      ret = NULL;
    }
  } else {
...
int
save_serial(const char* serialfile,
            const char* suffix,
            const BIGNUM* serial,
            ASN1_INTEGER** retai)
{
...
int
rotate_serial(const char* serialfile,
...
parse_name(const char* cp, int chtype, int canmulti, const char* desc)
{
  ...
  if (work == NULL) {
    BIO_printf(
      bio_err, "%s: Error copying %s name input\n", opt_getprog(), desc);
    goto err;
  }
  ...
  while (*cp != '\0') {
    ...
    *bp++ = '\0';
    ...
    if (!X509_NAME_add_entry_by_NID(n,
                                    nid,
                                    chtype,
                                    valstr,
             ...
             STACK_OF(OPENSSL_STRING)* sigopts)
{
  ...
  return EVP_DigestSignInit_ex(
           ctx, &pkctx, md, app_get0_libctx(), app_get0_propq(), pkey, NULL) &&
         do_pkey_ctx_init(pkctx, sigopts);
}

static int
adapt_keyid_ext(X509* cert,
                X509V3_CTX* ext_ctx,
                const char* name,
                const char* value,
                int add_default)
             ...
             X509V3_CTX* ext_ctx)
{
  ...
  if (!force_v1) {
    ...
    if (!adapt_keyid_ext(
          cert, ext_ctx, "authorityKeyIdentifier", "keyid, issuer", !self_sign))
      goto end;
  }
  ...
  switch (ERR_GET_REASON(err)) {
    ...
    case SSL_AD_REASON_OFFSET + TLS1_AD_UNKNOWN_CA:
      return "Server did not accept our TLS certificate, likely due to "
             "mismatch with server's trust anchor or missing revocation status";
                  ...
                  const ASN1_ITEM* it)
{
  ...
  if (use_ssl && ssl_ctx == NULL) {
    ERR_raise_data(
      ERR_LIB_HTTP, ERR_R_PASSED_NULL_PARAMETER, "missing SSL_CTX");
    goto end;
  }
  if (!use_ssl && ssl_ctx != NULL) {
...
WIN32_rename(const char* from, const char* to)
{
  ...
  if (sizeof(TCHAR) == 1) {
    ...
    if (!MultiByteToWideChar(
          CP_ACP, 0, from, (int)flen, (WCHAR*)tfrom, (int)flen))
#endif
      for (i = 0; i < flen; i++)
...
int
set_crl_nextupdate(X509_CRL* crl,
                         ...
                         const OSSL_PARAM* paramdefs)
{
  ...
  for (params_n = 0; params_n < sz; params_n++) {
    ...
    *vtmp = 0;
    ...
    vtmp++;
    if (!OSSL_PARAM_allocate_from_text(
          &params[params_n], paramdefs, stmp, vtmp, strlen(vtmp), &found))
      goto err;
    OPENSSL_free(stmp);
  }
