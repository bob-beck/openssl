...
int ctx_set_verify_locations(SSL_CTX* ctx, const char* CAfile, int noCAfile,
  ...
  if (CAfile == NULL && CApath == NULL && CAstore == NULL) {
    if (!noCAfile && SSL_CTX_set_default_verify_file(ctx) <= 0) return 0;
...
int ctx_set_ctlog_list_file(SSL_CTX* ctx, const char* path) {
  if (path == NULL) return SSL_CTX_set_default_ctlog_list_file(ctx);
...
static char* app_get_pass(const char* arg, int keepbio) {
  ...
  if (CHECK_AND_SKIP_PREFIX(arg, "pass:")) return OPENSSL_strdup(arg);
  ...
  if (!keepbio || pwdbio == NULL) {
    if (CHECK_AND_SKIP_PREFIX(arg, "file:")) {
      ...
      if (tmp == NULL || tmp - arg > PASS_SOURCE_SIZE_MAX)
        BIO_printf(bio_err,
                   "Invalid password argument, missing ':' within the first %d "
                   "chars\n",
...
int add_oid_section(CONF* conf) {
  ...
  if ((p = app_conf_try_string(conf, NULL, "oid_section")) == NULL) return 1;
...
CONF* app_load_config_modules(const char* configfile) {
  ...
  if (configfile != NULL) {
    if ((conf = app_load_config_verbose(configfile, 1)) == NULL) return NULL;
...
X509_REQ* load_csr(const char* file, int format, const char* desc) {
  ...
  if (format == FORMAT_UNDEF) format = FORMAT_PEM;
...
void cleanse(char* str) {
  if (str != NULL) OPENSSL_cleanse(str, strlen(str));
}

void clear_free(char* str) {
  if (str != NULL) OPENSSL_clear_free(str, strlen(str));
}
...
EVP_PKEY* load_keyparams_suppress(const char* uri, int format, int maybe_stdin,
  ...
  if (desc == NULL) desc = "key parameters";
...
int load_cert_certs(const char* uri, X509** pcert, STACK_OF(X509)** pcerts,
  ...
  if (desc == NULL) desc = pcerts == NULL ? "certificate" : "certificates";
  if (exclude_http &&
  ...
  if (ret) {
    if (pcert != NULL) warn_cert(uri, *pcert, 0, vpm);
    if (pcerts != NULL) warn_certs(uri, *pcerts, 1, vpm);
  } else {
    if (pcerts != NULL) {
...
STACK_OF(X509)* load_certs_multifile(char* files, const char* pass,
  ...
  if (files == NULL) goto err;
  if (result == NULL) goto oom;

  while (files != NULL) {
    char* next = next_item(files);

    if (!load_cert_certs(files, NULL, &certs, 0, pass, desc, vpm)) goto err;
...
static X509_STORE* sk_X509_to_store(
    X509_STORE* store /* may be NULL */,
    const STACK_OF(X509)* certs /* may NULL */) {
  int i;

  if (store == NULL) store = X509_STORE_new();
...
int load_certs(const char* uri, int maybe_stdin, STACK_OF(X509)** certs,
  ...
  if (desc == NULL) desc = "certificates";
...
int load_key_certs_crls(const char* uri, int format, int maybe_stdin,
  ...
  if (pcerts != NULL) {
    if (*pcerts == NULL && (*pcerts = sk_X509_new_null()) == NULL) {
      if (!quiet) BIO_printf(bio_err, "Out of memory loading");
      goto end;
    }
    /*
  ...
  if (ctx == NULL) {
    if (!quiet) BIO_printf(bio_err, "Could not open file or uri for loading");
    goto end;
  }
  /* expect == 0 means here multiple types of credentials are to be loaded */
  if (expect > 0 && !OSSL_STORE_expect(ctx, expect)) {
    if (!quiet) BIO_printf(bio_err, "Internal error trying to load");
    goto end;
  }
  ...
  while ((ppkey != NULL || ppubkey != NULL || pparams != NULL ||
    ...
    if (!ok) {
      ...
      if (!quiet) BIO_printf(bio_err, "Error reading");
      break;
    }
  }
  ...
  if (failed == NULL) {
    if (ncerts > 0) pcerts = NULL;
    if (ncrls > 0) pcrls = NULL;
    failed = FAIL_NAME;
    if (failed != NULL && !quiet) BIO_printf(bio_err, "Could not find");
  }
  ...
  if (failed != NULL && !quiet) {
    ...
    if (desc != NULL && strstr(desc, failed) != NULL) {
      ...
      if (desc != NULL) BIO_printf(bio_err, " of %s", desc);
    }
    if (uri != NULL) BIO_printf(bio_err, " from %s", uri);
  ...
  if (quiet || failed == NULL) /* clear any suppressed or spurious errors */
    ERR_pop_to_mark();
  else
    ERR_clear_last_mark();
  return failed == NULL;
...
int set_name_ex(unsigned long* flags, const char* arg) {
  static const NAME_EX_TBL ex_tbl[] = {
      ...
      {NULL, 0, 0}};
  if (set_multi_opts(flags, arg, ex_tbl) == 0) return 0;
...
int copy_extensions(X509* x, X509_REQ* req, int copy_type) {
  ...
  if (x == NULL || req == NULL) return 0;
  if (copy_type == EXT_COPY_NONE) return 1;
  ...
  for (i = 0; i < sk_X509_EXTENSION_num(exts); i++) {
    ...
    if (idx != -1) {
      ...
      if (copy_type == EXT_COPY_ADD) continue;
...
static int set_multi_opts(unsigned long* flags, const char* arg,
  ...
  for (i = 0; i < sk_CONF_VALUE_num(vals); i++) {
    ...
    if (!set_table_opts(flags, val->name, in_tbl)) ret = 0;
  }
...
void print_name(BIO* out, const char* title, const X509_NAME* nm) {
  ...
  if (out == NULL) return;
  if (title != NULL) BIO_puts(out, title);
...
err:
  if (ret == NULL) ERR_print_errors(bio_err);
  BIO_free(in);
...
CA_DB* load_index(const char* dbfile, DB_ATTR* db_attr) {
  ...
  if (dbattr_conf != NULL) {
    ...
    if (p != NULL) retdb->attributes.unique_subject = parse_yesno(p, 1);
  }

  retdb->dbfname = OPENSSL_strdup(dbfile);
  if (retdb->dbfname == NULL) goto err;
...
X509_NAME* parse_name(const char* cp, int chtype, int canmulti,
  ...
  while (*cp != '\0') {
    ...
    *bp++ = '\0';
    ...
    if (*valstr == '\0') {
      BIO_printf(bio_err,
                 "%s warning: No value provided for %s name attribute \"%s\", "
                 "skipped\n",
                 opt_getprog(), desc, typestr);
      continue;
    }
...
int pkey_ctrl_string(EVP_PKEY_CTX* ctx, const char* value) {
  ...
  if (stmp == NULL) return -1;
  vtmp = strchr(stmp, ':');
  if (vtmp == NULL) goto err;
...
int do_X509_sign(X509* cert, int force_v1, EVP_PKEY* pkey, const char* md,
  ...
  if (!force_v1) {
    if (!X509_set_version(cert, X509_VERSION_3)) goto end;
...
static const char* get_dp_url(DIST_POINT* dp) {
  ...
  if (!dp->distpoint || dp->distpoint->type != 0) return NULL;
...
static X509_CRL* load_crl_crldp(STACK_OF(DIST_POINT)* crldp) {
  ...
  for (i = 0; i < sk_DIST_POINT_num(crldp); i++) {
    ...
    if (urlptr != NULL) return load_crl(urlptr, FORMAT_UNDEF, 0, "CRL via CDP");
  }
  return NULL;
}
...
static STACK_OF(X509_CRL)* crls_http_cb(const X509_STORE_CTX* ctx,
  ...
  if (crl == NULL || !sk_X509_CRL_push(crls, crl)) goto error;
...
static const char* tls_error_hint(void) {
  ...
  if (ERR_GET_LIB(err) != ERR_LIB_SSL) return NULL; /* likely no TLS error */
...
static BIO* http_tls_shutdown(BIO* bio) {
  if (bio != NULL) {
    ...
    if (hint != NULL) BIO_printf(bio_err, "%s\n", hint);
    (void)ERR_set_mark();
...
BIO* app_http_tls_cb(BIO* bio, void* arg, int connect, int detail) {
  ...
  if (connect) {
    ...
    if (vpm != NULL) SSL_set_tlsext_host_name(ssl, host /* may be NULL */);
...
static int WIN32_rename(const char* from, const char* to) {
  ...
  if (err == ERROR_ALREADY_EXISTS || err == ERROR_FILE_EXISTS) {
    if (DeleteFile(tto) && MoveFile(tfrom, tto)) goto ok;
    err = GetLastError();
  }
...
BIO* bio_open_owner(const char* filename, int format, int private) {
  ...
  if (fp == NULL) goto err;
  bflags = BIO_CLOSE;
  if (textmode) bflags |= BIO_FP_TEXT;
...
void wait_for_async(SSL* s) {
  ...
  if (!SSL_get_all_async_fds(s, NULL, &numfds)) return;
  if (numfds == 0) return;
  ...
  FD_ZERO(&asyncfds);
  for (i = 0; i < numfds; i++) {
    if (width <= (int)fds[i]) width = (int)fds[i] + 1;
...
int set_cert_times(X509* x, const char* startdate, const char* enddate,
  ...
  if (!check_cert_time_string(startdate, "start date")) return 0;
  ...
  if (ASN1_TIME_compare(X509_get0_notAfter(x), X509_get0_notBefore(x)) < 0) {
    ...
    if (strict_compare_times) return 0;
  }
  return 1;
}
...
int set_crl_lastupdate(X509_CRL* crl, const char* lastupdate) {
  ...
  if (lastupdate == NULL) {
    if (X509_gmtime_adj(tm, 0) == NULL) goto end;
  } else {
    if (!ASN1_TIME_set_string_X509(tm, lastupdate)) goto end;
  }

  if (!X509_CRL_set1_lastUpdate(crl, tm)) goto end;
...
int set_crl_nextupdate(X509_CRL* crl, const char* nextupdate, long days,
  ...
  if (nextupdate == NULL) {
    ...
    if (!ASN1_TIME_set_string_X509(tm, nextupdate)) goto end;
  }

  if (!X509_CRL_set1_nextUpdate(crl, tm)) goto end;
...
int opt_legacy_okay(void) {
   ...
   */
  if (provider_options || libctx) return 0;
  return 1;
}
