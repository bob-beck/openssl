...
char* opt_progname(const char* argv0) {
  ...
  if (n > sizeof(prog) - 1) n = sizeof(prog) - 1;
...
char* opt_progname(const char* argv0) {
  ...
  p = opt_path_end(argv0);
  q = strrchr(p, '.');
  if (prog != p) strncpy(prog, p, sizeof(prog) - 1);
  prog[sizeof(prog) - 1] = '\0';
  if (q != NULL && q - p < sizeof(prog)) prog[q - p] = '\0';
  return prog;
}
...
static int opt_format_error(const char* s, unsigned long flags) {
  ...
  for (ap = formats; ap->name; ap++)
    if (flags & ap->retval) opt_printf_stderr(" %s", ap->name);
  opt_printf_stderr("\n");
  ...
  switch (*s) {
    ...
    case 'b':
      if (s[1] == '\0' || strcmp(s, "B64") == 0 || strcmp(s, "b64") == 0 ||
        ...
        if ((flags & OPT_FMT_B64) == 0) return opt_format_error(s, flags);
    ...
    case 'D':
    case 'd':
      if ((flags & OPT_FMT_DER) == 0) return opt_format_error(s, flags);
      *result = FORMAT_ASN1;
      break;
    case 'T':
    case 't':
      if ((flags & OPT_FMT_TEXT) == 0) return opt_format_error(s, flags);
      *result = FORMAT_TEXT;
      break;
    case 'N':
    case 'n':
      if ((flags & OPT_FMT_NSS) == 0) return opt_format_error(s, flags);
    ...
    case 'S':
    case 's':
      if ((flags & OPT_FMT_SMIME) == 0) return opt_format_error(s, flags);
      *result = FORMAT_SMIME;
      break;
    case 'M':
    case 'm':
      if ((flags & OPT_FMT_MSBLOB) == 0) return opt_format_error(s, flags);
      *result = FORMAT_MSBLOB;
      break;
    case 'E':
    case 'e':
      if ((flags & OPT_FMT_ENGINE) == 0) return opt_format_error(s, flags);
      *result = FORMAT_ENGINE;
      break;
    case 'H':
    case 'h':
      if ((flags & OPT_FMT_HTTP) == 0) return opt_format_error(s, flags);
      *result = FORMAT_HTTP;
      break;
    case '1':
      if ((flags & OPT_FMT_PKCS12) == 0) return opt_format_error(s, flags);
      *result = FORMAT_PKCS12;
      break;
    case 'P':
    case 'p':
      if (s[1] == '\0' || strcmp(s, "PEM") == 0 || strcmp(s, "pem") == 0) {
        if ((flags & OPT_FMT_PEM) == 0) return opt_format_error(s, flags);
        *result = FORMAT_PEM;
      } else if (strcmp(s, "PVK") == 0 || strcmp(s, "pvk") == 0) {
        if ((flags & OPT_FMT_PVK) == 0) return opt_format_error(s, flags);
        *result = FORMAT_PVK;
...
int opt_intmax(const char* value, ossl_intmax_t* result) {
  ...
  if ((ret = opt_long(value, &m))) *result = m;
  return ret;
}
...
int opt_uintmax(const char* value, ossl_uintmax_t* result) {
  ...
  if ((ret = opt_ulong(value, &m))) *result = m;
  return ret;
}
#endif
...
int opt_verify(int opt, X509_VERIFY_PARAM* vpm) {
  ...
  switch ((enum range)opt) {
    ...
    case OPT_V_VERIFY_DEPTH:
      i = atoi(opt_arg());
      if (i >= 0) X509_VERIFY_PARAM_set_depth(vpm, i);
      break;
    case OPT_V_VERIFY_AUTH_LEVEL:
      i = atoi(opt_arg());
      if (i >= 0) X509_VERIFY_PARAM_set_auth_level(vpm, i);
      break;
    case OPT_V_ATTIME:
      if (!opt_intmax(opt_arg(), &t)) return 0;
      if (t != (time_t)t) {
    ...
    case OPT_V_VERIFY_HOSTNAME:
      if (!X509_VERIFY_PARAM_set1_host(vpm, opt_arg(), 0)) return 0;
      break;
    case OPT_V_VERIFY_EMAIL:
      if (!X509_VERIFY_PARAM_set1_email(vpm, opt_arg(), 0)) return 0;
      break;
    case OPT_V_VERIFY_IP:
      if (!X509_VERIFY_PARAM_set1_ip_asc(vpm, opt_arg())) return 0;
      break;
  ...
  for (o = opts; o->name; ++o) {
    ...
    switch (o->valtype) {
      ...
      case 'N':
        ...
        break;
      case 'M':
        if (!opt_intmax(arg, &imval)) return -1;
        break;
      case 'U':
        if (!opt_uintmax(arg, &umval)) return -1;
        break;
      case 'l':
        if (!opt_long(arg, &lval)) return -1;
        break;
      case 'u':
        if (!opt_ulong(arg, &ulval)) return -1;
        break;
      case 'c':
      case 'E':
      case 'F':
...
static void opt_print(const OPTIONS* o, int doingparams, int width) {
  ...
  if (width >= (int)sizeof(start)) width = (int)sizeof(start) - 1;
...
void opt_help(const OPTIONS* list) {
  ...
  for (o = list; o->name; o++) {
    if (o->name == OPT_MORE_STR) continue;

    i = 2 + (int)strlen(o->name);
    if (o->valtype != '-') i += 1 + (int)strlen(valtype2param(o));

    if (i > width) width = i;
  }

  if (width > MAX_OPT_HELP_WIDTH) width = MAX_OPT_HELP_WIDTH;
  ...
  /* Now let's print. */
  for (o = list; o->name; o++) {
    if (o->name == OPT_PARAM_STR) sawparams = 1;
