...
int
log_set_verbosity (const char *prog, int level)
{
...
#ifdef HTTP_DAEMON
static int
print_syslog (const char *str, size_t len, void *levPtr)
{
...
static void
log_with_prefix (const char *prog, const char *fmt, va_list ap)
{
  char prefix[80];
...
void
trace_log_message (int category, const char *prog, int level, const char *fmt,
                   ...)
{
  va_list ap;
  va_start (ap, fmt);
