  ...
  for (ai = res; ai != NULL; ai = BIO_ADDRINFO_next(ai)) {
    ...
    if (bindaddr != NULL) {
      for (bi = bindaddr; bi != NULL; bi = BIO_ADDRINFO_next(bi)) {
        if (BIO_ADDRINFO_family(bi) == BIO_ADDRINFO_family(ai)) break;
      }
      if (bi == NULL) continue;
      ++found;
    }
...
void get_sock_info_address(int asock, char** hostname, char** service) {
  ...
  if ((info.addr = BIO_ADDR_new()) != NULL &&
    ...
    if (hostname != NULL) *hostname = BIO_ADDR_hostname_string(info.addr, 1);
    if (service != NULL) *service = BIO_ADDR_service_string(info.addr, 1);
  }
...
int report_server_accept(BIO* out, int asock, int with_address, int with_pid) {
  ...
  if (BIO_printf(out, "ACCEPT") <= 0) return 0;
  if (with_address) {
  ...
  if (with_pid) success *= BIO_printf(out, " PID=%d", getpid()) > 0;
...
int do_server(int* accept_sock, const char* host, const char* port, int family,
  ...
  if (tfo && sock_type == SOCK_STREAM) sock_options |= BIO_SOCK_TFO;
#ifdef AF_INET6
  if (sock_family == AF_INET6) sock_options |= BIO_SOCK_V6_ONLY;
  ...
  if (accept_sock != NULL) *accept_sock = asock;
  for (;;) {
    char sink[64];
    ...
    if (type == SOCK_STREAM) {
      ...
      if (naccept != -1) naccept--;
      if (naccept == 0) BIO_closesocket(asock);
