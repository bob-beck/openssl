...
static int ui_open(UI* ui) {
  ...
  if (opener != NULL) return opener(ui);
  return 1;
}
...
static int ui_read(UI* ui, UI_STRING* uis) {
  ...
  if (reader != NULL) return reader(ui, uis);
...
static int ui_write(UI* ui, UI_STRING* uis) {
  ...
  if (writer != NULL) return writer(ui, uis);
  return 1;
}
...
static int ui_close(UI* ui) {
  ...
  if (closer != NULL) return closer(ui);
  return 1;
}
...
static char* ui_prompt_construct(UI* ui, const char* phrase_desc,
  ...
  if (phrase_desc == NULL) phrase_desc = "pass phrase";
...
int set_base_ui_method(const UI_METHOD* ui_meth) {
  if (ui_meth == NULL) ui_meth = UI_null();
...
int password_callback(char* buf, int bufsiz, int verify, PW_CB_DATA* cb_data) {
  ...
  if ((ui = UI_new_method(ui_method)) == NULL) return 0;
