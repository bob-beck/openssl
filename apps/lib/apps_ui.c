...
static char*
ui_prompt_construct(UI* ui, const char* phrase_desc, const char* object_name)
{
...
int
set_base_ui_method(const UI_METHOD* ui_meth)
{
  if (ui_meth == NULL)
    ui_meth = UI_null();
...
static void*
ui_malloc(int sz, const char* what)
{
...
int
password_callback(char* buf, int bufsiz, int verify, PW_CB_DATA* cb_data)
{
  int res = 0;
  UI* ui;
  int ok = 0;
  ...
  if (ok >= 0 && verify) {
    ...
    ok = UI_add_verify_string(
      ui, prompt, ui_flags, buff, PW_MIN_LENGTH, bufsiz - 1, buf);
  }
  if (ok >= 0)
    do {
      ok = UI_process(ui);
