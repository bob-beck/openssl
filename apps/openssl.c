...
static int apps_startup(void) {
  ...
  if (use_libctx != NULL) {
    ...
    if (strcmp(use_libctx, "1") == 0) {
      if (app_create_libctx() == NULL) return 0;
    }
  }
...
static void setup_trace_category(int category) {
  ...
  if (OSSL_trace_enabled(category)) return;
...
int main(int argc, char* argv[]) {
  ...
  if (!app_RAND_write()) ret = EXIT_FAILURE;
...
static int do_cmd(LHASH_OF(FUNCTION)* prog, int argc, char* argv[]) {
  ...
  if (fp != NULL) {
    if (fp->deprecated_alternative != NULL) warn_deprecated(fp);
...
static LHASH_OF(FUNCTION)* prog_init(void) {
  ...
  if ((ret = lh_FUNCTION_new(function_hash, function_cmp)) == NULL) return NULL;
