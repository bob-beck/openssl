...
int ocsp_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_ISSUER:
        ...
        if (issuer == NULL) goto end;
        if (issuers == NULL) {
          if ((issuers = sk_X509_new_null()) == NULL) goto end;
        }
        if (!sk_X509_push(issuers, issuer)) goto end;
        break;
      case OPT_CERT:
        reset_unknown();
        X509_free(cert);
        ...
        if (cert_id_md == NULL) cert_id_md = (EVP_MD*)EVP_sha1();
        if (!add_ocsp_cert(&req, cert, cert_id_md, issuer, ids)) goto end;
        if (!sk_OPENSSL_STRING_push(reqnames, opt_arg())) goto end;
        trailing_md = 0;
        break;
      case OPT_SERIAL:
        reset_unknown();
      ...
      case OPT_RMD: /* Response MessageDigest */
        ...
        break;
      case OPT_RSIGOPT:
        if (rsign_sigopts == NULL) rsign_sigopts = sk_OPENSSL_STRING_new_null();
      ...
      case OPT_HEADER:
        ...
        *value++ = '\0';
        if (!X509V3_add_value(header, value, &headers)) goto end;
        break;
      case OPT_RCID:
        if (!opt_md(opt_arg(), &resp_certid_md)) goto opthelp;
        break;
      case OPT_MD:
        if (trailing_md) {
        ...
        if (!opt_md(opt_unknown(), &cert_id_md)) goto opthelp;
        trailing_md = 1;
        break;
      case OPT_MULTI:
#ifdef HTTP_DAEMON
  ...
  if (respdigname != NULL) {
    if (!opt_md(respdigname, &rsign_md)) goto end;
  }
  ...
  if (req == NULL && (add_nonce != 2)) add_nonce = 0;

  if (req == NULL && reqin != NULL) {
    derbio = bio_open_default(reqin, 'r', FORMAT_ASN1);
    if (derbio == NULL) goto end;
  ...
  if (rsignfile != NULL) {
    if (rkeyfile == NULL) rkeyfile = rsignfile;
...
#ifdef HTTP_DAEMON
  if (n_responders != 0 && acbio != NULL) spawn_loop(prog);
  if (acbio != NULL && req_timeout > 0) signal(SIGALRM, socket_timeout);
#endif
...
redo_accept:
  ...
  if (req != NULL && add_nonce) {
    if (!OCSP_request_add1_nonce(req, NULL, -1)) goto end;
  }

  if (signfile != NULL) {
    if (keyfile == NULL) keyfile = signfile;
  ...
  if (rdb != NULL) {
    ...
    if (resp == NULL) goto end;
    if (cbio != NULL) send_ocsp_response(cbio, resp);
  } else if (host != NULL) {
#ifndef OPENSSL_NO_SOCK
...
done_resp:
  ...
  if (!print_ocsp_summary(out, bs, req, reqnames, ids, nsec, maxage)) ret = 1;
...
static int add_ocsp_cert(OCSP_REQUEST** req, X509* cert,
  ...
  if (*req == NULL) *req = OCSP_REQUEST_new();
  if (*req == NULL) goto err;
  id = OCSP_cert_to_id(cert_id_md, cert, issuer);
  if (id == NULL || !sk_OCSP_CERTID_push(ids, id)) goto err;
...
static int print_ocsp_summary(BIO* out, OCSP_BASICRESP* bs, OCSP_REQUEST* req,
  ...
  if (req == NULL || !sk_OPENSSL_STRING_num(names)) return 1;

  if (bs == NULL || !sk_OCSP_CERTID_num(ids)) return 0;
  ...
  for (i = 0; i < sk_OCSP_CERTID_num(ids); i++) {
    ...
    if (status != V_OCSP_CERTSTATUS_REVOKED) continue;
...
static void make_ocsp_response(BIO* err, OCSP_RESPONSE** resp,
  ...
  if (ndays != -1) nextupd = X509_time_adj_ex(NULL, ndays, nmin * 60, NULL);
...
OCSP_RESPONSE* process_responder(OCSP_REQUEST* req, const char* host,
  ...
  if (resp == NULL) BIO_printf(bio_err, "Error querying OCSP responder\n");
