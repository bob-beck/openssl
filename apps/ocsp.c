...
static int
add_ocsp_cert(OCSP_REQUEST** req,
              X509* cert,
              const EVP_MD* cert_id_md,
              X509* issuer,
              STACK_OF(OCSP_CERTID)* ids);
static int
add_ocsp_serial(OCSP_REQUEST** req,
                char* serial,
                const EVP_MD* cert_id_md,
                X509* issuer,
                STACK_OF(OCSP_CERTID)* ids);
static int
print_ocsp_summary(BIO* out,
                   OCSP_BASICRESP* bs,
                   OCSP_REQUEST* req,
...
static int
do_responder(OCSP_REQUEST** preq, BIO** pcbio, BIO* acbio, int timeout);
static int
send_ocsp_response(BIO* cbio, const OCSP_RESPONSE* resp);
static char* prog;
...
ocsp_main(int argc, char** argv)
{
  ...
  if (rsignfile != NULL) {
    ...
    if (rcertfile != NULL) {
      if (!load_certs(
            rcertfile, 0, &rother, NULL, "responder other certificates"))
        goto end;
    }
    ...
    rkey = load_key(
      rkeyfile, FORMAT_UNDEF, 0, passin, NULL, "responder private key");
    if (rkey == NULL)
      goto end;
  }
  ...
  if (acbio != NULL)
    trace_log_message(
      -1, prog, LOG_INFO, "waiting for OCSP client connections...");
...
redo_accept:
  ...
  if (signfile != NULL) {
    ...
    if (sign_certfile != NULL) {
      if (!load_certs(
            sign_certfile, 0, &sign_other, NULL, "signer certificates"))
        goto end;
    }
...
done_resp:
  ...
  if (verify_certfile != NULL) {
    if (!load_certs(
          verify_certfile, 0, &verify_other, NULL, "validator certificates"))
      goto end;
  }
                   ...
                   long maxage)
{
  ...
  for (i = 0; i < sk_OCSP_CERTID_num(ids); i++) {
    ...
    if (!OCSP_resp_find_status(
          bs, id, &status, &reason, &rev, &thisupd, &nextupd)) {
  ...
  for (i = 0; i < id_count; i++) {
    ...
    if (!found) {
      OCSP_basic_add1_status(
        bs, cid, V_OCSP_CERTSTATUS_UNKNOWN, 0, NULL, thisupd, nextupd);
      continue;
    }
    if (inf == NULL) {
      OCSP_basic_add1_status(
      ...
      OCSP_basic_add1_status(
        bs, cid, V_OCSP_CERTSTATUS_GOOD, 0, NULL, thisupd, nextupd);
      ...
      single = OCSP_basic_add1_status(
        bs, cid, V_OCSP_CERTSTATUS_REVOKED, reason, revtm, thisupd, nextupd);
      if (single == NULL) {
        *resp = OCSP_response_create(OCSP_RESPONSE_STATUS_INTERNALERROR, NULL);
        goto end;
      }
      if (invtm != NULL)
        ...
        OCSP_SINGLERESP_add1_ext_i2d(
          single, NID_hold_instruction_code, inst, 0, 0);
...
{
#ifndef OPENSSL_NO_SOCK
  return http_server_send_asn1_resp(prog,
                                    cbio,
                                    0 /* no keep-alive */,
                  ...
                  int req_timeout)
{
  ...
  resp = (OCSP_RESPONSE*)app_http_post_asn1(host,
                                            port,
                                            path,
                                            proxy,
                                            no_proxy,
