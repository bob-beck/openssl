...
int ts_main(int argc, char** argv) {
  ...
  if (!opt_md(digestname, &md)) goto opthelp;
  ...
  if (mode == OPT_QUERY) {
    if (vpmtouched) goto opthelp;
    if ((data != NULL) && (digest != NULL)) goto opthelp;
    ret =
        !query_command(data, digest, md, policy, no_nonce, cert, in, out, text);
  } else if (mode == OPT_REPLY) {
    if (vpmtouched) goto opthelp;
    if ((in != NULL) && (queryfile != NULL)) goto opthelp;
    if (in == NULL) {
      if ((conf == NULL) || (token_in != 0)) goto opthelp;
    }
    ...
    if ((in == NULL) || !EXACTLY_ONE(queryfile, data, digest)) goto opthelp;
...
static CONF* load_config_file(const char* configfile) {
  ...
  if (conf != NULL) {
    ...
    if (!add_oid_section(conf)) ERR_print_errors(bio_err);
  }
  return conf;
}
...
static int query_command(const char* data, const char* digest, const EVP_MD* md,
  ...
  if (text) {
    if ((out_bio = bio_open_default(out, 'w', FORMAT_TEXT)) == NULL) goto end;
    if (!TS_REQ_print_bio(out_bio, query)) goto end;
  } else {
    if ((out_bio = bio_open_default(out, 'w', FORMAT_ASN1)) == NULL) goto end;
    if (!i2d_TS_REQ_bio(out_bio, query)) goto end;
  }
...
static TS_REQ* create_query(BIO* data_bio, const char* digest, const EVP_MD* md,
  ...
  if ((ts_req = TS_REQ_new()) == NULL) goto err;
  if (!TS_REQ_set_version(ts_req, 1)) goto err;
  if ((msg_imprint = TS_MSG_IMPRINT_new()) == NULL) goto err;
  if ((algo = X509_ALGOR_new()) == NULL) goto err;
  if ((algo->algorithm = OBJ_nid2obj(EVP_MD_get_type(md))) == NULL) goto err;
  if ((algo->parameter = ASN1_TYPE_new()) == NULL) goto err;
  algo->parameter->type = V_ASN1_NULL;
  if (!TS_MSG_IMPRINT_set_algo(msg_imprint, algo)) goto err;
  if ((len = create_digest(data_bio, digest, md, &data)) == 0) goto err;
  if (!TS_MSG_IMPRINT_set_msg(msg_imprint, data, len)) goto err;
  if (!TS_REQ_set_msg_imprint(ts_req, msg_imprint)) goto err;
  if (policy && (policy_obj = txt2obj(policy)) == NULL) goto err;
  if (policy_obj && !TS_REQ_set_policy_id(ts_req, policy_obj)) goto err;

  /* Setting nonce if requested. */
  if (!no_nonce && (nonce_asn1 = create_nonce(NONCE_LENGTH)) == NULL) goto err;
...
static int create_digest(BIO* input, const char* digest, const EVP_MD* md,
  ...
  if (input != NULL) {
    ...
    *md_value = app_malloc(md_value_len, "digest buffer");
    if (!EVP_DigestInit(md_ctx, md)) goto err;
    while ((length = BIO_read(input, buffer, sizeof(buffer))) > 0) {
      if (!EVP_DigestUpdate(md_ctx, buffer, length)) goto err;
    }
    if (!EVP_DigestFinal(md_ctx, *md_value, NULL)) goto err;
    md_value_len = EVP_MD_get_size(md);
  } else {
    long digest_len;
...
static ASN1_INTEGER* create_nonce(int bits) {
  ...
  if ((nonce = ASN1_INTEGER_new()) == NULL) goto err;
...
static TS_RESP* read_PKCS7(BIO* in_bio) {
  ...
  if ((token = d2i_PKCS7_bio(in_bio, NULL)) == NULL) goto end;
  if ((tst_info = PKCS7_to_TS_TST_INFO(token)) == NULL) goto end;
  if ((resp = TS_RESP_new()) == NULL) goto end;
  if ((si = TS_STATUS_INFO_new()) == NULL) goto end;
  if (!TS_STATUS_INFO_set_status(si, TS_STATUS_GRANTED)) goto end;
...
static TS_RESP* create_response(CONF* conf, const char* section,
  ...
  if ((query_bio = BIO_new_file(queryfile, "rb")) == NULL) goto end;
  if ((section = TS_CONF_get_tsa_section(conf, section)) == NULL) goto end;
  if ((resp_ctx = TS_RESP_CTX_new()) == NULL) goto end;
  ...
  if (!TS_CONF_set_signer_key(conf, section, inkey, passin, resp_ctx)) goto end;
  ...
  if (!TS_CONF_set_ess_cert_id_digest(conf, section, resp_ctx)) goto end;
  ...
  if (!TS_CONF_set_clock_precision_digits(conf, section, resp_ctx)) goto end;
...
static ASN1_INTEGER* next_serial(const char* serialfile) {
  ...
  if ((serial = ASN1_INTEGER_new()) == NULL) goto err;
  ...
  if ((in = BIO_new_file(serialfile, "r")) == NULL) {
    ...
    if ((bn = ASN1_INTEGER_to_BN(serial, NULL)) == NULL) goto err;
    ASN1_INTEGER_free(serial);
    serial = NULL;
    if (!BN_add_word(bn, 1)) goto err;
    if ((serial = BN_to_ASN1_INTEGER(bn, NULL)) == NULL) goto err;
  }
  ret = 1;
...
static int save_ts_serial(const char* serialfile, ASN1_INTEGER* serial) {
  ...
  if ((out = BIO_new_file(serialfile, "w")) == NULL) goto err;
...
static int verify_command(const char* data, const char* digest,
  ...
  if (token_in) {
    ...
    if ((response = d2i_TS_RESP_bio(in_bio, NULL)) == NULL) goto end;
  }
...
static TS_VERIFY_CTX* create_verify_ctx(const char* data, const char* digest,
  ...
  if (data != NULL || digest != NULL) {
    if ((ctx = TS_VERIFY_CTX_new()) == NULL) goto err;
    f = TS_VFY_VERSION | TS_VFY_SIGNER;
    if (data != NULL) {
      BIO* out = NULL;

      f |= TS_VFY_DATA;
      if ((out = BIO_new_file(data, "rb")) == NULL) goto err;
    ...
    if ((input = BIO_new_file(queryfile, "rb")) == NULL) goto err;
    if ((request = d2i_TS_REQ_bio(input, NULL)) == NULL) goto err;
    if ((ctx = TS_REQ_to_TS_VERIFY_CTX(request, NULL)) == NULL) goto err;
  } else {
    return NULL;
  }
  ...
  if (untrusted != NULL) {
    ...
    if (certs == NULL || !TS_VERIFY_CTX_set0_certs(ctx, certs)) goto err;
  }
  ret = 1;
...
static X509_STORE* create_cert_store(const char* CApath, const char* CAfile,
  ...
  if (vpm != NULL) X509_STORE_set1_param(cert_ctx, vpm);
