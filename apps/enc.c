...
const OPTIONS enc_options[] = {
  ...
  { OPT_MORE_STR, 0, 0,
    "Use -iter to change the iteration count from " STR (
        PBKDF2_ITER_DEFAULT) },
          ...
          if (pbkdf2 == 1)
            {
              ...
              if (!PKCS5_PBKDF2_HMAC (str, (int)str_len, sptr, islen, iter,
                                      dgst, iklen + ivlen, tmpkeyiv))
                {
...
static int
set_hex (const char *in, unsigned char *out, int size)
{
  int i, n;
  unsigned char j;
