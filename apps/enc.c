...
int enc_main(int argc, char** argv) {
  ...
  if (strcmp(argv[0], "base64") == 0) base64 = 1;
#ifndef OPENSSL_NO_ZLIB
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_ITER:
        ...
        break;
      case OPT_SALTLEN:
        if (!opt_int(opt_arg(), &saltlen)) goto opthelp;
        if (saltlen > (int)sizeof(salt)) saltlen = (int)sizeof(salt);
        break;
      case OPT_PBKDF2:
        pbkdf2 = 1;
  ...
  if (digestname != NULL) {
    if (!opt_md(digestname, &dgst)) goto opthelp;
  }
  if (dgst == NULL) dgst = (EVP_MD*)EVP_sha256();
...
#ifndef OPENSSL_NO_ZLIB
  if (do_zlib) {
    if ((bzl = BIO_new(BIO_f_zlib())) == NULL) goto end;
    if (debug) {
  ...
  if (do_brotli) {
    if ((bbrot = BIO_new(BIO_f_brotli())) == NULL) goto end;
    if (debug) {
  ...
  if (do_zstd) {
    if ((bzstd = BIO_new(BIO_f_zstd())) == NULL) goto end;
    if (debug) {
  ...
  if (base64) {
    if ((b64 = BIO_new(BIO_f_base64())) == NULL) goto end;
    if (debug) {
  ...
  if (cipher != NULL) {
    if (str != NULL) { /* a passphrase is available */
      ...
      if (pbkdf2 == 1) {
        ...
        if (!PKCS5_PBKDF2_HMAC(str, (int)str_len, sptr, islen, iter, dgst,
    ...
    if ((benc = BIO_new(BIO_f_cipher())) == NULL) goto end;
    ...
    if (nopad) EVP_CIPHER_CTX_set_padding(ctx, 0);
