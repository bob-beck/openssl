...
int asn1parse_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_HELP:
        ...
        goto end;
      case OPT_INFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ASN1, &informat)) goto opthelp;
        break;
      case OPT_IN:
        infile = opt_arg();
        break;
      ...
      case OPT_DLIMIT:
        ...
        break;
      case OPT_STRPARSE:
        if (sk_OPENSSL_STRING_push(osk, opt_arg()) <= 0) goto end;
        break;
      case OPT_GENSTR:
        genstr = opt_arg();
        break;
  ...
  if ((in = bio_open_default(infile, 'r', informat)) == NULL) goto end;
  ...
  if (genconf == NULL && genstr == NULL && informat == FORMAT_PEM) {
    ...
    if (genstr || genconf) {
      ...
      num = 0;
      for (;;) {
        if (!BUF_MEM_grow(buf, num + BUFSIZ)) goto end;
        i = BIO_read(in, &(buf->data[num]), BUFSIZ);
        if (i <= 0) break;
        /* make sure num doesn't overflow */
        if (i > LONG_MAX - num) goto end;
        num += i;
      }
    }
...
static int do_generate(char* genstr, const char* genconf, BUF_MEM* buf) {
  ...
  if (genconf != NULL) {
    if ((cnf = app_load_config(genconf)) == NULL) goto err;
    if (genstr == NULL) genstr = NCONF_get_string(cnf, "default", "asn1");
    if (genstr == NULL) {
