...
/**
 * Parse the passed OpenSSL configuration file (or the default one/specified in
 * the OPENSSL_CONF environment variable) and write it back in a canonical
 * format with all includes and variables expanded.
 */
int configutl_main(int argc, char* argv[]) {
  ...
  if (configfile == NULL) goto end;

  if ((cnf = NCONF_new(NULL)) == NULL) goto end;
  ...
  if (no_header == 0)
    BIO_printf(
        out, "# This configuration file was linearized and expanded from %s\n",
        configfile);
  ...
  for (idx = 0; idx < sk_OPENSSL_CSTRING_num(sections); idx++) {
    ...
    if (idx == default_section_idx) continue;
