...
static void list_ciphers(const char* prefix) {
  ...
  for (i = 0; i < sk_EVP_CIPHER_num(ciphers); i++) {
    ...
    if (select_name != NULL && !EVP_CIPHER_is_a(c, select_name)) continue;
...
static void list_digests(const char* prefix) {
  ...
  for (i = 0; i < sk_EVP_MD_num(digests); i++) {
    ...
    if (select_name != NULL && !EVP_MD_is_a(m, select_name)) continue;
...
static void list_macs(void) {
  ...
  for (i = 0; i < sk_EVP_MAC_num(macs); i++) {
    ...
    if (select_name != NULL && !EVP_MAC_is_a(m, select_name)) continue;
...
static void list_kdfs(void) {
  ...
  for (i = 0; i < sk_EVP_KDF_num(kdfs); i++) {
    ...
    if (select_name != NULL && !EVP_KDF_is_a(k, select_name)) continue;
  ...
  if (drbg != NULL) {
    ...
    if (gettables != NULL)
      for (; gettables->key != NULL; gettables++) {
        ...
        if (EVP_RAND_CTX_get_params(drbg, params)) print_param_value(params, 2);
      }
  }
}
...
static void list_encoders(void) {
  ...
  for (i = 0; i < sk_OSSL_ENCODER_num(encoders); i++) {
    ...
    if (select_name != NULL && !OSSL_ENCODER_is_a(k, select_name)) continue;
...
static void list_decoders(void) {
  ...
  for (i = 0; i < sk_OSSL_DECODER_num(decoders); i++) {
    ...
    if (select_name != NULL && !OSSL_DECODER_is_a(k, select_name)) continue;
...
static void list_keymanagers(void) {
  ...
  for (i = 0; i < sk_EVP_KEYMGMT_num(km_stack); i++) {
    ...
    if (select_name != NULL && !EVP_KEYMGMT_is_a(k, select_name)) continue;
...
static void list_skeymanagers(void) {
  ...
  for (i = 0; i < sk_EVP_SKEYMGMT_num(km_stack); i++) {
    ...
    if (select_name != NULL && !EVP_SKEYMGMT_is_a(k, select_name)) continue;
...
static void list_signatures(void) {
  ...
  for (i = 0; i < sk_EVP_SIGNATURE_num(sig_stack); i++) {
    ...
    if (select_name != NULL && !EVP_SIGNATURE_is_a(k, select_name)) continue;
...
static int list_provider_tls_sigalgs(const OSSL_PARAM params[], void* data) {
  ...
  if (p != NULL && p->data_type == OSSL_PARAM_UTF8_STRING) {
    if (*((int*)data) > 0) BIO_printf(bio_out, ":");
...
static void list_kems(void) {
  ...
  for (i = 0; i < sk_EVP_KEM_num(kem_stack); i++) {
    ...
    if (select_name != NULL && !EVP_KEM_is_a(k, select_name)) continue;
...
static void list_asymciphers(void) {
  ...
  for (i = 0; i < sk_EVP_ASYM_CIPHER_num(asymciph_stack); i++) {
    ...
    if (select_name != NULL && !EVP_ASYM_CIPHER_is_a(k, select_name)) continue;
...
static void list_keyexchanges(void) {
  ...
  for (i = 0; i < sk_EVP_KEYEXCH_num(kex_stack); i++) {
    ...
    if (select_name != NULL && !EVP_KEYEXCH_is_a(k, select_name)) continue;
...
static void list_objects(void) {
  ...
  for (i = 1; i < max_nid; i++) {
    ...
    if (OBJ_obj2nid(obj) == NID_undef) continue;
    ...
    if (OBJ_obj2txt(oid_buf, oid_size, obj, 1) < 0) break; /* Error */
...
static void list_options_for_command(const char* command) {
  ...
  for (fp = functions; fp->name != NULL; fp++)
    if (strcmp(fp->name, command) == 0) break;
  if (fp->name == NULL) {
  ...
  if ((o = fp->help) == NULL) return;

  for (; o->name != NULL; o++) {
    char c = o->valtype;

    if (o->name == OPT_PARAM_STR) break;
...
static void list_type(FUNC_TYPE ft, int one) {
  ...
  if (!one) calculate_columns(functions, &dc);

  for (fp = functions; fp->name != NULL; fp++) {
    if (fp->type != ft) continue;
    switch (ft) {
      case FT_cipher:
        if (!is_cipher_available(fp->name)) continue;
        break;
      case FT_md:
        if (!is_md_available(fp->name)) continue;
        break;
      default:
        break;
    }
    if (one) {
      ...
      if (i % dc.columns == 0 && i > 0) BIO_printf(bio_out, "\n");
...
#define MAYBE_ADD_NL(cmd)        \
  ...
  if (todo.commands) MAYBE_ADD_NL(list_type(FT_general, one));
  ...
  if (todo.random_instances) MAYBE_ADD_NL(list_random_instances());
  if (todo.random_generators) MAYBE_ADD_NL(list_random_generators());
  if (todo.digest_commands) MAYBE_ADD_NL(list_type(FT_md, one));
  if (todo.digest_algorithms) MAYBE_ADD_NL(list_digests(""));
  if (todo.kdf_algorithms) MAYBE_ADD_NL(list_kdfs());
  if (todo.mac_algorithms) MAYBE_ADD_NL(list_macs());
  if (todo.cipher_commands) MAYBE_ADD_NL(list_type(FT_cipher, one));
  if (todo.cipher_algorithms) MAYBE_ADD_NL(list_ciphers(""));
  if (todo.encoder_algorithms) MAYBE_ADD_NL(list_encoders());
  if (todo.decoder_algorithms) MAYBE_ADD_NL(list_decoders());
  if (todo.keymanager_algorithms) MAYBE_ADD_NL(list_keymanagers());
  if (todo.skeymanager_algorithms) MAYBE_ADD_NL(list_skeymanagers());
  if (todo.signature_algorithms) MAYBE_ADD_NL(list_signatures());
  if (todo.tls_signature_algorithms) MAYBE_ADD_NL(list_tls_signatures());
  if (todo.asym_cipher_algorithms) MAYBE_ADD_NL(list_asymciphers());
  if (todo.keyexchange_algorithms) MAYBE_ADD_NL(list_keyexchanges());
  if (todo.kem_algorithms) MAYBE_ADD_NL(list_kems());
#if !defined(OPENSSL_NO_TLS1_3) || !defined(OPENSSL_NO_TLS1_2)
  if (todo.tls_groups)
    MAYBE_ADD_NL(list_tls_groups(tls_version, all_tls_groups));
#endif
  if (todo.pk_algorithms) MAYBE_ADD_NL(list_pkey());
  if (todo.pk_method) MAYBE_ADD_NL(list_pkey_meth());
  if (todo.store_loaders) MAYBE_ADD_NL(list_store_loaders());
  if (todo.provider_info) MAYBE_ADD_NL(list_provider_info());
#ifndef OPENSSL_NO_DEPRECATED_3_0
  if (todo.engines) MAYBE_ADD_NL(list_engines());
#endif
  if (todo.disabled) MAYBE_ADD_NL(list_disabled());
  if (todo.objects) MAYBE_ADD_NL(list_objects());

#undef MAYBE_ADD_NL

  if (!done) goto opthelp;
