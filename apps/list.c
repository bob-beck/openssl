...
static void
legacy_cipher_fn(const EVP_CIPHER* c,
                 const char* from,
                 const char* to,
                 void* arg)
{
...
static int
cipher_cmp(const EVP_CIPHER* const* a, const EVP_CIPHER* const* b)
{
...
static void
collect_ciphers(EVP_CIPHER* cipher, void* stack)
{
...
static void
list_ciphers(const char* prefix)
{
  ...
  for (i = 0; i < sk_EVP_CIPHER_num(ciphers); i++) {
    ...
    if (names != NULL && EVP_CIPHER_names_do_all(c, collect_names, names)) {
      ...
      if (verbose) {
        ...
        print_param_types(
          "retrievable algorithm parameters", EVP_CIPHER_gettable_params(c), 4);
        print_param_types("retrievable operation parameters",
                          EVP_CIPHER_gettable_ctx_params(c),
                          4);
        print_param_types("settable operation parameters",
                          EVP_CIPHER_settable_ctx_params(c),
                          4);
      }
    }
...
#ifndef OPENSSL_NO_DEPRECATED_3_0
static void
legacy_md_fn(const EVP_MD* m, const char* from, const char* to, void* arg)
{
  if (m != NULL) {
    BIO_printf(arg, "  %s\n", EVP_MD_get0_name(m));
  } else {
...
static int
md_cmp(const EVP_MD* const* a, const EVP_MD* const* b)
{
...
static void
collect_digests(EVP_MD* digest, void* stack)
{
...
static void
list_digests(const char* prefix)
{
    ...
    if (names != NULL && EVP_MD_names_do_all(m, collect_names, names)) {
      ...
      if (verbose) {
        ...
        if (desc != NULL)
          BIO_printf(bio_out, "    description: %s\n", desc);
        print_param_types(
          "retrievable algorithm parameters", EVP_MD_gettable_params(m), 4);
        print_param_types(
          "retrievable operation parameters", EVP_MD_gettable_ctx_params(m), 4);
        print_param_types(
          "settable operation parameters", EVP_MD_settable_ctx_params(m), 4);
      }
    }
    sk_OPENSSL_CSTRING_free(names);
  }
...
static int
mac_cmp(const EVP_MAC* const* a, const EVP_MAC* const* b)
{
...
static void
collect_macs(EVP_MAC* mac, void* stack)
{
    ...
    if (names != NULL && EVP_MAC_names_do_all(m, collect_names, names)) {
      ...
      if (verbose) {
        ...
        if (desc != NULL)
          BIO_printf(bio_out, "    description: %s\n", desc);
        print_param_types(
          "retrievable algorithm parameters", EVP_MAC_gettable_params(m), 4);
        print_param_types("retrievable operation parameters",
                          EVP_MAC_gettable_ctx_params(m),
                          4);
        print_param_types(
          "settable operation parameters", EVP_MAC_settable_ctx_params(m), 4);
      }
    }
    sk_OPENSSL_CSTRING_free(names);
  }
...
static int
kdf_cmp(const EVP_KDF* const* a, const EVP_KDF* const* b)
{
...
static void
collect_kdfs(EVP_KDF* kdf, void* stack)
{
    ...
    if (names != NULL && EVP_KDF_names_do_all(k, collect_names, names)) {
      ...
      if (verbose) {
        ...
        if (desc != NULL)
          BIO_printf(bio_out, "    description: %s\n", desc);
        print_param_types(
          "retrievable algorithm parameters", EVP_KDF_gettable_params(k), 4);
        print_param_types("retrievable operation parameters",
                          EVP_KDF_gettable_ctx_params(k),
                          4);
        print_param_types(
          "settable operation parameters", EVP_KDF_settable_ctx_params(k), 4);
      }
    }
    sk_OPENSSL_CSTRING_free(names);
  }
...
static int
rand_cmp(const EVP_RAND* const* a, const EVP_RAND* const* b)
{
...
static void
collect_rands(EVP_RAND* rand, void* stack)
{
  ...
  for (i = 0; i < sk_EVP_RAND_num(rands); i++) {
    ...
    if (verbose) {
      ...
      if (desc != NULL)
        BIO_printf(bio_out, "    description: %s\n", desc);
      print_param_types(
        "retrievable algorithm parameters", EVP_RAND_gettable_params(m), 4);
      print_param_types(
        "retrievable operation parameters", EVP_RAND_gettable_ctx_params(m), 4);
      print_param_types(
        "settable operation parameters", EVP_RAND_settable_ctx_params(m), 4);
    }
  }
...
static void
display_random(const char* name, EVP_RAND_CTX* drbg)
{
  EVP_RAND* rand;
  uint64_t u;
  const char* p;
...
static int
encoder_cmp(const OSSL_ENCODER* const* a, const OSSL_ENCODER* const* b)
{
...
static void
collect_encoders(OSSL_ENCODER* encoder, void* stack)
{
    ...
    if (names != NULL && OSSL_ENCODER_names_do_all(k, collect_names, names)) {
      ...
      if (verbose) {
        ...
        if (desc != NULL)
          BIO_printf(bio_out, "    description: %s\n", desc);
        print_param_types("settable operation parameters",
                          OSSL_ENCODER_settable_ctx_params(k),
                          4);
      }
    }
    sk_OPENSSL_CSTRING_free(names);
...
static int
decoder_cmp(const OSSL_DECODER* const* a, const OSSL_DECODER* const* b)
{
...
static void
collect_decoders(OSSL_DECODER* decoder, void* stack)
{
    ...
    if (names != NULL && OSSL_DECODER_names_do_all(k, collect_names, names)) {
      ...
      if (verbose) {
        ...
        if (desc != NULL)
          BIO_printf(bio_out, "    description: %s\n", desc);
        print_param_types("settable operation parameters",
                          OSSL_DECODER_settable_ctx_params(k),
                          4);
      }
    }
    sk_OPENSSL_CSTRING_free(names);
...
static int
keymanager_cmp(const EVP_KEYMGMT* const* a, const EVP_KEYMGMT* const* b)
{
...
static void
collect_keymanagers(EVP_KEYMGMT* km, void* stack)
{
    ...
    if (names != NULL && EVP_KEYMGMT_names_do_all(k, collect_names, names)) {
      ...
      if (verbose) {
        print_param_types("settable key generation parameters",
                          EVP_KEYMGMT_gen_settable_params(k),
                          4);
        print_param_types(
          "settable operation parameters", EVP_KEYMGMT_settable_params(k), 4);
        print_param_types("retrievable operation parameters",
                          EVP_KEYMGMT_gettable_params(k),
                          4);
      }
    }
    sk_OPENSSL_CSTRING_free(names);
...
static int
skeymanager_cmp(const EVP_SKEYMGMT* const* a, const EVP_SKEYMGMT* const* b)
{
...
static void
collect_skeymanagers(EVP_SKEYMGMT* km, void* stack)
{
...
static int
signature_cmp(const EVP_SIGNATURE* const* a, const EVP_SIGNATURE* const* b)
{
...
static int
list_provider_tls_sigalgs(const OSSL_PARAM params[], void* data)
{
  const OSSL_PARAM* p;
...
static int
list_tls_sigalg_caps(OSSL_PROVIDER* provider, void* cbdata)
{
  OSSL_PROVIDER_get_capabilities(
    provider, "TLS-SIGALG", list_provider_tls_sigalgs, cbdata);
  /* As built-in providers don't have this capability, never error */
  return 1;
}
...
static void
list_tls_groups(int version, int all)
{
  SSL_CTX* ctx = NULL;
  ...
  for (i = 0; i < num; ++i) {
    BIO_printf(bio_out,
               "%s%c",
               sk_OPENSSL_CSTRING_value(groups, i),
               (i < num - 1) ? ':' : '\n');
  }
err:
  SSL_CTX_free(ctx);
...
static int
kem_cmp(const EVP_KEM* const* a, const EVP_KEM* const* b)
{
    ...
    if (names != NULL && EVP_KEM_names_do_all(k, collect_names, names)) {
      ...
      if (verbose) {
        ...
        if (desc != NULL)
          BIO_printf(bio_out, "    description: %s\n", desc);
        print_param_types(
          "settable operation parameters", EVP_KEM_settable_ctx_params(k), 4);
        print_param_types("retrievable operation parameters",
                          EVP_KEM_gettable_ctx_params(k),
                          4);
      }
    }
    sk_OPENSSL_CSTRING_free(names);
...
static int
asymcipher_cmp(const EVP_ASYM_CIPHER* const* a, const EVP_ASYM_CIPHER* const* b)
{
...
static void
collect_asymciph(EVP_ASYM_CIPHER* asym_cipher, void* stack)
{
...
list_asymciphers(void)
{
  ...
  EVP_ASYM_CIPHER_do_all_provided(
    app_get0_libctx(), collect_asymciph, asymciph_stack);
...
static int
kex_cmp(const EVP_KEYEXCH* const* a, const EVP_KEYEXCH* const* b)
{
...
static void
collect_kex(EVP_KEYEXCH* kex, void* stack)
{
    ...
    if (names != NULL && EVP_KEYEXCH_names_do_all(k, collect_names, names)) {
      ...
      if (verbose) {
        ...
        if (desc != NULL)
          BIO_printf(bio_out, "    description: %s\n", desc);
        print_param_types("settable operation parameters",
                          EVP_KEYEXCH_settable_ctx_params(k),
                          4);
        print_param_types("retrievable operation parameters",
                          EVP_KEYEXCH_gettable_ctx_params(k),
                          4);
      }
    }
    sk_OPENSSL_CSTRING_free(names);
...
static void
list_options_for_command(const char* command)
{
  const FUNCTION* fp;
  const OPTIONS* o;
...
static int
is_md_available(const char* name)
{
  EVP_MD* md;
  const char* propq = app_get0_propq();
...
static int
is_cipher_available(const char* name)
{
  EVP_CIPHER* cipher;
  const char* propq = app_get0_propq();
...
static void
list_type(FUNC_TYPE ft, int one)
{
  FUNCTION* fp;
  int i = 0;
  DISPLAY_COLUMNS dc;
  ...
  if (select_name == NULL && include_legacy()) {
    ...
    for (i = 0; i < EVP_PKEY_asn1_get_count(); i++) {
      ...
      EVP_PKEY_asn1_get0_info(
        &pkey_id, &pkey_base_id, &pkey_flags, &pinfo, &pem_str, ameth);
...
static int
store_cmp(const OSSL_STORE_LOADER* const* a, const OSSL_STORE_LOADER* const* b)
{
...
static void
collect_store_loaders(OSSL_STORE_LOADER* store, void* stack)
{
...
list_store_loaders(void)
{
  ...
  OSSL_STORE_LOADER_do_all_provided(
    app_get0_libctx(), collect_store_loaders, stores);
    ...
    if (names != NULL &&
      ...
      if (verbose) {
        ...
        if (desc != NULL)
          BIO_printf(bio_out, "    description: %s\n", desc);
        print_param_types("settable operation parameters",
                          OSSL_STORE_LOADER_settable_ctx_params(l),
                          4);
      }
    }
    sk_OPENSSL_CSTRING_free(names);
...
static int
provider_cmp(const OSSL_PROVIDER* const* a, const OSSL_PROVIDER* const* b)
{
  return strcmp(OSSL_PROVIDER_get0_name(*a), OSSL_PROVIDER_get0_name(*b));
}

static int
collect_providers(OSSL_PROVIDER* provider, void* stack)
{
    ...
    if (!OSSL_PROVIDER_get_params(prov, params)) {
      ...
      if (verbose) {
        ...
        print_param_types("gettable provider parameters",
                          OSSL_PROVIDER_gettable_params(prov),
                          4);
      }
    }
  }
