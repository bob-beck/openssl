...
int s_time_main(int argc, char** argv) {
  ...
  if ((ctx = SSL_CTX_new(meth)) == NULL) goto end;

  SSL_CTX_set_quiet_shutdown(ctx, 1);
  if (SSL_CTX_set_min_proto_version(ctx, min_version) == 0) goto end;
  if (SSL_CTX_set_max_proto_version(ctx, max_version) == 0) goto end;

  if (st_bugs) SSL_CTX_set_options(ctx, SSL_OP_ALL);
  if (cipher != NULL && !SSL_CTX_set_cipher_list(ctx, cipher)) goto end;
  if (ciphersuites != NULL && !SSL_CTX_set_ciphersuites(ctx, ciphersuites))
    goto end;
  if (!set_cert_stuff(ctx, certfile, keyfile)) goto end;
  ...
  for (;;) {
    ...
    if ((scon = doConnection(NULL, host, ctx)) == NULL) goto end;

    if (www_path != NULL) {
      buf_len = BIO_snprintf(buf, sizeof(buf), fmt_http_get_cmd, www_path);
      if (buf_len <= 0 || SSL_write(scon, buf, buf_len) <= 0) goto end;
...
next:
  ...
  SSL_set_shutdown(scon, SSL_SENT_SHUTDOWN | SSL_RECEIVED_SHUTDOWN);
  if ((fd = SSL_get_fd(scon)) >= 0) BIO_closesocket(fd);
  ...
  for (;;) {
    ...
    if ((doConnection(scon, host, ctx)) == NULL) goto end;
...
static SSL* doConnection(SSL* scon, const char* host, SSL_CTX* ctx) {
  ...
  if ((conn = BIO_new(BIO_s_connect())) == NULL) return NULL;
  ...
  if (i <= 0) {
    ...
    if (verify_args.error != X509_V_OK)
      ...
      ERR_print_errors(bio_err);
    if (scon == NULL) SSL_free(serverCon);
    return NULL;
  }
