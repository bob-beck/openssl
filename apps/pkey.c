...
int pkey_main(int argc, char** argv) {
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_HELP:
        ...
        goto end;
      case OPT_INFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &informat)) goto opthelp;
        break;
      case OPT_OUTFORM:
      ...
      case OPT_EC_CONV_FORM:
        ...
        if (!opt_string(point_format, point_format_options)) goto opthelp;
        break;
#endif
      case OPT_EC_PARAM_ENC:
        ...
        if (!opt_string(asn1_encoding, asn1_encoding_options)) goto opthelp;
        break;
#endif
      case OPT_PROV_CASES:
  ...
  if (asn1_encoding != NULL || point_format != NULL) {
    ...
    if (!EVP_PKEY_is_a(pkey, "EC")) goto end;
    ...
    *p = OSSL_PARAM_construct_end();
    if (EVP_PKEY_set_params(pkey, params) <= 0) goto end;
  }
#endif
  ...
  if (!noout) {
    if (outformat == FORMAT_PEM) {
      if (pubout) {
        if (!PEM_write_bio_PUBKEY(out, pkey)) goto end;
      } else {
        assert(private);
        if (traditional) {
      ...
      if (pubout) {
        if (!i2d_PUBKEY_bio(out, pkey)) goto end;
      } else {
        assert(private);
        if (traditional) {
          if (!i2d_PrivateKey_bio(out, pkey)) goto end;
        } else {
  ...
  if (text_pub) {
    if (EVP_PKEY_print_public(out, pkey, 0, NULL) <= 0) goto end;
  } else if (text) {
    assert(private);
    if (EVP_PKEY_print_private(out, pkey, 0, NULL) <= 0) goto end;
  }
