...
static void
print_entry(CA_DB* db, int indx, int verbose, char* s)
{
  if (indx >= 0 && verbose) {
    int j;
...
static void
print_index(CA_DB* db, int indexindex, int verbose)
{
  print_entry(db, indexindex, verbose, "g N entry");
}

static void
print_user(CA_DB* db, int userindex, int verbose)
{
  if (verbose > 0) {
...
static int
update_index(CA_DB* db, char** row)
{
  char** irow;
  int i;
...
static char*
srp_verify_user(const char* user,
                ...
                int verbose)
{
  ...
  if (len > 0) {
    ...
    if ((gNid = SRP_create_verifier(
           user, password, &srp_usersalt, &verifier, N, g)) == NULL) {
  ...
  if (len > 0) {
    ...
    if (verbose)
      BIO_printf(
        bio_err, "Creating\n user=\"%s\"\n g=\"%s\"\n N=\"%s\"\n", user, g, N);
    if ((gNid = SRP_create_verifier(
           user, password, &salt, srp_verifier, N, g)) == NULL) {
...
srp_main(int argc, char** argv)
{
  ...
  if (mode == OPT_ERR) {
    BIO_printf(bio_err,
               "Exactly one of the options -add, -delete, -modify -list must "
               "be specified.\n");
    goto opthelp;
  }
  ...
  while (mode == OPT_LIST || user != NULL) {
    ...
    if (mode == OPT_LIST) {
      ...
      if (userindex < 0) {
        BIO_printf(
          bio_err, "user \"%s\" does not exist, operation ignored.\n", user);
        errors++;
      } else {
        ...
        if (type == 'v') {
          BIO_printf(
            bio_err, "user \"%s\" already updated, operation ignored.\n", user);
          errors++;
        } else {
          char* gNid;
          ...
          if (row[DB_srptype][0] == 'V') {
            ...
            if (!srp_verify_user(user,
                                 row[DB_srpverifier],
                                 row[DB_srpsalt],
                                 irow ? irow[DB_srpsalt] : row[DB_srpgN],
                                 irow ? irow[DB_srpverifier] : NULL,
                                 passin,
                                 verbose)) {
              BIO_printf(
                bio_err,
      ...
      if (userindex < 0) {
        BIO_printf(
          bio_err, "user \"%s\" does not exist, operation ignored. t\n", user);
        errors++;
      } else {
