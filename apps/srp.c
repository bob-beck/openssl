...
static char* srp_verify_user(const char* user, const char* srp_verifier,
  ...
  if (len > 0) {
    ...
    if (verbose > 1) BIO_printf(bio_err, "Pass %s\n", password);
    ...
    if ((gNid = SRP_create_verifier(user, password, &srp_usersalt, &verifier, N,
      ...
      if (strcmp(verifier, srp_verifier)) gNid = NULL;
...
int srp_main(int argc, char** argv) {
  ...
  if (doupdatedb) {
    ...
    if (!save_index(srpvfile, "new", db)) goto end;

    if (verbose) BIO_printf(bio_err, "Temporary srpvfile created.\n");
    if (!rotate_index(srpvfile, "new", "old")) goto end;
  ...
  if (errors != 0)
    if (verbose) BIO_printf(bio_err, "User errors %d.\n", errors);

  if (verbose) BIO_printf(bio_err, "SRP terminating with code %d.\n", ret);
