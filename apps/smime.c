...
int smime_main(int argc, char** argv) {
  ...
  if ((vpm = X509_VERIFY_PARAM_new()) == NULL) return 1;
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_OUT:
        ...
        break;
      case OPT_ENCRYPT:
        if (!SET_OPERATION(SMIME_ENCRYPT)) goto end;
        break;
      case OPT_DECRYPT:
        if (!SET_OPERATION(SMIME_DECRYPT)) goto end;
        break;
      case OPT_SIGN:
        if (!SET_OPERATION(SMIME_SIGN)) goto end;
        break;
      case OPT_RESIGN:
        if (!SET_OPERATION(SMIME_RESIGN)) goto end;
        break;
      case OPT_VERIFY:
        if (!SET_OPERATION(SMIME_VERIFY)) goto end;
        break;
      case OPT_PK7OUT:
        if (!SET_OPERATION(SMIME_PK7OUT)) goto end;
        break;
      case OPT_TEXT:
        flags |= PKCS7_TEXT;
        break;
  ...
  if (digestname != NULL) {
    if (!opt_md(digestname, &sign_md)) goto opthelp;
  }
  ...
  if (operation & SMIME_SIGNERS) {
    ...
    if (signerfile != NULL) {
      if (sksigners == NULL &&
        ...
        goto end;
      if (sk_OPENSSL_STRING_push(sksigners, signerfile) <= 0) goto end;
      if (!skkeys && (skkeys = sk_OPENSSL_STRING_new_null()) == NULL) goto end;
  ...
  if (!(operation & SMIME_OP)) {
    if (flags & PKCS7_BINARY) outformat = FORMAT_BINARY;
  }

  if (!(operation & SMIME_IP)) {
    if (flags & PKCS7_BINARY) informat = FORMAT_BINARY;
  }

  if (operation == SMIME_ENCRYPT) {
    if (cipher == NULL) cipher = (EVP_CIPHER*)EVP_aes_256_cbc();
    encerts = sk_X509_new_null();
    if (encerts == NULL) goto end;
    while (*argv != NULL) {
  ...
  if (operation == SMIME_ENCRYPT) {
    ...
    if (operation == SMIME_SIGN) {
      if (flags & PKCS7_DETACHED) {
        if (outformat == FORMAT_SMIME) flags |= PKCS7_STREAM;
      } else if (indef) {
        flags |= PKCS7_STREAM;
      }
      flags |= PKCS7_PARTIAL;
    ...
    for (i = 0; i < sk_OPENSSL_STRING_num(sksigners); i++) {
      ...
      if (signer == NULL) goto end;
      key = load_key(keyfile, keyform, 0, passin, e, "signing key");
      if (key == NULL) goto end;

      if (!PKCS7_sign_add_signer(p7, signer, key, sign_md, flags)) goto end;
      X509_free(signer);
      signer = NULL;
      EVP_PKEY_free(key);
      key = NULL;
    ...
    if ((operation == SMIME_SIGN) && !(flags & PKCS7_STREAM)) {
      if (!PKCS7_final(p7, in, flags)) goto end;
    }
  }
  ...
  if (operation == SMIME_DECRYPT) {
    ...
    if (to) BIO_printf(out, "To: %s%s", to, mime_eol);
    if (from) BIO_printf(out, "From: %s%s", from, mime_eol);
    if (subject) BIO_printf(out, "Subject: %s%s", subject, mime_eol);
