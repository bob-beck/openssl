...
int rand_main(int argc, char** argv) {
  ...
  if (argc == 1) {
    ...
    if (shift != 0) {
      ...
      if (scaled_num == 0) scaled_num = num;
    }
  ...
  while (scaled_num > 0) {
    ...
    if (format != FORMAT_TEXT) {
      if (BIO_write(out, buf, chunk) != chunk) goto end;
    } else {
      for (i = 0; i < chunk; i++)
        if (BIO_printf(out, "%02x", buf[i]) != 2) goto end;
    }
    scaled_num -= chunk;
  }
  if (format == FORMAT_TEXT) BIO_puts(out, "\n");
  if (BIO_flush(out) <= 0) goto end;
