...
int speed_main(int argc, char **argv)
{
    ...
    for (idx = 0; idx < (unsigned int)sk_EVP_SIGNATURE_num(sig_stack); idx++)
    {
        ...
        else if (strncmp(sig_name, "RSA", 3) && strncmp(sig_name, "DSA", 3) && strncmp(sig_name, "ED25519", 7) &&
                 strncmp(sig_name, "ED448", 5) && strncmp(sig_name, "ECDSA", 5) && strcmp(sig_name, "HMAC") &&
                 strcmp(sig_name, "SIPHASH") && strcmp(sig_name, "POLY1305") && strcmp(sig_name, "CMAC") &&
                 strcmp(sig_name, "SM2"))
        { /* skip alg */
    ...
    for (testnum = 0; testnum < sigs_algs_len; testnum++)
    {
        ...
        for (i = 0; i < loopargs_len; i++)
        {
            ...
            if (EVP_PKEY_sign_init(sig_sign_ctx) <= 0 &&
                (alg == NULL || EVP_PKEY_sign_message_init(sig_sign_ctx, alg, NULL) <= 0))
            {
                ERR_clear_last_mark();
            ...
            {
                BIO_printf(bio_err, "Error while obtaining signature bufffer length for %s.\n", sig_name);
                goto sig_err_break;
            }
            ...
            if (EVP_PKEY_verify_init(sig_verify_ctx) <= 0 &&
                (alg == NULL || EVP_PKEY_verify_message_init(sig_verify_ctx, alg, NULL) <= 0))
            {
                ERR_clear_last_mark();
