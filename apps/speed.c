...
static int
opt_found (const char *name, unsigned int *result, const OPT_PAIR pairs[],
           unsigned int nbelem)
{
  unsigned int idx;
...
static const OPT_PAIR dsa_choices[DSA_NUM]
    = { { "dsa1024", R_DSA_1024 }, { "dsa2048", R_DSA_2048 } };
...
static const OPT_PAIR eddsa_choices[EdDSA_NUM]
    = { { "ed25519", R_EC_Ed25519 }, { "ed448", R_EC_Ed448 }
...
static int
mac_setup (const char *name, EVP_MAC **mac, OSSL_PARAM params[],
...
init_evp_cipher_ctx (const char *ciphername, const unsigned char *key,
                     int keylen)
{
...
RSA_encrypt_loop (void *args)
{
  ...
  for (count = 0; COND (rsa_c[testnum][2]); count++)
    {
      *rsa_num = tempargs->buflen;
      ret = EVP_PKEY_encrypt (rsa_encrypt_ctx[testnum], buf2, rsa_num, buf,
                              36);
      if (ret <= 0)
        {
...
check_block_size (EVP_CIPHER_CTX *ctx, int length)
{
  ...
  if (length % blocksize != 0)
    {
      BIO_printf (bio_err,
                  "\nRequested encryption length not a multiple of block size "
                  "for %s!\n",
...
static int
run_benchmark (int async_jobs, int (*loop_function) (void *),
               loopargs_t *loopargs)
{
  int job_op_count = 0;
...
speed_main (int argc, char **argv)
{
  ...
  openssl_speed_sec_t seconds
      = { SECONDS,      RSA_SECONDS,   DSA_SECONDS, ECDSA_SECONDS,
          ECDH_SECONDS, EdDSA_SECONDS, SM2_SECONDS, FFDH_SECONDS,
  ...
  for (idx = 0; idx < (unsigned int)sk_EVP_SIGNATURE_num (sig_stack); idx++)
    {
      ...
      /* skipping these algs as tested elsewhere - and b/o setup is a pain */
               ...
               && strcmp (sig_name, "CMAC") && strcmp (sig_name, "SM2"))
        { /* skip alg */
      ...
      for (i = 0; i < loopargs_len; i++)
        {
          ...
          if (EVP_PKEY_sign_init (sig_sign_ctx) <= 0
              && (alg == NULL
                  || EVP_PKEY_sign_message_init (sig_sign_ctx, alg, NULL)
                         <= 0))
            {
              ERR_clear_last_mark ();
              BIO_printf (
          ...
          ERR_pop_to_mark ();
          if (use_params == 1
              && EVP_PKEY_CTX_set_rsa_padding (sig_sign_ctx, RSA_PKCS1_PADDING)
                     <= 0)
            {
              BIO_printf (bio_err,
          ...
          if (EVP_PKEY_sign (sig_sign_ctx, NULL, &max_sig_len, md, md_len)
              <= 0)
            {
              BIO_printf (
                  bio_err,
          ...
          if (EVP_PKEY_verify_init (sig_verify_ctx) <= 0
              && (alg == NULL
                  || EVP_PKEY_verify_message_init (sig_verify_ctx, alg, NULL)
                         <= 0))
            {
              ERR_clear_last_mark ();
              BIO_printf (
...
static void
pkey_print_message (const char *str, const char *str2, unsigned int bits,
                    int tm)
{
  BIO_printf (bio_err,
...
static void
multiblock_speed (const EVP_CIPHER *evp_cipher, int lengths_single,
                  const openssl_speed_sec_t *seconds)
{
      ...
      for (count = 0; run && COND (count); count++)
        {
          ...
          if (packlen > 0)
          ...
          else
            {
              ...
              ciph_success
                  = EVP_Cipher (ctx, out, inp, (unsigned int)(len + pad));
            }
        }
      d = Time_F (STOP);
      BIO_printf (bio_err, mr ? "+R:%d:%s:%f\n" : "%d %s ops in %.2fs\n",
