...
static int have_md(const char* name) {
  ...
  if (opt_md_silent(name, &md)) {
    ...
    if (ctx != NULL && EVP_DigestInit(ctx, md) > 0) ret = 1;
    EVP_MD_CTX_free(ctx);
    EVP_MD_free(md);
  }
  return ret;
...
static int EVP_Digest_loop(const char* mdname, ossl_unused int algindex,
  ...
  if (!opt_md_silent(mdname, &md)) return -1;
  if (EVP_MD_xof(md)) {
    ctx = EVP_MD_CTX_new();
    if (ctx == NULL) {
      count = -1;
...
static int mac_setup(const char* name, EVP_MAC** mac, OSSL_PARAM params[],
  ...
  *mac = EVP_MAC_fetch(app_get0_libctx(), name, app_get0_propq());
  ...
  for (i = 0; i < loopargs_len; i++) {
    ...
    if (loopargs[i].mctx == NULL) return 0;
...
static int GHASH_loop(void* args) {
  ...
  for (count = 0; COND(c[D_GHASH][testnum]); count++) {
    if (!EVP_MAC_update(mctx, buf, lengths[testnum])) return -1;
  }
  return count;
}
...
static EVP_CIPHER_CTX* init_evp_cipher_ctx(const char* ciphername,
  ...
  if (!opt_cipher_silent(ciphername, &cipher)) return NULL;

  if ((ctx = EVP_CIPHER_CTX_new()) == NULL) goto end;
...
static int EVP_Update_loop(void* args) {
  ...
  if (rc == 0) BIO_printf(bio_err, "Error finalizing cipher loop\n");
  return count;
}
...
static int EVP_Update_loop_aead_enc(void* args) {
  ...
  for (count = 0; COND(c[D_EVP][testnum]); count++) {
    ...
    if (EVP_EncryptFinal_ex(ctx, buf, &outl)) realcount++;
  }
  return realcount;
}
...
static int EVP_Update_loop_aead_dec(void* args) {
  ...
  for (count = 0; COND(c[D_EVP][testnum]); count++) {
    ...
    if (EVP_DecryptFinal_ex(ctx, outbuf, &outl)) realcount++;
  }
  return realcount;
}
...
static int run_benchmark(int async_jobs, int (*loop_function)(void*),
  ...
  while (num_inprogress > 0) {
    ...
    for (i = 0; i < async_jobs && num_inprogress > 0; i++) {
      if (loopargs[i].inprogress_job == NULL) continue;
      ...
      if (job_fd > max_fd) max_fd = job_fd;
    }
    ...
    for (i = 0; i < async_jobs; i++) {
      ...
      if (num_job_fds == 1 && !FD_ISSET(job_fd, &waitfdset)) continue;
...
int speed_main(int argc, char** argv) {
      ...
      case OPT_ASYNCJOBS:
        ...
        if (!ASYNC_is_capable()) {
          BIO_printf(bio_err,
                     ...
                     prog);
          if (testmode) /* Return success in the testmode. */
            return 0;
          goto opterr;
        }
#endif
...
#ifndef NO_FORK
  if (multi && do_multi(multi, size_num)) goto show_res;
#endif
  ...
  if (argc == 0 && !doit[D_EVP] && !doit[D_HMAC] && !doit[D_EVP_CMAC] &&
    ...
    for (i = D_MD2; i <= D_WHIRLPOOL; i++) {
      if (!have_md(names[i])) doit[i] = 0;
    }
    for (i = D_CBC_DES; i <= D_CBC_256_CML; i++) {
      if (!have_cipher(names[i])) doit[i] = 0;
    }
  ...
  if (doit[D_HMAC]) {
    ...
    if (evp_mac_mdname == NULL) goto end;
    ...
    if (mac_setup("HMAC", &mac, params, loopargs, loopargs_len) < 1) goto end;
    for (testnum = 0; testnum < size_num; testnum++) {
  ...
  if (doit[D_CBC_DES]) {
    ...
    for (testnum = 0; st && testnum < size_num; testnum++) {
      if (!check_block_size(loopargs[0].ctx, lengths[testnum])) break;
  ...
  if (doit[D_GHASH]) {
    ...
    if (mac_setup("GMAC", &mac, params, loopargs, loopargs_len) < 1) goto end;
    /* b/c of the definition of GHASH_loop(), init() calls are needed here */
    for (i = 0; i < loopargs_len; i++) {
      if (!EVP_MAC_init(loopargs[i].mctx, NULL, 0, NULL)) goto end;
    }
    for (testnum = 0; testnum < size_num; testnum++) {
  ...
  if (doit[D_EVP_CMAC]) {
    ...
    if (!opt_cipher(evp_mac_ciphername, &cipher)) goto end;
    ...
    if (mac_setup("CMAC", &mac, params, loopargs, loopargs_len) < 1) goto end;
    for (testnum = 0; testnum < size_num; testnum++) {
  ...
  for (i = 0; i < loopargs_len; i++)
    if (RAND_bytes(loopargs[i].buf, 36) <= 0) goto end;

  for (testnum = 0; testnum < RSA_NUM; testnum++) {
    EVP_PKEY* rsa_key = NULL;
    int st = 0;

    if (!rsa_doit[testnum]) continue;
    ...
    if (primes > RSA_DEFAULT_PRIME_NUM) {
      ...
      st = bn != NULL && BN_set_word(bn, RSA_F4) &&
           ...
           EVP_PKEY_CTX_set_rsa_keygen_bits(genctx, rsa_keys[testnum].bits) >
               0 &&
  ...
  for (testnum = 0; testnum < ECDSA_NUM; testnum++) {
    ...
    if (!ecdsa_doit[testnum]) continue;
  ...
  for (testnum = 0; testnum < EdDSA_NUM; testnum++) {
    ...
    if (!eddsa_doit[testnum]) continue; /* Ignore Curve */
    for (i = 0; i < loopargs_len; i++) {
  ...
  for (testnum = 0; testnum < SM2_NUM; testnum++) {
    ...
    if (!sm2_doit[testnum]) continue; /* Ignore Curve */
    ...
    if (st == 0) {
      ...
      if (st == 0) {
        ...
        BIO_printf(bio_err,
                   mr ? "+R12:%ld:%u:%s:%.2f\n"
                      : "%ld %u bits %s sign ops in %.2fs \n",
                   count, sm2_curves[testnum].bits, sm2_curves[testnum].name,
                   d);
      ...
      if (op_count <= 1) {
        /* if longer than 10s, don't do any more */
        for (testnum++; testnum < SM2_NUM; testnum++) sm2_doit[testnum] = 0;
      }
    }
  }
#endif /* OPENSSL_NO_SM2 */
  ...
  for (testnum = 0; testnum < kems_algs_len; testnum++) {
    ...
    if (!kems_doit[testnum] || !do_kems) continue;
  ...
  for (testnum = 0; testnum < sigs_algs_len; testnum++) {
    ...
    if (!sigs_doit[testnum] || !do_sigs) continue;
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (sig_sign_ctx == NULL) {
        BIO_printf(bio_err, "Error while initializing signing ctx for %s.\n",
                   sig_name);
        goto sig_err_break;
      }
      ERR_set_mark();
      ...
      if (use_params == 1 &&
        ...
        BIO_printf(bio_err, "Error while initializing padding for %s.\n",
                   sig_name);
        goto sig_err_break;
      }
      ...
      if (sig_verify_ctx == NULL) {
        BIO_printf(bio_err, "Error while initializing verify ctx for %s.\n",
                   sig_name);
        goto sig_err_break;
      }
      ERR_set_mark();
    ...
    if (op_count <= 1) stop_it(sigs_doit, testnum);
  }
...
show_res:
  ...
  for (k = 0; k < RSA_NUM; k++) {
    ...
    if (testnum && !mr) {
      printf(
          "%19ssign    verify    encrypt   decrypt   sign/s verify/s  encr./s  "
          "decr./s\n",
          " ");
      testnum = 0;
    }
  ...
  for (k = 0; k < kems_algs_len; k++) {
    ...
    if (!kems_doit[k] || !do_kems) continue;
    if (testnum && !mr) {
  ...
  for (k = 0; k < sigs_algs_len; k++) {
    ...
    if (!sigs_doit[k] || !do_sigs) continue;
    if (testnum && !mr) {
  ...
  if (ret == 0 && testmode) ret = testmoderesult;
  ERR_print_errors(bio_err);
  for (i = 0; i < loopargs_len; i++) {
    ...
    for (k = 0; k < FFDH_NUM; k++) EVP_PKEY_CTX_free(loopargs[i].ffdh_ctx[k]);
#endif
#ifndef OPENSSL_NO_DSA
    for (k = 0; k < DSA_NUM; k++) {
  ...
  if (kem_stack != NULL) sk_EVP_KEM_pop_free(kem_stack, EVP_KEM_free);
