...
WINBASEAPI
BOOL WINAPI
VirtualLock(_In_ LPVOID lpAddress, _In_ SIZE_T dwSize);
#endif
...
Time_F(int s);
static void
print_message(const char* s, int length, int tm);
static void
pkey_print_message(const char* str,
                   const char* str2,
                   unsigned int bits,
                   int sec);
static void
kskey_print_message(const char* str, const char* str2, int tm);
static void
print_result(int alg, int run_no, int count, double time_used);
#ifndef NO_FORK
static int
do_multi(int multi, int size_num);
#endif
...
static void
multiblock_speed(const EVP_CIPHER* evp_cipher,
                 int lengths_single,
                 const openssl_speed_sec_t* seconds);

static int
opt_found(const char* name,
          unsigned int* result,
          const OPT_PAIR pairs[],
          unsigned int nbelem)
{
  unsigned int idx;
...
static int
EVP_Digest_loop(const char* mdname, ossl_unused int algindex, void* args)
{
...
static void
mac_teardown(EVP_MAC** mac, loopargs_t* loopargs, unsigned int loopargs_len)
{
  unsigned int i;
...
static int
EVP_MAC_loop(ossl_unused int algindex, void* args)
{
  loopargs_t* tempargs = *(loopargs_t**)args;
  unsigned char* buf = tempargs->buf;
...
static int
KMAC128_loop(void* args)
{
  return EVP_MAC_loop(D_KMAC128, args);
}

static int
KMAC256_loop(void* args)
{
 ...
 */
static int
EVP_Update_loop_aead_enc(void* args)
{
  loopargs_t* tempargs = *(loopargs_t**)args;
  unsigned char* buf = tempargs->buf;
  ...
  for (count = 0; COND(c[D_EVP][testnum]); count++) {
    ...
    if (mode_op != EVP_CIPH_SIV_MODE) {
      if (!EVP_CIPHER_CTX_ctrl(
            ctx, EVP_CTRL_AEAD_SET_IVLEN, sizeof(aead_iv), NULL)) {
 ...
 */
static int
EVP_Update_loop_aead_dec(void* args)
{
  loopargs_t* tempargs = *(loopargs_t**)args;
  unsigned char* buf = tempargs->buf;
...
RSA_decrypt_loop(void* args)
{
  ...
  for (count = 0; COND(rsa_c[testnum][3]); count++) {
    ...
    ret = EVP_PKEY_decrypt(
      rsa_decrypt_ctx[testnum], buf, &rsa_num, buf2, tempargs->encsize);
    if (ret <= 0) {
...
static int
FFDH_derive_key_loop(void* args)
{
  loopargs_t* tempargs = *(loopargs_t**)args;
...
static int
ECDH_EVP_derive_key_loop(void* args)
{
  loopargs_t* tempargs = *(loopargs_t**)args;
...
SM2_sign_loop(void* args)
{
  ...
  for (count = 0; COND(sm2_c[testnum][0]); count++) {
    ...
    if (!EVP_DigestSignInit(
          sm2ctx[testnum], NULL, EVP_sm3(), NULL, sm2_pkey[testnum])) {
...
static int
check_block_size(EVP_CIPHER_CTX* ctx, int length)
{
...
static int
run_benchmark(int async_jobs, int (*loop_function)(void*), loopargs_t* loopargs)
{
  int job_op_count = 0;
...
static EVP_PKEY*
get_ecdsa(const EC_CURVE* curve)
{
...
static int
kems_cmp(const EVP_KEM* const* a, const EVP_KEM* const* b)
{
...
static void
collect_kem(EVP_KEM* kem, void* stack)
{
...
static int
kem_locate(const char* algo, unsigned int* idx)
{
  unsigned int i;
...
static int
signatures_cmp(const EVP_SIGNATURE* const* a, const EVP_SIGNATURE* const* b)
{
...
static void
collect_signatures(EVP_SIGNATURE* sig, void* stack)
{
...
static int
sig_locate(const char* algo, unsigned int* idx)
{
  unsigned int i;
...
static int
get_max(const uint8_t doit[], size_t algs_len)
{
  size_t i = 0;
  int maxcnt = 0;
...
speed_main(int argc, char** argv)
{
  ...
  openssl_speed_sec_t seconds = { SECONDS,       RSA_SECONDS,  DSA_SECONDS,
                                  ...
                                  SM2_SECONDS,   FFDH_SECONDS, KEM_SECONDS,
                                  SIG_SECONDS };
    ...
    switch (o) {
      ...
      case OPT_SECONDS:
        seconds.sym = seconds.rsa = seconds.dsa = seconds.ecdsa = seconds.ecdh =
          seconds.eddsa = seconds.sm2 = seconds.ffdh = seconds.kem =
      ...
      case OPT_MLOCK:
        ...
        BIO_printf(
          bio_err, "%s: -mlock not supported on this platform\n", prog);
        goto end;
#endif
        break;
      case OPT_TESTMODE:
  ...
  EVP_SIGNATURE_do_all_provided(
    app_get0_libctx(), collect_signatures, sig_stack);
  ...
  for (idx = 0; idx < (unsigned int)sk_EVP_SIGNATURE_num(sig_stack); idx++) {
    ...
    else if (strncmp(sig_name, "RSA", 3) && strncmp(sig_name, "DSA", 3) &&
             strncmp(sig_name, "ED25519", 7) && strncmp(sig_name, "ED448", 5) &&
             strncmp(sig_name, "ECDSA", 5) && strcmp(sig_name, "HMAC") &&
        ...
        for (k = 0; k < loopargs_len; k++) {
          ...
          if (!ae_mode) {
            if (!EVP_CipherInit_ex(
                  loopargs[k].ctx, NULL, NULL, loopargs[k].key, iv, -1)) {
          ...
          if (ae_mode && decrypt) {
            ...
            if (mode_op != EVP_CIPH_GCM_MODE && mode_op != EVP_CIPH_SIV_MODE &&
                mode_op != EVP_CIPH_GCM_SIV_MODE) {
              if (!EVP_CIPHER_CTX_ctrl(
                    loopargs[k].ctx, EVP_CTRL_AEAD_SET_TAG, TAG_LEN, NULL)) {
                BIO_printf(bio_err, "\nFailed to set tag length\n");
                dofail();
                exit(1);
              }
            ...
            if (!EVP_CipherInit_ex(
                  loopargs[k].ctx, NULL, NULL, loopargs[k].key, aead_iv, -1)) {
              BIO_printf(bio_err, "\nFailed to set the key\n");
              dofail();
              exit(1);
            }
            ...
            if (mode_op == EVP_CIPH_CCM_MODE) {
              if (!EVP_EncryptUpdate(
                    loopargs[k].ctx, NULL, &outlen, NULL, lengths[testnum])) {
                BIO_printf(bio_err, "\nCouldn't set input text length\n");
                dofail();
                exit(1);
              }
            ...
            if (aead) {
              if (!EVP_EncryptUpdate(
                    loopargs[k].ctx, NULL, &outlen, aad, sizeof(aad))) {
                BIO_printf(bio_err, "\nCouldn't insert AAD when encrypting\n");
                dofail();
                exit(1);
              }
            ...
            if (!EVP_EncryptFinal_ex(
                  loopargs[k].ctx, loopargs[k].buf, &outlen)) {
            ...
            if (!EVP_CipherInit_ex(
                  loopargs[k].ctx, evp_cipher, NULL, NULL, NULL, 0)) {
  ...
  for (testnum = 0; testnum < RSA_NUM; testnum++) {
    ...
    if (primes > RSA_DEFAULT_PRIME_NUM) {
      ...
      st = (rsa_key = d2i_PrivateKey(
              EVP_PKEY_RSA, NULL, &p, rsa_keys[testnum].length)) != NULL;
    }
    ...
    if (!st) {
      ...
      pkey_print_message(
        "private", "rsa sign", rsa_keys[testnum].bits, seconds.rsa);
    ...
    if (!st) {
      ...
      pkey_print_message(
        "public", "rsa verify", rsa_keys[testnum].bits, seconds.rsa);
      Time_F(START);
    ...
    if (!st) {
      ...
      pkey_print_message(
        "public", "rsa encrypt", rsa_keys[testnum].bits, seconds.rsa);
      /* RSA_blinding_on(rsa_key[testnum],NULL); */
      Time_F(START);
    ...
    if (!st) {
      ...
      pkey_print_message(
        "private", "rsa decrypt", rsa_keys[testnum].bits, seconds.rsa);
      /* RSA_blinding_on(rsa_key[testnum],NULL); */
      Time_F(START);
  ...
  for (testnum = 0; testnum < ECDSA_NUM; testnum++) {
    ...
    if (!st) {
      ...
      pkey_print_message(
        "sign", "ecdsa", ec_curves[testnum].bits, seconds.ecdsa);
      Time_F(START);
    ...
    if (!st) {
      ...
      pkey_print_message(
        "verify", "ecdsa", ec_curves[testnum].bits, seconds.ecdsa);
      Time_F(START);
  ...
  for (testnum = 0; testnum < EdDSA_NUM; testnum++) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (!EVP_DigestSignInit(
            loopargs[i].eddsa_ctx[testnum], NULL, NULL, NULL, ed_pkey)) {
        st = 0;
        EVP_PKEY_free(ed_pkey);
        break;
      }
      if (!EVP_DigestVerifyInit(
            loopargs[i].eddsa_ctx2[testnum], NULL, NULL, NULL, ed_pkey)) {
        st = 0;
        EVP_PKEY_free(ed_pkey);
        break;
      }
  ...
  for (testnum = 0; testnum < SM2_NUM; testnum++) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (!EVP_DigestSignInit(
            loopargs[i].sm2_ctx[testnum], NULL, EVP_sm3(), NULL, sm2_pkey))
        break;
      if (!EVP_DigestVerifyInit(
            loopargs[i].sm2_vfy_ctx[testnum], NULL, EVP_sm3(), NULL, sm2_pkey))
        break;
  ...
  for (testnum = 0; testnum < FFDH_NUM; testnum++) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (CRYPTO_memcmp(
            loopargs[i].secret_ff_a, loopargs[i].secret_ff_b, secret_size)) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (kem_encaps_ctx == NULL ||
          ...
          EVP_PKEY_encapsulate(
            kem_encaps_ctx, NULL, &out_len, NULL, &send_secret_len) <= 0) {
        BIO_printf(bio_err,
      ...
      if (EVP_PKEY_encapsulate(
            kem_encaps_ctx, out, &out_len, send_secret, &send_secret_len) <=
          0) {
      ...
      if (kem_decaps_ctx == NULL ||
          ...
          EVP_PKEY_decapsulate(
            kem_decaps_ctx, NULL, &rcv_secret_len, out, out_len) <= 0) {
        BIO_printf(bio_err,
      ...
      if (EVP_PKEY_decapsulate(
            kem_decaps_ctx, rcv_secret, &rcv_secret_len, out, out_len) <= 0 ||
  ...
  for (testnum = 0; testnum < sigs_algs_len; testnum++) {
    ...
    for (i = 0; i < loopargs_len; i++) {
      ...
      if (sig_sign_ctx == NULL) {
        BIO_printf(
          bio_err, "Error while initializing signing ctx for %s.\n", sig_name);
        goto sig_err_break;
      }
      ERR_set_mark();
      ...
      if (use_params == 1 &&
        ...
        BIO_printf(
          bio_err, "Error while initializing padding for %s.\n", sig_name);
        goto sig_err_break;
      }
      ...
      if (sig_verify_ctx == NULL) {
        BIO_printf(
          bio_err, "Error while initializing verify ctx for %s.\n", sig_name);
        goto sig_err_break;
      }
      ERR_set_mark();
...
static void
pkey_print_message(const char* str, const char* str2, unsigned int bits, int tm)
{
  BIO_printf(bio_err,
...
static int
strtoint(const char* str, const int min_val, const int upper_val, int* res)
{
  char* end = NULL;
  long int val = 0;
                 ...
                 const openssl_speed_sec_t* seconds)
{
  ...
  if (EVP_CIPHER_CTX_ctrl(
        ctx, EVP_CTRL_AEAD_SET_MAC_KEY, sizeof(no_key), no_key) <= 0)
  ...
  for (j = 0; j < num; j++) {
    ...
    for (count = 0; run && COND(count); count++) {
      ...
      packlen = EVP_CIPHER_CTX_ctrl(
        ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_AAD, sizeof(mb_param), &mb_param);
      ...
      if (packlen > 0) {
        ...
        (void)EVP_CIPHER_CTX_ctrl(
          ctx, EVP_CTRL_TLS1_1_MULTIBLOCK_ENCRYPT, sizeof(mb_param), &mb_param);
      } else {
        int pad;
        ...
        pad = EVP_CIPHER_CTX_ctrl(
          ctx, EVP_CTRL_AEAD_TLS1_AAD, EVP_AEAD_TLS1_AAD_LEN, aad);
