...
load_content_info (int informat, BIO *in, int flags, BIO **indata,
                   const char *name)
{
...
cms_main (int argc, char **argv)
{
          ...
          if (o == OPT_RECIP_KDF)
            {
              if (recip_opt->kdf != NULL)
                {
                  BIO_puts (bio_err,
                            "Illegal multiple -recip_kdf for one -recip\n");
                  goto end;
                }
          ...
          else
            {
              if (recip_opt->ukm_data != NULL)
                {
                  BIO_puts (bio_err,
                            "Illegal multiple -recip_ukm for one -recip\n");
                  goto end;
                }
              recip_opt->ukm_data = OPENSSL_hexstr2buf (
                  opt_arg (), &recip_opt->ukm_data_length);
              if (recip_opt->ukm_data == NULL)
                {
  ...
  if ((operation & SMIME_SIGNERS) == 0)
    {
      if ((flags & CMS_DETACHED) == 0)
        BIO_printf (bio_err, "Warning: -nodetach option is ignored for "
  ...
  if (operation != SMIME_ENCRYPT && recip_first != NULL)
    BIO_printf (bio_err, "Warning: -recip_kdf and -recip_ukm parameters "
                         "ignored for operation other than -encrypt\n");
  ...
  if (recipfile != NULL && (operation == SMIME_DECRYPT)
      && (recip
          = load_cert (recipfile, FORMAT_UNDEF, "recipient certificate file"))
             == NULL)
    goto end;
