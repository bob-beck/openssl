...
static void
receipt_request_print(CMS_ContentInfo* cms);
...
static cms_recip_opt*
alloc_recip_opt(int recipidx)
{
  cms_recip_opt* opt;
...
cms_main(int argc, char** argv)
{
  ...
  if (recipfile != NULL && (operation == SMIME_DECRYPT) &&
      (recip = load_cert(
         recipfile, FORMAT_UNDEF, "recipient certificate file")) == NULL)
    goto end;

  if (originatorfile != NULL &&
      (originator = load_cert(
         originatorfile, FORMAT_UNDEF, "originator certificate file")) == NULL)
    goto end;

  if (operation == SMIME_SIGN_RECEIPT &&
      (signer = load_cert(
         signerfile, FORMAT_UNDEF, "receipt signer certificate file")) == NULL)
    goto end;
  ...
  if ((operation == SMIME_VERIFY) || (operation == SMIME_VERIFY_RECEIPT)) {
    if ((store = setup_verify(
           CAfile, noCAfile, CApath, noCApath, CAstore, noCAstore)) == NULL)
      goto end;
  ...
  if (operation == SMIME_DATA_CREATE) {
    ...
    for (i = 0; i < sk_X509_num(encerts); i++) {
      ...
      res = EVP_PKEY_CTX_ctrl(
        pctx, -1, -1, EVP_PKEY_CTRL_CIPHER, EVP_CIPHER_get_nid(cipher), NULL);
      if (res <= 0 && res != -2)
        goto end;
      ...
      if (ropt != NULL && ri_type == CMS_RECIPINFO_KEM) {
        if (ropt->ukm_data != NULL) {
          if (!CMS_RecipientInfo_kemri_set_ukm(
                ri, ropt->ukm_data, (int)ropt->ukm_data_length))
            goto end;
        }
        if (ropt->kdf != NULL) {
          X509_ALGOR* kdf_algo;
    ...
    if (pwri_pass != NULL) {
      ...
      if (CMS_add0_recipient_password(
            cms, -1, NID_undef, NID_undef, pwri_tmp, -1, kekcipher) == NULL)
        goto end;
      pwri_tmp = NULL;
    }
    if (!(flags & CMS_STREAM)) {
    ...
    for (i = 0; i < sk_OPENSSL_STRING_num(sksigners); i++) {
      ...
      if (si == NULL) {
        BIO_printf(
          bio_err, "Error adding SignerInfo with key from %s\n", keyfile);
        goto end;
      }
  ...
  if (operation == SMIME_DECRYPT) {
    ...
    if (secret_key != NULL) {
      if (!CMS_decrypt_set1_key(
            cms, secret_key, secret_keylen, secret_keyid, secret_keyidlen)) {
    ...
    if (!CMS_EncryptedData_decrypt(
          cms, secret_key, secret_keylen, indata, out, flags))
      goto end;
...
static void
gnames_stack_print(STACK_OF(GENERAL_NAMES)* gns)
{
                     ...
                     STACK_OF(OPENSSL_STRING)* rr_from)
{
  ...
  rr = CMS_ReceiptRequest_create0_ex(
    NULL, -1, rr_allorfirst, rct_from, rct_to, app_get0_libctx());
  if (rr == NULL)
    goto err;
  return rr;
err:
