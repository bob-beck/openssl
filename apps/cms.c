...
const OPTIONS cms_options[] = {
    ...
    {"wrap", OPT_WRAP, 's', "Key wrap algorithm to use when encrypting with key agreement or key encapsulation"},
    ...
    while ((o = opt_next()) != OPT_EOF)
    {
        ...
        case OPT_RECIP_UKM:
            ...
            else
            {
                ...
                recip_opt->ukm_data = OPENSSL_hexstr2buf(opt_arg(), &recip_opt->ukm_data_length);
                if (recip_opt->ukm_data == NULL)
                {
    ...
    else if (operation == SMIME_ENCRYPT)
    {
        ...
        for (i = 0; i < sk_X509_num(encerts); i++)
        {
            ...
            if (ropt != NULL && ri_type == CMS_RECIPINFO_KEM)
            {
                ...
                {
                    if (!CMS_RecipientInfo_kemri_set_ukm(ri, ropt->ukm_data, (int)ropt->ukm_data_length))
                        goto end;
                }
                if (ropt->kdf != NULL)
                {
