...
const OPTIONS cms_options[] = {
    ...
    {"wrap", OPT_WRAP, 's',
     "Key wrap algorithm to use when encrypting with key agreement or key "
     "encapsulation"},
    ...
    {"recip_kdf", OPT_RECIP_KDF, 's',
     "Set KEMRecipientInfo KDF for current recipient"},
    {"recip_ukm", OPT_RECIP_UKM, 's',
     "KEMRecipientInfo user keying material for current recipient, in hex "
     "notation"},
    ...
    {OPT_MORE_STR, 0, 0,
     "or to use as preferred signer certs and for chain building when "
     "verifying"},
    {"cades", OPT_CADES, '-',
...
int cms_main(int argc, char** argv) {
  ...
  if (encerts == NULL || vpm == NULL) goto end;
  ...
  while ((o = opt_next()) != OPT_EOF) {
    switch (o) {
      ...
      case OPT_HELP:
        ...
        goto end;
      case OPT_INFORM:
        if (!opt_format(opt_arg(), OPT_FMT_PDS, &informat)) goto opthelp;
        break;
      case OPT_OUTFORM:
        if (!opt_format(opt_arg(), OPT_FMT_PDS, &outformat)) goto opthelp;
        break;
      case OPT_OUT:
        outfile = opt_arg();
        break;
      ...
      case OPT_RR_FROM:
        if (rr_from == NULL && (rr_from = sk_OPENSSL_STRING_new_null()) == NULL)
          goto end;
        if (sk_OPENSSL_STRING_push(rr_from, opt_arg()) <= 0) goto end;
        break;
      case OPT_RR_TO:
        if (rr_to == NULL && (rr_to = sk_OPENSSL_STRING_new_null()) == NULL)
          goto end;
        if (sk_OPENSSL_STRING_push(rr_to, opt_arg()) <= 0) goto end;
        break;
      case OPT_PRINT:
        noout = print = 1;
        break;
      ...
      case OPT_SIGNER:
        ...
        if (signerfile != NULL) {
          if (sksigners == NULL &&
            ...
            goto end;
          if (sk_OPENSSL_STRING_push(sksigners, signerfile) <= 0) goto end;
          if (keyfile == NULL) keyfile = signerfile;
          if (skkeys == NULL && (skkeys = sk_OPENSSL_STRING_new_null()) == NULL)
            goto end;
          if (sk_OPENSSL_STRING_push(skkeys, keyfile) <= 0) goto end;
          keyfile = NULL;
        }
        signerfile = opt_arg();
        break;
      ...
      case OPT_RECIP:
        if (operation == SMIME_ENCRYPT) {
          cert =
              load_cert(opt_arg(), FORMAT_UNDEF, "recipient certificate file");
          if (cert == NULL) goto end;
          if (!sk_X509_push(encerts, cert)) goto end;
          cert = NULL;
        } else {
          recipfile = opt_arg();
        }
      ...
      case OPT_RECIP_UKM:
        recipidx = -1;
        if (operation == SMIME_ENCRYPT) {
          if (sk_X509_num(encerts) > 0) recipidx += sk_X509_num(encerts);
        }
        if (recipidx < 0) {
        ...
        if (o == OPT_RECIP_KDF) {
          ...
          recip_opt->ukm_data =
              OPENSSL_hexstr2buf(opt_arg(), &recip_opt->ukm_data_length);
          if (recip_opt->ukm_data == NULL) {
      ...
      case OPT_KEYOPT:
        keyidx = -1;
        if (operation == SMIME_ENCRYPT) {
          if (sk_X509_num(encerts) > 0) keyidx += sk_X509_num(encerts);
        } else {
          if (keyfile != NULL || signerfile != NULL) keyidx++;
          if (skkeys != NULL) keyidx += sk_OPENSSL_STRING_num(skkeys);
        }
        if (keyidx < 0) {
        ...
        if (sk_OPENSSL_STRING_push(key_param->param, opt_arg()) <= 0) goto end;
        break;
      case OPT_V_CASES:
  ...
  if (digestname != NULL) {
    if (!opt_md(digestname, &sign_md)) goto end;
  }
  if (!opt_cipher_any(ciphername, &cipher)) goto end;
  if (kekciphername != NULL && !opt_cipher_any(kekciphername, &kekcipher))
    goto end;
  if (wrapname != NULL) {
    if (!opt_cipher_any(wrapname, &wrap_cipher)) goto end;
  }
  ...
  if (operation != SMIME_ENCRYPT && *argv != NULL)
    BIO_printf(bio_err,
               "Warning: recipient certificate file parameters ignored for "
               "operation other than -encrypt\n");
  if (operation != SMIME_ENCRYPT && recip_first != NULL)
    BIO_printf(bio_err,
               "Warning: -recip_kdf and -recip_ukm parameters ignored for "
               "operation other than -encrypt\n");

  if ((flags & CMS_BINARY) != 0) {
    if (!(operation & SMIME_OP)) outformat = FORMAT_BINARY;
    if (!(operation & SMIME_IP)) informat = FORMAT_BINARY;
    if ((operation & SMIME_SIGNERS) != 0 && (flags & CMS_DETACHED) != 0)
      binary_files = 1;
    if ((operation & SMIME_IP) != 0 && contfile == NULL) binary_files = 1;
  }

  if (operation == SMIME_ENCRYPT) {
    if (!cipher) cipher = (EVP_CIPHER*)EVP_aes_256_cbc();
  ...
  if ((operation == SMIME_DECRYPT) || (operation == SMIME_ENCRYPT)) {
    if (keyfile == NULL) keyfile = recipfile;
  ...
  if ((operation == SMIME_VERIFY) || (operation == SMIME_VERIFY_RECEIPT)) {
    ...
    if (vpmtouched) X509_STORE_set1_param(store, vpm);
  }
  ...
  if (operation == SMIME_DATA_CREATE) {
    ...
    for (i = 0; i < sk_X509_num(encerts); i++) {
      ...
      if (ropt != NULL && ri_type == CMS_RECIPINFO_KEM) {
        ...
        if (ropt->kdf != NULL) {
          ...
          if (!X509_ALGOR_set0(kdf_algo, kdf_obj, V_ASN1_UNDEF, NULL)) goto end;
        }
      }
    }
    ...
    if (sis == NULL) goto end;
    si = sk_CMS_SignerInfo_value(sis, 0);
    srcms = CMS_sign_receipt(si, signer, key, other, flags);
    if (srcms == NULL) goto end;
    ...
    if (operation == SMIME_SIGN) {
      if ((flags & CMS_DETACHED) != 0 && digestbin == NULL) {
        if (outformat == FORMAT_SMIME) flags |= CMS_STREAM;
      }
      flags |= CMS_PARTIAL;
      ...
      if (econtent_type != NULL) CMS_set1_eContentType(cms, econtent_type);
  ...
  if (operation == SMIME_DECRYPT) {
    ...
    if (!CMS_data(cms, out, flags)) goto end;
  } else if (operation == SMIME_UNCOMPRESS) {
    if (!CMS_uncompress(cms, indata, out, flags)) goto end;
    ...
    if (CMS_verify(cms, other, store, indata, out, flags) > 0) {
      ...
      BIO_printf(bio_err, "%s Verification failure\n",
                 (flags & CMS_CADES) != 0 ? "CAdES" : "CMS");
      if (verify_retcode) ret = verify_err + 32;
      goto end;
    }
    if (signerfile != NULL) {
    ...
    if (rr_print) receipt_request_print(cms);
    ...
    if (noout) {
      ...
      if (to != NULL && BIO_printf(out, "To: %s%s", to, mime_eol) < 0) goto end;
  ...
  if (ret != 0) ERR_print_errors(bio_err);
...
static STACK_OF(GENERAL_NAMES)* make_names_stack(STACK_OF(OPENSSL_STRING)* ns) {
  ...
  for (i = 0; i < sk_OPENSSL_STRING_num(ns); i++) {
    ...
    if (gen == NULL) goto err;
    gens = GENERAL_NAMES_new();
    if (gens == NULL) goto err;
...
static CMS_ReceiptRequest* make_receipt_request(
  ...
  if (rct_to == NULL) goto err;
  if (rr_from != NULL) {
    rct_from = make_names_stack(rr_from);
    if (rct_from == NULL) goto err;
  } else {
    rct_from = NULL;
  }
...
static int cms_set_pkey_param(EVP_PKEY_CTX* pctx,
  ...
  char* keyopt;
  int i;
  if (sk_OPENSSL_STRING_num(param) <= 0) return 1;
