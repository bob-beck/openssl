...
static void
user_data_init(struct user_data_st* user_data,
               SSL* con,
               char* buf,
               size_t bufmax,
               int mode);
static int
user_data_add(struct user_data_st* user_data, size_t i);
static int
user_data_process(struct user_data_st* user_data, size_t* len, size_t* off);
static int
user_data_has_data(struct user_data_st* user_data);
...
static void
print_stuff(BIO* berr, SSL* con, int full);
#ifndef OPENSSL_NO_OCSP
static int
ocsp_resp_cb(SSL* s, void* arg);
static void
print_ocsp_response(BIO* bp, OCSP_RESPONSE* rsp);
#endif
static int
ldap_ExtendedResponse_parse(const char* buf, long rem);
static int
is_dNS_name(const char* host);
              ...
              void* arg)
{
  ...
  ctx->status = SSL_select_next_proto(
    out, outlen, in, inlen, ctx->data, (unsigned int)ctx->len);
  return SSL_TLSEXT_ERR_OK;
}
                        ...
                        void* arg)
{
  ...
  BIO_snprintf(
    pem_name, sizeof(pem_name), "SERVERINFO FOR EXTENSION %d", ext_type);
 ...
 */
static ossl_ssize_t
hexdecode(const char** inptr, void* result)
{
 ...
 */
static ossl_ssize_t
checked_uint8(const char** inptr, void* out)
{
...
static int
tlsa_import_rr(SSL* con, const char* rrdata)
{
  ...
  if (ret == 0) {
    ...
    BIO_printf(
      bio_err, "%s: warning: unusable TLSA rrdata: %s\n", prog, rrdata);
    return 0;
  }
  if (ret < 0) {
    ERR_print_errors(bio_err);
    BIO_printf(
      bio_err, "%s: warning: error loading TLSA rrdata: %s\n", prog, rrdata);
    return 0;
  }
  return ret;
}
...
static int
tlsa_import_rrset(SSL* con, STACK_OF(OPENSSL_STRING)* rrset)
{
...
const OPTIONS s_client_options[] = {
  ...
  { "ocsp_check_leaf",
    OPT_STATUS_OCSP_CHECK_LEAF,
    '-',
    "Require checking leaf certificate status, attempting to use OCSP stapling "
    "first" },
  { "ocsp_check_all",
    OPT_STATUS_OCSP_CHECK_ALL,
    '-',
...
static void
freeandcopy(char** dest, const char* source)
{
  OPENSSL_free(*dest);
  *dest = NULL;
  if (source != NULL)
...
static int
new_session_cb(SSL* s, SSL_SESSION* sess)
{
...
/* Known false-positive of MemorySanitizer. */
  ...
  if (bindstr != NULL) {
    ...
    if (!res) {
      BIO_printf(
        bio_err, "%s: -bind argument parameter malformed or ambiguous\n", prog);
      goto end;
    }
  }
  ...
  if (key_file != NULL) {
    key = load_key(
      key_file, key_format, 0, pass, e, "client certificate private key");
    if (key == NULL)
      goto end;
  }
  ...
  if (dane_tlsa_domain != NULL) {
    if (SSL_CTX_dane_enable(ctx) <= 0) {
      BIO_printf(
        bio_err, "%s: Error enabling DANE TLSA authentication.\n", prog);
      goto end;
    }
  }
...
re_start:
  ...
  if (proxystr != NULL) {
    ...
    if (!OSSL_HTTP_proxy_connect(sbio,
                                 thost,
                                 tport,
                                 proxyuser,
                                 proxypass,
...
static void
print_cert_key_info(BIO* bio, X509* cert)
{
...
print_stuff(BIO* bio, SSL* s, int full)
{
  ...
  if (full) {
    ...
    if (sk != NULL) {
      ...
      for (i = 0; i < sk_X509_num(sk); i++) {
        ...
        X509_NAME_print_ex(
          bio, X509_get_subject_name(chain_cert), 0, get_nameopt());
        BIO_puts(bio, "\n");
        BIO_printf(bio, "   i:");
        X509_NAME_print_ex(
          bio, X509_get_issuer_name(chain_cert), 0, get_nameopt());
        BIO_puts(bio, "\n");
...
#ifndef OPENSSL_NO_SRTP
  {
    ...
    if (srtp_profile)
      BIO_printf(
        bio, "SRTP Extension negotiated, profile=%s\n", srtp_profile->name);
  }
#endif
...
static int
user_data_execute(struct user_data_st* user_data, int cmd, char* arg)
{
  switch (cmd) {
...
user_data_process(struct user_data_st* user_data, size_t* len, size_t* off)
{
  ...
  if (user_data->mode == USER_DATA_MODE_BASIC) {
    switch (buf_start[0]) {
      ...
      case 'k':
        user_data->buflen = user_data->bufoff = *len = *off = 0;
        return user_data_execute(user_data,
      ...
      *cmd_end = '\0';
      ...
      if (cmd == -1) {
        ...
        if (ret == USER_DATA_PROCESS_BAD_ARGUMENT) {
          BIO_printf(
            bio_err, "BAD ARGUMENT (COMMAND IGNORED): %s\n", arg_start);
