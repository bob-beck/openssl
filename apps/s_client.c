...
static void user_data_init (struct user_data_st *user_data, SSL *con,
                            char *buf, size_t bufmax, int mode);
...
static int
psk_use_session_cb (SSL *s, const EVP_MD *md, const unsigned char **id,
...
static void
user_data_init (struct user_data_st *user_data, SSL *con, char *buf,
...
user_data_execute (struct user_data_st *user_data, int cmd, char *arg)
{
  switch (cmd)
    ...
    case USER_COMMAND_HELP:
      ...
      BIO_printf (bio_err, "To issue a command insert {cmd} or {cmd:arg} "
      ...
      if (SSL_is_quic (user_data->con))
      ...
      else if (SSL_version (user_data->con) == TLS1_3_VERSION)
        {
          ...
          BIO_printf (bio_err, "                     req   = peer update "
                               "requested (default)\n");
          BIO_printf (
              bio_err,
