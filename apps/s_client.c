...
static unsigned int psk_client_cb(SSL* ssl, const char* hint, char* identity,
  ...
  if (c_debug) BIO_printf(bio_c_out, "psk_client_cb\n");
  if (!hint) {
  ...
  if (ret < 0 || (unsigned int)ret > max_identity_len) goto out_err;
  if (c_debug)
  ...
  if (c_debug) BIO_printf(bio_c_out, "created PSK len=%ld\n", key_len);

  return key_len;
out_err:
  if (c_debug) BIO_printf(bio_err, "Error in PSK client callback\n");
  return 0;
}
#endif
...
static int psk_use_session_cb(SSL* s, const EVP_MD* md,
  ...
  if (psksess != NULL) {
    if (!SSL_SESSION_up_ref(psksess)) goto err;
    usesess = psksess;
  } else {
    long key_len;
...
static int next_proto_cb(SSL* s, unsigned char** out, unsigned char* outlen,
  ...
  ctx->status = SSL_select_next_proto(out, outlen, in, inlen, ctx->data,
...
static int tlsa_import_rrset(SSL* con, STACK_OF(OPENSSL_STRING)* rrset) {
  ...
  for (i = 0; i < num; ++i) {
    ...
    if (tlsa_import_rr(con, rrdata) > 0) ++count;
  }
  return count > 0;
}
...
const OPTIONS s_client_options[] = {
    ...
    {"ocsp_check_leaf", OPT_STATUS_OCSP_CHECK_LEAF, '-',
     "Require checking leaf certificate status, attempting to use OCSP "
     "stapling first"},
    {"ocsp_check_all", OPT_STATUS_OCSP_CHECK_ALL, '-',
     "Require checking status of full chain, attempting to use OCSP stapling "
     "first"},
#endif
...
static void freeandcopy(char** dest, const char* source) {
  OPENSSL_free(*dest);
  *dest = NULL;
  if (source != NULL) *dest = OPENSSL_strdup(source);
}
...
/* Known false-positive of MemorySanitizer. */
  ...
  while ((o = opt_next()) != OPT_EOF) {
    ...
    switch (o) {
      ...
      case OPT_SESS_IN:
        ...
        break;
      case OPT_CERTFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &cert_format)) goto opthelp;
        break;
      case OPT_CRLFORM:
      ...
      case OPT_FALLBACKSCSV:
        ...
        break;
      case OPT_KEYFORM:
        if (!opt_format(opt_arg(), OPT_FMT_ANY, &key_format)) goto opthelp;
        break;
      case OPT_PASS:
        passarg = opt_arg();
        break;
      ...
      case OPT_SERVERINFO:
        ...
        for (start = 0, i = 0; i <= len; ++i) {
          if (i == len || p[i] == ',') {
            ...
            if (++serverinfo_count == MAX_SI_TYPES) break;
            start = i + 1;
          }
        }
        break;
      case OPT_STARTTLS:
        if (!opt_pair(opt_arg(), services, &starttls_proto)) goto end;
        break;
      case OPT_TFO:
        tfo = 1;
        break;
  ...
  if (c_ign_eof) cmdmode = USER_DATA_MODE_NONE;
  ...
  if (connectstr != NULL) {
    ...
    if (tmp_host != host) OPENSSL_free(tmp_host);
    if (tmp_port != port) OPENSSL_free(tmp_port);
    if (!res) {
      BIO_printf(
          bio_err,
  ...
  if (!set_cert_key_stuff(ctx, cert, key, chain, build_chain)) goto end;
  ...
  con = SSL_new(ctx);
  if (con == NULL) goto end;

  if (enable_pha) SSL_set_post_handshake_auth(con, 1);
  ...
  if (fallback_scsv) SSL_set_mode(con, SSL_MODE_SEND_FALLBACK_SCSV);
  ...
  if (tfo) BIO_printf(bio_c_out, "Connecting via TFO\n");
re_start:
   ...
   */
  if (c_nbio || isquic) {
    if (!BIO_socket_nbio(sock, 1)) goto end;
    if (c_nbio) {
      if (isquic && !SSL_set_blocking_mode(con, 0)) goto end;
  ...
  switch ((PROTOCOL_CHOICE)starttls_proto) {
    ...
    case PROTO_SMTP: {
      ...
      if (protohost == NULL) protohost = "mail.example.com";
      ...
      do {
        mbuf_len = BIO_gets(fbio, mbuf, BUFSIZZ);
        if (strstr(mbuf, "STARTTLS")) foundit = 1;
      } while (mbuf_len > 3 && mbuf[3] == '-');
      (void)BIO_flush(fbio);
      BIO_pop(fbio);
      BIO_free(fbio);
    ...
    case PROTO_XMPP_SERVER: {
      ...
      sbuf[seen] = '\0';
      if (!strstr(sbuf, "<proceed")) goto shut;
      mbuf[0] = '\0';
    } break;
    case PROTO_TELNET: {
      ...
      if (bytes != 3 || memcmp(mbuf, tls_do, 3) != 0) goto shut;
      ...
      bytes = BIO_read(sbio, mbuf, BUFSIZZ);
      if (bytes != 6 || memcmp(mbuf, tls_follows, 6) != 0) goto shut;
    } break;
    case PROTO_IRC: {
      int numeric;
      BIO* fbio = BIO_new(BIO_f_buffer());
      ...
      do {
        ...
        mbuf_len = BIO_gets(fbio, mbuf, BUFSIZZ);
        if (mbuf_len < 1 || sscanf(mbuf, "%*s %d", &numeric) != 1) break;
    ...
    case PROTO_SIEVE: {
      ...
      do {
        ...
        if (mbuf_len > 1 && mbuf[0] == '"') {
          make_uppercase(mbuf);
          if (HAS_PREFIX(mbuf, "\"STARTTLS\"")) foundit = 1;
        }
      } while (mbuf_len > 1 && mbuf[0] == '"');
      (void)BIO_flush(fbio);
      BIO_pop(fbio);
  ...
  if (early_data_file != NULL &&
    ...
    while (!finish) {
      if (!BIO_read_ex(edfile, cbuf, BUFSIZZ, &readbytes)) finish = 1;
  ...
  for (;;) {
    ...
    if (!SSL_is_init_finished(con) && SSL_total_renegotiations(con) == 0 &&
      ...
      if (in_init) {
        ...
        if (starttls_proto) {
          ...
          if (!reconnect) starttls_proto = PROTO_OFF;
        }
    ...
    if (!ssl_pending) {
      ...
      if (tty_on) {
         ...
         */
        if (read_tty && !at_eof) openssl_fdset(fileno_stdin(), &readfds);
#if !defined(OPENSSL_SYS_VMS)
        if (write_tty) openssl_fdset(fileno_stdout(), &writefds);
#endif
      }
      ...
      if (!write_tty) {
        if (read_tty) {
          ...
          if (!i && (!has_stdin_waiting() || !read_tty)) continue;
        } else
...
#endif
    {
      ...
      switch (SSL_get_error(con, k)) {
        ...
        case SSL_ERROR_WANT_ASYNC:
          ...
          write_tty = 0;
          read_ssl = 1;
          if ((read_tty == 0) && (write_ssl == 0)) write_ssl = 1;
          break;
        case SSL_ERROR_WANT_WRITE:
...
shut:
  ...
  if (in_init) print_stuff(bio_c_out, con, full_log);
  do_ssl_shutdown(con);
  ...
  if (ret > 0) ERR_print_errors(bio_err); /* show any new or remaining errors */
  if (con != NULL) {
    if (prexit != 0) print_stuff(bio_c_out, con, 1);
    SSL_free(con);
  }
...
static void print_cert_key_info(BIO* bio, X509* cert) {
  ...
  if (keyalg == NULL) keyalg = OBJ_nid2ln(EVP_PKEY_get_base_id(pkey));
  if (EVP_PKEY_id(pkey) == EVP_PKEY_EC) curve = ec_curve_name(pkey);
  if (curve != NULL)
  ...
  if (full) {
    ...
    if (sk != NULL) {
      ...
      for (i = 0; i < sk_X509_num(sk); i++) {
        ...
        BIO_puts(bio, "\n");
        if (c_showcerts) PEM_write_bio_X509(bio, chain_cert);
      }
    }
    ...
    if (peer != NULL) {
      ...
      if (!(c_showcerts && got_a_chain)) PEM_write_bio_X509(bio, peer);
...
static int user_data_add(struct user_data_st* user_data, size_t i) {
  if (user_data->buflen != 0 || i > user_data->bufmax) return 0;
...
static int user_data_execute(struct user_data_st* user_data, int cmd,
  ...
  switch (cmd) {
    case USER_COMMAND_HELP:
      ...
      BIO_printf(bio_err,
                 "To issue a command insert {cmd} or {cmd:arg} anywhere in the "
                 "text\n");
      ...
      if (SSL_is_quic(user_data->con)) {
        BIO_printf(bio_err,
                   "  {fin}: Send FIN on the stream. No further writing is "
                   "possible\n");
    ...
    case USER_COMMAND_RENEGOTIATE:
      ...
      if (!SSL_renegotiate(user_data->con)) break;
    ...
    case USER_COMMAND_KEY_UPDATE: {
      ...
      if (!SSL_key_update(user_data->con, updatetype)) break;
      return USER_DATA_PROCESS_CONTINUE;
    }

    case USER_COMMAND_FIN:
      if (!SSL_stream_conclude(user_data->con, 0)) break;
      user_data->isfin = 1;
      return USER_DATA_PROCESS_NO_DATA;
