         ...
         */
{
  ...
  {
    EVP_EncodeFinal(
      ctx->base64, (unsigned char*)ctx->buf, &(ctx->len)); /* no decl */
    /* push out the bytes */
    goto again;
  }
  {
  ...
  }
  {
    ...
    if (pass_str != NULL) {
      ...
      res = OSSL_CMP_CTX_set1_secretValue(
        ctx, (unsigned char*)pass_str, strlen(pass_str));
      clear_free(pass_str);
    }
  }
}
...
bool
UTIL_iterate_dir(int (*fn)(const char* file, void* arg),
                 void* arg,
                 const char* path,
                 bool recursive);
size_t
UTIL_url_encode(size_t* size_needed);
size_t
UTIL_url_encode(const char* source,
                char* destination,
...
f() /* struct/union/enum in function return type */
{}
static void
f(struct pem_pass_data* data) /* struct/union/enum in arg list */
{
}
